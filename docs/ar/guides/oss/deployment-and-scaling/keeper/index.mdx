...
<div id="keeper-configuration-settings">
  #### إعدادات تهيئة Keeper {#keeper-configuration-settings}
</div>
...
| `log_readahead_chunk_size`                | عدد إدخالات Log التي تُفك شفرتها لكل фрагمент في مهمة ملء القراءة الاستباقية.                                                                                                                                                                                                                                                    | `16`              |
| `log_readahead_commit_window_bytes`       | الحد الأقصى للحجم الإجمالي لإدخالات Log المفككة الشيفرة والمخزنة مؤقتًا مسبقًا أمام خيط تنفيذ commit. تُعطّل `0` القراءة الاستباقية لعملية commit (تقرأ commit الإدخالات من القرص واحدًا تلو الآخر).                                                                                                                             | `500MiB`          |
...
* `ydld`: طلب التخلي عن القيادة والتحول إلى تابع. إذا كان الخادم الذي يتلقى الطلب هو القائد، فسيُوقف عمليات الكتابة مؤقتًا أولًا، وينتظر حتى يُنهي الخلف (لا يمكن للقائد الحالي أن يكون الخلف أبدًا) استدراك أحدث سجل، ثم يتنحى. سيُختار الخلف تلقائيًا. يُرجع `Sent yield leadership request to leader.` إذا أُرسل الطلب، أو `Failed to send yield leadership request to leader.` إذا لم يُرسل. وإذا كانت العقدة تابعًا بالفعل، تكون النتيجة مماثلة لنتيجة إرسال الطلب.

```response
Sent yield leadership request to leader.
```

* `pfev`: يُرجِع قيم جميع الأحداث المجمّعة. ولكل حدث، يُرجِع اسم الحدث وقيمته ووصفه.
...
<div id="migration-from-zookeeper">
  ### الترحيل من ZooKeeper {#migration-from-zookeeper}
</div>
...
<div id="pre-migration-preparation">
  #### التحضير قبل الترحيل {#pre-migration-preparation}
</div>
...
5. شغّل `clickhouse-keeper-converter` على العقدة القائدة. إذا كان لديك الملف التنفيذي الكامل لـ ClickHouse مثبّتًا، فاستخدم الأمر الفرعي `keeper-converter` بدلًا من ذلك (`clickhouse keeper-converter`). إذا لم يكن أيٌّ منهما متاحًا، [نزّل الملف التنفيذي](/ar/get-started/setup/self-managed/quick-install#install-clickhouse-using-curl).
...
<div id="consolidating-multiple-zookeeper-clusters">
  #### دمج عدة عناقيد ZooKeeper {#consolidating-multiple-zookeeper-clusters}
</div>
...
<div id="handling-encryption-and-acls">
  #### التعامل مع التشفير وقوائم ACL {#handling-encryption-and-acls}
</div>
...
<div id="verifying-the-migration">
  #### التحقق من الترحيل {#verifying-the-migration}
</div>
...
<div id="post-migration-tuning">
  #### الضبط بعد الترحيل {#post-migration-tuning}
</div>
...
<div id="recovering-after-losing-quorum">
  ### الاستعادة بعد فقدان النصاب {#recovering-after-losing-quorum}
</div>
...
<div id="using-disks-with-keeper">
  ## استخدام الأقراص مع Keeper {#using-disks-with-keeper}
</div>
...
<div id="changing-disk-setup">
  ### تغيير إعداد الأقراص {#changing-disk-setup}
</div>
...
يُتحكَّم في حدّ ذاكرة التخزين المؤقت لأحدث السجلات بواسطة:

* `latest_logs_cache_size_threshold` - الحجم الإجمالي لأحدث السجلات المخزنة مؤقتًا

إذا كانت القيمة الافتراضية كبيرة جدًا، يمكنك تقليل استخدام الذاكرة بتقليل هذا الإعداد.

تُقدَّم إدخالات السجل اللازمة لاحقًا لإجراء الالتزام بواسطة قارئ استباقي يفكّ الترميز، ويُحدَّد حجمه بواسطة
...
اللذين يُحتفظ بهما فقط للتوافق مع الإعدادات (لا يزال الأول يُعيَّن إلى `log_readahead_commit_window_bytes` إذا
...
<div id="clickhouse-keeper-user-guide">
  ## دليل المستخدم لـ ClickHouse Keeper {#clickhouse-keeper-user-guide}
</div>
...
<Steps>
  <Step title="هيِّئ العقد باستخدام إعدادات Keeper" id="1-configure-nodes-with-keeper-settings">
...
<div id="configuring-clickhouse-keeper-with-unique-paths">
  ## تهيئة ClickHouse Keeper بمسارات فريدة {#configuring-clickhouse-keeper-with-unique-paths}
</div>
...
<div id="procedures-to-set-up-tables-to-use-uuid">
  ### إجراءات إعداد الجداول لاستخدام `{uuid}` {#procedures-to-set-up-tables-to-use-uuid}
</div>
...
<div id="reconfiguration">
  ## إعادة التهيئة الديناميكية لـ ClickHouse Keeper {#reconfiguration}
</div>
...
<div id="converting-a-single-node-keeper-into-a-cluster">
  ## تحويل Keeper أحادي العقدة إلى عنقود {#converting-a-single-node-keeper-into-a-cluster}
</div>
