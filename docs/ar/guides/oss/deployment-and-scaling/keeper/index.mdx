...
| `log_readahead_chunk_size`                | عدد إدخالات Log التي تُفك شفرتها لكل фрагмент في مهمة ملء القراءة الاستباقية.                                                                                                                                                                                                                                                    | `16`              |
...
| `log_startup_read_max_streams`            | الحد الأقصى لعدد ملفات changelog التي تُقرأ بالتزامن عند بدء تشغيل Keeper. `0` = استخدام عدد أنوية CPU تلقائيًا. `1` = استخدام قراءة بدء التشغيل التسلسلية (قبل التوازي). يُقيَّد التوازي الفعلي بعدد ملفات changelog المطلوب قراءتها؛ فكّر في خفضه عند استخدام تخزين مقيّد بعمليات seek (HDD، ووحدات التخزين المقيّدة بـ IOPS). | `0`               |
| `log_startup_read_buffer_size`            | حجم مخزن القراءة المؤقت لكل stream (بالبايت) المستخدم عند قراءة changelogs أثناء بدء تشغيل Keeper. يجب أن يكون أكبر من `0`. كما يُقيَّد المخزن المؤقت بحجم الملف.                                                                                                                                                                | `8MiB`            |
