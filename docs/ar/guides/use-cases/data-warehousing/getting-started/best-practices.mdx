...
استخدم بديل S3 مع AWS S3 وGCS. ولدى Azure ونظام الملفات المحلي بدائل مخصّصة (`icebergAzure` و`icebergLocal`، وما يقابلهما للتنسيقات الأخرى). راجع [الاستعلام مباشرةً](/ar/guides/use-cases/data-warehousing/getting-started/querying-directly) للاطلاع على القائمة الكاملة.

يوفّر [Paimon](/ar/reference/functions/table-functions/paimon) دوال جداول فقط.
...
بالنسبة إلى اتصالات الكتالوج، لكل نوع من أنواع الكتالوج علامة خاصة به. راجع [الاتصال بالكتالوجات](/ar/guides/use-cases/data-warehousing/getting-started/connecting-catalogs) للحصول على نظرة عامة، و[مرجع DataLakeCatalog](/ar/reference/engines/database-engines/datalake) للاطّلاع على تفاصيل الإعدادات. أما إعداد كل كتالوج على حدة، فتجده في [أدلة الكتالوج](/ar/guides/use-cases/data-warehousing/reference).

بالنسبة إلى عمليات الكتابة، يتطلب Iceberg [allow&#95;insert&#95;into&#95;iceberg](/ar/reference/settings/session-settings#allow_insert_into_iceberg) ‏(25.7+، وBeta بدءًا من 26.2). راجع [الكتابة إلى بحيرات البيانات](/ar/guides/use-cases/data-warehousing/getting-started/writing-data). ويتطلب Delta Lake [allow&#95;delta&#95;lake&#95;writes](/ar/reference/settings/session-settings#allow_experimental_delta_lake_writes) ‏(25.9+). وتوضّح [مصفوفة الدعم](/ar/guides/use-cases/data-warehousing/support-matrix) العلامات التي تنطبق على كل تنسيق وكل عملية.
...
* بالنسبة إلى Iceberg، اقرأ عرضًا عند نقطة زمنية محددة باستخدام [iceberg&#95;snapshot&#95;id](/ar/reference/settings/session-settings#iceberg_snapshot_id) أو [iceberg&#95;timestamp&#95;ms](/ar/reference/settings/session-settings#iceberg_timestamp_ms) (25.4+). وبالنسبة إلى الجداول من نوع append-only، ادمج إعدادات اللقطة مع عوامل تصفية التقسيم في `WHERE`. استخدم [system.iceberg&#95;history](/ar/reference/system-tables/iceberg_history) (25.6+) للعثور على معرّفات اللقطات بين مرات التشغيل.
* بالنسبة إلى Delta Lake، اقرأ التغييرات بين إصدارين باستخدام [delta&#95;lake&#95;snapshot&#95;start&#95;version](/ar/reference/settings/session-settings#delta_lake_snapshot_start_version) و[delta&#95;lake&#95;snapshot&#95;end&#95;version](/ar/reference/settings/session-settings#delta_lake_snapshot_end_version) (25.12+). واقرأ لقطة واحدة باستخدام [delta&#95;lake&#95;snapshot&#95;version](/ar/reference/settings/session-settings#delta_lake_snapshot_version) (25.8+). راجع [Delta change data feed](#delta-incremental-sync) للاطلاع على مثال على CDF.

<div id="filesystem-cache">
  ### تخزين ملفات Parquet مؤقتًا محليًا
</div>

يدعم كل من التنسيقين [enable&#95;filesystem&#95;cache](/ar/reference/settings/session-settings#enable_filesystem_cache) للاحتفاظ بملفات [Parquet](/ar/reference/formats/Parquet/Parquet) كثيرة الاستخدام على القرص المحلي بين الاستعلامات. في بيئات النشر المُدارة ذاتيًا، اضبط [قرص التخزين المؤقت لنظام الملفات](/ar/concepts/features/configuration/server-config/storing-data#using-local-cache) في إعدادات الخادم لكي تتوفر مساحة تخزين يكتب إليها هذا الإعداد. يدير ClickHouse Cloud التخزين المؤقت تلقائيًا. عيّن `enable_filesystem_cache = 0` عند إجراء اختبارات الأداء حتى لا تُخفي نتائج التخزين المؤقت التغييرات بين مرات التشغيل.
...
| ------------------------------------------------------------------------------------------------------------------ | ---- | -------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| [use&#95;iceberg&#95;partition&#95;pruning](/ar/reference/settings/session-settings#use_iceberg_partition_pruning)           | 25.1 | `1` اعتبارًا من 25.6 | يتجاوز ملفات البيانات بالاستناد إلى البيانات الوصفية للتقسيم في ملفات manifest                                                 |
| [use&#95;iceberg&#95;metadata&#95;files&#95;cache](/ar/reference/settings/session-settings#use_iceberg_metadata_files_cache) | 25.4 | `1`                  | يخزّن قوائم manifest وJSON البيانات الوصفية مؤقتًا في الذاكرة                                                                  |
| [iceberg&#95;metadata&#95;staleness&#95;ms](/ar/reference/settings/session-settings#iceberg_metadata_staleness_ms)           | 26.3 | `0`                  | إعداد للاستعلام. استخدم البيانات الوصفية المخزنة مؤقتًا عندما تكون أحدث من هذه النافذة بدلًا من استدعاء الكتالوج في كل استعلام |
...
2. اضبط [iceberg&#95;metadata&#95;staleness&#95;ms](/ar/reference/settings/session-settings#iceberg_metadata_staleness_ms) (26.3+) على الاستعلامات لقبول بيانات وصفية قديمة قليلًا مقابل تجنب رحلة الذهاب والإياب إلى الكتالوج.
...
إلى جانب [allow&#95;insert&#95;into&#95;iceberg](/ar/reference/settings/session-settings#allow_insert_into_iceberg) ‏(25.7+، Beta بدءًا من 26.2)، يمكنك التحكم في حجم ملف الإخراج وعدد التقسيمات أثناء الإدراج:

| الإعداد                                                                                                                              | منذ   | الغرض                                                    |
| ------------------------------------------------------------------------------------------------------------------------------------ | ----- | -------------------------------------------------------- |
| [iceberg&#95;insert&#95;max&#95;rows&#95;in&#95;data&#95;file](/ar/reference/settings/session-settings#iceberg_insert_max_rows_in_data_file)   | 25.9  | الحد الأقصى لعدد الصفوف في كل ملف بيانات ناتج            |
| [iceberg&#95;insert&#95;max&#95;bytes&#95;in&#95;data&#95;file](/ar/reference/settings/session-settings#iceberg_insert_max_bytes_in_data_file) | 25.9  | الحد الأقصى لعدد البايتات في كل ملف بيانات ناتج          |
| [iceberg&#95;insert&#95;max&#95;partitions](/ar/reference/settings/session-settings#iceberg_insert_max_partitions)                             | 25.12 | الحد الأقصى لعدد التقسيمات المكتوبة في عملية إدراج واحدة |
...
ابتداءً من الإصدار 25.6، يقرأ ClickHouse جداول Delta Lake على S3 وGCS باستخدام نواة Delta Lake المكتوبة بلغة Rust ([allow&#95;experimental&#95;delta&#95;kernel&#95;rs](/ar/reference/settings/session-settings#allow_experimental_delta_kernel_rs)، 25.5+). على Azure Blob Storage، استخدم [deltaLakeAzure()](/ar/reference/functions/table-functions/deltalake) مع القارئ legacy لأن هذه النواة معطّلة هناك. وبدون هذه النواة، لن تتوفر استبعاد الأقسام أو تغذية بيانات التغيير أو قراءة snapshot version.

<div id="delta-kernel">
  #### Delta Kernel
</div>

يجب تمكين [allow&#95;experimental&#95;delta&#95;kernel&#95;rs](/ar/reference/settings/session-settings#allow_experimental_delta_kernel_rs) لاستخدام استبعاد الأقسام، وتغذية بيانات التغيير، وقراءة إصدارات اللقطات. ويكون مفعّلًا افتراضيًا على S3 وGCS بدءًا من الإصدار 25.5. فعِّله صراحةً في الإصدارات الأقدم أو عند استكشاف الأخطاء وإصلاحها:
...
| الإعداد                                                                                                                                                                                                                                 | منذ   | الافتراضي | ملاحظات                                                                              |
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- | --------- | ------------------------------------------------------------------------------------ |
| [delta&#95;lake&#95;enable&#95;engine&#95;predicate](/ar/reference/settings/session-settings#delta_lake_enable_engine_predicate)                                                                                                                  | 25.8  | `1`       | يُمرِّر عوامل التصفية إلى النواة لاستبعاد الأقسام. يتطلب [Delta Kernel](#delta-kernel) |
| [delta&#95;lake&#95;reload&#95;schema&#95;for&#95;consistency](/ar/reference/settings/session-settings#delta_lake_reload_schema_for_consistency)                                                                                                  | 26.3  | `0`       | يعيد تحميل المخطط قبل كل query عندما يُجري الكتّاب المتزامنون تغييرات على المخطط     |
| [delta&#95;lake&#95;snapshot&#95;start&#95;version](/ar/reference/settings/session-settings#delta_lake_snapshot_start_version) / [delta&#95;lake&#95;snapshot&#95;end&#95;version](/ar/reference/settings/session-settings#delta_lake_snapshot_end_version) | 25.12 | `-1`      | يقرأ تغييرات CDF بين إصدارَي لقطتين. يتطلب تمكين CDF في المصدر                       |
| [delta&#95;lake&#95;snapshot&#95;version](/ar/reference/settings/session-settings#delta_lake_snapshot_version)                                                                                                                                    | 25.8  | `-1`      | يقرأ لقطة تاريخية واحدة. اضبط `-1` للحصول على أحدث لقطة (`0` قيمة صالحة)             |
...
لقراءة الصفوف التي تغيّرت فقط بين لقطتَي Delta، عيّن [delta&#95;lake&#95;snapshot&#95;start&#95;version](/ar/reference/settings/session-settings#delta_lake_snapshot_start_version) و[delta&#95;lake&#95;snapshot&#95;end&#95;version](/ar/reference/settings/session-settings#delta_lake_snapshot_end_version) (25.12+). يجب أن تكون تغذية بيانات التغيير مفعّلة للجدول في المصدر الأصلي (`delta.enableChangeDataFeed`). عيّن كلاً من إصدارَي البداية والنهاية في إعدادات الاستعلام. سيؤدي تعيين إصدار النهاية فقط إلى حدوث خطأ.
...
إلى جانب [allow&#95;delta&#95;lake&#95;writes](/ar/reference/settings/session-settings#allow_experimental_delta_lake_writes) ‏(25.9+)، تحكَّم في حجم ملف الإخراج أثناء الإدراج:

| الإعداد                                                                                                                                        | منذ  | الغرض                                       |
| ---------------------------------------------------------------------------------------------------------------------------------------------- | ---- | ------------------------------------------- |
| [delta&#95;lake&#95;insert&#95;max&#95;rows&#95;in&#95;data&#95;file](/ar/reference/settings/session-settings#delta_lake_insert_max_rows_in_data_file)   | 25.9 | الحد الأقصى للصفوف في كل ملف بيانات إخراج   |
| [delta&#95;lake&#95;insert&#95;max&#95;bytes&#95;in&#95;data&#95;file](/ar/reference/settings/session-settings#delta_lake_insert_max_bytes_in_data_file) | 25.9 | الحد الأقصى للبايتات في كل ملف بيانات إخراج |
...
تتطلب عمليات الكتابة استخدام Delta Kernel مع S3 أو GCS. راجع [مرجع محرك DeltaLake](/ar/reference/engines/table-engines/integrations/deltalake) للاطلاع على أمثلة.
...
قارن بين `read_rows` و`read_bytes` في [system.query&#95;log](/ar/reference/system-tables/query_log) قبل إضافة عوامل التصفية أو ضبط الإعدادات وبعدها. تُظهر ProfileEvents مثل `ReadBufferFromS3Bytes` و`CachedReadBufferReadFromCacheBytes` مقدار البيانات التي جاءت من التخزين الكائني مقابل ذاكرة التخزين المؤقت المحلية. راجع [تحسين الاستعلام](/ar/guides/clickhouse/performance-and-monitoring/query-optimization) للحصول على شرح كامل لـ query&#95;log وEXPLAIN.

عطّل [enable&#95;filesystem&#95;cache](/ar/reference/settings/session-settings#enable_filesystem_cache) عند إجراء اختبارات الأداء حتى لا تُخفي مرات الوصول الناجح إلى ذاكرة التخزين المؤقت الفروق بين التشغيلات.
...
| ----------------------------------------------------------------------------------------------- | ---------- | ----- | --------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------- |
| [system.iceberg&#95;metadata&#95;log](/ar/reference/system-tables/iceberg_metadata_log)           | Iceberg    | 25.9  | [iceberg&#95;metadata&#95;log&#95;level](/ar/reference/settings/session-settings#iceberg_metadata_log_level) في الاستعلام | تتبّع ملفات البيانات الوصفية المقروءة وقرارات استبعاد الأقسام |
...
| [system.delta&#95;lake&#95;metadata&#95;log](/ar/reference/system-tables/delta_metadata_log) | Delta Lake | 25.10 | [delta&#95;lake&#95;log&#95;metadata](/ar/reference/settings/session-settings#delta_lake_log_metadata) = `1` في الاستعلام | تتبّع ملفات البيانات الوصفية في Delta وتحديد اللقطة           |
...
* [البدء](/ar/guides/use-cases/data-warehousing/getting-started/overview) — شرح متكامل من الاستعلام المباشر إلى إعادة الكتابة إلى المصدر
* [الاستعلام المباشر](/ar/guides/use-cases/data-warehousing/getting-started/querying-directly) — دوال الجداول، والمحركات، وإصدارات العناقيد لمختلف التنسيقات الأربعة
* [الاتصال بالكتالوجات](/ar/guides/use-cases/data-warehousing/getting-started/connecting-catalogs) — إعداد `DataLakeCatalog` باستخدام Unity Catalog
* [الكتابة إلى بحيرات البيانات](/ar/guides/use-cases/data-warehousing/getting-started/writing-data) — كتابة البيانات مجددًا إلى Iceberg وDelta Lake
* [مصفوفة الدعم](/ar/guides/use-cases/data-warehousing/support-matrix) — مقارنة الميزات عبر التنسيقات، والكتالوجات، وأنظمة التخزين الخلفية