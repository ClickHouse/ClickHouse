...
* يُضبط TTL عبر `TTL toDateTime(Timestamp) + toIntervalDay(3)`، وهو يتوافق مع القيمة المحددة في تهيئة collector. وتعني [`ttl_only_drop_parts=1`](/ar/reference/settings/merge-tree-settings/other#ttl_only_drop_parts) أنه لا تُحذف إلا الأجزاء الكاملة عندما تكون جميع الصفوف التي تحتويها قد انتهت صلاحيتها. وهذا أكثر كفاءة من حذف الصفوف داخل الأجزاء، إذ يترتب على ذلك تنفيذ عملية حذف مكلفة. نوصي دائمًا بضبط هذا الإعداد. راجع [إدارة البيانات باستخدام TTL](/ar/guides/use-cases/observability/build-your-own/managing-data#data-management-with-ttl-time-to-live) لمزيد من التفاصيل.
