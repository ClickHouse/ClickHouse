...
عادةً ما يضطر المستخدمون إلى إرسال دفعات أصغر عندما يكون معدل النقل في collector منخفضًا، ومع ذلك يظلون يتوقعون أن تصل البيانات إلى ClickHouse بأقل زمن انتقال ممكن من طرف إلى طرف. في هذه الحالة، تُرسل الدفعات الصغيرة عند انتهاء مهلة `timeout` الخاصة بـ معالج الدفعات. وقد يؤدي ذلك إلى مشكلات، وهنا تبرز الحاجة إلى asynchronous inserts. تظهر هذه الحالة عادةً عندما **تُضبط collectors التي تعمل بدور agent للإرسال مباشرةً إلى ClickHouse**. ويمكن لـ gateways، من خلال عملها كمجمِّعات مركزية، أن تخفف هذه المشكلة — راجع [التوسع باستخدام gateways](#scaling-with-gateways).
...
أخيرًا، لا يكون سلوك deduplication السابق المرتبط بالإدراجات المتزامنة في ClickHouse مُمكّنًا افتراضيًا عند استخدام asynchronous inserts. وإذا لزم الأمر، فراجع الإعداد [`async_insert_deduplicate`](/ar/reference/settings/session-settings#async_insert_deduplicate).
