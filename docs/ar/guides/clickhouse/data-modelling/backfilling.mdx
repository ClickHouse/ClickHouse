...
نفترض أن المستخدم يستهلك تدفقًا من هذه البيانات، على سبيل المثال من Kafka أو تخزين الكائنات، وذلك للبيانات اللاحقة لهذا التاريخ. يوضَّح مخطط هذه البيانات أدناه:
...
* **توازي insert** - عدد خيوط insert المستخدمة لتنفيذ insert. يتم التحكم فيه عبر [`max_insert_threads`](/ar/reference/settings/session-settings/max-insert#max_insert_threads). **ملاحظة**: هذه القيمة مقيّدة بالحد `max_threads`، لذا فإن توازي insert الفعلي هو `min(max_insert_threads, max_threads)`. في ClickHouse Cloud، يتحدد ذلك بحسب حجم المثيل (بين 2 و4)، ويتم ضبطه على 1 في OSS. قد تؤدي زيادة هذه القيمة إلى تحسين الأداء على حساب زيادة استخدام الذاكرة.
