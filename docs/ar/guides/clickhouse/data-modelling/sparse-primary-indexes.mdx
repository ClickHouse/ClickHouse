...
slug: /ar/guides/clickhouse/data-modelling/sparse-primary-indexes
...
<div id="a-full-table-scan">
  ### مسح كامل للجدول {#a-full-table-scan}
</div>
...
<div id="clickhouse-index-design">
  ## تصميم الفهارس في ClickHouse {#clickhouse-index-design}
</div>

<div id="an-index-design-for-massive-data-scales">
  ### تصميم فهرس لنطاقات بيانات هائلة {#an-index-design-for-massive-data-scales}
</div>
...
<div id="a-table-with-a-primary-key">
  ### جدول ذو مفتاح أساسي {#a-table-with-a-primary-key}
</div>
...
### تُخزَّن البيانات على القرص مرتبةً بحسب أعمدة المفتاح الأساسي {#data-is-stored-on-disk-ordered-by-primary-key-columns}
...
<Note>
  - لو كنا قد حدّدنا مفتاح الفرز فقط، لكان المفتاح الأساسي سيُعرَّف ضمنيًا على أنه يساوي مفتاح الفرز.
...
ClickHouse هو <a href="/ar/get-started/about/distinctive-features#true-column-oriented-database-management-system" target="_blank">نظام إدارة قواعد بيانات موجَّه بالأعمدة</a>. وكما هو موضح في المخطط أدناه:
...
### تُنظَّم البيانات في حبيبات لمعالجتها بالتوازي {#data-is-organized-into-granules-for-parallel-data-processing}
...
### يحتوي الفهرس الأساسي على إدخال واحد لكل حبيبة {#the-primary-index-has-one-entry-per-granule}
...
### يُستخدم الفهرس الأساسي لاختيار الحبيبات {#the-primary-index-is-used-for-selecting-granules}
...
### تُستخدم ملفات العلامات لتحديد مواقع الحبيبات {#mark-files-are-used-for-locating-granules}
...
<div id="using-multiple-primary-indexes">
  ## استخدام فهارس أولية متعددة {#using-multiple-primary-indexes}
</div>

<a name="filtering-on-key-columns-after-the-first" />

### قد تكون أعمدة المفتاح الثانوية غير فعّالة (أو لا) {#secondary-key-columns-can-not-be-inefficient}
...
<Note>
  ...
  عندما يطبّق الاستعلام عامل تصفية على كلٍّ من عمود المفتاح الأول وأي أعمدة مفتاح بعده، فإن ClickHouse يشغّل البحث الثنائي على علامات الفهرس لعمود المفتاح الأول.
</Note>
...
### خوارزمية البحث بالاستبعاد العامة {#generic-exclusion-search-algorithm}
...
### ملاحظة حول فهرس تخطي البيانات {#note-about-data-skipping-index}
...
<div id="a-need-to-use-multiple-primary-indexes">
  ### الحاجة إلى استخدام عدة فهارس أساسية {#a-need-to-use-multiple-primary-indexes}
</div>
...
### خيارات إنشاء فهارس أساسية إضافية {#options-for-creating-additional-primary-indexes}
...
### الخيار 1: الجداول الثانوية {#option-1-secondary-tables}
...
ويؤكد سجل الخادم المقابل ذلك (في الإصدارات الحديثة، تُسجَّل رسائل البحث الثنائي على مستوى `test` — راجع [الملاحظة أعلاه](#the-primary-index-is-used-for-selecting-granules)):
...
### الخيار 2: العروض المادية {#option-2-materialized-views}
...
ويؤكد سجل الخادم المقابل أن ClickHouse يُجري البحث الثنائي على علامات الفهرس (في الإصدارات الحديثة، تُسجَّل رسائل البحث الثنائي عند المستوى `test` — انظر [الملاحظة أعلاه](#the-primary-index-is-used-for-selecting-granules)):
...
### الخيار 3: الإسقاطات {#option-3-projections}
...
ويؤكد سجل الخادم المقابل أن ClickHouse يُجري بحثًا ثنائيًا عبر علامات الفهرس (في الإصدارات الحديثة، تُسجَّل رسائل البحث الثنائي عند المستوى `test` — راجع [الملاحظة أعلاه](#the-primary-index-is-used-for-selecting-granules)):
...
<div id="ordering-key-columns-efficiently">
  ## ترتيب أعمدة المفتاح بكفاءة {#ordering-key-columns-efficiently}
</div>
...
<div id="efficient-filtering-on-secondary-key-columns">
  ### التصفية بكفاءة على أعمدة المفاتيح الثانوية {#efficient-filtering-on-secondary-key-columns}
</div>
...
### أفضل نسبة ضغط لملفات البيانات {#optimal-compression-ratio-of-data-files}
...
<div id="identifying-single-rows-efficiently">
  ## تحديد الصفوف الفردية بكفاءة {#identifying-single-rows-efficiently}
</div>
