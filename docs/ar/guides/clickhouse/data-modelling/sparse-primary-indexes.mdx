...
إذا كان <a href="/ar/reference/settings/server-settings/settings/other#logger" target="_blank">تسجيل التتبّع</a> مُمكّنًا، فإن ملف سجل خادم ClickHouse يُظهر أن ClickHouse كان يُجري <a href="https://github.com/ClickHouse/ClickHouse/blob/22.3/src/Storages/MergeTree/MergeTreeDataSelectExecutor.cpp#L1452" target="_blank">بحثًا ثنائيًا</a> عبر 1083 من علامات فهرس UserID، لتحديد الحبيبات التي قد تحتوي على صفوف تكون فيها قيمة العمود UserID هي `749927693`. ويتطلّب ذلك 19 خطوة بمتوسط تعقيد زمني قدره `O(log2 n)`:
...
:::note
في الإصدارات الحديثة من ClickHouse، تُسجَّل رسائل `Running binary search on index range` و`Found (LEFT) boundary mark` و`Found (RIGHT) boundary mark` عند مستوى `test` (أعلى مستوى من الإسهاب) بدلًا من `trace`. لرؤيتها، ارفع مستوى إسهاب السجل إلى `test` (على سبيل المثال، `SET send_logs_level = 'test'` في العميل، أو `<logger><level>test</level></logger>` في تهيئة الخادم). لا تزال الأسطر المتبقية المعروضة أعلاه تُسجَّل عند مستوى `trace`. ينطبق هذا على كل مثال لسجل البحث الثنائي في هذا الدليل.
:::
