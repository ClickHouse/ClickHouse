...
تُحوَّل بنية SQLAlchemy القياسية `values()` عند الترجمة إلى صياغة دالة الجدول `VALUES` في ClickHouse، بما في ذلك عند استخدامها في تعبير الجدول الشائع (يتطلب شكل CTE استخدام SQLAlchemy 2.x، إذ أُضيفت `Values.cte()`).
