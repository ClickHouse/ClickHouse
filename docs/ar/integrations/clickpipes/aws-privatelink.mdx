...
إذا كانت خدمتك لا تدعم استخدام private DNS المُدار من PrivateLink (على سبيل المثال، إذا كان المزوّد يتطلب
من المستهلكين إدارة DNS resolution، كما تفعل Confluent Cloud)، فراجع [DNS خاص مخصص](#custom-private-dns).
...
<Info>
  ميزة DNS الخاص المخصص ما تزال في **Private Preview**. تواصل مع [فريق دعم ClickHouse](https://clickhouse.com/support/program) لتمكينها لخدمتك.
</Info>

يوفّر AWS PrivateLink [DNS خاصًا مُدارًا](https://docs.aws.amazon.com/vpc/latest/privatelink/manage-dns-names.html)
...
في هذه الحالات، يدعم ClickPipes إرفاق أسماء DNS خاصة مخصصة بـ نقطة نهاية خاصة عكسية.
...
يُكمل DNS الخاص المخصص DNS الخاص المُدار — ولا يحل محله. وإذا كانت خدمة
PrivateLink لديك توفّر بالفعل أسماء DNS خاصة، فلست بحاجة إلى أسماء مخصصة.

تتوفّر أسماء DNS الخاصة المخصصة لنوعي endpoint [VPC endpoint service](#vpc-endpoint-service)
و[VPC resource](#vpc-resource). ويوفّر MSK multi-VPC DNS خاصًا مُدارًا
