...
* كما أن المفاتيح التي تحتوي على نقاط (مثل `"app.kubernetes.io/name"`) تُفسَّر على أنها مسارات متداخلة في نوع JSON لدى ClickHouse، مما قد يغيّر بنية البيانات.

للتعامل مع سلاسل JSON في المراحل اللاحقة لـ ClickPipe لـ Postgres CDC، يمكنك استخدام [دوال JSON](/ar/reference/functions/regular-functions/json-functions) مباشرةً على الأعمدة لاستخراج البيانات والعمل معها. ولتحسين أداء الاستعلام، يمكنك أيضًا استخدام عرض مُجسَّد لاستخراج حقول محددة إلى أعمدة ذات أنواع معرّفة، أو لتحويل العمود إلى نوع JSON الأصلي (مع مراعاة أن التحويل قد يفشل إذا صادف حالة عدم توافق بين الأنواع عبر المنصات).
