...
تُحدَّد تعيين الأنواع أدناه بواسطة `io.confluent.connect.avro.AvroConverter`، وهو التنفيذ الرسمي لتسلسل/إلغاء تسلسل Avro في Kafka Connect. راجع [وثائق](https://docs.confluent.io/platform/current/connect/userguide.html#avro) Kafka Connect للحصول على معلومات متقدمة حول منطق التحويل.
...
| int32                                   | INT8/INT16/INT32                        | Int32                                          | ✅     | القيمة الافتراضية هي INT32. ويُحوَّل إلى INT8 إذا كان المخطط يتضمن الخيار `connect.type=int8` (وبالمثل إلى INT16 إذا كان `connect.type=int16`)                                                |
| sint32                                  | INT8/INT16/INT32                        | Int32                                          | ✅     | القيمة الافتراضية هي INT32. ويُحوَّل إلى INT8 إذا كان المخطط يتضمن الخيار `connect.type=int8` (وبالمثل إلى INT16 إذا كان `connect.type=int16`)                                                |
| sfixed32                                | INT8/INT16/INT32                        | Int32                                          | ✅     | القيمة الافتراضية هي INT32. ويُحوَّل إلى INT8 إذا كان المخطط يتضمن الخيار `connect.type=int8` (وبالمثل إلى INT16 إذا كان `connect.type=int16`)                                                |
| uint32                                  | INT64                                   | UInt32                                         | ✅     |                                                                                                                                                                                               |
...
| bytes                                   | BYTES                                   | String                                         | ✅     |                                                                                                                                                                                               |
| enum                                    | INT32/STRING                            | Int32                                          | ✅     | القيمة الافتراضية هي STRING. ويُحوَّل إلى INT32 إذا كان `int.for.enums=true` (راجع [وثائق schema registry](https://docs.confluent.io/platform/current/schema-registry/connect.html#protobuf)) |
...
* بُنى اتحاد متعددة الرسائل (**قبل الإصدار 26.1 من CH**)
...
اعتبارًا من الإصدار 26.1 من CH وما بعده، أصبح هذا المخطط مدعومًا عند ضبط `allow_experimental_nullable_tuple_type=1` (راجع [صفحة التوثيق هذه](/ar/reference/settings/session-settings/allow-experimental#allow_experimental_nullable_tuple_type)).
