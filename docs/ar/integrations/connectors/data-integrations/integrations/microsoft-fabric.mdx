...
<div id="copying-data-from-onelake">
  ## نسخ البيانات من OneLake
</div>

تتيح لك عملية Copy &amp; Sync نسخ جداول OneLake إلى ClickHouse:

1. افتح عنصر ClickHouse وحدد **Copy &amp; Sync**
2. اختر Lakehouse من مساحة عملك
3. حدد الجداول التي تريد تسريعها
4. ابدأ بنسخ لقطة من بياناتك في نقطة زمنية محددة

<Frame>
  ![إعداد Copy & Sync ينشئ جدول وجهة جديدًا بمفتاح فرز](/images/integrations/tools/data-integration/microsoft-fabric/fabric-03-copy-sync-table-selection.webp)
</Frame>
...
<Frame>
  ![تعيين أعمدة Copy & Sync من جدول مصدر في OneLake إلى جدول ClickHouse موجود](/images/integrations/tools/data-integration/microsoft-fabric/fabric-04-copy-sync-existing-table.webp)
</Frame>
...
يمكن نسخ البيانات من OneLake إلى ClickHouse مجانًا خلال فترة المعاينة العامة لـ Microsoft Fabric. تبدأ الفوترة عندما يصل عبء عمل Fabric إلى مرحلة التوافر العام. وستُنشر تفاصيل الأسعار عند وصول عبء العمل إلى مرحلة GA.
...
حدّد **Open ClickHouse** لفتح واجهة ClickHouse Cloud الكاملة بعد تسجيل الدخول باستخدام هوية Microsoft الخاصة بك. يمكنك نسخ البيانات إلى ClickHouse وتشغيل أي عبارات SQL من عبء العمل Fabric، ولكن لإجراء أي مهام إدارية، ستحتاج إلى الدخول إلى وحدة تحكم ClickHouse Cloud لإدارة الخدمة.
...
تعمل كل عملية Copy &amp; Sync من OneLake باعتبارها ClickPipe. راقب تقدّم النسخ ومعدل النقل والأخطاء في [صفحة ClickPipes](/ar/integrations/clickpipes/monitoring) في وحدة تحكم ClickHouse Cloud.
...
1. أنشئ عنصر ClickHouse في مساحة العمل نفسها، كما هو موضح في قسم [التجهيز](#provisioning)
2. افتح **Copy &amp; Sync**، واختر Lakehouse، ثم حدّد جدول `publicholidays`
...
* **جداول Lakehouse فقط.** تقرأ Copy &amp; Sync جداول Delta من Lakehouse ضمن مساحة العمل نفسها. عناصر Fabric الأخرى (Warehouse وقواعد بيانات KQL وقواعد البيانات المعكوسة) غير مدعومة بعد كمصادر.
...
* **تسري تغييرات الدور عند الفتح التالي.** يُعيَّن دورك في مؤسسة ClickHouse استنادًا إلى دورك في مساحة عمل Fabric عند فتح عنصر من ClickHouse. لا يمكن لـ Fabric إبلاغ ClickHouse بتغييرات الأدوار، لذا يسري تغيير دور مساحة العمل في المرة التالية التي تفتح فيها العنصر، وليس فورًا.
