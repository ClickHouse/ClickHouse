---
description: 'ينشئ محرك الجدول اسم مستعار وكيلاً شفافًا لجدولٍ آخر. وتُمرَّر جميع العمليات إلى الجدول الهدف، بينما لا يخزّن جدول الاسم المستعار نفسه أي بيانات.'
sidebar_label: 'اسم مستعار'
sidebar_position: 5
slug: /engines/table-engines/special/alias
title: 'محرك الجدول اسم مستعار'
doc_type: 'reference'
---
...
ينشئ المحرك `Alias` وكيلاً لجدول آخر. وتُحوَّل جميع عمليات القراءة والكتابة إلى الجدول الهدف، بينما لا يخزّن الاسم المستعار نفسه أي بيانات، ويحتفظ فقط بمرجع إلى الجدول الهدف.
...
:::note
لا يدعم جدول `Alias` تعريف الأعمدة بشكل صريح. تُورَّث الأعمدة تلقائيًا من الجدول الهدف. وهذا يضمن أن يتوافق الاسم المستعار دائمًا مع مخطط الجدول الهدف.
:::
...
:::note
عند حذف `target_db` وعدم تحديد `target_table` بالكامل (على سبيل المثال، `Alias('my_table')`)، يُفسَّر الهدف على أنه في قاعدة البيانات نفسها الخاصة بالاسم المستعار، وليس قاعدة البيانات الحالية للجلسة.
:::
...
تُمرَّر هذه العمليات إلى الجدول الهدف:
...
| `ALTER TABLE PARTITION`      | ✅     | عمليات التقسيم ‏(DETACH/ATTACH/DROP) على الجدول الهدف |
...
<div id="operations-on-alias">
  ### العمليات على الاسم المستعار نفسه
</div>

تؤثر هذه العمليات على الاسم المستعار فقط، **وليس** على الجدول الهدف:

| العملية        | الدعم | الوصف                                                      |
| -------------- | ----- | ---------------------------------------------------------- |
| `DROP TABLE`   | ✅     | يحذف الاسم المستعار فقط، ويظل الجدول الهدف دون تغيير       |
| `RENAME TABLE` | ✅     | يعيد تسمية الاسم المستعار فقط، ويظل الجدول الهدف دون تغيير |
...
<div id="basic-alias-creation">
  ### إنشاء اسم مستعار بسيط
</div>

أنشئ اسمًا مستعارًا بسيطًا ضمن قاعدة البيانات نفسها:
...
<div id="cross-database-alias">
  ### اسم مستعار بين قواعد البيانات
</div>

أنشئ اسمًا مستعارًا يُشير إلى جدول في قاعدة بيانات أخرى:
...
العمليتان UPDATE وDELETE مدعومتان:
...
بالنسبة إلى الجداول المُقسَّمة، تُمرَّر عمليات التقسيم:
...
يمكن إعادة تسمية الأسماء المستعارة أو حذفها كلٌّ على حدة:
