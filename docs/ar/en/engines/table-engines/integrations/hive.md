---
description: 'يتيح لك محرك Hive إجراء استعلامات `SELECT` على جدول Hive في HDFS.'
sidebar_label: 'Hive'
sidebar_position: 84
...
يتيح لك محرك Hive تنفيذ استعلامات `SELECT` على جدول Hive في HDFS. ويدعم حاليًا تنسيقات الإدخال التالية:

* Text: يدعم فقط أنواع الأعمدة السكالارية البسيطة باستثناء `binary`

* ORC: يدعم أنواع الأعمدة السكالارية البسيطة فقط باستثناء `char`؛ ولا يدعم من الأنواع المعقدة سوى أنواع مثل `array`

* Parquet: يدعم جميع أنواع الأعمدة السكالارية البسيطة؛ ولا يدعم من الأنواع المعقدة سوى أنواع مثل `array`
...
اطّلع على وصف مفصل لاستعلام [CREATE TABLE](/ar/sql-reference/statements/create/table).

يمكن أن تختلف بنية الجدول عن بنية جدول Hive الأصلي:

* يجب أن تكون أسماء الأعمدة هي نفسها الموجودة في جدول Hive الأصلي، ولكن يمكنك استخدام بعض هذه الأعمدة فقط وبأي ترتيب، كما يمكنك أيضًا استخدام أعمدة مستعارة محسوبة من أعمدة أخرى.
* يجب أن تكون أنواع الأعمدة مطابقة لتلك الموجودة في جدول Hive الأصلي.
* يجب أن يكون تعبير التقسيم متوافقًا مع جدول Hive الأصلي، ويجب أن تكون الأعمدة الواردة في تعبير التقسيم موجودة في بنية الجدول.
...
ننصح بشدة بتمكين ذاكرة التخزين المؤقت المحلية لأنظمة الملفات البعيدة. تُظهر الاختبارات المعيارية أن الأداء يصبح أسرع بما يقارب الضعف عند استخدام ذاكرة التخزين المؤقت.
...
* bytes&#95;read&#95;before&#95;flush: يحدد عدد البايتات المقروءة قبل إجراء flush إلى نظام الملفات المحلي عند تنزيل ملف من نظام الملفات البعيد. القيمة الافتراضية هي 1MB.

<div id="query-hive-table-with-orc-input-format">
  ### الاستعلام عن جدول Hive بتنسيق الإدخال ORC
</div>
...
جدول في ClickHouse يسترجع البيانات من جدول Hive المُنشأ أعلاه:
