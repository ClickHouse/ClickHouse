---
description: 'توثيق clickhousectl، وهي واجهة CLI لـ ClickHouse: المحلي والسحابي'
...
`clickhousectl` هي واجهة CLI لـ ClickHouse: المحلي والسحابي.
...
* إعداد ClickHouse Cloud وإنشاء مجموعات ClickHouse مُدارة سحابيًا
...
* تثبيت حزمة ClickHouse agent skills الرسمية في وكلاء البرمجة المدعومين
* نقل بيئة تطوير ClickHouse المحلية لديك إلى السحابة
...
يُنزّل برنامج التثبيت النصي الإصدار المناسب لنظام التشغيل لديك ويثبّته في `~/.local/bin/clickhousectl`. كما يُنشئ تلقائيًا alias باسم `chctl` لتسهيل الاستخدام.
...
* تتطلب أوامر Cloud [مفتاح واجهة برمجة تطبيقات ClickHouse Cloud](/ar/cloud/manage/api/api-overview)
...
يقوم `clickhousectl` بتنزيل الملفات التنفيذية لـ ClickHouse من `builds.clickhouse.com`، ويلجأ إلى `packages.clickhouse.com` ‏(Linux) أو [إصدارات GitHub](https://github.com/ClickHouse/ClickHouse/releases) ‏(macOS) إذا لم يكن الـ `build` متوفرًا هناك.
...
ينشئ `local use` أيضًا رابطًا رمزيًا في `~/.local/bin/clickhouse` يشير إلى الملف التنفيذي للإصدار المحدد، بحيث يصبح أمر `clickhouse` العادي (مثل `clickhouse local` و`clickhouse client`) متاحًا على `PATH`. مرّر `--no-global` لتخطي ذلك. وإذا كان هناك بالفعل ملف عادي في هذا المسار، فسيُترك كما هو مع إصدار تحذير. كما أن `local remove` للإصدار الافتراضي النشط يزيل الرابط الرمزي أيضًا.

<div id="binary-storage">
  #### تخزين ملف تنفيذي لـ ClickHouse
</div>

تُخزَّن الملفات التنفيذية لـ ClickHouse في مستودع عام، بحيث يمكن لعدة مشاريع استخدامها من دون تكرار التخزين. وتُحفَظ هذه الملفات في `~/.clickhouse/`:
...
يقوم `init` بإعداد دليل العمل الحالي لديك باستخدام بنية مجلدات قياسية لملفات مشروع ClickHouse وPostgres. وهو اختياري؛ ويمكنك استخدام بنية المجلدات الخاصة بك إذا كنت تفضل ذلك.

وينشئ البنية التالية:
...
ابدأ مثيلات خادم ClickHouse وأدرها. يحصل كل خادم على دليل بيانات معزول خاص به في `.clickhouse/servers/<name>/data/`.
...
**تسمية الخادم:** من دون `--name`، يُسمّى الخادم الأول &quot;default&quot;. وإذا كان &quot;default&quot; قيد التشغيل بالفعل، يُنشأ اسم عشوائي (مثل &quot;bold-crane&quot;). استخدم `--name` للحصول على هويات ثابتة يمكنك تشغيلها وإيقافها بشكل متكرر.

**المنافذ:** القيم الافتراضية هي HTTP 8123 وTCP 9000. إذا كانت هذه المنافذ مستخدمة بالفعل، تُخصَّص منافذ متاحة تلقائيًا وتظهر في المخرجات. استخدم `--http-port` و`--tcp-port` لتعيين منافذ محددة صراحةً.

**الإدارة العامة للخوادم:** استخدم `--global` مع `list` و`stop` و`stop-all` للعمل على مستوى جميع المشاريع في النظام. يعرض `server list --global` جميع خوادم ClickHouse قيد التشغيل، مع عمود Project يوضّح الدليل الذي ينتمي إليه كل خادم.

<div id="custom-config-files">
  #### ملفات تهيئة مخصّصة للخوادم المحلية
</div>

تبدأ الخوادم المحلية بإعدادات افتراضية مناسبة، لكنك قد تحتاج أحيانًا إلى تعديل أحد الإعدادات. ضع ملف تهيئة في `~/.clickhouse/configs/` ثم طبّقه بالاسم عند بدء تشغيل الخادم:
...
يُطبَّق **الملف المُسمّى فوق الإعدادات الافتراضية المضمّنة في ClickHouse** (عبر `config.d`)، لذلك لا يلزم أن يتضمّن إلا الإعدادات التي تريد تغييرها، ولا حاجة إلى إعادة إنشاء `config` كامل. يمكن أن تكون الملفات بصيغة `.xml` أو `.yaml` أو `.yml`، ويمكنك الإشارة إليها بالاسم سواء مع الامتداد أو من دونه.

<div id="project-local-data">
  #### مجلد البيانات الخاص بالمشروع
</div>

توجد جميع بيانات الخادم داخل `.clickhouse/` في مجلد مشروعك:
...
لكل خادم مُسمّى دليل بيانات خاص به، لذا تكون الخوادم معزولة تمامًا عن بعضها البعض. تظل البيانات محفوظة بين عمليات إعادة التشغيل. أوقِف الخادم ثم ابدأه بالاسم لمتابعة العمل من حيث توقفت. استخدم `clickhousectl local server remove <name>` لحذف بيانات الخادم نهائيًا.

<div id="local-postgres">
  ### تشغيل Postgres محليًا
</div>

إلى جانب ClickHouse، يمكن لـ `clickhousectl` أيضًا تشغيل مثيلات Postgres المحلية وإدارتها. ويعتمد Postgres المحلي على Docker، لذا يجب أن يكون Docker مثبّتًا وقيد التشغيل. ويُعرَّف كل مثيل باسمه وإصداره الرئيسي، لذا يمكن تشغيل عدة إصدارات من Postgres جنبًا إلى جنب مع دلائل بيانات منفصلة.
...
قم بالمصادقة على ClickHouse Cloud باستخدام مفاتيح واجهة برمجة تطبيقات (موصى به) أو OAuth (عبر المتصفح).

إذا لم يكن لديك حساب على ClickHouse Cloud بعد، فإن `clickhousectl cloud auth signup` يفتح صفحة التسجيل في متصفحك.

<div id="api-key">
  ### مفتاح/سر واجهة برمجة تطبيقات (موصى به)
</div>

تُعد مفاتيح واجهة برمجة تطبيقات الطريقة الموصى بها للمصادقة، خاصةً عند تشغيل CLI عبر وكيل ذكاء اصطناعي. يمكنك [إنشاء مفاتيح واجهة برمجة تطبيقات بنطاقات محددة](/ar/cloud/manage/openapi) تمنح فقط الأذونات التي تختارها (للقراءة فقط أو للقراءة والكتابة)، ويرتبط كل مفتاح بمؤسسة واحدة فقط. وهذا يجعلها وسيلة آمنة لمنح CLI إمكانية الوصول وفق مبدأ أقل الامتيازات.
...
تُحفَظ بيانات الاعتماد في `.clickhouse/credentials.json` (محليًا على مستوى المشروع).

يمكنك أيضًا استخدام متغيرات البيئة، إما بعد تصديرها ضمن جلستك:
...
أو مرّر بيانات الاعتماد مباشرةً عبر خيارات سطر الأوامر في أي أمر:
...
سيؤدي ذلك إلى فتح متصفحك لإتمام المصادقة عبر تدفّق أجهزة OAuth. تُحفَظ الرموز المميّزة في `.clickhouse/tokens.json` (محلي على مستوى المشروع).

:::note
يكون وصول OAuth حاليًا **للقراءة فقط**، ويمنح إمكانية الوصول إلى **جميع المؤسسات التي تنتمي إليها**. للحصول على إذن الكتابة، أو لقصر CLI على مؤسسة واحدة، [أنشئ مفتاح واجهة برمجة تطبيقات مقيّد النطاق](#api-key) بدلًا من ذلك.
:::
...
ترتيب البحث عن بيانات الاعتماد: خيارات CLI &gt; `.clickhouse/credentials.json` &gt; متغيرات البيئة المُصدَّرة &gt; ملف `.env` &gt; رموز OAuth.

<div id="debug-credentials">
  ### استكشاف الأخطاء وإصلاحها لمعرفة مصدر بيانات الاعتماد المستخدم
</div>

مرّر `--debug` إلى أي أمر `cloud` لطباعة مصدر بيانات الاعتماد الذي تم تحديده (و`URL` واجهة برمجة تطبيقات) إلى stderr قبل تشغيل الأمر.
...
أدِر خدمات ClickHouse Cloud عبر واجهة برمجة تطبيقات.
...
| `--min-replica-memory-gb`                  | الحد الأدنى للذاكرة لكل replica بالغيغابايت (8-356، بمضاعفات 4) |
| `--max-replica-memory-gb`                  | الحد الأقصى للذاكرة لكل replica بالغيغابايت (8-356، بمضاعفات 4) |
| `--num-replicas`                           | عدد الـ replicas (1-20)                                         |
| `--idle-scaling`                           | السماح بالتحجيم إلى الصفر (الافتراضي: `true`)                   |
| `--idle-timeout-minutes`                   | الحد الأدنى لمهلة الخمول بالدقائق (&gt;= 5)                     |
| `--ip-allow`                               | نطاق IP CIDR المسموح به (يمكن تكراره، الافتراضي: `0.0.0.0/0`)   |
| `--backup-id`                              | معرّف النسخة الاحتياطية للاستعادة منها                          |
| `--release-channel`                        | قناة الإصدار: `slow`، `default`، `fast`                         |
| `--data-warehouse-id`                      | معرّف مستودع البيانات (لـ read replicas)                        |
| `--readonly`                               | جعل الخدمة للقراءة فقط                                          |
| `--encryption-key`                         | مفتاح تشفير القرص الخاص بالعميل                                 |
| `--encryption-role`                        | ARN الخاص بالدور لتشفير القرص                                   |
| `--enable-tde`                             | تمكين Transparent Data Encryption                               |
| `--compliance-type`                        | الامتثال: `hipaa`، `pci`                                        |
| `--profile`                                | ملف تعريف المثيل (Enterprise)                                   |
| `--tag`                                    | إرفاق وسم خدمة GA (`key` أو `key=value`)                        |
| `--enable-endpoint` / `--disable-endpoint` | تفعيل أو تعطيل endpoints خدمة GA (حاليًا `mysql`)               |
| `--private-preview-terms-checked`          | قبول شروط المعاينة الخاصة عند الحاجة                            |
| `--enable-core-dumps`                      | تمكين أو تعطيل جمع ملفات core dump الخاصة بالخدمة               |

<div id="query-api-auth-modes">
  #### أوضاع مصادقة واجهة برمجة تطبيقات الاستعلام
</div>

تُعد `cloud service query` الطريقة القياسية لتشغيل SQL على خدمة سحابية عبر HTTP، من دون الحاجة إلى الملف التنفيذي `clickhouse` أو إلى كلمة مرور الخدمة. وهي تعمل مع وضعي بيانات الاعتماد كليهما:

* **مصادقة API key** (قراءة + كتابة SQL): عند تشغيل `cloud service query` لأول مرة على خدمة لا تحتوي على مفتاح مخزَّن، فإنه يجهّز نقطة نهاية لواجهة برمجة تطبيقات الاستعلام لتلك الخدمة وينشئ API key مخصصًا مرتبطًا بها. يُخزَّن المفتاح (`keyId` و`keySecret` و`endpointId`) في `.clickhouse/credentials.json` تحت `service_query_keys.<service-id>`. يقتصر نطاق المفتاح على خدمة واحدة، لذا يمكنه القراءة والكتابة (SELECT وINSERT وDDL) على تلك الخدمة، لكنه لا يستطيع الوصول إلى أي خدمة أخرى في المؤسسة. مرِّر `--no-auto-enable` لكي يفشل بدلًا من التجهيز.
* **OAuth** (`cloud auth login`): يُنفَّذ الاستعلام بهويتك أنت، تمامًا كما في SQL-console على الويب. تكون أذونات SQL الخاصة بك على الخدمة **للقراءة فقط** عند استخدام OAuth. لا يتم تجهيز أو تخزين مفتاح واجهة برمجة تطبيقات الاستعلام. ليس للخيار `--no-auto-enable` أي تأثير في هذا الوضع.

يؤدي الاستعلام عن خدمة في حالة **idled** إلى إيقاظها تلقائيًا في كلا وضعي المصادقة (قد يستغرق الاستعلام الأول دقيقة). أما الخدمة **stopped** فلا يتم إيقاظها مطلقًا: يفشل الاستعلام مع تلميح لتشغيل `cloud service start`. عيّن `CLICKHOUSE_CLOUD_QUERY_HOST` لتجاوز مضيف واجهة برمجة تطبيقات الاستعلام المُشتق.
...
يمكن أيضًا لـ `clickhousectl` إنشاء خدمات [ClickHouse Cloud Postgres](/ar/cloud/managed-postgres) وإدارتها، على غرار أوامر خدمة ClickHouse المذكورة أعلاه.
...
| -------------------------- | ------------------------------------------------------ |
| `--name`                   | اسم الخدمة (مطلوب)                                     |
| `--region`                 | المنطقة، مثل `us-east-1` (مطلوب)                       |
| `--size`                   | حجم المثيل، مثل `m7i.2xlarge` (مطلوب)                  |
| `--provider`               | موفّر السحابة (الافتراضي: `aws`)                       |
| `--pg-version`             | الإصدار الرئيسي: `18`، `17`                            |
| `--ha-type`                | التوفّر العالي: `none`، `async`، `sync`                |
| `--tag`                    | وسم resource بصيغة `key` أو `key=value` (قابل للتكرار) |
| `--pg-config-file`         | مسار ملف JSON يحتوي على كائن `PgConfig`                |
| `--pg-bouncer-config-file` | مسار ملف JSON يحتوي على كائن `PgBouncerConfig`         |
...
أدِر ClickPipes لاستيعاب البيانات في ClickHouse Cloud من مصادر خارجية.
...
لكل نوع من أنواع المصادر أمر فرعي خاص به ضمن `clickpipe create`:
...
استخدم `clickhousectl cloud clickpipe create <source> --help` للحصول على القائمة الكاملة بالخيارات لكل نوع من أنواع المصادر.
...
تتبع رموز الخروج اتفاقيات `gh` CLI:

| الرمز | المعنى                                                                   |
| ----- | ------------------------------------------------------------------------ |
| `0`   | نجاح                                                                     |
| `1`   | خطأ (أي شيء غير مُصنَّف أدناه)                                           |
| `2`   | تم الإلغاء (أوقفه المستخدم)                                              |
| `4`   | المصادقة مطلوبة (لا توجد بيانات اعتماد، 401/403، والكتابة عبر OAuth فقط) |
...
<div id="non-interactive-flags">
  ### الخيارات غير التفاعلية
</div>

| الخيار            | الوصف                                                        |
| ----------------- | ------------------------------------------------------------ |
| `--agent <name>`  | ثبّت Skills لوكيل محدد (يمكن تكرار هذا الخيار)               |
| `--global`        | استخدم النطاق العام؛ وإذا لم يُحدَّد، فسيُستخدم نطاق المشروع |
...
يتحقق CLI أيضًا من التحديثات في الخلفية (بحد أقصى مرة واحدة كل 24 ساعة)، ويعرض إشعارًا عند توفر إصدار أحدث.