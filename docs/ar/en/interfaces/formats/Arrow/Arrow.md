...
| `input_format_arrow_use_native_reader`                                       | استخدم القارئ الأصلي في ClickHouse لتنسيقي `Arrow` و`ArrowStream` بدلًا من مكتبة Apache Arrow. اضبطه على `0` لاستخدام قارئ مكتبة Apache Arrow.                                                                                    | `1`         |
...
| `output_format_arrow_unsupported_types_as_binary`                            | أخرج أي نوع لا يملك مكافئًا في Arrow (مثل `BFloat16` و`AggregateFunction`) على هيئة بيانات ثنائية خام. إذا كانت القيمة `false`، فسيؤدي هذا النوع إلى إثارة استثناء. ينطبق ذلك على كلٍّ من الكاتب الأصلي وكاتب مكتبة Apache Arrow. | `1`         |
...
| `output_format_arrow_use_native_writer`                                      | استخدم الكاتب الأصلي في ClickHouse لتنسيقي `Arrow` و`ArrowStream` بدلًا من مكتبة Apache Arrow. اضبطه على `0` لاستخدام كاتب مكتبة Apache Arrow.                                                                                    | `1`         |
