...
`ArrowStream` هو تنسيق Apache Arrow في «وضع التدفق». وقد صُمم لمعالجة التدفقات في الذاكرة.
...
عن بُعد باستخدام `clickhouse-client` عبر المضيف `sql-clickhouse.clickhouse.com`
والمستخدم `demo` (من دون كلمة مرور). يوجد جدول `forex` في
قاعدة البيانات `forex`، لذا نحدّدها لتكون قاعدة البيانات الافتراضية:

```bash
clickhouse-client --secure --host sql-clickhouse.clickhouse.com --user demo --database forex
```

يخزّن جدول `forex` أسعار صرف العملات. يمكننا فحص حجمه
ومدى كفاءة ضغطه على القرص عبر الاستعلام عن [`system.columns`](/ar/operations/system-tables/columns):
...
على خلاف تنسيق [`Arrow`](/ar/interfaces/formats/Arrow) في &quot;وضع الملف&quot;، الذي
يتطلب اكتمال النتيجة بالكامل قبل أن يمكن قراءتها، يُرسَل `ArrowStream` على
شكل تسلسل من دفعات السجلات يمكن للمستهلك قراءتها تدريجيًا فور وصولها.
وهذا يجعله مناسبًا جدًا لبث نتيجة استعلام مباشرةً إلى أداة
تصور أو تحليلات دون الحاجة أولًا إلى تخزين مجموعة البيانات كاملةً.

لبث النتيجة، أرسِل الاستعلام عبر واجهة HTTP الخاصة بـ ClickHouse باستخدام
طلب `POST`، ثم اقرأ الاستجابة كتدفق Arrow. نعطّل الضغط
لمخرجات Arrow عبر الإعداد
[`output_format_arrow_compression_method`](/ar/operations/settings/formats#output_format_arrow_compression_method)
حتى يتمكن المستهلكون من فك ترميز الدفعات مباشرةً عند استلامها.

مخرجات `ArrowStream` هي بيانات ثنائية خام، لذلك بدلًا من طباعتها في
الطرفية، نمررها عبر تمرير إلى مستهلك. وهذا التدفق ذاتي الوصف (يحمل
مخططه بنفسه)، لذا نمرره هنا مباشرةً إلى
[`clickhouse-local`](/ar/operations/utilities/clickhouse-local)، الذي يقرأ
الدفعات الواردة باستخدام `--input-format ArrowStream` ويستعلم عنها كأنها جدول.
جدول `forex` كبير، لذلك قيّدنا الاستعلام عن بُعد باستخدام شرط `WHERE`
...
يمكن لأي عميل يدعم Arrow استهلاك نفس التدفّق تدريجيًا، إذ
يقرأه دفعةً تلو الأخرى بدلًا من تخزين النتيجة كاملةً مؤقتًا. على سبيل المثال،
باستخدام [مكتبة Apache Arrow لـ JavaScript](https://arrow.apache.org/docs/js/)، فإن
`RecordBatchReader` يُرجع كل دفعة سجلات فور تدفّقها من
الخادم:
...
للاطلاع على شرح كامل لكيفية بث بيانات `ArrowStream` من ClickHouse إلى
تصور في الوقت الفعلي باستخدام [Perspective](https://perspective.finos.org/)، راجع
مقال المدونة
[بث تصورات في الوقت الفعلي باستخدام ClickHouse وApache Arrow وPerspective](https://clickhouse.com/blog/streaming-real-time-visualizations-clickhouse-apache-arrow-perpsective).

<div id="format-settings">
  ## إعدادات التنسيق
</div>

يستخدم `ArrowStream` إعدادات التنسيق نفسها المستخدمة في تنسيق [`Arrow`](/ar/interfaces/formats/Arrow).
...
| `input_format_arrow_import_nested`                                           | إعداد متقادم، ولا يفعل شيئًا.                                                                                                                                                                               | `0`         |
...
| `output_format_arrow_date_as_uint16`                                         | كتابة قيم Date كأرقام عادية من 16 بت (تُقرأ لاحقًا كـ UInt16)، بدلًا من تحويلها إلى نوع Arrow DATE32 بحجم 32 بت (تُقرأ لاحقًا كـ Date32).                                                                   | `0`         |
...
| `output_format_arrow_unsupported_types_as_binary`                            | إخراج نوع ليس له مكافئ في Arrow (مثل `BFloat16` و`AggregateFunction`) كبيانات ثنائية خام. إذا كانت القيمة false، فإن هذا النوع يثير استثناءً. ينطبق ذلك على كلٍّ من الكاتب الأصلي وكاتب مكتبة Apache Arrow. | `1`         |
