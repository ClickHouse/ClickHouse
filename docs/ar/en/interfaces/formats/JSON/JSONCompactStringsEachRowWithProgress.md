---
alias: []
description: 'وثائق تنسيق JSONCompactStringsEachRowWithProgress'
input_format: false
...
مشابه لـ [`JSONCompactEachRowWithProgress`](/ar/interfaces/formats/JSONCompactEachRowWithProgress)، لكن جميع القيم تُحوَّل إلى سلاسل نصية.
ويفيد ذلك عندما تحتاج إلى تمثيل نصي متسق لجميع أنواع البيانات.

الميزات الرئيسية:

* البنية نفسها لـ `JSONCompactEachRowWithProgress`
* تُمثَّل جميع القيم كسلاسل نصية (فالأرقام والمصفوفات وغيرها كلها سلاسل نصية بين علامتَي اقتباس)
* يتضمن تحديثات التقدّم، والإجماليات، ومعالجة الاستثناءات
* مفيد للعملاء الذين يفضّلون البيانات المعتمدة على السلاسل النصية أو يتطلبونها
