---
alias: []
description: 'توثيق لتنسيق JSONCompactColumns'
input_format: true
...
في هذا التنسيق، تُمثَّل جميع البيانات كمصفوفة JSON واحدة.

:::note
يخزّن تنسيق الإخراج `JSONCompactColumns` جميع البيانات مؤقتًا في الذاكرة لإخراجها ككتلة واحدة، مما قد يؤدي إلى ارتفاع استهلاك الذاكرة.
:::
...
اقرأ البيانات باستخدام التنسيق `JSONCompactColumns`:
...
ستُملأ الأعمدة غير الموجودة في `block` بالقيم الافتراضية (يمكنك هنا استخدام الإعداد [`input_format_defaults_for_omitted_fields`](/ar/operations/settings/settings-formats.md/#input_format_defaults_for_omitted_fields))
