---
description: 'توثيق تعليمة ALTER TABLE ... MODIFY QUERY'
sidebar_label: 'VIEW'
sidebar_position: 50
...
يمكنك تعديل استعلام `SELECT` الذي جرى تحديده عند إنشاء [عرض مادي](/ar/sql-reference/statements/create/view#materialized-view) باستخدام التعليمة `ALTER TABLE ... MODIFY QUERY` من دون مقاطعة عملية إدخال البيانات.

صُمم هذا الأمر لتغيير عرض مادي أُنشئ باستخدام العبارة `TO [db.]name`. وهو لا يغيّر بنية جدول التخزين الأساسي، كما لا يغيّر تعريف الأعمدة في العرض المادي، ولذلك يكون استخدام هذا الأمر محدودًا جدًا مع العروض المادية التي أُنشئت من دون العبارة `TO [db.]name`.
...
هذا الأسلوب محدود جدًا، إذ لا يمكنك سوى تغيير قسم `SELECT` من دون إضافة أعمدة جديدة.
...
تُغيّر `ALTER TABLE ... MODIFY REFRESH` معلمات تحديث [العرض المادي القابل للتحديث](../create/view.md#refreshable-materialized-view)، بما في ذلك الجدول الزمني والتبعيات والتوزيع العشوائي و[إعدادات التحديث](../create/view.md#refresh-settings).

```sql
ALTER TABLE [db.]name MODIFY REFRESH EVERY|AFTER ... [RANDOMIZE FOR ...] [DEPENDS ON ...] [SETTINGS ...]
```

الجدول الزمني (`EVERY` أو `AFTER`) إلزامي: تستبدل هذه التعليمة *جميع* معلمات التحديث دفعةً واحدة. وأي بند غير محدد — `RANDOMIZE FOR` أو `DEPENDS ON` أو `SETTINGS` — يُزال أو يُعاد ضبطه إلى القيم الافتراضية. لتغيير إعدادات التحديث فقط، أعد ذكر الجدول الزمني الحالي.
...
* `ALTER TABLE ... MODIFY SETTING` غير مدعوم للعروض المادية؛ ولا يمكن تغيير إعدادات التحديث إلا عبر `MODIFY REFRESH`.
* إضافة `APPEND` أو إزالته غير مدعومة.
* لا يمكن تغيير إعداد التحديث `all_replicas` بعد إنشاء العرض.

القائمة الكاملة لإعدادات التحديث موثقة في [إعدادات التحديث](../create/view.md#refresh-settings). كما تظهر حالة التحديث، بما في ذلك الإعدادات المطبقة حاليًا، في [`system.view_refreshes`](../../../operations/system-tables/view_refreshes.md).