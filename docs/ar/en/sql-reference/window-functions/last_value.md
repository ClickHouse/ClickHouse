...
تعيد آخر قيمة جرى تقييمها ضمن الإطار المرتب الخاص بها. افتراضيًا، يتم تجاهل الوسيطات ذات القيمة `NULL`، إلا أنه يمكن استخدام المُعدِّل `RESPECT NULLS` لتجاوز هذا السلوك.
...
:::note
يضمن استخدام المُعدِّل الاختياري `RESPECT NULLS` بعد `first_value(column_name)` عدم تخطي الوسائط `NULL`.
...
* آخر قيمة جرى تقييمها ضمن إطارها المرتَّب.

**مثال**

في هذا المثال، تُستخدم الدالة `last_value` للعثور على لاعب كرة القدم الأقل أجرًا ضمن مجموعة بيانات خيالية لرواتب لاعبي الدوري الإنجليزي الممتاز.
