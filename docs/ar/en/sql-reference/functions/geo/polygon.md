---
description: 'توثيق Polygon'
sidebar_label: 'مضلع'
slug: /sql-reference/functions/geo/polygons
title: 'دوال للتعامل مع المضلعات'
doc_type: 'reference'
---
...
يعيد كائنًا هندسيًا بتنسيق WKT ‏(Well Known Text) من [أنواع البيانات الجغرافية](../../data-types/geo.md) المختلفة. كائنات WKT المدعومة هي:
...
يمكن أن تكون `geo_data` أحد [أنواع البيانات الجغرافية](../../data-types/geo.md) التالية أو أنواعها الأولية الأساسية:
...
* يُعاد الكائن الهندسي WKT `POINT` للنوع Point.
* يُعاد الكائن الهندسي WKT `POLYGON` للنوع Polygon
* يُعاد الكائن الهندسي WKT `MULTIPOLYGON` للنوع MultiPolygon.
* يُعاد الكائن الهندسي WKT `LINESTRING` للنوع LineString.
* يُعاد الكائن الهندسي WKT `MULTILINESTRING` للنوع MultiLineString.
...
POLYGON من مصفوفة من الأزواج المرتبة أو من مصفوفة من مصفوفات الأزواج المرتبة:
...
MULTIPOLYGON من مصفوفة تحتوي على مصفوفات Tuple متعددة الأبعاد:
...
يحوّل قيمة MultiPolygon بتنسيق WKT ‏(Well Known Text) إلى النوع Polygon.
...
تقوم الدالة `readWKTPoint` في ClickHouse بتحليل تمثيل Well-Known Text ‏(WKT) لهندسة من نوع Point وتُرجع نقطة بتنسيق ClickHouse الداخلي.
...
* `wkt_string`: سلسلة WKT المُدخلة التي تُمثّل هندسة من النوع `Point`.
...
يحلّل تمثيل Well-Known Text ‏(WKT) لكائن LineString الهندسي ويُعيده بتنسيق ClickHouse الداخلي.
...
* `wkt_string`: سلسلة WKT المُدخلة التي تُمثّل عنصرًا هندسيًا من نوع LineString.

<div id="returned-value-2">
  ### القيمة المعادة
</div>

تعيد الدالة التمثيل الداخلي في ClickHouse لهندسة LineString.
...
يحلّل تمثيل Well-Known Text ‏(WKT) لهندسة من نوع MultiLineString ويُعيدها بتنسيق ClickHouse الداخلي.
...
* `wkt_string`: سلسلة WKT المُدخلة التي تُمثّل هندسة من نوع MultiLineString.

<div id="returned-value">
  ### القيمة المُعادة
</div>

تعيد الدالة التمثيل الداخلي في ClickHouse لهندسة من نوع multilinestring.
...
يحلّل تمثيل Well-Known Text ‏(WKT) لهندسة من نوع Polygon ويُعيد Ring (سلسلة خطوط مغلقة) بالتنسيق الداخلي لـ ClickHouse.
...
* `wkt_string`: سلسلة WKT المُدخلة التي تمثل شكلاً هندسيًا من نوع Polygon.

<div id="returned-value-2">
  ### القيمة المعادة
</div>

تُرجع الدالة التمثيل الداخلي في ClickHouse لهندسة الحلقة (closed linestring).
...
تُرجِع true أو false وفقًا لما إذا كان أحد المضلعات يقع بالكامل داخل مضلع آخر أم لا. المرجع: https://www.boost.org/doc/libs/1&#95;62&#95;0/libs/geometry/doc/html/geometry/reference/algorithms/within/within&#95;2.html
...
يحوّل قيمة MultiPolygon بتنسيق WKB ‏(Well Known Binary) إلى نوع MultiPolygon.
...
قيمة `String` تبدأ بـ `MULTIPOLYGON`
...
يحوّل قيمة من نوع MultiPolygon بتنسيق WKB ‏(Well Known Binary) إلى نوع Polygon.
...
تقوم الدالة `readWKBPoint` في ClickHouse بتحليل تمثيل Well-Known Binary ‏(WKB) لهندسة Point وتُرجع نقطة بالتنسيق الداخلي في ClickHouse.
...
* `wkb_string`: سلسلة WKB المُدخلة التي تمثل كائنًا هندسيًا من نوع Point.

<div id="returned-value-2">
  ### القيمة المعادة
</div>

تعيد الدالة التمثيل الداخلي في ClickHouse للشكل الهندسي Point.
...
يحلّل تمثيل Well-Known Binary ‏(WKB) لكائن هندسي من نوع LineString ويُعيده بالتنسيق الداخلي لـ ClickHouse.
...
* `wkb_string`: سلسلة WKB المُدخلة التي تمثل كائنًا هندسيًا من نوع LineString.

<div id="returned-value">
  ### القيمة المُعادة
</div>

تُرجع الدالة تمثيل ClickHouse الداخلي للكائن الهندسي LineString.
...
يحلّل تمثيل Well-Known Binary ‏(WKB) لكائن MultiLineString الهندسي ويُرجعه بتنسيق ClickHouse الداخلي.
...
* `wkb_string`: سلسلة WKB المُدخلة التي تُمثّل شكلًا هندسيًا من نوع MultiLineString.
...
UInt8، 0 عند false و1 عند true

<div id="polygonsdistancespherical">
  ## polygonsDistanceSpherical
</div>

تحسب الحد الأدنى للمسافة بين نقطتين، بحيث تقع إحدى النقطتين ضمن المضلع الأول وتقع الأخرى ضمن مضلع آخر. والمقصود بـ «الكروية» هنا أن الإحداثيات تُفسَّر على أنها إحداثيات على كرة مثالية خالصة، وهو ما لا ينطبق على الأرض. يسرّع استخدام هذا النوع من أنظمة الإحداثيات التنفيذ، لكنه بالطبع غير دقيق.
...
يعيد true إذا كان المضلعان متساويين
...
يحسب الفرق المتماثل المكاني وفق نظرية المجموعات (XOR) بين مضلعين
...
مماثل لـ `polygonsSymDifferenceSpherical`، لكن الإحداثيات تكون في نظام الإحداثيات الديكارتية؛ وهو أقرب إلى نموذج الأرض الحقيقي.
...
يحسب تقاطع (AND) بين المضلعات، حيث تكون الإحداثيات كروية.
...
يعيد true إذا كان المضلع الثاني يقع داخل المضلع الأول.
...
UInt8، حيث تمثل 0 القيمة false وتمثل 1 القيمة true

<div id="polygonsintersectcartesian">
  ## polygonsIntersectCartesian
</div>

تعيد true إذا كان المضلعان يتقاطعان (أي إذا كان بينهما أي مساحة مشتركة أو حد مشترك).
...
UInt8، حيث 0 لـ false و1 لـ true

<div id="polygonsintersectspherical">
  ## polygonsIntersectSpherical
</div>

تعيد true إذا كان المضلعان يتقاطعان (أي يشتركان في أي مساحة أو في أي جزء من الحدود). المرجع: https://www.boost.org/doc/libs/1&#95;62&#95;0/libs/geometry/doc/html/geometry/reference/algorithms/intersects.html
...
يحسب الغلاف المحدب. [مرجع](https://www.boost.org/doc/libs/1_61_0/libs/geometry/doc/html/geometry/reference/algorithms/convex_hull.html)

الإحداثيات ضمن نظام الإحداثيات الديكارتي.
...
<div id="polygon-representing-zimbabwe">
  #### Polygon يمثّل زيمبابوي
</div>

هذا هو Polygon الذي يمثّل زيمبابوي:
...
لمزيد من المعلومات حول أنظمة البيانات الهندسية، راجع هذا [العرض التقديمي](https://archive.fosdem.org/2020/schedule/event/working_with_spatial_trajectories_in_boost_geometry/attachments/slides/3988/export/events/attachments/working_with_spatial_trajectories_in_boost_geometry/slides/3988/FOSDEM20_vissarion.pdf) عن مكتبة Boost التي يستخدمها ClickHouse.