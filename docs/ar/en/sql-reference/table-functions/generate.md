---
description: 'يولّد بيانات عشوائية وفق مخطط محدد. يتيح تعبئة جداول الاختبار
  بهذه البيانات. ليست كل الأنواع مدعومة.'
...
يولّد بيانات عشوائية وفق مخطط محدد.
يتيح تعبئة جداول الاختبار بهذه البيانات.
ليست كل الأنواع مدعومة.
...
| الوسيطة             | الوصف                                                                                              |
| ------------------- | -------------------------------------------------------------------------------------------------- |
| `name`              | اسم العمود المقابل.                                                                                |
| `TypeName`          | نوع العمود المقابل.                                                                                |
| `random_seed`       | حدِّد البذرة العشوائية يدويًا للحصول على نتائج ثابتة. إذا كانت `NULL` — فستُولَّد البذرة عشوائيًا. |
| `max_string_length` | الحد الأقصى لطول جميع السلاسل المُولَّدة. القيمة الافتراضية هي `10`.                               |
| `max_array_length`  | الحد الأقصى لعدد العناصر في جميع المصفوفات أو الخرائط المُولَّدة. القيمة الافتراضية هي `10`.       |

<div id="returned_value">
  ## القيمة المُعادة
</div>

كائن جدول بالمخطط المطلوب.
...
بالاقتران مع [generateRandomStructure](../../sql-reference/functions/other-functions.md#generateRandomStructure):
...
باستخدام البذرة العشوائية لكلٍّ من البنية العشوائية والبيانات العشوائية:
...
:::note
قد يؤدي `generateRandom(generateRandomStructure(), [random seed], max_string_length, max_array_length)` عند ضبط `max_array_length` على قيمة كبيرة بما يكفي إلى إنشاء مخرجات هائلة جدًا بسبب العمق المحتمل الكبير للتداخل (حتى 16) في الأنواع المعقدة (`Array`, `Tuple`, `Map`, `Nested`).
:::
...
* مدونة: [إنشاء بيانات عشوائية في ClickHouse](https://clickhouse.com/blog/generating-random-test-distribution-data-for-clickhouse)