---
description: 'نظرة عامة على بنية بيانات متداخلة في ClickHouse'
...
بنية بيانات متداخلة تشبه جدولًا داخل خلية. تُحدَّد معلمات بنية بيانات متداخلة — أي أسماء الأعمدة وأنواعها — بالطريقة نفسها المتبعة في استعلام [CREATE TABLE](../../../sql-reference/statements/create/table.md). ويمكن أن يقابل كل صف في الجدول أي عدد من الصفوف في بنية بيانات متداخلة.

:::tip[تجنّب استخدام النقاط في أسماء الأعمدة]
يمكن تفسير أسماء الأعمدة التي تحتوي على نقاط، والأعمدة التي تشترك في بادئة نقطية، والأعمدة من النوع `Array`، على أنها جزء من بنية `Nested` مسطّحة عندما تكون `flatten_nested = 1` (القيمة الافتراضية). وقد يؤدي ذلك إلى تحقق غير متوقع من طول المصفوفات عند عمليات الإدراج وفرض قيود على إعادة التسمية.
...
تُعرِّف عبارة DDL ‏`CREATE TABLE` أعلاه بنية بيانات متداخلة `Goals`، التي تحتوي على بيانات حول التحويلات، أي الأهداف التي تم تحقيقها.
ويقابل كل صف في جدول &#39;visits&#39; صفراً أو أكثر من التحويلات.

عندما يُضبط الإعداد [`flatten_nested`](/ar/operations/settings/settings#flatten_nested) على `0` (القيمة الافتراضية هي `flatten_nested=1`)، تصبح مستويات التداخل بأي عمق مدعومة.

في معظم الحالات، عند العمل مع بنية بيانات متداخلة، تُحدَّد أعمدتها باستخدام أسماء الأعمدة المفصولة بنقطة.
وتُكوِّن هذه الأعمدة مصفوفة من أنواع متطابقة.
ويكون طول جميع مصفوفات الأعمدة ضمن بنية بيانات متداخلة نفسها متساوياً.
...
:::tip
من الأسهل تصوّر بنية بيانات متداخلة على أنها مجموعة من عدة مصفوفات للأعمدة متساوية الطول.
:::
...
لأن كل عمود في بنية `Nested` يُخزَّن على هيئة `Array`، فإن الإشارة إليه في عبارة `WHERE` تمنحك المصفوفة كاملة لكل صف، لا عنصرًا منفردًا. ولا يمكنك مقارنة عمود متداخل مباشرةً بقيمة scalar، لذا يجب استخدام [دوال المصفوفات](/ar/sql-reference/functions/array-functions) بدلًا من ذلك.
...
استخدم [`has`](/ar/sql-reference/functions/array-functions#has) للتحقق مما إذا كانت المصفوفة تحتوي على قيمة معيّنة:
...
استخدم [`arrayExists`](/ar/sql-reference/functions/array-functions#arrayExists) إذا كان الشرط أكثر تعقيدًا:
...
لتصفية العناصر الفردية داخل بنية متداخلة بدلًا من تصفية الصفوف كاملةً، استخدم `ARRAY JOIN` لبسط المصفوفات أولًا.
بعد `ARRAY JOIN`، يصبح كل عنصر صفًا مستقلًا، لذا تُطبَّق عبارة `WHERE` على القيم المفردة.
لمزيد من المعلومات، راجع [عبارة `ARRAY JOIN`](/ar/sql-reference/statements/select/array-join). مثال:
...
لا يمكنك تنفيذ `SELECT` على بنية بيانات متداخلة كاملة. يمكنك فقط تحديد الأعمدة الفردية التي تُشكّل جزءًا منها صراحةً.

<div id="inserting-data">
  ### إدراج البيانات
</div>

بالنسبة إلى استعلام `INSERT`، يجب تمرير جميع مصفوفات أعمدة المكوّنات الخاصة ببنية بيانات متداخلة كلًّا على حدة (كما لو كانت مصفوفات أعمدة منفصلة). أثناء الإدراج، يتحقق النظام من أن أطوالها متساوية.
...
يجب أن تتساوى أطوال جميع مصفوفات الأعمدة الفرعية المتداخلة ضمن الصف الواحد. وأي اختلاف في الأطوال يؤدي إلى خطأ:
...
في استعلام `DESCRIBE`، تُسرد الأعمدة في بنية بيانات متداخلة كلٌّ على حدة، بالطريقة نفسها.

<div id="alter-limitations">
  ### قيود ALTER
</div>

تخضع استعلامات `ALTER` على بنية بيانات متداخلة للقيود التالية:

**تتم إضافة الأعمدة الفرعية** بشكل طبيعي. يمكنك إضافة عمود فرعي جديد إلى بنية `Nested` موجودة:

```sql
ALTER TABLE test.visits ADD COLUMN Goals.Revenue Float64;
```

**حذف الأعمدة الفرعية** مدعوم للأعمدة الفرعية الفردية:

```sql
ALTER TABLE test.visits DROP COLUMN Goals.Revenue;
```

**تعديل النوع** لعمود فرعي مدعوم ويؤدي إلى تنفيذ mutation (إعادة كتابة البيانات):

```sql
ALTER TABLE test.visits MODIFY COLUMN Goals.Price Int32;
```

**إعادة التسمية** لها قيود. يمكنك إعادة تسمية عمود فرعي ضمن البنية المتداخلة نفسها:
...
* إعادة تسمية البنية المتداخلة بالكامل (على سبيل المثال، من `Goals` إلى `Conversions`).
* نقل عمود فرعي إلى بنية متداخلة أخرى (على سبيل المثال، من `Goals.ID` إلى `OtherNested.ID`).
* نقل عمود فرعي من بنية متداخلة أو إليها (على سبيل المثال، من `Goals.ID` إلى `GoalID` أو العكس).