---
description: 'توثيق لنوع البيانات Dynamic في ClickHouse، والذي يمكنه تخزين قيم من أنواع
  مختلفة في عمود واحد'
...
يتيح هذا النوع تخزين قيم من أي نوع دون الحاجة إلى معرفة جميع الأنواع مسبقًا.
...
حيث إن `N` مَعْلَمة اختيارية بين `0` و`254` تحدد عدد أنواع البيانات المختلفة التي يمكن تخزينها كأعمدة فرعية منفصلة داخل عمود من النوع `Dynamic` ضمن كتلة واحدة من البيانات المخزنة بشكل منفصل (على سبيل المثال ضمن جزء البيانات واحد لجدول MergeTree). وإذا تم تجاوز هذا الحد، فستُخزَّن جميع القيم ذات الأنواع الجديدة معًا في بنية بيانات مشتركة خاصة بصيغة ثنائية. والقيمة الافتراضية لـ `max_types` هي `32`.

<div id="creating-dynamic">
  ## إنشاء Dynamic
</div>

استخدام النوع `Dynamic` في تعريف عمود بجدول:
...
<div id="reading-dynamic-nested-types-as-subcolumns">
  ## قراءة الأنواع المتداخلة في `Dynamic` كأعمدة فرعية
</div>

يدعم النوع `Dynamic` قراءة نوع متداخل واحد من عمود `Dynamic` باستخدام اسم النوع كعمود فرعي.
لذا، إذا كان لديك عمود `d Dynamic`، فيمكنك قراءة عمود فرعي من أي نوع صالح `T` باستخدام الصياغة `d.T`.
وسيكون نوع هذا العمود الفرعي هو `Nullable(T)` إذا كان يمكن أن يأتي `T` داخل `Nullable`، وإلا فسيكون `T`. كما سيكون هذا العمود الفرعي
بالحجم نفسه للعمود الأصلي `Dynamic`، وسيحتوي على قيم `NULL` (أو قيم فارغة إذا تعذّر أن يأتي `T` داخل `Nullable`)
في جميع الصفوف التي لا يكون فيها العمود الأصلي `Dynamic` من النوع `T`.

يمكن أيضًا قراءة الأعمدة الفرعية في `Dynamic` باستخدام الدالة `dynamicElement(dynamic_column, type_name)`.
...
لمعرفة أي نوع فعلي مخزَّن في كل صف، يمكن استخدام الدالة `dynamicType(dynamic_column)`. تُرجع هذه الدالة قيمة من النوع `String` تحتوي على اسم نوع القيمة لكل صف (أو `'None'` إذا كانت قيمة الصف `NULL`).
...
توجد 4 عمليات تحويل يمكن إجراؤها مع عمود `Dynamic`.

<div id="converting-an-ordinary-column-to-a-dynamic-column">
  ### تحويل عمود عادي إلى عمود Dynamic
</div>
...
لتحليل قيم من النوع `Dynamic` انطلاقًا من عمود `String`، يمكنك تفعيل الإعداد `cast_string_to_dynamic_use_inference`:
...
يمكن تحويل عمود `Dynamic` إلى عمود عادي. في هذه الحالة، ستُحوَّل جميع الأنواع المتداخلة إلى نوعٍ هدف:
...
<div id="converting-a-dynamicmax_typesn-column-to-another-dynamicmax_typesk">
  ### تحويل عمود من النوع `Dynamic(max_types=N)` إلى `Dynamic(max_types=K)` آخر
</div>

إذا كان `K >= N`، فلن تتغير البيانات أثناء عملية التحويل:
...
إذا كان `K < N`، فستُدرَج القيم ذات الأنواع الأقل شيوعًا في عمود فرعي خاص واحد، لكن سيظل بالإمكان الوصول إليها:
...
تُرجِع الدالة `isDynamicElementInSharedData` القيمة `true` للصفوف المخزَّنة في بنية بيانات مشتركة خاصة داخل `Dynamic`، وكما نرى، فإن العمود الناتج لا يحتوي إلا على نوعين غير مخزَّنين في بنية البيانات المشتركة.
...
تدعم جميع التنسيقات النصية (TSV, CSV, CustomSeparated, Values, JSONEachRow, etc) قراءة النوع `Dynamic`. وأثناء تحليل البيانات، يحاول ClickHouse استنتاج نوع كل قيمة واستخدامه عند إدراجها في عمود `Dynamic`.
...
إذا تعذر تنفيذ الدالة على أحد الأنواع داخل العمود `Dynamic`، فسيتم طرح الاستثناء:
...
يمكننا تصفية الأنواع غير المطلوبة:
...
يتحكّم الإعداد `dynamic_throw_on_type_mismatch` في ما يحدث عند تطبيق دالة على عمود `Dynamic` وكان النوع المخزَّن الفعلي في أحد الصفوف غير متوافق مع الدالة:

* `true` (الافتراضي) — إصدار استثناء (`ILLEGAL_TYPE_OF_ARGUMENT`) عند أول صف غير متوافق.
...
تُحدَّد نتيجة المعامل `<` للقيمتين `d1` ذات النوع الأساسي `T1` و`d2` ذات النوع الأساسي `T2` من النوع `Dynamic` كما يلي:
...
لا يُسمح افتراضيًا باستخدام النوع `Dynamic` في مفاتيح `GROUP BY`/`ORDER BY`. وإذا أردت استخدامه، فضع قاعدة المقارنة الخاصة به في الاعتبار وفعّل الإعدادين `allow_suspicious_types_in_group_by`/`allow_suspicious_types_in_order_by`.
...
**ملاحظة:** تُعَدّ قيم أنواع Dynamic ذات الأنواع الرقمية المختلفة قيماً مختلفة، ولا تُقارَن ببعضها، بل تُقارَن أسماء الأنواع بدلاً منها.
...
**ملاحظة:** لا تُطبَّق قاعدة المقارنة الموضَّحة عند تنفيذ دوال المقارنة مثل `<`/`>`/`=` وغيرها، بسبب [الطريقة الخاصة](#using-dynamic-type-in-functions) لعمل الدوال مع النوع `Dynamic`

<div id="reaching-the-limit-in-number-of-different-data-types-stored-inside-dynamic">
  ## بلوغ الحد الأقصى لعدد أنواع البيانات المختلفة المخزّنة داخل Dynamic
</div>

لا يمكن لنوع البيانات `Dynamic` تخزين سوى عدد محدود من أنواع البيانات المختلفة كأعمدة فرعية منفصلة. يكون هذا الحد افتراضيًا 32، ولكن يمكنك تغييره في تعريف النوع باستخدام الصياغة `Dynamic(max_types=N)`، حيث تكون N بين 0 و254 (ونظرًا إلى تفاصيل التنفيذ، يستحيل وجود أكثر من 254 نوع بيانات مختلفًا يمكن تخزينه كأعمدة فرعية منفصلة داخل Dynamic).
عند بلوغ هذا الحد، ستُدرج جميع أنواع البيانات الجديدة التي تُدرج في العمود `Dynamic` داخل بنية بيانات مشتركة واحدة تخزّن القيم ذات أنواع البيانات المختلفة بصيغة ثنائية.

لنرَ ما الذي يحدث عند بلوغ هذا الحد في سيناريوهات مختلفة.

<div id="reaching-the-limit-during-data-parsing">
  ### بلوغ الحد أثناء تحليل البيانات
</div>

أثناء تحليل قيم `Dynamic` من البيانات، عند بلوغ الحد الأقصى في كتلة البيانات الحالية، ستُدرج جميع القيم الجديدة في بنية البيانات المشتركة:
...
كما نرى، بعد إدراج 3 أنواع بيانات مختلفة هي `Int64` و`Array(Int64)` و`String`، أُدرجت جميع الأنواع الجديدة في البنية الخاصة للبيانات المشتركة.

<div id="during-merges-of-data-parts-in-mergetree-table-engines">
  ### أثناء عمليات دمج أجزاء البيانات في محركات جداول MergeTree
</div>

أثناء دمج عدة أجزاء بيانات في جدول MergeTree، قد يصل العمود `Dynamic` في جزء البيانات الناتج إلى الحد الأقصى لعدد أنواع البيانات المختلفة التي يمكن تخزينها داخله كأعمدة فرعية منفصلة، ولن يعود قادرًا على تخزين جميع الأنواع كأعمدة فرعية من أجزاء المصدر.
في هذه الحالة، يختار ClickHouse الأنواع التي ستبقى كأعمدة فرعية منفصلة بعد الدمج، والأنواع التي ستُدرج في بنية بيانات مشتركة. وفي معظم الحالات، يحاول ClickHouse الاحتفاظ بالأنواع الأكثر شيوعًا وتخزين الأنواع الأقل شيوعًا في بنية بيانات مشتركة، لكن ذلك يعتمد على آلية التنفيذ.

لنرَ مثالًا على هذا النوع من الدمج. أولًا، لنُنشئ جدولًا يحتوي على عمود `Dynamic`، ونضبط حد أنواع البيانات المختلفة على `3`، ثم نُدرج قيَمًا من `5` أنواع مختلفة:
...
سينشئ كل `insert` جزءًا منفصلًا من البيانات مع عمود `Dynamic` يحتوي على نوع واحد:
...
الآن، لندمج جميع الأجزاء معًا في جزء واحد ونرَ ما الذي سيحدث:
...
كما نرى، احتفظ ClickHouse بالنوعين الأكثر شيوعًا `UInt64` و`Array(UInt64)` كأعمدة فرعية، وأدخل جميع الأنواع الأخرى في البيانات المشتركة.
...
<div id="binary-output-format">
  ### تنسيق الإخراج الثنائي
</div>

في تنسيق RowBinary، تُسلسَل قيم النوع `Dynamic` وفق التنسيق التالي:
