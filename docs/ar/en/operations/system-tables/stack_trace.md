...
keywords: ['جدول نظام', 'stack_trace']
...
يحتوي على تتبعات المكدس لجميع خيوط الخادم. ويتيح للمطورين فحص الحالة الداخلية للخادم.

لتحليل إطارات المكدس، استخدم `addressToLine` و`addressToLineWithInlines` و`addressToSymbol` و`demangle` [دوال الاستبطان](../../sql-reference/functions/introspection.md).
...
* `thread_name` ([String](/ar/sql-reference/data-types/string)) — اسم الخيط.
* `thread_id` ([UInt64](/ar/sql-reference/data-types/int-uint)) — معرّف الخيط.
* `query_id` ([String](/ar/sql-reference/data-types/string)) — معرّف الاستعلام الذي ينتمي إليه هذا الخيط.
* `trace` ([Array(UInt64)](/ar/sql-reference/data-types/array)) — الـ stacktrace لهذا الخيط. وهو في الأساس مجرد مصفوفة من العناوين.
* `untracked_memory` ([Int64](/ar/sql-reference/data-types/int-uint)) — عدّاد غير ذري لكل خيط لتخصيصات الذاكرة التي لم تُمرَّر بعد إلى MemoryTracker الأب. وقد تكون القيمة سالبة إذا جرى تحرير ذاكرة أكثر مما خُصِّص منذ آخر عملية تفريغ.
...
الحصول على الرموز من ملفات الكائنات الخاصة بـ ClickHouse:
...
* [دوال الاستبطان](../../sql-reference/functions/introspection.md) — ما دوال الاستبطان المتاحة وكيفية استخدامها.
* [system.trace&#95;log](../system-tables/trace_log.md) — يحتوي على تتبعات المكدس التي يجمعها محلل الاستعلامات بأخذ العينات.
* [arrayMap](/ar/sql-reference/functions/array-functions#arrayMap)) — وصف ومثال استخدام للدالة `arrayMap`.
* [arrayFilter](/ar/sql-reference/functions/array-functions#arrayFilter) — وصف ومثال استخدام للدالة `arrayFilter`.