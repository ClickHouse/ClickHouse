---
description: 'جدول نظامي مفيد لخبراء C++ ولمهندسي ClickHouse، ويحتوي على معلومات لاستبطان ملف `clickhouse` التنفيذي.'
keywords: ['جدول نظامي', 'الرموز']
...
يحتوي على معلومات لاستبطان الملف التنفيذي `clickhouse`. ويتطلب الوصول إليه امتياز الاستبطان.
ولا يفيد هذا الجدول إلا خبراء C++ ومهندسي ClickHouse.
...
* `symbol` ([String](/ar/sql-reference/data-types/string)) — اسم الرمز في الملف التنفيذي. وهو مُشوَّه الاسم. يمكنك تطبيق demangle(symbol) للحصول على اسم مقروء.
* `symbol_demangled` ([Nullable(String)](/ar/sql-reference/data-types/nullable)) — الرمز بعد فكّ التشويه، ويُستخدم في instrumentation الخاصة بـ XRay.
* `function_id` ([Nullable(Int32)](/ar/sql-reference/data-types/nullable)) — معرّف الدالة في خريطة instrumentation الخاصة بـ XRay.
* `address_begin` ([UInt64](/ar/sql-reference/data-types/int-uint)) — عنوان بداية الرمز في الملف التنفيذي.
* `address_end` ([UInt64](/ar/sql-reference/data-types/int-uint)) — عنوان نهاية الرمز في الملف التنفيذي.
