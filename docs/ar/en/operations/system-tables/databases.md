---
description: 'جدول نظامي يحتوي على معلومات عن قواعد البيانات المتاحة
  للمستخدم الحالي.'
...
* `name` ([String](/ar/sql-reference/data-types/string)) — اسم قاعدة البيانات.
* `engine` ([String](/ar/sql-reference/data-types/string)) — محرك قاعدة البيانات.
* `data_path` ([String](/ar/sql-reference/data-types/string)) — مسار البيانات.
* `metadata_path` ([String](/ar/sql-reference/data-types/string)) — مسار البيانات الوصفية.
* `uuid` ([UUID](/ar/sql-reference/data-types/uuid)) — معرّف UUID لقاعدة البيانات.
* `engine_full` ([String](/ar/sql-reference/data-types/string)) — معلمات محرك قاعدة البيانات.
* `comment` ([String](/ar/sql-reference/data-types/string)) — تعليق قاعدة البيانات.
* `is_external` ([UInt8](/ar/sql-reference/data-types/int-uint)) — قاعدة البيانات خارجية (أي PostgreSQL/DataLakeCatalog).

**الأسماء المستعارة:**

* `database` — اسم مستعار لـ `name`.

{/*AUTOGENERATED_END*/ }

يُستخدم العمود `name` من الجدول النظامي هذا لتنفيذ الاستعلام `SHOW DATABASES`.
...
تحقّق من جميع قواعد البيانات المتاحة لهذا المستخدم.
