...
title: 'قياسات أداء'
description: 'قياسات أداء تقارن بين ClickHouse Managed Postgres وAWS Aurora وRDS وخدمات PostgreSQL المُدارة الأخرى'
...
<Info>
  ...
  * **أُجري اختبار أداء** لخدمة ClickHouse Managed Postgres مقارنةً مع AWS RDS ‏(16k provisioned IOPS) وAurora IO Optimized باستخدام اختبارات [`pgbench`](https://www.postgresql.org/docs/current/pgbench.html) القياسية
  * **الأداء**: يوفّر ClickHouse Managed Postgres، والمدعوم بتخزين NVMe، **أداءً أسرع بمقدار 4.3 إلى 9 مرات** في workloads كثيفة الـIO، و**أسرع بنسبة 12%** في السيناريوهات المقيّدة بالـ CPU
...
**التحليل**: تُبرز أعباء العمل المختلطة بين القراءة والكتابة أوضح مزايا الأداء التي يوفرها تخزين NVMe، وتمثل **السيناريو الأكثر واقعية لأعباء العمل سريعة النمو أعباء العمل المدفوعة بالذكاء الاصطناعي** التي تتطلب كلاً من استيعاب البيانات بمعدل نقل مرتفع وعمليات قراءة بزمن استجابة منخفض. وقد حقق **ClickHouse Managed Postgres معدل 19.8K TPS مع تزامن أعلى**، مما يوضح كيف يتوسع تخزين NVMe بكفاءة تحت الضغط. وهذا يجعله **أسرع بمقدار 4.3-4.5x من RDS وAurora**. في المقابل، واجهت حلول التخزين المتصل بالشبكة صعوبة في العمليات التي يغلب عليها الطابع الكتابي، إذ بلغ الحد الأقصى لكل من RDS وAurora ما بين 4.4K و4.6K TPS رغم السعة المخصصة، وحتى مع إعداد Aurora ‏IO Optimized.
...
| الإعداد       | ClickHouse Managed Postgres | RDS with 16k IOPS          | Aurora IO Optimized         |
| ------------- | --------------------------- | -------------------------- | --------------------------- |
| **إصدار PG**  | 17                          | 17                         | 17                          |
| **vCPUs**     | 16                          | 16                         | 16                          |
...
**التحليل**: تتسع فجوة الأداء بشكل كبير في أعباء العمل كثيفة القراءة والمقيّدة بعمليات الإدخال/الإخراج. **حقق ClickHouse Managed Postgres ‏84.8K TPS**، بينما لم يحقق RDS مع 16,000 IOPS مخصصة سوى ‏9.4K TPS رغم امتلاكه موارد حوسبة مماثلة. ويكمن الفرق الأساسي في أن **تخزين NVMe** في ClickHouse Managed Postgres يتوسع مع زيادة التزامن، بينما يظل **التخزين المتصل بالشبكة** مقيّدًا بحدود IOPS المخصصة. وحتى مع IOPS المخصصة، ظل RDS أبطأ بتسع مرات من ClickHouse Managed Postgres، مما يبرز الأهمية الحاسمة لمعمارية التخزين في أعباء العمل كثيفة الإدخال/الإخراج.
...
| الإعداد       | ClickHouse Managed Postgres | RDS with 16k IOPS          |
| ------------- | --------------------------- | -------------------------- |
| **إصدار PG**  | 17                          | 17                         |
| **vCPUs**     | 16                          | 16                         |
| **RAM**       | 64 GB                       | 64 GB                      |
...
**التحليل**: حتى في السيناريوهات المقيّدة بالـ CPU حيث يكون إدخال/إخراج القرص في حدّه الأدنى، **تصدّر ClickHouse Managed Postgres بنتيجة 36.5 ألف TPS**. وعلى الرغم من وصول الخدمتين كلتيهما إلى استخدام CPU بنسبة 100%، فقد وفّر تخزين NVMe في ClickHouse Managed Postgres أداءً أفضل مع معدلات إصابة أعلى للـ cache. وتُظهر الأفضلية البالغة 12% على RDS كفاءة البنية التحتية الأساسية حتى عندما تكون أعباء العمل مقيّدة بالـ CPU بالدرجة الأولى.
...
| التكوين       | ClickHouse Managed Postgres | RDS PostgreSQL        |
| ------------- | --------------------------- | --------------------- |
| **إصدار PG**  | 17                          | 17                    |
| **vCPUs**     | 2                           | 2                     |
| **RAM**       | 8 GB                        | 8 GB                  |
...
في جميع سيناريوهات اختبار الأداء الثلاثة، قدّم ClickHouse Managed Postgres أداءً متفوقًا باستمرار:
...
| الجانب                     | تخزين NVMe (ClickHouse Managed Postgres) | التخزين المتصل بالشبكة (Provisioned IOPS)           |
| -------------------------- | ---------------------------------------- | --------------------------------------------------- |
...
إلى جانب الأداء الخام، يوفّر ClickHouse Managed Postgres تفوقًا واضحًا في السعر مقابل الأداء:
...
بالنسبة إلى workloads المقيّدة حاليًا بحدود IOPS، يمكن أن يؤدي الانتقال إلى ClickHouse Managed Postgres إلى إلغاء الحاجة إلى provisioned IOPS المكلفة أو تهيئات IO Optimized، مع تحقيق أداء أفضل بكثير.
...
* [نظرة عامة على ClickHouse Managed Postgres](/ar/products/managed-postgres/overview)
