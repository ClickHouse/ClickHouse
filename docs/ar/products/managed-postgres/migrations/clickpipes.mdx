...
<div id="step-1-connect">
  ## الخطوة 1: الاتصال بقاعدة البيانات المصدر
</div>

افتح [ClickHouse Cloud console](https://clickhouse.cloud) وحدد Managed Postgres service الخاص بك.

<Image img="/images/managed-postgres/pgpg/servicecard.webp" alt="بطاقة Managed Postgres service في قائمة خدمات ClickHouse Cloud" size="lg" border />

في الشريط الجانبي الأيسر، انقر على **مصادر البيانات**.

<Image img="/images/managed-postgres/pgpg/overview.webp" alt="إدخال مصادر البيانات في الشريط الجانبي لـ Managed Postgres service" size="lg" border />
...
<div id="step-2-export-schema">
  ## الخطوة 2: صدِّر مخطط قاعدة البيانات
</div>

يعرض المعالج أمر `pg_dump` معبأً مسبقًا بتفاصيل الاتصال بالمصدر. شغِّله في الطرفية:
...
سيؤدي ذلك إلى إنشاء `pg.sql` في الدليل الحالي.
...
حدِّد قاعدة البيانات الوجهة من القائمة المنسدلة، أو انقر على **إنشاء قاعدة بيانات جديدة** لإنشاء واحدة.

يعرض المعالج أمر `psql` لتطبيق ملف schema dump على خدمة Managed Postgres الخاصة بك. شغِّله في الطرفية:
...
<Image img="/images/managed-postgres/pgpg/psqlimport.webp" alt="مخرجات الطرفية بعد تشغيل أمر استيراد مخطط psql" size="lg" border />

انقر على **التالي**.

<div id="step-4-ingestion-settings">
  ## الخطوة 4: تكوين إعدادات الإدخال
</div>
...
* [بيانات الاتصال بـ Managed Postgres](/ar/products/managed-postgres/connection)
* [FAQ الخاصة بـ ClickPipes Postgres](/ar/integrations/clickpipes/postgres/faq)