...
يوفّر ClickHouse Cloud الآن ClickPipes لترحيل قاعدة بيانات PostgreSQL الخارجية لديك إلى خدمة ClickHouse Managed Postgres. يوفّر هذا التكامل المضمّن تجربة سلسة للاتصال بقاعدة البيانات المصدر، وتصدير المخطط، واستيراده إلى ClickHouse Managed Postgres، وإعداد النسخ المتماثل المستمر.
...
* **نشر تغييرات DDL**: يلتقط النسخ المتماثل المستمر (CDC) عمليات DML و`ADD COLUMN`. أما تغييرات DDL الأخرى، مثل `DROP COLUMN` و`ALTER COLUMN`، فلا تُنقل ويجب تطبيقها يدويًا على الهدف.

<Note>
  إذا واجهت مشكلات أثناء الترحيل، فراجع [الأسئلة الشائعة حول عمليات ترحيل ClickHouse Managed Postgres](/ar/products/managed-postgres/migrations/faq) للاطلاع على الأخطاء الشائعة وحلولها.
</Note>

<Steps>
  <Step title="الاتصال بقاعدة البيانات المصدر" id="step-1-connect">
    افتح [ClickHouse Cloud console](https://clickhouse.cloud) وحدد خدمة ClickHouse Managed Postgres الخاصة بك.

    <Image img="/images/managed-postgres/pgpg/servicecard.webp" alt="بطاقة خدمة ClickHouse Managed Postgres في قائمة خدمات ClickHouse Cloud" size="lg" border />

    في الشريط الجانبي الأيسر، انقر على **مصادر البيانات**.

    <Image img="/images/managed-postgres/pgpg/overview.webp" alt="إدخال مصادر البيانات في الشريط الجانبي لـ خدمة ClickHouse Managed Postgres" size="lg" border />
    ...
    * **التحميل الأولي + CDC** — ينسخ البيانات الحالية، ثم يُبقي الهدف متزامنة مع التغييرات المستمرة.
  ...
  <Step title="استيراد المخطط إلى خدمة ClickHouse Managed Postgres الخاصة بك" id="step-3-import-schema">
    حدد قاعدة البيانات الهدف من القائمة المنسدلة، أو انقر على **Create a new database** لإنشاء واحدة جديدة.

    يعرض المعالج أمر `psql` لتطبيق ملف تفريغ المخطط على خدمة ClickHouse Managed Postgres الخاصة بك. شغّله في الطرفية:
...
**حوِّل حركة مرور التطبيق.** وجِّه عمليات القراءة والكتابة إلى خدمة ClickHouse Managed Postgres لديك، وراقب الأخطاء ومخالفات القيود وسلامة النسخ المتماثل.
...
* [البدء السريع لـ ClickHouse Managed Postgres](/ar/products/managed-postgres/quickstart)
* [تفاصيل الاتصال بـ ClickHouse Managed Postgres](/ar/products/managed-postgres/connection)
