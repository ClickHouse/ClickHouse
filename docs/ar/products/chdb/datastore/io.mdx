...
keywords: ['chdb', 'DataStore', 'I/O', 'قراءة', 'كتابة', 'csv', 'parquet', 'json', 'excel']
doc_type: 'مرجع'
---
...
صدِّر إلى قاعدة بيانات SQL أو أنشئ عبارة SQL.
...
| **JSON**    | بطيئة        | متوسطة       | كبير      | جزئي   | واجهات API، البيانات المتداخلة      |
| **Excel**   | بطيئة        | بطيئة        | متوسط     | جزئي   | المشاركة مع المستخدمين غير التقنيين |
| **Feather** | سريعة جدًا   | سريعة جدًا   | متوسط     | نعم    | الاستخدام بين العمليات، pandas      |
...
| الضغط    | السرعة    | النسبة     | حالة استخدام         |
| -------- | --------- | ---------- | -------------------- |
| `snappy` | سريع جدًا | منخفضة     | الافتراضي لـ Parquet |
| `lz4`    | سريع جدًا | منخفضة     | أولوية السرعة        |
| `gzip`   | متوسطة    | عالية      | التوافق              |
| `zstd`   | سريع      | عالية جدًا | أفضل توازن           |
| `bz2`    | بطيء      | عالية جدًا | أقصى ضغط             |
...
راجع [Factory Methods](/ar/products/chdb/datastore/factory-methods) للتعرّف على خيارات cloud storage.
