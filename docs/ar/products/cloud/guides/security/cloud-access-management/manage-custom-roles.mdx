...
<Steps>
  ...
  <Step title="سمِّ الدور" id="name-the-role">
...
<Info>
  تكون للأدوار المعيّنة يدويًا في قاعدة البيانات باستخدام [اصطلاح التسمية `sql-console-role:<email>`](/ar/products/cloud/guides/security/cloud-access-management/manage-database-users#sql-console-users-and-roles) أولوية على الأدوار المعيّنة عبر Console.
</Info>

<Steps>
  <Step title="إضافة أذونات للبيانات إلى دور" id="add-data-permissions">
    [أنشئ دورًا مخصصًا](#create-custom-role) أو حرّر دورًا موجودًا. في مربع حوار الدور، انقر فوق الزر `Allow permissions` واختر `Data`.
  </Step>
  ...
  <Step title="اختيار مستوى الوصول" id="choose-data-access-level">
    حدد مستوى الوصول `Read-only` أو `Admin` أو `Custom`. عند اختيار `Custom`، عرّف أذونات محددة للوصول إلى البيانات باستخدام عبارات [`GRANT`](/ar/reference/statements/grant) في ClickHouse.
  ...
  <Step title="إنشاء الدور" id="create-role-with-data-permissions">
    انقر فوق `Create role`. تُنشأ أدوار قاعدة البيانات في الخدمات المحددة.
  </Step>
</Steps>
...
تُنشأ الأدوار المُدارة عبر Console في قاعدة البيانات بالبادئة `cloud:` ونوع التخزين `cloud`. لعرضها، نفّذ query على table `system.roles` في الخدمة:
