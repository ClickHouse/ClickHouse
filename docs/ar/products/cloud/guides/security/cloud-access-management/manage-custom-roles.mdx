...
<Tip>
  تحل أدوار قاعدة البيانات التي يديرها Cloud محل أدوار [`sql-console-role:<email>`](/ar/products/cloud/guides/security/cloud-access-management/manage-database-users#sql-console-users-and-roles) المعيّنة يدويًا، وهي غير متوافقة مع [مصادقة JWT](/ar/products/cloud/guides/security/cloud-access-management/jwt-authentication-setup).
</Tip>
...
<Steps>
  ...
  <Step title="اختيار مستوى الوصول" id="choose-data-access-level">
    حدد مستوى الوصول `Read-only` أو `Admin` أو `Custom`. عند اختيار `Custom`، عرّف أذونات محددة للوصول إلى البيانات باستخدام عبارات [`GRANT`](/ar/reference/statements/grant) في ClickHouse. لا يمكن تحديد مستوى الوصول `Custom` إلا عندما تستهدف الأذونات خدمة واحدة؛ ولتحديد أذونات مخصصة لخدمات متعددة، أضف إذن `Data` منفصلًا لكل خدمة.
