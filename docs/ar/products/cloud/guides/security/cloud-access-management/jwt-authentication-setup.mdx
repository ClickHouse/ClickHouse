...
<EnterprisePlanFeatureBadge feature="مصادقة JWT باستخدام موفر هوية مخصص" support="true" />

<Tip>
  يشرح هذا الدليل كيفية تهيئة موفري JWKS في Cloud console. لمعرفة كيفية إنشاء JWT وبنيته — بما في ذلك المطالبات المطلوبة ومطالبات الأدوار والمنح وكيفية عمل المستخدمين المؤقتين — راجع [مصادقة JWT](/ar/operations/external-authenticators/jwt).
</Tip>
...
بعد تمكين هذه الميزة لمؤسستك، يمكنك تهيئة موفري JWKS بنفسك لكل خدمة من **Settings → Security** في ClickHouse Cloud console.

<Note>
  لتمكين موفري JWT لمؤسستك، تواصل مع [الدعم](https://clickhouse.com/support/program).
</Note>
...
* عنوان URL علني يمكن الوصول إليه عبر **HTTPS** لـ JWKS ينشر مفتاح **RSA** واحدًا على الأقل (يجب توقيع الرموز المميزة باستخدام **RS256**).

<Note>
  لا يقبل الموفّرون المستندون إلى JWKS سوى **مفاتيح RSA**. لا يمكن التحقق من الرموز المميزة الموقّعة بمفاتيح HMAC (`HS256`) أو EC (`ES256`) عبر نقطة نهاية JWKS، ولذلك تُرفض. قد يحتوي مستند JWKS على أنواع مفاتيح أخرى، ولكن يجب أن يحتوي على مفتاح RSA صالح للاستخدام واحد على الأقل.
</Note>
...
ينشئ العميل (موفّر الهوية أو تطبيقك) JWT ويوقّعه باستخدام **مفتاحه الخاص**. يجب أن يتبع الرمز المميز [تنسيق الرمز المميز](/ar/operations/external-authenticators/jwt#token-claims) المتوقع. ثم يتحقق ClickHouse منه باستخدام المفاتيح **العامة** المنشورة على عنوان URL الخاص بـ JWKS:

1. يقرأ ClickHouse ترويسة `kid` (معرّف المفتاح) للرمز المميز ويختار المفتاح المطابق من مستند JWKS.
2. يتحقق من توقيع الرمز المميز باستخدام مفتاح RSA العام، ويتحقق من مطالبتي `iss` (المُصدِر) و`aud` (الجمهور) بمقارنتهما مع تهيئة الموفّر.
3. عند نجاح التحقق، يعمل الاتصال كمستخدم مؤقت، وتُستمد حقوق وصوله من مطالبتي `clickhouse:grants` و`clickhouse:roles` في الرمز المميز، ضمن الحد الأقصى للأذونات (المستخدم `default`). راجع [حقوق الوصول](/ar/operations/external-authenticators/jwt#access-rights) للتفاصيل.
...
<Steps>
  ...
  <Step title="أدخل تفاصيل الموفّر" id="fill-provider-details">
    ...
    | **URL لـ JWKS**              | عنوان HTTPS عام ينشر مفاتيح RSA العامة المستخدمة للتحقق من توقيعات الرموز المميزة.                                                                                                             |
...
* لا يقبل موفرو JWKS سوى مفاتيح **RSA / RS256**.
...
* [مصادقة JWT](/ar/operations/external-authenticators/jwt) — مطالبات الرمز المميز، والمستخدمون المؤقتون، واستخدام العميل.