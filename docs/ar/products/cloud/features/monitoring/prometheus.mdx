...
تدعم هذه الميزة التكامل مع [Prometheus](https://prometheus.io/) لمراقبة خدمات ClickHouse Cloud. ويُتاح الوصول إلى مقاييس Prometheus عبر نقطة نهاية [ClickHouse Cloud واجهة برمجة تطبيقات](/ar/products/cloud/features/admin-features/api/api-overview)، ما يتيح لك الاتصال بأمان وتصدير المقاييس إلى مجمّع مقاييس Prometheus لديك. كما يمكن دمج هذه المقاييس مع لوحات المعلومات مثل Grafana وDatadog لعرضها بصريًا.
...
| GET     | `https://api.clickhouse.cloud/v1/organizations/:organizationId/prometheus/discovery?filtered_metrics=[true \| false]`           | يعيد أهداف scrape في Prometheus لجميع الخدمات ضمن مؤسسة، بتنسيق [اكتشاف الخدمة عبر HTTP](https://prometheus.io/docs/prometheus/latest/http_sd/) |

**معلمات الطلب**

| الاسم                | الموقع         | النوع             |
| -------------------- | -------------- | ----------------- |
| Organization ID      | عنوان نقطة نهاية | uuid              |
| Service ID           | عنوان نقطة نهاية | uuid (اختياري)    |
...
تعرض نقطة نهاية الاكتشاف هدف جمع واحدًا لكل خدمة في مؤسستك، بتنسيق Prometheus [لاكتشاف الخدمات عبر HTTP](https://prometheus.io/docs/prometheus/latest/http_sd/) (`http_sd`). يُحدِّث Prometheus قائمة الأهداف عند كل عملية poll للاكتشاف ويجري جمعًا لكل هدف مُكتشَف، لذلك تُكتشف الخدمات المنشأة حديثًا تلقائيًا وتُزال الخدمات المحذوفة من قائمة الأهداف. ولا تُدرج إلا الخدمات التي يملك مفتاح واجهة برمجة تطبيقات صلاحية عرضها؛ وتُستبعد الخدمات التي يجري حذفها أو التي حُذفت بالفعل.
