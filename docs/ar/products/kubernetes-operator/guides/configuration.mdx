...
يكشف المشغّل مجموعة ثابتة من المنافذ على كل Pod لـ ClickHouse وعلى خدمته العامة عديمة الرأس (headless Service): `8123` لـ HTTP، و`9000` لـ native، و`9009` للاتصال بين الخوادم، و`9001`/`9002` للإدارة، و`9363` لمقاييس Prometheus، بالإضافة إلى منافذ TLS البديلة `8443`/`9440` عند تمكين TLS. كما يُعرَض منفذ `interserver` ومنافذ الإدارة بشكل إضافي عبر الخدمات الداخلية المخصّصة لكل نسخة متماثلة، حتى تتمكن النسخ المتماثلة والمشغّل من التواصل قبل أن تصبح النسخة المتماثلة جاهزة. لجعل ClickHouse يستمع إلى بروتوكولات إضافية — مثل MySQL أو PostgreSQL أو gRPC أو أي منفذ مخصّص — عرِّفها في `spec.additionalPorts`:
...
يضيف المشغّل هذه المنافذ إلى `containerPorts` الخاصة بالـ Pod وإلى الخدمة العامة عديمة الرأس.
...
يحمل كل Pod للنسخة المتماثلة بوابة الجاهزية `clickhouse.com/ReplicaInitialized`، بحيث لا تُنشر النسخة المتماثلة الجديدة عبر الخدمة العامة عديمة الرأس إلا بعد أن يُكمل المشغّل تهيئتها: تُحوَّل قاعدة البيانات `default` إلى محرك Replicated وتُزامَن الأسكيما. وحتى ذلك الحين، يصل إليها المشغّل وسائر النسخ المتماثلة عبر خدمتها الداخلية. وعند تعيين `enableDatabaseSync: false`، يعتبر المشغّل النسخ المتماثلة مهيّأة على الفور، بحيث تعتمد الجاهزية على فحوصات الحاوية وحدها.

لا يحذف المشغّل أبدًا قاعدة بيانات `default` غير المُنسوخة (Replicated) إن كانت مملوءة بالبيانات. تبدأ نسخة متماثلة كهذه بخدمة حركة العملاء بعد تحضير الأسكيما، غير أن العنقود يُبلِّغ بحالة `SchemaInSync=False` مع السبب `DefaultDatabaseNotReplicated` إلى أن تُعالج تلك القاعدة بنفسك.

قبل إزالة نسخة متماثلة عند تقليص الحجم، يُفرِّغ المشغّل أولًا حركة العملاء عنها، ثم ينتظر حتى يتوقف نشرها، ثم يُنسخ ما تبقى من بياناتها إلى النسخ المتماثلة الباقية، ولا يحذفها إلا بعد ذلك.
