---
position: 2
slug: /ar/products/kubernetes-operator/guides/configuration
...
<div id="clickhousecluster-configuration">
  ## إعداد ClickHouseCluster {#clickhousecluster-configuration}
</div>
...
<div id="replicas-and-shards">
  ### النسخ المتماثلة والشظايا {#replicas-and-shards}
</div>
...
<div id="keeper-integration">
  ### تكامل Keeper {#keeper-integration}
</div>
...
<div id="keepercluster-configuration">
  ## إعداد KeeperCluster {#keepercluster-configuration}
</div>
...
<div id="storage-configuration">
  ## تهيئة التخزين {#storage-configuration}
</div>
...
بالنسبة إلى النسخ المتماثلة في ClickHouse، ينشئ المشغِّل خدمة عديمة الرأس حاكمة باسم `<cluster-name>-clickhouse-headless` لتخدم حركة مرور العملاء.
...
وطلبات الإدارة من المشغِّل. يمكن استخدامها لاستعادة النسخة المتماثلة إذا لم تتمكن من أن تصبح جاهزة بنفسها.
...
<div id="pod-configuration">
  ## إعدادات الكبسولة {#pod-configuration}
</div>
...
<div id="see-api-referenceproductskubernetes-operatorreferenceapi-referencepodtemplatespec-for-all-supported-pod-template-options">
  ### راجع [مرجع واجهة برمجة التطبيقات](/ar/products/kubernetes-operator/reference/api-reference#podtemplatespec) للاطلاع على جميع الخيارات المدعومة لقوالب الـ Pod. {#see-api-reference-for-all-supported-pod-template-options}
</div>

<div id="pod-disruption-budgets">
  ## ميزانيات تعطيل الكبسولات {#pod-disruption-budgets}
</div>
...
<div id="pdb-overrides">
  ### تجاوز القيم الافتراضية {#pdb-overrides}
</div>
...
<div id="pdb-cluster-wide-disable">
  ### التعطيل على مستوى العنقود بالكامل {#pdb-cluster-wide-disable}
</div>
...
<div id="container-configuration">
  ## تكوين الحاوية {#container-configuration}
</div>

<div id="custom-image">
  ### صورة حاوية مخصّصة {#custom-image}
</div>
...
<div id="container-resources">
  ### موارد الحاويات {#container-resources}
</div>
...
<div id="volume-mounts">
  ### ربط وحدات التخزين {#volume-mounts}
</div>
...
<div id="see-api-referenceproductskubernetes-operatorreferenceapi-referencecontainertemplatespec-for-all-supported-container-template-options">
  ### راجع [مرجع واجهة برمجة التطبيقات](/ar/products/kubernetes-operator/reference/api-reference#containertemplatespec) للاطلاع على جميع خيارات قوالب الحاويات المدعومة. {#see-api-reference-for-all-supported-container-template-options}
</div>

<div id="tls-ssl-configuration">
  ## إعدادات TLS/SSL {#tls-ssl-configuration}
</div>

<div id="configure-secure-endpoints">
  ### تهيئة نقاط نهاية آمنة {#configure-secure-endpoints}
</div>
...
<div id="ssl-certificate-secret-format">
  ### تنسيق Secret الخاص بشهادة SSL {#ssl-certificate-secret-format}
</div>
...
<div id="clickhouse-keeper-communication-over-tls">
  ### اتصال ClickHouse-Keeper عبر TLS {#clickhouse-keeper-communication-over-tls}
</div>
...
<div id="external-secret-required-keys">
  ### المفاتيح المطلوبة {#external-secret-required-keys}
</div>
...
<div id="external-secret-policy">
  ### السياسة: المراقبة مقابل الإدارة {#external-secret-policy}
</div>
...
<div id="external-secret-status">
  ### شرط الحالة واستكشاف الأخطاء وإصلاحها {#external-secret-status}
</div>
...
<div id="additional-ports-mysql-example">
  ### مثال متكامل: MySQL wire protocol {#additional-ports-mysql-example}
</div>
...
<div id="additional-ports-constraints">
  ### قيود الحقول {#additional-ports-constraints}
</div>
...
<div id="additional-ports-reserved">
  ### المنافذ والأسماء المحجوزة {#additional-ports-reserved}
</div>
...
<div id="version-probe-and-upgrade-channel">
  ## فحص الإصدار وقناة الترقية {#version-probe-and-upgrade-channel}
</div>
...
<div id="upgrade-channel-choosing">
  ### اختيار قناة الترقية {#upgrade-channel-choosing}
</div>
...
<div id="version-status-conditions">
  ### شروط الحالة {#version-status-conditions}
</div>
...
<div id="version-probe-template">
  ### تجاوز `Job` فحص الإصدار {#version-probe-template}
</div>
...
<div id="version-operator-flags">
  ### عناصر تحكم على مستوى المشغّل {#version-operator-flags}
</div>
...
<div id="default-user-password">
  ### كلمة مرور المستخدم `default` {#default-user-password}
</div>
...
<div id="password-types">
  #### أنواع كلمات المرور {#password-types}
</div>
...
<div id="default-password-secret-example">
  #### مثال كامل باستخدام Secret {#default-password-secret-example}
</div>
...
<div id="using-configmap-for-user-passwords">
  #### استخدام ConfigMap {#using-configmap-for-user-passwords}
</div>
...
<div id="database-sync">
  ### مزامنة قاعدة البيانات {#database-sync}
</div>
...
تحمل كل Pod للنسخة المتماثلة بوابة الجاهزية `clickhouse.com/ReplicaInitialized`، لذلك لا تُنشر النسخة المتماثلة الجديدة عبر الخدمة عديمة الرأس العامة إلا بعد أن يُكمل المُشغِّل تهيئتها: إذ تُحوَّل قاعدة البيانات `default` إلى محرك Replicated ويُزامَن المخطط. وحتى ذلك الحين، يصل إليها المُشغِّل والنسخ المتماثلة الأخرى عبر خدمتها الداخلية. عند استخدام `enableDatabaseSync: false`، يضع المُشغِّل علامة التهيئة على النسخ المتماثلة فورًا، لذا تعتمد الجاهزية على فحوصات الحاوية وحدها.

لا يحذف المُشغِّل مطلقًا قاعدة بيانات `default` غير Replicated والمملوءة بالبيانات. تبدأ هذه النسخة المتماثلة مع ذلك في خدمة حركة مرور العملاء بعد إعداد المخطط، لكن العنقود يبلّغ عن `SchemaInSync=False` بالسبب `DefaultDatabaseNotReplicated` إلى أن تعالج قاعدة البيانات تلك بنفسك.

قبل إزالة نسخة متماثلة عند التخفيض التدريجي، يُفرِّغ المُشغِّل أولًا حركة مرور العملاء منها، وينتظر حتى تتوقف عن النشر، ويُكرِّر بياناتها المتبقية إلى النسخ المتماثلة الباقية، ثم يحذفها فقط بعد ذلك.

<div id="server-logging">
  ### تسجيل الخادم {#server-logging}
</div>
...
<div id="custom-configuration">
  ## إعداد مخصص {#custom-configuration}
</div>

<div id="embedded-extra-configuration">
  ### تهيئة إضافية مضمنة {#embedded-extra-configuration}
</div>
...
<div id="embedded-extra-users-configuration">
  ### إعدادات المستخدمين الإضافيين المضمّنة {#embedded-extra-users-configuration}
</div>
...
<div id="see-documentationconceptsfeaturesconfigurationsettingssettings-users-for-all-supported-clickhouse-users-configuration-options">
  #### راجع [الوثائق](/ar/concepts/features/configuration/settings/settings-users) للاطلاع على جميع خيارات إعداد مستخدمي ClickHouse المدعومة. {#see-documentation-for-all-supported-clickhouse-users-configuration-options}
</div>

<div id="configuration-example">
  ### مثال على التهيئة {#configuration-example}
</div>
