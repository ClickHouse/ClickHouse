...
[إعدادات خادم `openSSL`](/ar/reference/settings/server-settings/settings/other#openssl)
للاطلاع على الخيارات المتاحة، و
[Configuration → التهيئة الإضافية المضمّنة](/ar/products/kubernetes-operator/guides/configuration#embedded-extra-configuration)
...
* [Configuration → تهيئة TLS/SSL](/ar/products/kubernetes-operator/guides/configuration#tls-ssl-configuration) — مرجع الحقول
* [Configuration → `additionalPorts`](/ar/products/kubernetes-operator/guides/configuration#additional-ports) — المنافذ المحجوزة
* [مرجع واجهة برمجة التطبيقات → ClusterTLSSpec](/ar/products/kubernetes-operator/reference/api-reference#clustertlsspec)
* [إعدادات الخادم `openSSL`](/ar/reference/settings/server-settings/settings/other#openssl) — خيارات TLS التي يمكنك تجاوزها عبر `extraConfig`