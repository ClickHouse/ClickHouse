---
noindex: true
title: "دمج Teleport مع ClickHouse Private"
sidebarTitle: "دمج Teleport"
description: "إعداد Teleport Enterprise للوصول الآمن إلى قواعد البيانات في عناقيد ClickHouse Private ضمن البيئات الحكومية/‏FIPS."
---

قم بإعداد Teleport Enterprise للوصول الآمن إلى قواعد البيانات في عناقيد ClickHouse Private ضمن البيئات الحكومية/‏FIPS. يغطي هذا الدليل نشر Teleport على EKS، وتهيئة الوصول إلى قواعد البيانات، والمصادقة عبر SAML، وتدوير الشهادات.

<div id="prerequisites">
  ## المتطلبات الأساسية
</div>

* يجب أن يكون [نشر FIPS/Government](/ar/products/clickhouse-private/tutorials/deploy-clickhouse-with-fips-compliance-government) قد اكتمل مع `SERVER_VERIFICATION_MODE=relaxed` و `KEEPER_VERIFICATION_MODE=relaxed` (مطلوب لـ Teleport mTLS)
* ترخيص Teleport Enterprise
* يجب أن يتضمن حقل SAN في شهادة الخادم `DNS.2 = c-${CLUSTER_NAME}-server-any.ns-${CLUSTER_NAME}.svc.${KUBERNETES_DOMAIN}` (راجع [إنشاء شهادات متوافقة مع FIPS](/ar/products/clickhouse-private/guides/generate-fips-compliant-certificates-for-clickhouse))

***

<div id="create-a-node-group-for-teleport">
  ## أنشئ مجموعة عُقد لـ Teleport
</div>

تُعد مجموعة عُقد مخصصة لـ Teleport اختيارية. إذا لم تكن لديك عُقد Bottlerocket ARM FIPS متاحة، فأنشئ مجموعة عُقد بالإعدادات التالية في Console الخاصة بـ EKS ضمن Compute:

* **الاسم**: fips-arm64
* **دور IAM للعُقد**: أنشئ دورًا جديدًا موصى به
* **Taints في Kubernetes**:
  * **المفتاح**: `group`, **القيمة**: `fips-arm64`, **التأثير**: `NoSchedule`
* **نوع AMI**: Bottlerocket ARM FIPS
* **أنواع المثيلات**: m7g.large
* **التحجيم**: المطلوب 2، الحد الأدنى 2، الحد الأقصى 3

***

<div id="install-teleport-cluster-on-eks">
  ## تثبيت عنقود Teleport على EKS
</div>

<div id="create-namespace-and-license-secret">
  ### إنشاء مساحة الاسم وسرّ الترخيص
</div>
...
<div id="generate-fips-certificates-for-teleport">
  ### إنشاء شهادات FIPS لـ Teleport
</div>

أنشئ السكربت التالي (`generate_fips_certs_in_container_for_teleport.sh`):
...
نفِّذه (استبدل `<CLUSTER_FQDN>` بالاسم المؤهل بالكامل (FQDN) لخدمة Teleport):
...
أنشئ الملف `eks_teleport_values.yaml`. استبدل العناصر النائبة التالية:

* `<CLUSTER_NAME>`: الاسم المؤهل بالكامل (FQDN) للعنقود
* `<VPN_SUBNETS>`: الشبكات الفرعية الخاصة بـ VPN، مفصولة بفواصل (على سبيل المثال: `private-subnet-us-east-2a,private-subnet-us-east-2b,private-subnet-us-east-2c`)
* `<BUCKET_NAME>`: حاوية S3 لسجلات جلسات Teleport
* `<NODE_SELECTOR>`: قيمة محدد العُقد (على سبيل المثال: `fips-arm64`)
...
> يوضّح هذا المثال كيفية نشر Teleport مع FIPS على EKS. بالنسبة إلى موفري الخدمات السحابية الآخرين، راجع [وثائق Teleport](https://goteleport.com/docs/zero-trust-access/deploy-a-cluster/helm-deployments/).
...
<div id="get-the-teleport-service-external-ip">
  ### احصل على عنوان IP الخارجي لخدمة Teleport
</div>
...
استخدم عنوان IP الخارجي وأنشئ سجل CNAME في Route53 أو لدى موفّر DNS لديك. إذا لم يكن لموازن التحميل اسم مضيف:

```bash
kubectl get svc teleport -n teleport -o jsonpath='{.status.loadBalancer.ingress[0].ip}'
```

ثم أنشئ سجل A بدلًا من ذلك.

***

<div id="create-a-teleport-provision-token">
  ## أنشئ رمز تزويد خاصًا بـ Teleport
</div>

يتيح رمز التزويد لوكلاء Teleport الانضمام إلى العنقود. أنشئ `teleport_provision_token_for_clickhouse.yaml`:
...
أنشئ سرًا لسلطة الشهادات (CA) الخاصة بوسيط Teleport في مساحة اسم الوكيل:
...
أنشئ سر لـ CA الخاصة بقاعدة البيانات (ادمج CAs إذا كنت تستخدم قواعد بيانات متعددة ذات CAs مختلفة). أنشئ ملفًا باسم `db_cas.pem`، ثم:
...
أنشئ `teleport_agent_values.yaml`. استبدل:

* `<CLICKHOUSE_CLUSTER_NAME>`: اسم الـ عنقود الذي تم توفيره (على سبيل المثال، `default-aa-01`)
* `<ENV>`: وسم ثابت لقاعدة البيانات هذه (على سبيل المثال، `dev`)
...
> ينشئ ملف YAML موردَي قاعدة بيانات لقاعدة بيانات واحدة، لأن ClickHouse يدعم بروتوكولين (HTTPS على 8443 وTLS الأصلي على 9440). احذف ما لا تحتاج إليه منهما.
...
<div id="add-teleport-ca-to-clickhouse-trusted-list">
  ## أضِف CA الخاص بـ Teleport إلى قائمة الثقة في ClickHouse
</div>

اجلب CA الجذر لـ Teleport وأضِفه إلى كائن سر الخاص بشهادة خادم ClickHouse:
...
أعِد تشغيل جرابات خادم ClickHouse لتفعيل CA الجديدة.
...
<div id="create-a-database-access-role">
  ### أنشئ دورًا للوصول إلى قاعدة البيانات
</div>

أنشئ `saml_clickhouse_user_role.yaml`. استبدل مفتاح سمة المستخدم بالسمة من موفّر الهوية لديك (على سبيل المثال، `http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress`):
...
لمزيد من المعلومات عن الأدوار، راجع [وثائق Teleport](https://goteleport.com/docs/reference/infrastructure-as-code/operator-resources/resources-teleport-dev-rolesv8/).

<div id="create-a-saml-connector">
  ### إنشاء موصل SAML
</div>

أنشئ `saml_connector.yaml`. استبدل `<DOMAIN>` بنطاق Route53 الخاص بك، و`<Entity Descriptor>` بالقيمة الواردة من موفّر الهوية لديك:
...
لمزيد من المعلومات حول موصلات SAML، راجع [وثائق Teleport](https://goteleport.com/docs/reference/infrastructure-as-code/operator-resources/resources-teleport-dev-samlconnectors/).
...
نفّذ `exec` داخل جراب خادم ClickHouse:
...
أنشئ مستخدمًا تتم مصادقته باستخدام CN لشهادة SSL (استبدل `<DB_USER>` باسم المستخدم -- ويكون عادةً عنوان البريد الإلكتروني للمستخدم لدى موفّر الهوية):
...
<div id="connect-via-teleport">
  ## الاتصال عبر Teleport
</div>

أضف CA الخاصة بـ Teleport إلى قائمة جهات إصدار الشهادات الموثوقة في نظامك، أو استخدم الخيار `--insecure` في بيئات التطوير.
...
استبدل: - `<DOMAIN>`: نطاق Route53 لديك - `<AUTH_TYPE>`: نوع المصادقة (مثلًا، `samlconnector`) - `<TELEPORT_USER>`: اسم مستخدم Teleport - `<DB_USER>`: اسم مستخدم قاعدة البيانات - `<CLICKHOUSE_CLUSTER_NAME>`: اسم عنقود ClickHouse (مثلًا، `default-aa-01`)

***

<div id="day-2-certificate-rotation">
  ## اليوم الثاني: تدوير الشهادات
</div>

تنتهي صلاحية شهادات Teleport، ويجب تدويرها قبل انتهائها.

<div id="rotate-teleport-proxy-certificates">
  ### تدوير شهادات Teleport Proxy
</div>

1. **أنشئ شهادات جديدة** باستخدام البرنامج النصي من [إنشاء شهادات FIPS لـ Teleport](#generate-fips-certificates-for-teleport). تأكد من أن `CLUSTER_FQDN` والمتغيرات الأخرى تطابق إعدادات العنقود لديك.

   > يجب تشغيل البرنامج النصي على مضيف مُمكَّن عليه FIPS. يقوم Docker بتحميل `/proc` تلقائيًا، لذا ما دامت الحاوية تعمل على مضيف مُمكَّن عليه FIPS، فستكون الشهادات المُنشأة متوافقة مع FIPS.

2. **ألحِق CA الجديدة بالحزمة الحالية** في كلتا مساحتي الأسماء:
...
3. **أعِد تشغيل Teleport إعادةً متدرجة**:
...
4. **أضف Teleport CA الجديدة إلى قائمة الثقة في ClickHouse**:
...
* [وثائق Teleport](https://goteleport.com/docs/)