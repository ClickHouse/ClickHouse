...
<Steps>
  ...
  <Step title="إعداد البنية التحتية لنموذج BYOC" id="setup-byoc-infrastructure">
    ...
    * **نطاق CIDR لـ VPC/VNet**: نستخدم افتراضيًا `10.0.0.0/16` كنطاق CIDR لـ VPC الخاص بـ BYOC (في AWS/GCP) أو VNet (في Azure). إذا كنت تخطط لاستخدام VPC/VNet peering مع حساب آخر، فتأكد من عدم تداخل نطاقات CIDR. خصص لـ BYOC نطاق CIDR مناسبًا بحجم أدنى `/23` لاستيعاب أعباء العمل المطلوبة. إذ لا يستوعب الحد الأدنى `/23` سوى نحو 30 وحدة pod للخادم، وتستهلك كل نسخة متماثلة عناوين IP؛ لذا خصص نطاقًا أكبر (ويُوصى بأن يكون `/22` على الأقل) إذا كنت تتوقع نمو عملية النشر.
