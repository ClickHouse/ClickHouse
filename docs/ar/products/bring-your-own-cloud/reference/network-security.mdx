...
| -------------------------------------------------- | --------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **العمليات اليومية — خادم Kubernetes API**         | عام مع تصفية عناوين IP (افتراضيًا)، أو خاص عبر Tailscale أو AWS VPC Lattice | تتواصل خدمات الإدارة مع خادم EKS API عبر الشبكة العامة، مع تقييد الوصول باستخدام قوائم السماح لعناوين IP. بعد النشر الأولي، يمكنك اختياريًا التبديل إلى وصول خاص عبر Tailscale أو، على AWS، VPC Lattice. |
...
2. **حاوية وكيل Tailscale**: تعمل حاوية وكيل Tailscale في عنقود EKS لديك، وتكون مسؤولة عن:
...
3. **كبسولة Nginx**: كبسولة Nginx تقوم بما يلي:
   ...
   * توجيه حركة المرور إلى عناوين IP المناسبة داخل عنقود EKS لديك
