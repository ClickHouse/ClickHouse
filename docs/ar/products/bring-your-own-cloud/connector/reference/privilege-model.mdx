...
* **`pcm_scraper` و`pcm_troubleshooter`** هما مستخدما ClickHouse للقراءة فقط اللذان يتصل بهما السحّاب ومستكشف الأخطاء ومصلحها. أما المنفّذ فليس له مستخدم ClickHouse.
...
* **هوية AWS المتاحة من البيئة لعقدة المنفّذ أو مضيفه** تخدم غرضين: فهي تسجّل الدخول إلى Amazon ECR، عبر STS، عندما تسحب عملية مزامنة أو إنشاء للمنصة مخطط Helm. كما تتولّى دور السحب للقراءة فقط الذي سجّلته من أجل فحص الصورة أثناء تحديث المنصة. ولا توجد أي استدعاءات لـ S3 أو IAM. أما عمليات سحب الصور فتتم على عُقدك بموجب دور السحب الخاص بها. ولا يحتفظ المنفّذ بأي بيانات اعتماد لحاويات التخزين أو IAM الخاصة بك.
...
| مثيلات ClickHouse الخاصة بك           | البروتوكول الأصلي لـ ClickHouse                                                                    | استعلامات للقراءة فقط بصفة `pcm_scraper` و`pcm_troubleshooter`، بالإضافة إلى عبارة إفراغ السجل الخاصة بالساحب (انظر [الأذونات](#clickhouse-grants))؛ ولا يفتح المُنفِّذ أي اتصال بـ ClickHouse                                                                                                                                                                                                              |
| خادم واجهة برمجة تطبيقات Kubernetes   | HTTPS                                                                                              | عمليات قراءة محدودة النطاق بمساحة الأسماء وطلبات رموز ServiceAccount للساحب ومستكشف الأخطاء ومصلحها (لكلا الهدفين)؛ وعمليات كتابة المُنفِّذ داخل مساحات أسماء الخدمات، وداخل مساحات أسماء المنصة أثناء أي تحديث معتمد للمنصة؛ وقراءة وتحديث بالاسم الدقيق لسر mTLS الخاص بالموصّل لحفظ الشهادات المُجدَّدة (في تثبيتات Kubernetes فقط؛ أما الجهاز الافتراضي فيكتب التجديدات في ملفات TLS المحلية الخاصة به) |
| Amazon ECR                            | HTTPS، هوية AWS المتاحة من البيئة للمُنفِّذ، أو دور السحب للقراءة فقط الذي يتقلده للتحقق من الصورة | تسجيل الدخول إلى السجل عندما تسحب مزامنة منصة أو عملية إنشاء مخططاً من ECR؛ و`DescribeImages` أثناء تحديث المنصة للتأكد من وجود الصور                                                                                                                                                                                                                                                                       |
| AWS STS                               | HTTPS، هوية AWS المتاحة من البيئة لعقدة المُنفِّذ أو مضيفه                                         | بيانات الاعتماد لتسجيل الدخول إلى ECR، وتقلّد دور السحب للقراءة فقط الذي سجّلته للتحقق من الصورة. ويُعدّ ECR وSTS استدعاءي واجهة برمجة التطبيقات السحابية الوحيدين للمُنفِّذ                                                                                                                                                                                                                                |
...
* **لا بيانات اعتماد لحاويات التخزين الخاصة بك أو لـ IAM.** يُنشئ الأمر `clicklink clctl executor prepare` حاويات التخزين ودور كل خدمة، ويزيلها الأمر `clicklink clctl executor teardown`، وكلاهما يعمل ببيانات اعتمادك. ولا يمكن تولّي هذا الدور إلا من قِبَل الحاويات (pods) الخاصة بتلك الخدمة. ويستخدم المنفّذ هوية AWS المحيطة للعقدة أو المضيف الخاص به في حالتين فقط: تسجيل الدخول إلى Amazon ECR عبر STS عند سحب مخطط Helm، وتولّي دور السحب للقراءة فقط للتحقق من الصورة أثناء تحديث المنصة. ولا يستطيع قراءة بياناتك أو حذفها.
...
* **إنشاء خدمة.** يعمل الأمر `clicklink clctl executor prepare` باستخدام بيانات اعتماد AWS الخاصة بك لإنشاء الـ buckets وIAM role، ويقدّم الأمر `clicklink clctl instances create` التعريف. لا تستطيع ClickHouse Cloud إنشاء خدمة في عنقودك بدونهما معًا. راجع [إنشاء خدمة](/ar/products/bring-your-own-cloud/connector/managed-services#create-a-service).
...
* **التنظيف بعد الحذف.** حذف خدمة يزيل موارد Kubernetes الخاصة بها فقط. أما الأمر `clicklink clctl executor teardown`، الذي يُشغَّل ببيانات اعتمادك، فيزيل IAM role، ويزيل البيانات والنسخ الاحتياطية فقط عند تمرير أعلام صريحة. راجع [حذف خدمة](/ar/products/bring-your-own-cloud/connector/managed-services#delete-a-service).
