...
<Note>
  تتطلب سير عمل الوضع المُدار الموصوفة هنا [دعمًا متوافقًا في واجهة سطر الأوامر وفي النشر](/ar/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) يؤكده فريق حسابك. الإصدار العام `v0.16.0` لا يتضمن `init --managed` ولا `init --no-managed`، كما أنه لا يوفّر سير عمل إنشاء العميل ولا الأمر `platform`. وتصف الأذونات أدناه سير العمل هذه بمجرد توفّر ذلك الدعم.
</Note>

## الهويات {#identities}

يستخدم التسجيل في الوضع المُدار، وإنشاء خدمات العملاء، والموافقة على المنصة هذه الهويات فقط في حالة [بنية ونشر مدعومَين](/ar/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability).
...
* **ملف تعريف المثيل الخاص بجهاز EC2 الافتراضي للموصّل**، من أجل الوصول المباشر إلى المنصة، يُستخدم لتولّي دور سحب ECR للقراءة فقط الخاص ببيئتك، وذلك لتسجيل الدخول لمخططات Helm ولفحوصات الصور معًا. وتستخدم واجهة سطر الأوامر الخاصة بالموافقة المسار نفسه لملف تعريف المثيل؛ ولا تحل محله ملفات تعريف AWS على محطة العمل ولا بيانات اعتماد SSO. أما سجلات مخططات ECR الأخرى فتستخدم بيانات اعتماد AWS المتاحة من البيئة. ولا توجد أي استدعاءات لـ S3 أو IAM. أما عمليات سحب الصور فتتم على عُقدك بموجب دور السحب الخاص بها. ولا يحتفظ المنفّذ بأي بيانات اعتماد لحاويات التخزين أو IAM الخاصة بك. راجع [بيانات اعتماد السجل](/ar/products/bring-your-own-cloud/connector/platform-updates#registry-credentials).
...
هذه هي القائمة الكاملة للاتصالات التي يفتحها الموصّل، وجميعها تنشأ من داخل بيئتك.
...
| مثيلات ClickHouse الخاصة بك           | البروتوكول الأصلي لـ ClickHouse                                                                                                                                                      | استعلامات للقراءة فقط بصفة `pcm_scraper` و`pcm_troubleshooter`، بالإضافة إلى عبارة إفراغ السجل الخاصة بـ scraper (انظر [الأذونات](#clickhouse-grants))؛ ولا يفتح المُنفِّذ أي اتصال بـ ClickHouse                                                                                                                                                                                                              |
| خادم واجهة برمجة تطبيقات Kubernetes   | HTTPS                                                                                                                                                                                | عمليات قراءة محدودة النطاق بمساحة الأسماء وطلبات رموز ServiceAccount لـ scraper ومستكشف الأخطاء ومصلحها (لكلا الهدفين)؛ وعمليات كتابة المُنفِّذ داخل مساحات أسماء الخدمات، وداخل مساحات أسماء المنصة أثناء أي تحديث معتمد للمنصة؛ وقراءة وتحديث بالاسم الدقيق لسر mTLS الخاص بالموصّل لحفظ الشهادات المُجدَّدة (في تثبيتات Kubernetes فقط؛ أما الجهاز الافتراضي فيكتب التجديدات في ملفات TLS المحلية الخاصة به) |
...
أما على الجانب الوارد، فيُتيح كل خادم خفي منفذ سلامة محلياً واحداً يقدّم أيضاً مقاييس Prometheus الخاصة به، إلى جانب بوابة الجلسات الاختيارية. وترتبط واجهة برمجة تطبيقات الأوامر المحلية للمُنفِّذ بالحلقة المحلية (loopback) فقط، ولا يستمع أي شيء آخر. ولا تتصل ClickHouse Cloud أبداً ببيئتك: فكل ما تستطيعه هو الرد على اتصالي WebSocket الصادرين.

### امتيازات ClickHouse {#clickhouse-grants}

ينشئ التوفير مستخدمًا واحدًا بصلاحية القراءة فقط لكل مكوّن مراقِب. يُنشأ المستخدمون باستخدام `IDENTIFIED WITH bcrypt_hash`، لذا لا تحتوي SQL الخاصة بالتوفير إلا على تجزئة bcrypt مملّحة. أما كلمة المرور بنص واضح فلا توجد إلا في ملف بيانات الاعتماد الذي يقرأه البرنامج الخفي وقت التشغيل. وسطر `ALTER USER` الذي يلي كل `CREATE USER` هو ما يعيد توليد التجزئة عند استخدام `--force`. ولا يملك المنفّذ أي مستخدم ClickHouse ولا يتصل بـ ClickHouse إطلاقًا. وهذه هي الامتيازات تحديدًا، مع مجموعات الجداول الافتراضية:
...
يلزم امتياز `READ ON REMOTE` لأن استعلامات السحب تُغلّف كل جدول نظام ضمن `clusterAllReplicas()`. أما `SYSTEM FLUSH LOGS` فهو الامتياز الوحيد على مستوى النظام؛ إذ يفرض على جداول النظام `*_log` حفظ الإدخالات المخزّنة مؤقتًا لديها بحيث ترى عمليات السحب البيانات الحالية، ولا يتيح قراءة أي شيء أو تعديله. ولا يقبل ClickHouse هذا الامتياز إلا على النطاق العام، ولا يطبّقه إلا على جداول السجلات، لذا يكون الامتياز الممنوح أوسع من القدرة الفعلية.
...
| `namespaces`                                          | `get`، `create`، `update`، `delete`                           | إنشاء مساحة الأسماء للخدمة وحذفه مع الخدمة؛ الأسماء ذات البادئة فقط                                                                                                                                                                                                                                                  |
| `roles`، `rolebindings` (`rbac.authorization.k8s.io`) | `get`، `create`، `update`، `patch`                            | وصول الـ scraper للقراءة فقط داخل مساحة الأسماء كل خدمة؛ وآلية منع التصعيد في Kubernetes تقصر ذلك على الصلاحيات التي يمتلكها الـ executor بالفعل. ولا يجهّز الـ executor أي شيء لمستكشف الأخطاء ومصلحها؛ راجع [جلسات الدعم لخدمة مُدارة](/ar/products/bring-your-own-cloud/connector/managed-services#support-sessions) |
| `serviceaccounts/token`                               | `create`                                                      | بأسماء محددة بدقة فقط: `pcm-executor` وServiceAccount الخاص بالـ scraper، لتجديد الرموز                                                                                                                                                                                                                          |
| `secrets`                                             | `get`، `update`                                               | Secret الخاص بـ mTLS للموصل فقط، بالاسم المحدد بدقة، لحفظ شهادة العميل المجدَّدة. على عمليات التثبيت على Kubernetes فقط، ويُمنح عبر Role الخاص بمخطط Helm في مساحة أسماء الموصل                                                                                                                                    |
| `configmaps`                                          | `get`، `update`، `patch`، `create`                            | ConfigMap باسم `clicklink-instance-registry` في مساحة أسماء الموصل. على عمليات التثبيت على Kubernetes فقط، ويُمنح عبر Role الخاص بمخطط Helm؛ والأفعال `get` و`update` و`patch` مقيّدة بالاسم، وسياسة الـ admission تحصر `create` في ذلك الاسم                                                                      |
...
* **إنشاء خدمة.** يعمل الأمر `clicklink clctl executor prepare` باستخدام بيانات اعتماد AWS الخاصة بك لإنشاء الـ buckets ودور IAM، ويقدّم الأمر `clicklink clctl instances create` التعريف. لا تستطيع ClickHouse Cloud إنشاء خدمة في عنقودك بدونهما معًا. راجع [إنشاء خدمة](/ar/products/bring-your-own-cloud/connector/managed-services#create-a-service).
...
* **إتاحة Gateway.** تكون بوابة الجلسة معطّلة ما لم تفعّلها، ولا يمكن الوصول إليها إلا عبر إعادة توجيه المنفذ ما لم تختر استخدام مورد Ingress. على جهاز افتراضي، يجب على كل مشغّل تثبيت بصمة شهادتها الموقّعة ذاتيًا، أو التحقق منها باستخدام CA bundle يُمرَّر عبر `--gateway-ca`، قبل أن تتمكن أوامر الجلسة من الاتصال بها.
* **حركة الشبكة الصادرة.** عند استخدام CNI يفرض السياسات، لا يصل أي خفي إلى نقطة نهاية الموصل لديك حتى تدرج نطاقات CIDR الخاصة بها في `networkPolicy.allowEgressCIDRs`. كما يحتاج executor إلى نطاقات CIDR الخاصة بخادم API في Kubernetes (`networkPolicy.apiserverCIDRs`)، والتي يشتقها `init --managed` من VPC الخاص بالعنقود. وإذا كانت مخططات المنصة تأتي من Amazon ECR، فأضف نطاقات ECR وSTS أيضًا.

## كيف يُنسب الوصول {#how-access-is-attributed}

* **هوية النشر.** الاسم الشائع (common name) في شهادة عميل mTLS هو معرّف مؤسستك، مع اسم DNS واحد مرتبط بمضيف نقطة النهاية لديك، بحيث يكون كل اتصال بواجهة برمجة التطبيقات قابلًا للنسب إلى مؤسستك. ويجري التجديد تلقائيًا داخل البرنامج الخفي؛ ولا يتعامل أي مشغّل مع مادة المفتاح.
...
* **`query_log` مُستثنى من عمليات السحب افتراضيًا.** فأعمدته تحمل SQL خامًا يتضمن قيمًا حرفية قد تحتوي على بيانات شخصية أو أسرار، ولذلك لا يخرج من حدود بيئتك إلا إذا أضفته بنفسك.
