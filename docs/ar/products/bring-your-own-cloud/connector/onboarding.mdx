...
تنقلك هذه الصفحة من رمز التسجيل إلى connector متحقَّق منه في وضع Managed، حيث يطبّق executor دورة حياة الخدمة التي تديرها ClickHouse Cloud. يُثبَّت connector على عنقود Kubernetes ‏(Helm) أو جهاز افتراضي يعمل بنظام Linux ‏(systemd). يُعد التسجيل بالرمز المسار القياسي؛ وعندما لا تستطيع بيئتك الوصول مباشرةً إلى نقطة نهاية connector الخاصة بك، فراجع [عمليات التثبيت المعزولة شبكيًا والمنعكسة](#air-gapped-and-mirrored-installs). ولمراقبة ClickHouse الذي تشغّله بنفسك دون executor، مرّر `--no-managed`.

## توافر الوضع المُدار {#managed-mode-availability}

<Warning>
  اعتبارًا من 2026-09-22، يقدّم المُثبِّت العام الإصدار `v0.16.0`، وهو لا يتضمّن العلامتين `init --managed` و`init --no-managed` ولا الأمر `platform`. كما أن موافقة المنصة في عمليات التثبيت على جهاز افتراضي يعمل بنظام Linux وعلى Kubernetes، وإنشاء خدمة العميل (`instances create --from-prepare`)، غير متوفرة بعد في أي إصدار عام. وتتطلب موافقة المنصة على Kubernetes أيضًا دعمًا في واجهة سطر الأوامر وفي المخطط للخيار `platform approve --secret-namespace`.

  قبل اتباع سير عمل الوضع المُدار، اطلب من فريق حسابك في ClickHouse تأكيد دعم هدف النشر الخاص بك، وتزويدك بواجهة سطر الأوامر المطابقة، ومخطط الموصّل أو الصورة، وتعليمات التثبيت. ويتطلب إنشاء خدمة العميل أيضًا دعمًا في نقطة نهاية الموصّل وفي ClickHouse Cloud؛ إذ لا يكفي وجود إصدار أحدث من واجهة سطر الأوامر وحده. أما أوامر التثبيت العامة الواردة أدناه فهي لا تُثبِّت حاليًا إصدارًا يدعم عمليات سير العمل هذه.
</Warning>
...
* ما إذا كان connector يعمل على جهاز افتراضي يعمل بنظام Linux أم داخل العنقود
* مجموعة العقد المخصّصة لمكوّنات المنصة
* بادئة النطاق الاسمي للخدمات (القيمة الافتراضية `ns-`)
* كيفية سحب عنقودك لصور ClickHouse: عبر وصول السجل الذي تهيّئه ClickHouse Cloud لحسابك، أو عبر مرآتك الخاصة

بعدها تُصدر ClickHouse Cloud نقطة نهاية connector الخاصة بك ورمز التسجيل يُستخدم لمرة واحدة. ويحدّد هذا الرمز بيئتك، لذا لن تحتاج إلى إدخال أي معرّف إضافي عند الالتحاق.

### لكل عملية تثبيت {#prerequisites-every-install}

* **إصدار يدعم [الوضع المُدار](#managed-mode-availability)** لاتباع تعليمات الوضع المُدار الواردة أدناه. لا يحتوي الإصدار العام `v0.16.0` على `--managed` ولا على `--no-managed`.
...
* **عنقود Amazon EKS** للوضع المُدار. يرفض `init` الوضع المُدار على أي عنقود آخر، ويشير إلى `--no-managed` بوصفه وسيلة التثبيت من دون executor.
...
* **ملف kubeconfig** يشير سياقه الحالي إلى عنقود EKS، ويجب أن يمتلك صلاحية إنشاء وقراءة namespace الخاص بالـ connector، وتطبيق Secrets، وإنشاء ServiceAccounts وRoles وRoleBindings، وتثبيت الـ chart، ومنح الـ executor صلاحياته على مستوى العنقود.
* **AWS credentials** على جهاز العمل (عبر البيئة أو profile) تتضمن `eks:DescribeCluster` و`ec2:DescribeVpcs`. يستخدمها `init` لقراءة كتل CIDR الخاصة بشبكة VPC لعنقودك من أجل NetworkPolicy. وفي حال فشل البحث، يُصدر تحذيرًا ويترك لك تعبئة تلك القائمة.
* **نطاقات CIDR الواقعة خلف نقطة نهاية الـ connector لديك**، لتمكين NetworkPolicy ذات الرفض الافتراضي في الـ chart أثناء التثبيت. وبدونها، يهيّئ `init` السياسة معطّلة ويعرض كيفية تمكينها لاحقًا.
...
<Steps>
  <Step title="احصل على نقطة نهاية الموصل ورمز التسجيل" id="get-endpoint-and-token">
    ...
    الرمز صالح للاستخدام مرة واحدة فقط وتنتهي صلاحيته سريعًا، لذا نفّذ عملية التسجيل بعد استلامه بوقت قصير. تعامل معه كمعلومة سرية: تقرأه واجهة CLI من موجّه مخفي أو من السطر الأول من stdin، ولا تقرأه مطلقًا من وسيطات سطر الأوامر أو القرص أو السجلات. وإذا انتهت صلاحيته قبل إتمام التسجيل، اطلب رمزًا جديدًا من فريق حسابك في ClickHouse.
  </Step>

  <Step title="ثبّت واجهة سطر الأوامر (CLI) وتحقّق من عملها" id="install-and-verify-the-cli">
    أمر واحد يثبّت ملفًا تنفيذيًا موثّقًا باسم `clicklink`. فهو يكتشف منصّتك (macOS أو Linux، amd64 أو arm64)، وينزّل الإصدار الحالي، ويتحقق من قيمة التحقق SHA-256، ومن توقيع الإصدار أيضًا إذا كان cosign مثبّتًا. ويثبّت الملف التنفيذي في `/usr/local/bin` إن كان قابلًا للكتابة، وإلا ففي `~/.local/bin` (أضف هذا المسار إلى `PATH` إذا لزم الأمر؛ ويمكن تجاوز ذلك عبر `--bin-dir`). أما للتثبيت على Kubernetes، فشغّل الأمر من أي حاسوب إداري يملك وصولًا إلى العنقود عبر kubeconfig:

    ```bash
    curl -fsSL https://releases.clicklink.clickhouse.com/install.sh | bash
    ```

    للتثبيت على جهاز افتراضي، شغّل البرنامج النصي نفسه على المضيف مع `--host`. وبعد التحقق من التنزيل، يُنشئ أيضاً مستخدم النظام `clicklink`، وأدلة `/etc/clicklink` و`/var/lib/clicklink` و`/var/log/clicklink`، ووحدات systemd للمكوّنات الثلاثة جميعها. كما يهيّئ الملف `/etc/clicklink/redaction-patterns.yaml` بمجموعة أولية من patterns (مع الإبقاء على الملف إن كان موجوداً)، بحيث تبدأ الخطوة التالية من enrollment:
  ...
  <Step title="سجّل الموصل وثبّته" id="enroll-and-install">
    تتطلب هذه الأوامر دعم البناء والنشر الموضّح في [توافر الوضع المُدار](#managed-mode-availability). أما الإصدار العام `v0.16.0` فلا يتضمّن `--managed` ولا `--no-managed`.

    التسجيل يتم بأمر واحد. فهو يستهلك الرمز الخاص بك، ويحصل على شهادة عميل موقّعة وعلى منحة الـ executor على مستوى العنقود، ثم يثبّت الـ connector ويتحقق منه.
  ...
  <Step title="تنظيف" id="clean-up">
    توجد حزمة التسجيل `handoff.yaml` (بوضع `0600`، في دليل العمل) حتى لا تحتاج عمليات إعادة التشغيل والاسترداد أثناء التثبيت إلى رمز ثانٍ. وهي تحتوي على سر واجهة برمجة التطبيقات الخاص بالموصّل كنص صريح، لذا احذفها بمجرد التحقق من نجاح التثبيت:
    ...
    يحتفظ الـ connector قيد التشغيل بنسخته الخاصة من الـ credentials، لذا لا يعتمد أي جانب تشغيلي على هذا الملف. كما أن عمليات الترقية وتغييرات التهيئة لا تحتاج إليه إطلاقًا. وإذا احتجت لاحقًا إلى تنفيذ `init` مجددًا، فاطلب enrollment token جديدًا من فريق حسابك في ClickHouse ثم نفّذ `init --enroll --force`.
  </Step>
</Steps>
...
إذا رفض الأمر `init` الخيار `--managed` أو `--no-managed` باعتباره علامة غير معروفة، فتحقق من إصدار واجهة سطر الأوامر لديك: فالإصدار العام `v0.16.0` لا يدعم أيًّا منهما. احصل على [إصدار يدعم الوضع المُدار](#managed-mode-availability) من فريق حسابك قبل اتّباع تعليمات استعادة الوضع المُدار أدناه.

أعد تنفيذ الأمر نفسه. فالأمر `init` متكافئ الاستدعاء: إذ تؤدي إعادة تنفيذه إلى الوصول إلى الحالة نفسها، مع الحفاظ على الإعداد الحالي والملفات المُجهَّزة، وتخطّي ما اكتمل من عمل. وعندما تفشل إحدى الخطوات في منتصفها، تطبع واجهة سطر الأوامر أوامر الاستعادة الدقيقة الملائمة لحالتك، وهي آمنة للتكرار.

إذا رُفض التسجيل، فهذا يعني أن الرمز قد استُخدم بالفعل، أو أنه غير صالح أو منتهي الصلاحية. في حالة الرمز المُستخدَم، أعد التنفيذ مع `--handoff handoff.yaml`، وهو ملف يبقى موجودًا حتى خطوة التنظيف؛ وإلا فاطلب رمزًا جديدًا من فريق حساب ClickHouse الخاص بك. وإذا فشل التسجيل بسبب خطأ في النقل، فأعد تنفيذ الأمر نفسه. وإذا رُفضت إعادة التنفيذ، فهذا يعني أن المحاولة الأولى استهلكت الرمز فعلًا، لذا اطلب رمزًا جديدًا.
...
`--force` هو إعادة تعيين صريحة، وليس إعادة محاولة روتينية. فهو يستبدل الإعداد المحفوظ أو طبقة القيم المُعدِّلة، ويعيد توليد مفتاح العميل، ويستبدل شهادة عميل لم تنتهِ صلاحيتها بعد (وورود الرمز `409` من نقطة نهاية التوقيع يعني أن هناك شهادة موجودة بالفعل). يُحفَظ معرّف UUID الخاص بعنقود الـ موصل عند استخدام `--force`، وكذلك وضعه في نمط Managed. وتُبقي عمليات إعادة التشغيل غير التفاعلية على هذا الوضع، وعلى الطرفية يكون الـ prompt مضبوطًا عليه افتراضيًا، ما لم تمرّر `--managed` أو `--no-managed`. استخدم `--force` عند تدوير بيانات الاعتماد أو استبدال شهادة، وراجع [العمليات](/ar/products/bring-your-own-cloud/connector/operations) للاطلاع على النموذج الكامل لإعادة التشغيل والاستعادة.