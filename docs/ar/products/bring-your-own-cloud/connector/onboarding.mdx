...
* **نقطة نهاية connector ورمز التسجيل الخاصان بك**، ويُقدَّمان أثناء [التسجيل](#registration).
* **حركة الخروج (egress) على المنفذ 443** إلى `https://<subdomain>.<connector-domain>` و`https://<subdomain>.enroll.<connector-domain>`، إضافة إلى `releases.clicklink.clickhouse.com` وAmazon ECR Public وقت التثبيت. وفي الوضع المُدار (managed mode) يتصل executor أيضًا بـ Amazon ECR وSTS عندما تسحب مزامنة أو عملية إنشاء على مستوى المنصة مخططًا (chart) من ECR. وإذا تعذّر الوصول إلى أيٍّ من هذه الوجهات، راجع [عمليات التثبيت المعزولة شبكيًا والمنعكسة](#air-gapped-and-mirrored-installs).
...
* **لا وجود لـ ClickHouse بعد.** يبدأ التثبيت المُدار من دون أي خدمات؛ أما الخدمات التي ينشئها executor فتسجّل نفسها تلقائيًا. ولا يسجّل `init` في الوضع المُدار أي ClickHouse تشغّله بنفسك (وعلى Kubernetes يرفض `--instance` و`--instance-namespace`). ولمراقبة عنقود من هذا النوع، ثبّت نشرًا (deployment) منفصلًا لـ connector باستخدام `--no-managed`؛ ويتيح `--ch-user-suffix` للنشرين مشاركة instance واحد من دون تعارض في مستخدمي connector. ويحتاج ذلك النشر إلى مستمع native يمكن الوصول إليه: المنفذ الآمن 9440 أو المنفذ بنص صريح 9000، ويُكتشف تلقائيًا على Kubernetes. كما يحتاج إلى وصول ClickHouse admin من أجل التجهيز: مستخدم `default` بلا كلمة مرور، أو كلمة مرور تُمرَّر عبر `--ch-admin-password-stdin` أو عند المطالبة بها، أو حقن CR للـ instances المُدارة بواسطة مشغّل.
...
<Steps>
  ...
  <Step title="سجّل الموصل وثبّته" id="enroll-and-install">
    التسجيل يتم بأمر واحد. فهو يستهلك الرمز الخاص بك، ويحصل على شهادة عميل موقّعة وعلى grant الخاص بالـ executor على مستوى العنقود، ثم يثبّت الـ connector ويتحقق منه.
...
أعد تنفيذ الأمر نفسه. فالأمر `init` متكافئ الاستدعاء: إذ تؤدي إعادة تنفيذه إلى الوصول إلى الحالة نفسها، مع الحفاظ على الإعداد الحالي والملفات المُجهَّزة، وتخطّي ما اكتمل من عمل. وعندما تفشل إحدى الخطوات في منتصفها، تطبع الـ CLI أوامر الاستعادة الدقيقة الملائمة لحالتك، وهي آمنة للتكرار.
