...
تتيح لك جلسات الدعم منح ClickHouse وصولًا مؤقتًا لأغراض التشخيص عبر ClickHouse Connector. تغطي هذه الصفحة ماهية الجلسة، وكيفية تفعيلها وإيقافها، وما يمكن لمهندسي دعم ClickHouse فعله أثناءها، وكيفية تدقيقها.

## ما هي جلسة الدعم {#what-a-support-session-is}

جلسة الدعم هي نافذة زمنية محددة المدة يقبل خلالها مستكشف الأخطاء ومصلحها الأوامر من مهندسي دعم ClickHouse. أما خارج الجلسة فيرفض مستكشف الأخطاء ومصلحها كل أمر، حتى مع بقاء اتصال WebSocket الصادر الخاص به قائماً. ولا يوجد أي مسار تنفيذ آخر: فمستكشف الأخطاء ومصلحها لا ينفّذ شيئاً من دون جلسة، ولا تستطيع ClickHouse فتح جلسة نيابةً عنك. كما أن ClickHouse Cloud لا تتصل إطلاقاً ببيئتك من الخارج؛ فهي لا تتلقى سوى ما يرسله مستكشف الأخطاء ومصلحها عبر قناته الصادرة.
...
<Tabs>
  <Tab title="Kubernetes" id="sessions-kubernetes">
    تستمع البوابة على المنفذ 8443 في حاوية pod الخاصة بـ مستكشف الأخطاء ومصلحها. مع وجود وصول إلى الكتلة، فصِل إليها عبر إعادة توجيه المنفذ:
...
تنتهي صلاحية الجلسات تلقائيًا. المدة الافتراضية هي 4 ساعات؛ ويقبل الأمر `session enable --duration` مدة تصل إلى 24 ساعة. عند انتهاء صلاحية الجلسة أو فور تشغيل `session disable`، تتوقف أداة مستكشف الأخطاء ومصلحها عن قبول الأوامر. التعطيل هو مسار الإلغاء الفوري: لا إعادة تشغيل، ولا تنسيق مع ClickHouse.

## قائمة السماح بالمشغّلين {#operator-allowlist}

يُصرَّح بكل استدعاء للبوابة بناءً على قائمة سماح بعناوين البريد الإلكتروني للمشغّلين. وتتم المطابقة مع عنوان البريد الإلكتروني المثبت في رمز OIDC الذي تم التحقق من صحته، وليس مع أي معلومات يدّعيها العميل عن نفسه.
...
وفي كلا الهدفين تعيد البوابة قراءة الملف عند الطلب التالي بعد مرور 30 ثانية على آخر قراءة، لذا تدخل التعديلات حيز التنفيذ دون إعادة تشغيل.
