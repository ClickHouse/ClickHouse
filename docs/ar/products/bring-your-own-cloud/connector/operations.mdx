...
إصدارات Chart هي وسوم الإصدار من دون بادئة `v` (يتوافق Chart بالإصدار `X.Y.Z` مع الوسم `vX.Y.Z`). يشير Chart المنشور مسبقاً إلى image الحاوية العامة، لذا لا تتطلب عمليات التثبيت والترقية العادية قيماً لـ image. وللاطلاع على القيم الافتراضية لـ Chart، شغّل `helm show values clicklink-connector --repo https://releases.clicklink.clickhouse.com/charts`. يعمل executor بنسخة replica واحدة باستخدام strategy الخاصة بـ `Recreate`، لذا تؤدي الترقية إلى قطع اتصاله لفترة وجيزة. راجع [عندما يكون connector غير متصل](/ar/products/bring-your-own-cloud/connector/managed-services#when-the-connector-is-offline) لمعرفة ما يحدث للأوامر خلال تلك المدة.
...
أعِد تشغيل برنامج التثبيت على المضيف. إذ سيُنزّل الإصدار الأحدث ويتحقق منه بالطريقة نفسها المتبعة أثناء [الإعداد الأولي](/ar/products/bring-your-own-cloud/connector/onboarding). وسيُنشئ نسخة احتياطية من الملف التنفيذي السابق، ويثبّت وحدات systemd الحالية، مع الاحتفاظ بأنماط الإخفاء وملف البيئة النشطين لديك. ثم أعد تشغيل الخدمات:
...
على جهاز افتراضي، يقتصر `init` على تمكين وبدء الوحدات التي لا تعمل بعد، لذا أعد تشغيل الخدمات الخفية بعد التدوير:

```bash
sudo systemctl restart clicklink-scraper clicklink-troubleshooter clicklink-executor
```

في Kubernetes، ينشئ `init` مسبقًا Secretَي `clicklink-hmac` و`clicklink-mtls` المُدوَّرين؛ ولا ينشئهما المخطط. ولذلك لا يشملهما التعليق التوضيحي لقيمة التحقق الخاص بالمخطط، وتحتفظ البودات قيد التشغيل ببيانات الاعتماد التي بدأت بها. أعد تشغيل كل مكوّن بعد التدوير:
...
على جهاز افتراضي، نفِّذها على المضيف مع تمرير كلمة مرور المستخدم الإداري عبر pipe؛ وعلى الجهاز الافتراضي تكون كلمة المرور إلزامية إلا إذا مرّرت `--ch-user-via cr`:
...
<Note>
  لا تتضمّن واجهة سطر الأوامر العامة `v0.16.0` الخيار `--managed` ولا `--no-managed`. ويتطلب سلوك الحفاظ على وضع مُدار وتجاوزه المذكور أدناه إصدارًا مبنيًا يدعم [وضع مُدار](/ar/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)؛ تأكّد من إصدارك مع فريق حسابك قبل تغيير ذلك الوضع أثناء الاسترداد.
</Note>

تتقارب عمليات إعادة تشغيل `init`، لذا تكون إعادة تنفيذ الأمر نفسه دائمًا هي الخطوة الأولى. من دون `--force`، يُحتفَظ بملف `/etc/clicklink/config.yaml` الموجود (VM) أو تراكب `clicklink-values.yaml` (Kubernetes)، ويُعاد استخدام مفتاح العميل الموجود؛ ويستبدل `init` بيانات الاعتماد وسلسلة CA بصورة ذرية. ويمكن تكرار أوامر الاسترداد التي تطبعها CLI بعد فشل جزئي بأمان.
...
**الموصل غير متصل.** أثناء تعطل الموصل أو تعذّر الوصول إليه، تستمر الخدمات التي يديرها في العمل. إذ يبقيها ClickHouse Operator في العنقود الخاص بك قيد التشغيل، كما أن المنفّذ ليس ضمن مسار البيانات. تحتاج تغييرات دورة الحياة إلى اتصال المنفّذ، لذا لا يُطبَّق أي منها حتى يعيد الاتصال. وعند إعادة التشغيل، يستعيد المنفّذ من قاعدة بيانات الأوامر الخاصة به الأوامرَ التي كانت قيد التنفيذ عند توقفه. ويُجاب عن أي أمر تعيد ClickHouse Cloud تسليمه من النتيجة المسجَّلة بدلًا من تنفيذه مرتين؛ ولا يُعاد تنفيذ سوى الأمر الفاشل. لمعرفة ما يحدث لأوامر دورة الحياة في حال عدم اتصال أي منفّذ، راجع [عندما يكون الموصل غير متصل](/ar/products/bring-your-own-cloud/connector/managed-services#when-the-connector-is-offline).
...
يتوفر `uninstall.sh` ضمن أرشيف tar الخاص بالإصدار. إذا لم يتبقَّ على المضيف أي أرشيف tar مستخرج، فاجلب أرشيفًا واستخرجه كما هو موضح في [التنزيل والتحقق اليدويين](/ar/products/bring-your-own-cloud/connector/onboarding#manual-download-and-verification)، ثم شغّله من الدليل المستخرج:

```bash
sudo ./uninstall.sh
```

يؤدي ذلك إلى إيقاف وتعطيل وحدات `clicklink-scraper` و`clicklink-troubleshooter` و`clicklink-executor` وإزالة وحدات systemd والملف التنفيذي. ويُبقي على ما يلي، بحيث تستخدم إعادة التثبيت لاحقًا التكوين الحالي:
...
أنشأ الـ grant أيضًا مساحة الأسماء `clicklink-system`؛ احذفها عندما لا يستخدمها أي شيء آخر.
...
يحمل PersistentVolumeClaim الخاص بالـ executor التعليق التوضيحي `helm.sh/resource-policy: keep`، لذا يُبقيه إلغاء التثبيت قائمًا ومعه قاعدة بيانات الأوامر وسجل الخدمات. كذلك فإن الـ Secrets التي أنشأها `init`، وSecret حزمة المنصة الذي كتبته موافقة المنصة، وConfigMap سجل الخدمات الذي يتولاه الـ executor ليست مملوكة للمخطط، وتبقى هي الأخرى بعد إلغاء التثبيت. احذفها صراحةً، بما في ذلك زوج Secret الوصول وServiceAccount لكل instance سجّلته بنفسك ولكل خدمة مُدارة جهّزت لها مستكشف الأخطاء ومصلحها (ويحمل اسم الخدمة). ويغطي `--ignore-not-found` الكائنات التي لم تُنشأ أصلًا، مثل Secret حزمة المنصة قبل أول موافقة:
...
طبّق `init` صلاحية executor على مستوى العنقود بأكمله، وليس المخطط. أزِلها بصفتك مسؤول العنقود بعد ألّا تبقى أي خدمات مُدارة:
