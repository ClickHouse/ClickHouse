...
<Steps>
  ...
  <Step title="تنفيذ الموافقة" id="run-approve">
    ...
    يحتاج `approve` إلى kube context يمتلك صلاحيات cluster-admin على العنقود المُدار (`--context <name>` إذا كان kubeconfig يحتوي على عدة سياقات). وعندما يكون المخطط موجودًا في Amazon ECR فإنه يحتاج أيضًا إلى AWS credentials قادرة على قراءته. أما `--dry-run` فيُصيّر شطر الـ permission ويسرده دون تطبيق أو إصدار أي شيء.
    ...
    <Tabs>
      ...
      <Tab title="Kubernetes" id="approve-kubernetes">
        يقرأ `approve` الحزمة المجهّزة عبر الـ port-forward من الخطوة السابقة. وهو يسلّم حزمة الـ token على هيئة Secret في نطاق أسماء الـ connector، ويربطه المخطط داخل الـ pod. نفّذه من workstation يصل kubeconfig الخاص به إلى العنقود، مع نسخة من تهيئة الـ connector. فالمخطط يُصيّر نسخة منها في ConfigMap الخاص بالـ executor؛ ويحتاجها `approve` من أجل prefix نطاق الأسماء، ولا يقرأ منها أي credentials.
