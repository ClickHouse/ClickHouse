...
description: 'هيّئ ClickHouse Connector: الوضع المُدار، ومثيلات ClickHouse، وقوائم السماح الخاصة بالمشغّل، وسياسات الشبكة، وإخفاء المعلومات الحساسة، والمرايا الخاصة، والتخزين'
...
<Note>
  لا تحتوي واجهة سطر الأوامر العامة `v0.16.0` على `--managed` ولا `--no-managed`. وقبل استخدام أي من العلامتين لتغيير التهيئة، تأكد من [توافر الوضع المُدار](/ar/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) واحصل على الإصدار المطابق من فريق حسابك.
</Note>

يشغّل الوضع المُدار المنفّذ (المنفّذ) الذي يطبّق على عنقودك دورة حياة الخدمة التي تديرها ClickHouse Cloud. ويفعّله الأمر `init` عند التسجيل ويملأ هوية العنقود (راجع [تمكين الوضع المُدار](/ar/products/bring-your-own-cloud/connector/managed-services#enable-managed-mode))؛ أما المفاتيح الواردة أدناه فهي التي قد تعود إليها لاحقًا. ولتغيير الوضعية المُهيّأة، أعد تنفيذ `init` مع `--force` و`--managed` أو `--no-managed`. ومثل كل تنفيذ للأمر `init`، يتطلب ذلك نقطة دخول: إما `--handoff` إن كنت قد احتفظت بالحزمة (bundle)، أو `--enroll` مع رمز جديد، كما هو موضّح في [إعادة التنفيذ والاستعادة](/ar/products/bring-your-own-cloud/connector/operations#re-runs-and-recovery).

على جهاز افتراضي، يعطّل `--no-managed` المنفّذ في التهيئة لكنه لا يوقف وحدة systemd قيد التشغيل فعلًا ولا يعطّلها. أوقفها صراحةً قبل تغيير التهيئة:

```bash
sudo systemctl disable --now clicklink-executor
```

على Kubernetes، يؤدي ضبط `executor.enabled: false` في القيم وتطبيق التغيير باستخدام `helm upgrade` إلى إزالة Deployment الخاص بالمنفّذ. وفي كلا الهدفين تبقى صلاحية (grant) العنقود قائمة إلى أن تزيلها؛ اتبع [إيقاف الوضع المُدار](/ar/products/bring-your-own-cloud/connector/reference/faq#stop-managed-mode) لإزالة ذلك الوصول أيضًا.
...
<Tabs>
  <Tab title="Kubernetes" id="managed-mode-kubernetes">
    ...
    * **`cluster.namespacePrefix`** تشترك فيه جميع المكوّنات، وهو البادئة التي يقصر إذن المُنفِّذ (executor) نطاقه عليها. ويتطلب تغييره بعد التثبيت إعادة تنفيذ منح الإذن.
  ...
  <Tab title="جهاز افتراضي يعمل بنظام Linux" id="managed-mode-vm">
    ...
    * **`namespace_prefix`** مفتاح عالي المستوى تتشاركه جميع المكوّنات، وهو البادئة التي يقتصر عليها منح (grant) المنفّذ (executor). ويتطلّب تغييره بعد التثبيت إعادة تنفيذ المنح.
...
<Tabs>
  ...
  <Tab title="جهاز Linux افتراضي" id="instances-vm">
    ...
    ثم وفّر الوصول لكل مكوّن على المضيف بصفتك root. على الجهاز الافتراضي تكون كلمة مرور المستخدم الإداري إلزامية ما لم تمرّر `--ch-user-via cr`؛ مرّرها عبر pipe باستخدام `--ch-admin-password-stdin`. يطبّق كل أمر امتيازات ClickHouse ويعيد تشغيل برنامجه الخفي (يمكنك تخطي إعادة التشغيل باستخدام `--skip-restart`):
...
* **`allowEgressCIDRs`** أنت من يوفّرها: وهي النطاقات الواقعة خلف connector endpoint الخاصة بك، وتُحدَّد عند التثبيت عبر `--egress-cidrs` أو عند المطالبة. وتحتاج إليها جميع المكوّنات للوصول إلى connector endpoint الخاصة بك. وفي الوضع المُدار، عندما تكون مخططات المنصة موجودة في Amazon ECR، أضف كذلك نطاقات ECR وSTS الخاصة بمنطقتك (أو نطاق mirror الخاص بك)؛ فمن دونها تفشل عمليات سحب مخططات executor بموجب السياسة.
* **`apiserverCIDRs`** يشتقّها `init` في الوضع المُدار من شبكة VPC الخاصة بعنقود EKS (`eks:DescribeCluster` و`ec2:DescribeVpcs`). فكل مكوّن يُصدر طلبات رموز Kubernetes، ويدير executor الخدمات عبر واجهة برمجة التطبيقات، ولذلك تُحجب جميعها في ظل CNI يفرض السياسات إلى أن تُضبط هذه القائمة. وعند فشل الـ lookup، يُصدر `init` تحذيرًا ويترك القائمة فارغة لتملأها بكتل CIDR الخاصة بشبكة VPC لديك.

تُحدَّد نطاقات أسماء الخدمة بطريقتين. فالنطاقات التي ينشئها executor تحمل الوسم `clicklink.clickhouse.com/managed-by: executor`، وتسمح بها السياسة استنادًا إلى هذا الوسم، ولذلك يمكن في التثبيت المُدار ترك `clickhouseNamespaces` فارغة. أما النطاقات المنشأة خارج executor، لأجل ClickHouse الذي تشغّله بنفسك، فهي وحدها التي يلزم إدراجها بالاسم.
...
وعند تفعيل session gateway، تبرز قاعدة خروج إضافية مهمة: `clctl.gateway.jwksEgressCIDRs`. إذ يجلب مستكشف الأخطاء ومصلحها JWKS الخاص بمزوّد الهوية لديك للتحقق من operator tokens، ولذلك فإن ترك القائمة فارغة في ظل الرفض الافتراضي يحجب كل عمليات التحقق من الرموز:
...
يتبقى مفتاحان للدخول: `metricsScrapeSelector` الذي يقصر دخول سحب المقاييس على نطاق أسماء واحد لـ Prometheus استنادًا إلى الوسم، و`kubeletProbeCIDRs` الذي يسمح صراحةً بفحوصات سلامة kubelet في ظل سياسة الرفض الافتراضي الصارمة. راجع [مرجع التهيئة](/ar/products/bring-your-own-cloud/connector/reference/configuration) للاطلاع على قائمة المفاتيح الكاملة.

## أنماط إخفاء المعلومات الحساسة {#redaction-patterns}

يُخفي مستكشف الأخطاء وإصلاحها المعلومات الحساسة من مخرجاته قبل أن تغادر نطاقك. تغطي الأنماط المضمّنة `ipv4` و`ipv6` و`bearer-token` و`aws-access-key` و`email` و`jwt` و`ssh-private-key` و`connection-string-credentials`. يمكنك إضافة أنماطك الخاصة في ملف YAML. إذ تُطبَّق أنماطك أولًا حسب ترتيبها في الملف، ثم الأنماط المضمّنة؛ ويستبدل الإدخال الذي يعيد استخدام `name` لنمط مضمّن ذلك النمط.
...
على جهاز افتراضي، يوجد الملف في `/etc/clicklink/redaction-patterns.yaml`. ويهيّئ المثبّت الملف بمجموعة أولية من الأنماط النشطة (مفاتيح واجهات برمجة التطبيقات لأطراف خارجية، ومفاتيح AWS السرية، وما إلى ذلك). يمكنك تقليص هذه المجموعة أو توسيعها كما تشاء؛ ويحافظ المثبّت على نسختك عبر عمليات الترقية. في Kubernetes، ضع ملف YAML في ConfigMap تحت المفتاح `redaction-patterns.yaml` واضبط `troubleshooter.redaction.patternsConfigMap` على اسمه؛ إذ يقوم مخطط بربطه في المسار نفسه.
...
يتطلب مثال الوضع المُدار أدناه بنية مُجمّعة تتضمن `--managed`، وهو ما يفتقر إليه الإصدار العام `v0.16.0`. تأكّد من [توافر الوضع المُدار](/ar/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) قبل استخدامه؛ فعمل مرآة للإصدار العام لا يضيف هذا الدعم.
...
<Tabs>
  ...
  <Tab title="جهاز افتراضي يعمل بنظام Linux" id="storage-vm">
    يخزّن الساحب المقاييس مؤقتًا في `/var/lib/clicklink/buffer` لضمان التسليم مرة واحدة على الأقل عند تعذّر الوصول إلى نقطة نهاية الموصّل لديك. ويحتفظ بها لمدة تصل إلى 168 ساعة أو بحجم يصل إلى 1024 ميغابايت، ويرفعها بمعدل محدود يبلغ 1 ميغابايت/ثانية افتراضيًا:
