...
يشغّل وضع Managed المنفّذ (executor) الذي يطبّق على عنقودك دورة حياة الخدمة التي تديرها ClickHouse Cloud. ويفعّله الأمر `init` عند التسجيل ويملأ هوية العنقود (راجع [تمكين وضع Managed](/ar/products/bring-your-own-cloud/connector/managed-services#enable-managed-mode))؛ أما المفاتيح الواردة أدناه فهي التي قد تعود إليها لاحقًا. ولتفعيل وضع Managed لموصّل (connector) مثبّت بدونه، أو لتعطيله، أعد تنفيذ `init` مع `--force` و`--managed` أو `--no-managed`. يوقف الخيار `--no-managed` المنفّذ لكنه يُبقي صلاحية (grant) العنقود الخاصة به قائمة؛ أزِلها كما هو موضّح في [إيقاف وضع Managed](/ar/products/bring-your-own-cloud/connector/reference/faq#stop-managed-mode). ومثل كل تنفيذ للأمر `init`، يتطلب ذلك نقطة دخول: إما `--handoff` إن كنت قد احتفظت بالحزمة (bundle)، أو `--enroll` مع رمز جديد، كما هو موضّح في [إعادة التنفيذ والاستعادة](/ar/products/bring-your-own-cloud/connector/operations#re-runs-and-recovery).
...
## إضافة نسخ ClickHouse أو تغييرها {#clickhouse-instances}

تُسجِّل الخدمات التي يُنشئها المنفّذ نفسها تلقائيًا، لذا لن تحتاج في الوضع المُدار إلى إضافتها هنا إطلاقًا. أما عملية نشر connector منفصلة مُثبَّتة باستخدام `--no-managed` فتراقب ClickHouse الذي تُشغّله بنفسك؛ ومرِّر `--ch-user-suffix` عند تنفيذ `init` الخاص بها إذا كانت عمليتا النشر تتشاركان النسخة نفسها. ينطبق هذا القسم على عملية النشر تلك. يُحدِّد كل مدخل ضمن `instances` نقطة نهاية للبروتوكول native في ClickHouse يقرأ منها connector: `host` و`port` و`database` و`secure` و`cluster`، وعلى Kubernetes كذلك `namespace`؛ ويحدّ كل من `max_open_conns` و`max_idle_conns` سعة مجمّع الاتصالات. لا تُخزَّن بيانات الاعتماد أبدًا في التهيئة؛ إذ يستخرج كل مكوّن مستخدم ClickHouse الخاص به للقراءة فقط من حزمة الوصول التي يُنشئها التجهيز.
