...
export const globalTransformSource = (source) => {
  ...
  if (isPreview || /clickhouse\.com\/docs(\/|$)/.test(url)) {
    tabs.push('الوثائق');
  } else if (url.indexOf('github.com') !== -1 && /\/issues(\/|$)/.test(url)) tabs.push('الوثائق');
  else if (/\/blog(\/|$)/.test(url)) tabs.push('الوثائق');
  else if (url.indexOf('clickhouse.com') !== -1) tabs.push('الوثائق');
...
ابحث في وثائق ClickHouse وغيرها من موارد ClickHouse.
