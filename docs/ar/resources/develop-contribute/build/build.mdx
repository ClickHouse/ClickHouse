...
نوصي بإنشاء دليل منفصل باسم `build` داخل `ClickHouse` ليحتوي على جميع نواتج البناء:
...
يمكنك استخدام عدة أدلة مختلفة (مثل `build_release` و`build_debug` وما إلى ذلك) لأنواع البناء المختلفة.
