---
title: تكوين إمكانات CAP_IPC_LOCK وCAP_SYS_NICE في Docker
...
عند تشغيل ClickHouse في Docker، يشتكي Docker من عدم توفّر الإمكانتين `CAP_IPC_LOCK` و`CAP_SYS_NICE` في النظام. كيف يمكنني حلّ ذلك؟

فيما يلي شكل رسائل السجل عند عدم توفّر الإمكانية `CAP_SYS_NICE` أو `CAP_SYS_NICE`:
...
1. أضِف وسيطتي `--cap-add` لمنح الحاوية إمكانتَي `IPC_LOCK` و`SYS_NICE`:
...
2. تحقّق من أن إمكانات Linux الأمنية ظاهرة داخل الحاوية باستخدام الأمر التالي:
...
3. اضبط يدويًا كلتا الإمكانتين لـ ClickHouse

```bash
setcap "cap_ipc_lock=+ep cap_sys_nice=+ep" /usr/bin/clickhouse
```

4. تحقّق من أن الإمكانات مطبَّقة.
...
5. أعد تشغيل خادم ClickHouse، ويُفترض ألّا تظهر رسائل السجل.

<br />

اطّلع على هذا [المقال عن إمكانات Linux](https://docs.docker.com/engine/reference/run/#runtime-privilege-and-linux-capabilities) لمزيد من التفاصيل.