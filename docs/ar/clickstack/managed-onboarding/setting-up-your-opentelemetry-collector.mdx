...
<AgentPrompt
  prompt="استخدم curl لتنزيل هذا الملف وقراءته واتباع تعليماته: https://clickhouse.com/docs/skills/clickstack-otel-collector/SKILL.md"
  description="سيتولى وكيل لديك إعداد ClickStack OpenTelemetry مجمّع نيابةً عنك. يعمل مع Claude Code وCursor وCodex وغير ذلك من الوكلاء البرمجية."
  outline={[
...
"شغّل ClickStack OpenTelemetry مجمّع محليًا في Docker، مع توجيهه إلى خدمتك.",
...
<Tabs>
    ...
    <Steps>
      ...
      <Step>
        ...
        <Accordion title="إنشاء بيانات قياس عن بُعد أكثر ثراءً (اختياري)">
          ...
          أرسل مقاييس من الأنواع الثلاثة الشائعة، بحيث تتضمن المخططات Counter وGauge وdistribution:
  ...
  <Tab title="لديّ مُجمِّع">
    <Steps>
      <Step>
        ## اجمع بيانات الاعتماد الخاصة بك

        <GatherCredentials />
      </Step>

      <Step>
        ## أنشئ مستخدمًا للإدخال

        <CreateIngestionUser />
      </Step>

      <Step>
        ## عدّل إعدادات collector لديك
      ...
      <Step>
        ...
        <Info>
          **ما المتوقع**
          ...
          يستغرق كل تشغيل لـ `tg` نحو 20 ثانية (كما يحدده `--duration 20s`) ويُخرج سجلات مفصلة طوال هذه المدة، وهذا أمر متوقع؛ كما ينتهي كل تشغيل تلقائيًا بمجرد انقضاء 20 ثانية. تكفي السجلات أدناه لتأكيد خط الأنابيب؛ أما المجموعة الاختيارية الأكثر شمولًا فتضيف عدة عمليات تشغيل أخرى وتستغرق بضع دقائق.
        </Info>
        ...
        أرسل السجلات بمزيج واقعي من مستويات الخطورة عبر الخدمات، بحيث تكون معظمها معلوماتية مع تحذير وخطأ، بدلاً من تدفق خطأ موحّد:
        ...
        <Accordion title="أنشئ بيانات قياس عن بُعد أكثر تفصيلاً (اختياري)">
