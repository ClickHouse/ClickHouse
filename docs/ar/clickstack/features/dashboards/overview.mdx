...
يقبل `color` أي token من القوائم الفئوية والدلالية أعلاه (مثل `chart-blue` أو `chart-success`). يمكن استخدام `backgroundChart.color` اختياريًا لتجاوز لون المخطط المصغّر باستخدام token enum نفسه. راجع [مرجع واجهة برمجة تطبيقات ClickStack](/ar/clickstack/api-reference) للتعرّف على المصادقة وعناوين URL الأساسية.
