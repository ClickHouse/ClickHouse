...
يقبل `color` أي token من القوائم الفئوية والدلالية أعلاه (مثل `chart-blue` أو `chart-success`). يمكن استخدام `backgroundChart.color` اختياريًا لتجاوز لون المخطط المصغّر باستخدام token enum نفسه. راجع [مرجع واجهة برمجة تطبيقات ClickStack](/ar/use-cases/observability/clickstack/api-reference) للتعرّف على المصادقة وعناوين URL الأساسية.
...
<Steps>
  ...
  <Step title="أضف فلترًا جديدًا" id="add-new-filter">
  ...
  <Step title="استخدم الفلتر" id="use-filter">
  ...
  <Step title="(اختياري) احفظ قيم الفلتر كإعداد افتراضي" id="save-default-filters">
