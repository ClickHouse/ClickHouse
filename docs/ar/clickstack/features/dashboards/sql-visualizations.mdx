...
يمكن أيضًا تضمين قيم المتغير مباشرةً بصيغة `$name` أو `${name}`، مع تنسيق اختياري: `${name:sqlstring}` (الافتراضي)، أو `${name:csv}`، أو `${name:regex}`. في التنسيق الافتراضي، يُنتج المرجع المباشر القيمة `NULL` قبل تحديد أي قيمة، لذا يُفضَّل استخدام وحدات الماكرو المذكورة أعلاه كلما كان متوقعًا وجود شرط.
...
* تُقيَّم التنبيهات على بلاطة SQL الذي يشير إلى متغير في لوحة المعلومات بحيث تكون جميع المتغيرات في حالتها الفارغة، لا بالقيم المحددة في لوحة المعلومات.