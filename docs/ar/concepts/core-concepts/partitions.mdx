...
<Steps>
  ...
  <Step title="تقليم الحبيبة">
    تشير الأسطر من 19 إلى 24 من ناتج `EXPLAIN` أعلاه إلى أن ClickHouse يستخدم بعد ذلك [الفهرس الأساسي](/ar/guides/clickhouse/data-modelling/sparse-primary-indexes) (الذي أُنشئ على الحقل `town`) لجزء البيانات الذي جرى تحديده في خطوة تقليم الأقسام، لتقليل عدد الحبيبات أكثر (التي تحتوي على صفوف قد تطابق أيضًا عامل التصفية `town` الخاص بالاستعلام) من 11 إلى 1. وينعكس ذلك أيضًا في ناتج عميل ClickHouse الذي طبعناه أعلاه للاستعلام المُنفَّذ:
