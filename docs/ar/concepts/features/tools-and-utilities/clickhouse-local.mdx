...
* `--highlight`, `--hilite` `<bool>` — بدّل تمييز الصياغة في موجّه الأوامر والاستعلامات المطبوعة عبر echo. يكون مفعّلًا افتراضيًا. لا يُطبَّق التمييز إلا عند الكتابة إلى طرفية.
