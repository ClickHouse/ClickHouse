...
<Warning>
  أبقِ `readonly` خارج قائمة `changeable_in_readonly` في كل ملف تعريف، وتذكّر أن قيود ملف التعريف تُطبَّق على مستوى النظام بالكامل، لا على كل مستخدم على حدة.
</Warning>

لا تضع علامة `changeable_in_readonly` على `readonly` نفسه. فإن فعلت، فإن أي session تبدأ بـ `readonly = 1` يمكنها تنفيذ `SET readonly = 0` واستعادة القدرة على تنفيذ استعلامات الكتابة التي تسمح بها امتيازاتها الحالية أصلاً.

ولهذا أهمية في كل الحالات، وليس فقط بالنسبة لملف التعريف المُعيَّن للمستخدم:

* يمكن لأي session الانتقال إلى أي ملف تعريف عبر `SET profile`، لأن اختيار ملف التعريف لا يخضع لفحص الوصول (وإنما تحدّه فقط قيود الإعدادات النشطة مسبقاً).
