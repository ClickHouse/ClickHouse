...
| [`timeout_overflow_mode`](/ar/reference/settings/session-settings/timeout-overflow-mode#timeout_overflow_mode)                                 | يحدد ما يجب فعله إذا استمر تنفيذ الاستعلام مدة أطول من `max_execution_time` أو إذا كان زمن التنفيذ التقديري أطول من `max_estimated_execution_time`. |
| [`max_execution_time_leaf`](/ar/reference/settings/session-settings/max-execution#max_execution_time_leaf)                                     | مماثل دلاليًا لـ `max_execution_time`، لكنه يُطبَّق فقط على العقد الطرفية للاستعلامات الموزعة أو البعيدة.                                           |
...
| [`join_overflow_mode`](/ar/reference/settings/session-settings/join#join_overflow_mode)                                                        | يحدد الإجراء الذي ينفذه ClickHouse عند بلوغ أيٍّ من حدود join التالية.                                                                              |
...
| [`max_sessions_for_user`](/ar/reference/settings/session-settings/max#max_sessions_for_user)                                                   | الحد الأقصى لعدد الجلسات المتزامنة لكل مستخدم موثَّق على خادم ClickHouse.                                                                           |
