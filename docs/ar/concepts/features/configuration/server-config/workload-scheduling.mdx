...
يوضح المثال التالي كيفية تعريف التسلسلات الهرمية لجدولة IO كما هو موضح في الصورة أعلاه:
...
لكي تتمكن من الاستفادة من السعة الكاملة للمورد الأساسي، يجب عليك استخدام `inflight_limit`. لاحظ أن القيمة المنخفضة لـ `max_requests` أو `max_cost` قد تؤدي إلى عدم الاستفادة الكاملة من المورد، في حين أن القيم المرتفعة جدًا قد تؤدي إلى وجود طوابير فارغة داخل المجدول، ما سيؤدي بدوره إلى تجاهل السياسات (عدم الإنصاف أو تجاهل الأولويات) في الشجرة الفرعية. ومن ناحية أخرى، إذا كنت تريد حماية الموارد من الاستهلاك المرتفع جدًا، فيجب عليك استخدام `bandwidth_limit`. فهو يفرض تقييدًا عندما تتجاوز كمية المورد المستهلَكة خلال `duration` ثانية `max_burst + max_speed * duration` بايت. ويمكن استخدام عقدتَي `bandwidth_limit` على المورد نفسه لتقييد ذروة عرض النطاق خلال الفترات القصيرة، ومتوسط عرض النطاق خلال الفترات الأطول.

<div id="workload-classifiers">
  ### مُصنِّفات أعباء العمل المهملة
</div>

تُستخدم مُصنِّفات أعباء العمل لتحديد تعيين قيمة `workload` المحددة بواسطة استعلام إلى قوائم الانتظار النهائية التي ينبغي استخدامها لموارد معيّنة. في الوقت الحالي، يكون تصنيف أعباء العمل بسيطًا: لا يتوفر سوى التعيين الثابت.
