...
ويدعم عمليات `INSERT`.
نسخ الجداول متماثلًا غير مدعوم.
[عمليات التعديل](/ar/reference/statements/alter/index#mutations) وعمليات `ALTER`
مدعومة فقط إذا كان القرص مُهيّأً مع `enable_hard_links`، انظر أدناه.
...
للجدول. وقد يكون هذا مفيدًا، على سبيل المثال، لجداول النظام.
...
#### الروابط الصلبة (hard links) على أقراص plain rewritable {#plain-rewritable-hard-links}

تتشارك عمليات `ALTER` وعمليات التعديل في جدول `MergeTree` الملفات غير المتغيّرة
بين الـ جزء وخَلَفه الناتج عن عملية التعديل عبر hard links بدلًا من نسخها.
يتيح الإعداد `enable_hard_links` (قيمته الافتراضية `0`) للقرص الذي يستخدم بيانات وصفية من نوع
`plain_rewritable` إنشاء hard links حقيقية، وهو ما يفتح المجال أمام هذه العمليات:
...
يجعل الـ hard link البيانات الوصفية الخاصة بالدليل تسرد ملفاته صراحةً، إلى جانب مفاتيح الكائنات التي تخزّنها، بحيث يمكن لملف أن يشير إلى كائن مخزّن تحت prefix دليل آخر. ضع ما يلي في الاعتبار قبل تفعيله:

* الخوادم الأقدم من `26.9` لا تستطيع قراءة دليل بهذه الصيغة. والقرص الذي يحتوي على
  مثل هذه الأدلة يصبح غير قابل للقراءة بعد الرجوع إلى إصدار أقدم، ولا يمكن مشاركته مع
  خوادم أقدم. أما الأدلة التي لم تحتوِ قط على hard links فتحتفظ بالـ layout السابق،
  لذا يظل القرص قابلاً للقراءة من قبل الخوادم الأقدم حتى أول عملية
  تُنشئ hard link — أي عملية التعديل أو `ALTER` أو `FREEZE` أو نسخة احتياطية. وإعادة
  إيقاف الإعداد لا تُعيد الأدلة الموجودة إلى صيغتها السابقة.
* الكائن الذي يُزال آخر رابط له يُحذف على خطوتين: تُعاد كتابة البيانات الوصفية
  الخاصة بالدليل أولاً، ثم يُحذف الكائن بعد ذلك. وإذا أُنهي
  الخادم قسراً بين الخطوتين، يبقى الكائن في الـ حاوية دون أن
  يشير إليه أي شيء، ومن ثم يجب حذفه يدوياً.
