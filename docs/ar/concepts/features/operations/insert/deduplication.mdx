...
في هذه الحالة، يمكنك تحديد `insert_deduplication_token` خاص بك لكل دفعة من الصفوف لضمان ألّا تؤدي عمليات `INSERT` متعددة للدفعة نفسها إلى إعادة إدراج الصفوف نفسها. يُرجى مراجعة [وثائق `insert_deduplication_token`](/ar/reference/settings/session-settings#insert_deduplication_token) لمزيد من التفاصيل حول كيفية استخدام هذا الإعداد.