...
* **(3)** إذا كان الجدول الأيمن يتّسع في الذاكرة، فاستخدم hash JOIN (`hash` و`parallel_hash` اسمان بديلان؛ واختيار أحدهما لا يتحكّم في التوازي). وعادةً ما يكون هذا الخيار الأسرع داخل الذاكرة، بحسب حجم البيانات وأنواع البيانات وتوزيع قيم أعمدة مفتاح الـ JOIN.
