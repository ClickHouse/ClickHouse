...
## كشف مقاييس خادم ClickHouse {#expose}
...
## واجهة Prometheus HTTP واجهة برمجة تطبيقات وPromQL {#prometheus-http-api-and-promql}
...
### تهيئة واجهة برمجة تطبيقات Prometheus {#configure-prometheus-api}
...
يُعاد التعبير مُسلسلاً من الاستعلام المُحلَّل، مع توحيد المسافات البيضاء، وإزالة التعليقات، وحذف الأقواس الزائدة، وتحويل المدد إلى أعداد بالثواني: `sum by (job) (http_requests_total{code="200"}) / 2`. لا تقوم نقطة النهاية هذه بتقييم التعبير، لذا فهي لا تحتاج إلى المعاملين `database` و`table`.
