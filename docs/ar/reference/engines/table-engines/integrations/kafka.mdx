...
### مصادقة AWS MSK IAM {#kafka-aws-msk-iam}
...
### دعم Kerberos {#kafka-kerberos-support}
...
## ربط الأعمدة بالبيانات الوصفية لرسائل Kafka {#mapping-columns-to-kafka-message-metadata}
...
## دعم تنسيقات البيانات {#data-formats-support}
...
## محرك لتخزين الإزاحات المعتمدة في ClickHouse Keeper {#engine-to-store-committed-offsets-in-clickhouse-keeper}
...
### التخصيص الثابت للأقسام إلى الشظايا {#static-partition-to-shard-affinity}

عند استخدام `StorageKafka2`، يمكنك اختياريًا تفعيل التخصيص الثابت للأقسام إلى الشظايا عبر تحديد `kafka_partition_shard_num` و`kafka_shard_count`. يتيح ذلك لعدة مثيلات من ClickHouse (شظايا) الاستهلاك من موضوع Kafka نفسه، بحيث تعالج كل شظية مجموعة فرعية حتمية فقط من الأقسام استنادًا إلى الصيغة:
...
عند استخدامه مع النسخ المتماثلة (قيم متعددة لـ `kafka_replica_name` تشترك في `kafka_keeper_path` نفسه)، يُطبَّق مرشح الألفة أولًا لتحديد الأقسام المؤهلة، ثم توزّع أقفال ZooKeeper هذه الأقسام بين النسخ المتماثلة.
