...
* `array` لا يُجري أي تقسيم إلى رموز، أي إن كل قيمة في row هي رمز (راجع الدالة [array](/ar/reference/functions/regular-functions/array-functions#array)).
...
3. **التجذير**. يؤدي ربط كل رمز بجذره إلى تحسين شمولية البحث عبر مطابقة المتغيرات الصرفية التي تشترك في الجذر نفسه.
   ...
   مثال: `stem(str, 'en')`
4. **تطبيع حالة الأحرف**. تحويل الـرموز إلى أحرف صغيرة أو كبيرة لتمكين case-insensitive matching، مثل [lower](/ar/reference/functions/regular-functions/string-functions#lower) و[lowerUTF8](/ar/reference/functions/regular-functions/string-functions#lowerUTF8).
...
يُطبَّق المعالج اللاحق على كل رمز مُولَّد أثناء إنشاء الفهرس (بالنسبة إلى مُجزِّئ `array`، يكون كل عنصر في المصفوفة رمزًا). وعند تنفيذ الاستعلام، يعتمد السلوك على الدالة:

* بالنسبة إلى `hasToken` و`hasAllTokens` و`hasAnyTokens` و`hasPhrase` (مع أي مُجزِّئ مدعوم): يُطبَّق المعالج اللاحق على كلٍّ من رموز النص المُبحَث فيه وإبرة البحث، مما يتيح مطابقة مطبَّعة بالكامل (مثل البحث غير الحساس لحالة الأحرف). بالنسبة إلى `hasPhrase`، تُرتَّب الرموز بعد المعالجة اللاحقة بشكل متتالٍ، لذلك لا يترك الرمز الذي يستبعده المعالج اللاحق فجوة موضعية، وتظل العبارة متطابقة عبره — على سبيل المثال، مع معالج لاحق لكلمات التوقف يستبعد `the`، فإن `hasPhrase(col, 'see cat')` يطابق مستندًا يحتوي على `see the cat`.
...
| [hasPhrase](/ar/reference/functions/regular-functions/string-search-functions#hasPhrase)                   | نعم                                      | `splitByNonAlpha`, `splitByString`, `ngrams`, `asciiCJK`, `icu` | نعم                 |
...
لاحظ أن الدالة تتطلب أحد مجزئات `splitByNonAlpha` أو `splitByString` أو `ngrams` أو `asciiCJK` أو `icu`.
...
يمكن استخدام `hasPhrase` مع مُقسِّمات الرموز `splitByNonAlpha` و`splitByString` و`ngrams` و`asciiCJK` و`icu`.
