...
⁵ يتجاهل عمود *مُقسِّمات الرموز المتوافقة* مُقسِّم الرموز [`keyValuePairs`](#tokenizer-key-value-pairs)، وهو مُقسِّم الرموز متخصص لأعمدة `Map`.
...
#### عمليات lookup المدمجة للأزواج key-value باستخدام مُقسِّم الرموز `keyValuePairs` {#tokenizer-key-value-pairs}

استخدم مُقسِّم الرموز `keyValuePairs` على column من النوع `Map` نفسه عند البحث عن key محدد يحمل قيمة محددة:
...
يدمج الـ مُقسِّم الرموز كل زوج `(key, value)` في الصف في token واحد، بحيث يعرف الـ index أي قيمة تنتمي إلى أي مفتاح.
أما indexes الخاصة بـ `mapKeys` و`mapValues` الموضّحة أدناه فلا تستطيع الإجابة عن مثل هذا الاستعلام، لأنها تُفهرس المفاتيح والقيم بشكل مستقل: فهي تستطيع أن تخبرك بأن أحد attributes الصف يحمل القيمة `error`، لكن لا تستطيع تحديد أنه كان attribute المسمّى `level`.

يُخزَّن الزوج على هيئة الـ token التالي: `key ‖ value ‖ length(key)`.
...
* يجب إنشاء الـ index على `Map` تكون مفاتيحه وقيمه من النوع `String` أو `LowCardinality(String)`.
...
* تُرفض الوسيطات `preprocessor` و`postprocessor` و`support_phrase_search` عند إنشاء الـ table.
* حتى الآن، لا يُجاب من الـ index إلا عن `=` على عنصر من الـ map.
...
* يُعيد `map['key']` القيمة الافتراضية لنوع القيمة عندما يكون المفتاح غير موجود، لذا فإن `map['key'] = ''` تكون true أيضًا للصفوف التي لا تحتوي على المفتاح وبالتالي ليس لها token.
  ويعود مثل هذا الـ مسند إلى الفحص الشامل (brute-force scan).
* إذا احتوى الصف على المفتاح نفسه أكثر من مرة، فإن `map['key']` هي قيمة أول ظهور له، ويطابق الـ index ذلك الظهور.

#### البحث في المفاتيح والقيم باستخدام `mapKeys` و`mapValues` {#text-index-map-keys-and-values}
...
إذا كان الفهرس النصي معرّفًا باستخدام مُقسِّم الرموز `array`، فإن `direct read` تدعم أيضًا الدوال `equals` و`has` و`hasAny` و`hasAll` و`mapContainsKey` و`mapContainsValue`.
إذا كان الفهرس النصي معرّفًا باستخدام مُقسِّم الرموز `keyValuePairs`، فإن `direct read` مدعومة للدالة `equals` على عنصر في map (‏`map['key'] = 'value'`).
