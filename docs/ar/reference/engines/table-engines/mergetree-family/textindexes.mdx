...
بالنسبة إلى النصوص غير ASCII، يُوصى باستخدام محلل الرموز `asciiCJK` لأنه يتعامل بصورة صحيحة مع حدود الكلمات في Unicode، بما في ذلك محارف CJK.
بالنسبة إلى اللغات التي لا تفصل الكلمات بمسافات بيضاء (مثل الصينية أو اليابانية أو التايلاندية)، ينتج محلل الرموز `icu(locale)` رموزًا لكلمات متعددة المحارف ذات معنى عبر تجزئة الكلمات المعتمدة على قاموس ICU.
وبالنسبة إلى اليابانية تحديدًا، يقسم محلل الرموز `japanese` ‏(MeCab) النص إلى كلمات بدلًا من محارف مفردة، ويعطي عمومًا نتائج بحث أفضل.
