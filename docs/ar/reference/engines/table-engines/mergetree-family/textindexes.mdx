...
إذا أضفت فهرسًا إلى جدول موجود، فنوصي بإنشاء الفهرس فعليًا لأجزاء الجدول الحالية (وإلا فسيعتمد البحث في الأجزاء التي لا تحتوي على فهرس على عمليات مسح بالقوة الغاشمة بطيئة).
...
* `splitByRegexp(re)` يقسم السلاسل النصية باستخدام فاصل تعبير نمطي `re` يحدده المستخدم (راجع الدالة [splitByRegexp](/ar/reference/functions/regular-functions/splitting-merging-functions#splitByRegexp)).
  الوسيطة `re` إلزامية، على سبيل المثال: `tokenizer = splitByRegexp('[^\p{L}\p{N}#+]+')`.
  بخلاف سلاسل الفواصل الثابتة في `splitByString`، يمكن لفاصل التعبير النمطي الاحتفاظ بالرموز التي تحتوي على محارف خاصة (مثل `C++` أو `C#`) والتي ستقسمها مُقسِّمات الرموز الأخرى.
...
جميع مُقسِّمات الرموز المتاحة مُدرجة في [system.tokenizers](/ar/reference/system-tables/tokenizers).

### قاموس مُقسِّم الرموز الياباني {#japanese-tokenizer-dictionary}

<CloudNotSupportedBadge />

يحتاج مُقسِّم الرموز `japanese` إلى قاموس MeCab، وهو غير مُضمَّن مع ClickHouse. وفِّر قاموسًا في إعدادات الخادم:
...
* `dictionary_sha` هو قيمة SHA-256 لذلك الأرشيف. يتم التحقق منها قبل تحميل القاموس؛ وعند عدم التطابق، لا يُحمَّل القاموس ويُثار خطأ.

يُنزَّل الأرشيف ويُستخرج مرة واحدة، ثم يُخزَّن مؤقتًا محليًا. ولأن القاموس يحدد كيفية تقسيم المستندات إلى رموز، يجب تهيئة `dictionary_sha` نفسه على جميع النسخ المتماثلة.
...
بعد تهيئة Dictionary، يمكن استخدام مُقسِّم الرموز `japanese` على النحو التالي:
...
لفهم كيفية قيام مُقسِّم الرموز بتقسيم سلسلة الإدخال، يمكنك استخدام الدالتين [tokens](/ar/reference/functions/regular-functions/splitting-merging-functions#tokens) و[tokensForLikePattern](/ar/reference/functions/regular-functions/splitting-merging-functions#tokensForLikePattern):
...
بالنسبة إلى النصوص غير ASCII، يُوصى باستخدام مُقسِّم الرموز `asciiCJK` لأنه يتعامل بصورة صحيحة مع حدود الكلمات في Unicode، بما في ذلك محارف CJK.
بالنسبة إلى اللغات التي لا تفصل الكلمات بمسافات بيضاء (مثل الصينية أو اليابانية أو التايلاندية)، ينتج مُقسِّم الرموز `icu(locale)` رموزًا لكلمات متعددة المحارف ذات معنى عبر تجزئة الكلمات المعتمدة على قاموس ICU.
وبالنسبة إلى اليابانية تحديدًا، يقسم مُقسِّم الرموز `japanese` ‏(MeCab) النص إلى كلمات بدلًا من محارف مفردة، ويعطي عمومًا نتائج بحث أفضل.
وبالنسبة إلى الصينية تحديدًا، يقسم مُقسِّم الرموز `chinese` ‏(jieba) النص إلى كلمات بدلًا من محارف مفردة، ويعطي عمومًا نتائج بحث أفضل.
...
لاحظ أنه نظرًا إلى أن المعالج المسبق يُطبَّق فقط على مسار الفهرس النصي، فقد تختلف نتائج هذه الدوال بين الاستعلامات التي تستخدم الفهرس النصي وتلك التي لا تستخدمه (مثلًا `SETTINGS use_skip_indexes = 0`).
...
2. **إزالة الطابع الزمني**. كثيرًا ما تبدأ أسطر السجل بطابع زمني منظَّم مثل `2024-01-15T10:23:45` أو تتضمنه.
   ...
   * **نهج المعالج اللاحق**: استخدم مُقسِّم الرموز ‏`splitByString` (التقسيم حسب المسافات البيضاء) بحيث يصبح الطابع الزمني بالكامل رمزًا واحدًا، ثم استخدم `parseDateTimeOrNull` لاكتشافه واستبعاده.
   ...
   * **نهج المعالج المسبق**: أزل الطابع الزمني من سطر السجل الكامل *قبل* التقسيم إلى رموز باستخدام regular expression.
     ...
     ينجح هذا مع أي مُقسِّم للرموز ويكون أكثر كفاءة لأن أحرف الطابع الزمني لا تخضع لعملية التقسيم إلى رموز أصلًا.
...
يحوِّل تعبير المعالج اللاحق الرموز من النوع [String](/ar/reference/data-types/string) إلى رموز من النوع نفسه.
كذلك، يجب ألا يشير تعبير المعالج اللاحق إلا إلى العمود أو التعبير الذي يُعرَّف عليه الفهرس النصي.
...
يُطبَّق المعالج اللاحق على كل رمز مُولَّد أثناء إنشاء الفهرس (بالنسبة إلى مُقسِّم الرموز `array`، يكون كل عنصر في المصفوفة رمزًا). وعند تنفيذ الاستعلام، يعتمد السلوك على الدالة:

* بالنسبة إلى `hasToken` و`hasAllTokens` و`hasAnyTokens` و`hasPhrase` (مع أي مُقسِّم للرموز مدعوم): يُطبَّق المعالج اللاحق على كلٍّ من رموز النص المُبحَث فيه وإبرة البحث، مما يتيح مطابقة مطبَّعة بالكامل (مثل البحث غير الحساس لحالة الأحرف). بالنسبة إلى `hasPhrase`، تُرتَّب الرموز بعد المعالجة اللاحقة بشكل متتالٍ، لذلك لا يترك الرمز الذي يستبعده المعالج اللاحق فجوة موضعية، وتظل العبارة متطابقة عبره — على سبيل المثال، مع معالج لاحق لكلمات التوقف يستبعد `the`، فإن `hasPhrase(col, 'see cat')` يطابق مستندًا يحتوي على `see the cat`. الاستثناء الوحيد هو `hasPhrase` على فهرس `splitByRegexp`، إذ لا يدعم معالجًا لاحقًا (ويُرفض الجمع بينهما باستثناء).
...
من دون `support_phrase_search`، تستخدم `hasPhrase` الفهرس على سبيل الاستدلال فقط، وتتحقق من كل صف متبقٍّ باستخدام الشرط الأصلي؛ وتقوم المعالجة اللاحقة أيضًا بتطبيع كلٍّ من العبارة ورموز الـ haystack بالطريقة نفسها، بحيث تكون النتيجة مستقلة عن مسار القراءة، ولا تؤدي الرموز التي تُسقطها المعالجة اللاحقة إلى كسر تجاور العبارة. ومع `support_phrase_search = 1`، تستخدم `hasPhrase` قراءات مباشرة دقيقة (مع الاستمرار في تطبيق المعالجة اللاحقة، إن وُجدت). لا يمتد دعم المعالجة اللاحقة هذا إلى مُقسِّم الرموز `splitByRegexp`: يُرفض استخدام `hasPhrase` على فهرس `splitByRegexp` مدمج مع معالجة لاحقة (انظر الحاشية السفلية ³ أدناه).
...
¹ يستخدم `LIKE` و`match` القراءة المباشرة كتلميح مع مُقسِّمات الرموز المذكورة، وإلا فيلجآن إلى مسح بالقوة الغاشمة.
ويدعم `LIKE` أيضًا التقييم عبر فحص القاموس (مفعّل عبر `use_text_index_like_evaluation_by_dictionary_scan`) مع مُقسِّمَي الرموز `splitByNonAlpha` و`array` من دون معالجة مسبقة أو لاحقة.
عندئذٍ يكون النمط `%value%` *قراءة مباشرة (من دون تلميح)*، بينما يظل النمطان `value%` و`%value` تلميحًا، انظر [استعلامات LIKE/ILIKE](#like-ilike-queries-perf).
...
ولا يوجد مسار بديل لاستخدام الفهرس كتلميح للأنماط التي لا يدعمها فحص القاموس: إذا كان هذا الإعداد معطّلًا أو لم يكن مُقسِّم الرموز ضمن المجموعة المدعومة، فلن يُستخدَم الفهرس مع `ILIKE`.
...
## استخدام فهرس نصي {#using-a-text-index}
...
### الدوال المدعومة {#functions-support}
...
#### `multiSearchAny` و `multiMatchAny` {#functions-example-multisearchany-multimatchany}
...
وعلى عكس `hasAnyTokens` و `hasAllTokens`، فإنها لا تُجزِّئ مصطلح البحث إلى رموز (إذ تفترض أن المُدخل عبارة عن رمز واحد).
...
تُجزَّأ عبارة البحث إلى رموز باستخدام مُقسِّم الرموز نفسه المُعدّ لعمود الفهرس.
وعندما يستخدم فهرس النص معالجًا لاحقًا، تُطبَّع عبارة البحث أيضًا قبل البحث في الفهرس.
لاحظ أن الدالة تتطلب أحد مُقسِّمات الرموز `splitByNonAlpha` أو `splitByString` أو `splitByRegexp` أو `ngrams` أو `asciiCJK` أو `icu`.
...
#### `hasAny` و `hasAll` {#functions-example-hasany-hasall}
...
#### `mapContainsKeyLike` و `mapContainsValueLike` {#functions-example-mapcontainslike}
...
### فهرسة أعمدة Array(String) {#text-index-example-array}
...
### فهرسة أعمدة Map {#text-index-example-map}
...
### فهرسة أعمدة JSON {#text-index-example-json}
...
#### فهارس على أعمدة فرعية محددة {#json-indexes-on-subcolumns}
...
#### الفهارس المستندة إلى المسار باستخدام JSONAllPaths {#json-indexes-jsonallpaths}
...
#### الفهارس القائمة على القيم باستخدام JSONAllValues {#json-indexes-jsonallvalues}
...
##### إنشاء الفهرس {#json-all-values-creating-the-index}
...
##### أنماط الاستعلام المدعومة {#json-all-values-supported-query-patterns}
...
### البحث بالعبارات {#text-index-phrase-search}
...
يمكن استخدام `hasPhrase` مع مُقسِّمات الرموز `splitByNonAlpha` و`splitByString` و`splitByRegexp` و`ngrams` و`asciiCJK` و`icu`.
...
#### مثال {#text-index-phrase-search-example}
...
تُجيب آلية تحسين `direct read` عن الاستعلام بالاعتماد حصريًا على الفهرس النصي (أي من خلال عمليات lookup في الفهرس النصي) من دون الوصول إلى عمود النص الأساسي.
...
إذا كان الفهرس النصي معرّفًا باستخدام المُجزِّئ `array`، فإن `direct read` تدعم أيضًا الدوال `equals` و`has` و`hasAny` و`hasAll` و`mapContainsKey` و`mapContainsValue`.
...
تعتمد القراءة المباشرة كتلميح على المبادئ نفسها التي تعتمد عليها القراءة المباشرة العادية، لكنها تضيف بدلًا من ذلك عامل تصفية إضافيًا مُنشأً من بيانات الفهرس النصي، من دون الاستغناء عن العمود النصي الأساسي.
وتُستخدم مع الدوال التي قد تؤدي فيها القراءة من الفهرس النصي فقط إلى مطابقات إيجابية كاذبة.
...
### استعلامات LIKE/ILIKE {#like-ilike-queries-perf}

عندما يكون نمط استعلام LIKE/ILIKE بالشكل `%<alpha-numeric-characters-without-spaces>%` أو `<alpha-numeric-characters-without-spaces>%` أو `%<alpha-numeric-characters-without-spaces>`، ويكون الـ مُقسِّم الرموز الخاص بفهرس النص هو `splitByNonAlpha` أو `array`، فإن ClickHouse يستفيد من الفهرس المعكوس لتسريع استعلامات LIKE/ILIKE بدرجة كبيرة. ولتحقيق ذلك، يفحص ClickHouse قاموس الفهرس المعكوس بدلاً من إجراء مسح كامل للجدول للعثور على النمط المطابق.

تعتمد طريقة استخدام نتيجة فحص القاموس على موضع تثبيت الـ needle:

* يطابق `%value%` صفاً إذا وفقط إذا طابق أحد tokens ذلك الصف، وبالتالي يحسم الفهرس الاستعلام بمفرده: [قراءة مباشرة (بدون تلميح)](#direct-read) تزيل الـ مسند الأصلي.
* يثبّت كل من `value%` و`%value` الـ needle عند القيمة بأكملها، بينما لا يستطيع فحص القاموس تثبيته إلا عند token، لذا يُرجع الفحص مجموعة فائقة من الصفوف المطابقة ويُستخدم بوصفه [قراءة مباشرة كـ تلميح](#direct-read).

ويخدم فحص القاموس نفسه كلاً من `startsWith(col, 'value')` و`endsWith(col, 'value')` عندما لا يحتوي الـ needle على token كامل للبحث عنه.
...
أما الـ needles التي تمتد عبر عدة tokens فتظل تستخدم الـ tokens الكاملة الخاصة بها ولا تحتاج إلى فحص القاموس.

عند تفعيل هذا التحسين، يُفترض أن تكون استعلامات LIKE/ILIKE أسرع بكثير من المسح الكامل للجدول. غير أنه إذا طابق النمط معظم tokens القاموس، فقد يصبح الأداء أسوأ مقارنةً بالمسح الكامل للجدول. ولحسن الحظ، توجد آلية fallback تحول دون ذلك.
...
### استعلامات العد البسيطة {#count-queries-perf}
...
#### إعدادات ذاكرة التخزين المؤقت لرموز الفهرس النصي {#caching-tokens}
...
#### إعدادات ذاكرة التخزين المؤقت للترويسات {#caching-header}
...
#### إعدادات ذاكرة التخزين المؤقت لقوائم الترحيل {#caching-posting-lists}
...
## فهارس النص مقابل الفهارس المعتمدة على Bloom filter {#text-index-vs-bloom-filter-indexes}
...
## مثال: مجموعة بيانات Hacker News {#hacker-news-dataset}
...
### 1. استخدام `hasToken` {#using-hasToken}
...
### 2. استخدام `hasAnyTokens` {#using-hasAnyTokens}
...
### 3. استخدام `hasAllTokens` {#using-hasAllTokens}
...
### 4. البحث المركب: OR, AND, NOT, ... {#compound-search}
