...
¹ يستخدم `LIKE` و`match` القراءة المباشرة كتلميح مع مُقسِّمات الرموز المذكورة، وإلا فيلجآن إلى مسح بالقوة الغاشمة.
...
³ لا تدعم `hasPhrase` على فهرس نصي من نوع `splitByRegexp` معالجًا لاحقًا: يُرفَض هذا الجمع باستثناء، لأن إعادة الكتابة على مستوى الصف للمعالج اللاحق تفترض رموزًا بأسلوب `splitByNonAlpha` مقسَّمة وفق المسافات البيضاء. وبدون معالج لاحق، يكون `splitByRegexp` مدعومًا بالكامل بواسطة `hasPhrase`.
...
عندما يكون نمط استعلام LIKE/ILIKE هو `%<alpha-numeric-characters-without-spaces>%` ويكون مُقسِّم الرموز للفهرس النصي `text index` هو `splitByNonAlpha` أو `array`، يستفيد ClickHouse من الفهرس المعكوس `inverted index` لتسريع استعلامات LIKE/ILIKE بشكل كبير. ولتحقيق ذلك، يفحص ClickHouse القاموس `Dictionary` الخاص بالفهرس المعكوس بدلًا من إجراء فحص كامل للجدول للعثور على النمط المطابق.
