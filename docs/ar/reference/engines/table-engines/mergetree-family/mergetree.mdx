...
## تخزين البيانات {#mergetree-data-storage}
...
## المفاتيح الأساسية والفهارس في الاستعلامات {#primary-keys-and-indexes-in-queries}
...
### اختيار مفتاح أساسي {#selecting-a-primary-key}
...
### اختيار مفتاح أساسي يختلف عن مفتاح الفرز {#choosing-a-primary-key-that-differs-from-the-sorting-key}
...
### استخدام الفهارس والتقسيمات في الاستعلامات {#use-of-indexes-and-partitions-in-queries}
...
تُظهر الاستعلامات أعلاه أن الفهرس يُستخدم حتى مع التعبيرات المعقدة. وتُنظَّم القراءة من الجدول بحيث لا يمكن أن يكون استخدام الفهرس أبطأ من مسح كامل.
...
### استخدام الفهرس مع التعبيرات الحتمية في المفاتيح الأساسية {#use-of-index-for-deterministic-expressions-in-primary-keys}
...
### استخدام الفهرس مع المفاتيح الأساسية الرتيبة جزئيًا {#use-of-index-for-partially-monotonic-primary-keys}
...
### فهارس تخطي البيانات {#table_engine-mergetree-data_skipping-indexes}
...
### أنواع فهارس التخطي {#skip-index-types}
...
#### مرشح Bloom لـ N-gram *(مهمل)* {#n-gram-bloom-filter}
...
#### مرشح bloom للرموز {#token-bloom-filter}
...
#### مرشح Bloom لـ sparse grams {#sparse-grams-bloom-filter}
...
#### تشابه المتجهات {#vector-similarity}
...
### استعلام الإسقاط {#projection-query}
...
### فهارس الإسقاط {#projection-index}
...
#### الصياغة {#projection-index-syntax}
...
#### أنواع الفهارس {#projection-index-types}
...
### تخزين الإسقاطات {#projection-storage}
...
### تحليل الاستعلام {#projection-query-analysis}
...
## الوصول المتزامن إلى البيانات {#concurrent-data-access}
...
## TTL للأعمدة والجداول {#table_engine-mergetree-ttl}
...
### TTL للعمود {#mergetree-column-ttl}
...
#### إنشاء جدول باستخدام `TTL`: {#creating-a-table-with-ttl}
...
#### إضافة TTL إلى عمود في جدول موجود {#adding-ttl-to-a-column-of-an-existing-table}
...
#### تعديل TTL للعمود {#altering-ttl-of-the-column}
...
### TTL للجدول {#mergetree-table-ttl}
...
#### إنشاء جدول مع `TTL`: {#creating-a-table-with-ttl-1}
...
#### تعديل `TTL` للجدول: {#altering-ttl-of-the-table}
...
#### إنشاء جدول يُعاد فيه ضغط الصفوف منتهية الصلاحية: {#creating-a-table-where-expired-rows-are-recompressed}
...
### إزالة البيانات منتهية الصلاحية {#mergetree-removing-expired-data}
...
## استخدام عدة أجهزة تخزين كتلية لتخزين البيانات {#table_engine-mergetree-multiple-volumes}
...
### التكوين {#table_engine-mergetree-multiple-volumes_configure}
...
## استخدام التخزين الخارجي لتخزين البيانات {#table_engine-mergetree-s3}
...
### استخدام أقراص S3 مع وحدات تخزين متعددة {#s3-multiple-volumes}
...
## إحصاءات الأعمدة {#column-statistics}
...
#### تقليم الأجزاء باستخدام الإحصاءات {#part-pruning-with-statistics}

عند تمكين `use_statistics_for_part_pruning`، يمكن استخدام الإحصاءات في تقليم الأجزاء.
حاليًا، لا يدعم تقليم الأجزاء إلا إحصاءات `basic` (وإحصاءات `minmax` المهملة). وعند تعريف مثل هذه الإحصاءات لعمود، يتتبّع ClickHouse القيمتَين الصغرى والكبرى لذلك العمود في كل جزء.
...
### الأنواع المتاحة من إحصاءات الأعمدة {#available-types-of-column-statistics}

* `basic`

  حزمة مضغوطة من الملخصات ذات القيمة الواحدة المستمدة من عمود. وبحسب نوع العمود، تُعبَّأ العناصر التالية:

  * لأي عمود: عدد الصفوف التي تساوي القيمة الافتراضية للنوع (`0` للأعداد الصحيحة والأعداد فاصلة عائمة، و`''` لـ `String`، و`[]` لـ `Array`، و`NULL` لـ `Nullable`، إلخ)، مما يتيح للمُحسِّن تقدير مسندات `col = <default>` وعوامل التصفية `IS NULL`؛
  * لأي عمود تُمثَّل قيمه كأعداد (أعداد صحيحة، أعداد فاصلة عائمة، `Decimal*`، `Date*`، `DateTime*`، `Enum*`، `IPv4`، ...): القيمة الدنيا والقيمة القصوى، مما يتيح تقدير انتقائية عوامل تصفية النطاق وتمكين تقليم الأجزاء؛
  * لأعمدة `String` و`FixedString`: إجمالي طول القيم غير `NULL` بالبايت (الذي يمكن اشتقاق متوسط طول السلسلة منه).

    يمكن لإحصائية `basic` واحدة أن تعبّئ عدة عناصر من هذه في الوقت نفسه — فعلى سبيل المثال، في عمود `Nullable(UInt32)`، تتتبّع كلاً من الحد الأدنى/الأقصى الرقمي وعدد قيم `NULL`. ولأن عدد القيم الافتراضية مُعرَّف لكل نوع من أنواع الأعمدة، يمكن تعريف `basic` على أي عمود، بما في ذلك الأنواع المركبة مثل `Array` و`Tuple` و`Map`.
...
إلا عندما تُطابق القيمة الحرفية للمُعدِّد العدد الصحيح الخام `0`؛ وإذا لم تكن لأي مُعدِّد قيمة خام قدرها `0`،
فلن تُسهم الإحصائية في تقدير المساواة. أما بالنسبة إلى القيم الأخرى، فيلجأ المُحسِّن
إلى إحصاءات أخرى.
...
## إعدادات على مستوى العمود {#column-level-settings}
