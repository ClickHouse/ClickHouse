...
يستخدم المحرك `ReplicatedMergeTree` thread pool منفصلًا لعمليات fetches المكررة. ويُحدَّد حجم هذا pool بواسطة الإعداد [background&#95;fetches&#95;pool&#95;size](/ar/reference/settings/server-settings/settings/background#background_fetches_pool_size)، والذي يمكن ضبطه مع إعادة تشغيل الخادم.
...
سيُوسَّع `{shard}` إلى معرّف الـ شارد.
