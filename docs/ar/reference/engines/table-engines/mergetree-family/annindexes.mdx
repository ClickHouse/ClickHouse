...
تُخزَّن النقاط في فضاء المتجهات في عمود `vectors` من النوع [Array(Float64)](/ar/reference/data-types/array) أو [Array(Float32)](/ar/reference/data-types/array) أو [Array(BFloat16)](/ar/reference/data-types/array).
...
لتمكين مرمّز الضغط `Quantized`، فعِّل أولاً `SET allow_experimental_codecs = 1`.
إذا واجهت أي مشكلات، يُرجى فتح بلاغ في [مستودع ClickHouse](https://github.com/clickhouse/clickhouse/issues).
...
لا يعاني الفحص الشامل من أيٍّ من هذين القيدين: فهو لا يتطلب بنية بيانات إضافية، وتندمج الأجزاء طبيعيًا بضمّها معًا، كما أن التصفية تقلل ببساطة عدد الصفوف المطلوب فحصها.
...
حدِّد المرمّز `Quantized(...)` لعمود من النوع `Array(Float32)` (أو `Array(Float64)` / `Array(BFloat16)`).
...
بدونه، يُنفَّذ الاستعلام نفسه كفحص دقيق اعتيادي للمتجهات الأصلية.
