...
مثل جميع [فهارس التخطي](/ar/concepts/features/performance/skip-indexes/skipping-indexes) العادية، تُنشأ فهارس تشابه المتجهات على مستوى الحبيبات، وتتألف كل كتلة مفهرسة من `GRANULARITY = [N]` حبيبات (`[N]` = 1 افتراضيًا في فهارس التخطي العادية).
...
* `element_type` – نوع كل عنصر في المتجه. والأنواع المدعومة هي `BFloat16` و`Float32` و`Float64`
