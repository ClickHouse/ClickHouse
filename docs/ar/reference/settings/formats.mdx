...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "1"},{"label": "إعداد جديد يتحكم في ما إذا كانت صيغة GeoJSON تفرض التحقق من صلاحية الأشكال الهندسية وفق RFC 7946 (الحد الأدنى لعدد النقاط لكل خط وحلقة، وإغلاق الحلقة، وعدم فراغ الأشكال الهندسية المتعددة) عند القراءة والكتابة"}]}]} />

يتحكم هذا الإعداد في ما إذا كانت صيغة `GeoJSON` تفرض التحقق من صلاحية الأشكال الهندسية وفق RFC 7946، في الاتجاهين.

عند التمكين (افتراضيًا)، يُرفَض أي شكل هندسي يخالف قواعد أشكال GeoJSON: `LineString` (أو خط داخل `MultiLineString`) يحتوي على أقل من نقطتين؛ أو حلقة في `Polygon` أو `MultiPolygon` تحتوي على أقل من أربع نقاط أو تختلف فيها النقطة الأولى عن الأخيرة (أي حلقة غير مغلقة)؛ أو `MultiLineString` أو `Polygon` أو `MultiPolygon` فارغ. وينطبق ذلك عند القراءة (فيُرفَض مثل هذا المستند) وعند الكتابة أيضًا (فتُرفَض قيمة ClickHouse هذه بدلًا من إنتاج مستند سترفضه صيغة الإدخال عند قراءته).

عند التعطيل، لا تُفرَض قواعد الشكل هذه: فتُقرأ هذه الأشكال الهندسية كما هي وتُكتب كما هي، وبذلك يمكن للأشكال الهندسية المنحلة أن تمر ذهابًا وإيابًا، لكن قد لا يكون المستند الناتج GeoJSON صالحًا.

التحقق هنا بنيوي فقط: فهو يفحص عدد النقاط وإغلاق الحلقة. ولا يتحقق من الصحة الهندسية الفعلية للشكل — فلا يُفرَض اتجاه الحلقة (قاعدة اليد اليمنى / ترتيب الالتفاف)، وتُقبَل الأشكال الهندسية الصالحة بنيويًا لكنها منحلة هندسيًا، مثل مضلع مساحته صفر، أو حلقة ذات تقاطع ذاتي، أو مضلع تقع ثقوبه خارج حلقته الخارجية. وتُرفَض الإحداثيات غير المنتهية (`NaN` و`Inf`) دائمًا بغض النظر عن هذا الإعداد، لأنه لا يمكن تمثيلها كأرقام JSON.
...
يتحكم هذا الإعداد في كيفية قراءة `Nullable(String)` عند وجود قيمة مفقودة في CSV. والقيمة المفقودة هي فراغ بين الفواصل أو قبلها أو بعدها، من دون أن تكون محاطة بعلامات اقتباس. إذا كان هذا الإعداد مُمكّنًا، فبغض النظر عن قيمة `input_format_csv_empty_as_default`، ستُفسَّر القيمة المفقودة لـ `Nullable(String)` على أنها `String` فارغة، وليس NULL.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "19.12"},{"label": "1"},{"label": "تمكين احتساب تعبيرات القيم الافتراضية المعقّدة للحقول المُغفلة في بعض تنسيقات الإدخال، لأن هذا هو السلوك المتوقَّع"}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "25.5"},{"label": "1"},{"label": "عند قراءة ملفات Parquet، يتم تخطي مجموعات الصفوف بأكملها استنادًا إلى تعبيري WHERE/PREWHERE ومرشح بلوم في البيانات الوصفية لـ Parquet."}]}, {"id": "row-2","items": [{"label": "24.10"},{"label": "0"},{"label": "عند قراءة ملفات Parquet، يتم تخطي مجموعات الصفوف بأكملها استنادًا إلى تعبيري WHERE/PREWHERE ومرشح بلوم في البيانات الوصفية لـ Parquet."}]}]} />

عند قراءة ملفات Parquet، يتم تخطي مجموعات الصفوف بأكملها استنادًا إلى تعبير WHERE ومرشح بلوم في البيانات الوصفية لـ Parquet.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "24.9"},{"label": "0"},{"label": "محاولة استنتاج النوع Variant في التنسيقات النصية عند وجود أكثر من نوع محتمل لعناصر العمود/المصفوفة"}]}]} />

إذا كان مُمكّنًا، فسيحاول ClickHouse استنتاج النوع [`Variant`](/ar/reference/data-types/variant) ضمن استنتاج المخطط للتنسيقات النصية عندما يكون هناك أكثر من نوع محتمل لعناصر العمود/المصفوفة.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "1"},{"label": "إعداد جديد. عند تمكين output_format_csv_serialize_tuple_into_separate_columns، تُبسَّط الآن ترويسة CSVWithNames\/CSVWithNamesAndTypes أعمدة Tuple إلى حقولها الطرفية بحيث يطابق عدد أعمدة الترويسة البيانات. اضبطه على false لاستعادة الترويسة السابقة ذات الاسم الواحد."}]}]} />

عند تمكين [output&#95;format&#95;csv&#95;serialize&#95;tuple&#95;into&#95;separate&#95;columns](#output_format_csv_serialize_tuple_into_separate_columns)، تُبسِّط صفوف الترويسة في `CSVWithNames` و`CSVWithNamesAndTypes` كل عمود Tuple إلى حقوله الطرفية (أسماء منقوطة مثل `t.a` و`t.b`، وأسماء الأنواع الطرفية)، بحيث يكون في الترويسة العدد نفسه من الأعمدة الموجودة في البيانات. بالنسبة إلى `CustomSeparated*`، لا ينطبق هذا التبسيط إلا عندما تكون `format_custom_escaping_rule = 'CSV'` و`format_custom_field_delimiter` محرفًا واحدًا يساوي `format_csv_delimiter`؛ وإلا (على سبيل المثال، محدِّد tab الافتراضي أو `format_custom_field_delimiter = '|'`) تبقى الترويسة غير مبسطة بحيث تظل مطابقة للبيانات. اضبطه على `0` للإبقاء على السلوك السابق، حيث تحتفظ الترويسة باسم Tuple الأعلى المفرد ونوعه.

ملاحظة: لا يُعاد قراءة الترويسة المبسطة إلى Tuple بالاسم عندما تكون `input_format_with_names_use_header = 1`. لقراءة مثل هذه البيانات مرة أخرى إلى Tuple، اضبط هذا الإعداد على `0` عند الإخراج، أو اقرأ باستخدام `input_format_with_names_use_header = 0` (وبالنسبة إلى تنسيقات `*WithNamesAndTypes`، أي `CSVWithNamesAndTypes` و`CustomSeparatedWithNamesAndTypes`، اضبط أيضًا `input_format_with_types_use_header = 0`، لأن صف الأنواع المبسط سيُتحقق منه بخلاف ذلك مقابل حقل الإدخال الأعلى المفرد من Tuple وسيُرفض).
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "25.6"},{"label": "0"},{"label": "إضافة عمليات التسلسل المسطحة لـ Dynamic/JSON إلى صيغة الإخراج Native"}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "24.3"},{"label": "1"},{"label": "يتيح ClickHouse استخدام بيانات ثنائية عشوائية في نوع البيانات String، والذي يكون عادةً بترميز UTF-8. لا تدعم سلاسل Parquet/ORC/Arrow سوى UTF-8. لذلك يمكنك اختيار نوع بيانات Arrow الذي سيُستخدم مع نوع البيانات String في ClickHouse — String أو Binary. ورغم أن Binary أكثر دقةً وتوافقًا، فإن استخدام String افتراضيًا يتوافق مع توقعات المستخدمين في معظم الحالات."}]}]} />
