...
اسم المجموعة المُسمّاة التي يستخدمها `aiEmbed` عندما لا يمرّر الاستدعاء `credentials` ضمن خريطة المعلمات. وتعني القيمة الفارغة عدم وجود قيمة افتراضية: لذا يجب على مثل هذه الاستدعاءات تمرير `credentials` صراحةً. ويُحتفَظ به منفصلًا عن `ai_function_text_default_credentials` لأن نقطة نهاية التضمين والنموذج يختلفان عن نظيريهما الخاصَّين بالدردشة.
...
اسم المجموعة المُسمّاة التي تستخدمها دوال الذكاء الاصطناعي النصية (`aiGenerate` و`aiClassify` و`aiExtract` و`aiTranslate`) عندما لا يمرّر الاستدعاء `credentials` ضمن خريطة المعلمات. وتعني القيمة الفارغة عدم وجود قيمة افتراضية: لذلك يجب على هذه الاستدعاءات تمرير `credentials` صراحةً. تختلف نقطة النهاية `chat-completions` والنموذج المرتبط بها عن نظيريهما في `التضمين`، لذا فهذا الإعداد منفصل عن `ai_function_embedding_default_credentials`.
...
السماح باستخدام محلل الاستعلامات.
...
عندما يمكن أن يشير معرّف متعدد الأجزاء مثل `b.id` إما إلى العمود `id` في جدول يحمل الاسم المستعار `b` أو إلى عمود فرعي `b.id` من نوع Tuple تابع لعمود آخر، فامنح الأفضلية لتفسير بادئة الاسم المستعار (أي العمود `id` في `b`). افتراضيًا، يفضّل مُحلِّل العمود الفرعي. فعِّل هذا الإعداد لمطابقة آلية التحليل في المُحلِّل القديم.
...
<Note>
  يؤثر هذا الإعداد أيضًا في الدُفعات التالفة (التي قد تظهر بسبب توقّف غير طبيعي لـ server (الجهاز)، مع عدم استخدام `fsync_after_insert`/`fsync_directories` لمحرك الجدول [Distributed](/ar/reference/engines/table-engines/special/distributed)).
</Note>
...
أما بالنسبة إلى الجداول غير المكررة، فراجع [non&#95;replicated&#95;deduplication&#95;window](/ar/reference/settings/merge-tree-settings#non_replicated_deduplication_window).
...
* عندما يكون `insert_quorum_parallel` معطّلًا وتُجرى محاولة لكتابة البيانات قبل أن تكون الكتلة السابقة قد أُدرجت بعد في `insert_quorum` من النسخ المتماثلة. وقد يحدث هذا إذا حاول المستخدم تنفيذ استعلام `INSERT` آخر على الجدول نفسه قبل اكتمال الاستعلام السابق الذي يستخدم `insert_quorum`.
...
* 0 — إذا كان الجدول الأيمن يحتوي على أكثر من صف مطابق واحد، فسيُضم أول صف يُعثر عليه فقط.
* 1 — إذا كان الجدول الأيمن يحتوي على أكثر من صف مطابق واحد، فسيُضم آخر صف يُعثر عليه فقط.
...
<Note>
  ...
  فسيتم تفعيل وضع «التجميع الخارجي» (تفريغ إلى القرص).
...
على سبيل المثال، يتكون الـ عنقود من shardين، ويحتوي كل shard على جدول
...
يُتحقق من هذا القيد لكل كتلة بيانات تتم معالجتها.
...
لا تخلط بين الكتل المستخدمة للضغط (كتلة بيانات من الذاكرة تتكوّن من بايتات) والكتل المستخدمة في معالجة الاستعلامات (مجموعة من الصفوف من جدول).
...
قيد على إسقاط التقسيمات أثناء تنفيذ الاستعلام. تعني القيمة `0` أنه يمكنك إسقاط التقسيمات دون أي قيود.

القيمة الافتراضية في Cloud: 1 تيرابايت.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "19.5"},{"label": "100"},{"label": "إضافة حدّ لعدد التقسيمات في كتلة واحدة"}]}]} />

يحدّد الحد الأقصى لعدد التقسيمات في كتلة واحدة مُدرجة،
ويتم طرح استثناء إذا كانت الكتلة تحتوي على عدد كبير جدًا من التقسيمات.

* عدد صحيح موجب.
* `0` — عدد غير محدود من التقسيمات.

**التفاصيل**

عند إدراج البيانات، يحسب ClickHouse عدد التقسيمات في
الكتلة المُدرجة. وإذا كان عدد التقسيمات أكبر من
...
> &quot;عدد التقسيمات كبير جدًا بالنسبة إلى كتلة INSERT واحدة (عدد التقسيمات `partitions_count`، والحد هو &quot; + toString(max&#95;partitions) + &quot;).
> يتم التحكم في هذا الحد بواسطة الإعداد &#39;max&#95;partitions&#95;per&#95;insert&#95;block&#39;.
> يُعد العدد الكبير من التقسيمات من المفاهيم الخاطئة الشائعة. وسيؤدي ذلك إلى
> تأثير سلبي حاد على الأداء، بما في ذلك بطء تشغيل server، وبطء استعلامات INSERT
> وبطء استعلامات SELECT. العدد الإجمالي الموصى به للتقسيمات في table هو
> أقل من 1000..10000. يرجى ملاحظة أن التقسيم ليس مخصصًا لتسريع
> استعلامات SELECT (يكفي order by key لجعل الاستعلامات النطاقية سريعة).
> التقسيمات مخصصة لمعالجة البيانات (DROP PARTITION، إلخ).&quot;

<Note>
  هذا الإعداد هو safety threshold لأن استخدام عدد كبير من التقسيمات يُعد من المفاهيم الخاطئة الشائعة.
</Note>
...
يحدّد الحد الأقصى لعدد التقسيمات التي يمكن الوصول إليها في استعلام واحد.
...
أعِد كتابة الدوال التجميعية التي تستخدم تعبير if كوسيطة عندما تكون مكافئة منطقيًا.
...
حدِّد أي طرف من طرفَي JOIN يجب أن يكون جدول البناء (ويُسمى أيضًا الداخلي، وهو الذي يُدرَج في جدول التجزئة في `hash join`) في خطة الاستعلام. لا يدعم هذا الإعداد إلا مع درجة الصرامة `ALL` في JOIN باستخدام عبارة `JOIN ON`. القيم الممكنة هي:
...
وهو مفيد لتجنّب طول وقت التحسين في الاستعلامات المعقّدة.
...
يبدّل تحسينًا على مستوى خطة الاستعلام يدمج عوامل التصفية المتتالية.
...
* &#39;dpsub&#39; - يطبّق خوارزمية DPsub التي تدعم كلًا من عمليات INNER JOIN وغير INNER JOIN - يراعي جميع ترتيبات JOIN الممكنة ويعثر على الترتيب الأمثل، لكنه قد يكون بطيئًا للاستعلامات التي تتضمن عددًا كبيرًا من الجداول وشروط JOIN.
...
يحدّد الفترة الزمنية لمؤقّت الساعة الفعلية الخاص بـ [أداة توصيف الاستعلام](/ar/concepts/features/performance/troubleshoot/sampling-query-profiler). ويقيس مؤقّت الساعة الفعلية وقت الساعة.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "0.2"},{"label": "أُعيدت تسميته من `text_index_density_threshold` (مع الإبقاء عليه كاسم بديل)؛ ويحدّد خوارزمية تقاطع الـ قائمة النشر في وضع التطبيق lazy للـ قائمة النشر."}]}]} />

عتبة كثافة الـ قائمة النشر التي تحدّد خوارزمية التقاطع في وضع التطبيق lazy للـ قائمة النشر (`text_index_posting_list_apply_mode = 'lazy'`).
دون العتبة: تقاطع leapfrog (يفضّل الـ قوائم النشر المتفرقة). عند العتبة أو أعلى منها: تقاطع brute-force bitmap (يفضّل الـ قوائم النشر الكثيفة).
...
&#39;lazy&#39; يستخدم فك ترميز عند الطلب يعتمد على المؤشر (يتطلب تنسيق فهرس مع codec مُسلسل).
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "21.1"},{"label": "1"},{"label": "استخدام التنسيق المضغوط افتراضيًا لعمليات غير متزامن INSERT في جداول Distributed"}]}]} />

يستخدم التنسيق المضغوط لتخزين الكتل لعمليات INSERT في الخلفية (`distributed_foreground_insert`) في الجداول التي تستخدم المحرك `Distributed`.
