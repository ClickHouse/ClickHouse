...
## معاملات الوصول {#access-operators}
...
## معامل النفي العددي {#numeric-negation-operator}

`-a` – الدالة `negate (a)`.

لنفي `tuple`: [tupleNegate](/ar/reference/functions/regular-functions/tuple-functions#tupleNegate).

## معاملات الضرب والقسمة {#multiplication-and-division-operators}
...
## معاملات الجمع والطرح {#addition-and-subtraction-operators}
...
## المعاملات الخاصة بالتعامل مع السلاسل النصية {#operators-for-working-with-strings}
...
## المعاملات الخاصة بالعمل مع مجموعات البيانات {#operators-for-working-with-data-sets}

راجع [معاملات IN](/ar/reference/statements/in) و[المعامل EXISTS](/ar/reference/operators/exists).

### الدالة in {#in-function}

`a IN ...` – الدالة `in(a, b)`.

### دالة notIn {#notin-function}

`a NOT IN ...` – الدالة `notIn(a, b)`.

### دالة globalIn {#globalin-function}

`a GLOBAL IN ...` – الدالة `globalIn(a, b)`.

### دالة globalNotIn {#globalnotin-function}

`a GLOBAL NOT IN ...` – الدالة `globalNotIn(a, b)`.

### دالة in للاستعلامات الفرعية {#in-subquery-function}

`a = ANY (subquery)` – وهي الدالة `in(a, subquery)`.

### notIn استعلام فرعي function {#notin-subquery-function}

`a != ANY (subquery)` – وهو مماثل لـ `a NOT IN (SELECT singleValueOrNull(*) FROM subquery)`.

### دالة in استعلام فرعي {#in-subquery-function-1}

`a = ALL (subquery)` – هي نفسها `a IN (SELECT singleValueOrNull(*) FROM subquery)`.

### دالة notIn للاستعلام الفرعي {#notin-subquery-function-1}
...
### `SOME` / `ALL` على المصفوفات {#some-all-on-arrays}
...
## المعاملات للعمل مع التواريخ والأوقات {#operators-for-working-with-dates-and-times}
...
### إضافة التاريخ والوقت {#date-time-addition}
...
### AT TIME ZONE وAT LOCAL {#at-time-zone}
...
## المعامل المنطقي AND {#logical-and-operator}
...
## معامل OR المنطقي {#logical-or-operator}
...
## معامل النفي المنطقي {#logical-negation-operator}
...
## المعامل الشرطي {#conditional-operator}
...
## معامل ربط السلاسل {#concatenation-operator}

`s1 || s2` – دالة `concat(s1, s2)`.

## معامل إنشاء Lambda {#lambda-creation-operator}

`x -> expr` – الدالة `lambda(x, expr)`.

لا أولوية للمعاملات التالية لأنها أقواس:

## معامل إنشاء المصفوفة {#array-creation-operator}

`[x1, ...]` – الدالة `array(x1, ...)`.

## معامل إنشاء Tuple {#tuple-creation-operator}
...
## التحقق من وجود `NULL` {#checking-for-null}
...
يمكن تحسين ذلك بتمكين الإعداد [optimize&#95;functions&#95;to&#95;subcolumns](/ar/reference/settings/session-settings/optimize#optimize_functions_to_subcolumns). عند ضبط `optimize_functions_to_subcolumns = 1`، تقرأ الدالة العمود الفرعي [null](/ar/reference/data-types/nullable#finding-null) فقط بدلًا من قراءة بيانات العمود بالكامل ومعالجتها. ويتحوّل الاستعلام `SELECT n IS NULL FROM table` إلى `SELECT n.null != 0 FROM TABLE`.
...
يمكن تحسين ذلك عبر تفعيل إعداد [optimize&#95;functions&#95;to&#95;subcolumns](/ar/reference/settings/session-settings/optimize#optimize_functions_to_subcolumns). عند ضبط `optimize_functions_to_subcolumns = 1`، لا تقرأ الدالة سوى العمود الفرعي [null](/ar/reference/data-types/nullable#finding-null) بدلًا من قراءة بيانات العمود بالكامل ومعالجتها. ويتحوّل الاستعلام `SELECT n IS NOT NULL FROM table` إلى `SELECT NOT n.null FROM TABLE`.

## التحقق من القيم المنطقية {#checking-boolean-values}
