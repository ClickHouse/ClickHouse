...
تُرجع الدوال `avg` و`min` و`max` و`any` عند تطبيقها على column من النوع `Interval` فاصلًا زمنيًا من النوع نفسه.
وتحسب `avg` متوسط عدد الوحدات ثم تقرّب النتيجة إلى عدد صحيح منها:
...
الفواصل الزمنية ذات الوحدات المختلفة غير قابلة للقياس المشترك — فالشهر ليس عددًا ثابتًا من الأيام — لذا
لا يوجد تحويل بينها، ويُجمَّع كل نوع من الفواصل الزمنية على حِدة.
