...
في استعلام يبدأ بـ `FROM`، تكون عبارة `SELECT` اختيارية. عند حذفها، يعمل الاستعلام كما لو كُتبت `SELECT *`:
...
يُستخدم هذا الشكل غالبًا في الاستعلامات التي تستخدم [عوامل pipe](/ar/reference/statements/select/pipe-operators):
...
تُنفَّذ استعلامات `SELECT` التي تتضمن `FINAL` بالتوازي. ويحدّد الإعداد [max&#95;final&#95;threads](/ar/reference/settings/session-settings/max#max_final_threads) عدد سلاسل التنفيذ المستخدمة.
...
يمكن تطبيق `FINAL` تلقائيًا باستخدام إعداد [FINAL](/ar/reference/settings/session-settings/other#final) على جميع الجداول في الاستعلام باستخدام جلسة أو ملف تعريف مستخدم.
