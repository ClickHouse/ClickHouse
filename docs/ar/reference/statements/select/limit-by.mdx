...
يحدِّد الاستعلام الذي يحتوي على البند `LIMIT n BY expressions` أول `n` صفوف لكل قيمة مميزة من `expressions`. ويمكن أن يحتوي المفتاح الخاص بـ `LIMIT BY` على أي عدد من [تعبيرات](/ar/reference/syntax#expressions).
...
إذا أردت استخدام أرقام الأعمدة بدلًا من أسماء الأعمدة في البند `LIMIT BY`، ففعّل الإعداد [enable&#95;positional&#95;arguments](/ar/reference/settings/session-settings/enable-positional-arguments#enable_positional_arguments).
