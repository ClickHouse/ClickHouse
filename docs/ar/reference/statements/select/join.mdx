...
| Type                                                | Description                                                                                                                          |
| --------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| `LEFT SEMI JOIN`, `RIGHT SEMI JOIN`                 | قائمة سماح على &quot;مفاتيح الربط&quot;، من دون إنتاج حاصل الضرب الديكارتي.                                                              |
| `LEFT ANTI JOIN`, `RIGHT ANTI JOIN`                 | قائمة حظر على &quot;مفاتيح الربط&quot;، من دون إنتاج حاصل الضرب الديكارتي.                                                               |
...
<Note>
  عند ضبط [join&#95;algorithm](/ar/reference/settings/session-settings/join#join_algorithm) على `partial_merge`، لا يكون `RIGHT JOIN` و`FULL JOIN` مدعومَين إلا مع strictness من النوع `ALL` (`SEMI` و`ANTI` و`ANY` و`ASOF` غير مدعومة).
</Note>
...
يمكن تجاوز نوع `JOIN` الافتراضي باستخدام الإعداد [`join_default_strictness`](/ar/reference/settings/session-settings/join#join_default_strictness).

يعتمد سلوك ClickHouse server في عمليات `ANY JOIN` على الإعداد [`any_join_distinct_right_table_keys`](/ar/reference/settings/session-settings/other#any_join_distinct_right_table_keys).
...
* [`partial_merge_join_rows_in_right_blocks`](/ar/reference/settings/session-settings/partial-merge#partial_merge_join_rows_in_right_blocks)
* [`join_on_disk_max_files_to_merge`](/ar/reference/settings/session-settings/join#join_on_disk_max_files_to_merge)
* [`any_join_distinct_right_table_keys`](/ar/reference/settings/session-settings/other#any_join_distinct_right_table_keys)
...
يدعم ClickHouse حاليًا `ALL/ANY/SEMI/ANTI INNER/LEFT/RIGHT/FULL JOIN` مع شروط عدم المساواة بالإضافة إلى شروط المساواة. ولا تُدعَم شروط عدم المساواة إلا مع خوارزميات JOIN ‏`hash` و`parallel_hash` و`grace_hash`. لا يجوز أن يحتوي شرط عدم مساواة يُقيَّم أثناء JOIN على `arrayJoin`، إذ يجب أن يحافظ هذا الشرط على عدد الصفوف؛ أما الشرط الذي ينطبق على جانب واحد فقط ومفتاح المساواة على `arrayJoin`، فيُستخرجان قبل JOIN ولا يتأثران؛ كما لا يتأثر شرط `ALL INNER JOIN` غير الانفصالي، لأن الشرط يُطبَّق هناك بعد JOIN بدلًا من ذلك. عندما يعتمد التوسيع على جانب واحد فقط، انقله إلى `ARRAY JOIN` في استعلام فرعي قبل JOIN؛ أما الشرط الذي يقرأ وسيط `arrayJoin` فيه أعمدة من كلا الجانبين، فيجب إعادة صياغته.
...
<div id="null-values-in-join-keys">
  ## قيم `NULL` و`NaN` في مفاتيح الربط
</div>

لا تساوي `NULL` أي قيمة، بما في ذلك نفسها. وهذا يعني أنه إذا كانت قيمة مفتاح الربط هي `NULL` في أحد الجدولين، فلن تطابق قيمة `NULL` في الجدول الآخر.
...
لاحظ أن الصف الذي يحتوي على `Charlie` من الجدول `A`، والصف الذي درجته 88 من الجدول `B`، لا يظهران في النتيجة بسبب القيمة `NULL` في مفتاح الربط.

إذا أردت مطابقة قيم `NULL`، فاستخدم الدالة `isNotDistinctFrom` لمقارنة مفاتيح الربط.
...
لا تتبع قيم `NaN` في مفاتيح الربط من نوع float قاعدة `NULL` المذكورة أعلاه.
تكون المقارنة القياسية بين قيمتي `NaN` (`NaN = NaN`) هي `0`، لكن لا تُقارن مفاتيح الربط وفق دلالات المقارنة القياسية؛ وقد تتطابق مفاتيح `NaN`.
وما إذا كانت تتطابق فعليًا يعتمد على تفاصيل التنفيذ وخوارزمية الربط ونوع المفتاح وإعدادات الجلسة.
لا تعتمد على سلوك محدد.
إذا كنت تحتاج إلى عدم تطابق الصفوف التي تحتوي على `NaN`، فحوّلها إلى قيم `NULL`: `ON if(isNaN(A.id), NULL, A.id) = B.id`.
في `ASOF JOIN`، يُقارن العمود الخاص بأقرب تطابق وفق الترتيب، وهو ما لا تدعمه `NaN` — لذا رشّح هذه الصفوف من كلا الجانبين.
...
أثناء ربط الجداول، قد تظهر خلايا فارغة. يحدّد الإعداد [join&#95;use&#95;nulls](/ar/reference/settings/session-settings/join#join_use_nulls) كيفية ملء ClickHouse لهذه الخلايا.

إذا كانت مفاتيح الربط `JOIN` حقولًا من النوع [Nullable](/ar/reference/data-types/nullable)، فلن تُربط الصفوف التي تكون قيمة واحد على الأقل من هذه المفاتيح فيها [NULL](/ar/reference/syntax#null).
...
افتراضيًا، يستخدم ClickHouse خوارزمية [hash join](https://en.wikipedia.org/wiki/Hash_join). يأخذ ClickHouse الجدول الأيمن `right_table` وينشئ له جدول تجزئة في RAM. إذا كان `join_algorithm = 'auto'` مفعّلًا، فبعد تجاوز عتبة معيّنة من استهلاك الذاكرة، يعود ClickHouse إلى خوارزمية [merge](https://en.wikipedia.org/wiki/Sort-merge_join) join. للاطلاع على وصف خوارزميات `JOIN`، راجع الإعداد [join&#95;algorithm](/ar/reference/settings/session-settings/join#join_algorithm).

إذا كنت بحاجة إلى تقييد استهلاك الذاكرة لعملية `JOIN`، فاستخدم الإعدادات التالية:

* [max&#95;rows&#95;in&#95;join](/ar/reference/settings/session-settings/max-rows#max_rows_in_join) — يحدّ من عدد الصفوف في جدول التجزئة.
* [max&#95;bytes&#95;in&#95;join](/ar/reference/settings/session-settings/max-bytes#max_bytes_in_join) — يحدّ من حجم جدول التجزئة.

عند بلوغ أيٍّ من هذه الحدود، يتصرّف ClickHouse وفقًا لما يحدّده الإعداد [join&#95;overflow&#95;mode](/ar/reference/settings/session-settings/join#join_overflow_mode).
