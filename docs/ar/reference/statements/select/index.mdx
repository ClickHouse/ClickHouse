...
* [بند PREWHERE](/ar/reference/statements/select/prewhere) (يرشّح الجدول الوحيد الذي يُطبَّق عليه، لذا يُنفَّذ قبل `JOIN` رغم موضعه في هذه القائمة)
...
يمكن أيضًا كتابة الاستعلام كسلسلة خطية من التحويلات باستخدام [عوامل الأنبوب](/ar/reference/statements/select/pipe-operators):
...
تدعم بنود `GROUP BY` و`ORDER BY` و`LIMIT BY` الوسيطات الموضعية. لتمكين ذلك، فعِّل الإعداد [enable&#95;positional&#95;arguments](/ar/reference/settings/session-settings/enable-positional-arguments#enable_positional_arguments). بعد ذلك، على سبيل المثال، سيؤدي `ORDER BY 1,2` إلى فرز صفوف الجدول حسب العمود الأول ثم العمود الثاني.
