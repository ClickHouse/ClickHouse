...
<Note>
  لا يدعم PostgreSQL `FROM unnest(...)` و`CROSS JOIN UNNEST(...)` و`LATERAL`. استخدم `ARRAY JOIN` بدلًا من ذلك. الاسم `unnest` (منذ الإصدار 26.5) هو اسم مستعار لاستدعاء الدالة `arrayJoin` (`SELECT unnest(arr)`)، وليس دالة جدول.
</Note>
...
يُعد [التقييم المختصر للدوال](/ar/reference/settings/session-settings/short-circuit-function-evaluation#short_circuit_function_evaluation) ميزة تُحسّن تنفيذ التعبيرات المعقدة في دوال محددة مثل `if` و`multiIf` و`and` و`or`. وهي تمنع حدوث استثناءات محتملة، مثل القسمة على صفر، أثناء تنفيذ هذه الدوال.
