...
الجدول الزمني (`EVERY` أو `AFTER`) إلزامي: إذ تستبدل التعليمة *جميع* معلمات التحديث دفعةً واحدة. وأي عبارة لا تُحدَّد — `RANDOMIZE FOR` أو `DEPENDS ON` أو `SETTINGS` — تُزال أو تُعاد إلى قيمها الافتراضية. ولتغيير إعدادات التحديث فقط، أعِد استخدام الجدول الزمني الحالي.

يُحدِّث الأمر تكوين التحديث للعرض الحالي في موضعه، دون إعادة إنشاء العرض المادي أو جدول الهدف الخاص به، أو مسح بيانات الهدف الحالية، أو إلغاء تحديث قيد التشغيل بالفعل. وتستمر عمليات القراءة من جدول الهدف الحالي أثناء تغيير التكوين.

تكرار الأمر باستخدام مواصفة `REFRESH` الكاملة نفسها آمن. احرص على تكرار كل عبارة ينبغي أن تظل مُكوَّنة، لأن كل تنفيذ يستبدل جميع معلمات التحديث كما هو موضح أعلاه.
