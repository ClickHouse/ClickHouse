...
يُحدَّد تزامن معالجة الاستعلام بواسطة الإعداد [mutations&#95;sync](/ar/reference/settings/session-settings/mutations#mutations_sync). وهو غير متزامن افتراضيًا.
...
* الإعداد [mutations&#95;sync](/ar/reference/settings/session-settings/mutations#mutations_sync)
...
إذا كان تعبير عمود [`MATERIALIZED`](/ar/reference/statements/create/table#materialized) يقرأ عمودًا
مُحدَّثًا، تُعيد عملية التعديل حسابه، بحيث تظل قيمته المخزنة متسقة مع البيانات الجديدة.

<div id="columns-calculated-from-ephemeral-columns">
  ### الأعمدة المحسوبة من أعمدة EPHEMERAL
</div>

لا يوجد العمود [`EPHEMERAL`](/ar/reference/statements/create/table#ephemeral) إلا طوال مدة عملية
`INSERT` ولا يُخزَّن مطلقًا، لذلك لا يمكن لعملية تعديل إعادة حساب عمود `MATERIALIZED` محسوب منه.
ويحتفظ هذا العمود بالقيمة المحسوبة وقت تنفيذ `INSERT`، التي لا تعود بعد ذلك مطابقة لتعبيره:
...
`m` تساوي `8`، وهي القيمة المحسوبة أثناء `INSERT`، وليست `2 + 7`: فقيمة `e` غير متاحة
خارج عملية `INSERT` التي زوّدتها بها. تسجّل عملية التعديل تحذيرًا في سجل الخادم عند تخطي
عمود لهذا السبب. لتحديث هذا العمود، أعد تنفيذ `INSERT` للصفوف المتأثرة.
