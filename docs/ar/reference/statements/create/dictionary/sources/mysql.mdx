...
<Note>
  تشير `ssl_ca` و`ssl_cert` و`ssl_key` إلى أسماء ملفات يفتحها الخادم باستخدام امتيازاته الخاصة، لذا لا تُقبل إلا لقاموس مُعرّف في ملف تهيئة الخادم، أو من خلال مجموعة مُسمّاة مُعرّفة فيه. يجب أن يمرّر استعلام `CREATE DICTIONARY` الذي يحدد بيانات اعتماد TLS مباشرةً محتوياتها بدلًا من ذلك في `ssl_ca_pem` و`ssl_cert_pem` و`ssl_key_pem`. تُخفى هذه القيم في السجلات واستعلامات `SHOW`، تمامًا كما تُخفى كلمات المرور.
</Note>

<Note>
  لا توجد معلمة صريحة باسم `secure`. عند إنشاء اتصال SSL، يكون استخدام الحماية إلزاميًا.
</Note>

يمكن الاتصال بـ MySQL على المضيف المحلي عبر المقابس. للقيام بذلك، عيّن `host` و`socket`.
