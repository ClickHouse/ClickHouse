...
| `ssl_ca`, `ssl_cert`, `ssl_key` | بيانات الاعتماد نفسها على هيئة مسارات إلى ملفات على الخادم. يُسمح بها فقط لقاموس معرّف في ملف تهيئة الخادم، انظر أدناه. اختياري.                                                                                                                                                                           |
...
<Note>
  تشير `ssl_ca` و`ssl_cert` و`ssl_key` إلى أسماء ملفات يفتحها الخادم باستخدام امتيازاته الخاصة، لذا لا تُقبل إلا لقاموس مُعرّف في ملف تهيئة الخادم. يجب أن يمرّر القاموس المُنشأ باستعلام `CREATE DICTIONARY` المحتويات بدلًا من ذلك في `ssl_ca_pem` و`ssl_cert_pem` و`ssl_key_pem`. تُخفى هذه القيم في السجلات واستعلامات `SHOW`، تمامًا كما تُخفى كلمات المرور.
</Note>
