...
sidebarTitle: 'السمات'
slug: /ar/reference/statements/create/dictionary/attributes
title: 'سمات القاموس'
doc_type: 'reference'
---
...
يمكن أن يكون المفتاح `tuple` مكوَّنًا من حقول بأي أنواع. ويجب أن يكون [التخطيط](/ar/reference/statements/create/dictionary/layouts/overview) في هذه الحالة `complex_key_hashed` أو `complex_key_cache`.

<Tip>
  يمكن أن يتكوّن المفتاح المركب من عنصر واحد. وهذا يتيح، على سبيل المثال، استخدام سلسلة نصية كمفتاح.
</Tip>

يُحدَّد تركيب المفتاح في العنصر `<key>`. وتُحدَّد حقول المفتاح بالتنسيق نفسه المستخدم في [سمات](#attributes) القاموس. مثال:
...
عندما يتكوّن المفتاح المركب من سمة واحدة، يمكن تمرير قيمة المفتاح مباشرةً، من دون تغليفها داخل `tuple`. على سبيل المثال، كلٌّ من `dictGetString('dict_name', 'attr_name', 'key')` و `dictGetString('dict_name', 'attr_name', tuple('key'))` صحيح.
