...
عندما يتكوّن المفتاح المركب من سمة واحدة، يمكن تمرير قيمة المفتاح مباشرةً، من دون تغليفها داخل `tuple`. على سبيل المثال، كلٌّ من `dictGetString('dict_name', 'attr_name', 'key')` و `dictGetString('dict_name', 'attr_name', tuple('key'))` صحيح.
