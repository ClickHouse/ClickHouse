...
تشبه العروض ذات المعلمات العروض العادية، لكنها يمكن أن تُنشأ بمعلمات لا تُفسَّر فورًا.
ويمكن استخدام هذه العروض مع دوال الجداول، بحيث يُستخدَم اسم العرض اسمًا للدالة، وتُمرَّر قيم المعلمات كوسائط لها.
...
بما أن العرض ذو المعلمات يعتمد على قيم المعلمات، فلا يكون له مخطط عند عدم توفير المعلمات.
وهذا يعني عدم وجود معلومات عن العروض ذات المعلمات في جدول `system.columns`.
كما أن استعلامات `DESCRIBE` لن تعمل إلا عند توفير المعلمات.
...
* **مع بند `TO`**: يُستبدل تعريف العرض فقط؛ ولا يتأثر الجدول الهدف أو بياناته.
...
عند إنشاء عرض مادي باستخدام `TO [db].[table]`، يمكنك أيضًا استخدام `POPULATE` لملء الجدول الهدف بالبيانات المصدر الموجودة (قد يحتوي الجدول الهدف على بيانات بالفعل، وفي هذه الحالة تُلحَق الصفوف المُعبّأة). لا يمكن دمج `POPULATE` مع `REFRESH`: إذ يُملأ [العرض المادي القابل للتحديث](#refreshable-materialized-view) عند أول تحديث له، لذا سيؤدي `POPULATE` إلى تحميل البيانات الأولية مرتين (استخدم `EMPTY` لتخطي التحديث الأول بدلًا من ذلك).
...
إذا حددت `POPULATE`، فستُدرَج بيانات الجدول المصدر الموجودة مسبقًا في العرض عند إنشائه. وإلا، فلن يحتوي العرض إلا على البيانات المُدرجة في الجدول المصدر بعد إنشاء العرض.

بالنسبة إلى `CREATE MATERIALIZED VIEW` عادي، يكون `POPULATE` **ذريًا** افتراضيًا (الإعداد `materialized_views_populate_atomically = 1`): يُشترَك العرض في عمليات الإدراج الجديدة في الجدول المصدر وتُؤخَذ لقطة من البيانات الموجودة معًا، ضمن قفل حصري قصير على الجدول المصدر، بحيث يُسلَّم إلى العرض كل صف يُدرج بالتزامن مع التعبئة **مرة واحدة بالضبط** — دون فقدان أو تكرار. ثم تقرأ عملية التعبئة (التي قد تستغرق وقتًا طويلًا) اللقطة المثبتة دون الاحتفاظ بأي قفل.

هذه ذرية محلية لمسار الإدراج: لا ينسّق القفل الحصري إلا مع عمليات الإدراج التي تحصل على قفل تخزين هذا الجدول المصدر **على الخادم نفسه**، لذا يغطي ضمان exactly-once عمليات الإدراج التي تصل عبر هذا الخادم. ولا يمثل ذلك ضمانًا على مستوى المجموعة — فالصفوف المُدرجة على نسخة متماثلة أخرى من مصدر `ReplicatedMergeTree`، أو عبر مسار كتابة موزع (مثلًا، في جدول `Distributed` أو عبر `ON CLUSTER`) بالتزامن مع التعبئة تقع خارج هذا الحد وقد تظل معرضة للفقدان أو التكرار.

إذا فشلت التعبئة — على سبيل المثال، إذا تعذر الحصول على القفل الحصري لجدول مصدر مشغول ضمن `lock_acquire_timeout`، أو إذا تسبب `SELECT` الخاص بالعرض في حدوث استثناء أثناء التنفيذ — فسيُحذف العرض الذي أُنشئ للتو ويفشل استعلام `CREATE`، دون ترك أي شيء مما أنشأه، لذلك يمكن إعادة محاولته ببساطة. وبالنسبة إلى صيغة `TO [db].[table]`، يحذف هذا التراجع العرض فقط، ولا يحذف أبدًا الجدول الهدف الموجود مسبقًا — لكن الصفوف التي أدخلتها التعبئة الفاشلة بالفعل إلى الهدف تبقى فيه، تمامًا كما بعد فشل `INSERT ... SELECT` في ذلك الجدول، لذا فإن إعادة محاولة `CREATE` تُدرجها مرة أخرى. إذا كان يجب أن تكون التعبئة اللاحقة دقيقة، فأعد المحاولة في جدول هدف مُفرَّغ أو جديد، أو استخدم محركًا يزيل التكرارات مثل `ReplacingMergeTree`.

<Note>
  تتطلب الذرية أن يدعم الجدول المصدر قراءة لقطة ثابتة عند نقطة زمنية محددة — عائلة `MergeTree` و`Memory`. أما أي مصدر آخر (عرض أو `Distributed` أو `Merge` أو `Buffer` أو عائلة `Log` أو جدول ليس ضمن قاعدة بيانات `Atomic`)، فتعود عملية التعبئة إلى السلوك القديم غير الذري (المسجل في سجل الخادم): إذ تُقرأ البيانات الموجودة باستخدام لقطة منفصلة غير منسقة، لذلك قد تُفوَّت الصفوف المُدرجة أثناء التعبئة أو تتكرر. في هذه الحالة، أنشئ العرض وشغّل `INSERT ... SELECT` منفصلًا إذا كنت تحتاج إلى بيانات دقيقة. يؤدي تعيين `materialized_views_populate_atomically = 0` إلى فرض هذا السلوك القديم على جميع المصادر.

  ينطبق التعبئة الذرية على `CREATE MATERIALIZED VIEW` العادي فقط. يستخدم `CREATE OR REPLACE` / `REPLACE MATERIALIZED VIEW ... POPULATE` دائمًا التعبئة القديمة غير الذرية.

  `POPULATE` غير مدعوم مع قواعد بيانات `Replicated` (استخدم `database_replicated_allow_heavy_create` للتجاوز) وغير مدعوم في ClickHouse Cloud. عند تمكينه عبر ذلك التجاوز، تكون التعبئة دائمًا بالنمط القديم غير الذري — إذ لا يمكن التراجع عن تعبئة فاشلة بصورة متسقة على جميع النسخ المتماثلة.
</Note>
...
<Note>
  يُعد الخيار `SQL SECURITY NONE` خيارًا مُهمَل. سيتمكن أي مستخدم لديه صلاحية إنشاء عروض باستخدام `SQL SECURITY NONE` من تنفيذ أي query عشوائي.
  لذلك، يلزم وجود `GRANT ALLOW SQL SECURITY NONE TO <user>` لإنشاء عرض باستخدام هذا الخيار.
</Note>

إذا لم يتم تحديد `DEFINER`/`SQL SECURITY`، فتعتمد النتيجة على إعداد الخادم [`ignore_empty_sql_security_in_create_view_query`](/ar/reference/settings/server-settings/settings/other#ignore_empty_sql_security_in_create_view_query).

باستخدام قيمته الافتراضية `true`، يُخزَّن الاستعلام كما كُتب ويحصل العرض على نوع أمان SQL فارغ. وعندئذٍ يعمل العرض العادي بصلاحيات المستدعي، أما العرض المادي ذو الجدول الهدف المحدد صراحةً، فتُتخطى عمليات التحقق من الوصول إلى ذلك الجدول: إذ لا يتطلب الإدراج في جدول المصدر امتياز `INSERT` على الجدول الهدف، ولا تتطلب القراءة من العرض امتياز `SELECT` عليه.

باستخدام `false`، تُكتب القيم الافتراضية التالية في تعريف العرض عند إنشائه:

* `SQL SECURITY`: `INVOKER` للعروض العادية (قابل للتهيئة عبر [`default_normal_view_sql_security`](/ar/reference/settings/session-settings/default#default_normal_view_sql_security)) و`DEFINER` للعروض المادية (قابل للتهيئة عبر [`default_materialized_view_sql_security`](/ar/reference/settings/session-settings/default#default_materialized_view_sql_security))
* `DEFINER`: `CURRENT_USER` (قابل للتهيئة عبر [`default_view_definer`](/ar/reference/settings/session-settings/default#default_view_definer))

تتلقى العروض المادية القابلة للتحديث هذه القيم الافتراضية دائمًا، بغض النظر عن الإعداد.

يحتفظ العرض بنوع أمان SQL من تعريفه المخزن عند إرفاقه أو إعادة تحميله عند بدء تشغيل الخادم، لذا يحتفظ العرض المخزن دون `DEFINER`/`SQL SECURITY` بنوع أمان SQL الفارغ.
...
يتم التنسيق عبر Keeper. ويُحدَّد مسار znode بواسطة إعداد الخادم [default&#95;replica&#95;path](/ar/reference/settings/server-settings/settings/default-replica#default_replica_path).
...
<Warning>
  **مهجورة**
  ...
  Window View مهجورة وقد تُزال في إصدار مستقبلي. لا يزال استخدامها مشروطًا بالإعداد [allow&#95;experimental&#95;window&#95;view](/ar/reference/settings/session-settings/allow-experimental#allow_experimental_window_view) (`SET allow_experimental_window_view = 1`) ولا يُوصى باستخدامها في التطبيقات الجديدة.
</Warning>
