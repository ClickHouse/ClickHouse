...
* `SQL SECURITY`: `INVOKER` للعروض العادية و`DEFINER` للعروض المادية ([قابل للتهيئة عبر الإعدادات](/ar/reference/settings/session-settings/default#default_normal_view_sql_security))
* `DEFINER`: `CURRENT_USER` ([قابل للتهيئة عبر الإعدادات](/ar/reference/settings/session-settings/default#default_view_definer))
...
<Info>
  هذه ميزة تجريبية وقد تتغير في الإصدارات القادمة على نحوٍ غير متوافق مع الإصدارات السابقة. لتمكين استخدام Window View واستعلام `WATCH`، استخدم الإعداد [allow&#95;experimental&#95;window&#95;view](/ar/reference/settings/session-settings/allow-experimental#allow_experimental_window_view). أدخل الأمر `set allow_experimental_window_view = 1`.
</Info>
