...
* `SYSTEM`. المستوى: `GROUP`
  ...
  * `SYSTEM BACKGROUND`. المستوى: `GROUP`
    * `SYSTEM STREAMING ENGINES`. المستوى: `TABLE`
    * `SYSTEM VIEWS`. المستوى: `VIEW`. الأسماء البديلة: `SYSTEM REFRESH VIEW`, `SYSTEM START VIEWS`, `SYSTEM STOP VIEWS`, `SYSTEM START VIEW`, `SYSTEM STOP VIEW`, `SYSTEM PAUSE VIEWS`, `SYSTEM PAUSE VIEW`, `SYSTEM CANCEL VIEW`, `REFRESH VIEW`, `START VIEWS`, `STOP VIEWS`, `START VIEW`, `STOP VIEW`, `PAUSE VIEWS`, `PAUSE VIEW`, `CANCEL VIEW`
...
<Note>
  ...
  وإلا، فيجب استخدام الصيغة `GRANT AZURE ON *.* TO user`، وهي مكافئة للصيغة الجديدة `GRANT READ, WRITE ON AZURE TO user`
</Note>
...
[`display_secrets_in_show_and_select` إعداد الخادم](/ar/reference/settings/server-settings/settings/other#display_secrets_in_show_and_select)
و
[`format_display_secrets_in_show_and_select` إعداد التنسيق](/ar/reference/settings/formats/format#format_display_secrets_in_show_and_select)
مفعّلَين.
