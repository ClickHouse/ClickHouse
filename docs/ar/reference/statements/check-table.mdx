...
* `SETTINGS`: (اختياري) يتيح لك إضافة إعدادات إضافية.
  * (اختياري): [check&#95;query&#95;single&#95;value&#95;result](/ar/reference/settings/session-settings/check#check_query_single_value_result): يتحكم هذا الإعداد في ما إذا كان الإخراج مفصلًا (`0`) أو مُلخّصًا (`1`).
...
للتحقق من حالة قاعدة بيانات كاملة بدلًا من سلامة الجداول الفردية، استخدم استعلام [`CHECK DATABASE`](/ar/reference/statements/check-database). وهو مخصّص أساسًا لقواعد البيانات التي تستخدم المحرك [`DataLakeCatalog`](/ar/reference/engines/database-engines/datalake)، حيث يتحقق من إمكانية الوصول إلى الكتالوج الخارجي.
