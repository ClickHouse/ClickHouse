...
في هذا التنسيق، يُفسَّر كائن JSON واحد باعتباره قيمة [JSON](/ar/reference/data-types/newjson) واحدة. وإذا كان الإدخال يحتوي على عدة كائنات JSON (مفصولة بفواصل)، فتُفسَّر على أنها صفوف منفصلة. وإذا كانت بيانات الإدخال محاطة بـ `[]`، فتُفسَّر على أنها مصفوفة من قيم JSON.
