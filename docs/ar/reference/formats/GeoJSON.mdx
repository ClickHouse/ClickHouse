...
عند تمكينه، تُرفَض أي Geometry تخالف قواعد شكل GeoJSON: مثل `LineString` (أو line ضمن `MultiLineString`) يحتوي على أقل من نقطتين؛ أو ring في `Polygon` أو `MultiPolygon` يحتوي على أقل من أربع نقاط، أو تكون نقطتاه الأولى والأخيرة مختلفتين (ring غير مغلق)؛ أو `MultiLineString` أو `Polygon` أو `MultiPolygon` فارغ. وتنطبق القواعد نفسها عند قراءة مثل هذا المستند وعند كتابة قيمة ClickHouse من هذا النوع، لذا فإن أي مستند يُكتَب يمكن دائمًا قراءته مرة أخرى.
...
يقتصر التحقق على البنية فقط: فهو يفحص عدد النقاط وإغلاق ring. ولا يفحص السلامة الهندسية للشكل، لذا تُقبَل أي Geometry صحيحة بنيويًا لكنها متدهورة هندسيًا في أي من الاتجاهين — مثل polygon مساحته صفر، أو ring ذاتي التقاطع، أو polygon تقع holes (الحلقات الداخلية) فيه خارج ring الخارجي. وبالمثل، لا يُفرَض مطلقًا اتجاه قاعدة اليد اليمنى (الالتفاف) لحلقات polygon.
