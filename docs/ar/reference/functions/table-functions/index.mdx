...
<Note>
  تحتاج معظم دوال الجداول إلى صلاحية `CREATE TEMPORARY TABLE` لتعمل، لذلك يُرفض أي `SELECT` أو `INSERT`
  ينفّذ إحداها عندما تكون قيمة
  [`readonly`](/ar/concepts/features/configuration/settings/permissions-for-queries#readonly) مضبوطة على 1.
</Note>