...
## تعيين أنواع البيانات {#data-type-mapping}
...
* تُرسل قيم `INT64` إلى `tabledata.insertAll` كسلاسل عشرية، لأن واجهة برمجة تطبيقات تحلل أرقام JSON كقيم ذات دقة مزدوجة، وإلا فقد تتلف القيم الواقعة خارج `[-2^53 + 1, 2^53 - 1]`.
