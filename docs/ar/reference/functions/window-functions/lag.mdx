---
description: 'توثيق دالة النافذة lag'
sidebarTitle: 'lag'
...
تعيد قيمةً جرى تقييمها في الصف الموجود عند إزاحة فعلية محددة قبل الصف الحالي ضمن الإطار المرتّب.
تشبه هذه الدالة [`lagInFrame`](/ar/reference/functions/window-functions/lagInFrame)، لكنها تستخدم دائمًا الإطار `ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING`.
...
يستعرض هذا المثال البيانات التاريخية لسهم معيّن، ويستخدم الدالة `lag` لحساب الفرق اليومي والتغير النسبي في سعر إغلاق السهم.
