...
| إطار `GROUPS`                                                           | ✅       | تحسب حدود الإطار مجموعات الصفوف المتكافئة كاملةً (الصفوف المتساوية في مفتاح `ORDER BY`)؛ ويحسب `N PRECEDING` و`N FOLLOWING` عدد `N` من مجموعات الصفوف المتكافئة قبل وبعد مجموعة الصفوف المتكافئة للصف الحالي.                                                                                                                                                                             |
...
| [`ntile(buckets)`](/ar/reference/functions/window-functions/ntile)         | ✅       | حدِّد النافذة، على سبيل المثال، على النحو التالي: `(PARTITION BY x ORDER BY y ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING)`.                                                                                                                                                                                                                                                 |
...
* `ROWS` و`RANGE` و`GROUPS` - يحدّد حدود الإطار، ويُحسَب aggregate&#95;function ضمن هذا الإطار. يحسب `ROWS` الصفوف الفعلية، ويحسب `RANGE` قيم `ORDER BY`، ويحسب `GROUPS` مجموعات الأقران (الصفوف المتساوية في مفتاح `ORDER BY`).
...
لا يمكن استخدام الدوال التالية إلا كدوال النافذة. معظمها دوال SQL قياسية، بينما تُعد `lagInFrame` و`leadInFrame` و`nonNegativeDerivative` امتدادات خاصة بـ ClickHouse.
...
| [`percent_rank()`](/ar/reference/functions/window-functions/percent_rank)                                                                      | احسب الترتيب النسبي للصف الحالي داخل القسم الخاص به، كقيمة بين 0 و1.                                       |
| [`cume_dist()`](/ar/reference/functions/window-functions/cume_dist)                                                                            | احسب التوزيع التراكمي لقيمة ضمن مجموعة من القيم.                                                           |
| [`lag(x, offset)`](/ar/reference/functions/window-functions/lag)                                                                               | أرجِع قيمة جرى تقييمها عند صف يسبق الصف الحالي بعدد صفوف الإزاحة داخل القسم.                               |
| [`lead(x, offset)`](/ar/reference/functions/window-functions/lead)                                                                             | أرجِع قيمة جرى تقييمها عند صف يلي الصف الحالي بعدد صفوف الإزاحة داخل القسم.                                |
...
| [`ntile(buckets)`](/ar/reference/functions/window-functions/ntile)                                                                             | قسِّم الصفوف المرتَّبة داخل القسم إلى العدد المحدد من المجموعات، وأرجِع رقم المجموعة الخاصة بالصف الحالي.  |
...
يحسب إطار `GROUPS` *مجموعات الأقران* كاملةً — وهي مجموعات من الصفوف المتساوية في مفتاح `ORDER BY` — بدلًا من الصفوف الفعلية (`ROWS`) أو قيم `ORDER BY` (`RANGE`). يحسب `N PRECEDING` و`N FOLLOWING` عدد `N` من مجموعات الأقران قبل مجموعة أقران الصف الحالي وبعدها، وتُحتسب جميع صفوف أي مجموعة أقران مُضمَّنة دائمًا.

يطبّق الاستعلام أدناه الحدود نفسها، `1 PRECEDING AND 1 FOLLOWING`، كما في إطارات `ROWS` و`RANGE` و`GROUPS`. يحتوي العمود `order` على قيم مكررة وغير متتالية، لذا تشمل الأوضاع الثلاثة صفوفًا مختلفة:
...
يفسّر كل وضع الحدود بصورة مختلفة:

* يحسب `ROWS` الصفوف الفعلية، لذا يقتصر الإطار على ثلاثة صفوف متجاورة كحد أقصى: الصف الحالي وصف واحد على كل جانب.
* يحسب `RANGE` قيم `order`، لذا يشمل `1 PRECEDING` و`1 FOLLOWING` الصفوف التي تقع قيمة `order` فيها ضمن مسافة 1 من قيمة الصف الحالي. وعند وجود فجوات بمقدار 10، لا يستوفي أي صف مجاور الشرط، لذلك لا يضم الإطار إلا الصفوف التي تشترك في قيمة `order` للصف الحالي.
* يحسب `GROUPS` مجموعات النظائر، لذا يشمل `1 PRECEDING` و`1 FOLLOWING` دائمًا المجموعات المتجاورة كاملةً، بغض النظر عن الفجوات بين قيم `order`.
