...
2. وإلا، إعداد بيانات الاعتماد الافتراضي المنطبق:
   * [`ai_function_text_default_credentials`](/ar/reference/settings/session-settings/ai-function#ai_function_text_default_credentials) لدوال النص (`aiGenerate` و`aiClassify` و`aiFilter` و`aiExtract` و`aiTranslate`);
...
يحسب تضمين كلٍّ من النصّين ويُرجع تشابه جيب التمام بين المتجهين
ضمن النطاق `[-1, 1]`: تعني `1` أن النصّين متطابقان دلاليًا، وتعني `0` أنهما غير مرتبطين،
وتعني القيم السالبة أنهما متعاكسان. وهو مكمّل `cosineDistance` للتضمينات
