...
### معلمات المجموعة المسماة {#named-collection-parameters}
...
### اختيار بيانات الاعتماد {#selecting-credentials}
...
### إعدادات على مستوى الاستعلام {#query-level-settings}
...
### تقييد مضيفات نقطة النهاية {#restricting-endpoint-hosts}
...
### أمان النقل (HTTP مقابل HTTPS) {#transport-security}
...
## الموفّرون المدعومون {#supported-providers}
...
<Note>
  هذه الدالة غير حتمية: إذ يمكن أن تُرجِع نتائج مختلفة للوسيطات نفسها.
</Note>
...
كما في `aiEmbed`، تُعدّ `model` وسيطة موضعية مطلوبة (قيمة ثابتة من نوع `String`) ولا يُقرأ من
...
<Note>
  هذه الدالة غير حتمية: فقد تُرجع نتائج مختلفة للوسيطات نفسها.
</Note>
