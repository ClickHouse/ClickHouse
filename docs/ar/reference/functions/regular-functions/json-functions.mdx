...
تُجري هذه الدوال مطابقة المفاتيح دون مراعاة حالة أحرف ASCII عند استخراج القيم من كائنات JSON.
...
تقبل الدوال `JSON_EXISTS` و`JSON_VALUE` و`JSON_QUERY` تعبير JSONPath
كوسيطة ثانية. يدعم ClickHouse مجموعة فرعية محدودة من JSONPath،
تستند بصورة عامة إلى معيار SQL/JSON ‏(ISO/IEC TR 19075-6)، وهي أضيق نطاقًا عمدًا من [RFC 9535](https://www.rfc-editor.org/rfc/rfc9535)
والتنفيذات الشائعة الأخرى. تُدرج عوامل التشغيل المدعومة أدناه.

يوجد التنفيذ المعتمد في
...
| البنية        | الوصف                                                                                                                                                    | المثال                            |
| ------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------- |
| `$`           | العنصر الجذر. يجب أن يبدأ كل مسار به.                                                                                                                    | `$`                               |
| `.key`        | الوصول إلى العضو بصيغة النقطة دون اقتباس. يمكن أن يبدأ المفتاح بأرقام أو أن يتكوّن منها فقط.                                                             | `$.store.book`, `$.1key`, `$.123` |
| `."key"`      | الوصول إلى العضو بصيغة النقطة مع اقتباس. استخدمه للمفاتيح التي تحتوي على مسافات أو أحرف خاصة. تُقبل علامات الاقتباس المزدوجة أو علامات الاقتباس الخلفية. | `$."foo bar"`                     |
| `[key]`       | الوصول إلى العضو بصيغة الأقواس دون اقتباس.                                                                                                               | `$[hello]`                        |
| `['key']`     | الوصول إلى العضو بصيغة الأقواس مع اقتباس. تُقبل علامات الاقتباس المفردة أو المزدوجة أو علامات الاقتباس الخلفية.                                          | `$['foo bar']`                    |
| `[n]`         | الوصول إلى عنصر في مصفوفة حسب الفهرس (يبدأ من 0).                                                                                                        | `$.items[0]`                      |
| `[n, m, ...]` | الوصول إلى عدة عناصر من المصفوفة.                                                                                                                        | `$.items[0, 2, 4]`                |
| `[n to m]`    | نطاق من عناصر المصفوفة. فهرس النهاية `m` غير شامل.                                                                                                       | `$.items[0 to 3]`                 |
| `[*]`         | حرف بدل يطابق جميع عناصر المصفوفة.                                                                                                                       | `$.items[*]`                      |

ملاحظات:

* يمكن دمج الفهارس والنطاقات داخل زوج الأقواس نفسه (على سبيل المثال، `[0 to 2, 4]`).
* لا يمكن دمج نطاقات الفهارس وحرف البدل داخل زوج الأقواس نفسه (على سبيل المثال، `[* , 0]` غير صالح).
* في النطاق `[n to m]`، يجب أن تكون البداية `n` أصغر من النهاية `m`، وإلا يُثار استثناء.
* الفهرس المفرد `[n]` مكافئ للنطاق &#96;[n to n+1].

<div id="unsupported-operators">
  ### العوامل غير المدعومة
</div>

عوامل JSONPath التالية، المتاحة في RFC 9535 أو في تطبيقات أخرى،
**غير** مدعومة في ClickHouse:

| بنية                  | الوصف                                                         |
| ----------------------- | ------------------------------------------------------------- |
| `..`                    | نزول تعاودي.                                                  |
| `?(@.expr)`             | تعبيرات التصفية.                                              |
| `[start:end:step]`      | صياغة الشرائح. استخدم `[n to m]` بدلًا منها.                  |
| `@`                     | مرجع العقدة الحالية.                                          |
| `[-n]`                  | فهارس المصفوفات السالبة (من النهاية).                         |
| Script expressions      | تعبيرات حسابية أو دوال داخل مسار.                             |
| `ON ERROR` / `ON EMPTY` | بنود SQL/JSON القياسية للتحكم في سلوك الخطأ والحالات الفارغة. |
...
تُحاط [الأعداد الصحيحة](/ar/reference/data-types/int-uint) بطول 64 بت أو أكثر (مثل `UInt64` أو `Int128`) بعلامات اقتباس افتراضيًا. ويحدّد [output&#95;format&#95;json&#95;quote&#95;64bit&#95;integers](/ar/reference/settings/formats/output-format#output_format_json_quote_64bit_integers) هذا السلوك.
تُستبدل القيم الخاصة `NaN` و `inf` بـ `null`. فعِّل الإعداد [output&#95;format&#95;json&#95;quote&#95;denormals](/ar/reference/settings/formats/output-format#output_format_json_quote_denormals) لإظهارها.
عند تحويل قيمة [Enum](/ar/reference/data-types/enum)، تُخرج الدالة اسمها.

انظر أيضًا:

* [output&#95;format&#95;json&#95;quote&#95;64bit&#95;integers](/ar/reference/settings/formats/output-format#output_format_json_quote_64bit_integers)
* [output&#95;format&#95;json&#95;quote&#95;denormals](/ar/reference/settings/formats/output-format#output_format_json_quote_denormals)
