...
يُصنّف النص المُدخل باستخدام نموذج Naive Bayes مع n-grams وتنعيم لابلاس.
يجب إعداد النموذج في ClickHouse قبل استخدامه.

**تفاصيل التنفيذ**

*الخوارزمية*

تستخدم خوارزمية تصنيف Naive Bayes مع [تنعيم لابلاس](https://en.wikipedia.org/wiki/Additive_smoothing) للتعامل مع n-grams التي لم تظهر سابقًا، وذلك بالاعتماد على احتمالات n-gram كما هو موضح [هنا](https://web.stanford.edu/~jurafsky/slp3/4.pdf).

*الميزات الرئيسية*

* تدعم n-grams بأي حجم.
* ثلاثة أوضاع للتقسيم إلى رموز:
  * `byte`: يعمل على بايتات خام. كل بايت يُعد رمزًا واحدًا.
  * `نقطة ترميز`: يعمل على القيم القياسية في Unicode المفكوكة من UTF-8. كل نقطة ترميز تُعد رمزًا واحدًا.
  * `رمز`: يقسم النص عند تتابعات المسافات البيضاء في Unicode (تعبير Regex ‏`\s+`). تكون الـ الرموز مقاطع فرعية غير محتوية على مسافات بيضاء؛ وتُعد علامات الترقيم جزءًا من الـ رمز إذا كانت ملاصقة له (على سبيل المثال، `you?` يُعد رمزًا واحدًا).

**إعداد النموذج**

تتوفر شيفرة مصدر نموذجية لإنشاء نموذج Naive Bayes لاكتشاف اللغة [هنا](https://github.com/nihalzp/ClickHouse-NaiveBayesClassifier-Models)، إلى جانب نماذج تجريبية وملفات الإعداد المرتبطة بها [هنا](https://github.com/nihalzp/ClickHouse-NaiveBayesClassifier-Models/tree/main/models).

مثال على إعداد نموذج Naive Bayes في ClickHouse:
...
| --------- | ------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------- | ------------ |
| `name`    | معرّف فريد للنموذج.                                                                                           | `language_detection`                                     | مطلوب        |
| `path`    | المسار الكامل إلى الملف الثنائي للنموذج.                                                                      | `/etc/clickhouse-server/config.d/language_detection.bin` | مطلوب        |
| `mode`    | طريقة تقسيم النص إلى رموز: ‏`byte` (تسلسلات بايت)، أو `نقطة ترميز` (محارف Unicode)، أو `رمز` (رموز الكلمات). | `token`                                                  | مطلوب        |
| `n`       | حجم N-gram: ‏`1` (كلمة واحدة)، أو `2` (أزواج كلمات)، أو `3` (ثلاثيات كلمات).                                  | `2`                                                      | مطلوب        |
| `alpha`   | عامل تمهيد لابلاس المستخدم أثناء التصنيف لسلاسل n-grams التي لا تظهر في النموذج.                              | `0.5`                                                    | `1.0`        |
| `priors`  | احتمالات الفئات (النسبة المئوية للمستندات التي تنتمي إلى كل فئة).                                             | 60% للفئة 0، و40% للفئة 1                                | توزيع متساوٍ |

**دليل تدريب النموذج**

*تنسيق الملف*

في تنسيق مقروء بشريًا، عند `n=1` وفي وضع `رمز`، قد يبدو النموذج كما يلي:
...
بالنسبة إلى `n=3` ووضع `نقطة ترميز`، قد يكون الشكل كما يلي:
...
لا يستخدم ClickHouse الصيغة المقروءة بشريًا مباشرةً؛ بل يجب تحويلها إلى التنسيق الثنائي الموضّح أدناه.

*تفاصيل التنسيق الثنائي*

يُخزَّن كل `n-gram` على النحو التالي:
...
قبل إنشاء النموذج من مجموعة المستندات، يجب إجراء المعالجة المسبقة على المستندات لاستخراج `n-grams` وفقًا لـ `mode` و`n` المحدَّدين:

1. أضِف علامات حدودية في بداية كل مستند ونهايته بناءً على وضع التجزئة إلى رموز:

   * `byte`: `0x01` (البداية)، `0xFF` (النهاية)
   * `codepoint`: `U+10FFFE` (البداية)، `U+10FFFF` (النهاية)
   * `token`: `<s>` (البداية)، `</s>` (النهاية)

   ملاحظة: تُضاف `(n - 1)` من الرموز في كلٍّ من بداية المستند ونهايته.

2. مثال على `n=3` في وضع `token`:
   * المستند: `ClickHouse is fast`
   * تتم معالجته على النحو التالي: `<s> <s> ClickHouse is fast </s> </s>`
   * الثلاثيات المتولدة:
     * `<s> <s> ClickHouse`
...
لتبسيط إنشاء النموذج لوضعي `byte` و`codepoint`، قد يكون من الملائم أولًا تجزئة المستند إلى رموز (قائمة من البايتات في وضع `byte`، وقائمة من نقاط الترميز في وضع `codepoint`)، ثم إلحاق `n - 1` من رموز البداية في أوله و`n - 1` من رموز النهاية في آخره، ثم إنشاء `n-grams` وكتابتها إلى الملف المتسلسل.
...
* `model_name` — اسم النموذج المُعَدّ مسبقًا. يجب تعريف النموذج في ملفات تكوين ClickHouse. [`String`](/ar/reference/data-types/string)
* `input_text` — النص المراد تصنيفه. تُعالَج المُدخلات تمامًا كما قُدِّمت (مع الحفاظ على حالة الأحرف/علامات الترقيم). [`String`](/ar/reference/data-types/string)

**القيمة المُعادة**

معرّف الفئة المتوقعة كعدد صحيح غير موقّع. تتوافق معرّفات الفئات مع التصنيفات المحددة أثناء إنشاء النموذج. [`UInt32`](/ar/reference/data-types/int-uint)
