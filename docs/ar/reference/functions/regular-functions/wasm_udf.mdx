...
* `wasmtime` (الافتراضي، والوحيد حاليًا) — يستخدم [Wasmtime](https://github.com/bytecodealliance/wasmtime)
...
## إدارة وحدات WASM عبر جدول النظام {#manage-wasm-modules-via-system-table}
...
### توزيع وحدة عبر عنقود {#distribute-a-module-across-a-cluster}
...
## إنشاء دالة UDF باستخدام WebAssembly {#create-a-webassembly-udf}
...
* `DETERMINISTIC`: يصرّح بأن الدالة حتمية — أي إنها تعيد دائمًا الناتج نفسه للمدخلات نفسها. عند تحديد ذلك، قد يطوي ClickHouse الاستدعاءات الثابتة التي تكون فيها جميع الوسيطات ثوابت: تُقيَّم الدالة مرة واحدة في مرحلة تحليل الاستعلام، ثم يُعاد استخدام النتيجة لكل صف.
...
* `SETTINGS`: إعدادات خاصة بكل دالة
  * `serialization_format` String — التنسيق المستخدم لتسلسل كتل الوسيطات المُمرّرة إلى الوحدة وتحليل النتيجة المُعادة. يُستخدم فقط بواسطة `ABI BUFFERED_V1`. القيم المدعومة: `MsgPack` و`JSONEachRow` و`CSV` و`TSV` و`TSVRaw` و`RowBinary` و`Buffers`. القيمة الافتراضية: `MsgPack`. يجب أن تُرجع التنسيقات المعتمدة على الكتل مثل `Buffers` عمودًا واحدًا يطابق نوعه توقيع الدالة المُصرَّح به.
...
<Note>
  واجهة ABI هذه تجريبية وقد تتغير في الإصدارات المستقبلية.
</Note>

يعالج كتلًا كاملة دفعةً واحدة باستخدام (إلغاء) التسلسل عبر ذاكرة WASM. ويدعم أي أنواع للوسيطات وأنواع الإرجاع.

يُتبادل البيانات عبر مخازن مؤقتة في ذاكرة WASM. المخزن المؤقت بنية بحجم 8 بايتات تحتوي على مؤشر إلى البيانات وحجمها (قيمتا `u32` بترتيب little-endian). تُمرَّر المخازن المؤقتة باستخدام مقابض، أي مؤشرات إلى هذه البنية لا إلى البيانات نفسها. يجب أن تصدّر شيفرة الضيف دالتين لإنشاء هذه المخازن المؤقتة وإتلافها. وعندما يكون الحجم غير صفري، يجب أن يكون مؤشر البيانات غير صفري أيضًا: فالمخزن المؤقت الذي يعلن حجمًا موجبًا لكنه يشير إلى الإزاحة `0` في الذاكرة الخطية ليس مخزنًا مؤقتًا صالحًا، ويجب ألا تنتج شيفرة الضيف مثل هذا المخزن؛ إذ يرفض ClickHouse مخزنًا مؤقتًا كهذا ويُفشل الاستعلام بخطأ `WASM_ERROR`.
...
تُستدعى الدالة مرة واحدة لكل كتلة إدخال: يُقسَّم الاستعلام الكبير إلى عدة كتل بواسطة مسار تنفيذ الاستعلام، لذا فإن عدد الصفوف المُمرَّر في كل استدعاء هو حجم تلك الكتلة، وليس حجم الاستعلام بأكمله.

ومع ذلك قد تظل كتلة مسار التنفيذ أوسع من أن تتسع في الذاكرة الخطية للوحدة، لذا تُقسَّم أكثر. افتراضيًا يقسّمها `webassembly_udf_input_split_memory_ratio` ديناميكيًا: يقيس ClickHouse الصفوف المتسلسلة ويبدأ استدعاءً جديدًا قبل أن يتجاوز الإدخال تلك النسبة من الذاكرة الخطية للضيف، لذا يتغير عدد الصفوف بتغير البيانات. أما القيمة غير الصفرية للإعداد `webassembly_udf_max_input_block_size` فتستبدل بذلك حدًا ثابتًا بهذا العدد من الصفوف لكل استدعاء، بينما تمرر النسبة `0` الكتلة بأكملها في استدعاء واحد. ولا يُخفَّض الاستدعاء أبدًا إلى أقل من صف واحد، لذا فإن الصف الأكبر من أن تتسع له ذاكرة الوحدة بمفرده سيظل يفشل داخل الضيف.

<Note>
  تحدد التنسيقات المختارة البايتات الدقيقة داخل المخازن المؤقتة، ويجب أن تفك شيفرة الضيف ترميزها وتعيد ترميزها بنفسها. وبشكل خاص، بالنسبة إلى قيم `String`:

  * `MsgPack`: يكتب ClickHouse قيم `String` باستخدام عائلة الأنواع `bin` (`0xc4`/`0xc5`/`0xc6`) وليس عائلة `str`. تُقبل كلتا العائلتين عند تحليل النتيجة.
  * `RowBinary`: تُسبق كل قيمة `String` بطولها بالبايتات على هيئة varint غير موقّع ([LEB128](https://en.wikipedia.org/wiki/LEB128))، وليس عددًا صحيحًا بعرض ثابت.
  * `JSONEachRow`: يجب أن يكون كل صف نتائج كائنًا يتضمن المفتاح `result`، مثلًا `{"result":"value"}`.
</Note>
...
مثال كامل بلغة C في بيئة freestanding: تعكس الدالة `str_reverse` بايتات كل سلسلة إدخال، باستخدام `serialization_format = 'RowBinary'`. يُعاد استخدام نسخ الوحدة عبر الكتل، لذا يجب أن تستعيد `clickhouse_destroy_buffer` الذاكرة فعليًا — وهنا يُعاد ضبط المُخصِّص بمجرد تدمير جميع المخزنات المؤقتة.
...
أنشئ باستخدام clang و`wasm-ld` (المرفق مع LLVM/lld):
...
### ملاحظة حول تطوير UDFs في Rust {#note-for-developing-udfs-in-rust}
...
## واجهة برمجة التطبيقات المضيفة المتاحة للوحدات {#host-api-available-to-modules}
...
* `webassembly_udf_max_input_block_size` — الحد الأقصى لعدد الصفوف التي تُمرَّر إلى WebAssembly UDF في كتلة واحدة، وذلك لكل ABI. عيّنه إلى 0 ليتولى `webassembly_udf_input_split_memory_ratio` تحديد حجم استدعاءات دالة من نوع `ABI BUFFERED_V1`؛ أما أنواع ABI الأخرى فتستقبل كتلة مسار التنفيذ بالكامل.

* `webassembly_udf_input_split_memory_ratio` — نسبة الذاكرة الخطية التي تُعلن الوحدة أنها تبدأ بها (أو من سقف ذاكرتها، في حالة وحدة تبدأ بدون أي صفحات) والتي يمكن أن يشغلها إدخال استدعاء واحد. تُستخدم لتقسيم كتل دالة من نوع `ABI BUFFERED_V1` عندما تكون قيمة `webassembly_udf_max_input_block_size` هي 0؛ أما أنواع ABI الأخرى فلا تحمل كتلة إدخال مُسلسَلة ولذلك تتجاهل هذا الإعداد. القيمة الافتراضية: `0.5`، بما يترك بقية الذاكرة لمجموعة العمل الخاصة بالضيف — فالنسبة القريبة من 1 قد تفشل داخل مُخصِّص ذاكرة الضيف. عيّنه إلى 0، أو عيّن `compatibility` إلى إصدار أسبق من 26.9، لتمرير كتلة مسار التنفيذ بالكامل في استدعاء واحد. ويجب أن تكون القيمة 0 على الأقل و1 كحد أقصى.
