...
`unnest` (منذ الإصدار 26.5) اسم مستعار لـ `arrayJoin` غير حساس لحالة الأحرف عند استخدامه بصيغة استدعاء الدالة (`SELECT unnest(arr)`). لا تُدعم صياغة PostgreSQL لمصدر الجدول (`FROM unnest(...)` و`CROSS JOIN UNNEST(...)` و`LATERAL`). استخدم عبارة [`ARRAY JOIN`](/ar/reference/statements/select/array-join) لهذه الاستعلامات.
...
ثمة استثناء واحد: لا يُسمح باستخدام `arrayJoin`، بما في ذلك عبر الاسم المستعار `unnest`، في شرط `JOIN ON` يُقيَّم أثناء عملية JOIN، لأن هذا الشرط يجب أن يحافظ على عدد الصفوف ويُقيَّم بصورة منفصلة لكل دفعة من الصفوف المربوطة. يُستخرج قبل JOIN الشرط الذي ينطبق على جانب واحد فقط، وكذلك مفتاح مساواة يعتمد على `arrayJoin`، ولا يتأثر أيٌّ منهما. كذلك لا يتأثر شرط `ALL INNER JOIN` غير المنفصل، لأن الشرط يُطبَّق فيه بعد JOIN بدلًا من ذلك. عندما يعتمد التوسيع على جانب واحد فقط، انقله إلى `ARRAY JOIN` في استعلام فرعي قبل JOIN؛ أما الشرط الذي يقرأ فيه وسيط `arrayJoin` أعمدة من كلا الجانبين، فيجب إعادة هيكلته.
