...
* `MVTEncodeGeom` — دالة قياسية تُسقِط هندسةً إلى فضاء البكسلات المحلي للبلاطة ضمن slippy-map وتقصّه
  إلى حدود البلاطة.
* `MVTEncode` — دالة تجميعية تجمع الهندسات المُسقطة لمجموعة ما في بايتات ثنائية
...
تدعم هذه الدوال الهندسات النقطية والخطية والمضلعة، بما في ذلك النوع `Geometry` والأنواع الجغرافية المحددة (`Point`,
...
يحوِّل هندسةً مُعطاةً بالإحداثيات الجغرافية (خط الطول/دائرة العرض) إلى فضاء البكسلات المحلي للبلاطة في
بلاطة `slippy-map` المحددة بواسطة `zoom` و`tile_x` و`tile_y`، ثم يطابقها مع شبكة
البكسلات الصحيحة، ويقصّها إلى حدود البلاطة، ويُرجع الهندسة في `tile-space`.
...
`MVTEncodeGeom` يدمج الهندسات الواقعة على الشبكة نفسها في عنقود واحد.

عندما يكون `clip` مفعّلاً (وهو الإعداد الافتراضي)، تُقصّ الهندسة إلى البلاطة الموسَّعة بمقدار `buffer` من البكسلات (النطاق
`[-buffer, extent + buffer]` على كل محور)؛ أما الهندسة التي تقع بالكامل خارجها فتصبح `NULL`. وهذا هو النظير لـ
...
`zoom` 18 و`extent` 4096 — لذلك، بالنسبة إلى البلاطات الواقعية، يُتحقَّق من صحة الهندسة من دون أن تُقصّ مطلقاً، ولا يؤثر هذا القيد إلا في
الهندسات الموضوعة عند قيم `zoom` أو `extent` القصوى.

يعتمد نوع الهندسة الناتجة على المُدخل: إذ يُرجع `Point` قيمة `Point`؛ ويُرجع `MultiPoint` قيمة `MultiPoint`؛ ويُرجع `LineString` أو `MultiLineString`
قيمة `MultiLineString`؛ وتُرجع `Ring` أو `Polygon` أو `MultiPolygon` قيمة `MultiPolygon` (إذ قد يؤدي القص إلى تقسيم الهندسة إلى
عدة أجزاء).
...
* `buffer` — هامش القص الاختياري بالبكسل، ضمن النطاق `[0, 2147483647]`. القيمة الافتراضية هي `1`. [`UInt32`](/ar/reference/data-types/int-uint).
* `clip` — علامة اختيارية؛ عند كونها غير صفرية (وهو الإعداد الافتراضي)، تُقص الهندسة إلى حدود البلاطة مع الهامش. [`UInt8`](/ar/reference/data-types/int-uint).

**القيمة المعادة**

يعيد الهندسة في `tile-space`، أو `NULL` إذا جرى قصّها بالكامل. [`Geometry`](/ar/reference/data-types/geo).
