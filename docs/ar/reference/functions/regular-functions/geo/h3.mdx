...
يُرجع السداسيات (عند مستوى الدقة المحدد) الواقعة ضمن الشكل الهندسي المُقدَّم، سواء كان حلقة أو مضلعًا (أو متعدد المضلعات). يُعد السداسي واقعًا ضمن الشكل عندما يحتوي الشكل الهندسي على مركزه، وهو وضع الاحتواء `0` في [h3PolygonToCellsWithContainment](#h3polygontocellswithcontainment).

يجب أن تقع كل قمة من قمم الشكل الهندسي على الكرة: ضمن نطاق خط الطول `[-180, 180]` وخط العرض `[-90, 90]` بالدرجات. لا يُضمن ترتيب الخلايا المُعادة، لذا رتّب المصفوفة عندما يكون الترتيب مهمًا.
...
يجب أن تكون كل قمة في الشكل الهندسي على الكرة: خط الطول ضمن النطاق `[-180, 180]` وخط العرض ضمن النطاق `[-90, 90]` درجة. لا يُضمن ترتيب الخلايا المُعادة، لذا رتّب المصفوفة عندما يكون الترتيب مهمًا.
...
تعيد المسافة، بوحدة خلايا الشبكة، بين الفهرسين المُقدَّمين.
...
يعيد فهرس H3 لحافة أحادية الاتجاه لفهرسي خليتي H3 متجاورتين (الأصل والوجهة). يطرح استثناءً إذا لم يكن أيٌّ من الإدخالين خلية H3 صالحة (يتحكم في ذلك الإعداد `functions_h3_default_if_invalid`). بالنسبة إلى الخلايا الصالحة غير المتجاورة (بما في ذلك الخلية نفسها مرتين)، يعيد `0`.
...
يعيد مؤشر السداسي الأصلي من H3Index الخاص بالحافة أحادية الاتجاه. يطرح استثناءً إذا لم يكن الإدخال حافة موجهة صالحة (يتحكم في ذلك الإعداد `functions_h3_default_if_invalid`)؛ ولا يعيد `0` إلا عند تمكين هذا الإعداد.
...
* رقم فهرس الخلية السداسية للمنشأ. [UInt64](/ar/reference/data-types/int-uint).
* يطرح `INCORRECT_DATA` للحواف الموجّهة غير الصالحة افتراضياً؛ ويعيد `0` عندما يكون `functions_h3_default_if_invalid = 1`.
...
يعيد فهرس الخلية السداسية للوجهة من H3Index للحافة أحادية الاتجاه. يطرح استثناءً إذا لم يكن الإدخال حافة موجهة صالحة (يتحكم في ذلك الإعداد `functions_h3_default_if_invalid`)؛ ولا يعيد `0` إلا عند تمكين هذا الإعداد.
...
* يُطلق `INCORRECT_DATA` عند وجود حواف موجّهة غير صالحة افتراضيًا؛ ويعيد `0` عندما يكون `functions_h3_default_if_invalid = 1`.
...
يطرح استثناءً إذا كانت قيمة الإدخال المقدَّمة ليست حافة موجَّهة صالحة (يتحكم في ذلك الإعداد `functions_h3_default_if_invalid`)؛ ولا يُرجِع `(0,0)` إلا عند تمكين ذلك الإعداد.
...
يوفّر جميع الحواف أحادية الاتجاه الخارجة من قيمة H3Index المقدَّمة. يُطلق استثناءً إذا لم يكن الإدخال خلية H3 صالحة (يتحكم في ذلك الإعداد `functions_h3_default_if_invalid`)؛ ولا يعيد مصفوفة فارغة إلا عند تفعيل هذا الإعداد.
...
* `index` — رقم فهرس الخلية السداسية الذي يمثّل خلية. [UInt64](/ar/reference/data-types/int-uint).
...
* يُطلق الخطأ `INCORRECT_DATA` للخلايا غير الصالحة افتراضيًا؛ ويعيد `[]` عندما يكون `functions_h3_default_if_invalid = 1`.
...
تُرجِع الإحداثيات التي تُعرِّف الحافة أحادية الاتجاه. تطرح استثناءً إذا لم يكن الإدخال حافة موجّهة صالحة (يتحكم في ذلك الإعداد `functions_h3_default_if_invalid`)؛ ولا تُرجِع مصفوفة فارغة إلا عندما يكون هذا الإعداد مُفعّلًا.
...
* [مصفوفة](/ar/reference/data-types/array) من الأزواج &#39;(lat, lon)&#39;. ([Float64](/ar/reference/data-types/float), [Float64](/ar/reference/data-types/float)).
* يُطلق `INCORRECT_DATA` للحواف الموجّهة غير الصالحة افتراضيًا؛ ويُرجع `[]` عندما يكون `functions_h3_default_if_invalid = 1`.
