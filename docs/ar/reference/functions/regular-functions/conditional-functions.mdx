...
### استخدام النتائج الشرطية مباشرةً {#using-conditional-results-directly}
...
### قيم `NULL` في التعبيرات الشرطية {#null-values-in-conditionals}
...
<Note title="استخدم الإعداد `least_greatest_legacy_null_behavior` لتغيير سلوك `NULL`">
...
تُرجِع أكبر قيمة بين الوسائط، بعد ترقيتها إلى أكبر نوع متوافق. [`Any`](/ar/reference/data-types/index)
...
يتحكّم الإعداد [`short_circuit_function_evaluation`](/ar/reference/settings/session-settings/short-circuit-function-evaluation#short_circuit_function_evaluation) في ما إذا كان التقييم المختصر مستخدمًا.
...
يتحكم الإعداد [`short_circuit_function_evaluation`](/ar/reference/settings/session-settings/short-circuit-function-evaluation#short_circuit_function_evaluation) في
