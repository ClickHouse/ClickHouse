...
## ملاحظات حول دوال `toString` {#to-string-functions}
...
## ملاحظات حول الدالتين `toDate`/`toDateTime` {#to-date-and-date-time-functions}
...
## kqlParameterCast {#kqlParameterCast}

أُضيف في: v26.9.0

يتحقق من قيمة ما مقابل نوع Kusto Query Language مُصرَّح به ويحوّلها إلى الحامل المقابل لذلك النوع في
ClickHouse: فأي argument لا ينتمي نوعه إلى نوع KQL المُصرَّح به يُرفض أثناء التحليل، أما القيمة التي لا
تتسع (overflow في `int` مثلاً) فتُعدّ error على مستوى كل row. الوسيط الثاني هو اسم نوع KQL المُصرَّح به، والثالث يصف
الـ parameter في رسالة الخطأ؛ ويجب أن يكون كلاهما سلاسل نصية ثابتة.

تدعم هذه الدالة معاملات الدوال ذات الأنواع المحددة عندما يكون `dialect = 'kusto'`، وهي ليست مخصصة
للاستدعاء المباشر من SQL.

**الصيغة**

```sql
kqlParameterCast(value, type, parameter)
```

**الوسيطات**

* `value` — الوسيط الذي يمرره استدعاء دالة KQL. - `type` — اسم نوع KQL المُعلَن، وهو constant string. - `parameter` — وصف الـ parameter، وهو constant string يُستخدم في رسالة الـ error.

**القيمة المُعادة**

`value` على هيئة carrier الخاص بـ ClickHouse للنوع المُعلَن.
...
```sql title=Query
SELECT kqlParameterCast(5, 'real', 'the parameter') AS x, toTypeName(x)
```
...
## kqlToTimespan {#kqlToTimespan}

أُضيف في: v26.8.0

يحوّل قيمةً إلى timespan (أي `Interval` بالنانو ثانية) تمامًا كما تفعل الدالة
`totimespan()` في Kusto Query Language: فقيمة الـ timespan تمرّ كما هي، والرقم يُحتسب أيامًا، والسلسلة النصية تُقرأ بالصيغة
`[-][d.]hh:mm:ss[.fffffff]` — وتُعاد `NULL` عند تعذّر تحليلها.

تستند `totimespan()` إلى هذه الدالة عندما يكون `dialect = 'kusto'`، وهي غير مخصّصة للاستدعاء
مباشرةً من SQL.
...
* `x` — قيمة timespan، أو عدد من الأيام، أو سلسلة نصية تمثّل قيمة timespan.

**القيمة المُعادة**

القيمة على هيئة interval بالنانوثانية؛ أو `NULL` إذا كانت السلسلة النصية لا تمثّل قيمة timespan.
...
إذا كانت القيمة خارج النطاق `[0000-01-01, 9999-12-31]`، فإن `toDate32` يعيد القيم الحدّية التي يدعمها [Date32](/ar/reference/data-types/date32).
...
يحوِّل الوسيط إلى نوع البيانات [Date32](/ar/reference/data-types/date32). إذا تعذّر تحويل الوسيط، بما في ذلك القيمة الرقمية الواقعة خارج نطاق [Date32](/ar/reference/data-types/date32)، فإن الدالة تُرجع قيمة افتراضية تكون إما الوسيط الثاني (إذا تم تحديده)، أو `1900-01-01` بخلاف ذلك، وهي القيمة الافتراضية التاريخية لـ [Date32](/ar/reference/data-types/date32) (وليست حدها الأدنى، وهو `0000-01-01`). وإذا كان الوسيط من النوع [Date](/ar/reference/data-types/date)، فتُؤخذ حدوده في الاعتبار.

لاحظ أنه، بخلاف [toDate32](#toDate32)، لا تقيِّد هذه الدالة الوسيط الرقمي الواقع خارج النطاق ضمن حدود [Date32](/ar/reference/data-types/date32)؛ بل يُعامل هذا الوسيط على أنه غير قابل للتحويل.
...
يحوّل قيمة الإدخال إلى قيمة من النوع [Date32](/ar/reference/data-types/date32)، لكنه يعيد القيمة الافتراضية `1900-01-01` إذا كانت الوسيطة غير صالحة.
وهو مماثل لـ [toDate32](#toDate32)، لكنه يعيد `1900-01-01` إذا كانت الوسيطة غير صالحة.
لاحظ أن `1900-01-01` قيمة افتراضية تاريخية للإدخال غير الصالح، وليست الحد الأدنى لـ [Date32](/ar/reference/data-types/date32)، وهو `0000-01-01`.
...
تُعيد قيمة `Date32` عند النجاح، وإلا تُعيد القيمة الافتراضية التاريخية للإدخال غير الصالح (`1900-01-01`). [`Date32`](/ar/reference/data-types/date32)
...
<Note>
  إذا كانت `expr` رقمًا، فسيُفسَّر على أنه عدد الثواني منذ بداية حقبة Unix (أي كـ طابع زمني لـ Unix).
  إذا كانت `expr` من النوع [String](/ar/reference/data-types/string)، فقد تُفسَّر على أنها طابع زمني لـ Unix أو على أنها تمثيل نصي لتاريخ أو لتاريخ مع وقت.
  لذلك، يُعطَّل صراحةً تحليل التمثيلات النصية للأرقام القصيرة (حتى 4 أرقام) بسبب الالتباس؛ فعلى سبيل المثال، قد تكون السلسلة `'1999'` سنةً (تمثيلًا نصيًا غير مكتمل لـ Date / DateTime) أو طابع زمني لـ Unix. ويُسمح بالسلاسل الرقمية الأطول.
</Note>

إذا كانت `expr` قيمة من النوع [Time](/ar/reference/data-types/time) أو [Time64](/ar/reference/data-types/time64)، فسيُفسَّر على أنه عدد الثواني منذ بداية حقبة Unix: ويُقرَّب الجزء الكسري من `Time64` نزولًا نحو سالب اللانهاية، أما القيم الواقعة خارج نطاق `DateTime` فتتبع الإعداد `date_time_overflow_behavior`.
...
* `expr` — القيمة. [`String`](/ar/reference/data-types/string) أو [`Int`](/ar/reference/data-types/int-uint) أو [`Date`](/ar/reference/data-types/date) أو [`Date32`](/ar/reference/data-types/date32) أو [`Time`](/ar/reference/data-types/time) أو [`Time64`](/ar/reference/data-types/time64) أو [`DateTime`](/ar/reference/data-types/datetime) أو [`DateTime64`](/ar/reference/data-types/datetime64)
...
يدعم التحويل من `String` أو `FixedString` أو `Date` أو `Date32` أو `Time` أو `Time64` أو `DateTime` أو `DateTime64` أو الأنواع الرقمية (`(U)Int8` أو `(U)Int16` أو `(U)Int32` أو `(U)Int64` أو `(U)Int128` أو `(U)Int256` أو `BFloat16` أو `Float32` أو `Float64`). لا تُدعم قيم `Decimal` وتؤدي إلى استثناء.
`DateTime32` اسم مستعار لـ `DateTime` ويدعم التواريخ من `1970-01-01 00:00:00` إلى `2106-02-07 06:28:15`.
عند التحويل من سلسلة أو من أحد الأنواع الرقمية القابلة للإشباع المذكورة أعلاه، تُقيَّد القيم الواقعة خارج هذا النطاق بحدوده. لا يمكن تحويل قيم الفاصلة العائمة غير المنتهية (`NaN` و`inf` و`-inf`) وتؤدي إلى استثناء.
تتبع التحويلات من أنواع التاريخ والوقت الأخرى، مثل `Date32` أو `DateTime64` أو `Time` أو `Time64`، الإعداد `date_time_overflow_behavior`: فقد ينتج عن وضع `ignore` الافتراضي قيم ملتفة، بينما يقيّد `saturate` القيم بحدود النطاق، ويطرح `throw` استثناءً. تُقرَّب قيم `Time64` أولاً نزولاً نحو ما لا نهاية السالبة إلى ثوانٍ كاملة.
...
* `x` — قيمة الإدخال المراد تحويلها. [`String`](/ar/reference/data-types/string) أو [`FixedString`](/ar/reference/data-types/fixedstring) أو [`(U)Int*`](/ar/reference/data-types/int-uint) أو [`Float*`](/ar/reference/data-types/float) أو [`BFloat16`](/ar/reference/data-types/float) أو [`Date`](/ar/reference/data-types/date) أو [`Date32`](/ar/reference/data-types/date32) أو [`Time`](/ar/reference/data-types/time) أو [`Time64`](/ar/reference/data-types/time64) أو [`DateTime`](/ar/reference/data-types/datetime) أو [`DateTime64`](/ar/reference/data-types/datetime64)
...
يحوّل قيمة الإدخال إلى قيمة من النوع [`DateTime64`](/ar/reference/data-types/datetime64).
...
* `expr` — تعبير يُرجع رقمًا أو تمثيلًا نصيًا لرقم. [`تعبير`](/ar/reference/data-types/special-data-types/expression)
* `scale` — حجم الـ tick (الدقة): 10^(-scale) ثانية. [`UInt8`](/ar/reference/data-types/int-uint)
...
يحوِّل قيمة الإدخال إلى قيمة من النوع [DateTime64](/ar/reference/data-types/datetime64)، لكنه يعيد قيمة [DateTime64](/ar/reference/data-types/datetime64) الصفرية (حقبة Unix، `1970-01-01 00:00:00`) إذا استلم وسيطة غير صالحة.
وهو مماثل لـ [toDateTime64](#toDateTime64)، لكنه يعيد قيمة [DateTime64](/ar/reference/data-types/datetime64) الصفرية إذا استلم وسيطة غير صالحة.
...
```sql
toDateTime64OrZero(x[, precision[, timezone]])
```

**الوسيطات**

* `x` — تمثيل نصي لتاريخ ووقت بدقة أجزاء من الثانية. [`String`](/ar/reference/data-types/string)
* `precision` — اختياري. دقة أجزاء الثانية للقيمة المعادة. [`UInt8`](/ar/reference/data-types/int-uint)
* `timezone` — اختياري. المنطقة الزمنية للقيمة المعادة. [`String`](/ar/reference/data-types/string)

**القيمة المعادة**

تعيد قيمة من النوع DateTime64 عند النجاح، وإلا فتعيد قيمة DateTime64 الصفرية (`1970-01-01 00:00:00`) بالدقة المطلوبة. [`DateTime64`](/ar/reference/data-types/datetime64)
...
يدعم التحويل من String وFixedString وDateTime وDateTime64 وTime64 أو من الأنواع الرقمية التي تمثل عدد الثواني منذ منتصف الليل.
تُقيَّد القيم الرقمية الواقعة خارج نطاق النوع (`-999:59:59` إلى `999:59:59`، أي `-3599999` إلى `3599999` ثانية) بحدود النطاق، ولا يمكن تحويل قيم الفاصلة العائمة غير المنتهية (`NaN` و`inf` و`-inf`) وينتج عن ذلك استثناء.
تُقرَّب قيم `Time64` نزولًا نحو اللانهاية السالبة إلى ثوانٍ كاملة وتُقيَّد بالنطاق نفسه.

<Note title="`toTime` القديم">
  قبل v25.5، كانت `toTime` دالة مختلفة تحوّل تاريخًا مع وقت إلى تاريخ ثابت (`1970-01-02`) مع الحفاظ على
  مكوّن الوقت. ولا تزال تلك الدالة متاحة بالاسم [`toTimeWithFixedDate`](/ar/reference/functions/regular-functions/date-time-functions#toTimeWithFixedDate).

  يؤدي أيضًا ضبط [`use_legacy_to_time`](/ar/reference/settings/session-settings#use_legacy_to_time) على `1` إلى إبقاء الاسم `toTime` مرتبطًا بالدالة القديمة،
  أي إن استدعاءات `toTime` تُفسَّر على أنها `toTimeWithFixedDate` بدلًا من دالة التحويل الموضحة هنا.
  أثناء تمكين الإعداد، استخدم `CAST(x AS Time)` أو `x::Time` للتحويل إلى النوع `Time`.
  تكون القيمة الافتراضية للإعداد `0` منذ v26.7، لكنها كانت `1` من v25.6 إلى v26.6، ولهذا تضبطها الأمثلة أدناه صراحةً.
</Note>
...
* `x` — قيمة الإدخال المراد تحويلها. [`String`](/ar/reference/data-types/string) أو [`FixedString`](/ar/reference/data-types/fixedstring) أو [`DateTime`](/ar/reference/data-types/datetime) أو [`DateTime64`](/ar/reference/data-types/datetime64) أو [`Time64`](/ar/reference/data-types/time64) أو [`(U)Int*`](/ar/reference/data-types/int-uint) أو [`BFloat16`](/ar/reference/data-types/float) أو [`Float*`](/ar/reference/data-types/float)

**القيمة المعادة**

تُرجِع القيمة المُحوَّلة. [`Time`](/ar/reference/data-types/time)
...
يشبه [`toTime64`](#toTime64)، لكنه يعيد `NULL` بدلًا من طرح استثناء عند حدوث أخطاء في التحويل.
...
```sql
toTime64OrNull(x[, precision])
```

**الوسيطات**

* `x` — تمثيل نصي لوقت بدقة أقل من الثانية. [`String`](/ar/reference/data-types/string)
* `precision` — اختياري. دقة أقل من الثانية للقيمة المُعادة. [`UInt8`](/ar/reference/data-types/int-uint)
...
يحوِّل قيمة الإدخال إلى قيمة من النوع `Time64`، لكنه يُرجع قيمة `Time64` الصفرية (`00:00:00`) في حال استلام وسيطة غير صالحة.
يشبه [`toTime64`](#toTime64)، لكنه يُرجع قيمة `Time64` الصفرية في حال استلام وسيطة غير صالحة.
...
تُرجِع قيمة من النوع `Time64` عند النجاح، وإلا القيمة الصفرية من النوع Time64 (`00:00:00`) بالدقة المطلوبة. [`Time64`](/ar/reference/data-types/time64)
