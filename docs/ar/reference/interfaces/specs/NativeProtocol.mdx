...
| 26 | الأدوار&#95;الحالية                   | UInt8 [+ قائمة String] | بين الخوادم | INTERSERVER&#95;CURRENT&#95;ROLES (v54488)                | حقل ختامي. `[UInt8 present]`، ثم، عندما تكون `present = 1`، قائمة `[VarUInt count][String]*count` بأسماء الأدوار النشطة (المُمكَّنة) لدى initiator. يتيح لعقدة ثانوية تقييد سياسات الصفوف بأدوار initiator بدلًا من الأدوار default للمستخدم. لا يُملأ إلا للاستعلامات بين الخوادم وفقط عندما تكون `push_external_roles_in_interserver_queries = 1`؛ وإلا، وبالنسبة إلى العملاء الخارجيين، تكون `present = 0`. بايت الوجود مطلوب بمجرد أن يكون الإصدار المتفق عليه ≥ 54488 (ويُرسل على جميع interfaces، وليس على TCP فقط). بالنسبة إلى الاستعلامات بين الخوادم، تُضمَّن القائمة المتسلسلة أيضًا في `auth_hash` (انظر [مصادقة بين الخوادم](#inter-server-authentication)).                                                                                                     |
