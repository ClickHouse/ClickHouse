...
يستخدم كل جدول المحرّك [`MergeTree`](/engines/table-engines/mergetree-family/mergetree)، ويُرتَّب حسب عمود المفتاح الأساسي `id`، بما يعكس مخطط PostgreSQL الأصلي حيث يعرّف كل جدول `id integer NOT NULL PRIMARY KEY`. وتُعيَّن أعمدة PostgreSQL التي تقبل NULL إلى الأنواع `Nullable(...)`.
