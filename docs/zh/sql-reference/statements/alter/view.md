...
当使用`ALTER TABLE ... MODIFY QUERY`语句创建一个[物化视图](/sql-reference/statements/create/view#materialized-view)时，可以修改`SELECT`查询。当物化视图在没有  `TO [db.]name` 的情况下创建时使用它。必须启用 `allow_experimental_alter_materialized_view_structure`设置。
