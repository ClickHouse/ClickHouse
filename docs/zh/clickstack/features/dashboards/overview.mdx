...
`color` 可接受上述 categorical 和语义列表中的任何标记 (例如 `chart-blue` 或 `chart-success`) 。可选的 `backgroundChart.color` 可使用相同的标记枚举覆盖迷你线图的颜色。有关身份验证和基础 URL，请参阅 [ClickStack API 参考文档](/zh/use-cases/observability/clickstack/api-reference)。
