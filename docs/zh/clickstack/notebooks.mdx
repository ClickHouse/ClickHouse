...
AI Notebooks 是 ClickStack 中的一种交互式调查工具，结合了 AI 智能体 和手动分析。你可以用自然语言描述问题，AI 智能体 会代你查询日志、链路追踪和指标，并以一系列卡片的形式呈现相关数据、图表和摘要。你也可以在 AI 生成的结果旁添加自己的卡片 (图表、表、搜索和 Markdown 注释) ，从而形成完整的调查记录。
