...
如果留空，数据库将仅存于内存中，并会在我们终止 Python 进程后立即丢失。
...
我们将使用 [`schema_inference_make_columns_nullable`](/zh/reference/settings/formats#schema_inference_make_columns_nullable) 设置，这样列类型就不会全部被设为 `Nullable`。
