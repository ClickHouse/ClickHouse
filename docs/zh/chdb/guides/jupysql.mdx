...
[JupySQL](https://jupysql.readthedocs.io/en/latest/quick-start.html) 是一个 Python 库，可让你在 Jupyter 笔记本和 IPython shell 中运行 SQL。
...
我们将创建一个直方图，但首先要编写 (并保存) 一个查询，用来计算每位球员曾达到的前 100 名中的名次。
然后，我们就可以用它来创建一个直方图，统计达到各个名次的球员人数：
