...
了解关于正则表达式中转义字符的说明可参看 "match" 那一节。

当选择需要读的表时，`Merge` 表本身会被排除，即使它正好匹配了该正则。这样设计为了避免循环。
当然，是能够创建两个相互无限递归读取对方数据的 `Merge` 表的，但这并没有什么意义。

`Merge` 引擎的一个典型用处是可以像使用一张表一样使用大量的 `TinyLog` 表。
...
`Merge` 类型的表包括一个 `String` 类型的 `_table` 虚拟列。（如果该表早已定义了一个 `_table` 的列，那这个虚拟列会命名为 `_table1` ；如果 `_table1` 也早被定义了，那这个虚拟列会被命名为 `_table2` ，依此类推）该列包含被读数据的表名。

如果 `WHERE/PREWHERE` 子句包含了带 `_table` 的条件，并且没有依赖其他的列（如作为一个连接的元素或作为一个完整的表达式），这些条件的作用会像索引一样。这些条件会在那些可能被读数据的表的表名上执行，并且读操作只会在那些满足了该条件的表上去执行。 <!-- as one of the conjunction elements, or as an entire expression -->
