...
引擎参数：`ANY|ALL` – 数据严密程度；`LEFT|INNER` – 连接类型。
这些参数设置不用带引号，但必须与要 JOIN 表匹配。 k1，k2，……是 USING 子句中要用于连接的关键列。

此引擎表不能用于 GLOBAL JOIN 。

类似于 Set 引擎，可以使用 INSERT 向表中添加数据。设置为 ANY 时，插入重复键的数据会被忽略。设置为 ALL 时，插入重复键的数据会保留，且都会用于连接。不能直接对 JOIN 表进行 SELECT。检索其数据的唯一方法是将其作为 JOIN 语句右边的表。
