...
此表可用于监视。 该表对于每个Replicated\* 表都包含一行。
...
-   `is_leader` (`UInt8`)-副本是否是领导者。
    多个副本可以同时成为领导者。使用该 `merge_tree` 设置 `replicated_can_become_leader`可以防止副本成为领导者。领导者负责选择要执行的后台合并。
    请注意，可以对任何可用且在ZK中具有会话的副本执行写操作，而不管该副本是否为leader。
-   `can_become_leader` (`UInt8`)-副本是否可以当选为领导者。
-   `is_readonly` (`UInt8`)-副本是否处于只读模式。
    如果配置中不包含ZooKeeper的部分，如果在ZooKeeper中重新初始化会话时以及在ZooKeeper中重新初始化会话过程中发生未知错误，将打开此模式。
-   `is_session_expired` (`UInt8`)-与ZooKeeper的会话已经过期。 基本上与 `is_readonly` 相同。
-   `future_parts` (`UInt32`)-由于尚未完成的插入或合并而显示的数据分区的数量。
-   `parts_to_check` (`UInt32`)-需要验证的队列中数据分区的数量。 如果怀疑分区可能已损坏，则将其放入验证队列。
-   `zookeeper_path` (`String`)-在ZooKeeper中的表数据路径。
-   `replica_name` (`String`)-在ZooKeeper中的副本名称。同一张表的不同副本具有不同的名称。
-   `replica_path` (`String`)-在ZooKeeper中的副本数据的路径。 与连接 ‘zookeeper_path/replicas/replica_path’相同。
...
-   `inserts_in_queue` (`UInt32`)-需要插入数据块的数量。 插入通常复制得相当快。 如果这个数字很大，这意味着出了问题。
-   `merges_in_queue` (`UInt32`)-等待进行合并的数量。 有时合并时间很长，因此很长一段时间该值可能大于零。
-   `part_mutations_in_queue` (`UInt32`）-等待进行的突变的数量。
-   `queue_oldest_time` (`DateTime`)-如果 `queue_size` 大于0，显示何时将最早的操作添加到队列中。
...
-   `log_pointer` (`UInt64`) -副本复制到其执行队列的常规活动日志中的最大条目数加一。 如果 `log_pointer` 小于 `log_max_index`，则出问题了。
-   `last_queue_update` (`DateTime`) -上次更新队列时间。
...
如果这个查询没有返回任何内容，则表示一切正常。

[原始文章](https://clickhouse.tech/docs/en/operations/system_tables/replicas) <!--hide-->
