...
包含系统表`system.asynchronous_metrics` (../../operations/system-tables/asynchronous_metrics.md#system_tables-asynchronous_metrics) 的历史值，每分钟保存一次。默认启用。
...
-   `name` ([String](../../sql-reference/data-types/string.md)) — 指标名称。
...
- [system.asynchronous_metrics](../../operations/system-tables/asynchronous_metrics.md) — 包含在后台定期计算的指标。
- [system.metric_log](../../operations/system-tables/metric_log.md) — 包含来自表 `system.metrics` 和 `system.events`的指标值的历史记录，并定期刷新到磁盘。

[原始文章](https://clickhouse.tech/docs/en/operations/system_tables/asynchronous_metric_log) <!--hide-->
