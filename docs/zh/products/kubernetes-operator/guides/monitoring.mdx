...
<Note>
  本指南介绍的是 **operator 进程本身** (controller manager) 。对于 ClickHouse server 指标 (查询、parts、复制延迟) ，请使用 [ClickHouse 中的 Prometheus 端点](/zh/reference/settings/server-settings/settings/other#prometheus) 单独进行抓取。
</Note>
