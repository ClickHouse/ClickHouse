...
[`openSSL` 服务器设置](/zh/reference/settings/server-settings/settings/other#openssl)
，`extraConfig` 的合并方式请参阅
[配置 → 嵌入式额外配置](/zh/products/kubernetes-operator/guides/configuration#embedded-extra-configuration)。
...
* [`openSSL` 服务器设置](/zh/reference/settings/server-settings/settings/other#openssl) — 可通过 `extraConfig` 覆盖的 TLS 选项