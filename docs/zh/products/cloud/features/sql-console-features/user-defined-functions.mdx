...
4. 使用 [Terraform](#manage-udfs-with-terraform) (alpha)
...
UI 中提供的所有功能也可通过 [ClickHouse Cloud API](/zh/cloud/manage/api/api-overview) 以编程方式使用。
...
* [`clickhouse_udf_attachment`](https://github.com/ClickHouse/terraform-provider-clickhouse/blob/main/docs/resources/udf_attachment.md) 用于将 UDF 版本关联到服务。一个服务在同一时间最多只能使用一个函数版本。您可以固定指定版本号，也可以引用 `clickhouse_udf.<name>.version`，以自动将服务升级到最新版本。

<Note>
  这些资源适用于 provider 3.24.0 及更高版本。它们目前处于 alpha 阶段，行为可能会在未来的 provider 版本中发生变化。
</Note>
