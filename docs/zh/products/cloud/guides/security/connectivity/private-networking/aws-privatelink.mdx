...
为此，请配置你的 AWS 安全组 (Security Group) ，允许 ClickHouse Cloud 连接到你的内部/私有数据库服务。请查看 [ClickHouse Cloud 各区域的默认出口 IP 地址](/zh/products/cloud/guides/data-sources/cloud-endpoints-api)以及[可用的静态 IP 地址](https://api.clickhouse.cloud/static-ips.json)。