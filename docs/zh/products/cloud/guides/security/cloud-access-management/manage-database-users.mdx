...
<Tip>
  优先使用[云托管数据库角色](/zh/products/cloud/guides/security/cloud-access-management/manage-custom-roles#manage-database-roles) (Beta) 从控制台管理数据库访问权限。`sql-console-role:<email>` 角色与 [JWT 身份验证](/zh/products/cloud/guides/security/cloud-access-management/jwt-authentication-setup) 不兼容。
</Tip>
...
<div id="database-ssh">
  ### 通过安全外壳 (SSH) 进行身份验证的数据库用户 {#database-ssh}
</div>
