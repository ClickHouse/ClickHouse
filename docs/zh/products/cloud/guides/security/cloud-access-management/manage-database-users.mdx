...
<Note>
  **从控制台管理数据库角色 (Beta)**
  ...
  您还可以通过向自定义角色添加数据权限，直接从 ClickHouse Cloud 控制台管理数据库角色。此功能目前处于 Beta 阶段，适用于运行 ClickHouse 版本 26.4 及更高版本的服务。使用 `sql-console-role:<email>` 命名约定手动分配的角色优先于通过控制台分配的角色。更多信息，请参阅 [管理数据库角色](/zh/products/cloud/guides/security/cloud-access-management/manage-custom-roles#manage-database-roles)。
</Note>
