...
我们支持服务提供商发起的单点登录、通过独立连接支持多个组织，以及即时预配。我们还以私有预览形式支持通过 [Okta](/zh/products/cloud/guides/security/cloud-access-management/scim-setup) 和 [Microsoft Entra ID](/zh/products/cloud/guides/security/cloud-access-management/scim-setup-entra) 进行 SCIM provisioning。暂不支持属性映射。
...
* SCIM provisioning — 通过 [Okta](/zh/products/cloud/guides/security/cloud-access-management/scim-setup) 或 [Microsoft Entra ID](/zh/products/cloud/guides/security/cloud-access-management/scim-setup-entra) 自动执行用户和组预配 (私有预览) 。
