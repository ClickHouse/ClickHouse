...
<Steps>
  ...
  <Step>
    ...
    点击左下角的组织名称，然后选择 `Users and Roles`。查看每位用户的 `Provider` 列；凡是显示为 `Signed in with SSO` 的用户，都需要在移除 SAML 后重新邀请回组织。
...
<Steps>
  <Step>
    ...
    点击左下角的组织名称，然后选择 `Users and Roles`。按照[邀请用户](/zh/products/cloud/guides/security/cloud-access-management/manage-cloud-users#invite-users)中的说明进行操作。
  </Step>
