...
### Scale (始终运行、自动扩缩容) ：每月 $499.38 起 {#scale-always-on-auto-scaling-from-49938-per-month}
...
### Enterprise：起始价格因配置而异 {#enterprise-starting-prices-vary}
...
### 计算资源如何计费？ {#how-is-compute-metered}
...
### 如果我使用自管理部署，ClickHouse 提供了哪些工具来估算在云中运行服务的成本？ {#what-tools-does-clickhouse-offer-to-estimate-the-cost-of-running-a-service-in-the-cloud-if-i-have-a-self-managed-deployment}
...
### ClickHouse Cloud 有哪些可用的计费方式？ {#what-billing-options-are-available-for-clickhouse-cloud}
...
### 我可以在哪里查看退款？ {#where-can-i-see-my-refunds}
...
### 如果我有一个有效的 PAYG Marketplace 订阅，之后又签订了承诺合同，是否会优先消耗我的承诺额度？ {#committed-credits-consumed-first-with-active-payg-subscription}
...
### ClickHouse Cloud 提供哪些控制项来管理 Scale 和 Enterprise 服务的成本？ {#what-controls-does-clickhouse-cloud-offer-to-manage-costs-for-scale-and-enterprise-services}
...
### ClickHouse Cloud 提供哪些控制选项来管理基础版服务的成本？ {#what-controls-does-clickhouse-cloud-offer-to-manage-costs-for-basic-services}

* [高级扩缩容控制](/zh/products/cloud/features/autoscaling/overview)可让您控制服务在无活动时的暂停/闲置行为。基础版服务不支持调整内存分配。
* 请注意，默认设置会在服务一段时间无活动后将其暂停。

### 如果我有多个服务，是按服务分别开具发票，还是合并开具一张发票？ {#if-i-have-multiple-services-do-i-get-an-invoice-per-service-or-a-consolidated-invoice}

对于同一组织，在一个计费周期内的所有服务都会合并开具一张发票。

### 如果我在试用期结束、试用额度失效前添加信用卡并升级，还会被收费吗？ {#if-i-add-my-credit-card-and-upgrade-before-my-trial-period-and-credits-expire-will-i-be-charged}
...
### 如何查看我所订阅的 ClickHouse Cloud 服务发票？ {#how-do-i-access-my-invoice-for-my-subscription-to-the-clickhouse-cloud-service}
...
### 为什么 用量 语句上的日期与我的 Marketplace Invoice 不一致？ {#why-do-the-dates-on-the-usage-statements-not-match-my-marketplace-invoice}
...
### 预付费额度的使用是否有任何限制？ {#are-there-any-restrictions-around-the-usage-of-prepaid-credits}
...
### 通过云提供商市场付费与直接向 ClickHouse 付费相比，ClickHouse Cloud 的定价是否有差异？ {#is-there-a-difference-in-clickhouse-cloud-pricing-whether-paying-through-the-cloud-provider-marketplace-or-directly-to-clickhouse}

通过市场计费与直接向 ClickHouse 注册，价格没有任何差异。
无论采用哪种方式，您使用 ClickHouse Cloud 的用量都会以 ClickHouse Cloud Credits (CHCs) 进行跟踪，
并以相同方式计量和计费。

### compute-compute separation 如何计费？ {#how-is-compute-compute-separation-billed}
