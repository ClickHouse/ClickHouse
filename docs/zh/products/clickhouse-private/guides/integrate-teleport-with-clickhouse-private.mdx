---
noindex: true
title: "将 Teleport 与 ClickHouse 私网集成"
sidebarTitle: "集成 Teleport"
description: "在政府/FIPS 环境中，为 ClickHouse 私网集群配置 Teleport Enterprise，以实现安全的数据库访问。"
---

在政府/FIPS 环境中，为 ClickHouse 私网集群配置 Teleport Enterprise，以实现安全的数据库访问。本指南涵盖在 EKS 上部署 Teleport、配置数据库访问、SAML 身份验证以及证书轮换。

<div id="prerequisites">
  ## 前置条件
</div>

* 已完成 [FIPS/Government 部署](/zh/products/clickhouse-private/tutorials/deploy-clickhouse-with-fips-compliance-government)，并设置 `SERVER_VERIFICATION_MODE=relaxed` 和 `KEEPER_VERIFICATION_MODE=relaxed` (Teleport mTLS 所必需)
* Teleport Enterprise 许可证
* 服务器证书的 SAN 必须包含 `DNS.2 = c-${CLUSTER_NAME}-server-any.ns-${CLUSTER_NAME}.svc.${KUBERNETES_DOMAIN}` (参见 [生成符合 FIPS 要求的证书](/zh/products/clickhouse-private/guides/generate-fips-compliant-certificates-for-clickhouse))
...
为 Teleport 单独创建一个专用节点组是可选的。如果当前没有可用的 Bottlerocket ARM FIPS 节点，请在 EKS Console 的 Compute 下按以下配置创建一个节点组：

* **名称**: fips-arm64
* **节点 IAM 角色**: 创建一个新的推荐角色
* **Kubernetes 污点**:
  * **键**: `group`, **值**: `fips-arm64`, **效果**: `NoSchedule`
* **AMI 类型**: Bottlerocket ARM FIPS
* **实例类型**: m7g.large
* **扩缩容**: 期望值 2，最小值 2，最大值 3

***

<div id="install-teleport-cluster-on-eks">
  ## 在 EKS 上安装 Teleport 集群
</div>
...
<div id="generate-fips-certificates-for-teleport">
  ### 为 Teleport 生成 FIPS 证书
</div>

创建以下脚本 (`generate_fips_certs_in_container_for_teleport.sh`) ：
...
运行以下命令 (将 `<CLUSTER_FQDN>` 替换为 Teleport 服务的 FQDN) ：
...
创建 `eks_teleport_values.yaml`。替换以下占位符：

* `<CLUSTER_NAME>`：集群的 FQDN
* `<VPN_SUBNETS>`：VPN 私网子网，用逗号分隔 (例如：`private-subnet-us-east-2a,private-subnet-us-east-2b,private-subnet-us-east-2c`)
* `<BUCKET_NAME>`：用于存储 Teleport 会话记录的 S3 bucket
* `<NODE_SELECTOR>`：节点选择器的值 (例如：`fips-arm64`)
...
> 本示例将在 EKS 上部署启用了 FIPS 的 Teleport。若使用其他云提供商，请参阅 [Teleport 文档](https://goteleport.com/docs/zero-trust-access/deploy-a-cluster/helm-deployments/)。
...
获取外部 IP，并在 Route53 或所用的 DNS 提供商中创建一条 CNAME 记录。如果负载均衡器没有主机名：
...
使用预配令牌可让 Teleport 代理加入集群。创建 `teleport_provision_token_for_clickhouse.yaml`：
...
在 agent 命名空间中，为 Teleport proxy CA 创建一个 Secret：
...
为数据库 CA 创建一个 secret (如果使用多个且 CA 不同的数据库，请将各个 CA 拼接在一起) 。然后创建一个名为 `db_cas.pem` 的文件：
...
创建 `teleport_agent_values.yaml`。替换：

* `<CLICKHOUSE_CLUSTER_NAME>`：已创建的集群名称 (例如 `default-aa-01`)
* `<ENV>`：此数据库的静态标签 (例如 `dev`)
...
> 该 YAML 会为同一个 database 创建 2 个数据库资源，因为 ClickHouse 支持 2 种协议 (8443 端口上的 HTTPS 和 9440 端口上的原生 TLS) 。删除不需要的那个即可。
...
<div id="add-teleport-ca-to-clickhouse-trusted-list">
  ## 将 Teleport CA 添加到 ClickHouse 受信任列表
</div>

获取 Teleport 根 CA，并将其添加到存放 ClickHouse 服务器证书的 Secret 中：
...
重新启动 ClickHouse server 的 Pod (容器组) ，以使新的 CA 生效。
...
创建 `saml_clickhouse_user_role.yaml`。将用户 trait 键替换为身份提供商中的 attribute (例如，`http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress`) ：
...
有关角色的更多信息，请参阅 [Teleport 文档](https://goteleport.com/docs/reference/infrastructure-as-code/operator-resources/resources-teleport-dev-rolesv8/)。

<div id="create-a-saml-connector">
  ### 创建 SAML 连接器
</div>

创建 `saml_connector.yaml`。将 `<DOMAIN>` 替换为你的 Route53 域名，并将 `<Entity Descriptor>` 替换为身份提供商提供的值：
...
有关 SAML 连接器的更多信息，请参阅 [Teleport 文档](https://goteleport.com/docs/reference/infrastructure-as-code/operator-resources/resources-teleport-dev-samlconnectors/)。
...
创建一个通过 SSL 证书 CN 进行身份验证的用户 (将 `<DB_USER>` 替换为用户名——通常是身份提供商中的用户电子邮件地址) ：
...
将 Teleport CA 添加到系统的受信任 CA 列表中，或在开发环境下使用 `--insecure` 参数。
...
替换：- `<DOMAIN>`：您的 Route53 域名 - `<AUTH_TYPE>`：身份验证类型 (例如 `samlconnector`)  - `<TELEPORT_USER>`：Teleport 用户名 - `<DB_USER>`：数据库用户名 - `<CLICKHOUSE_CLUSTER_NAME>`：ClickHouse 集群名称 (例如 `default-aa-01`)
...
Teleport 证书会过期，必须在到期前完成轮换。

<div id="rotate-teleport-proxy-certificates">
  ### 轮换 Teleport 代理证书
</div>

1. 使用[为 Teleport 生成 FIPS 证书](#generate-fips-certificates-for-teleport)中的脚本**生成新证书**。确保 `CLUSTER_FQDN` 和其他变量与您的集群一致。

   > 该脚本必须在启用了 FIPS 的主机上运行。Docker 会自动挂载 `/proc`，因此只要容器运行在启用了 FIPS 的主机上，生成的证书就会符合 FIPS 标准。

2. 在两个命名空间中，将**新的 CA 追加到现有证书包**：
...
4. 将**新的 Teleport CA 添加到 ClickHouse 受信任列表**：
...
* [Teleport 文档](https://goteleport.com/docs/)