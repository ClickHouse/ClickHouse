...
<Steps>
  ...
  <Step stepNumber={2}>
    ## 第 2 步：创建 Artifact Registry (`01-artifact-registry.sh`)
  ...
  <Step stepNumber={3}>
    ## 第 3 步：复制容器镜像 (`02-copy-images.sh`)
  ...
  <Step stepNumber={4}>
    ## 步骤 4：配置 VPC 和子网 (`03-vpc-subnet.sh`)
  ...
  <Step stepNumber={5}>
    ## 第 5 步：创建 Cloud NAT (`04-cloud-nat.sh`)
  ...
  <Step stepNumber={6}>
    ## 步骤 6：创建 GCP 服务账号 (`05-service-account.sh`)
  ...
  <Step stepNumber={7}>
    ## 步骤 7：创建 GCS 存储桶 (`06-gcs-bucket.sh`)
  ...
  <Step stepNumber={8}>
    ## 第 8 步：创建 GKE 集群 (`07-gke-cluster.sh`)
  ...
  <Step stepNumber={10}>
    ## 步骤 10：部署 Local SSD DaemonSet 守护进程集 (`09-local-ssd-daemonset.sh`)
  ...
  <Step stepNumber={11}>
    ## 第 11 步：添加拓扑标签 (`10-topology-labels.sh`)
  ...
  <Step stepNumber={14}>
    ## 步骤 14：安装 Operator (`16-install-operator.sh`)
  ...
  <Step stepNumber={15}>
    ## 第 15 步：部署 ClickHouse 集群 (`17-install-cluster.sh`)
  ...
  <Step stepNumber={16}>
    ## 第 16 步：绑定 Workload Identity (`18-bind-helm-workload-identity.sh`)
