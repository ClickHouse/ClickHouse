...
| 目的                            | 连接类型                                                    | 说明                                                                                                |
| ----------------------------- | ------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| **日常运维 — Kubernetes API 服务器** | 通过 IP 过滤的公网 (默认)，或通过 Tailscale 或 AWS VPC Lattice 实现私有访问 | 管理服务通过公网与 EKS API 服务器通信，并受 IP 允许列表限制。初始部署完成后，您也可以选择切换到通过 Tailscale 或在 AWS 上通过 VPC Lattice 实现私有访问。 |
