...
ClickHouse BYOC 支持多种私有网络方案，可增强安全性并为您的服务提供直接连接能力。本指南将介绍推荐的连接方式，帮助您将部署在自有 AWS 账户、GCP 项目或 Azure 订阅中的 ClickHouse Cloud 安全地连接到其他网络或服务，例如内部应用或分析工具。我们将涵盖 VPC Peering (AWS 和 GCP) 、AWS PrivateLink、GCP Private Service Connect 和 Azure Private Link 等选项，并概述每种方案的主要步骤和注意事项。
