...
* **Kubernetes cluster:** 根据您的云提供商不同，它可以是 Amazon EKS、Google GKE 或 Azure AKS。它承载架构图中显示的 ClickHouse 服务器及配套服务。
