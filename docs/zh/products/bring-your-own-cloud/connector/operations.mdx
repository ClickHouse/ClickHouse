...
在主机上重新运行安装程序。它会像[引导设置](/zh/products/bring-your-own-cloud/connector/onboarding)时一样下载并验证新版本。它会备份原有二进制文件，安装当前的 systemd unit，并保留正在使用的脱敏规则和环境文件。然后重启守护进程：
...
每个守护进程都会在其健康检查端口提供 `/livez` 端点。响应正文中的 JSON `status` 字段才是健康状态信号，而非 HTTP 状态码，因此应检查响应正文，而不是 `200`。Prometheus 指标在同一端口的 `/metrics` 上提供；没有单独的指标端口。两个目标的默认端口如下：
...
执行器还会在 `127.0.0.1:9999` 上提供其本地命令 API。该 API 自身不带任何身份验证，也绝不会暴露到主机或 pod (容器组) 之外；`clicklink clctl` 服务命令会在本地访问它。
...
此处描述的托管模式行为需要[受支持的构建版本](/zh/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)；公开的 `v0.16.0` 既没有 `--managed` 也没有 `--no-managed`。

向您的 ClickHouse account team 申请新的注册标记，然后使用 `--enroll` 和 `--force` 重新运行原来的 `init` 命令。保留首次安装时使用的所有目标专用标志 (`--target-namespace`、`--values` 以及任何 `--chart`、`--chart-repo` 或 `--chart-version` mirror 标志) ，因为 `--force` 会重新暂存已保留的配置。托管模式会从被替换的配置中沿用，因此这里的 `--managed` 是可选的。对于默认安装：
...
在 Kubernetes 上，`init` 会预先创建轮换后的 `clicklink-hmac` 和 `clicklink-mtls` Secret；chart 不会渲染它们。因此 chart 的 checksum annotation 不会覆盖这些 Secret，正在运行的 Pod (容器组) 仍会继续使用启动时的凭据。轮换完成后需重启所有组件：
...
在虚拟机上，请在主机上运行这些命令，并通过管道传入 admin 密码；在虚拟机上，除非传入 `--ch-user-via cr`，否则必须提供密码：
...
<Note>
  公开发布的 `v0.16.0` 命令行客户端既没有 `--managed` 也没有 `--no-managed`。下述托管模式保留与覆盖行为需要使用带有[托管模式支持](/zh/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)的构建版本；在恢复过程中更改该模式前，请先与你的客户团队确认你的构建版本。
</Note>

`init` 支持幂等重复运行，因此遇到问题时，首先应再次运行相同的命令。未使用 `--force` 时，会保留现有的 `/etc/clicklink/config.yaml` (VM) 或 `clicklink-values.yaml` 覆盖文件 (Kubernetes) ，并复用现有的客户端密钥；`init` 会以原子方式覆盖凭据和 CA 链。CLI 在部分失败后输出的恢复命令可以安全地重复执行。
...
**连接器离线。** 在连接器停机或不可达期间，其管理的服务仍会持续运行。集群中的 ClickHouse Operator 会维持它们的运行，而 executor 并不位于数据路径中。生命周期变更需要 executor 保持连接，因此在其重新连接之前不会应用任何变更。重启后，executor 会从其命令数据库中恢复停止时仍在执行中的命令。对于 ClickHouse Cloud 再次下发的命令，将依据已记录的结果作出响应，而不会重复执行；只有失败的命令才会再次运行。关于没有 executor 连接时生命周期命令的处理方式，请参阅[连接器离线时](/zh/products/bring-your-own-cloud/connector/managed-services#when-the-connector-is-offline)。
...
executor 的集群级 grant 保存在你的 cluster 中，而不在主机上。当不再有任何 managed service 时，请以 cluster admin 身份将其移除：
...
该 grant 还创建了 `clicklink-system` 命名空间；当没有其他资源使用它时，请将其删除。
...
executor 的 PersistentVolumeClaim 带有 `helm.sh/resource-policy: keep` 注解，因此卸载后它会被保留下来，其上的命令 database 和 service registry 也一并保留。`init` 创建的 Secret、平台审批写入的平台支持包 Secret，以及 executor 维护的 service registry ConfigMap 同样不归 chart 所有，卸载后也会继续存在。请显式删除它们，其中包括你自行注册的每个 instance、以及你为其 provision 了故障排查工具的每个 managed service (以该 service 命名) 所对应的 access Secret 与 ServiceAccount。`--ignore-not-found` 用于兼容那些从未创建过的 object，例如首次审批之前的平台支持包 Secret：
...
`init` 应用的是 executor 的集群级 grant，而非 chart。待不再存在任何 managed services 后，请以集群 admin 身份将其移除：
