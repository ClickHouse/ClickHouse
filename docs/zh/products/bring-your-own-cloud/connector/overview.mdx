...
在终端中运行时，`init` 会询问是否启用托管模式，并默认预选“是”；以脚本方式运行的 `init` 则仅在指定 `--managed` 时才会启用。你也可以不启用托管模式，同样能为自行运维的集群保留可观测性和支持能力；参见[不使用托管模式运行](#run-without-managed-mode)。
...
传入 `--managed` 时，`init` 会启用托管模式；在 terminal 中运行时会提示询问，默认为“是”。启用后会部署 executor 并应用其 Kubernetes 访问权限授权，该授权为一个 `pcm-executor` ServiceAccount。admission 策略会将其写入操作限制在 service 命名空间内 (默认前缀为 `ns-`) ，仅额外允许其自身的 token 续订、一个 registry ConfigMap，以及在 Kubernetes 上位于其所属命名空间中已续订的客户端证书。

创建一个 service 需要两条命令。`clicklink clctl executor prepare` 使用你自己的 AWS 凭证创建该 service 的桶和 IAM role，并一次性显示 default 用户的密码。`clicklink clctl instances create` 通过你的连接器端点将 service 定义提交给 ClickHouse Cloud。此后，ClickHouse Cloud 会通过 executor 的 出站 命令通道来驱动该 service：扩缩容、停止与启动、重启、备份、版本升级以及配置变更。完整操作流程参见 [managed services](/zh/products/bring-your-own-cloud/connector/managed-services)。

有两项操作仍由你负责。集群中平台组件的更新需经你批准才会执行；参见 [platform updates](/zh/products/bring-your-own-cloud/connector/platform-updates)。删除 service 仅会移除其 Kubernetes 资源；其桶和 IAM role 会一直保留，直到你运行 `clicklink clctl executor teardown`。

抓取器会按固定时间间隔读取允许列表中的系统表，并通过 mTLS 将 metrics、元数据和健康状况发送到你的连接器端点。故障排查工具会保持一个 出站 命令通道处于打开状态，但在你启用 support session 之前，该通道不会传输任何内容。mTLS 证书会自动续订。
...
* **`prepare`、`teardown` 和 `platform approve` 所需的 AWS 凭证。** 创建服务的桶和 IAM role，以及在服务删除后将其移除，均在你的 workstation 或连接器主机上以你的凭证运行。当暂存的图表位于 Amazon ECR 时，`platform approve` 还需要能够读取 Amazon ECR 的凭证。executor 从不持有访问你的桶或 IAM 的凭证，它唯一的云端调用是访问 Amazon ECR 和 STS：当平台同步或创建操作拉取图表时登录 registry，在平台更新期间使用只读拉取 role 检查镜像。它不会发起任何 S3 或 IAM 调用。
* **可访问的 ClickHouse 原生监听器**，适用于你自行运维的集群，由通过 `--no-managed` 安装的独立连接器部署进行观测。抓取器和故障排查工具通过原生协议与各个集群通信；在 Kubernetes 上会自动检测安全端口 (9440) 或明文端口 (9000) 。
