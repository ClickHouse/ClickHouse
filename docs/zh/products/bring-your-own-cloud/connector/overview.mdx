...
<Note>
  此处描述的托管模式工作流尚未在公开的连接器版本中提供。安装前请与您的客户团队确认所需的构建版本与部署支持；参见 [托管模式可用性](/zh/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)。
</Note>

## 连接器的作用 {#what-the-connector-does}

ClickHouse 连接器是部署在你自己环境中的组件。它让 ClickHouse Cloud 能够管理、监控并支持运行在你所运维的 Kubernetes 集群中的 ClickHouse 服务。ClickHouse Cloud 绝不会反向连接到你的环境：连接器只发起出站连接，并以限定在 ClickHouse 服务范围内的身份执行操作。连接器以单个二进制文件 `clicklink` 的形式发布，其中包含各个守护进程，以及用于安装和运维它们的 `clicklink clctl` 工具。
...
在[支持托管模式的构建版本](/zh/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)中，`init` 会在终端中询问是否启用托管模式，并默认预选“是”；以脚本方式运行的 `init` 则仅在指定 `--managed` 时才会启用。你也可以不启用托管模式，同样能为自行运维的集群保留可观测性和支持能力；参见[不使用托管模式运行](#run-without-managed-mode)。
...
* 对集群的写入仅限于 ClickHouse 所管理服务的命名空间；例外的是 executor 在其自身命名空间中的 token、registry 和续期后的证书。这些服务所依赖的平台组件只有在你逐次批准更新后才会变更。
* 你的数据存放在由你自行创建和删除的桶中，并受你自行管理的 IAM role 保护。executor 不持有访问这些桶或 IAM role 的任何凭据。
* 支持人员的访问有时间限制，由你开启，也可由你随时撤销。支持会话中的每一条命令都会记录到你所拥有的审计日志中。

## 工作原理 {#how-it-works}

下面的注册与服务创建流程需要由你的客户团队确认[托管模式支持](/zh/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)；在公开发布的 `v0.16.0` 中不可用。

ClickHouse 会在 onboarding 期间注册你的环境，并提供连接器端点和一个一次性的注册标记。你只需运行一次 `clicklink clctl init --enroll`：Kubernetes 安装方式下，从具备 kubeconfig 访问权限的 workstation 运行；Linux VM 安装方式下，则在该主机上运行。这一条命令会完成标记兑换、获取 mTLS 客户端证书、部署并启动守护进程，并验证其健康状况。

传入 `--managed` 时，`init` 会启用托管模式；在 terminal 中运行时会提示询问，默认为“是”。启用后会部署 executor 并应用其 Kubernetes 访问权限授权，该授权为一个 `pcm-executor` ServiceAccount。admission 策略会将其写入操作限制在服务命名空间内 (默认前缀为 `ns-`) ，仅额外允许其自身的 token 续订、一个 registry ConfigMap，以及在 Kubernetes 上位于其所属命名空间中已续订的客户端证书。

创建一个服务需要两条命令。`clicklink clctl executor prepare` 使用你自己的 AWS 凭证创建该服务的桶和 IAM role，并一次性显示 default 用户的密码。`clicklink clctl instances create` 通过你的连接器端点将服务定义提交给 ClickHouse Cloud。此后，ClickHouse Cloud 会通过 executor 的 出站 命令通道来驱动该服务：扩缩容、停止与启动、重启、备份、版本升级以及配置变更。完整操作流程参见 [managed 服务](/zh/products/bring-your-own-cloud/connector/managed-services)。
...
抓取器会按固定时间间隔读取允许列表中的系统表，并通过 mTLS 将指标、元数据和健康状况发送到你的连接器端点。故障排查工具会保持一个 出站 命令通道处于打开状态，但在你启用 support session 之前，该通道不会传输任何内容。mTLS 证书会自动续订。

## 在非托管模式下运行 {#run-without-managed-mode}

下述安装时选项需要[支持托管模式的构建版本](/zh/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)。公开发布的 `v0.16.0` 不提供 `--no-managed` 标志，也不会出现托管模式提示。
...
* **安装期间的集群管理员权限。** 启用托管模式时会一次性应用 executor 的访问授权：一个 ServiceAccount、绑定到它的集群角色，以及将其写操作限制在服务命名空间内的 admission 策略。批准平台更新同样需要集群管理员权限。在两种安装目标上，故障排查工具的访问包都锚定到 Kubernetes ServiceAccount。参见 [onboarding](/zh/products/bring-your-own-cloud/connector/onboarding)。
* **`prepare` 和 `teardown` 所需的 AWS 凭证，以及 `platform approve` 所需的 registry 访问权限。** 创建服务的桶和 IAM role，以及在服务删除后将其移除，均在你的 workstation 或连接器主机上以你的凭证运行。若要直接访问 ClickHouse 的 registry，平台批准与同步会使用连接器 EC2 虚拟机的实例 profile 来担任只读拉取 role；workstation 上的 AWS profile 或 SSO 凭证无法替代该身份。其他 ECR 图表 registry 则使用环境中的 AWS 凭证。参见 [registry 凭证](/zh/products/bring-your-own-cloud/connector/platform-updates#registry-credentials)。executor 从不持有访问你的桶或 IAM 的凭证。它唯一的云端调用是为拉取图表和检查平台镜像而访问 Amazon ECR 和 STS；它不会发起任何 S3 或 IAM 调用。
...
* [Managed services](/zh/products/bring-your-own-cloud/connector/managed-services)：启用托管模式、创建服务，并了解其生命周期、状态和删除操作。
