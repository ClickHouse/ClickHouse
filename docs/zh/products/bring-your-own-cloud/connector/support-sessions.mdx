...
support session 用于通过 ClickHouse 连接器向 ClickHouse 授予临时诊断访问权限。本页介绍会话的定义、如何启用和禁用会话、会话期间 ClickHouse 支持工程师可执行的操作，以及如何对其进行审计。

## 什么是 support session {#what-a-support-session-is}

support session 是一段有时限的窗口期，在此期间故障排查工具会接受 ClickHouse 支持工程师下发的命令。在会话之外，即使故障排查工具的 outbound WebSocket 仍处于连接状态，它也会拒绝一切命令。除此之外没有其他执行路径：没有会话，故障排查工具不会执行任何操作，而 ClickHouse 也无法替你开启会话。ClickHouse Cloud 绝不会主动连入你的环境；它只接收故障排查工具通过其 outbound 通道发送的内容。
...
<Tabs>
  <Tab title="Kubernetes" id="sessions-kubernetes">
    ...
    调用方的 OIDC 身份必须在 操作员 允许列表中。未经身份验证或不在允许列表中的调用方将收到 401 或 403，且 gateway 会将此次尝试记录到日志中。若不想要求集群凭据，可通过 chart 中选择性启用的入口公开 gateway，它会使用 CA 签发的证书终止 TLS；请参阅[配置](/zh/products/bring-your-own-cloud/connector/configuration)。
  </Tab>
...
会话会自动过期。默认时长为 4 小时；`session enable --duration` 最长可接受 24 小时。会话过期后，或运行 `session disable` 后，故障排查工具将不再接受命令。禁用即可立即撤销访问权限：无需重启，也无需与 ClickHouse 协调。

## 操作员允许列表 {#operator-allowlist}

每次gateway调用都会依据操作员电子邮件允许列表进行授权。匹配依据是经验证的 OIDC 标记中验证的电子邮件，绝不采信客户端对自身提出的任何声明。
...
在这两种目标上，gateway都会在距上次读取超过 30 秒后的下一次请求时重新读取该文件，因此修改无需重启即可生效。

## 操作员在会话期间可以执行哪些操作 {#what-operators-can-do}

会话处于活动状态期间，ClickHouse 支持工程师可以运行两类命令。

**只读 SQL** 以 `pcm_troubleshooter` 用户身份在你的 cluster 上执行，该用户的授权仅限于会话表允许列表。故障排查工具还会拒绝任何不以 `SELECT`、`WITH`、`SHOW`、`DESCRIBE` 或 `EXPLAIN` 开头的语句。该用户仅持有按表粒度的 `SELECT` 授权，不具备任何写入、DDL 或管理类特权。

默认允许列表涵盖 `system.parts`、`system.merges`、`system.replicas`、`system.metrics`、`system.settings` 等 `system` 表。`system.query_log` 和 `system.text_log` 无法加入该列表：守护进程会拒绝列出这两张表的配置，因此永远不会为它们配置任何授权。默认允许列表中确实包含 `system.processes`，其 `query` 列会显示当时正在运行的语句文本。如果不希望实时查询文本在会话中被看到，请将其从 `troubleshooter.allowedTables` (Helm 覆盖文件) 或 `troubleshooter.allowed_tables` (VM 配置文件) 中移除。
