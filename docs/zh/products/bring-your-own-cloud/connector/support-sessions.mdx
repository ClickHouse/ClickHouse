...
支持会话用于通过 ClickHouse 连接器向 ClickHouse 授予临时诊断访问权限。本页介绍会话的定义、如何启用和禁用会话、会话生效期间 ClickHouse 操作员可执行的操作，以及如何审计期间发生的所有活动。

## 什么是支持会话 {#what-a-support-session-is}
...
## 操作员在会话期间可以执行的操作 {#what-operators-can-do}
