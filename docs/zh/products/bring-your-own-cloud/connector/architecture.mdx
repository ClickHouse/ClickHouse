...
| Amazon ECR (执行器，仅托管模式)              | 出站          | HTTPS              | 执行器 所处环境自带的 AWS 身份，或它为镜像检查所担任的只读拉取角色                            | 登录 registry 以拉取用于平台同步或创建的 chart；在平台更新期间调用 `DescribeImages` 以确认镜像存在                                                              |
| AWS STS (执行器，仅托管模式)                 | 出站          | HTTPS              | 执行器 所在节点或主机自带的 AWS 身份                                           | 获取用于 ECR 登录的凭据，以及担任你为镜像检查注册的只读拉取角色。ECR 和 STS 是 执行器 仅有的云 API 调用                                                             |
...
* **执行器不持有访问你的桶或 IAM 的凭据。** `clicklink clctl executor prepare` 会创建某个服务的数据桶和备份桶及其 IAM 角色，`clicklink clctl executor teardown` 则将其移除；两者均使用你的凭据运行。该角色仅信任该服务的 Pod (容器组) ，因此执行器和 ClickHouse 都无法扮演它。执行器的云调用仅发往 Amazon ECR 和 STS。当平台同步或创建操作拉取 chart 时，它会登录 registry；在平台更新期间，它会使用只读拉取角色检查镜像。它不会发起任何 S3 或 IAM 调用。
* **网络策略。** 在 Kubernetes 上，chart 会为每个守护进程渲染一个默认拒绝的 NetworkPolicy。出站流量仅允许发往 DNS、你为连接器端点列出的 CIDR，以及 Kubernetes API 服务器的 CIDR。抓取器和故障排查工具的策略还允许访问执行器所管理命名空间中的 ClickHouse 服务 (通过执行器为其打上的标签选中) ，或你按名称列出的服务。执行器的策略不包含任何 ClickHouse 规则。如果平台捆绑包的 chart 存放在 Amazon ECR 中，请将 ECR 和 STS 的地址范围 (或你的 registry 镜像源) 添加到 `allowEgressCIDRs`。否则，在启用强制执行的 CNI 下同步将无法拉取 chart。强制执行取决于你的集群是否运行具备强制能力的 CNI；若没有，该策略将形同虚设。参见[配置](/zh/products/bring-your-own-cloud/connector/configuration)。
