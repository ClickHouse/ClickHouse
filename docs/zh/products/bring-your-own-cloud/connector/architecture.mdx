...
<Note>
  此处描述的托管模式 注册、客户创建和平台批准工作流需要经由您的客户团队确认的[匹配的命令行客户端与部署支持](/zh/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)。公开发布的 `v0.16.0` 既不包含托管模式的 init 标志，也不包含 `platform` 命令，并且未附带客户创建工作流。
</Note>
...
* **执行器 (执行器)&#x20;**&#x20;与连接器端点之间保持一条出站命令通道，并执行 ClickHouse Cloud 针对其在你的集群中所管理的服务下发的生命周期命令。这些命令包括创建、扩缩容、停止与启动、重启、备份与删除备份、删除，以及你已批准的平台更新。它执行的每一项操作都是一次 Kubernetes API 调用。它不会与 ClickHouse 建立任何连接，也不持有你的存储桶或 IAM 凭据。仅当启用托管模式时，它才会运行。
...
下面涉及客户创建和平台批准的连接适用于[受支持的托管模式部署](/zh/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)；这些工作流在公开发布的 `v0.16.0` 中不可用。
...
| Amazon ECR (执行器，仅托管模式)                  | 出站          | HTTPS              | 对于直接平台访问，使用连接器 EC2 虚拟机的实例配置文件所担任的只读拉取角色；对于其他 ECR 图表 registry，则使用环境自带的 AWS 凭据 | 登录 registry 以拉取用于平台同步或创建的图表；在平台更新期间调用 `DescribeImages` 以确认镜像存在                                                         |
| AWS STS (执行器，仅托管模式)                     | 出站          | HTTPS              | 对于直接平台访问，使用连接器 EC2 虚拟机的实例配置文件；对于其他 ECR 图表 registry，则使用所配置的 AWS 凭据链           | 担任只读拉取角色以完成直接平台图表登录和镜像检查；也可能为其他 ECR 登录提供凭据。ECR 和 STS 是 执行器 仅有的云 API 调用                                                |
...
在入站方向，每个守护进程只暴露一个本地健康状况端口：抓取器为 8082，故障排查工具为 8084，执行器 为 8086。该端口提供 `/livez`、`/readyz`、`/healthz`，并在 `/metrics` 暴露该守护进程的 Prometheus 指标；没有单独的指标端口。执行器 的本地命令 API 仅绑定到回环地址，也没有任何 Service 将其对外暴露。唯一的另一个监听器，是[支持会话](/zh/products/bring-your-own-cloud/connector/support-sessions)页面中介绍的需手动启用的会话网关。ClickHouse Cloud 绝不会连接其中任何一个。
...
* **抓取路径上的原始查询文本。** `system.query_log` 未包含在默认抓取集中，因为其查询相关列可能包含字面值，从而携带 PII 或密钥。若要重新加入，需要你在知情的前提下按部署逐一进行覆盖配置。默认的会话表允许列表中确实包含 `system.processes`，它会显示正在执行的查询文本；如需对其进行裁剪，请参阅[支持会话](/zh/products/bring-your-own-cloud/connector/support-sessions)。
...
服务创建和平台批准工作流需要[匹配的 命令行客户端 与部署支持](/zh/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)；公开发布的 `v0.16.0` 并不包含这些功能。
...
* **抓取器和故障排查工具无法更改你的集群。** 它们的 RBAC 通过连接器命名空间和服务命名空间中的 Role 授予，对工作负载资源仅有只读动词权限，并且只能按精确名称访问连接器自身的 Secret。不存在 `exec`、`delete` 或 `patch` 权限。
* **执行器以 `pcm-executor` 身份更改服务命名空间中的集群状态。** 它创建和删除带有服务前缀 (默认为 `ns-`) 的命名空间，并通过删除 pod (容器组) 来重启它们。在这些命名空间内，它写入 ClickHouseCluster、Backup 和 ScalingOverride 对象，服务所需的 Secret、ServiceAccount 和 Service，以及为抓取器提供读取权限的 Role 和 RoleBinding。ValidatingAdmissionPolicy 会拒绝该 ServiceAccount 在前缀范围之外的任何写入操作，仅有其自身的令牌续期以及所属命名空间中的 `clicklink-instance-registry` ConfigMap 例外。当该策略无法被评估时，会以拒绝方式失败。admission 不作用于读取操作，因此它对这些资源族的读取不受前缀限制。
* **执行器仅在你批准的更新期间以 `pcm-platform` 身份更改平台命名空间。** `clicklink clctl platform approve` 会为该身份签发有效期 2 小时的令牌；该令牌永不续期。凭此令牌，执行器只能写入 Deployment、Service、ConfigMap、Secret、Job 和 PodDisruptionBudget。命名空间、CRD、ServiceAccount、RBAC、webhook 与 admission 配置、PriorityClass 以及 StorageClass 则在批准过程中由你自己的凭据应用。参见[平台更新](/zh/products/bring-your-own-cloud/connector/platform-updates)。
* **执行器不持有访问你的桶或 IAM 的凭据。** `clicklink clctl executor prepare` 会创建某个服务的数据桶和备份桶及其 IAM 角色，`clicklink clctl executor teardown` 则将其移除；两者均使用你的凭据运行。该角色仅信任该服务的 Pod (容器组) ，因此执行器和 ClickHouse 都无法扮演它。执行器的云调用仅发往 Amazon ECR 和 STS。当平台同步或创建操作拉取图表时，它会登录 registry；在平台更新期间，它会使用只读拉取角色检查镜像。它不会发起任何 S3 或 IAM 调用。
* **网络策略。** 在 Kubernetes 上，图表会为每个守护进程渲染一个默认拒绝的 NetworkPolicy。出站流量仅允许发往 DNS、你为连接器端点列出的 CIDR，以及 Kubernetes API 服务器的 CIDR。抓取器和故障排查工具的策略还允许访问执行器所管理命名空间中的 ClickHouse 服务 (通过执行器为其打上的标签选中) ，或你按名称列出的服务。执行器的策略不包含任何 ClickHouse 规则。如果平台捆绑包的图表存放在 Amazon ECR 中，请将 ECR 和 STS 的地址范围 (或你的 registry 镜像源) 添加到 `allowEgressCIDRs`。否则，在启用强制执行的 CNI 下同步将无法拉取图表。强制执行取决于你的集群是否运行具备强制能力的 CNI；若没有，该策略将形同虚设。参见[配置](/zh/products/bring-your-own-cloud/connector/configuration)。
* **VM 上的主机加固。** 这些单元以不可登录的系统用户身份运行，并启用了 `ProtectSystem=strict`、`NoNewPrivileges`、只读配置路径以及 FIPS 模式。

关于连接器的确切授权和 RBAC 规则，参见[特权模型](/zh/products/bring-your-own-cloud/connector/reference/privilege-model)参考文档。如果由 ClickHouse 自行预配并运维集群，则信任模型有所不同；参见 [BYOC 架构](/zh/products/bring-your-own-cloud/overview/architecture)和 [BYOC 特权](/zh/products/bring-your-own-cloud/reference/privilege)页面。