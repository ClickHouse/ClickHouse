...
<Steps>
  ...
  <Step title="注册并安装连接器" id="enroll-and-install">
    注册只需一条命令。它会兑换你的注册标记，获取已签名的客户端证书以及 executor 的集群级 grant，然后安装并验证连接器。
...
* **在 Kubernetes 上**，被拒绝的授权会在 chart 部署之前中止 `init`，因为 executor 需要绑定到已授权的 ServiceAccount，缺少它便无法启动。已暂存的覆盖文件、Secrets 和 CSR 会被保留，`init` 会打印出两条命令，需在具备 cluster 管理员权限的主机上执行。执行完这两条命令后，再次运行相同的 `init` 命令即可继续：
