...
本页将指导您使用注册标记，在托管模式下完成连接器的验证；在该模式下，由 executor 执行由 ClickHouse Cloud 驱动的 service 生命周期。连接器可安装到 Kubernetes 集群 (Helm) 或 Linux VM (systemd) 。标记注册是标准流程；如果您的环境无法直接访问您的连接器端点，请参阅[隔离网络和镜像安装](#air-gapped-and-mirrored-installs)。如果要在不使用 executor 的情况下观测自行运行的 ClickHouse，请传入 `--no-managed`。

## 托管模式可用性 {#managed-mode-availability}

<Warning>
  截至 2026-09-22，公开安装程序提供的版本为 `v0.16.0`，该版本既不包含 `init --managed` 和 `init --no-managed` 标志，也不包含 `platform` 命令。Linux VM 和 Kubernetes 安装上的平台批准，以及客户服务创建 (`instances create --from-prepare`) ，在公开发布版本中均尚未提供。Kubernetes 平台批准还额外要求命令行客户端和 chart 支持 `platform approve --secret-namespace`。

  在使用托管模式工作流之前，请先联系您的 ClickHouse account team，确认其是否支持您的部署目标，并获取相应的命令行客户端、连接器 chart 或 image 以及安装指令。客户服务创建还需要您的连接器端点和 ClickHouse Cloud 提供支持，仅升级命令行客户端并不足够。下文的公开安装命令目前不会安装支持这些工作流的 release。
</Warning>
...
* 你的 AWS 账户 ID、region 以及 EKS cluster 名称
* 连接器是运行在 Linux VM 上，还是运行在 cluster 中
* 用于平台 components 的 node group
* services 的命名空间 prefix (默认为 `ns-`)
* 你的 cluster 以何种方式拉取 ClickHouse images：通过 ClickHouse Cloud 为你的 account 配置的 registry access，还是使用你自己的 mirror

随后，ClickHouse Cloud 会为你签发连接器端点和一个一次性注册标记。该标记本身即可标识你的环境，因此注册时无需再输入额外的 ID。

### 所有安装方式通用 {#prerequisites-every-install}

* **一个支持[托管模式](#managed-mode-availability)的构建版本**，用于下文的托管模式操作说明。公开发布的 `v0.16.0` 既没有 `--managed` 也没有 `--no-managed`。
* **你的连接器端点和注册标记**，在[注册](#registration)时提供。
* **443 端口出站访问权限**，目标为 `https://<subdomain>.<connector-domain>` 和 `https://<subdomain>.enroll.<connector-domain>`，以及安装时访问 `releases.clicklink.clickhouse.com` 和 Amazon ECR Public。在托管模式下，当平台同步或创建操作从 ECR 拉取 chart 时，executor 还会访问 Amazon ECR 和 STS。若其中任何一项无法访问，请参阅[隔离网络与镜像安装](#air-gapped-and-mirrored-installs)。
...
* **尚未部署 ClickHouse。** 托管安装从零个服务开始；由 executor 创建的服务会自行注册。托管模式下的 `init` 不会注册任何你自行运行的 ClickHouse (在 Kubernetes 上会拒绝 `--instance` 和 `--instance-namespace`) 。若要观测这类集群，请使用 `--no-managed` 单独安装一套连接器部署；借助 `--ch-user-suffix`，两套部署可以共用同一个实例，而不会在连接器的用户上发生冲突。该部署需要一个可访问的原生监听端口：安全端口 9440 或明文端口 9000，在 Kubernetes 上会自动检测。它还需要用于预配的 ClickHouse 管理员访问权限：一个无密码的 `default` 用户、通过 `--ch-admin-password-stdin` 或在提示符处输入的密码，或者对由 operator 管理的实例使用 CR 注入。
...
* **一个 kubeconfig**，其当前 context 指向目标 EKS cluster。它必须能够创建和读取连接器命名空间、应用 Secrets、创建 ServiceAccounts、Roles 和 RoleBindings、安装 chart，并为 executor 授权集群级访问权限。
* 工作站上具备 `eks:DescribeCluster` 和 `ec2:DescribeVpcs` 权限的 **AWS 凭证** (来自环境变量或 profile) 。`init` 会用它们读取 cluster VPC 的 CIDR 块，用于配置 NetworkPolicy。若读取失败，它会发出警告，并将该列表留给你自行填写。
...
* **镜像拉取权限**：cluster 节点必须能够拉取公共 ECR image，或你自行托管的 mirror。
...
* **对 Kubernetes API server 的管理员访问权限**，用于预配以及 executor 的授权：可通过主机上的 kubeconfig、`--server` 与 `--ca-data` 参数，或交互式提示提供。访问包在两侧目标上均锚定到 Kubernetes ServiceAccount。在托管模式下，kubeconfig 的当前 context 必须指向你所管理的 EKS cluster；通过其他任何 context，`init` 都会拒绝执行授权。
...
## 安装与注册 {#install-and-enroll}

请先与您的客户团队确认[托管模式可用性](#managed-mode-availability)。下方的公开安装程序提供的是 `v0.16.0`，无法运行本演练中的托管模式注册命令。
...
<Steps>
  <Step title="获取连接器端点和注册标记" id="get-endpoint-and-token">
    ...
    该 token 仅可使用一次，且很快过期，因此请在收到后尽快完成 enrollment。请将其视为 secret：命令行客户端只会通过隐藏式 prompt 或 stdin 的第一行读取该 token，绝不会从命令行参数、磁盘或日志中读取。若 token 已过期，请联系你的 ClickHouse account team 获取新的 token。
  </Step>

  <Step title="安装并验证命令行客户端" id="install-and-verify-the-cli">
    一条命令即可安装经过验证的 `clicklink` binary。它会检测您的平台(macOS 或 Linux,amd64 或 arm64),下载当前 release,并校验 SHA-256 checksum;若已安装 cosign,还会校验 release 签名。若 `/usr/local/bin` 可写,binary 将安装到该目录,否则安装到 `~/.local/bin`(如有需要,请将其加入 `PATH`;也可使用 `--bin-dir` 进行 override)。如需在 Kubernetes 中安装,可在任意拥有该 cluster kubeconfig 访问权限的 workstation 上运行:

    ```bash
    curl -fsSL https://releases.clicklink.clickhouse.com/install.sh | bash
    ```

    若在 VM 上安装，请在主机上使用 `--host` 运行同一脚本。完成校验下载后，脚本还会创建 `clicklink` 系统用户，`/etc/clicklink`、`/var/lib/clicklink` 和 `/var/log/clicklink` 目录，以及三个组件各自的 systemd 单元。同时会在 `/etc/clicklink/redaction-patterns.yaml` 中预置一组初始脱敏 patterns (若该文件已存在则保留原文件) ，因此下一步可直接从 enrollment 开始：
  ...
  <Step title="注册并安装连接器" id="enroll-and-install">
    这些命令需要 [managed 模式可用性](#managed-mode-availability)中所述的 build 和部署支持。公开发布的 `v0.16.0` 既不包含 `--managed`，也不包含 `--no-managed`。

    注册只需一条命令。它会兑换你的注册令牌，获取已签名的客户端证书以及 executor 的集群级授权 (grant) ，然后安装并验证连接器。
    ...
    <Tabs>
      ...
      <Tab title="Linux 虚拟机" id="enroll-vm">
        ...
        `init` 会兑换注册标记，并将 enrollment bundle 保存为 `handoff.yaml`。它会写入包含 `executor` 块的 `/etc/clicklink/config.yaml`，并安装 API 凭据和 CA 链。它会生成 private key 和 CSR，并由 ClickHouse Cloud 签发客户端证书。它会在 `/etc/clicklink/access/executor/_cluster` 下生成 executor 的集群级 access bundle，并通过主机的 kubeconfig 应用。最后，它会启用并启动 `clicklink-scraper`、`clicklink-troubleshooter` 和 `clicklink-executor` 单元，等待各单元报告存活，并运行完整的预检检查套件。
      </Tab>
    </Tabs>
  ...
  <Step title="清理" id="clean-up">
    enrollment bundle `handoff.yaml` (权限 `0600`，位于工作目录中) 的作用是确保安装过程中的重新运行和 recovery 无需再次使用 token。该文件以 plaintext 形式保存连接器的 API secret，因此安装验证通过后请将其删除：
    ...
    运行中的连接器会持有自己的一份凭据副本，因此任何运行操作都不依赖该文件。升级和配置变更也都用不到它。如果后续需要再次执行 `init`，请向 ClickHouse 客户团队申请新的注册标记，然后运行 `init --enroll --force`。
  </Step>
</Steps>
...
如果 `init` 将 `--managed` 或 `--no-managed` 视为未知标志而拒绝执行，请检查你的命令行客户端版本：公开发布的 `v0.16.0` 两者都不支持。在按照下文的托管模式恢复指令操作之前，请先从客户团队获取[支持托管模式的构建版本](#managed-mode-availability)。

重新运行相同的命令。`init` 是幂等的：重复运行会收敛到相同状态，保留现有配置和已暂存的文件，并跳过已完成的工作。若某个步骤中途失败，命令行客户端会针对当前情况打印出准确的恢复命令；这些命令可以安全地重复执行。

如果注册被拒绝，说明令牌已被兑换，或者无效或已过期。对于已兑换的令牌，请加上 `--handoff handoff.yaml` 重新运行，该文件在清理步骤之前一直存在；否则请向 ClickHouse 客户团队申请新的令牌。如果注册因传输错误而失败，请重新运行相同的命令。如果重新运行仍被拒绝，说明第一次尝试确实已消耗该令牌，请另行申请一个新令牌。
...
`--force` 是显式的重置，而非常规重试。它会覆盖保留下来的 config 或配置值覆盖层，重新生成客户端密钥，并取代尚未过期的客户端证书 (签名端点返回 `409` 即表示证书已存在) 。执行 `--force` 前后，连接器的集群 UUID 保持不变，其托管模式状态同样会被保留。无人值守的重新运行会沿用该状态；在终端中运行时，提示的默认值也是该状态，除非你传入 `--managed` 或 `--no-managed`。轮换凭据或取代证书时请使用 `--force`；完整的重新运行与恢复模型请参阅[运维](/zh/products/bring-your-own-cloud/connector/operations)。