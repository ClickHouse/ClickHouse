...
* **`pcm-platform`** 是 executor 用于平台 updates 的 identity。你执行 `clicklink clctl platform approve` 会为其签发有效期 2 小时的令牌，且不会续期；在两次批准之间，executor 对平台层没有写入 access。
* **按 service 划分的 IAM role** (默认为 `CH-S3-<service>-<region>-00-Role`) 由 `clicklink clctl executor prepare` 使用你的 credentials 创建。其作用范围仅限该 service 的数据桶和 backup 桶，且仅被该 service 的 Kubernetes ServiceAccount 信任，因此 executor 和 ClickHouse 都无法 assume 该 role。
* **executor 所在节点或主机的 ambient AWS identity** 有两个用途：当平台同步或创建操作拉取 chart 时，它通过 STS 登录 Amazon ECR；在平台 update 期间进行镜像检查时，它会 assume 你注册的只读 pull role。不涉及任何 S3 或 IAM 调用。Image pulls 发生在你的节点上，使用节点各自的 pull role。executor 不持有任何针对你的桶或 IAM 的 credentials。
* **你自己的 credentials** 用于授予每一项 grant：在 `init --managed` 时授予 executor 的 access grant，在 `prepare` 时创建桶和 role，在 `approve` 时提供每次平台 update 的 permission 部分，以及在 `teardown` 时执行清理。

## 连接器 的功能 {#what-the-connector-can-do}

### 出站连接 {#outbound-connections}

以下是连接器建立的全部连接。所有连接均由您的环境内部发起。
...
| 您的 ClickHouse 实例       | ClickHouse 原生协议                                         | 以 `pcm_scraper` 和 `pcm_troubleshooter` 身份执行只读查询，以及抓取器的日志刷写语句 (参见[授权](#clickhouse-grants)) ；executor 不会建立任何 ClickHouse 连接                                                                  |
...
| Amazon ECR             | HTTPS，使用 executor 的环境 AWS 身份，或其为镜像检查所 assume 的只读拉取 role | 当平台同步或创建操作从 ECR 拉取 chart 时进行 registry 登录；平台更新期间调用 `DescribeImages` 确认镜像存在                                                                                                                 |
| AWS STS                | HTTPS，使用 executor 所在节点或主机的环境 AWS 身份                     | 获取 ECR 登录所需的凭据，以及 assume 您为镜像检查注册的只读拉取 role。ECR 和 STS 是 executor 仅有的云 API 调用                                                                                                              |
...
预配会为每个观测组件创建一个只读用户。用户通过 `IDENTIFIED WITH bcrypt_hash` 创建，因此预配 SQL 中仅包含加盐的 bcrypt 哈希。明文密码仅保存在守护进程运行时读取的凭据文件中。每条 `CREATE USER` 之后的 `ALTER USER` 语句用于在指定 `--force` 时轮换哈希。executor 没有 ClickHouse 用户，也从不连接 ClickHouse。授权如下，使用默认表集：
...
由于抓取查询会通过 `clusterAllReplicas()` 包装每个系统表，因此需要 `READ ON REMOTE`。`SYSTEM FLUSH LOGS` 是唯一的系统级授权。它会强制 `*_log` 系统表将已缓冲的条目持久化，以便抓取能看到最新数据；它不能读取或修改任何内容。ClickHouse 仅接受在全局范围内授予该特权，并且仅对日志表执行该特权，因此授予范围比实际能力更广。
...
`clicklink clctl platform approve` 会使用你的凭据应用平台更新中与权限相关的那一半内容：命名空间、CustomResourceDefinitions、ServiceAccounts、集群角色与 ClusterRoleBindings、Roles 与 RoleBindings、webhook 与 admission 配置、PriorityClasses，以及存储类。随后它会确保 `pcm-platform` ServiceAccount、其集群角色、`pcm-platform-scope-guard` ValidatingAdmissionPolicy 以及相应绑定均已就绪，并签发一个有效期为 2 小时的绑定 token。
...
* **没有授予权限的权限。** 执行 `approve` 时，平台的命名空间、CRD、ServiceAccount、RBAC、webhook 与 admission 配置、PriorityClass 和存储类均以你的凭据应用。executor的平台身份无法提权、绑定或冒充他人。在服务命名空间内，executor只能创建授予其自身已持有权限的 Role。
* **没有访问你的桶或 IAM 的凭据。** `clicklink clctl executor prepare` 创建桶和按服务划分的角色，`clicklink clctl executor teardown` 将其移除，两者均在你的凭据下执行。该角色只能由相应服务的 Pod (容器组) 担任。executor仅在两类调用中使用其节点或主机的环境 AWS 身份：拉取 chart 时通过 STS 登录 Amazon ECR；以及在平台更新的镜像检查中担任只读拉取角色。它无法读取或删除你的数据。
...
* **不存在入站访问。** ClickHouse Cloud 绝不会向你的环境发起连接。唯一的命令通道是两条出站 WebSocket。除非你启用的支持会话处于活动状态，否则故障排查工具会拒绝一切命令。executor只接受你委托 ClickHouse 管理的服务的生命周期命令，且不超出上述范围。即使在会话期间，故障排查工具的触及范围在两侧同样受限。ClickHouse 查询仅限于表允许列表，该列表拒绝 `query_log` 和 `text_log`，因此二者永远不会被授权。Kubernetes 访问仅限于命名空间级 Role 所授予的只读视图和 pod (容器组) 日志。

## 需要您采取操作的事项 {#what-requires-your-action}

* **启用托管模式。** 只有在您于 `init` 时启用托管模式之后，executor 才会运行，其 Kubernetes grant 才会存在；该 grant 以 cluster admin 权限应用。请参阅[托管服务](/zh/products/bring-your-own-cloud/connector/managed-services#enable-managed-mode)。
...
* **部署身份。** mTLS 客户端证书的通用名称即为你的组织 ID，并绑定了一个指向你的端点主机的 DNS name，因此每一次 API 连接都可归属到你的组织。续订自动完成且在 daemon 内进行；不会有任何 operator 接触密钥材料。
...
* **操作者身份。** Gateway 调用归属于 operator 的 OIDC ID token 所证明的电子邮件地址，并针对你的身份提供商的 JWKS 进行验证。只要存在 token，就绝不信任自行上报的名称。
...
* **平台审批。** 你所批准的 bundle 的摘要会记录在 `pcm-platform` ServiceAccount 上，并且你的 credentials 所应用的每个 object 都带有内容哈希，executor 在写入任何内容之前都会加以校验。摘要不一致的 bundle 将被拒绝。
* **审计追踪。** 每条 session 命令，无论被接受还是被阻止，都会连同来自命令通道的组织身份一起追加到 NDJSON audit log 中。Gateway 调用会以结构化的 `clctl.gateway` 行记录在故障排查工具日志中，并附带已证明的 operator 电子邮件地址。虚拟机本地 session 变更会在 `session.json` 中记录主机用户。可使用 `clicklink clctl troubleshoot audit tail` 读取 audit log；参见 [CLI reference](/zh/products/bring-your-own-cloud/connector/reference/cli)。

## 数据最小化默认设置 {#data-minimization-defaults}

* **`query_log` 默认不参与抓取。** 其列中保存的是带字面量值的原始 SQL，可能包含个人数据或机密信息，因此除非你主动添加，否则这些数据不会离开你的边界。
