...
<Note>
  此处描述的托管模式工作流需要由你的客户团队确认[相匹配的命令行客户端与部署支持](/zh/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)。公开发布的 `v0.16.0` 既不支持 `init --managed`，也不支持 `init --no-managed`，并且不包含 customer-create 工作流或 `platform` 命令。下文所述权限描述的是这些支持可用之后的工作流。
</Note>

## Identities {#identities}

托管模式 enrollment、客户 service 创建和平台批准仅在[受支持的构建和部署](/zh/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)上使用这些 identities。
...
* **`pcm-platform`** 是 executor 用于平台 updates 的身份。你执行 `clicklink clctl platform approve` 会为其签发有效期 2 小时的令牌，且不会续期；在两次批准之间，executor 对平台层没有写入 access。
...
* **连接器 EC2 虚拟机的 instance profile** 用于直接的平台 access：在 chart 登录和镜像检查时 assume 你环境中的只读 ECR puller role。批准用的命令行客户端使用同一条 instance-profile 路径；workstation 上的 AWS profile 或 SSO 凭证无法替代它。其他 ECR chart registry 使用 ambient AWS 凭证。不涉及任何 S3 或 IAM 调用。Image pulls 发生在你的节点上，使用节点各自的 pull role。executor 不持有任何针对你的桶或 IAM 的凭证。参见 [registry credentials](/zh/products/bring-your-own-cloud/connector/platform-updates#registry-credentials)。
* **你自己的凭证** 用于授予每一项 授权：在 `init --managed` 时授予 executor 的 access 授权，在 `prepare` 时创建桶和 role，在 `approve` 时提供每次平台 update 的 permission 部分，以及在 `teardown` 时执行清理。
...
下文的 `init --managed` 工作流在公开的 `v0.16.0` 中尚不可用；使用前请确认[托管模式的可用性](/zh/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)。
...
公开的 `v0.16.0` 命令行客户端没有 `platform` 命令；该批准模型需要[命令行客户端与部署端均提供相应支持](/zh/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)。
...
* **不存在入站访问。** ClickHouse Cloud 绝不会向你的环境发起连接。唯一的命令通道是两条出站 WebSocket。除非你启用的 support session 处于活动状态，否则故障排查工具会拒绝一切命令。executor只接受你委托 ClickHouse 管理的服务的生命周期命令，且不超出上述范围。即使在会话期间，故障排查工具的触及范围在两侧同样受限。ClickHouse 查询仅限于表允许列表，该列表拒绝 `query_log` 和 `text_log`，因此二者永远不会被授权。Kubernetes 访问仅限于命名空间级 Role 所授予的只读视图和 pod (容器组) 日志。

## 需要您采取操作的事项 {#what-requires-your-action}

在使用下文的托管模式 onboarding、服务创建或平台审批命令之前，请与您的客户团队确认[可用性](/zh/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)。这些工作流在公开发布的 `v0.16.0` 中不可用。
