...
托管模式的 enrollment、客户创建以及平台批准流程需要[由你的客户团队确认支持](/zh/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)；在公开发布的 `v0.16.0` 中尚不可用。
...
有两件事需要你来操作。创建服务需要你执行 `clicklink clctl executor prepare` 和 `clicklink clctl instances create`，因为只有你才能创建桶和 IAM role。集群中平台组件的更新同样要等你操作：executor 会暂存每个待批的支持包，你执行 `clicklink clctl platform approve` 批准已暂存的支持包，并为其签发一个有效期 2 小时的 token；参见[平台更新](/zh/products/bring-your-own-cloud/connector/platform-updates)。

executor 的所有操作都通过 Kubernetes API 完成。其写入受 admission 策略限制，只能作用于带服务前缀 (默认为 `ns-`) 的命名空间，以及自身命名空间中的三项内容：续订自己的 token、服务 registry ConfigMap，以及在 Kubernetes 上存放于 `clicklink-mtls` Secret 中的已续订客户端证书。而它对同类资源的读取则是集群级的。已批准的平台更新以独立的 `pcm-platform` 身份运行，仅限于平台命名空间。你自行运维的集群绝不会被更改：scraper 和 故障排查工具均为只读。确切的作用域参见[特权模型](/zh/products/bring-your-own-cloud/connector/reference/privilege-model)页面。

### 哪些数据会离开我的环境？ {#what-data-leaves-my-environment}

共有三个出口会向外发送数据，且全部通过连接器自行建立的出站连接：抓取路径、executor，以及你启用的 支持会话。

抓取器会发送一组固定系统表的查询结果：默认包括 `metric_log`、`asynchronous_metric_log`、`tables`、`warnings` 和 `server_settings`。它还会发送健康与状态心跳、instance 与 backup 状态，以及连接器自身的 metrics。默认抓取集合不包含 `system.query_log`，因此除非你主动添加，否则原始 SQL 文本及其中的任何 literals 或个人数据都不会经由抓取路径外发。在 session 期间，默认的表 允许列表 包含 `system.processes`，它会显示正在执行的查询文本；如果这些内容必须保密，请将其从 允许列表 中移除。

executor 会发送它执行的每条命令的结果，以及每个 managed service 的 state 和副本数量。其心跳会报告命令通道是否已连接、平台 component versions、最后应用的 bundle 摘要，以及最近一次平台更新的结果。在 VM 安装方式下，心跳还会报告其 Kubernetes 访问 bundle 处于 `ok`、`expiring` 还是 `expired` 状态。当你创建 service 时，`clicklink clctl instances create` 会发送你的环境 ID、云与区域、service name，以及其存储桶名称和 IAM role ARN。它会发送 default user 密码的两个哈希值；密码本身绝不会外发。

在 [支持会话](/zh/products/bring-your-own-cloud/connector/support-sessions) 处于活跃状态时，故障排查工具 还会返回命令输出。该输出仅限于 允许列表 中的 ClickHouse 表和 只读 Kubernetes 视图，其中包括 pod (容器组) logs。输出在发送前会先经过 redaction (内置的 IP、credentials、令牌和密钥 patterns，以及你自定义的规则) 。你的表数据、backups 和 query history (`system.query_log`、`system.text_log`) 始终保留在你的环境中。确实会外发的有三类：
...
<Note>
  公开发布的 `v0.16.0` 命令行客户端既不支持 `--managed` 也不支持 `--no-managed`。下文提到的安装时选项需要使用带有[托管模式支持](/zh/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)的 build。停止已有的 executor 只需按照下文的 Helm 或 systemd 指令操作，无需上述任何 init 标志。
</Note>

以非托管模式运行。安装时向 `clicklink clctl init` 传入 `--no-managed`，或在出现提示时选择否。这样只会部署抓取器和故障排查工具，ClickHouse 便无法创建、更改或删除 service。连接器仍会预置只读 ClickHouse 用户和 ServiceAccount；参见[特权模型](/zh/products/bring-your-own-cloud/connector/reference/privilege-model)。

对于已完成的安装，请停止 executor 并撤销其集群管理员授权。要停止它，可在 Helm values 中设置 `executor.enabled: false` 并运行 `helm upgrade`，或在 VM 上运行 `systemctl disable --now clicklink-executor`。随后撤销该授权。最后一条命令会删除 ServiceAccount 本身；VM 安装对应的命名空间为 `clicklink-system`，Kubernetes 安装则为连接器所在的命名空间：
...
ClickHouse 已创建的 service 会继续运行，但 ClickHouse Cloud 无法再对其进行扩缩容、升级、备份或删除，针对它们的待处理命令也会失败。除非你运行 `clicklink clctl platform approve`，否则平台更新将始终不会生效。该命令未执行时，平台 layer 会处于冻结状态，而托管模式的其余部分仍照常运行。
...
### 删除某个 service 时会删除哪些内容？ {#what-is-deleted}

executor 会停止该 service、移除其 Kubernetes 资源，并删除其命名空间。命名空间被删除后，executor 即报告该 service 已终止。Kubernetes 之外的一切都会保留：数据桶、备份桶以及该 service 的 IAM role。executor 不持有你的桶或 IAM 的任何 credentials，因此不会改动 Kubernetes 之外的任何内容。

其余部分需要你使用自己的 credentials 运行 `clicklink clctl executor teardown` 来清除。默认情况下，该命令会移除 IAM role，但保留数据和备份；如需删除它们，必须显式指定相应 flags。参见 [delete a service](/zh/products/bring-your-own-cloud/connector/managed-services#delete-a-service)。

### 我的 default 用户密码在哪里？ {#default-user-password}

创建客户服务需要[匹配的 命令行客户端 与部署支持](/zh/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)，在公开发布的 `v0.16.0` 中不可用。
...
2. **停止托管模式。** 按照[如何阻止 ClickHouse 更改我的集群？](#stop-managed-mode)中的说明停止 executor 并撤销其 grant。平台更新本身就需要你逐项批准，因此不予批准即可使其停止。
...
* 如果使用[支持平台批准的 build](/zh/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)，平台 bundle 可通过带外方式移交，并以文件形式传递给 `clicklink clctl platform approve --bundle`。公开的 `v0.16.0` 不包含 `platform` 命令。
...
会话期间执行的每条命令，无论获准还是被阻止，都会以以换行分隔的 JSON 格式追加写入 `/var/log/clicklink/troubleshoot-audit.log`。每条记录都会记录 `submitted_by` (命令通道上的 Organization ID) 、命令文本、目标 instance、执行结果，以及被拒绝命令的 `blocked_by`。网关调用 (包括被拒绝的调用) 会以结构化的 `clctl.gateway` 行记录在 故障排查工具 自身的日志中，并归属于由令牌验证的操作员邮箱地址，绝不采用自行申报的姓名。VM 上的本地会话变更会将发起调用的主机用户记录为 `/var/lib/clicklink/session.json` 中的 `enabled_by`。
