...
### 如何阻止 ClickHouse 更改我的 cluster？ {#stop-managed-mode}
...
对于已完成的安装，请停止 executor 并撤销其 cluster 管理员授权。要停止它，可在 Helm values 中设置 `executor.enabled: false` 并运行 `helm upgrade`，或在 VM 上运行 `systemctl disable --now clicklink-executor`。随后撤销该授权。最后一条命令会删除 ServiceAccount 本身；VM 安装对应的命名空间为 `clicklink-system`，Kubernetes 安装则为连接器所在的命名空间：
