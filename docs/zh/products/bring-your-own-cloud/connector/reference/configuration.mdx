...
[托管模式](/zh/products/bring-your-own-cloud/connector/managed-services)下的 executor：运行在 PersistentVolumeClaim 之上的单副本 Deployment。`init --managed` 会将其预置为启用状态，并填入 cluster identity。
...
| `executor.syncInterval`                         | `30s`                                 | 从 cluster 刷新 service 状态的频率。                                                                                                                                                      |
| `executor.apiPort`                              | `9999`                                | 本地 API 端口。在 pod (容器组) 内绑定到 `127.0.0.1`，不通过 Service 对外暴露；可通过 `kubectl port-forward` 访问。                                                                                           |
| `executor.ports.health`                         | `8086`                                | 健康状况与 metrics 端口。                                                                                                                                                                |
| `executor.cluster.name`                         | `""` (由 `init` 填入)                    | 该 executor 所管理的那个 EKS cluster 的名称。启用 executor 时必填。                                                                                                                               |
| `executor.cluster.region`                       | `""` (由 `init` 填入)                    | 该 cluster 所在的 AWS 区域。必填。                                                                                                                                                         |
| `executor.cluster.accountId`                    | `""` (由 `init` 填入)                    | 该 cluster 所属的 AWS 账户。必填；同时也接受蛇形命名的 `account_id` 写法。                                                                                                                              |
| `executor.cluster.inCluster`                    | `false` (`init` 写入 `true`)            | 以 pod (容器组) 的 ServiceAccount 身份进行认证。这正是 Kubernetes 形态的意义所在：pod (容器组) 内不存在访问包。                                                                                         |
| `executor.cluster.delete_instance_data.enabled` | `false`                               | 在不带 `--delete-data` 的情况下，`clicklink clctl executor teardown` 是否删除已下线 service 的数据。`init` 不会设置该项。其他按 cluster 划分的 key 均以二进制程序使用的蛇形命名原样透传。                                           |
| `executor.targetCluster`                        | `""` (由 `cluster.name` 推导)            | inbound 命令分发到的 cluster。请勿设置；取值若与 `cluster.name` 不一致，渲染时会失败。                                                                                                                      |
...
| `executor.serviceAccount.create`                | `false`                               | 图表是否创建 executor 的 ServiceAccount。默认关闭：`init --managed` 会在图表之外按固定名称授予 executor 的 RBAC，图表只负责绑定。                                                                       |
...
| `executor.persistence.storageClass`             | `""` (cluster 默认值)                    | 该 claim 使用的存储类；`init` 会根据 `--storage-class` 或检测到的默认值填入。                                                                                                                          |
| `executor.persistence.size`                     | `5Gi`                                 | claim 大小。                                                                                                                                                                        |
| `executor.persistence.accessModes`              | `[ReadWriteOnce]`                     | claim 的 access mode。                                                                                                                                                             |

`executor.cluster` 是单个映射：每个 executor 只对应一个 cluster。使用 `clusters` 列表会导致渲染失败。
