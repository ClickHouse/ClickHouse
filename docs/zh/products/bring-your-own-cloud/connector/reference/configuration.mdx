...
下述 `init --managed` 行为需要[支持托管模式的构建版本](/zh/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)；公开发布的 `v0.16.0` 并未提供该标志。
...
| Key                                             | Default                               | 含义                                                                                                           |
| ----------------------------------------------- | ------------------------------------- | ------------------------------------------------------------------------------------------------------------ |
| `executor.enabled`                              | `false` (`init --managed` 会写入 `true`) | 运行 executor 守护进程。                                                                                          |
| `executor.cluster.name`                         | 由 `init` 写入                           | 该 executor 所管理的那个 EKS 集群的名称。启用 executor 时必填。                                                           |
| `executor.cluster.region`                       | 由 `init` 写入                           | 该 集群所在的 AWS 区域。必填。                                                                                     |
| `executor.cluster.account_id`                   | 由 `init` 写入                           | 该 集群所属的 AWS 账户。必填。                                                                                     |
...
| `executor.cluster.delete_instance_data.enabled` | `false`                               | 在不带 `--delete-data` 的情况下，`clicklink clctl executor teardown` 是否删除已退役 service 的数据。executor 守护进程本身不会删除任何数据。 |
| `executor.target_cluster`                       | `executor.cluster.name`               | inbound 命令分发到的 集群。请勿设置；填写任何其他值都会导致启动失败。                                                                 |
| `executor.poll_interval`                        | `5m`                                  | 轮询间隔。必须为正值。                                                                                         |
| `executor.sync_interval`                        | `30s`                                 | 从 集群刷新 service status 的频率。必须为正值。                                                                       |
...
`executor.cluster` 是单个映射：每个 executor 只对应一个 集群。若配置中只有一个条目的旧式 `clusters:` 列表且不存在 `cluster:` 映射，加载器会将其折叠为 `cluster`。在 executor 已启用的情况下，如果列表包含多个条目，或列表与 `cluster:` 键同时存在，加载器将拒绝加载。
...
下文的托管模式初始化与平台审批集成需要[配套的命令行客户端与 chart 支持](/zh/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)。公开发布的 `v0.16.0` 既不包含 `init --managed`，也不包含 `platform` 命令。

[托管模式](/zh/products/bring-your-own-cloud/connector/managed-services)下的 executor：运行在 PersistentVolumeClaim 之上的单副本 Deployment。`init --managed` 会将其预置为启用状态，并填入 集群 identity。
...
| `executor.cluster.name`                         | `""` (由 `init` 填入)                   | 该 executor 所管理的那个 EKS 集群的名称。启用 executor 时必填。                                                                                                                               |
...
| `executor.cluster.inCluster`                    | `false` (`init` 写入 `true`)           | 以 pod (容器组) 的 ServiceAccount 身份进行认证。这正是 Kubernetes 形态的意义所在：pod (容器组) 内不存在访问包。                                                                                                    |
...
| `executor.serviceAccount.create`                | `false`                              | chart 是否创建 executor 的 ServiceAccount。默认关闭：`init --managed` 会在 chart 之外按固定名称授予 executor 的 RBAC，chart 只负责绑定。                                                                       |
| `executor.serviceAccount.name`                  | `pcm-executor`                       | executor 运行时使用的 ServiceAccount。当 `create` 为 `false` 时必填。                                                                                                                         |
| `executor.persistence.enabled`                  | `true`                               | 使用 PersistentVolumeClaim 承载 `/var/lib/clicklink` (命令 database 与 实例 registry) 。                                                                                                   |
| `executor.persistence.storageClass`             | `""` (集群 默认值)                   | 该 claim 使用的存储类；`init` 会根据 `--storage-class` 或检测到的默认值填入。                                                                                                                          |
| `executor.persistence.size`                     | `5Gi`                                | claim 大小。                                                                                                                                                                        |
| `executor.persistence.accessModes`              | `[ReadWriteOnce]`                    | claim 的 access mode。                                                                                                                                                             |
...
| `networkPolicy.apiserverCIDRs`        | `[]` (在[受支持的构建版本](/zh/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)上，由 `init --managed` 从集群 VPC 填入) | Kubernetes API server 的 CIDR。留空不会生成 API server egress 规则；在托管 Kubernetes 环境中，守护进程在设置此项前首次请求 token 时会失败。请固定 VPC 的地址段，而不是 API server 自身的地址，后者会在集群的生命周期内发生变化。                                                                                                                    |