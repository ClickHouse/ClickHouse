...
<Note>
  公开发布的 `v0.16.0` 命令行客户端既没有 `--managed` 也没有 `--no-managed`。下文的 managed 模式标志及行为需要更新的构建版本，并需由客户团队确认已具备相应的部署支持；请参阅 [managed 模式可用性](/zh/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)。
</Note>

通过注册标记、已保存的注册包或带外签名证书引导连接器。一次调用即可暂存配置、配置 ClickHouse 访问权限、获取 mTLS 客户端证书、部署 (Helm 图表或 systemd 单元) 并验证运行状况。使用 `--managed` 时，它还会为检测到的 EKS cluster 启用 executor，并授予该 executor 的集群级访问权限。重复运行是安全的：`init` 会保留配置和集群 UUID，以原子方式覆盖凭据；除非传入 `--force`，否则会复用现有客户端密钥。有关完整流程，请参阅[入门配置](/zh/products/bring-your-own-cloud/connector/onboarding)；有关 executor 的作用，请参阅[托管服务](/zh/products/bring-your-own-cloud/connector/managed-services)。
...
| `--managed`                 | 启用托管模式：运行 executor，由 ClickHouse Cloud 管理该 cluster 中 ClickHouse 服务的生命周期；跳过相应提示。需要 Amazon EKS cluster。在公开发布的 `v0.16.0` 中不可用；请参见 [可用性](/zh/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)。 |
| `--no-managed`              | 不启用托管模式 (即不运行 executor) ；跳过相应提示。在公开发布的 `v0.16.0` 中不可用；请参见 [可用性](/zh/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)。                                                                     |
| `--force`                   | 覆盖已有的 config 或 覆盖文件 并重新生成客户端密钥；同时表示确认替换尚未过期的自动签发证书。即使使用 `--force`，集群 UUID 仍会保留。                                                                                                                             |
...
下面的 `init --managed` 工作流需要[支持 managed 模式的构建版本](/zh/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)；公开发布的 `v0.16.0` 并未提供该标志。

`init --managed` 会使用你的凭据为 executor 应用集群级授权。若无法完成，`init` 会打印出需通过具备集群管理员权限的 kubeconfig 执行的命令：在 VM 上只需执行 `access apply` (bundle 已就绪) ，在 Kubernetes 上则先执行 `access grant`，再执行 `access apply`。参见 [onboarding](/zh/products/bring-your-own-cloud/connector/onboarding#install-and-enroll)。
...
在一次幂等运行中完成 managed service 创建所需的、位于你这一侧的全部准备工作，并在遇到第一个失败时停止。它会选取或校验 service 名称，创建数据桶和备份桶以及 IAM role (或校验你自带的 role) ，并在 VM 上为该 service 命名空间渲染并应用 executor 的访问包。在 Kubernetes 上则会跳过访问包：executor 以其 pod ServiceAccount 身份运行。随后它会生成 `default` USER 的密码 (仅显示一次) ，并写出供 [`instances create`](#instances-create) 使用的创建请求体。参见 [managed services](/zh/products/bring-your-own-cloud/connector/managed-services#create-a-service)。
...
针对 ClickHouse Cloud 中已删除的 service，移除 `prepare` 为其创建的资源，包括：IAM role、`prepare` 创建的桶、已记录的名称、executor 中该 service 的记录，以及在 VM 上该 service 的集群角色和 ClusterRoleBinding、本地访问包和 registry entry。执行时 executor 必须处于运行状态，且 service 状态为 `terminated` (或 `stale`) ；若 service 命名空间中仍存在 ClickHouse cluster，则该命令会拒绝执行。除非显式指定，否则数据和备份会被保留，保留下来的桶会被重新打上 `clicklink:retained-from=<instance>` 标签。该命令会先读取全部信息，然后才开始删除。参见[删除 service](/zh/products/bring-your-own-cloud/connector/managed-services#delete-a-service)。
...
此客户创建工作流尚未在公开发行版中提供，需要命令行客户端、连接器端点以及 ClickHouse Cloud 的支持。使用前请确认 [托管模式可用性](/zh/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)。

通过你的连接器端点、使用连接器自身的凭证，向 ClickHouse Cloud 提交一次已准备好的创建请求。命令会打印 ClickHouse Cloud 分配的可读名称、状态，以及一条使用你的 Service 名称的 `watch:` 提示。创建请求正文来自 `prepare` 写入的文件，或由 `--deployed-name`、`--data-bucket`、`--backup-bucket` 和 `--irsa-role-arn` 这四个标志共同提供。使用相同输入重试将返回首次创建的结果。
...
以下三个子命令会将命令直接提交到 executor 的本地 API，不经过 ClickHouse Cloud。请仅在 ClickHouse account team 要求时运行。每个子命令都需要 `--name <name>` 以及 cluster (`--cluster`、`CLCTL_CLUSTER` 或 `~/.clicklink/clctl.yaml`) ，并会以 JSON 形式打印该命令。将其 `id` 传给 [`commands get`](#commands-get) 即可跟踪其状态；命令失败时，其 `result` 中会包含对应的 error。
...
原地修改单个 service。`--create-nlb` 是唯一可用的 patch，它会以 Kubernetes `Service` 的形式预配一个内部 NLB，使 cluster 外部的抓取器能够访问这些服务器，仅支持 AWS。executor 会在下一次同步时从 load balancer 回填抓取器的 host。若未指定 `--create-nlb`，该命令将以 `nothing to patch` 拒绝执行。
...
公开发布的 `v0.16.0` 命令行客户端不包含 `platform` 命令，因此该版本中既不支持 VM 审批流程，也不支持 Kubernetes 审批流程。Kubernetes 审批还额外需要下文所述的命令行客户端与 chart 对 Secret 交付的支持。在使用任一流程之前，请先确认[托管模式可用性](/zh/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)。

使用你自己的集群凭证授权一次平台变更。该命令会读取 executor 暂存的 bundle、计算其哈希，并以与 executor 完全相同的方式渲染其中的每个 chart。它使用你选择的 kube context 应用权限部分，确保 executor 的 `pcm-platform` 身份存在，并为其签发 token。该命令不会向你的连接器端点发起任何请求。权限部分包括 Namespaces、CustomResourceDefinitions、ServiceAccounts、集群角色及其 bindings、Roles 及其 bindings、webhook 与 admission 配置、PriorityClasses 以及存储类。随后 executor 仅应用 workload 部分，且仅在该 token 有效期内执行。参见[平台更新](/zh/products/bring-your-own-cloud/connector/platform-updates)。
...
公开的 `v0.16.0` 命令行客户端没有 `platform` 命令，也不包含 `platform reset`；参见 [托管模式可用性](/zh/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)。
...
| 标志                                                                                          | 描述                                                                                                                                                                    |
| ------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--provider <name>`                                                                         | 执行通道：对于远程 VM，可使用 `ssh`、`aws` (SSM) 或 `gcp` (IAP)；在目标 VM 上运行时，使用 `local`。未显式设置时，将根据各提供商相关的标志推断。未设置任何通道标志时默认为 `aws`，且绝不会推断为 `local`，因此在 VM 上运行时务必传入 `--provider local`。 |
