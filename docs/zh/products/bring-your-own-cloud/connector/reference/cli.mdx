...
title: '命令行客户端参考'
sidebarTitle: '命令行客户端'
description: 'clicklink clctl 命令参考：init、preflight、支持会话、gateway trust、审计和访问凭据预配'
...
连接器 以名为 `clicklink` 的单个二进制文件形式提供；可通过 `clicklink clctl` 运行相关命令。本页介绍安装和日常运维中使用的命令。对任意命令运行 `--help`，即可查看完整帮助文本。`troubleshoot` 和 `preflight` 子树中的 标志 还可通过 `CLCTL_*` 环境变量 (名称见各 标志 的帮助输出) 或 `~/.clicklink/clctl.yaml` 提供。
