...
通过你的连接器端点、使用连接器自身的凭据，向 ClickHouse Cloud 提交一次已准备好的创建请求。命令会打印 ClickHouse Cloud 分配的可读名称、状态，以及一条使用你的服务名称的 `watch:` 提示。创建请求正文来自 `prepare` 写入的文件，或由 `--deployed-name`、`--data-bucket`、`--backup-bucket` 和 `--irsa-role-arn` 这四个标志共同提供。使用相同输入重试将返回首次创建的结果。
...
| `--config <path>`                                                                                | 连接器配置文件 (环境变量 `CLCTL_EXECUTOR_CONFIG`；文件存在时默认为 `/etc/clicklink/config.yaml`) 。用于提供端点、其凭据以及环境 identity。                                                           |
| `--connector-namespace <ns>`                                                                     | 仅适用于 Kubernetes，且当该主机上没有连接器配置或凭据文件时：提供这些内容的 ConfigMap 与 Secret 所在的连接器命名空间 (默认 `clicklink`) 。每次集群读取都会有提示输出。                                                       |
| `--connector-configmap <name>`                                                                   | 该命名空间中保存 executor `config.yaml` 的 ConfigMap (默认 `clicklink-connector-executor`) 。                                                                                |
| `--hmac-secret-name <name>`、`--mtls-secret-name <name>`                                          | 当相应文件不存在时，保存 API 凭据和客户端证书的 Secret (默认为 `clicklink-hmac` 和 `clicklink-mtls`) 。证书对保存在内存中。                                                                          |
...
使用你自己的集群凭证授权一次平台变更。该命令会读取 executor 暂存的 bundle、计算其哈希，并以与 executor 完全相同的方式渲染其中的每个 chart。它使用你选择的 kube context 应用权限部分，确保 executor 的 `pcm-platform` 身份存在，并为其签发 token。该命令不会向你的连接器端点发起任何请求。若 bundle 中的某个 chart 位于亚马逊 ECR 中，则需要具备读取权限的 AWS 凭证。权限部分包括 Namespaces、CustomResourceDefinitions、ServiceAccounts、集群角色及其 bindings、Roles 及其 bindings、webhook 与 admission 配置、PriorityClasses 以及存储类。随后 executor 仅应用 workload 部分，且仅在该 token 有效期内执行。参见[平台更新](/zh/products/bring-your-own-cloud/connector/platform-updates)。
