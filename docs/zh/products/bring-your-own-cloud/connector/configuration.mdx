...
本页面介绍安装连接器后最常需要进行的配置更改。有关各项 key 的默认值及其含义，请参阅[配置参考](/zh/products/bring-your-own-cloud/connector/reference/configuration)；有关命令行 flags，请参阅 [CLI 参考](/zh/products/bring-your-own-cloud/connector/reference/cli)。创建、扩缩容和删除 services 不属于配置内容，请参阅 [managed services](/zh/products/bring-your-own-cloud/connector/managed-services)。
...
<Note>
  公开发布的 `v0.16.0` 命令行客户端既没有 `--managed` 也没有 `--no-managed`。在使用其中任一标志更改配置之前，请先确认[托管模式可用性](/zh/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)，并从你的客户团队获取对应的 build。
</Note>

托管模式会运行 executor，由它将 ClickHouse Cloud 驱动的服务生命周期应用到你的集群。`init` 会在注册时启用该模式，并填入集群 identity (参见[启用托管模式](/zh/products/bring-your-own-cloud/connector/managed-services#enable-managed-mode)) ；下面列出的这些 key 则是你之后可能需要再调整的。若要更改已配置的状态，可带上 `--force` 并搭配 `--managed` 或 `--no-managed` 重新运行 `init`。与每次运行 `init` 一样，这里同样需要一个 entry point：如果你保留了 bundle，就使用 `--handoff`；否则使用 `--enroll` 搭配新的 token，详见[重新运行与 recovery](/zh/products/bring-your-own-cloud/connector/operations#re-runs-and-recovery)。

在 VM 上，`--no-managed` 只会在配置中禁用 executor，但不会停止或禁用已在运行的 systemd unit。请在更改配置之前显式将其停止：

```bash
sudo systemctl disable --now clicklink-executor
```

在 Kubernetes 上，在配置值中设置 `executor.enabled: false` 并通过 `helm upgrade` 应用该变更，即可移除 executor 的 Deployment。在这两种目标环境中，集群 grant 都会一直保留，直到你将其移除；请按[停止托管模式](/zh/products/bring-your-own-cloud/connector/reference/faq#stop-managed-mode)中的说明一并移除该 access。
...
<Tabs>
  <Tab title="Kubernetes" id="managed-mode-kubernetes">
    ...
    * **`cluster.namespacePrefix`** 由所有组件共用，executor 的 grant 就限定在该 prefix 范围内。安装后若要修改，必须重新执行一次 grant。
  ...
  <Tab title="Linux VM" id="managed-mode-vm">
    ...
    * **`namespace_prefix`** 是所有组件共享的顶层键，也是 executor grant 所限定的作用前缀。安装后若要更改，需重新执行 grant。
...
executor 创建的 service 会自行注册，因此在托管模式下无需在此手动添加。不运行 executor 的独立连接器部署用于观测你自行运行的 ClickHouse；当两个部署共用同一个实例时，请在其 `init` 时传入 `--ch-user-suffix`。本节适用于该部署。`instances` 下的每个条目对应一个连接器读取数据的 ClickHouse 原生协议端点，包括 `host`、`port`、`database`、`secure`、`cluster`，在 Kubernetes 上还包括 `namespace`；`max_open_conns` 与 `max_idle_conns` 用于限制连接池的规模。凭据绝不保存在配置中；每个组件都从预配过程创建的访问包中获取各自的只读 ClickHouse 用户。

<Tabs>
  <Tab title="Kubernetes" id="instances-kubernetes">
    将实例添加到 `clicklink-values.yaml` 中顶层的 `instances` 映射内 (字段与 VM registry 相同) ，并将其命名空间添加到 `networkPolicy.clickhouseNamespaces`，根据命名空间的 `kubernetes.io/metadata.name` 标签匹配。图表会将该映射渲染为两个守护进程都会读取的 `clicklink-instance-registry` ConfigMap。这仅在 `executor.enabled` 为 `false` 时有效；启用 executor 后，渲染会失败，因为该 ConfigMap 由 executor 拥有。
  ...
  <Tab title="Linux VM" id="instances-vm">
    ...
    然后以 root 身份在主机上为各组件预配访问权限。在 VM 上，除非传入 `--ch-user-via cr`，否则必须提供管理员密码；请使用 `--ch-admin-password-stdin` 通过管道传入。每条命令都会应用 ClickHouse 授权并重启对应的守护进程 (可使用 `--skip-restart` 跳过重启) ：
...
运维人员电子邮件地址允许列表限制着网关管理的会话：发送至会话网关的每个请求都必须携带短时有效的 OIDC ID 标记，且其中经认证的电子邮件地址必须在列表中。允许列表为空时，网关将关闭，任何人都无法通过它打开会话。在 VM 上，主机的 root 用户也可以通过本地会话文件直接管理会话；允许列表仅约束经由网关的访问路径。有关完整的信任模型，请参阅[支持会话](/zh/products/bring-your-own-cloud/connector/support-sessions)。

<Tabs>
  <Tab title="Kubernetes" id="allowlist-kubernetes">
    允许列表位于覆盖层中，chart 会将其渲染为 ConfigMap。要更改允许列表，请编辑该列表并运行 `helm upgrade`：
...
## Network policy and 出站 {#network-policy}

在 Kubernetes 上，该图表默认提供一个带出站允许列表的 default-deny NetworkPolicy。NetworkPolicy 对象只有在 CNI 强制执行时才会生效。在具备强制执行能力的 CNI 下，只有集群 DNS 和 ClickHouse 服务所在的命名空间可达，直到两个 CIDR 列表都填写完毕为止。在此之前，连接器无法通过出站访问你的连接器端点或 Kubernetes API server。因此，只有当 `init` 获知连接器端点背后的 CIDR 后，才会将该 policy 以 `enabled` 状态部署。
...
* **`allowEgressCIDRs`** 需要你自行提供：即连接器端点背后的地址范围，可在安装时通过 `--egress-cidrs` 指定，或在提示时输入。所有组件都需要它才能访问你的连接器端点。在托管模式下，如果平台图表位于 Amazon ECR 中，还需一并添加所在区域的 ECR 和 STS 地址范围 (或你的 mirror 所在范围) ；否则在该策略下 executor 拉取图表会失败。
* **`apiserverCIDRs`** 在托管模式下由 `init` 根据 EKS cluster 所在的 VPC 推导得出 (`eks:DescribeCluster` 和 `ec2:DescribeVpcs`) 。所有组件都会发起 Kubernetes 标记请求，executor 也通过 API 驱动 service，因此在强制执行的 CNI 下，未设置该项时这些操作都会被阻断。若 lookup 失败，`init` 会发出警告并将列表留空，由你填入自己 VPC 的 CIDR 块。
...
**稍后启用该策略。** 如果安装时跳过了端点 CIDR，`init` 会预置 `enabled: false` 和 `allowEgressCIDRs: []`，并输出完成该配置所需的步骤。确定 CIDR 后，在 `clicklink-values.yaml` 中设置 `enabled: true` 和 `allowEgressCIDRs`，确认 `apiserverCIDRs` 已包含你 VPC 的地址范围，然后执行 [配置面](#configuration-surfaces) 中给出的 `helm upgrade`。
...
在 VM 上，该文件位于 `/etc/clicklink/redaction-patterns.yaml`。安装程序会为其预置一组初始的活动规则 (第三方 API key、AWS 密钥等) 。您可以按需精简或扩展该集合；安装程序会在升级时保留您修改后的版本。在 Kubernetes 上，将 YAML 放入 ConfigMap，并使用键名 `redaction-patterns.yaml`，然后将 `troubleshooter.redaction.patternsConfigMap` 设置为该 ConfigMap 的名称；chart 会将其挂载到相同路径。
...
下面的托管模式示例需要使用带 `--managed` 的 build，而公开的 `v0.16.0` 并不具备该功能。使用前请先确认[托管模式可用性](/zh/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)；镜像公开发行版并不会增加此项支持。
...
<Tabs>
  <Tab title="Kubernetes" id="storage-kubernetes">
    ...
    executor 的 PVC 保存其命令数据库以及它所管理的 services 的 registry：
  ...
  <Tab title="Linux VM" id="storage-vm">
    当连接器端点不可达时，抓取器会将指标暂存到 `/var/lib/clicklink/buffer`，以实现至少一次投递。默认最多保留 168 小时或 1024 MB 的数据，上传速率限制为 1 MB/s：
    ...
    executor 将其命令数据库保存在 `/var/lib/clicklink/executor.db`，service registry 保存在 `/var/lib/clicklink/instances.yaml`。两者在重启和升级后均会保留；除非传入 `--purge`，否则 `uninstall.sh` 也会保留它们。
