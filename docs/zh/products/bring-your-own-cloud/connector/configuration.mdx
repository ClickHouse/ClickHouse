...
托管模式会运行 executor，由它将 ClickHouse Cloud 驱动的服务生命周期应用到你的集群。`init` 会在 enrollment 时启用该模式，并填入集群 identity (参见[启用托管模式](/zh/products/bring-your-own-cloud/connector/managed-services#enable-managed-mode)) ；下面列出的这些 key 则是你之后可能需要再调整的。若某个连接器安装时未启用托管模式，或想将其关闭，可带上 `--force` 并搭配 `--managed` 或 `--no-managed` 重新运行 `init`。`--no-managed` 会停止 executor，但其集群 grant 仍会保留；请按[停止托管模式](/zh/products/bring-your-own-cloud/connector/reference/faq#stop-managed-mode)中的说明将其移除。与每次运行 `init` 一样，这里同样需要一个 entry point：如果你保留了 bundle，就使用 `--handoff`；否则使用 `--enroll` 搭配新的 token，详见[重新运行与 recovery](/zh/products/bring-your-own-cloud/connector/operations#re-runs-and-recovery)。
