...
切换是唯一会导致停机的步骤，通常不到一分钟。如果配置了[维护窗口](/zh/products/managed-postgres/upgrades#maintenance-windows)，切换会等待该窗口，除非磁盘使用率达到 95%。切换期间，现有连接会断开，进行中的事务会回滚。连接字符串不会改变：DNS 会更新为指向新的主节点，具有标准重连逻辑的应用程序会自动恢复。
...
3. 替换服务器追赶完成后，系统将执行切换；如果配置了维护窗口，则会在该窗口内执行。连接会中断不到一分钟，您的应用会重新连接到同一主机名，使用率将回落至约 45%。
