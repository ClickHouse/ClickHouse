...
<div id="metrics-access">
  ### 如何访问我的 ClickHouse Managed Postgres 实例指标？
</div>
...
ClickHouse Managed Postgres 提供每日自动备份和持续 WAL 归档，支持将数据库恢复到 7 天保留期内的任意时间点。备份存储在 S3 中。
...
<div id="terraform-support">
  ### ClickHouse Managed Postgres 是否支持 Terraform？
</div>

ClickHouse Managed Postgres 目前尚不支持 Terraform。建议使用 ClickHouse Cloud 控制台或 [OpenAPI](/zh/products/managed-postgres/openapi) 创建和管理您的实例。
...
<div id="migration-tools">
  ### 可用于迁移到 ClickHouse Managed Postgres 的工具有哪些？
</div>
