...
实例配置分为三类，便于根据工作负载特征简化基础设施选型。并非所有 AWS 区域都提供所有实例类型。
...
* **CPU 优化型：**专为计算密集型工作负载设计，内存与 CPU 配比为 1:4。支持基于 x86 的 `m8id` 和 `m6id` 系列，以及基于 AWS Graviton 的 ARM64 `m8gd` 和 `m6gd` 系列。最适合高并发事务型工作负载和 CPU 受限的查询。
