...
<Visibility for="humans">
  <View title="Cloud UI">
    ...
    * 使用 `pg_clickhouse` 直接从 Postgres 查询 ClickHouse
    ...
    ## Part 1：NVMe Postgres 入门 {#part-1}
    ...
    ***
    ...
    ## 第二部分：使用 ClickHouse 添加实时分析功能 {#part-2}
    ...
    * **ClickHouse** 用于对数十亿行数据进行亚秒级分析
    ...
    ### 设置 ClickHouse 集成 {#setup-integrate-clickhouse}
    ...
    * 点击 **将数据复制到 ClickHouse**。
    ...
    ### 实际体验数据分析功能 {#analytics-after-integration}
    ...
    ### 比较 Postgres 与 ClickHouse 的性能 {#performance-comparison}
