...
sidebarTitle: '基准测试'
title: '性能基准测试'
description: '比较 ClickHouse Managed Postgres 与 AWS Aurora、RDS 及其他托管 PostgreSQL 服务的性能基准测试'
...
<Info>
  ...
  * 使用标准的 [`pgbench`](https://www.postgresql.org/docs/current/pgbench.html) 测试，对 **ClickHouse Managed Postgres**、AWS RDS (16k 预配置 IOPS) 和 Aurora IO Optimized 进行了**基准测试**
  * **性能**：由 NVMe 存储提供支持的 ClickHouse Managed Postgres 在 IO 密集型工作负载下可实现 **4.3–9 倍性能提升**，在 CPU 密集型场景下快 **12%**
...
**分析**：混合读写工作负载最能体现 NVMe 存储的显著性能优势，也代表了**快速增长的 AI 驱动型工作负载中最贴近真实的场景**——这类场景既需要高吞吐的数据摄取，也需要低延迟读取。**ClickHouse Managed Postgres 在更高并发下达到 19.8K TPS**，表明 NVMe 存储在高负载下依然能够有效扩展。这比 **RDS 和 Aurora 快 4.3-4.5 倍**。网络附加存储方案在写密集型操作中表现吃力；即使已经预配容量，且 Aurora 还启用了 IO Optimized 配置，RDS 和 Aurora 的 TPS 最高也只有 4.4K-4.6K。
...
| 配置        | ClickHouse Managed Postgres | 配备 16k IOPS 的 RDS    | Aurora IO Optimized   |
| --------- | --------------------------- | -------------------- | --------------------- |
| **PG 版本** | 17                          | 17                   | 17                    |
| **vCPU**  | 16                          | 16                   | 16                    |
...
**分析**：对于受 IO 瓶颈限制的读密集型工作负载，性能差距会显著拉大。**ClickHouse Managed Postgres 实现了 84.8K TPS**，而在计算资源相同的情况下，配置了 16,000 预配置 IOPS 的 RDS 仅达到 9.4K TPS。关键差异在于：ClickHouse Managed Postgres 的 NVMe 存储能够随并发度提升而扩展，而网络附加存储仍受限于预配置 IOPS 的上限。即使配置了预配置 IOPS，RDS 仍比 ClickHouse Managed Postgres 慢 9 倍，这表明对于 IO 密集型工作负载而言，存储架构至关重要。
...
| 配置        | ClickHouse Managed Postgres | 配备 16k IOPS 的 RDS    |
| --------- | --------------------------- | -------------------- |
| **PG 版本** | 17                          | 17                   |
| **vCPU**  | 16                          | 16                   |
| **RAM**   | 64 GB                       | 64 GB                |
...
**分析**：即使在磁盘 I/O 极少、主要受 CPU 限制的场景下，**ClickHouse Managed Postgres 仍以 36.5K TPS 领跑**。尽管两项服务的 CPU 利用率都达到 100%，ClickHouse Managed Postgres 中的 NVMe 存储仍凭借更高的缓存命中率带来了更优的性能。相较于 RDS 的 12% 优势表明，即使在工作负载主要受 CPU 限制时，底层基础设施依然具备很高的效率。
...
| 配置        | ClickHouse Managed Postgres | RDS PostgreSQL |
| --------- | --------------------------- | -------------- |
| **PG 版本** | 17                          | 17             |
| **vCPU**  | 2                           | 2              |
| **RAM**   | 8 GB                        | 8 GB           |
...
在这三种基准测试场景中，ClickHouse Managed Postgres 均持续展现出更优的性能：
...
| 方面        | NVMe 存储 (ClickHouse Managed Postgres) | 网络附加存储 (预配置 IOPS) |
| --------- | ------------------------------------- | ----------------- |
...
除了原始性能外，ClickHouse Managed Postgres 还具备更出色的性价比：
...
对于当前受 IOPS 限制的工作负载，切换到 ClickHouse Managed Postgres 可以免去昂贵的预配置 IOPS 或 IO Optimized 配置，同时显著提升性能。
...
* [ClickHouse Managed Postgres 概览](/zh/products/managed-postgres/overview)
* [对 Postgres 实例进行扩缩容](/zh/products/managed-postgres/scaling)