...
**分析**：混合读写工作负载最能体现 NVMe 存储的显著性能优势，也代表了**快速增长的 AI 驱动型工作负载中最贴近真实的场景**——这类场景既需要高吞吐的数据摄取，也需要低延迟读取。**由 ClickHouse 托管的 Postgres 在更高并发下达到 19.8K TPS**，表明 NVMe 存储在高负载下依然能够有效扩展。这比 **RDS 和 Aurora 快 4.3-4.5 倍**。网络附加存储方案在写密集型操作中表现吃力；即使已经预配容量，且 Aurora 还启用了 IO Optimized 配置，RDS 和 Aurora 的 TPS 最高也只有 4.4K-4.6K。
