...
将`value`转换为精度为`S`的[十进制](../../sql_reference/functions/type_conversion_functions.md)。`value`可以是数字或字符串。`S`参数为指定的小数位数。
...
支持将数据转换为[可为空](../../sql_reference/functions/type_conversion_functions.md)。例如：
