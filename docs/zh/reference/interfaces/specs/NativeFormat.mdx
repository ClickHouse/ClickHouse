...
每项功能背后都有一个 `DBMS_MIN_REVISION_WITH_*` 阈值。只有当修订版本达到该阈值时，写入端才会写出该功能；读取端也会按完全相同的规则来判断是否读取，因此两端才能保持一致——只要任一侧的修订版本判断出错，就会失步。对于 Native 格式，关键的门槛如下：
...
这意味着，修订版本 `0` 对几乎所有情况来说都是最保守的编码方式：stream 中没有 `BlockInfo`、没有 `has_custom_serialization` 字节、使用 V1 `Dynamic`/`JSON`、没有聚合函数版本，并且 `DateTime` 会去掉时区参数。
...
因此，在修订版本 `0`——也就是 `FORMAT Native` 的默认值——下，`LowCardinality(T)` **不会**被剥离。它的类型字符串和每块状态前缀都会保留在 stream 中，而修订版本为 `0` 的读取端也会原样读回。只有当修订版本为非零且低于 `54405`，或者在不考虑修订版本的情况下被强制剥离时，才会发生剥离。
...
<Note>
  ...
  * `QBit(T, N[, stride])` 存储的是一个向量，其 bit planes 经过转置，用于向量搜索 workload；它的线上传输 stream 布局 (按组优先的 `FixedString` bit-plane 流，共 `element_size * (N / stride)` 个，并带有显式的 `stride`) 以及它的 binary type encoding (标签为 `0x36`，或者当 `stride != N` 时为 `0x37` `QBitWithStride`) 已在 [`QBit` data type page](/zh/reference/data-types/qbit) 和 [binary type encoding](/zh/reference/data-types/data-types-binary-encoding) 参考文档中说明，因此 `Native` reader 无需从 C++ 源码中还原这些信息。
</Note>
...
| 字节     | 方法   | 主体编码                                                     |
| ------ | ---- | -------------------------------------------------------- |
| `0x02` | NONE | 主体为原始字节 (无压缩) 。帧仍会被发送；接收端会校验 checksum。                   |
| `0x82` | LZ4  | 主体采用 **LZ4 块格式**——*不是* LZ4 frame format。没有 magic number。 |
| `0x90` | ZSTD | 主体是原始的 zstd 单帧 stream (标准 zstd magic number 是主体的一部分) 。   |

方法字节还会编码 [column-level codecs](/zh/reference/statements/create/table#column_compression_codec)。这些编解码器是按列应用在 MergeTree 的 磁盘 path 上，而不是用于整个流的分帧；但 `decompress=1` 的 HTTP 输入路径会从每个帧的方法字节中读取 codec，因此这些字节都可能合法地出现在输入中。因此，符合规范的 decoder 必须识别整个已分配的取值空间，并在遇到尚未实现的字节时拒绝处理，而不是误读主体。它们的主体内容取决于具体编解码器，不属于这个通用帧约定的范围：
...
`0x9d` (`SZ3`) 是一种 **Experimental** 的、有误差边界的*有损*编解码器，用于 `Float32`、`Float64` 以及这些类型的 `Array`。只有在设置了 `allow_experimental_codecs` 时，才能使用 `CODEC(SZ3)` 创建表；但在解压时始终接受该方法字节，以确保之前写入的数据仍然可读。`0x9f` (`ZXC`) 是一种非对称 LZ 编解码器：压缩较慢，但解压非常快，压缩比介于 `LZ4` 和 `ZSTD` 之间。字节 `0x99` (`DeflateQpl`) 和 `0x9b` (`ZSTD_QPL`) 曾分配给现已移除的编解码器；它们现已保留，不会复用。
