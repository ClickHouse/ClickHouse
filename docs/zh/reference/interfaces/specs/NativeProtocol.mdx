...
| STRING&#95;WITH&#95;SIZE&#95;STREAM&#95;SERIALIZATION        | 54492 | 块 (列)                           | `String` 列从逐值长度前缀布局切换为独立的累计字节偏移流 (与 `Array` 使用的偏移布局相同) ，按原样发送：`[UInt64 × num_rows offsets][data blob]`。由协商后的修订版本控制；低于 54492 时，写入器使用逐值长度前缀。线协议不携带此项的逐列标记——双方仅根据修订版本确定。参见 [String 列布局](/zh/reference/interfaces/specs/NativeFormat#string-type)。                                                                                                                                                                                  |
...
| 7  | Scalar                                 | 未指定                                                                               | 标量数据块                                                                                                                                                                                                                                |
| 8  | IgnoredPartUUIDs                       | 未指定                                                                               | 查询中要排除的分片                                                                                                                                                                                                                        |
...
| 14 | MergeTreeAllRangesAnnouncementResponse | [MergeTreeAllRangesAnnouncementResponse](#mergetreeallrangesannouncementresponse) | initiator 对 follower 的 [`MergeTreeAllRangesAnnouncement`](#packet-type-reference) 的回复 (受 `parallel_replicas_protocol_version ≥ 8` 限制——参见 [VERSIONED&#95;PARALLEL&#95;REPLICAS&#95;PROTOCOL](#feature-table)) 。仅用于服务器间通信——外部客户端绝不会发送。 |
