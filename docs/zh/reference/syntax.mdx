...
<Accordion title="启用完整解析器">
  你也可以通过 [`input_format_values_interpret_expressions`](/zh/reference/settings/formats/input-format#input_format_values_interpret_expressions) 设置，
  为数据启用完整解析器。
...
数据可以采用任意格式。
收到查询时，server 只会将请求中不超过 [max&#95;query&#95;size](/zh/reference/settings/session-settings/max#max_query_size) 字节的内容放入 RAM 中进行计算
...
<Note>
  你可以将 [prefer&#95;column&#95;name&#95;to&#95;alias](/zh/reference/settings/session-settings/prefer#prefer_column_name_to_alias) 设置为 `1`，以更改这一默认行为。
</Note>
