...
如果你为现有表添加了索引，我们建议为现有表 parts 物化该索引 (否则，对没有索引的 parts 进行搜索时，将回退为较慢的穷举扫描) 。
...
对于标记搜索函数 (`hasToken`, `hasAllTokens`, `hasAnyTokens`) ，文本索引是主要的求值路径：ClickHouse 会通过与索引构建时相同的预处理器、分词器和后处理器对 needle 进行归一化，并将这一规范化结果同时用于带索引和不带索引的 table parts。使用后处理器时，haystack 标记也会在查询时被归一化 (适用于任何分词器，而不仅仅是 `array`) ，因此比较两侧都会以一致的方式进行转换，结果也不依赖于索引是通过直接读取 (设置 `query_plan_direct_read_from_text_index`) 访问，还是某个 part 是否具有 materialized 索引——例如，配合 `lower` 后处理器，可为 `hasAllTokens(col, ['FOO'])` 启用不区分大小写的匹配。
在未使用 `positions` 时，`hasPhrase` 仅将索引用作提示，并使用原始谓词验证每一条保留下来的行；后处理器还会以相同方式对短语和 haystack 标记进行归一化，因此结果与读取路径无关，而且被后处理器丢弃的标记也不会破坏短语的相邻关系。在 `positions = 1` 时，`hasPhrase` 会使用精确的直接读取 (如果有后处理器，仍会应用) 。
...
| [hasPhrase](/zh/reference/functions/regular-functions/string-search-functions#hasPhrase)                   | 是                        | `splitByNonAlpha`, `splitByString`, `ngrams`, `asciiCJK` | 是      |
| [startsWith](/zh/reference/functions/regular-functions/string-functions#startsWith)                        | 是                        | `splitByNonAlpha`, `ngrams`, `sparseGrams`, `asciiCJK`   | 是      |
| [endsWith](/zh/reference/functions/regular-functions/string-functions#endsWith)                            | 是                        | `splitByNonAlpha`, `ngrams`, `sparseGrams`, `asciiCJK`   | 是      |
...
实验性参数 `positions` (默认值：`0`) 用于控制索引是否存储标记位置。
...
因此，使用 `positions = 1` 创建索引时，需要启用 MergeTree 设置 [`allow_experimental_text_index_positions`](/zh/reference/settings/merge-tree-settings#allow_experimental_text_index_positions)。
将 `positions = 0` (默认值) 可保持仅倒排列表存储；未使用此参数创建的文本索引将不包含位置信息。

<Warning>
  此参数为实验性功能，仅应用于测试。
  设置 MergeTree [`allow_experimental_text_index_positions`](/zh/reference/settings/merge-tree-settings#allow_experimental_text_index_positions) 以启用位置存储。
</Warning>
...
使用设置 [use&#95;text&#95;index&#95;header&#95;cache](/zh/reference/settings/session-settings#use_text_index_header_cache)、[use&#95;text&#95;index&#95;tokens&#95;cache](/zh/reference/settings/session-settings#use_text_index_tokens_cache) 和 [use&#95;text&#95;index&#95;postings&#95;cache](/zh/reference/settings/session-settings#use_text_index_postings_cache)，可按查询禁用各个缓存的读写。
