...
* `splitByRegexp(re)` 按用户定义的正则表达式分隔符 `re` 拆分字符串 (参见函数 [splitByRegexp](/zh/reference/functions/regular-functions/splitting-merging-functions#splitByRegexp)) 。
  `re` 参数为必需参数，例如 `tokenizer = splitByRegexp('[^\p{L}\p{N}#+]+')`。
  与 `splitByString` 的固定分隔符字符串不同，正则表达式分隔符可以保留包含特殊字符的标记 (如 `C++` 或 `C#`)，而其他分词器会将其拆分。
...
此外，对于不带预处理器或后处理器的 `splitByNonAlpha` 和 `array` 分词器，`LIKE` 还支持通过字典扫描进行求值 (通过 `use_text_index_like_evaluation_by_dictionary_scan` 启用) 。
此时 `%value%` 模式属于 *直接读取 (不使用提示)&#x20;*，而 `value%` 和 `%value` 模式仍作为提示使用，参见 [LIKE/ILIKE 查询](#like-ilike-queries-perf)。
...
### 为 Array(String) 列创建索引 {#text-index-example-array}
...
### 为 Map 列创建索引 {#text-index-example-map}
...
启用该 optimization 后，LIKE/ILIKE 查询应显著快于全表扫描。但当模式匹配了字典中的大部分标记时，性能可能反而不如全表扫描。好在系统提供了 fallback 机制来避免这种情况。
...
该 optimization 仅支持 `like`、`ilike`、`startsWith` 和 `endsWith` 函数。

### 简单计数查询 {#count-queries-perf}
...
## 文本索引与基于布隆过滤器的索引对比 {#text-index-vs-bloom-filter-indexes}
...
### 4. 复合搜索：或、AND、NOT、... {#compound-search}
