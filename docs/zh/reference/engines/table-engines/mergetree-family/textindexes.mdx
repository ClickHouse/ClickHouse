...
<div id="japanese-tokenizer-dictionary">
  ### 日语分词器字典
</div>
...
* 对于 `hasToken`、`hasAllTokens`、`hasAnyTokens` 和 `hasPhrase` (使用任何受支持的分词器)：后处理器会同时应用于 haystack 标记和搜索 needle，从而实现完全归一化的匹配 (例如，不区分大小写的搜索)。对于 `hasPhrase`，后处理后的标记会被连续定位，因此后处理器丢弃的标记不会留下位置间隙，短语仍可跨越该标记匹配——例如，使用会丢弃 `the` 的停用词后处理器时，`hasPhrase(col, 'see cat')` 可匹配文档 `see the cat`。
...
| [hasPhrase](/zh/reference/functions/regular-functions/string-search-functions#hasPhrase)                   | 是                       | `splitByNonAlpha`, `splitByString`, `ngrams`, `asciiCJK`, `icu` | 是      |
...
请注意，该函数要求使用 `splitByNonAlpha`、`splitByString`、`ngrams`、`asciiCJK` 或 `icu` 分词器之一。
...
`hasPhrase` 可与分词器 `splitByNonAlpha`、`splitByString`、`ngrams`、`asciiCJK` 和 `icu` 搭配使用。
