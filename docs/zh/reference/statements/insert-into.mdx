...
若要向非 Nullable 数据类型的列中插入默认值而不是 `NULL`，请启用 [insert&#95;null&#95;as&#95;default](/zh/reference/settings/session-settings/insert#insert_null_as_default) 设置。
...
对于小而频繁的插入操作，可以采用异步插入的方式。此类插入的数据会先合并为批次，然后再安全地写入表中。要使用异步插入，请启用 [`async_insert`](/zh/reference/settings/session-settings/async-insert#async_insert) 设置。
...
当插入大量数据时，ClickHouse 会通过称为“squashing”的过程来优化写入性能。内存中较小的已插入数据块会先合并并压缩成更大的块，然后再写入磁盘。squashing 可减少每次写入操作的相关开销。在此过程中，ClickHouse 每完成写入 [`max_insert_block_size`](/zh/reference/settings/session-settings/max-insert#max_insert_block_size) 行后，这部分已插入的数据就可以被查询到。

**另请参见**

* [async&#95;insert](/zh/reference/settings/session-settings/async-insert#async_insert)
* [wait&#95;for&#95;async&#95;insert](/zh/reference/settings/session-settings/wait-for#wait_for_async_insert)
* [wait&#95;for&#95;async&#95;insert&#95;timeout](/zh/reference/settings/session-settings/wait-for#wait_for_async_insert_timeout)
* [async&#95;insert&#95;max&#95;data&#95;size](/zh/reference/settings/session-settings/async-insert#async_insert_max_data_size)
* [async&#95;insert&#95;busy&#95;timeout&#95;ms](/zh/reference/settings/session-settings/async-insert#async_insert_busy_timeout_max_ms)
