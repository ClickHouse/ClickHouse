...
单独定义一个假设索引并不会产生任何效果——要了解它会如何影响查询，请对一个具有代表性的 `SELECT` 运行 [`EXPLAIN WHATIF`](/zh/reference/statements/explain#explain-whatif)。估算器会报告每个候选索引的适用性、将读取的标记数、最终的跳过率，以及该估算是如何得出的 (`empirical`、`statistical` 或 `applicability_only`) 。
