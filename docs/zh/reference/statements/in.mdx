...
这意味着数据类型将变为 [Nullable](/zh/reference/data-types/nullable)，若转换无法执行，则返回 [NULL](/zh/reference/settings/formats/input-format#input_format_null_as_default)。
...
在请求处理过程中，`IN` 运算符认为，与 [NULL](/zh/reference/settings/formats/input-format#input_format_null_as_default) 进行运算的结果始终为 `0`，无论 `NULL` 位于运算符左侧还是右侧。如果 [transform&#95;null&#95;in = 0](/zh/reference/settings/session-settings/other#transform_null_in)，则任何数据集中都不会包含 `NULL` 值；它们彼此不对应，也无法比较。
...
你可以使用 [`max_rows_in_set`](/zh/reference/settings/session-settings/max-rows#max_rows_in_set) 和 [`max_bytes_in_set`](/zh/reference/settings/session-settings/max-bytes#max_bytes_in_set) 来控制分布式查询期间传输的数据量。
...
如果 `some_predicate` 的选择性不够高，就会返回大量数据并引发性能问题。在这种情况下，最好限制通过网络传输的数据量。另外请注意，[`set_overflow_mode`](/zh/reference/settings/session-settings/other#set_overflow_mode) 默认设置为 `throw`，这意味着当达到这些阈值时会抛出异常。
...
如果表没有采样键，则可以使用更灵活的 [parallel&#95;replicas&#95;custom&#95;key](/zh/reference/settings/session-settings/parallel-replicas#parallel_replicas_custom_key) 选项，从而实现不同且更优的行为。