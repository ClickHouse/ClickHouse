...
| `execute_direct`              | 如果 `execute_direct` = `1`，则会在 [user&#95;scripts&#95;path](/zh/reference/settings/server-settings/settings/user#user_scripts_path) 指定的 user&#95;scripts 文件夹中查找 `command`。可使用空白分隔符指定额外的脚本参数。示例：`script_name arg1 arg2`。如果 `execute_direct` = `0`，则会将 `command` 作为参数传递给 `bin/sh -c`。默认值为 `1`。可选。 |
