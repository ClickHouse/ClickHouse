...
| `ssl_ca`, `ssl_cert`, `ssl_key` | 与上述相同的凭据，但以服务器上文件的路径形式指定。仅允许用于在服务器配置文件中定义的字典，见下文。可选。                                                           |
...
<Note>
  `ssl_ca`、`ssl_cert` 和 `ssl_key` 指定由服务器以自身权限打开的文件名，因此仅适用于在服务器配置文件中定义的字典。使用 `CREATE DICTIONARY` 查询创建的字典则必须通过 `ssl_ca_pem`、`ssl_cert_pem` 和 `ssl_key_pem` 传递文件内容。这些值会像密码一样，在日志和 `SHOW` 查询中被掩码处理。
</Note>
