...
* `SETTINGS`： (可选) 用于添加其他设置。
  * (可选) ：[check&#95;query&#95;single&#95;value&#95;result](/zh/reference/settings/session-settings/check#check_query_single_value_result)：此设置控制输出是详细模式 (`0`) 还是汇总模式 (`1`) 。
...
要检查整个数据库的健康状况，而非单个表的完整性，请使用 [`CHECK DATABASE`](/zh/reference/statements/check-database) 查询。它主要用于采用 [`DataLakeCatalog`](/zh/reference/engines/database-engines/datalake) 引擎的数据库，以验证外部 catalog 是否可访问。
