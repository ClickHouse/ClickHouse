...
使用以下语句可修改现有PROJECTION的 [`WITH SETTINGS`](#with-settings) 子句，而无需重建其数据：
...
对于[PROJECTION索引](/zh/reference/engines/table-engines/mergetree-family/mergetree#projection-index)，请重新声明 `INDEX`，而不是 `SELECT` 查询：
...
该语句会重新声明完整的PROJECTION定义，但仅 `WITH SETTINGS` 子句可以与现有定义不同。
PROJECTION查询本身 (对于PROJECTION索引，则是索引表达式和类型) 必须保持不变，因为现有投影 parts 中存储的数据基于此构建；如需更改，请先使用 [`DROP PROJECTION`](#drop-projection)，再使用 [`ADD PROJECTION`](#add-projection)。

该命令仅修改表元数据，不会重写任何数据：现有投影 parts 会保留写入时使用的设置，而后续插入和合并写入的投影 parts 将使用新设置。
要使用新设置重建现有 parts，请运行 [`MATERIALIZE PROJECTION`](#materialize-projection)。
...
命令 `ADD`、`MODIFY`、`DROP` 和 `CLEAR` 都属于轻量级操作，因为它们只会修改元数据或删除文件。
...
执行查询时，ClickHouse 会在读取原始表还是其某个PROJECTION之间进行选择。
读取原始表还是某个PROJECTION，这一决策会针对每个表分片单独作出。
ClickHouse 通常会尽可能少读数据，并采用一些方法来识别最佳读取分片，例如对分片的主键进行采样。
在某些情况下，源表分片没有对应的PROJECTION分片。
例如，这可能是因为在 SQL 中为表创建PROJECTION默认是“惰性”的——它只影响新插入的数据，而不会修改现有分片。

由于某个PROJECTION已经包含预先计算好的聚合值，ClickHouse 会尽量从对应的PROJECTION分片中读取，以避免在查询运行时再次聚合。如果某个特定分片缺少对应的PROJECTION分片，查询执行就会回退到原始分片。
...
在 ClickHouse v24.8 之前，PROJECTION分片要么会悄然与主数据失去同步，要么某些操作 (如更新和删除) 根本无法执行，因为如果表包含PROJECTION，数据库会自动抛出异常。

从 v24.8 开始，新增了一个表级设置 [`deduplicate_merge_projection_mode`](/zh/reference/settings/merge-tree-settings/other#deduplicate_merge_projection_mode)，用于控制当前述非平凡后台合并操作发生在原始表分片中时的行为。

删除变更是另一类会删除原始表分片中行的分片合并操作。自 v24.7 起，我们还提供了一个设置，用于控制由轻量级删除触发的删除变更相关行为：[`lightweight_mutation_projection_mode`](/zh/reference/settings/merge-tree-settings/other#lightweight_mutation_projection_mode)。

下面是 `deduplicate_merge_projection_mode` 和 `lightweight_mutation_projection_mode` 的可能值：

* `throw` (默认) ：抛出异常，防止PROJECTION分片失去同步。
* `drop`：删除受影响的PROJECTION表分片。对于受影响的PROJECTION分片，查询将回退到原始表分片。
* `rebuild`：重建受影响的PROJECTION分片，使其与原始表分片中的数据保持一致。
