...
该命令会就地更新现有视图的刷新配置，无需重新创建 materialized view 或其目标表，也不会清除现有目标数据或取消正在进行的刷新。更改配置期间，读取操作会继续针对现有目标表进行。

使用相同的完整 `REFRESH` 规范重复执行该命令是安全的。请务必重复应保留配置的每个子句，因为如上所述，每次执行都会替换所有刷新参数。
