...
启用 [`apply_patches_on_merge`](/zh/reference/settings/merge-tree-settings/other#apply_patches_on_merge) 设置 (默认) 后，补丁分区片段通常会在合并过程中自动应用。不过，在以下场景中，你可能希望手动触发补丁应用：
...
* [`apply_patches_on_merge` 设置](/zh/reference/settings/merge-tree-settings/other#apply_patches_on_merge) - 控制在合并过程中自动应用补丁