...
默认情况下，字典采用延迟加载 (参见 [dictionaries&#95;lazy&#95;load](/zh/reference/settings/server-settings/settings/dictionaries#dictionaries_lazy_load)) ，因此不会在启动时自动加载，而是在首次访问时通过调用 [`dictGet`](/zh/reference/functions/regular-functions/ext-dict-functions#dictGet) 函数，或对 `ENGINE = Dictionary` 的表执行 `SELECT` 时初始化。
...
重新计算所有[异步指标](/zh/reference/system-tables/asynchronous_metrics)。由于异步指标会根据设置 [asynchronous&#95;metrics&#95;update&#95;period&#95;s](/zh/reference/settings/server-settings/settings/asynchronous-metrics#asynchronous_metrics_update_period_s) 定期更新，因此通常无需使用此语句手动更新。
...
其大小由服务器级设置 [`primary_index_cache_size`](/zh/reference/settings/server-settings/settings/primary-index#primary_index_cache_size) 控制。
...
清除未压缩数据缓存。
可通过查询/用户/profile 级设置 [`use_uncompressed_cache`](/zh/reference/settings/session-settings/use#use_uncompressed_cache) 启用或禁用未压缩数据缓存。
其大小可通过服务器级设置 [`uncompressed_cache_size`](/zh/reference/settings/server-settings/settings/uncompressed-cache#uncompressed_cache_size) 配置。
...
清除已编译表达式缓存。
可通过查询/用户/profile 级设置 [`compile_expressions`](/zh/reference/settings/session-settings/compile#compile_expressions) 启用或禁用已编译表达式缓存。
...
清除从 [`format_schema_path`](/zh/reference/settings/server-settings/settings/format#format_schema_path) 加载的 schema 缓存。

支持的目标：

* Protobuf：从内存中移除已导入的 Protobuf 消息定义。
* Files：删除本地 [`format_schema_path`](/zh/reference/settings/server-settings/settings/format#format_schema_path) 中缓存的 schema 文件，这些文件会在 `format_schema_source` 设置为 `query` 时生成。
...
<Note>
  如果启用了 [`prefer_localhost_replica`](/zh/reference/settings/session-settings/prefer#prefer_localhost_replica) (默认启用) ，数据仍会照常插入本地分片。
</Note>
...
* [可刷新materialized view](/zh/reference/statements/create/view#refreshable-materialized-view) (定期刷新) ；以及
* 持续从外部来源消费数据的流式表引擎：[Kafka](/zh/reference/engines/table-engines/integrations/kafka)、[RabbitMQ](/zh/reference/engines/table-engines/integrations/rabbitmq)、[NATS](/zh/reference/engines/table-engines/integrations/nats)、[S3Queue](/zh/reference/engines/table-engines/integrations/s3queue) 和 [AzureQueue](/zh/reference/engines/table-engines/integrations/azure-queue)。

对于可刷新materialized view，每个动词都是[管理可刷新 Materialized View](#managing-refreshable-materialized-views)中对应 `SYSTEM ... VIEW` 命令的别名。因此，`SYSTEM STOP [db.]name` 的行为与 `SYSTEM STOP VIEW [db.]name` 完全相同，其他命令也同样如此。
...
`STOP` 和 `CANCEL` 会尽快中断消费。对于 [Kafka](/zh/reference/engines/table-engines/integrations/kafka)、[RabbitMQ](/zh/reference/engines/table-engines/integrations/rabbitmq) 和 [NATS](/zh/reference/engines/table-engines/integrations/nats)，它们会停止从来源读取数据，但不会中断已开始的插入：正在写入 materialized view 的块仍会完成并提交。[S3Queue](/zh/reference/engines/table-engines/integrations/s3queue) 和 [AzureQueue](/zh/reference/engines/table-engines/integrations/azure-queue) 会在同一管道中读取和插入数据。启用去重 (默认) 时，插入也会被取消，文件会在之后重新处理。禁用去重时，为避免重复行，正在处理的批次会完成并提交 (与上述引擎相同) 。已读取但尚未提交的数据会在之后再次消费，因此不会丢失数据；但核心 NATS (不含 JetStream) 例外，它无法重新投递，会丢弃这些数据。
