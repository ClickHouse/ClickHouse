...
各可选子句的具体说明分别见单独章节，下面按其执行顺序列出 (但为了方便起见，`SELECT` 和 `DISTINCT` 先列出：如[下文](#select-clause)所述，它们的表达式只有在前面的子句执行完成后才会被计算) ：

* [WITH 子句](/zh/reference/statements/select/with)
* [SELECT 子句](#select-clause)
* [ALL 子句](/zh/reference/statements/select/all)
...
* [ARRAY JOIN 子句](/zh/reference/statements/select/array-join)
* [JOIN 子句](/zh/reference/statements/select/join)
* [PREWHERE 子句](/zh/reference/statements/select/prewhere) (它会过滤所应用的单个表，因此尽管在此列表中的位置如此，仍会在 `JOIN` 之前执行)
...
* [ORDER BY 子句](/zh/reference/statements/select/order-by)
...
查询也可以写成使用[管道运算符](/zh/reference/statements/select/pipe-operators)构成的线性转换链：
...
`GROUP BY`、`ORDER BY` 和 `LIMIT BY` 子句支持位置参数。要启用此功能，请打开 [enable&#95;positional&#95;arguments](/zh/reference/settings/session-settings/enable-positional-arguments#enable_positional_arguments) 设置。例如，`ORDER BY 1,2` 会先按表中的第 1 列、再按第 2 列对行进行排序。
