...
如果你想按列号而不是列名对表中的数据进行分组，请启用设置 [enable&#95;positional&#95;arguments](/zh/reference/settings/session-settings/enable-positional-arguments#enable_positional_arguments)。
...
在小计行中，已“已分组”的键表达式的值会被设为 `0` 或空字符串。
...
* [group&#95;by&#95;use&#95;nulls](/zh/reference/settings/session-settings/group-by#group_by_use_nulls) 设置，用于兼容 SQL 标准。
...
* 用于兼容 SQL 标准的 [group&#95;by&#95;use&#95;nulls](/zh/reference/settings/session-settings/group-by#group_by_use_nulls) 设置项。
...
这是最通用的一种修饰符。
该修饰符允许手动指定多个聚合键集合 (聚合键集合) 。
系统会分别对每个聚合键集合执行聚合，之后再将所有结果合并。
如果某列未出现在某个聚合键集合中，则会以默认值填充。
...
* 用于兼容 SQL 标准的 [group&#95;by&#95;use&#95;nulls](/zh/reference/settings/session-settings/group-by#group_by_use_nulls) 设置。
...
如果表按某个键排序，且 `GROUP BY` 表达式至少包含排序键的前缀或单射函数，则可以更高效地进行聚合。在这种情况下，当从表中读取到新键时，聚合的中间结果即可完成最终计算并发送给客户端。此行为由 [optimize&#95;aggregation&#95;in&#95;order](/zh/reference/settings/session-settings/optimize-aggregation-in-order#optimize_aggregation_in_order) 设置启用。这种优化可以减少聚合期间的内存使用量，但在某些情况下也可能会降低查询执行速度。
...
[max&#95;bytes&#95;before&#95;external&#95;group&#95;by](/zh/reference/settings/session-settings/max-bytes#max_bytes_before_external_group_by) 设置决定了将 `GROUP BY` 临时数据转储到文件系统时的 RAM 使用阈值。如果设为 0 (默认值) ，则表示禁用。
或者，您也可以设置 [max&#95;bytes&#95;ratio&#95;before&#95;external&#95;group&#95;by](/zh/reference/settings/session-settings/max-bytes#max_bytes_ratio_before_external_group_by)，这样只有当查询使用的内存达到一定阈值时，才允许 `GROUP BY` 使用外部内存。
...
例如，如果 [max&#95;memory&#95;usage](/zh/reference/settings/session-settings/max-memory-usage#max_memory_usage) 设置为 10000000000，并且您希望使用外部聚合，那么将 `max_bytes_before_external_group_by` 设为 10000000000、将 `max_memory_usage` 设为 20000000000 是合理的。触发外部聚合时 (如果至少发生过一次临时数据转储) ，RAM 的最大消耗只会比 `max_bytes_before_external_group_by` 略高一点。
