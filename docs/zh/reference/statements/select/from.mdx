...
在以 `FROM` 开头的查询中，`SELECT` 子句是可选的；省略时，该查询的行为如同写入了 `SELECT *`：
...
此形式主要用于使用[管道运算符](/zh/reference/statements/select/pipe-operators)的查询：
...
带有 `FINAL` 的 `SELECT` 查询会并行执行。[max&#95;final&#95;threads](/zh/reference/settings/session-settings/max#max_final_threads) 设置会限制使用的线程数。
...
还可以通过会话或用户 profile，使用 [FINAL](/zh/reference/settings/session-settings/other#final) 设置将 `FINAL` 自动应用于查询中的所有表。
