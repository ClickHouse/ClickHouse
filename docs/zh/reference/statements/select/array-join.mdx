...
<Note>
  不支持 PostgreSQL `FROM unnest(...)`、`CROSS JOIN UNNEST(...)` 和 `LATERAL`。请改用 `ARRAY JOIN`。`unnest` 名称 (自 26.5 版本起) 是 `arrayJoin` 的函数调用别名 (`SELECT unnest(arr)`) ，而非表函数。
</Note>
...
[短路函数求值](/zh/reference/settings/session-settings/short-circuit-function-evaluation#short_circuit_function_evaluation) 是一项功能，用于优化 `if`、`multiIf`、`and` 和 `or` 等特定函数中复杂表达式的执行。它可以防止这些函数在执行过程中出现潜在异常，例如除以零。
