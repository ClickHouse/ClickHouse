...
| 参数                           | 描述                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `addresses_expr`             | 远程服务器地址，或用于生成多个远程服务器地址的表达式。格式：`host` 或 `host:port`。<br /><br />    `host` 可以指定为服务器名称，也可以是 IPv4 或 IPv6 地址。IPv6 地址必须写在 `[]` 中。<br /><br />    `port` 是远程服务器的 TCP 端口。如果省略端口，则表函数 `remote` 使用服务器配置文件中的 [tcp&#95;port](/zh/reference/settings/server-settings/settings/tcp-port#tcp_port) (默认值为 9000) ，表函数 `remoteSecure` 使用 [tcp&#95;port&#95;secure](/zh/reference/settings/server-settings/settings/tcp-port#tcp_port_secure) (默认值为 9440) 。<br /><br />    对于 IPv6 地址，必须指定端口。<br /><br />    如果只指定了参数 `addresses_expr`，则 `db` 和 `table` 默认使用 `system.one`。<br /><br />    类型：[String](/zh/reference/data-types/string)。 |
...
| `SETTINGS name = value, ...` | 该函数创建的 Distributed 表的设置，例如 `skip_unavailable_shards`。可选。查询中指定的设置优先于它。此子句仅可在表函数内使用；`Remote` 和 `RemoteSecure` 表引擎在引擎定义后接受相同的设置，参见 [Remote 和 RemoteSecure 引擎](/zh/reference/engines/table-engines/special/distributed#distributed-remote-engines)。                                                                                                                                                                                                                                                                                                                                                                 |
...
相同的参数也可用于 `Remote` 和 `RemoteSecure` 表引擎，以创建持久表而非临时表，参见 [Remote 和 RemoteSecure 引擎](/zh/reference/engines/table-engines/special/distributed#distributed-remote-engines)。唯一的区别在于 `SETTINGS` 子句：引擎在引擎定义后指定该子句，即 `ENGINE = Remote(...) SETTINGS skip_unavailable_shards = 1`，而不是将其放在参数中。
...
查询将发送到第一个健康的副本。不过，对于 `remote`，会按照当前 [load&#95;balancing](/zh/reference/settings/session-settings/load-balancing#load_balancing) 设置中指定的顺序依次遍历副本。
生成的地址数量受 [table&#95;function&#95;remote&#95;max&#95;addresses](/zh/reference/settings/session-settings/table#table_function_remote_max_addresses) 设置的限制。
