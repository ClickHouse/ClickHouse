...
* `id_expr` — 键值。对于具有简单键的字典，它是返回 `UInt64` 值的表达式。对于具有复合 (复杂) 键的字典，它是返回键值元组的表达式。如果复合键仅包含单个属性，则可以直接传递其值，而无需将其包装在 `tuple` 中。[`UInt64`](/zh/reference/data-types/int-uint) 或 [`Tuple(T)`](/zh/reference/data-types/tuple)
