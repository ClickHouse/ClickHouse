...
AI 函数会引用一个**命名集合**，其中存储了提供商凭据和配置信息。可以针对不同的函数或函数调用创建并使用不同的命名集合。例如，你可能希望为文本函数 (`aiGenerate`、`aiClassify`、`aiFilter`、`aiExtract`、`aiTranslate`) 定义一个命名集合，而为嵌入向量函数 (`aiEmbed`、`aiSimilarity`) 定义另一个，因为它们需要不同的端点，通常也会使用不同的模型。
...
2. 否则，使用适用的默认凭据设置：
   * 文本函数 (`aiGenerate`、`aiClassify`、`aiFilter`、`aiExtract`、`aiTranslate`) 使用 [`ai_function_text_default_credentials`](/zh/reference/settings/session-settings/ai-function#ai_function_text_default_credentials)；
...
计算两段文本的嵌入向量，并返回这两个向量的余弦相似度，
取值范围为 `[-1, 1]`：`1` 表示两段文本在语义上完全一致，`0` 表示毫不相关，
负值表示语义相反。这与基于相同嵌入向量计算的 `cosineDistance` 互为补集
