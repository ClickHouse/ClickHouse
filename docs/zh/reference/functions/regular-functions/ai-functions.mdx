...
AI 函数会引用一个**命名集合**，其中存储了提供商凭据和配置信息。可以针对不同的函数或函数调用创建并使用不同的命名集合。例如，你可能希望为文本函数 (`aiGenerate`、`aiClassify`、`aiExtract`、`aiTranslate`) 定义一个命名集合，而为嵌入向量函数 (`aiEmbed`、`aiSimilarity`) 定义另一个，因为它们需要不同的端点，通常也会使用不同的模型。
