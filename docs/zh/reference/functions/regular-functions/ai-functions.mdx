...
每个函数都接受一个可选的末尾 `Map(String, String)` 参数映射。所有值都必须是字符串 (数字也要加引号，例如 `'0.2'`) 。未知键会被拒绝。已提供的键会覆盖对应的命名集合值；未提供的键则回退到命名集合 (对于 `model`/`max_tokens`) 或内置默认值。例外情况是 `aiEmbed`，它将 `model` 作为必需的位置参数 (`aiEmbed(text, model[, params])`) 传入；如果改为在参数映射或命名集合中设置，则会报错。这是为了确保嵌入向量可复现。
