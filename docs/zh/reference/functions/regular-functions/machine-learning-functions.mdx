...
使用带有 n-gram 和拉普拉斯平滑的朴素贝叶斯模型对输入文本进行分类。
该模型必须先在 ClickHouse 中配置后才能使用。

**实现细节**

*算法*

使用朴素贝叶斯分类算法，并结合 [拉普拉斯平滑](https://en.wikipedia.org/wiki/Additive_smoothing) 处理未见过的 n-gram，依据 [此处](https://web.stanford.edu/~jurafsky/slp3/4.pdf) 所述的 n-gram 概率。
...
* 三种标记化模式：
  * `byte`：基于原始字节进行处理。每个字节都是一个标记。
  * `codepoint`：基于从 UTF-8 解码出的 Unicode 标量值进行处理。每个码点都是一个标记。
  * `token`：按连续的 Unicode 空白字符 (正则 `\s+`) 拆分。标记是非空白子字符串；如果标点符号与其相邻，则会被视为标记的一部分 (例如，`you?` 是一个标记) 。

**模型配置**

可在 [此处](https://github.com/nihalzp/ClickHouse-NaiveBayesClassifier-Models) 获取用于创建语言检测朴素贝叶斯模型的示例源代码，也可在 [此处](https://github.com/nihalzp/ClickHouse-NaiveBayesClassifier-Models/tree/main/models) 获取示例模型及其关联的配置文件。

ClickHouse 中朴素贝叶斯模型的一个示例配置如下：
...
| 参数       | 描述                                                               | 示例                                                       | 默认值   |
| -------- | ---------------------------------------------------------------- | -------------------------------------------------------- | ----- |
| `name`   | 唯一的模型标识。                                                         | `language_detection`                                     | 必填    |
| `path`   | 模型二进制文件的完整路径。                                                    | `/etc/clickhouse-server/config.d/language_detection.bin` | 必填    |
| `mode`   | 标记化方式：`byte` (字节序列) 、`codepoint` (Unicode 字符) 或 `token` (词标记) 。 | `token`                                                  | 必填    |
| `n`      | N-gram 大小：`1` (单个单词) 、`2` (词对) 或 `3` (三词组合) 。                    | `2`                                                      | 必填    |
| `alpha`  | 分类时使用的拉普拉斯平滑因子，用于处理模型中未出现的 n-grams。                              | `0.5`                                                    | `1.0` |
| `priors` | 各类别的概率 (属于某个类别的文档占比) 。                                           | 类别 0 为 60%，类别 1 为 40%                                    | 等概率分布 |

**模型训练指南**

*文件格式*

在便于阅读的格式中，对于 `n=1` 和 `token` 模式，模型可能如下所示：
...
ClickHouse 不直接使用便于人类阅读的格式；必须先将其转换为下文所述的二进制格式。
...
在根据文档语料库创建模型之前，必须先按指定的 `mode` 和 `n` 对文档进行预处理，以提取 n-gram：
...
1. 根据标记化模式，在每个文档的开头和结尾添加边界标记：
   ...
   * `token`：`<s>` (开头) ，`</s>` (结尾)
   ...
   注：文档开头和结尾各添加 `(n - 1)` 个标记。

2. `token` 模式下 `n=3` 的示例：
   * 文档：`ClickHouse is fast`
   * 处理结果为：`<s> <s> ClickHouse is fast </s> </s>`
   * 生成的三元 n-gram：
     * `<s> <s> ClickHouse`
     * `<s> ClickHouse is`
     * `ClickHouse is fast`
...
为简化 `byte` 和 `codepoint` 模式的模型构建，通常可以先对文档进行标记化 (`byte` 模式为字节列表，`codepoint` 模式为码点列表) ，然后在开头添加 `n - 1` 个起始标记，在结尾添加 `n - 1` 个结束标记，再生成 n-gram 并将其写入序列化文件。
...
* `model_name` — 预配置模型的名称。该模型必须在 ClickHouse 的配置文件中定义。[`String`](/zh/reference/data-types/string)
* `input_text` — 要分类的文本。输入会严格按原样处理 (保留大小写和标点) 。[`String`](/zh/reference/data-types/string)

**返回值**

预测类别的 ID，以无符号整数表示。类别 ID 对应于模型构建时定义的类别。[`UInt32`](/zh/reference/data-types/int-uint)
