...
| `GROUPS` 窗口帧                                                        | ✅   | 窗口帧边界按完整的对等组 (即在 `ORDER BY` 键上相等的行) 计数；`N PRECEDING` 和 `N FOLLOWING` 分别计算当前行所在对等组之前和之后的 `N` 个对等组。                                                                                                                                                                                 |
...
* `ROWS`、`RANGE` 或 `GROUPS` - 定义窗口帧的边界，aggregate&#95;function 在窗口帧内进行计算。`ROWS` 按物理行计数，`RANGE` 按 `ORDER BY` 值计数，`GROUPS` 按对等组计数 (即 `ORDER BY` 键值相等的行) 。
...
`GROUPS` 窗口帧以完整的*对等组*为单位计数——即 `ORDER BY` 键相同的行集合——而非物理行 (`ROWS`) 或 `ORDER BY` 值 (`RANGE`) 。`N PRECEDING` 和 `N FOLLOWING` 分别表示当前行所属对等组之前和之后的 `N` 个对等组；每个纳入窗口的对等组均包含其所有行。

以下查询针对 `ROWS`、`RANGE` 和 `GROUPS` 窗口帧，均使用相同的 `1 PRECEDING AND 1 FOLLOWING` 边界。`order` 列包含重复且不连续的值，因此这三种模式覆盖的行各不相同：
...
* `ROWS` 按物理行计数，因此窗口帧最多包含三个相邻行：当前行及其前后各一行。
* `RANGE` 按 `order` 值计数，因此 `1 PRECEDING` 和 `1 FOLLOWING` 覆盖与当前行 `order` 值相差不超过 1 的行。当间隔为 10 时，没有相邻行符合条件，因此窗口帧仅包含与当前行具有相同 `order` 值的行。
* `GROUPS` 按对等组计数，因此 `1 PRECEDING` 和 `1 FOLLOWING` 始终完整包含相邻组，无论 `order` 值之间的间隔有多大。
