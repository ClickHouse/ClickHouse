...
`a[N]` – 访问数组中的元素。对应 `arrayElement(a, N)` 函数。
`N` 也可以是整数数组，此时会将这些位置上的所有元素作为数组返回，
与 `arrayMap(i -> a[i], N)` 的行为相同。这些位置可以为 Nullable。当元素类型
可封装在 `Nullable` 中时，`NULL` 位置会返回 `NULL`；对于不能包含在 `Nullable` 中的元素类型 (如 `Array`、`Map`) ，则会返回
该元素类型的默认值，与标量 `NULL` 索引时的行为相同。

`a.N` – 访问 Tuple 元素。对应 `tupleElement(a, N)` 函数。
...
### match 函数 {#match-function}

`a REGEXP b`：`match(a, b)` 函数。

`a ~ b`：`match(a, b)` 函数 (PostgreSQL 风格的正则表达式匹配) 。

### notMatch 函数 {#notmatch-function}

`a !~ b` – `notMatch(a, b)` 函数 (检查 `a` 是否与正则表达式 `b` 不匹配) 。

### matchCaseInsensitive 函数 {#matchcaseinsensitive-function}

`a ~* b` – `matchCaseInsensitive(a, b)` 函数 (执行不区分大小写的正则表达式匹配) 。

### notMatchCaseInsensitive 函数 {#notmatchcaseinsensitive-function}

`a !~* b` – `notMatchCaseInsensitive(a, b)` 函数 (检查 `a` 是否不匹配正则表达式 `b`，且不区分大小写) 。
...
## 用于处理字符串的运算符 {#operators-for-working-with-strings}
...
### 数组上的 `SOME` / `ALL` {#some-all-on-arrays}
...
<Note>
  结果使用[会话时区](/zh/reference/settings/session-settings/other#session_timezone) (如果未设置会话时区，则使用服务器默认时区) 。[`date_time_overflow_behavior`](/zh/reference/settings/formats/date-time#date_time_overflow_behavior)设置控制结果超出可表示范围时的行为。
</Note>
...
### AT TIME ZONE 和 AT LOCAL {#at-time-zone}
...
`zone` 可以是任何求值结果为有效时区名称的常量字符串表达式 (例如 `'America/Denver'`、`'UTC'` 或 `concat('America', '/', 'Denver')`) 。由于 `AT TIME ZONE` 会被展开为 `toTimeZone`，因此同样适用相同的时区参数规则：像列引用这样的非常量表达式，需要设置 [`allow_nonconst_timezone_arguments = 1`](/zh/reference/settings/session-settings/allow#allow_nonconst_timezone_arguments)。

`AT LOCAL` 使用当前的[会话时区](/zh/reference/settings/session-settings/other#session_timezone) (如果未设置会话时区，则使用服务器默认时区) 。在 `Distributed` 表上，必须显式设置 `session_timezone`；当其为空时，`timeZone()` 是分片本地的，不能作为常量 `toTimeZone` 参数使用，因此会导致 `ILLEGAL_COLUMN` 异常。
...
## Tuple 创建运算符 {#tuple-creation-operator}
...
可通过启用 [optimize&#95;functions&#95;to&#95;subcolumns](/zh/reference/settings/session-settings/optimize#optimize_functions_to_subcolumns) 设置来优化。将 `optimize_functions_to_subcolumns = 1` 时，该函数只会读取 [null](/zh/reference/data-types/nullable#finding-null) 子列，而不是读取并处理整个列数据。查询 `SELECT n IS NULL FROM table` 会被转换为 `SELECT n.null != 0 FROM TABLE`。
...
可通过启用 [optimize&#95;functions&#95;to&#95;subcolumns](/zh/reference/settings/session-settings/optimize#optimize_functions_to_subcolumns) 设置进行优化。将 `optimize_functions_to_subcolumns = 1` 时，该函数仅读取 [null](/zh/reference/data-types/nullable#finding-null) 子列，而不是读取并处理整个列数据。查询 `SELECT n IS NOT NULL FROM table` 会被转换为 `SELECT NOT n.null FROM TABLE`。
