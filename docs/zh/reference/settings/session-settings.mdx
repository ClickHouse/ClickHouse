...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": ""},{"label": "新设置"}]}]} />

当调用未在参数映射中传入 `credentials` 时，`aiEmbed` 所使用的命名集合名称。空值表示没有默认值：此类调用必须显式传入 `credentials`。之所以与 `ai_function_text_default_credentials` 分开，是因为嵌入向量使用的端点和模型与聊天使用的不同。
...
当调用未在参数映射中传递 `credentials` 时，文本 AI 函数 (`aiGenerate`、`aiClassify`、`aiExtract`、`aiTranslate`) 将使用的命名集合名称。空值表示没有默认值：这类调用必须显式传递 `credentials`。聊天补全使用的端点和模型与嵌入向量使用的不同，因此该设置与 `ai_function_embedding_default_credentials` 分开。
...
当像 `b.id` 这样的多部分标识符既可能表示别名为 `b` 的表的列 `id`，也可能表示另一列的 Tuple 子列 `b.id` 时，优先按别名前缀来解释 (即 `b` 的列 `id`) 。默认情况下，analyzer 会优先解析为子列。启用此设置后，可与旧 analyzer 的解析方式保持一致。
...
* `t1 ANY LEFT JOIN t2` 和 `t2 ANY RIGHT JOIN t1` 操作的结果不相同，因为 ClickHouse 使用的是从左表到右表的多对一表键映射逻辑。
...
* `ANY INNER JOIN` 操作的结果会为左表和右表中的每个键各包含一行。
...
有时，向远程分片发送某个批次可能会失败，原因是后续复杂管道 (例如带有 `GROUP BY` 的 `MATERIALIZED VIEW`) 导致出现 `Memory limit exceeded` 或类似错误。在这种情况下，重试无济于事 (而且会导致该表的分布式发送卡住)；但如果将该批次中的文件逐个发送，则可能成功执行 INSERT。
...
启用或禁用在 PROJECTION 定义中支持位置参数。另请参阅 [enable&#95;positional&#95;arguments](#enable_positional_arguments) 设置。
...
* 如果 `insert_quorum < 2`，则禁用quorum写入。
* 如果 `insert_quorum >= 2`，则启用quorum写入。
* 如果 `insert_quorum = 'auto'`，则使用多数值 (`number_of_replicas / 2 + 1`) 作为quorum数。
...
当禁用 `insert_quorum_parallel` 时，quorum中的所有副本都是一致的，也就是说，它们都包含此前所有 `INSERT` 查询的数据 (`INSERT` 序列会被线性化) 。读取通过 `insert_quorum` 写入的数据时，如果 `insert_quorum_parallel` 已禁用，可以使用 [select&#95;sequential&#95;consistency](#select_sequential_consistency) 为 `SELECT` 查询启用顺序一致性。
...
INSERT 查询中 VALUES 子句里的数据由单独的 stream 解析器 处理 (其 RAM 占用为 O(1)) ，因此不受此限制影响。

<Note>
  `max_query_size` 不能在 SQL 查询内部设置 (例如 `SELECT now() SETTINGS max_query_size=10000`) ，因为 ClickHouse 需要先分配一个 缓冲区 来解析查询，而该 缓冲区 的大小由 `max_query_size` 设置决定；因此，它必须在查询执行前完成配置。
</Note>
...
当逻辑上等价时，将参数为 `if` 表达式的 聚合函数 进行重写。
...
<Note>
  这是一个专家级设置，仅应供开发人员用于调试。该设置未来可能会以向后不兼容的方式更改，或被移除。
</Note>
...
<Note>
  这是一个专家级设置，仅供开发人员调试时使用。该设置未来可能会以不向后兼容的方式发生变化，或被移除。
</Note>
...
* &#39;dpsize&#39; - 实现了 DPsize 算法，目前仅适用于 inner joins - 会考虑所有可能的 JOIN 顺序，并找出最优方案，但对于包含大量表和 JOIN 谓词的查询，速度可能较慢。
* &#39;dpsub&#39; - 实现了 DPsub 算法，同时支持 inner joins 和非 inner joins - 会考虑所有可能的 JOIN 顺序，并找出最优方案，但对于包含大量表和 JOIN 谓词的查询，速度可能较慢。
* &#39;dphyp&#39; - 实现了 DPhyp (通过超图分区进行动态规划) 算法，目前仅适用于 inner joins - 搜索的空间与 `dpsize` 相同，但只会枚举连通子图对，因此在稀疏 JOIN 图上生成的中间 JOIN 更少，不过代价是不考虑交叉乘积
  可以将多个算法指定为逗号分隔的列表，例如 `dphyp,greedy`。系统会按顺序尝试它们；如果某个算法无法处理该查询 (例如由于 outer joins 或存在不连通分量) ，则会回退到下一个算法。
...
用于启用或禁用一项查询计划级别的优化，该优化会尝试从查询计划步骤中移除未使用的列 (包括输入列和输出列) 。
仅在设置 [query&#95;plan&#95;enable&#95;optimizations](#query_plan_enable_optimizations) 为 1 时生效。

<Note>
  这是一个专家级设置，仅应由开发人员用于调试。该设置未来可能会以不向后兼容的方式发生变更，或被移除。
</Note>
...
控制是否启用一项查询计划级别的优化：在为窗口函数进行排序时利用存储排序顺序。
仅在设置 [`query_plan_enable_optimizations`](#query_plan_enable_optimizations) 为 1 时生效。

<Note>
  这是一个专家级设置，仅供开发人员用于调试。该设置未来可能会以不向后兼容的方式变更，或被移除。
</Note>
...
<Note>
  这是一个专家级设置，仅应供开发人员在调试时使用。该设置未来可能会以不向后不兼容的方式变更，或被移除。
</Note>
...
启用顺序一致性后，ClickHouse 只允许客户端在包含此前所有通过 `insert_quorum` 执行的 `INSERT` 查询所写入数据的副本上执行 `SELECT` 查询。如果客户端访问的是不完整的副本，ClickHouse 将抛出异常。`SELECT` 查询不会包含尚未写入 quorum 副本的数据。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "0.2"},{"label": "由 `text_index_density_threshold` 重命名而来（保留为别名）；在 惰性倒排列表应用模式 下选择倒排列表交集算法。"}]}]} />

在 惰性倒排列表应用模式 (`text_index_posting_list_apply_mode = 'lazy'`) 下，用于选择交集算法的倒排列表密度阈值。
低于该阈值时：使用 leapfrog 交集 (更适合稀疏倒排列表) 。达到或高于该阈值时：使用穷举 bitmap 交集 (更适合稠密倒排列表) 。
...
&#39;lazy&#39; 使用基于游标的按需解码 (需要采用带有已序列化 codec 的索引格式) 。
