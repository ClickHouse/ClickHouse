...
<div id="min_rows_ratio_for_hash_join_row_store">
  ## min_rows_ratio_for_hash_join_row_store
</div>

<SettingsInfoBlock type="Double" default_value="5" />

<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.9"},{"label": "5"},{"label": "新增设置，用于控制允许将哈希 join 载荷转换为行存储格式所需的 join 输出行数与构建端行数的最小估计比率。0 表示始终允许转换。"}]}]} />

允许将哈希 join 载荷转换为行存储格式所需的 join 输出行数与构建端行数的最小估计比率。0 表示始终允许转换。
