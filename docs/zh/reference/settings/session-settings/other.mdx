...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.8"},{"label": "16384"},{"label": "新增设置，用于指定自适应聚合器（`enable_adaptive_aggregator`）冻结线程本地哈希表时的键数量。"}]}]} />

自适应聚合器冻结线程本地哈希表时的键数量 (参见 `enable_adaptive_aggregator`) 。较小的值可使冻结后的表保持在缓存中，较大的值则可让其吸收更多高频键。值为 0 时，表会在首次有机会时冻结，这会使该算法的行为类似于按键哈希进行的纯分片：每个键都根据其哈希进行路由，并由单个所有者聚合；区别仅在于，聚合会延后到合并阶段，而不是在扫描期间在线程之间交换。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.9"},{"label": "4194304"},{"label": "新增设置：以字节数限制自适应聚合器冻结本地表；该限制与键数量阈值中先达到者生效；0 将禁用字节限制。"}]}]} />

自适应聚合器冻结线程本地哈希表时对应的内存大小 (请参阅 `enable_adaptive_aggregator`) 。此设置与 `adaptive_aggregator_freeze_threshold` 中任一阈值先达到时，表即会被冻结。该大小指本地表自身已分配的字节数 (包括其哈希表缓冲区和 Arena) ，在块之间进行检查。当键或聚合状态较宽时，字节限制尤为重要：仅使用键数量阈值会使这类表增长到超出 CPU 缓存容量。使用默认值时，键和状态宽度常规的表仍会按键数量冻结。0 将禁用字节限制，因此仅由键数量阈值决定。
...
仅在 ClickHouse Cloud 中生效。启用 [cache&#95;populated&#95;by&#95;fetch](/zh/reference/settings/merge-tree-settings/cache-populated-by-fetch#cache_populated_by_fetch) 时，用于将新的数据分区片段预下载到文件系统缓存中的后台线程数。设为 0 可禁用。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.8"},{"label": ""},{"label": "新增设置，可对响应正文应用通用压缩。"}]}]} />

对响应正文应用通用压缩，例如 `compression=gz`。请注意，此设置独立于 `Content-Encoding` (HTTP 压缩) 和旧版 `compress` 参数 (ClickHouse 原生压缩) 。在 URL 路径中指定压缩文件扩展名也可达到相同效果。

这是一个用于调整 HTTP 接口响应的设置：它会在查询执行前被处理 (响应缓冲区会预先设置) ，因此必须通过 HTTP URL 参数、URL 路径中的文件扩展名或用户 profile 提供，不能通过查询内的 `SETTINGS` 子句提供 (在该子句中无效，并且会被拒绝) 。
...
该等待时间按副本生效，因为每个副本各有一个连接池。等待超时后，本次尝试会以 `NO_FREE_CONNECTION` 失败。在分布式 `SELECT` 所使用的故障转移路径上，这与其他不可用副本的处理方式相同：查询会继续尝试其余副本，只有在所有副本都尝试过之后才会以 `ALL_CONNECTION_TRIES_FAILED` 失败。因此该设置并不限制整个查询的耗时；在有 `N` 个副本、每个副本尝试 `T` 次的情况下，总等待时间可能达到 `connection_pool_max_wait_ms` 乘以 `N` 再乘以 `T`，其中 `T` 为 [connections&#95;with&#95;failover&#95;max&#95;tries](/zh/reference/settings/session-settings/other#connections_with_failover_max_tries)，但不会小于 1，因为在放弃某个副本之前总会先尝试一次。若要限制查询本身的耗时，请使用 [max&#95;execution&#95;time](/zh/reference/settings/session-settings/max-execution#max_execution_time)。

可能的值：

* 正整数。
* 0 — 无限超时。该等待仍可被 `KILL QUERY` 中断；在默认的 [timeout&#95;overflow&#95;mode](/zh/reference/settings/session-settings/timeout-overflow-mode#timeout_overflow_mode) `throw` 下，也会被 `max_execution_time` 中断。
  在 `break` 模式下，时间限制不会取消查询，因此等待会一直持续到有连接被释放为止；此时 `KILL QUERY` 仍可将其中断。
...
* `clickhouse` (默认) — 标准 ClickHouse SQL。
* `kusto` — Kusto 查询语言。需要启用 Experimental 设置 `allow_experimental_kusto_dialect`。
* `prql` — PRQL。需要启用 Experimental 设置 `allow_experimental_prql_dialect`。
* `polyglot` — 将其他方言 (MySQL、PostgreSQL 等) 的 SQL 转译为 ClickHouse SQL。需要启用 Experimental 设置 `allow_experimental_polyglot_dialect`。
* `promql` — PromQL (Prometheus 查询语言) ，基于由 `promql_database`、`promql_table` 和 `promql_evaluation_time` 设置指定的 TimeSeries 表进行计算。
* `clickhouse_json` — 查询将不作为 SQL 文本处理，而是解释为 JSON AST (`parseQueryToJSON` 的输出) 。仍可使用普通形式的 `SET` 查询，以便切换回其他方言。需要启用 Experimental 设置 `enable_json_ast_dialect`。
...
指定在 INSERT 期间不构建和存储某些跳过索引。被排除的跳过索引仍会在[合并期间](/zh/reference/settings/merge-tree-settings/materialize#materialize_skip_indexes_on_merge)构建并存储，或通过显式执行
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.8"},{"label": "1"},{"label": "从 26.8 版本开始，`EXPLAIN SYNTAX` 会将格式化后的查询作为单条记录（包含嵌入的换行符）返回，而非每行一条记录。将此设置为 `false` 可恢复 26.8 之前每行一条记录的输出。"}]}]} />

将 `EXPLAIN SYNTAX` 的输出作为单条记录 (包含嵌入的换行符) 返回，而非每行一条记录，从而使结果成为单个可恢复的行 (例如，`SELECT count() FROM (EXPLAIN SYNTAX ...)` 返回 `1`) 。

在 `EXPLAIN SYNTAX` 语句中显式指定 `single_record` 选项 (例如，`EXPLAIN SYNTAX single_record = 0 SELECT ...`) 将始终覆盖此设置。

将此设置为 `false` 可恢复 26.8 之前每行一条记录的输出；或者将 `compatibility` 设置为任何早于 `26.8` 的版本。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "24.2"},{"label": "1000"},{"label": "`extractKeyValuePairs` 函数可生成的键值对的最大数量。用于防止占用过多内存。"}]}]} />
...
类文件表引擎的默认分区策略。仅适用于路径中不含 glob 或 `{_partition_id}` 占位符的 `CREATE` 查询。包含 `{_partition_id}` 的路径始终使用 `wildcard`。包含其他 glob 的路径不使用分区策略，并忽略 `PARTITION BY`。如果此设置为 `wildcard`，但路径中没有 `{_partition_id}`，则不使用分区策略；无法将此选择持久化到引擎参数中的表引擎 (例如 `HDFS`) 将拒绝此类 `CREATE`。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.8"},{"label": ""},{"label": "新增设置，用于在查询外层添加 WHERE 子句。"}]}]} />

以包装子查询的方式为查询添加 `WHERE` 子句。多个过滤器会使用 AND 组合。HTTP 接口支持多个 `filter` URL 参数，这些参数会按顺序使用 AND 相互组合，并与此设置的值组合。

这是一项由引擎应用于已解析查询的查询构造设置 (将查询包装为派生表) ，因此可与现有查询组合使用，并适用于所有协议：可通过 HTTP URL 参数、查询内的 `SETTINGS` 子句或用户 profile 提供。

它会影响产生结果的 `SELECT` / `UNION` 查询。对于写入查询 (`INSERT … SELECT`、`CREATE … AS SELECT`) ，仅当源 `SELECT` 在自身的 `SETTINGS` 子句中指定此设置时才会生效；从 profile 或 session 继承的值，或在 `INSERT` / `CREATE` 语句本身中设置的值，不会传播到源 `SELECT`——这与其他所有设置的非传播规则相同。

<Danger>
  `filter` **不是**访问控制机制，不得用作 [行级安全策略](/zh/concepts/features/security/access-rights#row-policy-management) 或 `additional_table_filters` 设置的替代方案。它只会在包装子查询的外层添加 `WHERE`，因此在应用过滤器前，底层数据仍会被读取和处理——查询在处理过程中仍可能观察到被过滤掉的行 (例如，使用 `throwIf` 通过错误路径泄露信息) 。如需限制用户可访问的行，请使用行级安全或 `additional_table_filters`。
</Danger>
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.8"},{"label": ""},{"label": "新增设置，可覆盖查询的输入和输出 FORMAT。"}]}]} />

覆盖查询的输入和输出 `FORMAT`。其优先级高于查询中指定的 format 和文件扩展名。对于各自的方向，更具体的 `input_format` 和 `output_format` 设置优先于通用的 `format` 设置。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.8"},{"label": "None"},{"label": "用于选择一种帧格式的新设置。该格式可通过 HTTP 在单个输出流中复用数据、totals、extremes、Progress 信息、日志和 profile events 数据包。"}]}]} />

允许在单个 stream 中复用查询响应的不同部分：数据 chunks、totals 和 extremes、Progress 信息数据包、profile events (指标) 以及 server logs，即 native protocol 支持的所有内容。

帧格式独立于输出格式：它们通过分隔并可能编码这些字节 chunks，来封装任意输出格式生成的字节。所有 `data`、`totals` 和 `extremes` 数据包的载荷拼接后，与不使用 framing 时输出格式生成的内容完全一致。辅助数据包 (Progress 信息、日志、profile events、exceptions) 以 JSON 表示。

有一个特意设计的例外：某些输出格式因无法表示 totals 和 extremes 而在普通输出中省略它们 (`JSONCompactEachRow` 系列) ，但在 framing 下会将它们写入 `totals` 和 `extremes` 数据包。对于此类 formats，仅拼接 `data` 数据包即可得到与未分帧输出完全相同的结果，而 `totals` 和 `extremes` 数据包则携带未分帧输出中没有的额外行。

设置了 `send_logs_level` 时会包含 server logs；启用 `send_profile_events` 时会包含 profile events (它们最多每 `interactive_delay` 微秒发送一次，Progress 信息数据包同样受 `interactive_delay` 限流) 。

成功的 stream 会以最终的 `progress` 数据包结束。该数据包携带最终计数器 (`result_rows`、`result_bytes`、`memory_usage`) ，在查询完成日志记录产生的尾随 `log` 和 `profile_events` 数据包之后写入，类似于 native protocol 中的最终 Progress 信息数据包。失败时，`exception` 数据包会成为最后一个数据包；但有一个例外：如果在响应中已生成部分数据包 stream、无法再丢弃时发生失败 (例如数据包写入中途失败、`exception` 数据包本身发送失败，或响应 stream 在刷新或关闭时失败) ，framing 会以失败关闭的方式处理：stream 会在没有终止 `exception` 数据包的情况下结束，客户端看到的是截断的响应和中止的 HTTP connection，而不是可解析的终止数据包。部分数据包 stream 之后绝不会追加任何内容，因此普通 HTTP 错误正文绝不会混入其中。

查询仅通过自身的 `SETTINGS` clause 启用的内容——帧格式、`send_logs_level` 或 `send_profile_events`——在查询解析完成前无法得知，因此相应的日志和 profile events 只能从查询执行开始后捕获。只有当设置来自 session 或 URL 时，才能捕获 parse、plan 和 analysis 阶段的日志和 profile events。例如，某个查询在 analysis 期间失败 (如引用未知表) ，且仅在其 `SETTINGS` clause 中启用 `send_logs_level`，则只会传递 `exception` 数据包，不会传递 analysis 阶段的日志；请在 session 或 URL 中设置 `send_logs_level` 以捕获这些日志。

同样的延迟发现限制也适用于 `send_logs_source_regexp`：日志队列会在捕获每个条目时按 source 过滤，因此仅在查询自身的 `SETTINGS` clause 中设置的 regexp 会从查询执行开始后生效。parse、plan 和 analysis 阶段的 `log` 数据包会按该设置的 session 或 URL 值过滤 (如果未在其中设置，则不过滤) ，因此其中可能包含不匹配查询级别 regexp 的 sources；反之，被更严格的 session 或 URL regexp 丢弃的条目，也无法通过更宽泛的查询级别 regexp 恢复。请在 session 或 URL 中设置 `send_logs_source_regexp`，以过滤整个查询生命周期。

该设置目前仅适用于 HTTP protocol，其他 interfaces 会忽略它。

可能的值：

* `None` - 将所有适用内容 (数据、totals、extremes、Progress 信息) 透明地传递给输出格式，并忽略所有不适用内容 (指标、日志) ，因此默认情况下即可正常工作。
* `EventStream` - 将数据包封装为 HTTP 服务器发送事件 (`text/event-stream`) 。每个数据包都会作为具有相应名称的事件发送：`data`、`totals`、`extremes`、`progress`、`log`、`profile_events`、`exception`。Progress 信息和其他辅助数据包以 JSON 格式发送。由于服务器发送事件是一种将换行符 (包括回车符 `\r`) 视为分隔符的文本协议，格式化数据块会经过 base64 编码并置于事件的单个 `data` 字段中；解码后即可得到包含所有换行符的完整格式化载荷。`Content-Type` 包含 `payload=base64` 参数以表明这一点。通过这种方式，任何输出格式都可进行字节精确传输，无论是文本还是二进制格式。
* `JSONEachPacketBase64` - 每个数据包都是单独一行中的 JSON 对象，格式化数据经过 base64 编码，例如 `{"packet":"data","data":"eyJ4IjoxfQo="}`。适用于二进制输出格式。
* `JSONEachPacketString` - 每个数据包都是单独一行中的 JSON 对象，格式化数据会放入字符串中，例如 `{"packet":"data","data":"{\"x\":1}\n"}`。

`JSONEachPacketString` 会将载荷字节放入 JSON 字符串中，而不对其进行验证或重新编码。`String` 和 `FixedString` 列可存储任意字节，因此文本输出格式 (如 `JSONEachRow`、`TSV` 或 `CSV`) 可能会为此类值输出无效的 UTF-8——正如 ClickHouse&#39; 自身的 `JSONEachRow` 在默认 `output_format_json_validate_utf8 = 0` 时所做的一样——因此生成的 NDJSON 流不保证是有效的 UTF-8。若要以字节精确方式传输任意字节，请使用 `JSONEachPacketBase64`。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.8"},{"label": ""},{"label": "新增设置，可覆盖查询的输入格式。"}]}]} />

覆盖查询的输入格式，其优先级高于查询中指定的格式。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.8"},{"label": "0"},{"label": "类型从 UInt64 扩展为 Float，以支持负值和小数，并透传给 ClickHouse 原生对负数\/小数 `LIMIT` 的支持。"}]}]} />

设置从查询结果中获取的最大行数。它会调整 [LIMIT](/zh/reference/statements/select/limit) 子句指定的值。该值会透传给 `LIMIT`，并接受 `LIMIT` 支持的所有值，包括负值 (从结果末尾开始计数) 和 `(0, 1)` 范围内的小数 (表示结果的占比) 。
...
* 正整数 — 精确的行数。
* 负整数 — 返回最后 N 行。
* 开区间 `(0, 1)` 内的实数 — 返回结果中相应比例的数据。

此设置会影响产生结果的 `SELECT` / `UNION` 查询。对于写入查询 (`INSERT … SELECT`、`CREATE … AS SELECT`) ，只有当源 `SELECT` 在其自身的 `SETTINGS` 子句中指定此设置时才会生效；从 profile 或 session 继承的值，或在 `INSERT` / `CREATE` 语句本身设置的值，都不会传播到源 `SELECT`。这与其他所有设置适用的“不传播”规则相同。
...
启用后，会自动调整控制分布式查询计划暂不支持功能的相关设置：

* `enable_parallel_replicas = 0` 和 `automatic_parallel_replicas_mode = 0` — 分布式计划自行分配工作；
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.8"},{"label": "0"},{"label": "类型从 UInt64 扩展为 Float，以支持负值和小数；该值会直接传递给 ClickHouse 原生支持负值和小数的 `OFFSET`。"}]}]} />

设置查询返回行之前要跳过的行数。它会调整由 [OFFSET](/zh/reference/statements/select/offset) 子句设置的偏移量。该值会直接传递给 `OFFSET`，可接受 `OFFSET` 支持的所有值，包括负值和 `(0, 1)` 区间内的小数。
...
此设置会影响产生结果的 `SELECT` / `UNION` 查询。对于写入查询 (`INSERT … SELECT`、`CREATE … AS SELECT`) ，只有当源 `SELECT` 在自身的 `SETTINGS` 子句中指定了该设置时才会生效；从 profile 或会话继承的值，或在 `INSERT` / `CREATE` 语句本身中设置的值，都不会传播到源 `SELECT`——这与其他任何设置的非传播规则相同。

## order {#order}

<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.8"},{"label": ""},{"label": "新增设置，可为查询添加外层 ORDER BY 子句。"}]}]} />

将 `ORDER BY` 子句作为外层子查询添加到查询中。接受任意表达式列表。

这是引擎在已解析的查询上应用的查询构造设置 (将查询包装为派生表) ，因此可与现有查询组合使用，并适用于所有协议：可通过 HTTP URL 参数、查询内的 `SETTINGS` 子句或用户 profile 提供。

它会作用于产生结果的 `SELECT` / `UNION` 查询。对于写入查询 (`INSERT … SELECT`、`CREATE … AS SELECT`) ，只有源 `SELECT` 在自身的 `SETTINGS` 子句中包含该设置时才会生效；从 profile 或 会话 继承的值，或在 `INSERT` / `CREATE` 语句本身中设置的值，不会传播到源 `SELECT`——这与其他所有设置遵循相同的非传播规则。

## output_format {#output_format}

<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.8"},{"label": ""},{"label": "新增设置，可覆盖查询的输出格式。"}]}]} />

覆盖查询的输出格式。其优先级高于查询中指定的格式、文件扩展名指定的格式或通过 `default_format` 指定的格式。

## 页码 {#page}

<SettingsInfoBlock type="Double" default_value="0" />

<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.8"},{"label": "0"},{"label": "用于分页 HTTP 响应的新设置，等同于 offset = limit * (page - 1)。类型为 Float，因此可容纳负值或小数值（会传递给 SQL `LIMIT`\/`OFFSET`）。"}]}]} />

设置分页结果的页码。等同于 `offset = limit * (page - 1)`。仅当设置了 `limit` 且未设置 `offset` 时才能指定。页码从 1 开始计数。与 `limit` 和 `offset` 一样，支持负值和小数值。

这是引擎应用于已解析查询的查询构造设置 (会将查询包装为派生表) ，因此可与现有查询组合，并适用于所有协议：可通过 HTTP URL 参数、查询内的 `SETTINGS` 子句或用户 profile 提供。

它会影响产生结果的 `SELECT` / `UNION` 查询。对于写入查询 (`INSERT … SELECT`、`CREATE … AS SELECT`) ，仅当源 `SELECT` 在其自身的 `SETTINGS` 子句中包含此设置时才会生效；从 profile 或会话继承的值，或在 `INSERT` / `CREATE` 语句本身中设置的值，都不会传播到源 `SELECT`——这与任何其他设置适用的非传播规则相同。
...
如果设置为任意负值，则会采用 [query&#95;metric&#95;log 设置](/zh/reference/settings/server-settings/settings/query#query_metric_log) 中的 `collect_interval_milliseconds` 值；如果未设置，则默认使用 1000。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.8"},{"label": "0"},{"label": "用于启用可恢复 `BACKUP FROM SNAPSHOT` 的新 Experimental 设置。"}]}]} />

启用可恢复的 `BACKUP FROM SNAPSHOT`：失败后可重新执行，而无需重新复制已完成批次中的
条目。仅适用于 ClickHouse Cloud 中目录形式的
`S3` 和 `AzureBlobStorage` 目标端。在不支持 `BACKUP FROM SNAPSHOT` 的 ClickHouse 开源构建中启用此设置，
会使 `BACKUP` 因 `WRONG_BACKUP_SETTINGS` 而失败。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.8"},{"label": "0"},{"label": "新增设置，用于在后台运行查询，使其脱离提交该查询的连接，并丢弃结果。"}]}]} />

启用后，服务器会在后台调度查询，立即返回一个空的成功结果，并继续运行查询直至完成，不受连接状态影响。

后台查询不会在服务器重启后保留。关闭服务器时，它遵循与前台查询相同的服务器设置：`shutdown_wait_unfinished_queries` 决定取消查询还是等待其完成 (无论哪种情况，队列中的条目都会被丢弃，且不会写入 `system.query_log`) ，`shutdown_wait_unfinished` 则限制服务器的最长等待时间。

可通过其 `query_id` 跟踪查询：运行期间可在 `system.processes` 中查看；查询完成且查询日志条目已刷新后，可在 `system.query_log` 中查看。

适用于通过原生 TCP 和 HTTP 协议接收的查询。通过 HTTP 时，将该设置作为 URL 参数传递。无法通过 `SET` 更改；可按查询启用，或在用户或 profile 级别启用。

主要用例是长时间运行的 `INSERT ... SELECT`，以确保客户端连接断开时查询不会丢失。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.8"},{"label": ""},{"label": "新增设置，可将查询包装为 `SELECT <expr_list> FROM (<query>)`。"}]}]} />

将查询包装为带有显式 `SELECT` 表达式列表的子查询。非空时，会将产生结果的查询包装为 `SELECT <expr_list> FROM (<query>)`。

这是引擎应用于已解析查询的查询构造设置 (将其包装为派生表) ，因此可与现有查询组合使用，并适用于所有协议：可通过 HTTP URL 参数、查询内的 `SETTINGS` 子句或用户 profile 提供。

它会影响产生结果的 `SELECT` / `UNION` 查询。对于写入查询 (`INSERT … SELECT`、`CREATE … AS SELECT`) ，仅当源 `SELECT` 在其自身的 `SETTINGS` 子句中指定该设置时才会生效；从 profile 或 会话 继承的值，或在 `INSERT` / `CREATE` 语句本身上设置的值，不会传播到源 `SELECT`——这与其他任何设置均适用的不传播规则相同。
...
值为 &#39;&#39; (空字符串) 表示当前 session 或查询的隐式时区与[server 时区](/zh/reference/settings/server-settings/settings/other#timezone)相同。
...
* [timezone](/zh/reference/settings/server-settings/settings/other#timezone)
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.8"},{"label": ""},{"label": "新增设置，可在查询外层添加简单的 ORDER BY 子句。"}]}]} />

将简单的 `ORDER BY` 子句作为外层子查询添加到查询中。接受以逗号分隔的标识符列表或按位置引用的列 (正整数) ，并可带有可选的 `+` (ASC) 或 `-` (DESC) 前缀。示例：`sort=a,-b` 按 `a` 升序、`b` 降序排序；`sort=1,-2` 按第一列升序、第二列降序排序。不能与 `order` 一起使用。

这是引擎作用于已解析查询的查询构造设置 (将其包装为派生表) ，因此可与现有查询组合使用，并适用于所有协议：可通过 HTTP URL 参数、查询内的 `SETTINGS` 子句或用户 profile 提供。

它会影响产生结果的 `SELECT` / `UNION` 查询。对于写入查询 (`INSERT … SELECT`、`CREATE … AS SELECT`) ，仅当源 `SELECT` 在其自身的 `SETTINGS` 子句中指定该设置时才会生效；从 profile 或会话继承的值，或在 `INSERT` / `CREATE` 语句本身中设置的值，都不会传播到源 `SELECT`——这与其他任何设置适用的非传播规则相同。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.8"},{"label": "10000"},{"label": "新增设置，用于限制估算大型集合 `IN` 选择性时的成本：超过此限制后，估算器会使用集合大小及其边界范围，而非精确范围。在 26.8 之前，估算没有上限，因此先前的值为 0（无限制）；与早期版本兼容时，`compatibility` 会恢复任意大小集合的精确范围。"}]}]} />

`IN` 运算符右侧集合的最大大小。对于不超过该大小的集合，选择性估算器会推导出该集合覆盖的精确范围。推导这些范围需要为每个元素分配一个 `Field`、进行一次排序，并对每个元素执行一次统计信息探测；对于大型集合，这些操作会成为查询计划的主要开销。超过此限制后，估算器会改为根据集合大小及其边界范围推导选择性，只需对集合进行一次线性遍历，无需排序或逐元素执行统计信息探测。零表示无限制。
...
允许或禁止空 INSERT，默认启用 (空插入时会抛出错误) 。仅适用于通过 [`ClickHouse 客户端`](/zh/concepts/features/interfaces/client) 或 [gRPC 接口](/zh/concepts/features/interfaces/grpc) 执行的 INSERT。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.8"},{"label": "1"},{"label": "新增设置：当请求的时间范围完全落在 TimeSeries 表近期样本表的 TTL 窗口内时，从该表读取数据。"}]}]} />

当请求的整个时间范围完全落在近期样本表的生存时间 (TTL) 窗口内时，从 [TimeSeries](/zh/reference/engines/table-engines/integrations/time-series) 表的近期样本表而非主样本表读取数据 (请参阅 TimeSeries 表引擎的 `recent_samples_ttl_seconds` 设置) 。
