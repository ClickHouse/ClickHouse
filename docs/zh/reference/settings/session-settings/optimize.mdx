...
* 对于 `Map`，[has](/zh/reference/functions/regular-functions/array-functions#has) 和 [notHas](/zh/reference/functions/regular-functions/array-functions#notHas) 优化为读取 [keys](/zh/reference/data-types/map#reading-subcolumns-of-map) 子列。
* [mapContainsKeyLike](/zh/reference/functions/regular-functions/tuple-map-functions#mapContainsKeyLike) 优化为读取 [keys](/zh/reference/data-types/map#reading-subcolumns-of-map) 子列。
* [mapContainsValueLike](/zh/reference/functions/regular-functions/tuple-map-functions#mapContainsValueLike) 优化为读取 [values](/zh/reference/data-types/map#reading-subcolumns-of-map) 子列。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.9"},{"label": "1"},{"label": "新增设置，可根据 WHERE 子句自动为变更裁剪分区"}]}]} />

启用后，对于 `ReplicatedMergeTree` 家族的表，ClickHouse 会自动识别 `ALTER TABLE UPDATE`/`DELETE` 变更以及轻量级 `UPDATE`/`DELETE` 语句的 WHERE 子句中的分区键条件，并仅处理受影响的分区，而非全部分区。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.9"},{"label": "1"},{"label": "新增设置，用于在使用 `FINAL` 修饰符从 `ReplacingMergeTree` 表按 sorting key 逆序读取时启用按序读取（read-in-order）优化。"}]}]} />

允许在带 `FINAL` 修饰符的 `SELECT` 查询中，从 [ReplacingMergeTree](/zh/reference/engines/table-engines/mergetree-family/replacingmergetree) 表按 sorting key 的逆序读取数据。仅在同时启用 [optimize&#95;read&#95;in&#95;order](/zh/reference/settings/session-settings/optimize#optimize_read_in_order) 时生效。
