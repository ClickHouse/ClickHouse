...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.8"},{"label": ""},{"label": "新增设置，用于指定在 s3 表函数和 S3 表引擎中解析相对 URL 时使用的基准 URL。"}]}]} />

在 [s3](/zh/reference/functions/table-functions/s3) 表函数和 [S3](/zh/reference/engines/table-engines/integrations/s3) 表引擎中解析相对 URL 时使用的基准 URL；同样适用于共享其配置的表函数 (`s3Cluster`、`gcs`、`oss`) 。

设置后，不含 scheme 的 URL 会根据 RFC 3986 以 `s3_base` 为基准进行解析，规则与 [url&#95;base](/zh/reference/settings/session-settings/url#url_base) 相同。已包含 scheme 的 URL 将按原样使用。

例如，若 `s3_base` 为 `s3://clickhouse-public-datasets/`，则 `s3('hits_compatible/hits.csv')` 会读取 `s3://clickhouse-public-datasets/hits_compatible/hits.csv`。

基准 URL 可以采用 `s3` 表函数接受的任何形式，例如 `s3://bucket/`、`https://bucket.s3.amazonaws.com/` 或 `https://endpoint/bucket/`。
