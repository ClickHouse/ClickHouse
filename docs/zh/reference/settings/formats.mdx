...
对于 AvroConfluent 格式：与 Confluent Schema Registry 通信时遇到瞬时故障 (传输超时、连接被拒绝、DNS 错误、HTTP 5xx/408/429) 时的最大重试次数。设置为 0 可禁用重试。允许的最大值为 20。Schema 验证错误 (HTTP 409、格式错误的 Avro JSON) 不会重试。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "1"},{"label": "用于控制 GeoJSON 格式在读取和写入时是否强制执行 RFC 7946 几何有效性（每条 line 和 Ring 的最小点数、Ring 闭合、非空多重几何）的新设置"}]}]} />

控制 `GeoJSON` 格式是否在读写两个方向上强制执行 RFC 7946 的几何有效性。

启用时 (默认) ，违反 GeoJSON 形态规则的几何将被拒绝：少于两个 Point 的 `LineString` (或 `MultiLineString` 中的一条 line) ；少于四个 Point，或首尾 Point 不一致 (即 Ring 未闭合) 的 `Polygon` 或 `MultiPolygon` 的 Ring；以及空的 `MultiLineString`、`Polygon` 或 `MultiPolygon`。这同时适用于读取和写入 (读取时，此类文档会被拒绝；写入时，此类 ClickHouse 值会被拒绝，而不是生成一个会被输入格式拒绝的文档) 。

禁用时，将不再强制执行这些形态规则：这类几何会按原样读取和写入，因此退化几何也可以完整往返，但写出的文档可能不是有效的 GeoJSON。

此校验仅针对结构：只检查点数和 Ring 是否闭合。它不会检查形态在几何上的正确性——不会强制要求 Ring 方向 (右手法则 / winding order) ，并且会接受结构上有效但几何上退化的几何，例如面积为零的 Polygon、自相交的 Ring，或孔位于外环之外的 Polygon。无论此设置是否启用，非有限坐标 (`NaN`、`Inf`) 都会被拒绝，因为它们无法表示为 JSON 数值。
...
控制在 CSV 中如何将缺失值读取为 `Nullable(String)`。缺失值指的是逗号之间、之前或之后未被引号包围的空白内容。如果启用此设置，则无论 `input_format_csv_empty_as_default` 的值是什么，`Nullable(String)` 的缺失值都会被解释为空 `String`，而不是 NULL。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.2"},{"label": "0"},{"label": "用于限制输入格式在发出块之前最长等待时间（毫秒）的新设置"}]}]} />

限制在基于行的输入格式中，解析期间发出块前的最长等待时间 (以毫秒为单位) 。0 表示不限制。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "25.5"},{"label": "1"},{"label": "读取 Parquet 文件时，会根据 WHERE/PREWHERE 表达式以及 Parquet 元数据中的布隆过滤器跳过整个行组。"}]}, {"id": "row-2","items": [{"label": "24.10"},{"label": "0"},{"label": "读取 Parquet 文件时，会根据 WHERE/PREWHERE 表达式以及 Parquet 元数据中的布隆过滤器跳过整个行组。"}]}]} />

读取 Parquet 文件时，会根据 WHERE 表达式以及 Parquet 元数据中的布隆过滤器跳过整个行组。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "20.5"},{"label": "1"},{"label": "启用对带有 WithNames/WithNamesAndTypes 后缀的格式使用名称请求头"}]}]} />

启用或禁用在插入数据时检查列顺序。

为提高插入性能，如果你确定输入数据的列顺序与目标表一致，建议禁用此检查。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "1"},{"label": "新设置。当启用 output_format_csv_serialize_tuple_into_separate_columns 时，CSVWithNames\/CSVWithNamesAndTypes 的表头现在会将 Tuple 列展平为其叶子字段，使表头宽度与数据一致。设为 false 可恢复此前的单名称表头。"}]}]} />

当启用 [output&#95;format&#95;csv&#95;serialize&#95;tuple&#95;into&#95;separate&#95;columns](#output_format_csv_serialize_tuple_into_separate_columns) 时，`CSVWithNames` 和 `CSVWithNamesAndTypes` 的表头行会将每个 Tuple 列展平为其叶子字段 (点式名称，如 `t.a`、`t.b`，以及叶子类型名称) ，因此表头中的列数与数据相同。对于 `CustomSeparated*`，只有在 `format_custom_escaping_rule = 'CSV'` 且 `format_custom_field_delimiter` 是与 `format_csv_delimiter` 相同的单个字符时，才会进行这种展平；否则 (例如使用默认的制表符分隔符，或 `format_custom_field_delimiter = '|'`) 表头将保持未展平状态，从而继续与数据匹配。将其设为 `0` 可保留此前的行为，即表头保留单个顶层 Tuple 名称和类型。

注意：当 `input_format_with_names_use_header = 1` 时，扁平化后的表头不会按名称读回为 Tuple。要将这类数据读回到 Tuple 中，可以在输出时将此设置设为 `0`，或者在读取时使用 `input_format_with_names_use_header = 0` (对于 `*WithNamesAndTypes` 格式 `CSVWithNamesAndTypes` 和 `CustomSeparatedWithNamesAndTypes`，还需同时设置 `input_format_with_types_use_header = 0`，否则扁平化后的类型行会与单个顶层 Tuple 输入字段进行校验，并因此被拒绝) 。
