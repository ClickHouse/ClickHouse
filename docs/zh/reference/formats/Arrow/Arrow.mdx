...
| `input_format_arrow_use_native_reader`                                       | 对 `Arrow` 和 `ArrowStream` 格式使用 ClickHouse 原生读取器，而不是 Apache Arrow 库。设置为 `0` 时使用 Apache Arrow 库读取器。                        | `1`         |
...
| `output_format_arrow_use_native_writer`                                      | 对 `Arrow` 和 `ArrowStream` 格式使用 ClickHouse 原生写入器，而不是 Apache Arrow 库。设置为 `0` 时使用 Apache Arrow 库写入器。                        | `1`         |
