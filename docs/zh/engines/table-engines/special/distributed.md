...
- `async_insert_batch` - 等同于 [distributed_async_insert_batch](../../../operations/settings/settings.md#distributed_async_insert_batch)

- `async_insert_split_batch_on_failure` - 等同于[distributed_async_insert_split_batch_on_failure](../../../operations/settings/settings.md#distributed_async_insert_split_batch_on_failure)

- `async_insert_sleep_time_ms` - 等同于 [distributed_async_insert_sleep_time_ms](../../../operations/settings/settings.md#distributed_async_insert_sleep_time_ms)

- `async_insert_max_sleep_time_ms` - 等同于 [distributed_async_insert_max_sleep_time_ms](../../../operations/settings/settings.md#distributed_async_insert_max_sleep_time_ms)
...
数据是异步写入的。对于分布式表的 INSERT，数据块只写本地文件系统。之后会尽快地在后台发送到远程服务器。发送数据的周期性是由[distributed_async_insert_sleep_time_ms](../../../operations/settings/settings.md#distributed_async_insert_sleep_time_ms)和[distributed_async_insert_max_sleep_time_ms](../../../operations/settings/settings.md#distributed_async_insert_max_sleep_time_ms)设置。分布式引擎会分别发送每个插入数据的文件，但是你可以使用[distributed_async_insert_batch](../../../operations/settings/settings.md#distributed_async_insert_batch)设置启用批量发送文件。该设置通过更好地利用本地服务器和网络资源来提高集群性能。你应该检查表目录`/var/lib/clickhouse/data/database/table/`中的文件列表(等待发送的数据)来检查数据是否发送成功。执行后台任务的线程数可以通过[background_distributed_schedule_pool_size](../../../operations/settings/settings.md#background_distributed_schedule_pool_size)设置。
