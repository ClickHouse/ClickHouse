...
与 LSM trees <sup>[&#91;58&#93;](#page-13-7)</sup> 及其在各种数据库中的实现 <sup>[&#91;13,](#page-12-6)</sup> <sup>[26,](#page-12-7)</sup> <sup>[&#91;56&#93;](#page-13-8)</sup> 相比，ClickHouse 将所有分片一视同仁，而不是把它们组织成层级结构。因此，合并不再局限于同一层级中的分片。由于这也放弃了分片按时间顺序隐式排列的特性，因此需要采用不依赖 tombstone 的替代更新和删除机制 (见第 [3.4)](#page-4-0) 节) 。ClickHouse 会将插入直接写入磁盘，而其他基于 LSM tree 的存储通常会使用预写日志 (见第 [3.7)](#page-5-1) 节) 。
