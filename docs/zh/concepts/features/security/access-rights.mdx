...
* 为至少一个用户账户启用 基于 SQL 的访问控制与账户管理。
  ...
  默认情况下，所有用户的 基于 SQL 的访问控制与账户管理均处于禁用状态。你需要在 `users.xml` 配置文件中至少配置一个用户，并将 [`access_management`](/zh/concepts/features/configuration/settings/settings-users#access_management-user-setting)、`named_collection_control`、`show_named_collections` 和 `show_named_collections_secrets` 这些设置的值设为 1。
