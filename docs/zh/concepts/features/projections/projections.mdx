...
我们按维度 `toYear(date)`、`district` 和 `town` 创建一个聚合PROJECTION：
...
以下查询对比了使用投影与不使用投影时的性能差异。若要禁用投影的使用，我们使用设置 [`optimize_use_projections`](/zh/reference/settings/session-settings/optimize-use#optimize_use_projections)，该设置默认处于启用状态。
