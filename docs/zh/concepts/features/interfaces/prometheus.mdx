...
<Note>
  目前仅实现了查询端点 `/api/v1/query` 和 `/api/v1/query_range` 以及元数据端点 `/api/v1/series` 和 `/api/v1/labels`。`/api/v1/series` 至少需要一个 `match[]` series 选择器，支持可选的 `start`、`end` 和 `limit` 参数，并返回每个选择器匹配的 series 的并集。`/api/v1/labels` 接受相同的参数，其中 `match[]` 为可选项，并返回匹配的 series 的已排序标记名称 (未提供选择器时则返回所有 series 的标记名称) 。Grafana Prometheus 数据源用于浏览标记、模板变量和查询构建器自动补全的标记值端点 (`/api/v1/label/<name>/values`) 尚未实现，调用时会返回错误。请使用代码模式编写 PromQL 表达式，而不要使用查询构建器。
</Note>
