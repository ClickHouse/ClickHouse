...
为了能够充分利用底层资源的全部容量，你应当使用 `inflight_limit`。请注意，较低的 `max_requests` 或 `max_cost` 数值可能导致资源利用不充分，而过高的数值则可能导致调度器内部的队列为空，进而使子树中的策略被忽略 (造成不公平或忽略优先级) 。另一方面，如果你想保护资源不被过度使用，则应当使用 `bandwidth_limit`。当 `duration` 秒内消耗的资源量超过 `max_burst + max_speed * duration` 字节时，它会进行限流。同一资源上的两个 `bandwidth_limit` 节点可用于限制较短时间间隔内的峰值带宽，以及较长时间内的平均带宽。
