...
<Warning>
  在每个 profile 中都不要把 `readonly` 加入 `changeable_in_readonly` 列表，同时请记住：profile 的限制是全系统生效的，而非按用户生效。
</Warning>

不要把 `readonly` 本身标记为 `changeable_in_readonly`。否则，以 `readonly = 1` 启动的 session 就可以执行 `SET readonly = 0`，从而重新获得执行写入查询的能力 (只要其现有特权本就允许) 。

这一点在任何情况下都很重要，而不仅限于分配给该用户的 profile：

* 任何 session 都可以通过 `SET profile` 切换到任意 profile，因为 profile 的选择不做访问检查 (仅受当前已生效的设置约束限制) 。
* 通过 HTTP 访问时，只有在实际生效值本来为 `0` 的情况下，`GET` 请求才会被强制设为 `readonly = 2`。因此，如果某个 profile 设置了 `readonly = 1`，却又允许在只读模式下更改 `readonly`，这项保护便会形同虚设——一个 `GET` 请求即可将其改回 `0` 并执行写入。
