...
<Note>
  只有在 [enable&#95;analyzer](/zh/reference/settings/session-settings/allow-experimental#allow_experimental_analyzer) 设置为 true 时，查询条件缓存才会生效，而这也是默认值。
</Note>
...
<div id="top-k-queries">
  ### `ORDER BY ... LIMIT n` (TopK) 查询
</div>

默认情况下，使用 TopK 优化的查询 (`ORDER BY <column> LIMIT n`，通过动态筛选或排序列上的 minmax 跳过索引加速) 不会使用查询条件缓存：此类读取既不会查阅，也不会填充该缓存。
TopK 读取可根据查询执行期间不断变化的阈值跳过粒度，因此其缓存条目需要额外谨慎处理；在充分确认这些条目的正确性之前，需通过单独的设置 [`use_query_condition_cache_for_top_k`](/zh/reference/settings/session-settings/use-query) (默认：禁用) 启用。
仅当同时启用 `use_query_condition_cache` 时，该设置才会生效。
