...
* **Hash JOIN (默认) ：** 适用于能装入内存的中小型右侧表，速度很快。
...
你可以通过设置 `join_algorithm = 'auto'` 让 ClickHouse 选择最佳算法，也可以根据你的工作负载显式控制它。默认值是 `direct,parallel_hash,hash`，因此当右侧是字典或键值引擎时，ClickHouse 会使用 direct join，否则会使用 hash join（`hash` 与 `parallel_hash` 是同一实现，不会用来选择并行度）。如果你需要为优化性能或内存开销而选择 JOIN 算法，我们建议参考[本指南](/zh/concepts/features/operations/select/joining-tables#choosing-a-join-algorithm)。
