...
-   修复了 `Dictionary` 表的问题 (抛出 `偏移量大小与列大小不匹配` 或 `未知压缩方法` 异常). 这个bug出现在18.10.3版本. [#2913](https://github.com/ClickHouse/ClickHouse/issues/2913)
...
-   修复了如果从远程服务器请求包含带有子查询的 IN 表达式的单个列，则在执行分布式查询时发生的 `未找到列` 错误. [#3087](https://github.com/ClickHouse/ClickHouse/pull/3087)
...
-   修复了`windowFunnel聚合函数`中的一个错误 [Winter Zhang](https://github.com/ClickHouse/ClickHouse/pull/2735).
...
-   如果在分布式查询中同时使用版本 1.1.54388(或更高版本)和旧版本的服务器，并且查询具有不带 `AS` 关键字的 `cast(x, 'Type')` 表达式并且没有 大写的 `cast` 一词, 将抛出异常, 并显示类似 `块中未找到列 cast(0, 'UInt8')` 的消息. 解决方案: 更新整个集群上的服务器.
...
-   如果在左侧指定了数组，则删除了 IN 表达式的特殊解释. 以前, 表达式 `arr IN (set)` 被解释为"至少一个 `arr` 元素属于 `set`". 要在新版本中获得相同的行为，请编写 `arrayExists(x -> x IN (set), arr)`.
...
-   新增`maxIntersections(left_col, right_col)`聚合函数, 返回最大同时相交区间数`[left; 对]`. `maxIntersectionsPosition(left, right)` 函数返回"最大"间隔的开始. ([Michael Furmur](https://github.com/ClickHouse/ClickHouse/pull/2012)).
...
-   修复了对复制表进行 OPTIMIZE 查询后可能导致合并不一致的错误 (您可能会在日志消息中看到它 `Part ...与上一部分相交` ).
