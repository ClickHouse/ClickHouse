...
<Steps>
  <Step title="使用 curl 安装 ClickHouse" id="install-clickhouse-using-curl">
    ...
    在 Linux 和 macOS 上，这还会将 ClickHouse 命令行客户端 [`clickhousectl`](/zh/install/clickhousectl)
    安装到 `~/.local/bin` (并创建一个 `chctl` 别名)。`clickhousectl`
    可帮助你安装和管理多个本地 ClickHouse 版本、在后台运行服务器，
    以及管理 ClickHouse Cloud。有关基于命令行客户端的工作流程，请参阅 [clickhousectl 安装页面](/zh/install/clickhousectl)。
