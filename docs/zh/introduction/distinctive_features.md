...
在一些列式数据库管理系统(例如：InfiniDB CE and MonetDB) 并没有使用数据压缩。但是, 若想达到比较优异的性能，数据压缩确实起到了至关重要的作用。
...
ClickHouse支持基于SQL的声明性查询语言，该语言大部分情况下是与SQL标准兼容的。
...
ClickHouse支持在表中定义主键。为了使查询能够快速在主键中进行范围查找，数据总是以增量的方式有序的存储在MergeTree中。因此，数据可以持续不断地高效写入到表中，并且写入的过程中不会存在任何加锁的行为。
...
ClickHouse在允许牺牲数据精度的情况下提供了对查询进行加速的各种各样的方法：
