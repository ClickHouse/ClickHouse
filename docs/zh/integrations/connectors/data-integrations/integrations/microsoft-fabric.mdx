...
使用 Copy &amp; Sync 流程将 OneLake 表导入 ClickHouse：

1. 打开您的 ClickHouse 项目，选择 **Copy &amp; Sync**
2. 从您的 工作区 中选择 Lakehouse
3. 选择要加速的表
4. 开始复制数据的时间点快照

<Frame>
  ![配置 Copy & Sync：使用排序键创建新的目标表](/images/integrations/tools/data-integration/microsoft-fabric/fabric-03-copy-sync-table-selection.webp)
</Frame>

您可以根据 OneLake 中发现的 schema，使用推断的数据类型和默认配置创建目标表。如果希望更好地控制表定义，可以预先创建表，并将现有表用作目标端。

<Frame>
  ![将 OneLake 源表映射到现有 ClickHouse 表的 Copy & Sync 列映射](/images/integrations/tools/data-integration/microsoft-fabric/fabric-04-copy-sync-existing-table.webp)
</Frame>

每个表都会显示同步状态。表同步完成后，即可立即在 SQL 控制台中查询。大型表会在后台同步，因此您可以在加载期间继续工作。

<Note>
  在公开预览版中，仅支持复制时间点快照。未来的发行版计划提供更多复制选项。
</Note>
...
在 Microsoft Fabric 公开预览期间，将数据从 OneLake 复制到 ClickHouse 不收费。Fabric 工作负载正式发布后将开始计费。工作负载达到 GA 后将公布定价详情。
...
选择 **打开 ClickHouse**，即可打开完整的 ClickHouse Cloud 控制台，并使用您的 Microsoft 身份登录。您可以从 Fabric 工作负载将数据复制到 ClickHouse 并运行任何 SQL 语句；但如需执行管理任务，则需要进入 Cloud Console 管理服务。
...
从 OneLake 执行的每次复制和同步都会作为一个 ClickPipe 运行。您可以在 ClickHouse Cloud 控制台的 [ClickPipes 页面](/zh/integrations/clickpipes/monitoring)上跟踪复制进度、吞吐量和错误。
...
2. 打开 **Copy &amp; Sync**，选择你的 Lakehouse，然后选择 `publicholidays` 表
...
* **仅支持湖仓表。** Copy &amp; Sync 会从同一工作区中的 Lakehouse 读取 Delta 表。其他 Fabric 项目 (Warehouse、KQL 数据库、镜像数据库) 尚不支持作为源。
...
* **角色变更会在下次打开时生效。** 打开 ClickHouse 项目时，您的 ClickHouse 组织角色会根据您在 Fabric 工作区中的角色进行映射。Fabric 无法将角色变更通知给 ClickHouse，因此工作区角色变更会在您下次打开该项目时生效，而不会立即生效。
