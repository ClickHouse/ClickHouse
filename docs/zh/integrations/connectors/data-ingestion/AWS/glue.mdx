...
<Tabs>
  ...
  <Tab title="手动安装">
    ...
    1. 将最新的 Spark Connector JAR (`clickhouse-spark-runtime-3.X_2.X-0.10.X.jar`) 上传到一个 S3 bucket。
...
<Tabs>
  <Tab title="可视化编辑器">
    你可以在 Glue Studio 可视化编辑器中将 ClickHouse 连接器用作数据源或目标端。只需将 ClickHouse Spark Connector 组件拖到画布上，然后连接到你的数据管道即可。
...
如需了解更多详细信息，请参阅我们的 [Spark 文档](/zh/integrations/apache-spark)。