...
Vector 使用 [sources](https://vector.dev/docs/about/concepts/#sources) 通过 push 或 pull 模型获取数据；同时，[sinks](https://vector.dev/docs/about/concepts/#sinks) 则作为事件的目标端。因此，我们这里使用 Kafka source 和 ClickHouse sink。请注意，虽然 Kafka 支持作为 Sink，但目前还没有可用的 ClickHouse source。因此，如果你想将数据从 ClickHouse 传输到 Kafka，就不适合使用 Vector。
