---
sidebarTitle: 'dbt Core v2、Fusion 与平台（Beta）'
slug: /integrations/dbt/dbt-core-v2-fusion-and-platform
description: 'ClickHouse adapter 在 dbt Core v2、dbt Fusion 及 dbt 平台中的支持状态'
keywords: ['dbt', 'dbt core v2', 'dbt v2', 'dbt fusion', 'dbt platform']
title: 'dbt Core v2、Fusion 与平台'
doc_type: 'reference'
---
...
dbt 正在基于全新引擎重建，而 ClickHouse 从一开始就参与其中。现在你可以在新的 Rust 引擎 (dbt Core v2 与 dbt Fusion) 上针对 ClickHouse 运行 dbt，并且首次可以通过 v2 连接器从 dbt 平台连接到 ClickHouse。你的项目无需任何改动：当前在 dbt Core 1.x 上运行的模型、测试和 profile，在新引擎上同样可以运行，唯一的区别在于 binary。

ClickHouse 是首个登陆 dbt v2 及 dbt 平台的社区 adapter。dbt 自家的第一方 adapter 与我们同步推出，而我们是来自 dbt Labs 之外的第一个。面向 dbt Core 1.x 的 Python adapter `dbt-clickhouse` 不会被弃用：我们会持续跟进 dbt 1.x 的 releases，并同时维护这两个 adapter。

<Warning>
  **尚未可用于 production。** dbt Core v2 引擎、其 ClickHouse adapter 以及 dbt 平台连接均尚未正式发布 (GA) 。请仅在开发或 staging 项目中使用。
</Warning>

<Note>
  **dbt Core 1.x 的文档同样适用于 dbt Core v2。** 该 adapter 在两种引擎上的行为完全一致：其他 dbt 页面中介绍的 profiles、模型配置、物化类型和 macros，在 dbt Core v2、dbt Fusion 和 dbt 平台上均可原样使用。例外情况见下方的 [v1 与 v2 功能对照](#parity) 表。
</Note>

最新状态可在 [ClickHouse/dbt-clickhouse#660](https://github.com/ClickHouse/dbt-clickhouse/issues/660) (dbt Core v2 功能对照，每个领域一个子 issue) 和 [ClickHouse/dbt-clickhouse#555](https://github.com/ClickHouse/dbt-clickhouse/issues/555) (dbt Fusion) 中查看。
...
| ---------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **dbt Core 1.x** | 基于 Python 的 dbt engine，open source (Apache 2.0) 。我们的 adapter 是 `dbt-clickhouse`，由 [ClickHouse GitHub 组织](https://github.com/ClickHouse/dbt-clickhouse) 维护，与 `dbt-core` 一并安装。                             | **GA，持续维护。** 与 v2 并行持续发布新版本。                                                                                                                                                                                                                                                    |
| **dbt Core v2**  | 用 Rust 对 dbt engine 进行的彻底 rewrite，open source (Apache 2.0) 。新版本包含 v1 的全部功能，并带来多项性能提升。                                                                                                                    | **Beta。** 请参阅 dbt 文档中的 [dbt v2 升级指南](https://docs.getdbt.com/docs/dbt-versions/core-upgrade/upgrading-to-v2) 和 [v2 的 ClickHouse 设置页面](https://docs.getdbt.com/docs/local/connect-data-platform/clickhouse-setup?version=2)。ClickHouse adapter 已内置于 dbt binary 中，无需单独安装 package。 |
| **dbt Fusion**   | dbt v2 加上 dbt Labs 提供的附加功能：SQL 理解、静态分析、LSP、VS Code extension 以及列感知。特定 dialect 的 SQL 智能能力按数据库分别构建。闭源，依据 [dbt Product Licensing Agreement](https://www.getdbt.com/dbt-fusion-engine-license-agreement) 授权。 | **Beta。** adapter 可与 Fusion binary 配合运行，但新的 SQL 功能尚不支持 ClickHouse。                                                                                                                                                                                                              |
| **dbt platform** | 托管版 dbt，原名 dbt Cloud：Studio IDE、定时 jobs、环境、Catalog、Semantic Layer。该平台仅运行 dbt v2，因此平台中的 ClickHouse 即指 v2 adapter。                                                                                         | **Private Beta。** 参见[如何申请访问权限](#request-access)。设置方式：参阅 dbt 文档中的 [Connect ClickHouse](https://docs.getdbt.com/docs/platform/connect-data-platform/connect-clickhouse)。                                                                                                          |

## Beta 状态 {#beta-status}

我们使用 `dbt-clickhouse` 的 dbt Core 1.x 集成测试套件来检验 dbt Core v2。Beta 意味着核心 dbt 工作流和 ClickHouse Cloud 功能集在已记录的限制范围内均可正常使用；与 v1 的完全对等以及自管理集群支持将在之后提供。
...
* **Fusion 中的 ClickHouse SQL 智能能力** (dialect 感知的校验、静态分析、LSP、VS Code 扩展中的列感知) 。目前该 adapter 可以配合 Fusion 二进制文件运行，但尚未包含这些功能。在此之前，ClickHouse 项目的静态分析会被强制设为 `off`，引擎也不会写入列级血缘信息。进展跟踪见 [#736](https://github.com/ClickHouse/dbt-clickhouse/issues/736)。
* **dbt 平台中除用户名和密码之外的其他认证方式。**

## v1 与 v2 的功能对等情况 {#parity}

下表将 `dbt-clickhouse` (v1) 中可用的功能与其在 dbt Core v2 adapter 中的状态进行对比。除备注另有说明外，所列的每项功能在 v1 中均受支持；状态列对应的是 dbt Core v2。每个待处理项的进展都在 [ClickHouse/dbt-clickhouse#660](https://github.com/ClickHouse/dbt-clickhouse/issues/660) 中跟踪。
