...
<Accordion title="HouseOps">
  ...
  * 带语法高亮的查询构建器。可在表格或 JSON 视图中查看结果。
  * 将查询结果导出为 CSV 或 JSON。
  * 显示说明信息的进程列表。支持写入模式及停止 (`KILL`) 进程。
  * 数据库关系图。显示所有表及其列以及其他相关信息。
  * 快速查看列大小。
  * 服务器配置。
...
<Accordion title="Redash">
  ...
  支持包括 ClickHouse 在内的多种数据源。Redash 可以将来自不同数据源的查询结果联接为一个最终数据集。
...
<Accordion title="Grafana">
  ...
  “无论您的指标存储在何处，Grafana 都可以让您查询、可视化、告警并深入了解这些指标。您可以与团队一起创建、探索和共享仪表板，培养数据驱动的文化。深受社区信赖和喜爱。” — grafana.com。
...
<Accordion title="ClickHouse 表关系图">
  [ClickHouse table graph](https://github.com/mbaksheev/clickhouse-table-graph) 是一款简单的命令行工具，用于可视化 ClickHouse 表之间的依赖关系。该工具从 `system.tables` 表中获取表之间的关联关系，并以 [mermaid](https://mermaid.js.org/syntax/flowchart.html) 格式生成依赖关系流程图。借助此工具，您可以轻松直观地查看表依赖关系，并了解 ClickHouse 数据库中的数据流。得益于 mermaid，生成的流程图美观且易于添加到 Markdown 文档中。
</Accordion>

<Accordion title="xeus-clickhouse">
  [xeus-clickhouse](https://github.com/wangfenjin/xeus-clickhouse) 是 ClickHouse 的 Jupyter 内核，支持在 Jupyter 中使用 SQL 查询 CH 数据。
</Accordion>

<Accordion title="MindsDB Studio">
  [MindsDB](https://mindsdb.com/) 是面向包括 ClickHouse 在内的数据库的开源 AI 层，可让您轻松开发、训练和部署先进的机器学习模型。MindsDB Studio (GUI) 可让您基于数据库训练新模型、解读模型预测、识别潜在数据偏差，并使用可解释 AI 函数评估和可视化模型准确性，从而更快地调整和优化机器学习模型。
</Accordion>

<Accordion title="DBM">
  [DBM](https://github.com/devlive-community/dbm) DBM 是 ClickHouse 的可视化管理工具！
...
<Accordion title="clickhouse-monitoring">
  [clickhouse-monitoring](https://github.com/duyet/clickhouse-monitoring) 是一个简单的 Next.js 仪表板，依赖 `system.*` 表帮助监控并概览 ClickHouse 集群。

  功能：

  * 查询监控：当前查询、查询历史、查询资源 (内存、读取的 parts、file&#95;open 等) 、资源消耗最高的查询、使用最多的表或列等。
  * 集群监控：总内存/CPU 使用情况、分布式队列、全局设置、MergeTree 设置、指标等。
  * 表和 parts 信息：提供列级详情，包括大小、行数、压缩、分片大小等。
  * 实用工具：ZooKeeper 数据探索、查询 EXPLAIN、终止查询等。
  * 指标可视化图表：查询和资源使用情况、合并/变更数量、合并性能、查询性能等。
</Accordion>

<Accordion title="CKibana">
  [CKibana](https://github.com/TongchengOpenSource/ckibana) 是一项轻量级服务，可让你通过原生 Kibana UI 轻松搜索、探索和可视化 ClickHouse 数据。

  功能：

  * 将原生 Kibana UI 中的图表请求转换为 ClickHouse 查询语法。
  * 支持采样和缓存等高级功能，以提升查询性能。
  * 降低用户从 ElasticSearch 迁移到 ClickHouse 后的学习成本。
</Accordion>

<Accordion title="Telescope">
  [Telescope](https://iamtelescope.net/) 是一个现代化 Web 界面，用于探索存储在 ClickHouse 中的日志。它提供易用的 UI，可通过细粒度访问控制查询、可视化和管理日志数据。

  功能：

  * 简洁的响应式 UI，提供强大的过滤器和可自定义的字段选择。
  * 使用直观且富有表达力的 FlyQL 语法筛选日志。
  * 支持按时间绘制图表和分组，包括嵌套 JSON、Map 和 Array 字段。
  * 支持可选的原始 SQL `WHERE` 查询，以进行高级筛选 (包含权限检查) 。
...
<Accordion title="clickhouse-flow">
  ...
  * 根据 ClickHouse metadata 自动构建 schema 图。
...
<Accordion title="Chadmin">
  [Chadmin](https://github.com/bun4uk/chadmin) 是一个简单的 UI，可用于查看当前在 ClickHouse 集群上运行的查询及其相关信息，并可在需要时终止这些查询。
</Accordion>
