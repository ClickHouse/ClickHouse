...
<Info>
  在开始添加连接信息之前，请确保您已在防火墙规则中将 ClickPipes IP addresses 加入白名单。您可以在[此处](/zh/integrations/clickpipes/networking/static-ips)查看 ClickPipes IP addresses 列表。
...
希望保持数据传输私有时非常有用。
您可以按照[设置指南来建立连接](/zh/integrations/clickpipes/networking/aws-privatelink)进行操作。
...
<Warning>
  如果您在 ClickHouse 中定义的排序键与 Postgres 中的 primary key 不同，请务必阅读相关的[注意事项](/zh/integrations/clickpipes/postgres/ordering-keys)。
</Warning>

8. 此外，您还可以提供自定义的 `PARTITION BY <expr>` 表达式，以控制目标端 [ClickHouse 表分区](/zh/concepts/core-concepts/partitions)。
