---
title: '重新同步特定表'
description: '重新同步 Postgres ClickPipe 中的特定表'
slug: /integrations/clickpipes/postgres/table_resync
sidebarTitle: '重新同步表'
doc_type: 'guide'
keywords: ['clickpipes', 'postgresql', 'cdc', '数据摄取', '实时同步']
integration:
  - support_level: 'core'
  - category: 'clickpipes'
---

在某些情况下，重新同步某个管道中的特定表会很有用。例如，Postgres 发生了较大的 schema 变更，或者 ClickHouse 中进行了某些数据重建模。

虽然通过点击按钮重新同步单个表的功能仍在开发中，但本指南将介绍目前如何在 Postgres ClickPipe 中实现这一操作。

<div id="removing-table">
  ### 1. 从管道中移除表
</div>

可参考[移除表指南](/zh/integrations/clickpipes/postgres/remove-table)进行操作。

<div id="truncate-drop-table">
  ### 2. 在 ClickHouse 中 TRUNCATE 或删除该表
</div>

此步骤是为了避免在下一步重新添加该表时出现数据重复。你可以前往 ClickHouse Cloud 的 **SQL 控制台** 选项卡，运行查询来完成此操作。
请注意，如果该表已存在于 ClickHouse 中且不为空，系统会进行校验并阻止再次添加该表。

或者，如果你需要保留旧表，也可以直接将其重命名。当表非常大、删除操作可能需要一些时间时，这种做法也很有帮助。

```sql
RENAME TABLE table_A TO table_A_bak;
```

<div id="add-table-again">
  ### 3. 再次将该表添加到 ClickPipe
</div>

可参考[添加表指南](/zh/integrations/clickpipes/postgres/add-table)进行操作。