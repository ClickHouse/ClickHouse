...
无需使用密码，您可以通过 AWS IAM 基于角色的访问控制对 ClickPipes 用户进行身份验证。按如下方式创建该用户，然后授予其与上文所示相同的 schema 和复制特权：
...
有关设置 ClickPipes 以通过这种方式进行身份验证所需的 IAM 角色和策略的完整步骤，请参阅 [AWS IAM DB authentication (RDS/Aurora)](/zh/integrations/clickpipes/postgres/auth)。
