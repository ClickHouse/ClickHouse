...
每个插入块覆盖连续的偏移量范围，并携带格式为 `topic:partition:firstOffset-lastOffset` 的确定性[去重标记](/zh/guides/developer/deduplicating-inserts-on-retries)。重放时，ClickPipes 会生成相同的偏移量范围，因而使用相同的标记，ClickHouse 将拒绝重复数据。由于标记仅取决于偏移量范围，即使重建的块并非逐字节完全一致，重放的数据仍会被去重。
...
主要的权衡在于 part 大小。更大的插入块会在 ClickHouse 中生成更少但更大的 [parts](/zh/parts)，从而降低合并开销。ClickPipes 在构建块时会将一个分区的行保存在内存中，因此可达到的 part 大小取决于管道可用的内存：内存紧张时，它会构建较小的块，表中会累积更多 parts。为管道提供更多内存可使其构建更大的块，从而生成更少的 parts。
