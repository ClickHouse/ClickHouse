...
<Steps>
  ...
  <Step title="以无序模式创建 ClickPipe" id="create-clickpipe">
    ...
    **2.** 打开 **持续摄取**，然后选择 **Any order** 作为摄取模式，并提供与你的存储桶关联的 **Pub/Sub subscription** 名称。该订阅必须由此 ClickPipe 独占使用——请参见[创建 Google Cloud Pub/Sub 主题](#create-pubsub-topic)。订阅名称必须符合以下格式：
