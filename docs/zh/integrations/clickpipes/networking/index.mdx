...
| [GCP Private Service Connect](/zh/integrations/clickpipes/networking/gcp-psc)  | 是    | 适用于托管在 GCP 上的 `AlloyDB` 数据源。尚不支持通过 Global Access 实现跨区域连接；如需访问位于不受支持 GCP 区域中的私有 `AlloyDB` 数据源，请使用 [SSH 隧道](#ssh-tunneling)。 |
