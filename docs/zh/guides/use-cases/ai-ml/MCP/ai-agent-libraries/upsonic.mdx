...
<Steps>
  ...
  <Step>
    ## 初始化 MCP Server 与 Upsonic agent
    ...
    - 2025 年按平均价格排名的前几大城镇（交易量 >= 50 的城镇）
    - 年度对比：2024 年 vs 2025 年（计数、均值、中位数）
    - 2025 年按房产类型细分（计数、平均值、中位数），来源：uk.uk_price_paid

    关键数据（来自数据集）
    - 2025 年整体（已记录交易）：交易量 = 376,633；均价 = £362,282.66；中位价 = £281,000；最低 = £100；最高 = £127,700,000。
    - 按月（2025 年）：（月份、交易量、均价、中位价）
      ...
      （数据集中仅包含第 1–8 个月。）
    - 按平均价格排名的前几大城镇（2025 年，交易量 ≥50 的城镇）
    ...
    - "其他"房产类型的平均值不稳定，原因是样本量少、异质性高且存在异常值。
    - 我未按 is_new、duration 或其他元数据进行筛选；这些筛选条件可能改变结果（例如排除新建房或租赁房）。
    - 这些表是价格支付式交易记录（已记录销售）—— 不直接代表要价或估值。
