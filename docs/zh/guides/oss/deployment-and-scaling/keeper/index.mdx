...
| `log_readahead_commit_window_bytes`       | 提交线程前预先缓冲的已解码日志条目的最大总大小。`0` 会禁用提交预读 (提交操作会逐个从磁盘读取条目) 。                                                                         | `500MiB`         |
...
* `ydld`：请求让出 leader 身份并转为 follower。如果接收该请求的服务器是 leader，它会先暂停写操作，等待继任者 (当前 leader 不会成为继任者) 追赶至最新日志，然后辞去 leader 身份。继任者将自动选出。请求已发送时返回 `Sent yield leadership request to leader.`，未发送时返回 `Failed to send yield leadership request to leader.`。如果该节点已是 follower，结果与发送该请求时相同。
