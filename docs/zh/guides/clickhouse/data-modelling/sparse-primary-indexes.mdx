...
slug: /zh/guides/clickhouse/data-modelling/sparse-primary-indexes
...
<div id="an-index-design-for-massive-data-scales">
  ### 适用于海量数据规模的索引设计 {#an-index-design-for-massive-data-scales}
</div>
...
### 数据按粒度组织，以便并行处理数据 {#data-is-organized-into-granules-for-parallel-data-processing}
...
### 主索引中每个粒度对应一个条目 {#the-primary-index-has-one-entry-per-granule}
...
### 主索引用于筛选粒度 {#the-primary-index-is-used-for-selecting-granules}
...
### 次级键列也可能 (不) 高效 {#secondary-key-columns-can-not-be-inefficient}
...
### 选项 2：Materialized Views {#option-2-materialized-views}
...
<div id="ordering-key-columns-efficiently">
  ## 高效安排排序键列 {#ordering-key-columns-efficiently}
</div>
...
### 数据文件的最佳压缩率 {#optimal-compression-ratio-of-data-files}
