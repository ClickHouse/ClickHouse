...
在 ClickHouse `24.3` 版本中，新的查询分析器默认处于启用状态。
...
在 analyzer 的新版本中，用于确定 `USING` clause 中指定列共同超类型的规则已统一，从而带来更可预测的结果，
...
我们正在所有当前禁用了新查询分析器的实例上启用该功能，以支持新的功能和性能优化。此变更会实施更严格的 SQL 作用域规则，因此客户需要手动更新不符合要求的查询。
