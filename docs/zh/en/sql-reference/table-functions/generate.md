---
description: '使用给定的 schema 生成随机数据。可用这些数据填充测试表。
  并非所有类型都受支持。'
...
| `name`              | 对应列的名称。                               |
| `TypeName`          | 对应列的类型。                               |
| `random_seed`       | 手动指定随机种子，以生成稳定结果。如果为 `NULL`，则会随机生成种子。 |
| `max_string_length` | 所有生成的字符串的最大长度。默认为 `10`。               |
| `max_array_length`  | 所有生成的数组或 Map 的最大元素个数。默认为 `10`。        |
...
配合 [generateRandomStructure](../../sql-reference/functions/other-functions.md#generateRandomStructure) 使用：
...
:::note
如果 `generateRandom(generateRandomStructure(), [random seed], max_string_length, max_array_length)` 的 `max_array_length` 设置得足够大，由于复杂类型 (`Array`、`Tuple`、`Map`、`Nested`) 可能具有很深的嵌套层级 (最多 16 层) ，生成的输出可能会非常庞大。
:::
