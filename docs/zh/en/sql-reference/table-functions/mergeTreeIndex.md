---
description: '表示 MergeTree 表的索引文件和标记文件的内容。
  可用于查看内部信息。'
...
表示 MergeTree 表的索引文件和标记文件的内容。可用于查看内部信息。
...
一个表对象，包含以下列：源表的主索引和 min-max 索引的值列 (如果已启用) 、源表各数据分区片段中所有可能文件对应的标记值列 (如果已启用) ，以及虚拟列：

* `part_name` - 数据分区片段的名称。
* `mark_number` - 数据分区片段中当前标记的编号。
* `rows_in_granule` - 当前粒度中的行数。

如果某列在数据分区片段中不存在，或者其某个 `substream` 的标记未写入 (例如在 compact parts 中) ，则标记列可能包含 `(NULL, NULL)` 值。
