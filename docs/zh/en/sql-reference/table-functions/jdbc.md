...
:::note
clickhouse-jdbc-bridge 包含 Experimental 代码，且已不再受支持。它可能存在可靠性问题和安全漏洞。使用风险自负。
ClickHouse 建议使用 ClickHouse 内置的表函数，它们在临时查询场景中是更好的替代方案 (如 Postgres、MySQL、MongoDB 等) 。
:::

JDBC 表函数返回一个通过 JDBC 驱动连接的表。

此表函数需要单独运行 [clickhouse-jdbc-bridge](https://github.com/ClickHouse/clickhouse-jdbc-bridge) 程序。
它支持 Nullable 类型 (基于被查询远程表的 DDL) 。
