...
返回其有序窗口帧中第 n 行 (偏移量) 对应求值结果里的第一个非 NULL 值。
...
有关窗口函数语法的更多信息，请参阅：[Window Functions - 语法](./index.md/#syntax)。

**参数**

* `x` — 列名。
* `offset` — 用于与当前行进行比较求值的第 n 行。

**返回值**

* 在其有序窗口帧中，相对于第 n 行 (offset) 求值得到的第一个非 NULL 值。

**示例**

在此示例中，`nth-value` 函数用于从一个虚构的英超足球球员薪资数据集中找出第三高的薪资。
