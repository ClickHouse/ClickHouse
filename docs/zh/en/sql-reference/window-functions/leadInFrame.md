...
返回在有序窗口帧内、位于当前行之后 offset 行的值。

:::warning
`leadInFrame` 的行为与标准 SQL 窗口函数 `lead` 不同。
ClickHouse 窗口函数 `leadInFrame` 会遵循窗口帧。
如需获得与 `lead` 完全相同的行为，请使用 `ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING`。
:::
...
* `default` — 当计算出的行超出窗口帧边界时返回的值。 (可选，省略时使用列类型的默认值。) 

**返回值**

* 在有序窗口帧中，当前行之后偏移指定行数的那一行对应的值。
