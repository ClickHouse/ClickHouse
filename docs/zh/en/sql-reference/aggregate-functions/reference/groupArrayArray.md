...
将多个数组聚合成一个更大的数组。
将 [`groupArray`](/zh/sql-reference/aggregate-functions/reference/grouparray) 函数与 [Array](/zh/sql-reference/aggregate-functions/combinators#-array) 组合器结合起来。

别名：`array_concat_agg`

**示例**

假设我们有一份记录用户浏览会话的数据。每个会话都会记录某个用户访问页面的顺序。
