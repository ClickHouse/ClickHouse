...
[ROLLUP](../statements/select/group-by.md/#rollup-modifier) 和 [CUBE](../statements/select/group-by.md/#cube-modifier) 是 GROUP BY 的修饰符。两者都会计算小计。ROLLUP 接受一个有序列列表，例如 `(day, month, year)`，并在聚合的每一层级计算小计，最后再计算总计。CUBE 则会针对指定列的所有可能组合计算小计。GROUPING 用于标识由 ROLLUP 或 CUBE 返回的行中，哪些是超级聚合行，哪些是普通 GROUP BY 会返回的行。
...
默认情况下，CUBE 修饰符会为传递给 CUBE 的列的所有可能组合计算小计。GROUPING SETS 允许你指定要计算的特定组合。

分析层次化数据是 ROLLUP、CUBE 和 GROUPING SETS 修饰符的一个很好的用例。这里的示例是一张表，包含两个数据中心中安装了哪些 Linux 发行版及其版本的数据。按发行版、版本和位置查看这些数据可能很有价值。
...
在不使用 CUBE、ROLLUP 或 GROUPING SETS 时对数据进行细分：
...
下一个查询中的 CUBE，即 `CUBE(datacenter,distro,version)`，提供了一个可能并不合理的层级。跨这两个发行版来看 version 并没有意义 (因为 Arch 和 RHEL 的发布周期和版本命名标准并不相同) 。紧随其后的 GROUPING SETS 示例更合适，因为它将 `distro` 和 `version` 归为同一组。
...
:::note
如果上例中的 version 不与某个发行版关联，可能就没有实际意义；如果我们跟踪的是内核版本，那就可能有意义，因为内核版本可以与任一发行版关联。像下一个示例那样使用 GROUPING SETS，可能是更好的选择。
:::
