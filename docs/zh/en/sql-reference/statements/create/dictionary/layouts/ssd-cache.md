...
sidebar_position: 8
description: '通过内存索引将字典数据存储在 SSD 上：ssd_cache 或 complex_key_ssd_cache 类型'
doc_type: 'reference'
---
...
与 `cache` 类似，但数据存储在 SSD 上，索引存储在 RAM 中。所有与更新队列相关的缓存字典设置也都适用于 SSD 缓存字典。
  ...
  <TabItem value="xml" label="配置文件">
    ...
    <layout>
        <ssd_cache>
            ...
            <!-- 写入 SSD 前用于聚合元素的 RAM 缓冲区大小（以字节为单位）。 -->
...
这种存储类型用于复合[键](../attributes.md#composite-key)，与 `ssd_cache` 类似。