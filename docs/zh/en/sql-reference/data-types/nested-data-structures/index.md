---
description: 'ClickHouse 中嵌套数据结构概述'
...
嵌套数据结构就像单元格中的一张表。嵌套数据结构的参数——列名和类型——其指定方式与 [CREATE TABLE](../../../sql-reference/statements/create/table.md) 查询中相同。表中的每一行都可以对应嵌套数据结构中的任意多行。
...
上述 `CREATE TABLE` DDL 语句声明了 `Goals` 这一嵌套数据结构，其中包含有关转化或已达成目标的数据。
`visits` 表中的每一行都对应零次或多次转化。

当 [`flatten_nested`](/zh/operations/settings/settings#flatten_nested) 设置为 `0` 时 (默认值为 `flatten_nested=1`) ，支持任意级别的嵌套。

在大多数情况下，处理嵌套数据结构时，其列通过以点分隔的列名来指定。
这些列共同构成一组类型对应的数组。
...
:::tip
可以把嵌套数据结构理解为一组长度相同的列数组，这样最容易理解。
:::

<div id="filtering-nested-columns-in-where">
  ### 在 WHERE 中筛选 Nested 列
</div>

由于 `Nested` 结构中的每一列都以 `Array` 形式存储，因此在 `WHERE` 子句中引用它时，拿到的是每一行的整个数组，而不是某个单独的元素。你不能将嵌套列直接与标量值比较，因此必须改用[数组函数](/zh/sql-reference/functions/array-functions)。

例如，下面这个查询**不会**悄无声息地返回 0 行——它会引发异常，因为 `Goals.ID` 的类型是 `Array(UInt32)`，而 `equals(Array(UInt32), UInt32)` 不是有效的比较：
...
使用 [`has`](/zh/sql-reference/functions/array-functions#has) 检查数组中是否包含特定值：
...
当条件较为复杂时，请使用 [`arrayExists`](/zh/sql-reference/functions/array-functions#arrayExists)：
...
你可以使用 `length` 按数组长度筛选，或使用 `notEmpty` 排除空数组：
...
要针对嵌套结构中的单个元素进行过滤，而不是针对整行，请先使用 `ARRAY JOIN` 展开数组。
在 `ARRAY JOIN` 之后，每个元素都会变成单独的一行，因此 `WHERE` 子句会应用于标量值。
更多信息，请参见 [`ARRAY JOIN` 子句](/zh/sql-reference/statements/select/array-join)。示例：
...
对于 `INSERT` 查询，你需要分别传入嵌套数据结构中所有组成列的数组 (就像它们是彼此独立的列数组一样) 。插入时，系统会检查这些数组的长度是否一致。

每个嵌套子列都会通过点表示法 (`Goals.ID`、`Goals.Serial`、...) 列在列列表中，对应的值则为数组：
...
同一行中的所有嵌套子列数组长度必须相同。长度不一致会导致错误：
...
对于 `DESCRIBE` 查询，嵌套数据结构中的各列也会以同样的方式分别列出。

<div id="alter-limitations">
  ### ALTER 限制
</div>

对嵌套数据结构执行 `ALTER` 查询时，有以下限制：
