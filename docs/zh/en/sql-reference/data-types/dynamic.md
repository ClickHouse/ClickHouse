---
description: 'ClickHouse 中 Dynamic 数据类型的文档；该类型可在单个列中存储不同类型的值'
...
其中，`N` 是一个介于 `0` 到 `254` 之间的可选参数，用于指定在单个独立存储的数据块中 (例如 MergeTree 表中的单个数据分区片段) ，`Dynamic` 类型的列内最多可将多少种不同的数据类型存储为独立子列。如果超过此限制，所有新类型的值都会以二进制形式统一存储在一种特殊的共享数据结构中。`max_types` 的默认值为 `32`。
...
`Dynamic` 类型支持使用类型名称作为子列，从 `Dynamic` 列中读取某一种嵌套类型。
因此，如果有列 `d Dynamic`，就可以使用语法 `d.T` 读取任意有效类型 `T` 对应的子列。
如果 `T` 可以包含在 `Nullable` 中，则该子列的类型为 `Nullable(T)`；否则类型为 `T`。该子列的
大小与原始 `Dynamic` 列相同，并且在原始 `Dynamic` 列不是类型 `T` 的所有行中，
其值将为 `NULL` (如果 `T` 不能包含在 `Nullable` 中，则为空值) 。
...
要查看每一行中存储的是哪种变体，可以使用函数 `dynamicType(dynamic_column)`。它会为每一行返回一个 `String`，其值为该行的值类型名称 (如果该行为 `NULL`，则返回 `'None'`) 。
...
要从 `String` 列中解析出 `Dynamic` 类型的值，可启用设置 `cast_string_to_dynamic_use_inference`：
...
如果 `K < N`，则类型最少见的值会被插入到一个特殊的子列中，但仍然可以访问：
...
所有文本格式 (TSV、CSV、CustomSeparated、Values、JSONEachRow 等) 都支持读取 `Dynamic` 类型。在解析数据时，ClickHouse 会尝试推断每个值的类型，并在插入到 `Dynamic` 列时使用该类型。
...
大多数函数支持类型为 `Dynamic` 的参数。在这种情况下，函数会针对 `Dynamic` 列中存储的每种内部数据类型分别执行。
当函数的结果类型取决于参数类型时，以 `Dynamic` 参数执行该函数所得的结果类型将为 `Dynamic`。当函数的结果类型与参数类型无关时，结果类型将为 `Nullable(T)`，其中 `T` 为该函数通常的结果类型。
...
如果函数无法对 `Dynamic` 列中的某些类型执行，则会抛出异常：
...
设置 `dynamic_throw_on_type_mismatch` 用于控制当函数应用于 `Dynamic` 列，且某一行实际存储的类型与该函数不兼容时的处理方式：
...
在 `ORDER BY` 和 `GROUP BY` 中，`Dynamic` 类型的值会采用与 `Variant` 类型的值类似的方式进行比较：
对于 `Dynamic` 类型中底层类型为 `T1` 的值 `d1` 和底层类型为 `T2` 的值 `d2`，运算符 `<` 的结果定义如下：

* 如果 `T1 = T2 = T`，结果为 `d1.T < d2.T` (比较底层值) 。
* 如果 `T1 != T2`，结果为 `T1 < T2` (比较类型名称) 。

默认情况下，`Dynamic` 类型不能用于 `GROUP BY`/`ORDER BY` 键；如果你想使用它，请注意其特殊的比较规则，并启用 `allow_suspicious_types_in_group_by`/`allow_suspicious_types_in_order_by` 设置。
...
**注意：** 对于 Dynamic 类型的值，如果其底层数值类型不同，则会被视为不同的值，彼此之间不进行比较，而是比较其类型名称。
...
**注意：** 由于函数对 `Dynamic` 类型有[特殊行为](#using-dynamic-type-in-functions)，上述比较规则在执行诸如 `<`/`>`/`=` 等比较函数时不会应用

<div id="reaching-the-limit-in-number-of-different-data-types-stored-inside-dynamic">
  ## Dynamic 中存储的不同数据类型数量达到上限
</div>

`Dynamic` 数据类型只能将有限数量的不同数据类型存储为单独的子列。默认情况下，该上限为 32，但你可以在类型声明中使用 `Dynamic(max_types=N)` 语法进行修改，其中 N 的取值范围为 0 到 254 (由于实现细节限制，不可能在 Dynamic 中将超过 254 种不同数据类型存储为单独的子列) 。
当达到该上限时，后续插入到 `Dynamic` 列中的所有新数据类型，都会被插入到一个统一的共享数据结构中，该结构以二进制形式存储不同数据类型的值。

下面来看看在不同场景下达到该上限时会发生什么。

<div id="reaching-the-limit-during-data-parsing">
  ### 在数据解析过程中达到限制
</div>

在从数据中解析 `Dynamic` 值时，一旦当前数据块达到限制，所有新值都会被插入到共享数据结构中：
...
可以看到，在插入 3 种不同的数据类型 `Int64`、`Array(Int64)` 和 `String` 后，所有新类型都会被插入到特殊的共享数据结构中。
...
在这种情况下，ClickHouse 会决定哪些类型在合并后继续保留为独立子列，哪些类型会被写入共享数据结构。在大多数情况下，ClickHouse 会尽量保留出现最频繁的类型，并将较少见的类型存储到共享数据结构中，但具体行为仍取决于实现方式。

下面来看一个这类合并的示例。首先，创建一个包含 `Dynamic` 列的表，将不同数据类型的数量上限设置为 `3`，然后插入包含 `5` 种不同类型的值：
...
每次插入都会创建一个单独的数据 part，其中 `Dynamic` 列只包含一种类型：
...
正如我们所见，ClickHouse 将最常见的类型 `UInt64` 和 `Array(UInt64)` 保留为子列，并将其他所有类型都插入到共享数据中。
...
在 RowBinary 格式中，`Dynamic` 类型的值会按以下格式序列化：
