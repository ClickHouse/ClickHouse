...
用于表示时间和日期时间间隔的数据类型家族，也是 [INTERVAL](/zh/sql-reference/operators#interval) 运算符的结果类型。
...
每种时间间隔类型都对应一个单独的数据类型。例如，`DAY` 时间间隔对应 `IntervalDay` 数据类型：
...
你可以在算术运算中将 `Interval` 类型的值与 [Date](../../../sql-reference/data-types/date.md) 和 [日期时间](../../../sql-reference/data-types/datetime.md) 类型的值结合使用。例如，你可以给当前时间加上 4 天：
...
非前导字段会按 SQL 标准进行校验：`MONTH` 为 0-11，`HOUR` 为 0-23，`MINUTE` 为 0-59，`SECOND` 为 0-59。
...
* [INTERVAL](/zh/sql-reference/operators#interval) 运算符
