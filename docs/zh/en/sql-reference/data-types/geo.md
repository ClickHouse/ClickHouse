...
ClickHouse 支持用于表示地理对象的数据类型——例如位置、区域等。
...
`Point` 由其 X 和 Y 坐标表示，并存储为 [Tuple](tuple.md)([Float64](float.md), [Float64](float.md))。
...
`Ring` 是一种不含孔的简单多边形，存储为点的数组：[Array](array.md)([Point](#point))。
...
`LineString` 是一种以点数组形式存储的线：[Array](array.md)([Point](#point))。
...
`MultiLineString` 是由多个 `LineString` 组成并以数组形式存储的数据类型：[Array](array.md)([LineString](#linestring))。
...
`Polygon` 是一种带孔的多边形，以环数组的形式存储：[Array](array.md)([Ring](#ring))。外层数组的第一个元素表示多边形的外边界，后续所有元素都是孔。
...
`MultiPolygon` 由多个多边形构成，并以多边形数组的形式存储：[Array](array.md)([Polygon](#polygon))。
...
`Geometry` 是上述所有类型的通用类型，相当于这些类型组成的一个 Variant。
