---
description: 'Hive 引擎可用于对 HDFS 中的 Hive 表执行 `SELECT` 查询。'
sidebar_label: 'Hive'
sidebar_position: 84
...
Hive 引擎允许您对 HDFS 上的 Hive 表执行 `SELECT` 查询。目前支持以下输入格式：

* Text：仅支持简单标量列类型，但不支持 `binary`

* ORC：支持除 `char` 之外的简单标量列类型；仅支持 `array` 等复杂类型
...
* 列名应与原始 Hive 表中的列名一致，但你也可以只使用其中部分列，顺序不限，还可以使用基于其他列计算出的别名列。
* 列类型应与原始 Hive 表中的列类型一致。
* 分区表达式应与原始 Hive 表保持一致，且分区表达式中使用的列应包含在表结构中。
...
我们强烈建议为远程文件系统启用本地缓存。性能测试表明，启用缓存后，速度几乎可提升 2 倍。
...
* root&#95;dir：必填。用于存放远程文件系统本地缓存文件的根目录。
* limit&#95;size：必填。本地缓存文件的最大大小 (以字节为单位) 。
* bytes&#95;read&#95;before&#95;flush：控制从远程文件系统下载文件时，在刷写到本地文件系统之前读取的字节数。默认值为 1MB。
...
在 ClickHouse 中创建一个表，用于从上面创建的 Hive 表中获取数据：
...
在 ClickHouse 中创建表，从上面创建的 Hive 表中检索数据：
