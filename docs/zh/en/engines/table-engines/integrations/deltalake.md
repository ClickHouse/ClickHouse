---
description: '该引擎为存储在亚马逊 S3 中的现有 Delta Lake
  表提供只读集成。'
...
该引擎可与存储在 S3、GCP 和 Azure 中的现有 [Delta Lake](https://github.com/delta-io/delta) 表集成，并支持读取和写入 (自 v25.10 起) 。

<div id="create-table">
  ## 创建 DeltaLake 表
</div>

要创建 DeltaLake 表，该表必须已经存在于 S3、GCP 或 Azure 存储中。以下命令不接受用于创建新表的 DDL 参数。
...
<Tabs>
  <TabItem value="S3" label="S3" default>
    ...
    * `url` — 指向现有 Delta Lake 表的存储桶 URL (包含路径) 。
    * `aws_access_key_id`, `aws_secret_access_key` - [AWS](https://aws.amazon.com/) 账户用户的长期凭证。您可以使用它们对请求进行身份验证。该参数是可选的。如果未指定凭证，则会从配置文件中读取。
    * `extra_credentials` - 可选。用于传递 `role_arn`，以便在 ClickHouse Cloud 中实现基于角色的访问。有关配置步骤，请参见 [Secure S3](/zh/cloud/data-sources/secure-s3)。

    也可以使用[命名集合](/zh/operations/named-collections.md)来指定引擎参数。
  ...
  <TabItem value="GCP" label="GCP" default>
    **语法**
    ...
    不支持 `gs://clickhouse-docs-example-bucket` 这样的 gsutil URI，请使用以 `https://storage.googleapis.com` 开头的 URL
    :::

    **参数**

    * `url` — 指向 Delta Lake 表的 GCS 存储桶 URL。必须使用 `https://storage.googleapis.com/<bucket>/<path>/`
      格式 (GCS XML API 端点) ，或者使用会自动转换的 `gs://<bucket>/<path>/`。
...
使用 DeltaLake 表引擎创建表后，您可以通过以下方式向其中插入数据：
...
:::note
仅支持通过 delta kernel 使用该表引擎写入数据。
目前尚不支持写入 Azure，但写入 S3 和 GCS 可以正常工作。

Delta Lake 写入是一项 Beta 功能，必须通过 `SET allow_delta_lake_writes = 1` 启用 (自 26.7 version 起可用；更早版本请使用 `SET allow_experimental_delta_lake_writes = 1`) 。
:::
...
`DeltaLake` 表引擎和表函数支持数据缓存，和 `S3`、`AzureBlobStorage`、`HDFS` 存储相同。更多信息，请参见[&quot;S3 表引擎&quot;](../../../engines/table-engines/integrations/s3.md#data-cache)。
