---
description: '该引擎为现有的 Apache Paimon 表提供只读集成功能，这些表存储在亚马逊 S3、Azure、HDFS
  以及本地存储中。'
...
该引擎为现有的 Apache [Paimon](https://paimon.apache.org/) 表提供只读集成功能，这些表存储在亚马逊 S3、Azure、HDFS 以及本地存储中。
它支持快照读取、增量读取，以及引擎提供的基本分区裁剪功能。

<div id="create-table">
  ## 创建表
</div>

请注意，Paimon 表必须已存在于存储中；此命令不支持传入用于创建新表的 DDL 参数。
...
这些参数的说明分别与 `S3`、`AzureBlobStorage`、`HDFS` 和 `File 表引擎`中的参数说明一致。
...
* 从最新的表快照读取快照。
* 启用后，基于已提交的快照 ID 进行增量读取。
* 启用 `use_paimon_partition_pruning` 时进行分区裁剪。
* 配置后可选择在后台刷新元数据。
* 使用 Atomic/Replicated 数据库时，表 UUID 保持稳定，从而可在 Keeper 路径中使用 `{uuid}` 宏。

<div id="settings">
  ## 设置
</div>

该引擎使用与对应对象存储引擎相同的设置，并新增了 Paimon 特有的设置：
...
* `paimon_metadata_refresh_interval_sec` — 后台元数据刷新间隔，单位为秒。当设置为大于 0 的值时，后台任务会定期从对象存储中拉取最新的 快照 和 schema。默认值：30。
* `paimon_keeper_path` — 用于增量读取状态的 Keeper 路径。必须设置，且对每个表都唯一；支持 `{database}`、`{table}`、`{uuid}` 等宏。
* `paimon_replica_name` — 用于增量读取状态的副本名称。必须设置，且对每个副本都唯一；支持 `{replica}` 等宏。
...
以下设置均为**查询级**设置 (通过 `SELECT ... SETTINGS` 传递，而不是在 `CREATE TABLE` 中设置) 。它们用于控制增量读取在单次查询中的行为：

* `paimon_target_snapshot_id` — 仅读取指定 快照 的 delta。Keeper 中已 committed 的 watermark **不会** 向前推进，因此同一个 快照 可以重复读取任意次数。默认值：`-1` (禁用) 。
* `max_consume_snapshots` — 单次增量读取可消费的 快照s 最大数量。当 source 中累积了大量尚未读取的 快照s 时，该设置可限制每次查询消费的数量，以控制批次大小。`0` 表示不限制。默认值：`0`。

**定向 快照 读取** —— 无论当前 watermark 为何，始终返回 快照 1 的 delta：
...
**限制每个批次处理的快照数** — 如果有三个新快照待处理，则每次查询最多只消费两个：
...
你可以使用处于 `APPEND` 模式的可刷新materialized view，构建一条端到端管道，将数据从 Paimon 表持续同步到 MergeTree 表。每个刷新周期都只会从 Paimon 读取新的增量数据，并将其追加到目标表中。

**步骤 1 — 创建启用了增量读取和元数据刷新的 Paimon 源表。**

下面的示例使用 `PaimonLocal`。请根据你的存储后端，将引擎替换为 `PaimonS3`、`PaimonAzure`、`PaimonHDFS`，或可自动检测的 `Paimon` 引擎：
...
`paimon_metadata_refresh_interval_sec` 用于设置后台 metadata 刷新间隔，单位为秒。当该值大于 0 时，后台任务会定期从对象存储中拉取最新的 快照 和 schema，使 MV 刷新周期无需等待查询触发 metadata 更新，也能看到新提交的数据。默认值为 30。在大量表上使用时请谨慎，以避免对对象存储和 Keeper I/O 造成过高压力。

**步骤 2 — 创建 MergeTree 目标表 (schema 从 Paimon 表克隆) ：**
...
MV 每 10 秒执行一次 `SELECT * FROM paimon_mv_source`，该语句只会返回自上一个已提交快照以来新增的行，并将其追加到 `paimon_mv_dest`。
...
:::note
删除 MV 之前，请先将其停止，以避免后台刷新阻塞 DDL 操作。
:::
...
* 增量读取要求设置 `paimon_keeper_path`，且每个表都必须唯一。
* 在同一个 Keeper 路径 下，每个副本的 `paimon_replica_name` 都必须唯一。
* 增量读取采用最多一次投递：在收集数据文件时，已提交快照就会向前推进，此时数据实际上尚未被消费。如果查询在文件收集后失败，重试时不会重新读取这些被跳过的快照。
* 该表引擎为只读；不支持修改数据。
* 增量读取不会处理来自 Paimon 源端的历史数据删除。如果上游 Paimon 数据被删除或更新，已写入 ClickHouse MergeTree 目标表的对应行不会被自动移除。你必须在 MergeTree 表上手动执行 `ALTER TABLE ... DELETE`，以清理陈旧数据。

<div id="aliases">
  ## 别名
</div>

`Paimon` 表引擎会根据 `disk` 设置自动检测存储后端，并相应转发到 `PaimonS3`、`PaimonAzure` 或 `PaimonLocal`。如果未指定 `disk`，则默认使用 `PaimonS3` 实现。
