...
:::note
ClickHouse 还支持通过 [SQL 驱动的工作流](/zh/operations/access-rights#access-control-usage) 管理用户，推荐使用该方式。
:::
...
* 如需以明文形式设置密码 (**不推荐**) ，请将其放在 `password` 元素中。

  例如：`<password>qwerty</password>`。密码可以留空。

<a id="password_sha256_hex" />

* 如需使用密码的 SHA256 哈希值设置密码，请将其放在 `password_sha256_hex` 元素中。
...
* 为兼容 MySQL 客户端，也可以使用 double SHA1 哈希来指定密码。请将其放在 `password_double_sha1_hex` 元素中。
...
基于时间的一次性密码 (TOTP) 可用于对 ClickHouse 用户进行身份验证，其方式是生成在限定时间内有效的临时访问码。
这种 TOTP 身份验证方法符合 [RFC 6238](https://datatracker.ietf.org/doc/html/rfc6238) 标准，因此可兼容 Google Authenticator、1Password 等常见 TOTP 应用。
除了基于密码的身份验证外，还可以通过 `users.xml` 配置文件进行设置。
目前 SQL 驱动的访问控制尚不支持此功能。

使用 TOTP 进行身份验证时，用户必须提供主密码，以及由其 TOTP 应用生成的一次性密码。一次性密码既可以通过 `--one-time-password` 命令行选项提供，也可以用 &#39;+&#39; 字符拼接在主密码后面。
例如，如果主密码为 `some_password`，生成的 TOTP 代码为 `345123`，则用户在连接到 ClickHouse 时可以指定 `--password some_password+345123`，或指定 `--password some_password --one-time-password 345123`。如果未指定密码，`clickhouse-client` 会以交互方式提示输入。

要为用户启用 TOTP 身份验证，请在 `users.xml` 中配置 `time_based_one_time_password` 部分。该部分定义了 TOTP 设置，例如密钥、有效期、位数和哈希算法。
...
要为特定用户启用 TOTP，请在现有任一基于 `password` 的字段 (如 `password` 或 `password_sha256_hex`) 中，额外添加一个 `time_based_one_time_password` 部分。
...
为用户配置 TOTP 后，如上所述，可以在身份验证过程中使用一次性密码。

### 用户名/SSH 密钥

此设置支持使用 SSH 密钥进行身份验证。

给定一个 SSH 密钥 (例如由 `ssh-keygen` 生成) ，如下所示
...
如需使用其他受支持的算法，请将 `ssh-ed25519` 替换为 `ssh-rsa` 或 `ecdsa-sha2-nistp256`。

### 多种身份验证方法

可以使用 `<auth_methods>` 元素为同一个用户配置多种身份验证方法。这样，用户可以通过所列方法中的任意一种完成身份验证——例如，一个用户既可以设置密码，也可以配置 LDAP 凭据，使用其中任意一种登录都可以成功。

`<auth_methods>` 的每个子元素都是一个可任意命名的包装元素，其中必须且只能包含一种身份验证类型。包装元素的名称 (例如 `<method1>`、`<primary>`、`<a1>`) 并不重要；实际生效的只有其内部的身份验证元素。
...
* `<auth_methods>` **不能**与用户级别指定的身份验证方法同时使用。两者只能选择一种，不能混用。
* `<auth_methods>` 必须至少包含一种身份验证方法。
* `<auth_methods>` 中的每个包装元素必须且只能包含一种身份验证类型 (`<ssh_keys>` 例外，出于向后兼容考虑，它可以包含多个) 。
...
此设置用于为用户启用或禁用 SQL 驱动的[访问控制与账户管理](/zh/operations/access-rights#access-control-usage)。
...
此设置允许向所选用户授予任意权限。
列表中的每一项都应为不指定任何被授权主体的 `GRANT` 查询。
...
和 `allow_databases` 设置同时指定。

### user_name/networks

允许用户连接到 ClickHouse server 的网络列表。
...
* `<host>` — 主机名。
  ...
  检查访问权限时，会执行 DNS 查询，并将返回的所有 IP 地址与对端地址进行比较。

* `<host_regexp>` — 主机名的正则表达式。

  示例：`^example\d\d-\d\d-\d\.host\.ru$`

  检查访问权限时，会针对对端地址执行一次 [DNS PTR 查询](https://en.wikipedia.org/wiki/Reverse_DNS_lookup)，然后应用指定的正则表达式。接着，会对 PTR 查询结果再次执行 DNS 查询，并将返回的所有地址与对端地址进行比较。我们强烈建议正则表达式以 $ 结尾。

所有 DNS 请求结果都会被缓存，直到服务器重启。
...
:::note
除非你已正确配置防火墙，或者服务器未直接连接到互联网，否则向任何网络开放访问都是不安全的。
:::
...
你可以为该用户指定一个设置 profile。设置 profile 在 `users.xml` 文件中的独立章节里配置。更多信息，请参阅[设置 profile](../../operations/settings/settings-profiles.md)。

### user_name/quota

配额可用于在一段时间内跟踪或限制资源使用情况。配额在 `users.xml` 配置文件的 `quotas`
部分中配置。

你可以为用户分配一组配额。有关配额配置的详细说明，请参见 [Quotas](/zh/operations/quotas)。

### user_name/databases

在本节中，您可以限制 ClickHouse 针对当前用户发起的 `SELECT` 查询所返回的行数，从而实现基础的行级安全控制。
...
`filter` 可以是任何返回 [UInt8](../../sql-reference/data-types/int-uint.md) 类型值的表达式。它通常包含比较运算符和逻辑运算符。对于该用户，`database_name.table1` 中 `filter` 结果为 0 的行不会返回。此类筛选与 `PREWHERE` 操作不兼容，并会禁用 `WHERE→PREWHERE` 优化。

## 角色

您可以通过 `user.xml` 配置文件中的 `roles` 部分创建任何预定义角色。
