...
命名集合提供了一种存储键值对集合的方式，
可用于配置与外部源的集成。你可以将命名集合用于
字典、表、表函数和对象存储。

命名集合可以通过 DDL 或在配置文件中进行配置，并在
ClickHouse 启动时生效。它们可以简化对象的创建，并帮助向没有管理权限的用户隐藏凭据。

命名集合中的键必须与相应
函数、表引擎、数据库等的参数名称匹配。下面的示例中，
每种类型都链接到了对应的参数列表。

在命名集合中设置的参数可以在 SQL 中被覆盖，这一点会在下面的示例
中展示。可以使用 `[NOT] OVERRIDABLE` 关键字、XML 属性
和/或配置选项 `allow_named_collection_override_by_default` 来限制这种能力。

:::warning
如果允许覆盖，没有管理权限的用户可能就能够
推断出你试图隐藏的凭据。
如果你出于这一目的使用命名集合，则应禁用
`allow_named_collection_override_by_default` (该选项默认启用) 。
:::

<div id="storing-named-collections-in-the-system-database">
  ## 将 named collections 存储在 system database 中
</div>
...
要使用 DDL 管理命名集合，用户必须拥有 `named_collection_control` 特权。可通过在 `/etc/clickhouse-server/users.d/` 中添加文件来授予该特权。以下示例为用户 `default` 同时授予了 `access_management` 和 `named_collection_control` 特权：
...
:::tip
在上述示例中，`password_sha256_hex` 的值是密码的 SHA256 哈希的十六进制表示。用户 `default` 的这项配置包含 `replace=true` 属性，因为默认配置中已设置明文 `password`，而同一用户不能同时设置明文密码和 sha256 十六进制密码。
:::
...
命名集合既可存储在本地磁盘上，也可存储在 ZooKeeper/Keeper 中。默认使用本地存储。
它们也可以使用与[磁盘加密](storing-data#encrypted-virtual-file-system)相同的算法进行加密存储，
其中默认使用 `aes_128_ctr`。

要配置命名集合的存储，需要指定 `type`。其值可以是 `local` 或 `keeper`/`zookeeper`。对于加密存储，
可以使用 `local_encrypted` 或 `keeper_encrypted`/`zookeeper_encrypted`。

要使用 ZooKeeper/Keeper，还需要在 configuration file 的 `named_collections_storage` 部分中设置 `path` (即 ZooKeeper/Keeper 中存储命名集合的路径) 。
...
* `key_1` 始终可被覆盖。
* `key_2` 永远不可被覆盖。
* `url` 是否可被覆盖，取决于 `allow_named_collection_override_by_default` 的值。

<div id="modifying-named-collections">
  ## 修改命名集合
</div>

使用 DDL 查询创建的命名集合，可通过 DDL 进行修改或删除。使用 XML 文件创建的命名集合，则可通过编辑或删除相应的 XML 文件进行管理。

<div id="alter-a-ddl-named-collection">
  ### 修改 DDL 命名集合
</div>

更改或添加集合 `collection2` 中的键 `key1` 和 `key3`
...
修改或添加集合 `collection2` 的键 `key1`，并删除键 `key3`：

```sql
ALTER NAMED COLLECTION collection2 SET key1=4, DELETE key3
```

要强制某个键针对 `overridable` 标志使用默认设置，你必须
删除该键，然后重新添加。
...
参数说明请参见 [S3 表函数](../sql-reference/table-functions/s3.md)。
...
:::tip
上述 `s3()` 函数的第一个参数是集合名称 `s3_mydata`。如果不使用命名集合，那么每次调用 `s3()` 函数时，都需要传入访问密钥 ID、密钥、格式和 URL。
:::
...
:::note
该命名集合未指定 `table` 参数，因此需要在函数调用中将其指定为 `table = 'test'`。
:::
...
参数说明请参见 [postgresql](../sql-reference/table-functions/postgresql.md)。此外，还提供以下别名：

* `username` 作为 `user` 的别名
* `db` 作为 `database` 的别名。

在命名集合中，参数 `addresses_expr` 用于代替 `host:port`。该参数是可选的，因为还可以使用其他可选参数：`host`、`hostname`、`port`。下面的伪代码说明了其优先级：
...
:::note
创建表时，PostgreSQL 会从命名集合中复制数据。此后即使命名集合发生变化，也不会影响现有表。
:::
...
参数说明见 [remote](../sql-reference/table-functions/remote.md/#parameters)。
...
参数说明请参见 [Kafka](../engines/table-engines/integrations/kafka.md)。
...
参数说明请参见 [备份与恢复](/zh/operations/backup/overview)。
...
<div id="named-collections-for-accessing-mongodb-table-and-dictionary">
  ## 访问 MongoDB 表和字典时使用的命名集合
</div>

有关参数的说明，请参见 [mongodb](../sql-reference/table-functions/mongodb.md)。
...
:::note
该命名集合将 `my_collection` 指定为集合名称。在函数调用中，可通过 `collection = 'my_dict'` 覆盖它，以选择另一个集合。
:::