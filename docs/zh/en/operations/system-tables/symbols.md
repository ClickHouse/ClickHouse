---
description: '对 C++ 专家和 ClickHouse 工程师很有用的系统表，包含用于查看 `clickhouse` 可执行文件内部信息的相关信息。'
keywords: ['系统表', 'symbols']
...
包含用于查看 `clickhouse` 可执行文件内部信息的数据。访问此表需要 `内部信息` 权限。
...
* `symbol` ([String](/sql-reference/data-types/string)) — 二进制可执行文件中的符号名称。它是经过名称修饰的。你可以使用 demangle(symbol) 获取可读的名称。
* `symbol_demangled` ([Nullable(String)](/sql-reference/data-types/nullable)) — 用于 XRay 插桩的已解修饰符号。
* `function_id` ([Nullable(Int32)](/sql-reference/data-types/nullable)) — XRay 插桩映射中的函数 ID。
* `address_begin` ([UInt64](/sql-reference/data-types/int-uint)) — 二进制可执行文件中符号的起始地址。
* `address_end` ([UInt64](/sql-reference/data-types/int-uint)) — 二进制可执行文件中符号的结束地址。
