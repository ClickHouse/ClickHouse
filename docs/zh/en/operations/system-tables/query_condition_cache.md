...
* `key_hash` ([UInt128](/zh/sql-reference/data-types/int-uint)) — `(table&#95;uuid, part&#95;name, condition&#95;hash)` 的哈希值。
* `entry_size` ([UInt64](/zh/sql-reference/data-types/int-uint)) — 条目的大小 (以字节为单位) 。
* `matching_marks` ([String](/zh/sql-reference/data-types/string)) — 匹配到的标记。
