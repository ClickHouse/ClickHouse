...
与 [`TabSeparated`](/zh/interfaces/formats/TabSeparated) 格式的区别在于，该格式写入行时不进行转义。

:::note
使用此格式解析时，每个字段都不能包含制表符或换行符。
:::

有关 `TabSeparatedRaw` 格式与 `RawBlob` 格式的比较，请参见：[原始格式比较](../RawBLOB.md/#raw-formats-comparison)
