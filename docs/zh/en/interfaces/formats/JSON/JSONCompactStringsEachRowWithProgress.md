---
alias: []
description: 'JSONCompactStringsEachRowWithProgress 格式说明'
input_format: false
...
与 [`JSONCompactEachRowWithProgress`](/zh/interfaces/formats/JSONCompactEachRowWithProgress) 类似，但所有值都会被转换为字符串。
当你需要让所有数据类型都具有一致的字符串表示形式时，这种格式就很有用。

主要特性：

* 与 `JSONCompactEachRowWithProgress` 具有相同的结构
* 所有值都以字符串形式表示 (数字、数组等都会带上引号)
* 包含进度更新、总计和异常处理
