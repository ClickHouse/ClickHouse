...
在这种格式下，单个 JSON 对象会被解析为一个 [JSON](/zh/sql-reference/data-types/newjson.md) 值。如果输入包含多个 JSON 对象 (以逗号分隔) ，它们会被解析为单独的行。如果输入数据被 `[]` 包裹，则会被解析为 JSON 数组。

这种格式只能用于解析仅包含一个 [JSON](/zh/sql-reference/data-types/newjson.md) 类型字段的表。其余列必须设置为 [`DEFAULT`](/zh/sql-reference/statements/create/table.md/#default) 或 [`MATERIALIZED`](/zh/sql-reference/statements/create/view#materialized-view)。
