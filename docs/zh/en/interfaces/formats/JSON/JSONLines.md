...
这种格式也称为 `JSONEachRow`、`JSONLines`、`NDJSON` (以换行分隔的 JSON) 或 `JSONL`。这些名称都是同一种格式的别名，可在输入和输出中互换使用。
...
如果将设置 [input&#95;format&#95;skip&#95;unknown&#95;fields](/zh/operations/settings/settings-formats.md/#input_format_skip_unknown_fields) 设为 1，导入时会跳过名称未知的列。
