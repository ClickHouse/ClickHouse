...
`Protobuf` format 是 [Protocol Buffers](https://protobuf.dev/) 格式。

此格式需要一个外部 schema，该 schema 会在 queries 之间缓存。
...
如果列与 Protocol Buffers 消息中某个字段的类型不同，则会应用必要的转换。
...
嵌套消息适合作为[嵌套数据结构](/zh/sql-reference/data-types/nested-data-structures/index.md)的输入或输出。

在如下所示的 Protobuf schema 中定义的默认值不会生效，而是改为使用[表默认值](/zh/sql-reference/statements/create/table#default_values)：
...
如果消息包含 [oneof](https://protobuf.dev/programming-guides/proto3/#oneof)，并且设置了 `input_format_protobuf_oneof_presence`，ClickHouse 会填充一个列，用于指示找到的是 oneof 中的哪个字段。
...
用于指示是否存在的列名必须与 oneof 的名称相同。
支持嵌套消息 (参见 [basic-examples](#basic-examples)) 。也支持空消息。
允许的类型有 Int8、UInt8、Int16、UInt16、Int32、UInt32、Int64、UInt64、Enum、Enum8 和 Enum16。
Enum (以及 Enum8 或 Enum16) 必须包含 oneof 的所有可能标签，另外还必须包含用于表示不存在的 0；字符串表示形式无关紧要。

设置 [`input_format_protobuf_oneof_presence`](/zh/operations/settings/settings-formats.md#input_format_protobuf_oneof_presence) 默认是禁用的

ClickHouse 以 `带长度分隔的` 格式输入和输出 Protobuf Message。
这意味着在每条消息之前，都应写入其长度，编码为[可变宽度整数 (varint) ](https://developers.google.com/protocol-buffers/docs/encoding#varints)。
...
:::note 示例文件
本示例中使用的文件可在[示例仓库](https://github.com/ClickHouse/formats/ProtoBuf)中获取。
:::

在本示例中，我们将把文件 `protobuf_message.bin` 中的一些数据读入 ClickHouse 表。然后再使用 `Protobuf` 格式将其写出到名为 `protobuf_message_from_clickhouse.bin` 的文件中。
...
<details>
  ...
  如果你想使用其他语言，另请参见：[&quot;如何在常见语言中读取/写入带长度分隔的 Protobuf 消息&quot;](https://cwiki.apache.org/confluence/display/GEODE/Delimiting+Protobuf+Messages)。

  运行以下命令，在与 `schemafile.proto` 相同的目录中生成一个名为 `schemafile_pb2.py` 的 Python 文件。该文件包含表示你的 `UserData` Protobuf 消息的 Python 类：

  ```bash
  protoc --python_out=. schemafile.proto
  ```

  现在，在与 `schemafile_pb2.py` 相同的目录中创建一个名为 `generate_protobuf_data.py` 的新 Python 文件。将以下代码粘贴到其中：
  ...
  # --- 可选：验证（读回并打印） ---
  # 读回时，我们还会使用 Protobuf 内部的 varint 解码器。
  from google.protobuf.internal.decoder import _DecodeVarint32

  print("\n--- 通过读回进行验证 ---")
  with open(output_filename, "rb") as f:
      buf = f.read() # 将整个文件读入 buffer，便于进行 varint 解码
      n = 0
      while n < len(buf):
          # 解码 varint 长度前缀
...
您也可以使用 `Protobuf` 格式将数据重新写入二进制文件：
...
有了 Protobuf schema，现在你可以对 ClickHouse 写入文件 `protobuf_message_from_clickhouse.bin` 的数据进行反序列化。

<div id="basic-examples-cloud">
  ### 使用 ClickHouse Cloud 读取和写入数据
</div>

在 ClickHouse Cloud 中，您无法上传 Protobuf schema file。不过，您可以使用 `format_protobuf_schema`
设置在查询中指定 schema。在此示例中，我们将向您展示如何从本地
机器读取序列化数据，并将其插入到 ClickHouse Cloud 中的表。

与前一个示例一样，请根据您的 Protobuf schema，在 ClickHouse Cloud 中创建该表：
...
设置 `format_schema_source` 用于定义设置 `format_schema` 的来源
...
将数据插入 ClickHouse Cloud，并将 schema 以字符串形式指定，运行：
...
在 ClickHouse Cloud 上创建一个用于插入数据的表：
...
如果你的数据没有外部 Protobuf schema，仍然可以通过自动生成的 schema 以 Protobuf 格式输出/输入数据。为此，请使用 `format_protobuf_use_autogenerated_schema` 设置。
...
[`structureToProtobufSchema`](/zh/sql-reference/functions/other-functions#structureToProtobufSchema)根据表结构自动生成 Protobuf schema。随后，它会使用该 schema 将数据序列化为 Protobuf 格式。

你也可以使用自动生成的 schema 读取 Protobuf 文件。在这种情况下，文件必须使用同一个 schema 创建：

```bash
$ cat hits.bin | clickhouse-client --query "INSERT INTO test.hits SETTINGS format_protobuf_use_autogenerated_schema=1 FORMAT Protobuf"
```

设置 [`format_protobuf_use_autogenerated_schema`](/zh/operations/settings/settings-formats.md#format_protobuf_use_autogenerated_schema) 默认已启用，并且在未设置 [`format_schema`](/zh/operations/settings/formats#format_schema) 时生效。

你也可以在输入/输出时，使用设置 [`output_format_schema`](/zh/operations/settings/formats#output_format_schema) 将自动生成的 schema 保存到文件中。例如：
...
如需重新加载通过 [`format_schema_path`](/zh/operations/server-configuration-parameters/settings.md/#format_schema_path) 加载的 Protobuf schema，请使用 [`SYSTEM DROP ... FORMAT CACHE`](/zh/sql-reference/statements/system.md/#system-drop-schema-format) 语句。
