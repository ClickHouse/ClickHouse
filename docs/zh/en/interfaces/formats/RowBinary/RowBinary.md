...
`RowBinary` 格式按行以二进制格式解析数据。
行和值会连续排列，中间不使用分隔符。
由于数据采用二进制格式，`FORMAT RowBinary` 之后的分隔符有严格规定，如下所示：
...
:::note
由于该格式按行组织，因此不如 [Native](../Native.md) 格式高效。
:::
...
:::tip
示例中的大多数查询都可以使用 curl 执行，并将输出写入文件。
...
一种**无符号小端序**可变宽整数编码，用于编码 `String`、`Array` 和 `Map` 等可变大小数据类型的长度。示例实现可参见 [LEB128 wiki page](https://en.wikipedia.org/wiki/LEB128#Decode_unsigned_integer)。

<div id="integer-types">
  ### (U)Int8, (U)Int16, (U)Int32, (U)Int64, (U)Int128, (U)Int256
</div>

所有整数类型都会以适当的字节数按**小端序**编码。有符号类型 (`Int8` 到 `Int256`) 使用**补码**表示。大多数编程语言都支持借助内置工具或常用库，从字节数组中提取这类整数。对于 `Int128`/`Int256` 和 `UInt128`/`UInt256`，由于它们超出了大多数语言的原生整数位宽，可能需要自定义反序列化。
...
采用 **小端序** 编码的浮点数，`Float32` 为 4 字节，`Float64` 为 8 字节。与整数类似，大多数语言都提供了适当的工具来反序列化这些值。

<div id="bfloat16">
  ### BFloat16
</div>

[BFloat16](https://clickhouse.com/docs/sql-reference/data-types/float#bfloat16) (Brain Floating Point) 是一种 16 位浮点格式，具有 Float32 的取值范围，但精度更低，因此适用于机器学习场景。其传输格式本质上就是一个 Float32 值的高 16 位。如果你的编程语言原生不支持它，最简单的处理方式是按 UInt16 读取和写入，再与 Float32 相互转换：
...
其中，`trunc` 表示向零截断 (而不是向下取整；对于负值，这两者的结果不同) ，`scale` 是小数点后的位数。例如，对于 `Decimal(10, 2)` (等价于 `Decimal32(2)`) ，标度为 `2`，值 `12345` 将表示为 `(123, 45)`。
...
更多详情请参阅 [ClickHouse 文档中的 Decimal 类型](https://clickhouse.com/docs/sql-reference/data-types/decimal)。

<div id="string">
  ### String
</div>

ClickHouse 字符串是**任意字节序列**，不要求必须是有效的 UTF-8。长度前缀表示的是**字节长度**，而不是字符数。
...
与 `String` 不同，`FixedString` 的长度是固定的，并在 schema 中定义。如果该值短于 `N`，它会被编码为一个字节序列，并在末尾用零字节填充。

:::note
读取 `FixedString` 时，末尾的零字节可能是填充字节，也可能是数据中实际的 `\0` 字符；在传输格式中二者无法区分。ClickHouse 本身会原样保留全部 `N` 个字节。
:::

空的 `FixedString(3)` 仅包含用于填充的零字节：

```text
0x00, 0x00, 0x00
```

内容为字符串 `hi` 的非空 `FixedString(3)`：
...
在最后一个示例中，由于 *三个* 字节都已用上，因此无需填充。

<div id="date">
  ### Date
</div>

以 `UInt16` (两个字节) 存储，表示自 `1970-01-01` 起的天数。
...
存储为 `Int32` (四个字节) ，表示 `1970-01-01` ***之前或之后的*** 天数。
...
以 `UInt32` (4 字节) 存储，表示自 `1970-01-01 00:00:00 UTC` 起经过的秒数。
...
:::note
二进制值始终表示相对于 UTC 纪元的偏移量。时区不会改变编码。不过，时区**确实**会影响插入时对字符串值的解析方式：将 `'2024-01-15 10:30:00'` 插入 `DateTime('America/New_York')` 列时，存储的纪元值会不同于将同一字符串插入 `DateTime('UTC')` 列时的值，因为该字符串会按列时区的本地时间来解析。在传输格式中，两者都只是 `UInt32` 纪元秒。
:::
...
以 `Int64` (8 字节) 存储，表示 `1970-01-01 00:00:00 UTC` ***之前或之后*** 的 **ticks** 数量。tick 的分辨率由 `precision` 参数定义，参见下方语法：

```text
DateTime64(precision, [timezone])
```

其中，`precision` 是一个介于 `0` 到 `9` 之间的整数。通常只使用以下取值：`3` (毫秒) 、`6` (微秒) 、
`9` (纳秒) 。

有效的 DateTime64 定义示例包括：`DateTime64(0)`、`DateTime64(3)`、`DateTime64(6, 'UTC')` 或 `DateTime64(9, 'Europe/Amsterdam')`。

:::note
与 `DateTime` 一样，二进制值始终是相对于 UTC 纪元的偏移量。时区会影响插入时字符串值的解释方式 (参见 [DateTime](#datetime) 的说明) ，但编码本身始终是自 UTC 纪元以来的 `Int64` ticks。
:::

`DateTime64` 类型的底层 `Int64` 值可以解释为 UNIX 纪元之前或之后、以下单位的数量：
...
在内部，`Time64` 以 `Decimal64` (其本身存储为 `Int64`) 形式存储，用于表示带小数秒的时间值，精度可配置。负值有效。
...
底层的 `Int64` 值表示按 `10^precision` 缩放的秒的小数部分。
...
所有 Interval 类型都以 `Int64` 存储 (8 字节，小端序) 。该值表示相应时间单位的数量。负值也是有效的。
...
:::note
Interval 类型名称 (例如 `IntervalSecond` 与 `IntervalDay`) 决定了存储值的单位。其传输二进制编码始终相同。
:::
...
以单字节 (`Enum8` == `Int8`) 或双字节 (`Enum16` == `Int16`) 存储，表示该枚举值在枚举定义中的索引。请注意，存储类型是**有符号**的——枚举值可以是负数 (例如 `Enum8('a' = -128, 'b' = 0)`) 。
...
对于数据类型解析器来说，主要难点在于识别枚举定义中的转义字符 (例如 `\'`) ，以及可能出现在带引号字符串中的特殊字符 (如 `=`) 。

<div id="uuid">
  ### UUID
</div>

表示为一个由 16 个字节组成的序列。UUID 存储为**两个小端序 `UInt64` 值**：标准 UUID 表示中的前 8 个字节会按字节顺序反转，后 8 个字节也会单独按字节顺序反转。
...
它可用于插入新记录但未指定 UUID 值的情况。

<div id="ipv4">
  ### IPv4
</div>

以 `UInt32` 形式存储，占四个字节，采用**小端**字节序。请注意，这与 IP 地址通常使用的传统网络字节序 (大端) 不同。`IPv4` 的底层存储值示例：
...
以 **大端序 / 网络字节序** (MSB 在前) 存储，占 16 个字节。`IPv6` 的示例底层值：
...
1. 使用单个字节表示该值是否为 `NULL`：
   * `0x00` 表示该值不是 `NULL`。
   * `0x01` 表示该值为 `NULL`。
2. 如果该值不是 `NULL`，则其底层数据类型按常规方式编码；如果该值为 `NULL`，则**不会**为底层类型写入任何额外字节。
...
:::warning
这仅适用于 RowBinary。在 Native format 中，`LowCardinality` 使用不同的基于字典的编码方式。
:::

:::note
列可以定义为 `LowCardinality(Nullable(T))`，但不能定义为 `Nullable(LowCardinality(T))`——这样定义始终会导致服务器返回错误。
:::

在测试时，可将 [allow&#95;suspicious&#95;low&#95;cardinality&#95;types](https://clickhouse.com/docs/operations/settings/settings#allow_suspicious_low_cardinality_types) 设置为 `1`，以允许在 `LowCardinality` 中使用大多数数据类型，从而获得更好的覆盖率。
...
1. 使用一个[变长整数 (LEB128) ](#unsigned-leb128)表示数组中的元素数量。
2. 数组中的各个元素，按其底层数据类型的编码方式进行编码。
...
:::note
数组可以包含可为 NULL 的值，但数组本身不能是 Nullable。
:::
...
有关处理多维数组的示例，可参见 [Geo 部分](#geo-types)。

<div id="tuple">
  ### Tuple
</div>

Tuple 的编码方式是：其所有元素按照各自对应的传输格式依次编码，不带任何额外的元信息或分隔符。
...
Tuple 数据类型的字符串编码会带来与 [枚举类型](#enum8-enum16) 类似的挑战，例如需要处理转义符号和特殊字符；而对于 Tuple，还需要额外处理左右括号。此外请注意，较为复杂的 Tuple 还可能包含其他嵌套的 Tuple、Arrays、Map，甚至枚举。

例如，在下表中，这个 tuple 包含一个名称中带有单引号和括号的枚举；如果处理不当，可能会导致解析问题：
...
Map 可以看作 `Array(Tuple(K, V))`，其中 `K` 是键类型，`V` 是值类型。Map 的编码方式如下：

1. 一个[变长整数 (LEB128) ](#unsigned-leb128)，表示 Map 中的元素个数。
2. Map 中的元素以键值对形式存储，并按其对应类型进行编码。
...
:::note
也可以使用深度嵌套结构的 Map，例如 `Map(String, Map(Int32, Array(Nullable(String))))`，其编码方式与上文所述类似。
:::
...
该类型表示其他数据类型的 union。类型 `Variant(T1, T2, ..., TN)` 表示该类型的每一行都包含一个值，该值的类型可以是 `T1`、`T2`、…、`TN` 中的任意一种，也可以都不是 (即 `NULL` 值) 。

:::warning
虽然对于最终用户来说，`Variant(T1, T2)` 与 `Variant(T2, T1)` 完全等价，但定义中类型的顺序会影响传输格式：定义中的类型始终按字母顺序排序，这一点很重要，因为具体是哪个变体类型，是通过 &quot;discriminant&quot; (即该类型在定义中的索引) 来编码的。
:::
...
可使用 [allow&#95;suspicious&#95;variant&#95;types](https://clickhouse.com/docs/operations/settings/settings#allow_suspicious_variant_types) 设置，以便对 `Variant` 类型进行更充分的测试。

<div id="dynamic">
  ### Dynamic
</div>

`Dynamic` 类型可以容纳任意类型的值，具体类型在运行时确定。在 RowBinary format 中，每个值都是自描述的：第一部分是采用[这种格式](https://clickhouse.com/docs/sql-reference/data-types/data-types-binary-encoding)编码的类型说明。随后是内容部分，值的编码方式如本文档所述。因此，要解析某个值，你只需使用类型索引确定正确的解析器，然后复用你在其他地方已有的 RowBinary 解析逻辑即可。

```text
[BinaryTypeIndex][type-specific parameters...][value]
```

其中，`BinaryTypeIndex` 是用于标识类型的单字节。类型索引和参数请参阅[此处](https://clickhouse.com/docs/sql-reference/data-types/data-types-binary-encoding)的参考文档。

`NULL` Dynamic 值使用 `BinaryTypeIndex` `0x00` (即 `Nothing` 类型) 编码，不带任何额外字节：
...
1. **类型化路径** - 在 schema 中以显式类型声明的路径 (例如 `JSON(user_id UInt32, name String)`)
2. **动态路径/超出动态路径数量限制时的溢出路径** - 在运行时发现的路径，以 `Dynamic` 类型存储。值的编码前面带有类型定义。

这两类的传输格式和规则各不相同。

| 路径类别           | 是否包含在序列化中        | 值编码        | 是否允许 Variant/Nullable |
| -------------- | ---------------- | ---------- | --------------------- |
| **类型路径**       | 始终包含 (即使为 NULL)  | 特定类型的二进制格式 | 是                     |
| **Dynamic 路径** | 仅在非 NULL 时包含     | Dynamic    | 否                     |

路径按顺序分三组序列化：类型化路径、动态路径，以及共享数据 (溢出) 路径。类型化路径和动态路径以由具体实现决定的顺序写入 (由内部哈希映射迭代确定) ，共享数据路径则按字母顺序写入。读取方不应依赖任何特定的路径顺序。反序列化器按路径名称而非位置分发每条路径。

RowBinary 格式中的每个 JSON 行序列化为：
...
注意：带有 NULL 值的 `metadata` 路径**不会被包含**，因为动态路径仅在非 NULL 时才会被序列化。这是与类型化路径的一个关键区别。
...
**备选方案：JSON 作为 String 模式**

启用设置 `output_format_binary_write_json_as_string=1` 后，JSON 列会被序列化为单个 JSON 文本字符串，而不是结构化的二进制格式。还有一个对应的写入 JSON 列设置：`input_format_binary_read_json_as_string`。这里具体选择哪种设置，取决于你希望在客户端还是服务端解析 JSON。
...
Geo 值的传输格式与 Tuple 和 Array 完全一致。`RowBinaryWithNamesAndTypes` 格式的头部将包含这些类型的别名，例如 `Point`、`Ring`、`Polygon`、`MultiPolygon`、`LineString` 和 `MultiLineString`。
...
`Geometry` 是一种 `Variant` 类型，可容纳上文列出的任意 Geo 类型。在传输格式中，它的编码方式与 `Variant` 完全一致，使用一个 discriminant 字节来指示后续是哪一种 geo 类型。

Geometry 的 discriminant 索引如下：
...
在默认设置下，`Nested` 会展平为相互独立的数组。每个子列都会成为一个单独的 `Array` 列，列名以点号分隔：
...
每个数组都会分别序列化，如 [Array](#array) 一节所述：
...
使用 `flatten_nested = 0` 时，`Nested` 会保留为一个 `Array(Tuple(...))` 类型的单列。列名不采用点分隔：
...
编码格式为 `Array(Tuple(String, Int32))`：先是数组长度前缀，然后是每个元素中按顺序排列的元组字段：
...
请注意，这些字段是按元素交错排列的 (a₁, b₁, a₂, b₂) ，而不是像扁平化表示那样按列分组 (a₁, a₂, b₁, b₂) 。

<div id="simpleaggregatefunction">
  ### SimpleAggregateFunction
</div>

`SimpleAggregateFunction(func, T)` 的编码与其底层数据类型 `T` 完全相同。聚合函数名称不会影响传输格式。
...
RowBinaryWithNamesAndTypes 的头信息中将类型标记为 `SimpleAggregateFunction(max, UInt32)`，但在线上传输时，该值只是一个 `UInt32`：
...
`AggregateFunction(func, T)` 存储聚合函数的完整中间状态。与 `SimpleAggregateFunction` 不同，后者同样存储中间状态，但其编码方式与底层数据类型完全一致；`AggregateFunction` 存储的是不透明的二进制 blob，其格式由各个聚合函数各自定义。

:::warning
在 RowBinary 中，聚合状态**没有长度前缀**。解析器必须理解每个特定聚合函数的内部序列化格式，才能知道需要读取多少字节。实际使用中，大多数客户端会将聚合状态视为不透明对象，并使用 `*State` / `*Merge` 组合器，让服务端处理序列化。
:::
...
**`sumState`** — 将累积和存储在定长整数中。其位宽取决于参数类型 (整数参数为 `UInt64`) ：
...
**`minState` / `maxState`** — 存储一个标志字节，后面紧跟底层类型的值。空状态 (即未出现任何值) 时，标志为 `0x00`；有值时，标志为 `0x01`：
...
:::note
像 `uniq`、`quantile` 或 `groupArray` 这样的复杂函数会使用与具体实现相关的格式。如果你需要读取或写入这些状态，请查阅 ClickHouse 中对应函数的源代码。
:::
...
`QBit` 是一种向量类型，支持不同精度级别的高效查找。其内部以转置格式存储。在传输格式中，QBit 只是一个由底层元素类型 (`Int8`、`Float32`、`Float64` 或 `BFloat16`) 构成的 `Array`。用于存储的比特转置优化是在服务器端完成的，而不是在 RowBinary 协议中完成的。
...
其中，`element_type` 可以是 `Int8`、`Float32`、`Float64` 或 `BFloat16`，`dimension` 表示固定的向量维度。可选的 `stride` 仅控制服务端如何将位平面分组到存储流中；它不会影响 RowBinary 传输格式，后者始终是包含 `dimension` 个元素的完整数组。

传输格式：与 `Array(element_type)` 相同：
...
`QBit(Float32, 4)` 包含 `[1.0, 2.0, 3.0, 4.0]` 时的编码示例：
