---
description: '面向 s390x 架构的 ClickHouse 从源码构建指南'
...
ClickHouse 已实验性支持 s390x。

<div id="building-clickhouse-for-s390x">
  ## 为 s390x 构建 ClickHouse
</div>

与其他平台一样，s390x 会将 OpenSSL 构建为静态库。如果你想使用动态 OpenSSL 进行构建，需要向 CMake 传递 `-DENABLE_OPENSSL_DYNAMIC=1`。

以下说明假定主机为 Linux x86&#95;64/ARM，并且已根据[构建说明](../development/build.md)配齐原生构建所需的全部工具。它还假定主机系统为 Ubuntu 22.04，不过以下说明同样也适用于 Ubuntu 20.04。
...
要进行模拟运行，你需要适用于 s390x 的 QEMU user static 可执行文件。在 Ubuntu 上，可使用以下命令安装：

```bash
apt-get install binfmt-support binutils-s390x-linux-gnu qemu-user-static
```

构建完成后，即可按如下方式运行该二进制文件，例如：
...
在另一个 shell 中运行 LLDB 并附加到进程，将 `<Clickhouse Parent Directory>` 和 `<build directory>` 替换为与你所在环境相对应的值。
...
* 请确保在启动前先以调试模式运行 ClickHouse 可执行文件。 (也可以创建一个 `preLaunchTask` 来自动完成此操作)
...
这也会将不同的构建分别放到 `build` 文件夹下的不同子文件夹中。
...
定义了一个任务：在与二进制文件同级的 `tmp` 文件夹中，以 `server` 模式运行已编译的可执行文件，并使用 `programs/server/config.xml` 中的配置。
