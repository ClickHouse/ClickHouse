...
字段说明：https://www.gov.uk/guidance/about-the-price-付费数据
...
- 将`postcode` 拆分为两个不同的列 - `postcode1` 和 `postcode2`，这更适合存储和查询 
- 将`time` 字段转换为日期为它只包含 00:00 时间 
- 忽略 [UUid](../../sql-reference/data-types/uuid.md) 字段，因为我们不需要它进行分析 
- 转换 `type` 和 `duration ` 使用 [transform](../../sql-reference/functions/other-functions.md#transform) 函数将 `Enum` 字段转换为更易读的 `Enum` 字段 
