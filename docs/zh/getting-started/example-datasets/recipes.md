...
RecipeNLG 数据集可在 [此处](https://recipenlg.cs.put.poznan.pl/dataset) 下载. 它包含 220 万个食谱. 大小略小于 1 GB.

## 下载并解压数据集

1. 转到下载页面 [https://recipenlg.cs.put.poznan.pl/dataset](https://recipenlg.cs.put.poznan.pl/dataset).
2. 接受条款和条件并下载 zip 文件.
3. 使用 `unzip` 解压 zip 文件. 您将获得 `full_dataset.csv` 文件.
...
这将展示如何解析自定义CSV, 因为它需要多个调优.

解释: 
- 数据集为 CSV 格式, 但需要在插入时进行一些预处理; 我们使用表函数 [input](../../sql-reference/table-functions/input.md) 进行预处理;
- CSV 文件的结构在表函数 `input` 的参数中指定;
- 不需要字段 `num` (行号) ——我们从文件中解析它并忽略;
- 我们使用`FORMAT CSVWithNames`, 但CSV 中的标题将被忽略 (通过命令行参数 `--input_format_with_names_use_header 0` ), 因为标题不包含第一个字段的名称;
- 文件仅使用双引号将 CSV 字符串括起来; 有些字符串没有用双引号括起来, 单引号不能被解析为封闭的字符串——这就是为什么我们还要添加 `--format_csv_allow_single_quote 0` 参数;
- 来自 CSV 的某些字符串无法解析, 因为它们在值的开头包含 `\M/` 序列; CSV 中以反斜杠开头, 然后放入 CSV - 我们将它们解析为字符串, 然后使用 [JSONExtract](../../sql-reference/functions/json-functions/) 函数将其转换为数组.
...
在这个例子中，我们学习如何使用 [arrayJoin](../../sql-reference/functions/array-join/) 函数将数组扩展为一组行.
...
在这个例子中, 我们涉及到 [has](../../sql-reference/functions/array-functions/#hasarr-elem) 函数来过滤数组元素并按方向数排序.

有一种婚礼蛋糕需要126个步骤才能制作出来! 显示的方向:
...
该数据集也可以在 [Online Playground](https://gh-api.clickhouse.com/play?user=play#U0VMRUNUCiAgICBhcnJheUpvaW4oTkVSKSBBUyBrLAogICAgY291bnQoKSBBUyBjCkZST00gcmVjaXBlcwpHUk9VUCBCWSBrCk9SREVSIEJZIGMgREVTQwpMSU1JVCA1MA==) 中找到.

[来源文章](https://clickhouse.com/docs/en/getting-started/example-datasets/recipes/) <!--hide-->
