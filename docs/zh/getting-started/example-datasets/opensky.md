# OpenSky网络2020众包空中交通数据 {#opensky}

"该数据集中的数据源自完整的 OpenSky 数据集并进行了清理, 以说明 COVID-19 大流行期间空中交通的发展. 它涵盖了自 2019 年 1 月 1 日以来该网络的 2500 多名成员看到的所有航班.更多数据将在 定期包含在数据集中, 直到 COVID-19 大流行结束".

资料来源: https://zenodo.org/record/5092942#.YRBCyTpRXYd

Martin Strohmeier、Xavier Olive、Jannis Lübbe、Matthias Schäfer 和 Vincent Lenders "来自 OpenSky 网络的众包空中交通数据 2019–2020" 地球系统科学数据 13(2),  2021 https://doi.org/10.5194/essd-13-357-2021
...
在网络连接良好的情况下, 下载大约需要 2 分钟. 有 30 个文件, 总大小为 4.3 GB.
...
- 在这里, 我们将文件列表 (`ls -1 flightlist_*.csv.gz`) 传递给 `xargs` 以进行并行处理.
`xargs -P100` 指定最多使用 100 个并行工作器, 但由于我们只有 30 个文件, 因此工作器的数量仅为 30.
- 对于每个文件, `xargs` 将使用 `bash -c` 运行一个脚本. 脚本以 `{}` 的形式进行替换, `xargs` 命令会将文件名替换为它 (我们已经用 `-I{}` 要求它提供 `xargs`).
- 脚本会将文件 (`gzip -c -d "{}"`) 解压缩到标准输出 (`-c` 参数), 并将输出重定向到 `clickhouse-client`.
...
ClickHouse 中数据集的大小仅为 2.66 GiB, 检查它.
...
### 最繁忙的始发机场和看到的平均距离 {#busy-airports-average-distance}
...
### 每周从莫斯科三大机场起飞的航班数量 {#flights-from-moscow}
