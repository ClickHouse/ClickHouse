  ...
  <div className="relative">
    ...
    <div className="relative z-10 px-0 py-16 lg:py-12 lg:pb-40 max-w-6xl mx-auto">
      ...
      <div className="grid sm:grid-cols-2 min-[1300px]:grid-cols-4 gap-x-6 gap-y-4 mt-6">
        <HeroCard title="实时分析" galaxyEvent="docs.home.real-time-analytics" description="大规模交付即时洞察和仪表盘。实时分析数十亿行数据，结果可在毫秒内返回。" href={localizeHref("/get-started/use-cases/real-time-analytics")} showArrow />

        <HeroCard title="可观测性" galaxyEvent="docs.home.observability" description="借助 ClickStack（由 ClickHouse 提供支持的开源可观测性技术栈），大规模存储和查询日志、指标和链路追踪。" href={localizeHref("/clickstack/overview")} showArrow />
