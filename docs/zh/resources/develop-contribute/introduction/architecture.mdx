...
为了解决解释器中现有的问题，开发了新的 `InterpreterSelectQueryAnalyzer`。这是 `InterpreterSelectQuery` 的新版本，不使用 `ExpressionAnalyzer`，并在 `AST` 和 `QueryPipeline` 之间引入了一个名为 `QueryTree` 的额外抽象层。自 `26.9` 版本起，它是唯一的查询分析实现；曾用于切换回 `ExpressionAnalyzer` 的 `enable_analyzer` 设置已废弃，无法再被禁用。
