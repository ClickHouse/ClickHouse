...
- `-c`, `--config-file` — path to configuration file in same format as for ClickHouse server. When it is not given, `clickhouse-local` looks up its main configuration file itself, see ["Configuration files"](#configuration-files); the configuration is empty only when none of the searched files exists.
...
2. `./config.xml`, `./config.yaml` or `./config.yml`;
