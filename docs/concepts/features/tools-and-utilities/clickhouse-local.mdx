...
- `--ignore-error` — do not stop processing if a query failed: the error is reported on `stderr` and the next query is processed. Because the errors are ignored, the process exits with a success code even when some of the queries failed.
