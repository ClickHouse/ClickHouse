...
* `--config-file, -C` -- path to ClickHouse config. By default, the first of `/etc/clickhouse-server/config.xml`, `/etc/clickhouse-server/config.yaml` and `/etc/clickhouse-server/config.yml` that exists is used.
* `--save-logs` -- Log progress of invoked commands to `/var/log/clickhouse-server/clickhouse-disks.log`.
* `--log-level` -- What [type](/reference/settings/server-settings/settings/other#logger) of events to log, defaults to `none`.
