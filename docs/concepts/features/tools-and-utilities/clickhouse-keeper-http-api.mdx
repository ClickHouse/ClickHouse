...
Query parameters:
- `complete` - When present, tab-completes the CLI line prefix given as the parameter value (text up to the cursor), using the same logic as `clickhouse-keeper-client`. Returns completion candidates and a `replace_start` index. An empty value lists all registered command names. No command is executed.
...
`completions` are strings that replace the prefix from `replace_start` to the end of the `complete` value (same contract as replxx last-word completion). The embedded dashboard uses this endpoint for Tab completion so escaping and multi-argument handling stay on the server.
