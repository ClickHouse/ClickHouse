...
Keeper Client CLI responses always include `result`. An optional `cwd` field is present only when the command successfully changed the working directory (currently, a successful `cd`). Four-Letter Word responses include `result` only.

CLI command (for example, `ls` or `create`):
...
Failed `cd` (path does not exist; working directory unchanged):

```json
{
  "result": "Path /does_not_exist does not exist\n"
}
```
...
Pass the current working directory in the `cwd` query parameter on each request. Update your local working directory only when the response includes `cwd`.
...
# Change directory; cwd is returned only on success
