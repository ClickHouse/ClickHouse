...
-   `-c FILE_PATH`, `--config-file=FILE_PATH` — Set path of config file to get the connection string. By default, the first of `config.xml`, `config.yaml` and `config.yml` that exists in the current directory is used.
-   `--password=PASSWORD` — Password for authentication. Can also be set via the `CLICKHOUSE_KEEPER_PASSWORD` environment variable or in the configuration file under `<zookeeper><password>` (`zookeeper.password` in YAML).
-   `--identity=IDENTITY` — Identity for `digest` authentication scheme. Can also be set via the `CLICKHOUSE_KEEPER_IDENTITY` environment variable or in the configuration file under `<zookeeper><identity>` (`zookeeper.identity` in YAML).
...
3. `<zookeeper><password>` in the loaded configuration file

The configuration file in step 3 is the one given by `--config-file`, or, when that option is omitted, the first of `config.xml`, `config.yaml` and `config.yml` found in the current directory. Both the XML and the YAML forms are read, so `zookeeper.password` in a `config.yaml` works exactly like `<zookeeper><password>` in a `config.xml`.

The same priority applies to `--identity` / `CLICKHOUSE_KEEPER_IDENTITY` / `<zookeeper><identity>`.

Example configuration file with authentication settings:
