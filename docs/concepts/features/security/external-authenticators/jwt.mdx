...
| Grants claim | The name of the token claim that holds SQL `GRANT` fragments for the ephemeral user (the default is `clickhouse:grants`). The claim must contain a JSON array of grant fragments, for example `["SELECT ON analytics.*", "INSERT ON analytics.events"]`. |

The roles named in the roles claim must already exist in the service (create them with `CREATE ROLE`). The permission limit caps the effective access rights from both the roles and grants claims, as described in [Access rights](#access-rights).
