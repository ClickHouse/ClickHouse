...
| Claim | Description |
|---|---|
| `alg` | Signing algorithm (header claim). Supported value: `RS256`. From version 26.8, EC algorithms `ES256`, `ES384`, and `ES512` are also supported. |
...
For ClickHouse Cloud endpoints, when no `--oauth-*` option is passed, the client logs in with the ClickHouse Cloud IdP and automatically performs token exchange to obtain a ClickHouse-specific JWT. Tokens are refreshed transparently during the session. When a new token is obtained, the client reconnects automatically.

```bash
clickhouse-client --host your-instance.clickhouse.cloud --login
```

For a custom IdP, pass the OIDC issuer as `--oauth-url`. Passing any `--oauth-*` option selects the custom IdP flow, also for ClickHouse Cloud hosts (for example, when [JWT authentication with a custom identity provider](#enabling-jwt-authentication-for-your-service) is enabled for the service). The client discovers `device_authorization_endpoint` and `token_endpoint` from the issuer's `/.well-known/openid-configuration` (and related OAuth metadata URLs). If `grant_types_supported` is present, it must include `urn:ietf:params:oauth:grant-type:device_code`. If discovery fails, the client reports an error; set both `--oauth-device-uri` and `--oauth-token-uri` explicitly when metadata is unavailable.

During login the client prints the short `verification_uri` and `user_code` (RFC 8628). When the IdP also returns `verification_uri_complete`, that shortcut URL is printed, opened in a browser, and rendered as a terminal QR code when the `qrencode` tool is available. For the ClickHouse Cloud IdP, only `verification_uri_complete` and `user_code` are printed. Confidential clients may pass `--oauth-client-secret` with `--oauth-client-auth basic|post`.
...
|---|---|
| Name | A unique name for the provider on the service. |
...
| JWKS URL | The HTTPS endpoint where your identity provider publishes its JSON Web Key Set (for example, `https://idp.example.com/.well-known/jwks.json`). ClickHouse fetches the public keys from this endpoint to verify token signatures. JWKS verification works with RSA keys (`RS256`) and, from version 26.8, EC keys on the P-256, P-384, and P-521 curves (`ES256`, `ES384`, `ES512`), as described in [Required claims](#required-claims). |
| Roles claim (optional) | The name of the token claim that holds the ClickHouse roles for the ephemeral user. Leave blank to use the default claim name `clickhouse:roles`. The claim must contain a JSON array of role names, for example `["analyst", "reader"]`. |
