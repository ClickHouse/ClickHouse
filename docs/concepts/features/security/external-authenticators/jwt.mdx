...
Roles are read from the roles claim above, and SQL grants are read from the token's `clickhouse:grants` claim by default. The roles named in the roles claim must already exist in the service (create them with `CREATE ROLE`). The permission limit caps the effective access rights from both the roles and grants claims, as described in [Access rights](#access-rights).
