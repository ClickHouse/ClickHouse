...
<clickhouse>
    ...
    <ldap_servers>
        ...
        <my_ldap_server>
            ...
            <tls_maximum_protocol_version>tls1.3</tls_maximum_protocol_version>
...
| `operation_timeout` | `40` | Maximum time, in seconds, to wait for the result of a single LDAP operation (a bind, a StartTLS negotiation, or a search) on an established connection. Must be greater than `0`. |
...
| `search_timeout` | `20` | Time limit, in seconds, for LDAP search requests (user DN detection and role mapping). It is sent to the server as the requested time limit and enforced on the client as well. Must be greater than `0`. |
