...
| `groups` | empty | Optional allow-list of `<group>` entries; when non-empty, only values matching one of the entries are mapped and all others are ignored. An entry containing `=` is a full group DN: it is compared against the raw search result before `rdn_attribute` extraction, ignoring differences in letter case, whitespace and escaping, and the role name is derived from the `rdn_attribute` value of the configured DN (requires `rdn_attribute`). Any other entry is a plain group name compared ASCII-case-insensitively against the value after `rdn_attribute` extraction; the spelling configured here is used as the role name. Empty and duplicate entries are rejected at startup. See [Mapping pipeline](#ldap-role-mapping-pipeline). |
...
3. If the value starts with `prefix`, the prefix is removed; otherwise the value is dropped. `prefix` is compared case-sensitively.
...
The example above reads the `memberOf` attribute of the user's own entry: `base_dn` is `{user_dn}`, so `user_dn_detection` must be configured for the server, `scope` is `base` and `search_filter` is `(objectClass=*)`. The search runs under the identity bound during authentication, so the user must be allowed to read their own `memberOf` attribute, which is the default in Active Directory.
