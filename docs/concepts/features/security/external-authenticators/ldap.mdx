...
Names are matched exactly and are case-sensitive; patterns are not supported. An empty `<user>` entry is rejected at startup with `BAD_ARGUMENTS`. The configured list is shown in the `params` column of the [`system.user_directories`](/reference/system-tables/user_directories) table as `exclude_users`.
