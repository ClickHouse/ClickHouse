...
A row policy applies where the table data is read locally. For a [Distributed](/reference/engines/table-engines/special/distributed) table, or another table that delegates reading to remote servers, define the policy on the underlying tables on each server: a policy on the remote-backed table itself cannot be applied to the shipped query, and reads through it are rejected instead. Such a policy on an underlying table is applied whether the initiator ships the query as text or as an already-built read plan ([`serialize_query_plan`](/reference/settings/session-settings/serialize#serialize_query_plan)).
