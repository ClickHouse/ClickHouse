...
* The specialized **`jsonbf_v1`** indexes scalar leaves of one direct `JSON` column. Paths, container roles, runtime types, and values are hashed separately, so values at different paths do not match each other. Arrays are indexed by element, typed `Map` values by key, and `Tuple` values by field. Whole containers are not stringified.
  ...
  INDEX json_values data TYPE jsonbf_v1(
      ...
      skip_paths_regexp = ['^metadata\\.', '.*_id$']) GRANULARITY 1
  ```

  `skip_paths` excludes exact paths and their nested paths. `skip_paths_regexp` excludes paths using partial regular-expression matches. Queries on excluded paths do not use the index.
