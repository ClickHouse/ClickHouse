...
There are four Data Skipping Index types based on Bloom filters:
...
* The specialized **jsonbf_v1** indexes scalar leaves of one direct `JSON` column. Paths, container roles, runtime types, and values are hashed separately, so values at different paths do not match each other. Arrays are indexed by element, typed `Map` values by key, and `Tuple` values by field. Whole containers are not stringified. The optional `tokenizer = ngrams(N)` argument also indexes path-qualified ngrams for positive string predicates such as `LIKE`, `startsWith`, `endsWith`, `multiSearchAny`, and `match`.

  ```sql
  INDEX json_values data TYPE jsonbf_v1(false_positive_rate = 0.025) GRANULARITY 1
  INDEX json_search data TYPE jsonbf_v1(false_positive_rate = 0.025, tokenizer = ngrams(3)) GRANULARITY 1
  ```

  Runtime `Dynamic` types that cannot be represented by `jsonbf_v1` make the containing index granule conservative: ClickHouse reads that granule instead of risking a false negative. Complex runtime values add a path-qualified presence token so incompatible predicates on the same path cannot hide an exception. The `ngrams` tokenizer adds one path-qualified token for every distinct ngram, so long strings or JSON documents with many string leaves can produce large index granules.
