...
Data skipping index types based on Bloom filters include:
...
* The specialized **`jsonbf_v1`** indexes scalar leaves of one direct `JSON` column. Paths, container roles, runtime types, and values are hashed separately, so values at different paths do not match each other. Arrays are indexed by element, typed `Map` values by key, and `Tuple` values by field. Whole containers are not stringified. Path presence predicates, such as `isNotNull(data.path)` or `has(JSONAllPaths(data), 'path')`, are answered from the index as well. The index is experimental and requires the setting `allow_experimental_json_bloom_filter_index`.

  ```sql
  INDEX json_values data TYPE jsonbf_v1(
      false_positive_rate = 0.025,
      include_paths = ['tenant'],
      include_paths_regexp = ['(?:^|\\.)(?:.*_id|.*_at)$'],
      skip_paths = ['debug.request_id'],
      skip_paths_regexp = ['^private\\.']) GRANULARITY 1
  ```

  `include_paths` includes exact paths and their nested paths. `include_paths_regexp` includes paths using partial regular-expression matches. Array elements do not add a path segment, so a field below `items Array(JSON)` has a path such as `items.item_id`. When neither include option is specified, all paths are included. The skip options use the same exact-subtree and partial-match rules and take precedence over the include options. Queries on paths that are not included or are skipped do not use the index.

  Each path has a separate value filter. Queries can avoid reading large filters for paths that their predicates do not use. Runtime type presence is stored separately from the value filters, so Bloom false positives cannot make an absent runtime type appear present.

  Runtime `Dynamic` types that cannot be represented by `jsonbf_v1` make predicates on that path conservative. Predicates on other paths can still prune the granule. Complex runtime values add a path-qualified presence token so incompatible predicates on the same path cannot hide an exception.

  A typed path of the `JSON` type has the default value of its type in rows where the path is absent, for example an empty string for a `String` path. These default values are not indexed, the same as absent paths in shared data. Predicates that compare a typed path with the default value of its type, such as `data.name = ''`, do not use the index.
...
<Image img="/images/guides/best-practices/bad_skip.webp" size="md" alt="Bad Skip"/>
