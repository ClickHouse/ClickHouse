...
First running the query (with cold filesystem caches) with lazy materialization disabled (using [`query_plan_optimize_lazy_materialization`](/reference/settings/session-settings/query-plan#query_plan_optimize_lazy_materialization)):
