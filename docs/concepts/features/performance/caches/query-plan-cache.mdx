...
:::note
The query plan cache is experimental. To use it, set `enable_query_plan_cache` to `1`. Only queries that also run with [`enable_analyzer`](/reference/settings/session-settings/allow-experimental#allow_experimental_analyzer) (default) are eligible.
:::
...
Unlike the [query cache](/concepts/features/performance/caches/query-cache), which caches query *results*, the query plan cache caches only the *plan*: every cache hit still executes the query and reads up-to-date data.
...
The experimental session setting `enable_query_plan_cache` controls the feature and defaults to `0`.
...
- [`enable_analyzer`](/reference/settings/session-settings/allow-experimental#allow_experimental_analyzer) is `1` (the default).
...
- [Query cache](/concepts/features/performance/caches/query-cache) — caches query results, not plans.
- [Query condition cache](/concepts/features/performance/caches/query-condition-cache) — caches per-granule predicate evaluation outcomes.
