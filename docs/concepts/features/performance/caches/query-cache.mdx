...
able to report whether its data changed; if a query references a table that cannot do so (for example a table function such as `url`), or the
query calls a non-deterministic function (whose result can change while every referenced table is unchanged), the
