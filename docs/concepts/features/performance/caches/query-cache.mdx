...
query calls a non-deterministic function (whose result can change while every referenced table is unchanged), or the query runs inside a
transaction (the check sees the live table state, not the transaction's snapshot), the query cache is not used for that query. For `URL` and object-storage tables this report is based on the resource's strong `ETag` and is
