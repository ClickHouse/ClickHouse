...
to the in-memory query cache.

Setting `query_cache_share_between_users` is an exception: it has the same meaning for both backends - an entry written with it is served
