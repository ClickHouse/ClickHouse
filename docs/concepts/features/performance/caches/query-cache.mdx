...
Alternatively, setting [query_cache_use_only_when_data_was_not_changed](/reference/settings/session-settings/query-cache#query_cache_use_only_when_data_was_not_changed)
...
query calls a non-deterministic function (whose result can change while every referenced table is unchanged), the query runs inside a
transaction (the check sees the live table state, not the transaction's snapshot), or a referenced table has an active row policy for the
current user (the policy can change what the user reads while every referenced table is unchanged), the query cache is not used for that query. For `URL` and object-storage tables this report is based on the resource's strong `ETag` and is
...
`Distributed` are likewise best-effort if the set of underlying tables changes during a query, and row policies that exist only on remote shard
servers of a `Distributed` table are not seen by the check. This covers a table's regular columns and data
