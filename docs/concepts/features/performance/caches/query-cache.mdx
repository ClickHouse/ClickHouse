...
- `enable_writes_to_query_cache_disk` controls whether query results are written to the disk cache (in addition to the in-memory cache).
- `enable_reads_from_query_cache_disk` controls whether the server may serve a query result from the disk cache (after a memory cache miss).
...
event in system table [system.events](/reference/system-tables/events).
...
unless it was explicitly marked as shared (see [query_cache_share_between_users](/reference/settings/session-settings/query-cache#query_cache_share_between_users)).

The location and capacity of the disk cache are configured via [server configuration options](/reference/settings/server-settings/settings/query#query_cache):
...
On configuration reload, the `max_disk_size_in_bytes` and `max_disk_entries` limits affect future cache inserts. Existing entries are
evicted lazily. The `disk` and `path` settings are only read at server startup and require a server restart to take effect.

Column `type` in system table [system.query_cache](/reference/system-tables/query_cache) shows whether an entry is stored in `Memory` or on
