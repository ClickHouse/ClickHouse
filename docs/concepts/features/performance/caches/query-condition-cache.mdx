...
The grid step is proportional to the distance between the constant and the current time - one day for a condition looking back ten days, one hour for a condition looking back ninety minutes - and is capped at one day; the proportionality factor can be configured with the setting [`query_condition_cache_time_condition_grid_factor`](/reference/settings/session-settings/query-condition#query_condition_cache_time_condition_grid_factor) (default: 1.0).
