...
The query condition cache also works for Parquet files read from local files, object storage (e.g. S3) and data lakes (e.g. Iceberg), through tables as well as through table functions such as `file`, `s3` or `icebergS3`.
For these, the cache remembers the row groups of a file which contain no matching rows.
The entries are keyed by the location of the file and its version (the ETag of a remote object, the modification time, inode and size of a local file), so a query after the file was rewritten does not reuse them. Data files of data lakes are never rewritten, so their location alone identifies them.
A local file is only cached a few seconds after its last modification, as file system timestamps are too coarse to tell apart two writes in quick succession.
