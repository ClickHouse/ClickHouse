...
- Partial aggregate cache (experimental). See [`use_partial_aggregate_cache`](/operations/settings/settings#use_partial_aggregate_cache) and [`SYSTEM CLEAR AGGREGATE CACHE`](/sql-reference/statements/system#drop-aggregate-cache).
- Format schema cache.
