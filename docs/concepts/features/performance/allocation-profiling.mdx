...
- [`system.jemalloc_bins`](/reference/system-tables/jemalloc_bins) gives the ground truth about wasted memory: for every small size class, the `waste` alias (`curslabs * slab_size - curregs * size`) is the number of bytes held in slabs but not occupied by live regions.
...
```sql
-- Top pinner backtraces for the most wasteful size classes.
...
SELECT
    ...
    s.est_old_objects,
    s.stack
FROM system.jemalloc_bins AS b
INNER JOIN
(
    SELECT
        size_class,
    ...
    GROUP BY size_class, trace
) AS s ON s.size_class = b.index
...
The same join can attribute each size class's waste to backtraces proportionally to their estimated share of old objects, in FlameGraph-compatible collapsed format:

```sql
WITH 60e9 AS min_age_ns
SELECT format('{} {}', s.stack, toUInt64(b.waste * s.est_old_objects / sum(s.est_old_objects) OVER (PARTITION BY s.size_class)))
FROM system.jemalloc_bins AS b
INNER JOIN
(
    SELECT
