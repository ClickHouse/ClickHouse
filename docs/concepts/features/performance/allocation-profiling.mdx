...
Two system tables together make memory fragmentation analysis a plain SQL query:

- [`system.jemalloc_bins`](/reference/system-tables/jemalloc_bins) gives the ground truth about wasted memory: for every small size class, the `waste` alias (`curslabs * slab_size - curregs * size`) is the number of bytes held in slabs but not occupied by live regions. `system.jemalloc_arena_bins` exposes the same counters with one row per (arena, bin); since every slab belongs to a single arena, it locates the waste in a specific arena and is the correct join target for attribution. Its `purpose` column labels the dedicated arenas for long-lived state (`mergetree` metadata, `jit`, `cache`) — those hold old allocations by design, so fragmentation reports usually exclude them with `purpose = ''`.
- `system.jemalloc_sampled_allocations` exposes one row per live sampled allocation from the current heap profile (reading the table flushes a fresh profile): the allocating backtrace (`trace`), the age of the allocation (`age_ns`, with the approximate `alloc_time` alias), the requested and usable sizes (`size`, `usize`), the size class (`size_class`, joins to `system.jemalloc_bins.index` for small classes), the arena (`arena`) and the allocating thread (`thr_uid`).

Keep the estimator semantics in mind when interpreting the results:

- Only sampled allocations appear, and only threads with the profiler armed (query setting `jemalloc_enable_profiler` or the `jemalloc_enable_global_profiler` config) contribute rows.
- Sampling is byte-weighted: a row stands for roughly `weight` similar allocations, where the `weight` alias is `1 / (1 - exp(-usize / sample_interval))`. Small size classes need a small sampling interval (`jemalloc_profiler_sampling_rate`) to be represented at all.
- Sampled allocations are placed outside slabs (each occupies at least one page), so the rows are a statistical model of the allocation pattern — call site, size class, lifetime — never the slab residents themselves. Ground truth about waste comes from `system.jemalloc_bins`.

A slab cannot be released while at least one region in it is live, so long-lived allocations pin slabs and turn churn in the same size class into waste. To find the backtraces that likely pin the most wasteful size classes:

```sql
-- Top pinner backtraces for the most wasteful (arena, size class) pairs.
...
WHERE b.large = 0 AND b.waste > 1048576
ORDER BY b.waste DESC, s.est_old_objects DESC
```

Symbolization uses `addressToSymbol` and `demangle`, so the query requires `allow_introspection_functions = 1`.

### Example: generating a fragmentation flame graph {#example-fragmentation-flamegraph}

A `LEFT JOIN` of the same subquery can attribute each size class's waste to backtraces proportionally to their estimated share of old objects, in FlameGraph-compatible collapsed format. A `(arena, size class)` with waste but no old-enough sample is emitted as an `[unattributed]` frame, so the flame graph totals the full slab waste instead of silently dropping whatever sampling did not cover (an `INNER JOIN` would undercount exactly when coverage is incomplete). Both branches must come from a single read of `system.jemalloc_sampled_allocations` — every read flushes a fresh profile, so two independent subqueries could observe different dumps:
...
WHERE b.large = 0 AND b.waste > 0
```

```sh
clickhouse-client --allow_introspection_functions=1 -q "..." | flamegraph.pl --color=mem --title="Fragmentation Flame Graph" --width 2400 > result.svg
```
