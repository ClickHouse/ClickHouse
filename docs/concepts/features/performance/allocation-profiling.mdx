...
The server UI includes all tabs: Summary, Allocations, Arenas, Operations, Global Profiler, Query Profiler, Fragmentation, and Raw Output. The Fragmentation tab (26.9+) renders the [fragmentation report](#analyzing-fragmentation-from-sql) — waste per (arena, size class), the likely pinner stacks, and an attributed-waste flame graph.
...
- `system.jemalloc_arena_bins` gives the ground truth: per (arena, small size class), the `waste` alias is the bytes held in slabs but not occupied by live allocations. The `purpose` column labels dedicated arenas for long-lived state (`mergetree`, `jit`, `cache`) — those hold old allocations by design, so reports usually exclude them with `purpose = ''`. ([`system.jemalloc_bins`](/reference/system-tables/jemalloc_bins) is the same aggregated over arenas.)
- `system.jemalloc_sampled_allocations` has one row per live sampled allocation (reading it flushes a fresh heap profile): backtrace (`trace`), age (`age_ns`), sizes (`size`, `usize`), `size_class`, `arena`, `thr_uid`.

Estimator semantics:

- Only threads with the profiler armed contribute rows (`jemalloc_enable_profiler` setting or `jemalloc_enable_global_profiler` config).
- Sampling is byte-weighted: a row stands for roughly `weight` similar allocations. Small size classes need a small sampling interval (`jemalloc_profiler_sampling_rate`) to be represented at all.
- Sampled allocations live outside slabs — the rows model the allocation pattern, never the slab residents; and each live sample costs one page plus a `cache_oblivious` padding page (128 KiB on 64 KiB-page builds) for as long as the sampled allocation is alive (disarming the profiler stops new samples but does not release the existing ones).

A slab cannot be released while at least one allocation in it is live, so long-lived allocations turn churn in the same size class into waste. To find the backtraces likely holding the most wasteful size classes:
