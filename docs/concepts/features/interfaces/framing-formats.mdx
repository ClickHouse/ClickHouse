---
description: 'Framing formats multiplex data, totals, extremes, progress, profile events, stack trace samples, and server logs in a single response stream over HTTP'
...
Server logs are included as packets if the `send_logs_level` setting is set. Profile events are included if the `send_profile_events` setting is enabled (default). Sampled stack traces are included if `send_profile_traces` is enabled (disabled by default), independently of `send_profile_events`. Progress, profile events, and profile trace packets are sent at most once in `interactive_delay` microseconds during execution. A final drain sends the remaining samples before the terminal `progress` or `exception` packet.

A successful stream ends with a final `progress` packet carrying the final counters (`result_rows`, `result_bytes`, `memory_usage`), like the final progress packet of the native protocol. These counters are known only after the query has finished, so no earlier `progress` packet carries them. The final `progress` packet is written after the remaining `profile_traces` packets and the trailing `log` and `profile_events` packets emitted by the query-finish logging (for example the "peak memory usage" log entry), so it is really the last packet of the stream. On failure, the `exception` packet is the last packet instead, and the final-counters `progress` packet is not written at all - it is the success terminator of the stream - even when the failure happens after the query itself finished and the final counters were already known (for example, a failure while writing the query log).

Because this tail of the stream is written after the `QueryFinish` entry of `system.query_log` is recorded, the network-send profile events of the query (`NetworkSendBytes`, `NetworkSendElapsedMicroseconds`) do not include sending the trailing packets and closing the response. With full response buffering (`http_wait_end_of_query=1`, or the deprecated `wait_end_of_query=1` URL parameter), the buffered response body is also transmitted only after the query has finished, so its transmission is excluded. Setting `http_response_buffer_size` alone allows the response to spill to the network when the memory buffer fills; sends during execution still contribute to the query's counters. The native protocol likewise sends its trailing logs, profile events, and profile traces after the query log entry.

Logs and profile events enabled only through the query's `SETTINGS` clause, including a framing format enabled there, start capturing after analysis. Their parse, plan, and analysis entries require session or URL settings. In particular, a query that fails during analysis (before pipeline execution) - for example a reference to an unknown table - and enables `send_logs_level` only in its `SETTINGS` clause does not deliver the analysis-phase logs. Set `send_logs_level` on the session or the URL to capture those. Profile traces start capturing immediately after the SQL settings are applied, before subsequent analysis and synchronous interpreter work. Samples emitted before those settings are applied require both framing and trace delivery to be enabled through the session or URL.
...
The default. Transparently routes everything applicable (data, totals, extremes, progress) to the output format, and ignores everything that is not applicable (metrics, profile traces, logs). So everything works as it is by default, including formats that represent progress themselves, such as `JSONEachRowWithProgress`.
...
Server-sent events is a text protocol that treats line breaks (including carriage returns, `\r`) as field delimiters, so the bytes produced by the output format are not embedded verbatim: a block of formatted data is base64-encoded into a single `data:` field, which decodes to the fully formatted payload, with all of its newlines. This is what the `payload=base64` parameter of the `Content-Type` says. After decoding, reconstruct the unframed output using the packet-selection rule described above: concatenate only `data` payloads for the `JSONCompactEachRow` family, and `data`, `totals`, and `extremes` payloads for other supported output formats. This preserves the original bytes for text, binary (`Native`, `RowBinary`), and raw passthrough (`RawBLOB`, `TSVRaw`) formats.

The auxiliary JSON packets (`progress`, `log`, `profile_events`, `profile_traces`, `exception`) are never encoded: they are written as a single `data:` field of JSON, which contains no line breaks.
...
| `profile_traces` | An array of sampled stack traces as JSON: `host_name`, `query_id`, `trace_type`, `thread_id`, `event_time_microseconds`, `trace`, `symbols`, `size`. |
| `exception`      | The exception message as JSON.                                                                                   |

Unlike the `data`, `totals`, and `extremes` payloads (see the byte-exactness notes above), the string fields of the auxiliary packets (`query_id`, `text`, and `source` of `log`, `name` of `profile_events`, `host_name`, `query_id`, `trace_type`, and `symbols` of `profile_traces`, and the `exception` message) have no base64 escape hatch, and some of them (for example `query_id`, which is taken from the query) can hold arbitrary bytes. These fields are always sanitized to valid UTF-8, replacing invalid sequences with the replacement character (`U+FFFD`), so the auxiliary packets are always valid JSON.
...
## Profile trace packets {#framing-format-profile-traces}

Set `send_profile_traces=1` to receive samples from the query while it runs, without reading, flushing, or granting access to `system.trace_log`. The server's trace collector must be enabled in the server configuration. The setting uses the existing sampling controls: `query_profiler_cpu_time_period_ns`, `query_profiler_real_time_period_ns`, `memory_profiler_step`, and `memory_profiler_sample_probability`. Enabling delivery does not change the sampling configuration. Delivery is best effort: queues and packet batches are bounded, so samples can be dropped when the sampler or client cannot keep up. A batch contains at most 1024 samples. The final drain waits at most ten seconds for the collector; on timeout, remaining samples are discarded and a server warning is logged, while the original query result or exception is preserved. Disabling delivery discards queued samples without waiting for the collector. Only `CPU`, `Real`, `Memory`, `MemorySample`, and `MemoryPeak` samples are streamed. Other trace types, including `ProfileEvent` and `JemallocSample`, remain available through `system.trace_log` under their own collection settings.

With full HTTP response buffering (`http_wait_end_of_query=1`, or the deprecated `wait_end_of_query=1` URL parameter), samples remain in the bounded queue until the final drain. If a query exception replaces the buffered response, the replacement drains the retained samples before its `exception` packet. Long queries can fill the queue while the response is buffered; these losses are reported through `Dropped` rows.

Samples collected while draining or serializing profile trace packets are excluded from the stream. `CPU` and `Real` samples from that work can still be collected in `system.trace_log`.

In `JSONEachPacketBase64` and `JSONEachPacketString`, each batch has the following envelope:

```json
{"packet":"profile_traces","profile_traces":[{"host_name":"localhost","query_id":"example","trace_type":"CPU","thread_id":"123","event_time_microseconds":"1783814400000000","trace":["123456","789012"],"symbols":["DB::someFunction",""],"size":"0"}]}
```

In `EventStream`, the event name is `profile_traces` and its single `data:` field contains the same JSON array, without the envelope or base64 encoding.

| Field | Meaning |
|-------|---------|
| `host_name` | Host where the sample was collected. |
| `query_id` | Query identifier on that host. |
| `trace_type` | Sample kind: `CPU`, `Real`, `Memory`, `MemorySample`, or `MemoryPeak`; delivery status: `Dropped` or `Incomplete`. |
| `thread_id` | Sampled thread identifier, encoded as a decimal string. |
| `event_time_microseconds` | Sample collection time in microseconds since the Unix epoch, encoded as a decimal string. |
| `trace` | Stack frame addresses, innermost frame first, encoded as decimal strings using the same normalization as `system.trace_log`. |
| `symbols` | Server-resolved symbol names in the same order and with the same length as `trace`; an unresolved frame has an empty string. |
| `size` | Allocation size for `Memory`; signed allocation/deallocation size for `MemorySample`; total tracked usage at a new peak for `MemoryPeak` (all in bytes); zero for `CPU`, `Real`, and `Incomplete`; positive loss delta for `Dropped`. Encoded as a decimal string. |

All 64-bit integers use strings to preserve precision in JavaScript. All string fields, including symbol names and query identifiers, are sanitized to valid UTF-8 using the replacement character (`U+FFFD`) for invalid sequences.

Delivery status rows use empty `trace` and `symbols` arrays and zero thread/time fields. `Dropped` carries a positive delta of reported server queue losses in `size`, including samples discarded on collector timeout. Sum these deltas across the response, independently of any host filter. Each delta saturates at the maximum `Int64`, which means at least that many losses. `Incomplete` uses `size = "0"` and marks a collector flush timeout: further losses are unknown. Status rows count towards the 1024-row batch limit and may appear without any samples. They must not be added to a flame graph.

Coordinators aggregate forwarded loss deltas once, even when their sample queues are full. Status host/query fields identify the reporting coordinator, not where the losses originated. The count excludes losses in the signal-safe pipe before the collector and metadata discarded by older intermediaries. Absence of a status row does not guarantee completeness. Clients should ignore unrecognized kinds. Actual stack samples retain their producing host and query identifiers across multiple forwarding hops, and symbols are resolved on the producing host.

When `send_profile_traces` or framing is enabled only in the query's `SETTINGS` clause, capture starts immediately after those settings are applied, before subsequent analysis and synchronous interpreter work such as `BACKUP` or `RESTORE`. Set both on the session or URL to capture earlier samples as well. A query-level `send_profile_traces=0` disables delivery even when the session or URL enables it. With `framing_output_format=None`, HTTP responses contain no profile trace packets.

Remote queries request profile trace packets only when delivery is active for the coordinator's client. Plain HTTP responses and disabled delivery therefore also disable requests for remote trace packets. Remote SQL opt-ins cannot override this, while explicit remote SQL opt-outs remain effective. Remote `INSERT` uploads through `RemoteInserter` disable delivery; distributed `SELECT` execution can forward remote samples.

On the [native protocol](/reference/interfaces/specs/NativeProtocol#profiletraces), trace delivery is deferred while external tables, scalars, `INSERT` rows, or `input` rows are being uploaded. Samples stay in the bounded queue until that upload's input terminator. An exception or query completion before the terminator, including detached-query completion, discards those samples without waiting for the collector or sending trace or loss-status packets; the server sends the original terminal response before consuming any remaining input.
