...
Anything a query enables only through its own `SETTINGS` clause - a framing format, `send_logs_level`, `send_profile_events`, or `send_profile_traces` - is not known until the query has been parsed, so the corresponding logs, profile events, and profile traces are captured only from query execution onwards. The logs, profile events, and profile traces of the parse, plan, and analysis phase are captured only when the setting comes from the session or the URL. In particular, a query that fails during analysis (before pipeline execution) - for example a reference to an unknown table - and enables `send_logs_level` only in its `SETTINGS` clause does not deliver the analysis-phase logs. Set `send_logs_level` on the session or the URL to capture those.
...
When `send_profile_traces` or framing is enabled only in the query's `SETTINGS` clause, capture starts after analysis, when the framing queues are reconciled with those settings. Set both on the session or URL to capture earlier samples as well. A query-level `send_profile_traces=0` disables delivery even when the session or URL enables it. With `framing_output_format=None`, HTTP responses contain no profile trace packets.
