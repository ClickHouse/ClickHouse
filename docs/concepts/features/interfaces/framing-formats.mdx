...
When the experimental setting `query_result_previews` is enabled, the stream may also carry `preview` packets with real-time previews of the query result. Each `preview` packet contains a complete, self-contained document of the output format (including the format prefix and suffix) with a small intermediate result, and fully replaces the previous preview. Previews are additional content: they are not part of the byte-exact concatenation invariant above, and a client that reconstructs the unframed output must skip them. A `preview` packet with an empty payload says the intermediate result is empty at the moment (for example, `HAVING` filtered every row out) and replaces the previous preview as well. Previews stop before the first `data` packet of the real result.
...
| `preview`        | Bytes produced by the output format for a real-time preview of the query result (the `query_result_previews` setting): a complete, self-contained document of the format, replacing the previous preview. |
