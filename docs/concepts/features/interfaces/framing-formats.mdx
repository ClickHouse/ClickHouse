...
Remote queries request profile trace packets only when delivery is active for the coordinator's client. Plain HTTP responses and disabled delivery therefore also disable requests for remote trace packets. Remote SQL opt-ins cannot override this, while explicit remote SQL opt-outs remain effective.
