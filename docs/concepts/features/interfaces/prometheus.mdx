...
Every shard-local table behind such a `Distributed` table must be a `TimeSeries` table declaring the same `time_series` type as the `Distributed` table itself; a mismatch is refused rather than rounded, and the check asks every replica of every shard, once a minute per cluster and table. A read is not failed by a replica the check cannot reach or that has no table of that name: whether the read itself fails is decided, as before, by its own `skip_unavailable_shards` settings, and a verdict that missed a replica is not kept, so that replica is checked again on the next request. Remote write, however, is refused (with a retryable `5xx`) while any replica is unreachable or has no table of that name, because the samples the sink would queue for that replica are delivered later, into whatever answers to the name by then, without any such check. Reading through it needs `SELECT` on the `Distributed` table, `CREATE TEMPORARY TABLE` and `READ ON REMOTE`, because the query is rewritten into a `cluster()` call; writing needs `INSERT` on the `Distributed` table only.

Remote write into a `Distributed` target is an ordinary `INSERT` through that table's sink, so its acknowledgement means whatever the table is configured to mean: by default the samples are durably queued on the initiator and retried in the background, and with [`distributed_foreground_insert`](/reference/settings/session-settings/distributed#distributed_foreground_insert) the response is held until every shard has accepted them. Set it on the handler's user profile or on the `remote_write` URL if a `204` should mean the samples already reached the shards; note that `skip_unavailable_shards` weakens that, since a shard can then be dropped from the write. The routing settings `insert_shard_id` and `insert_distributed_one_random_shard` are refused on this path, whether they come from the URL or from the profile: a batch goes where the table's sharding key sends it, so a `Distributed` table with several shards and no key cannot be written through remote write.
