...
Remote Write 2.0 support is float-only. ClickHouse stores float samples and metric metadata. ClickHouse drops native histograms from mixed requests. Prometheus `send_native_histograms` is a no-op for `io.prometheus.write.v2.Request`, so senders cannot omit those histograms. ClickHouse rejects the whole request when exemplars are present. Senders can set `send_exemplars: false`.
