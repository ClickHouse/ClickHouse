...
- It asks each replica with `SHOW CREATE TABLE` and `DESC TABLE`, which need only `SHOW COLUMNS` on that table, a right any grant on it implies, so it reads nothing of `system` on the shard and a row policy or filter there cannot blind it.
...
The grants such a table asks for, each checked before the shard-local check above runs, so a caller without one is refused with a plain access error rather than told what the shards hold:
...
- Writing needs `INSERT` on the `Distributed` table for the columns the request names: `metric_name`, `tags` and the samples column, and the four metadata columns as well when the request brings metadata. Whenever the cluster has a shard that is this server itself, it also needs `INSERT` on that shard's own table for every column the sink sends, which is every column the `Distributed` table declares, whether or not the batch routes any sample there.
