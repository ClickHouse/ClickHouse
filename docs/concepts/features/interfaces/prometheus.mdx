...
PromQL reads a `TimeSeries` table through its selector rather than as an ordinary table, so a row policy or an `additional_table_filters` entry attached to the table does not apply to it. This is how a single-node `TimeSeries` table already behaves, and a `Distributed` target behaves the same way. The `/api/v1/series`, `/api/v1/labels`, `/api/v1/label/<name>/values` and `/api/v1/metadata` endpoints, which read the inner tables directly, refuse a table that has either instead of answering past it. Restrict the data a user can read through the inner tables instead.
