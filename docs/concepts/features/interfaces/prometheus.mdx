...
Every shard-local table behind such a `Distributed` table must be a `TimeSeries` table declaring the same `time_series` type as the `Distributed` table itself; a mismatch is refused rather than rounded, and the check runs against the shards once a minute per cluster and table. It judges only the shards it can reach: whether an unreachable shard fails the request is decided, as before, by the read or the write itself (a shard that was unreachable is checked again on the next request once it answers). Reading through it needs `SELECT` on the `Distributed` table, `CREATE TEMPORARY TABLE` and `READ ON REMOTE`, because the query is rewritten into a `cluster()` call; writing needs `INSERT` on the `Distributed` table only.
