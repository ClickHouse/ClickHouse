...
| `table` | none | The name of a `TimeSeries` table. When omitted, all Prometheus API requests except `/api/v1/format_query` and `/api/v1/status/buildinfo` must provide the `table` query parameter. The configured name can include a database. |
