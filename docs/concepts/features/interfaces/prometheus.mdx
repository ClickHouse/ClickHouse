...
The optional `limit` parameter caps vector and matrix responses to the specified number of series. A value of `0` disables the cap, scalar and string responses are unchanged, and a truncated response includes a warning; the order of returned series is not guaranteed.
