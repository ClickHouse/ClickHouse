...
Every shard-local table behind such a `Distributed` table must be a `TimeSeries` table declaring the same `time_series` type as the `Distributed` table itself; a mismatch is refused rather than rounded, and the check asks every replica of every shard, once a minute per cluster and table. A read is not failed by a replica the check cannot reach: whether the read itself fails is decided, as before, by its own `skip_unavailable_shards` settings, and a verdict that missed a replica is not kept, so that replica is checked again on the next request once it answers. Remote write, however, is refused (with a retryable `5xx`) while any replica is unreachable, because the samples the sink would queue for that replica are delivered later without any such check. Reading through it needs `SELECT` on the `Distributed` table, `CREATE TEMPORARY TABLE` and `READ ON REMOTE`, because the query is rewritten into a `cluster()` call; writing needs `INSERT` on the `Distributed` table only.
