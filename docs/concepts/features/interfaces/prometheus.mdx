...
To expose ClickHouse's own metrics for a Prometheus server to scrape, see the [Prometheus metrics endpoint](/interfaces/prometheus-metrics).

## Prerequisites {#prerequisites}

<Note>
ClickHouse Cloud services already have the `enable_time_series_table` setting and the Prometheus API endpoints configured. On a Cloud service, start at [Create a TimeSeries table](#create-a-timeseries-table).
</Note>
