...
The authenticated user must have the `INSERT` privilege on the target table. The privilege is checked before the table is looked up, so a user without it gets `ACCESS_DENIED` regardless of whether the table exists or has opted in.
