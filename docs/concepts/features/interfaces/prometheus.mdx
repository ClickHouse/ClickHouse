...
ClickHouse implements the Prometheus HTTP API over a [`TimeSeries`](/reference/engines/table-engines/integrations/time-series) table. One handler serves remote write, remote read, instant PromQL queries, range PromQL queries, and parser-only query formatting.
...
The example omits `database` and `table` from the handler. Requests to the query endpoints must provide the `table` query parameter. They can also provide `database`, use a qualified table name such as `prometheus.metrics`, or omit the database to use `default`. This allows one handler to serve multiple `TimeSeries` tables. The `/format_query` endpoint only parses and canonicalizes the PromQL expression, so it does not require a `TimeSeries` table.
