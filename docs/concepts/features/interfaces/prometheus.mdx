...
PromQL reads a single-node `TimeSeries` table through its selector rather than as an ordinary table, so a row policy or an `additional_table_filters` entry attached to that table does not apply to it; restrict the data a user can read through the inner tables instead. Over a `Distributed` target the read is rewritten to the shard-local `TimeSeries` tables, and a row policy on the `Distributed` table or an `additional_table_filters` entry for it or for the shard-local table, which a plain `SELECT` through the `Distributed` table applies, would not follow it: such a PromQL query is refused rather than answered past them. The `/api/v1/series`, `/api/v1/labels`, `/api/v1/label/<name>/values` and `/api/v1/metadata` endpoints, which read the inner tables directly, refuse any table that has either. A filter that is a literal true restricts nothing and is not refused.
