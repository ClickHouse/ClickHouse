...
ClickHouse rejects the complete request when native histograms or exemplars are present.
Prometheus `send_native_histograms` is a no-op for `io.prometheus.write.v2.Request`, so senders cannot use it to omit native histograms.
...
The rule removes the complete series, not only the histogram samples.
