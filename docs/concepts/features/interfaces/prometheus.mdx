...
For a `prometheus_api_v1` handler, URL-path routing applies only to `/write`. The read and query endpoints keep using a fixed table in the handler configuration or the `database` and `table` query parameters.
