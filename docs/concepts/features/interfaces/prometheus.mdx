...
| `enable_table_name_url_routing` | false | Takes the database and the table from the request path instead of the query parameters. Supported for the `prometheus_write` handler and for the remote-write endpoint of a `prometheus_api_v1` handler. Cannot be combined with a configured `database` or `table`. See [Route remote write by URL path](#remote-write-url-routing). |
...
#### Route remote write by URL path {#remote-write-url-routing}

One remote-write handler can ingest into many `TimeSeries` tables without any query parameter, by taking the target from the request path. Enable `enable_table_name_url_routing` in the handler and match a URL that carries the database and the table:
...
The path must have exactly the shape `/{database}/{table}/write`, or `/{database}/{table}/api/v1/write` for a `prometheus_api_v1` handler. A handler with `enable_table_name_url_routing` cannot also configure `database` or `table`, and a request to such a handler cannot pass the `database` or `table` query parameters.

Each target table must opt in with the [`prometheus_remote_write_dynamic_routing_enabled`](/reference/engines/table-engines/integrations/time-series) setting:
...
Prometheus then writes to a table chosen by the URL:

```yaml
remote_write:
  - url: https://clickhouse.example.com:8443/prometheus/metrics/write
    basic_auth:
      username: default
      password: <password>
```
