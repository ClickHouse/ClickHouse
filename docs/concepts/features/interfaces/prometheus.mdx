...
For a `TimeSeries` table with a [histograms table](/reference/engines/table-engines/integrations/time-series#histograms-table), remote read returns the [native histograms](#remote-write-native-histograms) of a time series too, exactly as they were written.
