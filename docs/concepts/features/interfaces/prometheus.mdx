...
<Note>
PromQL support in ClickHouse Cloud is in private preview. The `enable_time_series_table` setting and the Prometheus API endpoints are configured for the services that take part in the private preview, not for every ClickHouse Cloud service.
</Note>
