...
This documentation website embeds the same terminal as a drop-down panel, connected to the [ClickHouse playground](https://play.clickhouse.com) as the read-only `play` user, so that the examples on any page can be tried out without leaving it. Open and close it with the terminal icon at the left of the top navigation, or with the `~` key; pressing `Escape` inside the terminal closes the panel as well. Drag the bottom edge of the panel to resize it, and drag it all the way up to close it.
