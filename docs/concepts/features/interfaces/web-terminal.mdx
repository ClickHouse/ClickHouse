...
This documentation website embeds the same terminal as a panel that drops down from the top of the page, connected to the [ClickHouse playground](https://play.clickhouse.com) as the read-only `play` user, so that the examples on any page can be tried out without leaving it. Open it with the terminal icon at the left of the top navigation, or with the `~` key. The panel covers the navigation bar while it is open, so it closes with the `~` key, with `Escape`, with the close button in its top-right corner, or by dragging its bottom edge all the way up; that bottom edge also resizes it. Ending the session — `exit` or `Ctrl+D` — closes the panel too.
...
The panel is part of the desktop layout of the website, and is not available on narrow viewports where the top navigation is collapsed.
