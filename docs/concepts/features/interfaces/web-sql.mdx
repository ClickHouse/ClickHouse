...
A tab can be turned into a notebook — an ordered sequence of query and text cells — by adding a
cell with the `[+]` button that appears under a result. Notebook mode is per tab. See
[Web UI Notebook Mode](/concepts/features/interfaces/web-ui-notebook).

### Limitations {#limitations}

A query keeps running when you switch away from its tab. Its progress and resource meters keep
accumulating in the background, and reopening the tab re-adopts that live state, so the meters
continue from their current values instead of restarting near zero. Closing the tab, and navigating
back or forward to another history entry, do stop the query.
