...
To enable the PostgreSQL wire protocol, add the [postgresql_port](/reference/settings/server-settings/settings/postgresql#postgresql_port) setting to your server's configuration file. For example, you could define the port in a new XML file in your `config.d` folder:
