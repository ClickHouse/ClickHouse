...
The HTTP interface can interpret a URL path as a database, table, output format, and compression method. URL parameters can then shape the result, so simple read requests do not require SQL. These features are enabled by default since ClickHouse 26.10; in 26.8 and 26.9 they had to be enabled explicitly.

### Path routing settings {#enabling-path-routing}

Path interpretation is gated at two levels:

1. The server-level `http_allow_path_requests` configuration setting allows the HTTP interface to route path-style requests to the query handler. It is enabled by default; set it to `0` to make every path that is not a configured handler return `404` again:
...
2. The per-user settings below decide how a routed path is interpreted. All of them except `http_allow_filters_as_unrecognized_url_parameters` are enabled by default, and each can be enabled or disabled per user, role, or profile:
...
| `http_allow_filters_as_unrecognized_url_parameters` | Interprets unrecognized URL parameters as `WHERE` filters. Disabled by default. |

<Note>
Routing occurs before authentication, so it cannot depend on a per-user setting. When `http_allow_path_requests` is disabled, an unknown path returns a pre-authentication `404`. After routing and authentication, the per-user settings determine how the path is interpreted, allowing you to enable or disable the feature selectively for a user, role, or profile.
</Note>

<Note>
`http_allow_filters_as_unrecognized_url_parameters` stays disabled by default because it applies to every request of the HTTP interface, including the `/?query=...` endpoint: when it is enabled, any unrecognized URL parameter a client appends becomes a filter of the query. A misspelt setting name is then no longer reported as `UNKNOWN_SETTING`, but fails with `UNKNOWN_IDENTIFIER` — or, when the name happens to match a column of the queried table, silently filters the result.
</Note>

### Access tables as files {#table-as-file-examples}

With `http_allow_table_as_file` enabled, a request to `/table.format.compression` is processed as `SELECT * FROM table`. `http_allow_database_as_path` additionally allows `/database/table.format.compression`.
