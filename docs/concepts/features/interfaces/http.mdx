...
An explicit `format`, `output_format`, or `default_format` URL parameter overrides the format in the path. An explicit `compression` parameter must match the compression in the path; conflicting values result in an exception.
...
| `default_format` | Sets the output format when the query has no `FORMAT` clause and no other format override. It can also be set using the `X-ClickHouse-Format` header. |
