...
"Ok." is the default value defined in [`http_server_default_response`](/reference/settings/server-settings/settings/http#http_server_default_response) and can be changed if desired.
...
<Image img="/images/play.webp" size="md" alt="ClickHouse Web UI screenshot" />
...
## Access tables through URL paths and construct queries {#tables-as-files}

Available in ClickHouse 26.8 and later.

The HTTP interface can interpret a URL path as a database, table, output format, and compression method. URL parameters can then shape the result, so simple read requests do not require SQL. These features are disabled by default and must be enabled explicitly.

### Enable path routing {#enabling-path-routing}

Path interpretation is gated at two levels:

1. Enable the server-level `http_allow_path_requests` configuration setting, which allows the HTTP interface to route path-style requests to the query handler:
...
2. Enable the required per-user settings:
...
| `http_allow_filters_as_path` | Interprets Hive-style `/name=value/` path components as `WHERE` filters. |
| `http_allow_filters_as_unrecognized_url_parameters` | Interprets unrecognized URL parameters as `WHERE` filters. |

<Note>
Routing occurs before authentication, so it cannot depend on a per-user setting. When `http_allow_path_requests` is disabled, an unknown path returns a pre-authentication `404`. After routing and authentication, the per-user settings determine how the path is interpreted, allowing you to enable the feature selectively for a user, role, or profile.
</Note>

### Access tables as files {#table-as-file-examples}

With `http_allow_table_as_file` enabled, a request to `/table.format.compression` is processed as `SELECT * FROM table`. Enable `http_allow_database_as_path` to use `/database/table.format.compression`.
...
Format and compression extensions are recognized case-insensitively, so `hits.csv` and `hits.CSV` are equivalent.

An explicit `format` or `output_format` URL parameter overrides the format in the path. `default_format` does not: it only provides the format to use when nothing else selects one, and the path extension is more specific. An explicit `compression` parameter must match the compression in the path; conflicting values result in an exception.

### Construct a query {#query-construction}

The following settings wrap the base query as a derived table. They compose with one another and with an existing query. You can supply them as HTTP URL parameters, in an in-query `SETTINGS` clause, or through a user profile.

| Setting | Effect |
|---------|--------|
| `select` | `SELECT <expression_list> FROM (…)` |
| `filter` | Adds `WHERE <expression>`. Multiple `filter` URL parameters are combined with `AND`. |
| `order` | Adds `ORDER BY <expression_list>`. |
| `sort` | Adds `ORDER BY` from a comma-separated list of identifiers or column positions, each with an optional `+` (ascending) or `-` (descending) prefix, such as `sort=a,-b`. It cannot be combined with `order`. |
| [`limit`](/reference/settings/session-settings/other#limit) and [`offset`](/reference/settings/session-settings/other#offset) | Add `LIMIT <n> OFFSET <m>`. Both accept negative values for tail selection and fractional values representing a share of the rows. |
| `page` | Sets `offset = limit * (page - 1)`. It requires `limit` and cannot be combined with `offset`. |
...
#### Modify an existing query {#modify-an-existing-query}

The construction settings are not limited to path requests: they also shape a query you write yourself. The query becomes the derived table, so the settings apply to its result rather than being merged into it. For example, `?query=SELECT a, b FROM hits&filter=a > 0&sort=-b&limit=10` is executed as:

```sql
SELECT * FROM (SELECT a, b FROM hits) WHERE a > 0 ORDER BY b DESC LIMIT 10
```

This is useful for paging through, sorting, or narrowing the result of a query that is fixed elsewhere — in a saved dashboard query, a `predefined_query_handler`, or a client that only lets you append URL parameters:
...
Because the settings wrap the query instead of rewriting it, they compose with a `FORMAT` clause and with an `ORDER BY` inside the query, and they never change which rows an `INSERT` writes: a construction setting placed on an `INSERT ... SELECT` statement shapes the statement's own (empty) result, not its source `SELECT`. To shape the source, put the setting in the `SELECT`'s own `SETTINGS` clause:
...
<Warning>
`filter` is not an access-control mechanism. It adds a `WHERE` clause over a wrapping subquery, so the underlying data can be read and processed before the filter is applied. To restrict which rows a user can access, use [row policies](/concepts/features/security/access-rights#row-policy-management) or [`additional_table_filters`](/reference/settings/session-settings/additional#additional_table_filters).
</Warning>

### Override formats and compression {#format-and-compression-overrides}

| Setting | Effect |
|---------|--------|
| `output_format` | Overrides the output format. It takes precedence over the query's `FORMAT` clause, the path extension, `format`, and `default_format`. It can also be set using the `X-ClickHouse-Format` header. |
| `input_format` | Overrides the input format for `INSERT`. It takes precedence over the query's `FORMAT` clause and `format`. |
| `format` | Overrides the format in both directions unless a direction-specific setting is provided. |
| `default_format` | Sets the output format when the query has no `FORMAT` clause, no path extension, and no other format override. |
| `compression` | Compresses the response body, for example with `compression=gz`. This is independent of HTTP `Content-Encoding` and the ClickHouse-native `compress` parameter. |

The format settings also work with other protocols, including the native client and `clickhouse-local`. `input_format` and `output_format` correspond to the `--input-format` and `--output-format` options. The `--format` option maps to the bidirectional `format` setting in `clickhouse-local`, while in `clickhouse-client` it keeps its historical output-only meaning and maps to `output_format`.

`compression` is specific to HTTP response shaping and is consumed before query execution. Supply it through an HTTP URL parameter, a path extension, or a user profile. It is rejected in an in-query `SETTINGS` clause.

Binary and compressed HTTP responses include a `Content-Disposition: attachment; filename=…` header. The filename is derived from the URL path, or uses `result.<format>.<compression>` when the path does not provide one.

### Use a path table with a query {#implicit-table}

When the path identifies a table and you also provide a `query` parameter, the path table is exposed through [`implicit_table_at_top_level`](/reference/settings/session-settings/implicit#implicit_table_at_top_level). A `SELECT` without a `FROM` clause reads from the path table:
...
If the query already contains a `FROM` clause, the path component only supplies the download filename.
...
To increase the efficiency of data insertion, disable server-side checksum verification by using the [`http_native_compression_disable_checksumming_on_decompress`](/reference/settings/session-settings/http#http_native_compression_disable_checksumming_on_decompress) setting.
...
You can configure the data compression level using the [`http_zlib_compression_level`](/reference/settings/session-settings/http#http_zlib_compression_level) setting for all compression methods.
...
You can receive information about the progress of a query in the `X-ClickHouse-Progress` response headers. To do this, enable [`send_progress_in_http_headers`](/reference/settings/session-settings/send#send_progress_in_http_headers).
...
|-----------------------|-------------------------------------------|
| `query_id` (optional) | Can be passed as the query ID (any string). [`replace_running_query`](/reference/settings/session-settings/replace-running-query#replace_running_query)|
...
- [`http_response_buffer_size`](/reference/settings/session-settings/http-response#http_response_buffer_size)
- [`http_wait_end_of_query`](/reference/settings/session-settings/http#http_wait_end_of_query)
...
You can mitigate this problem by enabling `wait_end_of_query=1` ([Response Buffering](#response-buffering)). In this case, sending of the HTTP header is delayed until the entire query is resolved. This however, doesn't completely solve the problem because the result must still fit within the [`http_response_buffer_size`](/reference/settings/session-settings/http-response#http_response_buffer_size), and other settings like [`send_progress_in_http_headers`](/reference/settings/session-settings/send#send_progress_in_http_headers) can interfere with the delay of the header.
...
You can create a query with parameters and pass values for them from the corresponding HTTP request parameters. For more information, see [Queries with Parameters for CLI](/concepts/features/interfaces/client#cli-queries-with-parameters).
...
The following example defines the values of [`max_threads`](/reference/settings/session-settings/max-threads#max_threads) and [`max_final_threads`](/reference/settings/session-settings/max#max_final_threads) settings, then queries the system table to check whether these settings were set successfully.
...
To experiment with this functionality, the following example defines the values of [`max_threads`](/reference/settings/session-settings/max-threads#max_threads) and `max_final_threads` and `queries` whether the settings were set successfully.
...
## Structured JSON exceptions {#structured-json-exceptions}

Send `X-ClickHouse-Exception-Format: JSON` to receive errors as JSON objects. Successful query results retain their requested output format. Without this header, errors retain their existing representation.

```bash
curl -sS 'http://localhost:8123/?query_id=a74d4967-7461-4012-8e68-4c93535ac1da&wait_end_of_query=1' \
  -H 'X-ClickHouse-Exception-Format: JSON' \
  --data-binary 'SELECT val2 FROM test_play'
```

For example, when `test_play` exists but has no `val2` column:
...
```json
{
  ...
  "query_id": "a74d4967-7461-4012-8e68-4c93535ac1da",
  "query": "SELECT val2 FROM test_play",
  "formatted_message": "Code: 47. DB::Exception: ... (UNKNOWN_IDENTIFIER)"
}
```

`formatted_message` contains the complete exception text that the plain-text response would contain, without the response's trailing newline. The JSON object has exactly these six fields. It has no `has_nested` or separate `stack_trace` field. The existing `stacktrace=1` parameter includes a stack trace inside `formatted_message` when server configuration permits it.

`query` preserves the submitted SQL, including whitespace and parameter placeholders, independently of query-log truncation and formatting. As in normal HTTP query processing, SQL in the URL query parameter and request body is joined by a newline. Data following a parsed `INSERT` statement is excluded. Invalid UTF-8 is replaced so the response remains valid JSON. If the request fails before SQL is read, the query is empty or contains only the available URL query; if SQL exceeds the query size limit, only the prefix read for parsing is available. Query identifiers are strings and may be supplied by the caller; otherwise ClickHouse generates a UUID. An error before query-context creation has an empty identifier.

The response advertises `X-ClickHouse-Exception-Format: JSON`. A standalone error uses `Content-Type: application/json; charset=UTF-8` and the usual HTTP error status. Structured errors take precedence over `http_write_exception_in_output_format`.

Once result transmission starts, HTTP status and content type cannot be changed. An unframed response uses the existing tagged exception block, with JSON as its message, and an incomplete HTTP transfer. If the complete object cannot fit in that block's 16 KiB limit, the transfer is aborted; JSON is never truncated. With `framing_output_format=JSONEachPacketString`, `JSONEachPacketBase64`, or `EventStream`, the terminal packet's `exception` value is the JSON object. Use `wait_end_of_query=1` and sufficient buffering when a standalone JSON error is required.
...
To prevent it, you can use setting [`http_write_exception_in_output_format`](/reference/settings/session-settings/http#http_write_exception_in_output_format) (disabled by default) that will tell ClickHouse to write an exception in specified format (currently supported for XML and JSON* formats).
