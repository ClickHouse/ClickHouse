...
### Upload data to a path table {#path-table-inserts}

An HTTP `PUT` request to `/database/table.format` inserts the request body into an existing table. The path extension supplies the input format by default. As with a regular `INSERT`, the `input_format` setting takes precedence over the path format, followed by the bidirectional `format` setting. The request must contain at least one decoded byte and must not include a `query` parameter. This is useful when a file name already contains both the target table and the input format:

```bash
printf '1,"one"\n2,"two"\n' \
  | curl -X PUT -H 'Content-Type: text/csv' --data-binary @- \
      'http://localhost:8123/my_db/hits.CSV'

printf '{"a":3,"b":"three"}\n' \
  | curl -X PUT -H 'Content-Type: application/json' --data-binary @- \
      'http://localhost:8123/my_db/hits.JSONEachRow'
```

The table must already exist. For a database-prefixed upload, `http_allow_database_as_path` enables the
`/database/table.format` path. An unqualified `/table.format` upload still requires
`http_allow_table_as_file`. The request body may use a compression suffix such as `.CSV.gz` or `.CSV.snappy`; the server
decompresses it before inserting the rows. For `.snappy` path suffixes, the `snappy_mode` setting selects the
Snappy wire format for both path reads and path uploads. When `Content-Encoding: snappy` is also present, the HTTP
header selects the framed format and takes precedence over `snappy_mode`. Path-table `PUT` uploads use normal `INSERT` semantics, so repeating
the same request can insert the rows again. Auto-creation, schema inference, and multipart uploads are not
supported. Use the regular [`INSERT` over HTTP](#insert-queries) form when the query or upload behavior needs
to be customized.

If the table name contains characters used by path filters, use a backquoted identifier followed by the format
suffix. For example, `/my_db/%60a%3D1%60.CSV` uploads to the table named `` `a=1` `` even when
`http_allow_filters_as_path` is enabled. The final path component is percent-decoded after path splitting, so an
encoded format dot such as `hits%2ECSV` is also accepted.

```bash
# The path suffix describes the compressed request body.
gzip -c rows.csv \
  | curl -X PUT --data-binary @- \
      'http://localhost:8123/my_db/hits.CSV.gz'
```

## Insert queries over HTTP/HTTPS {#insert-queries}

The traditional `POST` method of transmitting data is useful when the query needs to be supplied explicitly. You can write the beginning of the query in the URL parameter, and use POST to pass the data to insert. The data to insert could be, for example, a tab-separated dump from MySQL. In this way, the `INSERT` query replaces `LOAD DATA LOCAL INFILE` from MySQL.
