...
An HTTP `PUT` request to `/database/table.format` inserts the request body into an existing table. The format is taken from the path extension, and the request must not include a `query` parameter. This is useful when a file name already contains both the target table and the input format:
...
decompresses it before inserting the rows. Auto-creation, schema inference, and multipart uploads are not
