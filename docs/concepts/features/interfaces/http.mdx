...
You can use the `default_format` URL parameter to specify a default format other than `TabSeparated`. The `X-ClickHouse-Format` header selects the format explicitly: it is an alias for the `format` setting, so it also overrides a `FORMAT` clause in the query.
...
| Setting | Effect |
|---------|--------|
| `output_format` | Overrides the output format. It takes precedence over the query's `FORMAT` clause, the path extension, `format`, and `default_format`. |
| `input_format` | Overrides the input format for `INSERT`. It takes precedence over the query's `FORMAT` clause and `format`. |
| `format` | Overrides the format in both directions unless a direction-specific setting is provided. It can also be set using the `X-ClickHouse-Format` header. |
...
The format settings also work with other protocols, including the native client and `clickhouse-local`, where they correspond to the `--format`, `--input-format`, and `--output-format` options.
