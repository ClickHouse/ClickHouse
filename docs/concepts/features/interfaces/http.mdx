...
2. The per-user settings below decide how a routed path is interpreted. All of them are enabled by default and can be disabled per user, role, or profile:
...
<Note>
Routing occurs before authentication, so it cannot depend on a per-user setting. When `http_allow_path_requests` is disabled, an unknown path returns a pre-authentication `404`. After routing and authentication, the per-user settings determine how the path is interpreted, allowing you to disable the feature selectively for a user, role, or profile.
</Note>

<Note>
Because `http_allow_filters_as_unrecognized_url_parameters` is enabled, a misspelt setting name in the URL is no longer reported as `UNKNOWN_SETTING`. It becomes a filter instead, so the query fails with `UNKNOWN_IDENTIFIER` — or, when the misspelt name happens to match a column of the queried table, silently filters the result. Disable the setting to get the stricter diagnostics back.
</Note>
