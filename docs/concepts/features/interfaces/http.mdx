...
The table must already exist. Auto-creation, schema inference, multipart uploads, and compression suffixes for `PUT` uploads are not supported. Use the regular [`INSERT` over HTTP](#insert-queries) form when the query or upload behavior needs to be customized.
