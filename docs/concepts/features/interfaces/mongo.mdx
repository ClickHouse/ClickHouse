...
A `find` supports a filter, a projection, `limit`, `skip` and `sort`. A projection may compute `$add`, `$sub`, `$mul` and `$div`. A `count` supports a `query` filter and the `limit` and `skip` bounds. A `listCollections` and a `listDatabases` support a `filter` on the `name` - by equality, `$eq`, `$in` or a regular expression - and `nameOnly`; a filter on another field or with another operator is an error rather than an unfiltered listing.
...
A collection created explicitly with `createCollection` has no document to infer a schema from, so until the first `insert` it is a table with a single `JSON` column named `json`, marked as such by the comment of the table. The first `insert` gives it the columns of the inserted document, so a collection created explicitly ends up with the same schema as one created by the insert itself. A table that carries no such marker is used as it is, whatever its columns look like: a table you created in ClickHouse yourself is never retyped by an `insert`. Creating a collection that already exists is the `NamespaceExists` error, as it is in MongoDB.
...
- The number of documents an `update` and a `delete` report is the number of documents their filter matches, counted before the mutation is submitted; counting them costs a read of the matching rows. A command that carries several specs applies them one after another, so each mutation is awaited before the next spec is counted, and a write command therefore returns only once its mutations have been applied on this server. A spec that cannot be translated stops the command there, as an ordered MongoDB write does: the writes of the specs before it are kept, and the error names what is unsupported rather than listing it in a `writeErrors` array. The reply of an `update` carries no `nModified`: a mutation is asynchronous and rewrites a matched row whether or not the assigned value differs from the one it already holds, so the number of documents that actually change is unknown here and is left unsaid rather than reported as a number that would not be true. A driver reports it as "not said" - `None` in `pymongo` - rather than as zero.
...
- `$toDecimal` keeps the exact scale of a constant argument, the way `$numberDecimal` keeps the scale of its value; a constant that fits no `Decimal128` of any scale is an error. Of a non-constant value it is only supported for integers and booleans, whose conversion is exact at scale 0 - including their `Nullable` and `LowCardinality` forms, which hold the same values.
...
- A database name must consist of letters, digits, `_` and `-`. A collection name may also contain a `.`, so a namespace such as `fs.files` works over the wire protocol; the dialect reads a statement as `<database>.<collection>.<operation>`, so a dotted collection name can only be addressed there through the wire protocol.
