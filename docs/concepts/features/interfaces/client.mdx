...
- Read-only queries run under `readonly = 1` with a 30-second and a 10 GiB limit, and may not use table functions that read external files or servers (`file`, `url`, `s3`, `remote`, `executable`, ...); every other query requires your explicit confirmation, with the query shown before you decide. If the session cannot be constrained to these limits (for example, a `readonly` settings profile forbids changing them), the unconfirmed query fails instead of running without the limits.
