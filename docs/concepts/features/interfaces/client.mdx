...
| `/dialect <name>`, `/lang <name>`, `/language <name>` | Switch the SQL dialect, or print the current one when entered without an argument (see [below](#switching-the-sql-dialect)) |
...
### Switching the SQL dialect {#switching-the-sql-dialect}

The client can parse queries in several dialects (see the [`dialect`](/reference/settings/session-settings/other#dialect) setting). Enter `/dialect` followed by a dialect name to switch (the equivalent forms `/language` and `/lang` also work):

```text
/dialect polyglot
```

The command is equivalent to `SET dialect = 'polyglot'`, but unlike a `SET` query it does not have to be parsed in the current dialect, so it also works when the current dialect cannot express such a query (for example, there is no `SET` statement in PromQL). While a non-default dialect is active, it is displayed in the prompt after the server display name:

```text
clickhouse-cloud (polyglot) :)
```

A custom prompt (see the `--prompt` option) does not have to contain the `{display_name}` placeholder - in that case the dialect is appended at the end of the prompt.

Entering `/dialect` on its own prints the current dialect.

The command is only recognized in interactive mode: a noninteractive script is parsed as SQL from end to end, and the dialect for it is selected with the `--dialect` option or with a `SET dialect = ...` statement. A `clickhouse-local` script that contains the command is rejected with a message saying that the command is available in interactive mode only.
