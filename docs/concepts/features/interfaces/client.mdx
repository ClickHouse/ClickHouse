...
The command is only recognized in interactive mode: a noninteractive script is parsed as SQL from end to end, and the dialect for it is selected with the `--dialect` option or with a `SET dialect = ...` statement.
