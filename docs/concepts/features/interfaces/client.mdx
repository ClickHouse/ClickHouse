...
In an interactive `clickhouse-client` session, you can run a query on a separate connection and continue using the main prompt while it executes. The client assigns each query a numeric job ID and spools its output so that it does not get mixed with the output at the prompt.
...
|                                | Client command `\bg`                                      | Server setting `run_query_in_background`                        |
|--------------------------------|-----------------------------------------------------------|-----------------------------------------------------------------|
...
Use the server setting for work such as `INSERT ... SELECT` that must survive a client disconnect. Use `\bg` when you want to keep using the same interactive client and retrieve a `SELECT` result later. The two mechanisms cannot be combined in one query.
