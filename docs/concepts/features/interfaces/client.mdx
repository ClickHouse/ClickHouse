...
| `--dump-schema[=<db1,db2,...>]`  | Dump the schema DDL - `CREATE` statements for databases, tables, views, materialized views, and dictionaries - for the given comma-separated database(s) (or all user databases, if no value is given) to stdout in dependency order, then exit. Similar to `pg_dump --schema-only`. <br/><br/>`--dump-schema-exclude=<db1,db2,...>` skips the listed databases when dumping all of them (cannot be combined with an explicit database list). <br/><br/>`--dump-schema-dir=<path>` writes one `<database>.sql` file per dumped database into `<path>` (created if missing) instead of writing everything to stdout. <br/><br/>Requires a server of version 26.6 or newer, and produces a script meant to be replayed into a server of the same version or newer than the one it was taken from. Replaying into an older server fails outright rather than partially, even one minor version apart: the script first replays the source server's experimental settings, and a setting name the target does not have aborts it before any object is created. `clickhouse-client --ignore-error` replays such a script in full; check afterwards that every reported error names an unknown setting, because that flag would equally hide a genuine `CREATE` failure.                                          |
