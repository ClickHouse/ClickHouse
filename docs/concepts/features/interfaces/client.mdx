...
| `--ssh-key-file [path-to-file]`  | File containing the SSH private key to authenticate with the server. <br/><br/>The file name can be omitted: then the key is looked up the same way as `ssh` does it - the identity files configured for this host in `~/.ssh/config`, the default identity files, such as `~/.ssh/id_ed25519`, and the keys held by the `ssh-agent`. A key held by the `ssh-agent` is preferred, because it does not need a passphrase. | -                                                                                                                |
