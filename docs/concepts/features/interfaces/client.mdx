...
- **Read-only queries without confirmation** — it runs `SELECT`, `SHOW`, `DESCRIBE`, `EXPLAIN`, `EXISTS` and `CHECK` statements under a sandbox: `readonly = 1`, a 30-second time limit and a 10 GiB memory limit. Table functions that reach outside of the server's tables (`file`, `url`, `s3`, `remote`, `executable`, ...) and named user tables or views are not allowed here. These queries are displayed and executed on your connection without asking.
...
3. If neither is found, it tries the server-side `aiGenerate` function.
