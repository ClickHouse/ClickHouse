...
`\jobs` includes the latest server-reported read rows and bytes, throughput, estimated completion percentage, writes, CPU, memory, and temporary-disk usage when those metrics are available. Metrics continue updating for both queries started with `\bg` and foreground queries detached with `Ctrl+B`.
