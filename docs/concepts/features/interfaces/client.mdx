...
| `--oauth-url`            | OAuth / OIDC issuer base URL. The client discovers `device_authorization_endpoint` and `token_endpoint` from `/.well-known/openid-configuration` (and related metadata URLs). Falls back to Auth0-style `/oauth/device/code` and `/oauth/token` if discovery fails. | - |
