...
- **Schema exploration** — it lists databases and tables and reads `CREATE TABLE` statements instead of guessing column names. The credentials of external-engine tables are always masked in what the assistant sees: its own queries force [`format_display_secrets_in_show_and_select = 0`](/reference/settings/formats/format#format_display_secrets_in_show_and_select), even when the session enables it.
...
- `enable_schema_access` - Let the assistant explore database schemas without confirmation. When disabled, schema tools and unconfirmed schema metadata queries are unavailable (default: `true`).
