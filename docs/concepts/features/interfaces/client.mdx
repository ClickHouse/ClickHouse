...
`clickhouse-local` also recognizes the command in noninteractive scripts, where the statements that follow it are parsed in the newly selected dialect. A rejected command (an unknown dialect, or one forbidden by a setting constraint) aborts the script, exactly like a rejected `SET dialect` statement.
