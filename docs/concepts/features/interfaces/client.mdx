...
| `--login`                        | Invokes the OAuth 2.0 device authorization grant to authenticate via an identity provider (IdP). <br/><br/>For ClickHouse Cloud hosts, OAuth settings are inferred. Otherwise provide `--oauth-url` (OIDC issuer) and `--oauth-client-id`, or explicit `--oauth-device-uri` / `--oauth-token-uri`. Optional: `--oauth-audience`, `--oauth-scope`.                                                                                                                                                                                                            | -                                                                                                                |
