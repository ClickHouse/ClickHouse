...
| `--login`                        | Invokes the OAuth 2.0 device authorization grant to authenticate via an identity provider (IdP). <br/><br/>For ClickHouse Cloud hosts, OAuth settings are inferred when no `--oauth-*` option is passed. Otherwise, including Cloud hosts that use JWT authentication with your own IdP, provide `--oauth-url` (OIDC issuer) and `--oauth-client-id`, or `--oauth-client-id` with explicit `--oauth-device-uri` / `--oauth-token-uri`. Optional: `--oauth-audience`, `--oauth-scope`.                                                                                                                                                                                                            | -                                                                                                                |
| `--oauth-url`            | OAuth / OIDC issuer base URL. The client discovers `device_authorization_endpoint` and `token_endpoint` from `/.well-known/openid-configuration` (and related metadata URLs). Discovery failure is reported as an error; use `--oauth-device-uri` and `--oauth-token-uri` when metadata is unavailable. | - |
| `--oauth-client-id`      | OAuth 2.0 client ID used for the device authorization grant. | - |
| `--oauth-client-secret`  | Optional client secret for confidential clients. | - |
| `--oauth-client-auth`    | Confidential client authentication method: `basic` (HTTP Basic, default when a secret is set) or `post` (`client_secret` in the form body). | `basic` when secret is set |
| `--oauth-audience`       | Optional `audience` form parameter (Auth0-style). | - |
| `--oauth-scope`          | OAuth scope string. Defaults to `openid profile email offline_access`. | - |
| `--oauth-device-uri`     | Explicit device authorization endpoint. Must be set together with `--oauth-token-uri` to skip discovery. | - |
| `--oauth-token-uri`      | Explicit token endpoint. Must be set together with `--oauth-device-uri` to skip discovery. | - |
