...
Like the [pinned columns](/concepts/features/interfaces/web-ui-pinned-columns), the modes belong to the result they color: to the query tab, and within a tab that is a [notebook](/concepts/features/interfaces/web-ui-notebook) to the individual cell. The tables of one `Run all` (multi-query) run share them, while other cells and other tabs keep their own.
