...
- `ARROW:FLIGHT:SQL:TYPE_NAME` contains the stable ClickHouse type family registered by `CommandGetXdbcTypeInfo`, such as `Date32` or `Decimal`. Top-level `Nullable` and `LowCardinality` wrappers are omitted. A bare `DateTime` (including transparent wrappers) does not set this key because its Arrow `uint32` representation cannot satisfy the catalog's timestamp contract; `DateTime('...')` does. Enums expose numeric codes and wide integers expose implementation-specific fixed-size binary values, so `Enum8`, `Enum16`, `Int128`, `UInt128`, `Int256`, and `UInt256` also do not set this key and only use `CLICKHOUSE:TYPE_NAME`. `String` is registered as the standard text family and uses Arrow `utf8`; its handling of invalid UTF-8 is described above.
...
- `ARROW:FLIGHT:SQL:PRECISION` and `ARROW:FLIGHT:SQL:SCALE` are included when they can be determined exactly from the ClickHouse type and the standard type mapping applies; a bare `DateTime` omits both.
