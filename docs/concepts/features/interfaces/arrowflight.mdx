...
- `ARROW:FLIGHT:SQL:TYPE_NAME` contains the stable ClickHouse type family registered by `CommandGetXdbcTypeInfo`, such as `Date32`, `Enum8`, or `Decimal`. Top-level `Nullable` and `LowCardinality` wrappers are omitted. Unsupported complex types do not set this standard key.
