...
- `ARROW:FLIGHT:SQL:TYPE_NAME` contains the stable ClickHouse type family registered by `CommandGetXdbcTypeInfo`, such as `Date32` or `Decimal`. Top-level `Nullable` and `LowCardinality` wrappers are omitted. Types without a lossless standard Arrow interpretation do not set this key. For example, enums expose numeric codes and wide integers expose fixed-size binary values, so `Enum8`, `Enum16`, `Int128`, `UInt128`, `Int256`, and `UInt256` only use `CLICKHOUSE:TYPE_NAME`.
