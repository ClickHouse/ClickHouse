...
| `x-clickhouse-session-close` | Set to `1` to close the session after the request completes. Requires `enable_arrow_close_session` to be `true` in the server config (see [CloseSession](#closesession)). |
...
| `enable_arrow_close_session` | `true` | Allow clients to close sessions via the `x-clickhouse-session-close` header and the [CloseSession](#closesession) `DoAction`. When set to `false`, the header is ignored and `CloseSession` returns `NOT_CLOSABLE` without releasing the session. |
...
Closes the session identified by the `x-clickhouse-session-id` header via the Flight SQL `CloseSession` action. The action body is an empty serialized `CloseSessionRequest`.

The response is a `CloseSessionResult` with one of these statuses:

- `CLOSED`: the session was closed, or no named session was in use (in which case the action succeeds as a no-op).
- `NOT_CLOSABLE`: session closing is disabled because `enable_arrow_close_session` is `false` in the server config.

When a session is closed, prepared statements created in that session are released (see [ClosePreparedStatement](#closepreparedstatement)).

```python
# Close the session from the current Flight call context
results = list(client.do_action(flight.Action("CloseSession", b""), options))
# results[0].body encodes CloseSessionResult; b"\x08\x01" means status=CLOSED
```
