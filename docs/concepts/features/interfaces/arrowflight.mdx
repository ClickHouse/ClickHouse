...
`dataset_schema` is also left out when the result contains a column whose Arrow type comes from [`output_format_arrow_unsupported_types`](/reference/settings/formats/output-format) — a `JSON`, `Dynamic` or `QBit` column, or a container holding one. A statement handle outlives the session that created it, and each later call reads that setting from whichever session serves it, so such a schema is not something the handle can promise. Execute the statement to obtain the schema instead. An `AggregateFunction` column does not cause this, because it is an Arrow `Binary` column under every value of the setting.
