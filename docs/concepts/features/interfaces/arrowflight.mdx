...
| `CommandGetXdbcTypeInfo` | Retrieve ODBC/JDBC type metadata for supported ClickHouse types. Supports optional `data_type` filter. |
...
The clearest example of such a difference is `DateTime` with an explicit time zone: Arrow Flight exposes `DateTime('...')` as `timestamp(s, tz)`, carrying the declared time zone, which is what a generic Flight SQL client expects for a column registered as `SQL_TYPE_TIMESTAMP`. A bare `DateTime` remains `uint32` epoch seconds because its effective time zone follows the mutable `session_timezone` setting; embedding that zone in the Arrow type could make `GetSchema`, `GetFlightInfo`, and `DoGet` disagree. Only the explicitly zoned form carries the standard `DateTime` XDBC field metadata; a bare `DateTime` retains `CLICKHOUSE:TYPE_NAME` but has no `ARROW:FLIGHT:SQL:TYPE_NAME`, `ARROW:FLIGHT:SQL:PRECISION`, or `ARROW:FLIGHT:SQL:SCALE`. The same rule applies to `DateTime64`: `DateTime64(..., '...')` is exposed as `timestamp` in its Arrow unit, while a bare `DateTime64` remains raw `int64` ticks — the value multiplied by `10^scale` — and carries only `CLICKHOUSE:TYPE_NAME`. On Arrow Flight ingestion, an `INT64` written into a `DateTime64` column is read back as raw ticks, so a table read over Flight can be written back through bulk ingest without corrupting the values; generic Arrow input (`FORMAT Arrow`, `ArrowStream`) keeps the contract that integer values are whole seconds. `FORMAT Arrow` and `FORMAT ArrowStream` also continue to encode `DateTime` as `uint32` for compatibility. The underlying value does not change when an explicit-zone `DateTime` is exposed as a timestamp: it holds the same epoch seconds, and the time zone only controls how they are interpreted and displayed. The `FORMAT Arrow`/`ArrowStream` mapping of `DateTime64` is unchanged.
...
Arrow `Utf8` columns must contain valid UTF-8, so ClickHouse replaces every invalid sequence with U+FFFD (`�`) before writing one. This applies both to real `String` columns, which Arrow Flight exposes as text, and to opaque columns using `text` mode. Consequently, the text mapping is not byte-exact for the complete ClickHouse `String` value domain. Only values that are already not valid text change. For opaque columns, use `binary` mode where the bytes have to be preserved exactly.
...
Flight SQL statement and prepared-statement results include column metadata that preserves the ClickHouse type identity. The schemas embedded by `CommandGetTables` when `include_schema=true` contain the same metadata:

- `ARROW:FLIGHT:SQL:TYPE_NAME` contains the stable ClickHouse type family registered by `CommandGetXdbcTypeInfo`, such as `Date32` or `Decimal`. Top-level `Nullable` and `LowCardinality` wrappers are omitted. A bare `DateTime` or `DateTime64` (including transparent wrappers) does not set this key because its raw epoch representation cannot satisfy the catalog's timestamp contract; `DateTime('...')` and `DateTime64(..., '...')` do. Enums expose numeric codes and wide integers expose implementation-specific fixed-size binary values, so `Enum8`, `Enum16`, `Int128`, `UInt128`, `Int256`, and `UInt256` also do not set this key and only use `CLICKHOUSE:TYPE_NAME`. `String` is registered as the standard text family and uses Arrow `utf8`; its handling of invalid UTF-8 is described above.
- `CLICKHOUSE:TYPE_NAME` contains the complete ClickHouse type expression, including wrappers, parameters, enum labels, and time zones.
- `ARROW:FLIGHT:SQL:PRECISION` and `ARROW:FLIGHT:SQL:SCALE` are included when they can be determined exactly from the ClickHouse type and the standard type mapping applies; a bare `DateTime` or `DateTime64` omits both.

This metadata does not change the Arrow physical array type. For example, `Date` and `Date32` are both still encoded as Arrow `date32`, while their `ARROW:FLIGHT:SQL:TYPE_NAME` values distinguish them.
