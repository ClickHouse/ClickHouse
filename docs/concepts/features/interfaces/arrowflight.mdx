...
ClickHouse data types are mapped to Arrow types during serialization. A type with no Arrow equivalent, such as `JSON`, `Dynamic`, `QBit` or `AggregateFunction`, is handled according to the setting [`output_format_arrow_unsupported_types`](/reference/settings/formats/output-format#output_format_arrow_unsupported_types):
...
Such a column is otherwise indistinguishable from a genuine `Utf8`/`Binary` one, so it is declared an Arrow extension type: the field metadata holds `ARROW:extension:name` = `clickhouse.opaque` and the original ClickHouse type name in `ARROW:extension:metadata`. A client that does not recognize the extension name sees the plain storage type, as the Arrow specification prescribes. Nested columns are tagged on their own field, so the child of an `Array(JSON)` carries the tag rather than the array itself.
