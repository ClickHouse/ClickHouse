...
ClickHouse data types are mapped to Arrow types during serialization. A type with no Arrow equivalent, such as `JSON`, `Dynamic`, `QBit` or `AggregateFunction`, is handled according to the setting [`output_format_arrow_unsupported_types`](/reference/settings/formats/output-format):
