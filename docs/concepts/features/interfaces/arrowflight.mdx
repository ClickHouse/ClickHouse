...
The clearest example of such a difference is `DateTime`: Arrow Flight exposes it as `timestamp(s, tz)`, carrying the type's effective time zone, which is what a generic Flight SQL client expects for a column registered as `SQL_TYPE_TIMESTAMP`. `FORMAT Arrow` and `FORMAT ArrowStream` still encode it as `uint32` epoch seconds for compatibility, so the same query can produce `timestamp` over Flight and `uint32` over those formats. The underlying value does not change: the timestamp holds the same epoch seconds, and the time zone only controls how they are interpreted and displayed.
...
- `ARROW:FLIGHT:SQL:TYPE_NAME` contains the stable ClickHouse type family registered by `CommandGetXdbcTypeInfo`, such as `Date32` or `Decimal`. Top-level `Nullable` and `LowCardinality` wrappers are omitted. Enums expose numeric codes and wide integers expose implementation-specific fixed-size binary values, so `Enum8`, `Enum16`, `Int128`, `UInt128`, `Int256`, and `UInt256` do not set this key and only use `CLICKHOUSE:TYPE_NAME`. `String` is registered as the standard text family and uses Arrow `utf8`; its handling of invalid UTF-8 is described above.
...
- `ARROW:FLIGHT:SQL:PRECISION` and `ARROW:FLIGHT:SQL:SCALE` are included when they can be determined exactly from the ClickHouse type.
