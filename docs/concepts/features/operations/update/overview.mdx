...
Lightweight updates also support correlated scalar subqueries in assignment expressions. The subquery can use values from the current target row to look up values in another table. Multiple assignments and composite correlation keys are supported.
...
WHERE target.id IN (SELECT id FROM source);
```

Read more: [Lightweight UPDATE](/reference/statements/update)

### On-the-fly mutations {#on-the-fly-mutations-summary}
