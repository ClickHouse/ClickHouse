...
Background queries let a client submit a query that continues running independently of the client connection.
Enable this behavior with the [`run_query_in_background`](/reference/settings/session-settings/other#run_query_in_background) setting.
After accepting the query, ClickHouse returns an empty successful result immediately and runs the query in the background.
...
The native protocol carries query settings separately from the SQL text. `clickhouse-client` parses most inline
query settings and sends them in this settings section. Native-protocol drivers can instead pass
...
Entries are buffered before they appear in `system.query_log`. For self-managed ClickHouse, the example server
configuration sets `query_log.flush_interval_milliseconds` to `7500`. ClickHouse Cloud entries can take up to
30 seconds to appear. Account for this delay when monitoring short-running background queries.
