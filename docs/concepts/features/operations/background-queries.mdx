...
Over HTTP the query text is read from the request body, so an `INSERT` whose data fits within `max_query_size` is accepted, while a larger body keeps streaming past the query text and is rejected.

Do not rely on that size boundary: use `INSERT ... SELECT`, or a table function such as `url` or `s3`,
