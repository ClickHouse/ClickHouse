...
- **(3)** If the right table fits into memory, use the hash join (`hash` and `parallel_hash` are aliases; choosing one does not control parallelism). This is typically the fastest in-memory option, depending on data size, data types, and the value distribution of the join key columns.
