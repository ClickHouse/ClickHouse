...
These algorithms dictate the manner in which a join query is planned and executed. The default `join_algorithm` list is `direct,hash,ie_join`: ClickHouse uses a direct join when the right-hand side is a dictionary or other key-value engine, prefers `hash` when that algorithm applies, and falls back to `ie_join` for eligible inequality-only joins (`parallel_hash` is an obsolete alias of `hash` and does not select parallelism). Alternatively, set `join_algorithm = 'auto'`: ClickHouse tries `hash` first. A memory-limit fallback exists only when `MergeJoin` or `GraceHashJoin` can run the join (one equality disjunct, supported kind and strictness). Then it spills through `GraceHashJoin` when `max_bytes_before_external_join` / `max_bytes_ratio_before_external_join` are enabled, otherwise it drains onto partial merge. A join with `OR` conditions stays on `hash` and can hit the memory limit. You can observe which algorithm was chosen via trace logging. ClickHouse also lets you list specific algorithms in `join_algorithm`.
...
- **(3)** If the right table fits into memory, use the hash join (`parallel_hash` is an obsolete alias of `hash`; choosing it does not control parallelism). This is typically the fastest in-memory option, depending on data size, data types, and the value distribution of the join key columns.
