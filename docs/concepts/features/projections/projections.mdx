...
The [`mutations_sync`](/reference/settings/session-settings/mutations#mutations_sync) setting is
...
Let's create an aggregate projection by the dimensions `year`, `district`, and `town`.
The projection groups sales by the year of each sale, so we materialize that
expression as a column to make it referable by filters:
...
The following queries contrast performance with and without projections. To disable projection use we use the setting [`optimize_use_projections`](/reference/settings/session-settings/optimize-use#optimize_use_projections), which is enabled by default.
...
The filter for rows from 2020 onwards references the stored projection dimension `year` directly:
...
<Note>
The filter must reference the stored dimension `year` directly. Writing
`WHERE toYear(date) >= 2020` doesn't use the projection: the optimizer rewrites
the condition into a range on `date`, and the projection doesn't store a `date`
column, so ClickHouse falls back to scanning the base table.
</Note>
