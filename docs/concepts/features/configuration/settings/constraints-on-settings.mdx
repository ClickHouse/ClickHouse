...
If several profiles apply to a user, their constraints are combined per setting:
`min` and `max` come from the last profile which sets them, the disallowed values
of all the profiles are collected together, and the writability (`readonly`,
`const`, `changeable_in_readonly`) comes from the last profile which constrains
that setting.
...
restriction that already applies to the session.
