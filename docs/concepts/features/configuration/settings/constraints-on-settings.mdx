...
The server setting [`access_control_improvements.readonly_can_only_be_tightened`](/reference/settings/server-settings/settings/access-control#access_control_improvements) closes the `GET` route above, by stopping that constraint from applying to `readonly` on HTTP requests whose method implies a read-only query. Over `POST` and the native protocol the constraint still applies, so it is a mitigation and not a substitute for keeping `readonly` off the list.
