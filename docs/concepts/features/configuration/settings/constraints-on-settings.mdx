...
The server setting [`access_control_improvements.readonly_can_only_be_tightened`](/reference/settings/server-settings/settings/access-control#access_control_improvements) stops that constraint from applying to `readonly` on HTTP requests whose method implies a read-only query, so such a request can no longer set `readonly` itself. Over `POST` and the native protocol the constraint still applies, and a profile whose own values include `readonly = 0` is applied to the session rather than checked against the request, so this is a mitigation and not a substitute for keeping `readonly` off the list.
