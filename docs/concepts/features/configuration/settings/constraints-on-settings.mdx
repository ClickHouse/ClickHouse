...
stricter rule, whatever `settings_constraints_replace_previous` is set to. It can
only narrow what is already in effect: raise a `min`, lower a `max`, add
`disallowed` values, or make a setting `readonly`. It cannot lift a constraint or
grant `changeable_in_readonly`. Any user can run `SET profile` with any profile,
so a session profile is not a way around the constraints in your configuration.
