...
If several profiles apply to a user, their constraints are merged. How they merge
depends on `settings_constraints_replace_previous`:
- **true** (recommended): the last constraint for a setting replaces the earlier
  one, including the fields the new constraint does not set.
- **false** (default): the last constraint replaces only the fields it sets, and
  the remaining fields are kept from the earlier one.

A profile applied with [`SET profile`](/sql-reference/statements/set) follows a
different rule. It can add restrictions, but it cannot remove or weaken a
restriction that already applies to the session. This rule does not depend on
`settings_constraints_replace_previous`.

For example, suppose the profile assigned to a user limits `max_threads` to `8`:
...
The user can apply another profile with a lower limit:
...
```sql
SET profile = 'single_threaded';
SET max_threads = 1; -- allowed
SET max_threads = 8; -- an exception: the active limit is now 1
```

In a new session with the original limit of `8`, a profile that sets
`max_threads` to `16` cannot raise that limit. The session remains limited to
`8` after `SET profile` applies that profile. The same rule means that `SET
profile` cannot remove `readonly`, remove a disallowed value, or grant
`changeable_in_readonly` access. Any user can apply a settings profile, so a
session profile cannot bypass the constraints in your configuration.
