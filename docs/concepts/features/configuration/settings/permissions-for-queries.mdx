...
The one exception is `readonly = 1`, the strictest value, which such a request may set when the
server enables [`access_control_improvements.readonly_can_only_be_tightened`](/reference/settings/server-settings/settings/access-control#access_control_improvements).
