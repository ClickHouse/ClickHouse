...
`ROW` constructors and types, `VALUES` tables, `OFFSET` before `LIMIT`, and several hundred functions, including
the ones whose names exist in both systems with different semantics (for example, Trino `length` counts Unicode
code points and is translated to `lengthUTF8`). ClickHouse functions that do not collide with Trino names remain
accessible, so both function families can be mixed in one query.
