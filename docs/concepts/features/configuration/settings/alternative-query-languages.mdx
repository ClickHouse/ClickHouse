...
- `logsql`: [LogsQL](https://docs.victoriametrics.com/victorialogs/logsql/), the query language of VictoriaLogs
...
LogsQL is the query language of [VictoriaLogs](https://docs.victoriametrics.com/victorialogs/logsql/).
ClickHouse translates a LogsQL query to a `SELECT` query over a configured log table.
To enable it and point it at a table with a time column and a message column:
...
```text title="Query"
_time:5m error | stats by (level) count() as hits | sort by (hits desc)
```

See the [LogsQL dialect guide](/guides/clickhouse/logsql) for the supported subset and the differences from VictoriaLogs.
