...
interrupted. A mutation of a `Memory` table leaves the table unchanged and reports
`TIMEOUT_EXCEEDED`. An `INSERT` may report `QUERY_WAS_CANCELLED`. Some operations that wait for
other replicas or for background work do not stop at `max_execution_time` at all: a quorum write
keeps waiting until its quorum is satisfied or `insert_quorum_timeout` elapses, then reports
