...
reconciles it with a listing of the metadata directory, so only the
directories changed after the snapshot are loaded from the object storage. A
read-only disk pointing to the same location (`readonly` set to `true`) reads
the snapshot on startup and on every periodic refresh (see the `MergeTree`
setting `refresh_parts_interval`), and skips the refresh when the ETag of the
snapshot did not change. The snapshot describes the state at the moment it was
written, so a read-only disk can lag behind the writer by the write delay.
...
| `metadata_snapshot_write_delay_ms`  | `0` rewrites the snapshot right after every modification. A larger value rewrites it in the background, not more often than the given number of milliseconds, as long as there were modifications. The snapshot is also written on server shutdown. | `0`     |
