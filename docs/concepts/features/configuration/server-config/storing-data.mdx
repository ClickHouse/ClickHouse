...
| `use_real_disk_size`                  | Boolean | `false`    | Accounts cache sizes in filesystem block-aligned units instead of written bytes. This approximates physical on-disk usage but is not exact; it affects `FilesystemCacheSize`, eviction-size metrics, and `current_size` in `system.filesystem_cache_settings`. The per-segment `size` in `system.filesystem_cache` remains the logical range size; use `downloaded_size` for bytes written to a partial segment. Restart is required to change this setting. |
