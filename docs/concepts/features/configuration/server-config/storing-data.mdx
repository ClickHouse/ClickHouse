...
| `filesystem_cache_max_download_size`                          | Size    | `128Gi`                 | Maximum amount of data a single query may write into the cache. Requires `enable_filesystem_query_cache_limit` in cache config.                                 |
| `filesystem_cache_skip_download_if_exceeds_per_query_cache_write_limit` | Boolean | `true`          | Controls behavior when `filesystem_cache_max_download_size` is reached: <br/>- `true`: Stops downloading new data <br/>- `false`: Evicts data cached by this query to make space for new data |
