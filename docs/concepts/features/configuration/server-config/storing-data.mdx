...
#### Metadata snapshot {#plain-rewritable-metadata-snapshot}

The metadata of a `plain_rewritable` disk is stored in the object storage as a
`prefix.path` object per directory. Building the in-memory state from these
objects requires listing the metadata directory, reading every `prefix.path`
and listing every directory, which is tens of thousands of requests even for a
disk with a single table.

To speed this up, the disk that writes the data also writes a compact
snapshot of the whole state to `__meta/snapshot.bin` (a versioned binary
format compressed with `ZSTD`). On startup, the writer reads the snapshot and
reconciles it with a listing of the metadata directory (a request per thousand
directories), so only the directories changed after the snapshot are loaded
from the object storage. A read-only disk pointing to the same location
(`readonly` set to `true`) does the same on startup and on every periodic
refresh (see the `MergeTree` setting `refresh_parts_interval`), and does not
read the snapshot again when its ETag did not change. Since the snapshot is
always reconciled with the listing, a snapshot that lags behind (because of the
write delay, a failed write, or a crash of the writer) never hides the changes
from the loaded state.

| Setting                             | Description                                                                                                                                                                                                                  | Default |
|-------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------|
| `metadata_snapshot_enabled`         | Write the snapshot after modifications (if the object storage is writable) and prefer it over listing the object storage when loading the state.                                                                              | `true`  |
| `metadata_snapshot_write_delay_ms`  | The snapshot is rewritten in the background, not more often than the given number of milliseconds, as long as there were modifications. `0` rewrites it right after every modification, in the committing thread, which makes every commit as expensive as serializing the metadata of the whole disk. The snapshot is also written on server shutdown, and removed right away when the disk becomes empty. | `1000`  |
...
<s3_plain_rewritable>
    ...
    <endpoint>https://s3.eu-west-1.amazonaws.com/clickhouse-eu-west-1.clickhouse.com/data/</endpoint>
    <use_environment_credentials>1</use_environment_credentials>
    <metadata_snapshot_write_delay_ms>10000</metadata_snapshot_write_delay_ms>
</s3_plain_rewritable>
```

The events `DiskPlainRewritableSnapshotRead`, `DiskPlainRewritableSnapshotUnchanged`,
`DiskPlainRewritableSnapshotWritten` and `DiskPlainRewritableSnapshotWriteFailed` in
[system.events](/reference/system-tables/events) show how the snapshot is used.
