...
[Mutations](/reference/statements/alter/index#mutations) and `ALTER` operations
are supported only if the disk is configured with `enable_hard_links`, see below.
...
#### Hard links on plain rewritable disks {#plain-rewritable-hard-links}

`ALTER` operations and mutations of a `MergeTree` table share the unchanged files
between a part and its mutated successor with hard links, instead of copying
them. The `enable_hard_links` setting (`0` by default) allows a disk with the
`plain_rewritable` metadata to create real hard links, which unlocks these
operations:
...
A hard link makes the metadata of a directory list its files explicitly, together
with the keys of the objects storing them, so that a file can refer to an object
stored under the prefix of another directory. Consider the following before
enabling it:

- Servers older than `26.9` cannot read a directory in this form. A disk that has
  such directories is not readable after a downgrade, and cannot be shared with
  older servers. Directories that never had hard links keep the previous layout,
  so a disk stays readable by older servers until the first operation that
  creates a hard link - a mutation, an `ALTER`, a `FREEZE` or a backup. Turning
  the setting off again does not convert the existing directories back.
- An object whose last link is removed is deleted in two steps: the metadata of
  the directory is rewritten first, and the object is deleted after that. If the
  server is killed in between, the object is left in the bucket and is not
  referenced by anything, so it has to be deleted manually.
...
| `enable_filesystem_query_cache_limit` | Boolean | `true`    | Allows a query to limit how much it writes into this cache, see `filesystem_cache_query_limit_bytes`. Costs nothing while no query sets that limit.                                                                                                           |
...
| `filesystem_cache_query_limit_bytes`                          | Size    | `0`                     | Maximum amount of data a single query may write into the cache. `0` (the default) means no limit. Counts the space the query reserves itself, so data another query cached is not counted. Requires `enable_filesystem_query_cache_limit` in cache config. The former name `filesystem_cache_max_download_size` stays as an alias. |
