...
- A server that does not have the `enable_hard_links` setting cannot read a
  directory in this form - it reads the whole metadata of the directory as its
  logical path. A disk that has such directories is therefore not readable after
  a downgrade to such a version, and cannot be shared with it. Directories that
  never had hard links keep the previous layout, so a disk stays readable by
  older servers until the first operation that creates a hard link - a mutation,
  an `ALTER`, a `FREEZE` or a backup. Turning the setting off again does not
  convert the existing directories back.
- The setting is read only when the disk is created: a change takes effect after
  a restart of the server, `SYSTEM RELOAD CONFIG` does not apply it.
