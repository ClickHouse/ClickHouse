...
- Servers older than `26.9` cannot read a directory in this form. A disk that has
  ...
  so a disk stays readable by older servers until the first `ALTER` or mutation.
  Turning the setting off again does not convert the existing directories back.
