...
1. A `type` section, equal to one of `s3`, `azure_blob_storage`, `hdfs` (unsupported), `local_blob_storage`, `web`, `gcs` (experimental, from `26.10`).
...
2. `object_storage_type`, equal to one of `s3`, `azure_blob_storage` (or just `azure` from `24.3`), `hdfs` (unsupported), `local_blob_storage` (or just `local` from `24.3`), `web`, `gcs` (experimental, from `26.10`, see [native Google Cloud Storage](/operations/storing-data#native-gcs-storage)).
...
The same disk can be created dynamically. A dynamic disk comes from user SQL, so it is gated by the experimental session setting [`use_native_gcs`](/reference/settings/session-settings/use#use_native_gcs) and is rejected without it (a disk defined in the server configuration above needs no session setting):
...
These are the parameters of a disk defined in the server configuration. A dynamic `disk(...)` definition created from user SQL understands the same ones except the credential parameters that would make the server read a local file or act as itself: `service_account_key_file` is rejected outright (use `service_account_key` to pass the key inline), and `service_account_key`, `google_adc_*` and `use_environment_credentials = 1` are subject to [`s3_allow_server_credentials_in_user_queries`](/reference/settings/session-settings/s3-allow#s3_allow_server_credentials_in_user_queries).
...
| `service_account_key_file`  | Path to a service account JSON key file. Server configuration only: a dynamic disk created from user SQL is rejected with `ACCESS_DENIED`, because it would make the server open a local file of the query's choosing. | -             |
| `google_adc_client_id`, `google_adc_client_secret`, `google_adc_refresh_token` | A Google "authorized user" refresh-token triple. All three must be given together. The access token minted from it is renewed as it nears expiry, so it is usable by a long-lived disk. | -             |
| `google_adc_token_uri`      | Where the refresh token is exchanged. Only applies together with the triple above; validated against `remote_url_allow_hosts`.                       | `https://oauth2.googleapis.com/token` |
| `no_sign_request`           | If `true`, send anonymous (unsigned) requests.                                                                                                       | `false`       |
| `use_environment_credentials` | If `0`, do not resolve Application Default Credentials; a disk with no explicit credential then sends anonymous requests.                          | `1`           |
...
| `max_connections`           | Upper bound on the keep-alive connections retained per endpoint.                                                                                     | `1024`        |
| `http_keep_alive_timeout`    | For how many seconds a pooled connection is reused before it is reconnected. `0` removes the bound.                                                  | `5`           |
| `http_keep_alive_max_requests` | How many requests a pooled connection serves before it is reconnected. `0` removes the bound.                                                      | `100`         |
| `retry_attempts`            | How many times a failed request is retried before the error is reported. `0` disables retries.                                                       | `10`          |
| `s3_max_get_rps`, `s3_max_get_burst` | Request-rate limit of the GET requests (downloads, metadata requests, listings): requests per second, and how many can be issued at once before the limit applies. The same keys, defaults and fallback to the query-level settings as for the `s3` disk type. | `0` (unlimited); burst: the rate |
| `s3_max_put_rps`, `s3_max_put_burst` | Request-rate limit of the PUT, POST and PATCH requests (uploads, server-side copies). DELETE requests are not limited. | `0` (unlimited); burst: the rate |
...
If none of the explicit credential parameters is set, and `use_environment_credentials` is not `0`, the client falls back to [Application Default Credentials](https://cloud.google.com/docs/authentication/application-default-credentials) (a key file referenced by the `GOOGLE_APPLICATION_CREDENTIALS` environment variable, or the metadata server on GCE/GKE).

#### Apache Iceberg on a native GCS disk {#native-gcs-storage-iceberg}

An [`Iceberg`](/reference/engines/table-engines/integrations/iceberg) table can be read from and written to a native GCS disk by naming the disk in the `disk` setting, which is what the `Iceberg` engine and the `iceberg` table function pick their backend from:

```sql
CREATE TABLE iceberg_on_gcs (a UInt64, b String)
ENGINE = Iceberg(path = 'table1/', format = Parquet)
SETTINGS disk = 'gcs_disk';

SELECT count() FROM iceberg(path = 'table1/', SETTINGS disk = 'gcs_disk');
```

The backend-named `icebergS3` engine and table function keep using the S3-compatible XML API, which is a different route to the same bucket.

#### Limitations {#native-gcs-storage-limitations}

- The backend is experimental. Dynamic GCS disks and the `gcs` table function require the experimental setting [`use_native_gcs`](/reference/settings/session-settings/use#use_native_gcs); server-configured disks are enabled by `object_storage_type: gcs`. The `GCS` table engine uses the S3-compatible backend.
- The `plain_rewritable` metadata type is not supported with the native `gcs` object storage type.
- Native GCS disks do not support a static `access_token` (a bearer token cannot be renewed for a long-lived client) or `access_header`; use `google_adc_*` refresh-token credentials, Application Default Credentials, or a service-account key instead.
- Authentication settings that only the S3-compatibility path understands (`access_key_id`, `secret_access_key`, `session_token`, `role_arn`, `http_client`, `service_account`, `metadata_service`, `request_token_path`, the `server_side_encryption_*` keys) are rejected rather than ignored, so a disk cannot end up authenticating as something other than what was configured.
