...
| `drop_cache_threads`                  | Integer | `16`       | Maximum number of threads for parallel removal of cache keys on `SYSTEM DROP FILESYSTEM CACHE`. Value 1 means the removal is performed by the query thread alone.                            |
