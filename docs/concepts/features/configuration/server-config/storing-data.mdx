...
| `use_real_disk_size`                  | Boolean | `false`    | Accounts cache reservations in filesystem block-aligned units instead of reserved bytes. This approximates the reservation footprint on disk but is not the exact allocated size; it affects `FilesystemCacheSize`, eviction-size metrics, and `current_size` in `system.filesystem_cache_settings`. The per-segment `size` in `system.filesystem_cache` remains the logical range size; use `downloaded_size` for bytes written to a partial segment. Restart is required to change this setting. |
