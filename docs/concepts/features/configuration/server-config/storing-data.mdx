...
| `filesystem_cache_query_limit_bytes`                          | Size    | `0`                     | Maximum amount of data a single query may write into the cache. `0` (the default) means no limit. Requires `enable_filesystem_query_cache_limit` in cache config. The former name `filesystem_cache_max_download_size` stays as an alias. |
| `filesystem_cache_skip_download_if_exceeds_per_query_cache_write_limit` | Boolean | `true`          | Controls behavior when `filesystem_cache_query_limit_bytes` is reached: <br/>- `true`: Stops downloading new data <br/>- `false`: Evicts data cached by this query to make space for new data |
