...
Optionally, `metadata_type` can be specified. Its default depends on the object storage type: `local` for durable object storages (`s3`, `azure_blob_storage`, `local_blob_storage`, `hdfs`), `web` for the `web` object storage, and `memory` for `borrow_from_cache`. It can also be set explicitly to `plain`, `web`, `plain_rewritable` (starting from `24.4`), or `memory`.
Usage of `plain` metadata type is described in [plain storage section](/concepts/features/configuration/server-config/storing-data#plain-storage), `web` metadata type can be used only with `web` object storage type, `local` metadata type stores metadata files locally (each metadata file contains mapping to files in object storage and some additional meta information about them). The `memory` metadata type keeps all mappings entirely in RAM with no persistence — see [borrow-from-cache storage](/concepts/features/configuration/server-config/storing-data#borrow-from-cache-storage); it can be used only with `object_storage_type='borrow_from_cache'`, and conversely `borrow_from_cache` requires `metadata_type='memory'` (any other combination is rejected with `INVALID_CONFIG_PARAMETER`).
...
The `borrow_from_cache` object storage type allocates space inside an existing [filesystem cache](/concepts/features/configuration/server-config/storing-data#using-local-cache) for data storage. Each stored object is backed by an ephemeral cache segment; when released, the cache reclaims the space. Combined with the `memory` metadata type, this provides a fully ephemeral disk where both data and metadata are lost on server restart.
...
| `cache_name`           | Name of an existing filesystem cache to borrow space from. The cache must already exist when a new `borrow_from_cache` disk is registered on a running server; see the restart-robustness note below for server startup and for reuse of an already-registered disk.|
...
- **Restart robustness.** A table on a `borrow_from_cache` disk persists its definition like any other table, so it is reattached on restart (necessarily empty, since its data does not survive a restart). If the referenced cache is not registered yet at that point — caches defined inline are materialized in an unspecified order, and the cache may have been dropped — the disk comes up with data writes blocked instead of aborting server startup, and becomes writable again once the cache appears. This applies both to disks defined inline in a table's DDL and to disks defined in the server configuration. This state is reported as `system.disks.is_read_only = 1`, but it is narrower than a "no writes at all" contract: only writes of data (blobs) are rejected, while the in-memory metadata stays mutable, so metadata-only operations - such as creating an empty table or dropping one - still succeed, and the failure surfaces on the first `INSERT`. Whether a later `CREATE` fails immediately depends on whether it registers the disk anew or reuses one already brought up: registering a `borrow_from_cache` disk on a running server validates the cache, so defining one whose `cache_name` does not exist is rejected right away, but reusing a disk that is already registered and has come up read-only skips that check, so `MergeTree` (which skips directory setup on a read-only disk) creates an empty table and defers the failure to the first `INSERT` until the cache reappears.
...
- **No in-place appends to a blob.** A stored object is only ever written as a whole. Table engines that append to their files — such as `Log`, `TinyLog`, and `StripeLog` — are still supported: each append is stored as a new object, and the file-to-object mapping in the metadata grows instead.
- **Data is stored locally.** The underlying data lives in the local filesystem cache, so reads are served locally without any network access. The disk still reuses the object-storage disk machinery, so at the disk-abstraction level it is reported as remote (`system.disks.is_remote = 1`) and `MergeTree` applies its remote-disk read tuning to it, even though no remote storage is involved.
- **Reported paths are virtual.** The disk has no real host filesystem path: `system.disks.path` shows the placeholder `/`, and paths derived from it — such as `system.tables.data_paths` — identify entries in the in-memory metadata, not files on the host filesystem. This mirrors `plain` and `plain_rewritable` disks over remote object storage, whose reported paths are object-key prefixes. ClickHouse never performs host-filesystem operations on these paths; for example, the `Atomic` database engine skips creating its `data/<database>/<table>` symlink for tables on such disks.
