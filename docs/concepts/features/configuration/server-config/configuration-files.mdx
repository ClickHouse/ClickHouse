...
The same change applies to a plain scalar substituted through an inline structural `<include from_zk="…"/>` under a leaf setting or a secret, for example `<password><include from_zk="/secret"/></password>`: a scalar is now kept as its exact original bytes there too, so `/secret = a&amp;b` (which used to decode to `a&b`) now resolves to the literal `a&amp;b` and must be stored raw as `a&b` instead. Do not use surrounding whitespace for this form, because that whitespace is part of the resulting setting value; prefer the direct `<password from_zk="…"/>` form when exact bytes matter. (A `from_zk` value that begins with `<`, or an actual YAML subtree referenced from `<include>`, is unaffected — only plain scalars are kept literal.)
</Note>
