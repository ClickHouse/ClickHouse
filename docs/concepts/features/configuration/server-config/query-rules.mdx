...
sidebarTitle: 'Query rewrite rules'
...
| `Int` | an integer literal (not a decimal or floating-point one) |
...
A rule template may itself be a `CREATE RULE` / `ALTER RULE` statement, but a `{name:Type}` placeholder inside such a nested rule template is not supported and is rejected at `CREATE RULE` / `ALTER RULE` time. This applies wherever the placeholder sits inside the nested template, including the query parts listed below that the matcher does not traverse.
