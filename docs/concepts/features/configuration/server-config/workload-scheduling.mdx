...
- **`priority`** — strict priority by the query's [`priority`](/operations/settings/settings#priority) setting (lower value means higher priority). Queries are served in priority order, FIFO within the same priority. This reuses the existing `priority` query setting, so a value set for the query's admission priority also orders it inside the workload. Like any strict-priority scheme it can starve low-priority queries.
