...
* `scheduler` - (time-shared only) the scheduling algorithm used between the queries of this workload: `fifo` (default, first-come-first-served) or `fair` (weighted fair queueing). See [Query-aware scheduling](#query_aware_scheduling).
