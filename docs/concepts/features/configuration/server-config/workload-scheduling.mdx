...
* `scheduler` - (time-shared only) the scheduling algorithm used between the queries of this workload: `fifo` (default, first-come-first-served), `fair` (weighted fair queueing), `las` (least-attained-service), or `priority` (strict order by the query `workload_priority` setting). See [Query-aware scheduling](#query_aware_scheduling).
...
- **`fifo`** (default) — first-come-first-served, preserving the historical behaviour. It is starvation-free, but provides no fairness and ignores `weight` and the related per-query settings.
...
- **`priority`** — strict priority by the query's `workload_priority` setting (an `Int64`, lower value means higher priority). Queries are served in priority order, FIFO within the same priority. The default `0` is the neutral baseline; a negative value raises a query above the default and a positive value lowers it. Like any strict-priority scheme it can starve low-priority queries.

The `scheduler` is time-shared only (CPU and IO) and can be set per resource with the `FOR <resource>` clause. Changing it on an existing workload swaps the algorithm in place, without interrupting other resources.

For CPU resources, the `scheduler` and the per-query settings below are only meaningful with CPU slot preemption (`cpu_slot_preemption = 1`, the default). Without preemption, CPU slots carry a fixed per-slot cost rather than real CPU time, so these settings no longer reflect actual CPU consumption.
...
| `weight_lowering_age_seconds` | Once the query's wall-clock age exceeds this, its weight is lowered. `0` disables it. |
| `weight_lowering_cpu_seconds` | Once the query has attained this many CPU-seconds, its weight is lowered (CPU resources). `0` disables it. |
| `weight_lowering_io_bytes` | Once the query has attained this many IO bytes, its weight is lowered (IO resources). `0` disables it. |
| `weight_lowering_factor` | The factor the weight is multiplied by when a threshold is crossed (e.g. `0.25`). The first threshold to trip applies the full lowering; thresholds do not combine. `1.0` disables lowering. Clamped to `[0, 1]`. |
...
* `queue` - leaf of the hierarchy capable of holding requests that exceed resource capacity.
