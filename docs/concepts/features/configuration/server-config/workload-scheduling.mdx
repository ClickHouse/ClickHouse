...
:::note
The `create_workloads_and_resources` restore setting selects how a workload or resource that already exists is handled while it is being restored (it does not turn restoring these entities on or off). Like the related `create_function` setting, it accepts:
