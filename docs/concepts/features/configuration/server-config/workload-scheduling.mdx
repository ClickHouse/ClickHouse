...
A thread may spend time not using the CPU while it still holds a CPU slot — for example while it has no task ready to run, or while it is otherwise blocked and doing no CPU work. By default the slot stays held for that whole time, so it cannot be used by other queries even though the CPU is idle. Enabling the `cpu_slot_parking` server setting lets a thread release ("park") its CPU slot during such non-CPU waits and acquire one again when it resumes CPU work. This allows other queries to use the freed CPU capacity while a query is stalled, which improves CPU utilization and query latency under overload. Parking applies only when `cpu_slot_preemption` is enabled and a single CPU resource defines both `MASTER THREAD` and `WORKER THREAD` (as in the `CREATE RESOURCE cpu (MASTER THREAD, WORKER THREAD)` example above); in other configurations it has no effect.
