...
The rules above select which workload gives up memory. Within that workload, the `memory_eviction_score` query setting decides which of its running queries is evicted first: the query with the highest `memory_eviction_score` is killed first, and queries with an equal score fall back to the largest reservation, where a reservation's size is its current allocation plus any pending increase. A query whose own reservation cannot fit within the limit even if everything else is evicted is stopped to fail its own request, regardless of score, rather than evicting other (possibly lower-precedence) workloads. It does not change which workload is selected — that is still governed by precedence and normalized memory usage as described above. The same order also decides growth within the workload under contention: when several of its queries compete to grow their reservations, the one with the lower `memory_eviction_score` grows first, so a higher-scored query is both grown last and evicted first. At the default `memory_eviction_score` of `0` the score does not influence this intra-workload order.
