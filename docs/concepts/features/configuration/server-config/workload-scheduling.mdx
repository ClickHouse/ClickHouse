...
* Pending allocation cannot kill an allocation of the same precedence. Note that running allocations of the same precedence may evict each other: the victim is chosen by the per-query `memory_eviction_score` setting (a higher score is evicted first), falling back to normalized memory usage (larger first) when scores are equal. The default `memory_eviction_score` is `0`, which leaves the eviction order unchanged.
