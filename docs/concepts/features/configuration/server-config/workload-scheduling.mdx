...
|---|---|
| `weight` | Base fair-scheduling weight of the query (default `1.0`). |
| `weight_lowering_age_seconds` | Once the query's wall-clock age exceeds this, its weight is lowered. |
...
| `weight_lowering_io_bytes` | Once the query has attained this many IO bytes, its weight is lowered (IO resources). |
