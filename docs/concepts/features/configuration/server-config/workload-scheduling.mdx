...
The rules above select which workload gives up memory. Within that workload, the `memory_eviction_score` query setting decides which of its running queries is evicted first: among the queries that can free memory, the one with the highest `memory_eviction_score` is killed first, and queries with an equal score fall back to the largest reservation. Evicting a query that holds no memory would free nothing, so such a query is not killed on behalf of another — the exception is the query whose own allocation hit the limit, which may be stopped to fail its own request. It does not change which workload is selected — that is still governed by precedence and normalized memory usage as described above. At the default `memory_eviction_score` of `0` the score does not influence this intra-workload order (queries are evicted largest-reservation-first among those that can free memory).
