...
* `scheduler` - (time-shared only) the scheduling algorithm used between the queries of this workload: `fifo` (default, first-come-first-served), `fair` (weighted fair queueing), or `las` (least-attained-service). See [Query-aware scheduling](#query_aware_scheduling).
