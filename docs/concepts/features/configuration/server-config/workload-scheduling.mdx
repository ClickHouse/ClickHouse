...
The rules above select which workload gives up memory. Within that workload, the `memory_eviction_score` query setting decides which of its running queries is evicted first. Among the queries that can relieve the pressure — a query that currently holds no memory is not chosen while one that holds memory exists, since evicting it would free nothing — the query with the highest `memory_eviction_score` is killed first, and queries with an equal score fall back to the largest reservation. It does not change which workload is selected — that is still governed by precedence and normalized memory usage as described above. The default `memory_eviction_score` is `0`, which leaves this intra-workload order unchanged.
