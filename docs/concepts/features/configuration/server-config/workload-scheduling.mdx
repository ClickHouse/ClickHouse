...
| `weight_lowering_cpu_seconds` | Once the query has attained this many CPU-seconds, its weight is lowered (CPU resources). Attained CPU is granted scheduler service (charged at grant, not as CPU is spent): with `cpu_slot_preemption = 1` (the default) it tracks granted CPU, leading actual consumption by at most one quantum (`cpu_slot_quantum_ns`) per active slot; without preemption it counts acquired slots (as noted above), not CPU-seconds. Use `weight_lowering_age_seconds` for a preemption-independent threshold. |
