...
A thread may block on work that does not consume CPU, for example while it waits for data from disk or from a remote replica, or while it has no task ready to process. By default such a thread keeps its CPU slot while blocked, so the slot cannot be used by other queries even though no CPU work is being done. Enabling the `cpu_slot_parking` server setting makes a thread release ("park") its CPU slot when it blocks on such non-CPU work, and acquire a slot again when it is ready to resume CPU work. This allows other queries to use the freed CPU capacity while a query is stalled, which improves CPU utilization and query latency under overload. Parking applies only when `cpu_slot_preemption` is enabled and a single CPU resource defines both `MASTER THREAD` and `WORKER THREAD` (as in the `CREATE RESOURCE cpu (MASTER THREAD, WORKER THREAD)` example above); in other configurations it has no effect.
