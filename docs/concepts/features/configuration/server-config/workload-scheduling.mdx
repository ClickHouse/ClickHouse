...
<Note>
Workload limits are independent from other ways to limit memory consumption like [max_memory_usage](/reference/settings/session-settings#max_memory_usage) query setting. They could be used together to achieve better control over memory consumption. It is possible to set independent memory limits based on users (not workloads). This is less flexible and does not provide features like memory reservation and queueing of pending queries. See [Memory overcommit](/concepts/features/configuration/settings/memory-overcommit)
</Note>
