...
While a thread holds a CPU slot it does not always use the CPU, and by default the slot stays held even when it is not — so other queries cannot use it even though the CPU is idle. Enabling the `cpu_slot_parking` server setting lets a thread release ("park") its CPU slot at certain waits where it is not using the CPU — an idle worker with no task to run, a thread waiting on a remote or asynchronous source, a cached read fetching data from storage, or a thread paused by read-bandwidth throttling — and acquire a slot again when it resumes CPU work. Not every non-CPU wait releases the slot; an ordinary local file read, for example, does not. This allows other queries to use the freed CPU capacity while a query is stalled, which improves CPU utilization and query latency under overload. Parking applies only when `cpu_slot_preemption` is enabled and a single CPU resource defines both `MASTER THREAD` and `WORKER THREAD` (as in the `CREATE RESOURCE cpu (MASTER THREAD, WORKER THREAD)` example above); in other configurations it has no effect.
