...
| `weight_lowering_cpu_seconds` | Once the query has attained this many CPU-seconds, its weight is lowered (CPU resources). Reflects real CPU time only with `cpu_slot_preemption = 1` (the default); without preemption attained CPU is counted per acquired slot (as noted above), so this threshold then counts acquired slots rather than CPU-seconds — use `weight_lowering_age_seconds` for a preemption-independent threshold. |
