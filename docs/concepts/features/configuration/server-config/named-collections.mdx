...
A collection that stores credentials (`password`, `access_key_id`, `secret_access_key`,
`account_key`, and similar) alongside the destination they are sent to (`url`, `host`,
`endpoint`, `addresses_expr`, and similar) is an exception: a query may repoint such a
destination key to another path of the same host, but not to a different host or port.
Otherwise the `NAMED_COLLECTION` privilege alone would be enough to have the server send
the stored credentials to a host of the user's choosing. Overriding a destination key is
still allowed when the collection stores no credentials, when it stores no destination for
the query to redirect, or when the key is explicitly declared `OVERRIDABLE`.

<Warning>
Keys that are not part of the destination are still overridable by default, so if a
collection exists purely to hide credentials, disable
`allow_named_collection_override_by_default` (which is enabled by default) or declare its
keys `NOT OVERRIDABLE`.
</Warning>
...
* `url` can be overridden or not depending on the value of `allow_named_collection_override_by_default`. Had this collection also stored a credential, `url` could only have been repointed to another path of the same host.
