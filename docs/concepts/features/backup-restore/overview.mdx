...
| `[EXCEPT DATA FROM {TABLE\|TABLES} ...]`                               | **(BACKUP only)** Backs up table DDL without data — the table structure is preserved but the table is empty after restore. Differs from `EXCEPT TABLES`, which omits the table entirely. Scoped to the element it is written on: see [Backing up a table without its data](#backup-without-data). |
...
`EXCEPT DATA FROM {TABLE|TABLES}` puts a table's definition in the backup without its data, so `RESTORE`
recreates the table empty. It is useful for tables whose contents can be regenerated after a restore, for
example the target tables of materialized views.
