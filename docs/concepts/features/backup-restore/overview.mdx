...
| `[EXCEPT DATA FROM {TABLE\|TABLES} ...]`                               | **(BACKUP only)** Backs up table DDL without data — the table structure is preserved but the table is empty after restore. Differs from `EXCEPT TABLES`, which omits the table entirely. |
