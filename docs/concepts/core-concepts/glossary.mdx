...
export const glossaryMetadata = {
  ...
  json: { aliases: ['semi-structured', 'semi structured', 'VARIANT', 'STRUCT', 'RECORD'], code: true, learnMore: '/sql-reference/data-types/newjson' },
  ...
  'replica-aware-routing': { aliases: ['replica aware routing', 'sticky routing', 'sticky replica routing', 'sticky replicas', 'sticky sessions', 'session affinity'], learnMore: '/manage/replica-aware-routing' },
...
The `JSON` type stores semi-structured documents whose paths and types may vary between rows. ClickHouse stores discovered paths as subcolumns so queries can read individual fields efficiently. Use typed columns or structural types such as `Tuple` when the schema is stable; ClickHouse `Variant` is a separate type for one of a declared set of alternatives.

## Lightweight update {#lightweight-update}

A lightweight `UPDATE` is a beta feature that creates patch parts containing only the updated columns and rows instead of rewriting entire columns or data parts. Updated values are immediately visible in `SELECT` queries through patch application, but they are physically materialized only during subsequent merges and mutations.
...
## Replica-aware routing {#replica-aware-routing}

Replica-aware routing in ClickHouse Cloud is a best-effort mechanism that routes HTTP requests with the same routing value to the same replica. It can preserve temporary tables or session state across related requests, improve reuse of replica-local caches, and provide read-after-write consistency. The mapping can change when the service's replica count changes, and it doesn't provide workload isolation.
...
ClickHouse is optimized for immutable, append-heavy data rather than frequent in-place row updates. Updates can be modeled by inserting new versions with specialized table engines, made immediately visible through lightweight `UPDATE` statements, or performed as mutations that rewrite affected data parts.
