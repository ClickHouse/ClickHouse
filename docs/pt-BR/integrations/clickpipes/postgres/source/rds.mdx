...
Em vez de uma senha, você pode autenticar o usuário do ClickPipes usando acesso baseado em funções do AWS IAM. Crie o usuário da seguinte forma e, em seguida, conceda a ele os mesmos privilégios de schema e replicação mostrados acima:
...
Consulte [autenticação de banco de dados do AWS IAM (RDS/Aurora)](/pt-BR/integrations/clickpipes/postgres/auth) para ver as etapas completas de configuração da função e da política do IAM necessárias para que o ClickPipes possa se autenticar dessa forma.
