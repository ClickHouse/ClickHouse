...
<div id="enable-binlog-logging">
  ### 1. Ative o registro em log binário por meio de backup automatizado
</div>

O recurso de backups automatizados determina se o registro em log binário está ativado ou desativado no MySQL. Os backups automatizados podem ser configurados para sua instância no RDS Console acessando **Modify** &gt; **Additional configuration** &gt; **Backup** e marcando a caixa de seleção **Enable automated backups** (caso ainda não esteja marcada).
