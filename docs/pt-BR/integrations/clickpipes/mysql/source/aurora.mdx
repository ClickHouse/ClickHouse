...
Em vez de uma senha, você pode autenticar o usuário do ClickPipes por meio do acesso baseado em função do AWS IAM. Crie o usuário da seguinte forma e, em seguida, conceda a ele os mesmos privilégios de schema e replicação mostrados acima:

```sql
CREATE USER 'clickpipes_iam_user' IDENTIFIED WITH AWSAuthenticationPlugin AS 'RDS';
```

Consulte [AWS IAM DB authentication (RDS/Aurora)](/pt-BR/integrations/clickpipes/postgres/auth) para ver o passo a passo completo de configuração da IAM role e da política necessárias para que o ClickPipes se autentique dessa forma.
