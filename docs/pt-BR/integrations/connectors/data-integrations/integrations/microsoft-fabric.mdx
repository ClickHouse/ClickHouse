...
<div id="copying-data-from-onelake">
  ## Copiando dados do OneLake
</div>

O fluxo Copy &amp; Sync permite copiar tabelas do OneLake para o ClickHouse:

1. Abra seu item do ClickHouse e selecione **Copy &amp; Sync**
2. Escolha um Lakehouse no seu workspace
3. Selecione as tabelas que deseja acelerar
4. Comece a copiar um snapshot pontual dos seus dados

<Frame>
  ![Configuração do Copy & Sync criando uma nova tabela de destino com chave de ordenação](/images/integrations/tools/data-integration/microsoft-fabric/fabric-03-copy-sync-table-selection.webp)
</Frame>
...
<Frame>
  ![Mapeamento de colunas do Copy & Sync de uma tabela de origem do OneLake para uma tabela existente do ClickHouse](/images/integrations/tools/data-integration/microsoft-fabric/fabric-04-copy-sync-existing-table.webp)
</Frame>
...
A cópia de dados do OneLake para o ClickHouse é gratuita durante a prévia pública do Microsoft Fabric. A cobrança terá início quando a carga de trabalho do Fabric atingir a disponibilidade geral. Os detalhes de preço serão publicados quando a carga de trabalho atingir GA.
...
Cada Copy &amp; Sync do OneLake é executado como um ClickPipe. Acompanhe o progresso da cópia, a vazão e os erros na [página do ClickPipes](/pt-BR/integrations/clickpipes/monitoring) no console do ClickHouse Cloud.
...
1. Crie um item do ClickHouse no mesmo workspace, conforme descrito em [provisionamento](#provisioning)
2. Abra **Copy &amp; Sync**, escolha seu Lakehouse e selecione a tabela `publicholidays`
3. Ordene pelos campos que você filtra com mais frequência: `countryOrRegion, date`
...
* **As cópias são snapshots em um momento específico.** Ainda não há replicação agendada ou contínua. Para atualizar uma tabela com os dados mais recentes do OneLake, execute a sincronização novamente. Atualizações e exclusões na tabela de origem não são propagadas para a cópia no ClickHouse.
* **Somente tabelas do Lakehouse.** O Copy &amp; Sync lê tabelas Delta de um Lakehouse no mesmo workspace. Outros itens do Fabric (Warehouse, bancos de dados KQL, bancos de dados espelhados) ainda não são compatíveis como fontes.
...
* **As alterações de função são aplicadas na próxima abertura.** Sua função na organização do ClickHouse é mapeada a partir da sua função no workspace do Fabric quando você abre um item do ClickHouse. O Fabric não consegue notificar o ClickHouse sobre alterações de função; portanto, uma alteração na função do workspace só entra em vigor na próxima vez que você abrir o item, não imediatamente.
...
* **O SSO do Console exige um email no seu perfil do Entra.** Os usuários do ClickHouse Cloud precisam de um endereço de email; se não houver um configurado no seu perfil do Microsoft Entra, o login no Console do Cloud falhará.
