...
<Accordion title="agx">
  [agx](https://github.com/agnosticeng/agx) é uma aplicação para desktop criada com Tauri e SvelteKit que fornece uma interface moderna para explorar e consultar dados usando o mecanismo de banco de dados incorporado do ClickHouse (chdb).
...
<Accordion title="HouseOps">
  ...
  * Construtor de consultas com realce de sintaxe. Visualize a resposta em uma tabela ou em JSON.
  * Exporte os resultados das consultas como CSV ou JSON.
  * Lista de processos com descrições. Modo de escrita. Possibilidade de interromper um processo (`KILL`).
  ...
  * Monitoramento de tabelas replicadas e do Kafka.
</Accordion>
...
<Accordion title="LightHouse">
  ...
  * Visualização de tabelas com filtragem e ordenação.
  * Execução de consultas somente para leitura.
</Accordion>
...
<Accordion title="Grafana">
  ...
  &quot;O Grafana permite consultar, visualizar, criar alertas e entender suas métricas, independentemente de onde estejam armazenadas. Crie, explore e compartilhe painéis com sua equipe e promova uma cultura orientada por dados. Aprovado e adorado pela comunidade&quot; — grafana.com.
...
<Accordion title="DBeaver">
  ...
  * Desenvolvimento de consultas com realce de sintaxe e preenchimento automático.
...
<Accordion title="Grafo de tabelas do ClickHouse">
  [ClickHouse table graph](https://github.com/mbaksheev/clickhouse-table-graph) é uma ferramenta de CLI simples para visualizar dependências entre tabelas do ClickHouse. Ela obtém as conexões entre tabelas a partir da tabela `system.tables` e cria um fluxograma de dependências no formato [mermaid](https://mermaid.js.org/syntax/flowchart.html). Com essa ferramenta, você pode visualizar facilmente as dependências entre tabelas e entender o fluxo de dados no seu banco de dados ClickHouse. Graças ao mermaid, o fluxograma resultante é visualmente atraente e pode ser facilmente adicionado à sua documentação em Markdown.
</Accordion>
...
<Accordion title="MindsDB Studio">
  [MindsDB](https://mindsdb.com/) é uma camada de IA de código aberto para bancos de dados, incluindo o ClickHouse, que permite desenvolver, treinar e implantar modelos avançados de machine learning sem esforço. O MindsDB Studio (GUI) permite treinar novos modelos a partir de dados do banco de dados, interpretar previsões feitas pelo modelo, identificar possíveis vieses nos dados e avaliar e visualizar a precisão do modelo usando a função de IA explicável, para adaptar e ajustar seus modelos de machine learning mais rapidamente.
</Accordion>
...
<Accordion title="DBM">
  ...
  * Suporte a histórico de consultas (paginação, limpar tudo etc.)
...
<Accordion title="clickhouse-monitoring">
  [clickhouse-monitoring](https://github.com/duyet/clickhouse-monitoring) é um dashboard simples em Next.js que usa as tabelas `system.*` para ajudar a monitorar e fornecer uma visão geral do seu cluster do ClickHouse.
  ...
  * Informações sobre tabelas e partes: tamanho, contagem de linhas, compressão, tamanho das partes etc., com detalhamento por coluna.
  * Ferramentas úteis: exploração de dados do ZooKeeper, EXPLAIN de consultas, interrupção de consultas etc.
  * Gráficos para visualização de métricas: consultas e uso de recursos, número de merges/mutações, desempenho de merges, desempenho de consultas etc.
</Accordion>
...
<Accordion title="Telescope">
  [Telescope](https://iamtelescope.net/) é uma interface web moderna para explorar logs armazenados no ClickHouse. Ele fornece uma UI fácil de usar para consultar, visualizar e gerenciar dados de logs com controle de acesso granular.

  Recursos:

  * UI limpa e responsiva com filtros poderosos e seleção de campos personalizável.
  ...
  * Suporte opcional a consultas `WHERE` em SQL bruto para filtragem avançada (com verificações de permissão).
  * Visualizações salvas: salve e compartilhe configurações personalizadas da UI para consultas e layout.
  * Controle de acesso baseado em funções (RBAC) e integração de autenticação do GitHub.
...
<Accordion title="clickhouse-flow">
  [clickhouse-flow](https://github.com/MikeAmputer/clickhouse-flow) é uma ferramenta de código aberto para visualizar fluxos de dados e dependências entre tabelas, views e visões materializadas do ClickHouse.
...
<Accordion title="Chadmin">
  [Chadmin](https://github.com/bun4uk/chadmin) é uma UI simples que permite visualizar as consultas em execução no cluster ClickHouse, ver informações sobre elas e encerrá-las, se desejar.
</Accordion>

<Accordion title="TABLUM.IO">
  [TABLUM.IO](https://tablum.io/) — uma ferramenta online de consultas e análises para ETL e visualização. Permite conectar-se ao ClickHouse, consultar dados por meio de um console SQL versátil e carregar dados de arquivos estáticos e serviços de terceiros. O TABLUM.IO pode visualizar os resultados dos dados em gráficos e tabelas.
