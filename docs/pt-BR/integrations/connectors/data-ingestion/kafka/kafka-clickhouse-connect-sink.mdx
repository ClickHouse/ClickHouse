...
Os seguintes schemas Protobuf não são suportados pelo conector:
...
A partir da versão 26.1 do CH, esse schema é suportado quando `allow_experimental_nullable_tuple_type=1` (consulte [esta página da documentação](/pt-BR/reference/settings/session-settings/allow-experimental#allow_experimental_nullable_tuple_type)).
