...
A partir da versão 26.9 do ClickHouse, esse schema é suportado por padrão. Nas versões 26.1 a 26.8, ele requer a configuração `allow_experimental_nullable_tuple_type = 1` (consulte [esta página da documentação](/pt-BR/reference/settings/session-settings/allow-experimental#allow_experimental_nullable_tuple_type)).
