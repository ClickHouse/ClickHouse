...
description: 'Status do adapter do ClickHouse no dbt Core v2, no dbt Fusion e na plataforma dbt'
keywords: ['dbt', 'dbt core v2', 'dbt v2', 'dbt fusion', 'dbt platform']
title: 'dbt Core v2, Fusion e Platform'
doc_type: 'reference'
---
...
O dbt está sendo reconstruído sobre um novo motor, e o ClickHouse faz parte disso desde o início. Agora você pode executar o dbt sobre o ClickHouse no novo motor em Rust (dbt Core v2 e dbt Fusion) e, pela primeira vez, pode se conectar ao ClickHouse a partir da plataforma dbt com o connector v2. Seu projeto não muda: os mesmos models, testes e profiles que hoje rodam no dbt Core 1.x rodam no novo motor; apenas o binary é diferente.

O ClickHouse é o primeiro adapter da comunidade no dbt v2 e na plataforma dbt. Os adapters próprios do dbt estão sendo lançados junto com o nosso; somos os primeiros de fora da dbt Labs. O adapter Python para dbt Core 1.x, `dbt-clickhouse`, não será descontinuado: nós o mantemos atualizado com os lançamentos do dbt 1.x e mantemos os dois adapters em paralelo.

<Warning>
  **Não está pronto para produção.** O motor dbt Core v2, seu adapter para ClickHouse e a conexão com a plataforma dbt ainda não estão generally available (GA). Use-os apenas em projetos de desenvolvimento ou staging.
</Warning>

<Note>
  **A documentação do dbt Core 1.x também se aplica ao dbt Core v2.** O adapter se comporta da mesma forma nos dois motores: os profiles, as configurações de model, as materializations e os macros descritos nas outras páginas do dbt funcionam sem alterações no dbt Core v2, no dbt Fusion e na plataforma dbt. As exceções estão documentadas na tabela de [paridade entre v1 e v2](#parity) abaixo.
</Note>

O status em tempo real é acompanhado em [ClickHouse/dbt-clickhouse#660](https://github.com/ClickHouse/dbt-clickhouse/issues/660) (paridade com o dbt Core v2, um sub-issue por área) e [ClickHouse/dbt-clickhouse#555](https://github.com/ClickHouse/dbt-clickhouse/issues/555) (dbt Fusion).
...
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **dbt Core 1.x** | O motor do dbt baseado em Python, open source (Apache 2.0). Nosso adapter é o `dbt-clickhouse`, mantido na [organização ClickHouse no GitHub](https://github.com/ClickHouse/dbt-clickhouse) e instalado junto com o `dbt-core`.                                                                                                                              | **GA, mantido.** Continua recebendo lançamentos em paralelo com a v2.                                                                                                                                                                                                                                                                                                                               |
| **dbt Core v2**  | Reescrita completa do motor do dbt em Rust, open source (Apache 2.0). Essa nova versão inclui todas as features da v1, além de diversas melhorias de desempenho.                                                                                                                                                                                             | **Beta.** Consulte o [guia de atualização para o dbt v2](https://docs.getdbt.com/docs/dbt-versions/core-upgrade/upgrading-to-v2) e a [página de configuração do ClickHouse para a v2](https://docs.getdbt.com/docs/local/connect-data-platform/clickhouse-setup?version=2) na documentação do dbt. O adapter do ClickHouse já vem incluído no binary do dbt; não há package separado para instalar. |
| **dbt Fusion**   | dbt v2 mais features adicionais da dbt Labs: compreensão de SQL, análise estática, LSP, uma extensão para o VS Code e reconhecimento de colunas. A inteligência de SQL específica de cada dialect é construída por database. Closed source, licenciado sob o [dbt Product Licensing Agreement](https://www.getdbt.com/dbt-fusion-engine-license-agreement). | **Beta.** O adapter funciona com o binary do Fusion, mas as novas features de SQL ainda não estão disponíveis para o ClickHouse.                                                                                                                                                                                                                                                                    |
...
Avaliamos o dbt Core v2 em relação ao test suite de integração do dbt Core 1.x do `dbt-clickhouse`. Beta significa que os workflows principais do dbt e o conjunto de recursos do ClickHouse Cloud funcionam dentro dos limites documentados; a paridade total com a v1 e o suporte a cluster autogerenciado virão depois.
...
* **Inteligência de ClickHouse SQL no Fusion** (validação com reconhecimento de dialect, análise estática, LSP, reconhecimento de colunas na extensão do VS Code). Atualmente, o adapter é executado com o binary do Fusion, mas ainda não inclui esses recursos. Até lá, a análise estática é forçada para `off` em projetos ClickHouse e o motor não grava linhagem em nível de coluna. Acompanhado em [#736](https://github.com/ClickHouse/dbt-clickhouse/issues/736).
* **Métodos de autenticação adicionais** além de username e password na plataforma dbt.

## Paridade entre v1 e v2 {#parity}

As tabelas abaixo comparam os recursos disponíveis no `dbt-clickhouse` (v1) com seu status no adapter do dbt Core v2. Todos os recursos listados são compatíveis na v1, salvo indicação em contrário nas notas; a coluna de status refere-se ao dbt Core v2. A evolução de cada item pendente é acompanhada em [ClickHouse/dbt-clickhouse#660](https://github.com/ClickHouse/dbt-clickhouse/issues/660).
...
| Feature                                        | O que faz                                                        | dbt Core v2                             | Notas                                                                                                                                          |
| ---------------------------------------------- | ---------------------------------------------------------------- | --------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
...
| Recurso                            | O que faz                                           | dbt Core v2                             | Observações                                                                                                                                                                                                                                      |
| ---------------------------------- | --------------------------------------------------- | --------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
...
| Recurso                                 | O que faz                                                                             | dbt Core v2                             | Notas                                                                                                                                                                                                                                       |
| --------------------------------------- | ------------------------------------------------------------------------------------- | --------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
