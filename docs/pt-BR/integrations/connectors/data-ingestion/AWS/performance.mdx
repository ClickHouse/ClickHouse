...
Esta seção se concentra em otimizar o desempenho da leitura e inserção de dados no S3 usando as [funções de tabela S3](/pt-BR/reference/functions/table-functions/s3).
...
```bash
① Extrair e analisar a próxima porção de dados e formar um bloco de dados em memória (um por chave de particionamento) a partir dela.

② Gravar o bloco em uma nova parte no armazenamento.

Ir para ① 
```

Em ①, o tamanho depende do tamanho do bloco de insert, que pode ser controlado com duas configurações:
...
Quando o número especificado de linhas for coletado no bloco de insert, ou a quantidade de dados configurada for atingida (o que ocorrer primeiro), isso acionará a gravação do bloco em uma nova parte. O loop de insert continua no passo ①.
...
<Image img="/images/integrations/data-ingestion/s3/insert_threads.webp" size="lg" border alt="Threads de insert paralelas no ClickHouse" />

Até que todos os dados de todos os arquivos sejam processados, cada thread de insert executa um loop:

```bash
① Obter a próxima porção de dados de arquivo não processados (o tamanho da porção é baseado no tamanho de bloco configurado) e criar um bloco de dados em memória a partir dela.

② Gravar o bloco em uma nova part no armazenamento.
...
Com o tempo, o escalonamento horizontal costuma se tornar necessário devido à disponibilidade de hardware e ao melhor custo-benefício. No ClickHouse Cloud, clusters de produção têm pelo menos 3 nós. Por isso, talvez você também queira utilizar todos os nós para um insert.
