...
O número dessas threads paralelas de inserção pode ser configurado pela setting [`max_insert_threads`](/pt-BR/reference/settings/session-settings/max-insert#max_insert_threads). O valor padrão é `1` no ClickHouse open-source e 4 no [ClickHouse Cloud](https://clickhouse.com/cloud).
