...
A construção padrão `values()` do SQLAlchemy é compilada para a sintaxe da função de tabela `VALUES` do ClickHouse, inclusive quando usada em uma expressão de tabela comum (a forma CTE exige o SQLAlchemy 2.x, no qual `Values.cte()` foi adicionada).
