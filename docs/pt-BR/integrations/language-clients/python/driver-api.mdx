...
| `show_clickhouse_errors`   | bool ou None                  | `True`                                            | Inclui detalhes de erro do servidor e nomes simbólicos de exceções nas exceções do cliente.                                                                                                       |
...
O módulo `clickhouse_connect.dbapi` implementa a interface de conexão e cursor do PEP 249. Ele declara nível de API 2.0, `threadsafety=2` e `paramstyle="pyformat"`.
...
Exceções personalizadas, incluindo a hierarquia de exceções da DB-API 2.0, são definidas em `clickhouse_connect.driver.exceptions`. `DatabaseError` e `OperationalError` expõem um atributo numérico `code` com o código de erro do ClickHouse e um atributo `name` com o nome simbólico, como `UNKNOWN_TABLE`, para que os aplicativos possam tomar decisões com base em `exc.code` em vez de analisar a mensagem. `code` é definido mesmo quando `show_clickhouse_errors` está desabilitado, enquanto `name` depende disso. Ambos são `None` quando indisponíveis, como em erros de transporte. Uma falha de conexão durante a leitura de um resultado de consulta gera `StreamFailureError` em vez de retornar um resultado truncado.
