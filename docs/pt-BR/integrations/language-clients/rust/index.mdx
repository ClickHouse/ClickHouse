...
* As linhas são enviadas progressivamente em fluxo para distribuir a carga na rede.
* O ClickHouse realiza inserts em lote de forma atômica somente se todas as linhas couberem na mesma partição e se o número delas for menor que [`max_insert_block_size`](/pt-BR/reference/settings/session-settings/max-insert#max_insert_block_size).
