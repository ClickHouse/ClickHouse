...
slug: /pt-BR/concepts/features/security/external-authenticators/http
title: 'HTTP'
doc_type: 'referência'
---
...
<div id="http-auth-server-definition">
  ## Definição de servidor de autenticação HTTP {#http-auth-server-definition}
</div>
...
<div id="enabling-http-auth-using-sql">
  ### Habilitando a autenticação HTTP com SQL {#enabling-http-auth-using-sql}
</div>
...
<div id="passing-session-settings">
  ### Passando configurações de sessão {#passing-session-settings}
</div>
