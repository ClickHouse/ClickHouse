...
| Claim de roles (opcional) | O nome do claim do token que contém as roles do ClickHouse para o usuário efêmero. Deixe em branco para usar o nome de claim padrão `clickhouse:roles`. O claim deve conter um array JSON de nomes de roles, por exemplo, `["analyst", "reader"]`.                                                                                                                            |

As roles são lidas do claim de roles acima, e os grants SQL são lidos por padrão do claim `clickhouse:grants` do token. As roles especificadas no claim de roles já devem existir no serviço (crie-as com `CREATE ROLE`). O limite de permissões restringe os direitos de acesso efetivos provenientes dos claims de roles e de grants, conforme descrito em [Direitos de acesso](#access-rights).
