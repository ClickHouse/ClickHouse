...
<EnterprisePlanFeatureBadge feature="Autenticação JWT com um provedor de identidade personalizado" support="true" />

Além do autenticador integrado, você pode configurar seu serviço ClickHouse Cloud para aceitar JWTs emitidos pelo seu próprio provedor de identidade (por exemplo, Microsoft Entra ou Okta). Este é um recurso beta, disponível no plano Enterprise para serviços que executam o ClickHouse versão 26.4 ou posterior. Para habilitá-lo, abra um [chamado de suporte](https://clickhouse.com/support/program) com os seguintes detalhes de configuração:

| Parâmetro                | Descrição                                                                                                                                                                                                                                                                                                                                                                          |
| ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ID do serviço            | O UUID do serviço ClickHouse Cloud no qual a autenticação JWT será habilitada. Você pode encontrá-lo na URL do Cloud Console ao abrir o serviço.                                                                                                                                                                                                                                   |
...
| URL do JWKS              | O endpoint HTTPS no qual seu provedor de identidade publica seu JSON Web Key Set (por exemplo, `https://idp.example.com/.well-known/jwks.json`). O ClickHouse obtém as chaves públicas desse endpoint para verificar as assinaturas dos tokens. A verificação de JWKS funciona apenas com chaves RSA (`RS256`), conforme descrito em [Claims obrigatórias](#required-claims). |
| Claim de roles    | O nome do claim do token que contém as roles do ClickHouse para o usuário efêmero (o padrão é `clickhouse:roles`). O claim deve conter um array JSON de nomes de roles, por exemplo, `["analyst", "reader"]`.                                                                                                                                                        |
| Claim de grants | O nome do claim do token que contém fragmentos SQL `GRANT` para o usuário efêmero (o padrão é `clickhouse:grants`). O claim deve conter um array JSON de fragmentos de grants, por exemplo, `["SELECT ON analytics.*", "INSERT ON analytics.events"]`.                                                                                                               |

As roles especificadas no claim de roles já devem existir no serviço (crie-as com `CREATE ROLE`). O limite de permissões restringe os direitos de acesso efetivos provenientes dos claims de roles e de grants, conforme descrito em [Direitos de acesso](#access-rights).
