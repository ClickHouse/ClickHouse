...
* **(3)** Se a tabela da direita couber na memória, use o hash join (`hash` e `parallel_hash` são aliases; escolher um não controla o paralelismo). Essa costuma ser a opção em memória mais rápida, dependendo do volume de dados, dos tipos de dados e da distribuição dos valores das colunas da chave de junção.
