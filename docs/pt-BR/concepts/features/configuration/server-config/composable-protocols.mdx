...
a [configuração do servidor](/pt-BR/operations/server-configuration-parameters/settings) `default_session_user`, cujo valor padrão é `default`.
...
protocolo `interserver`: as conexões interserver são autenticadas pelo Secret do cluster
