...
<Note>
  **Consultas excluídas da contabilização de QUOTA**

  Para permitir que a configuração e o uso da QUOTA sejam inspecionados mesmo quando uma QUOTA estiver esgotada, consultas `SELECT` que leem exclusivamente de uma das seguintes tabelas de sistema não são contabilizadas nas QUOTAs e não estão sujeitas a limites de complexidade de consulta:
...
No processamento de consultas distribuídas, os valores acumulados são armazenados no servidor que faz a solicitação. Portanto, se o usuário acessar outro servidor, a QUOTA nele vai &quot;recomeçar&quot;.
