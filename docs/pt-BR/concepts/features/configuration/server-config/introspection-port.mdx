...
Uma porta de introspecção é um endpoint de [protocolo componível](/pt-BR/operations/settings/composable-protocols) marcado com `<introspection>true</introspection>`. Por padrão, nenhuma porta de introspecção é configurada:
