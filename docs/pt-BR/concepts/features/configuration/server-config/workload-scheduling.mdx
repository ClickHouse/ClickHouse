...
Para conseguir usar toda a capacidade do recurso subjacente, você deve usar `inflight_limit`. Observe que um número baixo de `max_requests` ou `max_cost` pode fazer com que a utilização do recurso não seja total, enquanto números muito altos podem levar a filas vazias dentro do scheduler, o que, por sua vez, fará com que as políticas sejam ignoradas (injustiça ou ignorar prioridades) na subárvore. Por outro lado, se você quiser proteger os recursos contra uma utilização excessiva, deve usar `bandwidth_limit`. Ele aplica limitação quando a quantidade de recurso consumida em `duration` segundos excede `max_burst + max_speed * duration` bytes. Dois nós `bandwidth_limit` no mesmo recurso podem ser usados para limitar a largura de banda de pico durante intervalos curtos e a largura de banda média em intervalos mais longos.

<div id="workload-classifiers">
  ### Classificadores de carga de trabalho obsoletos
</div>
