...
<Warning>
  Mantenha `readonly` fora da lista `changeable_in_readonly` em todos os perfis e lembre-se de que as restrições de perfil valem para todo o sistema, e não por usuário.
</Warning>

Não marque o próprio `readonly` como `changeable_in_readonly`. Se fizer isso, uma sessão que inicia com `readonly = 1` poderá executar `SET readonly = 0` e voltar a executar as consultas de escrita que seus privilégios já permitem.

Isso vale para todos os casos, não apenas para o perfil atribuído ao usuário:

* Qualquer sessão pode alternar para qualquer perfil via `SET profile`, já que a seleção de perfil não passa por verificação de acesso (só é limitada pelas CONSTRAINTs de settings já ativas).
* Via HTTP, requisições `GET` são forçadas a `readonly = 2` apenas quando o valor efetivo seria `0`. Portanto, um perfil que define `readonly = 1` mas também permite alterar `readonly` em modo somente leitura anula essa proteção — uma requisição `GET` pode mudá-lo para `0` e gravar dados.
