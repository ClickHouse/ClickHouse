...
<Note>
  O cache de condições de consulta só funciona quando [enable&#95;analyzer](/pt-BR/reference/settings/session-settings/allow-experimental#allow_experimental_analyzer) está definido como true, que é o valor padrão.
</Note>
...
<div id="top-k-queries">
  ### Consultas `ORDER BY ... LIMIT n` (TopK)
</div>

As consultas que usam a otimização TopK (`ORDER BY <column> LIMIT n`, aceleradas por filtragem dinâmica ou por um índice de salto minmax na coluna de ordenação) não usam o cache de condições de consulta por padrão: essas leituras não o consultam nem o preenchem.
As leituras TopK podem ignorar grânulos com base em um limite que muda durante a execução da consulta, portanto, suas entradas no cache exigem atenção adicional; elas são habilitadas pela configuração separada [`use_query_condition_cache_for_top_k`](/pt-BR/reference/settings/session-settings/use-query) (desabilitada por padrão) até que a validade dessas entradas seja plenamente comprovada.
A configuração só tem efeito se `use_query_condition_cache` também estiver habilitada.
