...
<Steps>
  ...
  <Step title="Verifique a configuração do servidor" id="server-config">
    ...
    ```xml
    <!-- Log de rastreamento. Armazena stack traces coletados pelos profilers de consulta.
  ...
  <Step title={<>Analise a tabela de sistema <code>trace_log</code></>} id="analyze-trace-log-system-table">
    Para analisar a tabela de sistema `trace_log`, habilite as funções de introspecção com a configuração [`allow_introspection_functions`](/pt-BR/reference/settings/session-settings/allow#allow_introspection_functions):
