...
<Note>
  Apenas os endpoints de consulta `/api/v1/query` e `/api/v1/query_range` e o endpoint de metadados de séries `/api/v1/series` estão implementados. `/api/v1/series` requer pelo menos um seletor de séries `match[]`, oferece suporte aos parâmetros opcionais `start`, `end` e `limit` e retorna a união das séries correspondentes a cada seletor. Os endpoints de metadados de rótulos usados por uma fonte de dados Prometheus no Grafana para navegar por rótulos, variáveis de Template e preenchimento automático no construtor de consultas (`/api/v1/labels`, `/api/v1/label/<name>/values`) não estão implementados e retornam um erro. Escreva expressões PromQL no modo de código em vez de usar o construtor de consultas.
</Note>
