...
export const globalTransformSource = (source) => {
  ...
  if (isPreview || /clickhouse\.com\/docs(\/|$)/.test(url)) {
    tabs.push('Documentação');
  } else if (url.indexOf('github.com') !== -1 && /\/issues(\/|$)/.test(url)) tabs.push('Documentação');
  else if (/\/blog(\/|$)/.test(url)) tabs.push('Documentação');
  else if (url.indexOf('clickhouse.com') !== -1) tabs.push('Documentação');
...
Pesquise em toda a documentação do ClickHouse e em outros recursos do ClickHouse.

<GlobalSearch placeholder="Pesquise na documentação do ClickHouse..." />