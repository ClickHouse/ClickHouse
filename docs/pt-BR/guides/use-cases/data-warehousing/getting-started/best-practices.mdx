...
A partir da versão 25.6, o ClickHouse lê Delta Lake no S3 e no GCS por meio do kernel do Delta Lake em Rust. A configuração chama-se [`allow_delta_kernel_rs`](/pt-BR/reference/settings/session-settings/allow#allow_delta_kernel_rs) na versão 26.8 e posteriores, e `allow_experimental_delta_kernel_rs` nas versões 25.5 a 26.7. No Azure Blob Storage, use [deltaLakeAzure()](/pt-BR/sql-reference/table-functions/deltalake) com o leitor legado, porque o kernel fica desabilitado nesse ambiente. Sem o kernel, a poda de partições, o Feed de dados de alterações e a leitura de versões de snapshot não estão disponíveis.
