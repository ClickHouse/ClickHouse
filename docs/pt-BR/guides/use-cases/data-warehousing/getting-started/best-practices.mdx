...
Para gravação, o Iceberg exige [allow&#95;insert&#95;into&#95;iceberg](/pt-BR/reference/settings/session-settings/allow#allow_insert_into_iceberg) (25.7+, Beta a partir da 26.2). Consulte [Gravando em lagos de dados](/pt-BR/use-cases/data-lake/getting-started/writing-data). O Delta Lake exige [allow&#95;delta&#95;lake&#95;writes](/pt-BR/reference/settings/session-settings/allow-experimental#allow_experimental_delta_lake_writes) (25.9+). A [matriz de suporte](/pt-BR/use-cases/data-lake/support-matrix) lista quais flags se aplicam a cada formato e operação.
...
Consulte [Gravando em lagos de dados](/pt-BR/use-cases/data-lake/getting-started/writing-data) e a [referência do motor Iceberg](/pt-BR/engines/table-engines/integrations/iceberg).

<div id="delta-lake-settings">
  ### Delta Lake
</div>

A partir da versão 25.6, o ClickHouse lê Delta Lake no S3 e no GCS por meio do kernel do Delta Lake em Rust ([allow&#95;experimental&#95;delta&#95;kernel&#95;rs](/pt-BR/reference/settings/session-settings/allow-experimental#allow_experimental_delta_kernel_rs), 25.5+). No Azure Blob Storage, use [deltaLakeAzure()](/pt-BR/sql-reference/table-functions/deltalake) com o leitor legado, porque o kernel fica desabilitado nesse ambiente. Sem o kernel, a poda de partições, o change data feed e a leitura de versões de snapshot não estão disponíveis.

<div id="delta-kernel">
  #### Delta Kernel
</div>

[allow&#95;experimental&#95;delta&#95;kernel&#95;rs](/pt-BR/reference/settings/session-settings/allow-experimental#allow_experimental_delta_kernel_rs) deve estar habilitado para poda de partições, change data feed e leitura de versões de snapshot. Ele vem habilitado por padrão no S3 e no GCS a partir da versão 25.5. Habilite-o explicitamente em versões mais antigas ou ao solucionar problemas:
...
Além de [allow&#95;delta&#95;lake&#95;writes](/pt-BR/reference/settings/session-settings/allow-experimental#allow_experimental_delta_lake_writes) (25.9+), controle o tamanho do arquivo de saída no insert:
...
| ----------------------------------------------------------------------------------------------- | ---------- | ----- | -------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------- |
| [system.iceberg&#95;metadata&#95;log](/pt-BR/operations/system-tables/iceberg_metadata_log)           | Iceberg    | 25.9  | [iceberg&#95;metadata&#95;log&#95;level](/pt-BR/reference/settings/session-settings/iceberg-metadata#iceberg_metadata_log_level) na consulta | Rastrear arquivos de metadados lidos e decisões de poda de partições        |
...
| [system.delta&#95;lake&#95;metadata&#95;log](/pt-BR/operations/system-tables/delta_lake_metadata_log) | Delta Lake | 25.10 | [delta&#95;lake&#95;log&#95;metadata](/pt-BR/reference/settings/session-settings/delta-lake#delta_lake_log_metadata) = `1` na consulta       | Rastrear arquivos de metadados do Delta e a resolução de snapshots          |
