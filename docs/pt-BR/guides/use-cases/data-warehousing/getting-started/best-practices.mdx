...
Use a variante S3 para AWS S3 e GCS. O Azure e o sistema de arquivos local têm variantes dedicadas (`icebergAzure`, `icebergLocal` e equivalentes para outros formatos). Consulte [Consultar diretamente](/pt-BR/guides/use-cases/data-warehousing/getting-started/querying-directly) para ver a lista completa.

[Paimon](/pt-BR/reference/functions/table-functions/paimon) oferece apenas funções de tabela.
...
Para gravação, o Iceberg exige [allow&#95;insert&#95;into&#95;iceberg](/pt-BR/reference/settings/session-settings#allow_insert_into_iceberg) (25.7+, Beta a partir da 26.2). Consulte [Gravando em lagos de dados](/pt-BR/guides/use-cases/data-warehousing/getting-started/writing-data). O Delta Lake exige [allow&#95;delta&#95;lake&#95;writes](/pt-BR/reference/settings/session-settings#allow_experimental_delta_lake_writes) (25.9+). A [matriz de suporte](/pt-BR/guides/use-cases/data-warehousing/support-matrix) lista quais flags se aplicam a cada formato e operação.
...
| ------------------------------------------------------------------------------------------------------------------ | ----- | -------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| [use&#95;iceberg&#95;partition&#95;pruning](/pt-BR/reference/settings/session-settings#use_iceberg_partition_pruning)           | 25.1  | `1` a partir da 25.6 | Ignora arquivos de dados usando metadados de partição nos manifests                                                                                |
| [use&#95;iceberg&#95;metadata&#95;files&#95;cache](/pt-BR/reference/settings/session-settings#use_iceberg_metadata_files_cache) | 25.4  | `1`                  | Armazena em cache, na memória, listas de manifests e JSON de metadados                                                                             |
| [iceberg&#95;metadata&#95;staleness&#95;ms](/pt-BR/reference/settings/session-settings#iceberg_metadata_staleness_ms)           | 26.3  | `0`                  | Configuração de consulta. Usa metadados em cache quando estiverem mais recentes do que esta janela, em vez de consultar o catálogo a cada consulta |
...
2. Defina [iceberg&#95;metadata&#95;staleness&#95;ms](/pt-BR/reference/settings/session-settings#iceberg_metadata_staleness_ms) (26.3+) nas consultas para aceitar metadados ligeiramente desatualizados e, assim, evitar a ida e volta ao catálogo.
...
Além de [allow&#95;insert&#95;into&#95;iceberg](/pt-BR/reference/settings/session-settings#allow_insert_into_iceberg) (25.7+, Beta a partir da 26.2), controle o tamanho do arquivo de saída e o número de partições na inserção:

| Configuração                                                                                                                         | Desde | Finalidade                                         |
| ------------------------------------------------------------------------------------------------------------------------------------ | ----- | -------------------------------------------------- |
| [iceberg&#95;insert&#95;max&#95;rows&#95;in&#95;data&#95;file](/pt-BR/reference/settings/session-settings#iceberg_insert_max_rows_in_data_file)   | 25.9  | Limite de linhas por arquivo de dados de saída     |
| [iceberg&#95;insert&#95;max&#95;bytes&#95;in&#95;data&#95;file](/pt-BR/reference/settings/session-settings#iceberg_insert_max_bytes_in_data_file) | 25.9  | Limite de bytes por arquivo de dados de saída      |
| [iceberg&#95;insert&#95;max&#95;partitions](/pt-BR/reference/settings/session-settings#iceberg_insert_max_partitions)                             | 25.12 | Limite de partições gravadas em uma única inserção |

Consulte [Gravação em lagos de dados](/pt-BR/guides/use-cases/data-warehousing/getting-started/writing-data) e a [referência do motor Iceberg](/pt-BR/reference/engines/table-engines/integrations/iceberg).

<div id="delta-lake-settings">
  ### Delta Lake
</div>

A partir da versão 25.6, o ClickHouse lê Delta Lake no S3 e no GCS por meio do kernel do Delta Lake em Rust ([allow&#95;experimental&#95;delta&#95;kernel&#95;rs](/pt-BR/reference/settings/session-settings#allow_experimental_delta_kernel_rs), 25.5+). No Azure Blob Storage, use [deltaLakeAzure()](/pt-BR/reference/functions/table-functions/deltalake) com o leitor legado, porque o kernel fica desabilitado nesse ambiente. Sem o kernel, a poda de partições, o change data feed e a leitura de versões de snapshot não estão disponíveis.

<div id="delta-kernel">
  #### Delta Kernel
</div>

[allow&#95;experimental&#95;delta&#95;kernel&#95;rs](/pt-BR/reference/settings/session-settings#allow_experimental_delta_kernel_rs) deve estar habilitado para poda de partições, change data feed e leitura de versões de snapshot. Ele vem habilitado por padrão no S3 e no GCS a partir da versão 25.5. Habilite-o explicitamente em versões mais antigas ou ao solucionar problemas:
...
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- | ------ | ------------------------------------------------------------------------------------------- |
| [delta&#95;lake&#95;enable&#95;engine&#95;predicate](/pt-BR/reference/settings/session-settings#delta_lake_enable_engine_predicate)                                                                                                                  | 25.8  | `1`    | Encaminha filtros ao kernel para poda de partições. Requer [Delta Kernel](#delta-kernel)    |
| [delta&#95;lake&#95;reload&#95;schema&#95;for&#95;consistency](/pt-BR/reference/settings/session-settings#delta_lake_reload_schema_for_consistency)                                                                                                  | 26.3  | `0`    | Recarrega o esquema antes de cada consulta quando gravadores concorrentes alteram o esquema |
| [delta&#95;lake&#95;snapshot&#95;start&#95;version](/pt-BR/reference/settings/session-settings#delta_lake_snapshot_start_version) / [delta&#95;lake&#95;snapshot&#95;end&#95;version](/pt-BR/reference/settings/session-settings#delta_lake_snapshot_end_version) | 25.12 | `-1`   | Lê alterações de CDF entre duas versões de snapshot. Requer CDF habilitado na origem        |
| [delta&#95;lake&#95;snapshot&#95;version](/pt-BR/reference/settings/session-settings#delta_lake_snapshot_version)                                                                                                                                    | 25.8  | `-1`   | Lê um único snapshot histórico. Defina `-1` para o mais recente (`0` é válido)              |
...
Para ler apenas as linhas alteradas entre dois snapshots do Delta, defina [delta&#95;lake&#95;snapshot&#95;start&#95;version](/pt-BR/reference/settings/session-settings#delta_lake_snapshot_start_version) e [delta&#95;lake&#95;snapshot&#95;end&#95;version](/pt-BR/reference/settings/session-settings#delta_lake_snapshot_end_version) (25.12+). A tabela deve ter o change data feed habilitado na origem (`delta.enableChangeDataFeed`). Defina as versões inicial e final nas configurações da consulta. Definir apenas a versão final gera um erro.
...
Além de [allow&#95;delta&#95;lake&#95;writes](/pt-BR/reference/settings/session-settings#allow_experimental_delta_lake_writes) (25.9+), controle o tamanho do arquivo de saída no insert:

| Configuração                                                                                                                                   | Desde | Finalidade                                     |
| ---------------------------------------------------------------------------------------------------------------------------------------------- | ----- | ---------------------------------------------- |
| [delta&#95;lake&#95;insert&#95;max&#95;rows&#95;in&#95;data&#95;file](/pt-BR/reference/settings/session-settings#delta_lake_insert_max_rows_in_data_file)   | 25.9  | Limite de linhas por arquivo de dados de saída |
| [delta&#95;lake&#95;insert&#95;max&#95;bytes&#95;in&#95;data&#95;file](/pt-BR/reference/settings/session-settings#delta_lake_insert_max_bytes_in_data_file) | 25.9  | Limite de bytes por arquivo de dados de saída  |
...
Se as tabelas do catálogo não aparecerem em `system.tables`, habilite [show&#95;remote&#95;databases&#95;in&#95;system&#95;tables](/pt-BR/reference/settings/session-settings#show_remote_databases_in_system_tables) (25.8+). Por padrão, as tabelas do catálogo ficam ocultas na introspecção do sistema. Em versões anteriores à 26.6, use o nome antigo, `show_data_lake_catalogs_in_system_tables`.
...
Compare `read_rows` e `read_bytes` em [system.query&#95;log](/pt-BR/reference/system-tables/query_log) antes e depois de adicionar filtros ou ajustar configurações. ProfileEvents como `ReadBufferFromS3Bytes` e `CachedReadBufferReadFromCacheBytes` mostram quanto dos dados veio do armazenamento de objetos em comparação com o cache local. Consulte [Otimização de consultas](/pt-BR/guides/clickhouse/performance-and-monitoring/query-optimization) para um passo a passo completo sobre o query&#95;log e o EXPLAIN.

Desative [enable&#95;filesystem&#95;cache](/pt-BR/reference/settings/session-settings#enable_filesystem_cache) durante o benchmarking para que os acertos de cache não mascarem as mudanças entre execuções.
...
| ----------------------------------------------------------------------------------------------- | ---------- | ----- | -------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------- |
| [system.iceberg&#95;metadata&#95;log](/pt-BR/reference/system-tables/iceberg_metadata_log)           | Iceberg    | 25.9  | [iceberg&#95;metadata&#95;log&#95;level](/pt-BR/reference/settings/session-settings#iceberg_metadata_log_level) na consulta | Rastrear arquivos de metadados lidos e decisões de poda de partições    |
...
| [system.delta&#95;lake&#95;metadata&#95;log](/pt-BR/reference/system-tables/delta_metadata_log) | Delta Lake | 25.10 | [delta&#95;lake&#95;log&#95;metadata](/pt-BR/reference/settings/session-settings#delta_lake_log_metadata) = `1` na consulta | Rastrear arquivos de metadados do Delta e a resolução de snapshots      |
...
Os níveis de log Verbose do Iceberg desativam o cache de metadados para listas de manifest e arquivos, o que torna mais lentas as consultas subsequentes na mesma tabela. Use alta verbosidade apenas enquanto estiver investigando ativamente. Para problemas de predicado no Delta Lake, habilite [delta&#95;lake&#95;throw&#95;on&#95;engine&#95;predicate&#95;error](/pt-BR/reference/settings/session-settings#delta_lake_throw_on_engine_predicate_error) (25.8+) para falhar rapidamente quando o kernel não conseguir fazer pushdown de um filtro.
...
* [Primeiros passos](/pt-BR/guides/use-cases/data-warehousing/getting-started/overview) — Guia completo, da consulta direta à gravação de dados de volta
* [Consultando diretamente](/pt-BR/guides/use-cases/data-warehousing/getting-started/querying-directly) — Funções de tabela, motores e variantes de cluster para os quatro formatos
* [Conectando-se a catálogos](/pt-BR/guides/use-cases/data-warehousing/getting-started/connecting-catalogs) — Configuração do `DataLakeCatalog` com Unity Catalog
