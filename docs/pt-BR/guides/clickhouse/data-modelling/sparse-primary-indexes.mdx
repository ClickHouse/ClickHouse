...
slug: /pt-BR/guides/clickhouse/data-modelling/sparse-primary-indexes
...
<div id="a-full-table-scan">
  ### Varredura completa da tabela {#a-full-table-scan}
</div>
...
<div id="clickhouse-index-design">
  ## Design de índices no ClickHouse {#clickhouse-index-design}
</div>

<div id="an-index-design-for-massive-data-scales">
  ### Um design de índices para grandes escalas de dados {#an-index-design-for-massive-data-scales}
</div>
...
<div id="a-table-with-a-primary-key">
  ### Uma tabela com chave primária {#a-table-with-a-primary-key}
</div>
...
### Os dados são armazenados em disco ordenados pelas colunas da chave primária {#data-is-stored-on-disk-ordered-by-primary-key-columns}
...
### Os dados são organizados em grânulos para processamento paralelo de dados {#data-is-organized-into-granules-for-parallel-data-processing}
...
### O índice primário tem uma entrada por grânulo {#the-primary-index-has-one-entry-per-granule}
...
### O índice primário serve para selecionar grânulos {#the-primary-index-is-used-for-selecting-granules}
...
### Arquivos de marcação são usados para localizar grânulos {#mark-files-are-used-for-locating-granules}
...
<div id="using-multiple-primary-indexes">
  ## Como usar vários índices primários {#using-multiple-primary-indexes}
</div>

<a name="filtering-on-key-columns-after-the-first" />

### Colunas secundárias da chave podem (não) ser ineficientes {#secondary-key-columns-can-not-be-inefficient}
...
### Algoritmo de busca por exclusão genérica {#generic-exclusion-search-algorithm}
...
### Observação sobre índice de salto de dados {#note-about-data-skipping-index}
...
<div id="a-need-to-use-multiple-primary-indexes">
  ### A necessidade de usar vários índices primários {#a-need-to-use-multiple-primary-indexes}
</div>
...
### Opções para criar índices primários adicionais {#options-for-creating-additional-primary-indexes}
...
### Opção 1: Tabelas secundárias {#option-1-secondary-tables}
...
### Opção 2: Visões materializadas {#option-2-materialized-views}
...
### Opção 3: Projeções {#option-3-projections}
...
<div id="ordering-key-columns-efficiently">
  ## Ordenando com eficiência as colunas da chave {#ordering-key-columns-efficiently}
</div>
...
<div id="efficient-filtering-on-secondary-key-columns">
  ### Filtragem eficiente em colunas secundárias da chave {#efficient-filtering-on-secondary-key-columns}
</div>
...
### Taxa de compressão ideal dos arquivos de dados {#optimal-compression-ratio-of-data-files}
...
<div id="identifying-single-rows-efficiently">
  ## Identificando linhas individuais com eficiência {#identifying-single-rows-efficiently}
</div>
...
### Um exemplo concreto {#a-concrete-example}
