...
description: Saiba como resolver os avisos do Docker sobre as capacidades `CAP_IPC_LOCK` e `CAP_SYS_NICE` ao executar o ClickHouse em um contêiner.
date: 2023-06-07
...
Ao executar o ClickHouse no Docker, o Docker está reclamando da ausência das capacidades `CAP_IPC_LOCK` e `CAP_SYS_NICE` no sistema. Como posso resolver isso?

Veja abaixo como são as mensagens de log sobre a ausência da capacidade `CAP_SYS_NICE` ou `CAP_SYS_NICE`:
...
5. Reinicie o servidor ClickHouse, e as mensagens de log não devem mais ser exibidas.

<br />

Confira este [artigo sobre as capacidades do Linux](https://docs.docker.com/engine/reference/run/#runtime-privilege-and-linux-capabilities) para mais detalhes.