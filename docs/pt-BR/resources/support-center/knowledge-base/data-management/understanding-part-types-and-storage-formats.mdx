...
<div id="storage-formats-full-vs-packed">
  ## Formatos de armazenamento: Full vs Packed (ClickHouse Cloud)
</div>
...
| **Packed** | Todos os arquivos são agrupados em um único arquivo de arquivamento | somente ClickHouse Cloud |

<div id="full-storage">
  ### Armazenamento Full
</div>

No armazenamento Full, cada parte é composta por vários arquivos separados, armazenados individualmente em disco. Esta é a opção padrão e a única disponível na versão open source do ClickHouse.

<div id="packed-storage">
  ### Armazenamento Packed (somente no ClickHouse Cloud)
</div>
...
No ClickHouse Cloud, uma parte usa o armazenamento Packed se **qualquer** uma destas condições for true:
...
<Info>
  **Código aberto**
  ...
  Embora as configurações `min_bytes_for_full_part_storage`, `min_rows_for_full_part_storage` e `min_level_for_full_part_storage` estejam definidas no ClickHouse open source, elas não têm efeito porque a implementação do armazenamento Packed está disponível apenas no ClickHouse Cloud.
</Info>

A configuração `min_level_for_full_part_storage` pode ser usada para otimizar tanto o desempenho quanto os custos em ambientes do ClickHouse Cloud, especialmente para tabelas que recebem inserções contínuas.
...
* Quando as tabelas recebem ingestão regular de dados, as partes iniciais são mescladas rapidamente, o que torna desperdício armazená-las inicialmente no formato de parte completa
* Definir esse parâmetro evita requisições PUT ao S3 custosas durante as inserções. Por exemplo, uma análise mostrou que inserções que criavam partes completas tinham uma média de 31,3 requisições PUT por inserção, enquanto aquelas que criavam apenas partes packed tinham média de apenas 2,22 requisições PUT por inserção
...
Defina `min_level_for_full_part_storage = 2` para implantações no Cloud.
Isso garante que:

* Partes de nível 0 (inserções iniciais) usem armazenamento packed
* Partes de nível 1 continuem usando armazenamento packed
* Apenas partes no nível 2 ou superior usem o formato de armazenamento completo

<Tip>
  Evite essa configuração para tabelas que recebem gravações muito grandes, porém pouco frequentes, nas quais não ocorrem merges suficientes, pois gravações iniciais grandes podem se beneficiar imediatamente do armazenamento de parte completa.
</Tip>
...
Esses dois conceitos são independentes, e você pode usar qualquer combinação, dependendo de estar usando o ClickHouse Cloud ou o ClickHouse de código aberto:

| Combinação       | Caso de uso                                                     |
| ---------------- | --------------------------------------------------------------- |
| Wide + Full      | Partes grandes em armazenamento local (padrão no código aberto) |
| Wide + Packed    | Partes grandes em cloud storage                                 |
| Compact + Full   | Partes pequenas em armazenamento local                          |
| Compact + Packed | Partes pequenas em cloud storage                                |
