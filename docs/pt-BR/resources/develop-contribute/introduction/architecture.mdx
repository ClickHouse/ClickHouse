...
Para resolver problemas existentes nos interpretadores, foi desenvolvido um novo `InterpreterSelectQueryAnalyzer`. Esta é uma nova versão do `InterpreterSelectQuery`, que não usa o `ExpressionAnalyzer` e introduz uma camada adicional de abstração entre `AST` e `QueryPipeline`, chamada `QueryTree`. É a única análise de consulta desde a versão `26.9`; a configuração `enable_analyzer`, que antes permitia voltar ao `ExpressionAnalyzer`, está obsoleta e não pode mais ser desabilitada.
