...
<Steps>
  <Step title="Crie um serviço ClickHouse" id="1-create-a-clickhouse-service">
    ...
    * Faça login com o nome de usuário e a senha que você acabou de criar
    ...
    Após fazer login, o ClickHouse Cloud inicia o assistente de onboarding, que orienta você na criação de um novo serviço ClickHouse. Selecione a região em que ցանկանում implantar o serviço e dê um nome a ele:
  ...
  <Step title="Adicionar dados" id="3-add-data">
    ...
    * `metric`: um número de ponto flutuante de 32 bits ([Float32](/pt-BR/reference/data-types/float))
    ...
    #### Uma breve introdução às primary keys
    ...
    * as chaves primárias no ClickHouse ***não são exclusivas*** para cada linha de uma tabela
    ...
    A primary key também é a sorting key, que é uma tupla `(user_id, timestamp)`.  Portanto, os dados armazenados em cada
    column file serão ordenados por `user_id` e, em seguida, por `timestamp`.
    ...
    Você também pode se conectar ao seu ClickHouse Cloud service usando uma ferramenta de linha de comando chamada [**clickhouse client**](/pt-BR/concepts/features/interfaces/cli). Clique em `Connect` no menu à esquerda para acessar esses detalhes. Na caixa de diálogo, selecione `Native` na lista suspensa:
