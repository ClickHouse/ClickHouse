...
Mapeia os tipos `date` e `timestamp`/`timestamptz` do PostgreSQL para `Date32` e `DateTime64` do ClickHouse, que abrangem o intervalo de valores mais amplo desses tipos no PostgreSQL. Padrão: `1`.
Se definido como `0`, os tipos mais limitados `Date` e `DateTime` são usados em vez deles (valores fora do intervalo deles ou com precisão de subsegundos não podem ser representados).

Essa configuração controla apenas os tipos de coluna escolhidos pela inferência de tipos quando as tabelas aninhadas são criadas, portanto deve ser especificada no momento de `CREATE DATABASE`. Ela não pode ser alterada depois com `ALTER DATABASE ... MODIFY SETTING` (as tabelas aninhadas já criadas mantêm seus tipos de coluna fixos, e essa alteração é rejeitada); para mudá-la, recrie o banco de dados. Ela não se aplica ao motor de tabela `MaterializedPostgreSQL`, em que os tipos de coluna são declarados explicitamente.
...
1. [CREATE PUBLICATION](https://www.postgresql.org/docs/14/sql-createpublication.html) -- privilégio para executar a consulta de criação.
...
3. [pg&#95;drop&#95;replication&#95;slot](https://www.postgresql.org/docs/9.5/functions-admin.html#FUNCTIONS-REPLICATION) -- privilégio de replicação ou superusuário.

4. [DROP PUBLICATION](https://www.postgresql.org/docs/10/sql-droppublication.html) -- proprietário da publicação (`username` no próprio engine MaterializedPostgreSQL).
...
É possível fazer backup de um banco de dados `MaterializedPostgreSQL`. Os dados de cada tabela replicada ficam armazenados em uma tabela aninhada `ReplacingMergeTree`, portanto `BACKUP DATABASE` captura esses dados ao delegar para a tabela aninhada.

```sql
BACKUP DATABASE postgres_db TO Disk('backups', 'postgres_db.zip');
```

A restauração de um banco de dados ou tabela `MaterializedPostgreSQL` **no mesmo local não é suportada**. Um objeto `MaterializedPostgreSQL` restaurado começa imediatamente a replicar a partir da origem PostgreSQL ativa, portanto restaurar o snapshot do backup sobre ele misturaria o snapshot com o estado remoto atual. Por isso, o RESTORE é bloqueado por segurança nesse caso. Em vez disso, restaure os dados capturados em tabelas `ReplacingMergeTree` comuns:

* Em um backup de banco de dados, a definição armazenada de cada tabela já é a `ReplacingMergeTree` aninhada sintética (não a engine `MaterializedPostgreSQL`), portanto cada tabela pode ser restaurada diretamente em uma nova tabela, que ainda não exista:
...
* Para um backup de tabela `MaterializedPostgreSQL` isolada, a definição armazenada é a própria engine `MaterializedPostgreSQL`. Crie antes uma tabela `ReplacingMergeTree` com a mesma estrutura da tabela aninhada (incluindo as colunas `_sign` e `_version`) e restaure nela:
