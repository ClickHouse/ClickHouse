...
A forma como o resultado do dictionary scan é usado depende de onde a needle está ancorada:
...
* `value%` e `%value` ancoram a needle no valor inteiro, enquanto o dictionary scan só consegue ancorá-la em um token; assim, o scan retorna um superconjunto das linhas correspondentes e é usado como uma [leitura direta como hint](#direct-read).

O mesmo dictionary scan atende `startsWith(col, 'value')` e `endsWith(col, 'value')` quando a needle não possui um token completo para pesquisar.
...
Needles que abrangem vários tokens continuam usando os tokens completos da needle e dispensam o dictionary scan.
