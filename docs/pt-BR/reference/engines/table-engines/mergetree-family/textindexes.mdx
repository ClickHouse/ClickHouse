...
* `splitByRegexp(re)` divide strings usando um separador de expressão regular `re` definido pelo usuário (consulte a função [splitByRegexp](/pt-BR/reference/functions/regular-functions/splitting-merging-functions#splitByRegexp)).
  O argumento `re` é obrigatório; por exemplo, `tokenizer = splitByRegexp('[^\p{L}\p{N}#+]+')`.
  Diferentemente das strings separadoras fixas de `splitByString`, um separador de expressão regular pode preservar tokens que contêm caracteres especiais (como `C++` ou `C#`) que os outros tokenizadores separariam.
