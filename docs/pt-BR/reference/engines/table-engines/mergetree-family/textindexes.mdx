...
Além disso, `LIKE` oferece suporte à avaliação por varredura de dicionário (habilitada por `use_text_index_like_evaluation_by_dictionary_scan`) para os tokenizadores `splitByNonAlpha` e `array`, sem pré-processador nem pós-processador.
Nesse caso, um padrão `%value%` é uma *leitura direta (sem dica)*, enquanto os padrões `value%` e `%value` permanecem como dica; consulte [consultas LIKE/ILIKE](#like-ilike-queries-perf).
