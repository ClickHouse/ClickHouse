...
Os pontos no espaço vetorial são armazenados em uma coluna `vectors` do tipo [Array(Float64)](/pt-BR/reference/data-types/array), [Array(Float32)](/pt-BR/reference/data-types/array) ou [Array(BFloat16)](/pt-BR/reference/data-types/array).
Para uso prático, geralmente recomendamos usar arrays [BFloat16](/pt-BR/reference/data-types/float).
...
O tempo de execução dessas consultas geralmente é proporcional ao número de vetores armazenados, ao número de elementos do vetor (= a dimensão) e à largura em bits dos elementos do vetor.
Além disso, como o ClickHouse faz uma varredura por força bruta de todos os vetores, o tempo de execução também depende do número de threads usadas pela consulta (consulte a configuração [max&#95;threads](/pt-BR/reference/settings/session-settings/max-threads#max_threads)).
...
<Note>
  Para usar índices de similaridade vetorial, a configuração [compatibility](/pt-BR/reference/settings/session-settings/compatibility#compatibility) deve estar definida como `''` (o valor padrão), ou `'25.1'` ou posterior.
</Note>
...
Usuários avançados podem fornecer um valor personalizado para a configuração [hnsw&#95;candidate&#95;list&#95;size&#95;for&#95;search](/pt-BR/reference/settings/session-settings/other#hnsw_candidate_list_size_for_search) (também conhecida como hiperparâmetro HNSW &quot;ef&#95;search&quot;) para ajustar o tamanho da lista de candidatos durante a busca (por exemplo, `SELECT [...] SETTINGS hnsw_candidate_list_size_for_search = <value>`).
...
Mais especificamente, um erro é retornado se `<N>` for maior que o valor da configuração [max&#95;limit&#95;for&#95;vector&#95;search&#95;queries](/pt-BR/reference/settings/session-settings/max#max_limit_for_vector_search_queries), cujo valor padrão é 100.
...
Neste exemplo, 1 milhão de vetores do [conjunto de dados dbpedia](https://huggingface.co/datasets/KShivendu/dbpedia-entities-openai-1M), cada um com dimensão 1536, estão armazenados em 575 grânulos, ou seja, 1,7 mil linhas por grânulo.
...
<Tip>
  Para forçar o uso do índice, você pode executar a consulta SELECT com a configuração [force&#95;data&#95;skipping&#95;indices](/pt-BR/reference/settings/session-settings/force#force_data_skipping_indices) (informe o nome do índice como valor da configuração).
</Tip>
...
* Pós-filtragem significa que o índice de similaridade vetorial é avaliado primeiro; em seguida, o ClickHouse avalia o(s) filtro(s) adicional(is) especificado(s) na cláusula `WHERE`.
...
A configuração [vector&#95;search&#95;filter&#95;strategy](/pt-BR/reference/settings/session-settings/vector-search#vector_search_filter_strategy) (padrão: `auto`, que implementa as heurísticas acima) pode ser definida como `prefilter`.
...
Como abordagem alternativa para resolver o problema acima, [vector&#95;search&#95;index&#95;fetch&#95;multiplier](/pt-BR/reference/settings/session-settings/vector-search#vector_search_index_fetch_multiplier) (padrão: `1.0`, máximo: `1000.0`) pode ser configurado com um valor &gt; `1.0` (por exemplo, `2.0`).
...
Essa otimização vem habilitada por padrão; consulte a configuração [vector&#95;search&#95;with&#95;rescoring](/pt-BR/reference/settings/session-settings/vector-search#vector_search_with_rescoring).
...
Como resultado, a [compressão](/pt-BR/reference/statements/create/table/codec) deixa mais lentas as inserções e leituras na/da coluna vetorial.
Por isso, recomendamos desativar a compressão.
Para fazer isso, especifique `CODEC(NONE)` para a coluna vetorial assim:
...
O número máximo de threads para criação de índices pode ser configurado usando a configuração do servidor [max&#95;build&#95;vector&#95;similarity&#95;index&#95;thread&#95;pool&#95;size](/pt-BR/reference/settings/server-settings/settings/max#max_build_vector_similarity_index_thread_pool_size).
Para um desempenho ideal, esse valor deve ser configurado de acordo com o número de núcleos de CPU.

Segundo, para acelerar instruções INSERT, os usuários podem desativar a criação de índices de skipping em partes recém-inseridas usando a configuração de sessão [materialize&#95;skip&#95;indexes&#95;on&#95;insert](/pt-BR/reference/settings/session-settings/materialize#materialize_skip_indexes_on_insert).
...
Terceiro, para acelerar mesclagens, os usuários podem desativar a criação de índices de skipping em partes mescladas usando a configuração de sessão [materialize&#95;skip&#95;indexes&#95;on&#95;merge](/pt-BR/reference/settings/merge-tree-settings/materialize#materialize_skip_indexes_on_merge).
...
O tamanho máximo do cache pode ser configurado usando a configuração do servidor [vector&#95;similarity&#95;index&#95;cache&#95;size](/pt-BR/reference/settings/server-settings/settings/vector-similarity#vector_similarity_index_cache_size).
...
Nesse caso, também sugerimos habilitar o rescoring ([vector&#95;search&#95;index&#95;fetch&#95;multiplier](/pt-BR/reference/settings/session-settings/vector-search#vector_search_index_fetch_multiplier), [vector&#95;search&#95;with&#95;rescoring](/pt-BR/reference/settings/session-settings/vector-search#vector_search_with_rescoring)) para melhorar a precisão.
...
Para ativar o codec `Quantized`, primeiro ative `SET allow_experimental_codecs = 1`.
...
Um [índice de similaridade vetorial](#vector-similarity-index) responde a uma consulta de vizinhos mais próximos percorrendo um grafo e apresenta ótimo desempenho quando o grafo pode ser mantido na memória.
Duas condições limitam sua aplicabilidade:

* **Altos custos de construção.** Criar índices de similaridade vetorial é um processo caro.
* **Alto consumo de memória.** Os índices de similaridade vetorial consomem uma quantidade significativa de memória que, além da usada pelos próprios vetores, se torna o principal custo.
* **Filtragem.** Com um filtro `WHERE` seletivo, o percurso de grafo se torna ineficaz, pois não consegue alcançar o pequeno conjunto de linhas que satisfazem o predicado ou precisa inspecionar um número desproporcional de candidatos para encontrá-las.

Uma varredura exaustiva não está sujeita a nenhuma dessas limitações: ela não requer uma estrutura de dados auxiliar, as partes são mescladas naturalmente por concatenação, e a filtragem simplesmente reduz o número de linhas a serem varridas.
A maior desvantagem das varreduras exaustivas é o grande volume de dados que precisa ser lido: uma varredura de vetores armazenados com precisão total `Float32` ou `BFloat16` é limitada pela E/S de armazenamento, pois toda a coluna de vetores precisa ser carregada do disco.

O codec de coluna `Quantized` resolve essa desvantagem.
Ele armazena cada vetor duas vezes: os valores originais com precisão total e uma representação quantizada compacta.
Uma consulta de busca vetorial primeiro varre os códigos quantizados usando uma função de distância de baixo custo e compatível com SIMD para criar uma lista curta dos candidatos mais promissores.
Em uma segunda etapa, ele reordena esses candidatos com base nos vetores com precisão total.
A varredura inicial dos códigos quantizados lê muito menos bytes do armazenamento do que uma varredura dos vetores originais.

O codec é adequado ao ClickHouse porque a parte mais cara (a varredura) é exatamente o que o mecanismo do ClickHouse foi projetado para fazer bem:

* **Vetorizado.** Os kernels de varredura utilizam instruções SIMD para reduzir o consumo de CPU.
* **Em paralelo entre núcleos e partes.** Uma varredura é trivialmente paralelizável: as distâncias são calculadas em todas as threads disponíveis e em todas as partes de uma tabela ao mesmo tempo.
* **Distribuído.** Em um cluster fragmentado, o trabalho é distribuído entre as máquinas: cada shard varre sua própria fatia em paralelo, e o coordenador mescla as listas curtas.
* **Sem custos adicionais de construção.** Os códigos quantizados são produzidos à medida que os vetores são gravados. Não há índice adicional a criar, ajustar ou reconstruir; portanto, uma tabela está pronta para busca assim que seus dados são inseridos.

<div id="quantized-codecs-declaring">
  #### Aplicando o codec
</div>

Especifique o codec `Quantized(...)` para a coluna `Array(Float32)` (ou `Array(Float64)` / `Array(BFloat16)`).
...
O codec não pode ser adicionado nem alterado posteriormente usando `ALTER TABLE`.

<div id="quantized-codecs-methods">
  #### Métodos de quantização
</div>

Cada método de quantização apresenta diferentes trade-offs entre tamanho e precisão.
O argumento `dimensions` é o comprimento do vetor.
...
O codec é aplicado automaticamente no padrão usual de consultas top-`k` (consulte a [busca exata](#exact-nearest-neighbor-search)):
...
Definir `vector_search_use_quantized_codes = 1` permite que o otimizador reescreva a consulta como uma varredura em duas fases.
A configuração fica desativada por padrão.
Sem ela, a mesma consulta é executada como uma varredura exata regular nos vetores originais.
Todos os métodos oferecem suporte à função de distância `cosineDistance`; os métodos `int8`, `prefix` e `product` também oferecem suporte a `L2Distance` como função de distância.

A configuração `vector_search_index_fetch_multiplier` especifica quantos candidatos são selecionados para a lista curta em relação ao `LIMIT` da consulta.
Multiplicadores maiores melhoram a revocação, ao custo de mais reavaliação.
O padrão é `1` (sem superamostragem); geralmente é necessário aumentá-lo (por exemplo, para `10`) para obter uma boa revocação.
