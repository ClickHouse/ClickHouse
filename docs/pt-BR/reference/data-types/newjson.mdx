...
- Modificar uma type hint (ou remover um caminho tipado) **não** é uma operação somente de metadados e é rejeitado quando a subcoluna afetada é usada em uma estrutura persistida por posição:
  ...
  * em um **skip index de dados** explícito — primeiro remova o índice ou desative `allow_experimental_json_lazy_type_hints` para executar a alteração como uma mutação completa que reconstrói o índice.
