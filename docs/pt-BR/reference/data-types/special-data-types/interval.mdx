...
`avg`, `min`, `max` e `any` aplicados a uma coluna `Interval` retornam um intervalo do mesmo tipo.
`avg` calcula a média do número de unidades e arredonda o resultado para um número inteiro de unidades:
...
Intervalos de unidades diferentes não são comensuráveis — um mês não corresponde a um número fixo de dias —, portanto
não há conversão entre eles e cada tipo de intervalo é agregado separadamente.
