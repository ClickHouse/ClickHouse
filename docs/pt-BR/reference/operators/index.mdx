...
## Operadores de acesso {#access-operators}
...
## Operador de negação numérica {#numeric-negation-operator}

`-a` – A função `negate(a)`.

Para a negação de tuplas: [tupleNegate](/pt-BR/reference/functions/regular-functions/tuple-functions#tupleNegate).

## Operadores de Multiplicação e Divisão {#multiplication-and-division-operators}
...
## Operadores de adição e subtração {#addition-and-subtraction-operators}
...
### Função BETWEEN {#between-function}
...
## Operadores para trabalhar com strings {#operators-for-working-with-strings}
...
## Operadores para trabalhar com conjuntos de dados {#operators-for-working-with-data-sets}

Consulte os [operadores IN](/pt-BR/reference/statements/in) e o operador [EXISTS](/pt-BR/reference/operators/exists).

### função in {#in-function}

`a IN ...` – a função `in(a, b)`.

### função notIn {#notin-function}

`a NOT IN ...` – a função `notIn(a, b)`.

### função globalIn {#globalin-function}

`a GLOBAL IN ...` – a função `globalIn(a, b)`.

### função globalNotIn {#globalnotin-function}

`a GLOBAL NOT IN ...` – A função `globalNotIn(a, b)`.

### função in de subconsulta {#in-subquery-function}
...
### função in subconsulta {#in-subquery-function-1}

`a = ALL (subquery)` – O mesmo que `a IN (SELECT singleValueOrNull(*) FROM subquery)`.

### função de subconsulta `notIn` {#notin-subquery-function-1}
...
### `SOME` / `ALL` em arrays {#some-all-on-arrays}
...
## Operadores para trabalhar com datas e horas {#operators-for-working-with-dates-and-times}
...
### Adição de data e hora {#date-time-addition}
...
<Note>
  O resultado usa o [fuso horário da sessão](/pt-BR/reference/settings/session-settings/other#session_timezone) (ou o fuso horário padrão do servidor, se nenhum fuso horário da sessão estiver definido). A configuração [`date_time_overflow_behavior`](/pt-BR/reference/settings/formats/date-time#date_time_overflow_behavior) controla o que acontece quando o resultado fica fora do intervalo representável.
</Note>
...
### AT TIME ZONE e AT LOCAL {#at-time-zone}
...
`zone` pode ser qualquer expressão constante do tipo string que resulte em um nome de fuso horário válido (por exemplo, `'America/Denver'`, `'UTC'` ou `concat('America', '/', 'Denver')`). Como `AT TIME ZONE` é convertido em `toTimeZone`, aplicam-se as mesmas regras para argumentos de fuso horário: expressões não constantes, como uma referência de coluna, exigem [`allow_nonconst_timezone_arguments = 1`](/pt-BR/reference/settings/session-settings/allow#allow_nonconst_timezone_arguments).

`AT LOCAL` usa o [fuso horário da sessão](/pt-BR/reference/settings/session-settings/other#session_timezone) atual (ou o padrão do servidor, se nenhum fuso horário da sessão estiver definido). Em tabelas `Distributed`, `session_timezone` deve ser definido explicitamente; quando está vazio, `timeZone()` é local ao shard e não pode ser usado como argumento constante de `toTimeZone`, causando uma exceção `ILLEGAL_COLUMN`.
...
## Operador de negação lógica {#logical-negation-operator}
...
## Operador de concatenação {#concatenation-operator}

`s1 || s2` – A função `concat(s1, s2) function.`

## Operador de criação de lambda {#lambda-creation-operator}

`x -> expr` – a função `lambda(x, expr)`.

Os operadores a seguir não têm prioridade, pois são parênteses:

## Operador de criação de Array {#array-creation-operator}

`[x1, ...]` – A `função array(x1, ...).`

## Operador de criação de tupla {#tuple-creation-operator}
...
## Verificando `NULL` {#checking-for-null}
...
Pode ser otimizado habilitando a configuração [optimize&#95;functions&#95;to&#95;subcolumns](/pt-BR/reference/settings/session-settings/optimize#optimize_functions_to_subcolumns). Com `optimize_functions_to_subcolumns = 1`, a função lê apenas a subcoluna [null](/pt-BR/reference/data-types/nullable#finding-null), em vez de ler e processar todos os dados da coluna. A consulta `SELECT n IS NULL FROM table` é transformada em `SELECT n.null != 0 FROM TABLE`.
...
## Verificando valores booleanos {#checking-boolean-values}
