...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.10"},{"label": "0"},{"label": "Nova configuração para habilitar o spill de `DISTINCT` em disco quando o uso de memória ultrapassa o limiar informado em bytes. Se for 0, apenas `max_bytes_ratio_before_external_distinct` se aplica."}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.10"},{"label": "0.5"},{"label": "Nova configuração para habilitar o spill de `DISTINCT` para disco quando o uso de memória exceder a razão informada da memória disponível. Se 0, apenas `max_bytes_before_external_distinct` se aplica."}]}]} />
