...
Nome da coleção nomeada usada pelo `aiEmbed` quando a chamada não passa `credentials` no mapa de parâmetros. Vazio significa que não há valor padrão: essas chamadas devem passar `credentials` explicitamente. É mantido separado de `ai_function_text_default_credentials` porque o endpoint e o modelo de embeddings são diferentes dos de chat.
...
Nome da coleção nomeada usada pelas funções de IA de texto (`aiGenerate`, `aiClassify`, `aiExtract`, `aiTranslate`) quando a chamada não fornece `credentials` no mapa de parâmetros. Vazio significa que não há valor padrão: essas chamadas devem fornecer `credentials` explicitamente. Um endpoint e modelo de chat completions são diferentes de um endpoint e modelo de embeddings, portanto isso é separado de `ai_function_embedding_default_credentials`.
...
<Note>
  Use esta configuração apenas por compatibilidade retroativa, se seus casos de uso dependerem da sintaxe antiga.
</Note>
...
Às vezes, o envio de um lote específico para o shard remoto pode falhar por causa de algum pipeline posterior complexo (ou seja, `MATERIALIZED VIEW` com `GROUP BY`), devido a `Memory limit exceeded` ou erros semelhantes. Nesse caso, tentar novamente não ajudará (e isso deixará os envios distribuídos da tabela travados), mas enviar os arquivos desse lote um a um pode fazer com que o `INSERT` seja bem-sucedido.

Portanto, definir essa configuração como `1` desabilitará o batching para esses lotes (ou seja, desabilita temporariamente `distributed_background_insert_batch` para lotes com falha).
...
<Note>
  Essa configuração também afeta lotes corrompidos (que podem aparecer devido ao encerramento anormal do servidor (máquina) e à ausência de `fsync_after_insert`/`fsync_directories` para o motor de tabela [Distributed](/pt-BR/reference/engines/table-engines/special/distributed)).
</Note>

<Note>
  Você não deve depender da divisão automática de lotes, pois isso pode prejudicar o desempenho.
</Note>
...
<Note>
  `insert_deduplication_token` é rastreado por partição, portanto várias partições gravadas por uma única inserção podem carregar o mesmo token. Sem um token, o checksum de conteúdo padrão (`insert_deduplication_version = new_unified_hash`) é calculado sobre todo o bloco inserido, de modo que uma inserção é desduplicada apenas quando todos os seus dados correspondem aos de uma inserção anterior (uma nova tentativa), e não quando as linhas de uma única partição por acaso coincidem com as de uma inserção diferente.
</Note>
...
<Note>
  Esta configuração não se aplica ao SharedMergeTree; veja [consistência do SharedMergeTree](/pt-BR/products/cloud/features/infrastructure/shared-merge-tree#consistency) para mais informações.
</Note>
...
A profundidade máxima de aninhamento da árvore de sintaxe de uma consulta. Se for excedida, uma exceção será lançada.
...
<Note>
  ...
  ative o modo de agregação externa (spill de dados para disco).
...
> inferior a 1000..10000. Observe que o particionamento não se destina a acelerar
...
Ativa ou desativa uma otimização no nível do plano de consulta que move cláusulas ARRAY JOIN para cima no plano de execução.
Só entra em vigor se a configuração [query&#95;plan&#95;enable&#95;optimizations](#query_plan_enable_optimizations) for 1.
...
Ativa ou desativa uma otimização no nível do plano de consulta que move subárvores maiores do plano de consulta para o union, permitindo otimizações adicionais.
Só tem efeito se a configuração [`query_plan_enable_optimizations`](#query_plan_enable_optimizations) estiver definida como 1.
...
Limita o número total de otimizações aplicadas ao plano de consulta; consulte a configuração [query&#95;plan&#95;enable&#95;optimizations](#query_plan_enable_optimizations).
...
* &#39;dpsize&#39; - implementa o algoritmo DPsize, atualmente apenas para joins Inner - considera todas as ordens de join possíveis e encontra a mais otimizada, mas pode ser lento para consultas com muitas tabelas e predicados de join.
* &#39;dpsub&#39; - implementa o algoritmo DPsub, que oferece suporte tanto a joins inner quanto a joins não inner - considera todas as ordens de join possíveis e encontra a mais otimizada, mas pode ser lento para consultas com muitas tabelas e predicados de join.
* &#39;dphyp&#39; - implementa o algoritmo DPhyp (Programação Dinâmica via Particionamento de Hipergrafo), atualmente apenas para joins inner - explora o mesmo espaço de busca que `dpsize`, mas enumera apenas pares de subgrafos conectados, o que gera menos joins intermediários em grafos de join esparsos, sem considerar produtos cartesianos
...
Otimiza a ordem dos joins dentro da mesma subconsulta. No momento, há suporte apenas para casos muito limitados.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "23.2"},{"label": "1"},{"label": "Remove a etapa DISTINCT redundante no plano de consulta"}]}]} />

Ativa/desativa uma otimização no nível do plano de consulta que remove etapas DISTINCT redundantes.
...
<Warning>
  Nem todas as funções que analisam DateTime/DateTime64 respeitam `session_timezone`. Isso pode levar a erros sutis.
...
* Na segunda consulta, um DateTime é analisado de uma String e herda o tipo e o fuso horário da coluna existente `d`. Assim, a configuração `session_timezone` e o fuso horário global não são respeitados.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "0.2"},{"label": "Renomeado de `text_index_density_threshold` (mantido como alias); seleciona o algoritmo de interseção no modo lazy de aplicação de posting list."}]}]} />

Limite de densidade da posting list que seleciona o algoritmo de interseção no modo lazy de aplicação de posting list (`text_index_posting_list_apply_mode = 'lazy'`).
Abaixo do limite: interseção leapfrog (favorece posting lists esparsas). No limite ou acima: interseção de bitmap por força bruta (favorece posting lists densas).
...
&#39;lazy&#39; usa decodificação sob demanda baseada em cursor (requer um formato de índice com um codec serializado).
