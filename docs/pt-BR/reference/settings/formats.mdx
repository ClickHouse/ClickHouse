...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "1"},{"label": "Nova configuração que controla se o formato GeoJSON valida a geometria segundo a RFC 7946 (número mínimo de pontos por linha e anel, fechamento de anel e multi-geometrias não vazias) ao ler e gravar"}]}]} />

Controla se o formato `GeoJSON` valida a geometria segundo a RFC 7946, nos dois sentidos.

Quando habilitada (padrão), uma geometria que viola as regras de forma do GeoJSON é rejeitada: uma `LineString` (ou uma linha de uma `MultiLineString`) com menos de dois pontos; um anel de `Polygon` ou `MultiPolygon` com menos de quatro pontos ou cujo primeiro e último pontos sejam diferentes (um anel não fechado); ou uma `MultiLineString`, `Polygon` ou `MultiPolygon` vazia. Isso se aplica tanto na leitura (esse documento é rejeitado) quanto na gravação (esse valor do ClickHouse é rejeitado em vez de gerar um documento que o formato de entrada rejeitaria).

Quando desabilitada, essas regras de forma não são aplicadas: essas geometrias são lidas e gravadas como estão, de modo que geometrias degeneradas fazem round-trip, mas o documento gravado pode não ser um GeoJSON válido.

A validação é apenas estrutural: ela verifica a contagem de pontos e o fechamento dos anéis. Ela não avalia a correção geométrica de uma forma — a orientação do anel (a regra da mão direita / ordem de percurso) não é aplicada, e geometrias estruturalmente válidas, mas geometricamente degeneradas, são aceitas, como um polígono de área zero, um anel que se auto-intersecta ou um polígono cujos buracos ficam fora de seu anel externo. Coordenadas não finitas (`NaN`, `Inf`) são sempre rejeitadas, independentemente desta configuração, porque não podem ser representadas como números JSON.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.2"},{"label": "0"},{"label": "Nova configuração para permitir o parsing e o processamento dos dados restantes no buffer caso a conexão seja encerrada inesperadamente"}]}]} />

Quando esta opção está habilitada, se a conexão for encerrada inesperadamente, os dados restantes no buffer serão processados por parsing e processados em vez de serem tratados como erro

:::note
Habilitar esta opção desativa o parsing paralelo e impossibilita a desduplicação
:::
...
Controla como `Nullable(String)` é lido quando há um valor ausente em CSV. Um valor ausente é um espaço vazio entre/antes/depois de vírgulas, sem estar entre aspas. Se esta configuração estiver habilitada, independentemente do valor de `input_format_csv_empty_as_default`, o valor ausente de `Nullable(String)` será interpretado como uma `String` vazia, e não como NULL.
...
Ao executar consultas `INSERT`, substitua os valores omitidos das colunas de entrada pelos valores padrão das respectivas colunas. Esta opção se aplica aos formatos [JSONEachRow](/pt-BR/reference/formats/JSON/JSONEachRow) (e outros formatos JSON), [CSV](/pt-BR/reference/formats/CSV/CSV), [TabSeparated](/pt-BR/reference/formats/TabSeparated/TabSeparated), [TSKV](/pt-BR/reference/formats/TabSeparated/TSKV), [Parquet](/pt-BR/reference/formats/Parquet/Parquet), [Arrow](/pt-BR/reference/formats/Arrow/Arrow), [Avro](/pt-BR/reference/formats/Avro/Avro), [ORC](/pt-BR/reference/formats/ORC), [Native](/pt-BR/reference/formats/Native) e formatos com os sufixos `WithNames`/`WithNamesAndTypes`.
...
Quando habilitado, o ClickHouse tentará inferir o tipo [`Variant`](/pt-BR/reference/data-types/variant) na inferência de esquema para formatos de texto quando houver mais de um tipo possível para elementos de colunas/arrays.
...
Para melhorar o desempenho das inserções, recomendamos desativar essa verificação se você tiver certeza de que a ordem das colunas dos dados de entrada é a mesma da tabela de destino.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "1"},{"label": "Nova configuração. Quando output_format_csv_serialize_tuple_into_separate_columns está habilitada, o cabeçalho de CSVWithNames\/CSVWithNamesAndTypes agora achata as colunas Tuple em seus campos folha para que a largura do cabeçalho corresponda aos dados. Defina como false para restaurar o cabeçalho anterior com um único nome."}]}]} />

Quando [output&#95;format&#95;csv&#95;serialize&#95;tuple&#95;into&#95;separate&#95;columns](#output_format_csv_serialize_tuple_into_separate_columns) está habilitada, as linhas de cabeçalho de `CSVWithNames` e `CSVWithNamesAndTypes` achatam cada coluna Tuple em seus campos folha (nomes com ponto, como `t.a` e `t.b`, e os nomes dos tipos folha), para que o cabeçalho tenha o mesmo número de colunas que os dados. Para `CustomSeparated*`, esse achatamento se aplica somente quando `format_custom_escaping_rule = 'CSV'` e `format_custom_field_delimiter` é um único caractere igual a `format_csv_delimiter`; caso contrário (por exemplo, o delimitador de tabulação padrão ou `format_custom_field_delimiter = '|'`), o cabeçalho permanece sem achatamento para que ainda corresponda aos dados. Defina como `0` para manter o comportamento anterior, em que o cabeçalho preserva o único nome e tipo Tuple de nível superior.

Observação: um cabeçalho achatado não é lido de volta em uma Tuple pelo nome quando `input_format_with_names_use_header = 1`. Para ler esses dados de volta em uma Tuple, defina esta configuração como `0` na saída ou leia com `input_format_with_names_use_header = 0` (e, para os formatos `*WithNamesAndTypes` `CSVWithNamesAndTypes` e `CustomSeparatedWithNamesAndTypes`, também `input_format_with_types_use_header = 0`, já que, caso contrário, a linha de tipos achatados é validada em relação ao único campo de entrada Tuple de nível superior e rejeitada).
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "24.3"},{"label": "1"},{"label": "O ClickHouse permite dados binários arbitrários no tipo de dado String, que normalmente é UTF-8. Strings de Parquet\/ORC\/Arrow aceitam apenas UTF-8. Por isso, você pode escolher qual tipo de dado do Arrow usar para o tipo de dado String do ClickHouse: String ou Binary. Embora Binary seja mais correto e compatível, usar String por padrão atenderá às expectativas dos usuários na maioria dos casos."}]}]} />
