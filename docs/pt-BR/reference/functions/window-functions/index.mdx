...
| frame `GROUPS`                                                                        | ✅          | Os limites do frame contam grupos de pares inteiros (linhas iguais na chave `ORDER BY`); `N PRECEDING` e `N FOLLOWING` contam `N` grupos de pares antes e depois do grupo de pares da linha atual.                                                                                                                                                                                                        |
...
* `ROWS`, `RANGE` ou `GROUPS` - define os limites de um frame; aggregate&#95;function é calculada dentro desse frame. `ROWS` conta linhas físicas, `RANGE` conta valores de `ORDER BY` e `GROUPS` conta grupos de pares (linhas iguais na chave `ORDER BY`).
...
Um frame `GROUPS` conta *grupos de pares* completos — conjuntos de linhas iguais na chave `ORDER BY` — em vez de linhas físicas (`ROWS`) ou valores de `ORDER BY` (`RANGE`). `N PRECEDING` e `N FOLLOWING` contam `N` grupos de pares antes e depois do grupo de pares da linha atual, e cada grupo de pares incluído sempre contribui com todas as suas linhas.

A consulta abaixo aplica os mesmos limites de `1 PRECEDING AND 1 FOLLOWING` a frames `ROWS`, `RANGE` e `GROUPS`. A coluna `order` contém valores duplicados e não consecutivos; portanto, os três modos abrangem linhas diferentes:
...
Cada modo interpreta os limites de maneira diferente:

* `ROWS` conta linhas físicas, portanto o frame contém no máximo três linhas adjacentes: a linha atual e uma de cada lado.
* `RANGE` conta valores de `order`, portanto `1 PRECEDING` e `1 FOLLOWING` abrangem linhas cujo `order` está a uma distância de até 1 do valor da linha atual. Com lacunas de 10, nenhuma linha vizinha se qualifica, então o frame contém apenas as linhas que têm o mesmo `order` da linha atual.
* `GROUPS` conta grupos de pares, portanto `1 PRECEDING` e `1 FOLLOWING` sempre incluem integralmente os grupos adjacentes, independentemente das lacunas entre os valores de `order`.
