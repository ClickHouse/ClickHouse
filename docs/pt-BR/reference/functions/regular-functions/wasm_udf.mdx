...
## Gerencie módulos WASM por meio da tabela de sistema {#manage-wasm-modules-via-system-table}
...
### Inserir um módulo {#insert-a-module}
...
### Distribuir um módulo em um cluster {#distribute-a-module-across-a-cluster}
...
### Excluir um módulo {#delete-a-module}
...
## Criar uma WebAssembly UDF {#create-a-webassembly-udf}
...
Os dados são trocados por meio de buffers na memória WASM. Um buffer é uma estrutura de 8 bytes que contém um ponteiro para os dados e o tamanho deles (dois valores `u32` little-endian). Os buffers são passados por identificador — um ponteiro para essa estrutura, e não para os próprios dados. O código guest deve exportar duas funções para criar e destruir esses buffers. Quando o tamanho for diferente de zero, o ponteiro dos dados também deve ser diferente de zero: um buffer que declara um tamanho positivo, mas aponta para o deslocamento `0` da memória linear, não é um buffer válido, e o código guest não deve produzir um; o ClickHouse rejeita esse buffer e faz a consulta falhar com um `WASM_ERROR`.
...
A função é invocada uma vez por bloco de entrada: uma consulta grande é dividida em vários blocos pelo pipeline de consulta. Portanto, a contagem de linhas passada em cada chamada corresponde ao tamanho desse bloco, e não ao da consulta inteira.

Um bloco do pipeline ainda pode ser largo demais para a memória linear do módulo, e por isso é subdividido. Por padrão, `webassembly_udf_input_split_memory_ratio` faz essa divisão dinamicamente: o ClickHouse mede as linhas serializadas e inicia uma nova chamada antes que a entrada ultrapasse essa fração da memória linear do guest, de modo que a contagem de linhas varia conforme os dados. Um `webassembly_udf_max_input_block_size` diferente de zero substitui esse comportamento por um limite fixo desse número de linhas por chamada, e uma razão de `0` passa o bloco inteiro em uma única chamada. Uma chamada nunca fica abaixo de uma linha, portanto uma linha grande demais para a memória do módulo por si só ainda falha dentro do guest.
...
### Observação sobre o desenvolvimento de UDFs em Rust {#note-for-developing-udfs-in-rust}
...
## API do host disponível para módulos {#host-api-available-to-modules}
...
* `webassembly_udf_max_input_block_size` — Número máximo de linhas passadas para uma UDF em WebAssembly em um único bloco, para todas as ABIs. Defina como 0 para permitir que `webassembly_udf_input_split_memory_ratio` dimensione as chamadas de uma função com `ABI BUFFERED_V1`; as demais ABIs então recebem o bloco inteiro do pipeline.

* `webassembly_udf_input_split_memory_ratio` — A fração da memória linear que um módulo declara ter no início (do seu teto de memória, no caso de um módulo que começa sem páginas) que a entrada de uma chamada pode ocupar. Usada para dividir os blocos de uma função com `ABI BUFFERED_V1` quando `webassembly_udf_max_input_block_size` é 0; as demais ABIs não carregam bloco de entrada serializado e a ignoram. Padrão: `0.5`, deixando o restante da memória para o working set do próprio guest — uma razão próxima de 1 ainda pode falhar dentro do allocator do guest. Defina como 0, ou defina `compatibility` para uma versão anterior à 26.9, para passar o bloco inteiro do pipeline em uma única chamada. Deve ser no mínimo 0 e no máximo 1.
