...
## Problemas comuns na conversão de dados {#common-issues-with-data-conversion}
...
## Observações sobre as funções `toString` {#to-string-functions}
...
## Observações sobre as funções `toDate`/`toDateTime` {#to-date-and-date-time-functions}
...
Introduzida na: v26.9.0

Verifica um valor em relação a um tipo declarado da Kusto Query Language e o converte para o
tipo correspondente do ClickHouse: um argumento cujo tipo não pertence ao tipo KQL declarado é
rejeitado durante a análise, e um valor que não couber (um overflow de `int`, por exemplo) gera um
erro por linha. O segundo argumento é o nome do tipo KQL declarado e o terceiro descreve o
parâmetro para a mensagem de erro; ambos devem ser strings constantes.

Esta função dá suporte a parâmetros de função tipados quando `dialect = 'kusto'`. Ela não deve ser
chamada diretamente a partir do SQL.
...
* `value` — O argumento fornecido por uma chamada de função KQL. - `type` — O nome do tipo KQL declarado, uma string constante. - `parameter` — Uma descrição do parâmetro, uma string constante usada na mensagem de erro.

**Valor retornado**

`value` como o tipo ClickHouse correspondente ao tipo declarado.
...
Introduzida na: v26.8.0

Converte um valor em um intervalo de tempo (um `Interval` em nanossegundos) da mesma forma que a função
`totimespan()` da Kusto Query Language: um intervalo de tempo é repassado sem alteração, um número é interpretado como dias e uma string é lida no formato
`[-][d.]hh:mm:ss[.fffffff]` — retornando `NULL` quando não é possível interpretá-la.

Esta função dá suporte a `totimespan()` quando `dialect = 'kusto'`. Ela não deve ser chamada
diretamente no SQL.
...
* `x` — Um intervalo de tempo, um número de dias ou uma string que expressa um intervalo de tempo.

**Returned value**

O valor como um interval de nanossegundos; `NULL` para uma string que não expressa um intervalo de tempo.
...
Se o valor estiver fora do intervalo `[0000-01-01, 9999-12-31]`, `toDate32` retorna os valores de limite suportados por [Date32](/pt-BR/reference/data-types/date32).
...
Converte o argumento para o tipo de dado [Date32](/pt-BR/reference/data-types/date32). Se o argumento não puder ser convertido, incluindo um valor numérico fora do intervalo de [Date32](/pt-BR/reference/data-types/date32), a função retornará um valor padrão, que será o segundo argumento (se especificado) ou, caso contrário, `1900-01-01`, o valor padrão histórico de [Date32](/pt-BR/reference/data-types/date32) (e não seu limite inferior, que é `0000-01-01`). Se o argumento for do tipo [Date](/pt-BR/reference/data-types/date), os limites desse tipo serão levados em consideração.

Observe que, diferentemente de [toDate32](#toDate32), esta função não satura um argumento numérico fora do intervalo para os limites de [Date32](/pt-BR/reference/data-types/date32); esse argumento é tratado como não conversível.
...
Converte um valor de entrada em um valor do tipo [Date32](/pt-BR/reference/data-types/date32), mas retorna o valor padrão `1900-01-01` se receber um argumento inválido.
É igual a [toDate32](#toDate32), mas retorna `1900-01-01` se receber um argumento inválido.
Observe que `1900-01-01` é um valor padrão histórico para entradas inválidas, não o limite inferior de [Date32](/pt-BR/reference/data-types/date32), que é `0000-01-01`.
...
Retorna um valor `Date32` em caso de sucesso; caso contrário, o valor padrão histórico para entrada inválida (`1900-01-01`). [`Date32`](/pt-BR/reference/data-types/date32)
...
Se `expr` for um valor [Time](/pt-BR/reference/data-types/time) ou [Time64](/pt-BR/reference/data-types/time64), ele será interpretado como o número de segundos desde o início da epoch Unix: a parte fracionária de um `Time64` é arredondada em direção ao infinito negativo, e valores fora do intervalo de `DateTime` seguem a configuração `date_time_overflow_behavior`.
...
* `expr` — O valor. [`String`](/pt-BR/reference/data-types/string) ou [`Int`](/pt-BR/reference/data-types/int-uint) ou [`Date`](/pt-BR/reference/data-types/date) ou [`Date32`](/pt-BR/reference/data-types/date32) ou [`Time`](/pt-BR/reference/data-types/time) ou [`Time64`](/pt-BR/reference/data-types/time64) ou [`DateTime`](/pt-BR/reference/data-types/datetime) ou [`DateTime64`](/pt-BR/reference/data-types/datetime64)
...
Oferece suporte à conversão de `String`, `FixedString`, `Date`, `Date32`, `Time`, `Time64`, `DateTime`, `DateTime64` ou tipos numéricos (`(U)Int8`, `(U)Int16`, `(U)Int32`, `(U)Int64`, `(U)Int128`, `(U)Int256`, `BFloat16`, `Float32`, `Float64`). Não há suporte para valores `Decimal`, que resultam em uma exceção.
`DateTime32` é um alias de `DateTime` e oferece suporte a datas de `1970-01-01 00:00:00` a `2106-02-07 06:28:15`.
Ao converter uma string ou um dos tipos numéricos saturáveis listados acima, valores fora desse intervalo são saturados nos limites do intervalo. Valores de ponto flutuante não finitos (`NaN`, `inf`, `-inf`) não podem ser convertidos e resultam em uma exceção.
Conversões de outros tipos de data e hora, como `Date32`, `DateTime64`, `Time` ou `Time64`, seguem a configuração `date_time_overflow_behavior`: o modo padrão `ignore` pode gerar valores com wraparound, `saturate` os limita aos limites do intervalo e `throw` lança uma exceção. Valores `Time64` são primeiro arredondados para baixo, em direção ao infinito negativo, para segundos inteiros.
...
* `x` — Valor de entrada a converter. [`String`](/pt-BR/reference/data-types/string) ou [`FixedString`](/pt-BR/reference/data-types/fixedstring) ou [`(U)Int*`](/pt-BR/reference/data-types/int-uint) ou [`Float*`](/pt-BR/reference/data-types/float) ou [`BFloat16`](/pt-BR/reference/data-types/float) ou [`Date`](/pt-BR/reference/data-types/date) ou [`Date32`](/pt-BR/reference/data-types/date32) ou [`Time`](/pt-BR/reference/data-types/time) ou [`Time64`](/pt-BR/reference/data-types/time64) ou [`DateTime`](/pt-BR/reference/data-types/datetime) ou [`DateTime64`](/pt-BR/reference/data-types/datetime64)
...
Converte um valor de entrada em um valor do tipo [DateTime64](/pt-BR/reference/data-types/datetime64), mas retorna o valor zero de [DateTime64](/pt-BR/reference/data-types/datetime64) (a epoch Unix, `1970-01-01 00:00:00`) se receber um argumento inválido.
Igual a [toDateTime64](#toDateTime64), mas retorna o valor zero de [DateTime64](/pt-BR/reference/data-types/datetime64) se receber um argumento inválido.
...
* `precision` — Opcional. A precisão de frações de segundo do valor retornado. [`UInt8`](/pt-BR/reference/data-types/int-uint)
* `timezone` — Opcional. O fuso horário do valor retornado. [`String`](/pt-BR/reference/data-types/string)

**Valor retornado**

Retorna um valor DateTime64 em caso de sucesso; caso contrário, o valor DateTime64 zero (`1970-01-01 00:00:00`) na precisão solicitada. [`DateTime64`](/pt-BR/reference/data-types/datetime64)
...
Suporta conversão de String, FixedString, DateTime, DateTime64, Time64 ou tipos numéricos que representam segundos desde a meia-noite.
Valores numéricos fora do intervalo do tipo (`-999:59:59` a `999:59:59`, ou seja, de `-3599999` a `3599999` segundos) são limitados aos limites do intervalo, e valores de ponto flutuante não finitos (`NaN`, `inf`, `-inf`) não podem ser convertidos e resultam em uma exceção.
Valores `Time64` são arredondados em direção ao infinito negativo para segundos inteiros e limitados ao mesmo intervalo.

<Note title="`toTime` legado">
  Antes da v25.5, `toTime` era uma função diferente, que convertia uma data com hora em uma data fixa (`1970-01-02`) enquanto preservava o
  componente de hora. Essa função ainda está disponível com o nome [`toTimeWithFixedDate`](/pt-BR/reference/functions/regular-functions/date-time-functions#toTimeWithFixedDate).

  Definir [`use_legacy_to_time`](/pt-BR/reference/settings/session-settings#use_legacy_to_time) como `1` também mantém o nome `toTime` vinculado à função
  legada, ou seja, chamadas a `toTime` são resolvidas como `toTimeWithFixedDate` em vez da função de conversão descrita aqui.
  Enquanto a configuração estiver habilitada, use `CAST(x AS Time)` ou `x::Time` para converter para o tipo `Time`.
  A configuração usa `0` por padrão desde a v26.7, mas usava `1` por padrão da v25.6 à v26.6, motivo pelo qual os exemplos abaixo a definem explicitamente.
</Note>
...
* `x` — Valor de entrada a converter. [`String`](/pt-BR/reference/data-types/string) ou [`FixedString`](/pt-BR/reference/data-types/fixedstring) ou [`DateTime`](/pt-BR/reference/data-types/datetime) ou [`DateTime64`](/pt-BR/reference/data-types/datetime64) ou [`Time64`](/pt-BR/reference/data-types/time64) ou [`(U)Int*`](/pt-BR/reference/data-types/int-uint) ou [`BFloat16`](/pt-BR/reference/data-types/float) ou [`Float*`](/pt-BR/reference/data-types/float)
...
É como [`toTime64`](#toTime64), mas retorna `NULL` em vez de lançar uma exceção em caso de erro de conversão.
...
* `x` — Uma representação textual de um horário com precisão de frações de segundo. [`String`](/pt-BR/reference/data-types/string)
* `precision` — Opcional. A precisão de frações de segundo do valor retornado. [`UInt8`](/pt-BR/reference/data-types/int-uint)
...
Converte um valor de entrada em um valor do tipo `Time64`, mas retorna o valor zero de `Time64` (`00:00:00`) caso receba um argumento inválido.
O mesmo que [`toTime64`](#toTime64), mas retorna o valor zero de `Time64` caso receba um argumento inválido.
...
Retorna um valor `Time64` em caso de sucesso; caso contrário, o valor `Time64` zero (`00:00:00`) na precisão solicitada. [`Time64`](/pt-BR/reference/data-types/time64)
