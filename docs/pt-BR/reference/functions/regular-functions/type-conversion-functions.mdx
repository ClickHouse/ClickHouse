...
Oferece suporte à conversão de `String`, `FixedString`, `Date`, `Date32`, `Time`, `DateTime`, `DateTime64` ou tipos numéricos (`(U)Int8`, `(U)Int16`, `(U)Int32`, `(U)Int64`, `(U)Int128`, `(U)Int256`, `BFloat16`, `Float32`, `Float64`). Não há suporte para valores `Decimal` e `Time64`, que resultam em uma exceção; um valor do tipo `Time` sofre wraparound em vez de saturação.
