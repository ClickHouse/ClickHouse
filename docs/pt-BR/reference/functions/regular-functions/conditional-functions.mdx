...
* Para tipos `DateTime`, o tipo do resultado é promovido para o tipo mais amplo (por exemplo, `DateTime64` se combinado com `DateTime32`).

:::note Use a configuração `least_greatest_legacy_null_behavior` para alterar o comportamento de `NULL`
A versão [24.12](/pt-BR/resources/changelogs/oss/2024#a-id2412a-clickhouse-release-2412-2024-12-19) introduziu uma mudança incompatível com versões anteriores, de modo que os valores `NULL` são ignorados, enquanto antes a função retornava `NULL` se um dos argumentos fosse `NULL`.
