...
O tipo da geometria de saída depende da entrada: um `Point` retorna um `Point`; um `MultiPoint` retorna um `MultiPoint`; um `LineString` ou `MultiLineString` retorna uma
