---
description: 'Documentação do Polygon'
sidebarTitle: 'Polygons'
slug: /sql-reference/functions/geo/polygons
title: 'Funções para trabalhar com Polygons'
doc_type: 'reference'
---
...
Retorna um objeto geométrico em WKT (Well Known Text) a partir de vários [tipos de dados Geo](/pt-BR/reference/data-types/geo). Os objetos WKT compatíveis são:
...
`geo_data` pode ser um dos seguintes [Tipos de dados Geo](/pt-BR/reference/data-types/geo) ou um de seus tipos primitivos subjacentes:

* [Point](/pt-BR/reference/data-types/geo#point)
* [MultiPoint](/pt-BR/reference/data-types/geo#multipoint)
...
* [LineString](/pt-BR/reference/data-types/geo#linestring)
* [MultiLineString](/pt-BR/reference/data-types/geo#multilinestring)

**Valor retornado**

* O objeto geométrico WKT `POINT` é retornado para Point.
* O objeto geométrico WKT `MULTIPOINT` é retornado para MultiPoint.
* O objeto geométrico WKT `POLYGON` é retornado para Polygon
* O objeto geométrico WKT `MULTIPOLYGON` é retornado para MultiPolygon.
* O objeto geométrico WKT `LINESTRING` é retornado para LineString.
* O objeto geométrico WKT `MULTILINESTRING` é retornado para MultiLineString.
...
POLYGON a partir de um array de tuplas ou de um array de arrays de tuplas:
...
MULTIPOLYGON a partir de um array com arrays de tuplas multidimensionais:
...
Converte um MultiPolygon em WKT (Well Known Text) em um tipo MultiPolygon.
...
String que começa com `MULTIPOLYGON`
...
Converte um MultiPolygon em WKT (Well Known Text) para o tipo Polygon.
...
String que começa com `POLYGON`

<div id="returned-value-1">
  ### Valor de retorno
</div>
...
A função `readWKTPoint` do ClickHouse analisa uma representação em Well-Known Text (WKT) de uma geometria do tipo Point e retorna um ponto no formato interno do ClickHouse.
...
* `wkt_string`: A String WKT de entrada que representa uma geometria Point.

<div id="returned-value">
  ### Valor retornado
</div>

A função retorna uma representação interna do ClickHouse para a geometria Point.
...
Analisa uma representação em Well-Known Text (WKT) de uma geometria LineString e a retorna no formato interno do ClickHouse.
...
* `wkt_string`: a string WKT de entrada que representa uma geometria LineString.

<div id="returned-value">
  ### Valor retornado
</div>

A função retorna a representação interna do ClickHouse para a geometria LineString.
...
Analisa uma representação em Well-Known Text (WKT) de uma geometria MultiLineString e a retorna no formato interno do ClickHouse.
...
* `wkt_string`: a string WKT de entrada que representa uma geometria MultiLineString.

<div id="returned-value">
  ### Valor retornado
</div>

A função retorna uma representação interna do ClickHouse para a geometria MultiLineString.
...
Analisa uma representação em Well-Known Text (WKT) de uma geometria MultiPoint e a retorna no formato interno do ClickHouse.
...
* `wkt_string`: A string WKT de entrada que representa uma geometria MultiPoint.

<div id="returned-value">
  ### Valor retornado
</div>

A função retorna uma representação interna da geometria MultiPoint do ClickHouse.
...
Analisa uma representação em Well-Known Text (WKT) da geometria Polygon e retorna um anel (linestring fechada) no formato interno do ClickHouse.
...
* `wkt_string`: A string WKT de entrada que representa uma geometria do tipo Polygon.

<div id="returned-value">
  ### Valor retornado
</div>

A função retorna uma representação interna do ClickHouse para a geometria do anel (`linestring` fechada).
...
Retorna true ou false, dependendo de um polígono estar ou não completamente dentro de outro polígono. Referência: https://www.boost.org/doc/libs/1&#95;62&#95;0/libs/geometry/doc/html/geometry/reference/algorithms/within/within&#95;2.html
...
Converte um MultiPolygon em WKB (Well Known Binary) em um valor do tipo MultiPolygon.
...
Converte um MultiPolygon em WKB (Well Known Binary) no tipo Polygon.
...
A função `readWKBPoint` no ClickHouse interpreta uma representação em Well-Known Binary (WKB) de uma geometria Point e retorna um ponto no formato interno do ClickHouse.
...
* `wkb_string`: A string WKB de entrada que representa uma geometria do tipo Point.
...
Analisa uma representação em Well-Known Binary (WKB) de uma geometria LineString e a retorna no formato interno do ClickHouse.
...
* `wkb_string`: A string WKB de entrada que representa uma geometria do tipo LineString.

<div id="returned-value">
  ### Valor retornado
</div>

A função retorna uma representação interna do ClickHouse para a geometria LineString.
...
Analisa a representação Well-Known Binary (WKB) de uma geometria MultiLineString e a retorna no formato interno do ClickHouse.
...
* `wkb_string`: A string WKB de entrada que representa uma geometria do tipo MultiLineString.

<div id="returned-value">
  ### Valor retornado
</div>

A função retorna uma representação interna do ClickHouse da geometria MultiLineString.
...
Analisa uma representação Well-Known Binary (WKB) de uma geometria MultiPoint e a retorna no formato interno do ClickHouse.
...
* `wkb_string`: A string WKB de entrada que representa uma geometria MultiPoint.

<div id="returned-value">
  ### Valor retornado
</div>

A função retorna uma representação interna do ClickHouse para a geometria MultiPoint.
...
Calcula a distância mínima entre dois pontos, em que um ponto pertence ao primeiro polígono e o segundo, a outro polígono. Esférico significa que as coordenadas são interpretadas como coordenadas em uma esfera pura e ideal, o que não corresponde à Terra. O uso desse tipo de sistema de coordenadas acelera a execução, mas, naturalmente, não é preciso.
...
Calcula a distância entre dois polígonos
...
Retorna true se dois polígonos forem iguais
...
Calcula a diferença simétrica espacial da teoria dos conjuntos (XOR) entre dois polígonos
...
O mesmo que `polygonsSymDifferenceSpherical`, mas as coordenadas estão em um sistema de coordenadas cartesianas, que é mais próximo do modelo da Terra real.
...
Calcula a interseção (AND) entre polígonos; as coordenadas são esféricas.
...
Retorna true se o segundo polígono estiver contido no primeiro polígono.
...
UInt8, 0 para falso, 1 para true

<div id="polygonsintersectcartesian">
  ## polygonsIntersectCartesian
</div>

Retorna true se os dois polígonos se intersectam (compartilham alguma área ou borda em comum).
...
Retorna true se os dois polígonos se intersectam (compartilham alguma área ou fronteira em comum). Referência https://www.boost.org/doc/libs/1&#95;62&#95;0/libs/geometry/doc/html/geometry/reference/algorithms/intersects.html
...
Retorna true se duas geometrias se intersectam (têm algum ponto, linha ou área em comum).

Ao contrário de [`polygonsIntersectCartesian`](#polygonsintersectcartesian), aceita qualquer tipo de dado geométrico
(`Point`, `MultiPoint`, `LineString`, `MultiLineString`, `Ring`, `Polygon`, `MultiPolygon`), incluindo o tipo comum
[`Geometry`](/pt-BR/reference/data-types/geo#geometry), e os dois argumentos podem ser de tipos diferentes.

As coordenadas estão no sistema de coordenadas cartesianas.
...
Duas geometrias de qualquer tipo de dado geométrico (ou `Geometry`).
...
Diferentemente de [`polygonsIntersectSpherical`](#polygonsintersectspherical), ela aceita qualquer tipo de dado geométrico
(`Point`, `MultiPoint`, `LineString`, `MultiLineString`, `Ring`, `Polygon`, `MultiPolygon`), incluindo o tipo comum
[`Geometry`](/pt-BR/reference/data-types/geo#geometry), e os dois argumentos podem ser de tipos diferentes.

As coordenadas são interpretadas como pertencentes a uma esfera ideal.
...
Calcula um fecho convexo. [Referência](https://www.boost.org/doc/libs/1_61_0/libs/geometry/doc/html/geometry/reference/algorithms/convex_hull.html)

As coordenadas estão em um sistema de coordenadas cartesianas.
...
Calcula a área de um polígono na superfície da esfera.
...
<div id="polygon-representing-zimbabwe">
  #### Polígono que representa o Zimbábue
</div>

Este é o polígono que representa o Zimbábue:
...
<div id="usage-of-polygon-perimeter-spherical">
  #### Uso da função polygonPerimeterSpherical
</div>
...
Calcula a interseção entre polígonos.
...
Para mais informações sobre sistemas geométricos, veja esta [apresentação](https://archive.fosdem.org/2020/schedule/event/working_with_spatial_trajectories_in_boost_geometry/attachments/slides/3988/export/events/attachments/working_with_spatial_trajectories_in_boost_geometry/slides/3988/FOSDEM20_vissarion.pdf) sobre a biblioteca Boost, que é a usada pelo ClickHouse.