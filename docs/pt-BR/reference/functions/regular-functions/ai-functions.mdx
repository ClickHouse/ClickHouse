...
Calcula o embedding de ambos os textos e retorna a similaridade de cosseno entre os dois vetores
no intervalo `[-1, 1]`: `1` significa que os textos são semanticamente idênticos, `0` significa que não têm relação,
e valores negativos indicam significados opostos. É o complemento de `cosineDistance` para os
mesmos embeddings (`aiSimilarity = 1 - cosineDistance(embedding1, embedding2)`).
