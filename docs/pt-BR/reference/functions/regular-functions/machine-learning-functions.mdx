...
Classifica o texto de entrada usando um modelo de Naive Bayes com n-gramas e suavização de Laplace.
O modelo deve ser configurado no ClickHouse antes de ser usado.

**Detalhes de implementação**

*Algoritmo*

Usa o algoritmo de classificação Naive Bayes com [suavização de Laplace](https://en.wikipedia.org/wiki/Additive_smoothing) para lidar com n-gramas não observados, com base nas probabilidades de n-gramas, conforme descrito [aqui](https://web.stanford.edu/~jurafsky/slp3/4.pdf).

*Principais recursos*

* Oferece suporte a n-gramas de qualquer tamanho.
* Três modos de tokenização:
  * `byte`: Opera em bytes brutos. Cada byte é um token.
  * `codepoint`: Opera em valores escalares Unicode decodificados de UTF-8. Cada ponto de código é um token.
  * `token`: Divide em sequências de espaços em branco Unicode (regex `\s+`). Os tokens são substrings sem espaços em branco; a pontuação faz parte do token quando está adjacente (por exemplo, `you?` é um único token).

**Configuração do modelo**

O código-fonte de exemplo para criar um modelo de Naive Bayes para detecção de idioma está disponível [aqui](https://github.com/nihalzp/ClickHouse-NaiveBayesClassifier-Models), junto com modelos de exemplo e seus respectivos arquivos de configuração [aqui](https://github.com/nihalzp/ClickHouse-NaiveBayesClassifier-Models/tree/main/models).

Uma configuração de exemplo para um modelo de Naive Bayes no ClickHouse:
...
| Parâmetro | Descrição                                                                                                              | Exemplo                                                  | Padrão                |
| --------- | ---------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------- | --------------------- |
| `name`    | Identificador único do modelo.                                                                                         | `language_detection`                                     | Obrigatório           |
| `path`    | Caminho completo para o binário do modelo.                                                                             | `/etc/clickhouse-server/config.d/language_detection.bin` | Obrigatório           |
| `mode`    | Método de tokenização: `byte` (sequências de bytes), `codepoint` (caracteres Unicode) ou `token` (tokens de palavras). | `token`                                                  | Obrigatório           |
| `n`       | Tamanho do n-gram: `1` (uma palavra), `2` (pares de palavras) ou `3` (trios de palavras).                              | `2`                                                      | Obrigatório           |
| `alpha`   | Fator de suavização de Laplace usado durante a classificação para n-gramas que não aparecem no modelo.                  | `0.5`                                                    | `1.0`                 |
| `priors`  | Probabilidades das classes (percentual de documentos pertencentes a uma classe).                                       | 60% classe 0, 40% classe 1                               | Distribuição uniforme |

**Guia de treinamento do modelo**

*Formato de arquivo*

Em formato legível por humanos, para `n=1` e modo `token`, o modelo pode ter esta aparência:
...
Para `n=3` e o modo `codepoint`, pode ser algo como:
...
O formato legível por humanos não é usado diretamente pelo ClickHouse; ele deve ser convertido para o formato binário descrito abaixo.
...
1. `class_id` de 4 bytes (UInt, little-endian).
2. Comprimento de `n-gram`, em 4 bytes (UInt, little-endian).
3. Bytes brutos de `n-gram`.
4. `count` de 4 bytes (UInt, little-endian).

*Requisitos de pré-processamento*

Antes de o modelo ser criado a partir do corpus de documentos, os documentos devem ser pré-processados para extrair n-gramas de acordo com os valores especificados de `mode` e `n`:

1. Adicione marcadores de limite no início e no fim de cada documento com base no modo de tokenização:
...
Para simplificar a criação do modelo nos modos `byte` e `codepoint`, pode ser conveniente primeiro tokenizar o documento (uma lista de bytes no modo `byte`, uma lista de codepoints no modo `codepoint`), adicionar `n - 1` tokens de início no começo e `n - 1` tokens de fim no final e, em seguida, gerar os n-gramas e gravá-los no arquivo serializado.
...
* `model_name` — Nome do modelo pré-configurado. O modelo deve ser definido nos arquivos de configuração do ClickHouse. [`String`](/pt-BR/reference/data-types/string)
* `input_text` — Texto a ser classificado. A entrada é processada exatamente como fornecida (maiúsculas/minúsculas e pontuação preservadas). [`String`](/pt-BR/reference/data-types/string)

**Valor retornado**

ID da classe prevista como um inteiro sem sinal. Os IDs de classe correspondem às categorias definidas durante a construção do modelo. [`UInt32`](/pt-BR/reference/data-types/int-uint)

**Exemplos**

**Classificar o idioma de um texto**
