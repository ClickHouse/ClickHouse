...
| `compression_method`                    | O parâmetro é opcional. Valores aceitos: `none`, `gzip` ou `gz`, `deflate`, `brotli` ou `br`, `xz` ou `LZMA`, `zstd` ou `zst`, `lz4`, `bz2`, `snappy`. Por padrão, o método de compressão é detectado automaticamente pela extensão do arquivo. Para `snappy`, o formato de transmissão é definido pela configuração [snappy&#95;mode](/pt-BR/reference/settings/session-settings/other#snappy_mode) (`basic` por padrão).                                                                                                                                                                                                                                                                                 |
...
| `partition_strategy`                    | O parâmetro é opcional. Valores aceitos: `wildcard` ou `hive`. `wildcard` exige um `{_partition_id}` no caminho, que é substituído pela chave de partição. `hive` não permite caracteres curinga, presume que o caminho é a raiz da tabela e gera diretórios particionados no estilo Hive com IDs Snowflake como nomes de arquivo e o formato do arquivo como extensão. Sem uma estratégia explícita, um caminho com `{_partition_id}` usa `wildcard`. Um caminho com outro glob não usa estratégia de particionamento e ignora `PARTITION BY`. Um caminho sem glob usa `hive` quando `file_like_engine_default_partition_strategy` é `hive`; caso contrário, não usa estratégia de particionamento. |
...
| `extra_credentials`                     | O parâmetro é opcional. Usado para passar um `role_arn` para controle de acesso baseado em função no ClickHouse Cloud. Consulte [Secure S3](/pt-BR/products/cloud/guides/data-sources/accessing-s3-data-securely) para ver as etapas de configuração.                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
<Info>
  **GCS**
  ...
  A URL do GCS segue este formato, pois o endpoint da API XML do Google é diferente do da API JSON:
...
Os argumentos também podem ser passados usando [coleções nomeadas](/pt-BR/concepts/features/configuration/server-config/named-collections). Nesse caso, `url`, `access_key_id`, `secret_access_key`, `format`, `structure`, `compression_method` funcionam da mesma forma, e alguns parâmetros extras são compatíveis:
...
Conte o número total de linhas nos arquivos chamados `file-1.csv`, ..., `file-4.csv`:
...
O glob `**` pode ser usado para percorrer diretórios recursivamente. A consulta a seguir lê todos os arquivos chamados `some_file_1.csv` em `my-test-bucket-768`:
...
As chaves podem ser combinadas com `**` para corresponder a vários nomes de arquivos recursivamente:
...
O mesmo padrão recursivo também funciona com uma URL `s3://`:
...
Você pode adicionar um mapeador de URL personalizado em `config.xml`:
...
## Gravação particionada {#partitioned-write}

### Estratégia de particionamento {#partition-strategy}
...
## Acessando buckets públicos {#accessing-public-buckets}
...
## Usando credenciais do S3 (ClickHouse Cloud) {#using-s3-credentials-clickhouse-cloud}
...
## Trabalhando com arquivos compactados {#working-with-archives}
...
## Inserindo dados {#inserting-data}
...
## configuração use_hive_partitioning {#hive-style-partitioning}
...
## Acessando buckets requester-pays {#accessing-requester-pays-buckets}
...
## Resolução de URLs relativas {#resolving-relative-urls}

A configuração [s3&#95;base](/pt-BR/reference/settings/session-settings/s3#s3_base) permite fornecer uma URL relativa à função `s3`. Quando `s3_base` está definido e o argumento da função não inclui um esquema, ele é resolvido em relação à URL base, de acordo com a [RFC 3986](https://datatracker.ietf.org/doc/html/rfc3986), usando as mesmas regras da configuração [url&#95;base](/pt-BR/reference/settings/session-settings/url#url_base) da função [url](/pt-BR/reference/functions/table-functions/url#resolving-relative-urls). URLs absolutas são transmitidas sem alteração.

A configuração também se aplica ao mecanismo de tabela [S3](/pt-BR/reference/engines/table-engines/integrations/s3) e às funções de tabela que compartilham a configuração `s3` (`s3Cluster`, `gcs`, `oss`). Para o mecanismo de tabela `S3`, a URL resolvida é materializada na definição da tabela armazenada, de modo que a tabela não depende do valor de `s3_base` após sua criação.
...
* [s3&#95;truncate&#95;on&#95;insert](/pt-BR/reference/settings/session-settings/s3#s3_truncate_on_insert) - permite truncar o arquivo antes de inserir dados nele. Desativado por padrão.
* [s3&#95;create&#95;new&#95;file&#95;on&#95;insert](/pt-BR/reference/settings/session-settings/s3#s3_create_new_file_on_insert) - permite criar um novo arquivo a cada inserção se o formato tiver um sufixo. Desativado por padrão.
* [s3&#95;skip&#95;empty&#95;files](/pt-BR/reference/settings/session-settings/s3#s3_skip_empty_files) - permite ignorar arquivos vazios durante a leitura. Ativado por padrão.
* [s3&#95;base](/pt-BR/reference/settings/session-settings/s3#s3_base) - URL base para resolver URLs relativas passadas à função `s3`. Vazio (desativado) por padrão.

## Esquemas Avro aninhados {#nested-avro-schemas}
...
* [S3 engine](/pt-BR/reference/engines/table-engines/integrations/s3)
* [Integrating S3 with ClickHouse](/pt-BR/integrations/connectors/data-ingestion/AWS/integrating-s3-with-clickhouse)
