...
<Note>
  A maioria das funções de tabela precisa do privilégio `CREATE TEMPORARY TABLE` para ser executada, portanto um `SELECT` ou `INSERT`
  que execute uma delas é recusado quando
  [`readonly`](/pt-BR/concepts/features/configuration/settings/permissions-for-queries#readonly) está definido como 1.
</Note>