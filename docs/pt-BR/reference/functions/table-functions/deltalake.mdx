...
As gravações no Delta Lake são um recurso Beta desabilitado por padrão. Habilite-as com o seguinte (`allow_delta_lake_writes` está disponível a partir da versão 26.7; em versões anteriores, use `allow_experimental_delta_lake_writes`):
