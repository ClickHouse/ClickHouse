...
Use a instrução abaixo para alterar a cláusula [`WITH SETTINGS`](#with-settings) de uma projeção existente sem recriar seus dados:
...
Para um [índice de projeção](/pt-BR/reference/engines/table-engines/mergetree-family/mergetree#projection-index), especifique novamente a declaração `INDEX` em vez da consulta `SELECT`:
...
A instrução repete a definição completa da projeção, mas somente a cláusula `WITH SETTINGS` pode ser diferente da definição existente.
A consulta da projeção em si (ou, no caso de um índice de projeção, a expressão e o tipo do índice) deve permanecer a mesma, pois as partes de projeção existentes armazenam dados gerados a partir dela. Para alterá-la, use [`DROP PROJECTION`](#drop-projection) seguido de [`ADD PROJECTION`](#add-projection).
...
Para recriar as partes existentes com as novas configurações, execute [`MATERIALIZE PROJECTION`](#materialize-projection).
...
Os comandos `ADD`, `MODIFY`, `DROP` e `CLEAR` são leves, no sentido de que apenas alteram metadados ou removem arquivos.
...
Desde a v24.8, uma nova configuração no nível da tabela [`deduplicate_merge_projection_mode`](/pt-BR/reference/settings/merge-tree-settings/other#deduplicate_merge_projection_mode) controla o comportamento caso as operações de mesclagem em segundo plano não triviais mencionadas acima ocorram em partes da tabela original.

Mutações de exclusão são outro exemplo de operações de mesclagem de partes que removem linhas nas partes da tabela original. Desde a v24.7, também temos uma configuração para controlar o comportamento em relação às mutações de exclusão acionadas por exclusões leves: [`lightweight_mutation_projection_mode`](/pt-BR/reference/settings/merge-tree-settings/other#lightweight_mutation_projection_mode).
