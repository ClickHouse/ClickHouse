...
`SET variable = value` é um alias para `MODIFY SETTINGS variable = value`. Essa forma é oferecida porque `SET` soa natural e porque digitar a cláusula `SETTINGS` de substituição quando se pretendia uma alteração incremental é um erro comum.
