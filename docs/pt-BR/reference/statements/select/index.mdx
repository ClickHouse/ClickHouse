...
* [cláusula PREWHERE](/pt-BR/reference/statements/select/prewhere) (filtra a única tabela à qual é aplicada, portanto é executada antes de `JOIN` apesar de sua posição nesta lista)
...
Uma consulta também pode ser escrita como uma cadeia linear de transformações com [operadores de pipe](/pt-BR/reference/statements/select/pipe-operators):
...
As cláusulas `GROUP BY`, `ORDER BY` e `LIMIT BY` podem aceitar argumentos posicionais. Para habilitar isso, ative a configuração [enable&#95;positional&#95;arguments](/pt-BR/reference/settings/session-settings/enable-positional-arguments#enable_positional_arguments). Assim, por exemplo, `ORDER BY 1,2` ordenará as linhas da tabela pela primeira e depois pela segunda coluna.
