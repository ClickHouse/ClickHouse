...
Em uma consulta que começa com `FROM`, a cláusula `SELECT` é opcional: quando omitida, a consulta funciona como se `SELECT *` tivesse sido escrito:
...
Esse formato é usado principalmente em consultas com [operadores de pipe](/pt-BR/reference/statements/select/pipe-operators):
...
Consultas `SELECT` com `FINAL` são executadas em paralelo. A configuração [max&#95;final&#95;threads](/pt-BR/reference/settings/session-settings/max#max_final_threads) limita o número de threads usados.
...
`FINAL` pode ser aplicado automaticamente a todas as tabelas de uma consulta usando a configuração [FINAL](/pt-BR/reference/settings/session-settings/other#final), por meio de uma sessão ou de um perfil de usuário.
