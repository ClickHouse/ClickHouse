...
<Note>
  `FROM unnest(...)`, `CROSS JOIN UNNEST(...)` e `LATERAL` do PostgreSQL não são compatíveis. Use `ARRAY JOIN` em vez disso. O nome `unnest` (desde a versão 26.5) é um alias de chamada de função de `arrayJoin` (`SELECT unnest(arr)`), não uma função de tabela.
</Note>
...
[Avaliação de funções em curto-circuito](/pt-BR/reference/settings/session-settings/short-circuit-function-evaluation#short_circuit_function_evaluation) é um recurso que otimiza a execução de expressões complexas em funções específicas, como `if`, `multiIf`, `and` e `or`. Ele evita que possíveis exceções, como divisão por zero, ocorram durante a execução dessas funções.
