...
<Note>
  Quando [join&#95;algorithm](/pt-BR/reference/settings/session-settings/join#join_algorithm) está definido como `partial_merge`, `RIGHT JOIN` e `FULL JOIN` são suportados apenas com a strictness `ALL` (`SEMI`, `ANTI`, `ANY` e `ASOF` não são suportados).
</Note>
...
O tipo de junção padrão pode ser substituído usando a configuração [`join_default_strictness`](/pt-BR/reference/settings/session-settings/join#join_default_strictness).

O comportamento do servidor do ClickHouse para operações `ANY JOIN` depende da configuração [`any_join_distinct_right_table_keys`](/pt-BR/reference/settings/session-settings/other#any_join_distinct_right_table_keys).

**Veja também**

* [`join_algorithm`](/pt-BR/reference/settings/session-settings/join#join_algorithm)
* [`join_any_take_last_row`](/pt-BR/reference/settings/session-settings/join#join_any_take_last_row)
* [`join_use_nulls`](/pt-BR/reference/settings/session-settings/join#join_use_nulls)
* [`partial_merge_join_rows_in_right_blocks`](/pt-BR/reference/settings/session-settings/partial-merge#partial_merge_join_rows_in_right_blocks)
* [`join_on_disk_max_files_to_merge`](/pt-BR/reference/settings/session-settings/join#join_on_disk_max_files_to_merge)
* [`any_join_distinct_right_table_keys`](/pt-BR/reference/settings/session-settings/other#any_join_distinct_right_table_keys)
...
Atualmente, o ClickHouse oferece suporte a `ALL/ANY/SEMI/ANTI INNER/LEFT/RIGHT/FULL JOIN` com condições de desigualdade, além das condições de igualdade. As condições de desigualdade são compatíveis apenas com os algoritmos de junção `hash`, `parallel_hash` e `grace_hash`. Uma condição de não igualdade avaliada durante a junção não pode conter `arrayJoin`, pois deve preservar o número de linhas; uma condição aplicada a apenas um lado e uma chave de igualdade baseada em `arrayJoin` são extraídas antes da junção e não são afetadas; uma condição não disjuntiva de `ALL INNER JOIN` também não é afetada, pois, nesse caso, a condição é aplicada após a junção. Quando a expansão depende de apenas um lado, mova-a para um `ARRAY JOIN` em uma subconsulta antes da junção; uma condição cujo argumento de `arrayJoin` lê colunas de ambos os lados precisa ser reestruturada.
...
<div id="null-values-in-join-keys">
  ## Valores NULL e NaN nas chaves de junção
</div>

`NULL` não é igual a nenhum valor, nem a si mesmo. Isso significa que, se uma chave de junção tiver um valor `NULL` em uma tabela, ela não corresponderá a um valor `NULL` na outra tabela.
...
Observe que a linha com `Charlie` da tabela `A` e a linha com pontuação 88 da tabela `B` não aparecem no resultado por causa do valor `NULL` na chave de junção.

Caso você queira fazer a correspondência de valores `NULL`, use a função `isNotDistinctFrom` para comparar as chaves de junção.
...
Os valores `NaN` em chaves de junção de ponto flutuante não seguem a regra de `NULL` acima.
A comparação escalar de dois valores `NaN` (`NaN = NaN`) resulta em `0`; no entanto, as chaves de junção não são comparadas usando semântica escalar — chaves `NaN` podem corresponder.
Se isso de fato ocorre é um detalhe de implementação e depende do algoritmo de junção, do tipo de chave e das configurações da sessão.
Não dependa de um comportamento específico.
Se você precisar que linhas com `NaN` não correspondam, mapeie-as para valores `NULL`: `ON if(isNaN(A.id), NULL, A.id) = B.id`.
Em um `ASOF JOIN`, a coluna de correspondência mais próxima é comparada por ordenação, que `NaN` não suporta — filtre essas linhas em ambos os lados.
...
Ao fazer `JOIN` entre tabelas, podem surgir células vazias. A configuração [join&#95;use&#95;nulls](/pt-BR/reference/settings/session-settings/join#join_use_nulls) define como o ClickHouse preenche essas células.
...
Por padrão, o ClickHouse usa o algoritmo [hash join](https://en.wikipedia.org/wiki/Hash_join). O ClickHouse usa a `right_table` e cria uma tabela hash para ela na RAM. Se `join_algorithm = 'auto'` estiver habilitado, após determinado limite de consumo de memória, o ClickHouse passa a usar o algoritmo [merge](https://en.wikipedia.org/wiki/Sort-merge_join) de junção. Para obter uma descrição dos algoritmos de junção, consulte a configuração [join&#95;algorithm](/pt-BR/reference/settings/session-settings/join#join_algorithm).

Se você precisar restringir o consumo de memória da operação de junção `JOIN`, use as seguintes configurações:

* [max&#95;rows&#95;in&#95;join](/pt-BR/reference/settings/session-settings/max-rows#max_rows_in_join) — Limita o número de linhas na tabela hash.
* [max&#95;bytes&#95;in&#95;join](/pt-BR/reference/settings/session-settings/max-bytes#max_bytes_in_join) — Limita o tamanho da tabela hash.

Quando qualquer um desses limites é atingido, o ClickHouse age conforme instruído pela configuração [join&#95;overflow&#95;mode](/pt-BR/reference/settings/session-settings/join#join_overflow_mode).
