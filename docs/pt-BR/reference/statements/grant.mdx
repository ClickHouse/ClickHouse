...
* O privilégio `MODIFY SETTING` permite modificar as configurações do motor de tabela. Ele não afeta configurações nem parâmetros de configuração do servidor.
...
Permite que um usuário execute consultas [SYSTEM](/pt-BR/reference/statements/system) de acordo com a seguinte hierarquia de privilégios.
...
* `SYSTEM`. Nível: `GROUP`
  ...
  * `SYSTEM RELOAD`. Nível: `GROUP`
    ...
    * `SYSTEM RELOAD DICTIONARY`. Nível: `GLOBAL`. Aliases: `SYSTEM RELOAD DICTIONARIES`, `RELOAD DICTIONARY`, `RELOAD DICTIONARIES`, `SYSTEM UNLOAD DICTIONARY`, `SYSTEM UNLOAD DICTIONARIES`, `UNLOAD DICTIONARY`, `UNLOAD DICTIONARIES`
  ...
  * `SYSTEM BACKGROUND`. Nível: `GROUP`
    * `SYSTEM STREAMING ENGINES`. Nível: `TABLE`
    * `SYSTEM VIEWS`. Nível: `VIEW`. Aliases: `SYSTEM REFRESH VIEW`, `SYSTEM START VIEWS`, `SYSTEM STOP VIEWS`, `SYSTEM START VIEW`, `SYSTEM STOP VIEW`, `SYSTEM PAUSE VIEWS`, `SYSTEM PAUSE VIEW`, `SYSTEM CANCEL VIEW`, `REFRESH VIEW`, `START VIEWS`, `STOP VIEWS`, `START VIEW`, `STOP VIEW`, `PAUSE VIEWS`, `PAUSE VIEW`, `CANCEL VIEW`
...
Permite usar fontes de dados externas. Aplica-se a [motores de tabela](/pt-BR/reference/engines/table-engines/index) e [funções de tabela](/pt-BR/reference/functions/table-functions/index).
...
[`configuração do servidor` `display_secrets_in_show_and_select`](/pt-BR/reference/settings/server-settings/settings/other#display_secrets_in_show_and_select)
quanto a
[`configuração de formato` `format_display_secrets_in_show_and_select`](/pt-BR/reference/settings/formats/format#format_display_secrets_in_show_and_select)
estiverem ativadas.
