...
| `execute_direct`              | Se `execute_direct` = `1`, `command` será procurado dentro da pasta user&#95;scripts especificada por [user&#95;scripts&#95;path](/pt-BR/reference/settings/server-settings/settings/user#user_scripts_path). Argumentos adicionais do script podem ser especificados usando separação por espaço em branco. Exemplo: `script_name arg1 arg2`. Se `execute_direct` = `0`, `command` é passado como argumento para `bin/sh -c`. O valor padrão é `1`. Opcional. |
