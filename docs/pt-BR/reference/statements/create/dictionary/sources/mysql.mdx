...
| `ssl_ca`, `ssl_cert`, `ssl_key` | As mesmas credenciais, como caminhos para arquivos no servidor. Permitido apenas para um dicionário definido em um arquivo de configuração do servidor; veja abaixo. Opcional.                                                                                                                                                                                         |
...
<Note>
  `ssl_ca`, `ssl_cert` e `ssl_key` nomeiam arquivos que o servidor abre com seus próprios privilégios; portanto, são aceitos apenas para um dicionário definido em um arquivo de configuração do servidor. Um dicionário criado com uma consulta `CREATE DICTIONARY` deve fornecer o conteúdo em `ssl_ca_pem`, `ssl_cert_pem` e `ssl_key_pem`. Esses valores são mascarados nos logs e em consultas `SHOW`, da mesma forma que as senhas.
</Note>
