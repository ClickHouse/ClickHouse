...
| `ssl_ca_pem`                    | Conteúdo do certificado da CA usado para verificar o certificado do servidor MySQL. Opcional.                                                                                                                                                                                                                                                                          |
| `ssl_cert_pem`                  | Conteúdo do certificado do cliente, para autenticação baseada em certificado. Opcional.                                                                                                                                                                                                                                                                                |
| `ssl_key_pem`                   | Conteúdo da chave privada associada a `ssl_cert_pem`. Opcional.                                                                                                                                                                                                                                                                                                        |
| `ssl_ca`, `ssl_cert`, `ssl_key` | As mesmas credenciais, como caminhos para arquivos no servidor. Permitido apenas para um dicionário definido em um arquivo de configuração do servidor ou por meio de uma coleção nomeada definida nele; veja abaixo. Opcional.                                                                                                                                        |
...
<Note>
  `ssl_ca`, `ssl_cert` e `ssl_key` nomeiam arquivos que o servidor abre com seus próprios privilégios; portanto, são aceitos apenas para um dicionário definido em um arquivo de configuração do servidor ou por meio de uma coleção nomeada definida nele. Uma consulta `CREATE DICTIONARY` que especifica as credenciais TLS diretamente deve fornecer seu conteúdo em `ssl_ca_pem`, `ssl_cert_pem` e `ssl_key_pem`. Esses valores são mascarados nos logs e em consultas `SHOW`, da mesma forma que as senhas.
</Note>
...
É possível conectar-se ao MySQL em um host local via sockets. Para isso, defina `host` e `socket`.
