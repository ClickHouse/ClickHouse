...
slug: /pt-BR/reference/statements/create/dictionary/attributes
...
Quando a chave composta consiste em um único atributo, o valor da chave pode ser passado diretamente, sem encapsulá-lo em `tuple`. Por exemplo, tanto `dictGetString('dict_name', 'attr_name', 'key')` quanto `dictGetString('dict_name', 'attr_name', tuple('key'))` são válidos.
