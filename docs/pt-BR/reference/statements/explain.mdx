...
<Note>
  A versão atual do `EXPLAIN ANALYZE` não oferece suporte a consultas executadas em modo distribuído.
</Note>
...
* `Time` — tempo total dividido entre as fases de planejamento (isto é, criação do plano + otimização do plano + construção do pipeline) e de execução (execução do pipeline).
* `Read` — linhas e bytes não comprimidos lidos das tabelas, com a taxa de processamento — os mesmos números que o rodapé de uma consulta normal informa como &quot;Processed&quot;.
* `Peak memory` — pico de memória usado pela consulta.

Agora, vamos analisar as novas linhas que aparecem no plano da consulta.
...
`<n>` é o número de processadores na etapa. Uma grande diferença entre `median` e `max` indica um desequilíbrio de carga entre processadores paralelos.
