...
* `empirical_reason` — por que a estimativa empírica não pôde ser executada. Exibido apenas com `empirical_status: unsupported`. Por exemplo, um valor diferente de zero para `merge_tree_min_rows_for_seek` ou `merge_tree_min_bytes_for_seek` faz com que uma leitura real agrupe intervalos de marcas, algo que a contagem por grânulo não modela; portanto, a estimativa recorre a `statistical` ou `applicability_only`.
