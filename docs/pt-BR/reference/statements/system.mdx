...
Limpa o cache do índice primário, que mantém em memória as chaves primárias das tabelas [`MergeTree`](/pt-BR/reference/engines/table-engines/mergetree-family/mergetree).
Seu tamanho é configurado pela definição no nível do servidor [`primary_index_cache_size`](/pt-BR/reference/settings/server-settings/settings#primary_index_cache_size).
