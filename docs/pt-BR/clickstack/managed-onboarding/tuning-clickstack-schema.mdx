...
<Steps>
  <Step>
    ## Materialize atributos consultados com frequência
  ...
  <Step>
    ...
    * **Text indexes** (`text(tokenizer = ...)`) em colunas de string e em arrays `mapKeys`/`*AttributeItems`. O schema padrão de logs já inclui esses índices.
  ...
  <Step>
    ## Pré-calcule agregações repetidas
  ...
  <Step>
    ## Escolha uma chave primária para seu padrão de acesso

    A chave primária controla como as linhas são ordenadas em disco. Filtros nas colunas iniciais dessa chave permitem que o ClickHouse vá direto para a faixa relevante; filtros que não começam com uma dessas colunas varrem a partição inteira.
    ...
    O ClickHouse não permite alterar a chave primária diretamente, então isto é uma migração de tabela, e não um simples `ALTER`. O guia de ajuste de desempenho mostra como criar a nova tabela, redirecionar a ingestão e usar uma tabela `Merge` para que os dashboards existentes continuem funcionando com dados antigos e novos.
