...
Ao criar ou atualizar dashboards pela API externa (`POST` / `PUT` `/api/v2/dashboards`), defina `color` em uma configuração de bloco numérico do construtor como um token de paleta. Valores hex brutos são rejeitados.
