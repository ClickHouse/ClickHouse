---
noindex: true
title: "Implantar o ClickHouse no GCP GKE"
sidebarTitle: "Implantar no GCP"
description: "Este tutorial mostra como implantar o ClickHouse Private no Google Cloud Platform usando o Google Kubernetes Engine (GKE)."
---

Este tutorial mostra como implantar o ClickHouse Private no Google Cloud Platform usando o Google Kubernetes Engine (GKE). Ele abrange implantações padrão e implantações em conformidade com o FIPS 140-2. As etapas específicas do FIPS estão marcadas com **(FIPS)** ao longo do tutorial -- defina `ENABLE_FIPS=true` em `00-env.sh` para ativar o modo FIPS.

O tutorial está organizado em scripts numerados (`00-env.sh` a `19-verify`) que você executa em sequência. Cada script é independente e carrega o arquivo de ambiente.

Para ver as especificações detalhadas da infraestrutura, consulte [reference/infrastructure-requirements.md](/pt-BR/products/clickhouse-private/reference/infrastructure-requirements-reference).
...
* Projeto do GCP com faturamento ativado
* CLI `gcloud` instalada e autenticada
* `kubectl` instalado
* `helm` v3.x instalado
* `skopeo` instalado (para copiar imagens de container)
* Credenciais da AWS com acesso de leitura ao ECR do ClickHouse (para cópia de imagens)
...
* **(FIPS)** Acesso à imagem de container Red Hat UBI8

***

<Steps>
  <Step stepNumber={0} title="Passo 0: Configuração da conta de serviço" id="step-0-service-account-setup">
    ...
    3. Insira o nome `clickhouse-deployer` e o ID `clickhouse-deployer`
    4. Adicione os seguintes roles:
    ...
    1. Conceda a permissão Token Creator à conta de serviço do Compute Engine do bastion.
    ...
    ### Configurar impersonação no bastion
    ...
    Para remover a impersonação mais tarde:
  ...
  <Step stepNumber={1} title={<>Passo 1: Definir Variáveis de Ambiente (<code>00-env.sh</code>)</>} id="step-1-set-environment-variables-00-envsh">
    Crie este arquivo e carregue-o antes de executar qualquer outro script. Atualize todos os valores de acordo com o seu ambiente.
    ...
    **Importante:** Carregue este arquivo antes de executar outros scripts:
  ...
  <Step stepNumber={2} title={<>Etapa 2: Criar o Artifact Registry (<code>01-artifact-registry.sh</code>)</>} id="step-2-create-artifact-registry-01-artifact-registrysh">
    ```bash expandable
    #!/bin/bash
    set -e
    source ./00-env.sh
  ...
  <Step stepNumber={3} title={<>Etapa 3: Copiar imagens de contêiner (<code>02-copy-images.sh</code>)</>} id="step-3-copy-container-images-02-copy-imagessh">
    Requer [skopeo](https://github.com/containers/skopeo) para preservar todas as arquiteturas.
  ...
  <Step stepNumber={4} title={<>Etapa 4: Configurar VPC e Subnet (<code>03-vpc-subnet.sh</code>)</>} id="step-4-configure-vpc-and-subnet-03-vpc-subnetsh">
    Este script pressupõe que você já tenha uma VPC existente com um host bastion. Ele cria ou atualiza a subnet do GKE com os intervalos secundários necessários.

    **Importante: Requisitos de alinhamento de CIDR** - blocos `/14`: o terceiro octeto deve ser divisível por 4 (por exemplo, `10.244.0.0`, NÃO `10.245.0.0`) - blocos `/20`: o quarto octeto deve ser 0, e o terceiro octeto deve estar alinhado a 16 (por exemplo, `10.252.0.0`)
  ...
  <Step stepNumber={5} title={<>Etapa 5: Criar o Cloud NAT (<code>04-cloud-nat.sh</code>)</>} id="step-5-create-cloud-nat-04-cloud-natsh">
    Os nós privados do GKE exigem o Cloud NAT para acesso de saída para a internet.
  ...
  <Step stepNumber={6} title={<>Etapa 6: Criar conta de serviço do GCP (<code>05-service-account.sh</code>)</>} id="step-6-create-gcp-service-account-05-service-accountsh">
    ```bash
    #!/bin/bash
    set -e
    source ./00-env.sh
  ...
  <Step stepNumber={7} title={<>Etapa 7: Criar um bucket no GCS (<code>06-gcs-bucket.sh</code>)</>} id="step-7-create-gcs-bucket-06-gcs-bucketsh">
    ```bash expandable
    #!/bin/bash
    set -e
    source ./00-env.sh
  ...
  <Step stepNumber={8} title={<>Etapa 8: Criar cluster do GKE (<code>07-gke-cluster.sh</code>)</>} id="step-8-create-gke-cluster-07-gke-clustersh">
    Cria um **cluster do GKE totalmente privado** -- os nós worker têm apenas IPs internos, o plano de controle não tem endpoint público, e todo o acesso deve partir de dentro da VPC via bastion.
    ...
    **Configurações de Cluster Privado:**
    ...
    | `--master-ipv4-cidr`                  | `172.16.0.0/28`            | Faixa de IPs do plano de controle                    |
  ...
  <Step stepNumber={9} title={<>Etapa 9: Criar Node Pools (<code>08-node-pools.sh</code>)</>} id="step-9-create-node-pools-08-node-poolssh">
    | Node Pool | Tipo de máquina                             | Disco                        | Labels                   | Taints                                      |
    | --------- | ------------------------------------------- | ---------------------------- | ------------------------ | ------------------------------------------- |
    | Keeper    | n2-standard-4 (dev) / n2-standard-8 (prod)  | 20GB pd-ssd                  | `clickhouseGroup=keeper` | `clickhouse.com/do-not-schedule:NoSchedule` |
    | Servidor  | n2-standard-8 (dev) / n2-standard-64 (prod) | 20GB pd-standard + Local SSD | `clickhouseGroup=server` | `clickhouse.com/do-not-schedule:NoSchedule` |
  ...
  <Step stepNumber={10} title={<>Passo 10: Implantar o Conjunto de Daemon do Local SSD (<code>09-local-ssd-daemonset.sh</code>)</>} id="step-10-deploy-local-ssd-daemonset-09-local-ssd-daemonsetsh">
    O GKE Container-Optimized OS tem um sistema de arquivos raiz somente leitura, portanto não é possível criar `/nvme`. O Conjunto de Daemon formata e monta o Local SSD em `/mnt/disks/ssd0`.

    **Importante:** Implante isso **antes** do cluster do ClickHouse para evitar condições de corrida.
  ...
  <Step stepNumber={11} title={<>Etapa 11: Adicionar rótulos de topologia (<code>10-topology-labels.sh</code>)</>} id="step-11-add-topology-labels-10-topology-labelssh">
    ```bash
    #!/bin/bash
    set -e
    source ./00-env.sh
  ...
  <Step stepNumber={12} title="Passo 12: Instalar os Pré-requisitos do Kubernetes" id="step-12-install-kubernetes-prerequisites">
    ### Instalar VolumeSnapshot CRDs (`11-volumesnapshot-crds.sh`)
    ...
    ### Configurar o Workload Identity (`12-workload-identity.sh`)
    ...
    **Alinhamento de Workload Identity:**
    ...
    | ServiceAccount do K8s (criada pelo helm) | `ch-$CLUSTER_NAME-sa` no espaço de nomes `ns-$CLUSTER_NAME` |
    | Anotação da ServiceAccount do K8s        | `iam.gke.io/gcp-service-account=<GSA_EMAIL>`                |
    | Membro do binding do IAM                 | `serviceAccount:$PROJECT.svc.id.goog[<namespace>/<ksa>]`    |

    ### Instalar StorageClass (`13-storageclass.sh`) -- Opcional

    **Nota:** O GKE usa `topology.gke.io/zone` em vez de `topology.kubernetes.io/zone`.
  ...
  <Step stepNumber={13} title="Etapa 13: Etapas do FIPS (Condicional)" id="step-13-fips-steps-conditional">
    Ignore esta seção se `ENABLE_FIPS=false`.

    ### Criar certificados compatíveis com FIPS (`14-create-fips-certs.sh`)

    Certificados compatíveis com FIPS devem ser gerados com algoritmos aprovados pelo FIPS (RSA de no mínimo 3072 bits, SHA-256 ou superior) em um ambiente com FIPS habilitado. O script executa um pod do Kubernetes com Red Hat UBI8 no cluster do GKE, com a política criptográfica do FIPS habilitada.

    **Tipos de certificado necessários:**

    | Certificado | Arquivos                                 | SANs obrigatórios                                                                 |
    | ----------- | ---------------------------------------- | --------------------------------------------------------------------------------- |
    | CA          | `ca/ca.crt`, `ca/ca.key`                 | N/A                                                                               |
    | Servidor    | `server/server.crt`, `server/server.key` | `*.c-${CLUSTER_NAME}-server-headless.ns-${CLUSTER_NAME}.svc.${KUBERNETES_DOMAIN}` |
    | Cliente     | `client/client.crt`, `client/client.key` | Nenhum                                                                            |
    | Keeper      | `keeper/keeper.crt`, `keeper/keeper.key` | `*.c-${CLUSTER_NAME}-keeper-headless.ns-${CLUSTER_NAME}.svc.${KUBERNETES_DOMAIN}` |

    Para ver o procedimento completo de geração de certificados, consulte [how-to/configure-fips-certificates.md](/pt-BR/products/clickhouse-private/guides/generate-fips-compliant-certificates-for-clickhouse).

    ### Criar Secrets de certificados (`15-create-cert-secrets.sh`)
    ...
    **Estrutura do Secret:**
    ...
    | Nome do Secret                       | Chaves                                                           | Finalidade                                     |
    | ------------------------------------ | ---------------------------------------------------------------- | ---------------------------------------------- |
    | `${CLUSTER_NAME}-server-cert-secret` | `ca.crt`, `server.crt`, `server.key`, `client.crt`, `client.key` | TLS do servidor e conexões de saída do cliente |
    | `${CLUSTER_NAME}-keeper-cert-secret` | `ca.crt`, `keeper.crt`, `keeper.key`                             | TLS do Keeper                                  |

    ***
  </Step>

  <Step stepNumber={14} title={<>Passo 14: Instalar o Operator (<code>16-install-operator.sh</code>)</>} id="step-14-install-operator-16-install-operatorsh">
    ```bash expandable
    #!/bin/bash
    set -e
    source ./00-env.sh
  ...
  <Step stepNumber={15} title={<>Passo 15: Implantar o ClickHouse Cluster (<code>17-install-cluster.sh</code>)</>} id="step-15-deploy-clickhouse-cluster-17-install-clustersh">
    ```bash expandable
    #!/bin/bash
    set -e
    source ./00-env.sh
    ...
    * `storageClass.create=false` -- Não criar a StorageClass da AWS
    * `storageClass.name="premium-rwo"` -- Usar a StorageClass SSD nativa do GCP
    * `server.storage.s3.endpoint` -- `https://storage.googleapis.com` para a API compatível com S3 do GCS
    * `server.storage.s3.useEnvironmentCredentials=true` -- Necessário para Workload Identity
    * `keeper.storage.s3.useEnvironmentCredentials=true` -- **Também deve ser definido para o Keeper**

    ***
  </Step>

  <Step stepNumber={16} title={<>Etapa 16: Vincular o Workload Identity (`18-bind-helm-workload-identity.sh`)</>} id="step-16-bind-workload-identity-18-bind-helm-workload-identitysh">
    ...
    **Importante:** Os pods do Keeper e do servidor criados durante a etapa 15 são iniciados antes de essa vinculação existir, portanto falharão com erros 403 do GCS. Este script reinicia os pods após a vinculação.
  ...
  <Step stepNumber={17} title="Passo 17: Verifique a instalação" id="step-17-verify-installation">
    ### Modo padrão (sem TLS)
    ...
    Com `openSSL.required=true`, o ClickHouse aceita apenas conexões em portas seguras. Use a porta **9440** (TLS nativo):
    ...
    Para produção com certificados assinados por uma CA:
    ...
    **Importante (FIPS):** As portas 9000 (protocolo nativo sem criptografia) e 8123 (HTTP sem criptografia) recusarão conexões quando `openSSL.required=true`. Use a porta 9440 (TLS nativo) ou 8443 (HTTPS).
...
| Zonas de disponibilidade | Zonas do GKE                | Chave de topologia: `topology.gke.io/zone`                 |
