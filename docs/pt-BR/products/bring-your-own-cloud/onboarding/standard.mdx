...
<Steps>
  ...
  <Step title="Configure a infraestrutura de BYOC" id="setup-byoc-infrastructure">
    ...
    * **Intervalo CIDR da VPC/VNet**: Por padrão, usamos `10.0.0.0/16` como intervalo CIDR para a VPC BYOC (AWS/GCP) ou VNet (Azure). Se você planeja usar peering de VPC/VNet com outra conta, certifique-se de que os intervalos CIDR não se sobreponham. Aloque um intervalo CIDR adequado para BYOC, com tamanho mínimo de `/23` para acomodar os workloads necessários. O mínimo de `/23` comporta apenas cerca de 30 pods de servidor — cada réplica consome endereços IP; portanto, aloque um intervalo maior (recomenda-se pelo menos `/22`) se você espera que a implantação cresça.
