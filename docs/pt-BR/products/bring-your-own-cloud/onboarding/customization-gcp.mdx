...
<Steps>
  ...
  <Step title="Configure a infraestrutura do BYOC" id="set-up-byoc-infrastructure">
    <Note>
      Ao clicar em **Set up Infrastructure**, o ClickHouse Cloud executa automaticamente a [validação prévia](/pt-BR/products/bring-your-own-cloud/onboarding/standard#preflight-validation) antes do provisionamento, verificando se a conta de serviço de gerenciamento tem as permissões necessárias e se as APIs necessárias do Google Cloud estão ativadas. A validação automática de uma VPC própria ainda não faz parte da verificação prévia do GCP, portanto, certifique-se de que sua VPC atende aos [requisitos de rede](#configure-existing-vpc) acima antes de começar.
    </Note>
