...
<Steps>
  <Step>
    ...
    Ao realizar a [configuração inicial do BYOC](/pt-BR/cloud/reference/byoc/onboarding/standard), desative as permissões de gravação do IAM no role de gerenciamento. Com o template do CloudFormation, defina o parâmetro `IncludeIAMWritePermissions` como `false`. Com o módulo Terraform:
