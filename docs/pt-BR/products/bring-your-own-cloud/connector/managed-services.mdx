...
No modo managed, o connector executa um terceiro componente, o executor. É por meio dele que o ClickHouse Cloud opera os serviços ClickHouse no seu cluster do Kubernetes. Esta página aborda como criar um serviço, como consultar seu status, o que o ClickHouse Cloud faz depois que um serviço passa a existir e como um serviço é desativado.

<Note>
  Habilitar o modo managed e criar um serviço exigem suporte que ainda não está no lançamento público. A CLI pública `v0.16.0` não possui as flags de init `--managed` e `--no-managed` nem o workflow de criação pelo cliente descrito abaixo. Confirme a [disponibilidade do modo managed](/pt-BR/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) com o seu account team antes de fazer o enroll ou preparar recursos; a criação de serviços também requer suporte correspondente no seu connector endpoint e no ClickHouse Cloud.
</Note>

## O que o modo gerenciado faz {#what-managed-mode-does}

O executor é um daemon no mesmo binary `clicklink` que o scraper e o solucionador de problemas. Ele mantém um canal WebSocket de saída até o connector endpoint e recebe comandos de ciclo de vida do ClickHouse Cloud. Ele aplica cada comando ao único cluster do Kubernetes para o qual está configurado e informa o resultado por esse mesmo canal. Assim como os demais componentes, ele estabelece apenas connections de saída: o ClickHouse Cloud nunca se conecta ao seu cluster, e a API local do executor faz bind no loopback.

O modo gerenciado está disponível no Amazon EKS com S3 storage, como parte do private preview. Sua account team da ClickHouse o habilita ao registrar seu ambiente. O `init` recusa um kube context que não aponte para um EKS cluster.

Cada serviço precisa de três cloud resources de sua propriedade: um bucket de dados, um bucket de Backup e uma IAM role que os pods do ClickHouse assumem para acessá-los. Você os cria com suas próprias credentials antes de o serviço existir e os remove depois que ele deixa de existir. O executor não guarda credentials dos seus buckets nem do IAM e nunca exclui dados. Suas únicas chamadas à nuvem vão para o Amazon ECR e o STS. Ele faz login no registry quando uma sincronização de plataforma ou uma criação baixa um chart. Durante uma atualização de plataforma, ele verifica as imagens usando a role de pull read-only. Ele não faz chamadas ao S3 nem ao IAM. Para cada serviço, ele cria um `Service` do Kubernetes do tipo LoadBalancer, que o controller de load-balancer do seu cluster materializa como um NLB interno na sua conta.

Em uma VM, o executor roda como o systemd unit `clicklink-executor`, ao lado dos outros dois. No Kubernetes, ele é uma Implantação de réplica única no namespace do connector. Ele expõe health e metrics na porta 8086 e sua API local em `127.0.0.1:9999`.

## Habilitar o modo managed {#enable-managed-mode}

<Note>
  A CLI pública `v0.16.0` não oferece suporte a `init --managed` nem ao prompt de modo managed descrito abaixo. Primeiro, obtenha uma compilação e uma implantação com [suporte a modo managed](/pt-BR/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) junto ao seu account team.
</Note>

Você escolhe o modo managed no momento do enroll: passe `--managed` para `clicklink clctl init` ou responda ao prompt em um terminal. Em uma VM, o `init` obtém o cluster a partir do kubeconfig deste host (use `--cluster-name` quando ele contiver mais de um EKS cluster) e concede o acesso cluster-wide do executor de forma inline. No Kubernetes, passe `--egress-cidrs` com os CIDRs do seu connector endpoint para que o chart prepare sua NetworkPolicy de negação padrão já habilitada.
...
Este workflow não está disponível na versão pública `v0.16.0`. Antes de preparar os recursos, confirme a [disponibilidade de criação de serviço](/pt-BR/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) para sua CLI, connector endpoint e implantação do ClickHouse Cloud.
...
Apenas o `prepare` precisa de AWS credentials: é ele que cria os S3 buckets e o IAM role. O `instances create` precisa da configuração e das credentials do connector, e acessa a API local do executor somente no caso de `--wait`. Em uma VM, execute ambos como root no host do connector. No Kubernetes, execute ambos a partir de uma workstation com um contexto kube para o cluster gerenciado:

* O `prepare` precisa de uma cópia da configuração do connector (`--config`), de um `kubectl port-forward` para a API local do executor e de `--output-dir` apontando para um diretório gravável. Ele valida o service name junto ao executor e se recusa a executar quando não consegue alcançá-lo.
* O `instances create` lê os arquivos de credencial indicados na configuração a partir dos Secrets `clicklink-hmac` e `clicklink-mtls` quando este host não os possui, e informa cada leitura. Adicione `--connector-namespace` quando o espaço de nomes do connector não for `clicklink`. Somente o `instances create` conta com esse fallback.
...
<Steps>
  <Step title="Prepare o serviço" id="prepare">
    ...
    4. **Resumo.** Gera a senha do usuário `default` e grava o corpo de criação em `<output-dir>/_prepare/<name>.create.json` (mode `0600`; ele carrega apenas os hashes da senha). Imprime o próximo comando exato em uma linha `Next:`.

    <Warning>
      O `prepare` imprime a senha do usuário `default` uma única vez, em stderr, e nunca mais. Nem o corpo de criação nem o `--output json` a contêm, e o ClickHouse Cloud recebe apenas seus hashes. Guarde-a antes de prosseguir. Uma reexecução que encontre o corpo de criação mantém os hashes já gravados e informa isso.
    </Warning>
...
O executor responde a consultas de status por meio de sua API local. A API escuta em `127.0.0.1:9999` e não possui autenticação própria. Em uma VM, execute os comandos no host. No Kubernetes, abra primeiro um redirecionamento de porta e aponte os comandos para ele:
...
* **Scale.** Um serviço tem um número fixo de réplicas definido pelo ClickHouse Cloud, limitado por uma configuração do ClickHouse Cloud (20 na configuração padrão). Não há autoscaling.
...
* **Backups.** Os backups são executados quando o ClickHouse Cloud os aciona e são gravados no seu bucket de backup; a exclusão do backup os remove.
...
As operações de criação, scale e start são concluídas de forma assíncrona. O executor reporta o comando como em execução assim que aplica a definição e envia um relatório de progresso a cada 2 minutos. Ele reporta o resultado final assim que as réplicas do servidor ficam prontas. Se elas não ficarem prontas em até 2 horas, ele reporta o comando como falho. Caso as réplicas fiquem disponíveis depois, ele retorna o serviço para `running`.

Os subcomandos `instances scale`, `instances patch` e `instances delete` enviam comandos diretamente para a API local do executor, sem o envolvimento do ClickHouse Cloud. Não os utilize, a menos que o account team do ClickHouse peça que você execute algum deles; a [CLI reference](/pt-BR/products/bring-your-own-cloud/connector/reference/cli#instances-scale) descreve o que cada um faz.
...
Conceda as permissões ao usuário do ClickHouse via SQL, como acima, e não com `--ch-user-via cr`: a definição do serviço pertence ao ClickHouse Cloud, que a reaplica, removendo qualquer usuário adicionado ali. Quando o serviço é excluído, o executor remove o pacote do solucionador de problemas em uma VM. No Kubernetes, o Secret do pacote e o ServiceAccount permanecem até que você os exclua, conforme listado em [desinstalação](/pt-BR/products/bring-your-own-cloud/connector/operations#uninstall-kubernetes).

## Excluir um serviço {#delete-a-service}

Não há um comando de exclusão disponível ao cliente por meio do connector endpoint: peça ao seu account team da ClickHouse para excluir o serviço. O ClickHouse Cloud o encerra, e o executor exclui o workload e seu espaço de nomes (`terminating`, depois `terminated`). Nada na AWS é afetado: os buckets, seus dados e a IAM role permanecem até que você os remova.

Assim que o serviço reportar `terminated`, desfaça o que o `prepare` criou. Execute o comando no mesmo lugar em que executou o `prepare`, com as mesmas AWS credentials. No Kubernetes, isso significa a mesma workstation, a mesma cópia de configuração e o mesmo diretório de saída, além de um redirecionamento de porta aberto para o executor:
...
Enquanto nenhum executor estiver conectado, o ClickHouse Cloud não tem como repassar novos trabalhos a ele. Uma criação, exclusão ou sincronização de plataforma já aceita é mantida e repetida. Uma criação é repetida por 30 minutos a partir do último relatório de progresso, uma exclusão por 2 horas e uma sincronização de plataforma por até 10 tentativas. Ultrapassado esse limite, o seu connector endpoint marca o comando como falho. Todos os demais comandos de ciclo de vida (scale, stop, start, restart, backup, exclusão de backup) são recusados enquanto nenhum executor estiver conectado, e não enfileirados. Uma criação ou exclusão enviada enquanto nenhum executor estiver conectado é recusada da mesma forma; apenas os comandos já aceitos são repetidos.

Um comando que o executor já aceitou continua executando até a conclusão, e o executor envia na próxima conexão o resultado que não conseguiu reportar.

Os dois comandos de cliente têm suas próprias dependências. `instances create` exige que o seu connector endpoint esteja acessível a partir do host em que ele é executado. `--wait`, `instances list`, `instances get` e `commands list` precisam da API local do executor, portanto exigem que o executor esteja em execução. O token de uma aprovação de plataforma expira conforme o próprio relógio, independentemente da conectividade; consulte [a janela de aprovação](/pt-BR/products/bring-your-own-cloud/connector/platform-updates#the-approval-window).