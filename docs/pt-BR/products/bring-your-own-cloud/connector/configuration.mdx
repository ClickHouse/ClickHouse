...
O modo gerenciado executa o executor, que aplica ao seu cluster o ciclo de vida de serviço conduzido pelo ClickHouse Cloud. O `init` o ativa no momento da inscrição e preenche a identidade do cluster (consulte [habilitar o modo gerenciado](/pt-BR/products/bring-your-own-cloud/connector/managed-services#enable-managed-mode)); as chaves abaixo são as que você talvez precise revisitar depois. Para ativar o modo gerenciado em um conector instalado sem ele, ou para desativá-lo, execute novamente o `init` com `--force` e `--managed` ou `--no-managed`. O `--no-managed` interrompe o executor, mas mantém seu grant no cluster; remova-o conforme descrito em [interromper o modo gerenciado](/pt-BR/products/bring-your-own-cloud/connector/reference/faq#stop-managed-mode). Como toda execução do `init`, esta precisa de um entry point: `--handoff`, caso você tenha mantido o pacote, ou `--enroll` com um token novo, conforme descrito em [reexecuções e recuperação](/pt-BR/products/bring-your-own-cloud/connector/operations#re-runs-and-recovery).
...
Quando o endpoint do conector estiver por trás de uma CA privada dentro da sua fronteira, passe `--api-private-ca` para `init`. Isso configura `api.tls.caFile: /etc/clicklink/secrets/mtls/ca.crt`, e o conector verifica o endpoint em relação às raízes do sistema mais a cadeia de CAs do pacote de inscrição. Em uma VM, o equivalente é `api.tls.ca_file` em `/etc/clicklink/config.yaml`: `init` instala a cadeia do pacote em `/etc/clicklink/tls/ca.crt`, e o conector a acrescenta às raízes do sistema da mesma forma. Para inscrição e assinatura de certificados em ambientes totalmente isolados da internet, consulte [onboarding](/pt-BR/products/bring-your-own-cloud/connector/onboarding).
