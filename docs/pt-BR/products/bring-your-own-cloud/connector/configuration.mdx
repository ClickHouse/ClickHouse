...
description: 'Configure o ClickHouse Connector: modo gerenciado, instâncias do ClickHouse, listas de permissões de operadores, políticas de rede, redação, espelhos privados e armazenamento'
keywords: ['BYOC', 'cloud', 'bring your own cloud', 'conector', 'configuração', 'modo gerenciado']
doc_type: 'guide'
---
...
Esta página aborda as alterações de configuração que você provavelmente fará após instalar o conector. Para ver todas as chaves, seus valores padrão e significados, consulte a [referência de configuração](/pt-BR/products/bring-your-own-cloud/connector/reference/configuration); para flags de comando, consulte a [referência da CLI](/pt-BR/products/bring-your-own-cloud/connector/reference/cli). Criar, escalar e excluir serviços não é configuração; consulte [serviços gerenciados](/pt-BR/products/bring-your-own-cloud/connector/managed-services).
...
<Note>
  A CLI pública `v0.16.0` não possui nem `--managed` nem `--no-managed`. Antes de usar qualquer uma dessas flags para alterar a configuração, confirme a [disponibilidade do modo gerenciado](/pt-BR/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) e obtenha a compilação correspondente com sua account team.
</Note>

O modo gerenciado executa o executor, que aplica ao seu cluster o ciclo de vida de serviço conduzido pelo ClickHouse Cloud. O `init` o ativa no momento da inscrição e preenche a identidade do cluster (consulte [habilitar o modo gerenciado](/pt-BR/products/bring-your-own-cloud/connector/managed-services#enable-managed-mode)); as chaves abaixo são as que você talvez precise revisitar depois. Para alterar a postura configurada, execute novamente o `init` com `--force` e `--managed` ou `--no-managed`. Como toda execução do `init`, esta precisa de um entry point: `--handoff`, caso você tenha mantido o pacote, ou `--enroll` com um token novo, conforme descrito em [reexecuções e recuperação](/pt-BR/products/bring-your-own-cloud/connector/operations#re-runs-and-recovery).

Em uma VM, o `--no-managed` desativa o executor na configuração, mas não interrompe nem desabilita um systemd unit já em execução. Interrompa-o explicitamente antes de alterar a configuração:

```bash
sudo systemctl disable --now clicklink-executor
```

No Kubernetes, definir `executor.enabled: false` nos values e aplicar a alteração com `helm upgrade` remove a Implantação do executor. Em ambos os destinos, o grant no cluster permanece até que você o remova; siga [interromper o modo gerenciado](/pt-BR/products/bring-your-own-cloud/connector/reference/faq#stop-managed-mode) para remover também esse acesso.
...
<Tabs>
  <Tab title="Kubernetes" id="managed-mode-kubernetes">
    ...
    * **`cluster.namespacePrefix`** é compartilhado por todos os componentes e é o prefixo ao qual o grant do executor o restringe. Alterá-lo após a instalação exige executar o grant novamente.
  ...
  <Tab title="VM Linux" id="managed-mode-vm">
    ...
    * **`namespace_prefix`** é uma chave de nível superior compartilhada por todos os componentes e é o prefixo ao qual o grant do executor o restringe. Alterá-la após a instalação exige executar o grant novamente.
...
Os serviços criados pelo executor se registram automaticamente, portanto, no modo gerenciado, você nunca os adiciona aqui. Uma implantação separada do connector, executando sem o executor, monitora o ClickHouse que você mesmo executa; passe `--ch-user-suffix` no `init` dela quando as duas implantações compartilharem uma instância. Esta seção se aplica a essa implantação. Cada entrada em `instances` nomeia um endpoint de protocolo nativo do ClickHouse do qual o connector lê: `host`, `port`, `database`, `secure`, `cluster` e, no Kubernetes, `namespace`; `max_open_conns` e `max_idle_conns` limitam o connection pool. As credenciais nunca ficam na configuração; cada componente obtém seu usuário read-only do ClickHouse a partir do pacote de acesso criado pelo provisionamento.

<Tabs>
  <Tab title="Kubernetes" id="instances-kubernetes">
    Adicione a instância ao map `instances` de nível superior em `clicklink-values.yaml` (os mesmos campos do registro da VM) e adicione o respectivo espaço de nomes a `networkPolicy.clickhouseNamespaces`, correspondente ao label `kubernetes.io/metadata.name` do espaço de nomes. O chart renderiza o map no ConfigMap `clicklink-instance-registry` que ambos os daemons leem. Isso funciona apenas enquanto `executor.enabled` for `false`; com o executor ativo, a renderização falha porque o executor é o proprietário desse ConfigMap.
...
Em uma VM, o arquivo é `/etc/clicklink/redaction-patterns.yaml`. O instalador o inicializa com um conjunto inicial de padrões ativos (API keys de terceiros, AWS secret keys e assim por diante). Reduza ou amplie o conjunto como preferir; o instalador preserva sua versão durante os upgrades. No Kubernetes, coloque o YAML em um ConfigMap com a chave `redaction-patterns.yaml` e defina `troubleshooter.redaction.patternsConfigMap` como o nome dele; o chart o monta no mesmo caminho.
...
O exemplo de modo gerenciado abaixo requer uma compilação com `--managed`, que a versão pública `v0.16.0` não possui. Confirme a [disponibilidade do modo gerenciado](/pt-BR/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) antes de usá-lo; espelhar o lançamento público não adiciona esse suporte.
...
<Tabs>
  ...
  <Tab title="VM Linux" id="storage-vm">
    O scraper acumula métricas em `/var/lib/clicklink/buffer` para garantir a entrega at-least-once enquanto o endpoint do seu conector estiver inacessível. Ele retém até 168 horas ou 1024 MB de dados e faz o upload a uma taxa limitada a 1 MB/s por padrão:
