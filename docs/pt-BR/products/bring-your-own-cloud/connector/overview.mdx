...
keywords: ['BYOC', 'cloud', 'bring your own cloud', 'connector', 'modo managed', 'executor']
doc_type: 'guide'
---
...
<Note>
  Os workflows de modo managed descritos aqui ainda não estão disponíveis no lançamento público do connector. Confirme a compilação e o suporte de implantação necessários com o seu account team da ClickHouse antes de instalar; consulte [disponibilidade do modo managed](/pt-BR/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability).
</Note>
...
Com uma [compilação que dá suporte ao modo managed](/pt-BR/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability), o `init` pergunta se ele deve ser habilitado quando executado em um terminal e já deixa &quot;sim&quot; pré-selecionado; um `init` executado via script só o habilita com `--managed`. Você também pode executá-lo sem esse modo e manter a observabilidade e o suporte para clusters que você mesmo opera; consulte [executar sem o modo managed](#run-without-managed-mode).
...
O fluxo de inscrição e criação de serviço descrito abaixo requer [suporte ao modo managed](/pt-BR/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) confirmado pelo seu account team da ClickHouse; ele está indisponível na versão pública `v0.16.0`.
...
A escolha de instalação descrita abaixo requer uma [compilação com suporte ao modo managed](/pt-BR/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability). A versão pública `v0.16.0` não possui a flag `--no-managed` nem o prompt de modo managed.
...
* **Registro pela ClickHouse.** A ClickHouse registra seu ambiente para o modo managed e fornece o endpoint do seu connector e um token de inscrição de uso único durante o onboarding.
...
* **Credenciais da AWS para `prepare` e `teardown`, e acesso ao registry para `platform approve`.** A criação dos buckets e da IAM role de um serviço, bem como a remoção deles após a exclusão do serviço, é executada com suas credenciais na sua workstation ou no host do connector. Para acesso direto ao registry da ClickHouse, a aprovação e a sincronização de plataforma usam o instance profile da VM EC2 do connector para assumir a função de extração read-only; perfis da AWS na workstation ou credenciais de SSO não substituem essa identidade. Outros registries de charts no ECR usam as credenciais da AWS do ambiente. Consulte [credenciais do registry](/pt-BR/products/bring-your-own-cloud/connector/platform-updates#registry-credentials). O executor nunca detém credenciais dos seus buckets ou do IAM. Suas únicas chamadas à nuvem vão para o Amazon ECR e o STS, para extrair charts e verificar imagens de plataforma; ele não faz chamadas ao S3 nem ao IAM.
