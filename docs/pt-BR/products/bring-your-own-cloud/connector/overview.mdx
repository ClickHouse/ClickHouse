...
O ClickHouse Connector é um componente que você implanta dentro do seu próprio ambiente. Ele permite que o ClickHouse Cloud gerencie, monitore e dê suporte a serviços ClickHouse em um cluster do Kubernetes operado por você. O ClickHouse Cloud nunca se conecta ao seu ambiente: o Connector estabelece apenas conexões de saída e atua sob identidades com escopo restrito aos serviços ClickHouse. O Connector é distribuído como um único binary, `clicklink`, que contém os daemons e a ferramenta `clicklink clctl` usada para instalá-los e operá-los.

O Connector tem três componentes:

* **O executor** conduz o ciclo de vida dos serviços cuja gestão você delega ao ClickHouse. Ele cria cada serviço a partir de uma definição que você envia e, em seguida, aplica os comandos de escala, parada, início, reinício, backup e exclusão provenientes do ClickHouse Cloud. Executar o executor é o que chamamos de modo gerenciado.
...
Em um terminal, o `init` pergunta se o modo gerenciado deve ser habilitado e já deixa &quot;sim&quot; pré-selecionado; um `init` executado via script só o habilita com `--managed`. Você também pode executá-lo sem esse modo e manter a observabilidade e o suporte para clusters que você mesmo opera; consulte [executar sem o modo gerenciado](#run-without-managed-mode).

Toda conexão feita pelo Connector é de saída. O ClickHouse Cloud nunca inicia uma conexão para o seu ambiente. A página de [arquitetura](/pt-BR/products/bring-your-own-cloud/connector/architecture) lista todas as conexões, seu protocolo e como cada uma se autentica.
...
* As gravações no seu cluster ficam restritas aos espaços de nomes de serviço, com exceção do token, do registry e do certificado renovado do próprio executor, em seu espaço de nomes de origem. Os componentes de plataforma dos quais esses serviços dependem só mudam após a sua aprovação de cada atualização.
...
* O acesso do Support tem prazo determinado, é habilitado por você e pode ser revogado por você a qualquer momento. Todo comando executado em sessões de suporte é registrado em um log de auditoria que pertence a você.

## Como funciona {#how-it-works}

A ClickHouse registra seu ambiente durante o onboarding e fornece o connector endpoint e um token de inscrição de uso único. Você executa `clicklink clctl init --enroll` uma única vez: a partir de uma workstation com acesso ao kubeconfig, no caso de uma instalação em Kubernetes, ou no próprio host, no caso de uma instalação em Linux VM. Esse único comando resgata o token, obtém um certificado de cliente mTLS, implanta e inicia os daemons e verifica a saúde deles.

O `init` habilita o modo gerenciado quando você passa `--managed`; em um terminal, ele pergunta, e o padrão é sim. Ao habilitá-lo, o executor é implantado e sua concessão de acesso ao Kubernetes é aplicada. A concessão de acesso é uma ServiceAccount `pcm-executor`. Uma política de admission restringe suas escritas aos espaços de nomes de serviço (prefixo `ns-` por padrão), com exceção do renewal do próprio token, de um ConfigMap de registry e, no Kubernetes, do certificado de cliente renovado em seu espaço de nomes de origem.

Criar um serviço exige dois comandos. O `clicklink clctl executor prepare` cria os buckets e a IAM role do serviço com suas próprias AWS credentials e exibe uma única vez a senha do default user. O `clicklink clctl instances create` envia a definição do serviço ao ClickHouse Cloud por meio do seu connector endpoint. A partir daí, o ClickHouse Cloud passa a conduzir o serviço pelo canal de comandos de saída do executor: scaling, parada e início, reinicializações, backups, upgrades de versão e alterações de configuração. Consulte [serviços gerenciados](/pt-BR/products/bring-your-own-cloud/connector/managed-services) para o passo a passo completo.
...
O scraper lê as tabelas de sistema da lista de permissões em um intervalo fixo e envia métricas, metadados e status de saúde ao seu connector endpoint por mTLS. O solucionador de problemas mantém aberto um canal de comandos de saída, mas esse canal não transporta nada até que você habilite uma sessão de suporte. O certificado mTLS é renovado automaticamente.

## Executar sem o modo gerenciado {#run-without-managed-mode}

Passe `--no-managed` para `clicklink clctl init`, ou responda &quot;não&quot; quando for perguntado, para instalar apenas o scraper e o solucionador de problemas. Nesse caso, o connector observa e dá suporte aos clusters ClickHouse que você mesmo opera. O ClickHouse não pode criar, alterar ou excluir serviços. Mesmo assim, o connector se implanta e provisiona usuários ClickHouse read-only e ServiceAccounts; consulte o [modelo de privilégios](/pt-BR/products/bring-your-own-cloud/connector/reference/privilege-model). Esse também é o formato usado para destinos não cobertos pelo modo gerenciado. O modo gerenciado é executado no Amazon EKS com armazenamento S3; já a observabilidade e o suporte funcionam em qualquer cluster do Kubernetes em conformidade ou host Linux.

## Requisitos em resumo {#requirements-at-a-glance}

* **Um destino de implantação.** O modo gerenciado exige um cluster do Amazon EKS com armazenamento S3. A observabilidade e o suporte, isoladamente, funcionam em qualquer cluster do Kubernetes em conformidade (instalado com o chart do Helm) ou em qualquer host Linux com systemd, amd64 ou arm64.
* **Registro pela ClickHouse.** A ClickHouse registra seu ambiente para o modo gerenciado e fornece o endpoint do seu connector e um token de inscrição de uso único durante o onboarding.
* **Acesso de rede de saída na porta 443** ao endpoint do seu connector e ao respectivo endpoint de inscrição, além de `releases.clicklink.clickhouse.com` e do ECR Public no momento da instalação. Há alternativas isoladas da internet e espelhadas para cada etapa; consulte [onboarding](/pt-BR/products/bring-your-own-cloud/connector/onboarding). No modo gerenciado, os nós do seu cluster também extraem imagens do ClickHouse do registry no qual seu ambiente está registrado, e o executor acessa o Amazon ECR e o STS ao extrair um chart de plataforma do ECR.
* **Acesso de administrador do cluster durante a configuração.** Habilitar o modo gerenciado aplica a concessão de acesso do executor uma única vez: um ServiceAccount, a função de cluster vinculada a ele e a política de admission que restringe suas escritas aos espaços de nomes de serviço. A aprovação de uma atualização de plataforma também é executada com acesso de administrador do cluster. Em ambos os destinos de instalação, os access bundles do solucionador de problemas são ancorados em ServiceAccounts do Kubernetes. Consulte [onboarding](/pt-BR/products/bring-your-own-cloud/connector/onboarding).
* **Credenciais da AWS para `prepare`, `teardown` e `platform approve`.** A criação dos buckets e da IAM role de um serviço, bem como a remoção deles após a exclusão do serviço, é executada com suas credenciais na sua workstation ou no host do connector. O `platform approve` também precisa de credenciais capazes de ler o Amazon ECR quando os charts preparados estiverem lá. O executor nunca detém credenciais dos seus buckets ou do IAM. Suas únicas chamadas à nuvem vão para o Amazon ECR e o STS. Ele faz login no registry quando uma sincronização de plataforma ou uma criação extrai um chart. Durante uma atualização de plataforma, ele verifica as imagens usando a função de extração read-only. Ele não faz chamadas ao S3 nem ao IAM.
* **Um listener nativo do ClickHouse acessível** para clusters que você mesmo opera, observados por uma implantação separada do connector instalada com `--no-managed`. O scraper e o solucionador de problemas se comunicam com cada cluster pelo protocolo nativo; no Kubernetes, detectam automaticamente se a conexão é segura (9440) ou em plaintext (9000).
...
* [Serviços gerenciados](/pt-BR/products/bring-your-own-cloud/connector/managed-services): habilite o modo gerenciado, crie um serviço e acompanhe seu ciclo de vida, status e exclusão.
