...
`--force` substitui a configuração ou sobreposição mantida, regenera a chave de cliente e substitui um certificado de cliente ainda válido. Ele nunca gera um novo UUID de cluster, de modo que a identidade do connector é preservada. A postura de modo gerenciado da configuração substituída é mantida em reexecuções não assistidas e oferecida como padrão em um terminal, a menos que você passe `--managed` ou `--no-managed`.
