...
Cada daemon disponibiliza um endpoint `/livez` em sua porta de saúde. O campo JSON `status` no corpo da resposta é o indicador de saúde, não o código de status HTTP; portanto, verifique o corpo em vez do `200`. As métricas do Prometheus são disponibilizadas em `/metrics` na mesma porta; não há uma porta de métricas separada. Portas padrão em ambos os destinos:
...
O comportamento do modo managed descrito aqui requer uma [compilação compatível](/pt-BR/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability); a versão pública `v0.16.0` não possui nem `--managed` nem `--no-managed`.

Solicite um novo token de inscrição à account team da ClickHouse e execute novamente o comando `init` original com `--enroll` e `--force`. Mantenha todas as flags específicas do destino usadas na primeira instalação (`--target-namespace`, `--values` e quaisquer flags de espelhamento, como `--chart`, `--chart-repo` ou `--chart-version`), pois `--force` prepara novamente a configuração preservada. O modo managed é herdado da configuração que está sendo substituída, portanto `--managed` é opcional aqui. Em uma instalação padrão:
...
No Kubernetes, o `init` cria previamente os Secrets rotacionados `clicklink-hmac` e `clicklink-mtls`; o chart não os renderiza. Por isso, a annotation de checksum do chart não os abrange, e os pods em execução continuam usando as credenciais com as quais foram iniciados. Reinicie todos os componentes após a rotação:
...
Em uma VM, execute-os no host fornecendo a senha do admin por pipe; em uma VM a senha é obrigatória, a menos que você passe `--ch-user-via cr`:
...
<Note>
  A CLI pública `v0.16.0` não possui `--managed` nem `--no-managed`. A preservação e a substituição do modo managed descritas abaixo exigem uma compilação com [suporte ao modo managed](/pt-BR/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability); confirme sua compilação com sua account team antes de alterar esse modo durante a recuperação.
</Note>
...
`--force` substitui a configuração ou sobreposição mantida, regenera a chave de cliente e substitui um certificado de cliente ainda válido. Ele nunca gera um novo UUID de cluster, de modo que a identidade do connector é preservada. A postura do modo managed da configuração substituída é mantida em reexecuções não assistidas e oferecida como padrão em um terminal, a menos que você passe `--managed` ou `--no-managed`.
...
**Connector offline.** Enquanto o connector está fora do ar ou inacessível, os serviços que ele gerencia continuam em execução. O ClickHouse operator no seu cluster os mantém ativos, e o executor não está no data path. Mudanças de ciclo de vida exigem o executor conectado, portanto nenhuma é aplicada até que ele se reconecte. Ao reiniciar, o executor recupera do seu banco de dados de comandos aqueles que estavam em andamento quando ele parou. Um comando que o ClickHouse Cloud entrega novamente é respondido a partir do resultado registrado, em vez de ser executado duas vezes; somente um comando que falhou é executado de novo. Para saber o que acontece com os comandos de ciclo de vida enquanto nenhum executor está conectado, consulte [quando o connector está offline](/pt-BR/products/bring-your-own-cloud/connector/managed-services#when-the-connector-is-offline).
...
O PersistentVolumeClaim do executor tem a anotação `helm.sh/resource-policy: keep`, portanto a desinstalação o preserva, junto com o banco de dados de comandos e o service registry nele contidos. Os Secrets criados pelo `init`, o Secret do platform bundle gravado pela aprovação da plataforma e o ConfigMap do service registry mantido pelo executor também não pertencem ao chart e sobrevivem à desinstalação. Exclua-os explicitamente, incluindo o par de Secret de acesso e ServiceAccount de cada instance que você mesmo registrou e de cada serviço gerenciado para o qual você provisionou o solucionador de problemas (nomeado a partir do serviço). O `--ignore-not-found` cobre objetos que nunca chegaram a ser criados, como o Secret do platform bundle antes da primeira aprovação:
...
A CLI pública `v0.16.0` não possui o comando `platform`, incluindo `platform reset`; confirme a [disponibilidade](/pt-BR/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) antes de usá-lo.

Desinstalar o connector não remove a camada de plataforma. Os espaços de nomes e as cargas de trabalho da plataforma, o RBAC `pcm-platform` e os objetos de admission instalados pelas suas aprovações permanecem no cluster, assim como todos os serviços gerenciados. Exclua primeiro os serviços; consulte [excluir um serviço](/pt-BR/products/bring-your-own-cloud/connector/managed-services#delete-a-service). Em um cluster de teste sem serviços, o comando `clicklink clctl platform reset` desinstala os lançamentos da plataforma, mas as CustomResourceDefinitions, o RBAC e a StorageClass que ele deixa para trás precisam ser excluídos manualmente; consulte [redefinir um cluster de teste](/pt-BR/products/bring-your-own-cloud/connector/platform-updates#resetting-a-test-cluster). Fora de um cluster de teste, defina um plano de remoção junto à sua account team da ClickHouse antes de mexer na camada de plataforma.
