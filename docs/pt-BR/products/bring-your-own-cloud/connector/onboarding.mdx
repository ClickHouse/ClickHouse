...
* **Seu connector endpoint e enrollment token**, fornecidos durante o [registro](#registration).
* **Egress na porta 443** para `https://<subdomain>.<connector-domain>` e `https://<subdomain>.enroll.<connector-domain>`, além de `releases.clicklink.clickhouse.com` e Amazon ECR Public no momento da instalação. No modo gerenciado, o executor também acessa o Amazon ECR e o STS quando uma sincronização ou criação de plataforma baixa um chart do ECR. Caso algum desses destinos esteja inacessível, consulte [instalações isoladas da internet e com espelhamento](#air-gapped-and-mirrored-installs).
* **Um EKS cluster da Amazon** para o modo gerenciado. O `init` recusa o modo gerenciado em qualquer outro cluster e indica `--no-managed` como a forma de instalar sem o executor.
* **Permissão de cluster admin para o grant do executor.** O grant cria uma ClusterRole, um ClusterRoleBinding e uma ValidatingAdmissionPolicy que restringe o executor aos espaços de nomes sob o seu prefix. No Kubernetes, o kubeconfig com o qual você executa o `init` precisa dessa permissão. Em uma VM, quem precisa dela é o kubeconfig do host; quando o host não tem permissões de admin, o `init` imprime o comando a ser executado a partir de um host que as tenha.
* **Nenhum ClickHouse ainda.** Uma instalação gerenciada começa sem serviços; os que o executor cria se registram sozinhos. O `init` em modo gerenciado não registra nenhum ClickHouse executado por você (no Kubernetes, ele recusa `--instance` e `--instance-namespace`). Para observar um cluster desse tipo, instale uma implantação de connector separada com `--no-managed`; o `--ch-user-suffix` permite que as duas implantações compartilhem uma instance sem conflitos entre os usuários do connector. Essa implantação precisa de um listener native acessível: 9440 seguro ou 9000 em plaintext, detectado automaticamente no Kubernetes. Ela também precisa de acesso de ClickHouse admin para o provisioning: um usuário `default` sem password, uma password informada com `--ch-admin-password-stdin` ou no prompt, ou injection de CR para instances gerenciadas por operator.
...
<Steps>
  ...
  <Step title="Verifique se foi bem-sucedido" id="verify-success">
...
Se o modo gerenciado for recusado porque o cluster não é EKS, execute novamente com `--no-managed` para instalar sem o executor. Em uma VM, um kubeconfig cujo contexto atual aponta para outro lugar também é recusado: troque de contexto com `kubectl config use-context` e execute novamente.
