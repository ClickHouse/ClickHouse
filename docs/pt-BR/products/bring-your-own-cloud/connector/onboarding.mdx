...
description: 'Instale o ClickHouse Connector em modo gerenciado e configure-o no Kubernetes ou em uma VM Linux'
...
Esta página orienta você desde o enrollment token até um connector verificado em modo gerenciado, no qual o executor aplica o ciclo de vida do serviço conduzido pelo ClickHouse Cloud. O connector é instalado em um cluster do Kubernetes (Helm) ou em uma VM Linux (systemd). A inscrição por token é o fluxo padrão; quando o ambiente não puder acessar diretamente o endpoint do seu connector, consulte [instalações isoladas da internet e com espelhamento](#air-gapped-and-mirrored-installs). Para observar um ClickHouse que você mesmo executa, sem o executor, use `--no-managed`.

## Disponibilidade do modo gerenciado {#managed-mode-availability}

<Warning>
  Em 22/09/2026, o instalador público disponibiliza a `v0.16.0`, que não possui as flags `init --managed` e `init --no-managed` nem o comando `platform`. A aprovação de plataforma em instalações tanto em Linux VM quanto em Kubernetes, bem como a criação de serviço do cliente (`instances create --from-prepare`), ainda não estão disponíveis em um lançamento público. A aprovação de plataforma no Kubernetes exige, além disso, suporte na CLI e no chart para `platform approve --secret-namespace`.

  Antes de seguir os workflows do modo gerenciado, solicite ao account team da ClickHouse que confirme o suporte ao seu target de implantação e forneça a CLI, o chart ou image do connector e as instruções de instalação correspondentes. A criação de serviço do cliente também exige suporte no seu connector endpoint e no ClickHouse Cloud; apenas uma CLI mais recente não é suficiente. Os comandos de instalação públicos abaixo não instalam, atualmente, um lançamento compatível com esses workflows.
</Warning>
...
Antes de realizar a inscrição, sua account team registra seu ambiente e habilita o modo gerenciado para sua organização. Forneça a ela:
...
* como seu cluster obtém as images do ClickHouse: pelo registry access que o ClickHouse Cloud configura para sua account ou pelo seu próprio mirror

Em seguida, o ClickHouse Cloud emite seu connector endpoint e um enrollment token de uso único. O token identifica seu ambiente, portanto você não precisa digitar nenhum ID adicional na inscrição.

### Para toda instalação {#prerequisites-every-install}

* **Uma compilação com [suporte a modo managed](#managed-mode-availability)** para as instruções de modo managed abaixo. A versão pública `v0.16.0` não tem `--managed` nem `--no-managed`.
...
* **Um EKS cluster da Amazon** para o modo managed. O `init` recusa o modo managed em qualquer outro cluster e indica `--no-managed` como a forma de instalar sem o executor.
...
* **AWS credentials** na workstation (ambiente ou perfil) com `eks:DescribeCluster` e `ec2:DescribeVpcs`. O `init` as utiliza para ler os blocos CIDR da VPC do seu cluster para a NetworkPolicy. Quando o lookup falha, ele emite um aviso e deixa essa lista para você preencher.
...
* **Acesso de Admin ao Kubernetes API server** para o provisioning e para o grant do executor: um kubeconfig no host, `--server` e `--ca-data`, ou os prompts. Os access bundles são ancorados em ServiceAccounts do Kubernetes em ambos os targets. No modo gerenciado, o contexto atual do kubeconfig deve apontar para o EKS cluster que você gerencia; o `init` se recusa a conceder o grant por meio de qualquer outro contexto.

<Note>
  Em uma VM, `--skip-provision` é a única forma de contornar a exigência do Kubernetes API server, e vale apenas para stage. Ele ignora o provisioning do ClickHouse user, o grant do executor, a habilitação da unit e a verificação, ou seja, por si só não resulta em um connector em execução. No Kubernetes, ele ignora apenas o provisioning do ClickHouse user; o grant do executor continua sendo aplicado, porque o executor do chart não consegue ser agendado sem a ServiceAccount com o grant concedido.
</Note>

## Instalação e inscrição {#install-and-enroll}

Primeiro, confirme a [disponibilidade do modo gerenciado](#managed-mode-availability) com a equipe responsável pela sua conta. O instalador público abaixo distribui a versão `v0.16.0`, que não executa os comandos de inscrição em modo gerenciado apresentados neste tutorial.
...
<Steps>
  <Step title="Obtenha o endpoint do conector e o token de inscrição" id="get-endpoint-and-token">
    ...
    O token é de uso único e expira rapidamente, portanto faça a inscrição logo após recebê-lo. Trate-o como um segredo: a CLI o lê a partir de um prompt oculto ou da primeira linha do stdin, nunca de argumentos de linha de comando, do disco ou de logs. Caso ele expire antes, solicite um novo à sua account team da ClickHouse.
  </Step>

  <Step title="Instale e verifique a CLI" id="install-and-verify-the-cli">
    Um único comando instala um binary `clicklink` verificado. Ele detecta sua plataforma (macOS ou Linux, amd64 ou arm64), baixa o lançamento atual e verifica o checksum SHA-256 e, caso o cosign esteja instalado, a assinatura do lançamento. O binary é instalado em `/usr/local/bin` quando esse diretório é gravável; caso contrário, em `~/.local/bin` (adicione-o ao seu `PATH`, se necessário; `--bin-dir` sobrescreve esse comportamento). Para uma instalação em Kubernetes, execute-o em qualquer workstation com acesso ao cluster via kubeconfig:

    ```bash
    curl -fsSL https://releases.clicklink.clickhouse.com/install.sh | bash
    ```

    Para uma instalação em VM, execute o mesmo script no host com `--host`. Após o download verificado, ele também cria o usuário de sistema `clicklink`, os diretórios `/etc/clicklink`, `/var/lib/clicklink` e `/var/log/clicklink`, e as systemd units dos três componentes. Ele inicializa o `/etc/clicklink/redaction-patterns.yaml` com um conjunto inicial de patterns (preservado caso já exista), de modo que a próxima etapa comece na inscrição:
  ...
  <Step title="Registre e instale o conector" id="enroll-and-install">
    Esses comandos exigem o suporte de compilação e implantação descrito em [disponibilidade do modo gerenciado](#managed-mode-availability). A versão pública `v0.16.0` não possui `--managed` nem `--no-managed`.

    A inscrição é feita com um único comando. Ele resgata seu token, obtém um certificate de cliente assinado e o acesso cluster-wide do executor e, em seguida, instala e verifica o connector.
  ...
  <Step title="Limpar" id="clean-up">
    O pacote de inscrição `handoff.yaml` (modo `0600`, no diretório de trabalho) existe para que reexecuções e a recuperação durante a instalação nunca precisem de um segundo token. Ele armazena o secret de API do connector em plaintext, portanto exclua-o assim que a instalação for verificada:
    ...
    O connector em execução mantém sua própria cópia das credenciais, portanto nada operacional depende desse arquivo. Upgrades e alterações de configuração nunca precisam dele. Se mais tarde você precisar do `init` novamente, solicite um novo enrollment token à account team da ClickHouse e execute `init --enroll --force`.
  </Step>
</Steps>
...
Se o `init` rejeitar `--managed` ou `--no-managed` como uma flag desconhecida, verifique a versão da sua CLI: a versão pública `v0.16.0` não suporta nenhuma das duas. Obtenha uma [compilação com suporte a modo managed](#managed-mode-availability) com o seu account team antes de seguir as instruções de recuperação de modo managed abaixo.

Execute novamente o mesmo comando. O `init` é idempotente: novas execuções convergem para o mesmo estado, preservam sua configuração existente e os arquivos preparados e ignoram o trabalho já concluído. Quando um passo falha no meio do processo, a CLI imprime os comandos de recuperação exatos para a sua situação; eles podem ser repetidos com segurança.

Se a inscrição for recusada, é porque o token já foi utilizado, ou então é inválido ou expirou. Para um token já utilizado, execute novamente com `--handoff handoff.yaml`, que existe até o passo de limpeza; caso contrário, solicite um novo token ao seu account team da ClickHouse. Se a inscrição falhar com um erro de transporte, execute novamente o mesmo comando. Se a nova execução for recusada, a primeira tentativa consumiu o token, então solicite um novo.
...
`--force` é a redefinição explícita, não uma retentativa de rotina. Ele sobrescreve a config ou o values overlay mantidos, regenera a client key e substitui um certificado de cliente não expirado (um `409` do endpoint de assinatura significa que já existe um). O UUID do cluster do connector é preservado entre execuções com `--force`, assim como sua postura de modo managed. Reexecuções não assistidas mantêm a postura e, em um terminal, o prompt assume esse valor por padrão, a menos que você passe `--managed` ou `--no-managed`. Use `--force` ao rotacionar credenciais ou substituir um certificado, e consulte [operações](/pt-BR/products/bring-your-own-cloud/connector/operations) para o modelo completo de reexecução e recuperação.