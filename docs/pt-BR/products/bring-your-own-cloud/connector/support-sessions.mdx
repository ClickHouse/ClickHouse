...
As sessões de suporte permitem conceder ao ClickHouse acesso temporário para fins de diagnóstico por meio do ClickHouse Connector. Esta página aborda o que é uma sessão, como habilitá-la e desabilitá-la, o que os engenheiros de suporte do ClickHouse podem fazer durante ela e como auditá-la.

## O que é uma sessão de suporte {#what-a-support-session-is}

Uma sessão de suporte é uma janela de tempo limitado durante a qual o solucionador de problemas aceita comandos dos engenheiros de suporte do ClickHouse. Fora de uma sessão, o solucionador de problemas recusa todos os comandos, mesmo com seu WebSocket de saída conectado. Não existe outro caminho de execução: o solucionador de problemas não executa nada sem uma sessão, e o ClickHouse não pode abri-la por você. O ClickHouse Cloud nunca se conecta ao seu ambiente; ele recebe apenas o que o solucionador de problemas envia pelo canal de saída.
...
* **O gateway de sessão**, uma API autenticada embutida no solucionador de problemas. Ele atende `POST /v1/clctl/session/enable`, `POST /v1/clctl/session/disable` e `GET /v1/clctl/session/status`. Toda chamada exige um OIDC ID token de curta duração cujo e-mail esteja na sua lista de permissões de operadores.
* **O arquivo de sessão local**, em instalações em Linux VM, gravado no host com acesso root.

O transporte do gateway depende do target. Um gateway em VM serve TLS autossinado, cuja impressão digital cada operator fixa (pinning). Um gateway no Kubernetes escuta localmente no pod do Kubernetes via HTTP; você o acessa com `kubectl port-forward` (o túnel trafega sobre o TLS do API server) ou por meio de uma Entrada que terminates TLS com um certificate emitido por uma CA.

Você define a postura das suas sessões, incluindo a lista de permissões de operadores, durante o `clicklink clctl init`.
...
As sessões expiram automaticamente. A duração padrão é de 4 horas; `session enable --duration` aceita até 24 horas. Quando a sessão expira ou assim que você executa `session disable`, o solucionador de problemas deixa de aceitar comandos. Desabilitar é a forma de revogação imediata: não é necessário reiniciar nem coordenar com o ClickHouse.

## Lista de permissões de operadores {#operator-allowlist}

Cada chamada ao gateway é autorizada com base em uma lista de permissões de e-mails de operadores. A comparação é feita com o e-mail atestado pelo token OIDC validado, e nunca com informações que o cliente alegue sobre si mesmo.

* **Kubernetes:** defina `clctl.gateway.allowedOperators` na sua sobreposição de values. O chart renderiza a lista em um ConfigMap; portanto, uma alteração nos values seguida de `helm upgrade` atualiza ciclicamente a lista de permissões sem reiniciar o pod do Kubernetes.
...
Em ambos os targets, o gateway relê o arquivo na próxima requisição assim que 30 segundos tiverem se passado desde a última leitura, de modo que as edições entram em vigor sem reinicialização.
