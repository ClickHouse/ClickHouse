...
<Steps>
  ...
  <Step title="Execute approve" id="run-approve">
    ...
    `approve` precisa de um kube context com direitos de cluster-admin no cluster gerenciado (`--context <name>` quando seu kubeconfig contém vários). Quando um chart reside no Amazon ECR, também são necessárias AWS credentials com permissão de leitura sobre ele. `--dry-run` renderiza e lista a metade de permissões sem aplicar nem emitir nada.
    ...
    <Tabs>
      ...
      <Tab title="Kubernetes" id="approve-kubernetes">
        `approve` lê o pacote preparado por meio do redirecionamento de porta do passo anterior. Ele entrega o pacote de token como um Secret no espaço de nomes do connector, que o chart monta no pod do Kubernetes. Execute-o a partir de uma workstation cujo kubeconfig alcance o cluster, com uma cópia da configuração do connector. O chart renderiza uma no ConfigMap do executor; `approve` precisa dela para o prefix do espaço de nomes e não lê nenhuma credential dela.
