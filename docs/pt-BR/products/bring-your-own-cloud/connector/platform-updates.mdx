...
Um cluster gerenciado executa uma pequena camada de plataforma da qual os serviços ClickHouse dependem. O ClickHouse Cloud propõe atualizações para essa camada; o executor não aplica nada até que você aprove cada uma delas com suas próprias credenciais do cluster. Esta página explica o que é uma atualização de plataforma, como aprová-la em uma VM ou em uma instalação Kubernetes, por quanto tempo uma aprovação é válida e quais permissões ela concede.

<Note>
  A CLI pública `v0.16.0` não possui o comando `platform`, incluindo `platform approve` e `platform reset`. Nem o fluxo de trabalho da VM Linux nem o do Kubernetes descritos abaixo estão disponíveis nesse lançamento. A aprovação no Kubernetes exige, adicionalmente, suporte na CLI e no chart do executor para o pacote de tokens baseado em Secret (`--secret-namespace`). Confirme a [disponibilidade do modo gerenciado](/pt-BR/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) com sua equipe de conta antes de usar qualquer um dos fluxos de trabalho.
</Note>

## O que é uma atualização de plataforma {#what-a-platform-update-is}

A camada de plataforma é composta por três componentes: o controlador de snapshots, o ClickHouse Operator e os coletores de monitoramento. Eles são instalados nessa ordem porque cada um depende das definições de recurso personalizado do anterior. Junto a eles há uma StorageClass chamada `gp3-encrypted` (volumes gp3 criptografados) utilizada pelos serviços.

Um pacote de plataforma é o manifesto renderizado que o ClickHouse Cloud gera para o seu ambiente. Ele fixa as versões de chart e de imagem desses componentes e indica o registry do qual são obtidas. Todo pacote é identificado pelo sha256 desse manifesto. O ClickHouse Cloud envia o pacote ao executor por meio de seu canal de comandos, e o executor o prepara para a sua aprovação.

A aplicação de um pacote é dividida em duas metades:
...
Os dois procedimentos de aprovação abaixo exigem uma [compilação com suporte a plataforma](/pt-BR/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability); a versão pública `v0.16.0` não possui o comando `platform`. A aprovação no Kubernetes também exige suporte correspondente na CLI e no chart para a entrega de Secrets.

A autenticação no registry depende do modo de distribuição de imagens registrado para o seu ambiente:

* **Acesso direto ao registry da ClickHouse.** Execute a aprovação na VM EC2 do connector. Tanto o `approve` quanto a sincronização de plataforma do executor usam credenciais do instance metadata service do EC2 para assumir a função read-only de puller do ECR do seu ambiente. Essa função é usada para o login no chart e, no caso do executor, também para verificar se as imagens de plataforma existem. Perfis da AWS, credenciais de ambiente ou credenciais de SSO em uma workstation não substituem o instance profile. O seu kubeconfig continua fornecendo as credenciais separadas de cluster-admin para aplicar a metade de permissões.
...
Use o procedimento de workstation do Kubernetes abaixo somente em deployments com registry espelhado e com a aprovação de plataforma do Kubernetes habilitada. Para acesso direto, peça à sua equipe de conta que confirme um ambiente de execução compatível com o instance profile do EC2 necessário antes de prosseguir.

<Steps>
  <Step title="Localize o pacote preparado" id="find-the-staged-bundle">
    O ClickHouse Cloud envia cada pacote proposto ao executor antes de qualquer outra coisa. O executor o registra como o pacote pendente e informa seu sha256 no heartbeat. Existe apenas um pacote em espera por vez; uma proposta mais recente o substitui. Em seguida, o executor recusa a sincronização e a registra como um comando com falha, cujo resultado termina com o comando a ser executado.

    ```bash
    clicklink clctl commands list --status failed --action sync_platform
    ```

    O campo `result` termina com `run: clctl platform approve (pending bundle sha <sha256>)`. No Kubernetes, a indicação é `run: clctl platform approve --secret-namespace <connector-namespace> (pending bundle sha <sha256>)`. Um create que esbarrou em uma definição de custom resource ausente traz a mesma linha `clctl platform approve` no próprio comando que falhou e no erro exibido por `clicklink clctl instances create --wait`. A equipe de conta da ClickHouse também avisa você quando uma atualização de plataforma é proposta.
  ...
  <Step title="Execute approve" id="run-approve">
    `approve` sem flags lê o pacote preparado e calcula seu hash, de modo que o que você aprova é exatamente o que o executor recebeu. Ele renderiza cada chart do pacote exatamente como o executor fará e aplica a metade de permissões com o kube context que você escolher. Cria a identidade `pcm-platform`, se necessário, e emite seu token. Não faz nenhuma requisição ao seu connector endpoint.
...
Você aplica a metade de permissões, portanto ela carrega a sua autoridade. O `approve` a carimba com o sha256 do pacote, e o executor verifica, antes de tocar em qualquer coisa, se todos os objetos de permissão do pacote que lhe foi enviado estão presentes e aprovados. O executor não aplica nada dela.
...
A CLI pública `v0.16.0` não possui o comando `platform reset`; confirme a [disponibilidade](/pt-BR/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) antes de usar este procedimento.
...
Em um cluster vazio, ele desinstala os lançamentos da plataforma na ordem inversa de dependência. Em seguida, remove o pacote de tokens de plataforma do executor: o diretório local em uma VM ou o Secret indicado por `--secret-namespace <connector-namespace>` no Kubernetes. As definições de recursos personalizados, o RBAC e a StorageClass permanecem intactos. Execute `approve` novamente antes da próxima sincronização da plataforma.
