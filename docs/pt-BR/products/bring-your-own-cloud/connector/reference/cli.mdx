...
Autoriza uma alteração de plataforma com as suas próprias credenciais de cluster. Lê o pacote preparado pelo executor, calcula seu hash e renderiza cada chart contido nele exatamente como o executor fará. Aplica a metade de permissões usando o contexto kube que você escolher, garante a identidade `pcm-platform` do executor e emite seu token. Não faz nenhuma requisição ao seu connector endpoint. Quando um chart do pacote reside no Amazon ECR, são necessárias AWS credentials com permissão de leitura sobre ele. A metade de permissões é composta por Namespaces, CustomResourceDefinitions, ServiceAccounts, Funções de cluster e bindings, Roles e bindings, configurações de webhook e admission, PriorityClasses e a StorageClass. O executor então aplica apenas a metade de workload, e somente enquanto esse token for válido. Consulte [atualizações de plataforma](/pt-BR/products/bring-your-own-cloud/connector/platform-updates).
