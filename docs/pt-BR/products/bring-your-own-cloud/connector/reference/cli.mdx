...
| `--managed`                 | Habilita o modo gerenciado: executa o executor para que o ClickHouse Cloud gerencie o ciclo de vida dos serviços ClickHouse neste cluster; dispensa o prompt. Requer um cluster Amazon EKS. Indisponível na `v0.16.0` pública; consulte [disponibilidade](/pt-BR/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability).                                     |
| `--no-managed`              | Mantém o modo gerenciado (o executor) desativado; dispensa o prompt. Indisponível na `v0.16.0` pública; consulte [disponibilidade](/pt-BR/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability).                                                                                                                                                            |
...
O fluxo de trabalho `init --managed` abaixo requer uma [compilação com suporte a modo gerenciado](/pt-BR/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability); a versão pública `v0.16.0` não expõe essa flag.

`init --managed` aplica a concessão cluster-wide do executor com suas credenciais. Quando isso não é possível, o `init` exibe os comandos que devem ser executados a partir de um kubeconfig com admin do cluster: apenas `access apply` em uma VM (o pacote já está preparado), ou `access grant` seguido de `access apply` no Kubernetes. Consulte [onboarding](/pt-BR/products/bring-your-own-cloud/connector/onboarding#install-and-enroll).
...
Aplica o pacote `grant` renderizado para todo o cluster: o Espaço de nomes de origem da ServiceAccount, a ServiceAccount, a Função de cluster e o ClusterRoleBinding, além da política de admission. Em seguida, gera o token da ServiceAccount no pacote. Não registra nenhum serviço.

| Flag                     | Descrição                                                                                                  |
| ------------------------ | ---------------------------------------------------------------------------------------------------------- |
| `--cluster`              | Aplica o pacote para todo o cluster a partir de `<output-dir>/_cluster/` (obrigatório para este uso).      |
| `--config <path>`        | Arquivo de configuração do connector (variável de ambiente `CLCTL_EXECUTOR_CONFIG`).                       |
| `--context <name>`       | Context do kubeconfig do cluster gerenciado, assim como em `prepare`.                                      |
| `--output-dir <path>`    | Raiz de saída (padrão `/etc/clicklink/access/executor`; variável de ambiente `CLCTL_EXECUTOR_OUTPUT_DIR`). |
...
Prepara tudo do seu lado que a criação de um managed service exige, em uma única execução idempotente que para na primeira falha. Ele escolhe ou valida o nome do serviço, cria os buckets de dados e de backup e a IAM role (ou verifica uma role fornecida por você) e, em uma VM, renderiza e aplica o pacote de acesso do executor para o espaço de nomes do serviço. No Kubernetes, ele pula o pacote: o executor é executado com a ServiceAccount do seu pod. Em seguida, gera a senha do usuário `default` (exibida uma única vez) e grava o corpo de criação para [`instances create`](#instances-create). Consulte [managed services](/pt-BR/products/bring-your-own-cloud/connector/managed-services#create-a-service).

Em uma VM, ele é executado no host do connector. Em uma instalação no Kubernetes, ele é executado em uma workstation com AWS credentials e um contexto kube do cluster gerenciado. Essa workstation também precisa de uma cópia da configuração do connector (`--config`), de um redirecionamento de porta para a API local do executor e de um `--output-dir` gravável.
...
Remove o que o `prepare` criou para um serviço que o ClickHouse Cloud excluiu. Ou seja, o IAM role, os buckets criados pelo `prepare`, o nome registrado, o registro do serviço no executor e, em uma VM, a Função de cluster e o ClusterRoleBinding do serviço, o pacote de acesso local e a entrada no registry. O executor deve estar em execução e o serviço deve estar `terminated` (ou `stale`); a execução é recusada enquanto o espaço de nomes do serviço ainda contiver um cluster ClickHouse. Os dados e os backups são preservados, a menos que sua exclusão seja solicitada, e os buckets preservados recebem a nova etiqueta `clicklink:retained-from=<instance>`. O comando lê tudo antes de excluir qualquer coisa. Consulte [excluir um serviço](/pt-BR/products/bring-your-own-cloud/connector/managed-services#delete-a-service).
...
Este workflow de criação para o cliente ainda não está disponível no lançamento público e requer suporte na CLI, no connector endpoint e no ClickHouse Cloud. Confirme a [disponibilidade do modo gerenciado](/pt-BR/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) antes de utilizá-lo.

Envia uma criação preparada ao ClickHouse Cloud pelo endpoint do connector, usando as credenciais do próprio connector. Exibe o nome falado atribuído pelo ClickHouse Cloud, o state e uma dica `watch:` que usa o nome do seu serviço. O corpo da criação vem do arquivo gravado pelo `prepare` ou da combinação das quatro flags `--deployed-name`, `--data-bucket`, `--backup-bucket` e `--irsa-role-arn`. Um retry com as mesmas entradas retorna a primeira criação.
...
Os três subcomandos abaixo enviam comandos diretamente à API local do executor, sem envolvimento do ClickHouse Cloud. Execute-os somente quando a account team da ClickHouse solicitar. Cada um exige `--name <name>` e o cluster (`--cluster`, `CLCTL_CLUSTER` ou `~/.clicklink/clctl.yaml`), e cada um exibe o comando em JSON. Passe o `id` dele para [`commands get`](#commands-get) para acompanhá-lo; o `result` de um comando que falhou contém o erro correspondente.
...
Altera um serviço no próprio lugar. `--create-nlb` é o único patch disponível. Ele provisiona um NLB interno, na forma de um `Service` do Kubernetes, para que um scraper fora do cluster consiga alcançar os servidores — apenas na AWS. O executor preenche o host do scraper a partir do balanceador de carga na sincronização seguinte. Sem `--create-nlb`, o comando falha com `nothing to patch`.
...
A CLI pública `v0.16.0` não possui o comando `platform`, portanto nem o workflow de aprovação em VM nem o em Kubernetes estão disponíveis nesse lançamento. A aprovação em Kubernetes requer, adicionalmente, o suporte da CLI e do chart à entrega via Secret descrita abaixo. Confirme a [disponibilidade do modo gerenciado](/pt-BR/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) antes de usar qualquer um dos workflows.

Autoriza uma alteração de plataforma com as suas próprias credenciais de cluster. Lê o pacote preparado pelo executor, calcula seu hash e renderiza cada chart contido nele exatamente como o executor fará. Aplica a metade de permissões usando o contexto kube que você escolher, garante a identidade `pcm-platform` do executor e emite seu token. Não faz nenhuma requisição ao seu connector endpoint. A metade de permissões é composta por Namespaces, CustomResourceDefinitions, ServiceAccounts, Funções de cluster e bindings, Roles e bindings, configurações de webhook e admission, PriorityClasses e a StorageClass. O executor então aplica apenas a metade de workload, e somente enquanto esse token for válido. Consulte [atualizações de plataforma](/pt-BR/products/bring-your-own-cloud/connector/platform-updates).
...
A CLI pública `v0.16.0` não possui o comando `platform`, incluindo `platform reset`; consulte [disponibilidade do modo gerenciado](/pt-BR/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability).
...
| Flag                                                                                        | Descrição                                                                                                                                                                                                                                                                                                                                                                    |
| ------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--provider <name>`                                                                         | Canal de execução: `ssh`, `aws` (SSM) ou `gcp` (IAP) para VMs remotas, ou `local` quando executado na própria VM de destino. É inferido pelas flags específicas de cada provedor quando não definido explicitamente. Assume `aws` como padrão quando nenhuma flag de canal é definida, e `local` nunca é inferido, portanto, na própria VM, sempre passe `--provider local`. |
