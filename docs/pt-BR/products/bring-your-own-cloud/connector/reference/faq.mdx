...
Os serviços que o ClickHouse criou continuam em execução, mas o ClickHouse Cloud não pode mais escalá-los, fazer upgrade, backup ou excluí-los, e os comandos pendentes para eles falham. As atualizações da plataforma nunca são aplicadas a menos que você execute `clicklink clctl platform approve`. Não executar esse comando congela a camada da plataforma, enquanto o restante do modo gerenciado continua funcionando.
