...
Os fluxos de inscrição no modo managed, de criação pelo cliente e de aprovação de plataforma exigem [suporte confirmado pela sua account team](/pt-BR/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability); eles não estão disponíveis na versão pública `v0.16.0`.

No modo managed, o ClickHouse Cloud altera apenas os serviços que você pediu para ele gerenciar. Qualquer pessoa com acesso ao host ou ao pod do executor também pode controlá-los com comandos locais `clicklink clctl instances`; consulte [ciclo de vida do serviço](/pt-BR/products/bring-your-own-cloud/connector/managed-services#service-lifecycle). O ClickHouse Cloud envia comandos de ciclo de vida pelo canal de saída do executor:
...
Duas coisas dependem de você. Criar um serviço exige seus comandos `clicklink clctl executor prepare` e `clicklink clctl instances create`, porque somente você pode criar os buckets e a IAM role. As atualizações dos componentes de plataforma no seu cluster também dependem de você: o executor prepara cada pacote proposto, e o seu `clicklink clctl platform approve` aprova o pacote preparado e emite um token de 2 horas para ele; consulte [atualizações de plataforma](/pt-BR/products/bring-your-own-cloud/connector/platform-updates).

Tudo o que o executor faz acontece por meio da Kubernetes API. Suas gravações são restritas por uma política de admission aos espaços de nomes com o prefixo de serviço (`ns-` por padrão), além de três no seu próprio espaço de nomes: a renovação do seu token, o ConfigMap do registry de serviços e, no Kubernetes, o certificado de cliente renovado no Secret `clicklink-mtls`. Já suas leituras das mesmas famílias de resources abrangem todo o cluster. As atualizações de plataforma aprovadas são executadas com uma identidade separada, `pcm-platform`, restrita aos espaços de nomes da plataforma. Clusters que você mesmo opera nunca são alterados: o scraper e o solucionador de problemas são read-only. Os escopos exatos estão na página [modelo de privilégios](/pt-BR/products/bring-your-own-cloud/connector/reference/privilege-model).
...
O scraper envia resultados de um conjunto fixo de tabelas de sistema: por padrão, `metric_log`, `asynchronous_metric_log`, `tables`, `warnings` e `server_settings`. Ele também envia heartbeats de integridade e status, o estado das instâncias e dos backups e as métricas do próprio connector. O conjunto padrão de scrape não inclui `system.query_log`, portanto o texto SQL bruto, bem como quaisquer literais ou dados pessoais contidos nele, nunca sai pelo caminho de scrape, a menos que você o adicione. Durante uma sessão, a allowlist padrão de tabelas inclui `system.processes`, que exibe o texto das consultas em execução; remova-a da allowlist caso esses dados precisem permanecer ocultos.

O executor envia o resultado de cada comando que executa, além do estado e da contagem de réplicas de cada serviço gerenciado. Seu heartbeat informa se o canal de comandos está conectado, as versões dos componentes da plataforma, o digest do último pacote aplicado e o resultado da última atualização da plataforma. Em uma instalação em VM, o heartbeat também informa se o pacote de acesso ao Kubernetes está `ok`, `expiring` ou `expired`. Ao criar um serviço, `clicklink clctl instances create` envia o ID do seu ambiente, a nuvem e a região, o nome do serviço, os nomes dos buckets e o ARN da role do IAM. Ele envia dois hashes da senha do usuário `default`; a senha em si nunca sai.
...
<Note>
  A CLI pública `v0.16.0` não possui nem `--managed` nem `--no-managed`. A escolha no momento da instalação descrita abaixo requer uma compilação com [suporte ao modo managed](/pt-BR/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability). Para parar um executor existente, use as instruções do Helm ou do systemd abaixo; não é necessária nenhuma das flags de init.
</Note>
...
Os serviços que o ClickHouse criou continuam em execução, mas o ClickHouse Cloud não pode mais escalá-los, fazer upgrade, backup ou excluí-los, e os comandos pendentes para eles falham. As atualizações da plataforma nunca são aplicadas a menos que você execute `clicklink clctl platform approve`. Não executar esse comando congela a camada da plataforma, enquanto o restante do modo managed continua funcionando.
...
O executor interrompe o serviço, remove seus recursos do Kubernetes e exclui seu espaço de nomes. Ele informa que o serviço foi encerrado assim que o espaço de nomes deixa de existir. Tudo o que está fora do Kubernetes permanece: o bucket de dados, o bucket de backup e a IAM role do serviço. O executor não guarda credenciais dos seus buckets nem do IAM, portanto nada fora do Kubernetes é afetado.
...
A criação de serviços de cliente requer [suporte compatível na CLI e na implantação](/pt-BR/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) e não está disponível na versão pública `v0.16.0`.
...
2. **Interrompa o modo gerenciado.** Pare o executor e remova o grant dele conforme descrito em [Como impeço o ClickHouse de alterar meus clusters?](#stop-managed-mode). As atualizações de plataforma já exigem sua aprovação individual, portanto negar a aprovação as interrompe.
...
* Com uma [compilação que oferece suporte à aprovação de plataforma](/pt-BR/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability), pacotes de plataforma podem ser entregues fora de banda e passados como arquivo para `clicklink clctl platform approve --bundle`. A versão pública `v0.16.0` não possui o comando `platform`.
