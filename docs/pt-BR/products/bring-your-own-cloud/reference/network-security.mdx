...
| -------------------------------------------------------- | ---------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Operações diárias — servidor da API do Kubernetes**    | Pública com filtragem por IP (padrão), ou privada via Tailscale ou AWS VPC Lattice | Os serviços de gerenciamento se comunicam com o servidor da API do EKS pela rede pública, com acesso restrito por listas de permissão de IP. Após a implantação inicial, você pode opcionalmente alternar para acesso privado via Tailscale ou, na AWS, VPC Lattice. |
...
Por padrão, os serviços de gerenciamento da ClickHouse acessam seu cluster Kubernetes BYOC por meio do endereço IP público do servidor da API do EKS, que é restrito apenas aos endereços IP do gateway NAT da ClickHouse.

**Configuração opcional de endpoint privado**:

* Você pode configurar o servidor da API do EKS para usar apenas um endpoint privado
