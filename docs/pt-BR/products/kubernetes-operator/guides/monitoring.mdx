...
<Note>
  Este guia trata do **próprio processo do operador** (o controller manager). Para métricas do ClickHouse server (consultas, partes, defasagem de replicação), use o [Prometheus endpoint no ClickHouse](/pt-BR/reference/settings/server-settings/settings/other#prometheus) para coletá-las separadamente.
</Note>
