...
O ClickHouse Cloud agora oferece o ClickPipes para migrar seu banco de dados PostgreSQL externo para um serviço ClickHouse Managed Postgres. Essa integração nativa oferece uma experiência simplificada para conectar-se ao banco de dados de origem, exportar o esquema, importá-lo para o ClickHouse Managed Postgres e configurar a replicação contínua.
...
<Note>
  Se você encontrar problemas durante a migração, consulte o [FAQ de Migrações do ClickHouse Managed Postgres](/pt-BR/products/managed-postgres/migrations/faq) para ver os erros e as soluções mais comuns.
</Note>

<Steps>
  <Step title="Conecte-se ao seu banco de dados de origem" id="step-1-connect">
  ...
  <Step title="Importe o esquema para seu serviço ClickHouse Managed Postgres" id="step-3-import-schema">
  ...
  <Step title="Configure as definições de ingestão" id="step-4-ingestion-settings">
    Especifique a publication a ser usada na replicação lógica. Se você deixar esse campo em branco, uma publication será criada automaticamente.

    Expanda **Configurações avançadas de replicação** para ajustar a taxa de transferência:

    | Configuração                              | Padrão  | Descrição                                             |
    | ----------------------------------------- | ------- | ----------------------------------------------------- |
    | Intervalo de sincronização (segundos)     | 10      | Frequência com que o replication slot é consultado    |
    | Threads paralelas para carga inicial      | 4       | Número de threads para a fase de cópia em massa       |
    | Pull batch size                           | 100,000 | Linhas buscadas por batch de replicação               |
    | Número de linhas do snapshot por partição | 100000  | Tamanho da partição para snapshots de tabelas grandes |
    | Número de tabelas do snapshot em paralelo | 1       | Tabelas das quais é feito snapshot simultaneamente    |
  ...
  <Step title="Selecione as tabelas" id="step-5-select-tables">
    Selecione as tabelas que você deseja replicar. As tabelas são agrupadas por esquema. Selecione tabelas individuais ou expanda um esquema para selecionar todas.

    <Image img="/images/managed-postgres/pgpg/tablepicker.webp" alt="Etapa 5: seletor de tabelas agrupado por esquema com botão Criar migração" size="lg" border />
...
**Redirecione o tráfego da aplicação.** Aponte leituras e gravações para seu serviço ClickHouse Managed Postgres e monitore erros, violações de restrições e a integridade da replicação.
