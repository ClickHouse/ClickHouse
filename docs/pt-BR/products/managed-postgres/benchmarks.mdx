...
description: 'Benchmarks de desempenho que comparam o ClickHouse Managed Postgres com o AWS Aurora, RDS e outros serviços gerenciados de PostgreSQL'
...
<Info>
  ...
  * **Benchmark** do ClickHouse Managed Postgres em comparação com o AWS RDS (16k IOPS provisionadas) e o Aurora IO Optimized, usando os testes padrão do [`pgbench`](https://www.postgresql.org/docs/current/pgbench.html)
  * **Desempenho**: o ClickHouse Managed Postgres, com suporte a armazenamento NVMe, oferece **desempenho de 4,3 a 9x superior** para workloads com uso intensivo de E/S e **12% mais desempenho** em cenários limitados por CPU
...
**Análise**: Cargas de trabalho mistas de leitura/gravação evidenciam os ganhos de desempenho mais expressivos do armazenamento NVMe e representam o **cenário mais realista para cargas de trabalho impulsionadas por IA em rápido crescimento** que exigem tanto ingestão de dados com alta taxa de transferência quanto leituras de baixa latência. **ClickHouse Managed Postgres atingiu 19,8 mil TPS com maior concorrência**, demonstrando como o armazenamento NVMe escala de forma eficaz sob carga. Isso é **4,3-4,5x mais rápido que RDS e Aurora**. As soluções de armazenamento conectado via rede tiveram dificuldades com operações com uso intensivo de escrita, com RDS e Aurora atingindo no máximo 4,4 mil-4,6 mil TPS apesar da capacidade provisionada e mesmo com a configuração IO Optimized do Aurora.
...
| Configuração         | ClickHouse Managed Postgres | RDS com 16 mil IOPS            | Aurora IO Optimized             |
| -------------------- | --------------------------- | ------------------------------ | ------------------------------- |
...
**Análise**: A diferença de desempenho aumenta drasticamente em workloads intensivas em leitura e limitadas por E/S. **ClickHouse Managed Postgres entregou 84,8 mil TPS**, enquanto o RDS com 16.000 IOPS provisionadas alcançou apenas 9,4 mil TPS, apesar de contar com capacidade computacional equivalente. A principal diferença: o armazenamento NVMe no ClickHouse Managed Postgres escala com níveis mais altos de concorrência, enquanto o armazenamento conectado via rede continua limitado pelos limites de IOPS provisionadas. Mesmo com IOPS provisionadas, o RDS ainda foi 9x mais lento que o ClickHouse Managed Postgres, demonstrando a importância crítica da arquitetura de armazenamento para workloads intensivas em E/S.
...
| Configuração         | ClickHouse Managed Postgres | RDS com 16 mil IOPS              |
| -------------------- | --------------------------- | -------------------------------- |
| **Versão do PG**     | 17                          | 17                               |
| **vCPUs**            | 16                          | 16                               |
| **RAM**              | 64 GB                       | 64 GB                            |
...
**Análise**: Mesmo em cenários limitados por CPU, em que a E/S de disco é mínima, **o ClickHouse Managed Postgres liderou com 36,5 mil TPS**. Embora ambos os serviços tenham atingido 100% de utilização de CPU, o armazenamento NVMe no ClickHouse Managed Postgres entregou desempenho superior, com taxas de acerto de cache mais altas. A vantagem de 12% sobre o RDS demonstra a eficiência da infraestrutura subjacente, mesmo quando as cargas de trabalho são predominantemente limitadas por CPU.
...
| Configuração      | ClickHouse Managed Postgres | RDS PostgreSQL |
| ----------------- | --------------------------- | -------------- |
| **Versão do PG**  | 17                          | 17             |
| **vCPUs**         | 2                           | 2              |
| **RAM**           | 8 GB                        | 8 GB           |
...
Nos três cenários de benchmark, o ClickHouse Managed Postgres apresentou desempenho consistentemente superior:
...
| Aspecto                          | Armazenamento NVMe (ClickHouse Managed Postgres) | Armazenamento conectado via rede (IOPS provisionadas)         |
| -------------------------------- | ------------------------------------------------ | ------------------------------------------------------------- |
...
Além do desempenho bruto, o ClickHouse Managed Postgres oferece uma relação custo-desempenho superior:
...
Para cargas de trabalho atualmente limitadas por IOPS, migrar para o ClickHouse Managed Postgres pode eliminar a necessidade de IOPS provisionadas caras ou de configurações IO Optimized, ao mesmo tempo em que oferece um desempenho significativamente melhor.
...
* [Visão geral do ClickHouse Managed Postgres](/pt-BR/products/managed-postgres/overview)
* [Escalonando sua instância do Postgres](/pt-BR/products/managed-postgres/scaling)