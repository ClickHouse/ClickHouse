...
Vamos examinar o ciclo de vida de um serviço Postgres.

<Warning>
  **Ocultação de credenciais a partir de 31 de julho de 2026**

  A partir de **31 de julho de 2026**, a API não retorna mais as propriedades `password` e `connectionString` em suas respostas. Elas aparecem apenas na resposta de [criação](#create) e na resposta de [redefinição de senha] quando a solicitação não inclui senha. Recomendamos que você capture as credenciais da resposta de [criação](#create). Se você gerencia serviços com Terraform, faça upgrade do provider para a versão **v3.21.0 ou posterior** antes de 31 de julho de 2026 (consulte a [referência do Terraform](/pt-BR/products/managed-postgres/terraform)).
</Warning>
...
Content-Type. Salve a resposta para ter acesso às credenciais retornadas:
...
A saída será semelhante ao JSON retornado na criação (mas sem as propriedades `password` e `connectionString` após 31 de julho de 2026), mas fique de olho
...
[OpenAPI do ClickHouse]: /pt-BR/products/cloud/features/admin-features/api/api-overview "API do Cloud"

[OpenAPI]: https://www.openapis.org "Iniciativa OpenAPI"

[chaves de API]: /pt-BR/products/cloud/features/admin-features/api/openapi "Gerenciar chaves de API"
...
[endpoint do Prometheus]: /pt-BR/products/managed-postgres/monitoring/prometheus "Endpoint do Prometheus do ClickHouse Managed Postgres"

[referência de métricas]: /pt-BR/products/managed-postgres/monitoring/metrics "Referência de métricas do ClickHouse Managed Postgres"

[Query Insights]: /pt-BR/products/managed-postgres/monitoring/query-insights "Query Insights do Postgres"

[painel lateral de detalhes]: /pt-BR/products/managed-postgres/monitoring/query-insights#detail "Painel lateral de detalhes do Query Insights"

[contadores por execução]: /pt-BR/products/managed-postgres/monitoring/query-insights#counters "Contadores por execução do Query Insights"
