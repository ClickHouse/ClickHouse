...
<div id="metrics-access">
  ### Como posso acessar as métricas da minha instância do ClickHouse Managed Postgres?
</div>
...
O ClickHouse Managed Postgres inclui backups automáticos diários com arquivamento contínuo de WAL, permitindo a recuperação para qualquer momento dentro de uma janela de retenção de 7 dias. Os backups são armazenados no S3.
...
<div id="terraform-support">
  ### O ClickHouse Managed Postgres tem suporte ao Terraform?
</div>

No momento, o ClickHouse Managed Postgres ainda não oferece suporte ao Terraform. Recomendamos usar o console do ClickHouse Cloud ou [OpenAPI](/pt-BR/products/managed-postgres/openapi) para criar e gerenciar suas instâncias.
...
<div id="migration-tools">
  ### Quais ferramentas estão disponíveis para migrar para o ClickHouse Managed Postgres?
</div>
