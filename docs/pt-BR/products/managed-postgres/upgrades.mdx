...
keywords: ['upgrades do Managed Postgres', 'versão do Postgres', 'upgrade menor', 'upgrade principal', 'janela de manutenção']
doc_type: 'guide'
---
...
O ClickHouse Managed Postgres realiza upgrades de versão do PostgreSQL para manter suas instâncias seguras e atualizadas. Há suporte tanto para upgrades de versões secundárias quanto para upgrades entre versões principais, com interrupção mínima.
...
Para organizations do tier Enterprise, o ClickHouse Managed Postgres oferece suporte a janelas de manutenção, para que upgrades e outras operações de manutenção possam ser agendados no horário que cause o menor impacto possível à sua carga de trabalho. O suporte na UI e na API para configurar janelas de manutenção estará disponível em breve. Enquanto isso, entre em contato com o [suporte](https://clickhouse.com/support/program) para definir uma janela de manutenção para a sua instância.
