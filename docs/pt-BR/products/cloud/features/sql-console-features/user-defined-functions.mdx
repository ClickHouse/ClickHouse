...
4. Usando o [Terraform](#manage-udfs-with-terraform) (alpha)
...
Tudo o que está disponível na UI também pode ser acessado programaticamente por meio da [ClickHouse Cloud API](/pt-BR/cloud/manage/api/api-overview).
...
<Note>
  Esses recursos estão disponíveis na versão 3.24.0 e posteriores do provider. Eles estão em status alpha, e seu comportamento pode mudar em futuras versões do provider.
</Note>
