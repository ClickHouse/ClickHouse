...
Os motores de tabela a seguir também têm suporte e são usados exatamente como escritos. O motor de tabela `MySQL` compatível é diferente do motor de banco de dados `MySQL`, que não é compatível:
...
## Lista de funcionalidades não suportadas {#list-of-unsupported-features}
