...
<Note>
  **Gerenciar funções de banco de dados pelo console (beta)**

  Você também pode gerenciar funções de banco de dados diretamente no console do ClickHouse Cloud adicionando permissões de dados a funções personalizadas. Esse recurso está em beta e está disponível para serviços que executam o ClickHouse versão 26.4 e posteriores. As funções atribuídas manualmente usando a convenção de nomenclatura `sql-console-role:<email>` têm precedência sobre as funções atribuídas pelo console. Para mais informações, consulte [Gerenciar funções de banco de dados](/pt-BR/products/cloud/guides/security/cloud-access-management/manage-custom-roles#manage-database-roles).
</Note>
