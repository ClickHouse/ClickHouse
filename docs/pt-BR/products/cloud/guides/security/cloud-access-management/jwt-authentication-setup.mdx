...
<EnterprisePlanFeatureBadge feature="Autenticação JWT com provedor de identidade personalizado" support="true" />

<Tip>
  Este guia aborda a configuração de provedores JWKS no Cloud Console. Para saber como gerar um JWT e como é sua estrutura — as claims obrigatórias, as claims de funções e grants e o funcionamento dos usuários efêmeros — consulte a referência sobre [Autenticação JWT](/pt-BR/operations/external-authenticators/jwt).
</Tip>
...
Depois que o recurso for habilitado para sua organização, você poderá configurar esses provedores JWKS para cada serviço em **Settings → Security** no Cloud Console do ClickHouse.

<Note>
  Para habilitar provedores JWT para sua organização, entre em contato com o [suporte](https://clickhouse.com/support/program).
</Note>
...
* Uma URL JWKS **HTTPS** acessível publicamente que publique pelo menos uma chave **RSA** (os tokens devem ser assinados com **RS256**).

<Note>
  Provedores baseados em JWKS aceitam **apenas chaves RSA**. Tokens assinados com chaves HMAC (`HS256`) ou EC (`ES256`) não podem ser verificados em um endpoint JWKS e são rejeitados. Um documento JWKS pode conter outros tipos de chave, mas deve haver pelo menos uma chave RSA utilizável.
</Note>
...
O cliente (seu provedor de identidade ou aplicação) gera um JWT e o assina com sua **chave privada**. O token deve seguir o [formato de token](/pt-BR/operations/external-authenticators/jwt#token-claims) esperado. Em seguida, o ClickHouse o verifica usando as chaves **públicas** publicadas no URL do seu JWKS:

1. O ClickHouse lê o cabeçalho `kid` (ID da chave) do token e seleciona a chave correspondente no documento JWKS.
2. Ele verifica a assinatura do token usando essa chave pública RSA e valida as claims `iss` (emissor) e `aud` (audiência) em relação à configuração do seu provedor.
3. Em caso de sucesso, a conexão é estabelecida como um usuário efêmero, cujos direitos de acesso são derivados das claims `clickhouse:grants` e `clickhouse:roles` do token, limitados pelo limite de permissões (o usuário `default`). Consulte [Direitos de acesso](/pt-BR/operations/external-authenticators/jwt#access-rights) para obter detalhes.
...
<Steps>
  ...
  <Step title="Preencha os detalhes do provedor" id="fill-provider-details">
    ...
    | **JWKS URL**               | A URL HTTPS pública que disponibiliza suas chaves públicas RSA usadas para verificar assinaturas de tokens.                                                                                        |
...
* Provedores JWKS aceitam apenas chaves **RSA / RS256**.
...
* [Autenticação JWT](/pt-BR/operations/external-authenticators/jwt) — claims de tokens, usuários efêmeros e uso do cliente.