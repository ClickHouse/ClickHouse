...
<Steps>
  ...
  <Step>
    ...
    Clique no nome da organização no canto inferior esquerdo e selecione `Users and Roles`. Revise a coluna `Provider` de cada usuário; todos os usuários que exibirem `Signed in with SSO` precisarão ser convidados novamente para a organização depois que o SAML for removido.
...
<Steps>
  <Step>
    ...
    Clique no nome da organização no canto inferior esquerdo e, em seguida, selecione `Users and Roles`. Siga as instruções para [convidar usuários](/pt-BR/products/cloud/guides/security/cloud-access-management/manage-cloud-users#invite-users).
  </Step>
