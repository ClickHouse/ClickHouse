---
description: 'O motor Hive permite que você execute consultas `SELECT` na
  tabela Hive no HDFS.'
sidebar_label: 'Hive'
sidebar_position: 84
...
O motor Hive permite executar consultas `SELECT` em uma tabela Hive no HDFS. Atualmente, ele oferece suporte aos formatos de entrada abaixo:
...
A estrutura da tabela pode diferir da estrutura original da tabela Hive:

* Os nomes das colunas devem ser os mesmos da tabela Hive original, mas você pode usar apenas algumas dessas colunas, em qualquer ordem; também pode usar colunas de alias calculadas a partir de outras colunas.
* Os tipos de coluna devem ser os mesmos da tabela Hive original.
...
Recomendamos fortemente habilitar o cache local para sistemas de arquivos remotos. Benchmarks mostram que o desempenho é quase 2x melhor com cache.
...
* enable: o ClickHouse manterá um cache local para o sistema de arquivos remoto (HDFS) após a inicialização se estiver como `true`.
* root&#95;dir: Obrigatório. O diretório raiz para armazenar os arquivos do cache local do sistema de arquivos remoto.
* limit&#95;size: Obrigatório. O tamanho máximo (em bytes) dos arquivos do cache local.
* bytes&#95;read&#95;before&#95;flush: Controla a quantidade de bytes antes do flush no sistema de arquivos local ao baixar um arquivo do sistema de arquivos remoto. O valor padrão é 1MB.

<div id="query-hive-table-with-orc-input-format">
  ### Consultar tabela Hive com formato de entrada ORC
</div>
...
Tabela no ClickHouse que recupera dados da tabela Hive criada acima:
...
<div id="query-hive-table-with-parquet-input-format">
  ### Consultar tabela Hive com formato de entrada Parquet
</div>
...
<div id="query-hive-table-with-text-input-format">
  ### Consultar tabela Hive com o formato de entrada Text
</div>
