...
Para os casos em que você precisa de mais personalização do que a oferecida por outros formatos padrão,
o formato `Template` permite que o usuário defina sua própria string de formato com placeholders para valores
e especifique regras de escaping para os dados.
...
| `format_template_row_format`                                                                                             | Especifica a string de formato para linhas [inline](#inline_specification).                                           |
| `format_template_resultset_format`                                                                                       | Especifica a string de formato do conjunto de resultados [inline](#inline_specification).                             |
| Algumas configurações de outros formatos (por exemplo, `output_format_json_quote_64bit_integers` ao usar escaping `JSON` |                                                                                                                        |
...
A configuração `format_template_row` especifica o caminho para o arquivo que contém as strings de formato para as linhas com a seguinte sintaxe:
...
As seguintes regras de escape são compatíveis:

| Regra de escape      | Descrição                                   |
| -------------------- | ------------------------------------------- |
| `CSV`, `JSON`, `XML` | Semelhante aos formatos com os mesmos nomes |
| `Escaped`            | Semelhante a `TSV`                          |
| `Quoted`             | Semelhante a `Values`                       |
| `Raw`                | Sem escape, semelhante a `TSVRaw`           |
| `None`               | Sem regra de escape — veja a nota abaixo    |
...
Os valores a seguir serão exibidos (se estiver usando `SELECT`) ou esperados (se estiver usando `INPUT`),
...
* Ao fazer `INSERT`, a linha abaixo corresponde ao template esperado e leria os valores `bathroom interior design`, `2166`, `$3` para as colunas `Search phrase`, `count`, `ad price`.
...
A string de formato do conjunto de resultados tem a mesma sintaxe que uma string de formato para linhas.
Ela permite especificar um prefixo, um sufixo e uma forma de imprimir algumas informações adicionais, além de conter os seguintes placeholders em vez de nomes de colunas:
...
* `rows_before_limit` é o número mínimo de linhas que existiriam sem LIMIT. Só é gerado se a consulta contiver LIMIT. Se a consulta contiver GROUP BY, `rows_before_limit_at_least` será o número exato de linhas que existiriam sem LIMIT.
* `time` é o tempo de execução da requisição, em segundos.
* `rows_read` é o número de linhas lidas.
* `bytes_read` é o número de bytes (não comprimidos) lidos.
...
Para consultas INSERT, o formato permite ignorar algumas colunas ou campos se houver prefixo ou sufixo (veja o exemplo).

<div id="inline_specification">
  ### Especificação inline
</div>

Muitas vezes, é difícil ou impossível implantar as configurações de formato
...
em vez de usar o caminho para o arquivo que a contém.
...
Veja dois exemplos de como usar o formato `Template`: primeiro para selecionar dados e depois para inseri-los.
...
`PageViews`, `UserID`, `Duration` e `Sign` dentro dos placeholders são nomes de colunas da tabela. Os valores após `Useless field` em cada linha e após `\nTotal rows:` no sufixo serão ignorados.
Todos os delimitadores nos dados de entrada devem corresponder exatamente aos delimitadores nas strings de formato especificadas.

<div id="inline_specification">
  ### Especificação inline
</div>

Cansado de formatar tabelas em markdown manualmente? Neste exemplo, veremos como usar o formato `Template` e as configurações de especificação inline para realizar uma tarefa simples: fazer `SELECT` dos nomes de alguns formatos do ClickHouse da tabela `system.formats` e formatá-los como uma tabela em markdown. Isso pode ser feito facilmente usando o formato `Template` e as configurações `format_template_row_format` e `format_template_resultset_format`.

Nos exemplos anteriores, especificamos as strings de formato do conjunto de resultados e da linha em arquivos separados, com os caminhos para esses arquivos definidos usando as configurações `format_template_resultset` e `format_template_row`, respectivamente. Aqui, faremos isso in-line porque nosso template é trivial, consistindo apenas em alguns `|` e `-` para montar a tabela em markdown. Vamos especificar nossa string de template do conjunto de resultados usando a configuração `format_template_resultset_format`. Para criar o cabeçalho da tabela, adicionamos `|ClickHouse Formats|\n|---|\n` antes de `${data}`. Usamos a configuração `format_template_row_format` para especificar a string de template ``|`{0:XML}`|`` para nossas linhas. O formato `Template` inserirá nossas linhas com o formato fornecido no placeholder `${data}`. Neste exemplo, temos apenas uma coluna, mas, se você quisesse adicionar mais, poderia fazer isso acrescentando `{1:XML}`, `{2:XML}`... etc. à sua string de template de linha, escolhendo a regra de escape adequada. Neste exemplo, optamos pela regra de escape `XML`.
...
Olha só! Nos poupamos do trabalho de ter que adicionar manualmente todos aqueles `|` e `-` para criar essa tabela em markdown:
