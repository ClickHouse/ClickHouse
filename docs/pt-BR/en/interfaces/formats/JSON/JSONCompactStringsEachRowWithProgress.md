...
Semelhante a [`JSONCompactEachRowWithProgress`](/pt-BR/interfaces/formats/JSONCompactEachRowWithProgress), mas todos os valores são convertidos em strings.
Isso é útil quando você precisa de uma representação textual consistente para todos os tipos de dados.

Principais recursos:

* Mesma estrutura que `JSONCompactEachRowWithProgress`
* Todos os valores são representados como strings (números, arrays etc. são sempre strings entre aspas)
