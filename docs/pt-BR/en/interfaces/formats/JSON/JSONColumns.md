...
visualmente, os dados são girados 90 graus para a esquerda.
:::

Neste formato, todos os dados são representados como um único objeto JSON.

:::note
O formato `JSONColumns` armazena todos os dados em memória e depois os gera como um único bloco, portanto pode levar a um alto consumo de memória.
:::
...
Use o formato `JSONColumns` para ler os dados:
...
Durante a importação, colunas com nomes desconhecidos serão ignoradas se a configuração [`input_format_skip_unknown_fields`](/pt-BR/operations/settings/settings-formats.md/#input_format_skip_unknown_fields) estiver definida como `1`.
As colunas que não estiverem presentes no bloco serão preenchidas com valores padrão (você pode usar a configuração [`input_format_defaults_for_omitted_fields`](/pt-BR/operations/settings/settings-formats.md/#input_format_defaults_for_omitted_fields) nesse caso)