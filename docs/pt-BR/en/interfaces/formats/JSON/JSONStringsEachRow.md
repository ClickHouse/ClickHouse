...
Difere de [`JSONEachRow`](./JSONEachRow.md) apenas no fato de que os campos de dados são gerados como strings, e não como valores JSON tipados.
...
Use um arquivo JSON com os seguintes dados, chamado `football.json`:
...
Leia os dados usando o formato `JSONStringsEachRow`:
