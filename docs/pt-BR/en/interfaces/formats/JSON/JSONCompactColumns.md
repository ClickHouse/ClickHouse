---
alias: []
description: 'Documentação do formato JSONCompactColumns'
input_format: true
...
Neste formato, todos os dados são representados como um único array JSON.

:::note
O formato de saída `JSONCompactColumns` mantém todos os dados em buffer na memória para produzi-los como um único bloco, o que pode levar a um alto consumo de memória.
:::
...
Leia os dados usando o formato `JSONCompactColumns`:
...
Colunas que não estiverem presentes no bloco serão preenchidas com valores padrão (aqui, você pode usar a configuração [`input_format_defaults_for_omitted_fields`](/pt-BR/operations/settings/settings-formats.md/#input_format_defaults_for_omitted_fields))
