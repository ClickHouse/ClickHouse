...
description: 'Documentação sobre o formato JSONLines'
keywords: ['JSONLines']
...
Nesse formato, ClickHouse gera cada linha como um objeto JSON separado, delimitado por quebra de linha.

Esse formato também é conhecido como `JSONEachRow`, `JSONLines`, `NDJSON` (JSON delimitado por quebra de linha) ou `JSONL`. Esses nomes são aliases do mesmo formato e podem ser usados de forma intercambiável tanto para entrada quanto para saída.
...
Leia os dados no formato `JSONLines`:
...
Colunas de dados com nomes desconhecidos serão ignoradas durante a importação se a configuração [input&#95;format&#95;skip&#95;unknown&#95;fields](/pt-BR/operations/settings/settings-formats.md/#input_format_skip_unknown_fields) estiver definida como 1.
