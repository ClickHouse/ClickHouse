...
Ele é semelhante ao `dwarfdump`, mas muito mais rápido (centenas de MB/s) e oferece suporte a SQL.
...
:::info
`.debug_info` consiste em *units*, que correspondem a unidades de compilação:
...
* `ancestor_tags` - array de tags dos ancestrais do DIE atual na árvore, em ordem do mais interno para o mais externo
* `ancestor_offsets` - offsets dos ancestrais, em paralelo com `ancestor_tags`
* alguns atributos comuns duplicados do array de atributos por conveniência:
  * `name`
  * `linkage_name` - nome totalmente qualificado com name mangling; normalmente apenas funções o têm (mas nem todas as funções)
...
* arrays paralelos que descrevem os atributos:
  ...
  * `attr_int` - valor inteiro do atributo; 0 se o atributo não tiver valor numérico
  * `attr_str` - valor em string do atributo; vazio se o atributo não tiver valor em string
