...
sidebar_label: 'Sobrecarga do servidor'
...
Às vezes, o servidor pode ficar sobrecarregado por diferentes motivos. Para determinar a sobrecarga atual da CPU,
o servidor ClickHouse calcula a razão entre o tempo de espera da CPU (métrica `OSCPUWaitMicroseconds`) e o tempo em atividade
(métrica `OSCPUVirtualTimeMicroseconds`). Quando o servidor fica sobrecarregado acima de uma determinada razão,
...
Há uma configuração do servidor chamada `os_cpu_busy_time_threshold` que controla o tempo mínimo em atividade para considerar que a CPU
está realizando algum trabalho útil. Se o valor atual da métrica `OSCPUVirtualTimeMicroseconds` estiver abaixo desse valor,
presume-se que a sobrecarga da CPU seja 0.
...
com certa probabilidade se a razão de sobrecarga for pelo menos `min_os_cpu_wait_time_ratio_to_throw`. A probabilidade
é determinada por interpolação linear entre as razões mínima e máxima. Por exemplo, se `min_os_cpu_wait_time_ratio_to_throw = 2`,
...
A remoção de conexões é controlada pelas configurações de nível de servidor `min_os_cpu_wait_time_ratio_to_drop_connection` e
...
dessas configurações é semelhante à usada para rejeitar consultas. A única diferença, neste caso, é que, se o servidor estiver sobrecarregado,
a tentativa de conexão será rejeitada pelo servidor.
...
personalizar esses limiares na configuração do servidor.
