---
description: 'Tabela de sistema que contém stack traces de todas as threads do servidor. Permite
...
Contém stack traces de todas as threads do servidor. Permite aos desenvolvedores inspecionar o estado do servidor.

Para analisar os frames da stack, use as [funções de introspecção](../../sql-reference/functions/introspection.md) `addressToLine`, `addressToLineWithInlines`, `addressToSymbol` e `demangle`.
...
* `thread_name` ([String](/pt-BR/sql-reference/data-types/string)) — O nome da thread.
* `thread_id` ([UInt64](/pt-BR/sql-reference/data-types/int-uint)) — O identificador da thread
* `query_id` ([String](/pt-BR/sql-reference/data-types/string)) — O ID da consulta à qual esta thread pertence.
* `trace` ([Array(UInt64)](/pt-BR/sql-reference/data-types/array)) — O stacktrace desta thread. Basicamente, é apenas um array de endereços.
* `untracked_memory` ([Int64](/pt-BR/sql-reference/data-types/int-uint)) — Contador por thread não atômico das alocações de memória que ainda não foram propagadas para o MemoryTracker pai. Pode ser negativo se mais memória tiver sido liberada do que alocada desde o último flush.
...
Para habilitar funções de introspecção:

```sql
SET allow_introspection_functions = 1;
```

Obtendo símbolos dos arquivos objeto do ClickHouse:
...
* [Funções de introspecção](../../sql-reference/functions/introspection.md) — Quais funções de introspecção estão disponíveis e como usá-las.
* [system.trace&#95;log](../system-tables/trace_log.md) — Contém stack traces coletados pelo profiler de consultas por amostragem.
* [arrayMap](/pt-BR/sql-reference/functions/array-functions#arrayMap)) — Descrição e exemplo de uso da função `arrayMap`.
* [arrayFilter](/pt-BR/sql-reference/functions/array-functions#arrayFilter) — Descrição e exemplo de uso da função `arrayFilter`.