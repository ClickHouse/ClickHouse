---
description: 'Tabela de sistema útil para especialistas em C++ e engenheiros do ClickHouse, com
  informações para introspecção do binário `clickhouse`.'
keywords: ['tabela de sistema', 'símbolos']
...
Contém informações para introspecção do binário `clickhouse`. É necessário o privilégio de introspecção para acessá-la.
...
* `symbol` ([String](/pt-BR/sql-reference/data-types/string)) — Nome do símbolo no binário. Ele está com name mangling. Você pode aplicar demangle(symbol) para obter um nome legível.
* `symbol_demangled` ([Nullable(String)](/pt-BR/sql-reference/data-types/nullable)) — Símbolo com o name mangling removido, usado para a instrumentação do XRay.
* `function_id` ([Nullable(Int32)](/pt-BR/sql-reference/data-types/nullable)) — ID da função no mapa de instrumentação do XRay.
* `address_begin` ([UInt64](/pt-BR/sql-reference/data-types/int-uint)) — Endereço inicial do símbolo no binário.
* `address_end` ([UInt64](/pt-BR/sql-reference/data-types/int-uint)) — Endereço final do símbolo no binário.
