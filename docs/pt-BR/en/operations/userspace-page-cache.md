...
na memória do processo, em vez de depender do cache de páginas do SO.'
sidebar_label: 'Cache de páginas em espaço do usuário'
sidebar_position: 65
slug: /operations/userspace-page-cache
title: 'Cache de páginas em espaço do usuário'
doc_type: 'referência'
---
...
> O cache de páginas em espaço do usuário é um novo mecanismo de cache que permite armazenar
> dados na memória do próprio processo, em vez de depender do cache de páginas do SO.

O ClickHouse já oferece o [cache do sistema de arquivos](/pt-BR/docs/operations/storing-data)
como uma forma de cache sobre armazenamento remoto de objetos, como Amazon S3, Google
Cloud Storage (GCS) ou Azure Blob Storage. O cache de páginas em espaço do usuário foi projetado
para acelerar o acesso a dados remotos quando o cache normal do SO não é suficiente.

Ele difere do cache do sistema de arquivos das seguintes formas:

| Cache do sistema de arquivos                                           | Cache de páginas em espaço do usuário        |
| ---------------------------------------------------------------------- | -------------------------------------------- |
...
<div id="configuration-settings-and-usage">
  ## Configurações e uso da configuração
</div>
...
Para habilitar o cache de páginas em espaço do usuário, primeiro configure-o no servidor:
...
:::note
O cache de páginas em espaço do usuário usará até a quantidade de memória especificada, mas
essa quantidade de memória não fica reservada. A memória será removida do cache quando for necessária
...
| `use_page_cache_with_distributed_cache`                 | Usa o cache de páginas em espaço do usuário quando o distributed cache está em uso.                                                                                                                                                                                                                                                                                    | `0`         |
| `read_from_page_cache_if_exists_otherwise_bypass_cache` | Usa o cache de páginas em espaço do usuário no modo passivo, semelhante a [`read_from_filesystem_cache_if_exists_otherwise_bypass_cache`](/pt-BR/docs/operations/settings/settings#read_from_filesystem_cache_if_exists_otherwise_bypass_cache).                                                                                                                             | `0`         |
...
| `page_cache_policy`                                     | Nome da política de cache do cache de páginas em espaço do usuário.                                                                                                                                                                                                                                                                                                    | `SLRU`      |
...
| `page_cache_lookahead_blocks`                           | Em caso de cache miss no cache de páginas em espaço do usuário, lê até essa quantidade de blocos consecutivos de uma vez do armazenamento subjacente, se eles também não estiverem no cache. Cada bloco tem page&#95;cache&#95;block&#95;size bytes.                                                                                                                   | `16`        |
| `page_cache_shards`                                     | Distribui o cache de páginas em espaço do usuário por esta quantidade de shards para reduzir a contenção de mutex. Experimental, provavelmente não melhorará o desempenho.                                                                                                                                                                                             | `4`         |

<div id="related-content">
  ## Conteúdo relacionado
</div>

* [Cache do sistema de arquivos](/pt-BR/docs/operations/storing-data)
