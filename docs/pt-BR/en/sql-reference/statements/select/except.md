---
description: 'Documentação da cláusula EXCEPT, que retorna apenas as linhas produzidas pela primeira consulta, excluindo as da segunda.'
...
> A cláusula `EXCEPT` retorna apenas as linhas produzidas pela primeira consulta, excluindo as da segunda.

* Ambas as consultas devem ter o mesmo número de colunas, na mesma ordem e com o mesmo tipo de dado.
* O resultado de `EXCEPT` pode conter linhas duplicadas. Use `EXCEPT DISTINCT` se isso não for desejado.
* Várias instruções `EXCEPT` são executadas da esquerda para a direita se não houver parênteses.
* O operador `EXCEPT` tem a mesma prioridade que a cláusula `UNION` e prioridade inferior à da cláusula `INTERSECT`.
...
Aqui está um exemplo simples que retorna os números de 1 a 10 que *não* fazem parte do conjunto de números de 3 a 8:
...
`EXCEPT` e `INTERSECT` muitas vezes podem ser usados de forma intercambiável com lógicas booleanas diferentes, e ambos são úteis quando você tem duas tabelas que compartilham uma coluna em comum (ou várias colunas).
Por exemplo, suponha que temos alguns milhões de linhas de dados históricos de criptomoedas contendo preços de negociação e volume:
...
Isso significa que, das quatro criptomoedas que temos, apenas o Bitcoin nunca caiu abaixo de $10 (com base nos dados limitados deste exemplo).

<div id="using-except-distinct">
  ### Usando `EXCEPT DISTINCT`
</div>

Observe que, na consulta anterior, havia várias posições de Bitcoin no resultado. Você pode adicionar `DISTINCT` a `EXCEPT` para eliminar linhas duplicadas do resultado:
...
* [INTERSECT](/pt-BR/sql-reference/statements/select/intersect)