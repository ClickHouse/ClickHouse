...
[ROLLUP](../statements/select/group-by.md/#rollup-modifier) e [CUBE](../statements/select/group-by.md/#cube-modifier) são modificadores de GROUP BY. Ambos calculam subtotais. ROLLUP recebe uma lista ordenada de colunas, por exemplo `(day, month, year)`, e calcula subtotais em cada nível da agregação e, em seguida, o total geral. CUBE calcula subtotais para todas as combinações possíveis das colunas especificadas. GROUPING identifica quais linhas retornadas por ROLLUP ou CUBE são superagregados e quais seriam retornadas por um GROUP BY sem modificadores.

A função GROUPING recebe várias colunas como argumento e retorna uma máscara de bits.

* `1` indica que uma linha retornada por um modificador `ROLLUP` ou `CUBE` de `GROUP BY` é um subtotal
...
Por padrão, o modificador CUBE calcula subtotais para todas as combinações possíveis das colunas passadas para CUBE. GROUPING SETS permite especificar quais combinações devem ser calculadas.

Analisar dados hierárquicos é um bom caso de uso para os modificadores ROLLUP, CUBE e GROUPING SETS. O exemplo aqui é uma tabela que contém dados sobre quais distribuições Linux e quais versões dessas distribuições estão instaladas em dois data centers. Pode ser útil analisar os dados por distribuição, versão e local.
...
Obtenha o número de servidores em cada data center, por distribuição:
...
<div id="comparing-multiple-group-by-statements-with-grouping-sets">
  ### Comparando várias cláusulas GROUP BY com GROUPING SETS
</div>

Detalhando os dados sem CUBE, ROLLUP ou GROUPING SETS:
...
As mesmas informações com GROUPING SETS:
...
O CUBE na próxima consulta, `CUBE(datacenter,distro,version)`, fornece uma hierarquia que pode não fazer sentido. Não faz sentido analisar a versão entre as duas distribuições (pois Arch e RHEL não têm o mesmo ciclo de lançamentos nem os mesmos padrões de nomenclatura de versão). O exemplo de GROUPING SETS a seguir é mais apropriado, pois agrupa `distro` e `version` no mesmo conjunto.
