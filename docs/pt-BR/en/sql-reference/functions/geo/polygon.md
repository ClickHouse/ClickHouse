---
description: 'Documentação de referência para Polygon'
sidebar_label: 'polígonos'
slug: /sql-reference/functions/geo/polygons
title: 'Funções para trabalhar com polígonos'
doc_type: 'reference'
---
...
Retorna um objeto geométrico WKT (Well Known Text) a partir de vários [Tipos de Dados Geo](../../data-types/geo.md). Os objetos WKT suportados são:
...
`geo_data` pode ser um dos seguintes [Tipos de Dados Geo](../../data-types/geo.md) ou um de seus tipos primitivos subjacentes:
...
* O objeto geométrico WKT `POINT` é retornado para um Point.
* O objeto geométrico WKT `POLYGON` é retornado para um Polygon
* O objeto geométrico WKT `MULTIPOLYGON` é retornado para um multipolígono.
* O objeto geométrico WKT `LINESTRING` é retornado para um LineString.
* O objeto geométrico WKT `MULTILINESTRING` é retornado para um MultiLineString.
...
MULTIPOLYGON a partir de um array de arrays de tuplas multidimensionais:
...
Converte um multipolígono em formato WKT (Well Known Text) em um valor do tipo multipolígono.
...
| tipo         | saída                                                                                                            |
| :----------- | :--------------------------------------------------------------------------------------------------------------- |
| multipolígono | [[[(2,0),(10,0),(10,10),(0,10),(2,0)],[(4,4),(5,4),(5,5),(4,5),(4,4)]],[[(-10,-10),(-10,-9),(-9,10),(-10,-10)]]] |
...
Converte um multipolígono em WKT (Well Known Text) para o tipo Polygon.
...
A função `readWKTPoint` no ClickHouse analisa uma representação em Well-Known Text (WKT) de uma geometria Point e retorna um ponto no formato interno do ClickHouse.
...
* `wkt_string`: A string WKT de entrada que representa uma geometria do tipo Point.

<div id="returned-value">
  ### Valor retornado
</div>

A função retorna uma representação interna, no ClickHouse, da geometria Point.
...
Analisa a representação de uma geometria LineString em Well-Known Text (WKT) e a retorna no formato interno do ClickHouse.
...
Analisa uma representação no formato Well-Known Text (WKT) de uma geometria MultiLineString e a retorna no formato interno do ClickHouse.
...
Analisa uma representação no formato Well-Known Text (WKT) de uma geometria Polygon e retorna um ring (linestring fechada) no formato interno do ClickHouse.
...
A função retorna uma representação interna do ClickHouse para a geometria de anel (LineString fechada).
...
Retorna true ou false, dependendo de um polígono estar ou não completamente contido em outro polígono. Referência: https://www.boost.org/doc/libs/1&#95;62&#95;0/libs/geometry/doc/html/geometry/reference/algorithms/within/within&#95;2.html
...
Converte um multipolígono em WKB (Well Known Binary) para o tipo multipolígono.
...
String iniciada por `MULTIPOLYGON`
...
Converte um MultiPolygon em WKB (Well Known Binary) para o tipo Polygon.
...
A função `readWKBPoint` do ClickHouse analisa a representação Well-Known Binary (WKB) de uma geometria Point e retorna um ponto no formato interno do ClickHouse.
...
A função retorna a representação interna do ClickHouse para a geometria Point.
...
Analisa a representação Well-Known Binary (WKB) de uma geometria LineString e a retorna no formato interno do ClickHouse.
...
* `wkb_string`: A String WKB de entrada que representa uma geometria LineString.
...
A função retorna a representação interna do ClickHouse para a geometria multilinestring.
...
Calcula a distância mínima entre dois pontos, em que um ponto pertence ao primeiro polígono e o segundo a outro polígono. Esférica significa que as coordenadas são interpretadas como coordenadas em uma esfera pura e ideal, o que não corresponde à realidade da Terra. Usar esse tipo de sistema de coordenadas acelera a execução, mas, é claro, não é preciso.
...
Calcula a diferença simétrica espacial, na teoria dos conjuntos (XOR), entre dois polígonos
...
Igual a `polygonsSymDifferenceSpherical`, mas as coordenadas estão no sistema de coordenadas cartesianas, que é mais próximo do modelo da Terra real.
...
Calcula a interseção (AND) entre polígonos com coordenadas esféricas.
...
Retorna true se o segundo polígono estiver contido no primeiro.
...
Retorna true se os dois polígonos se intersectam (têm alguma área ou borda em comum).
...
Retorna true se os dois polígonos se cruzam (compartilham alguma área ou limite em comum). Referência: https://www.boost.org/doc/libs/1&#95;62&#95;0/libs/geometry/doc/html/geometry/reference/algorithms/intersects.html
...
Calcula uma envoltória convexa. [Referência](https://www.boost.org/doc/libs/1_61_0/libs/geometry/doc/html/geometry/reference/algorithms/convex_hull.html)
...
Calcula a área de superfície de um polígono.
...
<div id="polygon-representing-zimbabwe">
  #### Polygon representando o Zimbábue
</div>
...
Para mais informações sobre sistemas geométricos, consulte esta [apresentação](https://archive.fosdem.org/2020/schedule/event/working_with_spatial_trajectories_in_boost_geometry/attachments/slides/3988/export/events/attachments/working_with_spatial_trajectories_in_boost_geometry/slides/3988/FOSDEM20_vissarion.pdf) sobre a biblioteca Boost, que é a biblioteca usada pelo ClickHouse.