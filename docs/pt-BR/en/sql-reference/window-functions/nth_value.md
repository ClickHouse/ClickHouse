...
Retorna o primeiro valor não NULL avaliado na enésima linha (offset) dentro do frame ordenado.
...
* `x` — Nome da coluna.
* `offset` — enésima linha em relação à qual a linha atual será avaliada.

**Valor retornado**

* O primeiro valor não `NULL` avaliado em relação à enésima linha (`offset`) dentro do frame ordenado.
