---
description: 'Página de visão geral das funções de janela'
sidebar_label: 'Funções de Janela'
sidebar_position: 1
...
Elas podem ser usadas para realizar cálculos semelhantes aos feitos com funções de agregação, mas diferem porque uma função de janela não agrupa as linhas em uma única saída — em vez disso, as linhas individuais continuam sendo retornadas.
...
| Funcionalidade                                                                        | Compatível? | Comentário                                                                                                                                                                                                                                                                                                                                                                                                        |
| ------------------------------------------------------------------------------------- | ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| Sintaxe `INTERVAL` para frame `RANGE OFFSET` de `DateTime`                            | ❌           | Em vez disso, especifique o número de segundos (`RANGE` funciona com qualquer tipo numérico).                                                                                                                                                                                                                                                                                                                     |
| frame `GROUPS`                                                                        | ❌           |                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| `percent_rank()`/`percentRank()`                                                      | ✅           | Calcula com eficiência a posição relativa de um valor dentro de uma partição. Substitui o cálculo SQL manual mais detalhado e computacionalmente mais custoso, expresso como `ifNull((rank() OVER (PARTITION BY x ORDER BY y) - 1) / nullif(count(1) OVER (PARTITION BY x) - 1, 0), 0)`.                                                                                                                          |
| `cume_dist()`                                                                         | ✅           | Calcula a distribuição cumulativa de um valor dentro de um grupo de valores. Retorna a porcentagem de linhas com valores menores que ou iguais ao valor da linha atual.                                                                                                                                                                                                                                           |
| `lag/lead(value, offset)`                                                             | ✅           | Você também pode usar uma das soluções alternativas a seguir:<br /> 1) `any(value) OVER (... ROWS BETWEEN <offset> PRECEDING AND <offset> PRECEDING)`, ou `FOLLOWING` em vez de `PRECEDING` para `lead` <br /> 2) `lagInFrame/leadInFrame`, que são análogas, mas respeitam o frame da janela. Para obter um comportamento idêntico a `lag/lead`, use `ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING`. |
| [`ntile(buckets)`](./ntile.md)                                                        | ✅           | Especifique a janela como, por exemplo, `(PARTITION BY x ORDER BY y ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING)`.                                                                                                                                                                                                                                                                                   |
...
* `PARTITION BY` - define como particionar um conjunto de resultados em grupos.
* `ORDER BY` - define como ordenar as linhas dentro do grupo durante o cálculo da aggregate&#95;function.
* `ROWS or RANGE` - define os limites de um frame; a aggregate&#95;function é calculada dentro desse frame.
...
<div id="functions">
  ## Funções que só podem ser usadas como funções de janela
</div>
...
| Função                                                                                                     | Descrição                                                                                                                        |
| ---------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| [`row_number()`](./row_number.md)                                                                          | Numera a linha atual dentro da sua partição, começando em 1.                                                                     |
| [`first_value(x)`](./first_value.md)                                                                       | Retorna o primeiro valor avaliado dentro do seu frame ordenado.                                                                  |
| [`last_value(x)`](./last_value.md)                                                                         | Retorna o último valor avaliado dentro do seu frame ordenado.                                                                    |
| [`nth_value(x, offset)`](./nth_value.md)                                                                   | Retorna o primeiro valor não `NULL` avaliado na enésima linha (`offset`) do seu frame ordenado.                                  |
| [`rank()`](./rank.md)                                                                                      | Classifica a linha atual dentro da sua partição com lacunas.                                                                     |
| [`dense_rank()`](./dense_rank.md)                                                                          | Classifica a linha atual dentro da sua partição sem lacunas.                                                                     |
| [`lagInFrame(x)`](./lagInFrame.md)                                                                         | Retorna um valor avaliado na linha que está a um deslocamento físico especificado antes da linha atual dentro do frame ordenado. |
| [`leadInFrame(x)`](./leadInFrame.md)                                                                       | Retorna um valor avaliado na linha que está a um número de linhas especificado após a linha atual dentro do frame ordenado.      |
| [`ntile(buckets)`](./ntile.md)                                                                             | Divide as linhas ordenadas dentro de uma partição no número especificado de buckets e retorna o número do bucket da linha atual. |
| [`nonNegativeDerivative(metric_column, timestamp_column[, INTERVAL X UNITS])`](./nonNegativeDerivative.md) | Calcula a derivada não negativa de `metric_column` em relação a `timestamp_column`. Específica do ClickHouse.                    |

<div id="examples">
  ## Exemplos
</div>

Veja alguns exemplos de uso das funções de janela.
...
Os exemplos a seguir resolvem problemas práticos comuns.
...
<div id="moving--sliding-average-per-3-rows">
  ### Média móvel / média deslizante (por 3 linhas)
</div>
...
<div id="moving--sliding-average-per-10-seconds">
  ### Média móvel / média deslizante (a cada 10 segundos)
</div>
...
<div id="moving--sliding-average-per-10-days">
  ### Média móvel / média deslizante (a cada 10 dias)
</div>

A temperatura é armazenada com precisão de segundos, mas, usando `Range` e `ORDER BY toDate(ts)`, formamos um frame de 10 unidades e, como usamos `toDate(ts)`, a unidade é o dia.
...
O roadmap para o suporte inicial a funções de janela está [nesta issue](https://github.com/ClickHouse/ClickHouse/issues/18097).

Todas as issues do GitHub relacionadas a funções de janela têm a tag [comp-window-functions](https://github.com/ClickHouse/ClickHouse/labels/comp-window-functions).
...
* Blog: [Funções de janela e de array para sequências de commits do Git](https://clickhouse.com/blog/clickhouse-window-array-functions-git-commits)
