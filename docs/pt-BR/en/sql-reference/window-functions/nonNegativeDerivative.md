...
O resultado é a taxa de variação da métrica por `INTERVAL`, com qualquer valor negativo truncado em `0`.
Isso é útil para métricas monotonicamente crescentes, como contadores, em que uma queda geralmente indica uma reinicialização e não uma taxa negativa real.
...
Para mais detalhes sobre a sintaxe de window functions, consulte: [Window Functions - Sintaxe](./index.md/#syntax).

**Argumentos**

- `metric_column` — A coluna cuja derivada é calculada. [(U)Int*](../data-types/int-uint.md) ou [Float*](../data-types/float.md).
- `timestamp_column` — A coluna utilizada para medir o tempo decorrido entre a linha atual e a linha anterior na ordem da janela. Ela não ordena as linhas; o `ORDER BY` da janela é quem faz isso, e normalmente deve usar essa mesma coluna. [DateTime](../data-types/datetime.md) ou [DateTime64](../data-types/datetime64.md).
...
Se o valor calculado for negativo, ele é truncado em `0`. O tipo de retorno é [Float64](../data-types/float.md).

**Exemplo**

O exemplo a seguir calcula a taxa de variação por segundo de uma leitura de sensor.
Observe que a terceira linha cai de `110` para `105`, portanto sua derivada é truncada em `0`.
