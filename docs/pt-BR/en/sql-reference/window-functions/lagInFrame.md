...
:::warning
O comportamento de `lagInFrame` difere do da função de janela SQL padrão `lag`.
...
* `default` — Valor a ser retornado se a linha calculada ultrapassar os limites do frame da janela. (Opcional - valor padrão do tipo da coluna quando omitido).
...
Este exemplo analisa dados históricos de uma ação específica e usa a função `lagInFrame` para calcular um delta diário e a variação percentual no preço de fechamento da ação.
