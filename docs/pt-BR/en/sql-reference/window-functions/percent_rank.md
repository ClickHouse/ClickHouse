...
retorna a classificação relativa (ou seja, o percentil) das linhas em uma partição de janela.
...
A definição padrão e obrigatória de `window frame` é `RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING`.
