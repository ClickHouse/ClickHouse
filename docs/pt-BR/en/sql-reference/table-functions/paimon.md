---
description: 'Fornece uma interface semelhante a uma tabela, somente leitura, para tabelas do Apache Paimon no
...
<div id="paimon-table-function">
  # Função de tabela paimon
</div>

<ExperimentalBadge />

Fornece uma interface semelhante a uma tabela somente leitura para tabelas do Apache [Paimon](https://paimon.apache.org/) no Amazon S3, Azure, HDFS ou armazenadas localmente.
...
Para `paimonS3`, é possível usar o parâmetro opcional `extra_credentials` para passar um `role_arn` para acesso baseado em roles no ClickHouse Cloud. Consulte [Secure S3](/pt-BR/cloud/data-sources/secure-s3) para conferir as etapas de configuração.

<div id="returned-value">
  ### Valor retornado
</div>

Uma tabela com a estrutura especificada para ler dados da tabela Paimon especificada.

<div id="defining-a-named-collection">
  ## Definindo uma named collection
</div>

Aqui está um exemplo de como configurar uma named collection para armazenar a URL e as credenciais:
...
* [função de tabela cluster do Paimon](/pt-BR/sql-reference/table-functions/paimonCluster.md)