---
description: 'Permite executar consultas `SELECT` em dados armazenados em um
  servidor MongoDB remoto.'
...
Você também pode se conectar via URI:
...
| `collection`  | Nome da coleção remota.                                                                                        |
| `structure`   | O esquema da tabela do ClickHouse retornada por esta função.                                                   |
| `oid_columns` | Lista de colunas separadas por vírgulas que devem ser tratadas como `oid` na cláusula WHERE. `_id` é o padrão. |
| :::           |                                                                                                                |
...
Um objeto de tabela com as mesmas colunas da tabela original do MongoDB.

<div id="examples">
  ## Exemplos
</div>

Suponha que haja uma coleção chamada `my_collection` definida em um banco de dados MongoDB chamado `test`, e que insiramos alguns documentos:
...
Vamos consultar a coleção com a função de tabela `mongodb`:
...
* [O motor de tabela `MongoDB`](/pt-BR/engines/table-engines/integrations/mongodb.md)
* [Usando o MongoDB como fonte de dicionário](../statements/create/dictionary/sources/mongodb.md)