...
Esta função pode ser usada apenas na consulta `INSERT SELECT` e apenas uma vez, mas, fora isso, se comporta como uma função de tabela comum
(por exemplo, pode ser usada em uma subconsulta etc.).

Os dados podem ser enviados da mesma forma que em uma consulta `INSERT` comum e passados em qualquer [formato](/pt-BR/sql-reference/formats)
disponível, que deve ser especificado no final da consulta (diferentemente de um `INSERT SELECT` comum).

O principal recurso desta função é que, quando o servidor recebe dados do cliente, ele os converte simultaneamente
...
* Suponha que a tabela `test` tenha a seguinte estrutura `(a String, b String)`
  ...
  dados de `data.csv` na tabela `test` com conversão simultânea é a seguinte:
...
* Se `data.csv` contiver dados com a mesma estrutura `test_structure` que a tabela `test`, então estas duas consultas serão equivalentes:
