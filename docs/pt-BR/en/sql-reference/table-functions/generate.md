---
description: 'Gera dados aleatórios com um schema definido. Permite preencher tabelas
  de teste com esses dados. Nem todos os tipos têm suporte.'
...
Gera dados aleatórios com um schema definido.
Permite preencher tabelas de teste com esses dados.
Nem todos os tipos têm suporte.
...
| Argumento           | Descrição                                                                                                                  |
| ------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| `name`              | Nome da coluna correspondente.                                                                                             |
| `TypeName`          | Tipo da coluna correspondente.                                                                                             |
| `random_seed`       | Defina manualmente a semente aleatória para produzir resultados estáveis. Se `NULL`, a semente será gerada aleatoriamente. |
| `max_string_length` | Comprimento máximo de todas as strings geradas. O padrão é `10`.                                                           |
| `max_array_length`  | Número máximo de elementos em todos os arrays ou maps gerados. O padrão é `10`.                                            |

<div id="returned_value">
  ## Valor retornado
</div>

Um objeto de tabela com o schema solicitado.
...
Em conjunto com [generateRandomStructure](../../sql-reference/functions/other-functions.md#generateRandomStructure):
...
Sem o argumento `structure` (neste caso, a estrutura é aleatória):
...
Com uma semente aleatória tanto para a estrutura aleatória quanto para os dados aleatórios:
...
:::note
`generateRandom(generateRandomStructure(), [random seed], max_string_length, max_array_length)` com um `max_array_length` grande o suficiente pode gerar uma saída realmente enorme devido à grande profundidade de aninhamento possível (até 16) de tipos complexos (`Array`, `Tuple`, `Map`, `Nested`).
:::
...
* Blog: [Como gerar dados aleatórios no ClickHouse](https://clickhouse.com/blog/generating-random-test-distribution-data-for-clickhouse)