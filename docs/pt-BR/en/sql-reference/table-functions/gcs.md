---
description: 'Fornece uma interface semelhante a uma tabela para `SELECT` e `INSERT` de dados no Google
...
Fornece uma interface semelhante a uma tabela para `SELECT` e `INSERT` de dados no [Google Cloud Storage](https://cloud.google.com/storage/). Requer o [papel do IAM `Storage Object User`](https://cloud.google.com/storage/docs/access-control/iam-roles).

Este é um alias da [função de tabela s3](../../sql-reference/table-functions/s3.md).

Se você tiver várias réplicas no seu cluster, poderá usar a função [s3Cluster](../../sql-reference/table-functions/s3Cluster.md) (que funciona com GCS) para paralelizar inserções.
...
| Argumento                  | Descrição                                                                                                                                                                                                          |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `url`                      | Caminho do bucket até o arquivo. Suporta os seguintes wildcards no modo somente leitura: `*`, `**`, `?`, `{abc,def}` e `{N..M}`, em que `N`, `M` — números, `'abc'`, `'def'` — strings.                            |
| `NOSIGN`                   | Se esta palavra-chave for fornecida no lugar das credenciais, nenhuma requisição será assinada.                                                                                                                    |
| `hmac_key` e `hmac_secret` | Chaves que especificam as credenciais a serem usadas com o endpoint informado. Opcional.                                                                                                                           |
...
:::note GCS
O caminho do GCS usa este formato, pois o endpoint da Google XML API é diferente do da JSON API:
...
Os argumentos também podem ser passados usando [coleções nomeadas](/pt-BR/operations/named-collections.md). Nesse caso, `url`, `format`, `structure` e `compression_method` funcionam da mesma forma, e há suporte a alguns parâmetros extras:

| Parâmetro                     | Descrição                                                                                                                                                                                                                              |
| ----------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `access_key_id`               | `hmac_key`, opcional.                                                                                                                                                                                                                  |
| `secret_access_key`           | `hmac_secret`, opcional.                                                                                                                                                                                                               |
| `filename`                    | Adicionado à URL, se especificado.                                                                                                                                                                                                     |
| `use_environment_credentials` | Ativado por padrão, permite passar parâmetros extras usando as variáveis de ambiente `AWS_CONTAINER_CREDENTIALS_RELATIVE_URI`, `AWS_CONTAINER_CREDENTIALS_FULL_URI`, `AWS_CONTAINER_AUTHORIZATION_TOKEN`, `AWS_EC2_METADATA_DISABLED`. |
...
Selecionando as duas primeiras linhas do arquivo no GCS `https://storage.googleapis.com/clickhouse_public_datasets/my-test-bucket-768/data.csv.gz`. O método de compressão é detectado automaticamente com base na extensão de arquivo `.gz`:
...
Suponha que haja vários arquivos com as seguintes URIs no GCS:
...
Insira dados no arquivo `test-data.csv.gz` a partir da tabela existente:
...
O trecho abaixo obtém dados, de forma recursiva, de todos os arquivos `test-data.csv.gz` em qualquer pasta dentro do diretório `my-test-bucket`:
...
Se você especificar a expressão `PARTITION BY` ao inserir dados na tabela `GCS`, será criado um arquivo separado para cada valor de partição. Dividir os dados em arquivos separados ajuda a melhorar a eficiência das operações de leitura.
...
2. O uso do ID da partição no nome de um bucket cria arquivos em buckets diferentes:
