...
<div id="supported-promql-features">
  ## Funcionalidades compatíveis do PromQL
</div>
...
Seletores instantâneos, seletores de intervalo, filtros de rótulo (`=`, `!=`, `=~`, `!~`), modificadores `offset`, modificadores de timestamp `@` e subconsultas.
...
**Observação**: `histogram_quantile` usa interpolação linear em buckets de histogramas clássicos (identificados pelo rótulo `le`). Histogramas nativos ainda não são suportados, e o argumento `phi` (nível de quantil) atualmente deve ser um escalar constante — expressões que variam a cada passo, como `histogram_quantile(time() / 1000, ...)`, são rejeitadas com o erro `NOT_IMPLEMENTED`.
