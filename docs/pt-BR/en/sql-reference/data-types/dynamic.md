---
description: 'Documentação para o tipo de dado Dynamic no ClickHouse, que pode armazenar
...
Esse tipo permite armazenar valores de qualquer tipo sem que seja necessário conhecer todos eles antecipadamente.
...
Onde `N` é um parâmetro opcional entre `0` e `254` que indica quantos tipos de dado diferentes podem ser armazenados como subcolunas separadas dentro de uma coluna do tipo `Dynamic` em um único bloco de dados armazenado separadamente (por exemplo, em uma única parte de dados de uma tabela MergeTree). Se esse limite for excedido, todos os valores com novos types serão armazenados juntos em uma estrutura de dados compartilhada especial em forma binária. O valor padrão de `max_types` é `32`.

<div id="creating-dynamic">
  ## Criando Dynamic
</div>

Usando o tipo `Dynamic` na definição da coluna da tabela:
...
O tipo `Dynamic` permite ler um único tipo aninhado de uma coluna `Dynamic` usando o nome do tipo como subcoluna.
Assim, se você tiver a coluna `d Dynamic`, poderá ler uma subcoluna de qualquer tipo válido `T` usando a sintaxe `d.T`.
Essa subcoluna terá o tipo `Nullable(T)` se `T` puder estar dentro de `Nullable` e, caso contrário, `T`. Essa subcoluna
...
Para saber qual variante está armazenada em cada linha, pode ser usada a função `dynamicType(dynamic_column)`. Ela retorna `String` com o nome do tipo de valor em cada linha (ou `'None'` se a linha for `NULL`).
...
<div id="converting-an-ordinary-column-to-a-dynamic-column">
  ### Convertendo uma coluna comum para uma coluna Dynamic
</div>
...
<div id="converting-a-variant-column-to-dynamic-column">
  ### Convertendo uma coluna Variant em coluna Dynamic
</div>
...
Se `K >= N`, durante a conversão os dados não mudam:
...
A função `isDynamicElementInSharedData` retorna `true` para as linhas armazenadas em uma estrutura especial de dado compartilhada dentro de `Dynamic` e, como podemos ver, a coluna resultante contém apenas 2 tipos que não são armazenados nessa estrutura de dado compartilhada.
...
<div id="reading-dynamic-type-from-the-data">
  ## Lendo o tipo Dynamic dos dados
</div>

Todos os formatos de texto (TSV, CSV, CustomSeparated, Values, JSONEachRow etc.) suportam a leitura do tipo `Dynamic`. Durante o parsing dos dados, o ClickHouse tenta inferir o tipo de cada valor e usá-lo durante a inserção na coluna `Dynamic`.
...
Se a função não puder ser executada em algum tipo dentro da coluna `Dynamic`, a exceção será lançada:
...
Podemos filtrar tipos desnecessários:
...
Ou extraia o tipo necessário como uma subcoluna:
...
A configuração `dynamic_throw_on_type_mismatch` controla o que acontece quando uma função é aplicada a uma coluna `Dynamic` e o tipo real armazenado em uma linha é incompatível com a função:
...
O resultado do operador `<` para valores `d1`, com tipo subjacente `T1`, e `d2`, com tipo subjacente `T2`, de um tipo `Dynamic` é definido da seguinte forma:
...
Por padrão, o tipo `Dynamic` não é permitido em chaves de `GROUP BY`/`ORDER BY`. Se quiser usá-lo, considere sua regra especial de comparação e habilite as configurações `allow_suspicious_types_in_group_by`/`allow_suspicious_types_in_order_by`.
...
**Observação:** valores do tipo Dynamic com tipos numéricos diferentes são considerados distintos e não são comparados entre si; em vez disso, os nomes dos tipos são comparados.
...
**Nota:** a regra de comparação descrita não é aplicada durante a execução de funções de comparação, como `<`/`>`/`=` e outras, devido ao [funcionamento especial](#using-dynamic-type-in-functions) das funções com o tipo `Dynamic`

<div id="reaching-the-limit-in-number-of-different-data-types-stored-inside-dynamic">
  ## Atingindo o limite do número de tipos de dados diferentes armazenados em Dynamic
</div>

O tipo de dado `Dynamic` pode armazenar apenas uma quantidade limitada de tipos de dados diferentes em subcolunas separadas. Por padrão, esse limite é 32, mas você pode alterá-lo na declaração do tipo usando a sintaxe `Dynamic(max_types=N)`, em que N fica entre 0 e 254 (devido a detalhes de implementação, é impossível ter mais de 254 tipos de dados diferentes que possam ser armazenados como subcolunas separadas em Dynamic).
Quando esse limite é atingido, todos os novos tipos de dados inseridos na coluna `Dynamic` serão inseridos em uma única estrutura de dados compartilhada que armazena valores com diferentes tipos de dados em forma binária.
...
Durante o parsing de valores `Dynamic` dos dados, quando o limite é atingido no bloco de dados atual, todos os novos valores serão inseridos na estrutura de dados compartilhada:
...
<div id="during-merges-of-data-parts-in-mergetree-table-engines">
  ### Durante merges de partes de dados em motores de tabela MergeTree
</div>

Durante o merge de várias partes de dados em uma tabela MergeTree, a coluna `Dynamic` na parte de dados resultante pode atingir o limite de diferentes tipos de dados que podem ser armazenados nela como subcolunas separadas e não conseguirá armazenar todos os tipos das partes de origem como subcolunas.
Nesse caso, o ClickHouse escolhe quais tipos permanecerão como subcolunas separadas após o merge e quais serão inseridos na estrutura de dados compartilhada. Na maioria dos casos, o ClickHouse tenta manter os tipos mais frequentes e armazenar os mais raros na estrutura de dados compartilhada, mas isso depende da implementação.

Vamos ver um exemplo desse tipo de merge. Primeiro, vamos criar uma tabela com uma coluna `Dynamic`, definir o limite de diferentes tipos de dados como `3` e inserir valores com `5` tipos diferentes:
...
No formato RowBinary, valores do tipo `Dynamic` são serializados no formato a seguir:
