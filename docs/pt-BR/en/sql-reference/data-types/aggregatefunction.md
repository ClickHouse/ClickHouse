...
Todas as [funções de agregação](/pt-BR/sql-reference/aggregate-functions) no ClickHouse têm
um estado intermediário específico da implementação que pode ser serializado em um
tipo de dado `AggregateFunction` e armazenado em uma tabela. Isso normalmente é feito por
meio de uma [visão materializada](../../sql-reference/statements/create/view.md).

Há dois [combinadores](/pt-BR/sql-reference/aggregate-functions/combinators)
de função de agregação comumente usados com o tipo `AggregateFunction`:

* O combinador de função de agregação [`-State`](/pt-BR/sql-reference/aggregate-functions/combinators#-state), que, quando adicionado ao nome de uma função de agregação,
  produz estados intermediários `AggregateFunction`.
* O combinador de função de agregação [`-Merge`](/pt-BR/sql-reference/aggregate-functions/combinators#-merge),
  que é usado para obter o resultado final de uma agregação
...
[`-State`](/pt-BR/sql-reference/aggregate-functions/combinators#-state).
...
* `state` - string binária com o estado serializado (o padrão).
  Se você gerar um dump de dados em, por exemplo, o formato `TabSeparated` com uma
  consulta `SELECT`, esse dump poderá ser carregado novamente usando a consulta `INSERT`.
* `value` - o formato esperará um único valor do argumento da função de agregação ou, no caso de múltiplos argumentos, uma tupla com eles; isso será desserializado para formar o estado correspondente
...
Ao selecionar dados da tabela `AggregatingMergeTree`, use a cláusula `GROUP BY`
e as mesmas funções de agregação usadas ao inserir os dados, mas com o
combinador [`-Merge`](/pt-BR/sql-reference/aggregate-functions/combinators#-merge).

Uma função de agregação com o combinador `-Merge` anexado recebe um conjunto de
...
Consulte a descrição do motor [AggregatingMergeTree](../../engines/table-engines/mergetree-family/aggregatingmergetree.md).

<div id="related-content">
  ## Conteúdo relacionado
</div>

* Blog: [Usando combinadores de agregação no ClickHouse](https://clickhouse.com/blog/aggregate-functions-combinators-in-clickhouse-for-arrays-maps-and-states)
* [MergeState](/pt-BR/sql-reference/aggregate-functions/combinators#-mergestate)
  combinador.
* [State](/pt-BR/sql-reference/aggregate-functions/combinators#-state) combinador.