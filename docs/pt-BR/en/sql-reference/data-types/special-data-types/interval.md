...
A família de tipos de dados que representa intervalos de data e hora. Os tipos resultantes do operador [INTERVAL](/pt-BR/sql-reference/operators#interval).
...
Você pode usar valores do tipo `Interval` em operações aritméticas com valores do tipo [Date](../../../sql-reference/data-types/date.md) e [DateTime](../../../sql-reference/data-types/datetime.md). Por exemplo, você pode adicionar 4 dias à data e hora atuais:
...
E, para comparar valores com intervalos diferentes:
...
Intervalos mistos, por exemplo, com várias horas e vários minutos, podem ser criados usando a sintaxe `INTERVAL 'value' <from_kind> TO <to_kind>`.
...
Os campos que não vêm primeiro são validados de acordo com o padrão SQL: `MONTH` 0-11, `HOUR` 0-23, `MINUTE` 0-59, `SECOND` 0-59.
...
Um sinal inicial opcional `+` ou `-` se aplica a todos os componentes:
...
* operador [INTERVAL](/pt-BR/sql-reference/operators#interval)
* funções de conversão de tipos [toInterval](/pt-BR/sql-reference/functions/type-conversion-functions#toIntervalYear)