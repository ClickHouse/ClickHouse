...
<Steps>
  <Step stepNumber={0}>
    ## Шаг 0: Настройка сервисного аккаунта
  ...
  <Step stepNumber={1}>
    ## Шаг 1: Задание переменных окружения (`00-env.sh`)
  ...
  <Step stepNumber={2}>
    ## Шаг 2: Создайте Artifact Registry (`01-artifact-registry.sh`)
  ...
  <Step stepNumber={3}>
    ## Шаг 3: Скопируйте образы контейнеров (`02-copy-images.sh`)
  ...
  <Step stepNumber={4}>
    ## Шаг 4: Настройка VPC и подсети (`03-vpc-subnet.sh`)

    Этот скрипт рассчитан на то, что у вас уже есть VPC с бастионом. Он создает или обновляет подсеть GKE с необходимыми вторичными диапазонами.

    **Важно: требования к границам CIDR** - блоки `/14`: третий октет должен делиться на 4 (например, `10.244.0.0`, НЕ `10.245.0.0`) - блоки `/20`: четвертый октет должен быть равен 0, а третий — кратен 16 (например, `10.252.0.0`)
  ...
  <Step stepNumber={5}>
    ## Шаг 5: Создайте Cloud NAT (`04-cloud-nat.sh`)
  ...
  <Step stepNumber={6}>
    ## Шаг 6: Создайте сервисный аккаунт GCP (`05-service-account.sh`)
  ...
  <Step stepNumber={7}>
    ## Шаг 7: Создание бакета GCS (`06-gcs-bucket.sh`)
  ...
  <Step stepNumber={8}>
    ## Шаг 8: Создание кластера GKE (`07-gke-cluster.sh`)
  ...
  <Step stepNumber={9}>
    ## Шаг 9: Создайте пулы узлов (`08-node-pools.sh`)
  ...
  <Step stepNumber={10}>
    ## Шаг 10: Разверните ДемонСет Local SSD (`09-local-ssd-daemonset.sh`)
  ...
  <Step stepNumber={11}>
    ## Шаг 11: Добавьте топологические метки (`10-topology-labels.sh`)
  ...
  <Step stepNumber={12}>
    ## Шаг 12: Установка необходимых компонентов Kubernetes
  ...
  <Step stepNumber={13}>
    ## Шаг 13: Настройка FIPS (при необходимости)
  ...
  <Step stepNumber={14}>
    ## Шаг 14: Установите оператор (`16-install-operator.sh`)
  ...
  <Step stepNumber={15}>
    ## Шаг 15: Развёртывание кластера ClickHouse (`17-install-cluster.sh`)
  ...
  <Step stepNumber={16}>
    ## Шаг 16: Привязка Workload Identity (`18-bind-helm-workload-identity.sh`)
