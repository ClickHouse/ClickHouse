---
noindex: true
title: "Развертывание ClickHouse в GCP GKE"
sidebarTitle: "Развертывание в GCP"
description: "Это руководство поможет вам развернуть ClickHouse Private на платформе Google Cloud Platform с помощью Google Kubernetes Engine (GKE)."
---

Это руководство поможет вам развернуть ClickHouse Private на платформе Google Cloud Platform с помощью Google Kubernetes Engine (GKE). В нем рассматриваются как стандартные развертывания, так и развертывания, соответствующие требованиям FIPS 140-2. Шаги, относящиеся к FIPS, по всему тексту помечены как **(FIPS)** -- чтобы включить режим FIPS, установите `ENABLE_FIPS=true` в `00-env.sh`.

Руководство состоит из пронумерованных скриптов (от `00-env.sh` до `19-verify`), которые нужно запускать последовательно. Каждый скрипт самодостаточен и подключает файл окружения.
...
* Установленный и аутентифицированный CLI `gcloud`
* Установленный `kubectl`
* Установленный `helm` v3.x
* Установленный `skopeo` (для копирования образов контейнеров)
* Учетные данные AWS с доступом на чтение к ClickHouse ECR (для копирования образов)
* **Бастион-хост** в VPC для доступа к приватному кластеру
* **Сервисный аккаунт Deployer** с соответствующими разрешениями (см. ниже)
* **(FIPS)** Доступ к образу контейнера Red Hat UBI8

***

<Steps>
  <Step stepNumber={0} title="Шаг 0: Настройка сервисного аккаунта" id="step-0-service-account-setup">
    ...
    3. Введите имя `clickhouse-deployer`, ID `clickhouse-deployer`
    4. Добавьте следующие роли:
    ...
    | `roles/storage.admin`                   | Создание и управление GCS-бакетами                           |
    ...
    1. Выдайте разрешение Token Creator сервисному аккаунту Compute Engine бастиона.
    ...
    ### Настройте имперсонацию на бастионе
    ...
    Чтобы позже сбросить имперсонацию:
  ...
  <Step stepNumber={1} title={<>Шаг 1: Задание переменных окружения (<code>00-env.sh</code>)</>} id="step-1-set-environment-variables-00-envsh">
    Создайте этот файл и подключите его перед запуском других скриптов. Укажите актуальные значения для вашей среды.
    ...
    **Важно:** Выполните source для этого файла перед запуском других скриптов:
  ...
  <Step stepNumber={2} title={<>Шаг 2: Создайте Artifact Registry (<code>01-artifact-registry.sh</code>)</>} id="step-2-create-artifact-registry-01-artifact-registrysh">
    ```bash expandable
    #!/bin/bash
    set -e
    source ./00-env.sh
  ...
  <Step stepNumber={3} title={<>Шаг 3: Скопируйте образы контейнеров (<code>02-copy-images.sh</code>)</>} id="step-3-copy-container-images-02-copy-imagessh">
    Для сохранения всех архитектур требуется [skopeo](https://github.com/containers/skopeo).
  ...
  <Step stepNumber={4} title={<>Шаг 4: Настройка VPC и подсети (<code>03-vpc-subnet.sh</code>)</>} id="step-4-configure-vpc-and-subnet-03-vpc-subnetsh">
    Этот скрипт предполагает, что у вас уже есть существующая VPC с хостом-бастионом. Он создает или обновляет подсеть GKE с необходимыми вторичными диапазонами.

    **Важно: требования к границам CIDR** - блоки `/14`: третий октет должен делиться на 4 (например, `10.244.0.0`, НЕ `10.245.0.0`) - блоки `/20`: четвертый октет должен быть равен 0, а третий — быть кратным 16 (например, `10.252.0.0`)
  ...
  <Step stepNumber={5} title={<>Шаг 5: Создайте Cloud NAT (<code>04-cloud-nat.sh</code>)</>} id="step-5-create-cloud-nat-04-cloud-natsh">
    Для приватных узлов GKE требуется Cloud NAT для исходящего доступа в интернет.
  ...
  <Step stepNumber={6} title={<>Шаг 6: Создайте сервисный аккаунт GCP (<code>05-service-account.sh</code>)</>} id="step-6-create-gcp-service-account-05-service-accountsh">
    ```bash
    #!/bin/bash
    set -e
    source ./00-env.sh
  ...
  <Step stepNumber={7} title={<>Шаг 7: Создание бакета GCS (<code>06-gcs-bucket.sh</code>)</>} id="step-7-create-gcs-bucket-06-gcs-bucketsh">
    ```bash expandable
    #!/bin/bash
    set -e
    source ./00-env.sh
  ...
  <Step stepNumber={8} title={<>Шаг 8: Создание кластера GKE (<code>07-gke-cluster.sh</code>)</>} id="step-8-create-gke-cluster-07-gke-clustersh">
    Создаёт **полностью приватный кластер GKE** — у воркер-узлов есть только внутренние IP-адреса, у control plane нет публичной конечной точки, а весь доступ должен осуществляться из VPC через бастион.
    ...
    **Настройки приватного кластера:**
    ...
    | `--enable-private-endpoint`           | Обязательно           | У control plane нет публичного IP-адреса (FedRAMP)      |
    | `--master-ipv4-cidr`                  | `172.16.0.0/28`       | Диапазон IP-адресов control plane                       |
    | `--enable-master-authorized-networks` | Обязательно           | Включает сетевые ограничения                            |
    | `--master-authorized-networks`        | Подсети GKE + Bastion | CIDR-диапазоны, которым разрешён доступ к control plane |

    ***
  </Step>

  <Step stepNumber={9} title={<>Шаг 9: Создайте пулы узлов (<code>08-node-pools.sh</code>)</>} id="step-9-create-node-pools-08-node-poolssh">
  ...
  <Step stepNumber={10} title={<>Шаг 10: Разверните ДемонСет Local SSD (<code>09-local-ssd-daemonset.sh</code>)</>} id="step-10-deploy-local-ssd-daemonset-09-local-ssd-daemonsetsh">
    В GKE Container-Optimized OS корневая файловая система доступна **только для чтения**, поэтому создать `/nvme` нельзя. ДемонСет форматирует и монтирует локальный SSD в `/mnt/disks/ssd0`.

    **Важно:** Разверните его **до** кластера ClickHouse, чтобы избежать состояния гонки.
  ...
  <Step stepNumber={11} title={<>Шаг 11: Добавьте топологические метки (<code>10-topology-labels.sh</code>)</>} id="step-11-add-topology-labels-10-topology-labelssh">
    ```bash
    #!/bin/bash
    set -e
    source ./00-env.sh
  ...
  <Step stepNumber={12} title="Шаг 12: Установка необходимых компонентов Kubernetes" id="step-12-install-kubernetes-prerequisites">
    ### Установка VolumeSnapshot CRDs (`11-volumesnapshot-crds.sh`)
    ...
    ### Настройка Workload Identity (`12-workload-identity.sh`)
    ...
    **Соответствие Workload Identity:**
    ...
    | K8s ServiceAccount (созданный через helm) | `ch-$CLUSTER_NAME-sa` в пространстве имен `ns-$CLUSTER_NAME` |
    | Аннотация K8s ServiceAccount              | `iam.gke.io/gcp-service-account=<GSA_EMAIL>`                 |
    | Участник привязки IAM                     | `serviceAccount:$PROJECT.svc.id.goog[<namespace>/<ksa>]`     |

    ### Установка StorageClass (`13-storageclass.sh`) -- Необязательно

    **Note:** GKE использует `topology.gke.io/zone` вместо `topology.kubernetes.io/zone`.
  ...
  <Step stepNumber={13} title="Шаг 13: Настройка FIPS (при необходимости)" id="step-13-fips-steps-conditional">
    Пропустите этот раздел, если `ENABLE_FIPS=false`.

    ### Создание сертификатов FIPS (`14-create-fips-certs.sh`)

    Сертификаты, соответствующие требованиям FIPS, необходимо создавать с использованием одобренных FIPS алгоритмов (RSA не менее 3072 бит, SHA-256 или выше) в среде с включенной поддержкой FIPS. Скрипт запускает под Red Hat UBI8 в кластере GKE с включенной криптографической политикой FIPS.

    **Требуемые типы сертификатов:**

    | Сертификат | Файлы                                    | Требуемые SAN                                                                     |
    | ---------- | ---------------------------------------- | --------------------------------------------------------------------------------- |
    | CA         | `ca/ca.crt`, `ca/ca.key`                 | Неприменимо                                                                       |
    | Сервер     | `server/server.crt`, `server/server.key` | `*.c-${CLUSTER_NAME}-server-headless.ns-${CLUSTER_NAME}.svc.${KUBERNETES_DOMAIN}` |
    | Клиент     | `client/client.crt`, `client/client.key` | Не требуются                                                                      |
    | Keeper     | `keeper/keeper.crt`, `keeper/keeper.key` | `*.c-${CLUSTER_NAME}-keeper-headless.ns-${CLUSTER_NAME}.svc.${KUBERNETES_DOMAIN}` |

    Полную процедуру создания сертификатов см. в [how-to/configure-fips-certificates.md](/ru/products/clickhouse-private/guides/generate-fips-compliant-certificates-for-clickhouse).

    ### Создание секретов сертификатов (`15-create-cert-secrets.sh`)
    ...
    **Структура секрета:**
    ...
    | ------------------------------------ | ---------------------------------------------------------------- | --------------------------------------------- |
    | `${CLUSTER_NAME}-server-cert-secret` | `ca.crt`, `server.crt`, `server.key`, `client.crt`, `client.key` | TLS сервера и исходящие клиентские соединения |
    | `${CLUSTER_NAME}-keeper-cert-secret` | `ca.crt`, `keeper.crt`, `keeper.key`                             | TLS для Keeper                                |

    ***
  </Step>

  <Step stepNumber={14} title={<>Шаг 14: Установите оператор (<code>16-install-operator.sh</code>)</>} id="step-14-install-operator-16-install-operatorsh">
    ```bash expandable
    #!/bin/bash
    set -e
    source ./00-env.sh
  ...
  <Step stepNumber={15} title={<>Шаг 15: Развёртывание кластера ClickHouse (<code>17-install-cluster.sh</code>)</>} id="step-15-deploy-clickhouse-cluster-17-install-clustersh">
    ```bash expandable
    #!/bin/bash
    set -e
    source ./00-env.sh
    ...
    * `storageClass.create=false` -- Не создавать StorageClass AWS
    * `storageClass.name="premium-rwo"` -- Использовать встроенный SSD StorageClass GCP
    ...
    * `keeper.storage.s3.useEnvironmentCredentials=true` -- **Также необходимо задать для keeper**

    ***
  </Step>

  <Step stepNumber={16} title={<>Шаг 16: Привязка Workload Identity (<code>18-bind-helm-workload-identity.sh</code>)</>} id="step-16-bind-workload-identity-18-bind-helm-workload-identitysh">
    Helm-чарт создаёт собственный service account (`ch-${CLUSTER_NAME}-sa`). Необходимо привязать Workload Identity к этому SA для доступа к GCS.

    **Важно:** поды Keeper и server, созданные на шаге 15, запускаются до создания этой привязки, поэтому будут выдавать ошибки GCS 403. Этот скрипт перезапускает поды после привязки.
  ...
  <Step stepNumber={17} title="Шаг 17: Проверка установки" id="step-17-verify-installation">
    ...
    При `openSSL.required=true` ClickHouse принимает соединения только через защищённые порты. Используйте порт **9440** (нативный TLS):
    ...
    Для продакшна с сертификатами, подписанными CA:
    ...
    **Важно (FIPS):** Порты 9000 (native без шифрования) и 8123 (HTTP без шифрования) будут отклонять подключения, если `openSSL.required=true`. Вместо них используйте порт 9440 (native TLS) или 8443 (HTTPS).
...
| Компонент AWS    | Эквивалент GCP             | Ключевые различия                                         |
| ---------------- | -------------------------- | --------------------------------------------------------- |
...
| Роль IAM (IRSA)  | Workload Identity          | Добавьте к KSA аннотацию `iam.gke.io/gcp-service-account` |
| HMAC Keys        | Workload Identity          | Используйте `useEnvironmentCredentials=true`              |
...
| NLB              | Балансировщик нагрузки GCP | Используйте аннотацию `type: LoadBalancer`                |