...
ClickHouse Cloud теперь предлагает ClickPipes для миграции внешней базы данных PostgreSQL в сервис ClickHouse Managed Postgres. Эта встроенная интеграция упрощает подключение к исходной базе данных, экспорт схемы, её импорт в ClickHouse Managed Postgres и настройку непрерывной репликации.
...
<Note>
  Если во время миграции возникнут проблемы, ознакомьтесь с [FAQ по миграциям ClickHouse Managed Postgres](/ru/products/managed-postgres/migrations/faq), где описаны распространённые ошибки и способы их устранения.
</Note>

<Steps>
  <Step title="Подключитесь к исходной базе данных" id="step-1-connect">
    Откройте [консоль ClickHouse Cloud](https://clickhouse.cloud) и выберите свой сервис ClickHouse Managed Postgres.

    <Image img="/images/managed-postgres/pgpg/servicecard.webp" alt="Карточка сервиса ClickHouse Managed Postgres в списке сервисов ClickHouse Cloud" size="lg" border />

    На левой боковой панели нажмите **источники данных**.

    <Image img="/images/managed-postgres/pgpg/overview.webp" alt="Пункт источники данных на боковой панели сервиса ClickHouse Managed Postgres" size="lg" border />

    Нажмите **Start import**.

    <Image img="/images/managed-postgres/pgpg/startimport.webp" alt="Страница источников данных с кнопкой Start import" size="lg" border />
  ...
  <Step title="Импортируйте схему в свой сервис ClickHouse Managed Postgres" id="step-3-import-schema">
    Выберите базу данных назначения в раскрывающемся списке или нажмите **Create a new database**, чтобы создать новую.

    Мастер покажет команду `psql` для применения дампа схемы к вашему сервису ClickHouse Managed Postgres. Выполните её в терминале:
...
**Переключите трафик приложения.** Направьте операции чтения и записи на сервис ClickHouse Managed Postgres и отслеживайте ошибки, нарушения ограничений и состояние репликации.
...
* [Сведения о подключении к ClickHouse Managed Postgres](/ru/products/managed-postgres/connection)
