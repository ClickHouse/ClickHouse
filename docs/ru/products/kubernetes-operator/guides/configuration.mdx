...
Оператор предоставляет фиксированный набор портов на каждом поде ClickHouse и в его публичном headless Service: `8123` HTTP, `9000` native, `9009` interserver, `9001`/`9002` management, `9363` метрики Prometheus, а также варианты с TLS `8443`/`9440`, если TLS включен. Порты `interserver` и management дополнительно открыты через внутренние Service каждой реплики, чтобы реплики и оператор могли обмениваться данными до того, как реплика станет готовой. Чтобы ClickHouse прослушивал дополнительные протоколы — MySQL, PostgreSQL, gRPC — или любой пользовательский порт, объявите их в `spec.additionalPorts`:
...
Каждый под реплики несёт readiness gate `clickhouse.com/ReplicaInitialized`, поэтому новая реплика публикуется через публичный headless Service только после того, как оператор завершит её инициализацию: база данных `default` переводится на движок Replicated и синхронизируется схема. До этого момента оператор и другие реплики обращаются к ней через её внутренний Service. При `enableDatabaseSync: false` оператор помечает реплики инициализированными сразу, поэтому готовность определяется только пробами контейнера.

Оператор никогда не удаляет заполненную non-Replicated базу данных `default`. Такая реплика всё равно начинает обслуживать клиентский трафик после подготовки схемы, но кластер сообщает `SchemaInSync=False` с причиной `DefaultDatabaseNotReplicated`, пока вы самостоятельно не решите вопрос с этой базой.

Прежде чем удалить реплику при уменьшении масштаба, оператор сначала снимает с неё клиентский трафик, ждёт, пока она перестанет публиковаться, реплицирует её оставшиеся данные на выжившие реплики и только затем удаляет её.
