---
title: 'Поддерживаемые облачные регионы'
sidebarTitle: 'Поддерживаемые облачные регионы'
keywords: ['aws', 'gcp', 'google cloud', 'azure', 'облако', 'регионы']
description: 'Поддерживаемые регионы ClickHouse Cloud'
slug: /cloud/reference/supported-regions
doc_type: 'reference'
---

import { EnterprisePlanFeatureBadge } from "/snippets/ru/components/EnterprisePlanFeatureBadge/EnterprisePlanFeatureBadge.jsx";

<div id="supported-regions">
  ## Поддерживаемые регионы
</div>

В следующих таблицах перечислены поддерживаемые регионы для каждого облачного провайдера. Значки указывают на доступность Частного региона, HIPAA и PCI. Регионы без значков пока не поддерживают ни одну из этих возможностей.

<Tabs>
  <Tab title="AWS" id="aws-regions" icon="aws">
    | Регион           | Доступность                                                                                              |
    | ---------------- | -------------------------------------------------------------------------------------------------------- |
    | `af-south-1`     | <Badge color="purple">частный</Badge> <Badge color="green">HIPAA</Badge> <Badge color="blue">PCI</Badge> |
    | `ap-east-1`      | <Badge color="purple">частный</Badge>                                                                    |
    | `ap-east-2`      | <Badge color="purple">частный</Badge>                                                                    |
    | `ap-northeast-1` | —                                                                                                        |
    | `ap-northeast-2` | —                                                                                                        |
    | `ap-south-1`     | <Badge color="green">HIPAA</Badge> <Badge color="blue">PCI</Badge>                                       |
    | `ap-southeast-1` | —                                                                                                        |
    | `ap-southeast-2` | —                                                                                                        |
    | `ap-southeast-3` | <Badge color="purple">частный</Badge>                                                                    |
    | `ap-southeast-5` | <Badge color="purple">частный</Badge>                                                                    |
    | `ca-central-1`   | <Badge color="green">HIPAA</Badge> <Badge color="blue">PCI</Badge>                                       |
    | `eu-central-1`   | <Badge color="green">HIPAA</Badge> <Badge color="blue">PCI</Badge>                                       |
    | `eu-north-1`     | <Badge color="purple">частный</Badge> <Badge color="green">HIPAA</Badge> <Badge color="blue">PCI</Badge> |
    | `eu-west-1`      | <Badge color="green">HIPAA</Badge> <Badge color="blue">PCI</Badge>                                       |
    | `eu-west-2`      | <Badge color="green">HIPAA</Badge> <Badge color="blue">PCI</Badge>                                       |
    | `il-central-1`   | —                                                                                                        |
    | `me-central-1`   | —                                                                                                        |
    | `mx-central-1`   | <Badge color="purple">частный</Badge> <Badge color="green">HIPAA</Badge> <Badge color="blue">PCI</Badge> |
    | `sa-east-1`      | <Badge color="purple">частный</Badge> <Badge color="green">HIPAA</Badge> <Badge color="blue">PCI</Badge> |
    | `us-east-1`      | <Badge color="green">HIPAA</Badge> <Badge color="blue">PCI</Badge>                                       |
    | `us-east-2`      | <Badge color="green">HIPAA</Badge> <Badge color="blue">PCI</Badge>                                       |
    | `us-west-2`      | <Badge color="green">HIPAA</Badge> <Badge color="blue">PCI</Badge>                                       |
  </Tab>

  <Tab title="Google Cloud" id="google-cloud-regions" icon="/images/icons/cloud-providers/google-cloud-color.svg">
    | Регион                    | Доступность                                                        |
    | ------------------------- | ------------------------------------------------------------------ |
    | `asia-northeast1`         | —                                                                  |
    | `asia-southeast1`         | —                                                                  |
    | `australia-southeast1`    | <Badge color="purple">частный</Badge>                              |
    | `europe-west2`            | —                                                                  |
    | `europe-west3`            | <Badge color="purple">частный</Badge>                              |
    | `europe-west4`            | <Badge color="green">HIPAA</Badge> <Badge color="blue">PCI</Badge> |
    | `europe-west6`            | <Badge color="purple">частный</Badge>                              |
    | `northamerica-northeast1` | <Badge color="purple">частный</Badge>                              |
    | `us-central1`             | <Badge color="green">HIPAA</Badge> <Badge color="blue">PCI</Badge> |
    | `us-east1`                | <Badge color="green">HIPAA</Badge> <Badge color="blue">PCI</Badge> |
    | `us-west1`                | <Badge color="purple">частный</Badge>                              |
  </Tab>

  <Tab title="Azure" id="azure-regions" icon="/images/icons/cloud-providers/azure.svg">
    | Регион                 | Доступность                           |
    | ---------------------- | ------------------------------------- |
    | `Australia East`       | <Badge color="purple">частный</Badge> |
    | `East US 2`            | —                                     |
    | `Germany West Central` | —                                     |
    | `Japan East`           | <Badge color="purple">частный</Badge> |
    | `UAE North`            | <Badge color="purple">частный</Badge> |
    | `West US 3`            | —                                     |
  </Tab>
</Tabs>

<Note>
  Нужно развернуть сервис в регионе, которого пока нет в списке? [Отправьте запрос](https://clickhouse.com/pricing?modal=open).
</Note>

<div id="private-regions">
  ## Частные регионы
</div>

<EnterprisePlanFeatureBadge feature="Возможность частных регионов" />

Мы предлагаем Частные регионы для сервисов уровня Enterprise. Чтобы запросить Частный регион, [свяжитесь с нами](https://clickhouse.com/company/contact).

Ключевые особенности частных регионов:

* Сервисы не поддерживают автомасштабирование; однако поддерживается ручное вертикальное и горизонтальное масштабирование.
* Сервисы нельзя перевести в неактивное состояние.

Для соответствия требованиям HIPAA могут предъявляться дополнительные требования, включая подписание соглашения Business Associate Agreement (BAA). HIPAA в настоящее время доступна только для сервисов уровня Enterprise.

<div id="hipaa-compliant-regions">
  ## Регионы, соответствующие требованиям HIPAA
</div>

<EnterprisePlanFeatureBadge feature="HIPAA" support="true" />

Чтобы развернуть сервисы в регионах, отмеченных в таблице как соответствующие требованиям HIPAA, клиенты должны подписать BAA и запросить онбординг через отдел продаж или службу поддержки.

<div id="pci-compliant-regions">
  ## Регионы, соответствующие требованиям PCI
</div>

<EnterprisePlanFeatureBadge feature="PCI" support="true" />

Чтобы развернуть сервисы в отмеченных в таблице регионах, соответствующих требованиям PCI, клиентам необходимо запросить онбординг через отдел продаж или службу поддержки.