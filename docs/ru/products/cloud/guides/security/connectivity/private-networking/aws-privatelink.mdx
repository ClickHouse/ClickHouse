...
Предположим, вы хотите использовать табличные функции [MySQL](/ru/reference/functions/table-functions/mysql) или [PostgreSQL](/ru/reference/functions/table-functions/postgresql) в ClickHouse Cloud и подключиться к базе данных, размещённой в VPC Amazon Web Services (AWS). Для безопасной организации такого подключения AWS PrivateLink использовать нельзя. PrivateLink — это одностороннее, однонаправленное соединение. Он позволяет вашей внутренней сети или Amazon VPC безопасно подключаться к ClickHouse Cloud, но не позволяет ClickHouse Cloud подключаться к вашей внутренней сети.
...
Для этого настройте AWS Security Groups так, чтобы разрешить подключения из ClickHouse Cloud к вашему внутреннему или частному сервису базы данных. См. [IP-адреса egress по умолчанию для регионов ClickHouse Cloud](/ru/products/cloud/guides/data-sources/cloud-endpoints-api), а также [доступные статические IP-адреса](https://api.clickhouse.cloud/static-ips.json).