---
sidebar_title: 'Настройка эндпоинтов Query API'
slug: /cloud/get-started/query-endpoints
description: 'Легко создавайте конечные точки REST API из сохранённых запросов'
keywords: ['api', 'эндпоинты Query API', 'конечные точки запросов', 'query rest api']
...
Возможность **эндпоинты Query API** позволяет создавать API-конечные точки напрямую из любого сохранённого SQL-запроса в консоли ClickHouse Cloud. Вы сможете обращаться к API-конечным точкам по HTTP, чтобы выполнять сохранённые запросы без необходимости подключаться к своему сервису ClickHouse Cloud через нативный драйвер.
...
<Steps>
  <Step title="Создание сохраненного запроса" id="creating-a-saved-query">
  ...
  <Step title="Настройка эндпоинта Query API" id="configuring-the-query-api-endpoint">
    Эндпоинты Query API можно настраивать прямо из представления запроса: нажмите кнопку **Share** и выберите `API Endpoint`.
    Затем вам будет предложено указать, какие ключ API смогут обращаться к этому эндпоинту Query API:
    ...
    * Указать разрешенные домены CORS
    ...
    После выбора этих параметров эндпоинт Query API будет создан автоматически.
  ...
  <Step title="Параметры Query API" id="query-api-parameters">
  ...
  <Step title="Тестирование и мониторинг" id="testing-and-monitoring">
    После создания эндпоинта Query API вы можете проверить его работу с помощью `curl` или любого другого HTTP-клиента:
