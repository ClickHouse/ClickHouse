...
## `ALTER` и видимость мутаций {#alter-and-mutation-visibility}

`SharedMergeTree` координирует изменения метаданных через ClickHouse Keeper. Реплики получают изменённые метаданные асинхронно, поэтому изменение метаданных может стать видимым не на всех репликах сразу. Операции чтения в том же сеансе на том же узле уже используют актуальные метаданные. Если чтение нужно выполнить на другой реплике сразу после изменения метаданных, используйте `SYSTEM SYNC REPLICA LIGHTWEIGHT`, чтобы эта реплика получила метаданные из ClickHouse Keeper.

Операции `ALTER`, порождающие мутации, выполняют фоновую работу: создают новые части в общем хранилище и координируют их видимость через ClickHouse Keeper. В отличие от `ReplicatedMergeTree`, здесь не происходит перезаписи или передачи данных на каждой реплике по отдельности.

В ClickHouse Cloud параметр [`alter_sync`](/ru/reference/settings/session-settings/alter#alter_sync) по умолчанию равен `0`; задайте его, если клиент должен дожидаться завершения операций с метаданными или перезаписи данных при `MODIFY COLUMN`, которая затрагивает не только метаданные. Для операций `UPDATE`, `DELETE` и `MATERIALIZE ...` управляйте ожиданием с помощью [`mutations_sync`](/ru/reference/settings/session-settings/mutations#mutations_sync), а ход выполнения отслеживайте в [`system.mutations`](/ru/reference/system-tables/mutations).