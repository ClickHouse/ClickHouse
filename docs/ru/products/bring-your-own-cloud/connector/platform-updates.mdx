...
В управляемом кластере работает небольшой платформенный слой, от которого зависят сервисы ClickHouse. ClickHouse Cloud предлагает обновления для этого слоя, однако исполнитель ничего не применяет, пока вы не одобрите каждое из них своими учётными данными кластера. На этой странице описано, что такое обновление платформы, как одобрить его на ВМ или в установке Kubernetes, как долго действует одобрение и какие привилегии оно даёт.

<Note>
  В публичной версии CLI `v0.16.0` отсутствует команда `platform`, включая `platform approve` и `platform reset`. Ни описанный ниже сценарий для Linux VM, ни сценарий для Kubernetes в этом релизе недоступны. Кроме того, для одобрения в Kubernetes требуется поддержка token bundle на основе Secret (`--secret-namespace`) в CLI и в чарте исполнителя. Прежде чем использовать любой из этих сценариев, уточните [доступность managed-режима](/ru/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) у команды сопровождения аккаунта ClickHouse.
</Note>

## Что такое обновление платформы {#what-a-platform-update-is}

Платформенный слой состоит из трех компонентов: контроллер снимков, ClickHouse operator и коллекторы мониторинга. Устанавливаются они именно в этом порядке, поскольку каждому следующему нужны определения пользовательских ресурсов предыдущего. Рядом с ними существует класс хранилища с именем `gp3-encrypted` (зашифрованные тома gp3), который используют сервисы.

Bundle платформы — это готовый манифест, который ClickHouse Cloud формирует для вашей среды. Он фиксирует версии chart и образов этих компонентов и указывает registry, из которого они загружаются. Каждый bundle идентифицируется по sha256 этого манифеста. ClickHouse Cloud отправляет bundle исполнителю по своему командному каналу, а исполнитель подготавливает его к вашему одобрению.

Применение bundle делится на две части:
...
* **Часть с рабочими нагрузками** — это то, что запускается: Deployments, Services, ConfigMaps, Secrets, Jobs и PodDisruptionBudgets. Исполнитель применяет ее от выделенной identity `pcm-platform`, которая может записывать только ресурсы этих видов и ничего больше, и только пока действителен кратковременный token, выпущенный при вашем одобрении.
...
Для обеих описанных ниже процедур одобрения требуется [сборка с поддержкой платформы](/ru/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability); в публичной версии `v0.16.0` команда `platform` отсутствует. Для одобрения через Kubernetes дополнительно нужна поддержка доставки Secret со стороны CLI и чарта.

Способ authentication в registry зависит от режима распространения образов, зарегистрированного для вашего окружения:

* **Прямой доступ к registry ClickHouse.** Выполняйте одобрение на ВМ EC2 коннектора. И команда `approve`, и платформенная синхронизация исполнителя используют учётные данные из instance metadata service EC2, чтобы assume роль ECR puller вашего окружения с доступом только для чтения. Эта роль используется для входа в registry чартов; исполнитель также применяет её для проверки наличия платформенных образов. Профили AWS, учётные данные из переменных окружения или учётные данные SSO на workstation не заменяют instance profile. Отдельные учётные данные администратора кластера для применения части с разрешениями по-прежнему берутся из вашего kubeconfig.
...
<Steps>
  ...
  <Step title="Выполните approve" id="run-approve">
    `approve` без флагов читает подготовленный bundle и вычисляет его hash, поэтому вы одобряете ровно то, что получил executor. Команда рендерит каждый chart из bundle точно так же, как это сделает executor, и применяет часть с разрешениями в выбранном вами kube-контексте. При необходимости она создаёт identity `pcm-platform` и выпускает для неё token. Никаких запросов к вашей connector endpoint при этом не выполняется.
...
При одобрении выпускается token для identity `pcm-platform`, действительный по умолчанию 2 часа (изменяется параметром `--ttl`). Исполнитель никогда не продлевает этот token. После истечения срока действия исполнитель больше не может обращаться к платформенному слою и снова отклоняет очередную синхронизацию платформы, выдавая сообщение о необходимости одобрения. Это предусмотренное поведение, а не сбой, и оно не зависит от наличия соединения: одобрение, выданное при отключённом connector, всё равно истекает по собственному таймеру.
...
Вы применяете часть с разрешениями, поэтому она наделяется вашими полномочиями. `approve` подписывает её значением sha256 переданного bundle, и исполнитель, прежде чем что-либо менять, проверяет, что все объекты разрешений из полученного им bundle присутствуют и одобрены. Сам исполнитель ничего из неё не применяет.
...
В публичной версии CLI `v0.16.0` команда `platform reset` отсутствует; прежде чем использовать эту процедуру, проверьте [доступность](/ru/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability).
