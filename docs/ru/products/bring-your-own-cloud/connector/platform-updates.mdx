...
В управляемом кластере работает небольшой платформенный слой, от которого зависят сервисы ClickHouse. ClickHouse Cloud предлагает обновления для этого слоя, однако исполнитель ничего не применяет, пока вы не подтвердите каждое из них своими учётными данными кластера. На этой странице описано, что такое обновление платформы, как подтвердить его на ВМ или в установке Kubernetes, как долго действует одобрение и какие привилегии оно даёт.
...
<Steps>
  ...
  <Step title="Выполните approve" id="run-approve">
    ...
    Для `approve` нужен kube-контекст с правами cluster-admin на управляемом кластере (`--context <name>`, если в вашем kubeconfig несколько контекстов). Если chart находится в Amazon ECR, потребуются также AWS credentials с правом на чтение. `--dry-run` рендерит и выводит часть с разрешениями, ничего не применяя и не выпуская.
