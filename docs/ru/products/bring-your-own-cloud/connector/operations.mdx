...
В Kubernetes `init` заранее создает обновленные Secret-ы `clicklink-hmac` и `clicklink-mtls`; chart их не формирует. Поэтому annotation с checksum в chart их не охватывает, и запущенные поды продолжают использовать те учетные данные, с которыми были запущены. После ротации перезапустите каждый component:
