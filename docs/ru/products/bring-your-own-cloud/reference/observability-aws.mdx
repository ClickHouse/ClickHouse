...
Развертывания BYOC включают широкие возможности обсервабилити, позволяя отслеживать сервисы ClickHouse с помощью выделенного стека мониторинга Prometheus и прямых конечных точек метрик серверов ClickHouse. Ваши данные мониторинга остаются в пределах вашей облачной учетной записи, обеспечивая полный контроль над инфраструктурой мониторинга. Ограниченная телеметрия использования и состояния, перечисленная в разделе [границы сети](/ru/products/bring-your-own-cloud/reference/network-security#outbound-connections), — единственные данные мониторинга, экспортируемые в ClickHouse; отдельно панели мониторинга ClickHouse и, при утвержденной эскалации, инженеры ClickHouse могут выполнять запросы к этому стеку на месте через Tailscale, при этом на стороне ClickHouse ничего не сохраняется — см. [доступ ClickHouse к данным](/ru/products/bring-your-own-cloud/reference/clickhouse-data-access).

<div id="prometheus-monitoring">
  ## Подходы к мониторингу с Prometheus {#prometheus-monitoring}
</div>
...
1. **Включите приватный балансировщик нагрузки** для инфраструктуры BYOC в консоли ClickHouse Cloud (см. [Load Balancers](/ru/products/bring-your-own-cloud/configuration/configurations#load-balancers)).
2. **Запросите URL конечной точки Prometheus** у службы поддержки ClickHouse.
3. **Проверьте подключение по частной сети** к конечной точке Prometheus — как правило, через пиринг VPC/VNet или другой вариант настройки частной сети.

Форматы конечных точек зависят от типа подключения:

| Подключение            | Формат конечной точки                                                          |
| ---------------------- | ------------------------------------------------------------------------------ |
| Пиринг VPC/VNet        | `https://prometheus-internal.<subdomain>.<region>.<cloud>.clickhouse-byoc.com` |
| Частная конечная точка | `https://prometheus.<label>.<subdomain>.<region>.<cloud>.clickhouse-byoc.com`  |

`<cloud>` — это `aws`, `gcp` или `azure`. Для частных конечных точек значение `<label>` зависит от облачного провайдера: `vpce` в AWS, `p` в GCP и `privatelink` в Azure.

<Note>
  URL стека Prometheus доступен только через подключения по частной сети и не требует аутентификации. Доступ ограничен сетями, которые могут обращаться к вашей VPC/VNet BYOC через пиринг или другие варианты частного подключения.
</Note>
...
<div id="direct-prometheus-integration">
  ## Интеграция Prometheus с сервисом ClickHouse {#direct-prometheus-integration}
</div>
...
* `<service-subdomain>.<byoc-subdomain>.<region>.<provider>.clickhouse-byoc.com:8443` на фактическую конечную точку вашего сервиса
