...
<Steps>
  ...
  <Step title="Настройте инфраструктуру BYOC" id="set-up-byoc-infrastructure">
    <Note>
      При нажатии **Set up Infrastructure** ClickHouse Cloud автоматически выполняет [предварительную проверку](/ru/products/bring-your-own-cloud/onboarding/standard#preflight-validation) перед подготовкой инфраструктуры. Проверяется, что учетная запись сервиса управления имеет необходимые разрешения и что включены требуемые API Google Cloud, а также проверяется предоставленная вами VPC: что сеть и подсеть разрешаются, что основной диапазон подсети и вторичный диапазон для подов достаточно велики и что сеть покрыта Cloud NAT gateway. Если чего-то не хватает, настройка прерывается с указанием конкретных проблем, которые нужно устранить.
    </Note>
