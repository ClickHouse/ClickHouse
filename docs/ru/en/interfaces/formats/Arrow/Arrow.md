...
| `input_format_arrow_use_native_reader`                                       | Использовать встроенное в ClickHouse средство чтения для форматов `Arrow` и `ArrowStream` вместо библиотеки Apache Arrow. Установите `0`, чтобы использовать средство чтения из библиотеки Apache Arrow.                                                                 | `1`         |
...
| `output_format_arrow_unsupported_types_as_binary`                            | Выводить типы, у которых нет эквивалента в Arrow (например, `BFloat16`, `AggregateFunction`), как необработанные двоичные данные. Если `false`, возникает исключение. Применяется как к встроенному средству записи, так и к средству записи из библиотеки Apache Arrow. | `1`         |
...
| `output_format_arrow_use_native_writer`                                      | Использовать встроенное в ClickHouse средство записи для форматов `Arrow` и `ArrowStream` вместо библиотеки Apache Arrow. Установите `0`, чтобы использовать средство записи из библиотеки Apache Arrow.                                                                 | `1`         |
