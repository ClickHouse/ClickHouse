...
:::note
При разборе в этом формате символы табуляции и перевода строки не допускаются ни в одном поле.
:::
...
Читайте данные в формате `TabSeparatedRawWithNamesAndTypes`:
...
Результат будет в формате с разделителями-табуляциями и двумя строками заголовка для имен столбцов и типов:
