...
:::note
При разборе в этом формате в каждом поле не допускаются символы табуляции или перевода строки.
:::
...
Для чтения данных используйте формат `TabSeparatedRawWithNames`:
...
Вывод будет в формате с разделением полей табуляцией и заголовком в одну строку:
