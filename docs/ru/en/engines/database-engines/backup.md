...
Движок базы данных Backup позволяет мгновенно подключить таблицу/базу данных из [резервных копий](/ru/operations/backup/overview) в режиме только для чтения.

Движок базы данных Backup работает как с инкрементными, так и с обычными резервными копиями.
...
Пунктом назначения резервной копии может быть любой допустимый [пункт назначения резервной копии](/ru/operations/backup/disk#configure-backup-destinations-for-disk), например `Disk`, `S3` или `File`. Оно передаётся в виде функции, например `Disk('disk_name', 'backup_name')`.
...
Рассмотрим пример с пунктом назначения резервной копии `Disk`. Сначала настроим диск для резервных копий в `storage.xml`:
...
Итак, резервная копия `test_database_backup` у нас уже есть, давайте создадим базу данных Backup:

```sql
CREATE DATABASE test_database_backup ENGINE = Backup('test_database', Disk('backups', 'test_database_backup'));
```

Теперь можно выполнить запрос к любой таблице в базе данных:
...
С этой базой данных Backup также можно работать как с любой обычной базой данных. Например, выполнять в ней запросы к таблицам:
