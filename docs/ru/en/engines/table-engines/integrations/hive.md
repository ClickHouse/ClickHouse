...
Движок Hive позволяет выполнять запросы `SELECT` к Hive-таблицам в HDFS. В настоящее время поддерживаются следующие форматы ввода:

* Text: поддерживаются только простые скалярные типы столбцов, кроме `binary`

* ORC: поддерживаются простые скалярные типы столбцов, кроме `char`; из сложных типов поддерживаются только такие, как `array`

* Parquet: поддерживаются все простые скалярные типы столбцов; из сложных типов поддерживаются только такие, как `array`
...
* Имена столбцов должны совпадать с именами столбцов в исходной таблице Hive, но можно использовать только часть этих столбцов и в любом порядке, а также столбцы-алиасы, вычисляемые на основе других столбцов.
* Типы столбцов должны совпадать с типами столбцов в исходной таблице Hive.
...
Мы настоятельно рекомендуем включать локальный кэш для удалённых файловых систем. Бенчмарк показывает, что с кэшем работа почти в 2 раза быстрее.
...
* enable: Если задано значение true, ClickHouse после запуска будет поддерживать локальный кэш для удаленной файловой системы (HDFS).
* root&#95;dir: Обязательно. Корневой каталог для хранения файлов локального кэша удаленной файловой системы.
* limit&#95;size: Обязательно. Максимальный размер файлов локального кэша (в байтах).
* bytes&#95;read&#95;before&#95;flush: Управляет количеством байтов до flush в локальную файловую систему при загрузке файла из удаленной файловой системы. Значение по умолчанию — 1MB.
...
<div id="query-hive-table-with-text-input-format">
  ### Запрос к таблице Hive с использованием формата ввода Text
</div>
