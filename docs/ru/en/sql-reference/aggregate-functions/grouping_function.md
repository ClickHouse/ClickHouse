...
[ROLLUP](../statements/select/group-by.md/#rollup-modifier) и [CUBE](../statements/select/group-by.md/#cube-modifier) — модификаторы GROUP BY. Оба вычисляют промежуточные итоги. ROLLUP принимает упорядоченный список столбцов, например `(day, month, year)`, и вычисляет промежуточные итоги на каждом уровне агрегации, а затем общий итог. CUBE вычисляет промежуточные итоги для всех возможных комбинаций указанных столбцов. GROUPING определяет, какие строки, возвращаемые ROLLUP или CUBE, являются суперагрегатами, а какие — строками, которые вернул бы GROUP BY без модификаторов.

Функция GROUPING принимает несколько столбцов в качестве аргумента и возвращает битовую маску.

* `1` означает, что строка, возвращённая модификатором `ROLLUP` или `CUBE` для `GROUP BY`, является промежуточным итогом
* `0` означает, что строка, возвращённая `ROLLUP` или `CUBE`, не является промежуточным итогом

<div id="grouping-sets">
  ## GROUPING SETS
</div>

По умолчанию модификатор CUBE вычисляет промежуточные итоги для всех возможных комбинаций столбцов, переданных в CUBE. GROUPING SETS позволяет указать, какие именно комбинации нужно вычислить.

Для анализа иерархических данных хорошо подходят модификаторы ROLLUP, CUBE и GROUPING SETS. Пример ниже — таблица с данными о том, какие дистрибутивы Linux и их версии установлены в двух датацентрах. Эти данные может быть полезно анализировать по дистрибутиву, версии и местоположению.
...
Та же информация с использованием GROUPING SETS:
...
CUBE в следующем запросе, `CUBE(datacenter,distro,version)`, задает иерархию, которая может быть не вполне осмысленной. Нет смысла сравнивать `version` между двумя дистрибутивами, поскольку у Arch и RHEL разные циклы релизов и разные принципы именования версий. Следующий за ним пример с GROUPING SETS более уместен, так как в нем `distro` и `version` сгруппированы в одном наборе.
...
:::note
Поле version в приведённом выше примере может быть не вполне осмысленным, если оно не связано с дистрибутивом. Если бы мы отслеживали версию ядра, это имело бы смысл, поскольку версия ядра может относиться к любому дистрибутиву. В таком случае лучшим выбором может быть использование GROUPING SETS, как в следующем примере.
:::
