---
description: 'Представляет содержимое файлов индекса и файлов меток таблиц MergeTree.
...
Представляет содержимое файлов индекса и файлов меток таблиц MergeTree. Может использоваться для диагностики.
...
Табличный объект со столбцами, содержащими значения первичного индекса и min-max индекса (если они включены) исходной таблицы, столбцами со значениями меток (если они включены) для всех возможных файлов в частях данных исходной таблицы, а также виртуальными столбцами:

* `part_name` - Имя части данных.
* `mark_number` - Номер текущей метки в части данных.
* `rows_in_granule` - Число строк в текущей грануле.
