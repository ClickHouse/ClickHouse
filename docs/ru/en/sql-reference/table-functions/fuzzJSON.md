---
description: 'Вносит случайные изменения в JSON-строку.'
...
Вносит случайные изменения в JSON-строку.
...
| Аргумент                           | Описание                                                                                               |
| ---------------------------------- | ------------------------------------------------------------------------------------------------------ |
| `named_collection`                 | [Именованная коллекция](/ru/sql-reference/statements/create/named-collection.md).                         |
...
| `reuse_output` (boolean)           | Повторно использовать результат процесса fuzzing в качестве входных данных для следующего fuzzer.      |
| `malform_output` (boolean)         | Сгенерировать строку, которую нельзя разобрать как объект JSON.                                        |
| `max_output_length` (UInt64)       | Максимально допустимая длина сгенерированной или изменённой JSON-строки.                               |
| `probability` (Float64)            | Вероятность применения fuzzing к полю JSON (паре ключ-значение). Должна находиться в диапазоне [0, 1]. |
...
| `max_object_size` (UInt64)         | Максимально допустимое число полей на одном уровне объекта JSON.                                       |
