...
`deltaLake` — это псевдоним `deltaLakeS3`, который поддерживается для совместимости.
...
Аргумент `format` обозначает формат файлов данных в таблице Delta Lake.

Необязательный параметр `extra_credentials` можно использовать для передачи `role_arn` для доступа на основе ролей в ClickHouse Cloud. Инструкции по настройке см. в [Secure S3](/ru/cloud/data-sources/secure-s3).
...
Рассмотрим таблицу в хранилище S3 по пути `s3://ch-docs-s3-bucket/people_10k/`.
Запись в Delta Lake — это бета-возможность, отключенная по умолчанию. Включите её следующим образом (`allow_delta_lake_writes` доступен начиная с версии 26.7; в более ранних версиях используйте `allow_experimental_delta_lake_writes`):
...
* [кластерная табличная функция DeltaLake](/ru/sql-reference/table-functions/deltalakeCluster.md)