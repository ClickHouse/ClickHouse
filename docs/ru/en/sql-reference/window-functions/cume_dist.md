...
Определение рамки окна по умолчанию, которое также является обязательным, — `RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING`.
...
В следующем примере вычисляется накопительное распределение зарплат в команде:
...
Строки с одинаковыми значениями получают одно и то же значение кумулятивного распределения, соответствующее наивысшей позиции в группе строк с одинаковыми значениями.