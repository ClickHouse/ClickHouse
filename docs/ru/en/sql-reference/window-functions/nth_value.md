...
* Первое отличное от NULL значение, вычисленное по n-й строке (`offset`) в пределах её упорядоченной рамки окна.

**Пример**

В этом примере функция `nth-value` используется для поиска третьей по величине зарплаты в вымышленном наборе данных о зарплатах футболистов Английской Премьер-лиги.
