...
Их можно использовать для вычислений, аналогичных тем, что выполняются с помощью агрегатных функций, однако, в отличие от них, оконная функция не объединяет строки в один результирующий набор — вместо этого по-прежнему возвращаются отдельные строки.
...
| Клауза `WINDOW` (`SELECT ... FROM table WINDOW w AS (PARTITION BY id)`)               | ✅               |                                                                                                                                                                                                                                                                                                                                                                                                                         |
| Рамка `ROWS`                                                                          | ✅               |                                                                                                                                                                                                                                                                                                                                                                                                                         |
| Рамка `RANGE`                                                                         | ✅               | Используется по умолчанию, если рамка не указана явно (`RANGE BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW`).                                                                                                                                                                                                                                                                                                            |
| Синтаксис `INTERVAL` для рамки `DateTime` `RANGE OFFSET`                              | ❌               | Вместо этого укажите число секунд (`RANGE` работает с любым числовым типом).                                                                                                                                                                                                                                                                                                                                            |
| Рамка `GROUPS`                                                                        | ❌               |                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `percent_rank()`/`percentRank()`                                                      | ✅               | Эффективно вычисляет относительную позицию значения в пределах партиции. Заменяет более многословный и вычислительно затратный ручной SQL-расчет: `ifNull((rank() OVER (PARTITION BY x ORDER BY y) - 1) / nullif(count(1) OVER (PARTITION BY x) - 1, 0), 0)`.                                                                                                                                                           |
| `cume_dist()`                                                                         | ✅               | Вычисляет кумулятивное распределение значения в группе значений. Возвращает процент строк, значения в которых меньше или равны значению текущей строки.                                                                                                                                                                                                                                                                 |
...
| [`ntile(buckets)`](./ntile.md)                                                        | ✅               | Укажите окно, например: `(PARTITION BY x ORDER BY y ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING)`.                                                                                                                                                                                                                                                                                                         |
...
* `PARTITION BY` — определяет, как разбить результирующий набор строк на группы.
* `ORDER BY` — определяет порядок строк внутри группы при вычислении aggregate&#95;function.
* `ROWS or RANGE` — определяет границы рамки окна; aggregate&#95;function вычисляется в пределах этой рамки.
...
<div id="functions">
  ## Функции, которые можно использовать только как оконные
</div>

Следующие функции можно использовать только как оконные. Большинство из них — стандартные SQL-функции; `lagInFrame`, `leadInFrame` и `nonNegativeDerivative` — расширения ClickHouse.
...
| [`nth_value(x, offset)`](./nth_value.md)                                                                   | Возвращает первое значение, отличное от NULL, вычисленное для n-й строки (`offset`) в пределах упорядоченной рамки окна.                     |
...
| [`lagInFrame(x)`](./lagInFrame.md)                                                                         | Возвращает значение, вычисленное для строки, находящейся на указанное число строк перед текущей строкой в пределах упорядоченной рамки окна. |
| [`leadInFrame(x)`](./leadInFrame.md)                                                                       | Возвращает значение, вычисленное для строки, находящейся на указанное число строк после текущей строки в пределах упорядоченной рамки окна.  |
| [`ntile(buckets)`](./ntile.md)                                                                             | Делит упорядоченные строки в пределах партиции на заданное количество бакетов и возвращает номер бакета для текущей строки.                  |
...
Ниже приведены примеры решения распространённых практических задач.

<div id="maximumtotal-salary-per-department">
  ### Максимальная/суммарная зарплата по отделам
</div>
...
Температура хранится с точностью до секунды, но с помощью `Range` и `ORDER BY toDate(ts)` мы задаём рамку окна размером 10 единиц, и благодаря `toDate(ts)` этой единицей является день.
...
Дорожная карта первоначальной поддержки оконных функций доступна [в этом issue](https://github.com/ClickHouse/ClickHouse/issues/18097).

Все issue на GitHub, связанные с оконными функциями, имеют метку [comp-window-functions](https://github.com/ClickHouse/ClickHouse/labels/comp-window-functions).

<div id="tests">
  ### Тесты
</div>

Эти тесты содержат примеры грамматики, поддерживаемой в настоящее время:
