...
Эта функция похожа на [`lagInFrame`](./lagInFrame.md), но всегда использует рамку `ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING`.
...
В этом примере рассматриваются исторические данные по конкретной акции, а функция `lag` используется для вычисления дневной дельты и процентного изменения цены закрытия.
