...
Этот тип позволяет хранить значения любого типа без необходимости заранее знать все возможные типы.
...
Где `N` — необязательный параметр в диапазоне от `0` до `254`, который указывает, сколько различных типов данных может храниться в виде отдельных подстолбцов внутри столбца типа `Dynamic` в рамках одного отдельно хранимого блока данных (например, в рамках одной части данных в таблице семейства MergeTree). Если этот предел превышен, все значения новых типов будут храниться вместе в специальной общей структуре данных в бинарной форме. Значение `max_types` по умолчанию — `32`.
...
Использование CAST для столбца типа `Variant`:
...
Тип `Dynamic` поддерживает чтение отдельного вложенного типа из столбца `Dynamic` с использованием имени типа в качестве подстолбца.
Так, если у вас есть столбец `d Dynamic`, вы можете читать подстолбец любого допустимого типа `T`, используя синтаксис `d.T`.
...
того же размера, что и исходный столбец `Dynamic`, и будет содержать значения `NULL` (или пустые значения, если `T` не может находиться внутри `Nullable`)
...
Чтобы узнать, какой вариант хранится в каждой строке, можно использовать функцию `dynamicType(dynamic_column)`. Она возвращает `String` с именем типа значения для каждой строки (или `'None'`, если значение строки — `NULL`).
...
Возможны 4 варианта преобразования со столбцом `Dynamic`.
...
Все текстовые форматы (TSV, CSV, CustomSeparated, Values, JSONEachRow и т. д.) поддерживают чтение типа `Dynamic`. При разборе данных ClickHouse пытается автоматически определить тип каждого значения и использовать его при вставке в столбец `Dynamic`.
...
Большинство функций поддерживают аргументы с типом `Dynamic`. В этом случае функция выполняется отдельно для каждого внутреннего типа данных, хранящегося в столбце `Dynamic`.
Если тип результата функции зависит от типов аргументов, то результат такой функции, вызванной с аргументами типа `Dynamic`, будет иметь тип `Dynamic`. Если тип результата функции не зависит от типов аргументов, результатом будет `Nullable(T)`, где `T` — обычный тип результата этой функции.
...
Если функцию невозможно выполнить для какого-либо типа в столбце `Dynamic`, будет сгенерировано исключение:
...
Или извлеките нужный тип как подстолбец:
...
* `false` — вернуть `NULL` для несовместимых строк и сохранить результат для совместимых.
...
* Если `T1 = T2 = T`, результат будет `d1.T < d2.T` (будут сравниваться внутренние значения).
* Если `T1 != T2`, результат будет `T1 < T2` (будут сравниваться имена типов).

По умолчанию тип `Dynamic` нельзя использовать в ключах `GROUP BY`/`ORDER BY`; если вы хотите его использовать, учитывайте особое правило сравнения и включите настройки `allow_suspicious_types_in_group_by`/`allow_suspicious_types_in_order_by`.
...
**Примечание:** значения типа Dynamic с разными числовыми типами считаются различными и не сравниваются друг с другом; вместо этого сравниваются названия их типов.
...
**Примечание:** описанное правило сравнения не применяется при вызове функций сравнения, таких как `<`/`>`/`=` и других, из-за [специфики работы](#using-dynamic-type-in-functions) функций с типом `Dynamic`

<div id="reaching-the-limit-in-number-of-different-data-types-stored-inside-dynamic">
  ## Достижение предела по количеству различных типов данных, хранящихся в Dynamic
</div>

Тип данных `Dynamic` может хранить лишь ограниченное количество различных типов данных в виде отдельных подстолбцов. По умолчанию этот предел составляет 32, но его можно изменить в объявлении типа с помощью синтаксиса `Dynamic(max_types=N)`, где N — число от 0 до 254 (из-за подробностей реализации невозможно иметь более 254 различных типов данных, которые могут храниться в Dynamic в виде отдельных подстолбцов).
Когда этот предел достигнут, все новые типы данных, вставленные в столбец `Dynamic`, будут помещаться в единую общую структуру данных, которая хранит значения разных типов данных в бинарной форме.

Посмотрим, что происходит при достижении этого предела в разных сценариях.

<div id="reaching-the-limit-during-data-parsing">
  ### При достижении лимита во время разбора данных
</div>

При разборе значений `Dynamic` из данных, когда для текущего блока данных достигается лимит, все новые значения будут вставляться в общую структуру данных:
...
Как видно, после вставки 3 разных типов данных — `Int64`, `Array(Int64)` и `String` — все новые типы были записаны в специальную общую структуру данных.

<div id="during-merges-of-data-parts-in-mergetree-table-engines">
  ### При слиянии частей данных в движках таблиц MergeTree
</div>

При слиянии нескольких частей данных в таблице семейства MergeTree столбец `Dynamic` в результирующей части данных может достигнуть предела по числу различных типов данных, которые можно хранить в отдельных подстолбцах, и не сможет сохранить все типы в виде подстолбцов из исходных частей.
В этом случае ClickHouse выбирает, какие типы останутся в отдельных подстолбцах после слияния, а какие будут помещены в общую структуру данных. В большинстве случаев ClickHouse старается сохранить наиболее часто встречающиеся типы, а самые редкие — хранить в общей структуре данных, но это зависит от реализации.

Рассмотрим пример такого слияния. Сначала создадим таблицу со столбцом `Dynamic`, зададим предел количества различных типов данных равным `3` и вставим значения `5` разных типов:
...
Как видно, ClickHouse сохранил наиболее часто встречающиеся типы `UInt64` и `Array(UInt64)` в виде подстолбцов, а все остальные типы поместил в общие данные.
