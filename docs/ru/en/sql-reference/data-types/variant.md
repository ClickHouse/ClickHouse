...
содержит значение либо типа `T1`, либо `T2`, либо ... либо `TN`, либо не содержит ни одного из них (значение `NULL`).
...
:::note
Не рекомендуется использовать в качестве вариантов похожие типы (например, разные числовые типы, такие как `Variant(UInt32, Int64)`, или разные типы дат, такие как `Variant(Date, DateTime)`),
поскольку работа со значениями таких типов может приводить к неоднозначности. По умолчанию создание такого типа `Variant` вызывает исключение, но это можно включить с помощью настройки `allow_suspicious_variant_types`
:::
...
Использование CAST с обычными столбцами:
...
Использование функций `if/multiIf`, если у аргументов нет общего типа (для этого должна быть включена настройка `use_variant_as_common_type`):
...
Используйте функции &#39;array/map&#39;, если элементы массива/значения map не имеют общего типа (для этого должен быть включен параметр `use_variant_as_common_type`):
...
Итак, если у вас есть столбец `variant Variant(T1, T2, T3)`, вы можете прочитать подстолбец типа `T2`, используя синтаксис `variant.T2`,
...
во всех строках, в которых исходный столбец `Variant` не имеет типа `T2`.
...
Примечание: преобразование из типа `String` всегда выполняется через парсинг; если вам нужно преобразовать столбец `String` в вариант `String` типа `Variant` без парсинга, можно сделать следующее:
...
Столбец `Variant` можно преобразовать в другой столбец `Variant`, но только если целевой столбец `Variant` содержит все вложенные типы исходного `Variant`:
...
Все текстовые форматы (TSV, CSV, CustomSeparated, Values, JSONEachRow и т. д.) поддерживают чтение типа `Variant`. При парсинге данных ClickHouse пытается выполнить вставку значения в наиболее подходящий тип варианта.
...
применяя сравнение отдельно для каждого типа варианта. Это можно отключить с помощью настройки `use_variant_default_implementation_for_comparisons = 0`,
...
* Если `T1 = T2 = T`, результат будет `v1.T < v2.T` (будут сравниваться внутренние значения).
...
Если вам нужно найти строку с определённым значением типа `Variant`, вы можете сделать одно из следующего:
...
**Примечание:** значения вариантов с разными числовыми типами считаются разными вариантами и не сравниваются друг с другом — вместо этого сравниваются названия их типов.
...
**Примечание:** по умолчанию тип `Variant` нельзя использовать в ключах `GROUP BY`/`ORDER BY`; если вы хотите его использовать, учитывайте особое правило сравнения для этого типа и включите настройки `allow_suspicious_types_in_group_by`/`allow_suspicious_types_in_order_by`.

<div id="jsonextract-functions-with-variant">
  ## Функции JSONExtract с типом Variant
</div>
...
Начиная с версии `26.1`, если функция, которая не поддерживает типы Variant явным образом, получает столбец Variant, ClickHouse:

1. Извлекает каждый тип варианта из столбца Variant
2. Выполняет функцию отдельно для каждого типа варианта
3. Соответствующим образом объединяет результаты в зависимости от их типов
...
Оператор сравнения автоматически применяется к каждому типу варианта по отдельности, что позволяет фильтровать столбцы Variant.
...
деление на ноль или нехватка памяти, по-прежнему вызываются обычным образом, чтобы не скрывать реальные проблемы.
:::
...
Параметр `variant_throw_on_type_mismatch` определяет, что происходит, когда функция применяется к столбцу `Variant`, а фактически хранимый в строке тип несовместим с этой функцией:
