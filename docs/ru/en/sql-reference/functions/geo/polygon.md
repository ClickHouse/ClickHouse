---
description: 'Документация по Polygon'
...
Возвращает геометрический объект в формате WKT (Well Known Text) из различных [гео-типов данных](../../data-types/geo.md). Поддерживаются следующие объекты WKT:
...
`geo_data` может быть одним из следующих [гео-типов данных](../../data-types/geo.md) или их соответствующих примитивных типов:
...
POLYGON из массива Tuple или массива массивов Tuple:
...
Преобразует WKT (Well Known Text) Multipolygon в значение типа Multipolygon.
...
Преобразует WKT-представление MultiPolygon в тип Polygon.
...
Функция `readWKTPoint` в ClickHouse разбирает представление геометрии Point в формате Well-Known Text (WKT) и возвращает точку во внутреннем формате ClickHouse.
...
Функция возвращает внутреннее представление геометрического типа Point в ClickHouse.
...
Разбирает геометрию LineString, представленную в формате Well-Known Text (WKT), и возвращает её во внутреннем формате ClickHouse.
...
* `wkt_string`: Входная строка WKT, представляющая геометрический объект LineString.

<div id="returned-value">
  ### Возвращаемое значение
</div>

Функция возвращает внутреннее представление объекта геометрии LineString в ClickHouse.
...
Разбирает геометрию MultiLineString, представленную в формате Well-Known Text (WKT), и возвращает её во внутреннем формате ClickHouse.
...
* `wkt_string`: Входная строка WKT, представляющая геометрический объект MultiLineString.

<div id="returned-value">
  ### Возвращаемое значение
</div>

Функция возвращает внутреннее представление ClickHouse для геометрии MultiLineString.
...
Разбирает представление геометрии Polygon в формате Well-Known Text (WKT) и возвращает кольцо (замкнутую линию) во внутреннем формате ClickHouse.
...
* `wkt_string`: Входная строка WKT, представляющая геометрию типа Polygon.

<div id="returned-value">
  ### Возвращаемое значение
</div>

Функция возвращает внутреннее представление ClickHouse для геометрии Ring (замкнутой ломаной).
...
Возвращает true или false в зависимости от того, находится ли один многоугольник целиком внутри другого многоугольника. Reference https://www.boost.org/doc/libs/1&#95;62&#95;0/libs/geometry/doc/html/geometry/reference/algorithms/within/within&#95;2.html
...
Преобразует WKB (Well Known Binary) типа MultiPolygon в тип Polygon.
...
* `wkb_string`: Входная строка WKB, представляющая геометрический объект Point.

<div id="returned-value">
  ### Возвращаемое значение
</div>

Функция возвращает внутреннее представление ClickHouse для геометрии Point.
...
Разбирает геометрию LineString, представленную в формате Well-Known Binary (WKB), и возвращает её во внутреннем формате ClickHouse.
...
* `wkb_string`: Входная строка WKB, представляющая геометрический объект LineString.

<div id="returned-value">
  ### Возвращаемое значение
</div>

Функция возвращает внутреннее представление геометрического объекта LineString в ClickHouse.
...
Разбирает геометрию MultiLineString, представленную в формате Well-Known Binary (WKB), и возвращает её во внутреннем формате ClickHouse.
...
* `wkb_string`: Входная строка WKB, представляющая геометрию типа MultiLineString.
...
Вычисляет минимальное расстояние между двумя точками, где одна точка принадлежит первому многоугольнику, а вторая — другому многоугольнику. Spherical означает, что координаты интерпретируются как координаты на идеальной сфере, что для Земли неверно. Использование такой системы координат ускоряет вычисления, но, разумеется, не обеспечивает высокой точности.
...
Вычисляет пространственную симметрическую разность (XOR) двух многоугольников
...
То же, что и `polygonsSymDifferenceSpherical`, но координаты заданы в декартовой системе координат, которая ближе к модели реальной Земли.
...
Вычисляет пересечение (AND) многоугольников; координаты — сферические.
...
Возвращает true, если второй многоугольник находится внутри первого.
...
Возвращает true, если два многоугольника пересекаются (имеют общую область или общую границу).
...
Возвращает `true`, если два многоугольника пересекаются (имеют общую область или общую границу). См. https://www.boost.org/doc/libs/1&#95;62&#95;0/libs/geometry/doc/html/geometry/reference/algorithms/intersects.html
...
<div id="polygon-representing-zimbabwe">
  #### Многоугольник, соответствующий Зимбабве
</div>

Вот многоугольник, соответствующий Зимбабве:
