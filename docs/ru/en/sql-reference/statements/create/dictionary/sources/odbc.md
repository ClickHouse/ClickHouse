...
title: 'ODBC-источник словаря'
sidebar_position: 6
sidebar_label: 'ODBC'
description: 'Настройка ODBC-подключения как источника словаря в ClickHouse.'
doc_type: 'reference'
---
...
Вы можете использовать этот метод для подключения любой базы данных, для которой есть ODBC-драйвер.
...
| `background_reconnect` | Повторно подключаться к реплике в фоновом режиме, если не удается установить соединение. Необязательно.                                       |
| `query`                | Пользовательский запрос. Необязательно.                                                                                                       |

:::note
Поля `table` и `query` нельзя использовать одновременно. При этом должно быть объявлено либо поле `table`, либо поле `query`.
:::

ClickHouse получает символы кавычек от ODBC-драйвера и заключает в кавычки все настройки в запросах к драйверу, поэтому имя таблицы нужно задавать с учетом регистра, используемого в базе данных.

Если при использовании Oracle у вас возникают проблемы с кодировками, см. соответствующий пункт [FAQ](/ru/knowledgebase/oracle-odbc).
...
Настройка `/etc/odbc.ini` (или `~/.odbc.ini`, если вы вошли в систему под пользователем, запускающим ClickHouse):
...
При необходимости отредактируйте `odbc.ini`, указав полный путь к библиотеке драйвера: `DRIVER=/usr/local/lib/psqlodbcw.so`.
...
* чтобы определить самую раннюю версию TDS, поддерживаемую той или иной версией SQL Server, обратитесь к документации продукта или к разделу [MS-TDS Product Behavior](https://docs.microsoft.com/en-us/openspecs/windows_protocols/ms-tds/135d0ebe-5c4c-4a94-99bf-1811eccb9f4a)
