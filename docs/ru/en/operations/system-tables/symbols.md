---
description: 'Системная таблица, полезная для специалистов по C++ и инженеров ClickHouse; содержит
...
Эта таблица будет полезна только специалистам по C++ и инженерам ClickHouse.
...
* `symbol` ([String](/ru/sql-reference/data-types/string)) — Имя символа в бинарном файле. Оно манглировано. Чтобы получить читаемое имя, можно применить `demangle(symbol)`.
* `symbol_demangled` ([Nullable(String)](/ru/sql-reference/data-types/nullable)) — Деманглированный символ, используемый для инструментирования XRay.
* `function_id` ([Nullable(Int32)](/ru/sql-reference/data-types/nullable)) — Идентификатор функции в карте инструментирования XRay.
* `address_begin` ([UInt64](/ru/sql-reference/data-types/int-uint)) — Начальный адрес символа в бинарном файле.
* `address_end` ([UInt64](/ru/sql-reference/data-types/int-uint)) — Конечный адрес символа в бинарном файле.
