---
description: 'Системная таблица, содержащая сведения, полезные для общего обзора использования
  памяти и ProfileEvents пользователей.'
...
Эту системную таблицу можно использовать для получения общего представления об использовании памяти и ProfileEvents пользователей.
...
* `user` ([String](/ru/sql-reference/data-types/string)) — Имя пользователя.
* `memory_usage` ([Int64](/ru/sql-reference/data-types/int-uint)) — Суммарный объём оперативной памяти, используемой всеми процессами пользователя. Может не включать некоторые типы выделенной памяти. См. настройку max&#95;memory&#95;usage.
* `peak_memory_usage` ([Int64](/ru/sql-reference/data-types/int-uint)) — Пиковое использование памяти пользователем. Может быть сброшено, если для этого пользователя не выполняются запросы.
* `ProfileEvents` ([Map(LowCardinality(String), UInt64)](/ru/sql-reference/data-types/map)) — Сводка ProfileEvents, измеряющих различные метрики для пользователя. Их описание приведено в таблице system.events
