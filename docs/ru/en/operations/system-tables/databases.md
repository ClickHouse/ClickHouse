---
description: 'Системная таблица, содержащая информацию о базах данных, доступных
...
* `name` ([String](/ru/sql-reference/data-types/string)) — Имя базы данных.
* `engine` ([String](/ru/sql-reference/data-types/string)) — Движок базы данных.
* `data_path` ([String](/ru/sql-reference/data-types/string)) — Путь к данным.
* `metadata_path` ([String](/ru/sql-reference/data-types/string)) — Путь к метаданным.
* `uuid` ([UUID](/ru/sql-reference/data-types/uuid)) — UUID базы данных.
* `engine_full` ([String](/ru/sql-reference/data-types/string)) — Параметры движка базы данных.
* `comment` ([String](/ru/sql-reference/data-types/string)) — Комментарий к базе данных.
* `is_external` ([UInt8](/ru/sql-reference/data-types/int-uint)) — База данных является внешней (то есть PostgreSQL/DataLakeCatalog).

**Псевдонимы:**

* `database` — Псевдоним для `name`.

{/*AUTOGENERATED_END*/ }

Столбец `name` этой системной таблицы используется при выполнении запроса `SHOW DATABASES`.
...
Проверьте все базы данных, к которым у пользователя есть доступ.
