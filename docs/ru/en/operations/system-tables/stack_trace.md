---
description: 'Системная таблица, содержащая трассировки стека всех потоков сервера. Позволяет
  разработчикам исследовать внутреннее состояние сервера.'
...
Содержит трассировки стека всех потоков сервера. Позволяет разработчикам исследовать внутреннее состояние сервера.

Для анализа фреймов стека используйте [функции интроспекции](../../sql-reference/functions/introspection.md) `addressToLine`, `addressToLineWithInlines`, `addressToSymbol` и `demangle`.
...
* `thread_name` ([String](/ru/sql-reference/data-types/string)) — Имя потока.
* `thread_id` ([UInt64](/ru/sql-reference/data-types/int-uint)) — Идентификатор потока
* `query_id` ([String](/ru/sql-reference/data-types/string)) — ID запроса, к которому относится этот поток.
* `trace` ([Array(UInt64)](/ru/sql-reference/data-types/array)) — Трассировка стека этого потока. По сути, это просто массив адресов.
* `untracked_memory` ([Int64](/ru/sql-reference/data-types/int-uint)) — Непотокобезопасный счётчик выделений памяти для потока, ещё не переданных в родительский MemoryTracker. Может быть отрицательным, если с момента последнего сброса памяти было освобождено больше, чем выделено.
...
* [Функции интроспекции](../../sql-reference/functions/introspection.md) — Какие функции интроспекции доступны и как ими пользоваться.
* [system.trace&#95;log](../system-tables/trace_log.md) — Содержит трассировки стека, собранные выборочным профилировщиком запросов.
* [arrayMap](/ru/sql-reference/functions/array-functions#arrayMap)) — Описание и пример использования функции `arrayMap`.
* [arrayFilter](/ru/sql-reference/functions/array-functions#arrayFilter) — Описание и пример использования функции `arrayFilter`.