...
* `--no-confirmation` — Если указан, не будет запрашивать подтверждение для ряда команд. Значение по умолчанию: `false` в интерактивном режиме и `true` для запроса
...
* `CLICKHOUSE_KEEPER_IDENTITY` — Используется в качестве значения идентификатора по умолчанию, если `--identity` не указан в командной строке.

<div id="clickhouse-keeper-client-auth">
  ## Аутентификация
</div>

При подключении к серверу Keeper, требующему аутентификации, пароль выбирается в следующем порядке приоритета (используется первое совпадение):
...
Тот же порядок приоритета действует для `--идентификатора` / `CLICKHOUSE_KEEPER_IDENTITY` / `<zookeeper><идентификатор>`.
...
* `set '<path>' <value> [version]` -- Обновляет значение узла. Обновление выполняется, только если `version` совпадает (по умолчанию: -1)
...
* `rm '<path>' [version]` -- Удаляет узел, только если `version` совпадает (по умолчанию: -1)
* `rmr '<path>' [limit]` -- Рекурсивно удаляет путь, если размер поддерева меньше `limit`. Требуется подтверждение (значение `limit` по умолчанию = 100)
* `flwc <command>` -- Выполняет четырехбуквенную команду
* `help` -- Выводит это сообщение
* `get_direct_children_number '[path]'` -- Возвращает количество прямых дочерних узлов по указанному пути
...
* `find_super_nodes <threshold> '[path]'` -- Находит узлы, у которых количество дочерних узлов превышает заданный порог, для указанного пути (по умолчанию `.`)
* `delete_stale_backups` -- Удаляет узлы ClickHouse, используемые для резервных копий, которые больше не активны
* `find_big_family [path] [n]` -- Возвращает top n узлов с наибольшим числом потомков в поддереве (путь по умолчанию = `.` и n = 10)
* `sync '<path>'` -- Синхронизирует узел между процессами и лидером
* `reconfig <add|remove|set> "<arg>" [version]` -- Выполняет реконфигурацию кластера Keeper. См. /docs/en/guides/sre/keeper/clickhouse-keeper#reconfiguration