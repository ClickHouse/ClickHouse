...
[Visual Studio Code](https://code.visualstudio.com/) и [Neovim](https://neovim.io/) хорошо зарекомендовали себя для разработки ClickHouse. Если вы используете VS Code, мы советуем установить [расширение clangd](https://marketplace.visualstudio.com/items?itemName=llvm-vs-code-extensions.vscode-clangd) вместо IntelliSense, так как оно значительно быстрее.

[CLion](https://www.jetbrains.com/clion/) — ещё один хороший вариант. Однако на больших проектах, таких как ClickHouse, он может работать медленнее.
Несколько вещей, которые стоит учитывать при работе с CLion:

- CLion самостоятельно создаёт `build` директорию и автоматически выбирает тип сборки `debug`.
- Использует встроенную в CLion версию CMake вместо установленной вами.
- Для выполнения задач сборки CLion использует `make` вместо `ninja` (это нормальное поведение).

Другие редакторы, которые можно использовать: [Sublime Text](https://www.sublimetext.com/), [Qt Creator](https://www.qt.io/product/development-tools) и [Kate](https://kate-editor.org/).
