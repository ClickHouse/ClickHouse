...
Обратите внимание, что репозиторий ClickHouse использует подмодули (submodules) — так называются ссылки на дополнительные репозитории (например, внешние библиотеки, от которых зависит проект). Это значит, что при клонировании репозитория следует указывать ключ `--recursive` как в примере выше. Если репозиторий был клонирован без подмодулей, то для их скачивания, необходимо выполнить:

    git submodule init
    git submodule update

Проверить наличие подмодулей можно с помощью команды `git submodule status`.
...
Как правило это означает, что отсутствуют ssh ключи для соединения с GitHub. Ключи расположены в каталоге `~/.ssh`. В интерфейсе GitHub, в настройках, необходимо загрузить публичные ключи, чтобы он их понимал.
...
Работа с подмодулями git может быть достаточно болезненной. Следующие команды позволят содержать их в порядке:

    # ! Каждая команда принимает аргумент
    # Обновить URLs удалённого репозитория для каждого подмодуля, используется относительно редко
    git submodule sync
    # Добавить новые подмодули
    git submodule init
    # Обновить подмодули до актуального состояния
    git submodule update
    # Две последние команды могут быть объединены вместе:
    git submodule update --init

Следующие команды помогут сбросить все подмодули в изначальное состояние (!ВНИМАНИЕ! — все изменения в подмодулях будут утеряны):
...
ClickHouse использует для сборки некоторое количество внешних библиотек. Но ни одну из них не требуется отдельно устанавливать, так как они собираются вместе с ClickHouse, из исходников, которые расположены в подмодулях. Посмотреть набор этих библиотек можно в каталоге contrib.

## Компилятор C++ {#kompiliator-c}

В качестве компилятора C++ поддерживается Clang начиная с версии 11.

Впрочем, наша среда непрерывной интеграции проверяет около десятка вариантов сборки, включая gcc, но сборка с помощью gcc непригодна для использования в продуктивном режиме.
...
Теперь вы готовы к сборке ClickHouse. Для размещения собранных файлов, рекомендуется создать отдельный каталог build внутри каталога ClickHouse:

    mkdir build
    cd build

Вы можете иметь несколько разных каталогов (build_release, build_debug) для разных вариантов сборки.

Находясь в каталоге build, выполните конфигурацию сборки с помощью CMake.
...
Вы можете изменить вариант сборки, выполнив новую команду в каталоге build.
...
Для полной сборки требуется около 30 ГБ свободного места или 15 GB для сборки только основных программ.
...
На машинах с 4 ГБ памяти рекомендуется указывать значение 1, а если памяти до 8 GB, то укажите значение 2.
...
Для запуска сервера из-под текущего пользователя с выводом журналов в терминал и с использованием примеров конфигурационных файлов, расположенных в исходниках, перейдите в каталог `ClickHouse/programs/server/` (этот каталог находится не в каталоге build) и выполните:

    ../../build/programs/clickhouse server

В этом случае ClickHouse будет использовать конфигурационные файлы, расположенные в текущем каталоге. Вы можете запустить `clickhouse server` из любого каталога, передав ему путь к конфигурационному файлу в аргументе командной строки `--config-file`.

Для подключения к ClickHouse с помощью clickhouse-client, в соседнем терминале, зайдите в каталог `ClickHouse/build/programs/` и выполните `./clickhouse client`.
...
Обратите внимание, что `clickhouse-client`, `clickhouse-server` и другие, являеются символическими ссылками на один и тот же двоичный файл `clickhouse`.
...
Откройте ответвление (fork) репозитория в интерфейсе GitHub. Если вы вели разработку в отдельной ветке (branch), выберите её. На странице будет доступна кнопка «Pull request». По сути это означает: «создать заявку на принятие моих изменений в основной репозиторий».

Заявку на изменение можно создать даже если работа над задачей ещё не завершена. В этом случае добавьте в его название слово «WIP» (work in progress). Название можно будет изменить позже. Это полезно для совместного просмотра и обсуждения изменений, а также для запуска всех имеющихся тестов. Введите краткое описание изменений — впоследствии оно будет использовано для журналов изменений релиза (changelog).

Тесты будут запущены, как только сотрудники ClickHouse поставят для заявки тег «Can be tested». Результаты первых проверок (стиль кода) появятся уже через несколько минут. Результаты сборки появятся примерно через полчаса. Результаты основного набора тестов будут доступны в пределах часа.

Система подготовит сборки ClickHouse специально для вашей заявки. Для их получения нажмите на ссылку «Details» у проверки «Clickhouse build check». Там вы сможете найти прямые ссылки на собранные .deb-пакеты ClickHouse, которые при желании вы даже сможете установить на свои продуктивные серверы (если не страшно).

Вероятнее всего часть сборок не будет успешной с первого раза: ведь мы проверяем сборку кода и gcc и clang, а при сборке с помощью clang включаются почти все существующие в природе warnings (всегда с флагом `-Werror`). На той же странице вы сможете найти журналы сборки — вам не обязательно самому собирать ClickHouse всеми возможными способами.
