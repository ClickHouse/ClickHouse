...
25. 
    ...
    :   То есть, параметры шаблона указываются только в using-е и затем не повторяются в коде.
        ...
        [std::map&lt;std::string, std::shared_ptr&lt;Stream&gt;&gt; streams;]{.strike}
...
27. C-style cast не используется.
 ...
 [std::cerr &lt;&lt; (int)c &lt;&lt; std::endl;]{.strike}
...
14.  Не нужно писать в комментарии диалог (лучше сказать устно).
    ...
    :   [/// Зачем ты сделал эту фигню?]{.strike}
...
17.  Имена файлов с исходниками на C++ должны иметь расширение только .cpp. Заголовочные файлы - только .h.
    ...
    :   [.hpp]{.strike} [.cc]{.strike} [.C]{.strike} [.inl]{.strike}
        Можно `.inl.h`, но не [.h.inl:strike:]{.strike}
...
25.  Не нужно объявлять и инициализировать переменные так:
    ...
    :   [auto s = std::string{"Hello"};]{.strike}
        Надо так:
        `std::string s = "Hello";`
        `std::string s{"Hello"};`
