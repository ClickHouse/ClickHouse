...
-   `aws_access_key_id`, `aws_secret_access_key` - данные пользователя учетной записи [AWS](https://aws.amazon.com/ru/). В качестве имени пользователя и пароля вы можете использовать их для аутентификации ваших запросов. Необязательный параметр.
