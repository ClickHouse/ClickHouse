...
Возможные значения:
    ...
    - `plain` - правило для плоских метрик (без тегов). Регулярное выражение в regexp
    - `tagged` - правило для теггированных метрик (метрика хранится в БД в формате `someName?tag1=value1&tag2=value2&tag3=value3`), регулярное выражение должно быть отсортированно по именам тегов, первым - значение тега `__name__`, если есть). Регулярное выражение в regexp
    - `tag_list` - правило для теггированных метрик, простой DSL для упрощения задания регулярного выражения (в формате тегов graphite `someName;tag1=value1;tag2=value2`, `someName` или `tag1=value1;tag2=value2`). При процессинге транслируется в правило `tagged`. Cортировать по именам тегов не обязательно (отсортируется при трансляции). Значение тега (но не имя) может быть регулярным выражением (например `env=(dev|staging)`). Задается также в regexp
