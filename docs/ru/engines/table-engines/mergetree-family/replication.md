...
:::note
В ClickHouse Cloud репликация настраивается за вас. Просто создавайте таблицы без дополнительных аргументов. Например, в тексте ниже следует заменить:
...
ClickHouse использует [ClickHouse Keeper](/docs/ru/operations/clickhouse-keeper.md) для хранения метаинформацию о репликах. Так же возможно использовать [Apache ZooKeeper](https://zookeeper.apache.org) версии 3.4.5 или новее, но ClickHouse Keeper является рекомендуемым.
...
:::
Пример указания адресов кластера ClickHouse Keeper:
...
ClickHouse также поддерживает хранение метаинформации о репликах в дополнительном ZooKeeper кластере. Сделать это, указав имя и путь кластера ZooKeeper в качестве аргументов table engine.
Другими словами, поддерживается возможность хранить метаданные разных таблиц в разных ZooKeeper кластерах. 

Пример указания адресов кластера дополнительного ZooKeeper кластера:
...
Для того, чтобы сохранить метаданные в дополнительном ZooKeeper кластере вместо ZooKeeper кластера по умолчанию, мы можем использовать SQL, чтобы создать таблицу с
ReplicatedMergeTree движком, как ниже:

```
CREATE TABLE table_name ( ... ) ENGINE = ReplicatedMergeTree('zookeeper_name_configured_in_auxiliary_zookeepers:path', 'replica_name') ...
```
Вы можете указать любой существующий ZooKeeper кластер, и система будет использовать директорию на нем для своих данных (директория указывается при создании реплицируемой таблицы).
...
Для очень больших кластеров, можно использовать разные кластеры ZooKeeper для разных шардов. Впрочем, исходя из нашего опыта, это не оказалось необходимым для production кластеров, содержащих примерно 300 серверов.
...
Количество реплик одних и тех же данных может быть произвольным. Исходя из нашего опыта, относительно надежным и удобным решением может быть использование двойной репликации в production среде, где на каждом сервере используется RAID-5 или RAID-6, в некоторых случаях RAID-10.
...
:::tip
Добавление `Replicated` является опциональным в ClickHouse Cloud, так как все таблицы реплицируются.
:::
...
Путь к таблице в ClickHouse Keeper должен быть разным для каждой реплицируемой таблицы. В том числе, для таблиц на разных шардах должны быть разные пути.
...
`table_name` - имя узла для таблицы в ClickHouse Keeper. Разумно делать его таким же, как имя таблицы. Оно указывается явно, так как, в отличие от имени таблицы, оно не меняется после запроса RENAME.
*Подсказка*: можно также указать имя базы данных перед `table_name`, например `db_name.table_name`

Можно использовать две встроенных подстановки `{database}` и `{table}`, они раскрываются в имя таблицы и в имя базы данных соответственно (если эти подстановки не переопределены в секции `macros`). Т.е. zookeeper путь можно задать как `'/clickhouse/tables/{shard}/{database}/{table}'`.
Будьте осторожны с переименованиями таблицы при использовании этих автоматических подстановок. Путь в ClickHouse Keeper-е нельзя изменить, а подстановка при переименовании таблицы раскроется в другой путь, таблица будет обращаться к несуществующему в ClickHouse Keeper пути и перейдет в режим только для чтения.
...
## Восстановление после сбоя {#recovery-after-failures}

Если при старте сервера, недоступен ClickHouse Keeper, реплицируемые таблицы переходят в режим только для чтения. Система будет пытаться периодически установить соединение с ClickHouse Keeper.

Если при `INSERT` недоступен ClickHouse Keeper, или происходит ошибка при взаимодействии с ним, будет выкинуто исключение.

При подключении к ClickHouse Keeper, система проверяет соответствие между имеющимся в локальной файловой системе набором данных и ожидаемым набором данных (информация о котором хранится в ClickHouse Keeper). Если имеются небольшие несоответствия, то система устраняет их, синхронизируя данные с реплик.

Обнаруженные битые куски данных (с файлами несоответствующего размера) или неизвестные куски (куски, записанные в файловую систему, но информация о которых не была записана в ClickHouse Keeper) переносятся в поддиректорию detached (не удаляются). Недостающие куски скачиваются с реплик.
...
Для запуска восстановления, создайте в ClickHouse Keeper узел `/path_to_table/replica_name/flags/force_restore_data` с любым содержимым или выполните команду для восстановления всех реплицируемых таблиц:

``` bash
$ sudo -u clickhouse touch /var/lib/clickhouse/flags/force_restore_data
```

Затем запустите сервер(перезапустите, если уже запущен). При старте, сервер удалит эти флаги и запустит восстановление.

## Восстановление в случае потери всех данных {#recovery-after-complete-data-loss}
...
4.  Создайте в ClickHouse Keeper узел `/path_to_table/replica_name/flags/force_restore_data` с любым содержимым или выполните команду для восстановления всех реплицируемых таблиц: `sudo -u clickhouse touch /var/lib/clickhouse/flags/force_restore_data`

Затем запустите сервер (перезапустите, если уже запущен). Данные будут скачаны с реплик.

В качестве альтернативного варианта восстановления, вы можете удалить из ClickHouse Keeper информацию о потерянной реплике (`/path_to_table/replica_name`), и затем создать реплику заново, как написано в разделе [Создание реплицированных таблиц](#creating-replicated-tables) .
...
## Восстановление в случае потери или повреждения метаданных на ClickHouse Keeper кластере {#recovery-when-metadata-in-the-zookeeper-cluster-is-lost-or-damaged}

Если данные в ClickHouse Keeper оказались утеряны или повреждены, то вы можете сохранить данные, переместив их в нереплицируемую таблицу, как описано в пункте выше.
