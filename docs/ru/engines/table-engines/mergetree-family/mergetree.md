...
-   `ORDER BY` — ключ сортировки.
    ...
    ClickHouse использует ключ сортировки в качестве первичного ключа, если первичный ключ явно не определен как `PRIMARY KEY`.

    Используйте синтаксис `ORDER BY tuple()`, если вам не нужна сортировка. Смотрите [выбор первичного ключа](#vybor-pervichnogo-kliucha).    
