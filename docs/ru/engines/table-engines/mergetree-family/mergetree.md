...
- `volume_priority` — определяет порядок заполнения томов. Меньшее значение означает больший приоритет. Значениями могут быть натуральные числа или `0`. `0` означает, что приоритет не установлен (так же, как если бы этот параметр был не указан).
  ...
  * Если его имеют лишь _некоторые_, то не имеющие этого параметра тома имеют более высокий приоритет чем те, у которых он указан. Те, у которых он указан, приоритизируются в соответствии со значением тега. При этом, порядок среди томов с неуказанным приоритетом не определен, поэтому рекомендуется либо указать приоритет для всех описанных томов, либо не указывать его вообще.
