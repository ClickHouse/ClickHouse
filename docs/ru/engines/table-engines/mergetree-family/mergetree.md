...
-   `SETTINGS` — дополнительные параметры, регулирующие поведение `MergeTree` (необязательные):
    ...
    -   `merge_with_ttl_timeout` — минимальное время в секундах перед повторным слиянием для удаления данных с истекшим TTL. По умолчанию: `4*3600` секунд.
    -   `merge_with_recompression_ttl_timeout` — минимальное время в секундах перед повторным слиянием для повторного сжатия данных с истекшим TTL. По умолчанию: `4*3600` секунд.    
    -   `try_fetch_recompressed_part_timeout` — в большинстве случаев повторное сжатие работает медленно. Поэтому ClickHouse выжидает заданное время перед запуском слияния с повторным сжатием данных и пытается считать нужный кусок данных с той реплики, которая запустила слияние. Время устанавливается в секундах. Значение по умолчанию: `7200` секунд.    
