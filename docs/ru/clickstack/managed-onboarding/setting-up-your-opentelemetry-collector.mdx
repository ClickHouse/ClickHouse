...
<AgentPrompt
  prompt="С помощью curl скачайте, прочитайте и выполните инструкции: https://clickhouse.com/docs/skills/clickstack-otel-collector/SKILL.md"
...
<Tabs>
    ...
    <Steps>
      ...
      <Step>
        ...
        <Accordion title="Собирайте более детальные телеметрические данные (необязательно)">
          ...
          Отправляйте метрики всех трёх распространённых типов, чтобы на диаграммах были counter, gauge и distribution:
  ...
  <Tab title="У меня есть коллектор">
    <Steps>
      ...
      <Step>
        ## Создайте пользователя для приёма данных

        <CreateIngestionUser />
      </Step>

      <Step>
        ## Адаптируйте конфигурацию коллектора
      ...
      <Step>
        ...
        <Info>
          **Чего ожидать**
          ...
          Каждый запуск `tg` длится около 20 секунд (задаётся через `--duration 20s`) и на протяжении всего этого времени выводит подробные записи журнала — это нормально; каждый запуск завершается сам, как только истекают эти 20 секунд. Приведённых ниже журналов достаточно, чтобы подтвердить работу конвейера; дополнительный расширенный набор включает ещё несколько запусков и занимает несколько минут.
        </Info>
        ...
        <Accordion title="Включите более подробную телеметрию (необязательно)">
          Отправьте трейсы с несколькими спанами из нескольких исправно работающих сервисов и одной зависимостью со сбоем. Так карта сервисов будет выглядеть реалистично: в основном всё работает нормально, но один сервис выдаёт ошибки, и при этом представления ошибок тоже будут заполнены:
          ...
          Для большего разнообразия добавьте ещё пару сервисов и экспоненциальную гистограмму:
