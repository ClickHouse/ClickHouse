...
В качестве альтернативы укажите клиенту пакет сертификатов certifi напрямую. [ClickHouse Connect](/ru/integrations/python) сам по себе не использует certifi в качестве резервного варианта, поэтому передайте пакет через параметр `ca_cert`:
...
См. [документацию по конфигурации клиента ClickHouse](/ru/interfaces/client#configuration_files).