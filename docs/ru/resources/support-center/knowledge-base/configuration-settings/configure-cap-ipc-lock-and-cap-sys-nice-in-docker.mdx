...
Вот как выглядят сообщения лога об отсутствии привилегий `CAP_SYS_NICE` или `CAP_SYS_NICE`:
...
2. Проверьте, что привилегии отображаются в контейнере с помощью следующей команды:

```bash
apt-get update > /dev/null && apt-get install -y libcap2-bin > /dev/null && capsh --print
```

В ответе будет примерно следующее:
...
3. Вручную задайте для ClickHouse обе привилегии

```bash
setcap "cap_ipc_lock=+ep cap_sys_nice=+ep" /usr/bin/clickhouse
```

4. Убедитесь, что привилегии применены.
...
5. Перезапустите сервер ClickHouse, и сообщения лога больше не будут отображаться.

<br />

Подробнее см. в [статье о привилегиях Linux](https://docs.docker.com/engine/reference/run/#runtime-privilege-and-linux-capabilities).