...
-   некоторые строки из CSV не могут быть считаны корректно, поскольку они начинаются с символов`\M/`, тогда как в CSV начинаться с обратной косой черты могут только символы `\N`, которые распознаются как `NULL` в SQL. Чтобы пропустить 10 некорректных записей, используется параметр `--input_format_allow_errors_num 10`;
-   массивы `ingredients`, `directions` `NER` представлены в необычном виде: они сериализуются в строку формата JSON, а затем помещаются в CSV — теперь они представлены в формате `String`. Чтобы преобразовать строку в массив используется функция [JSONExtract](../../sql-reference/functions/json-functions.md).
