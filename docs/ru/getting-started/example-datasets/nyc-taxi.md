...
Этот массив может быть получен двумя способами:

-   импорт из сырых данных;
-   скачивание готовых секций.
...
После скачивания получится порядка 227 Гб несжатых данных в CSV-файлах. Скачивание занимает порядка часа на соединении с пропускной способностью 1 Гбит/с (параллельное скачивание с s3.amazonaws.com задействует как минимум половину гигабитного канала).
...
(согласно записи в блоге Марка Литвинщика — это немногим более 1,1 млрд строк )
...
Слепок данных создаётся со скоростью около 50 Мб в секунду. Во время создания слепка, PostgreSQL читает с диска со скоростью около 28 Мб в секунду.
...
Она нужна для преобразование полей к более правильным типам данных и, если возможно, чтобы избавиться от пустых значений (NULL).
...
Для начала мы создадим таблицу на одном сервере. Позже мы сделаем таблицу распределённой.
...
Таблица заняла 126 Гб пространства хранения.
...
## Скачивание готовых секций {#skachivanie-gotovykh-sektsii}
...
Время выполнения — лучшее из трёх запусков.
...
Мы выполняли запросы с помощью клиента, расположенного в другом датацентре, не там где кластер, что добавляет порядка 20 мс задержки.
