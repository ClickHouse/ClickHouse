...
<Steps>
  ...
  <Step title={<>Проанализируйте системную таблицу <code>trace_log</code></>} id="analyze-trace-log-system-table">
    Чтобы анализировать системную таблицу `trace_log`, разрешите функции интроспекции с помощью настройки [`allow_introspection_functions`](/ru/reference/settings/session-settings/allow#allow_introspection_functions):
