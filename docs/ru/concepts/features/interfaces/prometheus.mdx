...
Список функций и операторов агрегации, поддерживаемых HTTP API, диалектом `promql` и табличными функциями, см. в разделе [Поддерживаемые возможности PromQL](/ru/sql-reference/table-functions/prometheusQueryRange#supported-promql-features).
...
ClickHouse использует один и тот же конвертер PromQL для HTTP API, диалекта `promql`, а также табличных функций [`prometheusQuery`](/ru/sql-reference/table-functions/prometheusQuery) и [`prometheusQueryRange`](/ru/sql-reference/table-functions/prometheusQueryRange).
