...
<Note>
  Реализованы только конечные точки запросов `/api/v1/query` и `/api/v1/query_range`, а также конечные точки метаданных `/api/v1/series` и `/api/v1/labels`. Для `/api/v1/series` требуется как минимум один селектор серий `match[]`; она поддерживает необязательные параметры `start`, `end` и `limit` и возвращает объединение серий, соответствующих каждому селектору. `/api/v1/labels` принимает те же параметры, при этом `match[]` необязателен, и возвращает отсортированные имена меток соответствующих серий (или всех серий, если селекторы не указаны). Конечная точка значений меток (`/api/v1/label/<name>/values`), используемая источником данных Prometheus в Grafana для просмотра меток, переменных шаблона и автодополнения в конструкторе запросов, не реализована и возвращает ошибку. Вводите выражения PromQL в режиме кода, а не в конструкторе запросов.
</Note>
