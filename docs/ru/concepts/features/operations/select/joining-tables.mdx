...
* **(3)** Если правая таблица помещается в память, используйте hash JOIN (`hash` и `parallel_hash` — псевдонимы; выбор одного из них не управляет параллелизмом). Обычно это самый быстрый вариант в памяти; скорость зависит от объёма данных, типов данных и распределения значений в столбцах ключа JOIN.
