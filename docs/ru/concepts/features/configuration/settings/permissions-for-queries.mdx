...
<Note>
  ...
  [ограничения на настройки](/ru/concepts/features/configuration/settings/constraints-on-settings).
</Note>
...
<Info>
  **KILL QUERY**
  ...
  `readonly` и `allow_ddl`. Однако требуется привилегия `SELECT` на `system.processes`. Завершение запроса,
  принадлежащего другому пользователю, а также любой `KILL QUERY ... ON CLUSTER` требуют привилегии `KILL QUERY`,
  которую `readonly = 1` и `readonly = 2` не допускают.
</Info>
