...
| [`timeout_overflow_mode`](/ru/reference/settings/session-settings/timeout-overflow-mode#timeout_overflow_mode)                                 | Задаёт, что делать, если запрос выполняется дольше `max_execution_time` или если расчётное время выполнения превышает `max_estimated_execution_time`.           |
