...
Репликация таблиц не поддерживается.
[Мутации](/ru/reference/statements/alter/index#mutations) и операции `ALTER`
поддерживаются только в том случае, если диск настроен с `enable_hard_links`, см. ниже.

Этот тип диска можно использовать для нереплицируемых таблиц `MergeTree`. Хотя
тип диска `s3` подходит для нереплицируемых таблиц `MergeTree`, можно выбрать
тип диска `s3_plain_rewritable`, если вам не нужны локальные метаданные
таблицы. Это может быть полезно, например, для системных таблиц.
...
#### Жёсткие ссылки на дисках plain rewritable {#plain-rewritable-hard-links}

Операции `ALTER` и мутации таблицы `MergeTree` используют жёсткие ссылки вместо
копирования, разделяя неизменившиеся файлы между частью и её преемником,
полученным в результате мутации. Настройка `enable_hard_links` (по умолчанию `0`)
позволяет диску с метаданными `plain_rewritable` создавать настоящие жёсткие
ссылки, что делает доступными следующие операции:
...
Жёсткая ссылка приводит к тому, что метаданные каталога явно перечисляют его файлы
вместе с ключами объектов, в которых они хранятся, благодаря чему файл может
ссылаться на объект, сохранённый под префиксом другого каталога. Прежде чем
включать эту возможность, учтите следующее:

* Серверы версий старше `26.9` не могут читать каталог в таком виде. Диск, на
  котором есть такие каталоги, становится нечитаемым после понижения версии, и его
  нельзя использовать совместно с более старыми серверами. Каталоги, в которых
  никогда не создавались жёсткие ссылки, сохраняют прежнюю структуру, поэтому диск
  остаётся читаемым для старых серверов вплоть до первой операции, создающей
  жёсткую ссылку, — мутации, `ALTER`, `FREEZE` или резервного копирования. Повторное
  отключение настройки не возвращает уже существующие каталоги к прежнему виду.
* Объект, у которого удалена последняя ссылка, удаляется в два шага: сначала
  перезаписываются метаданные каталога, а затем удаляется сам объект. Если в
  промежутке сервер будет завершён, объект останется в бакете, и на него ничего не
  будет ссылаться, поэтому его придётся удалить вручную.
