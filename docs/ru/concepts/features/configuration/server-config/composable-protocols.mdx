...
`default_session_user`, [настройку сервера](/ru/operations/server-configuration-parameters/settings)
со значением по умолчанию `default`.

Тег `<default_session_user>` переопределяет эту настройку для отдельной конечной точки. Это
позволяет разным портам прослушивания обслуживать разных анонимных пользователей:
