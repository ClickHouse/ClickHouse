...
<div id="copying-data-from-onelake">
  ## Копирование данных из OneLake
</div>

Процесс Copy &amp; Sync позволяет переносить таблицы OneLake в ClickHouse:

1. Откройте свой элемент ClickHouse и выберите **Copy &amp; Sync**
2. Выберите Lakehouse в рабочем пространстве
3. Выберите таблицы, которые хотите ускорить
4. Запустите копирование снимка данных на определённый момент времени

<Frame>
  ![Настройка Copy & Sync для создания новой целевой таблицы с ключом сортировки](/images/integrations/tools/data-integration/microsoft-fabric/fabric-03-copy-sync-table-selection.webp)
</Frame>
...
<Frame>
  ![Сопоставление столбцов Copy & Sync из исходной таблицы OneLake с существующей таблицей ClickHouse](/images/integrations/tools/data-integration/microsoft-fabric/fabric-04-copy-sync-existing-table.webp)
</Frame>
...
В период публичной предварительной версии Microsoft Fabric копирование данных из OneLake в ClickHouse бесплатно. Тарификация начнётся, когда рабочая нагрузка Fabric станет общедоступной. Информация о ценах будет опубликована, когда рабочая нагрузка достигнет стадии GA.
...
Нажмите **Open ClickHouse**, чтобы открыть полную консоль ClickHouse Cloud с входом через свою учетную запись Microsoft. Из рабочей нагрузки Fabric можно копировать данные в ClickHouse и выполнять любые SQL-операторы, но для административных задач необходимо перейти в Cloud Console, чтобы управлять сервисом.
...
Каждая операция Copy &amp; Sync из OneLake выполняется в виде ClickPipe. Отслеживайте ход копирования, пропускную способность и ошибки на [странице ClickPipes](/ru/integrations/clickpipes/monitoring) в консоли ClickHouse Cloud.
...
2. Откройте **Copy &amp; Sync**, выберите свой Lakehouse и таблицу `publicholidays`
...
* **Поддерживаются только таблицы Lakehouse.** Copy &amp; Sync считывает Delta-таблицы из Lakehouse в том же рабочем пространстве. Другие элементы Fabric (Warehouse, базы данных KQL, зеркальные базы данных) пока не поддерживаются в качестве источников.
