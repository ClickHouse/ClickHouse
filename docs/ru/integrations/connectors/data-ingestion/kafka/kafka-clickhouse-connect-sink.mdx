...
Начиная с версии ClickHouse 26.9 эта схема поддерживается по умолчанию. В версиях с 26.1 по 26.8 требуется настройка `allow_experimental_nullable_tuple_type = 1` (см. [эту страницу документации](/ru/reference/settings/session-settings/allow-experimental#allow_experimental_nullable_tuple_type)).
