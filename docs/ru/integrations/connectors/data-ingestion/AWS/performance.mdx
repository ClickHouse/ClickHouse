...
Количество таких параллельных потоков вставки можно настроить с помощью параметра [`max_insert_threads`](/ru/reference/settings/session-settings/max-insert#max_insert_threads). Значение по умолчанию — `1` для open-source ClickHouse и 4 для [ClickHouse Cloud](https://clickhouse.com/cloud).
