...
## Стриминг запросов {#streaming-queries}
...
### Примеры стриминга {#streaming-examples}

#### Поток строк {#stream-rows}
...
#### Потоковая передача блоков со строками {#stream-row-blocks}
...
#### Потоковая передача Pandas DataFrame {#stream-pandas-dataframes}
...
#### Потоковая передача батчей Arrow {#stream-arrow-batches}
...
#### Асинхронный стриминг строк {#async-stream-rows}
...
### DataFrames на базе Arrow {#arrow-backed-dataframes}
...
#### Запрос к DataFrame на базе Arrow {#query-to-arrow-backed-dataframe}
...
### Параметры форматов чтения (типы Python) {#read-format-options-python-types}
...
| Interval*             | int                     | string            | Значения представляют собой знаковые 64-битные счётчики в единицах соответствующего типа interval.                                                                                                                                         |
...
| Time64                | datetime.timedelta      | int, string, time | Поддерживаются scale от 0 до 9. Целочисленный формат возвращает ticks с точностью столбца. Python `timedelta` ограничен микросекундами.                                                                                                    |
...
| Tuple                 | dict or tuple           | tuple, dict, json | Именованные Tuple по умолчанию возвращают словари; неименованные Tuple возвращают кортежи. Значение `Tuple()` возвращает `()`.                                                                                                             |
| Map                   | dict                    | pairs             | По умолчанию используется `dict`. `pairs` возвращает список кортежей ключ/значение и сохраняет повторяющиеся ключи.                                                                                                                        |
...
| MultiPoint            | list[tuple]             | -                 | Каждый Point возвращается как 2-элементный кортеж.                                                                                                                                                                                         |
| Geometry              | tuple or list           | typed             | Значения Point представляются 2-элементными кортежами. Остальные элементы, включая MultiPoint, используют вложенные списки. Укажите `typed` с ключом `Geometry`, чтобы сохранить тип элемента. Настройки формата `Variant` не применяются. |
| QBit                  | list[float]             | -                 | Если установлен NumPy, он автоматически используется для более быстрого транспонирования битов.                                                                                                                                            |

`query_np`, `query_np_stream`, `query_df` и `query_df_stream` поддерживают для `Time64` scale 0, 3, 6 и 9. Остальные scale приводят к `ProgrammingError`, поскольку для них нет соответствующей единицы времени в NumPy. Чтобы сохранить такую точность, используйте обычный запрос Python с форматом чтения `int` или `string`. В документации экспериментального [Rust codec](/ru/integrations/language-clients/python/rust-codec#known-behavior-differences) описано исключение для `Time64`, хранящегося внутри `Dynamic`.

### Значения Map с повторяющимися ключами {#map-pairs}

Значения типа Map в ClickHouse могут содержать повторяющиеся ключи. Обычный словарь Python сохраняет для каждого ключа только последнее значение. Чтобы сохранить все пары в том порядке, в котором их возвращает сервер, выберите формат чтения `pairs`:
...
Этот формат применяется рекурсивно к значениям Map, вложенным в другие типы ClickHouse, включая Array, Tuple, типизированные пути JSON, Dynamic, Variant и другие Map. Он также работает с `query_np`, `query_df` и их потоковыми аналогами. Чтобы выбрать этот формат для Map только в одном столбце результата, используйте `column_formats={"m": {"Map": "pairs"}}`. Чтобы задать его глобально, используйте `set_read_format("Map", "pairs")`.

Это формат чтения. Для вставки Map в формате Native по-прежнему нужны словари; списки пар не принимаются. Чтобы сохранить дублирующиеся ключи без потерь при чтении и последующей записи, используйте `query_arrow` и `insert_arrow`. Форматы чтения не влияют на методы Arrow. Как и в случае других переопределений формата чтения, при `native_codec="rust"` используется резервная реализация на Python, а при `native_codec="rust_strict"` вызывается исключение `NotSupportedError`.
