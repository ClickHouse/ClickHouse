...
Если соединение прерывается во время чтения результата, вместо молчаливого возврата усечённого результата возникает `StreamFailureError`. Исключение содержит сообщение об ошибке на стороне сервера, если ClickHouse его вернул.
