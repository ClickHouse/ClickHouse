...
| FixedString           | bytes                   | string            | Если значение вставляется как строка, дополнительные байты будут заполнены нулями                                                                                     |
...
Python интерпретирует `datetime` без часового пояса в локальном часовом поясе системы при вызове `.timestamp()`. Из-за этого операции вставки зависят от среды выполнения. Рекомендуется использовать один из следующих подходов:

1. Используйте объект datetime с часовым поясом.
2. Убедитесь, что часовой пояс процесса — UTC.
3. Явно укажите нужный часовой пояс или преобразуйте значение в целое число epoch.
