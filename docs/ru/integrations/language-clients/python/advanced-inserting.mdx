...
## Вставка данных с помощью ClickHouse Connect: расширенные возможности {#inserting-data-with-clickhouse-connect--advanced-usage}
...
#### Параметры форматов записи {#write-format-options}
...
#### Значения Date и Date32 {#date-and-date32-values}
...
#### Вставка из Pandas DataFrame {#pandas-dataframe-insert}
...
#### Вставка DataFrame на базе Arrow (pandas 2.x) {#arrow-backed-dataframe-insert-pandas-2}
...
### Создать таблицу по схеме PyArrow {#create-table-from-pyarrow-schema}
...
#### Объекты datetime с часовым поясом {#timezone-aware-datetime-objects}
