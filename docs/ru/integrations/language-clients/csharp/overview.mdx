...
## Поддерживаемые версии ClickHouse {#supported-clickhouse-versions}
...
#### Базовое подключение {#basic-connection}
...
#### Пропуск запроса для определения схемы {#skip-schema-query}
...
#### Эволюция схемы {#poco-insert-schema-evolution}
...
#### Размещение запроса вставки {#insert-query-placement}
...
##### Регистрация {#poco-read-registration}
...
##### Сопоставление столбцов {#poco-read-column-matching}
...
##### Поддерживаемые типы свойств {#poco-read-types}
...
##### Материализация одной строки {#poco-read-mapto}
...
##### Конвертеры значений при чтении {#poco-read-converters}
...
##### Диагностика регистрации {#poco-read-diagnostics}
...
#### Плейсхолдеры `@name` в стиле ADO {#at-style-placeholders}
...
#### Параметры `Identifier` {#identifier-parameters}
...
### Пользовательское сопоставление типов параметров {#parameter-type-mapping}
...
### Пользовательское форматирование значений параметров {#parameter-value-formatting}
...
### Пользовательское преобразование значений при чтении {#read-value-conversion}
...
### Прямая потоковая передача {#raw-streaming}
...
### Сжатие передачи для отдельного запроса {#per-query-accept-encoding}
...
#### Настройка HttpClient {#per-query-accept-encoding-httpclient}
...
#### Тела ошибок {#per-query-accept-encoding-errors}
...
### Распаковка ответа {#response-decompression}
...
Готовый к запуску пример см. в [Select&#95;007&#95;ResponseCompression.cs](https://github.com/ClickHouse/clickhouse-cs/blob/main/examples/Select/Select_007_ResponseCompression.cs).

#### Сжатие вставок (запросов) {#insert-compression}
...
##### Пользовательский кодек {#custom-compressor}
...
#### Одно число, которое всё решает {#the-one-number-that-decides-it}
...
#### Ориентировочные рекомендации по вариантам развертывания {#rough-guide-by-deployment}
...
#### Измерение собственной точки перехода {#measuring-your-own-crossover}
...
### Вставка из сырого потока {#raw-stream-insert}
...
### Управление жизненным циклом с ClickHouseDataSource {#ado-net-datasource}
...
### Время жизни соединений и пул соединений {#best-practices-connection-lifetime}
...
### Сеансы {#best-practices-sessions}
...
#### Типы чисел с плавающей точкой {#type-map-reading-floating-points}
...
#### Десятичные типы {#type-map-reading-decimal}
...
#### Логический тип {#type-map-reading-boolean}
...
##### Типизированные пути и null-значения {#type-map-reading-json-nulls}
...
##### Строки внутри JSON-столбца {#type-map-reading-json-strings}
...
##### Пересекающиеся пути {#overlapping-paths}
...
#### Геометрические типы {#type-map-reading-geometry}
...
#### Типы с плавающей точкой {#type-map-writing-floating-point}
...
#### HTTP-параметры vs пакетная загрузка {#datetime-http-param-vs-bulkcopy}
...
#### Десятичные типы {#type-map-writing-decimal}
...
#### Не поддерживается при записи {#type-map-writing-not-supported}
...
#### Пример: Диагностика проблем с подключением {#logging-config-example}
...
### Режим отладки: сетевая трассировка и диагностика {#logging-debugmode}
...
### Чтение: выбор пути материализации {#perf-read-path}
...
### Вставка: размер батча и параллелизм {#perf-insert-batching}
...
### Сжатие: два направления передачи ведут себя по-разному {#perf-compression}
...
### Среда выполнения и сборка мусора {#perf-gc}
...
### Задержка: переиспользуйте соединения {#perf-latency}
...
#### Запросы в объекты POCO {#dapper-pocos}
...
#### Собственный синтаксис параметров ClickHouse {#dapper-clickhouse-param-syntax}
...
#### Пакетная вставка {#ef-core-bulk-insert}
...
#### Пользовательские преобразования типов {#ef-core-value-converters}
...
#### Аннотации типов столбцов {#ef-core-column-types}
...
### Tuple из 8+ элементов с вложенным кортежем на последней позиции {#valuetuple-caveat}
