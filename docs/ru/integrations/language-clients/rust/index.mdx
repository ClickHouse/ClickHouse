...
* `lz4` (включена по умолчанию) — включает варианты `Compression::Lz4` и `Compression::Lz4Hc(_)`. Если эта возможность включена, `Compression::Lz4` по умолчанию используется для всех запросов, кроме `WATCH`, который принимается только ClickHouse версий ниже v26.9.
...
* `watch` — включает функциональность `client.watch`. Она выполняет запрос `WATCH`, который был удалён в ClickHouse v26.9 вместе с `WINDOW VIEW`, поэтому работает только с более ранними серверами.
...
### Асинхронная вставка (батчинг на стороне сервера) {#async-insert-server-side-batching}
...
### Возможность Inserter (батчинг на стороне клиента) {#inserter-feature-client-side-batching}
...
### Выполнение DDL-запросов {#executing-ddls}
...
### Собственный HTTP-клиент {#custom-http-client}
...
Крейт предоставляет утилиты для мокирования сервера CH и тестирования запросов DDL, `SELECT`, `INSERT` и `WATCH` (`WATCH` принимается только ClickHouse версий ниже v26.9). Эту функциональность можно включить, активировав возможность `test-util`. Используйте её **только** как зависимость для разработки.
