...
Вместо пароля можно выполнять аутентификацию пользователя ClickPipes с помощью доступа на основе ролей AWS IAM. Создайте пользователя, как показано ниже, а затем выдайте ему те же привилегии на схему и право на репликацию, что указаны выше:
...
См. [AWS IAM DB authentication (RDS/Aurora)](/ru/integrations/clickpipes/postgres/auth), где приведены полные шаги по настройке роли IAM и политики, необходимых ClickPipes для аутентификации таким способом.
