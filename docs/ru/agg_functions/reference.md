...
Возвращает произвольное значение из столбца, которое встречается большое количество раз.
...
Рекомендуем использовать значения `N <= 10`, поскольку на небольших `N` функция выполняет задачу эффективнее, чем алгоритм, основанный на сортировке. Максимально возможное значение `N = 65536`.
...
**Потребление памяти**
 - `N * (the_size_of_the_key + 16)` байт для счетчиков.
 - `48 * N` байт для отображения (map) альфа-значений.
