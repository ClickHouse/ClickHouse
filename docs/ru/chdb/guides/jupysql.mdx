...
С помощью условия `REPLACE` приведём столбцы с денежными значениями к типу `Float64`, а с помощью функции [`transform`](/ru/sql-reference/functions/other-functions#transform) преобразуем числовой столбец `pickup_borocode` в понятное человеку название боро:
