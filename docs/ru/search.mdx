...
export const globalTransformSource = (source) => {
  ...
  if (isPreview || /clickhouse\.com\/docs(\/|$)/.test(url)) {
    tabs.push('Документация');
  } else if (url.indexOf('github.com') !== -1 && /\/issues(\/|$)/.test(url)) tabs.push('Документация');
  else if (/\/blog(\/|$)/.test(url)) tabs.push('Документация');
  else if (url.indexOf('clickhouse.com') !== -1) tabs.push('Документация');
...
Ищите в документации ClickHouse и других ресурсах ClickHouse.
