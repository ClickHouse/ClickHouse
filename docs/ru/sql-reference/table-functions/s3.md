...
``` sql
s3(path [,access_key_id, secret_access_key [,session_token]] [,format] [,structure] [,compression] [,headers] [,extra_credentials])
```
...
-   `headers` - необязательный параметр. Позволяет передавать заголовки в запросе к S3. Передаётся в формате `headers(key=value)`, например: `headers('x-amz-request-payer' = 'requester')`.
-   `extra_credentials` - необязательный параметр. Позволяет указать ARN роли и `role_session_name` для `AssumeRole` (см. далее). Указывается в формате `extra_credentials(key=value)`.
