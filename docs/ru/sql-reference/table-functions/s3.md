...
При добавлении данных в таблицу S3 вы можете сохранить таблицу в нескольких файлах, указав выражение `PARTITION BY`. В этом случае для каждого значения выражения партиционирования создается отдельный файл. Сохранение таблицы в нескольких отдельных файлах повышает эффективность операций чтения.

**Примеры**

1. При использовании ID партиции в названии файла создаются отдельные файлы:
...
```
В результате данные будут записаны в три файла: `file_1.csv`, `file_10.csv` и `file_20.csv`.
