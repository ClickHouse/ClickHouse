...
:::warning
Проверка переполнения не реализована для Decimal128 и Decimal256. В случае переполнения, неверный результат всегда возвращается без выбрасывания исключения.
:::
