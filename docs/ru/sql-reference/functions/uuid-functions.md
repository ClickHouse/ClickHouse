...
Возвращает случайный и уникальный UUID, который генерируется при первом запуске сервера и сохраняется навсегда. Результирующий записанный файл находится в каталоге сервера ClickHouse, имеет идентификатор UUID и находится на том же уровне, что и файл статуса.
