...
Создает постоянный столбец со случайным значением.
...
-   `x` — [Выражение](../syntax.md#syntax-expressions), которое приводится к одному из [поддерживаемых типов данных](../data-types/index.md). Его значение используется, чтобы избежать [склейки одинаковых выражений](index.md#skleika-odinakovykh-vyrazhenii), если функция вызывается несколько раз в одном запросе. Необязательный параметр.
