...
!!! warning "Предупреждение"
    В отличие от [initialQueryID](#initial-query-id), функция `queryID` может возвращать различные значения для разных шардов (см. пример), что неверно для столбца констант.
