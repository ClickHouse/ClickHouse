...
Заполняет текущую строку слева пробелами или указанной строкой (несколько раз, если необходимо), пока результирующая строка не достигнет заданной длины. Соответствует MySQL функции `LPAD`.
...
-   `string` — входная строка, которую необходимо заполнить. [String](../data-types/string.md).
...
-   `pad_string` — строка, используемая для заполнения входной строки. [String](../data-types/string.md). Необязательный параметр. Если не указано, то входная строка заполняется пробелами.
...
Заполняет текущую строку слева пробелами или указанной строкой (несколько раз, если необходимо), пока результирующая строка не достигнет заданной длины. Соответствует MySQL функции `LPAD`. Функция [leftPad](#leftpad) измеряет длину строки в байтах, а функция `leftPadUTF8` — в кодовых точках Unicode.
...
Заполняет текущую строку справа пробелами или указанной строкой (несколько раз, если необходимо), пока результирующая строка не достигнет заданной длины. Соответствует MySQL функции `RPAD`.
...
Заполняет текущую строку слева пробелами или указанной строкой (несколько раз, если необходимо), пока результирующая строка не достигнет заданной длины. Соответствует MySQL функции `RPAD`. Функция [rightPad](#rightpad) измеряет длину строки в байтах, а функция `rightPadUTF8` — в кодовых точках Unicode.
