...
Она не обязательно на 100% соответствует всем HTML, XML или XHTML стандартам, но ее реализация достаточно точная и быстрая. Правила обработки следующие:

1. Комментарии удаляются. Пример: `<!-- test -->`. Комментарий должен оканчиваться `-->`. Вложенные комментарии недопустимы.
...
2. CDATA вставляется дословно. Примечание: CDATA специфична для XML/XHTML. Но она обрабатывается всегда, чтобы достичь наилучших результатов.
...
Примечание: тег без конца удаляется до конца текста: `<hello `
...
- Последовательные пробелы заменяются одним пробелом.
- Но если текст разделен другими элементами и в этом месте еще нет пробела, то он вставляется.
- Это может привести к неестественным примерам: `Hello<b>world</b>`, `Hello<!-- -->world` - в HTML нет пробелов, но функция вставляет их. Также следует учитывать такие примеры: `Hello<p>world</p>`, `Hello<br>world`. Такое поведение подходит для анализа данных, например, для преобразования HTML в мешок слов (bag of words).
