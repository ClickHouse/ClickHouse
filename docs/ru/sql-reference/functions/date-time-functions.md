...
Функция преобразования даты-с-временем в строку согласно заданному шаблону. Важно: шаблон является константным выражением, т.е. не получится использовать разные шаблоны в одной колонке.
...
| %G          | четырехзначный формат вывода года, рассчитывается согласно [стандарту ISO 8601](https://ru.wikipedia.org/wiki/ISO_8601), обычно используется вместе с %V  | 2018 |
