...
**Плейсхолдеры**
Используйте плейсхолдеры, чтобы определить шаблон для результирующей строки. Колонка «Пример» показывает работу функции для времени `2018-01-02 22:33:44`.
...
| %G          | четырехзначный формат для количества недель в ISO-годе, вычисляется по основанному на неделях году, который [определен в стандарте ISO 8601](https://ru.wikipedia.org/wiki/ISO_8601), обычно используется с %V  | 2018 |
| %g          | двухзначный формат года, согласно стандарту ISO 8601, сокращение от четырехзначной записи      | 18      |
