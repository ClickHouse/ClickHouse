...
Оператор `INTERSECT` возвращает только те строки, которые есть только в результатах первого и второго запросов. В запросах должны совпадать количество столбцов, их порядок и тип. Результат `INTERSECT` может содержать повторяющиеся строки.

Если используется несколько операторов `INTERSECT` и скобки не указаны, пересечение выполняется слева направо. У оператора `INTERSECT` более высокий приоритет выполнения, чем у `UNION` и `EXCEPT`.
