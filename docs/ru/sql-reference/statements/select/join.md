...
Оператор `OR` внутри секции `ON` работает, используя алгоритм хэш-соединения — на каждый агрумент `OR` с ключами соединений для `JOIN` создается отдельная хэш-таблица, поэтому потребление памяти и время выполнения запроса растет линейно при увеличении количества выражений `OR` секции `ON`.
