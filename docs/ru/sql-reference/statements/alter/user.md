...
Определяет пользователей или роли, которым разрешено получать [привилегии](../../../sql-reference/statements/grant.md#grant-privileges) от указанного пользователя при условии, что этому пользователю также предоставлен весь необходимый доступ опцией [GRANT](../../../sql-reference/statements/grant.md). Параметры секции `GRANTEES`:
...
Вы можете исключить любого пользователя или роль, используя выражение `EXCEPT`. Например, `ALTER USER user1 GRANTEES ANY EXCEPT user2`. Это означает, что если `user1` имеет некоторые привилегии, предоставленные с помощью параметра `GRANT`, он сможет предоставить их любому, кроме `user2`.
