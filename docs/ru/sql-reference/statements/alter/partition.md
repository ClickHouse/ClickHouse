...
-   Кортеж из выражений, совпадающий (в типах) с кортежем партиционирования. В случае ключа партиционирования из одного элемента, следует обернуть его в функцию `tuple(...)`. Например, `ALTER TABLE visits DETACH PARTITION tuple(toYYYYMM(toDate('2019-01-25')))`.
