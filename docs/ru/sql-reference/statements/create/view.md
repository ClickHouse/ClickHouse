...
Создаёт представление. Представления бывают [обычные](#normal), [материализованные](#materialized) (MATERIALIZED), [LIVE](#live-view) и [оконные] (#window-view) (последние две — экспериментальный функционал).
...
## Оконное представление [Экспериментальный функционал] {#window-view}

!!! important "Важно"
    Это экспериментальная возможность. Ее использование может повлечь потерю совместимости в будущих версиях.
    Чтобы пользоваться оконными представлениями, и запросами `WATCH`, включите настройку [allow_experimental_window_view](../../../operations/settings/settings.md#allow-experimental-window-view). Введите команду `set allow_experimental_window_view = 1`.
...
Оконное представление может агрегировать данные к временному окну и выводить результаты, когда окно готово к запуску. Оно хранит результаты частичной агрегации во внутренней (или заявленной) таблице, чтобы сократить время задержки, и может отправить результат обработки в заявленную таблицу или отправить уведомления с помощью запроса `WATCH`.

Создание оконного представления похоже на создание `MATERIALIZED VIEW`. Оконному представлению нужен внутренний движок для хранения промежуточных данных. Внутреннее хранилище использует `AggregatingMergeTree` в качестве движка по умолчанию.

### Функции оконного представления {#window-view-windowviewfunctions}

[Функции оконного представления](../../functions/window-view-functions.md) используют, чтобы получить нижний и верхний пределы окна записей. Оконное представление нужно использовать с функцией оконного представления.

### Атрибуты времени {#window-view-timeattributes}

Оконное представление поддерживает процессы **время обработки** и **время наступления события**.

**Время обработки** позволяет оконному представлению производить результаты, основанные на времени локального компьютера, и работает по умолчанию. Это самое простое понятие о времени, но оно не обеспечивает детерминизм. Атрибут время обработки можно определить, установив `time_attr` функции оконного представления в столбец таблицы или воспользовавшись функцией `now()`. Следующий запрос создает оконное представление со временем обработки.

``` sql
CREATE WINDOW VIEW wv AS SELECT count(number), tumbleStart(w_id) as w_start from date GROUP BY tumble(now(), INTERVAL '5' SECOND) as w_id
```

**Время наступления события** — это время, когда когда каждое обособленное событие наступает на своем устройстве. Это время обычно находится среди записей, когда событие сгенерировано. Обработка времени наступления события обеспечивает последовательность в результатах, даже в случае нестандартных событий или поздних событий. Оконное представление поддерживает обработку времени наступления события с помощью синтаксиса `WATERMARK`.

Оконное представление позволяет использовать три стратегии инкремента:

* `СТРОГО_ВОЗРАСТАЮЩИЙ`: Производит инкремент максимального наблюдаемого timestamp. Строки с timestamp меньше максимального — не поздние.
* `ВОЗРАСТАЮЩИЙ`: Производит инкремент максимального наблюдаемого timestamp минус 1. Строки с timestamp меньше максимального или равным ему — не поздние.
* `ОГРАНИЧЕННЫЙ`: ИНКРЕМЕНТ=ИНТЕРВАЛ. Производит инкременты максимального наблюдаемого timestamp минус конкретная задержка.

Следующие запросы служат примерами создания окна представления с `WATERMARK`:
...
По умолчанию, когда появляется инкремент, окно будет запущено, а элементы, прибывшие за инкрементом, будут опущены. Оконное представление поддерживает обработку поздних событий параметром `ALLOWED_LATENESS=INTERVAL`. Пример обработки задержки:

``` sql
CREATE WINDOW VIEW test.wv TO test.dst WATERMARK=ASCENDING ALLOWED_LATENESS=INTERVAL '2' SECOND AS SELECT count(a) AS count, tumbleEnd(wid) AS w_end FROM test.mt GROUP BY tumble(timestamp, INTERVAL '5' SECOND) AS wid;
```

Обратите внимание на то, что к элементам, произведенным за счет позднего запуска, нужно относиться как к обновленным результатам предыдущего вычисления. Вместо запуска в конце окон, оконное представление запустится немедленно после того, как случилось позднее событие. Таким образом, появится несколько выходных данных для одного окна. Пользователям нужно помнить об этих дублированных результатах или дедуплицировать их.

### Мониторинг новых окон {#window-view-monitoring}

Оконное представление поддерживает запрос `WATCH`, чтобы постоянно присоединять обрабатываемые результаты к консоли или с помощью синтаксиса `TO` вывести результаты в таблицу.

``` sql
WATCH [db.]name [LIMIT n]
```

Запрос `WATCH` работает похожим образом с `LIVE VIEW`. `LIMIT` можно уточнить, чтобы установить количество получаемых обновлений перед завершение запроса. 

### Параметры {#window-view-settings}

- `window_view_clean_interval`: Чистый интервал оконного представления в секундах до устаревших открытых данных. Система сохранит окна, которые были не до конца активированы в соответствии с временем системы или конфигурацией `WATERMARK`, остальные данные будут удалены.
- `window_view_heartbeat_interval`: Интервал хартбита в секундах, который показывает, что запрос `WATCH` работает.

### Пример {#window-view-example}

Предположим, нам нужно подсчитать число клик-логов в каждые 10 секунд в таблице логов под названием `data`. Вот ее структура:

``` sql
CREATE TABLE data ( `id` UInt64, `timestamp` DateTime) ENGINE = Memory;
```

Для начала мы создаем оконное представление с переворачивающимся окном и интервалом в 10 секунд:
...
Когда логи вставлены в таблицу `data`,
...
В качестве альтернативы, можно присоединить выходные данные к другой таблице с помощью синтаксиса `TO`.

``` sql
CREATE WINDOW VIEW wv TO dst AS SELECT count(id), tumbleStart(w_id) as window_start FROM data GROUP BY tumble(timestamp, INTERVAL '10' SECOND) as w_id
```

Другие примеры вы найдете в тестах с сохранением данных ClickHouse (они называются `*window_view*`).

### Использование оконного представления {#window-view-usage}

Оконное представление пригодится при следующих сценариях:

* **Мониторинг**: Собирает и подсчитывает метрики логов за определенный период времени и выводит результаты в таблицу назначения. Дашборд может использовать таблицу назначения в качестве таблицы-источника.
* **Анализ**: Автоматически собирает и предварительно обрабатывает данные в окне времени. Это может пригодиться при анализе большого числа логов. Предварительная обработка устраняет повторные вычисления в различных запросах и сокращает время задержки запроса.
