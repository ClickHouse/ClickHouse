...
- [lizard](https://github.com/inikep/lizard) (прежнее название `LZ5`) — реализует алгоритмы сжатия без потерь на основе 4 методов: `fastLZ4`, `LIZv1`, `fastLZ4 + Huffman` и `LIZv1 + Huffman`. Хотя библиотека `lizard` и основана на библиотеке `LZ4`, формат сжатия `lizard` не совместим с форматом `LZ4`. Высокая скорость сжатия и распаковки данных достигается без использования расширений `SSE` или `AVX`.
- [LZSSE](https://github.com/ConorStokes/LZSSE) — разработана для реализации `SSE` распаковки без ветвлений. Поддерживает три варианта: `LZSSE2` для сильного сжатия файлов с короткими литералами, `LZSSE4` для файлов, содержащих примерно поровну литералов и совпадений (серий), и `LZSSE8` для несильного сжатия данных с более длинными совпадениями. 
- [density](https://github.com/centaurean/density) — работает не с байтами, а с группами из 4 байтов. Для обработки данных активно использует регистры (а не оперативную память). Имеет "защиту от взрывного роста", которая существенно увеличивает скорость обработки несжимаемых исходных данных. 
