...
Эти кодеки не сжимают данные, вместо этого они зашифровывают данные на диске. Воспользоваться кодеками можно, только когда ключ шифрования задан параметрами [шифрования] (../../ru/operations/server-configuration-parameters/settings.md#server-settings-encryption). Отметим, ставить кодеки шифрования имеет смысл в самый конец цепочки кодеков, потому что зашифрованные данные, как правило, нельзя сжать релевантным образом.
...
Эти кодеки используют фиксированный одноразовый код. Таким образом, это детерминированное шифрование. Оно совместимо с поддерживающими дедупликацию движками, в частности, [ReplicatedMergeTree](../../ru/engines/table-engines/mergetree-family/replication.md). Однако у шифрования имеется недостаток: если дважды зашифровать один и тот же блок данных, шифротекст на выходе получится одинаковым, и злоумышленник, у которого есть доступ к диску, заметит эту эквивалентность (при этом доступа к содержимому он не получит).
...
!!! attention "Внимание"
    Если вы выполняете запрос SELECT с упоминанием конкретного значения в зашифрованном столбце (например, при использовании секции WHERE), это значение может появиться в [system.query_log](../../ru/operations/system-tables/query_log.md). Рекомендуем отключить логирование.
...
!!!note "Замечание"
    Если необходимо применить сжатие, это нужно эксплицитно прописать в запросе. Без этого произойдет лишь зашифровка данных.
