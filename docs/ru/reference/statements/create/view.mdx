...
* `SQL SECURITY`: `INVOKER` для обычных представлений и `DEFINER` для materialized view ([настраивается через settings](/ru/reference/settings/session-settings/default#default_normal_view_sql_security))
* `DEFINER`: `CURRENT_USER` ([настраивается через settings](/ru/reference/settings/session-settings/default#default_view_definer))
