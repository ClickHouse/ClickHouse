...
Параметризованные представления похожи на обычные представления, но могут создаваться с параметрами, значения которых определяются не сразу.
Эти представления можно использовать с табличными функциями: имя представления указывается как имя функции, а значения параметров — как её аргументы.
...
Поскольку параметризованное представление зависит от значений параметров, без них у него нет схемы.
...
При создании materialized view с `TO [db].[table]` также можно использовать `POPULATE` для дозагрузки целевой таблицы существующими исходными данными (целевая таблица может уже содержать данные; в этом случае дозагружаемые строки добавляются). `POPULATE` нельзя сочетать с `REFRESH`: [refreshable materialized view](#refreshable-materialized-view) заполняется при первом обновлении, поэтому `POPULATE` загрузил бы исходные данные дважды (вместо этого используйте `EMPTY`, чтобы пропустить первое обновление).
...
<Note>
  Атомарность требует, чтобы исходная таблица поддерживала чтение закреплённого снимка на фиксированный момент времени — это семейство `MergeTree` и `Memory`. Для любого другого источника (представления, `Distributed`, `Merge`, `Buffer`, семейства `Log` или таблицы не из базы данных `Atomic`) заполнение выполняется с устаревшим, неатомарным поведением (что фиксируется в журнале сервера): существующие данные считываются с отдельного, нескоординированного снимка, поэтому строки, вставленные во время заполнения, могут быть пропущены или продублированы. В таком случае создайте представление и выполните отдельный `INSERT ... SELECT`, если вам нужны точные данные. Установка `materialized_views_populate_atomically = 0` принудительно включает это устаревшее поведение для всех источников.

  Атомарное заполнение применяется только к обычному `CREATE MATERIALIZED VIEW`. `CREATE OR REPLACE` / `REPLACE MATERIALIZED VIEW ... POPULATE` всегда используют устаревшее, неатомарное заполнение.

  `POPULATE` не поддерживается для баз данных `Replicated` (для переопределения используйте `database_replicated_allow_heavy_create`) и не поддерживается в ClickHouse Cloud. Если он включён с помощью этого переопределения, заполнение всегда будет устаревшим и неатомарным — неудавшееся заполнение нельзя было бы согласованно откатить на всех репликах.
</Note>
...
Если `DEFINER`/`SQL SECURITY` не указаны, результат зависит от настройки сервера [`ignore_empty_sql_security_in_create_view_query`](/ru/reference/settings/server-settings/settings/other#ignore_empty_sql_security_in_create_view_query).
...
* `SQL SECURITY`: `INVOKER` для обычных представлений (настраивается через [`default_normal_view_sql_security`](/ru/reference/settings/session-settings/default#default_normal_view_sql_security)) и `DEFINER` для materialized view (настраивается через [`default_materialized_view_sql_security`](/ru/reference/settings/session-settings/default#default_materialized_view_sql_security))
* `DEFINER`: `CURRENT_USER` (настраивается через [`default_view_definer`](/ru/reference/settings/session-settings/default#default_view_definer))
...
Координация выполняется через Keeper. Путь znode определяется настройкой сервера [default&#95;replica&#95;path](/ru/reference/settings/server-settings/settings/default-replica#default_replica_path).
...
<Warning>
  **Устарело**
  ...
  Оконные представления устарели и могут быть удалены в будущих выпусках. Их использование по-прежнему требует включения настройки [allow&#95;experimental&#95;window&#95;view](/ru/reference/settings/session-settings/allow-experimental#allow_experimental_window_view) (`SET allow_experimental_window_view = 1`); для новых сценариев они не рекомендуются.
</Warning>
