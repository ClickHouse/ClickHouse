...
| `sslmode`                          | Режим TLS/SSL, передаваемый в `libpq`: `disable`, `allow`, `prefer`, `require`, `verify-ca` или `verify-full`. Если не задан, применяется значение `prefer` по умолчанию для `libpq`. Необязательно.             |
| `sslrootcert_pem`                  | Содержимое CA‑сертификата, по которому проверяется сертификат сервера PostgreSQL. Необязательно.                                                                                                                 |
| `sslcert_pem`                      | Содержимое клиентского сертификата для аутентификации на основе сертификатов. Необязательно.                                                                                                                     |
| `sslkey_pem`                       | Содержимое закрытого ключа, соответствующего `sslcert_pem`. Необязательно.                                                                                                                                       |
| `sslrootcert`, `sslcert`, `sslkey` | Те же учетные данные в виде путей к файлам на сервере. Допускается только для словаря, определенного в файле конфигурации сервера, или через именованную коллекцию, определенную в нем; см. ниже. Необязательно. |
...
<Note>
  `sslrootcert`, `sslcert` и `sslkey` указывают имена файлов, которые сервер открывает с использованием собственных привилегий, поэтому они допустимы только для словаря, определённого в файле конфигурации сервера, или через определённую в нём именованную коллекцию. Словарь, созданный запросом `CREATE DICTIONARY`, вместо этого должен передавать содержимое в `sslrootcert_pem`, `sslcert_pem` и `sslkey_pem`. Эти значения маскируются в журналах и запросах `SHOW` так же, как пароли.
</Note>