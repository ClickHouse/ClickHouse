...
| `ssl_ca`, `ssl_cert`, `ssl_key` | Те же учётные данные в виде путей к файлам на сервере. Разрешено только для словаря, определённого в файле конфигурации сервера; см. ниже. Необязательно.                                                                                                                                                                                                                                   |
...
<Note>
  `ssl_ca`, `ssl_cert` и `ssl_key` указывают имена файлов, которые сервер открывает со своими правами, поэтому они принимаются только для словаря, определённого в файле конфигурации сервера. Словарь, созданный запросом `CREATE DICTIONARY`, должен вместо этого передавать содержимое в `ssl_ca_pem`, `ssl_cert_pem` и `ssl_key_pem`. Эти значения маскируются в журналах и в запросах `SHOW`, так же как пароли.
</Note>
