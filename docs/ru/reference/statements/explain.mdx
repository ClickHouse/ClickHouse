...
<Note>
  Текущая версия `EXPLAIN ANALYZE` не поддерживает запросы, выполняемые в распределённом режиме.
</Note>
...
Давайте рассмотрим вывод. Сначала посмотрим на заголовок.
...
* `Read` — строки и несжатые байты, прочитанные из таблиц, а также пропускная способность — те же значения, которые в нижнем колонтитуле обычного запроса отображаются как &quot;Processed&quot;.
* `Peak memory` — пиковое потребление памяти запроса.
...
`<n>` — это количество процессоров на этапе. Большой разрыв между `median` и `max` указывает на неравномерное распределение нагрузки между параллельными процессорами.
