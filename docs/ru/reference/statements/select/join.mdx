...
<Note>
  Когда для [join&#95;algorithm](/ru/reference/settings/session-settings/join#join_algorithm) задано значение `partial_merge`, `RIGHT JOIN` и `FULL JOIN` поддерживаются только со strictness `ALL` (`SEMI`, `ANTI`, `ANY` и `ASOF` не поддерживаются).
</Note>
...
Тип JOIN по умолчанию можно переопределить с помощью настройки [`join_default_strictness`](/ru/reference/settings/session-settings/join#join_default_strictness).

Поведение сервера ClickHouse для операций `ANY JOIN` зависит от настройки [`any_join_distinct_right_table_keys`](/ru/reference/settings/session-settings/other#any_join_distinct_right_table_keys).
...
* [`partial_merge_join_rows_in_right_blocks`](/ru/reference/settings/session-settings/partial-merge#partial_merge_join_rows_in_right_blocks)
* [`join_on_disk_max_files_to_merge`](/ru/reference/settings/session-settings/join#join_on_disk_max_files_to_merge)
...
В настоящее время ClickHouse поддерживает `ALL/ANY/SEMI/ANTI INNER/LEFT/RIGHT/FULL JOIN` не только с условиями равенства, но и с условиями неравенства. Условия неравенства поддерживаются только для алгоритмов JOIN `hash`, `parallel_hash` и `grace_hash`. Условие неравенства, вычисляемое во время JOIN, не может содержать `arrayJoin`, поскольку оно должно сохранять количество строк; условие, применяемое только к одной стороне, и ключ равенства с `arrayJoin` извлекаются до JOIN и не затрагиваются; недизъюнктивное условие `ALL INNER JOIN` также не затрагивается, поскольку в этом случае условие применяется после JOIN. Если разворачивание зависит только от одной стороны, перенесите его в `ARRAY JOIN` в подзапросе перед JOIN; условие, аргумент `arrayJoin` которого читает столбцы с обеих сторон, необходимо реструктурировать.
...
<div id="null-values-in-join-keys">
  ## Значения NULL и NaN в ключах JOIN
</div>
...
Значения `NaN` в ключах `JOIN` типа float не подчиняются приведённому выше правилу для `NULL`.
Скалярное сравнение двух значений `NaN` (`NaN = NaN`) равно `0`, однако ключи `JOIN` сравниваются не по правилам скалярной семантики — ключи `NaN` могут сопоставляться.
Будут ли они фактически сопоставлены, зависит от деталей реализации, алгоритма JOIN, типа ключа и настроек сеанса.
Не полагайтесь на конкретное поведение.
Если необходимо, чтобы строки с `NaN` не сопоставлялись, преобразуйте их в значения `NULL`: `ON if(isNaN(A.id), NULL, A.id) = B.id`.
В `ASOF JOIN` столбец для поиска ближайшего совпадения сравнивается по порядку, который `NaN` не поддерживает — отфильтруйте такие строки с обеих сторон.
...
При JOIN таблиц могут появляться пустые ячейки. Настройка [join&#95;use&#95;nulls](/ru/reference/settings/session-settings/join#join_use_nulls) определяет, как ClickHouse заполняет эти ячейки.
...
* [max&#95;rows&#95;in&#95;join](/ru/reference/settings/session-settings/max-rows#max_rows_in_join) — Ограничивает количество строк в хеш-таблице.
* [max&#95;bytes&#95;in&#95;join](/ru/reference/settings/session-settings/max-bytes#max_bytes_in_join) — Ограничивает размер хеш-таблицы.

При достижении любого из этих ограничений ClickHouse действует в соответствии с настройкой [join&#95;overflow&#95;mode](/ru/reference/settings/session-settings/join#join_overflow_mode).
