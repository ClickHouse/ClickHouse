...
<Note>
  PostgreSQL `FROM unnest(...)`, `CROSS JOIN UNNEST(...)` и `LATERAL` не поддерживаются. Используйте вместо них `ARRAY JOIN`. Имя `unnest` (начиная с версии 26.5) является псевдонимом вызова функции `arrayJoin` (`SELECT unnest(arr)`), а не табличной функцией.
</Note>
...
[Вычисление функций с коротким замыканием](/ru/reference/settings/session-settings/short-circuit-function-evaluation#short_circuit_function_evaluation) — это возможность, которая оптимизирует выполнение сложных выражений в некоторых функциях, таких как `if`, `multiIf`, `and` и `or`. Она позволяет избежать потенциальных исключений, например деления на ноль, при выполнении этих функций.
