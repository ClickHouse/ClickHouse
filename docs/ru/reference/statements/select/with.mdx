...
**Пример 6:** Повторное использование определённого лямбда-функцией общего скалярного выражения в подзапросе

Определите общее скалярное выражение как лямбда-функцию, чтобы явно привязать его аргумент:
