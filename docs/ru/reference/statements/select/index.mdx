---
description: 'Документация по SELECT-запросу'
sidebarTitle: 'Обзор'
...
Особенности каждой необязательной секции описаны в отдельных разделах, перечисленных в том же порядке, в котором они выполняются (за исключением `SELECT` и `DISTINCT`, которые для удобства указаны первыми: их выражения вычисляются только после завершения предыдущих секций, как описано [ниже](#select-clause)):

* [конструкция WITH](/ru/reference/statements/select/with)
* [секция SELECT](#select-clause)
* [секция ALL](/ru/reference/statements/select/all)
...
* [секция ARRAY JOIN](/ru/reference/statements/select/array-join)
* [секция JOIN](/ru/reference/statements/select/join)
* [секция PREWHERE](/ru/reference/statements/select/prewhere) (фильтрует единственную таблицу, к которой применяется, поэтому выполняется до `JOIN`, несмотря на свою позицию в этом списке)
...
* [предложение ORDER BY](/ru/reference/statements/select/order-by)
...
Запрос также можно записать в виде линейной цепочки преобразований с [конвейерными операторами](/ru/reference/statements/select/pipe-operators):
...
В секциях `GROUP BY`, `ORDER BY` и `LIMIT BY` можно использовать позиционные аргументы. Чтобы включить эту возможность, активируйте настройку [enable&#95;positional&#95;arguments](/ru/reference/settings/session-settings/enable-positional-arguments#enable_positional_arguments). Тогда, например, `ORDER BY 1,2` будет сортировать строки таблицы сначала по первому, а затем по второму столбцу.
