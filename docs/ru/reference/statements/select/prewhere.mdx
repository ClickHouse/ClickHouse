...
Это предложение фильтрует выражение одной таблицы, к которому оно применяется, обычно во время чтения этой таблицы. При ручном управлении `PREWHERE` его следует использовать для условий фильтрации, которые задействуют лишь небольшую часть столбцов в запросе, но при этом обеспечивают сильную фильтрацию данных. Это уменьшает объем считываемых данных.

Запрос может одновременно содержать `PREWHERE` и `WHERE`. В этом случае `PREWHERE` выполняется перед `WHERE`.

Если настройка [optimize&#95;move&#95;to&#95;prewhere](/ru/reference/settings/session-settings/optimize-move-to-prewhere#optimize_move_to_prewhere) установлена в 0, эвристики, автоматически переносящие части выражений из `WHERE` в `PREWHERE`, отключаются.

Если запрос имеет модификатор [FINAL](/ru/reference/statements/select/from#final-modifier), оптимизация `PREWHERE` не всегда корректна. Она включается только в том случае, если включены обе настройки: [optimize&#95;move&#95;to&#95;prewhere](/ru/reference/settings/session-settings/optimize-move-to-prewhere#optimize_move_to_prewhere) и [optimize&#95;move&#95;to&#95;prewhere&#95;if&#95;final](/ru/reference/settings/session-settings/optimize-move-to-prewhere#optimize_move_to_prewhere_if_final).
...
<Note>
  `PREWHERE` относится к выражению одной таблицы, поэтому оно фильтрует этот входной набор до выполнения [JOIN](/ru/reference/statements/select/join). `WHERE` вычисляется для результата JOIN, и оптимизатор может перенести его ниже JOIN, если это возвращает те же строки. Поэтому одно и то же условие может давать разные результаты в этих двух предложениях.

  Например, `SELECT a.id FROM a LEFT JOIN b ON a.id = b.x PREWHERE b.y > 50` эквивалентен `SELECT a.id FROM a LEFT JOIN (SELECT * FROM b WHERE y > 50) AS b ON a.id = b.x`, а не тому же запросу с `WHERE b.y > 50`: строки `b` удаляются до выполнения JOIN, поэтому строки `a`, утратившие соответствие, возвращаются, а не отбрасываются, при этом столбцы `b` заполняются согласно настройке [join&#95;use&#95;nulls](/ru/reference/settings/session-settings/join#join_use_nulls).
</Note>
