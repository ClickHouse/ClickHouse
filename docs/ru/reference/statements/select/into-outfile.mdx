...
`level` — числовой литерал. Поддерживаются положительные целые числа в следующих диапазонах: `1-12` для типов `gzip`, `deflate` и `lz4`, `1-22` для типа `zstd` и `1-9` для остальных типов сжатия. Для `gzip` и `deflate` уровни выше `9` требуют сборки по умолчанию с `libdeflate`; сборка без `libdeflate` поддерживает уровни `1-9`.
