...
Само по себе определение гипотетического индекса ничего не даёт — чтобы понять, как он повлияет на запрос, выполните [`EXPLAIN WHATIF`](/ru/reference/statements/explain#explain-whatif) для репрезентативного `SELECT`. Оценщик показывает применимость каждого кандидатного индекса, количество читаемых меток, итоговую долю пропуска и способ получения оценки (`empirical`, `statistical` или `applicability_only`).
