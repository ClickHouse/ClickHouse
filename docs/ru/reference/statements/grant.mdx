...
* `SYSTEM`. Уровень: `GROUP`
  ...
  * `SYSTEM BACKGROUND`. Уровень: `GROUP`
    * `SYSTEM STREAMING ENGINES`. Уровень: `TABLE`
    * `SYSTEM VIEWS`. Уровень: `VIEW`. Псевдонимы: `SYSTEM REFRESH VIEW`, `SYSTEM START VIEWS`, `SYSTEM STOP VIEWS`, `SYSTEM START VIEW`, `SYSTEM STOP VIEW`, `SYSTEM PAUSE VIEWS`, `SYSTEM PAUSE VIEW`, `SYSTEM CANCEL VIEW`, `REFRESH VIEW`, `START VIEWS`, `STOP VIEWS`, `START VIEW`, `STOP VIEW`, `PAUSE VIEWS`, `PAUSE VIEW`, `CANCEL VIEW`
...
[`display_secrets_in_show_and_select` настройка сервера](/ru/reference/settings/server-settings/settings/other#display_secrets_in_show_and_select),
и
[`format_display_secrets_in_show_and_select` настройка формата](/ru/reference/settings/formats/format#format_display_secrets_in_show_and_select).
...
Разрешает использовать указанный движок таблицы при создании таблицы. Относится к [движкам таблицы](/ru/reference/engines/table-engines/index).
...
Для некоторых движков таблицы с внешними источниками могут требоваться разрешения `READ`/`WRITE` на соответствующий источник. См. [Источники](#sources).
