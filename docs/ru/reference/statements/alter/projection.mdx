...
Проекции могут включать предложение `WHERE`, чтобы хранить только подмножество строк. Это полезно, когда запросы часто используют известный предикат фильтрации — проекция материализует только подходящие строки, уменьшая объём хранимых данных и повышая производительность запросов.
...
Когда `WHERE`-предложение запроса **подразумевает** `WHERE`-предложение проекции (то есть каждое условие в фильтре проекции также присутствует в фильтре запроса), оптимизатор может автоматически использовать проекцию, если сочтёт это целесообразным:
...
Проверка импликации консервативна — она использует точное сопоставление конъюнктов в канонической форме выражения. Из-за этого она может пропустить некоторые допустимые возможности оптимизации (например, импликации для диапазонов), но никогда не приведёт к неверным результатам.
...
Используйте оператор ниже, чтобы добавить описание проекции к метаданным таблицы:
...
<Note>
  Когда проекция задаёт предложение `WHERE`, материализуются только строки, соответствующие предикату. Оптимизатор может использовать такую проекцию, если `WHERE` в запросе логически следует из `WHERE` проекции и эта проекция выгодна для плана запроса. Это относится и к обычным, и к агрегатным проекциям.
</Note>
...
Используйте приведённый ниже оператор, чтобы изменить предложение [`WITH SETTINGS`](#with-settings) существующей проекции без пересоздания её данных:
...
Для [индекса-проекции](/ru/reference/engines/table-engines/mergetree-family/mergetree#projection-index) вместо `SELECT`-запроса повторно укажите объявление `INDEX`:
...
Оператор повторно указывает полное определение проекции, но от существующего определения может отличаться только предложение `WITH SETTINGS`.
Сам запрос проекции (или, для индекса-проекции, выражение и тип индекса) должен оставаться прежним, поскольку существующие части проекции содержат построенные на его основе данные; чтобы изменить его, используйте [`DROP PROJECTION`](#drop-projection), а затем [`ADD PROJECTION`](#add-projection).

Команда изменяет только метаданные таблицы и не переписывает данные: существующие части проекции сохраняют настройки, с которыми были записаны, тогда как части проекции, созданные последующими вставками и слияниями, используют новые настройки.
Чтобы перестроить существующие части с новыми настройками, выполните [`MATERIALIZE PROJECTION`](#materialize-projection).
...
Команды `ADD`, `MODIFY`, `DROP` и `CLEAR` считаются легковесными, поскольку изменяют только метаданные или удаляют файлы.
...
Начиная с v24.8, новый параметр уровня таблицы [`deduplicate_merge_projection_mode`](/ru/reference/settings/merge-tree-settings/other#deduplicate_merge_projection_mode) управляет поведением в случае, если вышеупомянутые нетривиальные фоновые операции слияния происходят в частях исходной таблицы.

Мутации удаления — ещё один пример операций слияния частей, при которых удаляются строки в частях исходной таблицы. Начиная с v24.7, также есть параметр для управления поведением в отношении мутаций удаления, запускаемых легковесными удалениями: [`lightweight_mutation_projection_mode`](/ru/reference/settings/merge-tree-settings/other#lightweight_mutation_projection_mode).
