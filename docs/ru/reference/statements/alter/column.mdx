...
Добавляет в таблицу новый столбец с указанными `name`, `type`, [`кодек`](/ru/reference/statements/create/table/codec) и `default_expr` (см. раздел [Выражения по умолчанию](/ru/reference/statements/create/table#default_values)). Модификаторы, следующие после типа, можно записывать в любом порядке, каждый — не более одного раза — см. [описание столбца](/ru/reference/statements/create/table#with-explicit-schema).
...
Модификаторы после типа можно указывать в любом порядке, при этом каждый из них — не более одного раза; см. [описание столбца](/ru/reference/statements/create/table#with-explicit-schema).
...
Примеры изменения кодеков сжатия столбцов см. в разделе [Column Compression Codecs](/ru/reference/statements/create/table/codec).
...
<Note>
  `MATERIALIZE COLUMN` всегда перезаписывает **значения столбца**. Перестраиваются ли при этом [индексы пропуска данных](/ru/reference/statements/alter/skipping-index) (включая текстовые индексы), зависит от структуры части и способа хранения индекса:

  * В частях со структурой **wide + full-storage** **автономные** файлы индексов пропуска данных (и текстовые индексы) **не** перестраиваются автоматически, если перезаписывается только столбец. После изменения данных исходного столбца, от которых зависит индекс, выполните [`ALTER TABLE ... MATERIALIZE INDEX`](/ru/reference/statements/alter/skipping-index#materialize-index), если требуется обновить эти файлы индексов.
  * **Исключение:** обычные индексы пропуска данных, упакованные в `skp_idx.packed` (небольшие подстримы индексов пропуска данных, размер которых не превышает значения [`packed_skip_index_max_bytes`](/ru/reference/settings/merge-tree-settings/other#packed_skip_index_max_bytes) по умолчанию; полнотекстовые индексы таким образом не упаковываются), принудительно пересчитываются даже в частях wide + full-storage.
  * В **любой части, структура которой отличается от wide + full-storage** (включая **compact + full**, **compact + packed** и **wide + packed**), мутация выполняется по пути полной перезаписи части и принудительно пересчитывает существующие в ней вторичные индексы. Небольшие части обычно имеют структуру compact, но при этом по умолчанию используют полное хранилище части, поэтому этот вариант типичен для недавно выполненных вставок.

  Кроме того, `ADD INDEX` обновляет только метаданные: он **не** материализует новый индекс (включая текстовые индексы) в исторических частях сразу. Исторические части материализуются либо явной командой [`MATERIALIZE INDEX`](/ru/reference/statements/alter/skipping-index#materialize-index), либо при последующем слиянии, если включён [`materialize_skip_indexes_on_merge`](/ru/reference/settings/merge-tree-settings/materialize#materialize_skip_indexes_on_merge) и индекс не указан в [`exclude_materialize_skip_indexes_on_merge`](/ru/reference/settings/merge-tree-settings/exclude#exclude_materialize_skip_indexes_on_merge). Явный `MATERIALIZE INDEX` — детерминированный способ построить индекс, не дожидаясь слияния и его настроек.
</Note>

**См. также**

* [MATERIALIZED](/ru/reference/statements/create/view#materialized-view).
* [MATERIALIZE INDEX](/ru/reference/statements/alter/skipping-index#materialize-index).
