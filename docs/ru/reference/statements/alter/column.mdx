...
Добавляет в таблицу новый столбец с указанными `name`, `type`, [`codec`](/ru/reference/statements/create/table/codec) и `default_expr` (см. раздел [Выражения по умолчанию](/ru/reference/statements/create/table#default_values)).
...
<Note>
  `MATERIALIZE COLUMN` всегда перезаписывает **значения столбца**. Перестраиваются ли при этом [индексы пропуска данных](/ru/sql-reference/statements/alter/skipping-index) (включая текстовые индексы), зависит от структуры части и способа хранения индекса:

  * В частях со структурой **wide + full-storage** **автономные** файлы индексов пропуска данных (и текстовые индексы) **не** перестраиваются автоматически, если перезаписывается только столбец. После изменения данных исходного столбца, от которых зависит индекс, выполните [`ALTER TABLE ... MATERIALIZE INDEX`](/ru/sql-reference/statements/alter/skipping-index#materialize-index), если требуется обновить эти файлы индексов.
