...
* `SETTINGS`: (Необязательно) Позволяет задать дополнительные настройки.
  * (Необязательно): [check&#95;query&#95;single&#95;value&#95;result](/ru/reference/settings/session-settings/check#check_query_single_value_result): Эта настройка определяет, будет ли вывод подробным (`0`) или сводным (`1`).
...
Чтобы проверить состояние всей базы данных, а не целостность отдельных таблиц, используйте запрос [`CHECK DATABASE`](/ru/reference/statements/check-database). Он в первую очередь предназначен для баз данных с движком [`DataLakeCatalog`](/ru/reference/engines/database-engines/datalake), где проверяет, доступен ли внешний каталог.
