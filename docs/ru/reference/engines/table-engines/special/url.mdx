...
* Параметр `URL` должен соответствовать структуре Uniform Resource Locator. Для URL `http`/`https` (backend по умолчанию) он должен указывать на сервер, использующий HTTP или HTTPS, а для получения ответа от сервера не требуются дополнительные заголовки. URL с распознаваемой схемой, отличной от HTTP (`file://`, `s3://`, `az://`, `hdfs://`, …), вместо этого передаётся соответствующему движку — см. [Dispatching by URL scheme](#scheme-dispatch) ниже.
