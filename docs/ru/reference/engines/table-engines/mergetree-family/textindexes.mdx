...
#### Комбинированный поиск по паре ключ-значение с токенизатором `keyValuePairs` {#tokenizer-key-value-pairs}
...
#### Поиск по ключам и значениям с помощью `mapKeys` и `mapValues` {#text-index-map-keys-and-values}
