...
<div id="japanese-tokenizer-dictionary">
  ### Словарь японского токенизатора
</div>
...
³ `hasPhrase` для текстового индекса `splitByRegexp` **не** поддерживает постпроцессор: это сочетание приводит к исключению, поскольку переписывание на уровне строки в постпроцессоре предполагает токены в стиле `splitByNonAlpha`, разделённые пробелами. Без постпроцессора `splitByRegexp` полностью поддерживается функцией `hasPhrase`.
