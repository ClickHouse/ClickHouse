...
* `splitByRegexp(re)` разбивает строки по заданному пользователем разделителю в виде регулярного выражения `re` (см. функцию [splitByRegexp](/ru/reference/functions/regular-functions/splitting-merging-functions#splitByRegexp)).
  Аргумент `re` обязателен, например, `tokenizer = splitByRegexp('[^\p{L}\p{N}#+]+')`.
  В отличие от фиксированных строк-разделителей в `splitByString`, разделитель в виде регулярного выражения может сохранять токены со специальными символами (например, `C++` или `C#`), которые другие токенизаторы разделили бы на части.
...
`LIKE` также поддерживает вычисление через dictionary scan (включается через `use_text_index_like_evaluation_by_dictionary_scan`) для токенизаторов `splitByNonAlpha` и `array` без препроцессора и постпроцессора.
Шаблон `%value%` в этом случае обрабатывается как *прямое чтение (без подсказки)*, тогда как шаблоны `value%` и `%value` остаются подсказкой, см. [Запросы LIKE/ILIKE](#like-ilike-queries-perf).

² `ILIKE` поддерживается только через вычисление с помощью dictionary scan (`use_text_index_like_evaluation_by_dictionary_scan = 1`, токенизатор `splitByNonAlpha` или `array`).
Перехода к использованию индекса как подсказки для шаблонов, которые не поддерживает dictionary scan, нет: если настройка отключена или токенизатор не входит в поддерживаемый набор, индекс для `ILIKE` не используется.
...
#### Индексы для определённых подстолбцов {#json-indexes-on-subcolumns}
...
#### Индексы по путям с JSONAllPaths {#json-indexes-jsonallpaths}
...
### Фразовый поиск {#text-index-phrase-search}
...
### Тривиальные запросы подсчёта {#count-queries-perf}
