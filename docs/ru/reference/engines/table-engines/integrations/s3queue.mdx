...
* exclusive — В режиме exclusive не выполняется никакая синхронизация обработки файлов через ZooKeeper. В этом режиме url (первый параметр движка `S3Queue()`) обязательно должен указывать на уникальный хост или бакет. Такой режим применим в особых случаях, которые требуют высокой пропускной способности. 
