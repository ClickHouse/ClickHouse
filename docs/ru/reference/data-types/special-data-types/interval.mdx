...
`avg`, `min`, `max` и `any` по столбцу типа `Interval` возвращают интервал того же типа.
`avg` усредняет количество единиц и округляет результат до целого числа единиц:
...
Интервалы в разных единицах несоизмеримы — месяц не равен фиксированному числу дней, — поэтому
преобразование между ними невозможно, и каждый тип интервала агрегируется отдельно.
