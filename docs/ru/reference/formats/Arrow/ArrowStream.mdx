...
| `input_format_arrow_use_native_reader`                                       | Использовать встроенный reader ClickHouse для форматов `Arrow` и `ArrowStream` вместо библиотеки Apache Arrow. Установите `0`, чтобы использовать reader библиотеки Apache Arrow.                                                                   | `1`         |
...
| `output_format_arrow_unsupported_types_as_binary`                            | Выводить тип, у которого нет эквивалента в Arrow (например, `BFloat16`, `AggregateFunction`), как необработанные бинарные данные. Если false, такой тип вызовет исключение. Применяется и к встроенному writer, и к writer библиотеки Apache Arrow. | `1`         |
...
| `output_format_arrow_use_native_writer`                                      | Использовать встроенный writer ClickHouse для форматов `Arrow` и `ArrowStream` вместо библиотеки Apache Arrow. Установите `0`, чтобы использовать writer библиотеки Apache Arrow.                                                                   | `1`         |
