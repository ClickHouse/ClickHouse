...
Формат вывода `Hash` вычисляет одно хеш-значение для всех столбцов и строк результата.
Это полезно для вычисления «отпечатка» результата, например когда передача данных становится узким местом.
...
Прочитайте данные в формате `Hash`:
