...
Имя именованной коллекции, которую `aiEmbed` использует, если при вызове в карте параметров не переданы `credentials`. Пустое значение означает, что значения по умолчанию нет: в таких вызовах `credentials` нужно передавать явно. Эта настройка хранится отдельно от `ai_function_text_default_credentials`, поскольку конечная точка и модель для эмбеддингов отличаются от конечной точки и модели для чата.
...
Имя именованной коллекции, которую текстовые функции ИИ (`aiGenerate`, `aiClassify`, `aiExtract`, `aiTranslate`) используют, если при вызове в карте параметров не переданы `credentials`. Пустое значение означает, что значения по умолчанию нет: в таких вызовах `credentials` нужно передавать явно. Конечная точка и модель для chat-completions отличаются от конечной точки и модели для эмбеддинга, поэтому эта настройка отделена от `ai_function_embedding_default_credentials`.
...
<Note>
  ...
  Однако параметр compatibility можно переопределить на уровне пользователя, роли, профиль, запроса или сеанса с помощью стандартных механизмов настройки ClickHouse, например `SET compatibility = '22.3'` в сеансе или `SETTINGS compatibility = '22.3'` в запросе.
</Note>
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "21.1"},{"label": "1"},{"label": "По умолчанию использовать параллельные вставки с кворумом. Это значительно удобнее, чем использовать последовательные вставки с кворумом"}]}]} />

<Note>
  Этот параметр не применяется к SharedMergeTree; подробнее см. в разделе [консистентность SharedMergeTree](/ru/products/cloud/features/infrastructure/shared-merge-tree#consistency).
</Note>

Включает или отключает параллельное выполнение запросов `INSERT` с кворумом. Если параметр включен, дополнительные запросы `INSERT` можно отправлять, пока предыдущие еще не завершились. Если параметр отключен, дополнительные записи в ту же таблицу будут отклонены.
...
Для таблиц [MergeTree](/ru/reference/engines/table-engines/mergetree-family/mergetree). Чтобы уменьшить задержку при обработке запросов, блок сжимается при записи следующей метки, если его размер не менее `min_compress_block_size`. По умолчанию — 65 536.

Фактический размер блока, если объём несжатых данных меньше `max_compress_block_size`, будет не меньше этого значения и не меньше объёма данных для одной метки.
...
Указывает, какие алгоритмы порядка JOIN следует использовать при оптимизации плана запроса. Доступны следующие алгоритмы:
...
* &#39;dpsub&#39; - реализует алгоритм DPsub, который поддерживает как inner, так и non-inner JOIN - перебирает все возможные порядки JOIN и находит оптимальный, но может работать медленно для запросов с большим количеством таблиц и предикатов JOIN.
* &#39;dphyp&#39; - реализует алгоритм DPhyp (Dynamic Programming via Hypergraph Partitioning), в настоящее время только для inner JOIN - исследует то же пространство поиска, что и `dpsize`, но перебирает только пары связанных подграфов, что даёт меньше промежуточных JOIN в разреженных графах JOIN, ценой того, что декартовы произведения не рассматриваются
...
Включает или отключает оптимизацию чтения по порядку на уровне плана запроса.
Действует только если значение настройки [`query_plan_enable_optimizations`](#query_plan_enable_optimizations) равно 1.
...
Задаёт период таймера реального времени для [профилировщика запросов](/ru/concepts/features/performance/troubleshoot/sampling-query-profiler). Таймер реального времени отсчитывает астрономическое время.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "0.2"},{"label": "Переименовано из `text_index_density_threshold` (сохранено как псевдоним); выбирает алгоритм пересечения списков вхождений в режиме отложенного применения списков вхождений."}]}]} />

Порог плотности списка вхождений, определяющий алгоритм пересечения в режиме отложенного применения списков вхождений (`text_index_posting_list_apply_mode = 'lazy'`).
Ниже порога — leapfrog-пересечение (предпочтительно для разреженных списков вхождений). На пороге и выше — brute-force-пересечение битмапов (предпочтительно для плотных списков вхождений).
...
&#39;lazy&#39; использует декодирование по требованию на основе курсора (требует формат индекса с сериализованным кодеком).
