...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "1"},{"label": "Новая настройка, которая определяет, проверяет ли формат GeoJSON корректность геометрии по RFC 7946 (минимальное число точек в линии и кольце, замкнутость кольца, непустые мультигеометрии) при чтении и записи"}]}]} />

Определяет, проверяет ли формат `GeoJSON` корректность геометрии по RFC 7946 при чтении и записи.

Когда включено (по умолчанию), геометрия, нарушающая правила GeoJSON для геометрических объектов, отклоняется: `LineString` (или линия из `MultiLineString`) с менее чем двумя точками; кольцо `Polygon` или `MultiPolygon` с менее чем четырьмя точками либо с различающимися первой и последней точками (незамкнутое кольцо); а также пустой `MultiLineString`, `Polygon` или `MultiPolygon`. Это действует и при чтении (такой документ отклоняется), и при записи (такое значение ClickHouse отклоняется вместо создания документа, который входной формат отклонил бы при чтении).

Когда отключено, эти правила не проверяются: такие геометрии читаются и записываются как есть, поэтому вырожденные геометрии проходят цикл чтения и записи без изменений, но записанный документ может не быть корректным GeoJSON.

Проверка носит только структурный характер: она проверяет число точек и замкнутость кольца. Геометрическая корректность формы не проверяется — ориентация кольца (правило правой руки / порядок обхода) не контролируется, и структурно корректные, но геометрически вырожденные объекты допускаются, например полигон нулевой площади, самопересекающееся кольцо или полигон, чьи отверстия находятся вне внешнего кольца. Неконечные значения координат (`NaN`, `Inf`) всегда отклоняются независимо от этой настройки, поскольку их нельзя представить числами JSON.
...
Управляет тем, как значение `Nullable(String)` считывается из отсутствующего значения в CSV. Отсутствующее значение — это пустое место между запятыми, перед запятой или после неё, не заключённое в кавычки. Если эта настройка включена, то независимо от значения `input_format_csv_empty_as_default` отсутствующее значение `Nullable(String)` будет интерпретироваться как пустая `String`, а не как NULL.
...
Ограничивает максимальное время ожидания в миллисекундах перед выдачей блока при разборе данных во входных форматах на основе строк. `0` означает отсутствие ограничения.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "25.5"},{"label": "1"},{"label": "При чтении файлов Parquet пропускать целые группы строк по выражениям WHERE/PREWHERE и bloom-фильтр в метаданных Parquet."}]}, {"id": "row-2","items": [{"label": "24.10"},{"label": "0"},{"label": "При чтении файлов Parquet пропускать целые группы строк по выражениям WHERE/PREWHERE и bloom-фильтр в метаданных Parquet."}]}]} />

При чтении файлов Parquet пропускать целые группы строк по выражениям WHERE и bloom-фильтр в метаданных Parquet.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "1"},{"label": "Новая настройка. Когда включен output_format_csv_serialize_tuple_into_separate_columns, заголовок CSVWithNames\/CSVWithNamesAndTypes теперь разворачивает столбцы Tuple в их листовые поля, чтобы число столбцов в заголовке совпадало с данными. Установите false, чтобы вернуть прежний заголовок с одним именем."}]}]} />

Когда включен [output&#95;format&#95;csv&#95;serialize&#95;tuple&#95;into&#95;separate&#95;columns](#output_format_csv_serialize_tuple_into_separate_columns), строки заголовка `CSVWithNames` и `CSVWithNamesAndTypes` разворачивают каждый столбец Tuple в его листовые поля (имена с точечной нотацией, такие как `t.a`, `t.b`, и имена листовых типов), так что в заголовке столько же столбцов, сколько и в данных. Для `CustomSeparated*` такое разворачивание применяется только при `format_custom_escaping_rule = 'CSV'`, когда `format_custom_field_delimiter` состоит из одного символа и равен `format_csv_delimiter`; в противном случае (например, если используется разделитель табуляции по умолчанию или `format_custom_field_delimiter = '|'`) заголовок остается неразвернутым, чтобы по-прежнему соответствовать данным. Установите значение `0`, чтобы сохранить прежнее поведение, при котором в заголовке остается одно имя и один тип Tuple верхнего уровня.

Примечание: при `input_format_with_names_use_header = 1` развернутый заголовок не считывается обратно в Tuple по имени. Чтобы прочитать такие данные обратно в Tuple, либо установите для этой настройки значение `0` при выводе, либо читайте с `input_format_with_names_use_header = 0` (а для форматов `*WithNamesAndTypes` — `CSVWithNamesAndTypes` и `CustomSeparatedWithNamesAndTypes` — также `input_format_with_types_use_header = 0`, поскольку иначе развернутая строка типов проверяется на соответствие единственному входному полю Tuple верхнего уровня и отклоняется).
