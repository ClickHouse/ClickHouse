...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.9"},{"label": "sainte_lague"},{"label": "Сохранять распределение Sainte-Lague независимо от `compatibility`."}]}, {"id": "row-2","items": [{"label": "26.8"},{"label": "sainte_lague"},{"label": "Включить распределение Sainte-Lague по умолчанию."}]}, {"id": "row-3","items": [{"label": "26.7"},{"label": "water_filling"},{"label": "Новая настройка, которая определяет, какой алгоритм координатор слияний использует для распределения слияний между репликами"}]}]} />
