...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.9"},{"label": "0"},{"label": "Новая настройка, позволяющая хранить небольшие файлы части blob-list непосредственно в консолидированном blobs.list вместо отдельных blob-объектов"}]}]} />

Хранит файлы части blob-list размером не более указанного числа байтов (например,
count.txt, metadata&#95;version.txt, индексы minmax) непосредственно в
консолидированном файле blobs.list вместо отдельного blob-объекта
в объектном хранилище, экономя одну операцию записи и чтения blob-объекта для каждого небольшого файла. Значение 0 отключает встраивание. Настройка
действует только для частей, использующих хранилище blob-list (см.
shared&#95;merge&#95;tree&#95;use&#95;blobs&#95;list&#95;for&#95;parts). Доступно только в ClickHouse Cloud.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.8"},{"label": "sainte_lague"},{"label": "Включить распределение Sainte-Lague по умолчанию."}]}, {"id": "row-2","items": [{"label": "26.7"},{"label": "water_filling"},{"label": "Новая настройка, которая определяет, какой алгоритм координатор слияний использует для распределения слияний между репликами"}]}]} />

Алгоритм, используемый потоком координатора слияний для распределения слияний между репликами.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.9"},{"label": "0"},{"label": "Новая настройка: карта blob-объектов для каждого файла части SharedMergeTree хранится в одном объединённом узле Keeper вместо отдельного узла для каждого файла"}]}]} />

Храните части таблицы SharedMergeTree (как Wide, так и Compact) в виде одного
объединённого узла метаданных blob-list вместо отдельного узла Keeper для каждого файла.
Сокращает объём метаданных Keeper для каждой части с O(файлов) до небольшой постоянной величины,
сохраняя один blob-объект в объектном хранилище для каждого файла. Доступно только в ClickHouse Cloud.
