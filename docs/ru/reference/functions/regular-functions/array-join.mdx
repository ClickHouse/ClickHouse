...
`unnest` (начиная с версии 26.5) — регистронезависимый псевдоним `arrayJoin` в форме вызова функции (`SELECT unnest(arr)`). Синтаксис PostgreSQL для указания источника таблицы не поддерживается (`FROM unnest(...)`, `CROSS JOIN UNNEST(...)` и `LATERAL`). Для таких запросов используйте предложение [`ARRAY JOIN`](/ru/reference/statements/select/array-join).
...
Есть одно исключение: `arrayJoin`, в том числе через его псевдоним `unnest`, нельзя использовать в условии `JOIN ON`, которое вычисляется в ходе JOIN, поскольку такое условие должно сохранять количество строк и вычисляется отдельно для каждого батча объединяемых строк. Условие, применяемое только к одной стороне, и ключ равенства по `arrayJoin` извлекаются до JOIN и не затрагиваются. Недизъюнктивное условие `ALL INNER JOIN` также не затрагивается, поскольку в этом случае условие применяется после JOIN. Если разворачивание зависит только от одной стороны, перенесите его в `ARRAY JOIN` в подзапросе перед JOIN; условие, аргумент `arrayJoin` которого использует столбцы с обеих сторон, необходимо переработать.
