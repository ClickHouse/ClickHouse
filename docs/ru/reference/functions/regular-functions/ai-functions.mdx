...
Вычисляет эмбеддинги обоих текстов и возвращает косинусное сходство двух векторов
в диапазоне `[-1, 1]`: `1` означает, что тексты семантически идентичны, `0` — что они не связаны,
а отрицательные значения указывают на противоположность. Это дополнение `cosineDistance` для
тех же эмбеддингов (`aiSimilarity = 1 - cosineDistance(embedding1, embedding2)`).
