...
### Выбор учетных данных {#selecting-credentials}
...
### Карта параметров {#parameter-map}
...
### Безопасность передачи данных (HTTP vs HTTPS) {#transport-security}
...
Генерирует эмбеддинг-вектор для заданного текста с использованием настроенного провайдера ИИ.
...
<Note>
  Эта функция недетерминированная: она может возвращать разные результаты для одинаковых аргументов.
</Note>
...
<Note>
  Эта функция недетерминированная: при одинаковых аргументах она может возвращать разные результаты.
</Note>
...
* `params` — Необязательный константный `Map(String, String)` с параметрами. Специфичные для функции ключи: `temperature` (температура сэмплирования, управляющая случайностью; по умолчанию `0.7`), `max_tokens` (максимальное количество выходных токенов за один вызов; по умолчанию `1024`), `system_prompt` (константная системная инструкция, определяющая поведение модели; по умолчанию — общий промпт ассистента). Также применяются общие параметры `credentials` и `model` (см. [функции ИИ](/ru/reference/functions/regular-functions/ai-functions)). [`Map(String, String)`](/ru/reference/data-types/map)
...
<Note>
  Эта функция недетерминированная: она может возвращать разные результаты при одинаковых аргументах.
</Note>
