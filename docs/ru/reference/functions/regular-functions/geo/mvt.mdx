...
* `buffer` — Необязательный буфер отсечения в пикселях, в диапазоне `[0, 2147483647]`. По умолчанию `1`. [`UInt32`](/ru/reference/data-types/int-uint).
...
Возвращает геометрию в пространстве тайла или `NULL`, если она полностью обрезается. [`Geometry`](/ru/reference/data-types/geo).
