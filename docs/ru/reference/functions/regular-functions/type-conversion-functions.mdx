...
Поддерживает преобразование из `String`, `FixedString`, `Date`, `Date32`, `Time`, `DateTime`, `DateTime64` или числовых типов (`(U)Int8`, `(U)Int16`, `(U)Int32`, `(U)Int64`, `(U)Int128`, `(U)Int256`, `BFloat16`, `Float32`, `Float64`). Значения `Decimal` и `Time64` не поддерживаются и приводят к исключению; значение типа `Time` выполняет циклическое переполнение вместо насыщения.
