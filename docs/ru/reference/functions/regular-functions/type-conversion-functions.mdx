...
## Распространённые проблемы при преобразовании данных {#common-issues-with-data-conversion}
...
Проверяет значение на соответствие объявленному типу Kusto Query Language и преобразует его в соответствующий
тип-носитель ClickHouse. Аргумент, тип которого не соответствует объявленному типу KQL,
отклоняется на этапе анализа, а значение, не укладывающееся в тип (например, при переполнении `int`), вызывает ошибку
для каждой строки. Второй аргумент — имя объявленного типа KQL, а третий описывает
параметр для сообщения об ошибке; оба должны быть постоянными строками.

Эта функция обеспечивает поддержку типизированных параметров функций, когда `dialect = 'kusto'`. Она не предназначена для
прямого вызова из SQL.
...
* `value` — аргумент, передаваемый при вызове функции KQL. - `type` — объявленное имя типа KQL, постоянная строка. - `parameter` — описание параметра, постоянная строка, используемая в сообщении об ошибке.

**Возвращаемое значение**

`value` в представлении ClickHouse для объявленного типа.
...
Преобразует значение в timespan (`Interval` в наносекундах) так же, как функция
`totimespan()` в Kusto Query Language: timespan передаётся без изменений, число трактуется как количество дней, а строка разбирается по шаблону
`[-][d.]hh:mm:ss[.fffffff]` — если разобрать её не удалось, возвращается `NULL`.

Эта функция реализует `totimespan()` при `dialect = 'kusto'`. Она не предназначена для прямого вызова
из SQL.
...
* `x` — timespan, количество дней или строка, представляющая timespan.

**Возвращаемое значение**

Значение в виде интервала в наносекундах; `NULL`, если строка не представляет timespan.
...
Возвращает `time_string`, преобразованную в [`DateTime64`](/ru/reference/data-types/datetime64) с приоритетом американского формата даты, или `NULL`, если входное значение не удаётся разобрать. [`DateTime64`](/ru/reference/data-types/datetime64) или [`NULL`](/ru/reference/syntax#null)
...
Возвращает `time_string`, преобразованную в [`DateTime64`](/ru/reference/data-types/datetime64) с предпочтением американского формата даты, или нулевую дату/дату и время (`1970-01-01 00:00:00.000`), если входное значение не удаётся разобрать. [`DateTime64`](/ru/reference/data-types/datetime64)
...
Возвращает `time_string` как DateTime с приоритетом американского формата даты или `NULL`, если входное значение не удаётся разобрать. [`DateTime`](/ru/reference/data-types/datetime) или [`NULL`](/ru/reference/syntax#null)
...
Если значение выходит за пределы диапазона `[0000-01-01, 9999-12-31]`, `toDate32` возвращает граничные значения, поддерживаемые [Date32](/ru/reference/data-types/date32).
...
Преобразует аргумент в тип данных [Date32](/ru/reference/data-types/date32). Если аргумент невозможно преобразовать, в том числе если числовое значение выходит за пределы диапазона [Date32](/ru/reference/data-types/date32), функция возвращает значение по умолчанию: второй аргумент (если указан) или, в противном случае, `1900-01-01` — историческое значение по умолчанию для [Date32](/ru/reference/data-types/date32) (а не его нижнюю границу, которой является `0000-01-01`). Если аргумент имеет тип [Date](/ru/reference/data-types/date), учитываются его границы.

Обратите внимание: в отличие от [toDate32](#toDate32), эта функция не ограничивает числовой аргумент, выходящий за пределы диапазона, границами [Date32](/ru/reference/data-types/date32); такой аргумент считается непреобразуемым.
...
Преобразует входное значение в значение типа [Date32](/ru/reference/data-types/date32), но при получении некорректного аргумента возвращает значение по умолчанию `1900-01-01`.
То же, что и [toDate32](#toDate32), но при получении некорректного аргумента возвращает `1900-01-01`.
Обратите внимание, что `1900-01-01` — это историческое значение по умолчанию для некорректного входного значения, а не нижняя граница [Date32](/ru/reference/data-types/date32), которой является `0000-01-01`.
...
Возвращает значение Date32 в случае успеха, в противном случае — историческое значение по умолчанию для некорректного ввода (`1900-01-01`). [`Date32`](/ru/reference/data-types/date32)
...
Если `expr` — значение [Time](/ru/reference/data-types/time) или [Time64](/ru/reference/data-types/time64), оно интерпретируется как количество секунд с начала эпохи Unix: дробная часть `Time64` округляется вниз в сторону отрицательной бесконечности, а значения за пределами диапазона `DateTime` обрабатываются в соответствии с настройкой `date_time_overflow_behavior`.
...
* `expr` — Значение. [`String`](/ru/reference/data-types/string) или [`Int`](/ru/reference/data-types/int-uint) или [`Date`](/ru/reference/data-types/date) или [`Date32`](/ru/reference/data-types/date32) или [`Time`](/ru/reference/data-types/time) или [`Time64`](/ru/reference/data-types/time64) или [`DateTime`](/ru/reference/data-types/datetime) или [`DateTime64`](/ru/reference/data-types/datetime64)
...
Поддерживает преобразование из `String`, `FixedString`, `Date`, `Date32`, `Time`, `Time64`, `DateTime`, `DateTime64` или числовых типов (`(U)Int8`, `(U)Int16`, `(U)Int32`, `(U)Int64`, `(U)Int128`, `(U)Int256`, `BFloat16`, `Float32`, `Float64`). Значения `Decimal` не поддерживаются и приводят к исключению.
`DateTime32` — псевдоним `DateTime`; он поддерживает даты от `1970-01-01 00:00:00` до `2106-02-07 06:28:15`.
При преобразовании из строки или одного из перечисленных выше числовых типов с насыщением значения за пределами этого диапазона насыщаются до его границ. Неконечные значения с плавающей точкой (`NaN`, `inf`, `-inf`) нельзя преобразовать — они приводят к исключению.
Преобразования из других типов даты и времени, таких как `Date32`, `DateTime64`, `Time` или `Time64`, выполняются в соответствии с настройкой `date_time_overflow_behavior`: режим `ignore` по умолчанию может приводить к циклическому переполнению, `saturate` ограничивает значения границами диапазона, а `throw` генерирует исключение. Значения `Time64` сначала округляются вниз до целых секунд в сторону отрицательной бесконечности.
...
* `x` — Входное значение для преобразования. [`String`](/ru/reference/data-types/string), [`FixedString`](/ru/reference/data-types/fixedstring), [`(U)Int*`](/ru/reference/data-types/int-uint), [`Float*`](/ru/reference/data-types/float), [`BFloat16`](/ru/reference/data-types/float), [`Date`](/ru/reference/data-types/date), [`Date32`](/ru/reference/data-types/date32), [`Time`](/ru/reference/data-types/time), [`Time64`](/ru/reference/data-types/time64), [`DateTime`](/ru/reference/data-types/datetime) или [`DateTime64`](/ru/reference/data-types/datetime64)
* `timezone` — Необязательный часовой пояс для возвращаемого значения `DateTime`. [`String`](/ru/reference/data-types/string)

**Возвращаемое значение**

Возвращает преобразованное входное значение. [`DateTime`](/ru/reference/data-types/datetime)
...
* `scale` — Размер тика (точность): 10^(-scale) секунд. [`UInt8`](/ru/reference/data-types/int-uint)
...
* `expr` — Выражение, возвращающее число или строковое представление числа. [`String`](/ru/reference/data-types/string) или [`(U)Int*`](/ru/reference/data-types/int-uint) или [`Float*`](/ru/reference/data-types/float)
* `scale` — Размер тика (точность): 10^-точность секунд. [`UInt8`](/ru/reference/data-types/int-uint)
* `timezone` — Необязательно. Часовой пояс. [`String`](/ru/reference/data-types/string)
* `default` — Необязательно. Значение по умолчанию, которое возвращается, если разбор неуспешен. [`DateTime64`](/ru/reference/data-types/datetime64)
...
Преобразует входное значение в значение типа [DateTime64](/ru/reference/data-types/datetime64), но при получении некорректного аргумента возвращает нулевое значение [DateTime64](/ru/reference/data-types/datetime64) (эпоху Unix, `1970-01-01 00:00:00`).
То же, что и [toDateTime64](#toDateTime64), но при получении некорректного аргумента возвращает нулевое значение [DateTime64](/ru/reference/data-types/datetime64).
...
* `precision` — Необязательный параметр. Субсекундная точность возвращаемого значения. [`UInt8`](/ru/reference/data-types/int-uint)
* `timezone` — Необязательный параметр. Часовой пояс возвращаемого значения. [`String`](/ru/reference/data-types/string)

**Возвращаемое значение**

Если преобразование успешно, возвращает значение DateTime64, в противном случае — нулевое значение DateTime64 (`1970-01-01 00:00:00`) с запрошенной точностью. [`DateTime64`](/ru/reference/data-types/datetime64)
...
* Строковые представления значений Float*, включая `NaN` и `Inf`.
* Строковые представления двоичного и шестнадцатеричного значения, например `SELECT toInt128OrNull('0xc0fe');`.
...
Поддерживает преобразование из String, FixedString, DateTime, DateTime64, Time64 или числовых типов, обозначающих количество секунд с полуночи.
Числовые значения за пределами диапазона типа (от `-999:59:59` до `999:59:59`, то есть от `-3599999` до `3599999` секунд) ограничиваются границами диапазона, а неконечные значения с плавающей запятой (`NaN`, `inf`, `-inf`) не могут быть преобразованы и приводят к исключению.
Значения `Time64` округляются в сторону отрицательной бесконечности до целых секунд и ограничиваются тем же диапазоном.

<Note title="Устаревшая `toTime`">
  До v25.5 `toTime` была другой функцией, которая преобразовывала дату и время в фиксированную дату (`1970-01-02`), сохраняя
  компонент времени. Эта функция по-прежнему доступна под именем [`toTimeWithFixedDate`](/ru/reference/functions/regular-functions/date-time-functions#toTimeWithFixedDate).

  Установка [`use_legacy_to_time`](/ru/reference/settings/session-settings#use_legacy_to_time) в значение `1` также сохраняет привязку имени `toTime` к устаревшей
  функции, то есть вызовы `toTime` разрешаются в `toTimeWithFixedDate` вместо описанной здесь функции преобразования.
  Пока настройка включена, используйте `CAST(x AS Time)` или `x::Time` для преобразования в тип `Time`.
  Значение настройки по умолчанию равно `0` начиная с v26.7, но с v25.6 по v26.6 оно было равно `1`, поэтому в приведённых ниже примерах оно задаётся явно.
</Note>
...
* `x` — Входное значение для преобразования. [`String`](/ru/reference/data-types/string) or [`FixedString`](/ru/reference/data-types/fixedstring) or [`DateTime`](/ru/reference/data-types/datetime) or [`DateTime64`](/ru/reference/data-types/datetime64) или [`Time64`](/ru/reference/data-types/time64) or [`(U)Int*`](/ru/reference/data-types/int-uint) or [`BFloat16`](/ru/reference/data-types/float) or [`Float*`](/ru/reference/data-types/float)
...
* `precision` — необязательный. Субсекундная точность возвращаемого значения. [`UInt8`](/ru/reference/data-types/int-uint)
...
Преобразует входное значение в значение типа `Time64`, но при получении некорректного аргумента возвращает нулевое значение `Time64` (`00:00:00`).
Аналогично [`toTime64`](#toTime64), но при получении некорректного аргумента возвращает нулевое значение `Time64`.
...
Возвращает значение типа Time64 в случае успеха, иначе — нулевое значение Time64 (`00:00:00`) с запрошенной точностью. [`Time64`](/ru/reference/data-types/time64)
