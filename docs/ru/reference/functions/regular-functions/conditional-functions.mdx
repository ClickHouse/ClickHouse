...
:::note Используйте настройку `least_greatest_legacy_null_behavior`, чтобы изменить поведение `NULL`
В версии [24.12](/ru/resources/changelogs/oss/2024#a-id2412a-clickhouse-release-2412-2024-12-19) было внесено изменение, нарушающее обратную совместимость: значения `NULL` теперь игнорируются, тогда как ранее функция возвращала `NULL`, если один из аргументов был `NULL`.
