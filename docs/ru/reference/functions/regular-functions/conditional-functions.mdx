...
* Для типов `DateTime` результирующий тип повышается до наиболее широкого типа (например, `DateTime64`, если он используется вместе с `DateTime32`).

<Info>
  **Используйте настройку `least_greatest_legacy_null_behavior`, чтобы изменить поведение `NULL`**

  В версии [24.12](/ru/resources/changelogs/oss/2024#a-id2412a-clickhouse-release-2412-2024-12-19) было внесено обратно несовместимое изменение: значения `NULL` теперь игнорируются, тогда как ранее функция возвращала `NULL`, если один из аргументов был `NULL`.
...
* `x1[, x2, ...]` — Одно или несколько значений для сравнения. Все аргументы должны иметь сопоставимые типы. [`Any`](/ru/reference/data-types/index)

**Возвращаемое значение**

Возвращает наибольшее из аргументов, приведённое к наибольшему совместимому типу. [`Any`](/ru/reference/data-types/index)
...
Параметр [`short_circuit_function_evaluation`](/ru/reference/settings/session-settings/short-circuit-function-evaluation#short_circuit_function_evaluation) определяет, используется ли укороченное вычисление.
...
<Info>
  **Используйте настройку `least_greatest_legacy_null_behavior`, чтобы изменить поведение `NULL`**
  ...
  В версии [24.12](/ru/resources/changelogs/oss/2024#a-id2412a-clickhouse-release-2412-2024-12-19) было внесено обратно несовместимое изменение: значения `NULL` теперь игнорируются, тогда как раньше функция возвращала `NULL`, если один из аргументов был `NULL`.
  Чтобы сохранить прежнее поведение, установите для настройки `least_greatest_legacy_null_behavior` (по умолчанию: `false`) значение `true`.
</Info>
...
Параметр [`short_circuit_function_evaluation`](/ru/reference/settings/session-settings/short-circuit-function-evaluation#short_circuit_function_evaluation) определяет,
