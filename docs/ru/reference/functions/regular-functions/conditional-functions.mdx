...
<Note title="Используйте настройку `least_greatest_legacy_null_behavior`, чтобы изменить поведение `NULL`">
...
* `x1[, x2, ...]` — Одно или несколько значений для сравнения. Все аргументы должны иметь сравнимые типы. [`Any`](/ru/reference/data-types/index)
