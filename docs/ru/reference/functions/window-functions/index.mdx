...
| Рамка окна `GROUPS`                                                             | ✅               | Границы рамки учитывают целые группы равных строк (строк с одинаковым ключом `ORDER BY`); `N PRECEDING` и `N FOLLOWING` учитывают `N` групп равных строк до и после группы, к которой относится текущая строка.                                                                                                                                                                                                          |
...
* `ROWS`, `RANGE` или `GROUPS` - определяет границы рамки окна, в пределах которой вычисляется aggregate&#95;function. `ROWS` считает физические строки, `RANGE` — значения `ORDER BY`, а `GROUPS` — группы равноправных строк (строки с одинаковым ключом `ORDER BY`).
...
Рамка `GROUPS` считает целые *группы равных строк* — наборы строк с одинаковым ключом `ORDER BY` — а не физические строки (`ROWS`) или значения `ORDER BY` (`RANGE`). `N PRECEDING` и `N FOLLOWING` отсчитывают `N` групп равных строк до и после группы равных строк текущей строки, при этом каждая включённая группа учитывается целиком.

В приведённом ниже запросе используются одинаковые границы `1 PRECEDING AND 1 FOLLOWING` для рамок `ROWS`, `RANGE` и `GROUPS`. Столбец `order` содержит повторяющиеся и непоследовательные значения, поэтому три режима охватывают разные строки:
...
Каждый режим по-разному интерпретирует границы:

* `ROWS` считает физические строки, поэтому рамка окна включает не более трёх соседних строк: текущую и по одной с каждой стороны.
* `RANGE` считает значения `order`, поэтому `1 PRECEDING` и `1 FOLLOWING` охватывают строки, чьё значение `order` отличается от значения текущей строки не более чем на 1. При разрывах в 10 ни одна соседняя строка не попадает в рамку, поэтому она содержит только строки с тем же значением `order`, что и у текущей строки.
* `GROUPS` считает группы равных строк, поэтому `1 PRECEDING` и `1 FOLLOWING` всегда полностью включают соседние группы независимо от разрывов между значениями `order`.
