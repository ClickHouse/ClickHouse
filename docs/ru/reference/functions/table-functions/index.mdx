...
<Note>
  Для выполнения большинства табличных функций требуется привилегия `CREATE TEMPORARY TABLE`, поэтому запрос `SELECT` или `INSERT`,
  который её вызывает, будет отклонён, если
  [`readonly`](/ru/concepts/features/configuration/settings/permissions-for-queries#readonly) установлен в 1.
</Note>