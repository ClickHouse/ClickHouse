...
### функция in для подзапроса {#in-subquery-function}
...
### функция in для подзапроса {#in-subquery-function-1}
...
### `SOME` / `ALL` для массивов {#some-all-on-arrays}
...
<Note>
  В результате используется [часовой пояс сеанса](/ru/reference/settings/session-settings/other#session_timezone) (или часовой пояс сервера по умолчанию, если часовой пояс сеанса не задан). Настройка [`date_time_overflow_behavior`](/ru/reference/settings/formats/date-time#date_time_overflow_behavior) определяет, что произойдет, если результат выйдет за пределы представимого диапазона.
</Note>
...
### AT TIME ZONE и AT LOCAL {#at-time-zone}
...
`zone` может быть любым константным строковым expression, которое вычисляется в корректное имя часового пояса (например, `'America/Denver'`, `'UTC'` или `concat('America', '/', 'Denver')`). Поскольку `AT TIME ZONE` сводится к `toTimeZone`, действуют те же правила для аргумента часового пояса: неконстантные выражения, такие как ссылка на столбец, требуют [`allow_nonconst_timezone_arguments = 1`](/ru/reference/settings/session-settings/allow#allow_nonconst_timezone_arguments).

`AT LOCAL` использует текущий [часовой пояс сеанса](/ru/reference/settings/session-settings/other#session_timezone) (или часовой пояс сервера по умолчанию, если часовой пояс сеанса не задан). В таблицах `Distributed` параметр `session_timezone` должен быть задан явно; если он пуст, `timeZone()` определяется на уровне сегмента и не может использоваться как константный аргумент `toTimeZone`, что приводит к исключению `ILLEGAL_COLUMN`.
...
## Оператор создания Array {#array-creation-operator}

`[x1, ...]` – Функция `array(x1, ...)`.

## Оператор создания Tuple {#tuple-creation-operator}
...
Можно оптимизировать, включив настройку [optimize&#95;functions&#95;to&#95;subcolumns](/ru/reference/settings/session-settings/optimize#optimize_functions_to_subcolumns). При `optimize_functions_to_subcolumns = 1` функция читает только подстолбец [null](/ru/reference/data-types/nullable#finding-null) вместо чтения и обработки данных всего столбца. Запрос `SELECT n IS NULL FROM table` преобразуется в `SELECT n.null != 0 FROM TABLE`.
...
Можно повысить производительность, включив настройку [optimize&#95;functions&#95;to&#95;subcolumns](/ru/reference/settings/session-settings/optimize#optimize_functions_to_subcolumns). При `optimize_functions_to_subcolumns = 1` функция читает только подстолбец [null](/ru/reference/data-types/nullable#finding-null) вместо чтения и обработки всех данных столбца. Запрос `SELECT n IS NOT NULL FROM table` преобразуется в `SELECT NOT n.null FROM TABLE`.

## Проверка булевых значений {#checking-boolean-values}
