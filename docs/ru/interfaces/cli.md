...
-   `--vertical, -E` — если указано, использовать формат [Vertical](../formats#vertical) по умолчанию для вывода результата. То же самое, что `–format=Vertical`. В этом формате каждое значение выводится на отдельной строке, что удобно при отображении широких таблиц.
