...
Вы можете создать запрос с параметрами и передать им значения соответствующих параметров клиентского приложения. Например:
...
- `name` — идентификатор подстановки, который следует использовать как часть имени параметра приложения `--param_<name> = value`.
- `data type` — тип данных значения. Например, структура данных `(integer, ('string', integer))` может иметь тип данных `Tuple(UInt8, Tuple(String, UInt8))` ([целочисленный](../data_types/int_uint.md) тип может быть и другим).
