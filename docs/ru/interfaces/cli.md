...
-   `--port` — порт для соединения, по умолчанию — 9000. Обратите внимание: для HTTP-интерфейса и собственного интерфейса используются разные порты.
...
-   `--vertical, -E` — если указано, использовать формат [Vertical](../interfaces/formats.md#vertical) по умолчанию для вывода результата. То же самое, что `–format=Vertical`. В этом формате каждое значение выводится на отдельной строке, что удобно при отображении широких таблиц.
-   `--time, -t` — если указано, в неинтерактивном режиме вывести время выполнения запроса в потоке ‘stderr’.
