...
Этот диск используется только для чтения. Данные с него только считываются и никогда не изменяются. Новая таблица загружается на этот диск с помощью запроса `ATTACH TABLE` (см. пример ниже). Любое изменение данных запрещено, т.е. не допускаются следующие типы запросов: [CREATE TABLE](../sql-reference/statements/create/table.md), [ALTER TABLE](../sql-reference/statements/alter/index.md), [RENAME TABLE](../sql-reference/statements/rename.md#misc_operations-rename_table), [DETACH TABLE](../sql-reference/statements/detach.md) и [TRUNCATE TABLE](../sql-reference/statements/truncate.md).
