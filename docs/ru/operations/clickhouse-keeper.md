...
-    `operation_timeout_ms` — время ожидания для одной клиентской операции в миллисекундах (по умолчанию: 10000).
-    `session_timeout_ms` — время ожидания для клиентской сессии в миллисекундах (по умолчанию: 30000).
...
-    `heart_beat_interval_ms` — частота, с которой узел-лидер ClickHouse Keeper отправляет сердцебиение узлам-последователям, в миллисекундах (по умолчанию: 500).
...
-    `rotate_log_storage_interval` — количество записей журнала для хранения в одном файле (по умолчанию: 100000).
-    `reserved_log_items` — количество записей журнала координации для хранения перед сжатием (по умолчанию: 100000).
...
-    `fresh_log_gap` — время обновления узла (по умолчанию: 200).
