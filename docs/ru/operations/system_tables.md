...
Содержит логи выполняемых запросов. Например, дату запуска, длительность выполнения, текст возникших ошибок и т. д.

!!! note "Внимание"
    Логи для запросов `INSERT` сохраняются без данных, которые добавлялись в таблицу.
 
Таблица `system.query_log` создаётся только в том случае, если задана серверная настройка [query_log](server_settings/settings.md#server_settings-query-log). Эта настройка определяет правила логирования. Например, с какой периодичностью логи будут записываться в таблицу. Также в настройке можно изменить название этой таблицы.

Чтобы включить логирование запросов, необходимо установить параметр [log_queries](settings/settings.md#settings-log-queries) в 1. Это можно сделать глобально, в рамках сессии или для отдельного запроса. Подробнее см. в разделе [Настройки](settings/settings.md).

Столбцы:

- `type` — тип события, которое возникло при выполнении запроса. Возможные значения:
...
- `event_date` — дата возникновения события;
- `event_time` — время возникновения события;
- `query_start_time` — время запуска запроса;
- `query_duration_ms` — длительность выполнения запроса;
- `read_rows` — количество прочитанных строк;
- `read_bytes` — количество прочитанных байт;
- `written_rows` — количество записанных строк;
- `written_bytes` — количество записанных байт;
- `result_rows` — количество строк, выведенных в результате;
- `result_bytes` — количество байт, выведенных в результате;
- `memory_usage` — потребление памяти запросом;
- `query` — строка запроса;
- `exception` — сообщение об ошибке;
- `stack_trace` — стектрейс (список методов, которые были вызваны до возникновения ошибки). Пустая строка, если запрос завершился успешно;
- `is_initial_query` — тип запроса:
    - 1 — запрос был запущен клиентом;
    - 0 — запрос был вызван другим запросом (при распределенном выполнении запросов).
- `user` — имя пользователя, запустившего запрос;
- `query_id` — идентификатор запроса;
- `address` — имя хоста, с которого был отправлен запрос;
- `port` — порт удалённого сервера, принимающего запрос;
- `initial_user` —  имя пользователя, вызвавшего первоначальный запрос. Для распределенных запросов будет `default`;
- `initial_query_id` — идентификатор первоначального запроса;
- `initial_address` — имя хоста, с которого был отправлен первоначальный запрос;
- `initial_port` — порт удалённого сервера, принимающего первоначальный запрос;
- `interface` — используемый интерфейс. Возможные значения:
    - 1 — TCP.
    - 2 — HTTP.
- `os_user` — операционная система на клиенте;
- `client_hostname` — имя хоста, к которому подключен клиент [clickhouse-client](../interfaces/cli.md);
- `client_name` — имя клиента [clickhouse-client](../interfaces/cli.md);
- `client_revision` — ревизия [clickhouse-client](../interfaces/cli.md);
- `client_version_major` — мажорная версия [clickhouse-client](../interfaces/cli.md);
- `client_version_minor` — минорная версия [clickhouse-client](../interfaces/cli.md);
- `client_version_patch` — patch-компонент версии [clickhouse-client](../interfaces/cli.md);
- `http_method` — используемый HTTP-метод;
- `http_user_agent` — содержимое заголовка `UserAgent`;
- `quota_key` (String) — ключ квоты, заданный в настройке [quotas](quotas.md);
- `revision` — ревизия сервера ClickHouse;
- `thread_numbers` — номера потоков, участвующих в выполнении запроса;
- `ProfileEvents.Names` — счётчики, измеряющие метрики: 
...
- `ProfileEvents.Values` — значения счётчиков, перечисленных в `ProfileEvents.Names`.
- `Settings.Names` — настройки, которые были изменены при выполнении запроса. Чтобы включить отслеживание изменений настроек, установите параметр `log_query_settings` в 1.
- `Settings.Values` — значения настроек, перечисленных в `Settings.Names`.
