...
- `written_rows` — количество записанных строк, для запросов `INSERT`. Для остальных запросов столбец принимает значение 0.
- `written_bytes` — количество записанных байт, для запросов `INSERT`. Для остальных запросов столбец принимает значение 0.
...
- `initial_user` —  имя пользователя, вызвавшего родительский запрос (для распределенного выполнения запросов);
- `initial_query_id` — идентификатор родительского запроса, породившего исходный запрос;
- `initial_address` — имя хоста, с которого был вызван родительский запрос;
- `initial_port` — порт удалённого сервера, принимающего родительский запрос;
