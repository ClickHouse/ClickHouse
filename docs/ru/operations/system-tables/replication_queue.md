...
-   `type` ([String](../../sql-reference/data-types/string.md)) — тип задачи в очереди:
    ...
    -   `GET_PART` — взять кусок из другой реплики.
    -   `ATTACH_PART` — присоединить кусок. Задача может быть выполнена и с куском из нашей собственной реплики (если она находится в папке `detached`). Эта задача практически идентична задаче `GET_PART`, лишь немного оптимизирована.
    -   `MERGE_PARTS` — выполнить слияние кусков.
    -   `DROP_RANGE` — удалить куски в партициях из указнного диапазона.
    -   `CLEAR_COLUMN` — удалить указанный столбец из указанной партиции. Примечание: не рекомендуется использовать.
    -   `CLEAR_INDEX` — удалить указанный индекс из указанной партиции. Примечание: не рекомендуется использовать. 
    -   `REPLACE_RANGE` — удалить указанный диапазон партиций и заменить их на новые.
