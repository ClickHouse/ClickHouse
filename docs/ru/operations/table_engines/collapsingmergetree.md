...
- `sign` — Имя столбца с типом строки: `1` — строка состояния ("state"-строка), `-1` — строка отмены состояния ("cancel"-строка).
...
Используйте специальный столбец `Sign`. Если `Sign = 1`, то это означает, что строка является состоянием объекта, назовём её "state"-строкой. Если `Sign = -1`, то это означает отмену состояния объекта с теми же атрибутами, назовём такую строку "cancel"-строкой.
...
1. Программа, которая записывает данные, должна помнить состояние объекта, чтобы иметь возможность отменить его. "Cancel"-cтрока должна быть копией предыдущей "state"-строки с противоположным значением `Sign`. Это увеличивает начальный размер хранилища, но позволяет быстро записывать данные.
...
Когда ClickHouse объединяет куски данных, каждая группа последовательных строк с одним и тем же первичным ключом уменьшается до не более чем двух строк, одна из которых имеет `Sign = 1` ("state"-строка) а другая строка с `Sign = -1` ("cancel"-строка). Другими словами, записи сворачиваются.

Для каждого результирующего куска данных ClickHouse сохраняет:

  1. Первую "cancel"-строку и последнюю "state"-строку, если количество "state" и "cancel" строк совпадает.

  1. Последнюю "state"-строку, если "state"-строк на одну больше, чем "cancel"-строк.

  1. Первую "cancel"-строку, "cancel"-строк на одну больше, чем "state"-строк.
