...
Для `interval` можно задать любое выражение, возвращающее значение типа `DateTime`. Например, вы можете использовать операторы [интервала времени](../../query_language/operators.md#operators-datetime).
...
Когда ClickHouse видит, что некоторые данные устарели, он выполняет внеплановые мёржи. Для управление частотой подобных мёржей, можно задать [merge_with_ttl_timeout](#mergetree_setting-merge_with_ttl_timeout). Если значение слишком низкое, он будет выполнять много внеплановых мёржей, которые могут потреблять много ресурсов.
