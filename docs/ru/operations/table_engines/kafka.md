...
- `kafka_skip_broken_messages` – режим парсинга сообщений Кафка. Если `kafka_skip_broken_messages = 1`, то движок отбрасывает сообщения Кафки, которые не получилось распарсить (одно сообщение в точности соответствует одной записи (строке)).
