...
При старте сервера для каждого конфигурационного файла создаются `file-preprocessed.xml` файлы. Они предназначены для информационного использования, в них содержутся все завершенные подстановки и переопределения. Если при этом в конфигурационных файлах были использованы подстановки ZooKeeper, но ZooKeeper при этом не доступен на старте сервера, то сервер загружает конфигурацию из предпробработанного файла.
