...
Эти значения можно задать (перекрыть) у таблиц задав в секции `Settings` у команды `CREATE TABLE`.
...
Eсли число кусков в партиции превышает значение `parts_to_throw_insert` INSERT прерывается с исключением 'Too many parts (300). Merges are processing significantly slower than inserts'.
...
`INSERT` будет замедлен на max_delay_to_insert/(количество кусков в партиции - parts_to_delay_insert).
Т.е. если в партиции уже 299 кусков и parts_to_delay_insert = 150, max_delay_to_insert = 1, `INSERT` замедлится на 1/(299-150) (~0.5) секунд.
