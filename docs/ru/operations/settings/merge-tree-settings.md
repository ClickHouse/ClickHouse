...
Значения настроек всех MergeTree таблиц собраны в таблице `system.merge_tree_settings`. Их можно переопределить в разделе `merge_tree` файла `config.xml`, а задать — в секции `SETTINGS` каждой таблицы.
...
Можно установить значение побольше — 600 (1200) кусков. Тогда ошибка `Too many parts` будет появляться реже, но при этом могут возникнуть проблемы с фоновыми слияниями и производительностью `SELECT`-запросов.
...
Количество хеш-сумм вставленных последними блоков, которые хранятся в Zookeeper.
...
Время (в секундах) хранения неактивных кусков для защиты от потери данных при спонтанной перезагрузке сервера или ОС.
...
После объединения нескольких кусков в один новый происходит следующее: ClickHouse помечает исходные куски как неактивные и удаляет их по прошествию `old_parts_lifetime` секунд.
...
Неактивные куски удаляются со временем, потому что `fsync` не вызывается при записи нового куска. Получается, что новый кусок некоторое время находится только в оперативной памяти сервера. Если сервер вдруг перезагружается, новый (слитый) кусок может исчезнуть или испортиться. Тогда ClickHouse обнаружит проблему на старте во время проверки целостности кусков, вернет неактивные куски в список активных и снова сольет их позже. В этом случае испорченный кусок получает новое имя (добавляется префикс broken_) и попадает в папку `detached`. Если проверка целостности не выявляет проблем в слитом куске, то исходные неактивные куски переименовываются (добавляется префикс ignored_) и перемещаются в папку `detached`.

Стандартное значение `Linux dirty_expire_centisecs` — 30 секунд. Это максимальное время, в течение которого записанные данные хранятся только в оперативной памяти. Если нагрузка на дисковую систему большая, то данные записываются намного позже. Эксперименты показали 480 секунд — это время, за которое новый кусок гарантированно запишется на диск.
...
Планировщик слияний периодически анализирует размер и количество кусков в партициях. Если хватает ресурсов, то начинает фоновое слияние в фоновом пуле. Слияния происходят до тех пор, пока суммарный размер входных кусков не достигнет `max_bytes_to_merge_at_max_space_in_pool`.

Слияния, начатые по `optimize final`, не учитывают `max_bytes_to_merge_at_max_space_in_pool` и сливают куски, используя только доступные ресурсы (например, свободное дисковое пространство) до тех пор, пока в партиции не останется один кусок.
...
`max_bytes_to_merge_at_min_space_in_pool` задает максимальный суммарный размер кусков, которые можно объединить несмотря на нехватку свободных ресурсов в фоновом пуле (дискового пространства). Это нужно, чтобы уменьшить количество маленьких кусков и снизить вероятность ошибки `Too many parts`.

Слияния резервируют дисковое пространство, удваивая суммарный размер кусков в слиянии. Если свободного места на диске мало, то оно может быть уже зарезервировано текущими слиянияними. Поэтому другие слияния не начинаются, а количество маленьких кусков в партиции растет с каждым `Insert`.
...
На старте ClickHouse читает все куски из всех таблиц (файлы с метаданными кусков), чтобы построить в ОЗУ список всех кусков. В некоторых системах с большим количеством кусков этот процесс бывает долгим. Это время уменьшается за счет увеличения `max_part_loading_threads` (если при этом хватает ресурсов CPU и диска).
