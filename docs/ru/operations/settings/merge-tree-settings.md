...
Количество последних вставленных блоков в нереплицированной `MergeTree` таблице, для которых хранятся хэш-суммы для проверки дубликатов.
...
Используется механизм дедупликации, аналогичный реплицированным таблицам (см. описание настройки [replicated_deduplication_window](#replicated-deduplication-window)). Хэш-суммы вставленных кусков записываются в локальный файл на диске.
