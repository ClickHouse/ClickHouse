...
Включает или отключает асинхронные вставки для нескольких параллельных соединений. Работает только для вставок по протоколу HTTP. Обратите внимание, что при таких вставках дедупликация не производится.
...
Вставка данных происходит либо как только привышен [async_insert_max_data_size](#async-insert-max-data-size), либо через [async_insert_busy_timeout](#async-insert-busy-timeout) миллисекунд после первого запроса `INSERT`. Если [async_insert_stale_timeout](#async-insert-stale-timeout) задано ненулевое значение, то данные вставляются через `async_insert_stale_timeout` после последнего запроса.
