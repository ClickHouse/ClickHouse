...
Включает или отключает перезапись все агрегатные функции в запросе, добавляя к ним суффикс `-OrNull`. Включите для совместимости со стандартом SQL.
Это реализуется с помощью перезаписи запросов (аналогично настройке `count_distinct_implementation`, чтобы получить согласованные результаты для распределенных запросов.
