...
Включает или выключает тихий пропуск:

- Ноды, если её имя не разрешается через DNS.

    Когда пропуск выключен, ClickHouse требует, чтобы все ноды в [конфигурации кластера](../server_settings/settings.md#server_settings_remote_servers) могли быть разрешены через DNS. В противном случае, ClickHouse генерирует исключение при попытке выполнить запрос на кластере.

    Если пропуск включен, то ClickHouse считает неразрешённые ноды недоступными и пытается разрешить их при каждой попытке подключения. Подобное поведение создаёт риск некорректной конфигурации кластера, поскольку пользователь может указать неправильное имя ноды, а ClickHouse не оповестит об этом. Несмотря на это, функция пропуска может быть полезной в системах с динамическим DNS, например, [Kubernetes](https://kubernetes.io), где ноды могут не разрешаться в выключенном состоянии и это не ошибка.

- Шарда, если для него нет доступных реплик.

    Когда пропуск выключен, ClickHouse генерирует исключение.

    Когда пропуск включен, ClickHouse возвращает неполный ответ и не оповещает о проблемах с доступностью нод. 
