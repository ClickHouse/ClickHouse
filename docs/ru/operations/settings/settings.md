...
Обычно, её включение не требуется, так как это будет сделано автоматически, если это возможно. 
...
!!! note "Примечание"
    При использованаии этой настройки, результат запроса может быть неточным.
