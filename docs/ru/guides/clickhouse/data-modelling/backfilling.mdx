...
* **Параллелизм вставки** - Количество потоков вставки, используемых для выполнения вставки. Управляется через [`max_insert_threads`](/ru/reference/settings/session-settings/max-insert#max_insert_threads). **Примечание**: Это значение ограничивается `max_threads`, поэтому фактический параллелизм вставки равен `min(max_insert_threads, max_threads)`. В ClickHouse Cloud он определяется размером инстанса (от 2 до 4), а в OSS установлен в 1. Увеличение этого значения может повысить производительность ценой более высокого использования памяти.
