...
Используйте вариант S3 для AWS S3 и GCS. Для Azure и локальной файловой системы предусмотрены отдельные варианты (`icebergAzure`, `icebergLocal` и эквиваленты для других форматов). Полный список см. в разделе [Прямые запросы](/ru/guides/use-cases/data-warehousing/getting-started/querying-directly).

Для [Paimon](/ru/reference/functions/table-functions/paimon) доступны только табличные функции.
