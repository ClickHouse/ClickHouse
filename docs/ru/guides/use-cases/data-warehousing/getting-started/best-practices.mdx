...
Для записи в Iceberg требуется [allow&#95;insert&#95;into&#95;iceberg](/ru/reference/settings/session-settings/allow#allow_insert_into_iceberg) (25.7+, бета с 26.2). См. [Запись в озера данных](/ru/use-cases/data-lake/getting-started/writing-data). Для Delta Lake требуется [allow&#95;delta&#95;lake&#95;writes](/ru/reference/settings/session-settings/allow-experimental#allow_experimental_delta_lake_writes) (25.9+). В [матрице поддержки](/ru/use-cases/data-lake/support-matrix) указано, какие флаги применяются к каждому формату и операции.
...
1. Установите [iceberg&#95;metadata&#95;async&#95;prefetch&#95;period&#95;ms](/ru/engines/table-engines/integrations/iceberg#async-metadata-prefetch) при создании таблицы, чтобы предварительно подгружать метаданные в фоновом режиме.
...
Начиная с версии 25.6 ClickHouse читает Delta Lake из S3 и GCS с помощью Rust-ядра Delta Lake ([allow&#95;experimental&#95;delta&#95;kernel&#95;rs](/ru/reference/settings/session-settings/allow-experimental#allow_experimental_delta_kernel_rs), 25.5+). Для Azure Blob Storage используйте [deltaLakeAzure()](/ru/sql-reference/table-functions/deltalake) со старым механизмом чтения, поскольку там это ядро отключено. Без ядра недоступны отсечение партиций, change data feed и чтение версий снимков.

<div id="delta-kernel">
  #### Delta Kernel
</div>

[allow&#95;experimental&#95;delta&#95;kernel&#95;rs](/ru/reference/settings/session-settings/allow-experimental#allow_experimental_delta_kernel_rs) должен быть включен для отсечения партиций, change data feed и чтения версии снимка. Начиная с версии 25.5, он включен по умолчанию для S3 и GCS. Явно включите его в более старых версиях или при устранении неполадок:
...
Помимо [allow&#95;delta&#95;lake&#95;writes](/ru/reference/settings/session-settings/allow-experimental#allow_experimental_delta_lake_writes) (25.9+), можно управлять размером выходного файла данных при вставке:
...
| ----------------------------------------------------------------------------------------------- | ---------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------- |
| [system.iceberg&#95;metadata&#95;log](/ru/operations/system-tables/iceberg_metadata_log)           | Iceberg    | 25.9     | [iceberg&#95;metadata&#95;log&#95;level](/ru/reference/settings/session-settings/iceberg-metadata#iceberg_metadata_log_level) в запросе | Отслеживания чтения файлов метаданных и решений по отсечению партиций |
...
| [system.delta&#95;lake&#95;metadata&#95;log](/ru/operations/system-tables/delta_lake_metadata_log) | Delta Lake | 25.10    | [delta&#95;lake&#95;log&#95;metadata](/ru/reference/settings/session-settings/delta-lake#delta_lake_log_metadata) = `1` в запросе       | Отслеживания файлов метаданных Delta и процесса выбора снимка         |
