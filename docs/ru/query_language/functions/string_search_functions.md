...
Возвращает позицию (в байтах) найденной подстроки `needle` в строке `haystack`, начиная с 1, или 0, если подстрока не найдена.

Работает при допущении, что строка содержит набор байт, представляющий текст в однобайтовой кодировке. Если допущение не выполнено — то возвращает какой-нибудь результат (не кидает исключение).
...
- Начальная позиция, если подстрока найдена.
...
Возвращает позицию (в байтах) найденной подстроки `needle` в строке `haystack`, начиная с 1. Возвращает 0, если подстрока не найдена. Используйте эту функцию для поиска без учета регистра. 
