...
Форматирует дату-с-временем в заданном формате. Формат такой-же как у функции strftime(), дополнительно поддерживает всё что поддерживает cctz::format(). Тайм зона определяется автоматически из первого аргумента, для вывода в другой тайм зоне используйте функцию toTimeZone(). Для форматирования даты используйте функции преобразования типов, например, toDateTime(). 
