...
Форматирует дату-с-временем в заданном формате. Формат точно такой-же как у функции strftime(). Тайм зона определяется автоматически из первого аргумента, для вывода в другой тайм зоне используйте функцию toTimeZone(). Для форматирования даты используйте функции преобразования типов, например, toDateTime().
