...
- `time_zone` — часовой пояс. Функция анализирует `s` в соответствии с заданным часовым поясом. [String](../../data_types/string.md).
...
- Строка с датой, но без времени: `YYYY`, `YYYYMM`, `YYYY*MM`, `DD/MM/YYYY`, `DD-MM-YY`, `DD` и т.д. .
- Строка с временем, но без даты:  `hh:mm`, `hhmm`, `hh` - если в строке `s` указан день месяца.
- Строка, содержащая дату и время вместе с информацией о часовом поясе: `2020-12-12 17:36:00 -5:00` и т.д.
