...
Преобразует тип `value` в тип [Decimal](../../data_types/decimal.md), имеющий точность `S`. `value` может быть числом или строкой. Параметр `S` (scale) устанавливает количество знаков после запятой.
...
- `S` — количество знаков после запятой в результирующем значении.

**Возвращаемое значение**

Значение типа `Nullable(Decimal(P,S))`. `P` равно числовой части имени функции. Например, для функции `toDecimal32OrNull`, `P = 32`. Значение содержит:

- Число с `S` знаками после запятой, если ClickHouse распознал число во входной строке.
- `NULL`, если ClickHouse не смог распознать число во входной строке или входное число содержит больше чем `S`  знаков после запятой.
...
- Число с `S` знаками после запятой, если ClickHouse распознал число во входной строке.
- 0 c `S` знаками после запятой, если ClickHouse не смог распознать число во входной строке или входное число содержит больше чем `S` знаков после запятой.
