...
Возвращает массив `arr`, отсортированный в восходящем порядке.  
...
Функция `arraySort` является [функцией высшего порядка](higher_order_functions.md) — в качестве первого аргумента ей можно передать лямбда-функцию. В этом случае порядок сортировки определяется результатом применения лямбда-функции на элементы массива. Ниже приведён пример.
...
Элементы, указанные во втором массиве ([2,1]), определяют позицию соответствующих элементов из исходного массива (['hello', 'world']). То есть, 'hello' будет вторым элементом в отсортированном массиве, а 'world' — первым.

Ниже приведен ещё один пример. В нём для описания лямбда-функции используется стрелочка (оператор `->`):
...
Возвращает массив `arr`, отсортированный в нисходящем порядке.
...
В этом случае, массив сортируется следующим образом:
