...
Для каждого элемента исходного массива лямбда-функция возвращает "вес" этого элемента — он будет использоваться при сортировке. То есть [1 –> -1, 2 –> -2, 3 –> -3]. Так как элементы сортируются по возрастанию их "веса", результат будет [3, 2, 1].

Лямбда-функция может принимать несколько аргументов. В этом случае, в функцию `arraySort` нужно передавать несколько массивов, которые будут соответствовать аргументам лямбда-функции. Обратите внимание, массивы должны быть одинаковой длины. Например:
