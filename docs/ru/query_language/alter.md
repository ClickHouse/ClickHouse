...
Обратите внимание, запрос `ALTER TABLE t FREEZE PARTITION` не реплицируется. Он создает резервную копию только на локальном сервере. После создания резервной копии данные из `/var/lib/clickhouse/shadow/` можно скопировать на удалённый сервер и затем удалить на локальном сервере.
...
Изначально резервная копия не занимает места на диске. При дальнейшей работе системы, по мере модификации данных, резервная копия может уже отнимать какое-то место на диске. Если резервная копия создана для достаточно старых данных, она не будет занимать места на диске.
