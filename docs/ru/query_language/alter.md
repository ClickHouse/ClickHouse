...
Этот запрос реплицируется. Каждая реплика проверяет, есть ли данные в директории 'detached'. Если данные есть, то запрос проверяет их целостность и соответствие данным на сервере-инициаторе запроса. В случае успеха данные добавляются в таблицу. В противном случае, реплика скачивает данные с реплики-инициатора запроса или с другой реплики, на которой уже добавлены эти данные.
...
Копирует партицию из таблицы `table1` в таблицу `table2`. Следует иметь в виду:

- Таблицы должны иметь одинаковую структуру.
- Для таблицы `table2` должен быть задан такой же ключ партиционирования, что и для таблицы `table1`.
...
Создаёт резервную копию для заданной партиции. Для таблиц старого стиля имя партиций можно задавать в виде префикса (например, '2019'). В этом случае резервные копии будут созданы для всех соответствующих партиций. Если выражение `PARTITION` опущено, резервные копии будут созданы для всех партиций. 

Подробнее о том, как корректно задать имя партиции, см. в разделе [Как задавать имя партиции в запросах ALTER](#alter-how-to-specify-part-expr).

Запрос делает следующее — для текущего состояния таблицы формирует жесткие ссылки на данные в этой таблице. Ссылки размещаются в директории `/var/lib/clickhouse/shadow/N/...`, где
...
Изначально резервная копия не занимает места на диске. При дальнейшей работе системы, по мере модификации данных, резервная копия может отнимать место на диске. Если резервная копия создана для достаточно старых данных, она не занимает места на диске.

Запрос `ALTER ... FREEZE PARTITION` не реплицируется. Он создает резервную копию только на локальном сервере.

После создания резервной копии данные из `/var/lib/clickhouse/shadow/` можно скопировать на удалённый сервер и затем удалить на локальном сервере. Весь процесс создания резервной копии не требует остановки сервера.

Другой способ создать локальную копию — это вручную скопировать данные из директории `/var/lib/clickhouse/data/database/table`. Имейте в виду, что копировать данные следует при остановленном сервере. Если это сделать при запущенном сервере, то при копировании директории с добавляющимися/изменяющимися файлами может возникнуть состояние гонки, и резервная копия может быть неконсистентной. Поэтому копировать данные следует только при остановленном сервере – это гарантирует, что полученные данные будут такими же, как после запроса `ALTER TABLE t FREEZE PARTITION`.

`ALTER TABLE ... FREEZE PARTITION` копирует только данные, но не метаданные таблицы. Чтобы сделать резервную копию метаданных таблицы, скопируйте файл `/var/lib/clickhouse/metadata/database/table.sql`

Чтобы добавить данные в таблицу из резервной копии, выполните следующее:
...
3. С помощью запросов `ALTER TABLE ... ATTACH PARTITION` добавьте данные в таблицу.
...
1. Загружает партицию с указанного шарда. Путь к шарду задается в секции `FROM` ('path-in-zookeeper'). Обратите внимание, нужно задавать путь, который указан в ZooKeeper.
...
- Запрос `ALTER ... FETCH PARTITION` не реплицируется. Он загружает партицию в директорию 'detached' только на локальном сервере.
- Запрос `ALTER TABLE ... ATTACH` реплицируется — он добавляет данные в таблицу сразу на всех репликах. На одной из реплик данные будут добавлены из директории 'detached', а на других репликах загружены из соседних реплик.

Перед скачиванием данных система проверяет, существует ли партиция и совпадает ли её структура со структурой таблицы. При этом для скачивания автоматически выбирается наиболее актуальная реплика среди всех живых реплик.
...
#### Как задавать партицию в запросах ALTER {#alter-how-to-specify-part-expr}
...
- Для запросов [ALTER ATTACH PART](#alter_attach-partition): чтобы задать имя куска партиции, используйте значение из столбца `name` системной таблицы `system.parts`. Например, `ALTER TABLE visits ATTACH PART 201901_1_1_0`.
