...
- `Delta(delta_bytes)` - способ предварительного сжатия, при котором вместо числовых значений поля сохраняется разность между двумя соседними значениями. Значение `delta_bytes` - число байт для хранения дельты.
Возможные значения: 1, 2, 4, 8. Значение по умолчанию: если `sizeof(type)` равен 1, 2, 4, 8 - `sizeof(type)`, иначе - 1.
- `T64` - способ предварительного сжатия, обрезающий неиспльзуемые старшие биты у нестроковых типов данных, помещающихся в 64 бита. Неиспользуемые биты определяются сравнением минимального и максимального значения в куске.
Алгоритм сжатия берет 64 входных значения, записывает их в битовую матрицу 64х64, транспонирует ее и выдает обрезанную на незначащие биты последовательность.
