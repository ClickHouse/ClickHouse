...
| ---- | ------------- | --------- |
| `name` | Имя столбца. | Да |
| `type` | Тип столбца.<br/>Устанавливает способ интерпретации данных в источнике. Например, в случае MySQL, в таблице-источнике поле может быть `TEXT`, `VARCHAR`, `BLOB`, но загружено может быть как `String`. | Да |
| `null_value` | Значение по умолчанию для несуществующего элемента.<br/>В примере это пустая строка. | Да |
...
| `injective` | Признак инъективности отображения `id -> attribute`. <br/>Если `true`, то можно оптимизировать секцию `GROUP BY`.<br/><br/>Значение по умолчанию: `false`. | Нет |
