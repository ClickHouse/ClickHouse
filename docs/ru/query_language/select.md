...
`ASOF JOIN` применим в том случае, когда необходимо объединять записи, которые не имеют точного совпадения. Например, рассмотрим следующие таблицы:
...
`ASOF JOIN` принимает метку времени пользовательского события из `table_1` и находит такое событие в `table_2` метка времени которого наиболее близка (равна или меньше) к метке времени события из `table_1`. В нашем примере `event_1_1` может быть объединено с `event_2_1`, `event_1_2` может быть объединено с `event_2_3`, а `event_2_2` не объединяется.

Таблицы для `ASOF JOIN` должны иметь столбец с отсортированной последовательностью. Этот столбец не может быть единственным в таблице. Столбцы могут быть следующих типов: `UInt32`, `UInt64`, `Float32`, `Float64`, `Date` и `DateTime`.
...
`ASOF JOIN` использует `equi_columnX` для объединения по равенству (`user_id` из нашего примера) и `asof_column` для объединения по ближайшему совпадению.
