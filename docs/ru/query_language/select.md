...
При сэмплировании запросы выполняются не по всем данным, а только по некоторой выборке. Это имеет смысл, когда нужно строить отчеты "налету" по большому количеству данных. Например, если нужно посчитать статистику по всем визитам, достаточно обработать 1/10 всех визитов и затем результат домножить на 10.

Сэмплирование поддерживается только таблицами семейства [MergeTree](../operations/table_engines/mergetree.md) и только в том случае, если для таблиц был указан ключ сэмплирования (выражение, на основе должна производиться выборка). Подробнее см. в разделе [Создание таблиц MergeTree](../operations/table_engines/mergetree.md#table_engine-mergetree-creating-a-table).
...
- `SAMPLE k OFFSET m` – запрос будет выполнен по `k` доле данных. При этом выборка будет сформирована со смещением на `m` долю. [Подробнее](#select-sample-offset)

#### SAMPLE k {#select-sample-k}

Здесь `k` – это дробное число в интервале от 0 до 1 (десятичная форма записи также поддерживается). Например, `SAMPLE 0.5` или `SAMPLE 1/2`.
...
При задании `SAMPLE n` нет информации, какая относительная доля данных была обработана, и на что следует домножить агрегатные функции, чтобы получить приблизительный результат. Например, при задании `SAMPLE 10000000` неизвестно, выборка была взята из 100,000,000 строк или из 1,000,000,000 строк.

Чтобы посчитать приблизительный результат, используйте столбец `_sample_factor`. Этот столбец автоматически создается в тех таблицах, для которых задано выражение `SAMPLE BY` (подробнее см. в разделе [Создание таблицы MergeTree](../operations/table_engines/mergetree.md#table_engine-mergetree-creating-a-table)). Ниже приведены примеры использования столбца `_sample_factor`.
...
В примере ниже рассчитывается среднее время на сайте:
...
Обратите внимание, при расчете средних значений, умножать результат на коэффициент сэмплирования не нужно.

#### SAMPLE k OFFSET m {#select-sample-offset}

Здесь `k` и `m` – числа от 0 до 1. Например, `SAMPLE 0.1 OFFSET 0.5`. Поддерживается как дробная, так и десятичная форма записи.
