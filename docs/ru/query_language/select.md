...
Чтобы включить в результат несколько стоблцов, выбрав их имена с помощью регулярных выражений [re2](https://en.wikipedia.org/wiki/RE2_(software)), используйте выражение `COLUMNS`.
...
Аккуратно используйте функции, поскольку выражение `COLUMN` возвращает переменное число столбцов и если функция не поддерживает результирующее число аргументов, то ClickHouse генерирует исключение.
...
Стоблцы, которые возвращаются выражением `COLUMNS` могут быть разных типов. Если `COLUMNS` не возвращает ни одного столбца и это единственное выражение в запросе `SELECT`, то ClickHouse генерирует исключение.
