...
!!! note "Внимание"
    Не стоит использовать сэмплирование тех в задачах, где важна точность расчетов. Например, при работе с финансовыми отчетами.
...
Здесь `k` – число в интервале от 0 до 1. Поддерживаются как дробная, так и десятичная формы записи. Например, `SAMPLE 1/2` или `SAMPLE 0.5`.
...
Здесь `k` и `m` – числа в интервале от 0 до 1. Например, `SAMPLE 0.1 OFFSET 0.5`. Поддерживаются как дробная, так и десятичная формы записи.
