...
Секция `SAMPLE` позволяет использовать сэмплирование, то есть выполнять запросы приближённо. 

При сэмплировании запросы выполняются не по всем данным, а только по некоторой выборке. Это имеет смысл, когда нужно строить отчеты "налету" по большому количеству данных. Например, если нужно посчитать статистику по всем визитам, достаточно обработать 1/10 всех визитов и результат домножить на 10.
