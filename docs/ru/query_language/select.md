...
Аккуратно используйте функции, поскольку выражение `COLUMNS` возвращает переменное число столбцов и если функция не поддерживает получившееся число аргументов, то ClickHouse генерирует исключение.
