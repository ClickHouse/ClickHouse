...
ClickHouse использует алгоритм [hash join](https://en.wikipedia.org/wiki/Hash_join). ClickHouse принимает `<right_subquery>` и создает для него хэш-таблицу  в ОЗУ. Чтобы ограничить потребление памяти операцией `JOIN`, используйте следующие параметры:
...
Если есть `ORDER BY` с небольшим `LIMIT` после `GROUP BY`, то `ORDER BY` не будет потреблять существенное количество RAM.
Но если `ORDER BY` используется без `LIMIT`, не забудьте включить внешнюю сотрировку (`max_bytes_before_external_sort`).
...
Вычисляются дополнительные две строчки - минимумы и максимумы, соответственно. Эти две дополнительные строки выводятся в форматах `JSON*`, `TabSeparated*`, и `Pretty*`. В остальных форматах они не выводится.
