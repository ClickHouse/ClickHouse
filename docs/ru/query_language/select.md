...
Секция `SAMPLE` позволяет выполнять запросы приближённо. 

При сэмплировании запросы выполняются не целиком по всем данным, а только по некоторой выборке. Это имеет смысл, когда необходимо строить различные отчеты налету, и при этом допускается некоторая погрешность в результатах. Например, для подсчета общего числа визитов можно сначала обработать 1/10 всех визитов, а затем этот результат домножить на 10.

Приближённое выполнение запросов поддерживается только таблицами семейства [MergeTree]( ../operations/table_engines/mergetree.md) и только в том случае, если для таблиц был указан ключ сэмплирования (выражение, по которому должна производиться выборка).

Свойства сэмплирования:

- Сэмплирование работает детерминированно. Результат выполнения одного и того же запроса `SELECT .. SAMPLE` всегда будет одинаковым.
- Поддерживает консистентность для разных таблиц. Имеется в виду, что для таблиц с одним и тем же ключом сэмплирования, выборка с одинаковым коэффициентом всегда будет составлять одно и то же подмножество данных. Например, выборка по идентификаторам посетителей, выберет из разных таблиц строки с одинаковым подмножеством всех возможных идентификаторов посетителей. Это позволяет использовать выборку в подзапросах в секции `IN`, а также при ручном сопоставлении результатов разных запросов с выборками.
...
Выражение `SAMPLE` можно задать следующими способами:

- `SAMPLE k` – запрос будет выполнен по `k` доле данных. Например, если указано `SAMPLE 0.1`, то запрос будет выполнен для выборки из 1/10 данных. [Подробнее](#select-sample-k)
- `SAMPLE n` – запрос будет выполнен для выборки, состоящей из не менее чем `n` строк. Например, если указано `SAMPLE 10000000`, то запрос будет выполнен не менее чем для 10 000 000 строк. [Подробнее](#select-sample-n)
- `SAMPLE k OFFSET m` – запрос будет выполнен по `k` доле данных. При этом выборка будет сформирована со смещением на `m`. [Подробнее](#select-sample-offset)

#### SAMPLE k {#select-sample-k}

В выражении `SAMPLE k` задается доля данных, для которых будет выполнен запрос. где `k` – это дробное число в интервале от 0 до 1. В этом случае запрос будет выполнен по `k` доле от всех данных. 
...
В этом примере, запрос выполняется по выборке из 0.1 (10%) данных. Значения агрегатных функций не корректируются автоматически, поэтому для получения приближённого результата, значение count() нужно вручную умножить на 10.
...
!!! note
    Следует иметь в виде, что `n` должно быть достаточно большим целым числом. Так как минимальной единицей данных для чтения является одна гранула (её размер задаётся настройкой `index_granularity` для таблицы), имеет смысл создавать выборки, размер которых существенно превосходит размер гранулы.

При использовании выражения `SAMPLE n`, относительный коэффициент сэмплирования каждый раз рассчитывается динамически. Это означает, что явно неизвестно, какая доля данных использовалась для выборки, и значит, вы не будете знать, на какой коэффициент нужно умножить результат агрегатной функции, чтобы получить приблизительный результат (например, при указании `SAMPLE 1000000` вам будет неизвестно, взята ли эта выборка из 10,000,000 строк или из 1,000,000,000 строк). Чтобы посчитать приближённый результат, используйте столбец `_sample_factor`.

В столбце `_sample_factor` ClickHouse хранит относительные коэффициенты сэмплирования. Этот столбец содержится в тех таблицах, для которых при создании было задано выражение `SAMPLE BY` (подробнее см. в разделе [Создание таблицы MergeTree](../operations/table_engines/mergetree.md#table_engine-mergetree-creating-a-table)). Ниже приведены примеры.

Пример ниже показывает, как с помощью столбца `_sample_factor` рассчитать приблизительное число просмотров сайта:
...
Пример ниже показывает, как посчитать приблизительное число уникальных посетителей сайта:
...
Обратите внимание, для расчета среднего значения в запросе `SELECT.. SAMPLE n` столбец `_sample_factor` не нужен:
...
Также при работе с сэмплированием поддерживается синтаксис `SAMPLE k OFFSET m`, где `k` и `m` числа от 0 до 1. Примеры приведены ниже.
...
Здесь выборка из 1/10 доли взята из второй половины данных.
