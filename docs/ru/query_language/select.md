...
- `LEFT ARRAY JOIN` — пустые массивы попадут в результат выполнения `JOIN`. В качестве значений для пустых массивов устанавливается значения по умолчанию (в зависимости от типа элементов массива, обычно это 0, пустая строка или NULL).
...
В секции `ARRAY JOIN` можно через запятую указать сразу несколько массивов. В этом случае, `JOIN` делается с ними одновременно (прямая сумма, а не прямое произведение). Обратите внимание, массивы должны быть одинаковых размеров. Примеры:
