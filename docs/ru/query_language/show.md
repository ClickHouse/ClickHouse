...
**query** - текст запроса. В запросах INSERT данные для вставки не выводятся.

**query_id** - идентификатор запроса. Задается пользователем явно, по умолчанию пустая строка. При распределённой обработке запроса идентификатор запроса не передаётся на удалённые серверы.
