...
- `икс` - Столбец со значениями зависимой переменной.
...
Функция реализует стохастическую линейную регрессию. Поддерживает пользовательские параметры для скорости обучения, коэффициента регуляризации L2, размера mini-batch и имеет несколько методов обновления весов (simple SGD, Momentum, Nesterov).

Функция работает как обычная агрегатная функция в терминах распределенной обработки данных с последющим мёржем. По отношению к `stochasticLinearRegression` это означает, что различные состяния агрегатной функции мёржатся с весами, чем больше данных находится в состоянии агрегатной функции, тем с большим весом оно будет обладать при мёрже с другими состояниями.
...
1. `learning rate` коэффициент длины шага, при выполнении градиентного спуска. Слишком большая скорость обучения может привести к бесконечным весам модели. По умолчанию `0.00001`.
2. `l2 regularization coefficient` может помочь предотвратить подгонку. По умолчанию `0.1`.
3. `mini-batch size` задаёт количество элементов, чьи градиенты будут вычислены и просуммированы при выполнении одного шага градиентного спуска. Чистый стохастический спуск использует один элемент, однако использование mini-batch (около 10 элементов) делает градиентные шаги более стабильными. По умолчанию `15`.
4. `method for updating weights`, можно выбрать один из следующих: `SGD`, `Momentum`, `Nesterov`. `Momentum` и `Nesterov` более требовательные к вычислительным ресурсам и памяти, однако они имеют высокую скорость схождения и стальные методы стохастического градиента. По умолчанию `'SGD'`.
...
### Заметки {#agg_functions-stochasticlinearregression-notes}
...
1. Построение модели
    ...
    Смотрите раздел `Построение модели` в описании [stochasticLinearRegression](#agg_functions-stochasticlinearregression-parameters) .
