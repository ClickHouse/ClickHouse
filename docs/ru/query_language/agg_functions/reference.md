...
Создает массив значений исходного массива со всеми удаленными дубликатами. Порядок результирующих значений определяется частотой дублирования в исходном массиве.

- Если значение не имеет пересечений, то порядок определяется исходным массивом.
- Если передан опциональный параметр `N`, то будет создан срез массива с n элементами, возвращенными с самого начала.
...
- `array` — Исходный [массив](../../data_types/array.md).
- `N` — Количество возвращаемых значений. Опциональный параметр. [UInt8](../../data_types/int_uint.md).

**Возвращаемое значение**

Возвращает новый массив без пересекающих значений. 
