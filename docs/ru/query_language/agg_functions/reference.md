...
Вычисляет [среднее арифметическое взвешенное](https://en.wikipedia.org/wiki/Weighted_arithmetic_mean).
...
Типы всех аргументов должны совпадать, в том числе тип значения и его веса.

**Возвращаемое значение**

- Среднее арифметическое взвешенное. Тип: [число с плавающей запятой](../../data_types/float.md).
- Если все веса равны 0, возвращает `NaN`.
