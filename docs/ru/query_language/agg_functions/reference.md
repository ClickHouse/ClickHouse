...
`uid` уникальный идентификатор временного ряда, UInt64.`timestamp` имеет тип Int64 чтобы можно было учитывать милли и микросекунды.`value` представляет собой значение метрики.
