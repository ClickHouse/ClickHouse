...
`number_of_bins` — максимальное количество столбцов в гистограмме. Функция автоматически вычисляет количество столбцов. Она пытается получить указанное количество столбцов, но если не получилось, то в результате столбцов будет меньше.
...
- [Массив](../../data_types/array.md) [кортежей](../../data_types/tuple.md) следующего вида:
    ...
    - `lower` — нижняя граница столбца.
    - `upper` — верхняя граница столбца.
    - `height` — высота столбца.
...
В этом случае необходимо помнить, что границы столбцов гистограммы не известны.
