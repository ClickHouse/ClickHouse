...
- `window` — максимальная дистанция между парой событий в секундах.
...
- Функция отыскивает данные, на которых срабатывает первое условие из цепочки, и присваивает счетчику событий значение 1. 
- Если в пределах окна последовательно попадается следующее событие из цепочки, то счетчик увеличивается и скользящее окно перемещается. Если последовательность событий нарушается, то счетчик не растёт.
...
- Целое число. Максимальное количество последовательно сработавших условий из цепочки, при этом расстояние между каждой парой событий не превышает размера окна. Исследуются все цепочки в выборке.
