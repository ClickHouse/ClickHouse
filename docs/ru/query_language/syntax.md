...
Ключевые слова не зарезервированы (а всего лишь парсятся как ключевые слова в соответствующем контексте). Если вы используете [идентификаторы](#syntax-identifiers)  такие же, как и ключевые слова, заключите их в кавычки. Например, запрос `SELECT "FROM" FROM table_name` валиден, если таблица `table_name` имеет стоблец с именем `"FROM"`.
...
Кортежи носят служебное значение для использования в секции `IN` запроса `SELECT`. Кортежи могут быть получены врезультате запроса, но они не могут быть сохранены в базе данных (за исключением таблицы [Memory](../operations/table_engines/memory.md).)
