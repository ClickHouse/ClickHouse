...
- Стандарту SQL. Например, `SELECT = select`, `AS = as`.
- Реализации в некоторых популярных DBMS (MySQL или Postgres). Например, `DateTime = datetime`.

Зависимость от регистра для имён типов данных можно проверить в таблице `system.data_type_families`.

Все остальные ключевые слова (названия столбцов, функций и т.д.) зависят от регистра, что не соответствует стандарту SQL.
