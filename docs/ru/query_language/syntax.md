...
### Строковые {syntax-string_literal}
...
Синонимы не передаются внутрь подзапросов и между подзапросами. Например, при выполнении запроса `SELECT (SELECT sum(b.a) + num FROM b) - a.a AS num FROM a`  ClickHouse сгенерирует исключение `Unknown identifier: num`.
