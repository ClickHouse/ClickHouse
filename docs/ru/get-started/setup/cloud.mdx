...
<Steps>
  ...
  <Step title="Добавить данные" id="3-add-data">
    ...
    * Загрузите данные по URL-адресу файла
    ...
    [ClickPipes](/ru/integrations/clickpipes/home) — это управляемая интеграционная платформа, которая позволяет выполнять загрузку данных из самых разных источников буквально в несколько кликов. ClickPipes рассчитана на самые требовательные рабочие нагрузки: её надёжная и масштабируемая архитектура обеспечивает стабильную производительность и отказоустойчивость. ClickPipes подходит как для長期 постоянного стриминга, так и для разовой загрузки данных.
    ...
    * первичные ключи в ClickHouse ***не являются уникальными*** для каждой строки таблицы
    ...
    данных (это называется **гранулярностью индекса**) добавляют запись в файл индекса первичного ключа. Такой подход к
    гранулярности формирует **разреженный индекс**, который легко умещается в памяти, а гранулы представляют собой блок
    минимального объёма данных столбцов, обрабатываемых при выполнении запросов `SELECT`.

    Первичный ключ можно задать с помощью параметра `PRIMARY KEY`. Если при создании таблицы `PRIMARY KEY` не указан,
    то ключом становится кортеж, заданный в секции `ORDER BY`. Если указаны и `PRIMARY KEY`, и `ORDER BY`, первичный ключ должен быть подмножеством ключа сортировки.
    ...
    В ClickHouse можно использовать привычную команду [`INSERT INTO TABLE`](/ru/reference/statements/insert-into), однако важно понимать, что каждая вставка в таблицу [`MergeTree`](/ru/reference/engines/table-engines/mergetree-family/mergetree) приводит к созданию в хранилище нового **куска данных** (part).
    ...
    Также вы можете подключиться к своему сервису ClickHouse Cloud с помощью инструмента командной строки [**clickhouse client**](/ru/concepts/features/interfaces/cli). Чтобы получить эти данные, нажмите `Connect` в меню слева. В открывшемся диалоговом окне выберите `Native` в выпадающем списке:
