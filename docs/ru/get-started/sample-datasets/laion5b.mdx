...
<Steps>
  ...
  <Step title="Загрузите данные" id="load-table">
  ...
  <Step title="Выполнение полного перебора для векторного поиска по сходству" id="run-a-brute-force-vector-similarity-search">
  ...
  <Step title="Генерация эмбеддинг-векторов для поискового запроса" id="generating-embeddings-for-search-query">
