...
Нельзя объединять интервалы с различных типов. Не используйте интервалы вида `4 DAY 1 HOUR`. Вместо этого выражайте интервал в единицах меньших или равных минимальной единице интервала, например, интервал `1 day and an hour` можно выразить как `25 HOUR` или `90000 SECOND`.

Арифметические операции со значениями типов `Interval` не доступны, однако можно добавлять различные интервалы к значениям типов `Date` и `DateTime`. Например:
