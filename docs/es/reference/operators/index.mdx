...
`N` también puede ser un array de enteros; en ese caso, los elementos de todas esas posiciones se devuelven como un array,
igual que con `arrayMap(i -> a[i], N)`. Las posiciones pueden ser anulables. Una posición `NULL` produce `NULL` cuando el tipo de elemento
puede envolverse en `Nullable`; para los tipos de elemento que no pueden estar dentro de `Nullable` (como `Array` o `Map`), produce
el valor predeterminado del tipo de elemento, igual que un índice escalar `NULL`.
...
### función equals {#equals-function}

`a = b` – La función `equals(a, b)`.

`a == b` – La función `equals(a, b)`.

### función notEquals {#notequals-function}

`a != b`: la función `notEquals(a, b)`.

`a <> b`: la función `notEquals(a, b)`.

### función lessOrEquals {#lessorequals-function}

`a <= b`: la función `lessOrEquals(a, b)`.

### función greaterOrEquals {#greaterorequals-function}

`a >= b` – La función `greaterOrEquals(a, b)`.

### función less {#less-function}

`a < b` – La función `less(a, b)`.

### función greater {#greater-function}

`a > b` – La función `greater(a, b)`.

### función like {#like-function}

`a LIKE b` – La función `like(a, b)`.

### función notLike {#notlike-function}

`a NOT LIKE b` – La función `notLike(a, b)`.

### función ilike {#ilike-function}

`a ILIKE b` – La función `ilike(a, b)`.

### función match {#match-function}

`a REGEXP b` – La función `match(a, b)`.

`a ~ b` – La función `match(a, b)` (coincidencia con expresiones regulares al estilo de PostgreSQL).

### Función notMatch {#notmatch-function}

`a !~ b` – La función `notMatch(a, b)` (comprueba que `a` no coincide con la expresión regular `b`).

### función matchCaseInsensitive {#matchcaseinsensitive-function}

`a ~* b` – La función `matchCaseInsensitive(a, b)` (coincidencia con una expresión regular sin distinguir entre mayúsculas y minúsculas).

### función notMatchCaseInsensitive {#notmatchcaseinsensitive-function}

`a !~* b`: la función `notMatchCaseInsensitive(a, b)` (comprueba que `a` no coincida con la expresión regular `b`, sin distinguir entre mayúsculas y minúsculas).

### Función BETWEEN {#between-function}

`a BETWEEN b AND c` – Equivale a `a >= b AND a <= c`.

`a NOT BETWEEN b AND c` – Equivale a `a < b OR a > c`.

### operador `is not distinct from` (`<=>`) {#is-not-distinct-from}
...
## Operadores para trabajar con cadenas {#operators-for-working-with-strings}
...
### función in {#in-function}

`a IN ...` – La función `in(a, b)`.

### función notIn {#notin-function}

`a NOT IN ...` – La función `notIn(a, b)`.

### función globalIn {#globalin-function}

`a GLOBAL IN ...` – La función `globalIn(a, b)`.

### función globalNotIn {#globalnotin-function}

`a GLOBAL NOT IN ...` – La función `globalNotIn(a, b)`.

### función in subconsulta {#in-subquery-function}

`a = ANY (subquery)` – La función `in(a, subquery)`.

### notIn subconsulta function {#notin-subquery-function}

`a != ANY (subquery)` – Igual que `a NOT IN (SELECT singleValueOrNull(*) FROM subquery)`.

### función in subconsulta {#in-subquery-function-1}

`a = ALL (subquery)` – Lo mismo que `a IN (SELECT singleValueOrNull(*) FROM subquery)`.

### función notIn en subconsulta {#notin-subquery-function-1}
...
### `SOME` / `ALL` en arrays {#some-all-on-arrays}
...
## Operadores para trabajar con fechas y horas {#operators-for-working-with-dates-and-times}
...
### Suma de fecha y hora {#date-time-addition}
...
<Note>
  El resultado utiliza la [zona horaria de la sesión](/es/reference/settings/session-settings/other#session_timezone) (o la zona horaria predeterminada del servidor si no se ha configurado ninguna para la sesión). La configuración [`date_time_overflow_behavior`](/es/reference/settings/formats/date-time#date_time_overflow_behavior) controla qué sucede cuando el resultado queda fuera del rango representable.
</Note>
...
### AT TIME ZONE y AT LOCAL {#at-time-zone}
...
## Operador de creación de lambda {#lambda-creation-operator}

`x -> expr` – La función `lambda(x, expr)`.

Los siguientes operadores no tienen prioridad, ya que son corchetes:

## Operador de creación de Array {#array-creation-operator}
...
Puede optimizarse habilitando la configuración [optimize&#95;functions&#95;to&#95;subcolumns](/es/reference/settings/session-settings/optimize#optimize_functions_to_subcolumns). Con `optimize_functions_to_subcolumns = 1`, la función lee solo la [subcolumna](/es/reference/data-types/nullable#finding-null) `null` en lugar de leer y procesar todos los datos de la columna. La consulta `SELECT n IS NULL FROM table` se transforma en `SELECT n.null != 0 FROM TABLE`.
