...
<img className="w-full h-auto" src="/images/interfaces/native-format/block-structure.webp" alt="Estructura de un bloque columnar transmitido y sus familias de disposición de datos" width="1568" height="976" loading="lazy" />
...
<div id="block-wire-layout">
  ### disposición en formato wire del Block
</div>
...
<img className="w-full h-auto" src="/images/interfaces/native-format/decoder-dispatch.webp" alt="Enrutamiento del decodificador desde una descripción de tipo hasta su familia de codificación" width="1568" height="854" loading="lazy" />
...
Así que, en la revisión `0` —el valor predeterminado para `FORMAT Native`— `LowCardinality(T)` **no** se elimina. Su cadena de tipo y su prefijo de estado por bloque permanecen en el flujo, y el lector con revisión `0` los vuelve a leer tal cual. La reducción solo se activa con una revisión distinta de cero por debajo de `54405`, o cuando se fuerza independientemente de la revisión.

Ese forzado es el indicador `remove_low_cardinality`. La salida de `FORMAT Native` nunca lo activa, pero la ruta del protocolo TCP nativo sí, cuando `low_cardinality_allow_in_native_format = 0` (el valor predeterminado es `1`). En otras palabras, esa configuración cambia la salida de native TCP, pero no hace nada en `FORMAT Native`.

La conclusión práctica: un flujo `FORMAT Native` predeterminado puede contener legítimamente `LowCardinality`, así que no lo trate como una funcionalidad ausente en la revisión `0`.

<div id="revision-per-channel">
  ### De dónde proviene la revisión, según por dónde viajan los datos
</div>
...
En el [protocolo TCP nativo](/es/reference/interfaces/specs/NativeProtocol), la revisión se obtiene del handshake Hello. El client envía `DBMS_TCP_PROTOCOL_VERSION`, el server devuelve la suya y, a partir de ese momento, cada lado serializa con la **revisión que anunció el otro**: el server construye su `NativeReader`/`NativeWriter` a partir de `client_tcp_protocol_version`, mientras que el client usa la `server_revision` que recibió. No hay un `min` explícito, pero ninguna de las partes puede emitir una funcionalidad que no haya implementado, así que, en la práctica, cada dirección queda limitada por el más antiguo de los dos pares.
...
<div id="revision-output">
  #### salida de `FORMAT Native`: revisión `0` de forma predeterminada, elevable por HTTP
</div>

El formato de *salida* `Native` usa la revisión **`0`** de forma predeterminada. Esto abarca `SELECT ... FORMAT Native` por HTTP, `INTO OUTFILE ... FORMAT Native` y la salida `Native` que escribe `clickhouse-client`; en cada caso, la factoría de salida pasa `FormatSettings::client_protocol_version` directamente a `NativeWriter`.

En HTTP, ese valor predeterminado no lo explica todo. Un client puede elevarlo con el parámetro de consulta `?client_protocol_version=<n>`, que el handler HTTP trata como un parámetro reservado en lugar de un ajuste SQL: llega al contexto de la consulta y la capa de formato lo copia en `FormatSettings`. Si se establece en un valor lo bastante alto, la salida HTTP de `FORMAT Native` empieza a incluir el prefijo `BlockInfo` y el byte `has_custom_serialization`, igual que en la ruta TCP; por tanto, no asuma que un payload HTTP de `FORMAT Native` es siempre de revisión `0`. Las exportaciones a archivo y la salida local de `clickhouse-client` no tienen ese ajuste y se mantienen en `0`.

<div id="revision-input">
  #### `FORMAT Native` de entrada: siempre revisión 0
</div>

El formato `Native` de *entrada* funciona al revés: está **fijado de forma rígida a la revisión `0`** y no presta ninguna atención a `client_protocol_version`. Tanto si analiza el cuerpo de un `INSERT ... FORMAT Native` como si lee un archivo `Native`, construye su `NativeReader` con un `0` literal, por lo que nunca espera un prefijo `BlockInfo`, nunca lee el byte `has_custom_serialization` y siempre asume la serialización predeterminada.

Por tanto, `client_protocol_version` solo se aplica a la salida. Establecer un valor alto de `?client_protocol_version=` (por ejemplo, `DBMS_TCP_PROTOCOL_VERSION`) en una solicitud `INSERT ... FORMAT Native` no cambia en nada cómo se lee el cuerpo: el cuerpo sigue teniendo que ser de revisión `0`. Si se envía un cuerpo que sí incluye un prefijo `BlockInfo` o un byte `has_custom_serialization`, el lector pierde la sincronización, lo que se devuelve como un error de análisis (`INCORRECT_DATA` o `CANNOT_READ_ALL_DATA`) en lugar de una inserción correcta.

<div id="revision-round-trip">
  ### Implicaciones del recorrido de ida y vuelta
</div>

Para `FORMAT Native`, la opción segura es usar la revisión `0` en ambos extremos, que es la que se usa de forma predeterminada. Los datos escritos por `SELECT ... FORMAT Native` con revisión `0` se pueden volver a leer directamente en `INSERT ... FORMAT Native` sin sorpresas.

Los problemas solo empiezan si aumentas deliberadamente la revisión de salida. Un `SELECT ... FORMAT Native` realizado con `?client_protocol_version=<large>` produce un flujo que incluye bytes `BlockInfo` y `has_custom_serialization`, y la ruta de entrada con revisión `0` no puede volver a leerlos. Si necesitas que esos datos completen el recorrido de ida y vuelta, no incluyas `client_protocol_version` en el `SELECT` que los genera, o transfiere los datos mediante el protocolo TCP nativo —donde cada dirección usa la revisión negociada en el `handshake`— en lugar de `FORMAT Native`.
...
<img className="w-full h-auto" src="/images/interfaces/native-format/array-offsets.webp" alt="Desplazamientos que asignan filas a un flujo de valores compartido" width="1568" height="388" loading="lazy" />
...
<img className="w-full h-auto" src="/images/interfaces/native-format/nested-layout.webp" alt="Diseño de columna compuesta con y sin aplanamiento" width="1172" height="1068" loading="lazy" />
...
<img className="w-full h-auto" src="/images/interfaces/native-format/state-prefix-per-block.webp" alt="Prefijo de estado repetido para cada block no vacío" width="1226" height="1162" loading="lazy" />
...
<img className="w-full h-auto" src="/images/interfaces/native-format/variant-discriminators.webp" alt="Discriminadores que seleccionan valores de secuencias densas específicas de cada tipo" width="1568" height="624" loading="lazy" />
...
<img className="w-full h-auto" src="/images/interfaces/native-format/compression-frame.webp" alt="Campos de la trama de compresión y límite de tamaño comprimido" width="1568" height="206" loading="lazy" />

<div id="method-byte-values">
  ### Valores de byte del método
</div>

Estos tres códecs son los que produce el servidor para el framing `Native` de flujo completo: la salida HTTP `compress=1` siempre usa `LZ4`, y el protocolo TCP nativo usa `LZ4`, `ZSTD` o `NONE` según `network_compression_method`. Un client `Native` genérico solo necesita producir y consumir estos.

| Byte   | Método | Codificación del cuerpo                                                                                                           |
| ------ | ------ | --------------------------------------------------------------------------------------------------------------------------------- |
| `0x02` | NONE   | El cuerpo son los bytes sin procesar (sin compresión). La trama se sigue emitiendo; el receiver verifica la suma de comprobación. |
| `0x82` | LZ4    | El cuerpo está en el **formato de block LZ4** — *no* en el formato de trama LZ4. Sin número mágico.                               |
...
El byte de método también codifica los [códecs a nivel de columna](/es/reference/statements/create/table/codec). Estos se aplican por columna en las rutas en disco de MergeTree, en lugar de al framing de flujo completo, pero la ruta de entrada HTTP `decompress=1` toma el códec del byte de método de cada trama, por lo que cualquiera de estos bytes puede aparecer legítimamente en la entrada. Por lo tanto, un decodificador compatible debe reconocer todo el espacio asignado y rechazar cualquier byte que no implemente, en lugar de interpretar mal el cuerpo. Sus cuerpos son específicos de cada códec y quedan fuera de este contrato genérico de trama:
