...
Eso convierte a la revisión `0` en la codificación más conservadora para casi todo: el flujo no lleva `BlockInfo`, ni el byte `has_custom_serialization`, usa `Dynamic`/`JSON` V1, no incluye versión de función de agregación y anuncia un `DateTime` sin el parámetro de timezone.

`LowCardinality` es la única excepción, y es importante. La comprobación del emisor es `remove_low_cardinality || (client_revision && client_revision < DBMS_MIN_REVISION_WITH_LOW_CARDINALITY_TYPE)`. La clave está en el prefijo `client_revision &&`: cuando la revisión es exactamente `0`, toda la condición hace short-circuit y da false.
...
<Note>
  ...
  * `QBit(T, N[, stride])` almacena un vector con sus bit planes transpuestos para workloads de búsqueda vectorial; su disposición de stream on-wire (streams de bit planes `FixedString` en orden por grupo, `element_size * (N / stride)` de ellos con un `stride` explícito) y su codificación binaria de tipo (etiqueta `0x36`, o `0x37` `QBitWithStride` cuando `stride != N`) están documentadas en la [página del tipo de dato `QBit`](/es/reference/data-types/qbit) y en la referencia de [codificación binaria de tipos](/es/reference/data-types/data-types-binary-encoding), por lo que un reader `Native` no tiene que recuperarlas del código fuente en C++.
</Note>
...
El byte de método también codifica los [códecs a nivel de columna](/es/reference/statements/create/table#column_compression_codec). Estos se aplican por columna en las rutas on-disk de MergeTree, en lugar de al framing de flujo completo, pero la ruta de entrada HTTP `decompress=1` toma el códec del byte de método de cada trama, por lo que cualquiera de estos bytes puede aparecer legítimamente en la entrada. Por lo tanto, un decodificador compatible debe reconocer todo el espacio asignado y rechazar cualquier byte que no implemente, en lugar de interpretar mal el cuerpo. Sus cuerpos son específicos de cada códec y quedan fuera de este contrato genérico de trama:
...
`0x9d` (`SZ3`) es un códec *lossy* **experimental**, con error acotado, para `Float32`, `Float64` y `Array` de esos tipos. Una table solo puede crearse con `CODEC(SZ3)` cuando `allow_experimental_codecs` está habilitado, pero el byte de método siempre se acepta durante la descompresión para que los datos escritos previamente sigan siendo legibles. `0x9f` (`ZXC`) es un códec LZ asimétrico: lento al comprimir, pero muy rápido al descomprimir, con una ratio entre `LZ4` y `ZSTD`. Los bytes `0x99` (`DeflateQpl`) y `0x9b` (`ZSTD_QPL`) se asignaron a códecs que desde entonces se han eliminado; están reservados y no se reutilizan.

`0x9e` (`Quantized`) es un códec de columna **experimental** para columnas de vectores densos (`Array(Float32)` y similares). Al igual que `NONE`, es passthrough: el cuerpo con full-precision se almacena literalmente, pero su presencia adjunta una serialización que escribe un flujo complementario cuantizado compacto usado para acelerar la vector search. Una table solo puede crearse con `CODEC(Quantized(...))` cuando `allow_experimental_codecs` está habilitado, y el byte de método siempre se acepta durante la descompresión.
