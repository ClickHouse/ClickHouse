...
Nombre de la colección con nombre que usa `aiEmbed` cuando la llamada no proporciona `credentials` en su mapa de parámetros. Vacío significa que no hay ningún valor predeterminado: dichas llamadas deben proporcionar `credentials` explícitamente. Se mantiene separado de `ai_function_text_default_credentials` porque el endpoint y el modelo de embeddings son distintos de los de chat.
...
Nombre de la colección con nombre utilizada por las funciones de IA de texto (`aiGenerate`, `aiClassify`, `aiExtract`, `aiTranslate`) cuando la llamada no pasa `credentials` en su mapa de parámetros. Vacío significa que no hay ningún valor predeterminado: esas llamadas deben pasar `credentials` explícitamente. Un endpoint y un modelo de chat completions son distintos de los de embeddings, por lo que esto es independiente de `ai_function_embedding_default_credentials`.
...
Si algunos ajustes tienen valores distintos de los predeterminados, esos valores se respetan (solo los ajustes que no se han modificado se ve n afectados por la configuración `compatibility`).
...
el usuario puede evitar que se deduplicen los mismos datos insertados.
...
Habilita las escrituras con cuórum.

* Si `insert_quorum < 2`, las escrituras con cuórum están deshabilitadas.
* Si `insert_quorum >= 2`, las escrituras con cuórum están habilitadas.
* Si `insert_quorum = 'auto'`, usa la mayoría (`number_of_replicas / 2 + 1`) como número de cuórum.

Escrituras con cuórum
...
Cambia el comportamiento de las operaciones `JOIN` con estrictez `ANY`.
...
<Note>
  Actualmente, no se comprueba durante el análisis sintáctico, sino solo después del análisis sintáctico de la consulta.
  Esto significa que durante el análisis sintáctico puede crearse un árbol sintáctico demasiado profundo,
  pero la consulta fallará.
</Note>
...
y se lanza una excepción si el bloque contiene demasiadas particiones.
...
El número máximo de bytes de la cadena de consulta que analiza el analizador SQL.
...
<Note>
  Solo se admite con el analyzer (`enable_analyzer = 1`).
</Note>
...
Habilita o deshabilita la omisión de segmentos no utilizados para las consultas [SELECT](/es/reference/statements/select/index) que tienen una condición sobre la clave de sharding en `WHERE/PREWHERE`, y activa optimizaciones relacionadas para las consultas distribuidas (por ejemplo, la agregación por clave de sharding).

<Note>
  Se asume que los datos se distribuyen por la clave de sharding; de lo contrario, una consulta devuelve un resultado incorrecto.
</Note>
...
* &#39;dpsub&#39; - implementa el algoritmo DPsub, que admite tanto inner joins como joins no internos - considera todos los órdenes de JOIN posibles y encuentra el más óptimo, pero puede ser lento para consultas con muchas tablas y predicados de JOIN.
...
<Note>
  Esta es una configuración de nivel experto que solo deberían usar los desarrolladores para tareas de depuración. Esta configuración puede cambiar en el futuro de formas incompatibles con versiones anteriores o eliminarse.
</Note>
...
Activa o desactiva una optimización a nivel del plan de consulta que utiliza la ordenación del almacenamiento al realizar la ordenación para funciones de ventana.
Solo tiene efecto si el ajuste [`query_plan_enable_optimizations`](#query_plan_enable_optimizations) es 1.
...
Activa o desactiva una optimización a nivel del plan de consulta que intenta usar el índice de similitud vectorial.
Solo tiene efecto si la configuración [`query_plan_enable_optimizations`](#query_plan_enable_optimizations) es 1.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "0.2"},{"label": "Renombrado desde `text_index_density_threshold` (se mantiene como alias); selecciona el algoritmo de intersección de posting lists en el modo de aplicación diferida de posting lists."}]}]} />

Umbral de densidad de posting lists que selecciona el algoritmo de intersección en el modo de aplicación diferida de posting lists (`text_index_posting_list_apply_mode = 'lazy'`).
Por debajo del umbral: intersección leapfrog (favorece las posting lists dispersas). A partir del umbral: intersección de bitmap por fuerza bruta (favorece las posting lists densas).
...
Usa el formato compacto para almacenar bloques para `INSERT` en segundo plano (`distributed_foreground_insert`) en tablas con el motor `Distributed`.
