...
| `CROSS JOIN`       | produce el producto cartesiano de las tablas completas; no se especifican &quot;claves de JOIN&quot;.                                                                                                                                                                                                               |
...
| Tipo                                                | Descripción                                                                                                                                                  |
| --------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `LEFT SEMI JOIN`, `RIGHT SEMI JOIN`                 | Una lista de permitidos sobre &quot;claves de JOIN&quot;, sin generar un producto cartesiano.                                                               |
| `LEFT ANTI JOIN`, `RIGHT ANTI JOIN`                 | Una lista de bloqueados sobre &quot;claves de JOIN&quot;, sin generar un producto cartesiano.                                                               |
...
<Note>
  Cuando [join&#95;algorithm](/es/reference/settings/session-settings/join#join_algorithm) se establece en `partial_merge`, `RIGHT JOIN` y `FULL JOIN` solo son compatibles con strictness `ALL` (`SEMI`, `ANTI`, `ANY` y `ASOF` no son compatibles).
</Note>
...
El tipo de JOIN predeterminado puede sobrescribirse mediante la configuración [`join_default_strictness`](/es/reference/settings/session-settings/join#join_default_strictness).

El comportamiento del servidor ClickHouse en las operaciones `ANY JOIN` depende de la configuración [`any_join_distinct_right_table_keys`](/es/reference/settings/session-settings/other#any_join_distinct_right_table_keys).

**Véase también**

* [`join_algorithm`](/es/reference/settings/session-settings/join#join_algorithm)
* [`join_any_take_last_row`](/es/reference/settings/session-settings/join#join_any_take_last_row)
* [`join_use_nulls`](/es/reference/settings/session-settings/join#join_use_nulls)
* [`partial_merge_join_rows_in_right_blocks`](/es/reference/settings/session-settings/partial-merge#partial_merge_join_rows_in_right_blocks)
* [`join_on_disk_max_files_to_merge`](/es/reference/settings/session-settings/join#join_on_disk_max_files_to_merge)
* [`any_join_distinct_right_table_keys`](/es/reference/settings/session-settings/other#any_join_distinct_right_table_keys)
...
Actualmente, ClickHouse admite `ALL/ANY/SEMI/ANTI INNER/LEFT/RIGHT/FULL JOIN` con condiciones de desigualdad además de las condiciones de igualdad. Las condiciones de desigualdad solo se admiten con los algoritmos de join `hash`, `parallel_hash` y `grace_hash`. Una condición de no equivalencia evaluada durante el join no puede contener `arrayJoin`, ya que debe conservar el número de filas; las condiciones que se aplican a un solo lado y una clave de igualdad sobre `arrayJoin` se extraen antes del join y no se ven afectadas; una condición no disyuntiva de `ALL INNER JOIN` tampoco se ve afectada, ya que en ese caso se aplica después del join. Cuando la expansión depende de un solo lado, muévala a un `ARRAY JOIN` en una subconsulta antes del join; una condición cuyo argumento de `arrayJoin` lee columnas de ambos lados debe reestructurarse.
...
<div id="null-values-in-join-keys">
  ## Valores `NULL` y `NaN` en las claves de `JOIN`
</div>
...
Los valores `NaN` en claves `JOIN` de tipo float no siguen la regla de `NULL` anterior.
La comparación escalar de dos valores `NaN` (`NaN = NaN`) da como resultado `0`; sin embargo, las claves de `JOIN` no se comparan con semántica escalar, por lo que las claves `NaN` pueden coincidir.
Que realmente coincidan es un detalle de implementación y depende del algoritmo de join, el tipo de clave y la configuración de sesión.
No dependa de un comportamiento específico.
Si necesita que las filas con `NaN` no coincidan, asígnelas a valores `NULL`: `ON if(isNaN(A.id), NULL, A.id) = B.id`.
En un `ASOF JOIN`, la columna de coincidencia más próxima se compara mediante ordenación, algo que `NaN` no admite; filtre esas filas en ambos lados.
...
Al unir tablas, pueden aparecer celdas vacías. La configuración [join&#95;use&#95;nulls](/es/reference/settings/session-settings/join#join_use_nulls) define cómo ClickHouse completa estas celdas.
...
De forma predeterminada, ClickHouse usa el algoritmo [hash join](https://en.wikipedia.org/wiki/Hash_join). ClickHouse toma la right&#95;table y crea una tabla hash para ella en la RAM. Si `join_algorithm = 'auto'` está habilitado, al superar cierto umbral de consumo de memoria, ClickHouse cambia al algoritmo [merge](https://en.wikipedia.org/wiki/Sort-merge_join) join. Para ver la descripción de los algoritmos `JOIN`, consulte la configuración [join&#95;algorithm](/es/reference/settings/session-settings/join#join_algorithm).

Si necesita restringir el consumo de memoria de la operación `JOIN`, use la siguiente configuración:

* [max&#95;rows&#95;in&#95;join](/es/reference/settings/session-settings/max-rows#max_rows_in_join) — Limita el número de filas de la tabla hash.
* [max&#95;bytes&#95;in&#95;join](/es/reference/settings/session-settings/max-bytes#max_bytes_in_join) — Limita el tamaño de la tabla hash.

Cuando se alcanza cualquiera de estos límites, ClickHouse actúa según lo que indique la configuración [join&#95;overflow&#95;mode](/es/reference/settings/session-settings/join#join_overflow_mode).
