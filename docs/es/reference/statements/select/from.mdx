...
En una consulta que comienza con `FROM`, la cláusula `SELECT` es opcional; cuando se omite, la consulta funciona como si se hubiera escrito `SELECT *`:
...
Esta forma se utiliza principalmente en consultas con [operadores de canalización](/es/reference/statements/select/pipe-operators):
...
Las consultas `SELECT` con `FINAL` se ejecutan en paralelo. La configuración [max&#95;final&#95;threads](/es/reference/settings/session-settings/max#max_final_threads) limita el número de hilos utilizados.
...
`FINAL` puede aplicarse automáticamente mediante la configuración [FINAL](/es/reference/settings/session-settings/other#final) a todas las tablas de una consulta usando una sesión o un perfil de usuario.
