...
* `SETTINGS`: (Opcional) Permite ajustes adicionales.
  * (Opcional): [check&#95;query&#95;single&#95;value&#95;result](/es/reference/settings/session-settings/check#check_query_single_value_result): Este ajuste controla si la salida es detallada (`0`) o resumida (`1`).
...
Para comprobar el estado de toda una base de datos en lugar de la integridad de tablas individuales, utilice la consulta [`CHECK DATABASE`](/es/reference/statements/check-database). Está pensada principalmente para bases de datos con el motor [`DataLakeCatalog`](/es/reference/engines/database-engines/datalake), donde verifica que el catálogo externo sea accesible.
