...
El comando actualiza directamente la configuración de actualización de la vista existente sin volver a crear la vista materializada ni su tabla de destino, borrar los datos existentes de destino ni cancelar una actualización que ya esté en ejecución. Las lecturas continúan en la tabla de destino existente mientras se cambia la configuración.

Es seguro repetir el comando con la misma especificación `REFRESH` completa. Asegúrese de repetir cada cláusula que deba permanecer configurada, porque cada ejecución reemplaza todos los parámetros de actualización como se describió anteriormente.
