...
Este comando desencadena manualmente la materialización física de las partes de parche creadas por la [actualización ligera](/es/reference/statements/update). Aplica por la fuerza los parches pendientes a las partes de datos reescribiendo únicamente las columnas afectadas.
...
Las partes de parche normalmente se aplican automáticamente durante las fusiones cuando la configuración [`apply_patches_on_merge`](/es/reference/settings/merge-tree-settings/other#apply_patches_on_merge) está habilitada (de forma predeterminada). Sin embargo, puede que quieras desencadenar manualmente la aplicación de parches en estos casos:
...
* [configuración `apply_patches_on_merge`](/es/reference/settings/merge-tree-settings/other#apply_patches_on_merge) - Controlar la aplicación automática de parches durante las fusiones