...
Las proyecciones pueden incluir una cláusula `WHERE` para almacenar solo un subconjunto de filas. Esto resulta útil cuando las consultas filtran con frecuencia según un predicado conocido: la proyección materializa únicamente las filas que coinciden, lo que reduce el almacenamiento y mejora el rendimiento de las consultas.
...
Cuando la cláusula `WHERE` de una consulta **implica** la cláusula `WHERE` de la proyección (es decir, cada condición del filtro de la proyección también está presente en el filtro de la consulta), el optimizador puede usar automáticamente la proyección cuando determina que resulta beneficioso:
...
La comprobación de implicación es conservadora: se basa en una coincidencia exacta de conjunciones sobre la forma canónica de la expresión. Puede pasar por alto algunas oportunidades de optimización válidas (p. ej., implicaciones de rango), pero nunca producirá resultados incorrectos.
...
Use la siguiente sentencia para añadir la descripción de una proyección a los metadatos de una tabla:
...
Utilice la siguiente sentencia para modificar la cláusula [`WITH SETTINGS`](#with-settings) de una proyección existente sin reconstruir sus datos:
...
Para un [índice de proyección](/es/reference/engines/table-engines/mergetree-family/mergetree#projection-index), especifique de nuevo la declaración `INDEX` en lugar de la consulta `SELECT`:
...
La sentencia vuelve a declarar la definición completa de la proyección, pero solo la cláusula `WITH SETTINGS` puede diferir de la definición actual.
La consulta de la proyección (o, en el caso de un índice de proyección, la expresión y el tipo del índice) debe permanecer igual, ya que las partes de proyección existentes almacenan datos generados a partir de ella; para modificarla, use [`DROP PROJECTION`](#drop-projection) seguido de [`ADD PROJECTION`](#add-projection).

El comando solo modifica los metadatos de la tabla y no reescribe datos: las partes de proyección existentes conservan los ajustes con los que se escribieron, mientras que las partes de proyección creadas por futuras inserciones y fusiones usan los nuevos ajustes.
Para reconstruir las partes existentes con los nuevos ajustes, ejecute [`MATERIALIZE PROJECTION`](#materialize-projection).
...
Los comandos `ADD`, `MODIFY`, `DROP` y `CLEAR` son ligeros en el sentido de que solo modifican metadatos o eliminan archivos.
