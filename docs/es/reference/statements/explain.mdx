...
<Note>
  La versión actual de `EXPLAIN ANALYZE` no admite consultas ejecutadas en modo distribuido.
</Note>
...
Examinemos la salida. Primero, veamos la cabecera.
...
* `Read` — filas y bytes sin comprimir leídos de las tablas, con su rendimiento; las mismas cifras que el pie de una consulta normal muestra como &quot;Processed&quot;.
...
`<n>` es el número de procesadores de la fase. Una gran diferencia entre `median` y `max` apunta a un desequilibrio de carga entre los procesadores paralelos.
