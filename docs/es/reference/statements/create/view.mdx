...
* `SQL SECURITY`: `INVOKER` para las vistas normales y `DEFINER` para las vistas materializadas ([configurable en Settings](/es/reference/settings/session-settings/default#default_normal_view_sql_security))
* `DEFINER`: `CURRENT_USER` ([configurable en Settings](/es/reference/settings/session-settings/default#default_view_definer))
...
<Info>
  Esta es una función experimental que puede cambiar de forma incompatible con versiones anteriores en futuras versiones. Habilite el uso de las Window View y de la consulta `WATCH` mediante la opción de configuración [allow&#95;experimental&#95;window&#95;view](/es/reference/settings/session-settings/allow-experimental#allow_experimental_window_view). Introduzca el comando `set allow_experimental_window_view = 1`.
</Info>
