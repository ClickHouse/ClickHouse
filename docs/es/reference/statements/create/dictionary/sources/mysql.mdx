...
<Note>
  `ssl_ca`, `ssl_cert` y `ssl_key` son nombres de archivos que el servidor abre con sus propios privilegios, por lo que solo se aceptan para un diccionario definido en un archivo de configuración del servidor o mediante una colección con nombre definida allí. Una consulta `CREATE DICTIONARY` que especifica directamente las credenciales TLS debe proporcionar su contenido en su lugar, en `ssl_ca_pem`, `ssl_cert_pem` y `ssl_key_pem`. Esos valores se ocultan en los logs y en las consultas `SHOW`, del mismo modo que las contraseñas.
</Note>
...
Es posible conectarse a MySQL en un host local mediante sockets. Para ello, configure `host` y `socket`.
