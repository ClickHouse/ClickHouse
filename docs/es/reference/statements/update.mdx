...
Una consulta de [eliminación ligera](/es/reference/statements/delete) puede ejecutarse como una actualización ligera `UPDATE` en lugar de una mutación `ALTER UPDATE`. La implementación de la eliminación ligera se controla mediante el ajuste [`lightweight_delete_mode`](/es/reference/settings/session-settings#lightweight_delete_mode).
