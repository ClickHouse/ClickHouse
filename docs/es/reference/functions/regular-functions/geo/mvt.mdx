...
El tipo de geometría de salida depende de la entrada: un `Point` devuelve un `Point`; un `MultiPoint` devuelve un `MultiPoint`; un `LineString` o `MultiLineString` devuelve un
...
* `geometry` — Geometría en grados de longitud/latitud. La longitud se limita a `[-180, 180]` y la latitud al rango de Web Mercator `[-85.05112878, 85.05112878]`. [`Point`](/es/reference/data-types/geo) / [`MultiPoint`](/es/reference/data-types/geo) / [`LineString`](/es/reference/data-types/geo) / [`MultiLineString`](/es/reference/data-types/geo) / [`Ring`](/es/reference/data-types/geo) / [`Polygon`](/es/reference/data-types/geo) / [`MultiPolygon`](/es/reference/data-types/geo) / [`Geometry`](/es/reference/data-types/geo).
