...
Clasifica el texto de entrada mediante un modelo de Naive Bayes con n-gramas y suavizado de Laplace.
El modelo debe configurarse en ClickHouse antes de usarlo.

**Detalles de implementación**

*Algoritmo*

Utiliza el algoritmo de clasificación Naive Bayes con [suavizado de Laplace](https://en.wikipedia.org/wiki/Additive_smoothing) para manejar n-gramas no vistos, en función de las probabilidades de los n-gramas, como se describe [aquí](https://web.stanford.edu/~jurafsky/slp3/4.pdf).
...
* Tres modos de tokenización:
  * `byte`: Opera sobre bytes sin procesar. Cada byte es un token.
  * `codepoint`: Opera sobre valores escalares Unicode decodificados desde UTF-8. Cada punto de código es un token.
  * `token`: Divide en secuencias de espacios en blanco Unicode (regex `\s+`). Los tokens son subcadenas sin espacios en blanco; la puntuación forma parte del token si está adyacente (p. ej., `you?` es un único token).

**Configuración del modelo**

Hay disponible [aquí](https://github.com/nihalzp/ClickHouse-NaiveBayesClassifier-Models) código fuente de ejemplo para crear un modelo de Naive Bayes para la detección de idioma, junto con modelos de ejemplo y sus archivos de configuración asociados [aquí](https://github.com/nihalzp/ClickHouse-NaiveBayesClassifier-Models/tree/main/models).

Un ejemplo de configuración de un modelo de Naive Bayes en ClickHouse:
...
| --------- | ---------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------- | --------------------- |
| `name`    | Identificador único del modelo.                                                                                        | `language_detection`                                     | Obligatorio           |
| `path`    | Ruta completa del archivo binario del modelo.                                                                          | `/etc/clickhouse-server/config.d/language_detection.bin` | Obligatorio           |
| `mode`    | Método de tokenización: `byte` (secuencias de bytes), `codepoint` (caracteres Unicode) o `token` (tokens de palabras). | `token`                                                  | Obligatorio           |
| `n`       | Tamaño del n-grama: `1` (una sola palabra), `2` (pares de palabras) o `3` (tripletes de palabras).                     | `2`                                                      | Obligatorio           |
| `alpha`   | Factor de suavizado de Laplace utilizado durante la clasificación para n-gramas que no aparecen en el modelo.          | `0.5`                                                    | `1.0`                 |
| `priors`  | Probabilidades de clase (porcentaje de documentos que pertenecen a una clase).                                         | 60% clase 0, 40% clase 1                                 | Distribución uniforme |
...
Para `n=3` y el modo `codepoint`, sería algo así:
...
El formato legible para humanos no se usa directamente en ClickHouse; debe convertirse al formato binario que se describe a continuación.
...
1. `class_id` de 4 bytes (UInt, little-endian).
2. Longitud en bytes del `n-gram` de 4 bytes (UInt, little-endian).
3. Bytes sin procesar del `n-gram`.
4. `count` de 4 bytes (UInt, little-endian).

*Requisitos de preprocesamiento*

Antes de crear el modelo a partir del corpus de documentos, los documentos deben preprocesarse para extraer n-gramas según el `mode` y `n` especificados:

1. Agregue marcadores de límite al inicio y al final de cada documento según el modo de tokenización:

   * `byte`: `0x01` (inicio), `0xFF` (fin)
   * `codepoint`: `U+10FFFE` (inicio), `U+10FFFF` (fin)
   * `token`: `<s>` (inicio), `</s>` (fin)

   Nota: se agregan `(n - 1)` tokens tanto al principio como al final del documento.

2. Ejemplo para `n=3` en modo `token`:
   * Documento: `ClickHouse is fast`
   * Se procesa como: `<s> <s> ClickHouse is fast </s> </s>`
   * Trigramas generados:
     * `<s> <s> ClickHouse`
...
Para simplificar la creación del modelo en los modos `byte` y `codepoint`, puede resultar útil tokenizar primero el documento (una lista de bytes para el modo `byte` y una lista de puntos de código para el modo `codepoint`), agregar `n - 1` tokens de inicio al principio y `n - 1` tokens de fin al final, generar luego los n-gramas y escribirlos en el archivo serializado.
...
* `model_name` — Nombre del modelo preconfigurado. El modelo debe estar definido en los archivos de configuración de ClickHouse. [`String`](/es/reference/data-types/string)
* `input_text` — Texto que se va a clasificar. La entrada se procesa exactamente como se proporciona (se conservan las mayúsculas/minúsculas y la puntuación). [`String`](/es/reference/data-types/string)

**Valor devuelto**

ID de la clase predicha como un entero sin signo. Los ID de clase corresponden a las categorías definidas durante la construcción del modelo. [`UInt32`](/es/reference/data-types/int-uint)

**Ejemplos**

**Clasificar el idioma de un texto**
