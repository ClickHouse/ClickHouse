...
Calcula el embedding de ambos textos y devuelve la similitud coseno entre los dos vectores
en el rango `[-1, 1]`: `1` significa que los textos son semánticamente idénticos, `0` que no están relacionados
y los valores negativos que son opuestos. Es el complemento de `cosineDistance` para los
mismos embeddings (`aiSimilarity = 1 - cosineDistance(embedding1, embedding2)`).
