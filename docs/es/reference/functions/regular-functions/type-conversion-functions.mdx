...
## Problemas habituales con la conversión de datos {#common-issues-with-data-conversion}
...
## Notas sobre las funciones `toString` {#to-string-functions}
...
## Notas sobre las funciones `toDate`/`toDateTime` {#to-date-and-date-time-functions}
...
Introducida en: v26.9.0

Comprueba un valor con un tipo declarado de Kusto Query Language y lo convierte al tipo equivalente
de ClickHouse: un argumento cuyo tipo no corresponde al tipo KQL declarado se rechaza durante el
análisis, y un valor que no cabe (por ejemplo, por desbordamiento de `int`) genera un error por fila.
El segundo argumento es el nombre del tipo KQL declarado y el tercero describe el parámetro para el
mensaje de error; ambos deben ser cadenas constantes.

Esta función respalda los parámetros de función con tipo cuando `dialect = 'kusto'`. No está pensada
para llamarse directamente desde SQL.
...
* `value` — El argumento proporcionado en una llamada a una función KQL. - `type` — El nombre de tipo KQL declarado, una cadena constante. - `parameter` — Una descripción del parámetro, una cadena constante utilizada en el mensaje de error.

**Valor devuelto**

`value` como portador de ClickHouse del tipo declarado.
...
Convierte un valor en un intervalo de tiempo (un `Interval` en nanosegundos), como hace `totimespan()` de Kusto Query Language: un intervalo de tiempo se mantiene sin cambios, un número representa días y una cadena se interpreta como
`[-][d.]hh:mm:ss[.fffffff]`; se convierte en `NULL` si no se puede analizar.

Esta función implementa `totimespan()` cuando `dialect = 'kusto'`. No está pensada para invocarse
directamente desde SQL.
...
* `x` — Un timespan, un número de días o una cadena que especifica un timespan.

**Valor devuelto**

El valor como un intervalo de nanosegundos; `NULL` si la cadena no especifica un timespan.
...
Devuelve `time_string` convertido a [`DateTime64`](/es/reference/data-types/datetime64), o `NULL` si no se puede analizar la entrada. [`DateTime64`](/es/reference/data-types/datetime64) o [`NULL`](/es/reference/syntax#null)
...
* `time_string` — Cadena que contiene una fecha o una fecha con hora para convertir. [`String`](/es/reference/data-types/string)
* `precision` — Opcional. Precisión requerida. `3` para milisegundos, `6` para microsegundos. Valor predeterminado: `3`. [`UInt8`](/es/reference/data-types/int-uint)
* `time_zone` — Opcional. Zona horaria. La función analiza `time_string` según la zona horaria. [`String`](/es/reference/data-types/string)
...
* Una cadena que contiene una Unix timestamp de 9..10 dígitos.
* Una cadena con una fecha y un componente de hora: `YYYYMMDDhhmmss`, `DD/MM/YYYY hh:mm:ss`, `DD-MM-YY hh:mm`, `YYYY-MM-DD hh:mm:ss`, etc.
* Una cadena con una fecha, pero sin componente de hora: `YYYY`, `YYYYMM`, `YYYY*MM`, `DD/MM/YYYY`, `DD-MM-YY` etc.
* Una cadena con un día y una hora: `DD`, `DD hh`, `DD hh:mm`. En este caso, `MM` se sustituye por `01`.
...
Convierte el argumento al tipo de dato [Date32](/es/reference/data-types/date32).
Si el valor está fuera del rango `[0000-01-01, 9999-12-31]`, `toDate32` devuelve los valores límite admitidos por [Date32](/es/reference/data-types/date32).
...
Convierte el argumento al tipo de dato [Date32](/es/reference/data-types/date32). Si el argumento no se puede convertir, lo que incluye un valor numérico fuera del rango de [Date32](/es/reference/data-types/date32), la función devuelve un valor predeterminado, que es el segundo argumento (si se especifica) o, de lo contrario, `1900-01-01`, el valor predeterminado histórico de [Date32](/es/reference/data-types/date32) (no su límite inferior, que es `0000-01-01`). Si el argumento es de tipo [Date](/es/reference/data-types/date), se tienen en cuenta sus límites.

Tenga en cuenta que, a diferencia de [toDate32](#toDate32), esta función no satura un argumento numérico fuera de rango a los límites de [Date32](/es/reference/data-types/date32); dicho argumento se considera no convertible.
...
Convierte un valor de entrada en un valor de tipo [Date32](/es/reference/data-types/date32), pero devuelve el valor predeterminado `1900-01-01` si se recibe un argumento no válido.
Es igual que [toDate32](#toDate32), pero devuelve `1900-01-01` si se recibe un argumento no válido.
Tenga en cuenta que `1900-01-01` es un valor predeterminado histórico para entradas no válidas, no el límite inferior de [Date32](/es/reference/data-types/date32), que es `0000-01-01`.
...
Devuelve un valor de Date32 si la operación se realiza correctamente; en caso contrario, el valor predeterminado histórico para una entrada no válida (`1900-01-01`). [`Date32`](/es/reference/data-types/date32)
...
Si `expr` es un valor [Time](/es/reference/data-types/time) o [Time64](/es/reference/data-types/time64), se interpreta como la cantidad de segundos transcurridos desde el inicio de la época Unix: la parte fraccionaria de un `Time64` se redondea hacia menos infinito, y los valores fuera del rango de `DateTime` siguen la configuración `date_time_overflow_behavior`.
...
* `expr` — El valor. [`String`](/es/reference/data-types/string) o [`Int`](/es/reference/data-types/int-uint) o [`Date`](/es/reference/data-types/date) o [`Date32`](/es/reference/data-types/date32) o [`Time`](/es/reference/data-types/time) o [`Time64`](/es/reference/data-types/time64) o [`DateTime`](/es/reference/data-types/datetime) o [`DateTime64`](/es/reference/data-types/datetime64)
...
Admite conversiones desde `String`, `FixedString`, `Date`, `Date32`, `Time`, `Time64`, `DateTime`, `DateTime64` o tipos numéricos (`(U)Int8`, `(U)Int16`, `(U)Int32`, `(U)Int64`, `(U)Int128`, `(U)Int256`, `BFloat16`, `Float32`, `Float64`). Los valores `Decimal` no son compatibles y generan una excepción.
`DateTime32` es un alias de `DateTime` y admite fechas desde `1970-01-01 00:00:00` hasta `2106-02-07 06:28:15`.
Al convertir desde una cadena o desde uno de los tipos numéricos saturables enumerados anteriormente, los valores fuera de este intervalo se saturan en sus límites. Los valores de coma flotante no finitos (`NaN`, `inf`, `-inf`) no se pueden convertir y generan una excepción.
Las conversiones desde otros tipos de fecha y hora, como `Date32`, `DateTime64`, `Time` o `Time64`, siguen la configuración `date_time_overflow_behavior`: el modo predeterminado `ignore` puede producir valores con desbordamiento, `saturate` los limita a los límites del intervalo y `throw` genera una excepción. Los valores `Time64` se redondean primero hacia menos infinito a segundos enteros.
...
* `x` — Valor de entrada que se convertirá. [`String`](/es/reference/data-types/string), [`FixedString`](/es/reference/data-types/fixedstring), [`(U)Int*`](/es/reference/data-types/int-uint), [`Float*`](/es/reference/data-types/float), [`BFloat16`](/es/reference/data-types/float), [`Date`](/es/reference/data-types/date), [`Date32`](/es/reference/data-types/date32), [`Time`](/es/reference/data-types/time), [`Time64`](/es/reference/data-types/time64), [`DateTime`](/es/reference/data-types/datetime) o [`DateTime64`](/es/reference/data-types/datetime64)
* `timezone` — Opcional. Zona horaria del valor `DateTime` devuelto. [`String`](/es/reference/data-types/string)
...
**A partir de una marca temporal Unix**
...
* `scale` — Tamaño del tick (precisión): 10^(-scale) segundos. [`UInt8`](/es/reference/data-types/int-uint)
...
* `timezone` — Opcional. Zona horaria. [`String`](/es/reference/data-types/string)
* `default` — Opcional. El valor predeterminado que se devuelve si el análisis no se realiza correctamente. [`DateTime64`](/es/reference/data-types/datetime64)
...
Convierte un valor de entrada en un valor del tipo [DateTime64](/es/reference/data-types/datetime64), pero devuelve el valor cero de [DateTime64](/es/reference/data-types/datetime64) (la época Unix, `1970-01-01 00:00:00`) si recibe un argumento no válido.
Es igual que [toDateTime64](#toDateTime64), pero devuelve el valor cero de [DateTime64](/es/reference/data-types/datetime64) si recibe un argumento no válido.
...
Devuelve un valor DateTime64 si la operación se realiza correctamente; de lo contrario, devuelve el valor DateTime64 cero (`1970-01-01 00:00:00`) con la precisión solicitada. [`DateTime64`](/es/reference/data-types/datetime64)
...
* Valores de tipo Float*, incluidos `NaN` e `Inf`.
* Representaciones en cadena de Float*, incluidos `NaN` e `Inf` (sin distinción entre mayúsculas y minúsculas).
...
Devuelve un valor de coma flotante de 64 bits si la conversión se realiza correctamente; en caso contrario, `NULL`. [`Float64`](/es/reference/data-types/float) o [`NULL`](/es/reference/syntax#null)
...
Devuelve un valor de tipo Int8; de lo contrario, `0` si la conversión no se realiza correctamente. [`Int8`](/es/reference/data-types/int-uint)
...
Admite la conversión desde String, FixedString, DateTime, DateTime64, Time64 o tipos numéricos que representan segundos transcurridos desde la medianoche.
Los valores numéricos fuera del rango del tipo (de `-999:59:59` a `999:59:59`, es decir, de `-3599999` a `3599999` segundos) se saturan a los límites del rango, y los valores de coma flotante no finitos (`NaN`, `inf`, `-inf`) no se pueden convertir y generan una excepción.
Los valores `Time64` se redondean hacia el infinito negativo a segundos enteros y se saturan al mismo rango.

<Note title="`toTime` heredada">
  Antes de la v25.5, `toTime` era una función diferente que convertía una fecha con hora a una fecha fija (`1970-01-02`) mientras conservaba el
  componente de hora. Esa función sigue disponible con el nombre [`toTimeWithFixedDate`](/es/reference/functions/regular-functions/date-time-functions#toTimeWithFixedDate).

  Al establecer [`use_legacy_to_time`](/es/reference/settings/session-settings#use_legacy_to_time) en `1`, también se mantiene el nombre `toTime` vinculado a la función
  heredada; es decir, las llamadas a `toTime` se resuelven como `toTimeWithFixedDate` en lugar de la función de conversión descrita aquí.
  Mientras la configuración está habilitada, use `CAST(x AS Time)` o `x::Time` para convertir al tipo `Time`.
  La configuración tiene el valor predeterminado `0` desde la v26.7, pero tenía el valor predeterminado `1` de la v25.6 a la v26.6, por lo que los ejemplos siguientes la establecen explícitamente.
</Note>
...
* `x` — Valor de entrada que se debe convertir. [`String`](/es/reference/data-types/string) o [`FixedString`](/es/reference/data-types/fixedstring) o [`DateTime`](/es/reference/data-types/datetime) o [`DateTime64`](/es/reference/data-types/datetime64) o [`Time64`](/es/reference/data-types/time64) o [`(U)Int*`](/es/reference/data-types/int-uint) o [`BFloat16`](/es/reference/data-types/float) o [`Float*`](/es/reference/data-types/float)
...
Convierte un valor de entrada en un valor de tipo `Time64`, pero devuelve el valor `Time64` cero (`00:00:00`) si recibe un argumento no válido.
Al igual que [`toTime64`](#toTime64), pero devuelve el valor `Time64` cero si recibe un argumento no válido.
...
Devuelve un valor Time64 si la operación se realiza correctamente; de lo contrario, el valor Time64 cero (`00:00:00`) con la precisión solicitada. [`Time64`](/es/reference/data-types/time64)
...
Al igual que [`toTime`](#toTime), pero devuelve `NULL` en lugar de lanzar una excepción si se producen errores de conversión.
...
* Representaciones en cadena de valores Float*, incluidos `NaN` e `Inf`.
* Representaciones en cadena de valores binarios y hexadecimales; por ejemplo, `SELECT toUInt16OrNull('0xc0fe');`.
