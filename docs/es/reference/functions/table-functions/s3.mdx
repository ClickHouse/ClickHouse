...
| `storage_class_name`                  | El parámetro es opcional. Valores admitidos: `STANDARD`, `REDUCED_REDUNDANCY`, `STANDARD_IA`, `ONEZONE_IA`, `INTELLIGENT_TIERING`, `GLACIER_IR`, `EXPRESS_ONEZONE`. Solo se admiten clases de almacenamiento de S3 que permitan la recuperación inmediata (las clases de archivado como `GLACIER` y `DEEP_ARCHIVE` no son compatibles). Permite especificar [AWS S3 Intelligent Tiering](https://aws.amazon.com/s3/storage-classes/intelligent-tiering/). El valor predeterminado es `STANDARD`.                                                                   |
