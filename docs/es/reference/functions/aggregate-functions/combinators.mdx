...
* `stop` — Valor final de todo el intervalo requerido para los valores de `resampling_key`. El intervalo completo no incluye el valor `stop`: `[start, stop)`. Debe ser mayor que `start`: un rango que no abarque ningún subintervalo lanza una excepción.
