...
| Marco `GROUPS`                                                                          | ✅            | Los límites del marco cuentan grupos completos de pares (filas iguales según la clave `ORDER BY`); `N PRECEDING` y `N FOLLOWING` cuentan `N` grupos de pares antes y después del grupo de pares de la fila actual.                                                                                                                                                                                              |
...
* `ROWS`, `RANGE` o `GROUPS` - define los límites del marco; aggregate&#95;function se calcula dentro de ese marco. `ROWS` cuenta filas físicas, `RANGE` cuenta valores de `ORDER BY` y `GROUPS` cuenta grupos de pares (filas iguales en la clave `ORDER BY`).
...
Un marco `GROUPS` cuenta *grupos de pares* completos —conjuntos de filas iguales según la clave `ORDER BY`—, en lugar de filas físicas (`ROWS`) o valores de `ORDER BY` (`RANGE`). `N PRECEDING` y `N FOLLOWING` cuentan `N` grupos de pares antes y después del grupo de pares de la fila actual, y cada grupo de pares incluido siempre aporta todas sus filas.

La siguiente consulta aplica los mismos límites `1 PRECEDING AND 1 FOLLOWING` a marcos `ROWS`, `RANGE` y `GROUPS`. La columna `order` contiene valores duplicados y no consecutivos, por lo que los tres modos abarcan filas distintas:
...
Cada modo interpreta los límites de forma diferente:

* `ROWS` cuenta filas físicas, por lo que el marco incluye como máximo tres filas adyacentes: la fila actual y una a cada lado.
* `RANGE` cuenta valores de `order`, por lo que `1 PRECEDING` y `1 FOLLOWING` abarcan las filas cuyo `order` difiere en como máximo 1 del de la fila actual. Con huecos de 10, ninguna fila vecina cumple esta condición, por lo que el marco contiene solo las filas que comparten el `order` actual.
* `GROUPS` cuenta grupos de pares, por lo que `1 PRECEDING` y `1 FOLLOWING` siempre incluyen por completo los grupos adyacentes, independientemente de los huecos entre los valores de `order`.
