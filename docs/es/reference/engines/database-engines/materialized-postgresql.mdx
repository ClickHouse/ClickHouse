...
* En una copia de seguridad de `database`, la definición almacenada de cada `table` ya es el `ReplacingMergeTree` anidado sintético (no el `engine` `MaterializedPostgreSQL`), por lo que cada `table` puede restaurarse directamente en una `table` nueva que aún no exista:
...
* En el caso de una copia de seguridad independiente de una `table` `MaterializedPostgreSQL`, la definición almacenada es el propio `engine` `MaterializedPostgreSQL`. Cree previamente una `table` `ReplacingMergeTree` con la misma estructura que la `nested table` (incluidas las `columns` `_sign` y `_version`) y restáurela en ella:
