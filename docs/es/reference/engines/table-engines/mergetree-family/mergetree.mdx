...
* [`gcs` para GCS](/es/reference/functions/table-functions/gcs#creating-a-disk)
...
Actualmente, solo las estadísticas `MinMax` y `Basic` admiten la poda de partes. Cuando se definen dichas estadísticas en una columna, ClickHouse registra los valores mínimo y máximo de esa columna en cada parte.
...
* `Basic`
  ...
  * para columnas `Nullable` y `LowCardinality(Nullable)`: el recuento de valores `NULL`, que el optimizador usa para descontar las filas `NULL` de las estimaciones de selectividad.
    ...
    Una única estadística `Basic` puede rellenar varias de estas a la vez; por ejemplo, en una columna `Nullable(UInt32)` lleva un seguimiento tanto del mínimo/máximo numérico como del recuento de nulos. En comparación con `MinMax`, `Basic` además funciona en columnas `String` / `FixedString` y puede declararse en envolturas `Nullable` de tipos como `UUID` o `IPv6` únicamente para llevar un seguimiento del recuento de nulos.

    Sintaxis: `basic`

* `MinMax`

  El valor mínimo y máximo de la columna, lo que permite estimar la selectividad de los filtros por rango en columnas numéricas.
...
* `Uniq`
  ...
  Resúmenes [HyperLogLog](https://en.wikipedia.org/wiki/HyperLogLog) que permiten estimar cuántos valores distintos contiene una columna.

  Sintaxis: `uniq`

* `CountMin`

  Resúmenes [CountMin](https://en.wikipedia.org/wiki/Count%E2%80%93min_sketch) que proporcionan un recuento aproximado de la frecuencia de cada valor de una columna.
