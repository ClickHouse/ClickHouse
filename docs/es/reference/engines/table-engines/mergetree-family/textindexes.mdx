...
* `splitByRegexp(re)` divide cadenas usando un separador de expresión regular `re` definido por el usuario (consulte la función [splitByRegexp](/es/reference/functions/regular-functions/splitting-merging-functions#splitByRegexp)).
  El argumento `re` es obligatorio; por ejemplo, `tokenizer = splitByRegexp('[^\p{L}\p{N}#+]+')`.
  A diferencia de las cadenas separadoras fijas de `splitByString`, un separador de expresión regular puede conservar tokens que contienen caracteres especiales (como `C++` o `C#`) que los demás tokenizadores dividirían.
...
Además, `LIKE` admite la evaluación mediante un dictionary scan (habilitada mediante `use_text_index_like_evaluation_by_dictionary_scan`) para los tokenizadores `splitByNonAlpha` y `array` sin preprocesador ni posprocesamiento.
Un patrón `%value%` es entonces una *lectura directa (sin indicación)*, mientras que los patrones `value%` y `%value` siguen siendo una indicación; véase [consultas LIKE/ILIKE](#like-ilike-queries-perf).
...
## Uso de un índice de texto {#using-a-text-index}
...
### Funciones compatibles {#functions-support}
...
#### `hasAny` y `hasAll` {#functions-example-hasany-hasall}
...
### Indexación de columnas Array(String) {#text-index-example-array}
...
### Indexación de columnas de tipo Map {#text-index-example-map}
...
### Indexación de columnas JSON {#text-index-example-json}
...
#### Índices sobre subcolumnas específicas {#json-indexes-on-subcolumns}
...
#### Índices basados en rutas con JSONAllPaths {#json-indexes-jsonallpaths}
...
#### Índices basados en valores con JSONAllValues {#json-indexes-jsonallvalues}
...
##### Creación del índice {#json-all-values-creating-the-index}
...
##### Patrones de consulta admitidos {#json-all-values-supported-query-patterns}
...
### Búsqueda por frases {#text-index-phrase-search}
...
#### Ejemplo {#text-index-phrase-search-example}
...
### Consultas LIKE/ILIKE {#like-ilike-queries-perf}
...
* `%value%` coincide con una fila si y solo si coincide con alguno de los tokens de esa fila, por lo que el índice resuelve la consulta por sí solo: una [lectura directa (sin hint)](#direct-read) que elimina el predicate original.
* `value%` y `%value` anclan el needle al valor completo, mientras que el escaneo del Diccionario solo puede anclarlo a un token, de modo que el scan devuelve un superset de las filas coincidentes y se utiliza como una [lectura directa a modo de hint](#direct-read).
...
### Consultas de recuento simples {#count-queries-perf}
...
#### Configuración de la caché de tokens {#caching-tokens}
...
#### Configuración de la caché de encabezados {#caching-header}
...
#### Configuración de la caché de listas de postings {#caching-posting-lists}
...
## Índices de texto frente a índices basados en filtros Bloom {#text-index-vs-bloom-filter-indexes}
...
## Ejemplo: conjunto de datos de Hacker News {#hacker-news-dataset}
...
### 1. Uso de `hasToken` {#using-hasToken}
...
### 2. Uso de `hasAnyTokens` {#using-hasAnyTokens}
...
### 3. Uso de `hasAllTokens` {#using-hasAllTokens}
...
### 4. Búsqueda compuesta: OR, AND, NOT, ... {#compound-search}
