...
#### Búsquedas combinadas de key-value con el tokenizador `keyValuePairs` {#tokenizer-key-value-pairs}

Utilice el tokenizador `keyValuePairs` sobre la propia columna `Map` cuando busque una key concreta que contenga un valor concreto:
...
#### Búsqueda de claves y valores con `mapKeys` y `mapValues` {#text-index-map-keys-and-values}
