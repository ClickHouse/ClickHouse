...
³ `hasPhrase` en un índice de texto `splitByRegexp` **no** admite un posprocesamiento: la combinación se rechaza con una excepción, porque la reescritura a nivel de fila del posprocesamiento presupone tokens de estilo `splitByNonAlpha` divididos por espacios en blanco. Sin un posprocesamiento, `splitByRegexp` es totalmente compatible con `hasPhrase`.
