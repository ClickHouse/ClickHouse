...
`color` acepta cualquier token de las listas categóricas y semánticas anteriores (por ejemplo, `chart-blue` o `chart-success`). De forma opcional, `backgroundChart.color` sobrescribe el color de la minigráfica con el mismo token de la enumeración. Consulta la [referencia de la API de ClickStack](/es/use-cases/observability/clickstack/api-reference) para obtener información sobre la autenticación y las URL base.
