...
Para crear una visualización basada en SQL, abra el editor de tile de un dashboard y seleccione la pestaña **SQL**.
...
3. Escriba su consulta SQL en el editor, usando parámetros de consulta y macros para integrarla con el intervalo de tiempo, los filtros y las variables del dashboard.
...
`$__filters` solo aplica los filtros que tienen habilitada la **condición de filtro de difusión**. Los filtros expuestos como variables se referencian explícitamente, como se describe a continuación.

<div id="dashboard-variables">
  ### Macros de variables de dashboard
</div>

Cuando un filtro de dashboard está [disponible como variable](/es/clickstack/features/dashboards/overview#dashboard-variables), se puede hacer referencia a los valores seleccionados en cualquier parte de la consulta. Estas macros solo están disponibles en los tiles que pertenecen a un dashboard que declara al menos una variable.

| Macro                                         | Descripción                                                                                                                             |
| --------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| `$__filter($<variable>)`                      | Se expande a `toString(<filter expression>) IN ($variable)` cuando se seleccionan valores para `variable`, y a `1=1` en caso contrario. |
| `$__filter(<expression>, $<variable>)`        | Se expande a `<expression> IN ($variable)` cuando se seleccionan valores para `variable`, y a `1=1` en caso contrario.                  |
| `$__conditionalAll(<condition>, $<variable>)` | Se expande a `<condition>` cuando la variable tiene una selección, y a `1=1` en caso contrario.                                         |

Los valores de una variable también se pueden interpolar directamente como `$name` o `${name}`, con un formato opcional: `${name:sqlstring}` (el predeterminado), `${name:csv}` o `${name:regex}`. En el formato predeterminado, una referencia directa se muestra como `NULL` antes de seleccionar cualquier valor, por lo que se recomienda usar las macros anteriores donde se espere un predicado.
...
<div id="example-line-chart-variables">
  ### Gráfico de líneas — uso de variables del dashboard
</div>

Dado un dashboard con una variable `service` para `ServiceName` y una variable `severity` para `SeverityText`, esta consulta limita el gráfico a la selección actual del servicio y excluye las severidades seleccionadas. Ambas macros se expanden a `1=1` cuando su variable no tiene ninguna selección, por lo que el gráfico se muestra sin filtrar hasta que el Viewer selecciona un valor.
...
* Las variables del dashboard no necesitan un source; los valores proceden de los menús desplegables de filtros del dashboard, no del source del propio tile.
* Las alertas en un tile SQL que hace referencia a una variable del dashboard se evalúan con todas las variables en su estado vacío, no con los valores seleccionados en el dashboard.