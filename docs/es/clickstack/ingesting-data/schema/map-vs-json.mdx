...
El [esquema predeterminado](/es/clickstack/ingesting-data/schemas) de ClickStack almacena los atributos de recurso, scope, log y span en columnas `Map(LowCardinality(String), String)`. ClickHouse también admite un [tipo fuertemente tipado `JSON`](/es/reference/formats/JSON/JSON), y ClickStack ofrece compatibilidad beta para usarlo en lugar de `Map`.
...
<Steps>
  <Step title="Detén el OTel collector" id="stop-the-collector" />

  <Step title="Cambia el nombre de las tablas existentes y actualiza las fuentes" id="rename-existing-tables-sources">
  ...
  <Step title="Despliega el collector" id="deploy-the-collector">
    Despliega el collector con `OTEL_AGENT_FEATURE_GATE_ARG` configurado.
  </Step>

  <Step title="Reinicia el contenedor de HyperDX con compatibilidad para esquemas JSON" id="restart-the-hyperdx-container">
    ```shell
    export BETA_CH_OTEL_JSON_SCHEMA_ENABLED=true
    ```
  </Step>

  <Step title="Crea nuevas fuentes de datos" id="create-new-data-sources">
