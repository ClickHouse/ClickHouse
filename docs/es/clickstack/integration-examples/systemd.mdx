...
<Steps>
  ...
  <Step title="Verifica que el journal de systemd esté en ejecución" id="verify-systemd">
  ...
  <Step title="Crea la configuración de OpenTelemetry Collector" id="create-otel-config">
  ...
  <Step title="Implementa con Docker Compose" id="deploy-docker-compose">
    <Note>
  ...
  <Step title="Verifica los logs en HyperDX" id="verifying-logs">
    ...
    <Image img="/images/clickstack/systemd/systemd-search-view.webp" alt="Vista Búsqueda de logs" />
...
<Steps>
  ...
  <Step title="Crear la configuración del collector de demostración" id="demo-config">
  ...
  <Step title="Ejecutar ClickStack con datos de demostración" id="run-demo">
  ...
  <Step title="Verificar los logs en HyperDX" id="verify-demo-logs">
    ...
    2. Ve a la Búsqueda view y selecciona `Logs` como source
...
<Steps>
  <Step title="Descargar la configuración del dashboard" id="download">
    <TrackedLink href={'https://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/examples/systemd-logs-dashboard.json'} download="systemd-logs-dashboard.json" eventName="docs.systemd_logs_monitoring.dashboard_download">Descargar el archivo de configuración del dashboard</TrackedLink>.
  </Step>
...
Consulta [Ingesta con OpenTelemetry](/es/clickstack/ingesting-data/opentelemetry) para conocer los patrones de despliegue en producción.