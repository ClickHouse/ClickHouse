...
Supongamos que intentas usar las funciones de tabla de [MySQL](/es/reference/functions/table-functions/mysql) o [PostgreSQL](/es/reference/functions/table-functions/postgresql) en ClickHouse Cloud y conectarte a tu base de datos alojada en una VPC de Amazon Web Services (AWS). AWS PrivateLink no se puede usar para habilitar esta conexión de forma segura. PrivateLink es una conexión unidireccional. Permite que tu red interna o tu VPC de Amazon se conecten de forma segura a ClickHouse Cloud, pero no permite que ClickHouse Cloud se conecte a tu red interna.
...
Para ello, configura tus grupos de seguridad de AWS para permitir conexiones desde ClickHouse Cloud a tu servicio de base de datos interno o privado. Consulta las [direcciones IP de salida predeterminadas de las regiones de ClickHouse Cloud](/es/products/cloud/guides/data-sources/cloud-endpoints-api), junto con las [direcciones IP estáticas disponibles](https://api.clickhouse.cloud/static-ips.json).