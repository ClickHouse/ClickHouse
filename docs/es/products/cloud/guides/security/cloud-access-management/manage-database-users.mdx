...
<Note>
  **Administrar roles de base de datos desde la consola (beta)**

  También puedes administrar roles de base de datos directamente desde la consola de ClickHouse Cloud añadiendo permisos de datos a roles personalizados. Esta función está en beta y está disponible para servicios que ejecutan ClickHouse versión 26.4 y posteriores. Los roles asignados manualmente mediante la convención de nomenclatura `sql-console-role:<email>` tienen prioridad sobre los roles asignados a través de la consola. Para obtener más información, consulta [Manage database roles](/es/products/cloud/guides/security/cloud-access-management/manage-custom-roles#manage-database-roles).
</Note>
