...
<EnterprisePlanFeatureBadge feature="Autenticación con JWT mediante un proveedor de identidad personalizado" support="true" />

<Tip>
  Esta guía explica cómo configurar proveedores de JWKS en la consola de Cloud. Para saber cómo generar un JWT y cuál es su estructura —incluidos los claims obligatorios, los claims de roles y grants, y el funcionamiento de los usuarios efímeros—, consulte la referencia sobre [autenticación con JWT](/es/operations/external-authenticators/jwt).
</Tip>
...
Una vez habilitada la funcionalidad para su organización, puede configurar estos proveedores de JWKS para cada servicio desde **Configuración → Seguridad** en la consola de ClickHouse Cloud.

<Note>
  Para habilitar proveedores de JWT para su organización, póngase en contacto con [soporte](https://clickhouse.com/support/program).
</Note>
...
* Una URL de JWKS **HTTPS** accesible públicamente que publique al menos una clave **RSA** (los tokens deben estar firmados con **RS256**).

<Note>
  Los proveedores basados en JWKS aceptan **únicamente claves RSA**. Los tokens firmados con claves HMAC (`HS256`) o EC (`ES256`) no se pueden verificar mediante un endpoint de JWKS y se rechazan. Un documento JWKS puede contener otros tipos de claves, pero debe incluir al menos una clave RSA utilizable.
</Note>
...
El Client (su proveedor de identidad o aplicación) genera un JWT y lo firma con su **clave privada**. El token debe seguir el [formato de token](/es/operations/external-authenticators/jwt#token-claims) previsto. A continuación, ClickHouse lo verifica con las claves **públicas** publicadas en su URL de JWKS:

1. ClickHouse lee el encabezado `kid` (ID de clave) del token y selecciona la clave correspondiente de su documento JWKS.
2. Verifica la firma del token con esa clave pública RSA y comprueba las claims `iss` (emisor) y `aud` (audiencia) con la configuración de su proveedor.
3. Si la verificación se realiza correctamente, la conexión se establece como un usuario efímero cuyos derechos de acceso proceden de las claims `clickhouse:grants` y `clickhouse:roles` del token, limitados por el límite de permisos (el usuario `default`). Consulte [Derechos de acceso](/es/operations/external-authenticators/jwt#access-rights) para obtener más información.
...
<Steps>
  ...
  <Step title="Complete los datos del proveedor" id="fill-provider-details">
    ...
    | **URL de JWKS**            | La URL HTTPS pública que publica las claves públicas RSA utilizadas para verificar las firmas de los tokens.                                                                                                           |
...
* Los proveedores de JWKS solo aceptan claves **RSA / RS256**.
...
* [Autenticación con JWT](/es/operations/external-authenticators/jwt) — claims de token, usuarios efímeros y uso del Client.