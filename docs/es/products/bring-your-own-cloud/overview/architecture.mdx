...
Sus datos de ClickHouse, las copias de seguridad, los logs y los datos de monitorización permanecen en su cuenta en la nube; los únicos datos que se exportan a otro lugar son los datos acotados de telemetría de uso y estado que se enumeran en [límites de red](/es/products/bring-your-own-cloud/reference/network-security#outbound-connections). Las partes de datos y las copias de seguridad se almacenan en su almacenamiento de objetos (por ejemplo, Amazon S3), mientras que los logs se almacenan en los volúmenes de almacenamiento conectados a sus nodos de ClickHouse. En una futura actualización, los logs se escribirán en LogHouse, un servicio de logging basado en ClickHouse que también se ejecuta dentro de su VPC de BYOC. Las métricas pueden almacenarse localmente o, para su conservación a largo plazo, en un bucket dedicado dentro de su propia cuenta: el almacenamiento de objetos se sitúa fuera de la VPC/VNet y se accede a él a través de la ruta de la API de almacenamiento del proveedor. La conectividad del plano de control entre la VPC de ClickHouse y su VPC de BYOC se proporciona a través de un canal seguro y de alcance muy restringido (por ejemplo, mediante Tailscale, como se muestra en el diagrama); se utiliza únicamente para operaciones de gestión, no para el tráfico de consultas.

<div id="control-plane-communication">
  ### Comunicación con el plano de control {#control-plane-communication}
</div>
...
<div id="key-requirements">
  ## Requisitos clave para BYOC {#key-requirements}
</div>
...
<div id="tailscale-private-network">
  ### Conexión de red privada de Tailscale {#tailscale-private-network}
</div>
