...
<Note>
  Los flujos de trabajo en managed mode que se describen aquí aún no están disponibles en la versión pública del connector. Confirme con su account team el build y el soporte de despliegue necesarios antes de instalar; consulte [disponibilidad de managed mode](/es/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability).
</Note>
...
Con un [build que admite el modo administrado](/es/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability), `init` pregunta si quieres habilitarlo en un terminal y preselecciona que sí; un `init` ejecutado desde un script solo lo habilita con `--managed`. También puedes ejecutarlo sin ese modo y conservar la observabilidad y el soporte para los clusters que operas por tu cuenta; consulta [ejecutar sin el modo administrado](#run-without-managed-mode).
...
El flujo de inscripción y creación de servicios que se describe a continuación requiere [soporte de modo administrado](/es/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) confirmado por tu account team; no está disponible en la versión pública `v0.16.0`.
...
`init` habilita el modo administrado cuando pasas `--managed`; en un terminal te lo pregunta, y el valor predeterminado es sí. Al habilitarlo se despliega el executor y se aplica su concesión de acceso a Kubernetes. Esa concesión es un ServiceAccount `pcm-executor`. Una política de admission limita sus escrituras a los espacios de nombres de servicios (con el prefijo `ns-` de forma predeterminada), salvo la renewal de su propio token, un ConfigMap del registry y, en Kubernetes, el certificado de client renovado en su espacio de nombres de origen.

Crear un servicio requiere dos comandos. `clicklink clctl executor prepare` crea los buckets y el IAM role del servicio con tus propias credenciales de AWS y muestra una sola vez la password del usuario `default`. `clicklink clctl instances create` envía la definición del servicio a ClickHouse Cloud a través de tu endpoint del conector. A partir de ahí, ClickHouse Cloud gestiona el servicio mediante el canal de comandos saliente del executor: scaling, detención e inicio, reinicios, copias de seguridad, upgrades de versión y cambios de configuración. Consulta [servicios gestionados](/es/products/bring-your-own-cloud/connector/managed-services) para ver el recorrido completo.
...
La elección en tiempo de instalación que se describe a continuación requiere un [build con soporte para el modo administrado](/es/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability). La versión pública `v0.16.0` no incluye el indicador `--no-managed` ni el prompt del modo administrado.
...
* **Registro por parte de ClickHouse.** ClickHouse registra tu entorno para el modo administrado y te proporciona el endpoint del conector y un token de inscripción de un solo uso durante el onboarding.
* **Acceso de red saliente por el puerto 443** a tu endpoint del conector y a su endpoint de inscripción, además de `releases.clicklink.clickhouse.com` y ECR Public en el momento de la instalación. Existen alternativas aisladas y con réplicas para cada paso; consulta [onboarding](/es/products/bring-your-own-cloud/connector/onboarding). En el modo administrado, los nodos de tu cluster también descargan imágenes de ClickHouse desde el registro en el que está registrado tu entorno, y el executor accede a Amazon ECR y STS cuando descarga un chart de plataforma desde ECR.
...
* **Credenciales de AWS para `prepare` y `teardown`, y acceso al registro para `platform approve`.** La creación de los buckets y el IAM role de un servicio, así como su eliminación una vez borrado el servicio, se ejecutan con tus credenciales en tu workstation o en el host del conector. Para el acceso directo al registro de ClickHouse, la aprobación y la sincronización de plataforma utilizan el instance profile de la VM EC2 del conector para asumir el rol de descarga de solo lectura; los perfiles de AWS de la workstation o las credenciales de SSO no sustituyen a esa identidad. Otros registros de charts en ECR usan las credenciales de AWS del entorno. Consulta [credenciales del registro](/es/products/bring-your-own-cloud/connector/platform-updates#registry-credentials). El executor nunca posee credenciales de tus buckets ni de IAM. Sus únicas llamadas a la nube van a Amazon ECR y STS para descargar charts y comprobar imágenes de plataforma; no realiza ninguna llamada a S3 ni a IAM.
