...
Cada demonio expone un endpoint `/livez` en su puerto de estado de salud. El campo JSON `status` del cuerpo de la respuesta indica el estado de salud, no el código de estado HTTP; por tanto, compruebe el cuerpo en lugar del `200`. Las métricas de Prometheus se exponen en `/metrics` en el mismo puerto; no hay un puerto de métricas separado. Puertos predeterminados en ambos destinos:
...
El comportamiento en modo administrado que se describe aquí requiere un [build compatible](/es/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability); la versión pública `v0.16.0` no incluye ni `--managed` ni `--no-managed`.

Solicite un token de inscripción nuevo a su account team de ClickHouse y vuelva a ejecutar el comando `init` original con `--enroll` y `--force`. Conserve todos los indicadores específicos del destino de la primera instalación (`--target-namespace`, `--values` y cualquier indicador de mirror como `--chart`, `--chart-repo` o `--chart-version`), ya que `--force` vuelve a preparar la configuración conservada. El modo Managed se hereda de la configuración que se está reemplazando, por lo que `--managed` es opcional aquí. En una instalación predeterminada:
...
En Kubernetes, `init` crea de antemano los secretos `clicklink-hmac` y `clicklink-mtls` rotados; el chart no los genera. Por lo tanto, la anotación de suma de comprobación del chart no los incluye, y los pods en ejecución conservan las credenciales con las que se iniciaron. Reinicie todos los componentes después de la rotación:
...
En una VM, ejecútelos en el host canalizando la password del usuario admin; en una VM la password es obligatoria a menos que pase `--ch-user-via cr`:
...
<Note>
  La CLI pública `v0.16.0` no tiene ni `--managed` ni `--no-managed`. El comportamiento de conservación y sobrescritura del modo Managed que se describe a continuación requiere un build con [soporte de modo Managed](/es/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability); confirme su build con su account team antes de cambiar ese modo durante la recuperación.
</Note>
...
**Connector sin conexión.** Mientras el connector está caído o inaccesible, los servicios que administra siguen funcionando. El ClickHouse Operator de su cluster los mantiene activos, y el executor no está en el data path. Los cambios de ciclo de vida requieren que el executor esté conectado, por lo que ninguno se aplica hasta que se reconecte. Al reiniciarse, el executor recupera de su base de datos de comandos aquellos que estaban en curso cuando se detuvo. Un comando que ClickHouse Cloud vuelve a entregar se responde con el resultado registrado en lugar de ejecutarse dos veces; solo se vuelve a ejecutar un comando fallido. Para saber qué ocurre con los comandos de ciclo de vida cuando no hay ningún executor conectado, consulte [cuando el connector está sin conexión](/es/products/bring-your-own-cloud/connector/managed-services#when-the-connector-is-offline).
...
El PersistentVolumeClaim del executor está anotado con `helm.sh/resource-policy: keep`, por lo que la desinstalación lo deja intacto junto con la base de datos de comandos y el registry de servicios que contiene. Los secretos creados por `init`, el secreto del paquete de plataforma que escribió la aprobación de plataforma y el ConfigMap del registry de servicios que mantiene el executor tampoco pertenecen al chart y sobreviven a la desinstalación. Elimínalos de forma explícita, incluido el par de secreto de acceso y ServiceAccount de cada instancia que hayas registrado tú mismo y de cada managed service para el que hayas aprovisionado el solucionador de problemas (con el nombre del service). `--ignore-not-found` contempla los objetos que nunca llegaron a crearse, como el secreto del paquete de plataforma antes de una primera aprobación:
...
La CLI pública `v0.16.0` no tiene el comando `platform`, incluido `platform reset`; confirma su [disponibilidad](/es/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) antes de usarlo.

Desinstalar el connector no elimina la capa de plataforma. Los espacios de nombres y los workloads de la plataforma, el RBAC `pcm-platform` y los objetos de admission que instalaron tus aprobaciones permanecen en el cluster, al igual que todos los managed services. Elimina primero los services; consulta [eliminar un service](/es/products/bring-your-own-cloud/connector/managed-services#delete-a-service). En un cluster de pruebas sin services, `clicklink clctl platform reset` desinstala los releases de la plataforma, y las CustomResourceDefinitions, el RBAC y la clase de almacenamiento que quedan deben eliminarse manualmente; consulta [restablecer un cluster de pruebas](/es/products/bring-your-own-cloud/connector/platform-updates#resetting-a-test-cluster). Fuera de un cluster de pruebas, acuerda un plan de eliminación con tu account team de ClickHouse antes de modificar la capa de plataforma.
