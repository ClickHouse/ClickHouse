...
<Note>
  La CLI pública `v0.16.0` no tiene el comando `platform`, lo que incluye `platform approve` y `platform reset`. Ni el flujo de trabajo de VM Linux ni el de Kubernetes que se describen a continuación están disponibles en esa release. La aprobación en Kubernetes requiere además compatibilidad de la CLI y del chart del executor con el paquete de tokens basado en secreto (`--secret-namespace`). Confirme la [disponibilidad del modo gestionado](/es/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) con su account team antes de utilizar cualquiera de los dos flujos de trabajo.
</Note>
...
Ambos procedimientos de aprobación que se describen a continuación requieren un [build con compatibilidad con la plataforma](/es/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability); la versión pública `v0.16.0` no incluye el comando `platform`. La aprobación en Kubernetes también necesita compatibilidad equivalente en la CLI y en el chart para la entrega del secreto.

La authentication del registry depende del modo de distribución de imágenes registrado para su entorno:

* **Acceso directo al registry de ClickHouse.** Ejecute la aprobación en la VM de EC2 del connector. Tanto `approve` como la sincronización de plataforma del executor utilizan credenciales del instance metadata service de EC2 para asumir el rol de extracción de ECR de solo lectura de su entorno. Ese rol se emplea para iniciar sesión en el chart, y el executor también lo usa para comprobar que existen las imágenes de la plataforma. Los perfiles de AWS, las credenciales de entorno o las credenciales de SSO en una workstation no sustituyen al perfil de instancia. Su kubeconfig sigue aportando las credenciales independientes de administración del cluster necesarias para aplicar la mitad correspondiente a los permisos.
  ...
  <Step title="Ejecute approve" id="run-approve">
    ...
    <Tabs>
      ...
      <Tab title="Kubernetes" id="approve-kubernetes">
        En una implementación con registry replicado y la aprobación de plataforma de Kubernetes habilitada, `approve` lee el paquete preparado a través del reenvío de puertos del paso anterior. Entrega el paquete de tokens como un Secret en el espacio de nombres del conector, que el chart monta en el pod de Kubernetes. Ejecútalo desde una estación de trabajo cuyo kubeconfig llegue al cluster, con una copia de la configuración del conector y acceso al registry como se describe más arriba. El chart renderiza la configuración en el ConfigMap del executor; `approve` la necesita para el prefijo del espacio de nombres y no lee ninguna credencial de ella.
...
La CLI pública `v0.16.0` no incluye el comando `platform reset`; confirme la [disponibilidad](/es/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) antes de utilizar este procedimiento.
...
En un cluster vacío, desinstala las releases de la plataforma en orden inverso al de dependencias. A continuación, elimina el paquete de tokens de plataforma del executor: el directorio local en una VM, o el secreto indicado por `--secret-namespace <connector-namespace>` en Kubernetes. Las CustomResourceDefinitions, el RBAC y la StorageClass se mantienen. Ejecute `approve` de nuevo antes de la siguiente sincronización de la plataforma.
