...
<Note>
  La CLI pública `v0.16.0` no incluye ni `--managed` ni `--no-managed`. Antes de usar cualquiera de estos indicadores para cambiar la configuración, confirma la [disponibilidad del modo administrado](/es/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) y obtén el build correspondiente de tu account team.
</Note>

El modo administrado ejecuta el executor, que aplica a tu cluster el ciclo de vida del servicio que gestiona ClickHouse Cloud. `init` lo activa durante la inscripción y completa la identidad del cluster (consulta [habilitar el modo administrado](/es/products/bring-your-own-cloud/connector/managed-services#enable-managed-mode)); las claves que se indican a continuación son las que quizá quieras revisar después. Para cambiar la postura configurada, vuelve a ejecutar `init` con `--force` y `--managed` o `--no-managed`. Como toda ejecución de `init`, esta necesita un entry point: `--handoff` si conservaste el paquete, o `--enroll` con un token nuevo, tal como se describe en [reejecuciones y recovery](/es/products/bring-your-own-cloud/connector/operations#re-runs-and-recovery).

En una VM, `--no-managed` desactiva el executor en la configuración, pero no detiene ni deshabilita una systemd unit que ya esté en ejecución. Deténla explícitamente antes de cambiar la configuración:

```bash
sudo systemctl disable --now clicklink-executor
```

En Kubernetes, establecer `executor.enabled: false` en los valores y aplicar el cambio con `helm upgrade` elimina la Implementación del executor. En ambos destinos, el grant sobre el cluster permanece hasta que lo elimines; sigue [detener el modo administrado](/es/products/bring-your-own-cloud/connector/reference/faq#stop-managed-mode) para eliminar también ese acceso.
...
<Tabs>
  <Tab title="Kubernetes" id="managed-mode-kubernetes">
    ...
    * **`cluster.namespacePrefix`** lo comparten todos los componentes y es el prefijo al que el grant del executor lo restringe. Cambiarlo después de la instalación exige volver a ejecutar el grant.
  ...
  <Tab title="VM Linux" id="managed-mode-vm">
    ...
    * **`namespace_prefix`** es una clave de nivel superior compartida por todos los componentes y es el prefijo al que el grant del executor lo limita. Cambiarlo después de la instalación exige volver a ejecutar el grant.
...
Los servicios que crea el executor se registran por sí mismos, de modo que en modo administrado nunca se añaden aquí. Una implementación independiente del conector que se ejecuta sin el executor observa el ClickHouse que usted mismo ejecuta; pase `--ch-user-suffix` en su `init` cuando ambas implementaciones compartan una instancia. Esta sección se aplica a esa implementación. Cada entrada de `instances` designa un endpoint del protocolo nativo de ClickHouse del que lee el conector: `host`, `port`, `database`, `secure`, `cluster` y, en Kubernetes, `namespace`; `max_open_conns` y `max_idle_conns` limitan el connection pool. Las credenciales nunca residen en la configuración; cada componente resuelve su usuario de ClickHouse de solo lectura a partir del paquete de acceso que genera el aprovisionamiento.

<Tabs>
  <Tab title="Kubernetes" id="instances-kubernetes">
    Añada la instancia al mapa `instances` de nivel superior en `clicklink-values.yaml` (los mismos campos que el registro de la VM) e incluya su espacio de nombres en `networkPolicy.clickhouseNamespaces`, que se compara con la etiqueta `kubernetes.io/metadata.name` del espacio de nombres. El chart representa el mapa en el ConfigMap `clicklink-instance-registry` que leen ambos daemons. Esto solo funciona mientras `executor.enabled` sea `false`; con el executor activado, la representación falla porque el executor es el propietario de ese ConfigMap.
...
## Lista de operadores permitidos {#operator-allowlist}
...
* **`allowEgressCIDRs`** debe proporcionarla usted: los rangos que hay detrás de su endpoint del conector, indicados durante la instalación con `--egress-cidrs` o en el prompt. Todos los componentes los necesitan para alcanzar su endpoint del conector. En modo administrado, cuando los charts de la plataforma residen en Amazon ECR, añada también los rangos de ECR y STS de su region (o el rango de su mirror); sin ellos, las descargas de charts del executor fallan por efecto de la policy.
...
El solucionador de problemas redacciona su salida antes de que salga de su perímetro. Los patrones integrados cubren `ipv4`, `ipv6`, `bearer-token`, `aws-access-key`, `email`, `jwt`, `ssh-private-key` y `connection-string-credentials`. Puede añadir sus propios patrones en un archivo YAML. Sus patrones se aplican primero, en el orden en que aparecen en el archivo, seguidos de los integrados; una entrada que reutilice el `name` de un patrón integrado sustituye dicho patrón.
...
En una VM, el archivo se encuentra en `/etc/clicklink/redaction-patterns.yaml`. El instalador lo inicializa con un conjunto inicial de patrones activos (API keys de terceros, claves secretas de AWS, etc.). Puede recortar o ampliar el conjunto como prefiera; el instalador conserva su versión durante las actualizaciones. En Kubernetes, incluya el YAML en un ConfigMap con la clave `redaction-patterns.yaml` y establezca `troubleshooter.redaction.patternsConfigMap` con su nombre; el chart lo monta en la misma ruta.
...
El ejemplo de modo gestionado que aparece a continuación requiere un build con `--managed`, del que carece la versión pública `v0.16.0`. Confirma la [disponibilidad del modo gestionado](/es/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) antes de usarlo; replicar la release pública no añade esta compatibilidad.
...
<Tabs>
  ...
  <Tab title="Linux VM" id="storage-vm">
    El scraper almacena temporalmente las métricas en `/var/lib/clicklink/buffer` para garantizar una entrega al menos una vez mientras el endpoint del conector no esté accesible. Conserva hasta 168 horas o 1024 MB de datos y los sube a una velocidad limitada a 1 MB/s de forma predeterminada:
