...
El modo administrado ejecuta el executor, que aplica a tu cluster el ciclo de vida del servicio que gestiona ClickHouse Cloud. `init` lo activa durante la inscripción y completa la identidad del cluster (consulta [habilitar el modo administrado](/es/products/bring-your-own-cloud/connector/managed-services#enable-managed-mode)); las claves que se indican a continuación son las que quizá quieras revisar después. Para activar el modo administrado en un conector instalado sin él, o para desactivarlo, vuelve a ejecutar `init` con `--force` y `--managed` o `--no-managed`. `--no-managed` detiene el executor, pero mantiene su grant sobre el cluster; elimínalo tal como se describe en [detener el modo administrado](/es/products/bring-your-own-cloud/connector/reference/faq#stop-managed-mode). Como toda ejecución de `init`, esta necesita un entry point: `--handoff` si conservaste el paquete, o `--enroll` con un token nuevo, tal como se describe en [reejecuciones y recovery](/es/products/bring-your-own-cloud/connector/operations#re-runs-and-recovery).
