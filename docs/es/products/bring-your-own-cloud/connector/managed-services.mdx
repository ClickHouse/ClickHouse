...
En modo administrado, el conector ejecuta un tercer componente: el executor. A través de él, ClickHouse Cloud opera los servicios de ClickHouse en su Kubernetes cluster. Esta página explica cómo crear un servicio, cómo consultar su status, qué hace ClickHouse Cloud una vez que el servicio existe y cómo se retira un servicio.

<Note>
  Habilitar el modo administrado y crear un servicio requiere soporte que aún no está disponible en el release público. La CLI pública `v0.16.0` no incluye los indicadores de init `--managed` y `--no-managed` ni el flujo de trabajo de creación por parte del cliente que se describe a continuación. Confirme la [disponibilidad del modo administrado](/es/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) con su account team antes de registrar o preparar recursos; la creación de servicios también requiere el soporte correspondiente en su endpoint del conector y en ClickHouse Cloud.
</Note>

## Qué hace el modo gestionado {#what-managed-mode-does}

El executor es un demonio incluido en el mismo binary `clicklink` que el scraper y el solucionador de problemas. Mantiene un canal WebSocket saliente hacia el endpoint del conector y recibe comandos de ciclo de vida desde ClickHouse Cloud. Aplica cada comando al único Kubernetes cluster para el que está configurado e informa del resultado por ese mismo canal. Al igual que los demás componentes, solo establece conexiones salientes: ClickHouse Cloud nunca se conecta hacia el interior de tu cluster, y la API local del executor se enlaza al loopback.

El modo gestionado está disponible en Amazon EKS con S3 storage, como parte del private preview. El account team de ClickHouse lo habilita al registrar tu entorno. `init` rechaza un contexto de kube que no apunte a un EKS cluster.

Cada service necesita tres cloud resources de tu propiedad: un bucket de datos, un bucket de copias de seguridad y un IAM role que los ClickHouse pods asumen para acceder a ellos. Los creas con tus propias credentials antes de que exista el service y los eliminas una vez que desaparece. El executor no guarda credentials de tus buckets ni de IAM, y nunca elimina datos. Sus únicas llamadas a la nube van a Amazon ECR y STS. Inicia sesión en el registry cuando una sincronización o una creación de plataforma descarga un chart. Durante una actualización de plataforma comprueba las imágenes con el role de descarga read-only. No realiza llamadas a S3 ni a IAM. Para cada service crea un `Service` de Kubernetes de tipo LoadBalancer, que el load-balancer controller de tu cluster materializa como un NLB interno en tu account.
...
## Habilitar el modo administrado {#enable-managed-mode}

<Note>
  La CLI pública `v0.16.0` no admite `init --managed` ni el prompt de modo administrado que se describe a continuación. Primero obtenga un build y una implementación con [soporte de modo administrado](/es/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) a través de su account team.
</Note>

El modo administrado se elige durante el enroll: pase `--managed` a `clicklink clctl init` o responda al prompt en un terminal. En una VM, `init` toma el cluster del kubeconfig de este host (`--cluster-name` cuando contiene más de un EKS cluster) y concede el acceso a todo el cluster del executor de forma inline. En Kubernetes, pase `--egress-cidrs` con los CIDR de su endpoint del conector para que el chart deje su NetworkPolicy de denegación predeterminada enabled.
...
Este flujo de trabajo no está disponible en la versión pública `v0.16.0`. Antes de preparar los recursos, confirma la [disponibilidad de creación de servicios](/es/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) para tu CLI, tu endpoint del conector y tu despliegue de ClickHouse Cloud.
...
* `prepare` necesita una copia de la configuración del conector (`--config`), un `kubectl port-forward` a la API local del executor y `--output-dir` apuntando a un directorio con permisos de escritura. Verifica el nombre del servicio con el executor y se niega a ejecutarse si no logra alcanzarlo.
* `instances create` lee los archivos de credentials indicados en la configuración desde los Secrets `clicklink-hmac` y `clicklink-mtls` cuando este host no los tiene, y anuncia cada lectura. Añade `--connector-namespace` cuando el espacio de nombres del conector no sea `clicklink`. Solo `instances create` cuenta con este fallback.
...
<Steps>
  <Step title="Prepare el servicio" id="prepare">
    ...
    4. **Resumen.** Genera la password del usuario `default` y escribe el cuerpo de creación en `<output-dir>/_prepare/<name>.create.json` (mode `0600`; solo contiene los hash de la password). Imprime el siguiente comando exacto en una línea `Next:`.

    <Warning>
      `prepare` imprime la password del usuario `default` una sola vez, en stderr, y nunca más. Ni el cuerpo de creación ni `--output json` la contienen, y ClickHouse Cloud solo recibe sus hash. Guárdela antes de continuar. Una nueva ejecución que encuentre el cuerpo de creación conserva los hash ya escritos y así lo indica.
    </Warning>
  ...
  <Step title="Envíe la creación" id="create">
    ...
    Es seguro reintentar con las mismas entradas. La clave de idempotencia toma de forma predeterminada un valor derivado de tu entorno y del nombre del service, por lo que volver a enviar la solicitud devuelve la primera creación.
...
El executor responde a las consultas de estado desde su API local. La API se enlaza a `127.0.0.1:9999` y no cuenta con autenticación propia. En una VM, ejecute los comandos en el host. En Kubernetes, abra primero un reenvío de puertos y apunte los comandos hacia él:
...
* **Escalado.** Un servicio tiene un número fijo de réplicas que establece ClickHouse Cloud, limitado por un ajuste de ClickHouse Cloud (20 en la configuración predeterminada). No hay autoescalado.
...
* **Copias de seguridad.** Las copias de seguridad se ejecutan cuando ClickHouse Cloud las activa y se almacenan en tu bucket de copias de seguridad; al eliminarlas, se borran de allí.
...
Las creaciones, los escalados y los inicios se completan de forma asíncrona. El executor informa de que el comando está en ejecución en cuanto aplica la definición y envía un informe de progreso cada 2 minutos. Informa del resultado final una vez que las réplicas del servidor están listas. Si no lo están en un plazo de 2 horas, informa de que el comando ha fallado. Si las réplicas se levantan más tarde, devuelve el servicio al estado `running`.

Los subcomandos `instances scale`, `instances patch` e `instances delete` envían comandos directamente a la API local del executor, sin intervención de ClickHouse Cloud. No los utilices a menos que tu account team de ClickHouse te pida ejecutar alguno; la [CLI reference](/es/products/bring-your-own-cloud/connector/reference/cli#instances-scale) describe qué hace cada uno.
...
El comando lee el registro que el executor tiene del servicio: dónde residen sus datos y qué role tenía. Elimina el IAM role que creó `prepare` y hace que el executor olvide el servicio, con lo que libera el nombre. En una VM también elimina los objetos RBAC del servicio a nivel de cluster, el paquete de acceso local y la entrada del registry.
...
Sin `--yes`, la ejecución se detiene tras el paso de registro e indica los nombres de los buckets que vaciaría. `--dry-run` lo lee todo y no escribe nada. La ejecución informa como conservados tanto un role que aportaste con `--role-arn` como un bucket que `prepare` no creó, y nunca los toca. La ejecución se rechaza mientras el espacio de nombres del servicio siga alojando un cluster de ClickHouse. Lee todo antes de eliminar nada, por lo que una ejecución rechazada no cambia nada.

## Cuando el conector está desconectado {#when-the-connector-is-offline}

Los services en ejecución no dependen del executor. El ClickHouse operator de tu cluster los mantiene en funcionamiento, y un conector desconectado no interrumpe nada que ya esté atendiendo queries.

Mientras no haya ningún executor conectado, ClickHouse Cloud no puede asignarle trabajo nuevo. Una creación, eliminación o sincronización de plataforma que ya se haya aceptado se conserva y se reintenta. Una creación se reintenta durante 30 minutos desde su último informe de progreso; una eliminación, durante 2 horas; y una sincronización de plataforma, hasta 10 intentos. Superado ese margen, el endpoint de tu conector marca el comando como fallido. Cualquier otro comando del ciclo de vida (scale, stop, start, restart, copia de seguridad, eliminación de copia de seguridad) se rechaza mientras no haya ningún executor conectado; no se pone en cola. Una creación o eliminación que envíes mientras no haya ningún executor conectado se rechaza del mismo modo; solo se reintentan los comandos ya aceptados.

Un comando que el executor ya haya aceptado sigue ejecutándose hasta completarse, y el executor envía en su siguiente conexión el resultado que no pudo comunicar.

Los dos comandos de cliente tienen sus propias dependencias. `instances create` necesita que el endpoint de tu conector sea accesible desde el host donde se ejecuta. `--wait`, `instances list`, `instances get` y `commands list` necesitan la API local del executor, por lo que requieren que el executor esté en ejecución. El token de una aprobación de plataforma caduca según su propio reloj, independientemente de la conectividad; consulta [la ventana de aprobación](/es/products/bring-your-own-cloud/connector/platform-updates#the-approval-window).