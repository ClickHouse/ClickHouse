...
sidebarTitle: 'Servicios gestionados'
...
## Crear un service {#create-a-service}

Crear un service requiere dos comandos. `prepare` crea todo lo necesario en tu lado; `instances create` envía la solicitud de creación a ClickHouse Cloud a través del endpoint del conector. ClickHouse Cloud genera la definición del service y envía la creación al executor por su canal saliente. El executor la aplica a tu cluster e informa del resultado.
...
* `prepare` necesita una copia de la configuración del conector (`--config`), un `kubectl port-forward` a la API local del executor y `--output-dir` apuntando a un directorio con permisos de escritura. Verifica el service name con el executor y se niega a ejecutarse si no logra alcanzarlo.
...
<Image img="/images/cloud/reference/byoc-connector-service-lifecycle.svg" size="lg" alt="Ciclo de vida del service de ClickHouse Connector en modo gestionado" />
...
## Sesiones de soporte para un managed service {#support-sessions}

El executor integra el scraper en cada service que crea. No aprovisiona el solucionador de problemas, por lo que una [sesión de soporte](/es/products/bring-your-own-cloud/connector/support-sessions) no podrá ejecutar diagnósticos en un managed service hasta que lo hagas. Aprovisiónalo una vez por cada service, con el espacio de nombres del service `ns-<name>`, igual que para una instance que hayas registrado tú mismo:
...
<Tabs>
  <Tab title="Kubernetes" id="troubleshooter-kubernetes">
    ...
    Luego añade el par Secret y ServiceAccount a `troubleshooter.accessBundles` y ejecuta `helm upgrade`, como se muestra en [añadir instances de ClickHouse](/es/products/bring-your-own-cloud/connector/configuration#clickhouse-instances).
  </Tab>
