...
Los flujos de trabajo de inscripción en modo administrado, de creación por parte del cliente y de aprobación de plataforma requieren [soporte confirmado por tu account team](/es/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability); no están disponibles en la versión pública `v0.16.0`.
...
Dos cosas dependen de ti. Crear un servicio requiere tus comandos `clicklink clctl executor prepare` y `clicklink clctl instances create`, porque solo tú puedes crear los buckets y el rol de IAM. Las actualizaciones de los componentes de la plataforma en tu cluster también dependen de ti: el executor deja preparado cada paquete propuesto, y tu comando `clicklink clctl platform approve` aprueba ese paquete y emite un token de 2 horas para él; consulta [actualizaciones de la plataforma](/es/products/bring-your-own-cloud/connector/platform-updates).

Todo lo que hace el executor ocurre a través de la Kubernetes API. Sus escrituras quedan restringidas por una política de admission a los espacios de nombres con el prefijo del servicio (`ns-` de forma predeterminada), más tres en su propio espacio de nombres: la renovación de su token, el ConfigMap del registry de servicios y, en Kubernetes, el certificado de client renovado en el Secret `clicklink-mtls`. Sus lecturas de esas mismas familias de recursos abarcan todo el cluster. Las actualizaciones de plataforma aprobadas se ejecutan con una identidad `pcm-platform` independiente, limitada a los espacios de nombres de la plataforma. Los clusters que operas tú mismo nunca se modifican: el scraper y el solucionador de problemas son de solo lectura. Los ámbitos exactos figuran en la página del [modelo de privilegios](/es/products/bring-your-own-cloud/connector/reference/privilege-model).
...
El scraper envía resultados de un conjunto fijo de system tables: de forma predeterminada, `metric_log`, `asynchronous_metric_log`, `tables`, `warnings` y `server_settings`. También envía latidos de salud y estado, el estado de la instance y de las copias de seguridad, y las métricas propias del connector. El conjunto de scrape predeterminado excluye `system.query_log`, de modo que el texto SQL en bruto, y cualquier literal o dato personal que contenga, nunca sale por la ruta de scrape a menos que lo añadas. Durante una sesión, la lista de permitidos de tablas predeterminada incluye `system.processes`, que muestra el texto de las consultas en curso; elimínala de la lista de permitidos si eso debe permanecer oculto.

El executor envía el resultado de cada comando que ejecuta, junto con el state y el número de réplicas de cada managed service. Su latido indica si el canal de comandos está conectado, las versiones de los componentes de la plataforma, el resumen del último paquete aplicado y el resultado de la última actualización de la plataforma. En una instalación sobre VM, el latido también indica si su paquete de acceso a Kubernetes está `ok`, `expiring` o `expired`. Cuando creas un service, `clicklink clctl instances create` envía el ID de tu entorno, la nube y la región, el nombre del service, los nombres de sus buckets y el ARN del rol de IAM. Envía dos hashes de la contraseña del usuario `default`; la contraseña en sí nunca sale.
...
<Note>
  La CLI pública `v0.16.0` no tiene ni `--managed` ni `--no-managed`. La opción durante la instalación que se describe a continuación requiere un build con [soporte de modo gestionado](/es/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability). Para detener un executor existente se usan las instrucciones de Helm o systemd que se indican abajo, sin necesidad de ninguno de los dos indicadores de init.
</Note>
...
Los servicios que ClickHouse creó siguen en ejecución, pero ClickHouse Cloud ya no puede escalarlos, actualizarlos, respaldarlos ni eliminarlos, y los comandos pendientes para ellos fallan. Las actualizaciones de la plataforma no se aplican nunca a menos que ejecutes `clicklink clctl platform approve`. Si no ejecutas ese comando, la capa de la plataforma queda congelada mientras el resto del modo gestionado sigue funcionando.
...
### ¿Qué se elimina cuando se elimina un service? {#what-is-deleted}

El executor detiene el service, elimina sus recursos de Kubernetes y borra su espacio de nombres. Informa de que el service ha terminado una vez que el espacio de nombres ya no existe. Todo lo que está fuera de Kubernetes se conserva: el bucket de datos, el bucket de copias de seguridad y el rol de IAM del service. El executor no dispone de credenciales para tus buckets ni para IAM, por lo que nada fuera de Kubernetes se ve afectado.

El resto se elimina con `clicklink clctl executor teardown`, ejecutado con tus credenciales. De forma predeterminada, elimina el rol de IAM y conserva los datos y las copias de seguridad; para borrarlos hay que usar indicadores explícitos. Consulta [eliminar un service](/es/products/bring-your-own-cloud/connector/managed-services#delete-a-service).

### ¿Dónde está la contraseña de mi usuario `default`? {#default-user-password}

La creación de servicios de cliente requiere [compatibilidad equivalente entre la CLI y el despliegue](/es/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) y no está disponible en la versión pública `v0.16.0`.
...
2. **Detenga el modo gestionado.** Detenga el executor y elimine su grant como se describe en [¿Cómo impido que ClickHouse modifique mis clusters?](#stop-managed-mode). Las actualizaciones de la plataforma ya requieren su aprobación una por una, por lo que negarla las detiene.
...
* En el modo gestionado, las imágenes de ClickHouse para sus servicios y componentes de la plataforma pueden provenir de su propio registry una vez que las haya importado allí; ClickHouse registra ese registry para su entorno.
* Con un [build que admita la aprobación de la plataforma](/es/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability), los paquetes de la plataforma pueden entregarse fuera de banda y pasarse a `clicklink clctl platform approve --bundle` como un archivo. La versión pública `v0.16.0` no tiene el comando `platform`.
...
Cada comando ejecutado durante una sesión, tanto si se acepta como si se bloquea, se añade a `/var/log/clicklink/troubleshoot-audit.log` en formato JSON delimitado por saltos de línea. Cada entrada registra `submitted_by` (el Organization ID en el canal de comandos), el texto del comando, la instance de destino, el resultado y `blocked_by` para los comandos rechazados. Las llamadas al gateway, incluidas las denegadas, se registran como líneas `clctl.gateway` estructuradas en el propio registro del solucionador de problemas, atribuidas al correo electrónico del operador acreditado por el token, nunca a un nombre declarado por el propio usuario. Los cambios de sesión local en una VM registran el usuario del host que los invoca como `enabled_by` en `/var/lib/clicklink/session.json`.
