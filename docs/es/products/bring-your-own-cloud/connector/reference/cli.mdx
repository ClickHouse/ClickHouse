...
| `--managed`                 | Habilita el modo gestionado: ejecuta el executor para que ClickHouse Cloud gestione el ciclo de vida de los servicios de ClickHouse en este cluster; omite el prompt. Requiere un cluster de Amazon EKS. No disponible en la versión pública `v0.16.0`; consulte [disponibilidad](/es/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability).                      |
| `--no-managed`              | Deja desactivado el modo gestionado (el executor); omite el prompt. No disponible en la versión pública `v0.16.0`; consulte [disponibilidad](/es/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability).                                                                                                                                                           |
...
El flujo de trabajo `init --managed` que se describe a continuación requiere un [build con soporte de modo gestionado](/es/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability); la versión pública `v0.16.0` no expone ese indicador.

`init --managed` aplica el grant a todo el cluster del executor con tus credenciales. Cuando no puede hacerlo, `init` muestra los comandos que debes ejecutar desde un kubeconfig con permisos de admin del cluster: `access apply` por sí solo en una VM (el paquete ya está preparado) o `access grant` seguido de `access apply` en Kubernetes. Consulta el [onboarding](/es/products/bring-your-own-cloud/connector/onboarding#install-and-enroll).
...
Aplica el paquete `grant` renderizado a nivel de cluster: el Espacio de nombres principal de la ServiceAccount, la ServiceAccount, el Rol de clúster y el ClusterRoleBinding, y la política de admission. A continuación, genera el token de la ServiceAccount dentro del paquete. No registra ningún service.
...
Elimina lo que `prepare` creó para un servicio que ClickHouse Cloud ha borrado. Es decir, el IAM role, los buckets que creó `prepare`, el nombre registrado, el registro del servicio en el executor y, en una VM, el Rol de clúster y el ClusterRoleBinding del servicio, el paquete de acceso local y la entrada del registry. El executor debe estar en ejecución y el servicio debe estar `terminated` (o `stale`); la ejecución se rechaza mientras el espacio de nombres del servicio siga alojando un cluster de ClickHouse. Los datos y las copias de seguridad se conservan salvo que se indique lo contrario, y los buckets conservados se vuelven a etiquetar como `clicklink:retained-from=<instance>`. El comando lo lee todo antes de borrar nada. Consulte [Delete a service](/es/products/bring-your-own-cloud/connector/managed-services#delete-a-service).
...
Los tres subcomandos siguientes envían comandos directamente a la API local del executor, sin que intervenga ClickHouse Cloud. Ejecútalos únicamente cuando el account team de ClickHouse te lo indique. Cada uno requiere `--name <name>` y el cluster (`--cluster`, `CLCTL_CLUSTER` o `~/.clicklink/clctl.yaml`), e imprime el comando en formato JSON. Pasa su `id` a [`commands get`](#commands-get) para hacerle seguimiento; el `result` de un comando fallido contiene su error.
...
Modifica un servicio in situ. `--create-nlb` es el único patch disponible. Aprovisiona un NLB interno, en forma de `Service` de Kubernetes, para que un scraper externo al cluster pueda alcanzar los servidores; solo en AWS. El executor completa por backfill el host del scraper a partir del balanceador de carga en su siguiente sincronización. Sin `--create-nlb`, el comando se rechaza con `nothing to patch`.
...
La CLI pública `v0.16.0` no tiene el comando `platform`, incluido `platform reset`; consulte [disponibilidad del modo gestionado](/es/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability).
...
| `--secret-namespace <ns>`, `--secret-name <name>` | Instalaciones en Kubernetes: elimina en su lugar el secreto del paquete de plataforma en el espacio de nombres del connector (nombre predeterminado `clicklink-platform-bundle`). |
