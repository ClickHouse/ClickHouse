...
Autoriza un cambio de plataforma con tus propias credenciales del cluster. Lee el paquete que preparó el executor, calcula su hash y renderiza cada chart que contiene exactamente igual que lo hará el executor. Aplica la mitad de permisos con el contexto de kube que elijas, garantiza la identidad `pcm-platform` del executor y emite su token. No realiza ninguna petición a tu endpoint del connector. Cuando un chart del paquete reside en Amazon ECR, se necesitan AWS credentials con permiso de lectura sobre él. La mitad de permisos comprende Namespaces, CustomResourceDefinitions, ServiceAccounts, Roles de clúster y sus bindings, Roles y sus bindings, configuraciones de webhook y admission, PriorityClasses y la clase de almacenamiento. Después, el executor aplica únicamente la mitad de workload, y solo mientras ese token siga siendo válido. Consulta [actualizaciones de plataforma](/es/products/bring-your-own-cloud/connector/platform-updates).
...
| `--secret-namespace <ns>` | Instalaciones de Kubernetes: lee el paquete preparado desde la API local del executor y entrega el paquete del token como un Secret en este espacio de nombres (el del connector) en lugar de en `<output-dir>/_platform`.    |
| `--secret-name <name>`    | Nombre del Secret para `--secret-namespace` (valor predeterminado `clicklink-platform-bundle`); debe coincidir con `executor.platformBundleSecret` del chart.                                                                |
