...
<Steps>
  ...
  <Step title="Registra e instala el conector" id="enroll-and-install">
    ...
    En un terminal, `init` pregunta `Enable managed mode (ClickHouse manages the lifecycle of ClickHouse services in this cluster)?` con `yes` preseleccionado. Pase `--managed` para omitir el prompt, o `--no-managed` para una instalación de solo observación sin el executor. Una instalación nueva desatendida sin ninguno de los dos indicadores deja el modo gestionado desactivado (una nueva ejecución con `--force` conserva la postura de la configuración reemplazada), así que pase siempre `--managed` en los scripts.
...
Si se rechaza el modo gestionado porque el cluster no es EKS, vuelve a ejecutar con `--no-managed` para instalar sin el executor. En una VM, también se rechaza un kubeconfig cuyo contexto actual apunte a otro lugar: cambia de contexto con `kubectl config use-context` y vuelve a ejecutar.
