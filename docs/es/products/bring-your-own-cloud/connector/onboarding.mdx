...
description: 'Instala ClickHouse Connector en modo gestionado y configúralo en Kubernetes o una VM Linux'
...
Esta página le guía desde un token de inscripción hasta un conector verificado en modo gestionado, donde el executor aplica el ciclo de vida del service que dirige ClickHouse Cloud. El conector se instala en un cluster de Kubernetes (Helm) o una VM Linux (systemd). La inscripción mediante token es el flujo estándar; si su entorno no puede acceder directamente al endpoint del conector, consulte las [instalaciones aisladas y con mirror](#air-gapped-and-mirrored-installs). Para observar una instancia de ClickHouse que ejecute usted mismo sin el executor, utilice `--no-managed`.

## Disponibilidad del modo administrado {#managed-mode-availability}

<Warning>
  A fecha del 22-09-2026, el instalador público ofrece la `v0.16.0`, que no incluye ni los indicadores `init --managed` e `init --no-managed` ni el comando `platform`. La aprobación de plataforma en instalaciones tanto en VM Linux como en Kubernetes, así como la creación de servicios de cliente (`instances create --from-prepare`), aún no están disponibles en una release pública. Además, la aprobación de plataforma en Kubernetes requiere compatibilidad de la CLI y del chart con `platform approve --secret-namespace`.

  Antes de seguir los flujos de trabajo del modo administrado, solicite a su ClickHouse account team que confirme la compatibilidad con su destino de despliegue y le proporcione la CLI correspondiente, el chart o la imagen del connector y las instrucciones de instalación. La creación de servicios de cliente también requiere compatibilidad en su endpoint del conector y en ClickHouse Cloud; no basta con disponer de una CLI más reciente. Los comandos de instalación públicos que se indican a continuación no instalan actualmente una release compatible con estos flujos de trabajo.
</Warning>
...
Antes de realizar la inscripción, su account team registra su entorno y habilita el modo gestionado para su organización. Proporciónele:

* el id de su AWS account, la región y el nombre del EKS cluster
* si el conector se ejecuta en una VM Linux o en el cluster
...
ClickHouse Cloud emite entonces su endpoint del conector y un token de inscripción de un solo uso. El token identifica su entorno, por lo que no tendrá que escribir ningún id adicional durante la inscripción.

### Para toda instalación {#prerequisites-every-install}

* **Una compilación con [soporte de modo administrado](#managed-mode-availability)** para las instrucciones de modo administrado que figuran a continuación. La versión pública `v0.16.0` no incluye `--managed` ni `--no-managed`.
* **El endpoint del conector y el token de inscripción**, proporcionados durante el [registro](#registration).
...
* **Un EKS cluster** para el modo administrado. `init` rechaza el modo administrado en cualquier otro cluster e indica `--no-managed` como la forma de instalar sin el executor.
...
* **Todavía sin ClickHouse.** Una instalación administrada comienza sin servicios; los que crea el executor se registran por sí mismos. En modo administrado, `init` no registra ningún ClickHouse que usted mismo ejecute (en Kubernetes rechaza `--instance` e `--instance-namespace`). Para observar un cluster de ese tipo, instale una implementación del conector aparte con `--no-managed`; `--ch-user-suffix` permite que ambas implementaciones compartan una instance sin colisionar en los usuarios del conector. Esa implementación necesita un listener native accesible: 9440 seguro o 9000 en plaintext, detectado automáticamente en Kubernetes. También necesita acceso de administración de ClickHouse para el aprovisionamiento: un usuario `default` sin password, un password pasado con `--ch-admin-password-stdin` o introducido en el prompt, o injection de CR para instances gestionadas por el operator.
...
* **Un kubeconfig** cuyo contexto actual apunte al cluster de EKS. Debe poder crear y leer el espacio de nombres del conector, aplicar Secrets, crear ServiceAccounts, Roles y RoleBindings, instalar el chart y otorgar al executor su acceso a nivel de cluster.
...
* **Los CIDR que hay detrás de tu endpoint del conector**, para habilitar la NetworkPolicy de denegación predeterminada del chart durante la instalación. Sin ellos, `init` deja la política preparada pero deshabilitada e indica cómo habilitarla más adelante.
...
* **Acceso de administración al Kubernetes API server** para el aprovisionamiento y el grant del executor: un kubeconfig en el host, `--server` y `--ca-data`, o los prompts. Los access bundles están anclados a ServiceAccounts de Kubernetes en ambos destinos. En modo gestionado, el contexto actual del kubeconfig debe apuntar al EKS cluster que administras; `init` se niega a otorgar el grant a través de cualquier otro contexto.

<Note>
  En una VM, `--skip-provision` es la única manera de sortear el requisito del Kubernetes API server, y solo aplica a stage. Omite el aprovisionamiento del ClickHouse user, el grant del executor, la habilitación de la unidad y la verificación, de modo que por sí solo no deja un connector en ejecución. En Kubernetes solo omite el aprovisionamiento del ClickHouse user; el grant del executor sí se aplica, porque el executor del chart no puede programarse sin la ServiceAccount con el grant otorgado.
</Note>

## Instalación e inscripción {#install-and-enroll}

Antes de empezar, confirma con tu account team la [disponibilidad del modo gestionado](#managed-mode-availability). El instalador público que se muestra a continuación distribuye la versión `v0.16.0`, que no puede ejecutar los comandos de inscripción en modo gestionado de esta guía.
...
<Steps>
  <Step title="Obtenga el endpoint del conector y el token de inscripción" id="get-endpoint-and-token">
    ...
    El token es de un solo uso y caduca rápidamente, por lo que conviene ejecutar la inscripción poco después de recibirlo. Trátelo como un secreto: la CLI lo lee desde un prompt oculto o desde la primera línea de stdin, nunca desde los argumentos de línea de comandos, el disco o los logs. Si caduca antes, solicite uno nuevo a su account team de ClickHouse.
  </Step>

  <Step title="Instale y verifique la CLI" id="install-and-verify-the-cli">
    Un solo comando instala un binary `clicklink` verificado. Detecta tu plataforma (macOS o Linux, amd64 o arm64), descarga la release actual y verifica la suma de comprobación SHA-256 y, si cosign está instalado, la firma de la release. Instala el binary en `/usr/local/bin` si ese directorio permite escritura; de lo contrario, en `~/.local/bin` (añádelo a tu `PATH` si es necesario; `--bin-dir` permite sobrescribir esta ubicación). Para una instalación en Kubernetes, ejecútalo desde cualquier equipo con acceso al cluster mediante kubeconfig:

    ```bash
    curl -fsSL https://releases.clicklink.clickhouse.com/install.sh | bash
    ```

    Para una instalación en una VM, ejecute el mismo script en el host con `--host`. Tras la descarga verificada, también crea el usuario de sistema `clicklink`, los directorios `/etc/clicklink`, `/var/lib/clicklink` y `/var/log/clicklink`, y las systemd units de los tres componentes. Inicializa `/etc/clicklink/redaction-patterns.yaml` con un conjunto inicial de patterns (que se conserva si ya existe), de modo que el siguiente paso comienza en la inscripción:
  ...
  <Step title="Registra e instala el conector" id="enroll-and-install">
    Estos comandos requieren el build y el soporte de implementación descritos en [disponibilidad del modo gestionado](#managed-mode-availability). La versión pública `v0.16.0` no incluye ni `--managed` ni `--no-managed`.

    La inscripción se realiza con un único comando. Canjea su token, obtiene un certificado de client firmado y el grant cluster-wide del executor, y luego instala y verifica el connector.
  ...
  <Step title="Limpieza" id="clean-up">
    El enrollment bundle `handoff.yaml` (modo `0600`, en el directorio de trabajo) existe para que las reejecuciones y la recuperación durante la instalación nunca requieran un segundo token. Contiene el secreto de API del connector en plaintext, así que elimínelo una vez verificada la instalación:
    ...
    El conector en ejecución conserva su propia copia de las credenciales, por lo que ninguna operación depende del archivo. Ni las actualizaciones ni los cambios de configuración lo requieren. Si más adelante vuelve a necesitar `init`, solicite un nuevo token de inscripción al equipo de cuenta de ClickHouse y ejecute `init --enroll --force`.
  </Step>
</Steps>
...
Si `init` rechaza `--managed` o `--no-managed` como un indicador desconocido, comprueba la versión de tu CLI: la `v0.16.0` pública no admite ninguno de los dos. Obtén un [build con soporte de modo administrado](#managed-mode-availability) a través de tu account team antes de seguir las instrucciones de recuperación de modo administrado que figuran a continuación.

Vuelve a ejecutar el mismo comando. `init` es idempotente: las reejecuciones convergen al mismo estado, conservan tu configuración existente y los archivos preparados, y omiten el trabajo ya completado. Cuando un paso falla a medio camino, la CLI imprime los comandos de recuperación exactos para tu caso; se pueden repetir sin riesgo.

Si se rechaza la inscripción, es porque el token ya fue canjeado, o bien porque es inválido o expiró. Si el token ya fue canjeado, vuelve a ejecutar con `--handoff handoff.yaml`, que existe hasta el paso de limpieza; de lo contrario, solicita un token nuevo a tu account team de ClickHouse. Si la inscripción falla con un error de transporte, vuelve a ejecutar el mismo comando. Si la reejecución se rechaza, el primer intento sí consumió el token, así que solicita uno nuevo.
...
`--force` es el reset explícito, no un reintento rutinario. Sobrescribe la configuración conservada o el values overlay, regenera el client key y reemplaza un certificado de cliente no expirado (un `409` del endpoint de firma significa que ya existe uno). El UUID del cluster del connector se conserva entre ejecuciones con `--force`, igual que su postura de modo administrado. Las reejecuciones desatendidas mantienen esa postura y, en un terminal, el prompt la ofrece como valor predeterminado, salvo que indiques `--managed` o `--no-managed`. Usa `--force` al rotar credenciales o al reemplazar un certificado, y consulta [operaciones](/es/products/bring-your-own-cloud/connector/operations) para conocer el modelo completo de reejecución y recuperación.