...
description: 'Instala ClickHouse Connector y configúralo en Kubernetes o una VM Linux'
...
Esta página le guía desde un token de inscripción hasta un conector en buen estado y verificado. El conector se instala en uno de estos dos destinos: un clúster de Kubernetes (Helm) o una VM Linux (systemd). La inscripción mediante token es el flujo estándar; si su entorno no puede acceder directamente a los endpoints de ClickHouse, consulte las [instalaciones aisladas y con espejo](#air-gapped-and-mirrored-installs).
...
## Instalaciones aisladas y con espejo {#air-gapped-and-mirrored-installs}
