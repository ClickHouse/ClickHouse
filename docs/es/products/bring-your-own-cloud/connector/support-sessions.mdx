...
Las sesiones de soporte permiten otorgar a ClickHouse acceso temporal con fines de diagnóstico mediante ClickHouse Connector. Esta página explica qué es una sesión, cómo activarla y desactivarla, qué pueden hacer los ingenieros de soporte de ClickHouse durante una sesión y cómo auditarla.

## Qué es una sesión de soporte {#what-a-support-session-is}

Una sesión de soporte es una ventana de tiempo acotada durante la cual el solucionador de problemas acepta comandos de los ingenieros de soporte de ClickHouse. Fuera de una sesión, el solucionador de problemas rechaza todos los comandos, incluso mientras su WebSocket saliente permanece conectado. No existe ninguna otra vía de ejecución: el solucionador de problemas no ejecuta nada sin una sesión, y ClickHouse no puede abrir una en su nombre. ClickHouse Cloud nunca se conecta a su entorno; solo recibe lo que el solucionador de problemas envía por su canal saliente.
...
Las sesiones caducan automáticamente. La duración predeterminada es de 4 horas; `session enable --duration` acepta hasta 24 horas. Cuando la sesión caduca o en cuanto ejecuta `session disable`, el solucionador de problemas deja de aceptar comandos. Deshabilitarla es la vía de revocación inmediata: no requiere reiniciar ni coordinarse con ClickHouse.

## Lista de permitidos de operadores {#operator-allowlist}

Cada llamada al gateway se autoriza conforme a una lista de permitidos de correos electrónicos de operadores. El cotejo se realiza con el correo electrónico acreditado por el token OIDC validado, nunca con lo que el client declare sobre sí mismo.
...
En ambos targets, el gateway vuelve a leer el archivo en la siguiente request una vez transcurridos 30 segundos desde la última lectura, por lo que las modificaciones surten efecto sin reiniciar.
...
Cada comando ejecutado durante una sesión se añade a `/var/log/clicklink/troubleshoot-audit.log` como un objeto JSON por línea (NDJSON). Los comandos bloqueados también se registran, indicando en `blocked_by` el motivo (por ejemplo, `session inactive`). `submitted_by` es el Organization ID del canal de comandos, es decir, el ID de su propia organización. El ingeniero de ClickHouse concreto queda registrado del lado de ClickHouse Cloud, no en este archivo. Una entrada de comando de sesión tiene este aspecto:
...
El audit log es un archivo plano dentro de su entorno; envíelo a su propio SIEM como cualquier otro log de host o de contenedor.
