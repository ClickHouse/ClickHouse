...
| ----------------------------------------------------------- | ------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Operaciones diarias — servidor de API de Kubernetes**     | Pública con filtrado de IP (predeterminado), o privada mediante Tailscale o AWS VPC Lattice | Los servicios de administración se comunican con el servidor de API de Kubernetes (EKS/GKE/AKS) a través de la red pública, con acceso restringido mediante listas de IP permitidas. Después de la implementación inicial, opcionalmente puede cambiar esto a acceso privado mediante Tailscale o, en AWS, VPC Lattice. |
...
| **Aplicabilidad de Tailscale** | **Ninguna.** Estas llamadas van directamente desde la red de ClickHouse Cloud al proveedor de nube y no pueden enrutarse a través de Tailscale ni de su red                                                                                                                                                                  | Endpoint público restringido de forma predeterminada a las IP de salida de ClickHouse; puede cambiarse a acceso privado mediante Tailscale o, en AWS, VPC Lattice                                                                                                                                                                                                                         |
...
En resumen: solo el tráfico de la API de Kubernetes y el de resolución de problemas pueden usar Tailscale. Las llamadas a la API del proveedor de nube siempre se originan en la red de ClickHouse Cloud y terminan en los endpoints del proveedor; no es posible enrutarlas a través de Tailscale, ya que nunca entran en su red.
...
De forma predeterminada, el servicio de administración de ClickHouse accede a su clúster de Kubernetes BYOC a través de la dirección IP pública del servidor de la API, restringida exclusivamente a las direcciones IP del gateway NAT de ClickHouse.
...
* De forma predeterminada, el endpoint público (restringido a las direcciones IP NAT de ClickHouse) se mantiene como mecanismo de respaldo para casos de investigación y soporte de emergencia; una vez verificado el acceso privado, puede deshabilitarse por completo en coordinación con ClickHouse
...
State Exporter envía información sobre el estado del servicio de ClickHouse y de las copias de seguridad (eventos de estado operativo, no el contenido de las copias de seguridad) a una cola de SQS propiedad de ClickHouse Cloud.