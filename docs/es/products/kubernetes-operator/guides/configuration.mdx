...
Cada pod de réplica lleva la readiness gate `clickhouse.com/ReplicaInitialized`, de modo que una nueva réplica solo se publica a través del Service headless público una vez que el operador completa su inicialización: la base de datos `default` se convierte al motor Replicated y el esquema se sincroniza. Hasta entonces, el operador y las demás réplicas la alcanzan a través de su Service interno. Con `enableDatabaseSync: false`, el operador marca las réplicas como inicializadas de inmediato, por lo que la readiness depende únicamente de las probes del contenedor.

El operador nunca elimina una base de datos `default` poblada que no sea Replicated. Una réplica en esta situación aun así empieza a atender el tráfico de clientes tras la preparación del esquema, pero el clúster reporta `SchemaInSync=False` con motivo `DefaultDatabaseNotReplicated` hasta que resuelvas esa base de datos por tu cuenta.

Antes de eliminar una réplica al reducir la escala, el operador primero descarga el tráfico de clientes de esa réplica, espera hasta que deje de publicarse, replica sus datos restantes en las réplicas supervivientes, y solo entonces la elimina.
