...
ClickHouse Cloud ahora ofrece ClickPipes para migrar su base de datos externa de PostgreSQL a un servicio de ClickHouse Managed Postgres. Esta integración incorporada ofrece una experiencia optimizada para conectarse a su base de datos de origen, exportar el esquema, importarlo en ClickHouse Managed Postgres y configurar la replicación continua.
...
<Note>
  Si tienes problemas durante la migración, consulta las [preguntas frecuentes sobre migraciones de ClickHouse Managed Postgres](/es/products/managed-postgres/migrations/faq) para ver los errores más comunes y sus soluciones.
</Note>
...
<Steps>
  <Step title="Conéctate a tu base de datos de origen" id="step-1-connect">
    ...
    <Image img="/images/managed-postgres/pgpg/overview.webp" alt="Entrada fuentes de datos en la barra lateral del servicio de ClickHouse Managed Postgres" size="lg" border />
  ...
  <Step title="Importa el esquema en tu servicio de ClickHouse Managed Postgres" id="step-3-import-schema">
    ...
    El asistente muestra un comando `psql` para aplicar el volcado de esquema a tu servicio de ClickHouse Managed Postgres. Ejecútalo en una terminal:
  ...
  <Step title="Configura los ajustes de ingestión" id="step-4-ingestion-settings">
    Especifica la publication que se usará para la replicación lógica. Si dejas este campo en blanco, se creará una publication automáticamente.
    ...
    | Número de tablas de la instantánea en paralelo  | 1              | Tablas para las que se toma una instantánea de forma concurrente |

    <Image img="/images/managed-postgres/pgpg/advancedsettings.webp" alt="Paso 4: formulario de ajustes de ingestión con publication y opciones avanzadas de replicación" size="lg" border />

    Haz clic en **Next**.
  </Step>

  <Step title="Selecciona tablas" id="step-5-select-tables">
    Selecciona las tablas que quieres replicar. Las tablas se agrupan por esquema. Selecciona tablas individuales o expande un esquema para elegirlas todas.

    <Image img="/images/managed-postgres/pgpg/tablepicker.webp" alt="Paso 5: selector de tablas agrupadas por esquema con el botón Create migration" size="lg" border />
...
**Redirija el tráfico de la aplicación.** Dirija las operaciones de lectura y escritura a su servicio ClickHouse Managed Postgres y supervise los errores, las violaciones de restricciones y el estado de la replicación.
...
* [Detalles de conexión de ClickHouse Managed Postgres](/es/products/managed-postgres/connection)
