...
description: 'Benchmarks de rendimiento que comparan ClickHouse Managed Postgres con AWS Aurora, RDS y otros servicios gestionados de PostgreSQL'
...
<Info>
  ...
  * **Benchmark** de ClickHouse Managed Postgres frente a AWS RDS (16k IOPS aprovisionado) y Aurora IO Optimized con pruebas estándar de [`pgbench`](https://www.postgresql.org/docs/current/pgbench.html)
  * **Rendimiento**: ClickHouse Managed Postgres, respaldado por almacenamiento NVMe, ofrece un **rendimiento entre 4,3 y 9 veces superior** en cargas de trabajo intensivas en IO y **un 12 % más de rendimiento** en escenarios limitados por CPU
...
**Análisis**: Las cargas de trabajo mixtas de lectura/escritura muestran las ventajas de rendimiento más notables del almacenamiento NVMe y representan el **escenario más realista para carga de trabajo de IA en rápido crecimiento** que requieren tanto una ingestión de datos de alto rendimiento como lecturas de baja latencia. **ClickHouse Managed Postgres alcanzó 19.8K TPS con mayor concurrencia**, lo que demuestra cómo el almacenamiento NVMe escala eficazmente bajo carga. Esto es **4.3-4.5x más rápido que RDS y Aurora**. Las soluciones de almacenamiento conectado a la red tuvieron dificultades con las operaciones con predominio de escritura, y RDS y Aurora alcanzaron un máximo de 4.4K-4.6K TPS a pesar de la capacidad aprovisionada e incluso con la configuración IO Optimized de Aurora.
...
| Configuración        | ClickHouse Managed Postgres | RDS con 16k IOPS                 | Aurora IO Optimized               |
| -------------------- | --------------------------- | -------------------------------- | --------------------------------- |
...
| Configuración        | ClickHouse Managed Postgres | RDS con 16k IOPS                 |
| -------------------- | --------------------------- | -------------------------------- |
| **Versión de PG**    | 17                          | 17                               |
| **vCPU**             | 16                          | 16                               |
| **RAM**              | 64 GB                       | 64 GB                            |
...
**Análisis**: Incluso en escenarios limitados por CPU, donde la E/S de disco es mínima, **ClickHouse Managed Postgres lideró el grupo con 36,5 K TPS**. Aunque ambos servicios alcanzaron un 100 % de uso de CPU, el almacenamiento NVMe en ClickHouse Managed Postgres ofreció un rendimiento superior con mejores tasas de aciertos de caché. La ventaja del 12 % sobre RDS demuestra la eficiencia de la infraestructura subyacente incluso cuando las cargas de trabajo dependen principalmente de la CPU.
...
| Configuración        | ClickHouse Managed Postgres | RDS PostgreSQL           |
| -------------------- | --------------------------- | ------------------------ |
| **Versión de PG**    | 17                          | 17                       |
| **vCPU**             | 2                           | 2                        |
| **RAM**              | 8 GB                        | 8 GB                     |
...
En los tres escenarios de benchmark, ClickHouse Managed Postgres ofreció sistemáticamente un rendimiento superior:
...
| Aspecto                      | Almacenamiento NVMe (ClickHouse Managed Postgres) | Almacenamiento conectado a la red (IOPS aprovisionadas)                  |
| ---------------------------- | ------------------------------------------------- | ------------------------------------------------------------------------ |
...
Más allá del rendimiento bruto, ClickHouse Managed Postgres ofrece una relación precio-rendimiento superior:
...
En los workloads que actualmente están limitados por IOPS, cambiar a ClickHouse Managed Postgres puede eliminar la necesidad de IOPS aprovisionadas costosas o de configuraciones IO Optimized, al tiempo que ofrece un rendimiento significativamente superior.
...
* [Descripción general de ClickHouse Managed Postgres](/es/products/managed-postgres/overview)
