---
noindex: true
title: "Desplegar ClickHouse en GCP GKE"
sidebarTitle: "Desplegar en GCP"
description: "Este tutorial le guía en el despliegue de ClickHouse Private en Google Cloud Platform mediante Google Kubernetes Engine (GKE)."
---

Este tutorial le guía en el despliegue de ClickHouse Private en Google Cloud Platform mediante Google Kubernetes Engine (GKE). Abarca tanto las implementaciones estándar como las compatibles con FIPS 140-2. Los pasos específicos de FIPS están marcados con **(FIPS)** en todo el tutorial; establezca `ENABLE_FIPS=true` en `00-env.sh` para habilitar el modo FIPS.

El tutorial está organizado en scripts numerados (`00-env.sh` a `19-verify`) que se ejecutan en secuencia. Cada script es autónomo y carga el archivo de entorno.
...
* Credenciales de AWS con acceso de lectura a ClickHouse ECR (para copiar imágenes)
* **Host bastión** en la VPC para acceder al clúster privado
* **Cuenta de servicio del implementador** con los permisos correspondientes (consulta a continuación)
* **(FIPS)** Acceso a la imagen de contenedor UBI8 de Red Hat

***

<Steps>
  <Step stepNumber={0} title="Paso 0: Configuración de la cuenta de servicio" id="step-0-service-account-setup">
    ...
    3. Introduzca el nombre `clickhouse-deployer` y el ID `clickhouse-deployer`
    4. Añada los siguientes roles:
    ...
    1. Otorgue el permiso Token Creator a la cuenta de servicio de Compute del bastión.
    ...
    Para quitar la suplantación más tarde:
  ...
  <Step stepNumber={1} title={<>Paso 1: Configurar variables de entorno (<code>00-env.sh</code>)</>} id="step-1-set-environment-variables-00-envsh">
    Cree este archivo e inclúyalo antes de ejecutar cualquier otro script. Actualice todos los valores según su entorno.
    ...
    **Importante:** Cargue este archivo antes de ejecutar otros scripts:
  ...
  <Step stepNumber={2} title={<>Paso 2: Crear Artifact Registry (<code>01-artifact-registry.sh</code>)</>} id="step-2-create-artifact-registry-01-artifact-registrysh">
    ```bash expandable
    #!/bin/bash
    set -e
    source ./00-env.sh
  ...
  <Step stepNumber={3} title={<>Paso 3: Copiar imágenes de contenedor (<code>02-copy-images.sh</code>)</>} id="step-3-copy-container-images-02-copy-imagessh">
    Se requiere [skopeo](https://github.com/containers/skopeo) para conservar la compatibilidad con todas las arquitecturas.
  ...
  <Step stepNumber={4} title={<>Paso 4: Configurar la VPC y la subred (<code>03-vpc-subnet.sh</code>)</>} id="step-4-configure-vpc-and-subnet-03-vpc-subnetsh">
    Este script supone que ya dispone de una VPC existente con un bastión. Crea o actualiza la subred de GKE con los rangos secundarios necesarios.

    **Importante: requisitos sobre los límites CIDR** - bloques `/14`: el tercer octeto debe ser divisible por 4 (p. ej., `10.244.0.0`, NO `10.245.0.0`) - bloques `/20`: el cuarto octeto debe ser 0 y el tercer octeto debe estar alineado en múltiplos de 16 (p. ej., `10.252.0.0`)
  ...
  <Step stepNumber={5} title={<>Paso 5: Crear Cloud NAT (<code>04-cloud-nat.sh</code>)</>} id="step-5-create-cloud-nat-04-cloud-natsh">
    Los nodos Private de GKE requieren Cloud NAT para acceder a internet saliente.
  ...
  <Step stepNumber={6} title={<>Paso 6: Crear una cuenta de servicio de GCP (<code>05-service-account.sh</code>)</>} id="step-6-create-gcp-service-account-05-service-accountsh">
    ```bash
    #!/bin/bash
    set -e
    source ./00-env.sh
  ...
  <Step stepNumber={7} title={<>Paso 7: Crear un bucket de GCS (<code>06-gcs-bucket.sh</code>)</>} id="step-7-create-gcs-bucket-06-gcs-bucketsh">
    ```bash expandable
    #!/bin/bash
    set -e
    source ./00-env.sh
  ...
  <Step stepNumber={8} title={<>Paso 8: Crear clúster de GKE (<code>07-gke-cluster.sh</code>)</>} id="step-8-create-gke-cluster-07-gke-clustersh">
    Crea un **clúster de GKE totalmente privado** -- los nodos worker solo tienen IP internas, el plano de control no tiene endpoint público y todo el acceso debe hacerse desde la VPC a través del bastión.
  ...
  <Step stepNumber={9} title={<>Paso 9: Crear grupos de nodos (<code>08-node-pools.sh</code>)</>} id="step-9-create-node-pools-08-node-poolssh">
  ...
  <Step stepNumber={10} title={<>Paso 10: Desplegar el conjunto de daemon de Local SSD (<code>09-local-ssd-daemonset.sh</code>)</>} id="step-10-deploy-local-ssd-daemonset-09-local-ssd-daemonsetsh">
    GKE Container-Optimized OS tiene un sistema de archivos raíz de solo lectura, por lo que no se puede crear `/nvme`. El conjunto de daemon formatea y monta el Local SSD en `/mnt/disks/ssd0`.

    **Importante:** Despliega esto **antes** del clúster de ClickHouse para evitar condiciones de carrera.
  ...
  <Step stepNumber={11} title={<>Paso 11: Agregar etiquetas de topología (<code>10-topology-labels.sh</code>)</>} id="step-11-add-topology-labels-10-topology-labelssh">
    ```bash
    #!/bin/bash
    set -e
    source ./00-env.sh
  ...
  <Step stepNumber={12} title="Paso 12: Instalar los requisitos previos de Kubernetes" id="step-12-install-kubernetes-prerequisites">
    ### Instalar los CRDs de VolumeSnapshot (`11-volumesnapshot-crds.sh`)
    ...
    ### Configurar Workload Identity (`12-workload-identity.sh`)
    ...
    **Alineación de Workload Identity:**
    ...
    | ServiceAccount de K8s (creada por helm) | `ch-$CLUSTER_NAME-sa` en el espacio de nombres `ns-$CLUSTER_NAME` |
    ...
    **Nota:** GKE utiliza `topology.gke.io/zone` en lugar de `topology.kubernetes.io/zone`.
  ...
  <Step stepNumber={13} title="Paso 13: Pasos de FIPS (condicional)" id="step-13-fips-steps-conditional">
    Omita esta sección si `ENABLE_FIPS=false`.

    ### Crear certificados compatibles con FIPS (`14-create-fips-certs.sh`)

    Los certificados compatibles con FIPS deben generarse con algoritmos aprobados por FIPS (RSA de 3072 bits como mínimo, SHA-256 o superior) en un entorno con FIPS habilitado. El script ejecuta un pod de Kubernetes con Red Hat UBI8 en el clúster de GKE, con la política criptográfica de FIPS habilitada.

    **Tipos de certificados requeridos:**

    | Certificado | Archivos                                 | SAN requeridos                                                                    |
    | ----------- | ---------------------------------------- | --------------------------------------------------------------------------------- |
    ...
    | Cliente     | `client/client.crt`, `client/client.key` | No se requiere ninguno                                                            |
    | Keeper      | `keeper/keeper.crt`, `keeper/keeper.key` | `*.c-${CLUSTER_NAME}-keeper-headless.ns-${CLUSTER_NAME}.svc.${KUBERNETES_DOMAIN}` |

    Para consultar el procedimiento completo de generación de certificados, vea [how-to/configure-fips-certificates.md](/es/products/clickhouse-private/guides/generate-fips-compliant-certificates-for-clickhouse).

    ### Crear secretos de certificados (`15-create-cert-secrets.sh`)
    ...
    **Estructura del Secret:**
    ...
    | Nombre del Secret                    | Claves                                                           | Propósito                                           |
    | ------------------------------------ | ---------------------------------------------------------------- | --------------------------------------------------- |
    | `${CLUSTER_NAME}-server-cert-secret` | `ca.crt`, `server.crt`, `server.key`, `client.crt`, `client.key` | TLS del servidor y conexiones salientes del cliente |
    | `${CLUSTER_NAME}-keeper-cert-secret` | `ca.crt`, `keeper.crt`, `keeper.key`                             | TLS de Keeper                                       |

    ***
  </Step>

  <Step stepNumber={14} title={<>Paso 14: Instalar el operador (<code>16-install-operator.sh</code>)</>} id="step-14-install-operator-16-install-operatorsh">
    ```bash expandable
    #!/bin/bash
    set -e
    source ./00-env.sh
  ...
  <Step stepNumber={15} title={<>Paso 15: Desplegar el clúster de ClickHouse (<code>17-install-cluster.sh</code>)</>} id="step-15-deploy-clickhouse-cluster-17-install-clustersh">
    ```bash expandable
    #!/bin/bash
    set -e
    source ./00-env.sh
    ...
    **Configuraciones importantes específicas de GCP:**

    * `storageClass.create=false` -- No cree la StorageClass de AWS
  ...
  <Step stepNumber={16} title={<>Paso 16: Vincular Workload Identity (<code>18-bind-helm-workload-identity.sh</code>)</>} id="step-16-bind-workload-identity-18-bind-helm-workload-identitysh">
    ...
    **Importante:** Los pods de Keeper y del servidor creados durante el paso 15 se inician antes de que exista esta vinculación, por lo que fallarán con errores 403 de GCS. Este script reinicia los pods después de realizar la vinculación.
  ...
  <Step stepNumber={17} title="Paso 17: Verificar la instalación" id="step-17-verify-installation">
    ...
    Con `openSSL.required=true`, ClickHouse solo acepta conexiones en puertos seguros. Usa el puerto **9440** (TLS nativo):
    ...
    Para entornos de producción con certificados firmados por una CA:
    ...
    **Importante (FIPS):** El puerto 9000 (protocolo nativo sin cifrar) y el 8123 (HTTP sin cifrar) rechazarán las conexiones cuando `openSSL.required=true`. Use en su lugar el puerto 9440 (TLS nativo) o el 8443 (HTTPS).
...
| EBS CSI Driver          | GCE PD CSI Driver           | Preinstalado; aprovisionador: `pd.csi.storage.gke.io` |
