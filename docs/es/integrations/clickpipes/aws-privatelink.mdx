...
Si su servicio no puede usar el DNS privado administrado por PrivateLink (por ejemplo, si el proveedor requiere
que los consumidores administren la resolución de DNS, como hace Confluent Cloud), consulte [DNS privado personalizado](#custom-private-dns).
...
<div id="custom-private-dns">
  ## DNS privada personalizada
</div>

<PrivatePreviewBadge />

<Info>
  La DNS privada personalizada está en **Private Preview**. Ponte en contacto con el [soporte de ClickHouse](https://clickhouse.com/support/program) para habilitarla en tu servicio.
</Info>

AWS PrivateLink proporciona [DNS privada administrada](https://docs.aws.amazon.com/vpc/latest/privatelink/manage-dns-names.html)
para servicios de endpoint de VPC con un dominio verificado. Algunos servicios requieren que cada consumidor gestione la resolución DNS en su propia VPC; por ejemplo, Confluent Cloud exige que los consumidores resuelvan los nombres de host de los brokers al endpoint de PrivateLink.

Para estos casos, ClickPipes admite asociar nombres de DNS privada personalizada a un endpoint privado inverso. ClickPipes resuelve estos nombres a las direcciones privadas del endpoint, para que tu fuente de datos
sea accesible mediante su propio hostname a través de conectividad privada.

La DNS privada personalizada complementa la DNS privada administrada; no la sustituye. Si tu
servicio de PrivateLink ya proporciona nombres de DNS privada, no necesitas nombres personalizados.

Los nombres de DNS privada personalizada son compatibles con los tipos de endpoint [servicio de endpoint de VPC](#vpc-endpoint-service)
y [recurso de VPC](#vpc-resource). MSK multi-VPC proporciona DNS privada administrada
de forma predeterminada y no admite nombres personalizados.

Las siguientes reglas se aplican a los nombres de DNS privada personalizada:
...
Para configurar nombres de DNS privada personalizada:
