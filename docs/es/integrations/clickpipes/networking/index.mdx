...
| [GCP Private Service Connect](/es/integrations/clickpipes/networking/gcp-psc)  | Sí         | Para fuentes de datos `AlloyDB` alojadas en GCP. La conectividad entre regiones mediante Global Access aún no es compatible; use [túneles SSH](#ssh-tunneling) para acceder a una fuente de datos `AlloyDB` privada en una región de GCP no compatible. |
| [Azure Private Link](/es/integrations/clickpipes/networking/azure-privatelink) | No         | Use [túneles SSH](#ssh-tunneling) para acceder a una fuente de datos privada de Azure.                                                                                                                                                                  |
...
mediante un túnel SSH a través de un host bastión. Agregue las [IP de NAT estáticas](/es/integrations/clickpipes/networking/static-ips) correspondientes a la región desde la que se conecta ClickPipes a las reglas del firewall del host bastión de origen.
