...
<Info>
  Antes de empezar a añadir los datos de tu conexión, asegúrate de haber incluido las direcciones IP de ClickPipes en la lista de permitidos de las reglas de tu firewall. Puedes encontrar la lista de direcciones IP de ClickPipes [aquí](/es/integrations/clickpipes/networking/static-ips).
...
Puede seguir la [guía de configuración para establecer la conexión](/es/integrations/clickpipes/networking/aws-privatelink).
...
<Note>
  Asegúrese de permitir [dirección IP de ClickPipes](/es/integrations/clickpipes/networking/static-ips) en las reglas de su firewall para el host bastión SSH, de modo que ClickPipes pueda establecer el túnel SSH.
</Note>
...
8. Además, puede proporcionar una expresión `PARTITION BY <expr>` personalizada para controlar el particionado de la [tabla de ClickHouse de destino](/es/concepts/core-concepts/partitions).
