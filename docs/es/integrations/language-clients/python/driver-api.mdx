...
| `show_clickhouse_errors`   | bool o None                  | `True`                                         | Incluye detalles de los errores del servidor y nombres simbólicos de excepción en las excepciones del client.                                                                                                                           |
...
El módulo `clickhouse_connect.dbapi` implementa la interfaz de conexión y cursor definida por PEP 249. Declara el nivel de API 2.0, `threadsafety=2` y `paramstyle="pyformat"`.
...
`Cursor.execute` y `Cursor.executemany` aceptan un argumento adicional de palabra clave, `settings`, para los ajustes de ClickHouse. `executemany` usa la ruta nativa de inserción masiva del driver para las sentencias `INSERT ... VALUES` compatibles con una secuencia materializada de filas. `fetchone`, `fetchmany` y `fetchall` consumen el resultado materializado actual.
