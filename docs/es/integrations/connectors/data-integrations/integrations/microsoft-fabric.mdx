...
<div id="copying-data-from-onelake">
  ## Copia de datos desde OneLake
</div>

El flujo Copy &amp; Sync permite transferir tablas de OneLake a ClickHouse:

1. Abra su elemento de ClickHouse y seleccione **Copy &amp; Sync**
2. Elija un Lakehouse de su espacio de trabajo
3. Seleccione las tablas que desea acelerar
4. Inicie la copia de una instantánea de sus datos en un momento concreto

<Frame>
  ![Configuración de Copy & Sync para crear una nueva tabla de destino con una clave de ordenación](/images/integrations/tools/data-integration/microsoft-fabric/fabric-03-copy-sync-table-selection.webp)
</Frame>
...
<Frame>
  ![Correspondencia de columnas de Copy & Sync desde una tabla de origen de OneLake a una tabla de ClickHouse existente](/images/integrations/tools/data-integration/microsoft-fabric/fabric-04-copy-sync-existing-table.webp)
</Frame>
...
<Note>
  En la versión preliminar pública, solo se admite la copia de una instantánea en un momento concreto. Se prevén más opciones de replicación en futuras versiones.
</Note>
...
La copia de datos de OneLake a ClickHouse es gratuita durante la versión preliminar pública de Microsoft Fabric. La facturación comenzará cuando la carga de trabajo de Fabric alcance la disponibilidad general. Los detalles de precios se publicarán cuando la carga de trabajo alcance la GA.
...
Selecciona **Abrir ClickHouse** para abrir la consola completa de ClickHouse Cloud con tu identidad de Microsoft. Puedes copiar datos a ClickHouse y ejecutar cualquier sentencia SQL desde la carga de trabajo de Fabric, pero para realizar tareas administrativas deberás acceder a Cloud Console para administrar el servicio.
...
Cada Copy &amp; Sync desde OneLake se ejecuta como un ClickPipe. Supervise el progreso de la copia, el rendimiento y los errores en la [página de ClickPipes](/es/integrations/clickpipes/monitoring) de la consola de ClickHouse Cloud.
...
2. Abra **Copy &amp; Sync**, elija su Lakehouse y seleccione la tabla `publicholidays`.
...
* **Las copias son instantáneas puntuales.** Aún no hay replicación programada ni continua. Para actualizar una tabla con los datos más recientes de OneLake, vuelva a ejecutar la sincronización. Las actualizaciones y eliminaciones de la tabla de origen no se propagan a la copia de ClickHouse.
* **Solo tablas de Lakehouse.** Copy &amp; Sync lee tablas Delta de un Lakehouse en el mismo espacio de trabajo. Otros elementos de Fabric (Warehouse, bases de datos KQL y bases de datos replicadas) aún no se admiten como fuentes.
...
* **Los cambios de rol se aplican la próxima vez que abras un elemento.** Tu rol en la organización de ClickHouse se asigna según tu rol en el espacio de trabajo de Fabric al abrir un elemento de ClickHouse. Fabric no puede notificar a ClickHouse los cambios de rol, por lo que un cambio en el rol del espacio de trabajo surte efecto la próxima vez que abras el elemento, no de inmediato.
