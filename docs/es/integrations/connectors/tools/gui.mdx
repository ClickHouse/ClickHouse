...
<Accordion title="CHOps">
  [CHOps](https://ch-ops.io) es un dashboard web gratuito, de código abierto y autohospedado para administrar y supervisar clústeres de ClickHouse. Ofrece la interfaz HTTP de ClickHouse en una UI de navegador para ejecutar SQL, inspeccionar consultas lentas, configurar alertas, gestionar copias de seguridad y controlar el acceso. Funciona con implementaciones de ClickHouse accesibles mediante HTTP. Actualmente, CHOps admite ClickHouse 26.3 LTS; otras versiones recientes también funcionan, aunque puede haber pequeñas carencias de funcionalidad.
  ...
  * Supervisión con reproducción estilo DVR para revisar métricas históricas fotograma a fotograma.
...
<Accordion title="DataPup">
  [DataPup](https://github.com/DataPupOrg/DataPup) es un cliente de bases de datos moderno, multiplataforma y asistido por IA, con soporte nativo para ClickHouse.
...
<Accordion title="HouseOps">
  ...
  * Constructor de consultas con resaltado de sintaxis. Visualiza la respuesta en una tabla o en formato JSON.
  * Exportación de resultados de consultas como CSV o JSON.
  * Lista de procesos con descripciones. Modo de escritura. Posibilidad de detener (`KILL`) un proceso.
...
<Accordion title="Redash">
  ...
  Admite múltiples fuentes de datos, incluido ClickHouse, y puede combinar los resultados de consultas de diferentes fuentes de datos en un único conjunto de datos final.
...
<Accordion title="Grafana">
  ...
  &quot;Grafana permite consultar, visualizar, generar alertas sobre las métricas y comprenderlas, sin importar dónde estén almacenadas. Crea, explora y comparte paneles con tu equipo, y fomenta una cultura basada en datos. La comunidad confía en ella y la aprecia&quot; — grafana.com.
...
<Accordion title="ClickHouse table graph">
  [ClickHouse table graph](https://github.com/mbaksheev/clickhouse-table-graph) es una sencilla herramienta de CLI para visualizar las dependencias entre tablas de ClickHouse. Obtiene las conexiones entre tablas de la tabla `system.tables` y genera un diagrama de flujo de dependencias en formato [mermaid](https://mermaid.js.org/syntax/flowchart.html). Con esta herramienta puede visualizar fácilmente las dependencias entre tablas y comprender el flujo de datos de su base de datos en ClickHouse. Gracias a mermaid, el diagrama de flujo resultante tiene un aspecto atractivo y se puede añadir fácilmente a su documentación en Markdown.
</Accordion>
...
<Accordion title="MindsDB Studio">
  [MindsDB](https://mindsdb.com/) es una capa de IA de código abierto para bases de datos, incluido ClickHouse, que permite desarrollar, entrenar y desplegar sin esfuerzo modelos de aprendizaje automático de vanguardia. MindsDB Studio (GUI) permite entrenar nuevos modelos a partir de bases de datos, interpretar las predicciones del modelo, identificar posibles sesgos en los datos y evaluar y visualizar la precisión del modelo mediante la función Explainable AI para adaptar y ajustar más rápidamente sus modelos de aprendizaje automático.
</Accordion>

<Accordion title="DBM">
  [DBM](https://github.com/devlive-community/dbm) es una herramienta visual de administración para ClickHouse.

  Características:

  * Historial de consultas (paginación, borrar todo, etc.)
  * Consultas de cláusulas SQL seleccionadas
  * Finalización de consultas
  * Administración de tablas (metadatos, eliminación, vista previa)
  * Administración de bases de datos (eliminación, creación)
  * Consultas personalizadas
  * Administración de múltiples fuentes de datos (prueba de conexión, monitorización)
  * Monitorización (procesador, conexión, consulta)
  * Migración de datos
</Accordion>
...
<Accordion title="clickhouse-monitoring">
  [clickhouse-monitoring](https://github.com/duyet/clickhouse-monitoring) es un sencillo dashboard de Next.js que se basa en tablas `system.*` para ayudar a supervisar y ofrecer una visión general de su clúster de ClickHouse.
  ...
  * Información de tablas y partes: tamaño, recuento de filas, compresión, tamaño de las partes, etc., con detalle a nivel de columna.
  * Herramientas útiles: exploración de datos de ZooKeeper, EXPLAIN de consultas, finalización de consultas, etc.
  * Gráficos de métricas: consultas y uso de recursos, número de fusiones/mutaciones, rendimiento de fusiones, rendimiento de consultas, etc.
</Accordion>
...
<Accordion title="CKibana">
  ...
  * Traduce las solicitudes de gráficos de la UI nativa de Kibana a sintaxis de consultas de ClickHouse.
  * Admite características avanzadas como el muestreo y el almacenamiento en caché para mejorar el rendimiento de las consultas.
  * Minimiza la curva de aprendizaje para los usuarios después de migrar de ElasticSearch a ClickHouse.
</Accordion>

<Accordion title="Telescope">
  [Telescope](https://iamtelescope.net/) es una interfaz web moderna para explorar registros almacenados en ClickHouse. Proporciona una UI fácil de usar para consultar, visualizar y gestionar datos de registros con control de acceso granular.

  Características:

  * UI limpia y adaptable con filtros potentes y selección de campos personalizable.
  * Sintaxis FlyQL para un filtrado de registros intuitivo y expresivo.
  * Gráfico basado en el tiempo con compatibilidad con agrupaciones, incluidos campos JSON anidados, Map y Array.
  * Compatibilidad opcional con cláusulas `WHERE` de SQL sin procesar para filtrado avanzado (con comprobaciones de permisos).
  * Vistas guardadas: guarde y comparta configuraciones personalizadas de la UI para consultas y diseño.
  * Integración de control de acceso basado en roles (RBAC) y autenticación de GitHub.
  * No se requieren agentes ni componentes adicionales en el lado de ClickHouse.
...
<Accordion title="clickhouse-flow">
  [clickhouse-flow](https://github.com/MikeAmputer/clickhouse-flow) es una herramienta open-source para visualizar flujos de datos y dependencias entre tablas, vistas y vistas materializadas de ClickHouse.
...
<Accordion title="Chadmin">
  [Chadmin](https://github.com/bun4uk/chadmin) es una UI sencilla en la que puede visualizar las consultas que se están ejecutando actualmente en su clúster de ClickHouse, consultar información sobre ellas y cancelarlas si lo desea.
</Accordion>
