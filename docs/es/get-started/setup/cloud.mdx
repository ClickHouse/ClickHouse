...
<Steps>
  <Step title="Crear un servicio de ClickHouse" id="1-create-a-clickhouse-service">
    ...
    * Inicia sesión con el nombre de usuario y la contraseña que acabas de crear
    ...
    De forma predeterminada, las nuevas organizaciones se asignan al nivel Scale y crearán 3 réplicas, cada una con 4 vCPU y 16 GiB de RAM. El [escalado vertical automático](/es/products/cloud/features/autoscaling/vertical) estará habilitado de forma predeterminada en el nivel Scale. Puedes cambiar el nivel de la organización más adelante en la página &#39;Plans&#39;.

    Si es necesario, personaliza los recursos del servicio especificando un tamaño mínimo y máximo entre los que puedan escalar las réplicas. Cuando estés listo, selecciona `Create service`.
  ...
  <Step title="Conectarse a ClickHouse" id="2-connect-to-clickhouse">
    ...
    * Conectarse con su aplicación
    ...
    Para empezar rápidamente, ClickHouse ofrece una consola SQL web a la que se le redirigirá al completar el onboarding.

    <Image img="/images/_snippets/createservice8.webp" size="md" alt="Consola SQL" border />
    ...
    <Image img="/images/_snippets/service_connect.webp" size="md" alt="Conectar al servicio" border />

    <br />

    Si no ves el cliente para tu lenguaje, quizá quieras consultar nuestra lista de [Integrations](/es/integrations/home).
  </Step>

  <Step title="Agregar datos" id="3-add-data">
    ...
    * Sube datos desde la URL de un archivo
    ...
    Al igual que la mayoría de los sistemas de gestión de bases de datos, ClickHouse agrupa lógicamente las tablas en **bases de datos**. Use el comando [`CREATE DATABASE`](/es/reference/statements/create/database) para crear una nueva base de datos en ClickHouse:
    ...
    * `message`: un tipo de dato [String](/es/reference/data-types/string), que sustituye a tipos como `VARCHAR`, `BLOB`, `CLOB` y otros de otros sistemas de bases de datos
    ...
    * `metric`: un número de coma flotante de 32 bits ([Float32](/es/reference/data-types/float))
    ...
    <Info>
      ...
      * Si los datos se replican o no
      ...
      Hay muchos motores de tabla para elegir, pero para una tabla sencilla en un servidor ClickHouse de un solo nodo, [`MergeTree`](/es/reference/engines/table-engines/mergetree-family/mergetree) suele ser la opción más adecuada.
    </Info>
    ...
    * las claves primarias en ClickHouse ***no son únicas*** para cada fila de una tabla
    ...
    datos (denominado **granularidad de índice**) crea una entrada en el archivo de índice de clave primaria. Este concepto de granularidad
    crea un **índice disperso** que cabe fácilmente en memoria, y los gránulos representan una franja de la cantidad mínima de
    ...
    la clave pasa a ser la tupla indicada en la cláusula `ORDER BY`. Si se especifican tanto `PRIMARY KEY` como `ORDER BY`, la clave primaria debe ser un subconjunto del orden de clasificación.
    ...
    Puede utilizar el conocido comando [`INSERT INTO TABLE`](/es/reference/statements/insert-into) con ClickHouse, pero es importante entender que cada inserción en una tabla [`MergeTree`](/es/reference/engines/table-engines/mergetree-family/mergetree) provoca que se cree una **parte** en el almacenamiento.

    <Tip>
      **Práctica recomendada de ClickHouse**

      Inserte una gran cantidad de filas en cada lote: decenas de miles o incluso millones de
      filas de una sola vez. No se preocupe: ClickHouse puede manejar fácilmente ese volumen y [ahorrarle dinero](/es/concepts/best-practices/selecting-an-insert-strategy#batch-inserts-if-synchronous) al enviar menos solicitudes de escritura a su servicio.
    </Tip>
    ...
    <Note>
      Observe que la columna `timestamp` se rellena mediante varias funciones [**Date**](/es/reference/data-types/date) y [**DateTime**](/es/reference/data-types/datetime). ClickHouse tiene cientos de funciones útiles que puede [ver en la sección **Funciones**](/es/reference/functions/index).
    </Note>
    ...
    También puede conectarse a su servicio de ClickHouse Cloud mediante una herramienta de línea de comandos llamada [**clickhouse client**](/es/concepts/features/interfaces/cli). Haga clic en `Connect` en el menú izquierdo para acceder a estos detalles. En el cuadro de diálogo, seleccione `Native` en el menú desplegable:
    ...
    4. Añade una cláusula [`FORMAT`](/es/reference/statements/select/format) para especificar uno de los [muchos formatos de salida que admite ClickHouse](/es/reference/formats/index):
    ...
    5. Para salir de `clickhouse client`, escriba el comando **exit**:
    ...
    Una tarea habitual al comenzar a trabajar con una base de datos es insertar algunos datos que ya tienes en archivos. Disponemos de datos de muestra en línea que puedes insertar y que representan datos de clickstream: incluyen un ID de usuario, una URL visitada y la marca de tiempo del evento.
    ...
    2. Observe que las nuevas filas ya aparecen en la tabla al consultar desde SQL Console:
...
* Tenemos una lista de [conjuntos de datos de ejemplo](/es/get-started/sample-datasets/index) con instrucciones sobre cómo insertarlos
* Mira nuestro video de 12 minutos sobre [Primeros pasos con ClickHouse](https://clickhouse.com/company/events/getting-started-with-clickhouse/)
* Si tus datos provienen de una fuente externa, consulta nuestra [colección de guías de integración](/es/integrations/home) para conectarte a colas de mensajes, bases de datos, canalizaciones y más
* Si usas una herramienta de visualización UI/BI, consulta las [guías de usuario para conectar una UI a ClickHouse](/es/integrations/connectors/data-visualization/index)
