...
<Steps>
  ...
  <Step title="Añadir datos" id="3-add-data">
    ...
    * `metric`: un número de coma flotante de 32 bits ([Float32](/es/reference/data-types/float))
    ...
    #### Breve introducción a las primary keys

    Antes de seguir adelante, es importante entender cómo funcionan las primary keys en ClickHouse (¡la implementación
    de las primary keys puede resultar sorprendente!):

    * las claves primarias de ClickHouse ***no son únicas*** para cada fila de una tabla

    La primary key de una tabla de ClickHouse determina cómo se ordenan los datos al escribirlos en disco. Cada 8192 filas o 10 MB de
    datos (lo que se conoce como **index granularity**) se crea una entrada en el archivo del índice de primary key. Este concepto de granularidad
    da lugar a un **índice disperso** que cabe fácilmente en memoria, y los granules representan un stripe con la menor cantidad de
    ...
    la clave pasa a ser la tupla indicada en la cláusula `ORDER BY`. Si especifica tanto una `PRIMARY KEY` como un `ORDER BY`, la clave primaria debe ser un subconjunto del orden distinto.

    La primary key es también la sorting key, que es una tupla de `(user_id, timestamp)`.  Por lo tanto, los datos almacenados en cada
    column file estarán ordenados por `user_id` y, a continuación, por `timestamp`.
    ...
    ### Añadir datos con el ClickHouse Client

    También puede conectarse a su ClickHouse Cloud service mediante una herramienta de línea de comandos llamada [**clickhouse client**](/es/concepts/features/interfaces/cli). Haga clic en `Conectar` en el menú de la izquierda para acceder a estos datos. En el cuadro de diálogo, seleccione `Native` en la lista desplegable:
    ...
    Observe que la respuesta se devuelve en un cómodo formato de tabla:
    ...
    datos de muestra que puede insertar y que representan datos de flujo de clics: incluyen un ID de usuario, una URL visitada y
