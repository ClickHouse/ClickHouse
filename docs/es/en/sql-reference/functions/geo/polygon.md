---
description: 'Documentación de Polygon'
sidebar_label: 'Polígonos'
slug: /sql-reference/functions/geo/polygons
title: 'Funciones para trabajar con polígonos'
doc_type: 'reference'
---
...
Devuelve un objeto geométrico WKT (Well Known Text) a partir de varios [tipos de datos Geo](../../data-types/geo.md). Los objetos WKT admitidos son:
...
`geo_data` puede ser uno de los siguientes [Tipos de datos Geo](../../data-types/geo.md) o uno de sus tipos primitivos subyacentes:
...
* Para un Point, se devuelve el objeto geométrico WKT `POINT`.
* Para un Polygon, se devuelve el objeto geométrico WKT `POLYGON`.
* Para un MultiPolygon, se devuelve el objeto geométrico WKT `MULTIPOLYGON`.
* Para un LineString, se devuelve el objeto geométrico WKT `LINESTRING`.
* Para un MultiLineString, se devuelve el objeto geométrico WKT `MULTILINESTRING`.

**Ejemplos**

POINT a partir de Tuple:
...
POLYGON a partir de un Array de Tuples o de un Array de Arrays de Tuples:
...
MULTIPOLYGON a partir de un Array de Arrays de Tuple multidimensionales:
...
Convierte un MultiPolygon en formato WKT (Well Known Text) al tipo MultiPolygon.
...
Convierte un MultiPolygon en formato WKT (Well Known Text) en un valor de tipo Polygon.
...
Cadena que empieza por `POLYGON`
...
La función devuelve la representación interna de ClickHouse para la geometría Point.
...
La función devuelve una representación interna en ClickHouse de la geometría LineString.
...
Analiza una representación en Well-Known Text (WKT) de una geometría MultiLineString y la devuelve en el formato interno de ClickHouse.
...
La función devuelve una representación interna de ClickHouse de la geometría MultiLineString.
...
Analiza una representación en Well-Known Text (WKT) de una geometría Polygon y devuelve un ring (linestring cerrado) en el formato interno de ClickHouse.
...
La función devuelve una representación interna de ClickHouse de la geometría del ring (cadena de líneas cerrada).
...
Convierte un MultiPolygon en WKB (Well Known Binary) en un valor de tipo MultiPolygon.
...
Convierte un `MultiPolygon` WKB (Well Known Binary) en un `Polygon`.
...
La función `readWKBPoint` de ClickHouse interpreta una representación Well-Known Binary (WKB) de una geometría Point y devuelve un punto en el formato interno de ClickHouse.
...
* `wkb_string`: La cadena WKB de entrada que representa una geometría Point.

<div id="returned-value">
  ### Valor devuelto
</div>

La función devuelve la representación interna en ClickHouse de la geometría Point.
...
Analiza una representación en Well-Known Binary (WKB) de una geometría LineString y la devuelve en el formato interno de ClickHouse.
...
La función devuelve la representación interna en ClickHouse de la geometría linestring.
...
Analiza una representación en Well-Known Binary (WKB) de una geometría MultiLineString y la devuelve en el formato interno de ClickHouse.
...
La función devuelve la representación interna de ClickHouse de la geometría MultiLineString.
...
Calcula la diferencia simétrica espacial de teoría de conjuntos (XOR) entre dos polígonos
...
Igual que `polygonsSymDifferenceSpherical`, pero con las coordenadas en el sistema de coordenadas cartesianas, que se acerca más al modelo de la Tierra real.
...
Calcula la intersección (AND) entre polígonos; las coordenadas son esféricas.
...
Devuelve true si los dos polígonos se intersectan (comparten alguna área o borde).
...
UInt8: 0 para false y 1 para true

<div id="polygonsintersectspherical">
  ## polygonsIntersectSpherical
</div>

Devuelve `true` si los dos polígonos se intersecan (comparten alguna zona o borde en común). Referencia: https://www.boost.org/doc/libs/1&#95;62&#95;0/libs/geometry/doc/html/geometry/reference/algorithms/intersects.html
...
Calcula el casco convexo. [Referencia](https://www.boost.org/doc/libs/1_61_0/libs/geometry/doc/html/geometry/reference/algorithms/convex_hull.html)

Las coordenadas están en el sistema de coordenadas cartesianas.
...
Este es el polígono que representa Zimbabue:
...
Calcula la intersección entre polígonos.
...
Para obtener más información sobre los sistemas geométricos, consulta esta [presentación](https://archive.fosdem.org/2020/schedule/event/working_with_spatial_trajectories_in_boost_geometry/attachments/slides/3988/export/events/attachments/working_with_spatial_trajectories_in_boost_geometry/slides/3988/FOSDEM20_vissarion.pdf) sobre la biblioteca Boost, que es la que utiliza ClickHouse.