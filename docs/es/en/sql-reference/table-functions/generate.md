---
description: 'Genera datos aleatorios con un esquema determinado. Permite poblar tablas de prueba
  con esos datos. No todos los tipos son compatibles.'
...
Genera datos aleatorios con un esquema determinado.
Permite poblar tablas de prueba con esos datos.
...
| `TypeName`          | Tipo de la columna correspondiente.                                                                                               |
| `random_seed`       | Especifique manualmente la semilla aleatoria para obtener resultados estables. Si es `NULL`, la semilla se genera aleatoriamente. |
| `max_string_length` | Longitud máxima de todas las cadenas generadas. El valor predeterminado es `10`.                                                  |
| `max_array_length`  | Número máximo de elementos de todos los arrays o mapas generados. El valor predeterminado es `10`.                                |
...
En combinación con [generateRandomStructure](../../sql-reference/functions/other-functions.md#generateRandomStructure):
...
:::note
`generateRandom(generateRandomStructure(), [random seed], max_string_length, max_array_length)` con un `max_array_length` suficientemente grande puede generar una salida enorme debido a la gran profundidad de anidamiento posible (hasta 16) de los tipos complejos (`Array`, `Tuple`, `Map`, `Nested`).
:::
...
* Blog: [Cómo generar datos aleatorios en ClickHouse](https://clickhouse.com/blog/generating-random-test-distribution-data-for-clickhouse)