...
sidebar_label: 'Funciones de ventana'
sidebar_position: 1
...
Pueden usarse para realizar cálculos similares a los de las funciones de agregación, pero se diferencian en que una función de ventana no agrupa las filas en una única salida; en cambio, sigue devolviendo filas individuales.
...
La tabla siguiente muestra qué características son compatibles actualmente:

| Funcionalidad                                                                           | ¿Compatible? | Comentario                                                                                                                                                                                                                                                                                                                                                                                                          |
| --------------------------------------------------------------------------------------- | ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| Marco `ROWS`                                                                            | ✅            |                                                                                                                                                                                                                                                                                                                                                                                                                     |
| Marco `RANGE`                                                                           | ✅            | Se usa de forma predeterminada cuando no se especifica un marco explícitamente (`RANGE BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW`).                                                                                                                                                                                                                                                                               |
...
| `percent_rank()`/`percentRank()`                                                        | ✅            | Calcula de forma eficiente la posición relativa de un valor dentro de una partición. Sustituye el cálculo manual en SQL, más detallado y computacionalmente costoso, expresado como `ifNull((rank() OVER (PARTITION BY x ORDER BY y) - 1) / nullif(count(1) OVER (PARTITION BY x) - 1, 0), 0)`.                                                                                                                     |
...
| `lag/lead(value, offset)`                                                               | ✅            | También puede usar una de las siguientes alternativas:<br /> 1) `any(value) OVER (... ROWS BETWEEN <offset> PRECEDING AND <offset> PRECEDING)`, o `FOLLOWING` en lugar de `PRECEDING` para `lead` <br /> 2) `lagInFrame/leadInFrame`, que son análogas, pero respetan el marco de la ventana. Para obtener un comportamiento idéntico a `lag/lead`, use `ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING`. |
| [`ntile(buckets)`](./ntile.md)                                                          | ✅            | Especifique la ventana como, por ejemplo, `(PARTITION BY x ORDER BY y ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING)`.                                                                                                                                                                                                                                                                                   |
...
* `ROWS or RANGE` - define los límites de un marco; aggregate&#95;function se calcula dentro de ese marco.
...
| Función                                                                                                    | Descripción                                                                                                                                     |
| ---------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| [`row_number()`](./row_number.md)                                                                          | Numera la fila actual dentro de su partición a partir de 1.                                                                                     |
| [`first_value(x)`](./first_value.md)                                                                       | Devuelve el primer valor evaluado dentro de su marco ordenado.                                                                                  |
| [`last_value(x)`](./last_value.md)                                                                         | Devuelve el último valor evaluado dentro de su marco ordenado.                                                                                  |
| [`nth_value(x, offset)`](./nth_value.md)                                                                   | Devuelve el primer valor no NULL evaluado en la enésima fila (`offset`) de su marco ordenado.                                                   |
| [`rank()`](./rank.md)                                                                                      | Asigna una clasificación a la fila actual dentro de su partición con huecos.                                                                    |
| [`dense_rank()`](./dense_rank.md)                                                                          | Asigna una clasificación a la fila actual dentro de su partición sin huecos.                                                                    |
| [`lagInFrame(x)`](./lagInFrame.md)                                                                         | Devuelve un valor evaluado en la fila situada un desplazamiento físico de filas especificado antes de la fila actual dentro del marco ordenado. |
| [`leadInFrame(x)`](./leadInFrame.md)                                                                       | Devuelve un valor evaluado en la fila situada un número de filas después de la fila actual dentro del marco ordenado.                           |
| [`ntile(buckets)`](./ntile.md)                                                                             | Divide las filas ordenadas dentro de una partición en el número indicado de grupos y devuelve el número de grupo de la fila actual.             |
| [`nonNegativeDerivative(metric_column, timestamp_column[, INTERVAL X UNITS])`](./nonNegativeDerivative.md) | Calcula la derivada no negativa de `metric_column` con respecto a `timestamp_column`. Es específica de ClickHouse.                              |

<div id="examples">
  ## Ejemplos
</div>

Veamos algunos ejemplos de cómo pueden utilizarse las funciones de ventana.
...
Los siguientes ejemplos resuelven problemas prácticos habituales.
...
<div id="moving--sliding-average-per-3-rows">
  ### Media móvil / media deslizante (cada 3 filas)
</div>
...
<div id="moving--sliding-average-per-10-seconds">
  ### Media móvil / media deslizante (cada 10 segundos)
</div>
...
<div id="moving--sliding-average-per-10-days">
  ### Media móvil / media deslizante (cada 10 días)
</div>

La temperatura se almacena con precisión de un segundo, pero al usar `Range` y `ORDER BY toDate(ts)` formamos un frame de 10 unidades y, debido a `toDate(ts)`, la unidad es un día.
...
La hoja de ruta del soporte inicial para las funciones de ventana está [en este issue](https://github.com/ClickHouse/ClickHouse/issues/18097).
...
Estas pruebas contienen ejemplos de la gramática actualmente compatible:
...
* Blog: [Funciones de ventana y de array para secuencias de commits de Git](https://clickhouse.com/blog/clickhouse-window-array-functions-git-commits)
