---
description: 'Documentación de la sentencia ALTER TABLE ... MODIFY QUERY'
sidebar_label: 'VIEW'
sidebar_position: 50
...
Puede modificar la consulta `SELECT` especificada al crear una [vista materializada](/es/sql-reference/statements/create/view#materialized-view) con la sentencia `ALTER TABLE ... MODIFY QUERY` sin interrumpir el proceso de ingestión.

Este comando está pensado para modificar una vista materializada creada con la cláusula `TO [db.]name`. No cambia la estructura de la tabla de almacenamiento subyacente ni la definición de las columnas de la vista materializada; por ello, su uso es muy limitado en las vistas materializadas creadas sin la cláusula `TO [db.]name`.
...
Su uso es muy limitado, ya que solo se puede cambiar la sección `SELECT` sin añadir nuevas columnas.
...
`ALTER TABLE ... MODIFY REFRESH` cambia los parámetros de actualización de una [vista materializada actualizable](../create/view.md#refreshable-materialized-view), incluida la programación, las dependencias, la aleatoriedad y la [configuración de actualización](../create/view.md#refresh-settings).

```sql
ALTER TABLE [db.]name MODIFY REFRESH EVERY|AFTER ... [RANDOMIZE FOR ...] [DEPENDS ON ...] [SETTINGS ...]
```

La programación (`EVERY` o `AFTER`) es obligatoria: la sentencia reemplaza *todos* los parámetros de actualización a la vez. Cualquier cláusula no especificada — `RANDOMIZE FOR`, `DEPENDS ON` o `SETTINGS` — se elimina o se restablece a los valores predeterminados. Para cambiar solo la configuración de actualización, repita la programación actual.
...
* `ALTER TABLE ... MODIFY SETTING` no se admite en las vistas materializadas; la configuración de actualización solo puede cambiarse mediante `MODIFY REFRESH`.
...
La lista completa de configuraciones de actualización está documentada en [Configuración de actualización](../create/view.md#refresh-settings). El estado de la actualización, incluida la configuración aplicada actualmente, puede verse en [`system.view_refreshes`](../../../operations/system-tables/view_refreshes.md).