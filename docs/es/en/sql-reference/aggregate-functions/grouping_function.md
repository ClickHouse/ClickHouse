...
[ROLLUP](../statements/select/group-by.md/#rollup-modifier) y [CUBE](../statements/select/group-by.md/#cube-modifier) son modificadores de GROUP BY. Ambos calculan subtotales. ROLLUP toma una lista ordenada de columnas, por ejemplo `(day, month, year)`, y calcula subtotales en cada nivel de la agregación, además de un total general. CUBE calcula subtotales para todas las combinaciones posibles de las columnas especificadas. GROUPING identifica qué filas devueltas por ROLLUP o CUBE son filas superagregadas y cuáles son filas que devolvería un GROUP BY sin modificar.
...
De forma predeterminada, el modificador CUBE calcula subtotales para todas las combinaciones posibles de las columnas incluidas en CUBE. GROUPING SETS permite especificar las combinaciones concretas que se deben calcular.
...
Obtén el número de servidores en cada centro de datos por distribución:
...
<div id="comparing-multiple-group-by-statements-with-grouping-sets">
  ### Comparación de varias sentencias GROUP BY con GROUPING SETS
</div>
...
<div id="comparing-cube-with-grouping-sets">
  ### Comparación de CUBE con GROUPING SETS
</div>

El CUBE de la siguiente consulta, `CUBE(datacenter,distro,version)`, establece una jerarquía que puede no resultar lógica. No tiene sentido analizar la versión entre ambas distribuciones (ya que Arch y RHEL no comparten el mismo ciclo de lanzamientos ni las mismas convenciones de nomenclatura de versiones). El ejemplo de GROUPING SETS que aparece a continuación es más adecuado, ya que agrupa `distro` y `version` en el mismo conjunto.
...
:::note
Es posible que la versión del ejemplo anterior no tenga sentido si no está asociada a una distro; si estuviéramos rastreando la versión del kernel, sí podría tenerlo, porque la versión del kernel puede asociarse a cualquiera de las dos distros. En ese caso, usar GROUPING SETS, como en el siguiente ejemplo, puede ser una mejor opción.
:::
