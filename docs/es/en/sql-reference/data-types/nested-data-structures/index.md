...
Una estructura de datos anidada es como una tabla dentro de una celda. Los parámetros de una estructura de datos anidada —los nombres y tipos de las columnas— se especifican de la misma forma que en una consulta [CREATE TABLE](../../../sql-reference/statements/create/table.md). Cada fila de la tabla puede corresponderse con cualquier número de filas de una estructura de datos anidada.

:::tip[Evita usar puntos en los nombres de las columnas]
Los nombres de columnas que contienen puntos, las columnas que comparten un prefijo común terminado en punto y las columnas de tipo `Array` pueden interpretarse como parte de una estructura `Nested` aplanada cuando `flatten_nested = 1` (el valor predeterminado). Esto puede provocar validaciones inesperadas de la longitud de los arrays en las inserciones y restricciones al cambiar nombres.

Evita usar puntos en los nombres de las columnas siempre que sea posible.
Usa guiones bajos (`_`) u otro separador en lugar de puntos en los nombres de las columnas, a menos que necesites intencionadamente la semántica de `Nested`.
:::
...
La sentencia DDL `CREATE TABLE` anterior declara la estructura de datos anidada `Goals`, que contiene datos sobre conversiones, o objetivos alcanzados.
Cada fila de la tabla &#39;visits&#39; corresponde a cero o más conversiones.

Cuando la configuración [`flatten_nested`](/es/operations/settings/settings#flatten_nested) se establece en `0` (`flatten_nested=1` de forma predeterminada), se admiten niveles arbitrarios de anidamiento.

En la mayoría de los casos, al trabajar con una estructura de datos anidada, sus columnas se especifican mediante nombres de columna separados por puntos.
...
Como cada columna de una estructura `Nested` se almacena como un `Array`, al hacer referencia a ella en una cláusula `WHERE` se obtiene el array completo de cada fila, no un elemento individual. No puede comparar directamente una columna anidada con un valor escalar, por lo que debe usar [funciones de array](/es/sql-reference/functions/array-functions).
...
Usa [`has`](/es/sql-reference/functions/array-functions#has) para comprobar si un array contiene un valor concreto:
...
Usa [`arrayExists`](/es/sql-reference/functions/array-functions#arrayExists) cuando la condición sea más compleja:
...
Puedes filtrar según la longitud del array con `length` o excluir los arrays vacíos con `notEmpty`:
...
Para filtrar por elementos individuales de una estructura anidada en lugar de por filas completas, use `ARRAY JOIN` para expandir primero los arrays.
...
Para obtener más información, consulte la [cláusula `ARRAY JOIN`](/es/sql-reference/statements/select/array-join). Ejemplo:
...
No puedes ejecutar `SELECT` sobre una estructura de datos anidada completa. Solo puedes indicar explícitamente las columnas individuales que forman parte de ella.

<div id="inserting-data">
  ### Inserción de datos
</div>

Para una consulta `INSERT`, debe pasar por separado todos los arrays de las columnas que componen una estructura de datos anidada (como si fueran arrays de columnas individuales). Durante la inserción, el sistema comprueba que tengan la misma longitud.

Cada subcolumna anidada se incluye en la lista de columnas mediante notación de puntos (`Goals.ID`, `Goals.Serial`, ...), y los valores correspondientes son arrays:
...
En una consulta `DESCRIBE`, las columnas de una estructura de datos anidada también se enumeran por separado, de la misma manera.
...
**Añadir subcolumnas** funciona con normalidad. Puede añadir una subcolumna nueva a una estructura `Nested` existente:

```sql
ALTER TABLE test.visits ADD COLUMN Goals.Revenue Float64;
```

**La eliminación de subcolumnas** funciona con subcolumnas individuales:

```sql
ALTER TABLE test.visits DROP COLUMN Goals.Revenue;
```

**Cambiar el tipo** de una subcolumna funciona y desencadena una mutación (reescritura de datos):

```sql
ALTER TABLE test.visits MODIFY COLUMN Goals.Price Int32;
```

**Renombrar** tiene restricciones. Puedes renombrar una subcolumna dentro de la misma estructura anidada:
...
* Cambiar el nombre de toda la estructura anidada (p. ej., `Goals` por `Conversions`).
* Mover una subcolumna a otra estructura anidada (p. ej., `Goals.ID` a `OtherNested.ID`).
* Sacar una subcolumna de una estructura anidada o moverla a una estructura anidada (p. ej., `Goals.ID` a `GoalID` o viceversa).