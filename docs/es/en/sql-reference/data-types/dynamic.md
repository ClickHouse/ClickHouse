...
Este tipo permite almacenar valores de cualquier tipo sin conocerlos todos de antemano.
...
Donde `N` es un parámetro opcional entre `0` y `254` que indica cuántos tipos de datos diferentes pueden almacenarse como subcolumnas separadas dentro de una columna de tipo `Dynamic` en un único bloque de datos que se almacena por separado (por ejemplo, en una sola parte de datos de una tabla MergeTree). Si se supera este límite, todos los valores con tipos nuevos se almacenarán juntos en una estructura de datos compartida especial en formato binario. El valor predeterminado de `max_types` es `32`.
...
<div id="reading-dynamic-nested-types-as-subcolumns">
  ## Leer tipos anidados de Dynamic como subcolumnas
</div>
...
en todas las filas en las que la columna `Dynamic` original no tenga el tipo `T`.

Las subcolumnas de `Dynamic` también se pueden leer con la función `dynamicElement(dynamic_column, type_name)`.
...
Para saber qué variante se almacena en cada fila, puede usarse la función `dynamicType(dynamic_column)`. Devuelve `String` con el nombre del tipo del valor para cada fila (o `'None'` si la fila es `NULL`).
...
<div id="conversion-between-dynamic-column-and-other-columns">
  ## Conversión entre la columna Dynamic y otras columnas
</div>

Hay 4 conversiones posibles que pueden realizarse con la columna `Dynamic`.
...
<div id="converting-a-string-column-to-a-dynamic-column-through-parsing">
  ### Conversión de una columna String a una columna Dynamic mediante parseo
</div>

Para parsear valores de tipo `Dynamic` a partir de una columna `String`, puede habilitar la configuración `cast_string_to_dynamic_use_inference`:
...
<div id="converting-a-dynamic-column-to-an-ordinary-column">
  ### Conversión de una columna Dynamic en una columna ordinaria
</div>
...
<div id="converting-a-dynamicmax_typesn-column-to-another-dynamicmax_typesk">
  ### Convertir una columna `Dynamic(max_types=N)` en otra `Dynamic(max_types=K)`
</div>

Si `K >= N`, durante la conversión los datos no cambian:
...
Si `K < N`, los valores con los tipos menos frecuentes se insertarán en una única subcolumna especial, pero seguirán siendo accesibles:
...
La función `isDynamicElementInSharedData` devuelve `true` para las filas almacenadas en una estructura especial de datos compartida dentro de `Dynamic` y, como podemos ver, la columna resultante contiene solo 2 tipos que no se almacenan en esa estructura de datos compartida.
...
Todos los formatos de texto (TSV, CSV, CustomSeparated, Values, JSONEachRow, etc.) admiten la lectura del tipo `Dynamic`. Durante el análisis de los datos, ClickHouse intenta inferir el tipo de cada valor y usarlo durante la inserción en una columna `Dynamic`.
...
Si la función no puede ejecutarse con algún tipo dentro de la columna `Dynamic`, se producirá una excepción:
...
Podemos filtrar los tipos que no necesitemos:
...
O extraiga el tipo necesario como subcolumna:
...
* `false` — devolver `NULL` para las filas incompatibles y conservar el resultado para las filas compatibles.
...
<div id="using-dynamic-type-in-order-by-and-group-by">
  ## Uso de Dynamic en ORDER BY y GROUP BY
</div>

Durante `ORDER BY` y `GROUP BY`, los valores del tipo `Dynamic` se comparan de forma similar a los valores del tipo `Variant`:
...
De forma predeterminada, el tipo `Dynamic` no se permite en las claves de `GROUP BY`/`ORDER BY`; si desea usarlo, tenga en cuenta su regla especial de comparación y habilite las opciones `allow_suspicious_types_in_group_by`/`allow_suspicious_types_in_order_by`.
...
**Nota:** los valores de tipo Dynamic con distintos tipos numéricos se consideran valores diferentes y no se comparan entre sí; en su lugar, se comparan sus nombres de tipo.
...
**Nota:** la regla de comparación descrita no se aplica al ejecutar funciones de comparación como `<`/`>`/`=` y otras, debido al [funcionamiento especial](#using-dynamic-type-in-functions) de las funciones con el tipo `Dynamic`

<div id="reaching-the-limit-in-number-of-different-data-types-stored-inside-dynamic">
  ## Al alcanzar el límite del número de tipos de datos diferentes almacenados en Dynamic
</div>

El tipo de dato `Dynamic` solo puede almacenar un número limitado de tipos de datos diferentes como subcolumnas separadas. De forma predeterminada, este límite es de 32, pero puede cambiarlo en la declaración del tipo mediante la sintaxis `Dynamic(max_types=N)`, donde N debe estar entre 0 y 254 (debido a detalles de implementación, es imposible tener más de 254 tipos de datos diferentes que puedan almacenarse como subcolumnas separadas dentro de Dynamic).
Cuando se alcanza este límite, todos los nuevos tipos de datos insertados en la columna `Dynamic` se insertarán en una única estructura de datos compartida que almacena valores de distintos tipos de datos en forma binaria.
...
Durante el análisis de valores `Dynamic` a partir de los datos, cuando se alcanza el límite del bloque de datos actual, todos los valores nuevos se insertarán en la estructura de datos compartida:
...
Como podemos ver, después de insertar 3 tipos de datos distintos, `Int64`, `Array(Int64)` y `String`, todos los tipos nuevos se insertaron en una estructura especial de datos compartida.
...
Durante la fusión de varias partes de datos en una tabla MergeTree, la columna `Dynamic` de la parte de datos resultante puede alcanzar el límite de tipos de datos diferentes que se pueden almacenar en subcolumnas separadas en su interior y no podrá almacenar todos los tipos como subcolumnas procedentes de las partes de origen.
En este caso, ClickHouse elige qué tipos permanecerán como subcolumnas separadas después de la fusión y qué tipos se insertarán en la estructura de datos compartida. En la mayoría de los casos, ClickHouse intenta conservar los tipos más frecuentes y almacenar los menos frecuentes en la estructura de datos compartida, pero esto depende de la implementación.

Veamos un ejemplo de una fusión de este tipo. Primero, vamos a crear una tabla con una columna `Dynamic`, establecer el límite de tipos de datos diferentes en `3` e insertar valores con `5` tipos diferentes:
...
Cada inserción creará una parte de datos independiente con una columna `Dynamic` que contiene un único tipo:
...
Ahora, combinemos todas las partes en una sola y veamos qué ocurre:
...
En el formato RowBinary, los valores de tipo `Dynamic` se serializan en el siguiente formato:
