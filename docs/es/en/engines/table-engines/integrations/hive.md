---
description: 'El motor Hive le permite realizar consultas `SELECT` en tablas de
  Hive en HDFS.'
sidebar_label: 'Hive'
sidebar_position: 84
slug: /engines/table-engines/integrations/hive
title: 'Motor de tabla de Hive'
doc_type: 'guide'
---
...
* ORC: admite tipos de columnas escalares simples, excepto `char`; solo admite tipos complejos como `array`

* Parquet: admite todos los tipos de columnas escalares simples; solo admite tipos complejos como `array`
...
* Los nombres de las columnas deben ser los mismos que en la tabla de Hive original, pero puede usar solo algunas de ellas y en cualquier orden; además, puede usar algunas columnas de alias calculadas a partir de otras columnas.
* Los tipos de columna deben ser los mismos que los de la tabla de Hive original.
* La expresión `PARTITION BY` debe ser coherente con la tabla de Hive original, y las columnas de la expresión `PARTITION BY` deben estar en la estructura de la tabla.
...
Recomendamos encarecidamente habilitar la caché local para los sistemas de archivos remotos. Los benchmark muestran que con caché es casi 2 veces más rápido.
...
* bytes&#95;read&#95;before&#95;flush: Controla la cantidad de bytes antes de hacer flush al sistema de archivos local al descargar un archivo desde el sistema de archivos remoto. El valor predeterminado es 1 MB.
...
Tabla de ClickHouse que recupera datos de la tabla de Hive creada anteriormente:
...
tabla de ClickHouse para recuperar datos de la tabla de Hive creada anteriormente:
