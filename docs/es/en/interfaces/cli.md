...
* Iniciar y gestionar servidores ClickHouse locales
* Ejecutar y gestionar instancias locales de Postgres
* Ejecutar consultas en servidores ClickHouse
* Configurar ClickHouse Cloud y crear clústeres de ClickHouse gestionados en la nube
* Crear y gestionar servicios Postgres de ClickHouse Cloud
...
* Instalar las ClickHouse agent skills oficiales en agentes de código compatibles
...
El script de instalación descarga la versión adecuada para tu sistema operativo y la instala en `~/.local/bin/clickhousectl`. También se crea automáticamente el alias `chctl` para mayor comodidad.
...
* Los comandos de Cloud requieren una [API key de ClickHouse Cloud](/es/cloud/manage/api/api-overview)
...
`clickhousectl` descarga los binarios de ClickHouse desde `builds.clickhouse.com` y, si allí no hay una compilación disponible, recurre a `packages.clickhouse.com` (Linux) o a las [versiones publicadas en GitHub](https://github.com/ClickHouse/ClickHouse/releases) (macOS).
...
`local use` también crea un enlace simbólico en `~/.local/bin/clickhouse` que apunta al binario de la versión seleccionada, de modo que el comando `clickhouse` sin más (por ejemplo, `clickhouse local`, `clickhouse client`) esté en `PATH`. Pasa `--no-global` para omitirlo. Si ya existe un archivo normal en esa ruta, se deja intacto y se muestra una advertencia. `local remove` de la versión predeterminada activa también elimina el enlace simbólico.

<div id="binary-storage">
  #### Almacenamiento de los binarios de ClickHouse
</div>

Los binarios de ClickHouse se almacenan en un repositorio global, por lo que pueden usarse en varios proyectos sin duplicar el espacio de almacenamiento. Los binarios se almacenan en `~/.clickhouse/`:
...
`init` inicializa tu directorio de trabajo actual con una estructura de carpetas estándar para los archivos de tu proyecto de ClickHouse y Postgres. Es opcional; puedes usar tu propia estructura de carpetas si lo prefieres.
...
Inicie y gestione instancias del servidor de ClickHouse. Cada servidor tiene su propio directorio de datos aislado en `.clickhouse/servers/<name>/data/`.
...
**Nombres de los servidores:** Sin `--name`, el primer servidor se llama &quot;default&quot;. Si &quot;default&quot; ya está en ejecución, se genera un nombre aleatorio (p. ej., &quot;bold-crane&quot;). Use `--name` para tener identidades estables que pueda iniciar y detener repetidamente.

**Puertos:** Los puertos predeterminados son HTTP 8123 y TCP 9000. Si ya están en uso, se asignan automáticamente puertos libres y se muestran en la salida. Use `--http-port` y `--tcp-port` para establecer puertos explícitos.
...
Los servidores locales se inician con valores predeterminados adecuados, pero a veces necesitas cambiar algún ajuste. Coloca un archivo de configuración en `~/.clickhouse/configs/` y aplícalo por nombre al iniciar el servidor:
...
Cada servidor con nombre tiene su propio directorio de datos, por lo que los servidores están completamente aislados entre sí. Los datos se conservan entre reinicios. Detén e inicia un servidor por su nombre para continuar donde lo dejaste. Usa `clickhousectl local server remove <name>` para eliminar permanentemente los datos de un servidor.

<div id="local-postgres">
  ### Ejecutar Postgres local
</div>

Además de ClickHouse, `clickhousectl` puede ejecutar y gestionar instancias locales de Postgres. Postgres local se ejecuta sobre Docker, por lo que Docker debe estar instalado y en funcionamiento. Cada instancia se identifica por su nombre y versión principal, por lo que varias versiones de Postgres pueden ejecutarse en paralelo con directorios de datos independientes.
...
Autentícate en ClickHouse Cloud con claves de API (recomendado) o con OAuth (desde el navegador).

Si todavía no tienes una cuenta de ClickHouse Cloud, `clickhousectl cloud auth signup` abre la página de registro en tu navegador.

<div id="api-key">
  ### API key/secreto de API (recomendado)
</div>

Las API keys son la forma recomendada de autenticarse, especialmente al usar la CLI desde un agente de IA. Puede [crear API keys con alcance específico](/es/cloud/manage/openapi) que otorguen solo los permisos que elija (solo lectura o lectura/escritura), y cada clave está vinculada a una única organización. Esto hace que sea una forma segura y de mínimo privilegio de dar acceso a la CLI.
...
Las credenciales se guardan en `.clickhouse/credentials.json` (a nivel de proyecto).
...
O bien, en un archivo `.env` ubicado en el directorio de trabajo actual:
...
O pase las credenciales directamente mediante indicadores en cualquier comando:
...
Esto abre tu navegador para autenticarte mediante el flujo de dispositivos de OAuth. Los tokens se guardan en `.clickhouse/tokens.json` (del proyecto).

:::note
El acceso con OAuth es actualmente de **solo lectura** y da acceso a **todas las organizaciones a las que perteneces**. Si necesitas acceso de escritura o limitar la CLI a una sola organización, [crea una API key con alcance específico](#api-key).
:::
...
Orden de prioridad en la resolución de credenciales: indicadores de CLI &gt; `.clickhouse/credentials.json` &gt; variables de entorno exportadas &gt; archivo `.env` &gt; tokens de OAuth.

<div id="debug-credentials">
  ### Depuración de la fuente de credenciales utilizada
</div>

Pasa `--debug` a cualquier comando `cloud` para imprimir en stderr la fuente de credenciales resuelta (y la URL de la API) antes de que se ejecute el comando.
...
Administre los servicios de ClickHouse Cloud mediante la API.
...
| `--provider`                               | Proveedor de Cloud: `aws`, `gcp`, `azure` (predeterminado: `aws`)            |
...
| `--idle-timeout-minutes`                   | Tiempo mínimo de inactividad antes de entrar en espera, en minutos (&gt;= 5) |
| `--ip-allow`                               | CIDR de IP permitido (se puede repetir, predeterminado: `0.0.0.0/0`)         |
| `--backup-id`                              | ID de la copia de seguridad desde la que restaurar                           |
| `--release-channel`                        | Canal de versiones: `slow`, `default`, `fast`                                |
...
| `--tag`                                    | Añadir una etiqueta de servicio GA (`key` o `key=value`)                     |
...
| `--enable-core-dumps`                      | Habilitar o deshabilitar la recopilación de core dumps del servicio          |

<div id="query-api-auth-modes">
  #### Modos de autenticación de Query API
</div>

`cloud service query` es la forma canónica de ejecutar SQL en un servicio de Cloud a través de HTTP, sin el binario `clickhouse` y sin necesidad de la contraseña del servicio. Funciona con ambos modos de credenciales:

* **Autenticación con API key** (SQL de lectura y escritura): la primera vez que `cloud service query` se ejecuta en un servicio sin una clave almacenada, aprovisiona un endpoint de Query API para ese servicio y crea una API key dedicada asociada a él. La clave (`keyId`, `keySecret` y `endpointId`) se almacena en `.clickhouse/credentials.json`, en `service_query_keys.<service-id>`. La clave queda limitada a un único servicio, por lo que puede leer y escribir (SELECT, INSERT, DDL) en ese servicio, pero no puede acceder a ningún otro servicio de la org. Pase `--no-auto-enable` para que falle en lugar de aprovisionar.
* **OAuth** (`cloud auth login`): la consulta se ejecuta con su propia identidad, igual que en la consola web de SQL. Sus permisos de SQL en el servicio son de **solo lectura** cuando usa OAuth. No se aprovisiona ni se almacena ninguna clave de Query API. `--no-auto-enable` no tiene efecto en este modo.

Consultar un servicio **inactivo** lo reactiva automáticamente en ambos modos de autenticación (la primera consulta puede tardar un minuto). Un servicio **detenido** nunca se reactiva: la consulta falla con una sugerencia para ejecutar `cloud service start`. Configure `CLICKHOUSE_CLOUD_QUERY_HOST` para sobrescribir el host de Query API derivado.

<div id="query-endpoints">
  #### Gestión de endpoints para consultas
</div>
...
`clickhousectl` también puede crear y administrar servicios de [ClickHouse Cloud Postgres](/es/cloud/managed-postgres), siguiendo el mismo esquema que los comandos del servicio de ClickHouse anteriores.
...
<div id="postgres-create-options">
  #### Opciones de creación del servicio de Postgres
</div>
...
| `--provider`               | Proveedor de Cloud (predeterminado: `aws`)                 |
...
| `--pg-config-file`         | Ruta de un archivo JSON con un objeto `PgConfig`           |
| `--pg-bouncer-config-file` | Ruta de un archivo JSON con un objeto `PgBouncerConfig`    |
...
Cada tipo de origen tiene su propio subcomando dentro de `clickpipe create`:
...
Usa `clickhousectl cloud clickpipe create <source> --help` para obtener la lista completa de opciones para cada tipo de fuente.
...
Usa el indicador `--json` para imprimir respuestas en formato JSON.
...
`clickhousectl` detecta automáticamente contextos de agentes de código (Claude Code, Cursor, Codex, Gemini CLI, Goose, Devin y cualquier herramienta que establezca la variable de entorno estándar `AGENT`) y envía JSON a stdout automáticamente sin necesidad de indicar `--json`.
...
| `2`    | Cancelado (el usuario abortó la operación)                                       |
...
<div id="non-interactive-flags">
  ### Indicadores para modo no interactivo
</div>

| Indicador         | Descripción                                                          |
| ----------------- | -------------------------------------------------------------------- |
| `--agent <name>`  | Instala Skills para un agente específico (puede repetirse)           |
| `--global`        | Usa el ámbito global; si se omite, se usa el ámbito del proyecto     |
...
La CLI también comprueba si hay actualizaciones en segundo plano (como máximo una vez cada 24 horas) y muestra un aviso cuando hay una versión más reciente disponible.