---
alias: []
description: 'Documentación del formato JSONCompactColumns'
input_format: true
...
En este formato, todos los datos se representan como un único array JSON.

:::note
El formato de salida `JSONCompactColumns` almacena en búfer todos los datos en memoria para generarlos como un único bloque, lo que puede provocar un alto consumo de memoria.
:::
...
Con un archivo JSON llamado `football.json` que contiene los siguientes datos:
...
Las columnas que no estén presentes en el bloque se rellenarán con sus valores predeterminados (aquí puede usar la opción [`input_format_defaults_for_omitted_fields`](/es/operations/settings/settings-formats.md/#input_format_defaults_for_omitted_fields))
