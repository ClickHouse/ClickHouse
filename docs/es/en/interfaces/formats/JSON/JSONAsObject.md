...
En este formato, un único objeto JSON se interpreta como un solo valor [JSON](/es/sql-reference/data-types/newjson.md). Si la entrada contiene varios objetos JSON (separados por comas), se interpretan como filas independientes. Si los datos de entrada están entre `[]`, se interpretan como un array de JSON.

Este formato solo puede analizarse para una tabla con un único campo de tipo [JSON](/es/sql-reference/data-types/newjson.md). Las columnas restantes deben establecerse como [`DEFAULT`](/es/sql-reference/statements/create/table.md/#default) o [`MATERIALIZED`](/es/sql-reference/statements/create/view#materialized-view).
