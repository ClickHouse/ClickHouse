---
description: 'Control del comportamiento cuando hay sobrecarga de CPU en el servidor.'
sidebar_label: 'Sobrecarga del servidor'
...
ClickHouse server calcula la relación entre el tiempo de espera de la CPU (métrica `OSCPUWaitMicroseconds`) y el tiempo de actividad
(métrica `OSCPUVirtualTimeMicroseconds`). Cuando el servidor supera una determinada proporción de sobrecarga,
puede ser conveniente descartar algunas consultas o incluso rechazar solicitudes de conexión para no aumentar aún más la carga.

Hay una configuración del servidor, `os_cpu_busy_time_threshold`, que controla el tiempo mínimo de actividad necesario para considerar que la CPU
...
`max_os_cpu_wait_time_ratio_to_throw`. Si esos ajustes están establecidos y `min_os_cpu_wait_time_ratio_to_throw` es menor
que `max_os_cpu_wait_time_ratio_to_throw`, la consulta se rechaza y se produce el error `SERVER_OVERLOADED`
...
El rechazo de conexiones se controla mediante los ajustes a nivel de servidor `min_os_cpu_wait_time_ratio_to_drop_connection` y
`max_os_cpu_wait_time_ratio_to_drop_connection`. Estos ajustes pueden cambiarse sin reiniciar el servidor. La idea de
estos ajustes es similar a la del rechazo de consultas. La única diferencia en este caso es que, si el servidor está sobrecargado,
el intento de conexión será rechazado por parte del servidor.
...
personalizar estos umbrales mediante la configuración del servidor.
