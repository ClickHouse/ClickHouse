...
description: 'Tabla del sistema que contiene información útil para obtener una visión general del uso de memoria
  y de los ProfileEvents de los usuarios.'
...
* `user` ([String](/es/sql-reference/data-types/string)) — Nombre de usuario.
* `memory_usage` ([Int64](/es/sql-reference/data-types/int-uint)) — Suma de la RAM utilizada por todos los procesos del usuario. Puede que no incluya algunos tipos de memoria dedicada. Consulte la configuración max&#95;memory&#95;usage.
* `peak_memory_usage` ([Int64](/es/sql-reference/data-types/int-uint)) — Pico de uso de memoria del usuario. Puede restablecerse cuando el usuario no ejecuta consultas.
* `ProfileEvents` ([Map(LowCardinality(String), UInt64)](/es/sql-reference/data-types/map)) — Resumen de los ProfileEvents que miden distintas métricas del usuario. Su descripción puede encontrarse en la tabla system.events
