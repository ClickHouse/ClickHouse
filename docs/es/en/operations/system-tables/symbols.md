...
description: 'Tabla del sistema útil para expertos en C++ e ingenieros de ClickHouse, que contiene
  información para la introspección del binario `clickhouse`.'
...
Contiene información para la introspección del binario `clickhouse`. Para acceder, se requiere el privilegio de introspección.
Esta tabla solo es útil para expertos en C++ y para los ingenieros de ClickHouse.
...
* `symbol` ([String](/es/sql-reference/data-types/string)) — Nombre del símbolo en el binario. Está mangleado. Puede aplicar demangle(symbol) para obtener un nombre legible.
* `symbol_demangled` ([Nullable(String)](/es/sql-reference/data-types/nullable)) — Símbolo desmangleado utilizado para la instrumentación de XRay.
* `function_id` ([Nullable(Int32)](/es/sql-reference/data-types/nullable)) — ID de la función en el mapa de instrumentación de XRay.
* `address_begin` ([UInt64](/es/sql-reference/data-types/int-uint)) — Dirección de inicio del símbolo en el binario.
* `address_end` ([UInt64](/es/sql-reference/data-types/int-uint)) — Dirección final del símbolo en el binario.
