---
description: 'Tabla del sistema que contiene las trazas de pila de todos los hilos
  del servidor. Permite a los desarrolladores inspeccionar el estado del servidor.'
...
Contiene las trazas de pila de todos los hilos del servidor. Permite a los desarrolladores inspeccionar el estado interno del servidor.

Para analizar los frames de la pila, use `addressToLine`, `addressToLineWithInlines`, `addressToSymbol` y `demangle` [funciones de introspección](../../sql-reference/functions/introspection.md).
...
* `thread_name` ([String](/es/sql-reference/data-types/string)) — El nombre del hilo.
* `thread_id` ([UInt64](/es/sql-reference/data-types/int-uint)) — El identificador del hilo
* `query_id` ([String](/es/sql-reference/data-types/string)) — El ID de la consulta a la que pertenece este hilo.
* `trace` ([Array(UInt64)](/es/sql-reference/data-types/array)) — El stacktrace de este hilo. Básicamente, es solo un array de direcciones.
* `untracked_memory` ([Int64](/es/sql-reference/data-types/int-uint)) — Contador por hilo sin atomicidad de asignaciones de memoria que aún no se han propagado al MemoryTracker padre. Puede ser negativo si, desde el último vaciado, se ha liberado más memoria de la que se ha asignado.
...
Obtención de símbolos de los archivos objeto de ClickHouse:
...
* [Funciones de introspección](../../sql-reference/functions/introspection.md) — Qué funciones de introspección están disponibles y cómo utilizarlas.
* [system.trace&#95;log](../system-tables/trace_log.md) — Contiene trazas de pila recopiladas por el perfilador de consultas por muestreo.
* [arrayMap](/es/sql-reference/functions/array-functions#arrayMap)) — Descripción y ejemplo de uso de la función `arrayMap`.
* [arrayFilter](/es/sql-reference/functions/array-functions#arrayFilter) — Descripción y ejemplo de uso de la función `arrayFilter`.