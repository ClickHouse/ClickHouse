...
sidebar_label: 'utilidad clickhouse-keeper-client'
...
* `-q QUERY`, `--query=QUERY` — Consulta que se va a ejecutar. Si no se proporciona este parámetro, `clickhouse-keeper-client` se iniciará en modo interactivo.
...
* `--connection-timeout=TIMEOUT` — Establece el tiempo de espera de conexión en segundos. Valor predeterminado: 10s.
...
* `--log-level=LEVEL` — Establece el nivel de log. Valor predeterminado: `information`.
* `--no-confirmation` — Si se establece, no requerirá confirmación en varios comandos. Valor predeterminado: `false` en modo interactivo y `true` para consulta
...
* `CLICKHOUSE_KEEPER_PASSWORD` — Se usa como contraseña predeterminada si no se especifica `--password` en la línea de comandos.
* `CLICKHOUSE_KEEPER_IDENTITY` — Se usa como identidad predeterminada si no se especifica `--identity` en la línea de comandos.

<div id="clickhouse-keeper-client-auth">
  ## Autenticación
</div>

Al conectarse a un servidor Keeper que requiere autenticación, la contraseña se determina según el siguiente orden de prioridad (se usa la primera coincidencia):

1. argumento de línea de comandos `--password`
2. variable de entorno `CLICKHOUSE_KEEPER_PASSWORD`
3. `<zookeeper><password>` en el archivo XML de configuración especificado por `--config-file`

La misma prioridad se aplica a `--identity` / `CLICKHOUSE_KEEPER_IDENTITY` / `<zookeeper><identity>`.

Ejemplo de archivo XML de configuración con opciones de autenticación:
...
* `cp '<src>' '<dest>'`  -- Copia el nodo &#39;src&#39; a la ruta &#39;dest&#39;
* `cpr '<src>' '<dest>'`  -- Copia el subárbol del nodo &#39;src&#39; a la ruta &#39;dest&#39;
...
* `set '<path>' <value> [version]` -- Actualiza el valor del nodo. Solo se actualiza si la versión coincide (por defecto: -1)
* `create '<path>' <value> [mode]` -- Crea un nodo nuevo con el valor especificado
* `touch '<path>'` -- Crea un nodo nuevo con una cadena vacía como valor. No lanza una excepción si el nodo ya existe
* `get '<path>' [watch_id]` -- Devuelve el valor del nodo. Opcionalmente, establece un watch de datos identificado por `watch_id`
* `watch <watch_id> [timeout_seconds]` -- Espera el evento de watch identificado por `watch_id` e imprime el tipo de evento y la ruta. Si se especifica `timeout_seconds`, devuelve un error cuando se alcanza el tiempo de espera indicado
...
* `get_direct_children_number '[path]'` -- Obtiene el número de nodos hijo directos bajo una ruta específica
* `get_all_children_number '[path]'` -- Obtiene el número total de nodos hijo bajo una ruta específica
* `get_stat '[path]'` -- Devuelve las estadísticas del nodo (por defecto `.`)
...
* `find_big_family [path] [n]` -- Devuelve los n nodos con la familia más grande del subárbol (ruta por defecto = `.` y n = 10)
* `sync '<path>'` -- Sincroniza el nodo entre procesos y el leader
* `reconfig <add|remove|set> "<arg>" [version]` -- Reconfigura el Keeper cluster. Consulta /docs/en/guides/sre/keeper/clickhouse-keeper#reconfiguration