...
* **Paralelismo de inserción** - El número de hilos de inserción utilizados para insertar datos. Se controla mediante [`max_insert_threads`](/es/reference/settings/session-settings/max-insert#max_insert_threads). **Nota**: este valor está limitado por `max_threads`, por lo que el paralelismo de inserción efectivo es `min(max_insert_threads, max_threads)`. En ClickHouse Cloud, esto viene determinado por el tamaño de la instancia (entre 2 y 4) y, en OSS, se establece en 1. Aumentar este valor puede mejorar el rendimiento a costa de un mayor uso de memoria.
