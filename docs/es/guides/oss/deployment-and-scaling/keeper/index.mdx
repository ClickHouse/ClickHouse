...
<div id="keeper-configuration-settings">
  #### Parámetros de configuración de Keeper {#keeper-configuration-settings}
</div>
...
| `log_readahead_commit_window_bytes`       | Tamaño total máximo de las entradas de log decodificadas almacenadas temporalmente en el búfer antes del hilo de commit. `0` deshabilita la lectura anticipada de commit (el commit lee las entradas del disco una por una).                                                                                                                                                                                          | `500MiB`             |
...
* `ydld`: Solicita ceder el liderazgo y convertirse en seguidor. Si el servidor que recibe la solicitud es el líder, primero pausará las operaciones de escritura, esperará a que el sucesor (el líder actual nunca puede ser el sucesor) termine de ponerse al día con el último log y, a continuación, renunciará. El sucesor se elegirá automáticamente. Devuelve `Sent yield leadership request to leader.` si se envía la solicitud o `Failed to send yield leadership request to leader.` si no se envía. Si el nodo ya es seguidor, el resultado es el mismo que si se envía la solicitud.

```response
Sent yield leadership request to leader.
```

* `pfev`: Devuelve los valores de todos los eventos recopilados. Para cada evento, devuelve su nombre, valor y descripción.
...
<div id="migration-from-zookeeper">
  ### Migración desde ZooKeeper {#migration-from-zookeeper}
</div>
...
<div id="pre-migration-preparation">
  #### Preparación previa a la migración {#pre-migration-preparation}
</div>
...
<div id="consolidating-multiple-zookeeper-clusters">
  #### Consolidación de varios clústeres de ZooKeeper {#consolidating-multiple-zookeeper-clusters}
</div>
...
<div id="handling-encryption-and-acls">
  #### Gestión del cifrado y las ACL {#handling-encryption-and-acls}
</div>
...
<div id="verifying-the-migration">
  #### Verificación de la migración {#verifying-the-migration}
</div>
...
<div id="post-migration-tuning">
  #### Ajustes posteriores a la migración {#post-migration-tuning}
</div>
...
<div id="recovering-after-losing-quorum">
  ### Recuperación tras perder el quórum {#recovering-after-losing-quorum}
</div>
...
<div id="using-disks-with-keeper">
  ## Uso de discos con Keeper {#using-disks-with-keeper}
</div>
...
<div id="changing-disk-setup">
  ### Cambio de la configuración de discos {#changing-disk-setup}
</div>
...
## Configuración de la caché de logs {#configuring-logs-cache}

Para minimizar la cantidad de datos leídos del disco, Keeper almacena en caché las entradas de log en memoria.
Si las solicitudes son grandes, las entradas de log consumirán demasiada memoria, por lo que se limita la cantidad de logs almacenados en caché.
El límite de la caché de los logs más recientes se controla mediante:

* `latest_logs_cache_size_threshold` - tamaño total de los logs más recientes almacenados en caché

Si el valor predeterminado es demasiado alto, puede reducir el uso de memoria disminuyendo esta configuración.

Las entradas de log necesarias para el siguiente commit se sirven mediante un lector de lectura anticipada decodificado, cuyo tamaño se define mediante
...
<Note>
  Puede usar el comando `pfev` para comprobar la cantidad de logs leídos de cada caché y de un archivo.
...
<div id="clickhouse-keeper-user-guide">
  ## Guía de usuario de ClickHouse Keeper {#clickhouse-keeper-user-guide}
</div>
...
<div id="configuring-clickhouse-keeper-with-unique-paths">
  ## Configuración de ClickHouse Keeper con rutas únicas {#configuring-clickhouse-keeper-with-unique-paths}
</div>
...
<div id="example-environment">
  ### Entorno de ejemplo {#example-environment}
</div>
...
<div id="procedures-to-set-up-tables-to-use-uuid">
  ### Procedimiento para configurar tablas para usar `{uuid}` {#procedures-to-set-up-tables-to-use-uuid}
</div>
...
<div id="reconfiguration">
  ## Reconfiguración dinámica de ClickHouse Keeper {#reconfiguration}
</div>
...
<div id="converting-a-single-node-keeper-into-a-cluster">
  ## Convertir un keeper de un solo nodo en un clúster {#converting-a-single-node-keeper-into-a-cluster}
</div>
...
<div id="unsupported-features">
  ## Funciones no compatibles {#unsupported-features}
</div>
