...
Utiliza la variante S3 para AWS S3 y GCS. Azure y el sistema de archivos local tienen variantes específicas (`icebergAzure`, `icebergLocal` y sus equivalentes para otros formatos). Consulta [Consultar directamente](/es/guides/use-cases/data-warehousing/getting-started/querying-directly) para ver la lista completa.

[Paimon](/es/reference/functions/table-functions/paimon) solo admite funciones de tabla.
...
Esta opción escala mejor que crear definiciones de tablas individuales cuando administras muchas tablas o varios catálogos. Consulta [Conectarse a catálogos](/es/guides/use-cases/data-warehousing/getting-started/connecting-catalogs) y las [guías de catálogos](/es/guides/use-cases/data-warehousing/reference).
...
Para las operaciones de escritura, Iceberg requiere [allow&#95;insert&#95;into&#95;iceberg](/es/reference/settings/session-settings#allow_insert_into_iceberg) (25.7+, Beta desde 26.2). Consulta [Escritura en lagos de datos](/es/guides/use-cases/data-warehousing/getting-started/writing-data). Delta Lake requiere [allow&#95;delta&#95;lake&#95;writes](/es/reference/settings/session-settings#allow_experimental_delta_lake_writes) (25.9+). La [matriz de compatibilidad](/es/guides/use-cases/data-warehousing/support-matrix) indica qué flags se aplican a cada formato y operación.
...
En ClickHouse Cloud y en los servicios multinodo autogestionados, las variantes para clúster de las funciones de tabla de lake distribuyen las lecturas de archivos [Parquet](/es/reference/formats/Parquet/Parquet) entre las réplicas. El nodo iniciador reparte los archivos entre los workers en paralelo. Use las variantes para clúster para lecturas por lotes y cargas programadas sobre tablas grandes. En implementaciones de un solo nodo, la función de tabla estándar es suficiente.
...
* Para Iceberg, lea una vista en un momento concreto con [iceberg&#95;snapshot&#95;id](/es/reference/settings/session-settings#iceberg_snapshot_id) o [iceberg&#95;timestamp&#95;ms](/es/reference/settings/session-settings#iceberg_timestamp_ms) (25.4+). En tablas de solo anexado, combine la configuración de instantáneas con filtros de partición en `WHERE`. Use [system.iceberg&#95;history](/es/reference/system-tables/iceberg_history) (25.6+) para buscar IDs de instantánea entre ejecuciones.
* Para Delta Lake, lea los cambios entre dos versiones con [delta&#95;lake&#95;snapshot&#95;start&#95;version](/es/reference/settings/session-settings#delta_lake_snapshot_start_version) y [delta&#95;lake&#95;snapshot&#95;end&#95;version](/es/reference/settings/session-settings#delta_lake_snapshot_end_version) (25.12+). Lea una sola instantánea con [delta&#95;lake&#95;snapshot&#95;version](/es/reference/settings/session-settings#delta_lake_snapshot_version) (25.8+). Consulte [Delta change data feed](#delta-incremental-sync) para ver un ejemplo de CDF.

<div id="filesystem-cache">
  ### Almacenar archivos Parquet en caché localmente
</div>

Ambos formatos respetan [enable&#95;filesystem&#95;cache](/es/reference/settings/session-settings#enable_filesystem_cache) para mantener en el disco local los archivos [Parquet](/es/reference/formats/Parquet/Parquet) de uso frecuente entre consultas. En implementaciones autogestionadas, configure un [disco de caché del sistema de archivos](/es/concepts/features/configuration/server-config/storing-data#using-local-cache) en la configuración del servidor para que este ajuste tenga almacenamiento donde escribir. ClickHouse Cloud gestiona el almacenamiento en caché automáticamente. Establezca `enable_filesystem_cache = 0` al realizar benchmarking para que los aciertos de la caché no oculten los cambios entre ejecuciones.
...
| ------------------------------------------------------------------------------------------------------------------ | ----- | -------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| [use&#95;iceberg&#95;partition&#95;pruning](/es/reference/settings/session-settings#use_iceberg_partition_pruning)           | 25.1  | `1` desde 25.6 | Omite archivos de datos mediante metadatos de partición en los manifiestos                                                                   |
| [use&#95;iceberg&#95;metadata&#95;files&#95;cache](/es/reference/settings/session-settings#use_iceberg_metadata_files_cache) | 25.4  | `1`            | Almacena en caché en memoria las listas de manifiestos y el JSON de metadatos                                                                |
| [iceberg&#95;metadata&#95;staleness&#95;ms](/es/reference/settings/session-settings#iceberg_metadata_staleness_ms)           | 26.3  | `0`            | Configuración de consulta. Usa metadatos en caché cuando son más recientes que esta ventana, en lugar de llamar al catálogo en cada consulta |
...
2. Define [iceberg&#95;metadata&#95;staleness&#95;ms](/es/reference/settings/session-settings#iceberg_metadata_staleness_ms) (26.3+) en las consultas para aceptar metadatos ligeramente desactualizados a cambio de evitar la ida y vuelta al catálogo.
...
Además de [allow&#95;insert&#95;into&#95;iceberg](/es/reference/settings/session-settings#allow_insert_into_iceberg) (25.7+, Beta desde 26.2), puede controlar el tamaño del archivo de salida y el número de particiones durante la inserción:

| Configuración                                                                                                                        | Desde | Propósito                                            |
| ------------------------------------------------------------------------------------------------------------------------------------ | ----- | ---------------------------------------------------- |
| [iceberg&#95;insert&#95;max&#95;rows&#95;in&#95;data&#95;file](/es/reference/settings/session-settings#iceberg_insert_max_rows_in_data_file)   | 25.9  | Límite de filas por archivo de datos de salida       |
| [iceberg&#95;insert&#95;max&#95;bytes&#95;in&#95;data&#95;file](/es/reference/settings/session-settings#iceberg_insert_max_bytes_in_data_file) | 25.9  | Límite de bytes por archivo de datos de salida       |
| [iceberg&#95;insert&#95;max&#95;partitions](/es/reference/settings/session-settings#iceberg_insert_max_partitions)                             | 25.12 | Límite de particiones escritas en una sola inserción |
...
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- | -------------- | ----------------------------------------------------------------------------------------------------- |
| [delta&#95;lake&#95;enable&#95;engine&#95;predicate](/es/reference/settings/session-settings#delta_lake_enable_engine_predicate)                                                                                                                  | 25.8  | `1`            | Pasa los filtros al kernel para la poda de particiones. Requiere [Delta Kernel](#delta-kernel)        |
| [delta&#95;lake&#95;reload&#95;schema&#95;for&#95;consistency](/es/reference/settings/session-settings#delta_lake_reload_schema_for_consistency)                                                                                                  | 26.3  | `0`            | Recarga el esquema antes de cada consulta cuando escritores concurrentes hacen evolucionar el esquema |
| [delta&#95;lake&#95;snapshot&#95;start&#95;version](/es/reference/settings/session-settings#delta_lake_snapshot_start_version) / [delta&#95;lake&#95;snapshot&#95;end&#95;version](/es/reference/settings/session-settings#delta_lake_snapshot_end_version) | 25.12 | `-1`           | Lee cambios de CDF entre dos versiones de instantánea. Requiere que CDF esté habilitado en upstream   |
| [delta&#95;lake&#95;snapshot&#95;version](/es/reference/settings/session-settings#delta_lake_snapshot_version)                                                                                                                                    | 25.8  | `-1`           | Lee una única instantánea histórica. Establece `-1` para la más reciente (`0` es válido)              |
...
Para leer solo las filas que cambiaron entre dos snapshots de Delta, establezca [delta&#95;lake&#95;snapshot&#95;start&#95;version](/es/reference/settings/session-settings#delta_lake_snapshot_start_version) y [delta&#95;lake&#95;snapshot&#95;end&#95;version](/es/reference/settings/session-settings#delta_lake_snapshot_end_version) (25.12+). La tabla debe tener activada en origen la fuente de cambios de datos (`delta.enableChangeDataFeed`). Establezca tanto la versión inicial como la versión final en los ajustes de consulta. Establecer solo la versión final produce un error.
...
Además de [allow&#95;delta&#95;lake&#95;writes](/es/reference/settings/session-settings#allow_experimental_delta_lake_writes) (25.9+), controle el tamaño del archivo de salida al insertar:

| Configuración                                                                                                                                  | Desde | Propósito                                      |
| ---------------------------------------------------------------------------------------------------------------------------------------------- | ----- | ---------------------------------------------- |
| [delta&#95;lake&#95;insert&#95;max&#95;rows&#95;in&#95;data&#95;file](/es/reference/settings/session-settings#delta_lake_insert_max_rows_in_data_file)   | 25.9  | Límite de filas por archivo de datos de salida |
| [delta&#95;lake&#95;insert&#95;max&#95;bytes&#95;in&#95;data&#95;file](/es/reference/settings/session-settings#delta_lake_insert_max_bytes_in_data_file) | 25.9  | Límite de bytes por archivo de datos de salida |
...
Si las tablas del catálogo no aparecen en `system.tables`, habilite [show&#95;remote&#95;databases&#95;in&#95;system&#95;tables](/es/reference/settings/session-settings#show_remote_databases_in_system_tables) (25.8+). De forma predeterminada, las tablas del catálogo están ocultas para la introspección del sistema. En las versiones anteriores a 26.6, use su nombre anterior, `show_data_lake_catalogs_in_system_tables`.
...
Compare `read_rows` y `read_bytes` en [system.query&#95;log](/es/reference/system-tables/query_log) antes y después de añadir filtros o ajustar la configuración. ProfileEvents como `ReadBufferFromS3Bytes` y `CachedReadBufferReadFromCacheBytes` muestran cuántos datos provinieron del almacenamiento de objetos frente a la caché local. Consulte [Optimización de consultas](/es/guides/clickhouse/performance-and-monitoring/query-optimization) para obtener una guía completa sobre query&#95;log y EXPLAIN.

Desactive [enable&#95;filesystem&#95;cache](/es/reference/settings/session-settings#enable_filesystem_cache) durante el benchmark para que los aciertos de la caché no oculten los cambios entre ejecuciones.
...
| ----------------------------------------------------------------------------------------------- | ---------- | ----- | ----------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- |
| [system.iceberg&#95;metadata&#95;log](/es/reference/system-tables/iceberg_metadata_log)           | Iceberg    | 25.9  | [iceberg&#95;metadata&#95;log&#95;level](/es/reference/settings/session-settings#iceberg_metadata_log_level) en la consulta | Rastrear los archivos de metadatos leídos y las decisiones de poda de particiones |
| [system.iceberg&#95;history](/es/reference/system-tables/iceberg_history)                         | Iceberg    | 25.6  | Se pobla automáticamente para las tablas Iceberg de ClickHouse                                                    | Inspeccionar el linaje de snapshots antes de las consultas de viaje en el tiempo  |
| [system.delta&#95;lake&#95;metadata&#95;log](/es/reference/system-tables/delta_metadata_log) | Delta Lake | 25.10 | [delta&#95;lake&#95;log&#95;metadata](/es/reference/settings/session-settings#delta_lake_log_metadata) = `1` en la consulta | Rastrear los archivos de metadatos de Delta y la resolución de snapshots          |
...
* [Primeros pasos](/es/guides/use-cases/data-warehousing/getting-started/overview) — Guía completa desde la consulta directa hasta la escritura en destino
* [Consulta directa](/es/guides/use-cases/data-warehousing/getting-started/querying-directly) — Funciones de tabla, motores y variantes de clúster para los cuatro formatos
* [Conectarse a catálogos](/es/guides/use-cases/data-warehousing/getting-started/connecting-catalogs) — Configuración de `DataLakeCatalog` con Unity Catalog
* [Escribir en lagos de datos](/es/guides/use-cases/data-warehousing/getting-started/writing-data) — Escritura de datos en Iceberg y Delta Lake
* [Matriz de compatibilidad](/es/guides/use-cases/data-warehousing/support-matrix) — Comparación de características entre formatos, catálogos y backends de almacenamiento