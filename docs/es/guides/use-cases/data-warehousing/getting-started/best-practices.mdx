...
Para las operaciones de escritura, Iceberg requiere [allow&#95;insert&#95;into&#95;iceberg](/es/reference/settings/session-settings/allow#allow_insert_into_iceberg) (25.7+, Beta desde 26.2). Consulta [Escritura en lagos de datos](/es/use-cases/data-lake/getting-started/writing-data). Delta Lake requiere [allow&#95;delta&#95;lake&#95;writes](/es/reference/settings/session-settings/allow-experimental#allow_experimental_delta_lake_writes) (25.9+). La [matriz de compatibilidad](/es/use-cases/data-lake/support-matrix) indica qué flags se aplican a cada formato y operación.
...
* Para Iceberg, lea una vista en un momento concreto con [iceberg&#95;snapshot&#95;id](/es/reference/settings/session-settings/iceberg#iceberg_snapshot_id) o [iceberg&#95;timestamp&#95;ms](/es/reference/settings/session-settings/iceberg#iceberg_timestamp_ms) (25.4+). En tablas de solo anexado, combine la configuración de instantáneas con filtros de partición en `WHERE`. Use [system.iceberg&#95;history](/es/operations/system-tables/iceberg_history) (25.6+) para buscar IDs de instantánea entre ejecuciones.
* Para Delta Lake, lea los cambios entre dos versiones con [delta&#95;lake&#95;snapshot&#95;start&#95;version](/es/reference/settings/session-settings/delta-lake#delta_lake_snapshot_start_version) y [delta&#95;lake&#95;snapshot&#95;end&#95;version](/es/reference/settings/session-settings/delta-lake#delta_lake_snapshot_end_version) (25.12+). Lea una sola instantánea con [delta&#95;lake&#95;snapshot&#95;version](/es/reference/settings/session-settings/delta-lake#delta_lake_snapshot_version) (25.8+). Consulte [Delta change data feed](#delta-incremental-sync) para ver un ejemplo de CDF.
...
| [iceberg&#95;metadata&#95;staleness&#95;ms](/es/reference/settings/session-settings/iceberg-metadata#iceberg_metadata_staleness_ms)      | 26.3  | `0`            | Configuración de consulta. Usa metadatos en caché cuando son más recientes que esta ventana, en lugar de llamar al catálogo en cada consulta |
...
Desde la versión 25.6, ClickHouse lee Delta Lake en S3 y GCS mediante el kernel de Rust de Delta Lake ([allow&#95;experimental&#95;delta&#95;kernel&#95;rs](/es/reference/settings/session-settings/allow-experimental#allow_experimental_delta_kernel_rs), 25.5+). En Azure Blob Storage, use [deltaLakeAzure()](/es/sql-reference/table-functions/deltalake) con el lector heredado, ya que allí el kernel está deshabilitado. Sin el kernel, no están disponibles la poda de particiones, el flujo de cambios de datos ni la lectura de versiones de instantáneas.

<div id="delta-kernel">
  #### Delta Kernel
</div>

[allow&#95;experimental&#95;delta&#95;kernel&#95;rs](/es/reference/settings/session-settings/allow-experimental#allow_experimental_delta_kernel_rs) debe estar habilitado para la poda de particiones, el flujo de cambios de datos y la lectura de versiones de instantáneas. Está activado de forma predeterminada en S3 y GCS a partir de la versión 25.5. Habilítelo explícitamente en versiones anteriores o para la resolución de problemas:
...
Además de [allow&#95;delta&#95;lake&#95;writes](/es/reference/settings/session-settings/allow-experimental#allow_experimental_delta_lake_writes) (25.9+), controle el tamaño del archivo de salida al insertar:
...
Desactive [enable&#95;filesystem&#95;cache](/es/reference/settings/session-settings/enable-filesystem#enable_filesystem_cache) durante el benchmark para que los aciertos de la caché no oculten los cambios entre ejecuciones.
...
| ----------------------------------------------------------------------------------------------- | ---------- | ----- | ----------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- |
| [system.iceberg&#95;metadata&#95;log](/es/operations/system-tables/iceberg_metadata_log)           | Iceberg    | 25.9  | [iceberg&#95;metadata&#95;log&#95;level](/es/reference/settings/session-settings/iceberg-metadata#iceberg_metadata_log_level) en la consulta | Rastrear los archivos de metadatos leídos y las decisiones de poda de particiones |
| [system.iceberg&#95;history](/es/operations/system-tables/iceberg_history)                         | Iceberg    | 25.6  | Se pobla automáticamente para las tablas Iceberg de ClickHouse                                                                            | Inspeccionar el linaje de instantáneas antes de las consultas de viaje temporal   |
| [system.delta&#95;lake&#95;metadata&#95;log](/es/operations/system-tables/delta_lake_metadata_log) | Delta Lake | 25.10 | [delta&#95;lake&#95;log&#95;metadata](/es/reference/settings/session-settings/delta-lake#delta_lake_log_metadata) = `1` en la consulta       | Rastrear los archivos de metadatos de Delta y la resolución de instantáneas       |
