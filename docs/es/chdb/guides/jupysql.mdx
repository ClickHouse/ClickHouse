...
La base de datos predeterminada no conserva los datos en disco, por lo que primero debemos crear otra base de datos:
...
Usaremos la cláusula `REPLACE` para convertir a `Float64` las columnas relacionadas con importes monetarios y la función [`transform`](/es/sql-reference/functions/other-functions#transform) para convertir la columna numérica `pickup_borocode` en un nombre de distrito legible para humanos:
