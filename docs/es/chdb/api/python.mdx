...
ClickHouse integrado. Admite varios formatos de salida y puede trabajar con bases de datos temporales
...
| `path`          | str  | `""`           | Ruta del archivo de la base de datos. De forma predeterminada, se usa una base de datos temporal y no persistente (equivalente a `":memory:"`).<br />Proporcione una ruta de archivo para persistir los datos en disco                                                                                                                                  |
| `udf_path`      | str  | `""`           | Ruta al directorio de UDF heredadas basadas en subprocesos. No es necesaria para las UDF nativas de Python ([`@func`](#func-decorator) / [`create_function`](#create-function))                                                                                                                                                                         |
...
motor ClickHouse integrado. Admite varios formatos de salida y puede trabajar con bases de datos temporales
o basadas en archivos.
...
| `output_format` | str  | `"CSV"`        | Formato de salida de los resultados. Formatos compatibles:<br />• `"CSV"` - Valores separados por comas<br />• `"JSON"` - Formato JSON<br />• `"Arrow"` - Formato Apache Arrow<br />• `"Parquet"` - Formato Parquet<br />• `"DataFrame"` - DataFrame de Pandas<br />• `"ArrowTable"` - Tabla de PyArrow<br />• `"Debug"` - Activa el registro detallado |
| `path`          | str  | `""`           | Ruta del archivo de la base de datos. De forma predeterminada, usa una base de datos temporal no persistente (equivalente a `":memory:"`).<br />Proporcione una ruta de archivo para persistir en el disco                                                                                                                                              |
| `udf_path`      | str  | `""`           | Ruta al directorio de UDF heredadas basadas en subprocesos. No es necesaria para las UDF nativas de Python ([`@func`](#func-decorator) / [`create_function`](#create-function))                                                                                                                                                                         |
...
Cada llamada devuelve una conexión independiente, y puede haber abiertas al mismo tiempo cualquier cantidad de conexiones a la misma ruta de base de datos.
...
| Formato                   | Descripción                                |
| ------------------------- | ------------------------------------------ |
| `":memory:"`              | Base de datos temporal (predeterminada)    |
...
| `"file::memory:?verbose&log-level=test"`           | Temporal con parámetros      |
...
<Note>
  Un proceso ejecuta un motor integrado vinculado a una ruta de base de datos. Varias conexiones a la misma ruta pueden coexistir y ejecutar consultas simultáneamente. Abrir una ruta *diferente* mientras haya una conexión abierta genera un `RuntimeError`; primero cierra las conexiones abiertas.
</Note>
...
Si path es None, la sesión usa la base de datos temporal (`:memory:`) compartida por todo el proceso, por lo que todas las sesiones sin ruta pueden ver las tablas de las demás; su directorio temporal solo se elimina cuando se cierra la última sesión o conexión de este tipo.
...
| `"file::memory:?verbose&log-level=test"`           | Base de datos temporal con parámetros de consulta |
...
<Note>
  ...
  * Pueden coexistir varias sesiones, siempre que compartan la misma ruta de la base de datos.
  * Para usar una ruta de base de datos diferente, cierre primero las sesiones abiertas.
</Note>
...
| `udf_path` | str  | `""`           | Ruta al directorio de UDF heredadas basadas en subprocesos. No es necesaria para las UDF nativas de Python ([`@func`](#func-decorator) / [`create_function`](#create-function)). Si no se especifica, se usa la ruta de UDF de la inicialización de la sesión                                                                                                                |
...
<Warning>
  **Advertencia**
  ...
  memoria. Para conjuntos de resultados grandes, considera usar [`send_query()`](#chdb-session-session-send_query) para
...
<Warning>
  **Advertencia**
  ...
  Este método ejecuta la consulta de forma síncrona y carga todos los resultados en
  memoria.
  Para conjuntos de resultados grandes, considere usar [`send_query()`](#chdb-session-session-send_query) para transmitir el resultado en streaming.
</Warning>
...
Cada llamada devuelve una conexión independiente, y puede haber abiertas al mismo tiempo cualquier número de conexiones a la misma ruta de base de datos.
...
<Note>
  Un proceso ejecuta un motor integrado vinculado a una ruta de base de datos. Pueden coexistir varias conexiones a la misma ruta y ejecutar consultas simultáneamente. Abrir una ruta *diferente* mientras haya alguna conexión abierta genera un `RuntimeError`; primero cierre las conexiones abiertas.
</Note>
...
| Parámetro | Tipo | Predeterminado | Descripción                                                                               |
| --------- | ---- | -------------- | ----------------------------------------------------------------------------------------- |
| `path`    | str  | `None`         | Ruta del archivo de la base de datos. None para una base de datos temporal no persistente |
...
Esta clase proporciona una interfaz estándar de DB-API para conectarse a bases de datos chDB e interactuar con ellas. Admite tanto bases de datos temporales como bases de datos basadas en archivos.

La conexión administra el engine de chDB subyacente y proporciona métodos para
ejecutar consultas, gestionar transacciones (sin efecto en ClickHouse) y crear cursores.
...
| Parámetro | Tipo | Predeterminado | Descripción                                                                                                                                                                                                                                        |
| --------- | ---- | -------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `path`    | str  | `None`         | Ruta del archivo de la base de datos. Si es `None` (el valor predeterminado), se usa una base de datos temporal y no persistente (equivalente a `':memory:'`). Proporcione una ruta de archivo como &#39;database.db&#39; para persistir en disco. |
...
chDB admite UDF nativas de Python que se ejecutan en el mismo proceso, con argumentos tipados, inferencia automática de tipos y manejo configurable de NULL y excepciones. Las funciones de Python registradas como UDF pueden invocarse directamente desde consultas SQL.

Los ejemplos siguientes usan el formato de salida CSV predeterminado. Los comentarios en línea muestran los valores lógicos de los resultados; la salida sin procesar representa `NULL` como `\N` y aplica comillas de CSV a los valores de cadena y fecha.

<div id="create-function">
  ### `chdb.create_function`
</div>

Registra una función de Python como función SQL de chDB.
...
| Parámetro     | Tipo                              | Predeterminado    | Descripción                                                                                                                 |
| ------------- | --------------------------------- | ----------------- | --------------------------------------------------------------------------------------------------------------------------- |
| `name`        | str                               | *(obligatorio)*   | Nombre de la función SQL que se va a registrar                                                                              |
| `func`        | callable                          | *(obligatorio)*   | Función de Python que se va a registrar                                                                                     |
| `arg_types`   | list de ChdbType/str/type, o None | `None`            | Lista de tipos de argumentos. Si es `None`, se infieren a partir de las anotaciones de tipo                                 |
| `return_type` | ChdbType/str/type, o None         | `None`            | Tipo de retorno. Si es `None`, se infiere de la anotación de retorno de la función; el registro falla si esta también falta |
| `on_null`     | str o NullHandling                | `None` (omitir)   | Cómo manejar entradas NULL: `"skip"` o `"pass"`. Solo con palabras clave                                                    |
| `on_error`    | str o ExceptionHandling           | `None` (propagar) | Cómo manejar excepciones: `"propagate"` o `"ignore"`. Solo con palabras clave                                               |

Cada parámetro de tipo (los elementos de `arg_types` y `return_type`) acepta:

* Una constante `ChdbType`: `INT64`, `STRING`, `FLOAT64`, etc.
* Una cadena de tipo de ClickHouse: `"Int64"`, `"String"`, `"DateTime64(6)"`, `"DateTime('UTC')"`, etc.
* Un tipo de Python: `int`, `float`, `str`, `bool`, `bytes`, `datetime.date`, `datetime.datetime` — asignado según la [correspondencia automática de tipos](#udf-automatic-type-mapping)

Registrar un nombre ya registrado genera un error; las UDFs no se reemplazan silenciosamente. Llame primero a [`drop_function`](#drop-function) para volver a registrar una función.
...
Elimina una UDF de Python registrada previamente. No hace nada si la función no está registrada, por lo que se puede llamar sin condiciones.
...
| Parámetro | Tipo | Descripción                               |
| --------- | ---- | ----------------------------------------- |
| `name`    | str  | Nombre de la función SQL que se eliminará |
...
Decorador para registrar una función de Python como función SQL de chDB. La función sigue pudiendo llamarse como una función de Python normal y, al mismo tiempo, está disponible en consultas SQL con su `__name__`.
...
Los mismos que [`create_function`](#create-function), excepto `name` y `func`, que se derivan de la función decorada.
...
Importa los tipos desde `chdb.sqltypes`:
...
<div id="udf-automatic-type-mapping">
  #### Correspondencia automática de tipos
</div>

Cuando los tipos se infieren a partir de anotaciones de Python, se utiliza la siguiente correspondencia:
...
Los tipos se pueden especificar de varias formas:
...
Controla cómo se tratan los valores NULL mediante el parámetro `on_null`.

| Valor    | Enum                | Comportamiento                                               |
| -------- | ------------------- | ------------------------------------------------------------ |
| `"skip"` | `NullHandling.SKIP` | Devuelve NULL sin llamar a la función (valor predeterminado) |
| `"pass"` | `NullHandling.PASS` | Convierte NULL en `None` y llama a la función                |
...
Controla cómo se gestionan las excepciones con el parámetro `on_error`.

| Valor         | Enum                          | Comportamiento                                                   |
| ------------- | ----------------------------- | ---------------------------------------------------------------- |
| `"propagate"` | `ExceptionHandling.PROPAGATE` | Propaga la excepción como un error de SQL (valor predeterminado) |
| `"ignore"`    | `ExceptionHandling.IGNORE`    | Captura la excepción y devuelve NULL                             |
...
Las UDF admiten plenamente los tipos `Date`, `Date32`, `DateTime` y `DateTime64` con compatibilidad con zonas horarias.
...
* Los valores `DateTime`/`DateTime64` de entrada de ClickHouse se convierten en objetos `datetime` de Python con información de zona horaria
* Los objetos `datetime` de Python de salida conservan la información de zona horaria al devolverse a ClickHouse
* El tipo `DATETIME64` de `chdb.sqltypes` usa de forma predeterminada la escala 6 (microsegundos), equivalente a `DateTime64(6)`
...
<Warning>
  **Obsoleta:** El decorador `@chdb_udf` es el mecanismo heredado de UDF basado en subprocesos: ejecuta la función en un proceso de Python independiente y pasa todos los argumentos como cadenas. Sigue disponible, pero se recomienda la API nativa de UDF de Python ([`@func`](#func-decorator) / [`create_function`](#create-function)); las UDF nativas se ejecutan en el mismo proceso, con argumentos tipados, manejo de NULL y control de excepciones. Consulte la [guía de UDF de Python](/es/chdb/guides/python-udf) para ver una explicación completa.
</Warning>
