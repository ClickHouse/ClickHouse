...
Para consultar la guía completa sobre métodos de registro, sistema de tipos, manejo de NULL y excepciones, y compatibilidad con DateTime, consulte la [guía de UDF de Python](/es/chdb/guides/python-udf). Para consultar la referencia completa de la API, vea la [referencia de la API de UDF de Python](/es/chdb/api/python#user-defined-functions). El antiguo decorador `@chdb_udf` sigue estando disponible, pero ha sido reemplazado por esta API; consulte la [API heredada](/es/chdb/api/python#legacy-udf).
