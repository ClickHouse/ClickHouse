...
Los roles se leen de la claim de roles anterior, y los grants de SQL se leen de forma predeterminada de la claim `clickhouse:grants` del token. Los roles indicados en la claim de roles ya deben existir en el servicio (créelos con `CREATE ROLE`). El límite de permisos establece el máximo de derechos de acceso efectivos que pueden otorgar tanto las claims de roles como las de grants, como se describe en [Derechos de acceso](#access-rights).
