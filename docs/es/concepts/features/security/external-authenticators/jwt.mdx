...
<EnterprisePlanFeatureBadge feature="Autenticación con JWT con un proveedor de identidad personalizado" support="true" />

Además del autenticador integrado, puede configurar su servicio de ClickHouse Cloud para que acepte JWT emitidos por su propio proveedor de identidad (por ejemplo, Microsoft Entra u Okta). Esta función beta está disponible en el plan Enterprise para servicios que ejecutan ClickHouse versión 26.4 o posterior. Para habilitarla, abra un [ticket de soporte](https://clickhouse.com/support/program) con los siguientes datos de configuración:

| Parámetro       | Descripción                                                                                                                                                                                                                                                                                                                                                                 |
| --------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ID del servicio | El UUID del servicio de ClickHouse Cloud en el que desea habilitar la autenticación con JWT. Puede encontrarlo en la URL de la consola de Cloud al abrir el servicio.                                                                                                                                                                                                           |
...
| URL de JWKS     | El endpoint HTTPS donde su proveedor de identidad publica su JSON Web Key Set (por ejemplo, `https://idp.example.com/.well-known/jwks.json`). ClickHouse obtiene las claves públicas de este endpoint para verificar las firmas de los tokens. La verificación de JWKS solo funciona con claves RSA (`RS256`), como se describe en [Claims obligatorias](#required-claims). |
| Claim de roles  | El nombre de la claim del token que contiene los roles de ClickHouse del usuario efímero (el valor predeterminado es `clickhouse:roles`). La claim debe contener un array JSON de nombres de roles, por ejemplo, `["analyst", "reader"]`.                                                                                                                                   |
| Claim de grants | El nombre de la claim del token que contiene fragmentos de SQL `GRANT` para el usuario efímero (el valor predeterminado es `clickhouse:grants`). La claim debe contener un array JSON de fragmentos de grants, por ejemplo, `["SELECT ON analytics.*", "INSERT ON analytics.events"]`.                                                                                      |

Los roles indicados en la claim de roles ya deben existir en el servicio (créelos con `CREATE ROLE`). El límite de permisos establece el máximo de derechos de acceso efectivos que pueden otorgar tanto las claims de roles como las de grants, como se describe en [Derechos de acceso](#access-rights).
