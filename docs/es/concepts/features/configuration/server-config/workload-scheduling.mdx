...
El siguiente ejemplo muestra cómo definir las jerarquías de planificación de E/S que aparecen en la imagen anterior:
...
Para poder usar toda la capacidad del recurso subyacente, debe usar `inflight_limit`. Tenga en cuenta que un valor bajo de `max_requests` o `max_cost` podría hacer que no se aproveche por completo el recurso, mientras que valores demasiado altos podrían provocar colas vacías dentro del planificador, lo que a su vez hará que las políticas se ignoren (falta de equidad o ignorar las prioridades) en el subárbol. Por otro lado, si desea proteger los recursos frente a una utilización excesiva, debe usar `bandwidth_limit`. Este limita la velocidad cuando la cantidad de recurso consumido en `duration` segundos supera `max_burst + max_speed * duration` bytes. Se podrían usar dos nodos `bandwidth_limit` sobre el mismo recurso para limitar el ancho de banda pico durante intervalos cortos y el ancho de banda promedio durante intervalos más largos.

<div id="workload-classifiers">
  ### Clasificadores de cargas de trabajo obsoletos
</div>
