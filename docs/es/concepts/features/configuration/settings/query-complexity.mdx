...
| [`timeout_overflow_mode`](/es/reference/settings/session-settings/timeout-overflow-mode#timeout_overflow_mode)                                 | Establece qué hacer si la consulta se ejecuta durante más tiempo que `max_execution_time` o si el tiempo estimado de ejecución es mayor que `max_estimated_execution_time`. |
