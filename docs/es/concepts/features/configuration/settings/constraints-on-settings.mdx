...
<Warning>
  Mantén `readonly` fuera de la lista `changeable_in_readonly` en todos los perfiles y recuerda que las restricciones de perfil se aplican a todo el sistema, no por usuario.
</Warning>

No marques el propio `readonly` como `changeable_in_readonly`. Si lo haces, una sesión que comienza con `readonly = 1` podrá ejecutar `SET readonly = 0` y recuperar la capacidad de lanzar las consultas de escritura que sus privilegios actuales ya le permiten.

Esto importa en todos los casos, no solo en el perfil asignado al usuario:

* Cualquier sesión puede cambiar a cualquier perfil mediante `SET profile`, ya que la selección de perfil no pasa por una comprobación de acceso (solo la limitan las restricciones de configuración que ya estén activas).
* Por HTTP, las solicitudes `GET` se fuerzan a `readonly = 2` únicamente cuando el valor efectivo sería `0`. Así, un perfil que establece `readonly = 1` pero además permite modificar `readonly` en modo de solo lectura anula esa protección: una solicitud `GET` puede cambiarlo a `0` y escribir.
