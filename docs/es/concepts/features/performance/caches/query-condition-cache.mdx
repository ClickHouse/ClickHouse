...
<Note>
  La caché de condiciones de consulta solo funciona cuando [enable&#95;analyzer](/es/reference/settings/session-settings/allow-experimental#allow_experimental_analyzer) está establecido en true, que es el valor predeterminado.
</Note>
...
Las consultas que utilizan la optimización TopK (`ORDER BY <column> LIMIT n`, acelerada mediante filtrado dinámico o un índice de omisión minmax en la columna de ordenación) no usan la caché de condiciones de consulta de forma predeterminada: estas lecturas no la consultan ni la pueblan.
Las lecturas TopK pueden omitir gránulos según un umbral que cambia mientras se ejecuta la consulta, por lo que sus entradas de caché requieren especial atención; su uso está sujeto a la configuración independiente [`use_query_condition_cache_for_top_k`](/es/reference/settings/session-settings/use-query) (predeterminado: deshabilitado) hasta que se confirme plenamente la validez de estas entradas.
La configuración solo surte efecto si `use_query_condition_cache` también está habilitada.
