...
* **(3)** Si la tabla derecha cabe en memoria, usa el hash join (`hash` y `parallel_hash` son alias; elegir uno no controla el paralelismo). Suele ser la opción en memoria más rápida, según el volumen de datos, los tipos de datos y la distribución de valores de las columnas de la clave de join.
