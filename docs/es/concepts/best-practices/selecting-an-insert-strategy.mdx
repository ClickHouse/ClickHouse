...
La compresión no solo reduce el tráfico de red, sino que también mejora la eficiencia en el uso de CPU y memoria en el servidor. Con datos comprimidos, ClickHouse recibe menos bytes y dedica menos tiempo al análisis de entradas grandes. Este beneficio es especialmente importante al ingestar desde varios clientes concurrentes, como en escenarios de observabilidad.
...
Al usar la interfaz nativa (p. ej., [clickhouse-client](/es/concepts/features/interfaces/client)), la compresión LZ4 está habilitada de forma predeterminada. Opcionalmente, puedes cambiar a ZSTD mediante la configuración.
