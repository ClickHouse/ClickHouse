...
Para resolver los problemas existentes en los intérpretes, se ha desarrollado un nuevo `InterpreterSelectQueryAnalyzer`. Esta es una nueva versión de `InterpreterSelectQuery` que no utiliza `ExpressionAnalyzer` e introduce una capa adicional de abstracción entre `AST` y `QueryPipeline`, llamada `QueryTree`. Es el único análisis de consultas desde la versión `26.9`; la configuración `enable_analyzer`, que antes permitía volver a `ExpressionAnalyzer`, está obsoleta y ya no puede desactivarse.
