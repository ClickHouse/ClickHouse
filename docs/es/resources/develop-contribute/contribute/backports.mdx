...
<Steps>
  ...
  <Step title="Gestión de ramas durante el despliegue" id="rolling-out-branch-handling">
    Cuando un PR de lanzamiento lleva la etiqueta `rolling-out`, las etiquetas generales de backport (`pr-must-backport`, `pr-critical-bugfix`) omiten esa rama. El bot cierra cualquier PR de cherry-pick o backport creado previamente para esa rama con un comentario explicativo. Una etiqueta específica de versión (por ejemplo, `v25.3-must-backport`) siempre prevalece sobre esto — para la versión indicada y para cada rama de lanzamiento activa más reciente a la que se extienda. `pr-must-backport-force` omite la comprobación de `rolling-out` para todas las ramas.
  </Step>
  ...
  <Step title="Merge automático de PR de cherry-pick sin conflictos" id="auto-merge-conflict-free-cherry-pick-prs">
  ...
  <Step title="Finalización" id="completion">
  ...
  <Step title="Verificación previa" id="pre-check">
