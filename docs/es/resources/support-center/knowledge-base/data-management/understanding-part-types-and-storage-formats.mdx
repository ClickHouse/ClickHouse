...
<div id="storage-formats-full-vs-packed">
  ## Formatos de almacenamiento: Full vs Packed (ClickHouse Cloud)
</div>
...
| **Packed** | Todos los archivos se agrupan en un único archivo                         | Solo en ClickHouse Cloud |

<div id="full-storage">
  ### Almacenamiento Full
</div>

En el almacenamiento Full, cada parte está formada por varios archivos independientes que se almacenan por separado en disco. Es la opción predeterminada y la única disponible en ClickHouse de código abierto.

<div id="packed-storage">
  ### Almacenamiento Packed (solo ClickHouse Cloud)
</div>
...
En ClickHouse Cloud, una parte utiliza almacenamiento Packed si se cumple **alguna** de estas condiciones:
...
<Info>
  **Código abierto**
  ...
  Aunque los ajustes `min_bytes_for_full_part_storage`, `min_rows_for_full_part_storage` y `min_level_for_full_part_storage` están definidos en ClickHouse de código abierto, no tienen efecto porque la implementación del almacenamiento Packed solo está disponible en ClickHouse Cloud.
</Info>

El ajuste `min_level_for_full_part_storage` puede utilizarse para optimizar tanto el rendimiento como los costos en entornos de ClickHouse Cloud, especialmente en tablas que reciben inserciones continuas.
...
Establezca `min_level_for_full_part_storage = 2` para implementaciones en la nube.
Esto garantiza que:
...
Estos dos conceptos son ortogonales y puede haber cualquier combinación, según si se utiliza ClickHouse Cloud o ClickHouse de código abierto:

| Combinación      | Caso de uso                                                                       |
| ---------------- | --------------------------------------------------------------------------------- |
| Wide + Full      | Partes grandes en almacenamiento local (predeterminado en la versión open source) |
| Wide + Packed    | Partes grandes en almacenamiento en la nube                                       |
| Compact + Full   | Partes pequeñas en almacenamiento local                                           |
| Compact + Packed | Partes pequeñas en almacenamiento en la nube                                      |
