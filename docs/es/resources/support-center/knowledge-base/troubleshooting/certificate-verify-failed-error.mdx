...
Como alternativa, apunte su client directamente al paquete de certifi. [ClickHouse Connect](/es/integrations/python) no usa certifi automáticamente como alternativa, así que pase el paquete mediante el parámetro `ca_cert`:
...
Consulta la [documentación sobre la configuración de `clickhouse-client`](/es/interfaces/client#configuration_files).