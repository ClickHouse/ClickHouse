---
noindex: true
title: "Integrate Teleport with ClickHouse Private"
sidebarTitle: "Integrate Teleport"
description: "Set up Teleport Enterprise for secure database access to ClickHouse Private clusters in government/FIPS environments."
---
...
- The [FIPS/Government deployment](/products/clickhouse-private/tutorials/deploy-clickhouse-with-fips-compliance-government) has been completed with `SERVER_VERIFICATION_MODE=relaxed` and `KEEPER_VERIFICATION_MODE=relaxed` (required for Teleport mTLS)
- Teleport Enterprise license
- The server certificate SAN must include `DNS.2 = c-${CLUSTER_NAME}-server-any.ns-${CLUSTER_NAME}.svc.${KUBERNETES_DOMAIN}` (see [Generate FIPS certificates](/products/clickhouse-private/guides/generate-fips-compliant-certificates-for-clickhouse))
...
> This example deploys Teleport with FIPS on EKS. For other cloud providers, refer to the [Teleport documentation](https://goteleport.com/docs/zero-trust-access/deploy-a-cluster/helm-deployments/).
...
For more information on roles, see the [Teleport documentation](https://goteleport.com/docs/reference/infrastructure-as-code/operator-resources/resources-teleport-dev-rolesv8/).
...
For more information on SAML connectors, see the [Teleport documentation](https://goteleport.com/docs/reference/infrastructure-as-code/operator-resources/resources-teleport-dev-samlconnectors/).
...
Replace: - `<DOMAIN>`: Your Route53 domain - `<AUTH_TYPE>`: Authentication type (e.g., `samlconnector`) - `<TELEPORT_USER>`: Teleport username - `<DB_USER>`: Database username - `<CLICKHOUSE_CLUSTER_NAME>`: ClickHouse cluster name (e.g., `default-aa-01`)
...
1.  **Generate new certificates** using the script from [Generate FIPS Certificates for Teleport](#generate-fips-certificates-for-teleport). Ensure `CLUSTER_FQDN` and other variables match your cluster.
