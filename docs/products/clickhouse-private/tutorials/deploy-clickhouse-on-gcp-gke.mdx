...
## Step 1: Set Environment Variables (`00-env.sh`)
...
## Step 2: Create Artifact Registry (`01-artifact-registry.sh`)
...
## Step 3: Copy Container Images (`02-copy-images.sh`)
...
## Step 4: Configure VPC and Subnet (`03-vpc-subnet.sh`)
...
## Step 5: Create Cloud NAT (`04-cloud-nat.sh`)
...
## Step 6: Create GCP Service Account (`05-service-account.sh`)
...
## Step 7: Create GCS Bucket (`06-gcs-bucket.sh`)
...
## Step 8: Create GKE Cluster (`07-gke-cluster.sh`)
...
## Step 9: Create Node Pools (`08-node-pools.sh`)
...
## Step 10: Deploy Local SSD DaemonSet (`09-local-ssd-daemonset.sh`)
...
## Step 11: Add Topology Labels (`10-topology-labels.sh`)
...
## Step 12: Install Kubernetes Prerequisites
...
## Step 13: FIPS Steps (Conditional)
...
## Step 14: Install Operator (`16-install-operator.sh`)
...
## Step 15: Deploy ClickHouse Cluster (`17-install-cluster.sh`)
...
## Step 16: Bind Workload Identity (`18-bind-helm-workload-identity.sh`)
