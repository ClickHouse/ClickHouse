...
This page takes you from an enrollment token to a verified connector in managed mode, where the executor applies the service lifecycle that ClickHouse Cloud drives. The connector installs on a Kubernetes cluster (Helm) or a Linux VM (systemd). Token enrollment is the standard flow; when your environment cannot reach your connector endpoint directly, see [air-gapped and mirrored installs](#air-gapped-and-mirrored-installs). To observe ClickHouse you run yourself without the executor, pass `--no-managed`.

## Managed-mode availability {#managed-mode-availability}

<Warning>
As of 2026-09-22, the public installer serves `v0.16.0`, which has neither the `init --managed` and `init --no-managed` flags nor the `platform` command. Platform approval on both Linux VM and Kubernetes installs, and customer service creation (`instances create --from-prepare`), are not yet available in a public release. Kubernetes platform approval additionally requires CLI and chart support for `platform approve --secret-namespace`.

Before following the managed-mode workflows, ask your ClickHouse account team to confirm support for your deployment target and provide the matching CLI, connector chart or image, and installation instructions. Customer service creation also requires support on your connector endpoint and in ClickHouse Cloud; a newer CLI alone is not sufficient. The public installation commands below do not currently install a release that supports these workflows.
</Warning>
...
ClickHouse Cloud then issues your connector endpoint and a single-use enrollment token. The token identifies your environment, so you type no extra id at enrollment.

### For every install {#prerequisites-every-install}

- **A build with [managed-mode support](#managed-mode-availability)** for the managed-mode instructions below. Public `v0.16.0` has neither `--managed` nor `--no-managed`.
- **Your connector endpoint and enrollment token**, provided during [registration](#registration).
- **Egress on port 443** to `https://<subdomain>.<connector-domain>` and `https://<subdomain>.enroll.<connector-domain>`, plus `releases.clicklink.clickhouse.com` and Amazon ECR Public at install time. In managed mode the executor also reaches Amazon ECR and STS when a platform sync or create pulls a chart from ECR. When any of these are unreachable, see [air-gapped and mirrored installs](#air-gapped-and-mirrored-installs).
...
The token is single-use and expires quickly, so run enrollment soon after you receive it. Treat it as a secret: the CLI reads it from a hidden prompt or the first line of stdin, never from command-line arguments, disk, or logs. If it expires first, ask your ClickHouse account team for a fresh one.
</Step>

<Step title="Install and verify the CLI" id="install-and-verify-the-cli">
One command installs a verified `clicklink` binary. It detects your platform (macOS or Linux, amd64 or arm64), downloads the current release, and verifies the SHA-256 checksum and, with cosign installed, the release signature. It installs the binary into `/usr/local/bin` when that is writable, otherwise into `~/.local/bin` (add that to your `PATH` if needed; `--bin-dir` overrides). For a Kubernetes install, run it on any workstation with kubeconfig access to the cluster:

```bash
curl -fsSL https://releases.clicklink.clickhouse.com/install.sh | bash
```

For a VM install, run the same script on the host with `--host`. After the verified download it also creates the `clicklink` system user, the `/etc/clicklink`, `/var/lib/clicklink`, and `/var/log/clicklink` directories, and the systemd units for all three components. It seeds `/etc/clicklink/redaction-patterns.yaml` with a starter set of patterns (preserved if one exists), so the next step starts at enrollment:
...
These commands require the build and deployment support described under [managed-mode availability](#managed-mode-availability). Public `v0.16.0` has neither `--managed` nor `--no-managed`.

Enrollment is a single command. It redeems your token, obtains a signed client certificate and the executor's cluster-wide grant, then installs and verifies the connector.
...
`init` redeems the token and saves the enrollment bundle as `handoff.yaml`. It writes `/etc/clicklink/config.yaml` with the `executor` block and installs the API credentials and CA chain. It generates a private key and CSR and has ClickHouse Cloud sign the client certificate. It generates the executor's cluster-wide access bundle under `/etc/clicklink/access/executor/_cluster` and applies it through the host's kubeconfig. Finally it enables and starts the `clicklink-scraper`, `clicklink-troubleshooter`, and `clicklink-executor` units, waits for each to report live, and runs the full preflight check suite.
</Tab>
</Tabs>
...
The enrollment bundle `handoff.yaml` (mode `0600`, in the working directory) exists so that re-runs and recovery during the install never need a second token. It holds the connector's API secret in plaintext, so delete it once the install has verified:
...
The running connector holds its own copy of the credentials, so nothing operational depends on the file. Upgrades and configuration changes never need it. If you later need `init` again, request a fresh enrollment token from your ClickHouse account team and run `init --enroll --force`.
</Step>
</Steps>
...
If `init` rejects `--managed` or `--no-managed` as an unknown flag, check your CLI version: public `v0.16.0` supports neither. Obtain a [build with managed-mode support](#managed-mode-availability) from your account team before following the managed-mode recovery instructions below.

Re-run the same command. `init` is idempotent: re-runs converge on the same state, keep your existing config and staged files, and skip completed work. When a step fails partway through, the CLI prints the exact recovery commands for your situation; they are safe to repeat.

If enrollment is refused, the token was already redeemed, or it is invalid or expired. For a redeemed token, re-run with `--handoff handoff.yaml`, which exists until the cleanup step; otherwise ask your ClickHouse account team for a fresh token. If enrollment fails with a transport error, re-run the same command. If the re-run is refused, the first attempt did spend the token, so request a fresh one.
...
`--force` is the explicit reset, not a routine retry. It overwrites the kept config or values overlay, regenerates the client key, and supersedes an unexpired client certificate (a `409` from the signing endpoint means one already exists). The connector's cluster UUID is preserved across `--force`, and so is its managed-mode posture. Unattended re-runs keep the posture, and on a terminal the prompt defaults to it, unless you pass `--managed` or `--no-managed`. Use `--force` when rotating credentials or superseding a certificate, and see [operations](/products/bring-your-own-cloud/connector/operations) for the full re-run and recovery model.
