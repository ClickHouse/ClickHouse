...
<Warning>
As of 2026-09-22, the public installer serves `v0.16.0`, which does not include `init --managed`. Customer service creation (`instances create --from-prepare`) and Kubernetes platform approval (`platform approve --secret-namespace`) are also not yet available in a public release.
