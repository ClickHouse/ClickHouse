...
Managed mode runs the executor, the component that applies the service lifecycle ClickHouse drives to your cluster. `init` turns it on and fills in the cluster identity; the keys below are what you may revisit afterwards. Managed mode is chosen at enrollment (see [enable managed mode](/products/bring-your-own-cloud/connector/managed-services#enable-managed-mode)). To turn it on for a connector installed without it, or off, re-run `init` with `--force` and `--managed` or `--no-managed`; like every `init` run this needs an entry point, `--handoff` if you kept the bundle or `--enroll` with a fresh token, as described in [re-runs and recovery](/products/bring-your-own-cloud/connector/operations#re-runs-and-recovery).
...
cluster:
  ...
  # prefix you registered with ClickHouse.
  namespacePrefix: ns-
...
executor:
  ...
  cluster:
    ...
    # with each service, so leave it out unless ClickHouse asks for it.
    # s3_storage:
    #   bucket: <data-bucket>
...
- **`executor.cluster`** is exactly one cluster: `name`, `region`, `accountId`, and `inCluster: true`, which makes the executor authenticate as the pod's ServiceAccount. Keys other than those four pass through to the executor verbatim in snake_case, such as `s3_storage`.
...
Services the executor creates register themselves, so in managed mode you never add them here. ClickHouse you run yourself is observed by a separate connector deployment installed with `--no-managed`; pass `--ch-user-suffix` at its `init` when the two deployments share an instance. This section applies to that deployment. Each entry under `instances` names a ClickHouse native-protocol endpoint the connector reads from: `host`, `port`, `database`, `secure`, plus `namespace` and `cluster` on Kubernetes. Credentials never live in the configuration; each component resolves its read-only ClickHouse user from the access bundle that provisioning creates.
...
On Kubernetes the chart ships a default-deny NetworkPolicy with an egress allowlist. NetworkPolicy objects only take effect when your CNI enforces them; under an enforcing CNI the connector has no egress at all until both CIDR lists are filled in, so `init` stages the policy enabled only when it knows the CIDRs behind your connector endpoint.
...
- **`allowEgressCIDRs`** is yours to supply: the ranges behind your connector endpoint, given at install with `--egress-cidrs` or at the prompt. Every component needs them to reach ClickHouse.
...
**Enabling the policy later.** When you skipped the endpoint CIDRs at install, `init` staged `enabled: false` with `allowEgressCIDRs: []` and printed the step to finish it. Once you know the CIDRs, set `enabled: true` and `allowEgressCIDRs` in `clicklink-values.yaml`, confirm `apiserverCIDRs` holds your VPC's ranges, and run the `helm upgrade` shown in [configuration surfaces](#configuration-surfaces).

One further egress rule matters when the session gateway is enabled. **`clctl.gateway.jwksEgressCIDRs`**: the troubleshooter fetches your identity provider's JWKS to validate operator tokens, and under a default-deny posture leaving this empty blocks every token check:
...
The scraper spools metrics under `/var/lib/clicklink/buffer` for at-least-once delivery while your connector endpoint is unreachable, retaining up to 168 hours or 1024 MB, and uploads at a rate limited to 1 MB/s by default:
