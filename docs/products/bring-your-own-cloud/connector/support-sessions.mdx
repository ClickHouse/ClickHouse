...
**Read-only Kubernetes views** are `get`, `list`, and `watch` on pods, pod status, pod logs, services, configmaps, events, PersistentVolumeClaims, deployments, statefulsets, and replicasets in the granted namespaces. Access bundles are anchored to Kubernetes ServiceAccounts on both install targets. Without a provisioned bundle, the troubleshooter refuses kubectl-type commands outright.
