...
`approve` needs a kube context with cluster-admin rights on the managed cluster (`--context <name>` when your kubeconfig holds several). When a chart lives in Amazon ECR it also needs AWS credentials that can read it. `--dry-run` renders and lists the permission half without applying or minting anything.
...
`approve` reads the staged bundle over the port-forward from the previous step. It delivers the token bundle as a Secret in the connector namespace, which the chart mounts into the pod. Run it from a workstation whose kubeconfig reaches the cluster, with a copy of the connector configuration. The chart renders one into the executor's ConfigMap; `approve` needs it for the namespace prefix and reads no credentials from it.
