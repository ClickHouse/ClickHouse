...
<Note>
The public `v0.16.0` CLI has no `platform` command, including `platform approve` and `platform reset`. Neither the Linux VM nor the Kubernetes workflow below is available in that release. Kubernetes approval additionally requires CLI and executor-chart support for the Secret-based token bundle (`--secret-namespace`). Confirm [managed-mode availability](/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) with your account team before using either workflow.
</Note>
...
A platform bundle is the rendered manifest ClickHouse Cloud produces for your environment. It pins the chart and image versions of those components and names the registry they are pulled from. Every bundle is identified by the sha256 of that manifest. ClickHouse Cloud sends the bundle to the executor over its command channel, and the executor stages it for your approval.
...
Both approval procedures below require a [build with platform support](/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability); public `v0.16.0` has no `platform` command. Kubernetes approval also needs matching CLI and chart support for Secret delivery.

Registry authentication depends on the image distribution mode registered for your environment:

- **Direct access to ClickHouse's registry.** Run approval on the connector's EC2 VM. Both `approve` and the executor's platform sync use credentials from the EC2 instance metadata service to assume your environment's read-only ECR puller role. They use that role for chart login; the executor also uses it to check that the platform images exist. AWS profiles, environment credentials, or SSO credentials on a workstation do not replace the instance profile. Your kubeconfig still supplies the separate cluster-admin credentials for applying the permission half.
...
Use the Kubernetes workstation procedure below only for mirrored-registry deployments with Kubernetes platform approval enabled. For direct access, ask your account team to confirm a supported execution environment with the required EC2 instance profile before proceeding.

<Steps>
<Step title="Find the staged bundle" id="find-the-staged-bundle">
ClickHouse Cloud sends each proposed bundle to the executor before anything else happens. The executor stages it as the pending bundle and reports its sha256 on the heartbeat. Exactly one staged bundle exists at a time; a newer proposal replaces it. The executor then refuses the sync and records it as a failed command whose result ends with the command to run.

```bash
clicklink clctl commands list --status failed --action sync_platform
```

The `result` field ends with `run: clctl platform approve (pending bundle sha <sha256>)`. On Kubernetes the hint reads `run: clctl platform approve --secret-namespace <connector-namespace> (pending bundle sha <sha256>)`. A create that hit a missing custom resource definition carries the same `clctl platform approve` line in its own failed command and in the error `clicklink clctl instances create --wait` prints. Your ClickHouse account team tells you as well when a platform update is proposed.
...
`approve` with no flags reads the staged bundle and hashes it, so what you approve is exactly what the executor received. It renders every chart in the bundle exactly as the executor will and applies the permission half with the kube context you choose. It creates the `pcm-platform` identity if needed and mints its token. It makes no request to your connector endpoint.
...
For a mirrored-registry deployment with Kubernetes platform approval enabled, `approve` reads the staged bundle over the port-forward from the previous step. It delivers the token bundle as a Secret in the connector namespace, which the chart mounts into the pod. Run it from a workstation whose kubeconfig reaches the cluster, with a copy of the connector configuration and registry access as described above. The chart renders the configuration into the executor's ConfigMap; `approve` needs it for the namespace prefix and reads no credentials from it.
...
You apply the permission half, so it carries your authority. `approve` stamps it with the bundle's sha256, and the executor verifies before touching anything that every permission object of the bundle it was sent is present and approved. The executor applies nothing in it.
...
The public `v0.16.0` CLI has no `platform reset` command; confirm [availability](/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) before using this procedure.
