...
- `instances create` reads the credential files the configuration names from the `clicklink-hmac` and `clicklink-mtls` Secrets when this host lacks them, and announces each read. Add `--connector-namespace` when the connector namespace is not `clicklink`. Only `instances create` and `platform approve` have this fallback.
