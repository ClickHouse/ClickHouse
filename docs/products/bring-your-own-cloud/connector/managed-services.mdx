...
<Note>
Enabling managed mode and creating a service require support that is not yet in the public release. The public `v0.16.0` CLI has neither the `--managed` and `--no-managed` init flags nor the customer-create workflow below. Confirm [managed-mode availability](/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) with your account team before enrolling or preparing resources; service creation also needs matching support in your connector endpoint and ClickHouse Cloud.
</Note>
...
<Note>
The public `v0.16.0` CLI does not support `init --managed` or the managed-mode prompt described below. First obtain a build and deployment with [managed-mode support](/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) from your account team.
</Note>
...
This workflow is unavailable in public `v0.16.0`. Before preparing resources, confirm [service-creation availability](/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) for your CLI, connector endpoint, and ClickHouse Cloud deployment.
