...
Each service needs three cloud resources you own: a data bucket, a backup bucket, and an IAM role the ClickHouse pods assume to reach them. You create them with your own credentials before the service exists and remove them after it is gone. The executor calls no AWS API directly and never deletes data. For each service it creates a Kubernetes `Service` of type LoadBalancer, which your cluster's load-balancer controller realizes as an internal NLB in your account.
...
Both commands need AWS credentials that can create S3 buckets and an IAM role. On a VM, run both as root on the connector host. On Kubernetes, run both from a workstation:

- `prepare` needs a kube context for the managed cluster and a copy of the connector configuration (`--config`). It also needs a `kubectl port-forward` to the executor's local API and `--output-dir` pointing at a writable directory. Extract the configuration from the `clicklink-connector-executor` ConfigMap as shown under [platform updates](/products/bring-your-own-cloud/connector/platform-updates#approve-a-platform-update). `prepare` checks the service name against the executor and refuses to run when it cannot reach it.
- `instances create` reads the connector's configuration and credentials from the connector namespace when this host holds no copy (`--connector-namespace`, default `clicklink`), and announces each read. Only `instances create` and `platform approve` have this fallback.
...
3. **Grant and apply.** Renders the executor's access bundle for the service namespace (`ns-<name>`) and applies its RBAC. On a VM this also registers the service in the executor's registry. On Kubernetes the executor runs as the pod ServiceAccount and ignores per-service bundles; it registers the service itself when the create arrives.
...
<Warning>
`prepare` prints the `default` user's password once, on the progress output, and never again. Neither the create body nor `--output json` contains it, and ClickHouse Cloud receives only its hashes. Store it before moving on. A re-run that finds the create body keeps the hashes already written, and says so.
</Warning>
...
The command reads the executor's record of the service: where its data lives and which role it had. It deletes the IAM role that `prepare` created, removes the local access bundle and registry entry, and makes the executor forget the service, which frees the name.

By default it keeps the service's data and backups. It re-tags a kept bucket from `clicklink:deployed-name` to `clicklink:retained-from=<name>`, so a re-created service with the same name never inherits it, and the summary says where the data is. To delete them, pass the data flags together with `--yes`:
