...
Each daemon serves a `/livez` endpoint on its health port. The JSON `status` field in the response body is the health signal, not the HTTP status code, so check the body rather than the `200`. Prometheus metrics are served at `/metrics` on the same port; there is no separate metrics port. Default ports on both targets:
...
The managed-mode behavior described here requires a [supported build](/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability); public `v0.16.0` has neither `--managed` nor `--no-managed`.
...
On Kubernetes, `init` pre-creates the rotated `clicklink-hmac` and `clicklink-mtls` Secrets; the chart does not render them. The chart's checksum annotation therefore does not cover them, and running pods keep the credentials they started with. Restart every component after the rotation:
...
On a VM, run them on the host with the admin password piped in; on a VM the password is mandatory unless you pass `--ch-user-via cr`:
...
<Note>
The public `v0.16.0` CLI has neither `--managed` nor `--no-managed`. The managed-mode preservation and override behavior below requires a build with [managed-mode support](/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability); confirm your build with your account team before changing that mode during recovery.
</Note>
...
**Connector offline.** While the connector is down or unreachable, the services it manages keep running. The ClickHouse operator in your cluster keeps them up, and the executor is not in the data path. Lifecycle changes need the executor connected, so none are applied until it reconnects. On restart the executor recovers commands that were in flight when it stopped from its command database. A command ClickHouse Cloud delivers again is answered from the recorded result rather than run twice; only a failed command runs again. For what happens to lifecycle commands while no executor is connected, see [when the connector is offline](/products/bring-your-own-cloud/connector/managed-services#when-the-connector-is-offline).
