...
| Amazon ECR (executor, managed mode only) | Outbound | HTTPS | The executor's ambient AWS identity, or the read-only pull role it assumes for the image check | Registry login to pull a chart for a platform sync or create; `DescribeImages` during a platform update to confirm the images exist |
| AWS STS (executor, managed mode only) | Outbound | HTTPS | The ambient AWS identity of the executor's node or host | Credentials for the ECR login, and the assume-role into the read-only pull role you registered for the image check. ECR and STS are the executor's only cloud API calls |
