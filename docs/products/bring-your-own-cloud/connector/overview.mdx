...
On a terminal, `init` asks whether to enable managed mode and defaults to yes; scripts must pass `--managed`. You can also run without it and keep observability and support for clusters you operate yourself; see [run without managed mode](#run-without-managed-mode).
...
`init` enables managed mode when you pass `--managed`; on a terminal it asks and defaults to yes. Enabling it deploys the executor and applies its Kubernetes access grant. The grant is a `pcm-executor` ServiceAccount. An admission policy confines its writes to service namespaces (prefix `ns-` by default), apart from its own token renewal, one registry ConfigMap, and, on Kubernetes, the renewed client certificate in its home namespace.
