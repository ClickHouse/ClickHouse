...
<Note>
The managed-mode workflows described here are not yet available in the public connector release. Confirm the required build and deployment support with your account team before installing; see [managed-mode availability](/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability).
</Note>

## What the connector does {#what-the-connector-does}

The ClickHouse Connector is a component you deploy inside your own environment. It lets ClickHouse Cloud manage, monitor, and support ClickHouse services in a Kubernetes cluster you operate. ClickHouse Cloud never connects into your environment: the connector makes outbound connections only, and acts under identities scoped to the ClickHouse services. The connector ships as one binary, `clicklink`, which contains the daemons and the `clicklink clctl` tooling you install and operate them with.
...
With a [build that supports managed mode](/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability), `init` asks whether to enable it on a terminal and preselects yes; a scripted `init` enables it only with `--managed`. You can also run without it and keep observability and support for clusters you operate yourself; see [run without managed mode](#run-without-managed-mode).
...
- Writes to your cluster are confined to the namespaces of the services ClickHouse manages, apart from the executor's own token, registry, and renewed certificate in its home namespace. The platform components those services depend on change only after you approve each update.
...
- Support access is time-boxed, enabled by you, and revocable by you at any moment. Every support-session command is recorded in an audit log you own.

## How it works {#how-it-works}

The enrollment and service-creation flow below requires [managed-mode support](/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) confirmed by your account team; it is unavailable in public `v0.16.0`.

ClickHouse registers your environment during onboarding and provides your connector endpoint and a single-use enrollment token. You run `clicklink clctl init --enroll` once: from a workstation with kubeconfig access for a Kubernetes install, or on the host for a Linux VM install. That one command redeems the token, obtains an mTLS client certificate, deploys and starts the daemons, and verifies their health.

`init` enables managed mode when you pass `--managed`; on a terminal it asks, and the default is yes. Enabling it deploys the executor and applies its Kubernetes access grant. The grant is a `pcm-executor` ServiceAccount. An admission policy confines its writes to service namespaces (prefix `ns-` by default), apart from its own token renewal, one registry ConfigMap, and, on Kubernetes, the renewed client certificate in its home namespace.

Creating a service takes two commands. `clicklink clctl executor prepare` creates the service's buckets and IAM role with your own AWS credentials and shows the default user's password once. `clicklink clctl instances create` submits the service definition to ClickHouse Cloud through your connector endpoint. After that, ClickHouse Cloud drives the service through the executor's outbound command channel: scaling, stop and start, restarts, backups, version upgrades, and configuration changes. See [managed services](/products/bring-your-own-cloud/connector/managed-services) for the full walkthrough.
...
The scraper reads the allowlisted system tables on a fixed interval and ships metrics, metadata, and health status to your connector endpoint over mTLS. The troubleshooter keeps an outbound command channel open, but that channel carries nothing until you enable a support session. The mTLS certificate renews automatically.

## Run without managed mode {#run-without-managed-mode}

The install-time choice below requires a [build with managed-mode support](/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability). Public `v0.16.0` has no `--no-managed` flag or managed-mode prompt.
...
- **AWS credentials for `prepare` and `teardown`, and registry access for `platform approve`.** Creating a service's buckets and IAM role, and removing them after the service is deleted, run under your credentials on your workstation or the connector host. For direct access to ClickHouse's registry, platform approval and sync use the connector EC2 VM's instance profile to assume the read-only pull role; workstation AWS profiles or SSO credentials do not replace that identity. Other ECR chart registries use ambient AWS credentials. See [registry credentials](/products/bring-your-own-cloud/connector/platform-updates#registry-credentials). The executor never holds credentials to your buckets or IAM. Its only cloud calls go to Amazon ECR and STS for chart pulls and platform image checks; it makes no S3 or IAM calls.
...
The connector runs in a cluster you provide and control; ClickHouse Cloud manages services inside it through the executor. If you would rather have ClickHouse provision and operate the cluster itself inside your cloud account, that is the [BYOC](/products/cloud/guides/infrastructure/deployment-options/byoc/overview) deployment model. The [BYOC architecture](/products/bring-your-own-cloud/overview/architecture) page explains how the two differ.
