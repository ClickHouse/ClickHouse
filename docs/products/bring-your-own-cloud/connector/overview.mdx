...
- **AWS credentials for `prepare`, `teardown`, and `platform approve`.** Creating a service's buckets and IAM role, and removing them after the service is deleted, run under your credentials on your workstation or the connector host. `platform approve` also needs credentials that can read Amazon ECR when the staged charts live there. The executor never holds credentials to your buckets or IAM. Its only cloud calls go to Amazon ECR and STS. It logs in to the registry when a platform sync or create pulls a chart. During a platform update it checks the images under the read-only pull role. It makes no S3 or IAM calls.
