...
The ClickHouse Connector is a component you deploy inside your own environment. It lets ClickHouse Cloud manage, monitor, and support ClickHouse services in a Kubernetes cluster you operate. ClickHouse gets no standing access to your environment. The connector ships as one binary, `clicklink`, which contains the daemons and the `clicklink clctl` tooling you install and operate them with.
...
- Changes to your cluster are confined to the namespaces of the services ClickHouse manages. The platform components those services depend on change only after you approve each update.
