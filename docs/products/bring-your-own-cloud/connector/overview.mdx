...
- **AWS credentials for `prepare`, `teardown`, and `platform approve`.** Creating a service's buckets and IAM role, and removing them after the service is deleted, run under your credentials on your workstation or the connector host. `platform approve` also needs credentials that can read Amazon ECR when the staged charts live there. The executor never holds credentials to your buckets or IAM. Its only cloud call is an Amazon ECR login with the host's or pod's ambient AWS identity, made when a platform sync or create pulls a chart.
