...
On a terminal, `init` asks whether to enable managed mode and preselects yes; a scripted `init` enables it only with `--managed`. You can also run without it and keep observability and support for clusters you operate yourself; see [run without managed mode](#run-without-managed-mode).
