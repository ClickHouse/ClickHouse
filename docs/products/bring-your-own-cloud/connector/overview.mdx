...
- Writes to your cluster are confined to the namespaces of the services ClickHouse manages, apart from the executor's own token and registry in its home namespace. The platform components those services depend on change only after you approve each update.
...
- **AWS credentials for `prepare` and `teardown`.** Creating a service's buckets and IAM role, and removing them after the service is deleted, run under your credentials on your workstation or the connector host. The executor never holds credentials to your buckets or IAM. Its only cloud call is an Amazon ECR login with the host's or pod's ambient AWS identity, made when a platform sync or create pulls a chart.
