...
Prepares everything on your side that a managed service create needs, in one idempotent run that stops at the first failure. It picks or validates the service name, creates the data and backup buckets and the IAM role (or verifies a role you bring), and renders and applies the executor's access bundle for the service namespace. It then mints the `default` user's password (shown once) and writes the create body for [`instances create`](#instances-create). See [managed services](/products/bring-your-own-cloud/connector/managed-services#create-a-service).
