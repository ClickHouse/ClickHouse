...
This page is the security reference for the ClickHouse Connector: the identities it uses, the complete set of connections it opens, the exact privileges it holds, what it structurally cannot do, and how every action is attributed. For how the pieces fit together, see [architecture](/products/bring-your-own-cloud/connector/architecture).

## Identities {#identities}

<Image img="/images/cloud/reference/byoc-connector-identity-model.svg" size="lg" alt="ClickHouse Connector identity model" />

- **The connector's mTLS client certificate**, with your organization ID as its common name, authenticates every daemon to your connector endpoint. `clicklink clctl instances create` and `clicklink clctl platform approve` use the same certificate and HMAC key when they call the endpoint.
...
- **`pcm-executor`** is the executor's Kubernetes ServiceAccount for the lifecycle of managed services. Its writes are confined to service namespaces by a ValidatingAdmissionPolicy.
- **`pcm-platform`** is the executor's identity for platform updates. Its token is minted by your `clicklink clctl platform approve` for 2 hours and never renewed; between approvals the executor cannot touch the platform layer.
...
|-------------|----------|---------|
| Your connector endpoint | HTTPS with mTLS, every request HMAC-signed | `POST /v1/metrics`, `/v1/self-metrics`, `/v1/status`, `/v1/instance/sync`, `/v1/infra/sync`, `/v1/backup/sync`, `/v1/pcm/cert/renew`; from `clicklink clctl`, `POST /v1/pcm/instances` (submit a service definition) and `GET /v1/pcm/platform/bundles/{digest}` (fetch a proposed platform bundle) |
...
Inbound, each daemon exposes one local health port, which also serves its Prometheus metrics, plus the opt-in session gateway. The executor's local command API binds to loopback only. Nothing else listens, and ClickHouse Cloud never connects into your environment: it can only answer the two outbound WebSockets.

### ClickHouse grants {#clickhouse-grants}

Provisioning creates one read-only user per observing component. The single exception to pure reads is the scraper's `SYSTEM FLUSH LOGS` grant listed below, which cannot read or modify anything; it only forces log tables to persist entries they already buffer. The users are created with `IDENTIFIED WITH bcrypt_hash`, so only a salted bcrypt hash exists in the provisioning SQL; the plaintext password lives only in the credential file the daemon reads at runtime. The executor has no ClickHouse user and never connects to ClickHouse. The grants are exactly these, with the default table sets:
...
For the scraper and troubleshooter the chart creates namespace-scoped Roles only; there is no ClusterRole or ClusterRoleBinding.
...
| `roles`, `rolebindings` (`rbac.authorization.k8s.io`) | `get`, `create`, `update`, `patch` | The scraper's read-only access inside each service namespace; Kubernetes escalation prevention limits these to permissions the executor already holds |
| `serviceaccounts/token` | `create` | Exact names only: `pcm-executor` and the scraper's ServiceAccount, for token renewal |
| `configmaps` | `get`, `update`, `patch`, `create` | The `clicklink-instance-registry` ConfigMap in the connector namespace, by name |
...
- **No writes to ClickHouse data or state.** The grants above contain no `INSERT`, no DDL, and no user-management, settings, or process-control privileges; the single system-class grant, the scraper's `SYSTEM FLUSH LOGS`, only makes log tables persist what they already buffer. The executor has no ClickHouse user at all. No component can modify data, schemas, users, or settings through ClickHouse.
- **No exec.** No RBAC contains `pods/exec`; no component can run commands in your pods.
- **No writes outside service and platform namespaces.** The scraper and troubleshooter hold no `delete` or `patch`; their only mutations are the exact-name `update` on the connector's own mTLS Secret and `create` on `serviceaccounts/token`, which mints short-lived tokens for their own ServiceAccounts and modifies no stored object. The executor's writes are denied by admission outside the service prefix, and its platform writes exist only while a token you minted is valid.
- **No permission to grant permissions.** Namespaces, CRDs, RBAC, admission configurations, and StorageClasses for the platform are applied by your credentials during `approve`; the executor's platform identity cannot escalate, bind, or impersonate. Inside a service namespace the executor can create only Roles that grant permissions it already holds.
- **No access to your cloud resources.** The executor holds no permission on your buckets or IAM. Buckets and the per-service role are created by `clicklink clctl executor prepare` and removed by `clicklink clctl executor teardown` under your credentials, and the role is assumable only by that service's pods. The executor cannot read or delete your data.
- **No knowledge of your default user password.** `clicklink clctl instances create` generates it on your side and shows it once; only its hashes are submitted, so neither the connector nor ClickHouse Cloud can recover it.
- **Nothing inbound.** ClickHouse Cloud never opens a connection into your environment. The only command paths are the two outbound WebSockets: the troubleshooter refuses every command unless a support session you enabled is active, and the executor accepts lifecycle commands only for the services you asked ClickHouse to manage, applied within the scopes above. Even during a session, the troubleshooter's reach is bounded on both sides: ClickHouse queries are limited to the table allowlist, with `query_log` and `text_log` denied by the validator regardless of configuration, and Kubernetes access is separately limited to the read-only views and pod logs the namespace-scoped Roles grant.
...
- **Network egress.** Under an enforcing CNI, no daemon has egress until you allowlist your connector endpoint's CIDRs in the chart's NetworkPolicy (`networkPolicy.allowEgressCIDRs`); the executor also needs the Kubernetes API server CIDRs (`networkPolicy.apiserverCIDRs`), which `init --managed` derives from the cluster's VPC.
...
- **Lifecycle commands.** The executor records every command it receives in its local store with its origin (ClickHouse Cloud or a local `clicklink clctl` call), its stage, and its result, and reports the result back over the command channel. Read the record with `clicklink clctl commands list` and `clicklink clctl commands get`; see the [CLI reference](/products/bring-your-own-cloud/connector/reference/cli).
...
- **Audit trail.** Every gateway call and every troubleshoot command, accepted or blocked, is appended to the NDJSON audit log: gateway entries with the attested operator email, VM local session changes with the invoking host user, and session commands with the org identity carried on the authenticated channel. Read it with `clicklink clctl troubleshoot audit tail`; see the [CLI reference](/products/bring-your-own-cloud/connector/reference/cli).
