...
- **The ambient AWS identity of the executor's node or host**, the read-only pull role you registered, serves one call only: an Amazon ECR login when a platform sync or create pulls a chart from ECR. Image pulls happen on your nodes under their own pull role. The executor holds no credentials for your buckets or IAM.
