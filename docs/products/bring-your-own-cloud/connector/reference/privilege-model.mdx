...
- **The ambient AWS identity of the executor's node or host**, the read-only pull role you registered, serves one purpose: an Amazon ECR login, and the STS call behind it, when a platform sync or create pulls a chart from ECR. No S3 or IAM calls. Image pulls happen on your nodes under their own pull role. The executor holds no credentials for your buckets or IAM.
