...
- **The ambient AWS identity of the executor's node or host**, the read-only pull role you registered, serves one call only: an Amazon ECR login to pull charts and images. The executor holds no credentials for your buckets or IAM.
...
| Amazon ECR | HTTPS, ambient AWS identity of the executor's node or host | Registry login when a platform sync or create pulls a chart from ECR; the executor's only cloud API call |
...
- **No credentials for your buckets or IAM.** `clicklink clctl executor prepare` creates the buckets and the per-service role, and `clicklink clctl executor teardown` removes them, both under your credentials. The role is assumable only by that service's pods. The executor uses the ambient AWS identity of its node or host (the read-only pull role you registered) only to obtain Amazon ECR tokens for chart and image pulls. It cannot read or delete your data.
