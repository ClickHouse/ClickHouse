...
- **The ambient AWS identity of the executor's node or host** serves two purposes. It logs in to Amazon ECR, through STS, when a platform sync or create pulls a chart. It assumes the read-only pull role you registered for the image check during a platform update. No S3 or IAM calls. Image pulls happen on your nodes under their own pull role. The executor holds no credentials for your buckets or IAM.
...
| Amazon ECR | HTTPS, the executor's ambient AWS identity, or the read-only pull role it assumes for the image check | Registry login when a platform sync or create pulls a chart from ECR; `DescribeImages` during a platform update to confirm the images exist |
| AWS STS | HTTPS, ambient AWS identity of the executor's node or host | Credentials for the ECR login, and the assume-role into the read-only pull role you registered for the image check. ECR and STS are the executor's only cloud API calls |
