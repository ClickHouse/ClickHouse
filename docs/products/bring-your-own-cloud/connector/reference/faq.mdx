...
The managed-mode enrollment, customer-create, and platform-approval workflows require [support confirmed by your account team](/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability); they are unavailable in public `v0.16.0`.
...
<Note>
The public `v0.16.0` CLI has neither `--managed` nor `--no-managed`. The install-time choice below requires a build with [managed-mode support](/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability). Stopping an existing executor uses the Helm or systemd instructions below and does not require either init flag.
</Note>
...
Customer service creation requires [matching CLI and deployment support](/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) and is unavailable in public `v0.16.0`.
...
- With a [build that supports platform approval](/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability), platform bundles can be handed over out of band and passed to `clicklink clctl platform approve --bundle` as a file. Public `v0.16.0` has no `platform` command.
