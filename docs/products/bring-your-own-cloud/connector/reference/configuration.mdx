...
On a Linux VM the connector reads `/etc/clicklink/config.yaml`. `clicklink clctl init` writes it and keeps it across re-runs unless you pass `--force`. Keys omitted from the file fall back to the compiled defaults below. Credentials never live in this file.
...
| `api.access_key`, `api.secret_key` | unset | Inline HMAC keys, the alternative to the `*_file` keys. The daemons refuse a file with both shapes set. |
