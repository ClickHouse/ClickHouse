...
In order to use PrivateLink, Private Service Connect, or Private Link, the Endpoint ID of your client connection must be explicitly allowed for each ClickHouse service. Add it yourself in the ClickHouse Cloud console: open the service's **Settings** page, click **Set up private endpoint**, and register the Endpoint ID — allowlisting is per service, so repeat this for each service you connect to (see the [AWS](/products/bring-your-own-cloud/onboarding/network-aws) and [GCP](/products/bring-your-own-cloud/onboarding/network-gcp) guides for the full flow). Contact ClickHouse Support if you need assistance.
