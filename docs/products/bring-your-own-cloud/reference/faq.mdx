...
The minimum differs by cloud. On AWS the floor is `/23`, but that yields `/25` private subnets and limits you to roughly 30 server pods, so only choose it if you are confident the deployment will stay that small. On GCP the minimum is `/20`, because half the range is assigned to the GKE cluster pod range, which must be at least `/21` for the cluster to provision.
</Accordion>
