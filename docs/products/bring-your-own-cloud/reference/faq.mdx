...
In the cloud account: yes, as long as your resources do not touch ClickHouse-provisioned ones (on AWS these all carry the tag `clickhouse-byoc=true`). Keep in mind that on GCP and Azure the onboarding identities hold project- or subscription-scoped permissions, so anything you co-locate there falls within ClickHouse's permission scope — a dedicated account, project, or subscription remains the strong recommendation.
