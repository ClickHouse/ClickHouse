...
BYOC deployments include comprehensive observability capabilities, allowing you to monitor your ClickHouse services through a dedicated Prometheus monitoring stack, and direct metric endpoints from ClickHouse Servers. Your monitoring data remains within your cloud account, giving you complete control over your monitoring infrastructure; the only exceptions are the bounded usage and health telemetry listed in [network boundaries](/products/bring-your-own-cloud/reference/network-security#outbound-connections).
