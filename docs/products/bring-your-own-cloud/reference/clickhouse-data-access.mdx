...
Query traffic, table contents, and schemas never flow through these channels. Your full monitoring dataset — the Prometheus and Thanos stack, and your logs — stays inside your BYOC network; the bounded telemetry in the table above is the only observability data that leaves. See [network boundaries](/products/bring-your-own-cloud/reference/network-security#network-boundaries) for the complete inbound and outbound flow reference.
