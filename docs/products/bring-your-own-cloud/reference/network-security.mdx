...
| --- | --- | --- | --- | --- |
| Public client ingress load balancer (on by default) | TCP 8443 (HTTPS interface) and 9440 (native protocol over TLS); 443 also routes to the HTTPS interface | Your ClickHouse clients | Query traffic. TLS terminates at the Istio ingress gateway inside your own network | IP access list; can be disabled entirely once a private path is in place |
...
- **Flow logs** (VPC Flow Logs, VPC flow logs, or NSG flow logs): every connection in the tables above, including the Tailscale channel and telemetry egress. Enable these yourself if you want them retained.
