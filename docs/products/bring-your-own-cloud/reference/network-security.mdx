...
All egress from your BYOC network leaves through a small, fixed set of NAT addresses on every cloud, so you can pin them in your own egress controls. Ask your ClickHouse team for the current values for your deployment.

### Inbound connections {#inbound-connections}

| Listener | Ports | Source | Purpose | Your controls |
| --- | --- | --- | --- | --- |
| Public client ingress load balancer (on by default) | TCP 8443 and 443 (HTTPS), 9440 (native protocol over TLS), 3306 (MySQL wire protocol, where enabled), 15021 (load balancer health check) | Your ClickHouse clients | Query traffic. TLS terminates at the Istio ingress gateway inside your own network | IP access list; can be disabled entirely once a private path is in place |
| Private client ingress load balancer (optional) | Same ports as above | Networks you nominate | Query traffic from peered networks | Source-range allowlist you control |
...
By default, ingress is publicly reachable with IP allow list filtering. You can add a private path — see [Private networking setup](/products/bring-your-own-cloud/onboarding/network) — and then disable public access. We strongly recommend configuring an [IP filter](/products/cloud/guides/security/connectivity/setting-ip-filters) in every case.

<Note>
There are no other inbound listeners. In particular there is no SSH, no bastion host, and no standing administrative credential. Query traffic never traverses ClickHouse-owned infrastructure in either direction: your clients connect straight to the ingress inside your own network.
</Note>
...
<Note>
Only Kubernetes API and troubleshooting traffic can be moved onto a private path. Cloud provider API calls always originate from ClickHouse Cloud's network — see [Cloud provider APIs vs the Kubernetes API](#cloud-api-vs-kubernetes-api).
</Note>
...
| Your own object storage | TCP 443 | ClickHouse servers, backup jobs | Data parts and backups | Your data — it stays in your account. Same-region traffic uses the private path in [Provider equivalents](#provider-equivalents) |
...
Metrics and logs themselves stay inside your BYOC network on every cloud. Metrics use a Prometheus and Thanos stack running in your cluster, with optional long-term retention in a bucket in your own account. Logs are currently written to the node volumes attached to your ClickHouse nodes; in a future update they will be written to LogHouse, a ClickHouse-based log store that also runs inside your BYOC network. ClickHouse engineers query both in place over Tailscale during support; Prometheus remote-write to ClickHouse Cloud is disabled in BYOC builds.
...
Because the whole data plane runs in your own account, your existing tooling observes every flow on this page:
...
- **Flow logs** (VPC Flow Logs, VPC flow logs, or NSG flow logs): every connection in the tables above, including the Tailscale channel and telemetry egress.
- **Kubernetes audit logs**, delivered to your provider's logging service in your own account.
- **Object storage access logs** on your data and backup buckets.
