...
The full monitoring dataset stays in your own account on every cloud. The reduced usage and health telemetry described under [Billing scraper](#billing-scraper), plus these alert payloads, are the only observability data that reaches ClickHouse-owned infrastructure. Metrics use a Prometheus and Thanos stack running in your cluster, with optional long-term retention in a bucket in your own account; where you enable that retention, the writes leave your BYOC network for the provider's storage API over the private path in [Provider equivalents](#provider-equivalents), but the data stays within your account. Logs are currently written to the node volumes attached to your ClickHouse nodes; in a future update they will be written to LogHouse, a ClickHouse-based log store that also runs inside your BYOC network. ClickHouse engineers query both in place over Tailscale during support; Prometheus remote-write to ClickHouse Cloud is disabled in BYOC builds.
