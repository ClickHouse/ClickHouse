...
- In this case, management services access the API server via Tailscale (similar to human troubleshooting access) or over the cloud provider’s own path — AWS VPC Lattice, the GKE DNS-based endpoint, or Azure Private Link (see [Kubernetes API Private Connection](/products/bring-your-own-cloud/configuration/configurations#k8s-api-private-connection))
