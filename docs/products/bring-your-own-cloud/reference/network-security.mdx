...
| **Your audit trail** | AWS CloudTrail (or the GCP/Azure equivalents) in your account records every call with the assumed identity | Managed Kubernetes control-plane logs, where enabled — on AWS, EKS control-plane logs are delivered to a CloudWatch log group in your account (see [billable AWS services](/products/bring-your-own-cloud/reference/billable-aws-services)); contact support to confirm or enable audit logging for your cluster |
