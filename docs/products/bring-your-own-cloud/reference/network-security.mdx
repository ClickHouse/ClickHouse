...
| Your own object storage | TCP 443 | ClickHouse servers, backup jobs, and the monitoring stack where long-term retention is enabled | Data parts, backups, and long-term metric retention | Your data — it stays in your account. Same-region traffic uses the private path in [Provider equivalents](#provider-equivalents). The monitoring stack writes with its own identity, documented in [privilege](/products/bring-your-own-cloud/reference/privilege) |
