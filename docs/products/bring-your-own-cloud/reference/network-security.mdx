...
The full monitoring dataset stays inside your BYOC network on every cloud. The reduced usage and health telemetry described under [Billing scraper](#billing-scraper), plus these alert payloads, are the only observability data that crosses the boundary. Metrics use a Prometheus and Thanos stack running in your cluster, with optional long-term retention in a bucket in your own account. Logs are currently written to the node volumes attached to your ClickHouse nodes; in a future update they will be written to LogHouse, a ClickHouse-based log store that also runs inside your BYOC network. ClickHouse engineers query both in place over Tailscale during support; Prometheus remote-write to ClickHouse Cloud is disabled in BYOC builds.
