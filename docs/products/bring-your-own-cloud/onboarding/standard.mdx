...
When you click **Set up Infrastructure**, ClickHouse Cloud automatically runs a set of **read-only** pre-flight checks against your account before creating any resources — you don't trigger them yourself. If everything passes, provisioning continues and nothing extra is shown. If a check fails, setup is halted and the console shows the specific problems so you can fix them and start setup again.
...
If setup is halted, use the table below to resolve the reported error, then start setup again.

<Note>
The pre-flight checks are read-only and never modify your account. They run automatically as part of infrastructure setup — there is no separate validation button.
</Note>
