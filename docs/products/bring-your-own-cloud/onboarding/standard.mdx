...
<Step title="Validate your cloud setup" id="validate-cloud-setup">
Before provisioning begins, run **pre-flight validation** from the console to confirm your account is configured correctly. ClickHouse Cloud runs a set of **read-only** checks against your AWS account, GCP project, or Azure subscription — it makes no changes — and reports any problems up front, so you can fix them before provisioning rather than discovering them after a failed deployment.

Select **Validate cloud setup** once the initial stack (CloudFormation/Terraform) has been created. Validation confirms that:

- **Permissions** — the management identity (IAM role / service account / service principal) created by the onboarding stack holds the permissions ClickHouse Cloud needs to provision and operate your deployment.
...
- **Network** *(AWS bring-your-own-VPC)* — your VPC and subnets meet the [network requirements](/products/bring-your-own-cloud/onboarding/network): subnet sizes, required tags, availability-zone spread, outbound connectivity, and available IP addresses.
...
If a check fails, the report names the specific problem. Use the table below to resolve it, then re-run validation.

<Note>
Pre-flight validation is read-only and safe to re-run as often as you like. Passing validation confirms your setup is ready — it does not itself provision any resources.
</Note>

| Validation result | Likely cause | How to resolve |
| --- | --- | --- |
| **Cannot assume the management role / impersonation failed** | The onboarding stack wasn't completed, or the role/service account or external ID doesn't match | Re-run the latest onboarding stack and confirm the [external ID](#aws-external-id) shown in the console matches the one in your trust policy |
