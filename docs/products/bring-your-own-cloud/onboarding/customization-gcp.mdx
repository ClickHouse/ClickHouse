...
<Note>
When you click **Set up Infrastructure**, ClickHouse Cloud automatically runs [pre-flight validation](/products/bring-your-own-cloud/onboarding/standard#preflight-validation) before provisioning, checking that the management service account has the required permissions and the required Google Cloud APIs are enabled. Automatic validation of a bring-your-own VPC is not yet part of GCP pre-flight, so make sure your VPC meets the [network requirements](#configure-existing-vpc) above before you begin.
</Note>
