...
<Note>
These resources are available in provider version 3.24.0 and later. They are in beta and their behavior may change in future provider versions. Terraform prints a `Beta Resource` warning for them at plan time.
</Note>
