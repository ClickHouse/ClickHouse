...
In ClickHouse Cloud, [`alter_sync`](/reference/settings/session-settings/alter#alter_sync) defaults to `0`; set it when a client must wait for metadata operations. For mutation-producing `ALTER` operations, use [`mutations_sync`](/reference/settings/session-settings/mutations#mutations_sync) to control waiting and monitor progress in [`system.mutations`](/reference/system-tables/mutations).
