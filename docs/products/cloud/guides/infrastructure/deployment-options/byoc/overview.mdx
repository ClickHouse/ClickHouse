...
- **Network basics.** The VPC/VNet needs a CIDR range of at least `/23`, working DNS resolution, and outbound internet access (directly or via NAT) — including when you [bring your own VPC](/products/bring-your-own-cloud/onboarding/customization-aws). `/23` is the floor, not a recommendation: every replica consumes IP addresses, so larger deployments need a larger range — reserve at least `/22` if you expect to scale (the ClickHouse-managed default is `10.0.0.0/16`).
