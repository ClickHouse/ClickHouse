...
Bootstraps the connector from an enrollment token, a saved enrollment bundle, or an out-of-band signed certificate. One invocation stages configuration, provisions ClickHouse access, obtains the mTLS client certificate, deploys (Helm chart or systemd units), and verifies health. Re-running is safe: config and the cluster UUID are preserved, credentials are overwritten atomically, and an existing client key is reused unless you pass `--force`. See [onboarding](/products/cloud/guides/infrastructure/deployment-options/byoc/connector/onboarding) for the full flow.
...
| `--no-gateway` | Disables support sessions (the troubleshooter gateway); skips the prompt. |
...
Enables, disables, and inspects the support session: the time-boxed window during which the troubleshooter accepts commands. When no session is active, the daemon refuses all commands even while its WebSocket is connected. See [support sessions](/products/cloud/guides/infrastructure/deployment-options/byoc/connector/support-sessions).
...
| `--gateway-audience <aud>` | Audience claim the OIDC token is bound to. Defaults to the scheme and host of `--gateway-url`, which does not match the gateway's default required audience (`clicklink-clctl`), so pass `--gateway-audience clicklink-clctl` explicitly unless you reconfigured the gateway. |
