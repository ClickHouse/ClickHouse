...
description: 'Every BYOC connector configuration key: the VM config.yaml and the Helm chart values, with defaults'
keywords: ['BYOC', 'cloud', 'bring your own cloud', 'connector', 'configuration', 'reference']
doc_type: 'reference'
---

This page lists every configuration key you may need to touch, with its default and meaning: first the VM configuration file, then the Helm chart values. For task-oriented guidance, see the [configuration guide](/products/cloud/guides/infrastructure/deployment-options/byoc/connector/configuration).
...
| `troubleshooter.gateway.tls.enabled` | `false` | Serve the gateway over TLS. Enable on a VM so the bearer API is not served in cleartext. |
...
The session gateway for [support sessions](/products/cloud/guides/infrastructure/deployment-options/byoc/connector/support-sessions).
...
| `clctl.gateway.audience` | `clicklink-clctl` | Audience claim required on operator tokens; must match what operators pass (or derive) as `--gateway-audience`. |
