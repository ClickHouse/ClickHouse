...
| The Kubernetes API server | HTTPS | Namespace-scoped reads, ServiceAccount token requests, and exact-name read and update of the connector's own mTLS Secret to persist renewed certificates |
...
- **Audit trail.** Every gateway call and every troubleshoot command, accepted or blocked, is appended to the NDJSON audit log: gateway entries with the attested operator email, VM local session changes with the invoking host user, and session commands with the org identity carried on the authenticated channel. Read it with `clicklink clctl troubleshoot audit tail`; see the [CLI reference](/products/cloud/guides/infrastructure/deployment-options/byoc/connector/reference/cli).
...
- **The troubleshooter reads only allowlisted tables**, and the validator denies `query_log` and `text_log` unconditionally, so query history is never readable. The default allowlist does include `system.processes` (live query text); trim `allowedTables` if that must stay hidden during sessions.
