...
Provisioning creates one least-privilege user per component, read-only except for the scraper's single log-flush grant listed below. The users are created with `IDENTIFIED WITH bcrypt_hash`, so only a salted bcrypt hash exists in the provisioning SQL; the plaintext password lives only in the credential file the daemon reads at runtime. The grants are exactly these, with the default table sets:
