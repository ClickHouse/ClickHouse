...
| The Kubernetes API server | HTTPS | Namespace-scoped reads and ServiceAccount token requests on both install targets; on Kubernetes installs only, exact-name read and update of the connector's own mTLS Secret to persist renewed certificates (on a VM the renewed certificate is written to the local TLS files instead) |
