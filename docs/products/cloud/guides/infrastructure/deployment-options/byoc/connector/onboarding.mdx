...
**Out-of-band certificate signing.** When the enrollment endpoint is unreachable from where you run `init`, add `--no-auto-sign`: `init` stages everything and writes `clicklink.csr`. Send the CSR to ClickHouse through your account team, then complete the install with the returned certificate and chain: `clicklink clctl init --signed-cert client.crt --chain ca-chain.crt` (on Kubernetes, the staged run prints the full completion command including `--target helm`). Only the CSR travels; the private key never leaves your environment.
