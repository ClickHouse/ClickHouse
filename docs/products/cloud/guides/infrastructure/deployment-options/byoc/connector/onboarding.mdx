...
description: 'Install the BYOC connector and enroll it on Kubernetes or a Linux VM'
...
- **Egress on port 443** to `https://<subdomain>.<connector-domain>` and `https://<subdomain>.enroll.<connector-domain>`, plus `releases.clicklink.clickhouse.com` and Amazon ECR Public at install time. This applies to the standard enrollment path; when any of these are unreachable, every artifact and step has an out-of-band alternative, see [air-gapped and mirrored installs](#air-gapped-and-mirrored-installs).
...
- **cosign** wherever you download release artifacts, to verify signatures.
...
- **Optionally, a kubeconfig** (or `--server` and `--ca-data`) if the connector should generate Kubernetes access bundles.
...
Download the release tarball together with its checksum, signature, and signing certificate. For a Kubernetes install, run the CLI from any workstation with kubeconfig access to the cluster; macOS (`darwin`) workstation builds are available. For a VM install, download the Linux tarball for the host's architecture onto the host itself.
...
The block detects your platform and architecture; run it as is on macOS or Linux, amd64 or arm64.
...
On a VM, keep the verified tarball on the host; the next step extracts it and runs the installer.
</Step>
...
<Image img="/images/cloud/reference/byoc-connector-enrollment-flow.svg" size="lg" alt="BYOC connector enrollment flow" />
...
For unattended runs, answer the prompts with flags instead:
...
clicklink clctl init --enroll https://<subdomain>.<connector-domain> --target helm \
  ...
  --ch-admin-password-stdin < <password-file>
```
...
<Tab title="Linux VM">
On the host, extract the verified Linux tarball and run the installer as root:
...
`install.sh` is idempotent: it creates the `clicklink` system user, the `/etc/clicklink`, `/var/lib/clicklink`, and `/var/log/clicklink` directories, installs the binary to `/usr/local/bin/clicklink` (backing up any existing binary), installs the scraper and troubleshooter systemd units, and seeds a default `/etc/clicklink/redaction-patterns.yaml` (preserved if one exists).
...
The enrollment bundle `handoff.yaml` (written with mode `0600` in the working directory) exists so that re-runs and recovery never need a second token. It contains the connector's API secret, so once the install has verified, delete it:
...
If your environment cannot reach the enrollment endpoint, ClickHouse can provide the enrollment bundle directly; run `clicklink clctl init --handoff <bundle-file>` in place of `--enroll` and the rest of the flow is identical.
...
A manual CSR signing path also exists via `--no-auto-sign`; see the [CLI reference](/products/cloud/guides/infrastructure/deployment-options/byoc/connector/reference/cli).
...
If enrollment is refused, the token was either already redeemed (re-run with `--handoff handoff.yaml`) or is invalid or expired (contact your ClickHouse account team for a fresh token). If enrollment fails with a transport error, the token was not spent; re-run the same command.

`--force` is the explicit reset, not a routine retry: it overwrites the kept config or values overlay, regenerates the client key, and supersedes an unexpired client certificate (a `409` from the signing endpoint means one already exists). The connector's cluster UUID is preserved even across `--force`, so a re-initialized connector keeps its identity. Reach for it when rotating credentials or superseding a certificate, and see [operations](/products/cloud/guides/infrastructure/deployment-options/byoc/connector/operations) for the full re-run and recovery model.
