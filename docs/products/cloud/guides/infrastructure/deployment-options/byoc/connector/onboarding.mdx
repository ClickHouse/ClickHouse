...
- **Admin access to a Kubernetes API server** for provisioning: a kubeconfig on the host, or `--server` and `--ca-data`, or the prompts. Access bundles are anchored to Kubernetes ServiceAccounts on both targets, so every provisioning run needs this; the only opt-out is `--skip-provision`, which is stage-only: it skips ClickHouse user provisioning, and on a VM also the unit enable and verification, so it does not produce a running connector by itself.
