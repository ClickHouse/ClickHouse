...
- **Read-only Kubernetes views** on any deployment provisioned with Kubernetes access bundles (always present on Kubernetes installs; on a VM only when you supplied a Kubernetes API server during provisioning): `get`, `list`, and `watch` on pods, pod logs, services, configmaps, events, PersistentVolumeClaims, deployments, statefulsets, and replicasets in the granted namespaces. On deployments without Kubernetes access, the troubleshooter refuses kubectl-type commands outright.
