...
| Your ClickHouse clusters | Outbound, inside your environment | ClickHouse native protocol | Dedicated least-privilege users `pcm_scraper` and `pcm_troubleshooter`, stored as bcrypt hashes | System-table reads for scraping and session diagnostics, plus the scraper's log-table flush |
...
For the connector's exact grants and RBAC rules, see the [privilege model](/products/cloud/guides/infrastructure/deployment-options/byoc/connector/reference/privilege-model) reference. If ClickHouse operates the clusters for you instead, the trust model is different; see the [BYOC architecture](/products/bring-your-own-cloud/overview/architecture) and [BYOC privilege](/products/bring-your-own-cloud/reference/privilege) pages.
