...
| Kubernetes API server (every provisioned deployment: access bundles are anchored to Kubernetes ServiceAccounts on both targets) | Outbound, inside your environment | HTTPS | ServiceAccounts bound to namespace-scoped Roles | Read-only workload views for the troubleshooter; on Kubernetes installs both daemons also persist the automatically renewed client certificate back to the mTLS Secret |
