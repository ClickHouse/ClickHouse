...
| Kubernetes API server (whenever Kubernetes access bundles are provisioned: always on Kubernetes installs, optional on a VM) | Outbound, inside your environment | HTTPS | ServiceAccounts bound to namespace-scoped Roles | Read-only workload views for the troubleshooter; on Kubernetes installs both daemons also persist the automatically renewed client certificate back to the mTLS Secret |
