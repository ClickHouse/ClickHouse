...
The block reapplies your edited values at the chart version already installed, so a configuration change never doubles as an unplanned upgrade; moving to a new version is a deliberate step covered in [operations](/products/cloud/guides/infrastructure/deployment-options/byoc/connector/operations). On a mirrored install, swap `--repo` for your mirror as described in [private mirrors](#private-mirrors).
</Tab>
<Tab title="Linux VM">
