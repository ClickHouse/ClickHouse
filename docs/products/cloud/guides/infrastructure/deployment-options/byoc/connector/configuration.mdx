...
description: 'Configure the BYOC connector: ClickHouse instances, operator allowlists, network policy, redaction, private mirrors, and storage'
keywords: ['BYOC', 'cloud', 'bring your own cloud', 'connector', 'configuration']
doc_type: 'guide'
---

This page covers the configuration changes you are most likely to make after installing the BYOC connector. For every key with its default and meaning, see the [configuration reference](/products/cloud/guides/infrastructure/deployment-options/byoc/connector/reference/configuration); for command flags, see the [CLI reference](/products/cloud/guides/infrastructure/deployment-options/byoc/connector/reference/cli).
...
Then wire the Secret and ServiceAccount pair each command creates into the matching `accessBundles` map and run the `helm upgrade` shown above:
...
Support sessions are gated by an allowlist of operator email addresses: every request to the session gateway must carry a short-lived OIDC ID token whose attested email is on the list. An empty allowlist is closed, so nobody can open a session. See [support sessions](/products/cloud/guides/infrastructure/deployment-options/byoc/connector/support-sessions) for the full trust model.
