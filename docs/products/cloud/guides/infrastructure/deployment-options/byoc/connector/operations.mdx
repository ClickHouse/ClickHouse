...
- **API (HMAC) credentials**: request a fresh enrollment token from your ClickHouse account team, then re-run enrollment with `--force`. On Kubernetes, from your workstation: `clicklink clctl init --enroll https://<subdomain>.<connector-domain> --target helm --force`. On a VM, on the host: `sudo clicklink clctl init --enroll https://<subdomain>.<connector-domain> --force`.
