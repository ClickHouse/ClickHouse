...
- **API (HMAC) credentials**: request a fresh enrollment token from your ClickHouse account team, then re-run your original `init` command with `--enroll` and `--force`, keeping every target-specific flag from the first install (`--target-namespace`, `--values`, and any `--chart`, `--chart-repo`, or `--chart-version` mirror flags on Kubernetes), because `--force` restages the kept configuration. On a default install that is `clicklink clctl init --enroll https://<subdomain>.<connector-domain> --target helm --force` from the workstation, or `sudo clicklink clctl init --enroll https://<subdomain>.<connector-domain> --force` on the VM host.
