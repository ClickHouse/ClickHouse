...
description: 'Day-2 operations for the BYOC connector: upgrades, health, certificates, credential rotation, recovery, and uninstall'
keywords: ['BYOC', 'cloud', 'bring your own cloud', 'connector', 'operations']
doc_type: 'guide'
---

This page covers day-2 operation of the BYOC connector on both install targets. For install and enrollment, see [onboarding](/products/cloud/guides/infrastructure/deployment-options/byoc/connector/onboarding).

## Upgrades {#upgrades}

### Kubernetes {#upgrades-kubernetes}

Upgrade the Helm release from the public chart repository, reusing the values overlay that `init` staged:

```bash
helm upgrade --install clicklink-connector clicklink-connector \
  --repo https://releases.clicklink.clickhouse.com/charts \
  --version <version> \
  -n <connector-namespace> \
...
Alternatively, download and verify the newer CLI and re-run `init`; re-runs converge on the desired state (see [re-runs and recovery](#re-runs-and-recovery)).
...
When support sessions are enabled, the gateway additionally serves HTTPS on port 8443.
...
# Kubernetes
kubectl get secret clicklink-mtls -n <connector-namespace> -o jsonpath='{.data.tls\.crt}' \
  | base64 -d | openssl x509 -noout -enddate
```

## Credential rotation {#credential-rotation}

- **API (HMAC) credentials**: request a fresh enrollment token from your ClickHouse account team, then re-run enrollment with `--force`: `clicklink clctl init --enroll https://<subdomain>.<connector-domain> --force` (add `--target helm` on Kubernetes).
- **ClickHouse users**: re-provision the connector's read-only users (run with `sudo` on a VM):

```bash
clicklink clctl scraper access provision --force
clicklink clctl troubleshoot access provision --force
```
...
```bash
clicklink clctl init --signed-cert client.crt --chain ca-chain.crt
```

On Kubernetes the CLI prints the full form, including `--target helm`, `--target-namespace`, and `--values`; use the printed command as is.
...
```bash
helm uninstall clicklink-connector -n <connector-namespace>
```
...
```bash
kubectl delete secret clicklink-hmac clicklink-mtls -n <connector-namespace>
kubectl delete secret -n <connector-namespace> \
