...
This page covers day-2 operation of the ClickHouse Connector on both install targets. For install and enrollment, see [onboarding](/products/cloud/guides/infrastructure/deployment-options/byoc/connector/onboarding).

## Upgrades {#upgrades}

### Kubernetes {#upgrades-kubernetes}

Day-2 Kubernetes operations use the `helm` CLI on your workstation; only `init` carries a built-in Helm client, so install `helm` before your first upgrade. Upgrade the release from the public chart repository, reusing the values overlay that `init` staged:
...
Download and verify the newer Linux tarball on the host exactly as during [onboarding](/products/cloud/guides/infrastructure/deployment-options/byoc/connector/onboarding), extract it, then re-run the installer and restart the daemons:
...
`install.sh` backs up the previous binary and preserves your live redaction patterns and environment file.
...
- **API (HMAC) credentials**: request a fresh enrollment token from your ClickHouse account team, then re-run your original `init` command with `--enroll` and `--force`, keeping every target-specific flag from the first install (`--target-namespace`, `--values`, and any `--chart`, `--chart-repo`, or `--chart-version` mirror flags on Kubernetes), because `--force` restages the kept configuration. On a default install that is `clicklink clctl init --enroll https://<subdomain>.<connector-domain> --target helm --force` from the workstation, or `sudo clicklink clctl init --enroll https://<subdomain>.<connector-domain> --force` on the VM host. For unattended rotation where SQL provisioning needs a password, stdin carries both secrets in order, the token on the first line and the password on the second (the token read consumes exactly one line): `printf '%s\n' "$ENROLLMENT_TOKEN" "$CH_ADMIN_PASSWORD" | clicklink clctl init --enroll <endpoint> --force --ch-admin-password-stdin ...`.
- **ClickHouse users**: re-provision the connector's read-only users per instance. On Kubernetes, from your workstation:
...
For operator-managed instances, add `--ch-user-via cr` and the pod-selection flags to either form; see the [CLI reference](/products/cloud/guides/infrastructure/deployment-options/byoc/connector/reference/cli).

- **Client certificate**: renewal is automatic (see [certificates](#certificates)). To supersede an unexpired certificate immediately, re-run `init` with `--force`.
