...
<Note>
Access to SQL console and access to the underlying ClickHouse data are separate permission scopes. A custom Cloud role can grant both console access and database permissions; ClickHouse Cloud creates a corresponding database role with the `cloud:` prefix for SQL console passwordless authentication.

To manage both scopes from ClickHouse Cloud, see [Manage custom roles](/products/cloud/guides/security/cloud-access-management/manage-custom-roles). To review predefined Cloud permissions, see [Console roles and permissions](/products/cloud/reference/security/console-roles).
</Note>
