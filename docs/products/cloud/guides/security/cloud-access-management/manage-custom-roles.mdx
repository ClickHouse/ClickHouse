...
Console access and database access are separate permission scopes, but a custom Cloud role can grant both. Console permissions determine which Cloud features a user can open. Database permissions determine which data the user can access through those features. To configure database permissions directly in SQL, see [Manage database permissions](/products/cloud/guides/security/cloud-access-management/common-access-management-queries).
