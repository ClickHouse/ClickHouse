...
## Manage database roles with unified RBAC (beta) {#manage-database-roles-with-unified-rbac}

<BetaBadge/>

In addition to console permissions, custom roles can include data permissions that control access to the databases of selected services. With unified RBAC, you manage console and database access in one place instead of maintaining separate roles in each database.

<Note>
Unified RBAC is in beta and is available for services running ClickHouse version 26.4 and above.
</Note>
