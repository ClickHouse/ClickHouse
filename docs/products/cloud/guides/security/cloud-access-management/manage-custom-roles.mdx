...
Custom database roles on a service depend on [JWT authentication](/products/cloud/guides/security/cloud-access-management/jwt-authentication-setup) and are not applied while it is disabled. Before adding data permissions to a role, navigate to the service, open `Settings`, and find the `JWT authentication` card in the `Security` section. Make sure the `SQL Console` toggle is enabled.
...
<Step title="Add organization and service permissions" id="add-console-permissions">

In the role dialog, click the `Allow permissions` button, select `Organization`, and enable `Access organization`. Click `Allow permissions` again, select `Service`, choose the services the role will grant data permissions to, and enable `View selected services`.

Members of the role need both permissions to sign in to the console and open the service in the SQL console, where the data permissions apply.
</Step>
<Step title="Add data permissions to the role" id="add-data-permissions">

Click `Allow permissions` again and select `Data`.
</Step>
