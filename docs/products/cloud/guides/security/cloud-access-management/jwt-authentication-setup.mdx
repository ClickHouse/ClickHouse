...
<Note>
JWKS-based providers accept **RSA** keys and, from version 26.8, **EC** keys on the P-256, P-384, and P-521 curves. Tokens signed with HMAC (`HS256`) keys cannot be verified against a JWKS endpoint and are rejected. A JWKS document may contain other key types, but at least one usable key must be present.
</Note>
