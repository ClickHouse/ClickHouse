...
<EnterprisePlanFeatureBadge feature="Custom JWT authentication"/>

<Tip>
This guide covers configuring JWKS providers in the Cloud console. To learn how to generate a JWT and what its structure looks like — required claims (`iss`, `aud`, `sub`, `kid`), the roles and grants claims, and how ephemeral users work — see the [JWT Authentication](/operations/external-authenticators/jwt) reference.
</Tip>
...
- The **Admin**, **Service admin**, or **Service manager** role on the service (the `control-plane:service:manage` permission). Members without it see the section as read only.
...
3. On success, the connection runs as an ephemeral user with the `default` user's grants, optionally mapped to roles carried in the token's roles claim.
...
Select **Set up JWT providers** (or **Manage JWT providers** if you already have some) to open the providers panel.

<Image img="/images/cloud/security/jwt/jwt-manage-providers.png" size="lg" alt="Manage JWT providers flyout" force/>
</Step>
<Step title="Fill in the provider details" id="fill-provider-details">

Complete the provider form and select **Save**.

<Image img="/images/cloud/security/jwt/jwt-add-provider-form.png" size="lg" alt="New JWT provider form" force/>
...
| **JWKS URL** | The public HTTPS URL that publishes your RSA signing keys. |
| **Roles claim** (optional) | The token claim to read ClickHouse roles from (for example `clickhouse:roles`). Leave blank to skip role mapping. |
...
<Image img="/images/cloud/security/jwt/jwt-add-another-provider.png" size="lg" alt="Adding another JWT provider" force/>
</Step>
</Steps>
