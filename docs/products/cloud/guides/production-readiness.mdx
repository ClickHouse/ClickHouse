...
ClickHouse Cloud supports SCIM provisioning in private preview with [Okta](/products/cloud/guides/security/cloud-access-management/scim-setup) and [Microsoft Entra ID](/products/cloud/guides/security/cloud-access-management/scim-setup-entra). SCIM automates user and group lifecycle management, so users are created and removed automatically as you assign or unassign them in your identity provider. If you aren't using SCIM, users must be manually removed from the ClickHouse Cloud console after being removed from your IdP — plan for manual user management processes in that case.
