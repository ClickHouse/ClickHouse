...
Because every service in a warehouse shares a single ClickHouse Keeper, this limit exists to protect Keeper stability. 50 is a soft limitation and dependent on your data/workload. The ceiling is higher for those on 26.6+ and we plan to raise this in the future. 
...
Because services in a warehouse share one catalog (the same tables, databases, views, and so on), the [service-level data object limits](#service-limits) below are counted **once per warehouse** rather than multiplied per service. Separate warehouses are isolated from each other, so each warehouse counts its own tables and databases toward these limits.

## Service limits {#service-limits}

The limits below apply to a **standalone service** (and, for data object counts, to that service's storage).

If the service belongs to a **warehouse**, the data object limits (databases, tables, columns, partitions, and parts) instead apply to the warehouse's shared catalog, as described in [Warehouse limits](#warehouse-limits) above — the values are unchanged, but they are measured across the whole warehouse rather than the single service. The remaining limits, such as **query concurrency** and **batch ingest**, stay per-service (or per-replica) in both cases.
