...
ClickHouse Cloud is a fully managed cloud service created by the original creators of ClickHouse, the fastest and most popular open-source columnar online analytical processing database.

With Cloud, infrastructure, maintenance, scaling, and operations are taken care of for you, so that you can focus on what matters most to you, which is building value for your organization and your customers faster.
...
| **On-Demand Compute** | Run eligible SELECT queries on additional workers through your existing ClickHouse Cloud service and endpoint, without resizing the service or provisioning another one. Available in private preview. | ❌ | ✅ |
