...
To use this feature, you need at least one of the following roles in an organization with committed contract credits allocated:
...
The table below the summary contains one row per organization:
...
| **Expires soon** | The organization has only one active prepaid balance, and its expiration date is 60 days away. |
| **Expired** | All prepaid balances on the organization have expired. |
...
- A transfer changes the organizations that consume the credits. It doesn't change the total active credits granted under your committed contract.
