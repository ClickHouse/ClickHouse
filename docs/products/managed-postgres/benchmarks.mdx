...
description: 'Performance benchmarks comparing ClickHouse Managed Postgres with AWS Aurora, RDS, and other managed PostgreSQL services'
...
- **Benchmarked** ClickHouse Managed Postgres against AWS RDS (16k provisioned IOPS) and Aurora IO Optimized using standard [`pgbench`](https://www.postgresql.org/docs/current/pgbench.html) tests
- **Performance**: ClickHouse Managed Postgres, backed by NVMe storage, delivers **4.3-9x faster performance** for IO-intensive workloads and **12% faster** for CPU-bound scenarios
...
**Analysis**: Mixed read/write workloads showcase the most dramatic performance advantages of NVMe storage and represent the **most realistic scenario for fast-growing AI-driven workloads** that require both high-throughput data ingestion and low-latency reads. **ClickHouse Managed Postgres achieved 19.8K TPS with higher concurrency**, demonstrating how NVMe storage scales effectively under load. This is **4.3-4.5x faster than RDS and Aurora**. Network-attached storage solutions struggled with write-heavy operations, with RDS and Aurora maxing out at 4.4K-4.6K TPS despite provisioned capacity and even with Aurora's IO Optimized configuration.
...
**Analysis**: The performance gap widens dramatically for read-intensive workloads that are IO Bound. **ClickHouse Managed Postgres delivered 84.8K TPS**, while RDS with 16,000 provisioned IOPS achieved only 9.4K TPS despite having equivalent compute resources. The key difference: the NVMe storage in ClickHouse Managed Postgres scales with higher concurrency, while network-attached storage remains constrained by provisioned IOPS limits. Even with provisioned IOPS, RDS was still 9x slower than ClickHouse Managed Postgres, demonstrating the critical importance of storage architecture for IO-intensive workloads.
...
**Analysis**: Even in CPU-bound scenarios where disk I/O is minimal, **ClickHouse Managed Postgres led the pack with 36.5K TPS**. Despite both services hitting 100% CPU utilization, the NVMe storage in ClickHouse Managed Postgres delivered superior performance with better cache hit rates. The 12% advantage over RDS demonstrates the efficiency of the underlying infrastructure even when workloads are primarily CPU-bound.
...
Across all three benchmark scenarios, ClickHouse Managed Postgres consistently delivered superior performance:
...
| Aspect                  | NVMe Storage (ClickHouse Managed Postgres)     | Network-Attached Storage (Provisioned IOPS)        |
|-------------------------|-------------------------------------|----------------------------------------------------|
...
Beyond raw performance, ClickHouse Managed Postgres offers superior price-performance:
...
For workloads currently constrained by IOPS limits, switching to ClickHouse Managed Postgres can eliminate the need for expensive provisioned IOPS or IO-optimized configurations while delivering significantly better performance.
...
- [ClickHouse Managed Postgres overview](/products/managed-postgres/overview)
