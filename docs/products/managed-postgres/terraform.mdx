...
| `cloud_provider` | For a standard create | Cloud provider hosting the instance: `aws`, or `gcp` (\[Private Preview\](TODO KG GCP Waitlist HERE)). Omit for a read replica or point-in-time restore (inherited from the source). |
