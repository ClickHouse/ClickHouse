...
ClickHouse Cloud now offers ClickPipes to migrate your external PostgreSQL database into a ClickHouse Managed Postgres service. This built-in integration provides a streamlined experience to connect to your source database, export the schema, import it into ClickHouse Managed Postgres, and set up continuous replication.
...
<Note>
If you run into issues during migration, check the [ClickHouse Managed Postgres Migrations FAQ](/products/managed-postgres/migrations/faq) for common errors and solutions.
</Note>
