...
Once the initial load is complete and replication lag is near zero:

- stop writes to Neon
- perform final data and schema validation
- reset sequences
- update the application connection string to point to ClickHouse Managed Postgres.

Keep Neon available in read-only mode for a short period, in case rollback is required.
Once the new environment is stable, remove the ClickPipe and its replication slot.
</Step>
</Steps>
...
- For serverless workloads that create many short-lived connections, use the [bundled PgBouncer instance](/products/managed-postgres/connection#pgbouncer-connection-pooling).
