...
- **Memory Optimized:** Designed for memory-intensive workloads with a higher memory-to-CPU ratio (1:8). Supports x86-based `r8id` and `r6id` families, and ARM64 (AWS Graviton-based) `r6gd` and `r8gd` families. Best suited for large working sets, high cache hit ratios, and memory-bound database workloads.
...
- **CPU Optimized:** Designed for compute-intensive workloads with a lower memory-to-CPU ratio (1:4). Supports x86-based `m8id` and `m6id` families, and ARM64 (AWS Graviton-based) `m6gd` and `m8gd` families. Best suited for high-concurrency transactional workloads and CPU-bound queries.
