...
`avg`, `min`, `max` and `any` over an `Interval` column return an interval of the same type.
`avg` averages the number of units and rounds the result to a whole number of them:
...
Intervals of different units are not commensurable - a month is not a fixed number of days - so
there is no conversion between them and every interval type is aggregated on its own.
...
- [avg](/reference/functions/aggregate-functions/avg) aggregate function
