...
Values of different units are brought to their common unit before they are aggregated, so
summing a minute and a second gives 61 seconds. `Month`, `Quarter` and `Year` have no common
...
- [sum](/reference/functions/aggregate-functions/sum) aggregate function
