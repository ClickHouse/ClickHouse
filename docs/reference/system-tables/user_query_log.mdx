...
The columns are the same as in [`system.query_log`](/reference/system-tables/query_log), except that `LowCardinality` wrappers are removed from the exposed column types.
