...
Number of threads in the background query thread pool.

### BackgroundQueryThreadsActive {#backgroundquerythreadsactive}

Number of threads in the background query thread pool running a task.

### BackgroundQueryThreadsScheduled {#backgroundquerythreadsscheduled}

Number of queued or active jobs in the background query thread pool.
...
Limit on number of tasks in BackgroundStreamingSchedulePool for streaming queries tasks

### BackgroundStreamingSchedulePoolTask {#backgroundstreamingschedulepooltask}

Number of active tasks in BackgroundStreamingSchedulePool for streaming queries tasks
...
Total size of the encryption header cache in bytes

### EncryptionHeaderCacheEntries {#encryptionheadercacheentries}

Total number of entries in the encryption header cache
...
Sum size of currently allocated fiber stacks.

### FiberStacks {#fiberstacks}

Number of currently allocated fiber stacks (fibers are used for asynchronous communication with remote replicas).
...
Number of threads in the filesystem cache pool for parallel keys removal on SYSTEM DROP FILESYSTEM CACHE

### FilesystemCacheDropCacheThreadsActive {#filesystemcachedropcachethreadsactive}

Number of threads in the filesystem cache keys removal pool running a task

### FilesystemCacheDropCacheThreadsScheduled {#filesystemcachedropcachethreadsscheduled}

Number of queued or active jobs in the filesystem cache keys removal pool
...
Number of background tasks for removing data files superseded by Iceberg compaction, which are scheduled or still retained by Iceberg tables.

### IcebergCompactionMerge {#icebergcompactionmerge}

Number of executing background Iceberg compaction merges.
...
Number of input source files (data files and position delete files) participating in currently executing Iceberg compaction merges.
...
Number of blocks in the Keeper storage block cache.

### KeeperBlockCacheBytes {#keeperblockcachebytes}

Total size of blocks in the Keeper storage block cache, in bytes.

### KeeperChangelogReadAheadThreads {#keeperchangelogreadaheadthreads}

Number of threads in the threadpool for Keeper changelog read-ahead fill tasks.

### KeeperChangelogReadAheadThreadsActive {#keeperchangelogreadaheadthreadsactive}

Number of active threads in the threadpool for Keeper changelog read-ahead fill tasks.

### KeeperChangelogReadAheadThreadsScheduled {#keeperchangelogreadaheadthreadsscheduled}

Number of queued or active jobs in the threadpool for Keeper changelog read-ahead fill tasks.

### KeeperChangelogStartupReadThreads {#keeperchangelogstartupreadthreads}

Number of threads in the threadpool for Keeper changelog parallel startup reads.

### KeeperChangelogStartupReadThreadsActive {#keeperchangelogstartupreadthreadsactive}

Number of active threads in the threadpool for Keeper changelog parallel startup reads.

### KeeperChangelogStartupReadThreadsScheduled {#keeperchangelogstartupreadthreadsscheduled}

Number of queued or active jobs in the threadpool for Keeper changelog parallel startup reads.

### KeeperContainerNodes {#keepercontainernodes}

Number of container nodes currently stored in Keeper.
...
Size of the Paimon metadata cache in bytes

### PaimonMetadataFilesCacheFiles {#paimonmetadatafilescachefiles}

Number of cached files in the Paimon metadata cache
...
Number of threads in the QueryRunner thread pool.

### QueryRunnerThreadsActive {#queryrunnerthreadsactive}

Number of threads in the QueryRunner thread pool running a task.

### QueryRunnerThreadsScheduled {#queryrunnerthreadsscheduled}

Number of queued or active jobs in the QueryRunner thread pool.
...
Number of long source connections currently held open by ReaderExecutor for sequential read optimization.
...
Number of blob reference-count nodes (one per data blob shared by 2+ blob-list parts) in the cluster-shared `_blob_manager/refcount` Keeper subtrees, measured periodically by the blob-manager stats task and summed over the server's Keeper-metadata disks.
...
Number of distinct columns substreams lists shared across data parts (sum over per-table caches)

### SharedPartColumnSubstreamsEntriesCacheSize {#sharedpartcolumnsubstreamsentriescachesize}

Number of distinct per-column substream entries shared across data parts (sum over per-table caches)

### SharedPartSerializationGroupsCacheSize {#sharedpartserializationgroupscachesize}

Number of distinct per-column serialization groups shared across data parts (sum over per-table caches)

### SharedPartSerializationsCacheSize {#sharedpartserializationscachesize}

Number of distinct serializations maps shared across data parts (sum over per-table caches)
...
Number of stateless workers that have been evicted and are awaiting removal, as tracked by the stateless worker discovery service.

### StatelessWorkerDiscoveryFreeWorkers {#statelessworkerdiscoveryfreeworkers}

Number of stateless workers that are registered and available to be leased, as tracked by the stateless worker discovery service.

### StatelessWorkerDiscoveryLeasedWorkers {#statelessworkerdiscoveryleasedworkers}

Number of stateless workers currently leased to tenants, as tracked by the stateless worker discovery service.

### StatelessWorkerDiscoveryTenants {#statelessworkerdiscoverytenants}

Number of tenants registered with the stateless worker discovery service.

### StatelessWorkerIdle {#statelessworkeridle}

Number of stateless workers currently held by the server but not used by any running query.

### StatelessWorkerInUse {#statelessworkerinuse}

Number of stateless workers currently in use by running queries.

### StatelessWorkerServerActiveTasks {#statelessworkerserveractivetasks}

Number of query execution tasks currently active on a stateless worker (accepted from a tenant and not yet released).
...
- `value` ([Int64](/reference/data-types/int-uint)) — Metric value.
