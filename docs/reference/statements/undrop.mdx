...
during the period set by [`database_atomic_delay_before_drop_table_sec`](/reference/settings/server-settings/settings#database_atomic_delay_before_drop_table_sec) (8 minutes by default) after issuing `DROP TABLE`.
Tables dropped from `Atomic` databases are listed in `system.dropped_tables`.

For a `Shared` database in ClickHouse Cloud, this period is instead controlled by [`database_shared_drop_table_delay_seconds`](/reference/settings/session-settings#database_shared_drop_table_delay_seconds), which defaults to 8 hours.
