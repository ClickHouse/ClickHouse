...
A trailing `SETTINGS` clause after the `RETURNING` subquery is applied to the **source `SELECT`**, not to the `RETURNING` subquery. To set options that shape the returned rows, place the `SETTINGS` inside the subquery itself, for example `RETURNING (SELECT ... SETTINGS ...)`, but note that delayed `RETURNING` rejects `database`, query-construction settings (`select`, `filter`, `order`, `sort`, `limit`, `offset`, `page`), query-cache settings, and query-global resource/admission limits.

For `INSERT ... RETURNING`, source-side `SETTINGS` that bind query-global state at query registration are rejected with `NOT_IMPLEMENTED` (the same rejection class as for unsupported `RETURNING` subquery settings). This includes settings such as `max_execution_time`, memory/admission limits, throttling settings, query-cache settings (`use_query_cache`, `enable_reads_from_query_cache`, `enable_writes_to_query_cache`, `query_cache_nondeterministic_function_handling`, `query_cache_system_table_handling`), and `profile`. The query-construction settings (`select`, `filter`, `order`, `sort`, `limit`, `offset`, `page`) are also rejected in source-side `SETTINGS`. `use_concurrency_control` is an exception: it is accepted on source-side `SETTINGS`.

The parenthesized subquery is required. The client receives a single result set: the result of the `RETURNING` subquery.

The `INSERT` runs first using the normal insert pipeline. If the `INSERT` fails, the `RETURNING` subquery is not executed. If the `INSERT` succeeds, the subquery runs in the same session and can reference any table. The subquery may carry its own `SETTINGS`, except for the rejected classes listed under Limitations (including `database`, query-construction settings, query-cache settings, and query-global resource/admission limits).
...
- Query-construction settings (`select`, `filter`, `order`, `sort`, `limit`, `offset`, `page`) are also rejected when inherited by the delayed `RETURNING` phase from session/profile/outer-query context, because that phase does not run the standalone construction rewrite pipeline (`applyQueryConstructionSettings` / `wrapNestedConstructionSettings`).
...
- The same source-side rejection also includes query-construction settings (`select`, `filter`, `order`, `sort`, `limit`, `offset`, `page`) in `INSERT ... SELECT ... RETURNING ... SETTINGS ...`.
