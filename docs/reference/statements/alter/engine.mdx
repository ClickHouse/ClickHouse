...
- `GraphiteMergeTree` when the path or time column is nullable, or when the time column is not an integer, `Enum`, `Date`, `Date32` or `DateTime`. The rollup reads those columns with `getDataAt` and `getUInt`, which throw on a `NULL` or on an unsupported type.
