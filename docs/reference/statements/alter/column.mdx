...
Separately, `ADD INDEX` only updates metadata: a newly added index stays unmaterialized on historical parts until you run `MATERIALIZE INDEX`. A later merge builds skip indexes only when [`materialize_skip_indexes_on_merge`](/reference/settings/merge-tree-settings/materialize#materialize_skip_indexes_on_merge) is enabled and the index is not listed in [`exclude_materialize_skip_indexes_on_merge`](/reference/settings/merge-tree-settings/exclude#exclude_materialize_skip_indexes_on_merge); otherwise historical parts remain unmaterialized until an explicit `MATERIALIZE INDEX`.
</Note>
