...
The projection query itself must stay the same, because existing projection parts store data built from it; to change the query, use [`DROP PROJECTION`](#drop-projection) followed by [`ADD PROJECTION`](#add-projection).
