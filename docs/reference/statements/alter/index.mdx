...
- Serialize `ALTER` statements and retry on code 517. For mutation entries specifically, wait for the previous entry to leave the unassigned state (see [`system.mutations`](/reference/system-tables/mutations) and [`mutations_sync`](/reference/settings/session-settings/mutations#mutations_sync)).
