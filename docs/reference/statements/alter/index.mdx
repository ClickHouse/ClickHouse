...
### Preventing accidental mutations {#preventing-accidental-mutations}

The [`mutations_restrict`](/reference/settings/session-settings/mutations#mutations_restrict) session setting acts as a safety catch that rejects mutation-producing statements before they run. Set to `1` to reject any `ALTER TABLE` form that would create an entry in [`system.mutations`](/reference/system-tables/mutations); set to `2` to additionally reject standalone lightweight `DELETE` and `UPDATE`. Metadata-only `ALTER`s, `INSERT`, and `SELECT` remain unaffected.

Users can set the value in a session (default is `0`) and lower it when they need to mutate. To enforce it globally or per user, pin it in a profile with a `<readonly/>` constraint (see the [`mutations_restrict` setting docs](/reference/settings/session-settings/mutations#mutations_restrict) for the config snippet).
