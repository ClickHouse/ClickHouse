...
The constraints of that profile can only narrow the ones already in effect for
the session. A profile applied this way cannot lift a constraint which comes from
your configuration. See [Constraints on settings](/operations/settings/constraints-on-settings).
