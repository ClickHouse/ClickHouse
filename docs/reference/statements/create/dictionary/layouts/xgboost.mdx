...
| `verbosity` | Logging verbosity: `0` (silent) to `3` (debug). Affects only XGBoost's own log output, which does not currently go through the ClickHouse logger, and never the trained model. |
