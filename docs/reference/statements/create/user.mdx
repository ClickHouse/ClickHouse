...
A session authenticated with such a method does not read from or write to the [query result cache](/concepts/features/performance/caches/query-cache). The query result cache is not access-control-aware on a hit (it isolates entries by user and roles but does not re-check access and is not invalidated by `GRANT`/`REVOKE`), so a limited credential bypasses it to remain fail-close: a query result must not outlive the grant it was produced under.
