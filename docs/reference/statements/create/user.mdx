...
<Warning>
**Enforcement is initiator-only.** The limit is enforced only on the node that receives the query (the initiator). It is **not** propagated to other nodes of a cluster: for distributed queries and `ON CLUSTER` DDL, the access rights are checked on the initiator, but the remote nodes execute with the full rights of the user. This behavior is consistent with `VALID UNTIL` and external roles. Do not rely on the clause to constrain execution cluster-wide. The clause is also not available in `users.xml`.
</Warning>
...
The [query result cache](/concepts/features/performance/caches/query-cache) is shared for a user regardless of the authentication method used to log in. The cache is not access-control-aware on a hit (it isolates entries by user and roles but does not re-check access and is not invalidated by `GRANT`/`REVOKE`), so a session limited by the clause can be served a result that the same user cached under a different authentication method. Keep this in mind if you enable the query result cache and hand out limited credentials.
