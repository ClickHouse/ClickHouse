...
A session authenticated with a method that has a `GRANTS` clause does not read from or write to the [query result cache](/concepts/features/performance/caches/query-cache). The cache is not access-control-aware on a hit (it isolates entries by user and roles but does not re-check access and is not invalidated by `GRANT`/`REVOKE`), so bypassing it keeps limited credentials fail-closed: a query result must not outlive the grant that allowed it.
