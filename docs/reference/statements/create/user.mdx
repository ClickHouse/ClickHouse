...
Notes:
1. Multiple authentication methods are supported in ClickHouse 24.9 and later. Before downgrading to a version before 24.9, configure each user with a single authentication method. Otherwise, users with multiple authentication methods become unusable and some user-related operations fail. If the server was downgraded without this preparation, drop the affected users.
