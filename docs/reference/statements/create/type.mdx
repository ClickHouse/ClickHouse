...
- The `base_type` must resolve to a valid data type. Referencing an unknown type
  ...
  exception, and naming a known type family with a wrong number of arguments (for
  example `Map(String)`) is rejected as well.
...
through the [`SHOW TYPES`](/reference/statements/show#show-types) and
[`SHOW TYPE`](/reference/statements/show#show-type) statements.

## Related content {#related-content}

- [`DROP TYPE`](/reference/statements/drop#drop-type)
- [`SHOW TYPES` / `SHOW TYPE`](/reference/statements/show#show-types)
