...
* `workload` - Workload used for the complete refresh: scheduler admission, internal refresh
  operations, and the SELECT query. If a `QUERY` resource is configured and the workload resolves
  to it, the refresh waits asynchronously for a query slot before execution, without occupying a
  background schedule pool worker. It also controls CPU and I/O resource scheduling when those
  resources are configured. A refresh-level workload overrides a workload specified in the SELECT
  query. Without this refresh setting, a SELECT-level workload keeps its existing execution
  behavior but does not enable admission before the refresh starts. Default: empty.
...
The workload can be changed without modifying the SELECT query:

```sql
ALTER TABLE rmv
MODIFY REFRESH EVERY 1 MINUTE SETTINGS workload = 'batch';
```

The new workload is used by subsequent refresh attempts. A refresh that is already running keeps
the workload with which it started.
