...
This avoids recomputing the view when the underlying data is unchanged. Change detection uses the same mechanism as the [`modification_hash`](/reference/system-tables/tables) column of `system.tables`, and inherits its best-effort limitations: for `URL` and object-storage sources (which rely on the resource's strong `ETag`) and for `Merge`/`Distributed` sources whose set of underlying tables changes during the refresh, a refresh may in rare cases be skipped even though a source changed. It also covers a source's regular columns and data only, not query-visible virtual columns that expose placement or external metadata (for example `_disk_name`, `_tags`, `_headers`), so a refresh of a view that reads only such a virtual column may be skipped after that column changed. If a source table cannot tell whether its data changed (for example a table function such as `url`), the view's `SELECT` calls a non-deterministic function (whose result can change while every source table is unchanged), or a source table has an active row policy in the refresh context (the policy can change what the refresh reads while every source table is unchanged), the refresh is not skipped. A manual `SYSTEM REFRESH VIEW` always rebuilds the view, ignoring `IF CHANGED`. The sources are examined once, when the scheduled refresh is about to start, and the skip decision is made against that snapshot: a write that commits right after that moment does not un-skip the refresh, and is picked up by the next scheduled refresh instead. The state the check compares against - the source hash of the last refresh that rebuilt the view, together with the identity of the view definition that produced it - is kept in the view's coordination state, so it survives a server restart and a handover to another replica, and an `ALTER ... MODIFY QUERY` / `MODIFY REFRESH` discards it. The check is still best-effort in the other direction: one refresh may run even if nothing changed, when the previous state is unknown or no longer comparable. This is the case for a source whose change detection is process-local rather than persistent - a local `MergeTree` table reports a modification hash that deliberately does not survive a server restart - and for a view in a database that does not coordinate refreshes through Keeper, where the whole refresh schedule is in memory and a refresh runs on startup anyway. The same applies when `APPEND ... SETTINGS all_replicas = 1` disables coordination in a `Replicated` database: the `IF CHANGED` state is in memory and does not survive a restart. With `APPEND`, such a refresh appends another copy of the unchanged source data; combine `IF CHANGED` with `APPEND` only when duplicate rows are acceptable, or use sources whose modification state is persistent (`URL` and object storage report a strong `ETag`).
