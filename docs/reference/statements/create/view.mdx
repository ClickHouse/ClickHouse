...
If the atomic setup fails — for example, the exclusive lock on a busy source table cannot be acquired within `lock_acquire_timeout` — the just-created view is dropped and the `CREATE` query fails, leaving nothing behind, so it can simply be retried.
