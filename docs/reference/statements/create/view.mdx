...
* `workload` - Workload used by refresh queries. If a `QUERY` resource is configured and the
  workload resolves to it, the refresh waits asynchronously for a query slot before execution,
  without occupying a background schedule pool worker. The workload is also used for CPU and I/O
  resource scheduling when the corresponding resources are configured. Default: empty.
