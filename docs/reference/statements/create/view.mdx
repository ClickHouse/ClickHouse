...
* `workload` - Workload used by refresh queries. When non-empty, the refresh acquires query slots and memory reservations from this workload in addition to using it for CPU and I/O resource scheduling. Default: empty.
