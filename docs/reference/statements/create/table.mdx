...
- [Log](/reference/engines/table-engines/log-family/index) family — the `Log` and `TinyLog` engines use the default `ZSTD(3)` compression method and support column compression codecs.
- [StripeLog](/reference/engines/table-engines/log-family/index) writes all columns into a single shared data stream compressed with the default `ZSTD(3)` method, so per-column compression codecs are not applied to it.
