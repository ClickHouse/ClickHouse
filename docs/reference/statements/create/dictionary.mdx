...
Naming a user other than yourself requires the `SET DEFINER` privilege. A user that is the definer of an existing dictionary cannot be dropped, in a database whose objects have UUIDs. This includes the default `Atomic` engine, but not `Memory` or `Ordinary`, where the dictionary stops loading if its definer is dropped. The same holds for views.
...
`DEFINER` is the only accepted security type. The settings the load runs under are bounded by the definer's own settings constraints. Settings the dictionary loader reads from the definition rather than from the load context, such as `dictionary_lazy_load`, are not.
