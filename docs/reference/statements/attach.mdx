...
```sql
ATTACH TABLE|[MATERIALIZED] VIEW|DICTIONARY|DATABASE [IF NOT EXISTS] [db.]name [ON CLUSTER cluster] ...
```
...
```sql
ATTACH TABLE|[MATERIALIZED] VIEW [IF NOT EXISTS] [db.]name [ON CLUSTER cluster]
```
