...
- `Cost` — the cost of the whole join subtree under this step, which is the value the optimizer
  minimizes when it compares candidate join orders. The unit is output rows, not input rows: the
  cost of a join is the number of rows it is estimated to produce plus the cost of its inputs, so
  the top join of a subtree accumulates the estimated output rows of every join below it.
...
- `Output rows` — the estimated number of rows the join produces, computed as
  `<selectivity> * <left_rows> * <right_rows>`. An outer join cannot produce fewer rows than its
  preserved side(s), so the estimate is floored accordingly.
...
- `Cost` — both values count output rows, not input rows. The estimate is the optimizer's cost of the join subtree: the rows this join is estimated to produce plus the cost of its inputs. The actual value is measured the same way — the matched output rows of this join plus the actual cost of every join below it that belongs to the same *reorder cluster* (the group of joins the optimizer reordered together) — so the two numbers are directly comparable.
