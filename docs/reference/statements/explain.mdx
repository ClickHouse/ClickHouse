...
- `Peak memory` — peak memory the query used. Asynchronous background work started by the query (for example, prefetched reads) is charged to the analyzed pipeline as it runs, so only allocations that outlive the analyzed pipeline are not included in this figure.
