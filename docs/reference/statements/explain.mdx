...
- `Peak memory` — peak memory the query used. Memory allocated by asynchronous background work started by the query (for example, prefetched reads) is accounted at the thread or global level rather than to the query being analyzed, so it is not included.
