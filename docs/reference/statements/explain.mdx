...
**Block nested loop join.** The operator that takes an `ON` section determining no join key, unless one of the paths before it claims the condition (see [`allow_block_nested_loop_join`](/reference/settings/session-settings/allow#allow_block_nested_loop_join)). It accepts every kind and strictness except `ASOF`, `PASTE` and `ANY FULL`:
