...
- `Peak memory` — peak memory the query used. Memory allocated by asynchronous background work started by the query (for example, prefetched reads) is accounted to the outer query rather than to the analyzed pipeline's scope, so it is not included in this figure (it still counts toward the memory limits of the query, such as `max_memory_usage`).
