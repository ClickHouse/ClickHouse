...
`full_sorting_merge` and `parallel_full_sorting_merge` need it for nothing: they walk the two sorted
inputs and see every row of an equal range as they consume it.
...
**Merge algorithms.** `full_sorting_merge` and `parallel_full_sorting_merge` report **both** sides for every combination they accept, and without `matches = 1` — they walk the two sorted inputs and see every row of an equal range as they consume it, so nothing has to be reconstructed afterwards. They accept the four `ALL` kinds, `ANY INNER`, `ANY LEFT`, `ANY RIGHT`, `ASOF` and `ASOF LEFT`.
