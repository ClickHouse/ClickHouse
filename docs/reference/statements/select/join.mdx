...
`INNER JOIN`, `LEFT JOIN`, `RIGHT JOIN`, `FULL JOIN`, and `SEMI JOIN` queries support the implicit type conversion for "join keys". However the query can not be executed, if join keys from the left and the right tables cannot be converted to a single type (for example, `String` and `Int32`).
...
This is applied to the equality conditions in the `ON` section, and to the `USING` section of an `INNER JOIN` or a `SEMI JOIN`, where the result contains only the matched values. It is not applied to:
- the kinds of `JOIN` with `USING` that can emit unmatched rows (such as `LEFT`, `RIGHT`, `FULL`, and `ANTI` joins), where the join key is also a column of the result and can hold the unmatched values;
...
- the `ASOF` inequality (the last column in the `USING` list), which needs the order of the values, not only their equality;
- a [`Join`](/engines/table-engines/special/join) table engine whose stored key is not itself the common type, because its hash table is prebuilt over the original key values and cannot be converted (the other direction, where only the probe key has to be converted, is supported; the `LowCardinality` and `Nullable` wrappers of the stored key do not matter for this comparison).

It requires the analyzer, which is enabled by default; with [`enable_analyzer = 0`](/reference/settings/session-settings/allow-experimental#allow_experimental_analyzer) such a query is still rejected.
