...
<Note>
A condition that references columns from different tables may use any operator. Only conditions that use the equality operator (`=`) become join keys; the rest are evaluated on the joined rows. An `ON` section that contains no equality between the two tables at all is executed as a [block nested loop join](#join-with-an-arbitrary-on-condition), unless another algorithm can claim the condition: an `ALL INNER JOIN` becomes a `CROSS JOIN` with the condition as a filter on top of it, and a condition made of two inequalities goes to `ie_join` when that algorithm is enabled.
</Note>
...
When the `ON` section contains no equality between the two tables, there is no join key to match on. Two inequalities over the same pair of tables are executed by [`ie_join`](/reference/settings/session-settings/join#join_algorithm) when that algorithm is enabled; otherwise, and for any other shape of the condition, the join is executed as a [block nested loop join](#join-with-an-arbitrary-on-condition).
...
An `ON` section of this shape was rejected with `INVALID_JOIN_ON_EXPRESSION` before this operator existed; with the analyzer enabled (`enable_analyzer = 1`, the default) such a query now runs instead of failing. There is no setting that turns the operator off, so a condition that determines no join key is executed however expensive it turns out to be, bounded only by the general limits described below.
...
`ANY` and `SEMI` normally keep one row per group of rows that share a join key. There is no such group here, so they keep one row per row of the table that drives the join: `LEFT ANY` and `LEFT SEMI` emit each left row at most once, `RIGHT ANY` and `RIGHT SEMI` each right row at most once. `ANY INNER` limits both sides at once — each row of either table is used at most once, so the result has at most as many rows as the smaller table. Which pairs make up such a result is arbitrary, exactly as `ANY` implies, and it may differ between two runs of the same query.
