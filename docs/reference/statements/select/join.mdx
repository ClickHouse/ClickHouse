...
An `ON` section of this shape was rejected with `INVALID_JOIN_ON_EXPRESSION` before this operator existed; such a query now runs instead of failing. There is no setting that turns the operator off, so a condition that determines no join key is executed however expensive it turns out to be, bounded only by the general limits described below.
