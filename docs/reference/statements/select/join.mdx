...
- a [`Join`](/engines/table-engines/special/join) table engine whose stored key is not itself the common type, because its hash table is prebuilt over the original key values and cannot be converted (the other direction, where only the probe key has to be converted, is supported).
