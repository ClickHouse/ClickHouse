...
<Note>
Experimental. Enable for a session with:
...
- Engines that only wrap a `MergeTree` table, such as `Merge`, `MaterializedView`
  and an alias table, pass the clause through to the underlying table.
  `Buffer` is an exception: it rejects `SAMPLE` when its destination table has no
  `SAMPLE BY` key, because the rows that are still resident in the in-memory
  buffers are read on a separate path and would be returned unsampled.
