...
`FROM t CROSS JOIN unnest(t.arr)` is the PostgreSQL `LATERAL` shape. ClickHouse rewrites it to `ARRAY JOIN t.arr AS unnest`. The original array column is kept. Empty arrays drop rows, same as `ARRAY JOIN`.
...
This rewrite requires the analyzer (enabled by default). For a constant array without a left table, use the [`unnest`](/sql-reference/table-functions/unnest) table function: `SELECT * FROM unnest([1, 2, 3])`.
...
- Table function: [`unnest`](/sql-reference/table-functions/unnest)
