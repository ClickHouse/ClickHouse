...
<Note>
PostgreSQL `FROM unnest(...)` and `CROSS JOIN UNNEST(column)` (including `LATERAL`) are not supported. Use `ARRAY JOIN` instead. The `unnest` name is a function-call alias of `arrayJoin` (`SELECT unnest(arr)`), not a table function.
</Note>
