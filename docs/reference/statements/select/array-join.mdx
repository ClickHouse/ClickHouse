...
<Note>
PostgreSQL `FROM unnest(...)` and `CROSS JOIN UNNEST(column)` (including `LATERAL`) are not supported. Use `ARRAY JOIN` instead. The `unnest` name (since 26.5) is a function-call alias of `arrayJoin` (`SELECT unnest(arr)`), not a table function.
</Note>
