...
With [group_by_use_nulls](/reference/settings/session-settings/group-by#group_by_use_nulls) `= 1` and the analyzer enabled (`enable_analyzer = 1`, the default), the real `GROUP BY` key columns whose types can be wrapped in [Nullable](/reference/data-types/nullable) are converted to `Nullable` and are reported as `NULL` (instead of their default value) in the totals row. Keys whose types cannot be made `Nullable` (for example `Array` or `Map`) keep their default value. A constant key that is folded away before aggregation is not a real key column, so it also keeps its constant value in the totals row. With the old query analyzer (`enable_analyzer = 0`) this conversion is not applied, and the key columns keep their default value in the totals row.
