...
A detached view can be reattached either with `ATTACH VIEW` or with `ATTACH TABLE`, so `ATTACH TABLE` is also the way to reattach an entity whose kind you do not know.
