...
Manually clears all failed file entries from the ZooKeeper metadata of an S3Queue or AzureQueue table.
...
This command:
- Removes all failed file entries from ZooKeeper (the `/failed/` znodes)
...
Failed files can also be cleaned up automatically using the [`failed_file_ttl_sec`](/engines/table-engines/integrations/s3queue#failed_file_ttl_sec) table setting, which removes old failed file entries based on a time-to-live.
