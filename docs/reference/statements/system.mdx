...
All of these forms require the `SYSTEM DROP REPLICA` privilege: on the specified table for the first form, on the specified database for the second one, and on every database on the server for the third and the fourth ones. If the privilege is missing, the query is rejected with `ACCESS_DENIED` and lists the databases the privilege is missing for, instead of silently doing nothing.
...
The privileges are checked in the same way as for `SYSTEM DROP REPLICA`: the `SYSTEM DROP REPLICA` privilege is required on the specified database for the first form, and on every database on the server for the second and the third ones.
