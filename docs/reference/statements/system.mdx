...
Terminal failed files can also be cleaned up automatically using the [`failed_file_ttl_sec`](/engines/table-engines/integrations/s3queue#failed_file_ttl_sec) table setting, which removes old terminal failed file entries based on a time-to-live.
