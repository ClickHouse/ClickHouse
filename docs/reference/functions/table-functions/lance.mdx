---
description: 'Provides a read-only table-like interface to Lance datasets in local storage or Amazon S3.'
sidebarTitle: 'lance'
sidebar_position: 91
...
The `lanceLocal` and `lanceS3` table functions provide a read-only interface to existing [Lance](https://lancedb.github.io/lance/) datasets. Use `lanceLocal` for local storage and `lanceS3` for Amazon S3 or S3-compatible object storage.

`lanceLocal` and `lanceS3` read existing Lance datasets. They do not create datasets, write data, build indexes, or perform vector search.

Enable the experimental table functions before using them, including `lanceS3Cluster`:

```sql
SET allow_experimental_lance = 1;
```

The setting defaults to `0` and must also be enabled for queries executed on cluster workers.

## Syntax {#syntax}

```sql
lanceLocal(path_to_dataset)

lanceLocal(path_to_dataset, SETTINGS disk = 'disk_name')
...
`lanceLocal` takes the path to an existing dataset directory, including its manifests and data files. Use an absolute path on the host running ClickHouse, readable by the server process or by `clickhouse-local`. With `SETTINGS disk = 'disk_name'`, the path is relative to a configured local disk. The files must be accessible to the process executing the query.
...
Fragment packing is controlled by the same settings as single-node multi-stream reads (`lance_enable_fragment_parallelism`, `lance_fragment_pack_mode`, `lance_max_fragment_packs`, and related options). See [Lance table engine](/reference/engines/table-engines/integrations/lance#read-parallelism).
...
`lanceLocal`, `lanceS3`, and `lanceS3Cluster` return a table whose schema is inferred from the Lance dataset version selected at query analysis time.
...
The Lance table functions recursively convert the following Arrow types stored by Lance:
...
```sql
SELECT * FROM lanceLocal('/var/lib/clickhouse/user_files/datasets/example.lance');
```

Read through a configured local disk:

```sql
SELECT * FROM lanceLocal('example.lance', SETTINGS disk = 'lance_local_disk');
```
...
`lanceLocal` and `lanceS3` support the file-like virtual columns provided by the object-storage read path, including `_path`, `_file`, `_size`, `_time`, and `_etag`.

For data lake reads, `_data_lake_snapshot_version` contains the Lance dataset version used by the query.

## Limitations {#limitations}

- `lanceLocal`, `lanceS3`, and `lanceS3Cluster` are read-only.
...
- [`LanceLocal` / `LanceS3` table engines](/reference/engines/table-engines/integrations/lance)
- [`s3` table function](/reference/functions/table-functions/s3)
- [`icebergS3` table function](/reference/functions/table-functions/iceberg)
