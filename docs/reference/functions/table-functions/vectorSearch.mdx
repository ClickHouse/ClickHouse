...
sidebarTitle: 'vectorSearch'
slug: /sql-reference/table-functions/vectorSearch
title: 'vectorSearch'
doc_type: 'reference'
---

Returns the K approximate nearest neighbors of a reference vector from a `MergeTree` table's `vector_similarity` index. The search is approximate: it queries the persisted index and has no exact-search fallback, so the result may differ from the exact K nearest neighbors (see [Approximate vector search](/reference/engines/table-engines/mergetree-family/annindexes#approximate-nearest-neighbor-search)).

<Note>
The table function is experimental. To enable it, set `allow_experimental_search_topk_table_functions` to `1`. See [Experimental gate](#experimental-gate) below.
</Note>
...
In addition to the returned columns above, `vectorSearch` exposes every [virtual column](/reference/engines/table-engines/mergetree-family/mergetree#virtual-columns) of the source `MergeTree` table — `_part`, `_part_index`, `_part_offset`, `_partition_id`, `_partition_value`, `_block_number`, `_block_offset`, `_part_data_version`, `_disk_name`, `_row_exists`, and so on. Each carries the same value it would have for a plain `SELECT` from the source table; for example `_part` is the source part each row was read from, and `_part_offset` is the row's offset within that part. Like all virtual columns, they are not included in `SELECT *` and must be referenced explicitly, and they can be used in the `WHERE` clause. Most of them drive the prefilter, `_part` prunes whole parts, and predicates on the row-locator virtual columns `_part_index` and `_part_offset` are post-filtered exceptions — see [WHERE-clause prefilter](#where-clause-prefilter).
...
The row-locator virtual columns `_part_index` and `_part_offset` are post-filtered exceptions among the source virtual columns. The bitmap subquery reads these two columns internally to map matched rows back to their parts, so a user predicate on them would collide with that internal use and is *not* pushed into the prefilter. Like a `_score` predicate, it is applied as an ordinary filter after the top-K rows have been produced. As a consequence, `WHERE _part_offset = 0` returns the subset of the top-K rows that happen to sit at part-offset `0` — it can return fewer than `K` rows and is not the top-K within that offset subset.

`_part` names a whole part, so a predicate on it prunes the list of parts the index is scored over rather than becoming a bitmap prefilter. A query filtered only by `_part` therefore reads no rows at all and does not consume `search_topk_prefilter_max_rows`; for a mixed predicate such as `WHERE _part = '...' AND payload = 1`, the parts are pruned first and the bitmap is built from the residual row-level predicate over the surviving parts only. Every other source virtual column (`_partition_id`, `_block_number`, `_disk_name`, and so on) participates in the bitmap prefilter.
...
`vectorSearch` reads the source table, so it requires the same privileges as a plain `SELECT` from it:

- `SELECT` on every source column the query reads, plus the column the `vector_similarity` index is built on, which the scorer always reads through the index. Column-level grants are enough; `SELECT id, _score FROM vectorSearch(db, tab, idx, [...], 10)` needs `GRANT SELECT(id, vec) ON db.tab`. `_score` and `_distance` are produced by the table function and need no grant.
- `SHOW TABLES` on the source table before the index is resolved, so that a user with no grants on it cannot learn whether an index of a given name exists, whether it is a `vector_similarity` index, or which vector indexes the table has. Any grant on the table, including a column-level `SELECT`, implies this privilege.
- `SHOW COLUMNS` on the source table for `DESCRIBE vectorSearch(...)` and `CREATE TABLE ... AS vectorSearch(...)`, which expose the source table's full structure. This is the same privilege `DESCRIBE TABLE` on the source table requires, so a user holding only column-level grants can search but not describe.
...
- The source table must have a `vector_similarity` index. See [Approximate vector search](/reference/engines/table-engines/mergetree-family/annindexes#approximate-nearest-neighbor-search) for how to create one.
- The source table must belong to the `MergeTree` family. `Distributed` tables are not accepted directly; use [`remote`](/reference/functions/table-functions/remote) (or `clusterAllReplicas`) to fan the table function out across replicas.
...
- Each selected part must carry a single `vector_similarity` sub-index, i.e. at most one index granule per part; parts with more than one granule are rejected. This holds with the default index `GRANULARITY` (100 million) for parts of up to 100 million rows. Smaller explicit `GRANULARITY` values, as discussed in [Approximate vector search](/reference/engines/table-engines/mergetree-family/annindexes#approximate-nearest-neighbor-search), are not supported by `vectorSearch` yet.
- `K` must not exceed the setting [`max_limit_for_vector_search_queries`](/reference/settings/session-settings/max#max_limit_for_vector_search_queries). The legacy `ORDER BY ... LIMIT` surface falls back to an exact search above the cap; `vectorSearch` has no exact-search fallback, so it rejects the query instead.
...
- Rows hidden by lightweight deletes or by a pending `ALTER TABLE ... DELETE` are excluded through the bitmap prefilter, because the prefilter is read through the normal `MergeTree` readers, which drop them. When the query has no `WHERE` clause and a selected part has such rows, an implicit prefilter is built, so such queries are subject to `search_topk_prefilter_max_rows` like any prefiltered query.
- The legacy `SELECT [...] ORDER BY <DistanceFunction>(vectors, reference_vector) LIMIT N` SQL surface described in [Approximate vector search](/reference/engines/table-engines/mergetree-family/annindexes#approximate-nearest-neighbor-search) continues to work unchanged and remains the recommended surface for queries that do not need an explicit `_score` projection.
