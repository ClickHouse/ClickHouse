...
The same setting switches the [`GCS` table engine](/engines/table-engines/integrations/gcs) between the two backends. Native GCS is also available as a MergeTree storage disk via `object_storage_type: gcs` (or `type: gcs`).
