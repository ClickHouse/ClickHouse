...
| Calculating aggregate functions over a frame (`sum(value) OVER (ORDER BY time)`)    | ✅         | All aggregate functions are supported. Over a frame that carries an `EXCLUDE`, the ones that allocate in an arena are not (see the next row).                                                                                                                                                                                                                                                                                  |
| Frame exclusion (`EXCLUDE CURRENT ROW`, `EXCLUDE GROUP`, `EXCLUDE TIES`, `EXCLUDE NO OTHERS`) | ✅         | Supported for aggregate functions. Rejected with `NOT_IMPLEMENTED` for the ones that allocate in an arena, such as `groupArray`, because the state is rebuilt for every row while the arena is only released when the partition ends, and for the window functions that walk the frame themselves: `nth_value`, `lagInFrame`/`leadInFrame` and the `exponentialTimeDecayed*` family. `EXCLUDE GROUP` and `EXCLUDE TIES` are also rejected when the window `ORDER BY` carries a `COLLATE` and something on that window reads the frame, because peers are decided without consulting the collator; `rank` and `row_number` do not read the frame and are left alone. Where the exclusion does take rows out, it takes them out per row, so a frame whose start does not move costs one pass over the frame per row; where it takes nothing out, the frame costs what it would without the clause. |
...
aggregate_function (column_name)
  OVER ([[PARTITION BY grouping_column] [ORDER BY sorting_column] 
        [ROWS, RANGE, or GROUPS expression_to_bound_rows_within_the_group [frame_exclusion]]] | [window_name])
FROM table_name
WINDOW window_name as ([
  [PARTITION BY grouping_column]
  [ORDER BY sorting_column]
  [ROWS, RANGE, or GROUPS expression_to_bound_rows_within_the_group [frame_exclusion]]
])
```
...
- `EXCLUDE CURRENT ROW`, `EXCLUDE GROUP`, `EXCLUDE TIES` or `EXCLUDE NO OTHERS` - takes rows back out of the frame, after its bounds have been applied. `CURRENT ROW` drops the current row, `GROUP` drops the current row together with its peers (the rows equal to it on the `ORDER BY` key), `TIES` drops those peers but keeps the current row, and `NO OTHERS`, the default, drops nothing.
