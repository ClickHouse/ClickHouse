...
- The session key and the threshold accept exactly the types `RANGE OFFSET` accepts: keys up to 128-bit signed integers, `Float32`/`Float64` and `DateTime`, and a native-numeric threshold. Wider integers, `BFloat16`, `Decimal` and `DateTime64` raise an error, as they do for `RANGE OFFSET`.
