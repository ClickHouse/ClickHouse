...
| Calculating aggregate functions over a frame (`sum(value) OVER (ORDER BY time)`)    | ✅         | All aggregate functions are supported, except over a frame that carries an `EXCLUDE` (see the next row).                                                                                                                                                                                                                                                                                  |
| Frame exclusion (`EXCLUDE CURRENT ROW`, `EXCLUDE GROUP`, `EXCLUDE TIES`, `EXCLUDE NO OTHERS`) | ✅         | Supported for aggregate functions whose state is of a fixed size, so `sum`, `avg`, `max`, `quantile` and the like. Rejected with `NOT_IMPLEMENTED` for aggregates that allocate in an arena, such as `groupArray`, and for the window functions that walk the frame themselves: `nth_value`, `lagInFrame`/`leadInFrame` and the `exponentialTimeDecayed*` family. `EXCLUDE GROUP` and `EXCLUDE TIES` are also rejected when the window `ORDER BY` carries a `COLLATE`, because peers are decided without consulting the collator. The excluded rows are taken out per row, so a frame whose start does not move costs one pass over the frame per row. |
