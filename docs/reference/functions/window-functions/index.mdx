...
- The session key must be an integer, a float, or `DateTime`, the same key types `RANGE OFFSET` supports. `Decimal` and `DateTime64` keys are not supported and raise an error.
