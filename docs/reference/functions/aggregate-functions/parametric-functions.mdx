...
Bins are merged while data arrives, so once a single aggregation state has received more than `2 * number_of_bins` values, the result can depend on the order in which the values are processed. Aggregating the same rows in a different order may then produce different bin borders and heights.
