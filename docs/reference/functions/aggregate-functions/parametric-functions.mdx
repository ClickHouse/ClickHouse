...
Bins are merged while data arrives, so once a single aggregation state has received more than `2 * number_of_bins` values, the result depends on the order in which the values are processed. Aggregating the same rows in a different order can therefore produce different bin borders and heights.
...
<Note>
Events that share a timestamp may lay in the sequence in an undefined order affecting the result. A `DateTime64` timestamp orders by its sub-second part too, so only events that share the same tick are ambiguous.
</Note>

**Arguments**

- `timestamp` — Column considered to contain time data. Typical data types are `Date`, `DateTime` and `DateTime64`. You can also use any of the supported [UInt](/reference/data-types/int-uint) data types.
...
- `(?t operator value)` — Sets the time that should separate two events. For example, pattern `(?1)(?t>1800)(?2)` matches events that occur more than 1800 seconds from each other. An arbitrary number of any events can lay between these events. You can use the `>=`, `>`, `<`, `<=`, `==` operators. For a `DateTime` or `DateTime64` timestamp the value is in seconds, whatever the precision of `DateTime64`, so the same pattern keeps its meaning when the column changes between `DateTime` and `DateTime64` of any scale. For a `Date` timestamp the value is in days, and for an unsigned integer timestamp it is in the units of the integer.
