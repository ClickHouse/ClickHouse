...
`Nullable` arguments are supported: a row whose `x` or forwarded argument is `NULL` is skipped. The result type stays `String` regardless of the nested aggregate function's null semantics, so an input consisting only of `NULL`s yields the empty sparkbar string rather than `NULL`.
