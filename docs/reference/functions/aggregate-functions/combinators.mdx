...
Applied as a suffix to an aggregate function name (e.g. `sumTuple`), it makes the function take one `Tuple` argument per argument of the underlying function (all tuples of the same size) and aggregate each element position independently, producing a tuple of per-element results. Elements are paired by position: `corrTuple((a1, a2), (b1, b2)) = (corr(a1, b1), corr(a2, b2))`.

For example, `sumTuple` for the tuples `(1, 2.5)` and `(3, 4.5)` returns the result `(4, 7)` after adding together the corresponding tuple elements.
