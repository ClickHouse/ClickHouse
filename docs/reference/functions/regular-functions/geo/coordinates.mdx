...
- `longitude` — Longitude in degrees. Range: `[-180°, 180°]`. [`Float32`](/reference/data-types/float)/[`Float64`](/reference/data-types/float).
- `latitude` — Latitude in degrees. Range: `[-80°, 84°]`. [`Float32`](/reference/data-types/float)/[`Float64`](/reference/data-types/float).
- `zone` — Optional. Force the projection into this UTM zone instead of selecting it automatically. Range: `[1, 60]`. [`(U)Int*`](/reference/data-types/int-uint).

**Returned value**

A named tuple `(easting, northing, zone, band)`: `easting` and `northing` in metres ([`Float64`](/reference/data-types/float)), the UTM `zone` number ([`UInt8`](/reference/data-types/int-uint)), and the MGRS latitude `band` letter ([`FixedString(1)`](/reference/data-types/fixedstring)). A `band` of `'N'` or later denotes the northern hemisphere.
...
- `easting` — Easting in metres (includes the 500000 m false easting). [`(U)Int*`](/reference/data-types/int-uint)/[`Float*`](/reference/data-types/float).
- `northing` — Northing in metres (includes the 10000000 m false northing on the southern hemisphere). [`(U)Int*`](/reference/data-types/int-uint)/[`Float*`](/reference/data-types/float).
- `zone` — UTM zone number. Range: `[1, 60]`. [`(U)Int*`](/reference/data-types/int-uint).
- `is_north` — Hemisphere: `1` for the northern hemisphere, `0` for the southern. [`(U)Int*`](/reference/data-types/int-uint).

**Returned value**

A named tuple `(longitude, latitude)` in degrees. [`Tuple(Float64, Float64)`](/reference/data-types/tuple).
...
- `longitude` — Longitude in degrees. Range: `[-180°, 180°]`. [`Float32`](/reference/data-types/float)/[`Float64`](/reference/data-types/float).
- `latitude` — Latitude in degrees. Range: `[-80°, 84°]`. [`Float32`](/reference/data-types/float)/[`Float64`](/reference/data-types/float).
- `precision` — Optional. Number of digits for each of easting and northing. Default: `5`. Range: `[0, 5]`. [`(U)Int*`](/reference/data-types/int-uint).

**Returned value**

The MGRS reference string. [`String`](/reference/data-types/string).
...
- `mgrs` — MGRS reference string to decode. [`String`](/reference/data-types/string)/[`FixedString`](/reference/data-types/fixedstring).
...
- `(x, y)` — Coordinates of a point on the plane. Data type — [Tuple](/reference/data-types/tuple) — A tuple of two numbers, or a [Point](/reference/data-types/geo#point).
- `[(a, b), (c, d) ...]` — Polygon vertices. Data type — [Array](/reference/data-types/array) or [Ring](/reference/data-types/geo#ring). Each vertex is represented by a pair of coordinates `(a, b)`. Vertices should be specified in a clockwise or counterclockwise order. The minimum number of vertices is 3.
...
- The polygon argument can also be a [Geometry](/reference/data-types/geo#geometry) column holding polygon-shaped values (`Ring`, `Polygon`, or `MultiPolygon`).

The polygon-shaped types ([Ring](/reference/data-types/geo#ring), [Polygon](/reference/data-types/geo#polygon), [MultiPolygon](/reference/data-types/geo#multipolygon) and [Geometry](/reference/data-types/geo#geometry)) may be passed either as constants or as regular (non-constant) table columns. When the polygon is provided across several separate arguments (an outer ring followed by holes, or several polygons of a multipolygon), all of those arguments must be constant.
