...
Returns a unidirectional edge H3 index based on the provided origin and destination. Throws an exception if either input is not a valid H3 cell (controlled by the `functions_h3_default_if_invalid` setting).
