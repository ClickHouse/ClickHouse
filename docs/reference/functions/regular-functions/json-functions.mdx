...
They accept either a JSON string or a value of the [`JSON`](/sql-reference/data-types/newjson) data type as their first argument. The functions use optimized access to `JSON` subcolumns where possible; see the individual function documentation for their supported arguments and result semantics.

The `JSON` data type can differ from a JSON string in ways that affect results: it does not retain `null` keys or the order of object keys. Consequently, functions whose results depend on key order — `JSONKey`, `JSONExtractKeys`, `JSONExtractKeysAndValues`, `JSONExtractKeysAndValuesRaw`, and an integer index addressing an object member — use the order in which the column stores keys, which is not necessarily the order in which they were written.
