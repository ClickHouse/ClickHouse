...
`unnest` is a case-insensitive alias of `arrayJoin` in function-call form (`SELECT unnest(arr)`). It works anywhere `arrayJoin` works, including `WHERE`. PostgreSQL table-source syntax is not supported (`FROM unnest(...)`, `CROSS JOIN UNNEST(...)`, and `LATERAL`). Use the [`ARRAY JOIN`](/reference/statements/select/array-join) clause for those queries.
