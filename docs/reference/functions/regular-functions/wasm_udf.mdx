...
- `SETTINGS`: Per-function settings
    ...
    - `is_spatial_predicate` Bool — Declares that the function honours the same pruning contract as builtins like `pointInPolygon`: for a call `f(geom_column, const_geom, ...)`, a match guarantees that `geom_column`'s value intersects the bounding box of `const_geom`. Default: `false`. When `true`, a `spatial_bbox` skip index on `geom_column` and the `Parquet` reader's GeoParquet bbox statistics may skip data without calling the function. Declaring it on a function that does not honour the contract silently drops matching rows. Pruning needs exactly one geometry-shaped constant argument; other constants are allowed, but a `String` one is treated as a possible WKB payload and disables pruning for the surrounding `AND` if it does not parse — prefer a non-`String` type for those.
