...
Processes entire blocks at once using a (de)serialization through WASM memory. Supports the argument and return types the chosen `serialization_format` can encode: the default `MsgPack` cannot encode `Dynamic`, `Variant` or `JSON` / `Object` values, which need a format such as `JSONEachRow` or `CSV`. Those types are supported only nested inside another type, such as `Array(Dynamic)`; a bare top-level `Dynamic` or `Variant` argument is not accepted, whatever the format.
