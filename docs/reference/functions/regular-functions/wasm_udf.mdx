...
Processes entire blocks at once using a (de)serialization through WASM memory. Supports the argument and return types that the chosen `serialization_format` can encode: the default `MsgPack` has no encoding for `Dynamic`, `Variant` or `JSON` / `Object` values, so a signature declaring one of those - including nested inside another type - needs a format that does, such as `JSONEachRow` or a text format like `CSV`.
