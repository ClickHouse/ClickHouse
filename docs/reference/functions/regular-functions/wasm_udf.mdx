...
Processes entire blocks at once using a (de)serialization through WASM memory. Supports the argument and return types that the chosen `serialization_format` can encode: the default `MsgPack` has no encoding for `Dynamic`, `Variant` or `JSON` / `Object` values, so a signature carrying one of those nested inside another type, such as `Array(Dynamic)`, needs a format that does, such as `JSONEachRow` or a text format like `CSV`. An argument cannot be declared as a bare top-level `Dynamic` or `Variant`, whatever the format: such a call is dispatched to the concrete alternative type before the function is reached, and that type no longer matches the declared signature.
...
A pipeline block can still be too wide for the module's linear memory, so `webassembly_udf_max_input_block_size` controls how it is divided further. Its default, `0`, hands the decision to `webassembly_udf_input_split_memory_ratio`, which is non-zero by default and splits each block dynamically: ClickHouse measures the serialized size of the rows and starts a new call before the input would grow past that fraction of the guest's linear memory, so the row count varies with the data. A non-zero `webassembly_udf_max_input_block_size` replaces dynamic splitting with a fixed cap of that many rows per call, regardless of how large the rows are. Setting `webassembly_udf_input_split_memory_ratio` to `0` sets no part of the memory aside for a call's input and so restores the earlier behaviour, in which `webassembly_udf_max_input_block_size = 0` passes the whole pipeline block in one call; a module written against that contract can also get it back by setting `compatibility` to a version before 26.9. Splitting stops at one row per call, so it cannot help with a row that is on its own too large for the module's linear memory: such a row is passed alone and fails inside the guest's allocator. Either way the number of rows in a call is passed to the guest as the second `i32` argument, so a module must read it rather than assume a block size.
...
- `webassembly_udf_max_input_block_size` — Maximum number of rows passed to a WebAssembly UDF in a single block. A non-zero value caps the rows per call and applies to every ABI. Set to 0 to let `webassembly_udf_input_split_memory_ratio` decide how the block is divided, which it does for `ABI BUFFERED_V1` alone. For `ROW_DIRECT` and `ASSEMBLYSCRIPT`, whose calls carry no serialized input block, 0 leaves the pipeline block whole.

- `webassembly_udf_input_split_memory_ratio` — The fraction of the linear memory the module declares it starts with that one call's input may occupy when a block of an `ABI BUFFERED_V1` function is split dynamically; a module declared as `memory 0 N` starts with no pages, so for it the fraction is taken of the reachable ceiling instead. The declared initial size is used rather than the current one because the current size moves with `memory.grow`, never shrinks, and belongs to a pooled instance, so a batch sized against it would depend on which instance a worker picked up. Default: `0.5`, leaving the other half of the memory to the guest's own working set beside the input buffer. Must be at least 0 and at most 1. The margin below 1 is what makes the batching safe: the guest's own data, stack and allocator share that memory and are invisible to the host, so a ratio close to 1 can still fail inside the guest's allocator. Set it to 0, or set `compatibility` to a version before 26.9, to turn dynamic splitting off: the whole pipeline block is then passed in one call, as earlier versions did. A non-zero value sizes the batches only when `webassembly_udf_max_input_block_size` is 0. A batch is never taken below a single row, so a row whose own serialized size is past the budget is passed on its own and, if it is too large for the module's linear memory, fails inside the guest's allocator. It is read only for `ABI BUFFERED_V1`: `ROW_DIRECT` passes its arguments as WebAssembly values and `ASSEMBLYSCRIPT` builds one object per row, so a call of theirs carries no serialized input block to split, and neither reads `serialization_format`.
