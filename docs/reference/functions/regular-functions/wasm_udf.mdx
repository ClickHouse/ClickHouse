...
A WASM UDF is never handed a nullable argument column. `NULL` arguments are propagated by
ClickHouse around the call, exactly as they are for built-in functions: the `Nullable` wrappers
are stripped before the module runs, the module sees only the nested values, and the null maps of
all arguments are combined onto the result afterwards. A row whose argument is `NULL` therefore
yields `NULL` without the module deciding anything about it.

One consequence is that the declared return type is not the type of the expression. Calling a
function declared `RETURNS UInt64` with a `Nullable(String)` argument produces `Nullable(UInt64)`,
because the wrapper is reapplied on the way out.
