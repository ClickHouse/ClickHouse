...
- `SETTINGS`: Per-function settings
    ...
    - `is_spatial_predicate` Bool — Declares that the function implements a spatial predicate with the same pruning contract as builtins like `pointInPolygon`: for a call `f(geom_column, const_geom, ...)`, whenever the function returns a match, `geom_column`'s value is guaranteed to intersect the bounding box of the constant geometry argument(s). Default: `false`. When `true`, ClickHouse may use this guarantee to skip data whose stored bounding box doesn't intersect the constant geometry, without calling the function at all. Two readers consume it: a `spatial_bbox` skip index on `geom_column` prunes granules in `MergeTree`, and the `Parquet` reader prunes GeoParquet row groups by their `covering.bbox` columns (controlled by `input_format_parquet_spatial_filter_push_down`). Setting this on a function that doesn't actually honor the contract (for example, one that ignores its geometry argument) causes either of them to silently drop rows that should have matched — verify the contract holds for every possible input before enabling it. Extra constant arguments besides `const_geom` are allowed (for example a distance threshold in `f(geom_column, const_geom, threshold)`) and never affect correctness, but an auxiliary constant argument of type `String` reduces pruning effectiveness: it is conservatively treated as a possible WKB payload, and if it fails to parse as one, pruning is disabled for the whole surrounding `AND` (the query still returns correct results, just without the speedup). Prefer a non-`String` type (for example an `Int32` mode flag) for auxiliary constant arguments that are not meant to be geometry.
