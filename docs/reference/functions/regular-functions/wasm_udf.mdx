...
Whether a WASM UDF sees `NULL` values at all is decided by the return type declared in `RETURNS`.

- If the declared return type is **not** `Nullable`, ClickHouse applies its ordinary null propagation:
  `NULL` arguments are stripped before the call, the module is never handed a nullable column, and any
  row where an argument is `NULL` yields `NULL` in the result without the module being consulted for it.
  This is the same behaviour as built-in functions. The one exception is a return type that cannot be
  placed inside `Nullable` at all, such as `Array(...)`: there the result stays non-nullable and rows with
  a `NULL` argument are evaluated over the argument's default value instead.
- If the declared return type **is** `Nullable`, the module owns null propagation. ClickHouse passes the
  nullable argument columns through unchanged, and the module decides, per row, which results are `NULL`.
  With `ABI BUFFERED_V1`, nullability is visible through the chosen `serialization_format` — for example
  the `COL_IS_NULLABLE` flag and null map of [`ColumnBinary`](/reference/formats/ColumnBinary), or a JSON
  `null` with `JSONEachRow`.

So a module that needs to distinguish "the argument was `NULL`" from "the argument was the default value"
must declare a `Nullable` return type, even if it never actually returns `NULL`.
