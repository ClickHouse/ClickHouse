...
- `dict_name` — Name of the dictionary. [`String`](/reference/data-types/string)
- `key` — Key value. [`UInt64`](/reference/data-types/int-uint)

**Returned value**

Returns the topmost ancestor (the root) for the key. [`UInt64`](/reference/data-types/int-uint)
...
## dictGetUUID2 {#dictGetUUID2}

Introduced in: v26.8.0

Converts a dictionary attribute value to `UUID2` data type regardless of the dictionary configuration.
...
```sql
dictGetUUID2(dict_name, attr_name, id_expr)
```
...
## dictGetUUID2OrDefault {#dictGetUUID2OrDefault}

Introduced in: v26.8.0

Converts a dictionary attribute value to `UUID2` data type regardless of the dictionary configuration, or returns the provided default value if the key is not found.
...
```sql
dictGetUUID2OrDefault(dict_name, attr_name, id_expr, default_value_expr)
```
