...
Classifies input text using a [`NAIVE_BAYES`](/en/sql-reference/statements/create/dictionary/layouts/naive-bayes) dictionary. Returns the same predicted class value as `dictGet(dictionary_name, class_attribute, input_text)`, where class_attribute is the name of the class label attribute configured in the dictionary's [layout](/en/sql-reference/statements/create/dictionary/layouts/naive-bayes#layout-parameters). Unlike `dictGet`, the result type is always `UInt32` rather than the declared type of the class attribute, and `input_text` must be a `String` (no key type conversion is applied).
...
Classifies input text using a [`NAIVE_BAYES`](/en/sql-reference/statements/create/dictionary/layouts/naive-bayes) dictionary and returns all classes with their probabilities, ordered from most to least probable.
...
Classifies input text using a [`NAIVE_BAYES`](/en/sql-reference/statements/create/dictionary/layouts/naive-bayes) dictionary and returns the predicted class with its probability.
