...
- `params` — Optional constant `Map(String, String)` of parameters. Function-specific keys: `dimensions` (target dimensionality of the output vector; `0` or omitted means the model's native size). The common parameters `credentials` and `model` also apply (see [AI Functions](/reference/functions/regular-functions/ai-functions)). [`Map(String, String)`](/reference/data-types/map)
...
**Embed a single string (map parameter can be omitted if the `ai_function_embedding_default_credentials` setting is set)**

```sql title=Query
SELECT aiEmbed('Hello world', map('credentials', 'ai_embedding_credentials'))
```

**With explicit dimensions**

```sql title=Query
SELECT aiEmbed('Hello world', map('credentials', 'ai_embedding_credentials', 'dimensions', '256'))
```

**Embed a column of texts**

```sql title=Query
SELECT aiEmbed(title, map('credentials', 'ai_embedding_credentials', 'dimensions', '256')) FROM articles LIMIT 10
```
