...
AI functions reference a **named collection** that stores provider credentials and configuration. Different named collections can be created and used for different functions or functions calls. For example you may want to define a different named collection to use with the text functions (`aiGenerate`, `aiClassify`, `aiFilter`, `aiExtract`, `aiTranslate`, `aiRedact`), the embedding functions (`aiEmbed`, `aiSimilarity`), and the reranking function (`aiRerank`), which require different endpoints and usually use different models.

Example statement to create a named collection with provider credentials, one with a chat endpoint, one with an embedding endpoint, and one with a reranking endpoint:
```sql
...
-- The reranking function (`aiRerank`) is experimental: set `allow_experimental_ai_rerank_function = 1`
-- to use it. It resolves `model` the same way as the text functions: the named collection's `model`
-- is a default, overridable via the parameter map.
...
|-----------|------|---------|-------------|
| `provider` | String | — | Model provider. Supported: `'openai'`, `'anthropic'`, `'cohere'`. See note below. |
| `endpoint` | String | — | API endpoint URL. |
| `model` | String | — | Model name (e.g. `'gpt-4o-mini'`). Used by the text and reranking functions as a default, overridable via the parameter map; the embedding functions (`aiEmbed`, `aiSimilarity`) require `model` as a positional argument instead and error if `model` is specified in the named collection. |
...
2. otherwise the applicable default-credentials setting:
   ...
   - [`ai_function_rerank_default_credentials`](/reference/settings/session-settings/ai-function#ai_function_rerank_default_credentials) for the experimental reranking function (`aiRerank`).

If neither is set, the call fails. The text, embedding, and reranking functions use separate default settings because a chat-completions endpoint differs from an embeddings or a reranking one.
...
| `model` | Overrides the collection's `model` (text functions and `aiRerank`; the embedding functions `aiEmbed`/`aiSimilarity` take `model` as a required positional argument, not a map key). |
...
| Provider | `provider` value | Chat functions | Embedding functions | Rerank function | Notes |
|----------|------------------|----------------|---------------------|-----------------|-------|
| OpenAI | `'openai'` | Yes | Yes | No | Default provider. |
| Anthropic | `'anthropic'` | Yes | No | No | Uses `/v1/messages` endpoint. |
| Cohere | `'cohere'` | No | No | Yes (experimental) | Uses the [Cohere Rerank API](https://docs.cohere.com/reference/rerank) `/v2/rerank` endpoint. Cohere bills reranking in search units, which the token limits don't count, but `input_tokens`/`output_tokens` in `meta.billed_units` are counted if the endpoint reports them. `aiRerank` requires `allow_experimental_ai_rerank_function = 1`. |
...
| `AISearchUnits` | Total search units billed for reranking (`aiRerank`). |
