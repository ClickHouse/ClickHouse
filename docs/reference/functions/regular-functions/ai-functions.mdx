...
- **Concurrent requests**: Requests are issued from a server-wide thread pool, several at a time (controlled by [`ai_function_max_concurrent_requests`](/reference/settings/session-settings/ai-function#ai_function_max_concurrent_requests)).
...
[`ai_function_max_concurrent_requests`](/reference/settings/session-settings/ai-function#ai_function_max_concurrent_requests) bounds a single function call's share of that pool.

The `ai_function_max_concurrent_requests` limit is per AI function call per block, so a query whose pipeline runs several streams may have more requests in flight than the setting allows for a single stream. Where the per-query total matters, put a ceiling on the setting with a settings profile constraint, which a query setting cannot raise:
