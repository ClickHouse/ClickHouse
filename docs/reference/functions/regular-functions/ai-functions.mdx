...
- **Concurrent requests**: Requests are issued from a server-wide thread pool, several at a time (controlled by [`ai_function_max_concurrent_requests_per_stream`](/reference/settings/session-settings/ai-function#ai_function_max_concurrent_requests_per_stream)).

## Throughput {#throughput}

The requests are issued from a server-wide thread pool. Its size (`max_ai_request_thread_pool_size`, a server setting) caps how many AI provider requests the whole server has in flight at once, across every query.

[`ai_function_max_concurrent_requests_per_stream`](/reference/settings/session-settings/ai-function#ai_function_max_concurrent_requests_per_stream) bounds each pipeline stream's share of that pool, so a query running several streams (at most `max_threads`) can have up to that many requests in flight per stream. To stop queries from raising it, put a ceiling on the setting with a settings profile constraint. The ceiling is still per stream, so a query can have up to the ceiling times its number of streams in flight. For example:
...
The default is `16`. A value above `1` loosens two guarantees:
...
<Note>
For a materialized view, the settings must go on the `INSERT` that feeds the view's source table, not on the `CREATE MATERIALIZED VIEW` statement: the quotas are read from the top-level query, and a `SETTINGS` clause in a view definition is ignored.
</Note>
