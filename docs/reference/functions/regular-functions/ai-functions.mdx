...
The `ai_function_max_concurrent_requests` limit is per AI function call per block, so a query whose pipeline runs several streams may have more requests in flight than the setting allows for a single stream. Where the per-query total matters, put a ceiling on the setting with a settings profile constraint, which a query cannot raise:
