...
- **Concurrent requests**: Requests are issued several at a time ([`ai_function_max_concurrent_requests`](/reference/settings/session-settings/ai-function#ai_function_max_concurrent_requests)).
...
[`ai_function_max_concurrent_requests`](/reference/settings/session-settings/ai-function#ai_function_max_concurrent_requests) sets that concurrency. It applies the same way to a function evaluated in an `INSERT ... SELECT` and to one evaluated inside a materialized view, and it does not depend on how the source data is spread over parts, on block sizes, or on [`parallel_view_processing`](/reference/settings/session-settings/settings#parallel_view_processing).

The limit is per AI function call per block, so a query whose pipeline runs several streams may have more requests in flight than the setting allows for a single stream. Where the total matters - a shared provider endpoint, or an account-wide rate limit - put a ceiling on the setting with a settings profile constraint, which a query cannot raise:
...
- A batch of requests is dispatched before any of their errors is seen, so a query that fails on its first row may still have issued, and been billed for, as many requests as the concurrency allows.
