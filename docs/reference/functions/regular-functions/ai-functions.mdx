...
- **Concurrent requests**: Requests are issued from a server-wide thread pool, several at a time ([`ai_function_max_concurrent_requests`](/reference/settings/session-settings/ai-function#ai_function_max_concurrent_requests)).
...
The requests are issued from a server-wide thread pool rather than by the function itself. Its size (`max_ai_request_thread_pool_size`, a server setting) caps how many AI provider requests the whole server has in flight at once, across every query.
