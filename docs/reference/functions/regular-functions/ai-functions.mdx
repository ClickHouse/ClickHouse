...
The requests are issued by a server-wide AI service rather than by the function itself. Its thread pool (`ai_service_threadpool_pool_size`, a server setting) caps how many AI provider requests the whole server has in flight at once, across every query.
