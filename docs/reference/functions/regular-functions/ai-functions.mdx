...
The requests are issued from a server-wide thread pool rather than by the function itself. Its size (`ai_request_threadpool_pool_size`, a server setting) caps how many AI provider requests the whole server has in flight at once, across every query.
