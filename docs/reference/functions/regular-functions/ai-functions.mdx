...
[`ai_function_max_concurrent_requests`](/reference/settings/session-settings/ai-function#ai_function_max_concurrent_requests) bounds a single query's share of that pool: the requests the query has in flight at once, across all of its AI function calls and pipeline streams. To put a ceiling on it that a query setting cannot raise, use a settings profile constraint:
...
- Requests are dispatched before the errors of earlier ones are seen, so a query that fails on its first row may still have issued, and been billed for, as many requests as the concurrency allows.
