...
| `shared_memory_path`          | Directory (expected to be `tmpfs`, e.g. mounted at `/dev/shm`) in which the shared-memory file is created. Valid only when `use_shared_memory` is enabled                                                                                                                                                                                                                                     | Optional  | `/dev/shm`            |
...
- The `version` field lets the protocol evolve while keeping older commands working: a command that does not recognize the version must answer with an error `status`.
