...
For `type` `executable_pool`, a region that grew beyond `shared_memory_size` is trimmed back to `shared_memory_size` when the query that grew it finishes, so a single outsized chunk does not pin up to `shared_memory_max_size` per pooled process against `max_server_memory_usage` for the lifetime of the server. Growth is therefore amortized within a query and paid again by the next query that needs a larger region; set `shared_memory_size` to the size your chunks usually need. A pooled process that is discarded rather than returned to the pool (a protocol failure, a dead command, a cancelled query) releases its regions entirely.
