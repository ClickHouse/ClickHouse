...
By default the bulk data is exchanged with the command through the `stdin`/`stdout` pipes. When `use_shared_memory` is enabled, the bulk data is instead placed into a shared-memory file that both `ClickHouse` and the command `mmap`, while the pipes carry only small control commands. This avoids copying every byte through the kernel pipe buffer and is most useful with `executable_pool`, where the same process — and the same shared-memory file — is reused across invocations.
...
An executable UDF is server-side code: it is configured by the administrator in the server's configuration files and runs as the ClickHouse user, with everything that implies - including the ability to signal the server it is a child of. The protocol checks described below (region bounds, region size, row counts, exit status) are there to keep a *mistake* in a command from taking the server down or corrupting a result; they do not contain a command that is actively hostile, and they are not a substitute for reviewing what you configure as a UDF. In particular, the server maps the region while the command can also write into it. The command's answer is not read through that mapping - it is copied out of the region file with `pread`, so a file the command shortened gives a short read and an ordinary error rather than a fatal `SIGBUS` - but the *input* is still written through the mapping, so a command that shrinks the file at the exact moment the server is serializing the next chunk into it can still crash the server. That window is much narrower than it sounds: the command is waiting for its next request at that point and has to be shrinking the file from another thread to hit it. A command that shrinks the region at any other point is caught by the checks on both sides of a request and fails only its own query.
...
- Only the server resizes the region: a command must never call `ftruncate` on the file it was given, and must not open it with `O_TRUNC`. A file shorter than the mapping the server holds would make the server fault (`SIGBUS`) on the very pages the command removed, so the server checks that the file has not become shorter before it serializes into the region and again after every response, fails the query when it has, and discards a pooled worker that did it.
...
- The response frame is everything the command may write to its `STDOUT`: the bulk output goes into the shared-memory region, so a byte written past the frame - a stray newline, a debug print - would be read as the beginning of the response to the *next* request. Diagnostics belong on `STDERR`, and should be written before the response rather than after it: `STDERR` is read together with the response, so anything written afterwards is only picked up by the next request. For `type` `executable_pool` the server checks both pipes before it returns a process to the pool and discards a process that left something in either of them, so such a command costs itself a worker rather than breaking the next query that would have borrowed it. `STDERR` is exempt from that check under `stderr_reaction` `none`, which discards those bytes rather than attributing them to a query - they are still read off the pipe, so a command whose diagnostics outgrow the pipe buffer does not block, but they reach nobody.
