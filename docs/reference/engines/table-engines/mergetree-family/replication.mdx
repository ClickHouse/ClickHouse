...
ClickHouse can group replicas into region to solve this issue. You can enable this feature by setting `geo_replication_control_region` when create replicated table. The feature is experimental: creating a table with a non-empty `geo_replication_control_region` requires the session setting `allow_experimental_geo_replication_control = 1`, otherwise the `CREATE TABLE` query is rejected. Existing tables are attached regardless of the session setting.
...
If the target part is not ready on any replica within the region, followers will defer execution of the log entry (default 5 seconds). If a follower waits for a leader for too long (default 300 seconds), it will stop waiting and try to fetch on any replica (or check if the part is broken). The defer time and timeout can be changed by modifying `geo_replication_control_leader_wait` and `geo_replication_control_leader_wait_timeout`.
...
When fetching from replica outside of the region, by default it only allows to fetch the exact part (not covering part), unless the exact part cannot be found in any replica. This is to avoid repeated data transmission across regions. To enable fetching covering parts across region, set `fetch_covered_part_within_region_only` to `0`.

The `ALTER TABLE ... FETCH PARTITION` and `ALTER TABLE ... FETCH PART` queries also prefer the replicas of the same region when the source table is in the same ClickHouse Keeper cluster. This is a best effort: unlike replication queue entries, a user query is never deferred, so a replica outside of the region is used when no active replica of the same region has the requested data. The same holds for cloning a part from another shard: a replica of the same region is preferred among the replicas of the source shard that already have the part.
...
- `/path_to_table/replicas/{replica}/region`: ephemeral node, ZooKeeper information about the region of the current replica
