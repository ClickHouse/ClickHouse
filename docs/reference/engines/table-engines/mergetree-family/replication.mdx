...
The `ALTER TABLE ... FETCH PARTITION` and `ALTER TABLE ... FETCH PART` queries also prefer the replicas of the same region when the source table is in the same ClickHouse Keeper cluster. This is a best effort: unlike replication queue entries, a user query is never deferred, so a replica outside of the region is used when no active replica of the same region has the requested data.
