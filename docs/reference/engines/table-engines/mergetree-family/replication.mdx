...
For an `Atomic` database, the converted `ReplicatedMergeTree` table uses the values of the `default_replica_path` and `default_replica_name` settings.
For an `Ordinary` database, it always uses `/clickhouse/tables/{database}/{table}` as its Keeper path and `default_replica_name` as its replica name.
