...
The points in the vector space are stored in a column `vectors` of type [Array(Float64)](/reference/data-types/array), [Array(Float32)](/reference/data-types/array), or [Array(BFloat16)](/reference/data-types/array).
For practical usage, we generally recommend using [BFloat16](/reference/data-types/float) arrays.
...
Also, since ClickHouse performs a brute-force scan of all vectors, the runtime depends also on the number of threads by the query (see setting [max_threads](/reference/settings/session-settings/max-threads#max_threads)).
...
The available methods are HNSW ([academic paper](https://arxiv.org/abs/1603.09320)), a popular technique for approximate vector search based on hierarchical proximity graphs, and the experimental [ScaNN](#scann) backend.
...
The memory layout depends on the selected approximate search method.
The following estimate applies specifically to HNSW indexes.

Memory consumption required to load an HNSW vector index:
...
The HNSW-specific formula above does not account for additional memory required to allocate runtime data structures like pre-allocated buffers and caches.

#### Using a Vector Similarity Index {#using-a-vector-similarity-index}

<Note>
To use vector similarity indexes, setting [compatibility](/reference/settings/session-settings/compatibility#compatibility) has be `''` (the default value), or `'25.1'` or newer.
</Note>
...
Advanced users may provide a custom value for setting [hnsw_candidate_list_size_for_search](/reference/settings/session-settings/other#hnsw_candidate_list_size_for_search) (also know as HNSW hyperparameter "ef_search") to tune the size of the candidate list during search (e.g.  `SELECT [...] SETTINGS hnsw_candidate_list_size_for_search = <value>`).
...
More specifically, an error is returned if `<N>` is bigger than the value of setting [max_limit_for_vector_search_queries](/reference/settings/session-settings/max#max_limit_for_vector_search_queries) with default value 100.
...
<Tip>
To enforce index usage, you can run the SELECT query with setting [force_data_skipping_indices](/reference/settings/session-settings/force#force_data_skipping_indices) (provide the index name as setting value).
</Tip>
...
Setting [vector_search_filter_strategy](/reference/settings/session-settings/vector-search#vector_search_filter_strategy) (default: `auto` which implements above heuristics) may be set to `prefilter`.
...
As an alternative approach to resolve above issue, setting [vector_search_index_fetch_multiplier](/reference/settings/session-settings/vector-search#vector_search_index_fetch_multiplier) (default: `1.0`, maximum: `1000.0`) may be configured to a value > `1.0` (for example, `2.0`).
...
The optimization is enabled by default, see setting [vector_search_with_rescoring](/reference/settings/session-settings/vector-search#vector_search_with_rescoring).
...
As a result, [compression](/reference/statements/create/table/codec) slows down inserts and reads into/from the vector column.
...
The maximum number of index creation threads can be configured using server setting [max_build_vector_similarity_index_thread_pool_size](/reference/settings/server-settings/settings/max#max_build_vector_similarity_index_thread_pool_size).
For optimal performance, the setting value should be configured to the number of CPU cores.

Second, to speed up INSERT statements, users may disable the creation of skipping indexes on newly inserted parts using session setting [materialize_skip_indexes_on_insert](/reference/settings/session-settings/materialize#materialize_skip_indexes_on_insert).
...
Third, to speed up merges, users may disable the creation of skipping indexes on merged parts using session setting [materialize_skip_indexes_on_merge](/reference/settings/merge-tree-settings/materialize#materialize_skip_indexes_on_merge).
...
The maximum cache size can be configured using server setting [vector_similarity_index_cache_size](/reference/settings/server-settings/settings/vector-similarity#vector_similarity_index_cache_size).
...
In this case, we also suggest enabling rescoring ([vector_search_index_fetch_multiplier](/reference/settings/session-settings/vector-search#vector_search_index_fetch_multiplier), [vector_search_with_rescoring](/reference/settings/session-settings/vector-search#vector_search_with_rescoring)) to improve accuracy.
...
It combines IVF (Inverted File Index) partitioning with asymmetric hashing for approximate nearest-neighbor search on large, high-dimensional datasets.

<Note>
The ScaNN backend is experimental. Enable setting `allow_experimental_scann_index` before creating a ScaNN index:

```sql
SET allow_experimental_scann_index = 1;
```

On x86-64, the ScaNN backend requires SSE4.1 and AVX support.
ClickHouse returns a `SUPPORT_IS_DISABLED` exception when a ScaNN index is created or used on a CPU without either instruction set.
</Note>

A ScaNN index accepts either three or five arguments. The optional `<precision>` and `<num_leaves>` arguments must be specified together:
...
The current ClickHouse ScaNN integration supports distance functions `L2Distance`, `cosineDistance`, and `dotProduct` on columns of type `Array(Float32)` or `Array(Float64)`.
Support for `Array(BFloat16)` input columns has not been implemented yet.

The HNSW memory estimate described above does not apply to ScaNN because ScaNN does not maintain an in-memory HNSW graph.
Its memory consumption instead consists primarily of IVF partitioning data, asymmetric-hashing codes, and the exact-reordering vectors stored using the selected precision.
It also depends on the number and size of index granules, the vector dimensions, and the number of IVF leaves.

The ScaNN-specific build arguments are:

- `<precision>` controls the persisted precision of the exact-reordering vectors. Supported values are `f32`, `bf16`, and `i8`; the default is `f32`. Training continues to use full Float32 precision.
- `<num_leaves>` controls the number of IVF partitions in each index granule. It must be a positive integer. If both optional arguments are omitted, ClickHouse selects the number of leaves automatically from the number of vectors in the granule.

For example, the following index uses `bf16` reordering vectors and `1024` IVF partitions per index granule:

```sql
INDEX index_name vectors TYPE vector_similarity('scann', 'cosineDistance', 768, 'bf16', 1024)
```

The recall and latency trade-off can be adjusted per query with these settings:

- `scann_num_leaves_to_search` controls the number of IVF partitions searched. Value `0` selects a balanced default from the number of leaves in the index granule. ClickHouse uses `0.75 * floor(1 + num_leaves * exp(2.0 * 0.8) * 0.015)`, clamped to the range from `1` to `num_leaves`. If the index definition does not specify `num_leaves`, it is selected automatically as approximately the square root of the number of vectors in the granule.
- `scann_candidate_pool_size` controls the size of the asymmetric-hashing candidate pool passed to exact reranking. Value `0` selects a balanced default based primarily on the query `LIMIT`, using `20 * floor(LIMIT^0.65 * sqrt(2.0)) * 2.5`. ClickHouse also adjusts this value for granules containing more than 10 million vectors and for vector dimensions of `1024` or greater (with the dimensional adjustment capped at `1536`), then clamps it between the requested result count and the number of vectors in the granule.

```sql
WITH [0.0, 1.0, ...] AS reference_vec
SELECT id, cosineDistance(vectors, reference_vec) AS dist
FROM table
ORDER BY dist ASC
LIMIT 10
...
ScaNN requires at least `1000` vectors in an index granule. For smaller granules, ClickHouse does not build the ScaNN index and uses an exact scan instead. The index is built automatically after parts merge into a sufficiently large granule; `OPTIMIZE TABLE ... FINAL` can be used to force such a merge.
