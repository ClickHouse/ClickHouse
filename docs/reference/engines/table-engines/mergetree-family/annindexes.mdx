...
The [`vectorSearch`](/reference/functions/table-functions/vectorSearch) table function provides direct access to a `vector_similarity` index. It returns the K approximate nearest neighbors of a reference vector together with a `_score` virtual column carrying USearch's native distance for the index's metric, so `ORDER BY _score ASC` is the universal sort regardless of whether the index uses `L2Distance`, `cosineDistance`, or `dotProduct`. Unlike the `ORDER BY ... LIMIT` surface above, the table function always searches the index and never falls back to an exact search. See [`vectorSearch`](/reference/functions/table-functions/vectorSearch) for the full reference.
