...
Lookup indexes are supported for non-replicated `MergeTree` tables.
...
- Lookup indexes are not supported for `ReplicatedMergeTree`, see [Replication](#replication).

## Replication {#replication}

A lookup index cannot be created on a `ReplicatedMergeTree` table: both `CREATE TABLE ... LOOKUP INDEX` and `ALTER TABLE ... ADD LOOKUP INDEX` throw `SUPPORT_IS_DISABLED`.

Lookup index metadata would be stored in the replicated table metadata in ZooKeeper as an extra line, and a replica running a build without support for the feature ignores that line and rewrites the metadata without it on the next unrelated `ALTER`, which would silently drop the lookup indexes for the whole cluster. While the feature is experimental, and until the replicated metadata carries a version that lets a replica refuse metadata it does not understand, lookup indexes are restricted to non-replicated tables.
