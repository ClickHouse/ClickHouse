...
Container-level `NULL` values in `Array` and `Map` are rejected because ClickHouse cannot represent them without losing information. Nullable `Struct` values require `enable_nullable_tuple_type = 1`. See the [`lanceS3` data type mapping](/reference/functions/table-functions/lance#data-types) for the complete mapping and unsupported types.
...
| Setting | Default | Description |
|---|---|---|
| `lance_scan_in_order` | `0` | When true, batches are returned in deterministic fragment order **within one stream**. Set to `0` to allow higher internal fragment concurrency. |
...
For multi-node parallel reads, use [`lanceS3Cluster`](/reference/functions/table-functions/lance#cluster-reads). There is no separate `LanceS3Cluster` table engine; the cluster table function builds a distributed read on top of the same Lance metadata path as `lanceS3`.

## See also {#see-also}

- [`lanceLocal` / `lanceS3` / `lanceS3Cluster` table functions](/reference/functions/table-functions/lance)
- [`S3` table engine](/reference/engines/table-engines/integrations/s3)
- [`IcebergS3` table engine](/reference/engines/table-engines/integrations/iceberg)
