...
Additionally, the following optional boolean parameters are supported for native compression. Both default to `0` (disabled) and accept a boolean value, so the `=0` form is an explicit disabled state rather than an unset flag:

- `name_decompress` — `name_decompress=1` decompresses the data using the ClickHouse native compression format before parsing; omitted or `name_decompress=0` leaves the data as-is. This is useful when sending data that was compressed with `compress=1` on the server side (i.e., in ClickHouse native compression format).
- `name_disable_checksum` — `name_disable_checksum=1` disables checksum verification during decompression; omitted or `name_disable_checksum=0` keeps verification enabled. This parameter only has an effect when `name_decompress=1`.
