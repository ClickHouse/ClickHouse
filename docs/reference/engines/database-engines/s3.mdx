...
The `S3` database engine exposes objects in Amazon S3 and S3-compatible object storage as read-only tables. A table name is resolved as an object URL through the [`s3`](/reference/table-functions/s3) table function.
...
S3 URLs are checked against the server's remote-host filter. When [`table_engines_require_grant`](/reference/settings/server-settings/settings/access-control#table_engines_require_grant) is enabled, creating this database requires the appropriate `S3` source privilege, for example:
...
- [`s3` table function](/reference/table-functions/s3)
- [Using S3 with ClickHouse](/integrations/data-ingestion/s3)
