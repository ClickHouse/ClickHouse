...
The only exception is a table that is currently served through the remote-replica fallback described above (the local replica of a shard does not have the database or the table): a `Distributed` table over the whole cluster performs no such fallback, and the fallback subset of the cluster has no name of its own, so no equivalent re-executable definition exists, and `SHOW CREATE TABLE` reports the `THERE_IS_NO_QUERY` error until the local replica has the objects again.
