...
`SHOW CREATE TABLE` prints each table as a [`Distributed`](/reference/engines/table-engines/special/distributed) table over the named cluster (including the implicit sharding key of a multi-shard database), so the emitted definition recreates a standalone table with the same behavior:

```sql
CREATE TABLE ... ENGINE = Distributed('cluster_name', 'database', 'table'[, rand()])
```

There are two exceptions where no equivalent re-executable definition exists and `SHOW CREATE TABLE` reports the `THERE_IS_NO_QUERY` error:
