...
- A configuration reload can change a one-shard cluster to a multi-shard cluster. The live proxy then acquires an implicit `rand()` key to distribute `INSERT` rows, while a table serialized before the reload has no key. Serializing the key unconditionally is not equivalent either, because it would make a standalone `Distributed` table use it for read shard pruning; `CREATE TABLE` has no way to express an insert-only sharding key.
