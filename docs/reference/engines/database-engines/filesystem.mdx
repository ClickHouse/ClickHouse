...
The `Filesystem` database engine exposes files in a local directory as read-only tables. A table name is resolved as a path relative to the database directory and is read using the [`file`](/reference/table-functions/file) table function.
...
On ClickHouse server, the database directory and every resolved file must be inside [`user_files_path`](/operations/server-configuration-parameters/settings#user_files_path); this restriction also applies after following symlinks. `clickhouse-local` is not restricted to `user_files_path`.

When [`table_engines_require_grant`](/reference/settings/server-settings/settings/access-control#table_engines_require_grant) is enabled, creating this database also requires the appropriate `FILE` source privilege. Grant it with, for example:
...
- [`file` table function](/reference/table-functions/file)
