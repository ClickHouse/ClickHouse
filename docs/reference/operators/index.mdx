...
`SIMILAR TO` is the SQL-standard pattern matching predicate. Its pattern combines the `LIKE` wildcards `%` (any number of arbitrary characters) and `_` (a single arbitrary character) with the regular expression metacharacters `|`, `*`, `+`, `?`, `{`, `}`, `(`, `)`, `[` and `]`. The metacharacters `^`, `$` and `.` are **not** part of the `SIMILAR TO` grammar and denote the corresponding literal characters. The pattern must match the whole left operand, not a substring of it. `\` escapes any of the metacharacters and itself.
