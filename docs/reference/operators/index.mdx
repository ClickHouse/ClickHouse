...
The escape character must be followed by a special character of the predicate or by itself; anything else is an error, because an escape before an ordinary character has no effect. Inside a bracket expression such as `[...]` the escape character is not special and denotes itself, following the POSIX and PostgreSQL behavior.
