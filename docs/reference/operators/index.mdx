...
`f | g` – Function composition: a function that applies `f` and then `g`. It can be used wherever a function is expected, that is, as an argument of a higher-order function:

```sql
SELECT arrayMap(plus(_, 1) | multiply(_, 2), [1, 2, 3]); -- [4, 6, 8]
```

The composition reads left to right: the value flows through the operands in the order they are written, so `f | g` means `g(f(x))` and `f | g | h` means `h(g(f(x)))`. This is the direction of the shell pipe and of the C++ ranges `|`, which is why the same glyph is used. It is the opposite of the mathematical `∘` and of Haskell's `.`, where `g . f` applies `f` first; the left-to-right equivalents in those languages are spelled `>>>` or `>>`.

Each operand can be a lambda, the name of a function, another composition, or an expression with argument placeholders. The placeholders turn an expression into a lambda: `_1`, `_2`, ... refer to the first, second, ... lambda argument and may be used at any depth of the expression, and the largest placeholder number determines the number of arguments. The anonymous placeholder `_` introduces one argument per occurrence, in the order of occurrence, and must be a direct argument of a function call. For example, `plus(_, 1)` and `plus(_1, 1)` both mean `x -> plus(x, 1)`.

Placeholders can also be used without the composition operator in the lambda position of a higher-order function: `arrayMap(_1 * 2 + 1, [1, 2, 3])`. Names bound in the query take priority over placeholders, so a column, an alias, or a lambda bound with `WITH` named `_1` keeps its meaning.

The composition is resolved at query analysis time by inlining `f` into `g`, like SQL user defined functions are, and the result is an ordinary lambda. The right operand must be a function of one argument.

A bare placeholder is the identity function, so `_1 | toString` is the same as `toString(_1)`.

Note that the single `|` is not bitwise OR; use the `bitOr` function for that. The operator has no user-visible function name of its own and reserves no name: every identifier, including `compose`, remains available for a user defined function.
