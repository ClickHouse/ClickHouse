...
Placeholders can also be used without the composition operator in the lambda position of a higher-order function: `arrayMap(_1 * 2 + 1, [1, 2, 3])`. Names bound in the query take priority over placeholders there, so a column or alias named `_1` keeps its meaning.
...
Note that the single `|` is not bitwise OR; use the `bitOr` function for that. The operator has no user-visible function name of its own, so `compose` is an ordinary name that can be used for a user defined function.
