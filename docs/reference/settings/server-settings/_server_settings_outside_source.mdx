...
| `readonly_can_only_be_tightened` | Sets whether the `readonly` setting may only be changed towards more restriction while readonly mode is in force. The order of restriction is not the numeric order: `0` (no restriction) &lt; `2` (read queries only, any setting but `readonly`) &lt; `1` (read queries only, no setting changes). If set to true, a client may drop to `1` from `0` or `2`, and no other change of `readonly` is accepted; additionally, `changeable_in_readonly` stops applying to `readonly` itself on an HTTP request whose method implies readonly, so such a request can no longer leave readonly mode. Every other setting keeps that constraint type, and so does `readonly` over `POST` and the native protocol. | `false` |
