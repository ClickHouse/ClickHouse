...
| `only_synced_users` | serve only the users the last successful synchronization materialized, `true` by default. A name outside the snapshot is reported as not found for every kind of credential, so the user directories that follow are consulted. |
