...
| `readonly_can_only_be_tightened` | Sets whether a session at `readonly = 2` may change `readonly` only towards more restriction. The restriction order is not the numeric one: `0` (no restriction) &lt; `2` (read queries only, any setting but `readonly`) &lt; `1` (read queries only, no setting changes). If true, such a session may drop `readonly` to `1` and make no other change to it. At `readonly = 1` a profile marking `readonly` as `changeable_in_readonly` still lets a session leave readonly mode over `POST` and the native protocol; this setting stops that constraint from covering `readonly` on a read-only HTTP method. A profile whose own values set `readonly` to `0` is applied to the session rather than checked against the request, so it is not covered either. | `false` |
