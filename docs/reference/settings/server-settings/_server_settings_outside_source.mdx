...
| `readonly_restricts_set_role` | Sets whether `SET ROLE` is refused when the session or the query is readonly. When `false`, `readonly` does not restrict `SET ROLE`. | `false` |
