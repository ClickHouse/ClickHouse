...
| `readonly_can_only_be_tightened` | Sets whether the `readonly` setting may only be changed towards more restriction while readonly mode is in force. The order of restriction is not the numeric order: `0` (no restriction) &lt; `2` (read queries only, any setting but `readonly`) &lt; `1` (read queries only, no setting changes). If set to true, a session in readonly mode may drop `readonly` to `1` and every other change of `readonly` from `1` or `2` is refused, while at `0` nothing is in force and `readonly` keeps its ordinary constraints; additionally, `changeable_in_readonly` stops applying to `readonly` itself on an HTTP request whose method implies readonly, so such a request can no longer leave readonly mode. Every other setting keeps that constraint type, and so does `readonly` over `POST` and the native protocol, where a profile declaring it on `readonly` can still take a session out of readonly mode: enabling this setting therefore lets a session at `readonly = 2` reach `0` in two steps there, and because a profile's name is selectable by any session it is enough that some profile on the server declares the constraint. The narrowing covers the request's own context and its in-process copies, not a query forwarded to another server. | `false` |
...
<access_control_improvements>
    ...
    <readonly_can_only_be_tightened>false</readonly_can_only_be_tightened>
