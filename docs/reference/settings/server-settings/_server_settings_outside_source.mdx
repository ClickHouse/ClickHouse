...
| `max_staleness` | seconds; when greater than `0`, logins of synchronized users are refused while the last successful run is older than this. Requires `only_synced_users` = `true`, `interval` > `0` and a value greater than `interval`; at most `315360000` (ten years). `0` (the default) disables the gate. |
