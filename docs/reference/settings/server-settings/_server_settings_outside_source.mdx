...
| `functions_requiring_grant` | Functions that can be called only with an explicit `GRANT FUNCTION ON <name>`, each listed in its own `<function>` element. Applies to ordinary functions and to SQL, executable and WebAssembly user-defined functions; aggregate and window function names are rejected at startup, and table functions are not covered. Names are matched after resolving aliases, both in this list and in the grant, so `hex` also covers `HEX`. A name that is not a known function is kept with a warning, because a user-defined function with that name may be created later. `GRANT ALL` includes `FUNCTION ON *`. Read at server start only. | empty |
