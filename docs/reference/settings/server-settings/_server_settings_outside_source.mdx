...
The section is optional. The default server configuration file `config.xml` contains this section with `merge_rotated_tables` enabled, so the `all_...` tables exist out of the box; the `cluster` setting has to be configured explicitly. A table that already exists under one of the `all_...` names and was not created by this feature (for example, a hand-made table with data) is never replaced: it is left intact and a warning is written to the server log. If the section is present, at least one of the following settings has to be specified:
