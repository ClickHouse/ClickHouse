...
| Setting | Description |
|---------|-------------|
| `interval` | seconds between two synchronizations, `900` by default; the first run starts after a random delay of up to a tenth of the interval, a failed run is retried after min(`interval`, 60) seconds. `0` runs the synchronization at startup and on `SYSTEM RELOAD USERS` only. |
...
| `max_staleness` | seconds; when greater than `0`, logins of synchronized users are refused while the last successful run is older than this. Requires `only_synced_users` = `true`, `interval` > `0` and a value greater than `interval`. `0` (the default) disables the gate. |
