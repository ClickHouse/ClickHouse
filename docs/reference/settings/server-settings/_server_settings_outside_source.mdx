...
The section is optional. The default server configuration file `config.xml` contains this section with `merge_rotated_tables` enabled, so the `all_...` tables are created automatically on a default server: at the first flush of each log, or on startup if `prepare_system_log_tables_on_startup` is enabled; the `cluster` setting has to be configured explicitly. A table that already exists under one of the `all_...` names and was not created by this feature (for example, a hand-made table with data) is never replaced: it is left intact and a warning is written to the server log, and the union table is created at the next flush of the log once that table is dropped or renamed. The tables created by this feature are recognized by the marker sentence at the end of their comment: `It is safe to drop this table at any time: it will be recreated automatically.` If the section is present, at least one of the following settings has to be specified:
