...
They also work in DDL statements: in place of database and table names (for example `CREATE TABLE {db:Identifier}.{tbl:Identifier}`), and in `ALTER` statements in place of the names of the columns, indices, projections, and constraints being referenced (for example `ALTER TABLE t RENAME COLUMN {c:Identifier} TO {c2:Identifier}`).
</Note>
