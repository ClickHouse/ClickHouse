...
by using the [`input_format_values_interpret_expressions`](/reference/settings/formats/input-format#input_format_values_interpret_expressions) setting.
...
When a query is received, the server calculates no more than [max_query_size](/reference/settings/session-settings/max#max_query_size) bytes of the request in RAM
...
- The numeric literal is first parsed as a 64-bit unsigned integer, using the [strtoull](https://en.cppreference.com/w/cpp/string/byte/strtoul) function.
  ...
  - A negative value whose absolute magnitude is greater than 2<sup>63</sup> does not fit `Int64`, so it is handled like an overflow of the 64-bit range.
- If the integer does not fit a 64-bit type (a positive value beyond the `UInt64` range, or a negative value beyond the `Int64` range), the behavior depends on the ClickHouse version:
  - **Version 26.10 and later**: a *decimal* integer literal is parsed as a wide integer (`UInt128`, `Int128`, `UInt256`, or `Int256`), whichever is the smallest signed or unsigned type that fits. If the value exceeds all integer types, it falls back to `Float64`. An overflowing hexadecimal (`0x`/`0X`) integer literal is kept as `Float64` for compatibility, not widened, while an overflowing binary (`0b`) literal is a syntax error. Decimal-point literals (e.g. `3.14`) and exponent literals (e.g. `1e10`) are stored internally as deferred-parsed values and resolved to `Float64` by default. Next to a `Decimal` argument they are parsed from the original text instead: a comparison uses a wide `Decimal`, and any other function casts the literal to the `Decimal` type of the other argument (`price * 1.1` with `price Decimal64(2)` treats `1.1` as `Decimal64(2)`), widening it only where it does not fit. The result then follows the usual `Decimal` arithmetic rules, exactly as with an explicit `Decimal` constant. A literal with more than 76 significant digits does not fit `Decimal256` and goes through `Float64`. An integer literal too large for `UInt64` next to a floating-point argument is read as `Float64`.
  - **Before version 26.10**: the literal is parsed as a `Float64` using [strtod](https://en.cppreference.com/w/cpp/string/byte/strtof), which can silently lose precision for large integers. For example, `100000000000000000000000` is parsed as `Float64(1e23)` and adjacent values become indistinguishable. To avoid this, string-based conversion was required: `toUInt128('100000000000000000000000')`.
...
#### Changes in version 26.10 {#numeric-literal-changes-26-10}

Starting with version 26.10, numeric literal parsing was improved in two ways:

**1. Wide integer literals are parsed natively.** Integer values wider than 64-bit no longer require explicit casts:

```sql
-- These now work directly (version 26.10+):
SELECT -170141183460469231731687303715884105728;  -- Int128
SELECT 340282366920938463463374607431768211455;   -- UInt128

-- Before version 26.10, string-based conversion was required to avoid Float64 precision loss:
...
```sql
-- Version 26.10+: literal "1.123456789012345678" is compared exactly as Decimal128(18)
SELECT count() FROM table WHERE decimal_col = 1.123456789012345678;

-- Before 26.10: the literal was first parsed as Float64(1.1234567890123457),
...
<Note>
You can change this default behavior by setting [prefer_column_name_to_alias](/reference/settings/session-settings/prefer#prefer_column_name_to_alias) to `1`.
</Note>
