...
- If the integer does not fit a 64-bit type (a positive value beyond the `UInt64` range, or a negative value beyond the `Int64` range), the behavior depends on the ClickHouse version:
  - **Version 26.10 and later**: a *decimal* integer literal is parsed as a wide integer (`UInt128`, `Int128`, `UInt256`, or `Int256`), whichever is the smallest signed or unsigned type that fits. If the value exceeds all integer types, it falls back to `Float64`. An overflowing hexadecimal (`0x`/`0X`) integer literal is kept as `Float64` for compatibility, not widened, while an overflowing binary (`0b`) literal is a syntax error. Decimal-point literals (e.g. `3.14`) and exponent literals (e.g. `1e10`) are stored internally as deferred-parsed values and resolved to `Float64` by default, but when compared with a `Decimal` column they are parsed directly to the target `Decimal` type from the original text, preserving full precision. A literal with more than 76 significant digits does not fit `Decimal256` and is compared through `Float64`.
