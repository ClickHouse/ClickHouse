...
- **The `with_source` type parameter adds a sub-column, not a version.** `JSON(with_source=1)` keeps the same serialization versions and adds a `__source` `String` sub-column in every version except `STRING` (where the JSON text is the payload itself). The parameter is a part of the type name, and in [binary type encoding](/reference/data-types/data-types-binary-encoding) it bumps the `JSON` type encoding version to `1`.
...
When the type is declared with `with_source=1` and the setting `output_format_json_type_use_source = 1` is set, the emitted text is the JSON text stored in the column's `__source` sub-column (so the original formatting of the input is preserved); otherwise it's constructed from the parsed object. Either way the wire layout is the same, and there is no separate `__source` stream in this tier.
...
A column whose type is declared with `with_source=1` carries one more sub-column, the `__source` sub-column with the JSON text of every row. It's a plain `String` column that is not a path, and it comes right after the version, before any path state prefix and path data:

```text
[per block with rows > 0]:
  -- prefix phase (repeated at the start of every non-empty block):
  [8 bytes:  UInt64 LE version = 3]                ← state prefix
  [__source state prefix]                          ← only with `with_source=1`; String state prefix
  ...
  -- data phase:
  [__source data]                                  ← only with `with_source=1`; num_rows JSON texts
