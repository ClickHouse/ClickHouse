...
At revision `54493` and above, `JSON` with declared typed paths uses the same recursive shape: `[json_kind][path_0_kind]...[path_n_kind]`. Typed paths are ordered lexicographically by their full declared path names; dynamic paths and shared data do not have entries in this metadata. Below revision `54493`, declared paths are materialized dense and only the outer `JSON` kind is present.

At revision `54493` and above, when `Nullable` wraps a composite that carries recursive serialization info, such as `Nullable(JSON(...))`, the payload is `[nullable_kind][inner_kind...]`. The first node is normally `DEFAULT`, but it preserves an outer `REPLICATED` or `DETACHED` representation. Below revision `54493`, only the `Nullable` column's own kind is present and nested declared `JSON` paths are materialized dense. A `Nullable` that is itself sparse instead has the single sparse kind described below.
...
| Recursive serialization for declared `JSON` paths | `DBMS_MIN_REVISION_WITH_JSON_TYPED_PATHS_SERIALIZATION` | `54493` | Declared typed paths are materialized dense and their recursive kind-stack entries are omitted. |
