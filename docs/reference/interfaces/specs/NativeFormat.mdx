...
- the body of `Data`, `Totals`, `Extremes`, `Log`, `ProfileEvents`, and `ProfileTraces` packets in the [native TCP protocol](/reference/interfaces/specs/NativeProtocol) (the `TableColumns` packet is **not** a Native block — it carries two binary strings, so its layout belongs to the [native protocol spec](/reference/interfaces/specs/NativeProtocol));
...
The `ProfileTraces` packet, available at revision `54493`+, carries a block with `host_name String`, `query_id String`, `trace_type String`, `thread_id UInt64`, `event_time_microseconds UInt64`, `trace Array(UInt64)`, `symbols Array(String)`, and `size Int64`, in that order. The supported `trace_type` values are `CPU`, `Real`, `Memory`, `MemorySample`, and `MemoryPeak`; other trace types are excluded. These columns use the ordinary negotiated `Native` encodings, including the revision-dependent `String` layout for `symbols` elements. The block body uses the query's compression flag; the packet-type code and empty `table_name` string remain outside the compression frame. Delivery is best effort: a collector timeout discards remaining samples with a server warning while preserving the query result. Native client uploads defer trace blocks until their input terminator; an earlier exception or query completion discards queued samples without trace or loss-status packets. See [trace packet delivery and schema](/reference/interfaces/specs/NativeProtocol#profiletraces).

These auxiliary blocks may be interleaved with query results and trailing `INSERT` metadata. Trace delivery waits until the client finishes uploading external tables, scalars, or insertion rows. After external input ends, a pre-schema exception may be preceded by a trace drain without any schema block. A trace block must not be interpreted as a schema, result block, or completion marker; see the native protocol's [query phase](/reference/interfaces/specs/NativeProtocol#query-phase) and [INSERT phase](/reference/interfaces/specs/NativeProtocol#insert-phase).

The same block can contain delivery status rows with `trace_type = 'Dropped'` or `'Incomplete'`. Both use empty arrays and zero thread/time fields. `Dropped` carries a positive queue-loss delta in `size`, saturating at the maximum `Int64`; `Incomplete` uses zero and marks a collector flush timeout. These are metadata, not stack samples. A block contains at most 1024 rows including status rows. Their host/query fields identify the reporting coordinator, and forwarded loss deltas are aggregated once across its distributed subtree. Counts exclude losses before the collector. See the [status semantics](/reference/interfaces/specs/NativeProtocol#profiletraces) for completeness and compatibility limits.
...
- **Native TCP protocol.** Compression is opt-in per query via the `compression` flag in the [Query packet](/reference/interfaces/specs/NativeProtocol#query). When active, the body of each `Data`, `Totals`, `Extremes`, `Log`, `ProfileEvents`, and `ProfileTraces` packet — the bytes after the `table_name` string — is wrapped in the frame format, subject to the negotiated revision: `Log` and `ProfileEvents` remain uncompressed below `54481`, and `ProfileTraces` is available from `54493`. The packet envelope itself, the packet-type code and the `table_name` string, is **not** compressed; the server writes those to the raw stream. Everything the `NativeWriter` emits goes into the compressed stream, so the `BlockInfo` prefix is the first thing inside the frame, along with the dimensions and columns. A client must therefore decompress the frame before it can read `BlockInfo`.
...
For packets carrying a `Native` block over TCP, the packet envelope — the packet-type VarUInt and the `table_name` string — is written to the **raw** stream, outside the compressed payload; only the block body (BlockInfo + columns) is framed. The HTTP `compress`/`decompress` path has no such envelope: the whole stream is framed blocks.

### Negotiation {#compression-negotiation}

On the native TCP protocol, compression is per-query, not per-connection. The Query packet's `compression: bool` field requests it for that single query. The server honours the request and emits compressed `Data`/`Totals`/`Extremes`/`Log`/`ProfileEvents`/`ProfileTraces` bodies for the lifetime of the query (`Log`/`ProfileEvents` only at v54481+, `ProfileTraces` available at v54493+). It also expects the client's *outgoing* Data blocks — external tables, the empty end-of-data marker, and INSERT rows — to be framed the same way. Subsequent queries on the same connection may differ.

[`TableColumns`](/reference/interfaces/specs/NativeProtocol#tablecolumns) also follows the query's compression flag from revision `54481`. Its body contains two strings, `external_table` and `columns_description`, rather than a `Native` block; both strings are inside the compressed payload, while the packet-type code remains outside. Below that revision the body is uncompressed.
