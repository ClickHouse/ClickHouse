...
Within that binary type encoding, the `JSON` tag (`0x30`) is followed by a separate `UInt8` type-encoding version. Version `0` ends after the typed paths, skipped paths, and skipped-path regular expressions. Version `1` then writes a `VarUInt` count of `SHARED REGEXP` rules; each rule is one match-mode byte (`0` = partial, `1` = full) followed by its pattern as a `String`. A final `String` stores the internal root-relative path prefix used by derived JSON sub-object types. Writers keep using version `0` when no `SHARED REGEXP` policy is present, so existing JSON type encodings remain byte-identical. This type-encoding version is independent of the JSON column serialization/state versions described later on this page.
</Info>
