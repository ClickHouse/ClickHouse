...
These auxiliary blocks may be interleaved with query results and `INSERT` metadata, including before a schema block, during row upload, and during the final drain. They must not be interpreted as schemas, result blocks, or completion markers; see the native protocol's [query phase](/reference/interfaces/specs/NativeProtocol#query-phase) and [INSERT phase](/reference/interfaces/specs/NativeProtocol#insert-phase).
...
- **Native TCP protocol.** Compression is opt-in per query via the `compression` flag in the [Query packet](/reference/interfaces/specs/NativeProtocol#query). When active, the body of each `Data`, `Totals`, `Extremes`, `Log`, `ProfileEvents`, and `ProfileTraces` packet — the bytes after the `table_name` string — is wrapped in the frame format. The packet envelope itself, the packet-type code and the `table_name` string, is **not** compressed; the server writes those to the raw stream. Everything the `NativeWriter` emits goes into the compressed stream, so the `BlockInfo` prefix is the first thing inside the frame, along with the dimensions and columns. A client must therefore decompress the frame before it can read `BlockInfo`.
