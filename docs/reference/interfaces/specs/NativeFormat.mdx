...
- `count` is at most the number of kinds, since each kind wraps the serialization once;
- `DETACHED`, when present, is the last entry, because a `ColumnBLOB` holds the serialized form of the whole column and nothing can wrap it.
