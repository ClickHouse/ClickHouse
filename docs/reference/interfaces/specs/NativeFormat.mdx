...
Both size fields are 32-bit on the wire, but each is capped at `0x40000000` (1 GiB) **inclusive**: exactly `0x40000000` is accepted and only a strictly larger value is rejected. A decoder checks both from the header alone, before allocating anything, and fails with `TOO_LARGE_SIZE_COMPRESSED`. Nested frames inside a `Multiple` codec use the same limit.
...
A writer caps its own frame at the same value, so a current writer never emits a frame a current reader rejects: a writer asked for a larger buffer emits several frames within the limit instead of one above it. Readers of data written before that cap existed keep accepting larger frames, so previously written `Log`, `StripeLog`, MergeTree part, backup and `clickhouse compressor` files stay readable. A frame arriving over a socket is always subject to the limit.
