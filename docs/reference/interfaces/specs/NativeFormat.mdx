...
Sparse array elements are materialized before writing because `Array` has no recursive serialization-info payload in the `Native` format. This applies to all element types, including arrays without `JSON` descendants.
