...
- **`V3` is on-disk only.** `Object::V3` and `Dynamic::V3` are written into MergeTree parts and never onto the wire, so a decoder never sees them here, and an encoder must not send them: ClickHouse rejects either of them in a `Native` stream with `INCORRECT_DATA`. This matters beyond tidiness, because `Object::V3` is what carries the shared-data sub-version, and the bucket and granule layouts of `MAP_WITH_BUCKETS` and `ADVANCED` describe an on-disk part rather than a block. Clients consuming the native TCP protocol see `V1`, `V2`, `STRING` or `FLATTENED`.
