...
The value is emitted as its compact JSON text — `{"a":1}`, with the integer left as an integer. When `output_format_json_skip_null_value_in_json_columns = 1`, paths with null values are omitted from this text. The text is just a `String` value, so the client receives the JSON for opaque transit but does not recover the individual paths and their ClickHouse types; faithful per-path typing requires the Tier 2 encoding below.
