...
The [binary type encoding](/sql-reference/data-types/data-types-binary-encoding) of `Row` is tag `0x38` followed by the field count and, per field, the name and the encoded field type — the same shape `Nested` uses. A block header carrying a `Row` type is therefore writable with `output_format_native_encode_types_in_binary_format = 1`.
