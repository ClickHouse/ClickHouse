...
`0xa0` (`Wallaby`) is an **experimental** lossless codec for `Float32` and `Float64` columns that splits data into vectors of 1024 values and adaptively picks the cheapest of several encodings per vector (constant collapse, integerization with Frame-of-Reference or zigzag-delta bit-packing, XOR against a ring of 256 recent reference values, or raw). A table can be created with `CODEC(Wallaby)` only when the `enable_wallaby_codec` setting is set, but the method byte is always accepted on decompression so that previously written data stays readable.
