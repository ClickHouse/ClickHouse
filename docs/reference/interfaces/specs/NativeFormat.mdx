...
When ClickHouse decodes these text values, it reads the current parser setting and session timezone for each value, including changes made within an existing session. Implicit `DateTime` and `DateTime64` typed paths, including nested paths, use the session timezone at decoding time; explicitly declared timezones remain fixed. Cached `JSON` serializations account for the effective session timezone of typed `DateTime` and `DateTime64` paths. Immutable `JSON` serializations can be shared; each active parse leases its own parser and extraction tree from the format settings. Default format settings do not allocate a parsing holder. The holder is created when settings are first copied or used for parsing. Parsing pools, parsers, and extraction trees are created on the first text value and retained with those settings. Concurrent parsers use independent leases from sharded pools. Typed-path schemas are validated when the serialization is constructed; parsing a type declaration alone does not run this validation. This does not change the version marker or the bytes on the wire.
