...
This section documents the wire encoding of the types the Native format can carry within a column's `data`, grouped into four families of increasing decoder complexity. Two types — `AggregateFunction(func, ...)` and `QBit(T, N[, stride])` — are valid `Native` column types with specialized payloads rather than alias encodings. Their decoding requirements are called out below.
...
<Note>
Two related types are **not** aliases. They are valid `Native` column types — a client can receive an `AggregateFunction` column from a `-State` combinator or distributed aggregation, for instance — but each requires a decoder for its specialized payload. Function-specific aggregate state payloads are outside the scope of this page:
