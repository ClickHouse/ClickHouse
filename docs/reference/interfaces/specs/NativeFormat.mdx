...
`Native` protocol revision `54493` explicitly guarantees that the receiver accepts `NONE` frames mixed with the configured network codec. When both peers support this revision, the sender uses `NONE` for frames with fewer than `network_compression_min_bytes` uncompressed bytes. The setting defaults to `128`; `0` disables the bypass. Frames at or above the threshold retain `network_compression_method`. This decision applies independently to each frame, including frames within the same block or query, and preserves the 9-byte header, 16-byte checksum, and existing frame boundaries. It applies in both directions of the native TCP protocol, including schema headers, terminal empty blocks, logs, and profile events. HTTP and storage compression are unaffected.

Peers advertising an earlier revision continue receiving the configured codec for every frame. Support must not be inferred from a client name or an earlier revision: some third-party `Native` drivers only implement `LZ4` and `ZSTD`, despite `NONE` being an existing method byte. A client advertising revision `54493` or later must implement this mixed-frame contract.
