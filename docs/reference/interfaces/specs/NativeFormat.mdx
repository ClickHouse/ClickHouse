...
| 64-bit hashes in `uniq` state | `DBMS_MIN_REVISION_WITH_UNIQ_STATE_VERSION_1` | `54492` | `AggregateFunction(uniq, ...)` states are written at state version `0`: no leading flags byte, and the stored hash values truncated to 32 bits. See the note under [type aliases](#type-aliases). |
