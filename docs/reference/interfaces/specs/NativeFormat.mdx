...
`0xa0` (`ByteStreamSplit`) is an **experimental** preprocessing codec for fixed-width data: it transposes the bytes of every element so that the bytes at the same position within each element form a contiguous stream, which a following general-purpose codec compresses better. Like the other preprocessors it must be chained with a compression codec. A table can be created with `CODEC(ByteStreamSplit)` only when the `enable_bytestreamsplit_codec` setting is set, but the method byte is always accepted on decompression so that previously written data stays readable.
