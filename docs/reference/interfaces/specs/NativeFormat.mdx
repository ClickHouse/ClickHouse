...
In that encoding the `JSON` tag (`0x30`) is followed by a `UInt8` version. Version 0 ends after the skipped-path regular expressions. Version 1 then adds `<var_uint_number_of_shared_path_regexps><var_uint_shared_path_regexp_size_1><shared_path_regexp_data_1>...` and is written only for `JSON` types with `SHARED REGEXP`, so other `JSON` types are encoded as before.
</Info>
