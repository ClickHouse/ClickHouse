...
Both size fields are 32-bit on the wire, but each is capped at `0x40000000` (1 GiB) **inclusive**: exactly `0x40000000` is accepted and only a strictly larger value is rejected. A decoder checks both from the header alone, before allocating anything, and fails with `TOO_LARGE_SIZE_COMPRESSED`. A `Multiple` codec bounds the sizes of its nested frames by the same constant, but exclusively: a nested frame of exactly `0x40000000` is rejected.
...
A writer caps its own frame so that both fields stay within the limit: a writer asked for a larger buffer emits several frames within the limit instead of one above it. The cap is derived from the worst-case expansion the codec in use reports, so the frame is smaller than the limit by that amount, and a codec that reports its expansion correctly never emits a frame a reader rejects. A codec that computes that figure in 32-bit arithmetic can understate it; a frame that exceeds the limit anyway is rejected by the reader's own check. Data written before the cap existed can carry larger frames, and it stays readable where it is read back from local storage; a frame arriving over a socket is always subject to the limit.
