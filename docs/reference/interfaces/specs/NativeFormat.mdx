...
Typed paths are always materialized to their regular dense columns before `JSON` is written in the `Native` format, including when the `JSON` column is nested in `Array`, `Nullable`, `Tuple`, or a replicated representation. MergeTree parts may store an individual declared path with sparse serialization, but that per-path choice is not added to the outer column's `Native` kind stack and does not change the wire layout below. Sparse serializations nested in `Array` are also materialized because `Array` has no recursive serialization-info payload in the `Native` format.
