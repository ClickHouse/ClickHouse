...
The `ProfileTraces` packet, available at revision `54493`+, carries a block with `host_name String`, `query_id String`, `trace_type String`, `thread_id UInt64`, `event_time_microseconds UInt64`, `trace Array(UInt64)`, `symbols Array(String)`, and `size Int64`, in that order. The supported `trace_type` values are `CPU`, `Real`, `Memory`, `MemorySample`, and `MemoryPeak`; other trace types are excluded. These columns use the ordinary negotiated `Native` encodings, including the revision-dependent `String` layout for `symbols` elements. The block body uses the query's compression flag; the packet-type code and empty `table_name` string remain outside the compression frame. Delivery is best effort: a collector timeout discards remaining samples with a server warning while preserving the query result. See [trace packet delivery and schema](/reference/interfaces/specs/NativeProtocol#profiletraces).

These auxiliary blocks may be interleaved with query results and `INSERT` metadata. For a direct streamed `INSERT` with no external tables, trace packets follow the schema block during row upload and final drain; an early exception may instead be preceded by a trace drain without any schema block. Traces can also be sent while receiving nonempty external-table blocks, before the insertion schema is prepared. A trace block must not be interpreted as a schema, result block, or completion marker; see the native protocol's [query phase](/reference/interfaces/specs/NativeProtocol#query-phase) and [INSERT phase](/reference/interfaces/specs/NativeProtocol#insert-phase).
