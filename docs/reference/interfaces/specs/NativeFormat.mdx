...
| Aggregate-function versioning | `DBMS_MIN_REVISION_WITH_AGGREGATE_FUNCTIONS_VERSIONING` | `54452` | The `AggregateFunction` *state* is written at version `0`, and no version is negotiated from the revision. An ordinary leaf's *type string* omits the version, because version `0` is never printed, so a decoder derives it from the revision rather than from the string. A leaf nested inside a [`SimpleAggregateFunction`](#type-aliases) is a special case, in the same shape as `LowCardinality` above: a version token is spelled out whenever the versionless spelling would re-parse to something other than the version negotiated for the peer, which happens at revision `0` because that is the local persister, rebuilding the leaf by parsing back its own printed name, and equally at a non-zero revision that is below the *function's own* threshold but at or above `54452`, where the token is parseable and the negotiated version is still `0`. At a *non-zero* revision below `54452` the peer predates versioning entirely and has no grammar for the version token, so the leaf is announced without one, which is also what such a peer writes itself. |
