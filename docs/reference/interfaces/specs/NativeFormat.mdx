...
The `ProfileTraces` packet, available at revision `54493`+, carries a block with `host_name String`, `query_id String`, `trace_type String`, `thread_id UInt64`, `event_time_microseconds UInt64`, `trace Array(UInt64)`, `symbols Array(String)`, and `size Int64`, in that order. These columns use the ordinary negotiated `Native` encodings, including the revision-dependent `String` layout for `symbols` elements. The block body uses the query's compression flag; the packet-type code and empty `table_name` string remain outside the compression frame. See [trace packet delivery and schema](/reference/interfaces/specs/NativeProtocol#profiletraces).
