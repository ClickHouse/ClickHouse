...
### Size limits {#frame-size-limits}

Both size fields are 32-bit on the wire, but each is capped at `0x40000000` (1 GiB) **inclusive**: exactly `0x40000000` is accepted and only a strictly larger value is rejected. A decoder checks both from the header alone, before allocating anything, and fails with `TOO_LARGE_SIZE_COMPRESSED`. A `Multiple` codec applies the same constant to its nested frames, but exclusively (exactly `0x40000000` is rejected) and to two quantities only: the length of the intermediate buffer it tracks itself, and each nested header's declared `uncompressed_size`. A nested header's declared `compressed_size` is not inspected, because each nested decoder is handed the length the outer decoder tracks rather than one read off the nested header.

The checksum does not gate these checks. It is an unkeyed CityHash128 over bytes the sender chose, so it detects corruption in transit but not a crafted header, and a decoder that sized a buffer from `uncompressed_size` before checking it would honour whatever the sender declared.

A writer caps its own frame, so a writer asked for a larger buffer emits several frames within the limit instead of one above it. The cap is derived from the worst-case expansion the codec in use reports, and it reduces the frame whenever that figure leaves room to subtract it. Two cases leave no room: a reported expansion larger than the limit can absorb (a deep codec chain), and one computed in wrapping 32-bit arithmetic, which understates it. In both the request passes through unreduced, and the reader's own bound is the backstop that rejects the frame. Data written before the cap existed can carry larger frames, and it stays readable where it is read back from local storage; a frame arriving over a socket is always subject to the limit.

For a method byte whose codec stores the body verbatim (`NONE`, `Quantized`), the body length and `uncompressed_size` describe the same bytes, so `compressed_size - 9` must equal `uncompressed_size`; a frame where they disagree is rejected at header-parse time.
