...
When `Nullable` wraps a composite that carries recursive serialization info, such as `Nullable(JSON(...))`, it adds no kind-stack node of its own: the payload is the wrapped composite's recursive payload. A `Nullable` that is itself sparse instead has the single sparse kind described below.
