...
An omitted token therefore does not mean the function's default outside a `SimpleAggregateFunction`, and a decoder must not read it that way. A leaf declared with an explicit `AggregateFunction(0, ...)` is announced without a token at revision `0`, and at a non-zero revision below the function's own threshold, because version `0` is never printed. The payload is version `0` in both cases, so the stream is consistent, but only for a decoder that derives the version from the revision the way `NativeReader` does (`setVersionToAggregateFunctions` with the negotiated revision) rather than from the printed name. Re-parsing the name instead yields the function's default, which is `1` for the map and bitmap families, and version `1` of the map family serializes promoted values: a value type that promotion widens, such as `Decimal32`, then decodes to wrong values or fails outright, while one it leaves alone, such as `UInt64`, happens to stay byte-compatible.
