...
A limit is not a validation, so for a method byte whose codec stores the body verbatim (`NONE`, `Quantized`) the two fields are checked against each other as well: they describe the same bytes, so `compressed_size - 9` must equal `uncompressed_size`, and a frame where they disagree is rejected. The same rule applies to a nested frame, where the body length is the length the outer decoder tracks for it rather than a field of the nested header, and the comparison runs before the buffer sized from `uncompressed_size` is allocated.

All of these checks belong to the decoder. Beyond them the format does not constrain the sizes a writer puts in a header.
