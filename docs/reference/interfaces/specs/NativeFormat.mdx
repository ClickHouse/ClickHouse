...
Typed paths are always materialized to their regular dense columns before `JSON` is written in the `Native` format, including when the `JSON` column is nested in `Array`, `Nullable`, `Tuple`, or a replicated representation. MergeTree parts may store an individual declared path with sparse serialization, but that per-path choice is not added to the outer column's `Native` kind stack and does not change the wire layout below. This materialization is limited to the `JSON` subtree; other sparse elements of the surrounding composite type remain sparse.
