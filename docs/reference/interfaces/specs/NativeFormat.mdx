...
- the typed query parameter block appended to a `Query` packet at protocol revision `54493` and above;
...
| Typed query parameter block | N > 0 | 1 | One query parameter per named column; carried directly in a `Query` packet at revision `54493` and above. |
...
| Typed query parameter block | `DBMS_MIN_PROTOCOL_VERSION_WITH_TYPED_QUERY_PARAMETERS` | `54493` | No Native block follows the text parameter list in a `Query` packet; clients must use text parameters. |
