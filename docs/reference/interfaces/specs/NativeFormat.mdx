...
These auxiliary blocks may arrive before an `INSERT` schema block, during row upload, and during the final drain. They must not be interpreted as insertion schemas, result blocks, or completion markers; see the [native protocol's INSERT phase](/reference/interfaces/specs/NativeProtocol#insert-phase).
