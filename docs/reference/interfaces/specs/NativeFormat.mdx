...
An `Exception` packet contains no `Native` block. Starting with negotiated TCP revision 54493, its legacy fields are followed by the original query, symbolic error code name, and query identifier, each encoded as a binary string. Handshake exceptions retain the legacy layout. See the [exception packet specification](/interfaces/specs/NativeProtocol#exception) for the field order and negotiation rules. This revision does not change column encoding or compression frames.
