...
The `count` is the full stack length **including the leading `DEFAULT` entry** that begins every stack. The compact codes cover the legacy one- and two-entry stacks, but an automatic `LowCardinality` serialization uses the two-entry stack `{DEFAULT, LOW_CARDINALITY}`. It is consequently encoded as `COMBINATION` with `count = 2` and nested bytes `0x00, 0x04`.
...
**Sparse wire format.** When the compact `kind_stack` code is `0x01` (`SPARSE`), the column `data` is two streams written back-to-back in the single shared TCP stream:
...
| Automatic `LowCardinality` serialization | `DBMS_MIN_REVISION_WITH_AUTOMATIC_LOW_CARDINALITY_SERIALIZATION` | `54493` | Non-native `LOW_CARDINALITY` kind stacks are materialized to the default `String` or `FixedString` serialization. This prevents older peers, which do not recognize kind `4`, from reading an unknown custom serialization kind. |
