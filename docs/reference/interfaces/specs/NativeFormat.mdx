...
| `first_value` and `is_null` in `singleValueOrNull` state | `DBMS_MIN_REVISION_WITH_SINGLE_VALUE_OR_NULL_STATE_VERSION` | `54493` | Below this revision, `AggregateFunction(singleValueOrNull, ...)` states are written at state version `0` without the trailing semantic flags. At or above it, version `1` appends the flags. |
...
- `singleValueOrNull` states are versioned from revision `54493`: version `1` appends trailing `Bool first_value` and `Bool is_null` fields, while version `0` remains the legacy payload without those semantic flags.
- `singleValueOrNull` states are versioned from revision `54493`: version `1` appends trailing `Bool first_value` and `Bool is_null` fields, while version `0` remains the legacy payload without those semantic flags. When reading version `0`, an empty payload represents no value and a non-empty payload must be treated as `NULL`, because the legacy bytes cannot distinguish one distinct value from multiple distinct values.
- `singleValueOrNull` states are versioned from revision `54493`: version `1` appends trailing `Bool first_value` and `Bool is_null` fields, while version `0` remains the legacy payload without those semantic flags. When reading version `0`, after decoding the legacy nested state, `has() == false` means no value, while `has() == true` must be finalized as `NULL`, because the legacy bytes cannot distinguish one distinct value from multiple distinct values.
