...
- no kind occurs more than once, since each kind wraps the serialization once — a repeat would nest a column inside its own layout, such as a `ColumnSparse` whose values column is itself sparse, which no materialization step unwraps back to a full column. This also bounds `count` by the number of kinds;
