...
| Row-presence marker in unlimited `groupConcat` state | `DBMS_MIN_REVISION_WITH_GROUP_CONCAT_STATE_VERSION` | `54493` | Unlimited `AggregateFunction(groupConcat(...), ...)` states are written at state version `0`, without the trailing `VarUInt has_rows`. The limited form is unaffected. See [`groupConcat` state versions](#groupconcat-state-versions). |
...
The unlimited form of `AggregateFunction(groupConcat(delimiter), T)` has two state layouts. Version `0` is `[VarUInt data_size][data bytes]`. Version `1` appends `[VarUInt has_rows]`, where the marker is `0` or `1`. The marker distinguishes a genuinely empty state (`data_size = 0, has_rows = 0`) from a state containing one or more empty strings (`data_size` may be `0`, `has_rows = 1`). A version-`0` state cannot retain that distinction; when reading one, a non-empty payload restores only the fact that at least one row exists.

The limited form, `AggregateFunction(groupConcat(delimiter, limit), T)`, remains unversioned because its existing layout already stores the count: `[VarUInt data_size][data bytes][VarUInt num_rows][VarUInt start_offset][VarUInt end_offset]...`, with one start/end pair per row.

Negotiated revision `54493` and above selects version `1` for the unlimited form. Lower revisions select version `0`. An unversioned type keeps version `0` so persisted data written before this change remains readable; newly created types pin version `1` explicitly. The general revision-`0` and pinned-type rules described above still apply.
