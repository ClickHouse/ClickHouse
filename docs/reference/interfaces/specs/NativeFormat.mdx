...
`Native` uses the `basic` map serialization. The bucket-count and statistics streams used by `MergeTree` bucketed maps, including maps nested in `advanced` JSON shared data, are not part of the `Native` wire format. In those on-disk streams, disabled statistics omit both the presence flag and the statistics payload.
