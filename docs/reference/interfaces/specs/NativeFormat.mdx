...
| Aggregate-function versioning | `DBMS_MIN_REVISION_WITH_AGGREGATE_FUNCTIONS_VERSIONING` | `54452` | At a *non-zero* revision below the threshold the peer predates versioning and has no grammar for the version token, so the state is written at version `0` and its type string is announced without a token, which is also what such a peer writes itself. A leaf that no negotiated version is assigned to is the exception, because there is nothing to suppress: it keeps its declared spelling at every revision (see [type aliases](#type-aliases) for a leaf under a composite inside a `SimpleAggregateFunction`). Revision `0` is not that case: it keeps a version pinned on the type (see the paragraph below), and writes version `0` only where nothing is pinned. Inside a [`SimpleAggregateFunction`](#type-aliases) a pinned version must therefore be spelled out, because the alias rebuilds its value type by parsing the printed name back and version `0` is never printed, so an omitted token re-parses as the function's default. A version the peer negotiates replaces a declared one, so the token announced under the alias is the negotiated version and always matches the payload. |
...
<Note>
The *textual* header announces a versioned `AggregateFunction` leaf so that the announced `T` re-parses to exactly the version its payload was written at. That re-parse equivalence is the guarantee a decoder can rely on, and the writer enforces it where the leaf **is** `T` itself, which is the plain `SimpleAggregateFunction(anyLast, AggregateFunction(sumMap, ...))` shape. A composite between the alias and the leaf is outside it: the alias contributes a custom name, and the version walker leaves a wrapper whose custom name it cannot faithfully rebuild unreplaced, so that subtree keeps the type object declaration produced. No peer-negotiated version is assigned there, and the leaf is announced with the version its type carries after declaration, at *every* revision, including a non-zero one below `54452` at which the peer has no grammar for the token. This holds for `Array`, `Tuple`, `Map` and `Nested` alike, so a versionless declaration under one of them is announced `AggregateFunction(1, sumMap, ...)` even at `54450`, and an explicit `AggregateFunction(0, ...)` under one is announced versionless at every revision. `Nullable` cannot hold an `AggregateFunction` directly, but it can sit further up such a path (as in `Tuple(Nullable(Tuple(AggregateFunction(...))))`, which needs `enable_nullable_tuple_type = 1`), and a path through one behaves the same way. This is the behaviour of the version walker rather than of this writer, and it predates the textual header described here. Where the guarantee does apply, the announced version is the *negotiated* one, and it is spelled out whenever the type carries a version at all rather than being minimised against the versionless spelling. A declared version does not survive negotiation: it is kept only where it already equals the negotiated one, so a leaf declared `AggregateFunction(0, sumMap, ...)` inside a `SimpleAggregateFunction` is announced `AggregateFunction(1, sumMap, ...)` from `54452`, not versionless, even though the versionless spelling would re-parse to that same default `1`. Only a revision below the function's own threshold negotiates `0`: `groupBitmapAnd`, whose threshold is `54455`, is announced `AggregateFunction(0, ...)` at `54452` through `54454`, and `AggregateFunction(1, ...)` from `54455`. Revision `0` is the one case that keeps a declared version, so an explicit `AggregateFunction(0, ...)` is announced with its `0` there. The payload is always the announced version's payload: at any *non-zero* revision the explicit-`0`, versionless and explicit-`1` declarations produce byte-identical blocks, so the announced name and the payload agree. At revision `0` they do not, because that is the one revision where a declared `0` is kept rather than negotiated: the explicit-`0` declaration is announced and written at version `0` while the other two are announced and written at version `1`. A token is withheld for a second, independent reason at a *non-zero* revision below `54452`: that peer predates versioning and cannot parse the token at all (see [what the revision gates](#what-the-revision-gates)). The version is negotiated per peer, so a decoder must take it from the announced `T` rather than from the version a `CREATE TABLE` declared.

Any other type along that path is opaque to the walker. A leaf below one is never assigned a negotiated version, so the announced spelling keeps whatever the declaration carried, and because version `0` is never printed a decoder that re-parses that name sees the function's default there. This is the case below another `AggregateFunction` (its `argument_types` are never visited) and below a [`Dynamic`](#dynamic), among others. It is not an inconsistency in the stream: no negotiated version is assigned to either side, so the version the announced name re-parses to and the one the payload is serialized at are the same, and the round trip is lossless. That is a statement about the two versions agreeing, not about one type object: under a `SimpleAggregateFunction` the name and the payload come from the cached and storage trees respectively, which is exactly why they can disagree once a version *is* assigned. Such a position simply carries no peer-negotiated version to read.

A [`Variant`](#variant) is not opaque: the walker descends its alternatives, keeping their discriminator order and failing the transfer rather than collapsing two alternatives that would become the same type. Its alternatives are therefore re-versioned like any other descended container, and the type-name renderer does not follow it there, so a `SimpleAggregateFunction` alternative holding an explicitly version-`0` leaf is announced with the spelling described above while its payload is written at the assigned version.

The versioned base implementations are the map family (`sumMap`, `minMap`, `maxMap`, `sumMapWithOverflow`, `sumMapFiltered`, `sumMapFilteredWithOverflow`, `sumMappedArrays`, `minMappedArrays`, `maxMappedArrays`), `groupBitmapAnd`, `groupBitmapOr`, `groupBitmapXor`, and `quantileDeterministic` (with the `quantiles`/`median` spellings of the same function), whose default version is `0` rather than `1`, so an omitted token there re-parses to `0`; combinators propagate versioning, so a derived name such as `sumMapIf` is versioned too. A combinator that also forwards the revision lookup, such as `If`, inherits its nested function's threshold, while one that does not, such as `Merge` or `OrNull`, reports a versioned function that negotiates version `0` at every revision. Each decides its own version from the revision, and the thresholds differ (`54452` for the map family, `54455` for the bitmap ones), so plain `groupBitmap` is not versioned at all and no single revision marks the boundary. Outside a `SimpleAggregateFunction` version `0` is never printed, and an omitted version there does not mean the function's default: a decoder must derive it from the revision, exactly as `NativeReader` does.

One case does not satisfy that rule today, and a decoder cannot repair it from the stream: a leaf declared with an explicit `AggregateFunction(0, ...)` written at revision `0`, whether it is the column type itself or sits under any nesting of the supported containers, inside a `SimpleAggregateFunction` or outside one. Revision `0` is the one revision that keeps a declared version instead of negotiating one, so the payload is written at version `0`, and version `0` is never printed, so the type string omits the token. A decoder that derives the version by re-parsing that name reads the function's default instead, which is `1` for the map and bitmap families. At every non-zero revision the declared `0` is replaced by the negotiated version and announced explicitly, so the name and the payload agree there and this does not arise. Whether it is observable at revision `0` depends on the value type: version `1` of the map family serializes promoted values, so a type that promotion widens, such as `Decimal32`, makes the two layouts differ and decoding either fails or reconstructs wrong values, while a type it leaves alone, such as `UInt64`, keeps them byte-compatible and decodes successfully despite the version being unavailable. A decoder must therefore treat an omitted token outside a `SimpleAggregateFunction` as the version its own revision derives, which is `0` at revision `0`, exactly as `NativeReader` does.
