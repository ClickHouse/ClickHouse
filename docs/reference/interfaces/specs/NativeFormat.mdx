...
This section documents the wire encoding of the types the Native format can carry within a column's `data`, grouped into four families of increasing decoder complexity. Two types — `AggregateFunction(func, ...)` and `QBit(T, N[, stride])` — are valid `Native` column types with specialized payloads rather than alias encodings. Their decoding requirements are called out below; selected [geo aggregate states](#geo-aggregate-states) are also specified here.
...
<Note>
Two related types are **not** aliases. They are valid `Native` column types — a client can receive an `AggregateFunction` column from a `-State` combinator or distributed aggregation, for instance — but each requires a decoder for its specialized payload. Selected geo aggregate states are described below; other function-specific payloads are outside the scope of this page:
...
### Geo aggregate states {#geo-aggregate-states}

The states of `groupConvexHull`, `groupPolygonUnion`, and `groupPolygonIntersection` begin with their own `UInt8` format version **inside each state payload**. This byte is distinct from the aggregate version in a type string and is not negotiated using the client protocol revision. Coordinates in these payloads are pairs of little-endian `Float64` values, even when intermediate geometry calculations use a wider type.

#### Convex hull state {#convex-hull-state}

`groupConvexHull` writes version `3` and also reads legacy version `2`. Both versions have the same fields:

```text
UInt8 version
VarUInt point_count
VarUInt compression_watermark
point_count × (Float64 x, Float64 y)
```

`point_count` cannot exceed `100000000`; the watermark cannot exceed the point count. Version `2` additionally requires `point_count - compression_watermark <= 10000`. Version `3` permits adaptive compression and requires `point_count - compression_watermark <= max(10000, compression_watermark)`. These metadata constraints are checked before reading coordinates. All coordinates must be finite. The watermark describes the point count after the last compression; it does not prove that the serialized prefix is already a convex hull, so readers must not return that prefix as a trusted final result without computing or validating the hull.

The current writer compacts the transmitted points to their convex hull, omits the repeated closing endpoint, and writes `compression_watermark = point_count`. It does not mutate the source aggregate state. Readers continue accepting earlier version-`3` payloads with an uncompressed suffix under the adaptive bound above. A local flag identifying a previously computed hull is not serialized; incoming coordinates must not be trusted solely because their watermark equals their count.

Version-`2` readers cannot read version-`3` states. New readers accept valid version-`2` states and write them back using version `3`; applications exchanging these states must account for this direction of compatibility.

#### Polygonal states {#polygonal-aggregate-states}

Both polygonal aggregates retain format version `1`. A serialized multipolygon is a `VarUInt polygon_count` followed by that many polygons. Each polygon stores its outer ring, a `VarUInt inner_ring_count`, and its inner rings. Each ring stores a `VarUInt point_count` followed by that many `(Float64 x, Float64 y)` pairs.

- `groupPolygonUnion`: `UInt8 version`, `VarUInt chunk_count`, then that many multipolygons. Readers accept up to `16` chunks, including arbitrary chunk arrangements from the previous writer. The current writer groups chunks by their point-count size tier and stays within that bound; the tier and cached point count are reconstructed and are not serialized.
- `groupPolygonIntersection`: `UInt8 version`, `UInt8 mode`. Modes `0` (uninitialized) and `2` (empty) have no further payload. Mode `1` (nonempty) is followed by a `VarUInt chunk_count` and the multipolygons. Readers accept `1` through `8` chunks for compatibility; the current writer stores the already-computed running intersection in one chunk.

Polygonal readers enforce a cumulative limit of `10000000` points and validate finite coordinates and polygon topology. Normalization can add closing points to open rings, so the point limit is enforced again after normalization. The serialized state budget is distinct from the temporary memory used to compute an overlay.
