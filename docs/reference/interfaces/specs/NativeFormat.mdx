...
That makes revision `0` the most conservative encoding for almost everything: the stream carries no `BlockInfo`, no `has_custom_serialization` byte, V1 `Dynamic`/`JSON`, no aggregate-function version, and a bare `DateTime` with the timezone parameter dropped when type names are text-encoded. Binary-encoded types retain the `DateTime` timezone parameter even at revision `0`. Aggregate-function *state* versions are the exception at revision `0`: there is no peer to derive a version for — the stream is written and read back by whoever produced it (a `StripeLog` data file, a `Set`/`Join` backup, a `Native` format file) — so the writer keeps a version pinned on the type (spelling it out in the type string as usual) instead of downgrading the state to version `0`, and only a type that pins no version is written at version `0`.
...
The type appears as `DateTime64(s)` (implicit server-default timezone) or `DateTime64(s, 'TimezoneName')` (explicit timezone, display only). Negative values represent ticks before the epoch. Decoding these ticks requires no timezone lookup table; the scale and timezone parameter remain part of the datatype metadata.

ClickHouse's `NativeReader` uses timezone-free serializers for top-level `DateTime` and `DateTime64` columns without custom serialization. Nested datetime types, including `Array`, `Nullable`, `Tuple`, and `LowCardinality` wrappers, still use their ordinary serializers and may initialize timezone lookup tables. This implementation limitation does not change the wire representation.
