...
A writer caps its own frame so that both fields stay within the limit, so a current writer never emits a frame a current reader rejects: a writer asked for a larger buffer emits several frames within the limit instead of one above it. Because the cap accounts for how much the codec in use may expand the data, the frame a writer emits is smaller than the limit by that codec's worst-case overhead. Data written before the cap existed can carry larger frames, and it stays readable where it is read back from local storage; a frame arriving over a socket is always subject to the limit.
