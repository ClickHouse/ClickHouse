...
`ExponentialTimeDecaying(decay_length)` is a custom logical type for one exponentially time-decaying curve. The positive `decay_length` parameter is part of the type string and is **not** repeated in each row. The legacy experimental spelling `ExponentialTimeDecayingFloat64(decay_length)` uses the same payload.
...
The server may derive an additional sortable ordering prefix in memory for `ORDER BY`, primary-key comparison, and related operations. That prefix is acceleration metadata only: it is reconstructed from the two direct streams and is **not present in the Native payload or MergeTree column data**.

For a direct `ExponentialTimeDecaying` primary-key column, the sparse MergeTree `primary.idx` stores only the 8-byte `UInt64` ordering prefix for each mark. Prefix collisions are treated as equal index buckets, which may read extra granules but must not exclude matching rows. The authoritative table column remains the two-`Float64` direct payload above.
