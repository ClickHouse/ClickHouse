...
| SINGLE_VALUE_OR_NULL_STATE_VERSION | 54493 | Block (Column) | Bumps the state version of `AggregateFunction(singleValueOrNull, ...)` from `0` to `1`, which appends trailing `Bool first_value` and `Bool is_null` fields to each serialized state. Only the state payload of this aggregate function changes; below this revision the writer emits version `0` states, so an older peer is unaffected. |
