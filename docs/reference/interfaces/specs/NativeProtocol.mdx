...
An `INSERT` supplied through the [`input`](/sql-reference/table-functions/input) table function also receives an input-schema block. Its initializer can additionally send [`TimezoneUpdate`](#timezoneupdate) immediately after that block at revision `54464`+. An `INSERT ... SELECT` without client-supplied input uses the ordinary [query phase](#query-phase), not this row-upload exchange.
