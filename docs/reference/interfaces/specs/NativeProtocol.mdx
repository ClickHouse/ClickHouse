...
| TIMEZONE_UPDATES                | 54464   | TimezoneUpdate         | Adds the `TimezoneUpdate` server packet (type 17). Body: single `String` carrying the session timezone. Sent only by the `input` table function initializer, before column-default metadata and the input-schema block, so the client parses the rows it sends with the server's `session_timezone`. See [TimezoneUpdate](#timezoneupdate) for older-server ordering. |
...
| PROGRESS_IN_ASYNC_INSERT        | 54484   | Progress (INSERT)      | For rows accepted by the **asynchronous** `INSERT` queue (`async_insert = 1`), the server sends an extra [`Progress`](#progress) packet after queueing, waiting for the flush first only if `wait_for_async_insert = 1`. This precedes the insert's `ProfileEvents` and `EndOfStream`. `ProfileTraces` may be interleaved when trace delivery is enabled at revision `54493`+. Gated on the *negotiated* version ≥ 54484; below it the server omits this trailing `Progress`. The `Progress` wire format is unchanged — only the emission is new. In practice the increment carries the elapsed time. Accompanying `ProfileEvents` report the submitting query's counters, such as `AsyncInsertQuery` and `AsyncInsertBytes`, not the separate flush's write totals. With `wait_for_async_insert = 0`, the response does not confirm that the queued rows have been written. A client that already drains interleaved `Progress` needs no format change, only to tolerate one more packet. |
...
| PROFILE_TRACES | 54493 | ProfileTraces | The server may emit sampled query stack traces when `send_profile_traces = 1`. The block body follows the query's compression flag. |
...
sequenceDiagram
    ...
    loop until EndOfStream or Exception
        ...
        S->>C: ProfileInfo / ProfileEvents / ProfileTraces (profiling)
    end
    S->>C: Data — empty block (boundary marker)
    S->>C: Progress — final updates
    opt pending profile traces
        S->>C: ProfileTraces — final drain
    end
...
When enabled, `ProfileTraces` packets may also arrive before the header `Data` block and after the final `Progress` updates. Clients must continue draining until `EndOfStream` or `Exception`; a trace block is neither a schema block nor a completion marker. See [ProfileTraces](#profiletraces) for the revision gate, final-drain bounds, and client cancellation behavior.

1. The client sends [`Query`](#query) with a unique `query_id` (typically a UUID).
2. The client sends any external tables, then the empty Data marker. The empty Data packet has `table_name = ""`, `num_columns = 0`, `num_rows = 0`. The server does not begin executing the query or delivering `ProfileTraces` until it receives this marker. An external table may be split over several `Data` packets with the same `table_name`; the first of them binds the table's schema, and every later packet for that name must carry the same column names and types, in the same order, or the server rejects the query with `INCORRECT_DATA` (see [Data](#data)).
...
4. The client reads response packets in a loop, dispatching by type:
   ...
   | `ProfileTraces` (19)  | Sampled query stack traces, enabled by `send_profile_traces` at revision `54493`+. |
...
For an `INSERT` with client-supplied rows, the client submits the statement and terminates the external-table input phase; the server replies with a **schema block** describing the target table; the client streams `Data` packets with the rows, then a second empty `Data` marker; the server finishes with `EndOfStream` or `Exception`.

Starting from `READY`, the SQL is an `INSERT` of the form `INSERT INTO <table> [(<cols>)] VALUES` — with no inline `VALUES (...)` literal, since the row data flows through `Data` packets. The following flow covers a direct streamed `INSERT` without external tables, as sent by `clickhouse-client`:
...
sequenceDiagram
    ...
    C->>S: Empty Data marker — end of external-table input
    opt logs before schema
        S->>C: Log
    end
    alt error before schema
        opt pending profile traces
            S->>C: ProfileTraces — final drain
        end
        S->>C: Exception
    else ready to receive rows
        opt column-default metadata enabled and supported
            S->>C: TableColumns
        end
        S->>C: Data packet — schema block (columns, 0 rows)
        Note over C,S: Rows sent next must match the schema
        loop row blocks
            C->>S: Data packet (rows N)
            opt interleaved metadata
                S->>C: Log / ProfileEvents
            end
        end
        C->>S: Empty Data marker — end of insertion rows
        loop trailing metadata
            S->>C: Progress / Log / ProfileEvents / ProfileTraces
        end
        S->>C: EndOfStream (success) or Exception (failure)
    end
```

1. The client sends [`Query`](#query) with `body` set to the `INSERT` SQL.
2. The client sends an empty `Data` packet (`table_name = ""`, 0 columns, 0 rows) to terminate the external-table phase, just as in the [Query phase](#query-phase). This marker is separate from the one that terminates insertion rows in step 5. The server finishes reading external tables before preparing the insertion schema.
3. The client reads `Log` packets and an optional [`TableColumns`](#tablecolumns) packet until it receives the schema `Data` packet — a block with 0 rows but full column structure (names and types). `Progress`, `ProfileInfo`, and `ProfileEvents` are not emitted on this pre-schema success path. If setup fails, the server may drain `ProfileTraces` before an `Exception` without sending a schema. Only the schema `Data` packet permits row upload; a trace block is auxiliary metadata, not the insertion schema.
4. The client sends data block(s). For each block it writes `VarUInt(ClientPacket::Data = 2)`, then `String("")` for the empty external-table name, then the Block. Column types must align with the schema block's columns by position. The server may send interleaved `Log` and `ProfileEvents` packets while the client uploads rows. It keeps profile traces in the bounded queue until the input terminator, avoiding opposing bulk socket writes even when the client reads replies between blocks.
5. The client sends the end-of-input terminator: a Data packet with an empty Block (0 columns, 0 rows).
6. The client drains the response stream, including `Progress`, `Log`, `ProfileEvents`, and `ProfileTraces`, until `EndOfStream` (success) or `Exception` (failure). `ProfileInfo` is not part of this direct streamed `INSERT` response. The final trace drain can emit additional `ProfileTraces` packets before the terminal packet. Trace delivery requires revision `54493`+ and `send_profile_traces = 1`; see [ProfileTraces](#profiletraces) for its bounded-drain and cancellation behavior. An exception during upload can terminate the response before the client reaches step 5.

The external-table phase is distinct from insertion-row upload. `clickhouse-client` sends no external-table blocks for this direct `INSERT` form. A client that does send nonempty external-table blocks can receive `Log` and `ProfileEvents` while the server reads them, before the insertion schema is prepared. Trace delivery is deferred through this phase, including scalar uploads, until its empty `Data` marker.

An `INSERT` supplied through the [`input`](/sql-reference/table-functions/input) table function also receives an input-schema block. At revision `54464`+, its initializer sends [`TimezoneUpdate`](#timezoneupdate) before any `TableColumns` metadata and the schema `Data` packet, allowing the client to configure its input parser before row upload. An `INSERT ... SELECT` without client-supplied input uses the ordinary [query phase](#query-phase), not this row-upload exchange.

**Asynchronous `INSERT` (v54484+).** For rows accepted by the asynchronous insert queue (`async_insert = 1`), the server emits an extra [`Progress`](#progress) packet at negotiated version ≥ 54484 (`PROGRESS_IN_ASYNC_INSERT`) before the insert's `ProfileEvents` and `EndOfStream`. With `wait_for_async_insert = 1`, this happens after the flush completes; with `wait_for_async_insert = 0`, it happens after queueing without waiting for the flush. `ProfileTraces` packets may be interleaved with this metadata and emitted by the final drain. Below 54484 the server skips that trailing `Progress`. The packet is an ordinary `Progress`; because the server resets the query pipeline before folding in the write counts, the increment in practice carries only the elapsed time. Accompanying `ProfileEvents` report the submitting query's counters, including queueing counters such as `AsyncInsertQuery` and `AsyncInsertBytes`, not the separate flush's write totals. With `wait_for_async_insert = 0`, neither these packets nor `EndOfStream` confirm that the queued rows have been written. A client that already drains interleaved `Progress` in step 6 needs only to accept one more packet.
...
### ProfileTraces (packet type 19) {#profiletraces}

Server → Client, available at negotiated revision `54493` and above when `send_profile_traces = 1`. This setting is independent of `send_profile_events`. Existing profiler settings control which traces are sampled; enabling delivery alone does not enable a sampler.

The server's trace collector must be enabled by the `trace_log` server configuration. Streaming does not read or flush `system.trace_log` and does not require permission to select from that table.

The envelope is a packet-type `VarUInt` of `19`, an empty `table_name` binary string, and a [Native block](/reference/interfaces/specs/NativeFormat). Only the block body follows the query's compression flag. Blocks contain at most 1024 rows with this schema, in order:

| # | Name | Type | Description |
|---|------|------|-------------|
| 1 | `host_name` | `String` | Host that collected the sample, preserved when forwarded. |
| 2 | `query_id` | `String` | Query that produced the sample, including remote query IDs. |
| 3 | `trace_type` | `String` | Sample kind: `CPU`, `Real`, `Memory`, `MemorySample`, or `MemoryPeak`; delivery status: `Dropped` or `Incomplete`. |
| 4 | `thread_id` | `UInt64` | OS thread ID on the originating host. |
| 5 | `event_time_microseconds` | `UInt64` | Sample timestamp in microseconds since the Unix epoch. |
| 6 | `trace` | `Array(UInt64)` | Normalized instruction addresses, ordered from the leaf frame outward. |
| 7 | `symbols` | `Array(String)` | Symbol corresponding to each address; an empty string denotes an unresolved frame. |
| 8 | `size` | `Int64` | Allocation size for `Memory`; signed allocation/deallocation size for `MemorySample`; total tracked usage at a new peak for `MemoryPeak` (all in bytes); zero for `CPU`, `Real`, and `Incomplete`; positive loss delta for `Dropped`. |

Delivery status rows use empty `trace` and `symbols` arrays and zero `thread_id` and `event_time_microseconds`. `Dropped` reports a positive delta in the number of samples discarded by bounded queues or by a collector flush timeout. Add these deltas across the response; each delta saturates at `9223372036854775807`, which denotes a lower bound. `Incomplete` has `size = 0` and reports that a collector flush timed out, so additional losses cannot be quantified. Status rows count towards the 1024-row batch limit and can be sent without any samples.

A coordinator consumes remote loss deltas and includes them once in its own status rows, even when its sample queue is full. For status rows, `host_name` and `query_id` identify the reporting coordinator; they do not attribute losses to a particular sampled host. The count covers reported queue losses across the response's distributed query subtree. It excludes losses in the signal-safe pipe before the collector and any metadata discarded by intermediaries that do not recognize these status kinds. An absent `Incomplete` row does not certify a lossless profile. Clients must handle status rows separately from samples and ignore unrecognized kinds.

Packets may be interleaved with query results and trailing `INSERT` metadata. During Native uploads of external tables, scalars, `INSERT` rows, or `input` rows, samples stay in the bounded queue until the corresponding empty `Data` terminator has been received. Reading replies between upload blocks does not prevent opposing bulk writes from blocking, so no `ProfileTraces` packet is sent during those phases.

Once input is complete, the server drains accepted samples before `EndOfStream` or, when the connection is still usable, before an `Exception`. An exception before the insertion schema can therefore include a trace drain if external input has already ended. If an exception or query completion occurs with input still pending, including detached-query completion, the server instead discards queued samples without waiting for the collector or sending trace or loss-status packets. It sends the original terminal response before attempting to consume the remaining input.

The final drain waits at most ten seconds for the collector. If that deadline expires, remaining samples are discarded and a server warning is logged; the original query result or exception is preserved. When a client `Cancel` cancels the query, the server discards queued samples without waiting for the collector and sends `EndOfStream`. Clients must continue receiving until the terminal packet; a `ProfileTraces` block is never a result block or a completion marker. Delivery uses bounded queues, so samples may be dropped when a consumer cannot keep up, including while upload delivery is deferred. Disabling delivery discards queued samples without waiting for the collector. Excluded trace types, including `ProfileEvent` and `JemallocSample`, can still be collected in `system.trace_log` under their own settings.
...
Wire settings enable collection before query analysis. A `send_profile_traces` value supplied only in SQL `SETTINGS` takes effect immediately after those settings are applied, before subsequent analysis and synchronous interpreter work such as `BACKUP` or `RESTORE`. Earlier samples cannot be recovered. Distributed SELECT queries forward remote trace blocks with their original host and query identifiers. Remote INSERT uploads disable delivery to avoid blocking while both peers are sending data. When connecting to a server below revision `54493`, the native client omits the `send_profile_traces` wire setting, and an older client never receives this packet from a newer server.

Secondary queries request profile trace blocks only when delivery is active for the coordinator's client. Remote SQL opt-ins cannot enable delivery when it is disabled at the coordinator; explicit remote SQL opt-outs remain effective.
...
Server → Client, gated by `TIMEZONE_UPDATES` (v54464). Sent in exactly one place: the initializer for the `input` table function (a query of the form `INSERT INTO <table> SELECT ... FROM input('<structure>')`, which streams rows from the client). Before column-default metadata and the input-schema `Data` block (see the [INSERT phase](#insert-phase)), it emits `TimezoneUpdate` carrying the query context's current `session_timezone` so the client parses the rows it is about to send with the same timezone. The server does **not** emit this packet for arbitrary mid-query `SET session_timezone` changes, nor to tell the client how to format later result blocks.

| # | Field    | Type   | Role      | Description |
|---|----------|--------|-----------|-------------|
| 1 | timezone | String | universal | The new session-default timezone (e.g., `"UTC"`, `"Europe/Berlin"`). |

The packet arrives once, before the input-schema block and before the client starts sending row blocks. Older servers can send it after the schema; receivers must also accept that order, although an update received after input parsing starts cannot affect rows already parsed. A decoder that ignores `TimezoneUpdate` MUST still consume the trailing `String` to keep the wire aligned.
...
| 19   | ProfileTraces                  | [ProfileTraces](#profiletraces) | Sampled query stack traces |
...
**End-of-client-data marker** — an empty `Data` packet (0 columns, 0 rows) the client sends to close an input stream:
...
- **`INSERT` with client-supplied rows:** one marker terminates the external-table phase before the schema block; a second terminates the insertion-row stream after the schema and row `Data` blocks. See the [INSERT phase](#insert-phase).
...
**Packet type code** — the leading VarUInt of a packet that identifies its format. Values 0–19 are currently assigned. See the [packet type reference](#packet-type-reference).
