...
**Cancel** — a client-initiated packet (type 3) with no body. Normally it aborts the running query. With `partial_result_on_first_cancel = 1`, the first `Cancel` stops reading and lets processors, including set-building pipelines, finish processing data already read. This read-stop remains in effect for subsequently started pipelines belonging to the same query. Successful partial completion ends with `EndOfStream`; partial results are not written to shared prepared-set or query-result caches. An unfinished scalar subquery instead turns the first `Cancel` into full query cancellation because a partial prefix cannot prove that the subquery returned at most one row. A non-empty `EXISTS` subquery remains eligible for partial completion once its first row has determined the result.
