...
| QUERY_PLAN_SERIALIZATION        | 54477   | ServerHello, QueryPlan packet | ServerHello appends `VarUInt query_plan_serialization_version` after server settings. Also introduces `ClientPacket::QueryPlan` (code `13`) for inter-server delivery of pre-built query plans — external clients never send. Current value: `10` (`DBMS_QUERY_PLAN_SERIALIZATION_VERSION`). Version `10` appends `VarUInt min_frame_rows_for_aggregate_tree` (the `min_window_frame_rows_for_aggregate_tree` setting of the initiator) to the serialized `Window` step; below `10` the field is absent on the wire, and a reader at `10` or above treats such a step as having the aggregate tree disabled. |
