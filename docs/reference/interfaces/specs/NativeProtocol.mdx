...
| PROGRESS_IN_ASYNC_INSERT        | 54484   | Progress (INSERT)      | On an **asynchronous** INSERT (`async_insert = 1`), once the insert is flushed the server sends an extra [`Progress`](#progress) packet, then the insert's `ProfileEvents`, before `EndOfStream`. `ProfileTraces` may be interleaved when trace delivery is enabled at revision `54493`+. Gated on the *negotiated* version ≥ 54484; below it the server omits this trailing Progress. The Progress wire format is unchanged — only the emission is new. In practice the increment carries the elapsed time; the written-row counters are reported via the accompanying ProfileEvents. A client that already drains interleaved Progress needs no format change, only to tolerate one more packet. |
...
sequenceDiagram
    ...
    opt metadata before schema
        S->>C: TableColumns / Progress / ProfileInfo / Log / ProfileEvents / ProfileTraces
    end
    ...
    opt async_insert = 1 and negotiated_version ≥ 54484
        S->>C: trailing Progress
    end
    loop until EndOfStream or Exception
        S->>C: Progress / ProfileInfo / Log / ProfileEvents / ProfileTraces
    end
    S->>C: EndOfStream
```
