...
**Cancel** — a client-initiated packet (type 3) with no body. Normally it aborts the running query. With `partial_result_on_first_cancel = 1`, the first `Cancel` stops reading and lets processors finish processing data already read. This read-stop remains in effect for subsequently started pipelines belonging to the same query. Successful partial completion ends with `EndOfStream`; partial results are not written to shared prepared-set or query-result caches. An unfinished scalar subquery instead turns the first `Cancel` into full query cancellation because a partial prefix cannot prove that the subquery returned at most one row. An incomplete `IN` or `NOT IN` set build also requires full cancellation because membership requires the complete right-hand side. `EXPLAIN ANALYZE` fully cancels because its runtime metrics are only meaningful after the inner query completes. A non-empty `EXISTS` subquery remains eligible for partial completion once its first row has determined the result.

A second `Cancel` aborts the query. Disconnecting or cancelling through `KILL QUERY` also aborts it; neither requests successful partial completion. Clients must drain the response stream before reusing the connection. The partial-result behavior does not apply while the server receives `INSERT` or `input` data.

Pipelines with side effects, including `INSERT ... SELECT`, table or materialized-view population, and `PARALLEL WITH`, treat the first `Cancel` as full query cancellation even when `partial_result_on_first_cancel = 1`. A partial-result request received before the write's pipeline starts, for example while building a set, prevents that pipeline from starting. Cancellation follows the normal exception and cleanup paths; it does not guarantee rollback of writes already committed outside a transaction.
