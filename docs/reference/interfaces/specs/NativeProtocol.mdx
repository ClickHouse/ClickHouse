...
| COLUMN_LESS_BLOCK_ROW_COUNT | 54493 | Block | A block with no columns may declare `num_rows > 0`: its rows carry no values at all, and the row count is the whole result — what a shard returns for `SELECT count() OVER () FROM distributed_table`, where the window function is evaluated on the initiator and takes nothing from the shard. Only the block framing changes (`[VarUInt num_columns = 0][VarUInt num_rows = M]`, no column entries follow); below this revision the writer sends `num_rows = 0` instead, because an older reader treats a column-less block that declares rows as corrupt data (`INCORRECT_DATA`). See [block variants](/reference/interfaces/specs/NativeFormat#block-variants). |
