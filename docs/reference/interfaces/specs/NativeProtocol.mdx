...
| QUERY_PLAN_SERIALIZATION        | 54477   | ServerHello, QueryPlan packet | ServerHello appends `VarUInt query_plan_serialization_version` after server settings. Also introduces `ClientPacket::QueryPlan` (code `13`) for inter-server delivery of pre-built query plans — external clients never send. Query-plan serialization version `10` prefixes the serialized plan with `VarUInt max_threads` and `Bool concurrency_control`; when either is non-default, initiators use SQL shipping for peers below version `10` so the peer builds its plan from the transmitted query settings. Query-plan serialization version `12` adds the read-in-order description to the `ReadFromMergeTree` step payload. Query-plan serialization version `13` adds the `only_merge` flag (bit `128`) to the `AggregatingStep` payload. Query-plan serialization version `14` registers the `IntersectOrExcept` step. Query-plan serialization version `15` adds a per-step `UInt8 security_barrier` flag (a step of a `SQL SECURITY DEFINER` / `SQL SECURITY NONE` view that may hide rows); an initiator never sends a plan containing a barrier step to a peer below version `15` and fails the query instead. |
