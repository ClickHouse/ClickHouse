...
| QUERY_RESULT_PREVIEWS           | 54493   | PreviewData            | Adds the [`PreviewData`](#previewdata) server packet (type 19), carrying real-time previews of the query result. Sent only when the experimental `query_result_previews` setting is enabled; the server force-disables the setting for clients below this revision, so an older client never sees the packet. |
...
### PreviewData (packet type 19) {#previewdata}

Server → Client, gated by `QUERY_RESULT_PREVIEWS` (v54493). Sent only when the experimental `query_result_previews` setting is enabled: while a long-running aggregation or sorting is accumulating, the server periodically emits a small snapshot of the intermediate result, processed by the rest of the query pipeline like ordinary data. Wire format is identical to [Data](#data); the block has the same structure as the result blocks. Each preview fully replaces the previous one, and previews stop before the first row-bearing `Data` packet of the real result. Previews are best effort: a client is free to ignore the packet (after consuming its body), and must never count preview rows as part of the result.

```text
[VarUInt: 19]               packet type
[String:  table_name]       always empty
[Block]                     see the Native Format spec
```
...
| 19   | PreviewData                    | [PreviewData](#previewdata) | Real-time preview of the query result |
