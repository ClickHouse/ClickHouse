    ...
    alt error before schema
        ...
        loop row blocks
            ...
            opt interleaved metadata
                S->>C: Log / ProfileEvents / ProfileTraces
            end
        end
...
4. The client sends data block(s). For each block it writes `VarUInt(ClientPacket::Data = 2)`, then `String("")` for the empty external-table name, then the Block. Column types must align with the schema block's columns by position. The server may send interleaved `Log`, `ProfileEvents`, and `ProfileTraces` packets while the client uploads rows.
...
The external-table phase is distinct from insertion-row upload. `clickhouse-client` sends no external-table blocks for this direct `INSERT` form. A client that does send nonempty external-table blocks can receive `Log`, `ProfileEvents`, and `ProfileTraces` while the server reads them, before the insertion schema is prepared.
...
Packets may be interleaved with query results and INSERT metadata. On successful completion, the server drains accepted samples before `EndOfStream`. When a query fails and the connection is still usable, the server also drains accepted samples before sending the exception. The final drain waits at most ten seconds for the collector. If that deadline expires, remaining samples are discarded and a server warning is logged; the original query result or exception is preserved. When a client `Cancel` cancels the query, the server discards queued samples without waiting for the collector and sends `EndOfStream`. Clients must continue receiving until the terminal packet; a `ProfileTraces` block is never a result block or a completion marker. Delivery uses bounded queues, so samples may be dropped when a consumer cannot keep up. Disabling delivery discards queued samples without waiting for the collector. Excluded trace types, including `ProfileEvent` and `JemallocSample`, can still be collected in `system.trace_log` under their own settings.
