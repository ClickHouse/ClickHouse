...
| 8 | part_checksum_high64 | VarUInt                                          | `DBMS_PARALLEL_REPLICAS_MIN_VERSION_WITH_PART_FINGERPRINT` (PR v10)  | High half of the same fingerprint. `(0, 0)` means the sender did not populate the fingerprint, and the receiver falls back to comparing field 6. |
...
A sender below PR v9 omits fields 6-9 entirely, and a sender below PR v10 omits fields 7-9; the receiver then sees the defaults (`0`, `(0, 0)`, `0`) and cannot verify by fingerprint that same-named parts hold the same data. A PR v10 sender also sends `(0, 0)` for a part whose checksums are not loaded, so an absent fingerprint means the same thing in both cases.
