...
| INTERSERVER_SECRET_TABLES_STATUS | 54487  | TablesStatusRequest    | An inter-server `TablesStatusRequest` (client packet `5`) is preceded by a 32-byte cluster-secret hash that also covers the requested table set. See [Inter-server authentication](#inter-server-authentication). External clients never enter inter-server mode and see no wire change. |
...
**`TablesStatusRequest` (v54487+).** The replica health-check an inter-server client sends while establishing a connection (client packet `5`) travels before any Query has authenticated the connection, so it carries its own hash. When `INTERSERVER_SECRET_TABLES_STATUS` (v54487) is negotiated and a cluster secret is configured, the client writes a 32-byte `encodeSHA256(salt + nonce + cluster_secret + "TablesStatusRequest" + tables_digest)` immediately after the packet type, before the request body. `salt` and `nonce` are the same as in the per-query hash (`nonce` in its decimal string form, present only when `INTERSERVER_SECRET_V2` (v54462) is negotiated). `tables_digest` binds the hash to the requested table set, so a captured hash cannot be replayed for a different one: it is built from the request's `(database, table)` pairs with every field length-prefixed as `<decimal length>:<bytes>` — each pair becomes the length-prefixed database name followed by the length-prefixed table name, the entries are sorted, and the digest is the length-prefixed decimal entry count followed by each entry, itself length-prefixed. The server reads the hash (32-byte cap) and then the body, recomputes the digest, and rejects the request on mismatch — or when it has no salt or no cluster secret for the named cluster. The tables are resolved only after the hash validates.

A peer below v54487 sends no hash. Its request is answered with a placeholder response that reports every requested table as present, not replicated and writable, independently of the actual state of the tables — so no table status is disclosed, while a distributed query initiated on a not-yet-upgraded node keeps working during a rolling upgrade (without taking replica staleness or readonly state into account). The `interserver_tables_status_require_auth` server setting rejects such requests instead.
