...
| AUTOMATIC_LOW_CARDINALITY_SERIALIZATION | 54493 | Block (Column)         | Server may emit a `String` or `FixedString` column whose `kind_stack` is the two-entry stack `{DEFAULT, LOW_CARDINALITY}` (nested kind byte `0x04`), i.e. a dictionary-encoded representation of a column whose declared type stays `String`/`FixedString`. Below this revision the writer materializes such columns to the default serialization before sending, so an older peer never sees the unknown kind. See [kind_stack and sparse encoding](/reference/interfaces/specs/NativeFormat#kind-stack-and-sparse-encoding). |
