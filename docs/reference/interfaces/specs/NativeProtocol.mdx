...
| VERSIONED_PARALLEL_REPLICAS_PROTOCOL | 54471 | ServerHello, Addendum | Both sides exchange a `VarUInt` parallel-replicas coordination protocol version. ServerHello's field is positioned **immediately after `protocol_version`** (before `timezone`). Addendum's field is appended after the chunked-protocol strings. Current value: `10` (`DBMS_PARALLEL_REPLICAS_PROTOCOL_VERSION`). Version `8` adds [`MergeTreeAllRangesAnnouncementResponse`](#mergetreeallrangesannouncementresponse) (client packet `14`): when the negotiated parallel-replicas version is `≥ 8` the initiator replies to every non-`Default`-mode follower announcement with the authoritative parts list for that stream, and the follower waits for it before issuing read requests. Below `8` the announcement is fire-and-forget. Versions `9` and `10` append fields to [`RangesInDataPartDescription`](#rangesindatapartdescription): `9` a part's total mark count, `10` a part content fingerprint plus a part-name identity classification. |
...
| 4a | parallel_replicas_protocol_version | VarUInt | universal | VERSIONED_PARALLEL_REPLICAS_PROTOCOL (v54471) | Server's parallel-replicas coordination protocol version. **Wire position: immediately after `protocol_version`**, before `timezone`. Current: `10`. |
...
| 6 | total_marks_in_part | VarUInt                                           | `DBMS_PARALLEL_REPLICAS_MIN_VERSION_WITH_TOTAL_MARKS_IN_PART` (PR v9) | Mark count of the whole part on disk, not of the analyzed `ranges` in field 2. `0` means the sender did not populate it. |
| 7 | part_checksum_low64 | VarUInt                                           | `DBMS_PARALLEL_REPLICAS_MIN_VERSION_WITH_PART_FINGERPRINT` (PR v10)  | Low half of the part's content fingerprint (the two halves of the part's total checksum over its file contents). Replicas holding the same on-disk part produce the same pair; replicas holding different parts that share a name do not. |
| 8 | part_checksum_high64 | VarUInt                                          | `DBMS_PARALLEL_REPLICAS_MIN_VERSION_WITH_PART_FINGERPRINT` (PR v10)  | High half of the same fingerprint. `(0, 0)` means the sender did not populate the fingerprint, and the receiver falls back to comparing field 6 only when neither side reports node-local part names in field 9. |
| 9 | part_name_identity | VarUInt                                            | `DBMS_PARALLEL_REPLICAS_MIN_VERSION_WITH_PART_FINGERPRINT` (PR v10)  | Whether a part name implies identical content on every cluster member: `0` = unknown, `1` = node-local part names (same-named parts may diverge), `2` = cluster-wide part names. The decoder rejects any other value with `UNKNOWN_PROTOCOL`. |

A sender below PR v9 omits fields 6-9 entirely, and a sender below PR v10 omits fields 7-9; the receiver then sees the defaults (`0`, `(0, 0)`, `0`) and cannot verify by fingerprint that same-named parts hold the same data. A PR v10 sender also sends `(0, 0)` for a part whose checksums are not loaded, so an absent fingerprint means the same thing in both cases. Whenever the fingerprint is missing on either side and either side reports node-local part names, the receiver rejects the query with `BAD_ARGUMENTS` instead of falling back to field 6, because a part name then implies nothing about content. A fingerprint is compared only when both sides carry one, so a v10 sender's fingerprint does not rescue a pre-v10 peer's announcement of the same part.
