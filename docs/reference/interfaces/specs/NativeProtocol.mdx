...
| GROUP_CONCAT_STATE_VERSION        | 54493   | Block (Column)         | Bumps the state version of unlimited `AggregateFunction(groupConcat(...), ...)` from `0` to `1`. Version `1` appends a `VarUInt` marker after the concatenated payload to record whether the state has rows, preserving the distinction between an empty state and a state containing empty strings. The limited form keeps its existing unversioned layout. Below this revision the writer emits version `0` states. See [`groupConcat` state versions](/reference/interfaces/specs/NativeFormat#groupconcat-state-versions). |
