...
| QUERY_PLAN_SERIALIZATION        | 54477   | ServerHello, QueryPlan packet | ServerHello appends `VarUInt query_plan_serialization_version` after server settings. Also introduces `ClientPacket::QueryPlan` (code `13`) for inter-server delivery of pre-built query plans — external clients never send. Query-plan serialization version `11` prefixes the serialized plan with `VarUInt max_threads` and `Bool concurrency_control`; when either is non-default, initiators use SQL shipping for peers below version `11` so the peer builds its plan from the transmitted query settings. Query-plan serialization version `12` adds the read-in-order description to the `ReadFromMergeTree` step payload. Query-plan serialization version `13` adds the `only_merge` flag (bit `128`) to the `AggregatingStep` payload. Query-plan serialization version `14` registers the `IntersectOrExcept` step. Query-plan serialization version `15` registers the `LimitRange` step. Query-plan serialization version `16` carries the estimate-derived decisions of `JoinStepLogical`. Query-plan serialization version `17` adds the `always_read_till_end` flag to the `LimitByStep` payload. Query-plan serialization version `18` registers the `Filling` step and adds the `WITH FILL` bounds (`FROM`, `TO`, `STEP`, `STALENESS` and the column alias) to a serialized sort description. Query-plan serialization version `19` writes a per-step `VarUInt` serialization version next to every step. Query-plan serialization version `20` adds `legacy_join_size_limits_trigger_spilling` to the join step settings. Query-plan serialization version `21` adds a per-step `UInt8 security_barrier` flag (a step of a `SQL SECURITY DEFINER` / `SQL SECURITY NONE` view that may hide rows); an initiator never sends a plan containing a barrier step to a peer below version `21` and fails the query instead. |
...
| 2 | max_threads | VarUInt | serialization version ≥ `11` | Plan-level thread limit. `0` means no plan-specific limit. |
| 3 | concurrency_control | Bool | serialization version ≥ `11` | Whether the plan participates in concurrency control. |
...
Before version `11`, fields 2 and 3 are absent. An initiator with either non-default execution limit must not send that older layout: it ships the original SQL query instead, so the older receiver reconstructs the limits from the query settings.

Within the `plan` stream, every step is written after its children as `String serialization_name`, then, from serialization version `19`, `VarUInt step_serialization_version`, then `String step_description`, then, from serialization version `21`, `UInt8 security_barrier`, followed by the step's output header, its changed serialization settings, and its step-specific payload. `security_barrier` is `1` for a step that belongs to a `SQL SECURITY DEFINER` or `SQL SECURITY NONE` view and may hide rows (a filter, limit, aggregation, join, and similar), and `0` otherwise. The receiver must keep such steps as optimization barriers: it must not merge, reorder, or push predicates, limits, or projections across them when it re-optimizes the deserialized fragment, because doing so would let the invoker's expressions observe rows the view definer meant to hide.

Before version `21` the flag is absent and the receiver treats every step as `0`. An older receiver cannot be told about a barrier, so the initiator fails closed: a plan that contains at least one barrier step is never serialized at a version below `21`; the query fails with `NOT_IMPLEMENTED` instead of shipping a plan that would silently lose its protection. Plans without barrier steps are unaffected and may still be sent to older peers.
