...
| QUERY_PLAN_SERIALIZATION        | 54477   | ServerHello, QueryPlan packet | ServerHello appends `VarUInt query_plan_serialization_version` after server settings. Also introduces `ClientPacket::QueryPlan` (code `13`) for inter-server delivery of pre-built query plans — external clients never send. Query-plan serialization version `10` prefixes the serialized plan with `VarUInt max_threads` and `Bool concurrency_control`; when either is non-default, initiators use SQL shipping for peers below version `10` so the peer builds its plan from the transmitted query settings. Query-plan serialization version `20` additionally lets serialized join steps carry the `max_memory_usage` and `enable_join_in_memory_compression` settings inside their changed-settings list; below version `20` those two names are omitted entirely, because such a receiver does not know them and rejects unknown setting names (a receiver reading a stream that omits `max_memory_usage` restores it from the query settings, which travel with the query; a join step whose `max_memory_usage` was overridden by a subquery-local `SETTINGS` clause cannot be restored that way, so its plan requires version `20` and the serializer fails closed below it). |
...
| 13 | query_plan_serialization_version | VarUInt | universal | QUERY_PLAN_SERIALIZATION (v54477) | Server's supported query-plan serialization version. Current value: `20` (`DBMS_QUERY_PLAN_SERIALIZATION_VERSION`); see the [QUERY_PLAN_SERIALIZATION row](#feature-table) for what the individual versions add. The two ends serialize at the `min` of the values they advertise. External clients decode and ignore. |
