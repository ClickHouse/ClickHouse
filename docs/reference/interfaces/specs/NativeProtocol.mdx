...
| TYPED_QUERY_PARAMETERS          | 54493   | Query                  | Appends one Native `Block` after the text parameter list. Each column is one typed parameter and the block has exactly one row; an empty typed-parameter set is encoded as an empty block. See [Typed query parameter block](#typed-query-parameter-block). |
...
| 9 | typed_parameters | Native Block | client     | TYPED_QUERY_PARAMETERS (v54493)          | See [Typed query parameter block](#typed-query-parameter-block). |
...
For a negotiated revision below `54493`, this field is absent. A compatible client must serialize typed values into the existing text parameter list instead; sending a Native block to an older peer desynchronizes the connection.
