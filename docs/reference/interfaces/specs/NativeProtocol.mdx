...
| 3 | `trace_type` | `String` | Sample type, such as `CPU`, `Real`, or `MemorySample`. |
...
Packets may be interleaved with query results and INSERT metadata. The server drains accepted samples before `EndOfStream`, and before a query exception when the connection is still usable. Clients must continue receiving until the terminal packet; a `ProfileTraces` block is never a result block or a completion marker. Delivery uses bounded queues, so samples may be dropped when a consumer cannot keep up.
