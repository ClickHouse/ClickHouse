...
| UNIQ_STATE_VERSION_1 | 54492 | Block (Column) | Bumps the state version of `AggregateFunction(uniq, ...)` from `0` to `1`, which stores full 64-bit hash values instead of ones truncated to 32 bits (fixing the overflow of the estimate at cardinalities above ten billion). A version `1` state starts with a flags byte: bit `0` set means the values carry only 32 bits of hash information (a state downgraded by contact with version `0` data) and are written as 4 bytes each; otherwise the values are 8 bytes each. The rest of the layout matches version `0`. Only the state payload of that one aggregate function changes; the column framing is untouched, and every other aggregate function keeps its own version. Below this revision the writer emits version `0` states (with the stored hashes truncated to 32 bits), so an older peer is unaffected. |
