...
Version `21` adds version 1 of the `Window` step, which carries whether the window is computed by hashing the partition keys (`query_plan_window_functions_hash_partitioning`) and the settings for spilling its rows. A plan with such a window is not serialized for a receiver below version `21`.
