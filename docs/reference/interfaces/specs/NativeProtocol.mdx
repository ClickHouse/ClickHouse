...
| 13 | query_plan_serialization_version | VarUInt | universal | QUERY_PLAN_SERIALIZATION (v54477) | Server's supported query-plan serialization version. Current: `18` (`DBMS_QUERY_PLAN_SERIALIZATION_VERSION`). See [QueryPlan](#queryplan) for compatibility rules. External clients decode and ignore. |
...
Version `18` registers the `max_parallel_ordered_merge_materialization_threads` plan setting. Senders may include this setting only when serializing a plan at version `18` or above. For older versions, the setting must be omitted entirely, even when its value is `0`: older receivers reject unknown plan-setting names. Omitting it leaves the older receiver's serial materialization behavior unchanged. This gate applies to the settings inside the plan stream, not to the native TCP protocol revision.
