...
| 14 | cluster_function_protocol_version | VarUInt | universal | VERSIONED_CLUSTER_FUNCTION_PROTOCOL (v54479) | Server's `*Cluster` table-function protocol version. Current: `11`. The value gates additive fields and encodings in the inter-server cluster read-task payload (the otherwise-unspecified `ReadTaskResponse` body); version `7` is reserved for a private-repository feature (Iceberg compaction), `8` adds an optional `read_source_index`, and `11` sends Iceberg delete-file paths as absolute URIs instead of object keys. External clients do not participate in cluster reads — they decode and ignore this field. |
