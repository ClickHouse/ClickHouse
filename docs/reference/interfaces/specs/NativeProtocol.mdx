...
| EXCEPTION_QUERY_INFO | 54493 | Exception | Appends `query`, `code_name`, and `query_id` strings after the legacy fields, only after a successful server `Hello`. |
...
| 6 | query | String | universal | Original SQL text, present at negotiated revision ≥ 54493 after server `Hello` |
| 7 | code_name | String | universal | Symbolic error code name, such as `UNKNOWN_IDENTIFIER`; same revision condition |
| 8 | query_id | String | universal | Current query identifier; same revision condition |

The first five fields retain their existing encoding. The three additional strings are appended in the order shown; there is no additional version or length envelope. `message` remains the existing human-readable message, and `stack_trace` remains a separate field. The stack trace is empty when the server disables sending it.

An exception replacing the server's `Hello`, including authentication failure, **always uses the five-field legacy layout**, even when the client announced revision 54493 or later. The client cannot negotiate the extension until it has received the server revision. Clients must reset their remembered server revision before reconnecting.

`query` is the SQL from the current `Query` packet, without data sent in subsequent `Data` packets. `query_id` is the actual query identifier, including a server-generated identifier when the client supplied an empty one. Query identifiers are strings: UUIDs are generated by default, but caller-supplied identifiers need not be UUIDs. For errors outside query execution, query text and identifier are empty. For an incompletely received query, these fields contain only information already available to the server. A distributed query reports the SQL and identifier of the query on the responding server.

The additional strings use the same binary string encoding and reader size limits as the existing exception strings. Readers must validate declared lengths and avoid allocating an entire declared payload before it arrives.
