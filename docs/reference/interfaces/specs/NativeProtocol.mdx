...
**Cancel** — a client-initiated packet (type 3) with no body. Normally it aborts the running query. With `partial_result_on_first_cancel = 1`, the first `Cancel` stops reading and lets processors finish processing data already read, including in scalar subqueries and set-building pipelines. This read-stop remains in effect for subsequently started pipelines belonging to the same query. Successful partial completion ends with `EndOfStream`; partial results are not written to shared prepared-set or query-result caches.
