...
| SMALL_FRAME_COMPRESSION | 54493 | Compression frame | Explicitly guarantees that the receiver supports `NONE` frames mixed with `LZ4` or `ZSTD` frames in otherwise compressed `Native` streams. When both peers support this revision, each sending direction can bypass compression for frames below `network_compression_min_bytes` while retaining framing and checksums. Older peers continue receiving the configured codec. No packet fields are added. See [compression methods](/interfaces/specs/NativeFormat#method-byte-values). |
