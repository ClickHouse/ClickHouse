...
HIPAA- and PCI-regulated cells are now available in the AWS Seoul (`ap-northeast-2`) and AWS Sydney (`ap-southeast-2`) regions. Select HIPAA or PCI at service creation to run compliant workloads in these regions; see the [compliance overview](/products/cloud/reference/security/compliance-overview#hipaa-since-2024) and [supported regions](/products/cloud/reference/supported-regions) for details.
