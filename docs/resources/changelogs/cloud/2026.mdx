...
This week expands ClickHouse Cloud's global reach with new AWS regions in Taipei and Malaysia, and completes Private Service Connect with cross-region connectivity.
...
### Private Service Connect adds cross-region connectivity {#private-service-connect-adds-cross-region-connectivity}

Private Service Connect (PSC) on GCP now supports cross-region connectivity via [global access](https://docs.cloud.google.com/vpc/docs/about-accessing-vpc-hosted-services-endpoints#global-access). This lets you connect to private endpoints from GCP regions not natively supported in ClickHouse Cloud, such as `asia-northeast3` (Seoul).
