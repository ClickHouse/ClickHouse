...
This week's updates expand ClickHouse Cloud's compliance footprint into new APAC regions and introduce private previews for PromQL support and On-Demand Compute. HIPAA- and PCI-regulated cells are now available in AWS Seoul and Sydney, while eligible customers can evaluate Prometheus-compatible metrics workloads and give individual heavy queries elastic access to additional workers without resizing their services.
...
PromQL support lets you store Prometheus metrics in the `TimeSeries` table engine and query them through the Prometheus HTTP API on your existing ClickHouse service and endpoint. ClickHouse accepts Prometheus remote-write v1 and translates PromQL to SQL in the server, allowing existing collectors, Grafana dashboards, and Prometheus tooling to work without a proxy or query rewrite. You can also issue PromQL from a SQL session with the `prometheusQuery` and `prometheusQueryRange` table functions.

The preview is available to selected ClickHouse Cloud and BYOC customers on AWS, GCP, and Azure through the [private-preview waitlist](https://clickhouse.com/cloud/promql-support-waitlist). It is actively developed and not feature-complete; current limitations include downsampling, native and exponential histograms, exemplars, recording and alerting rules, OpenMetrics- and OpenTelemetry-only features, and remote-write v2. See the [Prometheus interface documentation](https://clickhouse.com/docs/concepts/features/interfaces/prometheus) and [announcement](https://clickhouse.com/blog/introducing-promql).
