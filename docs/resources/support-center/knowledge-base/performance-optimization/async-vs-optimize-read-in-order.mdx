...
SETTINGS
    ...
    allow_asynchronous_read_from_io_pool_for_merge_tree= 1,
    max_streams_for_merge_tree_reading= 60,
    -- Keep a single merge node so this example stays focused on how the data is read.
    -- By default (`max_streams_per_hierarchical_merge = 16`) the 20 sorted streams are
    -- merged by a tree of `MergingSortedTransform` nodes instead of one 20 → 1 node.
