...
Refreshes are coordinated through ClickHouse Keeper for a view in a [`Replicated`](/reference/engines/database-engines/replicated) database, and a `REFRESH ... APPEND` view there can opt out of coordination with `SETTINGS all_replicas = 1`. Every other refreshable materialized view holds its last refresh time in memory only, so a restart leaves it overdue on any ordinary schedule. `RANDOMIZE FOR` does not spread these refreshes out.

## The extra refreshes can also duplicate data {#the-extra-refreshes-can-also-duplicate-data}

An `APPEND` view appends another full result. An `APPEND INCREMENTAL` view loses the cursor it resumes from unless its target table commits one itself, so it reprocesses rows it had already appended.
...
Set [`stop_refreshable_materialized_views_on_startup`](/reference/settings/session-settings/other#stop_refreshable_materialized_views_on_startup) in the server's own settings profile, which is the profile named by `system_profile`, falling back to `default_profile` and then to `default`:
...
[`SYSTEM START VIEWS`](/reference/statements/system#start-view-start-views) starts all of the views at once, which produces the same burst, so start each view by name and let its refresh finish before starting the next one. [`system.view_refreshes`](/reference/system-tables/view_refreshes) lists them:
...
Resuming an overdue view starts its refresh right away, and [`SYSTEM WAIT VIEW`](/reference/statements/system#wait-view) blocks while that refresh runs.
