...
While the setting is in place, an unplanned restart leaves every refreshable materialized view stopped, and a newly created view does not begin refreshing until it is started either. `RESTORE` is the exception: it starts every included refreshable materialized view. An uncoordinated view may then be immediately overdue and replay its source. Make whatever automation restarts your servers and creates your views responsible for starting them.
