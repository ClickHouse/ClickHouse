...
- [Projections](/concepts/features/projections/projections) and [materialized views](/concepts/features/materialized-views/index), which precompute results at insert time so queries read less data.

## Controlling and limiting concurrency {#controlling-and-limiting-concurrency}

By default, ClickHouse doesn't limit the number of concurrent queries. You can configure limits at several levels:
