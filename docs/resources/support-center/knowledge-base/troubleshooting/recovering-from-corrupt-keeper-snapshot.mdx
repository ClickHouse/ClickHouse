...
Keeper then drops the orphaned subtrees and starts. The paths it removed are listed in the log at
`WARNING` level.
...
Keeper deliberately refuses to start if a local log entry above the snapshot references one of the
removed nodes, because replaying that entry against the repaired tree would silently produce a different
state. If that happens, recover the node from a healthy peer instead: stop it, clear its coordination
directory, and let it re-sync from the leader — or use strategy 1 or 2.
