...
description: "ClickHouse supports static PIVOT clauses, and aggregate function combinators remain useful for dynamic or map-shaped pivots. This article shows the combinator approach using the UK housing prices dataset."
date: 2024-12-11
...
ClickHouse supports a static `PIVOT` clause when the pivot values are known in advance. For dynamic pivots, or when a map result is more convenient, you can also use [aggregate function combinators](/reference/functions/aggregate-functions/combinators) and, in particular, the ones with the [`-Map` suffix](/reference/functions/aggregate-functions/combinators#-map).

In this article, we're going to learn the combinator approach. 
