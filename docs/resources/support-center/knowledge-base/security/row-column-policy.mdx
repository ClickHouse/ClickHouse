...
Row policies filter the table whose data is read on that server. To filter queries that go through a [Distributed](/reference/engines/table-engines/special/distributed) table, create the policy on the underlying local table on each server of the cluster: a policy on the `Distributed` table itself cannot be applied to the query shipped to the remote servers, and such reads are rejected instead of silently returning unfiltered rows. The policy on the local table is applied when the initiator ships the query as text, and, when it ships an already-built read plan ([`serialize_query_plan`](/reference/settings/session-settings/serialize#serialize_query_plan)), for engines that accept filter push-down, such as the `MergeTree` family. See [CREATE ROW POLICY](/reference/statements/create/row-policy#distributed-and-remote-backed-tables) for the engines it is missed on.
