...
2.  In ClickHouse `config.xml`, add the `ldap_servers` configuration to each ClickHouse node. The example uses a dedicated AD service account (`svc.clickhouse`) for every search, so regular users do not need permission to read the group OU or their own `memberOf`; the user's own password is verified by binding as the DN that the search found (search-and-bind, selected by `<bind_dn>{user_dn}</bind_dn>`).
```
<ldap_servers>
	<marsnet2_ad>
		...
		<tls_ca_cert_file>/etc/clickhouse-server/marsnet2_ca.pem</tls_ca_cert_file>
		<lookup_bind_dn>CN=svc.clickhouse,OU=ServiceAccounts,DC=marsnet2,DC=local</lookup_bind_dn>
...
| port |Active Directory Port, usually 389 for non-ssl or 636 for SSL|636|
| enable_tls |Use LDAPS (`yes`), plain LDAP (`no`) or StartTLS (`starttls`); LDAPS with `tls_require_cert` set to `demand` and the domain CA in `tls_ca_cert_file` is recommended|yes|
| lookup_bind_dn |DN of the service account that performs the user search and the group searches; it needs read access to the user and group OUs|CN=svc.clickhouse,OU=ServiceAccounts,DC=marsnet2,DC=local|
| lookup_password |Password of the service account; read from an environment variable with `from_env` and hidden from the preprocessed configuration with `hide_in_preprocessed`|NA|
| bind_dn |DN the user's password is verified against; `{user_dn}` means "the DN that `user_dn_detection` found" (search-and-bind). A template such as `{user_name}@marsnet2.local` performs a direct bind instead| `{user_dn}`|
...
<user_directories>
	...
	<ldap>
		...
		<role_mapping>
			...
			<groups>
				<group>CN=clickhouse_ad_admins,OU=Groups,OU=ClickHouse,DC=marsnet2,DC=local</group>
				<group>CN=clickhouse_ad_db1_users,OU=Groups,OU=ClickHouse,DC=marsnet2,DC=local</group>
				<group>CN=clickhouse_ad_db2_users,OU=Groups,OU=ClickHouse,DC=marsnet2,DC=local</group>
			</groups>
...
|base_dn|The entry to read; `{user_dn}` is the DN of the user who logged in, detected by `user_dn_detection`|`{user_dn}`|
|scope|Read only the user's own entry|base|
|search_filter|ldap search filter; the user's own entry always matches|`(objectClass=*)`|
|attribute|The AD attribute listing the DNs of the groups the user is a direct member of|memberOf|
|rdn_attribute|The part of each group DN that becomes the role name|CN|
|groups|Allow-list of the AD groups that may grant ClickHouse roles. The full DN pins the OU, so a group with the same CN in another OU does not match|`CN=clickhouse_ad_admins,OU=Groups,OU=ClickHouse,DC=marsnet2,DC=local`|
|prefix|Prefix for the names of the groups in AD, this prefix will be removed to find the roles in ClickHouse|clickhouse_|

To grant roles through nested AD groups (`memberOf` lists direct memberships only), search the groups with the `LDAP_MATCHING_RULE_IN_CHAIN` rule instead, keeping `rdn_attribute` and `groups` as above:
```
<role_mapping>
