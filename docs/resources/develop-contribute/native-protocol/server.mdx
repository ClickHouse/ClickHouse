...
The `Data`, `Totals`, `Extremes`, and `ProfileTraces` block bodies follow the query's compression flag. `Log` and `ProfileEvents` do so from revision `54481`; below that revision their block bodies are uncompressed. The packet-type code and `table_name` string remain outside the compressed block body. See the [compression frame specification](/reference/interfaces/specs/NativeFormat#compression-frame) and the current [native protocol specification](/reference/interfaces/specs/NativeProtocol) for the revision gates and interleaving rules.
