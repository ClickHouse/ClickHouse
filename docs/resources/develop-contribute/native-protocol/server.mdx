...
| 19    | [`ProfileTraces`](/reference/interfaces/specs/NativeProtocol#profiletraces) | Sampled query stacks and delivery status; revision `54493`+ with `send_profile_traces = 1` |

The `Data`, `Totals`, `Extremes`, and `ProfileTraces` block bodies follow the query's compression flag. `Log`, `ProfileEvents`, and `TableColumns` do so from revision `54481`; below that revision their bodies are uncompressed. `TableColumns` contains two strings, `external_table` and `columns_description`, and both are inside its compressed body. The packet-type code always remains outside the compressed payload; for packets carrying a `Native` block, the `table_name` string also remains outside. See the [compression frame specification](/reference/interfaces/specs/NativeFormat#compression-frame) and the current [native protocol specification](/reference/interfaces/specs/NativeProtocol) for the revision gates and interleaving rules.
...
| display_name  | String  | `<display_name-or-hostname>` | Server name for UI   |
...
<Tip>
Encoded as a `Native` block of columns. Below revision `54481` the block is uncompressed; from `54481` it follows the query's compression flag. See the canonical [`Log` packet specification](/reference/interfaces/specs/NativeProtocol#log).
</Tip>
...
<Tip>
Encoded as a `Native` block of columns. Below revision `54481` the block is uncompressed; from `54481` it follows the query's compression flag. See the canonical [`ProfileEvents` packet specification](/reference/interfaces/specs/NativeProtocol#profileevents).
