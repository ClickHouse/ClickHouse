...
pull request - the startup path and one pipeline - plus the focused checks in
`utils/windows-selftest`. Everything past that is still unexecuted: the client against a real
server, `clickhouse-local` over actual files, any of the test suites, and the binary on a real
Windows machine rather than on Wine, which stubs enough of the platform that some behaviour
cannot be checked there at all (see [CI](#ci)). Note for a development host that is
...
The `Build (amd_windows)` job builds `clickhouse`, `clickhouse-windows-ported` and
`clickhouse-windows-selftest`. It is
...
Answering a query does not reach all of it, and the cross-build configures with
`ENABLE_TESTS=0`, so behaviour that is on no path a `SELECT` takes has nowhere else to be
checked. `utils/windows-selftest` is that place: a small executable, linking only `_poco_net`
and `common`, that the same step runs right after `clickhouse.exe`. It covers the emulation of
`MSG_DONTWAIT`, which Winsock has no equivalent for, and the memory limit a job object imposes,
which is what a Windows container caps a process with. Add a check there for anything the port
implements that a query does not exercise.

One limit of running on Wine rather than on Windows is worth knowing before writing such a
check: Wine stubs whole interfaces while reporting success. `QueryInformationJobObject` for
`JobObjectExtendedLimitInformation` zeroes the caller's struct and returns true (see
`NtQueryInformationJobObject` in Wine's `dlls/ntdll/unix/sync.c`), so a limit set with
`SetInformationJobObject` cannot be read back and the clamp in `getMemoryAmountOrZero` cannot
be observed end to end there. The self-test therefore exercises the decision - `windowsJobObjectMemoryLimit`,
which is declared separately for this reason - against the values the operating system would
supply, and separately asserts that whatever the job does report, the amount stays within it.
