...
<Note>
`apt.llvm.org` provides Clang 23 packages for Ubuntu 24.04 (Noble) and newer and for Debian 12 (Bookworm) and newer.
There is no Clang 23 repository for Ubuntu 22.04 (Jammy); on Jammy, install Clang 21 instead with `sudo ./llvm.sh 21`.
</Note>
