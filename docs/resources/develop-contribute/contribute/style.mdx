...
**18.** The product name is written `ClickHouse` - one word, with a capital `C` and a capital `H`. Two case variants are accepted: `clickhouse`, for lowercase-only tokens such as package, binary and host names; `ClickHouse` is also accepted in CamelCase identifiers - and `CLICKHOUSE`, for macros and environment variables. Everything else is a misspelling, and the `clickhouse_spelling` style check reports it, in code as well as in comments, messages, documentation and file names.
