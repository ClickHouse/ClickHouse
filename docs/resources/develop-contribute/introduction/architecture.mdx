...
To address problems that exist in interpreters, a new `InterpreterSelectQueryAnalyzer` has been developed. This is a new version of the `InterpreterSelectQuery`, which does not use the `ExpressionAnalyzer` and introduces an additional layer of abstraction between `AST` and `QueryPipeline`, called `QueryTree`. It is the only query analysis since version `26.9`; the `enable_analyzer` setting that used to switch back to `ExpressionAnalyzer` is obsolete and cannot be disabled anymore.
