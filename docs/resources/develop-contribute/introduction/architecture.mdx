...
`InterpreterSelectQueryAnalyzer` interprets a `SELECT` query. It introduces an additional layer of abstraction between `AST` and `QueryPipeline`, called `QueryTree`, and it is the only way a query is analyzed since version `26.9`. Before that, `InterpreterSelectQuery` did the same job with the `ExpressionAnalyzer` and `ExpressionActions` machinery, and the `enable_analyzer` setting chose between the two; that setting is obsolete now and cannot be disabled anymore.
