...
아래 표는 퍼블릭 인터넷 또는 리전 간 이그레스의 데이터 전송 요금이 클라우드 제공업체와 리전에 따라 어떻게 달라지는지 보여줍니다.
...
$^*$데이터 전송 요금은 전송된 데이터 1GB당 미국 달러($) 기준입니다
...
$^*$데이터 전송 요금은 전송된 데이터 1GB당 달러($) 기준으로 표시됩니다