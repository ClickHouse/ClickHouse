export const SectionSearch = ({ section = "/reference/statements", label = "Statements", placeholder = "Search this section..." }) => {
  ...
  useEffect(() => {
    ...
    // 레거시 *.mintlify.app 소스는 호스트 루트에서 페이지를 노출했지만,
    ...
    const loadEmbed = (trap) => {
      ...
      if (trap && !window.__inkeepTrapped) {
        ...
        // embed.js가 이미 window.Inkeep을 채운 후에 모달 로드가 늦게 도착할 수 있습니다.
        // 트랩이 숨기기 전에 사용 가능한 인라인 검색 API를 보존하여,
        // 이후 리마운트가 모달을 무한히 폴링하지 않도록 합니다.
      ...
      if (!document.getElementById("inkeep-embed-js-script")) {
        ...
        s.src = EMBED
        s.onload = () => {
          // 이 컴포넌트가 리마운트되기 전에 모달 번들이 공유 window.Inkeep 전역을
          // 교체하더라도 embed API를 유지합니다.
    ...
    return () => {
      cancelled = true
      // Mintlify는 테마 전환 시 이 컴포넌트를 언마운트/리마운트합니다. 분리된 인스턴스를
      // 고아 상태로 남겨두면(박스가 사라지는 현상으로 나타남) 안 되므로,
      // 위젯을 정리하고 가드를 초기화하여 리마운트가 깨끗하게 시작되도록 합니다.
      try {
