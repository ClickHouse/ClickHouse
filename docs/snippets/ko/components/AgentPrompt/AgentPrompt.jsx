export const AgentPrompt = ({ prompt, title = "에이전트 지원 설정", description, outline, outlineLabel = "에이전트가 수행할 작업", repositoryUrl, repositoryLabel = "ClickHouse/agent-skills" }) => {
  ...
  const handleCopy = async () => {
    ...
    try {
      ...
      // 복사 실패는 나머지 페이지에 영향을 주지 않아야 합니다.
    }
  }
      ...
      <div className="ch-agent-prompt-main-row">
        ...
        >
          ...
          <span style={{ display: "grid", justifyItems: "center" }}>
            <span style={{ gridArea: "1 / 1", visibility: copied ? "hidden" : "visible" }}>프롬프트 복사</span>
            <span style={{ gridArea: "1 / 1", visibility: copied ? "visible" : "hidden" }}>복사됨</span>
          </span>
        </button>
      </div>
