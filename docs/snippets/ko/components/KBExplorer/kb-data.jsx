export const kbIndex = {
  ...
  "articles": [
    ...
    },
    {
      ...
      "description": "이 문서에서는 ZooKeeper 데이터의 경쟁 조건 또는 수동 개입으로 인해 발생하는 ClickHouse의 파트 교차 관련 DB::Exception 오류를 해결하는 방법을 설명합니다.",
    ...
    },
    {
      ...
      "description": "이 가이드에서는 내장 방법 또는 Grafana를 사용하여 자체 관리형 ClickHouse에서 쿼리 트레이스를 수집하고 시각화하는 방법을 설명합니다. 복잡한 쿼리를 다루면서 EXPLAIN이 제공하는 정보 이상으로 내부 실행 메커니즘을 이해해야 할 때 특히 유용합니다.",
    ...
    },
    {
      ...
      "title": "서버를 재시작하면 갱신 가능 구체화 뷰에 어떤 일이 발생하나요?",
      "description": "서버를 재시작할 때 갱신 가능 구체화 뷰가 모두 한꺼번에 갱신될 수 있는 이유와 이를 하나씩 다시 시작하는 방법을 설명합니다.",
    ...
    },
    {
      ...
      "description": "ClickHouse의 TTL 규칙은 최종적으로 적용되며, `merge_with_ttl_timeout` 설정을 사용하여 실행 시점을 제어할 수 있습니다. TTL을 강제로 적용하는 방법과 TTL 실행을 위한 백그라운드 스레드를 관리하는 방법을 알아보세요.",
    ...
    },
    {
      ...
      "description": "ClickHouse Keeper는 디스크 공간 사용량 감소, 빠른 복구, 낮은 메모리 소비 등의 개선 사항을 통해 ClickHouse 클러스터에서 ZooKeeper보다 더 나은 성능을 제공합니다.",
