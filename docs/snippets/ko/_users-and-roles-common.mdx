...
<Steps>
  ...
  <Step>
    ...
    특정 컬럼에 대한 접근 제한을 보여주는 데 사용할 일반 사용자를 생성합니다:
  ...
  <Step>
    ...
    특정 값을 가진 행에 대한 접근 제한을 보여주는 데 사용할 일반 사용자를 생성합니다:
...
<Steps>
  <Step>
    ##### 이 역할에 속한 사용자가 데이터베이스 `db1`의 `table1`에서 `column1`만 볼 수 있도록 제한하는 역할 생성
  ...
  <Step>
    ##### 이 역할에 속한 사용자가 선택된 행만 볼 수 있도록 제한하는 역할 생성. 여기서는 `column1`에 `A`가 있는 행만 볼 수 있습니다
  ...
  <Step>
    ##### `column1` 값이 `A`인 경우에만 볼 수 있도록 정책 생성
...
<Steps>
  ...
  <Step>
    ##### 지정되었고 허용된 컬럼만 사용하는 `SELECT` 쿼리를 확인합니다:
...
<Steps>
  ...
  <Step>
    ##### `A_row_users` 역할이 `column1`만 SELECT할 수 있도록 허용합니다
