...
## ClickHouse 서비스 생성 {#create-service}
...
## 서비스 프로비저닝이 완료될 때까지 기다리기 {#wait-for-provisioning}
...
출력은 터미널에서는 기본적으로 `PrettyCompact`, 파이프로 연결된 경우에는 `TabSeparated`를 사용합니다. 위에서 사용한 `--json`은 `JSONEachRow`를 선택합니다. 다른 포맷을 사용하려면 대신 `--format`을 전달하십시오(`--json`과 `--format`은 함께 사용할 수 없습니다).
...
## clickhouse client로 연결 {#native-client}
