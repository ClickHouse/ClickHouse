...
<Tip>
  [ClickHouse Managed Postgres](/ko/products/managed-postgres/overview) 서비스도 살펴보십시오. 컴퓨트와 물리적으로 동일한 위치에 배치된 NVMe storage를 기반으로 하므로, EBS와 같은 network-attached storage를 사용하는 대안과 비교해 디스크 입출력에 병목이 있는 워크로드에서 최대 10배 더 빠른 성능을 제공하며, ClickPipes의 Postgres CDC 커넥터를 사용해 Postgres 데이터를 ClickHouse로 복제할 수 있습니다.
</Tip>
...
<Steps>
  <Step title="PostgreSQL 설정" id="1-setting-up-postgresql">
  ...
  <Step title="통합 테스트" id="3-test-the-integration">
    1. ClickHouse에서 첫 몇 개의 행을 확인합니다:

    ```sql
    SELECT * FROM db_in_ch.table1
    ```

    ClickHouse 테이블은 PostgreSQL 테이블에 이미 존재하는 2개의 행으로 자동으로 채워져야 합니다:
    ...
    4. 새로 추가한 두 행이 ClickHouse 테이블에 표시됩니다:
    ...
    5. ClickHouse 테이블에 행을 추가하면 어떤 일이 발생하는지 살펴보겠습니다:
    ...
    6. ClickHouse에 추가된 행이 PostgreSQL 테이블에 표시되어야 합니다:
...
<Steps>
  ...
  <Step title="ClickHouse에서" id="2-in-clickhouse">
    ...
    2. 데이터베이스 엔진에 PostgreSQL 실험적 기능을 활성화합니다:
    ...
    | 매개변수      | 설명                     | 예시                                                                 |
    | --------- | ---------------------- | ------------------------------------------------------------------ |
    | host:port | 호스트명 또는 IP 주소 및 포트     | postgres-host.domain.com:5432                                      |
    ...
    <Info>
      PostgreSQL 데이터베이스 엔진에 대한 전체 가이드는 https://clickhouse.com/docs/engines/database-engines/materialized-postgresql/#settings를 참조하십시오.
    </Info>
  ...
  <Step title="기본 복제 테스트" id="3-test-basic-replication">
  ...
  <Step title="요약" id="4-summary">
    이 통합 가이드에서는 테이블이 포함된 데이터베이스를 복제하는 간단한 예시를 다루었지만, 전체 데이터베이스를 복제하거나 기존 복제에 새 테이블과 스키마를 추가하는 등 더 고급 옵션도 사용할 수 있습니다. 이 복제에서는 DDL 명령이 지원되지 않지만, 구조 변경이 발생하면 엔진이 변경 사항을 감지하고 테이블을 다시 로드하도록 설정할 수 있습니다.

    <Info>
      고급 옵션에서 사용할 수 있는 추가 기능은 [참고 문서](/ko/reference/engines/database-engines/materialized-postgresql)를 참조하십시오.
    </Info>
  </Step>
</Steps>