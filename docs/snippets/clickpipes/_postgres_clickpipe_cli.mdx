...
- `--table-mapping` is required and repeatable, one `schema.table:target_table` per source table. The replicated tables land in the `default` database on the ClickHouse service, named by the mapping targets — mapping to a different target name is how you rename a table during replication
