...
A `JSON`-typed schema is available in beta for evaluation on workloads with a small, stable attribute key-set. It is **not recommended** as the default. See {<a href={schemaUrl}>Map vs JSON type</a>} for the full comparison and the env vars required to enable JSON support.
