...
All settings except `database`, `table` and `engine` can also be set once for all system log tables in the `system_tables` section. A setting inside a particular table's section takes precedence, and a per-table `engine` replaces the whole engine definition, including the global `partition_by`, `ttl`, `order_by`, `storage_policy` and `settings`:
...
The `system_tables` section additionally accepts the following setting, which applies to all system log tables and cannot be set per table:
...
The previous location of this setting, `default_system_log_flush_policy.skip_alias_columns`, is deprecated and is used only if `system_tables.skip_alias_columns` is not set.
