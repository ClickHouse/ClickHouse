    ...
    <div className="not-prose glossary-browser">
      <div className="glossary-search-row">
        ...
        <div className="glossary-search-wrap">
          ...
          <input id="glossary-search" type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="用語と定義を検索..." autoComplete="off" />
        </div>
        <span className="glossary-count" aria-live="polite">
          {visibleEntries.length} {visibleEntries.length === 1 ? "件の用語" : "件の用語"}
        </span>
      </div>
