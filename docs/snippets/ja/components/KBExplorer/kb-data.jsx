export const kbIndex = {
  ...
  "articles": [
    ...
    },
    {
      ...
      "description": "この記事では、ClickHouseにおけるパートの交差に関連するDB::Exceptionエラーの解決方法を説明します。このエラーは、競合状態またはZooKeeperデータへの手動介入によって引き起こされることが多いです。",
    ...
    },
    {
      ...
      "description": "このガイドでは、組み込みの方法またはGrafanaを使用して、セルフマネージドのClickHouseでクエリトレースを収集・描画する方法を説明します。これは、複雑なクエリを扱う際に、EXPLAINが提供する情報を超えた内部実行メカニズムを理解する必要がある場合に特に役立ちます。",
    ...
    },
    {
      ...
      "title": "ClickHouse でメモリ使用量が多いクエリを特定する",
    ...
    },
    {
      ...
      "title": "ClickHouse の SSL 証明書検証エラーを解決する",
    ...
    },
    {
      ...
      "title": "ClickHouse の「パーツが多すぎる」例外を解消する",
      "description": "挿入のバッチ化、非同期挿入の活用、適切なパーティションキーの選択によって、「パーツが多すぎる」例外を診断し、解消する方法を学びます。",
    ...
    },
    {
      ...
      "description": "ClickHouse のスキーマ移行ツールと、変更され続けるデータベーススキーマを長期的に管理する方法について説明します。",
    ...
    },
    {
      ...
      "title": "サーバーを再起動するとリフレッシュ可能なマテリアライズドビューはどうなるか",
      "description": "サーバーの再起動時にリフレッシュ可能なマテリアライズドビューが一斉にリフレッシュされる理由と、1つずつ再開する方法。",
    ...
    },
    {
      ...
      "title": "ClickHouse Cloud SQL Console で `SET ROLE` の設定が永続化されない理由",
      "description": "ClickHouse Cloud SQL Console で `SET ROLE` の設定が永続化されない理由と、ユーザーごとに永続的な権限を割り当てる方法について説明します。",
      "href": "/ja/resources/support-center/knowledge-base/security/set-role-not-persisting-in-sql-console",
      "category": "Security & access control",
      "tags": [
        "Security and Authentication",
        "Managing Cloud",
