export const quickStartsData = [
  {
    id: "connect-your-iceberg-catalog",
    title: "Cómo conectar tu catálogo Iceberg en ClickHouse Cloud",
    description: "Aprende a conectar ClickHouse Cloud a tu catálogo de datos y a consultar tablas Iceberg.",
    href: "/es/get-started/quickstarts/connect-your-iceberg-catalog",
    useCases: ["data-warehousing"],
    products: ["cloud"]
  },
  {
    id: "create-your-first-materialized-view",
    title: "Crea tu primera vista materializada",
    description:
      "Aprende a usar las vistas materializadas en ClickHouse para precalcular y almacenar resultados de consultas con un orden distinto, lo que permite búsquedas rápidas en columnas que no forman parte de tu clave primaria.",
    href: "/es/get-started/quickstarts/create-your-first-materialized-view",
    useCases: ["real-time-analytics", "data-warehousing"],
    products: ["cloud"]
  },
  {
    id: "create-your-first-mergetree-table",
    title: "Crea tu primera tabla MergeTree",
    description:
      "Aprende cómo funciona el principal motor de tablas de ClickHouse creando una tabla MergeTree, cargando datos sobre precios de viviendas del Reino Unido y observando cómo las partes y las fusiones afectan al almacenamiento y al rendimiento de las consultas.",
    href: "/es/get-started/quickstarts/create-your-first-mergetree-table",
    useCases: ["all"],
    products: ["cloud"]
  },
  {
    id: "create-your-first-projection",
    title: "Crea tu primera proyección",
    description: "Aprende a usar las proyecciones en ClickHouse para almacenar una copia ordenada adicional de tus datos en la misma tabla, lo que permite búsquedas rápidas en columnas no incluidas en tu clave primaria.",
    href: "/es/get-started/quickstarts/create-your-first-projection",
    useCases: ["real-time-analytics", "data-warehousing"],
    products: ["cloud"]
  },
  {
    id: "create-your-first-service-on-cloud",
    title: "Inicio rápido de ClickHouse Cloud",
    description: "Guía de inicio rápido para ClickHouse Cloud",
    useCases: ["all"],
    products: ["cloud"],
    href: "/es/get-started/setup/cloud"
  },
  {
    id: "creating-tables",
    title: "Creación de tablas en ClickHouse",
    description: "Aprende a crear tablas en ClickHouse",
    href: "/es/get-started/quickstarts/creating-tables",
    useCases: ["all"],
    products: ["self-managed"]
  },
  {
    id: "insert-data-using-clickhouse-client",
    title: "Insertar datos en ClickHouse Cloud con clickhouse-client",
    description: "Aprende a usar clickhouse-client para insertar datos desde archivos CSV y Parquet locales en un servicio de ClickHouse Cloud desde la línea de comandos.",
    href: "/es/get-started/quickstarts/insert-data-using-clickhouse-client",
    useCases: ["all"],
    products: ["cloud"]
  },
  {
    id: "mutations",
    title: "Actualización y eliminación de datos de ClickHouse",
    description: "Describe cómo realizar operaciones de actualización y eliminación en ClickHouse",
    href: "/es/get-started/quickstarts/mutations",
    useCases: ["data-warehousing"],
    products: ["self-managed"]
  },
  {
    id: "obtain-your-cloud-connection-details",
    title: "Obtén los detalles de conexión de tu servicio en Cloud",
    description: "Aprende a encontrar el hostname, el puerto y las credenciales de tu servicio de ClickHouse Cloud para conectarte desde clientes externos, CLI y aplicaciones.",
    href: "/es/get-started/quickstarts/obtain-your-cloud-connection-details",
    useCases: ["all"],
    products: ["cloud"]
  },
  {
    id: "tutorial",
    title: "Tutorial avanzado",
    description: "Aprenda a ingestar y consultar datos en ClickHouse con un conjunto de datos de ejemplo de taxis de la ciudad de Nueva York.",
    href: "/es/get-started/quickstarts/tutorial",
    useCases: ["real-time-analytics", "data-warehousing"],
    products: ["cloud", "self-managed"]
  },
  {
    id: "working-with-the-map-type",
    title: "Uso del tipo Map en ClickHouse",
    description: "Aprenda a usar el tipo Map en ClickHouse para almacenar, consultar y agregar datos dinámicos de clave-valor, usando atributos de recurso de OTel como ejemplo práctico.",
    href: "/es/get-started/quickstarts/working-with-the-map-type",
    useCases: ["observability"],
    products: ["self-managed"]
  },
  {
    id: "writing-queries",
    title: "Seleccionar datos de ClickHouse",
    description: "Aprende a seleccionar datos de ClickHouse",
    href: "/es/get-started/quickstarts/writing-queries",
    useCases: ["all"],
    products: ["self-managed"]
  }
]