...
<Steps>
  ...
  <Step title="Defina una tabla en ClickHouse" id="2-define-a-table-in-clickhouse">
    ...
    2. Creemos una nueva base de datos:
    ...
    | --------- | -------------------------------------------- | ----------------------------- |
    | host:port | hostname o dirección IP y puerto             | postgres-host.domain.com:5432 |
    | database  | nombre de la base de datos PostgreSQL        | db&#95;in&#95;psg             |
  ...
  <Step title="Pruebe la integración" id="3-test-the-integration">
    ...
    2. De vuelta en PostgreSQL, agregue un par de filas a la tabla:
    ...
    4. Esas dos nuevas filas deberían aparecer en tu tabla de ClickHouse:
    ...
    5. Veamos qué sucede al añadir filas a la tabla de ClickHouse:
    ...
    6. Las filas añadidas en ClickHouse deberían aparecer en la tabla de PostgreSQL:
...
<Steps>
  ...
  <Step title="En ClickHouse" id="2-in-clickhouse">
    ...
    | user      | nombre de usuario para conectarse a Postgres | clickhouse&#95;user                                                |
    | password  | contraseña para conectarse a Postgres        | ClickHouse&#95;123                                                 |
    | settings  | configuración adicional del motor            | materialized&#95;postgresql&#95;tables&#95;list = &#39;table1&#39; |

    <Info>
      Para consultar la guía completa sobre el motor de base de datos PostgreSQL, consulte https://clickhouse.com/docs/engines/database-engines/materialized-postgresql/#settings
    </Info>

    4. Verifique que la tabla inicial contiene datos:
  ...
  <Step title="Pruebe la replicación básica" id="3-test-basic-replication">
