...
A inserção de 1 milhão de linhas leva cerca de 7 segundos no `m6gd.large` (a menor configuração), graças ao armazenamento NVMe. Verifique com uma consulta; a contagem de linhas varia entre as execuções porque os dados são gerados com `random()`:
