...
## Crie um serviço do ClickHouse {#create-service}
...
## Aguarde o provisionamento do serviço {#wait-for-provisioning}
...
## Execute SQL com a Query API {#run-sql}
...
A saída usa `PrettyCompact` por padrão em um terminal e `TabSeparated` quando enviada via pipe. `--json`, usado acima, seleciona `JSONEachRow`; para qualquer outro formato, use `--format` (`--json` e `--format` não podem ser combinados).

## Crie um banco de dados e uma tabela {#create-database-and-table}
...
## Carregue um arquivo CSV {#load-csv-file}
...
## Conecte-se usando o clickhouse client {#native-client}
