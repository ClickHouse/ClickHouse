...
<Steps>
  <Step title="Configurando o PostgreSQL" id="1-setting-up-postgresql">
  ...
  <Step title="Defina uma tabela no ClickHouse" id="2-define-a-table-in-clickhouse">
    ...
    3. Crie uma tabela que use o mecanismo de tabela `PostgreSQL`:
    ...
    <Note>
      Consulte a página de documentação do [PostgreSQL table engine](/pt-BR/reference/engines/table-engines/integrations/postgresql) para ver a lista completa de parâmetros.
    </Note>
  </Step>

  <Step title="Teste a integração" id="3-test-the-integration">
    1. No ClickHouse, visualize as primeiras linhas:
    ...
    4. Essas duas novas linhas devem aparecer na tabela do ClickHouse:
    ...
    6. As linhas adicionadas no ClickHouse devem aparecer na tabela no PostgreSQL:
...
<Steps>
  ...
  <Step title="No ClickHouse" id="2-in-clickhouse">
    1. acesse a CLI do ClickHouse

    ```bash
    clickhouse-client --user default --password ClickHouse123!
    ```

    2. Habilite o recurso experimental do PostgreSQL para o mecanismo de banco de dados:
    ...
    <Info>
      Para obter o guia completo sobre o mecanismo de banco de dados PostgreSQL, consulte https://clickhouse.com/docs/engines/database-engines/materialized-postgresql/#settings
    </Info>

    4. Verifique se a tabela inicial contém dados:
  ...
  <Step title="Testar a replicação básica" id="3-test-basic-replication">
