...
O comportamento principal é controlado pela configuração [`async_insert`](/pt-BR/reference/settings/session-settings/async-insert#async_insert).
...
* O buffer atinge um tamanho de dados especificado ([`async_insert_max_data_size`](/pt-BR/reference/settings/session-settings/async-insert#async_insert_max_data_size), padrão de 100 MiB).
* Um limite de tempo é atingido ([`async_insert_busy_timeout_ms`](/pt-BR/reference/settings/session-settings/async-insert#async_insert_busy_timeout_max_ms), padrão de 200 ms ou 1000 ms no Cloud).
* Um número máximo de consultas de inserção é acumulado ([`async_insert_max_query_number`](/pt-BR/reference/settings/session-settings/async-insert#async_insert_max_query_number), padrão de 450).
...
O comportamento das inserções assíncronas é refinado ainda mais com a configuração [`wait_for_async_insert`](/pt-BR/reference/settings/session-settings/wait-for#wait_for_async_insert).
...
Desde a versão 24.2, o ClickHouse usa timeouts adaptativos de flush por padrão ([`async_insert_use_adaptive_busy_timeout`](/pt-BR/reference/settings/session-settings/async-insert#async_insert_use_adaptive_busy_timeout)). Em vez de um intervalo de flush fixo, o timeout se ajusta dinamicamente entre um mínimo ([`async_insert_busy_timeout_min_ms`](/pt-BR/reference/settings/session-settings/async-insert#async_insert_busy_timeout_min_ms), padrão de 50 ms) e um máximo ([`async_insert_busy_timeout_max_ms`](/pt-BR/reference/settings/session-settings/async-insert#async_insert_busy_timeout_max_ms), padrão de 200 ms ou 1000 ms no Cloud), com base na taxa de chegada dos dados.
...
Desde a versão 26.2, o ClickHouse realiza desduplicação automática tanto para inserções assíncronas quanto para síncronos, o que torna seguras as novas tentativas em tabelas que mantêm um registro de desduplicação. Os motores `*ReplicatedMergeTree` mantêm um por padrão; uma tabela `MergeTree` simples precisa ter `non_replicated_deduplication_window` definido com um valor positivo. Ambos os tipos de insert são controlados por [`deduplicate_insert`](/pt-BR/reference/settings/session-settings/deduplicate-insert#deduplicate_insert), cujo valor padrão é `enable`. Visões materializadas dependentes são compatíveis.

Na prática, se o mesmo insert for repetido — por exemplo, devido a um timeout ou a uma queda de rede — o ClickHouse poderá ignorar a duplicata com segurança. Isso ajuda a manter a idempotência e evita a gravação duplicada de dados. Para inserções assíncronas, a desduplicação funciona por consulta do usuário, e não por batch descarregado; portanto, uma consulta duplicada não descarta as outras consultas agrupadas com ela.

Para obter detalhes e conhecer a única restrição aplicável a visões materializadas em inserções assíncronas, consulte [Desduplicação de inserts em novas tentativas](/pt-BR/concepts/features/operations/insert/deduplicating-inserts-on-retries#deduplication-for-asynchronous-inserts).
