...
To send OpenTelemetry data to ClickHouse Observability, you're recommended to use an OpenTelemetry Collector. The collector acts as a gateway that receives OpenTelemetry data from your applications (and other collectors) and forwards it to ClickHouse Cloud.
...
While this command uses the `default` user to connect ClickHouse Observability, you should create a dedicated user when [going to production](/clickstack/managed/managing/production#create-a-database-ingestion-user-managed) and modifying your configuration.
</Info>
...
For further details on configuring OpenTelemetry collectors, see ["Ingesting with OpenTelemetry."](/clickstack/managed/ingesting-data/opentelemetry)
...
To instrument your applications to collect traces and logs, use the [supported language SDKs](/clickstack/managed/ingesting-data/sdks) which send data to your OpenTelemetry Collector acting as a gateway for ingestion into ClickHouse Observability.

Logs can be [collected using OpenTelemetry Collectors](/clickstack/managed/integration-examples/host-logs) running in agent mode, forwarding data to the same collector. For Kubernetes monitoring, follow the [dedicated guide](/clickstack/managed/integration-examples/kubernetes). For other integrations, see our [quickstart guides](/clickstack/managed/integration-examples).

### Demo data

Alternatively, if you don't have existing data, try one of our sample datasets.

- [Example dataset](/clickstack/managed/example-datasets/sample-data) - Load an example dataset from our public demo. Diagnose a simple issue.
- [Local files and metrics](/clickstack/managed/example-datasets/local-data) - Load local files and monitor the system on OSX or Linux using a local OTel collector.
...
The only strict requirement for ClickHouse Observability, is that the data includes a **timestamp column** (or equivalent time field), which can be declared when configuring the data source in the ClickStack UI.
</Info>
...
We strongly recommend understanding how [Primary keys](/concepts/core-concepts/primary-indexes) work in ClickHouse and choosing an ordering key based on your access patterns. See the [ClickStack-specific](/clickstack/managed/managing/performance-tuning#choosing-a-primary-key) guidance on choosing a primary key.
...
For more examples of ingesting data with Vector, see ["Ingesting with Vector"](/clickstack/managed/ingesting-data/vector) or the [Vector ClickHouse sink documentation](https://vector.dev/docs/reference/configuration/sinks/clickhouse/) for advanced options.
<br/>
</Tab>
</Tabs>
