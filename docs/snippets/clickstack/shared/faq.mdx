...
- [Search alerts](./features/alerts#search-alerts): trigger notifications when the count of matching log or trace results within a time window exceeds or falls below a threshold.
- [Dashboard chart alerts](./features/alerts#dashboard-alerts): trigger notifications when a metric plotted on a dashboard tile crosses a defined threshold.

Both alert types use static threshold conditions. For full details, see [Alerts](./features/alerts).
</Accordion>

<Accordion title="Can I alert on complex metric conditions such as ratios, p95/p99, or multi-metric formulas?">
Ratios of two metrics, p95, and p99 values can be plotted on a dashboard tile using the [chart builder](./features/dashboards/overview#navigate-chart-explorer) UI. You can then create threshold-based alerts on those tiles.
...
For details on creating dashboards and applying filters, see [Dashboards](./features/dashboards/overview).
</Accordion>

<Accordion title="What drill-down capabilities are available?">
ClickStack supports the following drill-down workflows:

- [Dashboard-level filtering](./features/dashboards/overview#filter-dashboards): Lucene or SQL filters and time range adjustments applied at the dashboard level propagate to all tiles.
...
- Click-to-view events: clicking on data in a dashboard tile and selecting **View Events** navigates to the [Search](./features/search) page with relevant filters for log and trace data.
- [Table row-click drilldowns](./features/dashboards/row-click-drilldowns): a table tile can be configured so that clicking a row opens the Search page or another dashboard, carrying the row's values through as filters. This supports multi-level workflows, such as a service inventory that drills into a per-service detail dashboard.
- [Prebuilt dashboard drill-downs](./features/dashboards/overview#presets): the [Services](./features/dashboards/overview#services-dashboard), [ClickHouse](./features/dashboards/overview#clickhouse-dashboard), and [Kubernetes](./features/dashboards/overview#kubernetes-dashboard) dashboards include richer, built-in drill-down navigation across tabs.

<Note>
The **View Events** drill-down works best with log and trace data. Because metrics data cannot be viewed on the [Search](./features/search) page, drilling down from a metrics tile will link to logs from around the selected time frame instead.
</Note>
</Accordion>
...
- [Alerts](./features/alerts): search alerts, dashboard chart alerts, and webhook integrations.
- [Dashboards](./features/dashboards/overview): creating visualizations, building dashboards, and applying filters.
- [Search](./features/search): querying logs and traces with Lucene and SQL syntax.
- [Schemas](./ingesting-data/schemas): OpenTelemetry data schemas for logs, traces, and metrics.
- [Architecture](./architecture): ClickStack components and how they fit together.
