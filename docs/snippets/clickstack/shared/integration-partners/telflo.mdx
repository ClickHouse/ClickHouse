...
- Send the telemetry to Open Source or ClickHouse Observability, or write it straight into ClickHouse
...
Telflo is also accessible outside the browser, through a [command-line interface](https://docs.telflo.com/docs/cli/). Pair that with the [ClickStack MCP server](../mcp) and the same agent that queries your telemetry can fix the pipeline that produced it.
...
- A destination: [ClickStack](/clickstack/getting-started/oss), [ClickHouse Observability](/clickstack/getting-started/managed), or a self-managed ClickHouse instance
...
- **ClickStack**: your ClickStack OTLP endpoint (`4318` for HTTP, `4317` for gRPC) and your ingestion API key, found in HyperDX under **Team Settings → API Keys** ([view docs here for reference](../ingesting-data/opentelemetry#sending-otel-data))
- **ClickHouse Observability**: your ClickHouse Cloud service endpoint (port `8443`) and an ingestion user (see [Creating an ingestion user](../ingesting-data/collector#creating-an-ingestion-user))
- **Self-managed ClickHouse**: your ClickHouse endpoint, a database for the [ClickStack tables](../ingesting-data/schemas), and a user that can create and write to them (see [Creating an ingestion user](../ingesting-data/collector#creating-an-ingestion-user-oss))
...
All three paths use the same editor, the same publish step, and the same fleet; only the exporter differs. Pick the tab for your deployment: ClickStack takes OTLP with an ingestion key; ClickHouse Observability and self-managed ClickHouse take direct writes to the ClickStack tables.
...
ClickHouse Observability does not ship a gateway collector, so there is no hosted OTLP endpoint to send telemetry to. The collector you configure here becomes that [gateway](../ingesting-data/collector#collector-roles): give it an **OTLP** receiver alongside the ClickHouse exporter, and it writes to your ClickHouse Cloud service. Your SDKs and agent collectors then send OTLP to it on `4318` (HTTP) or `4317` (gRPC).
...
4. Under **Schema & Tables**: if the [ClickStack tables](../ingesting-data/schemas) don't exist yet, leave **Create Schema on Startup** on and the exporter creates them. Turn it off once they exist.

Keep **Batch** upstream of the exporter: ClickHouse performs best with batched inserts. If you already run a [ClickStack collector](../ingesting-data/collector#configuring-the-collector) as your OTLP gateway, you can instead follow the ClickStack steps, with the [`OTLP_AUTH_TOKEN`](../ingesting-data/collector#securing-the-collector) you set on that collector as the header value.

</Tab>
<Tab title="Self-managed ClickHouse">

The collector you configure here acts as your [gateway](../ingesting-data/collector#collector-roles): give it an **OTLP** receiver alongside the ClickHouse exporter. Your SDKs and agent collectors send OTLP to it, and it writes to ClickHouse.
...
<Frame>
  <img src="/images/clickstack/telflo/telflo-configuration.webp" alt="A collector pipeline in the Telflo editor with the ClickHouse exporter configured" />
</Frame>
</div>
...
<div className="ch-image-lg">
<Frame>
  <img src="/images/clickstack/telflo/telflo-fleet.webp" alt="A Telflo fleet with every collector in sync after a rollout" />
</Frame>
</div>
</Step>
<Step title="Verify telemetry" id="verify-telemetry">

Open HyperDX and search for one of your services in the **Search** view; use **Chart Explorer** for metrics. With ClickHouse Observability, open your service in the ClickHouse Cloud console and click **Launch ClickStack**. With self-managed ClickHouse, query the [ClickStack tables](../ingesting-data/schemas) directly, for example `SELECT ServiceName, count() FROM otel_logs GROUP BY ServiceName`.
</Step>
</Steps>
...
- **ClickHouse exporter**: keep **Async Insert** on, leave **Compression** at lz4, and add a **Batch** processor upstream. See [Optimizing inserts](../ingesting-data/collector#optimizing-inserts).
...
- [Ingesting with OpenTelemetry](../ingesting-data/opentelemetry)
- [ClickStack table schemas](../ingesting-data/schemas)
