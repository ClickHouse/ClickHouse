...
We [recommend using the official ClickStack distribution of the collector](/clickstack/deployment/hyperdx-only#otel-collector) for the gateway role when sending to ClickHouse Observability, where possible. If you choose to bring your own, ensure it includes the [ClickHouse exporter](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/exporter/clickhouseexporter).
...
Create a `values.yaml` configuring the ClickStack image and ClickHouse Observability credentials:
...
For details on retrieving your ClickHouse Observability credentials, see [here](/products/cloud/guides/sql-console/connection-details).

<Info>
**Production user**

You should use a user with the [appropriate credentials](./collector#creating-an-ingestion-user) in production.
</Info>

### Modifying configuration {#modifying-otel-collector-configuration-managed}

#### Configuring ClickHouse Observability instance {#configuring-managed-clickstack}

The OpenTelemetry collector can be configured to use a ClickHouse Observability instance via the environment variables `CLICKHOUSE_ENDPOINT`, `CLICKHOUSE_USER` and `CLICKHOUSE_PASSWORD`. How these are set depends on your deployment method:
...
We recommend creating a dedicated database and user for the OTel collector for ingestion into ClickHouse Observability. This should have the ability to create and insert into the [tables created and used by ClickStack](./schemas).
...
We recommend creating a dedicated database and user for the OTel collector for ingestion into ClickHouse. This should have the ability to create and insert into the [tables created and used by ClickStack](./schemas).
...
<Frame>
  <img src="/images/use-cases/observability/observability-6.webp" alt="Async inserts" />
</Frame>
</div>
...
<div className="ch-image-lg">
<Frame>
  <img src="/images/use-cases/observability/clickstack-with-gateways.webp" alt="Scaling with gateways" />
</Frame>
</div>
...
<div className="ch-image-lg">
<Frame>
  <img src="/images/use-cases/observability/observability-8.webp" alt="Adding kafka" />
</Frame>
</div>
...
For the full comparison, when each is appropriate, the env vars required to enable the JSON-typed schema, and the migration walkthrough, see [Map vs JSON type](./schema/map-vs-json).
