...
How this is achieved depends on your current agents for collection - see ["Migrating Agents"](./migrating-agents).
</Step>
<Step title="Adjust retention periods" id="adjust-retention-period">

Configure Elastic's TTL settings to match your desired retention period. Set up the ClickStack [TTL](../../managing/production#configure-ttl) to maintain data for the same duration.
</Step>
...
ClickHouse uses [TTL clauses](../../managing/production#configure-ttl) on MergeTree tables to manage data expiration. TTL policies can:
...
We recommend aligning your ClickHouse TTL configuration with your existing Elastic retention policies to maintain a consistent data lifecycle during the migration. For examples, see [ClickStack production TTL setup](../../managing/production#configure-ttl).
...
Create a table in ClickHouse for the index being migrated from Elasticsearch. You can map [Elasticsearch types to their ClickHouse](./types) equivalent. Alternatively, you can simply rely on the JSON data type in ClickHouse, which will dynamically create columns of the appropriate type as data is inserted.
