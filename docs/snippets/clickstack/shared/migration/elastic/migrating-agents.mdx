...
Where possible we recommend migrating to the [OpenTelemetry (OTel) Collector](https://opentelemetry.io/docs/collector/) for all log, metric, and trace collection, deploying the collector at the [edge in an agent role](../../ingesting-data/collector#collector-roles). This represents the most efficient means of sending data and avoids architectural complexity and data transformation.
...
All data is ingested into ClickStack via an **OpenTelemetry (OTel) collector** instance, which acts as the primary entry point for logs, metrics, traces, and session data. We recommend using the official [ClickStack distribution](../../ingesting-data/opentelemetry#installing-otel-collector) of the collector for this instance, if not [already bundled in your ClickStack deployment model](../../deployment/overview).

Users send data to this collector from [language SDKs](../../ingesting-data/sdks) or through data collection agents collecting infrastructure metrics and logs (such OTel collectors in an [agent](../../ingesting-data/collector#collector-roles) role or other technologies e.g. [Fluentd](https://www.fluentd.org/) or [Vector](https://vector.dev/)). For teams that want a managed OpenTelemetry pipeline, [Bindplane](../../integration-partners/bindplane)offers an OpenTelemetry-native solution with a native ClickStack destination, simplifying telemetry collection, processing, and routing.
...
<Frame>
  <img src="/images/use-cases/observability/clickstack-migrating-agents.webp" alt="Migrating agents"  background />
</Frame>
</div>
...
<Frame>
  <img src="/images/use-cases/observability/add-logstash-output.webp" alt="Add Logstash output" />
</Frame>
</div>
...
<div className="ch-image-md">
<Frame>
  <img src="/images/use-cases/observability/agent-output-settings.webp" alt="Agent settings" />
</Frame>
</div>
...
By default, an API ingestion key isn't required if running an OpenTelemetry Collector standalone for ClickHouse Observability. Ingestion can be secured however, by specifying an OTLP auth token. See ["Securing the collector"](../../ingesting-data/collector#securing-the-collector).
</Info>
