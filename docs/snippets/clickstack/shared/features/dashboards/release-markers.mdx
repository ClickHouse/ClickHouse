...
<Frame>
  <img src="/images/clickstack/dashboards/release-markers-menu.webp" alt="Dashboard overflow menu with the Show release markers item under the View section, below Show alert annotations" />
</Frame>
</div>
...
Markers draw on time-series tiles whose source is a log or trace source. [Alert annotations](../alerts) come from the same menu and can be shown at the same time — the two sets of markers are drawn together, and each keeps its own labels.

## Configure where the version comes from {#version-expression}

By default, release markers use `ResourceAttributes['service.version']`, the OpenTelemetry resource attribute. If your services follow the [service resource semantic conventions](https://opentelemetry.io/docs/specs/semconv/registry/attributes/service/), markers work with no setup.
...
<Frame>
  <img src="/images/clickstack/dashboards/service-version-expression.webp" alt="Service Version Expression field in a log source's optional fields, showing the default placeholder ResourceAttributes['service.version']" />
</Frame>
</div>
...
The field is also exposed as `serviceVersionExpression` on log and trace sources in the [sources API](../../api-reference), so you can set it when provisioning sources programmatically. See the full list of source settings for [logs](../../managing/config#logs) and [traces](../../managing/config#traces).
...
The releases query runs against the tile's own source with the tile's own predicates: its `WHERE` clause, each series' filter, and any [dashboard filters](./overview#custom-filters). What a tile shows depends on what it charts:
...
<div className="ch-image-sm">
<Frame>
  <img src="/images/clickstack/dashboards/release-marker-tooltip.webp" alt="Hover tooltip on a release marker naming the service frontend and version 2.0.2 with the time the version first appeared, on a chart of two services" />
</Frame>
</div>
...
- **Empty version values are skipped.** A service that emits no version contributes no markers. If a tile finds no version changes at all, the chart displays `No releases found` rather than leaving you to wonder whether the feature is broken.
