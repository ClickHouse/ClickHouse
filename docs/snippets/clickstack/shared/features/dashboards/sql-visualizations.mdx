...
<Frame>
  <img src="/images/use-cases/observability/sql-editor-button.webp" alt="SQL Editor Button" />
</Frame>
</div>
...
When a dashboard filter is [available as a variable](./overview#dashboard-variables), its selected values can be referenced anywhere in the query. These macros are only available on tiles belonging to a dashboard that declares at least one variable.
