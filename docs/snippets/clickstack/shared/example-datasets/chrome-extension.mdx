...
Unlike the [Session Replay Demo](./session-replay), which instruments a demo application you control, this approach works on **any** URL you open in Chrome. You generate session data by interacting with the site as a normal user.

For background on session replay and how it fits into ClickStack, see the [Session Replay](../features/session-replay) feature page.
...
For ClickHouse Observability or HyperDX Cloud ingestion, open HyperDX, go to **Team Settings → API Keys**, and copy your **Ingestion API Key**.
</Step>
...
<Frame>
  <img src="/images/clickstack/chrome-extension/extension-config.png" alt="HyperDX Chrome extension configuration popup with local ClickStack settings" />
</Frame>
</div>
...
- [Session Replay Demo](./session-replay) — instrument a demo application from source code
