...
This is a different job from the [session replay demo](./session-replay), which instruments a browser-only app against local Docker ClickStack. Here you get backend auto-instrumentation, ClickHouse query spans, and session replay from the same app.
...
- [Session Replay Demo](./session-replay): a self-contained demo with a local ClickStack instance.
