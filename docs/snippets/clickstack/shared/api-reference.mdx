...
ClickStack exposes a REST API for programmatically managing dashboards, alerts, and data sources. The API is available for both **ClickHouse Observability** (ClickHouse Cloud) and **ClickStack** deployments, though the endpoints and authentication differ between the two.
...
For ClickHouse Observability, the API is accessed through the **ClickHouse Cloud API**. The ClickStack endpoints are available in the [Cloud API reference](/products/cloud/api-reference/clickstack/click-stack-list-dashboards).
...
For ClickStack, the full API specification is maintained in the [HyperDX repository](https://github.com/hyperdxio/hyperdx) and can be viewed interactively or downloaded as an OpenAPI spec:
...
ClickHouse Observability uses the **ClickHouse Cloud API key** for authentication via [HTTP Basic Authentication](https://developer.mozilla.org/en-US/docs/Web/HTTP/Authentication). To create and manage API keys, see [Managing API keys](/products/cloud/features/admin-features/api/openapi).
...
ClickStack uses a **Bearer token** for authentication via a **Personal API Access Key**.
...
<Frame>
  <img src="/images/clickstack/api-key-personal.webp" alt="ClickStack API Key" />
</Frame>
</div>
...
All ClickHouse Observability API requests are sent to the ClickHouse Cloud API:
...
All ClickStack API requests are sent to the HyperDX API server on port `8000`:
