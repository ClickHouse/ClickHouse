...
L’insertion de 1 M de lignes s’effectue en environ 7 secondes sur `m6gd.large` (la plus petite taille), grâce au stockage NVMe. Vérifiez-le avec une requête ; le nombre de lignes varie d’une exécution à l’autre, car les données sont générées avec `random()` :
