...
<Steps>
  ...
  <Step title="Définir une table dans ClickHouse" id="2-define-a-table-in-clickhouse">
    ...
    3. Créez une table qui utilise `PostgreSQL` :
    ...
    | --------- | ------------------------------------------------ | ----------------------------- |
    | host:port | hostname ou adresse IP et port                   | postgres-host.domain.com:5432 |
    ...
    <Note>
      Consultez la page de documentation [PostgreSQL table engine](/fr/reference/engines/table-engines/integrations/postgresql) pour obtenir la liste complète des paramètres.
    </Note>
  </Step>

  <Step title="Tester l’intégration" id="3-test-the-integration">
    1. Dans ClickHouse, affichez les premières lignes :

    ```sql
    SELECT * FROM db_in_ch.table1
    ```

    La table ClickHouse devrait être automatiquement renseignée avec les deux lignes déjà présentes dans la table PostgreSQL :
    ...
    2. De retour dans PostgreSQL, ajoutez quelques lignes à la table :
...
<Steps>
  ...
  <Step title="Dans ClickHouse" id="2-in-clickhouse">
    1. connectez-vous à l’interface CLI de ClickHouse
    ...
    <Info>
      Pour consulter le guide complet sur le moteur de base de données PostgreSQL, rendez-vous sur https://clickhouse.com/docs/engines/database-engines/materialized-postgresql/#settings
    </Info>
  ...
  <Step title="Tester la réplication" id="3-test-basic-replication">
    ...
    2. Dans ClickHouse, vérifiez que les nouvelles lignes sont visibles :
  ...
  <Step title="Résumé" id="4-summary">
    Ce guide d’intégration présente un exemple simple de réplication d’une base de données contenant une table. Des options plus avancées permettent toutefois de répliquer l’intégralité de la base de données ou d’ajouter de nouvelles tables et de nouveaux schémas aux réplications existantes. Bien que les commandes DDL ne soient pas prises en charge par cette réplication, le moteur peut être configuré pour détecter les modifications structurelles et recharger les tables en conséquence.

    <Info>
      Pour découvrir davantage de fonctionnalités des options avancées, consultez la [documentation de référence](/fr/reference/engines/database-engines/materialized-postgresql).
    </Info>
  </Step>
</Steps>