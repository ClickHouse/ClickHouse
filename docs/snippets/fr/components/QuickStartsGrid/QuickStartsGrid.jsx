export const QuickStartsGrid = ({ quickStartsData = [], featured = [] }) => {
  ...
  // Lit de manière sécurisée un tableau de chaînes persisté dans localStorage. Une valeur
  // corrompue ou modifiée manuellement ne doit jamais provoquer d'exception dans un initialiseur
  // useState, ce qui ferait planter tout le rendu de la page — on revient à la valeur par défaut.
  ...
  const featuredQuickStarts = featured
    .map((f) => {
      ...
      return {
        id: f.id,
        image: f.image,
        href: (qs && qs.href) || `/fr/get-started/quickstarts/${f.id}`,
        title: (qs && qs.title) || ""
      }
    })
    .filter((f) => f.image)
      ...
      <div style={{ maxWidth: "1312px", marginLeft: "max(0px, calc((100vw - 1312px) / 2 - 19rem))", marginRight: "auto", paddingLeft: "1.75rem", paddingRight: "1.75rem" }}>
        <div className="my-8">
          ...
          {featuredQuickStarts.length > 0 && (
            <div className="mb-12">
              <h2 className="text-2xl font-semibold text-gray-900 dark:text-zinc-50 mb-6">Guides de démarrage rapide à la une</h2>
                  ...
                  <div className="space-y-6">
                    ...
                    <div>
                      ...
                      <div className="relative w-full">
                        ...
                        <input
                          type="text"
                          placeholder="Rechercher des guides de démarrage rapide..."
                          value={searchTerm}
                    ...
                    <div>
                      <div className="space-y-5">
                        <FilterGroup label="Cas d'usage" options={useCaseOptions} selectedOptions={selectedUseCases} onToggle={toggleUseCase} />
                        <FilterGroup label="Domaine produit" options={productOptions} selectedOptions={selectedProducts} onToggle={toggleProduct} />
                      </div>
                    </div>
