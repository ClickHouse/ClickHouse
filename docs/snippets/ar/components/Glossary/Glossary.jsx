    ...
    <div className="not-prose glossary-browser">
      <div className="glossary-search-row">
        <label className="sr-only" htmlFor="glossary-search">
          البحث في مصطلحات المسرد وتعريفاتها
        </label>
        <div className="glossary-search-wrap">
          ...
          <input id="glossary-search" type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="ابحث في المصطلحات والتعريفات..." autoComplete="off" />
        </div>
        <span className="glossary-count" aria-live="polite">
          {visibleEntries.length} {visibleEntries.length === 1 ? "مصطلح" : "مصطلحات"}
        </span>
      </div>
      ...
      {visibleEntries.length > 0 ? (
            ...
            <article key={entry.id} className="glossary-entry">
              ...
              {entry.learnMore && (
                <a className="glossary-entry-link" href={entry.learnMore}>
                  معرفة المزيد <span aria-hidden="true">→</span>
                </a>
              )}
            </article>
          ))}
        ...
        <div className="glossary-empty">
          <p>لا توجد مصطلحات في المسرد تطابق «{query}».</p>
