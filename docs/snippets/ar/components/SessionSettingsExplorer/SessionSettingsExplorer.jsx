const SessionSettingsExplorer = ({ href: baseRoute }) => {
  ...
  const renderGroup = (entry, continuations = [], isLast = false, path = []) => {
    ...
    const items = [...entry.settings.map((setting) => ({ type: "إعداد", value: setting })), ...entry.children.map((child) => ({ type: "group", value: child }))]
    const countLabel = `${entry.count} ${entry.count === 1 ? "إعداد" : "settings"}`
