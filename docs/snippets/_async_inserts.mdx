...
Destination lookup and authorization are re-evaluated at flush time as well, against the catalog and access-control state current then, not the state when the insert was received. A materialized view attached to the destination, or a grant revoked, between receiving the insert and flushing its buffer affects the flush.
