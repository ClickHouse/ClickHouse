export const IntegrationGrid = () => {
  ...
  function getSectionDescription(type) {
    const descriptions = {
      ...
      "Language client": "用您最熟悉的语言编写代码！ClickHouse 的语言客户端集成让您在多种编程语言中都能流畅访问数据。",
  ...
  if (loading) {
    return (
      <div className="text-center py-12">
        <p className="text-gray-600 dark:text-gray-400">正在加载集成...</p>
      </div>
    )
  }
  ...
  if (error) {
    return (
      <div className="text-center py-12">
        <p className="text-red-600 dark:text-red-400">加载集成失败：{error}</p>
        <p className="text-gray-600 dark:text-gray-400">请尝试刷新页面。</p>
      </div>
    )
  }
  ...
  if (integrations.length === 0) {
    return (
      <div className="text-center py-12">
        <p className="text-gray-600 dark:text-gray-400">未找到任何集成。</p>
      </div>
    )
  }
    ...
    <>
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        {filteredIntegrations.length === 0 && (
          <div className="text-center py-12">
            <p className="text-gray-600 dark:text-gray-400 text-lg">未找到符合条件的集成。</p>
          </div>
        )}
      </div>
    </>
