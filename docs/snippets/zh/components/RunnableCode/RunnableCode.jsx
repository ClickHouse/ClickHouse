  ...
  return (
    <div className="not-prose" style={{ margin: "1rem 0", width: "100%", boxSizing: "border-box", contain: "inline-size" }}>
      ...
      {showResults && (
        ...
        >
          <div>
            {loading && <div style={{ padding: "24px", textAlign: "center", color: mutedColor }}>正在执行查询...</div>}
            ...
            {results && results.meta && results.data && (
              ...
              >
                {results.meta.map((col, i) => (
                  ...
                  >
                    {col.name}
                    <span style={{ color: mutedColor, fontWeight: 400, marginLeft: "4px", fontSize: "10px" }}>{col.type}</span>
                  </div>
                ))}
