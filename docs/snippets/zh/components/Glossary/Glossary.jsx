    ...
    <div className="not-prose glossary-browser">
      <div className="glossary-search-row">
        ...
        <div className="glossary-search-wrap">
          ...
          <input id="glossary-search" type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索术语和定义……" autoComplete="off" />
        </div>
        <span className="glossary-count" aria-live="polite">
          {visibleEntries.length} {visibleEntries.length === 1 ? "个术语" : "个术语"}
        </span>
      </div>
