...
| `aws_role_arn`          | ARN of the IAM role to assume for AWS/Glue access. When set, ClickHouse uses AWS STS `AssumeRole` instead of static access keys. The base credentials that call `AssumeRole` come from `aws_access_key_id` / `aws_secret_access_key` if provided, or from the default AWS credential chain (instance profile, environment, etc.) otherwise. |
