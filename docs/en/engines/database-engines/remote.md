...
When the address expression describes several shards, each proxy table queries all of them, so a table is exposed only when every shard has it: a table missing on some shard is reported as missing, because the proxy could not serve it.
