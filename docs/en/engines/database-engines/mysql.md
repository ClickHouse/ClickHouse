...
The conversion of spatial types (other than `POINT`, which is always converted) is controlled by the `geometry` flag of the [`mysql_datatypes_support_level`](../../operations/settings/settings.md#mysql_datatypes_support_level) setting, enabled by default. `MULTIPOINT` and `GEOMETRYCOLLECTION` have no ClickHouse counterpart and, like all other MySQL data types, are converted into [String](../../sql-reference/data-types/string.md).
