...
The [s3_base](/operations/settings/settings.md#s3_base) setting allows using a relative URL in the `S3` engine. When `s3_base` is set and the path has no scheme, it is resolved against the base URL per [RFC 3986](https://datatracker.ietf.org/doc/html/rfc3986). For a full description of the resolution rules, see the [s3 table function docs](../../../sql-reference/table-functions/s3.md#resolving-relative-urls). The resolved URL is materialized into the stored table definition, so the table does not depend on the value of `s3_base` after creation.
...
- [s3_base](/operations/settings/settings.md#s3_base) - base URL for resolving relative URLs passed to the engine. Empty (disabled) by default.
