...
- `aws_access_key_id`, `aws_secret_access_key` - Long-term credentials for the [AWS](https://aws.amazon.com/) account user. You can use these to authenticate your requests. Parameter is optional. If credentials are not specified, the engine does not inherit top-level `<s3>` credentials from the server configuration. Use `NOSIGN` for public buckets, pass explicit credentials, or configure operator-managed endpoint-scoped `<s3>` credentials that match the URL. For `MergeTree` storage policies backed by `S3` disks, see [Using S3 for Data Storage](../mergetree-family/mergetree.md#table_engine-mergetree-s3).
