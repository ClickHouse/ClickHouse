...
For `ordered` mode without partitioning, ClickHouse may use S3 `ListObjectsV2` with `StartAfter` to avoid re-listing the full prefix history. In bucketed ordered mode, the starting point is conservatively chosen as the smallest processed key across all buckets to avoid skipping unprocessed files.
The StartAfter optimization is used only for S3-backed queues in ordered mode without partitioning (not for AzureQueue or when `partitioning_mode` is set).
