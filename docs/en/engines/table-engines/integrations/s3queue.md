...
For `ordered` mode without partitioning, ClickHouse may resume S3 ListObjectsV2 listing from the last processed key to avoid re-listing the full prefix history. In bucketed ordered mode, the resume point is conservatively chosen as the smallest processed key across all buckets to avoid skipping unprocessed files.
