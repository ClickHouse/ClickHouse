...
Creating a new table is experimental and requires both `allow_experimental_delta_kernel_rs = 1` and `allow_experimental_delta_lake_writes = 1`. If the location already contains a `_delta_log`, the statement attaches to the existing table instead: for a plain (non-catalog) `DeltaLake` table no settings are required, while onboarding an existing table into a `DataLakeCatalog` (Unity) database still requires `allow_experimental_delta_kernel_rs = 1` (its schema is read through the kernel to register it). With an explicit column list the declared columns are attached as-is and adapted to the data on read. When creating a new table the `PARTITION BY`, `ORDER BY`, `PRIMARY KEY` and `SAMPLE BY` clauses are not yet supported. Column types with no compatible Delta type are rejected (for example `UInt64`, `Decimal` with precision above 38, and `LowCardinality`); other types are stored as the closest Delta type, so a column may be read back as a different but compatible ClickHouse type (for example `UInt8` as `Int16`, `Date` as `Date32`, and `DateTime` as `DateTime64(6)`).
