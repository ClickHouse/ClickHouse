...
It is forbidden to perform SELECT queries from AzureQueue. This is a common approach for all widespread queues: you read from a queue just once, and when you read the data from the queue, the data is removed from it. So SELECT is forbidden in order not to lose the data in a queue.
