...
- `background_compaction_target_segment_bytes` — Maximum engine-accounted size of the live rows selected for one background small-segment compaction, and the size at which an individual segment stops being a candidate. The default is 64 MiB.
- `background_compaction_min_segment_count` — Minimum number of small live segments combined by one background compaction. The default is `8`; accepted values range from `2` to `64`.
- `background_compaction_min_interval_ms` — Minimum time between two background small-segment compaction passes. The default is `1000`. A pass publishes a generation and takes an exclusive row lock for every row it moves, which contends with lookups resolving those rows, so consecutive passes are paced rather than run back to back. A backlog still drains, one group of segments per interval. Set it to `0` to remove the pacing.
