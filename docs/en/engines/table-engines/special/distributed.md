...
| `skip_unavailable_shards`                  | If true, ClickHouse silently skips unavailable shards. The behavior is controlled by `skip_unavailable_shards_mode`. Shard is marked as unavailable when: 1) The shard cannot be reached due to a connection failure. 2) Shard is unresolvable through DNS. 3) Table does not exist on the shard.   | `false`       |
| `skip_unavailable_shards_mode`             | Controls what exceptions are ignored when `skip_unavailable_shards` is enabled. `unavailable` ignores connection-related exceptions; `unavailable_or_exception` ignores all exceptions from remote instances.                          | `unavailable` |
