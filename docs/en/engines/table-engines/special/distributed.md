    ...
    <logs>
        ...
        <shard>
            ...
            <replica>
                ...
                <port>9000</port>
                <!-- Optional. Exclude this replica from ON CLUSTER DDL queries. Default: false. -->
...
The parameters `host`, `port`, and optionally `user`, `password`, `secure`, `compression`, `bind_host`, `skip_distributed_ddl` are specified for each server:
...
| `skip_distributed_ddl` | If `true`, this replica is excluded from `ON CLUSTER` DDL queries (such as `CREATE`, `DROP`, `ALTER`, etc.) while still being used normally for `Distributed` table query routing and data reads/writes. Useful when multiple replicas in a cluster share the same underlying host (e.g. `localhost`) and you want to avoid duplicate DDL execution on that host. | `false`      |
