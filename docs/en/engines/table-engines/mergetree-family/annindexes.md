...
With setting `vector_search_with_rescoring = 1`, ClickHouse uses these exact row positions as a filter, reads the original full-precision vectors for the candidate rows, and computes the final distance in the regular SQL pipeline.
...
For queries with `vector_search_with_rescoring = 1`, ClickHouse applies these row positions as an exact filter before computing the final distance from the stored vectors.
...
With `vector_search_with_rescoring = 1`, ClickHouse then reads the matching row positions and computes the exact distance for those rows.
