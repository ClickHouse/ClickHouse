...
- `asciiCJK_v2` uses [StringZilla](https://github.com/ashvardanian/StringZilla)'s UTF-8 implementation of Unicode word segmentation (UAX #29). It keeps connected Unicode word runs together while emitting individual CJK ideographs.
...
| [hasPhrase](/sql-reference/functions/string-search-functions.md/#hasPhrase)                 | yes | `splitByNonAlpha`, `splitByString`, `ngrams`, `asciiCJK`, `asciiCJK_v2`, `icu` | yes |
...
Note that the function requires one of the `splitByNonAlpha`, `splitByString`, `ngrams`, `asciiCJK`, `asciiCJK_v2`, or `icu` tokenizers.
...
`hasPhrase` can be used together with tokenizers `splitByNonAlpha`, `splitByString`, `ngrams`, `asciiCJK`, `asciiCJK_v2`, and `icu`.
