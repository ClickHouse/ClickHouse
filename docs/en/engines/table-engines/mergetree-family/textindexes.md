...
The setting is a preference, not a hard constraint: every index is written in the preferred format if that format can represent it, and in the oldest format that can otherwise.
An index with `support_phrase_search` is always written in `v2_with_positions` (older formats cannot represent token positions), and an index with a posting list codec is written at least in `v1_with_codec` (the `v0_initial` format does not persist the codec type).
An index without positions is written in the preferred format as well, so with the default setting every text index uses `v2_with_positions` and its header records that the index has no positions.
As a consequence, writing a text index never fails because of this setting, and readers always take the format version from the on-disk header of each part.
...
Explicit contradictions are still rejected at creation time (`CREATE TABLE` or `ALTER TABLE ... ADD INDEX`): an index with `support_phrase_search` cannot be created while `text_index_serialization_version` is set below `v2_with_positions`, and an index with a `posting_list_codec` argument other than `none` cannot be created while it is `v0_initial`; the table settings combination `text_index_serialization_version = 'v0_initial'` with a `text_index_posting_list_codec` other than `none` is likewise rejected on `CREATE TABLE` and `ALTER TABLE`.
