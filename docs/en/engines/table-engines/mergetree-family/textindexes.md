...
Caching tokens that are absent from a data part is disabled by default and can be enabled independently with [`use_text_index_negative_tokens_cache`](/operations/settings/settings#use_text_index_negative_tokens_cache).
