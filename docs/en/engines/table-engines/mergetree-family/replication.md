...
`ReplicatedMergeTree` engine uses a separate thread pool for replicated fetches. Size of the pool limited by setting [background_fetches_pool_size](../../../operations/settings/settings.md#background_fetches_pool_size) setting which can be tuned with a server restart. 
