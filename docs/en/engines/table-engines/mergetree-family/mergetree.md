...
The optional `false_positive_rate` is a floating-point value strictly between `0` and `1` (default `0.025` when the argument is omitted). The value is quantized to the fingerprint widths supported by the implementation, currently 8-bit or 16-bit fingerprints. Therefore nearby values can produce the same encoded filter, and smaller requested values only reduce false positives or increase index size when they cross a fingerprint-width boundary. The value is **not an upper bound**: rounding to the nearest width means the effective false positive rate can be above or below the requested value. Requested rates of roughly `0.002` and above select 8-bit fingerprints with an effective false positive rate of about `0.031`; smaller requested rates select 16-bit fingerprints with an effective rate of about `0.00012`. For example, `cuckoo_filter(0.01)` selects 8-bit fingerprints, so its effective false positive rate is about `0.031` — looser than requested.
...
The optional `false_positive_rate` is a floating-point value strictly between `0` and `1` (default `0.025` when the argument is omitted). The value is quantized to the fingerprint widths supported by the implementation, currently 8-bit or 16-bit fingerprints. Therefore nearby values can produce the same encoded filter, and smaller requested values only reduce false positives or increase index size when they cross a fingerprint-width boundary. The value is **not an upper bound**: rounding to the nearest width means the effective false positive rate can be above or below the requested value. Requested rates of roughly `0.00024` and above select 8-bit fingerprints with an effective false positive rate of about `0.0039`; smaller requested rates select 16-bit fingerprints with an effective rate of about `0.000015`. For example, `binary_fuse_filter(0.001)` selects 8-bit fingerprints, so its effective false positive rate is about `0.0039` — looser than requested. Internally the granule is encoded as one of `Empty`, `Single`, `SmallList` (small/exact set), or a `Fuse3`/`Fuse4` filter, depending on the number of distinct keys; this is transparent to the user.
