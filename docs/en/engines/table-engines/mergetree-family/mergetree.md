...
The key in `PRIMARY KEY` and `ORDER BY` can be `Nullable` type for [MergeTree](../../../engines/table-engines/mergetree-family/mergetree.md#table_engines-mergetree) family. The setting [allow_nullable_key](../../../operations/settings/settings.md#allow-nullable-key) is responsible for supporting this feature.
