...
- Leader: inserts, `INSERT`-driven merges, mutations, `DROP`/`DETACH`/`ATTACH`/`MOVE`/`REPLACE PARTITION`, `OPTIMIZE` all run normally. Background merges, mutations, moves, and cleanup are active. When leadership is acquired, the leader refreshes its in-memory view of parts from shared storage and advances the local block-number counter past anything the previous leader wrote. The latter uses both the disk's local metadata cache and a direct `listObjects` probe of the object storage prefix, so block-number safety is preserved on shared-key metadata storage policies such as `s3_plain_rewritable` even when the local metadata cache has not yet been populated. On the default `s3` disk (random per-replica object keys), block-number collisions cannot occur at the storage level because each replica writes to its own opaque keys.
