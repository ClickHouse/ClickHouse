...
If you need the data to be stored in the order of inserting instead, disable the reordering with `optimize_row_order_if_no_order_by = 0` (or `optimize_row_order = 0`) in the table's `SETTINGS` clause, and, when inserting data by `INSERT ... SELECT` queries, also set [max_insert_threads = 1](/operations/settings/settings#max_insert_threads).
