...
-   `SETTINGS` — Additional parameters that control the behavior of the `MergeTree` (optional):
	...
	-   `max_compress_block_size` — Maximum size of blocks of uncompressed data before compressing for writing to a table. See [max_compress_block_size](../../../operations/settings/settings.md#max-compress-block-size) setting.
	-   `min_compress_block_size` — Minimum size of blocks of uncompressed data required for compression when writing the next mark. See [min_compress_block_size](../../../operations/settings/settings.md#min-compress-block-size) setting.
