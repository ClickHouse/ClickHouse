...
6. Main branch(develop/master) is meant to be synced with upstream. If a patch has to be commited to a fork it should remain on top of main branch. Main branch update process is done via git rebase and allow a) to remove patches that were pushed to upstream and b) to maintain simple list of changes over upstream version.
7. Every time the custom changes have to be made in the library code, a dedicated branch should be created, like `clickhouse/my-fix`. Then this branch should be merged into the main branch in a fork when it is ready.
8. ClickHouse main repository shouls reference only commit in `clickhouse/`branches 
