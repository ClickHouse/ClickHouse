...
Notion of cpu `slot` is introduced. Slot is unit of concurrency: to run a thread query has to acquire a slot in advance and release it when thread stops. Amount of slots is globally limited in a server. Multiple concurrent queries are competing for cpu slots if total demand exceed total amount of slots. `ConcurrencyControl` is responsible to resolve this competition by doing cpu slot scheduling in a fair manner.
...
Note that `allocated` slot can be in two different states: `granted` and `acquired`. The former is transitional state, that actually should be short. From instant when slot is allocated to a query till the moment when autoscaling procedure is run by any thread of a query.
...
1. Create an allocation for a query: `auto slots = ConcurrencyControl::instance().allocate(1, max_threads);`. It will allocate at least 1 and at most `max_threads` slots. Note that the first slot is granted immediately, but remaining slots may be granted later. Thus limit is soft, because every query will obtain at least one thread.
