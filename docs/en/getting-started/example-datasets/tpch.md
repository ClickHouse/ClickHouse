...
Then, generate the data. Parameter `-s` specifies the scale factor. For example, with `-s 100`, 600 million rows are generated for LINEITEM.
...
``` bash
declare -a tables=("nation" "region" "part" "supplier" "partsupp" "customers" "orders" "lineitem")
