...
- Equality and `$in` compare the value a field holds, so they do not match an element of an array the way MongoDB does - `{"tags": "red"}` matches a document whose `tags` **is** `"red"`, not one whose `tags` holds it. Over a table created in ClickHouse the match is element-wise, as described below. The array functions ClickHouse would need for that do not accept the `Dynamic` value a path of a document is.
