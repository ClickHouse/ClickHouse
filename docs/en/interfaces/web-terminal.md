...
The `user` field is optional: when it is omitted or empty, the user name falls back to the [`default_session_user`](/operations/server-configuration-parameters/settings#default_session_user) server setting (or its per-endpoint override in a [composable protocols](/operations/settings/composable-protocols) configuration), `default` unless configured otherwise. If `default_session_user` is set to an empty string, connections without a user name are prohibited: an `auth` message with an omitted or empty `user` fails authentication, the server closes the WebSocket with code `1008`, and the reject is recorded in [`system.session_log`](/operations/system-tables/session_log) as a `LoginFailure` event with an empty `user`.
