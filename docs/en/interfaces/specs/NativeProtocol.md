...
| 4  | initial_address              | String  | universal    | always                                 | Originating client socket address. Accepted grammar: IPv4 `a.b.c.d:port` or bracketed IPv6 `[addr]:port`. The host must be an IP literal and the port a decimal number in `0..65535`; hostnames and service names are not resolved. Other forms (for example `localhost:9000`, `host:http`, `:9000`, or a UNIX socket path such as `/tmp/ch.sock`) are rejected with `INCORRECT_DATA`. |
