...
| 4  | initial_address              | String  | universal    | always                                 | Originating client socket address. Accepted grammar: IPv4 `a.b.c.d:port`, bracketed IPv6 `[addr]:port`, or (on UNIX) a local socket path starting with `/`. The host must be an IP literal and the port a decimal number in `0..65535`; hostnames and service names are not resolved. Other forms (for example `localhost:9000`, `host:http`, `:9000`) are rejected with `INCORRECT_DATA`. |
