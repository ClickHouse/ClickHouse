...
| QUERY_PLAN_SERIALIZATION        | 54477   | ServerHello, QueryPlan packet | ServerHello appends `VarUInt query_plan_serialization_version` after server settings. Also introduces `ClientPacket::QueryPlan` (code `13`) for inter-server delivery of pre-built query plans — external clients never send. The serialized plan body itself begins with its own `VarUInt` version; each side serializes at `min` of the two advertised versions. **Version `1`** is the original plan encoding. **Version `2`** lets serialized join steps carry the `max_memory_usage`, `enable_join_in_memory_compression`, and `join_decompressed_columns_cache_bytes` settings inside their changed-settings list; when the negotiated version is `1` these three names are omitted entirely, because a version-`1` receiver does not know them and rejects unknown setting names. Current value: `2` (`DBMS_QUERY_PLAN_SERIALIZATION_VERSION`). |
...
| VERSIONED_CLUSTER_FUNCTION_PROTOCOL | 54479 | ServerHello           | Adds `VarUInt cluster_function_protocol_version` at the tail of ServerHello. Used for `*Cluster` table functions (`s3Cluster`, etc.). Current value: `7` (`DBMS_CLUSTER_PROCESSING_PROTOCOL_VERSION`); `7` adds an optional `read_source_index` to the inter-server cluster read-task payload (the `ReadTaskResponse` body, which stays unspecified here — see below). External clients decode and ignore. |
...
| 13 | query_plan_serialization_version | VarUInt | universal | QUERY_PLAN_SERIALIZATION (v54477) | Server's supported query-plan serialization version. Current value: `2` (version `1` is the original encoding; version `2` additionally lets serialized join steps carry `max_memory_usage`, `enable_join_in_memory_compression`, and `join_decompressed_columns_cache_bytes`, omitting those three names for a version-`1` peer). The two ends serialize at the `min` of the values they advertise. External clients decode and ignore. |
