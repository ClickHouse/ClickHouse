...
| STRING_WITH_SIZE_STREAM_SERIALIZATION | 54489 | Block (Column)         | `String` columns switch from the per-value length-prefixed layout to a separate stream of cumulative byte offsets (the same offsets layout an `Array` uses), sent as-is: `[UInt64 × num_rows offsets][data blob]`. Gated on the negotiated revision; below 54489 the writer uses the per-value length prefixes. The wire carries no per-column marker for this — both peers derive it from the revision alone. See [String column layout](/interfaces/specs/NativeFormat#string-type). (In the `Native`/`Buffers` *format* this layout is not revision-gated; it is opt-in through `output_format_native_write_string_with_size_stream` / `input_format_native_read_string_with_size_stream`.) |
