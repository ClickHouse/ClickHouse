...
| QUERY_PLAN_SERIALIZATION        | 54477   | ServerHello, QueryPlan packet | ServerHello appends `VarUInt query_plan_serialization_version` after server settings. Also introduces `ClientPacket::QueryPlan` (code `13`) for inter-server delivery of pre-built query plans — external clients never send. The serialized plan body itself begins with its own `VarUInt` version; each side serializes at `min` of the two advertised versions. **Version `1`** is the original plan encoding. **Version `2`** serializes a bucketed `ReadFromMergeTree` leaf read as just its bucket count (the per-bucket marks travel in the `read_bucket` task parameter). **Version `3`** adds the parallel-replicas flag (bit `32`) on a serialized `ReadFromMergeTree`, telling the receiving replica to rebuild the read in parallel-reading mode (an older replica would ignore the bit and do a full non-parallel read, so the serializer fails closed when this flag is set below version `3`). **Version `4`** adds `WindowStep` to the set of serializable steps (a receiver below version `4` does not register a `Window` step at all, so the serializer fails closed instead). **Version `5`** lets serialized join steps carry the `max_memory_usage` and `enable_join_in_memory_compression` settings inside their changed-settings list; when the negotiated version is below `5` these two names are omitted entirely, because such a receiver does not know them and rejects unknown setting names (a version-`5` receiver reading a stream that omits `max_memory_usage` restores it from the query settings, which travel with the query; a join step whose `max_memory_usage` was overridden by a subquery-local `SETTINGS` clause cannot be restored that way, so its plan requires version `5` and the serializer fails closed below it). Current value: `5` (`DBMS_QUERY_PLAN_SERIALIZATION_VERSION`). |
...
| 13 | query_plan_serialization_version | VarUInt | universal | QUERY_PLAN_SERIALIZATION (v54477) | Server's supported query-plan serialization version. Current value: `5` (version `1` is the original encoding; version `2` serializes a bucketed `ReadFromMergeTree` leaf read as just its bucket count; version `3` adds the parallel-replicas flag (bit `32`) on a serialized `ReadFromMergeTree`; version `4` adds `WindowStep` to the serializable steps; version `5` additionally lets serialized join steps carry `max_memory_usage` and `enable_join_in_memory_compression`, omitting those two names for a peer below version `5`). The two ends serialize at the `min` of the values they advertise. External clients decode and ignore. |
