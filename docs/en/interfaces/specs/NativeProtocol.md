...
| INTERSERVER_SECRET_TABLES_STATUS | 54487  | TablesStatusRequest    | Inter-server only: the client appends an auth-hash `String` (a 32-byte SHA-256 digest) right after the `TablesStatusRequest` body, computed over `salt + nonce + cluster_secret + "TablesStatusRequest"` — the same salt and nonce exchanged during the handshake as for the per-query `auth_hash`. The server validates it before serving the response; `TablesStatusRequest` is sent during connection establishment, before any Query authenticates the connection, so the request carries its own proof. A peer below 54487 sends no hash and is rejected only when the `interserver_tables_status_require_auth` server setting is enabled. External clients never enter inter-server mode and see no wire change. See [Inter-server authentication](#inter-server-authentication). |
...
**TablesStatusRequest authentication (v54487+).** `TablesStatusRequest` is sent during connection establishment — before any Query packet carries an `auth_hash` — so on its own it would let an unauthenticated peer read table existence, readonly and replication-delay status from the inter-server port. At negotiated version ≥ 54487 (`INTERSERVER_SECRET_TABLES_STATUS`), an inter-server client authenticates the request itself: right after the `TablesStatusRequest` body it writes a `String` holding the 32-byte digest `encodeSHA256(salt + nonce + cluster_secret + "TablesStatusRequest")` — the same `salt` and `nonce` as the per-query hash (with `nonce` in its decimal string form, present only when `INTERSERVER_SECRET_V2` (v54462) is negotiated), with the literal string `"TablesStatusRequest"` in place of the query-specific fields. The server reads the hash with a 32-byte cap, recomputes it, and rejects the request on mismatch (or when it has no salt or no cluster secret for the named cluster). A peer below 54487 sends no hash; the server rejects such requests only when the `interserver_tables_status_require_auth` server setting is enabled and the connection was not already authenticated by an earlier Query (the setting is disabled by default so rolling upgrades keep working; operators enable it once the whole cluster is upgraded). External clients never enter inter-server mode and are unaffected.
...
| 5    | TablesStatusRequest       | not specified       | Table status check. In inter-server mode at v54487+, the body is followed by an auth-hash `String` — see [Inter-server authentication](#inter-server-authentication). |
