...
| 6 | user             | String  | universal | Username for authentication. Empty means the server's default session user (the `default_session_user` server setting; rejected by servers older than 26.7). See [Authentication](#authentication). |
