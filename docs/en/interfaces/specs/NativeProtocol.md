...
| 13 | query_plan_serialization_version | VarUInt | universal | QUERY_PLAN_SERIALIZATION (v54477) | Server's supported query-plan serialization version. Current value: `4` (version `1` is the original encoding; version `2` serializes a bucketed `ReadFromMergeTree` leaf read as just its bucket count; version `3` adds the parallel-replicas flag (bit `32`) on a serialized `ReadFromMergeTree`; version `4` additionally lets serialized join steps carry `max_memory_usage` and `enable_join_in_memory_compression`, omitting those two names for a peer below version `4`). The two ends serialize at the `min` of the values they advertise. External clients decode and ignore. |
