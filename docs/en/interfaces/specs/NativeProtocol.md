...
| JSON_TYPED_PATHS_SERIALIZATION  | 54489   | Block (Column)         | Extends a `JSON` column's recursive `kind_stack` with one entry for each declared typed path, ordered lexicographically by full path name. This permits sparse serialization of declared paths. Below this version the writer materializes those paths dense and omits their entries. See [kind_stack and sparse encoding](/interfaces/specs/NativeFormat#kind-stack-and-sparse-encoding). |
