...
| QUERY_PLAN_SERIALIZATION        | 54477   | ServerHello, QueryPlan packet | ServerHello appends `VarUInt query_plan_serialization_version` after server settings. Also introduces `ClientPacket::QueryPlan` (code `13`) for inter-server delivery of pre-built query plans — external clients never send. The serialized plan body itself begins with its own `VarUInt` version; each side serializes at `min` of the two advertised versions. **Version `1`** is the original plan encoding. **Version `2`** serializes a bucketed `ReadFromMergeTree` leaf read as just its bucket count (the per-bucket marks travel in the `read_bucket` task parameter). **Version `3`** lets serialized join steps carry the `max_memory_usage`, `enable_join_in_memory_compression`, and `join_decompressed_columns_cache_bytes` settings inside their changed-settings list; when the negotiated version is below `3` these three names are omitted entirely, because such a receiver does not know them and rejects unknown setting names. Current value: `3` (`DBMS_QUERY_PLAN_SERIALIZATION_VERSION`). |
...
| 13 | query_plan_serialization_version | VarUInt | universal | QUERY_PLAN_SERIALIZATION (v54477) | Server's supported query-plan serialization version. Current value: `3` (version `1` is the original encoding; version `2` serializes a bucketed `ReadFromMergeTree` leaf read as just its bucket count; version `3` additionally lets serialized join steps carry `max_memory_usage`, `enable_join_in_memory_compression`, and `join_decompressed_columns_cache_bytes`, omitting those three names for a peer below version `3`). The two ends serialize at the `min` of the values they advertise. External clients decode and ignore. |
