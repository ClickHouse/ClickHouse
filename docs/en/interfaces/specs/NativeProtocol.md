...
| # | Field | Type    | Role   | Description |
|---|-------|---------|--------|-------------|
| 1 | count | VarUInt | client | Number of parameter pairs that follow. |
| 2 | name  | String  | client | Parameter name (repeated `count` times, interleaved with `value`). |
| 3 | value | String  | client | Parameter value as string (repeated `count` times). See the note below on quoting. |
