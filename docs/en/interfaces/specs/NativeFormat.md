...
At or above the threshold the size-stream layout applies to every `String` in the block, including `String` nested inside composite types (`Array(String)`, `Nullable(String)`, `Map`, `Tuple`, `Variant`, `Dynamic`, `JSON`) — with two exceptions, both of which keep the per-value layout at every revision:

- the dictionary of a [`LowCardinality(String)`](#lowcardinality) column;
- the `String` alternatives of a `Dynamic`/`JSON` column written in the flattened representation (`output_format_native_use_flattened_dynamic_and_json_serialization = 1`). Only the default (non-flattened) `Dynamic`/`JSON` serialization carries `String` subcolumns in the size-stream layout.
