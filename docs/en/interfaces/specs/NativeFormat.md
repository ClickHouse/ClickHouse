...
| Aggregate-function versioning | `DBMS_MIN_REVISION_WITH_AGGREGATE_FUNCTIONS_VERSIONING` | `54452` | The `AggregateFunction` *state* is written at version `0`, and no version is negotiated from the revision. An ordinary leaf's *type string* omits the version, because version `0` is never printed, so a decoder derives it from the revision rather than from the string. A leaf nested inside a [`SimpleAggregateFunction`](#type-aliases) is a special case, in the same shape as `LowCardinality` above: the version is spelled out as an explicit `AggregateFunction(0, ...)` only when the revision is exactly `0`, because that is the local persister, which rebuilds the leaf by parsing back its own printed name. At a *non-zero* revision below `54452` the peer predates versioning entirely and has no grammar for the version token, so the leaf is announced without one, which is also what such a peer writes itself. |
...
That makes revision `0` the most conservative encoding for almost everything: the stream carries no `BlockInfo`, no `has_custom_serialization` byte, V1 `Dynamic`/`JSON`, version-`0` aggregate-function state, and a bare `DateTime` with the timezone parameter dropped.
...
:::note
The *textual* header announces a versioned `AggregateFunction` leaf so that the announced `T` re-parses to exactly the version its payload was written at. That re-parse equivalence is the guarantee a decoder can rely on, and the writer enforces it whenever the leaf is reachable from `T` by descending zero or more supported containers. In other words the leaf qualifies when it **is** `T` itself, which is the plain `SimpleAggregateFunction(anyLast, AggregateFunction(sumMap, ...))` shape, and equally when it sits under any nesting of the supported containers. The supported containers are `Nullable`, `Array`, `Tuple` and `Map`, which are exactly the ones the version walker `setVersionToAggregateFunctions` descends, and the writer's type-name renderer mirrors that same set on purpose. `Nested` qualifies as well, because its storage shape *is* `Array(Tuple(...))` and so is already made of descended containers; the renderer additionally preserves the `Nested(...)` spelling instead of degrading it to `Array(Tuple(...))`. A numeric version token is emitted when, and only when, it is needed for that equivalence, so a leaf whose versionless spelling already re-parses to the negotiated version is announced without one at any revision: a leaf declared `AggregateFunction(0, sumMap, ...)` *inside* a `SimpleAggregateFunction` is announced versionless at revision `54452` and above, because version `0` is never printed and the versionless spelling therefore re-parses to the function's default, which is the version negotiated there. Its payload is the negotiated version's payload, byte-identical to the one the versionless and explicit-`1` declarations produce at that revision, so the announced name and the payload agree and this is not an inconsistency. The explicit `0` reaches the writer already dropped in that position, because the alias rebuilds its value type by parsing back the printed name. A leaf outside a `SimpleAggregateFunction` keeps its explicit `0` instead, and is not covered by this guarantee. A token is withheld for a second, independent reason at a *non-zero* revision below `54452`: that peer predates versioning and cannot parse the token at all (see [what the revision gates](#what-the-revision-gates)). The version is negotiated per peer, so a decoder must take it from the announced `T` rather than from the version a `CREATE TABLE` declared.

Any other type along that path is opaque to the walker. A leaf below one is never assigned a negotiated version, so the announced spelling keeps whatever the declaration carried, and because version `0` is never printed a decoder that re-parses that name sees the function's default there. This is the case below another `AggregateFunction` (its `argument_types` are never visited), below a [`Variant`](#variant), and below a [`Dynamic`](#dynamic), among others. It is not an inconsistency in the stream: no negotiated version is assigned to either side, so the version the announced name re-parses to and the one the payload is serialized at are the same, and the round trip is lossless. That is a statement about the two versions agreeing, not about one type object: under a `SimpleAggregateFunction` the name and the payload come from the cached and storage trees respectively, which is exactly why they can disagree once a version *is* assigned. Such a position simply carries no peer-negotiated version to read.

The versioned base implementations are the map family (`sumMap`, `minMap`, `maxMap`, `sumMapWithOverflow`, `sumMapFiltered`, `sumMapFilteredWithOverflow`, `sumMappedArrays`, `minMappedArrays`, `maxMappedArrays`) and `groupBitmapAnd`, `groupBitmapOr`, `groupBitmapXor`; combinators propagate versioning, so a derived name such as `sumMapIf` is versioned too. A combinator that also forwards the revision lookup, such as `If`, inherits its nested function's threshold, while one that does not, such as `Merge` or `OrNull`, reports a versioned function that negotiates version `0` at every revision. Each decides its own version from the revision, and the thresholds differ (`54452` for the map family, `54455` for the bitmap ones), so plain `groupBitmap` is not versioned at all and no single revision marks the boundary. Outside a `SimpleAggregateFunction` version `0` is never printed, and an omitted version there does not mean the function's default: a decoder must derive it from the revision, exactly as `NativeReader` does.

One case does not satisfy that rule today, and a decoder cannot repair it from the stream: a leaf declared with an explicit `AggregateFunction(0, ...)` anywhere outside a `SimpleAggregateFunction`, whether it is the column type itself or sits under any nesting of the supported containers. The declared `0` is a set version, so the revision-driven assignment leaves it alone, and the payload is written at version `0` while the type string omits the token. That only diverges once the revision-derived version is non-zero, which is each function's own threshold and above (`54452` for the map family, `54455` for the bitmap one); at revision `0` and below the threshold a decoder derives `0` too, so the stream is consistent there and byte-identical to the versionless declaration. Above it the decoder reads version-`0` bytes with the negotiated layout. Whether that is observable depends on the value type: version `1` of the map family serializes promoted values, so a type that promotion widens, such as `Decimal32`, makes the two layouts differ and decoding either fails or reconstructs wrong values, while a type it leaves alone, such as `UInt64`, keeps them byte-compatible and decodes successfully despite the version being unavailable. This is independent of the alias above: it needs the version to be re-negotiated for the wire, or to be spelled out, at the layer that owns it.

A `SimpleAggregateFunction` whose value type is itself a `SimpleAggregateFunction` is also outside the guarantee. Each alias rebuilds its value type by parsing back the printed name, so with two of them nested the leaf's version passes through that name twice: the first pass leaves it unset, which then reads as the function's default, and the second spells that default out. By the time the header is rendered both trees already agree on it, so the announced version is the default rather than the one the payload was written at.

The **binary** type encoding does not yet carry the negotiated version for this shape: it encodes the nested `AggregateFunction`'s version from the type as declared, and its decoder reconstructs the value type by re-parsing the printed name, which drops an explicit `0` again. A decoder that needs a version-accurate `SimpleAggregateFunction` value type should therefore use the textual header (`output_format_native_encode_types_in_binary_format = 0`, the default).
:::
