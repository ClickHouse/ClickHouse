...
- the typed query parameter block appended to a `Query` packet at protocol revision `54489` and above;
...
| Typed query parameter block | N > 0 | 1 | One query parameter per named column; carried directly in a `Query` packet at revision `54489` and above. |

An empty typed query parameter set uses the ordinary empty-block representation. Unlike a `Data` packet, a typed parameter block has no `table_name` prefix and is not an end-of-input sentinel; its position in the `Query` packet determines its meaning. See [Typed query parameter block](/interfaces/specs/NativeProtocol#typed-query-parameter-block).
...
| Typed query parameter block | `DBMS_MIN_PROTOCOL_VERSION_WITH_TYPED_QUERY_PARAMETERS` | `54489` | No Native block follows the text parameter list in a `Query` packet; clients must use text parameters. |
