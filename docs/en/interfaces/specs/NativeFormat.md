...
| `MacAddress`        | 8               | 48-bit MAC address in the low 6 bytes; the upper 2 bytes are always zero | Little-endian UInt64 (see [MacAddress](#macaddress)) |
...
`MacAddress` is 8 bytes on the wire: a little-endian UInt64 whose low 48 bits hold the six octets in network order (the first octet is the most significant of the 48), and whose upper 16 bits are always zero.

`00:1A:2B:3C:4D:5E` (48-bit value `0x001A2B3C4D5E`):

```text
5E 4D 3C 2B 1A 00 00 00  Little-endian UInt64
```

A decoder must mask off the upper 16 bits when reading: writers always zero them, but the mask is what keeps the value canonical, and comparison and hashing are defined on the 48-bit value only. Note that the type occupies 8 bytes even though a MAC address is 6 bytes wide.
