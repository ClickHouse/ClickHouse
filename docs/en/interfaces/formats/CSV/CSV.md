...
- `Date`, `Date32`, `DateTime`, `DateTime64`, `Time`, and `Time64` values are enclosed in double quotes by default. This can be disabled with the setting [output_format_csv_quote_date_time_types](/operations/settings/settings-formats.md/#output_format_csv_quote_date_time_types). `Time` and `Time64` values remain quoted if the configured delimiter can occur in their output.
...
| [output_format_csv_quote_date_time_types](/operations/settings/settings-formats.md/#output_format_csv_quote_date_time_types)                               | if it is set to false, date and time values are written without surrounding double quotes in CSV output. `Time` and `Time64` remain quoted if the delimiter can occur in their output. | `true`  |                                                                                                                                                                                              |
