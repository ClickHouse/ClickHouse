...
Some valid GeoJSON geometry types &mdash; such as `GeometryCollection` and `MultiPoint` &mdash; are not supported by the format. You can control what happens when such a geometry must be stored in the `geometry` column using the `input_format_geojson_unsupported_geometry_handling` setting. Possible values are:
...
- `GeometryCollection` and `MultiPoint` are not supported — see [Handling unsupported geometry types](#unsupported-geometry).
