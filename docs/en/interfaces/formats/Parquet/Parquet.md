...
| `output_format_parquet_column_field_ids`                                       | Explicit Parquet `field_id` overrides per output column. A `Map(String, Int32)` keyed by column name. Useful for producing files compatible with Apache Iceberg, which identifies columns by `field_id`. Cannot be combined with a datalake-provided column-id mapping. | `{}`        |
| `output_format_parquet_auto_assign_field_ids`                                  | Auto-assign a unique sequential Parquet `field_id` (starting at `1`) to every output column, matching the Apache Iceberg writer convention. Overrides from `output_format_parquet_column_field_ids` win for the columns they cover. | `0`         |
