...
In this format, data is written and read by [blocks](/development/architecture#block) in a binary format. Buffers uses the same per-column binary representation as the [Native](./Native.md) format and respects the same Native format settings, including `client_protocol_version`: a raised protocol version selects the same revision-dependent column encodings as in Native (for example, the offsets `String` layout), and the reader has to be given the same version. The version reaches the query output and the body of an `INSERT ... FORMAT Buffers`, and — as a consequence of it living on the query context — a server-side `INSERT INTO FUNCTION file(..., 'Buffers')` write too; table-function reads of `Buffers` data always parse at revision `0`, so a raised-revision server-side write does not read back. See [the Native format's protocol-revision section](/interfaces/specs/NativeFormat#revision-round-trip) for the full treatment; it applies identically to `Buffers`. Unlike Native, Buffers never uses per-column serialization kinds (such as Sparse), because there is no place for them in the framing.
