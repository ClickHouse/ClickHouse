...
- [input_format_csv_empty_string_is_not_null](/docs/en/operations/settings/settings-formats.md/#input_format_csv_empty_string_is_not_null) - Controls how `Nullable(String)` is read from a missing value in CSV. A missing value is an empty space between/before/after commas, not surrounded by quotes. If this setting is enabled, regardless of the value of `input_format_csv_empty_as_default`, the missing value of `Nullable(String)` will be interpreted as an empty `String`, not as NULL. Default value - `false`.
