...
ClickHouse supports [NULL](../sql-reference/syntax.md), which is displayed as `null` in the JSON output. Set the [output\_format\_json\_quote\_denormals](../operations/settings/settings.md#settings-output_format_json_quote_denormals) to 1, to enables `+nan`, `-nan`, `+inf`, `-inf` values in output.
