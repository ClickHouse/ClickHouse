...
| `-s [ --secure ]`                | Whether to use TLS. <br/><br/>Enabled automatically when connecting to port 9440 (the default secure port) or ClickHouse Cloud, and also when neither `--port` nor `--secure` is specified and the plain port does not answer within 100 milliseconds after the secure port - either because it is unreachable (e.g. `play.clickhouse.com`, where the plain port is firewalled) or because it is merely slower. In this case the plain port is not tried again, so a secure port with an untrusted or misconfigured certificate makes the connection fail instead of downgrading to the plain protocol. <br/><br/>You might need to configure your CA certificates in the [configuration file](#configuration_files). The available configuration settings are the same as for [server-side TLS configuration](../operations/server-configuration-parameters/settings.md#openssl). | Auto-enabled when connecting to port 9440 or ClickHouse Cloud                                                   |
