...
The array form is recognised for the comparison operators `=`, `==`, `!=`, `<>`, `<=>`, `<`, `<=`, `>`, `>=`, and additionally for the keyword comparison predicates `IS DISTINCT FROM` and `IS NOT DISTINCT FROM` (the latter two only for the array form, not the subquery form). Other operators that accept an array on the right — for example `LIKE`, `ILIKE`, `NOT LIKE`, `REGEXP`, and `IN` — are **not** rewritten to the array quantifier and keep their ordinary meaning. For instance, `'abc' LIKE SOME(['a%', 'b%'])` is not the array quantifier syntax; use `arrayExists` / `arrayAll` directly for those cases.
