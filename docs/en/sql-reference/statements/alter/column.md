...
For tables that do not store data themselves (such as [Merge](/docs/en/sql-reference/statements/alter/overview.md) and [Distributed](/docs/en/sql-reference/statements/alter/overview.md)), `ALTER` just changes the table structure, and does not change the structure of subordinate tables. For example, when running ALTER for a `Distributed` table, you will also need to run `ALTER` for the tables on all remote servers.
