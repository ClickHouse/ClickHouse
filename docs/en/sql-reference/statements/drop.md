...
Deletes a SQL cluster catalog endpoint. Requires [`cluster_metadata`](/operations/sql-clusters#configuration). The endpoint must not be referenced by any SQL shard.
...
```sql
DROP ENDPOINT IF EXISTS replica_01;
```

## DROP SHARD {#drop-shard}

Deletes a SQL cluster catalog shard. Requires [`cluster_metadata`](/operations/sql-clusters#configuration). The shard must not be referenced by any SQL cluster.
...
```sql
DROP SHARD IF EXISTS shard_01;
```

## DROP CLUSTER {#drop-cluster}

Deletes a SQL-managed cluster definition. Requires [`cluster_metadata`](/operations/sql-clusters#configuration).
...
```sql
DROP CLUSTER IF EXISTS analytics;
```
