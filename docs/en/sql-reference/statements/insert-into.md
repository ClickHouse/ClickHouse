...
- `parallel_distributed_insert_select = 1` is not supported with `INSERT ... RETURNING` and is rejected with `NOT_IMPLEMENTED`, because `RETURNING` runs once on the initiator after the insert phase and cannot be executed on each shard's remote `INSERT`.
