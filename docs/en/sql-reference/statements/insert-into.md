...
- Query-global resource and execution limits cannot be set in the `RETURNING` subquery's `SETTINGS`. `max_memory_usage`, `max_memory_usage_for_user`, `max_execution_time`, `timeout_overflow_mode`, `max_temporary_data_on_disk_size_for_query`, `max_temporary_data_on_disk_size_for_user`, `max_network_bandwidth_for_user` and `max_network_bandwidth_for_all_users` are rejected with `NOT_IMPLEMENTED`, because the `INSERT` and `RETURNING` phases share a single query whose memory tracker, time limit, temporary-data-on-disk scope and network throttlers are established once from the outer settings. Result-shaping settings such as `max_result_rows`, `max_result_bytes` and `result_overflow_mode` are applied to the subquery result.
