...
For `INSERT ... RETURNING`, source-side `SETTINGS` that bind query-global state at query registration are rejected with `NOT_IMPLEMENTED` (the same rejection class as for unsupported `RETURNING` subquery settings). This includes settings such as `max_execution_time`, memory/admission limits, throttling settings, and `profile`.
