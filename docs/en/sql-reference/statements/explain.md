...
- Role grants of the form `GRANT some_role TO user` are accepted by the parser but produce no rows, because granted roles are not stored in `system.grants` (use `system.role_grants` to introspect them).
