...
- `single_record` – Return the reformatted query as a single multi-line record instead of one record per line. Default: `1` (controlled by the `explain_syntax_single_record` setting). Set to `0` to restore the historical one-record-per-line output, or set `explain_syntax_single_record = 0` (globally or in per-query `SETTINGS`), or set `compatibility` to any version older than `26.7`.
...
- **Quotas and limits.** It is charged against the same [quotas](/operations/quotas)
  and subject to the same [limits](/operations/settings/query-complexity)
...
- **Failed transactions.** Inside a [transaction](/guides/developer/transactional)
...
- **Distributed queries.** It is not supported for queries executed in
  [distributed](/engines/table-engines/special/distributed) mode.
:::
