...
```
:::note
Recursive CTEs rely on the [new query analyzer](https://clickhouse.com/docs/en/operations/analyzer) introduced in version **`24.3`**. If you're using version **`24.3+`** and encounter a **`Code: 60. DB::Exception … (UNKNOWN_TABLE)`** exception, it suggests that the new analyzer is disabled on your instance, role, or profile. To activate the analyzer, enable the setting **`allow_experimental_analyzer`**.
