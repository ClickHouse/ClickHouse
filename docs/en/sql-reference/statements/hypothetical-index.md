...
`EXPLAIN WHATIF` errors with `NOT_IMPLEMENTED` when the query is served from a projection, because a parent-table index is not materialized on projection parts.

Queries with `FINAL` are estimated only through the empirical path, since the statistical one does not model the `FINAL` merge. When `use_skip_indexes_if_final_exact_mode` is enabled, the candidate's surviving granules are widened by the same `PrimaryKeyExpand` pass a real read applies, so the reported marks account for granules re-included to keep the merge correct — a candidate that prunes well on its own can still show little benefit under `FINAL`. The pass is skipped, exactly as in a real read, when every column of every useful skip index is part of the primary key.

`status: not_applicable` is reported under `FINAL` when skip indexes are not used there (`use_skip_indexes_if_final = 0`), when empirical estimation is unavailable for the candidate, or when `PrimaryKeyExpand` would fall back to reading whole parts — with an unsortable primary key, or with parts written without a final mark (for example `index_granularity_bytes = 0`) — and that fallback would read more than the query's current baseline. Combined multi-candidate rows are not reported under `FINAL`.
