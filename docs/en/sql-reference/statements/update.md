...
They are related to the [virtual columns](/engines/table-engines/mergetree-family/mergetree.md/#virtual-columns) in the original part, which are added for reading if patch parts should be applied.
...
The memory usage of applying a patch part is bounded by the largest range of rows with equal sorting key values, not by the size of the patch part.
...
Because of that a patch part may need to be applied to a part created by a merge that finished after the `UPDATE` was executed.
Rows are matched by their values rather than by their positions in the original parts, therefore the same merge algorithm handles both parts that participated in merges and parts that did not.

### Patch parts format in mixed-version clusters {#patch-parts-format-in-mixed-version-clusters}

The format of newly written patch parts is controlled by the table-level setting [`patch_parts_version`](/operations/settings/merge-tree-settings#patch_parts_version).
...
During a rolling upgrade from such a version, keep writing the `v1` format until all replicas are upgraded: set `patch_parts_version = 'v1'` for tables with lightweight updates, or set the [`compatibility`](/operations/settings/settings#compatibility) setting to the version you are upgrading from.
