...
## SHOW CREATE SHARD | CLUSTER {#show-create-shard--cluster}

Returns the `CREATE` statement for a SQL catalog shard or cluster. Requires [`cluster_metadata`](/operations/sql-clusters#configuration) and the corresponding `SHOW CREATE SHARD` or `SHOW CREATE CLUSTER` privilege.

### Syntax {#syntax-show-create-shard-cluster}
...
### Example {#example-show-create-cluster}

```sql
SHOW CREATE CLUSTER analytics;
```

See also: [SQL clusters](/operations/sql-clusters).
