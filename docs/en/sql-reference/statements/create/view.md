...
This avoids recomputing the view when the underlying data is unchanged. Change detection uses the same mechanism as the [`modification_hash`](/operations/system-tables/tables) column of `system.tables`. If a source table cannot report a value that changes on every change and never repeats across a change-and-change-back (an `A -> B -> A` transition) - a table function such as `url`, or a `File`, `URL` or object storage (`S3`, ...) table, whose only validators (size, modification time, `ETag`) can repeat - the refresh is not skipped: it fails closed and always rebuilds. A manual `SYSTEM REFRESH VIEW` always rebuilds the view, ignoring `IF CHANGED`. The check is best-effort: after a server restart, or on a replica that did not previously run the refresh, one refresh may run even if nothing changed.
