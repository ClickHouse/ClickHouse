...
- DEFLATE_QPL is disabled by default and can only be used after set server setting `enable_deflate_qpl_codec` to be true. Refer to [Data compression settings](/docs/en/operations/server-configuration-parameters/settings#server-settings-compression) for more details.
