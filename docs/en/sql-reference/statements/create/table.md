...
- DEFLATE_QPL is disabled by default and and can only be used after setting server setting enable_deflate_qpl_codec to true. Refer to [server-settings-enable-deflate-qpl](/docs/en/operations/server-configuration-parameters/settings#server-settings-enable-deflate-qpl) for more details.
