...
- `path` — The relative path to the file from [user_files_path](/docs/operations/server-configuration-parameters/settings.md#user_files_path). Supports in read-only mode the following [globs](#globs-in-path): `*`, `?`, `{abc,def}` (with `'abc'` and `'def'` being strings) and `{N..M}` (with `N` and `M` being numbers).
...
- [storage_file_read_method](/docs/operations/settings/settings.md#engine-file-empty_if-not-exists) - method of reading data from storage file, one of: read, pread, mmap (only for clickhouse-local). Default value: `pread` for clickhouse-server, `mmap` for clickhouse-local.


**See Also**

- [Virtual columns](/docs/engines/table-engines/index.md#table_engines-virtual_columns)
- [Rename files after processing](/docs/operations/settings/settings.md#rename_files_after_processing)
