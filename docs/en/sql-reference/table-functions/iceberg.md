...
Parsed deletion vectors can be cached in memory when `use_puffin_files_cache` is enabled and the puffin file has a non-empty `etag`. Empty deletion vectors are cached as well, so repeated reads do not re-fetch the puffin file. Parsed footers are cached separately by storage identity, path, and `etag`, so multiple deletion-vector slices from one coalesced Puffin file share a single footer fetch/parse. The cache can be cleared with `SYSTEM DROP PUFFIN FILES CACHE`.
