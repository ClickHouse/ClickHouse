...
:::note
ClickHouse uses filename extensions to determine the format of the data and also the compression of the file. For example, if the file was zipped up with a `.csv.gz` extension, ClickHouse would decompress the file.

To demonstrate, try the following command - which works the same as the example above:
