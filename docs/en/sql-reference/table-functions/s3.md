...
`wildcard`: Replaces the `{_partition_id}` wildcard in the file path with the actual partition key. Selected by default only under `compatibility` settings older than `26.5`; otherwise the default is `hive` (see the `file_like_engine_default_partition_strategy` setting).
