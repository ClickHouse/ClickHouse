...
SQL-created named collections for `s3` cannot use `use_environment_credentials` and cannot inherit top-level `<s3>` credentials from the server configuration. Use explicit credentials, `NOSIGN` for public buckets, or operator-managed endpoint-scoped `<s3>` configuration that matches the URL.
