...
| `server_side_encryption_customer_key_base64` | Parameter is optional. The base64-encoded key for accessing S3 objects with [SSE-C](https://docs.aws.amazon.com/AmazonS3/latest/userguide/ServerSideEncryptionCustomerKeys.html) (server-side encryption with customer-provided keys). The same key is used to write and to read the object and is never stored by the server. Passed as a key-value argument: `server_side_encryption_customer_key_base64 = '...'`. Supported by S3-compatible stores such as Linode Object Storage. |
...
| `server_side_encryption_customer_key_base64` | base64-encoded SSE-C customer-provided key. See the plain parameter of the same name above.                                                                       |
...
Some S3-compatible object stores (for example, Linode Object Storage) encrypt objects with a key that
you provide on every request, using [SSE-C](https://docs.aws.amazon.com/AmazonS3/latest/userguide/ServerSideEncryptionCustomerKeys.html).
Pass the base64-encoded 256-bit key with the `server_side_encryption_customer_key_base64` argument; the
same key must be supplied to write the object and to read it back. ClickHouse expands it into the
`x-amz-server-side-encryption-customer-algorithm`, `x-amz-server-side-encryption-customer-key` and
`x-amz-server-side-encryption-customer-key-md5` request headers, so you do not need to compute the key
MD5 yourself.

```sql
-- Write an SSE-C encrypted object
INSERT INTO FUNCTION s3(
    'https://<bucket>.<region>.linodeobjects.com/export.parquet',
    '<KEY>', '<SECRET>',
    format = 'Parquet',
    server_side_encryption_customer_key_base64 = '<BASE64_ENCODED_KEY>')
...
The key is a credential: it is masked as `'[HIDDEN]'` in `system.query_log`, `SHOW CREATE` and `EXPLAIN`
output. The same argument is accepted in [named collections](/operations/named-collections.md) and, for
the [`S3` table engine](/engines/table-engines/integrations/s3) and disks, as the
`server_side_encryption_customer_key_base64` configuration option.
