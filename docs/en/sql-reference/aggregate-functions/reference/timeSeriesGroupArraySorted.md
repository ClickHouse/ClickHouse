...
slug: /sql-reference/aggregate-functions/reference/timeSeriesGroupArraySorted
title: 'groupArraySorted'
---

# timeSeriesGroupArraySorted
...
```sql
timeSeriesGroupArraySorted(timestamp, value)
```
...
SELECT timeSeriesGroupArraySorted(timestamp, value)
FROM
(
...
WITH
    [110, 120, 130, 140, 140, 100]::Array(UInt32) AS timestamps,
    [1, 6, 8, 17, 19, 5]::Array(Float32) AS values -- array of values corresponding to timestamps above
SELECT timeSeriesGroupArraySorted(timestamps, values);
```
