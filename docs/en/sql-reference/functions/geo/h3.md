...
Returns the centroid longitude and latitude corresponding to the provided [H3](#h3-index) index. The order of the returned tuple can be configured using the use_legacy_h3ToGeo_order setting.

**Syntax**

``` sql
h3ToGeo(h3Index[, Option])
```
...
- When `use_legacy_h3ToGeo_order` is enabled: `tuple(lon,lat)`. `lon` — Longitude. [Float64](../../data-types/float.md). 
`lat` — Latitude. [Float64](../../data-types/float.md).
- When `use_legacy_h3ToGeo_order` is disabled: `tuple(lat, lon)`. `lat` — Latitude. [Float64](../../data-types/float.md). 
`lon` — Longitude. [Float64](../../data-types/float.md).
...
``` sql
SET use_legacy_h3ToGeo_order = 1;
SELECT h3ToGeo(644325524701193974) AS coordinates;
```
Result:
