...
:::note
The H3 functions are implemented on top of the Rust [`h3o`](https://github.com/HydroniumLabs/h3o) library and are therefore only available in builds where Rust is enabled. On platforms where Rust is unavailable (for example, FreeBSD) or in builds configured with `-DENABLE_RUST=OFF`, the H3 functions are not built and are unavailable. This is a change from earlier versions, which used a C library backend that did not depend on Rust.
:::
...
Mode `3` (`CONTAINMENT_OVERLAPPING_BBOX`) is not supported by the `h3o` backend, which has no bounding-box containment mode. Passing `3` raises an `ARGUMENT_OUT_OF_BOUND` exception.
...
- `geometry` can be one of the following [Geo Data Types](../../data-types/geo.md) or their underlying primitive types:
  ...
  - [MultiPolygon](../../data-types/geo.md#multipolygon)
- `resolution` — Index resolution. Range: `[0, 15]`. [UInt8](../../data-types/int-uint.md).
- `flags` — Containment mode. Range: `[0, 2]`. Use integer literals such as `0`, `1`, and `2`, or [UInt32](../../data-types/int-uint.md) values (for example, `toUInt32(2)`). Values are passed to the H3 API as `UInt32`. Other native integer types are converted with an accurate cast.
...
Parsing follows `sscanf("%" PRIx64, ...)` semantics: the longest valid hex prefix is consumed (after optional whitespace, sign, and `0x`/`0X` prefix) and trailing garbage is ignored. Inputs containing at least one hex digit succeed (so `'foo'` parses to `0xf`). The returned value is not validated as a real H3 cell — use [`h3IsValid`](#h3isvalid) for that.
...
- Parsed `H3Index`. A returned value of `0` is ambiguous: it can come either from a successful parse of a zero value (for example `'0'`, `'000'`, `'0x'`, `'+0x'`) or from the no-hex-digit path where the parse error is swallowed, so `0` does not by itself prove invalid input. [UInt64](../../data-types/int-uint.md).
