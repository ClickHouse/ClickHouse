...
Note 1: In ClickHouse version v23.12 or older, `h3ToGeo()` returns values in order `(lon, lat)`. This behaviour is fixed in v25.1, which switches the order to be `(lat, lon)`. The previous behaviour can be restored with setting `h3togeo_lon_lat_result_order=true`.
