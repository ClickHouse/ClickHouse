...
Returns the hexagons (at specified resolution) contained by the provided geometry, either ring or (multi-)polygon, using the experimental algorithm with different containment modes.

**Syntax**

``` sql
h3PolygonToCellsExperimental(geometry, resolution, mode)
```
...
- `geometry` can be one of the following [Geo Data Types](../../data-types/geo.md) or their underlying primitive types:
  ...
  - [MultiPolygon](../../data-types/geo.md#multipolygon)
- `resolution` — Index resolution. Range: `[0, 15]`. [UInt8](../../data-types/int-uint.md).
- `mode` — Containment mode. Constant [String](../../data-types/string.md). One of:
  - `'center'` — Cell center is contained in the shape.
  - `'full'` — Cell is fully contained in the shape.
  - `'overlapping'` — Cell overlaps the shape at any point.
  - `'overlapping_bbox'` — Cell bounding box overlaps the shape.
...
``` sql
SELECT h3PolygonToCellsExperimental([(-122.4089866999972145,37.813318999983238),(-122.3544736999993603,37.7198061999978478),(-122.4798767000009008,37.8151571999998453)], 7, 'center') AS h3index;
```
