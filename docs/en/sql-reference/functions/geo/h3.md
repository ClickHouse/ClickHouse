...
Note: In ClickHouse v25.3 or older, `geoToH3()` takes values in order `(lon, lat)`. As per ClickHouse v25.4, the input values are in order `(lat, lon)`. The previous behaviour can be restored using setting `geotoh3_lon_lat_input_order = true`.
