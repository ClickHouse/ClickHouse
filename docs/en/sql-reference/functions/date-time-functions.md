...
```
Additionally, the first argument can be specified as a string in a format supported by [`parseDateTime64BestEffort`](./type-conversion-functions.md#parsedatetime64besteffort). This was implemented for MySQL compatibility reasons only, specifically to support queries generated by 3rd party tools, and it is strongly advised to avoid deliberate usage of a string argument outside of MySQL interface and `DAYOFWEEK` alias context.
