...
- `SETTINGS`: Per-function settings
    - `serialization_format` String — Serialization format for ABI requires it. Supported values: `MsgPack`, `JSONEachRow`, `CSV`, `TSV`, `TSVRaw`, `RowBinary`, and `Buffers`. Default: `MsgPack`. Under `ABI BUFFERED_V1`, `Buffers` uses the declared function schema and ClickHouse Native per-column binary encoding. When `serialization_format = 'Buffers'`, the guest must return a single-column `Buffers` block whose `num_rows` equals the input block row count and whose column type matches the declared `RETURNS` type.
    - `webassembly_udf_enable_fuel` Bool — Enables finite fuel budgeting for the function. Default: `true`. Set at function creation time via `CREATE FUNCTION ... LANGUAGE WASM ... SETTINGS ...`; the value is persisted with the function definition. When `false`, the query-level setting `webassembly_udf_max_fuel` is ignored for this function. `webassembly_udf_max_memory` still applies. Query cancellation via `max_execution_time`, `KILL QUERY`, and shutdown still applies. For `webassembly_udf_engine = 'wasmedge'`, ClickHouse keeps WasmEdge cost measuring active internally to deliver cancellation; for Wasmtime, epoch interruption remains enabled.
