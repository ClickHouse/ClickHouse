...
The dictionary key has the [UInt64](../../../sql-reference/data-types/int-uint.md) type and the value is limited to the `max_array_size` value (by default — 500,000). If a larger key is discovered when creating the dictionary, ClickHouse throws an exception and does not create the dictionary. The initial number of records created is defined by the `initial_array_size` value (by default — 1024).
