...
A list of supported time zones can be found in the [IANA Time Zone Database](https://www.iana.org/time-zones). You may also look for time zone names on [Wikipedia](https://en.wikipedia.org/wiki/List_of_tz_database_time_zones) query them by `SELECT * FROM system.time_zones`.
