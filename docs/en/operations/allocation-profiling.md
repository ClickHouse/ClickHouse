...
- Analyze memory fragmentation from SQL using [`system.jemalloc_sampled_allocations`](#analyzing-fragmentation-from-sql) (26.9+).
...
The server UI includes all tabs: Summary, Allocations, Arenas, Operations, Global Profiler, Query Profiler, Fragmentation, and Raw Output.
...
Once enabled, the UI provides the same visualizations as the server — Summary, Allocations, Arenas, Operations, Global Profiler, and Raw Output — except for the Query Profiler and Fragmentation tabs, which require SQL access to the system tables.
...
## Analyzing fragmentation from SQL {#analyzing-fragmentation-from-sql}

<Note>
This section is applicable for versions 26.9+.
</Note>

jemalloc serves small allocations from slabs: fixed-size blocks of memory split into regions of one size class. A slab is freed only when every region in it is free. If most allocations of a size class are freed quickly, but a few of them live for a long time, the slabs stay pinned with mostly free regions. The memory stays resident, but it is not allocated. This is what fragmentation means here.

Two system tables help find where it comes from:

- [`system.jemalloc_arena_bins`](/reference/system-tables/jemalloc_bins) shows how much memory each size class wastes in each arena. `waste` is the bytes held in slabs but not used by live allocations. `purpose` marks dedicated arenas (`mergetree`, `jit`, `cache`); they hold long-lived data by design, so filter them out with `purpose = ''` when looking for unexpected fragmentation. `system.jemalloc_bins` has the same data summed over arenas.
- `system.jemalloc_sampled_allocations` lists the sampled allocations that are currently alive, one row per sample: the backtrace (`trace`), how long it has been alive (`age_ns`), the size and size class, and the arena. Reading the table flushes a new heap profile.

Joining them on arena and size class shows which code allocated the old objects in the wasteful size classes.

Keep in mind when reading the results:

- Only threads with the profiler enabled produce samples (`jemalloc_enable_global_profiler` in the config or the `jemalloc_enable_profiler` setting).
- Each row stands for about `weight` real allocations, and small allocations are sampled rarely. Adjust `jemalloc_profiler_sampling_rate` if the size classes you care about have no samples.
- Sampled allocations are not stored in slabs, so they do not add to `waste` themselves. They only tell you what code allocates in that size class.
- Every live sample costs two extra pages (128 KiB on builds with 64 KiB pages) until the allocation is freed. Disabling the profiler stops new samples but does not free the existing ones.

The following query lists the backtraces of allocations older than a minute in the size classes that waste more than 1 MiB:
...
### Example: generating a fragmentation flame graph {#example-fragmentation-flamegraph}

The same query can split the waste of each size class between its backtraces, in proportion to their number of old allocations, and print the result in the collapsed format for `flamegraph.pl`. Size classes with waste but without old enough samples go under an `[unattributed]` frame, so the total of the flame graph is the total waste. Read `system.jemalloc_sampled_allocations` once per query: every read flushes a new profile, so two subqueries would see different data.
