...
| `full_url`           | To match the complete request URL `schema://host:port/path`. The query string is ignored when matching, and the host is the connection IP address (not the `Host` header). You can use the 'regex:' prefix for a regex match, or the 'prefix:' prefix to match `schema://host:port/base_path` and everything below it on a path-segment boundary (optional) |
