...
A `{name:Type}` placeholder in a query part that the matcher does not traverse is not supported either, and is likewise rejected at `CREATE RULE` / `ALTER RULE` time. These are parts that the parser keeps outside the query's AST children, so a placeholder there could be neither bound by matching nor substituted into the result: the `LIMIT` or `WHERE` of a `SHOW` statement, a `SETTINGS` clause value (`SETTINGS max_threads = {n:Int}`), a `BACKUP` / `RESTORE` setting, a `CREATE ROW POLICY` filter, a masking policy's assignments or condition, the user names of `CREATE USER` / `ALTER USER`, the new name of `ALTER USER ... RENAME TO`, the per-method `VALID UNTIL` of an `IDENTIFIED ...` clause, and the parameters or lambda of a column matcher's `APPLY` transformer.
...
- Rewrite rules are applied only on the initiator of a query. For distributed queries the `query_rules` setting is not propagated to shards, so a rule is never re-applied to a secondary query, and the client-supplied query kind cannot be used to bypass a rule. The same holds for distributed DDL: `query_rules` is stripped from the settings recorded in the `ON CLUSTER` DDL log entry, and DDL workers never apply rewrite rules to the replayed query — not even a worker-side profile default `query_rules`, and regardless of `distributed_ddl_entry_format_version`.
- Rules are applied to the initial user query only, never to SQL the server synthesizes on the user's behalf: internal helper queries (for example the `SELECT` that implements `SHOW PRIVILEGES`), the SQL generated for a PromQL request over the Prometheus HTTP API, the statements generated for the PostgreSQL wire protocol's `COPY` and cancel requests, the SQL that the MySQL wire protocol substitutes for the commands it emulates (`SHOW WARNINGS`, `SHOW TABLE STATUS LIKE`, `SHOW VARIABLES`, `KILL QUERY`, mapped `SET` variables), and the SQL that Arrow Flight synthesizes for its non-SQL requests (a `PATH` descriptor, the metadata commands such as `CommandGetTables` and `CommandGetDbSchemas`, `CommandStatementIngest`, `GetSessionOptions`, and the schema inference of `CreatePreparedStatement`) are all executed without applying rewrite rules, even when the session or profile sets `query_rules`. SQL that the client itself submits over those interfaces — a MySQL query that is not emulated, an Arrow Flight `CommandStatementQuery` / `CommandStatementUpdate` or prepared-statement execution — is matched as usual.
