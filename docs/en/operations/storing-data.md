...
| `use_real_disk_size`                  | Boolean | `false`    | Accounts cache usage in filesystem allocation units (block size, `f_frsize`) instead of the written byte count. When enabled, `FilesystemCacheSize`, the eviction-size metrics, and `current_size` in `system.filesystem_cache_settings` are computed with block-aligned sizes, so a sub-block file (e.g. 1 byte) is charged for the whole block it occupies. This is a block-aligned approximation of the physical on-disk usage, not the exact allocated size: it can differ from the blocks actually allocated on filesystems with transparent compression, sparse files, reflinks/CoW, or inline data. The per-segment `size` in `system.filesystem_cache` is not affected by this setting — it always reports the segment's logical range size (`range().size()`); for a partially downloaded segment, use `downloaded_size` for the bytes actually written — so per-segment sizes do not sum to the block-aligned aggregate metrics. |
