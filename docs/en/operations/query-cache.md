...
When several clients run the same `SELECT` simultaneously on a cache miss, one execution computes the result and streams it into the cache while others wait briefly and then re-read the cache. Waiters are grouped by the same rules as cache read access: if `query_cache_share_between_users` is false (default), only sessions with the same user and roles coalesce; if it is true, concurrent identical queries share one herd regardless of user. Setting [query_cache_herd_wait_timeout](/operations/settings/settings#query_cache_herd_wait_timeout) bounds how long a waiting query blocks before it gives up and runs the query itself (trading duplicate work for lower latency). The default is 300 seconds. Set it to `0` to disable herd coalescing entirely, so that every concurrent identical query executes independently (the behavior before this feature was introduced); this is also the compatibility value for versions before 26.8.

Coalescing applies to cached subqueries ([query_cache_for_subqueries](/operations/settings/settings#query_cache_for_subqueries), or an explicit `SETTINGS use_query_cache = true` on the subquery) in the same way: concurrent queries containing the same cold subquery run it once and reuse the result. Subquery herds are separate from top-level herds, because the two are stored as separate cache entries. A query containing the same subquery twice never waits for itself.
