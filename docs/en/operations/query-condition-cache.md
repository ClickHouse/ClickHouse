...
To clear the query condition cache, run [`SYSTEM CLEAR QUERY CONDITION CACHE`](../sql-reference/statements/system.md#drop-query-condition-cache). Add `FOR TABLE [database.]table` to clear entries for one table without affecting other tables.
...
If you like to investigate individual filter conditions, you can check field `condition` in `system.query_condition_cache` in debug and sanitizer builds.
