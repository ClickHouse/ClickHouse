...
:::note
Do not set `throw_on_unknown_workload` to `true` unless `CREATE WORKLOAD default` is executed. It could lead to issues with startup of the server if a query without explicit setting `workload` is excuted during startup.
:::
