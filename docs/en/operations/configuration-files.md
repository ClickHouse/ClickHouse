...
- A value substituted via `from_zk` depends on where it is used and on whether the value begins with `<`. A value that begins with `<` is always parsed as an XML fragment: this is how a subtree is spliced into any element, both a structural `<include from_zk="…"/>` and an ordinary container such as `<profiles from_zk="…"/>` — it is kept for backward compatibility, because earlier versions always interpreted a `from_zk` value as XML. A value that does not begin with `<` is interpreted according to where it is used: only a structural `<include from_zk="…"/>` parses a non-`<` value as a YAML subtree, autodetected the same way as for configuration files (this allows a ZooKeeper node to hold an XML or a YAML subtree, see [Substitution with file content](#substitution-with-file-content) below); on any other element — whether a leaf substitution such as `<some_setting from_zk="…"/>` or `<password from_zk="…"/>`, or an ordinary container such as `<profiles from_zk="…"/>` — a non-`<` value is kept as literal text using its exact original bytes (XML special characters are escaped automatically, the same as for `from_env`). YAML autodetection is deliberately not applied on those elements, because such an element may just as well be a leaf holding a scalar, so a non-`<` value is never reinterpreted by the YAML parser and an existing scalar or secret is preserved unchanged: a value such as `abc: def` stays the literal text `abc: def` instead of becoming an `<abc>def</abc>` sub-element, and `abc # rotated` keeps its `# rotated` suffix instead of being treated as a YAML comment. To splice a subtree into an ordinary element (rather than a structural `<include>`), provide it as an XML fragment (a value beginning with `<`).
