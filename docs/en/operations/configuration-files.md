...
- A value substituted via `from_env` is always taken as literal text. XML special characters such as `&`, `<` and `>` are escaped automatically, so an environment variable can contain any XML-representable value, for example `a&b<c>d`, without breaking the configuration (see the note on XML-illegal control characters below). The value is never interpreted as XML, even if it happens to look like an XML fragment: `<a>1</a>` substitutes the literal text `<a>1</a>`.
- A value substituted via `from_zk` depends on where it is used and on whether the value begins with `<`. A value that begins with `<` is always parsed as an XML fragment: this is how a subtree is spliced into any element, both a structural `<include from_zk="…"/>` and an ordinary container such as `<profiles from_zk="…"/>` — it is kept for backward compatibility, because earlier versions always interpreted a `from_zk` value as XML. A value that does not begin with `<` is interpreted according to where it is used: only a structural `<include from_zk="…"/>` parses a non-`<` value as YAML, and only a value that actually is a YAML *subtree* (a mapping or a sequence) is expanded, autodetected the same way as for configuration files (this allows a ZooKeeper node to hold an XML or a YAML subtree, see [Substitution with file content](#substitution-with-file-content) below). A plain scalar is kept as literal text even on `<include>`: `<include from_zk="…"/>` is not only the subtree-splicing form, it is also the generic "replace this element with the node contents" form, so it may sit under a leaf setting or a secret — for example `<password><include from_zk="/secret"/></password>` — and YAML is not identity-preserving for scalars, so `abc # rotated` keeps its `# rotated` suffix instead of being treated as a YAML comment. Note the one remaining ambiguity of the autodetection: a value referenced from `<include>` that is shaped like a YAML mapping or sequence is expanded as a subtree; to keep such a value literal, reference it from the leaf element directly (for example `<password from_zk="…"/>`), which never applies YAML autodetection. On any other element — whether a leaf substitution such as `<some_setting from_zk="…"/>` or `<password from_zk="…"/>`, or an ordinary container such as `<profiles from_zk="…"/>` — a non-`<` value is kept as literal text using its exact original bytes (XML special characters are escaped automatically, the same as for `from_env`), so a value such as `abc: def` stays the literal text `abc: def` instead of becoming an `<abc>def</abc>` sub-element. To splice a subtree into an ordinary element (rather than a structural `<include>`), provide it as an XML fragment (a value beginning with `<`).

:::note Literal values must be valid XML 1.0 text
