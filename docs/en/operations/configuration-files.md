...
- A value substituted via `from_zk` depends on where it is used and on whether the value begins with `<`. A value that begins with `<` is always parsed as an XML fragment, both for a structural `<include from_zk="…"/>` and for a leaf substitution — this is kept for backward compatibility, because earlier versions always interpreted a `from_zk` value as XML. A value that does not begin with `<` is interpreted according to where it is used: a structural `<include from_zk="…"/>` may reference a whole subtree, so a non-`<` value is parsed as a YAML subtree, autodetected the same way as for configuration files (this allows a ZooKeeper node to hold an XML or a YAML subtree, see [Substitution with file content](#substitution-with-file-content) below); a leaf substitution, such as `<some_setting from_zk="…"/>` or `<password from_zk="…"/>`, keeps a non-`<` value as literal text using its exact original bytes (XML special characters are escaped automatically, the same as for `from_env`). Such a non-`<` leaf value is never reinterpreted by the YAML parser, so an existing scalar or secret is preserved unchanged: a value such as `abc: def` stays the literal text `abc: def` instead of becoming an `<abc>def</abc>` sub-element, and `abc # rotated` keeps its `# rotated` suffix instead of being treated as a YAML comment.
