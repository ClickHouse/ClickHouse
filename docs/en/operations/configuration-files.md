...
- A value substituted via `from_zk` depends on where it is used. A structural `<include from_zk="…"/>` may reference a whole subtree, so its format is autodetected the same way as for configuration files: if the value begins with `<` it is parsed as an XML fragment, otherwise it is parsed as a YAML subtree. This allows a ZooKeeper node to hold an XML or a YAML subtree (see [Substitution with file content](#substitution-with-file-content) below). A leaf substitution, such as `<some_setting from_zk="…"/>` or `<password from_zk="…"/>`, keeps the value as literal text using its exact original bytes (XML special characters are escaped automatically, the same as for `from_env`). It is never reinterpreted by the YAML parser, so an existing scalar or secret is preserved unchanged: a value such as `abc: def` stays the literal text `abc: def` instead of becoming an `<abc>def</abc>` sub-element, and `abc # rotated` keeps its `# rotated` suffix instead of being treated as a YAML comment.
