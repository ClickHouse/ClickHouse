...
-    `stale_log_gap` — Threshold when leader consider follower as stale and send snapshot to it instead of logs (default: 10000).
...
4. Copy snapshot to ClickHouse server nodes with configured `keeper` or start ClickHouse Keeper instead of ZooKeeper. The snapshot must persist on all nodes, otherwise, empty nodes can be faster and one of them can become a leader.
