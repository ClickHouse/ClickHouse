...
- `duration_ms` ([UInt64](/sql-reference/data-types/int-uint)) — Duration of this operation.
- `database` ([String](/sql-reference/data-types/string)) — Name of the database the data part is in.
- `table` ([String](/sql-reference/data-types/string)) — Name of the table the data part is in.
- `table_uuid` ([UUID](/sql-reference/data-types/uuid)) — UUID of the table the data part belongs to.
- `part_name` ([String](/sql-reference/data-types/string)) — Name of the data part.
...
- `size_in_bytes` ([UInt64](/sql-reference/data-types/int-uint)) — Size of the data part on disk in bytes.
- `merged_from` ([Array(String)](/sql-reference/data-types/array)) — An array of the source parts names which the current part was made up from.
- `bytes_uncompressed` ([UInt64](/sql-reference/data-types/int-uint)) — Uncompressed size of the resulting part in bytes.
- `read_rows` ([UInt64](/sql-reference/data-types/int-uint)) — The number of rows was read during the merge.
- `read_bytes` ([UInt64](/sql-reference/data-types/int-uint)) — The number of bytes was read during the merge.
- `peak_memory_usage` ([UInt64](/sql-reference/data-types/int-uint)) — The maximum amount of used during merge RAM
- `deduplication_block_ids` ([Array(String)](/sql-reference/data-types/array)) — An array of block IDs used for deduplication when inserting this part.
...
- `mutation_ids` ([Array(String)](/sql-reference/data-types/array)) — An array of mutation IDs applied to the source part (merged_from) for the event with type MUTATE_PART_START and MUTATE_PART.
...
- `projections_duration_ms` ([Map(LowCardinality(String), UInt64)](/sql-reference/data-types/map)) — Per-projection merge/rebuild duration in milliseconds.
