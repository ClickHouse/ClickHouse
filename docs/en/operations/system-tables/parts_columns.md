...
| `physical_name` | String | Physical name of the column on disk. Differs from the logical column name when [physical name mapping](/engines/table-engines/mergetree-family/mergetree#physical-column-names) is active (e.g. `0`, `1`, `2`). Equals the logical column name when physical names are not enabled. |
