...
| `column_id` | String | Column ID of the column on disk. Differs from the logical column name when [column ID mapping](/engines/table-engines/mergetree-family/mergetree#physical-column-names) is active (e.g. `0`, `1`, `2`). Equals the logical column name when column IDs are not enabled. |
