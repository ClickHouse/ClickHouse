...
- `memory_context` ([Enum8('Unknown' = -1, 'Global' = 0, 'User' = 1, 'Process' = 2, 'Scope' = 3, 'Thread' = 4, 'Max' = 5)](/sql-reference/data-types/enum)) — Memory Tracker context (only for Memory/MemoryPeak): `Unknown` context is not defined for this trace_type. `Global` represents server context. `User` represents user/merge context. `Process` represents process (i.e. query) context. `Scope` represents a scope for a piece of a process (for example, a background part write, a merge stage or a materialized view), accounting for all jobs scheduled in thread pools from that scope. `Thread` represents thread (thread of particular process) context. `Max` this is a special value means that memory tracker is not blocked (for blocked_context column).
- `memory_blocked_context` ([Enum8('Unknown' = -1, 'Global' = 0, 'User' = 1, 'Process' = 2, 'Scope' = 3, 'Thread' = 4, 'Max' = 5)](/sql-reference/data-types/enum)) — Context for which memory tracker is blocked (for ClickHouse developers only): `Unknown` context is not defined for this trace_type. `Global` represents server context. `User` represents user/merge context. `Process` represents process (i.e. query) context. `Scope` represents a scope for a piece of a process (for example, a background part write, a merge stage or a materialized view), accounting for all jobs scheduled in thread pools from that scope. `Thread` represents thread (thread of particular process) context. `Max` this is a special value means that memory tracker is not blocked (for blocked_context column).
