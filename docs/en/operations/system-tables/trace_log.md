...
- `trace` ([Array(UInt64)](/sql-reference/data-types/array)) — Stack trace at the moment of sampling. Each element is a virtual memory address inside the ClickHouse server process; this is the same representation for all trace types, including `Instrumentation`. These are absolute (ASLR-dependent) runtime addresses, so symbolization (`symbols`, `lines`, `arrayMap(addressToSymbol, trace)`) is reliable only while the same process that collected the row is alive. Rows persisted across a server restart of a position-independent (PIE) binary may no longer symbolize, because the load base changes.
