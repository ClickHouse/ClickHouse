...
- `modification_hash` ([Nullable(UInt128)](/sql-reference/data-types/nullable)) — A value that is guaranteed to change whenever the data behind the table changes (similar to an HTTP ETag) and that never repeats across a change-and-change-back (an `A -> B -> A` transition). It is not a hash of the data, so two tables with identical data may have different values. `NULL` if the engine cannot provide such a value - either it cannot tell whether its data changed, or (as for `File`, `URL` and object storage such as `S3`) it can only offer a value derived from the current content (size, modification time, `ETag`) that may repeat. Computing it may be expensive for some engines (e.g. `Merge`, `Distributed`), so it is only calculated when this column is selected.
