...
- `required_quorum` ([UInt32](../../sql-reference/data-types/int-uint.md)) — The number of replicas waiting for the task to complete with confirmation of completion. This column is only relevant for the `GET_PART` task.
...
- `last_exception_time` ([DateTime](../../sql-reference/data-types/datetime.md)) — Date and time when the last error occurred.
...
The following columns provide information for [selective replication](/engines/table-engines/mergetree-family/replication.md/#selective-replication):

- `selective_partition_id` ([String](../../sql-reference/data-types/string.md)) — Partition ID associated with this queue entry for selective replication lookup. Empty if selective replication is disabled.
- `selective_assigned_replicas` ([Array](../../sql-reference/data-types/array.md)([String](../../sql-reference/data-types/string.md))) — List of replicas assigned to the partition. Empty if selective replication is disabled.
- `selective_is_assigned` ([UInt8](../../sql-reference/data-types/int-uint.md)) — Whether this entry's partition is assigned to the current replica. When selective replication is disabled (`replication_factor = 0`), all replicas participate and this is `1` for all entries.
