...
- `accepts_lambda` ([Nullable(UInt8)](/sql-reference/data-types/int-uint)) — Whether the function is higher-order — i.e. accepts at least one lambda expression as an argument (e.g. `arrayMap`, `arrayFilter`, `mapApply`). `NULL` when unknown.
<!--AUTOGENERATED_END-->
