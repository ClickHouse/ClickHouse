...
- `written_rows` ([UInt64](/sql-reference/data-types/int-uint)) — The number of rows written by the query, including any rows written by downstream inserts triggered by the pipeline, such as attached materialized views. This applies to inserts, covering both synchronous inserts (`query_kind` = `Insert`) and asynchronous insert flushes (`query_kind` = `AsyncInsertFlush`); for queries that do not write rows, it is 0.
- `written_bytes` ([UInt64](/sql-reference/data-types/int-uint)) — The number of bytes written by the query (uncompressed), including any bytes written by downstream inserts triggered by the pipeline, such as attached materialized views. This applies to inserts, covering both synchronous inserts (`query_kind` = `Insert`) and asynchronous insert flushes (`query_kind` = `AsyncInsertFlush`); for queries that do not write data, it is 0.
- `result_rows` ([UInt64](/sql-reference/data-types/int-uint)) — Number of rows in the result of a SELECT query, or the number of rows written by an insert. For inserts, covering both synchronous inserts (`query_kind` = `Insert`) and asynchronous insert flushes (`query_kind` = `AsyncInsertFlush`), this includes any rows written by downstream inserts triggered by the pipeline, such as attached materialized views.
