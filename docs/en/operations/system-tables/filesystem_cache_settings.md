...
- `expose_prometheus_cache_usage_metrics_per_user` ([UInt8](/sql-reference/data-types/int-uint)) — Expose asynchronously sampled Prometheus gauges for filesystem cache usage by cache name and user id. Off by default and applied when the cache is created. Cardinality grows with distinct cache users.
