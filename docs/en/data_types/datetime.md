...
Data structure storing Unix timestamp (`UInt32`) and a time zone.
...
The [clickhouse-client](../interfaces/cli.md) applies the server time zone by default. To use the client time zone, run it with the `--use_client_time_zone` parameter.

ClickHouse outputs values in the `YYYY-MM-DD hh:mm:ss` text format. When inserting data into ClickHouse, you can use different formats of date and time strings depending on the [date_time_input_format](../operations/settings/settings.md#settings-date_time_input_format) setting value.
