...
Mapping `_key` to a `String` column stores the original Kafka message key. To extract fields from a structured key, map source fields beginning with `_key.`. For example, for the JSON key `{"customer":{"id":42}}`, map `_key.customer.id` to a destination column such as `customer_id`.
...
Raw keys, JSON scalars or arrays, and malformed JSON do not populate nested key fields; their destination columns receive type defaults. If ClickPipes recognizes a registry wire header but cannot retrieve or apply its schema, it writes the Kafka record to the ClickPipes error table.

An Avro key can also have a non-record root schema, such as a primitive, array, map, or fixed type. To decode one, map `_key` directly to a compatible destination type other than exactly `String`. An exact `_key` mapped to `String` always remains raw, and non-record schemas cannot populate nested key fields.
