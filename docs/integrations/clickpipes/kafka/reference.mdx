...
The Kafka consumer isolation level determines whether a ClickPipe will only read and insert messages that were committed by a Kafka transaction.  It it set set at **read\_committed** for pipes that are configured as Exactly Once (skipping any messages that were written but aborted during a Kafka transaction), and **read\_uncommitted** otherwise.  The setting can be changed under the Advanced Settings tab when creating a pipe.
...
Note that the `_raw_message` column is only recommended for JSON data. For use cases where only the JSON string is required (such as using ClickHouse [`JsonExtract*`](/reference/functions/regular-functions/json-functions#jsonextract-functions) functions to populate a downstream materialized view), it may improve ClickPipes performance to delete all the "non-virtual" columns.