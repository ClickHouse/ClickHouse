...
| `naive_datetime_insert` | `"local"` | `"local"`, `"server"` | Controls Python object inserts of naive `datetime` values into `DateTime` and `DateTime64`, and naive ISO strings accepted by `DateTime64`. `local` uses the process timezone for compatibility. `server` uses the declared column timezone, then the server timezone. `Date` and `Date32` use the value's own calendar date. `datetime64`-dtype NumPy and Pandas columns are unchanged. |
