...
| `WithAsync` | `bool` (wait) | Sync | Both | Async insert mode. Sets `async_insert=1`. `wait=true` adds `wait_for_async_insert=1`. Requires ClickHouse 21.11+. *(Since v2.41.0; supersedes the older `WithStdAsync`.)* | Use for high-throughput inserts. | `wait=false`: errors may be async (check `system.asynchronous_insert_log`). With `INSERT ... SELECT`: the async queue applies to eligible local queries when the entire result is a single block within `async_insert_max_data_size`; `wait_for_async_insert` applies as usual, so `wait=false` returns as soon as the block is handed to the queue. Runs synchronously instead when the result spans more than one block, exceeds `async_insert_max_data_size`, is empty, runs inside a transaction (explicit or `implicit_transaction = 1`), uses non-parallel quorum (`insert_quorum` set, `insert_quorum_parallel = 0`), or reads its own destination table (a self-referencing `INSERT INTO t SELECT ... FROM t`). Only a `MergeTree` family destination without dependent materialized views is eligible; a `Distributed` or remote destination, a table function destination, and parallel replicas bypass the async gate. Routing decided earlier wins, so with `parallel_distributed_insert_select` (on by default) an `INSERT ... SELECT` from a cluster source such as `s3Cluster` into a replicated table is executed by the distributed insert path and never reaches the async queue. On this route the write-side `ProfileEvents` and the write-side `system.query_log` row belong to the internal flush query, not to `WithQueryID`; measuring writes by query id on this route sees only the read side. A cancelled or timed-out `INSERT ... SELECT` on this route may still write its rows, because the block was already queued before the wait for the flush started. Old server: `"Unknown setting async_insert"`. |
