...
[CHOps](https://ch-ops.io) is a free, open-source, self-hosted web dashboard for administering and monitoring ClickHouse clusters. It wraps ClickHouse's HTTP interface in a browser UI for running SQL, inspecting slow queries, configuring alerts, managing backups, and controlling access. CHOps works against any ClickHouse server reachable over HTTP, whether self-managed on bare metal or VMs, or running on Kubernetes.

Features:

- SQL editor with autocomplete, EXPLAIN types, export results in multiple formats, query profiler and a query metrics timeline.
...
- Custom dashboards with a chart builder, 12+ chart types, and configurable grid layouts.
...
- Crash, error, and text log viewers with calendar heatmaps and filtered search.
