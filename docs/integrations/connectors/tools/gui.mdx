...
[CHOps](https://ch-ops.io) is a free, open-source, self-hosted web dashboard for administering and monitoring ClickHouse clusters. It wraps ClickHouse's HTTP interface in a browser UI for running SQL, inspecting slow queries, configuring alerts, managing backups, and controlling access. It connects over the HTTP interface, so it works with self-managed deployments on bare metal, VMs, or Kubernetes. CHOps currently supports ClickHouse 26.3 LTS; other recent versions work, though there may be minor feature gaps.
