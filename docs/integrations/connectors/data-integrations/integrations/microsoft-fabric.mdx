...
* **Customer-facing applications**: serve analytical queries at application concurrency without consuming Fabric capacity

<Image size="md" img="/images/integrations/tools/data-integration/microsoft-fabric/fabric-01-workload-hub-card.webp" alt="ClickHouse workload page in the Microsoft Fabric workload hub" border />
<br/>
...
<Image size="md" img="/images/integrations/tools/data-integration/microsoft-fabric/fabric-02-provisioning-canvas.webp" alt="Setting up ClickHouse: creating the Cloud organization and provisioning the service" border />
<br/>

The first time a ClickHouse item is created in a workspace, a Fabric administrator with billing rights installs the workload and accepts the terms of service. Workspace members then have access.
...
Creating your first ClickHouse item starts a trial automatically. There is no separate sign-up: the trial organization and a dedicated service are provisioned with your Microsoft Entra identity.

The trial includes:

* Trial credits valid for 30 days
* A dedicated service on the ClickHouse **Scale** tier
* Service idling enabled by default, so an inactive trial service does not consume credits

When the trial ends or the credits run out, convert to a paid organization either through the [ClickHouse Cloud offer on Azure Marketplace](https://marketplace.microsoft.com/en-us/product/clickhouse.clickhouse_cloud), which counts toward your Azure committed spend, or by adding a credit card in the ClickHouse Cloud console.

## Copying data from OneLake {#copying-data-from-onelake}

The Copy & Sync flow is how OneLake tables get into ClickHouse:

1. Open your ClickHouse item and select **Copy & Sync**
2. Pick a Lakehouse from your workspace
3. Select the tables you want to accelerate
4. Start copying a point-in-time snapshot of your data

<Image size="md" img="/images/integrations/tools/data-integration/microsoft-fabric/fabric-03-copy-sync-table-selection.webp" alt="Copy & Sync configuration creating a new destination table with sorting key" border />
<br/>
...
<Image size="md" img="/images/integrations/tools/data-integration/microsoft-fabric/fabric-04-copy-sync-existing-table.webp" alt="Copy & Sync column mapping from a OneLake source table to an existing ClickHouse table" border />
<br/>
...
OneLake remains the source of truth. The ClickHouse copy is an accelerated replica organized for fast analytical access.
...
**Powered by ClickPipes.** Each synced table is powered by a [ClickPipe](/integrations/clickpipes/home), the managed ingestion service of ClickHouse Cloud:
...
Copying data from OneLake to ClickHouse is free during the Microsoft Fabric public preview. Billing starts when the Fabric workload reaches general availability, according to the [listed pricing dimensions](/products/cloud/reference/billing/clickpipes/clickpipes-for-streaming-and-object-storage).
...
<Image size="md" img="/images/integrations/tools/data-integration/microsoft-fabric/fabric-05-sql-console.webp" alt="Embedded ClickHouse SQL console querying the synced NYC taxi table" border />
<br/>
...
ClickHouse offers far more than fast SELECTs: materialized views that pre-compute answers as data arrives, full-text search, approximate aggregations, and hundreds of specialized functions. Good starting points are [ClickHouse best practices](/concepts/best-practices/index), [materialized views](/concepts/features/materialized-views/index), and [example use cases](/guides/use-cases/index).
...
* **Analytics and BI**: Power BI via the certified ClickHouse connector (DirectQuery against accelerated tables), Fabric notebooks with the Python client
...
<Image size="md" img="/images/integrations/tools/data-integration/microsoft-fabric/fabric-06-connect-power-bi.webp" alt="Connect screen showing Power BI connection details for the ClickHouse service" border />
<br/>

MCP access is opt-in per service and read-only. Enabling it exposes a governed endpoint that agents can query with SQL; they get the same sub-second answers as the console, without touching your Fabric capacity. See [enabling the remote MCP server](/products/cloud/features/ai-ml/remote-mcp#enabling) for setup steps and the [ClickHouse MCP server reference](https://github.com/ClickHouse/mcp-clickhouse) for the available tools.

## Administration and management {#administration-and-management}

### Open in ClickHouse Cloud {#open-in-clickhouse-cloud}

Select **Open ClickHouse** to open the full ClickHouse Cloud console, signed in with your Microsoft identity. Day to day you stay in Fabric; the console is where administrators manage the service.

<Image size="md" img="/images/integrations/tools/data-integration/microsoft-fabric/fabric-07-cloud-console-service.webp" alt="ClickHouse Cloud console service settings reached from the workload" border />
<br/>
...
Service health, query performance, and resource usage are monitored from the ClickHouse Cloud console. Health and telemetry data are retained for a minimum of 30 days (see the [performance section of the attestation](https://clickhouse.com/legal/clickhouse-microsoft-fabric-workload-attestation)). For platform-wide health and incident history, see [status.clickhouse.com](https://status.clickhouse.com).

### Monitoring ClickPipes {#monitoring-clickpipes}

Each Copy & Sync from OneLake runs as a ClickPipe. Track copy progress, throughput, and errors on the [ClickPipes page](/integrations/clickpipes/monitoring) in the ClickHouse Cloud console.
...
* Marketplace purchases count toward your Azure committed spend.
...
<Image size="md" img="/images/integrations/tools/data-integration/microsoft-fabric/fabric-08-tutorial-sample-lakehouse.webp" alt="Public holidays sample table in a Fabric Lakehouse" border />
<br/>
...
2. Open **Copy & Sync**, pick your Lakehouse, and select the `publicholidays` table
...
1. Open the **Connect** screen and select **Power BI**
2. Follow the connection steps with the certified ClickHouse connector
3. Build a simple visual: holidays by country on a map

### Ask an agent {#ask-an-agent}

1. On the **Connect** screen, enable **MCP** for the service
2. Register the ClickHouse MCP server with your agent, as described in [getting started with the remote MCP server](/products/cloud/features/ai-ml/remote-mcp#getting-started)
...
* Terms: [general terms and conditions](https://clickhouse.com/legal/clickhouse-general-terms-and-conditions), [ClickHouse Cloud addendum](https://clickhouse.com/legal/clickhouse-cloud-addendum), [ClickHouse for Fabric addendum](https://www.clickhouse.com/legal/clickhouse-for-fabric-addendum)
