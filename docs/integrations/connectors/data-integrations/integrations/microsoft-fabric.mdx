...
<Frame>
  ![Copy & Sync configuration creating a new destination table with sorting key](/images/integrations/tools/data-integration/microsoft-fabric/fabric-03-copy-sync-table-selection.webp)
</Frame>
...
<Frame>
  ![Copy & Sync column mapping from a OneLake source table to an existing ClickHouse table](/images/integrations/tools/data-integration/microsoft-fabric/fabric-04-copy-sync-existing-table.webp)
</Frame>
...
Copying data from OneLake to ClickHouse is free during the Microsoft Fabric public preview. Billing starts when the Fabric workload reaches general availability. Pricing details will be published when the workload reaches GA.
...
Select **Open ClickHouse** to open the full ClickHouse Cloud console, signed in with your Microsoft identity. You can copy data into ClickHouse and run any SQL statements from the Fabric workload, but for any administrative tasks you'll need to enter the Cloud Console to manage the service.
...
* **Lakehouse tables only.** Copy & Sync reads Delta tables from a Lakehouse in the same workspace. Other Fabric items (Warehouse, KQL databases, mirrored databases) are not yet supported as sources.
...
### Fabric platform integration {#fabric-platform-integration}

* **Not yet integrated in preview:** Fabric Monitoring Hub (monitor copy jobs from the ClickHouse Cloud console instead), deployment pipelines and git integration (ALM/CI/CD), item export and restore across workspaces, the OneLake data hub, data lineage, Microsoft Purview sensitivity labels, workspace-level settings, global search, and cross-tenant (B2B) sharing.
* **Private Link is not configurable through the workload.** Azure Private Link is supported from the ClickHouse Cloud console, but not from Fabric.
