...
* **Lakehouse tables only.** Sync reads Delta tables from a Lakehouse in the same workspace. Other Fabric items (Warehouse, KQL databases, mirrored databases) are not yet supported as sources.
