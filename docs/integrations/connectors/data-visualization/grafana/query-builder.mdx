...
The toggles are enabled by default, but each link appears only when the data source is configured for the corresponding signal. You can hide them with the **Show "View trace" links** and **Show "View logs" links** toggles in the data source's [trace](/integrations/connectors/data-visualization/grafana/config#disabling-view-trace-links) and [log](/integrations/connectors/data-visualization/grafana/config#disabling-view-logs-links) configuration sections.
