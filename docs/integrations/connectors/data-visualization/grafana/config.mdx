...
The toggle is enabled by default, and the plugin attaches the link only when the data source is configured for logs.
...
The toggle is enabled by default, and the plugin attaches the link only when the data source is configured for traces.
