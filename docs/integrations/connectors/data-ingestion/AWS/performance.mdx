...
The number of such parallel insert threads can be configured with the [`max_insert_threads`](/reference/settings/session-settings/max-insert#max_insert_threads) setting. The default value is `0` (auto: the number of CPU cores) for open-source ClickHouse since 26.8, and 4 for [ClickHouse Cloud](https://clickhouse.com/cloud).
