...
The S3 endpoint listens on port 8333. Once the stack is up, more buckets can be created at any time with `echo "s3.bucket.create -name mybucket" | docker exec -i seaweedfs weed shell`.
