...
| `auto.evolve` (since v1.3.8)                    | Automatically create new columns when incoming records contain fields not present in the destination table. Only applies to column creation - type changes and column removal are not supported. Requires schema-based data. See [Schema Evolution](#schema-evolution). | `"false"`                                                |
| `auto.evolve.ddl.refresh.retries` (since v1.3.8) | Number of retries when waiting for DDL changes to propagate after schema evolution. | `"3"`                                                    |
| `auto.evolve.struct.to.json` (since v1.3.8)    | When `auto.evolve=true`, automatically map STRUCT fields to [JSON](/sql-reference/data-types/json) columns. Without this flag, STRUCT fields that need auto-evolution will cause an error. See [Schema Evolution](#schema-evolution). | `"false"`                                                |
...
From ClickHouse version 26.9 onwards, this schema is supported by default. In versions 26.1 to 26.8 it requires the setting `allow_experimental_nullable_tuple_type = 1` (see [this documentation page](/reference/settings/session-settings/allow-experimental#allow_experimental_nullable_tuple_type)).
