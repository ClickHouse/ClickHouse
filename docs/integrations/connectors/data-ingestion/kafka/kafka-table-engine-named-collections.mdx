...
### OAuth authentication with `OAUTHBEARER` {#oauth-authentication-with-oauthbearer}

To use OAuth authentication, set `sasl_mechanism` to `OAUTHBEARER` in the named collection's nested `<kafka>` section. The settings in this section are [librdkafka configuration properties](https://github.com/confluentinc/librdkafka/blob/master/CONFIGURATION.md): ClickHouse replaces underscores in XML element names with periods before passing them to `librdkafka`.

For example, the following named collection connects to an Azure Event Hubs namespace using the OAuth client credentials flow:
...
<clickhouse>
    <named_collections>
        <domain_one>
            ...
            <kafka>
                <security_protocol>SASL_SSL</security_protocol>
                <sasl_mechanism>OAUTHBEARER</sasl_mechanism>
                <sasl_oauthbearer_method>oidc</sasl_oauthbearer_method>
                <sasl_oauthbearer_client_id>client-id</sasl_oauthbearer_client_id>
                <sasl_oauthbearer_client_secret from_env="KAFKA_DOMAIN_ONE_CLIENT_SECRET"/>
                <sasl_oauthbearer_token_endpoint_url>https://login.microsoftonline.com/tenant-id/oauth2/v2.0/token</sasl_oauthbearer_token_endpoint_url>
...
Create a separate named collection for every Event Hubs namespace. Each collection can set its own `kafka_broker_list` and `sasl_oauthbearer_scope`; use the collection when creating its Kafka table:

```sql
CREATE TABLE domain_one_queue
(
    message String
)
ENGINE = Kafka(domain_one);
```

The `from_env` attribute reads the client secret from the ClickHouse server process environment. See [substitution by environment variables](/concepts/features/configuration/server-config/configuration-files#from_env_zk) for deployment and default-value details. Do not put OAuth properties at the top level of a named collection: only values under `<kafka>` are passed to `librdkafka`.
