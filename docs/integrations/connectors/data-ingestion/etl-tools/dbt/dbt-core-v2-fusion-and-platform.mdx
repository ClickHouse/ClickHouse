---
sidebarTitle: 'dbt Core v2, Fusion and Platform (Beta)'
slug: /integrations/dbt/dbt-core-v2-fusion-and-platform
description: 'Status of the ClickHouse adapter in dbt Core v2, dbt Fusion and the dbt platform'
keywords: ['dbt', 'dbt core v2', 'dbt v2', 'dbt fusion', 'dbt platform']
title: 'dbt Core v2, Fusion and Platform'
doc_type: 'reference'
---
...
ClickHouse is the first community adapter on dbt v2 and in the dbt platform. dbt's own first-party adapters are launching alongside us; we are the first from outside dbt Labs. The dbt Core 1.x Python adapter, `dbt-clickhouse`, is not going away: we keep it current with dbt 1.x releases and maintain both adapters in parallel.
...
<Note>
**The dbt Core 1.x documentation also applies to dbt Core v2.** The adapter behaves the same way on both engines: the profiles, model configs, materializations and macros described in the other dbt pages work unchanged on dbt Core v2, dbt Fusion and the dbt platform. The exceptions are documented in the [v1 vs v2 parity](#parity) table below.
</Note>

Live status is tracked in [ClickHouse/dbt-clickhouse#660](https://github.com/ClickHouse/dbt-clickhouse/issues/660) (dbt Core v2 parity, one sub-issue per area) and [ClickHouse/dbt-clickhouse#555](https://github.com/ClickHouse/dbt-clickhouse/issues/555) (dbt Fusion).
...
|------|------------|-------------------------------------|
| **dbt Core 1.x** | The Python-based dbt engine, open source (Apache 2.0). Our adapter is `dbt-clickhouse`, maintained in the [ClickHouse GitHub organization](https://github.com/ClickHouse/dbt-clickhouse) and installed alongside `dbt-core`. | **GA, maintained.** Keeps receiving releases in parallel with v2. |
| **dbt Core v2** | Full rewrite of the dbt engine in Rust, open source (Apache 2.0). This new version includes all v1 features plus several performance improvements. | **Beta.** Follow the [dbt v2 upgrade guide](https://docs.getdbt.com/docs/dbt-versions/core-upgrade/upgrading-to-v2) and the [ClickHouse setup page for v2](https://docs.getdbt.com/docs/local/connect-data-platform/clickhouse-setup?version=2) in the dbt documentation. The ClickHouse adapter ships inside the dbt binary; there is no separate package to install. |
| **dbt Fusion** | dbt v2 plus additional features from dbt Labs: SQL comprehension, static analysis, LSP, a VS Code extension and column awareness. Dialect-specific SQL intelligence is built per database. Closed source, licensed under the [dbt Product Licensing Agreement](https://www.getdbt.com/dbt-fusion-engine-license-agreement). | **Beta.** The adapter runs with the Fusion binary, but the new SQL features are not available for ClickHouse yet. |
...
We measure dbt Core v2 against the dbt Core 1.x integration test suite of `dbt-clickhouse`. Beta means that core dbt workflows and the ClickHouse Cloud feature set work with documented limits; full v1 parity and self-managed cluster support come after.
...
The tables below compare the features available in `dbt-clickhouse` (v1) with their status in the dbt Core v2 adapter. Every feature listed is supported in v1 unless the notes say otherwise; the status column is for dbt Core v2. The evolution of every pending item is tracked in [ClickHouse/dbt-clickhouse#660](https://github.com/ClickHouse/dbt-clickhouse/issues/660).
...
| Feature | What it does | dbt Core v2 | Notes |
|---------|--------------|-------------|-------|
