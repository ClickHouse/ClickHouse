...
|--------|------|-------------|
| `_fivetran_synced` | `DateTime64(9, 'UTC')` | Timestamp when the record was synced by Fivetran. Used as the version column for `SharedReplacingMergeTree`. |
| `_fivetran_start` | `DateTime64(9, 'UTC')` | Timestamp when the record revision became active. Part of the table's sorting key. |
| `_fivetran_end` | `Nullable(DateTime64(9, 'UTC'))` | Timestamp when the record revision was superseded. Set to `2262-04-11 23:47:16` for currently active records. |
...
- The superseded record's `_fivetran_end` is set to the new record's `_fivetran_start` minus one nanosecond, and `_fivetran_active` is set to `false`.
