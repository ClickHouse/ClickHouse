...
-   `expr` — Expression resulting in a [رشته](../../sql_reference/data_types/string.md) نوع ارزش. همه بک اسلش باید در ارزش حاصل فرار.
...
-   `x` — Value to be checked for infinity. Type: [شناور\*](../../sql_reference/data_types/float.md).
-   `y` — Fallback value. Type: [شناور\*](../../sql_reference/data_types/float.md).
...
شما می توانید نتیجه مشابه با استفاده از [اپراتور سه تایی](conditional_functions.md#ternary-operator): `isFinite(x) ? x : y`.
...
همان است که برای [عدم پذیرش](./other_functions.md#other_functions-runningdifference), تفاوت ارزش ردیف اول است, ارزش سطر اول بازگشت, و هر سطر بعدی تفاوت از ردیف قبلی را برمی گرداند.
...
بازگرداندن تعدادی از زمینه های در [شمارشی](../../sql_reference/data_types/enum.md).
...
-   `0` برای اعداد.
-   رشته خالی برای رشته.
-   `ᴺᵁᴸᴸ` برای [Nullable](../../sql_reference/data_types/nullable.md).
...
مورد استفاده برای اجرای داخلی [ارریجین](array_join.md#functions_arrayjoin).
...
بازگرداندن ظرفیت فایل سیستم در بایت. برای ارزیابی [مسیر](../../operations/server_configuration_parameters/settings.md#server_configuration_parameters-path) به دایرکتوری داده ها باید پیکربندی شود.
...
تابع شما اجازه می دهد استخراج داده ها از جدول به همان شیوه به عنوان از یک [واژهنامه](../../sql_reference/dictionaries/index.md).

می شود داده ها از [پیوستن](../../engines/table_engines/special/join.md#creating-a-table) جداول با استفاده از کلید ملحق مشخص.
...
اطلاعات بیشتر در مورد `join_use_nulls` داخل [پیوستن به عملیات](../../engines/table_engines/special/join.md).
...
نوع: [رشته](../../sql_reference/data_types/string.md)
