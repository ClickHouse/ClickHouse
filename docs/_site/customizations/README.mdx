...
- `webterminal.js` — adds the drop-down web terminal, the counterpart of the one on clickhouse.com and in `play.html`: a terminal icon in the top navigation, before "Home", and the `~` key both toggle a panel that drops down from the top of the page and embeds `https://play.clickhouse.com/webterminal?user=play` — the playground terminal that clickhouse.com uses as well. The panel covers the navigation bar while open, so it carries its own close button next to the `~`, `Escape` and drag-up ways out. Desktop only, because the icon lives in the desktop tab bar. Two deployment details:
