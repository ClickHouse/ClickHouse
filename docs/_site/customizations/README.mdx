...
- `webterminal.js` — adds the drop-down web terminal, the counterpart of the one on clickhouse.com and in `play.html`: a terminal icon in the top navigation, before "Home", and the `~` key both toggle a panel that embeds `https://play.clickhouse.com/webterminal?user=play` — the playground terminal that clickhouse.com uses as well. Desktop only, because the icon lives in the desktop tab bar. Two deployment details:
    ...
    - The terminal reports Escape and disconnects back to the page over `postMessage`, and accepts the page's handshake only from `clickhouse.com` and `clickhouse.cloud` origins. On preview deployments and on `localhost` the terminal itself still works, but Escape pressed inside it does not close the panel.
