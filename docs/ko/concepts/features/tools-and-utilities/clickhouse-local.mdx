...
* `--echo [ <bool> ]` — 실행 전에 각 쿼리를 출력합니다. 선택적 불리언 값을 받습니다. interactive mode에서는 기본적으로 활성화되고 batch mode에서는 비활성화됩니다. 참고: 이제 `--echo`는 선택적 값을 받으므로, 값 없이 사용한 `--echo` 바로 뒤에 위치 인수 쿼리를 두면 그 쿼리가 값으로 처리됩니다. 대신 `--echo --query "..."`, `--echo -q "..."`, `--echo=false` 또는 파이프로 전달된 `stdin`을 사용하십시오.
