...
<Note>
  **쿼터 계산에서 제외되는 쿼리**
  ...
  쿼터가 소진된 경우에도 쿼터 구성과 사용량을 검사할 수 있도록, 다음 시스템 테이블 중 하나에서만 읽는 `SELECT` 쿼리는 쿼터에 부과되지 않으며 쿼리 복잡성 제한의 적용 대상도 아닙니다:
...
쿼터는 여러 키의 리소스를 각각 독립적으로 보고하기 위해 &quot;쿼터 key&quot; 기능을 사용할 수 있습니다. 다음은 그 예시입니다:
...
정규화된 쿼리 해시를 기준으로 쿼터를 설정할 수도 있으며, 그러면 서로 다른 각 쿼리 패턴에 독립적인 쿼터 버킷이 할당됩니다. XML 구성에서는 이를 `<keyed_by_normalized_query_hash />`로 작성합니다:
