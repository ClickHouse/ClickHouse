...
| `enable_filesystem_query_cache_limit` | 불리언 | `false`    | `max_query_cache_size`를 기준으로 쿼리별 캐시 크기 제한을 활성화합니다.                                                                          |
| `enable_cache_hits_threshold`         | 불리언 | `false`    | 활성화하면 데이터를 여러 번 읽은 뒤에만 캐시합니다.                                                                                               |
