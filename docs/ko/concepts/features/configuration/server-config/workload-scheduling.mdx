...
다음 예시는 위 그림에 표시된 IO 스케줄링 계층 구조를 정의하는 방법을 보여줍니다:
...
기반 리소스의 전체 용량을 활용하려면 `inflight_limit`를 사용해야 합니다. `max_requests` 또는 `max_cost` 값이 너무 낮으면 리소스를 충분히 활용하지 못할 수 있고, 반대로 값이 너무 높으면 scheduler 내부의 큐가 비게 될 수 있습니다. 그러면 결과적으로 하위 트리에서 정책이 무시될 수 있습니다(불공정성이 발생하거나 우선순위가 무시됨). 반면, 리소스 사용률이 지나치게 높아지는 것을 방지하려면 `bandwidth_limit`를 사용해야 합니다. 이는 `duration`초 동안 소비된 리소스의 양이 `max_burst + max_speed * duration`바이트를 초과하면 속도를 제한합니다. 동일한 리소스에 두 개의 `bandwidth_limit` 노드를 두면 짧은 인터벌의 피크 대역폭과 더 긴 인터벌의 평균 대역폭을 제한할 수 있습니다.
