...
<Warning>
  모든 프로필에서 `readonly`는 `changeable_in_readonly` 목록에 넣지 마십시오. 또한 프로필 제약은 사용자별이 아니라 시스템 전체에 적용된다는 점에 유의하십시오.
</Warning>

`readonly` 자체를 `changeable_in_readonly`로 지정하지 마십시오. 그렇게 하면 `readonly = 1` 상태로 시작한 세션이 `SET readonly = 0`을 실행해, 기존 권한으로 이미 허용된 쓰기 쿼리를 다시 실행할 수 있게 됩니다.

이는 사용자에게 할당된 프로필에만 국한된 문제가 아니라 모든 상황에서 중요합니다:

* 프로필 선택은 액세스 검사를 거치지 않으므로(이미 활성화된 설정 제약 조건에 의해서만 제한됨), 어떤 세션이든 `SET profile`을 통해 임의의 프로필로 전환할 수 있습니다.
* HTTP에서는 유효 값이 `0`이 되는 경우에만 `GET` 요청이 강제로 `readonly = 2`로 설정됩니다. 따라서 `readonly = 1`을 설정하면서도 읽기 전용 모드에서 `readonly` 변경을 허용하는 프로필은 이 보호 장치를 무력화합니다. 즉, `GET` 요청이 값을 `0`으로 바꾼 뒤 쓰기를 수행할 수 있습니다.
