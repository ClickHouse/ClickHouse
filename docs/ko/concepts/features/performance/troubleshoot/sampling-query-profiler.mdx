...
<Steps>
  ...
  <Step title={<>Analyze the <code>trace_log</code> system table</>} id="analyze-trace-log-system-table">
    `trace_log` 시스템 테이블을 분석하려면 [`allow_introspection_functions`](/ko/reference/settings/session-settings/allow#allow_introspection_functions) 설정으로 인트로스펙션 함수를 허용하십시오:
