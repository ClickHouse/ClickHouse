...
역할은 위의 역할 클레임에서 읽어 오며, SQL 권한 부여는 기본적으로 토큰의 `clickhouse:grants` 클레임에서 읽어 옵니다. 역할 클레임에 지정된 역할은 서비스에 미리 존재해야 합니다(`CREATE ROLE`로 생성). [접근 권한](#access-rights)에 설명된 대로 권한 한도은 역할 및 권한 부여 클레임에서 얻는 유효 접근 권한을 제한합니다.
