...
| `표시 타임스탬프 컬럼`             | UI 표시에 사용하는 선택적 고정밀도 타임스탬프 컬럼입니다. 지정하지 않으면 Timestamp Column을 기본값으로 사용합니다.                                                                                                                                                                                                                                                     | 아니요 | 예           | `Timestamp`                                        |
| `연관된 메트릭 소스`              | 연결된 메트릭 소스(예: HyperDX 메트릭)입니다.                                                                                                                                                                                                                                                                                                | 아니요 | No          | –                                                  |
...
| `알려진 컬럼 목록`               | 대상 테이블의 컬럼 집합이 서로 다른 [`Distributed`](/ko/reference/engines/table-engines/special/distributed) 또는 [`Merge`](/ko/reference/engines/table-engines/special/merge) 테이블의 소스에 사용합니다. 전체 행 데이터(예: 행 측면 패널)를 가져올 때 `SELECT *` 대신 사용할 수 있도록, 모든 대상 테이블에 공통으로 있는 컬럼을 쉼표로 구분해 나열합니다. 모든 컬럼을 선택하려면 비워 두십시오. 컬럼명만 지정할 수 있으며 표현식이나 별칭은 사용할 수 없습니다. | 아니요 | No          | –                                                  |
| `텍스트 인덱스 사용`              | Lucene 기반 검색에서 암시적 컬럼을 검색할 때 `hasAllTokens`를 사용할지 여부입니다.                                                                                                                                                                                                                                                                      | No  | No          | `자동 (스키마에서 자동 감지)`                                 |
