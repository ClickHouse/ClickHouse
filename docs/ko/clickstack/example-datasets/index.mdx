...
| [Synthetic data with telemetrygen](/ko/clickstack/example-datasets/telemetrygen) | `telemetrygen`을 사용해 서비스, 심각도, 스팬 상태, 메트릭 타입 전반에 걸쳐 flag로 형태를 지정한 다양한 합성 logs, traces, metrics를 실행 중인 ClickStack OpenTelemetry collector로 전송합니다 |
| [HackerNews Analyzer](/ko/clickstack/example-datasets/instrument-application)    | Node.js 애플리케이션인 HackerNews Analyzer에 OpenTelemetry 계측을 추가하고 logs, metrics, traces를 Managed ClickStack으로 전송합니다                                  |