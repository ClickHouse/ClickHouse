...
<Steps>
  ...
  <Step title="시각화 만들기" id="create-visualization">
    ...
    * Alias: `Average Time`
    ...
    <Note>
      시각화는 메트릭, 트레이스, 로그 등 모든 데이터 소스에서 만들 수 있습니다. ClickStack은 이 모두를 wide 이벤트로 처리합니다. 모든 **numeric column**은 시간에 따라 차트로 표시할 수 있으며, **string**, **date**, **numeric** 컬럼은 그룹화에 사용할 수 있습니다.
...
<Image img="/images/clickstack/dashboards/number-tile-background-chart.webp" alt="Number 타일의 Display Settings 드로어에서 Background chart가 Area로 설정된 상태" size="lg" />
...
Table 타일을 선택한 상태에서 **Display Settings**를 열고 **Alternate Row Background**를 켜십시오. 줄무늬는 순전히 시각적인 표시이므로 query-builder와 raw SQL Table 타일 모두에서 작동합니다.

<Image img="/images/clickstack/dashboards/table-tile-display-settings.webp" alt="Table 타일의 Display Settings 드로어에서 Alternate Row Background가 켜진 상태" size="lg" />
...
<Steps>
  ...
  <Step title="새 필터 추가" id="add-new-filter">
    **Add new filter**를 클릭합니다. **Name**을 입력하고, **데이터 소스**를 선택하고, **Filter expression**(드롭다운을 채울 고유값을 가져오는 SQL 컬럼 또는 표현식)을 입력해 필터를 구성합니다. 그런 다음 **Save filter**를 클릭합니다.
    ...
    <Image img="/images/clickstack/dashboards/add-filter.webp" alt="Name, 데이터 소스, Filter expression 필드가 있는 필터 추가 대화상자" size="md" />
