...
`color`에는 위의 categorical 및 semantic 목록에 있는 모든 토큰(예: `chart-blue` 또는 `chart-success`)을 사용할 수 있습니다. 선택 사항인 `backgroundChart.color`는 동일한 토큰 enum을 사용하여 스파크라인 색상을 재정의합니다. authentication 및 기준 URL은 [ClickStack API 참조](/ko/clickstack/api-reference)를 참조하십시오.
