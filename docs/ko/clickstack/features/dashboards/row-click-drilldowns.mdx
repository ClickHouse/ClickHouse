...
<Steps>
  ...
  <Step>
    ...
    <Image img="/images/clickstack/dashboards/row-click-drilldown.webp" alt="행 클릭 후의 Service Detail 대시보드. Service 필터가 클릭한 서비스로 설정되어 있고 RED 핵심 지표, 지연 시간 백분위수, endpoint별 분석이 모두 해당 서비스 범위로 한정됨" size="lg" />
  </Step>
</Steps>
...
<Steps>
  <Step>
    ...
    인벤토리 테이블에서 **Row Click Action**을 열고 **검색**을 선택합니다. traces 소스를 선택하십시오(로그 및 trace 소스만 표시됩니다). 필터를 하나 추가합니다.
  ...
  <Step>
    ...
    이제 서비스 행을 클릭하면 해당 traces 소스의 검색 페이지가 열리고, `ServiceName = <service>`로 필터링되므로 동일한 시간 범위에서 해당 서비스의 스팬만 바로 확인할 수 있습니다.

    <Image img="/images/clickstack/dashboards/row-click-search-drilldown.webp" alt="행 클릭 후 traces 소스의 검색 페이지가 열리고, 클릭한 서비스로 필터링된 상태" size="lg" />
  </Step>
</Steps>
