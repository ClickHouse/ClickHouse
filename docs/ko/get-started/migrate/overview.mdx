...
| OSS to Cloud                                                      | 자가 관리형 ClickHouse 배포에서 [`remoteSecure`](/ko/get-started/migrate/oss-to-cloud/clickhouse-to-cloud) 또는 [`BACKUP` 및 `RESTORE`](/ko/get-started/migrate/oss-to-cloud/oss-to-cloud-backup-restore)를 사용하여 데이터를 이전합니다. |
