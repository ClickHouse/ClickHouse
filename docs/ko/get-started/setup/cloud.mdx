...
<Steps>
  ...
  <Step title="데이터 추가" id="3-add-data">
    ...
    * `metric`: 32비트 부동 소수점 수([Float32](/ko/reference/data-types/float))
    ...
    #### 기본 키(Primary Key) 간단히 살펴보기

    더 진행하기 전에 ClickHouse에서 기본 키(primary key)가 어떻게 동작하는지 이해하는 것이 중요합니다(기본 키의
    구현 방식이 예상과 다를 수 있습니다!).

    * ClickHouse의 프라이머리 키는 테이블의 각 행에서 ***고유하지 않습니다***

    ClickHouse 테이블의 프라이머리 키(primary key)는 데이터가 디스크에 기록될 때 정렬되는 순서를 결정합니다. 8,192개의 행 또는 10MB의
    데이터(**인덱스 세분화 수준(index granularity)**이라고 함)마다 프라이머리 키 인덱스 파일에 항목이 하나씩 생성됩니다. 이러한 세분화 개념 덕분에
    메모리에 손쉽게 담을 수 있는 **희소 인덱스(sparse index)**가 만들어지며, 그래뉼은 `SELECT` 쿼리 실행 시 처리되는 최소 단위의
    컬럼 데이터 묶음을 의미합니다.

    기본 키(primary key)는 `PRIMARY KEY` 매개변수로 정의할 수 있습니다. `PRIMARY KEY`를 지정하지 않고 테이블을 정의하면,
    `ORDER BY` 절에 지정된 튜플이 기본 키가 됩니다. `PRIMARY KEY`와 `ORDER BY`를 모두 지정하는 경우, 기본 키는 정렬 순서의 부분 집합이어야 합니다.

    기본 키(primary key)는 정렬 키(sorting key)이기도 하며, `(user_id, timestamp)` 튜플로 구성됩니다. 따라서 각
    컬럼 파일(column file)에 저장되는 데이터는 `user_id` 순으로, 그다음 `timestamp` 순으로 정렬됩니다.
    ...
    [**clickhouse client**](/ko/concepts/features/interfaces/cli)라는 명령줄 도구를 사용하여 ClickHouse Cloud 서비스에 연결할 수도 있습니다. 왼쪽 메뉴에서 `Connect`를 클릭하면 관련 정보를 확인할 수 있습니다. 대화 상자의 드롭다운에서 `Native`를 선택하십시오:
    ...
    데이터베이스를 처음 사용할 때 흔히 수행하는 작업 중 하나는 이미 파일로 보유한 데이터를 삽입하는 것입니다. 온라인에서 바로 삽입해 볼 수 있는
    샘플 데이터를 제공하며, 이 데이터는 클릭스트림 데이터로 사용자 ID, 방문한 URL,
    이벤트 타임스탬프가 포함되어 있습니다.
