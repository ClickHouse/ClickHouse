...
<Steps>
  <Step title="curl을 사용하여 ClickHouse 설치" id="install-clickhouse-using-curl">
    ...
    Linux 및 macOS에서는 ClickHouse CLI인 [`clickhousectl`](/ko/install/clickhousectl)도
    `~/.local/bin`에 설치되며 (`chctl` 별칭 포함), `clickhousectl`을 사용하면
    여러 로컬 ClickHouse 버전을 설치하고 관리하고, 서버를 백그라운드에서 실행하며,
    ClickHouse Cloud를 관리할 수 있습니다. CLI 기반 워크플로는 [clickhousectl 설치 페이지](/ko/install/clickhousectl)를
    참조하십시오.
