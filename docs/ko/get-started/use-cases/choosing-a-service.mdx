...
| 주문, 재고 예약, 상태 전이에 트랜잭션과 제약 조건이 필요한 작업처럼 트랜잭션이 필요한 애플리케이션 상태 | [ClickHouse Cloud의 ClickHouse Managed Postgres](/ko/products/managed-postgres/overview)             | 트랜잭션 경계, 제약 조건, 인덱스, 커넥션 관리, 해당 서비스가 지원하는 기능과 가용성 |
| 트랜잭션 기반 애플리케이션 상태와 더불어 독립적인 서빙 시스템이 필요한 분석 쿼리                    | [ClickHouse Cloud의 Postgres 및 ClickHouse 서비스](/ko/products/managed-postgres/clickhouse-integration) | 복제가 필요한 테이블, 허용 가능한 복제 지연, 스키마 변경, 두 서비스의 비용과 운영       |
...
두 워크로드가 각각 별도의 서비스를 둘 만한 이유가 있다면, 트랜잭션 상태는 Postgres에 유지하고 필요한 테이블만 [ClickPipes](/ko/products/managed-postgres/clickhouse-integration)로 ClickHouse에 복제하십시오. 이때 복제 지연(replication lag)을 감안해야 하며, 최신 애플리케이션 상태가 필요한 판단에는 계속 트랜잭션 원본을 사용하십시오. 복제한다고 해서 Postgres 쓰기와 ClickHouse 읽기가 하나의 트랜잭션으로 묶이지는 않습니다.
