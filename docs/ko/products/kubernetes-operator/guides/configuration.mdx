...
오퍼레이터는 모든 ClickHouse 파드와 해당 퍼블릭 헤드리스 Service에 고정된 포트 집합을 노출합니다: `8123` HTTP, `9000` 네이티브, `9009` interserver, `9001`/`9002` management, `9363` Prometheus 메트릭, 그리고 TLS가 활성화된 경우 `8443`/`9440`의 TLS 변형 포트도 함께 노출됩니다. `interserver` 및 management 포트는 레플리카별 내부 Service를 통해서도 추가로 노출되므로, 레플리카가 Ready 상태가 되기 전에도 레플리카와 오퍼레이터가 서로 통신할 수 있습니다. ClickHouse가 MySQL, PostgreSQL, gRPC 또는 사용자 지정 포트 같은 추가 프로토콜을 수신하도록 하려면 `spec.additionalPorts`에 선언하십시오:
...
연산자는 해당 포트를 파드의 `containerPorts`와 퍼블릭 헤드리스 Service에 추가합니다.
...
모든 레플리카 파드에는 `clickhouse.com/ReplicaInitialized` readiness gate가 부여되므로, 새 레플리카는 오퍼레이터가 초기화를 완료한 후에만 퍼블릭 헤드리스 Service를 통해 게시됩니다. 초기화 과정에서는 `default` 데이터베이스가 Replicated 엔진으로 변환되고 스키마가 동기화됩니다. 그때까지는 오퍼레이터와 다른 레플리카가 내부 Service를 통해 해당 레플리카에 접근합니다. `enableDatabaseSync: false`인 경우 오퍼레이터는 레플리카를 즉시 초기화됨으로 표시하므로 readiness는 컨테이너 probe만을 따르게 됩니다.

오퍼레이터는 데이터가 이미 채워진 비 Replicated `default` 데이터베이스를 절대 삭제하지 않습니다. 이러한 레플리카도 스키마 준비가 끝나면 클라이언트 트래픽을 처리하기 시작하지만, 해당 데이터베이스를 직접 해결할 때까지 클러스터는 reason이 `DefaultDatabaseNotReplicated`인 `SchemaInSync=False`를 보고합니다.

스케일 다운 시 레플리카를 제거하기 전에, 오퍼레이터는 먼저 해당 레플리카에서 클라이언트 트래픽을 언로드하고, 게시가 중단될 때까지 기다린 뒤 남은 데이터를 살아 있는 레플리카에 복제한 다음에야 레플리카를 삭제합니다.
