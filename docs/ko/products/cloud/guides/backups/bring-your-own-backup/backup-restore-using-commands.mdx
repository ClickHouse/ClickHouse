...
<Tabs>
  ...
  <Tab title="복원">
    ...
    이는 백업에 복원을 수행하는 사용자가 부여할 수 있는 권한보다 많은 권한이 포함된 경우 `RESTORE`가 `ACCESS_DENIED`로 실패하는 대신, [`GRANT CURRENT GRANTS`](/ko/sql-reference/statements/grant)와 동일한 의미 체계를 적용합니다.
