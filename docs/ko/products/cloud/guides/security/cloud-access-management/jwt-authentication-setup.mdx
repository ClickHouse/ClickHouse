...
3. 검증에 성공하면 연결은 ephemeral user로 실행됩니다. 이 사용자의 접근 권한은 토큰의 `clickhouse:grants` 및 `clickhouse:roles` 클레임에서 가져오며, permission limit(`default` 사용자)을 초과할 수 없습니다. 자세한 내용은 [접근 권한](/ko/concepts/features/security/external-authenticators/jwt#access-rights)을 참조하십시오.
