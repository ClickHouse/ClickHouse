...
<EnterprisePlanFeatureBadge feature="사용자 정의 아이덴티티 제공자를 통한 JWT 인증" support="true" />

<Tip>
  이 가이드에서는 Cloud 콘솔에서 JWKS 제공자를 구성하는 방법을 다룹니다. JWT 생성 방법과 JWT의 구조(필수 클레임, 역할 및 권한 부여 클레임, 임시 사용자의 작동 방식)에 관해서는 [JWT 인증](/ko/operations/external-authenticators/jwt) 참고 문서를 참조하십시오.
</Tip>
...
조직에서 이 기능을 활성화하면 ClickHouse Cloud 콘솔의 **설정 → 보안**에서 서비스별로 JWKS 제공자를 직접 구성할 수 있습니다.

<Note>
  조직에서 제공자를 활성화하려면 [지원](https://clickhouse.com/support/program)에 문의하십시오.
</Note>
...
* 최소 1개의 **RSA** 키를 게시하는 공개적으로 접근 가능한 **HTTPS** JWKS URL(토큰은 **RS256**으로 서명되어야 함)

<Note>
  JWKS 기반 provider는 **RSA 키만** 허용합니다. HMAC(`HS256`) 또는 EC(`ES256`) 키로 서명된 토큰은 엔드포인트를 통해 검증할 수 없으므로 거부됩니다. JWKS 문서에 다른 키 타입이 포함될 수는 있지만, 사용 가능한 RSA 키가 최소 1개 있어야 합니다.
</Note>
...
클라이언트(IdP(Identity Provider) 또는 애플리케이션)는 JWT를 생성하고 **private key**로 서명합니다. 토큰은 예상되는 [토큰 형식](/ko/operations/external-authenticators/jwt#token-claims)을 따라야 합니다. 이후 ClickHouse는 JWKS URL에 게시된 **공개** 키를 사용하여 토큰을 검증합니다.

1. ClickHouse는 토큰의 `kid`(key ID) header를 읽고 문서에서 일치하는 key를 선택합니다.
2. 해당 RSA 공개 키로 토큰 서명을 검증하고, `iss`(issuer) 및 `aud`(audience) 클레임을 provider 구성과 대조합니다.
3. 검증에 성공하면 연결은 ephemeral user로 실행됩니다. 이 사용자의 접근 권한은 토큰의 `clickhouse:grants` 및 `clickhouse:roles` 클레임에서 가져오며, permission limit(`default` 사용자)을 초과할 수 없습니다. 자세한 내용은 [접근 권한](/ko/operations/external-authenticators/jwt#access-rights)을 참조하십시오.
...
<Steps>
  ...
  <Step title="provider 플라이아웃 열기" id="open-flyout">
    **Set up JWT providers**를 선택합니다(이미 provider가 있는 경우 **Manage JWT providers**). 입력할 새 provider 양식이 표시된 플라이아웃이 열립니다.
  </Step>
  ...
  <Step title="provider 세부 정보 입력" id="fill-provider-details">
    ...
    | **JWKS URL**            | 토큰 서명 검증에 사용하는 RSA 공개 키를 게시하는 공개 HTTPS URL입니다.                                                            |
...
<Warning>
  제공자를 삭제하면 해당 provider용으로 발급된 토큰은 즉시 허용되지 않으며, 해당 토큰으로 계속 인증하는 모든 워크로드가 작동을 멈춥니다.
</Warning>
...
* 서비스당 제공자는 최대 **5개**까지 사용할 수 있습니다.
* JWKS provider는 **RSA / RS256** key만 지원합니다.
...
* [JWT 인증](/ko/operations/external-authenticators/jwt) — 토큰 클레임, 임시 사용자 및 클라이언트 사용법.