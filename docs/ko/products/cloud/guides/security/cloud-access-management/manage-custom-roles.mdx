...
사용자 지정 역할에는 조직, service, 데이터베이스 권한을 조합하여 포함할 수 있습니다. 권한은 모든 service와 데이터베이스 또는 그 부분 집합에 적용할 수 있습니다.
...
<Steps>
  ...
  <Step>
    ### 오른쪽 상단에서 `Create new role`을 선택합니다
  ...
  <Step>
    ...
    `Allow` 버튼을 클릭한 다음 조직, Service 및/또는 Database 권한 중에서 선택합니다. 모든 권한에 대한 설명은 [Console roles and permissions](/ko/products/cloud/reference/security/console-roles)를 참조하십시오.

    <Tip>
      console에 로그인할 사용자는 최소한 조직 &gt; Access 조직 권한이 있어야 합니다.
    </Tip>
