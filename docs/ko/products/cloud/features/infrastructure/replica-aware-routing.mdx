...
<Warning>
  **HTTP 인터페이스 필요**
  ...
  Replica-aware 라우팅은 [HTTP/HTTPS 인터페이스](/ko/concepts/features/interfaces/http)를 통해 프록시 계층에서 적용됩니다. ClickHouse Cloud는 Replica-aware 라우팅 방식을 `session_id`에서 `X-ClickHouse-Replica-Tag` 헤더로 전환하고 있습니다. 아래 탭에서는 롤아웃 기간 동안 두 방법을 모두 설명합니다.
...
<Tabs>
  ...
  <Tab title="session_id (legacy)">
    ...
    * ClickHouse HTTP 세션 상태가 필요하면 동일한 `session_id`를 공유하는 요청을 순차적으로 처리하십시오.
    ...
    워크로드를 특정 레플리카에 고정하려면 [HTTPS 인터페이스](/ko/concepts/features/interfaces/http)에서 `session_id` 쿼리 매개변수를 전송하십시오. 프록시는 매개변수 값을 일관된 해싱에 사용하므로 레플리카 수가 변경되지 않는 한 이를 공유하는 요청은 동일한 레플리카로 라우팅됩니다. 다른 값은 독립적으로 해싱되며 동일하거나 다른 레플리카에 배정될 수 있지만, 값이 *어느* 레플리카에 매핑될지는 선택할 수 없습니다.
    ...
    모든 레플리카에 걸쳐 더 강력한 보장이 필요한 경우, ClickHouse Cloud에서 [`select_sequential_consistency`](/ko/reference/settings/session-settings#select_sequential_consistency)를 `1`로 설정할 수도 있습니다.
