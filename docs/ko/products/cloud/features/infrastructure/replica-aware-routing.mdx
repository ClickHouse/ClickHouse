...
description: '쓰기 후 읽기 일관성, 임시 테이블, 세션, 캐시 재사용을 위해 관련 요청을 동일한 ClickHouse Cloud 레플리카로 전달합니다'
keywords: ['클라우드', 'sticky 엔드포인트', 'sticky', '엔드포인트', '스티키 라우팅', '라우팅', 'Replica-aware 라우팅', 'session_id', '세션 어피니티', '쓰기 후 읽기 일관성', '임시 테이블']
doc_type: 'guide'
noindex: true
---
...
Replica-aware 라우팅(스티키 세션, 스티키 라우팅 또는 세션 어피니티라고도 함)은 관련 요청을 동일한 ClickHouse 레플리카로 라우팅합니다. 쓰기 후 읽기 일관성(read-after-write consistency), [임시 테이블](/ko/sql-reference/statements/create/table#temporary-tables) 또는 [이름이 지정된 세션 상태](/ko/interfaces/http#using-clickhouse-sessions-in-the-http-protocol)가 여러 쿼리에서 계속 접근 가능해야 하거나, 관련 쿼리가 동일한 레플리카의 로컬 캐시를 재사용하도록 하려는 경우 사용하십시오.
...
쓰기 작업과 그 이후의 읽기 쿼리에는 동일한 `session_id`를 사용합니다. Replica-aware 라우팅은 이러한 요청을 같은 레플리카로 보내므로, 다른 레플리카가 아직 이를 받지 못했더라도 읽기 시 최신 쓰기 결과를 확인할 수 있습니다. 따라서 `session_id`가 계속 같은 레플리카에 매핑되는 동안 쓰기 후 읽기 일관성이 보장됩니다.
