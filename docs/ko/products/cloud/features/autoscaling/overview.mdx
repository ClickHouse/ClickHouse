...
스케일링은 클라이언트 요구를 충족할 수 있도록 사용 가능한 리소스를 조정하는 기능입니다. Scale 및 Enterprise(표준 1:4 프로필 사용) tier 서비스는 API를 프로그래밍 방식으로 호출하거나 UI에서 설정을 변경해 시스템 리소스를 조정함으로써 수평으로 스케일링할 수 있습니다. 또한 이러한 서비스는 애플리케이션 요구에 맞춰 수직으로 **자동 스케일링**될 수 있습니다.

<ScalePlanFeatureBadge feature="자동 수직 스케일링" />

<Note>
  Scale 및 Enterprise tier는 단일 및 다중 레플리카 서비스를 모두 지원하는 반면, Basic tier는 단일 레플리카 서비스만 지원합니다. 단일 레플리카 서비스는 크기가 고정되도록 설계되어 수직 또는 수평 스케일링을 지원하지 않습니다. 서비스를 스케일링하려면 Scale 또는 Enterprise tier로 업그레이드할 수 있습니다.
</Note>
