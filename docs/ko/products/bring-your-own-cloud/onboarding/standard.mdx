...
<Steps>
  ...
  <Step title="BYOC 인프라 구성" id="setup-byoc-infrastructure">
    ...
    * **VPC/VNet CIDR 범위**: 기본적으로 BYOC VPC(AWS/GCP) 또는 VNet(Azure)의 CIDR 범위로 `10.0.0.0/16`을 사용합니다. 다른 계정과 VPC/VNet 피어링을 사용할 계획이라면 CIDR 범위가 겹치지 않도록 하십시오. 필요한 워크로드를 수용할 수 있도록 BYOC에 최소 `/23` 크기의 적절한 CIDR 범위를 할당하십시오. `/23`의 최소 범위로는 서버 파드를 약 30개만 수용할 수 있습니다. 레플리카마다 IP 주소를 사용하므로 배포 규모가 커질 것으로 예상되면 더 큰 범위(최소 `/22` 권장)를 할당하십시오.
    ...
    * **가용 영역**: VPC 피어링을 사용할 계획이라면 소스 계정과 BYOC 계정의 가용 영역을 일치시키면 AZ 간 트래픽 비용을 줄이는 데 도움이 될 수 있습니다. 예를 들어 AWS에서는 가용 영역 접미사(`a`, `b`, `c`)가 계정마다 서로 다른 물리적 영역 ID를 나타낼 수 있습니다. 자세한 내용은 [AWS 가이드](https://docs.aws.amazon.com/prescriptive-guidance/latest/patterns/use-consistent-availability-zones-in-vpcs-across-different-aws-accounts.html)를 참조하십시오.
    ...
    <Image img="/images/cloud/reference/byoc-onboarding-3.webp" size="lg" alt="BYOC 인프라 설정" background="black" />
  </Step>
</Steps>
