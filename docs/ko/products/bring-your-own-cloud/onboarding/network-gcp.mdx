...
<Steps>
  <Step title="PSC 서비스 설정 요청" id="step-1-request-psc-setup">
  ...
  <Step title="GCP PSC 서비스 이름 및 DNS 이름 확인" id="step-2-obtain-gcp-service-attachment-and-dns-name-for-private-service-connect">
  ...
  <Step title="네트워크에 PSC endpoint 생성" id="step-3-create-endpoint">
  ...
  <Step title="엔드포인트의 프라이빗 DNS 이름 설정" id="step-4-set-private-dns-name-for-endpoint">
    <Note>
  ...
  <Step title="엔드포인트의 PSC 연결 ID를 서비스 허용 목록에 추가" id="step-5-add-endpoint-id-allowlist">
  ...
  <Step title="PSC를 통해 ClickHouse에 연결하기" id="step-6-connect-via-psc-endpoint">
