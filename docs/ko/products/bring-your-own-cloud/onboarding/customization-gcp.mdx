...
<Steps>
  ...
  <Step title="BYOC 인프라 설정" id="set-up-byoc-infrastructure">
    ...
    5. VPC가 공유 VPC 호스트 프로젝트에 있는 경우 **공유 VPC host project ID**를 입력하십시오. VPC가 인프라와 동일한 프로젝트에 있다면 비워 두십시오. 아래 [호스트 프로젝트의 공유 VPC](#shared-vpc-host-project)를 참조하십시오.
    6. 프로비저닝을 시작하려면 **Set up Infrastructure**를 클릭하십시오.

    <Image img="/images/cloud/reference/byoc-gcp-existing-vpc-ui.webp" size="lg" alt="GCP용으로 Use existing VPC가 선택되어 VPC network name, subnet name, secondary range names, 공유 VPC host project ID 필드를 보여주는 ClickHouse Cloud BYOC 설정 UI" />
  </Step>
</Steps>
