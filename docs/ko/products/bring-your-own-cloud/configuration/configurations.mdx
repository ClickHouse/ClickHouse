...
기본적으로 BYOC 클러스터의 Kubernetes API 서버 엔드포인트는 공용 인터넷에서 접근할 수 있지만, IP 필터링을 통해 ClickHouse NAT Gateway IP에서만 접근이 허용됩니다. 보안을 더욱 강화하려면 Kubernetes API 서버가 Tailscale을 사용하는 비공개 네트워크 연결을 통해서만 접근 가능하도록 제한할 수 있습니다.
