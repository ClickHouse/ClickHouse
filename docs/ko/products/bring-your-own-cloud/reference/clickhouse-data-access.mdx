...
| --------------- | ------------------------------------------------------------------------------------- |
| State exporter  | ClickHouse Cloud가 소유한 `SQS` 큐로 서비스 및 Backup 상태(헬스, 상태 — Backup 내용이 아닌 운영 이벤트)를 전송합니다. |
| Billing scraper | ClickHouse Cloud가 소유한 객체 스토리지 버킷으로 CPU 및 메모리 메트릭을 전송합니다.                              |
