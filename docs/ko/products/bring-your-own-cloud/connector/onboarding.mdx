...
<Steps>
  <Step title="커넥터 엔드포인트 및 등록 토큰 확인" id="get-endpoint-and-token">
    ...
    토큰은 일회용이며 빠르게 만료되므로, 토큰을 받은 직후에 등록을 진행하십시오. 토큰은 시크릿으로 취급해야 합니다. CLI는 화면에 표시되지 않는 프롬프트나 stdin의 첫 번째 줄에서만 토큰을 읽으며, 명령줄 인수나 디스크, 로그에서는 읽지 않습니다. 토큰이 먼저 만료된 경우 ClickHouse 어카운트 팀에 새 토큰을 요청하십시오.
  </Step>
...
* **Kubernetes에서는** 권한 부여(grant)가 거부되면 차트가 배포되기 전에 `init`이 중단됩니다. 실행기가 부여된 ServiceAccount에 바인딩되므로 해당 권한 없이는 시작할 수 없기 때문입니다. 스테이징된 오버레이, 시크릿, CSR은 그대로 유지되며, `init`은 클러스터 관리자 권한이 있는 호스트에서 실행해야 할 두 개의 명령어를 출력합니다. 해당 명령어를 실행한 뒤 동일한 `init` 명령어를 다시 실행하여 계속 진행하십시오:
