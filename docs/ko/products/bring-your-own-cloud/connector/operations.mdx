...
ClickHouse 계정 팀에 새 등록 토큰을 요청한 후, `--enroll` 및 `--force`를 지정하여 기존 `init` 명령을 다시 실행하십시오. `--force`는 기존 구성을 다시 준비하므로, 최초 설치 시 사용한 모든 대상별 플래그(`--target-namespace`, `--values` 및 모든 `--chart`, `--chart-repo`, `--chart-version` 미러 플래그)를 그대로 유지하십시오. 관리형 모드는 대체되는 구성에서 그대로 이어지므로 여기서 `--managed`는 선택 사항입니다. 기본 설치에서는 다음과 같습니다:
