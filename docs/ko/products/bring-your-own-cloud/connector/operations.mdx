...
각 데몬은 상태 확인 포트에서 `/livez` 엔드포인트를 제공합니다. 응답 본문의 JSON `status` 필드는 HTTP 상태 코드가 아닌 상태 신호이므로, `200`이 아닌 본문을 확인하십시오. Prometheus 메트릭은 동일한 포트의 `/metrics`에서 제공되며, 별도의 메트릭 포트는 없습니다. 두 대상의 기본 포트는 다음과 같습니다.
...
여기서 설명하는 관리형 모드 동작에는 [지원되는 빌드](/ko/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)가 필요합니다. 공개된 `v0.16.0`에는 `--managed`와 `--no-managed`가 모두 없습니다.

ClickHouse 계정 담당 팀에 새 등록 토큰을 요청한 후, `--enroll` 및 `--force`를 지정하여 기존 `init` 명령을 다시 실행하십시오. `--force`는 기존 구성을 다시 준비하므로, 최초 설치 시 사용한 모든 대상별 플래그(`--target-namespace`, `--values` 및 모든 `--chart`, `--chart-repo`, `--chart-version` 미러 플래그)를 그대로 유지하십시오. 관리형 모드는 대체되는 구성에서 그대로 이어지므로 여기서 `--managed`는 선택 사항입니다. 기본 설치에서는 다음과 같습니다:
...
Kubernetes에서는 `init`이 교체된 `clicklink-hmac` 및 `clicklink-mtls` 시크릿을 미리 생성하며, 차트는 이를 렌더링하지 않습니다. 따라서 차트의 체크섬 annotation에도 이들 시크릿이 포함되지 않으며, 실행 중인 파드는 시작 시점의 자격 증명을 계속 사용합니다. 교체 후에는 모든 구성 요소를 다시 시작하십시오:
...
직접 등록한 각 인스턴스에 대해 connector의 읽기 전용 사용자를 다시 프로비저닝합니다. Kubernetes에서는 워크스테이션에서 명령어를 실행하십시오. `--apply-ch-grants`는 재생성된 권한을 파드 내에서 다시 적용하여 새 자격 증명이 ClickHouse에 반영되도록 합니다. admin user에 password가 설정되어 있으면 `--ch-admin-password-stdin`을 추가하고 파이프로 전달하십시오:
...
VM에서는 admin password를 파이프로 전달하여 호스트에서 실행하십시오. VM에서는 `--ch-user-via cr`을 전달하지 않는 한 password가 필수입니다:
...
<Note>
  공개된 `v0.16.0` CLI에는 `--managed`와 `--no-managed`가 모두 없습니다. 아래에 설명된 관리형 모드 유지 및 재정의 동작에는 [관리형 모드 지원](/ko/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)이 포함된 빌드가 필요합니다. 복구 중에 해당 모드를 변경하기 전에 계정 담당 팀에 빌드를 확인하십시오.
</Note>
...
**커넥터 오프라인.** 커넥터가 중단되었거나 연결할 수 없는 동안에도 커넥터가 관리하는 서비스는 계속 실행됩니다. 클러스터의 ClickHouse Operator가 서비스를 유지하며, 실행기는 데이터 경로에 포함되지 않습니다. 라이프사이클 변경에는 실행기 연결이 필요하므로, 다시 연결될 때까지는 어떤 변경도 적용되지 않습니다. 재시작 시 실행기는 중지 시점에 진행 중이던 명령어를 자체 명령 데이터베이스에서 복구합니다. ClickHouse Cloud가 다시 전달하는 명령어는 두 번 실행되지 않고 기록된 결과로 응답합니다. 실패한 명령어만 다시 실행됩니다. 연결된 실행기가 없는 동안 라이프사이클 명령어에 어떤 일이 발생하는지는 [커넥터가 오프라인일 때](/ko/products/bring-your-own-cloud/connector/managed-services#when-the-connector-is-offline)를 참조하십시오.
...
실행기의 클러스터 전체 grant는 호스트가 아니라 클러스터에 존재합니다. 관리형 서비스가 더 이상 남아 있지 않으면 클러스터 관리자 권한으로 이를 제거하십시오:
...
이 grant는 `clicklink-system` 네임스페이스도 생성했으므로, 다른 곳에서 사용하지 않는다면 삭제하십시오.
...
실행기의 PersistentVolumeClaim에는 `helm.sh/resource-policy: keep` 어노테이션이 지정되어 있으므로, 제거(uninstall) 후에도 명령 데이터베이스와 서비스 레지스트리를 담은 채로 남아 있습니다. `init`이 생성한 시크릿, 플랫폼 승인 과정에서 기록된 플랫폼 번들 시크릿, 실행기가 유지 관리하는 서비스 레지스트리 ConfigMap도 차트 소유 리소스가 아니므로 제거 후에도 그대로 남습니다. 이러한 리소스는 직접 등록한 모든 인스턴스와 Troubleshooter를 프로비저닝한 모든 관리형 서비스의 액세스 시크릿 및 ServiceAccount 쌍(서비스 이름을 따서 명명됨)까지 포함하여 명시적으로 삭제하십시오. `--ignore-not-found`를 사용하면 최초 승인 이전의 플랫폼 번들 시크릿처럼 생성된 적이 없는 객체도 문제없이 처리됩니다:
...
`init`은 차트가 아니라 실행기의 클러스터 전역 grant를 적용했습니다. 남아 있는 관리형 서비스가 없으면 클러스터 관리자 권한으로 해당 grant를 제거하십시오:
