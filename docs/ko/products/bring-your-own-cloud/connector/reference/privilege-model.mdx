...
<Note>
  여기에서 설명하는 관리형 모드 워크플로우를 사용하려면 계정 담당 팀이 확인한 [호환되는 CLI 및 배포 지원](/ko/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)이 필요합니다. 공개된 `v0.16.0`에는 `init --managed`와 `init --no-managed`가 모두 없으며, 고객 생성 워크플로우나 `platform` 명령도 포함되어 있지 않습니다. 아래 권한은 해당 지원이 제공된 이후의 워크플로우를 설명합니다.
</Note>

## 아이덴티티 {#identities}

관리형 모드 등록, 고객 서비스 생성, 플랫폼 승인은 [지원되는 빌드 및 배포](/ko/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)에서만 이러한 아이덴티티를 사용합니다.
...
* **커넥터 EC2 VM의 인스턴스 프로필**은 직접 플랫폼 액세스를 위해 사용되며, 차트 로그인과 이미지 검사 모두에서 환경의 읽기 전용 ECR puller 역할을 assume하는 데 사용됩니다. 승인 CLI도 동일한 인스턴스 프로필 경로를 사용하며, workstation의 AWS 프로필이나 SSO 자격 증명이 이를 대체하지 않습니다. 다른 ECR 차트 레지스트리는 ambient AWS 자격 증명을 사용합니다. S3나 IAM 호출은 하지 않습니다. 이미지 pull은 사용자의 노드에서 해당 노드의 pull 역할로 수행됩니다. 실행기는 사용자의 버킷이나 IAM에 대한 자격 증명을 보유하지 않습니다. [레지스트리 자격 증명](/ko/products/bring-your-own-cloud/connector/platform-updates#registry-credentials)을 참조하십시오.
...
커넥터가 여는 연결의 전체 목록입니다. 모든 연결은 사용자 환경 내부에서 시작됩니다.
...
| ClickHouse 인스턴스                    | ClickHouse native protocol                                                                                 | `pcm_scraper` 및 `pcm_troubleshooter`로 수행하는 읽기 전용 쿼리와 스크레이퍼의 로그 플러시 구문([권한 부여](#clickhouse-grants) 참조); 실행기는 ClickHouse 연결을 열지 않음                                                                                                                           |
...
| Amazon ECR                         | HTTPS, 직접 플랫폼 액세스를 위해 커넥터 EC2 VM의 인스턴스 프로필을 사용하여 assume하는 읽기 전용 풀 역할; 그 외 ECR 차트 레지스트리에는 ambient AWS 자격 증명 | 플랫폼 동기화 또는 생성 시 ECR에서 차트를 가져올 때의 레지스트리 로그인; 플랫폼 업데이트 중 이미지 존재 여부를 확인하기 위한 `DescribeImages`                                                                                                                                                                 |
...
스크레이프 쿼리는 각 시스템 테이블을 `clusterAllReplicas()`로 감싸므로 `READ ON REMOTE` 권한이 필요합니다. `SYSTEM FLUSH LOGS`는 유일한 시스템 수준 권한입니다. 이 권한은 `*_log` 시스템 테이블이 이미 버퍼링한 항목을 영구 저장하도록 강제하여 스크레이프가 최신 데이터를 볼 수 있게 할 뿐, 어떤 데이터도 읽거나 수정하지 않습니다. ClickHouse는 이 권한을 전역 범위에서만 허용하고 로그 테이블에 대해서만 적용하므로, 부여 범위가 실제 기능보다 넓습니다.
...
아래의 `init --managed` 워크플로는 공개 버전 `v0.16.0`에서는 사용할 수 없습니다. 사용하기 전에 [관리형 모드 가용성](/ko/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)을 확인하십시오.

관리형 모드에서는 `clicklink clctl init --managed`(또는 `clicklink clctl executor access grant --cluster` 실행 후 `apply --cluster`)가 사용자의 자격 증명으로 실행기의 권한 부여를 적용합니다. 이 권한 부여는 `pcm-executor` ServiceAccount(Kubernetes에서는 커넥터 네임스페이스, VM 설치에서는 `clicklink-system`), 여기에 바인딩된 클러스터 역할 하나, 그리고 `pcm-executor-prefix-guard` ValidatingAdmissionPolicy로 구성됩니다.

RBAC로는 네임스페이스 접두사를 표현할 수 없으므로 verb는 클러스터 전체 범위로 부여됩니다. 대신 admission 정책이 서비스 접두사(기본값 `ns-`) 밖의 네임스페이스를 대상으로 하는 `pcm-executor`의 모든 쓰기를 거부합니다. 실행기의 홈 네임스페이스에서는 세 가지 쓰기만 허용됩니다. 자체 토큰 갱신, 스크레이퍼가 읽는 `clicklink-instance-registry` ConfigMap, 그리고 Kubernetes에서는 `clicklink-mtls` 시크릿에 다시 기록되는 갱신된 클라이언트 인증서입니다. 이 정책은 해당 ServiceAccount에만 적용되며 `failurePolicy: Fail`과 `validationActions: [Deny]`를 사용합니다. ClusterRoleBinding은 정책과 그 바인딩보다 나중에 가장 마지막으로 적용되므로, 권한 부여가 일부만 적용되더라도 실행기는 보호되지 않은 권한을 갖는 것이 아니라 아무 권한도 갖지 않게 됩니다.

VM에서는 `clicklink clctl executor prepare`가 서비스별 bundle도 함께 적용합니다. 이 bundle은 `ns-<name>` 내부의 `pcm-executor` ServiceAccount, 동일한 resource 계열을 담은 역할, 그리고 해당 네임스페이스 하나에 대해 `get`과 `delete`로 고정된 클러스터 역할로 구성됩니다. 실행기는 해당 bundle이 있으면 그 서비스에 대해 bundle을 사용하고, 없으면 클러스터 전체 권한 부여를 사용합니다. Kubernetes에서는 파드가 항상 커넥터 네임스페이스의 `pcm-executor` ServiceAccount로 동작합니다.
...
공개된 `v0.16.0` CLI에는 `platform` 명령이 없습니다. 이 승인 모델을 사용하려면 [CLI와 배포 측 지원이 모두 필요합니다](/ko/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability).

`clicklink clctl platform approve`는 사용자의 자격 증명으로 플랫폼 업데이트 중 권한에 해당하는 부분을 적용합니다. 적용 대상은 네임스페이스, CustomResourceDefinition, ServiceAccount, 클러스터 역할 및 ClusterRoleBinding, 역할 및 RoleBinding, 웹훅 및 admission 구성, PriorityClass, StorageClass입니다. 이어서 `pcm-platform` ServiceAccount와 해당 클러스터 역할, `pcm-platform-scope-guard` ValidatingAdmissionPolicy 및 바인딩이 존재하는지 확인한 뒤, 2시간 동안 유효한 바운드 토큰을 발급합니다.
...
* **사용자의 버킷이나 IAM에 대한 자격 증명 없음.** `clicklink clctl executor prepare`는 버킷과 서비스별 역할을 생성하고, `clicklink clctl executor teardown`은 이를 제거하며, 두 작업 모두 사용자의 자격 증명으로 수행됩니다. 해당 역할은 그 서비스의 파드만 assume할 수 있습니다. 직접적인 플랫폼 액세스의 경우, 실행기는 커넥터 EC2 VM의 인스턴스 프로필을 사용해 차트 로그인과 이미지 확인 모두에 쓰이는 읽기 전용 ECR puller 역할을 assume합니다. 그 외의 ECR 차트 레지스트리는 [아이덴티티](#identities)에서 설명한 대로 주변 AWS 자격 증명을 사용합니다. 클라우드 API 호출은 ECR과 STS로만 이루어지며, 사용자의 데이터를 읽거나 삭제할 수는 없습니다.
...
* **인바운드 연결 없음.** ClickHouse Cloud는 사용자 환경으로 연결을 여는 일이 결코 없습니다. 유일한 명령 경로는 두 개의 아웃바운드 WebSocket입니다. troubleshooter는 사용자가 활성화한 지원 세션이 진행 중이지 않으면 모든 명령을 거부합니다. 실행기는 사용자가 ClickHouse에 관리를 요청한 서비스에 대해서만, 그리고 위의 범위 내에서만 수명 주기 명령을 수락합니다. 세션 중이라 하더라도 troubleshooter의 접근 범위는 양쪽 모두에서 제한됩니다. ClickHouse 쿼리는 테이블 허용 목록으로 제한되며, 이 허용 목록은 `query_log`와 `text_log`를 거부하므로 해당 권한은 결코 부여되지 않습니다. Kubernetes 접근은 네임스페이스 범위의 역할이 부여하는 읽기 전용 뷰와 파드 로그로 제한됩니다.

## 직접 조치해야 하는 사항 {#what-requires-your-action}

아래의 관리형 모드 enrollment, service 생성 또는 플랫폼 승인 명령어를 사용하기 전에, 계정 담당 팀에 [availability](/ko/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)를 확인하십시오. 해당 워크플로는 공개된 `v0.16.0`에서는 사용할 수 없습니다.
...
* **플랫폼 업데이트.** 플랫폼 component에 대한 각 업데이트는 해당 bundle에 대해 `clicklink clctl platform approve`를 실행한 후에만 적용됩니다. 이때 발급되는 토큰은 2시간 후 만료되며 갱신되지 않습니다. [platform updates](/ko/products/bring-your-own-cloud/connector/platform-updates#approve-a-platform-update)를 참조하십시오.
...
* **네트워크 이그레스.** 강제 적용 CNI 환경에서는 `networkPolicy.allowEgressCIDRs`에 connector 엔드포인트의 CIDR을 등록하기 전까지 어떤 데몬도 해당 엔드포인트에 연결할 수 없습니다. 실행기에는 Kubernetes API server CIDR(`networkPolicy.apiserverCIDRs`)도 필요하며, 이 값은 `init --managed`가 클러스터의 VPC에서 도출합니다. 플랫폼 chart를 Amazon ECR에서 가져오는 경우 ECR과 STS 범위도 추가하십시오.

## 액세스 귀속 방식 {#how-access-is-attributed}

* **배포 아이덴티티.** mTLS 클라이언트 인증서의 common name은 org ID이며, 엔드포인트 host에 단일 DNS name이 바인딩되므로 모든 API 연결은 해당 org로 귀속됩니다. 갱신은 데몬 내부에서 자동으로 이루어지며, 운영자가 키 자료를 직접 다루는 일은 없습니다.
...
* **플랫폼 승인.** 승인한 bundle의 다이제스트는 `pcm-platform` ServiceAccount에 기록되며, 자격 증명으로 적용한 모든 객체에는 실행기가 쓰기 작업 전에 검증하는 콘텐츠 해시가 포함됩니다. 다이제스트가 일치하지 않는 bundle은 거부됩니다.
...
* **`query_log`는 기본적으로 스크레이프 대상에서 제외됩니다.** 이 테이블의 컬럼에는 리터럴 값이 그대로 포함된 원본 SQL이 담기며, 여기에 개인 데이터나 시크릿이 포함될 수 있으므로 직접 추가하지 않는 한 경계 밖으로 나가지 않습니다.
