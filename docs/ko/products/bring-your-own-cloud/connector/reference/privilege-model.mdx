...
* **실행기 노드 또는 호스트의 ambient AWS 아이덴티티**는 두 가지 용도로 사용됩니다. 플랫폼 동기화 또는 생성 작업이 차트를 가져올 때 STS를 통해 Amazon ECR에 로그인합니다. 또한 플랫폼 업데이트 중 이미지 검사를 위해 등록한 읽기 전용 pull 역할을 assume합니다. S3나 IAM 호출은 하지 않습니다. 이미지 pull은 사용자의 노드에서 해당 노드의 pull 역할로 수행됩니다. 실행기는 사용자의 버킷이나 IAM에 대한 자격 증명을 보유하지 않습니다.
...
| Kubernetes API server              | HTTPS                                                           | 스크레이퍼와 Troubleshooter(두 대상 모두)를 위한 네임스페이스 범위 읽기 및 ServiceAccount 토큰 요청; 서비스 네임스페이스 내부에서 실행기가 수행하는 쓰기 작업과, 승인된 플랫폼 업데이트 중 플랫폼 네임스페이스 내부에서의 쓰기 작업; 갱신된 인증서를 저장하기 위한 커넥터 자체 mTLS Secret의 정확한 이름 기반 읽기 및 업데이트(Kubernetes 설치에만 해당하며, VM은 갱신 내용을 로컬 TLS 파일에 기록함) |
| Amazon ECR                         | HTTPS, 실행기의 ambient AWS 아이덴티티 또는 이미지 확인을 위해 assume하는 읽기 전용 풀 역할 | 플랫폼 동기화 또는 생성 시 ECR에서 차트를 가져올 때의 레지스트리 로그인; 플랫폼 업데이트 중 이미지 존재 여부를 확인하기 위한 `DescribeImages`                                                                                                                                                                 |
| AWS STS                            | HTTPS, 실행기 노드 또는 호스트의 ambient AWS 아이덴티티                         | ECR 로그인용 자격 증명과, 이미지 확인을 위해 등록한 읽기 전용 풀 역할로의 assume-role. ECR과 STS는 실행기가 호출하는 유일한 클라우드 API입니다                                                                                                                                                              |
...
| IdP(Identity Provider)의 JWKS 엔드포인트 | HTTPS                                                           | 세션 게이트웨이가 활성화된 경우에 한해 수행되는 운영자 토큰 검증                                                                                                                                                                                                                       |

인바운드 측면에서 각 데몬은 하나의 로컬 상태 확인 포트를 노출하며, 이 포트는 Prometheus 메트릭도 함께 제공합니다. 여기에 선택적으로 활성화하는 세션 게이트웨이가 추가됩니다. 실행기의 로컬 명령 API는 루프백에만 바인딩됩니다. 그 외에는 아무것도 수신 대기하지 않습니다. ClickHouse Cloud는 사용자 환경으로 접속하지 않으며, 두 개의 아웃바운드 WebSocket에 응답할 수만 있습니다.
...
* **서비스 및 플랫폼 네임스페이스 외부에 대한 쓰기 불가.** 스크레이퍼와 troubleshooter에는 `delete`나 `patch` 권한이 없습니다. 이들의 유일한 뮤테이션은 커넥터 자체의 mTLS Secret에 대한 정확한 이름 기반 `update`와 `serviceaccounts/token`에 대한 `create`뿐이며, 후자는 자체 ServiceAccount용 단기 토큰을 발급할 뿐 저장된 객체는 변경하지 않습니다. admission은 서비스 접두사 외부에 대한 실행기의 쓰기를 거부하며, 플랫폼 쓰기는 사용자가 발급한 토큰이 유효한 동안에만 가능합니다.
...
* **삭제 후 정리.** service를 삭제하면 해당 Kubernetes resources만 제거됩니다. 사용자의 자격 증명으로 실행하는 `clicklink clctl executor teardown`은 IAM role을 제거하며, 명시적인 플래그를 지정한 경우에만 데이터와 백업도 제거합니다. [delete a service](/ko/products/bring-your-own-cloud/connector/managed-services#delete-a-service)를 참조하십시오.
