...
아래의 `init --managed` 동작은 [관리형 모드를 지원하는 빌드](/ko/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)가 필요하며, 공개된 `v0.16.0`에서는 해당 플래그가 제공되지 않습니다.
...
| `executor.cluster.account_id`                   | `init`이 기록                             | 해당 클러스터의 AWS 계정입니다. 필수입니다.                                                                                           |
| `executor.cluster.kubeconfig_path`              | 미설정                                    | 실행기가 인증에 사용하는 kubeconfig입니다. 설정하지 않으면 모든 서비스 명령어가 `/etc/clicklink/access/executor` 아래의 액세스 번들을 사용해 실행됩니다.             |
...
| `executor.cluster.delete_instance_data.enabled` | `false`                                | `clicklink clctl executor teardown`이 `--delete-data` 없이도 Retired된 서비스의 데이터를 삭제할지 여부입니다. 실행기 데몬 자체는 어떤 데이터도 삭제하지 않습니다. |
...
| `executor.sync_interval`                        | `30s`                                  | 클러스터에서 서비스 상태를 갱신하는 주기입니다. 양수여야 합니다.                                                                                  |
...
아래의 관리형 모드 초기화 및 플랫폼 승인 통합에는 [이를 지원하는 CLI와 차트](/ko/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)가 필요합니다. 공개된 `v0.16.0`에는 `init --managed`도 `platform` 명령어도 없습니다.
...
| `executor.cluster.inCluster`                    | `false` (`init`은 `true`로 기록)           | 파드의 ServiceAccount로 인증합니다. 파드 내에는 액세스 번들이 존재하지 않으므로, 이것이 Kubernetes 형태를 사용하는 이유입니다.                                                                                                                          |
...
| `networkPolicy.apiserverCIDRs`        | `[]` ([지원되는 빌드](/ko/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)에서 `init --managed`가 클러스터 VPC에서 채움) | Kubernetes API server CIDR입니다. 비어 있으면 API server 이그레스 규칙이 생성되지 않으며, 관리형 Kubernetes에서는 이를 설정할 때까지 데몬이 첫 번째 token 요청에 실패합니다. API server 자체 주소는 클러스터의 수명 동안 변경되므로, 해당 주소가 아닌 VPC의 범위를 고정하십시오.                                                                                                                                  |