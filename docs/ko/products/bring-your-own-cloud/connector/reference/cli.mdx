...
준비된 create를 커넥터 자체의 자격 증명을 사용해 엔드포인트를 통해 ClickHouse Cloud로 제출합니다. ClickHouse Cloud가 할당한 표시 이름, state, 그리고 service name을 사용하는 `watch:` 힌트를 출력합니다. create body는 `prepare`가 작성한 파일에서 가져오거나, `--deployed-name`, `--data-bucket`, `--backup-bucket`, `--irsa-role-arn` 네 가지 flag를 함께 지정해 구성합니다. 동일한 입력으로 다시 시도하면 최초의 create가 반환됩니다.
...
| `--default-user-password-file <path>`                                                               | flag 방식으로 지정할 때 사용하는, `default` USER의 비밀번호를 담은 파일(첫 번째 줄)입니다. 지정하지 않으면 명령어가 비밀번호를 생성해 한 번만 표시하며, 이후 다시 시도할 때는 이 파일을 통해 해당 비밀번호를 전달해야 합니다. 그렇지 않으면 service는 최초 비밀번호를 유지한 채 새로운 비밀번호가 표시됩니다.    |
...
| `--idempotency-key <key>`                                                                           | 다시 시도할 때 최초의 create를 반환받기 위해 재사용하는 key입니다(기본값: 환경과 service name에서 파생).                                                                                                                        |
| `--wait`                                                                                            | service가 `running` 상태가 될 때까지 실행기의 로컬 API를 폴링합니다. 해당 이름에 대한 create가 실패한 경우, 또는 service가 `terminating`, `terminated`, `stale` 상태로 전환되면 즉시 실패 처리합니다.                                             |
...
사용자 본인의 클러스터 자격 증명으로 플랫폼 변경 하나를 승인합니다. 실행기가 스테이징한 번들을 읽어 해시를 계산한 후, 그 안의 모든 차트를 실행기와 완전히 동일한 방식으로 렌더링합니다. 선택한 kube context로 권한 부분을 적용하고, 실행기의 `pcm-platform` 아이덴티티를 확보한 뒤 해당 토큰을 발급합니다. 커넥터 엔드포인트로는 어떠한 요청도 보내지 않습니다. 번들의 차트가 Amazon ECR에 있다면 이를 읽을 수 있는 AWS 자격 증명이 필요합니다. 권한 부분은 네임스페이스, CustomResourceDefinitions, ServiceAccounts, 클러스터 역할 및 바인딩, 역할 및 바인딩, 웹훅 및 admission 구성, PriorityClasses, StorageClass입니다. 실행기는 그다음 워크로드 부분만, 그것도 해당 토큰이 유효한 동안에만 적용합니다. [플랫폼 업데이트](/ko/products/bring-your-own-cloud/connector/platform-updates)를 참조하십시오.
...
| `--output-dir <path>`     | 액세스 루트: 스테이징된 번들을 여기에서 읽고, 토큰은 이 아래 `_platform`에 기록됩니다(기본값 `/etc/clicklink/access/executor`, 환경 변수 `CLCTL_EXECUTOR_OUTPUT_DIR`). |
| `--secret-namespace <ns>` | Kubernetes 설치: 실행기의 로컬 API에서 스테이징된 번들을 읽고, 토큰 번들을 `<output-dir>/_platform` 대신 이 네임스페이스(커넥터 네임스페이스)에 시크릿으로 전달합니다.                 |
...
| `--bundle <path>`         | 재정의: 별도 경로로 전달받은 렌더링된 플랫폼 매니페스트로, 스테이징된 번들 대신 읽습니다. 일반적으로 생략합니다.                                                                 |
