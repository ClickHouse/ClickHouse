...
<Note>
  공개된 `v0.16.0` CLI에는 `--managed`와 `--no-managed`가 모두 없습니다. 아래의 관리형 모드 플래그와 동작에는 더 최신 빌드와 계정 담당 팀이 확인한 배포 지원이 필요합니다. [managed-mode availability](/ko/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)를 참조하십시오.
</Note>

등록 토큰, 저장된 등록 번들 또는 대역 외에서 서명된 인증서를 사용하여 커넥터를 Bootstrap합니다. 한 번의 호출로 구성을 스테이지하고, ClickHouse 액세스를 프로비저닝하며, mTLS 클라이언트 인증서를 가져오고, 배포(Helm 차트 또는 systemd unit)한 후 상태를 확인합니다. `--managed`를 사용하면 감지된 EKS 클러스터에 대해 실행기를 활성화하고 실행기의 클러스터 전체 액세스 권한을 부여합니다. 다시 실행해도 안전합니다. `init`은 구성과 클러스터 UUID를 유지하며, 자격 증명은 원자적으로 덮어쓰고, `--force`를 전달하지 않는 한 기존 클라이언트 키를 재사용합니다. 전체 흐름은 [onboarding](/ko/products/bring-your-own-cloud/connector/onboarding)을, 실행기의 역할은 [관리형 서비스](/ko/products/bring-your-own-cloud/connector/managed-services)를 참조하십시오.
...
| 플래그                         | 설명                                                                                                                                                                                                                                                             |
| --------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--target <shape>`          | 배포 형태: `systemd`(기본값, 현재 사용 중인 VM을 부트스트랩) 또는 `helm`(kubeconfig가 있는 워크스테이션에서 `clicklink-connector` 차트를 스테이지).                                                                                                                                                   |
...
| `--managed`                 | 관리형 모드를 활성화합니다. 실행기를 구동하여 ClickHouse Cloud가 이 클러스터 내 ClickHouse 서비스의 수명 주기를 관리하도록 합니다. 프롬프트를 건너뜁니다. Amazon EKS 클러스터가 필요합니다. 공개 `v0.16.0`에서는 사용할 수 없습니다. [availability](/ko/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)를 참조하십시오. |
| `--no-managed`              | 관리형 모드(실행기)를 비활성화된 상태로 둡니다. 프롬프트를 건너뜁니다. 공개 `v0.16.0`에서는 사용할 수 없습니다. [availability](/ko/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)를 참조하십시오.                                                                                    |
...
| `--skip-provision`          | 스테이지만 수행합니다. 역할별 ClickHouse 액세스 프로비저닝(및 systemd 대상에서는 유닛 활성화와 검증)을 건너뜁니다. `clicklink clctl {scraper,troubleshoot} access provision`을 별도로 실행하십시오. `--target helm`은 실행기의 권한 부여만은 그대로 적용하지만, VM에서는 이마저도 건너뜁니다.                                                    |
...
아래의 `init --managed` workflow에는 [관리형 모드를 지원하는 build](/ko/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)가 필요합니다. 공개된 `v0.16.0`에서는 해당 플래그가 제공되지 않습니다.

`init --managed`는 사용자의 자격 증명으로 실행기의 클러스터 전체 권한 부여를 적용합니다. 적용에 실패하면 `init`이 클러스터 관리자 권한을 가진 kubeconfig에서 실행해야 할 명령어를 출력합니다. VM에서는 `access apply`만 실행하면 되며(bundle이 이미 스테이지되어 있음), Kubernetes에서는 `access grant`를 실행한 뒤 `access apply`를 실행합니다. [onboarding](/ko/products/bring-your-own-cloud/connector/onboarding#install-and-enroll)을 참조하십시오.
...
렌더링된 클러스터 전역 `grant` 번들을 적용합니다. 여기에는 ServiceAccount의 홈 네임스페이스, ServiceAccount, 클러스터 역할 및 ClusterRoleBinding, admission 정책이 포함됩니다. 이어서 ServiceAccount 토큰을 발급해 번들에 포함시킵니다. service는 등록하지 않습니다.

| 플래그                     | 설명                                                                              |
| ------------------------ | ------------------------------------------------------------------------------- |
| `--cluster`              | `<output-dir>/_cluster/`의 클러스터 전역 번들을 적용합니다(이 용도에서는 필수).                        |
| `--config <path>`        | 커넥터 설정 파일(환경 변수 `CLCTL_EXECUTOR_CONFIG`).                                       |
...
managed service 생성에 필요한 사용자 측의 모든 준비 작업을, 첫 번째 실패 지점에서 중단되는 멱등한 단일 실행으로 수행합니다. service 이름을 선택하거나 검사하고, 데이터 및 백업 버킷과 IAM role을 생성하며(또는 직접 제공한 role을 검증), VM에서는 service 네임스페이스에 대한 실행기의 액세스 번들을 렌더링하고 적용합니다. Kubernetes에서는 bundle 단계를 건너뜁니다. 실행기가 자신의 파드 ServiceAccount로 실행되기 때문입니다. 그런 다음 `default` USER의 password를 발급하고(한 번만 표시됨) [`instances create`](#instances-create)에 사용할 생성 본문을 기록합니다. [managed services](/ko/products/bring-your-own-cloud/connector/managed-services#create-a-service)를 참조하십시오.

VM에서는 커넥터 호스트에서 실행됩니다. Kubernetes 설치 환경이라면 AWS credentials와 managed 클러스터용 kube context를 갖춘 workstation에서 실행됩니다. 해당 workstation에는 커넥터 구성 사본(`--config`), 실행기의 로컬 API로 연결되는 포트 포워딩, 그리고 쓰기 가능한 `--output-dir`도 필요합니다.

| 플래그                                              | 설명                                                                                                                                                            |
| ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--config <path>`                                | 커넥터 설정 파일(필수, 환경 변수 `CLCTL_EXECUTOR_CONFIG`). 액세스 번들은 이 파일을 기반으로 렌더링되며, kube context는 파일의 `executor.cluster`와 일치해야 합니다.                                   |
| `--instance <name>`                              | service 이름(환경 변수 `CLCTL_EXECUTOR_INSTANCE`). 기본값: 자동 생성된 이름이며, `<output-dir>/_prepare/<eks-cluster-name>.name`에 기록되어 다음 실행에서 이어서 사용됩니다.                       |
...
| `--restore-from <name>`                          | 이 service가 백업을 복원해 올 대상 service이며, IAM role에는 해당 service의 백업 버킷에 대한 읽기 액세스도 부여됩니다.                                                                            |
| `--region <region>`                              | AWS 리전(기본값: kube context가 가리키는 클러스터의 리전).                                                                                                                     |
| `--token-duration <dur>`                         | ServiceAccount 토큰 유효 기간(기본값 `2160h`, 90일).                                                                                                                 |
| `--force`                                        | 기존 액세스 번들을 재사용하지 않고 전체를 다시 생성합니다.                                                                                                                         |
| `--output-dir <path>`                            | 출력 루트(기본값 `/etc/clicklink/access/executor`, 환경 변수 `CLCTL_EXECUTOR_OUTPUT_DIR`).                                                                               |
...
ClickHouse Cloud에서 삭제한 서비스에 대해 `prepare`가 생성한 항목을 제거합니다. 여기에는 IAM role, `prepare`가 생성한 버킷, 기록된 이름, 해당 서비스에 대한 executor의 레코드가 포함되며, VM에서는 서비스의 클러스터 역할과 ClusterRoleBinding, 로컬 액세스 번들, registry entry도 함께 제거됩니다. executor가 실행 중이어야 하고 서비스는 `terminated`(또는 `stale`) 상태여야 하며, 서비스 네임스페이스에 ClickHouse 클러스터가 아직 남아 있으면 실행이 거부됩니다. 데이터와 백업은 명시적으로 요청하지 않는 한 유지되며, 유지된 버킷에는 `clicklink:retained-from=<instance>` 태그가 다시 지정됩니다. 이 명령은 어떤 것을 삭제하기 전에 모든 항목을 먼저 읽습니다. [서비스 삭제](/ko/products/bring-your-own-cloud/connector/managed-services#delete-a-service)를 참조하십시오.
...
| `--data-bucket <name>`, `--backup-bucket <name>` | executor 레코드에 기록된 버킷을 재정의합니다(기본값: 서비스의 role에 바인딩되었거나 해당 서비스용으로 태그된 버킷).                                                       |
| `--role-name <name>`                             | IAM role 이름(기본값 `CH-S3-<instance>-<region>-00-Role`).                                                                         |
...
이 고객 create 워크플로는 아직 공개 릴리스에서 제공되지 않으며, CLI, 커넥터 엔드포인트, ClickHouse Cloud의 지원이 필요합니다. 사용하기 전에 [관리형 모드 지원 여부](/ko/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)를 확인하십시오.

준비된 create를 커넥터 자체의 자격 증명을 사용해 엔드포인트를 통해 ClickHouse Cloud로 제출합니다. ClickHouse Cloud가 할당한 표시 이름, state, 그리고 서비스 name을 사용하는 `watch:` 힌트를 출력합니다. create body는 `prepare`가 작성한 파일에서 가져오거나, `--deployed-name`, `--data-bucket`, `--backup-bucket`, `--irsa-role-arn` 네 가지 flag를 함께 지정해 구성합니다. 동일한 입력으로 다시 시도하면 최초의 create가 반환됩니다.
...
service 하나를 생성 당시 지정된 저장소 정보와 함께 JSON으로 출력합니다. `--name <name>`은 필수이며 클러스터 지정도 필수입니다. `--cluster`를 전달하거나, `CLCTL_CLUSTER`를 설정하거나, `~/.clicklink/clctl.yaml`에 지정하십시오.

아래 세 가지 하위 명령어는 ClickHouse Cloud를 거치지 않고 실행기의 로컬 API로 명령어를 직접 전송합니다. ClickHouse account team의 요청이 있을 때만 실행하십시오. 각 명령어에는 `--name <name>`과 클러스터 지정(`--cluster`, `CLCTL_CLUSTER`, 또는 `~/.clicklink/clctl.yaml`)이 필요하며, 실행하면 해당 명령어가 JSON으로 출력됩니다. 출력된 `id`를 [`commands get`](#commands-get)에 전달하면 진행 상황을 추적할 수 있습니다. 실패한 명령어의 경우 `result`에 오류 내용이 담깁니다.
...
서비스 하나를 그 자리에서 변경합니다. 사용할 수 있는 패치는 `--create-nlb` 하나뿐입니다. 이 옵션은 내부 NLB를 Kubernetes `Service`로 프로비저닝하여 클러스터 외부의 스크레이퍼가 서버에 접근할 수 있도록 하며, AWS에서만 지원됩니다. 실행기는 다음 동기화 시점에 로드 밸런서로부터 스크레이퍼의 호스트를 백필합니다. `--create-nlb` 없이 실행하면 명령이 `nothing to patch`를 반환하며 실행을 거부합니다.
...
공개 `v0.16.0` CLI에는 `platform` 명령이 없으므로, 해당 릴리스에서는 VM 방식과 Kubernetes 방식의 승인 워크플로 모두 사용할 수 없습니다. Kubernetes 승인에는 아래에 설명된 시크릿 전달을 지원하는 CLI와 차트가 추가로 필요합니다. 두 워크플로 중 어느 것이든 사용하기 전에 [관리형 모드 지원 여부](/ko/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)를 확인하십시오.

사용자 본인의 클러스터 자격 증명으로 플랫폼 변경 하나를 승인합니다. 실행기가 스테이징한 bundle을 읽어 해시를 계산한 후, 그 안의 모든 차트를 실행기와 완전히 동일한 방식으로 렌더링합니다. 선택한 kube context로 권한 부분을 적용하고, 실행기의 `pcm-platform` 아이덴티티를 확보한 뒤 해당 토큰을 발급합니다. 커넥터 엔드포인트로는 어떠한 요청도 보내지 않습니다. 권한 부분은 네임스페이스, CustomResourceDefinitions, ServiceAccounts, 클러스터 역할 및 바인딩, 역할 및 바인딩, 웹훅 및 admission 구성, PriorityClasses, StorageClass입니다. 실행기는 그다음 워크로드 부분만, 그것도 해당 토큰이 유효한 동안에만 적용합니다. [플랫폼 업데이트](/ko/products/bring-your-own-cloud/connector/platform-updates)를 참조하십시오.
...
공개 `v0.16.0` CLI에는 `platform reset`을 포함한 `platform` 명령이 없습니다. [관리형 모드 지원 여부](/ko/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)를 참조하십시오.

서비스가 없는 테스트 클러스터용: 플랫폼 릴리스를 의존성 역순으로 제거하고 실행기의 플랫폼 토큰 bundle을 삭제하여 최초 설치를 다시 수행해볼 수 있도록 합니다. 변경을 시작하기 전에 이 커넥터가 소유한 네임스페이스의 ClickHouse 클러스터 목록을 조회하고, 하나라도 존재하면 작업을 거부합니다. CustomResourceDefinition, RBAC, StorageClass는 그대로 유지됩니다. 다음 플랫폼 동기화 전에 `platform approve`를 다시 실행하십시오.
...
| `--dry-run`                                       | 서비스 존재 여부를 확인하고, 클러스터를 변경하지 않고 reset 계획만 출력합니다.                                        |
...
| 플래그                                                                                         | 설명                                                                                                                                                                                                                                      |
| ------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--provider <name>`                                                                         | 실행 채널입니다. 원격 VM에는 `ssh`, `aws`(SSM), `gcp`(IAP)를 사용하며, 대상 VM에서 직접 실행하는 경우에는 `local`을 사용합니다. 명시적으로 설정하지 않으면 공급자별 플래그를 기반으로 추론됩니다. 채널 플래그를 설정하지 않으면 기본값은 `aws`이며, `local`은 자동으로 추론되지 않으므로 VM에서 직접 실행할 때는 항상 `--provider local`을 전달해야 합니다. |
