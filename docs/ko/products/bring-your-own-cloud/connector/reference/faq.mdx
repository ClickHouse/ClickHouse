...
이미 설치된 환경에서는 실행기를 중지하고 클러스터 관리자 권한을 제거하십시오. 중지하려면 Helm values에서 `executor.enabled: false`로 설정한 뒤 `helm upgrade`를 실행하거나, VM에서 `systemctl disable --now clicklink-executor`를 실행하십시오. 그런 다음 권한을 제거하십시오. 마지막 명령은 ServiceAccount 자체를 제거하며, 해당 네임스페이스는 VM 설치에서는 `clicklink-system`, Kubernetes 설치에서는 커넥터 네임스페이스입니다:
