...
관리형 모드 온보딩(enrollment), 고객 생성, 플랫폼 승인 워크플로는 [계정 담당 팀이 확인한 지원](/ko/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)이 필요하며, 공개된 `v0.16.0`에서는 사용할 수 없습니다.
...
반면 사용자의 조치를 기다리는 작업이 두 가지 있습니다. 첫째, 서비스 생성에는 `clicklink clctl executor prepare`와 `clicklink clctl instances create`가 필요합니다. 버킷과 IAM role은 사용자만 생성할 수 있기 때문입니다. 둘째, 클러스터 내 플랫폼 구성 요소의 업데이트 역시 사용자의 조치를 기다립니다. 실행기가 제안된 각 bundle을 스테이징하면, `clicklink clctl platform approve`로 스테이징된 bundle을 승인하고 해당 bundle에 대한 2시간짜리 토큰을 발급합니다. [플랫폼 업데이트](/ko/products/bring-your-own-cloud/connector/platform-updates)를 참조하십시오.

실행기가 수행하는 모든 작업은 Kubernetes API를 통해 이루어집니다. 실행기의 쓰기는 admission 정책에 의해 서비스 접두사(기본값 `ns-`)가 붙은 네임스페이스로 제한되며, 여기에 자체 네임스페이스 내 세 가지가 추가됩니다: 토큰 갱신, 서비스 registry ConfigMap, 그리고 Kubernetes에서는 `clicklink-mtls` 시크릿에 저장된 갱신된 클라이언트 인증서입니다. 동일한 리소스 계열에 대한 읽기는 클러스터 전체를 대상으로 합니다. 승인된 플랫폼 업데이트는 플랫폼 네임스페이스로 제한된 별도의 `pcm-platform` 아이덴티티로 실행됩니다. 사용자가 직접 운영하는 클러스터는 절대 변경되지 않습니다. 스크레이퍼와 Troubleshooter는 읽기 전용입니다. 정확한 범위는 [권한 모델](/ko/products/bring-your-own-cloud/connector/reference/privilege-model) 페이지에서 확인할 수 있습니다.
...
스크레이퍼는 고정된 system tables 집합에서 조회한 결과를 전송합니다. 기본값은 `metric_log`, `asynchronous_metric_log`, `tables`, `warnings`, `server_settings`입니다. 또한 헬스 및 상태 하트비트, instance 및 Backup 상태, 그리고 커넥터 자체의 메트릭을 전송합니다. 기본 스크레이프 집합에는 `system.query_log`가 포함되지 않으므로, 원시 SQL 텍스트와 그 안에 포함된 literals 또는 개인 데이터는 사용자가 직접 추가하지 않는 한 스크레이프 경로를 통해 외부로 전송되지 않습니다. 세션 중에는 기본 테이블 허용 목록에 `system.processes`가 포함되며, 이 테이블은 실행 중인 query text를 노출합니다. 해당 정보가 노출되지 않아야 한다면 허용 목록에서 제거하십시오.

실행기는 자신이 실행한 각 명령의 결과와 각 managed service의 state 및 레플리카 수를 전송합니다. 실행기의 하트비트는 명령 채널의 연결 여부, 플랫폼 구성 요소 버전, 마지막으로 적용한 bundle의 다이제스트, 그리고 마지막 플랫폼 업데이트의 결과를 보고합니다. VM 설치 환경에서는 하트비트가 Kubernetes 액세스 bundle이 `ok`, `expiring`, `expired` 중 어떤 상태인지도 함께 보고합니다. service를 생성하면 `clicklink clctl instances create`가 환경 ID, 클라우드 및 리전, service name, 그리고 버킷 이름과 IAM role ARN을 전송합니다. 또한 default user의 password에 대한 두 개의 해시를 전송하며, password 자체는 절대 외부로 전송되지 않습니다.
...
<Note>
  공개된 `v0.16.0` CLI에는 `--managed`도 `--no-managed`도 없습니다. 아래의 설치 시점 선택은 [관리형 모드 지원](/ko/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)이 포함된 build가 필요합니다. 이미 실행 중인 실행기를 중지하는 작업은 아래의 Helm 또는 systemd 안내를 따르며, 두 init 플래그 중 어느 것도 필요하지 않습니다.
</Note>
...
ClickHouse가 생성한 서비스는 계속 실행되지만, ClickHouse Cloud는 더 이상 해당 서비스를 확장, 업그레이드, Backup, 삭제할 수 없으며 해당 서비스에 대한 대기 중인 명령어는 실패합니다. `clicklink clctl platform approve`를 실행하지 않으면 플랫폼 업데이트는 적용되지 않습니다. 이 명령어를 실행하지 않은 상태로 두면 관리형 모드의 나머지 기능은 그대로 작동하지만 플랫폼 계층은 동결됩니다.
...
### service를 삭제하면 무엇이 삭제됩니까? {#what-is-deleted}

실행기는 service를 중지하고 해당 Kubernetes resources를 제거한 뒤 네임스페이스를 삭제합니다. 네임스페이스가 사라지면 service가 종료된 것으로 보고합니다. Kubernetes 외부의 항목은 모두 그대로 남습니다. 즉, 데이터 버킷, Backup 버킷, service의 IAM role은 유지됩니다. 실행기는 사용자의 버킷이나 IAM에 대한 자격 증명을 보유하지 않으므로 Kubernetes 외부의 어떤 것도 변경되지 않습니다.

나머지는 사용자의 자격 증명으로 `clicklink clctl executor teardown`을 실행하여 제거합니다. 기본적으로 IAM role은 제거하고 데이터와 Backup은 유지하며, 이를 삭제하려면 명시적인 flags를 지정해야 합니다. [Delete a service](/ko/products/bring-your-own-cloud/connector/managed-services#delete-a-service)를 참조하십시오.

### default 사용자 비밀번호는 어디에서 확인합니까? {#default-user-password}

고객 서비스 생성에는 [CLI와 배포 지원이 일치](/ko/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)해야 하며, 공개 버전 `v0.16.0`에서는 사용할 수 없습니다.
...
2. **관리형 모드를 중지합니다.** [ClickHouse가 내 클러스터를 변경하지 못하게 하려면 어떻게 합니까?](#stop-managed-mode)에 설명된 대로 실행기를 중지하고 해당 grant를 제거합니다. 플랫폼 업데이트는 이미 건별로 승인이 필요하므로, 승인을 보류하면 업데이트가 중단됩니다.
...
* 관리형 모드에서는 서비스 및 플랫폼 구성 요소용 ClickHouse image를 자체 registry로 가져온 후 해당 registry에서 제공할 수 있습니다. ClickHouse가 해당 registry를 사용자 환경에 등록합니다.
* [플랫폼 승인을 지원하는 build](/ko/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)에서는 플랫폼 bundle을 대역 외로 전달한 뒤 파일 형태로 `clicklink clctl platform approve --bundle`에 전달할 수 있습니다. 공개 `v0.16.0`에는 `platform` 명령이 없습니다.
