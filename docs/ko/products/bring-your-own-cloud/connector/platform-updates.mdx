...
<Steps>
  ...
  <Step title="승인 실행" id="run-approve">
    ...
    `approve`에는 관리형 클러스터에 대한 cluster-admin 권한을 가진 kube context가 필요합니다(kubeconfig에 여러 개가 있는 경우 `--context <name>` 사용). chart가 Amazon ECR에 있다면 이를 읽을 수 있는 AWS credentials도 필요합니다. `--dry-run`은 아무것도 적용하거나 발급하지 않고 permission 부분을 렌더링하여 나열합니다.
    ...
    <Tabs>
      ...
      <Tab title="Kubernetes" id="approve-kubernetes">
        `approve`는 이전 단계에서 설정한 포트 포워딩을 통해 스테이징된 번들을 읽습니다. 토큰 번들은 커넥터 네임스페이스의 Secret으로 전달되며, chart가 이를 파드에 mount합니다. kubeconfig로 클러스터에 접근할 수 있는 workstation에서 커넥터 구성 사본과 함께 실행하십시오. chart가 실행기의 ConfigMap에 구성을 렌더링해 두며, `approve`는 네임스페이스 접두사를 확인하는 데 이 구성이 필요할 뿐 여기에서 자격 증명을 읽지는 않습니다.
