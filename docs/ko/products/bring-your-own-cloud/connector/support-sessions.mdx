...
지원 세션을 사용하면 ClickHouse Connector를 통해 ClickHouse에 임시 진단 액세스 권한을 부여할 수 있습니다. 이 페이지에서는 세션의 의미, 세션을 활성화 및 비활성화하는 방법, 세션이 진행되는 동안 ClickHouse 지원 엔지니어가 수행할 수 있는 작업, 그리고 이를 감사하는 방법을 다룹니다.

## 지원 세션이란 {#what-a-support-session-is}

지원 세션은 troubleshooter가 ClickHouse 지원 엔지니어의 명령어를 수락하는, 시간이 정해진 윈도우입니다. 세션이 아닐 때는 아웃바운드 WebSocket이 연결되어 있더라도 troubleshooter가 모든 명령어를 거부합니다. 다른 실행 경로는 존재하지 않습니다. troubleshooter는 세션 없이 아무것도 실행하지 않으며, ClickHouse가 사용자를 대신해 세션을 열 수도 없습니다. ClickHouse Cloud는 사용자의 환경으로 접속하지 않으며, troubleshooter가 아웃바운드 채널로 전송하는 내용만 수신합니다.
...
<Tabs>
  <Tab title="Kubernetes" id="sessions-kubernetes">
    ...
    호출자의 OIDC 아이덴티티는 연산자 허용 목록에 있어야 합니다. 인증되지 않았거나 허용 목록에 없는 호출자는 401 또는 403을 받으며, Gateway가 해당 시도를 기록합니다. 클러스터 자격 증명을 요구하지 않으려면 차트의 선택적 인그레스를 통해 Gateway를 노출하십시오. 이 인그레스는 CA가 발급한 인증서로 TLS를 종료합니다. 자세한 내용은 [구성](/ko/products/bring-your-own-cloud/connector/configuration)을 참조하십시오.
  </Tab>
...
세션은 자동으로 만료됩니다. 기본 지속 시간은 4시간이며, `session enable --duration`은 최대 24시간까지 허용합니다. 세션이 만료되거나 `session disable`을 실행하면 troubleshooter는 즉시 명령어 수락을 중지합니다. 비활성화는 즉시 권한을 해지하는 방법으로, 재시작하거나 ClickHouse와 조정할 필요가 없습니다.

## 연산자 허용 목록 {#operator-allowlist}

모든 gateway 호출은 연산자 이메일 허용 목록을 기준으로 권한이 부여됩니다. 일치 여부는 검증된 OIDC 토큰으로 증명된 이메일을 기준으로 판단하며, 클라이언트가 자체적으로 주장하는 정보는 결코 사용하지 않습니다.
...
두 대상 모두에서 gateway는 마지막으로 읽은 시점부터 30초가 지나면 다음 request 시 파일을 다시 읽으므로, 재시작 없이 편집 내용이 적용됩니다.

## 세션 중 운영자가 수행할 수 있는 작업 {#what-operators-can-do}
...
Troubleshooter의 RBAC에는 `exec`, `delete`, `patch` 권한이 없으므로, 운영자는 파드에서 셸을 열거나 세션을 통해 어떤 것도 변경할 수 없습니다. 전체 권한 및 RBAC 목록은 [권한 모델](/ko/products/bring-your-own-cloud/connector/reference/privilege-model) 참고 문서에서 확인할 수 있습니다.

## 감사 로그 {#audit-log}

세션 중 실행된 모든 명령어는 한 줄에 하나의 JSON 객체 형식(NDJSON)으로 `/var/log/clicklink/troubleshoot-audit.log`에 기록됩니다. 차단된 명령어도 함께 기록되며, `blocked_by` 필드에 차단 사유(예: `session inactive`)가 표시됩니다. `submitted_by`는 명령 채널의 조직 ID로, 사용자 본인의 조직 ID입니다. 명령을 실행한 개별 ClickHouse 엔지니어 정보는 이 파일이 아니라 ClickHouse Cloud 측에 기록됩니다. 세션 명령어 항목은 다음과 같은 형태입니다:
...
Gateway 호출은 별도로 기록됩니다. 거부된 호출을 포함한 모든 호출은 Troubleshooter 자체 로그에 구조화된 `clctl.gateway` 줄로 남습니다. VM에서는 journal에, Kubernetes에서는 컨테이너 로그에 기록됩니다. 이 줄에는 `submitted_by`(검사를 통과한 토큰으로 증명된 이메일이며, 클라이언트가 제공한 값은 절대 사용되지 않습니다), `command_type`(`clctl.session.enable`, `clctl.session.disable`, `clctl.session.status`), `reason`(enable 시 지정한 `--reason`), `remote_addr`, `status`가 포함됩니다. `status`는 `ok`, `unauthorized`, `forbidden`, `rate_limited`, `bad_request`, `conflict`, `error` 중 하나이므로 거부된 액세스도 로그에 남습니다.
