...
<Note>
  여기서 설명하는 관리형 모드 등록, 고객 생성 및 플랫폼 승인 워크플로는 계정 담당 팀이 확인한 [호환되는 CLI 및 배포 지원](/ko/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)이 필요합니다. 공개 버전 `v0.16.0`에는 관리형 모드 init 플래그도 `platform` 명령도 없으며, 고객 생성 워크플로도 제공되지 않습니다.
</Note>
...
아래의 고객 생성 및 플랫폼 승인 연결은 [지원되는 관리형 모드 배포](/ko/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)에 적용됩니다. 해당 워크플로는 공개 `v0.16.0`에서는 사용할 수 없습니다.
...
| Amazon ECR(실행기, 관리형 모드 전용)                    | 아웃바운드                  | HTTPS                | 직접 플랫폼 접근에는 커넥터 EC2 VM의 인스턴스 프로필로 assume하는 읽기 전용 pull 역할, 그 외 ECR 차트 registry에는 ambient AWS 자격 증명 | 플랫폼 동기화 또는 생성 시 차트를 가져오기 위한 registry 로그인, 플랫폼 업데이트 중 image 존재 확인을 위한 `DescribeImages`                                                                                             |
| AWS STS(실행기, 관리형 모드 전용)                       | 아웃바운드                  | HTTPS                | 직접 플랫폼 접근에는 커넥터 EC2 VM의 인스턴스 프로필, 그 외 ECR 차트 registry에는 구성된 AWS 자격 증명 체인                          | 직접 플랫폼 차트 로그인 및 image check를 위해 읽기 전용 pull 역할을 assume하며, 다른 ECR 로그인을 위한 자격 증명도 제공할 수 있습니다. ECR과 STS는 실행기가 호출하는 유일한 클라우드 API입니다                                                    |
| 컨테이너 registry                                 | 아웃바운드, 사용자 클러스터 node에서 | HTTPS                | 사용자의 node 아이덴티티: ClickHouse가 등록된 node 역할에 부여하는 읽기 전용 pull 역할, 또는 image를 가져온 뒤 사용하는 자체 registry    | 관리형 서비스 및 플랫폼 구성 요소를 위한 image pull. 사용자의 클러스터가 pull하며, ClickHouse는 아무것도 push하지 않습니다                                                                                           |
...
서비스 생성 및 플랫폼 승인 워크플로에는 [호환되는 CLI 및 배포 지원](/ko/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)이 필요하며, 공개 버전 `v0.16.0`에는 포함되어 있지 않습니다.
...
* **실행기는 사용자가 승인한 업데이트 중에만 `pcm-platform`으로서 플랫폼 네임스페이스를 변경합니다.** `clicklink clctl platform approve`는 해당 ID의 토큰을 2시간 동안 발급하며, 갱신되지 않습니다. 이 토큰으로 실행기가 작성할 수 있는 대상은 Deployment, Service, ConfigMap, 시크릿, Job, PodDisruptionBudget뿐입니다. 네임스페이스, CRD, ServiceAccount, RBAC, 웹훅 및 admission 구성, PriorityClass, StorageClass는 승인 과정에서 사용자의 자격 증명으로 적용됩니다. [플랫폼 업데이트](/ko/products/bring-your-own-cloud/connector/platform-updates)를 참조하십시오.
