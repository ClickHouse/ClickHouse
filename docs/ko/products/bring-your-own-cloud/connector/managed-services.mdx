...
관리형 모드에서는 커넥터가 세 번째 구성 요소인 실행기를 함께 실행합니다. ClickHouse Cloud는 이 실행기를 통해 사용자의 Kubernetes 클러스터에서 ClickHouse 서비스를 운영합니다. 이 페이지에서는 서비스 생성 방법, 서비스 상태 확인 방법, 서비스가 생성된 이후 ClickHouse Cloud가 수행하는 작업, 그리고 서비스가 폐기(retire)되는 과정을 다룹니다.

<Note>
  관리형 모드 활성화 및 서비스 생성에는 아직 공개 릴리스에 포함되지 않은 지원 기능이 필요합니다. 공개된 `v0.16.0` CLI에는 `--managed` 및 `--no-managed` init 플래그도, 아래에 설명된 고객 생성 워크플로도 없습니다. 등록이나 리소스 준비를 진행하기 전에 계정 담당 팀과 [관리형 모드 지원 여부](/ko/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)를 확인하십시오. 서비스 생성에는 사용자의 엔드포인트와 ClickHouse Cloud 양쪽에서 이에 대응하는 지원도 필요합니다.
</Note>

## 관리형 모드의 동작 방식 {#what-managed-mode-does}

실행기는 scraper, troubleshooter와 동일한 `clicklink` 바이너리에 포함된 데몬입니다. 커넥터 엔드포인트로 향하는 아웃바운드 WebSocket 채널을 유지하면서 ClickHouse Cloud로부터 수명 주기 명령어를 수신합니다. 실행기는 자신이 구성된 단일 Kubernetes cluster에 각 명령어를 적용하고, 동일한 채널을 통해 결과를 보고합니다. 다른 구성 요소와 마찬가지로 아웃바운드 연결만 생성합니다. 즉, ClickHouse Cloud가 사용자 클러스터로 접속하는 일은 없으며, 실행기의 로컬 API는 루프백에 바인딩됩니다.

관리형 모드는 비공개 프리뷰의 일환으로 S3 storage를 사용하는 Amazon EKS에서 제공됩니다. 환경을 등록할 때 ClickHouse account team이 이를 활성화합니다. `init`은 EKS cluster를 가리키지 않는 kube context를 거부합니다.

각 서비스에는 사용자가 소유한 세 가지 cloud resources가 필요합니다. 데이터 버킷, 백업 버킷, 그리고 ClickHouse 파드가 이들에 접근하기 위해 assume하는 IAM role입니다. 이들은 서비스가 생성되기 전에 사용자의 자격 증명으로 직접 생성하고, 서비스가 제거된 후에 삭제합니다. 실행기는 사용자의 버킷이나 IAM에 대한 자격 증명을 보유하지 않으며, 데이터를 삭제하지도 않습니다. 실행기의 클라우드 호출은 Amazon ECR과 STS로만 향합니다. 플랫폼 동기화나 생성 작업이 차트를 가져올 때 registry에 로그인합니다. 플랫폼 업데이트 중에는 읽기 전용 pull role로 이미지를 확인합니다. S3나 IAM 호출은 수행하지 않습니다. 각 서비스에 대해 LoadBalancer 타입의 Kubernetes `Service`를 생성하며, 이는 클러스터의 load-balancer controller에 의해 사용자 계정 내 내부 NLB로 구현됩니다.
...
<Note>
  공개된 `v0.16.0` CLI는 `init --managed`나 아래에 설명된 관리형 모드 프롬프트를 지원하지 않습니다. 먼저 계정 담당 팀으로부터 [관리형 모드 지원](/ko/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)이 포함된 빌드와 배포를 받으십시오.
</Note>

관리형 모드는 등록할 때 선택합니다. `clicklink clctl init`에 `--managed`를 전달하거나, 터미널에 표시되는 프롬프트에 응답하십시오. VM에서 `init`은 해당 호스트의 kubeconfig에서 클러스터를 가져오며(EKS cluster가 두 개 이상 있는 경우 `--cluster-name`으로 지정), 실행기에 클러스터 전체 액세스 권한을 인라인으로 부여합니다. Kubernetes에서는 엔드포인트의 CIDR을 `--egress-cidrs`로 전달하여 차트가 기본 거부(default-deny) NetworkPolicy를 활성화된 상태로 배치하도록 하십시오.
...
이 워크플로는 공개 버전 `v0.16.0`에서는 사용할 수 없습니다. 리소스를 준비하기 전에 사용 중인 CLI, 엔드포인트, ClickHouse Cloud 배포에 대해 [서비스 생성 가용성](/ko/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)을 확인하십시오.
...
* `prepare`에는 커넥터 구성 사본(`--config`), 실행기의 로컬 API로 연결되는 `kubectl port-forward`, 그리고 쓰기 가능한 디렉터리를 가리키는 `--output-dir`가 필요합니다. 이 명령어는 실행기를 통해 서비스 이름을 확인하며, 실행기에 연결할 수 없으면 실행을 거부합니다.
* `instances create`는 현재 호스트에 자격 증명 파일이 없으면 구성에 지정된 해당 파일들을 `clicklink-hmac` 및 `clicklink-mtls` 시크릿에서 읽어오며, 읽을 때마다 이를 알립니다. 커넥터 네임스페이스가 `clicklink`가 아니면 `--connector-namespace`를 추가하십시오. 이 폴백 동작은 `instances create`에만 적용됩니다.
...
<Steps>
  <Step title="서비스 준비" id="prepare">
    ...
    4. **Summary.** `default` USER의 password를 발급하고 create body를 `<output-dir>/_prepare/<name>.create.json`에 작성합니다(mode `0600`, password의 해시만 포함). 그리고 다음에 실행할 명령을 `Next:` 줄에 그대로 출력합니다.

    <Warning>
      `prepare`는 `default` USER의 password를 stderr에 단 한 번만 출력하며, 이후에는 다시 출력하지 않습니다. create body에도 `--output json` 결과에도 password는 포함되지 않으며, ClickHouse Cloud는 해시만 수신합니다. 다음 단계로 넘어가기 전에 반드시 저장하십시오. 재실행 시 create body가 이미 존재하면 기록된 해시를 그대로 유지하며, 그 사실을 알려 줍니다.
    </Warning>
  ...
  <Step title="생성 요청을 제출합니다" id="create">
    ...
    동일한 입력으로 재시도해도 안전합니다. 멱등성 key는 기본적으로 환경과 service name에서 파생된 값을 사용하므로, 다시 제출하면 최초 생성 결과가 반환됩니다.
...
실행기는 로컬 API를 통해 상태 조회에 응답합니다. 이 API는 `127.0.0.1:9999`에 바인딩되며 자체 인증(authentication) 기능은 제공하지 않습니다. VM에서는 해당 호스트에서 명령어를 실행하십시오. Kubernetes에서는 먼저 포트 포워딩을 설정한 뒤, 명령어가 이를 바라보도록 지정하십시오:
...
* **Scale.** 서비스에는 ClickHouse Cloud가 지정하는 고정된 레플리카 수가 있으며, 그 상한은 ClickHouse Cloud 설정으로 결정됩니다(기본 구성에서는 20). 자동 스케일링은 지원되지 않습니다.
...
* **Backups.** 백업은 ClickHouse Cloud가 트리거할 때 실행되어 백업 버킷에 저장되며, 백업을 삭제하면 제거됩니다.
...
생성, 스케일, 시작 작업은 비동기적으로 완료됩니다. 실행기는 정의를 적용하는 즉시 해당 명령어를 실행 중으로 보고하고 2분마다 진행 상황을 보고합니다. 서버 레플리카가 준비되면 최종 결과를 보고합니다. 2시간 이내에 준비되지 않으면 명령어를 실패로 보고합니다. 이후 레플리카가 정상적으로 기동되면 서비스를 다시 `running` 상태로 전환합니다.

`instances scale`, `instances patch`, `instances delete` 하위 명령어는 ClickHouse Cloud를 거치지 않고 실행기의 로컬 API에 직접 명령어를 전송합니다. ClickHouse 계정 팀이 실행을 요청하지 않는 한 사용하지 마십시오. 각 명령어의 동작은 [CLI 참고](/ko/products/bring-your-own-cloud/connector/reference/cli#instances-scale)에 설명되어 있습니다.
...
<Tabs>
  <Tab title="Kubernetes" id="troubleshooter-kubernetes">
    ...
    `$CH_DEFAULT_PASSWORD`는 `prepare`가 출력한 `default` 사용자의 password입니다. 이 command는 SQL 권한 부여를 적용하기 전에 인증하는 데 해당 값이 필요하며, 별도로 입력을 요청하지 않습니다. 그런 다음 [ClickHouse instance 추가](/ko/products/bring-your-own-cloud/connector/configuration#clickhouse-instances)에 설명된 대로 Secret과 ServiceAccount 쌍을 `troubleshooter.accessBundles`에 추가하고 `helm upgrade`를 실행하십시오.
  </Tab>
...
ClickHouse 사용자에게는 `--ch-user-via cr`가 아니라 위와 같이 SQL로 권한을 부여하십시오. 서비스 정의는 ClickHouse Cloud가 소유하며 이를 반복해서 다시 적용하므로, 그곳에 추가한 사용자는 삭제됩니다. 서비스가 삭제되면 실행기는 VM에서 Troubleshooter의 bundle을 제거합니다. Kubernetes에서는 [제거](/ko/products/bring-your-own-cloud/connector/operations#uninstall-kubernetes)에 나열된 대로 직접 삭제하기 전까지 bundle Secret과 ServiceAccount가 그대로 남아 있습니다.

## 서비스 삭제 {#delete-a-service}

커넥터 엔드포인트를 통한 고객용 삭제 명령은 제공되지 않습니다. 서비스 삭제는 ClickHouse account team에 요청하십시오. ClickHouse Cloud가 서비스를 종료하면 실행기가 해당 워크로드와 네임스페이스를 삭제합니다(`terminating`을 거쳐 `terminated`). AWS 측에는 아무런 변경도 가해지지 않습니다. 버킷, 그 안의 데이터, IAM role은 직접 제거하기 전까지 그대로 유지됩니다.

서비스 상태가 `terminated`로 보고되면 `prepare`가 생성한 리소스를 정리하십시오. `prepare`를 실행했던 곳에서 동일한 AWS 자격 증명으로 실행해야 합니다. Kubernetes에서는 동일한 workstation, 동일한 구성 사본과 출력 디렉터리, 그리고 실행기로 연결된 포트 포워딩이 필요합니다.
...
이 명령은 실행기가 보관 중인 서비스 기록, 즉 데이터가 어디에 있고 어떤 역할을 사용했는지를 읽습니다. 그런 다음 `prepare`가 생성한 IAM role을 삭제하고, 실행기가 해당 서비스를 잊도록 하여 이름을 해제합니다. VM에서는 서비스의 cluster-wide RBAC 객체, 로컬 액세스 bundle, registry entry도 함께 제거합니다.
...
실행기가 연결되어 있지 않은 동안에는 ClickHouse Cloud가 새 작업을 전달할 수 없습니다. 이미 수락된 생성, 삭제, 플랫폼 동기화 작업은 보관되었다가 재시도됩니다. 생성은 마지막 진행 상황 보고 시점부터 30분 동안, 삭제는 2시간 동안, 플랫폼 동기화는 최대 10회까지 재시도됩니다. 이 한도를 초과하면 커넥터 엔드포인트가 해당 명령어를 실패로 표시합니다. 그 외 모든 수명 주기 명령어(scale, stop, start, restart, backup, backup 삭제)는 실행기가 연결되어 있지 않은 동안 큐에 대기되지 않고 거부됩니다. 실행기가 연결되어 있지 않은 상태에서 제출한 생성 또는 삭제 명령어도 동일하게 거부되며, 이미 수락된 명령어만 재시도됩니다.

실행기가 이미 수락한 명령어는 완료될 때까지 계속 실행되며, 보고하지 못한 결과는 다음 연결 시 실행기가 전송합니다.

두 가지 고객 명령어에는 각각의 의존성이 있습니다. `instances create`는 명령어를 실행하는 호스트에서 커넥터 엔드포인트에 연결할 수 있어야 합니다. `--wait`, `instances list`, `instances get`, `commands list`는 실행기의 로컬 API를 사용하므로 실행기가 실행 중이어야 합니다. 플랫폼 승인 토큰은 연결 상태와 관계없이 자체 시간에 따라 만료됩니다. [승인 윈도우](/ko/products/bring-your-own-cloud/connector/platform-updates#the-approval-window)를 참조하십시오.