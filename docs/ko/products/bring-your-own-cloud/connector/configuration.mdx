...
이 페이지에서는 커넥터 설치 후 가장 자주 변경하는 구성 항목을 다룹니다. 각 키의 기본값과 의미는 [구성 참고](/ko/products/bring-your-own-cloud/connector/reference/configuration)를, 명령 플래그는 [CLI 참고](/ko/products/bring-your-own-cloud/connector/reference/cli)를 참조하십시오. 서비스 생성, 스케일링, 삭제는 구성에 해당하지 않으며, [관리형 서비스](/ko/products/bring-your-own-cloud/connector/managed-services)를 참조하십시오.
...
<Note>
  공개된 `v0.16.0` CLI에는 `--managed`와 `--no-managed`가 모두 없습니다. 두 플래그 중 하나를 사용해 구성을 변경하기 전에 [관리형 모드 지원 여부](/ko/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)를 확인하고 계정 담당 팀으로부터 해당 build를 받으십시오.
</Note>

관리형 모드는 실행기를 구동하며, 실행기는 ClickHouse Cloud가 클러스터에 적용하는 서비스 수명 주기를 반영합니다. `init`는 등록 시점에 이 모드를 켜고 클러스터 아이덴티티를 채웁니다([관리형 모드 활성화](/ko/products/bring-your-own-cloud/connector/managed-services#enable-managed-mode) 참조). 아래 키들은 그 이후에 다시 확인하게 될 항목입니다. 구성된 상태를 변경하려면 `--force`와 함께 `--managed` 또는 `--no-managed`를 지정하여 `init`를 다시 실행하십시오. 모든 `init` 실행과 마찬가지로 entry point가 필요합니다. bundle을 보관해 두었다면 `--handoff`를, 그렇지 않다면 [재실행 및 recovery](/ko/products/bring-your-own-cloud/connector/operations#re-runs-and-recovery)에 설명된 대로 새 토큰과 함께 `--enroll`을 사용하십시오.

VM에서 `--no-managed`는 구성에서 실행기를 비활성화하지만, 이미 실행 중인 systemd unit을 중지하거나 비활성화하지는 않습니다. 구성을 변경하기 전에 명시적으로 중지하십시오.

```bash
sudo systemctl disable --now clicklink-executor
```

Kubernetes에서는 values에 `executor.enabled: false`를 설정하고 `helm upgrade`로 변경 사항을 적용하면 실행기 Deployment가 제거됩니다. 두 대상 모두에서 클러스터 권한 부여는 직접 제거하기 전까지 그대로 남아 있습니다. 해당 액세스까지 제거하려면 [관리형 모드 중지](/ko/products/bring-your-own-cloud/connector/reference/faq#stop-managed-mode)를 따르십시오.
...
실행기가 생성하는 서비스는 스스로 등록되므로, 관리형 모드에서는 여기에 직접 추가할 필요가 없습니다. 실행기 없이 실행되는 별도의 커넥터 배포는 사용자가 직접 운영하는 ClickHouse를 관찰합니다. 두 배포가 동일한 인스턴스를 공유하는 경우 해당 배포의 `init` 시점에 `--ch-user-suffix`를 전달하십시오. 이 섹션은 그러한 배포에 적용됩니다. `instances` 아래의 각 항목은 커넥터가 읽어 들이는 ClickHouse 네이티브 프로토콜 엔드포인트를 지정하며, `host`, `port`, `database`, `secure`, `cluster`, 그리고 Kubernetes에서는 `namespace`를 포함합니다. `max_open_conns`와 `max_idle_conns`는 연결 풀의 상한을 정합니다. 자격 증명은 구성에 저장되지 않으며, 각 컴포넌트는 프로비저닝이 생성한 액세스 번들에서 읽기 전용 ClickHouse 사용자를 가져옵니다.

<Tabs>
  <Tab title="Kubernetes" id="instances-kubernetes">
    `clicklink-values.yaml`의 최상위 `instances` 맵에 인스턴스를 추가하고(VM 레지스트리와 동일한 필드), `networkPolicy.clickhouseNamespaces`에 해당 네임스페이스를 추가하십시오(네임스페이스의 `kubernetes.io/metadata.name` 레이블과 일치). 차트는 이 맵을 두 데몬이 읽는 `clicklink-instance-registry` ConfigMap으로 렌더링합니다. 이 방식은 `executor.enabled`가 `false`인 경우에만 동작합니다. 실행기가 켜져 있으면 실행기가 해당 ConfigMap을 소유하므로 렌더링이 실패합니다.
  ...
  <Tab title="Linux VM" id="instances-vm">
    ...
    그런 다음 호스트에서 root 사용자로 각 구성 요소에 대한 액세스를 프로비저닝하십시오. VM에서는 `--ch-user-via cr`를 전달하지 않는 한 관리자 비밀번호가 필수이며, `--ch-admin-password-stdin`으로 파이프를 통해 전달하십시오. 각 명령어는 ClickHouse 권한을 적용하고 해당 데몬을 재시작합니다(`--skip-restart`로 재시작 생략).
...
Kubernetes에서 이 차트는 egress allowlist가 적용된 기본 거부(default-deny) NetworkPolicy를 함께 제공합니다. NetworkPolicy 객체는 사용 중인 CNI가 이를 적용(enforce)할 때만 효력이 있습니다. 정책을 적용하는 CNI 환경에서는 두 CIDR 목록이 모두 채워지기 전까지 클러스터 DNS와 ClickHouse 서비스 네임스페이스에만 연결할 수 있습니다. 그 전까지 커넥터는 연결하려는 엔드포인트나 Kubernetes API server로 나가는 egress 경로가 없습니다. 따라서 `init`은 커넥터 엔드포인트 뒤에 있는 CIDR를 파악한 경우에만 해당 정책을 enabled 상태로 구성합니다.
...
* **`allowEgressCIDRs`**는 사용자가 직접 제공해야 합니다. 커넥터 엔드포인트 뒤에 있는 범위로, 설치 시 `--egress-cidrs`로 지정하거나 프롬프트에서 입력합니다. 모든 구성 요소가 커넥터 엔드포인트에 도달하려면 이 값이 필요합니다. 관리형 모드에서 플랫폼 차트가 Amazon ECR에 있는 경우, 해당 리전의 ECR 및 STS 범위(또는 미러의 범위)도 함께 추가하십시오. 이를 추가하지 않으면 정책에 막혀 실행기의 차트 pull이 실패합니다.
...
Troubleshooter는 출력이 사용자 환경을 벗어나기 전에 민감 정보를 마스킹합니다. 기본 제공 패턴은 `ipv4`, `ipv6`, `bearer-token`, `aws-access-key`, `email`, `jwt`, `ssh-private-key`, `connection-string-credentials`를 처리합니다. YAML 파일에서 사용자 지정 패턴을 추가할 수 있습니다. 사용자 지정 패턴은 파일에 정의된 순서대로 먼저 실행되고, 이후 기본 제공 패턴이 실행됩니다. 기본 제공 패턴의 `name`을 재사용하는 항목은 해당 기본 제공 패턴을 대체합니다.
...
VM에서는 파일 경로가 `/etc/clicklink/redaction-patterns.yaml`입니다. 설치 프로그램은 활성 패턴의 기본 세트(타사 API Key, AWS 시크릿 키 등)를 함께 설치합니다. 필요에 따라 이 세트를 줄이거나 확장할 수 있으며, 설치 프로그램은 업그레이드해도 기존 버전을 유지합니다. Kubernetes에서는 YAML을 `redaction-patterns.yaml` 키를 사용해 ConfigMap에 저장하고 `troubleshooter.redaction.patternsConfigMap`에 해당 ConfigMap 이름을 설정하십시오. 차트가 이를 동일한 경로에 마운트합니다.
...
아래의 관리형 모드 예시는 `--managed`를 지원하는 빌드가 필요하며, 공개 `v0.16.0`에는 이 기능이 없습니다. 사용하기 전에 [관리형 모드 지원 여부](/ko/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)를 확인하십시오. 공개 릴리스를 미러링한다고 해서 이 지원이 추가되지는 않습니다.
...
<Tabs>
  ...
  <Tab title="Linux VM" id="storage-vm">
    scraper는 커넥터 엔드포인트에 연결할 수 없는 동안 최소 1회 전달을 보장하기 위해 `/var/lib/clicklink/buffer` 아래에 메트릭을 스풀링합니다. 최대 168시간 또는 1024 MB까지 보관하며, 기본적으로 1 MB/s로 제한된 속도로 업로드합니다:
