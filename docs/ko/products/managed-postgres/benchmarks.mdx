...
sidebarTitle: '벤치마크'
title: '성능 벤치마크'
description: 'ClickHouse Managed Postgres와 AWS Aurora, RDS 및 기타 관리형 PostgreSQL 서비스의 성능을 비교한 벤치마크'
...
<Info>
  ...
  * 표준 [`pgbench`](https://www.postgresql.org/docs/current/pgbench.html) 테스트를 사용해 **ClickHouse Managed Postgres**와 AWS RDS(프로비저닝된 IOPS 16k), Aurora IO Optimized의 성능을 **벤치마크**했습니다
  * **성능**: NVMe 스토리지를 기반으로 하는 ClickHouse Managed Postgres는 IO 집약적 워크로드에서 **4.3~9배 더 빠른 성능**을 제공하며, CPU 바운드 시나리오에서는 **12% 더 빠릅니다**
...
| 구성         | ClickHouse Managed Postgres | RDS with 16k IOPS      | Aurora IO Optimized     |
| ---------- | --------------------------- | ---------------------- | ----------------------- |
| **PG 버전**  | 17                          | 17                     | 17                      |
| **vCPUs** | 16                          | 16                     | 16                      |
...
**분석**: IO 병목이 발생하는 읽기 집약적 워크로드에서는 성능 격차가 크게 벌어집니다. **ClickHouse Managed Postgres는 84.8K TPS를 기록한 반면**, 16,000 프로비저닝된 IOPS를 갖춘 RDS는 동일한 컴퓨트 리소스를 사용하고도 9.4K TPS에 그쳤습니다. 핵심적인 차이는 다음과 같습니다. ClickHouse Managed Postgres의 NVMe 스토리지는 더 높은 동시성에서도 확장되지만, 네트워크 연결형 스토리지는 프로비저닝된 IOPS 한계로 인해 계속 제약을 받습니다. 프로비저닝된 IOPS를 적용하더라도 RDS는 여전히 ClickHouse Managed Postgres보다 9배 느렸으며, 이는 IO 집약적 워크로드에서 스토리지 아키텍처가 얼마나 중요한지를 분명하게 보여줍니다.
...
| 구성         | ClickHouse Managed Postgres | 16k IOPS RDS           |
| ---------- | --------------------------- | ---------------------- |
| **PG 버전**  | 17                          | 17                     |
| **vCPUs**  | 16                          | 16                     |
| **RAM**    | 64 GB                       | 64 GB                  |
...
**분석**: 디스크 I/O가 거의 발생하지 않는 CPU 바운드 시나리오에서도 **ClickHouse Managed Postgres가 36.5K TPS로 가장 앞선 성능을 보였습니다**. 두 서비스 모두 CPU 사용률 100%에 도달했지만, ClickHouse Managed Postgres의 NVMe 스토리지는 더 높은 캐시 적중률을 바탕으로 더 우수한 성능을 제공했습니다. RDS 대비 12%의 우위는 워크로드가 주로 CPU 바운드인 경우에도 기반 인프라의 효율성이 뛰어남을 보여줍니다.
...
| 구성         | ClickHouse Managed Postgres | RDS PostgreSQL |
| ---------- | --------------------------- | -------------- |
| **PG 버전**  | 17                          | 17             |
| **vCPUs**  | 2                           | 2              |
| **RAM**    | 8 GB                        | 8 GB           |
...
세 가지 벤치마크 시나리오 모두에서 ClickHouse Managed Postgres는 일관되게 더 우수한 성능을 보였습니다:
...
| 항목           | NVMe 스토리지 (ClickHouse Managed Postgres) | 네트워크 연결 스토리지 (프로비저닝된 IOPS) |
| ------------ | --------------------------------------- | -------------------------- |
...
단순한 성능을 넘어, ClickHouse Managed Postgres는 더 뛰어난 가격 대비 성능을 제공합니다.
...
현재 IOPS 한계로 성능이 제약되는 워크로드는 ClickHouse Managed Postgres로 전환하면 값비싼 프로비저닝된 IOPS나 IO Optimized 구성이 필요 없어지는 동시에 훨씬 더 우수한 성능을 얻을 수 있습니다.
...
* [ClickHouse Managed Postgres 개요](/ko/products/managed-postgres/overview)
