...
Postgres 서비스의 수명 주기를 살펴보겠습니다.

<Warning>
  **2026년 7월 31일부터 자격 증명 가리기 적용**

  **2026년 7월 31일**부터 API 응답에는 더 이상 `password` 및 `connectionString` 속성이 포함되지 않습니다. 이 속성은 요청에 비밀번호가 포함되지 않은 경우에만 [create](#create) 응답과 [password reset] 응답에 표시됩니다. 자격 증명은 [create](#create) 응답에서 확보하는 것이 좋습니다. Terraform으로 서비스를 관리하는 경우 2026년 7월 31일 이전에 provider를 **v3.21.0 이상**으로 업그레이드하십시오([Terraform 참고](/ko/products/managed-postgres/terraform) 참조).
</Warning>
...
헤더가 필요합니다. 반환되는 자격 증명을 확인할 수 있도록 응답을 저장하세요:
...
출력은 생성 시 반환된 JSON과 비슷하지만(단, 2026년 7월 31일 이후에는 `password` 및 `connectionString` 속성이 제외됨), `state`를 잘 확인하십시오. 값이 `running`으로 바뀌면
서버가 준비된 것입니다:
...
[ClickHouse OpenAPI]: /ko/products/cloud/features/admin-features/api/api-overview "Cloud API"

[OpenAPI]: https://www.openapis.org "OpenAPI Initiative"

[API keys]: /ko/products/cloud/features/admin-features/api/openapi "API Key 관리"
...
[Prometheus endpoint]: /ko/products/managed-postgres/monitoring/prometheus "ClickHouse Managed Postgres Prometheus 엔드포인트"

[metrics reference]: /ko/products/managed-postgres/monitoring/metrics "ClickHouse Managed Postgres 메트릭 참고"

[Query Insights]: /ko/products/managed-postgres/monitoring/query-insights "Postgres 쿼리 인사이트"

[상세 플라이아웃]: /ko/products/managed-postgres/monitoring/query-insights#detail "쿼리 인사이트 상세 플라이아웃"

[per-execution counters]: /ko/products/managed-postgres/monitoring/query-insights#counters "쿼리 인사이트 실행별 카운터"
