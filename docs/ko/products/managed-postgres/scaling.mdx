...
전환은 다운타임이 발생하는 유일한 단계이며, 일반적으로 1분 이내에 완료됩니다. [유지 관리 기간](/ko/products/managed-postgres/upgrades#maintenance-windows)이 구성되어 있으면 디스크 사용량이 95%에 도달하지 않는 한 전환은 해당 기간까지 대기합니다. 전환 중에는 열려 있는 연결이 끊기고 진행 중인 트랜잭션이 롤백됩니다. 연결 문자열은 변경되지 않습니다. DNS가 새 프라이머리를 가리키도록 업데이트되며, 표준 재연결 로직을 사용하는 애플리케이션은 자동으로 복구됩니다.
...
3. 대체 서버가 최신 상태를 따라잡으면, 유지 관리 기간이 구성된 경우 해당 기간 내에 전환이 진행됩니다. 연결은 1분 미만 동안 끊기며, 애플리케이션은 동일한 호스트명으로 다시 연결되고 사용량은 약 45%로 돌아갑니다.
