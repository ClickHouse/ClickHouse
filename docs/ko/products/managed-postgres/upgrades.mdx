...
Enterprise Tier 조직에서는 ClickHouse Managed Postgres의 유지 관리 기간을 지원하므로, 업그레이드 및 기타 유지 관리 작업을 워크로드에 미치는 영향이 가장 적은 시간대에 예약할 수 있습니다. 유지 관리 기간 구성을 위한 UI 및 API 지원은 곧 제공될 예정입니다. 그전까지는 인스턴스의 유지 관리 기간을 설정하려면 [support](https://clickhouse.com/support/program)에 문의하십시오.
