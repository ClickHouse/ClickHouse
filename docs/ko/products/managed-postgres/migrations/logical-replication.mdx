...
먼저 Managed Postgres 인스턴스를 설정해 두었는지 확인하십시오. 가능하면 소스와 동일한 리전에 설정하는 것이 좋습니다. [여기](/ko/products/managed-postgres/quickstart#create-postgres-database)의 빠른 시작 가이드를 따라 진행할 수 있습니다. 이 가이드에서는 다음과 같은 구성을 생성합니다.
...
축하합니다! `pg_dump` 및 `pg_restore`를 사용하여 PostgreSQL 데이터베이스를 ClickHouse Managed Postgres로 성공적으로 마이그레이션했습니다. 이제 Managed Postgres의 기능과 ClickHouse와의 통합을 살펴볼 준비가 되었습니다. 시작에 도움이 되는 10분짜리 빠른 시작 가이드는 다음과 같습니다:
