...
축하합니다! `pg_dump` 및 `pg_restore`를 사용해 PostgreSQL 데이터베이스를 ClickHouse Managed Postgres로 성공적으로 마이그레이션했습니다. 이제 Managed Postgres의 기능과 ClickHouse 통합을 살펴볼 준비가 되었습니다. 시작하는 데 도움이 되는 10분 분량의 quickstart는 다음과 같습니다.

* [Managed Postgres quickstart 가이드](/ko/products/managed-postgres/quickstart)