...
이제 ClickHouse Cloud는 외부 PostgreSQL 데이터베이스를 ClickHouse Managed Postgres 서비스로 마이그레이션할 수 있도록 ClickPipes를 제공합니다. 이 기본 제공 통합을 사용하면 원본 데이터베이스에 연결하고, 스키마를 내보내고, 이를 ClickHouse Managed Postgres로 가져오고, 지속적 복제를 설정하는 작업을 간편하게 수행할 수 있습니다.
...
* **DDL 전파**: 지속적 복제(CDC)는 DML 작업과 `ADD COLUMN`을 캡처합니다. `DROP COLUMN` 및 `ALTER COLUMN`과 같은 다른 DDL 변경 사항은 전파되지 않으므로 대상에 수동으로 적용해야 합니다.

<Note>
  마이그레이션 중 문제가 발생하면 일반적인 오류와 해결 방법은 [ClickHouse Managed Postgres Migrations FAQ](/ko/products/managed-postgres/migrations/faq)에서 확인하십시오.
</Note>
...
<Steps>
  <Step title="원본 데이터베이스에 연결" id="step-1-connect">
    ...
    * **초기 적재 + CDC** — 기존 데이터를 복사한 다음, 이후 변경 사항도 계속 반영하여 대상을 동기화 상태로 유지합니다.
    * **초기 적재 only** — 한 번만 복사하고, 이후 복제는 수행하지 않습니다.
  ...
  <Step title="수집 설정 구성" id="step-4-ingestion-settings">
    ...
    | Parallel threads for 초기 적재            | 4       | 대량 복사 단계에 사용할 스레드 수       |
    | Pull batch size                       | 100,000 | 복제 batch마다 가져오는 행 수       |
    | Snapshot number of rows per partition | 100000  | 대형 테이블 snapshot의 파티션 크기   |
    | Snapshot number of tables in parallel | 1       | 동시에 snapshot하는 테이블 수      |

    <Image img="/images/managed-postgres/pgpg/advancedsettings.webp" alt="4단계: publication 및 고급 복제 옵션이 있는 수집 설정 양식" size="lg" border />

    **다음**을 클릭합니다.
  </Step>

  <Step title="테이블 선택" id="step-5-select-tables">
    복제할 테이블을 선택합니다. 테이블은 스키마별로 그룹화되어 있습니다. 개별 테이블을 선택하거나 스키마를 펼쳐 전체를 선택합니다.
...
**애플리케이션 트래픽을 전환하세요.** 읽기 및 쓰기 트래픽이 ClickHouse Managed Postgres 서비스로 향하도록 전환하고, 오류, 제약 조건 위반, 복제 상태를 모니터링하세요.

**정리하세요.**  전환을 완료하고 새 서비스가 정상 상태임을 확인했으면 **데이터 소스**에서 마이그레이션을 삭제하세요. CDC를 사용했다면 리소스를 확보할 수 있도록 원본에서 replication slot을 삭제하세요:
...
* [ClickHouse Managed Postgres quickstart](/ko/products/managed-postgres/quickstart)
