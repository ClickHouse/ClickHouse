...
축하합니다! `pg_dump` 및 `pg_restore`를 사용하여 PostgreSQL 데이터베이스를 ClickHouse Managed Postgres로 성공적으로 마이그레이션했습니다. 이제 Managed Postgres의 기능과 ClickHouse와의 통합을 살펴볼 수 있습니다. 시작하는 데 도움이 되는 10분 분량의 빠른 시작 가이드는 다음과 같습니다.
