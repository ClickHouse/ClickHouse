...
sidebarTitle: '보안'
title: '보안'
description: 'IP 허용 목록, 암호화, AWS PrivateLink, GCP Private Service Connect를 포함한 ClickHouse Managed Postgres 보안 기능'
keywords: ['postgres 보안', 'IP 허용 목록', '암호화', 'tls', 'ssl', 'private link', 'private service connect', 'psc', '백업 보존']
doc_type: 'guide'
---
...
GCP Private Service Connect(PSC)를 사용하면 트래픽을 공용 인터넷에 노출하지 않고도 ClickHouse Managed Postgres 인스턴스와 Google Cloud VPC(Virtual Private Cloud) 간에 프라이빗 연결을 구성할 수 있습니다. 이를 통해 네트워크 격리와 보안을 한층 더 강화할 수 있습니다.

<Info>
  **수동 설정 필요**

  GCP Private Service Connect를 지원하지만, ClickHouse 지원팀이 직접 구성해야 합니다. 이 기능은 네트워크 격리 요구 사항이 엄격한 엔터프라이즈 고객에게 적합합니다.
</Info>

### GCP Private Service Connect 설정 요청 {#requesting-gcp-private-service-connect}

ClickHouse Managed Postgres 인스턴스에 GCP Private Service Connect를 활성화하려면 다음 절차를 따르십시오.

1. **ClickHouse 지원팀에 문의**: 지원 티켓을 생성합니다.

2. **다음 정보 제공**:
   * ClickHouse 조직 ID
   * Postgres 서비스의 ID/호스트명
   * Private Service Connect 엔드포인트를 연결할 GCP 프로젝트 ID

3. **ClickHouse 지원팀 수행 작업**:
   * ClickHouse Managed Postgres 측에 Private Service Connect 서비스 어태치먼트를 프로비저닝합니다.
   * 엔드포인트 생성에 사용할 서비스 어태치먼트 ID를 제공합니다.

4. **GCP Private Service Connect 설정**:
   * 앞서 제공한 GCP 프로젝트 중 하나에서, ClickHouse 지원팀이 제공한 서비스 어태치먼트 ID를 사용하여 Postgres 서비스와 동일한 리전의 VPC에 Private Service Connect 엔드포인트를 생성합니다.
   * 엔드포인트가 &quot;Accepted&quot; 상태가 되면 엔드포인트 IP의 포트 `5432`(direct) 또는 `6432`(pooler)로 연결할 수 있습니다.
   * 엔드포인트 IP에는 Postgres 호스트명과 일치하는 DNS 이름이 없으므로 `sslmode=require`를 사용하십시오. `sslmode=verify-full`을 사용하려면 Postgres 호스트명을 엔드포인트 IP에 매핑하는 비공개 Cloud DNS 레코드를 추가하십시오.
