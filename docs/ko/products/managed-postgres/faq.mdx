...
<div id="metrics-access">
  ### ClickHouse Managed Postgres 인스턴스의 메트릭은 어떻게 확인하나요?
</div>
...
ClickHouse Managed Postgres에는 지속적인 WAL 아카이빙과 함께 매일 자동 백업이 포함되어 있어 7일 보관 기간 내의 임의 시점으로 시점 복구를 수행할 수 있습니다. 백업은 S3에 저장됩니다.
...
<div id="terraform-support">
  ### ClickHouse Managed Postgres에서 Terraform을 지원하나요?
</div>

현재 ClickHouse Managed Postgres에서는 Terraform을 지원하지 않습니다. 인스턴스를 생성하고 관리할 때는 ClickHouse Cloud Console 또는 [OpenAPI](/ko/products/managed-postgres/openapi)를 사용하는 것이 좋습니다.
...
<div id="migration-tools">
  ### ClickHouse Managed Postgres로 마이그레이션할 때 사용할 수 있는 도구는 무엇인가요?
</div>
