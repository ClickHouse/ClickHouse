...
<Steps>
  ...
  <Step stepNumber={2}>
    ## 2단계: Artifact Registry 생성 (`01-artifact-registry.sh`)
  ...
  <Step stepNumber={3}>
    ## 3단계: 컨테이너 image 복사 (`02-copy-images.sh`)
  ...
  <Step stepNumber={4}>
    ## 단계 4: VPC 및 서브넷 구성 (`03-vpc-subnet.sh`)
  ...
  <Step stepNumber={5}>
    ## 5단계: Cloud NAT 생성 (`04-cloud-nat.sh`)
  ...
  <Step stepNumber={6}>
    ## 6단계: GCP 서비스 계정 생성 (`05-service-account.sh`)
  ...
  <Step stepNumber={7}>
    ## 7단계: GCS 버킷 만들기 (`06-gcs-bucket.sh`)
  ...
  <Step stepNumber={8}>
    ## 단계 8: GKE 클러스터 생성 (`07-gke-cluster.sh`)
  ...
  <Step stepNumber={10}>
    ## 10단계: 로컬 SSD 데몬셋 배포 (`09-local-ssd-daemonset.sh`)
  ...
  <Step stepNumber={11}>
    ## 11단계: 토폴로지 레이블 추가 (`10-topology-labels.sh`)
  ...
  <Step stepNumber={14}>
    ## 14단계: Operator 설치 (`16-install-operator.sh`)
  ...
  <Step stepNumber={15}>
    ## 15단계: ClickHouse 클러스터 배포 (`17-install-cluster.sh`)
  ...
  <Step stepNumber={16}>
    ## 단계 16: Workload Identity 바인딩 (`18-bind-helm-workload-identity.sh`)
