...
<Steps>
  <Step title="단계 1: Terraform 설치" id="phase-1-install-terraform">
    ```text
  ...
  <Step title="단계 2: 예시 스크립트 다운로드" id="phase-2-download-example-scripts">
  ...
  <Step title="단계 3: AWS 자격 증명 구성" id="phase-3-configure-aws-credentials">
  ...
  <Step title="단계 4: Terraform 초기화 및 적용" id="phase-4-initialize-and-apply-terraform">
  ...
  <Step title="단계 5: Ansible 의존성 설치" id="phase-5-install-ansible-dependencies">
  ...
  <Step title="단계 6: RKE2 프로비저닝" id="phase-6-provision-rke2">
    ### 노드 준비 상태 확인
  ...
  <Step title="단계 7: cert-manager 및 인증서 구성" id="phase-7-configure-cert-manager-and-certificates">
  ...
  <Step title="단계 8: 로컬 스토리지 설치" id="phase-8-install-local-storage">
  ...
  <Step title="단계 9: 노드에 레이블 지정" id="phase-9-label-nodes">
  ...
  <Step title="단계 10: ClickHouse Operator 및 클러스터 설치" id="phase-10-install-clickhouse-operator-and-cluster">
    ### 환경 변수 설정
