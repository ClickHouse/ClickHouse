...
export const IntegrationsBanner = () => {
    const assetBase = typeof window !== 'undefined' && window.location.pathname.startsWith('/docs') ? '/docs' : '';
    const href = '/ko/integrations/home';
    return (
        <a
    ...
    <div className="relative z-10 px-0 py-16 lg:py-12 lg:pb-40 max-w-6xl mx-auto">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-4 mt-24">
        <HeroCard
          ...
          description="저는 여기 처음이라 시작 방법을 익히고 더 알아보고 싶습니다"
        ...
        { label: "샘플 데이터셋", href: "/ko/get-started/sample-datasets/index" },
        ...
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 256 256" className="ch-goal-icon">
          ...
          links={[
        { label: "SQL 레퍼런스", href: "/ko/reference/statements" },
        { label: "설정", href: "/ko/reference/settings/index" },
        { label: "함수", href: "/ko/reference/functions/regular-functions/overview" },
        { label: "엔진", href: "/ko/reference/engines/engines" },
        { label: "시스템 테이블", href: "/ko/reference/system-tables" },
        { label: "포맷", href: "/ko/reference/formats/index" },
        { label: "데이터 타입", href: "/ko/reference/data-types/index" }
    ]}
        />
        ...
        { label: "ClickHouse Managed Postgres", href: "/ko/products/managed-postgres/overview" },
        { label: "Langfuse", href: "https://langfuse.com/docs", external: true },
        { label: "chDB", href: "/ko/chdb" },
      ...
      <div className="grid sm:grid-cols-2 min-[1300px]:grid-cols-4 gap-x-6 gap-y-4 mt-6">
        <HeroCard title="실시간 분석" galaxyEvent="docs.home.real-time-analytics" description="대규모 환경에서도 즉각적인 인사이트와 대시보드를 제공합니다. 수십억 개의 행을 실시간으로 분석하고 밀리초 단위로 결과를 확인할 수 있습니다." href="/ko/get-started/use-cases/real-time-analytics" showArrow />

        <HeroCard title="관측성" galaxyEvent="docs.home.observability" description="ClickHouse 기반의 오픈 소스 관측성 스택인 ClickStack을 사용해 대규모 로그, 메트릭, 트레이스를 저장하고 쿼리할 수 있습니다." href="/ko/clickstack/overview" showArrow />

        <HeroCard title="데이터 웨어하우징" galaxyEvent="docs.home.data-warehousing" description="인사이트와 앱을 위해 데이터를 즉시 분석하고 탐색할 수 있습니다. 무거운 워크로드를 오프로드해 더 빠르게 확장할 수 있습니다." href="/ko/get-started/use-cases/data-warehousing" showArrow />

        <HeroCard title="에이전트형 분석" galaxyEvent="docs.home.agentic-analytics" description="AI 에이전트가 실시간 데이터를 직접 쿼리하여 즉각적인 대화형 인사이트와 시각화를 제공하도록 합니다." href="/ko/get-started/use-cases/agentic-analytics" showArrow />
      </div>
      ...
      <div className="space-y-4 mt-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 items-stretch">
          <div className="col-span-2">
            <HeroCard
              ...
              links={[
              { label: "1. 일반적인 데이터 소스에서 최소한의 작업으로 데이터를 수집하는 방법", href: "/ko/integrations" },
              { label: "2. analytics와 확장성을 위해 데이터를 구성하는 모범 사례", href: "/ko/guides/clickhouse/data-modelling/schema-design" },
              { label: "3. 대규모 데이터셋을 실시간으로 쿼리하는 방법", href: "/ko/get-started/use-cases/real-time-analytics" },
              { label: "4. 그 위에 대시보드, 알림, 이벤트 기반 기능을 구축하는 방법", href: "/ko/get-started/use-cases/data-warehousing" },
              { label: "5. 높은 성능을 유지하면서 비용을 낮추는 팁", href: "/ko/concepts/best-practices/index" },
          ]}
            />
          </div>

          <div className="flex flex-col gap-4">
            <HeroCard filename="thumbnail-clickstack-light.webp" title="ClickStack" galaxyEvent="docs.home.clickstack" description="ClickHouse 기반의 올인원 관측성 스택으로, logs, 메트릭, traces, 세션 리플레이, 오류에 대해 매우 빠른 쿼리와 강력한 집계 기능을 제공합니다." href="/ko/clickstack/overview" extraClassName="flex-1" showArrow arrowBottom />

            <HeroCard filename="thumbnail-postgres-light.webp" title="ClickHouse Managed Postgres" galaxyEvent="docs.home.managed-postgres" description="ClickHouse와 네이티브로 통합된 빠르고 확장 가능하며 엔터프라이즈급 Postgres 서비스입니다. 트랜잭션에는 Postgres를, analytics에는 ClickHouse를 손쉽게 함께 사용할 수 있습니다." href="/ko/products/managed-postgres/overview" extraClassName="flex-1" showArrow arrowBottom />
          </div>
        </div>

        {/* 전체 너비 Integrations 배너 */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <HeroCard filename="thumbnail-kubernetes-light.webp" title="Kubernetes Operator" galaxyEvent="docs.home.kubernetes-operator" description="Kubernetes에서 ClickHouse 및 ClickHouse Keeper 클러스터의 배포, 구성, 관리를 자동화합니다." href="/ko/products/kubernetes-operator/overview" showArrow />

          <HeroCard filename="thumbnail-chdb-light.webp" title="chDB" galaxyEvent="docs.home.chdb" description="빠르고 안정적이며 확장 가능한 in-process 데이터베이스입니다. 프로세스 내에서 ClickHouse의 강력함을 경험할 수 있습니다." href="/ko/chdb/index" showArrow />
        </div>
      </div>
      ...
      <div id="mcp-setup" className="mt-16 scroll-mt-20">
        <h2 className="text-2xl font-semibold text-gray-900 dark:text-zinc-50 mb-0">docs MCP server 설정</h2>
        <p className="text-sm text-gray-500 dark:text-zinc-500 mb-6">Model Context Protocol을 사용하여 AI 도구에서 ClickHouse 문서를 직접 조회합니다.</p>

        <div className="mt-6">
          <Tabs>
            <Tab title="Claude Code" icon="https://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/logo-claudecode-color.svg">
              <p className="mb-2 text-[#9ca3af] text-[13px]">터미널에서 실행하세요:</p>
            ...
            <Tab title="Claude Desktop" icon="https://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/logo-claude.svg">
              <p className="mb-2 text-[#9ca3af] text-[13px]"><code>claude&#95;desktop&#95;config.json</code>에 추가하세요:</p>
            ...
            <Tab title="Cursor" icon="https://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/logo-cursor.png">
              <p className="mb-2 text-[#9ca3af] text-[13px]"><code>.cursor/mcp.json</code>에 추가하세요:</p>
            ...
            <Tab title="Windsurf" icon="https://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/logo-windsurf.svg">
              <p className="mb-2 text-[#9ca3af] text-[13px]"><code>~/.codeium/windsurf/mcp&#95;config.json</code>에 추가하세요:</p>
