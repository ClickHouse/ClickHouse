...
<Note>
  `system.query_log` 테이블이 없다면 쿌리 로깅이 활성화되어 있지 않을 가능성이 높습니다. 활성화 방법에 대한 자세한 내용은 [`query_log` 설정](/ko/reference/settings/server-settings/settings#server_configuration_parameters-query-log)을 참조하십시오.
</Note>
