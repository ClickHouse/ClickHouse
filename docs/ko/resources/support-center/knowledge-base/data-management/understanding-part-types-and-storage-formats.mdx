...
<div id="storage-formats-full-vs-packed">
  ## 저장 포맷: Full vs 패킹 (ClickHouse Cloud)
</div>
...
Full 스토리지에서는 각 파트가 여러 개의 개별 파일로 이루어지며, 각 파일은 디스크에 따로 저장됩니다. 이는 오픈 소스 ClickHouse에서 기본이자 유일한 옵션입니다.

<div id="packed-storage">
  ### 패킹 스토리지 (ClickHouse Cloud 전용)
</div>
...
ClickHouse Cloud에서는 다음 조건 중 **하나라도** 참이면 해당 파트가 패킹 스토리지를 사용합니다.
...
<Info>
  **오픈 소스**
  ...
  설정 `min_bytes_for_full_part_storage`, `min_rows_for_full_part_storage`, `min_level_for_full_part_storage`는 오픈 소스 ClickHouse에도 정의되어 있지만, 패킹 스토리지 구현은 ClickHouse Cloud에서만 사용할 수 있으므로 실제로는 효과가 없습니다.
</Info>

`min_level_for_full_part_storage` 설정은 ClickHouse Cloud 환경에서 성능과 비용을 모두 최적화하는 데 사용할 수 있으며, 특히 지속적으로 삽입이 발생하는 테이블에서 유용합니다.
...
Cloud 배포에서는 `min_level_for_full_part_storage = 2`로 설정하십시오.
이렇게 하면 다음이 보장됩니다.

* 수준 0 파트(초기 삽입)는 패킹 스토리지를 사용합니다
* 수준 1 파트도 계속 패킹 스토리지를 사용합니다
* 수준 2 이상의 파트만 전체 스토리지 포맷을 사용합니다
...
이 두 개념은 서로 독립적이므로 ClickHouse Cloud 또는 오픈 소스 ClickHouse 사용 여부에 따라 모든 조합을 사용할 수 있습니다:

| 조합               | 사용 사례                      |
| ---------------- | -------------------------- |
| Wide + Full      | 로컬 스토리지의 대형 파트(오픈 소스의 기본값) |
| Wide + Packed    | 클라우드 스토리지의 대형 파트           |
| Compact + Full   | 로컬 스토리지의 소형 파트             |
| Compact + Packed | 클라우드 스토리지의 소형 파트           |
