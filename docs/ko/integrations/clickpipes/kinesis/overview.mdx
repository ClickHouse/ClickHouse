...
4. ClickPipe 이름, 설명(선택 사항), IAM role 또는 자격 증명, 기타 연결 정보를 입력하여 양식을 작성합니다.

<Image img="/images/integrations/data-ingestion/clickpipes/cp_step2_kinesis.webp" alt="연결 정보 입력" size="lg" border />

5. Kinesis 스트림과 시작 OFFSET을 선택합니다. UI에는 선택한 소스(Kafka topic 등)의 샘플 document가 표시됩니다. Kinesis streams의 경우 Enhanced Fan-out을 활성화하여 ClickPipe의 성능과 안정성을 높일 수도 있습니다(Enhanced Fan-out에 대한 자세한 내용은 [여기](https://aws.amazon.com/blogs/aws/kds-enhanced-fanout)에서 확인할 수 있습니다).

<Image img="/images/integrations/data-ingestion/clickpipes/cp_step3_kinesis.webp" alt="데이터 포맷 및 topic 설정" size="lg" border />

6. 다음 단계에서는 데이터를 새 ClickHouse table에 수집할지, 기존 table을 재사용할지 선택할 수 있습니다. 화면의 안내에 따라 table 이름, schema, 설정을 수정하십시오. 상단의 샘플 table에서 변경 사항을 실시간으로 미리 볼 수 있습니다.

<Image img="/images/integrations/data-ingestion/clickpipes/cp_step4a.webp" alt="table, schema 및 설정 구성" size="lg" border />
...
7. 또는 데이터를 기존 ClickHouse table에 수집하도록 선택할 수도 있습니다. 이 경우 UI에서 소스의 field를 선택한 대상 table의 ClickHouse field에 매핑할 수 있습니다.
...
**Permissions:** ClickPipes는 대상 table에 데이터를 쓰기 위한 전용 사용자를 생성합니다. 사용자 지정 role 또는 미리 정의된 role 중 하나를 사용해 이 내부 사용자의 role을 선택할 수 있습니다.

* `Full access`: cluster 전체에 대한 액세스 권한입니다. 대상 table과 함께 materialized view 또는 딕셔너리(Dictionary)를 사용하는 경우 유용할 수 있습니다.
  * `Only destination table`: 대상 table에 대해서만 `INSERT` 권한을 부여합니다.
...
요약 table에서는 소스 또는 ClickHouse의 대상 table에서 샘플 데이터를 표시하는 컨트롤을 제공합니다.
...
10. **축하합니다!** 첫 번째 ClickPipe를 성공적으로 설정했습니다. streaming ClickPipe인 경우 원격 데이터 소스에서 데이터를 실시간으로 수집하면서 계속 실행됩니다. 그렇지 않으면 batch를 수집한 후 완료됩니다.
