...
비밀번호 대신 AWS IAM 역할 기반 접근을 통해 ClickPipes 사용자를 인증할 수 있습니다. 다음과 같이 사용자를 생성한 후, 위에 표시된 것과 동일한 스키마(schema) 및 복제(replication) 권한을 부여하십시오:

```sql
CREATE USER 'clickpipes_iam_user' IDENTIFIED WITH AWSAuthenticationPlugin AS 'RDS';
```

ClickPipes가 이 방식으로 인증하도록 필요한 IAM role 및 policy를 설정하는 전체 단계는 [AWS IAM DB authentication (RDS/Aurora)](/ko/integrations/clickpipes/postgres/auth)에서 확인하십시오.
...
Aurora MySQL 인스턴스로의 트래픽을 제한하려면 [문서에 안내된 정적 NAT IP 주소](/ko/integrations/clickpipes/home#list-of-static-ips)를 RDS 보안 그룹의 **인바운드 규칙**에 추가하십시오.
