...
자동 백업 기능 설정에 따라 MySQL의 바이너리 로깅 활성화 여부가 결정됩니다. 자동 백업은 RDS Console에서 **Modify** &gt; **Additional configuration** &gt; **Backup**으로 이동한 다음 **Enable automated backups** 체크박스를 선택하여 인스턴스에 대해 구성할 수 있습니다(아직 선택하지 않은 경우).
