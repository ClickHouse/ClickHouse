...
4. 사전 요구사항 단계에서 구성한 소스 Postgres 데이터베이스의 연결 정보를 입력합니다.

<Info>
  연결 정보를 입력하기 전에 방화벽 규칙의 허용 목록에 ClickPipes IP 주소를 추가했는지 확인하십시오. ClickPipes IP 주소 목록은 [여기](/ko/integrations/clickpipes/networking/static-ips)에서 확인할 수 있습니다.
...
[연결 설정 가이드](/ko/integrations/clickpipes/networking/aws-privatelink)를 따르십시오.
...
<Note>
  ClickPipes가 SSH 터널을 설정할 수 있도록 SSH 배스천 호스트의 방화벽 규칙에 [ClickPipes IP 주소](/ko/integrations/clickpipes/networking/static-ips)를 허용 목록에 추가하십시오.
</Note>
...
8. 또한, 대상 [ClickHouse 테이블 파티셔닝](/ko/concepts/core-concepts/partitions)을 제어하기 위해 사용자 지정 `PARTITION BY <expr>` 표현식을 지정할 수 있습니다.
