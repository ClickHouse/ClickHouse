...
비밀번호 대신 AWS IAM 역할 기반 접근을 사용해 ClickPipes 사용자를 인증할 수 있습니다. 이를 위해서는 `rds.iam_auth_for_replication` 매개변수를 `1`로 설정해야 합니다. 다음과 같이 사용자를 생성한 후, 앞서 설명한 것과 동일한 스키마 및 복제 권한을 부여하십시오:
...
이 방식으로 ClickPipes가 인증할 수 있도록 필요한 IAM role과 정책을 설정하는 전체 단계는 [AWS IAM DB authentication (RDS/Aurora)](/ko/integrations/clickpipes/postgres/auth)를 참고하십시오.
