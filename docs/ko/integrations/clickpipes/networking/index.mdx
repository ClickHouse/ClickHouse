...
| [GCP Private Service Connect](/ko/integrations/clickpipes/networking/gcp-psc)  | 예     | GCP에서 호스팅되는 `AlloyDB` 데이터 소스에 사용할 수 있습니다. Global Access를 통한 교차 리전 연결은 아직 지원되지 않습니다. 지원되지 않는 GCP 리전의 프라이빗 `AlloyDB` 데이터 소스에 연결하려면 [SSH 터널링](#ssh-tunneling)을 사용하십시오. |
