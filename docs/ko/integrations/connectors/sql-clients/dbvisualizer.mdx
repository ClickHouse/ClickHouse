...
<div id="2-built-in-jdbc-driver-management">
  ## 2. 기본 제공 JDBC 드라이버 관리
</div>

DbVisualizer에는 ClickHouse용 최신 JDBC 드라이버가 포함되어 있습니다. 또한 최신 릴리스와 이전 버전의 드라이버를 가리키는 JDBC 드라이버 관리 기능이 기본으로 완전히 내장되어 있습니다.

<Image img="/images/integrations/sql-clients/dbvisualizer-driver-manager.webp" size="lg" border alt="ClickHouse JDBC 드라이버 구성을 보여주는 DbVisualizer Driver Manager 인터페이스" />
