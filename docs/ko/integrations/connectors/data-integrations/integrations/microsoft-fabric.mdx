...
Copy &amp; Sync를 통해 OneLake 테이블을 ClickHouse로 가져올 수 있습니다.

1. ClickHouse 항목을 열고 **Copy &amp; Sync**를 선택합니다.
2. 워크스페이스에서 Lakehouse를 선택합니다.
3. 가속할 테이블을 선택합니다.
4. 데이터의 특정 시점 스냅샷 복사를 시작합니다.

<Frame>
  ![정렬 키를 사용해 새 대상 테이블을 생성하는 Copy & Sync 구성](/images/integrations/tools/data-integration/microsoft-fabric/fabric-03-copy-sync-table-selection.webp)
</Frame>
...
<Frame>
  ![OneLake 원본 테이블에서 기존 ClickHouse 테이블로의 Copy & Sync 컬럼 매핑](/images/integrations/tools/data-integration/microsoft-fabric/fabric-04-copy-sync-existing-table.webp)
</Frame>
...
Microsoft Fabric 공개 미리 보기 기간에는 OneLake에서 ClickHouse로 데이터를 복사해도 무료입니다. Fabric 워크로드가 일반 제공되면 청구가 시작됩니다. 워크로드가 일반 제공(GA)되면 가격 정보가 공개됩니다.
...
**Open ClickHouse**를 선택하면 Microsoft 아이덴티티로 로그인된 전체 ClickHouse Cloud 콘솔이 열립니다. Fabric 워크로드에서 데이터를 ClickHouse로 복사하고 모든 SQL 문을 실행할 수 있지만, 관리 작업을 수행하려면 Cloud Console에서 서비스 관리 페이지로 이동해야 합니다.
...
OneLake의 각 Copy &amp; Sync는 ClickPipe로 실행됩니다. ClickHouse Cloud Console의 [ClickPipes 페이지](/ko/integrations/clickpipes/monitoring)에서 복사 진행률, 처리량, 오류를 확인하십시오.
...
2. **Copy &amp; Sync**를 열고 Lakehouse를 선택한 다음 `publicholidays` 테이블을 선택합니다.
...
* **Lakehouse 테이블만 지원됩니다.** Copy &amp; Sync는 동일한 워크스페이스에 있는 Lakehouse의 Delta 테이블을 읽습니다. 다른 Fabric 항목(Warehouse, KQL 데이터베이스, 미러링된 데이터베이스)은 아직 소스로 지원되지 않습니다.
...
* **역할 변경은 항목을 다음에 열 때 적용됩니다.** ClickHouse 항목을 열면 Fabric 워크스페이스 역할이 ClickHouse 조직 역할에 매핑됩니다. Fabric은 역할 변경 사항을 ClickHouse에 알릴 수 없으므로, 변경된 워크스페이스 역할은 즉시 적용되지 않으며 항목을 다음에 열 때 적용됩니다.
