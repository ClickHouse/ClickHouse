...
<Accordion title="CHOps">
  ...
  * 클러스터 개요: 종료 제어 기능을 갖춘 실시간 쿼리 모니터, 쿼리 분석 및 로그, 테이블과 파트 검사, 머지 및 뮤테이션, 분산 DDL 대기열.
  * 차트 빌더, 10가지 이상의 차트 유형, 구성 가능한 그리드 레이아웃을 갖춘 사용자 지정 대시보드.
  * 과거 메트릭을 프레임 단위로 재생할 수 있는 DVR 스타일 재생 기능을 제공하는 모니터링.
  * 임곗값 평가와 이메일 알림을 지원하는 SQL 기반 알림 규칙.
  * 접근 제어: ClickHouse 사용자 및 역할 관리, 권한 시각화, 설정 프로필 편집.
...
<Accordion title="HouseOps">
  ...
  * 구문 강조 기능이 있는 쿼리 빌더. 응답을 테이블 또는 JSON 보기로 확인할 수 있습니다.
...
<Accordion title="LightHouse">
  ...
  * 필터링 및 메타데이터를 제공하는 테이블 목록.
  * 필터링 및 정렬이 가능한 테이블 미리 보기.
  * 읽기 전용 쿼리 실행.
</Accordion>
...
<Accordion title="Grafana">
  ...
  &quot;Grafana를 사용하면 메트릭이 어디에 저장되어 있든 쿼리, 시각화, 알림 설정 및 분석을 수행할 수 있습니다. 팀과 함께 대시보드를 만들고, 탐색하고, 공유하여 데이터 기반 문화를 조성하십시오. 커뮤니티의 신뢰와 사랑을 받고 있습니다&quot; — grafana.com.
...
<Accordion title="ClickHouse 테이블 그래프">
  [ClickHouse 테이블 그래프](https://github.com/mbaksheev/clickhouse-table-graph)는 ClickHouse 테이블 간의 의존성을 시각화하는 간단한 CLI 도구입니다. 이 도구는 `system.tables` 테이블에서 테이블 간 연결 정보를 가져와 [mermaid](https://mermaid.js.org/syntax/flowchart.html) 형식의 의존성 흐름도를 생성합니다. 이 도구를 사용하면 테이블 의존성을 쉽게 시각화하고 ClickHouse 데이터베이스 내 데이터 흐름을 파악할 수 있습니다. mermaid를 사용하므로 생성된 흐름도는 보기 좋고 Markdown 문서에 쉽게 추가할 수 있습니다.
</Accordion>
...
<Accordion title="MindsDB Studio">
  [MindsDB](https://mindsdb.com/)는 ClickHouse를 포함한 데이터베이스를 위한 오픈 소스 AI 계층으로, 최첨단 머신 러닝 모델을 손쉽게 개발, 학습, 배포할 수 있습니다. MindsDB Studio(GUI)를 사용하면 데이터베이스의 데이터로 새 모델을 학습하고, 모델의 예측을 해석하며, 잠재적인 데이터 편향을 식별하고, Explainable AI 함수를 사용해 모델 정확도를 평가 및 시각화하여 머신 러닝 모델을 더 빠르게 조정하고 튜닝할 수 있습니다.
</Accordion>
...
<Accordion title="clickhouse-monitoring">
  [clickhouse-monitoring](https://github.com/duyet/clickhouse-monitoring)은 `system.*` 테이블을 활용하여 ClickHouse 클러스터를 모니터링하고 전반적인 현황을 파악할 수 있도록 지원하는 간단한 Next.js 대시보드입니다.

  기능:

  * 쿼리 모니터: 현재 쿼리, 쿼리 이력, 쿼리 리소스(메모리, 읽은 파트, file&#95;open 등), 리소스를 가장 많이 사용하는 쿼리, 가장 많이 사용되는 테이블 또는 컬럼 등
  * 클러스터 모니터: 총 메모리/CPU 사용량, 분산 큐, 전역 설정, mergetree 설정, 메트릭 등
  * 테이블 및 파트 정보: 컬럼 수준에서 크기, 행 수, 압축, 파트 크기 등의 세부 정보 제공
...
<Accordion title="CKibana">
  ...
  * ElasticSearch에서 ClickHouse로 마이그레이션한 후 사용자의 학습 비용을 최소화합니다.
</Accordion>

<Accordion title="Telescope">
  [Telescope](https://iamtelescope.net/)는 ClickHouse에 저장된 로그를 탐색하기 위한 현대적인 웹 인터페이스입니다. 세분화된 접근 제어를 통해 로그 데이터를 쿼리, 시각화, 관리할 수 있는 사용자 친화적인 UI를 제공합니다.

  기능:

  * 강력한 필터와 사용자 지정 가능한 필드 선택 기능을 갖춘 깔끔하고 반응형인 UI.
  * 직관적이고 표현력 있는 로그 필터링을 위한 FlyQL 구문.
  * 중첩된 JSON, Map 및 Array 필드를 포함해 그룹화 기능을 지원하는 시간 기반 그래프.
  * 고급 필터링을 위한 선택적 원시 SQL `WHERE` 쿼리 지원(권한 확인 포함).
  * 저장된 뷰: 쿼리 및 레이아웃을 위한 사용자 지정 UI 구성을 저장하고 공유합니다.
...
<Accordion title="Chadmin">
  [Chadmin](https://github.com/bun4uk/chadmin)은 현재 실행 중인 ClickHouse 클러스터의 쿼리와 관련 정보를 시각화하고, 필요에 따라 쿼리를 종료할 수 있는 간단한 UI입니다.
</Accordion>
