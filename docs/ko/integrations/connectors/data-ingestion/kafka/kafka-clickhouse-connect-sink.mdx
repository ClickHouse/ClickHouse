...
ClickHouse 버전 26.9부터는 이 스키마가 기본적으로 지원됩니다. 26.1부터 26.8까지의 버전에서는 `allow_experimental_nullable_tuple_type = 1` 설정이 필요합니다([이 문서 페이지](/ko/reference/settings/session-settings/allow-experimental#allow_experimental_nullable_tuple_type) 참조).
