...
CH 버전 26.1부터는 `allow_experimental_nullable_tuple_type=1`로 설정하면 이 스키마가 지원됩니다([이 문서 페이지](/ko/reference/settings/session-settings/allow-experimental#allow_experimental_nullable_tuple_type) 참조).
