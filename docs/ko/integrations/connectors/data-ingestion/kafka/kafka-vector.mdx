...
Vector는 푸시 또는 풀 모델을 통해 데이터를 가져오기 위해 [sources](https://vector.dev/docs/about/concepts/#sources)를 사용합니다. 반면 [Sinks](https://vector.dev/docs/about/concepts/#sinks)는 이벤트의 대상을 제공합니다. 따라서 여기서는 Kafka 소스와 ClickHouse 싱크를 사용합니다. Kafka는 싱크로 지원되지만 ClickHouse 소스는 제공되지 않습니다. 따라서 ClickHouse에서 Kafka로 데이터를 전송하려는 경우 Vector는 적합하지 않습니다.
