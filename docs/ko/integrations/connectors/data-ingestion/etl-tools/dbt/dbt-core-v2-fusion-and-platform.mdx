---
sidebarTitle: 'dbt Core v2, Fusion 및 Platform (베타)'
slug: /integrations/dbt/dbt-core-v2-fusion-and-platform
description: 'dbt Core v2, dbt Fusion 및 dbt 플랫폼에서의 ClickHouse 어댑터 지원 현황'
keywords: ['dbt', 'dbt core v2', 'dbt v2', 'dbt fusion', 'dbt platform']
title: 'dbt Core v2, Fusion 및 Platform'
doc_type: 'reference'
---
...
dbt는 새로운 엔진 위에서 다시 만들어지고 있으며, ClickHouse는 그 출발점부터 함께하고 있습니다. 이제 새로운 Rust 엔진(dbt Core v2 및 dbt Fusion)에서 ClickHouse를 대상으로 dbt를 실행할 수 있고, v2 커넥터를 통해 dbt 플랫폼에서 ClickHouse에 연결하는 것도 처음으로 가능해졌습니다. 프로젝트 자체는 달라지지 않습니다. 현재 dbt Core 1.x에서 실행되는 model, 테스트, profiles가 새 엔진에서도 그대로 실행되며, 달라지는 것은 바이너리뿐입니다.

ClickHouse는 dbt v2와 dbt 플랫폼을 지원하는 최초의 커뮤니티 어댑터입니다. dbt의 자체 퍼스트파티 어댑터도 함께 출시되고 있으며, dbt Labs 외부에서 나온 어댑터로는 ClickHouse가 처음입니다. dbt Core 1.x Python 어댑터인 `dbt-clickhouse`는 중단되지 않습니다. dbt 1.x releases에 맞춰 계속 최신 상태로 유지하며, 두 어댑터를 병행해 관리합니다.

<Warning>
  **production 사용에는 적합하지 않습니다.** dbt Core v2 엔진과 해당 ClickHouse 어댑터, dbt 플랫폼 연결은 아직 일반 제공(GA)되지 않았습니다. 개발 또는 staging 프로젝트에서만 사용하십시오.
</Warning>

<Note>
  **dbt Core 1.x 문서는 dbt Core v2에도 적용됩니다.** 어댑터는 두 엔진에서 동일하게 동작합니다. 즉, 다른 dbt 페이지에 설명된 profiles, model 구성, materializations, macros는 dbt Core v2, dbt Fusion, dbt 플랫폼에서 변경 없이 작동합니다. 예외 사항은 아래 [v1 vs v2 동등성](#parity) 표에 정리되어 있습니다.
</Note>

최신 상태는 [ClickHouse/dbt-clickhouse#660](https://github.com/ClickHouse/dbt-clickhouse/issues/660)(dbt Core v2 동등성, 영역별 하위 issue 각 1건)과 [ClickHouse/dbt-clickhouse#555](https://github.com/ClickHouse/dbt-clickhouse/issues/555)(dbt Fusion)에서 확인할 수 있습니다.
...
| ---------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **dbt Core 1.x** | Python 기반 dbt 엔진으로, 오픈 소스(Apache 2.0)입니다. ClickHouse 어댑터는 `dbt-clickhouse`이며, [ClickHouse GitHub organization](https://github.com/ClickHouse/dbt-clickhouse)에서 유지 관리되고 `dbt-core`와 함께 설치됩니다.                                                   | **일반 제공, 유지 관리 중.** v2와 병행하여 계속 릴리스됩니다.                                                                                                                                                                                                                                                   |
| **dbt Core v2**  | dbt 엔진을 Rust로 전면 재작성한 버전으로, 오픈 소스(Apache 2.0)입니다. 이 새 버전에는 v1의 모든 기능과 여러 성능 개선 사항이 포함되어 있습니다.                                                                                                                                                  | **베타.** dbt 문서의 [dbt v2 업그레이드 가이드](https://docs.getdbt.com/docs/dbt-versions/core-upgrade/upgrading-to-v2)와 [v2용 ClickHouse 설정 페이지](https://docs.getdbt.com/docs/local/connect-data-platform/clickhouse-setup?version=2)를 참고하십시오. ClickHouse 어댑터는 dbt 바이너리에 포함되어 제공되므로 별도로 설치할 패키지가 없습니다. |
| **dbt Fusion**   | dbt v2에 dbt Labs의 추가 기능이 더해진 형태입니다. SQL 이해, 정적 분석, LSP, VS Code 확장 기능, 컬럼 인식 등을 제공합니다. 방언별 SQL 인텔리전스는 데이터베이스마다 구축됩니다. 클로즈드 소스이며 [dbt Product Licensing Agreement](https://www.getdbt.com/dbt-fusion-engine-license-agreement)에 따라 라이선스가 부여됩니다. | **베타.** 어댑터는 Fusion 바이너리에서 실행되지만, 새로운 SQL 기능은 아직 ClickHouse에서 사용할 수 없습니다.                                                                                                                                                                                                                 |
...
dbt Core v2는 `dbt-clickhouse`의 dbt Core 1.x 통합 테스트 스위트를 기준으로 검증하고 있습니다. 베타는 핵심 dbt 워크플로와 ClickHouse Cloud 기능 집합이 문서화된 제약 범위 내에서 동작함을 의미하며, 완전한 v1 동등성과 자가 관리형 클러스터 지원은 이후에 제공됩니다.

**현재** 어댑터는 전체 v1 테스트 스위트의 81% 이상, ClickHouse Cloud에 해당하는 테스트의 92%를 통과합니다. 모든 머티리얼라이즈 방식(`view`, `table`, `incremental`, `materialized_view`, `dictionary`, `snapshot`, `seed`, `ephemeral`)이 v1과 동등한 수준이며, 전체 테이블 config 집합, 모델 계약, 데이터 테스트 및 단위 테스트, 문서 persistence, `clickhouse_s3source()`, dbt Core 1.x 연결 설정도 모두 지원됩니다. 남은 실패 항목은 아래에 나열된 자가 관리형 클러스터 작업과 일부 엔진 측 차이에서 비롯된 것입니다. 기능별 상세 내용은 [v1 대 v2 동등성](#parity)을 참조하십시오.
...
* **Fusion의 ClickHouse SQL 인텔리전스**(방언 인식 유효성 검사, 정적 분석, LSP, VS Code 확장 기능의 컬럼 인식). 어댑터는 현재 Fusion 바이너리로 실행되지만 아직 이러한 기능은 포함하지 않습니다. 그전까지 ClickHouse 프로젝트에서는 정적 분석이 `off`로 강제되며, 엔진은 컬럼 수준 계보를 기록하지 않습니다. [#736](https://github.com/ClickHouse/dbt-clickhouse/issues/736)에서 추적 중입니다.
* dbt 플랫폼에서 username 및 password 외의 **추가 인증 방식**.

## v1 대 v2 기능 동등성 {#parity}

아래 표는 `dbt-clickhouse`(v1)에서 제공되는 기능과 dbt Core v2 어댑터에서의 지원 상태를 비교한 것입니다. 참고 사항에 별도로 명시하지 않은 한, 나열된 모든 기능은 v1에서 지원됩니다. 상태 컬럼은 dbt Core v2를 기준으로 합니다. 미구현 항목의 진행 상황은 모두 [ClickHouse/dbt-clickhouse#660](https://github.com/ClickHouse/dbt-clickhouse/issues/660)에서 추적됩니다.

<Badge color="green">지원됨</Badge> 현재 동작함 · <Badge color="yellow">부분 지원</Badge> 제한적으로 동작함 · <Badge color="red">Not yet</Badge> 구현 대기 중.

### 머티리얼라이제이션 {#parity-materializations}

| 기능                                             | 동작 내용                                   | dbt Core v2                            | 참고                                                                                            |
| ---------------------------------------------- | --------------------------------------- | -------------------------------------- | --------------------------------------------------------------------------------------------- |
| `view`                                         | 표준 dbt 뷰입니다.                            | <Badge color="green">지원됨</Badge> |                                                                                               |
| `table`                                        | engine 및 DDL 옵션을 적용해 전체를 재구성합니다.        | <Badge color="green">지원됨</Badge> |                                                                                               |
...
| `distributed_table`, `distributed_incremental` | 분산 engine 뒤에 두는 세그먼트 테이블입니다.            | <Badge color="red">Not yet</Badge>     |                                                                                               |

### 모델 구성 {#parity-model-configuration}

| 기능                                            | 설명                                    | dbt Core v2                      | 참고 |
| --------------------------------------------- | ------------------------------------- | -------------------------------- | -- |
...
| HTTP client 옵션             | `connect_timeout`, `send_receive_timeout`, `sync_request_timeout`, `compress_block_size`, `server_host_name`. | <Badge color="red">Not yet</Badge>     | 허용되지만 무시되며, `server_host_name`은 사용할 수 없습니다.               |