...
<Tabs>
  ...
  <Tab title="수동 설치">
    ...
    1. 최신 Spark 커넥터 JAR(`clickhouse-spark-runtime-3.X_2.X-0.10.X.jar`)을 S3 버킷에 업로드합니다.
    2. Glue 작업이 이 버킷에 액세스할 수 있는지 확인합니다.
    3. `Job details` 탭에서 아래로 스크롤하여 `Advanced properties` 드롭다운을 펼친 다음 `Dependent JARs path`에 JAR 경로를 입력합니다.
