...
| FixedString           | bytes                   | string            | 문자열로 삽입하면 남는 바이트는 0으로 채워집니다                                                                            |
...
Python에서는 `.timestamp()`를 호출하면 naive `datetime`을 시스템의 로컬 시간대로 해석합니다. 따라서 삽입 동작이 실행 환경에 종속됩니다. 다음 방법 중 하나를 사용하는 것이 좋습니다.

1. 시간대 정보가 있는 datetime 객체를 사용합니다.
2. 프로세스의 시간대가 UTC가 되도록 합니다.
3. 의도한 시간대를 지정하거나, 명시적으로 epoch 정수로 변환합니다.
...
이러한 컬럼에 삽입할 때 ClickHouse Connect는 자체 `tzinfo`에 따라 Python 값을 변환합니다. 쿼리하면 `column_tzs` 인수로 컬럼별 재정의를 지정하지 않는 한 해당 컬럼의 시간대가 결과에 적용됩니다. `query_tz` 인수는 컬럼에 선언된 시간대를 재정의하지 않습니다.
