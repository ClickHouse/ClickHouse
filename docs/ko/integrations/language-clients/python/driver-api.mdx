...
| `show_clickhouse_errors`   | bool or None                  | `True`                      | 클라이언트 예외에 서버 오류의 세부 정보와 심볼릭 예외 이름을 포함합니다.                                                                                                               |
...
`clickhouse_connect.dbapi` 모듈은 PEP 249의 연결 및 cursor 인터페이스를 구현합니다. 이 모듈은 API 수준 2.0, `threadsafety=2`, `paramstyle="pyformat"`를 선언합니다.
...
`Cursor.execute` 및 `Cursor.executemany`는 ClickHouse 설정을 위한 추가 `settings` 키워드 인수를 받습니다. `executemany`는 행 시퀀스가 구체화된 호환 가능한 `INSERT ... VALUES` SQL 문에 대해 드라이버의 네이티브 대량 삽입 경로를 사용합니다. `fetchone`, `fetchmany`, `fetchall`은 현재 구체화된 결과에서 데이터를 가져옵니다.
...
DB-API 2.0 예외 계층을 포함한 사용자 정의 예외는 `clickhouse_connect.driver.exceptions`에 정의되어 있습니다. `DatabaseError`와 `OperationalError`는 ClickHouse 오류 코드가 담긴 숫자 `code` 속성과 `UNKNOWN_TABLE` 같은 기호 이름이 담긴 `name` 속성을 제공하므로, 애플리케이션은 메시지를 파싱하지 않고 `exc.code`를 기준으로 분기할 수 있습니다. `show_clickhouse_errors`가 비활성화되어 있어도 `code`는 설정되지만, `name`은 그렇지 않습니다. 전송 오류처럼 사용할 수 없는 경우에는 둘 다 `None`입니다. 쿼리 결과를 읽는 도중 연결이 끊기면 잘린 결과를 반환하는 대신 `StreamFailureError`가 발생합니다.
