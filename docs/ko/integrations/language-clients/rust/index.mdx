...
* ClickHouse는 모든 행이 동일한 파티션에 속하고 행 수가 [`max_insert_block_size`](/ko/reference/settings/session-settings/max-insert#max_insert_block_size) 미만인 경우에만 배치를 원자적으로 삽입합니다.
