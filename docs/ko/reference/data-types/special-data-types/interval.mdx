...
`인터벌` 컬럼에 대한 `avg`, `min`, `max`, `any`는 동일한 타입의 인터벌을 반환합니다.
`avg`는 단위 수의 평균을 구한 뒤 그 결과를 정수 단위로 내림합니다:
...
단위가 서로 다른 인터벌은 서로 비교하거나 합칠 수 없습니다. 예를 들어 한 달은 고정된 일수가 아니므로 단위 간 변환이 불가능하며, 각 인터벌 유형은 개별적으로 집계됩니다.
