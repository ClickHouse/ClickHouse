---
description: '근사 벡터 검색을 위한 세밀한 양자화를 지원하는 ClickHouse의 QBit 데이터 유형 문서'
...
`QBit` 데이터 유형은 더 빠른 근사 벡터 검색을 위해 벡터 저장 방식을 재구성합니다. 각 벡터의 요소를 함께 저장하는 대신, 모든 벡터에서 동일한 비트 위치끼리 묶어 저장합니다.
...
* `stride` – 선택 사항입니다. 하나의 stream 그룹에 함께 저장되는 차원의 수입니다. 생략하면 기본값은 `dimension`(단일 그룹)입니다. 지정하는 경우 `dimension`은 `stride`의 배수여야 하며, `stride`가 `dimension`보다 작을 때는 `stride`도 8의 배수여야 합니다. [Strides](#strides)를 참조하십시오.
...
접근 가능한 서브컬럼 수는 요소 타입에 따라 달라지며, stride가 적용된 경우 stride 그룹 수에도 따라 달라집니다([Strides](#strides) 참조):
...
축소 차원 검색을 실행하려면 전치 거리 함수의 네 번째 인수로 읽을 차원 수를 전달하십시오(아래 참조). 기준 벡터는 정확히 그 개수만큼의 원소를 가져야 하며, 이 값은 `stride`의 배수여야 합니다.
...
stride가 적용된 `QBit`의 경우, 이러한 함수는 선택적인 네 번째 인수 `used_dims`(읽을 선행 차원의 수)를 받을 수 있으며, 이 경우 해당 차원을 포함하는 stride 그룹만 읽습니다:
