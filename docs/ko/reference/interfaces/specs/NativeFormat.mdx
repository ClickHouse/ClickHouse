...
<img className="w-full h-auto" src="/images/interfaces/native-format/block-structure.webp" alt="열 지향 wire 블록의 구조와 데이터 레이아웃 패밀리" width="1568" height="976" loading="lazy" />
...
<img className="w-full h-auto" src="/images/interfaces/native-format/decoder-dispatch.webp" alt="타입 설명에서 해당 인코딩 계열로의 디코더 라우팅" width="1568" height="854" loading="lazy" />
...
따라서 revision `0` — `FORMAT Native`의 기본값 — 에서는 `LowCardinality(T)`가 **제거되지 않습니다**. 해당 타입 문자열과 블록별 상태 접두사는 스트림에 그대로 남고, revision-`0` reader는 이를 그대로 다시 읽습니다. 제거는 `54405` 미만인 0이 아닌 revision에서만 시작되거나, revision과 무관하게 강제로 제거될 때만 적용됩니다.

이 강제 동작을 제어하는 것이 `remove_low_cardinality` flag입니다. `FORMAT Native` 출력에서는 이 값을 절대 설정하지 않지만, 네이티브 TCP 경로에서는 `low_cardinality_allow_in_native_format = 0`일 때 설정합니다(기본값은 `1`). 다시 말해, 이 setting은 네이티브 TCP 출력에는 영향을 주지만 `FORMAT Native`에는 아무 영향도 주지 않습니다.

실무적으로 중요한 요점은 다음과 같습니다. 기본 `FORMAT Native` 스트림에는 `LowCardinality`가 정상적으로 포함될 수 있으므로, revision `0`에서 이 기능이 없다고 간주해서는 안 됩니다.

<div id="revision-per-channel">
  ### 데이터가 이동하는 경로에 따라 revision이 어디서 결정되는지
</div>
...
[네이티브 TCP 프로토콜](/ko/reference/interfaces/specs/NativeProtocol)에서는 revision이 Hello 핸드셰이크에서 정해집니다. 클라이언트는 `DBMS_TCP_PROTOCOL_VERSION`을 보내고, 서버는 자신의 값을 돌려보내며, 그 이후부터는 각 측이 **상대 측이 알린 revision**에 맞춰 직렬화합니다. 즉, 서버는 `client_tcp_protocol_version`을 기준으로 `NativeReader`/`NativeWriter`를 구성하고, 클라이언트는 수신한 `server_revision`을 사용합니다. 명시적인 `min`은 없지만, 어느 쪽도 구현하지 않은 기능을 전송할 수는 없으므로 각 방향은 사실상 두 피어 중 더 오래된 쪽의 revision에 의해 제한됩니다.
...
<div id="revision-output">
  #### `FORMAT Native` 출력 — 기본값은 revision 0이며, HTTP에서는 더 높일 수 있습니다
</div>

`Native` *출력* 포맷의 기본 revision은 **`0`**입니다. 여기에는 HTTP를 통한 `SELECT ... FORMAT Native`, `INTO OUTFILE ... FORMAT Native`, 그리고 `clickhouse-client`가 기록하는 `Native` 출력이 포함됩니다. 각 경우 모두 출력 팩토리가 `FormatSettings::client_protocol_version`을 그대로 `NativeWriter`에 전달합니다.

하지만 HTTP에서는 기본값이 항상 고정되는 것은 아닙니다. 클라이언트는 `?client_protocol_version=<n>` 쿼리 매개변수로 이 값을 높일 수 있으며, HTTP handler는 이를 SQL 설정이 아니라 예약된 매개변수로 처리합니다. 이 값은 쿼리 Context에 저장되고, 포맷 계층이 이를 `FormatSettings`로 복사합니다. 값을 충분히 높이면 HTTP `FORMAT Native` 출력에도 TCP 경로와 마찬가지로 `BlockInfo` 접두부와 `has_custom_serialization` 바이트가 포함되기 시작합니다. 따라서 HTTP `FORMAT Native` payload가 항상 revision `0`이라고 단정해서는 안 됩니다. 반면 파일 내보내기와 로컬 `clickhouse-client` 출력에는 이런 조정 수단이 없으므로 `0`으로 유지됩니다.

<div id="revision-input">
  #### `FORMAT Native` 입력 — 항상 revision 0
</div>

`Native` *입력* 포맷은 반대로 동작합니다. 즉, **revision `0`으로 하드코딩**되어 있으며 `client_protocol_version`은 전혀 참조하지 않습니다. `INSERT ... FORMAT Native`의 본문(body)을 파싱하든 `Native` 파일을 읽든, `NativeReader`를 리터럴 `0`으로 생성하므로 `BlockInfo` 접두사를 전혀 예상하지 않고, `has_custom_serialization` 바이트도 읽지 않으며, 항상 기본 serialization을 가정합니다.

따라서 `client_protocol_version`은 출력 전용입니다. `INSERT ... FORMAT Native` 요청에 높은 `?client_protocol_version=` 값(예: `DBMS_TCP_PROTOCOL_VERSION`)을 붙여도 본문을 읽는 방식에는 아무런 영향이 없습니다. 본문은 여전히 revision `0`이어야 합니다. `BlockInfo` 접두사나 `has_custom_serialization` 바이트가 포함된 본문을 입력하면 리더의 동기화가 어긋나고, 그 결과 성공적으로 삽입되는 것이 아니라 parse 오류(`INCORRECT_DATA` 또는 `CANNOT_READ_ALL_DATA`)가 발생합니다.

<div id="revision-round-trip">
  ### 왕복 처리 시의 영향
</div>

`FORMAT Native`에서는 양쪽 모두 revision `0`을 사용하는 것이 가장 안전하며, 이것이 기본값입니다. revision `0`에서 `SELECT ... FORMAT Native`로 기록한 데이터는 문제 없이 그대로 `INSERT ... FORMAT Native`로 다시 읽어들일 수 있습니다.

문제가 생기는 경우는 출력 revision을 의도적으로 높였을 때뿐입니다. `?client_protocol_version=<large>`를 사용한 `SELECT ... FORMAT Native`는 `BlockInfo` 및 `has_custom_serialization` 바이트를 포함하는 스트림을 생성하며, revision-`0` 입력 경로에서는 이를 다시 읽을 수 없습니다. 이런 데이터가 왕복되어야 한다면, 데이터를 생성하는 `SELECT`에서 `client_protocol_version`을 지정하지 않거나, `FORMAT Native` 대신 각 방향에서 handshake로 협상된 revision을 사용하는 네이티브 TCP 프로토콜로 데이터를 전송하십시오.
...
| HTTP를 통한 `INSERT ... FORMAT Native`                       | 해당 없음                               | `0` (고정, `client_protocol_version` 무시) | 읽지 않음                                                                 |
| `INTO OUTFILE` / 파일 / `clickhouse-client` `FORMAT Native` | `0`                                 | `0`                                    | 없음(단, `LowCardinality`는 유지됨 — 위 참고 사항 참조)                             |

<Info>
  **Protocol revision과 serialization version**

  여기서 말하는 protocol revision을 [serialization version](#serialization-version-concept)과 혼동하지 마십시오. 여기서의 revision은 connection 또는 request 전체에 적용되며 바이트 자체에는 나타나지 않습니다. 반면 serialization version은 컬럼별로 적용되며, [versioned types](#versioned-types)에 의해 전달되고 비어 있지 않은 모든 block에 기록됩니다. revision은 해당 기능 자체의 존재 여부를 결정하고, serialization version은 versioned 컬럼 내부에서 해당 타입의 인코딩 variant 중 무엇이 뒤따를지를 결정합니다.
</Info>
...
<img className="w-full h-auto" src="/images/interfaces/native-format/array-offsets.webp" alt="공유 values 스트림에 행을 매핑하는 오프셋" width="1568" height="388" loading="lazy" />
...
<img className="w-full h-auto" src="/images/interfaces/native-format/nested-layout.webp" alt="평면화 적용 여부에 따른 복합 컬럼 레이아웃" width="1172" height="1068" loading="lazy" />
...
<img className="w-full h-auto" src="/images/interfaces/native-format/state-prefix-per-block.webp" alt="비어 있지 않은 모든 block에 반복되는 상태 접두사" width="1226" height="1162" loading="lazy" />
...
<img className="w-full h-auto" src="/images/interfaces/native-format/variant-discriminators.webp" alt="조밀하게 저장된 타입별 run에서 값을 선택하는 판별자" width="1568" height="624" loading="lazy" />
...
<img className="w-full h-auto" src="/images/interfaces/native-format/compression-frame.webp" alt="압축 프레임 필드 및 압축 크기 경계" width="1568" height="206" loading="lazy" />

<div id="method-byte-values">
  ### 메서드 바이트 값
</div>

이 세 가지 코덱은 서버가 전체 스트림 `Native` 프레이밍에 사용해 출력하는 코덱입니다. HTTP `compress=1` 출력은 항상 `LZ4`를 사용하며, 네이티브 TCP 프로토콜은 `network_compression_method`에 따라 `LZ4`, `ZSTD` 또는 `NONE`을 사용합니다. 일반적인 `Native` 클라이언트는 이 세 가지만 생성하고 처리할 수 있으면 됩니다.
...
`0x9d` (`SZ3`)는 `Float32`, `Float64`, 그리고 해당 타입의 `Array`를 위한 **실험적** 오차 한계 기반 *손실* 코덱입니다. `allow_experimental_codecs`가 설정된 경우에만 `CODEC(SZ3)`로 테이블을 생성할 수 있지만, 이전에 기록된 데이터를 계속 읽을 수 있도록 메서드 바이트는 압축 해제 시 항상 허용됩니다. `0x9f` (`ZXC`)는 **실험적** 비대칭 LZ 코덱입니다. 압축은 느리지만 압축 해제는 매우 빠르며, 압축률은 `LZ4`와 `ZSTD` 사이입니다. `allow_experimental_codecs`가 설정된 경우에만 `CODEC(ZXC)`로 테이블을 생성할 수 있지만, 이전에 기록된 데이터를 계속 읽을 수 있도록 메서드 바이트는 압축 해제 시 항상 허용됩니다. 바이트 `0x99` (`DeflateQpl`)와 `0x9b` (`ZSTD_QPL`)는 이후 제거된 코덱에 할당되어 있던 값입니다. 이 값들은 예약되어 있으며 재사용되지 않습니다.

`0x9e` (`Quantized`)는 조밀한 벡터 컬럼(`Array(Float32)` 등)을 위한 **실험적** 컬럼 코덱입니다. `NONE`과 마찬가지로 패스스루 방식이므로 전체 정밀도 body가 그대로 저장됩니다. 다만 이 코덱이 있으면 벡터 검색을 가속하기 위해 사용하는 압축된 양자화 보조 스트림을 기록하는 직렬화가 함께 적용됩니다. `allow_experimental_codecs`가 설정된 경우에만 `CODEC(Quantized(...))`로 테이블을 생성할 수 있으며, 메서드 바이트는 압축 해제 시 항상 허용됩니다.
