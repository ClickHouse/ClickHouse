...
| ----------------------------------- | ------------------------------------------------------------------ | -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `BlockInfo` prefix                  | (임의의 값 `> 0`)                                                      | `1`      | [`BlockInfo`](#blockinfo) prefix가 완전히 생략되며, block은 `num_columns`, `num_rows`, columns만으로 구성됩니다.                                                                             |
| `has_custom_serialization` byte     | `DBMS_MIN_REVISION_WITH_CUSTOM_SERIALIZATION`                      | `54454`  | 컬럼별 [`has_custom_serialization`](#column-wire-layout) byte가 생략되며, 모든 컬럼이 기본 직렬화를 사용합니다(희소, 복제된, 분리된 형식 없음).                                                       |
...
| 병렬 block 마샬링 (`DETACHED`)           | `DBMS_MIN_REVISON_WITH_PARALLEL_BLOCK_MARSHALLING`                 | `54478`  | 컬럼은 `ColumnBLOB`으로 래핑되지 않으며, `DETACHED` / `DETACHED_OVER_SPARSE` kind도 나타나지 않습니다([kind&#95;stack](#kind-stack-and-sparse-encoding) 참고).                                     |
...
즉, revision `0`은 거의 모든 항목에서 가장 보수적인 인코딩입니다. 스트림에는 `BlockInfo`가 없고, `has_custom_serialization` byte도 없으며, V1 `Dynamic`/`JSON`, 집계 함수 버전 정보 없음, 그리고 시간대 매개변수가 빠진 단순한 `DateTime`만 전달됩니다.
...
따라서 revision `0` — `FORMAT Native`의 기본값 — 에서는 `LowCardinality(T)`가 **제거되지 않습니다**. 해당 타입 문자열과 block별 상태 접두사는 스트림에 그대로 남고, revision-`0` reader는 이를 그대로 다시 읽습니다. 제거는 `54405` 미만인 0이 아닌 revision에서만 시작되거나, revision과 무관하게 강제로 제거될 때만 적용됩니다.
...
`Geometry`도 별칭이지만, 중첩 배열이 아니라 [`Variant`](#variant)의 별칭입니다. 즉, 그 payload는 위 7개의 geo 타입으로 이루어진 variant입니다. 컬럼 헤더에는 타입 문자열 `Geometry`만 들어 있으며, variant가 **명시적으로 풀어서 표기되지는 않습니다**. 따라서 디코더가 이를 직접 확장해야 합니다. 사용자가 직접 표기한 `Variant`와 달리 `Geometry`의 discriminator는 이름순 정렬을 따르지 않습니다. 호환성을 위해 고정되어 있으며, 새 geo 타입은 항상 끝에만 추가됩니다. 매핑은 `0` = `LineString`, `1` = `MultiLineString`, `2` = `MultiPolygon`, `3` = `Point`, `4` = `Polygon`, `5` = `Ring`, `6` = `MultiPoint`입니다(`MultiPoint`는 26.7에 추가되었으며, 이전 서버의 스트림에는 discriminator `6`이 포함되지 않음). 이후 선택된 각 값은 위의 해당 geo 별칭을 통해 디코딩됩니다(`NULL`은 `Variant`의 `NULL` discriminator `255`를 사용함).
...
<Note>
  ...
  * `QBit(T, N[, stride])`는 벡터 검색 workload를 위해 비트 평면이 전치된 벡터를 저장합니다. 이 타입의 on-wire 스트림 레이아웃(그룹 우선 `FixedString` 비트 평면 스트림이며, 명시적인 `stride`와 함께 `element_size * (N / stride)`개로 구성됨)과 바이너리 타입 인코딩(태그 `0x36`, 또는 `stride != N`일 때 `0x37` `QBitWithStride`)은 [`QBit` 데이터 타입 페이지](/ko/reference/data-types/qbit)와 [바이너리 타입 인코딩](/ko/reference/data-types/data-types-binary-encoding) 참고 문서에 설명되어 있으므로, `Native` reader가 이를 C++ 소스에서 직접 복원할 필요는 없습니다.
</Note>
