...
* `SETTINGS`: (선택 사항) 추가 설정을 지정할 수 있습니다.
  * (선택 사항): [check&#95;query&#95;single&#95;value&#95;result](/ko/reference/settings/session-settings/check#check_query_single_value_result): 이 설정은 출력이 상세 형태(`0`)인지 요약 형태(`1`)인지를 제어합니다.
...
개별 테이블의 무결성이 아니라 전체 데이터베이스의 상태를 확인하려면 [`CHECK DATABASE`](/ko/reference/statements/check-database) 쿼리를 사용합니다. 이 쿼리는 주로 [`DataLakeCatalog`](/ko/reference/engines/database-engines/datalake) 엔진을 사용하는 데이터베이스를 위한 것으로, 외부 카탈로그에 접근할 수 있는지 확인합니다.
