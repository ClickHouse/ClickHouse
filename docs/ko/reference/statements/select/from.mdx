...
`FROM`으로 시작하는 쿼리에서는 `SELECT` 절이 선택 사항이며, 생략하면 `SELECT *`를 작성한 것처럼 작동합니다:
...
이 형식은 주로 [파이프 연산자](/ko/reference/statements/select/pipe-operators)를 사용하는 쿼리에서 사용됩니다:
...
`FINAL`이 포함된 `SELECT` 쿼리는 병렬로 실행됩니다. [max&#95;final&#95;threads](/ko/reference/settings/session-settings/max#max_final_threads) 설정은 사용되는 스레드 수를 제한합니다.
...
`FINAL`은 세션 또는 사용자 프로필을 사용해 쿼리의 모든 테이블에 [FINAL](/ko/reference/settings/session-settings/other#final) 설정으로 자동 적용할 수 있습니다.
