...
`LIMIT n BY expressions` 절이 있는 쿼리는 `expressions`의 각 고유 값마다 처음 `n`개의 행을 선택합니다. `LIMIT BY`의 키에는 개수 제한 없이 [표현식](/ko/reference/syntax#expressions)을 얼마든지 포함할 수 있습니다.
...
`LIMIT BY` 절에서 컬럼 이름 대신 컬럼 번호를 사용하려면 [enable&#95;positional&#95;arguments](/ko/reference/settings/session-settings/enable-positional-arguments#enable_positional_arguments) 설정을 활성화하세요.
