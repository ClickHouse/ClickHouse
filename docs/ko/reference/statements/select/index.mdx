...
* [PREWHERE 절](/ko/reference/statements/select/prewhere) (적용되는 단일 테이블을 필터링하므로, 이 목록에서의 위치와 관계없이 `JOIN`보다 먼저 실행됩니다)
...
쿼리는 [파이프 연산자](/ko/reference/statements/select/pipe-operators)를 사용해 변환을 선형으로 연결하는 방식으로도 작성할 수 있습니다:
...
`GROUP BY`, `ORDER BY`, `LIMIT BY` 절은 위치 인수를 지원합니다. 이를 사용하려면 [enable&#95;positional&#95;arguments](/ko/reference/settings/session-settings/enable-positional-arguments#enable_positional_arguments) 설정을 켜십시오. 그러면 예를 들어 `ORDER BY 1,2`는 테이블을 첫 번째 컬럼, 그다음 두 번째 컬럼 기준으로 행을 정렬합니다.
