...
* `ALTER` 권한에는 다른 모든 `ALTER*` 권한이 포함됩니다.
* `ALTER CONSTRAINT` 권한에는 `ALTER ADD CONSTRAINT`, `ALTER DROP CONSTRAINT` 및 `ALTER MODIFY CONSTRAINT` 권한이 포함됩니다.
...
* `SYSTEM`. 수준: `GROUP`
  ...
  * `SYSTEM RELOAD`. 수준: `GROUP`
    ...
    * `SYSTEM RELOAD DICTIONARY`. 수준: `GLOBAL`. 별칭: `SYSTEM RELOAD DICTIONARIES`, `RELOAD DICTIONARY`, `RELOAD DICTIONARIES`, `SYSTEM UNLOAD DICTIONARY`, `SYSTEM UNLOAD DICTIONARIES`, `UNLOAD DICTIONARY`, `UNLOAD DICTIONARIES`
  ...
  * `SYSTEM BACKGROUND`. 수준: `GROUP`
    * `SYSTEM STREAMING ENGINES`. 수준: `TABLE`
    * `SYSTEM VIEWS`. 수준: `VIEW`. 별칭: `SYSTEM REFRESH VIEW`, `SYSTEM START VIEWS`, `SYSTEM STOP VIEWS`, `SYSTEM START VIEW`, `SYSTEM STOP VIEW`, `SYSTEM PAUSE VIEWS`, `SYSTEM PAUSE VIEW`, `SYSTEM CANCEL VIEW`, `REFRESH VIEW`, `START VIEWS`, `STOP VIEWS`, `START VIEW`, `STOP VIEW`, `PAUSE VIEWS`, `PAUSE VIEW`, `CANCEL VIEW`
  * `SYSTEM FLUSH`. 수준: `GROUP`
    * `SYSTEM FLUSH DISTRIBUTED`. 수준: `TABLE`. 별칭: `FLUSH DISTRIBUTED`
    * `SYSTEM FLUSH LOGS`. 수준: `GLOBAL`. 별칭: `FLUSH LOGS`

`SYSTEM UNLOAD DICTIONARY` 및 `SYSTEM UNLOAD DICTIONARIES` 명령은 `SYSTEM RELOAD DICTIONARY` 권한을 재사용하므로, `SYSTEM RELOAD DICTIONARY` 권한이 있는 사용자는 딕셔너리의 로드도 해제할 수 있습니다.
...
* `dictGet`. 별칭: `dictHas`, `dictGetHierarchy`, `dictGetRoot`, `dictGetChildren`, `dictGetDescendants`, `dictIsIn`

사용자가 [dictGet](/ko/reference/functions/regular-functions/ext-dict-functions#dictGet), [dictHas](/ko/reference/functions/regular-functions/ext-dict-functions#dictHas), [dictGetHierarchy](/ko/reference/functions/regular-functions/ext-dict-functions#dictGetHierarchy), [dictGetRoot](/ko/reference/functions/regular-functions/ext-dict-functions#dictGetRoot), [dictGetChildren](/ko/reference/functions/regular-functions/ext-dict-functions#dictGetChildren), [dictGetDescendants](/ko/reference/functions/regular-functions/ext-dict-functions#dictGetDescendants), [dictIsIn](/ko/reference/functions/regular-functions/ext-dict-functions#dictIsIn) 함수를 실행할 수 있게 합니다.
...
[`display_secrets_in_show_and_select` 서버 설정](/ko/reference/settings/server-settings/settings/other#display_secrets_in_show_and_select)
및
[`format_display_secrets_in_show_and_select` 포맷 설정](/ko/reference/settings/formats/format#format_display_secrets_in_show_and_select)
