...
경량 업데이트를 사용하려면 테이블 설정 [`enable_block_number_column`](/ko/reference/settings/merge-tree-settings/enable-block#enable_block_number_column) 및 [`enable_block_offset_column`](/ko/reference/settings/merge-tree-settings/enable-block#enable_block_offset_column)을 사용해 `_block_number` 및 `_block_offset` 컬럼의 머티리얼라이즈를 활성화해야 합니다.

<div id="lightweight-delete">
  ## 경량 DELETE
</div>

[경량 `DELETE`](/ko/reference/statements/delete) 쿼리는 `ALTER UPDATE` 뮤테이션 대신 경량 `UPDATE`로 실행할 수 있습니다. 경량 `DELETE`의 동작 방식은 [`lightweight_delete_mode`](/ko/reference/settings/session-settings/lightweight#lightweight_delete_mode) 설정으로 제어됩니다.
...
동시에 실행되는 경량 업데이트의 일관성은 설정 [`update_sequential_consistency`](/ko/reference/settings/session-settings/update#update_sequential_consistency) 및 [`update_parallel_mode`](/ko/reference/settings/session-settings/update#update_parallel_mode)로 제어됩니다.
...
패치 파트는 일반 파트와 동일하지만, 업데이트된 컬럼, 테이블의 정렬 키 컬럼 및 다음 시스템 컬럼만 포함합니다.
...
패치 파트는 테이블의 정렬 키를 기준으로 정렬되며, 여기에 `_block_number` 및 `_block_offset` 컬럼이 추가됩니다.
업데이트된 행당 스토리지 오버헤드는 정렬 키 컬럼 값의 크기에 따라 달라집니다.

패치 파트는 원본 파트와 서로 다른 파티션에 속합니다.
패치 파트의 파티션 ID는 `patch-<hash of the patch part structure>-<original_partition_id>`입니다.
...
패치 파트는 테이블의 정렬 키를 기준으로 데이터 파트와 머지하여 적용됩니다. 데이터 파트와 패치 파트에서 정렬 키 컬럼 및 `_block_number`, `_block_offset` 컬럼의 값이 같으면 해당 행이 업데이트됩니다.
쿼리에서 정렬 키 컬럼을 선택하지 않더라도 패치 파트를 적용하려면 데이터 파트의 정렬 키 컬럼을 읽어야 합니다.
패치 파트 적용 시 메모리 사용량은 정렬 키 값이 같은 행의 가장 큰 범위로 제한됩니다.

<div id="patch-parts-format-in-mixed-version-clusters">
  ### 혼합 버전 클러스터에서의 패치 파트 포맷
</div>

새로 기록되는 패치 파트의 포맷은 테이블 수준 설정인 [`patch_parts_version`](/ko/reference/settings/merge-tree-settings)으로 제어됩니다.
ClickHouse 26.8 이전 버전은 레거시 `v1` 포맷만 지원하며, 앞서 설명한 현재 `v2` 포맷의 패치 파트는 읽을 수 없습니다.
이러한 버전에서 롤링 업그레이드를 수행할 때는 모든 레플리카의 업그레이드가 완료될 때까지 `v1` 포맷으로 계속 기록해야 합니다. 경량 업데이트를 사용하는 테이블에는 `patch_parts_version = 'v1'`을 설정하거나, [`compatibility`](/ko/reference/settings/session-settings/compatibility#compatibility) 설정을 업그레이드 대상 이전 버전으로 지정하십시오.
업그레이드가 완료되면 재정의를 제거하여 `v2` 포맷으로 패치 파트를 기록하기 시작하십시오.
설정 값과 관계없이 두 포맷의 패치 파트는 모두 읽고 적용할 수 있습니다.
