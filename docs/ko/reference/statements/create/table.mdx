...
`MergeTree` 엔진 계열의 경우 서버 구성의 [압축](/ko/reference/settings/server-settings/settings/other#compression) 섹션에서 기본 압축 방식을 변경할 수 있습니다.
...
* [MergeTree](/ko/reference/engines/table-engines/mergetree-family/mergetree) 계열. 컬럼 압축 코덱을 지원하며, [압축](/ko/reference/settings/server-settings/settings/other#compression) 설정을 통해 기본 압축 방식을 선택할 수 있습니다.
...
`ZXC[(level)]` — `level`을 조정할 수 있는 비대칭 [`zxc` compression algorithm](https://github.com/hellobertrand/zxc)입니다. 가능한 수준: [1, 7]. 기본 수준: 3입니다.

`ZXC`는 압축 속도는 느리지만 압축 해제는 매우 빠르며, 압축률은 `LZ4`와 `ZSTD`의 중간 정도입니다. 한 번 압축하고 여러 번 압축 해제하는 패턴에 적합하며, 최신 ARM 코어에서 가장 빠르게 압축 해제됩니다. 수준이 높을수록 압축률은 더 좋아지지만 압축 속도는 더 느려지며, 압축 해제는 계속 빠르게 유지됩니다.
...
이 코덱은 선택적 `variant` 인수를 받을 수 있습니다.

* `ALP()` 또는 `ALP(AUTO)` (기본값) — 예상 압축된 크기에 따라 STD를 사용하고, 필요하면 RD로 대체합니다.
...
* `ALP(RD)` — Real Doubles variant입니다. 각 값의 비트 패턴을 재해석하여 상위 파트(부호 + 지수 + 상위 가수 비트)와 하위 파트로 나눕니다. 상위 파트는 딕셔너리 인코딩되며(최대 8개 엔트리), 하위 파트는 비트 패킹됩니다. 많은 값이 동일한 상위 비트를 공유할 때 가장 적합합니다.

<Note>
  이 코덱은 Experimental 기능이며, 사용하려면 `SET allow_experimental_codecs = 1`을 설정해야 합니다.
</Note>
...
`SZ3` 또는 `SZ3(algorithm, error_bound_mode, error_bound)` - Float32, Float64, Array(Float32) 또는 Array(Float64) 타입의 컬럼에 사용하는, 손실 압축이지만 오차 경계가 보장되는 코덱([SZ3 Lossy Compressor](https://szcompressor.org/))입니다. 배열 컬럼은 모든 배열의 길이가 같을 때 압축 효율이 가장 높습니다(이 경우 고정 폭 벡터로 압축됨). 길이가 서로 다른 배열도 지원되며, 평탄화된 값 시퀀스로 압축됩니다. 이 코덱은 맵 컬럼에는 적용할 수 없는데, 손실 압축으로 인해 키가 손상되기 때문입니다. `algorithm`에 지원되는 값은 `ALGO_LORENZO_REG`, `ALGO_INTERP_LORENZO`, `ALGO_INTERP`입니다. `error_bound_mode`에 지원되는 값은 `ABS`, `REL`, `PSNR`, `ABS_AND_REL`입니다. 인수 `error_bound`는 최대 오류이며 Float64 타입입니다.
...
이 코덱은 실제로 데이터를 압축하는 것이 아니라, 디스크의 데이터를 암호화합니다. 이러한 코덱은 [encryption](/ko/reference/settings/server-settings/settings/other#encryption) 설정에서 암호화 키를 지정한 경우에만 사용할 수 있습니다. 또한 암호화된 데이터는 일반적으로 의미 있게 압축할 수 없으므로, 암호화는 코덱 파이프라인의 마지막 단계에서만 사용하는 것이 적절합니다.
