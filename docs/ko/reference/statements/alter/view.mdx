...
이 명령은 materialized view 또는 해당 대상 테이블을 다시 생성하거나, 기존 대상 데이터를 지우거나, 이미 실행 중인 갱신을 취소하지 않고 기존 뷰의 갱신 구성을 직접 업데이트합니다. 구성이 변경되는 동안에도 기존 대상 테이블에 대한 읽기는 계속됩니다.

동일한 완전한 `REFRESH` 사양으로 명령을 반복해도 안전합니다. 위에서 설명한 대로 실행할 때마다 모든 갱신 매개변수가 대체되므로, 구성 상태를 유지해야 하는 모든 절을 반드시 반복하십시오.
