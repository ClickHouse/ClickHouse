...
제공된 Geometry(링 또는 (multi-)polygon)에 포함된 지정된 해상도의 육각형을 반환합니다. Geometry가 육각형의 중심을 포함할 때 해당 육각형이 포함된 것으로 간주하며, 이는 [h3PolygonToCellsWithContainment](#h3polygontocellswithcontainment)의 포함 모드 `0`입니다.

Geometry의 모든 꼭짓점은 구면 위에 있어야 합니다. 경도는 `[-180, 180]`, 위도는 `[-90, 90]`도 범위여야 합니다. 반환되는 셀의 순서는 보장되지 않으므로 순서가 중요할 때는 배열을 정렬하십시오.
...
* 선택한 포함 모드에 따른 H3 인덱스 배열입니다. [배열](/ko/reference/data-types/array)([UInt64](/ko/reference/data-types/int-uint)).
...
* 미터 단위의 정확한 에지의 길이. [Float64](/ko/reference/data-types/float).
...
* 킬로미터 단위의 정확한 에지 길이입니다. [Float64](/ko/reference/data-types/float).
...
* 라디안 단위의 정확한 에지 길이입니다. [Float64](/ko/reference/data-types/float).
...
GeoCoord 점(위도/경도) 쌍 간의 &quot;great circle&quot; 또는 &quot;haversine&quot; 거리를 미터 단위로 반환합니다.
...
* `lat1`, `lon1` — 점의 위도와 경도(도 단위)입니다. [Float64](/ko/reference/data-types/float).
...
GeoCoord 점(위도/경도) 쌍 사이의 &quot;great circle&quot; 또는 &quot;haversine&quot; 거리를 킬로미터 단위로 반환합니다.
...
* 킬로미터 단위의 haversine 또는 great circle 거리입니다. [Float64](/ko/reference/data-types/float).
...
인접한 두 H3 셀 인덱스(원점 및 대상)에 대한 에지 H3 인덱스를 반환합니다. 입력 중 하나라도 유효한 H3 셀이 아니면 예외가 발생합니다(`functions_h3_default_if_invalid` 설정으로 제어됨). 유효하지만 서로 이웃하지 않는 셀(같은 셀이 두 번 지정된 경우 포함)의 경우 `0`을 반환합니다.
...
* `originIndex` — 원점 육각형의 인덱스 번호. [UInt64](/ko/reference/data-types/int-uint).
...
에지 H3Index에서 원점 육각형 인덱스를 반환합니다. 입력이 유효한 방향성 에지가 아니면 예외를 발생시킵니다(`functions_h3_default_if_invalid` 설정으로 제어됨). 해당 설정이 활성화된 경우에만 `0`을 반환합니다.
...
* 기본적으로 유효하지 않은 방향성 에지에 대해 `INCORRECT_DATA`를 발생시키며, `functions_h3_default_if_invalid = 1`이면 `0`을 반환합니다.
...
에지 H3Index로부터 대상 육각형 인덱스를 반환합니다. 입력이 유효한 방향성 에지가 아니면 예외를 발생시킵니다(`functions_h3_default_if_invalid` 설정으로 제어됨). 해당 설정이 활성화된 경우에만 `0`을 반환합니다.
...
주어진 에지 H3Index로부터 원점 및 대상 육각형 인덱스를 반환합니다.
...
* `origin` — 원점 육각형 인덱스 번호입니다. [UInt64](/ko/reference/data-types/int-uint).
* `destination` — 대상 육각형 인덱스 번호입니다. [UInt64](/ko/reference/data-types/int-uint).

제공된 입력이 유효한 방향성 에지가 아니면 예외를 발생시킵니다(`functions_h3_default_if_invalid` 설정으로 제어됨). 해당 설정이 활성화된 경우에만 `(0,0)`을 반환합니다.
...
지정된 H3Index의 모든 에지를 반환합니다. 입력이 유효한 H3 셀이 아닌 경우 예외를 발생시킵니다(`functions_h3_default_if_invalid` 설정으로 제어됨). 이 설정이 활성화된 경우에만 빈 배열을 반환합니다.
...
* `index` — 셀을 나타내는 육각형 인덱스 번호입니다. [UInt64](/ko/reference/data-types/int-uint).

**반환 값**

* 각 에지를 나타내는 H3 인덱스의 배열입니다. [배열](/ko/reference/data-types/array)([UInt64](/ko/reference/data-types/int-uint)).
* 기본적으로 잘못된 셀에 대해 `INCORRECT_DATA`를 발생시키며, `functions_h3_default_if_invalid = 1`이면 `[]`를 반환합니다.
...
에지를 구성하는 좌표를 반환합니다. 입력이 유효한 방향성 에지가 아니면 예외를 발생시킵니다(`functions_h3_default_if_invalid` 설정으로 제어됨). 해당 설정이 활성화된 경우에만 빈 배열을 반환합니다.
...
* &#39;(lat, lon)&#39; 쌍으로 이루어진 배열입니다. [배열](/ko/reference/data-types/array)([Float64](/ko/reference/data-types/float), [Float64](/ko/reference/data-types/float)).
* 기본적으로 잘못된 방향성 에지에서는 `INCORRECT_DATA`를 발생시키며, `functions_h3_default_if_invalid = 1`인 경우 `[]`를 반환합니다.
