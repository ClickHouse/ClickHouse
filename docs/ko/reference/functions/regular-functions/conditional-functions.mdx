...
### 조건식 결과 직접 사용하기 {#using-conditional-results-directly}
...
### 조건식의 NULL 값 {#null-values-in-conditionals}
...
<Note title="`NULL` 동작을 변경하려면 `least_greatest_legacy_null_behavior` 설정을 사용하십시오">
...
[`short_circuit_function_evaluation`](/ko/reference/settings/session-settings/short-circuit-function-evaluation#short_circuit_function_evaluation) 설정은 단락 평가를 사용할지 여부를 제어합니다.
...
<Note title="`NULL` 동작을 변경하려면 설정 `least_greatest_legacy_null_behavior`를 사용하세요">
  버전 [24.12](/ko/resources/changelogs/oss/2024#a-id2412a-clickhouse-release-2412-2024-12-19)에서는 하위 호환되지 않는 변경이 도입되어, 이제 `NULL` 값은 무시됩니다. 이전에는 인수 중 하나가 `NULL`이면 `NULL`을 반환했습니다.
...
설정 [`short_circuit_function_evaluation`](/ko/reference/settings/session-settings/short-circuit-function-evaluation#short_circuit_function_evaluation)은
