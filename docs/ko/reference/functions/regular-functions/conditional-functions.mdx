...
인수 중 가장 큰 값을 반환합니다. 반환 값의 타입은 호환 가능한 가장 큰 타입으로 승격됩니다. [`Any`](/ko/reference/data-types/index)
...
:::note `NULL` 동작을 변경하려면 설정 `least_greatest_legacy_null_behavior`를 사용하세요
버전 [24.12](/ko/resources/changelogs/oss/2024#a-id2412a-clickhouse-release-2412-2024-12-19)에서는 하위 호환되지 않는 변경이 도입되어, 이제 `NULL` 값은 무시됩니다. 이전에는 인수 중 하나가 `NULL`이면 `NULL`을 반환했습니다.
...
인수 중 가장 작은 값을 반환합니다. 반환 타입은 호환 가능한 가장 큰 타입으로 승격됩니다. [`Any`](/ko/reference/data-types/index)
