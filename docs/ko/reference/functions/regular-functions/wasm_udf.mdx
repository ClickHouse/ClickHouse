...
## 시스템 테이블을 통해 WASM 모듈 관리 {#manage-wasm-modules-via-system-table}
...
### 클러스터 전체에 모듈 배포 {#distribute-a-module-across-a-cluster}
...
데이터는 WASM 메모리의 버퍼를 통해 교환됩니다. 버퍼는 데이터에 대한 포인터와 데이터 크기를 저장하는 8바이트 구조체이며, 리틀 엔디언 `u32` 값 2개로 구성됩니다. 버퍼는 데이터 자체가 아니라 이 구조체를 가리키는 포인터인 handle로 전달됩니다. 게스트 코드는 이러한 버퍼를 생성하고 제거하기 위한 2개의 함수를 내보내야 합니다. 크기가 0이 아닌 경우 데이터 포인터도 0이 아니어야 합니다. 양수 크기를 선언하면서 선형 메모리 오프셋 `0`을 가리키는 버퍼는 유효한 버퍼가 아니며, 게스트 코드는 이러한 버퍼를 생성해서는 안 됩니다. ClickHouse는 이러한 버퍼를 거부하고 `WASM_ERROR`와 함께 쿼리를 실패시킵니다.
...
함수는 입력 블록마다 한 번 호출됩니다. 대규모 쿼리는 쿼리 파이프라인에 의해 여러 블록으로 분할되므로, 각 호출에서 전달되는 행 수는 전체 쿼리가 아니라 해당 블록의 크기입니다.

파이프라인 블록도 모듈의 선형 메모리에 비해 지나치게 클 수 있으므로 추가로 분할됩니다. 기본적으로 `webassembly_udf_input_split_memory_ratio`가 이를 동적으로 분할합니다. ClickHouse는 직렬화된 행을 측정하여 입력이 게스트 선형 메모리의 해당 비율을 넘어서기 전에 새 호출을 시작하므로, 행 수는 데이터에 따라 달라집니다. `webassembly_udf_max_input_block_size`가 0이 아니면 이를 대체하여 호출당 지정된 행 수의 고정 상한이 적용되며, 비율이 `0`이면 전체 블록을 한 번의 호출로 전달합니다. 호출은 결코 1개 행 미만으로 줄어들지 않으므로, 단일 행 자체가 모듈 메모리에 비해 너무 큰 경우에는 여전히 게스트 내부에서 실패합니다.
...
freestanding C로 작성된 완전한 예시입니다. `str_reverse`는 `serialization_format = 'RowBinary'`를 사용하여 각 입력 문자열의 바이트를 뒤집습니다. 모듈 인스턴스는 여러 블록에 걸쳐 재사용되므로 `clickhouse_destroy_buffer`가 실제로 메모리를 회수해야 합니다. 여기서는 모든 버퍼가 소멸되면 할당자가 초기화됩니다.
...
### Rust로 UDF를 개발할 때 참고할 사항 {#note-for-developing-udfs-in-rust}
...
## 모듈에서 사용할 수 있는 호스트 API {#host-api-available-to-modules}
...
* `webassembly_udf_max_input_block_size` — 모든 ABI에 대해, 단일 블록에서 WebAssembly UDF에 전달되는 최대 행 수입니다. 0으로 설정하면 `ABI BUFFERED_V1` 함수의 호출 크기를 `webassembly_udf_input_split_memory_ratio`가 결정하며, 다른 ABI는 파이프라인 블록 전체를 받습니다.

* `webassembly_udf_input_split_memory_ratio` — 모듈이 시작 시점에 보유한다고 선언한 선형 메모리(페이지 없이 시작하는 모듈의 경우 메모리 상한) 중 한 번의 호출 입력이 차지할 수 있는 비율입니다. `webassembly_udf_max_input_block_size`가 0일 때 `ABI BUFFERED_V1` 함수의 블록을 분할하는 데 사용되며, 다른 ABI는 직렬화된 입력 블록을 전달하지 않으므로 이 설정을 무시합니다. 기본값: `0.5`로, 나머지 메모리는 게스트 자체의 작업 집합에 남겨둡니다 — 비율이 1에 가까우면 게스트의 할당자 내부에서 여전히 실패할 수 있습니다. 파이프라인 블록 전체를 한 번의 호출로 전달하려면 0으로 설정하거나 `compatibility`를 26.9 이전 버전으로 설정합니다. 값은 0 이상 1 이하여야 합니다.
