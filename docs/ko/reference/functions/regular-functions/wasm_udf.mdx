...
함수는 모듈 내보내기에 포함되어야 합니다. 위와 같이 `__attribute__((export_name("...")))`로 표시하거나 `wasm-ld --export-all`로 링크하십시오. `-fno-builtin`은 clang이 단순한 바이트 루프를 표준 라이브러리 없이는 사용할 수 없는 `memcpy`/`memset` 호출로 변환하는 것을 방지합니다.
