...
n-그램과 라플라스 스무딩을 사용하는 Naive Bayes 모델로 입력 텍스트를 분류합니다.
사용 전에 ClickHouse에서 모델을 구성해야 합니다.

**구현 세부 사항**

*알고리즘*

[라플라스 스무딩](https://en.wikipedia.org/wiki/Additive_smoothing)을 적용한 Naive Bayes 분류 알고리즘을 사용하며, [여기](https://web.stanford.edu/~jurafsky/slp3/4.pdf)에서 설명한 n-그램 확률을 기반으로 학습 데이터에 없던 n-그램을 처리합니다.

*주요 기능*

* 모든 크기의 n-그램을 지원합니다.
* 3가지 토큰화 모드:
  * `byte`: 원시 바이트(raw bytes) 단위로 처리합니다. 각 바이트는 하나의 토큰입니다.
  * `codepoint`: UTF-8에서 디코딩한 유니코드 스칼라 값 단위로 처리합니다. 각 코드포인트는 하나의 토큰입니다.
  * `token`: 연속된 유니코드 공백(정규식 `\s+`)을 기준으로 분할합니다. 토큰은 공백이 아닌 부분 문자열이며, 인접한 구두점은 토큰의 일부로 포함됩니다(예: `you?`는 하나의 토큰입니다).

**모델 구성**

언어 감지를 위한 Naive Bayes 모델을 생성하는 예시 소스 코드는 [여기](https://github.com/nihalzp/ClickHouse-NaiveBayesClassifier-Models)에서 확인할 수 있으며, 예시 모델과 관련 구성 파일은 [여기](https://github.com/nihalzp/ClickHouse-NaiveBayesClassifier-Models/tree/main/models)에서 확인할 수 있습니다.

ClickHouse에서 Naive Bayes 모델용 예시 구성:
...
| 매개변수     | 설명                                                                     | 예시                                                       | 기본값   |
| -------- | ---------------------------------------------------------------------- | -------------------------------------------------------- | ----- |
| `name`   | 고유한 모델 식별자입니다.                                                         | `language_detection`                                     | 필수    |
| `path`   | 모델 바이너리의 전체 경로입니다.                                                     | `/etc/clickhouse-server/config.d/language_detection.bin` | 필수    |
| `mode`   | 토큰화 방법입니다: `byte`(바이트 시퀀스), `codepoint`(유니코드 문자) 또는 `token`(단어 토큰)입니다. | `token`                                                  | 필수    |
| `n`      | n-그램 크기입니다: `1`(단일 단어), `2`(단어 쌍) 또는 `3`(단어 3개 묶음)입니다.                 | `2`                                                      | 필수    |
| `alpha`  | 모델에 없는 n-그램을 분류할 때 사용하는 라플라스 스무딩 계수입니다.                                | `0.5`                                                    | `1.0` |
| `priors` | 클래스 확률(문서가 각 클래스에 속할 비율)입니다.                                           | 클래스 0 60%, 클래스 1 40%                                     | 균등 분포 |

**모델 학습 가이드**

*파일 포맷*

사람이 읽기 쉬운 포맷에서 `n=1`이고 `token` 모드인 경우, 모델은 다음과 같을 수 있습니다:
...
`n=3`이고 `codepoint` 모드일 때는 다음과 같이 표시될 수 있습니다:
...
사람이 읽을 수 있는 형식은 ClickHouse에서 직접 사용하지 않으므로, 아래에 설명된 바이너리 형식으로 변환해야 합니다.
...
문서 코퍼스로부터 모델을 생성하기 전에, 지정된 `mode` 및 `n`에 따라 n-그램을 추출할 수 있도록 문서를 전처리해야 합니다.
...
1. 토큰화 모드에 따라 각 문서의 시작과 끝에 경계 마커를 추가합니다:
   ...
   * `token`: `<s>` (시작), `</s>` (끝)
   ...
   참고: 문서의 앞과 뒤에 각각 `(n - 1)`개의 토큰이 추가됩니다.

2. `token` 모드에서 `n=3`인 예시:
   * 문서: `ClickHouse is fast`
   * 처리 방식: `<s> <s> ClickHouse is fast </s> </s>`
   * 생성된 3-그램:
     * `<s> <s> ClickHouse`
     * `<s> ClickHouse is`
     * `ClickHouse is fast`
...
`byte` 및 `codepoint` 모드에서 모델 생성을 단순화하려면, 먼저 문서를 토큰화한 뒤(`byte` 모드에서는 바이트 목록, `codepoint` 모드에서는 코드포인트 목록), 앞에 `n - 1`개의 시작 토큰과 뒤에 `n - 1`개의 종료 토큰을 추가하고, затем n-그램을 생성해 직렬화된 파일에 기록하면 편리할 수 있습니다.
...
* `model_name` — 미리 구성된 모델의 이름입니다. 모델은 ClickHouse의 설정 파일에 정의되어 있어야 합니다. [`String`](/ko/reference/data-types/string)
* `input_text` — 분류할 텍스트입니다. 입력값은 제공된 그대로 처리됩니다(대소문자/구두점 유지). [`String`](/ko/reference/data-types/string)

**반환 값**

부호 없는 정수 형식의 예측 클래스 ID입니다. 클래스 ID는 모델을 구성할 때 정의된 범주에 대응합니다. [`UInt32`](/ko/reference/data-types/int-uint)
