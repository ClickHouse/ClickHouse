...
AI 함수는 프로바이더 자격 증명과 구성을 저장하는 **명명된 컬렉션**을 참조합니다. 서로 다른 함수 또는 함수 호출마다 서로 다른 명명된 컬렉션을 생성해 사용할 수 있습니다. 예를 들어 텍스트 함수(`aiGenerate`, `aiClassify`, `aiFilter`, `aiExtract`, `aiaiTranslate`)에 사용할 명명된 컬렉션과 임베딩 함수(`aiEmbed`, `aiSimilarity`)에 사용할 명명된 컬렉션을 별도로 정의할 수 있습니다. 각 함수는 서로 다른 엔드포인트가 필요하며, 일반적으로 사용하는 모델도 다릅니다.
...
2. 그렇지 않으면 해당 기본 자격 증명 설정:
   * 텍스트 함수(`aiGenerate`, `aiClassify`, `aiFilter`, `aiExtract`, `aiTranslate`)의 경우 [`ai_function_text_default_credentials`](/ko/reference/settings/session-settings/ai-function#ai_function_text_default_credentials)
...
두 텍스트의 임베딩을 계산하고 두 벡터 간 코사인 유사도를 `[-1, 1]` 범위의 값으로 반환합니다.
`1`은 텍스트가 의미적으로 동일함을, `0`은 서로 관련이 없음을, 음수 값은 서로 반대임을 의미합니다.
이는 동일한 임베딩에 대한 `cosineDistance`의 보수입니다
