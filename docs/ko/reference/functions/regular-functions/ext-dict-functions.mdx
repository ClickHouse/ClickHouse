...
[정규 표현식 트리 딕셔너리](/ko/reference/statements/create/dictionary/layouts/regexp-tree)에서 각 키와 일치한 모든 노드의 속성 값을 가져옵니다.
...
딕셔너리 구성과 관계없이 딕셔너리 속성 값을 `Int32` 데이터 타입으로 변환합니다.
...
딕셔너리 구성과 관계없이 딕셔너리 속성 값을 `Int64` 데이터 타입으로 변환합니다.
...
딕셔너리 구성과 관계없이 딕셔너리 속성 값을 `Int8` 데이터 타입으로 변환하며, 키를 찾지 못한 경우에는 지정한 기본값을 반환합니다.
...
[계층형 딕셔너리](/ko/reference/statements/create/dictionary/layouts/hierarchical#hierarchical-dictionaries)에서 키의 최상위 조상(루트)을 반환합니다.

이는 [dictGetHierarchy](#dictGetHierarchy)가 반환하는 배열의 마지막 요소를 가져오는 간편한 표현입니다. 즉, `dictGetHierarchy(dict_name, key)[-1]`입니다.

키 자체가 계층 구조의 최상위 노드이면 해당 키를 반환합니다. 키가 딕셔너리에 없으면 `0`을 반환합니다.
...
* `dict_name` — 딕셔너리 이름입니다. [`String`](/ko/reference/data-types/string)
* `key` — 키 값입니다. [`UInt64`](/ko/reference/data-types/int-uint)

**반환 값**

키의 최상위 조상(루트)을 반환합니다. [`UInt64`](/ko/reference/data-types/int-uint)
...
딕셔너리 구성과 관계없이 딕셔너리 속성 값을 `String` 데이터 타입으로 변환합니다.
...
딕셔너리 속성 값을 딕셔너리 구성과 관계없이 `UUID` 데이터 타입으로 변환합니다.
...
* `dict_name` — 딕셔너리 이름입니다. [`String`](/ko/reference/data-types/string)
* `attr_name` — 딕셔너리 컬럼의 이름입니다. [`String`](/ko/reference/data-types/string) 또는 [`Tuple(String)`](/ko/reference/data-types/tuple)
* `id_expr` — 키 값입니다. 키 타입 값 또는 튜플 값을 반환하는 표현식입니다(딕셔너리 구성에 따라 달라짐). [`Expression`](/ko/reference/data-types/special-data-types/expression) 또는 [`Tuple(T)`](/ko/reference/data-types/tuple)
