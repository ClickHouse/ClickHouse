...
집계 함수로 수행하는 계산과 비슷한 계산을 수행하는 데 사용할 수 있지만, 윈도우 함수는 행을 하나의 출력으로 그룹화하지 않으므로 개별 행이 그대로 반환됩니다.
...
| `GROUPS` 프레임                                                       | ✅     | 프레임 경계는 전체 피어 그룹(`ORDER BY` 키에서 같은 행)을 계산합니다. `N PRECEDING` 및 `N FOLLOWING`은 현재 행의 피어 그룹 전후에 있는 `N`개의 피어 그룹을 계산합니다.                                                                                                                                                                                                        |
...
* `ROWS`, `RANGE` 또는 `GROUPS` - 프레임의 경계를 정의하며, aggregate&#95;function은 프레임 내에서 계산됩니다. `ROWS`는 물리적 행을 세고, `RANGE`는 `ORDER BY` 값을 세며, `GROUPS`는 피어 그룹(`ORDER BY` 키 값이 같은 행)을 셉니다.
...
`GROUPS` 프레임은 물리적 행(`ROWS`)이나 `ORDER BY` 값(`RANGE`)이 아니라 `ORDER BY` 키가 같은 행들로 이루어진 전체 *피어 그룹*을 기준으로 계산합니다. `N PRECEDING` 및 `N FOLLOWING`은 현재 행이 속한 피어 그룹의 앞뒤에 있는 `N`개의 피어 그룹을 세며, 포함되는 피어 그룹은 항상 해당 그룹의 모든 행을 포함합니다.

아래 쿼리는 `ROWS`, `RANGE`, `GROUPS` 프레임에 동일한 `1 PRECEDING AND 1 FOLLOWING` 범위를 적용합니다. `order` 컬럼에는 중복되거나 연속되지 않는 값이 있으므로 세 모드에서 포함되는 행이 서로 다릅니다:
...
각 모드는 경계를 서로 다르게 해석합니다:

* `ROWS`는 물리적 행을 기준으로 계산하므로 프레임에는 현재 행과 양쪽의 행 1개씩, 최대 3개의 연속된 행이 포함됩니다.
* `RANGE`는 `order` 값을 기준으로 계산하므로 `1 PRECEDING` 및 `1 FOLLOWING`은 `order` 값이 현재 행의 `order` 값에서 1 이내인 행을 포함합니다. 간격이 10이면 인접 행은 조건을 충족하지 않으므로 프레임에는 현재 `order` 값을 공유하는 행만 포함됩니다.
* `GROUPS`는 피어 그룹을 기준으로 계산하므로 `1 PRECEDING` 및 `1 FOLLOWING`은 `order` 값 간의 간격과 관계없이 항상 인접 그룹 전체를 포함합니다.
