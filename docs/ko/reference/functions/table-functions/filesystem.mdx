...
`clickhouse-server` 모드에서는 경로가 [user&#95;files&#95;path](/ko/reference/settings/server-settings/settings/user#user_files_path) 디렉터리 내에 있어야 합니다. `user_files_path` 내부에 있으면서 외부를 가리키는 심볼릭 링크는 따라가지만, (심볼릭 링크를 통해) 경로가 `user_files_path`로 시작하는 항목만 반환됩니다.
