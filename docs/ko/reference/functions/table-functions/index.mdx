...
<Note>
  대부분의 테이블 함수는 실행하는 데 `CREATE TEMPORARY TABLE` 권한이 필요하므로,
  [`readonly`](/ko/concepts/features/configuration/settings/permissions-for-queries#readonly)이 1로 설정되어 있으면
  이러한 함수를 실행하는 `SELECT` 또는 `INSERT`는 거부됩니다.
</Note>