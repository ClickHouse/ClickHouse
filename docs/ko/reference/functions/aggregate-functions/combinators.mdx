...
이 조합자는 집계 함수의 결과를 [널 허용(Nullable)](/ko/reference/data-types/nullable) 데이터 타입으로 변환합니다. 집계 함수가 계산할 값이 없으면 [NULL](/ko/reference/settings/formats/input-format#input_format_null_as_default)을 반환합니다.
...
또한 `-OrNull`은 다른 조합자와 함께 사용할 수도 있습니다. 집계 함수가 빈 입력을 받지 못할 때 유용합니다.
...
* `stop` — `resampling_key` 값에 대해 필요한 전체 인터벌의 끝 값입니다. 전체 인터벌에는 `stop` 값이 포함되지 않습니다(`[start, stop)`). 이 값은 `start`보다 커야 하며, 하위 인터벌을 하나도 포함하지 않는 범위는 예외를 발생시킵니다.
