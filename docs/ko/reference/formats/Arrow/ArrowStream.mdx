...
| `output_format_arrow_unsupported_types_as_binary`                            | Arrow에 해당하는 타입이 없는 타입(예: `BFloat16`, `AggregateFunction`)을 원시 바이너리 데이터로 출력합니다. false이면 이러한 타입은 예외를 발생시킵니다. 이 설정은 ClickHouse 네이티브 writer와 Apache Arrow 라이브러리 writer 모두에 적용됩니다. | `1`         |
