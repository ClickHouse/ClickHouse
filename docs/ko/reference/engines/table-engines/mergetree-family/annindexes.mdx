...
개략적으로 보면, ClickHouse는 가장 유사한 벡터와 그 거리를 가상 컬럼 `_distances`를 통해 제공합니다.
...
<Note>
  재점수화 없이 실행되는 쿼리(`vector_search_with_rescoring = 0`)라도 병렬 레플리카가 활성화되어 있으면 재점수화로 폴백될 수 있습니다.
</Note>
