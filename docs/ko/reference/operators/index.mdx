...
`zone`은 유효한 시간대 이름으로 평가되는 임의의 상수 문자열 표현식일 수 있습니다(예: `'America/Denver'`, `'UTC'`, 또는 `concat('America', '/', 'Denver')`). `AT TIME ZONE`은 `toTimeZone`으로 변환되므로 동일한 시간대 인수 규칙이 적용됩니다. 따라서 컬럼 참조와 같은 비상수 표현식을 사용하려면 [`allow_nonconst_timezone_arguments = 1`](/ko/reference/settings/session-settings#allow_nonconst_timezone_arguments)이 필요합니다.

`AT LOCAL`은 현재 [세션 시간대](/ko/reference/settings/session-settings#session_timezone)을 사용합니다(세션 시간대이 설정되지 않은 경우 서버 기본값 사용). `Distributed` 테이블에서는 `session_timezone`을 명시적으로 설정해야 합니다. 비어 있으면 `timeZone()`은 세그먼트 로컬이므로 상수 `toTimeZone` 인수로 사용할 수 없어 `ILLEGAL_COLUMN` 예외가 발생합니다.

<Note>
  PostgreSQL에서는 `timestamp without time zone AT TIME ZONE zone`이 변환 전에 시계상 값을 지정된 시간대의 값으로 다시 해석하지만, ClickHouse는 항상 동일한 절대 시점을 유지하고 표시할 때 사용하는 시간대 레이블만 변경합니다. 두 형식 모두 `toTimeZone`과 동일하며 기본 timestamp 값은 변경하지 않습니다.
</Note>
