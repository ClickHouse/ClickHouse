...
<div id="notmatch-function">
  ### notMatch 함수
</div>

`a !~ b` – `notMatch(a, b)` 함수(`a`가 정규식 `b`와 일치하지 않는지 확인합니다).

<div id="matchcaseinsensitive-function">
  ### matchCaseInsensitive 함수
</div>

`a ~* b` – `matchCaseInsensitive(a, b)` 함수(대소문자를 구분하지 않는 정규식 일치)입니다.

<div id="notmatchcaseinsensitive-function">
  ### notMatchCaseInsensitive 함수
</div>
...
[optimize&#95;functions&#95;to&#95;subcolumns](/ko/reference/settings/session-settings/optimize#optimize_functions_to_subcolumns) 설정을 활성화하면 이 동작을 최적화할 수 있습니다. `optimize_functions_to_subcolumns = 1`이면 함수는 전체 컬럼 데이터를 읽고 처리하는 대신 [null](/ko/reference/data-types/nullable#finding-null) 서브컬럼만 읽습니다. `SELECT n IS NULL FROM table` 쿼리는 `SELECT n.null FROM TABLE`로 변환됩니다.
