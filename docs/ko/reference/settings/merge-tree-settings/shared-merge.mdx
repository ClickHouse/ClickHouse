...
<div id="shared_merge_tree_blobs_list_inline_file_max_bytes">
  ## shared_merge_tree_blobs_list_inline_file_max_bytes
</div>

<SettingsInfoBlock type="UInt64" default_value="0" />

<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.9"},{"label": "0"},{"label": "blob-list 파트의 작은 파일을 별도 blob 대신 통합된 blobs.list에 인라인으로 저장하는 새로운 설정"}]}]} />

blob-list 파트의 파일 중 크기가 이 바이트 수 이하인 파일(예:
count.txt, metadata&#95;version.txt, MinMax 인덱스)을 별도의 객체 스토리지 blob 대신
통합된 blobs.list 파일에 직접 인라인으로 저장합니다. 이를 통해 작은 파일마다 blob 쓰기와 읽기를 각각 1회 절약할 수 있습니다. 0으로 설정하면 인라인 저장이 비활성화됩니다.
blob-list 스토리지를 사용하는 파트에만 적용됩니다(참조:
shared&#95;merge&#95;tree&#95;use&#95;blobs&#95;list&#95;for&#95;parts). ClickHouse Cloud에서만 사용할 수 있습니다.
...
<SettingsInfoBlock type="MergeCoordinatorDistributionAlgorithm" default_value="sainte_lague" />

<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.8"},{"label": "sainte_lague"},{"label": "기본적으로 Sainte-Lague 분배를 활성화합니다."}]}, {"id": "row-2","items": [{"label": "26.7"},{"label": "water_filling"},{"label": "머지 코디네이터가 레플리카 간 머지를 분배할 때 사용할 알고리즘을 제어하는 새로운 설정"}]}]} />

머지 코디네이터 스레드가 레플리카 간 머지를 분배하는 데 사용하는 알고리즘입니다.
...
<div id="shared_merge_tree_use_blobs_list_for_parts">
  ## shared_merge_tree_use_blobs_list_for_parts
</div>

<SettingsInfoBlock type="Bool" default_value="0" />

<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.9"},{"label": "0"},{"label": "SharedMergeTree 파트의 파일별 blob 맵을 파일마다 하나의 노드에 저장하는 대신, 하나의 통합된 Keeper 노드에 저장하는 새로운 설정"}]}]} />

SharedMergeTree 테이블의 파트(Wide 및 Compact 모두)를 파일마다 하나의 Keeper 노드에 저장하는 대신,
하나의 통합된 blob-list 메타데이터 노드로 저장합니다.
파일마다 객체 스토리지 blob은 하나씩 유지하면서 파트별 Keeper 메타데이터를 O(파일 수)에서 작은 상수로
줄입니다. ClickHouse Cloud에서만 사용할 수 있습니다.
...
더 이상 사용되지 않는 설정으로, 아무런 작업도 수행하지 않습니다.
