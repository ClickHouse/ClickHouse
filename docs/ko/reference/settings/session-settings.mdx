...
호출 시 매개변수 맵에 `credentials`를 전달하지 않으면, `aiEmbed`는 이 명명된 컬렉션의 이름을 사용합니다. 비어 있으면 기본값이 없다는 뜻이며, 이러한 호출에서는 `credentials`를 명시적으로 전달해야 합니다. 임베딩 endpoint와 모델은 chat용 endpoint 및 모델과 다르므로 `ai_function_text_default_credentials`와 분리되어 있습니다.
...
호출 시 매개변수 맵에 `credentials`를 전달하지 않은 경우, 텍스트 AI 함수(`aiGenerate`, `aiClassify`, `aiExtract`, `aiTranslate`)가 사용하는 명명된 컬렉션의 이름입니다. 비어 있으면 기본값이 없다는 뜻이며, 이런 호출에서는 `credentials`를 명시적으로 전달해야 합니다. chat-completions endpoint와 모델은 임베딩용 endpoint 및 모델과 다르므로, 이 설정은 `ai_function_embedding_default_credentials`와 별개입니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "0"},{"label": "Delta Lake 쓰기가 베타 단계로 전환되었습니다."}]}, {"id": "row-2","items": [{"label": "25.9"},{"label": "0"},{"label": "새 설정입니다."}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "0"},{"label": "HTTP 인덱스 페이지를 나열해 `url` 테이블 함수에서 와일드카드를 확장할 수 있도록 하는 새로운 설정입니다."}]}]} />

HTTP 인덱스 페이지를 사용해 `url()` 및 `ENGINE = URL`에서 와일드카드를 실험적으로 확장할 수 있게 합니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "0"},{"label": "새로운 안전성 검사: 이제 `ALTER TABLE ... REPLACE PARTITION ... FROM ...`는 원본 테이블의 요청한 파티션에 파트가 없으면 예외를 발생시킵니다([#23727](https:\/\/github.com\/ClickHouse\/ClickHouse\/issues\/23727)의 조용한 데이터 손실 문제 수정). 이전 동작인 대상 파티션을 조용히 삭제하는 방식은 `allow_replace_partition_from_empty_source = 1`을 설정하면 유지됩니다."}]}]} />

원본의 해당 파티션에 파트가 없을 때 `ALTER TABLE ... REPLACE PARTITION ... FROM ...`가 대상 파티션을 조용히 삭제하도록 허용합니다.

기본적으로는 허용되지 않습니다. 요청한 파티션에 데이터가 없는 원본에서 `REPLACE PARTITION`을 수행하면 예외가 발생합니다. 이 경우 작업은 사실상 대상에 대한 조용한 `DROP PARTITION`이 되기 때문입니다(대상 데이터는 삭제되고 이를 대체할 데이터는 없음). 이는 실수로 인한 데이터 손실의 흔한 원인입니다([#23727](https://github.com/ClickHouse/ClickHouse/issues/23727) 참고).

예를 들어 비어 있는 원본 파티션을 의도적으로 사용해 대상 데이터를 비우는 경우, 이 설정을 활성화하여 이전 동작을 복원할 수 있습니다. 조건 없이 삭제하려면 대신 `ALTER TABLE ... DROP PARTITION ...`을 사용하는 것이 좋습니다.
...
`b.id`와 같은 여러 부분으로 된 식별자가 별칭이 `b`인 테이블의 컬럼 `id`를 의미할 수도 있고, 다른 컬럼의 Tuple 서브컬럼 `b.id`를 의미할 수도 있는 경우, 별칭 접두사 해석(`b`의 컬럼 `id`)을 우선합니다. 기본적으로 분석기는 서브컬럼을 우선합니다. 이전 분석기의 해석 방식과 일치시키려면 이 설정을 활성화하십시오.
...
`ANY INNER|LEFT JOIN` 연산에서 ClickHouse 서버의 기존 방식 방식을 활성화합니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "24.7"},{"label": "1000"},{"label": "Aws::Client::RetryStrategy에 대한 설정입니다. Aws::Client는 자체적으로 재시도하며, 0은 재시도하지 않음을 의미합니다. 이 설정은 백업\/복원에만 적용됩니다."}]}]} />

Aws::Client::RetryStrategy에 대한 설정입니다. Aws::Client는 자체적으로 재시도하며, 0은 재시도하지 않음을 의미합니다. 이 설정은 백업/복원에만 적용됩니다.
...
<Note>
  ClickHouse Cloud에서는 서비스 수준의 기본 compatibility 설정을 ClickHouse Cloud Support에서 설정해야 합니다. 설정이 필요하면 [지원 케이스를 제출하세요](https://clickhouse.cloud/support).
  하지만 compatibility 설정은 세션에서 `SET compatibility = '22.3'`를 사용하거나 쿼리에서 `SETTINGS compatibility = '22.3'`를 사용하는 등 표준 ClickHouse 설정 메커니즘을 통해 사용자, 역할(Role), 프로필(profile), 쿼리 또는 세션(session) 수준에서 재정의할 수 있습니다.
</Note>
...
활성화되면(기본값) 파티션 키 컬럼이 정렬 키의 일부가 아닌 테이블의 `FINAL` 쿼리에서는 파티션 프루닝이 적용되지 않습니다. 이는 26.3에서 도입된, 정확성을 보장하는 동작입니다. `FINAL`은 동일한 프라이머리 키(primary key)를 공유하지만 서로 다른 파티션에 있는 행을 중복 제거해야 할 수 있으며, 파티션 프루닝이 적용되면 이런 행이 중복 제거 입력에서 조용히 제외될 수 있습니다.

비활성화되면 `FINAL`을 사용하더라도 파티션 프루닝이 적용되어 26.3 이전 동작으로 되돌아갑니다. 이는 파티션 컬럼에 대한 `WHERE` 프레디케이트가 있는 쿼리에서 훨씬 더 빠를 수 있지만, 동일한 프라이머리 키를 가진 행이 서로 다른 파티션에 존재할 수 없을 때만 올바릅니다. 예를 들어, 삽입 시점에 파티션 컬럼이 설정되고 이후에는 절대 변경되지 않는 이벤트 로그 테이블이 이에 해당합니다.
...
<Note>
  이 설정은 손상된 배치에도 영향을 미칩니다(비정상적인 server(머신) 종료가 발생하고 [분산](/ko/reference/engines/table-engines/special/distributed) 테이블 엔진에 `fsync_after_insert`/`fsync_directories`가 없는 경우 이러한 배치가 생길 수 있습니다).
</Note>
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "0"},{"label": "`system.distributed_cache_registry`의 `certificate` 및 `signature` 컬럼을 표시하는 새로운 설정입니다."}]}]} />

ClickHouse Cloud에서만 적용됩니다. `system.distributed_cache_registry` 테이블에 `certificate` 및 `signature` 컬럼을 표시합니다. 기본적으로는 출력이 간결하게 유지되도록 이 컬럼이 비어 있으며, 확인하려면 이 설정을 활성화하십시오.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "0"},{"label": "분산 쿼리에 대해 임대할 stateless worker 수를 지정하는 새로운 실험적 설정입니다. 0이면 임대를 비활성화합니다."}]}]} />

이 쿼리를 실행하는 데 사용할 stateless worker 수입니다. 0이면 분산 계획에 대한 stateless worker 임대를 비활성화합니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "pretty"},{"label": "26.7부터 `EXPLAIN PLAN`의 기본값은 `actions=1, compact=1, pretty=1`입니다. 26.7 이전 출력으로 복원하려면 이 값을 `legacy`로 설정하십시오."}]}]} />

`EXPLAIN PLAN`에 사용되는 기본 포맷입니다.

가능한 값:

* `pretty` (26.7부터 기본값) — `actions`, `compact`, `pretty`의 기본값이 `true`로 설정되어, 간결하고 보기 좋은 형식의 action 정보가 포함된 계획을 생성합니다.
* `legacy` — 26.7 이전 출력입니다.

`EXPLAIN` 문에서 `actions`, `compact`, `pretty` 옵션을 명시적으로 지정하면(예: `EXPLAIN actions = 0, compact = 0, pretty = 0 SELECT ...`) 항상 이 설정을 재정의합니다.

`json = 1` 또는 `distributed = 1`을 사용하는 `EXPLAIN PLAN`은 이 설정과 관계없이, `actions`, `compact`, `pretty`를 명시적으로 설정하지 않는 한 `legacy`(26.7 이전) 기본값을 유지합니다. pretty 출력은 JSON 결과나 세그먼트별 분산 계획을 표현할 수 없으므로, 이러한 모드는 `legacy` 형식으로만 올바르게 렌더링됩니다.
...
함수 `formatDateTime`의 Formatter &#39;%e&#39;는 한 자리 수 날짜를 앞에 공백 한 칸을 넣어 출력합니다. 예를 들어 &#39;2&#39;가 아니라 &#39; 2&#39;로 출력됩니다.
...
<Note>
  `insert_deduplication_token`은 파티션별로 추적되므로, 하나의 삽입으로 기록되는 여러 파티션이 동일한 토큰을 가질 수 있습니다. 토큰이 없으면 기본 콘텐츠 checksum(`insert_deduplication_version = new_unified_hash`)이 삽입된 전체 블록에 대해 계산되므로, 전체 데이터가 이전 삽입(재시도)과 완전히 일치하는 경우에만 해당 삽입이 중복 제거되며, 단일 파티션의 행이 우연히 다른 삽입과 일치하는 경우에는 중복 제거되지 않습니다.
</Note>
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "21.1"},{"label": "1"},{"label": "기본적으로 병렬 쿼럼 삽입을 사용합니다. 이는 순차 쿼럼 삽입보다 훨씬 편리합니다"}]}]} />

<Note>
  이 설정은 SharedMergeTree에는 적용되지 않습니다. 자세한 내용은 [SharedMergeTree 일관성](/ko/products/cloud/features/infrastructure/shared-merge-tree#consistency)을 참조하십시오.
</Note>

쿼럼 `INSERT` 쿼리의 병렬 처리를 활성화하거나 비활성화합니다. 활성화하면 이전 쿼리가 아직 완료되지 않았더라도 추가 `INSERT` 쿼리를 보낼 수 있습니다. 비활성화하면 동일한 테이블에 대한 추가 쓰기가 거부됩니다.
...
<Note>
  현재는 구문 분석 중에는 이를 검사하지 않고, 쿼리 구문 분석이 끝난 후에만 검사합니다.
...
테이블에 쓰기 전에 데이터를 압축할 때, 비압축 데이터 블록의 최대 크기를 지정합니다. 기본값은 1,048,576(1 MiB)입니다. 더 작은 블록 크기를 지정하면 일반적으로 압축률은 약간 낮아지지만, cache 지역성 덕분에 압축 및 압축 해제 속도는 약간 빨라지고 메모리 사용량은 줄어듭니다.
...
이 매개변수는 현재 쿌리 실행 속도를 기준으로 한 보간값에 따라 동작합니다.
...
SQL 파서가 파싱할 수 있는 쿌리 문자열의 최대 바이트 수입니다.
INSERT 쿼리의 VALUES 절에 있는 데이터는 별도의 stream 파서(O(1) RAM 사용)로 처리되므로 이 제한의 적용을 받지 않습니다.
...
<Note>
  이 쿼리 설정은 해당 서버 설정을 덮어씁니다. [max&#95;table&#95;size&#95;to&#95;drop](/ko/reference/settings/server-settings/settings#max_table_size_to_drop)를 참조하십시오.
</Note>
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "1"},{"label": "`ORDER BY`가 `GROUP BY` 정렬 기준의 접두사일 때, 충분한 그룹이 생성되면 조기에 종료할 수 있도록 `LIMIT`을 aggregation-in-order에 적용하는 새로운 설정입니다."}]}]} />

활성화되어 있고 순서 기반 집계가 사용 중이면, 충분한 그룹이 생성된 뒤 조기에 종료할 수 있도록 `LIMIT`을 집계 단계에 적용합니다. 이렇게 하면 `ORDER BY`가 `GROUP BY` 키 접두사와 일치할 때 읽는 데이터 양을 줄일 수 있습니다. `rows_before_limit_at_least`에 보고되는 값이 줄어들 수 있으므로, 정확한 개수가 필요하면 `exact_rows_before_limit`를 사용하십시오.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "5000000"},{"label": "비교 `AND` 체인이 매우 많거나 매우 큰 쿼리를 분석할 때 `optimize_and_compare_chain` 최적화가 과도한 비중을 차지하지 않도록, 이 최적화의 작업량(해시된 쿼리 트리 노드 수 기준)에 상한을 두는 새로운 설정입니다. 이전 값인 `0`(무제한)은 이 최적화에 상한이 없던 26.7 이전 동작을 재현하므로, `compatibility`를 더 이른 버전으로 설정하면 예산 제한 없이 전이 프레디케이트를 계속 도출합니다. 예산을 비활성화하려면 `0`으로 설정하십시오."}]}]} />

쿼리 분석 중 `optimize_and_compare_chain` 최적화에 사용할 작업 예산으로, `getTreeHash`가 해시한 쿼리 트리 노드 수(이 최적화에서 가장 큰 비용)로 측정합니다. 최적화를 적용하는 동안 쿼리에서 해시한 노드 수가 이 값을 초과하면, 해당 쿼리의 나머지 부분에는 이 최적화를 더 이상 적용하지 않습니다. 이렇게 하면 비교 `AND` 체인이 매우 많거나 매우 큰 쿼리에서, 아무것도 접지(folding)하지 않으면서도 이 최적화가 분석 시간의 대부분을 차지할 수 있는 경우 분석 시간을 제한할 수 있습니다. 일찍 중단해도 항상 안전합니다. 단지 최적화를 생략할 뿐이며 결과는 절대 바뀌지 않습니다. 예산을 비활성화하려면 `0`으로 설정하십시오(무제한).
...
설정 [`query_plan_enable_optimizations`](#query_plan_enable_optimizations)이 1일 때만 적용됩니다.
...
쿼리 계획(쿼리 계획) 수준의 쿼리 최적화를 켜거나 끕니다.
...
실행 계획에서 필터를 더 아래 단계로 내리는 쿼리 계획 수준 최적화 기능을 켜거나 끕니다.
...
쿼리 계획에서 조인의 어느 쪽을 빌드 테이블(build table, inner라고도 하며 해시 조인 시 해시 테이블에 삽입되는 테이블)로 사용할지 결정합니다. 이 설정은 `JOIN ON` 절을 사용하는 `ALL` 조인 엄격성에서만 지원됩니다. 가능한 값은 다음과 같습니다:

* &#39;auto&#39;: 플래너가 어느 테이블을 빌드 테이블로 사용할지 결정합니다.
...
실행 계획에서 ARRAY JOIN을 상위로 끌어올리는 쿼리 계획 수준 최적화를 켜거나 끕니다.
...
쿼리 계획(쿼리 계획)에 적용되는 최적화의 총 개수를 제한합니다. [query&#95;plan&#95;enable&#95;optimizations](#query_plan_enable_optimizations) 설정을 참조하십시오.
...
<Note>
  이 설정은 전문가 수준의 설정이므로 개발자가 디버깅 목적으로만 사용해야 합니다. 이 설정은 향후 하위 호환되지 않는 방식으로 변경되거나 제거될 수 있습니다.
</Note>
...
연속된 필터를 머지하는 쿼리 계획 수준 최적화를 켜거나 끕니다.
설정 [query&#95;plan&#95;enable&#95;optimizations](#query_plan_enable_optimizations)의 값이 1일 때만 적용됩니다.
...
* &#39;dpsub&#39; - inner 조인과 non-inner 조인을 모두 지원하는 DPsub 알고리즘을 구현하며, 가능한 모든 조인 순서를 고려해 가장 최적인 순서를 찾지만 테이블과 조인 프레디케이트가 많은 쿼리에서는 느릴 수 있습니다.
...
중복된 정렬 단계를 제거하는 쿼리 계획 수준 최적화를 활성화하거나 비활성화합니다. 예를 들어 서브쿼리에서 사용됩니다.
...
필터를 표현식으로 분할하는 쿼리 계획 수준 최적화를 켜거나 끕니다.
설정 [query&#95;plan&#95;enable&#95;optimizations](#query_plan_enable_optimizations)의 값이 1일 때만 적용됩니다.
...
벡터 유사성 인덱스를 사용하려고 시도하는 쿼리 계획 수준 최적화를 켜거나 끕니다.
이 설정은 [`query_plan_enable_optimizations`](#query_plan_enable_optimizations)이 1일 때만 적용됩니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "0"},{"label": "쿼리를 시작하기 전에 특정 워크로드를 위해 메모리를 예약하는 새로운 설정입니다."}]}]} />

워크로드 스케줄링에 사용됩니다. 단일 server에서 쿼리를 실행하는 데 사용할 수 있도록 예약되는 최소 RAM 용량입니다. 예약은 `workload` 쿼리 설정 값을 사용해 WORKLOAD 계층 구조를 통해 수행됩니다.
워크로드에 사용할 수 있는 메모리가 충분하지 않으면 쿼리는 시작되지 않으며, 예약이 충족될 때까지 pending 상태로 대기합니다.
값이 `0`이면 예약하지 않음을 의미합니다.
이 설정은 MEMORY RESERVATION resource가 생성된 경우에만 적용됩니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "0.2"},{"label": "`text_index_density_threshold`에서 이름이 변경되었으며(별칭으로 유지), 지연 포스팅 리스트 적용 모드에서 포스팅 리스트 교집합 알고리즘을 선택합니다."}]}]} />

지연 포스팅 리스트 적용 모드(`text_index_posting_list_apply_mode = 'lazy'`)에서 사용할 교집합 알고리즘을 선택하는 포스팅 리스트 밀도 임계값입니다.
임계값 미만: leapfrog 교집합(희소 포스팅 리스트에 유리). 임계값 이상: 무차별 대입 비트맵 교집합(밀집 포스팅 리스트에 유리).
...
&#39;lazy&#39;는 커서 기반 온디맨드 디코딩을 사용합니다(직렬화된 코덱이 있는 인덱스 포맷 필요).
...
인덱스 페이지에서 URL 와일드카드를 확장할 때 순회할 수 있는 디렉터리의 최대 개수입니다.
...
<Note>
  ...
  - `use_compact_format_in_distributed_parts_names=1`을 사용하면 클러스터 정의에서 노드 순서를 변경할 경우 `shard_index`/`replica_index`가 변경되므로 주의하십시오.
</Note>
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "0"},{"label": "기존 `toTime` 대신 새로운 `toTime` 함수(`Time` 데이터 타입으로 값 변환)를 기본으로 사용합니다. 기존 `toTime`은 계속 `toTimeWithFixedDate`로 사용할 수 있습니다."}]}, {"id": "row-2","items": [{"label": "25.6"},{"label": "1"},{"label": "새 설정입니다. 사용자가 기존 `toTime` 함수 로직을 사용할 수 있게 하며, 이 로직은 `toTimeWithFixedDate`처럼 동작합니다."}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.2"},{"label": "1"},{"label": "텍스트 인덱스가 이제 일반 제공됩니다"}]}, {"id": "row-2","items": [{"label": "26.1"},{"label": "0"},{"label": "기본적으로 활성화"}]}, {"id": "row-3","items": [{"label": "25.9"},{"label": "0"},{"label": "새로운 설정"}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "1"},{"label": "텍스트 인덱스 header cache를 전역적으로 활성화했습니다."}]}, {"id": "row-2","items": [{"label": "25.11"},{"label": "0"},{"label": "새로운 설정"}]}]} />

역직렬화된 텍스트 인덱스 header를 메모리에 캐시할지 여부입니다.
텍스트 인덱스 header cache를 사용하면 많은 수의 텍스트 인덱스 쿼리를 처리할 때 지연 시간을 크게 줄이고 처리량을 높일 수 있습니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "1"},{"label": "텍스트 인덱스 tokens cache를 전역으로 활성화했습니다."}]}, {"id": "row-2","items": [{"label": "26.3"},{"label": "0"},{"label": "새로운 설정"}]}]} />

역직렬화된 텍스트 인덱스 token info를 메모리에 캐시할지 여부입니다.
텍스트 인덱스 tokens cache를 사용하면 다수의 텍스트 인덱스 쿼리를 처리할 때 지연 시간을 크게 줄이고 처리량을 높일 수 있습니다.
