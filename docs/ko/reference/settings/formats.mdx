...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "1"},{"label": "GeoJSON 포맷이 읽기 및 쓰기 시 RFC 7946 geometry 유효성(선 및 ring의 최소 point 수, ring 폐합, 비어 있지 않은 multi-geometry)을 적용할지 제어하는 새로운 설정"}]}]} />

`GeoJSON` 포맷이 양방향으로 RFC 7946 geometry 유효성을 적용할지 제어합니다.

활성화되면(기본값) GeoJSON shape 규칙을 위반하는 geometry는 거부됩니다. 예를 들어, point가 2개 미만인 `LineString`(또는 `MultiLineString`의 선), point가 4개 미만이거나 첫 번째 point와 마지막 point가 다른(닫히지 않은 ring) `Polygon` 또는 `MultiPolygon`의 ring, 또는 비어 있는 `MultiLineString`, `Polygon`, `MultiPolygon`이 이에 해당합니다. 이는 읽기 시(이러한 document가 거부됨)와 쓰기 시(입력 형식이 거부할 document를 생성하는 대신 이러한 ClickHouse 값이 거부됨) 모두에 적용됩니다.

비활성화되면 이러한 shape 규칙은 적용되지 않습니다. 이러한 geometry는 있는 그대로 읽고 그대로 기록하므로 퇴화된 geometry도 round-trip되지만, 기록된 document는 유효한 GeoJSON이 아닐 수 있습니다.

이 유효성 검사는 구조만 확인합니다. 즉, point 수와 ring 폐합만 검사합니다. shape의 기하학적 정확성은 검사하지 않습니다. ring 방향(오른손 법칙 / winding order)은 적용되지 않으며, 구조적으로는 유효하지만 기하학적으로는 퇴화된 geometry(예: 면적이 0인 polygon, 자기 교차하는 ring, 또는 바깥 ring 바깥에 hole이 있는 polygon)도 허용됩니다. 유한하지 않은 좌표(`NaN`, `Inf`)는 JSON 숫자로 표현할 수 없으므로 이 설정과 관계없이 항상 거부됩니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "0"},{"label": "`Nullable(String)`의 누락된 값을 CSV에서 NULL 대신 빈 문자열로 읽기 위한 새로운 설정입니다."}]}]} />

CSV에서 누락된 값을 `Nullable(String)`으로 읽는 방식을 제어합니다. 누락된 값은 따옴표로 둘러싸이지 않은, 쉼표 사이 또는 쉼표 앞뒤의 빈 공간을 의미합니다. 이 설정을 활성화하면 `input_format_csv_empty_as_default`의 값과 관계없이 `Nullable(String)`의 누락된 값은 NULL이 아니라 빈 `String`으로 해석됩니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "24.3"},{"label": "1"},{"label": "ClickHouse는 일반적으로 UTF-8인 String 데이터 타입에 임의의 바이너리 데이터를 허용합니다. Parquet/ORC/Arrow String은 UTF-8만 지원합니다. 따라서 ClickHouse String 데이터 타입에 대해 Arrow의 어떤 데이터 타입(String 또는 Binary)을 사용할지 선택할 수 있습니다. Binary가 더 정확하고 호환성도 높지만, 대부분의 경우 기본값으로 String을 사용하면 사용자 기대에 더 잘 부합합니다."}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "1"},{"label": "새 설정입니다. output_format_csv_serialize_tuple_into_separate_columns가 활성화되면 이제 CSVWithNames\/CSVWithNamesAndTypes 헤더에서 Tuple 컬럼이 리프 필드로 평탄화되어 헤더 너비가 데이터와 일치합니다. 이전의 단일 이름 헤더로 되돌리려면 false로 설정하십시오."}]}]} />

[output&#95;format&#95;csv&#95;serialize&#95;tuple&#95;into&#95;separate&#95;columns](#output_format_csv_serialize_tuple_into_separate_columns)이 활성화되면 `CSVWithNames` 및 `CSVWithNamesAndTypes`의 헤더 행은 각 Tuple 컬럼을 리프 필드(예: `t.a`, `t.b`와 같은 점 표기 이름과 리프 타입 이름)로 평탄화하므로, 헤더의 컬럼 수가 데이터와 같아집니다. `CustomSeparated*`에서는 `format_custom_escaping_rule = 'CSV'`이고 `format_custom_field_delimiter`가 `format_csv_delimiter`와 같은 단일 문자일 때만 이 평탄화가 적용됩니다. 그렇지 않은 경우(예: 기본 탭 구분자 또는 `format_custom_field_delimiter = '|'`)에는 헤더가 평탄화되지 않은 상태로 유지되어 계속 데이터와 일치합니다. 헤더가 단일 최상위 Tuple 이름과 타입을 유지하는 이전 동작을 유지하려면 `0`으로 설정하십시오.

참고: 평탄화된 헤더는 `input_format_with_names_use_header = 1`일 때 이름 기준으로 다시 Tuple로 읽어들이지 않습니다. 이런 데이터를 다시 Tuple로 읽으려면 출력할 때 이 설정을 `0`으로 설정하거나, `input_format_with_names_use_header = 0`으로 읽으십시오(`*WithNamesAndTypes` 포맷인 `CSVWithNamesAndTypes` 및 `CustomSeparatedWithNamesAndTypes`의 경우에는 `input_format_with_types_use_header = 0`도 설정해야 합니다. 그렇지 않으면 평탄화된 타입 행이 단일 최상위 Tuple 입력 필드를 기준으로 검증되어 거부됩니다).
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "24.3"},{"label": "zstd"},{"label": "Parquet\/ORC\/Arrow는 lz4와 zstd를 포함해 다양한 압축 방식을 지원합니다. ClickHouse는 이러한 모든 압축 방식을 지원합니다. 'duckdb'와 같은 일부 도구는 더 빠른 `lz4` 압축 방식을 지원하지 않으므로 기본값으로 zstd를 설정했습니다."}]}, {"id": "row-2","items": [{"label": "23.3"},{"label": "lz4_frame"},{"label": "기본적으로 ORC 출력 형식에서 lz4 압축을 사용합니다"}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "24.3"},{"label": "1"},{"label": "ClickHouse는 일반적으로 UTF-8인 String 데이터 타입에 임의의 바이너리 데이터를 저장할 수 있습니다. Parquet\/ORC\/Arrow의 String은 UTF-8만 지원합니다. 따라서 ClickHouse String 데이터 타입에 사용할 Arrow 데이터 타입으로 String 또는 Binary를 선택할 수 있습니다. Binary가 더 정확하고 호환성도 높지만, 대부분의 경우 기본값으로 String을 사용하는 것이 사용자 기대에 더 잘 부합합니다."}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "24.2"},{"label": "0"},{"label": "값이 true이면 '를 ''로 이스케이프하고, 그렇지 않으면 \\'로 감쌉니다"}]}]} />

값이 true이면 &#39;를 &#39;&#39;로 이스케이프하고, 그렇지 않으면 &#92;&#39;로 감쌉니다
...
:::note
`schema_inference_hints`의 형식이 올바르지 않거나 오타나 잘못된 데이터 타입 등이 있는 경우... schema&#95;inference&#95;hints 전체가 무시됩니다.
:::
