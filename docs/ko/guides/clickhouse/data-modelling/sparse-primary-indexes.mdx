...
slug: /ko/guides/clickhouse/data-modelling/sparse-primary-indexes
...
<div id="an-index-design-for-massive-data-scales">
  ### 대규모 데이터 처리를 위한 인덱스 설계 {#an-index-design-for-massive-data-scales}
</div>
...
### 데이터는 프라이머리 키 컬럼 순으로 정렬되어 디스크에 저장됩니다 {#data-is-stored-on-disk-ordered-by-primary-key-columns}
...
### 데이터는 병렬 데이터 처리를 위해 그래뉼로 구성됩니다 {#data-is-organized-into-granules-for-parallel-data-processing}
...
### 프라이머리 인덱스는 그래뉼당 하나의 항목을 가집니다 {#the-primary-index-has-one-entry-per-granule}
...
### 그래뉼 위치를 찾는 데 마크 파일을 사용합니다 {#mark-files-are-used-for-locating-granules}
...
<div id="using-multiple-primary-indexes">
  ## 여러 프라이머리 인덱스 사용 {#using-multiple-primary-indexes}
</div>

<a name="filtering-on-key-columns-after-the-first" />

### 보조 키 컬럼은 비효율적일 수도, 아닐 수도 있습니다 {#secondary-key-columns-can-not-be-inefficient}
...
### 일반 제외 검색 알고리즘 {#generic-exclusion-search-algorithm}
...
<div id="a-need-to-use-multiple-primary-indexes">
  ### 여러 프라이머리 인덱스를 사용해야 하는 이유 {#a-need-to-use-multiple-primary-indexes}
</div>
...
### 추가 프라이머리 인덱스를 생성하는 옵션 {#options-for-creating-additional-primary-indexes}
...
### 옵션 2: Materialized Views {#option-2-materialized-views}
...
### 옵션 3: PROJECTION {#option-3-projections}
...
<div id="ordering-key-columns-efficiently">
  ## 정렬 키 컬럼을 효율적으로 배치하기 {#ordering-key-columns-efficiently}
</div>
