...
버전 25.6부터 ClickHouse는 Delta Lake Rust 커널을 통해 S3 및 GCS의 Delta Lake를 읽습니다. 이 설정의 이름은 버전 26.8 이상에서는 [`allow_delta_kernel_rs`](/ko/reference/settings/session-settings/allow#allow_delta_kernel_rs)이며, 버전 25.5부터 26.7까지는 `allow_experimental_delta_kernel_rs`입니다. Azure Blob Storage에서는 커널이 비활성화되어 있으므로 legacy reader와 함께 [deltaLakeAzure()](/ko/sql-reference/table-functions/deltalake)를 사용하십시오. 커널을 사용하지 않으면 파티션 프루닝, 변경 데이터 피드, 스냅샷 버전 읽기는 지원되지 않습니다.
