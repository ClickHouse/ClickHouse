...
**OTLP 사용** - 이 경우 로그는 OTLP 프로토콜을 통해 OpenTelemetry SDKs에서 collector로 직접 전송(push)됩니다. [OpenTelemetry demo](https://opentelemetry.io/docs/demo/)는 이 방식을 사용하며, 각 언어의 OTLP exporter는 로컬 collector endpoint를 가정합니다. 이 경우 collector는 OTLP 수신기가 구성되어 있어야 합니다. 구성 예시는 위의 [demo 설정](https://github.com/ClickHouse/opentelemetry-demo/blob/main/src/otelcollector/otelcol-config.yml#L5-L12)을 참조하십시오. 이 방식의 장점은 로그 데이터에 트레이스 ID가 자동으로 포함되므로, 나중에 특정 로그에 해당하는 trace를 식별하거나 그 반대로도 가능하다는 점입니다.
...
[비동기 삽입이 활성화된 상태](/ko/concepts/features/operations/insert/asyncinserts#enabling-asynchronous-inserts)에서는 ClickHouse가 ① 삽입 쿼리를 수신하면 해당 쿼리의 데이터가 먼저 ② 즉시 메모리 내 버퍼에 기록됩니다. 이후 ③ 다음 버퍼 플러시가 발생하면 버퍼의 데이터가 [정렬된 뒤](/ko/guides/clickhouse/data-modelling/sparse-primary-indexes#data-is-stored-on-disk-ordered-by-primary-key-columns) 파트(part)로 데이터베이스 저장소에 기록됩니다. 데이터는 데이터베이스 저장소로 플러시되기 전까지는 쿼리로 검색할 수 없으며, 버퍼 플러시는 [구성할 수 있습니다](/ko/concepts/features/operations/insert/asyncinserts).
...
비동기 삽입으로 전송된 데이터는 ClickHouse 버퍼가 플러시되면 삽입됩니다. 이는 [`async_insert_max_data_size`](/ko/reference/settings/session-settings#async_insert_max_data_size)를 초과했거나 첫 번째 INSERT 쿼리 이후 [`async_insert_busy_timeout_ms`](/ko/reference/settings/session-settings#async_insert_max_data_size)밀리초가 경과한 경우에 발생합니다. `async_insert_stale_timeout_ms`가 0이 아닌 값으로 설정되어 있으면 마지막 쿼리 이후 `async_insert_stale_timeout_ms milliseconds`가 지나면 데이터가 삽입됩니다. 이러한 설정을 조정해 파이프라인의 종단 간 지연 시간을 제어할 수 있습니다. 버퍼 플러시를 조정하는 데 사용할 수 있는 추가 설정은 [여기](/ko/reference/settings/session-settings#async_insert)에 문서화되어 있습니다. 일반적으로는 기본값이 적절합니다.

<Info>
  **적응형 비동기 삽입을 고려하십시오**

  사용 중인 에이전트 수가 적고 처리량은 낮지만 종단 간 지연 시간 요구 사항이 엄격한 경우에는 [적응형 비동기 삽입](https://clickhouse.com/blog/clickhouse-release-24-02#adaptive-asynchronous-inserts)이 유용할 수 있습니다. 일반적으로 이는 ClickHouse에서 볼 수 있는 고처리량 관측성 사용 사례에는 적용되지 않습니다.
</Info>
...
exporters:
  otlp:
    ...
    tls:
      insecure: true # 보안 연결을 사용하는 경우 false로 설정하세요
service:
  telemetry:
    metrics:
      address: 0.0.0.0:9888 # 동일한 호스트에서 2개의 collector가 실행되므로 수정됨
  pipelines:
    logs:
      receivers: [filelog]
      processors: [batch]
