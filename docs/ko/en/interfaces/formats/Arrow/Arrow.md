...
| `input_format_arrow_use_native_reader`                                       | `Arrow` 및 `ArrowStream` 포맷에 Apache Arrow 라이브러리 대신 ClickHouse 네이티브 reader를 사용합니다. Apache Arrow 라이브러리 reader를 사용하려면 `0`으로 설정합니다.                                      | `1`         |
...
| `output_format_arrow_unsupported_types_as_binary`                            | Arrow에 대응되는 타입이 없는 경우(예: `BFloat16`, `AggregateFunction`) 원시 바이너리 데이터로 출력합니다. `false`이면 이러한 타입에서 예외가 발생합니다. 이 설정은 네이티브 writer와 Apache Arrow 라이브러리 writer 모두에 적용됩니다. | `1`         |
| `output_format_arrow_use_64_bit_indexes_for_dictionary`                      | Arrow 포맷에서 딕셔너리 인덱스에 항상 64비트 정수를 사용합니다                                                                                                                              | `0`         |
| `output_format_arrow_use_native_writer`                                      | `Arrow` 및 `ArrowStream` 포맷에 Apache Arrow 라이브러리 대신 ClickHouse 네이티브 writer를 사용합니다. Apache Arrow 라이브러리 writer를 사용하려면 `0`으로 설정합니다.                                      | `1`         |
