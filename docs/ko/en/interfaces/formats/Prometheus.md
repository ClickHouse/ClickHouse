...
[Prometheus 텍스트 기반 익스포지션 포맷](https://prometheus.io/docs/instrumenting/exposition_formats/#text-based-format)으로 메트릭을 노출합니다.

이 포맷에서는 출력 테이블이 다음 규칙에 따라 올바른 구조를 가져야 합니다.

* `name` ([String](/ko/sql-reference/data-types/string.md)) 및 `value` (number) 컬럼은 필수입니다.
* 행에는 선택적으로 `help` ([String](/ko/sql-reference/data-types/string.md)) 및 `timestamp` (number)가 포함될 수 있습니다.
* `type` ([String](/ko/sql-reference/data-types/string.md)) 컬럼은 `counter`, `gauge`, `histogram`, `summary`, `untyped` 중 하나이거나 비어 있어야 합니다.
* 각 메트릭 값에는 `labels` ([Map(String, String)](/ko/sql-reference/data-types/map.md))가 추가로 포함될 수 있습니다.
* 연속된 여러 행이 서로 다른 레이블로 하나의 메트릭을 나타낼 수 있습니다. 테이블은 메트릭 이름을 기준으로 정렬되어야 합니다(예: `ORDER BY name`).
