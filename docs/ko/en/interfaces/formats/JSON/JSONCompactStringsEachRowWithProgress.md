...
[`JSONCompactEachRowWithProgress`](/ko/interfaces/formats/JSONCompactEachRowWithProgress)와 유사하지만, 모든 값이 문자열로 변환됩니다.
모든 데이터 타입을 일관된 문자열 형태로 표현해야 할 때 유용합니다.

주요 기능:

* `JSONCompactEachRowWithProgress`와 동일한 구조
* 모든 값이 문자열로 표현됩니다(숫자, 배열 등도 모두 따옴표로 감싼 문자열로 표시됨)
* 진행 상황 업데이트, 합계, 예외 처리를 포함합니다
