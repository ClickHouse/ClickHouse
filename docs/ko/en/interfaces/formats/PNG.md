---
alias: []
description: 'PNG 이미지 출력 형식에 대한 문서'
input_format: false
keywords: ['PNG']
output_format: true
...
출력 이미지의 크기는
[`output_format_image_width`](/ko/operations/settings/formats#output_format_image_width) 및
[`output_format_image_height`](/ko/operations/settings/formats#output_format_image_height)
설정으로 고정됩니다(둘 다 기본값은 1024입니다). 결과가 차지하지 않는 픽셀은 검은색으로 채워지며
(`RGB` 및 회색조 모드), `RGBA` 모드에서는 투명한 검은색으로 채워집니다.
...
| `r`, `g`, `b`, `a` | 8비트 RGBA                                   |
| 정수 타입의 `v`         | 8비트 회색조                              |
| `Float*` 타입의 `v`   | 8비트 회색조 (`[0, 1]` 값을 `[0, 255]`로 변환) |
| `Bool` 타입의 `v`     | Binary (8비트 회색조: `0` 또는 `255`로 렌더링)  |

컬럼 이름은 대소문자를 구분하지 않고 매칭됩니다. 색상 모드를 명확하게
판별할 수 없는 경우(예: 알 수 없는 컬럼 이름, `v`와 `r`/`g`/`b`/`a`가 섞여 있는 경우, 또는 `r`/`g`/`b` 중 하나가 없는 경우),
쿼리는 예외를 발생시킵니다.

픽셀 채널의 경우, 정수 값은 `[0, 255]` 범위로 제한되며 부동소수점 값은
`[0, 1]` 범위로 제한된 후 `[0, 255]`로 스케일됩니다.

이미지에서 각 레코드의 위치는 다음 두 가지 모드 중 하나로 결정됩니다:

* **암시적** (기본값 — `x`와 `y`가 모두 없을 때). 각 레코드는
  단일 픽셀 하나에 대응하며, 픽셀은 왼쪽에서 오른쪽으로, 위에서 아래로 스캔라인 순서대로 채워집니다.
* **명시적** (`x`와 `y` 컬럼이 모두 존재하고 둘 다 정수 타입일 때).
  `x`와 `y` 컬럼은 픽셀 좌표를 나타냅니다. 이미지 범위를 벗어나는 좌표를 가진 레코드는
  별도 알림 없이 무시됩니다. 같은 좌표를 가진 레코드가 여러 개 있는 경우,
  마지막 레코드가 우선합니다(페인터 알고리즘).
...
[`output_format_image_terminal_mode`](/ko/operations/settings/formats#output_format_image_terminal_mode)를 사용하면 대신 인라인 이미지 프로토콜을 통해 이미지를 터미널에 직접 렌더링합니다:

| Value              | Behaviour                                                                                          |
| ------------------ | -------------------------------------------------------------------------------------------------- |
| &#96;&#96; (empty) | 원시 이미지 바이트를 출력합니다(기본값).                                                                            |
| `iterm`            | iTerm2 인라인 이미지 프로토콜을 사용합니다.                                                                        |
| `kitty`            | Kitty 그래픽 프로토콜을 사용합니다.                                                                             |
| `sixel`            | Sixel 프로토콜을 사용합니다. 이미지는 고정된 6×6×6 팔레트로 축소되며, alpha 채널이 있으면 검은색 배경에 합성됩니다.                          |
| `auto`             | 출력 대상이 터미널이면 지원 capability를 감지해 `iterm`, `kitty`, `sixel`을 이 순서대로 사용하고, 그렇지 않으면 원시 이미지 바이트를 출력합니다. |
...
| `output_format_image_terminal_mode` | 인라인 터미널 이미지 프로토콜(위 참조)입니다. | &#96;&#96; (비어 있음) |