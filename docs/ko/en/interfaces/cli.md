...
* 지원되는 coding agents에 공식 ClickHouse agent skills 설치
* 로컬 ClickHouse 개발 환경을 클라우드로 푸시

`clickhousectl`은 사용자와 AI agents가 ClickHouse로 개발할 수 있도록 지원합니다.
...
설치 스크립트는 사용 중인 OS에 맞는 버전을 다운로드하여 `~/.local/bin/clickhousectl`에 설치합니다. 편의를 위해 `chctl` 별칭도 자동으로 생성됩니다.
...
* Cloud 명령을 사용하려면 [ClickHouse Cloud API Key](/ko/cloud/manage/api/api-overview)가 필요합니다
...
`clickhousectl`은 `builds.clickhouse.com`에서 ClickHouse 바이너리를 다운로드하며, 해당 위치에서 빌드를 사용할 수 없으면 `packages.clickhouse.com`(Linux) 또는 [GitHub releases](https://github.com/ClickHouse/ClickHouse/releases)(macOS)에서 대신 가져옵니다.
...
`local use`는 선택한 버전의 실행 파일을 가리키는 `~/.local/bin/clickhouse` 심볼릭 링크도 생성하므로, 일반 `clickhouse` 명령(예: `clickhouse local`, `clickhouse client`)을 `PATH`에서 사용할 수 있습니다. 건너뛰려면 `--no-global`을 지정하십시오. 해당 경로에 일반 파일이 이미 있으면 경고를 표시하고 그대로 둡니다. 현재 활성화된 기본 버전에 대해 `local remove`를 실행하면 심볼릭 링크도 제거됩니다.

<div id="binary-storage">
  #### ClickHouse 바이너리 저장 위치
</div>

ClickHouse 바이너리는 전역 리포지토리에 저장되므로 저장 공간을 중복해서 사용하지 않고 여러 프로젝트에서 공유할 수 있습니다. 바이너리는 `~/.clickhouse/`에 저장됩니다:
...
`init`는 현재 작업 디렉터리에 ClickHouse 및 Postgres 프로젝트 파일을 위한 표준 폴더 구조를 생성합니다. 이 단계는 선택 사항이므로, 필요에 따라 자체 폴더 구조를 사용해도 됩니다.
...
ClickHouse 서버 인스턴스를 시작하고 관리합니다. 각 서버에는 `.clickhouse/servers/<name>/data/` 아래에 전용으로 격리된 데이터 디렉터리가 생성됩니다.
...
**서버 이름 지정:** `--name`을 지정하지 않으면 첫 번째 서버의 이름은 &quot;default&quot;입니다. &quot;default&quot;가 이미 실행 중이면 임의의 이름이 생성됩니다(예: &quot;bold-crane&quot;). 반복해서 시작하거나 중지할 때 동일하게 사용할 고정된 식별자가 필요하면 `--name`을 사용하세요.

**포트:** 기본 포트는 HTTP 8123과 TCP 9000입니다. 이 포트가 이미 사용 중이면 사용 가능한 포트가 자동으로 할당되어 출력에 표시됩니다. 포트를 명시적으로 설정하려면 `--http-port`와 `--tcp-port`를 사용하세요.

**전역 서버 관리:** 모든 프로젝트 전반에서 시스템 전체를 대상으로 작업하려면 `list`, `stop`, `stop-all`과 함께 `--global`을 사용하세요. `server list --global`은 실행 중인 모든 ClickHouse 서버를 표시하며, 각 서버가 속한 디렉터리를 나타내는 Project 컬럼도 함께 보여줍니다.

<div id="custom-config-files">
  #### 로컬 서버용 사용자 지정 구성 파일
</div>

로컬 서버는 적절한 기본값으로 시작하지만, 때로는 설정을 바꿔야 할 수 있습니다. `~/.clickhouse/configs/`에 구성 파일을 넣고 서버를 시작할 때 이름으로 적용하세요:
...
지정한 파일은 **ClickHouse의 내장 기본값 위에 덮어써 적용됩니다** (`config.d`를 통해). 따라서 변경하려는 설정만 포함하면 되며, 전체 구성을 다시 작성할 필요는 없습니다. 파일은 `.xml`, `.yaml` 또는 `.yml` 형식일 수 있으며, 확장자 포함 여부와 관계없이 이름으로 참조할 수 있습니다.
...
이름이 지정된 각 서버는 자체 데이터 디렉터리를 사용하므로 서버 간에는 완전히 격리됩니다. 데이터는 재시작 후에도 유지됩니다. 중단했던 지점부터 다시 시작하려면 서버 이름을 지정해 중지한 뒤 다시 시작하십시오. 서버 데이터를 영구적으로 삭제하려면 `clickhousectl local server remove <name>`를 사용하십시오.

<div id="local-postgres">
  ### 로컬 Postgres 실행
</div>

ClickHouse 외에도 `clickhousectl`을 사용해 로컬 Postgres 인스턴스를 실행하고 관리할 수 있습니다. 로컬 Postgres는 Docker 기반이므로 Docker가 설치되어 있고 실행 중이어야 합니다. 각 인스턴스는 이름과 주 버전으로 식별되므로, 여러 Postgres 버전을 각각 별도의 데이터 디렉터리와 함께 나란히 실행할 수 있습니다.
...
API Key(권장) 또는 OAuth(브라우저 기반)를 사용해 ClickHouse Cloud에 인증할 수 있습니다.
...
API Key는 인증 방식으로 권장되며, 특히 AI Agent가 CLI를 구동할 때 적합합니다. 선택한 permission만 부여하는(읽기 전용 또는 읽기/쓰기) [scoped API Key를 생성할 수 있으며](/ko/cloud/manage/openapi), 각 키는 단일 조직에 연결됩니다. 따라서 CLI에 안전하게 액세스 권한을 부여할 수 있는 최소 권한 방식입니다.
...
자격 증명은 `.clickhouse/credentials.json`에 저장됩니다(현재 프로젝트에만 적용).

환경 변수도 사용할 수 있으며, 현재 세션에 export하여 사용할 수도 있습니다:
...
또는 현재 작업 디렉터리에 있는 `.env` 파일에 넣을 수 있습니다:
...
또는 어느 명령에서든 플래그를 사용해 자격 증명을 직접 전달할 수 있습니다:
...
브라우저가 열리며 OAuth 디바이스 플로우를 통해 인증이 진행됩니다. 토큰은 `.clickhouse/tokens.json`에 저장됩니다(프로젝트 로컬).

:::note
OAuth 액세스는 현재 **읽기 전용**이며, **속한 모든 조직**에 대한 액세스를 제공합니다. 쓰기 액세스가 필요하거나 CLI 범위를 단일 조직으로 제한하려면 대신 [scoped API Key를 생성하세요](#api-key).
:::
...
자격 증명 확인 우선순위: CLI 플래그 &gt; `.clickhouse/credentials.json` &gt; export된 환경 변수 &gt; `.env` 파일 &gt; OAuth 토큰.

<div id="debug-credentials">
  ### 사용된 자격 증명 소스 디버깅하기
</div>

명령이 실행되기 전에 확인된 자격 증명 소스(및 API URL)를 stderr에 출력하려면 아무 `cloud` 명령에나 `--debug`를 전달하세요.
...
| `--idle-timeout-minutes`                   | 최소 idle timeout(분) (&gt;= 5)                  |
| `--ip-allow`                               | 허용할 IP CIDR(반복 지정 가능, 기본값: `0.0.0.0/0`)       |
| `--backup-id`                              | 복원에 사용할 Backup ID                             |
| `--release-channel`                        | 릴리스 채널: `slow`, `default`, `fast`             |
| `--data-warehouse-id`                      | 데이터 웨어하우스 ID(읽기 전용 레플리카용)                     |
| `--readonly`                               | 서비스를 읽기 전용으로 설정                               |
| `--encryption-key`                         | 고객 디스크 암호화 키                                  |
| `--encryption-role`                        | 디스크 암호화용 role ARN                             |
| `--enable-tde`                             | Transparent Data Encryption 활성화               |
| `--compliance-type`                        | 컴플라이언스: `hipaa`, `pci`                        |
| `--profile`                                | 인스턴스 profile (Enterprise)                     |
| `--tag`                                    | 일반 제공 서비스 태그 추가(`key` 또는 `key=value`)         |
| `--enable-endpoint` / `--disable-endpoint` | 일반 제공 서비스 endpoint 사용 여부 전환(현재 `mysql`)       |
| `--private-preview-terms-checked`          | 필요한 경우 비공개 프리뷰 약관 수락                          |
| `--enable-core-dumps`                      | 서비스 코어 덤프 수집 활성화 또는 비활성화                      |

<div id="query-api-auth-modes">
  #### Query API 인증 모드
</div>

`cloud service query`는 `clickhouse` 바이너리나 서비스 비밀번호 없이 HTTP를 통해 클라우드 서비스에서 SQL을 실행하는 표준 방식입니다. 이 방식은 두 가지 자격 증명 모드 모두에서 동작합니다.

* **API Key 인증** (읽기 + 쓰기 SQL): 저장된 키가 없는 서비스에 대해 `cloud service query`를 처음 실행하면, 해당 서비스용 Query API 엔드포인트를 프로비저닝하고 여기에 연결된 전용 API Key를 생성합니다. 이 키(`keyId`, `keySecret`, `endpointId`)는 `.clickhouse/credentials.json`의 `service_query_keys.<service-id>` 아래에 저장됩니다. 이 키는 단일 서비스 범위로 제한되므로, 해당 서비스에 대해서는 읽기 및 쓰기(SELECT, INSERT, DDL)가 가능하지만 org의 다른 서비스에는 접근할 수 없습니다. 프로비저닝하지 않고 실패하게 하려면 `--no-auto-enable`을 지정하십시오.
* **OAuth** (`cloud auth login`): 쿼리는 웹 SQL 콘솔과 마찬가지로 사용자 자신의 아이덴티티로 실행됩니다. OAuth를 사용할 때 서비스에 대한 SQL 권한은 **읽기 전용**입니다. Query API Key는 프로비저닝되거나 저장되지 않습니다. 이 모드에서는 `--no-auto-enable`이 아무 영향도 없습니다.

두 인증 모드 모두에서 **idled** 상태의 서비스에 쿼리하면 자동으로 다시 활성화됩니다(첫 번째 쿼리에는 1분 정도 걸릴 수 있습니다). **stopped** 서비스는 자동으로 다시 활성화되지 않으며, 쿼리는 `cloud service start`를 실행하라는 힌트와 함께 실패합니다. 자동으로 결정된 Query API host를 재정의하려면 `CLICKHOUSE_CLOUD_QUERY_HOST`를 설정하십시오.
...
`clickhousectl`은 앞서 설명한 ClickHouse 서비스 명령과 마찬가지로 [ClickHouse Cloud Postgres](/ko/cloud/managed-postgres) 서비스도 생성하고 관리할 수 있습니다.
...
| `--tag`                    | 리소스 태그 `key` 또는 `key=value` (반복 가능)  |
...
각 소스 유형에는 `clickpipe create` 아래에 해당하는 고유한 하위 명령이 있습니다:
...
소스 유형별 전체 옵션 목록을 보려면 `clickhousectl cloud clickpipe create <source> --help`를 사용하십시오.
...
JSON 포맷의 응답을 출력하려면 `--json` 플래그를 사용하세요.
...
`clickhousectl`은 코딩 에이전트 컨텍스트(Claude Code, Cursor, Codex, Gemini CLI, Goose, Devin 및 표준 `AGENT` 환경 변수를 설정하는 모든 도구)를 자동으로 감지하며, `--json`을 설정하지 않아도 JSON을 stdout으로 자동 출력합니다.

<div id="exit-codes">
  ### 종료 코드
</div>

종료 코드는 `gh` CLI 규약을 따릅니다:

| 코드  | 의미                                            |
| --- | --------------------------------------------- |
| `0` | 성공                                            |
| `1` | 오류 (아래 범주에 속하지 않는 모든 경우)                      |
| `2` | 취소됨 (사용자가 중단함)                                |
| `4` | 인증 필요 (자격 증명 없음, 401/403, OAuth로만 허용되는 쓰기 작업) |
...
| 플래그               | 설명                                    |
| ----------------- | ------------------------------------- |
| `--agent <name>`  | 특정 에이전트에 대해 Skills를 설치합니다(여러 번 지정 가능) |
| `--global`        | 전역 범위를 사용합니다. 생략하면 프로젝트 범위가 사용됩니다     |
| `--all`           | 지원되는 모든 에이전트에 대해 Skills를 설치합니다        |
| `--detected-only` | 시스템에서 감지된 지원 에이전트에 대해서만 Skills를 설치합니다 |

<div id="self-update">
  ## 자체 업데이트
</div>

`clickhousectl`은 최신 릴리스로 직접 업데이트할 수 있습니다:
...
CLI는 백그라운드에서 업데이트를 확인하고(24시간에 최대 1회), 더 새로운 버전이 उपलब्ध하면 알림을 표시합니다.