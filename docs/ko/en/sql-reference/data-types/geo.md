---
description: '지리적 객체와 위치를 표현하는 데 사용되는 ClickHouse의 기하 데이터 타입 문서'
sidebar_label: 'Geo'
sidebar_position: 54
...
ClickHouse는 위치, 지역 등 지리적 객체를 표현하기 위한 데이터 타입을 지원합니다.

**관련 항목**

* [간단한 지리 피처 표현](https://en.wikipedia.org/wiki/GeoJSON).

<div id="point">
  ## Point
</div>

`Point`는 X축과 Y축 좌표로 표현되며, [Tuple](tuple.md)([Float64](float.md), [Float64](float.md)) 형태로 저장됩니다.
...
`Ring`은 Point 배열로 저장되는, 내부 빈 영역이 없는 단순 다각형입니다: [배열](array.md)([Point](#point)).
...
`LineString`은 Point들의 배열로 저장되는 선입니다: [배열](array.md)([Point](#point)).
...
`MultiLineString`은 여러 개의 선을 `LineString`의 배열로 저장한 형식입니다: [배열](array.md)([LineString](#linestring)).
...
`Polygon`은 링 배열로 저장되는, 홀이 있는 다각형입니다: [배열](array.md)([Ring](#ring)). 바깥 배열의 첫 번째 요소는 다각형의 바깥 경계 shape이고, 그 뒤의 모든 요소는 홀입니다.
...
`MultiPolygon`은 여러 개의 다각형으로 구성되며, 다각형의 배열로 저장됩니다: [Array](array.md)([Polygon](#polygon)).

**예시**

이 `MultiPolygon`은 서로 분리된 2개의 다각형으로 이루어져 있습니다. 첫 번째는 홀이 없고, 두 번째는 홀 1개를 포함합니다:
...
`Geometry`는 위에 나열된 모든 타입을 포괄하는 공통 타입입니다. 즉, 이러한 타입들로 이루어진 Variant와 같습니다.
