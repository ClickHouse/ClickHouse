---
description: '서로 다른 타입의 값을 하나의 컬럼에 저장할 수 있는 ClickHouse의 Dynamic 데이터 타입에 대한 문서'
...
이 타입을 사용하면 가능한 모든 타입을 미리 알지 못해도 단일 컬럼에 모든 타입의 값을 저장할 수 있습니다.
...
여기서 `N`은 `0`부터 `254` 사이의 선택적 매개변수로, 별도로 저장되는 하나의 데이터 블록(예: MergeTree 테이블의 단일 데이터 파트) 내에서 `Dynamic` 타입 컬럼에 서로 다른 데이터 타입을 몇 개까지 별도의 서브컬럼으로 저장할 수 있는지를 나타냅니다. 이 한도를 초과하면 새로운 타입의 값은 모두 바이너리 형식의 특수한 공유 데이터 구조에 함께 저장됩니다. `max_types`의 기본값은 `32`입니다.

<div id="creating-dynamic">
  ## Dynamic 생성
</div>

테이블 컬럼 정의에 `Dynamic` 데이터 타입 사용:
...
`Dynamic` 데이터 타입은 타입 이름을 서브컬럼으로 사용해 `Dynamic` 컬럼에서 단일 중첩 타입을 읽을 수 있도록 지원합니다.
따라서 `d Dynamic` 컬럼이 있으면 `d.T` 구문을 사용해 유효한 타입 `T`의 서브컬럼을 읽을 수 있습니다.
이 서브컬럼의 타입은 `T`가 `Nullable` 안에 포함될 수 있으면 `Nullable(T)`이고, 그렇지 않으면 `T`입니다. 또한 이 서브컬럼은
원래 `Dynamic` 컬럼과 동일한 크기를 가지며, 원래 `Dynamic` 컬럼의 타입이 `T`가 아닌 모든 행에는 `NULL` 값(또는 `T`가 `Nullable` 안에 포함될 수 없는 경우 빈 값)이 들어 있습니다.
...
각 행에 어떤 유형의 값이 저장되어 있는지 확인하려면 `dynamicType(dynamic_column)` 함수를 사용할 수 있습니다. 이 함수는 각 행에 대해 값의 type name이 담긴 `String`을 반환합니다(행이 `NULL`이면 `'None'`을 반환합니다).
...
<div id="conversion-between-dynamic-column-and-other-columns">
  ## Dynamic 컬럼과 다른 컬럼 간 변환
</div>

`Dynamic` 컬럼과 관련해 수행할 수 있는 변환은 4가지입니다.
...
`K < N`이면, 가장 드물게 나타나는 타입의 값은 하나의 특수한 하위 컬럼(서브컬럼)에 삽입되지만, 여전히 접근할 수 있습니다:
...
함수 `isDynamicElementInSharedData`는 `Dynamic` 내부의 특수한 공유 데이터 구조에 저장된 행에 대해 `true`를 반환하며, 보시다시피 결과 컬럼에는 공유 데이터 구조에 저장되지 않은 2개의 타입만 있습니다.
...
<div id="reading-dynamic-type-from-the-data">
  ## 데이터에서 Dynamic 데이터 타입 읽기
</div>

모든 텍스트 형식(TSV, CSV, CustomSeparated, Values, JSONEachRow 등)은 `Dynamic` 데이터 타입을 읽을 수 있습니다. 데이터 파싱 중에 ClickHouse는 각 값의 유형을 추론하고, `Dynamic` 컬럼에 삽입할 때 해당 유형을 사용합니다.
...
함수의 결과 타입이 인수 타입에 따라 달라지는 경우, `Dynamic` 인수로 실행된 함수의 결과는 `Dynamic`이 됩니다. 함수의 결과 타입이 인수 타입에 의존하지 않는 경우, 결과는 `Nullable(T)`가 되며, 여기서 `T`는 해당 함수의 일반적인 결과 타입입니다.
...
설정 `dynamic_throw_on_type_mismatch`는 함수가 `Dynamic` 컬럼에 적용될 때, 행에 실제로 저장된 유형이 해당 함수와 호환되지 않는 경우 어떻게 처리할지를 제어합니다:

* `true` (기본값) — 처음 호환되지 않는 행에서 예외(`ILLEGAL_TYPE_OF_ARGUMENT`)를 발생시킵니다.
* `false` — 호환되지 않는 행에는 `NULL`을 반환하고, 호환되는 행의 결과는 유지합니다.
...
<div id="using-dynamic-type-in-order-by-and-group-by">
  ## ORDER BY 및 GROUP BY에서 Dynamic 데이터 타입 사용하기
</div>

`ORDER BY` 및 `GROUP BY`에서는 `Dynamic` 데이터 타입의 값이 `Variant` 타입의 값과 비슷한 방식으로 비교됩니다.
기반 타입이 `T1`인 값 `d1`과 기반 타입이 `T2`인 값 `d2`에 대해, `Dynamic` 데이터 타입 값에 대한 연산자 `<`의 결과는 다음과 같이 정의됩니다:

* `T1 = T2 = T`이면 결과는 `d1.T < d2.T`입니다(기반 값이 비교됩니다).
* `T1 != T2`이면 결과는 `T1 < T2`입니다(타입 이름이 비교됩니다).

기본적으로 `Dynamic` 데이터 타입은 `GROUP BY`/`ORDER BY` 키에 사용할 수 없습니다. 사용하려면 이 특수한 비교 규칙을 고려한 뒤 `allow_suspicious_types_in_group_by`/`allow_suspicious_types_in_order_by` 설정을 활성화하십시오.
...
**참고:** 숫자 타입이 다른 Dynamic 데이터 타입 값은 서로 다른 값으로 간주되며, 서로 직접 비교하지 않고 대신 타입 이름을 비교합니다.
...
**참고:** 앞서 설명한 비교 규칙은 `Dynamic` 데이터 타입 함수의 [특수한 동작](#using-dynamic-type-in-functions) 때문에 `<`/`>`/`=` 등의 비교 함수를 실행할 때는 적용되지 않습니다

<div id="reaching-the-limit-in-number-of-different-data-types-stored-inside-dynamic">
  ## Dynamic 내부에 저장되는 서로 다른 데이터 타입 수 제한에 도달한 경우
</div>

`Dynamic` 데이터 타입은 제한된 수의 서로 다른 데이터 타입만 별도의 서브컬럼으로 저장할 수 있습니다. 기본적으로 이 제한은 32이지만, `Dynamic(max_types=N)` 구문을 사용해 타입 선언 시 변경할 수 있습니다. 여기서 N은 0에서 254 사이의 값입니다(구현상의 이유로 Dynamic 내부에서 별도의 서브컬럼으로 저장할 수 있는 서로 다른 데이터 타입은 254개를 초과할 수 없습니다).
제한에 도달하면 `Dynamic` 컬럼에 새로 삽입되는 모든 데이터 타입은 서로 다른 데이터 타입의 값을 바이너리 형태로 저장하는 단일 공유 데이터 구조에 삽입됩니다.

이제 다양한 시나리오에서 제한에 도달하면 어떤 일이 발생하는지 살펴보겠습니다.

<div id="reaching-the-limit-during-data-parsing">
  ### 데이터 파싱 중 한도에 도달하는 경우
</div>

데이터에서 `Dynamic` 값을 파싱하는 동안 현재 데이터 블록에 대한 한도에 도달하면, 이후의 모든 새 값은 공유 데이터 구조에 삽입됩니다:
...
보시다시피, 서로 다른 3개의 데이터 타입 `Int64`, `Array(Int64)`, `String`을 삽입한 후 새 타입은 모두 특수한 공유 데이터 구조에 저장되었습니다.

<div id="during-merges-of-data-parts-in-mergetree-table-engines">
  ### MergeTree 테이블 엔진에서 데이터 파트를 머지하는 동안
</div>

MergeTree 테이블에서 여러 데이터 파트를 머지하는 동안, 결과 데이터 파트의 `Dynamic` 컬럼이 내부의 별도 서브컬럼에 저장할 수 있는 서로 다른 데이터 타입 수의 한도에 도달하여 원본 파트의 모든 타입을 서브컬럼으로 저장하지 못할 수 있습니다.
이 경우 ClickHouse는 머지 후 어떤 타입을 별도 서브컬럼으로 유지하고 어떤 타입을 공유 데이터 구조에 삽입할지 결정합니다. 대부분의 경우 ClickHouse는 가장 자주 나타나는 타입을 유지하고, 가장 드물게 나타나는 타입은 공유 데이터 구조에 저장하려고 하지만, 이는 구현에 따라 달라집니다.

이러한 머지의 예시를 살펴보겠습니다. 먼저 `Dynamic` 컬럼이 있는 테이블을 만들고, 서로 다른 데이터 타입 수의 한도를 `3`으로 설정한 다음, `5`개의 서로 다른 타입 값을 삽입하겠습니다:
...
각 삽입마다 단일 유형을 포함하는 `Dynamic` 컬럼이 있는 별도의 데이터 파트를 생성합니다:
...
보시다시피 ClickHouse는 가장 자주 나타나는 타입인 `UInt64`와 `Array(UInt64)`는 서브컬럼으로 유지하고, 그 외의 모든 타입은 공유 데이터에 삽입했습니다.

<div id="jsonextract-functions-with-dynamic">
  ## Dynamic을 사용하는 JSONExtract 함수
</div>

모든 `JSONExtract*` 함수는 `Dynamic` 데이터 타입을 지원합니다:
...
RowBinary format에서는 `Dynamic` 타입 값이 다음 형식으로 직렬화됩니다:
