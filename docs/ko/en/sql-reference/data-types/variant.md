...
이 타입은 다른 데이터 타입의 유니온을 나타냅니다. `Variant(T1, T2, ..., TN)` 타입은 이 타입의 각 행이
`T1`, `T2`, ..., `TN` 중 하나의 타입 값을 가지거나, 어느 타입에도 속하지 않는 값(`NULL` 값)을 가질 수 있음을 의미합니다.

중첩 타입의 순서는 중요하지 않습니다. Variant(T1, T2) = Variant(T2, T1)입니다.
중첩 타입은 Nullable(...), LowCardinality(Nullable(...)), Variant(...) 타입을 제외하면 어떤 타입이든 사용할 수 있습니다.

:::note
유사한 타입을 Variant로 사용하는 것은 권장되지 않습니다(예: `Variant(UInt32, Int64)`처럼 서로 다른 숫자 타입이나 `Variant(Date, DateTime)`처럼 서로 다른 날짜 타입).
이러한 타입의 값을 처리할 때 모호성이 발생할 수 있기 때문입니다. 기본적으로 이런 `Variant` 타입을 생성하면 예외가 발생하지만, `allow_suspicious_variant_types` setting을 사용해 활성화할 수 있습니다.
:::
...
인수에 공통 타입이 없을 때 `if/multiIf` 함수를 사용합니다(`use_variant_as_common_type` 설정이 활성화되어 있어야 합니다):
...
배열 요소/맵 값에 공통 타입이 없는 경우 &#39;array/map&#39; 함수를 사용합니다(이 기능을 사용하려면 설정 `use_variant_as_common_type`이 활성화되어 있어야 합니다):
...
<div id="reading-variant-nested-types-as-subcolumns">
  ## 서브컬럼으로 Variant 중첩 타입 읽기
</div>

Variant 타입은 타입 이름을 서브컬럼으로 사용해 Variant 컬럼에서 특정 중첩 타입 하나를 읽을 수 있습니다.
...
원래 `Variant` 컬럼과 동일한 크기를 가지며, 원래 `Variant` 컬럼의 타입이 `T2`가 아닌 모든 행에는 `NULL` 값(또는 `T2`가 `Nullable` 안에 포함될 수 없는 경우 빈 값)이 들어 있습니다.
...
각 행에 저장된 variant를 확인하려면 `variantType(variant_column)` 함수를 사용할 수 있습니다. 이 함수는 각 행에 대해 variant 유형 이름을 나타내는 `Enum`을 반환합니다(행이 `NULL`이면 `'None'`을 반환합니다).
...
`Variant` 유형의 컬럼에서 수행할 수 있는 변환은 4가지입니다.
...
참고: `String` 타입에서의 변환은 항상 파싱을 거칩니다. 파싱 없이 `String` 컬럼을 `Variant`의 `String` variant로 변환해야 한다면, 다음과 같이 수행할 수 있습니다:
...
`Variant` 컬럼은 일반 컬럼으로 변환할 수 있습니다. 이 경우 중첩된 모든 Variant도 대상 유형으로 변환됩니다:
...
`Variant` 컬럼은 다른 `Variant` 컬럼으로 변환할 수 있지만, 대상 `Variant` 컬럼에 기존 `Variant`의 모든 중첩 타입이 포함되어 있는 경우에만 가능합니다:
...
모든 텍스트 형식(TSV, CSV, CustomSeparated, Values, JSONEachRow 등)은 `Variant` 타입을 읽을 수 있습니다. 데이터를 파싱하는 동안 ClickHouse는 값을 가장 적절한 Variant 타입에 삽입하려고 시도합니다.
...
`Variant` 타입의 값은 같은 `Variant` 타입의 값과만 비교할 수 있습니다.

기본적으로 비교 연산자는 [Variant에 대한 기본 구현](#functions-with-variant-arguments)을 사용하여
각 variant type에 개별적으로 비교를 적용합니다. 이는 설정 `use_variant_default_implementation_for_comparisons = 0`으로 비활성화할 수 있으며,
이 경우 아래에 설명된 네이티브 Variant 비교 규칙이 사용됩니다. **참고:** `ORDER BY`는 항상 네이티브 비교를 사용합니다.

**네이티브 Variant 비교 규칙:**

`Variant(..., T1, ... T2, ...)` 타입에서, 내부 타입이 `T1`인 값 `v1`과 내부 타입이 `T2`인 값 `v2`에 대한 연산자 `<`의 결과는 다음과 같이 정의됩니다.

* `T1 = T2 = T`이면 결과는 `v1.T < v2.T`입니다(내부 값이 비교됨).
* `T1 != T2`이면 결과는 `T1 < T2`입니다(타입 이름이 비교됨).
...
때로는 Variant 타입을 추가로 확인하는 것이 유용할 수 있습니다. `Array/Map/Tuple`과 같은 복합 타입의 서브컬럼은 `Nullable` 안에 포함될 수 없으므로, 타입이 다른 행에서는 `NULL` 대신 기본값을 갖습니다:
...
**참고:** 숫자 타입이 다른 Variant의 값은 서로 다른 Variant로 간주되며, 값끼리는 서로 비교하지 않고 대신 타입 이름을 비교합니다.
...
**참고** 기본적으로 `Variant` 타입은 `GROUP BY`/`ORDER BY` 키에 사용할 수 없습니다. 사용하려면 이 타입의 특수한 비교 규칙을 고려한 뒤 `allow_suspicious_types_in_group_by`/`allow_suspicious_types_in_order_by` 설정을 활성화하십시오.

<div id="jsonextract-functions-with-variant">
  ## `Variant`를 지원하는 JSONExtract 함수
</div>
...
`26.1` 버전부터는 Variant 타입을 명시적으로 처리하지 않는 함수가 Variant 컬럼을 받으면 ClickHouse는 다음과 같이 동작합니다.
...
비교 연산자는 각 variant 타입에 자동으로 개별 적용되므로 Variant 컬럼에 대해 필터링할 수 있습니다.
...
:::note
**오류 처리:** 함수가 variant 타입을 처리할 수 없는 경우, 타입 관련 오류(ILLEGAL&#95;TYPE&#95;OF&#95;ARGUMENT,
TYPE&#95;MISMATCH, CANNOT&#95;CONVERT&#95;TYPE, NO&#95;COMMON&#95;TYPE)만 포착되며 해당 행에서는 결과가 NULL이 됩니다. 그 밖의 오류(예: 0으로 나누기, 메모리 부족)는 실제 문제가 조용히 가려지는 것을 방지하기 위해 평소와 같이 발생합니다.
:::
...
설정 `variant_throw_on_type_mismatch`는 함수가 `Variant` 컬럼에 적용될 때 행에 실제로 저장된 유형이 함수와 호환되지 않으면 어떻게 처리할지를 제어합니다.

* `true` (기본값) — 처음으로 호환되지 않는 행에서 예외(`ILLEGAL_TYPE_OF_ARGUMENT`)를 발생시킵니다.
* `false` — 호환되지 않는 행에는 `NULL`을 반환하고, 호환되는 행은 결과를 그대로 유지합니다.
