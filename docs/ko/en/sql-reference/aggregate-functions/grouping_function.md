...
[ROLLUP](../statements/select/group-by.md/#rollup-modifier) 및 [CUBE](../statements/select/group-by.md/#cube-modifier)는 GROUP BY의 수정자입니다. 두 수정자 모두 소계를 계산합니다. ROLLUP은 `(day, month, year)`와 같이 순서가 지정된 컬럼 목록을 받아, 집계의 각 수준에서 소계를 계산한 다음 전체 합계를 계산합니다. CUBE는 지정된 컬럼의 가능한 모든 조합에 대해 소계를 계산합니다. GROUPING은 ROLLUP 또는 CUBE가 반환한 행 가운데 어떤 행이 상위 집계(superaggregate)인지, 그리고 어떤 행이 수정자가 없는 GROUP BY에서 반환되는 행인지를 식별합니다.

GROUPING 함수는 여러 컬럼을 인수로 받아 비트마스크를 반환합니다.

* `1`은 `GROUP BY`의 `ROLLUP` 또는 `CUBE` 수정자가 반환한 행이 소계임을 나타냅니다
* `0`은 `ROLLUP` 또는 `CUBE`가 반환한 행이 소계가 아닌 일반 행임을 나타냅니다

<div id="grouping-sets">
  ## GROUPING SETS
</div>

기본적으로 CUBE 수정자는 CUBE에 전달된 컬럼의 가능한 모든 조합에 대해 소계를 계산합니다. GROUPING SETS를 사용하면 계산할 특정 조합을 지정할 수 있습니다.

계층형 데이터를 분석할 때는 ROLLUP, CUBE, GROUPING SETS 수정자가 유용합니다. 여기의 예시는 두 개의 데이터센터에 설치된 Linux 배포판과 해당 배포판의 버전에 대한 데이터를 담고 있는 테이블입니다. 이 데이터는 배포판, 버전, 위치별로 살펴보면 유용할 수 있습니다.
...
<div id="comparing-multiple-group-by-statements-with-grouping-sets">
  ### GROUPING SETS로 여러 GROUP BY SQL 문 비교하기
</div>
...
GROUPING SETS를 사용해 동일한 정보를 얻는 방법:
...
다음 쿼리의 CUBE인 `CUBE(datacenter,distro,version)`는 의미가 불분명한 계층 구조를 만들 수 있습니다. 두 배포판에 걸쳐 version을 비교하는 것은 적절하지 않습니다. Arch와 RHEL은 릴리스 주기나 버전 명명 방식이 서로 다르기 때문입니다. 이어지는 GROUPING SETS 예시는 `distro`와 `version`을 같은 집합으로 묶으므로 더 적절합니다.
...
:::note
위 예시에서 Version은 distro와 연결되지 않으면 의미가 없을 수 있습니다. 반면 커널 버전을 추적하는 경우에는 커널 버전이 어느 distro와도 연결될 수 있으므로 의미가 있을 수 있습니다. 다음 예시와 같이 GROUPING SETS를 사용하는 것이 더 적절할 수 있습니다.
:::
