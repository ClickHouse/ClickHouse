---
description: 'Amazon S3의 Delta Lake 테이블에 대해 읽기 전용 테이블 유사 인터페이스를
  제공합니다.'
...
Amazon S3, Azure Blob Storage 또는 로컬에 마운트된 파일 시스템의 [Delta Lake](https://github.com/delta-io/delta) 테이블에 대해 테이블 유사 인터페이스를 제공하며, 읽기와 쓰기를 모두 지원합니다(v25.10부터)
...
선택 사항인 `extra_credentials` 매개변수를 사용하면 ClickHouse Cloud에서 역할 기반 접근을 위한 `role_arn`을 전달할 수 있습니다. 구성 단계는 [Secure S3](/ko/cloud/data-sources/secure-s3)를 참조하십시오.

<div id="returned_value">
  ## 반환 값
</div>

지정된 Delta Lake 테이블에서 데이터를 읽거나 쓸 수 있도록, 지정된 구조의 테이블을 반환합니다.
...
`https://clickhouse-public-datasets.s3.amazonaws.com/delta_lake/hits/`에 있는 S3 스토리지의 테이블을 예로 들어 보겠습니다.
...
`s3://ch-docs-s3-bucket/people_10k/`에 있는 S3 스토리지의 테이블을 예로 들어 보겠습니다.
Delta Lake 쓰기는 기본적으로 비활성화되어 있는 베타 기능입니다. 다음과 같이 활성화하십시오(`allow_delta_lake_writes`는 버전 26.7부터 사용할 수 있으며, 이전 버전에서는 `allow_experimental_delta_lake_writes`를 사용하십시오):
...
테이블을 다시 조회하여 삽입이 성공했는지 확인할 수 있습니다:
...
* [DeltaLake 클러스터 테이블 함수](/ko/sql-reference/table-functions/deltalakeCluster.md)