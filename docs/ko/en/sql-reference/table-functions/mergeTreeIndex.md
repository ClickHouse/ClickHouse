---
description: 'MergeTree 테이블의 인덱스 및 마크 파일의 내용을 나타냅니다.
  내부 검사에 사용할 수 있습니다.'
...
MergeTree 테이블의 인덱스 및 마크 파일의 내용을 나타냅니다. 내부 검사에 사용할 수 있습니다.
...
원본 테이블의 프라이머리 인덱스와 MinMax 인덱스(사용 설정된 경우) 값이 들어 있는 컬럼, 원본 테이블의 데이터 파트에 있는 가능한 모든 파일에 대한 마크 값(사용 설정된 경우)이 들어 있는 컬럼, 그리고 가상 컬럼을 포함하는 테이블 객체입니다.

* `part_name` - 데이터 파트의 이름입니다.
* `mark_number` - 데이터 파트의 현재 마크 번호입니다.
* `rows_in_granule` - 현재 그래뉼의 행 수입니다.

컬럼이 데이터 파트에 없거나 해당 서브스트림 중 하나의 마크가 기록되지 않은 경우(예: compact 파트), 마크 컬럼에는 `(NULL, NULL)` 값이 들어 있을 수 있습니다.
