---
description: '지정된 스키마로 무작위 데이터를 생성합니다. 생성된 데이터로 테스트 테이블을
  채울 수 있습니다. 모든 타입이 지원되는 것은 아닙니다.'
...
지정된 스키마로 무작위 데이터를 생성합니다.
생성된 데이터로 테스트 테이블을 채울 수 있습니다.
모든 타입이 지원되는 것은 아닙니다.
...
| ------------------- | --------------------------------------------------------- |
| `name`              | 해당 컬럼의 이름입니다.                                             |
| `TypeName`          | 해당 컬럼의 유형입니다.                                             |
| `random_seed`       | 일관된 결과를 얻기 위해 랜덤 시드를 수동으로 지정합니다. `NULL`이면 시드는 무작위로 생성됩니다. |
...
[generateRandomStructure](../../sql-reference/functions/other-functions.md#generateRandomStructure)와 조합하면:
...
`structure` 인수가 없을 경우(이 경우 구조는 임의입니다):
...
무작위 구조와 무작위 데이터에 모두 랜덤 시드를 사용하는 경우:
...
:::note
`max_array_length`가 충분히 크면 `generateRandom(generateRandomStructure(), [random seed], max_string_length, max_array_length)`는 복합 타입(`Array`, `Tuple`, `Map`, `Nested`)의 중첩 깊이가 매우 깊어질 수 있으므로(최대 16) 정말로 매우 큰 출력을 생성할 수 있습니다.
:::
...
* 블로그: [ClickHouse에서 무작위 데이터 생성](https://clickhouse.com/blog/generating-random-test-distribution-data-for-clickhouse)