...
아래 공식이 적용되려면 윈도우 평가 순서에서 `timestamp_column`이 순증가해야 하므로, 일반적으로 `timestamp_column`을 오름차순으로 윈도우를 정렬해야 합니다(예: `nonNegativeDerivative(metric, ts)`와 함께 `... OVER (ORDER BY ts ASC)` 사용).
현재 행과 이전 행 사이의 경과 시간이 양수가 아닌 경우(즉, `ORDER BY timestamp_column DESC`를 사용하거나 중복된(동일한) 타임스탬프가 있는 경우), 함수는 공식 대신 해당 행에 대해 `0`을 반환합니다.
:::

결과는 `INTERVAL`당 메트릭의 변화율이며, 음수 값은 `0`으로 고정됩니다.
이 함수는 카운터와 같이 단조 증가하는 메트릭에 유용합니다. 이러한 메트릭에서 값의 감소는 실제 음수 변화율이 아닌 카운터 재설정을 의미하는 경우가 많습니다.
...
- `timestamp_column` — 윈도우 순서에서 현재 행과 이전 행 사이의 경과 시간을 측정하는 데 사용되는 컬럼. 행의 정렬 순서를 결정하지 않으며, 정렬은 윈도우의 `ORDER BY`가 담당합니다. 일반적으로 동일한 컬럼을 사용하는 것을 권장합니다. [DateTime](../data-types/datetime.md) 또는 [DateTime64](../data-types/datetime64.md).
- `INTERVAL X UNITS` — 선택 사항. 결과를 환산할 시간 단위. 기본값은 `INTERVAL 1 SECOND`. 고정 길이 단위(`NANOSECOND`, `MICROSECOND`, `MILLISECOND`, `SECOND`, `MINUTE`, `HOUR`, `DAY`, `WEEK`)만 지원되며, 가변 길이 단위(`MONTH`, `QUARTER`, `YEAR`)를 사용하면 예외가 발생합니다.
...
- 첫 번째 행: `0`
- 이전 행 이후 경과 시간이 양수가 아닌 행(즉, $\text{timestamp}_i - \text{timestamp}_{i-1} \le 0$인 경우로, 내림차순 정렬 또는 중복 타임스탬프가 있을 때 발생): `0`
