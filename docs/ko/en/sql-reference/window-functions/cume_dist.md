...
값 집합 내에서 특정 값의 누적 분포를 계산합니다. 즉, 현재 행의 값보다 작거나 같은 값을 가진 행의 비율을 계산합니다. 파티션 내에서 값의 상대적 위치를 파악하는 데 사용할 수 있습니다.
...
기본이며 필수인 윈도 프레임 정의는 `RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING`입니다.
...
`cume_dist()` 함수는 다음 공식으로 상대적 위치를 계산합니다:

```text
cume_dist = (number of rows ≤ current row value) / (total number of rows in partition)
```

동일한 값을 가진 행들(피어)은 모두 같은 누적 분포 값을 가지며, 이 값은 해당 피어 그룹의 가장 높은 순위에 해당합니다.