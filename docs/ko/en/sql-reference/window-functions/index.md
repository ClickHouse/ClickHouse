...
윈도우 함수는 현재 행과 관련된 행 집합을 기준으로 계산을 수행할 수 있게 해줍니다.
집계 함수로 수행하는 계산과 유사한 계산에 사용할 수 있지만, 윈도우 함수는 행을 하나의 결과로 그룹화하지 않는다는 점에서 다릅니다. 대신 각 행이 그대로 반환됩니다.

<div id="standard-window-functions">
  ## 표준 윈도우 함수
</div>

ClickHouse는 윈도우 및 윈도우 함수에 대한 표준 SQL 문법을 지원합니다.
아래 표는 현재 지원되는 기능을 보여줍니다.

| 기능                                                                 | 지원 여부 | 설명                                                                                                                                                                                                                                                                                                              |
| ------------------------------------------------------------------ | ----- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 임시 윈도우 지정 (`count(*) OVER (PARTITION BY id ORDER BY time DESC)`)   | ✅     |                                                                                                                                                                                                                                                                                                                 |
...
| `DateTime` `RANGE OFFSET` 프레임의 `INTERVAL` 구문                       | ❌     | 대신 초 단위 숫자를 지정하십시오 (`RANGE`는 모든 숫자형에서 동작합니다).                                                                                                                                                                                                                                                                   |
| `GROUPS` 프레임                                                       | ❌     |                                                                                                                                                                                                                                                                                                                 |
| 프레임에 대해 집계 함수를 계산 (`sum(value) OVER (ORDER BY time)`)              | ✅     | 모든 집계 함수가 지원됩니다.                                                                                                                                                                                                                                                                                                |
| `rank()`, `dense_rank()`/`denseRank()`, `row_number()`             | ✅     |                                                                                                                                                                                                                                                                                                                 |
| `percent_rank()`/`percentRank()`                                   | ✅     | 파티션 내 값의 상대적 순위를 효율적으로 계산합니다. 이는 `ifNull((rank() OVER (PARTITION BY x ORDER BY y) - 1) / nullif(count(1) OVER (PARTITION BY x) - 1, 0), 0)`로 표현되는, 더 장황하고 계산 비용이 큰 수동 SQL 계산을 대체합니다.                                                                                                                            |
| `cume_dist()`                                                      | ✅     | 값 그룹 내에서 값의 누적 분포를 계산합니다. 현재 행의 값보다 작거나 같은 값을 가진 행의 비율을 반환합니다.                                                                                                                                                                                                                                                  |
| `lag/lead(value, offset)`                                          | ✅     | 다음 우회 방법도 사용할 수 있습니다.<br /> 1) `any(value) OVER (... ROWS BETWEEN <offset> PRECEDING AND <offset> PRECEDING)` 또는 `lead`의 경우 `PRECEDING` 대신 `FOLLOWING` 사용 <br /> 2) `lagInFrame/leadInFrame`은 유사하지만 윈도우 프레임을 따릅니다. `lag/lead`와 동일한 동작을 얻으려면 `ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING`을 사용하십시오. |
| [`ntile(buckets)`](./ntile.md)                                     | ✅     | 예를 들어 윈도우를 `(PARTITION BY x ORDER BY y ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING)`와 같이 지정하십시오.                                                                                                                                                                                                   |
...
* `ORDER BY` - aggregate&#95;function 계산 중 그룹 내 행의 순서를 정의합니다.
* `ROWS or RANGE` - 프레임의 경계를 정의하며, aggregate&#95;function은 프레임 내에서 계산됩니다.
* `WINDOW` - 여러 표현식에서 동일한 윈도우 정의를 사용할 수 있게 합니다.
...
| ---------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------ |
| [`row_number()`](./row_number.md)                                                                          | 파티션 내 현재 행에 1부터 시작하는 번호를 매깁니다.                                                 |
| [`first_value(x)`](./first_value.md)                                                                       | 정렬된 프레임 내에서 평가된 첫 번째 값을 반환합니다.                                                 |
| [`last_value(x)`](./last_value.md)                                                                         | 정렬된 프레임 내에서 평가된 마지막 값을 반환합니다.                                                  |
| [`nth_value(x, offset)`](./nth_value.md)                                                                   | 정렬된 프레임의 n번째 행(`offset`)에서 평가된 첫 번째 non-NULL 값을 반환합니다.                         |
| [`rank()`](./rank.md)                                                                                      | 파티션 내 현재 행의 순위를 매기며, 중간 순위를 건너뛸 수 있습니다.                                        |
| [`dense_rank()`](./dense_rank.md)                                                                          | 파티션 내 현재 행의 순위를 매기며, 중간 순위를 건너뛰지 않습니다.                                         |
| [`lagInFrame(x)`](./lagInFrame.md)                                                                         | 정렬된 프레임 내에서 현재 행보다 지정된 물리적 오프셋만큼 앞선 행에서 평가된 값을 반환합니다.                          |
| [`leadInFrame(x)`](./leadInFrame.md)                                                                       | 정렬된 프레임 내에서 현재 행보다 지정된 오프셋만큼 뒤에 있는 행에서 평가된 값을 반환합니다.                           |
| [`ntile(buckets)`](./ntile.md)                                                                             | 파티션 내의 정렬된 행을 지정된 수의 버킷으로 나누고 현재 행의 버킷 번호를 반환합니다.                              |
...
다음 예시는 실제 환경에서 자주 접하는 일반적인 문제를 해결하는 방법을 보여줍니다.
...
온도는 초 단위 정밀도로 저장되지만, `Range`와 `ORDER BY toDate(ts)`를 사용하면 크기가 10인 프레임을 구성할 수 있으며, `toDate(ts)`를 사용하므로 그 단위는 일입니다.
...
윈도우 함수의 초기 지원에 대한 로드맵은 [이 이슈](https://github.com/ClickHouse/ClickHouse/issues/18097)에서 확인할 수 있습니다.
...
* 블로그: [Git 커밋 시퀀스를 위한 윈도우 및 배열 함수](https://clickhouse.com/blog/clickhouse-window-array-functions-git-commits)
