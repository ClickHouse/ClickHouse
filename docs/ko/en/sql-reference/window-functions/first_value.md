...
정렬된 프레임 내에서 평가된 첫 번째 값을 반환합니다. 기본적으로 `NULL` 인수는 건너뛰지만, `RESPECT NULLS` 수정자를 사용해 이 동작을 재정의할 수 있습니다.
...
* 정렬된 프레임 내에서 계산된 첫 번째 값입니다.

**예시**

이 예시에서는 `first_value` 함수를 사용하여 프리미어리그 축구 선수 연봉에 대한 가상의 데이터셋에서 가장 높은 연봉을 받는 선수를 찾습니다.
