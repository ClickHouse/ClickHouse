...
`WHERE` 절을 사용하면 `SELECT`의 [`FROM`](../../../sql-reference/statements/select/from.md) 절에서 가져온 데이터를 필터링할 수 있습니다.

`WHERE` 절이 있으면 그 뒤에는 `UInt8` 유형의 표현식이 와야 합니다.
이 표현식의 평가 결과가 `0`인 행은 이후 변환이나 결과에서 제외됩니다.

`WHERE` 절 뒤에 오는 표현식은 [비교 연산자](/ko/sql-reference/operators#comparison-operators), [논리 연산자](/ko/sql-reference/operators#operators-for-working-with-data-sets), 또는 여러 [일반 함수](/ko/sql-reference/functions/regular-functions) 중 하나와 함께 자주 사용됩니다.

기반 테이블 엔진이 이를 지원하는 경우, `WHERE` 표현식은 인덱스와 파티션 프루닝에 활용됩니다.

:::note PREWHERE
[`PREWHERE`](../../../sql-reference/statements/select/prewhere.md)라는 필터링 최적화도 있습니다.
...
값이 [`NULL`](/ko/sql-reference/syntax#null)인지 확인해야 하면 다음을 사용합니다:

* [`IS NULL`](/ko/sql-reference/operators#is_null) 또는 [`isNull`](../../../sql-reference/functions/functions-for-nulls.md#isNull)
* [`IS NOT NULL`](/ko/sql-reference/operators#is_not_null)   또는 [`isNotNull`](../../../sql-reference/functions/functions-for-nulls.md#isNotNull)

그렇지 않으면 `NULL`이 포함된 표현식은 조건을 절대 만족하지 않습니다.

<div id="filtering-data-with-logical-operators">
  ## 논리 연산자로 데이터 필터링하기
</div>

여러 조건을 조합하려면 `WHERE` 절과 함께 다음 [논리 함수](/ko/sql-reference/functions/logical-functions#and)를 사용할 수 있습니다:

* [`and()`](/ko/sql-reference/functions/logical-functions#and) 또는 `AND`
* [`not()`](/ko/sql-reference/functions/logical-functions#not) 또는 `NOT`
* [`or()`](/ko/sql-reference/functions/logical-functions#or) 또는 `NOT`
* [`xor()`](/ko/sql-reference/functions/logical-functions#xor)

<div id="using-uint8-columns-as-a-condition">
  ## UInt8 컬럼을 조건으로 사용하기
</div>

ClickHouse에서는 `UInt8` 컬럼을 Boolean 조건으로 직접 사용할 수 있으며, 이때 `0`은 `false`, 0이 아닌 모든 값(일반적으로 `1`)은 `true`를 의미합니다.
이에 대한 예시는 [아래](#example-uint8-column-as-condition) 섹션에서 확인할 수 있습니다.

<div id="using-comparison-operators">
  ## 비교 연산자 사용
</div>

다음 [비교 연산자](/ko/sql-reference/operators#comparison-operators)를 사용할 수 있습니다:
...
| `a ILIKE s`             | `ilike(a, b)`           | 패턴 일치 (대소문자 비구분) | `name ILIKE '%LAPTOP%'`         |
...
| 기능          | 구문                             | 대소문자 구분 | 성능   | 가장 적합한 용도        |
| ----------- | ------------------------------ | ------- | ---- | ---------------- |
| `LIKE`      | `col LIKE '%pattern%'`         | 예       | 빠름   | 대소문자를 구분하는 패턴 매칭 |
| `ILIKE`     | `col ILIKE '%pattern%'`        | 아니요     | 더 느림 | 대소문자를 구분하지 않는 검색 |
| `if()`      | `if(cond, a, b)`               | 해당 없음   | 빠름   | 단순한 2항 조건        |
...
<div id="expressions-with-literals-columns-subqueries">
  ## 리터럴, 컬럼 또는 서브쿼리를 사용하는 표현식
</div>

`WHERE` 절 뒤의 표현식에는 [리터럴](/ko/sql-reference/syntax#literals), 컬럼 또는 서브쿼리도 포함할 수 있습니다. 서브쿼리는 조건에 사용되는 값을 반환하는 중첩된 `SELECT` SQL 문입니다.

| 유형       | 정의         | 평가       | 성능       | 예시                         |
| -------- | ---------- | -------- | -------- | -------------------------- |
| **리터럴**  | 고정된 상수 값   | 쿼리 작성 시점 | 가장 빠름    | `WHERE price > 100`        |
| **컬럼**   | 테이블 데이터 참조 | 각 행마다    | 빠름       | `WHERE price > cost`       |
| **서브쿼리** | 중첩된 SELECT | 쿼리 실행 시점 | 달라질 수 있음 | `WHERE id IN (SELECT ...)` |
...
**4. `XOR` - 정확히 하나의 조건만 true여야 합니다(둘 다 true이면 안 됨):**
...
SQL 키워드 구문(`AND`, `OR`, `NOT`, `XOR`)이 일반적으로 가독성이 더 좋지만, 복잡한 표현식이나 동적 쿼리를 작성할 때는 함수 구문이 유용할 수 있습니다.

<div id="example-uint8-column-as-condition">
  ### UInt8 컬럼을 조건으로 사용하기
</div>

[이전 예시](#example-filtering-with-logical-operators)에서 사용한 테이블을 기준으로, 컬럼 이름을 직접 조건으로 사용할 수 있습니다:
...
아래 예시에서는 위의 [예시](#example-filtering-with-logical-operators)에 있는 테이블(table)과 데이터를 사용합니다. 간결성을 위해 결과는 생략합니다.
...
아래 예시에서 `(1, true)`는 [튜플](/ko/sql-reference/data-types/tuple)입니다.
...
이를 위해 [배열](/ko/sql-reference/data-types/array)도 사용할 수 있습니다:
...
아래 예시에서는 위의 [예시](#example-filtering-with-logical-operators)에 있는 테이블과 데이터를 사용합니다. 간결하게 하기 위해 결과는 생략합니다.
