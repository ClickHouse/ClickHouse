...
:::tip
마스킹 정책은 저장된 데이터를 변경하지 않고도 쿼리 시점에 민감한 데이터를 변환하여 컬럼 수준의 데이터 보안을 제공합니다.
:::
...
`UPDATE` 절은 어떤 컬럼을 마스킹할지와 해당 컬럼을 어떻게 변환할지를 지정합니다. 하나의 정책(policy)에서 여러 컬럼을 마스킹할 수 있습니다.
...
선택적 `WHERE` 절을 사용하면 행 값에 따라 조건부 마스킹을 적용할 수 있습니다. 조건을 만족하는 행에만 마스킹이 적용됩니다.
...
`TO` 섹션에서는 정책을 적용할 사용자 및 역할을 지정합니다.

* `TO user1, user2`: 특정 사용자/역할에 적용합니다
* `TO ALL`: 모든 사용자에 적용합니다
* `TO ALL EXCEPT user1, user2`: 지정한 사용자를 제외한 모든 사용자에 적용합니다

:::note
행 정책과 달리 마스킹 정책은 해당 정책이 적용되지 않는 사용자에게는 영향을 주지 않습니다. 사용자에게 적용되는 마스킹 정책이 없으면 원본 데이터를 보게 됩니다.
:::
...
여러 마스킹 정책이 한 사용자의 동일한 컬럼을 대상으로 하는 경우, `PRIORITY` 절이 적용 순서를 결정합니다. 정책은 우선순위가 높은 것부터 낮은 것 순으로 적용됩니다.
...
* 마스킹 정책은 표현식의 복잡도에 따라 쿼리 성능에 영향을 줄 수 있습니다
