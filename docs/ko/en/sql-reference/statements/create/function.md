...
람다 표현식으로 사용자 정의 함수(UDF)를 생성합니다. 표현식은 함수 매개변수, 상수, 연산자 또는 다른 함수 호출로만 구성되어야 합니다.
...
함수는 매개변수를 원하는 만큼 가질 수 있습니다.

다음과 같은 몇 가지 제한 사항이 있습니다:

* 함수 이름은 사용자 정의 함수와 시스템 함수 사이에서 고유해야 합니다.
* 재귀 함수는 허용되지 않습니다.
* 함수에서 사용하는 모든 변수는 해당 함수의 매개변수 목록에 지정되어야 합니다.

이 제한 사항 중 하나라도 위반되면 예외가 발생합니다.
...
다음 쿼리에서는 사용자 정의 함수 내에서 [조건 함수](../../../sql-reference/functions/conditional-functions.md)를 호출합니다:
...
<div id="executable-udfs">
  ### [실행형 UDF](/ko/sql-reference/functions/udf.md).
</div>
