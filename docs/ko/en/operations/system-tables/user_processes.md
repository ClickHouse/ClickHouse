---
description: '사용자의 메모리 사용량과 ProfileEvents를 전반적으로 파악하는 데 유용한 정보를 담고 있는 시스템 테이블입니다.'
...
이 시스템 테이블을 사용하면 사용자의 메모리 사용량과 ProfileEvents 개요를 확인할 수 있습니다.
...
* `user` ([String](/ko/sql-reference/data-types/string)) — 사용자 이름입니다.
* `memory_usage` ([Int64](/ko/sql-reference/data-types/int-uint)) — 사용자의 모든 프로세스에서 사용 중인 RAM 총합입니다. 일부 전용 메모리 유형은 포함되지 않을 수 있습니다. `max_memory_usage` 설정을 참조하십시오.
* `peak_memory_usage` ([Int64](/ko/sql-reference/data-types/int-uint)) — 사용자의 최대 메모리 사용량입니다. 해당 사용자에 대해 실행된 쿼리가 없으면 재설정될 수 있습니다.
* `ProfileEvents` ([Map(LowCardinality(String), UInt64)](/ko/sql-reference/data-types/map)) — 사용자의 다양한 메트릭을 측정하는 ProfileEvents의 요약입니다. 각 항목에 대한 설명은 `system.events` 테이블에서 확인할 수 있습니다.
