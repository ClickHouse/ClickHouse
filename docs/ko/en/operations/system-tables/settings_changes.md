---
description: '이전 ClickHouse 버전의 설정 변경 사항 정보를 담고 있는 시스템 테이블입니다.'
keywords: ['시스템 테이블', 'settings_changes']
slug: /operations/system-tables/settings_changes
title: 'system.settings_changes'
doc_type: 'reference'
---

<div id="description">
  ## 설명
</div>

이전 ClickHouse 버전의 설정 변경 사항 정보를 담고 있습니다.

<div id="columns">
  ## 컬럼
</div>

{/*AUTOGENERATED_START*/ }

* `type` ([Enum8(&#39;Session&#39; = 0, &#39;MergeTree&#39; = 1)](/ko/sql-reference/data-types/enum)) — 설정의 그룹입니다(Session, MergeTree...).
* `version` ([String](/ko/sql-reference/data-types/string)) — ClickHouse 서버의 버전입니다.
* `changes` ([Array(Tuple(name String, previous&#95;value String, new&#95;value String, reason String))](/ko/sql-reference/data-types/array)) — ClickHouse의 동작을 변경한 설정 변경 목록입니다.

{/*AUTOGENERATED_END*/ }

<div id="example">
  ## 예시
</div>

```sql
SELECT *
FROM system.settings_changes
WHERE version = '23.5'
FORMAT Vertical
```

```text
Row 1:
──────
type:    Core
version: 23.5
changes: [('input_format_parquet_preserve_order','1','0','Allow Parquet reader to reorder rows for better parallelism.'),('parallelize_output_from_storages','0','1','Allow parallelism when executing queries that read from file/url/s3/etc. This may reorder rows.'),('use_with_fill_by_sorting_prefix','0','1','Columns preceding WITH FILL columns in ORDER BY clause form sorting prefix. Rows with different values in sorting prefix are filled independently'),('output_format_parquet_compliant_nested_types','0','1','Change an internal field name in output Parquet file schema.')]
```

<div id="see-also">
  ## 관련 항목
</div>

* [설정](/ko/operations/system-tables/overview#system-tables-introduction)
* [system.settings](settings.md)