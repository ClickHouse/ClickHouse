---
description: '현재 세션에 정의된 가상(what-if) 인덱스를 나열하는 시스템 테이블'
...
현재 세션에 정의된 모든 가상(what-if) 스킵 인덱스를 나열합니다. [`CREATE HYPOTHETICAL INDEX`](/ko/sql-reference/statements/hypothetical-index#create-hypothetical-index) 및 [`EXPLAIN WHATIF`](/ko/sql-reference/statements/explain#explain-whatif)를 참조하십시오.

내용은 세션 범위로 관리됩니다. 각 connection에서는 해당 connection의 가상 인덱스만 볼 수 있으며, 현재 세션에서 인덱스를 생성하지 않았다면 이 테이블은 비어 있습니다.

현재 `(데이터베이스, 테이블)`은 쿼리 시점에 UUID를 기준으로 확인되므로 `RENAME TABLE`이 반영되며, 삭제된 테이블의 항목은 자동으로 숨겨집니다.
...
`type`은 기본 타입 이름이고 `type_full`에는 인수가 포함되므로, `bloom_filter(0.01)`와 `bloom_filter(0.001)` 같은 매개변수화된 변형을 서로 구분할 수 있습니다.

<div id="see-also">
  ## 관련 항목
</div>

* [`CREATE HYPOTHETICAL INDEX`](/ko/sql-reference/statements/hypothetical-index#create-hypothetical-index)
