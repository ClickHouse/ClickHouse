---
description: 'C++ 전문가와 ClickHouse 엔지니어에게 유용한 시스템 테이블로, `clickhouse` 바이너리의 내부 검사에 필요한 정보를 담고 있습니다.'
keywords: ['시스템 테이블', 'symbols']
...
`clickhouse` 바이너리를 내부 검사하는 데 필요한 정보를 포함합니다. 액세스하려면 내부 검사 권한이 필요합니다.
...
* `symbol` ([String](/ko/sql-reference/data-types/string)) — 바이너리 내 심볼 이름입니다. 맹글링된 상태이며, 사람이 읽을 수 있는 이름을 얻으려면 `demangle(symbol)`을 적용할 수 있습니다.
* `symbol_demangled` ([Nullable(String)](/ko/sql-reference/data-types/nullable)) — XRay 계측에 사용되는 디맹글 처리된 심볼입니다.
* `function_id` ([Nullable(Int32)](/ko/sql-reference/data-types/nullable)) — XRay 계측 맵에 있는 함수 ID입니다.
* `address_begin` ([UInt64](/ko/sql-reference/data-types/int-uint)) — 바이너리 내 심볼의 시작 주소입니다.
* `address_end` ([UInt64](/ko/sql-reference/data-types/int-uint)) — 바이너리 내 심볼의 끝 주소입니다.
