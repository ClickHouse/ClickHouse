---
description: '모든 서버 스레드의 스택 트레이스를 포함하는 시스템 테이블입니다. 개발자가
  서버 상태를 검사할 수 있습니다.'
...
스택 프레임을 분석하려면 `addressToLine`, `addressToLineWithInlines`, `addressToSymbol` 및 `demangle` [인트로스펙션 함수](../../sql-reference/functions/introspection.md)를 사용하십시오.
...
* `thread_name` ([String](/ko/sql-reference/data-types/string)) — 스레드 이름입니다.
* `thread_id` ([UInt64](/ko/sql-reference/data-types/int-uint)) — 스레드 식별자입니다.
* `query_id` ([String](/ko/sql-reference/data-types/string)) — 이 스레드가 속한 쿼리의 ID입니다.
* `trace` ([Array(UInt64)](/ko/sql-reference/data-types/array)) — 이 스레드의 스택트레이스입니다. 기본적으로 주소의 배열입니다.
* `untracked_memory` ([Int64](/ko/sql-reference/data-types/int-uint)) — 아직 상위 MemoryTracker에 전파되지 않은 메모리 할당을 위한 스레드별 비원자적 카운터입니다. 마지막 플러시 이후 할당된 양보다 해제된 양이 더 많으면 음수일 수 있습니다.
...
* [인트로스펙션 함수](../../sql-reference/functions/introspection.md) — 사용 가능한 인트로스펙션 함수와 사용 방법을 설명합니다.
* [system.trace&#95;log](../system-tables/trace_log.md) — 샘플링 쿼리 프로파일러가 수집한 스택 트레이스를 포함합니다.
* [arrayMap](/ko/sql-reference/functions/array-functions#arrayMap)) — `arrayMap` 함수의 설명과 사용 예시입니다.
* [arrayFilter](/ko/sql-reference/functions/array-functions#arrayFilter) — `arrayFilter` 함수의 설명과 사용 예시입니다.