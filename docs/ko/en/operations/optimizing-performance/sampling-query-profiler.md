---
description: 'ClickHouse의 샘플링 쿼리 프로파일러 도구 문서'
...
이 프로파일러를 사용하면 쿼리 실행 중 가장 자주 사용되는 소스 코드 루틴을 찾을 수 있습니다.
유휴 시간을 포함해 사용된 CPU 시간과 실제 경과 시간(wall-clock time)을 추적할 수 있습니다.

쿼리 프로파일러는 ClickHouse Cloud에서 자동으로 활성화됩니다.
다음 예시 쿼리는 프로파일링된 쿼리에서 함수 이름과 소스 위치를 해석한 가장 빈도가 높은 스택 트레이스를 찾습니다.

:::tip
`query_id` 값을 프로파일링하려는 쿼리의 ID로 바꾸십시오.
:::

<Tabs groupId="deployment">
  <TabItem value="cloud" label="ClickHouse Cloud">
    ClickHouse Cloud에서는 쿼리 결과 테이블 위 막대의 맨 오른쪽(테이블/chart 토글 옆)에 있는 **&quot;...&quot;**을 클릭해 쿼리 ID를 확인할 수 있습니다. 그러면 컨텍스트 메뉴가 열리며, 여기서 **&quot;Copy query ID&quot;**를 클릭할 수 있습니다.
...
자가 관리형 배포에서 쿼리 프로파일러를 사용하려면 다음 단계를 따르십시오.
...
<VerticalStepper headerLevel="h3">
  ...
  1. [&quot;Debian 리포지토리 설정&quot;](/ko/install/debian_ubuntu#setup-the-debian-repository) 단계의 지침을 따르십시오
  2. 디버그 정보가 포함된 컴파일된 ClickHouse 바이너리 파일을 설치하려면 `sudo apt-get install clickhouse-server clickhouse-client clickhouse-common-static-dbg`를 실행하십시오
  3. 서버를 시작하려면 `sudo service clickhouse-server start`를 실행하십시오
  4. `clickhouse-client`를 실행하십시오. `clickhouse-common-static-dbg`의 디버그 심볼은 서버에서 자동으로 인식되므로 별도로 활성화할 필요가 없습니다

  ### 서버 구성 확인

  [서버 구성 파일](/ko/operations/configuration-files)의 [`trace_log`](../../operations/server-configuration-parameters/settings.md#trace_log) 섹션이 설정되어 있는지 확인하십시오. 기본적으로 활성화되어 있습니다:
  ...
  <trace_log>
      ...
      <!-- 크래시 발생 시 로그를 디스크에 기록할지 여부를 나타냅니다 -->
      <flush_on_crash>false</flush_on_crash>
      <symbolize>true</symbolize>
  </trace_log>
  ```

  이 섹션은 프로파일러 동작 결과가 저장되는 [trace&#95;log](/ko/operations/system-tables/trace_log) 시스템 테이블을 구성합니다.
  이 테이블의 데이터는 서버가 실행 중일 때만 유효하다는 점에 유의하십시오.
  서버를 재시작한 후에는 ClickHouse가 이 테이블을 정리하지 않으므로, 저장된 가상 메모리 주소가 모두 더 이상 유효하지 않을 수 있습니다.
  ...
  이 설정으로 프로파일러 타이머를 구성할 수 있습니다.
  이들은 세션 설정이므로 전체 서버, 개별 사용자 또는 사용자 프로파일, 대화형 세션, 그리고 개별 쿼리마다 서로 다른 샘플링 주기를 적용할 수 있습니다.

  기본 샘플링 주기는 초당 1개 샘플이며, CPU 타이머와 실제 시간 타이머가 모두 활성화되어 있습니다.
  이 주기에서는 서버 성능에 영향을 주지 않으면서 ClickHouse 클러스터에 대한 충분한 정보를 수집할 수 있습니다.
  개별 쿼리 각각을 프로파일링해야 한다면 더 높은 샘플링 주기를 사용하십시오.
  ...
  ClickHouse 코드에서 함수 이름과 해당 위치를 확인하려면 `addressToLine`, `addressToLineWithInlines`, `addressToSymbol`, `demangle` [인트로스펙션 함수](../../sql-reference/functions/introspection.md)를 사용하십시오.
  특정 쿼리의 프로파일을 얻으려면 `trace_log` 테이블의 데이터를 집계해야 합니다.
  데이터는 개별 함수별로 또는 전체 스택 트레이스 기준으로 집계할 수 있습니다.

  :::tip
  `trace_log` 정보를 시각화해야 한다면 [flamegraph](/ko/interfaces/third-party/gui#clickhouse-flamegraph)와 [speedscope](https://www.speedscope.app)를 사용해 보십시오.
  :::
</VerticalStepper>

<div id="flamegraph">
  ## `flameGraph` 함수로 플레임 그래프 생성하기
</div>

ClickHouse는 `trace_log`에 저장된 스택 트레이스로부터 직접 플레임 그래프를 생성하는 [`flameGraph`](/ko/sql-reference/aggregate-functions/reference/flame_graph) 집계 함수를 제공합니다.
...
* `traces` — 스택 트레이스입니다. [`Array(UInt64)`](/ko/sql-reference/data-types/array).
* `size` — 메모리 프로파일링을 위한 할당 크기입니다. [`Int64`](/ko/sql-reference/data-types/int-uint).
* `ptr` — 할당 주소입니다. [`UInt64`](/ko/sql-reference/data-types/int-uint).
...
다음 쿼리에서 `flamegraph.pl`을 사용자 시스템에 있는 `flamegraph.pl`의 경로로 대체하십시오
:::
...
<div id="memory-flame-graph-unfreed">
  ### 메모리 플레임 그래프 — 해제되지 않은 메모리 할당
</div>

이 변형은 포인터를 기준으로 메모리 할당과 메모리 해제를 대조하여, 쿼리 중 해제되지 않은 메모리만 표시합니다.
...
이 방법을 사용하면 최대 메모리 사용량을 찾고 해당 시점에 무엇이 할당되었는지 시각화할 수 있습니다.
...
<div id="build-flame-graph-deallocations">
  #### 해당 시점 이후에 발생한 메모리 해제의 플레임 그래프 생성(나중에 무엇이 해제되었는지 파악하기 위해)
</div>
...
* 스택 트레이스별로 집계합니다.
* 다음에 대한 보고서를 얻기 위해 인트로스펙션 함수를 사용합니다.
  * 심볼 이름과 이에 해당하는 소스 코드 함수 이름
  * 이러한 함수의 소스 코드 위치
