...
이 인터페이스를 사용하면 웹 브라우저 또는 HTTP 클라이언트를 통해 클러스터 상태를 확인하고, 명령을 실행하고, Keeper 스토리지를 관리할 수 있습니다.
...
| `http_control.readiness.endpoint`         | `/ready` | 준비 상태 프로브용 사용자 지정 경로     |
| `http_control.storage.session_timeout_ms` | `30000`  | storage API 작업의 세션 시간 제한 |
...
Storage API는 HTTP 메서드로 작업 유형을 나타내는 REST 규칙을 따릅니다:

| 작업       | 경로                                     | 메서드    | 상태 코드 | 설명           |
| -------- | -------------------------------------- | ------ | ----- | ------------ |
| 가져오기     | `/api/v1/storage/{path}`               | GET    | 200   | 노드 데이터 가져오기  |
| 목록       | `/api/v1/storage/{path}?children=true` | GET    | 200   | 하위 노드 나열     |
| 존재 여부 확인 | `/api/v1/storage/{path}`               | HEAD   | 200   | 노드가 존재하는지 확인 |
