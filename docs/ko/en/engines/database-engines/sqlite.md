...
description: 'SQLite 데이터베이스에 연결하고 `INSERT` 및 `SELECT`
  쿼리를 수행하여 ClickHouse와 SQLite 간에 데이터를 주고받을 수 있습니다.'
...
[SQLite](https://www.sqlite.org/index.html) 데이터베이스에 연결하고 `INSERT` 및 `SELECT` 쿼리를 수행하여 ClickHouse와 SQLite 간에 데이터를 주고받을 수 있습니다.
...
* `db_path` — SQLite 데이터베이스가 저장된 파일의 경로.
...
[SQLite table engine](../../engines/table-engines/integrations/sqlite.md)을 사용해 테이블을 특정 ClickHouse 타입으로 명시적으로 정의하면, SQLite의 TEXT 컬럼에서 다음 ClickHouse 타입을 파싱할 수 있습니다:
...
SQLite는 동적 타이핑을 사용하며, 타입 접근 함수는 자동으로 타입을 강제 변환합니다. 예를 들어 TEXT 컬럼을 정수로 읽을 때 텍스트를 숫자로 파싱할 수 없으면 0을 반환합니다. 즉, ClickHouse 테이블이 실제 SQLite 컬럼과 다른 타입으로 정의되어 있으면 오류가 발생하는 대신 값이 별다른 경고 없이 강제 변환될 수 있습니다.

<div id="specifics-and-recommendations">
  ## 세부 사항 및 권장 사항
</div>

SQLite는 전체 데이터베이스(정의, 테이블, 인덱스, 데이터 자체)를 호스트 머신의 단일 크로스 플랫폼 파일로 저장합니다. SQLite는 쓰기 중에 전체 데이터베이스 파일을 잠그므로 쓰기 작업은 순차적으로 수행됩니다. 읽기 작업은 동시에 여러 개 수행할 수 있습니다.
SQLite는 서비스 관리(예: 시작 스크립트)나 `GRANT` 및 비밀번호 기반의 액세스 제어를 필요로 하지 않습니다. 액세스 제어는 데이터베이스 파일 자체에 부여된 파일 시스템 권한을 통해 처리됩니다.
