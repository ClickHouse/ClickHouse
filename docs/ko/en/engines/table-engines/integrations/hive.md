---
description: 'Hive 엔진을 사용하면 HDFS Hive 테이블에서 `SELECT` 쿼리를 실행할 수 있습니다.'
sidebar_label: 'Hive'
sidebar_position: 84
...
* ORC: `char`를 제외한 단순 스칼라 컬럼 타입을 지원하고, 복합 타입은 `array`와 같은 타입만 지원합니다

* Parquet: 모든 단순 스칼라 컬럼 타입을 지원하고, 복합 타입은 `array`와 같은 타입만 지원합니다
...
[CREATE TABLE](/ko/sql-reference/statements/create/table) 쿼리에 대한 자세한 설명은 참조하십시오.

테이블 구조는 원래 Hive 테이블의 구조와 다를 수 있습니다.

* 컬럼 이름은 원래 Hive 테이블과 동일해야 하지만, 이들 컬럼 중 일부만 사용하거나 순서를 임의로 지정할 수 있으며, 다른 컬럼으로부터 계산된 별칭 컬럼도 사용할 수 있습니다.
* 컬럼 타입은 원래 Hive 테이블과 동일해야 합니다.
* PARTITION BY 표현식은 원래 Hive 테이블과 일관되어야 하며, PARTITION BY 표현식에 사용된 컬럼은 테이블 구조에 포함되어야 합니다.
...
캐시를 사용하기 전에 `config.xml`에 해당 구성을 추가하십시오.
...
* enable: true로 설정하면 ClickHouse가 시작된 후 원격 파일 시스템(HDFS)에 대한 로컬 캐시를 유지합니다.
* root&#95;dir: 필수입니다. 원격 파일 시스템용 로컬 캐시 파일을 저장할 루트 디렉터리입니다.
* limit&#95;size: 필수입니다. 로컬 캐시 파일의 최대 크기(바이트 단위)입니다.
* bytes&#95;read&#95;before&#95;flush: 원격 파일 시스템에서 파일을 다운로드할 때 로컬 파일 시스템으로 플러시하기 전에 읽을 바이트 수를 제어합니다. 기본값은 1MB입니다.
...
위에서 생성한 Hive 테이블에서 데이터를 가져오는 ClickHouse 테이블입니다:
...
위에서 생성한 Hive 테이블에서 데이터를 가져오는 ClickHouse 테이블은 다음과 같습니다:
