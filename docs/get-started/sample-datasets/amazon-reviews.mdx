...
The commands below run as-is on any ClickHouse Cloud service. On a self-managed server, one of them needs a small change, noted at that step.
...
3. The following `INSERT` command uses the [`s3Cluster`](/reference/functions/table-functions/s3Cluster) table function, which allows the processing of multiple S3 files in parallel using all the nodes of your cluster. We also use a wildcard to insert any file that starts with the name `https://datasets-documentation.s3.eu-west-3.amazonaws.com/amazon_reviews/amazon_reviews_*.snappy.parquet`:
...
<Tip>
Every ClickHouse Cloud service has a cluster named `default`. On a self-managed server, run `SELECT cluster FROM system.clusters` to see which clusters are available and use one of those names. If the result is empty, use the [`s3`](/reference/functions/table-functions/s3) table function instead.
</Tip>
