...
<sub><span>*</span> ClickHouse additionally supports unsigned integers with extended ranges i.e. <a href='/sql-reference/data-types/int-uint'>`UInt8`, `UInt32`, `UInt32` and `UInt64`</a>.</sub><br />
<sub><span>**</span>ClickHouse’s String type is unlimited by default but can be constrained to specific lengths using <a href='/sql-reference/statements/create/table#constraints'>Constraints</a>.</sub>
