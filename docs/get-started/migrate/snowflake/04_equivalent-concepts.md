...
| Schema `OWNERSHIP` and `MANAGED ACCESS` (by default the role with `OWNERSHIP` holds grant authority on its objects; `WITH MANAGED ACCESS` centralizes that authority on the schema owner instead) | — | ClickHouse has no object-ownership model, so grants are always explicit. |
...
| Warehouse size (XS through 6X-Large) | Vertical [autoscaling](/cloud/features/autoscaling/vertical) bounds — replica memory and CPU | Sizing is configured as min/max memory and CPU bounds rather than discrete t-shirt sizes; setting min = max effectively fixes the size. |
| Multi-cluster warehouse | Manual [horizontal scaling](/cloud/features/autoscaling/horizontal) — replica count configured via API or console | Both add parallel compute to handle concurrency; ClickHouse scales replica count rather than cluster count. There's no direct equivalent to Snowflake's auto-scaling policies (`Standard`/`Economy`); horizontal replica count is set manually. |
...
| Iceberg table (managed or unmanaged) | [Iceberg engine](/engines/table-engines/integrations/iceberg) | Reads Iceberg tables stored in S3, Azure, HDFS, or local storage. See the engine page for the current list of supported features. |
...
| Network policies (IP allowlist) | IP allowlists and [private connectivity](/cloud/security/connectivity/private-networking) — PrivateLink (AWS, Azure) and Private Service Connect (GCP) for ingress restriction | Both restrict network ingress; ClickHouse adds private connectivity across the three major clouds. |
...
| Database replication / Account replication / Failover Groups (Snowgrid) | In-region high availability via multiple replicas (managed by Cloud); cross-region resiliency via [backups](/cloud/manage/backups/overview) and the Enterprise tier's advanced DR features | Snowgrid is the underlying Snowflake fabric powering cross-region replication, global data sharing, and Failover Groups. On ClickHouse Cloud, replicas inside a service provide in-region HA; cross-region DR is handled at the backup and service level rather than a single replication primitive. |
