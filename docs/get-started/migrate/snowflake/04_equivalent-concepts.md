...
| Snowpipe Streaming | [ClickPipes](/integrations/clickpipes) for Kafka, Kinesis, Pub/Sub, MongoDB, MySQL, Postgres, and BigQuery | Managed low-latency streaming and CDC ingest. |
...
| Cortex Analyst | [ClickHouse Agents](/cloud/features/ai-ml/agents) in the cloud console | Agents (currently in beta) are conversational: natural-language queries against your data with tool calls and chat workflows. Check the [Agents](/cloud/features/ai-ml/agents) page for the current capability surface. |
...
| Database replication / Account replication / Failover Groups (Snowgrid) | In-region high availability via multiple replicas (managed by Cloud); cross-region resiliency via [backups](/cloud/manage/backups/overview); see [Disaster recovery](/cloud/data-resiliency) | ClickHouse Cloud has no automatic cross-region replication, failover, or active-active sync today. Migrating from a Failover Groups deployment means restructuring DR around scheduled [backups](/cloud/manage/backups/configurable-backups) (configurable to every 6 hours, exportable cross-region to a customer-owned bucket) and manual restore into a new service. RPO becomes the backup interval; there's no replicated standby to fail over to. |
