...
| Dynamic table | [Refreshable MV](/materialized-view/refreshable-materialized-view) | Refreshable MVs run on a cron-style schedule; same end-state as a target-lag dynamic table. |
| Result cache | [Query cache](/operations/query-cache) | ClickHouse's query cache is per-replica and not transactionally consistent. |
...
| Cortex LLM functions (`CORTEX.COMPLETE`, `CORTEX.SUMMARIZE`, etc.) | No in-SQL equivalent — call LLM providers from the application layer or an orchestrator and write results back to ClickHouse | Ask-AI in the docs and console is a docs/console helper, not a SQL surface. |
| Cortex Analyst | — | ClickHouse has no in-product NL-to-SQL service. |
...
| Database replication / Account replication / Failover Groups (Snowgrid) | In-region high availability via multiple replicas (managed by Cloud); cross-region resiliency via [backups](/cloud/manage/backups/overview) — see [Disaster recovery](/cloud/data-resiliency) | Cross-region DR is handled at the backup and service level rather than via a single replication primitive. |
