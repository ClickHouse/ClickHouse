...
Correspondant [DDL-requête](../create.md#create-dictionary-query):
...
Ce type de stockage est pour une utilisation avec composite [touches](external_dicts_dict_structure.md). Semblable à `hashed`.
...
Soutenu [source](external_dicts_dict_sources.md): MySQL, ClickHouse, exécutable, HTTP.
...
Ce type de stockage est pour une utilisation avec composite [touches](external_dicts_dict_structure.md). Semblable à `cache`.
