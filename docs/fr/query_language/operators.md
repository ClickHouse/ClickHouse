...
*Voir [Dans les opérateurs](select.md#select-in-operators).*
...
Le `date` paramètre spécifie la date ou l'heure à traiter. Soit [Date](../data_types/date.md) ou [DateTime](../data_types/datetime.md) le type est pris en charge.
...
Crée un [Intervalle](../data_types/special_data_types/interval.md)- valeur de type qui doit être utilisée dans les opérations arithmétiques avec [Date](../data_types/date.md) et [DateTime](../data_types/datetime.md)-type de valeurs.
...
-   [Intervalle](../data_types/special_data_types/interval.md) type de données
-   [toInterval](functions/type_conversion_functions.md#function-tointerval) type fonctions de conversion
...
L'opérateur conditionnel calcule les valeurs de b et c, puis vérifie si la condition a est remplie, puis renvoie la valeur correspondante. Si `b` ou `C` est un [arrayJoin()](functions/array_join.md#functions_arrayjoin) fonction, chaque ligne sera répliquée indépendamment de la “a” condition.
...
-   Pour [Nullable](../data_types/nullable.md) type de valeurs, l' `IS NULL` opérateur retourne:
...
-   Pour [Nullable](../data_types/nullable.md) type de valeurs, l' `IS NOT NULL` opérateur retourne:
