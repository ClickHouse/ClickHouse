...
Chaque bloc d’insertion couvre une plage contiguë d’offsets et porte un [jeton de déduplication](/fr/guides/developer/deduplicating-inserts-on-retries) déterministe de la forme `topic:partition:firstOffset-lastOffset`. Lors d’une relecture, ClickPipes reproduit la même plage d’offsets et donc le même jeton, de sorte que ClickHouse rejette le doublon. Comme le jeton dépend uniquement de la plage d’offsets, une relecture est dédupliquée même lorsque le bloc reconstruit n’est pas identique octet pour octet.
