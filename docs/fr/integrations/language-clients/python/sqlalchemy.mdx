...
La construction standard SQLAlchemy `values()` est compilée vers la syntaxe de fonction de table `VALUES` de ClickHouse, y compris lorsqu&#39;elle est utilisée dans une expression de table commune (la forme CTE nécessite SQLAlchemy 2.x, où `Values.cte()` a été ajoutée).
