...
<div id="open-the-tablumio-startup-page">
  ## Ouvrez la page d’accueil de TABLUM.IO
</div>
...
<div id="1-sign-up-or-sign-in-to-the-service">
  ## 1. Inscrivez-vous ou connectez-vous au service
</div>

Commencez par vous inscrire à TABLUM.IO avec votre adresse e-mail, ou utilisez la connexion rapide via vos comptes Google ou Facebook.
...
<div id="2-add-a-clickhouse-connector">
  ## 2. Ajouter un connecteur ClickHouse
</div>
...
Accédez à l’onglet **Dataset**. Dans la liste déroulante, sélectionnez le connecteur ClickHouse que vous venez de créer. Dans le panneau de droite, vous verrez la liste des tables et des schémas disponibles.
...
<div id="4-input-a-sql-query-and-run-it">
  ## 4. Saisissez une requête SQL et exécutez-la
</div>

Saisissez une requête dans la SQL Console et cliquez sur **Run Query**. Les résultats s’afficheront sous forme de tableau.

<Tip>
  Cliquez avec le bouton droit sur le nom de la colonne pour ouvrir le menu déroulant proposant des options de tri, de filtrage et d’autres actions.
</Tip>
