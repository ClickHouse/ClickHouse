...
<Accordion title="CHOps">
  ...
  * Éditeur SQL avec saisie semi-automatique, types `EXPLAIN`, export des résultats dans 22 formats, profileur de requêtes et chronologie des métriques de requête.
...
<Accordion title="HouseOps">
  [HouseOps](https://github.com/HouseOps/HouseOps) est une interface utilisateur/un IDE pour OSX, Linux et Windows.

  Fonctionnalités :

  * Générateur de requêtes avec coloration syntaxique. Affichage des réponses sous forme de tableau ou en JSON.
  * Export des résultats de requête au format CSV ou JSON.
  * Liste des processus avec descriptions. Mode écriture. Possibilité d’arrêter un processus (`KILL`).
...
<Accordion title="Redash">
  ...
  Redash prend en charge plusieurs sources de données, dont ClickHouse, et peut joindre les résultats de requêtes provenant de différentes sources dans un seul jeu de données final.
...
<Accordion title="Grafana">
  ...
  « Grafana vous permet d’interroger, de visualiser, d’alerter sur et de comprendre vos métriques, quel que soit leur emplacement de stockage. Créez, explorez et partagez des tableaux de bord avec votre équipe et favorisez une culture axée sur les données. Apprécié et plébiscité par la communauté » — grafana.com.

  Le plugin de source de données ClickHouse prend en charge ClickHouse comme base de données dorsale.
</Accordion>
...
<Accordion title="ClickHouse table graph">
  [ClickHouse table graph](https://github.com/mbaksheev/clickhouse-table-graph) est un outil CLI simple qui permet de visualiser les dépendances entre les tables ClickHouse. Il récupère les connexions entre les tables à partir de la table `system.tables` et génère un diagramme de flux des dépendances au format [mermaid](https://mermaid.js.org/syntax/flowchart.html). Cet outil vous permet de visualiser facilement les dépendances entre les tables et de comprendre le flux de données dans votre base de données ClickHouse. Grâce à mermaid, le diagramme obtenu est attrayant et peut être facilement ajouté à votre documentation Markdown.
</Accordion>

<Accordion title="xeus-clickhouse">
  [xeus-clickhouse](https://github.com/wangfenjin/xeus-clickhouse) est un noyau Jupyter pour ClickHouse qui prend en charge l’interrogation des données CH à l’aide de SQL dans Jupyter.
</Accordion>

<Accordion title="MindsDB Studio">
  [MindsDB](https://mindsdb.com/) est une couche d’IA open source pour les bases de données, y compris ClickHouse, qui vous permet de développer, d’entraîner et de déployer facilement des modèles de machine learning de pointe. MindsDB Studio (GUI) vous permet d’entraîner de nouveaux modèles à partir de bases de données, d’interpréter les prédictions du modèle, d’identifier les biais potentiels dans les données, ainsi que d’évaluer et de visualiser la précision des modèles grâce à la fonction d’IA explicable afin d’adapter et d’ajuster plus rapidement vos modèles de machine learning.
</Accordion>
...
<Accordion title="DBM">
  ...
  * Prise en charge des requêtes portant sur des clauses SQL sélectionnées
...
<Accordion title="clickhouse-monitoring">
  [clickhouse-monitoring](https://github.com/duyet/clickhouse-monitoring) est un tableau de bord Next.js simple qui s’appuie sur les tables `system.*` pour vous aider à surveiller votre cluster ClickHouse et en fournir une vue d’ensemble.

  Fonctionnalités :

  * Moniteur de requêtes : requêtes en cours, historique des requêtes, ressources utilisées par les requêtes (mémoire, parties lues, file&#95;open, ...), requêtes les plus coûteuses, tables ou colonnes les plus utilisées, etc.
  * Surveillance du cluster : utilisation totale de la mémoire et du CPU, file d’attente distribuée, paramètres globaux, paramètres MergeTree, métriques, etc.
  ...
  * Outils utiles : exploration des données ZooKeeper, EXPLAIN de requêtes, arrêt de requêtes, etc.
...
<Accordion title="CKibana">
  [CKibana](https://github.com/TongchengOpenSource/ckibana) est un service léger qui vous permet de rechercher, d’explorer et de visualiser facilement les données ClickHouse à l’aide de l’UI Kibana native.

  Fonctionnalités :

  * Traduit les demandes de graphiques de l’UI Kibana native en syntaxe de requête ClickHouse.
  ...
  * Réduit la courbe d’apprentissage pour les utilisateurs migrant d’ElasticSearch vers ClickHouse.
</Accordion>

<Accordion title="Telescope">
  [Telescope](https://iamtelescope.net/) est une interface web moderne permettant d’explorer les logs stockés dans ClickHouse. Elle fournit une UI conviviale pour interroger, visualiser et gérer les données de logs avec un contrôle d’accès précis.
  ...
  * Graphique temporel prenant en charge le regroupement, y compris pour les champs JSON imbriqués, Map et Array.
  * Prise en charge facultative de clauses SQL `WHERE` brutes pour un filtrage avancé (avec vérification des permissions).
  * Vues enregistrées : enregistrez et partagez des configurations d’UI personnalisées pour les requêtes et la mise en page.
  * Contrôle d’accès basé sur les rôles (RBAC) et intégration de l’authentification GitHub.
  * Aucun agent ni composant supplémentaire requis côté ClickHouse.
...
<Accordion title="Chadmin">
  [Chadmin](https://github.com/bun4uk/chadmin) est une interface utilisateur simple permettant de visualiser les requêtes en cours d’exécution sur votre cluster ClickHouse, ainsi que les informations les concernant, et de les arrêter si nécessaire.
</Accordion>
