...
<Note>
  **dbt Core 1.x, dbt Core v2 et la plateforme dbt.** Toutes les commandes et tous les models de ce guide sont identiques dans les trois cas. Les exemples ont été testés avec dbt Core 1.12 et `dbt-clickhouse` 1.10, ainsi qu&#39;avec dbt Core v2 2.0 sur ClickHouse 26.8 ; la plateforme dbt s&#39;appuie sur le moteur dbt Core v2. La sortie de console présentée provient de dbt Core 1.x, et les quelques cas où les moteurs se comportent différemment sont signalés. Consultez la [page dbt Core v2](/fr/integrations/connectors/data-ingestion/etl-tools/dbt/dbt-core-v2-fusion-and-platform) pour connaître l&#39;état actuel de l&#39;adapter v2, et [Connect ClickHouse](https://docs.getdbt.com/docs/platform/connect-data-platform/connect-clickhouse) dans la documentation dbt pour faire vos premier pas sur la plateforme dbt.
</Note>
...
| Valeur CSV            | dbt Core 1.x | dbt Core v2     |
| --------------------- | ------------ | --------------- |
| `700`                 | `Int32`      | `Int64`         |
...
`dbt seed --full-refresh` supprime et recrée la table ; exécutez donc cette commande avant de construire quoi que ce soit qui dépende directement des données de cette table, comme la materialized view présentée plus loin dans ce guide.
