...
À partir de la version 26.9 de ClickHouse, ce schéma est pris en charge par défaut. Dans les versions 26.1 à 26.8, il nécessite le paramètre `allow_experimental_nullable_tuple_type = 1` (voir [cette page de la documentation](/fr/reference/settings/session-settings/allow-experimental#allow_experimental_nullable_tuple_type)).
