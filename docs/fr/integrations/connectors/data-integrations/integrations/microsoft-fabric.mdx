...
<div id="copying-data-from-onelake">
  ## Copie de données depuis OneLake
</div>

Le flux Copy &amp; Sync permet de copier des tables OneLake dans ClickHouse :

1. Ouvrez votre élément ClickHouse et sélectionnez **Copy &amp; Sync**
2. Choisissez un Lakehouse dans votre espace de travail
3. Sélectionnez les tables que vous souhaitez accélérer
4. Lancez la copie d’un instantané ponctuel de vos données

<Frame>
  ![Configuration de Copy & Sync créant une nouvelle table de destination avec une clé de tri](/images/integrations/tools/data-integration/microsoft-fabric/fabric-03-copy-sync-table-selection.webp)
</Frame>
...
<Frame>
  ![Mappage de colonnes Copy & Sync depuis une table source OneLake vers une table ClickHouse existante](/images/integrations/tools/data-integration/microsoft-fabric/fabric-04-copy-sync-existing-table.webp)
</Frame>
...
<Note>
  Dans l’aperçu public, seule la copie d’un instantané ponctuel est prise en charge. D’autres options de réplication sont prévues dans de prochaines versions.
</Note>
...
Sélectionnez **Open ClickHouse** pour ouvrir la console ClickHouse Cloud complète en vous connectant avec votre identité Microsoft. Vous pouvez copier des données dans ClickHouse et exécuter toute instruction SQL depuis la charge de travail Fabric, mais pour les tâches administratives, vous devrez accéder à la Cloud Console afin de gérer le service.
...
Chaque opération Copy &amp; Sync depuis OneLake s’exécute sous la forme d’un ClickPipe. Suivez la progression de la copie, le débit et les erreurs sur la [page ClickPipes](/fr/integrations/clickpipes/monitoring) de la console ClickHouse Cloud.
...
2. Ouvrez **Copy &amp; Sync**, choisissez votre Lakehouse et sélectionnez la table `publicholidays`
...
* **Tables Lakehouse uniquement.** Copy &amp; Sync lit les tables Delta d’un Lakehouse situé dans le même espace de travail. Les autres éléments Fabric (Warehouse, bases de données KQL, bases de données mises en miroir) ne sont pas encore pris en charge comme sources.
...
* **Les changements de rôle prennent effet à la prochaine ouverture.** Votre rôle au sein de l’organisation ClickHouse est associé à votre rôle dans l’espace de travail Fabric lorsque vous ouvrez un élément ClickHouse. Fabric ne peut pas informer ClickHouse des changements de rôle. Un changement de rôle dans l’espace de travail prend donc effet lors de la prochaine ouverture de l’élément, et non immédiatement.
...
* **Le SSO de la Console nécessite une adresse e-mail dans votre profil Entra.** Les utilisateurs de ClickHouse Cloud doivent disposer d’une adresse e-mail ; si aucune adresse n’est renseignée dans votre profil Microsoft Entra, la connexion à la console Cloud échouera.
