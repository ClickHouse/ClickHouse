...
Ce guide vous explique comment déployer un collector OpenTelemetry sur un service Managed ClickStack existant, ou adapter votre collector existant, puis vérifier que les données circulent correctement.

<AgentPrompt
  prompt="Utilisez curl pour télécharger, lire et suivre : https://clickhouse.com/docs/skills/clickstack-otel-collector/SKILL.md"
  description="Votre agent configurera le collector OpenTelemetry ClickStack pour vous. Fonctionne avec Claude Code, Cursor, Codex et d'autres agents de développement."
  outline={[
...
"Exécutez localement le collector OpenTelemetry ClickStack dans Docker, pointé vers votre service.",
...
<Info>
  **Collector existant**
  ...
  Si vous utilisez un collector OpenTelemetry existant, nous partons du principe qu&#39;il est déjà configuré dans un rôle de **gateway**. Nous ne recommandons pas d&#39;utiliser ce processus pour reconfigurer des collectors dans le rôle **agent**.
</Info>
...
<Tabs>
  <Tab title="Je n’ai pas de collector">
    <Steps>
      ...
      <Step>
        ## Vérifiez dans l’interface ClickStack
  ...
  <Tab title="J’ai un collector">
    <Steps>
      ...
      <Step>
        ## Adaptez la configuration de votre collector
      ...
      <Step>
        ...
        Définissez un petit helper `tg` afin que chaque commande ne précise que ce qui varie (service, sévérité, status, attributes) :
        ...
        Les logs seuls confirment que l&#39;endpoint fonctionne. Pour un jeu de données de démonstration plus riche, une Service Map multi-services et des graphiques couvrant différents types de métriques, développez et exécutez également les commandes ci-dessous. Celles-ci réutilisent l&#39;utilitaire `tg` ; exécutez-les donc dans le même shell.

        <Accordion title="Générez une télémétrie plus détaillée (facultatif)">
          Envoyez des traces comportant plusieurs spans provenant de plusieurs services sains, ainsi que d’une dépendance défaillante. Cela donne à la Service Map une forme réaliste, globalement saine avec un service en erreur, tout en alimentant les vues d’erreur :
          ...
          Pour varier davantage, ajoutez quelques services supplémentaires ainsi qu’un histogramme exponentiel :
        ...
        Pour la liste complète des options et plus de variantes, consultez [Données synthétiques avec telemetrygen](/fr/clickstack/example-datasets/telemetrygen).
      </Step>

      <Step>
        ## Vérifiez dans la ClickStack UI
