...
Lors de la création ou de la mise à jour de tableaux de bord via l’API externe (`POST` / `PUT` `/api/v2/dashboards`), définissez `color` sur un token de palette dans la config du builder d’une tuile Number. Les valeurs hex brutes sont rejetées.
...
`color` accepte n’importe quel token des listes catégorielles et sémantiques ci-dessus (par exemple `chart-blue` ou `chart-success`). Le paramètre facultatif `backgroundChart.color` remplace la couleur du sparkline par le même enum de token. Consultez la [référence de l’API ClickStack](/fr/use-cases/observability/clickstack/api-reference) pour l’authentification et les URL de base.
