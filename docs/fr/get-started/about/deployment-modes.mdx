...
[chDB](/chdb/index) est ClickHouse intégré sous forme de moteur de base de données in-process, Python en étant l’implémentation principale, bien qu’il soit également disponible pour Go, Rust, NodeJS et Bun. Cette option de déploiement intègre directement les puissantes capacités OLAP de ClickHouse au processus de votre application, sans nécessiter d’installation distincte d’une base de données.
