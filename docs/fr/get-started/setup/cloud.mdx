...
<Steps>
  ...
  <Step title="Ajouter des données" id="3-add-data">
    ...
    * Téléverser des données depuis l’URL d’un fichier
    ...
    [ClickPipes](/fr/integrations/clickpipes/home) est une plateforme d&#39;intégration gérée qui rend l&#39;ingestion de données depuis un ensemble diversifié de sources aussi simple que de cliquer sur quelques boutons. Conçue pour les charges de travail les plus exigeantes, l&#39;architecture robuste et évolutive de ClickPipes assure des performances et une fiabilité constantes. ClickPipes peut être utilisé pour des besoins de streaming à long terme ou pour un chargement de données ponctuel.
    ...
    * dans ClickHouse, les clés primaires ne sont ***pas uniques*** pour chaque ligne d’une table
    ...
    La clé primaire est également la clé de tri, qui est un tuple `(user_id, timestamp)`. Par conséquent, les données stockées dans chaque
    fichier de colonne seront triées par `user_id`, puis par `timestamp`.

    Pour une présentation approfondie des concepts fondamentaux de ClickHouse, consultez [&quot;Concepts fondamentaux&quot;](/fr/concepts/core-concepts/index).
    ...
    Une tâche courante lorsque l&#39;on commence à utiliser une base de données consiste à insérer des données que vous avez déjà dans des fichiers. Nous proposons des données
    d&#39;exemple en ligne que vous pouvez insérer, représentant des données de clickstream : elles comprennent un identifiant utilisateur, une URL visitée et
...
* Le [Tutoriel](/fr/get-started/quickstarts/tutorial) vous explique comment insérer 2 millions de lignes dans une table et écrire quelques requêtes analytiques
