...
<Steps>
  ...
  <Step title="Ajouter des données" id="3-add-data">
    ...
    * Téléversez des données à partir de l’URL d’un fichier
    ...
    [ClickPipes](/fr/integrations/clickpipes/home) est une plateforme d&#39;intégration managée qui rend l&#39;ingestion de données depuis un large éventail de sources aussi simple que quelques clics. Conçue pour les workloads les plus exigeants, l&#39;architecture robuste et évolutive de ClickPipes garantit des performances et une fiabilité constantes. ClickPipes peut être utilisé aussi bien pour des besoins de streaming à long terme que pour un job de chargement de données ponctuel.
    ...
    Exécutez la commande suivante pour créer une table nommée `my_first_table` dans la database `helloworld` :
    ...
    * `metric` : un nombre à virgule flottante de 32 bits ([Float32](/fr/reference/data-types/float))
    ...
    Avant d&#39;aller plus loin, il est important de comprendre le fonctionnement des clés primaires dans ClickHouse (leur
    implémentation peut sembler surprenante !) :

    * les clés primaires dans ClickHouse ne sont ***pas uniques*** pour chaque ligne d’une table

    La clé primaire d&#39;une table ClickHouse détermine la manière dont les données sont triées lors de leur écriture sur disque. Toutes les 8 192 lignes ou tous les 10 Mo de
    données (ce que l&#39;on appelle l&#39;**index granularity**) donnent lieu à une entrée dans le fichier d&#39;index de clé primaire. Ce concept de granularité
    produit un **index sparse** qui tient aisément en mémoire, et les granules constituent une bande correspondant à la plus petite quantité de
    ...
    la clé correspond alors au tuple indiqué dans la clause `ORDER BY`. Si vous spécifiez à la fois une `PRIMARY KEY` et un `ORDER BY`, la clé primaire doit être un sous-ensemble de l&#39;ordre de tri.

    La clé primaire est également la clé de tri, à savoir le tuple `(user_id, timestamp)`. Par conséquent, les données stockées dans chaque
    ...
    Avec ClickHouse, vous pouvez utiliser la commande [`INSERT INTO TABLE`](/fr/reference/statements/insert-into), qui vous est familière, mais il est important de comprendre que chaque insert dans une table [`MergeTree`](/fr/reference/engines/table-engines/mergetree-family/mergetree) entraîne la création d&#39;une **part** dans le stockage.
    ...
    Remarquez que la réponse est renvoyée dans un joli format de table :
    ...
    Une tâche courante lorsque l&#39;on débute avec une base de données consiste à insérer des données que vous possédez déjà dans des fichiers. Nous mettons à disposition en ligne des
    données d&#39;exemple que vous pouvez insérer et qui représentent des données de flux de clics : elles comprennent un ID utilisateur, une URL visitée et
