...
* **Hash JOIN (par défaut) :** rapide pour les tables de droite de petite à moyenne taille qui tiennent en mémoire.
...
Vous pouvez laisser ClickHouse choisir le meilleur algorithme en définissant `join_algorithm = 'auto'`, ou le contrôler explicitement selon votre charge de travail. La valeur par défaut est `direct,parallel_hash,hash`, de sorte que ClickHouse utilise une jointure directe lorsque le côté droit est un dictionnaire ou un moteur clé-valeur, et sinon le hash join (`hash` et `parallel_hash` désignent la même classe et ne sélectionnent pas le parallélisme). Si vous devez sélectionner un algorithme de JOIN pour optimiser les performances ou réduire le surcoût mémoire, nous vous recommandons [ce guide](/fr/concepts/features/operations/select/joining-tables#choosing-a-join-algorithm).
