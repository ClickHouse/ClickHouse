...
<div id="enabling-jwt-authentication-for-your-service">
  ## Activation de l’authentification JWT pour votre service
</div>
...
En plus de l’authentificateur intégré, vous pouvez configurer votre service ClickHouse Cloud pour accepter les JWT émis par votre propre fournisseur d’identité (par exemple, Microsoft Entra ou Okta). Cette fonctionnalité bêta est disponible avec le plan Enterprise pour les services exécutant ClickHouse version 26.4 ou ultérieure. Pour l’activer, ouvrez un [ticket d’assistance](https://clickhouse.com/support/program) en fournissant les informations de configuration suivantes :

| Paramètre                       | Description                                                                                                                                                                                                                                                                                                                                                                                       |
| ------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ID du service                   | L’UUID du service ClickHouse Cloud sur lequel activer l’authentification JWT. Vous le trouverez dans l’URL de la Cloud Console lorsque vous ouvrez le service.                                                                                                                                                                                                                                    |
...
| URL JWKS                        | Le endpoint HTTPS sur lequel votre fournisseur d’identité publie son jeu de clés Web JSON (par exemple, `https://idp.example.com/.well-known/jwks.json`). ClickHouse récupère les clés publiques depuis ce endpoint afin de vérifier les signatures des jetons. La vérification JWKS ne fonctionne qu’avec des clés RSA (`RS256`), comme décrit dans [Revendications requises](#required-claims). |
| Claim des rôles         | Le nom du claim du jeton contenant les rôles ClickHouse de l’utilisateur éphémère (par défaut, `clickhouse:roles`). Ce claim doit contenir un tableau JSON de noms de rôles, par exemple `["analyst", "reader"]`.                                                                                                                                                           |
| Claim des grants | Le nom du claim du jeton contenant les fragments SQL `GRANT` de l’utilisateur éphémère (par défaut, `clickhouse:grants`). Ce claim doit contenir un tableau JSON de fragments de grants, par exemple `["SELECT ON analytics.*", "INSERT ON analytics.events"]`.                                                                                                       |

Les rôles indiqués dans le claim des rôles doivent déjà exister dans le service (créez-les avec `CREATE ROLE`). La limite d’autorisations plafonne les droits d’accès effectifs provenant à la fois des claims de rôles et de grants, comme décrit dans [Droits d’accès](#access-rights).
