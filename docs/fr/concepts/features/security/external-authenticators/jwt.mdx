...
En plus de l’authentificateur intégré, vous pouvez configurer votre service ClickHouse Cloud pour accepter les JWT émis par votre propre fournisseur d’identité (par exemple, Microsoft Entra ou Okta). Cette fonctionnalité bêta est disponible avec le plan Enterprise pour les services exécutant ClickHouse version 26.4 ou ultérieure. Vous pouvez la configurer vous-même depuis la Cloud Console, sous **Settings → Security → JWT authentication** — consultez [Configuration de l’authentification JWT](/fr/cloud/security/jwt-authentication-setup) pour obtenir des instructions étape par étape. Chaque fournisseur est défini par les éléments suivants :
...
| URL JWKS                     | Le endpoint HTTPS sur lequel votre fournisseur d’identité publie son jeu de clés Web JSON (par exemple, `https://idp.example.com/.well-known/jwks.json`). ClickHouse récupère les clés publiques depuis ce endpoint afin de vérifier les signatures des jetons. La vérification JWKS ne fonctionne qu’avec des clés RSA (`RS256`), comme décrit dans [Claims obligatoires](#required-claims). |
...
Les rôles sont lus depuis le claim des rôles ci-dessus, et les grants SQL sont lus par défaut depuis le claim `clickhouse:grants` du jeton. Les rôles indiqués dans le claim des rôles doivent déjà exister dans le service (créez-les avec `CREATE ROLE`). La limite d’autorisations plafonne les droits d’accès effectifs provenant à la fois des claims de rôles et de grants, comme décrit dans [Droits d’accès](#access-rights).
