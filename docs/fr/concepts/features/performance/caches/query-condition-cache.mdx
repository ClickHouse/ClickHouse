...
<Note>
  Le cache des conditions de requête fonctionne uniquement lorsque [enable&#95;analyzer](/fr/reference/settings/session-settings/allow-experimental#allow_experimental_analyzer) est défini sur true, qui est la valeur par défaut.
</Note>
...
<div id="top-k-queries">
  ### Requêtes `ORDER BY ... LIMIT n` (TopK)
</div>

Les requêtes qui utilisent l’optimisation TopK (`ORDER BY <column> LIMIT n`, accélérée par le filtrage dynamique ou par un index d’omission minmax sur la colonne de tri) n’utilisent pas le cache des conditions de requête par défaut : ces lectures ne le consultent ni ne l’alimentent.
Les lectures TopK peuvent ignorer des granules en fonction d’un seuil qui évolue pendant l’exécution de la requête. Leurs entrées de cache nécessitent donc une attention particulière ; elles sont soumises au paramètre distinct [`use_query_condition_cache_for_top_k`](/fr/reference/settings/session-settings/use-query) (désactivé par défaut), jusqu’à ce que la fiabilité de ces entrées soit pleinement établie.
Le paramètre ne prend effet que si `use_query_condition_cache` est également activé.
