...
<Steps>
  ...
  <Step title={<>Analyser la table système <code>trace_log</code></>} id="analyze-trace-log-system-table">
    Pour analyser la table système `trace_log`, activez les fonctions d’introspection avec le paramètre [`allow_introspection_functions`](/fr/reference/settings/session-settings/allow#allow_introspection_functions) :
