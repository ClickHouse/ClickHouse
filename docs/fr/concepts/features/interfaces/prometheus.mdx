...
Consultez les [fonctionnalités PromQL prises en charge](/fr/sql-reference/table-functions/prometheusQueryRange#supported-promql-features) pour obtenir la liste des fonctions et des opérateurs d’agrégation utilisés par l’API HTTP, le dialecte `promql` et les fonctions de table.
...
ClickHouse utilise le même convertisseur PromQL pour l’API HTTP, le dialecte `promql` ainsi que les fonctions de table [`prometheusQuery`](/fr/sql-reference/table-functions/prometheusQuery) et [`prometheusQueryRange`](/fr/sql-reference/table-functions/prometheusQueryRange).
