...
Le champ `user` est facultatif : lorsqu’il est omis ou vide, le nom d’utilisateur est défini par le [paramètre du serveur](/fr/operations/server-configuration-parameters/settings) `default_session_user` (ou son remplacement par point de terminaison dans une configuration de [protocoles composables](/fr/operations/settings/composable-protocols)), à savoir `default` sauf configuration contraire. Si `default_session_user` est défini sur une chaîne vide, les connexions sans nom d’utilisateur sont interdites : un message `auth` dont le champ `user` est omis ou vide échoue à l’authentification, le serveur ferme le WebSocket avec le code `1008` et, lorsque la section `session_log` est activée dans la configuration du serveur, le refus est enregistré dans [`system.session_log`](/fr/operations/system-tables/session_log) en tant qu’événement `LoginFailure` avec un `user` vide.
