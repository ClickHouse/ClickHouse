...
* **(3)** Si la table de droite tient en mémoire, utilisez le hash join (`hash` et `parallel_hash` sont des alias ; en choisir un ne contrôle pas le parallélisme). C’est généralement l’option en mémoire la plus rapide, selon la taille des données, les types de données et la distribution des valeurs des colonnes de clé de jointure.
