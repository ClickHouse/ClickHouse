...
| [`timeout_overflow_mode`](/fr/reference/settings/session-settings/timeout-overflow-mode#timeout_overflow_mode)                                 | Définit le comportement si la requête s’exécute plus longtemps que `max_execution_time` ou si le temps d’exécution estimé dépasse `max_estimated_execution_time`.                          |
