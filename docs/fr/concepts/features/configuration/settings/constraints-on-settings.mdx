...
<Warning>
  Gardez `readonly` en dehors de la liste `changeable_in_readonly` dans chaque profil, et rappelez-vous que les restrictions de profil s&#39;appliquent à l&#39;échelle du système, et non par utilisateur.
</Warning>

Ne marquez pas `readonly` lui-même comme `changeable_in_readonly`. Si vous le faites, une session démarrée avec `readonly = 1` peut exécuter `SET readonly = 0` et retrouver ainsi la possibilité d&#39;exécuter les requêtes d&#39;écriture que ses privilèges existants autorisent déjà.

Ce point est valable partout, et pas seulement pour le profil affecté à l&#39;utilisateur :

* N&#39;importe quelle session peut basculer vers n&#39;importe quel profil via `SET profile`, car la sélection du profil n&#39;est pas soumise à un contrôle d&#39;accès (elle est uniquement limitée par les contraintes de paramètres déjà actives).
* En HTTP, les requêtes `GET` sont forcées à `readonly = 2` uniquement lorsque la valeur effective serait autrement `0`. Ainsi, un profil qui définit `readonly = 1` mais autorise aussi la modification de `readonly` en mode lecture seule réduit à néant cette protection : une requête `GET` peut le repasser à `0` et écrire.
