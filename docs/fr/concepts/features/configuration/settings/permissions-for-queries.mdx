...
<Note>
  ...
  [constraints on settings](/fr/concepts/features/configuration/settings/constraints-on-settings).
</Note>
...
<Info>
  **KILL QUERY**
  ...
  de `readonly` et d&#39;`allow_ddl`. En revanche, le privilège `SELECT` sur `system.processes` est requis. Interrompre une requête
  appartenant à un autre utilisateur, ainsi que tout `KILL QUERY ... ON CLUSTER`, exigent le privilège `KILL QUERY`, que
  `readonly = 1` et `readonly = 2` refusent.
</Info>
