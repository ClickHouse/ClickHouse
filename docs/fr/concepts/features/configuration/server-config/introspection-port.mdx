...
Un port d’introspection est un endpoint de [protocole composable](/fr/operations/settings/composable-protocols) identifié par `<introspection>true</introspection>`. Aucun port d’introspection n’est configuré par défaut :
