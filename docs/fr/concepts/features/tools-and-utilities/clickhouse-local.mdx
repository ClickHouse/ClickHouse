...
* `--hints <bool>` — affiche, pendant la saisie, des indications d&#39;autocomplétion (texte « fantôme » intégré) pour la suggestion la plus pertinente lorsque le curseur se trouve à la fin de la saisie. Naviguez parmi les indications avec Haut/Bas (ou Ctrl-Haut/Ctrl-Bas) ; acceptez l&#39;indication intégrée avec Tab ou Droite ; `Enter` n&#39;accepte une indication qu&#39;après qu&#39;elle a été explicitement sélectionnée et, sinon, exécute la requête ; `Tab` ouvre également la liste classique de complétion. Nécessite `--highlight` (les indications ont besoin de couleur) ainsi que le mécanisme de suggestion (donc `--disable_suggestion` les désactive également). Activé par défaut.
