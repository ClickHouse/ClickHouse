...
<div id="keeper-configuration-settings">
  #### Paramètres de configuration de Keeper {#keeper-configuration-settings}
</div>
...
| `log_readahead_chunk_size`                | Nombre d’entrées de log décodées par фрагмент dans la tâche de remplissage de lecture anticipée.                                                                                                                                                                                                                                                                                             | `16`                    |
| `log_readahead_commit_window_bytes`       | Taille totale maximale des entrées de log décodées mises en mémoire tampon avant le thread de commit. `0` désactive la lecture anticipée du commit (le commit lit les entrées du disque une par une).                                                                                                                                                                                        | `500MiB`                |
...
* `ydld` : Demande de céder le leadership et de devenir follower. Si le serveur qui reçoit la requête est leader, il suspend d’abord les opérations d’écriture, attend que le successeur (le leader actuel ne peut jamais être le successeur) ait terminé le rattrapage du dernier log, puis renonce à son rôle. Le successeur est choisi automatiquement. Renvoie `Sent yield leadership request to leader.` si la requête est envoyée ou `Failed to send yield leadership request to leader.` dans le cas contraire. Si le nœud est déjà un follower, le résultat est le même que si la requête avait été envoyée.

```response
Sent yield leadership request to leader.
```

* `pfev` : Renvoie les valeurs de tous les événements collectés. Pour chaque événement, renvoie son nom, sa valeur et sa description.
...
<div id="migration-from-zookeeper">
  ### Migration depuis ZooKeeper {#migration-from-zookeeper}
</div>
...
<div id="pre-migration-preparation">
  #### Préparation avant la migration {#pre-migration-preparation}
</div>
...
<div id="consolidating-multiple-zookeeper-clusters">
  #### Consolidation de plusieurs clusters ZooKeeper {#consolidating-multiple-zookeeper-clusters}
</div>
...
<div id="handling-encryption-and-acls">
  #### Gestion du chiffrement et des ACL {#handling-encryption-and-acls}
</div>
...
<div id="verifying-the-migration">
  #### Vérification de la migration {#verifying-the-migration}
</div>
...
<div id="post-migration-tuning">
  #### Ajustements après migration {#post-migration-tuning}
</div>
...
<div id="recovering-after-losing-quorum">
  ### Récupération après la perte du quorum {#recovering-after-losing-quorum}
</div>
...
<div id="using-disks-with-keeper">
  ## Utiliser des disques avec Keeper {#using-disks-with-keeper}
</div>
...
<div id="changing-disk-setup">
  ### Modifier la configuration des disques {#changing-disk-setup}
</div>
...
La limite du cache des derniers logs est définie par :

* `latest_logs_cache_size_threshold` - taille totale des derniers logs stockés dans le cache

Si la valeur par défaut est trop élevée, vous pouvez réduire l&#39;utilisation de la mémoire en diminuant cette valeur de configuration.
...
<div id="clickhouse-keeper-user-guide">
  ## Guide d’utilisation de ClickHouse Keeper {#clickhouse-keeper-user-guide}
</div>
...
<div id="configuring-clickhouse-keeper-with-unique-paths">
  ## Configuration de ClickHouse Keeper avec des chemins uniques {#configuring-clickhouse-keeper-with-unique-paths}
</div>
...
<div id="example-environment">
  ### Environnement d’exemple {#example-environment}
</div>
...
<div id="procedures-to-set-up-tables-to-use-uuid">
  ### Procédures pour configurer les tables afin d’utiliser `{uuid}` {#procedures-to-set-up-tables-to-use-uuid}
</div>
...
<div id="reconfiguration">
  ## Reconfiguration dynamique de ClickHouse Keeper {#reconfiguration}
</div>
...
<div id="converting-a-single-node-keeper-into-a-cluster">
  ## Conversion d’un Keeper mono-nœud en cluster {#converting-a-single-node-keeper-into-a-cluster}
</div>
