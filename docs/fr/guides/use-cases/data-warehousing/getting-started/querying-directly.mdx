...
<Info>
  **Server, local ou chDB**
  ...
  Les étapes de ce guide peuvent être exécutées à l’aide d’une installation existante de ClickHouse server. Pour des requêtes ad hoc, vous pouvez aussi utiliser [clickhouse-local](/fr/concepts/features/tools-and-utilities/clickhouse-local) et suivre le même workflow sans démarrer de serveur. Moyennant quelques ajustements mineurs, le processus peut également être effectué avec la distribution intégrée au processus de ClickHouse, [chDB](/chdb/index).
</Info>
