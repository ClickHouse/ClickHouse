...
[Paimon](/fr/reference/functions/table-functions/paimon) propose uniquement des fonctions de table.
...
Pour les connexions aux catalogues, chaque type de catalogue possède son propre flag. Consultez [Connexion aux catalogues](/fr/guides/use-cases/data-warehousing/getting-started/connecting-catalogs) pour une vue d’ensemble, et la [référence DataLakeCatalog](/fr/reference/engines/database-engines/datalake) pour le détail des paramètres. La configuration de chaque catalogue se trouve dans les [guides des catalogues](/fr/guides/use-cases/data-warehousing/reference).

Pour les écritures, Iceberg nécessite [allow&#95;insert&#95;into&#95;iceberg](/fr/reference/settings/session-settings#allow_insert_into_iceberg) (25.7+, Beta à partir de 26.2). Consultez [Écriture dans les lacs de données](/fr/guides/use-cases/data-warehousing/getting-started/writing-data). Delta Lake nécessite [allow&#95;delta&#95;lake&#95;writes](/fr/reference/settings/session-settings#allow_experimental_delta_lake_writes) (25.9+). La [matrice de compatibilité](/fr/guides/use-cases/data-warehousing/support-matrix) indique quels flags s’appliquent à chaque format et à chaque opération.
...
Pour les tables Iceberg avec [partitionnement masqué](https://iceberg.apache.org/docs/latest/partitioning/), filtrez sur la **colonne source** dans le schéma de la table, et non sur une colonne de partition distincte ou sur le nom d’un champ transformé. Si la table est partitionnée par `day(event_time)`, ajoutez un prédicat sur `event_time`. ClickHouse déduit l’élagage des partitions de ce filtre en s’appuyant sur la spécification de partitionnement d’Iceberg. Voir [Partition pruning](/fr/reference/engines/table-engines/integrations/iceberg#partition-pruning) et la [spécification d’Iceberg](https://iceberg.apache.org/spec/#partitioning).
...
| ------------------------------------------------------------------------------------------------------------------ | ------ | -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [use&#95;iceberg&#95;partition&#95;pruning](/fr/reference/settings/session-settings#use_iceberg_partition_pruning)           | 25.1   | `1` à partir de 25.6 | Ignore les fichiers de données à l’aide des métadonnées de partition présentes dans les manifestes                                                            |
| [use&#95;iceberg&#95;metadata&#95;files&#95;cache](/fr/reference/settings/session-settings#use_iceberg_metadata_files_cache) | 25.4   | `1`                  | Met en cache en mémoire les listes de manifestes et les métadonnées JSON                                                                                      |
| [iceberg&#95;metadata&#95;staleness&#95;ms](/fr/reference/settings/session-settings#iceberg_metadata_staleness_ms)           | 26.3   | `0`                  | Paramètre de requête. Utilise les métadonnées en cache lorsqu’elles sont plus récentes que cet intervalle, au lieu d’interroger le catalogue à chaque requête |
...
2. Définissez [iceberg&#95;metadata&#95;staleness&#95;ms](/fr/reference/settings/session-settings#iceberg_metadata_staleness_ms) (26.3+) dans les requêtes pour accepter des métadonnées légèrement périmées et ainsi éviter l&#39;aller-retour vers le catalogue.
...
Lisez un instantané historique avec [iceberg&#95;timestamp&#95;ms](/fr/reference/settings/session-settings#iceberg_timestamp_ms) ou [iceberg&#95;snapshot&#95;id](/fr/reference/settings/session-settings#iceberg_snapshot_id) (tous deux disponibles à partir de la version 25.4). Ne définissez pas les deux dans la même requête. Consultez la lignée des snapshots dans [system.iceberg&#95;history](/fr/reference/system-tables/iceberg_history) (25.6+) avant de choisir un ID. Pour des chargements par lot répétés, voir [Limiter les lectures par lot aux snapshots](#snapshot-bounds).
...
Au-delà de [allow&#95;insert&#95;into&#95;iceberg](/fr/reference/settings/session-settings#allow_insert_into_iceberg) (25.7+, bêta à partir de 26.2), contrôlez la taille des fichiers de sortie et le nombre de partitions à l’insertion :

| Paramètre                                                                                                                            | Depuis | Objectif                                                        |
| ------------------------------------------------------------------------------------------------------------------------------------ | ------ | --------------------------------------------------------------- |
| [iceberg&#95;insert&#95;max&#95;rows&#95;in&#95;data&#95;file](/fr/reference/settings/session-settings#iceberg_insert_max_rows_in_data_file)   | 25.9   | Nombre maximal de lignes par fichier de données de sortie       |
| [iceberg&#95;insert&#95;max&#95;bytes&#95;in&#95;data&#95;file](/fr/reference/settings/session-settings#iceberg_insert_max_bytes_in_data_file) | 25.9   | Taille maximale en octets par fichier de données de sortie      |
| [iceberg&#95;insert&#95;max&#95;partitions](/fr/reference/settings/session-settings#iceberg_insert_max_partitions)                             | 25.12  | Nombre maximal de partitions écrites lors d’une seule insertion |
...
À partir de la version 25.6, ClickHouse lit Delta Lake sur S3 et GCS via le kernel Rust de Delta Lake ([allow&#95;experimental&#95;delta&#95;kernel&#95;rs](/fr/reference/settings/session-settings#allow_experimental_delta_kernel_rs), 25.5+). Sur Azure Blob Storage, utilisez [deltaLakeAzure()](/fr/reference/functions/table-functions/deltalake) avec le lecteur legacy, car le kernel y est désactivé. Sans le kernel, l’élagage des partitions, le flux de données des modifications et la lecture de versions d&#39;instantané ne sont pas disponibles.

<div id="delta-kernel">
  #### Delta Kernel
</div>

[allow&#95;experimental&#95;delta&#95;kernel&#95;rs](/fr/reference/settings/session-settings#allow_experimental_delta_kernel_rs) doit être activé pour l’élagage des partitions, le flux de données des modifications et la lecture de versions de snapshot. Il est activé par défaut sur S3 et GCS à partir de la version 25.5. Activez-le explicitement sur les versions antérieures ou pour le dépannage :
...
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- | ------- | --------------------------------------------------------------------------------------------------------- |
| [delta&#95;lake&#95;enable&#95;engine&#95;predicate](/fr/reference/settings/session-settings#delta_lake_enable_engine_predicate)                                                                                                                  | 25.8  | `1`     | Transmet les filtres au kernel pour l&#39;élagage des partitions. Nécessite [Delta Kernel](#delta-kernel) |
| [delta&#95;lake&#95;reload&#95;schema&#95;for&#95;consistency](/fr/reference/settings/session-settings#delta_lake_reload_schema_for_consistency)                                                                                                  | 26.3  | `0`     | Recharge le schéma avant chaque requête lorsque des writers concurrents font évoluer le schéma            |
| [delta&#95;lake&#95;snapshot&#95;start&#95;version](/fr/reference/settings/session-settings#delta_lake_snapshot_start_version) / [delta&#95;lake&#95;snapshot&#95;end&#95;version](/fr/reference/settings/session-settings#delta_lake_snapshot_end_version) | 25.12 | `-1`    | Lit les changements CDF entre deux versions d&#39;instantané. Nécessite que CDF soit activé en amont      |
| [delta&#95;lake&#95;snapshot&#95;version](/fr/reference/settings/session-settings#delta_lake_snapshot_version)                                                                                                                                    | 25.8  | `-1`    | Lit un instantané historique unique. Définissez `-1` pour le plus récent (`0` est valide)                 |
...
Pour lire uniquement les lignes modifiées entre deux instantanés Delta, définissez [delta&#95;lake&#95;snapshot&#95;start&#95;version](/fr/reference/settings/session-settings#delta_lake_snapshot_start_version) et [delta&#95;lake&#95;snapshot&#95;end&#95;version](/fr/reference/settings/session-settings#delta_lake_snapshot_end_version) (25.12+). La table doit avoir le flux de données des modifications activé dans la source amont (`delta.enableChangeDataFeed`). Définissez les versions de début et de fin dans les paramètres de requête. Définir uniquement la version de fin provoque une erreur.
...
En plus de [allow&#95;delta&#95;lake&#95;writes](/fr/reference/settings/session-settings#allow_experimental_delta_lake_writes) (25.9+), contrôlez la taille des fichiers de sortie à l&#39;insertion :

| Paramètre                                                                                                                                      | Depuis | Objectif                                          |
| ---------------------------------------------------------------------------------------------------------------------------------------------- | ------ | ------------------------------------------------- |
| [delta&#95;lake&#95;insert&#95;max&#95;rows&#95;in&#95;data&#95;file](/fr/reference/settings/session-settings#delta_lake_insert_max_rows_in_data_file)   | 25.9   | Limite de lignes par fichier de données de sortie |
| [delta&#95;lake&#95;insert&#95;max&#95;bytes&#95;in&#95;data&#95;file](/fr/reference/settings/session-settings#delta_lake_insert_max_bytes_in_data_file) | 25.9   | Limite d’octets par fichier de données de sortie  |
...
Si les tables du catalogue n&#39;apparaissent pas dans `system.tables`, activez [show&#95;remote&#95;databases&#95;in&#95;system&#95;tables](/fr/reference/settings/session-settings#show_remote_databases_in_system_tables) (25.8+). Par défaut, les tables du catalogue sont masquées dans les informations d&#39;introspection du système. Dans les versions antérieures à 26.6, utilisez son ancien nom, `show_data_lake_catalogs_in_system_tables`.
...
| ----------------------------------------------------------------------------------------------- | ---------- | ------ | ----------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------- |
| [system.iceberg&#95;metadata&#95;log](/fr/reference/system-tables/iceberg_metadata_log)           | Iceberg    | 25.9   | [iceberg&#95;metadata&#95;log&#95;level](/fr/reference/settings/session-settings#iceberg_metadata_log_level) sur la requête | Suivre les metadata files lus et les décisions d’élagage des partitions |
| [system.iceberg&#95;history](/fr/reference/system-tables/iceberg_history)                         | Iceberg    | 25.6   | Alimentée automatiquement pour les tables Iceberg dans ClickHouse                                                 | Examiner la lignée des snapshots avant les requêtes de voyage temporel  |
| [system.delta&#95;lake&#95;metadata&#95;log](/fr/reference/system-tables/delta_metadata_log) | Delta Lake | 25.10  | [delta&#95;lake&#95;log&#95;metadata](/fr/reference/settings/session-settings#delta_lake_log_metadata) = `1` sur la requête | Suivre les metadata files Delta et la résolution des snapshots      |
...
* [Prise en main](/fr/guides/use-cases/data-warehousing/getting-started/overview) — Guide complet, de l’interrogation directe à la réécriture des données
...
* [Connexion aux catalogues](/fr/guides/use-cases/data-warehousing/getting-started/connecting-catalogs) — Configuration de `DataLakeCatalog` avec Unity Catalog
* [Écrire dans des lacs de données](/fr/guides/use-cases/data-warehousing/getting-started/writing-data) — Réécrire les données dans Iceberg et Delta Lake
* [Matrice de compatibilité](/fr/guides/use-cases/data-warehousing/support-matrix) — Comparaison des fonctionnalités entre formats, catalogues et backends de stockage