...
Pour les écritures, Iceberg nécessite [allow&#95;insert&#95;into&#95;iceberg](/fr/reference/settings/session-settings/allow#allow_insert_into_iceberg) (25.7+, Beta à partir de 26.2). Consultez [Écriture dans les lacs de données](/fr/use-cases/data-lake/getting-started/writing-data). Delta Lake nécessite [allow&#95;delta&#95;lake&#95;writes](/fr/reference/settings/session-settings/allow-experimental#allow_experimental_delta_lake_writes) (25.9+). La [matrice de compatibilité](/fr/use-cases/data-lake/support-matrix) indique quels flags s’appliquent à chaque format et à chaque opération.
...
Lisez un instantané historique avec [iceberg&#95;timestamp&#95;ms](/fr/reference/settings/session-settings/iceberg#iceberg_timestamp_ms) ou [iceberg&#95;snapshot&#95;id](/fr/reference/settings/session-settings/iceberg#iceberg_snapshot_id) (tous deux disponibles à partir de la version 25.4). Ne définissez pas les deux dans la même requête. Consultez la lignée des instantanés dans [system.iceberg&#95;history](/fr/operations/system-tables/iceberg_history) (25.6+) avant de choisir un ID. Pour des chargements par lot répétés, voir [Limiter les lectures par lot aux instantanés](#snapshot-bounds).
...
À partir de la version 25.6, ClickHouse lit Delta Lake sur S3 et GCS via le kernel Rust de Delta Lake ([allow&#95;experimental&#95;delta&#95;kernel&#95;rs](/fr/reference/settings/session-settings/allow-experimental#allow_experimental_delta_kernel_rs), 25.5+). Sur Azure Blob Storage, utilisez [deltaLakeAzure()](/fr/sql-reference/table-functions/deltalake) avec le lecteur legacy, car le kernel y est désactivé. Sans le kernel, l’élagage des partitions, le flux de données des modifications et la lecture de versions d’instantané ne sont pas disponibles.

<div id="delta-kernel">
  #### Delta Kernel
</div>

[allow&#95;experimental&#95;delta&#95;kernel&#95;rs](/fr/reference/settings/session-settings/allow-experimental#allow_experimental_delta_kernel_rs) doit être activé pour l’élagage des partitions, le flux de données des modifications et la lecture de versions d’instantané. Il est activé par défaut sur S3 et GCS à partir de la version 25.5. Activez-le explicitement sur les versions antérieures ou pour le dépannage :
...
En plus de [allow&#95;delta&#95;lake&#95;writes](/fr/reference/settings/session-settings/allow-experimental#allow_experimental_delta_lake_writes) (25.9+), contrôlez la taille des fichiers de sortie à l’insertion :

| Paramètre                                                                                                                                                        | Depuis | Objectif                                          |
| ---------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ | ------------------------------------------------- |
| [delta&#95;lake&#95;insert&#95;max&#95;rows&#95;in&#95;data&#95;file](/fr/reference/settings/session-settings/delta-lake#delta_lake_insert_max_rows_in_data_file)   | 25.9   | Limite de lignes par fichier de données de sortie |
| [delta&#95;lake&#95;insert&#95;max&#95;bytes&#95;in&#95;data&#95;file](/fr/reference/settings/session-settings/delta-lake#delta_lake_insert_max_bytes_in_data_file) | 25.9   | Limite d’octets par fichier de données de sortie  |
...
| ----------------------------------------------------------------------------------------------- | ---------- | ------ | ----------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------- |
| [system.iceberg&#95;metadata&#95;log](/fr/operations/system-tables/iceberg_metadata_log)           | Iceberg    | 25.9   | [iceberg&#95;metadata&#95;log&#95;level](/fr/reference/settings/session-settings/iceberg-metadata#iceberg_metadata_log_level) sur la requête | Suivre les metadata files lus et les décisions d’élagage des partitions |
| [system.iceberg&#95;history](/fr/operations/system-tables/iceberg_history)                         | Iceberg    | 25.6   | Alimentée automatiquement pour les tables Iceberg dans ClickHouse                                                                         | Examiner la lignée des instantanés avant les requêtes de voyage temporel  |
| [system.delta&#95;lake&#95;metadata&#95;log](/fr/operations/system-tables/delta_lake_metadata_log) | Delta Lake | 25.10  | [delta&#95;lake&#95;log&#95;metadata](/fr/reference/settings/session-settings/delta-lake#delta_lake_log_metadata) = `1` sur la requête       | Suivre les metadata files Delta et la résolution des instantanés          |
