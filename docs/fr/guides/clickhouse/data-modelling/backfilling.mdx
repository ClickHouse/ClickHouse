...
* **Parallélisme d’insertion** - Le nombre de threads d’insertion utilisés pour insérer les données. Contrôlé via [`max_insert_threads`](/fr/reference/settings/session-settings/max-insert#max_insert_threads). **Remarque** : cette valeur est plafonnée par `max_threads`, donc le parallélisme d’insertion effectif est `min(max_insert_threads, max_threads)`. Dans ClickHouse Cloud, il est déterminé par la taille de l’instance (entre 2 et 4) et est défini à 1 dans OSS. Augmenter cette valeur peut améliorer les performances, au prix d’une utilisation mémoire plus élevée.
