...
volume persistant, l’augmentation de capacité, les politiques de stockage personnalisées ainsi que les règles relatives à ce qui ne peut pas
...
`spec.clusterDomain` définit le suffixe DNS Kubernetes que l’opérateur utilise pour construire
les noms d’hôte complets des pods qu’il écrit dans la
configuration du serveur ClickHouse. Par défaut, sa valeur est `cluster.local`, et ce champ est présent à la fois dans
`ClickHouseCluster` et `KeeperCluster`.
...
L’opérateur contacte chaque pod via le Service headless à l’adresse
`<pod>.<headless-service>.<namespace>.svc.<clusterDomain>`. Ce suffixe se retrouve dans
deux parties de la configuration générée :
...
<Note>
  Ne remplacez cette valeur que si le `kubelet` de votre cluster s’exécute avec un `--cluster-domain`
  différent de `cluster.local`. Si la valeur ne correspond pas au domaine réel du cluster,
  ClickHouse ne pourra pas résoudre les noms d’hôte de Keeper ni ceux des répliques — la coordination et
  les requêtes `Distributed` échoueront avec des erreurs de résolution DNS. Définissez la **même** valeur sur le
  `ClickHouseCluster` et le `KeeperCluster` auquel il fait référence.
</Note>
...
<Note>
  L&#39;ensemble des clés requises dépend de la version de ClickHouse utilisée. `named-collections-key` n&#39;est validée qu&#39;une fois que la sonde de version de l&#39;opérateur a détecté ClickHouse `25.12` ou une version plus récente. Sur les versions antérieures, la clé peut être absente du Secret.
</Note>
