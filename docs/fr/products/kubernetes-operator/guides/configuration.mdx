...
L’opérateur expose un ensemble fixe de ports sur chaque Pod ClickHouse et sur son Service headless public : `8123` HTTP, `9000` natif, `9009` inter-serveur, `9001`/`9002` gestion, `9363` métriques Prometheus, ainsi que les variantes TLS `8443`/`9440` lorsque TLS est activé. Les ports `interserver` et de gestion sont en outre exposés via les Services internes propres à chaque réplique, afin que les répliques et l’opérateur puissent communiquer avant qu’une réplique ne devienne prête. Pour que ClickHouse écoute sur des protocoles supplémentaires — MySQL, PostgreSQL, gRPC ou tout autre port personnalisé — déclarez-les dans `spec.additionalPorts` :
...
Chaque Pod de réplique porte la readiness gate `clickhouse.com/ReplicaInitialized`, de sorte qu’une nouvelle réplique n’est publiée via le Service headless public qu’une fois que l’opérateur a terminé son initialisation : la base de données `default` est convertie vers le moteur Replicated et le schéma est synchronisé. Jusque-là, l’opérateur et les autres répliques la joignent via son Service interne. Avec `enableDatabaseSync: false`, l’opérateur marque immédiatement les répliques comme initialisées, si bien que la readiness ne dépend plus que des probes du conteneur.

L’opérateur ne supprime jamais une base de données `default` non-Replicated qui contient des données. Une telle réplique commence tout de même à servir le trafic client après la préparation du schéma, mais le cluster signale `SchemaInSync=False` avec pour raison `DefaultDatabaseNotReplicated` jusqu’à ce que vous résolviez cette base de données vous-même.

Avant qu’une réplique ne soit supprimée lors d’une réduction d’échelle, l’opérateur commence par en décharger le trafic client, attend qu’elle cesse d’être publiée, réplique ses données restantes vers les répliques survivantes, et ne la supprime qu’ensuite.
