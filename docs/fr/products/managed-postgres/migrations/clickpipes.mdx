...
ClickHouse Cloud propose désormais ClickPipes pour migrer votre base de données PostgreSQL externe vers un service ClickHouse Managed Postgres. Cette intégration native offre une expérience simplifiée pour se connecter à votre base de données source, exporter le schéma, l’importer dans ClickHouse Managed Postgres et configurer la réplication continue.
...
<Note>
  Si vous rencontrez des problèmes pendant la migration, consultez la [FAQ des migrations ClickHouse Managed Postgres](/fr/products/managed-postgres/migrations/faq) pour connaître les erreurs courantes et leurs solutions.
</Note>

<Steps>
  <Step title="Connectez-vous à votre base de données source" id="step-1-connect">
    Ouvrez la [console ClickHouse Cloud](https://clickhouse.cloud) et sélectionnez votre service ClickHouse Managed Postgres.

    <Image img="/images/managed-postgres/pgpg/servicecard.webp" alt="Carte du service ClickHouse Managed Postgres dans la liste des services ClickHouse Cloud" size="lg" border />

    Dans la barre latérale de gauche, cliquez sur **Sources de données**.

    <Image img="/images/managed-postgres/pgpg/overview.webp" alt="Entrée Sources de données dans la barre latérale du service ClickHouse Managed Postgres" size="lg" border />
  ...
  <Step title="Importez le schéma dans votre service ClickHouse Managed Postgres" id="step-3-import-schema">
    ...
    L&#39;assistant affiche une commande `psql` pour appliquer le dump du schéma à votre service ClickHouse Managed Postgres. Exécutez-la dans un terminal :
  ...
  <Step title="Configurez les paramètres d'ingestion" id="step-4-ingestion-settings">
    ...
    | Nombre de tables du snapshot en parallèle     | 1                 | Tables dont le snapshot est pris simultanément           |

    <Image img="/images/managed-postgres/pgpg/advancedsettings.webp" alt="Étape 4 : formulaire des paramètres d'ingestion avec options de publication et de réplication avancée" size="lg" border />
  ...
  <Step title="Sélectionnez les tables" id="step-5-select-tables">
    ...
    <Image img="/images/managed-postgres/pgpg/tablepicker.webp" alt="Étape 5 : sélecteur de tables regroupées par schéma avec bouton Créer la migration" size="lg" border />
...
**Basculez le trafic de l’application.** Redirigez les lectures et les écritures vers votre service ClickHouse Managed Postgres et surveillez les erreurs, les violations de contraintes et le bon fonctionnement de la réplication.
...
* [Guide de démarrage rapide pour ClickHouse Managed Postgres](/fr/products/managed-postgres/quickstart)
* [Informations de connexion à ClickHouse Managed Postgres](/fr/products/managed-postgres/connection)
