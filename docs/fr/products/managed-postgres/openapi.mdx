...
Examinons le cycle de vie d’un service Postgres.

<Warning>
  **Masquage des identifiants à partir du 31 juillet 2026**

  À compter du **31 juillet 2026**, l’API ne renvoie plus les propriétés `password` et `connectionString` dans ses réponses. Elles n’apparaissent plus que dans la réponse [create](#create) et dans la réponse [réinitialisation du mot de passe] lorsque la requête ne contenait aucun mot de passe. Nous vous recommandons d’enregistrer les identifiants à partir de la réponse [create](#create). Si vous gérez des services avec Terraform, mettez à niveau le provider vers la version **v3.21.0 ou ultérieure** avant le 31 juillet 2026 (voir la [référence Terraform](/fr/products/managed-postgres/terraform)).
</Warning>
...
Utilisez maintenant ces données pour créer une nouvelle instance ; notez que cela nécessite l’en-tête Content-Type. Enregistrez la réponse pour avoir accès aux informations d’authentification qu’elle renvoie :
...
Le résultat sera similaire au JSON renvoyé lors de la création (mais sans les propriétés `password` et `connectionString` après le 31 juillet 2026), mais surveillez
...
[ClickHouse OpenAPI]: /fr/products/cloud/features/admin-features/api/api-overview "Cloud API"
...
[Prometheus endpoint]: /fr/products/managed-postgres/monitoring/prometheus "Point de terminaison Prometheus de ClickHouse Managed Postgres"

[metrics reference]: /fr/products/managed-postgres/monitoring/metrics "Référence des métriques de ClickHouse Managed Postgres"

[Query Insights]: /fr/products/managed-postgres/monitoring/query-insights "Query insights de Postgres"

[detail flyout]: /fr/products/managed-postgres/monitoring/query-insights#detail "Volet de détails de Query insights"

[per-execution counters]: /fr/products/managed-postgres/monitoring/query-insights#counters "Compteurs par exécution de Query insights"
