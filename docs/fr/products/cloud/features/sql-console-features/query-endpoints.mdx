...
Les Query points de terminaison de l’API appliquent la liste d’autorisation d’IP définie au niveau de la clé API. Comme pour la SQL Console, les Query points de terminaison de l’API font transiter les requêtes via l’infrastructure de ClickHouse, de sorte que les paramètres de liste d’autorisation IP au niveau du service ne s’appliquent pas.

Pour limiter les clients autorisés à appeler vos Query points de terminaison de l’API :
...
<Steps>
  <Step>
    ...
    2. Cliquez sur **Edit** à côté de la clé API utilisée pour les Query points de terminaison de l’API
  ...
  <Step>
    ...
    La création de Query points de terminaison de l’API nécessite un rôle Admin dans la Console et une clé API disposant des autorisations appropriées.
  </Step>
</Steps>
...
<Tip>
  **Guide**
  ...
  Consultez le [guide des Query points de terminaison de l’API](/fr/products/cloud/guides/sql-console/query-endpoints) pour savoir comment configurer
  des Query points de terminaison de l’API en quelques étapes simples
</Tip>