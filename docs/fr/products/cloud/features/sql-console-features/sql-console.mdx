...
Si une requête a été partagée avec vous, elle s’affiche dans l’onglet « Queries » de la barre latérale gauche de la console SQL :
...
<div id="exporting-query-result-data">
  ### Exporter les données des résultats de requête
</div>
...
La console SQL prend en charge dix types de graphiques, sélectionnables depuis le sélecteur de type dans le volet de configuration du graphique. Par exemple, nous pouvons facilement faire passer le type de graphique précédent de Bar à Area :
...
Vous pouvez également ajuster plusieurs paramètres de graphique plus avancés dans la section &#39;Advanced&#39; du volet de configuration du graphique. Pour commencer, nous allons modifier les paramètres suivants :
