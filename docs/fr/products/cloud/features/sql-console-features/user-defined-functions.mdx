...
4. À l’aide de [Terraform](#manage-udfs-with-terraform) (alpha)
...
Tout ce qui est disponible dans l’UI est également accessible par programmation via l’[API ClickHouse Cloud](/fr/cloud/manage/api/api-overview).
...
* [`clickhouse_udf_attachment`](https://github.com/ClickHouse/terraform-provider-clickhouse/blob/main/docs/resources/udf_attachment.md) attache une version d&#39;UDF à un service. Un service ne peut utiliser qu&#39;une seule version d&#39;une fonction à la fois. Vous pouvez épingler un numéro de version fixe ou référencer `clickhouse_udf.<name>.version` pour mettre automatiquement à jour les services vers la dernière version.

<Note>
  Ces ressources sont disponibles à partir de la version 3.24.0 du provider. Elles sont au statut alpha et leur comportement peut changer dans les prochaines versions du provider.
</Note>
