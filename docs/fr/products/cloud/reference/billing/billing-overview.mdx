...
### Scale (toujours actif, mise à l’échelle automatique) : à partir de 499,38 $ par mois {#scale-always-on-auto-scaling-from-49938-per-month}
...
### Enterprise : les tarifs de départ varient {#enterprise-starting-prices-vary}
...
### Qu’est-ce qu’un ClickHouse Credit (CHC) ? {#what-is-chc}
...
### Où puis-je consulter les anciens tarifs ? {#find-legacy-pricing}

Les informations sur les anciens tarifs sont disponibles [ici](https://clickhouse.com/pricing?legacy=true).

### Comment le compute est-il facturé ? {#how-is-compute-metered}
...
### Comment le stockage sur disque est-il calculé ? {#how-is-storage-on-disk-calculated}
...
### Les sauvegardes sont-elles prises en compte dans le stockage total ? {#do-backups-count-toward-total-storage}
...
### Comment estimer la compression ? {#how-do-i-estimate-compression}
...
### Quels outils ClickHouse propose-t-il pour estimer le coût d’exploitation d’un service dans le cloud si j’ai un déploiement autogéré ? {#what-tools-does-clickhouse-offer-to-estimate-the-cost-of-running-a-service-in-the-cloud-if-i-have-a-self-managed-deployment}
...
### Quelles options de facturation sont disponibles pour ClickHouse Cloud ? {#what-billing-options-are-available-for-clickhouse-cloud}
...
### Puis-je supprimer ma carte bancaire ? {#can-i-delete-my-credit-card}
...
### Où puis-je voir mes remboursements ? {#where-can-i-see-my-refunds}
...
### Combien de temps faut-il pour qu&#39;un remboursement apparaisse ? {#how-long-does-it-take-for-a-refund-to-appear}
...
### Quelle est la durée du cycle de facturation ? {#how-long-is-the-billing-cycle}
...
### Si j’ai un abonnement Marketplace PAYG actif et que je signe ensuite un contrat avec engagement, mes crédits d’engagement seront-ils consommés en premier ? {#committed-credits-consumed-first-with-active-payg-subscription}
...
### Quels moyens de contrôle ClickHouse Cloud offre-t-il pour gérer les coûts des services Scale et Enterprise ? {#what-controls-does-clickhouse-cloud-offer-to-manage-costs-for-scale-and-enterprise-services}
...
### Quelles options ClickHouse Cloud offre-t-il pour gérer les coûts des services Basic ? {#what-controls-does-clickhouse-cloud-offer-to-manage-costs-for-basic-services}
...
### Si j’ai plusieurs services, recevrai-je une facture par service ou une facture consolidée ? {#if-i-have-multiple-services-do-i-get-an-invoice-per-service-or-a-consolidated-invoice}

Une facture consolidée est établie pour tous les services d’une organisation donnée sur une période de facturation.

### Si j’ajoute ma carte de crédit et passe à une offre payante avant la fin de ma période d’essai et l’expiration de mes crédits, serai-je facturé ? {#if-i-add-my-credit-card-and-upgrade-before-my-trial-period-and-credits-expire-will-i-be-charged}
...
### Comment puis-je suivre mes dépenses ? {#how-can-i-keep-track-of-my-spending}
...
### Comment accéder à mes factures pour mon abonnement au service ClickHouse Cloud ? {#how-do-i-access-my-invoice-for-my-subscription-to-the-clickhouse-cloud-service}
...
### Pourquoi les dates des relevés d&#39;utilisation ne correspondent-elles pas à ma facture AWS Marketplace ? {#why-do-the-dates-on-the-usage-statements-not-match-my-marketplace-invoice}
...
### Existe-t-il des restrictions concernant l’utilisation des crédits prépayés ? {#are-there-any-restrictions-around-the-usage-of-prepaid-credits}
...
### Y a-t-il une différence de tarification pour ClickHouse Cloud selon que vous payez via la marketplace du fournisseur de cloud ou directement auprès de ClickHouse ? {#is-there-a-difference-in-clickhouse-cloud-pricing-whether-paying-through-the-cloud-provider-marketplace-or-directly-to-clickhouse}
...
### Comment la fonctionnalité compute-compute separation est-elle facturée ? {#how-is-compute-compute-separation-billed}
...
## Tarification de ClickPipes {#clickpipes-pricing}
