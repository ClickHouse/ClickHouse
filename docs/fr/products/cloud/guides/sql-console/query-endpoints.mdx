...
<Steps>
  ...
  <Step>
    ...
    Les endpoints d’API de requête peuvent être configurés directement depuis la vue de requête en cliquant sur le bouton **Share** et en sélectionnant `API Endpoint`.
  ...
  <Step>
    ...
    Une fois un endpoint d’API de requête créé, vous pouvez tester son fonctionnement en utilisant `curl` ou tout autre client HTTP :
