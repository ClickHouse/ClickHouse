...
Supposons que vous essayiez d’utiliser les fonctions de table [MySQL](/fr/reference/functions/table-functions/mysql) ou [PostgreSQL](/fr/reference/functions/table-functions/postgresql) dans ClickHouse Cloud pour vous connecter à votre base de données hébergée dans un VPC Amazon Web Services (AWS). AWS PrivateLink ne peut pas être utilisé pour sécuriser cette connexion. PrivateLink est une connexion à sens unique, unidirectionnelle. Il permet à votre réseau interne ou à votre VPC Amazon de se connecter en toute sécurité à ClickHouse Cloud, mais ne permet pas à ClickHouse Cloud de se connecter à votre réseau interne.
...
Pour cela, configurez vos groupes de sécurité AWS afin d’autoriser les connexions de ClickHouse Cloud vers votre service de base de données interne/privé. Consultez les [adresses IP de sortie par défaut des régions ClickHouse Cloud](/fr/products/cloud/guides/data-sources/cloud-endpoints-api), ainsi que les [adresses IP statiques disponibles](https://api.clickhouse.cloud/static-ips.json).