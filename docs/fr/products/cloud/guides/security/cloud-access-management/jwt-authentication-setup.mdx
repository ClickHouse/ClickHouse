...
<EnterprisePlanFeatureBadge feature="Authentification JWT avec un fournisseur d'identité personnalisé" support="true" />

<Tip>
  Ce guide explique comment configurer des fournisseurs JWKS dans la console Cloud. Pour savoir comment générer un JWT et à quoi ressemble sa structure — les claims obligatoires, les claims de rôles et de grants, ainsi que le fonctionnement des utilisateurs éphémères — consultez la référence [Authentification JWT](/fr/operations/external-authenticators/jwt).
</Tip>
...
Une fois cette fonctionnalité activée pour votre organisation, vous pouvez configurer vous-même ces fournisseurs JWKS pour chaque service, depuis **Paramètres → Sécurité** dans la console ClickHouse Cloud.

<Note>
  Pour activer les fournisseurs JWT pour votre organisation, contactez le [support](https://clickhouse.com/support/program).
</Note>
...
* Une URL JWKS **HTTPS** accessible publiquement, publiant au moins une clé **RSA** (les jetons doivent être signés avec **RS256**).

<Note>
  Les fournisseurs basés sur JWKS n&#39;acceptent que les **clés RSA**. Les jetons signés avec des clés HMAC (`HS256`) ou EC (`ES256`) ne peuvent pas être vérifiés auprès d&#39;un endpoint JWKS et sont rejetés. Un document JWKS peut contenir d&#39;autres types de clés, mais il doit contenir au moins une clé RSA utilisable.
</Note>
...
Le client (votre fournisseur d’identité ou votre application) génère un JWT et le signe avec sa **clé privée**. Le jeton doit respecter le [format de jeton](/fr/operations/external-authenticators/jwt#token-claims) attendu. ClickHouse le vérifie ensuite à l’aide des clés **publiques** publiées à l’URL de votre JWKS :

1. ClickHouse lit l’en-tête `kid` (ID de clé) du jeton et sélectionne la clé correspondante dans votre document JWKS.
2. Il vérifie la signature du jeton à l’aide de cette clé publique RSA et contrôle les claims `iss` (émetteur) et `aud` (audience) par rapport à la configuration de votre fournisseur.
3. Si la vérification réussit, la connexion s’exécute sous la forme d’un utilisateur éphémère dont les droits d’accès proviennent des claims `clickhouse:grants` et `clickhouse:roles` du jeton, dans la limite des autorisations de l’utilisateur `default`. Consultez [Droits d’accès](/fr/operations/external-authenticators/jwt#access-rights) pour plus de détails.
...
<Steps>
  ...
  <Step title="Renseigner les informations du fournisseur" id="fill-provider-details">
    ...
    | **URL JWKS**                             | URL HTTPS publique qui publie vos clés publiques RSA utilisées pour vérifier les signatures des jetons.                                                                                                                                 |
...
* Les fournisseurs JWKS n’acceptent que les clés **RSA / RS256**.
...
* [Authentification JWT](/fr/operations/external-authenticators/jwt) — claims du jeton, utilisateurs éphémères et utilisation du client.