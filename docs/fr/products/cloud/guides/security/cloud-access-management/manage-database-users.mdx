...
<Note>
  **Gérer les rôles de base de données depuis la console (bêta)**

  Vous pouvez également gérer les rôles de base de données directement depuis la console ClickHouse Cloud en ajoutant des permissions de données aux rôles personnalisés. Cette fonctionnalité est en bêta et est disponible pour les services exécutant ClickHouse version 26.4 et ultérieure. Les rôles attribués manuellement à l’aide de la convention de nommage `sql-console-role:<email>` ont préséance sur les rôles attribués via la console. Pour en savoir plus, consultez [Gérer les rôles de base de données](/fr/products/cloud/guides/security/cloud-access-management/manage-custom-roles#manage-database-roles).
</Note>
