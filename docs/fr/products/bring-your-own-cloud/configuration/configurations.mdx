...
Par défaut, le point de terminaison du serveur d’API Kubernetes de votre cluster BYOC est accessible depuis l’internet public. Sur AWS, l’accès est restreint par filtrage IP afin de n’autoriser que les adresses IP de la passerelle NAT de ClickHouse ; sur GCP et Azure, il est plutôt contrôlé par l’IAM du cloud (voir [Exposition du serveur d’API Kubernetes](/fr/products/bring-your-own-cloud/reference/network-security#kubernetes-api-server-exposure)). Pour renforcer la sécurité, vous pouvez limiter l’accès au serveur d’API Kubernetes afin qu’il soit accessible exclusivement via des connexions à un réseau privé.
