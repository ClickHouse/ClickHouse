...
Cette page présente les modifications de configuration les plus courantes après l’installation du connector. Pour chaque clé, sa valeur par défaut et sa signification, consultez la [référence de configuration](/fr/products/bring-your-own-cloud/connector/reference/configuration) ; pour les options de ligne de commande, consultez la [référence CLI](/fr/products/bring-your-own-cloud/connector/reference/cli). La création, le scaling et la suppression de services ne relèvent pas de la configuration ; consultez [services managés](/fr/products/bring-your-own-cloud/connector/managed-services).
...
<Note>
  La CLI publique `v0.16.0` ne propose ni `--managed` ni `--no-managed`. Avant d&#39;utiliser l&#39;un ou l&#39;autre de ces indicateurs pour modifier la configuration, vérifiez la [disponibilité du mode managé](/fr/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) et obtenez le build correspondant auprès de votre account team.
</Note>

Le mode managé exécute l&#39;executor, qui applique au cluster le cycle de vie de service piloté par ClickHouse Cloud. `init` l&#39;active lors de l&#39;inscription et renseigne l&#39;identity du cluster (voir [activer le mode managé](/fr/products/bring-your-own-cloud/connector/managed-services#enable-managed-mode)) ; les keys ci-dessous sont celles que vous serez susceptible de modifier par la suite. Pour modifier la posture configurée, relancez `init` avec `--force` et `--managed` ou `--no-managed`. Comme toute exécution d&#39;`init`, celle-ci nécessite un entry point : `--handoff` si vous avez conservé le bundle, ou `--enroll` avec un token récent, comme décrit dans [nouvelles exécutions et recovery](/fr/products/bring-your-own-cloud/connector/operations#re-runs-and-recovery).

Sur une VM, `--no-managed` désactive l&#39;executor dans la configuration mais n&#39;arrête ni ne désactive un systemd unit déjà en cours d&#39;exécution. Arrêtez-le explicitement avant de modifier la configuration :

```bash
sudo systemctl disable --now clicklink-executor
```

Sur Kubernetes, définir `executor.enabled: false` dans les values et appliquer la modification avec `helm upgrade` supprime le Deployment de l&#39;executor. Sur les deux cibles, le grant sur le cluster reste en place jusqu&#39;à ce que vous le supprimiez ; suivez [arrêter le mode managé](/fr/products/bring-your-own-cloud/connector/reference/faq#stop-managed-mode) pour supprimer également cet access.
...
Les services créés par l&#39;executor s&#39;enregistrent eux-mêmes : en mode managé, vous n&#39;avez donc jamais à les ajouter ici. Un déploiement de connector distinct, exécuté sans l&#39;executor, observe le ClickHouse que vous exploitez vous-même ; passez `--ch-user-suffix` lors de son `init` lorsque les deux déploiements partagent une instance. Cette section concerne ce déploiement. Chaque entrée sous `instances` désigne un endpoint ClickHouse en protocole natif depuis lequel le connector lit : `host`, `port`, `database`, `secure`, `cluster`, et sur Kubernetes `namespace` ; `max_open_conns` et `max_idle_conns` limitent le connection pool. Les identifiants ne figurent jamais dans la configuration : chaque composant résout son ClickHouse user en lecture seule à partir du bundle d&#39;accès créé lors du provisioning.

<Tabs>
  <Tab title="Kubernetes" id="instances-kubernetes">
    Ajoutez l&#39;instance au mappage `instances` de premier niveau dans `clicklink-values.yaml` (les mêmes champs que le registre de la VM), puis ajoutez son espace de noms à `networkPolicy.clickhouseNamespaces`, mis en correspondance via le label `kubernetes.io/metadata.name` de l&#39;espace de noms. Le chart génère le mappage dans la ConfigMap `clicklink-instance-registry` que les deux démons lisent. Cela ne fonctionne que tant que `executor.enabled` vaut `false` ; lorsque l&#39;executor est activé, la génération échoue car c&#39;est l&#39;executor qui détient cette ConfigMap.
...
Une liste d&#39;autorisation d&#39;adresses e-mail d&#39;opérateurs conditionne les sessions gérées par la passerelle : chaque requête envoyée à la passerelle de sessions doit inclure un jeton d&#39;identité OIDC à courte durée de vie dont l&#39;adresse e-mail attestée figure dans cette liste. Une liste d&#39;autorisation vide ferme la passerelle : personne ne peut alors ouvrir de session via celle-ci. Sur une VM, root sur l&#39;hôte peut également gérer les sessions directement via le fichier de sessions local ; la liste d&#39;autorisation ne régit que le chemin passant par la passerelle. Consultez les [sessions de support](/fr/products/bring-your-own-cloud/connector/support-sessions) pour connaître le modèle de confiance complet.
...
Lorsque la passerelle de sessions est activée, une règle d&#39;egress supplémentaire entre en jeu : `clctl.gateway.jwksEgressCIDRs`. Le troubleshooter récupère le JWKS de votre identity provider pour valider les jetons d&#39;opérateur ; sous un refus par défaut, une liste vide bloque donc toute vérification de jeton :
...
L&#39;exemple en mode managé ci-dessous nécessite un build avec `--managed`, absent de la version publique `v0.16.0`. Vérifiez la [disponibilité du mode managé](/fr/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) avant de l&#39;utiliser ; mettre en miroir la release publique n&#39;ajoute pas cette prise en charge.
...
<Tabs>
  <Tab title="Kubernetes" id="storage-kubernetes">
    Deux composants conservent un état sur des PersistentVolumeClaims. Celui du troubleshooter contient l&#39;état de session et la piste d&#39;audit :
  ...
  <Tab title="VM Linux" id="storage-vm">
    Le scraper met les metrics en tampon sous `/var/lib/clicklink/buffer` afin de garantir une livraison au moins une fois tant que votre connector endpoint n&#39;est pas joignable. Il conserve jusqu&#39;à 168 heures ou 1024 Mo de données et les téléverse à un débit limité par défaut à 1 Mo/s :
