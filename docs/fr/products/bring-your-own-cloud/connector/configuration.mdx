...
Le mode Managed exécute l&#39;executor, qui applique au cluster le cycle de vie de service piloté par ClickHouse Cloud. `init` l&#39;active lors de l&#39;enrollment et renseigne l&#39;identity du cluster (voir [activer le mode Managed](/fr/products/bring-your-own-cloud/connector/managed-services#enable-managed-mode)) ; les keys ci-dessous sont celles que vous serez susceptible de modifier par la suite. Pour activer le mode Managed sur un connector installé sans celui-ci, ou pour le désactiver, relancez `init` avec `--force` et `--managed` ou `--no-managed`. `--no-managed` arrête l&#39;executor mais laisse en place son grant sur le cluster ; supprimez-le comme décrit dans [arrêter le mode Managed](/fr/products/bring-your-own-cloud/connector/reference/faq#stop-managed-mode). Comme toute exécution d&#39;`init`, celle-ci nécessite un entry point : `--handoff` si vous avez conservé le bundle, ou `--enroll` avec un token récent, comme décrit dans [nouvelles exécutions et recovery](/fr/products/bring-your-own-cloud/connector/operations#re-runs-and-recovery).
...
Les services créés par l&#39;executor s&#39;enregistrent eux-mêmes : en mode managed, vous n&#39;avez donc jamais à les ajouter ici. Un déploiement de connector distinct, installé avec `--no-managed`, observe le ClickHouse que vous exploitez vous-même ; passez `--ch-user-suffix` lors de son `init` lorsque les deux déploiements partagent une instance. Cette section concerne ce déploiement. Chaque entrée sous `instances` désigne un endpoint ClickHouse en protocole natif depuis lequel le connector lit : `host`, `port`, `database`, `secure`, `cluster`, et sur Kubernetes `namespace` ; `max_open_conns` et `max_idle_conns` limitent le connection pool. Les credentials ne figurent jamais dans la configuration : chaque composant résout son ClickHouse user en lecture seule à partir du bundle d&#39;accès créé lors du provisioning.
...
Lorsque l&#39;endpoint de votre connector se trouve derrière une CA privée au sein de votre périmètre, transmettez `--api-private-ca` à `init`. Cette option configure `api.tls.caFile: /etc/clicklink/secrets/mtls/ca.crt`, et le connector vérifie l&#39;endpoint par rapport aux racines système ainsi qu&#39;à la chaîne de CA de votre bundle d&#39;onboarding. Sur une VM, l&#39;équivalent est `api.tls.ca_file` dans `/etc/clicklink/config.yaml` : `init` installe la chaîne du bundle dans `/etc/clicklink/tls/ca.crt`, et le connector l&#39;ajoute aux racines système de la même manière. Pour un onboarding entièrement air-gapped et la signature de certificats, consultez [l&#39;onboarding](/fr/products/bring-your-own-cloud/connector/onboarding).
