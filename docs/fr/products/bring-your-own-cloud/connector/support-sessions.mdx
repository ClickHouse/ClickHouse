...
Les sessions d’assistance vous permettent d’accorder à ClickHouse un accès temporaire à des fins de diagnostic via le ClickHouse Connector. Cette page explique ce qu’est une session, comment l’activer et la désactiver, ce que les ingénieurs du support ClickHouse peuvent faire pendant celle-ci et comment l’auditer.

## Qu&#39;est-ce qu&#39;une session d&#39;assistance ? {#what-a-support-session-is}

Une session d&#39;assistance est une fenêtre de temps limitée pendant laquelle l&#39;outil de diagnostic accepte les commandes des ClickHouse support engineers. En dehors d&#39;une session, l&#39;outil de diagnostic refuse toute commande, même lorsque son WebSocket sortant est connecté. Il n&#39;existe aucune autre voie d&#39;exécution : l&#39;outil de diagnostic n&#39;exécute rien sans session, et ClickHouse ne peut pas en ouvrir une à votre place. ClickHouse Cloud ne se connecte jamais à votre environnement ; il reçoit uniquement ce que l&#39;outil de diagnostic envoie sur son canal sortant.
...
Les sessions expirent automatiquement. La durée par défaut est de 4 heures ; `session enable --duration` accepte une durée pouvant aller jusqu’à 24 heures. Lorsque la session expire ou dès que vous exécutez `session disable`, l’outil de diagnostic cesse d’accepter les commandes. La désactivation constitue la voie de révocation immédiate : aucun redémarrage ni aucune coordination avec ClickHouse.

## Liste d’autorisation des opérateurs {#operator-allowlist}

Chaque appel vers la passerelle est autorisé selon une liste d’adresses e-mail d’opérateurs. La comparaison porte sur l’adresse e-mail attestée par le jeton OIDC validé, et jamais sur ce que le client déclare à son sujet.

* **Kubernetes :** définissez `clctl.gateway.allowedOperators` dans votre fichier de surcharge de valeurs. Le chart génère la liste dans une ConfigMap ; ainsi, une modification des valeurs suivie de `helm upgrade` renouvelle la liste d’autorisation sans redémarrer le pod.
...
Sur les deux cibles, la passerelle relit le fichier lors de la requête suivante dès que 30 secondes se sont écoulées depuis la dernière lecture ; les modifications prennent donc effet sans redémarrage.
