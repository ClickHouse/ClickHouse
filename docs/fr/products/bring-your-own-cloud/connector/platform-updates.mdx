...
Un cluster managé exécute une petite couche plateforme dont dépendent les services ClickHouse. ClickHouse Cloud propose des mises à jour de cette couche ; l&#39;executor n&#39;applique rien tant que vous ne les avez pas approuvées une à une avec vos propres credentials de cluster. Cette page explique ce qu&#39;est une mise à jour de plateforme, comment en approuver une sur une VM ou sur une installation Kubernetes, combien de temps une approbation reste valable et ce qu&#39;elle autorise.
...
<Steps>
  ...
  <Step title="Exécuter approve" id="run-approve">
    ...
    `approve` nécessite un contexte kube disposant des droits cluster-admin sur le cluster managé (`--context <name>` lorsque votre kubeconfig en contient plusieurs). Lorsqu&#39;un chart réside dans Amazon ECR, elle nécessite également des AWS credentials capables de le lire. `--dry-run` effectue le rendu et liste la moitié « permissions » sans rien appliquer ni générer.
    ...
    <Tabs>
      ...
      <Tab title="Kubernetes" id="approve-kubernetes">
        `approve` lit le bundle mis en attente via le port-forward de l&#39;étape précédente. La commande livre le bundle de jetons sous forme de Secret dans le namespace du connecteur, que le chart monte dans le pod. Exécutez-la depuis un poste de travail dont le kubeconfig atteint le cluster, avec une copie de la configuration du connecteur. Le chart en rend une dans la ConfigMap de l&#39;executor ; `approve` en a besoin pour le préfixe de namespace et n&#39;y lit aucun credential.
