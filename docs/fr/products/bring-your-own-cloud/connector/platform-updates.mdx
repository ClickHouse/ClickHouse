...
Un cluster managé exécute une petite couche plateforme dont dépendent les services ClickHouse. ClickHouse Cloud propose des mises à jour de cette couche ; l&#39;executor n&#39;applique rien tant que vous ne les avez pas approuvées une à une avec vos propres identifiants de cluster. Cette page explique ce qu&#39;est une mise à jour de plateforme, comment en approuver une sur une VM ou sur une installation Kubernetes, combien de temps une approbation reste valable et ce qu&#39;elle autorise.

<Note>
  La CLI publique `v0.16.0` ne comporte pas de commande `platform`, ni `platform approve` ni `platform reset`. Ni le flux de travail VM Linux ni le flux de travail Kubernetes décrits ci-dessous ne sont disponibles dans cette release. L&#39;approbation sous Kubernetes nécessite en outre la prise en charge, par la CLI et par le chart de l&#39;executor, du bundle de jetons basé sur un Secret (`--secret-namespace`). Confirmez la [disponibilité du mode managé](/fr/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) auprès de votre équipe de compte avant d&#39;utiliser l&#39;un ou l&#39;autre de ces flux de travail.
</Note>

## Qu&#39;est-ce qu&#39;une mise à jour de plateforme {#what-a-platform-update-is}

La couche plateforme se compose de trois composants : le snapshot controller, le ClickHouse Operator et les collectors de supervision. Ils s&#39;installent dans cet ordre, car chacun a besoin des définitions de custom resource du précédent. À leurs côtés se trouve une StorageClass nommée `gp3-encrypted` (volumes gp3 chiffrés) qu&#39;utilisent les services.

Un bundle de plateforme est le manifest généré par ClickHouse Cloud pour votre environnement. Il fige les versions de chart et d&#39;image de ces composants et désigne le registry depuis lequel ils sont récupérés. Chaque bundle est identifié par le sha256 de ce manifest. ClickHouse Cloud transmet le bundle à l&#39;executor via son canal de commandes, et l&#39;executor le prépare en attente de votre approbation.

L&#39;application d&#39;un bundle se divise en deux volets :
...
Les deux procédures d&#39;approbation ci-dessous nécessitent un [build avec prise en charge de la plateforme](/fr/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) ; la version publique `v0.16.0` ne comporte pas de commande `platform`. L&#39;approbation Kubernetes exige également une prise en charge correspondante dans le CLI et le chart pour la distribution des Secret.

L&#39;authentication auprès du registry dépend du mode de distribution d&#39;image enregistré pour votre environnement :

* **Accès direct au registry de ClickHouse.** Exécutez l&#39;approbation sur la VM EC2 du connector. La commande `approve` comme la synchronisation de plateforme de l&#39;executor utilisent les identifiants de l&#39;instance metadata service EC2 pour assumer le rôle ECR puller en lecture seule de votre environnement. Ils s&#39;appuient sur ce rôle pour la connexion au chart ; l&#39;executor s&#39;en sert également pour vérifier l&#39;existence des images de plateforme. Les profils AWS, les identifiants d&#39;environnement ou les identifiants SSO présents sur un poste de travail ne remplacent pas le profil d&#39;instance. Votre kubeconfig fournit toujours les identifiants cluster-admin distincts nécessaires à l&#39;application du volet permissions.
...
Réservez la procédure depuis un poste de travail Kubernetes décrite ci-dessous aux deployments avec registry mirroré et approbation de plateforme Kubernetes activée. Pour l&#39;accès direct, demandez à votre account team de confirmer que votre environnement d&#39;exécution est pris en charge et dispose du profil d&#39;instance EC2 requis avant de poursuivre.

<Steps>
  <Step title="Recherchez le bundle préconfiguré" id="find-the-staged-bundle">
    ClickHouse Cloud envoie chaque bundle proposé à l&#39;executor avant toute autre opération. L&#39;executor le met en attente en tant que bundle pending et signale son sha256 dans le heartbeat. Un seul bundle en attente existe à la fois ; une proposition plus récente le remplace. L&#39;executor refuse alors la synchronisation et l&#39;enregistre comme une commande failed dont le résultat se termine par la commande à exécuter.

    ```bash
    clicklink clctl commands list --status failed --action sync_platform
    ```

    Le champ `result` se termine par `run: clctl platform approve (pending bundle sha <sha256>)`. Sur Kubernetes, l&#39;indication est `run: clctl platform approve --secret-namespace <connector-namespace> (pending bundle sha <sha256>)`. Une création ayant rencontré une définition de custom resource manquante comporte la même ligne `clctl platform approve` dans sa propre commande échouée ainsi que dans l&#39;erreur affichée par `clicklink clctl instances create --wait`. Votre account team ClickHouse vous prévient également lorsqu&#39;une mise à jour de plateforme est proposée.
  ...
  <Step title="Exécuter approve" id="run-approve">
    `approve` sans flags lit le bundle mis en attente et le hache, de sorte que ce que vous approuvez correspond exactement à ce que l&#39;executor a reçu. La commande effectue le rendu de chaque chart du bundle exactement comme le fera l&#39;executor et applique la moitié « permissions » avec le contexte kube que vous choisissez. Elle crée l&#39;identité `pcm-platform` si nécessaire et génère son jeton. Elle n&#39;émet aucune requête vers votre endpoint de connecteur.
...
Une approbation émet un jeton pour l&#39;identity `pcm-platform`, valable 2 heures par défaut (`--ttl` permet de modifier cette durée). L&#39;executor ne renouvelle jamais ce jeton. À son expiration, l&#39;executor ne peut plus agir sur la couche plateforme et refuse la synchronisation de plateforme suivante en affichant de nouveau le message d&#39;approbation. Il s&#39;agit du comportement attendu, et non d&#39;une défaillance ; il ne dépend pas non plus de la connectivité : une approbation accordée alors que le connector est hors ligne expire tout de même selon sa propre horloge.
...
Vous appliquez le volet permissions : elle porte donc votre autorité. `approve` y appose le sha256 du bundle, et l&#39;executor vérifie, avant de toucher à quoi que ce soit, que chaque objet de permission du bundle qui lui a été transmis est présent et approuvé. L&#39;executor n&#39;en applique aucun élément.
...
La CLI publique `v0.16.0` ne comporte pas de commande `platform reset` ; vérifiez sa [disponibilité](/fr/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) avant d&#39;appliquer cette procédure.
...
`reset` prend le manifest de plateforme généré sous forme de fichier (`--bundle`) ; il ne lit pas le bundle préparé. `--dry-run` vérifie la présence de services et affiche le plan sans modifier le cluster.