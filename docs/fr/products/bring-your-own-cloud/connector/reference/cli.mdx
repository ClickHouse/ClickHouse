...
| `--config <path>`                                                                                   | Fichier de configuration du connector (variable d&#39;environnement `CLCTL_EXECUTOR_CONFIG` ; `/etc/clicklink/config.yaml` par défaut lorsqu&#39;il existe). Fournit l&#39;endpoint, ses credentials et l&#39;identity de l&#39;environnement.                                                                                               |
| `--connector-namespace <ns>`                                                                        | Kubernetes uniquement, lorsque cet hôte ne dispose ni du fichier de configuration du connector ni des fichiers de credentials : le namespace du connector dont la ConfigMap et les Secrets les fournissent (`clicklink` par défaut). Chaque lecture du cluster est annoncée.                                                                 |
...
| `--hmac-secret-name <name>`, `--mtls-secret-name <name>`                                            | Secrets contenant les credentials d&#39;API et le certificat client lorsque les fichiers sont absents (valeurs par défaut `clicklink-hmac` et `clicklink-mtls`). La paire de certificat est conservée en mémoire.                                                                                                                            |
...
Autorise une modification de plateforme avec vos propres identifiants de cluster. La commande lit le bundle préparé par l&#39;executor, en calcule le hachage et effectue le rendu de chaque chart qu&#39;il contient exactement comme le fera l&#39;executor. Elle applique la moitié « permissions » avec le contexte kube que vous choisissez, garantit l&#39;identité `pcm-platform` de l&#39;executor et émet son jeton. Elle n&#39;envoie aucune requête à votre endpoint de connecteur. Si un chart du bundle réside dans Amazon ECR, elle a besoin d&#39;identifiants AWS permettant de le lire. La moitié « permissions » regroupe les Namespaces, CustomResourceDefinitions, ServiceAccounts, ClusterRoles et bindings, Roles et bindings, configurations de webhook et d&#39;admission, PriorityClasses, ainsi que la StorageClass. L&#39;executor n&#39;applique ensuite que la moitié « charges de travail », et uniquement tant que ce jeton reste valide. Voir [mises à jour de plateforme](/fr/products/bring-your-own-cloud/connector/platform-updates).

| Indicateur                | Description                                                                                                                                                                                                                          |
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `--config <path>`         | Fichier de configuration du connecteur (variable d&#39;environnement `CLCTL_EXECUTOR_CONFIG`). Fournit le préfixe de namespace ; sur un poste de travail, une copie de la configuration du connecteur.                               |
...
| `--secret-namespace <ns>` | Installations Kubernetes : lit le bundle préparé depuis l&#39;API locale de l&#39;executor et livre le bundle de jeton sous forme de Secret dans ce namespace (le namespace du connecteur) plutôt que dans `<output-dir>/_platform`. |
