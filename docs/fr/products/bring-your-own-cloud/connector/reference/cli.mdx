...
| `--instance <spec>`         | ClickHouse instance que vous exploitez vous-même, sous forme de paires `key=value` séparées par des virgules (`name`, `host`, `port`, `secure`, `database`, `namespace`, `cluster`) ; répétable. Ignore les prompts interactifs relatifs à l&#39;instance. Refusé en mode managé sur Kubernetes : c&#39;est un déploiement distinct installé avec `--no-managed` qui observe une telle instance. |
...
| `--managed`                 | Active le mode managé : exécute l&#39;executor afin que ClickHouse Cloud gère le cycle de vie des services ClickHouse de ce cluster ; ignore le prompt. Nécessite un EKS cluster Amazon. Indisponible dans la version publique `v0.16.0` ; voir [availability](/fr/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability).                                                   |
| `--no-managed`              | Laisse le mode managé (l&#39;executor) désactivé ; ignore le prompt. Indisponible dans la version publique `v0.16.0` ; voir [availability](/fr/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability).                                                                                                                                                                       |
...
| `--skip-provision`          | Stage uniquement : ignore le provisionnement des accès ClickHouse par rôle (et, sur la cible systemd, l&#39;activation et la vérification de l&#39;unit). Exécutez `clicklink clctl {scraper,troubleshoot} access provision` séparément. `--target helm` applique malgré tout le grant de l&#39;executor ; une VM l&#39;ignore également.                                                            |
...
Le flux de travail `init --managed` ci-dessous nécessite un [build prenant en charge le mode managed](/fr/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) ; la version publique `v0.16.0` n&#39;expose pas cet indicateur.

`init --managed` applique le grant à l’échelle du cluster de l&#39;executor avec vos identifiants. Lorsque ce n&#39;est pas possible, `init` affiche les commands à exécuter depuis un kubeconfig disposant des droits d&#39;admin du cluster : `access apply` seul sur une VM (le bundle est déjà préparé), ou `access grant` puis `access apply` sur Kubernetes. Voir [onboarding](/fr/products/bring-your-own-cloud/connector/onboarding#install-and-enroll).
...
Applique le bundle `grant` généré à l&#39;échelle du cluster : le Namespace d&#39;origine du ServiceAccount, le ServiceAccount, le ClusterRole et le ClusterRoleBinding, ainsi que la policy d&#39;admission. Il génère ensuite le token du ServiceAccount dans le bundle. Il n&#39;enregistre aucun service.
...
Supprime ce que `prepare` a créé pour un service que ClickHouse Cloud a supprimé, à savoir : le rôle IAM, les buckets créés par `prepare`, le nom enregistré, l&#39;enregistrement du service dans l&#39;executor et, sur une VM, les ClusterRole et ClusterRoleBinding du service, le bundle d&#39;accès local ainsi que l&#39;entrée du registry. L&#39;executor doit être en cours d&#39;exécution et le service à l&#39;état `terminated` (ou `stale`) ; l&#39;exécution est refusée tant que le namespace du service contient encore un cluster ClickHouse. Les données et les sauvegardes sont conservées sauf demande contraire, et les buckets conservés reçoivent le nouveau tag `clicklink:retained-from=<instance>`. La commande lit tout avant de supprimer quoi que ce soit. Voir [supprimer un service](/fr/products/bring-your-own-cloud/connector/managed-services#delete-a-service).
...
Modifie un service en place. `--create-nlb` est le seul patch disponible. Il provisionne un NLB interne, sous la forme d&#39;un `Service` Kubernetes, afin qu&#39;un scraper situé en dehors du cluster puisse joindre les serveurs, sur AWS uniquement. L&#39;executor renseigne par backfill le host du scraper à partir du répartiteur de charge lors de sa prochaine synchronisation. Sans `--create-nlb`, la commande échoue avec le message `nothing to patch`.
...
La CLI publique `v0.16.0` ne comporte pas de commande `platform` ; ni le flux de travail d&#39;approbation VM ni celui de Kubernetes ne sont donc disponibles dans cette version. L&#39;approbation Kubernetes nécessite en outre la prise en charge, par la CLI et le chart, de la livraison par Secret décrite ci-dessous. Vérifiez la [disponibilité du mode managé](/fr/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) avant d&#39;utiliser l&#39;un ou l&#39;autre flux de travail.

Autorise une modification de plateforme avec vos propres identifiants de cluster. La commande lit le bundle préparé par l&#39;executor, en calcule le hachage et effectue le rendu de chaque chart qu&#39;il contient exactement comme le fera l&#39;executor. Elle applique la moitié « permissions » avec le contexte kube que vous choisissez, garantit l&#39;identity `pcm-platform` de l&#39;executor et émet son jeton. Elle n&#39;envoie aucune requête à votre connector endpoint. La moitié « permissions » regroupe les Namespaces, CustomResourceDefinitions, ServiceAccounts, ClusterRoles et bindings, Roles et bindings, configurations de webhook et d&#39;admission, PriorityClasses, ainsi que la StorageClass. L&#39;executor n&#39;applique ensuite que la moitié « charges de travail », et uniquement tant que ce jeton reste valide. Voir [mises à jour de plateforme](/fr/products/bring-your-own-cloud/connector/platform-updates).
...
| `--output-dir <path>`                             | racine d’accès dont le bundle `_platform` est supprimé (par défaut `/etc/clicklink/access/executor`).                                                    |
...
| Option                                                                                      | Description                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--provider <name>`                                                                         | Canal d&#39;exécution : `ssh`, `aws` (SSM) ou `gcp` (IAP) pour les VM distantes, ou `local` lors de l&#39;exécution directement sur la VM cible. Déduit des options propres à chaque fournisseur s&#39;il n&#39;est pas défini explicitement. Vaut `aws` par défaut lorsqu&#39;aucune option de canal n&#39;est définie, et `local` n&#39;est jamais déduit : sur la VM elle-même, passez donc toujours `--provider local`. |
