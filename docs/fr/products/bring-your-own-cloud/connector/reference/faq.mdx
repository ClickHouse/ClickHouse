...
Les workflows d&#39;inscription en mode managé, de création par le client et d&#39;approbation de la plateforme nécessitent une [prise en charge confirmée par votre account team](/fr/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) ; ils sont indisponibles dans la version publique `v0.16.0`.
...
Deux opérations requièrent votre intervention. La création d&#39;un service passe par vos commandes `clicklink clctl executor prepare` et `clicklink clctl instances create`, car vous seul pouvez créer les buckets et l&#39;IAM role. Les mises à jour des composants de la plateforme dans votre cluster attendent également votre intervention : l&#39;executor met en attente chaque bundle proposé, et votre commande `clicklink clctl platform approve` approuve le bundle en attente et génère pour celui-ci un jeton valable 2 heures ; voir [mises à jour de la plateforme](/fr/products/bring-your-own-cloud/connector/platform-updates).

Tout ce que fait l&#39;executor passe par la Kubernetes API. Ses écritures sont limitées par une policy d&#39;admission aux namespaces portant le préfixe de service (`ns-` par défaut), plus trois éléments dans son propre namespace : le renouvellement de son jeton, la ConfigMap du registry de services et, sur Kubernetes, le certificat client renouvelé dans le Secret `clicklink-mtls`. Ses lectures sur ces mêmes familles de ressources sont cluster-wide. Les mises à jour de plateforme approuvées s&#39;exécutent sous une identity distincte, `pcm-platform`, limitée aux namespaces de la plateforme. Les clusters que vous exploitez vous-même ne sont jamais modifiés : le scraper et l&#39;outil de diagnostic sont en lecture seule. Les portées exactes figurent sur la page [modèle de privilèges](/fr/products/bring-your-own-cloud/connector/reference/privilege-model).
...
Le scraper envoie les résultats d&#39;un ensemble fixe de system tables : `metric_log`, `asynchronous_metric_log`, `tables`, `warnings` et `server_settings` par défaut. Il envoie également des signaux de santé et de statut (heartbeats), l&#39;état des instances et des sauvegardes, ainsi que les métriques propres au connector. L&#39;ensemble de scrape par défaut exclut `system.query_log` : le texte SQL brut, ainsi que les literals ou données personnelles qu&#39;il contient, ne quittent donc jamais votre environnement par le chemin de scrape, sauf si vous l&#39;ajoutez. Pendant une session, l&#39;allowlist de tables par défaut inclut `system.processes`, qui expose le texte des requêtes en cours d&#39;exécution ; retirez-la de l&#39;allowlist si cela doit rester masqué.

L&#39;executor envoie le résultat de chaque commande qu&#39;il exécute ainsi que l&#39;état et le nombre de répliques de chaque service managé. Son heartbeat indique si son canal de commandes est connecté, les versions des composants de la plateforme, l&#39;empreinte du dernier bundle appliqué et le résultat de la dernière mise à jour de la plateforme. Sur une installation VM, le heartbeat indique aussi si son bundle d&#39;accès Kubernetes est `ok`, `expiring` ou `expired`. Lorsque vous créez un service, `clicklink clctl instances create` envoie l&#39;identifiant de votre environnement, le cloud et la région, le nom du service, ainsi que les noms de ses buckets et l&#39;ARN de son IAM role. Il envoie deux hachages du mot de passe du default user ; le mot de passe lui-même ne quitte jamais votre environnement.

Pendant une [session d’assistance](/fr/products/bring-your-own-cloud/connector/support-sessions) active, le troubleshooter renvoie également la sortie des commandes. Cette sortie est limitée aux tables ClickHouse figurant sur l&#39;allowlist et aux vues Kubernetes en lecture seule, y compris les logs de pods. Elle passe par une redaction (patterns intégrés pour les adresses IP, credentials, jetons et clés, ainsi que les vôtres) avant d&#39;être envoyée. Les données de vos tables, vos sauvegardes et votre query history (`system.query_log`, `system.text_log`) restent dans votre environnement. Trois éléments, en revanche, en sortent :

* les lignes des tables d&#39;historique de métriques figurant sur l&#39;allowlist (`system.metric_log`, `system.asynchronous_metric_log`) sont transmises à chaque scrape
* le texte des requêtes en cours est visible dans une session via `system.processes`, sauf si vous la retirez de l&#39;allowlist
...
<Note>
  La CLI publique `v0.16.0` ne propose ni `--managed` ni `--no-managed`. Le choix à l&#39;installation décrit ci-dessous nécessite un build avec [prise en charge du mode managé](/fr/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability). L&#39;arrêt d&#39;un executor existant s&#39;effectue via les instructions Helm ou systemd ci-dessous et ne requiert aucun de ces deux indicateurs d&#39;init.
</Note>
...
Les services créés par ClickHouse continuent de fonctionner, mais ClickHouse Cloud ne peut plus les redimensionner, les mettre à niveau, les sauvegarder ni les supprimer, et les commandes en attente les concernant échouent. Les mises à jour de la plateforme ne s&#39;appliquent jamais tant que vous n&#39;exécutez pas `clicklink clctl platform approve`. Tant que cette commande n&#39;est pas exécutée, la couche plateforme reste figée, alors que le reste du mode managé continue de fonctionner.
...
L&#39;executor arrête le service, supprime ses resources Kubernetes et supprime son namespace. Il signale que le service est terminé dès que le namespace a disparu. Tout ce qui se trouve en dehors de Kubernetes est conservé : le bucket de données, le bucket de sauvegarde et l&#39;IAM role du service. L&#39;executor ne détient aucune credential pour vos buckets ni pour votre IAM : rien en dehors de Kubernetes n&#39;est donc touché.
...
La création d&#39;un service client nécessite [une prise en charge correspondante du CLI et du déploiement](/fr/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) et n&#39;est pas disponible dans la version publique `v0.16.0`.
...
2. **Arrêter le mode géré.** Arrêtez l&#39;executor et retirez son grant comme décrit dans [Comment empêcher ClickHouse de modifier mes clusters ?](#stop-managed-mode). Les mises à jour de la plateforme nécessitent déjà votre approbation au cas par cas : refuser votre approbation les interrompt.
...
4. **Couper la connectivité avec ClickHouse Cloud.** Bloquez le trafic sortant vers l&#39;endpoint de votre connecteur au niveau réseau, ou videz `networkPolicy.allowEgressCIDRs` avec un CNI appliquant les règles. Le connecteur est uniquement sortant, donc ClickHouse Cloud ne dispose d&#39;aucun chemin entrant pour le rétablir. Les lectures locales sur votre instance ClickHouse se poursuivent jusqu&#39;à ce que vous arrêtiez ou désinstalliez les workloads, ce qui constitue l&#39;arrêt complet.
5. **Révoquer les credentials.** Supprimez les utilisateurs ClickHouse `pcm_scraper` et `pcm_troubleshooter`, puis supprimez les Secrets du connecteur (Kubernetes) ou les fichiers sous `/etc/clicklink` (VM).
6. **Supprimer entièrement le connecteur.** Consultez les [opérations](/fr/products/bring-your-own-cloud/connector/operations).
...
* Avec un [build prenant en charge l&#39;approbation de plateforme](/fr/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability), les bundles de plateforme peuvent être transmis hors bande, puis fournis sous forme de fichier à `clicklink clctl platform approve --bundle`. La version publique `v0.16.0` ne dispose pas de la commande `platform`.
