...
## Disponibilité du mode managé {#managed-mode-availability}

<Warning>
  Au 22/09/2026, l&#39;installateur public fournit la version `v0.16.0`, qui ne comporte ni les indicateurs `init --managed` et `init --no-managed`, ni la commande `platform`. L&#39;approbation de plateforme sur les installations VM Linux et Kubernetes, ainsi que la création de service client (`instances create --from-prepare`), ne sont pas encore disponibles dans une release publique. L&#39;approbation de plateforme sous Kubernetes nécessite en outre la prise en charge de `platform approve --secret-namespace` par le CLI et le chart.

  Avant de suivre les flux de travail en mode managé, demandez à votre account team ClickHouse de confirmer la prise en charge de votre cible de déploiement et de vous fournir le CLI, le chart ou l&#39;image de connector correspondants, ainsi que les instructions d&#39;installation. La création de service client requiert également une prise en charge sur votre connector endpoint et dans ClickHouse Cloud ; un CLI plus récent ne suffit pas à lui seul. Les commandes d&#39;installation publiques ci-dessous n&#39;installent pas, à ce jour, de release prenant en charge ces flux de travail.
</Warning>
...
ClickHouse Cloud délivre ensuite votre connector endpoint ainsi qu&#39;un jeton d’inscription à usage unique. Ce token identifie votre environnement : aucun identifiant supplémentaire n&#39;est donc à saisir lors de l&#39;enrôlement.

### Pour toute installation {#prerequisites-every-install}

* **Une build prenant en charge le [mode managé](#managed-mode-availability)** pour les instructions relatives au mode managé ci-dessous. La version publique `v0.16.0` ne dispose ni de `--managed` ni de `--no-managed`.
...
* **Un EKS cluster Amazon** pour le mode managé. `init` refuse le mode managé sur tout autre cluster et indique `--no-managed` comme moyen d&#39;installer sans l&#39;executor.
...
* **Des AWS credentials** sur le poste de travail (environnement ou profil) disposant de `eks:DescribeCluster` et `ec2:DescribeVpcs`. `init` s&#39;en sert pour lire les blocs CIDR du VPC de votre cluster nécessaires à la NetworkPolicy. En cas d&#39;échec de la lookup, un avertissement est émis et la liste reste à compléter par vos soins.
...
<Note>
  Sur une VM, `--skip-provision` est le seul moyen de contourner l&#39;exigence relative au Kubernetes API server, et il ne s&#39;applique qu&#39;à l&#39;étape. Il ignore le provisioning du ClickHouse user, le grant de l&#39;executor, l&#39;activation de l&#39;unité et la vérification : à lui seul, il ne produit donc pas de connector en fonctionnement. Sur Kubernetes, il ignore uniquement le provisioning du ClickHouse user ; le grant de l&#39;executor reste appliqué, car l&#39;executor du chart ne peut pas être planifié sans le ServiceAccount granted.
</Note>

## Installation et inscription {#install-and-enroll}

Vérifiez d&#39;abord la [disponibilité du mode managé](#managed-mode-availability) auprès de votre équipe de compte. Le programme d&#39;installation public ci-dessous fournit la version `v0.16.0`, qui ne permet pas d&#39;exécuter les commandes d&#39;inscription en mode managé présentées dans ce guide.
...
<Steps>
  <Step title="Obtenez l’endpoint et le jeton d’inscription de votre connecteur" id="get-endpoint-and-token">
    ...
    Le jeton est à usage unique et expire rapidement : procédez donc à l&#39;enrollment peu après l&#39;avoir reçu. Traitez-le comme un secret : la CLI le lit depuis une invite masquée ou depuis la première ligne de stdin, jamais depuis les arguments de ligne de commande, le disque ou les logs. S&#39;il expire entre-temps, demandez-en un nouveau à votre account team ClickHouse.
  </Step>

  <Step title="Installer et vérifier la CLI" id="install-and-verify-the-cli">
    Une seule commande installe un binaire `clicklink` vérifié. Elle détecte votre plateforme (macOS ou Linux, amd64 ou arm64), télécharge la version actuelle et vérifie la somme de contrôle SHA-256 ainsi que, si cosign est installé, la signature de la version. Elle installe le binaire dans `/usr/local/bin` si ce répertoire est accessible en écriture, sinon dans `~/.local/bin` (ajoutez-le à votre `PATH` si nécessaire ; `--bin-dir` permet de le surcharger). Pour une installation Kubernetes, exécutez-la depuis n&#39;importe quel poste de travail disposant d&#39;un accès kubeconfig au cluster :

    ```bash
    curl -fsSL https://releases.clicklink.clickhouse.com/install.sh | bash
    ```

    Pour une installation sur VM, exécutez le même script sur l&#39;hôte avec `--host`. Une fois le téléchargement vérifié, il crée également l&#39;utilisateur système `clicklink`, les répertoires `/etc/clicklink`, `/var/lib/clicklink` et `/var/log/clicklink`, ainsi que les unités systemd des trois composants. Il initialise `/etc/clicklink/redaction-patterns.yaml` avec un ensemble de patterns de départ (conservé s&#39;il en existe déjà un), si bien que l&#39;étape suivante démarre directement à l&#39;enrôlement :
  ...
  <Step title="Enregistrer et installer le connecteur" id="enroll-and-install">
    Ces commandes nécessitent la prise en charge du build et du déploiement décrite sous [disponibilité du mode Managed](#managed-mode-availability). La version publique `v0.16.0` ne comporte ni `--managed` ni `--no-managed`.

    L&#39;enrollment tient en une seule commande. Elle consomme votre token, obtient un certificat client signé ainsi que le grant cluster-wide de l&#39;executor, puis installe et vérifie le connector.
  ...
  <Step title="Nettoyer" id="clean-up">
    L&#39;enrollment bundle `handoff.yaml` (mode `0600`, dans le répertoire de travail) existe afin que les ré-exécutions et la recovery pendant l&#39;installation ne nécessitent jamais un second jeton. Il contient l&#39;API secret du connector en plaintext : supprimez-le donc dès que l&#39;installation est vérifiée :
    ...
    Le connector en cours d&#39;exécution conserve sa propre copie des credentials : aucun aspect opérationnel ne dépend donc de ce fichier. Les mises à niveau et les modifications de configuration ne le requièrent jamais. Si vous avez de nouveau besoin d&#39;exécuter `init` par la suite, demandez un nouvel enrollment token à votre account team ClickHouse, puis lancez `init --enroll --force`.
  </Step>
</Steps>
...
Si `init` rejette `--managed` ou `--no-managed` comme indicateur inconnu, vérifiez la version de votre CLI : la version publique `v0.16.0` ne prend en charge ni l&#39;un ni l&#39;autre. Obtenez un [build avec prise en charge du mode managé](#managed-mode-availability) auprès de votre account team avant de suivre les instructions de recovery du mode managé ci-dessous.

Relancez la même commande. `init` est idempotente : les relances convergent vers le même état, conservent votre config et vos fichiers préparés, et ignorent le travail déjà effectué. Lorsqu&#39;une étape échoue en cours de route, la CLI affiche les commandes de récupération exactes adaptées à votre situation ; elles peuvent être répétées sans risque.

Si l&#39;enrôlement est refusé, c&#39;est que le jeton a déjà été utilisé, ou qu&#39;il est invalide ou expiré. Pour un jeton déjà utilisé, relancez avec `--handoff handoff.yaml`, qui existe jusqu&#39;à l&#39;étape de nettoyage ; sinon, demandez un nouveau jeton à votre account team ClickHouse. Si l&#39;enrôlement échoue sur une erreur de transport, relancez la même commande. Si la relance est refusée, c&#39;est que la première tentative a bien consommé le jeton : demandez-en un nouveau.
...
`--force` constitue la réinitialisation explicite, et non une simple nouvelle tentative de routine. Cette option écrase la config ou la superposition de values conservée, régénère la client key et remplace un certificat client non expiré (une réponse `409` de l&#39;endpoint de signature indique qu&#39;il en existe déjà un). L&#39;UUID du cluster du connector est préservé lors d&#39;un `--force`, tout comme sa posture de mode managé. Les réexécutions sans surveillance conservent cette posture et, sur un terminal, le prompt la propose par défaut, sauf si vous passez `--managed` ou `--no-managed`. Utilisez `--force` pour effectuer une rotation des credentials ou remplacer un certificat, et consultez la page [opérations](/fr/products/bring-your-own-cloud/connector/operations) pour le modèle complet de réexécution et de recovery.