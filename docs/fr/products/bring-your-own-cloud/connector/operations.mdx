...
Réexécutez le programme d’installation sur l’hôte. Il télécharge et vérifie la nouvelle version comme lors de l’[onboarding](/fr/products/bring-your-own-cloud/connector/onboarding). Il sauvegarde le binaire précédent, installe les systemd units actuels et préserve vos motifs de masquage actifs ainsi que votre fichier d’environnement. Redémarrez ensuite les démons :
...
Chaque démon expose un point de terminaison `/livez` sur son port de santé. Le champ JSON `status` du corps de la réponse indique l’état de santé, et non le code d’état HTTP ; vérifiez donc le corps plutôt que le code `200`. Les métriques Prometheus sont exposées sur `/metrics` sur le même port ; il n’y a pas de port de métriques distinct. Ports par défaut pour les deux cibles :
...
Le comportement en mode managé décrit ici nécessite une [build prise en charge](/fr/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) ; la version publique `v0.16.0` ne dispose ni de `--managed` ni de `--no-managed`.
...
Sur Kubernetes, `init` crée à l’avance les Secrets `clicklink-hmac` et `clicklink-mtls` renouvelés ; le chart ne les génère pas. L’annotation de checksum du chart ne les couvre donc pas, et les pods en cours d’exécution conservent les identifiants avec lesquels ils ont démarré. Redémarrez chaque composant après la rotation :
...
Recréez les utilisateurs en lecture seule du connecteur pour chaque instance que vous avez enregistrée vous-même. Dans Kubernetes, exécutez les commandes depuis votre poste de travail. `--apply-ch-grants` réapplique les privilèges régénérés dans le pod afin que les nouvelles informations d’identification parviennent à ClickHouse ; si l’utilisateur administrateur a un mot de passe, ajoutez `--ch-admin-password-stdin` et fournissez-le via un pipe :
...
Sur une VM, exécutez-les sur l’hôte en fournissant le mot de passe administrateur via un pipe ; sur une VM, le mot de passe est obligatoire sauf si vous passez `--ch-user-via cr` :
...
<Note>
  La CLI publique `v0.16.0` ne dispose ni de `--managed` ni de `--no-managed`. Le comportement de conservation et de remplacement du mode managé décrit ci-dessous nécessite un build avec [la prise en charge du mode managé](/fr/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) ; confirmez votre build auprès de votre account team avant de modifier ce mode pendant la récupération.
</Note>
...
**Connector hors ligne.** Lorsque le connector est arrêté ou injoignable, les services qu’il gère continuent de fonctionner. Le ClickHouse Operator de votre cluster les maintient actifs, et l’executor ne se trouve pas sur le data path. Les changements de cycle de vie nécessitent que l’executor soit connecté : aucun n’est donc appliqué tant qu’il ne s’est pas reconnecté. Au redémarrage, l’executor récupère depuis sa base de données de commandes celles qui étaient en cours lorsqu’il s’est arrêté. Une commande que ClickHouse Cloud transmet de nouveau reçoit une réponse issue du résultat enregistré plutôt que d’être exécutée deux fois ; seule une commande ayant échoué est réexécutée. Pour savoir ce qu’il advient des commandes de cycle de vie lorsqu’aucun executor n’est connecté, consultez [lorsque le connector est hors ligne](/fr/products/bring-your-own-cloud/connector/managed-services#when-the-connector-is-offline).
...
Le PersistentVolumeClaim de l&#39;executor porte l&#39;annotation `helm.sh/resource-policy: keep` : la désinstallation le conserve donc, avec la base de données de commandes et le registre de services qu&#39;il contient. Les Secrets créés par `init`, le Secret du platform bundle écrit lors de l&#39;approbation de la plateforme et le ConfigMap du registre de services maintenu par l&#39;executor n&#39;appartiennent pas non plus au chart et survivent eux aussi à la désinstallation. Supprimez-les explicitement, y compris la paire Secret d&#39;accès / ServiceAccount de chaque instance que vous avez enregistrée vous-même et de chaque service managé pour lequel vous avez provisionné le troubleshooter (nommée d&#39;après le service). `--ignore-not-found` couvre les objects qui n&#39;ont jamais été créés, comme le Secret du platform bundle avant une première approbation :
