...
En mode managé, le connector exécute un troisième component, l&#39;executor. C&#39;est par son intermédiaire que ClickHouse Cloud pilote les services ClickHouse dans votre Kubernetes cluster. Cette page décrit la création d&#39;un service, la lecture de son statut, ce que fait ClickHouse Cloud une fois le service créé, ainsi que la manière dont un service est retiré.
...
La création d&#39;un service nécessite deux commandes. `prepare` crée tout ce qui doit l&#39;être de votre côté ; `instances create` soumet la création à ClickHouse Cloud via votre connector endpoint. ClickHouse Cloud génère la définition du service et transmet la création à l&#39;executor par son canal outbound. L&#39;executor l&#39;applique à votre cluster et renvoie un compte rendu.
...
<Tabs>
  <Tab title="Kubernetes" id="troubleshooter-kubernetes">
    ...
    Ajoutez ensuite la paire Secret et ServiceAccount à `troubleshooter.accessBundles`, puis exécutez `helm upgrade`, comme indiqué à la section [ajout d&#39;instances ClickHouse](/fr/products/bring-your-own-cloud/connector/configuration#clickhouse-instances).
  </Tab>
