...
## Ce que fait le connecteur {#what-the-connector-does}

Le ClickHouse Connector est un composant que vous déployez dans votre propre environnement. Il permet à ClickHouse Cloud de gérer, superviser et prendre en charge des services ClickHouse dans un cluster Kubernetes que vous exploitez. ClickHouse Cloud ne se connecte jamais à votre environnement : le connecteur n&#39;établit que des connexions sortantes et agit sous des identités dont la portée est limitée aux services ClickHouse. Le connecteur est livré sous la forme d&#39;un binaire unique, `clicklink`, qui contient les démons ainsi que l&#39;outillage `clicklink clctl` permettant de les installer et de les exploiter.

Le connecteur comporte trois composants :
...
ClickHouse enregistre votre environnement lors de l&#39;onboarding et fournit votre connector endpoint ainsi qu&#39;un enrollment token à usage unique. Vous exécutez `clicklink clctl init --enroll` une seule fois : depuis un poste de travail disposant d&#39;un accès kubeconfig pour une installation Kubernetes, ou sur l&#39;hôte pour une installation sur VM Linux. Cette unique commande consomme le token, obtient un certificat client mTLS, déploie et démarre les démons, puis vérifie leur health.

`init` active le mode managé lorsque vous passez `--managed` ; dans un terminal, la commande pose la question et la réponse par défaut est oui. Cette activation déploie l&#39;executor et applique son octroi d’accès Kubernetes. Ce grant est un ServiceAccount `pcm-executor`. Une politique d&#39;admission limite ses écritures aux namespaces de service (préfixe `ns-` par défaut), à l&#39;exception du renewal de son propre token, d&#39;un ConfigMap de registry et, sur Kubernetes, du certificat client renouvelé dans son namespace d&#39;origine.

La création d&#39;un service tient en deux commandes. `clicklink clctl executor prepare` crée les buckets et l&#39;IAM role du service avec vos propres AWS credentials et affiche une seule fois le password du default user. `clicklink clctl instances create` soumet la définition du service à ClickHouse Cloud via votre connector endpoint. Ensuite, ClickHouse Cloud pilote le service par le canal de commandes sortant de l&#39;executor : scaling, arrêt et démarrage, redémarrages, sauvegardes, mises à niveau de version et changements de configuration. Consultez [services managés](/fr/products/bring-your-own-cloud/connector/managed-services) pour la procédure complète.
...
* **Enregistrement par ClickHouse.** ClickHouse enregistre votre environnement pour le mode managé et vous fournit votre connector endpoint ainsi qu&#39;un enrollment token à usage unique pendant l&#39;onboarding.
* **Un accès réseau sortant sur le port 443** vers votre connector endpoint et son enrollment endpoint, ainsi que vers `releases.clicklink.clickhouse.com` et ECR Public au moment de l&#39;installation. Des alternatives air-gapped et en miroir existent pour chaque étape ; consultez [onboarding](/fr/products/bring-your-own-cloud/connector/onboarding). En mode managé, les nœuds de votre cluster récupèrent également les images ClickHouse depuis le registry auprès duquel votre environnement est enregistré, et l&#39;executor contacte Amazon ECR et STS lorsqu&#39;il récupère un chart de plateforme depuis ECR.
...
* **Des credentials AWS pour `prepare`, `teardown` et `platform approve`.** La création des buckets et de l&#39;IAM role d&#39;un service, ainsi que leur suppression une fois le service supprimé, s&#39;exécutent avec vos credentials depuis votre poste de travail ou l&#39;hôte du connector. `platform approve` nécessite également des credentials capables de lire Amazon ECR lorsque les charts préparés y résident. L&#39;executor ne détient jamais de credentials sur vos buckets ni sur votre IAM. Ses seuls appels cloud vont vers Amazon ECR et STS. Il se connecte au registry lorsqu&#39;une synchronisation ou une création de plateforme récupère un chart. Pendant une mise à jour de plateforme, il vérifie les images avec le rôle de pull en lecture seule. Il n&#39;effectue aucun appel S3 ni IAM.
* **Un listener natif ClickHouse joignable** pour les clusters que vous exploitez vous-même, observés par un déploiement de connector distinct installé avec `--no-managed`. Le scraper et le troubleshooter communiquent avec chaque cluster via le native protocol ; sur Kubernetes, ils détectent automatiquement le mode sécurisé (9440) ou plaintext (9000).
