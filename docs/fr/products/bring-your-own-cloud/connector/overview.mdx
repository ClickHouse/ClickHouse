---
title: 'Présentation du connector'
...
<Note>
  Les flux de travail en mode managé décrits ici ne sont pas encore disponibles dans la release publique du connector. Confirmez le build et la prise en charge du déploiement requis auprès de l’équipe en charge de votre compte avant l’installation ; voir [disponibilité du mode managé](/fr/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability).
</Note>

## Ce que fait le connector {#what-the-connector-does}

Le ClickHouse Connector est un composant que vous déployez dans votre propre environnement. Il permet à ClickHouse Cloud de gérer, superviser et prendre en charge des services ClickHouse dans un cluster Kubernetes que vous exploitez. ClickHouse Cloud ne se connecte jamais à votre environnement : le connector n&#39;établit que des connexions sortantes et agit sous des identités dont la portée est limitée aux services ClickHouse. Le connector est livré sous la forme d&#39;un binaire unique, `clicklink`, qui contient les démons ainsi que l&#39;outillage `clicklink clctl` permettant de les installer et de les exploiter.
...
Avec un [build prenant en charge le mode managé](/fr/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability), `init` demande dans un terminal s&#39;il faut l&#39;activer et présélectionne oui ; un `init` scripté ne l&#39;active qu&#39;avec `--managed`. Vous pouvez aussi vous en passer et conserver l&#39;observability et le support pour les clusters que vous exploitez vous-même ; voir [exécuter sans le mode managé](#run-without-managed-mode).

Toutes les connexions établies par le connector sont sortantes. ClickHouse Cloud n&#39;initie jamais de connexion vers votre environnement. La page [architecture](/fr/products/bring-your-own-cloud/connector/architecture) répertorie chaque connexion, son protocole et son mode d&#39;authentification.

## Quand l&#39;utiliser {#when-to-use-it}

Déployez le connector lorsque vous souhaitez que ClickHouse Cloud exploite des services ClickHouse au sein de votre propre cloud account, dans le respect des contraintes suivantes :

* Tous les chemins d&#39;accès partent de l&#39;intérieur de votre boundary ; aucune connectivité entrante n&#39;est requise.
* Les writes vers votre cluster se limitent aux namespaces des services gérés par ClickHouse, à l&#39;exception du token propre à l&#39;executor, du registry et du certificat renouvelé dans son namespace d&#39;origine. Les composants de plateforme dont dépendent ces services n&#39;évoluent qu&#39;après que vous ayez approuvé chaque update.
* Vos données restent dans des buckets et derrière des IAM roles que vous créez et supprimez vous-même. L&#39;executor ne détient aucun credential pour vos buckets ni pour vos IAM roles.
* L&#39;accès du Support est limité dans le temps, activé par vous et révocable par vous à tout moment. Chaque commande passée lors d&#39;une session de support est consignée dans un audit log qui vous appartient.

## Fonctionnement {#how-it-works}

Le flux d&#39;inscription et de création de service décrit ci-dessous nécessite la [prise en charge du mode managé](/fr/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) confirmée par votre account team ; elle est indisponible dans la version publique `v0.16.0`.

ClickHouse enregistre votre environnement lors de l&#39;onboarding et fournit votre connector endpoint ainsi qu&#39;un jeton d&#39;inscription à usage unique. Vous exécutez `clicklink clctl init --enroll` une seule fois : depuis un poste de travail disposant d&#39;un accès kubeconfig pour une installation Kubernetes, ou sur l&#39;hôte pour une installation sur VM Linux. Cette unique commande consomme le jeton, obtient un certificat client mTLS, déploie et démarre les démons, puis vérifie leur health.

`init` active le mode managé lorsque vous passez `--managed` ; dans un terminal, la commande pose la question et la réponse par défaut est oui. Cette activation déploie l&#39;executor et applique son octroi d’accès Kubernetes. Ce grant est un ServiceAccount `pcm-executor`. Une politique d&#39;admission limite ses écritures aux namespaces de service (préfixe `ns-` par défaut), à l&#39;exception du renewal de son propre jeton, d&#39;un ConfigMap de registry et, sur Kubernetes, du certificat client renouvelé dans son namespace d&#39;origine.

La création d&#39;un service tient en deux commandes. `clicklink clctl executor prepare` crée les buckets et l&#39;IAM role du service avec vos propres identifiants AWS et affiche une seule fois le password du default user. `clicklink clctl instances create` soumet la définition du service à ClickHouse Cloud via votre connector endpoint. Ensuite, ClickHouse Cloud pilote le service par le canal de commandes sortant de l&#39;executor : scaling, arrêt et démarrage, redémarrages, sauvegardes, mises à niveau de version et changements de configuration. Consultez [services managés](/fr/products/bring-your-own-cloud/connector/managed-services) pour la procédure complète.
...
Le scraper lit les system tables autorisées par l&#39;allowlist à intervalle fixe et transmet les metrics, les métadonnées et l&#39;état de santé à votre connector endpoint via mTLS. Le troubleshooter maintient ouvert un canal de commandes sortant, mais ce canal ne transporte rien tant que vous n&#39;avez pas activé de session d&#39;assistance. Le certificat mTLS se renouvelle automatiquement.

## Exécution sans le mode managé {#run-without-managed-mode}

Le choix effectué à l&#39;installation décrit ci-dessous nécessite un [build prenant en charge le mode managé](/fr/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability). La version publique `v0.16.0` ne propose ni l&#39;indicateur `--no-managed` ni de prompt relatif au mode managé.
...
* **Des identifiants AWS pour `prepare` et `teardown`, et un accès au registry pour `platform approve`.** La création des buckets et de l&#39;IAM role d&#39;un service, ainsi que leur suppression une fois le service supprimé, s&#39;exécutent avec vos identifiants depuis votre poste de travail ou l&#39;hôte du connector. Pour un accès direct au registry de ClickHouse, l&#39;approbation et la synchronisation de plateforme utilisent le profil d&#39;instance de la VM EC2 du connector pour endosser le rôle de pull en lecture seule ; les profils AWS de votre poste de travail ou les identifiants SSO ne remplacent pas cette identité. Les autres registries de charts ECR utilisent les identifiants AWS ambiants. Consultez [identifiants du registry](/fr/products/bring-your-own-cloud/connector/platform-updates#registry-credentials). L&#39;executor ne détient jamais d&#39;identifiants sur vos buckets ni sur votre IAM. Ses seuls appels cloud vont vers Amazon ECR et STS pour la récupération des charts et la vérification des images de plateforme ; il n&#39;effectue aucun appel S3 ni IAM.
