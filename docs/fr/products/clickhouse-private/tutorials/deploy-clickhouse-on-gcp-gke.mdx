---
noindex: true
title: "Déployer ClickHouse sur GCP GKE"
sidebarTitle: "Déployer sur GCP"
description: "Ce tutoriel vous guide pour déployer ClickHouse Private sur Google Cloud Platform à l’aide de Google Kubernetes Engine (GKE)."
---

Ce tutoriel vous guide pour déployer ClickHouse Private sur Google Cloud Platform à l’aide de Google Kubernetes Engine (GKE). Il couvre à la fois les déploiements standard et les déploiements conformes à la norme FIPS 140-2. Les étapes propres à FIPS sont indiquées par **(FIPS)** tout au long du document -- définissez `ENABLE_FIPS=true` dans `00-env.sh` pour activer le mode FIPS.

Le tutoriel est organisé en scripts numérotés (`00-env.sh` à `19-verify`) à exécuter dans l’ordre. Chaque script est autonome et charge le fichier d’environnement.

Pour obtenir des spécifications détaillées de l’infrastructure, consultez [reference/infrastructure-requirements.md](/fr/products/clickhouse-private/reference/infrastructure-requirements-reference).
...
* `kubectl` installé
* `helm` v3.x installé
* `skopeo` installé (pour copier des images de conteneur)
* Identifiants AWS avec accès en lecture à ClickHouse ECR (pour la copie d’images)
* **Hôte bastion** dans le VPC pour accéder au cluster privé
* **Compte de service de déploiement** avec les autorisations appropriées (voir ci-dessous)
* **(FIPS)** Accès à l’image de conteneur Red Hat UBI8

***

<Steps>
  <Step stepNumber={0} title="Étape 0 : Configuration du compte de service" id="step-0-service-account-setup">
    Dans les environnements FedRAMP où l&#39;authentification directe des utilisateurs est restreinte, créez un compte de service pour l&#39;impersonation.

    ### Option 1 : Dans la console GCP

    1. Accédez à **IAM &amp; Admin** &gt; **Service Accounts**
    2. Cliquez sur **+ CREATE SERVICE ACCOUNT**
    3. Saisissez le nom `clickhouse-deployer` et l&#39;ID `clickhouse-deployer`
    ...
    1. Accordez l&#39;autorisation Token Creator au compte de service Compute du bastion.
    ...
    ### Configurer l’impersonation sur le bastion
    ...
    Pour désactiver l’emprunt d’identité plus tard :
  ...
  <Step stepNumber={1} title={<>Étape 1 : Définir les variables d'environnement (<code>00-env.sh</code>)</>} id="step-1-set-environment-variables-00-envsh">
    Créez ce fichier et sourcez-le avant d&#39;exécuter tout autre script. Adaptez toutes les valeurs à votre environnement.
    ...
    **Important :** Chargez ce fichier avec `source` avant d&#39;exécuter d&#39;autres scripts :
  ...
  <Step stepNumber={2} title={<>Étape 2 : Créer l’Artifact Registry (<code>01-artifact-registry.sh</code>)</>} id="step-2-create-artifact-registry-01-artifact-registrysh">
    ```bash expandable
    #!/bin/bash
    set -e
    source ./00-env.sh
  ...
  <Step stepNumber={3} title={<>Étape 3 : Copier les images de conteneur (<code>02-copy-images.sh</code>)</>} id="step-3-copy-container-images-02-copy-imagessh">
    Nécessite [skopeo](https://github.com/containers/skopeo) pour préserver la prise en charge de toutes les architectures.
  ...
  <Step stepNumber={4} title={<>Étape 4 : Configurer le VPC et le sous-réseau (<code>03-vpc-subnet.sh</code>)</>} id="step-4-configure-vpc-and-subnet-03-vpc-subnetsh">
    Ce script suppose que vous disposez d’un VPC existant avec un bastion. Il crée ou met à jour le sous-réseau GKE avec les plages secondaires requises.

    **Important : exigences relatives aux limites CIDR** - blocs `/14` : le troisième octet doit être divisible par 4 (par exemple, `10.244.0.0`, PAS `10.245.0.0`) - blocs `/20` : le quatrième octet doit être égal à 0, et le troisième octet doit être un multiple de 16 (par exemple, `10.252.0.0`)
  ...
  <Step stepNumber={5} title={<>Étape 5 : Créer Cloud NAT (<code>04-cloud-nat.sh</code>)</>} id="step-5-create-cloud-nat-04-cloud-natsh">
    Les nœuds GKE privés nécessitent Cloud NAT pour bénéficier d’un accès sortant à Internet.
  ...
  <Step stepNumber={6} title={<>Étape 6 : Créer un compte de service GCP (`05-service-account.sh`)</>} id="step-6-create-gcp-service-account-05-service-accountsh">
    ```bash
    #!/bin/bash
    set -e
    source ./00-env.sh
  ...
  <Step stepNumber={7} title={<>Étape 7 : Créer un bucket GCS (<code>06-gcs-bucket.sh</code>)</>} id="step-7-create-gcs-bucket-06-gcs-bucketsh">
    ```bash expandable
    #!/bin/bash
    set -e
    source ./00-env.sh
  ...
  <Step stepNumber={8} title={<>Étape 8 : Créer le cluster GKE (<code>07-gke-cluster.sh</code>)</>} id="step-8-create-gke-cluster-07-gke-clustersh">
    Crée un **cluster GKE entièrement privé** -- les nœuds de calcul n&#39;ont que des adresses IP internes, le control plane n&#39;a pas de point de terminaison public, et tout accès doit se faire depuis l&#39;intérieur du VPC via le bastion.
    ...
    **Paramètres du cluster privé :**

    | Paramètre                             | Valeur                     | Objectif                                            |
    | ------------------------------------- | -------------------------- | --------------------------------------------------- |
    | `--enable-private-nodes`              | Requis                     | Les nœuds obtiennent uniquement des IP internes     |
    | `--enable-private-endpoint`           | Requis                     | Le plan de contrôle n’a pas d’IP publique (FedRAMP) |
    | `--master-ipv4-cidr`                  | `172.16.0.0/28`            | Plage d’IP du plan de contrôle                      |
    | `--enable-master-authorized-networks` | Requis                     | Active les restrictions réseau                      |
    | `--master-authorized-networks`        | Sous-réseaux GKE + Bastion | CIDR autorisés à accéder au plan de contrôle        |

    ***
  </Step>

  <Step stepNumber={9} title={<>Étape 9 : Créer des pools de nœuds (<code>08-node-pools.sh</code>)</>} id="step-9-create-node-pools-08-node-poolssh">
    | Pool de nœuds | Type de machine                             | Disque                       | Labels                   | Taints                                      |
    | ------------- | ------------------------------------------- | ---------------------------- | ------------------------ | ------------------------------------------- |
  ...
  <Step stepNumber={10} title={<>Étape 10 : Déployer le DaemonSet Local SSD (<code>09-local-ssd-daemonset.sh</code>)</>} id="step-10-deploy-local-ssd-daemonset-09-local-ssd-daemonsetsh">
    Sur GKE Container-Optimized OS, le système de fichiers racine est en lecture seule ; il est donc impossible de créer `/nvme`. Le DaemonSet formate et monte le Local SSD sur `/mnt/disks/ssd0`.

    **Important :** Déployez-le **avant** le cluster ClickHouse afin d’éviter toute condition de concurrence.
  ...
  <Step stepNumber={11} title={<>Étape 11 : Ajouter des labels de topologie (<code>10-topology-labels.sh</code>)</>} id="step-11-add-topology-labels-10-topology-labelssh">
    ```bash
    #!/bin/bash
    set -e
    source ./00-env.sh
  ...
  <Step stepNumber={12} title="Étape 12 : Installer les prérequis Kubernetes" id="step-12-install-kubernetes-prerequisites">
    ### Installer les VolumeSnapshot CRDs (`11-volumesnapshot-crds.sh`)
    ...
    ### Configurer Workload Identity (`12-workload-identity.sh`)
    ...
    **Alignement de Workload Identity :**

    | Composant                             | Valeur                                                         |
    | ------------------------------------- | -------------------------------------------------------------- |
    | Compte de service GCP                 | `clickhouse@$PROJECT.iam.gserviceaccount.com`                  |
    | Compte de service K8s (manuel)        | `clickhouse` dans l’espace de noms `ns-$CLUSTER_NAME`          |
    | Compte de service K8s (créé par Helm) | `ch-$CLUSTER_NAME-sa` dans l’espace de noms `ns-$CLUSTER_NAME` |
    | Annotation du compte de service K8s   | `iam.gke.io/gcp-service-account=<GSA_EMAIL>`                   |
    | Membre de la liaison IAM              | `serviceAccount:$PROJECT.svc.id.goog[<namespace>/<ksa>]`       |

    ### Installer StorageClass (`13-storageclass.sh`) -- Facultatif

    **Remarque :** GKE utilise `topology.gke.io/zone` au lieu de `topology.kubernetes.io/zone`.
  ...
  <Step stepNumber={13} title="Étape 13 : Procédure FIPS (conditionnel)" id="step-13-fips-steps-conditional">
    Ignorez cette section si `ENABLE_FIPS=false`.

    ### Créer des certificats FIPS (`14-create-fips-certs.sh`)

    Les certificats conformes à la norme FIPS doivent être générés à l’aide d’algorithmes approuvés par la norme FIPS (RSA 3072 bits minimum, SHA-256 ou version ultérieure) dans un environnement où FIPS est activé. Le script exécute un pod Red Hat UBI8 sur le cluster GKE avec la politique cryptographique FIPS activée.

    **Types de certificats requis :**

    | Certificat | Fichiers                                 | SAN requis                                                                        |
    | ---------- | ---------------------------------------- | --------------------------------------------------------------------------------- |
    | CA         | `ca/ca.crt`, `ca/ca.key`                 | N/A                                                                               |
    | Serveur    | `server/server.crt`, `server/server.key` | `*.c-${CLUSTER_NAME}-server-headless.ns-${CLUSTER_NAME}.svc.${KUBERNETES_DOMAIN}` |
    | Client     | `client/client.crt`, `client/client.key` | Aucun                                                                             |
    | Keeper     | `keeper/keeper.crt`, `keeper/keeper.key` | `*.c-${CLUSTER_NAME}-keeper-headless.ns-${CLUSTER_NAME}.svc.${KUBERNETES_DOMAIN}` |

    Pour la procédure complète de génération des certificats, consultez [how-to/configure-fips-certificates.md](/fr/products/clickhouse-private/guides/generate-fips-compliant-certificates-for-clickhouse).

    ### Créer les secrets de certificats (`15-create-cert-secrets.sh`)
    ...
    **Structure du secret :**
    ...
    | Nom du secret                        | Clés                                                             | Objectif                                        |
    | ------------------------------------ | ---------------------------------------------------------------- | ----------------------------------------------- |
    | `${CLUSTER_NAME}-server-cert-secret` | `ca.crt`, `server.crt`, `server.key`, `client.crt`, `client.key` | TLS du serveur et connexions clientes sortantes |
    | `${CLUSTER_NAME}-keeper-cert-secret` | `ca.crt`, `keeper.crt`, `keeper.key`                             | TLS de Keeper                                   |

    ***
  </Step>

  <Step stepNumber={14} title={<>Étape 14 : Installer l’Operator (<code>16-install-operator.sh</code>)</>} id="step-14-install-operator-16-install-operatorsh">
    ```bash expandable
    #!/bin/bash
    set -e
    source ./00-env.sh
  ...
  <Step stepNumber={15} title={<>Étape 15 : Déployer le cluster ClickHouse (<code>17-install-cluster.sh</code>)</>} id="step-15-deploy-clickhouse-cluster-17-install-clustersh">
    ```bash expandable
    #!/bin/bash
    set -e
    source ./00-env.sh
    ...
    **Paramètres importants spécifiques à GCP :**

    * `storageClass.create=false` -- Ne pas créer de StorageClass AWS
    * `storageClass.name="premium-rwo"` -- Utiliser la StorageClass SSD intégrée de GCP
    * `server.storage.s3.endpoint` -- `https://storage.googleapis.com` pour l’API S3-compatible de GCS
    * `server.storage.s3.useEnvironmentCredentials=true` -- Obligatoire pour Workload Identity
    * `keeper.storage.s3.useEnvironmentCredentials=true` -- **Doit également être défini pour Keeper**

    ***
  </Step>

  <Step stepNumber={16} title={<>Étape 16 : Associer Workload Identity (<code>18-bind-helm-workload-identity.sh</code>)</>} id="step-16-bind-workload-identity-18-bind-helm-workload-identitysh">
    Le chart Helm crée son propre compte de service (`ch-${CLUSTER_NAME}-sa`). Vous devez associer Workload Identity à ce compte de service pour accéder à GCS.

    **Important :** Les pods Keeper et serveur créés à l’étape 15 démarrent avant que cette association ne soit en place ; ils échoueront donc avec des erreurs GCS 403. Ce script redémarre les pods après l’association.
  ...
  <Step stepNumber={17} title="Étape 17 : Vérifier l’installation" id="step-17-verify-installation">
    ### Mode standard (sans TLS)
    ...
    Avec `openSSL.required=true`, ClickHouse n’accepte que les connexions sur des ports sécurisés. Utilisez le port **9440** (TLS natif) :
    ...
    Pour un environnement de production avec des certificats signés par une CA :
    ...
    **Important (FIPS) :** les ports 9000 (protocole natif en clair) et 8123 (HTTP en clair) refusent les connexions lorsque `openSSL.required=true`. Utilisez plutôt les ports 9440 (TLS natif) ou 8443 (HTTPS).
...
<div id="appendix-aws-to-gcp-component-mapping">
  ## Annexe : correspondance des composants AWS et GCP
</div>

| Composant AWS      | Équivalent GCP    | Principales différences                                 |
| ------------------ | ----------------- | ------------------------------------------------------- |
| ECR                | Artifact Registry | Utilisez `gcloud auth configure-docker`                 |
| S3                 | GCS avec l’API S3 | Point de terminaison : `https://storage.googleapis.com` |
| IAM Role (IRSA)    | Workload Identity | Annotez le KSA avec `iam.gke.io/gcp-service-account`    |
| HMAC Keys          | Workload Identity | Utilisez `useEnvironmentCredentials=true`               |
| EBS CSI Driver     | GCE PD CSI Driver | Préinstallé, provisionneur : `pd.csi.storage.gke.io`    |
| Availability Zones | Zones GKE         | Clé de topologie : `topology.gke.io/zone`               |
| NLB                | Load Balancer GCP | Utilisez l’annotation `type: LoadBalancer`              |