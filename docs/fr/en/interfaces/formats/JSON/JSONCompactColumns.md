...
Dans ce format, toutes les données sont représentées sous la forme d’un unique tableau JSON.

:::note
Le format de sortie `JSONCompactColumns` conserve toutes les données en mémoire afin de les restituer sous la forme d’un seul bloc, ce qui peut entraîner une consommation mémoire élevée.
:::
...
Les colonnes absentes du bloc seront remplies avec des valeurs par défaut (vous pouvez utiliser dans ce cas le paramètre [`input_format_defaults_for_omitted_fields`](/fr/operations/settings/settings-formats.md/#input_format_defaults_for_omitted_fields))
