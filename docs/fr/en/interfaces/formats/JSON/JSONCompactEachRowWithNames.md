...
Ce format se distingue du format [`JSONCompactEachRow`](./JSONCompactEachRow.md) en ce qu’il affiche également la ligne d’en-tête avec les noms de colonnes, à l’instar du format [`TabSeparatedWithNames`](../TabSeparated/TabSeparatedWithNames.md).
...
À partir d’un fichier JSON nommé `football.json` et contenant les données suivantes :
