...
Ne diffère de [`JSONCompactEachRow`](./JSONCompactEachRow.md) que par le fait que les champs de données sont affichés sous forme de chaînes de caractères, et non comme des valeurs JSON typées.
...
Lisez les données à l’aide du format `JSONCompactStringsEachRow` :
