...
Dans ce format, ClickHouse génère chaque ligne sous la forme d’un objet JSON distinct, délimité par un saut de ligne.

Ce format est également connu sous les noms `JSONEachRow`, `JSONLines`, `NDJSON` (Newline Delimited JSON) ou `JSONL`. Ces noms sont des alias du même format et peuvent être utilisés indifféremment en entrée comme en sortie.
...
À l’aide d’un fichier JSON contenant les données suivantes et nommé `football.json` :
...
Lors de l’importation, les colonnes dont le nom est inconnu seront ignorées si le paramètre [input&#95;format&#95;skip&#95;unknown&#95;fields](/fr/operations/settings/settings-formats.md/#input_format_skip_unknown_fields) est défini sur 1.
