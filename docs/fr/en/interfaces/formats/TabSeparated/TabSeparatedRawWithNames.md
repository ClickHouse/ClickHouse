...
Se distingue du format [`TabSeparatedWithNames`](./TabSeparatedWithNames.md) par le fait que les lignes sont écrites sans échappement.
...
Lisez les données au format `TabSeparatedRawWithNames` :
...
La sortie sera au format séparé par des tabulations, avec un en-tête sur une seule ligne :
