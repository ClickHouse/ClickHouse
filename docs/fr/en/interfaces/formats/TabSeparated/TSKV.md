...
Semblable au format [`TabSeparated`](./TabSeparated.md), mais produit une valeur au format `name=value`.
Les noms sont échappés de la même manière que dans le format [`TabSeparated`](./TabSeparated.md), et le symbole `=` l’est également.
...
Néanmoins, en termes d&#39;efficacité, il n&#39;est pas moins performant que le format [`JSONEachRow`](../JSON/JSONEachRow.md).
:::

Lors de l&#39;analyse, les valeurs des différentes colonnes peuvent être fournies dans n&#39;importe quel ordre.
Certaines valeurs peuvent être omises, car elles sont alors considérées comme égales à leurs valeurs par défaut.
Dans ce cas, les zéros et les lignes vides sont utilisés comme valeurs par défaut.
Les valeurs complexes pouvant être spécifiées dans la table ne sont pas prises en charge comme valeurs par défaut.

L&#39;analyse permet d&#39;ajouter un champ supplémentaire `tskv` sans signe égal ni valeur. Ce champ est ignoré.

Lors de l&#39;importation, les colonnes dont les noms sont inconnus sont ignorées
...
[NULL](/fr/sql-reference/syntax.md) est représenté par `\N`.
...
La sortie sera au format tabulé, avec deux lignes d’en-tête pour les noms de colonnes et les types :
