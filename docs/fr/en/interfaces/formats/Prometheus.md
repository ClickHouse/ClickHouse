...
Expose des métriques au [format d&#39;exposition textuel de Prometheus](https://prometheus.io/docs/instrumenting/exposition_formats/#text-based-format).

Pour ce format, la table de sortie doit respecter une structure précise, selon les règles suivantes :

* Les colonnes `name` ([String](/fr/sql-reference/data-types/string.md)) et `value` (nombre) sont obligatoires.
* Les lignes peuvent contenir facultativement `help` ([String](/fr/sql-reference/data-types/string.md)) et `timestamp` (nombre).
* La colonne `type` ([String](/fr/sql-reference/data-types/string.md)) doit contenir l&#39;une des valeurs suivantes : `counter`, `gauge`, `histogram`, `summary`, `untyped`, ou être vide.
* Chaque valeur de métrique peut également avoir des `labels` ([Map(String, String)](/fr/sql-reference/data-types/map.md)).
...
Il existe des exigences particulières concernant `histogram` et `summary` ; voir la [documentation Prometheus](https://prometheus.io/docs/instrumenting/exposition_formats/#histograms-and-summaries) pour plus de détails.
