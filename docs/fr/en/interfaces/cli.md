---
description: 'Documentation pour clickhousectl, la CLI de ClickHouse : local et cloud'
...
`clickhousectl` est la CLI de ClickHouse : local et cloud.
...
* Démarrer et gérer des instances Postgres locales
...
* Créer et gérer des services Postgres ClickHouse Cloud
* Gérer les ressources ClickHouse Cloud
* Créer et gérer ClickPipes pour l’ingestion de données (S3, Kafka, Kinesis, Postgres, MySQL, MongoDB, BigQuery)
* Installer les ClickHouse agent skills officiels dans les agents de code pris en charge
* Transférer votre environnement de développement ClickHouse local vers le cloud

`clickhousectl` aide aussi bien les humains que les agents IA à développer avec ClickHouse.
...
* Les commandes Cloud nécessitent une [clé API ClickHouse Cloud](/fr/cloud/manage/api/api-overview)
...
`clickhousectl` télécharge les binaires de ClickHouse depuis `builds.clickhouse.com`, et se rabat sur `packages.clickhouse.com` (Linux) ou les [releases GitHub](https://github.com/ClickHouse/ClickHouse/releases) (macOS) lorsqu’aucune build n’y est disponible.
...
`local use` crée également un lien symbolique dans `~/.local/bin/clickhouse` pointant vers le binaire de la version sélectionnée, afin que la commande `clickhouse` seule (par ex. `clickhouse local`, `clickhouse client`) soit dans le `PATH`. Passez `--no-global` pour ne pas le créer. Si un fichier ordinaire existe déjà à cet emplacement, il est conservé et un avertissement est affiché. L’exécution de `local remove` sur la version par défaut active supprime également le lien symbolique.

<div id="binary-storage">
  #### Stockage des binaires ClickHouse
</div>

Les binaires ClickHouse sont stockés dans un emplacement global, afin qu’ils puissent être utilisés par plusieurs projets sans dupliquer le stockage. Les binaires sont stockés dans `~/.clickhouse/` :
...
`init` prépare votre répertoire de travail courant avec une arborescence standard pour vos fichiers de projet ClickHouse et Postgres. Cette étape est facultative ; vous pouvez tout à fait utiliser votre propre structure de dossiers si vous le préférez.
...
**Nommage des serveurs :** Sans `--name`, le premier serveur s&#39;appelle &quot;default&quot;. Si &quot;default&quot; est déjà lancé, un nom aléatoire est généré (par ex. &quot;bold-crane&quot;). Utilisez `--name` pour définir des identités stables que vous pourrez démarrer et arrêter à plusieurs reprises.
...
<div id="custom-config-files">
  #### Fichiers de configuration personnalisés pour les serveurs locaux
</div>

Les serveurs locaux démarrent avec des paramètres par défaut adaptés, mais il peut parfois être nécessaire de modifier un réglage. Déposez un fichier de configuration dans `~/.clickhouse/configs/` et appliquez-le par son nom au démarrage d’un serveur :
...
Le fichier nommé est **appliqué par-dessus les valeurs par défaut intégrées de ClickHouse** (via `config.d`) ; il n’a donc besoin de contenir que les paramètres que vous souhaitez modifier, sans qu’il soit nécessaire de reproduire une config complète. Les fichiers peuvent être au format `.xml`, `.yaml` ou `.yml`, et vous pouvez les référencer par leur nom, avec ou sans l’extension.

<div id="project-local-data">
  #### Répertoire local des données du projet
</div>

Toutes les données du serveur se trouvent dans le répertoire `.clickhouse/` de votre projet :
...
Chaque serveur nommé possède son propre répertoire de données, de sorte que les serveurs sont complètement isolés les uns des autres. Les données sont conservées entre les redémarrages. Arrêtez et redémarrez un serveur par son nom pour reprendre là où vous vous étiez arrêté. Utilisez `clickhousectl local server remove <name>` pour supprimer définitivement les données d&#39;un serveur.

<div id="local-postgres">
  ### Exécuter Postgres en local
</div>

En plus de ClickHouse, `clickhousectl` peut exécuter et gérer des instances Postgres locales. Postgres en local s’appuie sur Docker ; Docker doit donc être installé et en cours d’exécution. Chaque instance est identifiée par son nom et sa version majeure, ce qui permet d’exécuter plusieurs versions de Postgres côte à côte avec des répertoires de données distincts.
...
Authentifiez-vous sur ClickHouse Cloud à l’aide de clés API (recommandé) ou d’OAuth (via le navigateur).
...
Les clés API constituent la méthode d’authentification recommandée, en particulier lorsque la CLI est pilotée par un agent IA. Vous pouvez [créer des clés API avec une portée limitée](/fr/cloud/manage/openapi) qui n’accordent que les autorisations de votre choix (lecture seule ou lecture/écriture), et chaque clé est liée à une seule organisation. C’est donc un moyen sûr d’accorder à la CLI un accès avec le minimum de privilèges.
...
Les identifiants sont enregistrés dans `.clickhouse/credentials.json` (propres au projet).

Vous pouvez également utiliser des variables d’environnement, par exemple en les exportant dans votre session :
...
Ou placé dans un fichier `.env` situé dans votre répertoire de travail actuel :
...
Ou transmettez directement les identifiants via des options, quelle que soit la commande :
...
Cela ouvre votre navigateur pour vous authentifier via le flux OAuth pour les appareils. Les jetons sont enregistrés dans `.clickhouse/tokens.json` (spécifique au projet).

:::note
L&#39;accès OAuth est actuellement **en lecture seule** et donne accès à **toutes les organisations auxquelles vous appartenez**. Pour obtenir un accès en écriture, ou limiter la portée de la CLI à une seule organisation, [créez plutôt une clé API à portée limitée](#api-key).
:::

<div id="auth-status">
  ### Statut de l’authentification et déconnexion
</div>
...
Ordre de résolution des identifiants : options CLI &gt; `.clickhouse/credentials.json` &gt; variables d’environnement exportées &gt; fichier `.env` &gt; jetons OAuth.
...
| `--provider`                               | Fournisseur de Cloud : `aws`, `gcp`, `azure` (par défaut : `aws`)                    |
...
| `--backup-id`                              | ID de la sauvegarde à restaurer                                                      |
| `--release-channel`                        | Canal de publication : `slow`, `default`, `fast`                                     |
| `--data-warehouse-id`                      | ID de l’entrepôt de données (pour les répliques de lecture)                          |
| `--readonly`                               | Rendre le service en lecture seule                                                   |
...
| `--private-preview-terms-checked`          | Accepter les conditions de private preview lorsque nécessaire                        |
| `--enable-core-dumps`                      | Activer ou désactiver la collecte des core dumps du service                          |

<div id="query-api-auth-modes">
  #### Modes d&#39;authentification de Query API
</div>

`cloud service query` est la méthode standard pour exécuter du SQL sur un service Cloud via HTTP, sans nécessiter le binaire `clickhouse` ni le mot de passe du service. Il fonctionne avec les deux modes d&#39;authentification :

* **Authentification par clé API** (SQL en lecture + écriture) : lors de la première exécution de `cloud service query` sur un service sans clé enregistrée, la commande provisionne un point de terminaison Query API pour ce service et crée une clé API dédiée qui y est associée. La clé (`keyId`, `keySecret` et `endpointId`) est stockée dans `.clickhouse/credentials.json`, sous `service_query_keys.<service-id>`. La clé est limitée à un seul service : elle peut donc lire et écrire (SELECT, INSERT, DDL) sur ce service, mais ne peut accéder à aucun autre service de l&#39;org. Passez `--no-auto-enable` pour faire échouer la commande au lieu de provisionner.
* **OAuth** (`cloud auth login`) : la requête s&#39;exécute avec votre propre identité, comme dans la console SQL web. Vos permissions SQL sur le service sont en **lecture seule** lorsque vous utilisez OAuth. Aucune clé Query API n&#39;est provisionnée ni stockée. `--no-auto-enable` n&#39;a aucun effet dans ce mode.

Interroger un service **mis en veille** le réactive automatiquement dans les deux modes d&#39;authentification (la première requête peut prendre une minute). Un service **arrêté** n&#39;est jamais réactivé : la requête échoue avec une indication invitant à exécuter `cloud service start`. Définissez `CLICKHOUSE_CLOUD_QUERY_HOST` pour remplacer l&#39;hôte Query API dérivé.
...
`clickhousectl` peut également créer et gérer des services [ClickHouse Cloud Postgres](/fr/cloud/managed-postgres), en reprenant les commandes du service ClickHouse ci-dessus.
...
| -------------------------- | ---------------------------------------------------------------- |
| `--name`                   | Nom du service (obligatoire)                                     |
| `--region`                 | Région, par ex. `us-east-1` (obligatoire)                        |
| `--size`                   | Taille de l&#39;instance, par ex. `m7i.2xlarge` (obligatoire)    |
| `--provider`               | Fournisseur de cloud (par défaut : `aws`)                        |
| `--pg-version`             | Version majeure : `18`, `17`                                     |
| `--ha-type`                | Haute disponibilité : `none`, `async`, `sync`                    |
| `--tag`                    | Tag de ressource `key` ou `key=value` (réutilisable)             |
...
Gérez ClickPipes pour l’ingestion de données dans ClickHouse Cloud à partir de sources externes.
...
Utilisez `clickhousectl cloud clickpipe create <source> --help` pour afficher la liste complète des options pour chaque type de source.
...
`clickhousectl` détecte automatiquement les contextes d’agents de code (Claude Code, Cursor, Codex, Gemini CLI, Goose, Devin, ainsi que tout outil qui définit la variable d’environnement standard `AGENT`) et envoie automatiquement du JSON sur stdout, sans qu’il soit nécessaire de définir `--json`.
...
| `2`  | Annulé (interrompu par l’utilisateur)                                                 |
| `4`  | Authentification requise (aucun identifiant, 401/403, écritures via OAuth uniquement) |
...
| Option            | Description                                                                              |
| ----------------- | ---------------------------------------------------------------------------------------- |
| `--agent <name>`  | Installer Skills pour un agent donné (option répétable)                                  |
| `--global`        | Utiliser la portée globale ; si cette option est omise, la portée du projet est utilisée |
| `--all`           | Installer Skills pour tous les agents pris en charge                                     |
| `--detected-only` | Installer Skills pour les agents pris en charge détectés sur le système                  |
...
Le CLI vérifie également les mises à jour en arrière-plan (au maximum une fois toutes les 24 heures) et affiche une notification lorsqu&#39;une version plus récente est disponible.