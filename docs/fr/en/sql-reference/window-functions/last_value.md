...
Renvoie la dernière valeur évaluée dans son cadre ordonné. Par défaut, les arguments NULL sont ignorés, mais le modificateur `RESPECT NULLS` peut être utilisé pour modifier ce comportement.
...
Consultez [le traitement des valeurs NULL](../aggregate-functions/index.md/#null-processing) pour plus d’informations.
...
Dans cet exemple, la fonction `last_value` est utilisée pour trouver le footballeur le moins payé dans un jeu de données fictif sur les salaires des joueurs de Premier League.
