---
description: 'Page d’aperçu des fonctions de fenêtre'
sidebar_label: 'Fonctions de fenêtre'
sidebar_position: 1
...
Les fonctions de fenêtre permettent d’effectuer des calculs sur un ensemble de lignes liées à la ligne en cours.
Elles peuvent servir à effectuer des calculs similaires à ceux réalisés avec des fonctions d’agrégation, mais s’en distinguent par le fait qu’une fonction de fenêtre ne regroupe pas les lignes en une seule sortie ; au contraire, les lignes individuelles sont toujours renvoyées.
...
Le tableau ci-dessous indique quelles fonctionnalités sont actuellement prises en charge :

| Fonctionnalité                                                                         | Prise en charge ? | Commentaire                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| -------------------------------------------------------------------------------------- | ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Spécification de fenêtre ad hoc (`count(*) OVER (PARTITION BY id ORDER BY time DESC)`) | ✅                 |                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| Expressions impliquant des fonctions de fenêtre, par ex. `(count(*) OVER ()) / 2`      | ✅                 |                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| Syntaxe `INTERVAL` pour le cadre `DateTime` `RANGE OFFSET`                             | ❌                 | Indiquez plutôt le nombre de secondes (`RANGE` fonctionne avec n’importe quel type numérique).                                                                                                                                                                                                                                                                                                                                                         |
| Cadre `GROUPS`                                                                         | ❌                 |                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `percent_rank()`/`percentRank()`                                                       | ✅                 | Calcule efficacement la position relative d’une valeur au sein d’une partition. Cette fonction remplace le calcul SQL manuel, plus verbeux et plus coûteux en calcul, exprimé sous la forme `ifNull((rank() OVER (PARTITION BY x ORDER BY y) - 1) / nullif(count(1) OVER (PARTITION BY x) - 1, 0), 0)`.                                                                                                                                                |
| `cume_dist()`                                                                          | ✅                 | Calcule la distribution cumulée d’une valeur au sein d’un groupe de valeurs. Renvoyer le pourcentage de lignes dont les valeurs sont inférieures ou égales à la valeur de la ligne en cours.                                                                                                                                                                                                                                                           |
...
| [`ntile(buckets)`](./ntile.md)                                                         | ✅                 | Spécifiez la fenêtre, par exemple, comme suit : `(PARTITION BY x ORDER BY y ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING)`.                                                                                                                                                                                                                                                                                                                |
...
* `PARTITION BY` - définit comment répartir un jeu de résultats en groupes.
* `ORDER BY` - définit comment ordonner les lignes au sein du groupe lors du calcul de aggregate&#95;function.
* `ROWS or RANGE` - définit les bornes d’un cadre ; aggregate&#95;function est calculée à l’intérieur de ce cadre.
...
| ---------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| [`row_number()`](./row_number.md)                                                                          | Numérote la ligne en cours dans sa partition à partir de 1.                                                                              |
| [`first_value(x)`](./first_value.md)                                                                       | Renvoie la première valeur évaluée dans son cadre ordonné.                                                                               |
| [`last_value(x)`](./last_value.md)                                                                         | Renvoie la dernière valeur évaluée dans son cadre ordonné.                                                                               |
| [`nth_value(x, offset)`](./nth_value.md)                                                                   | Renvoie la première valeur non-NULL évaluée sur la n-ième ligne (`offset`) de son cadre ordonné.                                         |
| [`rank()`](./rank.md)                                                                                      | Attribue un rang à la ligne en cours dans sa partition, avec des écarts.                                                                 |
| [`dense_rank()`](./dense_rank.md)                                                                          | Attribue un rang à la ligne en cours dans sa partition, sans écarts.                                                                     |
| [`lagInFrame(x)`](./lagInFrame.md)                                                                         | Renvoie une valeur évaluée sur la ligne située, dans le cadre ordonné, au décalage physique spécifié avant la ligne en cours.            |
| [`leadInFrame(x)`](./leadInFrame.md)                                                                       | Renvoie une valeur évaluée sur la ligne située, dans le cadre ordonné, au décalage indiqué après la ligne en cours.                      |
| [`ntile(buckets)`](./ntile.md)                                                                             | Répartit les lignes ordonnées d&#39;une partition dans le nombre de buckets indiqué et renvoie le numéro du bucket de la ligne en cours. |
| [`nonNegativeDerivative(metric_column, timestamp_column[, INTERVAL X UNITS])`](./nonNegativeDerivative.md) | Calcule la dérivée non négative de `metric_column` par rapport à `timestamp_column`. Spécifique à ClickHouse.                            |
...
Les exemples suivants montrent comment résoudre des problèmes courants en pratique.

<div id="maximumtotal-salary-per-department">
  ### Salaire maximal et total par département
</div>
...
<div id="moving--sliding-average-per-10-seconds">
  ### Moyenne mobile / glissante (sur 10 secondes)
</div>
...
<div id="moving--sliding-average-per-10-days">
  ### Moyenne mobile / glissante (par 10 jours)
</div>

La température est stockée avec une précision à la seconde, mais en utilisant `Range` et `ORDER BY toDate(ts)`, nous définissons une fenêtre de 10 unités, et avec `toDate(ts)`, l’unité est le jour.
...
La feuille de route pour la prise en charge initiale des fonctions de fenêtre se trouve [dans cette issue](https://github.com/ClickHouse/ClickHouse/issues/18097).
...
Ces tests contiennent des exemples de la syntaxe actuellement prise en charge :
...
* Blog : [Fonctions de fenêtre et de tableau pour les séquences de commits Git](https://clickhouse.com/blog/clickhouse-window-array-functions-git-commits)
* Blog : [Intégrer des données dans ClickHouse - Partie 3 - Utiliser S3](https://clickhouse.com/blog/getting-data-into-clickhouse-part-3-s3)