---
description: 'Documentation de la fonction de fenêtre cume_dist'
...
Calcule la distribution cumulée d’une valeur au sein d’un groupe de valeurs, c’est-à-dire le pourcentage de lignes dont les valeurs sont inférieures ou égales à celle de la ligne en cours. Peut être utilisée pour déterminer la position relative d’une valeur dans une partition.
...
La définition du cadre de fenêtre par défaut, et requise, est `RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING`.

Pour plus de détails sur la syntaxe des fonctions de fenêtre, voir : [Fonctions de fenêtre - Syntaxe](./index.md/#syntax).

**Valeur renvoyée**

* Le rang relatif de la ligne en cours. Le type de retour est Float64 dans la plage [0, 1]. [Float64](../data-types/float.md).
...
La fonction `cume_dist()` calcule la position relative selon la formule suivante :

```text
cume_dist = (number of rows ≤ current row value) / (total number of rows in partition)
```

Les lignes ayant des valeurs égales (ex æquo) reçoivent la même valeur de distribution cumulée, qui correspond à la position la plus élevée du groupe ex æquo.