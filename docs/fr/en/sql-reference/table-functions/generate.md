---
description: "Génère des données aléatoires selon un schéma donné. Permet de remplir des tables de test avec ces données. Tous les types ne sont pas pris en charge."
...
Génère des données aléatoires selon un schéma donné.
Permet de remplir des tables de test avec ces données.
Tous les types ne sont pas pris en charge.
...
| ------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| `name`              | Nom de la colonne correspondante.                                                                                             |
| `TypeName`          | Type de la colonne correspondante.                                                                                            |
| `random_seed`       | Définit manuellement la graine aléatoire pour obtenir des résultats stables. Si `NULL` — la graine est générée aléatoirement. |
| `max_string_length` | Longueur maximale de toutes les chaînes générées. La valeur par défaut est `10`.                                              |
| `max_array_length`  | Nombre maximal d&#39;éléments dans tous les tableaux ou maps générés. La valeur par défaut est `10`.                          |

<div id="returned_value">
  ## Valeur renvoyée
</div>

Un objet de type table avec le schéma demandé.
...
En combinaison avec [generateRandomStructure](../../sql-reference/functions/other-functions.md#generateRandomStructure) :
...
En l’absence de l’argument `structure` (dans ce cas, la structure est aléatoire) :
...
Avec une graine aléatoire pour la structure aléatoire et les données aléatoires :
...
:::note
`generateRandom(generateRandomStructure(), [random seed], max_string_length, max_array_length)` avec une valeur de `max_array_length` suffisamment élevée peut générer une sortie extrêmement volumineuse en raison de la profondeur d’imbrication potentiellement importante (jusqu’à 16) des types complexes (`Array`, `Tuple`, `Map`, `Nested`).
:::
