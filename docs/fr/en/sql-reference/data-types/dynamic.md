...
Ce type permet de stocker des valeurs de n’importe quel type sans les connaître toutes à l’avance.
...
Où `N` est un paramètre facultatif compris entre `0` et `254` indiquant combien de types de données différents peuvent être stockés sous forme de subcolumns distinctes dans une colonne de type `Dynamic`, au sein d’un même bloc de données stocké séparément (par exemple, au sein d’une même partie de données pour une MergeTree table). Si cette limite est dépassée, toutes les values de nouveaux types seront stockées ensemble dans une structure spéciale de données partagée, sous forme binaire. La valeur par défaut de `max_types` est `32`.
...
<div id="reading-dynamic-nested-types-as-subcolumns">
  ## Lecture des types imbriqués de `Dynamic` en tant que sous-colonnes
</div>

Le type `Dynamic` permet de lire un type imbriqué unique depuis une colonne `Dynamic` en utilisant le nom du type comme sous-colonne.
Ainsi, si vous avez une colonne `d Dynamic`, vous pouvez lire une sous-colonne de n’importe quel type valide `T` avec la syntaxe `d.T` ;
cette sous-colonne aura le type `Nullable(T)` si `T` peut être contenu dans `Nullable`, et le type `T` sinon. Cette sous-colonne
aura la même taille que la colonne `Dynamic` d’origine et contiendra des valeurs `NULL` (ou des valeurs vides si `T` ne peut pas être contenu dans `Nullable`)
dans toutes les lignes où la colonne `Dynamic` d’origine n’a pas le type `T`.

Les sous-colonnes de `Dynamic` peuvent également être lues à l’aide de la fonction `dynamicElement(dynamic_column, type_name)`.
...
Pour savoir quelle variante est stockée dans chaque ligne, vous pouvez utiliser la fonction `dynamicType(dynamic_column)`. Elle renvoie une `String` contenant le nom du type de valeur pour chaque ligne (ou `'None'` si la ligne est `NULL`).
...
<div id="converting-an-ordinary-column-to-a-dynamic-column">
  ### Conversion d’une colonne ordinaire en colonne de type Dynamic
</div>
...
Il est possible de convertir une colonne `Dynamic` en colonne ordinaire. Dans ce cas, tous les types imbriqués seront convertis vers un type cible :
...
<div id="converting-a-variant-column-to-dynamic-column">
  ### Conversion d’une colonne Variant en colonne Dynamic
</div>
...
<div id="converting-a-dynamicmax_typesn-column-to-another-dynamicmax_typesk">
  ### Conversion d’une colonne Dynamic(max_types=N) vers une autre colonne Dynamic(max_types=K)
</div>

Si `K >= N`, les données ne changent pas lors de la conversion :
...
Si `K < N`, alors les valeurs correspondant aux types les plus rares seront insérées dans une seule sous-colonne spéciale, mais resteront accessibles :
...
La fonction `isDynamicElementInSharedData` renvoie `true` pour les lignes stockées dans une structure de données partagée spéciale au sein de `Dynamic` et, comme on peut le voir, la colonne résultante ne contient que 2 types qui ne sont pas stockés dans cette structure de données partagée.
...
<div id="reading-dynamic-type-from-the-data">
  ## Lecture du type Dynamic depuis les données
</div>

Tous les formats texte (TSV, CSV, CustomSeparated, Values, JSONEachRow, etc.) prennent en charge la lecture du type `Dynamic`. Lors de l’analyse des données, ClickHouse essaie d’inférer le type de chaque valeur et l’utilise lors de l’insertion dans une colonne `Dynamic`.
...
Si la fonction ne peut pas être exécutée pour un certain type dans la colonne `Dynamic`, une exception sera levée :
...
Nous pouvons filtrer les types inutiles :
...
Ou extraire le type requis sous forme de sous-colonne :
...
Le paramètre `dynamic_throw_on_type_mismatch` contrôle ce qui se passe lorsqu’une fonction est appliquée à une colonne `Dynamic` et que le type réellement stocké dans une ligne est incompatible avec cette fonction :
...
Dans `ORDER BY` et `GROUP BY`, les valeurs de type `Dynamic` sont comparées de manière similaire à celles du type `Variant` :
...
**Remarque :** les valeurs de type Dynamic ayant des types numériques différents sont considérées comme distinctes et ne sont pas comparées entre elles ; ce sont leurs noms de type qui sont comparés à la place.
...
**Remarque :** la règle de comparaison décrite n’est pas appliquée lors de l’exécution de fonctions de comparaison telles que `<`/`>`/`=` et autres, en raison du [fonctionnement particulier](#using-dynamic-type-in-functions) des fonctions utilisant le type `Dynamic`
...
Le type de données `Dynamic` ne peut stocker qu&#39;un nombre limité de types de données différents sous forme de sous-colonnes distinctes. Par défaut, cette limite est de 32, mais vous pouvez la modifier dans la déclaration du type à l&#39;aide de la syntaxe `Dynamic(max_types=N)`, où N est compris entre 0 et 254 (pour des raisons d&#39;implémentation, il est impossible d&#39;avoir plus de 254 types de données différents pouvant être stockés comme sous-colonnes distinctes dans Dynamic).
Lorsque la limite est atteinte, tous les nouveaux types de données insérés dans la colonne `Dynamic` le seront dans une structure de données partagée unique, qui stocke des valeurs de types de données différents sous forme binaire.
...
Comme on peut le voir, après l’insertion de 3 types de données différents `Int64`, `Array(Int64)` et `String`, tous les nouveaux types ont été insérés dans la structure spéciale de données partagée.
...
Comme on peut le voir, ClickHouse a conservé les types les plus fréquents `UInt64` et `Array(UInt64)` comme sous-colonnes, et a inséré tous les autres types dans les données partagées.
...
Dans le format RowBinary, les valeurs de type `Dynamic` sont sérialisées comme suit :
