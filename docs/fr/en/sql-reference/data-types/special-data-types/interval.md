...
Famille de types de données représentant des intervalles de date et d’heure. Types renvoyés par l’opérateur [INTERVAL](/fr/sql-reference/operators#interval).

Structure :

* Intervalle de temps sous la forme d’une valeur entière non signée.
* Type d’un intervalle.
...
Vous pouvez utiliser des valeurs de type `Interval` dans des opérations arithmétiques avec des valeurs de type [Date](../../../sql-reference/data-types/date.md) et [DateTime](../../../sql-reference/data-types/datetime.md). Par exemple, vous pouvez ajouter 4 jours à la date et à l’heure actuelles :
...
Il est également possible d’utiliser simultanément plusieurs intervalles :
...
Les intervalles de type mixte, par exemple plusieurs heures et plusieurs minutes, peuvent être créés à l’aide de la syntaxe `INTERVAL 'value' <from_kind> TO <to_kind>`.
Le résultat est un tuple de deux intervalles ou plus.

Combinaisons prises en charge :

| Syntaxe            | Format de chaîne | Exemple                               |
| ------------------ | ---------------- | ------------------------------------- |
...
Un signe `+` ou `-` facultatif placé au début s’applique à tous les éléments :
...
* opérateur [INTERVAL](/fr/sql-reference/operators#interval)
