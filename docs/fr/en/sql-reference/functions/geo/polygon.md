---
description: 'Documentation pour Polygon'
sidebar_label: 'Polygones'
slug: /sql-reference/functions/geo/polygons
title: 'Fonctions de manipulation des polygones'
doc_type: 'reference'
---
...
Renvoie un objet géométrique WKT (Well Known Text) à partir de divers [types de données géo](../../data-types/geo.md). Les objets WKT pris en charge sont :
...
`geo_data` peut correspondre à l&#39;un des [types de données Geo](../../data-types/geo.md) suivants ou à leurs types primitifs sous-jacents :
...
MULTIPOLYGON à partir d&#39;un tableau multidimensionnel de tableaux de tuples :
...
Convertit un WKT (Well Known Text) de type MultiPolygon en type Polygon.
...
La fonction `readWKTPoint` de ClickHouse analyse une représentation Well-Known Text (WKT) d&#39;une géométrie Point et renvoie un point dans le format interne de ClickHouse.
...
* `wkt_string` : la chaîne WKT d’entrée représentant une géométrie de type Point.

<div id="returned-value-2">
  ### Valeur renvoyée
</div>

La fonction renvoie la représentation interne de la géométrie Point dans ClickHouse.
...
Analyse une représentation Well-Known Text (WKT) d’une géométrie de type LineString et la renvoie dans le format interne de ClickHouse.
...
* `wkt_string` : La chaîne WKT en entrée représentant une géométrie de type LineString.
...
Analyse une représentation Well-Known Text (WKT) d&#39;une géométrie MultiLineString et la renvoie dans le format interne de ClickHouse.
...
* `wkt_string` : la chaîne WKT d’entrée qui représente une géométrie MultiLineString.
...
Analyse une représentation Well-Known Text (WKT) d’une géométrie Polygon et renvoie un Ring (linestring fermé) au format interne de ClickHouse.
...
* `wkt_string` : la chaîne WKT en entrée représentant une géométrie de type Polygon.

<div id="returned-value-2">
  ### Valeur renvoyée
</div>

La fonction renvoie la représentation interne de ClickHouse pour la géométrie de type ring (`linestring` fermé).
...
Convertit un MultiPolygon WKB (Well Known Binary) en Polygon.
...
La fonction `readWKBPoint` de ClickHouse analyse une représentation Well-Known Binary (WKB) d’un objet géométrique de type Point et renvoie un point au format interne de ClickHouse.
...
* `wkb_string` : la chaîne WKB en entrée représentant une géométrie de type Point.

<div id="returned-value-2">
  ### Valeur renvoyée
</div>

La fonction renvoie une représentation interne de ClickHouse du type géométrique Point.
...
Analyse une représentation Well-Known Binary (WKB) d&#39;une géométrie de type LineString et la renvoie au format interne de ClickHouse.
...
* `wkb_string` : chaîne WKB d’entrée représentant une géométrie de type LineString.

<div id="returned-value-2">
  ### Valeur renvoyée
</div>

La fonction renvoie une représentation interne, propre à ClickHouse, de la géométrie LineString.
...
Analyse la représentation Well-Known Binary (WKB) d’une géométrie MultiLineString et la renvoie dans le format interne de ClickHouse.
...
La fonction renvoie la représentation interne de ClickHouse pour la géométrie MultiLineString.
...
Renvoie true si deux polygones sont égaux
...
Calcule la différence symétrique spatiale entre deux polygones, au sens de la théorie des ensembles (XOR)
...
Identique à `polygonsSymDifferenceSpherical`, mais les coordonnées sont dans le système de coordonnées cartésien, qui se rapproche davantage du modèle de la Terre réelle.
...
Renvoie true si le deuxième polygone est contenu dans le premier polygone.
...
Renvoie true si les deux polygones s’intersectent (c’est-à-dire s’ils ont une zone ou une frontière commune).
...
Renvoie `true` si les deux polygones s’intersectent (partagent une zone ou une frontière communes). Référence https://www.boost.org/doc/libs/1&#95;62&#95;0/libs/geometry/doc/html/geometry/reference/algorithms/intersects.html
...
Calcule une enveloppe convexe. [Référence](https://www.boost.org/doc/libs/1_61_0/libs/geometry/doc/html/geometry/reference/algorithms/convex_hull.html)
...
<div id="polygon-representing-zimbabwe">
  #### Polygon représentant le Zimbabwe
</div>
...
Pour plus d’informations sur les systèmes géométriques, consultez cette [présentation](https://archive.fosdem.org/2020/schedule/event/working_with_spatial_trajectories_in_boost_geometry/attachments/slides/3988/export/events/attachments/working_with_spatial_trajectories_in_boost_geometry/slides/3988/FOSDEM20_vissarion.pdf) sur la bibliothèque Boost, utilisée par ClickHouse.