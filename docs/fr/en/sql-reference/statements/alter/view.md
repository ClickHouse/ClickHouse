---
description: 'Documentation de l’instruction ALTER TABLE ... MODIFY QUERY'
sidebar_label: 'VIEW'
sidebar_position: 50
...
Vous pouvez modifier la requête `SELECT` spécifiée lors de la création d’une [vue matérialisée](/fr/sql-reference/statements/create/view#materialized-view) à l’aide de l’instruction `ALTER TABLE ... MODIFY QUERY`, sans interrompre le processus d’ingestion.

Cette commande permet de modifier une vue matérialisée créée avec la clause `TO [db.]name`. Elle ne modifie ni la structure de la table de stockage sous-jacente ni la définition des colonnes de la vue matérialisée. Par conséquent, l’usage de cette commande est très limité pour les vues matérialisées créées sans clause `TO [db.]name`.
...
L’application est très limitée, puisque vous ne pouvez modifier que la section `SELECT`, sans pouvoir ajouter de nouvelles colonnes.
...
`ALTER TABLE ... MODIFY REFRESH` modifie les paramètres d&#39;actualisation d&#39;une [vue matérialisée actualisable](../create/view.md#refreshable-materialized-view), notamment la planification, les dépendances, l&#39;aléatoirisation et les [paramètres de rafraîchissement](../create/view.md#refresh-settings).

```sql
ALTER TABLE [db.]name MODIFY REFRESH EVERY|AFTER ... [RANDOMIZE FOR ...] [DEPENDS ON ...] [SETTINGS ...]
```

La planification (`EVERY` ou `AFTER`) est obligatoire : l’instruction remplace *tous* les paramètres de rafraîchissement en une seule fois. Toute clause non spécifiée — `RANDOMIZE FOR`, `DEPENDS ON` ou `SETTINGS` — est supprimée ou réinitialisée à ses valeurs par défaut. Pour modifier uniquement les paramètres de rafraîchissement, répétez la planification actuelle.
...
* `ALTER TABLE ... MODIFY SETTING` n’est pas pris en charge sur les vues matérialisées ; les paramètres de rafraîchissement ne peuvent être modifiés que via `MODIFY REFRESH`.
...
La liste complète des paramètres de rafraîchissement est documentée dans [Paramètres de rafraîchissement](../create/view.md#refresh-settings). L’état du rafraîchissement, y compris les paramètres actuellement appliqués, est visible dans [`system.view_refreshes`](../../../operations/system-tables/view_refreshes.md).