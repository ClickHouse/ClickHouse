---
description: 'Page consacrée au profilage des allocations dans ClickHouse'
sidebar_label: 'Profilage des allocations'
...
ClickHouse utilise [jemalloc](https://github.com/jemalloc/jemalloc) comme allocateur global. Jemalloc s’accompagne d’outils d’échantillonnage et de profilage des allocations.
...
* Afficher les statistiques mémoire en direct et récupérer des profils du tas via la [web UI jemalloc](#jemalloc-web-ui) intégrée (26.2+).
* Interroger directement depuis SQL le profil du tas actuel à l’aide de [`system.jemalloc_profile_text`](#fetching-heap-profiles-from-sql) (26.2+).
...
Pour les versions antérieures, veuillez consulter [le profilage des allocations pour les versions antérieures à 25.9](/fr/operations/allocation-profiling-old.md).
...
`jemalloc` échantillonnera les allocations et stockera les informations en interne.

Vous pouvez également activer l’échantillonnage pour chaque requête à l’aide du paramètre `jemalloc_enable_profiler`.

:::warning Avertissement
Comme ClickHouse est une application qui effectue beaucoup d’allocations, l’échantillonnage de jemalloc peut entraîner un surcoût en termes de performances.
:::
...
:::warning Avertissement
Comme ClickHouse est une application qui effectue beaucoup d’allocations mémoire, la collecte de tous les échantillons dans system.trace&#95;log peut entraîner une charge importante.
:::
...
Commencez par exécuter une requête avec le profileur jemalloc activé et enregistrez les échantillons dans `system.trace_log` :
...
:::note
Si ClickHouse a été démarré avec `jemalloc_enable_global_profiler`, il n’est pas nécessaire d’activer `jemalloc_enable_profiler`.
...
Ensuite, interrogez-le pour obtenir l’utilisation cumulée de la mémoire au fil du temps :
...
Repérez le moment où l’utilisation de la mémoire était la plus élevée :
...
À partir de ce résultat, voyez quelles piles d’allocation ont été les plus actives au pic :
...
Vous pouvez également récupérer directement depuis l’interface des profils de tas globaux et par requête.
...
<Tabs groupId="binary">
  ...
  <TabItem value="keeper" label="Keeper">
    ...
    :::warning Sécurité
    Le port de contrôle HTTP de Keeper ne dispose pas d’authentification au niveau de l’application. Contrairement à l’interface jemalloc de ClickHouse Server — où toutes les requêtes de données passent par le handler HTTP SQL et nécessitent des identifiants utilisateur/mot de passe — les points de terminaison de l’API REST de Keeper ne sont pas authentifiés. Cela est cohérent avec les autres points de terminaison de contrôle HTTP de Keeper (commands, storage, dashboard).

    Restreignez l’accès à ce port à l’aide de contrôles réseau : liez Keeper à localhost, utilisez des règles de pare-feu ou placez-le derrière un proxy inverse avec authentification. Lorsqu’aucun `listen_host` n’est configuré, Keeper écoute par défaut uniquement sur localhost.
    :::
    ...
    * `GET /jemalloc/profile?format={collapsed|raw}` — déclenche l’écriture d’un profil de tas avec symbolisation côté serveur, renvoie des collapsed stacks adaptés au rendu d’un flame graph (par défaut) ou le dump jemalloc brut
  </TabItem>
</Tabs>

<div id="fetching-heap-profiles-from-sql">
  ## Récupération des profils heap depuis SQL
</div>
...
Vous pouvez interroger la table directement — il n&#39;est pas nécessaire d&#39;écrire un profil du tas au préalable sur disque :
...
Le format de sortie est contrôlé par le paramètre `jemalloc_profile_text_output_format`, qui prend en charge trois valeurs :

* `raw` — profil du tas brut tel que généré par jemalloc.
* `symbolized` — format compatible avec jeprof, avec symboles de fonction intégrés. Les symboles étant déjà intégrés, `jeprof` peut analyser la sortie sans nécessiter le binaire ClickHouse.
* `collapsed` (par défaut) — collapsed stacks compatibles avec FlameGraph, une stack par ligne avec le nombre d’octets.
...
Pour obtenir une sortie avec symboles :
...
* `jemalloc_profile_text_symbolize_with_inline` (Bool, par défaut : `true`) — Indique s’il faut inclure les frames inline lors de la symbolisation. Désactiver ce paramètre accélère considérablement la symbolisation, mais réduit la précision, car les appels de fonctions inline n’apparaîtront pas dans les piles d’appels. Cela n’affecte que les formats `symbolized` et `collapsed`.
* `jemalloc_profile_text_collapsed_use_count` (Bool, par défaut : `false`) — Lors de l’utilisation du format `collapsed`, agrège selon le nombre d’allocations plutôt qu’en octets.

<div id="example-flamegraph-from-sql">
  ### Exemple : générer un flame graph à partir de SQL
</div>

Comme le format de sortie par défaut est `collapsed`, vous pouvez rediriger la sortie directement vers FlameGraph avec un pipe :
...
Pour générer un flame graph en fonction du nombre d’allocations plutôt qu’en octets :
...
<div id="flushing-heap-profiles">
  ## Enregistrement sur disque des profils du tas
</div>

Si vous devez enregistrer les profils du tas sous forme de fichiers pour une analyse hors ligne avec `jeprof`, vous pouvez les écrire sur disque.

Par défaut, le fichier de profil du tas est généré dans `/tmp/jemalloc_clickhouse._pid_._seqnum_.heap`, où `_pid_` correspond au PID de ClickHouse et `_seqnum_` au numéro de séquence global du profil du tas actuel.
Pour Keeper, le fichier par défaut est `/tmp/jemalloc_keeper._pid_._seqnum_.heap` et suit les mêmes règles.

Pour écrire le profil actuel :
...
Par exemple, si vous souhaitez générer des profils dans le dossier `/data` avec `my_current_profile` comme préfixe de nom de fichier, vous pouvez exécuter ClickHouse/Keeper avec la variable d’environnement suivante :

```sh
MALLOC_CONF=prof_prefix:/data/my_current_profile
```

Le fichier généré sera suffixé par le PID et un numéro de séquence.

<div id="analyzing-heap-profile-files-with-jeprof">
  ## Analyse des fichiers de profils de tas avec `jeprof`
</div>

Après avoir écrit les profils de tas sur le disque, ils peuvent être analysés à l&#39;aide de l&#39;outil `jemalloc` appelé [jeprof](https://github.com/jemalloc/jemalloc/blob/dev/bin/jeprof.in). Il peut être installé de plusieurs façons :

* En utilisant le gestionnaire de paquets du système
* En clonant le [dépôt jemalloc](https://github.com/jemalloc/jemalloc) et en exécutant `autogen.sh` depuis le dossier racine. Cela vous fournira le script `jeprof` dans le dossier `bin`
...
Le profil symbolisé (avec l’extension `.symbolized`) contient les symboles de fonction intégrés et peut être analysé par `jeprof` sans avoir besoin du binaire ClickHouse.
...
Vous pourrez ensuite l’analyser directement avec `jeprof` :
...
Si vous disposez d’un ancien profil du tas non symbolisé et avez toujours accès au binaire ClickHouse, vous pouvez utiliser l’approche traditionnelle :
...
Dans ce cas, il est recommandé d&#39;installer une [implémentation alternative](https://github.com/gimli-rs/addr2line) de cet outil.
...
Sinon, `llvm-addr2line` fonctionne tout aussi bien (mais notez que `llvm-objdump` n&#39;est pas compatible avec `jeprof`)
...
Lorsque vous comparez deux profils, vous pouvez utiliser l&#39;argument `--base` :
...
Avec des profils symbolisés (recommandé) :

* Générez un fichier texte avec chaque procédure sur une ligne distincte :
...
`jeprof` permet de générer des collapsed stacks pour créer des flame graphs.
...
Après cela, vous pouvez utiliser de nombreux outils pour visualiser des collapsed stacks.

Le plus populaire est [FlameGraph](https://github.com/brendangregg/FlameGraph), qui inclut un script appelé `flamegraph.pl` :

```sh
cat result.collapsed | /path/to/FlameGraph/flamegraph.pl --color=mem --title="Allocation Flame Graph" --width 2400 > result.svg
```

Un autre outil intéressant est [speedscope](https://www.speedscope.app/), qui vous permet d’analyser les piles d’appels collectées de façon plus interactive.

<div id="additional-options-for-profiler">
  ## Options supplémentaires du profileur
</div>

`jemalloc` propose de nombreuses options liées au profileur. Elles peuvent être réglées en modifiant la variable d&#39;environnement `MALLOC_CONF`.
Par exemple, l&#39;intervalle entre les échantillons d&#39;allocation peut être défini avec `lg_prof_sample`.
...
:::warning Avertissement
Il est important de garder à l&#39;esprit qu&#39;aucune de ces métriques n&#39;est synchronisée avec les autres et que leurs valeurs peuvent diverger.
:::
...
Contient des informations sur les allocations mémoire effectuées par l’allocateur jemalloc dans différentes classes de taille (`bins`), agrégées sur l’ensemble des arenas.
...
Renvoie l’intégralité de la sortie de `malloc_stats_print()` sous la forme d’une seule chaîne. Équivaut à la commande `SYSTEM JEMALLOC STATS`.
...
<div id="jmst-4lw-command-in-keeper">
  ### Commande 4LW `jmst` de Keeper
</div>

Keeper prend en charge la commande 4LW `jmst`, qui renvoie les [statistiques de base sur l’allocateur](https://github.com/jemalloc/jemalloc/wiki/Use-Case%3A-Basic-Allocator-Statistics) :
