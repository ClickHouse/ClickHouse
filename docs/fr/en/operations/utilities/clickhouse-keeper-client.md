...
Une application cliente pour interagir avec ClickHouse Keeper via son protocole natif.
...
* `-c FILE_PATH`, `--config-file=FILE_PATH` — Définit le chemin du fichier de configuration à utiliser pour obtenir la chaîne de connexion. Valeur par défaut : `config.xml`.
...
* `--connection-timeout=TIMEOUT` — Définit le délai d&#39;expiration de la connexion en secondes. Valeur par défaut : 10s.
* `--session-timeout=TIMEOUT` — Définit le délai d&#39;expiration de la session en secondes. Valeur par défaut : 10s.
...
* `--no-confirmation` — Si cette option est définie, aucune confirmation ne sera demandée pour certaines commandes. Valeur par défaut : `false` en mode interactif et `true` pour une requête
...
* `CLICKHOUSE_KEEPER_PASSWORD` — Utilisé comme mot de passe par défaut si `--password` n’est pas indiqué en ligne de commande.
* `CLICKHOUSE_KEEPER_IDENTITY` — Utilisé comme identité par défaut si `--identity` n’est pas indiqué en ligne de commande.

<div id="clickhouse-keeper-client-auth">
  ## Authentification
</div>

Lors de la connexion à un Keeper server nécessitant une authentification, le mot de passe est déterminé selon l&#39;ordre de priorité suivant (la première correspondance prévaut) :

1. l&#39;argument de ligne de commande `--password`
2. la variable d&#39;environnement `CLICKHOUSE_KEEPER_PASSWORD`
...
Exemple de fichier de configuration XML avec des paramètres d&#39;authentification :
...
* `ls '[path]' [watch_id]` -- Liste les nœuds pour le chemin indiqué (par défaut : cwd). Définit éventuellement un watch sur les nœuds enfants identifié par `watch_id`
* `cd '[path]'` -- Change le chemin de travail (par défaut `.`)
...
* `touch '<path>'` -- Crée un nouveau nœud avec une chaîne vide comme valeur. Ne lance pas d&#39;exception si le nœud existe déjà
* `get '<path>' [watch_id]` -- Renvoie la valeur du nœud. Définit éventuellement un watch sur les données identifié par `watch_id`
* `watch <watch_id> [timeout_seconds]` -- Attend l&#39;événement watch identifié par `watch_id` et affiche le type d&#39;événement et le chemin. Si `timeout_seconds` est spécifié, renvoie une erreur après le délai indiqué
...
* `get_direct_children_number '[path]'` -- Obtient le nombre de nœuds enfants directs sous un chemin spécifique
* `get_all_children_number '[path]'` -- Obtient le nombre total de nœuds enfants sous un chemin spécifique
...
* `delete_stale_backups` -- Supprime les nœuds ClickHouse utilisés pour les sauvegardes désormais inactives
* `find_big_family [path] [n]` -- Renvoie les n premiers nœuds avec la plus grande famille dans le sous-arbre (chemin par défaut = `.` et n = 10)
