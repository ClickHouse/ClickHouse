---
description: "Table système contenant les traces de pile de tous les threads du serveur. Permet aux développeurs d’inspecter l’état du serveur."
...
Contient les traces de pile de tous les threads du serveur. Permet aux développeurs d’examiner l’état du serveur.

Pour analyser les frames de pile, utilisez `addressToLine`, `addressToLineWithInlines`, `addressToSymbol` et `demangle` [fonctions d’introspection](../../sql-reference/functions/introspection.md).
...
* `thread_name` ([String](/fr/sql-reference/data-types/string)) — Le nom du thread.
* `thread_id` ([UInt64](/fr/sql-reference/data-types/int-uint)) — L’identifiant du thread
* `query_id` ([String](/fr/sql-reference/data-types/string)) — L’ID de la requête à laquelle ce thread appartient.
* `trace` ([Array(UInt64)](/fr/sql-reference/data-types/array)) — La stacktrace de ce thread. Il s’agit essentiellement d’un tableau d’adresses.
* `untracked_memory` ([Int64](/fr/sql-reference/data-types/int-uint)) — Compteur par thread, sans opérations atomiques, des allocations mémoire qui n’ont pas encore été propagées au MemoryTracker parent. Peut être négatif si davantage de mémoire a été libérée qu’allouée depuis le dernier flush.
...
Activer les fonctions d’introspection :

```sql
SET allow_introspection_functions = 1;
```

Extraction des symboles depuis les fichiers objet de ClickHouse :
...
* [Fonctions d’introspection](../../sql-reference/functions/introspection.md) — Quelles sont les fonctions d’introspection disponibles et comment les utiliser.
* [system.trace&#95;log](../system-tables/trace_log.md) — Contient les traces de pile collectées par le profiler de requêtes par échantillonnage.
* [arrayMap](/fr/sql-reference/functions/array-functions#arrayMap)) — Description et exemple d’utilisation de la fonction `arrayMap`.
* [arrayFilter](/fr/sql-reference/functions/array-functions#arrayFilter) — Description et exemple d’utilisation de la fonction `arrayFilter`.