---
description: 'Table système utile aux experts C++ et aux ingénieurs de ClickHouse, contenant
  des informations d''introspection sur le binaire `clickhouse`.'
...
Contient des informations permettant l’introspection du binaire `clickhouse`. Son accès nécessite le privilège d’introspection.
Cette table n’est utile qu’aux experts en C++ et aux ingénieurs de ClickHouse.
...
* `symbol` ([String](/fr/sql-reference/data-types/string)) — Nom du symbole dans le binaire. Il est manglé. Vous pouvez appliquer `demangle(symbol)` pour obtenir un nom lisible.
* `symbol_demangled` ([Nullable(String)](/fr/sql-reference/data-types/nullable)) — Symbole démanglé utilisé pour l’instrumentation XRay.
* `function_id` ([Nullable(Int32)](/fr/sql-reference/data-types/nullable)) — ID de la fonction dans la table de correspondance d’instrumentation XRay.
* `address_begin` ([UInt64](/fr/sql-reference/data-types/int-uint)) — Adresse de début du symbole dans le binaire.
* `address_end` ([UInt64](/fr/sql-reference/data-types/int-uint)) — Adresse de fin du symbole dans le binaire.
