---
description: 'Table système contenant des informations utiles pour un aperçu de l’utilisation de la mémoire
  et des ProfileEvents des utilisateurs.'
...
Cette table système peut être utilisée pour obtenir une vue d’ensemble de l’utilisation de la mémoire et des ProfileEvents des utilisateurs.
...
* `user` ([String](/fr/sql-reference/data-types/string)) — Nom de l’utilisateur.
* `memory_usage` ([Int64](/fr/sql-reference/data-types/int-uint)) — Somme de la RAM utilisée par tous les processus de l’utilisateur. Elle peut ne pas inclure certains types de mémoire dédiée. Voir le paramètre `max_memory_usage`.
* `peak_memory_usage` ([Int64](/fr/sql-reference/data-types/int-uint)) — Pic d’utilisation de la mémoire de l’utilisateur. Il peut être réinitialisé lorsqu’aucune requête n’est exécutée pour cet utilisateur.
* `ProfileEvents` ([Map(LowCardinality(String), UInt64)](/fr/sql-reference/data-types/map)) — Résumé des ProfileEvents qui mesurent différentes métriques pour l’utilisateur. Leur description se trouve dans la table `system.events`
