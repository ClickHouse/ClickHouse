---
description: 'Le moteur Hive vous permet d’effectuer des requêtes `SELECT` sur une
...
* Les noms de colonnes doivent être identiques à ceux de la table Hive d&#39;origine, mais vous pouvez n&#39;utiliser qu&#39;une partie de ces colonnes, dans n&#39;importe quel ordre, et vous pouvez également utiliser des colonnes d&#39;alias calculées à partir d&#39;autres colonnes.
* Les types de colonnes doivent être les mêmes que dans la table Hive d&#39;origine.
* L&#39;expression de partitionnement doit être cohérente avec celle de la table Hive d&#39;origine, et les colonnes utilisées dans cette expression doivent figurer dans la structure de la table.
...
* enable : ClickHouse conservera un cache local pour le système de fichiers distant (HDFS) après le démarrage si la valeur est `true`.
* root&#95;dir : Obligatoire. Le répertoire racine où stocker les fichiers du cache local pour le système de fichiers distant.
* limit&#95;size : Obligatoire. La taille maximale (en octets) des fichiers du cache local.
* bytes&#95;read&#95;before&#95;flush : Contrôle le nombre d’octets lus avant l’écriture sur le système de fichiers local lors du téléchargement d’un fichier depuis le système de fichiers distant. La valeur par défaut est de 1MB.
...
<div id="query-hive-table-with-parquet-input-format">
  ### Interroger une table Hive en utilisant le format d&#39;entrée Parquet
</div>
...
<div id="query-hive-table-with-text-input-format">
  ### Interroger une table Hive au format d’entrée Text
</div>
