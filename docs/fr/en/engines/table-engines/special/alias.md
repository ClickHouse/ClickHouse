...
<div id="alias-table-engine">
  # Moteur de table Alias
</div>

Le moteur `Alias` sert de proxy vers une autre table. Toutes les opérations de lecture et d’écriture sont transmises à la table cible, tandis que l’alias lui-même ne stocke aucune donnée et conserve uniquement une référence à cette table.
...
:::note
La table `Alias` ne prend pas en charge la définition explicite des colonnes. Les colonnes sont automatiquement héritées de la table cible. Cela garantit que l’alias correspond toujours au schéma de la table cible.
:::
...
:::note
Lorsque `target_db` est omis et que `target_table` n’est pas entièrement qualifiée (par exemple, `Alias('my_table')`), la cible est résolue dans la même base de données que l’alias lui-même, et non dans la base de données courante de la session.
:::
...
Le moteur de table `Alias` prend en charge toutes les principales opérations. 
...
| `INSERT SELECT`              | ✅               | Insérer par lot dans la table cible                             |
...
| `TRUNCATE TABLE`             | ✅               | Tronquer la table cible                                         |
...
Optimisez les opérations de fusion des parts dans la table cible :
