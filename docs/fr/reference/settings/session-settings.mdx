...
Nom de la collection nommée utilisée par `aiEmbed` lorsque l’appel ne transmet pas `credentials` dans sa map de paramètres. Une valeur vide signifie qu’il n’y a pas de valeur par défaut : ces appels doivent transmettre `credentials` explicitement. Ce paramètre est distinct de `ai_function_text_default_credentials`, car l’endpoint et le modèle d’embedding diffèrent de ceux d’un chat.
...
Nom de la collection nommée utilisée par les fonctions d’IA textuelles (`aiGenerate`, `aiClassify`, `aiExtract`, `aiTranslate`) lorsque l’appel ne transmet pas `credentials` dans sa map de paramètres. Une valeur vide signifie qu’il n’y a pas de valeur par défaut : ces appels doivent transmettre `credentials` explicitement. Un endpoint et un modèle de chat-completions étant différents de ceux utilisés pour les embeddings, ce paramètre est distinct de `ai_function_embedding_default_credentials`.
...
* Les résultats des opérations `t1 ANY LEFT JOIN t2` et `t2 ANY RIGHT JOIN t1` ne sont pas identiques, car ClickHouse utilise une logique de correspondance des clés des tables de gauche à droite, de plusieurs vers une.
...
<Note>
  ...
  Toutefois, le paramètre `compatibility` peut être redéfini au niveau de l’utilisateur, du rôle, du profil, de la requête ou de la session à l’aide des mécanismes standard de configuration de ClickHouse, par exemple `SET compatibility = '22.3'` dans une session ou `SETTINGS compatibility = '22.3'` dans une requête.
</Note>
...
Il peut arriver que l’envoi d’un lot donné vers le shard distant échoue à cause d’un pipeline en aval complexe (par ex. `MATERIALIZED VIEW` avec `GROUP BY`), en raison de `Memory limit exceeded` ou d’erreurs similaires. Dans ce cas, réessayer ne servira à rien (et bloquera les envois distribués pour la table), tandis que l’envoi des fichiers de ce lot un par un peut permettre à `INSERT` de réussir.

Ainsi, définir ce paramètre sur `1` désactive le batching pour ces lots (c.-à-d. désactive temporairement `distributed_background_insert_batch` pour les lots en échec).
...
<Note>
  Vous ne devez pas vous reposer sur le fractionnement automatique des lots, car cela peut nuire aux performances.
</Note>
...
Ce paramètre est nécessaire pour les requêtes où la cohérence interne des parties de données est importante. Exemple :
...
<Note>
  `max_query_size` ne peut pas être défini dans une requête SQL (par ex., `SELECT now() SETTINGS max_query_size=10000`) car ClickHouse doit allouer un tampon pour analyser la requête, et la taille de ce tampon est déterminée par le paramètre `max_query_size`, qui doit être configuré avant l’exécution de la requête.
</Note>
...
Pour les tables [MergeTree](/fr/reference/engines/table-engines/mergetree-family/mergetree). Afin de réduire la latence du traitement des requêtes, un bloc est compressé lors de l’écriture de la marque suivante si sa taille est au moins égale à `min_compress_block_size`. Par défaut, 65 536.

La taille réelle du bloc, si les données non compressées sont inférieures à `max_compress_block_size`, n’est inférieure ni à cette valeur, ni au volume de données correspondant à une marque.

Prenons un exemple. Supposons que `index_granularity` ait été défini sur 8192 lors de la création de la table.

Nous écrivons une colonne de type UInt32 (4 octets par valeur). Lors de l’écriture de 8192 lignes, cela représente un total de 32 Ko de données. Comme min&#95;compress&#95;block&#95;size = 65 536, un bloc compressé sera formé toutes les deux marques.

Nous écrivons une colonne URL de type String (taille moyenne de 60 octets par valeur). Lors de l’écriture de 8192 lignes, cela représente en moyenne un peu moins de 500 Ko de données. Comme cette valeur dépasse 65 536, un bloc compressé sera formé pour chaque marque. Dans ce cas, lors de la lecture des données sur le disque dans la plage d’une seule marque, aucune donnée supplémentaire ne devra être décompressée.
...
<Note>
  Il s’agit d’un paramètre réservé aux experts, qui ne doit être utilisé pour le débogage que par les développeurs. Ce paramètre pourra évoluer ultérieurement de manière non rétrocompatible, voire être supprimé.
</Note>
...
<Note>
  Il s&#39;agit d&#39;un paramètre réservé aux experts, qui ne doit être utilisé par les développeurs qu&#39;à des fins de débogage. Ce paramètre peut être modifié ultérieurement de manière non rétrocompatible, voire supprimé.
</Note>
...
<Note>
  Il s’agit d’un paramètre réservé aux experts qui ne doit être utilisé à des fins de débogage que par les développeurs. Ce paramètre pourra, à l’avenir, changer de manière non rétrocompatible avec les versions précédentes ou être supprimé.
</Note>
...
<Note>
  Il s’agit d’un paramètre réservé aux experts, qui ne doit être utilisé pour le débogage que par les développeurs. Ce paramètre peut évoluer à l’avenir de manière non rétrocompatible, voire être supprimé.
</Note>
...
<Note>
  Il s&#39;agit d&#39;un paramètre réservé aux experts, qui ne doit être utilisé à des fins de débogage que par les développeurs. Ce paramètre pourra, à l&#39;avenir, être modifié de manière non rétrocompatible avec les versions précédentes ou être supprimé.
</Note>
...
* &#39;dpsub&#39; - implémente l’algorithme DPsub, qui prend en charge à la fois les jointures internes et les jointures non internes - examine tous les ordres de jointure possibles et trouve le meilleur, mais peut être lent pour les requêtes comportant de nombreuses tables et de nombreux prédicats de jointure.
...
<Note>
  Il s&#39;agit d&#39;un paramètre réservé aux experts qui ne doit être utilisé que par les développeurs à des fins de débogage. Ce paramètre pourra, à l&#39;avenir, être modifié de manière non rétrocompatible ou supprimé.
</Note>
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "0.2"},{"label": "Anciennement `text_index_density_threshold` (conservé comme alias) ; sélectionne l’algorithme d’intersection des listes de postings en mode d’application lazy des listes de postings."}]}]} />

Seuil de densité des listes de postings qui sélectionne l’algorithme d’intersection en mode d’application lazy des listes de postings (`text_index_posting_list_apply_mode = 'lazy'`).
En dessous du seuil : intersection leapfrog (privilégie les listes de postings clairsemées). À partir du seuil : intersection bitmap en brute-force (privilégie les listes de postings denses).
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "materialize"},{"label": "Nouveau paramètre pour le mode lazy d’application des listes de postings"}]}]} />

Contrôle la façon dont les listes de postings sont appliquées lors des requêtes sur l’index de texte.
&#39;materialize&#39; (par défaut) décode immédiatement les listes de postings en Roaring Bitmaps.
&#39;lazy&#39; utilise un décodage à la demande basé sur un curseur (nécessite un format d’index avec un codec sérialisé).
...
<Note>
  - avec `use_compact_format_in_distributed_parts_names=0`, les modifications apportées à la définition du cluster ne seront pas appliquées aux INSERT en arrière-plan.
