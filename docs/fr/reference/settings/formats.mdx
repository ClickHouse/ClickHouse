...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "1"},{"label": "Nouveau paramètre qui détermine si le format GeoJSON applique la validation de la géométrie selon la RFC 7946 (nombre minimal de points par ligne et par anneau, fermeture des anneaux, multi-géométries non vides) à la lecture et à l'écriture"}]}]} />

Détermine si le format `GeoJSON` applique la validation de la géométrie selon la RFC 7946, à la lecture comme à l&#39;écriture.

Lorsqu&#39;elle est activée (par défaut), toute géométrie qui enfreint les règles de structure GeoJSON est rejetée : une `LineString` (ou une ligne d&#39;une `MultiLineString`) avec moins de deux points ; un anneau de `Polygon` ou de `MultiPolygon` avec moins de quatre points, ou dont le premier et le dernier point diffèrent (anneau non fermé) ; ou encore une `MultiLineString`, un `Polygon` ou un `MultiPolygon` vide. Cela s&#39;applique aussi bien à la lecture (un tel document est rejeté) qu&#39;à l&#39;écriture (une telle valeur ClickHouse est rejetée au lieu de produire un document que le format d&#39;entrée rejetterait).

Lorsqu&#39;elle est désactivée, ces règles de structure ne sont pas appliquées : ces géométries sont lues telles quelles et écrites telles quelles. Les géométries dégénérées peuvent donc faire un aller-retour complet, mais le document écrit peut ne pas être un GeoJSON valide.

La validation est uniquement structurelle : elle vérifie le nombre de points et la fermeture des anneaux. Elle n&#39;examine pas la validité géométrique d&#39;une forme — l&#39;orientation des anneaux (règle de la main droite / ordre d&#39;enroulement) n&#39;est pas imposée, et les géométries structurellement valides mais géométriquement dégénérées sont acceptées, comme un polygone de surface nulle, un anneau auto-intersectant ou un polygone dont les trous se trouvent à l&#39;extérieur de son anneau extérieur. Les coordonnées non finies (`NaN`, `Inf`) sont toujours rejetées, quel que soit ce paramètre, car elles ne peuvent pas être représentées sous forme de nombres JSON.
...
Lorsque cette option est activée, si la connexion se ferme de manière inattendue, les données restantes dans le tampon seront analysées et traitées au lieu d’être considérées comme une erreur
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "25.5"},{"label": "1"},{"label": "Lors de la lecture de fichiers Parquet, ignore des groupes de lignes entiers en fonction des expressions WHERE/PREWHERE et du filtre de Bloom dans les métadonnées Parquet."}]}, {"id": "row-2","items": [{"label": "24.10"},{"label": "0"},{"label": "Lors de la lecture de fichiers Parquet, ignore des groupes de lignes entiers en fonction des expressions WHERE/PREWHERE et du filtre de Bloom dans les métadonnées Parquet."}]}]} />

Lors de la lecture de fichiers Parquet, ignore des groupes de lignes entiers en fonction des expressions WHERE et du filtre de Bloom dans les métadonnées Parquet.
...
Utiliser le type String d’Arrow au lieu de Binary pour les colonnes de type String
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "1"},{"label": "Nouveau paramètre. Lorsque output_format_csv_serialize_tuple_into_separate_columns est activé, l’en-tête CSVWithNames\/CSVWithNamesAndTypes aplatit désormais les colonnes Tuple en champs feuilles afin que la largeur de l’en-tête corresponde aux données. Définissez false pour restaurer l’ancien en-tête à nom unique."}]}]} />

Lorsque [output&#95;format&#95;csv&#95;serialize&#95;tuple&#95;into&#95;separate&#95;columns](#output_format_csv_serialize_tuple_into_separate_columns) est activé, les lignes d’en-tête de `CSVWithNames` et `CSVWithNamesAndTypes` aplatissent chaque colonne Tuple en champs feuilles (noms à points comme `t.a`, `t.b`, ainsi que les noms de type des feuilles), de sorte que l’en-tête ait le même nombre de colonnes que les données. Pour `CustomSeparated*`, cet aplatissement s’applique uniquement lorsque `format_custom_escaping_rule = 'CSV'` et que `format_custom_field_delimiter` est un caractère unique égal à `format_csv_delimiter` ; sinon (par exemple avec le délimiteur de tabulation par défaut ou `format_custom_field_delimiter = '|'`), l’en-tête reste non aplati afin de continuer à correspondre aux données. Définissez-le sur `0` pour conserver le comportement précédent, où l’en-tête garde le nom et le type uniques du Tuple de niveau supérieur.

Remarque : un en-tête aplati n’est pas relu dans un Tuple par nom lorsque `input_format_with_names_use_header = 1`. Pour relire ces données dans un Tuple, définissez soit ce paramètre sur `0` en sortie, soit lisez avec `input_format_with_names_use_header = 0` (et, pour les formats `*WithNamesAndTypes` `CSVWithNamesAndTypes` et `CustomSeparatedWithNamesAndTypes`, définissez également `input_format_with_types_use_header = 0`, car sinon la ligne de types aplatie est validée par rapport à l’unique champ Tuple d’entrée de niveau supérieur, puis rejetée).
