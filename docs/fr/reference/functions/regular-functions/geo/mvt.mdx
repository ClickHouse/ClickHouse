...
tuile slippy-map identifiée par `zoom`, `tile_x` et `tile_y`, l’aligne sur la grille de pixels entiers, la découpe à la tuile
et renvoie la géométrie en espace de la tuile.
...
`MVTEncodeGeom` fusionne en un seul groupe les géométries qui tombent sur la même grille.
...
Les coordonnées des polygones sont bornées à une fenêtre `2^30` avant validation — soit exactement l’étendue en pixels du monde entier à
...
que les géométries placées à des valeurs extrêmes de `zoom` ou de `extent`.

Le type de géométrie de sortie dépend de l’entrée : un `Point` renvoie un `Point` ; un `MultiPoint` renvoie un `MultiPoint` ; un `LineString` ou `MultiLineString` renvoie un
...
* `buffer` — tampon de découpe facultatif, en pixels, dans la plage `[0, 2147483647]`. La valeur par défaut est `1`. [`UInt32`](/fr/reference/data-types/int-uint).
* `clip` — Indicateur facultatif ; lorsqu’il est différent de zéro (valeur par défaut), la géométrie est découpée selon la tuile plus le tampon de découpe. [`UInt8`](/fr/reference/data-types/int-uint).
