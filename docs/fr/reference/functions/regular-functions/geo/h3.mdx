...
Renvoie les hexagones (à la résolution spécifiée) compris dans la géométrie fournie, qu’il s’agisse d’un Ring ou d’un (multi-)Polygon. Un hexagone est considéré comme contenu lorsque la géométrie contient son centre, ce qui correspond au mode de containment `0` de [h3PolygonToCellsWithContainment](#h3polygontocellswithcontainment).

Chaque sommet de la géométrie doit se trouver sur la sphère : longitude dans la plage `[-180, 180]` et latitude dans la plage `[-90, 90]` degrés. L’ordre des cellules renvoyées n’est pas garanti ; triez donc le tableau lorsque l’ordre est important.
...
Chaque sommet de la géométrie doit se trouver sur la sphère : longitude comprise dans la plage `[-180, 180]` et latitude comprise dans la plage `[-90, 90]` degrés. L’ordre des cellules renvoyées n’est pas garanti ; triez donc le tableau lorsque l’ordre est important.
...
Renvoie un index H3 d’arête unidirectionnelle pour deux index de cellules H3 adjacentes (origine et destination). Lève une exception si l’une des entrées n’est pas une cellule H3 valide (ce comportement est contrôlé par le paramètre `functions_h3_default_if_invalid`). Pour des cellules valides qui ne sont pas voisines (y compris la même cellule deux fois), renvoie `0`.
...
Renvoie l’index de l’hexagone d’origine à partir de l’arête unidirectionnelle H3Index. Lève une exception si l’entrée n’est pas une arête dirigée valide (contrôlé par le paramètre `functions_h3_default_if_invalid`) ; renvoie `0` uniquement lorsque ce paramètre est activé.
...
* Lève `INCORRECT_DATA` pour les arêtes dirigées non valides par défaut ; renvoie `0` lorsque `functions_h3_default_if_invalid = 1`.
...
Renvoie l’index du hexagone de destination à partir de l’arête unidirectionnelle H3Index. Lève une exception si l’entrée n’est pas une arête dirigée valide (contrôlé par le paramètre `functions_h3_default_if_invalid`) ; renvoie `0` uniquement lorsque ce paramètre est activé.
...
Lève une exception si l’entrée fournie n’est pas une arête dirigée valide (contrôlé par le paramètre `functions_h3_default_if_invalid`) ; renvoie `(0,0)` uniquement lorsque ce paramètre est activé.
...
Renvoie toutes les arêtes unidirectionnelles de l’H3Index fourni. Lève une exception si l’entrée n’est pas une cellule H3 valide (selon le paramètre `functions_h3_default_if_invalid`) ; renvoie un tableau vide uniquement lorsque ce paramètre est activé.
...
* `index` — Numéro d&#39;index de l’hexagone représentant une cellule. [UInt64](/fr/reference/data-types/int-uint).
...
* Lève `INCORRECT_DATA` pour les cellules non valides par défaut ; renvoie `[]` lorsque `functions_h3_default_if_invalid = 1`.
...
Renvoie les coordonnées qui définissent l’arête unidirectionnelle. Lève une exception si l’entrée n’est pas une arête dirigée valide (contrôlé par le paramètre `functions_h3_default_if_invalid`) ; renvoie un tableau vide uniquement lorsque ce paramètre est activé.
...
* Array de couples &#39;(lat, lon)&#39;. [Array](/fr/reference/data-types/array)([Float64](/fr/reference/data-types/float), [Float64](/fr/reference/data-types/float)).
* Lève `INCORRECT_DATA` pour les arêtes dirigées non valides par défaut ; renvoie `[]` lorsque `functions_h3_default_if_invalid = 1`.
