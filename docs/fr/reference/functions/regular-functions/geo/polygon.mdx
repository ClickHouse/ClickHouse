---
description: 'Documentation sur Polygon'
sidebarTitle: 'Polygons'
slug: /sql-reference/functions/geo/polygons
title: 'Fonctions de manipulation des Polygons'
doc_type: 'reference'
---
...
Renvoie un objet géométrique WKT (Well Known Text) à partir de différents [types de données Geo](/fr/reference/data-types/geo). Les objets WKT pris en charge sont :
...
`geo_data` peut être l’un des [types de données Geo](/fr/reference/data-types/geo) suivants, ou l’un de leurs types primitifs sous-jacents :

* [Point](/fr/reference/data-types/geo#point)
* [MultiPoint](/fr/reference/data-types/geo#multipoint)
...
* [LineString](/fr/reference/data-types/geo#linestring)
* [MultiLineString](/fr/reference/data-types/geo#multilinestring)

**Valeur renvoyée**

* L’objet géométrique WKT `POINT` est renvoyé pour un Point.
* L’objet géométrique WKT `MULTIPOINT` est renvoyé pour un MultiPoint.
* L’objet géométrique WKT `POLYGON` est renvoyé pour un Polygon.
* L’objet géométrique WKT `MULTIPOLYGON` est renvoyé pour un MultiPolygon.
* L’objet géométrique WKT `LINESTRING` est renvoyé pour un LineString.
* L’objet géométrique WKT `MULTILINESTRING` est renvoyé pour un MultiLineString.

**Exemples**

POINT à partir d’un tuple :
...
POLYGON à partir d’un tableau de tuples ou d’un tableau de tableaux de tuples :
...
MULTIPOLYGON à partir d&#39;un tableau de tableaux de tuples multidimensionnels :
...
<div id="readwktmultipolygon">
  ## readWKTMultiPolygon
</div>

Convertit un MultiPolygon au format WKT (Well Known Text) en type MultiPolygon.
...
chaîne commençant par `MULTIPOLYGON`
...
<div id="readwktpolygon">
  ## readWKTPolygon
</div>

Convertit un MultiPolygon WKT (Well Known Text) en objet de type Polygon.
...
<div id="input-parameters-1">
  ### Paramètres d’entrée
</div>

Chaîne commençant par `POLYGON`
...
La fonction `readWKTPoint` de ClickHouse analyse la représentation Well-Known Text (WKT) d’une géométrie de type Point et renvoie un point dans le format interne de ClickHouse.
...
* `wkt_string` : la chaîne WKT en entrée représentant une géométrie Point.

<div id="returned-value-2">
  ### Valeur renvoyée
</div>

La fonction renvoie une représentation interne propre à ClickHouse de la géométrie de type Point.
...
<div id="readwktlinestring">
  ## readWKTLineString
</div>

Analyse une représentation Well-Known Text (WKT) d&#39;une géométrie LineString et la renvoie dans le format interne de ClickHouse.
...
* `wkt_string` : la chaîne WKT en entrée représentant une géométrie LineString.

<div id="returned-value-2">
  ### Valeur renvoyée
</div>

La fonction renvoie une représentation interne de ClickHouse de la géométrie LineString.
...
<div id="readwktmultilinestring">
  ## readWKTMultiLineString
</div>

Analyse la représentation Well-Known Text (WKT) d’une géométrie MultiLineString et la renvoie dans le format interne de ClickHouse.
...
* `wkt_string` : la chaîne WKT en entrée représentant une géométrie de type MultiLineString.

<div id="returned-value-2">
  ### Valeur renvoyée
</div>

La fonction renvoie une représentation interne de ClickHouse pour la géométrie multilinestring.
...
<div id="readwktmultipoint">
  ## readWKTMultiPoint
</div>

Analyse une représentation au format Well-Known Text (WKT) d’une géométrie MultiPoint et la renvoie au format interne de ClickHouse.
...
* `wkt_string` : la chaîne WKT d’entrée représentant une géométrie MultiPoint.

<div id="returned-value-2">
  ### Valeur renvoyée
</div>

La fonction renvoie une représentation interne de ClickHouse de la géométrie multipoint.
...
Analyse une représentation d&#39;une géométrie Polygon au format Well-Known Text (WKT) et renvoie un anneau (linestring fermé) dans le format interne de ClickHouse.
...
* `wkt_string` : Chaîne WKT d’entrée représentant une géométrie de type Polygon.

<div id="returned-value-2">
  ### Valeur renvoyée
</div>

La fonction renvoie une représentation interne ClickHouse de la géométrie de type anneau (linestring fermé).
...
<div id="polygonswithinspherical">
  ## polygonsWithinSpherical
</div>

Renvoie true ou false selon qu’un polygone est entièrement contenu ou non dans un autre polygone. Référence https://www.boost.org/doc/libs/1&#95;62&#95;0/libs/geometry/doc/html/geometry/reference/algorithms/within/within&#95;2.html
...
<div id="readwkbmultipolygon">
  ## readWKBMultiPolygon
</div>

Convertit un MultiPolygon WKB (Well Known Binary) en type MultiPolygon.
...
chaîne de caractères commençant par `MULTIPOLYGON`
...
<div id="readwkbpolygon">
  ## readWKBPolygon
</div>

Convertit un MultiPolygon WKB (Well Known Binary) en type Polygon.
...
La fonction `readWKBPoint` de ClickHouse interprète une représentation Well-Known Binary (WKB) d&#39;une géométrie de type Point et renvoie un point dans le format interne de ClickHouse.
...
* `wkb_string` : la chaîne WKB en entrée représentant une géométrie Point.

<div id="returned-value-2">
  ### Valeur renvoyée
</div>

La fonction renvoie la représentation interne de ClickHouse pour la géométrie Point.
...
<div id="readwkblinestring">
  ## readWKBLineString
</div>

Analyse une représentation binaire bien connue (WKB) d&#39;une géométrie LineString et la renvoie au format interne de ClickHouse.
...
* `wkb_string` : la chaîne WKB en entrée représentant une géométrie LineString.

<div id="returned-value-2">
  ### Valeur renvoyée
</div>

La fonction renvoie une représentation interne de ClickHouse de la géométrie de type linestring.
...
<div id="readwkbmultilinestring">
  ## readWKBMultiLineString
</div>

Analyse une représentation Well-Known Binary (WKB) d’une géométrie MultiLineString et la renvoie dans le format interne de ClickHouse.
...
* `wkb_string` : La chaîne WKB en entrée représentant une géométrie MultiLineString.

<div id="returned-value-2">
  ### Valeur renvoyée
</div>

La fonction renvoie une représentation interne à ClickHouse de la géométrie multilinestring.
...
UInt8, 0 pour false, 1 pour true

<div id="readwkbmultipoint">
  ## readWKBMultiPoint
</div>

Analyse une représentation au format Well-Known Binary (WKB) d&#39;une géométrie MultiPoint et la renvoie au format interne de ClickHouse.
...
* `wkb_string` : La chaîne WKB en entrée représentant une géométrie MultiPoint.
...
<div id="polygonsdistancespherical">
  ## polygonsDistanceSpherical
</div>

Calcule la distance minimale entre deux points, l’un appartenant au premier polygone et l’autre à un second polygone. Spherical signifie que les coordonnées sont interprétées comme celles d’une sphère parfaite et idéale, ce qui n’est pas le cas de la Terre. L’utilisation de ce type de système de coordonnées accélère l’exécution, mais n’est bien sûr pas précise.
...
<div id="polygonsdistancecartesian">
  ## polygonsDistanceCartesian
</div>

Calcule la distance entre deux polygones
...
<div id="polygonsequalscartesian">
  ## polygonsEqualsCartesian
</div>

Renvoie true lorsque deux polygones sont égaux
...
<div id="polygonssymdifferencespherical">
  ## polygonsSymDifferenceSpherical
</div>

Calcule la différence symétrique spatiale en théorie des ensembles (XOR) entre deux polygones
...
<div id="polygonssymdifferencecartesian">
  ## polygonsSymDifferenceCartesian
</div>

Identique à `polygonsSymDifferenceSpherical`, mais les coordonnées sont exprimées dans le système de coordonnées cartésien, qui se rapproche davantage du modèle réel de la Terre.
...
<div id="polygonsintersectionspherical">
  ## polygonsIntersectionSpherical
</div>

Calcule l’intersection (AND) entre des polygones ; les coordonnées sont sphériques.
...
<div id="polygonswithincartesian">
  ## polygonsWithinCartesian
</div>

Renvoie true si le second polygone est contenu dans le premier.
...
<div id="polygonsintersectcartesian">
  ## polygonsIntersectCartesian
</div>

Renvoie true si les deux polygones se recoupent (partagent une zone ou une limite commune).
...
<div id="polygonsintersectspherical">
  ## polygonsIntersectSpherical
</div>

Renvoie true si les deux polygones se recoupent (partagent une zone ou une frontière communes). Référence : https://www.boost.org/doc/libs/1&#95;62&#95;0/libs/geometry/doc/html/geometry/reference/algorithms/intersects.html
...
<div id="geometryintersectcartesian">
  ## geometryIntersectCartesian
</div>

Renvoie true si deux géométries se recoupent (ont au moins un point, une ligne ou une zone en commun).

Contrairement à [`polygonsIntersectCartesian`](#polygonsintersectcartesian), cette fonction accepte n&#39;importe quel type de données géométriques
(`Point`, `MultiPoint`, `LineString`, `MultiLineString`, `Ring`, `Polygon`, `MultiPolygon`), y compris le type commun
[`Geometry`](/fr/reference/data-types/geo#geometry), et les deux arguments peuvent être de types différents.

Les coordonnées sont exprimées dans le système de coordonnées cartésien.
...
Deux géométries de tout type de données géométriques (ou `Geometry`).
...
<div id="geometryintersectspherical">
  ## geometryIntersectSpherical
</div>

Renvoie true si deux géométries se recoupent (partagent un point, une ligne ou une surface en commun).

Contrairement à [`polygonsIntersectSpherical`](#polygonsintersectspherical), elle accepte n&#39;importe quel type de données géométriques
(`Point`, `MultiPoint`, `LineString`, `MultiLineString`, `Ring`, `Polygon`, `MultiPolygon`), y compris le type
[`Geometry`](/fr/reference/data-types/geo#geometry), et les deux arguments peuvent être de types différents.

Les coordonnées sont interprétées comme appartenant à une sphère idéale.
...
<div id="polygonconvexhullcartesian">
  ## polygonConvexHullCartesian
</div>

Calcule l’enveloppe convexe. [Référence](https://www.boost.org/doc/libs/1_61_0/libs/geometry/doc/html/geometry/reference/algorithms/convex_hull.html)

Les coordonnées sont exprimées dans le système de coordonnées cartésiennes.
...
<div id="polygonperimeterspherical">
...
<div id="polygon-representing-zimbabwe">
  #### Polygone représentant le Zimbabwe
</div>

Voici le polygone représentant le Zimbabwe :
...
<div id="usage-of-polygon-perimeter-spherical">
  #### Utilisation de la fonction polygonPerimeterSpherical
</div>
...
<div id="polygonsintersectioncartesian">
  ## polygonsIntersectionCartesian
</div>

Calcule l’intersection de polygones.
...
<div id="polygonperimetercartesian">
...
<div id="polygonsunioncartesian">
  ## polygonsUnionCartesian
</div>

Calcule l’union des polygones.
...
Pour en savoir plus sur les systèmes géométriques, consultez cette [présentation](https://archive.fosdem.org/2020/schedule/event/working_with_spatial_trajectories_in_boost_geometry/attachments/slides/3988/export/events/attachments/working_with_spatial_trajectories_in_boost_geometry/slides/3988/FOSDEM20_vissarion.pdf) de la bibliothèque Boost, utilisée par ClickHouse.