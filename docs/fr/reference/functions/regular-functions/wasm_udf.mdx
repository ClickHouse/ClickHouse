...
## Gérer les modules WASM via la table système {#manage-wasm-modules-via-system-table}
...
### Insérer un module {#insert-a-module}
...
### Distribuer un module à l’échelle d’un cluster {#distribute-a-module-across-a-cluster}
...
### Supprimer un module {#delete-a-module}
...
## Créer une UDF WebAssembly {#create-a-webassembly-udf}
...
La fonction est invoquée une fois par bloc d’entrée : une requête volumineuse est divisée en plusieurs blocs par le pipeline de requête. Le nombre de lignes transmis à chaque appel correspond donc à la taille de ce bloc, et non à celle de la requête entière.

Un bloc du pipeline peut néanmoins être trop large pour la mémoire linéaire du module ; il est alors découpé davantage. Par défaut, `webassembly_udf_input_split_memory_ratio` le découpe dynamiquement : ClickHouse mesure les lignes sérialisées et démarre un nouvel appel avant que l’entrée ne dépasse cette fraction de la mémoire linéaire de l’invité, de sorte que le nombre de lignes varie selon les données. Une valeur non nulle de `webassembly_udf_max_input_block_size` remplace ce mécanisme par une limite fixe de ce nombre de lignes par appel, tandis qu’un ratio de `0` transmet le bloc entier en un seul appel. Un appel ne descend jamais en dessous d’une ligne : une ligne trop volumineuse à elle seule pour la mémoire du module échoue donc toujours à l’intérieur de l’invité.
...
Un exemple complet en C autonome : `str_reverse` inverse les octets de chaque chaîne d&#39;entrée, en utilisant `serialization_format = 'RowBinary'`. Les instances de module sont réutilisées d&#39;un bloc à l&#39;autre, `clickhouse_destroy_buffer` doit donc réellement libérer la mémoire — ici, l&#39;allocator se réinitialise dès que tous les buffers ont été détruits.
...
### Remarque sur le développement de UDFs en Rust {#note-for-developing-udfs-in-rust}
...
## API hôte disponible pour les modules {#host-api-available-to-modules}
...
Les paramètres de requête suivants contrôlent l&#39;exécution des UDF WebAssembly :
...
* `webassembly_udf_max_input_block_size` — Nombre maximal de lignes transmises à une UDF WebAssembly dans un seul bloc, pour toutes les ABI. Définissez 0 pour laisser `webassembly_udf_input_split_memory_ratio` dimensionner les appels d&#39;une fonction `ABI BUFFERED_V1` ; les autres ABI reçoivent alors le bloc de pipeline entier.

* `webassembly_udf_input_split_memory_ratio` — La fraction de la mémoire linéaire qu&#39;un module déclare avoir au démarrage (de son plafond de mémoire, pour un module qui démarre sans aucune page) que l&#39;entrée d&#39;un appel peut occuper. Utilisé pour découper les blocs d&#39;une fonction `ABI BUFFERED_V1` lorsque `webassembly_udf_max_input_block_size` vaut 0 ; les autres ABI ne transportent aucun bloc d&#39;entrée sérialisé et l&#39;ignorent. Valeur par défaut : `0.5`, ce qui laisse le reste de la mémoire au working set propre du guest — un ratio proche de 1 peut malgré tout échouer au sein de l&#39;allocator du guest. Définissez 0, ou fixez `compatibility` à une version antérieure à 26.9, pour transmettre le bloc de pipeline entier en un seul appel. Doit être au minimum 0 et au maximum 1.
