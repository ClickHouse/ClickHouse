...
### Utiliser directement les résultats conditionnels {#using-conditional-results-directly}
...
### Valeurs NULL dans les expressions conditionnelles {#null-values-in-conditionals}
...
### Instruction CASE {#case-statement}
...
<Note title="Utilisez le paramètre `least_greatest_legacy_null_behavior` pour modifier le comportement de `NULL`">
...
Le paramètre [`short_circuit_function_evaluation`](/fr/reference/settings/session-settings/short-circuit-function-evaluation#short_circuit_function_evaluation) détermine si l&#39;évaluation en court-circuit est utilisée.
...
Renvoie la plus petite valeur parmi les arguments, promue au plus grand type compatible. [`Any`](/fr/reference/data-types/index)
...
Le paramètre [`short_circuit_function_evaluation`](/fr/reference/settings/session-settings/short-circuit-function-evaluation#short_circuit_function_evaluation) contrôle
