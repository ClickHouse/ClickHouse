...
Classe le texte en entrée à l’aide d’un modèle Naive Bayes avec n-grammes et lissage de Laplace.
Le modèle doit être configuré dans ClickHouse avant de pouvoir être utilisé.

**Détails d’implémentation**

*Algorithme*

Utilise l’algorithme de classification Naive Bayes avec le [lissage de Laplace](https://en.wikipedia.org/wiki/Additive_smoothing) pour traiter les n-grammes non observés, à partir des probabilités des n-grammes décrites [ici](https://web.stanford.edu/~jurafsky/slp3/4.pdf).

*Fonctionnalités clés*

* Prend en charge des n-grammes de n’importe quelle taille.
* Trois modes de tokenisation :
  * `byte` : fonctionne sur des octets bruts. Chaque octet constitue un token.
  * `codepoint` : fonctionne sur des valeurs scalaires Unicode décodées à partir de l’UTF-8. Chaque point de code constitue un token.
  * `token` : découpe en fonction des séquences d’espaces Unicode (regex `\s+`). Les tokens sont des sous-chaînes ne contenant pas d’espaces ; la ponctuation fait partie du token si elle y est adjacente (par exemple, `you?` constitue un seul token).

**Configuration du modèle**

Des exemples de code source pour créer un modèle Naive Bayes de détection de langue sont disponibles [ici](https://github.com/nihalzp/ClickHouse-NaiveBayesClassifier-Models), ainsi que des modèles d’exemple et leurs fichiers de configuration associés [ici](https://github.com/nihalzp/ClickHouse-NaiveBayesClassifier-Models/tree/main/models).

Exemple de configuration d’un modèle Naive Bayes dans ClickHouse :
...
| `name`    | Identifiant unique du modèle.                                                                                        | `language_detection`                                     | Obligatoire       |
| `path`    | Chemin complet vers le fichier binaire du modèle.                                                                    | `/etc/clickhouse-server/config.d/language_detection.bin` | Obligatoire       |
| `mode`    | Méthode de tokenisation : `byte` (séquences d’octets), `codepoint` (caractères Unicode) ou `token` (tokens de mots). | `token`                                                  | Obligatoire       |
| `n`       | Taille des n-grammes : `1` (mot unique), `2` (paires de mots) ou `3` (triplets de mots).                             | `2`                                                      | Obligatoire       |
| `alpha`   | Facteur de lissage de Laplace utilisé lors de la classification pour les n-grammes absents du modèle.                | `0.5`                                                    | `1.0`             |
| `priors`  | Probabilités des classes (pourcentage de documents appartenant à une classe).                                        | 60 % classe 0, 40 % classe 1                             | Répartition égale |

**Guide d’entraînement du modèle**

*Format de fichier*

En format lisible par l’homme, pour `n=1` et le mode `token`, le modèle peut ressembler à ceci :
...
Pour `n=3` et le mode `codepoint`, on pourrait obtenir quelque chose comme :
...
Le format lisible par l’être humain n’est pas utilisé directement par ClickHouse ; il doit être converti au format binaire décrit ci-dessous.
...
Avant de créer le modèle à partir du corpus de documents, les documents doivent être prétraités afin d’extraire les n-grams selon les valeurs spécifiées de `mode` et `n` :

1. Ajoutez des marqueurs de début et de fin au début et à la fin de chaque document selon le mode de tokenisation :
...
2. Exemple pour `n=3` en mode `token` :
   * Document : `ClickHouse is fast`
   * Traité comme : `<s> <s> ClickHouse is fast </s> </s>`
   * Trigrammes générés :
     * `<s> <s> ClickHouse`
     * `<s> ClickHouse is`
     * `ClickHouse is fast`
...
Pour simplifier la création du modèle pour les modes `byte` et `codepoint`, il peut être utile de commencer par tokeniser le document (une liste d’octets pour le mode `byte`, une liste de points de code pour le mode `codepoint`), d’ajouter `n - 1` tokens de début au début et `n - 1` tokens de fin à la fin, puis de générer les n-grammes et de les écrire dans le fichier sérialisé.
...
* `model_name` — Nom du modèle préconfiguré. Le modèle doit être défini dans les fichiers de configuration de ClickHouse. [`String`](/fr/reference/data-types/string)
* `input_text` — Texte à classifier. L’entrée est traitée exactement telle qu’elle est fournie (casse/ponctuation conservées). [`String`](/fr/reference/data-types/string)

**Valeur renvoyée**

ID de classe prédit, sous la forme d’un entier non signé. Les ID de classe correspondent aux catégories définies lors de la construction du modèle. [`UInt32`](/fr/reference/data-types/int-uint)

**Exemples**

**Classifier la langue d’un texte**
