...
## Problèmes courants de conversion des données {#common-issues-with-data-conversion}
...
Les fonctions `to<type>` et [cast](#CAST) se comportent différemment dans certains cas, par exemple avec [LowCardinality](/fr/reference/data-types/lowcardinality) : [cast](#CAST) supprime l&#39;attribut [LowCardinality](/fr/reference/data-types/lowcardinality), contrairement aux fonctions `to<type>`. Il en va de même pour [Nullable](/fr/reference/data-types/nullable). Ce comportement n&#39;est pas conforme à la norme SQL, et peut être modifié à l&#39;aide du paramètre [cast&#95;keep&#95;nullable](/fr/reference/settings/session-settings/cast#cast_keep_nullable).
...
## Remarques sur les fonctions `toString` {#to-string-functions}
...
## Remarques sur les fonctions `toDate`/`toDateTime` {#to-date-and-date-time-functions}
...
Vérifie une valeur par rapport à un type Kusto Query Language déclaré et la convertit vers le
type ClickHouse porteur correspondant : un argument dont le type n&#39;appartient pas au type KQL déclaré est
rejeté lors de l&#39;analyse, et une valeur qui n&#39;entre pas dans les limites du type (un dépassement d&#39;`int`, par exemple) provoque une
erreur ligne par ligne. Le deuxième argument est le nom du type KQL déclaré et le troisième décrit le
paramètre à mentionner dans le message d&#39;erreur ; les deux doivent être des chaînes constantes.

Cette fonction sert de base aux paramètres de fonction typés lorsque `dialect = 'kusto'`. Elle n&#39;est pas destinée à
être appelée directement depuis SQL.
...
* `value` — L&#39;argument fourni par un appel de fonction KQL. - `type` — Le nom du type KQL déclaré, une chaîne constante. - `parameter` — Une description du paramètre, une chaîne constante utilisée dans le message d&#39;erreur.

**Valeur renvoyée**

`value` dans le type ClickHouse correspondant au type déclaré.
...
Convertit une valeur en durée (un `Interval` en nanosecondes), à la manière de la fonction
`totimespan()` du Kusto Query Language : une durée est renvoyée telle quelle, un nombre est interprété comme un nombre de jours, et une chaîne est lue au format
`[-][d.]hh:mm:ss[.fffffff]` — le résultat étant `NULL` lorsque l&#39;analyse échoue.

Cette fonction implémente `totimespan()` lorsque `dialect = 'kusto'`. Elle n&#39;est pas destinée à être appelée
...
* `x` — Une durée, un nombre de jours ou une chaîne exprimant une durée.

**valeur renvoyée**

La valeur sous forme d&#39;intervalle en nanosecondes ; `NULL` si la chaîne n&#39;exprime pas une durée.
...
Identique à [`parseDateTime32BestEffort`](#parseDateTime32BestEffort), sauf qu&#39;elle renvoie une date zéro ou une date et heure nulles lorsqu&#39;elle rencontre un format de date qui ne peut pas être traité.
...
Identique à [`parseDateTime64BestEffort`](#parseDateTime64BestEffort), sauf que cette fonction privilégie le format de date américain (`MM/DD/YYYY`, etc.) en cas d’ambiguïté et renvoie `NULL` lorsqu’elle rencontre un format de date qui ne peut pas être analysé.
...
Identique à [`parseDateTime64BestEffort`](#parseDateTime64BestEffort), à ceci près que cette fonction privilégie le format de date américain (`MM/DD/YYYY`, etc.) en cas d’ambiguïté et renvoie une date zéro ou une date-heure zéro lorsqu’elle rencontre un format de date qui ne peut pas être analysé.
...
Identique à [`parseDateTimeBestEffort`](#parseDateTimeBestEffort), sauf qu&#39;elle renvoie `NULL` lorsqu&#39;elle rencontre un format de date qui ne peut pas être analysé.
...
Si les composants mois et jour ne peuvent pas être extraits sans ambiguïté, par ex. `MM/DD/YYYY`, `MM-DD-YYYY` ou `MM-DD-YY`, elle privilégie le format de date américain plutôt que `DD/MM/YYYY`, `DD-MM-YYYY` ou `DD-MM-YY`.
...
Identique à [`parseDateTime`](#parseDateTime), mais renvoie `NULL` lorsqu’il rencontre un format de date impossible à analyser.
...
Convertit une valeur d’entrée de type String en une valeur de type BFloat16.
Si la chaîne ne représente pas une valeur à virgule flottante, la fonction renvoie zéro.
...
<Note>
  La fonction autorise une perte de précision silencieuse lors de la conversion depuis une représentation sous forme de chaîne.
</Note>
...
Si la valeur est hors de la plage `[0000-01-01, 9999-12-31]`, `toDate32` renvoie les valeurs limites prises en charge par [Date32](/fr/reference/data-types/date32).
...
Convertit l’argument en type de données [Date32](/fr/reference/data-types/date32). Si l’argument ne peut pas être converti, notamment s’il s’agit d’une valeur numérique hors de la plage de [Date32](/fr/reference/data-types/date32), la fonction renvoie une valeur par défaut, qui correspond soit au deuxième argument (s’il est spécifié), soit à `1900-01-01`, la valeur par défaut historique de [Date32](/fr/reference/data-types/date32) (et non sa borne inférieure, qui est `0000-01-01`). Si l’argument est de type [Date](/fr/reference/data-types/date), ses bornes sont prises en compte.

Notez que, contrairement à [toDate32](#toDate32), cette fonction ne sature pas un argument numérique hors plage aux bornes de [Date32](/fr/reference/data-types/date32) ; un tel argument est traité comme non convertible.
...
Convertit une valeur d’entrée en une valeur de type [Date32](/fr/reference/data-types/date32), mais renvoie la valeur par défaut `1900-01-01` si l’argument est invalide.
Identique à [toDate32](#toDate32), mais renvoie `1900-01-01` si l’argument est invalide.
Notez que `1900-01-01` est une valeur par défaut historique pour les entrées non valides, et non la borne inférieure de [Date32](/fr/reference/data-types/date32), qui est `0000-01-01`.
...
Renvoie une valeur `Date32` en cas de réussite ; sinon, la valeur par défaut historique pour une entrée non valide (`1900-01-01`). [`Date32`](/fr/reference/data-types/date32)
...
Si `expr` est une valeur [Time](/fr/reference/data-types/time) ou [Time64](/fr/reference/data-types/time64), elle est interprétée comme le nombre de secondes écoulées depuis le début de l&#39;époque Unix : la partie fractionnaire d&#39;un `Time64` est arrondie vers moins l&#39;infini, et les valeurs situées en dehors de la plage `DateTime` suivent le paramètre `date_time_overflow_behavior`.
...
* `expr` — La valeur. [`String`](/fr/reference/data-types/string) ou [`Int`](/fr/reference/data-types/int-uint) ou [`Date`](/fr/reference/data-types/date) ou [`Date32`](/fr/reference/data-types/date32) ou [`Time`](/fr/reference/data-types/time) ou [`Time64`](/fr/reference/data-types/time64) ou [`DateTime`](/fr/reference/data-types/datetime) ou [`DateTime64`](/fr/reference/data-types/datetime64)
...
Prend en charge la conversion depuis `String`, `FixedString`, `Date`, `Date32`, `Time`, `Time64`, `DateTime`, `DateTime64` ou des types numériques (`(U)Int8`, `(U)Int16`, `(U)Int32`, `(U)Int64`, `(U)Int128`, `(U)Int256`, `BFloat16`, `Float32`, `Float64`). Les valeurs `Decimal` ne sont pas prises en charge et lèvent une exception.
`DateTime32` est un alias de `DateTime` et prend en charge les dates comprises entre `1970-01-01 00:00:00` et `2106-02-07 06:28:15`.
Lors d’une conversion depuis une chaîne ou l’un des types numériques saturants répertoriés ci-dessus, les valeurs hors de cette plage sont ramenées à ses bornes. Les valeurs à virgule flottante non finies (`NaN`, `inf`, `-inf`) ne peuvent pas être converties et lèvent une exception.
Les conversions depuis d’autres types de date et d’heure, tels que `Date32`, `DateTime64`, `Time` ou `Time64`, suivent le paramètre `date_time_overflow_behavior` : le mode `ignore` par défaut peut produire des valeurs ayant subi un bouclage, `saturate` les ramène aux bornes de la plage et `throw` lève une exception. Les valeurs `Time64` sont d’abord arrondies vers moins l’infini à la seconde entière.
...
* `x` — Valeur d’entrée à convertir. [`String`](/fr/reference/data-types/string), [`FixedString`](/fr/reference/data-types/fixedstring), [`(U)Int*`](/fr/reference/data-types/int-uint), [`Float*`](/fr/reference/data-types/float), [`BFloat16`](/fr/reference/data-types/float), [`Date`](/fr/reference/data-types/date), [`Date32`](/fr/reference/data-types/date32), [`Time`](/fr/reference/data-types/time), [`Time64`](/fr/reference/data-types/time64), [`DateTime`](/fr/reference/data-types/datetime) ou [`DateTime64`](/fr/reference/data-types/datetime64)
...
Renvoie une date calendaire et une heure de la journée, avec une précision sous-seconde. [`DateTime64`](/fr/reference/data-types/datetime64)
...
Convertit une valeur d’entrée en une valeur de type [DateTime64](/fr/reference/data-types/datetime64), mais renvoie la valeur zéro de [DateTime64](/fr/reference/data-types/datetime64) (l’époque Unix, `1970-01-01 00:00:00`) si l’argument est invalide.
Identique à [toDateTime64](#toDateTime64), mais renvoie la valeur zéro de [DateTime64](/fr/reference/data-types/datetime64) si l’argument est invalide.
...
Renvoie une valeur de type DateTime64 en cas de succès, sinon la valeur DateTime64 zéro (`1970-01-01 00:00:00`) à la précision demandée. [`DateTime64`](/fr/reference/data-types/datetime64)
...
Prend en charge la conversion à partir de String, FixedString, DateTime, DateTime64, Time64 ou de types numériques représentant le nombre de secondes écoulées depuis minuit.
Les valeurs numériques en dehors de la plage du type (de `-999:59:59` à `999:59:59`, soit de `-3599999` à `3599999` secondes) sont saturées aux limites de la plage, et les valeurs à virgule flottante non finies (`NaN`, `inf`, `-inf`) ne peuvent pas être converties et entraînent une exception.
Les valeurs `Time64` sont arrondies vers l’infini négatif à la seconde entière et saturées à la même plage.

<Note title="`toTime` legacy">
  Avant la v25.5, `toTime` était une fonction différente, qui convertissait une date avec heure en une date fixe (`1970-01-02`) tout en préservant la
  composante temporelle. Cette fonction est toujours disponible sous le nom de [`toTimeWithFixedDate`](/fr/reference/functions/regular-functions/date-time-functions#toTimeWithFixedDate).

  Définir [`use_legacy_to_time`](/fr/reference/settings/session-settings#use_legacy_to_time) sur `1` maintient également le nom `toTime` associé à la fonction
  legacy, c’est-à-dire que les appels à `toTime` sont résolus vers `toTimeWithFixedDate` au lieu de la fonction de conversion décrite ici.
  Lorsque le paramètre est activé, utilisez `CAST(x AS Time)` ou `x::Time` pour convertir vers le type `Time`.
  Le paramètre est défini sur `0` par défaut depuis la v26.7, mais était défini sur `1` par défaut de la v25.6 à la v26.6, c’est pourquoi les exemples ci-dessous le définissent explicitement.
</Note>
...
* `x` — Valeur d’entrée à convertir. [`String`](/fr/reference/data-types/string) ou [`FixedString`](/fr/reference/data-types/fixedstring) ou [`DateTime`](/fr/reference/data-types/datetime) ou [`DateTime64`](/fr/reference/data-types/datetime64) ou [`Time64`](/fr/reference/data-types/time64) ou [`(U)Int*`](/fr/reference/data-types/int-uint) ou [`BFloat16`](/fr/reference/data-types/float) ou [`Float*`](/fr/reference/data-types/float)
...
* `precision` — Optionnel. La précision à la sous-seconde de la valeur renvoyée. [`UInt8`](/fr/reference/data-types/int-uint)
...
Convertit une valeur d&#39;entrée en une valeur de type `Time64`, mais renvoie la valeur `Time64` nulle (`00:00:00`) si un argument invalide est reçu.
Identique à [`toTime64`](#toTime64), mais renvoie la valeur `Time64` nulle si un argument invalide est reçu.
...
Renvoie une valeur `Time64` en cas de réussite, sinon la valeur `Time64` nulle (`00:00:00`) à la précision demandée. [`Time64`](/fr/reference/data-types/time64)
...
Comme toTime, mais renvoie `00:00:00` au lieu de lever une exception en cas d’erreur de conversion.
