...
### Paramètres de la collection nommée {#named-collection-parameters}
...
### Sélection des identifiants {#selecting-credentials}
...
### Map de paramètres {#parameter-map}
...
### Paramètres au niveau de la requête {#query-level-settings}
...
### Restreindre les hôtes de point de terminaison {#restricting-endpoint-hosts}
...
### Sécurité du transport (HTTP vs HTTPS) {#transport-security}
...
<Note>
  Cette fonction est non déterministe : elle peut renvoyer des résultats différents avec les mêmes arguments.
</Note>
...
Le troisième argument peut être soit une instruction libre en langage naturel (par ex. `'le principal motif de plainte'`), soit un
schéma encodé en JSON de la forme `'{"field_a": "description of field a", "field_b": "description of field b"}'`.

En mode instruction, la fonction renvoie la valeur extraite sous forme de chaîne de caractères, ou une chaîne vide si aucune valeur n’est trouvée.
En mode schéma, la fonction renvoie une chaîne contenant un objet JSON dont les clés correspondent au schéma demandé ; les champs absents sont `null`.

Les informations d’identification (une collection nommée spécifiant le fournisseur, le modèle, le endpoint et, éventuellement, une clé API)
...
* `text` — Texte duquel extraire des informations. [`String`](/fr/reference/data-types/string)
...
* `params` — `Map(String, String)` constant facultatif de paramètres. Clés propres à la fonction : `temperature` (température d’échantillonnage contrôlant l’aléatoire ; valeur par défaut : `0.0`), `max_tokens` (nombre maximal de tokens de sortie par appel ; valeur par défaut : `1024`). Les paramètres communs `credentials` et `model` s’appliquent également (voir [Fonctions d’IA](/fr/reference/functions/regular-functions/ai-functions)). [`Map(String, String)`](/fr/reference/data-types/map)

**Valeur renvoyée**

Une valeur extraite unique (mode instruction) ou une chaîne représentant un objet JSON (mode schéma). Renvoie la valeur par défaut du type de colonne (chaîne vide) si la requête a échoué et que `ai_function_throw_on_error` est désactivé. [`String`](/fr/reference/data-types/string)
