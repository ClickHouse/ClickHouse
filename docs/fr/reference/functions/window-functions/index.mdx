...
| Cadre `GROUPS`                                                                            | ✅                | Les limites du cadre comptent des groupes de pairs entiers (lignes égales selon la clé `ORDER BY`) ; `N PRECEDING` et `N FOLLOWING` comptent `N` groupes de pairs avant et après le groupe de pairs de la ligne en cours.                                                                                                                                                                                                                              |
...
* `ROWS`, `RANGE` ou `GROUPS` - définit les bornes d’un cadre, aggregate&#95;function étant calculée dans ce cadre. `ROWS` compte les lignes physiques, `RANGE` compte les valeurs `ORDER BY` et `GROUPS` compte les groupes de pairs (lignes égales selon la clé `ORDER BY`).
...
Une frame `GROUPS` compte des *groupes de pairs* entiers — des ensembles de lignes égales selon la clé `ORDER BY` — plutôt que des lignes physiques (`ROWS`) ou des valeurs `ORDER BY` (`RANGE`). `N PRECEDING` et `N FOLLOWING` comptent `N` groupes de pairs avant et après le groupe de pairs de la ligne en cours, et chaque groupe de pairs inclus apporte toujours l&#39;ensemble de ses lignes.

La requête ci-dessous applique les mêmes limites `1 PRECEDING AND 1 FOLLOWING` à des frames `ROWS`, `RANGE` et `GROUPS`. La colonne `order` contient des valeurs dupliquées et non consécutives ; les trois modes couvrent donc des lignes différentes :
...
Chaque mode interprète les limites différemment :

* `ROWS` compte les lignes physiques ; la frame comporte donc au plus trois lignes adjacentes : la ligne en cours et une ligne de chaque côté.
* `RANGE` compte les valeurs d’`order` ; `1 PRECEDING` et `1 FOLLOWING` couvrent donc les lignes dont l’`order` diffère d’au plus 1 de celui de la ligne en cours. Avec des écarts de 10, aucune ligne voisine n’est incluse ; la frame ne contient donc que les lignes ayant le même `order` que la ligne en cours.
* `GROUPS` compte les groupes de pairs ; `1 PRECEDING` et `1 FOLLOWING` incluent donc toujours intégralement les groupes adjacents, quels que soient les écarts entre les valeurs d’`order`.
