...
Pour utiliser les connexions ODBC en toute sécurité, ClickHouse s&#39;appuie sur un programme distinct, `clickhouse-odbc-bridge`. Si le pilote ODBC est chargé directement depuis `clickhouse-server`, un problème lié au pilote peut faire planter le serveur ClickHouse. ClickHouse démarre automatiquement `clickhouse-odbc-bridge` lorsque nécessaire. Le pont ODBC est installé à partir du même paquet que `clickhouse-server`.
...
Cet exemple a été validé pour Ubuntu Linux 18.04 et MySQL server 5.7.
