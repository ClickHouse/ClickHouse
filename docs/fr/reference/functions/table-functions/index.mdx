...
<Note>
  La plupart des fonctions de table nécessitent le privilège `CREATE TEMPORARY TABLE` pour s’exécuter ; un `SELECT` ou un `INSERT`
  qui en exécute une est donc refusé lorsque
  [`readonly`](/fr/concepts/features/configuration/settings/permissions-for-queries#readonly) est défini à 1.
</Note>