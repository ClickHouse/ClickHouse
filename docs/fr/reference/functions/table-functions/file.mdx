...
Un moteur de table qui fournit une interface de type table pour exécuter des `SELECT` à partir de fichiers et des `INSERT` dans des fichiers, similaire à la fonction de table [s3](/fr/reference/functions/table-functions/s3). Utilisez `file` lorsque vous travaillez avec des fichiers locaux, et `s3` lorsque vous travaillez avec des buckets dans un stockage objet tel que S3, GCS ou MinIO.
