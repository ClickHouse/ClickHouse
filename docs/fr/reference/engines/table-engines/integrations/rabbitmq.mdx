...
* Pour les formats orientés blocs, il n’est pas possible de diviser un block en parties plus petites, mais le nombre de lignes dans un block peut être contrôlé par le paramètre général [max&#95;block&#95;size](/fr/reference/settings/session-settings#max_block_size).