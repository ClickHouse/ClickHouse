...
<Tabs>
  ...
  <Tab title="GCP">
    ...
    * `url` — URL du bucket GCS pointant vers la table Delta Lake. Doit utiliser le format `https://storage.googleapis.com/<bucket>/<path>/`
      (le endpoint de l&#39;API XML GCS), ou `gs://<bucket>/<path>/`, qui est converti automatiquement.
...
<Note>
  ...
  Les écritures Delta Lake sont une fonctionnalité Beta et doivent être activées avec `SET allow_delta_lake_writes = 1` (disponible à partir de la version 26.7 ; sur les versions antérieures, utilisez `SET allow_experimental_delta_lake_writes = 1`).
</Note>
