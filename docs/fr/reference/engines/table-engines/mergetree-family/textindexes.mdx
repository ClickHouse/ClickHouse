...
`LIKE` prend en outre en charge une *lecture directe (sans indication)* (activée via `use_text_index_like_evaluation_by_dictionary_scan`) pour les tokenizers `splitByNonAlpha` et `array`, sans préprocesseur ni postprocesseur.

² `ILIKE` est uniquement pris en charge via la lecture directe (sans indication) (`use_text_index_like_evaluation_by_dictionary_scan = 1`, tokenizer `splitByNonAlpha` ou `array`).
Il n&#39;y a pas de solution de repli consistant à utiliser l&#39;index comme indication : si le paramètre est désactivé ou si le tokenizer ne fait pas partie de l&#39;ensemble pris en charge, l&#39;index n&#39;est pas utilisé pour `ILIKE`.
Le préprocesseur, s&#39;il est présent, doit être `lower` ou `upper` ; les postprocesseurs ne sont pas pris en charge.

³ `hasPhrase` sur un index de texte `splitByRegexp` ne prend **pas** en charge de postprocesseur : la combinaison est rejetée avec une exception, car la réécriture au niveau des lignes du postprocesseur suppose des tokens de type `splitByNonAlpha` séparés par des espaces. Sans postprocesseur, `splitByRegexp` est entièrement pris en charge par `hasPhrase`.

**Expérimental : argument de prise en charge de la recherche d’expression (facultatif)**.
