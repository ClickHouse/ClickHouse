...
* `splitByRegexp(re)` divise les chaînes selon un séparateur d’expression régulière `re` défini par l’utilisateur (voir la fonction [splitByRegexp](/fr/reference/functions/regular-functions/splitting-merging-functions#splitByRegexp)).
  L’argument `re` est obligatoire, par exemple `tokenizer = splitByRegexp('[^\p{L}\p{N}#+]+')`.
  Contrairement aux chaînes séparatrices fixes de `splitByString`, un séparateur d’expression régulière peut préserver les tokens contenant des caractères spéciaux (comme `C++` ou `C#`) que les autres tokenizers sépareraient.
...
`LIKE` prend en outre en charge l&#39;évaluation par dictionary scan (activée via `use_text_index_like_evaluation_by_dictionary_scan`) pour les tokenizers `splitByNonAlpha` et `array`, sans préprocesseur ni postprocesseur.
Un pattern `%value%` correspond alors à une *lecture directe (sans indication)*, tandis que les patterns `value%` et `%value` restent une indication, voir [Requêtes LIKE/ILIKE](#like-ilike-queries-perf).
...
## Utiliser un index de texte {#using-a-text-index}
...
### Fonctions prises en charge {#functions-support}
...
#### `LIKE` et `match` {#functions-example-like-match}
...
#### `startsWith` et `endsWith` {#functions-example-startswith-endswith}
...
#### `hasAny` et `hasAll` {#functions-example-hasany-hasall}
...
#### `mapContainsKeyLike` et `mapContainsValueLike` {#functions-example-mapcontainslike}
...
### Indexation des colonnes Array(String) {#text-index-example-array}
...
### Indexation des colonnes de type Map {#text-index-example-map}
...
### Indexation des colonnes JSON {#text-index-example-json}
...
#### Index sur des sous-colonnes spécifiques {#json-indexes-on-subcolumns}
...
#### Index basés sur les chemins avec JSONAllPaths {#json-indexes-jsonallpaths}
...
#### Index basés sur les valeurs avec JSONAllValues {#json-indexes-jsonallvalues}
...
##### Création de l’index {#json-all-values-creating-the-index}
...
##### Types de requêtes pris en charge {#json-all-values-supported-query-patterns}
...
### Recherche d&#39;expressions {#text-index-phrase-search}
...
#### Exemple {#text-index-phrase-search-example}
...
La lecture directe comme indice est contrôlée par le paramètre [query&#95;plan&#95;text&#95;index&#95;add&#95;hint](/fr/reference/settings/session-settings/query-plan#query_plan_text_index_add_hint) (activé par défaut).
...
### Requêtes LIKE/ILIKE {#like-ilike-queries-perf}

Lorsqu&#39;un pattern de requête LIKE/ILIKE est `%<alpha-numeric-characters-without-spaces>%`, `<alpha-numeric-characters-without-spaces>%` ou `%<alpha-numeric-characters-without-spaces>` et que le tokenizer du text index est `splitByNonAlpha` ou `array`, ClickHouse s&#39;appuie sur l&#39;inverted index pour accélérer considérablement les requêtes LIKE/ILIKE. Pour ce faire, ClickHouse parcourt le dictionnaire de l&#39;inverted index au lieu d&#39;effectuer un parcours complet de la table afin de trouver le pattern correspondant.

L&#39;utilisation du résultat du dictionary scan dépend de l&#39;endroit où le needle est ancré :

* `%value%` correspond à une row si et seulement si elle correspond à l&#39;un des tokens de la row ; l&#39;index tranche donc à lui seul la query : une [lecture directe (sans indice)](#direct-read) qui supprime le prédicat d&#39;origine.
* `value%` et `%value` ancrent le needle sur la valeur entière, alors que le dictionary scan ne peut l&#39;ancrer que sur un token ; le scan renvoie donc un superset des rows correspondantes et sert de [lecture directe en tant qu&#39;indice](#direct-read).

Ce même dictionary scan est utilisé pour `startsWith(col, 'value')` et `endsWith(col, 'value')` lorsque le needle ne contient aucun token complet à rechercher.
...
Les needles qui s&#39;étendent sur plusieurs tokens continuent d&#39;utiliser les tokens complets du needle et n&#39;ont pas besoin d&#39;un dictionary scan.
...
Le gain de performance d&#39;un pattern `value%` ou `%value` provient du skipping de granules ; il dépend donc de la répartition des rows correspondantes. Les needles qui correspondent à des rows dans chaque granule n&#39;en éliminent aucun, et la query paie alors le coût du dictionary scan en plus du parcours complet qu&#39;elle aurait effectué de toute façon. Désactivez [use&#95;text&#95;index&#95;like&#95;evaluation&#95;by&#95;dictionary&#95;scan](/fr/reference/settings/session-settings/use-text#use_text_index_like_evaluation_by_dictionary_scan) pour de tels workloads.
...
### Requêtes de comptage simples {#count-queries-perf}
...
#### Paramètres du cache des tokens {#caching-tokens}
...
#### Paramètres du cache des en-têtes {#caching-header}
...
#### Paramètres du cache des listes de postings {#caching-posting-lists}
...
## Index de texte vs index basés sur des filtres de Bloom {#text-index-vs-bloom-filter-indexes}
...
## Exemple : jeu de données Hacker News {#hacker-news-dataset}
...
### 1. Utilisation de `hasToken` {#using-hasToken}
...
### 2. Utilisation de `hasAnyTokens` {#using-hasAnyTokens}
...
### 3. Utilisation de `hasAllTokens` {#using-hasAllTokens}
...
### 4. Recherche composée : OR, AND, NOT, ... {#compound-search}
