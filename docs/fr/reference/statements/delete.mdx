...
Si vous devez garantir que vos données soient supprimées du stockage dans un délai prévisible, envisagez d’utiliser le paramètre de table [`min_age_to_force_merge_seconds`](/fr/reference/settings/merge-tree-settings/min-age#min_age_to_force_merge_seconds). Vous pouvez également utiliser la commande [ALTER TABLE ... DELETE](/fr/reference/statements/alter/delete). Notez que la suppression de données à l’aide de `ALTER TABLE ... DELETE` peut consommer des ressources importantes, car elle recrée toutes les parts affectées.
