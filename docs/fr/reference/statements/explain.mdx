...
<Note>
  La version actuelle de `EXPLAIN ANALYZE` ne prend pas en charge les requêtes exécutées en mode distribué.
</Note>
...
* `Read` — lignes et octets non compressés lus dans les tables, avec leur débit — les mêmes valeurs que celles affichées dans le pied de page d’une requête ordinaire sous &quot;Processed&quot;.
* `Peak memory` — pic de mémoire utilisé par la requête.
...
Le nombre de lignes et d’octets est indiqué une seule fois pour l’ensemble de l’étape (la ligne `I/O`). Le temps et le parallélisme sont indiqués pour chaque phase de l’étape sur la ou les lignes en retrait qui suivent.
...
* `time <t> (<share>%)` — temps d’horloge pendant lequel la phase a été active, ainsi que sa part du temps d’exécution de la requête (c.-à-d. hors temps de build). Notez que les parts peuvent totaliser plus de 100 % car les phases et les étapes s’exécutent en parallèle.
* `parallelism <avg>/<max>` — nombre moyen de threads CPU travaillant simultanément dans cette phase, par rapport au maximum qu’elle pouvait utiliser. Une valeur proche du maximum signifie que la phase a été bien parallélisée ; une valeur proche de 1 signifie qu’elle s’est exécutée principalement en série.
* `Stage (<stage>)` — le nom de la phase. Une étape avec une seule phase affiche directement la ligne de temps, sans libellé `Stage (...)`. Les étapes comportant plusieurs phases affichent une ligne libellée par phase ; par exemple, `Aggregating` affiche `Stage (partial aggregation)` et `Stage (final aggregation)`, et un hash join affiche `Stage (build)` et `Stage (probe)`.
