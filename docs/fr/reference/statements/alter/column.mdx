...
Ajoute une nouvelle colonne à la table avec les `name`, `type`, [`codec`](/fr/reference/statements/create/table/codec) et `default_expr` spécifiés (voir la section [Expressions par défaut](/fr/reference/statements/create/table#default_values)).
...
<Note>
  `MATERIALIZE COLUMN` réécrit toujours les **valeurs de la colonne**. La reconstruction des [index d’élagage de données](/fr/sql-reference/statements/alter/skipping-index) (y compris les index de texte intégral) dépend également de la disposition de la part et du mode de stockage de l’index :

  * Sur les parts qui sont à la fois **wide + full-storage**, les fichiers autonomes d’index d’élagage (et les index de texte intégral) ne sont **pas** automatiquement reconstruits lorsque seule la colonne est réécrite. Après avoir modifié les données de la colonne sous-jacente dont dépend un index, exécutez [`ALTER TABLE ... MATERIALIZE INDEX`](/fr/sql-reference/statements/alter/skipping-index#materialize-index) si vous devez actualiser ces fichiers d’index.
