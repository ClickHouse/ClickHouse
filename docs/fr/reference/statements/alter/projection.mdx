...
Utilisez l’instruction ci-dessous pour modifier la clause [`WITH SETTINGS`](#with-settings) d’une projection existante sans reconstruire ses données :
...
Pour un [index de projection](/fr/reference/engines/table-engines/mergetree-family/mergetree#projection-index), indiquez de nouveau la déclaration `INDEX` plutôt que la requête `SELECT` :
...
L&#39;instruction reprend la définition complète de la projection, mais seule la clause `WITH SETTINGS` peut différer de la définition existante.
La requête de projection elle-même (ou, pour un index de projection, l&#39;expression et le type de l&#39;index) doit rester identique, car les parts existantes stockent des données construites à partir de celle-ci ; pour la modifier, utilisez [`DROP PROJECTION`](#drop-projection), suivi de [`ADD PROJECTION`](#add-projection).

La commande modifie uniquement les métadonnées de la table et ne réécrit aucune donnée : les parts existantes conservent les paramètres avec lesquels elles ont été écrites, tandis que celles créées par de futurs inserts et fusions utilisent les nouveaux paramètres.
Pour reconstruire les parties existantes avec les nouveaux paramètres, exécutez [`MATERIALIZE PROJECTION`](#materialize-projection).
...
Les commandes `ADD`, `MODIFY`, `DROP` et `CLEAR` sont légères, au sens où elles ne font que modifier les métadonnées ou supprimer des fichiers.
...
Dans certains cas, des parts de la table source n’ont pas de parts de projection correspondantes.
...
Comme l’une des projections contient déjà les valeurs d’agrégation précalculées, ClickHouse essaie de lire les parts de projection correspondantes pour éviter de refaire l’agrégation au moment de l’exécution de la requête. Si une part donnée ne possède pas de part de projection correspondante, l’exécution de la requête revient à la part d’origine.
...
Avant ClickHouse v24.8, les parts de projection se désynchronisaient silencieusement des données principales, ou bien certaines opérations comme les mises à jour et les suppressions ne pouvaient pas être exécutées du tout, car la database levait automatiquement une exception si la table avait des projections.

Depuis la v24.8, un nouveau paramètre au niveau de la table [`deduplicate_merge_projection_mode`](/fr/reference/settings/merge-tree-settings/other#deduplicate_merge_projection_mode) contrôle le comportement lorsque les background merge operations non triviales mentionnées ci-dessus se produisent dans les parts de la table d’origine.

Les delete mutations sont un autre exemple de part merge operations qui suppriment des rows dans les parts de la table d’origine. Depuis la v24.7, nous disposons également d’un paramètre pour contrôler le comportement vis-à-vis des delete mutations déclenchées par les lightweight deletes : [`lightweight_mutation_projection_mode`](/fr/reference/settings/merge-tree-settings/other#lightweight_mutation_projection_mode).
...
* `drop` : les parts de projection affectées sont supprimées. Les requêtes reviennent alors à la part de la table d’origine pour les projections affectées.
* `rebuild` : la part de projection affectée est reconstruite afin de rester cohérente avec les données de la part de la table d’origine.
