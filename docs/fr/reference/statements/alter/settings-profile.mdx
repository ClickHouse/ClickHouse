...
`SET variable = value` est un alias de `MODIFY SETTINGS variable = value`. Il est proposé parce que `SET` est naturel et que saisir la clause `SETTINGS` de remplacement alors qu&#39;une modification incrémentale était prévue est une erreur courante.
