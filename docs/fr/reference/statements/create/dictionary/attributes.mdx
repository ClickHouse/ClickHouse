...
slug: /fr/reference/statements/create/dictionary/attributes
...
Lorsque la clé composite est constituée d’un seul attribut, la valeur de la clé peut être transmise directement, sans être encapsulée dans `tuple`. Par exemple, `dictGetString('dict_name', 'attr_name', 'key')` et `dictGetString('dict_name', 'attr_name', tuple('key'))` sont tous deux valides.
