...
Dans une requête qui commence par `FROM`, la clause `SELECT` est facultative : lorsqu’elle est omise, la requête fonctionne comme si `SELECT *` avait été écrit :
...
Cette forme est principalement utilisée dans les requêtes utilisant des [opérateurs pipe](/fr/reference/statements/select/pipe-operators) :
...
Les requêtes `SELECT` avec `FINAL` sont exécutées en parallèle. Le paramètre [max&#95;final&#95;threads](/fr/reference/settings/session-settings/max#max_final_threads) limite le nombre de threads utilisés.
...
`FINAL` peut être appliqué automatiquement à toutes les tables d’une requête à l’aide du paramètre [FINAL](/fr/reference/settings/session-settings/other#final), via une session ou un profil utilisateur.
