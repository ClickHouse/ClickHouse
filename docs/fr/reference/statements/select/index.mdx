...
* [clause PREWHERE](/fr/reference/statements/select/prewhere) (filtre la seule table à laquelle elle s’applique et s’exécute donc avant `JOIN` malgré sa position dans cette liste)
...
Une requête peut également être écrite sous la forme d’une chaîne linéaire de transformations à l’aide d’[opérateurs pipe](/fr/reference/statements/select/pipe-operators) :
...
Les clauses `GROUP BY`, `ORDER BY` et `LIMIT BY` peuvent accepter des arguments positionnels. Pour activer cette fonctionnalité, activez le paramètre [enable&#95;positional&#95;arguments](/fr/reference/settings/session-settings/enable-positional-arguments#enable_positional_arguments). Ainsi, par exemple, `ORDER BY 1,2` triera les lignes de la table d’abord selon la première colonne, puis selon la deuxième.
