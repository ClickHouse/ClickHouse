...
Les patch parts sont identiques aux parts ordinaires, mais ne contiennent que les colonnes mises à jour, les colonnes de la clé de tri de la table et les colonnes système suivantes :
...
Elles sont liées aux [colonnes virtuelles](/fr/reference/engines/table-engines/mergetree-family/mergetree#virtual-columns) de la part d’origine, qui sont ajoutées à la lecture lorsque des patch parts doivent être appliquées.
Les patch parts sont triées selon la clé de tri de la table, à laquelle sont ajoutées les colonnes `_block_number` et `_block_offset`.
Le surcoût de stockage par ligne mise à jour dépend de la taille des valeurs des colonnes de la clé de tri.

Les patch parts appartiennent à des partitions différentes de celle de la part d’origine.
L’identifiant de partition de la patch part est `patch-<hash of the patch part structure>-<original_partition_id>`.
...
Une patch part est appliquée en la fusionnant avec la partie de données selon la clé de tri de la table : une ligne est mise à jour si les valeurs des colonnes de la clé de tri ainsi que des colonnes `_block_number` et `_block_offset` sont égales dans la partie de données et dans la patch part.
L’application des patch parts nécessite de lire les colonnes de la clé de tri de la partie de données, même si la requête ne les sélectionne pas.
L’utilisation de mémoire liée à l’application d’une patch part est limitée par la plus grande plage de lignes ayant des valeurs de clé de tri égales.

<div id="patch-parts-format-in-mixed-version-clusters">
  ### Format des patch parts dans les clusters à versions mixtes
</div>

Le format des nouveaux patch parts est contrôlé par le paramètre de table [`patch_parts_version`](/fr/reference/settings/merge-tree-settings).
Les versions de ClickHouse antérieures à la version 26.8 ne prennent en charge que le format legacy `v1` et ne peuvent pas lire les patch parts au format actuel `v2` décrit ci-dessus.
Lors d’une rolling upgrade depuis une telle version, continuez à écrire au format `v1` jusqu’à ce que toutes les répliques soient mises à niveau : définissez `patch_parts_version = 'v1'` pour les tables utilisant des mises à jour légères, ou définissez le paramètre [`compatibility`](/fr/reference/settings/session-settings/compatibility#compatibility) sur la version depuis laquelle vous effectuez la mise à niveau.
Une fois la mise à niveau terminée, supprimez l’override afin de commencer à écrire les patch parts au format `v2`.
Les patch parts des deux formats restent lisibles et applicables, quelle que soit la valeur du paramètre.
