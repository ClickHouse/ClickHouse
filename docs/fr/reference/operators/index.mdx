...
## Opérateurs d’accès {#access-operators}

`a[N]` – Accès à un élément d’un tableau. Fonction `arrayElement(a, N)`.
`N` peut également être un tableau d’entiers, auquel cas les éléments situés à toutes ces positions sont renvoyés sous forme de tableau,
comme avec `arrayMap(i -> a[i], N)`. Les positions peuvent être nullables. Une position `NULL` produit `NULL` lorsque le type d’élément
peut être encapsulé dans `Nullable` ; pour les types d’éléments qui ne peuvent pas être contenus dans `Nullable` (comme `Array`, `Map`), elle produit
la valeur par défaut du type d’élément, comme pour un index `NULL` scalaire.
...
## Opérateurs pour travailler avec les chaînes de caractères {#operators-for-working-with-strings}
...
## Opérateurs pour manipuler des ensembles de données {#operators-for-working-with-data-sets}

Voir les [opérateurs IN](/fr/reference/statements/in) et l’opérateur [EXISTS](/fr/reference/operators/exists).

### fonction IN {#in-function}

`a IN ...` – La fonction `in(a, b)`.

### fonction notIn {#notin-function}

`a NOT IN ...` – La fonction `notIn(a, b)`.

### fonction globalIn {#globalin-function}

`a GLOBAL IN ...` – La fonction `globalIn(a, b)`.

### fonction globalNotIn {#globalnotin-function}

`a GLOBAL NOT IN ...` – La fonction `globalNotIn(a, b)`.

### fonction in avec sous-requête {#in-subquery-function}

`a = ANY (subquery)` – La fonction `in(a, subquery)`.

### notIn sous-requête function {#notin-subquery-function}

`a != ANY (subquery)` – Équivaut à `a NOT IN (SELECT singleValueOrNull(*) FROM subquery)`.

### Fonction in sous-requête {#in-subquery-function-1}

`a = ALL (subquery)` – Identique à `a IN (SELECT singleValueOrNull(*) FROM subquery)`.

### fonction de sous-requête notIn {#notin-subquery-function-1}
...
### `SOME` / `ALL` sur les tableaux {#some-all-on-arrays}
...
## Opérateurs pour manipuler les dates et heures {#operators-for-working-with-dates-and-times}
...
### Addition de date et d’heure {#date-time-addition}
...
<Note>
  Le résultat utilise le [fuseau horaire de session](/fr/reference/settings/session-settings/other#session_timezone) (ou le fuseau horaire par défaut du serveur si aucun fuseau horaire de session n’est défini). Le paramètre [`date_time_overflow_behavior`](/fr/reference/settings/formats/date-time#date_time_overflow_behavior) contrôle ce qui se passe lorsque le résultat sort de la plage représentable.
</Note>
...
### AT TIME ZONE et AT LOCAL {#at-time-zone}
...
## Opérateur logique AND {#logical-and-operator}
...
## Opérateur logique OR {#logical-or-operator}
...
## Opérateur de création de lambda {#lambda-creation-operator}
...
## Vérifier `NULL` {#checking-for-null}
...
Cela peut être optimisé en activant le paramètre [optimize&#95;functions&#95;to&#95;subcolumns](/fr/reference/settings/session-settings/optimize#optimize_functions_to_subcolumns). Avec `optimize_functions_to_subcolumns = 1`, la fonction lit uniquement la sous-colonne [null](/fr/reference/data-types/nullable#finding-null), au lieu de lire et de traiter l’ensemble des données de la colonne. La requête `SELECT n IS NULL FROM table` est transformée en `SELECT n.null != 0 FROM TABLE`.
...
Peut être optimisé en activant le paramètre [optimize&#95;functions&#95;to&#95;subcolumns](/fr/reference/settings/session-settings/optimize#optimize_functions_to_subcolumns). Avec `optimize_functions_to_subcolumns = 1`, la fonction lit uniquement la sous-colonne [null](/fr/reference/data-types/nullable#finding-null), au lieu de lire et de traiter l’ensemble des données de la colonne. La requête `SELECT n IS NOT NULL FROM table` se transforme en `SELECT NOT n.null FROM TABLE`.

## Vérification des valeurs booléennes {#checking-boolean-values}
