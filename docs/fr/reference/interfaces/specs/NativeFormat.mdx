...
`client_protocol_version` n’agit donc que sur la sortie. Ajouter `?client_protocol_version=54485` à une requête `INSERT ... FORMAT Native` ne change rien à la façon dont le corps est lu : le corps doit toujours être en révision `0`. Si vous fournissez un corps qui contient un préfixe `BlockInfo` ou un octet `has_custom_serialization`, le lecteur se désynchronise, ce qui se traduit par une erreur d’analyse (`INCORRECT_DATA` ou `CANNOT_READ_ALL_DATA`) plutôt que par un insert réussi.
