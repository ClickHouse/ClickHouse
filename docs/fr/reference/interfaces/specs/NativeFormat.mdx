...
<img className="w-full h-auto" src="/images/interfaces/native-format/block-structure.webp" alt="Structure d’un bloc sur le réseau en colonnes et de ses familles d’organisation des données" width="1568" height="976" loading="lazy" />
...
<img className="w-full h-auto" src="/images/interfaces/native-format/decoder-dispatch.webp" alt="Routage du décodeur depuis une description de type vers sa famille d’encodage" width="1568" height="854" loading="lazy" />
...
Ainsi, avec la révision `0` — la valeur par défaut pour `FORMAT Native` — `LowCardinality(T)` n’est **pas** supprimé. Sa chaîne de type et son state prefix par bloc restent dans le flux, et le lecteur en révision `0` les lit tels quels. La suppression ne s’applique qu’à une révision non nulle inférieure à `54405`, ou lorsqu’elle est forcée indépendamment de la révision.

Ce forçage correspond au flag `remove_low_cardinality`. La sortie `FORMAT Native` ne l’active jamais, mais le chemin TCP natif, oui, lorsque `low_cardinality_allow_in_native_format = 0` (valeur par défaut : `1`). Autrement dit, ce setting modifie la sortie native TCP mais n’a aucun effet sur `FORMAT Native`.
...
<img className="w-full h-auto" src="/images/interfaces/native-format/array-offsets.webp" alt="Les offsets associent les lignes à un flux de valeurs partagé" width="1568" height="388" loading="lazy" />
...
<img className="w-full h-auto" src="/images/interfaces/native-format/nested-layout.webp" alt="Disposition des colonnes composites avec et sans aplatissement" width="1172" height="1068" loading="lazy" />
...
<img className="w-full h-auto" src="/images/interfaces/native-format/state-prefix-per-block.webp" alt="Préfixe d’état répété pour chaque bloc non vide" width="1226" height="1162" loading="lazy" />
...
<img className="w-full h-auto" src="/images/interfaces/native-format/variant-discriminators.webp" alt="Discriminateurs sélectionnant des valeurs dans des séquences denses propres à chaque type" width="1568" height="624" loading="lazy" />
...
<img className="w-full h-auto" src="/images/interfaces/native-format/compression-frame.webp" alt="Champs de la trame de compression et limite de taille compressée" width="1568" height="206" loading="lazy" />
...
L’octet de méthode encode également les [codecs au niveau des colonnes](/fr/reference/statements/create/table/codec). Ceux-ci sont appliqués par colonne sur les paths sur disque de MergeTree plutôt qu’au tramage sur flux complet, mais le chemin d’entrée HTTP `decompress=1` prend le codec dans l’octet de méthode de chaque trame, de sorte que n’importe lequel de ces octets peut légitimement apparaître en entrée. Un décodeur conforme doit donc reconnaître tout l’espace alloué et rejeter un octet qu’il n’implémente pas, plutôt que de mal interpréter le corps. Leurs corps sont spécifiques à chaque codec et sortent du cadre de ce contrat de trame générique :
