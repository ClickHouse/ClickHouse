...
| `input_format_arrow_use_native_reader`                                       | Utiliser le lecteur natif de ClickHouse pour les formats `Arrow` et `ArrowStream` au lieu du lecteur de la bibliothèque Apache Arrow. Définissez `0` pour utiliser le lecteur de la bibliothèque Apache Arrow.                                                           | `1`               |
...
| `output_format_arrow_unsupported_types_as_binary`                            | Produire sous forme de données binaires brutes un type qui n&#39;a pas d&#39;équivalent Arrow (par ex. `BFloat16`, `AggregateFunction`). Si false, ce type déclenche une exception. S&#39;applique à la fois au writer natif et à celui de la bibliothèque Apache Arrow. | `1`               |
...
| `output_format_arrow_use_native_writer`                                      | Utiliser le writer natif de ClickHouse pour les formats `Arrow` et `ArrowStream` au lieu du writer de la bibliothèque Apache Arrow. Définissez `0` pour utiliser le writer de la bibliothèque Apache Arrow.                                                              | `1`               |
