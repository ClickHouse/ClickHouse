...
description: Découvrez comment résoudre les avertissements Docker liés aux capacités `CAP_IPC_LOCK` et `CAP_SYS_NICE` lors de l’exécution de ClickHouse dans un conteneur.
date: 2023-06-07
...
Voici à quoi ressemblent les messages de log indiquant l’absence de la capacité `CAP_SYS_NICE` ou `CAP_SYS_NICE` :
...
1. Ajoutez deux arguments `--cap-add` pour doter le conteneur des capacités `IPC_LOCK` et `SYS_NICE` :
...
4. Vérifiez que les capacités sont bien appliquées.
...
5. Redémarrez le serveur ClickHouse et les messages de log ne devraient plus s’afficher.
