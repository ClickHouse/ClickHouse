...
<div id="storage-formats-full-vs-packed">
  ## Formats de stockage : Full vs Packed (ClickHouse Cloud)
</div>
...
| **Packed** | Tous les fichiers sont regroupés dans un seul fichier d’archive          | ClickHouse Cloud uniquement |

<div id="full-storage">
  ### Stockage Full
</div>

Dans le stockage Full, chaque part est constituée de plusieurs fichiers distincts, stockés individuellement sur le disque. C’est l’option par défaut, et la seule disponible, dans ClickHouse open source.

<div id="packed-storage">
  ### Stockage Packed (ClickHouse Cloud uniquement)
</div>
...
Dans ClickHouse Cloud, une part utilise le stockage Packed si **l’une** de ces conditions est remplie :
...
<Info>
  **Open source**
  ...
  Bien que les paramètres `min_bytes_for_full_part_storage`, `min_rows_for_full_part_storage` et `min_level_for_full_part_storage` soient définis dans la version open source de ClickHouse, ils n’ont aucun effet, car l’implémentation du stockage Packed n’est disponible que dans ClickHouse Cloud.
</Info>

Le paramètre `min_level_for_full_part_storage` peut être utilisé pour optimiser à la fois les performances et les coûts dans les environnements ClickHouse Cloud, en particulier pour les tables qui reçoivent des insertions en continu.
...
Définissez `min_level_for_full_part_storage = 2` pour les déploiements Cloud.
Cela garantit que :
...
Ces deux concepts sont orthogonaux, et toutes les combinaisons sont possibles selon que vous utilisez ClickHouse Cloud ou ClickHouse open source :

| Combinaison      | Cas d’usage                                                  |
| ---------------- | ------------------------------------------------------------ |
| Wide + Full      | Grandes parts sur stockage local (par défaut en open source) |
| Wide + Packed    | Grandes parts sur cloud storage                              |
| Compact + Full   | Petites parts sur stockage local                             |
| Compact + Packed | Petites parts sur cloud storage                              |
