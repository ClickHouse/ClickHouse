...
description: 'Setting up an OpenTelemetry Collector for ClickHouse Observability'
doc_type: 'guide'
...
This guide walks you through deploying an OpenTelemetry collector against an existing ClickHouse Observability service, or adapting your existing collector, before verifying that data is flowing through.
...
<AgentPrompt
  prompt="Use the clickstack-otel-collector skill to wire an OpenTelemetry collector into my ClickHouse Observability service."
...
Deploy the **ClickStack distribution of the OpenTelemetry collector**, which is preconfigured for ClickHouse Observability. In the example below, we run the collector locally and generate artificial telemetry from the same machine for simplicity.
...
Extend your existing collector configuration to write to ClickHouse Observability via the [ClickHouse exporter](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/exporter/clickhouseexporter).
...
For further details on configuring OpenTelemetry collectors against ClickHouse Observability, see [Ingesting with OpenTelemetry](/clickstack/ingesting-data/opentelemetry).
</Step>
