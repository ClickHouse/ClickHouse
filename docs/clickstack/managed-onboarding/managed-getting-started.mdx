...
title: 'Get started with ClickHouse Observability'
description: 'Deploy an OpenTelemetry collector against ClickHouse Observability and instrument an application end to end'
doc_type: 'guide'
...
This guide takes you from an empty ClickHouse Observability service all the way to logs, metrics, traces, and session replays flowing from a real application. You'll deploy a new OpenTelemetry collector (or adapt an existing one if you have one) against your service, instrument a sample Node.js application with no changes to its business logic, then explore the telemetry in the ClickStack UI.
...
- A **ClickHouse Observability service** in [ClickHouse Cloud](https://console.clickhouse.cloud).
...
Deploy the **ClickStack distribution of the OpenTelemetry collector**, which is preconfigured for ClickHouse Observability. In the example below, we run the collector locally for simplicity.
...
- [Tuning ClickHouse Observability](/clickstack/managed-onboarding/tuning-clickstack-schema): refine your schema for query performance and storage efficiency.
