...
export const CsCard = ({ title, img, darkImg, abbr, href }) => {
  const assetBase = '/docs';
  ...
  return <a href={href} className="cs-card-link" onClick={(e) => { e.preventDefault(); window.location.href = withBase(href); }} style={{ textDecoration: 'none', textDecorationLine: 'none', color: 'inherit' }}>
    ...
    >
      {img ? (
        <>
          <div className="cs-card-logo-light" style={{ width: '56px', height: '56px', flexShrink: 0, backgroundImage: `url(${withBase(img)})`, backgroundSize: 'contain', backgroundRepeat: 'no-repeat', backgroundPosition: 'center' }} />
          {darkImg && <div className="cs-card-logo-dark" style={{ width: '56px', height: '56px', flexShrink: 0, backgroundImage: `url(${withBase(darkImg)})`, backgroundSize: 'contain', backgroundRepeat: 'no-repeat', backgroundPosition: 'center' }} />}
        </>
      ) : (
        <div className="cs-card-abbr" style={{
      ...
      <span className="cs-card-title text-sm font-semibold text-center" style={{
        lineHeight: '1.3', color: '#111111',
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-6" style={{ gap: '16px', margin: '20px 0 36px' }}>
        {items.map(item => <CsCard key={item.title} {...item} />)}
      </div>
    </>
  );
};
