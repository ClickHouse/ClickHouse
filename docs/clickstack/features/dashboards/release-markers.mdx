...
To read the version from somewhere else, set **Service Version Expression** on the log or trace source. Edit the source under **Team Settings → Sources**, expand **Configure Optional Fields**, and fill in the field.

<Image img="/images/clickstack/dashboards/service-version-expression.webp" alt="Service Version Expression field in the source optional fields, showing the default placeholder ResourceAttributes['service.version']" size="lg"/>
...
| Aggregate line over many services | None                                                                |

In the third case the releases belong to services that have no line on the chart, so no markers are drawn.
