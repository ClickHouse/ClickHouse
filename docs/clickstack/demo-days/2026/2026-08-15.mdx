...
One backwards-incompatible cleanup landed with the feature. Variables passed to `$__filter` and `$__conditionalAll` must now use the `$var` form. Bare names such as `var` were previously accepted, which made it unclear whether an argument was a variable or plain text.

Variables and macros used as macro arguments are now expanded too, so `$__timeFilter($TimeColumn)` works as expected.
