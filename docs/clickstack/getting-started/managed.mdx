...
The form pre-populates expressions for the default OpenTelemetry schema. Replace them with expressions that match the Nginx table:

| Setting | Value |
|---|---|
| **Service Name Expression** | `'nginx'` |
| **Log Level Expression** | `multiIf(status >= 500, 'ERROR', status >= 400, 'WARN', 'INFO')` |
| **Log Attributes Expression** | `map('http.remote_addr', remote_addr, 'http.status_code', toString(status), 'http.request', request)` |
