...
For production, create a dedicated user and grant it access to the Vector target table. Open the [ClickHouse Cloud SQL console](/products/cloud/features/sql-console-features/sql-console) and run:
...
For OpenTelemetry, ClickStack creates the data sources automatically. Open the logs view in ClickStack.
...
<CardGroup cols={2}>
  <Card title="Prepare for production" icon="shield" href="/clickstack/managing/production">
