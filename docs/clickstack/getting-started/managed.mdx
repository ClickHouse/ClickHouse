---
title: 'Getting started with ClickHouse Observability'
sidebarTitle: 'Getting started'
description: 'Getting started with ClickHouse Observability'
doc_type: 'guide'
keywords: ['ClickHouse Observability', 'getting started', 'ClickHouse Cloud']
slug: /clickstack/getting-started/managed
---
...
Deploy ClickHouse Observability on ClickHouse Cloud, send a test event through your ingestion pipeline, and confirm that the event is available in the ClickHouse Observability UI.

ClickHouse Cloud operates the ClickHouse backend and observability UI while you retain control over the ingestion pipeline and schema. ClickHouse Observability provides:
...
- To start a new [OpenTelemetry Collector](/clickstack/managed/ingesting-data/collector), install [Docker](https://docs.docker.com/get-docker/). For Kubernetes, deploy the collector with [Helm](/clickstack/managed/ingesting-data/collector#configuring-the-collector).
- To use an existing collector, run it in the [gateway role](/clickstack/managed/ingesting-data/collector#collector-roles) and ensure that its distribution includes the [ClickHouse exporter](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/exporter/clickhouseexporter). You will add the required configuration in this guide.

## Set up ClickHouse Observability {#set-up-managed-clickstack}

<Steps titleSize="h3">
<Step title="Choose an ingestion source and collector setup" id="choose-an-ingestion-source">
From your ClickHouse Cloud service, launch **ClickStack** to open ClickHouse Observability. On the ClickHouse Observability **Getting Started** page, select **Start ingestion**.
...
ClickHouse Observability generates the collector command with the `default` administrator credentials. We recommend dedicated ingestion credentials to keep ingestion access separate from administration and avoid relying on the administrator password.
...
<Note>
This command exposes the OTLP ports without authentication and is intended for local evaluation. Before sending production traffic, [secure the collector](/clickstack/managed/ingesting-data/collector#securing-the-collector).
</Note>
...
After ClickHouse Observability detects the OpenTelemetry data sources, select **Start exploring** to open the **Search** view. Search for `ClickStack ingestion test`.

The result should include the test event with the `clickstack-docs-test` service name.

<Image img="/images/clickstack/getting-started/clickstack_ingestion_test.webp" size="lg" alt="ClickHouse Observability logs view showing the ClickStack ingestion test event" border/>
...
Start with an [existing Vector pipeline](/clickstack/managed/ingesting-data/vector) that can send data to ClickHouse.

## Set up ClickHouse Observability {#set-up-managed-clickstack-vector}
...
When using Vector with ClickHouse Observability, you define the schema. It can follow OpenTelemetry conventions or use fields specific to your events.

<Info>
**Existing Vector pipeline required**

Continue with this guide if you already run Vector with an input pipeline. The data sent through the pipeline must include a **timestamp column** or equivalent time field, which you select when configuring the data source in the ClickHouse Observability UI.
...
We strongly recommend understanding how [Primary keys](/concepts/core-concepts/primary-indexes) work in ClickHouse and choosing an ordering key based on your access patterns. See the [ClickHouse Observability-specific](/clickstack/managed/managing/performance-tuning#choosing-a-primary-key) guidance on choosing a primary key.
...
#### Create a ClickHouse Observability data source {#create-a-vector-data-source}

Create a data source for the table populated by your Vector pipeline. ClickHouse Observability prompts you to create one on your first login.
...
For other source settings, see the [ClickHouse Observability configuration reference](/clickstack/managed/managing/config).
...
For more Vector source and transformation examples, see [Ingesting with Vector](/clickstack/managed/ingesting-data/vector).
...
<Image img="/images/clickstack/getting-started/clickstack_managed_ui.webp" size="lg" alt="Logs in the ClickHouse Observability UI"/>
...
You now have a ClickHouse Observability service, a working ingestion path, and a test event that you can inspect in ClickHouse Observability.
...
Choose a guide for the data that you want to send to ClickHouse Observability:

<CardGroup cols={2}>
  <Card title="Instrument an application" icon="code" href="/clickstack/managed/ingesting-data/sdks">
    Send application traces and logs with a supported OpenTelemetry SDK.
  </Card>
  <Card title="Collect host logs" icon="server" href="/clickstack/managed/integration-examples/host-logs">
    Forward host logs from an OpenTelemetry Collector running in the agent role.
  </Card>
  <Card title="Monitor Kubernetes" icon="cubes" href="/clickstack/managed/integration-examples/kubernetes">
    Collect logs, metrics, and traces from a Kubernetes cluster.
  </Card>
  <Card title="Explore other integrations" icon="plug" href="/clickstack/managed/integration-examples">
...
Use a sample dataset to explore ClickHouse Observability with richer telemetry:

<CardGroup cols={2}>
  <Card title="Sample logs, traces, and metrics" href="/clickstack/managed/example-datasets/sample-data">
  ...
  <Card title="Local logs and metrics" href="/clickstack/managed/example-datasets/local-data">
...
<CardGroup cols={2}>
  <Card title="Generate data with otelgen" icon="terminal" href="/clickstack/managed/example-datasets/otelgen">
    Send a short burst of synthetic OTLP logs, traces, and metrics.
  </Card>
  <Card title="Generate data with telemetrygen" icon="terminal" href="/clickstack/managed/example-datasets/telemetrygen">
    Generate configurable OpenTelemetry signals across several services.
  </Card>
</CardGroup>

See [all ClickHouse Observability sample data and demos](/clickstack/managed/example-datasets).

### Prepare for production {#prepare-for-production}

Review production and sizing guidance before using ClickHouse Observability for sustained workloads:

<CardGroup cols={2}>
  <Card title="Going to production" icon="shield" href="/clickstack/managed/managing/production">
...
For deployment tasks, see the [ClickHouse Observability deployment guide](/clickstack/deployment/managed#additional-tasks).
