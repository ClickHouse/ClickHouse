...
description: 'Run ClickStack locally, send test data, and confirm that ingestion works'
doc_type: 'guide'
keywords: ['ClickStack', 'getting started', 'Docker deployment', 'OpenTelemetry', 'local deployment']
slug: /clickstack/getting-started/oss
---

import { Image } from "/snippets/components/Image.jsx";

Run ClickStack locally, send a test event through its OpenTelemetry Collector, and confirm that the event is available in the ClickStack UI.

With ClickStack, you run and manage the ClickHouse backend, ClickStack UI, and ingestion pipeline. The all-in-one image bundles these components with MongoDB in a single Docker container for local evaluation and testing.
...
## Set up ClickStack with the all-in-one image {#set-up-clickstack-open-source}
...
<Image img="/images/clickstack/getting-started/clickstack_ingestion_test_oss.webp" size="lg" alt="ClickStack logs view showing the ClickStack ingestion test event" border/>
</Step>
</Steps>
...
<CardGroup cols={2}>
  <Card title="Instrument an application" icon="code" href="/clickstack/ingesting-data/sdks">
  ...
  <Card title="Explore other integrations" icon="plug" href="/clickstack/integration-examples">
...
See [all ClickStack sample data and demos](/clickstack/example-datasets).
