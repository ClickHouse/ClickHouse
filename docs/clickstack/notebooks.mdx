---
title: 'AI Notebooks with ClickHouse Observability'
sidebarTitle: 'AI Notebooks'
description: 'AI-powered investigation notebooks for ClickHouse Observability'
doc_type: 'guide'
keywords: ['clickstack', 'AI notebooks', 'investigation', 'observability', 'HyperDX']
slug: /clickstack/notebooks
---
...
AI Notebooks are an interactive investigation tool in ClickHouse Observability that combines an AI agent with manual analysis. You can describe an issue in plain language, and the AI agent will query logs, traces, and metrics on your behalf — surfacing relevant data, charts, and summaries as a series of tiles. You can also add your own tiles (charts, tables, searches, and markdown notes) alongside the AI-generated output, building a complete record of an investigation.
...
<Info>
**ClickHouse Observability only**

AI Notebooks are only available in ClickHouse Observability deployments.
</Info>
...
| **Chart**    | `L`      | A time series line chart, using the same visualization builder as [Dashboards](/clickstack/managed/features/dashboards/overview). |
| **Table**    | `T`      | A tabular aggregation view.                                                                                                     |
| **Markdown** | `M`      | Freeform text for notes, hypotheses, or conclusions.                                                                            |

After adding a tile, it opens in inline edit mode where you can configure the data source, filters, and aggregations — the same interface used when building [dashboard visualizations](/clickstack/managed/features/dashboards/overview#creating-visualizations). Click **Save** to finalize the tile.
