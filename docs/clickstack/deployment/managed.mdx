...
Note that dashboards, saved searches and alerts are scoped to the service ClickStack was launched from, so anything saved on another service in the warehouse isn't available here and has to be recreated. Data sources are auto-detected, so search works immediately.
