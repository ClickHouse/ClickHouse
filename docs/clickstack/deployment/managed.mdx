...
slug: /clickstack/deployment/managed
---
...
This **guide is for existing users of ClickHouse Cloud**. If you're new to ClickHouse Cloud, we recommend our [Getting Started](/clickstack/getting-started/managed) guide for ClickHouse Observability.
...
In this mode, data ingestion is entirely left to the user. You can ingest data into ClickHouse Observability using your own hosted OpenTelemetry collector, direct ingestion from client libraries, ClickHouse-native table engines (such as Kafka or S3), ETL pipelines, or ClickPipes — ClickHouse Cloud's managed ingestion service. This approach offers the simplest and most performant way to operate ClickStack.
...
The following guide assumes you have already created a ClickHouse Cloud service. If you haven't created a service, follow the [Getting Started](/clickstack/getting-started/managed) guide for ClickHouse Observability. This will leave you with a service in the same state as this guide i.e. ready for observability data with ClickStack enabled.
...
From the ClickHouse Cloud landing page, select the service for which you wish to enable ClickHouse Observability.
...
For other possible options, see the [configuration reference](/clickstack/managed/managing/config#hyperdx).
...
### Granting access to ClickHouse Observability {#configure-access}
...
Note that different data sources in ClickStack—such as logs and traces—can be correlated with each other. To enable this, additional configuration is required on each source. For example, in the logs source, you can specify a corresponding trace source, and vice versa in the traces source. See ["Correlated sources"](/clickstack/managed/managing/config#correlated-sources) for further details.
</Info>
...
If using your own schema, we recommend creating a Logs source ensuring the required fields are specified - see ["Log source settings"](/clickstack/managed/managing/config#logs) for further details.

<JSONSupport schemaUrl="/clickstack/managed/ingesting-data/schema/map-vs-json" />
