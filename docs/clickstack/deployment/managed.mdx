...
How you grant access depends on whether your organization has migrated to custom roles:

<Tabs>
<Tab title="Custom roles">

[Assign a custom Cloud role](/products/cloud/guides/security/cloud-access-management/manage-custom-roles) with access to the organization, the selected service, and its database. Choose **Admin** database access for users who need to enable alerts, or **Read-only** for users who only need to view observability data and create dashboards.
</Tab>
<Tab title="Legacy SQL console access">

For an organization that has not migrated to custom roles:
...
3. Set the appropriate permission level for each user:
   ...
   - **No access** - Can't access Managed ClickStack.
...
To enable alerts, at least one user with **Admin** database access through a custom role, or legacy **Service Admin** permissions mapped to **Full Access**, must log into Managed ClickStack at least once. This provisions a dedicated user in the database that runs alert queries.
</Warning>
