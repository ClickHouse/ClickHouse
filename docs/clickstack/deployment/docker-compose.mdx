...
description: 'Deploying ClickStack with Docker Compose'
doc_type: 'guide'
...
slug: /clickstack/deployment/docker-compose
---
...
This distribution can be used with ClickHouse Cloud, but it differs from [ClickHouse Observability](/clickstack/deployment/managed). In this setup, you manage the ClickStack UI yourself while using ClickHouse Cloud only for compute and storage. Unless you have a specific reason to operate the UI independently, we recommend using ClickHouse Observability, which includes integrated authentication and additional enterprise features, and removes the need to manage the ClickStack UI yourself.
