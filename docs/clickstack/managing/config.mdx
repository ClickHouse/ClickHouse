...
| `Displayed Timestamp Column`  | Optional, higher-precision timestamp column used in UI display.                                                                                   | Yes       | Yes                         | `Timestamp`                                           |
...
| `Known Columns List`            | For sources over a [`Distributed`](/engines/table-engines/special/distributed) table whose target tables have non-matching column sets. A comma-separated list of the columns available across all target tables, used instead of `SELECT *` when fetching full row data (for example, the row side panel). Leave blank to select all columns. Column names only - no expressions or aliases. | No       | No                          |  –                       |
| `Use Text Index`                | Whether Lucene-based searches emit `hasAllTokens` when searching the implicit column.                | No       | No                          | `Auto (detect from schema)` |
| `Displayed Timestamp Column`    | High-precision timestamp column used in UI display.                                                                                   | Yes      | Yes                         | `Timestamp`              |
