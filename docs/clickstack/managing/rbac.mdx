...
|---|---|
| **SQL Console Admin** (Full Access) | Full access to ClickStack. Required for enabling [alerts](/clickstack/managed/features/alerts). |
...
- **[Dashboards](/clickstack/managed/features/dashboards/overview)** — saved dashboard layouts and charts.
- **[Saved searches](/clickstack/managed/features/search)** — persisted log/trace/event queries.
- **Sources** — ingestion source configurations.
- **[Alerts](/clickstack/managed/features/alerts)** — alert rules and their notification settings.
- **Webhooks** — outbound notification destinations (such as Slack, PagerDuty, and generic HTTP endpoints) that [alerts](/clickstack/managed/features/alerts) deliver to. This doesn't refer to the ClickStack API.
