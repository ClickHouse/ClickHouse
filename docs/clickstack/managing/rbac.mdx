...
2. **Have access to the service and its database.** How you grant access depends on whether your organization has migrated to custom roles:

   - For migrated organizations, [assign a custom Cloud role](/products/cloud/guides/security/cloud-access-management/manage-custom-roles) with access to the organization, the selected service, and its database.
   - For organizations that have not migrated, use the [legacy SQL console role assignments](/products/cloud/guides/security/cloud-access-management/manage-sql-console-role-assignments) in **Settings** → **SQL Console Access**.

| Database access | ClickStack access |
|---|---|
| **Admin** through a custom role, or legacy **Full Access** | Full access to ClickStack. Required for enabling [alerts](/clickstack/features/alerts). |
| **Read-only** through a custom role, or legacy **Read Only** | Can view observability data and create dashboards. |
| No database access | Can't access ClickStack. |
