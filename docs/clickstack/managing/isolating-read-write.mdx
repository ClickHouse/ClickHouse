...
Dashboards, saved searches and alerts belong to the service ClickStack was launched from, and don't follow you to another service in the same warehouse - even though both services share the same data. Data sources are auto-detected on the new service, so search works straight away, but anything you saved on the previous service has to be recreated.
...
These per-service queries are the reliable check, because `system` tables are local to each service. For an aggregate view across the warehouse, note that `clusterAllReplicas` with the `default` cluster name only targets the current service - use `all_groups.default` instead:
...
**Auto-idling.** The first query to a read-only service that has idled pays a wake-up cost, so intermittent use trades a little latency for lower spend. Alerts evaluating continuously will generally keep the service awake. Continuous ingestion likewise keeps the read-write service awake, but if your ingestion is bursty or scheduled, the first batch after an idle period pays the same cost - which surfaces as delayed telemetry.
