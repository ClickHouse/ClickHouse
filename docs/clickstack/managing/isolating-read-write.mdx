...
WHERE event_time > now() - toIntervalMinute(10) 
  ...
  AND arrayExists(
      t -> splitByChar('.', t)[-1] IN ('otel_logs', 'otel_traces', 'hyperdx_sessions')
           OR splitByChar('.', t)[-1] LIKE 'otel_metrics%', 
      tables
  )
GROUP BY ALL
ORDER BY count() DESC;
...
The filter on `tables` keeps the result to queries that touched the ClickStack tables, and `http_user_agent` distinguishes the UI from anything else connecting to the endpoint.
...
A service whose only workload is alert evaluation can still idle, and alerts don't evaluate while it's idled - so an alerting service left with auto-idling enabled can silently stop alerting. Turn auto-idling off on that service and plan for it to be always-on. The same applies wherever your alerts are evaluated: if they run on the service serving the UI, that service can't be left to idle either.
</Warning>
