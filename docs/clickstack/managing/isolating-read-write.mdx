...
Warehouses are a ClickHouse Cloud feature, so the topology described here applies to ClickStack running against ClickHouse Cloud. Self-managed ClickStack deployments have no equivalent way to serve the same data from separate compute.
