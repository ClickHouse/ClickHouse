...
Users, roles and grants aren't schema changes - they're shared by every service in the warehouse and only need to be created once. The [setup steps](#prepare-read-write-service) above create them on the read-write service, alongside the database.
