...
Users, roles and grants aren't schema changes - they're shared by every service in the warehouse, so each only needs to be created once, from any service. The [setup steps](#prepare-read-write-service) above create the ingestion user; a self-hosted ClickStack UI additionally needs its own query user, created the same way.
