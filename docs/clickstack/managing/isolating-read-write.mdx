...
- **Queries don't compete with ingestion.** A read-only service performs no background merges outside of system tables, so its resources are fully dedicated to read queries.
- **Ingestion can't be disrupted by an expensive query.** An unbounded dashboard query or ad-hoc search runs on separate compute and can't slow down or back up your ingest pipeline.
- **Each side scales independently.** The [sizing model](/clickstack/managing/estimating-resources) estimates ingest compute and query compute separately - its [worked example](/clickstack/managing/estimating-resources#worked-example) arrives at 58 vCPUs for ingest and 290 vCPUs for queries. A warehouse lets you provision each of those figures as its own service, and scale each as its side of the workload grows.
...
See our guide on [Creating an ingestion user](/clickstack/ingesting-data/collector#creating-an-ingestion-user) for further details.
</Step>
...
**Restricting who can reach which endpoint.** IP filtering is applied per service, so you can restrict the read-write endpoint to your collectors and the read-only endpoint to your ClickStack deployment. See our guide on [Network access control](/products/cloud/features/infrastructure/warehouses#network-access-control).

**Auto-idling.** Read-only services idle without delay, since they aren't kept awake by background merges. This makes them cheap to run for intermittent use, but the first query after idling pays a wake-up cost. If alerts run continuously, they'll generally keep the service awake.
