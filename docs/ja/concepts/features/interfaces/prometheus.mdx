...
range-query endpointを使用して、指定した時間範囲で式を評価します。
...
HTTP API、`promql` 方言、およびテーブル関数で使用される関数と集約演算子の一覧については、[サポートされている PromQL 機能](/ja/sql-reference/table-functions/prometheusQueryRange#supported-promql-features)を参照してください。
...
ClickHouse では、HTTP API、`promql` 方言、および [`prometheusQuery`](/ja/sql-reference/table-functions/prometheusQuery) と [`prometheusQueryRange`](/ja/sql-reference/table-functions/prometheusQueryRange) のテーブル関数で、同じ PromQL コンバーターを使用します。
