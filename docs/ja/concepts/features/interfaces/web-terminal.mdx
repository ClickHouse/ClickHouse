...
`user` フィールドは任意です。省略されているか空の場合、ユーザー名には `default_session_user` [サーバー設定](/ja/operations/server-configuration-parameters/settings) (または [コンポーザブルプロトコル](/ja/operations/settings/composable-protocols) 構成内のエンドポイントごとのオーバーライド) が使用され、別途設定されていない限り `default` になります。`default_session_user` が空文字列に設定されている場合、ユーザー名なしの接続は禁止されます。`user` が省略されているか空の `auth` メッセージは認証に失敗し、サーバーはコード `1008` で WebSocket を閉じます。また、サーバー構成で `session_log` セクションが有効になっている場合、この拒否は空の `user` を持つ `LoginFailure` イベントとして [`system.session_log`](/ja/operations/system-tables/session_log) に記録されます。
