...
<EnterprisePlanFeatureBadge feature="カスタム ID プロバイダーによる JWT 認証" support="true" />

組み込みの 認証器 に加えて、独自の ID プロバイダー (Microsoft Entra や Okta など) が発行する JWT を受け入れるよう、ClickHouse Cloud サービスを設定できます。これはベータ機能であり、ClickHouse バージョン 26.4 以降を実行するサービスでは Enterprise プランで利用できます。有効にするには、以下の設定情報を添えて[サポートチケット](https://clickhouse.com/support/program)を作成してください。

| パラメータ      | 説明                                                                                                                                                                                                                           |
| ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| サービス ID    | JWT 認証を有効にする ClickHouse Cloud サービスの UUID。サービスを開くと、Cloud Console の URL で確認できます。                                                                                                                                               |
| issuer        | ID プロバイダーが発行するトークンの `iss` クレームの値。通常はプロバイダーの URL です (例: `https://your-tenant.okta.com`) 。ClickHouse は、発行者がこの値と一致しないトークンを拒否します。                                                                                                |
| audience    | ID プロバイダーがこのサービス向けのトークンに設定する `aud` クレームの値。ClickHouse は、異なるオーディエンス向けに発行されたトークンを拒否します。                                                                                                                                         |
| JWKS URL   | ID プロバイダーが JSON Web Key Set を公開する HTTPS エンドポイント (例: `https://idp.example.com/.well-known/jwks.json`) 。ClickHouse はこのエンドポイントから公開鍵を取得してトークン署名を検証します。[必須クレーム](#required-claims)で説明しているとおり、JWKS 検証で使用できるのは RSA 鍵 (`RS256`) のみです。 |
| ロールクレーム    | 一時ユーザーの ClickHouse ロールを格納するトークンクレームの名前 (デフォルトは `clickhouse:roles`) 。このクレームには、`["analyst", "reader"]` のようにロール名の JSON 配列を含める必要があります。                                                                                       |
| grant クレーム | 一時ユーザーの SQL `GRANT` の断片を格納するトークンクレームの名前 (デフォルトは `clickhouse:grants`) 。このクレームには、`["SELECT ON analytics.*", "INSERT ON analytics.events"]` のように grant フラグメントの JSON 配列を含める必要があります。                                       |

ロールクレームで指定するロールは、サービス内にあらかじめ存在している必要があります (`CREATE ROLE` で作成します) 。[アクセス権](#access-rights)で説明しているとおり、権限上限によって、ロールクレームと grant クレームの両方から得られる有効なアクセス権が制限されます。
