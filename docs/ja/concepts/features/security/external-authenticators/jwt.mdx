...
<EnterprisePlanFeatureBadge feature="カスタム identity provider による JWT 認証" support="true" />

組み込みの 認証器 に加えて、独自の identity provider (Microsoft Entra や Okta など) が発行する JWT を受け入れるよう、ClickHouse Cloud サービスを設定できます。これはベータ機能であり、ClickHouse バージョン 26.4 以降を実行するサービスでは Enterprise プランで利用できます。Cloud Console の **Settings → Security → JWT authentication** からご自身で設定できます。手順については [JWT authentication setup](/ja/cloud/security/jwt-authentication-setup) を参照してください。各プロバイダーは、以下によって定義されます。
...
| JWKS URL     | identity provider が JSON Web Key Set を公開する HTTPS エンドポイント (例: `https://idp.example.com/.well-known/jwks.json`) 。ClickHouse はこのエンドポイントから公開鍵を取得してトークン署名を検証します。[必須クレーム](#required-claims)で説明しているとおり、JWKS 検証で使用できるのは RSA 鍵 (`RS256`) のみです。 |
...
ロールは上記のロールクレームから読み取られ、SQL grant はデフォルトでトークンの `clickhouse:grants` クレームから読み取られます。ロールクレームで指定するロールは、サービス内にあらかじめ存在している必要があります (`CREATE ROLE` で作成します) 。[アクセス権](#access-rights)で説明しているとおり、権限上限によって、ロールクレームと grant クレームの両方から得られる有効なアクセス権が制限されます。
