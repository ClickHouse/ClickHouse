...
次の例は、上の図に示した I/O スケジューリング階層を定義する方法を示しています。
...
基盤となるリソースの能力を最大限に活用できるようにするには、`inflight_limit` を使用してください。`max_requests` や `max_cost` の値が小さすぎるとリソースを十分に活用できない可能性がある一方で、大きすぎると scheduler 内のキューが空になり、その結果、サブツリー内でポリシーが無視される (不公平が生じたり優先度が無視されたりする) 可能性がある点に注意してください。一方、リソースの使用率が高くなりすぎるのを防ぎたい場合は、`bandwidth_limit` を使用してください。これは、`duration` 秒間に消費されたリソース量が `max_burst + max_speed * duration` バイトを超えるとスロットリングを行います。同じリソース上の 2 つの `bandwidth_limit` ノードを使うことで、短い時間帯におけるピーク帯域幅と、より長い時間帯における平均帯域幅を制限できます。
