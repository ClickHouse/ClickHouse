...
<Note>
  ...
  [設定に対する制約](/ja/concepts/features/configuration/settings/constraints-on-settings)を参照してください。
</Note>
...
<Info>
  **KILL QUERY**
  ...
  自分自身のクエリを kill する場合は `KILL QUERY` 権限が不要なため、`readonly` と `allow_ddl` のどの組み合わせでも動作します。ただし `system.processes` に対する `SELECT` は必要です。他のユーザーのクエリを kill する場合、および `KILL QUERY ... ON CLUSTER` には `KILL QUERY` 権限が必要であり、
