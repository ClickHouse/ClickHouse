...
| [`timeout_overflow_mode`](/ja/reference/settings/session-settings/timeout-overflow-mode#timeout_overflow_mode)                                 | クエリの実行時間が `max_execution_time` を超えた場合、または推定実行時間が `max_estimated_execution_time` を超える場合の動作を設定します。 |
