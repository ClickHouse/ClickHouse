...
<Steps>
  <Step title="クエリ条件を解析する">
    この例のクエリには、2 つの主キーカラムの両方に対する条件が含まれています: `town = 'LONDON' AND street = 'OXFORD STREET'`.
  </Step>

  <Step title="プライマリインデックスを読み込む">
    クエリを高速化するために、ClickHouse はテーブルのプライマリインデックスをメモリに読み込みます。
  </Step>

  <Step title="一致するグラニュールを特定する">
    次に、索引エントリを走査して、条件に一致する行を含む可能性があるグラニュール、つまりスキップできないグラニュールを特定します。
  </Step>

  <Step title="グラニュールを読み込み処理する">
...
ClickHouse のスパースプライマリ索引の仕組みや、従来のデータベース索引との違い、利用時のベストプラクティスについて詳しくは、索引に関する詳細な[解説](/ja/guides/clickhouse/data-modelling/sparse-primary-indexes)をご覧ください。

プライマリ索引のスキャンで選択されたデータを ClickHouse が高い並列度でどのように処理するのかに興味がある場合は、[クエリ並列度ガイド](/ja/concepts/core-concepts/query-parallelism)をご覧ください。