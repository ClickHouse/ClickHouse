...
<Steps>
  ...
  <Step title="データを追加" id="3-add-data">
    ...
    * ファイル URL からデータをアップロードする
    ...
    [ClickPipes](/ja/integrations/clickpipes/home) は、多様なソースからのデータ取り込みを数回のクリックだけで実現するマネージドインテグレーションプラットフォームです。最も要求の厳しいワークロードを想定して設計されており、ClickPipes の堅牢かつスケーラブルなアーキテクチャにより、一貫したパフォーマンスと信頼性が確保されます。ClickPipes は、長期的なストリーミング用途にも、一回限りのデータロードジョブにも利用できます。
    ...
    * ClickHouseの主キーは、テーブル内の各行&#x3067;***一意ではありません***
    ...
    ClickHouse テーブルの主キーは、データがディスクに書き込まれる際のソート順を決定します。8,192 行または 10MB のデータごとに
    (これを **インデックスグラニュラリティ** と呼びます)、主キーインデックスファイルにエントリが 1 つ作成されます。このグラニュラリティという考え方により、
    メモリに容易に収まる**スパースインデックス**が実現され、グラニュールは `SELECT` クエリの実行時に処理される最小単位の
    カラムデータのストライプを表します。

    主キーは `PRIMARY KEY` パラメータで定義できます。`PRIMARY KEY` を指定せずにテーブルを定義した場合、
    `ORDER BY` 句で指定したタプルがキーとなります。`PRIMARY KEY` と `ORDER BY` の両方を指定する場合、主キーはソート順のサブセットでなければなりません。

    主キーはソートキーでもあり、`(user_id, timestamp)` というタプルです。したがって、各カラムファイルに格納されるデータは、まず `user_id`、次に `timestamp` の順にソートされます。
    ...
    [**clickhouse client**](/ja/concepts/features/interfaces/cli) というコマンドラインツールを使用して ClickHouse Cloud サービスに接続することもできます。左メニューの `Connect` をクリックすると、接続情報を確認できます。表示されたダイアログのドロップダウンから `Native` を選択してください:
    ...
    <Image img="/images/_snippets/new_rows_from_csv.webp" size="lg" alt="CSVファイルの新規行" />
