...
<sub><span>*</span> ClickHouse は、<a href="/ja/sql-reference/data-types/int-uint">`UInt8`, `UInt32`, `UInt32` and `UInt64`</a> など、より広い範囲を扱える符号なし整数型もサポートしています。</sub><br />
<sub><span>**</span>ClickHouse の String 型はデフォルトでは長さに制限がありませんが、<a href="/ja/sql-reference/statements/create/table#constraints">制約</a>を使用して特定の長さに制限できます。</sub>
