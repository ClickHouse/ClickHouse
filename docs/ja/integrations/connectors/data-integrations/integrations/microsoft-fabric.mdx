...
Copy &amp; Sync フローを使用して、OneLake テーブルを ClickHouse に取り込みます。

1. ClickHouse アイテムを開き、**Copy &amp; Sync** を選択します
2. ワークスペースから Lakehouse を選択します
3. 高速化するテーブルを選択します
4. データの特定時点のスナップショットのコピーを開始します

<Frame>
  ![ソートキーを使用して新しい宛先テーブルを作成する Copy & Sync の設定](/images/integrations/tools/data-integration/microsoft-fabric/fabric-03-copy-sync-table-selection.webp)
</Frame>

OneLake で検出されたスキーマに基づいて、推論されたデータ型とデフォルト設定を使用する宛先テーブルを作成できます。テーブル定義をより細かく制御したい場合は、あらかじめテーブルを作成し、既存のテーブルを宛先として使用できます。

<Frame>
  ![OneLake ソーステーブルから既存の ClickHouse テーブルへの Copy & Sync カラムマッピング](/images/integrations/tools/data-integration/microsoft-fabric/fabric-04-copy-sync-existing-table.webp)
</Frame>
...
Microsoft Fabric のパブリック プレビュー期間中、OneLake から ClickHouse へのデータのコピーは無料です。Fabric ワークロードが一般提供になると、課金が開始されます。ワークロードの GA に伴い、料金の詳細が公開されます。
...
**ClickHouse を開く**を選択すると、Microsoft ID でサインインした状態で ClickHouse Cloud コンソール 全体が開きます。Fabric ワークロードから ClickHouse にデータをコピーし、任意の SQL ステートメントを実行できますが、管理タスクを実行するには Cloud Console に移動してサービスを管理する必要があります。
...
OneLake からの各 Copy &amp; Sync は ClickPipe として実行されます。[ClickHouse Cloud コンソールの ClickPipes ページ](/ja/integrations/clickpipes/monitoring)で、コピーの進行状況、スループット、エラーを確認できます。
...
2. **Copy &amp; Sync** を開き、Lakehouse を選択して `publicholidays` テーブルを選択します
...
* **Lakehouse テーブルのみ。** Copy &amp; Sync は、同じワークスペース内の Lakehouse から Delta テーブルを読み取ります。その他の Fabric アイテム (Warehouse、KQL データベース、ミラーリングされたデータベース) は、ソースとしてまだサポートされていません。
...
* **ロールの変更は次回開いたときに反映されます。** ClickHouse アイテムを開くと、Fabric ワークスペースのロールが ClickHouse 組織のロールにマッピングされます。Fabric はロールの変更を ClickHouse に通知できないため、ワークスペースのロールを変更してもすぐには反映されず、次回アイテムを開いたときに有効になります。
