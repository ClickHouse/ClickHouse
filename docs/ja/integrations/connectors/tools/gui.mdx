...
<Accordion title="HouseOps">
  ...
  * 構文ハイライト対応のクエリビルダー。応答をテーブルまたは JSON ビューで表示できます。
...
<Accordion title="LightHouse">
  [LightHouse](https://github.com/VKCOM/lighthouse) は、ClickHouse 向けの軽量な Web インターフェースです。
...
<Accordion title="Redash">
  [Redash](https://github.com/getredash/redash) は、データ可視化プラットフォームです。

  ClickHouse を含む複数のデータソースをサポートしており、異なるデータソースからのクエリ結果を1つの最終データセットに結合できます。
...
<Accordion title="Grafana">
  ...
  「Grafana では、保存場所を問わずメトリクスのクエリ、可視化、アラート設定、分析を行えます。チームでダッシュボードを作成、探索、共有し、データドリブンな文化を育みましょう。コミュニティから信頼され、愛されています」 — grafana.com。
...
<Accordion title="ClickHouse テーブルグラフ">
  [ClickHouse table graph](https://github.com/mbaksheev/clickhouse-table-graph) は、ClickHouse テーブル間の依存関係を可視化するシンプルな CLI ツールです。このツールは `system.tables` テーブルからテーブル間の関係を取得し、[mermaid](https://mermaid.js.org/syntax/flowchart.html) 形式の依存関係フローチャートを生成します。テーブルの依存関係を簡単に可視化して、ClickHouse データベース内のデータフローを把握できます。mermaid により、生成されたフローチャートは見やすく、Markdown ドキュメントにも簡単に追加できます。
</Accordion>

<Accordion title="xeus-clickhouse">
  [xeus-clickhouse](https://github.com/wangfenjin/xeus-clickhouse) は ClickHouse 用の Jupyter カーネルで、Jupyter 内で SQL を使用して ClickHouse データをクエリできます。
</Accordion>

<Accordion title="MindsDB Studio">
  [MindsDB](https://mindsdb.com/) は、ClickHouse を含むデータベース向けのオープンソース AI レイヤーで、最先端の機械学習モデルを簡単に開発、トレーニング、デプロイできます。MindsDB Studio (GUI) では、データベースから新しいモデルをトレーニングし、モデルの予測を解釈し、潜在的なデータバイアスを特定できます。また、Explainable AI 機能を使用してモデルの精度を評価・可視化することで、機械学習モデルをより迅速に適応・調整できます。
</Accordion>
...
<Accordion title="clickhouse-monitoring">
  [clickhouse-monitoring](https://github.com/duyet/clickhouse-monitoring)は、`system.*`テーブルを利用してClickHouseクラスターの監視と概要の把握を支援する、シンプルなNext.jsダッシュボードです。

  機能:

  * クエリ監視: 現在のクエリ、クエリ履歴、クエリリソース (メモリ、読み取ったパーツ、file&#95;open、...) 、最も負荷の高いクエリ、最も使用されているテーブルまたはカラムなど。
  * クラスター監視: 合計メモリ/CPU使用量、分散キュー、グローバル設定、MergeTree設定、メトリクスなど。
  * テーブルおよびパーツ情報: カラムレベルで、サイズ、行数、圧縮、パーツサイズなどの詳細を確認。
  * 便利なツール: ZooKeeperデータの探索、クエリのEXPLAIN、クエリの停止など。
  * メトリクスチャートの可視化: クエリとリソース使用量、マージ/ミューテーション数、マージパフォーマンス、クエリパフォーマンスなど。
</Accordion>

<Accordion title="CKibana">
  [CKibana](https://github.com/TongchengOpenSource/ckibana)は、ネイティブのKibana UIを使用してClickHouseのデータを簡単に検索、探索、可視化できる軽量サービスです。

  機能:

  * ネイティブKibana UIからのチャートリクエストをClickHouseクエリ構文に変換します。
  * クエリパフォーマンスを向上させるため、サンプリングやキャッシュなどの高度な機能をサポートします。
  * ElasticSearchからClickHouseへ移行したユーザーの学習コストを最小限に抑えます。
</Accordion>

<Accordion title="Telescope">
  [Telescope](https://iamtelescope.net/)は、ClickHouseに保存されたログを探索するためのモダンなWebインターフェイスです。きめ細かなアクセス制御により、ログデータのクエリ、可視化、管理を行うための使いやすいUIを提供します。

  機能:

  * 強力なフィルターとカスタマイズ可能なフィールド選択を備えた、すっきりとしたレスポンシブUI。
  * 直感的で表現力豊かなログフィルタリングのためのFlyQL構文。
  * ネストされたJSON、Map、Arrayフィールドを含む、グループ化対応の時系列グラフ。
  * 高度なフィルタリングのためのオプションのraw SQL `WHERE`クエリをサポート (権限チェック付き) 。
  * 保存済みビュー: クエリとレイアウト用のカスタムUI設定を保存して共有。
  * ロールベースのアクセス制御 (RBAC) とGitHub認証のインテグレーション。
  * ClickHouse側で追加のエージェントやコンポーネントは不要です。
...
<Accordion title="ClickLens">
  ...
  * SQL コンソール - 構文ハイライトとストリーミング結果に対応し、クエリの作成、実行、分析を行えます
...
<Accordion title="Chadmin">
  [Chadmin](https://github.com/bun4uk/chadmin) は、ClickHouse クラスターで現在実行中のクエリとその情報を可視化し、必要に応じて停止できるシンプルな UI です。
</Accordion>
