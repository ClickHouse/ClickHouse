---
sidebarTitle: 'dbt Core v2、Fusion、Platform（ベータ）'
slug: /integrations/dbt/dbt-core-v2-fusion-and-platform
description: 'dbt Core v2、dbt Fusion、dbt Platform における ClickHouse アダプターのステータス'
keywords: ['dbt', 'dbt core v2', 'dbt v2', 'dbt fusion', 'dbt platform']
title: 'dbt Core v2、Fusion、Platform'
doc_type: 'reference'
---
...
dbt は新しいエンジン上で再構築されており、ClickHouse は当初からその一部として組み込まれています。新しい Rust エンジン (dbt Core v2 および dbt Fusion) 上で ClickHouse に対して dbt を実行できるようになり、さらに v2 コネクタによって、初めて dbt プラットフォームから ClickHouse へ接続できるようになりました。プロジェクト側に変更は必要ありません。現在 dbt Core 1.x で動作しているモデル、テスト、プロファイルは、そのまま新しいエンジンでも動作します。異なるのはバイナリだけです。

ClickHouse は、dbt v2 および dbt プラットフォームにおける最初のコミュニティアダプターです。dbt 自身のファーストパーティアダプターも同時に提供が開始されますが、dbt Labs 以外から提供されるものとしては ClickHouse が最初です。dbt Core 1.x 向けの Python アダプター `dbt-clickhouse` が廃止されることはありません。dbt 1.x のリリースに追従して最新の状態を保ち、両方のアダプターを並行して保守していきます。

<Warning>
  **本番環境向けではありません。** dbt Core v2 エンジン、その ClickHouse アダプター、および dbt プラットフォーム接続は、まだ generally available (GA) ではありません。開発または staging のプロジェクトでのみ使用してください。
</Warning>

<Note>
  **dbt Core 1.x のドキュメントは dbt Core v2 にも当てはまります。** アダプターの挙動はどちらのエンジンでも同じです。他の dbt ページで説明されている profiles、model の config、materializations、macros は、dbt Core v2、dbt Fusion、dbt プラットフォームでも変更なしにそのまま動作します。例外については、以下の [v1 と v2 の互換性](#parity) の table に記載しています。
</Note>

最新の状況は [ClickHouse/dbt-clickhouse#660](https://github.com/ClickHouse/dbt-clickhouse/issues/660) (dbt Core v2 の互換性、領域ごとにサブ issue あり) および [ClickHouse/dbt-clickhouse#555](https://github.com/ClickHouse/dbt-clickhouse/issues/555) (dbt Fusion) で追跡しています。
...
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **dbt Core 1.x** | Python ベースの dbt エンジンで、open source(Apache 2.0)。ClickHouse が提供するアダプターは `dbt-clickhouse` で、[ClickHouse GitHub 組織](https://github.com/ClickHouse/dbt-clickhouse)でメンテナンスされており、`dbt-core` と併せてインストールします。                                    | **GA、メンテナンス中。** v2 と並列にリリースが継続されています。                                                                                                                                                                                                                                                                                       |
| **dbt Core v2**  | dbt エンジンを Rust で全面的に書き換えたもので、open source(Apache 2.0)。この新バージョンには、v1 のすべての機能に加えていくつかのパフォーマンス改善が含まれます。                                                                                                                                   | **ベータ。** dbt ドキュメントの [dbt v2 upgrade guide](https://docs.getdbt.com/docs/dbt-versions/core-upgrade/upgrading-to-v2) および [ClickHouse setup page for v2](https://docs.getdbt.com/docs/local/connect-data-platform/clickhouse-setup?version=2) を参照してください。ClickHouse アダプターは dbt のバイナリに同梱されているため、個別にインストールする package はありません。 |
| **dbt Fusion**   | dbt v2 に、dbt Labs による追加機能(SQL の解釈、静的解析、LSP、VS Code 拡張機能、カラム認識)を加えたもの。ダイアレクト固有の SQL インテリジェンスはデータベースごとに構築されます。クローズドソースであり、[dbt Product Licensing Agreement](https://www.getdbt.com/dbt-fusion-engine-license-agreement) に基づいてライセンスされます。 | **ベータ。** アダプターは Fusion のバイナリ上で動作しますが、新しい SQL 機能は ClickHouse ではまだ利用できません。                                                                                                                                                                                                                                                |
| **dbt platform** | ホスト型の dbt(旧 dbt Cloud)。Studio IDE、スケジュール実行される job、環境、Catalog、Semantic Layer を提供します。このプラットフォームで動作するのは dbt v2 のみであるため、プラットフォーム上の ClickHouse は v2 アダプターを指します。                                                                            | **Private Beta。** [アクセスをリクエストする方法](#request-access)を参照してください。セットアップについては、dbt ドキュメントの [Connect ClickHouse](https://docs.getdbt.com/docs/platform/connect-data-platform/connect-clickhouse) を参照してください。                                                                                                                        |

## ベータ状態 {#beta-status}

dbt Core v2 は、`dbt-clickhouse` の dbt Core 1.x インテグレーションテストスイートを基準に評価しています。ベータとは、主要な dbt ワークフローと ClickHouse Cloud の機能セットが、文書化された制約の範囲内で動作することを意味します。v1 との完全な互換性とセルフマネージドクラスターのサポートは、その後の対応となります。
...
* **Fusion における ClickHouse SQL インテリジェンス** (ダイアレクトを認識したバリデーション、静的解析、LSP、VS Code 拡張機能でのカラム認識) 。現時点でアダプターは Fusion のバイナリ上で動作しますが、これらの機能はまだ含まれていません。それまでの間、ClickHouse プロジェクトでは静的解析は強制的に `off` となり、エンジンはカラムレベルのリネージを書き出しません。[#736](https://github.com/ClickHouse/dbt-clickhouse/issues/736) で追跡されています。
* dbt プラットフォームにおける、ユーザー名とパスワード以外の**追加の認証方式**。

## v1 と v2 の対応状況 {#parity}

以下の表は、`dbt-clickhouse`(v1)で利用できる機能と、dbt Core v2 アダプターでのステータスを比較したものです。注記がない限り、記載されているすべての機能は v1 でサポートされています。ステータス列は dbt Core v2 についての情報です。未対応の各項目の進捗は [ClickHouse/dbt-clickhouse#660](https://github.com/ClickHouse/dbt-clickhouse/issues/660) で追跡されています。

<Badge color="green">サポート済み</Badge> 現在利用可能 · <Badge color="yellow">一部対応</Badge> 制限付きで利用可能 · <Badge color="red">未対応</Badge> 実装待ち。

### Materializations {#parity-materializations}

| feature                                       | 機能の内容                                          | dbt Core v2                         | Notes                                                                                            |
| --------------------------------------------- | ---------------------------------------------- | ----------------------------------- | ------------------------------------------------------------------------------------------------ |
