...
CH バージョン 26.1 以降では、`allow_experimental_nullable_tuple_type=1` を設定すると、このスキーマがサポートされます ([こちらのドキュメントページ](/ja/reference/settings/session-settings/allow-experimental#allow_experimental_nullable_tuple_type)を参照) 。
