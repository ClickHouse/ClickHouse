...
ClickHouse バージョン 26.9 以降では、このスキーマはデフォルトでサポートされます。バージョン 26.1 から 26.8 では、`allow_experimental_nullable_tuple_type = 1` の設定が必要です ([こちらのドキュメントページ](/ja/reference/settings/session-settings/allow-experimental#allow_experimental_nullable_tuple_type)を参照) 。
