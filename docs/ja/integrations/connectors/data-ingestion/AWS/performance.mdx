...
このような並列な挿入スレッドの数は、[`max_insert_threads`](/ja/reference/settings/session-settings/max-insert#max_insert_threads) 設定で構成できます。デフォルト値は、オープンソースの ClickHouse では `1`、[ClickHouse Cloud](https://clickhouse.com/cloud) では 4 です。
