...
password の代わりに、AWS IAM のロールベースアクセスを使用して ClickPipes ユーザーを認証できます。これには、`rds.iam_auth_for_replication` パラメータを `1` に設定する必要があります。次のようにユーザーを作成し、その後、上記と同じスキーマおよびレプリケーションの権限を付与してください。
...
この方法でClickPipesが認証できるようにするために必要なIAMロールとポリシーの設定手順については、[AWS IAM DB認証 (RDS/Aurora) ](/ja/integrations/clickpipes/postgres/auth)を参照してください。
