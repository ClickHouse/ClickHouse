...
<Info>
  接続情報の入力を始める前に、ファイアウォールルールで ClickPipes IP アドレスを許可リストに追加していることを確認してください。ClickPipes IP アドレスの一覧は[こちら](/ja/integrations/clickpipes/networking/static-ips)で確認できます。
...
[接続を設定するためのセットアップガイド](/ja/integrations/clickpipes/networking/aws-privatelink)を参照してください。
...
<Note>
  ClickPipes が SSH トンネルを確立できるように、SSH 踏み台ホストのファイアウォールルールで [ClickPipes IP アドレス](/ja/integrations/clickpipes/networking/static-ips) を必ず許可してください。
</Note>
...
8. さらに、宛先の [ClickHouse テーブルのパーティション化](/ja/concepts/core-concepts/partitions)を制御するために、カスタムの `PARTITION BY <expr>` 式を指定できます。
