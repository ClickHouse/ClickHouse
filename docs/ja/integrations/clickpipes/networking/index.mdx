...
| [GCP Private Service Connect](/ja/integrations/clickpipes/networking/gcp-psc)  | はい   | GCP でホストされている `AlloyDB` データソース向けです。Global Access を使用したクロスリージョン接続はまだサポートされていません。サポート対象外の GCP リージョンにあるプライベート `AlloyDB` データソースに接続するには、[SSH トンネリング](#ssh-tunneling)を使用してください。 |
...
* [マネージドプライベートエンドポイント](#managed-private-endpoints)を使用できない。たとえば、踏み台サーバー経由でしか到達できないオンプレミス環境の場合。