...
サービスで PrivateLink のマネージドプライベート DNS を使用できない場合 (たとえば、Confluent Cloud のようにプロバイダーが
コンシューマー側で DNS 解決を管理することを求める場合) は、[Custom private DNS](#custom-private-dns)を参照してください。
...
<Info>
  Custom プライベート DNS は**プライベートプレビュー**です。ご利用のサービスで有効にするには、[ClickHouse Support](https://clickhouse.com/support/program) にお問い合わせください。
</Info>
