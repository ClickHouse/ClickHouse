...
| **http&#95;agent**?: http.Agent | https.Agent <br /><Badge color="green" icon="flask">実験的</Badge>   | クライアント用のカスタム HTTP Agent。                     | -      | [HTTP agent ドキュメント](/ja/integrations/language-clients/js#custom-httphttps-agent-experimental-nodejs-only)            |
| **set&#95;basic&#95;auth&#95;header**?: boolean <br /><Badge color="green" icon="flask">実験的</Badge> | Basic 認証の認証情報を含む `Authorization` ヘッダーを設定します。 | `true` | [HTTP agent ドキュメントでのこの設定の使用方法](/ja/integrations/language-clients/js#custom-httphttps-agent-experimental-nodejs-only) |
