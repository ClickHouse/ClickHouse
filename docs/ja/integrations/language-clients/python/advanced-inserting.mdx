...
| FixedString           | bytes                   | string            | 文字列として挿入した場合、余分なバイトはゼロに設定されます                                                                                      |
...
Python では、ナイーブな `datetime` に対して `.timestamp()` を呼び出すと、システムのローカルタイムゾーンとして解釈されます。そのため、挿入は実行環境に依存します。次のいずれかの方法を推奨します。

1. タイムゾーン対応の `datetime` オブジェクトを使用します。
2. プロセスのタイムゾーンが UTC であることを確認します。
3. 想定するタイムゾーンを付与するか、明示的に epoch 整数に変換します。
...
このようなカラムに挿入する際、ClickHouse Connect は Python の値をその `tzinfo` に従って変換します。クエリ時には、`column_tzs` 引数でカラムごとの上書きを指定しない限り、結果にはそのカラムのタイムゾーンが使用されます。`query_tz` 引数では、カラムに宣言されたタイムゾーンは上書きされません。
