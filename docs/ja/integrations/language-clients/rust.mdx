...
* ClickHouse でバッチの insert がアトミックに行われるのは、すべての行が同じパーティションに収まり、かつその数が [`max_insert_block_size`](/ja/reference/settings/session-settings#max_insert_block_size) 未満の場合に限られます。
