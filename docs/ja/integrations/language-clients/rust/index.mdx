...
* ClickHouse がバッチをアトミックに挿入するのは、すべての行が同じパーティションに収まり、その数が [`max_insert_block_size`](/ja/reference/settings/session-settings/max-insert#max_insert_block_size) 未満の場合に限られます。
