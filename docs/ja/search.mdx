...
export const globalTransformSource = (source) => {
  let url = source.url || '';
  ...
  if (isPreview || /clickhouse\.com\/docs(\/|$)/.test(url)) {
    tabs.push('ドキュメント');
  } else if (url.indexOf('github.com') !== -1 && /\/issues(\/|$)/.test(url)) tabs.push('ドキュメント');
  else if (/\/blog(\/|$)/.test(url)) tabs.push('ドキュメント');
  else if (url.indexOf('clickhouse.com') !== -1) tabs.push('ドキュメント');
  return Object.assign({}, source, { tabs, url });
};

export const GLOBAL_SEARCH = {
  transformSource: globalTransformSource,
  tabs: ['ドキュメント'],
};
...
export const GlobalSearch = ({ placeholder = 'Search ClickHouse docs...' }) => {
  ...
  useEffect(() => {
    ...
    const TARGET_ID = 'inkeep-global-search-' + window.__globalSearchSeq;
    el.id = TARGET_ID;
    ...
    const config = {
      baseSettings: {
        ...
        // Use the same corpus rules as the global modal, while combining its
        // categories into the single inline result list used on this page.
        transformSource: GLOBAL_SEARCH.transformSource,
    ...
    const mount = () => {
      ...
      catch (e) { console.log('Inkeep global search failed:', e); }
    };
...
ClickHouse のドキュメントやその他の ClickHouse リソースを横断検索します。

<GlobalSearch placeholder="ClickHouse ドキュメントを検索..." />