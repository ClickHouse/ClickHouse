...
description: 'DataStore の導入ガイド - インストール、pandas からのワンライナーでの移行、基本的な使い方'
keywords: ['chdb', 'datastore', 'quickstart', 'installation', 'pandas', 'migration']
doc_type: 'guide'
---

DataStore を数分で使い始められます。このガイドでは、インストール、pandas からの移行、基本的な使い方を紹介します。
...
DataStore を使い始めるには、インポート文を変更するのが最も簡単です。
...
これで完了です！既存の pandas コードは DataStore を使うようになり、SQL 最適化のメリットを得られます。
...
DataStore は遅延評価を採用しているため、結果が必要になるまで操作は実行されません。
...
* DataStore を作成するための各種 [Factory Methods](/ja/chdb/datastore/factory-methods) について学ぶ
* SQL スタイルの操作については [Query Building](/ja/chdb/datastore/query-building) を参照する
* 文字列や datetime などを扱う [Accessors](/ja/chdb/datastore/accessors) を確認する
* 最適化のヒントについては [Performance Guide](/ja/chdb/guides/pandas-performance) を参照する