...
sidebarTitle: 'C/C++'
slug: /chdb/install/c
description: 'C/C++ で chDB をインストールして使用する方法'
keywords: ['chdb', 'c', 'cpp', 'embedded', 'clickhouse', 'sql', 'olap', 'api']
doc_type: 'guide'
---

chDB は、ClickHouse の機能をアプリケーションに直接組み込むためのネイティブ C/C++ API を提供します。この API は、シンプルなクエリから、永続的な connection や query result のストリーミングといった高度な機能までサポートしています。
...
<Steps>
  ...
  <Step title="ヘッダファイルをインクルード" id="include-headers">
    プロジェクトで chDB のヘッダファイルをインクルードします。
  ...
  <Step title="ライブラリをリンク" id="link-library">
    アプリケーションをコンパイルし、chDB とリンクします。
...
* **Issue とサポート**: Issue は [GitHub リポジトリ](https://github.com/chdb-io/chdb/issues)で報告してください
