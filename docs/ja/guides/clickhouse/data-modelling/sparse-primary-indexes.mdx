...
<Accordion title="プライマリインデックスの内容を確認する">
  ...
  そのためには、まずプライマリインデックスファイルを、稼働中のクラスター内のノードの [user&#95;files&#95;path](/ja/reference/settings/server-settings/settings#user_files_path) にコピーする必要があります。
...
主キーのエントリは、各索引エントリが特定のデータ範囲の開始位置を示すことから、索引マークと呼ばれます。サンプルテーブルでは、具体的には次のようになります。
...
[後で説明するように](#the-primary-index-is-used-for-selecting-granules)、この全体的な順序により、主キーの最初のカラムでクエリをフィルタリングする場合、ClickHouse は最初のキーカラムの索引マークに対して <a href="https://github.com/ClickHouse/ClickHouse/blob/22.3/src/Storages/MergeTree/MergeTreeDataSelectExecutor.cpp#L1452" target="_blank">二分探索アルゴリズムを使用</a>できます。

* URL 索引マーク:

  主キーカラム `UserID` と `URL` のカーディナリティがかなり近いため、一般に、2 番目以降のすべてのキーカラムの索引マークが示せるデータ範囲は、少なくとも現在の グラニュール 内のすべてのテーブル行で直前のキーカラムの値が同じである場合に限られます。<br />
