...
このガイドでは、ClickHouse を基盤とする高速なインプロセス SQL OLAP エンジン [chDB](/ja/products/chdb/index) を使って、Jupyter ノートブックで ClickHouse Cloud 上のデータセットを探索する方法を学びます。
...
また、chDB にこのデータをプロセス内で Pandas のデータフレームとして返させることもできます。これは、データを扱ううえで便利でなじみのある方法です。
...
上のスニペットでは、`chdb.query(query, "DataFrame")` は指定したクエリを実行し、その結果を Pandas のデータフレームとしてターミナルに出力します。
...
その後、結果は `df` という変数にデータフレームとして格納されます。

`df.head` は、返されたデータの先頭数行のみを表示します。

<Image size="md" img="/images/use-cases/AI_ML/jupyter/6.webp" alt="データフレームのプレビュー" />
...
`date` は ClickHouse では `Date` 型ですが、結果として得られるデータフレームでは `uint16` 型になることに注意してください。
chDB は、データフレームを返す際に最も適切な型を自動的に推論します。
...
<Image size="md" img="/images/use-cases/AI_ML/jupyter/7.webp" alt="データフレームのプレビュー" />
...
新しいセルで、ローカルの .csv ファイルから新しいデータフレームを作成するために、次のコマンドを実行します。
