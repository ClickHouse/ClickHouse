...
ClickHouse では、Prometheus メトリクスを [`TimeSeries`](/ja/reference/engines/table-engines/integrations/time-series) テーブルに保存し、PromQL クエリを実行できます。インジェスト、クエリ、可視化の設定については、[Prometheus プロトコルと PromQL](/ja/interfaces/prometheus) を参照してください。