...
または、クライアントが certifi バンドルを直接使用するように設定します。[ClickHouse Connect](/ja/integrations/language-clients/python) は自動的に certifi にフォールバックしないため、`ca_cert` パラメータでバンドルを渡してください。
