...
<div id="storage-formats-full-vs-packed">
  ## ストレージフォーマット: Full と Packed (ClickHouse Cloud)
</div>
...
| **Packed** | すべてのファイルが 1 つのアーカイブファイルにまとめられます | ClickHouse Cloud のみ |

<div id="full-storage">
  ### Full storage
</div>

Full storage では、各 パーツ は複数の個別ファイルで構成され、それぞれがディスク上に個別に保存されます。これはオープンソース版 ClickHouse におけるデフォルトかつ唯一の選択肢です。

<div id="packed-storage">
  ### Packed ストレージ (ClickHouse Cloud のみ)
</div>
...
ClickHouse Cloud では、以下の条件の**いずれか**が true の場合、パーツは Packed ストレージ を使用します。
...
<Info>
  **オープンソース**
  ...
  設定 `min_bytes_for_full_part_storage`、`min_rows_for_full_part_storage`、`min_level_for_full_part_storage` はオープンソース版 ClickHouse にも定義されていますが、Packed ストレージ の実装は ClickHouse Cloud でのみ利用可能なため、これらの設定は効果を持ちません。
</Info>

`min_level_for_full_part_storage` 設定は、特に継続的に挿入が発生するテーブルにおいて、ClickHouse Cloud 環境でパフォーマンスとコストの両方を最適化するために使用できます。
...
Cloud デプロイメントでは `min_level_for_full_part_storage = 2` を設定してください。
これにより、次のことが保証されます。
...
この 2 つの概念は互いに独立しているため、ClickHouse Cloud とオープンソース版 ClickHouse のどちらを使用しているかに応じて、いずれの組み合わせも使用できます。

| 組み合わせ            | 用途                                  |
| ---------------- | ----------------------------------- |
| Wide + Full      | ローカルストレージ上の大きなパーツ (オープンソース版のデフォルト)  |
