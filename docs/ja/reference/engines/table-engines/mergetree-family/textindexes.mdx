...
`LIKE` はさらに、プリプロセッサやポストプロセッサを使わない `splitByNonAlpha` および `array` トークナイザーに対して、辞書スキャンによる評価もサポートします (`use_text_index_like_evaluation_by_dictionary_scan` で有効化) 。
この場合、`%value%` パターンは *direct read (ヒントなし)&#x20;*&#x20;となり、`value%` および `%value` パターンはヒントのままです。[LIKE/ILIKE クエリ](#like-ilike-queries-perf) を参照してください。
...
* `value%` と `%value` は needle を値全体にアンカーしますが、辞書スキャンはトークン単位でしかアンカーできません。そのためスキャンは一致する行の superset を返し、[hint としての direct read](#direct-read) に利用されます。
