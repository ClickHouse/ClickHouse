...
<Note>
  テキスト索引は、[compatibility](/ja/reference/settings/session-settings/compatibility#compatibility) 設定に関係なく、ClickHouse バージョン &gt;= 26.2 であれば使用できます。
</Note>
...
* [Map](/ja/reference/data-types/map)。[`keyValuePairs`](#tokenizer-key-value-pairs) トークナイザーを使用して `Map` カラムに定義するか、[mapKeys](/ja/reference/functions/regular-functions/tuple-map-functions#mapKeys) または [mapValues](/ja/reference/functions/regular-functions/tuple-map-functions#mapValues) 関数を使用してマップのキーまたは値のみに定義します
...
* `splitByRegexp(regexp[, match_tokens])` は、ユーザー定義の正規表現 `regexp` に従って String を分割します。
  `regexp` 引数は必須です。たとえば、`tokenizer = splitByRegexp('[a-zA-Z]+[0-9]')` のように指定します。
  省略可能な `match_tokens` 引数 (デフォルトは `false`、`0`/`1` も指定可能) は、`regexp` が何を表すかを制御します。
  * `match_tokens = false` (デフォルト) の場合、`regexp` は区切り文字を表します。トークンは、連続するマッチの間にある (空でない) テキスト片です (関数 [splitByRegexp](/ja/reference/functions/regular-functions/splitting-merging-functions#splitByRegexp) と同じセマンティクスです) 。
  * `match_tokens = true` の場合、代わりに `regexp` が直接マッチされます。各マッチが生成するトークンは最大 1 つで、それは最初のキャプチャグループ、あるいは `regexp` にキャプチャグループがない場合はマッチ全体です。マッチの外側にあるものはすべて破棄されます。たとえば、`tokenizer = splitByRegexp('tag:(\w+)', true)` は `tag:hello tag:world` から `hello` と `world` を索引付けし、`tag:` というプレフィックスは取り除かれます。マッチングには、ClickHouse のすべての正規表現ベースの関数が使用するのと同じエンジンである [RE2](https://github.com/google/re2) を使用します。使用されるのは常に最初のキャプチャグループのみであるため、それ以降のグループはマッチ対象を制約するだけです。マッチに関与しなかったキャプチャグループ、または空文字列にマッチしたキャプチャグループはトークンを生成しません。また、スキャンは常にキャプチャされた span の後ではなくマッチ全体の後から再開されます (そのため、マッチが重複することはありません) 。
    `match_tokens = true` の場合、テキスト検索関数の search needle は索引自身のトークナイザーでトークン化される点に注意してください。そのため、それ自体が `regexp` にマッチしない単純な String の needle はトークンを生成せず、したがって結果も得られません。テキスト検索関数に Array として渡された needle はトークン化されないため、regexp トークナイザーと併用することが推奨されます。
* `asciiCJK` は、Unicode の単語境界規則を使用して String をトークンに分割します ([Unicode Text Segmentation (UAX #29)](https://unicode.org/reports/tr29/) に類似) 。ASCII の英数字とアンダースコアは、コネクタ (文字に対する ASCII `:`、同種の文字に対する `.` および `'`) を含むトークンを構成します。非 ASCII の Unicode 文字は、[CJK](https://en.wikipedia.org/wiki/CJK_characters) 文字を含め、1 文字のトークンになります。
* `chinese[(granularity)]` は、Dictionary と隠れマルコフモデルを使用して中国語テキストを単語に分割します (アルゴリズムは [jieba](https://github.com/fxsjy/jieba) に従い、埋め込み Dictionary とモデルデータは [cppjieba](https://github.com/yanyiwu/cppjieba) に由来します) 。非 ASCII 文字をすべて 1 文字のトークンとして扱う `asciiCJK` とは異なり、`chinese` は連続する中国語文字を単語にまとめます (たとえば、`北京大学` は 4 つの 1 文字トークンではなく、1 つのトークン `北京大学` になります) 。これにより、中国語テキストに対してより意味のあるトークンが得られ、検索品質が向上します。一般的なテキストや混在テキストには `asciiCJK` を、中国語のみのテキストには `chinese` を使用してください。省略可能な `granularity` 引数には、`'coarse_grained'` (指定しない場合のデフォルト) または `'fine_grained'` を指定します。後者では、たとえば `北京邮电大学` に対して `北京`、`邮电`、`大学` も生成されるよう、重複するサブワードも列挙します。きめ細かいトークン化は、より大きな索引を必要とする代わりに再現率を向上させます。
  `chinese` テキスト索引の検索には、`hasToken` (ASCII の区切り文字でのみ分割) ではなく、[hasAnyTokens](/ja/reference/functions/regular-functions/string-search-functions#hasAnyTokens) / [hasAllTokens](/ja/reference/functions/regular-functions/string-search-functions#hasAllTokens) (needle を `chinese` トークナイザーでトークン化) を使用します。
* `icu(locale)` は、[ICU](https://icu.unicode.org/) ライブラリの Unicode 単語分割 (UAX #29) を使用して String を単語トークンに分割します。単語間に空白を入れない文字体系 (中国語、日本語、タイ語など) では、ICU は Dictionaryベースの分割を適用するため、`asciiCJK` とは異なり、このようなテキストは単一文字ではなく意味のある複数文字の単語に分割されます。ここでの「Dictionary」とは、これらの文字体系用に ICU にバンドルされている単語リストを意味します ([`brkitr/dictionaries`](https://github.com/unicode-org/icu/tree/main/icu4c/source/data/brkitr/dictionaries) を参照) 。ICU はこれらの単語に基づいて最も可能性の高い分割を選択します。
  `locale` はセグメンターに渡される ICU ロケールです。分割は主に文字体系と Dictionary に基づいて行われ、ロケールは ICU のロケール固有の調整を選択します。これは必須パラメータであり、たとえば `tokenizer = icu('ja')` または `tokenizer = icu('zh')` のように指定します。
  使用可能なロケールは `SELECT * FROM system.collations` で一覧表示できます。
* `japanese` は、[MeCab](https://github.com/taku910/mecab) 形態素アナライザを使用して日本語テキストを単語に分割します。CJK 入力に対して単一文字のトークンを出力する `asciiCJK` とは異なり、このトークナイザーは適切な単語分割を行います。実行時にサーバー設定から読み込まれる Dictionary が必要です ([Japanese tokenizer dictionary](#japanese-tokenizer-dictionary) を参照) 。
...
* `array` はトークン化を行いません。つまり、各行の値がトークンになります (関数 [array](/ja/reference/functions/regular-functions/array-functions#array) を参照) 。他のシステムとの互換性のため、`keyword` は `array` のエイリアスとして使用できます。
* `keyValuePairs` は、`Map` カラムのキー・バリューのペアを 1 つのトークンにまとめます。これにより、`WHERE map['key'] = 'value'` のようなキーと値を組み合わせたルックアップを実行しやすくなります ([`keyValuePairs` トークナイザー](#tokenizer-key-value-pairs) を参照) 。

使用可能なすべてのトークナイザーは [system.tokenizers](/ja/reference/system-tables/tokenizers) に一覧表示されています。

### 日本語トークナイザー用のMeCab Dictionary {#japanese-tokenizer-dictionary}
...
* `dictionary_location` は、コンパイル済み MeCab Dictionary のアーカイブの場所です。[IPADIC](https://github.com/taku910/mecab) や [UniDic](https://clrd.ninjal.ac.jp/unidic/) など、任意の公式 Dictionary を使用できます。場所はサポートされるアーカイブ拡張子 (たとえば `.tar.gz`、`.tar.zst`、`.zip`) で終わる必要があります。アーカイブタイプはこの拡張子から検出されるため、このような拡張子のない URL (例: `https://example.com/download`) は拒否されます。サポートされる場所:
  * ローカルの `file://` パス。
...
認識される設定 (`access_key_id`、`secret_access_key`、`region`、`no_sign_request`、`use_environment_credentials`、...) は、ClickHouse の他の箇所で使用される S3 認証設定と同じです。これらの設定が存在することで、`http(s)://` URL は通常のダウンロードではなく、(リクエスト署名を行う) S3 クライアントを介して取得されます。
...
トークナイザーが入力文字列をどのように分割するかを確認するには、[tokens](/ja/reference/functions/regular-functions/splitting-merging-functions#tokens) 関数および [tokensForLikePattern](/ja/reference/functions/regular-functions/splitting-merging-functions#tokensForLikePattern) 関数を使用できます。
...
*非 ASCII 入力の処理。*
テキスト索引は、あらゆる言語および文字セットのテキストデータに対して構築できます。
非 ASCII テキストには、CJK 文字を含む Unicode の単語境界を正しく処理する `asciiCJK` トークナイザーを推奨します。
単語を空白で区切らない言語 (中国語、日本語、タイ語など) では、`icu(locale)` トークナイザーが ICU の辞書ベースの単語分割により、意味のある複数文字の単語トークンを生成します。
日本語の場合、`japanese` トークナイザー (MeCab) はテキストを単一文字ではなく単語に分割するため、一般により良い検索結果が得られます。
中国語の場合、`chinese` トークナイザー (jieba) はテキストを単一文字ではなく単語に分割するため、一般により良い検索結果が得られます。

<a id="preprocessor-argument-optional" />**プリプロセッサ引数 (任意)&#x20;**。プリプロセッサは、トークン化の前に入力文字列に適用される式を指します。
...
3. **ステミング**。各トークンをその語幹に対応付けることで、同じ語根を共有する語形の違いにも一致するようになり、検索の再現率が向上します。
   ...
   ClickHouse には、複数の言語向けの組み込み [stem](/ja/reference/functions/regular-functions/nlp-functions#stem) 関数があります。
   例: `stem(str, 'en')`
4. **大文字小文字の正規化**。たとえば [lower](/ja/reference/functions/regular-functions/string-functions#lower) や [lowerUTF8](/ja/reference/functions/regular-functions/string-functions#lowerUTF8) を使って、トークンを小文字化または大文字化し、大文字小文字を区別しないマッチングを可能にします。
   小文字化および大文字化には、ポストプロセッサではなくプリプロセッサを推奨します。

ポストプロセッサ式は、[String](/ja/reference/data-types/string) 型のトークンを同じ型のトークンに変換します。
また、ポストプロセッサ式は、テキスト索引が定義されているカラムまたは式のみを参照する必要があります。
カラムが `Array(String)` 型の場合でも、ポストプロセッサは通常の `String` 値として個々のトークンを処理します。

非決定論的関数の使用は許可されていません。

ポストプロセッサは、索引構築時に生成される各トークンに適用されます (`array` トークナイザーでは、各配列要素がトークンになります)。クエリ時の動作は関数によって異なります:

* `hasToken`、`hasAllTokens`、`hasAnyTokens`、および `hasPhrase` (サポートされている任意のトークナイザーの場合): ポストプロセッサは検索対象のトークンと検索ニードルの両方に適用され、完全に正規化されたマッチング (たとえば、大文字小文字を区別しない検索) を可能にします。`hasPhrase` では、ポストプロセッサ処理後のトークンが密に配置されるため、ポストプロセッサが除外したトークンによって位置の空白は生じず、そのトークンをまたいでフレーズは引き続き一致します。たとえば、`the` を除外するストップワードポストプロセッサでは、`hasPhrase(col, 'see cat')` はドキュメント `see the cat` に一致します。唯一の例外は `splitByRegexp` 索引に対する `hasPhrase` で、ポストプロセッサをサポートしていません (この組み合わせは例外として拒否されます)。
...
| [hasPhrase](/ja/reference/functions/regular-functions/string-search-functions#hasPhrase)                   | はい                          | `splitByNonAlpha`, `splitByString`, `splitByRegexp`³, `ngrams`, `asciiCJK`, `icu` | はい³        |
| [startsWith](/ja/reference/functions/regular-functions/string-functions#startsWith)                        | はい⁴                         | `splitByNonAlpha`, `ngrams`, `sparseGrams`, `asciiCJK`, `array`⁴                  | はい⁴        |
| [endsWith](/ja/reference/functions/regular-functions/string-functions#endsWith)                            | はい⁴                         | `splitByNonAlpha`, `ngrams`, `sparseGrams`, `asciiCJK`, `array`⁴                  | はい⁴        |
...
`LIKE` はさらに、プリプロセッサやポストプロセッサを使わない `splitByNonAlpha` および `array` トークナイザーに対して、辞書スキャンによる評価もサポートします (`use_text_index_like_evaluation_by_dictionary_scan` で有効化) 。以下の [LIKE/ILIKE クエリ](#like-ilike-queries-perf) のセクションを参照してください。

² `ILIKE` は、辞書スキャンによる評価でのみサポートされます (`use_text_index_like_evaluation_by_dictionary_scan = 1`、`splitByNonAlpha` または `array` トークナイザー) 。
辞書スキャンがサポートしないパターンについて、索引をヒントとして使うフォールバックはありません。設定が無効になっている場合、またはトークナイザーがサポート対象外の場合、`ILIKE` では索引は使用されません。
プリプロセッサがある場合は `lower` または `upper` である必要があり、ポストプロセッサはサポートされません。
さらに、一部の needle は対象外となります。[LIKE/ILIKE クエリ](#like-ilike-queries-perf) を参照してください。

³ `splitByRegexp` テキスト索引上の `hasPhrase` は ポストプロセッサ を**サポートしません**。ポストプロセッサ の row-level な rewrite が空白分割による `splitByNonAlpha` 形式のトークンを前提としているため、この組み合わせは例外で拒否されます。ポストプロセッサ を指定しない場合、`splitByRegexp` は `hasPhrase` で完全にサポートされます。

⁴ `startsWith` と `endsWith` は needle の完全なトークンを検索しますが、needle の開いた側の端にあるトークンは値がそこから続くため不完全です: `startsWith(col, 'ClickHouse is')` はトークン `ClickHouse` を検索しますが、`startsWith(col, 'ClickHouse')` には検索できる完全なトークンがありません。
後者は代わりに辞書スキャン (`use_text_index_like_evaluation_by_dictionary_scan = 1`、`splitByNonAlpha` または `array` トークナイザー、プリプロセッサ および ポストプロセッサ なし) によって評価されます。これは `col LIKE 'ClickHouse%'` が取るパスでもあります。アナライザのパス [optimize&#95;rewrite&#95;like&#95;perfect&#95;affix](/ja/reference/settings/session-settings/optimize-rewrite#optimize_rewrite_like_perfect_affix) がこれを `startsWith` に rewrite するためです。
[LIKE/ILIKE クエリ](#like-ilike-queries-perf) を参照してください。

⁵ *互換性のあるトークナイザー* カラムでは、`Map` カラム向けの特殊なトークナイザーである [`keyValuePairs`](#tokenizer-key-value-pairs) トークナイザーは対象外としています。

**Experimental: フレーズ検索サポートの引数 (任意)**。
...
そのため、`support_phrase_search = 1` を指定して索引を作成するには、MergeTree setting [`allow_experimental_text_index_phrase_search`](/ja/reference/settings/merge-tree-settings/allow-experimental#allow_experimental_text_index_phrase_search) を有効にする必要があります。
...
<Warning>
  ...
  位置情報の保存を有効にするには、MergeTree setting [`allow_experimental_text_index_phrase_search`](/ja/reference/settings/merge-tree-settings/allow-experimental#allow_experimental_text_index_phrase_search) を設定してください。
</Warning>
...
<details markdown="1">
  ...
  * `pfor` - 差分 (delta) 符号化を適用した後、PFor による bit-packing を適用します (いずれも固定サイズのブロック内で実行されます) 。`bitpacking` と似ていますが、ブロックごとのビット幅は合計サイズが最小になるように選択され、収まらない値はパッチ例外として個別に保存されるため、1 つの大きなギャップによってブロック全体が拡大することはなくなります。差分がすべて等しいブロックは定数に縮約されます。

  上記の詳細パラメータは、対応する MergeTree 設定を通じてテーブルレベルで設定することもできます: [`text_index_dictionary_block_size`](/ja/reference/settings/merge-tree-settings/text-index#text_index_dictionary_block_size), [`text_index_dictionary_block_frontcoding_compression`](/ja/reference/settings/merge-tree-settings/text-index#text_index_dictionary_block_frontcoding_compression), [`text_index_posting_list_block_size`](/ja/reference/settings/merge-tree-settings/text-index#text_index_posting_list_block_size), および [`text_index_posting_list_codec`](/ja/reference/settings/merge-tree-settings/text-index#text_index_posting_list_codec)。
...
この巨大な索引粒度により、テキスト索引はパート全体に対して作成されます。
...
#### `LIKE` と `match` {#functions-example-like-match}
...
`hasAnyTokens` および `hasAllTokens` とは異なり、検索語をトークン化しません (入力が単一のトークンであることを前提としています) 。
...
検索フレーズは、索引カラムに設定されているものと同じトークナイザーでトークン化されます。
テキスト索引で ポストプロセッサ を使用している場合、検索フレーズも索引のルックアップ前に正規化されます。
この関数を使用するには、`splitByNonAlpha`、`splitByString`、`splitByRegexp`、`ngrams`、`asciiCJK`、`icu` のいずれかのトークナイザーが必要です。
...
アクセス[operator[]](/ja/reference/operators/index#access-operators)は、テキスト索引と組み合わせて使用することで、キーと値を絞り込めます。
テキスト索引が使用されるのは、`Map` カラムに対して [`keyValuePairs`](#tokenizer-key-value-pairs) トークナイザーを使用している場合、または `mapKeys(map)` もしくは `mapValues(map)` の索引式に対して構築されている場合です。
前者 (`keyValuePairs` トークナイザー) の場合は direct read が使用でき、それ以外の場合はヒント付きの direct read が使用されます。
...
### Array(String) カラムの索引作成 {#text-index-example-array}
...
### Mapカラムの索引付け {#text-index-example-map}
...
解決策は、[Map](/ja/reference/data-types/map) カラムに対するテキスト索引です。
どの索引を作成すべきかは、検索対象によって決まります。すなわち、キーとその値をまとめて検索するのか、キーと値を個別に検索するのかによります。

#### `keyValuePairs` トークナイザーによるキー・バリューの複合ルックアップ {#tokenizer-key-value-pairs}

特定の値を持つ特定のキーを検索する場合は、`Map` カラム自体に `keyValuePairs` トークナイザーを使用します:
...
このトークナイザーは、行の各 `(key, value)` ペアを1つのトークンに結合するため、索引はどの値がどのキーに属するかを把握できます。
後述の `mapKeys` および `mapValues` 索引はキーと値を独立に索引付けするため、このようなクエリには答えられません。ある行のいずれかの属性が `error` という値を持つことは分かっても、それが `level` 属性であったかどうかは分からないのです。

ペアは `key ‖ value ‖ length(key)` というトークンとして格納されます。
末尾の長さによってキーと値の境界が一意に定まるため、どちらにも任意のバイト列を含められます。`key=value` のように区切り文字で連結する場合と異なり、区切り文字を含むキーや値があっても誤った一致は発生しません。
テーブル関数 [mergeTreeTextIndex](/ja/reference/functions/table-functions/mergeTreeTextIndex) は、各トークンをデコードした構成要素を `token_key` および `token_value` カラムで返します。

以下の制約が適用されます:

* 索引は、キーと値が `String` または `LowCardinality(String)` 型である `Map` に対して作成する必要があります。
  `FixedString` は拒否されます。カラムはパディングバイトを格納する一方、検索対象の定数は格納しないため、行を暗黙のうちに取りこぼすことになるからです。
  `Nullable` は拒否されます。エンコーディングが空の値と `NULL` を区別できないためです。
* `preprocessor`、`postprocessor`、`support_phrase_search` の各引数はテーブル作成時に拒否されます。
* 現時点では、マップ要素に対する `=` のみが索引から回答されます。
  `mapContainsKey`、`mapContainsValue`、それらの `*Like` バリアント、`IN` など、その他のマップ検索は総当たりスキャンにフォールバックします。
* キーが存在しない場合、`map['key']` は値型のデフォルト値を返すため、`map['key'] = ''` はキーを含まずトークンも持たない行に対しても true になります。
  このような述語は総当たりスキャンにフォールバックします。
* ある行に同じキーが複数回含まれる場合、`map['key']` は最初に出現した値となり、索引もその出現に一致します。

#### `mapKeys` と `mapValues` を使ったキーと値の検索 {#text-index-map-keys-and-values}
...
### JSONカラムの索引付け {#text-index-example-json}
...
#### JSONAllPaths を使用したパスベースの索引 {#json-indexes-jsonallpaths}
...
テキスト索引は、フレーズ内のすべてのトークンの ポスティングリスト の積集合を求めて候補となる グラニュール を特定することで、フレーズ検索を高速化します。
...
`hasPhrase` は、トークナイザー `splitByNonAlpha`、`splitByString`、`splitByRegexp`、`ngrams`、`asciiCJK`、`icu` とあわせて使用できます。
...
* 設定 [query&#95;plan&#95;direct&#95;read&#95;from&#95;text&#95;index](/ja/reference/settings/session-settings/query-plan#query_plan_direct_read_from_text_index) (デフォルトは true) は、direct read を全体として有効にするかどうかを指定します。
* 設定 [use&#95;skip&#95;indexes&#95;on&#95;data&#95;read](/ja/reference/settings/session-settings/use-skip-indexes#use_skip_indexes_on_data_read) は、ClickHouse バージョン &lt; 26.4 では direct read の前提条件でした。
...
テキスト索引が `keyValuePairs` トークナイザーで定義されている場合、direct read は map の要素に対する `equals` (`map['key'] = 'value'`) でサポートされます。
...
ヒントとしての direct read は、設定 [query&#95;plan&#95;text&#95;index&#95;add&#95;hint](/ja/reference/settings/session-settings/query-plan#query_plan_text_index_add_hint) で制御されます (デフォルトで有効) 。
...
LIKE/ILIKE のクエリパターンが `%<alpha-numeric-characters-without-spaces>%`、`<alpha-numeric-characters-without-spaces>%`、`%<alpha-numeric-characters-without-spaces>` のいずれかであり、テキスト索引のトークナイザーが `splitByNonAlpha` または `array` である場合、ClickHouse は転置索引を活用して LIKE/ILIKE クエリを大幅に高速化します。このとき ClickHouse は、フルテーブルスキャンではなく転置索引の辞書をスキャンして、一致するパターンを見つけます。

辞書スキャンの結果をどう利用するかは、needle がどこにアンカーされているかによって決まります。

* `%value%` は、その行のいずれかのトークンに一致する場合にのみ行に一致します。そのため索引だけでクエリを判定でき、元の述語を除去する [direct read (hint なし) ](#direct-read) となります。
* `value%` と `%value` は needle を値全体にアンカーしますが、辞書スキャンはトークン単位でしかアンカーできません。そのためスキャンは一致する行の superset を返し、[ヒントとしての direct read](#direct-read) に利用されます。

この同じ辞書スキャンは、needle に検索できる完全なトークンが存在しない場合の `startsWith(col, 'value')` と `endsWith(col, 'value')` にも使われます。
アナライザのパスである [optimize&#95;rewrite&#95;like&#95;perfect&#95;affix](/ja/reference/settings/session-settings/optimize-rewrite#optimize_rewrite_like_perfect_affix) (デフォルトで有効) が `col LIKE 'value%'` を `startsWith(col, 'value')` に、`col LIKE '%value'` を `endsWith(col, 'value')` に rewrite するため、実際にはほとんどの `value%` および `%value` パターンがこの経路をたどります。
複数のトークンにまたがる needle は needle の完全なトークンをそのまま使用するため、辞書スキャンは不要です。

特別なケースとして、索引のトークナイザーが `array` の場合は、*あらゆる* パターンがこの最適化の対象となります。アンカー、句読点、`_` の wildcard、`%` で区切られた複数の needle、escape された metacharacter などがすべて含まれます。

この最適化が有効な場合、LIKE/ILIKE クエリはフルテーブルスキャンよりも大幅に高速になります。ただし、パターンが辞書内のほとんどのトークンに一致する場合は、フルテーブルスキャンより性能が低下することがあります。幸い、これを防ぐための fallback の仕組みがあります。

`value%` や `%value` パターンの高速化はグラニュールのスキップによって得られるため、一致する行の配置状況に依存します。すべてのグラニュールに一致行が存在するような needle では何も prune されず、結局は行われる full scan に加えて辞書スキャンのコストまで支払うことになります。そのような workload では [use&#95;text&#95;index&#95;like&#95;evaluation&#95;by&#95;dictionary&#95;scan](/ja/reference/settings/session-settings/use-text#use_text_index_like_evaluation_by_dictionary_scan) を無効にしてください。

この最適化は次の設定で制御されます。

* [use&#95;text&#95;index&#95;like&#95;evaluation&#95;by&#95;dictionary&#95;scan](/ja/reference/settings/session-settings/use-text#use_text_index_like_evaluation_by_dictionary_scan)

fallback の仕組みは 2 つの設定で制御されます。

* [text&#95;index&#95;like&#95;min&#95;pattern&#95;length](/ja/reference/settings/session-settings/text-index#text_index_like_min_pattern_length)
* [text&#95;index&#95;like&#95;max&#95;postings&#95;to&#95;read](/ja/reference/settings/session-settings/text-index#text_index_like_max_postings_to_read)

この最適化がサポートするのは、`like`、`ilike`、`startsWith`、`endsWith` の各関数のみです。
また一般に、プリプロセッサやポストプロセッサを持たない索引が必要です (`ilike` はプリプロセッサ関数として `lower` または `upper` も受け付けます)。

`ilike` の場合、`splitByNonAlpha` トークナイザーと `array` トークナイザーのいずれにおいても、次の 2 つの特殊なケースでは検索がフルテーブルスキャンに falls back します。

* needle に文字 `k` が含まれる場合。`ilike` は `U+212A KELVIN SIGN` を `k` として扱いますが、辞書スキャンはバイト単位で比較するため、その表記の行を取りこぼしてしまいます。`U+212A` は `ilike` が ASCII の英数字に畳み込む唯一の文字であるため、影響を受ける needle 中の文字は `k` のみです。
* 索引のプリプロセッサ expression に `lowerUTF8` または `upperUTF8` が含まれる場合。これらは非 ASCII 文字を ASCII の文字に書き換える (`ß` を `SS` に、`ſ` を `S` に) ため、`ilike` が一致しない行を索引が報告してしまうことになります。

### 単純なカウントクエリ {#count-queries-perf}
...
テキスト索引から応答されるため、一致する行を読み取る必要はありません。
ClickHouse は索引内のポスティングリストのカーディナリティから count を取得するため、一致する行を示す仮想カラムやその集約をスキップします。単一トークンの場合は Dictionary のカーディナリティだけで済むため、ポスティングリストは読み取りません。複数トークンの述語では、ポスティングリストをユニオン (`hasAnyTokens`) または積集合 (`hasAllTokens`) する場合にのみ読み取ります。テキスト索引はパーツ全体をカバーするため、count は正確であり、非常に大きなパーツでも高速です。

この最適化は、単一の `hasToken`、`hasAnyTokens`、または `hasAllTokens` 述語 (または `Array`/`Map` での同等の表現) でフィルタリングされた単独の `count()` に適用されます。`AND`/`OR`/`NOT` で結合した述語、追加のフィルタ (例: `... AND id > 10`) 、`m['key'] = 'value'` のようなヒントモードの述語、count 以外の値の選択、フレーズ検索、または `LIKE`/パターン検索がある場合、クエリは代わりに行を読み取ります。マテリアライズされた索引を持たないパーツも、行を読み取ることで正しくカウントされます。
...
この最適化は、[query&#95;plan&#95;optimize&#95;count&#95;from&#95;text&#95;index](/ja/reference/settings/session-settings/query-plan#query_plan_optimize_count_from_text_index) (デフォルトで有効) で制御されます。[direct read](#direct-read)を前提としているため、[query&#95;plan&#95;direct&#95;read&#95;from&#95;text&#95;index](/ja/reference/settings/session-settings/query-plan#query_plan_direct_read_from_text_index)、[use&#95;skip&#95;indexes](/ja/reference/settings/session-settings/use-skip-indexes#use_skip_indexes)、および[optimize&#95;trivial&#95;count&#95;query](/ja/reference/settings/session-settings/optimize-trivial#optimize_trivial_count_query)も有効にする必要があります (いずれもデフォルトで有効) 。
...
設定 [use&#95;text&#95;index&#95;header&#95;cache](/ja/reference/settings/session-settings/use-text#use_text_index_header_cache)、[use&#95;text&#95;index&#95;tokens&#95;cache](/ja/reference/settings/session-settings/use-text#use_text_index_tokens_cache)、および [use&#95;text&#95;index&#95;postings&#95;cache](/ja/reference/settings/session-settings/use-text#use_text_index_postings_cache) を使用すると、クエリによる個々の cache への読み取りと書き込みを無効にできます。
データパーツに存在しないトークンのキャッシュはデフォルトで有効になっており、[`use_text_index_negative_tokens_cache`](/ja/reference/settings/session-settings/use-text) を使用して個別に制御できます。
...
#### テキスト索引トークンcacheの設定 {#caching-tokens}

| Setting                                                                                                                                                | Description                                    |
| ------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------- |
| [text&#95;index&#95;tokens&#95;cache&#95;policy](/ja/reference/settings/server-settings/settings/text-index#text_index_tokens_cache_policy)               | テキスト索引トークンcacheのcacheポリシー名。                    |
| [text&#95;index&#95;tokens&#95;cache&#95;size](/ja/reference/settings/server-settings/settings/text-index#text_index_tokens_cache_size)                   | cacheの最大サイズ (バイト単位) 。                          |
| [text&#95;index&#95;tokens&#95;cache&#95;max&#95;entries](/ja/reference/settings/server-settings/settings/text-index#text_index_tokens_cache_max_entries) | cache内のデシリアライズ済みトークンの最大数。                      |
| [text&#95;index&#95;tokens&#95;cache&#95;size&#95;ratio](/ja/reference/settings/server-settings/settings/text-index#text_index_tokens_cache_size_ratio)   | cache全体のサイズに対する、テキスト索引トークンcache内の保護キューのサイズの比率。 |

#### ヘッダーcacheの設定 {#caching-header}

| Setting                                                                                                                                                | Description                             |
| ------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------- |
| [text&#95;index&#95;header&#95;cache&#95;policy](/ja/reference/settings/server-settings/settings/text-index#text_index_header_cache_policy)               | テキスト索引ヘッダーcacheのポリシー名。                  |
| [text&#95;index&#95;header&#95;cache&#95;size](/ja/reference/settings/server-settings/settings/text-index#text_index_header_cache_size)                   | cacheの最大サイズ (バイト) 。                     |
| [text&#95;index&#95;header&#95;cache&#95;max&#95;entries](/ja/reference/settings/server-settings/settings/text-index#text_index_header_cache_max_entries) | cache内に保持できるデシリアライズ済みヘッダーの最大数。          |
| [text&#95;index&#95;header&#95;cache&#95;size&#95;ratio](/ja/reference/settings/server-settings/settings/text-index#text_index_header_cache_size_ratio)   | テキスト索引ヘッダーcache全体のサイズに対する、保護キューのサイズの比率。 |

#### ポスティングリスト cache の設定 {#caching-posting-lists}

| Setting                                                                                                                                                    | Description                                                  |
| ---------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------ |
| [text&#95;index&#95;postings&#95;cache&#95;policy](/ja/reference/settings/server-settings/settings/text-index#text_index_postings_cache_policy)               | テキスト索引のポスティング cache ポリシー名。                                   |
| [text&#95;index&#95;postings&#95;cache&#95;size](/ja/reference/settings/server-settings/settings/text-index#text_index_postings_cache_size)                   | cache の最大サイズ (バイト単位) 。                                       |
| [text&#95;index&#95;postings&#95;cache&#95;max&#95;entries](/ja/reference/settings/server-settings/settings/text-index#text_index_postings_cache_max_entries) | cache 内のデシリアライズ済みポスティングの最大数。                                 |
| [text&#95;index&#95;postings&#95;cache&#95;size&#95;ratio](/ja/reference/settings/server-settings/settings/text-index#text_index_postings_cache_size_ratio)   | テキスト索引のポスティング cache における保護キューのサイズを、cache 全体のサイズに対する比率で指定します。 |
...
## アップグレードに関する注意 {#upgrade-notes}

テキスト索引のオンディスクフォーマットのバージョンは、テーブルレベルの設定 [`text_index_serialization_version`](/ja/reference/settings/merge-tree-settings/text-index) (デフォルト: `v2_with_positions`) で制御されます。
この設定は厳密な制約ではなく優先指定です。設定されたバージョンで索引を表現できない場合は、表現可能なより新しいバージョンが自動的に選択されるため、この設定が原因でテキスト索引の書き込みに失敗することはありません。
ローリングアップグレード中は、すでにアップグレード済みのサーバーで [`compatibility`](/ja/reference/settings/session-settings/compatibility#compatibility) 設定を使用してフォーマットを固定します。この設定を対応するフォーマットが導入されたバージョンより古いバージョンにすると、`text_index_serialization_version` は自動的に古い値に戻り、新しいサーバーでも古いサーバーが引き続き読み取れるフォーマットで書き込まれます。
posting list コーデックはこのバージョンの管理対象外です。`posting_list_codec = 'pfor'` (または `text_index_posting_list_codec` 設定) で書き込まれたパーツは、そのコーデックが導入される前のサーバーでは読み取れず、`text_index_serialization_version` も `compatibility` もその使用を抑止できません。これは、`support_phrase_search` の場合と同様に、索引に対して指定されたコーデックがバージョンの優先指定よりも優先されるためです。そのテーブルを読み取る可能性のあるすべてのサーバーをアップグレードし終えるまで、`pfor` を有効にしないでください。

## テキスト索引とブルームフィルタベースの索引の違い {#text-index-vs-bloom-filter-indexes}
...
## 例: Hacker News データセット {#hacker-news-dataset}
