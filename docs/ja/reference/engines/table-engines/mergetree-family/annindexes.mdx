...
ベクトル空間内の点は、[Array(Float64)](/ja/reference/data-types/array)、[Array(Float32)](/ja/reference/data-types/array)、または [Array(BFloat16)](/ja/reference/data-types/array) 型のカラム `vectors` に格納されます。
実用上は、一般的に [BFloat16](/ja/reference/data-types/float) 配列の使用を推奨します。
...
このようなクエリの実行時間は、一般に格納されているベクトルの数、ベクトル要素数 (= 次元) 、およびベクトル要素のビット幅に比例します。
また、ClickHouse はすべてのベクトルを総当たりでスキャンするため、実行時間はクエリで使用されるスレッド数にも依存します ([max&#95;threads](/ja/reference/settings/session-settings/max-threads#max_threads) 設定を参照) 。
...
ベクトル類似度索引は、特殊なデータスキップ索引の一種です ([こちら](/ja/reference/engines/table-engines/mergetree-family/mergetree#table_engine-mergetree-data_skipping-indexes)および[こちら](/ja/concepts/features/performance/skip-indexes/skipping-indexes)を参照) 。
...
<Note>
  ベクトル類似度索引を使用するには、設定 [compatibility](/ja/reference/settings/session-settings/compatibility#compatibility) を `''` (デフォルト値) にするか、`'25.1'` 以降に設定する必要があります。
</Note>
...
上級ユーザーは、設定 [hnsw&#95;candidate&#95;list&#95;size&#95;for&#95;search](/ja/reference/settings/session-settings/other#hnsw_candidate_list_size_for_search) (HNSWハイパーパラメーター &quot;ef&#95;search&quot; とも呼ばれます) にカスタム値を指定することで、検索時の候補リストのサイズを調整できます (例：`SELECT [...] SETTINGS hnsw_candidate_list_size_for_search = <value>`) 。
...
具体的には、`<N>` が設定 [max&#95;limit&#95;for&#95;vector&#95;search&#95;queries](/ja/reference/settings/session-settings/max#max_limit_for_vector_search_queries) の値 (デフォルト値は 100) を超えている場合、エラーが返されます。
...
クエリは10個の近傍を求めており、ベクトル類似度索引はそれら10個の近傍を10個の別々のグラニュールで見つけます。
これら10個のグラニュールはクエリ実行時に読み取られます。

出力に `Skip` およびベクトル索引の名前とタイプ (この例では `idx` と `vector_similarity`) が含まれている場合、ベクトル類似度索引が使用されています。
この場合、ベクトル類似度索引は4つのグラニュールのうち2つ、つまりデータの50%をスキップしました。
スキップできるグラニュールが多いほど、索引の使用効率が向上します。

<Tip>
  索引の使用を強制するには、設定 [force&#95;data&#95;skipping&#95;indices](/ja/reference/settings/session-settings/force#force_data_skipping_indices) を指定して SELECT クエリを実行できます (設定値として索引名を指定します) 。
</Tip>
...
* ポストフィルタリングとは、まずベクトル類似度索引を評価し、その後で ClickHouse が `WHERE` 句で指定された追加のフィルタを評価することを意味します。
* プリフィルタリングとは、フィルタの評価順序がその逆であることを意味します。
...
追加のフィルタ条件を索引 (主キー索引、スキップ索引) で評価できない場合、ClickHouse はポストフィルタリングを適用します。
...
* フィルタ条件によってパーツ内で少なくとも 1 行が除外される場合、ClickHouse はそのパーツ内の &quot;残った&quot; 範囲に対してプリフィルタリングにフォールバックします。
* フィルタ条件によってパーツ内で 1 行も除外されない場合、ClickHouse はそのパーツに対してポストフィルタリングを行います。

実運用のユースケースでは、後者のケースが発生する可能性はかなり低いです。

*追加のフィルタはスキップ索引を使って評価できる*

追加のフィルタ条件を [スキップ索引](/ja/reference/engines/table-engines/mergetree-family/mergetree#table_engine-mergetree-data_skipping-indexes) (minmax 索引、set 索引など) で評価できる場合、ClickHouse はポストフィルタリングを行います。
このような場合、ベクトル類似度索引は他のスキップ索引と比べて最も多くの行を除外すると見込まれるため、最初に評価されます。

ポストフィルタリングとプリフィルタリングをより細かく制御するには、2 つの設定を使用できます。

設定 [vector&#95;search&#95;filter&#95;strategy](/ja/reference/settings/session-settings/vector-search#vector_search_filter_strategy) (デフォルト: 上記のヒューリスティックを実装する `auto`) は `prefilter` に設定できます。
...
上記の問題を解決する別の方法として、[vector&#95;search&#95;index&#95;fetch&#95;multiplier](/ja/reference/settings/session-settings/vector-search#vector_search_index_fetch_multiplier) (デフォルト: `1.0`、最大: `1000.0`) を `1.0` より大きい値 (たとえば `2.0`) に設定することもできます。
...
ClickHouse は各パーツのベクトル索引から 3.0 x 10 = 30 件の最近傍を取得し、その後で追加のフィルタ条件を適用します。
...
この最適化はデフォルトで有効になっています。設定 [vector&#95;search&#95;with&#95;rescoring](/ja/reference/settings/session-settings/vector-search#vector_search_with_rescoring) を参照してください。
...
その結果、[圧縮](/ja/reference/statements/create/table/codec) を有効にすると、ベクトルカラムへの書き込みや読み取りが遅くなります。
...
まず、索引作成は並列化できます。
索引作成スレッドの最大数は、サーバー設定[max&#95;build&#95;vector&#95;similarity&#95;index&#95;thread&#95;pool&#95;size](/ja/reference/settings/server-settings/settings/max#max_build_vector_similarity_index_thread_pool_size)で設定できます。
最適な性能を得るには、この設定値を CPU コア数に合わせて設定してください。

次に、INSERT ステートメントを高速化するために、ユーザーはセッション設定[materialize&#95;skip&#95;indexes&#95;on&#95;insert](/ja/reference/settings/session-settings/materialize#materialize_skip_indexes_on_insert)を使用して、新たに挿入されたパーツでのスキップ索引の作成を無効にできます。
そのようなパーツに対する SELECT クエリは、厳密検索にフォールバックします。
挿入されたパーツはテーブル全体のサイズと比べて小さい傾向があるため、性能への影響はごくわずかであると見込まれます。

3 つ目に、マージを高速化するために、ユーザーはセッション設定[materialize&#95;skip&#95;indexes&#95;on&#95;merge](/ja/reference/settings/merge-tree-settings/materialize#materialize_skip_indexes_on_merge)を使用して、マージ後のパーツでのスキップ索引の作成を無効にできます。
...
キャッシュサイズの最大サイズは、サーバー設定[vector&#95;similarity&#95;index&#95;cache&#95;size](/ja/reference/settings/server-settings/settings/vector-similarity#vector_similarity_index_cache_size)で設定できます。
デフォルトでは、cache は最大 5 GB まで拡張できます。

次のログメッセージ (`system.text_log`) は、ベクトル類似度索引が読み込まれていることを示します。
異なるベクトル検索クエリに対してこのようなメッセージが繰り返し表示される場合は、キャッシュサイズが小さすぎることを示しています。
...
この場合は、精度を向上させるために、rescoring ([vector&#95;search&#95;index&#95;fetch&#95;multiplier](/ja/reference/settings/session-settings/vector-search#vector_search_index_fetch_multiplier)、[vector&#95;search&#95;with&#95;rescoring](/ja/reference/settings/session-settings/vector-search#vector_search_with_rescoring)) も有効にすることを推奨します。
...
`Quantized` コーデックを有効にするには、まず `SET allow_experimental_codecs = 1` を設定してください。
...
[ベクトル類似度索引](#vector-similarity-index)は、グラフを走査して最近傍クエリに応答します。グラフ全体をメモリに保持できる場合、非常に高い性能を発揮します。
ただし、次の2つの条件により適用範囲が制限されます。

* **構築コストが高い。** ベクトル類似度索引の構築には多大なコストがかかります。
* **メモリ使用量が多い。** ベクトル類似度索引は大量のメモリを消費し、ベクトル自体のメモリ使用量に加えて、大きなコスト要因となります。
* **フィルタリング。** 選択性の高い `WHERE` フィルターでは、グラフ走査は有効に機能しません。述語を満たす少数の行に到達できないか、それらを見つけるために不釣り合いに多くの候補を調べる必要があるためです。

網羅スキャンには、これらの制限はいずれもありません。補助データ構造を必要とせず、パーツは連結するだけで自然にマージでき、フィルタリングではスキャン対象の行数が減るだけです。
網羅スキャンの最大の欠点は、読み取る必要があるデータ量が多いことです。フル精度の `Float32` または `BFloat16` で保存されたベクトルをスキャンする場合、ベクトルカラム全体をディスクから読み込む必要があるため、ストレージ I/O が支配的になります。

`Quantized` カラムコーデックは、この欠点を解消します。
各ベクトルを、元のフル精度の値とコンパクトな量子化表現の2通りで保存します。
ベクトル検索クエリでは、まず低コストで SIMD に適した距離関数を用いて量子化コードをスキャンし、最も有望な結果候補のショートリストを作成します。
次に、これらの候補をフル精度ベクトルに対して再ランク付けします。
量子化コードに対する初期スキャンでストレージから読み取るバイト数は、元のベクトルをスキャンする場合より大幅に少なくなります。

このコーデックは、コストの高い処理であるスキャンが、まさに ClickHouse エンジンが得意とする処理であるため、ClickHouse に適しています。

* **ベクトル化。** スキャンカーネルは SIMD 命令を活用し、CPU 使用量を削減します。
* **コアとパーツをまたいだ並列処理。** スキャンは容易に並列化できます。利用可能なすべてのスレッドとテーブルのすべてのパーツにわたって、距離が同時に計算されます。
* **分散。** 分片化されたクラスターでは、処理が複数のマシンに分散されます。各分片が自身のスライスを並列にスキャンし、コーディネーターがショートリストをマージします。
* **追加の構築コストなし。** 量子化コードはベクトルの書き込み時に生成されます。構築、調整、再構築が必要な追加の索引はないため、テーブルはデータが書き込まれるとすぐに検索可能になります。

<div id="quantized-codecs-declaring">
  #### コーデックの適用
</div>

`Array(Float32)` (または `Array(Float64)` / `Array(BFloat16)`) カラムに `Quantized(...)` コーデックを指定します。
...
コーデックは後から `ALTER TABLE` で追加または変更することはできません。

<div id="quantized-codecs-methods">
  #### 量子化方式
</div>

各量子化方式には、サイズと精度の間で異なるトレードオフがあります。`dimensions` 引数はベクトル長です。
...
通常の上位 `k` クエリ形式では、コーデック が自動的に使用されます ([完全検索](#exact-nearest-neighbor-search)を参照) 。
...
`vector_search_use_quantized_codes = 1` を設定すると、オプティマイザはクエリを2フェーズのスキャンに書き換えます。
この設定はデフォルトで無効です。
この設定を使用しない場合、同じクエリは元のベクトルに対する通常の完全スキャンとして実行されます。
すべての手法で距離関数 `cosineDistance` がサポートされます。手法 `int8`、`prefix`、`product` では、距離関数として `L2Distance` もサポートされます。

`vector_search_index_fetch_multiplier` の設定では、クエリの `LIMIT` に対してショートリストに含める候補数を指定します。
乗数を大きくすると、再スコアリングが増える代わりに再現率が向上します。
デフォルトは `1` (オーバーサンプリングなし) です。良好な再現率を得るには、通常これを (たとえば `10` に) 増やす必要があります。
