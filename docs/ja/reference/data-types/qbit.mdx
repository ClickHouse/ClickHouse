...
* `dimension` – 各ベクトルの次元数
* `stride` – 任意。1 つのストリームグループにまとめて格納される次元数です。省略した場合のデフォルトは `dimension` (単一グループ) です。指定する場合、`dimension` は `stride` の倍数である必要があり、また `stride` が `dimension` より小さい場合は、`stride` は 8 の倍数でなければなりません。[Strides](#strides) を参照してください。
...
アクセス可能なサブカラム数は、要素型 (ストライド化されている場合は stride group の数 — [Strides](#strides) を参照) によって異なります。
...
ここでは、4096 個の次元が 1024 個ずつ 4 つのグループに分割されています。サブカラムはグループ優先順に並びます。`BFloat16` (16 個のビットプレーン) の場合、`vec.1` … `vec.16` は最初の stride グループ (次元 1–1024) の 16 個のビットプレーン、`vec.17` … `vec.32` は 2 番目のグループ (次元 1025–2048) に属し、以降も同様です。一般に、`vec.N` は stride グループ `(N-1) / element_size` のビットプレーン `(N-1) % element_size` を読み取ります。

reduced-dimension search を実行するには、転置された距離関数の第 4 引数として、読み取る次元数を渡します (下記参照) 。参照ベクトルには、その数と完全に一致する数の要素が含まれている必要があり、この値は `stride` の倍数でなければなりません。
...
ストライド化された `QBit` の場合、これらの関数はオプションの第4引数 `used_dims` (読み取る先頭の次元数) を受け付け、指定した次元をカバーする stride group のみを読み取ります。
