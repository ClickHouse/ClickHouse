...
| `compression`                    | サポートされる値: `none`, `gzip/gz`, `deflate`, `brotli/br`, `xz/LZMA`, `zstd/zst`, `lz4`, `bz2`, `snappy`。デフォルトでは、ファイル拡張子から圧縮方式を自動判定します (`auto` を設定した場合と同じです) 。`snappy` の場合、ワイヤ形式は [snappy&#95;mode](/ja/reference/settings/session-settings/other#snappy_mode) 設定によって選択されます (デフォルトは `basic`) 。                                                                                                                                                                  |
| `structure`                      | テーブルの構造です。形式: `'column1_name column1_type, column2_name column2_type, ...'`。                                                                                                                                                                                                                                                                                                                                                                           |
| `partition_strategy`             | 任意。サポートされる値: `WILDCARD` または `HIVE`。`WILDCARD` では、パス内に `{_partition_id}` が必要で、これはパーティションキーに置き換えられます。`HIVE` ではワイルドカードは使用できず、パスがテーブルのルートであることを前提とし、ファイル名に Snowflake IDs、拡張子にファイルフォーマットを使用した Hive スタイルのパーティションディレクトリを生成します。明示的な方式を指定しない場合、`{_partition_id}` を含むパスでは `WILDCARD` が使用されます。別の glob を含むパスではパーティション方式は使用されず、`PARTITION BY` は無視されます。glob を含まないパスでは、`file_like_engine_default_partition_strategy` が `HIVE` の場合は `HIVE` を使用し、それ以外ではパーティション方式は使用されません。 |
...
### `storage_account_url` 形式での読み込み {#reading-with-storage-account-url}
...
### `connection_string` 形式を使った読み取り {#reading-with-connection-string}
...
### パーティションを指定して書き込む {#writing-with-partitions}

`{_partition_id}` を含むパスでは、`WILDCARD` パーティション方式が暗黙的に指定されます。別の glob を含むパスではパーティション方式は使用されず、`PARTITION BY` は無視されます。glob を含まないパスでは、`file_like_engine_default_partition_strategy` が `HIVE` の場合は `HIVE` を使用し、それ以外の場合はパーティション方式を使用しません。
...
`WILDCARD`: ファイルパス内の `{_partition_id}` ワイルドカードを実際のパーティションキーに置き換えます。パスに `{_partition_id}` が含まれる場合、デフォルトで選択されます。

`partition_strategy` が設定されていない場合、別の glob を含むパスではパーティション方式は使用されず、`PARTITION BY` は無視されます。glob を含まないパスでは、`file_like_engine_default_partition_strategy` が `HIVE` の場合は `HIVE` を使用し、それ以外の場合はパーティション方式を使用しません。
...
## Shared Access Signatures (SAS) の使用 {#using-shared-access-signatures-sas-sas-tokens}
