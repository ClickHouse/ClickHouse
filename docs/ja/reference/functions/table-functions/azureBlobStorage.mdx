...
| `partition_strategy`             | 任意。サポートされる値: `WILDCARD` または `HIVE`。`WILDCARD` では、パス内に `{_partition_id}` が必要で、これはパーティションキーに置き換えられます。`HIVE` ではワイルドカードは使用できず、パスがテーブルのルートであることを前提とし、ファイル名に Snowflake IDs、拡張子にファイルフォーマットを使用した Hive 形式のパーティションディレクトリを生成します。パスに `{_partition_id}` プレースホルダーが含まれる場合、このようなパスと互換性のある唯一の方式である `WILDCARD` がデフォルトになります。それ以外の場合、デフォルトは `file_like_engine_default_partition_strategy` 設定です (`compatibility` 設定が `26.6` より古い場合は `WILDCARD`、それ以外は `HIVE`)。 |
...
`{_partition_id}` を含むパスでは、`WILDCARD` パーティション方式が暗黙的に指定されます。プレースホルダーを含まないパスでは、`compatibility` が `26.6` 以降 (現在のデフォルトを含む) の場合、デフォルトのパーティション方式は `HIVE` です。`file_like_engine_default_partition_strategy` 設定を参照してください。
...
`WILDCARD`: ファイルパス内の `{_partition_id}` ワイルドカードを実際のパーティションキーに置き換えます。パスに `{_partition_id}` プレースホルダーが含まれる場合 (このようなパスと互換性がある唯一の方式) はデフォルトで選択され、それ以外では `compatibility` 設定が `26.6` より古い場合に選択されます。残りのケースではデフォルトは `HIVE` です (`file_like_engine_default_partition_strategy` 設定を参照) 。
