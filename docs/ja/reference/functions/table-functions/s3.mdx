...
| `storage_class_name`                    | 省略可能なパラメーターです。サポートされる値は `STANDARD`、`REDUCED_REDUNDANCY`、`STANDARD_IA`、`ONEZONE_IA`、`INTELLIGENT_TIERING`、`GLACIER_IR`、`EXPRESS_ONEZONE` です。即時取得が可能な S3 storage classes のみサポートされます (`GLACIER` や `DEEP_ARCHIVE` などのアーカイブクラスはサポートされません) 。[AWS S3 Intelligent Tiering](https://aws.amazon.com/s3/storage-classes/intelligent-tiering/) を指定できます。デフォルトは `STANDARD` です。 |
