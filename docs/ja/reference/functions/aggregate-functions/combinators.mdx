...
このコンビネータは、集約関数の結果を [Nullable](/ja/reference/data-types/nullable) データ型に変換します。集約関数に計算する値がない場合は、[NULL](/ja/reference/settings/formats/input-format#input_format_null_as_default) を返します。
...
* `stop` — `resampling_key` の値に対して必要な全インターバルの終了値。全インターバルには `stop` の値は含まれません `[start, stop)`。`start` より大きい必要があります。部分インターバルを 1 つも含まない範囲の場合は例外をスローします。
