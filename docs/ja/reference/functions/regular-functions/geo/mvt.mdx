...
`MultiPoint`, `LineString`, `MultiLineString`, `Ring`, `Polygon`, `MultiPolygon`) も含まれます。
...
出力ジオメトリの型は入力に応じて決まります。`Point` は `Point` を返します。`MultiPoint` は `MultiPoint` を返します。`LineString` または `MultiLineString` は
