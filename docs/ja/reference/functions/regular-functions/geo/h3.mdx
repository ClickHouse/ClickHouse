...
指定したジオメトリ (ring または (multi-)polygon) に含まれる、指定した解像度の六角形を返します。ジオメトリが六角形の中心を含む場合、その六角形は含まれると見なされます。これは [h3PolygonToCellsWithContainment](#h3polygontocellswithcontainment) の包含モード `0` です。

ジオメトリのすべての頂点は球面上に存在する必要があります。経度は `[-180, 180]`、緯度は `[-90, 90]` 度の範囲内でなければなりません。返されるセルの順序は保証されないため、順序が重要な場合は Array をソートしてください。
...
ジオメトリのすべての頂点は球面上になければなりません。経度は `[-180, 180]`、緯度は `[-90, 90]` 度の範囲です。返されるセルの順序は保証されないため、順序が重要な場合は Array をソートしてください。
...
入力されたH3 インデックスが表す単方向エッジの正確な辺の長さを、メートル単位で返します。
...
* 正確な辺の長さ (メートル単位) の値。[Float64](/ja/reference/data-types/float)
...
入力された h3 index が表す単方向エッジの正確な辺の長さを、キロメートル単位で返します。
...
* キロメートル単位の正確な辺の長さ。[Float64](/ja/reference/data-types/float)
...
入力された H3 インデックス が表す単方向エッジの正確な辺の長さを、ラジアン単位で返します。
...
隣接する 2 つの H3 セルインデックス (始点と宛先) の単方向エッジ H3 インデックスを返します。いずれかの入力が有効な H3 セルでない場合は例外をスローします (`functions_h3_default_if_invalid` 設定で制御されます) 。隣接していない有効なセル (同じセルを 2 回指定した場合を含む) の場合は、`0` を返します。
...
単方向エッジ H3Index から始点の六角形インデックスを返します。入力が有効な有向エッジでない場合は例外をスローします (`functions_h3_default_if_invalid` 設定で制御されます) 。この設定が有効な場合にのみ `0` を返します。
...
* デフォルトでは、無効な有向エッジに対して `INCORRECT_DATA` をスローします。`functions_h3_default_if_invalid = 1` の場合は `0` を返します。
...
単方向エッジの H3Index から、宛先の六角形インデックスを返します。入力が有効な有向エッジでない場合は例外をスローします (`functions_h3_default_if_invalid` 設定で制御されます) 。この設定が有効な場合にのみ `0` を返します。
...
指定された入力が有効な有向エッジでない場合は例外をスローします (`functions_h3_default_if_invalid` 設定で制御されます) 。その設定が有効な場合にのみ `(0,0)` を返します。
...
指定された H3Index から伸びるすべての単方向エッジを返します。入力が H3 セルでない場合は例外をスローします (`functions_h3_default_if_invalid` 設定で制御) 。この設定が有効な場合にのみ、空の Array を返します。
...
* `index` — cell を表す六角形インデックス番号。[UInt64](/ja/reference/data-types/int-uint)

**戻り値**

* 各一方向エッジを表す H3 index の Array。[Array](/ja/reference/data-types/array)([UInt64](/ja/reference/data-types/int-uint))。
* デフォルトでは無効な cell に対して `INCORRECT_DATA` をスローします。`functions_h3_default_if_invalid = 1` の場合は `[]` を返します。
...
単方向エッジを構成する座標を返します。入力が有効な有向エッジでない場合は例外をスローします (`functions_h3_default_if_invalid` 設定で制御されます) 。空の Array を返すのは、その設定が有効な場合のみです。
...
* `index` — 単方向エッジを表す六角形インデックス番号。[UInt64](/ja/reference/data-types/int-uint)。

**戻り値**

* &#39;(lat, lon)&#39; のペアからなる Array。[Array](/ja/reference/data-types/array)([Float64](/ja/reference/data-types/float), [Float64](/ja/reference/data-types/float))。
* デフォルトでは無効な有向エッジに対して `INCORRECT_DATA` をスローします。`functions_h3_default_if_invalid = 1` の場合は `[]` を返します。
