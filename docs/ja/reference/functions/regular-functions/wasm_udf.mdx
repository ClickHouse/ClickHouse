...
## system table 経由での WASM モジュール管理 {#manage-wasm-modules-via-system-table}
...
### クラスター全体にモジュールを配布する {#distribute-a-module-across-a-cluster}
...
## WebAssembly UDFを作成する {#create-a-webassembly-udf}
...
データは WASM メモリ内のバッファを介してやり取りされます。バッファは、データへのポインタとデータサイズを保持する 8 バイトの構造体です (リトルエンディアンの `u32` 値 2 つ) 。バッファはデータ自体へのポインタではなく、この構造体へのポインタであるハンドルとして渡されます。ゲストコードは、これらのバッファを作成および破棄するための 2 つの関数をエクスポートする必要があります。サイズが 0 でない場合、データポインタも 0 以外でなければなりません。正のサイズを宣言しながらリニアメモリのオフセット `0` を指すバッファは有効なバッファではなく、ゲストコードはそのようなバッファを生成してはなりません。ClickHouse はそのようなバッファを拒否し、クエリを `WASM_ERROR` で失敗させます。
...
関数は入力ブロックごとに 1 回呼び出されます。大きなクエリはクエリパイプラインによって複数のブロックに分割されるため、各呼び出しで渡される行数はクエリ全体の行数ではなく、そのブロックのサイズです。

パイプラインのブロックであっても、モジュールのリニアメモリに対して大きすぎる場合があるため、さらに分割されます。デフォルトでは `webassembly_udf_input_split_memory_ratio` によって動的に分割されます。ClickHouse はシリアライズされた行を測定し、入力がゲストのリニアメモリのその割合を超えて増加する前に新しい呼び出しを開始するため、行数はデータによって変動します。`webassembly_udf_max_input_block_size` に 0 以外の値を指定すると、これは呼び出しごとの固定された行数上限に置き換えられ、比率を `0` にするとブロック全体が 1 回の呼び出しで渡されます。呼び出しが 1 行未満になることはないため、単独でモジュールのメモリに収まらないほど大きな行は、依然としてゲスト内で失敗します。
...
フリースタンディング C による完全な例：`str_reverse` は `serialization_format = 'RowBinary'` を使用して、各入力文字列のバイトを逆順にします。モジュールインスタンスはブロック間で再利用されるため、`clickhouse_destroy_buffer` は実際にメモリを回収する必要があります。ここでは、すべてのバッファが破棄された時点でアロケータがリセットされます。
...
### RustでUDFを開発する際の注意 {#note-for-developing-udfs-in-rust}
...
## モジュールから利用できるホスト API {#host-api-available-to-modules}
...
* `webassembly_udf_max_input_block_size` — すべての ABI において、1 つのブロックで WebAssembly UDF に渡される最大行数です。0 に設定すると、`ABI BUFFERED_V1` 関数の呼び出しサイズは `webassembly_udf_input_split_memory_ratio` によって決定されます。それ以外の ABI では、パイプラインのブロック全体がそのまま渡されます。

* `webassembly_udf_input_split_memory_ratio` — モジュールが開始時に確保すると宣言しているリニアメモリ (開始時にページを持たないモジュールの場合はメモリ上限) のうち、1 回の呼び出しの入力が占有できる割合です。`webassembly_udf_max_input_block_size` が 0 の場合に、`ABI BUFFERED_V1` 関数のブロックを分割するために使用されます。それ以外の ABI はシリアライズされた入力ブロックを持たないため、この設定は無視されます。デフォルトは `0.5` で、残りのメモリは guest 自身の working set のために残されます。1 に近い比率にすると、guest のアロケータ内で失敗する可能性があります。パイプラインのブロック全体を 1 回の呼び出しで渡すには、0 に設定するか、`互換性設定` を 26.9 より前のバージョンに設定します。値は 0 以上 1 以下である必要があります。
