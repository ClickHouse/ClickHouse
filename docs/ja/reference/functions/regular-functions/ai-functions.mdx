...
### 転送のセキュリティ (HTTP と HTTPS) {#transport-security}
...
認証情報 (プロバイダー、モデル、エンドポイント、および必要に応じて API key を指定する名前付きコレクション)
...
パラメータ で設定可能) に置き換えられます。`categories` Array は、マスキングする PII の種類を制限します。空のArrayを指定した場合は、
