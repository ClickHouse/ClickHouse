...
### 条件式の結果をそのまま使用する {#using-conditional-results-directly}
...
<Note title="`NULL` の動作を変更するには、設定 `least_greatest_legacy_null_behavior` を使用します">
...
引数の中で最も大きい値を返します。戻り値の型は、最も広い互換型に昇格されます。[`Any`](/ja/reference/data-types/index)
...
設定 [`short_circuit_function_evaluation`](/ja/reference/settings/session-settings/short-circuit-function-evaluation#short_circuit_function_evaluation) では、短絡評価を使用するかどうかを制御します。
...
* `x1[, x2, ...]` — 比較する単一の値または複数の値。すべての引数は、比較可能な型である必要があります。[`Any`](/ja/reference/data-types/index)

**戻り値**

引数の中で最も小さい値を返します。戻り値の型は、最も広い互換型に昇格されます。[`Any`](/ja/reference/data-types/index)
...
設定 [`short_circuit_function_evaluation`](/ja/reference/settings/session-settings/short-circuit-function-evaluation#short_circuit_function_evaluation) は、
