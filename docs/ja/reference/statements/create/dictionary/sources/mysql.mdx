...
| `ssl_ca`, `ssl_cert`, `ssl_key` | 同じ認証情報をサーバー上のファイルへのパスとして指定します。サーバー設定ファイルで定義された Dictionary でのみ使用できます。以下を参照してください。省略可能です。                                                                              |
...
<Note>
  `ssl_ca`、`ssl_cert`、`ssl_key`は、サーバーが自身の権限で開くファイル名を指定するため、サーバー設定ファイルで定義されたDictionaryでのみ使用できます。`CREATE DICTIONARY`クエリで作成されたDictionaryでは、代わりに`ssl_ca_pem`、`ssl_cert_pem`、`ssl_key_pem`で内容を渡す必要があります。これらの値は、パスワードと同様にログおよび`SHOW`クエリでマスクされます。
</Note>
