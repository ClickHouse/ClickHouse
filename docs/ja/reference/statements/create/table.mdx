...
* [MergeTree](/ja/reference/engines/table-engines/mergetree-family/mergetree) ファミリー。カラム圧縮コーデックと、[compression](/ja/reference/settings/server-settings/settings/other#compression) 設定によるデフォルトの圧縮方式の選択をサポートします。
...
`ZXC[(level)]` — `level` を設定可能な非対称の [`zxc` 圧縮アルゴリズム](https://github.com/hellobertrand/zxc)。指定可能なレベル: [1, 7]。デフォルトレベル: 3。

`ZXC` は、圧縮速度を犠牲にする代わりに、非常に高速な展開を実現し、圧縮率は `LZ4` と `ZSTD` の中間です。一度圧縮して何度も展開する用途に適しており、最新の ARM コアでは最速で展開されます。レベルが高いほど圧縮率は向上しますが、圧縮は遅くなり、展開は引き続き高速です。
...
* `ALP()` または `ALP(AUTO)` (デフォルト)  — 推定される圧縮サイズに基づいて STD を使用し、必要に応じて RD にフォールバックします。
* `ALP(STD)` — 標準の ALP バリアントです。各値を 10 のべき乗を使った正確なスケーリング整数として表現し、得られた整数を Frame-of-Reference とビットパッキングで圧縮します。表現できない値は生の例外として保存されます。10 進数に由来する数値 (たとえば測定値や価格) に最適です。
* `ALP(RD)` — Real Doubles バリアントです。各値のビットパターンを再解釈し、高位部分 (符号 + 指数 + 上位の仮数ビット) と低位部分に分割します。高位部分は Dictionary エンコードされ (最大 8 エントリ) 、低位部分はビットパッキングされます。多くの値で上位ビットが共通している場合に最適です。

<Note>
  このコーデックは Experimental であり、使用するには `SET allow_experimental_codecs = 1` を設定する必要があります。
</Note>
...
`SZ3` または `SZ3(algorithm, error_bound_mode, error_bound)` - Float32、Float64、Array(Float32)、または Array(Float64) 型のカラム向けの、非可逆ながら誤差境界が保証された コーデック ([SZ3 Lossy Compressor](https://szcompressor.org/)) です。Arrayカラムでは、すべての配列の長さが同じ場合に最も高い圧縮効果が得られます (この場合、固定幅ベクトルとして圧縮されます) 。長さの異なる配列もサポートされますが、その場合はフラットな値の数列として圧縮されます。Mapカラムには適用できません。非可逆圧縮によってキーが破損するためです。`algorithm` に指定できる値は `ALGO_LORENZO_REG`、`ALGO_INTERP_LORENZO`、`ALGO_INTERP` です。`error_bound_mode` に指定できる値は `ABS`、`REL`、`PSNR`、`ABS_AND_REL` です。引数 `error_bound` は最大誤差で、型は Float64 です。

<Note>
  この コーデック は実験的機能であり、使用するには `SET allow_experimental_codecs = 1` の設定が必要です。
</Note>
...
これらのコーデックは実際にはデータを圧縮せず、代わりにディスク上のデータを暗号化します。これらを使用できるのは、[encryption](/ja/reference/settings/server-settings/settings/other#encryption) 設定で暗号化キーが指定されている場合のみです。通常、暗号化されたデータを意味のある形で圧縮することはできないため、暗号化コーデックを使うのはコーデックパイプラインの最後に限られます。
