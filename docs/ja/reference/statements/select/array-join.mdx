...
<Note>
  PostgreSQL の `FROM unnest(...)`、`CROSS JOIN UNNEST(...)`、および `LATERAL` はサポートされていません。代わりに `ARRAY JOIN` を使用してください。`unnest` という名前 (バージョン 26.5 以降) は `arrayJoin` (`SELECT unnest(arr)`) の関数呼び出し別名であり、テーブル関数ではありません。
</Note>
...
[短絡関数評価](/ja/reference/settings/session-settings/short-circuit-function-evaluation#short_circuit_function_evaluation) は、`if`、`multiIf`、`and`、`or` などの特定の関数における複雑な式の実行を最適化する機能です。これにより、これらの関数の実行中に、0 による除算のような例外が発生するのを防げます。
