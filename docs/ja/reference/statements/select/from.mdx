...
`FROM` で始まるクエリでは、`SELECT`句は任意です。省略した場合、`SELECT *` が記述されているかのように動作します。
...
この形式は主に、[パイプ演算子](/ja/reference/statements/select/pipe-operators)を使用するクエリで使用されます：
...
`FINAL` を使用する `SELECT` クエリは並列で実行されます。[max&#95;final&#95;threads](/ja/reference/settings/session-settings/max#max_final_threads) 設定は、使用するスレッド数を制限します。
...
`FINAL` は、セッションまたはユーザープロファイルを使用して、クエリ内のすべてのテーブルに対し [FINAL](/ja/reference/settings/session-settings/other#final) 設定で自動的に適用できます。
