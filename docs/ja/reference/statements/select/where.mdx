...
`WHERE` 句に続く式では、多くの場合、[比較](/ja/reference/operators/index#comparison-operators) や [論理演算子](/ja/reference/operators/index#operators-for-working-with-data-sets)、あるいは多数の[通常の関数](/ja/reference/functions/regular-functions/overview)のいずれかが使用されます。
