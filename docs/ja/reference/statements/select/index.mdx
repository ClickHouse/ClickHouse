...
* [PREWHERE 句](/ja/reference/statements/select/prewhere) (適用先の単一テーブルをフィルタリングするため、このリストでの位置にかかわらず `JOIN` より前に実行されます)
...
クエリは、[パイプ演算子](/ja/reference/statements/select/pipe-operators)を使用して、変換を線形に連結する形式で記述することもできます。
...
`GROUP BY`、`ORDER BY`、`LIMIT BY` 句では、位置引数を使用できます。これを有効にするには、[enable&#95;positional&#95;arguments](/ja/reference/settings/session-settings/enable-positional-arguments#enable_positional_arguments) 設定をオンにします。すると、たとえば `ORDER BY 1,2` は、テーブルの行を最初のカラム、次に 2 番目のカラムでソートします。
