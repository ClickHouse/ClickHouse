...
`EXPLAIN ANALYZE` は実際にクエリを実行し、結果の行を破棄したうえで、各ステップに実行時に実際に何が起きたかを注記として付けた、`EXPLAIN PLAN` と同じ実行計画ツリーを出力します。
...
<Note>
  現在のバージョンの `EXPLAIN ANALYZE` は、分散モードで実行されるクエリをサポートしていません。
</Note>
...
* `Time` — 合計時間。プランニング (つまり、プランの作成 + プランの最適化 + パイプラインの構築) フェーズと実行 (パイプラインの実行) フェーズに分かれます。
* `Read` — テーブルから読み取られた行数と非圧縮バイト数、およびスループット。通常のクエリのフッターに &quot;Processed&quot; として表示される数値と同じです。
* `Peak memory` — クエリが使用したピークメモリ。

では、クエリプランに新たに表示される行を見ていきましょう。
...
<Note>
  ClickHouse は、plan step 内のタスク実行だけでなく、plan step 自体の実行も並列化します。`parallelism` メトリクスが反映するのは、このステップの処理だけです。他のステップも同時に実行されることがあるため、この数値から、このステップの並列度をクエリ全体と比較することはできません。
</Note>
