...
圧縮は
[`output_format_arrow_compression_method`](/ja/reference/settings/formats/output-format#output_format_arrow_compression_method)
...
| `output_format_arrow_record_batch_size`                                      | 小さな block をまとめる際の、レコードバッチ あたりの目標行数です。バッファリングによりメモリ使用量が増加し、クエリが完了するまで最初の batch が遅延する可能性があります。`0` を指定すると行数の目標は無効になります。                                                                                                                                                                                                | `0`         |
| `output_format_arrow_record_batch_size_bytes`                                | レコードバッチ あたりに蓄積する block データの目標バイト数です。バッファリングによりメモリ使用量が増加し、クエリが完了するまで最初の batch が遅延する可能性があります。`0` を指定するとバイト数の目標は無効になります。                                                                                                                                                                                               | `0`         |
| `output_format_arrow_string_as_string`                                       | String 型のカラムでは、Binary の代わりに Arrow String type を使用します                                                                                                                                                                                                                                                                     | `1`         |
| `output_format_arrow_unsupported_types`                                      | Arrow に対応する型がない型 (例: `JSON`、`Dynamic`、`QBit`、`AggregateFunction`) の書き込み方法を指定します: `throw`、`text` (行ごとに 1 つの `serializeText` の値を、`String` カラムが使用する Arrow 型で書き込みます) 、`binary` (行ごとに 1 つの `serializeBinary` の値を Arrow `Binary` として書き込みます) 。`AggregateFunction` はテキスト形式が生の aggregate state であるため、`text` モードでも `Binary` になります。 | `binary`    |
| `output_format_arrow_unsupported_types_as_binary`                            | `output_format_arrow_unsupported_types` に置き換えられました: `0` は `throw`、`1` は `binary` を意味します。その設定がデフォルトのままの場合にのみ参照されます。                                                                                                                                                                                                       | `1`         |
