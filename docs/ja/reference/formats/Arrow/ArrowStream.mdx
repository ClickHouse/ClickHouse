...
| `input_format_arrow_use_native_reader`                                       | `Arrow` および `ArrowStream` フォーマットでは、Apache Arrow ライブラリの代わりに ClickHouse ネイティブ reader を使用します。Apache Arrow ライブラリ reader を使用するには `0` に設定します。                             | `1`         |
...
| `output_format_arrow_unsupported_types_as_binary`                            | Arrow に対応する型がない型 (例: `BFloat16`、`AggregateFunction`) を生のバイナリデータとして出力します。false の場合、そのような型では exception が発生します。これはネイティブ writer と Apache Arrow ライブラリ writer の両方に適用されます。 | `1`         |
| `output_format_arrow_use_64_bit_indexes_for_dictionary`                      | Arrow フォーマットの dictionary indexes には常に 64 ビット整数を使用します                                                                                                                | `0`         |
| `output_format_arrow_use_native_writer`                                      | `Arrow` および `ArrowStream` フォーマットでは、Apache Arrow ライブラリの代わりに ClickHouse ネイティブ writer を使用します。Apache Arrow ライブラリ writer を使用するには `0` に設定します。                             | `1`         |
