...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.9"},{"label": "5"},{"label": "ハッシュ結合のペイロードを行優先形式に変換できるようにするための、結合出力行とビルド側行の推定比率の最小値を制御する新しい設定。0 は変換が常に許可されることを意味します。"}]}]} />

ハッシュ結合のペイロードを行優先形式に変換できるようにするための、結合出力行とビルド側行の推定比率の最小値。0 は変換が常に許可されることを意味します。
