...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.10"},{"label": "0"},{"label": "メモリ使用量が指定したバイト単位のしきい値を超えた場合に、`DISTINCT` をディスクにスピルできるようにする新しい設定。`0` の場合は、`max_bytes_ratio_before_external_distinct` のみが適用されます。"}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.10"},{"label": "0.5"},{"label": "利用可能なメモリの指定した比率を超えた場合に、`DISTINCT` の中間データをディスクにスピルできるようにする新しい設定。`0` の場合は、`max_bytes_before_external_distinct` のみが適用されます。"}]}]} />
