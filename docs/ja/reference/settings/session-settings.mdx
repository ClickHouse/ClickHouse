...
パラメータマップで `credentials` が渡されない場合に、`aiEmbed` が使用する名前付きコレクションの名前です。空文字列はデフォルトがないことを意味し、そのような呼び出しでは `credentials` を明示的に指定する必要があります。埋め込み用のエンドポイントとモデルはチャット用とは異なるため、`ai_function_text_default_credentials` とは分けて管理されています。
...
呼び出しのパラメータマップで `credentials` が渡されない場合に、テキスト AI 関数 (`aiGenerate`、`aiClassify`、`aiExtract`、`aiTranslate`) が使用する名前付きコレクションの名前です。空の場合はデフォルトが設定されていないことを意味し、そのような呼び出しでは `credentials` を明示的に渡す必要があります。chat-completions 用の エンドポイント と モデル は embeddings 用のものとは異なるため、これは `ai_function_embedding_default_credentials` とは別に設定します。
...
`b.id` のような複合識別子が、別名 `b` を持つテーブルのカラム `id` を指す場合と、別のカラムの Tuple サブカラム `b.id` を指す場合の両方があり得るときは、別名プレフィックスとしての解釈 (`b` のカラム `id`) を優先します。デフォルトでは、アナライザはサブカラムを優先します。従来のアナライザの名前解決に合わせるには、これを有効にしてください。
...
`互換性設定` 設定を使用すると、ClickHouse は指定された以前の ClickHouse バージョンのデフォルト設定を使用します。

設定がデフォルト以外の値に変更されている場合、それらの設定はそのまま維持されます (`互換性設定` 設定の影響を受けるのは、変更されていない設定のみです) 。
...
<Note>
  ClickHouse Cloud では、サービスレベルのデフォルトの 互換性設定 設定は ClickHouse Cloud Support が設定する必要があります。設定を希望する場合は、[サポートケースを作成](https://clickhouse.cloud/support)してください。
  ただし、互換性設定 設定は、セッションでの `SET compatibility = '22.3'` やクエリでの `SETTINGS compatibility = '22.3'` など、標準的な ClickHouse の設定メカニズムを使用して、ユーザー、ロール、プロファイル、クエリ、またはセッションレベルで上書きできます。
</Note>
...
Replicated* テーブル からデータを受け取る materialized view に対する重複排除チェックを有効または無効にします。
...
有効にすると、ClickHouse は Replicated* テーブル に依存する materialized view 内のブロックの重複排除を実行します。
...
<Note>
  この設定は、破損したバッチにも影響します (異常な server (マシン) の停止や、[Distributed](/ja/reference/engines/table-engines/special/distributed) テーブルエンジン で `fsync_after_insert`/`fsync_directories` を使用していないことが原因で発生する場合があります) 。
</Note>
...
<Note>
  `insert_deduplication_token` はパーティションごとに追跡されるため、1 回の insert で書き込まれる複数のパーティションが同じトークンを持つことがあります。トークンがない場合、デフォルトのコンテンツ checksum (`insert_deduplication_version = new_unified_hash`) は挿入された ブロック 全体に対して計算されるため、insert が 重複排除 されるのは、データ全体が以前の insert (再試行) と一致する場合のみであり、単一のパーティションの行がたまたま別の insert と一致した場合は対象になりません。
</Note>
...
* 0 — 右テーブルに一致する行が複数ある場合、見つかった最初の 1 行のみが結合に使用されます。
* 1 — 右テーブルに一致する行が複数ある場合、見つかった最後の 1 行のみが結合に使用されます。
...
> &quot;単一の INSERT ブロック内のパーティション数が多すぎます (`partitions_count` 個のパーティション、上限は &quot; + toString(max&#95;partitions) + &quot;)。
> 上限は &#39;max&#95;partitions&#95;per&#95;insert&#95;block&#39; 設定によって制御されます。
> 多数のパーティションを使用するのは、よくある誤解です。これにより深刻な
> パフォーマンス低下が発生し、server の起動の遅延、INSERT クエリの低速化、
...
結果サイズをバイト単位 (非圧縮) で制限します。しきい値に達した場合、クエリはデータの block を処理し終えた後に停止しますが、
結果の最後の block は切り詰められないため、結果サイズがしきい値を超えることがあります。
...
[MergeTree](/ja/reference/engines/table-engines/mergetree-family/mergetree) テーブル向けの設定です。クエリ処理時のレイテンシを低減するため、サイズが `min_compress_block_size` 以上の ブロック は、次の データマーク の書き込み時に圧縮されます。デフォルト値は 65,536 です。

非圧縮データが `max_compress_block_size` 未満の場合、実際の ブロック サイズはこの値以上であり、かつ 1 つの データマーク 分のデータ量を下回ることはありません。

例を見てみましょう。テーブル作成時に `index_granularity` が 8192 に設定されていたとします。

UInt32 型のカラム (値あたり 4 バイト) を書き込むとします。8192 行を書き込むと、合計で 32 KB のデータになります。min&#95;compress&#95;block&#95;size = 65,536 なので、圧縮 ブロック は 2 つの データマーク ごとに形成されます。

String 型の URL カラム (値あたり平均 60 バイト) を書き込むとします。8192 行を書き込むと、平均で 500 KB 弱のデータになります。これは 65,536 を超えるため、圧縮 ブロック は データマーク ごとに形成されます。この場合、ディスクから 1 つの データマーク の範囲のデータを読み取る際に、余分なデータが展開されることはありません。
...
実行計画内でフィルターを下位にプッシュダウンする、クエリプランレベルの最適化の有効/無効を切り替えます。
...
<Note>
  これは上級者向けの設定であり、開発者がデバッグ目的でのみ使用すべきものです。今後、後方互換性のない形で変更されたり、削除されたりする可能性があります。
</Note>
...
* &#39;dpsub&#39; - INNER JOIN と非 INNER JOIN の両方をサポートする DPsub アルゴリズムを実装しています。可能なすべての JOIN 順序を考慮して最適なものを見つけますが、多数のテーブルや JOIN 述語を含むクエリでは遅くなる可能性があります。
* &#39;dphyp&#39; - 現在は INNER JOIN に対してのみ DPhyp (Hypergraph Partitioning による動的計画法) アルゴリズムを実装しています。`dpsize` と同じ探索空間を探索しますが、接続された部分グラフのペアだけを列挙するため、クロス積を考慮しない代わりに、スパースな JOIN グラフでは中間 JOIN の生成が少なくなります
  複数のアルゴリズムは、たとえば `dphyp,greedy` のようにカンマ区切りのリストとして指定できます。これらは順番に試行され、あるアルゴリズムでクエリを処理できない場合 (たとえば OUTER JOIN や非連結のコンポーネントが原因の場合) は、フォールバックとして次のものが使用されます。
...
実行計画内で LIMIT を下位に移動する、クエリプランレベルの最適化を切り替えます。
この設定は、[query&#95;plan&#95;enable&#95;optimizations](#query_plan_enable_optimizations) が 1 の場合にのみ有効です。
...
<Note>
  これは上級者向けの設定であり、開発者がデバッグ目的でのみ使用すべきです。この設定は、将来後方互換性のない形で変更されたり、削除されたりする可能性があります。
</Note>
...
<Note>
  これは上級者向けの設定であり、開発者がデバッグ目的でのみ使用してください。将来、後方互換性のない形で変更されたり、削除されたりする可能性があります。
</Note>
...
<Note>
  これは上級者向けの設定であり、開発者がデバッグ目的でのみ使用すべきです。将来的に後方互換性のない形で変更されたり、削除されたりする可能性があります。
</Note>
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "0.2"},{"label": "`text_index_density_threshold` からリネームされました（別名として引き続き使用可能）。遅延ポスティングリスト適用モードで、ポスティングリストの積集合アルゴリズムを選択します。"}]}]} />

遅延ポスティングリスト適用モード (`text_index_posting_list_apply_mode = 'lazy'`) で、積集合アルゴリズムを選択するポスティングリスト密度のしきい値です。
しきい値未満では leapfrog intersection (スパースなポスティングリストに適する) が、しきい値以上では brute-force bitmap intersection (高密度なポスティングリストに適する) が選択されます。
...
&#39;lazy&#39; は、カーソルベースのオンデマンドデコードを使用します (シリアライズされた codec を持つ索引フォーマットが必要です) 。
...
`Distributed` エンジンのテーブルへのバックグラウンド (`distributed_foreground_insert`) INSERT では、ブロックの保存に compact フォーマットを使用します。
