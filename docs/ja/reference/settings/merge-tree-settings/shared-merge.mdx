...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.9"},{"label": "sainte_lague"},{"label": "`compatibility` に関係なくSainte-Lague分散を維持する。"}]}, {"id": "row-2","items": [{"label": "26.8"},{"label": "sainte_lague"},{"label": "デフォルトでSainte-Lague分散を有効にする。"}]}, {"id": "row-3","items": [{"label": "26.7"},{"label": "water_filling"},{"label": "マージコーディネーターがレプリカ間でマージを分散する際に使用するアルゴリズムを制御する新しい設定"}]}]} />
