...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "1"},{"label": "GeoJSONフォーマットが、読み書き時に RFC 7946 のジオメトリ妥当性（線および Ring の最小 Point 数、Ring の閉鎖、空でない複合ジオメトリ）を検証するかどうかを制御する新しい設定"}]}]} />

`GeoJSON` フォーマットが、入出力の両方で RFC 7946 のジオメトリ妥当性を検証するかどうかを制御します。

有効な場合 (デフォルト) 、GeoJSON の shape ルールに違反するジオメトリは拒否されます。たとえば、Point が 2 つ未満の `LineString` (または `MultiLineString` 内の line) 、Point が 4 つ未満の `Polygon` または `MultiPolygon` の Ring、最初と最後の Point が異なる Ring (閉じていない Ring) 、あるいは空の `MultiLineString`、`Polygon`、`MultiPolygon` が該当します。これは読み取り時 (そのような document は拒否される) と書き込み時 (そのような ClickHouse の値は、input format で拒否される document を生成する代わりに拒否される) の両方に適用されます。

無効な場合、これらの shape ルールは適用されません。そのため、このようなジオメトリはそのまま読み取られ、そのまま書き込まれます。結果として、退化したジオメトリでも往復変換できますが、書き込まれた document は有効な GeoJSON ではない可能性があります。

この検証は構造のみを対象としています。つまり、Point 数と Ring が閉じているかどうかを確認します。shape の幾何学的な正しさまでは検査しません。Ring の向き (右手系の規則 / winding order) は強制されず、構造的には有効でも幾何学的には退化したジオメトリも受け入れられます。たとえば、面積が 0 の polygon、自己交差する Ring、または holes が外側の Ring の外にある polygon などです。有限でない座標 (`NaN`、`Inf`) は、この設定に関係なく常に拒否されます。これは、それらを JSON の数値として表現できないためです。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "0"},{"label": "CSV で `Nullable(String)` の欠損値を NULL ではなく空文字列として読み取るための新しい設定。"}]}]} />

CSV で欠損値を `Nullable(String)` としてどのように読み取るかを制御します。欠損値とは、引用符で囲まれていない、カンマの間や前後にある空欄のことです。この設定を有効にすると、`input_format_csv_empty_as_default` の値に関係なく、`Nullable(String)` の欠損値は NULL ではなく空の `String` として解釈されます。
...
String 型のカラムには Binary ではなく Arrow String 型を使用する
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.7"},{"label": "1"},{"label": "新しい設定。output_format_csv_serialize_tuple_into_separate_columns が有効な場合、CSVWithNames\/CSVWithNamesAndTypes のヘッダーでは タプル カラムがそのリーフフィールドにフラット化され、ヘッダーの幅がデータと一致するようになりました。以前の単一名ヘッダーに戻すには false に設定します。"}]}]} />

[output&#95;format&#95;csv&#95;serialize&#95;tuple&#95;into&#95;separate&#95;columns](#output_format_csv_serialize_tuple_into_separate_columns) が有効な場合、`CSVWithNames` と `CSVWithNamesAndTypes` のヘッダー行では、各 タプル カラムがそのリーフフィールド (`t.a`、`t.b` のようなドット区切りの名前と、リーフの型名) にフラット化されるため、ヘッダーのカラム数がデータと同じになります。`CustomSeparated*` では、このフラット化は `format_custom_escaping_rule = 'CSV'` かつ `format_custom_field_delimiter` が `format_csv_delimiter` と同じ単一文字である場合にのみ適用されます。それ以外の場合 (たとえばデフォルトのタブ区切り文字や `format_custom_field_delimiter = '|'` の場合) は、ヘッダーはフラット化されないままとなり、その場合でもデータと一致します。ヘッダーで単一の最上位 `Tuple` 名と型を保持する従来の動作にするには、`0` に設定してください。

注: フラット化されたヘッダーは、`input_format_with_names_use_header = 1` の場合、名前によって `Tuple` に読み戻されません。このようなデータを `Tuple` に読み戻すには、出力時にこの設定を `0` にするか、`input_format_with_names_use_header = 0` を指定して読み込んでください (さらに、`*WithNamesAndTypes` フォーマットの `CSVWithNamesAndTypes` と `CustomSeparatedWithNamesAndTypes` では、`input_format_with_types_use_header = 0` も必要です。そうしないと、フラット化された型の行が単一の最上位 `Tuple` 入力フィールドに対して検証され、拒否されます) 。
...
Parquet 出力フォーマットの圧縮方式。対応しているコーデック: snappy, lz4, brotli, zstd, gzip, none (非圧縮)
