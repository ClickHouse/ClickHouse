...
`color` には、上記のカテゴリ別およびセマンティックのリストにある任意の token (例: `chart-blue` または `chart-success`) を指定できます。任意の `backgroundChart.color` で、同じ token enum を使用してスパークラインの色を上書きできます。認証とベース URL については、[ClickStack API リファレンス](/ja/clickstack/api-reference)を参照してください。
