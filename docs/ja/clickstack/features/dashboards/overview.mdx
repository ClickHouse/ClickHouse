...
外部 API (`POST` / `PUT` `/api/v2/dashboards`) を使用してダッシュボードを作成または更新する場合は、builder の数値タイル config の `color` にパレットトークンを設定します。生の 16 進数値は使用できません。
...
`color` には、上記のカテゴリ別およびセマンティックのリストにある任意の token (例: `chart-blue` または `chart-success`) を指定できます。任意の `backgroundChart.color` で、同じ token enum を使用してスパークラインの色を上書きできます。認証とベース URL については、[ClickStack API リファレンス](/ja/use-cases/observability/clickstack/api-reference)を参照してください。
