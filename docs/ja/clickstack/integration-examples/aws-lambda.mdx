...
<Steps>
  <Step title="適切な Rotel Lambda Extension レイヤーを選択する" id="choose-layer">
  ...
  <Step title="Rotel レイヤーを Lambda 関数に追加する" id="add-layer">
  ...
  <Step title="拡張機能を ClickStack にエクスポートするよう設定する" id="configure-extension">
  ...
  <Step title="インテグレーションをテストする" id="test-integration">
  ...
  <Step title="HyperDX でログを確認する" id="verify-logs">
...
<Steps>
  <Step title="実行ロールから CloudWatch 関連の権限を削除する" id="remove-permissions">
  ...
  <Step title="CloudWatch へのログ出力が無効になっていることを確認する" id="verify-disabled">
...
<Steps>
  <Step title="使用する言語のインストルメンテーションレイヤーを選択する" id="choose-instrumentation">
  ...
  <Step title="両方のレイヤーを関数に追加する" id="add-both-layers">
  ...
  <Step title="自動インストルメンテーションを設定する" id="configure-instrumentation">
