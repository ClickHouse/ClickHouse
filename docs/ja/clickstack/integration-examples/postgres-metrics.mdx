...
<Steps>
  <Step title="監視ユーザーに必要な権限があることを確認する" id="monitoring-permissions">
  ...
  <Step title="カスタム OTel collector 設定を作成する" id="create-custom-config">
  ...
  <Step title="カスタム設定で ClickStack をデプロイする" id="deploy-clickstack">
    カスタム設定をマウントします。
  ...
  <Step title="メトリクス収集を確認する" id="verify-metrics">
...
<Steps>
  ...
  <Step title="メトリクスを ClickStack に読み込む" id="load-metrics">
  ...
  <Step title="HyperDX でメトリクスを確認する" id="verify-metrics-demo">
...
<Steps>
  <Step title="ダッシュボード設定をダウンロードする" id="download">
    <TrackedLink href={'https://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/examples/postgres-metrics-dashboard.json'} download="postgres-metrics-dashboard.json" eventName="docs.postgres_metrics_monitoring.dashboard_download">ダッシュボード設定ファイルをダウンロード</TrackedLink>します。
  </Step>
...
このガイドでは、素早くセットアップできるよう、ClickStack に組み込まれている OpenTelemetry Collector を拡張して利用します。本番環境でデプロイする場合は、独自の OTel Collector を実行し、ClickStack の OTLP エンドポイントにデータを送信することを推奨します。本番環境向けの構成については、[OpenTelemetry データの送信](/ja/clickstack/ingesting-data/opentelemetry) を参照してください。