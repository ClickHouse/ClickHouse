...
<Steps>
  <Step title="Temporal Cloud キーを作成する" id="create-temporal-cloud-key">
  ...
  <Step title="カスタム設定を読み込むように ClickStack を構成する" id="load-custom">
...
<Steps>
  <Step title="ダッシュボード設定をダウンロードする" id="download">
    <TrackedLink href={'https://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/examples/temporal-metrics-dashboard.json'} download="temporal-metrics-dashboard.json" eventName="docs.temporal_metrics_monitoring.dashboard_download">ダッシュボード設定ファイルをダウンロード</TrackedLink>。
  </Step>
...
このガイドでは、迅速にセットアップできるよう、ClickStack に組み込まれた OpenTelemetry Collector を使用します。本番環境にデプロイする場合は、独自の OTel Collector を実行し、ClickStack の OTLP エンドポイントへデータを送信することを推奨します。本番環境向けの構成については、[OpenTelemetry データの送信](/ja/clickstack/ingesting-data/opentelemetry)を参照してください。