...
<Steps>
  <Step title="Kafka のログ設定を確認する" id="verify-kafka">
  ...
  <Step title="Kafka 用のカスタム OTel collector 設定を作成する" id="custom-otel">
...
<Steps>
  <Step title="ダッシュボード設定をダウンロード" id="download">
    <TrackedLink href={'https://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/examples/kafka-logs-dashboard.json'} download="kafka-logs-dashboard.json" eventName="docs.kafka_logs_monitoring.dashboard_download">ダッシュボード設定ファイルをダウンロード</TrackedLink>。
  </Step>
  ...
  <Step title="ダッシュボードが作成され、すべての可視化が事前設定された状態になります" id="created-dashboard">
...
このガイドでは、素早くセットアップできるよう、ClickStack に組み込まれた OpenTelemetry Collector を利用しています。本番環境へのデプロイでは、独自の OTel Collector を実行し、データを ClickStack の OTLP エンドポイントに送信することを推奨します。本番環境向けの構成については、[OpenTelemetry データの送信](/ja/clickstack/ingesting-data/opentelemetry) を参照してください。