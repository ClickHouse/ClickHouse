...
<Steps>
  <Step title="Redis のログ設定を確認する" id="verify-redis">
...
ClickStack で Redis の監視を開始する際に役立つよう、Redis ログ向けの主要な可視化を用意しています。

<Steps>
  <Step title="ダッシュボード設定をダウンロード" id="download">
    <TrackedLink href={'https://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/examples/redis-logs-dashboard.json'} download="redis-logs-dashboard.json" eventName="docs.redis_logs_monitoring.dashboard_download">ダッシュボード設定ファイルをダウンロード</TrackedLink>。
  </Step>

  <Step title="あらかじめ用意されたダッシュボードをインポート" id="import-dashboard">
...
このガイドでは、ClickStack に組み込まれた OpenTelemetry Collector を使って、すばやくセットアップできるようにしています。本番環境へのデプロイでは、ご自身で OTel Collector を運用し、データを ClickStack の OTLP エンドポイントに送信することを推奨します。本番環境向けの構成については、[OpenTelemetry データの送信](/ja/clickstack/ingesting-data/opentelemetry)を参照してください。