...
<Steps>
  <Step title="PostgreSQL のログ設定" id="configure-postgres">
...
<Steps>
  <Step title="サンプルデータセットをダウンロード" id="download-sample">
  ...
  <Step title="テスト用の collector 設定を作成" id="test-config">
  ...
  <Step title="デモ設定で ClickStack を実行" id="run-demo">
  ...
  <Step title="HyperDX でログを確認" id="verify-demo-logs">
...
<Steps>
  <Step title="ダッシュボード設定をダウンロードする" id="download">
    <TrackedLink href={'https://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/examples/postgres-logs-dashboard.json'} download="postgresql-logs-dashboard.json" eventName="docs.postgres_logs_monitoring.dashboard_download">ダッシュボード設定ファイルをダウンロード</TrackedLink>。
  </Step>
...
このガイドでは、迅速にセットアップできるよう、ClickStack に組み込まれた OpenTelemetry Collector を拡張しています。本番環境でデプロイする場合は、独自の OTel collector を実行し、ClickStack の OTLP エンドポイントにデータを送信することを推奨します。本番環境向けの構成については、[OpenTelemetry データの送信](/ja/clickstack/ingesting-data/opentelemetry) を参照してください。