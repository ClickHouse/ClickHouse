---
slug: /ja/clickstack/deployment/managed
title: 'マネージド'
...
* Warehouses により、[読み取りワークロードと書き込みワークロードを個別に分離できる](/ja/clickstack/managing/isolating-read-write)
* 統合された認証
* 自動バックアップ
* セキュリティおよびコンプライアンス機能
* シームレスなアップグレード
  ...
  <Tab title="既存のサービスを使用する">
    ...
    <Steps>
      ...
      <Step title="インジェストを設定する" id="setup-ingestion-existing-service">
        ...
        「Start Ingestion」を選択すると、インジェストソースの選択を求められます。Managed ClickStackは主要なインジェストソースとしてOpenTelemetryおよび[Vector](https://vector.dev/)をサポートしています。ただし、ユーザーは[ClickHouse Cloudがサポートする統合](/ja/integrations/home)を使用して、独自のスキーマでClickHouseに直接データを送信することもできます。
...
### collector を起動する {#start-a-collector-existing}
...
### 既存の collector を設定する {#configure-existing-collectors-existing}
...
### インジェストを開始する (任意) {#start-ingestion-existing}
...
<div id="additional-tasks">
  ## その他のタスク {#additional-tasks}
</div>

### Managed ClickStack へのアクセス権の付与 {#configure-access}
...
<div id="clickstack-read-only-compute">
  ### 読み取り専用コンピュートで ClickStack を使用する {#clickstack-read-only-compute}
</div>
...
<div id="recommended-setup">
  #### 推奨構成 {#recommended-setup}
</div>

読み取り専用コンピュートで ClickStack を実行するには、次の手順に従います。

1. 読み取り専用サービスとして構成された warehouse 内で、ClickHouse Cloud サービスを作成するか、既存のサービスを選択します。
2. ClickHouse Cloud コンソールで、読み取り専用サービスを選択します。
3. 左側のナビゲーションメニューから ClickStack を起動します。

起動後、ClickStack UI は自動的にこの読み取り専用サービスに紐付けられます。

なお、ダッシュボード、保存済み検索、アラート、SOURCES は ClickStack を起動したサービスにスコープされるため、warehouse 内の別のサービスで保存したものはここでは利用できず、作り直す必要があります。標準の OpenTelemetry スキーマを使用する SOURCES は自動検出されるため、そのデータに対する検索はすぐに利用できます。一方、カスタムの SOURCES や手動で構成した SOURCES は自動検出されないため、改めて設定し直す必要があります。

インジェストの向き先、DDL を実行するサービス、アラートの動作といった全体構成については、[「読み取りワークロードと書き込みワークロードの分離」](/ja/clickstack/managing/isolating-read-write)を参照してください。
...
#### OpenTelemetry スキーマを使用する {#using-otel-schemas}
