...
<Steps>
  <Step title="Vector をインストールする" id="install-vector">
  ...
  <Step title="vectorの設定" id="configure-vector">
  ...
  <Step title="Filebeat を設定する" id="configure-filebeat">
...
<Steps>
  <Step title="Vector をインストールして設定する" id="install-configure-vector">
    Filebeat からの移行について記載されている[同じ手順](#install-vector)で、Vector をインストールして設定します。
  </Step>

  <Step title="Elastic Agent を設定する" id="configure-elastic-agent">
