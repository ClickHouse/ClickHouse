...
<Tabs>
  <Tab title="Managed ClickStack">
    ...
    * Warehouses により、[読み取りワークロードと書き込みワークロードを個別に分離](/ja/clickstack/managing/isolating-read-write)可能
    * 統合認証
...
### インジェストの保護 {#secure-ingestion-managed}
...
#### インジェスト用ユーザーを作成する {#create-a-database-ingestion-user-managed}
...
#### オブザーバビリティ ワークロードの分離 {#isolating-workloads}
...
#### 読み取りワークロードと書き込みワークロードの分離 {#isolating-read-write}

    Warehouses を使用すると、ClickStack 自身のインジェストおよびクエリのワークロードを専用のコンピュートに分離し、インジェストは読み書き可能なサービスに向け、ClickStack UI は読み取り専用サービス上で実行することもできます。これは、より大規模なデプロイにおいて推奨される本番トポロジーです。[&quot;読み取りワークロードと書き込みワークロードの分離&quot;](/ja/clickstack/managing/isolating-read-write)を参照してください。
...
#### インジェスト用ユーザーを作成する {#create-a-database-ingestion-user-oss}
