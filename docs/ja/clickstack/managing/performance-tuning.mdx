...
以下で説明する最適化はすべて、標準のClickHouse SQLを使って、[ClickHouse Cloud SQL Console](/ja/integrations/connectors/sql-clients/sql-console)または [clickhouse client](/ja/concepts/features/interfaces/client) から基になるテーブルに直接適用できます。
...
この属性を頻繁にクエリする場合は、トップレベルのカラムとしてマテリアライズドされる必要があります。
...
ただし、ユーザーが元の属性パス、たとえば `ResourceAttributes.k8s.pod.name:"checkout-675775c4cc-f2p9c"` を使って引き続きクエリする場合でも、**ClickStack はクエリを内部で自動的に書き換え**、マテリアライズド `PodName` カラムを使用します。つまり、次の述語が使われます:
...
を検出し、サーバーが以下の対応するしきい値を満たす場合にのみ
書き換え後の形式を出力します。古いバージョンでは、自動的に元の
Map 添字形式にフォールバックします。

<Tabs>
  <Tab title="ClickHouse Cloud">
    ClickHouse Cloud のビルドとオープンソースリリースでは、バージョン系統が独立しています。
    以下の Cloud の最小バージョンを使用してください。

    | ClickHouse ブランチ | 最小 Cloud バージョン |
    | --------------- | -------------- |
    | 26.2            | `26.2.1.307`   |
    | 26.3            | `26.3.0.0`     |
    | 26.4            | `26.4.1.1740`  |
    ...
    最小バージョンの `26.3.0.0` はブランチの下限であるため、すべての `26.3` ビルドが
    サポートされています。
  </Tab>

  <Tab title="オープンソース">
    | ClickHouse ブランチ | 最小オープンソースバージョン |
    | --------------- | -------------- |
    | 26.2            | `26.2.19.43`   |
    | 26.3            | `26.3.12.3`    |
    | 26.4            | `26.4.3.37`    |
...
<Steps>
  <Step title="新しいテーブルを作成する" id="create-new-table-with-key">
    ```sql
  ...
  <Step title="テーブルを入れ替えて削除する" id="exhange-and-drop-table">
...
<Steps>
  <Step title="新しいテーブルを作成する" id="create-new-table-with-key-2">
  ...
  <Step title="Merge テーブルを作成する" id="create-merge-table">
  ...
  <Step title="Merge テーブルを読み取るように ClickStack UI を更新する" id="update-clickstack-ui-to-read-from-merge-tree">
  ...
  <Step title="テーブルを入れ替える" id="exchange-the-tables">
...
<Steps>
  <Step title="新しいテーブルを作成する" id="create-new-table-with-key-3">
  ...
  <Step title="テーブルを入れ替える" id="exchange-the-tables-v2">
...
複数のソートキーが必要な場合、PROJECTIONだけが選択肢ではありません。運用上の制約や、ClickStack でクエリをどのようにルーティングしたいかに応じて、次を検討してください。

* OpenTelemetry collector が異なる `ORDER BY` キーを持つ 2 つのテーブルに書き込むように設定し、各テーブルに対して個別の ClickStack ログソースを作成します。
* materialized view をコピーパイプラインとして作成します。つまり、メインテーブルに materialized view をアタッチし、生の行を異なるソートキーを持つセカンダリテーブルに `SELECT` するようにします (非正規化またはルーティングのパターン) 。このターゲットテーブル用のログソースを作成します。例は[こちら](/ja/concepts/features/materialized-views/incremental-materialized-view#filtering-and-transformation)を参照してください。