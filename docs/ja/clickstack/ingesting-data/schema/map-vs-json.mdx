...
<Steps>
  <Step title="OTel collector を停止する" id="stop-the-collector" />

  <Step title="既存のテーブル名を変更し、データソースを更新する" id="rename-existing-tables-sources">
  ...
  <Step title="OTel collector をデプロイする" id="deploy-the-collector">
    `OTEL_AGENT_FEATURE_GATE_ARG` を設定して OTel collector をデプロイします。
  </Step>

  <Step title="JSONスキーマ対応を有効にして HyperDX コンテナーを再起動する" id="restart-the-hyperdx-container">
    ```shell
    export BETA_CH_OTEL_JSON_SCHEMA_ENABLED=true
    ```
  </Step>

  <Step title="新しいデータソースを作成する" id="create-new-data-sources">
