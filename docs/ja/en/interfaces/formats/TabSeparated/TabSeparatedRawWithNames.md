...
description: 'TabSeparatedRawWithNames フォーマットのドキュメント'
input_format: true
...
[`TabSeparatedWithNames`](./TabSeparatedWithNames.md) フォーマットとの違いは、
行がエスケープされずに書き込まれる点です。

:::note
このフォーマットでパースする場合、各フィールドにはタブ文字や改行を含めることはできません。
:::
...
`TabSeparatedRawWithNames` フォーマットを使用してデータを読み込みます：
...
出力は、1行のヘッダー付きのタブ区切りフォーマットになります。
