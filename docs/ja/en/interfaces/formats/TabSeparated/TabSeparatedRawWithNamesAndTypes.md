...
description: 'TabSeparatedRawWithNamesAndTypesフォーマットのドキュメント'
input_format: true
...
[`TabSeparatedWithNamesAndTypes`](./TabSeparatedWithNamesAndTypes.md) フォーマットとの違いは、
行がエスケープなしで書き込まれる点です。

:::note
このフォーマットをパースする場合、各フィールド内にタブや改行を含めることはできません。
:::
...
出力は、カラム名と型を示す2行のヘッダーを含むタブ区切りフォーマットになります。
