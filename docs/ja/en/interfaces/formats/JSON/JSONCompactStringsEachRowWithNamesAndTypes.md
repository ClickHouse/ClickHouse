...
`JSONCompactEachRow` フォーマットとの違いは、[TabSeparatedWithNamesAndTypes](/ja/interfaces/formats/TabSeparatedRawWithNamesAndTypes) と同様に、カラム名と型を含むヘッダー行も 2 行出力する点です。
...
入力データのカラムは名前に基づいてテーブルのカラムにマッピングされ、設定 [input&#95;format&#95;skip&#95;unknown&#95;fields](/ja/operations/settings/settings-formats.md/#input_format_skip_unknown_fields) が 1 に設定されている場合は、不明な名前のカラムはスキップされます。
