---
alias: []
description: 'JSONAsObjectフォーマットのリファレンス'
input_format: true
...
このフォーマットでは、1 つの JSON オブジェクトは 1 つの [JSON](/ja/sql-reference/data-types/newjson.md) 値として解釈されます。入力に複数の JSON オブジェクト (カンマ区切り) が含まれている場合、それらはそれぞれ別の行として解釈されます。入力データが `[]` で囲まれている場合は、JSON の配列として解釈されます。

このフォーマットを解析できるのは、[JSON](/ja/sql-reference/data-types/newjson.md) 型の単一フィールドを持つテーブルに限られます。残りのカラムは、[`DEFAULT`](/ja/sql-reference/statements/create/table.md/#default) または [`MATERIALIZED`](/ja/sql-reference/statements/create/view#materialized-view) に設定されている必要があります。
