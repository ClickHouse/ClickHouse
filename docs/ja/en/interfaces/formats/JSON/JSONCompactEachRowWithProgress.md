...
このフォーマットは、JSONCompactEachRow のコンパクトな行単位の出力と、ストリーミングされる進行状況
情報を組み合わせたものです。
メタデータ、個々の行、進行状況の更新、
totals、例外を、それぞれ個別の JSON オブジェクトとして出力します。値はネイティブ型のまま表現されます。

主な機能:

* 最初に、カラム名と型を含むメタデータを出力します
* 各行は個別の JSON オブジェクトとして出力され、`"row"` キーに値の配列が含まれます
* クエリ実行中の進行状況更新を含みます (`{"progress":...}` オブジェクトとして) 
* totals と extremes をサポートします
* 値はネイティブ型のまま保持されます (数値は数値、文字列は文字列)
