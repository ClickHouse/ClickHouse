...
description: 'JSONLinesフォーマットのリファレンス'
keywords: ['JSONLines']
...
このフォーマットは、`JSONEachRow`、`JSONLines`、`NDJSON` (Newline Delimited JSON) 、または `JSONL` とも呼ばれます。これらは同じフォーマットの別名であり、入力と出力の両方で同じように使用できます。
...
以下のデータが含まれる `football.json` という名前のJSONファイルを使用します：
...
setting [input&#95;format&#95;skip&#95;unknown&#95;fields](/ja/operations/settings/settings-formats.md/#input_format_skip_unknown_fields) が 1 に設定されている場合、名前が不明なデータのカラムはインポート時にスキップされます。
