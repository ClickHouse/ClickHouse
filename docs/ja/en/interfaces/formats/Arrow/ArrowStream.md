...
| `input_format_arrow_use_native_reader`                                       | Apache Arrow ライブラリの代わりに、`Arrow` および `ArrowStream` フォーマットでネイティブの ClickHouse リーダーを使用します。Apache Arrow ライブラリのリーダーを使用するには `0` に設定します。                 | `1`         |
...
| `output_format_arrow_unsupported_types_as_binary`                            | Arrow に対応する型がない型 (例: `BFloat16`、`AggregateFunction`) を生のバイナリデータとして出力します。false の場合、そのような型では例外が発生します。ネイティブライターと Apache Arrow ライブラリのライターの両方に適用されます。 | `1`         |
| `output_format_arrow_use_64_bit_indexes_for_dictionary`                      | Arrow フォーマットの dictionary indexes には常に 64 ビット整数を使用します。                                                                                            | `0`         |
| `output_format_arrow_use_native_writer`                                      | Apache Arrow ライブラリの代わりに、`Arrow` および `ArrowStream` フォーマットでネイティブの ClickHouse ライターを使用します。Apache Arrow ライブラリのライターを使用するには `0` に設定します。                 | `1`         |
