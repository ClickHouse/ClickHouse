...
| `input_format_arrow_case_insensitive_column_matching`                        | Arrow のカラムと CH カラム を照合する際に、大文字と小文字を区別しません。                                                                                                                 | `0`         |
...
| `input_format_arrow_use_native_reader`                                       | Apache Arrow ライブラリの代わりに、`Arrow` および `ArrowStream` フォーマットでは ClickHouse のネイティブ reader を使用します。Apache Arrow ライブラリの reader を使用するには `0` に設定します。                      | `1`         |
...
| `output_format_arrow_unsupported_types_as_binary`                            | Arrow に対応する型がない型 (例: `BFloat16`、`AggregateFunction`) を、生の binary data として出力します。false の場合、そのような型では例外が発生します。ネイティブ writer と Apache Arrow ライブラリ writer の両方に適用されます。 | `1`         |
| `output_format_arrow_use_64_bit_indexes_for_dictionary`                      | Arrow フォーマットの dictionary indexes には常に 64 ビット整数を使用します                                                                                                           | `0`         |
| `output_format_arrow_use_native_writer`                                      | Apache Arrow ライブラリの代わりに、`Arrow` および `ArrowStream` フォーマットでは ClickHouse のネイティブ writer を使用します。Apache Arrow ライブラリの writer を使用するには `0` に設定します。                      | `1`         |
