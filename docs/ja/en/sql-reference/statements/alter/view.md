...
`ALTER TABLE ... MODIFY QUERY` ステートメントを使用すると、[materialized view](/ja/sql-reference/statements/create/view#materialized-view) の作成時に指定した `SELECT` クエリを、インジェスト処理を中断することなく変更できます。

このコマンドは、`TO [db.]name` 句で作成された materialized view を変更するためのものです。これは基盤となるストレージテーブルの構造も、materialized view のカラム定義も変更しません。そのため、`TO [db.]name` 句を使わずに作成された materialized view に対しては、このコマンドの適用範囲は非常に限定されます。
...
新しいカラムを追加しない限り変更できるのは `SELECT` セクションだけなので、この方法でできることはかなり限られています。
...
`ALTER TABLE ... MODIFY REFRESH` は、スケジュール、依存関係、ランダム化、[リフレッシュ設定](../create/view.md#refresh-settings) など、[リフレッシャブルmaterialized view](../create/view.md#refreshable-materialized-view) のリフレッシュパラメーターを変更します。

```sql
ALTER TABLE [db.]name MODIFY REFRESH EVERY|AFTER ... [RANDOMIZE FOR ...] [DEPENDS ON ...] [SETTINGS ...]
```

スケジュール (`EVERY` または `AFTER`) の指定は必須です。このステートメントでは、*すべて* のリフレッシュ パラメータが一度に置き換えられます。`RANDOMIZE FOR`、`DEPENDS ON`、`SETTINGS` のいずれも、指定しなかった句は削除されるか、デフォルトにリセットされます。リフレッシュ設定のみを変更する場合は、現在のスケジュールも再度指定してください。
...
* materialized view では、`ALTER TABLE ... MODIFY SETTING` はサポートされていません。リフレッシュ設定は `MODIFY REFRESH` でのみ変更できます。
* `APPEND` の追加または削除はサポートされていません。
* `all_replicas` リフレッシュ設定は、ビューの作成後は変更できません。

リフレッシュ設定の一覧は、[リフレッシュ設定](../create/view.md#refresh-settings) に記載されています。現在適用されている設定を含むリフレッシュのステータスは、[`system.view_refreshes`](../../../operations/system-tables/view_refreshes.md) で確認できます。