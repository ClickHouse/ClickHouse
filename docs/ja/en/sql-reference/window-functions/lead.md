...
順序付けされたフレーム内で、現在の行から offset 行後の位置で評価された値を返します。
この関数は [`leadInFrame`](./leadInFrame.md) に似ていますが、常に `ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING` フレームを使用します。
...
* `default` — 算出された行がウィンドウフレームの境界を超えた場合に返される値。 (省略可能。省略時はカラム型のデフォルト値) 。
