...
順序付けされたフレーム内で、現在の行より前にある、指定した物理オフセットの行で評価された値を返します。
...
* `default` — 計算対象の行がウィンドウフレームの境界外になる場合に返す値。 (省略可能。省略時はカラム型のデフォルト値) 。

**戻り値**

* 順序付けられたフレーム内で、現在の行より前の指定した物理オフセットの位置にある行で評価された値。

**例**

この例では、特定の株式の過去データを対象に、`lagInFrame` 関数を使用して終値の日次差分と変化率を計算します。
