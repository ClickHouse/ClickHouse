...
順序付けされたフレーム内で、n 番目の行 (offset) に対応する最初の非 NULL 値を返します。
...
* `x` — カラム名。
* `offset` — 現在の行と比較する評価対象の n 番目の行。

**戻り値**

* 順序付けられたフレーム内で、n 番目の行 (offset) に対して評価される最初の非 NULL 値。

**例**

この例では、`nth-value` 関数を使用して、架空の Premier League サッカー選手の給与データセットから 3 番目に高い給与を求めます。
