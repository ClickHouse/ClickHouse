...
集約関数で行う計算と同様の処理に使用できますが、ウィンドウ関数では行が単一の出力にまとめられない点が異なります。代わりに、各行がそのまま返されます。

<div id="standard-window-functions">
  ## 標準的なウィンドウ関数
</div>

ClickHouse は、ウィンドウおよびウィンドウ関数に関する標準 SQL 構文をサポートしています。
...
| `ROWS` フレーム                                                            | ✅          |                                                                                                                                                                                                                                                                                                                       |
| `RANGE` フレーム                                                           | ✅          | フレームが明示的に指定されていない場合、デフォルトで使用されます (`RANGE BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW`) 。                                                                                                                                                                                                                              |
| `INTERVAL` syntax for `DateTime` `RANGE OFFSET` フレーム                   | ❌          | 代わりに秒数を指定してください (`RANGE` は任意の数値型で使用できます) 。                                                                                                                                                                                                                                                                            |
| `GROUPS` フレーム                                                          | ❌          |                                                                                                                                                                                                                                                                                                                       |
| フレームに対する 集約関数 の計算 (`sum(value) OVER (ORDER BY time)`)    | ✅          | すべての 集約関数 がサポートされています。                                                                                                                                                                                                                                                                                 |
| `rank()`, `dense_rank()`/`denseRank()`, `row_number()`                  | ✅          |                                                                                                                                                                                                                                                                                                                       |
| `percent_rank()`/`percentRank()`                                        | ✅          | パーティション内での値の相対順位を効率的に計算します。これは、`ifNull((rank() OVER (PARTITION BY x ORDER BY y) - 1) / nullif(count(1) OVER (PARTITION BY x) - 1, 0), 0)` で表される、より冗長で計算コストの高い手動の SQL 計算を置き換えるものです。                                                                                                                                    |
| `cume_dist()`                                                           | ✅          | 値のグループ内における値の累積分布を計算します。現在の行の値以下の値を持つ行の割合を返します。                                                                                                                                                                                                                                                                       |
| `lag/lead(value, offset)`                                               | ✅          | 次のいずれかの回避策も使用できます。<br /> 1) `any(value) OVER (... ROWS BETWEEN <offset> PRECEDING AND <offset> PRECEDING)`、または `lead` の場合は `PRECEDING` の代わりに `FOLLOWING` <br /> 2) `lagInFrame/leadInFrame`。これらは同様の関数ですが、ウィンドウフレームを考慮します。`lag/lead` と同一の動作を得るには、`ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING` を使用してください。 |
| [`ntile(buckets)`](./ntile.md)                                          | ✅          | たとえば、ウィンドウは `(PARTITION BY x ORDER BY y ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING)` のように指定します。                                                                                                                                                                                                         |
...
* `PARTITION BY` - 結果セットをどのようにグループ分けするかを定義します。
* `ORDER BY` - 集約関数 aggregate&#95;function の計算時に、グループ内の行をどのような順序で並べるかを定義します。
* `ROWS or RANGE` - フレームの範囲を定義し、aggregate&#95;function はそのフレーム内で計算されます。
...
| ---------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------ |
| [`row_number()`](./row_number.md)                                                                          | 現在の行に、そのパーティション内で 1 から始まる番号を付けます。                                        |
| [`first_value(x)`](./first_value.md)                                                                       | 順序付けされたフレーム内で評価された最初の値を返します。                                             |
| [`last_value(x)`](./last_value.md)                                                                         | 順序付けされたフレーム内で評価された最後の値を返します。                                             |
| [`nth_value(x, offset)`](./nth_value.md)                                                                   | 順序付けされたフレーム内の n 番目の行 (offset) に対して評価された、最初の非 NULL 値を返します。                |
| [`rank()`](./rank.md)                                                                                      | 現在の行に、そのパーティション内でギャップありの順位を付けます。                                         |
| [`dense_rank()`](./dense_rank.md)                                                                          | 現在の行に、そのパーティション内でギャップなしの順位を付けます。                                         |
| [`lagInFrame(x)`](./lagInFrame.md)                                                                         | 順序付けされたフレーム内で、現在の行より指定した物理オフセット分だけ前の行で評価された値を返します。                       |
| [`leadInFrame(x)`](./leadInFrame.md)                                                                       | 順序付けされたフレーム内で、現在の行より offset 行後の行で評価された値を返します。                            |
| [`ntile(buckets)`](./ntile.md)                                                                             | パーティション内の順序付けされた行を指定した数のバケットに分割し、現在の行のバケット番号を返します。                       |
...
気温は秒単位の精度で保存されていますが、`Range` と `ORDER BY toDate(ts)` を使うことで、サイズ 10 のフレームを作成できます。`toDate(ts)` を使っているため、この単位は日になります。
...
ウィンドウ関数に関連するすべての GitHub issue には、[comp-window-functions](https://github.com/ClickHouse/ClickHouse/labels/comp-window-functions)ラベルが付いています。
...
* ブログ: [Gitコミットの数列に対するウィンドウ関数とArray関数](https://clickhouse.com/blog/clickhouse-window-array-functions-git-commits)
* ブログ: [ClickHouseにデータを取り込む - パート3 - S3を使う](https://clickhouse.com/blog/getting-data-into-clickhouse-part-3-s3)