...
値のグループ内における値の累積分布、つまり現在の行の値以下の値を持つ行の割合を計算します。パーティション内での値の相対的な順位を把握するために使用できます。
...
window functionの構文の詳細については、[Window Functions - Syntax](./index.md/#syntax) を参照してください。

**戻り値**

* 現在の行の相対順位を返します。戻り値の型は、[0, 1] の範囲の Float64 です。[Float64](../data-types/float.md)。
...
等しい値を持つ行 (ピア) には同じ累積分布値が割り当てられ、その値はピアグループ内で最も高い位置に対応します。