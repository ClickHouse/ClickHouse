---
description: '地理的なオブジェクトや位置を表現するために ClickHouse で使用される幾何データ型のドキュメント'
sidebar_label: 'Geo'
sidebar_position: 54
...
ClickHouse は、位置情報や土地などの地理的オブジェクトを表現するためのデータ型をサポートしています。
...
`Point` は X 座標と Y 座標からなり、[Tuple](tuple.md)([Float64](float.md), [Float64](float.md)) として格納されます。
...
`Ring` は、[Point](#point) の [Array](array.md) として格納される、穴のない単純な多角形です。
...
`LineString` は、Point の配列として格納される線です: [Array](array.md)([Point](#point))。
...
`MultiLineString` は、複数の line を `LineString` の `Array` として格納する型です: [Array](array.md)([LineString](#linestring)).
...
`Polygon` は、リングの配列として格納される、穴を持つPolygonです: [Array](array.md)([Ring](#ring))。外側の配列の最初の要素はPolygonの外周を表し、以降の要素はすべて穴です。
...
`MultiPolygon` は複数のPolygonで構成され、Polygonの配列として保存されます: [Array](array.md)([Polygon](#polygon))。

**例**

この `MultiPolygon` は 2 つの独立したPolygonで構成されており、1 つ目には穴がなく、2 つ目には 1 つの穴があります:
...
`Geometry` は、上記のすべての型に共通する型です。これらの型からなる Variant と同等です。
...
* [大規模な実データセットを探る：ClickHouseで見る100年以上の気象記録](https://clickhouse.com/blog/real-world-data-noaa-climate-data)