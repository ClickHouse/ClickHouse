---
description: '指定したスキーマに基づくランダムデータを生成します。そのデータを使ってテスト用テーブルにデータを投入できます。すべての型に対応しているわけではありません。'
...
指定したスキーマに基づくランダムデータを生成します。
そのデータを使ってテスト用テーブルにデータを投入できます。
すべての型に対応しているわけではありません。
...
| `name`              | 対応するカラムの名前。                                                     |
| `TypeName`          | 対応するカラムの型。                                                      |
| `random_seed`       | 安定した結果を得るため、ランダムシード を手動で指定します。`NULL` の場合、seed はランダムに生成されます。 |
| `max_string_length` | 生成されるすべての文字列の最大長。デフォルトは `10` です。                                |
| `max_array_length`  | 生成されるすべてのArrayまたはMapの最大要素数。デフォルトは `10` です。                      |
...
[generateRandomStructure](../../sql-reference/functions/other-functions.md#generateRandomStructure)と組み合わせることで：
...
ランダムな構造とランダムデータの両方にランダムシードを使用する場合:
...
:::note
`max_array_length` を十分に大きくした `generateRandom(generateRandomStructure(), [ランダムシード], max_string_length, max_array_length)` は、複合型 (`Array`、`Tuple`、`Map`、`Nested`) のネスト深度が大きくなる可能性がある (最大 16) ため、非常に巨大な出力を生成することがあります。
:::
