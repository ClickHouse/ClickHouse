---
description: 'ユーザーのメモリ使用量とProfileEventsの概要を把握するのに役立つ情報を含むシステムテーブル。'
...
このシステムテーブルを使用すると、ユーザーごとのメモリ使用量と ProfileEvents の概要を取得できます。
...
* `user` ([String](/ja/sql-reference/data-types/string)) — ユーザー名。
* `memory_usage` ([Int64](/ja/sql-reference/data-types/int-uint)) — ユーザーのすべてのプロセスで使用されているRAMの合計です。一部の専用メモリタイプは含まれない場合があります。`max_memory_usage` 設定を参照してください。
* `peak_memory_usage` ([Int64](/ja/sql-reference/data-types/int-uint)) — ユーザーのメモリ使用量のピークです。ユーザーに対してクエリが実行されていない場合は、リセットされることがあります。
* `ProfileEvents` ([Map(LowCardinality(String), UInt64)](/ja/sql-reference/data-types/map)) — ユーザーに関するさまざまなメトリクスを測定する ProfileEvents の要約です。各項目の説明は `system.events` テーブルを参照してください。
