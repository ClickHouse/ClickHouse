---
description: 'すべてのサーバースレッドのスタックトレースを含むシステムテーブルです。これにより
  開発者はサーバーの内部状態を調査できます。'
...
すべてのサーバースレッドのスタックトレースが含まれています。これにより、開発者はサーバーの状態を詳しく調査できます。

スタックフレームを解析するには、`addressToLine`、`addressToLineWithInlines`、`addressToSymbol`、`demangle` の[イントロスペクション関数](../../sql-reference/functions/introspection.md)を使用します。
...
* `thread_name` ([String](/ja/sql-reference/data-types/string)) — スレッド名。
* `thread_id` ([UInt64](/ja/sql-reference/data-types/int-uint)) — スレッド識別子。
* `query_id` ([String](/ja/sql-reference/data-types/string)) — このスレッドが属するクエリの ID。
* `trace` ([Array(UInt64)](/ja/sql-reference/data-types/array)) — このスレッドのスタックトレース。基本的にはアドレスの配列です。
* `untracked_memory` ([Int64](/ja/sql-reference/data-types/int-uint)) — まだ親 MemoryTracker に反映されていない、スレッドごとの非アトミックなメモリ割り当てカウンター。前回の flush 以降、割り当てた量よりも解放した量のほうが多い場合は、負の値になることがあります。
...
ClickHouseのソースコードでファイル名と行番号を取得する方法:
...
* [イントロスペクション関数](../../sql-reference/functions/introspection.md) — 利用可能なイントロスペクション関数と、その使用方法を説明します。
* [system.trace&#95;log](../system-tables/trace_log.md) — sampling query profiler によって収集されたスタックトレースが含まれます。
* [arrayMap](/ja/sql-reference/functions/array-functions#arrayMap)) — `arrayMap` 関数の説明と使用例。
* [arrayFilter](/ja/sql-reference/functions/array-functions#arrayFilter) — `arrayFilter` 関数の説明と使用例。