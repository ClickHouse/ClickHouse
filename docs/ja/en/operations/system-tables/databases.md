---
description: '現在のユーザーが利用可能なデータベースに関する情報を含むシステムテーブル。'
keywords: ['システムテーブル', 'データベース']
slug: /operations/system-tables/databases
title: 'system.databases'
doc_type: 'reference'
---

<div id="description">
  ## 説明
</div>

現在のユーザーが利用可能なデータベースに関する情報が含まれます。

<div id="columns">
  ## カラム
</div>

{/*AUTOGENERATED_START*/ }

* `name` ([String](/ja/sql-reference/data-types/string)) — データベース名。
* `engine` ([String](/ja/sql-reference/data-types/string)) — データベースエンジン。
* `data_path` ([String](/ja/sql-reference/data-types/string)) — データパス。
* `metadata_path` ([String](/ja/sql-reference/data-types/string)) — メタデータパス。
* `uuid` ([UUID](/ja/sql-reference/data-types/uuid)) — データベースのUUID。
* `engine_full` ([String](/ja/sql-reference/data-types/string)) — データベースエンジンのパラメータ。
* `comment` ([String](/ja/sql-reference/data-types/string)) — データベースコメント。
* `is_external` ([UInt8](/ja/sql-reference/data-types/int-uint)) — データベースが外部のものかどうか (例: PostgreSQL/DataLakeCatalog) 。

**別名:**

* `database` — `name` の別名。

{/*AUTOGENERATED_END*/ }

このシステムテーブルの `name` カラムは、`SHOW DATABASES` クエリの実装に使用されます。

<div id="example">
  ## 例
</div>

データベースを作成します。

```sql title="Query"
CREATE DATABASE test;
```

ユーザーが利用できるすべてのデータベースを確認します。

```sql title="Query"
SELECT * FROM system.databases;
```

```text title="Response"
┌─name────────────────┬─engine─────┬─data_path────────────────────┬─metadata_path─────────────────────────────────────────────────────────┬─uuid─────────────────────────────────┬─engine_full────────────────────────────────────────────┬─comment─┐
│ INFORMATION_SCHEMA  │ Memory     │ /data/clickhouse_data/       │                                                                       │ 00000000-0000-0000-0000-000000000000 │ Memory                                                 │         │
│ default             │ Atomic     │ /data/clickhouse_data/store/ │ /data/clickhouse_data/store/f97/f97a3ceb-2e8a-4912-a043-c536e826a4d4/ │ f97a3ceb-2e8a-4912-a043-c536e826a4d4 │ Atomic                                                 │         │
│ information_schema  │ Memory     │ /data/clickhouse_data/       │                                                                       │ 00000000-0000-0000-0000-000000000000 │ Memory                                                 │         │
│ replicated_database │ Replicated │ /data/clickhouse_data/store/ │ /data/clickhouse_data/store/da8/da85bb71-102b-4f69-9aad-f8d6c403905e/ │ da85bb71-102b-4f69-9aad-f8d6c403905e │ Replicated('some/path/database', 'shard1', 'replica1') │         │
│ system              │ Atomic     │ /data/clickhouse_data/store/ │ /data/clickhouse_data/store/b57/b5770419-ac7a-4b67-8229-524122024076/ │ b5770419-ac7a-4b67-8229-524122024076 │ Atomic                                                 │         │
│ test                │ Atomic     │ /data/clickhouse_data/store/ │ /data/clickhouse_data/store/2a1/2a1b3c4d-5e6f-7890-abcd-ef1234567890/ │ 2a1b3c4d-5e6f-7890-abcd-ef1234567890 │ Atomic                                                 │         │
└─────────────────────┴────────────┴──────────────────────────────┴───────────────────────────────────────────────────────────────────────┴──────────────────────────────────────┴────────────────────────────────────────────────────────┴─────────┘
```