---
description: 'C++ に精通したユーザーや ClickHouse のエンジニアに役立つ、`clickhouse` バイナリの
  イントロスペクション情報を含むシステムテーブル。'
keywords: ['システムテーブル', 'シンボル']
...
`clickhouse` バイナリの内部調査に関する情報が含まれています。アクセスするには、イントロスペクション 権限が必要です。
このテーブルは、C++ の専門家や ClickHouse のエンジニアにとってのみ有用です。
...
* `symbol` ([String](/ja/sql-reference/data-types/string)) — バイナリ内のシンボル名です。これはマングルされています。`demangle(symbol)` を適用すると、可読な名前を取得できます。
* `symbol_demangled` ([Nullable(String)](/ja/sql-reference/data-types/nullable)) — XRay インストルメンテーションで使用される、デマングル済みのシンボルです。
* `function_id` ([Nullable(Int32)](/ja/sql-reference/data-types/nullable)) — XRay インストルメンテーションマップ内の関数 ID です。
* `address_begin` ([UInt64](/ja/sql-reference/data-types/int-uint)) — バイナリ内のシンボルの開始アドレスです。
* `address_end` ([UInt64](/ja/sql-reference/data-types/int-uint)) — バイナリ内のシンボルの終了アドレスです。
