...
<Steps>
  <Step>
    ...
    Follow the steps in section [&quot;Create table&quot;](/ja/get-started/sample-datasets/youtube-dislikes#create-the-table) to create the table on your Cloud service and insert data to it.
    ...
    You can specify クエリパラメータ using `{ }` together with the type of the parameter.
    The SQL console query editor automatically detects ClickHouse クエリパラメータ expressions and provides an input for each parameter.

    Let&#39;s quickly run this query to make sure that it works by specifying the year `2010` in the クエリ変数 input box on the right side of the SQL editor:
    ...
    More documentation around saved queries can be found in section [&quot;Saving a query&quot;](/ja/products/cloud/features/sql-console-features/sql-console#saving-a-query).
  </Step>
  ...
  <Step>
    ...
    <Image img="/images/cloud/sqlconsole/endpoints-configure.webp" size="md" alt="クエリ API エンドポイントを設定する" />

    After selecting an API key, you will be asked to:

    * Select the データベースロール that will be used to run the query (`Full access`, `Read only` or `Create a custom role`)
  ...
  <Step>
    ...
    クエリ内のクエリパラメータは、`{parameter_name: type}` 構文で指定できます。These parameters will be automatically detected and the example request payload will contain a `queryVariables` object through which you can pass these parameters.
  </Step>
