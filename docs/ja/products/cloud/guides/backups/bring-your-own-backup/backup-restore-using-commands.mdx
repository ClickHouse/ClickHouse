...
<Tabs>
  ...
  <Tab title="復元">
    ...
    ([`GRANT CURRENT GRANTS`](/ja/sql-reference/statements/grant) と同じセマンティクス) 。これにより、バックアップに復元を実行するユーザーが付与できる権限を超える権限が含まれていても、`RESTORE` が `ACCESS_DENIED` で失敗することはありません。
