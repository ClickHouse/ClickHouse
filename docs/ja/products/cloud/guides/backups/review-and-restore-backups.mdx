...
サービスは、デフォルトの毎日のスケジュールでも、ユーザーが選択した[カスタムスケジュール](/ja/products/cloud/guides/backups/configurable-backups)でも、設定されたスケジュールに従ってバックアップされます。利用可能なバックアップはすべて、サービスの**Backups**タブから確認できます。ここでは、バックアップのステータス、所要時間、バックアップのサイズを確認できます。また、**Actions**カラムから特定のバックアップを復元することもできます。
