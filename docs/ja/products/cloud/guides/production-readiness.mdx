...
ClickHouse Cloud は、[Okta](/ja/products/cloud/guides/security/cloud-access-management/scim-setup) および [Microsoft Entra ID](/ja/products/cloud/guides/security/cloud-access-management/scim-setup-entra) 向けの SCIM provisioning をプライベートプレビューで提供しています。SCIM ではユーザーとグループのライフサイクル管理が自動化されるため、アイデンティティ プロバイダー で割り当てや割り当て解除を行うと、ユーザーは自動的に作成または削除されます。SCIM を使用していない場合、アイデンティティ プロバイダー から削除しただけでは不十分で、ClickHouse Cloud コンソール からもユーザーを手動で削除する必要があります。その場合は、手動でのユーザー管理プロセスを考慮してください。
