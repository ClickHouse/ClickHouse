...
<EnterprisePlanFeatureBadge feature="カスタムアイデンティティプロバイダを使用したJWT認証" support="true" />

<Tip>
  このガイドでは、Cloud Console で JWKS プロバイダを設定する方法を説明します。JWT の生成方法や構造 (必須のクレーム、ロールおよび権限付与のクレーム、一時ユーザーの仕組みを含む) については、[JWT Authentication](/ja/operations/external-authenticators/jwt) リファレンスを参照してください。
</Tip>
...
この機能を組織で有効にすると、ClickHouse Cloud Console の **Settings → Security** から、サービスごとにこれらの JWKS プロバイダを設定できます。

<Note>
  組織で JWT プロバイダを有効にするには、[サポート](https://clickhouse.com/support/program) までお問い合わせください。
</Note>
...
* 少なくとも 1 つの **RSA** 鍵をパブリッシュする、パブリックから到達可能な **HTTPS** JWKS URL (トークンは **RS256** で署名されている必要があります) 。

<Note>
  JWKS ベースのプロバイダで使用できるのは **RSA 鍵のみ**です。HMAC (`HS256`) または EC (`ES256`) 鍵で署名されたトークンは JWKS エンドポイントで検証できないため、拒否されます。JWKS ドキュメントには他の種類の鍵を含めることができますが、使用可能な RSA 鍵が少なくとも 1 つ必要です。
</Note>
...
クライアント (アイデンティティプロバイダまたはアプリケーション) は JWT を生成し、**秘密鍵**で署名します。トークンは、想定される[トークン形式](/ja/operations/external-authenticators/jwt#token-claims)に従う必要があります。ClickHouse は、JWKS URL で公開されている**公開鍵**を使用してトークンを検証します。

1. ClickHouse はトークンの `kid` (鍵 ID) ヘッダーを読み取り、JWKS ドキュメントから一致する鍵を選択します。
2. その RSA 公開鍵を使用してトークンの署名を検証し、`iss` (発行者) および `aud` (オーディエンス) クレーム をプロバイダー設定と照合します。
3. 検証に成功すると、接続は一時ユーザーとして実行されます。アクセス権はトークンの `clickhouse:grants` および `clickhouse:roles` クレーム から取得され、権限上限 (`default` ユーザー) によって制限されます。詳細は[アクセス権](/ja/operations/external-authenticators/jwt#access-rights)を参照してください。
...
<Steps>
  ...
  <Step title="プロバイダの詳細を入力する" id="fill-provider-details">
    ...
    | **JWKS URL**         | トークン署名の検証に使用する RSA 公開鍵を公開する HTTPS 公開 URL。                                                                                   |
...
* JWKSプロバイダで使用できるのは **RSA / RS256** 鍵のみです。
...
* [JWT Authentication](/ja/operations/external-authenticators/jwt) — トークンのクレーム、一時ユーザー、クライアントの使用方法。