...
サービスプロバイダー開始型 SSO、個別の 接続 を使用する複数の組織、ならびにジャストインタイムプロビジョニングをサポートしています。また、[Okta](/ja/products/cloud/guides/security/cloud-access-management/scim-setup) および [Microsoft Entra ID](/ja/products/cloud/guides/security/cloud-access-management/scim-setup-entra) を使用した SCIM Provisioning をプライベートプレビューで提供しています。現時点では、属性マッピングには対応していません。
...
* SCIM Provisioning — [Okta](/ja/products/cloud/guides/security/cloud-access-management/scim-setup) または [Microsoft Entra ID](/ja/products/cloud/guides/security/cloud-access-management/scim-setup-entra) を使用して、ユーザーとグループのプロビジョニングを自動化します (プライベートプレビュー) 。
