...
サービスプロバイダー開始型 SSO、個別の connection を使用する複数の組織、ならびにジャストインタイムプロビジョニングをサポートしています。また、Okta をサポートする [SCIM provisioning](/ja/products/cloud/guides/security/cloud-access-management/scim-setup) をプライベートプレビューで提供しています。現時点では、属性マッピングには対応していません。
