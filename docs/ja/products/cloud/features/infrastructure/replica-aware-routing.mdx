...
レプリカ対応ルーティング (sticky sessions、スティッキールーティング、session affinity とも呼ばれます) は、関連するリクエストを同じ ClickHouse レプリカにルーティングします。[一時テーブル](/ja/sql-reference/statements/create/table#temporary-tables) や [名前付きセッション状態](/ja/interfaces/http#using-clickhouse-sessions-in-the-http-protocol) にクエリをまたいでアクセスできるようにしておく必要がある場合、または関連するクエリで同じレプリカのローカル cache を再利用したい場合に使用します。
