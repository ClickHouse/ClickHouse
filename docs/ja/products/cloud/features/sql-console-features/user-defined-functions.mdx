...
4. [Terraform](#manage-udfs-with-terraform) を使用する (alpha)
...
UI で利用できるすべての機能は、[ClickHouse Cloud API](/ja/cloud/manage/api/api-overview) を通じてプログラムからも利用できます。
...
<Note>
  これらのリソースは、プロバイダーのバージョン 3.24.0 以降で利用できます。alpha ステータスであり、今後のプロバイダーバージョンで動作が変更される可能性があります。
</Note>
