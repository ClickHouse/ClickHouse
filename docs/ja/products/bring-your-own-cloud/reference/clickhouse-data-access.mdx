...
| --------------- | ------------------------------------------------------------------------------------------------- |
| State exporter  | ClickHouse Cloud が所有する `SQS` キューに送信されるサービスおよびバックアップの状態 (正常性、ステータス — 運用イベントであり、バックアップの内容ではありません) 。 |
| Billing scraper | ClickHouse Cloud が所有するオブジェクトストレージバケットに送信される CPU およびメモリのメトリクス。                                     |
