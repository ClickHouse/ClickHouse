...
| 用途                                    | 接続タイプ                                                                      | 注記                                                                                                                                         |
| ------------------------------------- | -------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| **日常運用 — Kubernetes API サーバー**        | IP フィルタリング付きパブリック接続 (デフォルト)、または Tailscale もしくは AWS VPC Lattice 経由のプライベート接続 | 管理サービスは、IP 許可リストで制限されたパブリックネットワーク経由で EKS API サーバーと通信します。初回デプロイ後は、必要に応じてこれを Tailscale 経由、または AWS では VPC Lattice 経由のプライベートアクセスに切り替えることもできます。 |
