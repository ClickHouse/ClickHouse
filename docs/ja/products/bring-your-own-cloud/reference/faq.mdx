...
<Accordion title="作成される VPC/VNet の IP 範囲はどのくらいのサイズですか？">
  デフォルトでは、BYOC VPC (AWS/GCP) または VNet (Azure) に `10.0.0.0/16` を使用します。将来的なスケーリングに備え、少なくとも /22 を予約することを推奨します。
  ただし、サイズを制限したい場合は、server pod が 30 個に
  制限される可能性が高い場合に限り、/23 を使用できます。
</Accordion>
