...
ピアリングアカウントに移動し、(VPC -&gt; Peering connections -&gt; Actions -&gt; Accept request) ページで、この VPC ピアリングリクエストを承認します。
...
1. VPC ダッシュボード で Route Tables を選択します。
...
6. Target には「ピアリング接続」とピアリング接続 ID を選択します。
...
6. Target で「ピアリング接続」とピアリング接続の ID を選択します。
