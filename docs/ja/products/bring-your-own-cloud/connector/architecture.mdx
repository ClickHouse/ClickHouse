...
<Note>
  ここで説明するmanaged モードの登録、customer-create、およびplatform-approvalのワークフローを利用するには、account teamによる確認を受けた[対応するCLIおよびデプロイメントのサポート](/ja/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)が必要です。公開されている `v0.16.0` にはmanaged モードのinit flagsも `platform` コマンドも含まれておらず、customer-createのワークフローも提供されていません。
</Note>
...
以下のカスタマー作成およびプラットフォーム承認の接続は、[サポートされている managed モードのデプロイメント](/ja/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) に適用されます。これらのワークフローは公開版の `v0.16.0` では利用できません。
...
| Amazon ECR (executor、managed モードのみ)                      | Outbound            | HTTPS                 | プラットフォームへの direct アクセスの場合は、コネクタ EC2 VM のインスタンスプロファイルを使用して assume する読み取り専用の pull ロール、その他の ECR チャート registry の場合は ambient な AWS 認証情報 | プラットフォームの同期または作成時にチャートを取得するための registry ログイン、プラットフォーム更新中に image の存在を確認するための `DescribeImages`                                                                                             |
| AWS STS (executor、managed モードのみ)                         | Outbound            | HTTPS                 | プラットフォームへの direct アクセスの場合はコネクタ EC2 VM のインスタンスプロファイル、その他の ECR チャート registry の場合は設定された AWS 認証情報チェーン                                  | プラットフォームの direct なチャートログインと image チェックのために読み取り専用の pull ロールを assume する。その他の ECR ログイン用の認証情報を提供する場合もある。ECR と STS は executor が行う唯一のクラウド API 呼び出し                                             |
| コンテナ registry                                            | Outbound、クラスターノードから | HTTPS                 | ノードの identity：登録したノードロールに ClickHouse が付与する読み取り専用の pull ロール、または image をインポートした後の独自の registry                                        | managed サービス とプラットフォームコンポーネントの image の取得。クラスターが pull するのみで、ClickHouse は何もプッシュしない                                                                                                     |
...
サービス作成およびプラットフォーム承認のワークフローには、[対応する CLI とデプロイメントのサポート](/ja/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) が必要です。公開されている `v0.16.0` にはこれらは含まれていません。
...
* **scraper とトラブルシューターはクラスターを変更できません。** これらの RBAC は、コネクタおよびサービスのネームスペース内の Role を通じて付与され、ワークロードリソースに対する読み取り専用の verb と、コネクタ自身の Secret への名前を完全一致で指定したアクセスに限定されています。`exec`、`delete`、`patch` の権限はありません。
* **executor は `pcm-executor` としてサービスネームスペース内のクラスター状態を変更します。** サービスのプレフィックス (デフォルトでは `ns-`) を持つネームスペースを作成・削除し、ポッドを削除して再起動します。それらのネームスペース内では、ClickHouseCluster、Backup、ScalingOverride の各オブジェクト、サービスに必要な Secret、ServiceAccount、Service、そして scraper に読み取りアクセスを与える Role と RoleBinding を書き込みます。ValidatingAdmissionPolicy により、その ServiceAccount からのプレフィックス外への書き込みはすべて拒否されます。ただし、自身のトークン更新と、自身のホームネームスペース内の `clicklink-instance-registry` ConfigMap は例外です。このポリシーは評価できない場合、拒否側にフェイルします。admission は読み取りには適用されないため、これらのリソースファミリーに対する読み取りはプレフィックスによって制限されません。
...
* **executor はお客様の bucket や IAM への認証情報を保持しません。** `clicklink clctl executor prepare` はサービスのデータ用 bucket とバックアップ用 bucket、およびその IAM ロールを作成し、`clicklink clctl executor teardown` はそれらを削除します。いずれもお客様の認証情報で実行されます。このロールが信頼するのはそのサービスのポッドのみであるため、executor も ClickHouse もこのロールを引き受けることはできません。クラウドへの呼び出しは Amazon ECR と STS に対するもののみです。プラットフォームの同期や作成でチャートを取得する際にレジストリへログインします。プラットフォーム更新中には、読み取り専用の pull ロールでイメージを確認します。S3 や IAM への呼び出しは一切行いません。
