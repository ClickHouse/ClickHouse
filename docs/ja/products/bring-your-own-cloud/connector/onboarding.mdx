...
登録が拒否された場合、そのトークンはすでに使用済みであるか、無効または期限切れです。使用済みトークンの場合は、クリーンアップのステップまで残る `handoff.yaml` を使い、`--handoff handoff.yaml` を指定して再実行してください。それ以外の場合は、ClickHouse の account team に新しいトークンを依頼してください。登録が通信エラーで失敗した場合は、同じコマンドを再実行してください。再実行も拒否された場合は、最初の試行でトークンが消費されているため、新しいトークンをリクエストしてください。
