...
このページでは、登録トークンの取得から、ClickHouse Cloud が駆動するサービスライフサイクルを executor が適用する managed モードでの検証済みコネクタの準備までの手順を説明します。コネクタは、Kubernetes クラスター (Helm) または Linux VM (systemd) にインストールできます。標準的な手順はトークン登録です。環境からコネクタのエンドポイントに直接アクセスできない場合は、[エアギャップ環境およびミラーインストール](#air-gapped-and-mirrored-installs)を参照してください。executor を使わずに自分で運用する ClickHouse を監視する場合は、`--no-managed` を指定してください。

## Managed モードの提供状況 {#managed-mode-availability}

<Warning>
  2026-09-22 時点で、公開インストーラーが提供するのは `v0.16.0` であり、`init --managed` および `init --no-managed` フラグも `platform` コマンドも含まれていません。Linux VM と Kubernetes のいずれのインストールにおけるプラットフォーム承認も、顧客サービスの作成 (`instances create --from-prepare`) も、公開リリースではまだ利用できません。Kubernetes のプラットフォーム承認では、さらに `platform approve --secret-namespace` に対する CLI とチャートのサポートが必要です。

  Managed モードのワークフローを実行する前に、ClickHouse アカウントチームに対象のデプロイ先がサポートされているかを確認し、対応する CLI、コネクタのチャートまたはイメージ、およびインストール手順を提供してもらってください。顧客サービスの作成には、コネクタエンドポイント側と ClickHouse Cloud 側のサポートも必要であり、CLI を新しくするだけでは不十分です。以下の公開インストールコマンドでは、現時点ではこれらのワークフローに対応したリリースはインストールされません。
</Warning>
...
その後、ClickHouse Cloud からコネクタエンドポイントと使い捨ての登録トークン (登録トークン) が発行されます。このトークンによって環境が識別されるため、登録時に追加の ID を入力する必要はありません。

### すべてのインストールで共通 {#prerequisites-every-install}

* 以下の Managed モードの手順には **[managed モード対応](#managed-mode-availability)のビルド**が必要です。公開されている `v0.16.0` には `--managed` も `--no-managed` もありません。
...
* Managed モードには **Amazon EKS cluster** が必要です。`init` はそれ以外の cluster では Managed モードを拒否し、executor なしでインストールする方法として `--no-managed` を案内します。
* **executor の grant に必要な cluster admin 権限。** この grant では、executor を自身の prefix 配下の namespace に限定する ClusterRole、ClusterRoleBinding、ValidatingAdmissionPolicy が作成されます。Kubernetes では、`init` の実行に使う kubeconfig にこの権限が必要です。VM ではホストの kubeconfig に必要で、ホストに admin 権限がない場合、`init` は権限を持つホストで実行すべきコマンドを出力します。
* **ClickHouse はまだ不要です。** Managed インストールは service が存在しない状態から始まり、executor が作成した service が自動的に登録されます。Managed モードの `init` は、自身で運用している ClickHouse を登録しません (Kubernetes では `--instance` と `--instance-namespace` を拒否します) 。そのような cluster を監視するには、`--no-managed` を指定して別の connector deployment をインストールしてください。`--ch-user-suffix` を使えば、2 つの deployment が connector の user 名を衝突させることなく同一の instance を共有できます。この deployment には到達可能な native listener が必要で、セキュア接続なら 9440、plaintext なら 9000 を使用し、Kubernetes では自動検出されます。さらに、provisioning のために ClickHouse admin アクセスも必要です。パスワードなしの `default` user、`--ch-admin-password-stdin` もしくはプロンプトで渡すパスワード、または operator 管理の instance の場合は CR injection のいずれかを利用します。
...
* **kubeconfig**：現在のコンテキストが EKS cluster を指していること。コネクタのネームスペースの作成と読み取り、Secrets の適用、ServiceAccounts、Roles、RoleBindings の作成、チャートのインストール、および executor への cluster-wide なアクセス権の付与が行える必要があります。
* **AWS credentials**：workstation 上 (環境変数または profile) に、`eks:DescribeCluster` と `ec2:DescribeVpcs` の権限を持つもの。`init` はこれらを使用して、NetworkPolicy 用に cluster の VPC の CIDR ブロックを読み取ります。ルックアップに失敗した場合は警告を表示し、そのリストは手動で入力できるよう空のまま残します。
...
* **イメージのプルアクセス**：cluster のノードが、パブリック ECR イメージまたは自身でホストする mirror をプルできる必要があります。
...
* provisioning および executor の grant のための **Kubernetes API server への Admin アクセス**:ホスト上の kubeconfig、`--server` と `--ca-data`、またはプロンプトのいずれか。Access bundles は両方の target で Kubernetes ServiceAccount に紐付けられます。Managed モードでは、kubeconfig の current context が管理対象の EKS cluster を指している必要があります。それ以外の context を経由した grant は `init` が拒否します。

<Note>
  VM において Kubernetes API server の要件を回避する方法は `--skip-provision` のみで、これは stage 専用です。ClickHouse user の provisioning、executor の grant、ユニットの有効化、検証をスキップするため、これだけでは動作するコネクタにはなりません。Kubernetes ではスキップされるのは ClickHouse user の provisioning のみで、executor の grant は引き続き適用されます。チャートの executor は grant された ServiceAccount がなければスケジュールできないためです。
</Note>

## インストールと登録 {#install-and-enroll}

まず、[managed モードの提供状況](#managed-mode-availability)についてアカウントチームに確認してください。下記の公開インストーラーが配布するのは `v0.16.0` であり、本ウォークスルーで使用する managed モードの登録コマンドは実行できません。
...
<Steps>
  <Step title="コネクタのエンドポイントと登録トークンを取得する" id="get-endpoint-and-token">
    ...
    このトークンは一度しか使用できず、短時間で有効期限が切れるため、受け取ったら速やかに登録を実行してください。トークンはシークレットとして扱ってください。CLI はトークンを非表示のプロンプトまたは stdin の 1 行目から読み取り、コマンドライン引数やディスク、ログから読み取ることはありません。登録前に有効期限が切れてしまった場合は、ClickHouse のアカウントチームに新しいトークンを依頼してください。
  </Step>

  <Step title="CLI のインストールと確認" id="install-and-verify-the-cli">
    1つのコマンドで、検証済みの `clicklink` バイナリをインストールできます。プラットフォーム (macOS または Linux、amd64 または arm64) を自動検出し、最新のリリースをダウンロードして SHA-256 チェックサムを検証します。さらに cosign がインストールされている場合は、リリース署名も検証します。バイナリは `/usr/local/bin` が書き込み可能な場合はそこに、そうでない場合は `~/.local/bin` にインストールされます (必要に応じてそのディレクトリを `PATH` に追加してください。`--bin-dir` で変更できます) 。Kubernetes にインストールする場合は、対象のクラスターに kubeconfig でアクセスできる任意のマシンで実行してください:

    ```bash
    curl -fsSL https://releases.clicklink.clickhouse.com/install.sh | bash
    ```

    VM へのインストールでは、ホスト上で同じスクリプトを `--host` 付きで実行します。検証済みのダウンロードが完了すると、`clicklink` システムユーザー、`/etc/clicklink`、`/var/lib/clicklink`、`/var/log/clicklink` の各ディレクトリ、および 3 つのコンポーネントすべての systemd unit も作成されます。`/etc/clicklink/redaction-patterns.yaml` には初期パターン一式が書き込まれます (既存のファイルがある場合はそのまま保持されます) 。そのため、次のステップは登録から始まります:
  ...
  <Step title="コネクタを登録してインストールする" id="enroll-and-install">
    これらのコマンドを使用するには、[managed モードの提供状況](#managed-mode-availability)に記載されたビルドおよびデプロイメントのサポートが必要です。一般提供版の `v0.16.0` には `--managed` も `--no-managed` も存在しません。

    登録はコマンド 1 つで完了します。トークンを引き換え、署名済みのクライアント証明書と executor の cluster-wide grant を取得したうえで、コネクタをインストールして検証します。
  ...
  <Step title="クリーンアップ" id="clean-up">
    enrollment bundle である `handoff.yaml` (作業ディレクトリ内、mode `0600`) は、インストール中の再実行や復旧時に 2 つ目のトークンを不要にするために存在します。このファイルにはコネクタの API secret が平文で格納されているため、インストールの検証が完了したら削除してください。
    ...
    実行中のコネクタは認証情報のコピーを自身で保持するため、運用上このファイルに依存する処理はありません。アップグレードや設定変更でこのファイルが必要になることもありません。後から再度 `init` が必要になった場合は、ClickHouse のアカウントチームに新しい登録トークンをリクエストし、`init --enroll --force` を実行してください。
  </Step>
</Steps>
...
`init` が `--managed` または `--no-managed` を不明な flag として拒否する場合は、CLI のバージョンを確認してください。公開版の `v0.16.0` はどちらもサポートしていません。以下の managed モードの復旧手順に従う前に、アカウントチーム から [managed モードに対応した build](#managed-mode-availability) を入手してください。

同じコマンドを再実行してください。`init` は冪等であり、再実行しても同じ状態に収束し、既存の設定とステージ済みのファイルはそのまま維持され、完了済みの作業はスキップされます。ステップが途中で失敗した場合、CLI が状況に応じた復旧コマンドをそのまま出力します。これらは繰り返し実行しても安全です。

登録が拒否された場合、そのトークンはすでに使用済みであるか、無効または期限切れです。使用済みトークンの場合は、クリーンアップのステップまで残る `handoff.yaml` を使い、`--handoff handoff.yaml` を指定して再実行してください。それ以外の場合は、ClickHouse の アカウントチーム に新しいトークンを依頼してください。登録が通信エラーで失敗した場合は、同じコマンドを再実行してください。再実行も拒否された場合は、最初の試行でトークンが消費されているため、新しいトークンをリクエストしてください。
...
* **Kubernetes 上では**、grant が拒否されると chart のデプロイ前に `init` が停止します。executor は grant された ServiceAccount にバインドするため、それがないと起動できないからです。ステージングされた overlay、Secrets、CSR は保持され、`init` は cluster admin 権限を持つ host から実行すべき 2 つのコマンドを出力します。それらを実行したうえで、同じ `init` コマンドを再実行して続行してください:
...
`--force` は明示的なリセットであり、日常的な再試行の手段ではありません。保持されている config や values overlay を上書きし、client key を再生成し、有効期限内のクライアント証明書も差し替えます (署名エンドポイントから `409` が返る場合は、すでに証明書が存在することを意味します) 。コネクタのクラスター UUID は `--force` の実行後も保持され、managed モードの設定状態も同様です。無人での再実行ではこの状態が維持され、端末上ではプロンプトのデフォルト値にもその状態が使われます (`--managed` または `--no-managed` を指定した場合を除く) 。認証情報のローテーションや証明書の差し替えを行う場合に `--force` を使用してください。再実行と復旧のモデル全体については [operations](/ja/products/bring-your-own-cloud/connector/operations) を参照してください。