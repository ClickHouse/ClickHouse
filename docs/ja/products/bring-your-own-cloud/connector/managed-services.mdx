...
managed モードでは、コネクタは 3 つ目のコンポーネントである executor を実行します。ClickHouse Cloud はこれを介して、お使いの Kubernetes クラスター 内で ClickHouse サービス を運用します。このページでは、サービス の作成、その ステータス の確認、サービス 作成後に ClickHouse Cloud が行う処理、そして サービス が廃止されるまでの流れについて説明します。

<Note>
  managed モードの有効化と サービス の作成には、パブリックリリースにはまだ含まれていないサポートが必要です。パブリックの `v0.16.0` CLI には、`--managed` および `--no-managed` の init flag も、以下のカスタマー作成 ワークフロー も含まれていません。enroll や resource の準備を行う前に、アカウントチーム に [managed モードの availability](/ja/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) を確認してください。サービス の作成には、コネクタエンドポイント および ClickHouse Cloud 側の対応も必要です。
</Note>

## managed モードの動作 {#what-managed-mode-does}

executor は、scraper やトラブルシューターと同じ `clicklink` バイナリに含まれるデーモンです。コネクタエンドポイントへのアウトバウンドの WebSocket チャネルを保持し、ClickHouse Cloud からライフサイクルコマンドを受信します。受信した各コマンドは、設定対象となっている単一の Kubernetes クラスターに適用され、その結果は同じチャネルを通じて報告されます。他のコンポーネントと同様に、アウトバウンド接続のみを行います。すなわち、ClickHouse Cloud からクラスターへ接続されることはなく、executor のローカル API はループバックにバインドされます。

managed モードは、プライベートプレビューの一環として、S3 storage を使用する Amazon EKS で利用できます。環境を登録する際に、ClickHouse のアカウントチームが有効化します。`init` は、EKS クラスターを指していない kube context を拒否します。

各サービスには、ユーザー自身が所有する 3 つのクラウドリソースが必要です。データ用 bucket、バックアップ用 bucket、そして ClickHouse Pod がそれらにアクセスするために assume する IAM ロールです。これらはサービスの作成前にユーザー自身の認証情報で作成し、サービスの削除後に取り除きます。executor は bucket や IAM に対する認証情報を一切保持せず、データを削除することもありません。executor が行うクラウド呼び出しは Amazon ECR と STS に対するもののみです。プラットフォームの同期や作成でチャートを取得する際に レジストリ にログインします。プラットフォームの更新中は、読み取り専用の pull role でイメージを確認します。S3 や IAM への呼び出しは行いません。各サービスに対して LoadBalancer タイプの Kubernetes `Service` を作成し、クラスターの load-balancer controller がこれをユーザーの account 内の内部 NLB として実体化します。
...
<Note>
  公開されている `v0.16.0` の CLI は、`init --managed` や以下で説明する managed モードのプロンプトをサポートしていません。まず アカウントチーム から [managed モードのサポート](/ja/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) を含む build とデプロイメントを入手してください。
</Note>
...
このワークフローは公開版の `v0.16.0` では使用できません。リソースを準備する前に、お使いの CLI、コネクタエンドポイント、および ClickHouse Cloud デプロイメントについて [サービス作成の利用可否](/ja/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) を確認してください。
...
* `prepare` には、コネクタ 設定のコピー (`--config`) 、executor のローカル API への `kubectl port-forward`、および書き込み可能な directory を指す `--output-dir` が必要です。executor に対して service name を照合し、到達できない場合は実行を拒否します。
* `instances create` は、このホストに認証情報ファイルが存在しない場合、設定で指定された認証情報ファイルを `clicklink-hmac` および `clicklink-mtls` Secret から読み取り、読み取るたびにその旨を通知します。コネクタ のネームスペースが `clicklink` でない場合は `--connector-namespace` を追加してください。このフォールバックがあるのは `instances create` のみです。
...
<Steps>
  <Step title="サービスを準備する" id="prepare">
    ...
    4. **Summary.** `default` USER の password を生成し、create ボディを `<output-dir>/_prepare/<name>.create.json` に書き込みます (mode `0600`。含まれるのは password の hashes のみです) 。続けて実行すべきコマンドは `Next:` 行にそのまま出力されます。

    <Warning>
      `prepare` は `default` USER の password を stderr に一度だけ出力し、以降は二度と表示しません。create ボディにも `--output json` にも含まれず、ClickHouse Cloud が受け取るのは hashes のみです。次に進む前に必ず保存してください。create ボディが存在する状態で再実行した場合は、すでに書き込まれた hashes がそのまま維持され、その旨が表示されます。
    </Warning>
  ...
  <Step title="作成する" id="create">
    ...
    同じ入力での再試行は安全です。冪等キーはデフォルトで環境と service name から導出された値が使われるため、再送信しても最初の作成結果が返されます。
...
* **スケール。** サービスのレプリカ数は ClickHouse Cloud が設定する固定値で、ClickHouse Cloud 側の設定 (デフォルト設定では 20) が上限となります。オートスケーリングはありません。
* **停止と開始。** 停止するとサーバーはゼロにスケールされ、Keeper は維持されます。開始するとレプリカ数が元に戻ります。その間もデータはお客様のバケットに保持されます。
* **再起動。** サービス全体、その Keeper、または単一のポッドが対象です。
* **バックアップ。** バックアップは ClickHouse Cloud がトリガーしたときに実行され、お客様のバックアップバケットに保存されます。バックアップを削除すると、そこから取り除かれます。
...
`instances scale`、`instances patch`、`instances delete` の各サブコマンドは、ClickHouse Cloud を介さず、executor のローカル API に直接コマンドを送信します。ClickHouse のアカウントチームから実行を依頼された場合を除き、これらは使用しないでください。それぞれの動作については [CLI リファレンス](/ja/products/bring-your-own-cloud/connector/reference/cli#instances-scale)を参照してください。
...
<Tabs>
  <Tab title="Kubernetes" id="troubleshooter-kubernetes">
    ...
    `$CH_DEFAULT_PASSWORD` は、`prepare` が出力した `default` ユーザーのパスワードです。このコマンドは SQL の権限を適用する前に認証を行うためにこれを必要としますが、入力を求めるプロンプトは表示しません。続いて、[ClickHouse インスタンスの追加](/ja/products/bring-your-own-cloud/connector/configuration#clickhouse-instances) に示すとおり、Secret と ServiceAccount のペアを `troubleshooter.accessBundles` に追加し、`helm upgrade` を実行します。
  </Tab>
...
ClickHouse ユーザーへの権限付与は、`--ch-user-via cr` ではなく上記のとおり SQL で行ってください。サービス の定義は ClickHouse Cloud が管理して再適用するため、そこに追加したユーザーは削除されてしまいます。サービス を削除すると、executor は VM 上のトラブルシューターの bundle も削除します。Kubernetes の場合は、[アンインストール](/ja/products/bring-your-own-cloud/connector/operations#uninstall-kubernetes) に記載のとおり、bundle の Secret と ServiceAccount は手動で削除するまで残ります。

## サービスの削除 {#delete-a-service}

コネクタエンドポイント経由で顧客が実行できる削除コマンドはありません。サービスの削除は ClickHouse のアカウントチームに依頼してください。ClickHouse Cloud がサービスを終了し、executor がワークロードとそのネームスペースを削除します (`terminating`、その後 `terminated`) 。AWS 側には一切手が加えられません。バケット、そのデータ、および IAM ロールは、自分で削除するまで残り続けます。

サービスが `terminated` を報告したら、`prepare` が作成したリソースを撤去します。`prepare` を実行したのと同じ場所で、同じ AWS 認証情報 を使って実行してください。Kubernetes の場合は、同じワークステーション、同じ設定のコピーと出力ディレクトリ、そして executor へのポートフォワードが開いていることが前提となります。
...
このコマンドは、サービスに関する executor の記録、すなわちデータの所在と割り当てられていたロールを読み取ります。そのうえで `prepare` が作成した IAM ロールを削除し、executor にそのサービスを忘れさせることで名前を解放します。VM の場合は、サービスのクラスター全体にまたがる RBAC オブジェクト、ローカルのアクセスバンドル、レジストリのエントリも削除します。
...
executor が接続されていない間、ClickHouse Cloud は新しい作業を割り当てられません。すでに受理済みの作成、削除、プラットフォーム同期は保持され、再試行されます。作成は最後の進捗報告から 30 分間、削除は 2 時間、プラットフォーム同期は最大 10 回まで再試行されます。この上限を超えると、コネクタエンドポイントはそのコマンドを失敗としてマークします。その他のライフサイクルコマンド (スケール、停止、開始、再起動、バックアップ、バックアップ の削除) は、executor が接続されていない間はキューに入れられることなく拒否されます。executor が接続されていない間に送信した作成や削除も同様に拒否され、再試行されるのはすでに受理済みのコマンドのみです。

executor がすでに受理したコマンドは完了まで実行され続け、報告できなかった結果は次回の接続時に executor から送信されます。

2 つの顧客向けコマンドには、それぞれ固有の依存関係があります。`instances create` は、実行元のホストからコネクタエンドポイントに到達できる必要があります。`--wait`、`instances list`、`instances get`、`commands list` は executor のローカル API を利用するため、executor が稼働している必要があります。プラットフォーム承認のトークンは、接続状態に関係なく独自のタイマーで有効期限を迎えます。[承認ウィンドウ](/ja/products/bring-your-own-cloud/connector/platform-updates#the-approval-window)を参照してください。