...
managed モード では executor が動作し、ClickHouse Cloud がクラスターに対して適用するサービスライフサイクルを反映します。`init` は登録時に managed モード を有効化し、クラスターの identity を補完します ([enable managed mode](/ja/products/bring-your-own-cloud/connector/managed-services#enable-managed-mode) を参照) 。以下のオプションは、その後に見直す可能性があるものです。managed モード を有効にせずにインストールしたコネクタで managed モード を有効化する場合、または無効化する場合は、`--force` と `--managed` または `--no-managed` を指定して `init` を再実行してください。`--no-managed` は executor を停止しますが、クラスターに対する grant はそのまま残ります。削除方法は [stop managed mode](/ja/products/bring-your-own-cloud/connector/reference/faq#stop-managed-mode) を参照してください。他の `init` の実行と同様に、entry point の指定が必要です。bundle を保持している場合は `--handoff` を、新しいトークンを使用する場合は `--enroll` を指定します。詳細は [re-runs and recovery](/ja/products/bring-your-own-cloud/connector/operations#re-runs-and-recovery) を参照してください。
