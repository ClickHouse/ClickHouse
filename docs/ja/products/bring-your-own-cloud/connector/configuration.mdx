...
このページでは、コネクタのインストール後に特に変更する可能性が高い設定について説明します。各キーのデフォルト値と意味については[設定リファレンス](/ja/products/bring-your-own-cloud/connector/reference/configuration)を、コマンドフラグについては[CLI リファレンス](/ja/products/bring-your-own-cloud/connector/reference/cli)を参照してください。サービスの作成、スケーリング、削除は設定には含まれません。[マネージドサービス](/ja/products/bring-your-own-cloud/connector/managed-services)を参照してください。
...
<Note>
  公開されている `v0.16.0` の CLI には `--managed` も `--no-managed` もありません。いずれかの flag を使用して設定を変更する前に、[マネージドモードの利用可否](/ja/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) を確認し、account team から対応する build を入手してください。
</Note>

managed モード では executor が動作し、ClickHouse Cloud がクラスターに対して適用するサービスライフサイクルを反映します。`init` は登録時に managed モード を有効化し、クラスターの identity を補完します ([enable managed mode](/ja/products/bring-your-own-cloud/connector/managed-services#enable-managed-mode) を参照) 。以下のオプションは、その後に見直す可能性があるものです。設定されている状態を変更するには、`--force` と `--managed` または `--no-managed` を指定して `init` を再実行してください。他の `init` の実行と同様に、entry point の指定が必要です。bundle を保持している場合は `--handoff` を、新しいトークンを使用する場合は `--enroll` を指定します。詳細は [re-runs and recovery](/ja/products/bring-your-own-cloud/connector/operations#re-runs-and-recovery) を参照してください。

VM 上では、`--no-managed` は設定上で executor を無効化しますが、すでに動作している systemd unit を停止したり無効化したりはしません。設定を変更する前に、明示的に停止してください。

```bash
sudo systemctl disable --now clicklink-executor
```

Kubernetes では、values に `executor.enabled: false` を設定し、`helm upgrade` で変更を適用すると、executor のデプロイメントが削除されます。いずれの target でも、クラスターに対する grant は削除するまでそのまま残ります。この access も削除するには、[stop managed mode](/ja/products/bring-your-own-cloud/connector/reference/faq#stop-managed-mode) に従ってください。
...
<Tabs>
  <Tab title="Kubernetes" id="managed-mode-kubernetes">
    ...
    * **`cluster.namespacePrefix`** はすべてのコンポーネントで共有され、executor の grant による適用範囲を限定するプレフィックスです。インストール後に変更した場合は、grant を再実行する必要があります。
...
<Tabs>
  ...
  <Tab title="Linux VM" id="instances-vm">
    ...
    次に、root としてホスト上で各コンポーネントのアクセスをプロビジョニングします。VM では、`--ch-user-via cr` を指定しない限り管理者パスワードが必須です。`--ch-admin-password-stdin` でパイプ経由で渡してください。各コマンドは ClickHouse 権限を適用し、対応するデーモンを再起動します (`--skip-restart` を指定すると再起動をスキップできます) 。
...
operator のメールアドレスの許可リストが、ゲートウェイで管理されるセッションを制限します。セッションゲートウェイへのすべてのリクエストには、証明済みメールアドレスがリストに含まれている、短期間有効な OIDC ID トークンが必要です。許可リストが空の場合、ゲートウェイは閉鎖され、誰もゲートウェイ経由でセッションを開始できません。VM では、ホストの root ユーザーはローカルのセッションファイルを通じてセッションを直接管理することもできます。許可リストはゲートウェイ経由の経路にのみ適用されます。信頼モデルの詳細については、[サポートセッション](/ja/products/bring-your-own-cloud/connector/support-sessions)を参照してください。
...
Kubernetes では、このチャートは egress の allowlist を伴うデフォルト拒否の NetworkPolicy を同梱しています。NetworkPolicy オブジェクトは、CNI がこれを適用する場合にのみ有効になります。適用を行う CNI 配下では、2 つの CIDR リストの両方が入力されるまで、到達できるのはクラスター DNS と ClickHouse Service のネームスペースのみです。それまでは、コネクタから connector endpoint や Kubernetes API server への egress はありません。そのため `init` は、connector endpoint の背後にある CIDR が判明している場合にのみ、ポリシーを enabled の状態でステージングします。
...
* **`allowEgressCIDRs`** は利用者側で指定します。コネクタエンドポイントの背後にある範囲を、インストール時に `--egress-cidrs` で指定するか、プロンプトで入力します。すべてのコンポーネントが、コネクタエンドポイントに到達するためにこれを必要とします。managedモードでプラットフォームのチャートがAmazon ECRに置かれている場合は、対象リージョンのECRおよびSTSの範囲 (あるいは自身のミラーの範囲) も追加してください。これらがないと、ポリシーによってexecutorのチャート取得が失敗します。
...
トラブルシューター は、出力が境界外に送信される前にマスキングします。組み込みパターンでは、`ipv4`、`ipv6`、`bearer-token`、`aws-access-key`、`email`、`jwt`、`ssh-private-key`、`connection-string-credentials` を対象とします。独自のパターンは YAML ファイルに追加できます。独自のパターンはファイル内の記述順に最初に実行され、その後に組み込みパターンが実行されます。また、組み込みパターンと同じ `name` を使用するエントリは、その組み込みパターンを置き換えます。
...
VM では、ファイルは `/etc/clicklink/redaction-patterns.yaml` にあります。インストーラーは、有効なパターンの初期セット (third-party の API key、AWS のシークレットキーなど) を投入します。このセットは必要に応じて削減または拡張できます。インストーラーはアップグレード時にも独自のバージョンを保持します。Kubernetes では、YAML を `redaction-patterns.yaml` キーの ConfigMap に格納し、`troubleshooter.redaction.patternsConfigMap` にその名前を設定します。チャート はこれを同じパスにマウントします。
...
以下のマネージドモードの例には `--managed` を備えたビルドが必要ですが、公開されている `v0.16.0` にはこれが含まれていません。使用する前に[マネージドモードの利用可否](/ja/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)を確認してください。公開リリースをミラーしても、このサポートが追加されるわけではありません。
...
<Tabs>
  ...
  <Tab title="Linux VM" id="storage-vm">
    スクレイパーは、コネクタエンドポイントに到達できない間、at-least-once配信のためにメトリクスを `/var/lib/clicklink/buffer` にスプールします。保持期間は最大168時間、サイズは最大1024 MBで、アップロードのレートはデフォルトで1 MB/sに制限されます。
