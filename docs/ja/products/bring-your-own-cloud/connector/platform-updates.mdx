...
<Note>
  公開されている `v0.16.0` の CLI には、`platform approve` や `platform reset` を含む `platform` コマンドがありません。以下の Linux VM のワークフローも Kubernetes のワークフローも、そのリリースでは利用できません。さらに Kubernetes での承認には、Secret ベースのトークンバンドル (`--secret-namespace`) に対する CLI および executor チャートのサポートが必要です。いずれかのワークフローを使用する前に、[マネージドモードの利用可否](/ja/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)をアカウントチームにご確認ください。
</Note>
...
以下のどちらの承認手順でも、[プラットフォームサポートを含むビルド](/ja/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)が必要です。公開版の `v0.16.0` には `platform` コマンドがありません。Kubernetes での承認では、Secret の配信に対応した CLI とチャートのサポートも必要です。

レジストリの認証方式は、環境に登録されているイメージの配布モードによって異なります。

* **ClickHouse のレジストリへの直接アクセス。** 承認はコネクタの EC2 VM 上で実行します。`approve` と executor の platform sync はいずれも、EC2 instance metadata service から取得した認証情報を使用して、環境の読み取り専用 ECR プラーロールを assume します。チャートへのログインにはこのロールを使用し、executor はプラットフォームイメージの存在確認にもこのロールを使用します。ワークステーション上の AWS プロファイル、環境変数による認証情報、SSO 認証情報は instance profile の代わりにはなりません。権限側の半分を適用するためのクラスター管理者の認証情報は、引き続き kubeconfig から提供されます。
...
以下の Kubernetes ワークステーション手順は、Kubernetes プラットフォーム承認を有効にしたミラーレジストリ構成のデプロイメントでのみ使用してください。直接アクセスの場合は、作業を進める前に、必要な EC2 instance profile を備えたサポート対象の実行環境であることを account team に確認してください。
  ...
  <Step title="承認を実行" id="run-approve">
    ...
    <Tabs>
      ...
      <Tab title="Kubernetes" id="approve-kubernetes">
        ミラーレジストリを使用するデプロイメントで Kubernetes のプラットフォーム承認が有効になっている場合、`approve` は前の手順で設定したポートフォワード経由でステージングされたバンドルを読み取ります。トークンバンドルはコネクタのネームスペース内の Secret として配信され、チャートがそれをポッドにマウントします。kubeconfig からクラスターに到達できるワークステーションで、コネクタの設定のコピーと上記のレジストリへのアクセスを用意して実行してください。チャートはエグゼキューターの ConfigMap に設定をレンダリングします。`approve` はネームスペースのプレフィックスを得るためにこれを必要としますが、そこから認証情報を読み取ることはありません。
...
公開されている `v0.16.0` の CLI には `platform reset` コマンドはありません。この手順を使用する前に[利用可否](/ja/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)を確認してください。
