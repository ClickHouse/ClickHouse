...
`--force` は保持されている設定またはオーバーレイを上書きし、クライアント秘密鍵を再生成して、有効期限が切れていないクライアント証明書を置き換えます。新しいクラスター UUID が発行されることはないため、コネクタ&#39;のアイデンティティは保持されます。置き換えられる設定のマネージドモードの状態は、無人での再実行時には維持され、端末上ではデフォルトとして提示されます (`--managed` または `--no-managed` を指定した場合を除く) 。
...
**コネクタがオフラインの場合。** コネクタが停止している、または到達不能な間も、コネクタが管理する services は稼働し続けます。クラスター内の ClickHouse operator がそれらを維持し、executor はデータパス上には存在しません。ライフサイクルの変更には executor の接続が必要なため、再接続されるまで変更は一切適用されません。再起動時、executor は停止時点で処理中だったコマンドを自身のコマンド database から復旧します。ClickHouse Cloud が再送したコマンドは、2 回実行されるのではなく記録済みの結果を使って応答されます。再実行されるのは失敗したコマンドのみです。executor が接続されていない間にライフサイクルコマンドがどうなるかについては、[コネクタがオフラインの場合](/ja/products/bring-your-own-cloud/connector/managed-services#when-the-connector-is-offline)を参照してください。
