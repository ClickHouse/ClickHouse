...
各デーモンは、ヘルスポートで `/livez` エンドポイントを公開します。レスポンスボディの JSON `status` フィールドがヘルス状態を示すものであり、HTTP ステータスコードではありません。したがって、`200` ではなくボディを確認してください。Prometheus メトリクスは同じポートの `/metrics` で公開され、メトリクス専用のポートはありません。両方のターゲットのデフォルトポートは次のとおりです。
...
エグゼキューターは、ローカルのコマンド API を `127.0.0.1:9999` でも公開します。これ自体は認証を持たず、ホストやポッドの外部に公開されることはありません。`clicklink clctl` のサービスコマンドはローカルでこれに接続します。
...
ここで説明する managed モードの動作には[サポート対象のビルド](/ja/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)が必要です。公開されている `v0.16.0` には `--managed` も `--no-managed` もありません。

ClickHouse のアカウントチームに新しい登録トークンを依頼し、元の `init` コマンドに `--enroll` と `--force` を付けて再実行します。`--force` は保持されている設定を再ステージングするため、初回インストール時に指定したターゲット固有のフラグ (`--target-namespace`、`--values`、および `--chart`、`--chart-repo`、`--chart-version` のミラーフラグ) をすべて維持してください。Managed モードは置き換え対象の設定から引き継がれるため、ここでは `--managed` は任意です。デフォルトのインストールでは、以下のようになります。
...
Kubernetes では、`init` がローテーション後の `clicklink-hmac` および `clicklink-mtls` Secret を事前に作成するため、チャート側ではこれらをレンダリングしません。そのため、チャートの checksum annotation はこれらを対象に含まず、稼働中のポッドは起動時の認証情報を保持し続けます。ローテーション後は、すべてのコンポーネントを再起動してください。
...
VM では、admin パスワードをパイプで渡してホスト上で実行します。VM では、`--ch-user-via cr` を指定しない限り、パスワードは必須です：
...
<Note>
  公開されている `v0.16.0` の CLI には `--managed` も `--no-managed` もありません。以下で説明するmanaged モードの保持および上書きの動作には、[managed モードのサポート](/ja/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)を含むビルドが必要です。復旧中にそのモードを変更する前に、account team にビルドを確認してください。
</Note>
...
**コネクタがオフラインの場合。** コネクタが停止している、または到達不能な間も、コネクタが管理する services は稼働し続けます。クラスター内の ClickHouse operator がそれらを維持し、executor はデータパス上には存在しません。ライフサイクルの変更には executor の接続が必要なため、再接続されるまで変更は一切適用されません。再起動時、executor は停止時点で処理中だったコマンドを自身のコマンドデータベースから復旧します。ClickHouse Cloud が再送したコマンドは、2 回実行されるのではなく記録済みの結果を使って応答されます。再実行されるのは失敗したコマンドのみです。executor が接続されていない間にライフサイクルコマンドがどうなるかについては、[コネクタがオフラインの場合](/ja/products/bring-your-own-cloud/connector/managed-services#when-the-connector-is-offline)を参照してください。
...
executor の PersistentVolumeClaim には `helm.sh/resource-policy: keep` のアノテーションが付与されているため、アンインストールしてもコマンドデータベースとサービスレジストリを保持したまま残ります。`init` が作成した Secret、プラットフォーム承認によって書き込まれたプラットフォームバンドル Secret、および executor が管理するサービスレジストリの ConfigMap も同様にチャートの管理対象外であり、アンインストール後も残ります。これらは明示的に削除してください。その際、自分で登録したすべてのインスタンス、およびトラブルシューターをプロビジョニングしたすべてのマネージドサービスに対応するアクセス Secret と ServiceAccount のペア (サービス名に基づく名前) も削除対象に含めてください。`--ignore-not-found` を指定しておけば、初回承認前のプラットフォームバンドル Secret のように、そもそも作成されていないオブジェクトがあっても問題ありません:
