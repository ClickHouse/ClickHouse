...
端末上で実行する場合、`init` は managed モードを有効にするかどうかを尋ね、既定で「はい」が選択されています。スクリプトから実行する `init` では、`--managed` を指定した場合にのみ有効になります。managed モードを使わずに実行し、お客様自身で運用するクラスターに対してオブザーバビリティとサポートのみを利用することもできます。[managed モードなしで実行する](#run-without-managed-mode)を参照してください。
...
* **`prepare`、`teardown`、`platform approve` 用の AWS 認証情報。** service の buckets と IAM role の作成、および service 削除後のそれらの削除は、お客様の workstation または connector ホスト上で、お客様の AWS 認証情報を使用して実行されます。ステージングされたチャートが Amazon ECR にある場合、`platform approve` には Amazon ECR を読み取れる AWS 認証情報も必要です。executor がお客様の buckets や IAM に対する AWS 認証情報を保持することはありません。executor のクラウド呼び出し先は Amazon ECR と STS のみです。プラットフォームの sync または create がチャートを取得する際に registry にログインします。プラットフォーム更新中は、読み取り専用の pull role でイメージを確認します。S3 や IAM への呼び出しは行いません。
* **到達可能な ClickHouse ネイティブ listener** (お客様自身が運用する クラスター向け。`--no-managed` を指定してインストールした別の connector デプロイメントによって監視されます) 。scraper と トラブルシューター は各 クラスターとネイティブプロトコルで通信します。Kubernetes では、セキュア (9440) か平文 (9000) かを自動検出します。
