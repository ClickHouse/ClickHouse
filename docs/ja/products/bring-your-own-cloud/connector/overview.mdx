...
<Note>
  ここで説明するmanaged モードのワークフローは、一般提供されているコネクタのリリースでは未対応です。インストール前に、必要なビルドおよびデプロイのサポート状況をアカウントチームにご確認ください。[managed モードの提供状況](/ja/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)を参照してください。
</Note>

## コネクタの役割 {#what-the-connector-does}

ClickHouse Connector は、お客様自身の環境内にデプロイするコンポーネントです。これにより、ClickHouse Cloud はお客様が運用する Kubernetes クラスター上の ClickHouse サービスを管理・監視・サポートできるようになります。ClickHouse Cloud からお客様の環境へ接続することは一切ありません。コネクタはアウトバウンド接続のみを行い、ClickHouse サービスにスコープが限定されたアイデンティティで動作します。コネクタは単一のバイナリ `clicklink` として提供され、各デーモンと、それらのインストールおよび運用に使用する `clicklink clctl` ツールが含まれています。
...
[managed モードをサポートするビルド](/ja/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)では、端末上で実行する場合、`init` は managed モードを有効にするかどうかを尋ね、既定で「はい」が選択されています。スクリプトから実行する `init` では、`--managed` を指定した場合にのみ有効になります。managed モードを使わずに実行し、お客様自身で運用するクラスターに対してオブザーバビリティとサポートのみを利用することもできます。[managed モードなしで実行する](#run-without-managed-mode)を参照してください。

コネクタが行う接続はすべてアウトバウンドです。ClickHouse Cloud からお客様の環境へ接続を開始することは一切ありません。[アーキテクチャ](/ja/products/bring-your-own-cloud/connector/architecture)のページでは、すべての接続とそのプロトコル、および認証方法を一覧しています。
...
* クラスターへの書き込みは、ClickHouse が管理するサービスのネームスペースに限定されます (例外は、executor 自身のホームネームスペースにあるトークン、registry、更新された証明書のみです) 。これらのサービスが依存するプラットフォームコンポーネントは、各更新をユーザーが承認した場合にのみ変更されます。
* データは、自社で作成・削除する bucket 内、および自社の IAM role の背後に留まります。executor は自社の bucket や IAM role への認証情報を一切保持しません。
* サポートアクセスは時間制限付きで、有効化するのも、いつでも取り消すのも自社側で行えます。サポートセッション中のコマンドはすべて、自社が所有する監査ログに記録されます。

## 仕組み {#how-it-works}

以下の登録およびサービス作成のフローには、アカウントチームによって確認された [managed モードのサポート](/ja/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) が必要です。公開版の `v0.16.0` では使用できません。

ClickHouse はオンボーディング時にお客様の環境を登録し、コネクタエンドポイントと使い捨ての登録トークンを発行します。`clicklink clctl init --enroll` は一度だけ実行します。Kubernetes にインストールする場合は kubeconfig にアクセスできる作業端末から、Linux VM にインストールする場合はそのホスト上で実行します。このコマンド 1 つで、トークンの引き換え、mTLS クライアント証明書の取得、デーモンのデプロイと起動、およびそれらのヘルス確認までが完了します。

`init` は `--managed` を指定すると managed モードを有効にします。端末上では確認が求められ、デフォルトは「はい」です。有効にすると executor がデプロイされ、その Kubernetes アクセス権限が適用されます。この権限は `pcm-executor` ServiceAccount として付与されます。admission ポリシーにより、書き込み先はサービスのネームスペース (デフォルトでは接頭辞 `ns-`) に限定されます。ただし、自身のトークン更新、1 つの registry ConfigMap、および Kubernetes 上では自身のホームネームスペース内の更新済みクライアント証明書は例外です。

サービスの作成は 2 つのコマンドで行います。`clicklink clctl executor prepare` は、お客様自身の AWS 認証情報を使ってサービスの bucket と IAM role を作成し、default ユーザーのパスワードを一度だけ表示します。`clicklink clctl instances create` は、コネクタエンドポイントを介してサービス定義を ClickHouse Cloud に送信します。以降、ClickHouse Cloud は executor のアウトバウンドコマンドチャネルを通じてサービスを制御します。具体的には、スケーリング、停止と開始、再起動、バックアップ、バージョンアップグレード、設定変更です。詳細な手順は [マネージドサービス](/ja/products/bring-your-own-cloud/connector/managed-services) を参照してください。
