...
サポートセッションでは、ClickHouse Connector を介して ClickHouse への一時的な診断アクセスを付与できます。このページでは、セッションの概要、有効化と無効化の方法、セッション中に ClickHouse Support エンジニアが実行できる操作、およびその監査方法について説明します。

## サポートセッションとは {#what-a-support-session-is}

サポートセッションとは、トラブルシューターが ClickHouse Support エンジニアからのコマンドを受け付ける、時間制限付きの期間です。セッション外では、送信方向の WebSocket が接続されていても、トラブルシューターはあらゆるコマンドを拒否します。これ以外の実行経路はありません。トラブルシューターはセッションなしでは何も実行せず、ClickHouse が代わりにセッションを開くこともできません。ClickHouse Cloud がお客様の環境へ接続することは一切なく、受け取るのはトラブルシューターが送信方向のチャネルを通じて送信した内容のみです。
...
<Tabs>
  <Tab title="Kubernetes" id="sessions-kubernetes">
    ...
    呼び出し元の OIDC アイデンティティは、オペレーター の許可リストに含まれている必要があります。認証されていない呼び出し元や許可リストにない呼び出し元には 401 または 403 が返され、ゲートウェイがその試行をログに記録します。クラスター認証情報を必須にしたくない場合は、チャート のオプトインのイングレスを介してゲートウェイを公開してください。このイングレスは CA 発行の証明書で TLS を終端します。詳細は[設定](/ja/products/bring-your-own-cloud/connector/configuration)を参照してください。
  </Tab>
...
セッションは自動的に期限切れになります。デフォルトの有効期間は4時間で、`session enable --duration` は最大24時間まで指定できます。セッションの期限が切れるか、`session disable` を実行した時点で、トラブルシューターはコマンドを受け付けなくなります。無効化は即時の失効手段です。再起動やClickHouseとの協調は必要ありません。

## オペレーターの許可リスト {#operator-allowlist}

すべてのゲートウェイ呼び出しは、オペレーターのメールアドレスの許可リストに基づき認可されます。照合は検証済みの OIDC トークンで証明されたメールアドレスに対して行われ、クライアントが自身について申告した内容に基づいて照合されることはありません。
...
いずれの環境でも、ゲートウェイは前回の読み取りから 30 秒が経過すると次のリクエスト時にファイルを再読み込みするため、編集内容は再起動なしで反映されます。
...
セッション中に実行されたすべてのコマンドは、1行につき1つのJSONオブジェクト (NDJSON) の形式で `/var/log/clicklink/troubleshoot-audit.log` に追記されます。ブロックされたコマンドも記録され、`blocked_by` にその理由 (例: `session inactive`) が示されます。`submitted_by` はコマンドチャネル上の組織IDで、これはご自身の組織IDです。個々のClickHouseエンジニアは、このファイルではなくClickHouse Cloud側に記録されます。セッションコマンドのエントリは次のようになります:
...
ゲートウェイの呼び出しは別途記録されます。拒否されたものも含め、すべての呼び出しは構造化された `clctl.gateway` 行としてトラブルシューター自身のログ (VM ではジャーナル、Kubernetes ではコンテナーログ) に出力されます。この行には、`submitted_by` (検証済みトークンによって証明されたメールアドレスであり、クライアントから渡された値ではありません) 、`command_type` (`clctl.session.enable`、`clctl.session.disable`、`clctl.session.status`) 、`reason` (enable 時の `--reason`) 、`remote_addr`、`status` が含まれます。`status` は `ok`、`unauthorized`、`forbidden`、`rate_limited`、`bad_request`、`conflict`、`error` のいずれかであるため、拒否されたアクセスもログで確認できます。
