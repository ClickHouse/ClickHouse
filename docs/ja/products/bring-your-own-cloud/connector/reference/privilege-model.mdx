...
title: '権限モデル'
sidebarTitle: '権限モデル'
description: 'ClickHouse Connector に何ができて何ができないのかを正確に解説します。アイデンティティ、送信接続、ClickHouse 権限、Kubernetes RBAC、クラウドへのアクセス、アトリビューション、データ最小化について説明します'
...
<Note>
  ここで説明する managed モードのワークフローを利用するには、アカウントチームによる確認を経た [対応する CLI とデプロイメントのサポート](/ja/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) が必要です。公開されている `v0.16.0` には `init --managed` も `init --no-managed` もなく、customer-create ワークフローや `platform` コマンドも含まれていません。以下のアクセス許可は、そのサポートが利用可能になった時点でのワークフローについて説明しています。
</Note>

## アイデンティティ {#identities}

managed モードでの登録、顧客サービスの作成、プラットフォーム承認は、[サポートされているビルドおよびデプロイメント](/ja/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)上でのみ、これらのアイデンティティを使用します。
...
* **コネクタ EC2 VM のインスタンスプロファイル**は、プラットフォームへの直接アクセスのために使用され、チャートのログインとイメージチェックの両方で、お客様の環境の読み取り専用 ECR プラーロールを assume するために使われます。承認 CLI も同じインスタンスプロファイルの経路を使用します。workstation の AWS プロファイルや SSO 認証情報がこれを置き換えることはありません。その他の ECR チャートレジストリでは、ambient な AWS 認証情報が使用されます。S3 や IAM の呼び出しは行いません。イメージのプルは、お客様のノード上でそれぞれのプルロールのもとで実行されます。executor は、お客様のバケットや IAM に対する認証情報を一切保持しません。[レジストリ認証情報](/ja/products/bring-your-own-cloud/connector/platform-updates#registry-credentials)を参照してください。
* **お客様自身の認証情報**が、あらゆる grant を適用します。具体的には、`init --managed` における executor のアクセス grant、`prepare` におけるバケットとロール、`approve` における各プラットフォーム更新の permission に関する部分、そして `teardown` におけるクリーンアップです。

## コネクタでできること {#what-the-connector-can-do}

### 送信接続 {#outbound-connections}

以下は、コネクタが開く接続の完全な一覧です。いずれもお客様の環境内から発信されます。
...
| お客様の ClickHouse インスタンス                 | ClickHouse ネイティブプロトコル                                                                                                      | `pcm_scraper` および `pcm_troubleshooter` としての読み取り専用クエリ、および scraper のログフラッシュステートメント ([grants](#clickhouse-grants) を参照) 。executor は ClickHouse への接続を開きません                                                                                                                      |
...
スクレイプクエリでは各システムテーブルが `clusterAllReplicas()` でラップされるため、`READ ON REMOTE` が必要です。`SYSTEM FLUSH LOGS` は唯一のシステムレベルの権限です。これはスクレイプが最新のデータを参照できるよう、すでにバッファリングされているエントリを `*_log` システムテーブルに永続化するよう強制するものであり、何かを読み取ったり変更したりすることはできません。ClickHouse はこの権限をグローバルスコープでのみ受け付け、有効になるのはログテーブルに対してのみであるため、付与範囲は実際に利用できる機能よりも広くなります。
...
両方のユーザーに対する `system.user_directories` 権限は、1 つの診断のために必要です。`clicklink clctl preflight` はコネクタ自身の認証情報で実行され、インスタンスが ClickHouse ユーザーをどのように保存しているか (レプリケートかローカルか) を確認します。このテーブルにはユーザーデータではなくユーザーストレージの設定メタデータが格納されています。スクレイプ対象セットにもセッションテーブルの許可リストにも含まれていないため、スクレイプまたはセッションの出力パスから読み取られることはありません。この権限がない場合、この 1 つの事前チェックはスキップとして報告されますが、その他の処理はすべて続行されます。
...
以下の `init --managed` ワークフローは、公開版 `v0.16.0` では利用できません。使用する前に [managed モードの利用可否](/ja/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) を確認してください。

managed モードでは、`clicklink clctl init --managed`(または `clicklink clctl executor access grant --cluster` に続けて `apply --cluster`)が、ユーザーの認証情報を使って executor の grant を適用します。この grant は、`pcm-executor` ServiceAccount(Kubernetes ではコネクタのネームスペース内、VM インストールでは `clicklink-system` 内)、それにバインドされた 1 つの クラスター ロール、および `pcm-executor-prefix-guard` ValidatingAdmissionPolicy で構成されます。
...
| `roles`、`rolebindings`(`rbac.authorization.k8s.io`) | `get`、`create`、`update`、`patch`                         | 各 service ネームスペース内における scraper の読み取り専用アクセス。Kubernetes の escalation 防止機構により、これらは executor がすでに保持しているアクセス許可の範囲に制限されます。executor は troubleshooter 向けには何も provision しません。[マネージドサービスにおけるサポートセッション](/ja/products/bring-your-own-cloud/connector/managed-services#support-sessions) を参照してください |
...
#### プラットフォーム identity {#kubernetes-rbac-platform}

公開版の `v0.16.0` CLI には `platform` コマンドがありません。この承認モデルを利用するには、[対応する CLI とデプロイメントのサポート](/ja/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)が必要です。

`clicklink clctl platform approve` は、プラットフォーム更新のうち権限に関わる部分を、ユーザーの認証情報で適用します。対象は、ネームスペース、CustomResourceDefinition、ServiceAccount、クラスター ロールおよび ClusterRoleBinding、ロールおよび RoleBinding、webhook および admission の設定、PriorityClass、StorageClass です。続いて、`pcm-platform` ServiceAccount、その クラスター ロール、`pcm-platform-scope-guard` ValidatingAdmissionPolicy、およびそのバインディングが存在することを確認したうえで、有効期間 2 時間の bound トークンを発行します。
...
* **ClickHouse のデータや状態への書き込みはできません。** 上記の grant には `INSERT` も DDL も含まれず、ユーザー管理・設定・プロセス制御の権限もありません。唯一のシステム系 grant である scraper の `SYSTEM FLUSH LOGS` は、ログテーブルがすでにバッファしている内容を永続化させるだけです。executor は ClickHouse ユーザーを持ちません。いかなるコンポーネントも、ClickHouse 経由でデータ、スキーマ、ユーザー、設定を変更することはできません。
...
* **アクセス許可を付与する権限はありません。** プラットフォームの Namespace、CRD、ServiceAccount、RBAC、webhook および admission の設定、PriorityClass、StorageClass は、`approve` の実行時にお客様の認証情報で適用されます。executor のプラットフォーム identity は、権限昇格も bind も impersonate も行えません。サービスのネームスペース内では、executor はすでに自身が保持しているアクセス許可を付与する Role しか作成できません。
* **お客様の bucket や IAM の認証情報は持ちません。** `clicklink clctl executor prepare` が bucket とサービスごとの role を作成し、`clicklink clctl executor teardown` がそれらを削除します。いずれもお客様の認証情報で実行されます。その role を assume できるのは当該サービスのポッドだけです。プラットフォームへの直接アクセスでは、executor はコネクタの EC2 VM のインスタンスプロファイルを使用して読み取り専用の ECR プラー role を assume し、チャートへのログインとイメージ確認の両方を行います。その他の ECR チャートレジストリでは、[identities](#identities) で説明しているとおりアンビエントな AWS 認証情報を使用します。クラウド API の呼び出し先は ECR と STS のみであり、お客様のデータを読み取ったり削除したりすることはできません。
...
* **インバウンドの通信は一切ありません。** ClickHouse Cloud がお客様の環境に向けて connection を開くことはありません。コマンドの経路は 2 本のアウトバウンド WebSocket のみです。troubleshooter は、お客様が有効化したサポートセッションがアクティブでない限り、すべてのコマンドを拒否します。executor は、お客様が ClickHouse に管理を依頼したサービスに対してのみ、上記のスコープ内でライフサイクルコマンドを受け付けます。session 中であっても、troubleshooter の到達範囲は双方で制限されます。ClickHouse へのクエリはテーブルの許可リストに限定され、`query_log` と `text_log` は拒否されるため、これらが許可されることはありません。Kubernetes へのアクセスは、ネームスペーススコープの Role が付与する読み取り専用のビューとポッドのログに限定されます。

## 対応が必要な項目 {#what-requires-your-action}

以下のmanaged モードの登録、サービス作成、プラットフォーム承認の各コマンドを使用する前に、account teamに[提供状況](/ja/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)を確認してください。これらのワークフローは公開版の `v0.16.0` では使用不可です。

* **マネージドモードの有効化。** executorが動作し、そのKubernetes grantが存在するのは、`init`でマネージドモードを有効にした後のみです。grantはクラスター管理者権限で適用されます。[マネージドサービス](/ja/products/bring-your-own-cloud/connector/managed-services#enable-managed-mode)を参照してください。
...
* **デプロイメントのアイデンティティ。** mTLS クライアント証明書のコモンネームは組織 ID であり、エンドポイントホストに紐付いた単一の DNS name を持つため、すべての API 接続を組織に帰属させられます。更新は自動的にデーモン内で行われ、鍵素材を扱う operator は存在しません。
...
* **プラットフォームの承認。** 承認した bundle のダイジェストは `pcm-platform` ServiceAccount に記録されます。また、認証情報によって適用されたすべての object にはコンテンツハッシュが付与され、executor は書き込みを行う前にこれを検証します。それ以外のダイジェストを持つ bundle は拒否されます。
...
* **`query_log` はデフォルトで scrape 対象から除外されます。** そのカラムにはリテラル値を含む生の SQL が格納されており、個人データやシークレットが含まれる可能性があるため、明示的に追加しない限り境界の外に出ることはありません。
* **トラブルシューターは allowlist に登録されたテーブルのみを読み取ります。** `query_log` と `text_log` は allowlist の設定自体で拒否されるため、許可されることはなく、クエリ履歴が読み取られることもありません。デフォルトの allowlist には `system.processes` (実行中のクエリテキスト) が含まれています。セッション中もこれを秘匿する必要がある場合は、この設定 (Kubernetes では `troubleshooter.allowedTables`、VM では `troubleshooter.allowed_tables`) から削除してください。
