...
* **Troubleshooter の ServiceAccount** は、プロビジョニング済みのデプロイメントごとに 1 つ存在し、support session で利用できる読み取り専用の Kubernetes ビューを提供します。
...
* **executor のノードまたはホストが持つ ambient な AWS アイデンティティ**には 2 つの用途があります。1 つは、プラットフォーム同期や作成処理でチャートを取得する際に、STS を介して Amazon ECR にログインすることです。もう 1 つは、プラットフォーム更新中のイメージチェックのために登録した読み取り専用のプルロールを assume することです。S3 や IAM の呼び出しは行いません。イメージのプルは、お客様のノード上でそれぞれのプルロールのもとで実行されます。executor は、お客様のバケットや IAM に対する認証情報を一切保持しません。
...
| Amazon ECR                             | HTTPS、executor の ambient な AWS identity、またはイメージチェックのために assume する読み取り専用の pull ロール | プラットフォームの同期または作成が ECR からチャートを取得する際のレジストリログイン。プラットフォーム更新中にイメージの存在を確認するための `DescribeImages`                                                                                                                                                                                   |
| AWS STS                                | HTTPS、executor のノードまたはホストの ambient な AWS identity                                 | ECR ログイン用の認証情報、およびイメージチェック用に登録した読み取り専用 pull ロールへの assume-role。ECR と STS は executor が呼び出す唯一のクラウド API です                                                                                                                                                                      |
| クラスターノードからのコンテナレジストリ                   | HTTPS                                                                             | マネージドサービスおよびプラットフォームコンポーネントのイメージ取得。ClickHouse がお客様の登録したノードロールに付与する読み取り専用 pull ロール経由、またはイメージをインポートした後のお客様独自のレジストリから行われます                                                                                                                                                     |
...
* **お客様の bucket や IAM の認証情報は持ちません。** `clicklink clctl executor prepare` が bucket とサービスごとの role を作成し、`clicklink clctl executor teardown` がそれらを削除します。いずれもお客様の認証情報で実行されます。その role を assume できるのは当該サービスのポッドだけです。executor が自身のノードまたはホストのアンビエントな AWS identity を使うのは、次の 2 つの呼び出しに限られます。チャートを取得する際に STS を介して Amazon ECR にログインする場合と、プラットフォーム更新時のイメージ確認のために読み取り専用の pull role を assume する場合です。お客様のデータを読み取ったり削除したりすることはできません。
...
* **インバウンドの通信は一切ありません。** ClickHouse Cloud がお客様の環境に向けて connection を開くことはありません。コマンドの経路は 2 本のアウトバウンド WebSocket のみです。troubleshooter は、お客様が有効化した support session がアクティブでない限り、すべてのコマンドを拒否します。executor は、お客様が ClickHouse に管理を依頼したサービスに対してのみ、上記のスコープ内でライフサイクルコマンドを受け付けます。session 中であっても、troubleshooter の到達範囲は双方で制限されます。ClickHouse へのクエリはテーブルの allowlist に限定され、`query_log` と `text_log` は拒否されるため、これらが許可されることはありません。Kubernetes へのアクセスは、ネームスペーススコープの Role が付与する読み取り専用のビューとポッドのログに限定されます。
