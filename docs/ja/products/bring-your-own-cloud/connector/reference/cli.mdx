...
準備済みの create を、connector endpoint 経由で connector 自身の認証情報を用いて ClickHouse Cloud に送信します。ClickHouse Cloud が割り当てた呼称、state、および service name を用いた `watch:` のヒントを出力します。create のボディは、`prepare` が書き出したファイル、または `--deployed-name`、`--data-bucket`、`--backup-bucket`、`--irsa-role-arn` の4つのフラグの組み合わせから取得されます。同じ入力で再試行した場合は、最初の create が返されます。
...
| `--default-user-password-file <path>`                                                               | フラグで指定する場合に使用する、`default` ユーザーのパスワード (1行目) を保持するファイル。指定がない場合、コマンドはパスワードを生成して一度だけ表示します。その後の再試行ではこのファイルでパスワードを渡す必要があります。渡さない場合、service は最初のパスワードを保持したまま、新しいパスワードが表示されます。                |
| `--config <path>`                                                                                   | connector の設定ファイル (環境変数 `CLCTL_EXECUTOR_CONFIG`、存在する場合のデフォルトは `/etc/clicklink/config.yaml`) 。endpoint、その認証情報、および環境の identity を提供します。                                                  |
...
| `--idempotency-key <key>`                                                                           | 再試行時に最初の create を取得するために再利用されるキー (デフォルト: 環境と service name から導出) 。                                                                                                                     |
| `--wait`                                                                                            | service が `running` になるまで executor のローカル API をポーリングします。当該名称の create が失敗した場合、または service が `terminating`、`terminated`、`stale` に遷移した場合は即座に失敗します。                                        |
