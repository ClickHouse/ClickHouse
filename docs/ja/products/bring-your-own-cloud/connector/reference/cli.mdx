...
| `--managed`                 | managed モードを有効にします。executor を実行し、ClickHouse Cloud がこのクラスター内の ClickHouse services のライフサイクルを管理します。プロンプトをスキップします。Amazon EKS クラスターが必要です。公開版の `v0.16.0` では使用できません。[availability](/ja/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) を参照してください。 |
| `--no-managed`              | managed モード(executor)を無効のままにします。プロンプトをスキップします。公開版の `v0.16.0` では使用できません。[availability](/ja/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) を参照してください。                                                                                       |
...
以下の `init --managed` ワークフローには、[managed モードをサポートする build](/ja/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) が必要です。公開されている `v0.16.0` ではその flag は利用できません。
...
生成されたクラスター全体の bundle `grant` を適用します。対象は、ServiceAccount のホーム ネームスペース、ServiceAccount、クラスター ロールと ClusterRoleBinding、および admission ポリシーです。その後、ServiceAccount トークンを bundle に発行します。service の登録は行いません。
...
managed service の作成に必要なユーザー側の準備をすべて整えます。処理は冪等な 1 回の実行にまとめられ、最初の失敗で停止します。service 名の選択または検証、データ用バケットとバックアップ用バケットおよび IAM role の作成 (持ち込んだ role の場合は検証) を行い、VM 上では service のネームスペース向けに executor のアクセス bundle をレンダリングして適用します。Kubernetes 上では bundle をスキップします。executor は自身の Pod の ServiceAccount として動作するためです。その後、`default` ユーザーの password を発行し (表示は一度きりです) 、[`instances create`](#instances-create) 用の作成ボディを書き出します。[managed services](/ja/products/bring-your-own-cloud/connector/managed-services#create-a-service) を参照してください。
...
ClickHouse Cloud 上で削除された service について、`prepare` が作成したリソースを削除します。対象は、IAM role、`prepare` が作成した bucket、記録された名前、executor が保持する service のレコード、さらに VM 上では service の クラスター ロール と ClusterRoleBinding、ローカルの access bundle、registry エントリです。実行には executor が稼働中で、service が `terminated` (または `stale`) である必要があります。service のネームスペースに ClickHouse クラスター がまだ残っている場合、実行は拒否されます。データと backups は明示的に指定しない限り保持され、保持された bucket には `clicklink:retained-from=<instance>` のタグが付け直されます。このコマンドは、削除を開始する前にすべての情報を読み取ります。[delete a service](/ja/products/bring-your-own-cloud/connector/managed-services#delete-a-service) を参照してください。
...
この顧客向け create ワークフローはまだ公開リリースでは利用できず、CLI、connector endpoint、および ClickHouse Cloud 側でのサポートが必要です。使用前に [managed モードの提供状況](/ja/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) を確認してください。

準備済みの create を、connector endpoint 経由で connector 自身の認証情報を用いて ClickHouse Cloud に送信します。ClickHouse Cloud が割り当てた呼称、state、および サービス name を用いた `watch:` のヒントを出力します。create のボディは、`prepare` が書き出したファイル、または `--deployed-name`、`--data-bucket`、`--backup-bucket`、`--irsa-role-arn` の4つのフラグの組み合わせから取得されます。同じ入力で再試行した場合は、最初の create が返されます。
...
以下の 3 つのサブコマンドは、ClickHouse Cloud を介さずに executor のローカル API へ直接コマンドを送信します。実行は、ClickHouse の account team から依頼された場合のみにしてください。いずれも `--name <name>` とクラスターの指定 (`--cluster`、`CLCTL_CLUSTER`、または `~/.clicklink/clctl.yaml`) が必要で、実行したコマンドを JSON 形式で出力します。その `id` を [`commands get`](#commands-get) に渡すと進捗を追跡できます。失敗したコマンドの `result` には、その error が格納されます。
...
1 つのサービスをその場で変更します。指定できるパッチは `--create-nlb` のみです。このオプションは内部 NLB を Kubernetes の `Service` としてプロビジョニングし、クラスター外部のスクレイパーからサーバーへ到達できるようにします (AWS のみ) 。エグゼキューターは次回の同期時に、ロードバランサーからスクレイパーのホストをバックフィルします。`--create-nlb` を指定しない場合、コマンドは `nothing to patch` を返して処理を拒否します。
...
公開されている `v0.16.0` の CLI には `platform` コマンドがないため、そのリリースでは VM でも Kubernetes でも承認の workflow を利用できません。Kubernetes での承認にはさらに、以下で説明する Secret 配信に対応した CLI と chart が必要です。いずれの workflow を使う場合も、事前に [managed モードの提供状況](/ja/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability) を確認してください。

プラットフォーム変更 1 件を、自分のクラスター認証情報で承認します。executor がステージングしたバンドルを読み取ってハッシュを計算し、その中のすべての chart を executor とまったく同じようにレンダリングします。選択した kube context を使って権限側の半分を適用し、executor の `pcm-platform` identity を確保したうえで、そのトークンを発行します。connector endpoint へのリクエストは一切行いません。権限側の半分にあたるのは、ネームスペース、CustomResourceDefinition、ServiceAccount、クラスター ロールとその bindings、ロールとその bindings、webhook および admission の設定、PriorityClass、StorageClass です。executor が適用するのはワークロード側の半分のみで、しかもそのトークンが有効な間に限られます。[プラットフォームの更新](/ja/products/bring-your-own-cloud/connector/platform-updates)を参照してください。
...
公開版の `v0.16.0` CLI には `platform reset` を含む `platform` コマンドはありません。[managed モードの提供状況](/ja/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)を参照してください。
...
| フラグ                                                                                         | 説明                                                                                                                                                                                                                            |
| ------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--provider <name>`                                                                         | 実行チャネル。リモート VM には `ssh`、`aws` (SSM) 、または `gcp` (IAP) を、ターゲット VM 自体で実行する場合は `local` を指定します。明示的に設定しない場合は、プロバイダー固有のフラグから推論されます。チャネルフラグが未設定の場合のデフォルトは `aws` で、`local` が推論されることはないため、VM 自体で実行する場合は常に `--provider local` を指定してください。 |
