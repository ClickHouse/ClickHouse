...
マネージドモードでは、ClickHouse Cloud は管理を委任されたサービスのみを変更します。executor ホストまたは ポッド にアクセスできるユーザーであれば、ローカルの `clicklink clctl instances` コマンドで操作することも可能です。[サービスライフサイクル](/ja/products/bring-your-own-cloud/connector/managed-services#service-lifecycle)を参照してください。ClickHouse Cloud は、executor の外向きチャネルを介してライフサイクルコマンドを送信します:
...
ClickHouse が作成した services は稼働し続けますが、ClickHouse Cloud からはスケール、アップグレード、バックアップ、削除を実行できなくなり、これらに対する pending 状態のコマンドは失敗します。`clicklink clctl platform approve` を実行しない限り、プラットフォームの更新が適用されることはありません。このコマンドを実行しないままにすると、マネージド mode のその他の機能は動作を継続する一方で、プラットフォーム層は凍結されます。
