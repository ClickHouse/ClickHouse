...
managed モードの登録、顧客による作成、プラットフォーム承認の各ワークフローには、[account team による確認済みのサポート](/ja/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)が必要です。これらは公開版の `v0.16.0` では使用不可です。

managed モードでは、ClickHouse Cloud は管理を委任されたサービスのみを変更します。executor ホストまたは ポッド にアクセスできるユーザーであれば、ローカルの `clicklink clctl instances` コマンドで操作することも可能です。[サービスライフサイクル](/ja/products/bring-your-own-cloud/connector/managed-services#service-lifecycle)を参照してください。ClickHouse Cloud は、executor のアウトバウンドチャネルを介してライフサイクルコマンドを送信します:
...
scraper は、デフォルトでは固定されたシステムテーブル群 (`metric_log`、`asynchronous_metric_log`、`tables`、`warnings`、`server_settings`) の結果を送信します。あわせて、ヘルスおよびステータスのハートビート、インスタンスとバックアップの状態、コネクタ自身のメトリクスも送信します。デフォルトのスクレイプ対象に `system.query_log` は含まれないため、生の SQL テキストやその中のリテラル、個人データが、明示的に追加しない限りスクレイプ経路から外部に出ることはありません。セッション中は、デフォルトのテーブル許可リストに `system.processes` が含まれており、実行中のクエリテキストが表示されます。これを秘匿する必要がある場合は、許可リストから削除してください。

executor は、実行した各コマンドの結果と、各マネージドサービスの状態およびレプリカ数を送信します。そのハートビートでは、コマンドチャネルが接続されているかどうか、プラットフォームのコンポーネントバージョン、最後に適用したバンドルのダイジェスト、最後のプラットフォーム更新の結果が報告されます。VM インストールの場合は、Kubernetes アクセスバンドルが `ok`、`expiring`、`expired` のいずれであるかもハートビートで報告されます。サービスを作成すると、`clicklink clctl instances create` が環境 ID、クラウドとリージョン、サービス名、バケット名、IAM role ARN を送信します。また、デフォルトユーザーのパスワードのハッシュを 2 つ送信しますが、パスワード自体が外部に出ることはありません。
...
<Note>
  公開されている `v0.16.0` の CLI には `--managed` も `--no-managed` もありません。以下のインストール時の選択には、[managed モードのサポート](/ja/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)を含む build が必要です。既存の executor を停止する場合は以下の Helm または systemd の手順を使用し、いずれの init フラグも必要ありません。
</Note>

managed モードを使わずに実行してください。インストール時に `clicklink clctl init` へ `--no-managed` を渡すか、確認を求められた際に「いいえ」と回答します。デプロイされるのは scraper と トラブルシューター のみになるため、ClickHouse は service を作成・変更・削除できません。ただし、コネクタは引き続き読み取り専用の ClickHouse ユーザーと ServiceAccount をプロビジョニングします。詳細は[権限モデル](/ja/products/bring-your-own-cloud/connector/reference/privilege-model)を参照してください。
...
ClickHouse が作成した services は稼働し続けますが、ClickHouse Cloud からはスケール、アップグレード、バックアップ、削除を実行できなくなり、これらに対する pending 状態のコマンドは失敗します。`clicklink clctl platform approve` を実行しない限り、プラットフォーム更新が適用されることはありません。このコマンドを実行しないままにすると、managed モードのその他の機能は動作を継続する一方で、プラットフォーム層は凍結されます。
...
executor はサービスを停止し、その Kubernetes リソースを削除したうえで、ネームスペースを削除します。ネームスペースが消えた時点で、サービスは終了したと報告されます。Kubernetes の外にあるものはすべて残ります。データ用 bucket、バックアップ用 bucket、およびサービスの IAM role です。executor はお客様の bucket や IAM に対する認証情報を保持していないため、Kubernetes 外のリソースには一切変更を加えません。

残りは、お客様の認証情報を用いて実行する `clicklink clctl executor teardown` で削除します。デフォルトでは IAM role を削除し、データと backup は保持します。これらを削除するには明示的な flag の指定が必要です。[サービスの削除](/ja/products/bring-your-own-cloud/connector/managed-services#delete-a-service)を参照してください。

### default ユーザーのパスワードはどこで確認できますか？ {#default-user-password}

顧客サービスの作成には[対応する CLI とデプロイメントのサポート](/ja/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)が必要であり、公開版の `v0.16.0` では使用できません。
...
2. **マネージドモードを停止します。** [ClickHouse が自分のクラスターを変更しないようにするにはどうすればよいですか？](#stop-managed-mode) の説明に従って、executorを停止し、その権限を削除します。プラットフォームの更新は個別に承認が必要なため、承認を保留すれば更新は停止します。
...
* [プラットフォーム承認をサポートするビルド](/ja/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)では、プラットフォームバンドルを帯域外で受け渡し、ファイルとして `clicklink clctl platform approve --bundle` に渡すことができます。パブリックの `v0.16.0` には `platform` コマンドはありません。
...
セッション中に実行されたすべてのコマンドは、許可されたものもブロックされたものも、newline-delimited JSON形式で`/var/log/clicklink/troubleshoot-audit.log`に追記されます。各エントリには、`submitted_by` (コマンドチャネル上の組織ID) 、コマンドのテキスト、対象インスタンス、結果、および拒否されたコマンドの場合は`blocked_by`が記録されます。ゲートウェイ呼び出しは、拒否されたものも含め、トラブルシューター自身のログに構造化された`clctl.gateway`行として記録され、トークンによって証明されたオペレーターのメールアドレスに帰属されます (自己申告の名前は使用されません) 。VM上のローカルセッションの変更では、実行元のホストユーザーが`/var/lib/clicklink/session.json`の`enabled_by`として記録されます。
