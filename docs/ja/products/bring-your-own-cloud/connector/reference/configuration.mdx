...
[マネージドモード](/ja/products/bring-your-own-cloud/connector/managed-services)向けのエグゼキューター: PersistentVolumeClaim 上で動作する単一レプリカのデプロイメントです。`init --managed` は、クラスターの identity を埋め込んだ状態で有効化してステージングします。

| Key                                             | デフォルト                                       | 意味                                                                                                                                                                                                        |
| ----------------------------------------------- | ------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `executor.enabled`                              | `false` (`init --managed` は `true` を書き込みます) | エグゼキューターをデプロイします。                                                                                                                                                                                         |
...
| `executor.cluster.name`                         | `\"\"` (`init` が設定します)                        | このエグゼキューターが管理する単一の EKS クラスターの名前。エグゼキューターを有効にする場合は必須です。                                                                                                                                                    |
...
| `executor.serviceAccount.create`                | `false`                                     | チャートがエグゼキューターの ServiceAccount を作成するかどうか。デフォルトは無効です。`init --managed` は固定名に対してエグゼキューターの RBAC を別途付与し、チャートはそれにバインドします。                                                                                        |
| `executor.serviceAccount.name`                  | `pcm-executor`                              | エグゼキューターの実行に使用する ServiceAccount。`create` が `false` の場合は必須です。                                                                                                                                              |
...
`executor.cluster` は単一のマッピングであり、エグゼキューター 1 つにつきクラスターは 1 つです。`clusters` のリストを指定するとレンダリングに失敗します。
