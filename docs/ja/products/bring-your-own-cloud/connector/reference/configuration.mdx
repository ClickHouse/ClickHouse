...
以下の `init --managed` の動作には、[managed モードをサポートするビルド](/ja/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)が必要です。公開されている `v0.16.0` ではこのフラグは利用できません。
...
| `executor.observability.health_port`            | `8086`                                      | executor の ヘルス およびメトリクス用ポート。                                                                                   |
...
以下の managed モードの初期化およびプラットフォーム承認のインテグレーションには、[対応する CLI とチャートのサポート](/ja/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)が必要です。公開されている `v0.16.0` には `init --managed` も `platform` コマンドも含まれていません。

[マネージドモード](/ja/products/bring-your-own-cloud/connector/managed-services)向けの executor: PersistentVolumeClaim 上で動作する単一レプリカのデプロイメントです。`init --managed` は、クラスターの identity を埋め込んだ状態で有効化してステージングします。
...
| `executor.cluster.inCluster`                    | `false` (`init` は `true` を書き込みます)           | ポッドの ServiceAccount として認証します。これが Kubernetes 形態の要点です。ポッド内にアクセスバンドルは存在しません。                                                                                                                                 |
...
| `executor.persistence.storageClass`             | `""` (クラスターのデフォルト)                          | クレーム用の StorageClass。`init` は `--storage-class` または検出されたデフォルトから値を設定します。                                                                                                                                    |
| `executor.persistence.size`                     | `5Gi`                                       | クレームのサイズ。                                                                                                                                                                                                 |
| `executor.persistence.accessModes`              | `[ReadWriteOnce]`                           | クレームのアクセスモード。                                                                                                                                                                                             |
...
| `networkPolicy.apiserverCIDRs`        | `[]` ([サポート対象のビルド](/ja/products/bring-your-own-cloud/connector/onboarding#managed-mode-availability)では `init --managed` がクラスターの VPC から設定します) | Kubernetes API サーバーの CIDR です。空の場合、API サーバーへの egress ルールは生成されず、マネージド Kubernetes では設定するまでデーモンの最初のトークンリクエストが失敗します。API サーバー自身のアドレスはクラスターの運用期間中に変化するため、そのアドレスではなく VPC の範囲を指定してください。                                                                                                                                                                                             |