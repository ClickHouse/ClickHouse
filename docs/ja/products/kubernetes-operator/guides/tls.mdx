...
[`openSSL` サーバー設定](/ja/reference/settings/server-settings/settings/other#openssl)
を、`extraConfig` がどのようにマージされるかについては
[Configuration → 埋め込みの追加設定](/ja/products/kubernetes-operator/guides/configuration#embedded-extra-configuration)
を参照してください。
...
* [Configuration → TLS/SSL 設定](/ja/products/kubernetes-operator/guides/configuration#tls-ssl-configuration) — フィールドのリファレンス
* [Configuration → `additionalPorts`](/ja/products/kubernetes-operator/guides/configuration#additional-ports) — 予約済みポート
* [API リファレンス → ClusterTLSSpec](/ja/products/kubernetes-operator/reference/api-reference#clustertlsspec)
* [`openSSL` サーバー設定](/ja/reference/settings/server-settings/settings/other#openssl) — `extraConfig` で上書き可能な TLS オプション