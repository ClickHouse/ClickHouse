...
オペレーターは、すべての ClickHouse ポッドとそのパブリックなヘッドレス Service で、固定のポート群を公開します。具体的には、`8123` HTTP、`9000` ネイティブ、`9009` interserver、`9001`/`9002` management、`9363` Prometheus メトリクス、さらに TLS が有効な場合は TLS 用の `8443`/`9440` です。`interserver` および management のポートは、レプリカごとの内部 Service を通じても公開されます。これにより、レプリカが Ready になる前でも、レプリカとオペレーターが相互に通信できます。ClickHouse で MySQL、PostgreSQL、gRPC、または任意のカスタムポートなどの追加プロトコルを待ち受けるようにするには、`spec.additionalPorts` で宣言します。
...
operator は、それらのポートをポッドの `containerPorts` とパブリックなヘッドレス Service に追加します。
...
すべてのレプリカ ポッドには `clickhouse.com/ReplicaInitialized` readiness gate が付与されているため、新しいレプリカは、オペレーターが初期化を完了した後にのみパブリックなヘッドレス Service で公開されます。初期化では、`default` データベースが Replicated エンジンに変換され、スキーマが同期されます。それまでの間、オペレーターや他のレプリカは、内部 Service を介してそのレプリカに到達します。`enableDatabaseSync: false` の場合、オペレーターはレプリカを直ちに初期化済みとしてマークするため、Readiness はコンテナー probe のみに依存するようになります。

オペレーターは、データが投入された非 Replicated の `default` データベースを削除することはありません。そのようなレプリカも、スキーマの準備が完了するとクライアント トラフィックの処理を開始しますが、そのデータベースを自分で解消するまで、クラスターは reason `DefaultDatabaseNotReplicated` の `SchemaInSync=False` を報告し続けます。

スケールダウン時にレプリカを削除する前に、オペレーターはまずそのレプリカからクライアント トラフィックをオフロードし、公開が停止するまで待機してから、残りのデータを存続する他のレプリカに複製し、その後でレプリカを削除します。
