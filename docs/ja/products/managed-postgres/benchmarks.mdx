...
sidebarTitle: 'ベンチマーク'
title: 'パフォーマンスベンチマーク'
description: 'ClickHouse Managed Postgres と AWS Aurora、RDS、その他のマネージド PostgreSQL サービスのパフォーマンスを比較したベンチマーク'
...
<Info>
  ...
  * 標準的な [`pgbench`](https://www.postgresql.org/docs/current/pgbench.html) テストを使用して、**ClickHouse Managed Postgres** を AWS RDS (16k プロビジョンド IOPS) および Aurora IO Optimized とベンチマーク比較
  * **性能**: NVMe ストレージを基盤とする ClickHouse Managed Postgres は、I/O 集約型ワークロードで **4.3～9 倍高速**、CPU バウンドが支配的なシナリオでも **12% 高速**
...
| 構成           | ClickHouse Managed Postgres | RDS with 16k IOPS       | Aurora IO Optimized      |
| ------------ | --------------------------- | ----------------------- | ------------------------ |
| **PG バージョン** | 17                          | 17                      | 17                       |
| **vCPU**     | 16                          | 16                      | 16                       |
...
**分析**: I/O ボトルネックのある読み取り集約型ワークロードでは、性能差が大幅に広がります。**ClickHouse Managed Postgres は 84.8K TPS を記録**した一方、16,000 プロビジョンド IOPS の RDS は、同等のコンピュートリソースでありながら 9.4K TPS にとどまりました。主な違いは、ClickHouse Managed Postgres の NVMe ストレージは並行性の向上に合わせてスケールできるのに対し、ネットワーク接続型ストレージはプロビジョンド IOPS の上限に縛られる点です。RDS はプロビジョンド IOPS を設定していてもなお ClickHouse Managed Postgres より 9 倍遅く、I/O 集約型ワークロードではストレージアーキテクチャが決定的に重要であることを示しています。
...
| 構成           | ClickHouse Managed Postgres | 16k IOPS の RDS          |
| ------------ | --------------------------- | ----------------------- |
| **PG バージョン** | 17                          | 17                      |
| **vCPUs**    | 16                          | 16                      |
| **RAM**      | 64 GB                       | 64 GB                   |
...
**分析**: ディスク I/O がほとんど発生しない CPU バウンド のシナリオでも、**ClickHouse Managed Postgres は 36.5K TPS を記録し、最も高い性能を示しました**。両サービスとも CPU 使用率は 100% に達しましたが、ClickHouse Managed Postgres の NVMe ストレージは、より高い cache ヒット率により優れたパフォーマンスを発揮しました。RDS に対する 12% の優位性は、ワークロードが主に CPU バウンド となる場合でも、基盤となるインフラストラクチャが高効率であることを示しています。
...
3 つのベンチマークシナリオすべてで、ClickHouse Managed Postgres は一貫して優れたパフォーマンスを発揮しました。
...
3. **CPU バウンド ワークロード**: RDS より TPS が 12% 高い
...
| Aspect                  | NVMe ストレージ (ClickHouse Managed Postgres) | ネットワーク接続型 Storage (Provisioned IOPS) |
| ----------------------- | ---------------------------------------- | ------------------------------------ |
| **IOPS**                | 10万から実質無制限                               | 16,000 のプロビジョンド IOPS                 |
| **Network hops**        | なし (ローカルデバイス)                            | すべてのディスク操作でネットワークの往復が必要              |
| **Performance scaling** | 並行性に応じて線形にスケール                         | プロビジョンド IOPS によって制限される               |
...
純粋な性能だけでなく、ClickHouse Managed Postgres はコストパフォーマンスにも優れています。
...
現在 IOPS 制限がボトルネックになっているワークロードでは、ClickHouse Managed Postgres に切り替えることで、高価なプロビジョンド IOPS や IO Optimized 構成が不要になり、大幅に優れた性能を実現できます。
...
* [ClickHouse Managed Postgres の概要](/ja/products/managed-postgres/overview)
