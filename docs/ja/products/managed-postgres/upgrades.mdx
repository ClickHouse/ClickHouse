...
ClickHouse Managed Postgres では、インスタンスの安全性を保ち、最新の状態を維持するために、PostgreSQL のバージョンアップグレードを実施します。マイナーバージョンアップグレードとメジャーバージョンアップグレードの両方に対応しており、影響は最小限に抑えられます。
...
Enterprise Tier の組織では、ClickHouse Managed Postgres でメンテナンスウィンドウがサポートされているため、アップグレードやその他のメンテナンス作業を、ワークロードへの影響が最小となる時間帯にスケジュールできます。メンテナンスウィンドウを設定するための UI と API のサポートは近日中に提供予定です。それまでは、[support](https://clickhouse.com/support/program) に連絡して、インスタンスのメンテナンスウィンドウを設定してください。
