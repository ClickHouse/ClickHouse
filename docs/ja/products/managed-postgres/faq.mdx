...
<div id="metrics-access">
  ### ClickHouse Managed Postgres インスタンスのメトリクスにアクセスするにはどうすればよいですか？
</div>
...
ClickHouse Managed Postgres には、毎日の自動バックアップと継続的な WAL アーカイブが含まれており、7 日間の保持期間内であれば任意の時点へのポイントインタイム リカバリが可能です。バックアップは S3 に保存されます。
...
<div id="terraform-support">
  ### ClickHouse Managed Postgres で Terraform は利用できますか？
</div>

現在、ClickHouse Managed Postgres では Terraform はサポートされていません。インスタンスの作成と管理には、ClickHouse Cloud コンソール または [OpenAPI](/ja/products/managed-postgres/openapi) の使用をお勧めします。
...
<div id="migration-tools">
  ### ClickHouse Managed Postgres への移行にはどのようなツールを利用できますか？
</div>
