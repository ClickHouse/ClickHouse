...
<Visibility for="humans">
  <View title="Cloud UI">
    ...
    * `pg_clickhouse` を使用して、Postgres から ClickHouse に直接クエリ
    ...
    ## 第1部: NVMe Postgres を使い始める {#part-1}
    ...
    ***
    ...
    ## 第2部: ClickHouse でリアルタイム分析を追加する {#part-2}
    ...
    * 数十億行規模のデータに対するサブ秒の分析には **ClickHouse**
    ...
    ### ClickHouse 連携のセットアップ {#setup-integrate-clickhouse}
    ...
    * **Replicate data to ClickHouse** をクリックします。
    ...
    ### Postgres から ClickHouse をクエリする {#pg-clickhouse-extension}
    ...
    ### 分析機能を実際に確認する {#analytics-after-integration}
    ...
    ### Postgres と ClickHouse のパフォーマンスを比較する {#performance-comparison}
