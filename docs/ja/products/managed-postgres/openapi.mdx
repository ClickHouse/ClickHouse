...
<Warning>
  **2026年7月31日以降の認証情報のマスキング**

  **2026年7月31日**以降、APIはレスポンスで `password` および `connectionString` のプロパティを返さなくなります。これらが含まれるのは、[create](#create) レスポンスと、リクエストにパスワードが含まれていなかった場合の [password reset] レスポンスのみです。[create](#create) レスポンスから認証情報を取得しておくことを推奨します。Terraform でサービスを管理している場合は、2026年7月31日までにプロバイダを **v3.21.0 以降**にアップグレードしてください ([Terraform reference](/ja/products/managed-postgres/terraform) を参照) 。
</Warning>
...
次に、このデータを使用して新しいインスタンスを作成します。なお、そのためにはContent-Typeヘッダーの内容が必要です。返される認証情報にアクセスできるよう、レスポンスを保存してください：
...
出力は作成時に返されるJSONと似ています (ただし、2026年7月31日以降は `password` と `connectionString` プロパティは含まれません) が、`state` を確認して
...
[ClickHouse OpenAPI]: /ja/products/cloud/features/admin-features/api/api-overview "Cloud API"

[OpenAPI]: https://www.openapis.org "OpenAPI Initiative"

[API キー]: /ja/products/cloud/features/admin-features/api/openapi "API キーの管理"
...
[Prometheus endpoint]: /ja/products/managed-postgres/monitoring/prometheus "ClickHouse Managed Postgres Prometheus エンドポイント"

[metrics reference]: /ja/products/managed-postgres/monitoring/metrics "ClickHouse Managed Postgres メトリクス リファレンス"

[Query Insights]: /ja/products/managed-postgres/monitoring/query-insights "Postgres クエリインサイト"

[detail flyout]: /ja/products/managed-postgres/monitoring/query-insights#detail "クエリインサイトの詳細フライアウト"

[per-execution counters]: /ja/products/managed-postgres/monitoring/query-insights#counters "クエリインサイトの実行ごとのカウンター"

[slow patterns API]: /products/cloud/api-reference/postgres/slow-query-patterns-get-list "Postgres の低速クエリパターンを一覧表示"

[slow pattern API]: /products/cloud/api-reference/postgres/slow-query-pattern-get "最近の実行結果を含む Postgres の低速クエリパターンを取得"