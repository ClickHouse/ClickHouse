...
<Steps>
  <Step title="フェーズ 1: FIPS 準拠の証明書を作成する" id="phase-1-create-fips-certificates">
  ...
  <Step title="フェーズ 2: Kubernetes の前提条件のインストール" id="phase-2-install-kubernetes-prerequisites">
  ...
  <Step title="フェーズ 3: Operator をインストールする" id="phase-3-install-operator">
  ...
  <Step title="フェーズ 4: 証明書シークレットの作成" id="phase-4-create-certificate-secrets">
  ...
  <Step title="フェーズ5: ClickHouse クラスターのデプロイ" id="phase-5-deploy-clickhouse-cluster">
  ...
  <Step title="フェーズ 6: インストールの検証" id="phase-6-verify-installation">
    ### TLS ポートへのポートフォワード
