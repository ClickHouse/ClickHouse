...
declared `T: (*)` or `T: (col: type, ...)` which must come first. Arguments may be passed by
name in any order (`f(c = 7, a = 12)`). A body is any number of `let` statements followed by
one expression, and it can see the bindings that enclose it. A parameterless function may be
