...
- **`geo_*` takes longitude before latitude**, as Kusto does. Distances are computed on a
  sphere of radius 6371010 m, the value that reproduces Kusto's own documented results
  exactly; ClickHouse's `greatCircleDistance` uses a different radius and a faster
  approximation, and lands about 600 m out over 1500 km.
