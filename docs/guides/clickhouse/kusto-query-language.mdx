...
| Scalar and aggregate functions | 307 | 123 |
...
- **User-defined functions**: `let f = (x: long) { x * 2 }`. `let` binds a scalar or a whole
  tabular expression, but not a lambda with parameters.
