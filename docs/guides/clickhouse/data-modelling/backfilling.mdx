...
- **Insert Parallelism** - The number of insert threads used to insert. Controlled through [`max_insert_threads`](/reference/settings/session-settings/max-insert#max_insert_threads). **Note**: This value is capped by `max_threads`, so the effective insert parallelism is `min(max_insert_threads, max_threads)`. In ClickHouse Cloud this is determined by the instance size (between 2 and 4). In OSS since 26.8 the default is `0` (auto: the number of CPU cores). Increasing this value may improve performance at the expense of greater memory usage.
