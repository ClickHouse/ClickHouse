...
Both integer columns fit in [`UInt8`](/reference/data-types/int-uint), although `passenger_count` reaches its maximum value of 255. The example also uses [`Float32`](/reference/data-types/float) for `trip_distance` and [`Decimal32`](/reference/data-types/decimal) for monetary values. It replaces the inferred [`DateTime64`](/reference/data-types/datetime64) columns with [`DateTime`](/reference/data-types/datetime) in the same `UTC` timezone because the example queries do not require fractional-second precision.
