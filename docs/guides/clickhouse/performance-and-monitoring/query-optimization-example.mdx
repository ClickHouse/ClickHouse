...
Use the following setup to create the `nyc_taxi` database, infer the table schema, and load the data from an S3 bucket. The inferred schema is intentionally unoptimized so that the later changes can be measured against it.
...
Return to the [query optimization overview](/optimize/query-optimization), or learn how to [diagnose slow queries](/guides/clickhouse/performance-and-monitoring/diagnose-slow-queries) and [isolate query bottlenecks](/guides/clickhouse/performance-and-monitoring/isolate-query-bottlenecks).
