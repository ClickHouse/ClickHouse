...
The analyzer significantly changes the communication protocol between servers in the cluster. Thus, it's impossible to run distributed queries on servers that do not agree on whether the analyzer is used - which, for a cluster of servers older than `26.9`, means servers with different `enable_analyzer` setting values. A server of version `26.9` or newer has no other query analysis left, so it ignores the value an older initiator sends and analyzes the query with the analyzer regardless.
