...
Begin by [identifying a recurring slow query](/guides/clickhouse/performance-and-monitoring/diagnose-slow-queries) that you can run against a consistent dataset. Production deployments can execute many queries at once, so narrow `system.query_log` by its `user`, `databases`, or `tables` fields, then group by `normalized_query_hash` to find expensive patterns that run regularly. Treat isolated slow runs with caution: an ad hoc query or temporary system stress might not represent a recurring performance problem.

You can apply this method to your own query. To run the examples in this guide as written, create and load the `nyc_taxi.trips_small_inferred` table if you have not already done so:
...
Progressively simplifying a query lets you compare its duration before and after removing a stage of work. Each simplified form is a query shape in this comparison.
The differences help you decide whether to investigate scanning and filtering, grouping, or aggregate expressions and work after aggregation, such as sorting and output formatting:

1. Run the original query to establish the baseline measurements.
2. Replace its aggregate expressions with a grouped `count` to retain grouping while removing the original aggregate expressions and work after aggregation.
3. Remove the grouping and run an ungrouped `count` to approximate the work retained by scanning, filtering, and any joins.

These stages apply directly to conventional grouped aggregate queries. For more complex query shapes, apply the same principle to one `SELECT` block at a time: preserve equivalent data sources and filters, remove one operation at a time, and verify the execution plan after each change.
...
- Use the same bounded dataset and time range for every run.
- Keep the `FROM`, `JOIN`, `PREWHERE`, and `WHERE` clauses equivalent as you simplify the query.
- Run each query shape multiple times while concurrent query traffic and available compute resources remain similar.
- Warm each query shape consistently or disable the relevant caches. Do not combine cold and warm runs when calculating a representative value.
...
3. Flush the query log before looking up recently completed queries:
   ...
   If you cannot run `SYSTEM FLUSH LOGS`, wait for the query log to flush automatically, then retry the lookup. If the record never appears, verify that query logging is enabled and that you can read `system.query_log`.
...
5. For each query shape, use the median duration from the measured runs. Record `read_rows`, `read_bytes`, and peak memory from the run closest to that median so that the measurements remain tied to an actual run.
...
| Run | Query shape | Representative duration | `read_rows` | `read_bytes` | Peak memory |
| --- | --- | --- | --- | --- | --- |
| A | Original query |  |  |  |  |
| B | Grouped `count` |  |  |  |  |
| C | Ungrouped `count` |  |  |  |  |
...
<Step title="Run C: Remove grouping" id="run-c-retain-only-the-scan-and-filter">
...
| Run C remains slow | Scanning, filtering, or another operation retained by run C | Inspect rows and bytes read, primary-key use, data-skipping indexes, and the execution plan; then validate the suspected bottleneck |
| All three runs have similar durations | The source of latency might be common to all three query shapes, or simplification might have changed the execution plan | Compare `read_rows`, `read_bytes`, and peak memory across the runs. If they are also similar, investigate the operations retained by run C. Otherwise, compare the execution plans for differences |

Treat A minus B and B minus C as signals about where to investigate, not as timers for exact execution stages. The evidence is stronger when the simplified queries have similar `read_rows`, `read_bytes`, and execution plans. Run C provides the baseline for the operations its plan retains.

These estimates can overlap. Replacing aggregate expressions can reduce the number of columns read. Removing a grouping key can reduce both I/O and memory usage. Check `read_bytes` and peak memory alongside duration.

### Compare rows read with the count result {#compare-rows-read-with-count-result}

Compare `read_rows` with the value returned by `count`. This comparison is most straightforward for a single-table scan. For queries with joins or other operations that change row counts, interpret the values alongside the execution plan and the expected effect of those operations.

Keep the cache state consistent. The query-condition cache can reduce `read_rows` on later runs without a schema or query change.

For example, suppose `read_rows` is 100 million and `count` returns 1 million rows. ClickHouse scanned approximately 100 source rows for every row counted. This shows that the filter rejected most of the rows read from the table, but it does not identify why.
...
If `read_rows` is lower than the returned `count`, do not calculate a scan-to-match ratio. Inspect the execution plan for a projection or another optimization that changed the work performed.

The `read_rows` value includes rows read from all tables and table functions used by the query, including reads from subqueries used by `IN` and `JOIN`. For a distributed query, the initiating server combines its local value with the values reported by participating replicas. Account for those additional sources before comparing `read_rows` with the value returned by `count`.

For a distributed query, `memory_usage` in the initiating query's `QueryFinish` record is not a cluster-wide peak. If memory is the suspected bottleneck, use `initial_query_id` to inspect the child `QueryFinish` records on the participating nodes.
