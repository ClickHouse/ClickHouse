...
Before optimizing a query, identify a repeatable workload and collect a baseline. This guide introduces the query log and execution-plan tools used to find candidate queries and understand their resource usage.
...
### Select a representative query run {#select-a-representative-query-run}
...
-   Query 2 finds the number and average cost of rides by payment type. 
...
Let's quickly confirm how many rows are in this table.
...
Continue with [Isolate query bottlenecks](/guides/clickhouse/performance-and-monitoring/isolate-query-bottlenecks) to establish a repeatable comparison and determine which part of the query to investigate first.
