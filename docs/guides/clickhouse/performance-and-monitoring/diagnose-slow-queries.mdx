...
2. `Filter (WHERE)` corresponds to `WHERE speed_mph > 30`. For every row read, ClickHouse calculates `trip_time` using `dateDiff`, calculates `speed_mph`, and then keeps only rows above 30 miles per hour.
3. `Expression (Before GROUP BY)` passes the filtered `trip_distance` values to `quantiles`. ClickHouse uses this label for the step that prepares aggregate inputs even though this query has no `GROUP BY` clause.
4. `Aggregating` calculates the quantiles.
5. The top `Expression` produces the final result.
...
Next, test these findings with the controlled comparison method in [Isolate query bottlenecks](/guides/clickhouse/performance-and-monitoring/isolate-query-bottlenecks). Because this query has no `GROUP BY`, skip the grouped comparison and compare it with a `count` that retains `WHERE speed_mph > 30`. This helps distinguish the quantile calculation from the scan-and-filter work.
