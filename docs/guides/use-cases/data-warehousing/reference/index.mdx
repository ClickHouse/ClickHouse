...
ClickHouse integrates with a range of data lake catalogs through the [`DataLakeCatalog`](/reference/engines/database-engines/datalakecatalog) database engine. The following guides walk through connecting ClickHouse to each supported catalog, including configuration, authentication, and querying examples.
