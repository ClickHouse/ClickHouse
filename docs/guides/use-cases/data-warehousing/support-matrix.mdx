...
| Compaction | ⚠️ | Experimental. Requires `allow_experimental_iceberg_compaction = 1`. Manifest compaction only, via `OPTIMIZE TABLE ... MANIFEST`. Data compaction (merging position delete files into data files) is not implemented. See [Compaction](/reference/functions/table-functions/iceberg#iceberg-writes-compaction). |
