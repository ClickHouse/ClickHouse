...
| Expire snapshots | ⚠️ | Experimental. Requires `allow_insert_into_iceberg = 1` and `allow_experimental_expire_snapshots = 1`. Via `ALTER TABLE ... EXECUTE expire_snapshots(...)`. Requires format version 2 and a non-transactional catalog. See [Expire snapshots](/reference/functions/table-functions/iceberg#iceberg-expire-snapshots). |
| Remove orphan files | ⚠️ | Experimental. Requires `allow_insert_into_iceberg = 1` and `allow_iceberg_remove_orphan_files = 1`. Via `ALTER TABLE ... EXECUTE remove_orphan_files(...)`. Requires format version 2 or higher. See [Remove orphan files](/reference/functions/table-functions/iceberg#iceberg-remove-orphan-files). |
