...
| Compaction | ⚠️ | Experimental. Requires `allow_experimental_iceberg_compaction = 1`. Manifest compaction only, via `OPTIMIZE TABLE ... MANIFEST`. Data compaction (merging position delete files into data files) is not implemented in the open-source build. See [Compaction](/reference/functions/table-functions/iceberg#iceberg-writes-compaction). |
| UPDATE / MERGE | ❌ | Not supported. |
| Copy-on-write | ❌ | Not supported |
| Expire snapshots | ⚠️ | Experimental. Requires `allow_insert_into_iceberg = 1` and `allow_experimental_expire_snapshots = 1`. Via `ALTER TABLE ... EXECUTE expire_snapshots(...)`. Requires format version 2. Tables backed by a transactional catalog are rejected. See [Expire snapshots](/reference/functions/table-functions/iceberg#iceberg-expire-snapshots). |
| Remove orphan files | ⚠️ | Experimental. Requires `allow_insert_into_iceberg = 1` and `allow_iceberg_remove_orphan_files = 1`. Via `ALTER TABLE ... EXECUTE remove_orphan_files(...)`. Requires format version 2 or higher. Its reachable set is resolved through `version-hint.text`, so a hint left behind by another writer can classify committed files as orphans. See [Remove orphan files](/reference/functions/table-functions/iceberg#iceberg-remove-orphan-files). |
