...
- Credentials, one of:
  - A **service account key**: a JSON key file of a service account that has access to the catalog and to the GCS bucket, created via [Google Cloud Console](https://docs.cloud.google.com/iam/docs/keys-create-delete). This is the recommended option for servers, because it does not depend on a user session.
  - **Application Default credentials** (Oauth client ID and client secret) for an application, created via [Google Cloud Console](https://docs.cloud.google.com/docs/authentication/provide-credentials-adc), and a **refresh token** obtained by completing the OAuth flow with the appropriate scopes (e.g. `https://www.googleapis.com/auth/bigquery` and storage scope for GCS)
...
To authenticate with a service account key instead, pass the content of the JSON key file in `google_service_account_key` and the project in `google_project_id`. ClickHouse signs a JWT with the key, exchanges it for an access token, and renews it automatically. The same token is used to read the data files from GCS. The key is masked in `SHOW CREATE DATABASE` and in the logs.
...
`google_service_account_key` and the `google_adc_*` settings are mutually exclusive.
