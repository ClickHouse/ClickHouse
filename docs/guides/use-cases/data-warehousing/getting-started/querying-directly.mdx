...
ClickHouse provides experimental [`PaimonS3`, `PaimonAzure`, `PaimonHDFS`, and `PaimonLocal` table engines](/reference/engines/table-engines/integrations/paimon). Enable them with `enable_paimon_storage_engine = 1`.
