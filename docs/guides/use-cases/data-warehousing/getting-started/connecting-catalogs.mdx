...
<Note>
This integration supports tables in external storage locations on AWS, GCS, and Azure ADLS Gen2. Tables on Databricks managed storage are not supported because Unity Catalog does not provide credential vending for those locations. See the [Unity Catalog reference](/guides/use-cases/data-warehousing/unity-catalog) for details.
</Note>
