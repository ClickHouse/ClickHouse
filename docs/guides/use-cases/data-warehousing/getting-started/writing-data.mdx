...
For direct reads from an existing Iceberg table, we recommend the [`icebergS3` table function](/reference/functions/table-functions/iceberg). This guide creates a new standalone Iceberg table, so it uses the [`IcebergS3` table engine](/reference/engines/table-engines/integrations/iceberg) with an explicit schema. Subsequent inserts target that persistent table.
