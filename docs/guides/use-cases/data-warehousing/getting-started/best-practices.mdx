...
For writes, Iceberg requires [allow_insert_into_iceberg](/reference/settings/session-settings/allow#allow_insert_into_iceberg) (25.7+, Beta from 26.2). See [Writing to data lakes](/use-cases/data-lake/getting-started/writing-data). Delta Lake requires [allow_delta_lake_writes](/reference/settings/session-settings/allow-experimental#allow_experimental_delta_lake_writes) (25.9+). The [support matrix](/use-cases/data-lake/support-matrix) lists which flags apply to each format and operation.
...
From version 25.6, ClickHouse reads Delta Lake on S3 and GCS through the Delta Lake Rust kernel ([allow_delta_kernel_rs](/reference/settings/session-settings/allow#allow_delta_kernel_rs), 25.5+). Azure Blob Storage, read with [deltaLakeAzure()](/sql-reference/table-functions/deltalake), also goes through the kernel from version 26.4. Any other storage falls back to the legacy reader, where partition pruning, change data feed, and snapshot version reads aren't available.

#### Delta Kernel {#delta-kernel}

[allow_delta_kernel_rs](/reference/settings/session-settings/allow#allow_delta_kernel_rs) must be enabled for partition pruning, change data feed, and snapshot version reads. It's on by default on S3 and GCS from 25.5. Enable it explicitly on older versions or when troubleshooting:
...
Beyond [allow_delta_lake_writes](/reference/settings/session-settings/allow-experimental#allow_experimental_delta_lake_writes) (25.9+), control output file size on insert:
...
| ----------------------------------------------------------------------------------- | ---------- | ----- | --------------------------------------------------------------------------------------------------- | --------------------------------------------------------- |
| [system.iceberg_metadata_log](/operations/system-tables/iceberg_metadata_log)       | Iceberg    | 25.9  | [iceberg_metadata_log_level](/reference/settings/session-settings/iceberg-metadata#iceberg_metadata_log_level) on the query | Trace metadata files read and partition pruning decisions |
...
| [system.delta_lake_metadata_log](/operations/system-tables/delta_lake_metadata_log) | Delta Lake | 25.10 | [delta_lake_log_metadata](/reference/settings/session-settings/delta-lake#delta_lake_log_metadata) = `1` on the query | Trace Delta metadata files and snapshot resolution        |
