...
From version 25.6, ClickHouse reads Delta Lake on S3 and GCS through the Delta Lake Rust kernel. The setting is named [`allow_delta_kernel_rs`](/reference/settings/session-settings/allow#allow_delta_kernel_rs) in version 26.8 and later, and `allow_experimental_delta_kernel_rs` in versions 25.5 through 26.7. On Azure Blob Storage, use [deltaLakeAzure()](/sql-reference/table-functions/deltalake) with the legacy reader because the kernel is disabled there. Without the kernel, partition pruning, change data feed, and snapshot version reads aren't available.
