...
From version 25.6, ClickHouse reads Delta Lake on S3 and GCS through the Delta Lake Rust kernel ([allow_delta_kernel_rs](/reference/settings/session-settings/allow#allow_delta_kernel_rs), 25.5+). On Azure Blob Storage, use [deltaLakeAzure()](/sql-reference/table-functions/deltalake) with the legacy reader because the kernel is disabled there. Without the kernel, partition pruning, change data feed, and snapshot version reads aren't available.
