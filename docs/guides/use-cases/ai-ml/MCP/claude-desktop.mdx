...
Enter `/mcp` in the composer to inspect the connected server.

<Note>
ChatGPT web cannot connect to a local `stdio` server.
To use ClickHouse tools on the web, configure a plugin backed by a remote MCP server.
See [Remote MCP server in ClickHouse Cloud](/products/cloud/features/ai-ml/remote-mcp).
</Note>
</Tab>
