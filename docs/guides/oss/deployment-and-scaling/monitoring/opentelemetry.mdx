...
For a distributed `SELECT`, every read from a remote shard is covered by a span: `RemoteQueryExecutor::execute` on the asynchronous connection paths (the default) or `RemoteQueryExecutor::sendQuery` on the synchronous path. These spans carry attributes identifying the query fragment:
