...
sidebarTitle: 'ClickHouse proxy'
slug: /operations/clickhouse-proxy
title: 'ClickHouse proxy'
doc_type: 'reference'
keywords: ['proxy', 'routing', 'load balancing', 'deployment']
---
...
- **Terminate and re-encrypt** — set `<secure>1</secure>` on the listener. The proxy holds its own
  ...
  announces the backend's own host name as its SNI: the name the client used (an SNI, an HTTP `Host`
  header) identifies the proxy, not the backend. The backend's certificate chain is checked according
  to `openSSL.client.verificationMode` (`relaxed` by default), but the certificate is matched against
  the backend's host name only when `openSSL.client.extendedVerification` is enabled; without it, a
  certificate for any name signed by a trusted CA is accepted.
