...
| `disk_move_verify_destination_read_back` | Read back and verify the full destination file after a cross-disk move. The default avoids another 200–500 MiB changelog read and a potentially multi-GiB snapshot read. Keeper still forces a cache-bypassing read-back when an S3 multipart completion adopts an existing object. | `false` |
