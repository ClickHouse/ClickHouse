...
| `four_letter_word_white_list`        | White list of 4lw commands.                                                                                                                                                                                                                         | `conf, cons, crst, envi, ruok, srst, srvr, stat, wchs, dirs, mntr, isro, rcvr, apiv, csnp, lgif, rqld, rclc, clrs, ftfl, ydld, fcon, fcof, pfev, lgrq` |
...
| `full_consensus_lagging_member_grace_period_ms` | How long full consensus mode (see [`fcon`](#four-letter-word-commands)) keeps requiring the acknowledgement of a member that is alive but failing to sync, either lagging behind the committed log or receiving a snapshot. Commits are throttled down to the rate that member can sustain, which gives it a chance to catch up; afterwards it is excluded from the quorum and commits proceed with a majority. `0` excludes such a member immediately. A member that stops responding altogether is always excluded immediately, since it cannot catch up while it is down, and the grace period starts while it is down, so a member that was down for longer does not hold up commits while it catches up. **No write can be committed while the grace period holds a member**, so keep the value well below `operation_timeout_ms` unless write unavailability for that long is acceptable; if it exceeds `session_timeout_ms`, idle sessions expire too. | `0`                                                                                                        |
...
The 4lw commands have a white list configuration `four_letter_word_white_list` which has default value `conf,cons,crst,envi,ruok,srst,srvr,stat,wchs,dirs,mntr,isro,rcvr,apiv,csnp,lgif,rqld,rclc,clrs,ftfl,ydld,fcon,fcof,pfev,lgrq`.
...
- `fcon`: Request the leader to turn on full consensus mode. In this mode a write is committed only after it is acknowledged by all healthy voting members instead of a majority (a member that does not respond for several heartbeats, or lags behind the committed log by more than one append batch, is excluded from the quorum until it recovers, and commits never proceed with fewer acks than a majority). Note that "all healthy voting members" does not include members configured with `can_become_leader` set to `false`: those are non-voting learners and are never required to acknowledge. By default a member that is failing to sync is excluded as soon as it falls behind; set [`full_consensus_lagging_member_grace_period_ms`](#configuration) to keep requiring its acknowledgement for a bounded time instead, which throttles commits down to the rate that member can sustain and gives it a chance to catch up. That also covers a member receiving a snapshot, where throttling does not speed up the transfer, so the grace period there is purely a delay before the member is excluded — keep it small. While a member is being waited for, no write commits at all: `lgif` shows `last_log_idx` running ahead of `last_committed_log_idx`, and the leader logs a warning naming the member every five seconds. Allow-list both `fcon` and `fcof`, otherwise the mode can be turned on and not off. The command can be sent to any node: a follower forwards the request to the current leader, and the leader propagates the mode to all peers on a best-effort basis. Since the propagation is best-effort and the mode is neither persisted nor replicated through the Raft log, verify `zk_full_consensus_mode` in `mntr` output on every node after issuing the command, and re-issue it if some node did not receive it (for example, a restarted server always starts with the mode turned off, and a server running an older version ignores the request while still reporting success). Recovery mode (`rcvr`) takes precedence over full consensus mode for the commit quorum: while a custom quorum size is in force, commits follow it and not full consensus, although members are still told whether full consensus would have excluded them.

```response
Sent full consensus mode ON request to leader.
```

- `fcof`: Request the leader to turn off full consensus mode and return to majority quorum. The same propagation and verification caveats as for `fcon` apply.
