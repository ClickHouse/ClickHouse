...
- `bpon`: Ask the leader to wait for replicas that cannot keep up. While it is on, the leader does not advance the commit index past any replica it can reach, so a replica that has fallen behind can close the gap instead of drifting until it needs a snapshot. Writes then commit at the speed of the slowest reachable replica, so switch it on deliberately, watch the replicas catch up, and switch it off again with `bpof`.

  There is no threshold and no time limit: every replica the leader can reach is waited for, however far behind it is, including one that is receiving a snapshot. A replica is left out only when the leader cannot reach it at all - its last request failed, it has not answered within four heartbeat intervals, or it has not answered at all yet on a new connection - because waiting cannot help such a replica and would freeze the commit index for nothing.
  ...
  The command can be sent to any node: a follower forwards it to the current leader, which applies it and propagates it to all peers. Only the leader's copy has any effect; every node reports its own in `mntr` as `slow_member_backpressure`, so verify there and re-issue if a node did not receive it. The setting lasts for one leadership - a new leader switches it off - and is not persisted, so a restarted server starts with it off.

```bash
$ echo bpon | nc localhost 9181
Sent slow member backpressure ON request to leader. Writes now commit at the speed of the slowest reachable replica. A new leader switches it off, so send this again after a leader change.
```

- `bpof`: Ask the leader to stop waiting for replicas that cannot keep up. The same propagation and verification caveats as for `bpon` apply.
