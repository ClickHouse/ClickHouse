...
| `log_startup_read_max_streams`     | Maximum number of changelog files read concurrently during Keeper startup. `0` = use the serial (pre-parallel) startup read. Effective parallelism is capped by the number of changelog files that need to be read; consider lowering on seek-bound storage (HDD, IOPS-capped volumes). | `16`                                                                                                         |
