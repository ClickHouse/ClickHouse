...
#### Per-server coordination settings {#per-server-coordination-settings}

Coordination settings are normally identical on all servers of a cluster, and it is convenient to keep the whole `<keeper_server>` section identical too. If a setting has to differ on some servers, for example to migrate a cluster to a new storage engine one server at a time, put the overrides into the `<keeper_server>.<per_server_coordination_settings>` section. It contains one `<server-N>` element per server, where `N` is the `server_id` of the server the overrides apply to, with the settings to override inside. A server applies the overrides from its own element on top of `<coordination_settings>` and ignores the others. Setting names are validated on all servers, so a typo is reported everywhere. Settings that support hot reload also support it here.
