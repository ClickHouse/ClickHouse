...
- `bpon`: Ask the leader to wait for replicas that cannot keep up. While it is on, the leader does not advance the commit index past any replica it can reach, so a replica that has fallen behind can close the gap instead of drifting until it needs a snapshot. Writes then commit at the speed of the slowest reachable replica, so switch it on deliberately, watch the replicas catch up, and switch it off again with `bpof`.
  ...
  There is no lag threshold: every replica the leader can reach is waited for, however far behind it is, including one that is receiving a snapshot. A replica is left out only when waiting cannot get it anywhere - its last request failed, it made no progress within `slow_member_backpressure_no_progress_timeout_ms`, or it has not answered at all yet on a new connection and no snapshot is on its way to it either, which leaves the leader with no idea where it stands - because waiting for such a replica would freeze the commit index for nothing.
...
Slow member backpressure is ON. Writes now commit at the speed of the slowest reachable replica. A new leader switches it off, so send this again after a leader change.

$ echo bpon | nc localhost 9181   # sent to a follower
Sent slow member backpressure ON request to leader. Check `slow_member_backpressure` in `mntr` on the leader, which `server_state` identifies, to see whether it took effect.
```
