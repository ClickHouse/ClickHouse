...
| `slow_member_backpressure_max_hold_ms` | How long the leader holds the commit index back for a member that fell behind by more than `stale_log_gap` log entries, before giving up on it and leaving it behind. While the commit index is held, clients are not acknowledged, which stops the leader from outrunning that member and lets it catch up instead of drifting until it needs a snapshot; the member is released once it is back within one replication batch of the threshold, so the throttle applies in many short steps rather than one long stall. A member that does not respond, or that is receiving a snapshot, is never waited for, since it cannot catch up any faster. **No write commits while a member is being waited for**, so keep the value below `operation_timeout_ms`; if it exceeds `session_timeout_ms`, idle sessions expire too. `0` disables it. | `0`  |
