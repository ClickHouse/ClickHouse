...
`zk_sum_leader_unavailable_time` and `zk_cnt_leader_unavailable_time` are cumulative leader-only metrics. Keeper samples NuRaft leader availability once per `heart_beat_interval_ms`, so each boundary can differ from the actual transition by up to one heartbeat interval and windows shorter than that interval can be missed. `srst` resets both values.
