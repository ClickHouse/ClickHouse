...
| `log_readahead_commit_window_bytes`| Maximum total size of decoded log entries buffered ahead of the commit thread. `0` disables commit read-ahead (commit reads entries from disk one by one).                                              | `500MiB`                                                                                                     |
...
| `slow_member_backpressure_max_duration_ms` | How long the leader holds the commit index back for a replica that fell behind by more than `stale_log_gap` entries and stopped catching up, before giving up on it and leaving it behind. While it is active, clients are not acknowledged and new requests are refused, which stops the leader running further ahead of that replica and lets it catch up instead of drifting until it needs a snapshot. Nothing happens unless the backpressure is switched on with [`bpon`](#four-letter-word-commands); `0` disables it entirely. **No write commits while a replica is being waited for**, so keep this below `operation_timeout_ms`. | `0` |
| `slow_member_backpressure_gap_window_ms` | How long a replica's gap has to stop shrinking before the leader starts holding the commit index for it. Being behind is not by itself a reason to wait: a replica draining a backlog after a restart, after installing a snapshot, or right after joining is far behind and yet recovering on its own, and waiting for it would stall the cluster once per replica of a rolling restart. `0` uses four heartbeat intervals. | `0` |
| `slow_member_backpressure_max_uncommitted_log_entries` | The value `nuraft_max_uncommitted_log_entries` takes while a replica is being waited for. Holding the commit index does not stop the leader appending, so without this the log runs further ahead and the replica can fall further behind instead of catching up. `0` leaves the limit unchanged. | `0` |
...
- `bpon`: Ask the leader to slow down for replicas that cannot keep up. While on, the leader holds the commit index at the position of a replica that has fallen behind by more than `stale_log_gap` entries and has stopped closing the distance, and refuses new requests, so that the replica can catch up instead of drifting until it needs a snapshot. A replica that stops responding, or that is receiving a snapshot, is never waited for, since waiting cannot help it. The command can be sent to any node: a follower forwards it to the current leader, which applies it and propagates it to all peers. Only the leader's copy has any effect; every node reports its own in `mntr` as `slow_member_backpressure`, so verify there and re-issue if a node did not receive it. It has no effect until `slow_member_backpressure_max_duration_ms` is configured, and the setting is not persisted, so a restarted server starts with the backpressure off.

```bash
$ echo bpon | nc localhost 9181
Sent slow member backpressure ON request to leader.
```

- `bpof`: Ask the leader to stop slowing down for replicas that cannot keep up. The same propagation and verification caveats as for `bpon` apply.
...
The limit for the latest logs cache is controlled by:
- `latest_logs_cache_size_threshold` - total size of latest logs stored in cache

If the default value is too big, you can reduce the memory usage by reducing this config.
