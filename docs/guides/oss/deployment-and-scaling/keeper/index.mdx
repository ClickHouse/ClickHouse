...
| `disk_move_retries_after_init`     | The amount of retries after a failure which happened while a file was being moved between disks once the server is running. Giving up does not lose the file: a snapshot move is re-attempted by the maintenance pass of the next snapshot, and a changelog file keeps being served from the disk it is still on. It can leave an untracked copy on the source disk, in which case the space the move would have freed stays occupied until the next restart. `0` means retry until shutdown, which pins the snapshot thread or the background changelog operations thread for as long as the failure lasts | `30`                                                                                                        |
