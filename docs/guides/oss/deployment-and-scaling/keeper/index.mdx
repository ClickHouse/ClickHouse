...
The threshold counts the memory a cached entry really occupies, not just the size of the log entry
itself: every cached entry also carries the objects that keep it reachable, and its allocation is
rounded up to an allocator size class. For small entries that overhead is several times the entry,
so the number of entries the cache holds can be much lower than dividing the threshold by the entry
size suggests. `KeeperLatestLogsCacheSize` reports the same quantity the threshold bounds.
