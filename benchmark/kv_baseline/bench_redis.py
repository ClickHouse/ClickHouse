...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Benchmark Redis GET/MGET. Requires redis-py: python3 -m pip install redis.")
...
def worker(worker_id, args, keys, deadline, latencies, errors):
    try:
        import redis
    except ImportError:
        errors.append("redis-py is required. Install it with: python3 -m pip install redis")
        return
    ...
    try:
        ...
        errors.append(f"Cannot connect to Redis: {exc}")
        return
...
def main():
    ...
    for worker_id in range(args.concurrency):
        thread = threading.Thread(target=worker, args=(worker_id, args, keys, deadline, latencies, errors))
        thread.start()
    ...
    print(f"system=redis interface=resp requests={len(latencies)} elapsed={elapsed:.3f}s")
