...
{
    /// `tryParseQuery` reports a syntax error by returning null and filling in the message. A few
    /// checks in the parser still report an invalid query by throwing, and this build has no
    /// unwinding, so `wasm_runtime.cpp` turns such a throw into a jump back here - see the comment
    /// on `__cxa_throw` there.
