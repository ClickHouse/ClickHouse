...
public:
    ...
    std::string insert_query() {
        ...
        insert += ") VALUES \n";
        for (auto val_set_iter = values.begin(); val_set_iter != values.end(); ++val_set_iter) {
...
FuncRet simple_func(DB::ASTPtr ch, std::map<std::string, Column>& columns) {
    ...
    if (X) {
        ...
        } else if (func_to_param_type.count(boost::algorithm::to_lower_copy(X->name))) {
            if (no_indent) {
                std::ostringstream ss;
                formatAST(*ch, ss);
                return FuncRet(func_to_param_type[boost::algorithm::to_lower_copy(X->name)], ss.str());
            }
            return FuncRet(func_to_param_type[boost::algorithm::to_lower_copy(X->name)], "");
        }
    }
    else {
        for (const auto & child: (*ch).children) {
