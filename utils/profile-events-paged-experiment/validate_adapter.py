...
def actual_catalogue(stage):
    ...
    if len(names) != 1563 or len(set(names)) != 1563:
        raise ValueError('Expected exactly 1563 unique public builtin events')
    ...
    if len(rank) != 1563 or set(rank) != set(range(1563)):
...
def main():
    ...
    permutation = catalogue['rank'] if catalogue else required + [i for i in range(1563) if i not in required]
    ...
    def run(name, backend='paged', hot=128, test='normal', code=0, overrides=None, message=None):
        ...
        if test in ['normal', 'reserved_updates', 'allocation_failure']:
            ...
            base = 2 if backend == 'paged' and hot < 1563 else 1
            ...
            if test == 'reserved_updates':
                ...
                assert d['published_cold_allocations_sum'] == 0 if hot == 1563 or backend == 'dense' else d['published_cold_allocations_sum'] > 0
            assert d['final_requested_sum'] >= d['initial_requested_sum']
    ...
    invalid = [
        ('hot_zero', {'CH_COUNTER_HOT': '0'}), ('hot_large', {'CH_COUNTER_HOT': '1564'}),
