...
function gen_revision_author {
    ...
    if [[ $STANDALONE != 'yes' ]]; then
        ...
        if [ "$git_log_grep" == "" ]; then
            ...
            git commit -m "$auto_message [$VERSION_STRING] [$VERSION_REVISION]" dbms/cmake/version.cmake debian/changelog docker/*/Dockerfile src/Storages/System/StorageSystemContributors.generated.cpp
            if [ -z $NO_PUSH ]; then
                git push
            fi
