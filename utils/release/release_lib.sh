...
function gen_revision_author {
    ...
    if [[ $STANDALONE != 'yes' ]]; then
        ...
        if [ "$git_log_grep" == "" ]; then
            ...
            commit_message="$auto_message [$VERSION_STRING] [$VERSION_REVISION]"
            git branch "$branch"
            git checkout "$branch"
            git commit -m "$commit_message" cmake/autogenerated_versions.txt debian/changelog docker/*/Dockerfile src/Storages/System/StorageSystemContributors.generated.cpp utils/list-versions/version_date.tsv
            if [ -z $NO_PUSH ]; then
                git push origin "$branch"
                # TODO https://github.com/cli/cli/blob/trunk/docs/install_linux.md and auth
                gh pr create \
                    --base "master" \
                    --head "$branch" \
