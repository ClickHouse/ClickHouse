...
process_library() {
    ...
    then
        ...
        (${GREP_CMD} -q -i -F 'machine-executable object code generated by' "$LIB_LICENSE" &&
         echo "Boost") ||
        # Not an open-source licence. contrib/nvcomp is a proprietary NVIDIA binary that cuDF
        # requires and that has no source distribution; recognising it here keeps it visible in
        # system.licenses instead of hidden behind a missing LICENSE file.
        (${GREP_CMD} -q -i -F 'LICENSE AGREEMENT FOR NVIDIA SOFTWARE DEVELOPMENT KITS' "$LIB_LICENSE" &&
         echo "NVIDIA SDK") ||
        echo "Unknown")
