...
def replace_count(path, old, new, expected):
    ...
    n = s.count(old)
    if n != expected:
        raise RuntimeError(f"{path}: expected {expected} matches, found {n}: {old[:100]!r}")
    p.write_text(s.replace(old, new))


# Settings: retain current master workload admission settings and add the RMV-only selector.
replace_once(
...
replace_once(
    ...
    '''        addSettingsChanges(settings_changes_history, "26.9",\n        {\n            {"allow_experimental_nullable_tuple_type", false, true,''',
...
# StorageMaterializedView: keep master's RefreshMode/incremental cursor path and use one
# coherent SELECT/settings snapshot for admission and execution.
replace_once(
...
replace_once(
    ...
    '''    prepareRefresh(RefreshMode mode, ContextMutablePtr refresh_context, ASTPtr select_query, std::optional<StorageID> & out_temp_table_id,\n        const CursorTreeNodePtr & stream_cursor) const;''',
)
...
# RefreshTask header: add asynchronous admission state without regressing master's cursor mode.
replace_once(
...
replace_once(
    "src/Storages/MaterializedView/RefreshTask.cpp",
    '''    scheduling_task->deactivate();\n    execution_task->deactivate();\n\n    /// Best-effort final update of information in zookeeper''',
...
replace_once(
    ...
    "            std::tie(refresh_query, query_scope) = view->prepareRefresh(refresh_mode, refresh_context, std::move(select_query), table_to_drop, stream_cursor);",
)
replace_once(
    "src/Storages/MaterializedView/RefreshTask.cpp",
