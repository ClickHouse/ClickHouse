...
#include <cstdint>
#include <string_view>
#include <foundationdb/fdb_c_namespaced.h>

using FDBRWLockHodler = struct fdb_rwlocks_holder;

namespace foundationdb::api
{
// Signature for additional api in patch
...
#define APPLY_FDB_C_APIS(M) \
    ...
    M(fdb_delay) \
    M(fdb_rwlocks_create) \
    M(fdb_rwlocks_shared) \
    M(fdb_rwlocks_exclusive) \
    M(fdb_rwlocks_lock) \
    M(fdb_rwlocks_free)
...
// Open fdb_c shared library. The library files will be placed on path if not empty.
// Otherwise, it will be placed in /tmp and then cleared automatically.
// The library can be only loaded once. Duplicate calls will be ignored.
// fdb_load_library() must be called before any fdb api.
