 ...
 * @note The implementation balances work internally, so throughput is largely
...
/**
 * @brief The maximum supported compressed and decompressed chunk size in bytes for the Gzip decompressor when using the naive algorithm.
 * @note To maximize decompression performance, users are encouraged to compress in smaller chunks, for example 64KiB, or use the lookahead algorithm.
 */
static const size_t nvcompGzipNaiveDecompressionMaxAllowedChunkSize = (1ull << 32) - 1;

/**
 * @brief The maximum supported compressed and decompressed chunk size in bytes for the Gzip decompressor when using the lookahead algorithm.
