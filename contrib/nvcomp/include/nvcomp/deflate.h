 ...
 *
 * @note Although chunk sizes up to 2GiB are theoretically possible, compression
 * with large chunks may be very slow or use large amounts of temporary memory,
 * so caution is advised when using chunk sizes above 64KiB.
 */
static const size_t nvcompDeflateCompressionMaxAllowedChunkSize = 1u << 31;

/**
 * @brief The maximum supported compressed and decompressed chunk size in bytes for the Deflate decompressor.
