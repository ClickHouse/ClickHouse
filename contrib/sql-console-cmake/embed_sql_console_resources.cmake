...
if(NOT DEFINED SQL_CONSOLE_SOURCE_DIR)
    message(FATAL_ERROR "SQL_CONSOLE_SOURCE_DIR not defined")
endif()

if(NOT DEFINED SQL_CONSOLE_DIR)
    message(FATAL_ERROR "SQL_CONSOLE_DIR not defined")
endif()
...
if(NOT EXISTS "${SQL_CONSOLE_SOURCE_DIR}")
    message(FATAL_ERROR "SQL Console source directory not found at ${SQL_CONSOLE_SOURCE_DIR}")
endif()
...
file(GLOB_RECURSE source_files RELATIVE "${SQL_CONSOLE_SOURCE_DIR}" "${SQL_CONSOLE_SOURCE_DIR}/*")

foreach(file ${source_files})
    if(NOT IS_DIRECTORY "${SQL_CONSOLE_SOURCE_DIR}/${file}")
        # Create parent directory structure
        cmake_path(GET file PARENT_PATH file_dir)
        if(file_dir)
            file(MAKE_DIRECTORY "${SQL_CONSOLE_DIR}/${file_dir}")
        endif()

        # Gzip the file
        set(source_path "${SQL_CONSOLE_SOURCE_DIR}/${file}")
        set(dest_path "${SQL_CONSOLE_DIR}/${file}.gz")
...
# Compute relative path from generated file location to sql console dir for #embed directives
cmake_path(GET GENERATED_FILE PARENT_PATH generated_dir)
file(RELATIVE_PATH SQL_CONSOLE_DIR_RELATIVE "${generated_dir}" "${SQL_CONSOLE_DIR}")
...
function(get_mime_type filename output_var)
    ...
    elseif(ext_lower STREQUAL ".gif")
        set(${output_var} "image/gif" PARENT_SCOPE)
    ...
    elseif(ext_lower STREQUAL ".ttf")
        set(${output_var} "font/ttf" PARENT_SCOPE)
...
string(APPEND output_content "// Generated from: ${SQL_CONSOLE_DIR_RELATIVE}\n\n")
...
string(APPEND output_content "namespace DB::SQLConsole\n{\n\n")
...
file(GLOB_RECURSE all_files RELATIVE "${SQL_CONSOLE_DIR}" "${SQL_CONSOLE_DIR}/*")

# Filter out directories (only keep files)
set(resource_files "")
foreach(file ${all_files})
    if(NOT IS_DIRECTORY "${SQL_CONSOLE_DIR}/${file}")
...
foreach(file ${resource_files})
    ...
    set(relative_path "${SQL_CONSOLE_DIR_RELATIVE}/${file}")
