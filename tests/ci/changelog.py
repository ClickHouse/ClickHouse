...
def check_refs(from_ref: Optional[str], to_ref: str, with_testing_tags: bool) -> None:
    ...
    # The current release version. The loop below skips any candidate
    # predecessor tag that is *newer* than it (e.g. a tag from a parallel
    # branch). Read it from the tag name when TO_REF is a release tag — that is
    # authoritative and consistent with how each candidate's version is read
    # (get_version_from_tag) below. Only fall back to
    # cmake/autogenerated_versions.txt for a non-tag TO_REF (dev/branch use):
    # reading the file at the tag is unsafe on a recovery where the post-release
    # branch bump never landed, because the file there still describes an earlier
    # release, which makes the real predecessor look "too new" and be wrongly
    # excluded, walking `git describe` off the tag list.
    try:
        to_ref_version = get_version_from_tag(TO_REF)
    except ValueError:
