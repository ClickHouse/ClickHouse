...
def check_refs(from_ref: Optional[str], to_ref: str, with_testing_tags: bool) -> None:
    ...
    # The changelog interval starts at the previous release's commit. Read that
    # SHA straight from the version file at TO_REF (`VERSION_PREVIOUS_SHA`, which
    # the release bump records) instead of guessing the predecessor tag with
    # `git describe` + version comparison — tag-based detection breaks when a
    # branch's release tags are irregular (e.g. two releases share a patch line
    # because a bump was skipped). Fall back to `VERSION_GITHASH` for tags
    # written before `VERSION_PREVIOUS_SHA` existed (it is the previous release
    # in the normal, one-release-per-patch-line case). `with_testing_tags` no
    # longer applies (no tag walk).
    temp_cmake = get_abs_path("tests/ci/tmp/autogenerated_versions.txt")
    temp_cmake.parent.mkdir(parents=True, exist_ok=True)
    temp_cmake.write_text(runner(f"git show {TO_REF}:{FILE_WITH_VERSION_PATH}"))
    versions = read_versions(temp_cmake)
    temp_cmake.unlink()

    def _is_sha(value) -> bool:
        # read_versions int-coerces all-digit values; the 40-zero placeholder
        # therefore arrives as int 0 (falsy). A real SHA stays a non-empty str.
        return bool(value) and str(value) != "0"

    previous_sha = versions.get("previous_sha")
    if not _is_sha(previous_sha):
        previous_sha = versions.get("githash")
    assert _is_sha(previous_sha), (
        f"no previous-release SHA in {FILE_WITH_VERSION_PATH} at {TO_REF} "
        f"(VERSION_PREVIOUS_SHA / VERSION_GITHASH); pass --from explicitly"
    )
    FROM_REF = str(previous_sha)
