...
# This script lives in tests/ci but reads the release version through the
# canonical model in ci/jobs/scripts, so add the repo root to the import path.
sys.path.insert(0, os.path.join(os.path.dirname(__file__), "../.."))

from ci.jobs.scripts.clickhouse_version import CHVersion  # noqa: E402
...
def check_refs(from_ref: Optional[str], to_ref: str, with_testing_tags: bool) -> None:
    ...
    # SHA straight from the version file at TO_REF through the canonical model
    # (`VERSION_PREVIOUS_SHA`, which the release bump records) instead of guessing
    # the predecessor tag with `git describe` + version comparison — tag-based
    # detection breaks when a branch's release tags are irregular (e.g. two
    # releases share a patch line because a bump was skipped). Fall back to
    # `VERSION_GITHASH` for tags written before `VERSION_PREVIOUS_SHA` existed
    # (it is the previous release in the normal, one-release-per-patch-line
    # case). `with_testing_tags` no longer applies (no tag walk).
    version = CHVersion.get_release_version_at_ref(TO_REF)
    FROM_REF = version.previous_sha or version.githash
    assert FROM_REF and FROM_REF != "0" * 40, (
        f"no previous-release SHA in the version file at {TO_REF} "
        "(VERSION_PREVIOUS_SHA / VERSION_GITHASH); pass --from explicitly"
    )
