...
def check_refs(from_ref: Optional[str], to_ref: str, with_testing_tags: bool) -> None:
    ...
    # The version file at TO_REF bounds the baseline: the walk-back skips tags newer than it.
    ...
    while not from_ref:
        ...
        if from_ref_version <= to_ref_version:
            ...
            break
        # `-new` marks a branch point and legitimately outranks the branch; a release doing so means the version file at TO_REF is stale.
        assert not from_ref_tag.endswith(
            (f"-{VersionType.STABLE}", f"-{VersionType.LTS}")
        ), (
            f"the previous release [{from_ref_tag}] is newer than the version file at "
            f"[{TO_REF}] ([{to_ref_version.string}]): its post-release bump never "
            f"landed, and excluding it would re-list its entries here"
        )
