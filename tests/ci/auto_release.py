...
def _release_version_for_commit(commit_sha: str) -> str:
    ...
    Matches `version_helper.get_version_from_repo` (what the release driver on
    `master` uses): `major.minor.patch` come from the commit's
    `cmake/autogenerated_versions.txt`, and the tweak is the first-parent commit
    count since the nearest tag (`git describe`), i.e. `Git.tweak`. New release
    branches are already skipped by the caller (their latest tag ends with
    `new`), so the tag here is always a `-stable`/`-lts` release tag and the
    `-new` tweak adjustment does not apply.
    """
    ...
    for line in content.splitlines():
        ...
        if line.startswith("SET(VERSION_"):
            ...
            parts[name] = value
    latest_tag = Shell.get_output_or_raise(
        f"git describe --tags --abbrev=0 {commit_sha}"
    )
    tweak = Shell.get_output_or_raise(
        f"git rev-list {latest_tag}..{commit_sha} --first-parent --count"
    )
    return (
...
def _release_build_artifacts_ready(release_branch: str, commit_sha: str) -> bool:
    ...
    (or only part) was uploaded under this commit's own SHA. `CreateRelease`
    downloads each package strictly from
    `REFs/<branch>/<commit_sha>/<build_job>/<file>` (see `PackageDownloader`),
    so it would 404 on such a commit.

    A directory-level check is too weak: a build can upload some files and not
    the exact `<package>_<version>_<arch>.deb` `CreateRelease` asks for (and it
    also does not cover the macOS binaries). So enumerate the *exact* object
    keys via `PackageDownloader` — reusing it so the two cannot drift — and
    require every one to exist. This also surfaces a version-computation
    mismatch (the build stamped a different version) as a missing object, which
    is correct: `CreateRelease` would fail on it too.
    """
    # PackageDownloader lives with CreateRelease; import lazily so this module
    # stays importable where the release toolchain's deps are absent.
    from create_release import PackageDownloader

    version = _release_version_for_commit(commit_sha)
    downloader = PackageDownloader(
        release=release_branch, commit_sha=commit_sha, version=version
    )
    s3 = downloader.s3

    # The exact filenames CreateRelease downloads, grouped by their build job
    # dir: the deb/rpm/tgz/tgz.sha512 packages, plus the per-arch macOS binary
    # (a fixed `clickhouse` object under the darwin build dir).
    expected_by_job = {}  # type: dict[str, set]
    for package_file in (
        downloader.deb_package_files
        + downloader.rpm_package_files
        + downloader.tgz_package_files
    ):
        job = downloader.file_to_job_name[package_file]
        expected_by_job.setdefault(job, set()).add(package_file)
    for job in downloader.macos_binary_to_job_name.values():
        expected_by_job.setdefault(job, set()).add("clickhouse")

    for job, expected_files in expected_by_job.items():
        prefix = f"{downloader.s3_release_prefix}/{commit_sha}/{job}/"
