...
from git_helper import Git, GIT_PREFIX, Runner
        ...
        with checkout(self.release_branch):
            with checkout_new(branch_upd_version_contributors):
                ...
                cmd_commit_version_upd = (
                    f"{GIT_PREFIX} commit '{CMAKE_PATH}' '{CONTRIBUTORS_PATH}' -m 'Update autogenerated version to {self.version} and contributors'",
                )
...
class PackageDownloader:
    ...
    def __init__(self, release, commit_sha, version: str):
        ...
        for package_type in self.PACKAGE_TYPES:
            for package in self.package_names:
                package_file_name = (
                    ...
                    + self.version
                    + "_"
                    + self._get_arch_suffix(package_type, RepoTypes.DEBIAN)
                    + ".deb"
                )
                self.deb_package_files.append(package_file_name)
                self.file_to_type[package_file_name] = package_type
                ...
                package_file_name = (
                    ...
                    + self.version
                    + "."
                    + self._get_arch_suffix(package_type, RepoTypes.RPM)
                    + ".rpm"
                )
                self.rpm_package_files.append(package_file_name)
                self.file_to_type[package_file_name] = package_type
                ...
                package_file_name = (
                    ...
                    + self.version
                    + "-"
                    + self._get_arch_suffix(package_type, RepoTypes.TGZ)
                    + ".tgz"
                )
                self.tgz_package_files.append(package_file_name)
                self.file_to_type[package_file_name] = package_type
                package_file_name += ".sha512"
                self.tgz_package_files.append(package_file_name)
                self.file_to_type[package_file_name] = package_type
