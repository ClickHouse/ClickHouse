...
def main():
    ...
    with checkout(release_branch):
        with checkout_new(branch_upd_version_contributors):
            ...
            cmd_create_pr = f"gh pr create --repo {GITHUB_REPOSITORY} --title 'Update version after release' --head {branch_upd_version_contributors} --body-file '{body_file} --label 'do not test' --assignee @me"
            if not dry_run:
