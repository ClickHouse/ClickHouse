...
def push_with_pat(
    refspec: str, dry_run: bool, force: bool = False, strict: bool = True
) -> bool:
    """Push `refspec` to `origin` as robot-clickhouse, the way the original
    `create_release` did.

    This is a plain `git push origin` with the `GIT_PREFIX` identity, so it
    authenticates with the robot's own credential the release job provides --
    the `ROBOT_CLICKHOUSE_SSH_KEY` loaded into the ssh-agent in `main` (used
    via `GIT_PREFIX`'s `core.sshCommand`), or the robot PAT the checkout
    persists. That robot identity is allowed to push to the protected release
    branches and the release tag.

    It deliberately does NOT build an `https://x-access-token:...@github.com`
    URL: that form authenticates as the GitHub App (the token the `gh` session
    carries in the Praktika release job), and branch protection rejects an
    App-authenticated push to a release ref. Kept local to `create_release`
    for now rather than in `ci.praktika.git`, whose `Git.push` intentionally
    uses the App token for the non-release (native_jobs) push path.
    """
    force_flag = "--force " if force else ""
    return Shell.check(
        f"{GIT_PREFIX} push {force_flag}origin {refspec}",
        strict=strict,
        dry_run=dry_run,
        verbose=True,
    )
...
class ReleaseInfo:
    ...
    def prepare(
        ...
        if release_type == "patch":
            ...
            # Refuse to release a ref if a later release already exists on the
            # branch. Every release rewrites cmake/autogenerated_versions.txt
            # (post-release version bump), so its contents are constant from one
            # release to the next. If the file at `commit_ref` differs from the
    ...
    def push_release_tag(self, dry_run: bool) -> None:
        ...
        push_with_pat(
            f"refs/tags/{self.release_tag}:refs/tags/{self.release_tag}",
            dry_run=dry_run,
        )
    ...
    def push_new_release_branch(self, dry_run: bool) -> None:
        ...
        with checkout("master"):
            with checkout_new(new_release_branch):
                push_with_pat(
                    f"HEAD:refs/heads/{new_release_branch}", dry_run=dry_run
                )
    ...
    def update_version_and_contributors_list(self, dry_run: bool) -> None:
        ...
        with checkout(self.release_branch):
            ...
            diff_rc = Shell.run(
                f"git diff --quiet HEAD -- '{CMAKE_PATH}' '{CONTRIBUTORS_PATH}'"
            )
            ...
            if files_changed:
                ...
                push_with_pat(
                    f"HEAD:refs/heads/{self.release_branch}", dry_run=dry_run
                )
            if dry_run:
                Shell.check(
