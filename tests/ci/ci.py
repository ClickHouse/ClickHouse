...
from ci_config import CI_CONFIG, CommitTokens
...
def _update_gh_statuses(indata: Dict, s3: S3Helper) -> None:
    if indata["ci_flags"][CommitTokens.NO_CI_CACHE]:
...
def _fetch_commit_tokens(message: str) -> List[str]:
    ...
    res = [
        match for match in matches if match in CommitTokens or match.startswith("job_")
    ]
    return res
...
def main() -> int:
    ...
    if args.configure:
        ...
        tokens = []
        ci_flags = {
            CommitTokens.NO_MERGE_COMMIT: False,
            CommitTokens.NO_CI_CACHE: False,
        }
        if (pr_info.number != 0 and not args.skip_jobs) or args.commit_message:
            ...
            if CommitTokens.NO_MERGE_COMMIT in tokens and CI:
                git_runner.run(f"{GIT_PREFIX} checkout {pr_info.sha}")
                git_ref = git_runner.run(f"{GIT_PREFIX} rev-parse HEAD")
                ci_flags[CommitTokens.NO_MERGE_COMMIT] = True
                print("NOTE: Disable Merge Commit")
        if CommitTokens.NO_CI_CACHE in tokens:
            ci_flags[CommitTokens.NO_CI_CACHE] = True
        ...
        jobs_data = (
            _configure_jobs(
                ...
                pr_info.labels,
                tokens,
                ci_flags[CommitTokens.NO_CI_CACHE],
            )
    ...
    ### RUN action: start
        ...
        if is_build_job(check_name):
            build_result = (
                ...
                if not indata["ci_flags"][CommitTokens.NO_CI_CACHE]
                # if the cache is disabled build still could be reused but only from the current branch
                else BuildResult.load(check_name, pr_info.number, pr_info.head_ref)
            )
            if build_result:
