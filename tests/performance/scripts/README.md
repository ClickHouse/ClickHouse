...
In CI, most tests run only a random sample of their queries (`perf.py --max-queries 10`); only whole-suite benchmarks such as `tpch.xml` run every query. A test that is meant to be a stable, fixed workload can opt out of the sampling with `<test run_all_queries="1">` — then all of its queries run on every invocation, regardless of `--max-queries`.
