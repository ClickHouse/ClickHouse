#!/usr/bin/env python3
"""Integration test for Keeper V9 snapshot format with ZSTD compression.

Verifies that a single-node Keeper can:
- Start with `write_snapshot_version=9` and `use_compression=true`.
- Trigger a V9 snapshot by writing enough entries to exceed `snapshot_distance`.
- Restart and successfully restore state from the V9 snapshot.
...
# Node uses the base Keeper config, the compression overlay, and the V9 overlay.
...
def test_v8_snapshot_start_and_restart(started_cluster):
    ...
    data across a V9 snapshot, and restores state after a restart.

    The test forces an actual V9 snapshot to be created (not just WAL replay) by:
    ...
    3. Asserting all written data survives the restart, proving the V9 read path ran.
    """
    ...
    try:
        ...
        # guaranteeing a V9 snapshot is triggered before the restart.
    ...
    # Wait for Keeper to confirm it wrote the V9 snapshot to disk.
    # This verifies the V9 write path ran, not just WAL accumulation.
    ...
    # Restart so the node loads exclusively from the V9 snapshot on-disk.
    node_v8.restart_clickhouse(kill=True)
    keeper_utils.wait_nodes(cluster, [node_v8])

    # Verify all written nodes are present, proving the V9 read path restored state.
    zk2 = keeper_utils.get_fake_zk(cluster, node_v8.name, timeout=30.0)
    try:
        for i in range(5):
            data, _ = zk2.get(f"{prefix}/node{i}")
            assert data == f"value{i}".encode(), (
                f"Node {i} has wrong data after V9 snapshot restore: {data!r}"
            )
    finally:
        try:
            zk2.stop()
