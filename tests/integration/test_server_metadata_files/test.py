...
def test_window_view_attach_wrapped_interval(started_cluster):
    # Converted from stateless test 05019_window_view_attach_wrapped_interval.sh.
    #
    # A `WINDOW VIEW` stored before interval validation was added can hold an interval that
    # overflows when it is converted to seconds. The short `ATTACH` loads stored metadata and
    # must accept it (otherwise the view becomes permanently unattachable across an upgrade),
    # while a full-definition `ATTACH` is `CREATE`-like user input and stays rejected.
    settings = {
        "allow_experimental_window_view": 1,
        "allow_experimental_analyzer": 0,
    }
    view = "window_view_attach_wrapped_interval"

    node.query(f"DROP TABLE IF EXISTS {view}", settings=settings)
    node.query(
        f"CREATE WINDOW VIEW {view} ENGINE = Memory"
        " AS SELECT count() AS c, tumble(now(), toIntervalDay(1), 'UTC') AS w FROM system.one GROUP BY w",
        settings=settings,
    )

    metadata_path = node.query(f"SELECT metadata_path FROM system.tables WHERE database = 'default' AND name = '{view}'").strip()
    node.query(f"DETACH TABLE {view}", settings=settings)

    # Edit the metadata through `clickhouse disks`: with a remote database disk the `.sql` is
    # not on the local filesystem at all.
    metadata = read_metadata(node, metadata_path)
    assert "toIntervalDay(1)" in metadata, f"'toIntervalDay(1)' not found in {view} metadata before rewrite"
    write_metadata(node, metadata_path, metadata.replace("toIntervalDay(1)", "toIntervalDay(2147483648)"))
    db_disk_name = get_database_disk_name(node)
    if db_disk_name != "default":
        # A plain-rewritable database disk caches metadata; reload it to see the change.
        node.query(f"SYSTEM CLEAR DISK METADATA CACHE {db_disk_name}")
    assert "toIntervalDay(2147483648)" in read_metadata(node, metadata_path), f"'toIntervalDay(2147483648)' not present in {view} metadata after rewrite"

    node.query(f"ATTACH TABLE {view}", settings=settings)
    assert node.query(f"SELECT count() FROM system.tables WHERE database = 'default' AND name = '{view}'") == "1\n"

    node.query(f"DROP TABLE {view} SYNC", settings=settings)

    # A full-definition `ATTACH` of the same unsafe interval is user input and stays rejected.
    assert "BAD_ARGUMENTS" in node.query_and_get_error(
        f"ATTACH WINDOW VIEW {view} ENGINE = Memory"
        " AS SELECT count() AS c, tumble(now(), toIntervalDay(2147483648), 'UTC') AS w FROM system.one GROUP BY w",
        settings=settings,
    )
