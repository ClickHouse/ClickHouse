...
# A pooled shared-memory UDF that extends its region's file past the server's mapping, reports
# whether a fixed offset in that tail is clean, and then dirties it. Otherwise identical to
# shm_udf.py. Arguments: --extend-to BYTES --peek-at OFFSET.
#
...
def process(input_data, region, region_size):
    ...
    # "clean" if the 4 KiB at `--peek-at` are all zero, "dirty" otherwise. A
    ...
    probe_from = int(sys.argv[sys.argv.index("--peek-at") + 1])
