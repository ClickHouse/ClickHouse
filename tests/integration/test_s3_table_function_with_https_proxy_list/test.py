...
def test_s3_with_proxy_list(cluster):
    ...
    node.query(
        ...
        s3('https://minio1:9001/root/data/ch-proxy-test/test.csv', 'minio', 'minio123', 'CSV', 'key String, value String')
    ...
    assert (
        node.query(
            "SELECT * FROM s3('https://minio1:9001/root/data/ch-proxy-test/test.csv', 'minio', 'minio123', 'CSV') FORMAT Values"
        )
        == "('color','red'),('size','10')"
    )
