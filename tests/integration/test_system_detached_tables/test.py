...
node = cluster.add_instance("replica1",
    with_zookeeper=True,
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            "test_db_atomic", "Atomic", "MergeTree", "ORDER BY n", id="Atomic db with MergeTree table"
        ),
        pytest.param(
            "test_db_lazy", "Lazy(60)", "Log", "", id="Lazy db with Log table"
        ),
        pytest.param(
            "test_db_repl", "Replicated('/clickhouse/tables/test_table', 'shard1', 'replica1')", "ReplicatedMergeTree", "ORDER BY n", id="Replicated db with ReplicatedMergeTree table"
        ),
    ],
)
def test_system_detached_tables(start_cluster, db_name, db_engine, table_engine, table_engine_params):
    ...
    node.query(f"CREATE TABLE {db_name}.test_table (n Int64) ENGINE={table_engine} {table_engine_params};")
    node.query(f"CREATE TABLE {db_name}.test_table_perm (n Int64) ENGINE={table_engine} {table_engine_params};")
    ...
    assert "" == node.query(f"SELECT * FROM system.detached_tables WHERE database='{db_name}'")

    node.query(f"SET database_replicated_always_detach_permanently=1; DETACH TABLE {db_name}.test_table")
