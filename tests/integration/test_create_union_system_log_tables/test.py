...
def test_recreated_on_rotation(start_cluster):
    ...
    # A modified union table is left intact until the next rotation of the log table
    # (or a server restart). The replacement table imitates a union table generated by an
    # older version of the server: a proxy over a table function with an obsolete structure
    # and the ownership marker in the comment. A table that lacks the marker was created by
    # a user and is never replaced, see `test_preexisting_user_table_is_preserved` and
    # `test_preexisting_proxy_table_is_preserved`.
    node3.query("DROP TABLE system.all_query_log SYNC")
    node3.query(
        "CREATE TABLE system.all_query_log (dummy UInt8) AS merge('system', '^one$') "
...
def test_preexisting_user_table_is_preserved(start_cluster):
    ...
    # The conflict is not a permanent failure: once the name is free again, the union table
    # is created at the very next flush, without a restart or a rotation.
    node3.query("DROP TABLE system.all_query_log SYNC")
    node3.query("SYSTEM FLUSH LOGS query_log")
    assert "AS merge('system', '^query_log(_[0-9]+)?$')" in node3.query(
        "SHOW CREATE TABLE system.all_query_log FORMAT TSVRaw"
    )


def test_preexisting_proxy_table_is_preserved(start_cluster):
    # A hand-written proxy over a table function is stateless, but it is still a user table:
    # only the tables carrying the ownership marker in the comment are replaced.
    node3.query("SYSTEM FLUSH LOGS query_log")
    node3.query("DROP TABLE IF EXISTS system.all_query_log SYNC")
    node3.query(
        "CREATE TABLE system.all_query_log (dummy UInt8) AS merge('system', '^one$') COMMENT 'my proxy'"
    )

    # Trigger a rotation, so that the union table is checked in the most aggressive way.
    node3.query("ALTER TABLE system.query_log ADD COLUMN test_preexisting_proxy UInt8")
    node3.restart_clickhouse()
    node3.query("SYSTEM FLUSH LOGS query_log")

    create_query = node3.query("SHOW CREATE TABLE system.all_query_log FORMAT TSVRaw")
    assert "'^one$'" in create_query
    assert "my proxy" in create_query

    # Renaming the table frees the name, and the union table is created at the next flush.
    node3.query("RENAME TABLE system.all_query_log TO system.my_all_query_log")
    node3.query("SYSTEM FLUSH LOGS query_log")
    assert "AS merge('system', '^query_log(_[0-9]+)?$')" in node3.query(
        "SHOW CREATE TABLE system.all_query_log FORMAT TSVRaw"
    )
    assert "'^one$'" in node3.query(
        "SHOW CREATE TABLE system.my_all_query_log FORMAT TSVRaw"
    )
    node3.query("DROP TABLE system.my_all_query_log SYNC")
