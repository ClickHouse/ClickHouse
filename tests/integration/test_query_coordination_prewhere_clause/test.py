...
def started_cluster():
    try:
        ...
        node1.query(
            """CREATE TABLE local_table ON CLUSTER test_two_shards (A Int64, B Int8, C String) ENGINE = ReplicatedMergeTree('/clickhouse/tables/{shard}/local_table', '{replica}') ORDER BY A SETTINGS index_granularity=100;"""
        )

        node1.query(
            """CREATE TABLE distributed_table ON CLUSTER test_two_shards (A Int64, B Int8, C String) ENGINE = Distributed(test_two_shards, default, local_table, rand());"""
        )
