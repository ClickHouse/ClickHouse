...
def _create_tbl(node, name, interval):
    ...
    """)
    _query(node, f"INSERT INTO {name} SELECT number, toFloat64(rand())/4294967296.0 FROM numbers(200000)")
...
def _create_src_tbl(node, name, interval):
    ...
    _query_retry(node, f"""
        ...
        SETTINGS refresh_statistics_interval = {interval}
    """)
    _query(node, f"INSERT INTO {name} SELECT number, toFloat64(rand())/4294967296.0, toDate('2024-10-01') + (number%10) FROM numbers(300000)")
    _query_retry(node, f"ALTER TABLE {name} ADD STATISTICS v TYPE TDigest")
    _query_retry(node, f"ALTER TABLE {name} MATERIALIZE STATISTICS ALL")

def _create_join(node, prefix, interval):
    l, p = f"{prefix}_l", f"{prefix}_p"
    _query_retry(node, f"DROP TABLE IF EXISTS {l} SYNC; DROP TABLE IF EXISTS {p} SYNC")
    _query_retry(node, f"""
        CREATE TABLE {l} (
    ...
    _query_retry(node, f"""
        ...
        SETTINGS refresh_statistics_interval = {interval}
    """)
    _query(node, f"INSERT INTO {l} SELECT number, 1.0, 0.05, toDate('2024-09-01') + (number%30) FROM numbers(120000)")
    _query(node, f"INSERT INTO {p} SELECT number, if(number%5=0,'PROMO','OTHER') FROM numbers(120000)")
...
def _create_rep(interval):
    ...
    _query_retry(r1, f"ALTER TABLE {db}.{table} ADD STATISTICS v TYPE TDigest")
    _query_retry(r1, f"ALTER TABLE {db}.{table} MATERIALIZE STATISTICS ALL")

    _query_retry(r1, f"SYSTEM SYNC REPLICA {db}.{table}", retry_count=60, sleep_time=0.2)
    _query_retry(r2, f"SYSTEM SYNC REPLICA {db}.{table}", retry_count=60, sleep_time=0.2)
    return db, table
...
def test_staleness_after_inserts_stays_hit():
    ...
    _query(ch1, "INSERT INTO ins_tbl SELECT number+200000, toFloat64(rand())/4294967296.0 FROM numbers(200000)")
    _wait_hit(
        ch1, "ins-hit2",
...
def test_mutation_optimize_replace_drop_keep_hit():
    ...
    part = _query(ch1, "SELECT name FROM system.parts WHERE table='mut_tbl' AND active LIMIT 1 FORMAT TabSeparated").strip()
...
def test_types_smoke_and_nullable():
    ...
    _query(ch1, "INSERT INTO t_cm SELECT number, if(number%4=0,'PROMO',concat('X',toString(number%1000))) FROM numbers(200000)")
...
def test_per_replica_cache_and_restart_needed():
    db, table = _create_rep(0)
    _query(r1, f"SELECT count() FROM {db}.{table} WHERE v>0.99 AND id>=0 SETTINGS use_statistics_cache=1, log_comment='rep-r1-pre' FORMAT Null")
    ...
    _wait_hit(
        r2, "rep-post-r2",
        f"SELECT count() FROM {db}.{table} WHERE v>0.99 AND id>=0 SETTINGS use_statistics_cache=1, log_comment='rep-post-r2' FORMAT Null"
    )

def test_replication_inserts_keep_hit():
    db, table = _create_rep(1)
    _wait_hit(
        r1, "rep-warm",
        f"SELECT count() FROM {db}.{table} WHERE v>0.99 AND id>=0 SETTINGS use_statistics_cache=1, log_comment='rep-warm' FORMAT Null"
    )
    _query(r2, f"INSERT INTO {db}.{table} SELECT number+200000, toFloat64(rand())/4294967296.0 FROM numbers(100000)")
    _wait_hit(
        r1, "rep-post",
        f"SELECT count() FROM {db}.{table} WHERE v>0.99 AND id>=0 SETTINGS use_statistics_cache=1, log_comment='rep-post' FORMAT Null"
    )
