...
node1 = cluster.add_instance(
    "node1", main_configs=["configs/keeper_config.xml", "configs/disks.xml"], stay_alive=True, with_zookeeper=True, with_minio=True
)
...
def test_merge_session_expired_zero_copy(started_cluster):
    ...
    node1.query("alter table tab update x = x + 1, y = y + 1 where 1 settings mutations_sync=2")
    node1.query("select * from tab")
    node1.query("alter table tab update x = x + 1 where 1 settings mutations_sync=2")
    node1.query("select * from tab")

    node1.query("alter table tab add column z UInt64 materialized x + sleepEachRow(0.05) settings mutations_sync=2")
    node1.query("optimize table tab final settings alter_sync=0")
