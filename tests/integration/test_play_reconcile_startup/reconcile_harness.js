...
async function main() {
    ...
    /// Guard (no-saved-tabs adopt, entry re-owned + later fold lands): the analog of the merge/
    ...
    /// during load — the second edit lands AFTER reconciliation completes. A trusted edit updates
    /// only the in-memory tab (history entries are written on runs and structural tab changes, not
    /// per keystroke), so the entry deliberately keeps the adopted query; the re-own matters because
    /// the NEXT fold — here a structural `addTab`, which refreshes the tab being left — must land in
    /// the entry rather than bail on the dropped tab's foreign `history.state` (`tabId === 't7'`,
    /// `refreshCurrentHistoryEntry` bails on another tab's entry), which would also keep
    /// mis-resolving Back/Forward to the dropped tab. Reconciliation must re-own the entry for the
    /// recreated tab, exactly as the dirty branches do.
    {
        const stale_hash = Buffer.from('SELECT 111', 'utf8').toString('base64');
        const r = await runScenario(js, {
            href: base + '?tab=Scratch#' + stale_hash,
        ...
        /// A trusted keystroke AFTER reconciliation completes: it updates only the in-memory tab.
        /// The entry/URL deliberately keep the adopted query — keystrokes no longer rewrite browser
        /// history — but the entry must still be OWNED by the recreated tab, or the fold below bails.
        vm.runInContext(
            "query_area.value = 'SELECT 999';" +
            "query_area.dispatchEvent({ type: 'input', isTrusted: true });",
            r.sandbox);
        const edited_url = new URL(r.sandbox.location.href);
        const edited_live_query = vm.runInContext(
            "tabs.find(t => t.id === '" + live_tab.id + "').query", r.sandbox);
        check('adopt-stale-reload-entry-reowned', 'a trusted edit updates the tab in memory, not the entry',
            Buffer.from(edited_url.hash.slice(1), 'base64').toString('utf8') === 'SELECT 111'
                ...
                && edited_live_query === 'SELECT 999',
            { hash: r.sandbox.location.href, state: r.sandbox.history.state, live_query: edited_live_query });

        /// A structural tab change (addTab folds the tab being left into its entry via
        /// `refreshCurrentHistoryEntry`): with the entry re-owned the fold lands, recording the edit;
        /// with the foreign `t7` state it would bail and the edit would never reach the entry. The
        /// fold is synchronous (capture + refresh) while the switch to the new tab's own entry is
        /// deferred (`activateTab(...).then(syncHistory)`), so sample the entry right after the call.
        vm.runInContext('addTab();', r.sandbox);
        const folded_state = r.sandbox.history.state;
        const folded_url = new URL(r.sandbox.location.href);
        check('adopt-stale-reload-entry-reowned', 'a later structural fold lands in the re-owned entry instead of bailing',
            folded_state && folded_state.tabId === live_tab.id && folded_state.query === 'SELECT 999'
                && Buffer.from(folded_url.hash.slice(1), 'base64').toString('utf8') === 'SELECT 999',
            { state: folded_state, url: r.sandbox.location.href });
    }
