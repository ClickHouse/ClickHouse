...
node_1 = cluster.add_instance(
    ...
    main_configs=[
        ...
        "configs/enable_uuids_for_columns_is_false.xml",
    ],
    with_zookeeper=True,
...
def test_atomic_create_table_on_cluster(started_cluster, node):
    ...
    assert "Unknown expression identifier `uuid`" in str(exc.value)
    with pytest.raises(QueryRuntimeException) as exc:
        node_2.query(
            "SELECT uuid FROM system.columns WHERE table='atomic_cluster_table'"
        )
...
def test_atomic_create_table_on_single_node(started_cluster):
    """
    On non cluster queries in Atomic database the columns don't have uuid field in system.columns
    """
