...
node = cluster.add_instance(
    "node",
    main_configs=["configs/named_collections.xml", "configs/remote_host_filter.xml"],
    with_postgres=True,
    stay_alive=True,
)
...
def test_postgresql_database_engine_respects_remote_host_filter(started_cluster):
    ...
    # successfully, proving the filter accepts the whitelisted host rather than
    # rejecting every host.
    node.query("DROP DATABASE IF EXISTS pg_db_allowed")
    node.query(f"CREATE DATABASE pg_db_allowed ENGINE = PostgreSQL(pg_ssl_db, sslmode='verify-full', sslrootcert='{CA_CERT_PATH}')")
