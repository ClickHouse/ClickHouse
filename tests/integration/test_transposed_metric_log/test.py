...
def insert_into_transposed_metric_log(node, table_name):
...
def insert_into_metric_log(node, table_name):
    ...
    SELECT hostname(), toDate('2024-10-01'), toDateTime('2024-10-01 00:00:00') + number, toDateTime64(toDateTime('2024-10-01 00:00:00') + number, 6), {number_part} FROM numbers(86400)
    """

    query = INGEST_INTO_METRIC_LOG.format(i="number", number_part=data_str, table_name=table_name)
...
def test_data_correctness(start_cluster):
    ...
    insert_into_transposed_metric_log(node2, "transposed_metric_log")
    insert_into_metric_log(node2, "metric_log_0")
    ...
    old_data = TSV(node2.query("SELECT * EXCEPT (event_time_microseconds) FROM system.metric_log_0 WHERE event_date == toDate('2024-10-01') order by event_time LIMIT 1"))
    new_data = TSV(node2.query("SELECT * EXCEPT (event_time_microseconds) FROM system.metric_log WHERE event_date == toDate('2024-10-01') order by event_time LIMIT 1"))

    assert old_data == new_data

    old_data = TSV(node2.query("select sum(h) from (select cityHash64(*) as h FROM (SELECT * EXCEPT (event_time_microseconds) FROM system.metric_log_0 WHERE event_date == toDate('2024-10-01') order by event_time)) group by all"))
    new_data = TSV(node2.query("select sum(h) from (select cityHash64(*) as h FROM (SELECT * EXCEPT (event_time_microseconds) FROM system.metric_log WHERE event_date == toDate('2024-10-01') order by event_time)) group by all"))
