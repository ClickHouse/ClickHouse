    ...
    try:
        with ThreadPoolExecutor(max_workers=1) as executor:
            ...
            try:
                ...
                # 26.2 has no introspection listener for releasing the pause after
                # normal listeners close. Release it when the shutdown flag is set.
                shutdown_node.query("SYSTEM ENABLE FAILPOINT async_insert_disable_pause_before_schedule_on_shutdown")
                ...
                assert shutdown_node.contains_in_log("Shutting down the asynchronous insertion queue", from_host=True)
                ...
                if stop is None:
                    ...
                    shutdown_node.query("SYSTEM DISABLE FAILPOINT async_insert_disable_pause_before_schedule_on_shutdown")
                elif not stop.done():
                    stop.result(timeout=90)
                if stop is not None:
