...
def create_tables():
    ...
    q(
        ch1,
        "INSERT INTO rmt SELECT number, today(), '' FROM numbers(1e6);"
    )
    q(
        ch1,
        "INSERT INTO rmt SELECT number, today()-60, '' FROM numbers(1e5);"
    )
...
def check_tables(converted):
    ...
    ).strip() == f"replacing\t{engine_prefix}ReplacingMergeTree\nrmt\t{engine_prefix}MergeTree"
    assert q(
        ch1,
...
def set_convert_flags():
    ...
    ch1.exec_in_container(
        ["bash", "-c", f"touch /var/lib/clickhouse/data/{database_name}/log/flags/convert_to_replicated"]
    )
