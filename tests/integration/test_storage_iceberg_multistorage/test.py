...
def _rewrite_data_paths_to_local_uri(started_cluster, table_name, authority):
    """Rewrite the table's data-file references to `file://{authority}/<original absolute path>` and put
    the parquet files at that path on node1's filesystem, deleting the base-bucket copies so a read that
    wrongly resolves the path against the base storage cannot succeed. Returns `base_path`."""
    temp_dir, host_path, base_path = _download_table_for_relocation(started_cluster, table_name)
    data_dir = os.path.join(host_path, "data")

    _rewrite_manifests_and_reupload(
        started_cluster, host_path, base_path,
        lambda p: f"file://{authority}/{base_path}/data/{os.path.basename(p)}")
...
def test_file_uri_with_remote_authority_is_rejected(started_cluster):
    ...
    base_path = _rewrite_data_paths_to_local_uri(started_cluster, TABLE_NAME, "other-host")
    ...
    # `DETACH`, not `DROP`: this cluster enables `iceberg_delete_data_on_drop`, and the drop enumerates
    # the reachable files, which fails closed on the very path the test poisoned. `DatabaseCatalog` then
    # retries the drop forever, so `DROP TABLE ... SYNC` would never return.
