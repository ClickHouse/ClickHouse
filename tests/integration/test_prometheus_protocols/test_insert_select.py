...
def test_select_metrics_type_aware_family_membership():
    """Family membership is decided by the metric type on the metrics side, so a metric whose name merely ends
    in a reserved suffix links to its own family rather than a suffix-stripped one. A gauge named `queue_count`
    keeps family `queue_count` (it is not misfiled under `queue`), while a real histogram's `_bucket` series
    still links to its `http_request_duration` family."""
    # A gauge whose name genuinely ends in '_count'; its family is 'queue_count', type 'gauge'.
    node.query(
        "INSERT INTO prometheus (metric_name, tags, time_series, metric_family, type, unit, help) VALUES"
        " ('queue_count', {'q': 'jobs'}, [(toDateTime64(1000, 3), 7.0)], 'queue_count', 'gauge', 'items', 'Queue length')"
    )
    # A real histogram: its '_bucket' series links to family 'http_request_duration', type 'histogram'.
    node.query(
        "INSERT INTO prometheus (metric_name, tags, time_series, metric_family, type, unit, help) VALUES"
        " ('http_request_duration_bucket', {'le': '0.5'}, [(toDateTime64(2000, 3), 3.0)], 'http_request_duration', 'histogram', 'seconds', 'Request duration')"
    )

    # Each series links to its own family's metadata. Orphan member rows (the histogram's absent '_count'/'_sum'
    # members) carry an empty metric_name and are filtered out here.
    assert node.query(
        "SELECT metric_name, metric_family, type, unit FROM prometheus"
        " WHERE metric_name != '' ORDER BY metric_name"
    ) == TSV([
        ["http_request_duration_bucket", "http_request_duration", "histogram", "seconds"],
        ["queue_count",                  "queue_count",           "gauge",     "items"],
    ])


def test_select_where_metric_family_pushdown():
    """A filter on `metric_family` is pushed onto the metrics table read (its primary key is
    `metric_family_name`). Verifies the pushdown keeps results correct on both the metrics-only path and the
    tags+metrics join path (a too-strict pushed predicate would wrongly drop rows)."""
    insert_two_metric_families()  # series http_requests_total (counter), memory_bytes (gauge)

    # metrics-only path.
    assert node.query(
        "SELECT metric_family FROM prometheus WHERE metric_family = 'http_requests'"
    ) == TSV([["http_requests"]])
    assert node.query(
        "SELECT metric_family, type FROM prometheus WHERE startsWith(metric_family, 'mem')"
    ) == TSV([["memory_bytes", "gauge"]])

    # tags + metrics join path: only the series of the requested family survive.
    assert node.query(
        "SELECT metric_name, metric_family FROM prometheus WHERE metric_family = 'http_requests' ORDER BY metric_name"
    ) == TSV([["http_requests_total", "http_requests"]])
...
def test_select_timestamp_filter_keeps_series_with_null_min_max_time():
    ...
    does not maintain them). With `skip_reading_samples_for_timestamp_filter=0` the exact samples-table filter
    still applies: the coarse tags-table range pruning must not drop such a series, and it is correctly excluded
    for a window that genuinely does not contain its sample. (The default-on, coarser behaviour is covered by
    test_select_timestamp_filter_min_max_only_over_includes_null_min_max.)"""
    node.query("DROP TABLE IF EXISTS prom_exact SYNC")
    node.query("CREATE TABLE prom_exact ENGINE=TimeSeries SETTINGS skip_reading_samples_for_timestamp_filter=0")
    try:
        node.query(
            "INSERT INTO FUNCTION timeSeriesTags(prom_exact) (metric_name, tags) VALUES ('cpu', {'__name__': 'cpu'})"
        )
        node.query(
            "INSERT INTO FUNCTION timeSeriesData(prom_exact) (id, timestamp, value)"
            " SELECT id, toDateTime64(2000, 3), 1.0 FROM timeSeriesTags(prom_exact)"
        )
        # The sample at t=2000 satisfies these, so the series must be returned despite NULL min/max time.
        assert node.query("SELECT metric_name FROM prom_exact WHERE timestamp >= toDateTime64(1500, 3)") == TSV([["cpu"]])
        assert node.query("SELECT metric_name FROM prom_exact WHERE timestamp <= toDateTime64(2500, 3)") == TSV([["cpu"]])
        # ...and correctly excluded when the window genuinely does not contain the sample (exact samples filter).
        assert node.query("SELECT metric_name FROM prom_exact WHERE timestamp >= toDateTime64(3000, 3)") == ""
    finally:
        node.query("DROP TABLE IF EXISTS prom_exact SYNC")
...
def test_select_timestamp_filter_min_max_only():
    """With the default `skip_reading_samples_for_timestamp_filter=1`, a filter-only `timestamp` condition (not
    selected, `time_series` not requested) is served from the tags-table `min_time`/`max_time` range alone.
    `AND`/`OR`/`NOT` of `timestamp <cmp> const` comparisons (including `!=`) are modeled; a non-comparison
    function of `timestamp` (e.g. `toUnixTimestamp64Milli`) falls back to the exact samples path."""
    _insert_two_series_with_timestamps()  # cpu: 1000/2000/3000, mem: 5000/6000

    # Inclusive bounds.
    assert node.query(
        "SELECT metric_name FROM prometheus WHERE timestamp >= toDateTime64(5000,3) ORDER BY metric_name"
    ) == TSV([["mem"]])
    assert node.query(
        "SELECT metric_name FROM prometheus WHERE timestamp <= toDateTime64(1500,3) ORDER BY metric_name"
    ) == TSV([["cpu"]])
    # Strict bounds (handled via the smallest/largest representable step past the bound).
    assert node.query(
        "SELECT metric_name FROM prometheus WHERE timestamp > toDateTime64(3000,3) ORDER BY metric_name"
    ) == TSV([["mem"]])
    assert node.query(
    ...
    # Strict range and equality.
    assert node.query(
        "SELECT metric_name FROM prometheus"
        " WHERE timestamp > toDateTime64(2000,3) AND timestamp < toDateTime64(6000,3) ORDER BY metric_name"
    ) == TSV([["cpu"], ["mem"]])
    assert node.query(
        "SELECT metric_name FROM prometheus WHERE timestamp = toDateTime64(3000,3) ORDER BY metric_name"
    ) == TSV([["cpu"]])
    ...
    # OR of intervals: the feasible set is the union; a series is kept if it overlaps either window.
    assert node.query(
        "SELECT metric_name FROM prometheus"
        " WHERE (timestamp BETWEEN toDateTime64(900,3) AND toDateTime64(1100,3))"
        "    OR (timestamp BETWEEN toDateTime64(5900,3) AND toDateTime64(6100,3)) ORDER BY metric_name"
    ) == TSV([["cpu"], ["mem"]])
    assert node.query(
        "SELECT metric_name FROM prometheus"
        " WHERE (timestamp BETWEEN toDateTime64(900,3) AND toDateTime64(1100,3))"
        "    OR (timestamp BETWEEN toDateTime64(100,3) AND toDateTime64(200,3)) ORDER BY metric_name"
    ) == TSV([["cpu"]])

    # `!=` is modeled (union of two open half-lines); it imposes no min/max constraint, so every series is kept
    # (a coarse over-approximation) while still skipping samples.
    assert node.query(
        "SELECT count() FROM prometheus WHERE timestamp != toDateTime64(9999,3)"
    ) == "2\n"

    # A non-comparison function of `timestamp` is not modeled -> fall back to the exact samples path.
    assert node.query(
        "SELECT metric_name FROM prometheus WHERE toUnixTimestamp64Milli(timestamp) >= 5000000 ORDER BY metric_name"
    ) == TSV([["mem"]])


def test_select_timestamp_filter_min_max_only_over_includes_null_min_max():
    """The coarse min/max filter is an over-approximation: a series with unset (NULL) min_time/max_time always
    passes, so under the default setting it is returned even for a window it has no sample in (unlike the exact
    samples path -- see test_select_timestamp_filter_keeps_series_with_null_min_max_time)."""
    node.query(
    ...
    # The only sample is at 2000, but min_time/max_time are NULL, so the coarse filter keeps the series for a
    # window (>= 3000) that the exact samples filter would exclude.
    assert node.query("SELECT metric_name FROM prometheus WHERE timestamp >= toDateTime64(3000, 3)") == TSV([["cpu"]])


def test_select_timestamp_filter_skips_samples_table():
    """The min/max-only path must not read the heavy `samples` inner table; a query that needs the samples data
    (`time_series`) still does. Verified via system.query_log's `tables` column (the inner samples table name
    contains 'samples', unlike the tags/metrics inner tables)."""
    _insert_two_series_with_timestamps()

    def assert_reads_samples(sql, expected):
        query_id = f"ts-minmax-{uuid.uuid4()}"
        node.query(sql, query_id=query_id)
        node.query("SYSTEM FLUSH LOGS query_log")
        assert_eq_with_retry(
            node,
            "SELECT arrayExists(t -> position(t, 'samples') > 0, tables) FROM system.query_log"
            f" WHERE type = 'QueryFinish' AND query_id = '{query_id}'",
            expected,
        )

    # Engaged (filter-only timestamp, no time_series) -> reads tags only, not samples.
    assert_reads_samples(
        "SELECT count() FROM prometheus WHERE timestamp >= toDateTime64(1000,3)", "0\n"
    )
    # Requesting the samples array must read the samples table.
    assert_reads_samples(
        "SELECT count() FROM (SELECT time_series FROM prometheus WHERE timestamp >= toDateTime64(1000,3))", "1\n"
    )
    # With the setting off, even a filter-only timestamp query reads the samples table (exact path).
    node.query("DROP TABLE IF EXISTS prom_exact SYNC")
    node.query("CREATE TABLE prom_exact ENGINE=TimeSeries SETTINGS skip_reading_samples_for_timestamp_filter=0")
    try:
        node.query(
            "INSERT INTO prom_exact (metric_name, tags, time_series) VALUES"
            " ('cpu', {'host': 'a'}, [(toDateTime64(1000,3),1.0)])"
        )
        assert_reads_samples(
            "SELECT count() FROM prom_exact WHERE timestamp >= toDateTime64(1000,3)", "1\n"
        )
    finally:
        node.query("DROP TABLE IF EXISTS prom_exact SYNC")


def test_select_timestamp_filter_folds_query_scope_constant():
    """A `timestamp` bound need not be a literal: a query-scope-constant expression -- e.g.
    `now() - INTERVAL 5 MINUTE`, the usual "last N minutes" form -- is constant-folded by the analyzer before
    the read query is built, so it pushes down exactly like an explicit constant. It prunes series by the
    tags-table min_time/max_time range, and the filter-only min/max path still skips the samples table."""
    # An "old" series (sampled an hour ago) and a "recent" one (a minute ago). The hour/minute gaps versus the
    # 5-minute bound dwarf the sub-second now() drift between INSERT and SELECT, so the outcome is stable.
    node.query(
        "INSERT INTO prometheus (metric_name, tags, time_series) VALUES"
        " ('old',    {}, [(now64(3) - INTERVAL 1 HOUR, 1.0)]),"
        " ('recent', {}, [(now64(3) - INTERVAL 1 MINUTE, 2.0)])"
    )

    # Filter-only `timestamp` (only `metric_name` selected) -> served from the min_time/max_time range: an
    # upper bound in the near past keeps only 'old', a lower bound keeps only 'recent', a range keeps 'old'.
    assert node.query(
        "SELECT metric_name FROM prometheus WHERE timestamp < now() - INTERVAL 5 MINUTE"
    ) == TSV([["old"]])
    assert node.query(
        "SELECT metric_name FROM prometheus WHERE timestamp >= now() - INTERVAL 5 MINUTE"
    ) == TSV([["recent"]])
    assert node.query(
        "SELECT metric_name FROM prometheus"
        " WHERE timestamp BETWEEN now() - INTERVAL 2 HOUR AND now() - INTERVAL 30 MINUTE"
    ) == TSV([["old"]])

    # The folded bound lets the min/max-only path skip the samples inner table, just as a literal would. Were
    # `now() - INTERVAL` left unfolded, `tryBuildSatisfyingTimestamp` would bail and read the samples table.
    query_id = f"ts-now-fold-{uuid.uuid4()}"
    node.query(
        "SELECT metric_name FROM prometheus WHERE timestamp < now() - INTERVAL 5 MINUTE",
        query_id=query_id,
    )
