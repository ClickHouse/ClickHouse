...
def setup(request):
    try:
        ...
        node.query(
            ...
            " __series_labels LowCardinality(Nullable(String)),"
        ...
        node.query(
            ...
            "SETTINGS tags_to_columns = {'host': '__series_labels'} "
            "DATA prometheus_data TAGS prometheus_tags METRICS prometheus_metrics"
        )
        # This deliberately collides with the private alias used by the original /series query.
        # The endpoint must choose a different generated alias for the normalized label array.
        ...
        node.query(
            ...
            "  toDateTime64(1700000000, 3, 'UTC'), toDateTime64(1700000000, 3, 'UTC')),"
            " ('00000000-0000-0000-0000-00000000000e', 'invalid_utf8_label_name_metric', NULL, map(unhex('FF'), 'invalid'),"
            "  toDateTime64(1700000000, 3, 'UTC'), toDateTime64(1700000000, 3, 'UTC')),"
            ...
            "  toDateTime64(1700000000, 3, 'UTC'), toDateTime64(1700000000, 3, 'UTC')),"
            " ('00000000-0000-0000-0000-000000000010', unhex('FF'), NULL, map('host', 'invalid_utf8_name'),"
            "  toDateTime64(1700000000, 3, 'UTC'), toDateTime64(1700000000, 3, 'UTC')),"
            ...
            "  toDateTime64(1700000000, 3, 'UTC'), toDateTime64(1700000000, 3, 'UTC')),"
            " ('00000000-0000-0000-0000-00000000000f', 'duplicate_map_key_metric', NULL, map('host', 'map_host_a', 'host', 'map_host_b'),"
            "  toDateTime64(1700000000, 3, 'UTC'), toDateTime64(1700000000, 3, 'UTC'))"
            " , ('00000000-0000-0000-0000-000000000006', 'conflicting_metric_name', 'canonical_name', map('__name__', 'map_name'),"
            "  toDateTime64(1700000000, 3, 'UTC'), toDateTime64(1700000000, 3, 'UTC'))"
            " , ('00000000-0000-0000-0000-000000000011', 'duplicate_metric_name_key_metric', NULL,"
            "  map('__name__', 'duplicate_metric_name_key_metric', '__name__', 'different_metric_name'),"
            "  toDateTime64(1700000000, 3, 'UTC'), toDateTime64(1700000000, 3, 'UTC'))"
        )
        node.query(
...
@pytest.mark.parametrize("selector", ["duplicate_map_key_metric", '{host="map_host_a"}', '{host="map_host_b"}'])
def test_duplicate_map_keys_are_rejected_by_series(selector):
    response = request_api("/api/v1/series", params={"match[]": selector})
    assert response.status_code == 400, f"Expected 400, got {response.status_code}: {response.text}"
    data = response.json()
    assert data["status"] == "error", f"Unexpected body: {data}"
    assert data["errorType"] == "bad_data", f"Unexpected body: {data}"
    assert "same name" in data["error"], f"Unexpected error message: {data}"
...
def test_late_conflicting_row_with_limit_aborts_after_response_starts():
    """A malformed row beyond the response limit is validated after the prefix is sent."""
    response = request_api(
        "/api/v1/series",
        params={
    ...
    try:
        ...
        assert b"late_conflicting_metric" in received, received
        assert b"__exception__" in received, received
        assert b"host" in received, received
    finally:
        response.close()


def test_late_emittable_conflict_aborts_after_response_starts():
    """A malformed row after the first block aborts the already-started response."""
    response = request_api(
        "/api/v1/series",
        params={
            "match[]": "late_conflicting_metric",
...
def test_labels_reject_conflicting_tag_carriers():
    response = request_api("/api/v1/labels", params={"match[]": "conflicting_metric"})
    assert response.status_code == 400, f"Expected 400, got {response.status_code}: {response.text}"
    data = response.json()
    assert data["status"] == "error", f"Unexpected body: {data}"
    assert data["errorType"] == "bad_data", f"Unexpected body: {data}"
...
def test_labels_filter_before_validating_unrelated_rows():
    response = request_api(
        "/api/v1/labels",
        params={
            "match[]": "legacy_metric",
    ...
    assert response.json() == {
        "status": "success",
        "data": ["__name__", "host", "instance"],
    }, f"Unexpected body: {response.text}"


@pytest.mark.parametrize("selector", ["duplicate_map_key_metric", '{host="map_host_a"}', '{host="map_host_b"}'])
def test_labels_reject_duplicate_map_keys(selector):
    response = request_api("/api/v1/labels", params={"match[]": selector})
    assert response.status_code == 400, f"Expected 400, got {response.status_code}: {response.text}"
    data = response.json()
    assert data["status"] == "error", f"Unexpected body: {data}"
    assert data["errorType"] == "bad_data", f"Unexpected body: {data}"
...
def test_query_rejects_duplicate_map_keys_before_selector_evaluation():
    response = request_api(
        "/api/v1/query",
        params={"query": 'duplicate_map_key_metric{host="map_host_b"}', "time": 1700000000},
    )
    assert response.status_code == 400, f"Expected 400, got {response.status_code}: {response.text}"
    data = response.json()
    assert data["status"] == "error", f"Unexpected body: {data}"
...
def test_labels_reject_duplicate_metric_name_map_keys_when_first_value_matches_canonical_name():
    response = request_api("/api/v1/labels", params={"match[]": "duplicate_metric_name_key_metric"})
    assert response.status_code == 400, f"Expected 400, got {response.status_code}: {response.text}"
    data = response.json()
    assert data["status"] == "error", f"Unexpected body: {data}"
    assert data["errorType"] == "bad_data", f"Unexpected body: {data}"
...
def test_labels_reject_conflicting_metric_name_carriers():
    response = request_api("/api/v1/labels", params={"match[]": "conflicting_metric_name"})
    assert response.status_code == 400, f"Expected 400, got {response.status_code}: {response.text}"
    data = response.json()
    assert data["status"] == "error", f"Unexpected body: {data}"
    assert data["errorType"] == "bad_data", f"Unexpected body: {data}"
...
def test_labels_reject_invalid_utf8_label_values():
    response = request_api("/api/v1/labels", params={"match[]": "invalid_utf8_metric"})
    assert response.status_code == 400, f"Expected 400, got {response.status_code}: {response.text}"
    data = response.json()
    assert data["status"] == "error", f"Unexpected body: {data}"
    assert data["errorType"] == "bad_data", f"Unexpected body: {data}"
...
@pytest.mark.parametrize("selector", ['{host="empty_canonical_host"}', '{host="map_only_empty_name"}'])
def test_labels_reject_empty_canonical_metric_names(selector):
    response = request_api("/api/v1/labels", params={"match[]": selector})
    assert response.status_code == 400, f"Expected 400, got {response.status_code}: {response.text}"
    data = response.json()
    assert data["status"] == "error", f"Unexpected body: {data}"
    assert data["errorType"] == "bad_data", f"Unexpected body: {data}"
    assert "empty metric name" in data["error"], f"Unexpected body: {data}"


def test_labels_reject_invalid_utf8_metric_names():
    response = request_api(
        "/api/v1/labels",
        params={"match[]": '{host="invalid_utf8_name"}'},
    )
    assert response.status_code == 400, f"Expected 400, got {response.status_code}: {response.text}"
    data = response.json()
    assert data["status"] == "error", f"Unexpected body: {data}"
...
@pytest.mark.parametrize("metric_name", ["empty_label_name_metric", "empty_label_name_empty_value_metric"])
def test_labels_reject_empty_label_names(metric_name):
    response = request_api("/api/v1/labels", params={"match[]": metric_name})
    assert response.status_code == 400, f"Expected 400, got {response.status_code}: {response.text}"
    data = response.json()
    assert data["status"] == "error", f"Unexpected body: {data}"
    assert data["errorType"] == "bad_data", f"Unexpected body: {data}"
...
def test_labels_reject_invalid_utf8_label_names():
    response = request_api("/api/v1/labels", params={"match[]": "invalid_utf8_label_name_metric"})
    assert response.status_code == 400, f"Expected 400, got {response.status_code}: {response.text}"
    data = response.json()
    assert data["status"] == "error", f"Unexpected body: {data}"
    assert data["errorType"] == "bad_data", f"Unexpected body: {data}"
    assert "UTF-8" in data["error"], f"Unexpected error message: {data}"


def test_labels_reject_late_invalid_utf8_before_success_body():
    response = request_api(
        "/api/v1/labels",
        params={
            "match[]": "invalid_utf8_label_name_metric",
            "max_threads": 1,
            "max_block_size": 1,
            "http_response_buffer_size": 1,
        },
