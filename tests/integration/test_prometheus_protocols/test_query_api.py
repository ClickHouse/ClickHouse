...
def test_query_timeout_is_applied_to_instant_and_range_queries():
    for path, time_params in (
        ...
        url = (
            ...
            f"?query=post_body_metric&{time_params}&timeout=500ms&http_response_buffer_size=1"
        )
...
def test_query_timeout_clamps_the_effective_query_limit_after_profile_defaults():
    query_id = f"promql-timeout-{uuid.uuid4()}"
    url = (
        f"http://{node.ip_address}:9093/api/v1/query"
    ...
    assert_query_timeout_setting(query_id, "10.0")
...
def test_query_timeout_does_not_mask_range_validation_errors():
    ...
    assert "step must be positive" in response_json["error"]
