...
def test_series_time_range_applies_rfc3339_timezone_offsets():
    data = get_json_from_api(
        ...
        params={
            "match[]": "cpu_usage",
            "start": "1970-01-01T01:16:40.000000000+01:00",
            "end": "1970-01-01T01:16:40.000000000+01:00",
        },
    )["data"]
    assert len(data) == 2
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "1970-01-01T00:16:40+24:00",
        "1970-01-01T00:16:40+23:60",
    ],
)
...
def test_series_rejects_promql_timestamp_syntax_in_http_bounds(bound_name, bound_value):
    get_bad_data_from_api(
        "/api/v1/series",
        params={"match[]": "cpu_usage", bound_name: bound_value},
    )


def test_series_time_range_accepts_decimal_fraction_bounds():
    data = get_json_from_api(
        "/api/v1/series",
        params={"match[]": "cpu_usage", "start": "1000.125", "end": "1000.125"},
    )["data"]
    assert len(data) == 2
