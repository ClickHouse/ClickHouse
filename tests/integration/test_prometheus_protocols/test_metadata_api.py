...
def setup():
    try:
        ...
        # A table of version 3: its Metrics target names the column `metric_family_name` (see TimeSeriesVersion.h).
        node.query("CREATE TABLE prometheus_v3 ENGINE=TimeSeries SETTINGS version = 3")
        node.query(
            "INSERT INTO prometheus_v3 (metric_family, type, unit, help) VALUES "
...
def test_table_of_version_3():
    data = get_json_from_api("/v3/api/v1/metadata")["data"]
    assert data == {
        "cpu_usage": ALL_METADATA["cpu_usage"],
        "http_requests_total": ALL_METADATA["http_requests_total"],
    }
    data = get_json_from_api("/v3/api/v1/metadata?metric=cpu_usage")["data"]
    assert data == {"cpu_usage": ALL_METADATA["cpu_usage"]}
