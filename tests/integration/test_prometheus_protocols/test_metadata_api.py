...
def setup():
    try:
        ...
        # A table of version 4: its metric families target names the column `metric_family_name` (see TimeSeriesVersion.h).
        node.query("CREATE TABLE prometheus_v4 ENGINE=TimeSeries SETTINGS version = 4")
        node.query(
            "INSERT INTO prometheus_v4 (metric_family, type, unit, help) VALUES "
...
def test_table_of_version_4():
    data = get_json_from_api("/v4/api/v1/metadata")["data"]
    assert data == {
        "cpu_usage": ALL_METADATA["cpu_usage"],
        "http_requests_total": ALL_METADATA["http_requests_total"],
    }
    data = get_json_from_api("/v4/api/v1/metadata?metric=cpu_usage")["data"]
    assert data == {"cpu_usage": ALL_METADATA["cpu_usage"]}
