...
@pytest.mark.parametrize(
    ...
    [
        ...
        "vector(step())",
        "vector(range())",
        "last_over_time(vector(step())[20s:5s] offset 7s)",
        "last_over_time(vector(range())[20s:5s] @ 1005)",
        "last_over_time(last_over_time(vector(step())[10s:2s])[20s:5s])",
    ],
)
@pytest.mark.parametrize("timestamp", [1000.125, 1750000000.125])
def test_standalone_step_range_instant(query, timestamp):
    expected_query = query.replace("step()", "0").replace("range()", "0")
...
def test_standalone_step_range_range(query, start, end, step, span):
    ...
    expected_query = query.replace("step()", str(step)).replace("range()", str(span))
...
def test_standalone_step_range_current_time():
    assert node.query(
        "SELECT * FROM prometheusQuery(prometheus, 'step() + range()')"
    ) == "0\n"
