...
# Verify `count_over_time` preserves the exact `Float64` count 16777217 on a `Float32`-backed `TimeSeries` table.
def test_count_over_time_float32_precision():
    node.query(
        "CREATE TABLE prometheus_f32 (time_series Array(Tuple(DateTime64(3), Float32))) ENGINE=TimeSeries"
    )

    try:
        num_samples = 16777217  # One more than Float32 can represent exactly (2**24).
        node.query(
            "INSERT INTO prometheus_f32 (metric_name, tags, time_series) "
            "SELECT 'big_counter', map(), [(toDateTime64(number, 3), toFloat32(1))] "
            f"FROM numbers({num_samples})"
        )

        query = f"count_over_time(big_counter[{num_samples}s])"
        eval_time = num_samples - 1  # Covers samples 0..num_samples-1 in one window.

        # The declared/advertised column type must be Float64, not the table's Float32 sample type.
        assert (
            node.query(
                f"SELECT toTypeName(value) FROM prometheusQuery(prometheus_f32, '{query}', {eval_time})"
            )
            == "Float64\n"
        )

        # The exact count must reach the client via the SQL table-function path...
        assert (
            node.query(
                f"SELECT value FROM prometheusQuery(prometheus_f32, '{query}', {eval_time})"
            )
            == f"{num_samples}\n"
        )

        # ...and via the standalone PromQL HTTP API path (table name supplied dynamically, since
        # `prometheus_f32` isn't the table the "/api/v1/query" handler in configs/prometheus.xml is bound to).
        assert execute_query_via_http_api(
            node.ip_address,
            9093,
            "/dynamic_table/api/v1/query",
            query,
            timestamp=eval_time,
            params={"table": "default.prometheus_f32"},
        ) == (
            '{"resultType": "vector", "result": [{"metric": {}, "value": '
            f'[{eval_time}, "{num_samples}"]}}]}}'
        )

        # sum_over_time()/avg_over_time() on the same Float32 table must remain unaffected, i.e. still typed
        # the table's own Float32 sample type (only count_over_time()'s result type was changed).
        assert (
            node.query(
                f"SELECT toTypeName(value) FROM prometheusQuery(prometheus_f32, 'sum_over_time(big_counter[{num_samples}s])', {eval_time})"
            )
            == "Float32\n"
        )

        # Verify `scalar` and `abs` wrappers preserve the `Float64` result of `count_over_time`.
        for wrapped_query in (
            f"scalar(count_over_time(big_counter[{num_samples}s]))",
            f"abs(count_over_time(big_counter[{num_samples}s]))",
        ):
            assert (
                node.query(
                    f"SELECT toTypeName(value) FROM prometheusQuery(prometheus_f32, '{wrapped_query}', {eval_time})"
                )
                == "Float64\n"
            )
            assert (
                node.query(
                    f"SELECT value FROM prometheusQuery(prometheus_f32, '{wrapped_query}', {eval_time})"
                )
                == f"{num_samples}\n"
            )

        # Verify `count_over_time` keeps `Float64` for `SCALAR_GRID` range inputs, not only grouped table reads.
        scalar_sourced_query = f"count_over_time(vector(1)[{num_samples}s:1s])"
        assert (
            node.query(
                f"SELECT toTypeName(value) FROM prometheusQuery(prometheus_f32, '{scalar_sourced_query}', {eval_time})"
            )
            == "Float64\n"
        )
        assert (
            node.query(
                f"SELECT value FROM prometheusQuery(prometheus_f32, '{scalar_sourced_query}', {eval_time})"
            )
            == f"{num_samples}\n"
        )

        # Verify `count_over_time` keeps `Float64` even when its argument's range is compile-time empty (e.g.
        # [19:20] covers no grid point). Uses `DESCRIBE` since the query itself returns zero rows.
        empty_range_query = "count_over_time(vector(1)[19:20])"
        assert (
            node.query(f"SELECT * FROM prometheusQuery(prometheus_f32, '{empty_range_query}', 19)")
            == ""
        )
        assert re.search(
            r"\bvalue\s+Float64",
            node.query(
                f"DESCRIBE (SELECT * FROM prometheusQuery(prometheus_f32, '{empty_range_query}', 19))"
            ),
        )

        # An outer range function with no fixed type of its own must not drop the inner one's override on that same
        # compile-time-empty path, else the count narrows back to the table's `Float32`.
        nested_empty_query = "sum_over_time(count_over_time(vector(1)[19:20])[19:20])"
        assert (
            node.query(f"SELECT * FROM prometheusQuery(prometheus_f32, '{nested_empty_query}', 19)")
            == ""
        )
        assert re.search(
            r"\bvalue\s+Float64",
            node.query(
                f"DESCRIBE (SELECT * FROM prometheusQuery(prometheus_f32, '{nested_empty_query}', 19))"
            ),
        )

        # Same check for the range-query counterpart, via `finalizeRangeVectorAsSQL`'s `time_series` column
        # instead of `finalizeInstantVectorAsSQL`'s `value` column.
        assert (
            node.query(
                f"SELECT * FROM prometheusQueryRange(prometheus_f32, '{empty_range_query}', 19, 19, 1)"
            )
            == ""
        )
        assert re.search(
            r"\btime_series\b[^\n]*Float64",
            node.query(
                f"DESCRIBE (SELECT * FROM prometheusQueryRange(prometheus_f32, '{empty_range_query}', 19, 19, 1))"
            ),
        )

        # `abs()` (a simple function; see applySimpleFunction.cpp) must keep `Float64` on the same
        # compile-time-empty range instead of building a fresh `StoreMethod::EMPTY` piece without the override.
        empty_range_abs_query = f"abs({empty_range_query})"
        assert (
            node.query(f"SELECT * FROM prometheusQuery(prometheus_f32, '{empty_range_abs_query}', 19)")
            == ""
        )
        assert re.search(
            r"\bvalue\s+Float64",
            node.query(
                f"DESCRIBE (SELECT * FROM prometheusQuery(prometheus_f32, '{empty_range_abs_query}', 19))"
            ),
        )

        # Verify a vector/vector binary operator (see applySimpleBinaryOperator.cpp's applyOperatorToVectors)
        # also keeps `Float64` on the same compile-time-empty range.
        empty_range_plus_query = f"{empty_range_query} + vector(0)"
        assert (
            node.query(f"SELECT * FROM prometheusQuery(prometheus_f32, '{empty_range_plus_query}', 19)")
            == ""
        )
        assert re.search(
            r"\bvalue\s+Float64",
            node.query(
                f"DESCRIBE (SELECT * FROM prometheusQuery(prometheus_f32, '{empty_range_plus_query}', 19))"
            ),
        )

        # Verify scalar-wrapped `count_over_time` preserves `Float64` through scalar binary operations.
        single_scalar_query = f"scalar(count_over_time(big_counter[{num_samples}s])) + 0"
        assert (
            node.query(
                f"SELECT toTypeName(value) FROM prometheusQuery(prometheus_f32, '{single_scalar_query}', {eval_time})"
            )
            == "Float64\n"
        )
        assert (
            node.query(
                f"SELECT value FROM prometheusQuery(prometheus_f32, '{single_scalar_query}', {eval_time})"
            )
            == f"{num_samples}\n"
        )

        # `scalar()` of a compile-time-empty vector is a NaN constant (CONST_SCALAR); it must keep
        # `Float64`, and so must `vector()` of it (see finalizeSQL.cpp's CONST_SCALAR branches).
        empty_scalar_query = f"scalar({empty_range_query})"
        assert (
            node.query(
                f"SELECT toTypeName(value) FROM prometheusQuery(prometheus_f32, '{empty_scalar_query}', 19)"
            )
            == "Float64\n"
        )
        assert (
            node.query(
                f"SELECT toTypeName(value) FROM prometheusQuery(prometheus_f32, 'vector({empty_scalar_query})', 19)"
            )
            == "Float64\n"
        )
        assert re.search(
            r"\btime_series\b[^\n]*Float64",
            node.query(
                f"DESCRIBE (SELECT * FROM prometheusQueryRange(prometheus_f32, '{empty_scalar_query}', 19, 20, 1))"
            ),
        )

        # The empty fast path must merge overrides from every operand, not stop at the first empty
        # one: here the left operand is empty and only the right one carries `Float64`.
        order_query = f"last_over_time(vector(1)[19:20]) + scalar(count_over_time(big_counter[{num_samples}s]))"
        assert (
            node.query(f"SELECT * FROM prometheusQuery(prometheus_f32, '{order_query}', 19)") == ""
        )
        assert re.search(
            r"\bvalue\s+Float64",
            node.query(
                f"DESCRIBE (SELECT * FROM prometheusQuery(prometheus_f32, '{order_query}', 19))"
            ),
        )

        # `clamp()` with constant `max < min` empties the result at translation time; the operand's
        # override must survive that fast path too (see applyClampFunction.cpp).
        empty_clamp_query = f"clamp({empty_range_query}, 1, 0)"
        assert (
            node.query(f"SELECT * FROM prometheusQuery(prometheus_f32, '{empty_clamp_query}', 19)") == ""
        )
        assert re.search(
            r"\bvalue\s+Float64",
            node.query(
                f"DESCRIBE (SELECT * FROM prometheusQuery(prometheus_f32, '{empty_clamp_query}', 19))"
            ),
        )

        # `or` with both sides compile-time empty must merge overrides from both operands,
        # whichever side carries the fixed type.
        for or_query in (
            f"{empty_range_query} or last_over_time(vector(1)[19:20])",
            f"last_over_time(vector(1)[19:20]) or {empty_range_query}",
        ):
            assert (
                node.query(f"SELECT * FROM prometheusQuery(prometheus_f32, '{or_query}', 19)") == ""
            )
            assert re.search(
                r"\bvalue\s+Float64",
                node.query(
                    f"DESCRIBE (SELECT * FROM prometheusQuery(prometheus_f32, '{or_query}', 19))"
                ),
            )

        # Verify binary operators preserve `Float64` when combining two `count_over_time` results.
        binary_op_query = (
            f"count_over_time(big_counter[{num_samples}s]) + count_over_time(big_counter[{num_samples}s])"
        )
        assert (
            node.query(
                f"SELECT toTypeName(value) FROM prometheusQuery(prometheus_f32, '{binary_op_query}', {eval_time})"
            )
            == "Float64\n"
        )
        assert (
            node.query(
                f"SELECT value FROM prometheusQuery(prometheus_f32, '{binary_op_query}', {eval_time})"
            )
            == f"{2 * num_samples}\n"
        )

        # A comparison must keep the retained operand's type, not the discarded operand's override.
        node.query(
            "INSERT INTO prometheus_f32 (metric_name, tags, time_series) "
            f"SELECT 'small_metric', map(), [(toDateTime64({eval_time}, 3), toFloat32(5))]"
        )
        comparison_query = f"small_metric < count_over_time(big_counter[{num_samples}s])"
        assert (
            node.query(
                f"SELECT toTypeName(value) FROM prometheusQuery(prometheus_f32, '{comparison_query}', {eval_time})"
            )
            == "Float32\n"
        )
        assert (
            node.query(
                f"SELECT value FROM prometheusQuery(prometheus_f32, '{comparison_query}', {eval_time})"
            )
            == "5\n"
        )

        # `> bool` builds a fresh 0/1 outcome instead of carrying its operand's values, so it must report the table's
        # own `Float32` even though its operand carries `count_over_time`'s `Float64` (asserted first, below).
        counted_small = "count_over_time(small_metric[10s])"
        assert (
            node.query(
                f"SELECT toTypeName(value) FROM prometheusQuery(prometheus_f32, '{counted_small}', {eval_time})"
            )
            == "Float64\n"
        )
        assert (
            node.query(
                f"SELECT toTypeName(value), value FROM prometheusQuery(prometheus_f32, '{counted_small} > bool 0', {eval_time})"
            )
            == "Float32\t1\n"
        )

        # `count()` synthesizes values too, but a series count -- like `count_over_time`'s sample count -- is exact
        # only in `Float64`, which `Float32` stops being above 16777216. So it keeps its own fixed result type.
        assert (
            node.query(
                f"SELECT toTypeName(value), value FROM prometheusQuery(prometheus_f32, 'count({counted_small})', {eval_time})"
            )
            == "Float64\t1\n"
        )

        # The same must hold when the operand's range is compile-time empty, i.e. when the type is reported
        # through the `StoreMethod::EMPTY` fast paths.
        for empty_synthesizing_query, expected_type in (
            (f"{empty_range_query} > bool 0", "Float32"),
            (f"group({empty_range_query})", "Float32"),
            (f"count({empty_range_query})", "Float64"),
        ):
            assert (
                node.query(
                    f"SELECT * FROM prometheusQuery(prometheus_f32, '{empty_synthesizing_query}', 19)"
                )
                == ""
            )
            assert re.search(
                rf"\bvalue\s+{expected_type}",
                node.query(
                    f"DESCRIBE (SELECT * FROM prometheusQuery(prometheus_f32, '{empty_synthesizing_query}', 19))"
                ),
            ), empty_synthesizing_query
    finally:
