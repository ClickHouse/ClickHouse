...
cluster = ClickHouseCluster(__file__, zookeeper_config_path="configs/fdbkeeper.xml")
...
def replace_fdbkeeper_config(new_config):
...
def revert_fdbkeeper_config():
    with open(os.path.join(SCRIPT_DIR, "configs/fdbkeeper.xml"), "r") as f:
        replace_fdbkeeper_config(f.read())
...
def test_replication():
    node1.query("DROP FUNCTION IF EXISTS f2")
