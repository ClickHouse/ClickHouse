...
def clickhouse_killed_while_insert(clickhouse_node, mysql_node, service_name):
    ...
    mysql_node.query("TRUNCATE TABLE kill_clickhouse_while_insert.test")
    t = threading.Thread(target=insert, args=(2000,))
    t.start()

    clickhouse_node.restart_clickhouse(20, kill=False)
    t.join()
    result = mysql_node.query_and_get_data("SELECT COUNT(1) FROM kill_clickhouse_while_insert.test")
    for row in result:
        res = str(row[0]) + '\n'
