...
def drop_table_with_materialize_mysql_database(clickhouse_node, mysql_node):
    ...
    # create mapping
    clickhouse_node.query("CREATE DATABASE test_database ENGINE = MaterializeMySQL('{}:{}', 'test_database', '{}', '{}')".format(
        mysql_node.hostname, mysql_node.port, mysql_node.user, mysql_node.password))
...
def create_table_with_materialize_mysql_database(clickhouse_node, mysql_node):
    mysql_node.query("CREATE DATABASE test_database DEFAULT CHARACTER SET 'utf8'")
...
def rename_table_with_materialize_mysql_database(clickhouse_node, mysql_node):
    mysql_node.query("CREATE DATABASE test_database DEFAULT CHARACTER SET 'utf8'")
    mysql_node.query("CREATE TABLE test_database.test_table_1 (id INT NOT NULL PRIMARY KEY) ENGINE = InnoDB;")
...
def alter_add_column_with_materialize_mysql_database(clickhouse_node, mysql_node):
    mysql_node.query("CREATE DATABASE test_database DEFAULT CHARACTER SET 'utf8'")
    mysql_node.query("CREATE TABLE test_database.test_table_1 (id INT NOT NULL PRIMARY KEY) ENGINE = InnoDB;")
    ...
    assert "test_database" in clickhouse_node.query("SHOW DATABASES")
    check_query("SHOW TABLES FROM test_database FORMAT TSV", "test_table_1")
    check_query("DESC test_database.test_table_1 FORMAT TSV",
                "add_column_2\tInt32\t\t\t\t\nid\tInt32\t\t\t\t\nadd_column_1\tInt32\t\t\t\t\nadd_column_3\tInt32\t\t\t\t\nadd_column_4\tInt32\t\t\t\t")
    mysql_node.query("CREATE TABLE test_database.test_table_2 (id INT NOT NULL PRIMARY KEY) ENGINE = InnoDB;")
    check_query("SHOW TABLES FROM test_database FORMAT TSV", "test_table_1\ntest_table_2")
    ...
    mysql_node.query(
        "ALTER TABLE test_database.test_table_2 ADD COLUMN add_column_3 INT NOT NULL AFTER add_column_1, ADD COLUMN add_column_4 INT NOT NULL DEFAULT (id)")
    check_query("DESC test_database.test_table_2 FORMAT TSV",
                "add_column_2\tInt32\t\t\t\t\nid\tInt32\t\t\t\t\nadd_column_1\tInt32\t\t\t\t\nadd_column_3\tInt32\t\t\t\t\nadd_column_4\tInt32\t\t\t\t")
...
def alter_drop_column_with_materialize_mysql_database(clickhouse_node, mysql_node):
    mysql_node.query("CREATE DATABASE test_database DEFAULT CHARACTER SET 'utf8'")
...
def alter_rename_column_with_materialize_mysql_database(clickhouse_node, mysql_node):
    mysql_node.query("CREATE DATABASE test_database DEFAULT CHARACTER SET 'utf8'")
...
def alter_modify_column_with_materialize_mysql_database(clickhouse_node, mysql_node):
    mysql_node.query("CREATE DATABASE test_database DEFAULT CHARACTER SET 'utf8'")
