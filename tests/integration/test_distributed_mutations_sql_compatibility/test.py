...
def test_distributed_mutations(started_cluster):
    node1.query('''update replica_1.replicated_local on cluster cross_3shards_2replicas 
                    set shard_id=5 where part_key='2017-06-16';
                 ''')
    node1.query("SYSTEM SYNC REPLICA replica_2.replicated_local;", timeout=5)
    assert_eq_with_retry(node1, "SELECT count(*) FROM replica_2.replicated where shard_id= 5 and part_key='2017-06-16'", '3')

    node1.query('''update replica_1.replicated_local on cluster cross_3shards_2replicas set shard_id=shard_id+30;
                 ''')
    node1.query("SYSTEM SYNC REPLICA replica_2.replicated_local;", timeout=5)
    assert_eq_with_retry(node1, "SELECT count(*) FROM replica_2.replicated where shard_id >=30 ", '6')


    node1.query("delete from replica_1.replicated_local on cluster cross_3shards_2replicas where shard_id >=30 and shard_id<35;")
    node1.query("SYSTEM SYNC REPLICA replica_2.replicated_local;", timeout=5)
    assert_eq_with_retry(node1, "SELECT count(*) FROM replica_2.replicated where shard_id < 35", '0')
    node1.query("delete from replica_1.replicated_local on cluster cross_3shards_2replicas;")
    node1.query("SYSTEM SYNC REPLICA replica_2.replicated_local;", timeout=5)
