#!/usr/bin/env python3
"""Test `ALTER TABLE ... DROP PARTITION` against supported Iceberg partition transforms."""
...
def test_drop_partition_transforms(started_cluster_iceberg_no_spark, storage_type):
    ...
    run(
        ...
        "icebergBucket(4, k) = icebergBucket(4, 'banana')",
        "id, k",
    )
    run(
        "truncate_literal",
    ...
    run(
        ...
        "icebergTruncate(3, k) = icebergTruncate(3, 'apricot')",
        "id, k",
    )
    run(
        "year",
        ...
        "toYearNumSinceEpoch(d) = toYearNumSinceEpoch(toDate('2024-06-01'))",
        "id, d",
    )
    run(
        "month",
        ...
        "toMonthNumSinceEpoch(d) = toMonthNumSinceEpoch(toDate('2025-01-15'))",
        "id, d",
    )
    run(
        "day",
        ...
        "toRelativeDayNum(d) = toRelativeDayNum(toDate('2025-05-01'))",
        "id, d",
    )
    run(
        "hour",
        ...
        "toRelativeHourNum(ts) = toRelativeHourNum(toDateTime('2025-05-19 10:30:00'))",
        "id, ts",
    )


@pytest.mark.parametrize("storage_type", ["s3", "local"])
def test_drop_partition_multi_column_transform(
    ...
    expected1 = instance.query(
        f"SELECT a, b, payload FROM {table} WHERE NOT ({pred1}) ORDER BY ALL"
    )
    instance.query(
    ...
    assert instance.query(
        f"SELECT a, b, payload FROM {table} ORDER BY ALL"
    ) == expected1

    bucket_banana = instance.query("SELECT icebergBucket(4, 'banana')").strip()
    pred2 = "a = 2 AND icebergBucket(4, b) = icebergBucket(4, 'banana')"
    expected2 = instance.query(
        f"SELECT a, b, payload FROM {table} WHERE NOT ({pred2}) ORDER BY ALL"
    )
    instance.query(
...
@pytest.mark.parametrize("storage_type", ["s3", "local"])
def test_drop_partition_arity_mismatch(
    started_cluster_iceberg_no_spark, storage_type
):
    instance = started_cluster_iceberg_no_spark.instances["node1"]
    ...
    assert "INVALID_PARTITION_VALUE" in error
