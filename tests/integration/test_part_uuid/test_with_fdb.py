...
def test_part_uuid(started_cluster):
    ...
    for node in [node1, node2]:
        ...
        part_initial_uuid = uuid.UUID(
            node.query(
                "SELECT uuid FROM system.parts WHERE table = 't' AND active ORDER BY name"
            ).strip()
        )
    ...
    # Test detach / attach.
    node1.query(
        "ALTER TABLE t DETACH PARTITION tuple(); ALTER TABLE t ATTACH PARTITION tuple()"
    )
    ...
    node1.query(
        "ALTER TABLE t UPDATE value = 1 WHERE key = 1 SETTINGS mutations_sync = 2"
    )
    part_mutate_uuid = uuid.UUID(
        node1.query(
            "SELECT uuid FROM system.parts WHERE table = 't' AND active ORDER BY name"
        ).strip()
    )
    ...
    assert part_mutate_uuid == uuid.UUID(
        node2.query(
            "SELECT uuid FROM system.parts WHERE table = 't' AND active ORDER BY name"
        ).strip()
    )
