...
def test_backup_restore_correct_block_ids(cluster):
    ...
    data_blob = f"{data_path}/data/default/test_simple_merge_tree/all_1_1_0/data.bin"
    found = False
    for blob in blobs:
    ...
    assert azure_query(
        node, f"SELECT * from test_simple_merge_tree_restored ORDER BY key"
