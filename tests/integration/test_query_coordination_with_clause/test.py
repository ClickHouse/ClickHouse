...
def test_query(started_cluster):
    ...
    res = node1.query("with cte_numbers as (select id from distributed_table where id > 3 limit 1000 ) select count() from distributed_table where id in (select id from cte_numbers) SETTINGS allow_experimental_query_coordination = 1")

    res1 = node1.query("with cte_numbers as (select id from distributed_table where id > 3 limit 1000 ) select count() from distributed_table where id in (select id from cte_numbers)")
