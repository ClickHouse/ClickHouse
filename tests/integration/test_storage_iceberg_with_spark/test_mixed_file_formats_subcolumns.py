...
def test_mixed_file_formats_subcolumns(started_cluster_iceberg_with_spark, storage_type, use_prewhere):
    """
    A `Parquet`-configured Iceberg table whose data files are partly ORC. Subcolumns of a struct
    (`s.a`) are planned as direct reads from the table-level format, which only the `Parquet`
    reader can do; the ORC files must read the whole struct and extract the subcolumn instead of
    treating `s.a` as a missing column.
    """
    instance = started_cluster_iceberg_with_spark.instances["node1"]
    spark = started_cluster_iceberg_with_spark.spark_session
    TABLE_NAME = "test_mixed_file_formats_subcolumns_" + storage_type + "_" + get_uuid_str()
    ...
    spark.sql(f"ALTER TABLE {TABLE_NAME} SET TBLPROPERTIES ('write.format.default' = 'orc')")
    spark.sql(f"INSERT INTO {TABLE_NAME} VALUES (2, named_struct('a', 20, 'b', 'orc'))")
    ...
    assert instance.query(f"SELECT id, s FROM {TABLE_NAME} {where} s.b = 'orc'", settings=settings) == "2\t(20,'orc')\n"
