...
@pytest.mark.parametrize("storage_type", ["s3", "local"])
def test_partition_pruning_without_functions(started_cluster_iceberg_with_spark, storage_type):
    instance = started_cluster_iceberg_with_spark.instances["node1"]
    spark = started_cluster_iceberg_with_spark.spark_session
    TABLE_NAME = "test_partition_pruning_without_functions_" + storage_type + "_" + get_uuid_str()
    ...
    assert check_validity_and_get_prunned_files(select("ts_day = '2024-01-20 10:00:00'")) == 3
    assert check_validity_and_get_prunned_files(select("ts_day < '2024-01-21 00:00:00'")) == 3
    assert check_validity_and_get_prunned_files(select("ts_day >= '2024-02-01 00:00:00'")) == 2
    assert check_validity_and_get_prunned_files(select("ts_day BETWEEN '2024-01-21 00:00:00' AND '2024-02-21 00:00:00'")) == 2
    assert check_validity_and_get_prunned_files(select("ts_day IN ('2024-01-20 10:00:00', '2024-02-20 10:00:00')")) == 2

    assert check_validity_and_get_prunned_files(select("ts_hour = '2024-01-20 11:00:00'")) == 3
    assert check_validity_and_get_prunned_files(select("ts_hour > '2024-01-21 09:00:00'")) == 2

    assert check_validity_and_get_prunned_files(select("ts_month = '2024-02-20 10:00:00'")) == 3
    assert check_validity_and_get_prunned_files(select("ts_month < '2024-03-01 00:00:00'")) == 2

    assert check_validity_and_get_prunned_files(select("d_year = '2026-01-20'")) == 3
    assert check_validity_and_get_prunned_files(select("d_year < '2025-01-01'")) == 3
    assert check_validity_and_get_prunned_files(select("d_year >= '2026-01-01'")) == 2
    assert check_validity_and_get_prunned_files(select("d_year IN ('2024-01-20', '2026-01-20')")) == 2

    assert check_validity_and_get_prunned_files(select("ts_day = '2024-01-20 12:00:00'")) == 3
    assert check_validity_and_get_prunned_files(select("d_year = '2026-06-01'")) == 3
