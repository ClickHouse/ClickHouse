...
def test_delete_manifests_are_decoded_concurrently(started_cluster_iceberg_with_spark):
    instance = started_cluster_iceberg_with_spark.instances["node1"]
    TABLE_NAME = "test_delete_manifests_are_decoded_concurrently_" + get_uuid_str()
    ...
    no_cache = {"use_iceberg_metadata_files_cache": 0}
    check_delete_manifests(instance, table_function, no_cache)

    query = (
        f"SELECT id FROM {table_function} SETTINGS use_iceberg_metadata_files_cache = 0, "
        "iceberg_delete_manifest_decode_concurrency = {} FORMAT Null"
    )

    instance.query("SYSTEM ENABLE FAILPOINT iceberg_slow_manifest_read")
    try:
        serial = elapsed(instance, query.format(1))
        concurrent = elapsed(instance, query.format(DELETE_COUNT))
    finally:
        instance.query("SYSTEM DISABLE FAILPOINT iceberg_slow_manifest_read")

    logging.info(
        "%s delete manifests took %.3fs serially and %.3fs concurrently",
        DELETE_COUNT,
        serial,
        concurrent,
    )

    # Skip timing assert on slow build.
    if instance.is_built_with_sanitizer() or instance.is_built_with_llvm_coverage():
        logging.info("Instrumented build: skipping the timing asserts")
        return

    # Verify failpoint fired.
    serial_floor = 0.8 * DELETE_COUNT * SLEEP_PER_MANIFEST_SECONDS
    assert serial > serial_floor, (
        f"serial decoding of {DELETE_COUNT} delete manifests took only {serial:.3f}s, below the "
        f"{serial_floor:.3f}s expected from the injected delay"
    )

    # Concurrent costs about one delay against DELETE_COUNT serially, so half is a wide margin.
    assert concurrent < serial / 2, (
        f"decoding {DELETE_COUNT} delete manifests took {concurrent:.3f}s with "
        f"iceberg_delete_manifest_decode_concurrency = {DELETE_COUNT} versus {serial:.3f}s "
        f"serially, so they were likely still decoded one at a time"
    )
