...
def wait_for_cache_initialized(node, cache_path, max_attempts=50, from_host=True):
    initialized = False
    attempts = 0
    while not initialized:
        logs = node.grep_in_log("Initialized cache from", from_host=from_host)
        initialized = cache_path in logs
...
def test_parallel_cache_loading_on_startup(cluster, node_name):
    ...
    wait_for_cache_initialized(node, "parallel_loading_test", from_host=False)
