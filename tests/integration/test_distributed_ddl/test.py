...
def test_replicated_without_arguments(test_cluster):
    ...
    assert "Forced backward compatibility of ON CLUSTER queries is enabled" in \
           instance.query_and_get_error("CREATE TABLE test_atomic.rmt ON CLUSTER cluster (n UInt64, s String) ENGINE=ReplicatedMergeTree() ORDER BY n")
    test_cluster.ddl_check_query(instance,
