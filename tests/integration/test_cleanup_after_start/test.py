...
def test_readonly_toggle_preserves_part_removal_directory(start_cluster):
    node1.query(
        "CREATE TABLE readonly_part_removal (n UInt64) ENGINE = MergeTree ORDER BY n "
        "SETTINGS disk = 'default', table_readonly = 1, "
        "cleanup_delay_period = 3600, max_cleanup_delay_period = 3600"
    )
    data_path = node1.query(
        "SELECT data_paths[1] FROM system.tables "
        "WHERE database = currentDatabase() AND name = 'readonly_part_removal'"
    ).strip()
    removal_dir = data_path + "delete_tmp_all_1_1_0"
    temporary_dir = data_path + "tmp_insert_readonly_toggle"
    try:
        node1.exec_in_container(["mkdir", removal_dir, temporary_dir])
        node1.exec_in_container(
            ["touch", "-d", "2000-01-01 UTC", removal_dir, temporary_dir]
        )
        node1.query("ALTER TABLE readonly_part_removal MODIFY SETTING table_readonly = 0")
        assert node1.path_exists(removal_dir), "Live part removal owns `delete_tmp_` directories"
        assert not node1.path_exists(temporary_dir), "Unowned temporary directories must be cleaned"
    finally:
        node1.exec_in_container(["rm", "-rf", removal_dir, temporary_dir])
        node1.query("DROP TABLE readonly_part_removal SYNC")
