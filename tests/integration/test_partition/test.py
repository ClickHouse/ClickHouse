...
def partition_table_complex(started_cluster):
    ...
    q(
        ...
        "ENGINE = MergeTree PARTITION BY p ORDER BY k SETTINGS index_granularity=1, index_granularity_bytes=0, compress_marks=false, compress_primary_key=false, ratio_of_defaults_for_sparse_serialization=1, serialization_info_version='default', replace_long_file_name_to_hash=false"
    )
