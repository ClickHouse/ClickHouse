...
def test_wrong_database_name(start):
    node.query("""CREATE TABLE test.table_test (i Int64) ENGINE=Memory;
...
def test_wrong_table_name(started_cluster):
    node.query("""
        ...
        INSERT INTO test.table_test SELECT 1;
        """)
    with pytest.raises(QueryRuntimeException, match="DB::Exception: Table table_test1 doesn't exist. Maybe you wanted to type: \\['table_test2'\\]"):
        node.query("""
