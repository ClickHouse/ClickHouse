...
def test_types(started_cluster, use_delta_kernel):
    ...
    instance.query(
        f"""
        DROP TABLE IF EXISTS {TABLE_NAME};
        CREATE TABLE {TABLE_NAME} ENGINE=DeltaLake('http://{started_cluster.minio_ip}:{started_cluster.minio_port}/{bucket}/{result_file}/', 'minio', 'ClickHouse_Minio_P@ssw0rd')"""
    )
    ...
    table_function = f"deltaLake('http://{started_cluster.minio_ip}:{started_cluster.minio_port}/{bucket}/{result_file}/', 'minio', 'ClickHouse_Minio_P@ssw0rd', SETTINGS allow_experimental_delta_kernel_rs={use_delta_kernel})"
    assert (
...
def test_partition_columns(started_cluster, use_delta_kernel):
    ...
    result = instance.query(
        f"describe table deltaLake('http://{started_cluster.minio_ip}:{started_cluster.minio_port}/{bucket}/{result_file}/', 'minio', 'ClickHouse_Minio_P@ssw0rd', SETTINGS allow_experimental_delta_kernel_rs={use_delta_kernel})"
    ).strip()
    ...
    result = int(
        instance.query(
            f"""SELECT count()
            FROM deltaLake('http://{started_cluster.minio_ip}:{started_cluster.minio_port}/{bucket}/{result_file}/', 'minio', 'ClickHouse_Minio_P@ssw0rd', SETTINGS allow_experimental_delta_kernel_rs={use_delta_kernel})
            """
        )
    )
    ...
    instance.query(
       ...
       ENGINE=DeltaLake('http://{started_cluster.minio_ip}:{started_cluster.minio_port}/{bucket}/{result_file}/', 'minio', 'ClickHouse_Minio_P@ssw0rd')
       SETTINGS allow_experimental_delta_kernel_rs={use_delta_kernel}
        """
    )
    assert (
...
18	test18	2000-01-18	18	false"""
        == instance.query(
            f"""
SELECT * FROM deltaLake('http://{started_cluster.minio_ip}:{started_cluster.minio_port}/{bucket}/{result_file}/', 'minio', 'ClickHouse_Minio_P@ssw0rd', SETTINGS allow_experimental_delta_kernel_rs={use_delta_kernel}) ORDER BY c
        """
        ).strip()
    )
    assert (
...
def test_complex_types(started_cluster, use_delta_kernel):
    ...
    assert "1\n2\n3\n" in node.query(
        f"SELECT id FROM deltaLake('http://{started_cluster.minio_ip}:{started_cluster.minio_port}/root/{table_name}' , 'minio', 'ClickHouse_Minio_P@ssw0rd', SETTINGS allow_experimental_delta_kernel_rs={use_delta_kernel})"
    )
    assert (
        "('123 Elm St','Springfield','IL')\n('456 Maple St','Shelbyville','IL')\n('789 Oak St','Ogdenville','IL')"
        in node.query(
            f"SELECT address FROM deltaLake('http://{started_cluster.minio_ip}:{started_cluster.minio_port}/root/{table_name}' , 'minio', 'ClickHouse_Minio_P@ssw0rd', SETTINGS allow_experimental_delta_kernel_rs={use_delta_kernel})"
        )
    )
    assert (
        "{'key1':'value1','key2':'value2'}\n{'key1':'value3','key2':'value4'}\n{'key1':'value5','key2':'value6'}"
        in node.query(
            f"SELECT metadata FROM deltaLake('http://{started_cluster.minio_ip}:{started_cluster.minio_port}/root/{table_name}' , 'minio', 'ClickHouse_Minio_P@ssw0rd', SETTINGS allow_experimental_delta_kernel_rs={use_delta_kernel})"
        )
    )
...
def test_replicated_database_and_unavailable_s3(started_cluster, use_delta_kernel):
    ...
    with PartitionManager() as pm:
        ...
        node1.query(
            ...
            AS deltaLake('http://{started_cluster.minio_ip}:{started_cluster.minio_port}/root/{TABLE_NAME}' , 'minio', 'ClickHouse_Minio_P@ssw0rd')
            """
        )
