...
def test_metric_values_and_stability(started_cluster):
    ...
    if node.is_built_with_llvm_coverage() or node.is_built_with_thread_sanitizer():
        pytest.skip("the query matrix does not fit in the 900s per-test timeout on coverage/TSan builds")
    report = []
...
## `test_repro_unified_foreground_matches_legacy` was removed: the unified inline foreground is now
## the executor's only foreground path (no `reader_executor_unified_foreground` toggle), so
## `test_repro_legacy_vs_executor` already covers executor-vs-non-executor equivalence.
