...
def test_partial_table(started_cluster):
    ...
    check_tables_are_synchronized(
        instance, table, postgres_database=pg_manager.get_default_database(), columns=['key', 'z']
    )
...
def test_partial_and_full_table(started_cluster):
    ...
    check_tables_are_synchronized(
        instance, f"{table}1", postgres_database=pg_manager.get_default_database(), columns=['key', 'x', 'z']
    )
    check_tables_are_synchronized(
