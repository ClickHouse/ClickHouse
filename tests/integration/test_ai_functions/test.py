...
def test_generate_content_basic(started_cluster):
    result = instance.query(
        ...
        settings={**AI_SETTINGS, "ai_credentials": "ai_mock"},
    )
...
def test_generate_content_error_throw(started_cluster):
    error = instance.query_and_get_error(
        ...
        settings={**AI_SETTINGS, "ai_credentials": "ai_error"},
    )
...
def test_generate_content_error_graceful(started_cluster):
    result = instance.query(
        "SELECT aiGenerate('hello')",
        settings={**AI_SETTINGS, "ai_credentials": "ai_error", "ai_function_throw_on_error": 0},
    )
    assert result.strip() == ""
...
def test_generate_without_api_key(started_cluster):
    ...
    result = instance.query(
        ...
        settings={**AI_SETTINGS, "ai_credentials": "ai_no_key"},
    )
...
def test_embed_basic(started_cluster):
    ...
    result = instance.query(
        ...
        settings={**AI_SETTINGS, "ai_credentials": "ai_embed"},
    )
...
def test_embed_batching(started_cluster):
    ...
    instance.query(
        "SELECT aiEmbed(x) FROM test_input",
        settings={**AI_SETTINGS, "ai_credentials": "ai_embed", "ai_function_embedding_max_batch_size": 2},
        query_id=qid,
    )
    events = get_profile_events(qid)
...
def test_embed_error_throw(started_cluster):
    ...
    error = instance.query_and_get_error(
        "SELECT aiEmbed('hello')",
        settings={**AI_SETTINGS, "ai_credentials": "ai_embed_error"},
    )
    assert "RECEIVED_ERROR_FROM_REMOTE_IO_SERVER" in error
...
def test_embed_error_graceful(started_cluster):
    ...
    result = instance.query(
        "SELECT aiEmbed(x) FROM test_input",
        settings={
            **AI_SETTINGS, "ai_credentials": "ai_embed_error",
...
def test_embed_duplicate_index_rejected(started_cluster):
    ...
    error = instance.query_and_get_error(
        ...
        settings={**AI_SETTINGS, "ai_credentials": "ai_embed_dup_index", "ai_function_max_retries": 0},
    )
...
def test_embed_wrong_count_rejected(started_cluster):
    ...
    error = instance.query_and_get_error(
        "SELECT aiEmbed(x) FROM (SELECT arrayJoin(['a', 'b']) AS x)",
        settings={**AI_SETTINGS, "ai_credentials": "ai_embed_wrong_count", "ai_function_max_retries": 0},
    )
    assert "MALFORMED_AI_PROVIDER_RESPONSE" in error
...
def test_embed_quota_input_tokens_exceeded(started_cluster):
    ...
    result = instance.query(
        "SELECT aiEmbed(x) FROM test_input",
        settings={
            **AI_SETTINGS, "ai_credentials": "ai_embed",
