...
SETTINGS index_granularity = 8192, storage_policy = 's3'""")
    ...
    node1.query("INSERT INTO test1 SELECT toDate('2023-01-01') + toIntervalDay(number), number + 1000 from system.numbers limit 20")
    ...
    part_to_disk = get_part_to_disk(node1.query("SELECT disk_name, name FROM system.parts where table = 'test1' and active"))
...
SETTINGS index_granularity = 8192, storage_policy = 's3'""")
    ...
    part_to_disk = get_part_to_disk(node2.query("SELECT disk_name, name FROM system.parts where table = 'test1' and active"))
    assert part_to_disk['20221226_0_0_0'] == "s3"
    assert part_to_disk['20230102_0_0_0'] == "s3"
    assert part_to_disk['20230109_0_0_0'] == "s3"
    assert part_to_disk['20230116_0_0_0'] == "default"
