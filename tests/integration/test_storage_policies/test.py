...
def test_alter_storage_policy_with_existing_disk_contents(started_cluster):
    ...
    check_case("test_missing_directory", None)
    ...
    for table_name, command, expected_error, with_ignored_contents in [
        ...
        ("test_tmp_directory", "mkdir -p {disk2_data_path}/tmp_1_1_0", "already contain data", True),
        ("test_delete_tmp_directory", "mkdir -p {disk2_data_path}/delete_tmp_all_0_0_0", "already contain data", True),
        ("test_tmp_fetch_directory", "mkdir -p {disk2_data_path}/tmp-fetch_1_1_0", "already contain data", True),
        ("test_tmp_mutation_file", "mkdir -p {disk2_data_path} && touch {disk2_data_path}/tmp_mutation_1.txt", "already contain data", True),
