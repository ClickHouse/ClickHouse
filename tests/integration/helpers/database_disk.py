...
def replace_text_in_metadata(node, metadata_path: str, old_value: str, new_value: str):
    # Use clickchouse-disk to modify the metadata, as it can work with both local and remote disks
    ...
    command = f"{disk_cmd_prefix} 'read --path-from {metadata_path}' | sed 's/{old_value}/{new_value}/' | {disk_cmd_prefix} 'write --path-to {metadata_path}'"
