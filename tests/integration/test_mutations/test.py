...
def test_trivial_alter_in_partition_merge_tree_with_where(start_cluster):
    try:
        ...
        node1.query("ALTER TABLE {} UPDATE x = x + 1 IN PARTITION 2 WHERE p = 2".format(name))
...
def test_alter_in_partition_merge_tree_without_where(start_cluster):
    try:
        ...
        assert node1.query("SELECT sum(x) FROM {}".format(name)).splitlines() == ["6"]
        node1.query("ALTER TABLE {} UPDATE x = x + 1 IN PARTITION 3".format(name))
        assert node1.query("SELECT sum(x) FROM {}".format(name)).splitlines() == ["6"]
        with pytest.raises(helpers.client.QueryRuntimeException):
            node1.query("ALTER TABLE {} DELETE IN PARTITION 2".format(name))
        assert node1.query("SELECT sum(x) FROM {}".format(name)).splitlines() == ["6"]
        with pytest.raises(helpers.client.QueryRuntimeException):
            node1.query("ALTER TABLE {} DELETE IN PARTITION 3".format(name))
        assert node1.query("SELECT sum(x) FROM {}".format(name)).splitlines() == ["6"]
    finally:
        node1.query("DROP TABLE {}".format(name))
