...
def test_reload_cluster_config_if_host_address_change(cluster_ready):
    ...
    assert node.query("SELECT ip_address FROM system.dns_cache WHERE hostname='node8'") == "192.168.1.1\n192.168.1.2\n127.0.0.1\n"
    ...
    assert node.query("SELECT is_local FROM system.clusters WHERE cluster='test_cluster' AND host_name='node8'") == "1\n"
    assert node.query("SELECT ip_address FROM system.dns_cache WHERE hostname='node8'") == "192.168.1.2\n192.168.1.1\n127.0.0.1\n"
