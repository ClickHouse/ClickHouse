...
def test_table_functions():
    ...
    check_logs(
        must_contain=[
            ...
            "CREATE TABLE tablefunc49 (`x` int) AS jdbc('[HIDDEN]', 'mydb', 'mytable')",
            "CREATE TABLE tablefunc50 (`x` int) AS odbc('[HIDDEN]', 'mydb', 'mytable')",
            "CREATE TABLE tablefunc51 (`x` int) AS jdbc('jdbc://user:[HIDDEN]@localhost:5432/mydb', 'mydb', 'mytable')",
            "CREATE TABLE tablefunc52 (`x` int) AS odbc('odbc://user:[HIDDEN]@localhost:5432/mydb', 'mydb', 'mytable')",
            "CREATE TABLE tablefunc53 (`x` int) AS jdbc(named_collection_1, datasource = '[HIDDEN]')",
            "CREATE TABLE tablefunc54 (`x` int) AS odbc(named_collection_1, connection_settings = '[HIDDEN]')",
            "CREATE TABLE tablefunc55 (`x` int) AS jdbc(named_collection_1, datasource = 'jdbc://user:[HIDDEN]@localhost:5432/mydb')",
            "CREATE TABLE tablefunc56 (`x` int) AS odbc(named_collection_1, connection_settings = 'odbc://user:[HIDDEN]@localhost:5432/mydb')",
            "CREATE TABLE tablefunc57 (`x` int) AS jdbc(named_collection_1, datasource = '[HIDDEN]', connection_settings = '[HIDDEN]')",
            "CREATE TABLE tablefunc58 (`x` int) AS jdbc(named_collection_1, connection_settings = '[HIDDEN]', external_database = '[HIDDEN]', datasource = '[HIDDEN]')",
        ],
        must_not_contain=[password],
    )
