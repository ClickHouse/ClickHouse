...
def test_selecting_with_stale_vs_latest_metadata(spark_only, storage_type):
...
@pytest.mark.parametrize("storage_type", ["s3"])
def test_default_async_metadata_refresh(spark_only, storage_type):
    started_cluster_iceberg_with_spark = spark_only.cluster
    instance = started_cluster_iceberg_with_spark.instances["node1"]
    spark = started_cluster_iceberg_with_spark.spark_session
    TABLE_NAME = (
...
@pytest.mark.parametrize("storage_type", ["s3"])
def test_async_metadata_refresh(spark_only, storage_type):
    started_cluster_iceberg_with_spark = spark_only.cluster
    instance = started_cluster_iceberg_with_spark.instances["node1"]
    spark = started_cluster_iceberg_with_spark.spark_session
    TABLE_NAME = (
...
@pytest.mark.parametrize("storage_type", ["s3"])
def test_insert_updates_metadata_cache(spark_only, storage_type):
    started_cluster_iceberg_with_spark = spark_only.cluster
    instance = started_cluster_iceberg_with_spark.instances["node1"]
    TABLE_NAME = (
