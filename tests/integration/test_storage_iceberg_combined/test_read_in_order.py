...
def test_read_in_order(spark_only,  storage_type):
...
def test_defining_columns_with_special_character(spark_only):
    started_cluster_iceberg_with_spark = spark_only.cluster
    instance = started_cluster_iceberg_with_spark.instances["node1"]
...
@pytest.mark.parametrize("storage_type", ["s3", "azure", "local"])
def test_read_in_order_with_complex_bucket(spark_only,  storage_type):
    started_cluster_iceberg_with_spark = spark_only.cluster
    instance = started_cluster_iceberg_with_spark.instances["node1"]
    spark = started_cluster_iceberg_with_spark.spark_session
...
@pytest.mark.parametrize("storage_type", ["s3", "azure", "local"])
def test_read_in_order_with_complex_truncate(spark_only,  storage_type):
    started_cluster_iceberg_with_spark = spark_only.cluster
    instance = started_cluster_iceberg_with_spark.instances["node1"]
    spark = started_cluster_iceberg_with_spark.spark_session
    TABLE_NAME = "test_position_deletes_" + storage_type + "_" + get_uuid_str()
