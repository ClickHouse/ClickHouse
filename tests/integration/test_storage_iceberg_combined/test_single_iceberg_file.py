...
def test_single_iceberg_file(engine, node, format_version, storage_type):
    if not engine.supports_storage(storage_type):
        pytest.skip(f"{engine.name} backend does not support {storage_type}")

    table = engine.unique_table(f"test_single_iceberg_file_{format_version}_{storage_type}")
