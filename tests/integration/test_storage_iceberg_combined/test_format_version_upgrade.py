...
def test_format_version_upgrade_v1_to_v2(spark_only, storage_type):
...
@pytest.mark.parametrize("storage_type", ["local"])
def test_remove_orphan_files_after_external_upgrade(spark_only, storage_type):
...
def test_format_version_avro_metadata_fallback(
    spark_only, storage_type, format_version, tmp_path
):
