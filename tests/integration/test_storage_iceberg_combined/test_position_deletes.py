...
def test_position_deletes(engine, node, use_roaring_bitmaps, storage_type):
    if not engine.supports_storage(storage_type):
        pytest.skip(f"{engine.name} backend does not support {storage_type}")

    table = engine.unique_table(f"test_position_deletes_{storage_type}")
    engine.create_table(table, [("id", "long"), ("data", "string")], format_version=2)

    engine.insert(table, [(i, chr(ord("a") + (i % 26))) for i in range(10, 100)])
    ...
    assert int(node.query(f"SELECT count() FROM {target}", settings=settings)) == 90

    engine.delete(table, "id < 20")
    engine.sync(table, storage_type)

    assert get_array(node.query(f"SELECT id FROM {target}", settings=settings)) == list(range(20, 100))

    assert int(node.query(f"SELECT count() FROM {target} where id >= 15", settings=settings)) == 80
    assert (
        int(
            node.query(
                f"SELECT count() FROM {target} where id >= 15 SETTINGS optimize_trivial_count_query=1",
                settings=settings,
            )
        )
        == 80
    ...
    engine.delete(table, "id >= 90")
    engine.sync(table, storage_type)
    assert get_array(node.query(f"SELECT id FROM {target}", settings=settings)) == list(range(20, 90))

    engine.insert(table, [(i, chr(ord("a") + (i % 26))) for i in range(100, 200)])
    engine.sync(table, storage_type)
    assert get_array(node.query(f"SELECT id FROM {target}", settings=settings)) == list(range(20, 90)) + list(
        range(100, 200)
    )

    engine.delete(table, "id >= 150")
    engine.sync(table, storage_type)
    assert get_array(node.query(f"SELECT id FROM {target}", settings=settings)) == list(range(20, 90)) + list(
        range(100, 150)
    )
    ...
    assert get_array(
        node.query(
            f"SELECT id FROM {target} WHERE id % 3 = 0", settings=settings)) == list(range(21, 90, 3)) + list(range(102, 150, 3))


@pytest.mark.parametrize("use_roaring_bitmaps", [0, 1])
def test_position_deletes_out_of_order(spark_only, use_roaring_bitmaps):
...
def test_position_deletes_bounds_logging(spark_only, storage_type: str) -> None:
    """
