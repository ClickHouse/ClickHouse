...
# node1 -- distributed_async_insert_split_batch_on_failure=on
node1 = cluster.add_instance(
    "node1",
    main_configs=["configs/remote_servers.xml"],
    user_configs=["configs/overrides_1.xml"],
)
# node2 -- distributed_async_insert_split_batch_on_failure=off
...
def get_test_settings():
    settings = {"async_insert_batch": [0, 1]}
...
def test_distributed_async_insert_split_batch_on_failure_OFF(started_cluster):
    for setting, setting_value in get_test_settings():
        ...
        if setting == "async_insert_batch" and setting_value == 1:
            with pytest.raises(
...
def test_distributed_async_insert_split_batch_on_failure_ON(started_cluster):
    for setting, setting_value in get_test_settings():
        create_tables(**{setting: setting_value})
        for i in range(0, 100):
            limit = 100e3
