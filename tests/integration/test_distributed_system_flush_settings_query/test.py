...
def started_cluster():
    try:
        ...
        for node in (node1, node2):
            node.query(
                """CREATE TABLE local_table(id UInt32, val String) ENGINE = MergeTree PARTITION BY id ORDER BY id;"""
            )
...
def test_start_and_stop_replica_send(started_cluster):
    ...
    node1.query("SET max_partitions_per_insert_block = 1;")

    node1.query("INSERT INTO distributed_table VALUES (0, 'node1'),(1, 'node2')")
