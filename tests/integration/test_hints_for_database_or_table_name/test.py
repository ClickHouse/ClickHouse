...
def test_hints_for_database_name(started_cluster):
    ...
    with pytest.raises(QueryRuntimeException, match=r'DB::Exception: Database defaul doesn\'t exist. Maybe you meant: \[\'default\'\]'):
        node.query('''
            SELECT * FROM defaul.table_y LIMIT 1;
            ''')
...
def test_hints_for_table_name(started_cluster):
    node.query('''
        ...
        INSERT INTO default.table_x SELECT 1;
        ''')
    with pytest.raises(QueryRuntimeException, match=r'DB::Exception: Table default.table_y doesn\'t exist. Maybe you meant: \[\'default.table_x\'\]'):
        node.query('''
