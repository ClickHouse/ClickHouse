...
def check_patches_applied(comment, expected_sum, expected_modes):
    """
...
def test_patch_parts_upgrade(started_cluster):
    ...
    check_patches_applied("old_version_both_modes", 10000, (1, 1, 0))
    assert list(get_patch_partitions().values()) == [2]

    node.restart_with_latest_version()

    # Patches written by the old version are applied in the same modes after upgrade.
    check_patches_applied("new_version_old_patches", 10000, (1, 1, 0))
    ...
    check_patches_applied("new_version_v1_patch", 12000, (1, 1, 0))
    ...
    check_patches_applied("new_version_mixed_patches", 13000, (1, 1, 1))
    ...
    partitions = get_patch_partitions()
    ...
    assert get_patch_partitions()[old_v1_partition] == 1
    check_patches_applied("new_version_merged_patches", 13000, (1, 1, 1))
