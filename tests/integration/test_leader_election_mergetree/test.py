...
def test_detach_rejected_before_writing_detached_copy(started_cluster):
    """
    Regression for the epoch fence on `DETACH PART` / `DETACH PARTITION`: a stale leader must be
    rejected at the empty-part publish, and nothing of the rejected command may stay behind on the
    shared storage. The detached copy is written from the parts the commit reports as removed, so
    a command rejected at the fence never reaches `clonePartsToDetached` — otherwise a later
    `ATTACH PARTITION` could re-import data from a DDL that supposedly failed, and every retry
    would add another `_tryN` directory next to the orphan.
