...
def test_preconfigured_default_codec(start_cluster):
    for node in [node1, node2]:
        ...
        assert (
            node.query(
                "SELECT count(*) FROM compression_codec_multiple_with_key GROUP BY somedate SETTINGS enable_sharding_aggregator = 0 -- TODO: remove once sharded aggregation supports external aggregation (spill to disk)."
            )
            == "10003\n"
        )
