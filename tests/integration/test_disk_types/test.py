...
def test_different_types(cluster):
    ...
    for fields in response[1:]:  # skip header
        assert len(fields) >= 7
        assert disk_types.get(fields[name_col_ix], "UNKNOWN") == fields[type_col_ix], f"Wrong type ({fields[type_col_ix]}) for disk {fields[name_col_ix]}!"
        if "encrypted" in fields[name_col_ix]:
            assert fields[encrypted_col_ix] == "1", f"{fields[name_col_ix]} expected to be encrypted!"
        else:
            assert fields[encrypted_col_ix] == "0", f"{fields[name_col_ix]} expected to be non-encrypted!"
