...
def test_refreshable_mv_skip_ddls(started_cluster, append : bool, with_inner_table: bool):
    ...
    node1.query(
        f"CREATE DATABASE {db_name} ENGINE=Replicated('/test/{db_name}', " + r"'{shard}', '{replica}')"
    )
    ...
    # Make sure that the view is not refreshing, and schedule to be refreshed in at least 10 minutes
    ...
    assert table_info1 == node1.query_with_retry(f"SELECT uuid, name FROM system.tables WHERE database='{db_name}'", check_callback=lambda x: x == table_info1)
    data1 = set(node1.query(f"SELECT x FROM {db_name}.mv").strip().split("\n"))
    data2 = set(node2.query(f"SELECT x FROM {db_name}.mv").strip().split("\n"))
    assert data1 == data2
