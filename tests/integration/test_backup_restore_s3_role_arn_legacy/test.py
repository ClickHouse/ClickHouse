...
# Metadata written by a version that stripped the role identifiers names no credentials and carries no
# marker to reconstruct them from, so the base backup opens unauthenticated. Such a chain must still be
# recoverable with `use_same_s3_credentials_for_base_backup = 1`, which propagates the credentials of
# the locator the RESTORE was given -- here a role -- into every base backup along the way.
#
# The legacy shape is written into the metadata directly, so the test keeps exercising it whatever a
# current backup would store. The mock STS accepts the session name below.
...
def test_restore_legacy_role_chain_reusing_credentials(started_cluster):
    ...
    assert status_of(node.query(f"BACKUP TABLE data TO {base}")) == "BACKUP_CREATED"

    node.query("INSERT INTO data SELECT number FROM numbers(10, 10)")
    assert (
        status_of(
            node.query(f"BACKUP TABLE data TO {inc_1} SETTINGS base_backup = {base}")
        )
        == "BACKUP_CREATED"
    )
    ...
    assert (
        status_of(
            node.query(f"BACKUP TABLE data TO {inc_2} SETTINGS base_backup = {inc_1}")
        )
        == "BACKUP_CREATED"
    )
    ...
    assert (
        status_of(
            node.query(
                f"RESTORE TABLE data AS data_recovered FROM {inc_2} "
