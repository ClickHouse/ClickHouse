...
do
    ...
    "SELECT URLRegions as d, toTimeZone(ClientEventTime, 'Asia/Dubai') as a, MobilePhoneModel as b, ParamPrice as e, ClientIP6 as c FROM test.hits ORDER BY CounterID, EventDate, intHash32(UserID) LIMIT 50000 Format $format" | \
    ...
    echo "$format, true";
    $CLICKHOUSE_CLIENT --max_threads=0 --max_block_size=65505 --output_format_parallel_formatting=false -q \
    "SELECT URLRegions as d, toTimeZone(ClientEventTime, 'Asia/Dubai') as a, MobilePhoneModel as b, ParamPrice as e, ClientIP6 as c FROM ORDER BY CounterID, EventDate, intHash32(UserID) test.hits LIMIT 50000 Format $format" | \
