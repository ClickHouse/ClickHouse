...
if [ -x "$CLICKHOUSE_BINARY" ]; then
    ...
    CLICKHOUSE_BENCHMARK_BINARY=${CLICKHOUSE_BENCHMARK_BINARY:=$CLICKHOUSE_BINARY benchmark}
fi
...
export CLICKHOUSE_BENCHMARK=${CLICKHOUSE_BENCHMARK_BINARY:="${CLICKHOUSE_BINARY}-benchmark ${CLICKHOUSE_BENCHMARK_OPT0:-}"}
