...
-- Its description must also mention the setting required to enable the source.
SELECT
    name,
    position(syntax, 'http_proxy_urls') > 0 AS uses_correct_key,
    position(description, 'enable_ytsaurus_dictionary_source') > 0 AS mentions_required_setting
FROM system.dictionary_sources
WHERE name = 'ytsaurus';
