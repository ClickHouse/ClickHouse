...
QUERY_ID_HTTP=$(${CLICKHOUSE_CURL} -sS "${CLICKHOUSE_URL}&detach_non_readonly_queries=1&async_insert=0&query_id=${QUERY_ID_PREFIX}h1&query=INSERT+INTO+$TABLE+SELECT+3" -X POST -d "" 2>/dev/null || true)
...
INSERT_BODY_HTTP_CODE=$(${CLICKHOUSE_CURL} -sS -o /dev/null -w "%{http_code}" "${CLICKHOUSE_URL}&detach_non_readonly_queries=1&async_insert=0&query_id=${QUERY_ID_PREFIX}h2&query=INSERT+INTO+$TABLE+FORMAT+TabSeparated" -X POST -d "4" 2>/dev/null || echo "000")
...
ASYNC_POST_BODY_ONLY=$(${CLICKHOUSE_CURL} -sS "${CLICKHOUSE_URL}&detach_non_readonly_queries=1&async_insert=0&query_id=${QUERY_ID_PREFIX}h3" -X POST --data-binary "INSERT INTO $TABLE SELECT 5" 2>/dev/null)
...
QUERY_ID_NATIVE=$($CLICKHOUSE_CLIENT --query_id "${QUERY_ID_PREFIX}native" --detach_non_readonly_queries 1 --async_insert 0 -q "INSERT INTO $TABLE SELECT 7" 2>/dev/null || true)
