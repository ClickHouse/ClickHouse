...
mkdir "$CURDIR/$CLICKHOUSE_DATABASE"
...
$CLICKHOUSE_CLIENT --query="SELECT * FROM msgpack FORMAT MsgPack" > "$CURDIR/$CLICKHOUSE_DATABASE"/tmp_msgpac_test_all_types.msgpk;

cat "$CURDIR/$CLICKHOUSE_DATABASE"/tmp_msgpac_test_all_types.msgpk | $CLICKHOUSE_CLIENT --query="INSERT INTO msgpack FORMAT MsgPack";

rm  "$CURDIR/$CLICKHOUSE_DATABASE"/tmp_msgpac_test_all_types.msgpk
...
$CLICKHOUSE_CLIENT --query="SELECT * FROM msgpack FORMAT MsgPack" > "$CURDIR/$CLICKHOUSE_DATABASE"/tmp_msgpack_test_nested_arrays.msgpk;

cat "$CURDIR/$CLICKHOUSE_DATABASE"/tmp_msgpack_test_nested_arrays.msgpk | $CLICKHOUSE_CLIENT --query="INSERT INTO msgpack FORMAT MsgPack";
rm "$CURDIR/$CLICKHOUSE_DATABASE"/tmp_msgpack_test_nested_arrays.msgpk;
...
$CLICKHOUSE_CLIENT --query="SELECT * FROM msgpack FORMAT MsgPack" > "$CURDIR/$CLICKHOUSE_DATABASE"/tmp_msgpack_type_conversion.msgpk;

$CLICKHOUSE_CLIENT --query="DROP TABLE msgpack";

$CLICKHOUSE_CLIENT --query="CREATE TABLE msgpack (array Array(Int64)) ENGINE = Memory";

cat "$CURDIR/$CLICKHOUSE_DATABASE"/tmp_msgpack_type_conversion.msgpk | $CLICKHOUSE_CLIENT --query="INSERT INTO msgpack FORMAT MsgPack";
rm "$CURDIR/$CLICKHOUSE_DATABASE"/tmp_msgpack_type_conversion.msgpk;
...
rm -rf "${CURDIR:?}/${CLICKHOUSE_DATABASE:?}"
