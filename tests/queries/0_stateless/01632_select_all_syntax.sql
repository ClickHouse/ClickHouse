...
WITH 1 AS ALL SELECT if(ALL = 1, 1, 0);

WITH 1 AS DISTINCT SELECT if(DISTINCT = 1, 1, 0);

SELECT if(ALL % 2, 1, 0) FROM (SELECT number as ALL FROM numbers(10));
SELECT if(DISTINCT % 2 = 0, 1, 0)FROM (SELECT number as DISTINCT FROM numbers(10));
