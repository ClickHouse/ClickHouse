...
${CLICKHOUSE_LOCAL} "${local_opts[@]}" --query "SELECT count() > 0 FROM system.remote_data_paths WHERE disk_name = '05218_disk'"
...
${CLICKHOUSE_LOCAL} "${local_opts[@]}" --query "SELECT count() FROM system.remote_data_paths WHERE disk_name = '05218_disk'" 2>&1 \
