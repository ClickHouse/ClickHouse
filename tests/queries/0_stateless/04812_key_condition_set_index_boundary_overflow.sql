...
-- Every value of b is 255, so no granule can match and reading stays within the row limit.
-- Before the fix this read five granules: a debug build aborted, a release build exceeded the limit.
SELECT count() FROM t_setidx WHERE a = 1 AND b IN (0) SETTINGS max_rows_to_read = 4;
