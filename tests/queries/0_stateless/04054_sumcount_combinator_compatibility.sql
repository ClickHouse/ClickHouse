...
-- Stacked Distinct and OrDefault preserve the non-Nullable result and state format.
SELECT sumCountOrDefaultDistinct(x), hex(sumCountOrDefaultDistinctState(x)) FROM (SELECT CAST(NULL, 'Nullable(UInt8)') AS x);
SELECT sumCountDistinctOrDefault(x), hex(sumCountDistinctOrDefaultState(x)) FROM (SELECT CAST(NULL, 'Nullable(UInt8)') AS x);
