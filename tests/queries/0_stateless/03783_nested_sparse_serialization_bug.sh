...
select (tuple(tuple(''), ''), tuple(tuple('a'), 'a')) union all select (tuple(tuple('a'), 'a'), tuple(tuple(''), '')) INTO OUTFILE '03783_nested_sparse_serialization_bug.jer'
SQL
...
$CLICKHOUSE_CLIENT --query="insert into hmmm (a, e) select * FROM file('t.jer', 'CSV')"
