...
SELECT 'alias_column';
EXPLAIN indexes = 1 SELECT t, a FROM test_skip_idx_rewrites WHERE a > 97;

-- Test 2: the same expression written out verbatim.
SELECT 'verbatim_expression';
EXPLAIN indexes = 1 SELECT t FROM test_skip_idx_rewrites
...
EXPLAIN indexes = 1 SELECT t FROM test_skip_idx_rewrites WHERE multiIf(v > 0, v, NULL) > 97;
