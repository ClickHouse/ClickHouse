#!/usr/bin/env bash
# Tags: no-fasttest, no-random-settings, no-replicated-database, no-flaky-check, no-sanitizers, no-debug, no-llvm-coverage, no-random-detach
...
{ echo "USE tpcds;"; cat "$CURDIR/../../benchmarks/tpc-ds/queries/query_04.sql"; } | $CLICKHOUSE_CLIENT "${SETTINGS[@]}"
