...
for STORAGE_POLICY in 's3_cache' 'local_cache'; do
    ...
    ${CLICKHOUSE_CLIENT} --query "SELECT state, file_segment_range_begin, file_segment_range_end, size FROM system.filesystem_cache ORDER BY key, file_segment_range_begin, file_segment_range_end, size"
