...
-- it end-to-end: build the index, then answer text-search queries through it and assert the index
-- is actually applicable using `force_data_skipping_indices`.
--
-- We search with `hasAllTokens` / `hasAnyTokens`, which tokenize the needle with the index
-- tokenizer and therefore stay consistent regardless of `query_plan_direct_read_from_text_index`.
-- `hasToken` is deliberately not used here: it splits its argument on ASCII separators only, so for
-- Chinese text (which contains no ASCII separators) it disagrees with the segmented index tokens,
-- and its result would depend on whether the optimizer answers the predicate directly from the
-- index. The tokenizer-aware functions are the supported way to query such an index.
...
SELECT '-- hasAllTokens / hasAnyTokens match the segmented tokens';
SELECT id FROM tab WHERE hasAllTokens(doc, ['北京邮电大学']) ORDER BY id;
SELECT id FROM tab WHERE hasAnyTokens(doc, ['大厦']) ORDER BY id;
SELECT count() FROM tab WHERE hasAllTokens(doc, ['不存在']);
...
SELECT id FROM tab_fine WHERE hasAllTokens(doc, ['大学']) ORDER BY id SETTINGS force_data_skipping_indices = 'idx';
