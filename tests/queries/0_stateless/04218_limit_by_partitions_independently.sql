...
SELECT replaceRegexpOne(explain, '^[ ]*(.*)', '\\1') FROM (EXPLAIN actions = 1 SELECT a FROM test_with_filter_step WHERE b > 0 LIMIT 2 BY a SETTINGS allow_limit_by_partitions_independently = 1, optimize_move_to_prewhere = 0) WHERE explain LIKE '%Skip stream merging%' OR explain LIKE '%Read each partition through separate port%';
SELECT (SELECT count() FROM (SELECT a FROM test_with_filter_step WHERE b > 0 LIMIT 2 BY a SETTINGS allow_limit_by_partitions_independently = 0, optimize_move_to_prewhere = 0)) = (SELECT count() FROM (SELECT a FROM test_with_filter_step WHERE b > 0 LIMIT 2 BY a SETTINGS allow_limit_by_partitions_independently = 1, optimize_move_to_prewhere = 0));
...
INSERT INTO test_few_partitions_many_threads SELECT number FROM numbers_mt(1e3);
INSERT INTO test_few_partitions_many_threads SELECT number FROM numbers_mt(1e3);
