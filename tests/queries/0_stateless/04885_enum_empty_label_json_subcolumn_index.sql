...
-- A `Variant` or `Dynamic` constant reaches index analysis as a bare value that no longer names the
...
-- A `Tuple` key expression reads its own default at every element position, so a nested empty `Enum`
-- label is that value too and the same rule applies one level down.
...
-- The same holds one level down: the element types are walked, and a nested `Dynamic` still names no
-- alternative.
SELECT arraySort(groupArray(id)) FROM t_json_tuple WHERE data.alpha::Tuple(String) = tuple(CAST(CAST('', 'Enum8('''' = 3)'), 'Dynamic'));

-- A tuple constant that differs from that default leaves the index usable.
...
SELECT arraySort(groupArray(id)) FROM t_json_bf      WHERE data.alpha::String = CAST('', 'Nullable(FixedString(4))');
