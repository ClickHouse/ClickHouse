...
CREATE TABLE tab(k UInt64, s String, INDEX idx(s) TYPE text(tokenizer = 'ngrams', ngram_size = 2)) ENGINE = MergeTree() ORDER BY k;
...
ALTER TABLE tab ADD INDEX idx(s) TYPE text(tokenizer = 'ngrams', ngram_size = 2);
SELECT granularity FROM system.data_skipping_indices WHERE database = currentDatabase() AND table = 'tab' AND name = 'idx';
