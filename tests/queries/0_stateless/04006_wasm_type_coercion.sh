...
DROP FUNCTION IF EXISTS test_04006_wasm_identity_raw;
...
DELETE FROM system.webassembly_modules WHERE name = 'identity_int_04006';

EOF

cat ${CUR_DIR}/wasm/identity_int.wasm | ${CLICKHOUSE_CLIENT} --query "INSERT INTO system.webassembly_modules (name, code) SELECT 'identity_int_04006', code FROM input('code String') FORMAT RawBlob"

${CLICKHOUSE_CLIENT} --allow_experimental_analyzer=1 << EOF

CREATE FUNCTION test_04006_wasm_identity_raw          LANGUAGE WASM ABI ROW_DIRECT  FROM 'identity_int_04006' :: 'identity_raw'             ARGUMENTS (Int32) RETURNS Int32;
CREATE FUNCTION wasm_add               LANGUAGE WASM ABI ROW_DIRECT  FROM 'identity_int_04006' :: 'add'                  ARGUMENTS (Int32, Int32) RETURNS Int32;
CREATE FUNCTION wasm_identity_msgpack_i32 LANGUAGE WASM ABI BUFFERED_V1 FROM 'identity_int_04006' :: 'identity_msgpack_i32' ARGUMENTS (n Int32) RETURNS Int32;
CREATE FUNCTION wasm_identity_msgpack_i64 LANGUAGE WASM ABI BUFFERED_V1 FROM 'identity_int_04006' :: 'identity_msgpack_i64' ARGUMENTS (n Int64) RETURNS Int64;
CREATE FUNCTION wasm_identity_tsv_i32  LANGUAGE WASM ABI BUFFERED_V1 FROM 'identity_int_04006' :: 'identity_tsv_i32'     ARGUMENTS (n Int32) RETURNS Int32 SETTINGS serialization_format = 'TSV';

-- ROW_DIRECT: boundary values for all small-int types
SELECT test_04006_wasm_identity_raw(toInt8(-128)),   test_04006_wasm_identity_raw(toInt8(0)),    test_04006_wasm_identity_raw(toInt8(127));
SELECT test_04006_wasm_identity_raw(toUInt8(0)),     test_04006_wasm_identity_raw(toUInt8(128)), test_04006_wasm_identity_raw(toUInt8(255));
SELECT test_04006_wasm_identity_raw(toInt16(-32768)),test_04006_wasm_identity_raw(toInt16(0)),   test_04006_wasm_identity_raw(toInt16(32767));
SELECT test_04006_wasm_identity_raw(toUInt16(0)),    test_04006_wasm_identity_raw(toUInt16(1000)),test_04006_wasm_identity_raw(toUInt16(65535));
...
DROP FUNCTION test_04006_wasm_identity_raw;
