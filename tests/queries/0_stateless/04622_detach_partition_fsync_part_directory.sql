...
SELECT
    -- A DETACH walks the clone leaf + ancestor chain, so it fsyncs at least two more directories than a DROP.
    (SELECT synced FROM dir_sync WHERE table = 'detach_fsync_on')
        >= (SELECT synced FROM dir_sync WHERE table = 'detach_fsync_drop') + 2 AS clone_synced,
