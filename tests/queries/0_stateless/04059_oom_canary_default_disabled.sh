...
# This test verifies that the OOM canary stays disabled by default.
# We start a temporary server instance without overriding `oom_canary_enable`
# and verify that no canary child process appears.

test_dir=$(mktemp -d "${CLICKHOUSE_TMP}/04059_oom_canary_default_disabled.XXXXXX")
...
if grep -q "OOM canary is disabled" "$server_log"; then
    echo "default disabled confirmed"
else
...
if [[ "$has_canary" -eq 0 ]]; then
    echo "no default canary child process"
else
    echo "unexpected default canary child process found" >&2
    exit 1
fi
