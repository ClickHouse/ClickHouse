...
(
    ...
    arr SimpleAggregateFunction(groupArrayArray, Array(UInt8))
)
ENGINE = AggregatingMergeTree ORDER BY k;

INSERT INTO t_identity SELECT number, number, number, number, number, if(number % 3 = 0, NULL, number), toString(number), toString(number % 7), [toUInt8(number % 256)] FROM numbers(20);
INSERT INTO t_identity SELECT number, number + 100, number + 100, number, number + 1, NULL, toString(number), 'z', [255] FROM numbers(5, 3);
INSERT INTO t_identity SELECT number, number + 200, number + 200, number, 0, number, 'y', 'a', [1, 2] FROM numbers(30, 3);

SELECT 'final';
SELECT * FROM t_identity FINAL ORDER BY k SETTINGS max_block_size = 3;

SELECT 'group by';
SELECT k, min(mn), max(mx), sum(s), groupBitAnd(ba), anyLast(al), any(a), max(lc), groupArrayArray(arr) FROM t_identity GROUP BY k ORDER BY k;
...
(
    ...
    EXCEPT
    SELECT k, min(mn), max(mx), sum(s), groupBitAnd(ba), anyLast(al), any(a), max(lc), groupArrayArray(arr) FROM t_identity GROUP BY k
);
