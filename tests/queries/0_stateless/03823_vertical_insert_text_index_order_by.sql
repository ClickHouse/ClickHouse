-- Test: text index column also in ORDER BY works with vertical insert.
SET enable_full_text_index = 1;
SET materialize_skip_indexes_on_insert = 1;

DROP TABLE IF EXISTS t_vi_text_index_order_by;

CREATE TABLE t_vi_text_index_order_by
(
    id UInt64,
    s String,
...
INSERT INTO t_vi_text_index_order_by
SELECT number, toString(number) FROM numbers(1000);

SELECT count()
FROM system.parts
WHERE database = currentDatabase()
  AND table = 't_vi_text_index_order_by'
  AND active
  AND secondary_indices_uncompressed_bytes > 0;

DROP TABLE t_vi_text_index_order_by;
