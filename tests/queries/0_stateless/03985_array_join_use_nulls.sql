...
-- LowCardinality element type: the Nullable must go inside the wrapper, i.e. LowCardinality(Nullable(String)),
-- and empty arrays must still produce NULL rather than the default empty string.
SELECT 'low cardinality type';
SELECT id, x FROM (SELECT 1 AS id, ['a', 'b'] :: Array(LowCardinality(String)) AS arr UNION ALL SELECT 2 AS id, [] :: Array(LowCardinality(String)) AS arr) LEFT ARRAY JOIN arr AS x ORDER BY id, x;
SELECT 'low cardinality null check';
SELECT id, x IS NULL FROM (SELECT 1 AS id, ['a', 'b'] :: Array(LowCardinality(String)) AS arr UNION ALL SELECT 2 AS id, [] :: Array(LowCardinality(String)) AS arr) LEFT ARRAY JOIN arr AS x ORDER BY id, x;
SELECT 'low cardinality result type';
SELECT toTypeName(x) FROM (SELECT CAST([], 'Array(LowCardinality(String))') AS arr) LEFT ARRAY JOIN arr AS x;
