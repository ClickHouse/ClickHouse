...
(( $(${CLICKHOUSE_CLIENT} --user $user --query "INSERT INTO FUNCTION file('$file') SELECT 'a', 'b'" 2>&1 | grep -c "grant SOURCE WRITE ON FILE") >= 1 )) && echo "ACCESS DENIED" || echo "UNEXPECTED";

${CLICKHOUSE_CLIENT} --query "GRANT SOURCE WRITE ON FILE TO $user";
${CLICKHOUSE_CLIENT} --user $user --query "INSERT INTO FUNCTION file('$file') SELECT 'a', 'b'";

(( $(${CLICKHOUSE_CLIENT} --user $user --query "SELECT count() FROM file('$file')" 2>&1 | grep -c "grant SOURCE READ ON FILE") >= 1 )) && echo "ACCESS DENIED" || echo "UNEXPECTED";
...
${CLICKHOUSE_CLIENT} --query "GRANT SOURCES ON File TO $user";
${CLICKHOUSE_CLIENT} --user $user --query "INSERT INTO FUNCTION file('$file') SELECT 'a', 'b'";
${CLICKHOUSE_CLIENT} --user $user --query "SELECT count() FROM file('$file')";
