...
-- injective, so its atom conservatively stays relaxed and the negation reads everything.
...
-- A genuinely non-injective transform: the wrapped atom stays relaxed, the negation reads
-- everything and must not lose rows.
...
-- tuple membership: the component atom stays relaxed and the negation must not prune wrongly.
...
INSERT INTO test_tuple_negation SELECT intDiv(number, 4), number FROM numbers(24);

SELECT trimLeft(explain) FROM (EXPLAIN indexes = 1 SELECT count() FROM test_tuple_negation WHERE NOT has([(1, 5)], (x, y))) WHERE explain LIKE '%Condition%' OR explain LIKE '%Granules:%/%';
SELECT count() FROM test_tuple_negation WHERE NOT has([(1, 5)], (x, y));
SELECT count() FROM test_tuple_negation WHERE NOT has([(1, 5)], (x, y)) SETTINGS use_primary_key = 0, use_partition_pruning = 0, use_skip_indexes = 0;
