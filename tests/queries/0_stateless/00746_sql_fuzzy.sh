#!/usr/bin/env bash
# Tags: no-fasttest, no-coverage, no-msan
...
# Query replay:
# cat clickhouse-server.log  | grep -aF "<Debug> executeQuery: (from " | sed -E 's/^.*executeQuery: \(from [^ ]+\) (.*)/\1;/' | clickhouse-client -n --ignore-error
