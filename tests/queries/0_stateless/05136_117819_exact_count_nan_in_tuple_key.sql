...
-- Controls: a key that cannot hold a NaN, and a NaN-hiding key column that no condition reads. Both
-- predicates accept every row, so exact counting answers them from the index and reads only the single
-- row it produces; losing exactness here would read all eight.
...
SELECT log_comment, read_rows FROM system.query_log
    WHERE current_database = currentDatabase() AND type = 'QueryFinish'
      AND log_comment IN ('05136_int_key', '05136_unread_nan_key')
