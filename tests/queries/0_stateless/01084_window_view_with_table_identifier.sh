...
CREATE WINDOW VIEW ${db}.wv INNER ENGINE AggregatingMergeTree ORDER BY tuple(tumble(timestamp, INTERVAL '5' SECOND, 'US/Samoa'), market) ENGINE Memory WATERMARK=ASCENDING AS SELECT count(${db}.mt.a) AS count, market, tumbleEnd(wid) AS w_end FROM ${db}.mt GROUP BY tumble(${db}.mt.timestamp, INTERVAL '5' SECOND, 'US/Samoa') AS wid, market;
