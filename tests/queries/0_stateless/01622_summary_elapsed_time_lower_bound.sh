...
expected_elapsed_time=$(python3 -c "print($rows * $sleep_each_row * 1000000000)") # nanoseconds
elapsed_time=$(
...
if python3 -c "import sys; sys.exit(0 if $elapsed_time >= $expected_elapsed_time else 1)" ; then
