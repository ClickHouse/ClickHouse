...
CREATE TABLE t_04663 (k UInt64, v UInt64) ENGINE = Join(ANY, LEFT, k) SETTINGS max_rows_in_join; -- { error TYPE_MISMATCH }
CREATE TABLE t_04663 (k UInt64, v UInt64) ENGINE = Join(ANY, LEFT, k) SETTINGS disk; -- { error TYPE_MISMATCH }

-- The Bool ones are what the shorthand is for.
CREATE TABLE t_04663 (k UInt64, v UInt64) ENGINE = Join(ANY, LEFT, k) SETTINGS join_use_nulls;
...
CREATE TABLE t_04663 (k UInt64) ENGINE = Log SETTINGS disk; -- { error TYPE_MISMATCH }
CREATE TABLE t_04663 (k UInt64) ENGINE = StripeLog SETTINGS storage_policy; -- { error TYPE_MISMATCH }
