...
G="${DB}_g"   # IN subquery over a materialized CTE
S="${DB}_s"   # scalar subquery over a materialized CTE
K="${DB}_k"   # calls G
...
run_case "udf in"       "same_udf_in_${DB}"       "INSERT INTO dst SELECT $G(x) AS same_udf_in_${DB} FROM src WHERE x = 1"
...
run_case "apply lambda" "same_apply_lambda_${DB}" "INSERT INTO dst SELECT * APPLY (y -> y IN (WITH c AS MATERIALIZED (SELECT rand64() AS r FROM numbers(1)) SELECT toUInt64(a.r = b.r) FROM c AS a, c AS b)) FROM (SELECT x AS same_apply_lambda_${DB} FROM src WHERE x = 1)"
...
${CLICKHOUSE_CLIENT} -q "DROP TABLE dst SYNC; DROP TABLE src; DROP FUNCTION $G; DROP FUNCTION $S; DROP FUNCTION $K; DROP FUNCTION $H"
