...
INNER JOIN (SELECT number % 10 AS k FROM numbers(100000)) AS t2 USING (k)
...
SELECT count() FROM (SELECT number AS k FROM numbers(10)) AS t1
INNER JOIN (SELECT CAST(NULL, 'Nullable(UInt64)') AS k FROM numbers(100000)) AS t2 ON t1.k = t2.k
SETTINGS join_algorithm = 'hash';
SELECT count() FROM (SELECT number AS k FROM numbers(10)) AS t1
INNER JOIN (SELECT CAST(NULL, 'Nullable(UInt64)') AS k FROM numbers(100000)) AS t2 ON t1.k = t2.k
SETTINGS join_algorithm = 'grace_hash';
