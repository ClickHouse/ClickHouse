...
-- side of the multithreaded queries so the result stays 0-9.
...
SELECT * FROM (
    SELECT number, n, j1, j2
    FROM (SELECT number, number / 2 AS n FROM remote('127.0.0.{2,3}', system.numbers) ORDER BY number LIMIT 10)
    ANY LEFT JOIN (SELECT number / 3 AS n, number AS j1, 'Hello' AS j2 FROM system.numbers LIMIT 0)
    USING n
) ORDER BY number;
