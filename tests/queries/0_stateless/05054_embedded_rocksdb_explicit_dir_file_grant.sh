...
# WRITE ON FILE one this arm asserts on. Only where that source exists: on a branch without
# it the location is unchecked anyway, and the grant would be a syntax error.
if [ "$(${CLICKHOUSE_CLIENT} -q "SELECT count() FROM system.privileges WHERE privilege = 'DISK'")" != "0" ]; then
