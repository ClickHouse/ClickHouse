...
DROP TABLE IF EXISTS t_left_04093;
DROP TABLE IF EXISTS t_right_04093;
DROP TABLE IF EXISTS t_subq_04093;

CREATE TABLE t_left_04093 (a UInt64, b UInt64) ENGINE = MergeTree ORDER BY a;
CREATE TABLE t_right_04093 (a UInt64, c UInt64) ENGINE = MergeTree ORDER BY a;
CREATE TABLE t_subq_04093 (x UInt64) ENGINE = MergeTree ORDER BY x;

INSERT INTO t_left_04093 VALUES (1, 10), (2, 20), (3, 30);
INSERT INTO t_right_04093 VALUES (1, 100), (2, 200);
INSERT INTO t_subq_04093 VALUES (1);

-- Case 1: ANY LEFT JOIN with IN subquery from a real table.
-- The filter toUInt64(1 IN (SELECT x FROM t_subq_04093)) > 0 evaluates to TRUE
-- at runtime (1 IS in the subquery result), so all rows should pass.
SELECT a, b, c
FROM t_left_04093
ANY LEFT JOIN t_right_04093 USING (a)
WHERE toUInt64(1 IN (SELECT x FROM t_subq_04093)) > 0
ORDER BY a
...
SELECT a, b, c
FROM t_left_04093
ANY RIGHT JOIN t_right_04093 USING (a)
WHERE toUInt64(1 IN (SELECT number FROM numbers(10))) > 0
ORDER BY a
SETTINGS query_plan_filter_push_down = 0;

DROP TABLE t_left_04093;
DROP TABLE t_right_04093;
DROP TABLE t_subq_04093;
