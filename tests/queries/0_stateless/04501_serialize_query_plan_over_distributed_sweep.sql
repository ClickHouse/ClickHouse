-- Correctness sweep for serialize_query_plan = 1 over a Distributed table.
-- Every case is executed twice: through the plan-level distributed read and with
-- serialize_query_plan = 0 (the legacy path). The reference asserts that both
...
SELECT k, v * 2 AS d, concat(s, '_x') AS t FROM sweep_dist WHERE d > 40 AND k % 2 = 0 ORDER BY k, d, t SETTINGS serialize_query_plan = 0;
...
SELECT k, count() AS c, sum(v) AS sv FROM sweep_dist GROUP BY k HAVING sv > 130 ORDER BY k SETTINGS serialize_query_plan = 0;

SELECT '-- ORDER BY + LIMIT/OFFSET';
SELECT k, v FROM sweep_dist ORDER BY v DESC, k LIMIT 5 OFFSET 2;
SELECT k, v FROM sweep_dist ORDER BY v DESC, k LIMIT 5 OFFSET 2 SETTINGS serialize_query_plan = 0;

SELECT '-- DISTINCT';
SELECT DISTINCT k, s FROM sweep_dist ORDER BY k, s;
SELECT DISTINCT k, s FROM sweep_dist ORDER BY k, s SETTINGS serialize_query_plan = 0;

SELECT '-- literal IN (set is pushed to the shards with the plan)';
SELECT count(), sum(v) FROM sweep_dist WHERE k IN (1, 3);
SELECT count(), sum(v) FROM sweep_dist WHERE k IN (1, 3) SETTINGS serialize_query_plan = 0;
...
SELECT count(), sum(v) FROM sweep_dist WHERE k IN (SELECT k FROM sweep_dim) SETTINGS serialize_query_plan = 0;
...
SELECT count(), sum(v) FROM sweep_dist WHERE k IN (SELECT k FROM sweep_dist WHERE v < 10) SETTINGS distributed_product_mode = 'local', serialize_query_plan = 0;

-- With SET distributed_product_mode = 'local' the plan path falls back to the legacy execution
-- path (the plan path cannot implement per-shard subquery rewriting); serialize_query_plan = 1
-- there re-engages the eager per-shard plan building in SelectStreamFactory, and = 0 uses the AST
-- transport — both must agree. As above, the values match the classic per-shard local rewrite only
-- because both localhost shards read the same table.
...
SELECT count(), sum(v) FROM sweep_dist WHERE k IN (SELECT k FROM sweep_dist WHERE v < 10) SETTINGS serialize_query_plan = 0;
...
SELECT count(), sum(v) FROM sweep_dist WHERE k GLOBAL IN (SELECT k FROM sweep_dist WHERE v < 10) SETTINGS serialize_query_plan = 0;
...
WITH agg AS (SELECT k, sum(v) AS sv FROM sweep_dist GROUP BY k) SELECT k, sv FROM agg WHERE sv > 150 ORDER BY k SETTINGS serialize_query_plan = 0;
...
SELECT k, v FROM sweep_repl_dist FINAL ORDER BY k, v SETTINGS serialize_query_plan = 0;
...
SELECT count() FROM sweep_smpl_dist SAMPLE 1/2 SETTINGS serialize_query_plan = 0;
...
SELECT d.k, dim.name, sum(d.v) AS sv FROM sweep_dist AS d INNER JOIN sweep_dim AS dim ON d.k = dim.k GROUP BY d.k, dim.name ORDER BY d.k SETTINGS serialize_query_plan = 0;
...
SELECT count(), sum(v) FROM sweep_merge SETTINGS serialize_query_plan = 0;
