...
-- matching, so the reshaped type has to cover all of them, not just the first spelling.
...
-- struct root whose name folds to the same spelling.
...
-- A tuple element name may itself contain a dot, so `c0.a.b` is both the element `a.b` and the path
-- from `a` to `b`. The extractor always cuts at the first dot and takes the path, so the declared
-- type used as the nullability arbiter has to be resolved the same way; a full-path subcolumn lookup
-- binds to the element instead and its non-nullable type then collapses the genuine NULL.
...
SELECT `c0.a.b`, isNull(`c0.a.b`), `c0.c` FROM file(currentDatabase() || '_04401_dot.arrow', 'Arrow', '`c0.a.b` Nullable(Tuple(v UInt32)), `c0.c` String') ORDER BY `c0.c`;

-- Only the exact lowercase `null` is reserved as an element name, so `Null` is legal. Under
-- case-insensitive matching the request `c0.null` matches both that element and the parent's virtual
-- null-map subcolumn; the latter is never a flattened column, so it must not supply the declared type.
...
SELECT `c0.null`, isNull(`c0.null`), `c0.c` FROM file(currentDatabase() || '_04401_nullname.arrow', 'Arrow', '`c0.null` Nullable(Tuple(v UInt32)), `c0.c` Nullable(String)') ORDER BY `c0.c` SETTINGS input_format_arrow_case_insensitive_column_matching = 1;
