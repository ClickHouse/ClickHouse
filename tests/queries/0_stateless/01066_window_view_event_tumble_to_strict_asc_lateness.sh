...
CREATE WINDOW VIEW ${db}.wv TO ${db}.dst WATERMARK=STRICTLY_ASCENDING ALLOWED_LATENESS=INTERVAL '2' SECOND AS SELECT count(a) AS count, tumbleEnd(wid) AS w_end FROM ${db}.mt GROUP BY tumble(timestamp, INTERVAL '5' SECOND, 'US/Samoa') AS wid;
...
INSERT INTO ${db}.mt VALUES (1, '1990/01/01 12:00:05');
INSERT INTO ${db}.mt VALUES (1, '1990/01/01 12:00:04');
INSERT INTO ${db}.mt VALUES (1, '1990/01/01 12:00:03');
INSERT INTO ${db}.mt VALUES (1, '1990/01/01 12:00:07');
...
$CLICKHOUSE_CLIENT "${opts[@]}" --query="SELECT * FROM ${db}.dst ORDER BY w_end, count;"
