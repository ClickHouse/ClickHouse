...
SELECT
    ...
    (SELECT count() FROM t_relative_hour_partition
     WHERE d >= toDateTime64('1969-12-31 21:00:00', 0, 'UTC') AND d <= toDateTime64('1970-01-01 02:00:00', 0, 'UTC')) AS indexed_count,
...
SELECT
    (SELECT countIf(toRelativeHourNum(d) = 23) FROM t_relative_hour_pk SETTINGS force_primary_key = 0) AS full_scan_count,
    (SELECT count() FROM t_relative_hour_pk WHERE toRelativeHourNum(d) = 23) AS indexed_count,
    full_scan_count = indexed_count AS matches;
