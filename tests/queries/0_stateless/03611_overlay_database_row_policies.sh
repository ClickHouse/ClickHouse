...
# and the underlying source table (a row must pass both).
...
read_overlay() { ${CLICKHOUSE_CLIENT} --user="${USER}" --query "SELECT id FROM ${DB_OVL}.t ORDER BY id" | xargs; }
read_source()  { ${CLICKHOUSE_CLIENT} --user="${USER}" --query "SELECT id FROM ${DB_A}.t ORDER BY id" | xargs; }

echo "facade policy applies through the facade (expect 1 2)"
read_overlay
echo "facade policy does not affect direct reads of the source (expect 1 2 3 4)"
read_source
...
echo "through the facade both policies apply, id<=2 AND id>=2 (expect 2)"
read_overlay
echo "direct reads of the source apply only the source policy, id>=2 (expect 2 3 4)"
read_source
