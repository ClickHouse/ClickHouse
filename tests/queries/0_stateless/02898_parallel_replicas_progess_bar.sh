...
query="SELECT k, sipHash64(v) FROM t1 order by k limit 10 offset 995"

# default coordinator
${CLICKHOUSE_CURL} -sS "${CLICKHOUSE_URL}&send_progress_in_http_headers=1&http_headers_progress_interval_ms=0" -d @- <<< "${query} SETTINGS optimize_read_in_order=0, allow_experimental_parallel_reading_from_replicas = 1, max_parallel_replicas = 3, use_hedged_requests=0, cluster_for_parallel_replicas='test_cluster_one_shard_three_replicas_localhost', parallel_replicas_single_task_marks_count_multiplier = 0.001" -v |& grep -e "X-ClickHouse-Summary:" | grep -F -m 1 '"total_rows_to_read":"3000"' | awk -F ',' '{print $5}'

# reading in order coordinator
$CLICKHOUSE_CLIENT -q "explain plan actions=1 ${query} SETTINGS optimize_read_in_order=1, allow_experimental_parallel_reading_from_replicas=0" | grep -F 'ReadType: InOrder' | sed 's/^[ \t]*//'

${CLICKHOUSE_CURL} -sS "${CLICKHOUSE_URL}&send_progress_in_http_headers=1&http_headers_progress_interval_ms=0" -d @- <<< "${query} SETTINGS optimize_read_in_order=1, allow_experimental_parallel_reading_from_replicas = 1, max_parallel_replicas = 3, use_hedged_requests=0, cluster_for_parallel_replicas='test_cluster_one_shard_three_replicas_localhost', parallel_replicas_single_task_marks_count_multiplier = 0.001" -v |& grep -e "X-ClickHouse-Summary:" | grep -F -m 1 '"total_rows_to_read":"3000"' | awk -F ',' '{print $5}'
