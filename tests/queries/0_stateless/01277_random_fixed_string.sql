...
SELECT count() = 256 AND min(c) > 30000 FROM (SELECT arrayJoin(reinterpret(randomFixedString(100), 'Array(UInt8)')) AS byte, count() AS c FROM numbers(100000) GROUP BY byte) SETTINGS max_memory_usage = '600Mi', max_block_size = 65409;
