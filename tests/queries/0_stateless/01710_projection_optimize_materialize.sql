...
-- allow_experimental_adaptive_codec_selection=0: this test asserts exact part sizes, which adaptive codec selection would change.
create table z (pk Int64, d Date, id UInt64, c UInt64) Engine MergeTree partition by d order by pk settings ratio_of_defaults_for_sparse_serialization = 1.0, allow_experimental_adaptive_codec_selection = 0;
