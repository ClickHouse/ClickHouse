...
# These scenarios pin the deduplication path exactly: the source and view-level squash thresholds
# and the insert/thread counts decide how many deduplication tokens an insert carries and how they
# merge behind the alias hop. Settings randomization perturbs that (e.g. a different squash
# threshold changes the token identity and with it the deduplication outcome), so it is disabled.
...
# input rows into two 400-row source blocks, each carrying its own token (distinct source numbers
# keep the tokens from merging), and the nested INSERT behind the alias hop squashes them into one block -
# min_insert_block_size_rows_for_materialized_views raises the target-side threshold - so a
# two-token deduplication info rides a drifted view-output block. The first insert carries no
# duplicates and registers both tokens. The second insert repeats the first source block next to a
# fresh one: exactly one of the two tokens collides with the deduplication window, and slicing the
# collided token's rows out of the block is impossible after mv1's GROUP BY collapsed it, so the
# insert is rejected with NOT_IMPLEMENTED - pre-fix this walked the source-row offsets over the
# smaller view-output block: an abort in debug builds, an out-of-bounds read in release builds.
# dst keeps the rows of the first insert only.
...
{ for _ in $(seq 1 4); do seq 1 100; done; for _ in $(seq 1 4); do seq 101 200; done; } | $CLICKHOUSE_CLIENT $SPLIT_SETTINGS -q "INSERT INTO drift_src FORMAT TSV"
$CLICKHOUSE_CLIENT -q "SELECT (SELECT count() FROM drift_src), (SELECT count() FROM drift_inner), (SELECT count() FROM drift_dst)"

{ for _ in $(seq 1 4); do seq 1 100; done; for _ in $(seq 1 4); do seq 201 300; done; } | $CLICKHOUSE_CLIENT $SPLIT_SETTINGS -q "INSERT INTO drift_src FORMAT TSV" 2>&1 | grep -m1 -c "NOT_IMPLEMENTED"
$CLICKHOUSE_CLIENT -q "SELECT count() FROM drift_dst"
