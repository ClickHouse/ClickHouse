#!/usr/bin/env bash
# Tags: no-fasttest, no-random-settings, no-replicated-database, no-flaky-check, no-parallel, no-random-detach
...
{ echo "USE tpch;"; cat "$CURDIR/../../benchmarks/tpc-h/queries/query_15.sql"; } | $CLICKHOUSE_CLIENT "${SETTINGS[@]}"
