...
# Pinned to the query-based implementation, permanently: a UNION whose branches read the same table
...
# apart - the very failure this test guards. That is a limitation of the parallel replicas protocol
# rather than a gap to be closed, so this pin is not a candidate for removal later. The assertions
# above are not pinned and hold for both implementations.
