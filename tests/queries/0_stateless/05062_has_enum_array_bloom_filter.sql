...
-- `hasAny`/`hasAll` cast both arrays to their least supertype, so a constant `Array(Enum)` searched
-- over an indexed `Array(String)` or `Array(FixedString)` is also compared by the name of the enum
-- value, and the index has to hash the names as well.
DROP TABLE IF EXISTS t_has_any_enum_bf;

CREATE TABLE t_has_any_enum_bf
(
    id UInt64,
    s Array(String),
    f Array(FixedString(2)),
    INDEX idx_s s TYPE bloom_filter GRANULARITY 1,
    INDEX idx_f f TYPE bloom_filter GRANULARITY 1
)
ENGINE = MergeTree ORDER BY id SETTINGS index_granularity = 1;

INSERT INTO t_has_any_enum_bf VALUES (1, ['a'], ['a']), (2, ['b'], ['b']), (3, ['1'], ['1']);

SELECT id FROM t_has_any_enum_bf WHERE hasAny(s, CAST(['a'], 'Array(Enum8(\'a\' = 1, \'b\' = 2))')) ORDER BY id;
SELECT id FROM t_has_any_enum_bf WHERE hasAll(s, CAST(['a'], 'Array(Enum8(\'a\' = 1, \'b\' = 2))')) ORDER BY id;
SELECT id FROM t_has_any_enum_bf WHERE hasAny(f, CAST(['a'], 'Array(Enum8(\'a\' = 1, \'b\' = 2))')) ORDER BY id;
