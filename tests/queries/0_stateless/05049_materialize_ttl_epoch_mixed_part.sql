...
-- classify a part as fully expired from those bounds alone - `all_data_dropped` in `TTLTransform` and the
-- `TTLDrop` merge selector - must not fire for it, or the epoch row is deleted without ever being read.
