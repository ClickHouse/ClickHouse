...
#$client -q "create or replace table test_perf (query String, table String, time_ms UInt64, memory_usage UInt64) engine=MergeTree order by tuple()"
...
#          table="gharchive_${part_type}_shared_data_${impl}"
...
#              echo "Run query: $perf_query"
#              query_id="${perf_query}_${query_id_suffix}"
#              $client -m -q "$perf_query; $perf_query; $perf_query; $perf_query; $perf_query;" --query_id="$query_id"
#              $client -q "system flush logs on cluster default"
#              $client -q "insert into test_perf select '$query', '$table', avg(query_duration_ms), avg(memory_usage), $with_cache from (select query_duration_ms, memory_usage from clusterAllReplicas(default, system.query_log) where query_id like '%$perf_query%$query_id_suffix%' and type='QueryFinish' order by event_time_microseconds desc limit 3)"
#          done
#      done
#  done
#
...
for num_paths in 10 100 1000 10000; do
    for part_type in "wide" "compact"; do
        for impl in "json_no_shared_data_map" "json_shared_data_map" "json_shared_data_map_with_buckets" "json_shared_data_advanced"; do
            table="test_${part_type}_${num_paths}_paths_${impl}"
#            for query in "select json.key0" "select json.non_existing_key" "select json.key0, json.key1, json.key2, json.key3, json.key4"; do
            for query in "select json"; do
                perf_query="${query} from $table format Null"
                echo "Run query: $perf_query"
                query_id="${perf_query}_${query_id_suffix}"
                $client -m -q "$perf_query; $perf_query; $perf_query; $perf_query; $perf_query; $perf_query;" --query_id="$query_id"
                $client -q "system flush logs"
                $client -q "insert into test_perf (query, table, time_ms, memory_usage, query_id) select '$query', '$table', avg(query_duration_ms), avg(memory_usage), 4 from (select query_duration_ms, memory_usage from system.query_log where query_id like '%$perf_query%$query_id_suffix%' and type='QueryFinish' order by event_time_microseconds desc limit 5)"
            done
        done
...
#        for query in "select json['key0']" "select json['non_existing_key']" "select json['key0'], json['key1'], json['key2'], json['key3'], json['key4']"; do
        for query in "select json"; do
            ...
            $client -m -q "$perf_query; $perf_query; $perf_query; $perf_query; $perf_query; $perf_query;" --query_id="$query_id"
                $client -q "system flush logs"
            $client -q "insert into test_perf (query, table, time_ms, memory_usage, query_id) select '$(escape_single_quotes $query)', '$table', avg(query_duration_ms), avg(memory_usage), 4 from (select query_duration_ms, memory_usage from system.query_log where query_id like '%$(escape_single_quotes $perf_query)%$query_id_suffix%' and type='QueryFinish' order by event_time_microseconds desc limit 5)"
