...
# in Reader.cpp). It must never turn a readable file into a hard failure: a negative
# `PageEncodingStats.count` (from a buggy writer) should just make that chunk ineligible for the
# optimization - the same as a missing `encoding_stats` or an out-of-range enum value in it - and
# fall back to a full scan, not raise `INCORRECT_DATA`.
...
# ColumnMetaData.encoding_stats is field 13; PageEncodingStats.count is field 3.
...
# Negative `count` on the dictionary page's encoding_stats entry.
craft("negcount_dict", (4, 0, 1, 0, 3, 13, 0, 3))
# Negative `count` on the data page's encoding_stats entry.
craft("negcount_data", (4, 0, 1, 0, 3, 13, 1, 3))
PYEOF
...
for f in negcount_dict negcount_data; do
    echo "-- $f --"
