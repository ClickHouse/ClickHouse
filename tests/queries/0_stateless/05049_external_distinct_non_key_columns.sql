...
-- The final `DISTINCT` of a query with an `ORDER BY` expression runs above the sort, and its header carries
-- the sort column as a non-key, non-constant column, which is written into the spilled runs (the first run,
-- rebuilt from the extracted keys, gets default values for it: its rows are never emitted). The set of
