...
SELECT count() FROM cluster('test_cluster_two_shards', currentDatabase(), t_04652_src) WHERE a IN t_04652_set SETTINGS serialize_query_plan = 1;
SELECT count() FROM cluster('test_cluster_two_shards', currentDatabase(), t_04652_src) WHERE a IN t_04652_set_alias SETTINGS serialize_query_plan = 1;
SELECT count() FROM cluster('test_cluster_two_shards', currentDatabase(), t_04652_src) WHERE a IN t_04652_set_alias SETTINGS serialize_query_plan = 0;
