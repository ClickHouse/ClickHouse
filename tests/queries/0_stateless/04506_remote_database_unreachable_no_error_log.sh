...
#     They are visible in system.tables, so a concurrent query scanning it without
#     a database filter would also try to connect to the unreachable host.

CUR_DIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
# shellcheck source=../shell_config.sh
. "$CUR_DIR"/../shell_config.sh

# A tolerated connection failure to an unreachable remote database must be logged
# at Warning, not Error: Error-level messages from these paths trip the Upgrade
# check on unrelated PRs. Port 1 is privileged and never listens, so connections
# are refused instantly (unlike a blackholed address, which waits for a timeout).

# The tolerated failures are logged at Warning after the fix, and the default
# send_logs_level=warning would stream them to stderr and fail the test.
...
# The no-error assertions are query_id-wide on purpose: no logger on the tolerated path
# may write an Error-level line. The warning-seen assertions pin each downgraded site,
# proving the failure path actually fired (the no-error checks are not vacuous).
${CLICKHOUSE_CLIENT} -q "
