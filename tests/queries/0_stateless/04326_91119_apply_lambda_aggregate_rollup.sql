...
--
-- Under `group_by_use_nulls`, a matcher expanded with `APPLY` or `REPLACE` must behave exactly
-- as it does without the setting. Two invariants are pinned here.
--
-- A reference to a GROUP BY key that sits inside an aggregate function keeps its declared type,
-- while a reference outside one is wrapped into `Nullable`. Both can occur in the same expanded
-- expression, and for `* APPLY x -> agg(x, key)` the aggregate lives in a LAMBDA scope while the
-- keys are registered in the enclosing `QUERY` scope, so the two have to be considered together.
-- Wrapping the inner reference makes the aggregate's per-group input type disagree with the
-- function's signature, which raises an exception or, for aggregates that read their argument
-- through an unchecked cast, faults inside `addBatchSinglePlace`.
--
-- `REPLACE` must reach the other clauses of the query. The rewritten name has to be visible in
-- `WHERE`, `GROUP BY`, `HAVING`, `ORDER BY` and a named `WINDOW`, and must not capture a name
-- that belongs to something else: a lambda argument, a subquery-local column, a column removed
-- by `EXCEPT`, a column hidden from `*` because it is `MATERIALIZED`, or a `WITH FILL` bound.
--
-- Most cases run the same query at `group_by_use_nulls = 0` and at `= 1` and require the two to
-- agree, so they stay meaningful independently of how the setting is implemented.
--
-- The queries and the reference come from https://github.com/ClickHouse/ClickHouse/pull/104350,
-- which proposes a different fix for the same issue.
