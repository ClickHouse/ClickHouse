...
-- `has` applies the zero-padding rule, so the literal 'V0' matches the stored 'V0\0' and only the
-- 'abc' row satisfies `notHas`. The set index compares the padded element, which agrees here, but
-- no set atom must be built: the padding rule is not injective, so a set cannot represent it.
