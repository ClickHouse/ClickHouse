...
-- The dynamic filter shares the PREWHERE with the WHERE below, so the printed filter column
-- depends on that promotion. Either of these off leaves the predicate above the read.
SET optimize_move_to_prewhere = 1;
SET query_plan_optimize_prewhere = 1;
SET query_plan_remove_unused_columns = 1; -- keeps the printed action positions stable
