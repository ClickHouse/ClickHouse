...
-- a logical error in CollectSourceColumnsVisitor because a cross join
...
-- resolves a USING column from the SELECT projection with a cross join as
-- left table expression, assigning the join node as the column source.
...
-- assigning the join node as the column source from the projection alias.
SELECT 1 AS c0
