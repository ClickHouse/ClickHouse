...
WHERE query_id in (select query_id from system.query_log where current_database = currentDatabase() AND is_initial_query AND log_comment='02898_default_190aed82-2423-413b-ad4c-24dcca50f65b' and event_date >= yesterday() AND event_time >= now() - 600)
...
SYSTEM FLUSH LOGS text_log, query_log;
SELECT count() > 0 FROM system.text_log
WHERE query_id in (select query_id from system.query_log where current_database = currentDatabase() AND is_initial_query AND log_comment='02898_inorder_190aed82-2423-413b-ad4c-24dcca50f65b' and event_date >= yesterday() AND event_time >= now() - 600)
