...
select cast(1, 'UInt8') from (select arrayJoin([1, 2]) as a) t1 left join (select 1 as b) t2 on b = ignore('UInt8') SETTINGS enable_analyzer = 1, query_plan_lift_up_array_join = 0, query_plan_merge_filter_into_join_condition = 0;
...
select isConstant('UInt8'), toFixedString('hello', toUInt8(substring('UInt8', 5, 1))) from (select arrayJoin([1, 2]) as a) t1 left join (select 1 as b) t2 on b = ignore('UInt8') SETTINGS enable_analyzer = 1, query_plan_lift_up_array_join = 0, query_plan_merge_filter_into_join_condition = 0;
