-- A merge drops a column whose every value has expired by its TTL without reading it. A projection or a skip index
-- that depends on a subcolumn of such a column (here the typed path `props.a` of a `JSON` column) must not be taken
-- from the source parts, which describe the values before they expired: readers see the default value of the column.
...
(
    ...
    PROJECTION p_a (SELECT props.a, count() GROUP BY props.a)
)
...
SELECT count(), countIf(s = ''), countIf(props.a = 0) FROM t_ttl_subcolumn_deps SETTINGS optimize_use_projections = 0, use_skip_indexes = 0;
SELECT count() FROM t_ttl_subcolumn_deps WHERE props.a = 0 SETTINGS optimize_use_projections = 0, use_skip_indexes = 1;
SELECT count() FROM t_ttl_subcolumn_deps WHERE key < 1000 AND s = '' SETTINGS optimize_use_projections = 0, use_skip_indexes = 1;
SELECT props.a, count() FROM t_ttl_subcolumn_deps GROUP BY props.a ORDER BY props.a SETTINGS optimize_use_projections = 1, force_optimize_projection = 1;
...
SELECT count(), countIf(s = ''), countIf(props.a = 0) FROM t_ttl_subcolumn_deps_multi SETTINGS optimize_use_projections = 0, use_skip_indexes = 0;
SELECT count() FROM t_ttl_subcolumn_deps_multi WHERE props.a = 0 SETTINGS optimize_use_projections = 0, use_skip_indexes = 1;
SELECT count() FROM t_ttl_subcolumn_deps_multi WHERE key < 1000 AND s = '' SETTINGS optimize_use_projections = 0, use_skip_indexes = 1;
SELECT props.a, count() FROM t_ttl_subcolumn_deps_multi GROUP BY props.a ORDER BY props.a SETTINGS optimize_use_projections = 1, force_optimize_projection = 1;
