...
url_wait="${CLICKHOUSE_URL}&async_insert=1&wait_for_async_insert=1"
url_no_wait="${CLICKHOUSE_URL}&async_insert=1&wait_for_async_insert=1"
...
${CLICKHOUSE_CLIENT} -q "SELECT query, Settings['async_insert'], Settings['wait_for_async_insert'] FROM system.query_log \
