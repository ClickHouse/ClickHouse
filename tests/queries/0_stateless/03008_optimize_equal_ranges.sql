...
WHERE event_date >= yesterday() AND event_time >= now() - 600 AND type = 'QueryFinish' AND current_database = currentDatabase() AND is_initial_query AND query LIKE '%SELECT%FROM%t_optimize_equal_ranges%'
