...
echo "-- path file extension wins over default_format (path .CSV, default_format=JSONEachRow):"
http_get "${BASE_URL}/${DB}/hits.CSV?default_format=JSONEachRow"
echo "-- lowercase path file extension is recognized too (path .csv):"
http_get "${BASE_URL}/${DB}/hits.csv"
echo "-- X-ClickHouse-Format is an explicit override: it wins over the query FORMAT clause:"
