...
WHERE event_date >= yesterday() AND event_time >= now() - 600 AND current_database = currentDatabase() AND query LIKE 'OPTIMIZE TABLE t_dry_run_cleanup DRY RUN PARTS%' AND type = 'QueryFinish'
