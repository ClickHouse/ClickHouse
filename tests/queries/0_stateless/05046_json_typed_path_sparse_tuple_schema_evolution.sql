DROP TABLE IF EXISTS json_sparse_tuple_evolution;

CREATE TABLE json_sparse_tuple_evolution
(
...
INSERT INTO json_sparse_tuple_evolution
SELECT CAST(
...
ALTER TABLE json_sparse_tuple_evolution
...
(
    ...
    WHERE active
        ...
        AND table = 'json_sparse_tuple_evolution'
        AND column = 'j'
)
...
ALTER TABLE json_sparse_tuple_evolution
    MODIFY COLUMN j JSON(t Tuple(a Nullable(String)), max_dynamic_paths = 0)
    SETTINGS mutations_sync = 2;
...
WHERE tupleElement(path, 1) = 't.a';

DROP TABLE json_sparse_tuple_evolution;
