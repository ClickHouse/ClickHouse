-- `x = a OR x = b OR x = c` is rewritten into `x IN (a, b, c)`. Decimal, exponent and wide-integer
-- literals are now deferred (NumberLiteral) which has no ordering of its own, so the rewrite must
-- not sort raw NumberLiterals (Field::operator< on NumberLiteral used to throw). It orders by the
-- resolved value and keeps the exact literal, so the resulting IN is still correct.
...
SELECT count() FROM t_or_in WHERE x = 1.5 OR x = 2.5 OR x = 3.5;
SELECT count() FROM t_or_in WHERE x = 1.5e0 OR x = 25e-1 OR x = 0.35e1;
SELECT count() FROM t_or_in WHERE x = 1.5 OR x = 2.5 OR x = 9.5;
DROP TABLE t_or_in;
