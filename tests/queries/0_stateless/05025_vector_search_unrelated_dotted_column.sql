...
CREATE TABLE t_05025 (id UInt32, vec Array(Float32), `n.vec` Array(Float32),
    INDEX idx vec TYPE vector_similarity('hnsw', 'cosineDistance', 2))
ENGINE = MergeTree ORDER BY id SETTINGS index_granularity = 4;

INSERT INTO t_05025 SELECT number, [toFloat32(number), toFloat32(number + 1)], [toFloat32(number)] FROM numbers(64);

SELECT 'no reference to the dotted column (control): the vector column is replaced';
SELECT count() > 0 FROM (
    EXPLAIN actions = 1
    SELECT id FROM t_05025 ORDER BY cosineDistance(vec, [0., 1.]) LIMIT 1
        SETTINGS vector_search_with_rescoring = 0)
WHERE explain LIKE '%_distance%';

SELECT 'projection on the unrelated dotted column: correct result';
SELECT id, length(`n.vec`) FROM t_05025 ORDER BY cosineDistance(vec, [0., 1.]) LIMIT 1
    SETTINGS vector_search_with_rescoring = 0;
...
SELECT id FROM t_05025 WHERE length(`n.vec`) = 1 ORDER BY cosineDistance(vec, [0., 1.]) LIMIT 1
    SETTINGS vector_search_with_rescoring = 0;
...
SELECT count() = 0 FROM (
    EXPLAIN actions = 1
    SELECT id, length(vec) FROM t_05025 ORDER BY cosineDistance(vec, [0., 1.]) LIMIT 1
        SETTINGS vector_search_with_rescoring = 0)
WHERE explain LIKE '%_distance%';

DROP ROW POLICY IF EXISTS policy_05025 ON t_05025;
CREATE ROW POLICY policy_05025 ON t_05025 USING length(`n.vec`) = 1 TO CURRENT_USER;
...
DROP ROW POLICY policy_05025 ON t_05025;
DROP TABLE t_05025;
