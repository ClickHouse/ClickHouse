...
# Tag no-parallel: `system drop replica 'zookeeper2:doesntexist...'` (no `FROM DATABASE`/`FROM TABLE`)
#  iterates every `ReplicatedMergeTree` table on the server; a concurrent test dropping/detaching its
#  own replicated table mid-iteration could make this global scan throw unrelated to this test's logic.
