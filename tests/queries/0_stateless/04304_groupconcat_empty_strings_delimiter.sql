-- Empty strings must still get delimiters in groupConcat (https://github.com/ClickHouse/ClickHouse/issues/104507).

-- add(): single aggregation state, deterministic order via arrayJoin.
SELECT groupConcat(',')(arrayJoin(['', '', 'a']));
SELECT groupConcat(',')(arrayJoin(['', '', '']));
SELECT groupConcat(',')(arrayJoin(['', 'a', '']));

-- Same with an explicit limit (has_limit add path).
SELECT groupConcat(',', 3)(arrayJoin(['', '', 'a']));

-- merge() paths: length is order-independent and equals (count - 1) * delimiter size.
