...
-- Results on an integer column stay correct in both analyzers (the Analyzer still
-- folds such atoms, see the `folding shapes` section).
SELECT 'int column';
...
-- same atom for a Float column: a NaN row satisfies the constraint but not x >= c.
...
-- Pin the type-aware folding shapes in the Analyzer: Int and LowCardinality(String)
-- comparisons fold into the opposite comparison, a Float comparison keeps the
-- explicit `not`.
