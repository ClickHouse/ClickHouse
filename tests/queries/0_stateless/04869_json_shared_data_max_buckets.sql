...
-- p0, p1, p3, p4, p6, p8, p12, p20 and p22 land in buckets 128..255, the other paths in 0..127.
INSERT INTO t_json_max_buckets_wide
SELECT number, concat('{', arrayStringConcat(arrayMap(j -> concat('"p', toString(j), '":', toString(number * 100 + j)), range(24)), ','), '}')
FROM numbers(100);
...
SELECT number, arrayMap(k -> CAST(concat('{', arrayStringConcat(arrayMap(j -> concat('"p', toString(j), '":', toString(number * 100 + j + k)), range(24)), ','), '}'), 'JSON(max_dynamic_paths = 0)'), range(3))
FROM numbers(50);

SELECT sum(json.p0.:Int64), sum(json.p1.:Int64), sum(json.p3.:Int64), sum(json.p4.:Int64), sum(json.p6.:Int64),
       sum(json.p8.:Int64), sum(json.p12.:Int64), sum(json.p20.:Int64), sum(json.p22.:Int64),
...
SELECT sum(arraySum(x -> assumeNotNull(x), arr.p0.:Int64)), sum(arraySum(x -> assumeNotNull(x), arr.p1.:Int64)),
       sum(arraySum(x -> assumeNotNull(x), arr.p22.:Int64)), sum(arraySum(x -> assumeNotNull(x), arr.p7.:Int64))
