...
-- Basic: single text-index function on 'text'
SELECT count() FROM tab PREWHERE hasToken(text, 'clickhouse');

-- Basic: hasAnyTokens on 'text'
SELECT count() FROM tab PREWHERE hasAnyTokens(text, ['clickhouse', 'database']);

-- Basic: hasAllTokens on 'text'
SELECT count() FROM tab PREWHERE hasAllTokens(text, ['column', 'store']);

-- Basic: text-index functions on 'text2'
SELECT count() FROM tab PREWHERE hasToken(text2, 'fastio');

SELECT count() FROM tab PREWHERE hasAnyTokens(text2, ['search', 'index']);

SELECT count() FROM tab PREWHERE hasAllTokens(text2, ['merge', 'tree']);

-- Text condition + numeric condition (AND)
SELECT count() FROM tab PREWHERE hasToken(text, 'clickhouse') AND a > 0;

SELECT count() FROM tab PREWHERE hasAnyTokens(text, ['clickhouse', 'database']) AND id BETWEEN 10 AND 100;

SELECT count() FROM tab PREWHERE hasAllTokens(text, ['column', 'store']) AND (a % 2) = 0;

-- Text condition + numeric condition with OR and parentheses
SELECT count() FROM tab PREWHERE hasToken(text, 'analytics') OR a < 0;

SELECT count() FROM tab PREWHERE (hasAnyTokens(text, ['log', 'event']) OR hasToken(text2, 'error')) AND id > 0;

SELECT count() FROM tab PREWHERE (a > 100 AND hasToken(text, 'hot')) OR (a <= 100 AND hasAnyTokens(text2, ['cold', 'warm']));

-- Multiple text functions on the same column
SELECT count() FROM tab PREWHERE hasToken(text, 'clickhouse') AND hasAnyTokens(text, ['clickhouse', 'database']);

SELECT count() FROM tab PREWHERE hasAnyTokens(text, ['clickhouse', 'database']) AND NOT hasToken(text, 'mysql');

SELECT count() FROM tab PREWHERE hasAllTokens(text, ['column', 'store']) OR hasAnyTokens(text, ['olap', 'analytics']);

-- Cross-column combinations (text + text2)
SELECT count() FROM tab PREWHERE hasToken(text, 'clickhouse') AND hasToken(text2, 'fastio');

SELECT count() FROM tab PREWHERE hasAnyTokens(text, ['search', 'query']) OR hasAllTokens(text2, ['index', 'optimize']);

SELECT count() FROM tab PREWHERE (hasToken(text, 'user') AND hasAnyTokens(text2, ['login', 'logout']))
    OR (a >= 0 AND a <= 10);

-- Combinations using NOT
SELECT count() FROM tab PREWHERE NOT hasToken(text, 'debug') AND hasAnyTokens(text2, ['info', 'warn', 'error']);

SELECT count() FROM tab PREWHERE NOT hasAllTokens(text, ['internal', 'test']) AND a > 0 AND hasToken(text2, 'prod');

SELECT count() FROM tab PREWHERE NOT (hasAnyTokens(text, ['drop', 'truncate']) OR hasToken(text2, 'danger')) AND id IN (1, 2, 3, 4);

-- Text functions mixed into complex expressions
SELECT count() FROM tab PREWHERE (a > 10 AND hasToken(text, 'clickhouse')) OR (a <= 10 AND hasAnyTokens(text2, ['mysql', 'postgres']));

SELECT count() FROM tab PREWHERE (hasAllTokens(text, ['column', 'store']) AND a BETWEEN 1 AND 100) OR (hasAnyTokens(text2, ['lake', 'warehouse']) AND id > 1000);

SELECT count() FROM tab PREWHERE (NOT hasToken(text, 'archived') AND a > 0) OR (hasAnyTokens(text2, ['active', 'online']) AND id % 5 = 0);

--  Text index filter in PREWHERE, numeric filter in WHERE
SELECT count() FROM tab PREWHERE hasToken(text, 'clickhouse') WHERE a > 0;

-- PREWHERE uses hasAllTokens, WHERE excludes some special cases
SELECT count() FROM tab PREWHERE hasAllTokens(text, ['column', 'store']) WHERE a NOT IN (150, 160) AND id != 15;
