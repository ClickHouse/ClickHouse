...
CREATE TABLE t_index (data JSON(a String)) ENGINE = MergeTree ORDER BY tuple();
INSERT INTO t_index (data) VALUES ('{"a": "aaa", "b": "bbb"}');
SET mutations_sync = 2;

ALTER TABLE t_index ADD INDEX a_idx(data.a) TYPE text(tokenizer = splitByNonAlpha);
ALTER TABLE t_index MATERIALIZE INDEX a_idx;

ALTER TABLE t_index ADD INDEX b_idx(data.b::String) TYPE text(tokenizer = splitByNonAlpha);
...
SELECT count() FROM t_index WHERE data.a = 'aaa' SETTINGS force_data_skipping_indices = 'a_idx';
SELECT count() FROM t_index WHERE data.b::String = 'bbb' SETTINGS force_data_skipping_indices = 'b_idx';
...
ALTER TABLE t_index ADD COLUMN data JSON(a String);
