#!/usr/bin/env bash
# Tags: no-fasttest, no-random-settings, no-replicated-database, no-flaky-check, no-msan
...
{ echo "USE tpcds;"; cat "$CURDIR/../../benchmarks/tpc-ds/queries/query_23.sql"; } | $CLICKHOUSE_CLIENT "${SETTINGS[@]}"
