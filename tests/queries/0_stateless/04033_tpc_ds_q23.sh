#!/usr/bin/env bash
# Tags: no-fasttest, no-random-settings, no-replicated-database, long
...
# long: query exceeds the 180s limit under sanitizers in flaky check mode.
...
{ echo "USE datasets;"; cat "$CURDIR/../../benchmarks/tpc-ds/queries/query_23.sql"; } | $CLICKHOUSE_CLIENT "${SETTINGS[@]}"
