...
insert into function file(currentDatabase() || '_02322_data.sql', 'SQLInsert') select number as x, number % 3 as y, 'Hello' as z from numbers(5) settings output_format_sql_insert_max_batch_size=2, output_format_sql_insert_quote_names=0, engine_file_truncate_on_insert=1;
select * from file(currentDatabase() || '_02322_data.sql', 'MySQLDump');
insert into function file(currentDatabase() || '_02322_data.sql', 'SQLInsert') select number, number % 3, 'Hello' from numbers(5) settings output_format_sql_insert_max_batch_size=2, engine_file_truncate_on_insert=1;
select * from file(currentDatabase() || '_02322_data.sql', 'MySQLDump');
