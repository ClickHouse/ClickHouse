...
CREATE TABLE view_lookup_missing_dict_ref (id UInt64, name String) ENGINE = MergeTree ORDER BY id;
INSERT INTO view_lookup_missing_dict_ref VALUES (4242, 'match');

CREATE DICTIONARY view_lookup_missing_dict (id UInt64, name String)
PRIMARY KEY id SOURCE(CLICKHOUSE(TABLE 'view_lookup_missing_dict_ref')) LAYOUT(HASHED()) LIFETIME(0);

CREATE VIEW view_lookup_missing_dict_v AS
SELECT id, dictGetString('view_lookup_missing_dict', 'name', id) AS name FROM view_lookup_data;

DROP DICTIONARY view_lookup_missing_dict;

SET optimize_inverse_dictionary_lookup = 1;
SELECT count() FROM view_lookup_missing_dict_v WHERE name = 'match'; -- { serverError BAD_ARGUMENTS }

SET optimize_inverse_dictionary_lookup = 0;
SELECT count() FROM view_lookup_missing_dict_v WHERE name = 'match'; -- { serverError BAD_ARGUMENTS }

DROP VIEW view_lookup_missing_dict_v;
DROP TABLE view_lookup_missing_dict_ref;
