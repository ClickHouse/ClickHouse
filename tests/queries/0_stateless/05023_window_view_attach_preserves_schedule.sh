...
${CLICKHOUSE_CLIENT} --allow_experimental_window_view 1 --allow_experimental_analyzer 0 -q "DROP TABLE IF EXISTS ${VIEW}"
...
${CLICKHOUSE_CLIENT} --allow_experimental_window_view 1 --allow_experimental_analyzer 0 -q "DETACH TABLE ${VIEW}"
...
${CLICKHOUSE_CLIENT} --allow_experimental_window_view 1 --allow_experimental_analyzer 0 -q "ATTACH TABLE ${VIEW}"
${CLICKHOUSE_CLIENT} -q "INSERT INTO ${SOURCE} VALUES (1)"

for _ in {1..100}; do
    count=$(${CLICKHOUSE_CLIENT} -q "SELECT count() FROM ${VIEW}")
    if [ "${count}" = "1" ]; then
        break
    fi
    sleep 0.1
...
${CLICKHOUSE_CLIENT} -q "SELECT count() FROM ${VIEW}"

${CLICKHOUSE_CLIENT} --allow_experimental_window_view 1 --allow_experimental_analyzer 0 -q "DROP TABLE ${VIEW}"
