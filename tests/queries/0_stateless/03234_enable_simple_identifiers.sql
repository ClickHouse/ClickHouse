...
SETTINGS
    enable_simple_identifiers=true; -- { serverError BAD_ARGUMENTS }
DROP TABLE IF EXISTS `test_foo_#`;
...
COMMENT 'test' -- to end ENGINE definition, so SETTINGS will be in the query level
SETTINGS
    enable_simple_identifiers=true;
...
-- CREATE TABLE without `enable_simple_identifiers`
DROP TABLE IF EXISTS test_foo;
CREATE TABLE `test_foo` (
    `insecure_$` Int8,
    `date` Date,
...
PRIMARY KEY (town, date)
PARTITION BY toYear(date);
-- Then SHOW CREATE .. with `enable_simple_identifiers`
...
-- SHOW CREATE .. query contains an insecure identifier (`test_foo$`) with `enable_simple_identifiers`
SHOW CREATE TABLE `test_foo$`
SETTINGS
    enable_simple_identifiers=true; -- { serverError BAD_ARGUMENTS }
