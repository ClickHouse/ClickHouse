...
set -e
CLICKHOUSE_CLIENT="$CLICKHOUSE_CLIENT --allow_experimental_time_series_table=1 --max_threads=1 --max_block_size=8192 --prefer_localhost_replica=0 --analyzer_inline_views=1 --optimize_trivial_view_pushdown_to_distributed=1"
...
for shards in 1 2; do
    ...
    if (( shards == 2 )); then cluster=test_cluster_two_shards_localhost; fi
    $CLICKHOUSE_CLIENT --multiquery <<SQL
SELECT '-- ${shards} shard(s)';
CREATE TABLE ts_stream_dist AS ts_stream_remote_data
ENGINE = Distributed(${cluster}, currentDatabase(), ts_stream_remote_data);
...
$CLICKHOUSE_CLIENT --query "DROP TABLE ts_stream_remote_data"
