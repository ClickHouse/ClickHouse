...
----------------------------------------------------
SELECT 'Test text(tokenizer = "default")';

DROP TABLE IF EXISTS tab_x;

CREATE TABLE tab_x(k UInt64, s String, INDEX af(s) TYPE text(tokenizer = 'default'))
    ENGINE = MergeTree() ORDER BY k
<<<<<<< HEAD:tests/queries/0_stateless/02346_gin_index_queries.sql
    SETTINGS index_granularity = 2, index_granularity_bytes = '10Mi';
||||||| dd26a887163:tests/queries/0_stateless/02346_gin_index_queries.sql
    SETTINGS index_granularity = 2, index_granularity_bytes = '10Mi',
...
>>>>>>> master:tests/queries/0_stateless/02346_text_index_queries.sql
...
-- search text index with mapContains
...
-- search text index with map key
...
----------------------------------------------------
SELECT 'Test text(tokenizer = "ngram", ngram_size = 2) on UTF-8 data';
