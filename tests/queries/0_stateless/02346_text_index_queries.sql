...
----------------------------------------------------
SELECT 'Test text(tokenizer = "sparse_gram", min_length = 2,max_length = 100, min_cutoff_length=40) on UTF-8 data';

DROP TABLE IF EXISTS tab;

CREATE TABLE tab(k UInt64, s String, INDEX af(s) TYPE text(tokenizer = 'sparse_gram', min_length = 2, max_length = 100, min_cutoff_length=40) GRANULARITY 1)
    ENGINE = MergeTree()
    ORDER BY k
    SETTINGS index_granularity = 2, index_granularity_bytes = '10Mi';
