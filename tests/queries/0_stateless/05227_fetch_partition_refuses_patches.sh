...
#
# The source partition is pinned to two base parts with merges stopped: the update touches the older
# part only, and the later insert carries a higher data version than the update's patch. A guard that
# compared the patch against the newest part would let it pass; the check has to use the oldest one,
# as `assertNoPatchesForParts` does.
...
SYSTEM STOP MERGES t_fetch_patches_src;

INSERT INTO t_fetch_patches_src SELECT number, 0 FROM numbers(500);
"

${CLICKHOUSE_CLIENT} --enable_lightweight_update 1 -q "UPDATE t_fetch_patches_src SET v = 42 WHERE id < 500"

# A part inserted after the update: its data version is above the patch's, so it hides the patch
# from any check that looks at the newest part of the partition only.
${CLICKHOUSE_CLIENT} -q "INSERT INTO t_fetch_patches_src SELECT 500 + number, 0 FROM numbers(500)"

${CLICKHOUSE_CLIENT} -q "
SELECT 'the update is acknowledged on the source', count(), sum(v) FROM t_fetch_patches_src;
SELECT 'the partition has two base parts', count() FROM system.parts
...
new_part=$(${CLICKHOUSE_CLIENT} -q "
    SELECT name FROM system.parts
    WHERE database = currentDatabase() AND table = 't_fetch_patches_src' AND active AND partition_id = '0'
    ORDER BY min_block_number DESC LIMIT 1")

echo -n 'and so is the fetch of the patched part: '
${CLICKHOUSE_CLIENT} -q "ALTER TABLE t_fetch_patches_dst FETCH PART '$old_part' FROM '$zk_prefix/t_fetch_patches_src'" 2>&1 |
    grep -c -m1 'APPLY PATCHES'

# The part inserted after the update is not patched by it - the patch lies below its data version -
# so fetching that part alone is faithful and stays allowed.
${CLICKHOUSE_CLIENT} -q "
ALTER TABLE t_fetch_patches_dst FETCH PART '$new_part' FROM '$zk_prefix/t_fetch_patches_src';
ALTER TABLE t_fetch_patches_dst ATTACH PART '$new_part';
SELECT 'the fetch of the unpatched part is allowed', count(), sum(v) FROM t_fetch_patches_dst;
ALTER TABLE t_fetch_patches_dst DROP PARTITION 0;
"

# Once the patches are applied, the base parts hold the update and the fetch produces a faithful copy.
${CLICKHOUSE_CLIENT} -q "SYSTEM START MERGES t_fetch_patches_src"
