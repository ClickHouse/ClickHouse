...
# Two endpoints are the supported Student's T-test comparison case. Keep asserting that
# comparison mode stays enabled there.
two_endpoint_output=$($CLICKHOUSE_BENCHMARK \
    --host "$CLICKHOUSE_HOST" --host "$CLICKHOUSE_HOST" \
    --port "$CLICKHOUSE_PORT_TCP" \
    --concurrency 1 --iterations 100 --query 'SELECT 1' 2>&1) \
    || { echo FAIL; exit 1; }

grep -q '% confidence' <<< "$two_endpoint_output" || { echo FAIL; exit 1; }
echo OK
...
# endpoints.
three_endpoint_output=$($CLICKHOUSE_BENCHMARK \
    --host "$CLICKHOUSE_HOST" --host "$CLICKHOUSE_HOST" --host "$CLICKHOUSE_HOST" \
    --port "$CLICKHOUSE_PORT_TCP" --port "$CLICKHOUSE_PORT_TCP" \
    --concurrency 1 --iterations 100 --query 'SELECT 1' 2>&1) \
    || { echo FAIL; exit 1; }

grep -q '% confidence' <<< "$three_endpoint_output" && { echo FAIL; exit 1; }
echo OK

single_query_three_endpoint_output=$($CLICKHOUSE_BENCHMARK \
    --host "$CLICKHOUSE_HOST" --host "$CLICKHOUSE_HOST" --host "$CLICKHOUSE_HOST" \
    --port "$CLICKHOUSE_PORT_TCP" --port "$CLICKHOUSE_PORT_TCP" \
    --concurrency 1 --iterations 1 --query 'SELECT 1' 2>&1)

grep -q 'queries: 1,' <<< "$single_query_three_endpoint_output" || { echo FAIL; exit 1; }
echo OK
