...
SELECT count() > 0 FROM (SELECT * FROM generateRandom('x BFloat16', 42, 10, 5) LIMIT 10);

-- Time: stored as Int32.
SELECT count() > 0 FROM (SELECT * FROM generateRandom('x Time', 42, 10, 5) LIMIT 10);

-- Time64: stored as Decimal64.
SELECT count() > 0 FROM (SELECT * FROM generateRandom('x Time64(3)', 42, 10, 5) LIMIT 10);

-- Dynamic: values from a pool of scalar types.
SELECT count() > 0 FROM (SELECT * FROM generateRandom('x Dynamic', 42, 10, 5) LIMIT 10);

-- Variant: random selection among declared variant types per row.
SELECT count() > 0 FROM (SELECT * FROM generateRandom('x Variant(UInt64, String)', 42, 10, 5) LIMIT 10);

-- Variant with a single type.
SELECT count() > 0 FROM (SELECT * FROM generateRandom('x Variant(Int32)', 42, 10, 5) LIMIT 10);

-- JSON without typed paths (only dynamic paths generated).
SELECT count() > 0 FROM (SELECT * FROM generateRandom('x JSON', 42, 10, 5) LIMIT 10);

-- JSON with typed paths: typed paths are filled recursively.
SELECT count() > 0 FROM (SELECT * FROM generateRandom('x JSON(a UInt32, b String)', 42, 10, 5) LIMIT 10);
...
SELECT count() > 0 FROM (SELECT * FROM generateRandom('x Array(JSON)', 42, 5, 3) LIMIT 5);
...
SELECT count() > 0 FROM (SELECT * FROM t_gen_json LIMIT 1);
...
SELECT count() > 0 FROM (SELECT * FROM t_gen_dynamic LIMIT 1);
...
SELECT count() > 0 FROM (SELECT * FROM t_gen_variant LIMIT 1);
...
SELECT count() > 0 FROM (SELECT * FROM generateRandom('x Array(UInt32)', 42, 10, 20) LIMIT 1);

-- Verify generate_random_max_json_dynamic_keys setting: 0 means no dynamic paths.
SELECT count() > 0 FROM (SELECT * FROM generateRandom('x JSON', 42, 10, 5) LIMIT 5) SETTINGS generate_random_max_json_dynamic_keys = 0;
