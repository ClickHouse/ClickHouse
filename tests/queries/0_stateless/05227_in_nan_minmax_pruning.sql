...
-- rows into more than the 100 granules that `index_granularity = 1000` alone gives.
CREATE TABLE t_no_nan (k UInt64, f Float64, INDEX mm f TYPE minmax GRANULARITY 1) ENGINE = MergeTree ORDER BY k SETTINGS index_granularity = 1000, index_granularity_bytes = 0;
...
CREATE TABLE t_int_key (k UInt64, INDEX mm k TYPE minmax GRANULARITY 1) ENGINE = MergeTree ORDER BY k SETTINGS index_granularity = 1000, index_granularity_bytes = 0;
