...
for i in {1..20}
do
	echo $i, $i >> ${CLICKHOUSE_USER_FILES_UNIQUE}/${CLICKHOUSE_TEST_UNIQUE_NAME}/a.txt
done
...
${CLICKHOUSE_CLIENT} --query "create table file_log(k UInt8, v UInt8) engine=FileLog('${CLICKHOUSE_USER_FILES_UNIQUE}/${CLICKHOUSE_TEST_UNIQUE_NAME}/', 'CSV') settings poll_directory_watch_events_backoff_max = 1000;"
...
cp ${CLICKHOUSE_USER_FILES_UNIQUE}/${CLICKHOUSE_TEST_UNIQUE_NAME}/a.txt ${CLICKHOUSE_USER_FILES_UNIQUE}/${CLICKHOUSE_TEST_UNIQUE_NAME}/b.txt

# touch does not change file content, no event
touch ${CLICKHOUSE_USER_FILES_UNIQUE}/${CLICKHOUSE_TEST_UNIQUE_NAME}/a.txt

cp ${CLICKHOUSE_USER_FILES_UNIQUE}/${CLICKHOUSE_TEST_UNIQUE_NAME}/a.txt ${CLICKHOUSE_USER_FILES_UNIQUE}/${CLICKHOUSE_TEST_UNIQUE_NAME}/c.txt
cp ${CLICKHOUSE_USER_FILES_UNIQUE}/${CLICKHOUSE_TEST_UNIQUE_NAME}/a.txt ${CLICKHOUSE_USER_FILES_UNIQUE}/${CLICKHOUSE_TEST_UNIQUE_NAME}/d.txt

for i in {100..120}
do
	echo $i, $i >> ${CLICKHOUSE_USER_FILES_UNIQUE}/${CLICKHOUSE_TEST_UNIQUE_NAME}/d.txt
done
