...
SELECT 'Ok' FROM system.text_log WHERE event_date >= yesterday() AND event_time >= now() - 600 AND message LIKE '%(query 1, line 4)%' AND message LIKE '%This is the first query%' LIMIT 1 SETTINGS max_rows_to_read = 0;
SELECT 'Ok' FROM system.text_log WHERE event_date >= yesterday() AND event_time >= now() - 600 AND message LIKE '%(query 2, line 8)%' AND message LIKE '%This is the second query%' LIMIT 1 SETTINGS max_rows_to_read = 0;
