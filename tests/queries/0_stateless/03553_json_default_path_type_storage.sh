...
for type in Int64 String 'Nullable(Int64)'; do
    for version in map map_with_buckets advanced; do
        for layout in compact wide compact_without_substreams; do
            ...
            if [ "$type" = 'Nullable(Int64)' ]; then
                # Nullable defaults remain missing even after shared paths are flattened.
                # Ordinary MergeTree keeps duplicate ids (id=3 and id=4 each appear twice), so a
                # JOIN on id would produce cross-matches. Compare row-by-row after sorting instead.
                $CLICKHOUSE_CLIENT --multiquery --query "
                    INSERT INTO dpt_storage VALUES (3, '{\"a\":0}'), (4, '{}');
                    TRUNCATE TABLE dpt_expected;
                    INSERT INTO dpt_expected SELECT id, j, sipHash64(j) FROM dpt_storage;
                    OPTIMIZE TABLE dpt_storage FINAL;
                    SELECT throwIf(count() != 0, 'nullable merge changed values') FROM (
                        SELECT id, j, sipHash64(j) FROM dpt_storage ORDER BY id, j
                        EXCEPT ALL
