...
CREATE TABLE tab(k UInt64, s String, PROJECTION af INDEX s TYPE text(tokenizer = ngrams(2)))
...
CREATE TABLE tab_x(k UInt64, s String, PROJECTION af INDEX s TYPE text(tokenizer = 'splitByNonAlpha'))
    ENGINE = MergeTree() ORDER BY k
    SETTINGS index_granularity = 2, index_granularity_bytes = '10Mi';
...
CREATE TABLE tab(k UInt64, s String, PROJECTION af INDEX s TYPE text(tokenizer = sparseGrams(3, 100)))
    ENGINE = MergeTree()
    ORDER BY k
    SETTINGS index_granularity = 2, index_granularity_bytes = '10Mi';
...
CREATE TABLE tab(k UInt64, s String, PROJECTION af INDEX s TYPE text(tokenizer = sparseGrams(3, 100, 4)))
    ENGINE = MergeTree()
    ORDER BY k
    SETTINGS index_granularity = 2, index_granularity_bytes = '10Mi';
