...
# wait for the destination database to finish loading. `ATTACH DATABASE` publishes the database in
# the catalog before its tables are loaded, so a preflight that does not wait can see a partial
# table list, let the operation past the point of no return, and only then fail with
# `TOO_MANY_TABLES` -- losing the source table or the dropped-table record.
...
# This table is dropped asynchronously, so it stays in the dropped-table queue for `UNDROP`.
${CLIENT} -q "CREATE TABLE ${FULL_DB}.undrop_me (x UInt32) ENGINE = MergeTree ORDER BY x"
...
${CLIENT} -q "ALTER DATABASE ${FULL_DB} MODIFY SETTING max_tables = 50"
${CLIENT} -q "CREATE TABLE ${ORDINARY_DB}.src (x UInt32) ENGINE = Null"
...
for _ in {1..30}; do
    ${CLIENT} -q "RENAME TABLE ${ORDINARY_DB}.src TO ${FULL_DB}.src" 2>/dev/null
    ${CLIENT} -q "UNDROP TABLE ${FULL_DB}.undrop_me" 2>/dev/null
done
