...
# A one-partition non-transactional `OPTIMIZE FINAL` runs synchronously. It must nevertheless
# reserve a merge-executor slot, so that it cannot bypass the merge worker budget or begin work
# during executor shutdown. Pause its projection merge and verify that the executor task metric
# rises together with the visible merge. Before the fix the merge appeared in `system.merges`, but
# the task metric remained unchanged.

$CLICKHOUSE_CLIENT --query "DROP TABLE IF EXISTS t_optimize_single_partition_slot SYNC"
$CLICKHOUSE_CLIENT --query "
    CREATE TABLE t_optimize_single_partition_slot
    (
        k UInt64,
        v UInt64,
...
$CLICKHOUSE_CLIENT --query "INSERT INTO t_optimize_single_partition_slot SELECT number, number FROM numbers(1000)"
$CLICKHOUSE_CLIENT --query "INSERT INTO t_optimize_single_partition_slot SELECT number + 1000, number FROM numbers(1000)"

cleanup() {
    $CLICKHOUSE_CLIENT --query "SYSTEM DISABLE FAILPOINT merge_task_projection_stage_pause" 2>/dev/null
    $CLICKHOUSE_CLIENT --query "DROP TABLE IF EXISTS t_optimize_single_partition_slot SYNC" 2>/dev/null
}
trap cleanup EXIT
