...
-- `currentDatabase()` because the style check only recognizes that form; reading `is_loaded` does not load a lazy table.
SELECT name, engine, is_loaded FROM system.tables WHERE database = currentDatabase() AND (name LIKE 'm05100%' OR name = 't05100_local') ORDER BY name;
