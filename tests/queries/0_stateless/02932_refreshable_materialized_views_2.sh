#!/usr/bin/env bash
# Tags: atomic-database, memory-engine, long, no-msan
...
$CLICKHOUSE_CLIENT -q "
    ...
    create materialized view rmv_c refresh every 1 second (x Int64) engine Memory empty as select * from src;
    drop table src;"
...
$CLICKHOUSE_CLIENT -q "
    select '<19: exception>', exception ilike '%UNKNOWN_TABLE%' ? '1' : exception from refreshes where view = 'rmv_c';
    create table src (x Int64) engine Memory as select 1;
    system refresh view rmv_c;
    system wait view rmv_c;"
# Rename table.
$CLICKHOUSE_CLIENT -q "
    select '<20: unexception>', * from rmv_c;
    rename table rmv_c to rmv_d;
    select '<21: rename>', * from rmv_d;
...
$CLICKHOUSE_CLIENT -q "
    ...
    create materialized view rmv_e refresh every 1 second (x Int64) engine MergeTree order by x as select x + sleepEachRow(1) as x from src settings max_block_size = 1, max_threads = 1;
    system wait view rmv_e;"
# Stop refreshes.
$CLICKHOUSE_CLIENT -q "
    select '<23: simple refresh>', * from rmv_e;
    system stop view rmv_e;"
...
$CLICKHOUSE_CLIENT -q "
    create materialized view rmv_g refresh every 1 week offset 3 day 4 hour randomize for 4 day 1 hour (x Int64) engine Memory empty as select 42 as x;
    show create rmv_g;
    system test view rmv_g set fake time '2050-02-03 15:30:13';"
...
$CLICKHOUSE_CLIENT -q "
    ...
    truncate src;
    insert into src values (1);
    create materialized view rmv_h refresh every 1 second to dest as select x*10 as x from src;
    create materialized view rmv_hh refresh every 2 second to dest as select x from src; -- { serverError BAD_ARGUMENTS }
    show create rmv_h;
    system wait view rmv_h;
    system stop view rmv_h;"
while [ "`$CLICKHOUSE_CLIENT -q "select status from refreshes where view = 'rmv_h' -- $LINENO" | xargs`" != 'Disabled' ]
do
    sleep 0.5
done
$CLICKHOUSE_CLIENT -q "
...
$CLICKHOUSE_CLIENT -q "system stop view rmv_h;"
while [ "`$CLICKHOUSE_CLIENT -q "select status from refreshes where view = 'rmv_h' -- $LINENO" | xargs`" != 'Disabled' ]
do
    sleep 0.5
done
...
# Retries.
$CLICKHOUSE_CLIENT -q "
    create materialized view rmv_h2 refresh after 1 year settings refresh_retries = 10 (x Int64) engine Memory as select x*10 + throwIf(x % 2 == 0) as x from src;"
$CLICKHOUSE_CLIENT -q "system wait view rmv_h2; -- { serverError REFRESH_FAILED }"
$CLICKHOUSE_CLIENT -q "
    ...
    drop table src2;"
while [ "`$CLICKHOUSE_CLIENT -nq "select status, retry from refreshes where view = 'rmv_h2' -- $LINENO" | xargs`" != 'Scheduled 0' ]
do
    sleep 0.5
done
$CLICKHOUSE_CLIENT -q "
    select '<31.6: did retry>', x from rmv_h2;
    drop table rmv_h2"

# EMPTY
$CLICKHOUSE_CLIENT -q "
    create materialized view rmv_i refresh after 1 year engine Memory empty as select number as x from numbers(2);
    system wait view rmv_i;
    create materialized view rmv_j refresh after 1 year engine Memory as select number as x from numbers(2);"
...
# APPEND
$CLICKHOUSE_CLIENT -q "
    create materialized view rmv_k refresh every 10 year append (x Int64) engine Memory empty as select x*10 as x from src;
    select '<33: append>', * from rmv_k;
    system refresh view rmv_k;
    system wait view rmv_k;
    select '<34: append>', * from rmv_k;
    truncate table src;
    insert into src values (2), (3);
    system refresh view rmv_k;
    system wait view rmv_k;
    select '<35: append>', * from rmv_k order by x;"
# ALTER to non-APPEND
$CLICKHOUSE_CLIENT -q "
    alter table rmv_k modify refresh every 10 year; -- { serverError NOT_IMPLEMENTED }"
$CLICKHOUSE_CLIENT -q "
    drop table rmv_k;
    truncate table src;"
...
$CLICKHOUSE_CLIENT -q "
    ...
    create materialized view rmv_l refresh every 10 year append to mid empty as select x*10 as x from src;
    create materialized view rmv_m (x Int64) engine Memory as select x*10 as x from mid;
    insert into src values (1);
    system refresh view rmv_l;
    system wait view rmv_l;
    select '<37: append chain>', * from rmv_m;
    insert into src values (2);
    system refresh view rmv_l;
    system wait view rmv_l;
    select '<38: append chain>', * from rmv_m order by x;
    drop table rmv_l;
    drop table rmv_m;
    drop table mid;"
...
$CLICKHOUSE_CLIENT --create_table_empty_primary_key_by_default 0 -q "
    create materialized view rmv_n refresh every 1 second (x Int64) engine MergeTree as select 1 as x from numbers(2); -- { serverError BAD_ARGUMENTS }"
$CLICKHOUSE_CLIENT -q "
    create materialized view rmv_n refresh every 1 second (x Int64) engine MergeTree order by x as select 1 as x from numbers(2);
    drop table rmv_n;"

# Reading from table that doesn't exist yet.
$CLICKHOUSE_CLIENT -q "
    create materialized view rmv_o refresh every 1 second (x Int64) engine Memory as select x from nonexist; -- { serverError UNKNOWN_TABLE }
    create materialized view rmv_o (x Int64) engine Memory as select x from nonexist; -- { serverError UNKNOWN_TABLE }
    create materialized view rmv_o (x Int64) engine Memory as select x from nope.nonexist; -- { serverError UNKNOWN_DATABASE }
    create materialized view rmv_o refresh every 1 second (x Int64) engine Memory as select x from nope.nonexist settings allow_materialized_view_with_bad_select = 1;
    drop table rmv_o;"

$CLICKHOUSE_CLIENT -q "
    create materialized view rmv_o refresh every 1 second append (number UInt64) engine Memory as select number from numbers(2);
    system wait view rmv_o;
    select '<39: append>', count() % 2, count() > 0, sum(number) > 0, sum(number)*2 - count() from rmv_o;
    drop table rmv_o;"
