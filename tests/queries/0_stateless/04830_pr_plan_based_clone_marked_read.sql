...
-- ^ creates a DEFINER view; access entities are not part of the Replicated test database.
...
-- system-log storage or a DEFINER view builds a fresh context from the global context.
-- Related: #111677

DROP TABLE IF EXISTS t_clone_marked;
DROP VIEW IF EXISTS v_clone_marked;
...
SELECT count() > 0 FROM system.user_query_log;
SELECT count() > 0 FROM merge('system', '^user_query_log$');

-- Same for a DEFINER view: its inner read also runs under a context built from the global context.
CREATE VIEW v_clone_marked DEFINER = CURRENT_USER SQL SECURITY DEFINER AS SELECT a, b FROM t_clone_marked;
SELECT count(), sum(b), min(a), max(a) FROM v_clone_marked WHERE a > 5;
