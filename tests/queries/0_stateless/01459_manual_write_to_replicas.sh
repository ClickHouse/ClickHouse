...
function thread {
    for x in {0..99}; do
        ...
        $CLICKHOUSE_CLIENT -n --query "SET insert_keeper_fault_injection_probability=0;INSERT INTO r$1 SELECT $x % $NUM_REPLICAS = $1 ? $x - 1 : $x" 2>/dev/null  # Replace some records as duplicates so they will be written by other replicas
    done
}
