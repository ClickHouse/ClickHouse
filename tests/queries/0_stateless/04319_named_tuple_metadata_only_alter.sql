...
-- ============================================================
-- Case 16: key/partition safety checks. If the *specific subcolumn* referenced
-- in ORDER BY / PARTITION BY is unchanged (same type), the ALTER is allowed as
-- metadata-only. If the whole Tuple is referenced, it is still rejected because
...
SELECT 'Case 16a (subcolumn in ORDER BY allowed, type unchanged):';
ALTER TABLE t_named_tuple_alter
    MODIFY COLUMN t Tuple(a UInt64, b UInt64, c UInt64) SETTINGS alter_sync = 2;
SELECT 'mutations:', count() FROM system.mutations WHERE database = currentDatabase() AND table = 't_named_tuple_alter';
SELECT id, t, t.a, t.b, t.c FROM t_named_tuple_alter ORDER BY id;
...
SELECT 'Case 16b (subcolumn in PARTITION BY allowed, type unchanged):';
ALTER TABLE t_named_tuple_alter
    MODIFY COLUMN t Tuple(a UInt64, b UInt64, c UInt64) SETTINGS alter_sync = 2;
SELECT 'mutations:', count() FROM system.mutations WHERE database = currentDatabase() AND table = 't_named_tuple_alter';
SELECT id, t, t.a, t.b, t.c FROM t_named_tuple_alter ORDER BY id;
...
SELECT 'Case 17a (nested addition, subcolumn under it in ORDER BY allowed, type unchanged):';
ALTER TABLE t_named_tuple_alter
    MODIFY COLUMN t Tuple(a UInt64, sub Tuple(x UInt64, y UInt64)) SETTINGS alter_sync = 2;
SELECT 'mutations:', count() FROM system.mutations WHERE database = currentDatabase() AND table = 't_named_tuple_alter';
SELECT id, t.a, t.sub.x, t.sub.y FROM t_named_tuple_alter ORDER BY id;
...
-- A sibling subcolumn (not under the nested addition) is in ORDER BY. The
-- subcolumn t.a (UInt64) is unchanged, so the ALTER is allowed as metadata-only.
CREATE TABLE t_named_tuple_alter (id UInt64, t Tuple(a UInt64, sub Tuple(x UInt64)))
ENGINE = MergeTree ORDER BY (t.a, id);

INSERT INTO t_named_tuple_alter VALUES (1, (10, (100)));

SELECT 'Case 17c (nested addition, sibling subcolumn in ORDER BY allowed, type unchanged):';
ALTER TABLE t_named_tuple_alter
    MODIFY COLUMN t Tuple(a UInt64, sub Tuple(x UInt64, y UInt64)) SETTINGS alter_sync = 2;
SELECT 'mutations:', count() FROM system.mutations WHERE database = currentDatabase() AND table = 't_named_tuple_alter';
SELECT id, t.a, t.sub.x, t.sub.y FROM t_named_tuple_alter ORDER BY id;
...
-- ============================================================
-- Case 20: Projections referencing the modified column must block the
-- metadata-only path. The projection's primary.idx and data are serialized
-- with the old tuple shape; without a mutation the projection part becomes
-- stale.
-- ============================================================

-- 20a: projection on the whole Tuple column
CREATE TABLE t_named_tuple_alter (id UInt64, t Tuple(a UInt64, b UInt64))
ENGINE = MergeTree ORDER BY id;

ALTER TABLE t_named_tuple_alter ADD PROJECTION p (SELECT t ORDER BY t);
INSERT INTO t_named_tuple_alter VALUES (1, (10, 20));

SELECT 'Case 20a (projection on whole Tuple column rejected):';
ALTER TABLE t_named_tuple_alter
    MODIFY COLUMN t Tuple(a UInt64, b UInt64, c Nullable(UInt64)); -- { serverError ALTER_OF_COLUMN_IS_FORBIDDEN }

DROP TABLE t_named_tuple_alter;

-- 20b: Tuple column NOT in any projection still succeeds metadata-only
CREATE TABLE t_named_tuple_alter (id UInt64, t Tuple(a UInt64, b UInt64), other UInt64)
ENGINE = MergeTree ORDER BY id;

ALTER TABLE t_named_tuple_alter ADD PROJECTION p (SELECT other ORDER BY other);
INSERT INTO t_named_tuple_alter VALUES (1, (10, 20), 99);

SELECT 'Case 20b (Tuple not in projection still works):';
ALTER TABLE t_named_tuple_alter
    MODIFY COLUMN t Tuple(a UInt64, b UInt64, c Nullable(UInt64));
SELECT 'mutations:', count() FROM system.mutations WHERE database = currentDatabase() AND table = 't_named_tuple_alter';
SELECT t.a, t.b, t.c FROM t_named_tuple_alter;
...
-- 20c: skip index on subcolumn whose type is unchanged still allows metadata-only
CREATE TABLE t_named_tuple_alter
(id UInt64, t Tuple(a UInt64, b UInt64),
 INDEX idx t.a TYPE minmax GRANULARITY 1)
ENGINE = MergeTree ORDER BY id;

INSERT INTO t_named_tuple_alter VALUES (1, (10, 20));

SELECT 'Case 20c (skip index on unchanged subcolumn allows metadata-only):';
ALTER TABLE t_named_tuple_alter
    MODIFY COLUMN t Tuple(a UInt64, b UInt64, c UInt64) SETTINGS alter_sync = 2;
SELECT 'mutations:', count() FROM system.mutations WHERE database = currentDatabase() AND table = 't_named_tuple_alter';
SELECT id, t.a, t.b, t.c FROM t_named_tuple_alter ORDER BY id;

DROP TABLE t_named_tuple_alter;

-- ============================================================
-- Case 21: Nullable(Tuple(...)) wrapper. Adding a subfield inside a
-- Nullable-wrapped named Tuple triggers a full mutation because
...
INSERT INTO t_named_tuple_alter VALUES (1, (10,)), (2, NULL), (3, (30,));

SELECT 'Case 21 (Nullable(Tuple) triggers mutation):';
ALTER TABLE t_named_tuple_alter
    MODIFY COLUMN t Nullable(Tuple(a UInt64, b UInt64)) SETTINGS alter_sync = 2;
SELECT 'mutations:', count() FROM system.mutations WHERE database = currentDatabase() AND table = 't_named_tuple_alter';
SELECT id, t, t.a, t.b FROM t_named_tuple_alter ORDER BY id;

DROP TABLE t_named_tuple_alter;

-- ============================================================
-- Case 22: Disabling the setting disables the optimization.
-- With allow_metadata_only_named_tuple_alter = 0,
-- adding a subfield should trigger a full mutation.
-- ============================================================
SET allow_metadata_only_named_tuple_alter = 0;

CREATE TABLE t_named_tuple_alter (id UInt64, t Tuple(a UInt64))
ENGINE = MergeTree ORDER BY id;

INSERT INTO t_named_tuple_alter VALUES (1, (10,)), (2, (20,));

SELECT 'Case 22 (setting disables optimization):';
ALTER TABLE t_named_tuple_alter
    MODIFY COLUMN t Tuple(a UInt64, b UInt64) SETTINGS alter_sync = 2;
SELECT 'mutations:', count() FROM system.mutations WHERE database = currentDatabase() AND table = 't_named_tuple_alter';
SELECT id, t, t.a, t.b FROM t_named_tuple_alter ORDER BY id;
