...
-- 26.6 only: a trivial SELECT count() over a table where the only granted column is an ALIAS column throws
-- a logical error regardless of any filter, so grant one physical column too. secret_token stays denied.
GRANT SELECT(id) ON $DB.t TO $u_alias;
...
# 26.6 only: without `parallel_replicas_for_non_replicated_merge_tree` the old analyzer disables the parallel replicas
# for a plain MergeTree together with the custom key filter (#112942 is not in 26.6), so enable it to exercise the filter.
echo "-- analyzer=$analyzer: 15 parallel replicas custom key (sampling) over denied column"
run "$u_low" "SELECT count() FROM t SETTINGS enable_analyzer=$analyzer, parallel_replicas_for_non_replicated_merge_tree = 1, cluster_for_parallel_replicas = 'test_shard_localhost', max_parallel_replicas = 2, allow_experimental_parallel_reading_from_replicas = 1, parallel_replicas_mode = 'custom_key_sampling', parallel_replicas_count = 2, parallel_replica_offset = 1, parallel_replicas_custom_key = 'secret_token = ''$T'''"
