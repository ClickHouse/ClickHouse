...
do
    ...
    do
        ...
        echo -e "${whitespace}trim_${trim}_fixed_string_${whitespace}${whitespace_name}_everywhere${whitespace}" | $CLICKHOUSE_LOCAL -S "c1 FixedString(64)" --input_format_csv_trim_whitespaces=${trim} --input-format="CSV" -q "select toStringCutToZero(c1) from table FORMAT CSV"
...
do
    ...
    echo -e "${whitespace}custom_csv_fixed_string_${whitespace}${whitespace_name}_everywhere${whitespace}"  | $CLICKHOUSE_LOCAL -S "c1 FixedString(64)" --input-format="CustomSeparated"  --input_format_csv_trim_whitespaces=false --format_custom_escaping_rule=CSV --format_custom_field_delimiter=',' --format_csv_delimiter=',' -q "select toStringCutToZero(c1) from table FORMAT CSV"
