...
-- A NULL breaks the second assumption (`uniq` skips it, `count()` does not) and `WITH ROLLUP`/
-- `WITH CUBE` break the first (they emit extra super-aggregate rows).
...
SELECT uniq(x) FROM (SELECT x FROM values('x Int64', (0), (1), (1)) GROUP BY x WITH ROLLUP) SETTINGS enable_analyzer = 0;
SELECT uniq(x) FROM (SELECT x FROM values('x Int64', (0), (1), (1)) GROUP BY x WITH ROLLUP) SETTINGS enable_analyzer = 0, optimize_uniq_to_count = 0;

-- The rewrite still fires for a non-Nullable column without modifiers.
