...
$CLICKHOUSE_CLIENT -q "create table $db.t as system.query_log"   # Suppress database = currentDatabase()
...
$CLICKHOUSE_CLIENT -q "create table $db2.t2 as system.query_log" 2>&1| grep -Fac "Database is in readonly mode"   # Suppress database = currentDatabase()
...
$CLICKHOUSE_CLIENT -q "create table $db.t2 as system.query_log" 2>&1| grep -Fac "Database is in readonly mode"   # Suppress database = currentDatabase()
$CLICKHOUSE_CLIENT -q "show tables from $db"
