...
# if $zk is empty
if [ -z "$zk" ]; then
    echo "== Test drop database replica with default zookeeper=="
else
    echo "== Test drop database replica with auxiliary zookeeper ($zk)=="
    db_suffix="_$zk"
    zk_path_prefix="$zk:"
fi

db="rdb_$CLICKHOUSE_DATABASE$db_suffix"

$CLICKHOUSE_CLIENT -q "system flush logs query_log"
$CLICKHOUSE_CLIENT -q "create database $db engine=Replicated('$zk_path_prefix/test/$CLICKHOUSE_DATABASE/rdb', 's1', 'r1')"
...
$CLICKHOUSE_CLIENT -q "system drop database replica 's1|r1' from zkpath '$zk_path_prefix/test/$CLICKHOUSE_DATABASE/rdb'" 2>&1| grep -Fac "There is a local database"
$CLICKHOUSE_CLIENT -q "system drop database replica 's1|r1' from zkpath '$zk_path_prefix/test/$CLICKHOUSE_DATABASE/rdb/'" 2>&1| grep -Fac "There is a local database"
$CLICKHOUSE_CLIENT -q "system drop database replica 'r1' from shard 's1' from zkpath '$zk_path_prefix/test/$CLICKHOUSE_DATABASE/rdb/'" 2>&1| grep -Fac "There is a local database"

$CLICKHOUSE_CLIENT -q "system drop database replica 's1|r1' from zkpath '$zk_path_prefix/test/$CLICKHOUSE_DATABASE/'" 2>&1| grep -Fac "does not look like a path of Replicated database"
$CLICKHOUSE_CLIENT -q "system drop database replica 's2|r1' from zkpath '$zk_path_prefix/test/$CLICKHOUSE_DATABASE/rdb'" 2>&1| grep -Fac "does not exist"
$CLICKHOUSE_CLIENT -q "system drop database replica 's1' from shard 'r1' from zkpath '$zk_path_prefix/test/$CLICKHOUSE_DATABASE/rdb'" 2>&1| grep -Fac "does not exist"
$CLICKHOUSE_CLIENT -q "system drop database replica 's1|r1' from shard 's1' from zkpath '$zk_path_prefix/test/$CLICKHOUSE_DATABASE/rdb'" 2>&1| grep -Fac "does not exist"
$CLICKHOUSE_CLIENT -q "system drop database replica 's2/r1' from zkpath '$zk_path_prefix/test/$CLICKHOUSE_DATABASE/rdb'" 2>&1| grep -Fac "Invalid replica name"

db2="${db}_2$db_suffix"
db3="${db}_3$db_suffix"
$CLICKHOUSE_CLIENT -q "create database $db2 engine=Replicated('$zk_path_prefix/test/$CLICKHOUSE_DATABASE/rdb', 's1', 'r2')"
$CLICKHOUSE_CLIENT -q "create database $db3 engine=Replicated('$zk_path_prefix/test/$CLICKHOUSE_DATABASE/rdb', 's2', 'r1')"
...
$CLICKHOUSE_CLIENT -q "insert into system.zookeeper(name, path, value, zookeeperName) values ('active', '$zk_path_prefix/test/${CLICKHOUSE_DATABASE}/rdb/replicas/s9|r9', '${REPLICA_UUID}', '${zk}')"
fi
...
$CLICKHOUSE_CLIENT -q "system drop database replica 'r1' from shard 's1' from zkpath '$zk_path_prefix/test/$CLICKHOUSE_DATABASE/rdb/'"
...
db4="${db}_4$db_suffix"
$CLICKHOUSE_CLIENT -q "create database $db4 engine=Replicated('$zk_path_prefix/test/$CLICKHOUSE_DATABASE/rdb', 's1', 'r1')"
...
$CLICKHOUSE_CLIENT -q "system drop database replica '${zk_path_prefix}doesntexist$CLICKHOUSE_DATABASE' from shard 'doesntexist'"
...
$CLICKHOUSE_CLIENT -q "system drop replica '${zk_path_prefix}doesntexist$CLICKHOUSE_DATABASE' from database $db4"
$CLICKHOUSE_CLIENT -q "system drop replica '${zk_path_prefix}doesntexist$CLICKHOUSE_DATABASE'"
