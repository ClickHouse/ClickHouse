...
$CLICKHOUSE_CLIENT -q "create table $db.t as system.query_log"
...
$CLICKHOUSE_CLIENT -q "create table $db2.t2 as system.query_log" 2>&1| grep -Fac "Database is in readonly mode"
...
$CLICKHOUSE_CLIENT -q "create table $db.t2 as system.query_log" 2>&1| grep -Fac "Database is in readonly mode"
$CLICKHOUSE_CLIENT -q "show tables from $db"

db3="${db}_3"
$CLICKHOUSE_CLIENT -q "create database $db3 engine=Replicated('/test/$CLICKHOUSE_DATABASE/rdb', 's1', 'r1')"
