...
-- A bare `[NULL]` array has element type `Nullable(Nothing)`, which is not comparable with the key
-- type, so no set atom is built and the primary key does not prune. The result must still be
-- correct: `has([NULL], x)` matches only the NULL key row.
