...
CREATE TABLE tab (
    k UInt64,
    s Map(String,String),
    INDEX idx(mapKeys(s)) TYPE full_text(2)
)
ENGINE = MergeTree()
ORDER BY k
SETTINGS
...
CREATE TABLE tab(
    k UInt64,
    s String,
    INDEX idx(s) TYPE full_text(0, 12040)
)
Engine=MergeTree
ORDER BY (k)
AS
...
CREATE TABLE tab(
    k UInt64,
    s String,
    INDEX idx(s) TYPE full_text(3, 123)
)
Engine=MergeTree
ORDER BY (k)
AS
...
ENGINE = MergeTree
ORDER BY id
SETTINGS min_bytes_for_full_part_storage=0; -- Full text index works only with full parts.
