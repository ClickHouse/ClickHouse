...
# `REFRESH ... IF CHANGED`: a refresh that succeeded only on a retry must still leave a usable
# watermark. The view definition hash must not depend on per-attempt diagnostics (`log_comment`
# carries the attempt number), otherwise the next scheduled attempt discards the watermark and an
# `APPEND` view appends a duplicate copy of unchanged rows.
...
$CLICKHOUSE_CLIENT -q "
    ...
    CREATE MATERIALIZED VIEW mv REFRESH EVERY 1 SECOND IF CHANGED
        ...
        APPEND ENGINE = MergeTree ORDER BY cnt AS SELECT count() AS cnt FROM src;
"
