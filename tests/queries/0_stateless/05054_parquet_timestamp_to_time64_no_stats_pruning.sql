...
insert into function file(currentDatabase() || '_05054_ts.parquet', Parquet, 't DateTime64(3, \'UTC\')')
...
select toString(t) from file(currentDatabase() || '_05054_ts.parquet', Parquet, 't Time64(3)') order by t;

-- The value that falls outside the raw stats range must still be found, with and without pushdown.
select count() from file(currentDatabase() || '_05054_ts.parquet', Parquet, 't Time64(3)')
    where t = toTime64('01:00:00', 3) settings log_comment = '05054prune_ts_to_time64';
select count() from file(currentDatabase() || '_05054_ts.parquet', Parquet, 't Time64(3)')
...
select distinct log_comment, ProfileEvents['ParquetReadRowGroups'], ProfileEvents['ParquetPrunedRowGroups']
    ...
    where current_database = currentDatabase() and type = 'QueryFinish' and log_comment like '05054prune%'
