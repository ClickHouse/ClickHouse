...
select * from t0 final SETTINGS enable_vertical_final = 0;
DROP TABLE t0;

-- PartsSplitter should also work for reverse nullable keys.
CREATE TABLE t0(a Nullable(Int), b Nullable(Int)) Engine=ReplacingMergeTree order by (a desc, b desc) SETTINGS allow_experimental_reverse_key = 1, allow_nullable_key = 1, index_granularity = 8192, index_granularity_bytes = '10Mi';
INSERT INTO t0 select number, number from numbers(5);
INSERT INTO t0 select null, null union all select number, number from numbers(5, 2);
set max_threads = 2;
explain pipeline select * from t0 final SETTINGS enable_vertical_final = 0;
select * from t0 final SETTINGS enable_vertical_final = 0;
DROP TABLE t0;
