...
-- Compact with_key_columns layout: part type, substreams, lookup / exists_ /
-- full Map round-trip, and arrayElement -> key_ rewrite.
...
DROP TABLE IF EXISTS t_wkc_compact_layout;

CREATE TABLE t_wkc_compact_layout
(
    id UInt64,
    m Map(String, UInt64),
    s Map(String, String)
...
INSERT INTO t_wkc_compact_layout VALUES
...
SELECT 'rewrite_m_a';
SELECT count() > 0
FROM (EXPLAIN actions = 1 SELECT m['a'] FROM t_wkc_compact_layout)
...
CHECK TABLE t_wkc_compact_layout SETTINGS check_query_single_value_result = 1;

DROP TABLE t_wkc_compact_layout;
