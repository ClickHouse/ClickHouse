...
# A file with many row splits (a split has at most 100k rows), so that several threads decode it.
$CLICKHOUSE_LOCAL -q "
...
echo "Stopping early does not hang or leak the helpers:"
...
echo "A file with a single split needs no helpers:"
