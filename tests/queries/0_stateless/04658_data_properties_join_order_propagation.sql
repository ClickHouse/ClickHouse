...
SELECT '-- unique-key cardinality caps enabled: dpsub --';
...
SELECT '-- unique-key cardinality caps enabled: dphyp --';
...
SET query_plan_optimize_join_order_algorithm = 'dpsub';
SELECT 'dpsub', count(), sum(first_id), sum(second_first_id), sum(third_first_id)
FROM (SELECT first_id FROM data_properties_first GROUP BY first_id) AS first
INNER JOIN (SELECT second_first_id FROM data_properties_second GROUP BY second_first_id) AS second ON first_id = second_first_id
INNER JOIN data_properties_third ON first_id = third_first_id;

SET query_plan_optimize_join_order_algorithm = 'dphyp';
SELECT 'dphyp', count(), sum(first_id), sum(second_first_id), sum(third_first_id)
FROM (SELECT first_id FROM data_properties_first GROUP BY first_id) AS first
INNER JOIN (SELECT second_first_id FROM data_properties_second GROUP BY second_first_id) AS second ON first_id = second_first_id
INNER JOIN data_properties_third ON first_id = third_first_id;
