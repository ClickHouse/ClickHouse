...
-- same result); for outer kinds the fallback is the block nested loop join, whose operator is
-- not implemented yet.
...
SELECT count() FROM neg_l l LEFT JOIN neg_r r ON l.x < r.x AND l.y > 1; -- { serverError NOT_IMPLEMENTED }
...
SELECT count() FROM neg_l l LEFT JOIN neg_r r ON l.dec < r.f AND l.y > r.y; -- { serverError NOT_IMPLEMENTED }
...
SELECT count() FROM neg_l l LEFT JOIN neg_r r ON l.x < r.x AND l.y > r.y; -- { serverError NOT_IMPLEMENTED }
