...
-- 3. linearOr across multiple segments with partial range overlap
...
-- 'freq' in all 5000 rows -> ~39 segments of 128 -> many packed blocks.
...
CREATE TABLE tab_skip1(k UInt64, s String, INDEX idx s TYPE text(tokenizer = 'splitByNonAlpha', posting_list_block_size = 128))
...
CREATE TABLE tab_skip2(k UInt64, s String, INDEX idx s TYPE text(tokenizer = 'splitByNonAlpha', posting_list_block_size = 128))
    ENGINE = MergeTree() ORDER BY k
    SETTINGS index_granularity = 8192, index_granularity_bytes = '10Mi';
...
----------------------------------------------------
SELECT 'Test 3: linearOr across multiple segments with partial overlap';

-- 'wide' in all 3000 rows -> ~24 segments. Query all rows via hasToken.
-- linearOr must traverse all segments, each invoking linearOrImpl which
-- uses the block-skip optimization. Full coverage: all blocks overlap.

DROP TABLE IF EXISTS tab_skip3;

CREATE TABLE tab_skip3(k UInt64, s String, INDEX idx s TYPE text(tokenizer = 'splitByNonAlpha', posting_list_block_size = 128))
    ENGINE = MergeTree() ORDER BY k
    SETTINGS index_granularity = 8192, index_granularity_bytes = '10Mi';
...
-- 'aligned' in all 512 rows -> 4 full packed blocks (128 each) in one segment.
...
CREATE TABLE tab_skip4(k UInt64, s String, INDEX idx s TYPE text(tokenizer = 'splitByNonAlpha', posting_list_block_size = 4096))
    ENGINE = MergeTree() ORDER BY k
    SETTINGS index_granularity = 8192, index_granularity_bytes = '10Mi';
...
-- (which covers doc_ids 0..128 for segment 0).
-- linearOr/linearAnd for 'full' should decode only the block containing rows 65-66.

DROP TABLE IF EXISTS tab_skip5;

CREATE TABLE tab_skip5(k UInt64, s String, INDEX idx s TYPE text(tokenizer = 'splitByNonAlpha', posting_list_block_size = 128))
    ENGINE = MergeTree() ORDER BY k
    SETTINGS index_granularity = 8192, index_granularity_bytes = '10Mi';
...
CREATE TABLE tab_skip6(k UInt64, s String, INDEX idx s TYPE text(tokenizer = 'splitByNonAlpha', posting_list_block_size = 128))
    ENGINE = MergeTree() ORDER BY k
    SETTINGS index_granularity = 8192, index_granularity_bytes = '10Mi';
...
CREATE TABLE tab_skip7(k UInt64, s String, INDEX idx s TYPE text(tokenizer = 'splitByNonAlpha', posting_list_block_size = 128))
    ENGINE = MergeTree() ORDER BY k
    SETTINGS index_granularity = 8192, index_granularity_bytes = '10Mi';
...
CREATE TABLE tab_skip8(k UInt64, s String, INDEX idx s TYPE text(tokenizer = 'splitByNonAlpha', posting_list_block_size = 128))
    ENGINE = MergeTree() ORDER BY k
    SETTINGS index_granularity = 8192, index_granularity_bytes = '10Mi';
...
CREATE TABLE tab_skip9(k UInt64, s String, INDEX idx s TYPE text(tokenizer = 'splitByNonAlpha', posting_list_block_size = 128))
    ENGINE = MergeTree() ORDER BY k
    SETTINGS index_granularity = 8192, index_granularity_bytes = '10Mi';
...
CREATE TABLE tab_skip10(k UInt64, s String, INDEX idx s TYPE text(tokenizer = 'splitByNonAlpha', posting_list_block_size = 128))
    ENGINE = MergeTree() ORDER BY k
    SETTINGS index_granularity = 8192, index_granularity_bytes = '10Mi';
