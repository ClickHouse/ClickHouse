...
--
-- Every cell compares the lazy plan against the same query with the optimization off,
-- so a cell reads 1 only when both plans agree. The fixture makes the FINAL winner
...
-- branch is really used, which cell 10 asserts.
...
SELECT 'Nullable(UInt64)',
    (SELECT arraySort(groupArray((k, flag, version))) FROM t_null_u64 FINAL PREWHERE flag = 0
        SETTINGS query_plan_optimize_lazy_final = 1, min_filtered_ratio_for_lazy_final = 0.0)
  = (SELECT arraySort(groupArray((k, flag, version))) FROM t_null_u64 FINAL PREWHERE flag = 0
...
-- isNullableOrLowCardinalityNullable: DataTypeLowCardinality::isNullable() returns false,
-- so a plain isNullable() predicate leaves this shape broken while cells 1, 2 and 4 pass.
...
-- 5. control: a non-nullable primary key. Correct before the fix, and it is what makes
-- the cells above specific to nullability rather than to the fixture shape.
...
-- the fix, so this pins that the fix did not over-generalize into containers.
...
-- 10. The cells above compare the optimization against itself being disabled, so they
-- also pass if the optimization silently stops applying. Assert it still runs.
SELECT 'lazy FINAL still applies to a nullable key',
