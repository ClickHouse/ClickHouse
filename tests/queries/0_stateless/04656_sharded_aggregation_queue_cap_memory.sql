...
-- turns it into one, because queued chunks are tracked memory. Bisected on this exact shape:
-- the query needs 17 MiB with the cap honoured and 389 MiB without it, so the 96 MiB budget
-- below sits well above one requirement and well below the other. A smaller row count narrows
-- that gap (at 4M rows the two are 39 MiB and 83 MiB) and the test stops discriminating.
--
...
--     any queue reaches the cap.

DROP TABLE IF EXISTS test_116038;
CREATE TABLE test_116038 (a UInt64, b UInt64) ENGINE = MergeTree ORDER BY tuple()
SETTINGS index_granularity = 8192, min_bytes_for_wide_part = 0;
INSERT INTO test_116038 SELECT number % 4 AS a, cityHash64(number) AS b FROM numbers_mt(16000000);
...
FROM (
    EXPLAIN PIPELINE
    SELECT a, uniqCombined(b), uniqCombined(b + 1), uniqCombined(b + 2)
    FROM test_116038
    GROUP BY a
    SETTINGS enable_sharding_aggregator = 1, max_threads = 16, max_block_size = 1024,
...
SELECT a, uniqCombined(b) > 0, uniqCombined(b + 1) > 0, uniqCombined(b + 2) > 0
FROM test_116038
GROUP BY a
ORDER BY a
SETTINGS enable_sharding_aggregator = 0, max_threads = 16, max_block_size = 1024,
         max_rows_to_group_by = 0, enable_parallel_replicas = 0,
