...
$CLICKHOUSE_CLIENT -q "explain actions = 1 select sum(x), sum(y) from (select sipHash64(number) as x, bitAnd(number, 1024) as y from numbers_mt(1000000000) limit 1000000000) where y = 0 settings query_plan_execute_functions_after_sorting=1, query_plan_split_filter=1, query_plan_push_down_limit=1" | grep -o "FUNCTION sipHash64\|Filter column: equals"
...
$CLICKHOUSE_CLIENT -q "
    ...
    settings enable_optimize_predicate_expression=0, query_plan_execute_functions_after_sorting=0" | grep -o "Aggregating\|Filter"
$CLICKHOUSE_CLIENT -q "
    select s, y from (select sum(x) as s, y from (
        select number as x, number + 1 as y from numbers(10)) group by y
    ) where y != 0 order by s, y
    settings enable_optimize_predicate_expression=0, query_plan_execute_functions_after_sorting=0"
...
$CLICKHOUSE_CLIENT --enable_analyzer=0 -q "
    ...
    settings enable_optimize_predicate_expression=0, query_plan_execute_functions_after_sorting=0" | grep -o "Aggregating\|Filter\|COLUMN Const(UInt8) -> notEquals(y, 0)"
...
$CLICKHOUSE_CLIENT --enable_analyzer=1 -q "
    ...
    settings enable_optimize_predicate_expression=0, query_plan_execute_functions_after_sorting=0" | grep -o "Aggregating\|Filter\|COLUMN Const(UInt8) -> notEquals(__table1.y, 0_UInt8)"
$CLICKHOUSE_CLIENT -q "
...
$CLICKHOUSE_CLIENT --enable_analyzer=0  -q "
    ...
    ) array join x where y != 2 and x != 0 settings query_plan_execute_functions_after_sorting=0" |
...
$CLICKHOUSE_CLIENT -q "
    ...
    ) where number != 2 settings enable_optimize_predicate_expression=0, query_plan_merge_filters=1, query_plan_execute_functions_after_sorting=0" |
    grep -o "CreatingSets\|Filter"
$CLICKHOUSE_CLIENT -q "
    select number from (
        select number from numbers(5) where number in (select 1 + number from numbers(3))
    ) where number != 2 settings enable_optimize_predicate_expression=0, query_plan_execute_functions_after_sorting=0"
...
$CLICKHOUSE_CLIENT --enable_analyzer=0 -q "
    ...
    ) as r on a = r.b where a != 1 and b != 2 settings enable_optimize_predicate_expression = 0, query_plan_join_swap_table = 0, query_plan_execute_functions_after_sorting=0" |
...
$CLICKHOUSE_CLIENT -q "
    select number as a, r.b from numbers(4) as l any left join (
        select number + 2 as b from numbers(3)
    ) as r on a = r.b where a != 1 and b != 2 settings enable_optimize_predicate_expression = 0, query_plan_join_swap_table = 0, query_plan_execute_functions_after_sorting=0" | sort
...
$CLICKHOUSE_CLIENT --enable_analyzer=1 -q "
    ...
    select number as a, r.b from numbers(4) as l any inner join (
        select number + 2 as b from numbers(3)
    ) as r on a = r.b where a != 1 and b != 2 settings enable_optimize_predicate_expression = 0, query_plan_join_swap_table = 0, enable_join_runtime_filters = 0, query_plan_execute_functions_after_sorting=0" |
...
$CLICKHOUSE_CLIENT -q "
    ...
    ) where a != 1 settings enable_optimize_predicate_expression = 0, query_plan_execute_functions_after_sorting=0" |
    grep -o "Union\|Filter"
$CLICKHOUSE_CLIENT -q "
    select a, b from (
        select number + 1 as a, number + 2 as b from numbers(2) union all select number + 1 as b, number + 2 as a from numbers(2)
...
$CLICKHOUSE_CLIENT --enable_analyzer=0 -q "
    explain actions = 1 select number as n, sipHash64(n) from numbers(100) order by number + 1 limit 5 settings query_plan_execute_functions_after_sorting=1, query_plan_push_down_limit=1" |
