...
#
# The mutation is materialized by letting it run and waiting for it, not by issuing another
# mutation with `mutations_sync`: a second pending mutation may be squashed together with the
# `DELETE` into one mutation with the later version, and that separate defect (fixed in
# https://github.com/ClickHouse/ClickHouse/pull/119127) would make this test flaky.

function wait_for_mutations()
{
    for _ in $(seq 1 300)
    do
        if [ "$($CLICKHOUSE_CLIENT -q "SELECT count() FROM system.mutations WHERE database = currentDatabase() AND table = 't_pending_delete_patch' AND NOT is_done")" == "0" ]
        then
            return
        fi
        sleep 0.2
    done
    echo "Timed out waiting for mutations"
    $CLICKHOUSE_CLIENT -q "SELECT * FROM system.mutations WHERE database = currentDatabase() AND table = 't_pending_delete_patch' FORMAT Vertical"
}
