...
EXPLAIN indexes = 1
SELECT * FROM test_int_divide WHERE x != 0 AND intDiv(10, x) > 4 SETTINGS query_plan_merge_expressions=1, query_plan_execute_functions_after_sorting=0;
...
EXPLAIN indexes = 1
SELECT * FROM test_divide WHERE x != 0 AND divide(10, x) > 4 ORDER BY x SETTINGS query_plan_merge_expressions=1, query_plan_execute_functions_after_sorting=0;
...
EXPLAIN indexes = 1
SELECT * FROM test_div_by_zero_const WHERE divide(x, 0) > 0 SETTINGS query_plan_merge_expressions=1, query_plan_execute_functions_after_sorting=0;
