...
with client(name="client1>", log=log) as client1, client(
    ...
    client1.send("SET allow_experimental_analyzer = 0, output_format_pretty_squash_ms = 0;")
    client1.expect(prompt)
    client1.send("SET allow_experimental_window_view = 1")
    client1.expect(prompt)
    client1.send("SET window_view_heartbeat_interval = 1")
    client1.expect(prompt)
    client2.send("SET allow_experimental_window_view = 1, output_format_pretty_squash_ms = 0;")
    client2.expect(prompt)
