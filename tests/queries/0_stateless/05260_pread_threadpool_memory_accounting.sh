...
$CLICKHOUSE_CLIENT --query "
    DROP TABLE IF EXISTS 05260_pread_threadpool_memory;
    CREATE TABLE 05260_pread_threadpool_memory (id UInt64, s String)
    ENGINE = MergeTree ORDER BY id
    SETTINGS min_bytes_for_full_part_storage = 0;
    SYSTEM STOP MERGES 05260_pread_threadpool_memory;
"

for _ in $(seq 1 20); do
    $CLICKHOUSE_CLIENT --query "INSERT INTO 05260_pread_threadpool_memory SELECT number, randomPrintableASCII(64) FROM numbers(200)"
done
...
run_scan()
{
    local method=$1
    $CLICKHOUSE_CLIENT --query "
        SELECT count(), max(s) FROM 05260_pread_threadpool_memory /* ${COMMENT} */
        SETTINGS
...
$CLICKHOUSE_CLIENT --query "
    ...
    (
        SELECT
            maxIf(type = 'QueryFinish', log_comment = '05260_pread') AS pread_ok,
            maxIf(type = 'QueryFinish', log_comment = '05260_pread_threadpool') AS threadpool_ok,
            maxIf(memory_usage, log_comment = '05260_pread' AND type = 'QueryFinish') AS pread_memory,
            maxIf(memory_usage, log_comment = '05260_pread_threadpool' AND type = 'QueryFinish') AS threadpool_memory
        FROM system.query_log
        WHERE current_database = currentDatabase()
          AND event_date >= yesterday()
          AND event_time >= now() - 600
          AND log_comment IN ('05260_pread', '05260_pread_threadpool')
    )
"

$CLICKHOUSE_CLIENT --query "DROP TABLE 05260_pread_threadpool_memory"
