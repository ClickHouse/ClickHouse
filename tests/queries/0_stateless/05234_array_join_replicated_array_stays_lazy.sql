-- Chained ARRAY JOINs hand each other lazily replicated arrays; the next join must not materialize the whole block of them.
...
-- Same rows with and without lazy replication, across several windows, for inner, LEFT, aligned pairs, maps and a fused element filter.
...
-- The third join used to materialize max_block_size * 100 strings for its first output block.
