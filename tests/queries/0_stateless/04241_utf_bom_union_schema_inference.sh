...
$CLICKHOUSE_LOCAL -q "select name, type from (desc file('$CLICKHOUSE_TEST_UNIQUE_NAME/data{1,2}.jsonl') settings schema_inference_mode='union')"

# Read data (union mode)
$CLICKHOUSE_LOCAL -q "select obj.f1, obj.f2, obj.f3 from file('$CLICKHOUSE_TEST_UNIQUE_NAME/data{1,2}.jsonl') settings schema_inference_mode='union' order by obj.f2"
