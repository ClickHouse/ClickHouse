...
SELECT id, v1 FROM tab ORDER BY v1 DESC LIMIT 10 SETTINGS max_rows_to_read = 640, use_skip_indexes_for_top_n = 1, use_skip_indexes_on_data_read = 0;
...
SELECT trimLeft(explain) AS explain FROM (
    ...
    ORDER BY v1 ASC
    LIMIT 10
    SETTINGS use_skip_indexes_for_top_n = 1, use_skip_indexes_on_data_read = 0)
WHERE explain LIKE '%TopN%';
...
SELECT trimLeft(explain) AS explain FROM (
    ...
    ORDER BY v1 ASC
    LIMIT 10
    SETTINGS use_skip_indexes_for_top_n = 0, use_top_n_dynamic_filtering = 1)
WHERE explain LIKE '%topN%';
