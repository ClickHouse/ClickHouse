...
CREATE TABLE ${db}.dst(count UInt64, market Int64, w_end DateTime) Engine=MergeTree ORDER BY tuple();
...
while true; do
	$CLICKHOUSE_CLIENT "${opts[@]}" --query="SELECT count(*) FROM ${db}.dst" | grep -q "5" && break || sleep .5 ||:
done
