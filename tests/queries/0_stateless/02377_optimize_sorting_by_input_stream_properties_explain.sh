...
DISABLE_OPTIMIZATION="--optimize_sorting_by_input_stream_properties=0 --query_plan_read_in_order=0 --max_threads=1"
ENABLE_OPTIMIZATION="--optimize_sorting_by_input_stream_properties=1 --optimize_read_in_order=1 --query_plan_read_in_order=1 --max_threads=1"
MAKE_OUTPUT_STABLE="--optimize_read_in_order=1 --max_threads=3"
...
function explain_sorting {
    echo "-- QUERY:" "${@: -1}"
    $CLICKHOUSE_CLIENT "${@:1}" -q "${@: -1}" | eval $FIND_SORTING
}
function explain_sortmode {
    echo "-- QUERY: " "${@: -1}"
    $CLICKHOUSE_CLIENT "${@:1}" -q "${@: -1}" | eval $FIND_SORTMODE
}
