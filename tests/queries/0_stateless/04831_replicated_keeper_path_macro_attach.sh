...
# A table whose stored path still carries {database} cannot be moved to another database, attached or
# detached: the rename is refused, and the table keeps loading on its own path. A configured macro
...
${CLIENT} -q "RENAME DATABASE \`${LEGACY_DB}\` TO \`${LEGACY_DB}/d\`" 2>&1 | grep -q -F 'NOT_IMPLEMENTED' && echo REFUSED || echo RENAMED
${CLIENT} -q "ATTACH TABLE \`${LEGACY_DB}\`.t" 2>&1 | grep -q -F 'BAD_ARGUMENTS' && echo REJECTED || echo ATTACHED
${CLIENT} -q "SELECT count() FROM system.tables WHERE database = '${LEGACY_DB}' AND name = 't'"
...
${CLIENT} -q "SYSTEM RESTART REPLICA \`${RESTART_DB}\`.t" 2>&1 | grep -q -F 'BAD_ARGUMENTS' && echo REJECTED || echo RESTARTED
${CLIENT} -q "SELECT count() FROM system.tables WHERE database = '${RESTART_DB}' AND name = 't'"
