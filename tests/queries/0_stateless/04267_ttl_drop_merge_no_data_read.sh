...
# -------------------------------------------------------------------
# Case 17: a patched merge keeps live GROUP BY TTL bounds
#
# The aggregation algorithm recomputes bounds only for rolled-up rows, so
# a patched merge used to leave either a finished pre-patch entry or none
# at all; both hid the part from later TTL passes.
# -------------------------------------------------------------------
echo "-- Case 17: patched merge keeps live GROUP BY TTL bounds"
...
${CLICKHOUSE_CLIENT} -q "
    ...
    ENGINE = MergeTree()
    ORDER BY id
    TTL event_time GROUP BY id SET v = max(v)
    SETTINGS
        apply_patches_on_merge = 1,
        enable_block_number_column = 1,
        enable_block_offset_column = 1,
    ...
    -- A lightweight update moves 10 rows into the future.
    UPDATE t_ttl_patch_group_by SET event_time = now() + INTERVAL 1 DAY WHERE id < 10
    SETTINGS enable_lightweight_update = 1, mutations_sync = 2;
...
# The merged part must carry a live GROUP BY entry covering the patched future rows; a finished
# or missing entry would exclude the part from the TTL selectors and the rows would overstay.
${CLICKHOUSE_CLIENT} -q "
    SELECT any(group_by_ttl_info.max[1]) > now()
    FROM system.parts
