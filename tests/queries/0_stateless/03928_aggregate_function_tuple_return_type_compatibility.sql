...
-- Use the explicit tuple(name, ...)(value, ...) syntax so the named result type is the same in
-- both analyzers (the automatic naming of `tuple(a, b)` from column names is analyzer-only).
SELECT toTypeName(argMin(tuple('a', 'b')(a, b), c))
...
SELECT toTypeName(argMax(tuple('a', 'b')(a, b), c))
FROM values('a Nullable(Int32), b Nullable(Int32), c Nullable(Int32)', (1, 2, 3), (NULL, 1, 0), (3, 4, 5));
