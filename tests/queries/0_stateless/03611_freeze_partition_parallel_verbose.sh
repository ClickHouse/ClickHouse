...
for max_threads in 1 3; do
  ...
  ${CLICKHOUSE_CLIENT} --query "ALTER TABLE table_for_freeze FREEZE WITH NAME 'test_03612' $FREEZE_QUERY_PARAMS;" \
  ...
  ${CLICKHOUSE_CLIENT} --query "ALTER TABLE table_for_freeze FREEZE PARTITION '3' WITH NAME 'test_03612_single_part' $FREEZE_QUERY_PARAMS;" \
    | ${CLICKHOUSE_LOCAL} --structure "$ALTER_OUT_STRUCTURE, $FREEZE_OUT_STRUCTURE" \
        --query "SELECT command_type, partition_id, part_name, backup_name FROM table ORDER BY partition_id FORMAT TSVWithNames"
  ...
  ${CLICKHOUSE_CLIENT} --query "ALTER TABLE table_for_freeze FREEZE PARTITION '7' WITH NAME 'test_03612_single_part_7', ATTACH PART '5_6_6_0' $FREEZE_QUERY_PARAMS;" \
  ...
  # Unfreeze partition
  ${CLICKHOUSE_CLIENT} --query "ALTER TABLE table_for_freeze UNFREEZE PARTITION '7' WITH NAME 'test_03612_single_part_7' $FREEZE_QUERY_PARAMS;" \
    | ${CLICKHOUSE_LOCAL} --structure "$ALTER_OUT_STRUCTURE, $FREEZE_OUT_STRUCTURE" \
        --query "SELECT command_type, partition_id, part_name, backup_name FROM table ORDER BY partition_id FORMAT TSVWithNames"

  # Freeze partition with old syntax
  ${CLICKHOUSE_CLIENT} --query "ALTER TABLE table_for_freeze_old_syntax FREEZE PARTITION '202103' WITH NAME 'test_03612_single_part_old_syntax' $FREEZE_QUERY_PARAMS;" \
    | ${CLICKHOUSE_LOCAL} --structure "$ALTER_OUT_STRUCTURE, $FREEZE_OUT_STRUCTURE" \
        --query "SELECT command_type, partition_id, part_name, backup_name FROM table ORDER BY partition_id FORMAT TSVWithNames"

  # Unfreeze partition with old syntax
  ${CLICKHOUSE_CLIENT} --query "ALTER TABLE table_for_freeze_old_syntax UNFREEZE PARTITION '202103' WITH NAME 'test_03612_single_part_old_syntax' $FREEZE_QUERY_PARAMS;" \
    | ${CLICKHOUSE_LOCAL} --structure "$ALTER_OUT_STRUCTURE, $FREEZE_OUT_STRUCTURE" \
        --query "SELECT command_type, partition_id, part_name, backup_name FROM table ORDER BY partition_id FORMAT TSVWithNames"

  # Unfreeze the whole backup with SYSTEM query
  ${CLICKHOUSE_CLIENT} --query "ALTER TABLE table_for_freeze FREEZE PARTITION '7' WITH NAME 'test_03612_single_part_7_system'"
  ${CLICKHOUSE_CLIENT} --query "DROP TABLE table_for_freeze"
  ${CLICKHOUSE_CLIENT} --query "ALTER TABLE table_for_freeze UNFREEZE PARTITION '7' WITH NAME 'test_03612_single_part_7_system'" 2>/dev/null
  rc=$?
  if [ $rc -eq 0 ]; then
      echo "ALTER query shouldn't unfreeze removed table. Code: $rc"
      exit 1
  fi
  ${CLICKHOUSE_CLIENT} --query "SYSTEM UNFREEZE WITH NAME 'test_03612_single_part_7_system'" \
    | ${CLICKHOUSE_LOCAL} --structure "$ALTER_OUT_STRUCTURE, $FREEZE_OUT_STRUCTURE" \
