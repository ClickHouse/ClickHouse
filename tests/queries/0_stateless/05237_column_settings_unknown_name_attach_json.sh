...
$CLICKHOUSE_CLIENT -q "
SELECT 'attach_accepted', create_table_query LIKE '%SETTINGS (min_compress_block_size = 100)%'
...
${CLICKHOUSE_CURL} -sS "$JSON_URL" --data-binary '{"type":"AlterQuery","table":"t_column_settings_json","alter_object":"TABLE","command_list":{"type":"ExpressionList","children":[{"type":"AlterCommand","command_type":"MODIFY_COLUMN","col_decl":{"type":"ColumnDeclaration","name":"y","data_type":{"type":"DataType","name":"UInt64"},"settings":{"type":"SetQuery","default_settings":["not_a_setting"]}}}]}}' 2>&1 | grep -o -m 1 -F 'UNKNOWN_SETTING'

${CLICKHOUSE_CURL} -sS "$JSON_URL" --data-binary '{"type":"AlterQuery","table":"t_column_settings_json","alter_object":"TABLE","command_list":{"type":"ExpressionList","children":[{"type":"AlterCommand","command_type":"MODIFY_COLUMN","col_decl":{"type":"ColumnDeclaration","name":"y","data_type":{"type":"DataType","name":"UInt64"},"settings":{"type":"SetQuery","query_parameters":[{"name":"not_a_setting","value":"1"}]}}}]}}' 2>&1 | grep -o -m 1 -F 'UNKNOWN_SETTING'
...
$CLICKHOUSE_CLIENT -q "
SELECT 'json_accepted', create_table_query LIKE '%\`y\` UInt64 SETTINGS (min_compress_block_size = 100)%'
