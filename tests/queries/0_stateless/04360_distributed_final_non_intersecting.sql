...
INSERT INTO t_ni_rep SELECT number, number, 1 FROM numbers(4000);
INSERT INTO t_ni_rep SELECT number, number, 1 FROM numbers(4000);
...
CREATE TABLE t_ni_del (k UInt64, v UInt64, ver UInt64, is_deleted UInt8) ENGINE = ReplacingMergeTree(ver, is_deleted) ORDER BY k SETTINGS index_granularity = 64, merge_max_block_size = 8192;
INSERT INTO t_ni_del SELECT number, number, 1, number % 7 = 0 FROM numbers(4000);
INSERT INTO t_ni_del SELECT number, number, 1, number % 7 = 0 FROM numbers(4000);
...
CREATE TABLE t_ni_col (k UInt64, v UInt64, sign Int8) ENGINE = CollapsingMergeTree(sign) ORDER BY k SETTINGS index_granularity = 64, merge_max_block_size = 8192;
INSERT INTO t_ni_col SELECT number, number, 1 FROM numbers(4000);
INSERT INTO t_ni_col SELECT number, number, -1 FROM numbers(2000);
