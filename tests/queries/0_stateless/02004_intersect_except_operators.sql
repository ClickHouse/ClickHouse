-- The output order of INTERSECT/EXCEPT depends on the number of threads, so the queries with several result rows are ordered.
...
select * from (select number from numbers(20) intersect select number from numbers(5, 5)) order by all;
select * from (select number from numbers(10) except select number from numbers(5)) order by all;
select * from (select number, number+10 from numbers(12) except select number+5, number+15 from numbers(10)) order by all;
...
select * from (select number from numbers(10) except select 5) order by all;
select * from (select number from numbers(100) intersect select number from numbers(20, 60) except select number from numbers(30, 20) except select number from numbers(60, 20)) order by all;
