...
${CLICKHOUSE_CLIENT} --async_insert=0 --allow_experimental_json_ast_dialect 1 --dialect clickhouse_json -q "$JSON_APPLY"
...
OUT=$(${CLICKHOUSE_CLIENT} --async_insert=0 --allow_experimental_json_ast_dialect 1 --dialect clickhouse_json -q "$JSON_REJECT" 2>&1 || true)
