...
-- `max_rows_in_distinct` throws when exceeded, both before and after spilling.
...
-- `max_bytes_in_distinct` limits the retained hash state, so it does not apply to output volume after
-- the set is released. Spill thresholds trigger external processing but do not bound query memory.
-- The dynamically growing set for `UInt64` keys fits before spilling, while the output exceeds the
-- set's byte limit.
