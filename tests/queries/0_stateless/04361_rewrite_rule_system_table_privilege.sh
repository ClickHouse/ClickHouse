...
# `system.query_rules` exposes global rewrite/rejection policies (table names, filters and
# masked secrets). It must only be readable by users allowed to manage rules, mirroring how
# `system.named_collections` gates its rows behind a grant. A user without the
# CREATE/ALTER/DROP RULE grants must not be able to enumerate rules, even with plain SELECT
# access to the table. The rule grants are global, so a name unique per test database avoids
# colliding with concurrent tests.

# The rule name is unique per test database, so no pre-existing rule needs cleaning up
# first (and `DROP RULE` has no `IF EXISTS` form). The user is dropped defensively in case
# a previous run in the same database left one behind.
