...
-- arrive, as `parallel_hash` does. The joined output, the RIGHT/FULL non-joined rows and the blocks handed to
-- `GraceHashJoin` on a spill all read those columns back from that store. Checksums must match `hash`.
