-- Tests that the query-construction settings (`select` / `filter` / `order` / `sort` and
-- `limit` / `offset` / `page`) are applied correctly when they appear on a (sub)query's own
-- `SETTINGS` clause, on individual `UNION` arms, on an `INSERT ... SELECT`, or under `EXPLAIN`.
-- These are materialized by wrapping the (sub)query as a derived table in `executeQuery` rather than
-- folded into the query tree by the analyzer. A `SETTINGS` clause applies to its own scope (not to
-- deeper subqueries); session/user settings apply only to the outermost query.
