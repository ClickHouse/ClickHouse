...
DROP TABLE IF EXISTS tab_bitpacking_v1;
DROP TABLE IF EXISTS tab_bitpacking_v2;
DROP TABLE IF EXISTS tab_uncompressed;

CREATE TABLE tab_bitpacking_v1
(
...
CREATE TABLE tab_bitpacking_v2
(
    ts DateTime CODEC(LZ4),
    str String CODEC(LZ4),
    INDEX inv_idx str TYPE text(
...
OPTIMIZE TABLE tab_bitpacking_v1 FINAL;
OPTIMIZE TABLE tab_bitpacking_v2 FINAL;
OPTIMIZE TABLE tab_uncompressed FINAL;

-- Compare the size of the text index for the same dataset with V1, V2 and uncompressed formats.
SELECT
    table,
    sum(rows),
    sum(secondary_indices_compressed_bytes)
FROM system.parts
WHERE database = currentDatabase() AND active AND table IN ('tab_bitpacking_v1', 'tab_bitpacking_v2', 'tab_uncompressed')
GROUP BY table
ORDER BY table;

DROP TABLE tab_bitpacking_v1;
DROP TABLE tab_bitpacking_v2;
