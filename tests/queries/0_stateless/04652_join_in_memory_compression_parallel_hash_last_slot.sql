...
--
-- `ConcurrentHashJoin` publishes each slot's byte delta to the join's global counter only *after*
-- that slot's insert returns, while the compression trigger runs at the end of the insert. A
-- threshold crossed by the last non-empty slot of a source block was therefore observed by no slot
-- at all (every earlier slot saw a lower total), and - when several threads cross it together - none
-- of the concurrent inserts sees it either, because each of them evaluates the trigger against a
-- total its peers have not published yet. The next source block was then met first by
-- `SpillingHashJoin`'s pre-insert check, which switched to `GraceHashJoin` before anything
-- compressed. Two things keep the contract: the trigger adds the running insert's own unpublished
-- delta back to the global total, and `SpillingHashJoin` forces one compression pass over all slots
-- before it gives up on the in-memory join.
--
...
-- Control: with compression off the build side (~60 MiB) crosses the 48 MB threshold and the join
-- spills, which is what the compression-on runs below must avoid. The compressed build side settles
-- around 6 MB, far below the 24 MB at which compression fires, so the assertion does not depend on
-- how much per-block overhead a particular build carries.
...
SETTINGS join_algorithm = 'parallel_hash', max_threads = 4, max_block_size = 600,
         ...
         max_bytes_before_external_join = 48000000, query_plan_join_swap_table = 'false',
