-- GeoJSON input format: `GeometryCollection` handling.
--
-- A `GeometryCollection` cannot be represented in ClickHouse's `Geometry` type. By default
...
-- MultiPoint is representable, so the unsupported geometry handling does not apply to it.
SELECT count() FROM format(GeoJSON, '{"type":"FeatureCollection","features":[{"type":"Feature","geometry":{"type":"MultiPoint","coordinates":[[1,2],[3,4]]},"properties":{}}]}');
...
-- A GeometryCollection whose child is a valid MultiPoint is still NULL, because the collection itself is unrepresentable.
...
-- not selected the unrepresentable type is validation-only, so a GeometryCollection feature is accepted even
-- under the default `'throw'` policy (only the JSON grammar and shape are checked, nothing is stored).
SELECT count() FROM format(GeoJSON, 'id Nullable(String)', '{"type":"FeatureCollection","features":[{"type":"Feature","geometry":{"type":"GeometryCollection","geometries":[{"type":"Point","coordinates":[1,2]}]},"properties":{}}]}');
