...
${CLICKHOUSE_CLIENT} --send_logs_level 'debug' --allow_experimental_parallel_reading_from_replicas 1 --max_parallel_replicas 11  --use_hedged_requests 0 --cluster_for_parallel_replicas 'parallel_replicas' --parallel_replicas_for_non_replicated_merge_tree 1 -q "SELECT count() FROM test_parallel_replicas_unavailable_shards WHERE NOT ignore(*)" 2>&1 | grep -Fac 'Replica number 10 is unavailable'
