...
# The same holds for the explicit listing: an unreachable server must not be reported as a database
# without tables, which would contradict the answers of `EXISTS TABLE` and `SELECT` above.
${CLICKHOUSE_CLIENT} --query "SHOW TABLES FROM ${REMOTE_DB}_unreachable" 2>&1 | grep -c -m1 -E "NETWORK_ERROR|ALL_CONNECTION_TRIES_FAILED|CONNECTION_REFUSED"
${CLICKHOUSE_CLIENT} --query "SELECT name FROM system.tables WHERE database = '${REMOTE_DB}_unreachable'" 2>&1 | grep -c -m1 -E "NETWORK_ERROR|ALL_CONNECTION_TRIES_FAILED|CONNECTION_REFUSED"
