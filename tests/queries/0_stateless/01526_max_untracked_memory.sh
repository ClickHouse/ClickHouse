...
${CLICKHOUSE_CLIENT} --log_queries=1 --max_threads=1 --max_untracked_memory=0 --memory_profiler_sample_min_allocation_size=0 --memory_profiler_sample_max_allocation_size=0 --memory_profiler_sample_probability=1 --trace_profile_events 0 -q "with '$query_id_tcp_prefix' as __id $query FORMAT Null"
...
echo "$query" | ${CLICKHOUSE_CURL} -sSg -o /dev/null "${CLICKHOUSE_URL}&query_id=$query_id_http&max_untracked_memory=0&memory_profiler_sample_min_allocation_size=0&memory_profiler_sample_max_allocation_size=0&memory_profiler_sample_probability=1&max_threads=1&trace_profile_events=0" -d @-
