-- Test first-key multi-row skipping with Float64 / +-Inf / NaN keys in partial_merge join.
-- MergeJoin skips the run of rows whose first sort key is less than the other side's
-- current row in one IColumn::compareTrackAt call, comparing Float keys via
-- CompareHelper<Float64>::compare. To actually exercise the NaN branch of the skip the
...
-- first-key run skip. NaN is on both sides so the first-key comparison sees NaN vs
...
-- the first key compares NaN vs NaN equal on k1, but the second key differs, so no NaN row joins (expect 0).
