...
INSERT INTO retired_rule_04839 VALUES (999, '{"arr": [{"tag_x": 1}], "tag_top": 1}');
...
INSERT INTO control_04839 VALUES (999, '{"arr": [{"tag_x": 1}], "tag_top": 1}');
...
SELECT 'rule paths', arraySort(JSONDynamicPaths(j)), arraySort(JSONSharedDataPaths(j)) FROM retired_rule_04839 WHERE id = 999;
...
SELECT 'after removal', 'rule paths', arraySort(JSONDynamicPaths(j)), arraySort(JSONSharedDataPaths(j)) FROM retired_rule_04839 WHERE id = 999;
