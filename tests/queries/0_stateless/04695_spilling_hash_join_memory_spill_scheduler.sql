...
-- The memory spill scheduler must reach a `SpillingHashJoin`, not only a top-level `grace_hash`.
...
-- Keep the join's own spill threshold out of reach, so only the scheduler can trigger spilling.
