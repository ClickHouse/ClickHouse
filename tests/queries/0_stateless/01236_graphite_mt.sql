...
select 1 AS key, 'sum_1', toDateTime(today()) - number * 60 - 30, number, 1 AS Version, number from numbers(300) union all
...
insert into test_graphite
select 1 AS key, 'sum_1', toDateTime(today() - 3) - number * 60 - 30, number, 1 AS Version, number from numbers(1200) union all
