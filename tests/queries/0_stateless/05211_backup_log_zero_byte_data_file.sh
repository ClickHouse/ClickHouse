...
# No query can observe a data file's size, so the fixture's own subject is asserted on disk here:
# s.bin must hold zero bytes while the table holds rows.
s_bin_size() {
    local dir
    dir=$(${CLICKHOUSE_CLIENT} --query "SELECT data_paths[1] FROM system.tables WHERE database = currentDatabase() AND name = '$1'")
    printf '%s\t%s\n' "$2" "$(stat -c%s "${dir%/}/s.bin")"
}
...
s_bin_size t1 'source s.bin bytes'
...
s_bin_size t1r 'restored s.bin bytes'
