...
-- granules the index keeps. The last block compares the memory of an INSERT and of a mutation that rewrites the
-- part with such indexes and without them.
...
SELECT replaceRegexpOne(explain, '^[│└├─ ]+', '') FROM (EXPLAIN indexes = 1 SELECT count() FROM t_lc WHERE has(mapKeys(m), 'k3') SETTINGS use_skip_indexes_on_data_read = 0) WHERE explain ILIKE '%Name: i%' OR explain ILIKE '%Granules: %/%';
...
SELECT replaceRegexpOne(explain, '^[│└├─ ]+', '') FROM (EXPLAIN indexes = 1 SELECT count() FROM t_lc WHERE has(mapValues(m), 'v3') SETTINGS use_skip_indexes_on_data_read = 0) WHERE explain ILIKE '%Name: i%' OR explain ILIKE '%Granules: %/%';
...
SELECT replaceRegexpOne(explain, '^[│└├─ ]+', '') FROM (EXPLAIN indexes = 1 SELECT count() FROM t_plain WHERE has(mapKeys(m), 'k3') SETTINGS use_skip_indexes_on_data_read = 0) WHERE explain ILIKE '%Name: i%' OR explain ILIKE '%Granules: %/%';
...
SELECT replaceRegexpOne(explain, '^[│└├─ ]+', '') FROM (EXPLAIN indexes = 1 SELECT count() FROM t_mut WHERE has(mapKeys(m), 'n7') SETTINGS use_skip_indexes_on_data_read = 0) WHERE explain ILIKE '%Name: i%' OR explain ILIKE '%Granules: %/%';
...
SELECT replaceRegexpOne(explain, '^[│└├─ ]+', '') FROM (EXPLAIN indexes = 1 SELECT count() FROM t_order WHERE has(mapValues(m), 'v3') SETTINGS use_skip_indexes_on_data_read = 0) WHERE explain ILIKE '%Name: i%' OR explain ILIKE '%Granules: %/%';
...
SELECT replaceRegexpOne(explain, '^[│└├─ ]+', '') FROM (EXPLAIN indexes = 1 SELECT count() FROM t_mixed WHERE has(mapKeys(m), 'k3') SETTINGS use_skip_indexes_on_data_read = 0, ignore_data_skipping_indices = 'is,im,it') WHERE explain ILIKE '%Name: i%' OR explain ILIKE '%Granules: %/%';
SELECT replaceRegexpOne(explain, '^[│└├─ ]+', '') FROM (EXPLAIN indexes = 1 SELECT count() FROM t_mixed WHERE has(mapKeys(m), 'k3') SETTINGS use_skip_indexes_on_data_read = 0, ignore_data_skipping_indices = 'ik,im,it') WHERE explain ILIKE '%Name: i%' OR explain ILIKE '%Granules: %/%';
...
SELECT replaceRegexpOne(explain, '^[│└├─ ]+', '') FROM (EXPLAIN indexes = 1 SELECT count() FROM t_mixed WHERE has(mapKeys(m), 'k4') SETTINGS use_skip_indexes_on_data_read = 0, ignore_data_skipping_indices = 'is,im,it') WHERE explain ILIKE '%Name: i%' OR explain ILIKE '%Granules: %/%';
...
SELECT replaceRegexpOne(explain, '^[│└├─ ]+', '') FROM (EXPLAIN indexes = 1 SELECT count() FROM t_nested WHERE formatRowNoNewline('JSONEachRow', mapKeys(m)) = '{"mapKeys(m)":["k3","common"]}' SETTINGS use_skip_indexes_on_data_read = 0) WHERE explain ILIKE '%Name: i%' OR explain ILIKE '%Granules: %/%';
...
SELECT replaceRegexpOne(explain, '^[│└├─ ]+', '') FROM (EXPLAIN indexes = 1 SELECT count() FROM t_text WHERE has(mapKeys(m), 'k3') SETTINGS use_skip_indexes_on_data_read = 0) WHERE explain ILIKE '%Name: i%' OR explain ILIKE '%Granules: %/%';
...
-- The index input is the map's own column: with such indexes, an INSERT and a mutation that rewrites the part need less than twice the memory they need without them.
...
ENGINE = MergeTree ORDER BY id SETTINGS index_granularity = 131072, index_granularity_bytes = 268435456, min_bytes_for_wide_part = 0, map_serialization_version = 'basic', map_serialization_version_for_zero_level_parts = 'basic';
...
SELECT maxIf(memory_usage, query LIKE 'INSERT INTO t_mem_idx %') < 2 * maxIf(memory_usage, query LIKE 'INSERT INTO t_mem_none %')
...
ALTER TABLE t_mem_none DELETE WHERE id = 0 SETTINGS mutations_sync = 2;
ALTER TABLE t_mem_idx DELETE WHERE id = 0 SETTINGS mutations_sync = 2;
SYSTEM FLUSH LOGS part_log;
SELECT countIf(table = 't_mem_none'), countIf(table = 't_mem_idx'), maxIf(peak_memory_usage, table = 't_mem_idx') < 2 * maxIf(peak_memory_usage, table = 't_mem_none')
FROM system.part_log WHERE database = currentDatabase() AND event_date >= yesterday() AND event_type = 'MutatePart';
