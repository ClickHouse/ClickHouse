...
-- another type uses the same expression, and when the expression is nested in a larger one. Each probe compares
-- the indexed count with an unindexed one and shows the granules the index keeps.
