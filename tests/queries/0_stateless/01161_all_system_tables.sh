...
function run_selects()
{
    ...
    for t in "${tables_arr[@]}"
    do
        ${CLICKHOUSE_CLIENT} -q "SELECT * FROM $t LIMIT $LIMIT SETTINGS allow_introspection_functions = 1, max_execution_time = 1000 FORMAT Null" # Suppress style check: database=$CLICKHOUSE_DATABASE
    done
}
