...
-- The sparsity-stats trivial count would answer `count() WHERE empty(s)` / `notEmpty(s)` without
-- index analysis, hiding the `Condition`/`Granules` output this test pins.
...
-- Equality produces an exact atom on `s` and a relaxed atom on `substring(s, 1, 2)`.
-- The direct atom preserves exactness while both atoms contribute to pruning.
