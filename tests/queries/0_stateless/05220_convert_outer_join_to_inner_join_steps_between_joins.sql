...
SELECT '-- OFFSET does not allow converting.';
SELECT trim(explain) FROM (
    EXPLAIN PLAN actions = 1
    SELECT count() FROM (SELECT m.val AS val FROM fact AS f LEFT JOIN mid AS m ON f.id = m.id ORDER BY val NULLS FIRST OFFSET 60) AS g INNER JOIN small AS s ON g.val = s.val
) WHERE trim(explain) IN ('Type: INNER', 'Type: LEFT', 'Type: RIGHT', 'Type: FULL');
...
SELECT '-- WITH ROLLUP does not allow converting.';
SELECT trim(explain) FROM (
    EXPLAIN PLAN actions = 1
    SELECT count() FROM (SELECT m.val AS val FROM fact AS f LEFT JOIN mid AS m ON f.id = m.id GROUP BY m.val WITH ROLLUP) AS g INNER JOIN small AS s ON g.val = s.val
) WHERE trim(explain) IN ('Type: INNER', 'Type: LEFT', 'Type: RIGHT', 'Type: FULL');
