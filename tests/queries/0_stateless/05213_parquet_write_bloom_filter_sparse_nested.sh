...
# values it is going to hash, and refuses to write a filter at all when that size would exceed the 128 MiB
# that readers accept. For a repeated or nullable leaf only the entries at the maximum definition level
...
# get exactly the same filter as a plain `UInt64` column holding the same values, instead of no filter.
#
