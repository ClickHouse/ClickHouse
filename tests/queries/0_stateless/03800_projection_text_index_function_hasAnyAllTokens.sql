...
-- Must accept two or three arguments
...
(
    id UInt32,
    message String,
    PROJECTION idx INDEX message TYPE text(tokenizer = ngrams(4)),
)
ENGINE = MergeTree
ORDER BY (id);
...
(
    id UInt32,
    message String,
    PROJECTION idx INDEX message TYPE text(tokenizer = splitByString(['()', '\\'])),
)
ENGINE = MergeTree
ORDER BY (id);
...
(
    id UInt32,
    message String,
    PROJECTION idx INDEX message TYPE text(tokenizer = 'array'),
)
ENGINE = MergeTree
ORDER BY (id);
