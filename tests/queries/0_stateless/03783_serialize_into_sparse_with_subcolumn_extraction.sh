...
$CLICKHOUSE_CLIENT --query="SET optimize_trivial_insert_select = 0; insert into function file('${CLICKHOUSE_TEST_UNIQUE_NAME}.csv') select (tuple(tuple(''), ''), tuple(tuple('a'), 'a')) union all select (tuple(tuple('a'), 'a'), tuple(tuple(''), ''))"
...
$CLICKHOUSE_CLIENT --query="SET optimize_trivial_insert_select = 0; insert into hmmm (a, e) select * FROM file('${CLICKHOUSE_TEST_UNIQUE_NAME}.csv')"
