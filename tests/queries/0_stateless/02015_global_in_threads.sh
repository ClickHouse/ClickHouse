...
${CLICKHOUSE_CLIENT} -q "select length(thread_ids) >= 1 from system.query_log where event_date >= yesterday() AND event_time >= now() - 600 and query_id = '2015_${CLICKHOUSE_DATABASE}_query' and type = 'QueryFinish' and current_database = currentDatabase()"
