...
SELECT read_rows, result_rows from system.query_log 
        where   query_kind ='Select' and 
                startsWith(query, 'SELECT * FROM simple1 WHERE s LIKE \'%01%\'') 
...
SELECT read_rows, result_rows from system.query_log 
    where query_kind ='Select' 
        and startsWith(query, 'SELECT * FROM simple2 WHERE hasToken(s, \'Alick\')') 
...
SELECT read_rows, result_rows from system.query_log 
    where query_kind ='Select' 
        and startsWith(query, 'SELECT * FROM simple3 WHERE s LIKE \'%01%\'') 
...
SELECT read_rows, result_rows from system.query_log where query_kind ='Select' and startsWith(query, 'SELECT * FROM simple4 WHERE s LIKE \'%你好%\'') and result_rows==1 limit 1;
