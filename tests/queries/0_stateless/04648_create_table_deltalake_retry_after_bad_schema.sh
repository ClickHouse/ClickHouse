...
# (1) A CREATE with a non-round-tripping type (`UInt8`) is rejected before commit 0 is written
# (Code: 48 = NOT_IMPLEMENTED), so the target location must be left clean (no orphan `_delta_log`).
if $CLICKHOUSE_CLIENT --query "
SET allow_experimental_delta_kernel_rs = 1;
SET allow_experimental_delta_lake_writes = 1;
CREATE TABLE t_dl_retry (c UInt8) ENGINE = DeltaLakeLocal('${TABLE_PATH}', Parquet);
