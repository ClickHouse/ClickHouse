...
F="$WORKDIR/04321_field_ids_custom.parquet"
...
# 2. Auto-assign only.
F="$WORKDIR/04321_field_ids_auto.parquet"
...
F="$WORKDIR/04321_field_ids_mixed.parquet"
...
F="$WORKDIR/04321_field_ids_none.parquet"
...
F="$WORKDIR/04321_field_ids_nested_auto.parquet"
...
F="$WORKDIR/04321_field_ids_nested_overrides.parquet"
...
F="$WORKDIR/04321_field_ids_geo_override.parquet"
...
F="$WORKDIR/04321_field_ids_geo_auto.parquet"
...
${CLICKHOUSE_LOCAL} --query="
INSERT INTO FUNCTION file('$WORKDIR/04321_err2.parquet', 'Parquet')
...
${CLICKHOUSE_LOCAL} --query="
INSERT INTO FUNCTION file('$WORKDIR/04321_err6.parquet', 'Parquet')
SELECT [1, 2, 3] AS a
SETTINGS engine_file_truncate_on_insert = 1,
         output_format_parquet_column_field_ids = {'a': '1'};
" 2>&1 | grep -oE 'BAD_ARGUMENTS|does not cover every output column' | sort -u
...
${CLICKHOUSE_LOCAL} --query="
INSERT INTO FUNCTION file('$WORKDIR/04321_err3.parquet', 'Parquet')
SELECT 1 AS a, 2 AS b
SETTINGS engine_file_truncate_on_insert = 1,
...
${CLICKHOUSE_LOCAL} --query="
INSERT INTO FUNCTION file('$WORKDIR/04321_err4.parquet', 'Parquet')
SELECT 1 AS a
SETTINGS engine_file_truncate_on_insert = 1,
...
${CLICKHOUSE_LOCAL} --query="
INSERT INTO FUNCTION file('$WORKDIR/04321_err5.parquet', 'Parquet')
SELECT 1 AS a
SETTINGS engine_file_truncate_on_insert = 1,
         output_format_parquet_column_field_ids = {'a': '-1'};
...
${CLICKHOUSE_LOCAL} --query="
INSERT INTO FUNCTION file('$WORKDIR/04321_err7.parquet', 'Parquet')
...
${CLICKHOUSE_LOCAL} --query="
INSERT INTO FUNCTION file('$WORKDIR/04321_err8.parquet', 'Parquet')
SELECT (1)::Tuple(b UInt8) AS a, 2::UInt8 AS \`a.b\`
SETTINGS engine_file_truncate_on_insert = 1,
