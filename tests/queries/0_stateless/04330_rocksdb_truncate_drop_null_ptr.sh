#!/usr/bin/env bash
# Tags: no-fasttest, use-rocksdb, no-parallel
...
# Populate an on-disk RocksDB directory through a writable table, then detach it
# so the files remain on disk.
...
$CLICKHOUSE_CLIENT -q "DETACH TABLE rdb_rw"
