...
-- optimize_functions_to_subcolumns never applied through a Distributed table; now shards apply
-- it inside WHERE/PREWHERE, where it cannot change the header expected by the initiator.
...
INSERT INTO t_map_shard SELECT
    ...
    CAST(concat('{"a":[{"b":"', repeat('w', 200), toString(number % 5), '","pad":"', repeat('x', 2000), '"}]}') AS JSON)
FROM numbers(10000);
...
SELECT count() FROM t_map_shard_dist WHERE tags['k1'] != '' SETTINGS log_comment = '04838_map_dist_plan_remote', prefer_localhost_replica = 0, serialize_query_plan = 1;
...
-- Chained rewrite: j.a[1].b -> arrayElement(j.a.:`Array(JSON)`.b, 1) on the shard.
SELECT count() FROM t_map_shard WHERE j.a[1].b = concat(repeat('w', 200), '3') SETTINGS log_comment = '04838_json_local';
SELECT count() FROM t_map_shard_dist WHERE j.a[1].b = concat(repeat('w', 200), '3') SETTINGS log_comment = '04838_json_dist', prefer_localhost_replica = 1, serialize_query_plan = 0;
SELECT count() FROM t_map_shard_dist WHERE j.a[1].b = concat(repeat('w', 200), '3') SETTINGS log_comment = '04838_json_dist_noopt', prefer_localhost_replica = 1, serialize_query_plan = 0, optimize_functions_to_subcolumns = 0;

SELECT 'header stability';

-- Only the WHERE occurrence may be rewritten on the shard; the intermediate header must not change.
...
SELECT substring(tags['k1'], 201) AS v, count() FROM t_map_shard_dist WHERE tags['k1'] != '' GROUP BY v ORDER BY v SETTINGS prefer_localhost_replica = 0, serialize_query_plan = 1;
...
-- Distributed queries must read several times less data than with the optimization disabled.
WITH
...
WHERE current_database = currentDatabase() AND type = 'QueryFinish' AND is_initial_query
    AND log_comment IN ('04838_map_dist_local', '04838_map_dist_remote', '04838_map_dist_plan', '04838_map_dist_plan_remote')
...
WITH
    (SELECT max(read_bytes) FROM system.query_log
        WHERE current_database = currentDatabase() AND type = 'QueryFinish' AND is_initial_query AND log_comment = '04838_json_dist_noopt') AS whole_json_bytes,
    (SELECT max(read_bytes) FROM system.query_log
        WHERE current_database = currentDatabase() AND type = 'QueryFinish' AND is_initial_query AND log_comment = '04838_json_local') AS local_bytes
SELECT
    log_comment,
    max(read_bytes) * 3 < whole_json_bytes AS reads_subcolumn,
    max(read_bytes) < 3 * local_bytes AS parity_with_local
FROM system.query_log
WHERE current_database = currentDatabase() AND type = 'QueryFinish' AND is_initial_query
    AND log_comment = '04838_json_dist'
GROUP BY log_comment;
