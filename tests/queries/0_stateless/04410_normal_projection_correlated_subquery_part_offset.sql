...
-- projection (forced by PREWHERE on _part_offset). The all-parts-from-projection rewrite used to leak
-- the unused i/j columns past the join's declared header, raising the "Invalid number of columns in
-- chunk pushed to OutputPort" logical error (an exception in release builds). Must run cleanly.
...
-- plan and must still see all parts. Assert projection-on equals projection-off. The equality alone also
-- holds when the projection is declined before the structure check, so pin the reachability for this
-- statement too: its plan shape differs from the one above (aggregation over an unrestrictive predicate,
-- 4 parts instead of 1), so the assertion above does not cover it. Settings match the statement below.
SELECT count() = 1 FROM (EXPLAIN projections = 1 SELECT sum(i), sum(j), count() FROM t_proj_corr PREWHERE _part_offset < (_part_offset + _part_starting_offset) WHERE ((SELECT _part_starting_offset) + _part_offset) >= 0 SETTINGS correlated_subqueries_substitute_equivalent_expressions = 0, correlated_subqueries_default_join_kind = 'left', optimize_use_projections = 1) WHERE explain ILIKE '%does not match the structure it replaces%';
