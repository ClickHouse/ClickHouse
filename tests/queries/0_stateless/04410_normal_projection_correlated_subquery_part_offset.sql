...
-- take: both reads served by the projection AND no read declined at the structure check. A read whose
-- surplus pass-throughs cannot be narrowed away is declined instead, which makes the second conjunct
-- fail; declining also drops a `ReadFromMergeTree (p)`, which makes the first fail. Both conjuncts
-- hold only when the narrowing ran.
SELECT countIf(explain ILIKE '%ReadFromMergeTree (p)%') = 2 AND countIf(explain ILIKE '%does not match the structure it replaces%') = 0 FROM (EXPLAIN projections = 1 SELECT i, j FROM t_proj_corr PREWHERE _part_offset < (_part_offset + _part_starting_offset) WHERE ((SELECT _part_starting_offset) + _part_offset) = 8 ORDER BY ALL SETTINGS correlated_subqueries_substitute_equivalent_expressions = 0, correlated_subqueries_default_join_kind = 'left');
...
SELECT countIf(explain ILIKE '%ReadFromMergeTree (p)%') = 2 AND countIf(explain ILIKE '%does not match the structure it replaces%') = 0 FROM (EXPLAIN projections = 1 SELECT sum(i), sum(j), count() FROM t_proj_corr PREWHERE _part_offset < (_part_offset + _part_starting_offset) WHERE ((SELECT _part_starting_offset) + _part_offset) >= 0 SETTINGS correlated_subqueries_substitute_equivalent_expressions = 0, correlated_subqueries_default_join_kind = 'left', optimize_use_projections = 1);
...
-- structure check. Pin the plan they must take instead: both reads served by the projection AND no read
-- declined. Counting, not an existence test: one plan can hold several reads with opposite verdicts.
SELECT countIf(explain ILIKE '%ReadFromMergeTree (p_by_session)%') = 2 AND countIf(explain ILIKE '%does not match the structure it replaces%') = 0 FROM (EXPLAIN projections = 1 SELECT payload, id, (SELECT timestamp) FROM t_proj_corr_events WHERE (organisation_id = reinterpretAsUUID(1)) AND (session_id = reinterpretAsUUID(0)) ORDER BY id, payload, timestamp SETTINGS read_in_order_two_level_merge_threshold = 1, query_plan_remove_unused_columns = 0, correlated_subqueries_use_in_memory_buffer = 0);
SELECT countIf(explain ILIKE '%ReadFromMergeTree (p_by_session)%') = 2 AND countIf(explain ILIKE '%does not match the structure it replaces%') = 0 FROM (EXPLAIN projections = 1 SELECT payload, id, (SELECT timestamp) FROM t_proj_corr_events WHERE (organisation_id = reinterpretAsUUID(1)) AND (session_id = reinterpretAsUUID(0)) ORDER BY id, payload, timestamp SETTINGS read_in_order_two_level_merge_threshold = 1, query_plan_remove_unused_columns = 0, correlated_subqueries_use_in_memory_buffer = 0, correlated_subqueries_default_join_kind = 'left');
