...
FROM (
    ...
    WHERE (event_date >= yesterday()) AND (event_time >= NOW() - INTERVAL '15 MINUTES') AND (current_database = currentDatabase()) AND (is_initial_query) AND (log_comment LIKE '04102_query_%') AND (type = 'QueryFinish')
