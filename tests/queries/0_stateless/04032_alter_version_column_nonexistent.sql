...
CREATE TABLE t_special_col (a UInt64, b DateTime) ENGINE = ReplacingMergeTree(b) ORDER BY a SETTINGS add_minmax_index_for_numeric_columns = 0;
...
CREATE TABLE t_special_col (a UInt64, sign Int8) ENGINE = CollapsingMergeTree(sign) ORDER BY a SETTINGS add_minmax_index_for_numeric_columns = 0;
...
CREATE TABLE t_special_col (a UInt64, b UInt64, deleted UInt8) ENGINE = ReplacingMergeTree(b, deleted) ORDER BY a SETTINGS add_minmax_index_for_numeric_columns = 0;
