...
-- `NATURAL JOIN` does not exist on this release branch, so the original test's `NATURAL JOIN`s
-- are spelled here as the equivalent `JOIN ... USING (a)`, which reproduces the same segfault.
