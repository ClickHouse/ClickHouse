...
SELECT value2 FROM t_direct_left LEFT JOIN t_direct_right ON t_direct_right.key == t_direct_left.k ORDER BY value2 SETTINGS join_algorithm = 'direct', use_join_disjunctions_push_down = 1;
SELECT 'materialize(1)';
SELECT value2 FROM t_direct_left LEFT JOIN t_direct_right ON t_direct_right.key == t_direct_left.k WHERE materialize(1) ORDER BY value2 SETTINGS join_algorithm = 'direct', use_join_disjunctions_push_down = 1;
...
SELECT count() FROM (SELECT value2 FROM t_direct_left LEFT JOIN t_direct_right ON t_direct_right.key == t_direct_left.k WHERE t_direct_left.k < 3 SETTINGS join_algorithm = 'direct', use_join_disjunctions_push_down = 1);
...
SELECT count(), sum(a), sum(b) FROM t_join_left LEFT JOIN t_join_right USING (k) WHERE materialize(1) SETTINGS use_join_disjunctions_push_down = 1;
SELECT count(), sum(a), sum(b) FROM t_join_left LEFT JOIN t_join_right USING (k) SETTINGS use_join_disjunctions_push_down = 1;
...
SELECT count() FROM t_join_left LEFT JOIN t_join_right USING (k) WHERE materialize(toNullable(NULL)) SETTINGS use_join_disjunctions_push_down = 1;
...
SELECT count(), sum(a), sum(b) FROM t_join_left FULL JOIN t_join_right USING (k) WHERE materialize(1) SETTINGS use_join_disjunctions_push_down = 1;
