...
CAPN_PROTO_FILE=$USER_FILES_PATH/02483_data.capnp
touch $CAPN_PROTO_FILE

SCHEMADIR=$(clickhouse-client --query "select * from file('02483_data.capnp', 'CapnProto', 'val1 char') settings format_schema='nonexist:Message'" 2>&1 | grep Exception | grep -oP "file \K.*(?=/nonexist.capnp)")
