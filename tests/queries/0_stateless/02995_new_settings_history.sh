...
# Baselines generated with v26.5.1 (pre-release)
# clickhouse local --query "select name, default from system.settings order by name format TSV" > 02995_settings_26_5_1.tsv
# clickhouse local --query "select name, value from system.merge_tree_settings order by name format TSV" > 02995_merge_tree_settings_settings_26_5_1.tsv

# Previous Antalya release baseline: v26.3.13.20001.altinityantalya (same format as 02995_settings_26_2_1.tsv). Settings listed there are considered known; keep empty or generate from previous Antalya build.

# To generate the previous Antalya release baseline:
# docker run --rm --entrypoint clickhouse <antalya_docker_image> local -q "select name, default from system.settings order by name format TSV" > 02995_settings_26_3_13_20001_antalya.tsv

$CLICKHOUSE_LOCAL --query "
    WITH old_settings AS
    (
        SELECT * FROM file('${CUR_DIR}/02995_settings_26_5_1.tsv', 'TSV', 'name String, default String')
    ),
    ...
    (
        SELECT * FROM file('${CUR_DIR}/02995_merge_tree_settings_settings_26_5_1.tsv', 'TSV', 'name String, default String')
    ),
    new_settings AS
    (
    ...
    (
        ...
        WHERE (name NOT IN (
            ...
            WHERE type = 'Session' AND (splitByChar('.', version)[1]::UInt64 > 26 OR (splitByChar('.', version)[1]::UInt64 == 26 AND splitByChar('.', version)[2]::UInt64 > 5) OR position(version, 'altinityantalya') > 0)
        ))
        UNION ALL
        (
            ...
            WHERE (name NOT IN (
                ...
                SELECT arrayJoin(tupleElement(changes, 'name'))
                FROM system.settings_changes
                WHERE type = 'MergeTree' AND (splitByChar('.', version)[1]::UInt64 > 26 OR (splitByChar('.', version)[1]::UInt64 == 26 AND splitByChar('.', version)[2]::UInt64 > 5) OR position(version, 'altinityantalya') > 0)
            ))
        )
        UNION ALL
        (
