...
# Baselines generated with v25.11.1 (pre-release)
# clickhouse local --query "select name, default from system.settings order by name format TSV" > 02995_settings_25_11_1.tsv
# clickhouse local --query "select name, value from system.merge_tree_settings order by name format TSV" > 02995_merge_tree_settings_settings_25_11_1.tsv
$CLICKHOUSE_LOCAL --query "
    WITH old_settings AS
    (
        SELECT * FROM file('${CUR_DIR}/02995_settings_25_11_1.tsv', 'TSV', 'name String, default String')
    ),
    old_merge_tree_settings AS
    (
        SELECT * FROM file('${CUR_DIR}/02995_merge_tree_settings_settings_25_11_1.tsv', 'TSV', 'name String, default String')
    ),
    new_settings AS
    (
    ...
    (
        ...
        WHERE (name NOT IN (
            ...
            WHERE type = 'Session' AND splitByChar('.', version)[1]::UInt64 > 25 OR (splitByChar('.', version)[1]::UInt64 == 25 AND splitByChar('.', version)[2]::UInt64 > 11)
        ))
        UNION ALL
        (
            ...
            WHERE (name NOT IN (
                ...
                SELECT arrayJoin(tupleElement(changes, 'name'))
                FROM system.settings_changes
                WHERE type = 'MergeTree' AND splitByChar('.', version)[1]::UInt64 > 25 OR (splitByChar('.', version)[1]::UInt64 == 25 AND splitByChar('.', version)[2]::UInt64 > 11)
            ))
        )
        UNION ALL
        (
