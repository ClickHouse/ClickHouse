...
$CLICKHOUSE_LOCAL --query "
    ...
    (
        ...
        WHERE (name NOT IN (
            ...
            WHERE type = 'Session' AND splitByChar('.', version)[1]::UInt64 > 26 OR (splitByChar('.', version)[1]::UInt64 == 26 AND splitByChar('.', version)[2]::UInt64 > 3)
        ))
        UNION ALL
        (
            ...
            WHERE (name NOT IN (
                ...
                SELECT arrayJoin(tupleElement(changes, 'name'))
                FROM system.settings_changes
                WHERE type = 'MergeTree' AND splitByChar('.', version)[1]::UInt64 > 26 OR (splitByChar('.', version)[1]::UInt64 == 26 AND splitByChar('.', version)[2]::UInt64 > 3)
            ))
        )
        UNION ALL
        (
