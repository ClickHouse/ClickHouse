...
${CLICKHOUSE_LOCAL} --max_bytes_before_external_group_by=1 --max_bytes_ratio_before_external_group_by=0 --max_block_size=10 -q "SELECT sum(ignore(*)) FROM (SELECT number, count() FROM numbers(1000) GROUP BY number)"
echo
