...
-- The pre-aggregation key typing in `ExpressionAnalyzer`/`InterpreterSelectQuery` (legacy)
-- and `PlannerExpressionAnalysis` (analyzer) is also affected, so we run with both planners.
...
SELECT 'analyzer=1, LC(String), ROLLUP';
