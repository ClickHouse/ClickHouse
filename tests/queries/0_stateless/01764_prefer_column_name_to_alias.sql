...
SELECT max(number) AS number, sum(number) AS sum FROM numbers(10) ORDER BY number ASC; -- { serverError 215 }
set prefer_column_name_to_alias = 2;
SELECT max(number) AS number, sum(number) AS sum FROM numbers(10) ORDER BY number ASC;
set prefer_column_name_to_alias = 3;
SELECT max(number) AS number, sum(number) AS sum FROM numbers(10) ORDER BY number ASC;

set prefer_column_name_to_alias = 2;
SELECT max(number) AS number, sum(number) AS sum FROM numbers(10) GROUP BY number ORDER BY number ASC;
set prefer_column_name_to_alias = 3;
SELECT max(number) AS number, sum(number) AS sum FROM numbers(10) GROUP BY number ORDER BY number ASC; -- { serverError 46 }
