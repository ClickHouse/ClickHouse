...
SELECT count() FROM t_text_index_like_overflow WHERE message LIKE '%pa%'
    SETTINGS text_index_like_max_postings_to_read = 1;
SELECT count() FROM t_text_index_like_overflow WHERE message LIKE '%pa%'
    SETTINGS use_skip_indexes = 0;

-- NOT LIKE must also be correct after fallback.
SELECT 'NOT LIKE with overflow';
SELECT count() FROM t_text_index_like_overflow WHERE message NOT LIKE '%pa%'
    SETTINGS text_index_like_max_postings_to_read = 1;
SELECT count() FROM t_text_index_like_overflow WHERE message NOT LIKE '%pa%'
    SETTINGS use_skip_indexes = 0;
...
SELECT count() FROM t_text_index_like_overflow WHERE message LIKE '%p%'
    SETTINGS text_index_like_max_postings_to_read = 1;
SELECT count() FROM t_text_index_like_overflow WHERE message LIKE '%p%'
    SETTINGS use_skip_indexes = 0;
...
SELECT count() FROM t_text_index_like_overflow WHERE message LIKE '%pa%'
    SETTINGS text_index_like_max_postings_to_read = 10000;
SELECT count() FROM t_text_index_like_overflow WHERE message LIKE '%pa%'
    SETTINGS use_skip_indexes = 0;
