...
-- Huge (but finite) offset puts every real key past the safe window, with a bin sitting exactly on the
-- window edge (2^30): the empty intersection must still reject it, not accept the boundary key.
