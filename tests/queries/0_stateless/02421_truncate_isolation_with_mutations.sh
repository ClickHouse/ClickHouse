...
function concurrent_optimize_table_not_start()
{
    ...
    $CLICKHOUSE_CLIENT -q "select count(), _part from tt group by _part"
}
...
function drop_parts_which_already_outdated()
{
    ...
    tx 69             "select 'before optimize', count(), _part from tt group by _part"
    ...
    tx 69             "select 'after optimize', count(), _part from tt group by _part"
    tx 69             "alter table tt drop partition id 'all'" | grep -Eo "SERIALIZATION_ERROR" | uniq
    tx 69             "rollback"

    tx 62 "rollback"

    $CLICKHOUSE_CLIENT -q "select 'at the end', count(), _part from tt group by _part"
}
...
function unable_drop_one_part_which_outdated_but_visible()
{
    ...
    tx 79             "select 'before optimize', count(), _part from tt group by _part"

    tx 71 "begin transaction"
    tx 71 "optimize table tt final /*all_1_2_3*/"

    tx 79             "select 'after optimize', count(), _part from tt group by _part"
...
function drop_one_part_which_outdated_and_reverted()
{
    ...
    tx 89             "select 'before optimize', count(), _part from tt group by _part"

    tx 81 "begin transaction"
    tx 81 "optimize table tt final /*all_1_2_2*/"

    tx 89             "select 'after optimize', count(), _part from tt group by _part"
    tx 81 "rollback"

    tx 89             "select 'after rollback', count(), _part from tt group by _part"
...
function drop_one_part_which_outdated_and_reverted_no_name_intersection()
{
    ...
    tx 99             "select 'before optimize', count(), _part from tt group by _part"

    tx 91 "begin transaction"
    tx 91 "optimize table tt final /*all_1_2_1*/"

    tx 99             "select 'after optimize', count(), _part from tt group by _part"
    tx 91 "rollback"

    tx 99             "select 'after rollback', count(), _part from tt group by _part"
