...
# Request a couple of non-default backup-specific settings.
${CLICKHOUSE_CLIENT} --query "
backup table ${CLICKHOUSE_DATABASE}.\`04338_t\` to $backup_name
settings deduplicate_files=0, allow_s3_native_copy=0;
...
# engine-specific (e.g. S3) settings, so engine_settings is empty.
${CLICKHOUSE_CLIENT} -m --query "
select
