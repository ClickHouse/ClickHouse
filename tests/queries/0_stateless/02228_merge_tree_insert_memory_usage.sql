...
-- Compact parts keep a ~1MiB compressor per delayed part, Wide parts with adaptive write buffers do not,
-- so the 30MiB assertions below require the part format to stay Compact.
drop table if exists data_02228;
create table data_02228 (key1 UInt32, sign Int8, s UInt64) engine = CollapsingMergeTree(sign) order by (key1) partition by key1 % 100 settings auto_statistics_types = '', min_bytes_for_wide_part = 10485760, add_minmax_index_for_numeric_columns = 0;
...
create table data_rep_02228 (key1 UInt32, sign Int8, s UInt64) engine = ReplicatedCollapsingMergeTree('/clickhouse/{database}', 'r1', sign) order by (key1) partition by key1 % 100 settings auto_statistics_types = '', min_bytes_for_wide_part = 10485760, add_minmax_index_for_numeric_columns = 0;
