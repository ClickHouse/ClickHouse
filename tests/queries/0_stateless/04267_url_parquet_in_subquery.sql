...
-- Tag no-fasttest: depends on MinIO via s3_conn for write + HTTP read.
...
--
-- IN (subquery) on Parquet over URL must push down to page index / row-group stats / bloom filter. Covers ReadFromURL::applyFilters
-- The file() variant is 04267_parquet_in_subquery; the s3() variant is 04267_s3_parquet_in_subquery.

INSERT INTO FUNCTION s3(s3_conn,
    filename = '04267_url_parquet_in_subquery/' || currentDatabase() || '.parquet',
    format = Parquet,
...
SELECT count() FROM url('http://localhost:11111/test/04267_url_parquet_in_subquery/' || currentDatabase() || '.parquet', Parquet)
WHERE id IN (100, 100000, 199900)
SETTINGS enable_filesystem_cache = 0, log_comment = '100743_url_literal';

SELECT count() FROM url('http://localhost:11111/test/04267_url_parquet_in_subquery/' || currentDatabase() || '.parquet', Parquet)
WHERE id IN (SELECT arrayJoin([100, 100000, 199900])::UInt64)
SETTINGS enable_filesystem_cache = 0, log_comment = '100743_url_subquery';

WITH keys AS (SELECT arrayJoin([100::UInt64, 100000::UInt64, 199900::UInt64]) AS id)
SELECT count() FROM url('http://localhost:11111/test/04267_url_parquet_in_subquery/' || currentDatabase() || '.parquet', Parquet)
WHERE id IN (SELECT id FROM keys)
SETTINGS enable_filesystem_cache = 0, log_comment = '100743_url_cte';
...
SETTINGS enable_filesystem_cache = 0, log_comment = '100743_url_or_subqueries';
...
FROM (EXPLAIN SELECT count() FROM url('http://localhost:11111/test/04267_url_parquet_in_subquery/' || currentDatabase() || '.parquet', Parquet)
      WHERE id IN (SELECT arrayJoin([100])::UInt64));
...
WHERE event_date >= yesterday()
  ...
  AND log_comment IN ('100743_url_literal', '100743_url_subquery', '100743_url_cte', '100743_url_or_subqueries')
