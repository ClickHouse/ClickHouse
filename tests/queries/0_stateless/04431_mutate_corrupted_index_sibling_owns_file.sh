...
make_corrupted_part () {
    ...
    ${CLICKHOUSE_CLIENT} -q "INSERT INTO ${tbl} (k, s, w, u, m) SELECT number, concat('hello', number % 50, ' world', number % 50), number, number, number FROM numbers(500)"
...
# --- Path A: full-part rewrite (DROP COLUMN of the unrelated column m) ---
# `DROP COLUMN` affects all columns, so the mutation takes `MutateAllPartColumnsTask` and the
# classification drives `need_recalculate`: the corrupted index must be REBUILT.
#
...
# The corrupted file must not be carried into the new part. Both assertions below were measured to
# discriminate: with the classification counting the sibling's file as evidence of health, the
# orphan is hardlinked forward and `CHECK TABLE` returns 0.
echo "A_orphan_after_drop_column:"
orphan_on_disk t_full
echo "A_check_table:"
