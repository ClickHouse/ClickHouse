...
-- join with a filter, the outer kinds to the block nested loop join.
...
SELECT 'dynamic outer (keys disallowed)', count() FROM (SELECT id, CAST(f, 'Dynamic') AS d, y FROM ttk_l) l LEFT JOIN (SELECT id, CAST(f, 'Dynamic') AS d, y FROM ttk_r) r ON l.d < r.d AND l.y > r.y;
