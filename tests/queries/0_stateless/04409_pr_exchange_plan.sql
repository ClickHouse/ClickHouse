...
DROP TABLE IF EXISTS t_pr_exchange;

CREATE TABLE t_pr_exchange (a UInt64, b UInt64) ENGINE = MergeTree ORDER BY a;
INSERT INTO t_pr_exchange SELECT number, number % 10 FROM numbers(100000);

SET enable_parallel_replicas = 1;
SET parallel_replicas_for_non_replicated_merge_tree = 1;
SET max_parallel_replicas = 3;
SET cluster_for_parallel_replicas = 'test_cluster_one_shard_three_replicas_localhost';
SET parallel_replicas_exchange_plan = 1;
...
SELECT count(), sum(b), min(a), max(a) FROM t_pr_exchange WHERE a > 5;
SELECT b, count() FROM t_pr_exchange GROUP BY b ORDER BY b;
SELECT count() FROM t_pr_exchange;
...
FROM (EXPLAIN SELECT sum(b) FROM t_pr_exchange WHERE a > 5);

DROP TABLE t_pr_exchange;
