-- Tags: no-fasttest

DROP USER IF EXISTS test_user_02867;

CREATE USER test_user_02867 IDENTIFIED WITH ssh_key BY KEY 'clickhouse' ALGORITHM 'ssh-rsa'; -- { serverError 703 }
CREATE USER test_user_02867 IDENTIFIED WITH ssh_key BY KEY 'clickhouse' ALGORITHM 'clickhouse'; -- { serverError 703 }
CREATE USER test_user_02867 IDENTIFIED WITH ssh_key BY KEY 'key1' ALGORITHM 'ssh-rsa', KEY 'key2' ALGORITHM 'ssh-rsa'; -- { serverError 703 }
CREATE USER test_user_02867 IDENTIFIED WITH ssh_key
BY KEY ''
ALGORITHM 'ssh-rsa';

SHOW CREATE USER test_user_02867;

DROP USER test_user_02867;
