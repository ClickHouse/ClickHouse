...
SELECT id, val FROM 03408_memory ORDER BY id, val LIMIT 1 BY id LIMIT 3 FORMAT JsonCompact;

SELECT '';
SELECT '-- Assert rows_before_limit for memory HAVING + ORDER BY + LIMIT BY + LIMIT';
SELECT id, val FROM 03408_memory GROUP BY id, val HAVING id < 7 ORDER BY id, val DESC LIMIT 1 BY id LIMIT 3 FORMAT JsonCompact;
