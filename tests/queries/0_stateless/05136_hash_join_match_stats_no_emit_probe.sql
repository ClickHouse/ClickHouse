...
CREATE TABLE right_wide (k UInt64, v1 Int64, v2 UInt64, v3 UInt8) ENGINE = MergeTree ORDER BY tuple();
INSERT INTO right_wide SELECT number, number, number, number FROM numbers(20000);

-- Separate table for `parallel_hash`: `join_algorithm` is not part of the match stats cache key.
CREATE TABLE right_wide_ph (k UInt64, v1 Int64, v2 UInt64, v3 UInt8) ENGINE = MergeTree ORDER BY tuple();
INSERT INTO right_wide_ph SELECT number, number, number, number FROM numbers(20000);

SELECT r.v1, r.v2, r.v3 FROM (SELECT number % 20000 AS k FROM numbers(200000)) p JOIN right_wide r ON p.k = r.k FORMAT Null
...
SELECT r.v1, r.v2, r.v3 FROM (SELECT number % 20000 AS k FROM numbers(200000)) p JOIN right_wide_ph r ON p.k = r.k FORMAT Null
