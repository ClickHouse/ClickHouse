...
for disk in 's3_disk' 'local_disk' 'azure'; do
    ...
    # Pin the codec to `LZ4` so the cached on-disk byte sizes do not depend on the server's default compression codec.
    $CLICKHOUSE_CLIENT --echo --query "CREATE TABLE test_02241 (key UInt32 CODEC(LZ4), value String CODEC(LZ4)) Engine=MergeTree() ORDER BY key SETTINGS disk=disk(type='cache', path='$cache_path', disk='$disk', max_size=10_000_000_000, cache_on_write_operations=1), min_bytes_for_wide_part = 10485760, compress_marks=false, compress_primary_key=false, min_bytes_for_full_part_storage=0, ratio_of_defaults_for_sparse_serialization = 1, write_marks_for_substreams_in_compact_parts=1, serialization_info_version='basic', auto_statistics_types = ''"
