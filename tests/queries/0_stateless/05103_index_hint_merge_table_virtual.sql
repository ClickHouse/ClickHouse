...
-- `indexHint` contributes a second input for the same virtual column, and the block used to evaluate
-- which tables a `Merge` engine must read held each virtual column once, so the expression could not
-- be executed and the query failed with `NOT_FOUND_COLUMN_IN_BLOCK`.
