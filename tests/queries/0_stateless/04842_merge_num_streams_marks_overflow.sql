...
-- zero. The stream count is representable, passes the planner's bounds check and reaches
...
SELECT id FROM merge(currentDatabase(), '^t_marks_overflow$') ORDER BY id DESC SETTINGS optimize_read_in_order = 1;
...
SELECT count() FROM merge(currentDatabase(), '^t_marks_overflow$') SETTINGS optimize_read_in_order = 0;
