...
select '----- pr, serialize_query_plan=0 -----';
...
explain actions=1, distributed=1 SELECT sum(number) from test_parallel_replicas group by bitAnd(number, 3) settings serialize_query_plan=1;
