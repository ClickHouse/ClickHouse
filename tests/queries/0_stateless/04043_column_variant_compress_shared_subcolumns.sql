-- Tags: no-asan
-- ^^ Pre-existing bug: ColumnVariant sub-columns are shared via COW between pipeline columns.
-- When prepareForSquashing modifies shared sub-columns, it corrupts ColumnVariant consistency
-- (variant total sizes != discriminators size), causing an exception during compress/decompress
-- in the cross join path. This test documents the issue. Remove the no-asan tag when fixed.
-- See: https://github.com/ClickHouse/ClickHouse/issues/99920
