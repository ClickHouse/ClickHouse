#!/usr/bin/env bash
# Tags: long, no-parallel, no-shared-merge-tree, no-msan, no-azure-blob-storage, no-random-detach
...
for ((i=0; i<16; i++)) do
    ...
    ${CLICKHOUSE_CURL} -sS "${CLICKHOUSE_URL}" -d "CREATE TABLE src_$i (p UInt64, k UInt64, v UInt64) ENGINE=$engine
          PARTITION BY p % 10 ORDER BY k" 2>&1| grep -Pv "Retrying createReplica|created by another server at the same moment, will retry|is already started to be removing" 2>&1 &
done
wait
...
function optimize_thread()
{
    ...
    do
        ...
        ${CLICKHOUSE_CURL} -sS "${CLICKHOUSE_URL}" -d "OPTIMIZE TABLE ${TABLE}_$REPLICA" >&/dev/null
        sleep 0.$RANDOM;
    done
}
