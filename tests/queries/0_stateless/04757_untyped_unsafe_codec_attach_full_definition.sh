...
# there — the same way CREATE rejects it — instead of being silently dropped by the
# load-path sanitization.
...
# `T64` cannot compress an untyped stream, and no codec gate is involved in that.
# -m1 because the error message may contain the error code name multiple times.
$CLICKHOUSE_CLIENT -q "ATTACH TABLE t_t64_attach_full UUID '${UUID}' (x UInt64) ENGINE = MergeTree ORDER BY x SETTINGS default_compression_codec = 'T64';" 2>&1 | grep -m 1 -o -F 'BAD_ARGUMENTS'
$CLICKHOUSE_CLIENT -q "ATTACH TABLE t_t64_attach_full UUID '${UUID}' (x UInt64) ENGINE = MergeTree ORDER BY x SETTINGS marks_compression_codec = 'T64';" 2>&1 | grep -m 1 -o -F 'BAD_ARGUMENTS'
$CLICKHOUSE_CLIENT -q "ATTACH TABLE t_t64_attach_full UUID '${UUID}' (x UInt64) ENGINE = MergeTree ORDER BY x SETTINGS primary_key_compression_codec = 'T64';" 2>&1 | grep -m 1 -o -F 'BAD_ARGUMENTS'
