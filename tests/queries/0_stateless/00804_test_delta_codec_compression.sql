...
) ENGINE MergeTree() ORDER BY tuple() SETTINGS optimize_row_order_if_no_order_by = 0, min_bytes_for_wide_part = 0, compress_marks = false, compress_primary_key = false, ratio_of_defaults_for_sparse_serialization = 1, enable_block_number_column = 0, serialization_info_version = 'basic', auto_statistics_types = '', add_minmax_index_for_numeric_columns=0;

CREATE TABLE default_codec_synthetic
(
    id UInt64 Codec(ZSTD(3))
) ENGINE MergeTree() ORDER BY tuple() SETTINGS optimize_row_order_if_no_order_by = 0, min_bytes_for_wide_part = 0, compress_marks = false, compress_primary_key = false, ratio_of_defaults_for_sparse_serialization = 1, enable_block_number_column = 0, auto_statistics_types = '', add_minmax_index_for_numeric_columns=0;
