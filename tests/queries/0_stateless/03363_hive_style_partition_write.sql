...
CREATE TABLE t_03363_s3_sink_read (year UInt16, country String, counter UInt8)
ENGINE = S3(s3_conn, filename = 't_03363_s3_sink_root/**.parquet', format = Parquet);

-- distinct because minio isn't cleaned up
select distinct on (counter) replaceRegexpAll(_path, '/[0-9]+\\.parquet', '/<snowflakeid>.parquet') AS _path, counter from t_03363_s3_sink_read order by counter SETTINGS use_hive_partitioning=1, input_format_parquet_use_native_reader=0;
