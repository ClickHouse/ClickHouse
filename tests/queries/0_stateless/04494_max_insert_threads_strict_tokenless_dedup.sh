...
#
# The unified deduplication id of a token-less block is hash(data_hash : source_block_number) for a
# synchronous insert, so the source block number is part of the id. Two identical squashed blocks that
# land on different branches then both get source number 0 and the same data hash, i.e. an identical
# id, which MergeTreeSink treats as a duplicate and skips - silently dropping rows of a single INSERT.
...
# The strict token-less INSERT falls back to a single insert stream: one sink.
...
# strict block limits squash it into ten identical blocks. Those ten blocks are kept only because
# their source block numbers (0..9) differ; if the fan-out restarted numbering per branch, identical
# blocks would collide on source number 0 and dedup would drop rows.
