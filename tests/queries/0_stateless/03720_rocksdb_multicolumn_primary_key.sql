...
-- references it as non-Nullable, which used to throw a LOGICAL_ERROR in HashJoin::buildAdditionalFilter.
-- enable_analyzer = 1: the old analyzer rejects non-equi ON conjuncts (INVALID_JOIN_ON_EXPRESSION).
SELECT l.k1, l.k2, r.k1, r.k2 FROM 03720_join_str_left AS l
LEFT JOIN 03720_join_str_right AS r ON l.k1 = r.k1 AND l.k2 != r.k2
ORDER BY l.k1, l.k2 SETTINGS join_use_nulls = 1, enable_analyzer = 1;

SELECT COUNT(DISTINCT *) FROM 03720_join_str_left AS l
LEFT JOIN 03720_join_str_right AS r ON l.k1 = r.k1 AND l.k2 != r.k2 SETTINGS join_use_nulls = 1, enable_analyzer = 1;
