...
# column `CODEC(...)` clause must require its `enable_<family>_codec` setting there as well, unlike a short
...
$CLICKHOUSE_CLIENT --enable_zxc_codec 0 -q "ATTACH TABLE t_zxc_attach_column UUID '${UUID}' (x UInt64 CODEC(ZXC)) ENGINE = MergeTree ORDER BY tuple();" 2>&1 | grep -m 1 -o -F 'BAD_ARGUMENTS'
$CLICKHOUSE_CLIENT --enable_zxc_codec 0 -q "ATTACH TABLE t_zxc_attach_column UUID '${UUID}' (x UInt64 CODEC(Delta, ZXC)) ENGINE = MergeTree ORDER BY tuple();" 2>&1 | grep -m 1 -o -F 'BAD_ARGUMENTS'
...
$CLICKHOUSE_CLIENT --allow_repeated_settings --enable_zxc_codec 1 --send_logs_level fatal -q "ATTACH TABLE t_zxc_attach_column UUID '${UUID}' (x UInt64 CODEC(ZXC)) ENGINE = MergeTree ORDER BY tuple();"
...
$CLICKHOUSE_CLIENT --enable_zxc_codec 0 -q "DETACH TABLE t_zxc_attach_column;"
$CLICKHOUSE_CLIENT --enable_zxc_codec 0 -q "ATTACH TABLE t_zxc_attach_column;"
$CLICKHOUSE_CLIENT -q "SELECT count(), sum(x) FROM t_zxc_attach_column;"
