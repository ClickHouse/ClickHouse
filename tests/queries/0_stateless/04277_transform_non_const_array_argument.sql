...
SELECT transform(number % 2, [0, 1], [number, number + 10]) FROM numbers(3); -- { serverError ILLEGAL_COLUMN }

SELECT transform(number % 2, [0, 1], materialize([0, 10])) FROM numbers(3); -- { serverError ILLEGAL_COLUMN }
