-- Preserve materialized-view attribution through same-query `Alias` forwarding, but reset it when
-- a local `Distributed` shard starts a separate forwarded `INSERT`.
