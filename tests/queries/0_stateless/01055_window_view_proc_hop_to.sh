...
CREATE WINDOW VIEW ${db}.wv TO ${db}.dst AS SELECT count(a) AS count FROM ${db}.mt GROUP BY hop(timestamp, INTERVAL '5' SECOND, INTERVAL '5' SECOND, 'US/Samoa') AS wid;

INSERT INTO ${db}.mt VALUES (1, now('US/Samoa') + 1);
EOF
