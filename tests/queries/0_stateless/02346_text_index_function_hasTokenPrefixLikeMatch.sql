...
SELECT arraySort(groupArray(id)) FROM tab WHERE arrayExists(t -> startsWith(t, 'charg'), tokens(msg)) SETTINGS optimize_rewrite_array_exists_over_tokens = 0;
...
SELECT arraySort(groupArray(id)) FROM tab WHERE arrayExists(t -> like(t, '%harg%'), tokens(msg)) SETTINGS optimize_rewrite_array_exists_over_tokens = 0;
...
SELECT arraySort(groupArray(id)) FROM tab WHERE arrayExists(t -> match(t, '^[0-9]{5}$'), tokens(msg)) SETTINGS optimize_rewrite_array_exists_over_tokens = 0;
