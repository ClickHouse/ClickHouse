...
select this is too many words for an alias; -- { error SYNTAX_ERROR }
OPTIMIZE TABLE values_01564 DEDUPLICATE BY; -- { error SYNTAX_ERROR }
OPTIMIZE TABLE values_01564 DEDUPLICATE BY a EXCEPT a; -- { error SYNTAX_ERROR }
select 'a' || distinct one || 'c' from system.one; -- { error SYNTAX_ERROR }
