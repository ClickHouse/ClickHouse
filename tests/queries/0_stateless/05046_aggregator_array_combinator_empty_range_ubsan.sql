...
-- The constant-key branch no longer creates a group for a zero-row block, so the result is empty and
-- the same with the setting off; the signed-index cast in `addBatchSinglePlace` is now unreachable.
