...
insert into test_3 select number, [number] from numbers(0, 1000000);
...
FROM test_3 AS a, test_3 AS b, test_3 AS c, test_3 AS d
WHERE (a.id = (b.id + 1)) AND (b.id = (c.id + 100)) AND ((c.id % 11111) = ((d.id % 12345) + 17))
SETTINGS make_distributed_plan = 1, enable_parallel_replicas = 0, default_shuffle_join_bucket_count = 20;
