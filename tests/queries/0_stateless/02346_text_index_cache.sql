...
CREATE VIEW text_index_dictionary_block_cache_stats AS (
  SELECT
    concat('cache_hits = ', ProfileEvents['TextIndexDictionaryBlockCacheHits'], ', cache_misses = ', ProfileEvents['TextIndexDictionaryBlockCacheMisses'])
...
CREATE VIEW text_index_posting_list_cache_stats AS (
  SELECT
    concat('cache_hits = ', ProfileEvents['TextIndexPostingListCacheHits'], ', cache_misses = ', ProfileEvents['TextIndexPostingListCacheMisses'])
  FROM system.query_log
  WHERE query_kind ='Select'
      AND current_database = currentDatabase()
      AND endsWith(trimRight(query), concat('hasAnyTokens(message, [', {filter:String}, ']);'))
...
SYSTEM FLUSH LOGS query_log;
SELECT * FROM text_index_posting_list_cache_stats(filter = '\'text_pl_1\', \'text_pl_2\'');

SELECT '--- cache hits for the deserialized posting lists.';
SELECT count() FROM tab WHERE hasAnyTokens(message, ['text_pl_2', 'text_pl_3']);

SYSTEM FLUSH LOGS query_log;
SELECT * FROM text_index_posting_list_cache_stats(filter = '\'text_pl_2\', \'text_pl_3\'');
