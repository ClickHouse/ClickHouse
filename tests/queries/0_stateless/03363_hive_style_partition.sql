...
-- Should not fail while schema does not contain partition columns
CREATE TABLE s3_table_non_full_schema (key UInt64) engine=S3(s3_conn, filename='half_baked/**.parquet', format=Parquet) SETTINGS use_hive_partitioning=1;
SELECT DISTINCT * FROM s3_table_non_full_schema;
