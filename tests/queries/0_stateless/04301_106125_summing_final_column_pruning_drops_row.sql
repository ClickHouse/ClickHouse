...
-- Bot review (2026-07-08): PREWHERE-before-FINAL path. getSummingColumnsRequiredForFinal fixes the storage
-- read set, but PREWHERE column survival was decided by StorageInMemoryMetadata::getColumnsRequiredForFinal()
-- (sorting key only), so a PREWHERE touching only a subset of summing columns stripped the rest before
-- SummingSortedTransform, reintroducing the dropped-row bug. spreadMarkRangesAmongStreamsFinal now restores
-- the summing columns as PREWHERE outputs (alongside sign/version/is_deleted). The c0=0 row has v_keep=1
-- (non-zero) and v_zero=0; a PREWHERE on v_keep must not drop it. A real merge keeps all 6 rows.
DROP TABLE IF EXISTS s_106125_prewhere;
CREATE TABLE s_106125_prewhere (c0 UInt32, v_keep Int32, v_zero UInt32)
ENGINE = SummingMergeTree ORDER BY c0;

INSERT INTO s_106125_prewhere VALUES (0, 1, 0), (1, 1, 1), (2, 1, 2);
INSERT INTO s_106125_prewhere VALUES (3, 1, 3), (4, 1, 4), (5, 1, 5);

-- PREWHERE on the summing column that keeps the row (v_keep). Pre-fix this dropped c0=0.
SELECT 'prewhere-v_keep-projection-only-c0';
SELECT c0 FROM s_106125_prewhere FINAL PREWHERE v_keep >= 0 ORDER BY c0;

-- Same under the old analyzer (both planners route PREWHERE survival through the same read path).
SELECT 'prewhere-v_keep-old-analyzer';
SELECT c0 FROM s_106125_prewhere FINAL PREWHERE v_keep >= 0 ORDER BY c0
SETTINGS enable_analyzer = 0;

-- WHERE moved to PREWHERE under FINAL (optimize_move_to_prewhere_if_final).
SELECT 'prewhere-v_keep-where-moved';
SELECT c0 FROM s_106125_prewhere FINAL WHERE v_keep >= 0 ORDER BY c0
SETTINGS optimize_move_to_prewhere = 1, optimize_move_to_prewhere_if_final = 1;

-- PREWHERE on v_zero (0 for the c0=0 row) also keeps it: v_keep is still non-zero in the full row.
SELECT 'prewhere-v_zero-projection-only-c0';
SELECT c0 FROM s_106125_prewhere FINAL PREWHERE v_zero >= 0 ORDER BY c0;

DROP TABLE s_106125_prewhere;
