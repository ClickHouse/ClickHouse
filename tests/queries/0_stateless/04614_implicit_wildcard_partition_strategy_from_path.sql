...
DROP TABLE IF EXISTS test_04614_implicit_wildcard;

SET file_like_engine_default_partition_strategy = 'hive';

CREATE TABLE test_04614_implicit_wildcard (a UInt64, b String)
ENGINE = S3(s3_conn, filename='test_04614_implicit_wildcard_{_partition_id}', format=Parquet)
PARTITION BY a;

SET s3_truncate_on_insert = 1;
INSERT INTO test_04614_implicit_wildcard VALUES (1, 'a'), (22, 'b'), (333, 'c');
SELECT a, b FROM s3(s3_conn, filename='test_04614_implicit_wildcard_*', format=Parquet) ORDER BY a;
...
ENGINE = S3(s3_conn, filename='test_04614_explicit_hive_{_partition_id}', format=Parquet, partition_strategy='hive')
...
INSERT INTO FUNCTION s3(s3_conn, filename='test_04614_implicit_wildcard_fn_{_partition_id}', format=Parquet)
PARTITION BY a SELECT 55 AS a, 'e' AS b;
SELECT a, b FROM s3(s3_conn, filename='test_04614_implicit_wildcard_fn_*', format=Parquet) ORDER BY a;
