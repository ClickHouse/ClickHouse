DROP TABLE IF EXISTS base32; CREATE TABLE base32 (i UInt32 CODEC(NONE), f Float32 CODEC(NONE)) Engine = MergeTree ORDER BY i;
...
TRUNCATE TABLE base64; INSERT INTO base64 SELECT number,
    CASE number % 50
        ...
        WHEN 3 THEN reinterpretAsFloat64(0x7FF8000000000001)  -- quiet NaN variant
        -- Infinities
        WHEN 4 THEN inf
        WHEN 5 THEN -inf
        -- Zero variants
        ...
        ELSE round(number * 0.1 + sin(number), 2)
    END
FROM numbers(4000);
TRUNCATE TABLE alp64; INSERT INTO alp64 SELECT i, f FROM base64;
SELECT count(), sum(bin(a.f) <> bin(b.f)) FROM base64 AS b INNER JOIN alp64 AS a USING i;
...
TRUNCATE TABLE base32; INSERT INTO base32 SELECT number,
    CASE number % 36
        ...
        WHEN 3 THEN reinterpretAsFloat32(0x7FC00001)  -- quiet NaN variant
        -- Infinities
...
TRUNCATE TABLE base64; INSERT INTO base64 SELECT number, round(number * 0.1 + sin(number), 2) FROM numbers(1024);
TRUNCATE TABLE alp64; INSERT INTO alp64 SELECT i, f FROM base64;
SELECT 'Size 1024', count(), sum(bin(a.f) <> bin(b.f)) FROM base64 AS b INNER JOIN alp64 AS a USING i;

TRUNCATE TABLE base64; INSERT INTO base64 SELECT number, round(number * 0.1 + sin(number), 2) FROM numbers(1025);
TRUNCATE TABLE alp64; INSERT INTO alp64 SELECT i, f FROM base64;
SELECT 'Size 1025', count(), sum(bin(a.f) <> bin(b.f))FROM base64 AS b INNER JOIN alp64 AS a USING i;

TRUNCATE TABLE base64; INSERT INTO base64 SELECT number, round(number * 0.1 + sin(number), 2) FROM numbers(2047);
TRUNCATE TABLE alp64; INSERT INTO alp64 SELECT i, f FROM base64;
SELECT 'Size 2047', count(), sum(bin(a.f) <> bin(b.f)) FROM base64 AS b INNER JOIN alp64 AS a USING i;

TRUNCATE TABLE base64; INSERT INTO base64 SELECT number, round(number * 0.1 + sin(number), 2) FROM numbers(2048);
TRUNCATE TABLE alp64; INSERT INTO alp64 SELECT i, f FROM base64;
SELECT 'Size 2048', count(), sum(bin(a.f) <> bin(b.f)) FROM base64 AS b INNER JOIN alp64 AS a USING i;

TRUNCATE TABLE base64; INSERT INTO base64 SELECT number, round(number * 0.1 + sin(number), 2) FROM numbers(2049);
TRUNCATE TABLE alp64; INSERT INTO alp64 SELECT i, f FROM base64;
...
TRUNCATE TABLE base32; INSERT INTO base32 SELECT number, toFloat32(round(number * 0.1 + sin(number), 2)) FROM numbers(1025);
TRUNCATE TABLE alp32; INSERT INTO alp32 SELECT i, f FROM base32;
...
TRUNCATE TABLE base64; INSERT INTO base64 SELECT number,
    if((number / 1024) % 2 = 0,
        ...
        sin(number) * cos(number) * log(toFloat64(number))
    )
FROM numbers(4096);
TRUNCATE TABLE alp64; INSERT INTO alp64 SELECT i, f FROM base64;
SELECT count(), sum(bin(a.f) <> bin(b.f)) FROM base64 AS b INNER JOIN alp64 AS a USING i;
