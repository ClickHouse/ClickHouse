...
-- New columns on system.functions: min_arguments, max_arguments, is_deterministic, higher_order_function.

SELECT '-- columns exist with the expected types';
SELECT name, type FROM system.columns
WHERE database = 'system' AND table = 'functions'
    AND name IN ('min_arguments', 'max_arguments', 'is_deterministic', 'higher_order_function')
ORDER BY name;

SELECT '-- non-variadic deterministic function: arity is fixed and equal min == max';
SELECT name, min_arguments, max_arguments, is_deterministic, higher_order_function
...
SELECT name, case_insensitive, min_arguments, max_arguments, is_deterministic, higher_order_function
