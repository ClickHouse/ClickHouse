...
SELECT query FROM system.query_log WHERE event_date >= yesterday() AND event_time >= now() - 600 AND query_id = (SELECT * FROM tmp) AND current_database = currentDatabase() LIMIT 1;
DROP TABLE tmp;

CREATE TABLE tmp ENGINE = TinyLog AS SELECT initialQueryID();
SYSTEM FLUSH LOGS query_log;
SELECT query FROM system.query_log WHERE event_date >= yesterday() AND event_time >= now() - 600 AND initial_query_id = (SELECT * FROM tmp) AND current_database = currentDatabase() LIMIT 1;
DROP TABLE tmp;
