...
# allow_statistics_optimize=0: Statistics pruning would filter parts before projection check, causing force_optimize_projection_name to fail
...
while [[ $i -lt $retries ]]; do
    ...
    if [[ $res -eq 0 ]]; then
        ...
        ${CLICKHOUSE_CLIENT} --query "select x, y from tbl where y = 2 settings force_optimize_projection_name = 'p', allow_statistics_optimize = 0;"
