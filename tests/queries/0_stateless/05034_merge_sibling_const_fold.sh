#!/usr/bin/env bash
# Tags: no-fasttest
# no-fasttest: the fixture needs the system.s3_queue_* tables, which the fast test build omits.
...
for prewhere in 0 1; do
    ...
    SELECT toString(multiIf(${COND}, 'yesyes', toLowCardinality(isNull('nononono')))) AS c
    FROM ${MERGE} WHERE c NOT IN ('_') ORDER BY c;
    "
done

# The same sibling divergence on other wrappers of the folded column.
${CLICKHOUSE_CLIENT} -q "SELECT toString(multiIf(${COND}, 'yesyes', 'no')) AS c
    FROM ${MERGE} WHERE c NOT IN ('_') ORDER BY c;"
...
${CLICKHOUSE_CLIENT} --allow_suspicious_variant_types=1 --allow_suspicious_low_cardinality_types=1 \
    --use_variant_as_common_type=1 \
    -q "SELECT toString(multiIf(${COND}, NULL, toLowCardinality(isNull('nononono')))) AS c
    FROM ${MERGE} WHERE c IS NULL OR c IS NOT NULL ORDER BY c;"

${CLICKHOUSE_CLIENT} --allow_suspicious_low_cardinality_types=1 \
    -q "SELECT toString(multiIf(${COND}, toLowCardinality('yesyes'), toLowCardinality('no'))) AS c
    FROM ${MERGE} WHERE c NOT IN ('_') ORDER BY c;"

${CLICKHOUSE_CLIENT} -q "SELECT toString(multiIf(${COND}, ['a'], ['b'])) AS c
    FROM ${MERGE} WHERE c != '' ORDER BY c;"

${CLICKHOUSE_CLIENT} -q "SELECT toString(multiIf(${COND}, ('a', 1), ('b', 2))) AS c
    FROM ${MERGE} WHERE c != '' ORDER BY c;"

${CLICKHOUSE_CLIENT} -q "SELECT toString(multiIf(${COND}, map('a', 1), map('b', 2))) AS c
    FROM ${MERGE} WHERE c != '' ORDER BY c;"
