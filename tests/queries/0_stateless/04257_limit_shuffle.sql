...
INSERT INTO limit_shuffle_insert_sink SELECT number FROM numbers(10) LIMIT 1 SHUFFLE SETTINGS allow_experimental_shuffle_query = 1;
CREATE TEMPORARY TABLE limit_shuffle_create_sink ENGINE = Memory AS SELECT number FROM numbers(10) LIMIT 1 SHUFFLE SETTINGS allow_experimental_shuffle_query = 1;
...
INSERT INTO limit_shuffle_mv_src VALUES (1, 1);
...
CREATE TABLE {CLICKHOUSE_DATABASE_1:Identifier}.limit_shuffle_plain_local (number UInt64) ENGINE = Memory;
...
SELECT * FROM cluster('test_shard_localhost', {CLICKHOUSE_DATABASE_1:String}, 'limit_shuffle_plain_local') SETTINGS enable_analyzer = 1, use_query_cache = 1 FORMAT Null;
SELECT * FROM remote('127.0.0.1', {CLICKHOUSE_DATABASE_1:String}, 'limit_shuffle_plain_local') SETTINGS enable_analyzer = 1, use_query_cache = 1 FORMAT Null;
...
SELECT * FROM remote('127.0.0.1', `null`('number UInt64')) SETTINGS enable_analyzer = 1, use_query_cache = 1 FORMAT Null;
SELECT * FROM cluster('test_shard_localhost', `null`('number UInt64')) SETTINGS enable_analyzer = 1, use_query_cache = 1 FORMAT Null;
...
SELECT * FROM limit_shuffle_view FORMAT Null;
DROP TABLE {CLICKHOUSE_DATABASE_1:Identifier}.limit_shuffle_plain_local;
...
EXPLAIN SELECT number FROM numbers(10) LIMIT 1 SHUFFLE;
