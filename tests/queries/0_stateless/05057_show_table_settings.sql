...
-- `Memory` rather than `MergeTree`, deliberately: no server configuration section sets any of its
-- settings. A `<merge_tree>` section does exist on real servers - the CI ones set a
...
-- definition acted on it - which is what `source != default` means.
DROP TABLE IF EXISTS stated_default;
...
SELECT '-- NOT drops a setting whichever of its names the pattern gives';
-- On `Distributed`, which has aliases and no server configuration section in the test environment. An
-- unfiltered `NOT` lists every changed setting, so the table states each one it changes - including those
-- the engine would otherwise copy from the server's `distributed_background_insert_*` settings.
...
CREATE TABLE dist AS dist_src ENGINE = Distributed('test_shard_localhost', currentDatabase(), 'dist_src')
    SETTINGS background_insert_batch = 1, background_insert_split_batch_on_failure = 0,
             background_insert_sleep_time_ms = 100, background_insert_max_sleep_time_ms = 30000;
SHOW CHANGED TABLE SETTINGS FROM dist NOT LIKE 'monitor_batch_inserts';
DROP TABLE dist;
DROP TABLE dist_src;
