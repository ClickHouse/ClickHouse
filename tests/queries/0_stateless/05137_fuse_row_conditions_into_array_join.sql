-- A filter comparing an ARRAY JOIN element with another column of the row is fused into the ARRAY JOIN.
...
-- fused (the step carries the filter), and the result matches the unfused one
...
-- the filter column is kept when the projection reads it
...
-- the same as an arrayFilter, for a reference that does not depend on the ARRAY JOIN
SELECT sum(length(arrayFilter(x -> x = key, arr))) FROM t_aj_row;

-- the lift below the join must keep the columns the fused filter reads
...
-- lowered arrayJoin chain: the outer element is a row column for the inner step
...
-- a filter that reads no element stays out of the step
