...
$CLICKHOUSE_CLIENT --query="create database if not exists test_$RAND_ID;"
$CLICKHOUSE_CLIENT --query="drop table if exists test_$RAND_ID.ints;"

$CLICKHOUSE_CLIENT --query="create table test_$RAND_ID.ints
                            (key UInt64, i8 Int8, i16 Int16, i32 Int32, i64 Int64, u8 UInt8, u16 UInt16, u32 UInt32, u64 UInt64)
                            Engine = Memory;"

$CLICKHOUSE_CLIENT --query="insert into test_$RAND_ID.ints values (1, 1, 1, 1, 1, 1, 1, 1, 1);"
$CLICKHOUSE_CLIENT --query="insert into test_$RAND_ID.ints values (2, 2, 2, 2, 2, 2, 2, 2, 2);"
$CLICKHOUSE_CLIENT --query="insert into test_$RAND_ID.ints values (3, 3, 3, 3, 3, 3, 3, 3, 3);"
...
$CLICKHOUSE_CLIENT --query "DROP TABLE if exists test_$RAND_ID.ints"
$CLICKHOUSE_CLIENT -q "DROP DATABASE test_$RAND_ID"
