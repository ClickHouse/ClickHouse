...
# no-old-analyzer: the queries building the payloads read JSON/Dynamic subcolumns, which it cannot resolve.
...
function check_truncated_blocks()
{
    ...
    local accepted=0
    local n
    # Sent over HTTP: a client process per truncation is what made this test time out under sanitizers.
    for ((n = 1; n < size; n += 5)); do
        if head -c "$n" "$DATA_FILE" \
            | ${CLICKHOUSE_CURL} -sS "${CLICKHOUSE_URL}&query=INSERT+INTO+$table+FORMAT+Native" --data-binary @- \
            | grep -q "Exception"
        then
            continue
        fi
...
$CLICKHOUSE_CLIENT -q "
    CREATE TABLE t_json_flattened (json JSON(a UInt64, max_dynamic_paths=2)) ENGINE = Memory;
    CREATE TABLE t_json_array_flattened (arr Array(JSON)) ENGINE = Memory;
    CREATE TABLE t_dynamic_flattened (dyn Dynamic) ENGINE = Memory;
"
...
$CLICKHOUSE_CLIENT -q "
    DROP TABLE t_json_flattened;
    DROP TABLE t_json_array_flattened;
    DROP TABLE t_dynamic_flattened;
"
