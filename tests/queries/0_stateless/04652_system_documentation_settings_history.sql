...
-- Every recorded change, with the setting it is recorded for: the records are part of the declaration of a setting.
CREATE VIEW session_records AS
SELECT
    version,
    tupleElement(arrayJoin(changes) AS c, 'name') AS setting,
...
-- to the name it used to have, and the only account of the rename is the reason of a record — of either the new
-- name ("Rename of `x`") or the old one ("Obsolete setting, renamed to `x`"). The pairs of names of
...
-- The history of a setting is the history of every name it has, including the names it no longer has: a record of a
-- name the setting was renamed from is history of the setting under the name it has today as well. It stays under
-- the old name too, for a name that is still a setting of its own — an obsolete setting, say.
CREATE VIEW session_changes AS
SELECT setting AS name, setting AS recorded_name, version, previous_value, new_value, reason FROM session_records
UNION ALL
SELECT r.current_name AS name, setting AS recorded_name, version, previous_value, new_value, reason
FROM session_records AS c INNER JOIN session_renames AS r ON r.previous_name = c.setting;

-- The history of an alias is the history of that name as opposed to the history of the setting it resolves to:
-- the records of the setting that name the alias and say either that it is an alias or that the setting was
-- renamed (which is how keeping the old name is worded). Such a record may change the default value in the same
-- breath, so it is not required to leave it as it was.
...
-- The history lists exactly one item per recorded change of the setting. A change recorded both for a setting and
-- for the obsolete name it was renamed from is listed once — so the identity of a change is the version and the
-- values together with the setting it is recorded for.
...
-- A record that only registers an alias introduces the alias, and not the setting it aliases:
-- `max_insert_block_size` is older than the change history, and it lists the 26.1 record that added its alias
-- `max_insert_block_size_rows` without claiming to have been introduced in that version.
...
-- `allow_experimental_full_text_index` when it appeared in 24.6.
...
-- An alias has the history of the records of its setting that name it as an alias: the 24.2 record of
-- `async_insert_busy_timeout_max_ms` registers the alias `async_insert_busy_timeout_ms`, and it is the history of the
-- alias — its appearance in 24.2.
...
-- was flipped to `true`. The alias has that version in its history, together with its introduction in 25.5, and
-- the setting lists that one change once.
...
-- A setting has one record per version: `enable_max_bytes_limit_for_min_age_to_force_merge` lists its 25.1
-- introduction once.
...
-- A rename does not always keep the old name as an alias, and then the records of the old name are not reachable
-- through the aliases of the setting; they are followed by the name the reason of the rename record gives.
...
-- When the history of the old name cannot be recovered at all — the old name of
...
-- The rename can also be recorded for the old name ("Obsolete setting, renamed to ..."), and the two records of
