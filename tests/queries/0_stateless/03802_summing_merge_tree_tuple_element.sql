...
-- Tests for Tuple column flattening and aggregation in SummingMergeTree
...
-- Test 6: SummingMergeTree - Tuple with Nullable and LowCardinality
SELECT '=== Test 6: SummingMergeTree Complex Types ===';
...
-- Test 7: Tuple column before sorting key column (removeReplicatedFromSortingColumns bug)
-- When tuple columns precede the sort key in schema, flattening shifts column indices.
SELECT '=== Test 7: SummingMergeTree Tuple before sorting key ===';

CREATE TABLE test_summing_tuple_before_sort_key (
    data Tuple(
        val_a UInt64,
        val_b UInt64,
        nested Tuple(
...
INSERT INTO test_summing_tuple_before_sort_key VALUES ((100, 50, (10)), 1), ((200, 80, (20)), 2);
INSERT INTO test_summing_tuple_before_sort_key VALUES ((150, 60, (15)), 1), ((250, 85, (25)), 2);

SELECT 'Tuple before sort key - with FINAL:';
SELECT id, data FROM test_summing_tuple_before_sort_key FINAL ORDER BY id;

OPTIMIZE TABLE test_summing_tuple_before_sort_key FINAL;

SELECT 'Tuple before sort key - after OPTIMIZE:';
SELECT id, data FROM test_summing_tuple_before_sort_key ORDER BY id;
