...
-- whole result of a `LIMIT 10`, so deleting them used to empty it. The primary index granularity is
-- pinned so that the fallback reads the same rows regardless of the randomized table settings.
...
SETTINGS index_granularity = 8192, index_granularity_bytes = 10485760, enable_block_number_column = 1, enable_block_offset_column = 1;
