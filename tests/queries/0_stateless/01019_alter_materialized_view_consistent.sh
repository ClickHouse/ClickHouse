...
function alter_thread()
{
    ...
    while [[ $STOP_ALTER -eq 0 ]]; do
        ...
        echo "[ALTER] took ${duration_alter}ms"
        ...
        echo "[SLEEP] took ${duration_sleep}ms"
    ...
    if [[ $alter_count -gt 0 ]]; then
        echo "ALTER Count: $alter_count, Avg Duration: $((total_alter_time / alter_count)) ms"
    else
        echo "No ALTER executed."
    fi
    if [[ $sleep_count -gt 0 ]]; then
        echo "SLEEP Count: $sleep_count, Avg Duration: $((total_sleep_time / sleep_count)) ms"
    else
...
for i in {1..100}; do
    while true; do
        ...
        if $CLICKHOUSE_CLIENT -q "INSERT INTO src VALUES (1);" 2>/dev/null; then
            end_insert=$(date +%s%3N)
            duration_insert=$((end_insert - start_insert))
            echo "[INSERT #$i] took ${duration_insert}ms"
...
if [[ $insert_count -gt 0 ]]; then
    echo
    echo "==================== INSERT Stats ===================="
    echo "INSERT Count: $insert_count, Avg Duration: $((total_insert_time / insert_count)) ms"
    echo "======================================================"
fi
