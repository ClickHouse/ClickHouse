...
ONLY_VIRTUAL_ON=$(stream_count "SELECT any(_part) FROM t_narrow SETTINGS max_threads = 64, merge_tree_min_rows_for_concurrent_read = 0, merge_tree_min_bytes_for_concurrent_read = 0, merge_tree_min_read_task_size = 1")
ONLY_VIRTUAL_OFF=$(stream_count "SELECT any(_part) FROM t_narrow SETTINGS max_threads = 64, merge_tree_min_rows_for_concurrent_read = 0, merge_tree_min_bytes_for_concurrent_read = 0, merge_tree_min_read_task_size = 1, merge_tree_min_bytes_per_read_stream = 0")

echo "-- cap reduces streams for a narrow column --"
[ "$ON" -lt "$OFF" ] && echo 1 || echo 0
echo "-- virtual-only reads use the injected physical column for the cap --"
[ "$ONLY_VIRTUAL_ON" -lt "$ONLY_VIRTUAL_OFF" ] && echo 1 || echo 0
