...
-- 3. Exact bitpacking boundary doc counts (128, 256, 384 docs)
...
----------------------------------------------------
SELECT 'Test 1: Multiple segments with small posting_list_block_size=128';

-- posting_list_block_size=128 -> aligned to 128, so each segment holds exactly 128 docs.
-- A token appearing in 50% of 2000 rows -> 1000 docs -> ~8 segments.
-- This exercises multiple Index Sections and the cross-segment iteration in PostingListCursor.

DROP TABLE IF EXISTS tab_multi_seg;

CREATE TABLE tab_multi_seg(k UInt64, s String, INDEX idx s TYPE text(tokenizer = 'splitByNonAlpha', posting_list_block_size = 128))
    ENGINE = MergeTree() ORDER BY k
    SETTINGS index_granularity = 8192, index_granularity_bytes = '10Mi';

-- 'common' appears in even rows (1000 times), 'uncommon' in odd rows (1000 times)
INSERT INTO tab_multi_seg SELECT number, if(number % 2 = 0, 'common', 'uncommon') FROM numbers(2000);

-- Verify count: 'common' should match exactly 1000 rows
SELECT count() FROM tab_multi_seg WHERE hasToken(s, 'common');

-- Verify count: 'uncommon' should also match exactly 1000 rows
SELECT count() FROM tab_multi_seg WHERE hasToken(s, 'uncommon');

-- AND: common + uncommon have zero overlap
SELECT count() FROM tab_multi_seg WHERE hasToken(s, 'common') AND hasToken(s, 'uncommon');

-- OR: common + uncommon = all rows
SELECT count() FROM tab_multi_seg WHERE hasAnyTokens(s, ['common', 'uncommon']);
...
CREATE TABLE tab_seek(k UInt64, s String, INDEX idx s TYPE text(tokenizer = ngrams(3), posting_list_block_size = 128))
    ENGINE = MergeTree() ORDER BY k
    SETTINGS index_granularity = 8192, index_granularity_bytes = '10Mi';
...
-- OR across segments: 'aaa' + 'bbb' = all 2000 rows
SELECT count() FROM tab_seek WHERE s LIKE '%aaa%' OR s LIKE '%bbb%';

----------------------------------------------------
SELECT 'Test 3: Exact bitpacking boundary (128, 256, 384 docs)';
...
CREATE TABLE tab_boundary(k UInt64, s String, INDEX idx s TYPE text(tokenizer = 'splitByNonAlpha', posting_list_block_size = 128))
    ENGINE = MergeTree() ORDER BY k
    SETTINGS index_granularity = 8192, index_granularity_bytes = '10Mi';
...
-- 257 rows: first_doc_id + 256 docs = 2 full packed blocks, tail_size=0, one segment
DROP TABLE IF EXISTS tab_boundary2;

CREATE TABLE tab_boundary2(k UInt64, s String, INDEX idx s TYPE text(tokenizer = 'splitByNonAlpha', posting_list_block_size = 512))
    ENGINE = MergeTree() ORDER BY k
    SETTINGS index_granularity = 8192, index_granularity_bytes = '10Mi';
...
CREATE TABLE tab_boundary3(k UInt64, s String, INDEX idx s TYPE text(tokenizer = 'splitByNonAlpha', posting_list_block_size = 512))
    ENGINE = MergeTree() ORDER BY k
    SETTINGS index_granularity = 8192, index_granularity_bytes = '10Mi';
...
CREATE TABLE tab_boundary4(k UInt64, s String, INDEX idx s TYPE text(tokenizer = 'splitByNonAlpha', posting_list_block_size = 128))
    ENGINE = MergeTree() ORDER BY k
    SETTINGS index_granularity = 8192, index_granularity_bytes = '10Mi';
...
CREATE TABLE tab_multiway(k UInt64, s String, INDEX idx s TYPE text(tokenizer = 'splitByNonAlpha', posting_list_block_size = 128))
    ENGINE = MergeTree() ORDER BY k
    SETTINGS index_granularity = 8192, index_granularity_bytes = '10Mi';
...
CREATE TABLE tab_dense(k UInt64, s String, INDEX idx s TYPE text(tokenizer = 'splitByNonAlpha', posting_list_block_size = 128))
    ENGINE = MergeTree() ORDER BY k
    SETTINGS index_granularity = 8192, index_granularity_bytes = '10Mi';
...
-- The first doc_id is stored inline in the dictionary, not in bitpacking data.
-- PostingListCursor must prepend it when decoding segment 0's packed block 0.
-- Verify the exact first row is returned correctly.

DROP TABLE IF EXISTS tab_first_doc;

CREATE TABLE tab_first_doc(k UInt64, s String, INDEX idx s TYPE text(tokenizer = 'splitByNonAlpha', posting_list_block_size = 128))
    ENGINE = MergeTree() ORDER BY k
    SETTINGS index_granularity = 8192, index_granularity_bytes = '10Mi';
...
CREATE TABLE tab_empty_seek(k UInt64, s String, INDEX idx s TYPE text(tokenizer = 'splitByNonAlpha', posting_list_block_size = 128))
    ENGINE = MergeTree() ORDER BY k
    SETTINGS index_granularity = 8192, index_granularity_bytes = '10Mi';
...
----------------------------------------------------
SELECT 'Test 8: Multiple segments after merge';

-- Merge two parts, each generating large posting lists. After merge, the merged
-- posting list should span even more segments with correct segment index sections.

DROP TABLE IF EXISTS tab_merge_seg;

CREATE TABLE tab_merge_seg(k UInt64, s String, INDEX idx s TYPE text(tokenizer = 'splitByNonAlpha', posting_list_block_size = 128))
    ENGINE = MergeTree() ORDER BY k
    SETTINGS index_granularity = 8192, index_granularity_bytes = '10Mi';

INSERT INTO tab_merge_seg SELECT number, if(number % 2 = 0, 'common', 'uncommon') FROM numbers(1000);
INSERT INTO tab_merge_seg SELECT number + 1000, if((number + 1000) % 2 = 0, 'common', 'uncommon') FROM numbers(1000);

OPTIMIZE TABLE tab_merge_seg FINAL;

SELECT count() FROM tab_merge_seg WHERE hasToken(s, 'common');

SELECT count() FROM tab_merge_seg WHERE hasToken(s, 'uncommon');

-- AND after merge: common + uncommon = 0 overlap
SELECT count() FROM tab_merge_seg WHERE hasToken(s, 'common') AND hasToken(s, 'uncommon');

----------------------------------------------------
SELECT 'Test 9: Seek within and across segments with ngrams';

-- Use ngrams tokenizer for targeted seek via LIKE on per-row unique substrings.
-- 'freq' appears in every row -> 1500 docs -> many segments of 128

DROP TABLE IF EXISTS tab_seek_seg;

CREATE TABLE tab_seek_seg(k UInt64, s String, INDEX idx s TYPE text(tokenizer = ngrams(4), posting_list_block_size = 128))
    ENGINE = MergeTree() ORDER BY k
    SETTINGS index_granularity = 8192, index_granularity_bytes = '10Mi';

INSERT INTO tab_seek_seg SELECT number, concat('freq zz', leftPad(toString(number), 5, '0')) FROM numbers(1500);

-- Seek to a doc in the middle via LIKE
SELECT k FROM tab_seek_seg WHERE s LIKE '%freq%' AND s LIKE '%zz00750%';

-- Seek to last doc
SELECT k FROM tab_seek_seg WHERE s LIKE '%freq%' AND s LIKE '%zz01499%';

-- Seek to first doc
SELECT k FROM tab_seek_seg WHERE s LIKE '%freq%' AND s LIKE '%zz00000%';
