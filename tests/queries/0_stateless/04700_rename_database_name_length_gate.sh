...
# test_<8 lowercase letters and digits>, so every byte is a word character and the escaped length
# equals the character count. The unique prefix is what keeps this parallel-safe.
pad() { printf '%*s' "$1" '' | tr ' ' d; }
...
rename_outcome() {
    local label="$1" query="$2" client="$3" out
...
if [ "$use_s3_plain_rewriteable_as_db_disk" == "0" ]; then
    ...
    $CLICKHOUSE_CLIENT -q "DROP TABLE \`$edge_a\`.t0 SYNC" 2>&1 | grep -o -m 1 'Code: [0-9]*' || echo droppable-after-accept
    $CLICKHOUSE_CLIENT -q "DROP DATABASE IF EXISTS \`$src\`" >/dev/null 2>&1
    ...
    $CLICKHOUSE_CLIENT -q "DROP TABLE \`$edge_b\`.t0 SYNC" 2>&1 | grep -o -m 1 'Code: [0-9]*' || echo detached-droppable-after-accept
    $CLICKHOUSE_CLIENT -q "DROP DATABASE IF EXISTS \`$src\`" >/dev/null 2>&1
    ...
    $CLICKHOUSE_CLIENT -q "DROP TABLE \`$repl_edge\`.t0 SYNC" 2>&1 | grep -o -m 1 'Code: [0-9]*' || echo replicated-droppable-after-accept
    ...
    echo deps-off-into-long-rejected
    echo deps-on-into-long-rejected
    echo deps-off-accept-accepted
    echo droppable-after-accept
    echo deps-off-boundary-reject-rejected
    echo deps-off-boundary-accept-accepted
    echo detached-into-long-rejected
    echo detached-into-long-deps-on-rejected
    echo detached-accept-accepted
    echo detached-droppable-after-accept
    echo replicated-into-long-rejected
    echo replicated-accept-accepted
    echo replicated-droppable-after-accept
fi
