...
DROP DATABASE IF EXISTS test_meta_cache;
DROP TABLE IF EXISTS test_meta_cache.check_part_meta_cache;
CREATE DATABASE test_meta_cache ENGINE = Ordinary;
CREATE TABLE test_meta_cache.check_part_meta_cache  ( p Date, k UInt64, v1 UInt64, v2 Int64) ENGINE ReplicatedMergeTree('/clickhouse/tables/{database}/check_part_meta_cache', '{replica}') PARTITION BY toYYYYMM(p) ORDER BY k;
...
-- Add column.
alter table test_meta_cache.check_part_meta_cache  add column v3 UInt64;
INSERT INTO test_meta_cache.check_part_meta_cache (p, k, v1, v2) VALUES ('2018-06-15', 5, 1000, 2000), ('2018-06-16', 6, 3000, 4000), ('2018-06-17', 7, 5000, 6000), ('2018-06-18', 8, 7000, 8000);
with arrayJoin(checkPartMetaCache('test_meta_cache', 'check_part_meta_cache')) as info select count(1), countIf(info.5 = 0);

-- Delete column.
alter table test_meta_cache.check_part_meta_cache  drop column v1;
with arrayJoin(checkPartMetaCache('test_meta_cache', 'check_part_meta_cache')) as info select count(1), countIf(info.5 = 0);

-- Add TTL info.
alter table test_meta_cache.check_part_meta_cache  modify TTL p + INTERVAL 10 YEAR;
...
-- Modify TTL info.
alter table test_meta_cache.check_part_meta_cache  modify TTL p + INTERVAL 15 YEAR;
INSERT INTO test_meta_cache.check_part_meta_cache (p, k, v2, v3) VALUES ('2018-06-15', 5, 1000, 2000), ('2018-06-16', 6, 3000, 4000), ('2018-06-17', 7, 5000, 6000), ('2018-06-18', 8, 7000, 8000);
with arrayJoin(checkPartMetaCache('test_meta_cache', 'check_part_meta_cache')) as info select count(1), countIf(info.5 = 0);
...
drop table if exists test_meta_cache.check_part_meta_cache ;
CREATE TABLE test_meta_cache.check_part_meta_cache  ( p Date, k UInt64, v1 UInt64, v2 Int64, projection p1 (select p, sum(k), sum(v1), sum(v2) group by p)) ENGINE MergeTree() PARTITION BY toYYYYMM(p) ORDER BY k;
