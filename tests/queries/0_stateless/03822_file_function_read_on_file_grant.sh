...
# With a replicated access storage, the entity serialization round trip widens `GRANT READ ON FILE`
# to both `READ` and `WRITE` (see issue #111402), so the test would pass for the wrong reason.
...
write_txt="write_03822_${CLICKHOUSE_DATABASE}_$RANDOM.txt"
...
# Write side must stay denied with only READ ON FILE.
# With the widening bug (issue #111402), READ ON FILE round-trips back as READ|WRITE ON FILE
# under ReplicatedAccessStorage, making the insert succeed — a false green on the replicated path.
(( $(${CLICKHOUSE_CLIENT} --user $user --query "INSERT INTO FUNCTION file('$write_txt') SELECT 1" 2>&1 | grep -c "ACCESS_DENIED") >= 1 )) && echo "ACCESS_DENIED" || echo "UNEXPECTED";
