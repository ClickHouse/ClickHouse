...
(
    ...
    PROJECTION map_keys_idx INDEX mapKeys(map) TYPE text(tokenizer = 'splitByNonAlpha'),
    PROJECTION map_fixed_keys_idx INDEX mapKeys(map_fixed) TYPE text(tokenizer = 'splitByNonAlpha'),
)
ENGINE = MergeTree
ORDER BY (id);
...
(
    ...
    PROJECTION map_values_idx INDEX mapValues(map) TYPE text(tokenizer = 'splitByNonAlpha'),
    PROJECTION map_fixed_values_idx INDEX mapValues(map_fixed) TYPE text(tokenizer = 'splitByNonAlpha'),
)
ENGINE = MergeTree
ORDER BY (id);
