...
SELECT part_type FROM system.part_log WHERE event_date >= yesterday() AND event_time >= now() - 600 AND database = currentDatabase()
    AND table = 't_parts_profile_events' AND event_type = 'NewPart'
    ORDER BY event_time_microseconds;

SELECT part_type, count() > 0 FROM system.part_log WHERE event_date >= yesterday() AND event_time >= now() - 600 AND database = currentDatabase()
