...
-- N7: the single surviving conjunct is Bool and the filter column is selected, so the reported type
-- must stay the UInt8 the whole condition is declared with rather than follow the survivor.
