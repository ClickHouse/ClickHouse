...
-- no squashing after the join. The hash table starts at the estimated size and grows as `hash`'s does, and a
-- spill hands the kept build blocks to `GraceHashJoin`. Results must match `hash` for every join kind.
