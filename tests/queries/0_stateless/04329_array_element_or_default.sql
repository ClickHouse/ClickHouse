...
SELECT 'result type is the least common supertype of element and default';
...
SELECT 'consistency with arrayElementOrNull: present NULL element yields default';
...
SELECT 'NULL default produces a Nullable result';
SELECT arrayElementOrDefault([1, 2, 3], 9, NULL) AS v, toTypeName(v);

SELECT 'non-constant index over a column';
SELECT n, arrayElementOrDefault([10, 20, 30], n, -7) AS v FROM (SELECT number - 1 AS n FROM numbers(6)) ORDER BY n;

SELECT 'non-constant array and default';
SELECT
    arr,
    idx,
    arrayElementOrDefault(arr, idx, def) AS v
FROM
(
    SELECT [toInt64(number), toInt64(number) + 1] AS arr, toInt64(number % 3) AS idx, toInt64(number * 100) AS def
    FROM numbers(4)
)
ORDER BY idx, def;
