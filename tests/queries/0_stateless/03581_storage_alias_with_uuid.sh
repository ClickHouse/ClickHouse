...
REF_UUID=$($CLICKHOUSE_CLIENT --query "select uuid from system.tables where database = '$CLICKHOUSE_DATABASE' and name = 'ref_table'")
$CLICKHOUSE_CLIENT --query "create table alias_table Engine=Alias('$REF_UUID')"
