...
SELECT * FROM t_parquet_cache_1
    SETTINGS log_comment = '04108-q1', use_parquet_metadata_cache = 1
    FORMAT Null;

-- q2: same table, same file -> expect hit (sanity: caching works at all)
SELECT * FROM t_parquet_cache_1
    SETTINGS log_comment = '04108-q2', use_parquet_metadata_cache = 1
    FORMAT Null;
...
SELECT * FROM t_parquet_cache_2
    SETTINGS log_comment = '04108-q3', use_parquet_metadata_cache = 1
    FORMAT Null;

-- q4: drop the cache and re-query -> expect miss (confirms the cache is the causal factor)
SYSTEM DROP PARQUET METADATA CACHE;
SELECT * FROM t_parquet_cache_2
    SETTINGS log_comment = '04108-q4', use_parquet_metadata_cache = 1
    FORMAT Null;
