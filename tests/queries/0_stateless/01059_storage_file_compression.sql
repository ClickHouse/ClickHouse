...
CREATE TABLE file (x UInt64) ENGINE = File(TSV, 'data6.tsv.bz2');
TRUNCATE TABLE file;
...
SELECT count(), max(x) FROM file('data{1,2,3,4,5,6}.tsv.{gz,br,xz,zst,lz4,bz2}', TSV, 'x UInt64');
...
SELECT count() < 4000000, max(x) FROM file('data5.tsv.lz4', RowBinary, 'x UInt8', 'none');
SELECT count() < 2000000, max(x) FROM file('data6.tsv.bz2', RowBinary, 'x UInt8', 'none');
