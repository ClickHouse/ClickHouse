...
def write(name, values, *, precision, use_dictionary, as_integer, row_group_size=None, declare=None):
    ...
    # The patch is a blind byte replace: read the declared precision back and confirm pyarrow can
    # still parse every row group's statistics.
    f = pq.ParquetFile(path)
    want = f"Decimal(precision={declare or precision}, scale=2)"
    assert str(f.schema.column(0).logical_type) == want, f"{name}: {f.schema.column(0).logical_type}"
    assert f.metadata.num_rows == len(values)
    for g in range(f.metadata.num_row_groups):
        assert f.metadata.row_group(g).column(0).statistics is not None, f"{name}: rg{g} no stats"
...
write("int64_dict", col, precision=18, use_dictionary=True, as_integer=True, declare=9)
write("int64_plain", col, precision=18, use_dictionary=False, as_integer=True, declare=9)
# Physical FIXED_LEN_BYTE_ARRAY, declared precision 9, type_length 8 and 16.
write("flba8", col, precision=18, use_dictionary=True, as_integer=False, declare=9)
write("flba16", col, precision=38, use_dictionary=True, as_integer=False, declare=9)
# A value needing 12 digits, so it does not fit Decimal32(9, 2).
write("overflow", [decimal.Decimal("9999999999.99")] + col[1:],
      precision=18, use_dictionary=True, as_integer=True, declare=9)
...
prune_verdict "SELECT count() FROM file('$DATA/rowgroups_mismatch.parquet', Parquet) WHERE k > 500 SETTINGS input_format_parquet_filter_push_down = 1"
...
prune_verdict "SELECT count() FROM file('$DATA/rowgroups_control.parquet', Parquet) WHERE k > 500 SETTINGS input_format_parquet_filter_push_down = 1"
...
prune_verdict "SELECT count() FROM file('$DATA/rowgroups_mismatch.parquet', Parquet, 'k Decimal(18, 2)') WHERE k > 500 SETTINGS input_format_parquet_filter_push_down = 1"
