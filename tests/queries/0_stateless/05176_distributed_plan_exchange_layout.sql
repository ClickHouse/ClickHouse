...
-- The work around a streaming exchange stays spread over the streams of a task: every scattered
-- stream serializes its own packets ahead of the sinks, and the packets of one source are
-- deserialized on all threads. The checks read the processors of every task from
-- `processors_profile_log`. They are relations between counts, not exact counts, so they hold for
-- any stream count. When a check fails, the last query prints the layout of the tasks behind it.
...
SELECT check, ok FROM (
    ...
    UNION ALL SELECT 4, 'receive: deserializers >= sources in every task', min(deserializers >= sources) FROM v_exchange_layout WHERE sources > 0
    UNION ALL SELECT 5, 'receive: deserializers > sources in some task', max(deserializers > sources) FROM v_exchange_layout WHERE sources > 0
) ORDER BY step;
...
SELECT format('{} ({}): scatters={} sinks={} serializers={} max_rows_into_serializer={} max_rows_into_scatter={} sources={} deserializers={}',
    stage, query_id, toString(scatters), toString(sinks), toString(serializers), toString(max_rows_into_serializer),
    toString(max_rows_into_scatter), toString(sources), toString(deserializers))
...
WHERE (scatters > 0 AND (serializers != scatters * sinks OR max_rows_into_serializer > max_rows_into_scatter))
   ...
   OR (sources > 0 AND (SELECT max(deserializers > sources) FROM v_exchange_layout WHERE sources > 0) = 0)
ORDER BY stage;
