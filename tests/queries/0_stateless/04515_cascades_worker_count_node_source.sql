...
-- distributed_plan_execute_locally. One worker gives a single-node plan; without it the count came
-- from the configured worker cluster, so one worker would still build a multi-node plan.
...
-- A large table-size hint makes the distributed read worth splitting across nodes.
SET param__internal_join_table_stat_hints = '{"t_worker_count": {"cardinality": 600000000, "avg_row_bytes": 16, "distinct_keys": {"g": 200}}}';
...
SETTINGS enable_cascades_optimizer = 1, make_distributed_plan = 1, distributed_plan_execute_locally = 1,
    enable_parallel_replicas = 0, automatic_parallel_replicas_mode = 0, distributed_plan_workers_num = 1;

SELECT '-- four workers: distributed plan with a gather exchange';
EXPLAIN PLAN SELECT g, count() FROM t_worker_count GROUP BY g
SETTINGS enable_cascades_optimizer = 1, make_distributed_plan = 1, distributed_plan_execute_locally = 1,
    enable_parallel_replicas = 0, automatic_parallel_replicas_mode = 0, distributed_plan_workers_num = 4;
