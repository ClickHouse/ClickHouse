...
# rebuilds a projection whose output column is a PURE IDENTIFIER ALIAS of a base column (SELECT d AS renamed).
# fillProjectionDescriptionByQuery interprets the projection query with ignoreAlias(), so the projection's sample
# block - and therefore the rebuilt temporary part - carries the BASE column name (d), not the alias: the aliased
# form must be priced by exactly the same bare-identifier path as the unaliased SELECT k, d of 04641, including
# the legacy .bin recovery for a wide composite Dynamic variant that exceeds the static type capacity. This test
# pins that behavior: if projection aliases ever started leaking into the sample block as output names, the
# source-part probes by name would silently miss the base column and this scenario is the first to under-reserve.
#
...
WORKING_FOLDER="${CLICKHOUSE_TMP}/04643_merge_memory_reservation_projection_identifier_alias"
...
${CLICKHOUSE_LOCAL} --path "${DB_PATH}" -q "
    ...
    CREATE TABLE t_merge_mem_projection_alias (k UInt64, d Dynamic(max_types = 1))
    ENGINE = MergeTree ORDER BY k
    SETTINGS min_bytes_for_wide_part = 0, min_rows_for_wide_part = 0,
             deduplicate_merge_projection_mode = 'rebuild',
             merge_selecting_sleep_ms = 600000, max_merge_selecting_sleep_ms = 600000;

    SYSTEM STOP MERGES t_merge_mem_projection_alias;
    ...
    INSERT INTO t_merge_mem_projection_alias SELECT number,
    ...
    -- Add the ALIASED bare-identifier projection AFTER the parts exist, so none has it materialized and the
    -- row-reducing merge rebuilds it from the merged base rows. The output column is spelled 'd AS renamed',
    -- but ignoreAlias() resolves the projection column back to the base column d - the rebuilt temporary part
    -- stores d, and the estimator must price it through the same bare-identifier path as the unaliased form.
    ALTER TABLE t_merge_mem_projection_alias ADD PROJECTION p_alias (SELECT k, d AS renamed ORDER BY k);
"
...
${CLICKHOUSE_LOCAL} --path "${DB_PATH}" -q "
    OPTIMIZE TABLE t_merge_mem_projection_alias FINAL DEDUPLICATE SETTINGS optimize_throw_if_noop = 1;

    -- The duplicate part collapsed: 2000 distinct rows remain out of 3000.
    SELECT count() FROM t_merge_mem_projection_alias;
    SELECT count() FROM system.parts WHERE database = currentDatabase() AND table = 't_merge_mem_projection_alias' AND active;
    -- The rebuilt projection must be present in the single merged part, storing the BASE column name.
    SELECT name, groupArray(column) FROM system.projection_parts_columns
        WHERE database = currentDatabase() AND table = 't_merge_mem_projection_alias' AND active
    ...
    SELECT sum(CAST(d, 'Tuple(a UInt64, s String, f Float64, arr Array(UInt64), n Nullable(UInt64), dt Date)').a)
        FROM t_merge_mem_projection_alias;
