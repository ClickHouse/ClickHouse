...
${CLICKHOUSE_CURL} -vsS "${URL}"ping  2>&1 | grep 'Keep-Alive' | sed -r 's/(keep-alive: timeout=30, max=)[0-9]+/\1?/I' | grep -i 'keep-alive';
...
${CLICKHOUSE_CURL} -vsS "${URL}"404/not/found/ 2>&1 | grep 'Keep-Alive' | sed -r 's/(keep-alive: timeout=30, max=)[0-9]+/\1?/I';
