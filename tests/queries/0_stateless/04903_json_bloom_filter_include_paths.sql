...
{"id":1,"j":{"request_id":"req-1","request":"request-1","created_at":"time-1","private_id":"private-1","message":"message-1","ignored":[[1]],"payload":{"ids":{"primary":"primary-1","secret":"secret-1"},"updated_at":"updated-1","text":"text-1"},"items":[{"item_id":"item-1","name":"name-1"}],"tuple_map":{"k":{"item_id":"map-id-1","name":"map-name-1"}}}}
{"id":2,"j":{"request_id":"req-2","request":"request-2","created_at":"time-2","private_id":"private-2","message":"message-2","ignored":[[2]],"payload":{"ids":{"primary":"primary-2","secret":"secret-2"},"updated_at":"updated-2","text":"text-2"},"items":[{"item_id":"item-2","name":"name-2"}],"tuple_map":{"k":{"item_id":"map-id-2","name":"map-name-2"}}}}
;
...
CREATE TABLE json_bf_include_paths_invalid
(
    j JSON,
    INDEX idx j TYPE jsonbf_v1(include_paths = 'request_id') GRANULARITY 1
)
...
(
    j JSON,
    INDEX idx j TYPE jsonbf_v1(include_paths = ['']) GRANULARITY 1
)
ENGINE = MergeTree ORDER BY tuple(); -- { serverError BAD_ARGUMENTS }
...
(
    j JSON,
    INDEX idx j TYPE jsonbf_v1(include_paths_regexp = ['[']) GRANULARITY 1
)
ENGINE = MergeTree ORDER BY tuple(); -- { serverError BAD_ARGUMENTS }
