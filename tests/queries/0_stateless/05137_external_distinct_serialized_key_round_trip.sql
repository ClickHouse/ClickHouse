...
-- Each state contains three 6 MiB strings and exceeds the run byte target. Ordinary runs retain the
-- entire state as payload and restore all of its values; suppression runs retain only its fingerprint.
