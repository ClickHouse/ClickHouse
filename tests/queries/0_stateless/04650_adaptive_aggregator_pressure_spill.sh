...
# A freeze threshold above both the key count and the give-up bound, with the byte bound
# disabled, keeps every producer in the learning phase for the whole query, which is the one
# regime where no frozen table is ever built.
