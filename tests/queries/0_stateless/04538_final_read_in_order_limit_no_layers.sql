...
-- Read-in-order with a small limit and no filtering: intersecting ranges must not be split
-- into layers, the result is expected to be produced by a single lazy merging stream.
SELECT 'in-order small limit, layers:', countIf(explain LIKE '%FilterSortedStreamByRange%') > 0
FROM (EXPLAIN PIPELINE SELECT k, v FROM t_final_layers FINAL ORDER BY k LIMIT 10);

-- With a filter the number of rows to read is limit / selectivity, which is unknown, so the
-- hard read limit is not set and layers must be kept even with a small limit: for a rare-value
-- filter a single merging stream would process most of the table sequentially.
SELECT 'in-order filtered small limit, layers:', countIf(explain LIKE '%FilterSortedStreamByRange%') > 0
...
FROM (EXPLAIN PIPELINE SELECT k, v FROM t_final_layers FINAL ORDER BY k LIMIT 50000);
...
FROM (EXPLAIN PIPELINE SELECT k, v FROM t_final_layers FINAL ORDER BY k);
...
FROM (EXPLAIN PIPELINE SELECT k, v FROM t_final_layers FINAL ORDER BY k LIMIT 100000);
...
FROM (EXPLAIN PIPELINE SELECT k, v FROM t_final_layers FINAL ORDER BY v, k LIMIT 10);
