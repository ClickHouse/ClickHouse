...
SELECT name, count() AS c FROM default.test_pushdown_limit_to_shards_dist GROUP BY name ORDER BY c DESC LIMIT 2 SETTINGS enable_pushdown_limit_to_shards=1, limit_pushdown_fetch_multiplier=2;

WITH ifNull(sum(duration), 99999) AS tot SELECT name, tot, uniq(no) AS u FROM default.test_pushdown_limit_to_shards_dist GROUP BY name ORDER BY tot LIMIT 2 SETTINGS enable_pushdown_limit_to_shards=1;
WITH ifNull(sum(duration), 99999) AS tot SELECT name, tot, uniq(no) AS u FROM default.test_pushdown_limit_to_shards_dist GROUP BY name ORDER BY tot DESC LIMIT 2 SETTINGS enable_pushdown_limit_to_shards=1;
