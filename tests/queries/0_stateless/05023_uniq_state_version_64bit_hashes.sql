...
-- Version 1 of the `uniq` state keeps 64-bit hashes instead of 32-bit ones, which fixes
-- the overflow of the estimate at cardinalities above ten billion (issue #6078).
...
-- looks like once it is attached on a server that knows about version 1 - keeps the legacy 32-bit format.
...
-- Merging 64-bit states with legacy 32-bit states must not double-count the common elements:
...
-- Below the sampling threshold the 64-bit state is exact.
