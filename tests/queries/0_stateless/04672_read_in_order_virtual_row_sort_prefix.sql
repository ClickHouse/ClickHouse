...
FROM (EXPLAIN PLAN actions = 1, indexes = 0, compact = 1, pretty = 1 SELECT * FROM ev ORDER BY name, code LIMIT 5) WHERE e != '';

SELECT '-- cast on the only column';
SELECT extract(explain, '(?:Prefix sort description|Read type|Virtual row conversions).*') AS e
FROM (EXPLAIN PLAN actions = 1, indexes = 0, compact = 1, pretty = 1 SELECT * FROM ev ORDER BY name::Nullable(String) LIMIT 5) WHERE e != '';

SELECT '-- cast on the first of two columns';
SELECT extract(explain, '(?:Prefix sort description|Read type|Virtual row conversions).*') AS e
FROM (EXPLAIN PLAN actions = 1, indexes = 0, compact = 1, pretty = 1 SELECT * FROM ev ORDER BY name::Nullable(String), code LIMIT 5) WHERE e != '';

SELECT '-- cast on the last of two columns';
SELECT extract(explain, '(?:Prefix sort description|Read type|Virtual row conversions).*') AS e
FROM (EXPLAIN PLAN actions = 1, indexes = 0, compact = 1, pretty = 1 SELECT * FROM ev ORDER BY name, code::Nullable(String) LIMIT 5) WHERE e != '';

SELECT '-- toString on the first of two columns';
SELECT extract(explain, '(?:Prefix sort description|Read type|Virtual row conversions).*') AS e
FROM (EXPLAIN PLAN actions = 1, indexes = 0, compact = 1, pretty = 1 SELECT * FROM ev ORDER BY toString(name), code LIMIT 5) WHERE e != '';

SELECT '-- non-key column last';
SELECT extract(explain, '(?:Prefix sort description|Read type|Virtual row conversions).*') AS e
FROM (EXPLAIN PLAN actions = 1, indexes = 0, compact = 1, pretty = 1 SELECT * FROM ev ORDER BY name, code, ref LIMIT 5) WHERE e != '';

SELECT '--- non-strictly monotonic function: the prefix is cut after it';
SELECT extract(explain, '(?:Prefix sort description|Read type|Virtual row conversions).*') AS e
FROM (EXPLAIN PLAN actions = 1, indexes = 0, compact = 1, pretty = 1 SELECT * FROM by_day ORDER BY toStartOfMonth(d), num LIMIT 5) WHERE e != '';

SELECT '--- INNER JOIN: read-in-order needs the virtual row';

SELECT '-- cast on the first of two key columns';
SELECT extract(explain, '(?:Prefix sort description|Read type|Virtual row conversions).*') AS e
FROM (EXPLAIN PLAN actions = 1, indexes = 0, compact = 1, pretty = 1
