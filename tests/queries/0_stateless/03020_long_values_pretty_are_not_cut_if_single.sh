...
${CLICKHOUSE_CLIENT} --output-format-pretty-multiline-fields 0 --query "SHOW CREATE TABLE system.metric_log" --format Pretty | grep -P '^COMMENT'
${CLICKHOUSE_CLIENT} --output-format-pretty-multiline-fields 0 --query "SHOW CREATE TABLE system.metric_log" --format PrettyCompact | grep -P '^COMMENT'
${CLICKHOUSE_CLIENT} --output-format-pretty-multiline-fields 0 --query "SHOW CREATE TABLE system.metric_log" --format PrettySpace | grep -P '^COMMENT'
