...
# (required for the row-count cache to engage) and a recognizable custom header.
python3 -c "
...
SETTINGS use_cache_for_count_from_files = 1;
"
...
${CLICKHOUSE_CLIENT} -q "
SELECT count() FROM url('${URL}', TSV, 's String')
SETTINGS use_cache_for_count_from_files = 1, log_comment = '04843_repeat_count';
"
