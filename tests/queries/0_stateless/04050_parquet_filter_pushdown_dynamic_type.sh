...
# The native v3 reader (the only Parquet reader on master) exercises the row-group stats,
# bloom filter, and page-index pushdown paths in `Reader.cpp`.
opts=(
...
# `Dynamic` column with a JSON-typed `KeyCondition` constant. Before the fix, the reader decoded
# the String parquet stats and tried to compare them with the JSON constant from the query,
# throwing `BAD_TYPE_OF_FIELD`. After the fix, stats-based pushdown is skipped for `Dynamic`
# columns, so the parquet reader does not throw; the comparison is then handled at query
# execution and reports the type mismatch as `NO_COMMON_TYPE`.
