...
create table test_10m (key Int, value Int) engine=MergeTree() order by key partition by key % 500;
system stop merges test_10m;

insert into test_10m select number, number*100 from numbers(1e6) settings max_partitions_per_insert_block=500, max_block_size=1e6;
