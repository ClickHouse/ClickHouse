...
rows_read=$($CLICKHOUSE_CLIENT -q "SELECT a FROM m ORDER BY a LIMIT 10 FORMAT JSON" --max_threads=1 --max_block_size=20 --optimize_read_in_order=1 --query_plan_read_in_order=1 --query_plan_lift_up_union=1 --allow_prefetched_read_pool_for_remote_filesystem=0  --allow_prefetched_read_pool_for_local_filesystem=0 --use_top_k_dynamic_filtering=0 --use_skip_indexes_for_top_k=0 | grep "rows_read" | sed 's/[^0-9]*//g')
