...
-- Statistics are built by the insert, and the runner randomizes that setting off in some runs.
...
-- claims monotonicity for any day number. Nullable reaches the range analysis, which strips only
-- LowCardinality; LowCardinality reaches the read-in-order consumers, which strip neither.
