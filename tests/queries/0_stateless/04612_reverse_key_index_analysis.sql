...
ENGINE = MergeTree ORDER BY (g, r DESC) SETTINGS add_minmax_index_for_numeric_columns = 0;
...
ENGINE = MergeTree ORDER BY (r DESC, g) SETTINGS add_minmax_index_for_numeric_columns = 0;
...
ENGINE = MergeTree ORDER BY (a DESC, b DESC) SETTINGS index_granularity = 2, add_minmax_index_for_numeric_columns = 0;
...
ENGINE = MergeTree ORDER BY (g, r DESC) SETTINGS index_granularity = 4, add_minmax_index_for_numeric_columns = 0;
...
ENGINE = MergeTree ORDER BY (g, r DESC)
SETTINGS index_granularity = 3, index_granularity_bytes = 0, min_rows_for_wide_part = 0, min_bytes_for_wide_part = 0, add_minmax_index_for_numeric_columns = 0;
...
ENGINE = MergeTree ORDER BY (a, b DESC, c) SETTINGS index_granularity = 4, add_minmax_index_for_numeric_columns = 0;
...
SETTINGS index_granularity = 4,
         primary_key_ratio_of_unique_prefix_values_to_skip_suffix_columns = 0.01, add_minmax_index_for_numeric_columns = 0;
...
ENGINE = MergeTree ORDER BY (g, r DESC) PRIMARY KEY (g, r) SETTINGS add_minmax_index_for_numeric_columns = 0;
...
CREATE TABLE t_asc (g String, r Int8) ENGINE = MergeTree ORDER BY (g, r) SETTINGS add_minmax_index_for_numeric_columns = 0;
...
ENGINE = MergeTree ORDER BY (g, r DESC) SETTINGS index_granularity = 1, add_minmax_index_for_numeric_columns = 0;
...
ENGINE = MergeTree ORDER BY (g, r) SETTINGS index_granularity = 1, add_minmax_index_for_numeric_columns = 0;
...
SELECT 'realistic ranges across merged parts';
-- Several inserts merged with FINAL exercise the merge path writing reverse-sorted keys, and the
-- per-group time ranges are nested (the middle group is denser and wider), so range conditions cut
...
ENGINE = MergeTree ORDER BY (org, dt DESC, id) SETTINGS index_granularity = 128, add_minmax_index_for_numeric_columns = 0;
