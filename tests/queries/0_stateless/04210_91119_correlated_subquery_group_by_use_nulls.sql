...
-- https://github.com/ClickHouse/ClickHouse/issues/109509 (STID 4752-5ad8)
-- A correlated key referenced from inside a function argument (e.g. `if(1, 2, a.k)`) can be
-- resolved into more than one ColumnNode instance when expression sharing is disabled
-- (compatibility='24.3'). Under group_by_use_nulls the analyzer wraps the correlated reference
-- to Nullable in place; because addCorrelatedColumn deduplicated by (name, type), a later still
-- non-Nullable duplicate escaped dedup and was registered a second time. Decorrelation then built
-- a JOIN with a duplicated Nullable input, tripping "Input nodes size mismatch in dag" in the join
-- optimizer. Dedup by (name, source) makes the correlated-columns list robust to the in-place type
-- change. Run under 24.3 so the duplicate resolution actually occurs; the query must complete.
SELECT '-- 109509 correlated key in function argument, GROUPING SETS ---';
SELECT (SELECT max(if(1, 2, a.k)))
FROM (SELECT number AS k FROM numbers(4)) AS a
GROUP BY GROUPING SETS ((k), ())
SETTINGS compatibility = '24.3', group_by_use_nulls = 1, allow_experimental_correlated_subqueries = 1;

-- The exact fuzzer query from the report (STID 4752-5ad8): -OrFill combinator over a correlated
-- key, LowCardinality constant and a positional group-by key. Must complete, not abort.
SELECT '-- 109509 exact fuzzer query completes ---';
SELECT count()
FROM
(
    SELECT (SELECT kurtPopDistinctStateArgMax(if(100, assumeNotNull(toLowCardinality(materialize(20))), a.k), toNullable(41)) IGNORE NULLS)
    FROM (SELECT number AS k FROM numbers_mt(64)) AS a
    GROUP BY GROUPING SETS ((materialize(toFixedString('256', 3)), k), ())
)
SETTINGS compatibility = '24.3', group_by_use_nulls = 1, allow_experimental_correlated_subqueries = 1;

-- Type pin: after the fix the correlated-columns list holds exactly one Nullable `k` (the
-- duplicate is gone). Count references to `k` in the whole dumped tree so a re-introduced
-- duplicate would change the count.
SELECT '-- 109509 type pin: correlated key deduplicated ---';
SELECT count()
FROM (EXPLAIN QUERY TREE
    SELECT (SELECT max(if(1, 2, a.k))) FROM (SELECT number AS k FROM numbers(4)) AS a
    GROUP BY GROUPING SETS ((k), ())
    SETTINGS compatibility = '24.3', group_by_use_nulls = 1, allow_experimental_correlated_subqueries = 1)
WHERE explain ILIKE '%column_name: k, result_type: Nullable(UInt64)%';

-- A genuinely distinct second correlated key (`j`) must still be kept: dedup by (name, source)
-- must not over-collapse different columns of the same source.
-- Wrapped in a subquery and ordered by the result: the GROUPING SETS rows have no inherent
-- order, so join-order randomization (query_plan_optimize_join_order_randomize) permutes them.
SELECT '-- 109509 two distinct correlated keys both preserved ---';
SELECT x FROM (
    SELECT (SELECT max(a.k) + max(a.j)) AS x
    FROM (SELECT number AS k, number + 1 AS j FROM numbers(4)) AS a
    GROUP BY GROUPING SETS ((k, j), ())
