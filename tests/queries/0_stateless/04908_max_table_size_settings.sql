...
CREATE TABLE t_max_size_rows (x UInt64) ENGINE = MergeTree ORDER BY x SETTINGS max_table_size_rows = 100;

-- The limits are checked against the current table size, so an insert that crosses the limit succeeds.
INSERT INTO t_max_size_rows SELECT number FROM numbers(90);
INSERT INTO t_max_size_rows SELECT number FROM numbers(20);
...
INSERT INTO t_max_size_rows SELECT number FROM numbers(200);
...
INSERT INTO t_max_size_mv_src SELECT number FROM numbers(20);
