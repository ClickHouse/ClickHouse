...
INSERT INTO prop_rf_orders   SELECT number, number % 1000, toString(number) FROM numbers(1000000);
INSERT INTO prop_rf_lineitem SELECT number, number % 1000, toString(number) FROM numbers(1000000);
...
FROM (
    ...
    SELECT count()
    FROM (SELECT * FROM prop_rf_orders WHERE orderkey = 12345) AS o
