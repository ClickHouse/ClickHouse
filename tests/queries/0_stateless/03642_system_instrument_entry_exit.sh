...
$CLICKHOUSE_CLIENT -q "
    ...
    SELECT handler, entry_type FROM system.trace_log WHERE event_date >= yesterday() AND event_time >= now() - 600 AND query_id = '$query_id' AND trace_type = 'Instrumentation' AND function_name LIKE '%QueryMetricLog::startQuery%' AND arrayExists(x -> x LIKE '%dispatchHandler%', symbols) ORDER BY event_time_microseconds, entry_type;
    SELECT (max(event_time_microseconds) - min(event_time_microseconds)) > 2 FROM system.text_log WHERE event_date >= yesterday() AND event_time >= now() - 600 AND query_id = '$query_id' AND message ILIKE '%this is an%log%';
"
