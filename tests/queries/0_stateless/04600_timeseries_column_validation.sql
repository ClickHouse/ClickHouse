...
--   - the `samples` column must have type `Array(Tuple(timestamp, value))` (BAD_TYPE_OF_FIELD);
--   - the timestamp and value element types are validated (BAD_TYPE_OF_FIELD);
--   - the documented `samples` type override is honored;
...
-- The `samples` column must have type Array(Tuple(timestamp, value)), not a scalar type.
CREATE TABLE ts_bad_shape (samples String) ENGINE = TimeSeries; -- { serverError BAD_TYPE_OF_FIELD }

-- The timestamp element must be a date/time type, not String.
CREATE TABLE ts_bad_timestamp_type (samples Array(Tuple(String, Float64))) ENGINE = TimeSeries; -- { serverError BAD_TYPE_OF_FIELD }

-- The value element must be a floating-point type, not String.
CREATE TABLE ts_bad_value_type (samples Array(Tuple(DateTime64(3), String))) ENGINE = TimeSeries; -- { serverError BAD_TYPE_OF_FIELD }
...
-- The documented `samples` type override is honored: the declared tuple element types propagate both to
-- the outer `samples` column AND inward to the generated samples INNER COLUMNS. Asserting only the outer
...
CREATE TABLE ts_override (samples Array(Tuple(UInt32, Float32))) ENGINE = TimeSeries;
SELECT type FROM system.columns WHERE database = currentDatabase() AND table = 'ts_override' AND name = 'samples';
...
-- Extra outer columns other than `samples` (e.g. `metric_name`, `tags`) are NOT rejected: the outer
