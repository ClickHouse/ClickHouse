...
SETTINGS disk = disk(name = '../user_scripts/05216', type = object_storage, object_storage_type = local, path = '05216_name/'); -- { serverError BAD_ARGUMENTS }

-- A relative path is resolved inside `custom_local_disks_base_directory`, so it cannot leave it ...
CREATE TABLE t_05216 (x UInt64) ENGINE = MergeTree ORDER BY x
...
-- ... and an absolute path has to be inside it.
CREATE TABLE t_05216 (x UInt64) ENGINE = MergeTree ORDER BY x
SETTINGS disk = disk(name = '05216_outside_absolute', type = object_storage, object_storage_type = local, path = '/etc/clickhouse-server/05216/'); -- { serverError BAD_ARGUMENTS }
...
CREATE TABLE t_05216 (x UInt64) ENGINE = MergeTree ORDER BY x
SETTINGS disk = disk(name = '05216_ok', type = object_storage, object_storage_type = local, path = '05216_ok/');
