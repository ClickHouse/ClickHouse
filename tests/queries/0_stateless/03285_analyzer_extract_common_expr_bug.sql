...
HAVING a
SETTINGS optimize_extract_common_expressions = 0, optimize_redundant_comparisons = 0;
...
GROUP BY (number > 10) OR ((number > 2) AND (number > 10)) AS a
HAVING a
SETTINGS optimize_extract_common_expressions = 1, optimize_redundant_comparisons = 0;
