...
INSERT INTO t_04651_no_nulls SELECT tuple(tuple(number::UInt32)) FROM numbers(4);
...
SETTINGS index_granularity = 1, min_bytes_for_wide_part = 1000000000, write_marks_for_substreams_in_compact_parts = 1;
INSERT INTO t_04651_compact SELECT number % 2 ? tuple(NULL) : tuple(tuple(number::UInt32)) FROM numbers(4);
...
INSERT INTO t_04651_wide SELECT number % 2 ? tuple(NULL) : tuple(tuple(number::UInt32)) FROM numbers(4);
...
INSERT INTO t_04651_top_level SELECT number, number % 2 ? NULL : tuple(number::UInt32) FROM numbers(4);
