...
# Defense-in-depth cleanup. `run_case` enables the failpoint and disables it inline, but if
...
# it and drop the per-label tables on exit. `DISABLE` on an inactive failpoint and DROP
# IF EXISTS are no-ops, so this is safe alongside the per-case cleanup.
trap '
    ${CLICKHOUSE_CLIENT} --query "SYSTEM DISABLE FAILPOINT storage_merge_tree_alter_fail_after_change_settings_before_metadata" 2>/dev/null || true
    ${CLICKHOUSE_CLIENT} --query "DROP TABLE IF EXISTS ${CLICKHOUSE_DATABASE}.t_04159_mt SYNC; DROP TABLE IF EXISTS ${CLICKHOUSE_DATABASE}.t_04159_rmt SYNC;" 2>/dev/null || true
' EXIT
