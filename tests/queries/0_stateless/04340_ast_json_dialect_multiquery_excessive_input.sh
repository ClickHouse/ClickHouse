...
${CLICKHOUSE_LOCAL} --enable_json_ast_dialect 1 --dialect clickhouse_json --multiquery -q "$JSON"

# 2. Two JSON AST objects separated by `;` both execute (the delimiter is accepted).
${CLICKHOUSE_LOCAL} --enable_json_ast_dialect 1 --dialect clickhouse_json --multiquery -q "$JSON ; $JSON"

# 3. Trailing non-delimiter text after a balanced JSON object is rejected as excessive input; the
#    prefix must NOT be executed (no `1` is printed), only the error is reported. Assert BOTH that the
#    error is present and that the prefix result `1` is absent — a regression that executes the prefix
#    and only fails on the next pseudo-statement would otherwise still pass.
OUT=$(${CLICKHOUSE_LOCAL} --enable_json_ast_dialect 1 --dialect clickhouse_json --multiquery -q "$JSON garbage" 2>&1)
...
${CLICKHOUSE_LOCAL} --enable_json_ast_dialect 1 --dialect clickhouse_json --multiquery --max_query_size 50 -q "${WS_PREFIX}${JSON}" 2>&1 | grep -om1 'Max query size'
