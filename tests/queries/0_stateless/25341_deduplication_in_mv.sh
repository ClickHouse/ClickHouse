...
$CLICKHOUSE_CLIENT --query="CREATE TABLE test.landing (time DateTime, number Int64) Engine=ReplicatedReplacingMergeTree('/clickhouse/tables/{shard}/{database}/test.landing','{replica}') PARTITION BY toYYYYMMDD(time) ORDER BY time";
$CLICKHOUSE_CLIENT --query="CREATE MATERIALIZED VIEW IF NOT EXISTS test.mv ENGINE = ReplicatedSummingMergeTree('/clickhouse/tables/{shard}/{database}/test.mv','{replica}') PARTITION BY toYYYYMMDD(hour) ORDER BY hour AS SELECT toStartOfHour(time) AS hour, sum(number) AS sum_amount FROM test.landing GROUP BY hour";
