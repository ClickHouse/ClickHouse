...
SET optimize_rewrite_array_exists_over_tokens = 1;
...
SELECT 'startsWith', count(), sum(id) FROM tab WHERE arrayExists(x -> startsWith(x, 'charg'), tokens(msg)) SETTINGS optimize_rewrite_array_exists_over_tokens = 0;
...
SELECT 'startsWith', count(), sum(id) FROM tab_plain WHERE arrayExists(x -> startsWith(x, 'charg'), tokens(msg)) SETTINGS optimize_rewrite_array_exists_over_tokens = 0;

SELECT 'endsWith', count(), sum(id) FROM tab WHERE arrayExists(x -> endsWith(x, 'rged'), tokens(msg));
SELECT 'endsWith', count(), sum(id) FROM tab WHERE arrayExists(x -> endsWith(x, 'rged'), tokens(msg)) SETTINGS optimize_rewrite_array_exists_over_tokens = 0;
...
SELECT 'endsWith', count(), sum(id) FROM tab_plain WHERE arrayExists(x -> endsWith(x, 'rged'), tokens(msg)) SETTINGS optimize_rewrite_array_exists_over_tokens = 0;

SELECT 'position', count(), sum(id) FROM tab WHERE arrayExists(x -> position(x, 'harg') > 0, tokens(msg));
SELECT 'position', count(), sum(id) FROM tab WHERE arrayExists(x -> position(x, 'harg') > 0, tokens(msg)) SETTINGS optimize_rewrite_array_exists_over_tokens = 0;
...
SELECT 'position', count(), sum(id) FROM tab_plain WHERE arrayExists(x -> position(x, 'harg') > 0, tokens(msg)) SETTINGS optimize_rewrite_array_exists_over_tokens = 0;

SELECT 'like', count(), sum(id) FROM tab WHERE arrayExists(x -> x LIKE 'ch%ed', tokens(msg));
SELECT 'like', count(), sum(id) FROM tab WHERE arrayExists(x -> x LIKE 'ch%ed', tokens(msg)) SETTINGS optimize_rewrite_array_exists_over_tokens = 0;
...
SELECT 'like', count(), sum(id) FROM tab_plain WHERE arrayExists(x -> x LIKE 'ch%ed', tokens(msg)) SETTINGS optimize_rewrite_array_exists_over_tokens = 0;

SELECT 'match', count(), sum(id) FROM tab WHERE arrayExists(x -> match(x, '^[0-9]{5}$'), tokens(msg));
SELECT 'match', count(), sum(id) FROM tab WHERE arrayExists(x -> match(x, '^[0-9]{5}$'), tokens(msg)) SETTINGS optimize_rewrite_array_exists_over_tokens = 0;
...
SELECT 'match', count(), sum(id) FROM tab_plain WHERE arrayExists(x -> match(x, '^[0-9]{5}$'), tokens(msg)) SETTINGS optimize_rewrite_array_exists_over_tokens = 0;

SELECT 'explicit tokenizer', count(), sum(id) FROM tab WHERE arrayExists(x -> startsWith(x, 'Charg'), tokens(msg, 'splitByNonAlpha'));
SELECT 'explicit tokenizer', count(), sum(id) FROM tab WHERE arrayExists(x -> startsWith(x, 'Charg'), tokens(msg, 'splitByNonAlpha')) SETTINGS optimize_rewrite_array_exists_over_tokens = 0;
...
SELECT 'NOT', count(), sum(id) FROM tab_plain WHERE NOT arrayExists(x -> position(x, 'harg') > 0, tokens(msg)) SETTINGS optimize_rewrite_array_exists_over_tokens = 0;

SELECT 'SELECT list', arrayExists(x -> endsWith(x, 'rged'), tokens(msg)) AS h, count() FROM tab GROUP BY h ORDER BY h;
SELECT 'SELECT list', arrayExists(x -> endsWith(x, 'rged'), tokens(msg)) AS h, count() FROM tab_plain GROUP BY h ORDER BY h SETTINGS optimize_rewrite_array_exists_over_tokens = 0;

SELECT 'empty needle', count() FROM tab WHERE arrayExists(x -> position(x, '') > 0, tokens(msg));
SELECT 'empty needle', count() FROM tab_plain WHERE arrayExists(x -> position(x, '') > 0, tokens(msg)) SETTINGS optimize_rewrite_array_exists_over_tokens = 0;
...
SELECT count() FROM tab_plain WHERE arrayExists(x -> position(x, 'har') > 0, tokens(msg, 'ngrams(3)')) SETTINGS optimize_rewrite_array_exists_over_tokens = 0;
...
SELECT count(), sum(id) FROM tab WHERE arrayExists(x -> startsWith(x, 'Charg'), tokens(msg)) SETTINGS optimize_rewrite_array_exists_over_tokens = 0;
...
SELECT count(), sum(id) FROM tab_plain WHERE arrayExists(x -> x LIKE 'Charg%', tokens(msg)) SETTINGS optimize_rewrite_array_exists_over_tokens = 0;
...
SELECT count(), sum(id) FROM tab_plain WHERE arrayExists(x -> startsWith(x, 'charg'), tokens(lower(msg), 'splitByNonAlpha')) SETTINGS optimize_rewrite_array_exists_over_tokens = 0;
...
SELECT 'startsWith', arraySort(groupArray(id)) FROM tab WHERE arrayExists(x -> startsWith(x, '50%_'), tokens(msg, 'splitByString([\' \'])')) SETTINGS optimize_rewrite_array_exists_over_tokens = 0;
SELECT 'endsWith', arraySort(groupArray(id)) FROM tab WHERE arrayExists(x -> endsWith(x, '_y'), tokens(msg, 'splitByString([\' \'])'));
SELECT 'endsWith', arraySort(groupArray(id)) FROM tab WHERE arrayExists(x -> endsWith(x, '_y'), tokens(msg, 'splitByString([\' \'])')) SETTINGS optimize_rewrite_array_exists_over_tokens = 0;
SELECT 'position', arraySort(groupArray(id)) FROM tab WHERE arrayExists(x -> position(x, '\\') > 0, tokens(msg, 'splitByString([\' \'])'));
SELECT 'position', arraySort(groupArray(id)) FROM tab WHERE arrayExists(x -> position(x, '\\') > 0, tokens(msg, 'splitByString([\' \'])')) SETTINGS optimize_rewrite_array_exists_over_tokens = 0;
...
-- `compatibility` does not change a setting that was set explicitly.
SET optimize_rewrite_array_exists_over_tokens = DEFAULT;
EXPLAIN SYNTAX run_query_tree_passes = 1 SELECT count() FROM tab WHERE arrayExists(x -> startsWith(x, '50%_'), tokens(msg));
