-- Golden test for the IEJoin algorithm: all 16 combinations of inequality operators
-- on a duplicate-heavy fixture, verified against a brute-force cross-join oracle.
-- The oracle queries use the comma-join syntax with the conditions in WHERE, which is
-- executed as a cross join with a filter (`cross_to_inner_join_rewrite` is disabled
-- so that they cannot be routed through IEJoin).
