...
-- settings cap onto the set-operation node just like UNION ALL. On the legacy interpreter
-- (enable_analyzer = 0) the cap is applied once after IntersectOrExceptStep as well, so both paths agree.
...
SELECT n FROM ((SELECT number AS n FROM numbers(10) LIMIT AFTER number >= 0) INTERSECT (SELECT number AS n FROM numbers(10) LIMIT AFTER number >= 0)) ORDER BY n SETTINGS limit = 3, enable_analyzer = 0;
...
SELECT n FROM ((SELECT number AS n FROM numbers(10) LIMIT AFTER number >= 0) EXCEPT (SELECT number AS n FROM numbers(5) LIMIT AFTER number >= 0)) ORDER BY n SETTINGS limit = 2, enable_analyzer = 0;
...
SELECT n FROM ((SELECT number AS n FROM numbers(10) LIMIT UNTIL number >= 8) INTERSECT (SELECT number AS n FROM numbers(10) LIMIT AFTER number >= 0)) ORDER BY n SETTINGS limit = 2, enable_analyzer = 0;
...
-- UNION ALL does not promise branch order, so the result is ordered before applying the cap to keep the
