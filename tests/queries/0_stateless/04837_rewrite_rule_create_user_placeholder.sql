...
CREATE RULE rule_create_user_rename_source AS (ALTER USER user_04837_a RENAME TO {u:Identifier}) REJECT WITH 'blocked'; -- { serverError REWRITE_RULE_UNSUPPORTED_QUERY_PARAMETER_TYPE }

-- Result template, `RENAME TO`.
CREATE RULE rule_create_user_rename_result AS (SELECT {u:String}) REWRITE TO (ALTER USER user_04837_a RENAME TO {u:Identifier}); -- { serverError REWRITE_RULE_UNSUPPORTED_QUERY_PARAMETER_TYPE }
...
ALTER RULE rule_create_user_ok AS (ALTER USER user_04837_a RENAME TO {u:Identifier}) REJECT WITH 'blocked'; -- { serverError REWRITE_RULE_UNSUPPORTED_QUERY_PARAMETER_TYPE }
