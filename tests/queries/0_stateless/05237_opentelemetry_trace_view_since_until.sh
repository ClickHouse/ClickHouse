...
# `today` is evaluated per statement and a run can cross midnight: every date below is relative to one value.
base_date=$(${CLICKHOUSE_CLIENT} -q "select today()")
...
${CLICKHOUSE_CLIENT} -q "
    ...
    values
        ('h', '$trace_id', 1, 0, 'query', 'SERVER',   1000, 5000, toDate('$base_date') - 10, map('clickhouse.query_id', '$query_id')),
        ('h', '$trace_id', 2, 1, 'child', 'INTERNAL', 2000, 3000, toDate('$base_date'),      map())
"
${CLICKHOUSE_CLIENT} -q "system flush logs opentelemetry_span_log"
...
${CLICKHOUSE_CLIENT} -q "select span from traceView('$trace_id', since = toString(toDate('$base_date') - 1)) format TSV"

echo "=== until five days ago: the root only ==="
${CLICKHOUSE_CLIENT} -q "select span from traceView('$trace_id', until = toString(toDate('$base_date') - 5)) format TSV"

echo "=== inclusive bounds: both ==="
${CLICKHOUSE_CLIENT} -q "select span from traceView('$trace_id', since = toString(toDate('$base_date') - 10), until = '$base_date') format TSV"

echo "=== an empty window says so ==="
${CLICKHOUSE_CLIENT} -q "select span from traceView('$trace_id', since = toString(toDate('$base_date') + 1))" 2>&1 | grep -m1 -o "within the since/until window"

echo "=== query_id lookup honours the window ==="
${CLICKHOUSE_CLIENT} -q "select span from traceView(query_id = '$query_id', since = toString(toDate('$base_date') - 10)) format TSV"
${CLICKHOUSE_CLIENT} -q "select span from traceView(query_id = '$query_id', since = toString(toDate('$base_date') - 1))" 2>&1 | grep -m1 -o "No trace found for query_id.*" | grep -m1 -o "within the since/until window"
