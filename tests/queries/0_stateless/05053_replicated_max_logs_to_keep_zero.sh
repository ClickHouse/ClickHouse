...
zk_path="/clickhouse/tables/$CLICKHOUSE_TEST_ZOOKEEPER_PREFIX/r"
...
$CLICKHOUSE_CLIENT --query "
    ...
    CREATE TABLE max_logs_zero (x UInt64) ENGINE = ReplicatedMergeTree('$zk_path', '1') ORDER BY x
...
trimmed=0
for _ in {1..120}; do
    count=$($CLICKHOUSE_CLIENT --query "SELECT numChildren FROM system.zookeeper WHERE path = '$zk_path' AND name = 'log'" 2>"$poll_err")
    rc=$?
...
$CLICKHOUSE_CLIENT --query "DROP TABLE max_logs_zero SYNC"
