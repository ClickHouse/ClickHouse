...
$CLICKHOUSE_CLIENT --query_plan_merge_expressions=1 --query_plan_push_down_limit=1 --query "EXPLAIN actions=1 SELECT value FROM keeper_map_with_filter LIMIT 1" | grep -A 2 "ReadFromKeeperMap"
...
$CLICKHOUSE_CLIENT --query_plan_merge_expressions=1 --query "EXPLAIN actions=1 SELECT value FROM keeper_map_with_filter WHERE key = '5'" | grep -A 3 "ReadFromKeeperMap"
...
$CLICKHOUSE_CLIENT --query_plan_merge_expressions=1 --query "EXPLAIN actions=1 SELECT value FROM keeper_map_with_filter_and_complex_key WHERE key = '5'" | grep -A 3 "ReadFromKeeperMap"
