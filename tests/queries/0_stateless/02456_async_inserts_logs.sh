...
for _ in $(seq 1 60); do
    ...
    count=$(${CLICKHOUSE_CLIENT} -q "SELECT count() FROM system.asynchronous_insert_log WHERE event_date >= yesterday() AND event_time >= now() - 600 AND (database = '$CLICKHOUSE_DATABASE' AND table = 't_async_inserts_logs' OR query ILIKE 'INSERT INTO FUNCTION%$CLICKHOUSE_DATABASE%t_async_inserts_logs%') AND data_kind = 'Parsed'")
    [ "$count" -ge 6 ] && break
    sleep 0.5
done
${CLICKHOUSE_CLIENT} -q "
