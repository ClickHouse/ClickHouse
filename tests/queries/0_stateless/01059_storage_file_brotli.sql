...
CREATE TABLE file (x UInt64) ENGINE = File(TSV, 'data5.tsv.lz4');
TRUNCATE TABLE file;
...
SELECT count(), max(x) FROM file('data{1,2,3,4,5}.tsv.{gz,br,xz,zst,lz4}', TSV, 'x UInt64');
...
SELECT count() < 1000000, max(x) FROM file('data5.tsv.lz4', RowBinary, 'x UInt8', 'none');
