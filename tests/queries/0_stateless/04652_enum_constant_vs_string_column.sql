...
-- An `Enum` element inside a container keeps its name as well: the `Array` / `Tuple` / `Map` recursion of
-- `convertFieldToType` passes the matching element type hint down, so the element is converted the same
-- way a scalar is.
