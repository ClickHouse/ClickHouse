#!/usr/bin/env bash
# Tags: no-fasttest
# Inserting through a regular view runs a nested INSERT pipeline inside SinkToStorageView.
# When the outer query is cancelled without an exception (timeout_overflow_mode='break'),
# SinkToStorageView must finalize that nested executor; otherwise ~PushingPipelineExecutor aborts
...
${CLICKHOUSE_CLIENT} --query "
    CREATE TABLE view_cancel_target (x UInt64) ENGINE = MergeTree ORDER BY x;
    CREATE VIEW view_cancel AS SELECT x FROM view_cancel_target;
"

# The SELECT is throttled with sleepEachRow so the 0.3s execution timeout reliably fires
# while the nested SinkToStorageView insert pipeline is still running. timeout_overflow_mode='break'
# cancels the pipeline without raising an exception. Before the fix this crashed the server.
${CLICKHOUSE_CLIENT} --query "
    INSERT INTO view_cancel
...
${CLICKHOUSE_CLIENT} --query "
    DROP TABLE view_cancel;
    DROP TABLE view_cancel_target;
"
