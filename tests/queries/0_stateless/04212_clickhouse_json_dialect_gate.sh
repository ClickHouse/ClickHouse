...
# 1. Without `enable_json_ast_dialect`, the `clickhouse_json` dialect
...
${CLICKHOUSE_CURL} -sS "${CLICKHOUSE_URL}&enable_json_ast_dialect=1&dialect=clickhouse_json" --data-binary @- <<< "SET dialect = 'clickhouse'"
echo 'set ok'

# 2a. The `SET` escape hatch must work even when the experimental gate is off, so a
#     session with `dialect = clickhouse_json` and `enable_json_ast_dialect = 0`
...
${CLICKHOUSE_CLIENT} --enable_json_ast_dialect 1 --multiquery -q "SET dialect = 'clickhouse_json'; SET dialect = 'clickhouse'; SELECT 100;"

# 5. The same `SET` escape hatch must work in `clickhouse-local`.
${CLICKHOUSE_LOCAL} --enable_json_ast_dialect 1 --multiquery -q "SET dialect = 'clickhouse_json'; SET dialect = 'clickhouse'; SELECT 200;"
