...
-- `CANNOT_EXTRACT_TABLE_STRUCTURE` before it ever announces. The regex is anchored to the unique
-- table name so concurrent tests cannot leak into the merge. `index_granularity = 128` matters:
-- at the default granularity the initiator's replica claims every mark range and the followers
-- are cancelled before announcing, so the collision never fires.

USE default;

DROP TABLE IF EXISTS default.t_merge_pr_dup_announce;

CREATE TABLE default.t_merge_pr_dup_announce (dt DateTime, idx Int32, i Nullable(UInt64))
ENGINE = MergeTree PARTITION BY (idx % 3) ORDER BY idx SETTINGS index_granularity = 128;

INSERT INTO default.t_merge_pr_dup_announce SELECT toDateTime(number), number, number FROM numbers(500000);
...
SELECT count() FROM default.t_merge_pr_dup_announce
WHERE idx GLOBAL IN (i NOT IN (SELECT i FROM merge('^t_merge_pr_dup_announce$') WHERE dt >= -2147483648));

SET parallel_replicas_local_plan = 1;
SELECT count() FROM default.t_merge_pr_dup_announce
WHERE idx GLOBAL IN (i NOT IN (SELECT i FROM merge('^t_merge_pr_dup_announce$') WHERE dt >= -2147483648));

DROP TABLE default.t_merge_pr_dup_announce;
