...
select * from xp where i global in (select * from numbers(2)) SETTINGS enable_parallel_replicas=0;
...
select * from xp_d where i global in (select * from numbers(2)) SETTINGS enable_parallel_replicas=0;
