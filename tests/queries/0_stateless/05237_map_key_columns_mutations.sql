...
-- Mutation surface of the `with_key_columns` Map serialization: ALTER DELETE and
-- ALTER UPDATE assigning the Map column are rejected; updating other columns works
-- and the Map survives; MATERIALIZE COLUMN and TTL mutations don't corrupt the Map;
-- MOVE / ATTACH PARTITION between two with_key_columns tables works.
...
ALTER TABLE t_mut DELETE WHERE id = 1; -- { serverError SUPPORT_IS_DISABLED }
ALTER TABLE t_mut UPDATE m = map('z', 9) WHERE id = 2; -- { serverError SUPPORT_IS_DISABLED }

-- Updating a different column works and the Map survives.
ALTER TABLE t_mut UPDATE v = 99 WHERE id = 1 SETTINGS mutations_sync = 2;
...
SELECT id, v, m, mapContains(m, 'a'), mapContains(m, 'c') FROM t_mut ORDER BY id;
