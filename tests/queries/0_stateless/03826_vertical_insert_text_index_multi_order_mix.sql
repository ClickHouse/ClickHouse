-- Test: text indexes with mixed ORDER BY (text + non-text) work with vertical insert.
SET enable_full_text_index = 1;
SET materialize_skip_indexes_on_insert = 1;

DROP TABLE IF EXISTS t_vi_text_index_multi_order_mix;

CREATE TABLE t_vi_text_index_multi_order_mix
(
    id UInt64,
    s String,
    t String,
...
INSERT INTO t_vi_text_index_multi_order_mix
SELECT number, toString(number), toString(number * 2) FROM numbers(1000);

SELECT count()
FROM system.parts
WHERE database = currentDatabase()
  AND table = 't_vi_text_index_multi_order_mix'
  AND active
  AND secondary_indices_uncompressed_bytes > 0;

DROP TABLE t_vi_text_index_multi_order_mix;
