...
for _ in $(seq 1 60); do
    ...
    count=$($CLICKHOUSE_CLIENT -q "SELECT count() FROM system.query_log WHERE event_date >= yesterday() AND event_time >= now() - 600 AND current_database = currentDatabase() AND query LIKE 'INSERT INTO test_m%_insert_%_sh FORMAT TSV%' AND type = 'QueryFinish'")
    [ "$count" -ge 4 ] && break
    sleep 0.5
done

$CLICKHOUSE_CLIENT -q "SELECT count() FROM system.part_log WHERE event_date >= yesterday() AND event_time >= now() - 600 AND table = 'test_max_insert_bytes_sh' AND event_type = 'NewPart' AND (query_id = (SELECT argMax(query_id, event_time) FROM system.query_log WHERE event_date >= yesterday() AND event_time >= now() - 600 AND query LIKE '%INSERT INTO test_max_insert_bytes_sh FORMAT T%' AND current_database = currentDatabase()))"
$CLICKHOUSE_CLIENT -q "SELECT count() FROM system.part_log WHERE event_date >= yesterday() AND event_time >= now() - 600 AND table = 'test_min_insert_rows_bytes_sh' AND event_type = 'NewPart' AND (query_id = (SELECT argMax(query_id, event_time) FROM system.query_log WHERE event_date >= yesterday() AND event_time >= now() - 600 AND query LIKE '%INSERT INTO test_min_insert_rows_bytes_sh FORMAT TSV%' AND current_database = currentDatabase()))"
$CLICKHOUSE_CLIENT -q "SELECT count() FROM system.part_log WHERE event_date >= yesterday() AND event_time >= now() - 600 AND table = 'test_min_insert_rows_sh' AND event_type = 'NewPart' AND (query_id = (SELECT argMax(query_id, event_time) FROM system.query_log WHERE event_date >= yesterday() AND event_time >= now() - 600 AND query LIKE '%INSERT INTO test_min_insert_rows_sh FORMAT TSV%' AND current_database = currentDatabase()))"
$CLICKHOUSE_CLIENT -q "SELECT count() FROM system.part_log WHERE event_date >= yesterday() AND event_time >= now() - 600 AND table = 'test_min_insert_bytes_sh' AND event_type = 'NewPart' AND (query_id = (SELECT argMax(query_id, event_time) FROM system.query_log WHERE event_date >= yesterday() AND event_time >= now() - 600 AND query LIKE '%INSERT INTO test_min_insert_bytes_sh FORMAT TSV%' AND current_database = currentDatabase()))"
