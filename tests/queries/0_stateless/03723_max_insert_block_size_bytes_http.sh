...
$CLICKHOUSE_CLIENT -q "SELECT count() FROM system.part_log WHERE table = 'test_max_insert_bytes_sh' AND event_type = 'NewPart' AND (query_id IN (SELECT query_id FROM system.query_log WHERE query LIKE '%INSERT INTO test_max_insert_bytes_sh FORMAT T%' AND current_database = currentDatabase() ORDER BY event_time DESC LIMIT 1))"
$CLICKHOUSE_CLIENT -q "SELECT count() FROM system.part_log WHERE table = 'test_min_insert_rows_bytes_sh' AND event_type = 'NewPart' AND (query_id IN (SELECT query_id FROM system.query_log WHERE query LIKE '%INSERT INTO test_min_insert_rows_bytes_sh FORMAT TSV%' AND current_database = currentDatabase() ORDER BY event_time DESC LIMIT 1))"
$CLICKHOUSE_CLIENT -q "SELECT count() FROM system.part_log WHERE table = 'test_min_insert_rows_sh' AND event_type = 'NewPart' AND (query_id IN (SELECT query_id FROM system.query_log WHERE query LIKE '%INSERT INTO test_min_insert_rows_sh FORMAT TSV%' AND current_database = currentDatabase() ORDER BY event_time DESC LIMIT 1))"
$CLICKHOUSE_CLIENT -q "SELECT count() FROM system.part_log WHERE table = 'test_min_insert_bytes_sh' AND event_type = 'NewPart' AND (query_id IN (SELECT query_id FROM system.query_log WHERE query LIKE '%INSERT INTO test_min_insert_bytes_sh FORMAT TSV%' AND current_database = currentDatabase() ORDER BY event_time DESC LIMIT 1))"
