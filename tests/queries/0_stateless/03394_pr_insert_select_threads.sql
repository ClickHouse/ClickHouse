...
INSERT INTO t_rmt_target SELECT * FROM t_mt_source SETTINGS allow_experimental_parallel_reading_from_replicas=1, max_threads=8, max_insert_threads=4, max_block_size=1000, min_insert_block_size_rows=1000, parallel_replicas_local_plan=1;

SYSTEM FLUSH LOGS query_log;
SELECT
    if(is_initial_query, 'inital', 'secondary'),
    if(toUInt64OrZero(Settings['parallel_replicas_local_plan']) == 0 and is_initial_query, 1, 8) threads_limit,
    least(peak_threads_usage, threads_limit),
    format('local_plan={}', Settings['parallel_replicas_local_plan'])
