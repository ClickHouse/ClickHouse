...
-- Not `= 'default'`: a server whose `<merge_tree>` configuration section sets this reports `config`,
-- and the test harness randomizes MergeTree settings into the `CREATE` query of every table it creates.
SELECT name, source != 'definition' AS not_stated FROM system.table_settings
