...
SELECT 'grouping sets nonempty at empty result for empty set', min(number + 1) AS v FROM numbers(4) GROUP BY GROUPING SETS ((number % 2), ()) ORDER BY v SETTINGS optimize_arithmetic_operations_in_aggregate_functions = 1, empty_result_for_aggregation_by_empty_set = 1;
SELECT 'grouping sets nonempty at empty result for empty set', min(number + 1) AS v FROM numbers(4) GROUP BY GROUPING SETS ((number % 2), ()) ORDER BY v SETTINGS optimize_arithmetic_operations_in_aggregate_functions = 0, empty_result_for_aggregation_by_empty_set = 1;
