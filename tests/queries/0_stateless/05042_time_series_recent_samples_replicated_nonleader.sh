...
# Tag no-replicated-database: the test creates two replicas for inner and external samples targets.
...
ZK_PATH="/clickhouse/tables/${CLICKHOUSE_TEST_ZOOKEEPER_PREFIX}/05042_recent_samples"
...
SAMPLES_R1=$(${CLICKHOUSE_CLIENT} -q "
    ...
    WHERE database = currentDatabase() AND name LIKE '.inner_id.samples.%' AND engine_full LIKE '%${ZK_PATH}/samples%'")
...
${CLICKHOUSE_CLIENT} --allow_experimental_time_series_table=1 -q "
    ...
    ('nonleader_metric', map(), [(toDateTime64('2000-01-01 00:00:00', 3), 1.)]),
    ('nonleader_metric', map(), [(toDateTime64('2000-01-01 10:00:00', 3), 2.)])"

${CLICKHOUSE_CLIENT} -q "SYSTEM SYNC REPLICA \`${SAMPLES_R1}\`; SYSTEM SYNC REPLICA \`${RECENT_R1}\`"
...
${CLICKHOUSE_CLIENT} -q "DROP TABLE ts_recent_r2 SYNC; DROP TABLE ts_recent_r1 SYNC"
