-- The second query's GROUP BY constants fold to the same key, making the written list repeat it.
-- A repeated key gains its grouping sets only in the planner; `enable_analyzer = 0` still expands
-- from the deduplicated key list and produces fewer rows than the reference.
