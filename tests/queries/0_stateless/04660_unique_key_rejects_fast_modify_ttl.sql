...
-- The fast `MODIFY TTL` path records the materialization as an internal `MATERIALIZE TTL <delta>`
...
-- and the rewrite bypasses the dedup path), so it must reject the delta form as well - otherwise
...
-- delta mutation that arrives from elsewhere, e.g. a replication log entry written by a replica
...
-- The internal delta form is not reachable from SQL at all.
ALTER TABLE uk_fast_ttl MATERIALIZE TTL 100; -- { serverError BAD_ARGUMENTS }
