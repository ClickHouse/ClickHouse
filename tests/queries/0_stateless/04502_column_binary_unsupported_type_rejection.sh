...
# ColumnBinary/COLUMNAR_V1 cannot represent Map, LowCardinality, or nested Nullable/
# Variant inside Array/Tuple. These signatures must be rejected at format construction
# time, before any block is serialized, rather than failing partway through the first
# block. Fixed-width types of any size (UUID, IPv6, Int128/UInt128, Decimal128/256) and
# FixedString(N) of any length are supported via COL_FIXEDN — see
# 04506_column_binary_wide_fixed_width for their round-trip coverage.
