...
SELECT query, used_row_policies FROM system.query_log WHERE event_date >= yesterday() AND event_time >= now() - 600 AND current_database == currentDatabase() AND is_initial_query AND type == 'QueryStart' AND query_kind == 'Select' ORDER BY event_time_microseconds;
