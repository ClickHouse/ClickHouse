...
-- why: a heavyweight UPDATE/DELETE on a COMPACT part right after a metadata-only
-- RENAME (no reload) must remap part_columns by stamped ID -- the non-wide split
-- branch otherwise emits a READ_COLUMN for the part's stale load-time name.
CREATE TABLE t_compact_mutate_rename (a UInt32, b String, c Float64)
ENGINE = MergeTree ORDER BY a
SETTINGS serialization_info_version = 'with_column_ids',
         min_bytes_for_wide_part = 1000000000,
         min_rows_for_wide_part = 1000000000;
INSERT INTO t_compact_mutate_rename VALUES (1, 'hello', 1.5), (2, 'world', 2.5), (3, 'foo', 3.5);
ALTER TABLE t_compact_mutate_rename RENAME COLUMN b TO d;
ALTER TABLE t_compact_mutate_rename UPDATE c = c + 100 WHERE d = 'world' SETTINGS mutations_sync = 1;
SELECT 'compact_after_update', a, d, c FROM t_compact_mutate_rename ORDER BY a;
ALTER TABLE t_compact_mutate_rename DELETE WHERE d = 'foo' SETTINGS mutations_sync = 1;
SELECT 'compact_after_delete', a, d, c FROM t_compact_mutate_rename ORDER BY a;
DROP TABLE t_compact_mutate_rename SYNC;

-- why: DROP COLUMN q then RENAME a sibling INTO the freed name q (both metadata-only)
-- makes the ID-keyed part_columns remap target a name still present on the
-- not-yet-rewritten part -- the remap must resolve that collision without losing a column.
CREATE TABLE t_compact_rename_into_dropped (a UInt32, p String, q String, n Int32)
ENGINE = MergeTree ORDER BY a
SETTINGS serialization_info_version = 'with_column_ids',
         min_bytes_for_wide_part = 1000000000,
         min_rows_for_wide_part = 1000000000;
INSERT INTO t_compact_rename_into_dropped VALUES (1, 'p1', 'q1', 0), (2, 'p2', 'q2', 0);
ALTER TABLE t_compact_rename_into_dropped DROP COLUMN q;
ALTER TABLE t_compact_rename_into_dropped RENAME COLUMN p TO q;
ALTER TABLE t_compact_rename_into_dropped UPDATE n = n + 5 WHERE q = 'p1' SETTINGS mutations_sync = 1;
SELECT 'compact_rename_into_dropped', a, q, n FROM t_compact_rename_into_dropped ORDER BY a;
DROP TABLE t_compact_rename_into_dropped SYNC;

-- why: statistics files are named by stamped column ID; a heavyweight mutation on a
-- packed part after a metadata-only RENAME must not re-derive a stale-named READ_COLUMN
-- (previously surfaced as FILE_ALREADY_EXISTS when rewriting the statistics file).
SET allow_statistics = 1;
SET materialize_statistics_on_insert = 1;
CREATE TABLE t_packed_stats_rename (a UInt64, v UInt64 STATISTICS(basic))
ENGINE = MergeTree ORDER BY a
SETTINGS serialization_info_version = 'with_column_ids',
         min_bytes_for_wide_part = 1000000000,
         min_rows_for_wide_part = 1000000000,
         min_bytes_for_full_part_storage = 1000000000,
         min_rows_for_full_part_storage = 1000000000,
         auto_statistics_types = '';
INSERT INTO t_packed_stats_rename SELECT number, number * 10 FROM numbers(100);
ALTER TABLE t_packed_stats_rename RENAME COLUMN v TO w;
ALTER TABLE t_packed_stats_rename UPDATE w = w + 1 WHERE a < 5 SETTINGS mutations_sync = 1;
SELECT 'packed_stats', sum(w), count() FROM t_packed_stats_rename;
DROP TABLE t_packed_stats_rename SYNC;
