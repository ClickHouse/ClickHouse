...
WITH minSampleSizeContinous(baseline, sigma, mde, power, alpha) AS res SELECT res.1, roundBankers(res.2, 2), roundBankers(res.3, 2) FROM minimum_sample_size_continuos ORDER BY res.1;
DROP TABLE IF EXISTS minimum_sample_size_continuos;
...
WITH minSampleSizeConversion(p1, mdes, power, alpha) AS res SELECT res.1, roundBankers(res.2, 2), roundBankers(res.3, 2) FROM minimum_sample_size_conversion ORDER BY res.1;
DROP TABLE IF EXISTS minimum_sample_size_conversion;
