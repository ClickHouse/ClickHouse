-- Plan-level integration of the partitioned hash join with the optimizations that previously
-- enumerated only `hash` / `parallel_hash`. JOIN runtime filters are built for it
...
SELECT '-- a runtime filter is planted for hash';
SELECT count() > 0 FROM (
...
SELECT 'inner results are unchanged by the runtime filter', pa, pa = pf FROM (SELECT
    ...
    (SELECT (count(), sum(cityHash64(p.p, b.v))) FROM t_phj_plan_probe AS p INNER JOIN t_phj_plan_build AS b ON p.k = b.k SETTINGS join_algorithm = 'hash', enable_join_runtime_filters = 1) AS pf);

SELECT '-- non-joined rows come from parallel sources under hash';
SELECT count() > 0 FROM (
...
SELECT 'right parity across algorithms and stream counts', pa1, pa1 = pa4, pa1 = pa16 FROM (SELECT
    (SELECT (count(), sum(cityHash64(p.p, b.v))) FROM t_phj_plan_probe AS p RIGHT JOIN t_phj_plan_build AS b ON p.k = b.k SETTINGS join_algorithm = 'hash', max_threads = 1) AS pa1,
    (SELECT (count(), sum(cityHash64(p.p, b.v))) FROM t_phj_plan_probe AS p RIGHT JOIN t_phj_plan_build AS b ON p.k = b.k SETTINGS join_algorithm = 'hash', max_threads = 4) AS pa4,
    (SELECT (count(), sum(cityHash64(p.p, b.v))) FROM t_phj_plan_probe AS p RIGHT JOIN t_phj_plan_build AS b ON p.k = b.k SETTINGS join_algorithm = 'hash', max_threads = 16) AS pa16);

SELECT 'full parity across algorithms and stream counts', pa1, pa1 = pa16 FROM (SELECT
    (SELECT (count(), sum(cityHash64(p.p, b.v))) FROM t_phj_plan_probe AS p FULL JOIN t_phj_plan_build AS b ON p.k = b.k SETTINGS join_algorithm = 'hash', max_threads = 1) AS pa1,
    (SELECT (count(), sum(cityHash64(p.p, b.v))) FROM t_phj_plan_probe AS p FULL JOIN t_phj_plan_build AS b ON p.k = b.k SETTINGS join_algorithm = 'hash', max_threads = 16) AS pa16);

SELECT 'right parity with join_use_nulls', pa FROM (SELECT
    (SELECT (count(), sum(cityHash64(p.p, b.v))) FROM t_phj_plan_probe AS p RIGHT JOIN t_phj_plan_build AS b ON p.k = b.k SETTINGS join_algorithm = 'hash', join_use_nulls = 1, max_threads = 16) AS pa);

SELECT 'a one-partition build still emits every non-joined row', pa FROM (SELECT
    (SELECT (count(), sum(cityHash64(p.p, b.v))) FROM t_phj_plan_probe AS p RIGHT JOIN (SELECT * FROM t_phj_plan_build ORDER BY k LIMIT 8) AS b ON p.k = b.k SETTINGS join_algorithm = 'hash', max_threads = 16) AS pa);

SELECT '-- convert_join_to_in rewrites a qualifying hash join';
SELECT count() FROM (
    EXPLAIN SELECT p.k FROM t_phj_plan_probe AS p INNER JOIN (SELECT k FROM t_phj_plan_build) AS b ON p.k = b.k
    SETTINGS join_algorithm = 'hash', query_plan_convert_join_to_in = 1
) WHERE explain LIKE '%Join%';

SELECT 'the rewritten query gives the same rows as the join', j.1, j = i FROM (SELECT
    (SELECT (count(), sum(p.k)) FROM t_phj_plan_probe AS p INNER JOIN (SELECT k FROM t_phj_plan_build) AS b ON p.k = b.k SETTINGS join_algorithm = 'hash', query_plan_convert_join_to_in = 0) AS j,
    (SELECT (count(), sum(p.k)) FROM t_phj_plan_probe AS p INNER JOIN (SELECT k FROM t_phj_plan_build) AS b ON p.k = b.k SETTINGS join_algorithm = 'hash', query_plan_convert_join_to_in = 1) AS i);
