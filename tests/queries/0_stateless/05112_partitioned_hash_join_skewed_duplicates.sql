...
-- The scatter split into block ranges (a budget between about 81 million bytes, the predicted memory
-- with the split, and 96 million, without it) with one key of about 40000 rows: each block range appends
-- one more run of that key's duplicates, and the probe reads the runs in range order. The two keys pack
-- into `keys128`. The same budget for a RIGHT and a FULL join with non-joined rows, and for a build whose
-- hot key is the zero key.
