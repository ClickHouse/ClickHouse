#include <stdint.h>
#include <stddef.h>

typedef struct {
    uint8_t * data;
    uint32_t size;
} Span;

#define HEAP_SIZE (1 << 16)
static _Alignas(16) uint8_t heap[HEAP_SIZE];
static uint32_t heap_pos = 0;

#define MAX_SPANS 64
static Span spans[MAX_SPANS];
static uint32_t span_pos = 0;

Span * clickhouse_create_buffer(uint32_t size) {
    if (span_pos >= MAX_SPANS) return NULL;
    if (heap_pos + size > HEAP_SIZE) return NULL;
    Span * span = &spans[span_pos++];
    span->data = &heap[heap_pos];
    span->size = size;
    heap_pos += (size + 15) & ~15u;
    return span;
}

void clickhouse_destroy_buffer(Span * data) {
    (void)data;
}

/// Returns a buffer that declares a non-zero size, but points at linear-memory offset 0.
Span * returns_null_data_pointer(Span * span, uint32_t n) {
    (void)span;
    (void)n;
    Span * res = clickhouse_create_buffer(0);
    res->data = (uint8_t *)0;
    res->size = 42;
    return res;
}

/// A well-behaved function from the same module, to show the module itself is fine.
Span * null_data_test_func(Span * span, uint32_t n) {
    Span * res = clickhouse_create_buffer(n * 2);
    char * input = (char *)span->data;
    char * output = (char *)res->data;
    for (uint32_t row_num = 0; row_num < n; row_num++) {
        *output = *input;
        output++;
        *output = '\n';
        output++;
        while (*input != '\n') {
            input++;
        }
        input++;
    }
    return res;
}
