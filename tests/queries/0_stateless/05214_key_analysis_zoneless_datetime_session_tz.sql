...
SELECT timeZoneOf(dt) FROM 05214_day LIMIT 1 SETTINGS session_timezone = 'Asia/Tokyo';

SELECT '';
SELECT '-- monotonic chain over a zone-less key, equality and range';
SELECT (SELECT count() FROM 05214_day WHERE dt = '2024-01-03 05:00:00') AS pruned,
       (SELECT countIf(dt = '2024-01-03 05:00:00') FROM 05214_day) AS honest
SETTINGS session_timezone = 'Asia/Tokyo';
SELECT (SELECT count() FROM 05214_day WHERE dt >= '2024-01-03 05:00:00' AND dt < '2024-01-03 06:00:00') AS pruned,
       (SELECT countIf(dt >= '2024-01-03 05:00:00' AND dt < '2024-01-03 06:00:00') FROM 05214_day) AS honest
SETTINGS session_timezone = 'Asia/Tokyo';
...
SELECT (SELECT count() FROM 05214_hour WHERE dt = '2024-01-03 05:00:00') AS pruned,
       (SELECT countIf(dt = '2024-01-03 05:00:00') FROM 05214_hour) AS honest
SETTINGS session_timezone = 'Asia/Tokyo';
SELECT trim(explain)
FROM (
    EXPLAIN indexes = 1 SELECT count() FROM 05214_hour WHERE dt = '2024-01-03 05:00:00'
)
...
SELECT (SELECT count() FROM 05214_cast_date WHERE dt = '2024-01-03 05:00:00') AS pruned,
       (SELECT countIf(dt = '2024-01-03 05:00:00') FROM 05214_cast_date) AS honest
SETTINGS session_timezone = 'Asia/Tokyo';
...
SELECT trim(explain)
FROM (
    EXPLAIN indexes = 1 SELECT count() FROM 05214_day WHERE dt = '2024-01-03 05:00:00'
)
WHERE trim(explain) ilike 'condition: %' OR trim(explain) ilike 'parts: %'
SETTINGS session_timezone = 'Asia/Tokyo';
SELECT trim(explain)
FROM (
    EXPLAIN indexes = 1 SELECT count() FROM 05214_day WHERE dt = '2024-02-01 05:00:00'
)
WHERE trim(explain) ilike 'condition: %' OR trim(explain) ilike 'parts: %'
SETTINGS session_timezone = 'Asia/Tokyo';
...
SELECT (SELECT count() FROM 05214_explicit_zone WHERE dt = '2024-01-02 03:00:00') AS pruned,
       (SELECT countIf(dt = '2024-01-02 03:00:00') FROM 05214_explicit_zone) AS honest
SETTINGS session_timezone = 'Asia/Tokyo';

SELECT '';
SELECT '-- negative control: a constant that is not text is not parsed, so it keeps its own zone';
SELECT (SELECT count() FROM 05214_day WHERE dt = toDateTime64('2024-01-02 20:00:00', 6, 'UTC')) AS pruned,
       (SELECT countIf(dt = toDateTime64('2024-01-02 20:00:00', 6, 'UTC')) FROM 05214_day) AS honest
SETTINGS session_timezone = 'Asia/Tokyo';
...
INSERT INTO 05214_dynamic VALUES ('alpha', 1), ('beta', 2);
SELECT (SELECT count() FROM 05214_dynamic WHERE d = 'alpha') AS pruned,
       (SELECT countIf(d = 'alpha') FROM 05214_dynamic) AS honest
SETTINGS session_timezone = 'Asia/Tokyo';
SELECT trim(explain)
FROM (
    EXPLAIN indexes = 1 SELECT count() FROM 05214_dynamic WHERE d = 'alpha'
)
