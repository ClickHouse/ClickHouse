...
CREATE TABLE t_05042 (a UInt64, ts DateTime)
ENGINE = MergeTree PARTITION BY toYYYYMMDD(ts) ORDER BY (a, ts);

INSERT INTO t_05042 SELECT 1, toDateTime('2026-07-19 00:00:00') + toIntervalDay(number % 32) FROM numbers(3200);
INSERT INTO t_05042 SELECT 1, toDateTime('2026-08-20 00:00:00') + toIntervalSecond(number) FROM numbers(20000);

SELECT countIf(explain LIKE '%Condition: true%') AS indexes_not_used
FROM
(
    EXPLAIN indexes = 1
    SELECT count() FROM (SELECT a FROM t_05042 WHERE ts >= '2026-08-20 00:00:00'
