...
# `system.user_query_log` must be readable right after the server started, before the query log
# was ever flushed. The backing table of a system log is created lazily, on the first flush, so
# without preparing it upfront the very first read from the view fails with `UNKNOWN_TABLE`.
...
# sockets) running in parallel during the test run.
free_tcp_port()
{
    python3 -c "
import socket
...
<clickhouse>
    ...
    <!-- A very long flush interval, so that nothing is flushed while the test is running. -->
    <query_log>
        <database>system</database>
        <table>query_log</table>
        <flush_interval_milliseconds>600000</flush_interval_milliseconds>
...
# The source table of the view is prepared while the view is attached, without waiting for a flush.
${CLICKHOUSE_CLIENT_BINARY} --host 127.0.0.1 --port "${tcp_port}" --query "
    SELECT count()
    FROM system.tables
    WHERE database = 'system' AND name = 'query_log'"

# Nothing was flushed yet, so the view is readable and empty.
