...
# - no-object-storage: the table has one part per partition per INSERT across many partitions, each
#   part carrying a projection. Writing all of those parts to object storage is slow, and combined
#   with a sanitizer build and randomized settings it pushes the test past the per-test time limit
#   (it takes ~100s even when it passes). The property under test - that OPTIMIZE FINAL bounds the
#   number of concurrent partition merges by the worker budget - does not depend on the storage
#   backend, so skipping the object-storage runs loses no coverage.
