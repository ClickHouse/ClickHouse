...
FROM (SELECT arrayStringConcat(groupArray(explain), '\n') AS plan FROM (EXPLAIN indexes = 1 SELECT sum(length(time_series)) FROM timeSeriesSelector(ts_clustered, 'foo', 0, 1000)));
...
SELECT
    ...
    countIf(explain ILIKE '%filter column:%' AND explain ILIKE '%bucket%') > 0 AS filter_lines_present
FROM (EXPLAIN indexes = 1 SELECT sum(length(time_series)) FROM timeSeriesSelector(ts_clustered, 'foo', 0, 1000));
...
FROM (SELECT arrayStringConcat(groupArray(explain), '\n') AS plan FROM (EXPLAIN indexes = 1 SELECT sum(length(time_series)) FROM timeSeriesSelector(ts_clustered, 'foo{env=~".*"}', 0, 1000)));
...
SELECT plan LIKE '%ffffffff-ffff-ffff-ffff-ffffffffffff%' AS has_id_range
FROM (SELECT arrayStringConcat(groupArray(explain), '\n') AS plan FROM (EXPLAIN indexes = 1 SELECT sum(length(time_series)) FROM timeSeriesSelector(ts_clustered, 'foo{env="prod"}', 0, 1000)));
...
SELECT plan LIKE '%ffffffff-ffff-ffff-ffff-ffffffffffff%' AS has_id_range
FROM (SELECT arrayStringConcat(groupArray(explain), '\n') AS plan FROM (EXPLAIN indexes = 1 SELECT sum(length(time_series)) FROM timeSeriesSelector(ts_clustered, '{__name__=~"foo|bar"}', 0, 1000)));
...
SELECT plan LIKE '%ffffffff-ffff-ffff-ffff-ffffffffffff%' AS has_id_range
FROM (SELECT arrayStringConcat(groupArray(explain), '\n') AS plan FROM (EXPLAIN indexes = 1 SELECT sum(length(time_series)) FROM timeSeriesSelector(ts_plain, 'foo', 0, 1000)));
...
SELECT plan LIKE '%ffffffff-ffff-ffff-ffff-ffffffffffff%' AS has_id_range
FROM (SELECT arrayStringConcat(groupArray(explain), '\n') AS plan FROM (EXPLAIN indexes = 1 SELECT sum(length(time_series)) FROM timeSeriesSelector(ts_custom_gen, 'foo', 0, 1000)));
...
SELECT plan LIKE '%ffffffff-ffff-ffff-ffff-ffffffffffff%' AS has_id_range
FROM (SELECT arrayStringConcat(groupArray(explain), '\n') AS plan FROM (EXPLAIN indexes = 1 SELECT sum(length(time_series)) FROM timeSeriesSelector(ts_altered_gen, 'foo', 0, 1000)));
...
FROM (SELECT arrayStringConcat(groupArray(explain), '\n') AS plan FROM (EXPLAIN indexes = 1 SELECT sum(length(time_series)) FROM timeSeriesSelector(ts_u64, 'foo', 0, 1000)));
...
SELECT sum(arraySum(x -> x.2, time_series)), sum(length(time_series)) FROM timeSeriesSelector(ts_prune, 'foo', 0, 1000);
...
FROM (EXPLAIN indexes = 1 SELECT sum(length(time_series)) FROM timeSeriesSelector(ts_prune, 'foo', 0, 1000));
