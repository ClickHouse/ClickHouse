...
CREATE TABLE alp32 (i UInt32 CODEC(NONE), f Float32 CODEC(ALP(AUTO))) Engine = MergeTree ORDER BY i;

DROP TABLE IF EXISTS alp64;
CREATE TABLE alp64 (i UInt32 CODEC(NONE), f Float64 CODEC(ALP(AUTO))) Engine = MergeTree ORDER BY i;


SELECT '# Rational Numbers Test';
TRUNCATE TABLE base64; INSERT INTO base64 SELECT number, round(number * 0.1 + cos(number), 3) FROM numbers(3000);
...
SELECT '# Irrational Numbers Test';
TRUNCATE TABLE base64; INSERT INTO base64 SELECT number, number * pi() + sin(number) FROM numbers(3000);
TRUNCATE TABLE alp64; INSERT INTO alp64 SELECT i, f FROM base64;
TRUNCATE TABLE base32; INSERT INTO base32 SELECT i, toFloat32(f) FROM base64;
TRUNCATE TABLE alp32; INSERT INTO alp32 SELECT i, f FROM base32;
SELECT count(), sum(bin(a.f) <> bin(b.f)) FROM base64 AS b INNER JOIN alp64 AS a USING i;
