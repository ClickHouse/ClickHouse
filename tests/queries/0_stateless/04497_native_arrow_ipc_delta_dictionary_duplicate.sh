#!/usr/bin/env bash
# Tags: no-fasttest
# A delta dictionary batch appends to the base dictionary; here the delta value is patched from 'CCC'
# to 'AAA', so the merged dictionary ['AAA', 'BBB', 'AAA'] repeats a base value. Arrow allows duplicate
# dictionary values: the read must succeed, rows referencing either copy decode to the same value.
...
# Confirm a delta was actually emitted: the base dictionary bytes ('AAABBB') must appear exactly once;
# a replacement (non-delta) batch would rewrite the whole dictionary and repeat them.
...
echo "--- delta dictionary that duplicates a base value reads, duplicates deduplicated ---"
${CLICKHOUSE_LOCAL} \
    --query "SELECT d FROM file('${DATA_FILE}', 'ArrowStream') SETTINGS max_threads = 1"
