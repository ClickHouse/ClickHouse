...
CREATE TABLE t_sharded_map_3 (id UInt64, m Map(String, UInt64, 4))
ENGINE = MergeTree ORDER BY id
SETTINGS min_bytes_for_wide_part = 0;

INSERT INTO t_sharded_map_3 SELECT number, (arrayMap(x -> 'c_' || x::String, range(number % 10)), range(number % 10)) FROM numbers(10);
INSERT INTO t_sharded_map_3 SELECT number, (arrayMap(x -> 'c_' || x::String, range(number % 10)), range(number % 10)) FROM numbers(10);
...
SELECT arraySort(m::Array(Tuple(String, UInt64)))::Map(String, UInt64) AS m FROM t_sharded_map_3 ORDER BY id;
