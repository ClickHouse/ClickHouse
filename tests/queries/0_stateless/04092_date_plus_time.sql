...
-- Date + Time uses the Time column's internal seconds, so two distinct Time values
-- produce distinct DateTime results (and equal Times produce equal results). This does
-- not rely on any out-of-range value.
DROP TABLE IF EXISTS test_internal_time;
CREATE TABLE test_internal_time (t_a Time, t_b Time) ENGINE = Memory;
INSERT INTO test_internal_time VALUES (3600, 7200);
SELECT t_a, t_b, t_a = t_b AS same_time FROM test_internal_time;
SELECT
    toDate('2024-01-15') + t_a AS dt_a,
    toDate('2024-01-15') + t_b AS dt_b,
    dt_a = dt_b AS same_dt
FROM test_internal_time;
DROP TABLE test_internal_time;

-- A numeric VALUES expression into a Time column now respects date_time_overflow_behavior
-- (same rule as toTime/CAST), clamping the out-of-visible-range value instead of storing it raw.
-- The expression reaches convertFieldToType only with expression templates disabled: the default
-- input_format_values_deduce_templates_of_expressions = 1 routes the row through a template that
-- ends in castColumn, i.e. CAST itself, so the assertion would not exercise the coercion at all.
DROP TABLE IF EXISTS test_values_expr_time;
CREATE TABLE test_values_expr_time (x Time) ENGINE = Memory;
SET input_format_values_deduce_templates_of_expressions = 0;
INSERT INTO test_values_expr_time VALUES (9999999 + 0);
SET input_format_values_deduce_templates_of_expressions = 1;
SELECT toInt32(x) FROM test_values_expr_time;
DROP TABLE test_values_expr_time;

SET date_time_overflow_behavior = 'throw';

-- A numeric VALUES expression into a Time column raises in throw mode, like toTime/CAST (again with
-- templates disabled, for the same reason as above).
-- The overflow may surface as a client or a server error depending on async_insert, so the
-- error hint below accepts either kind rather than requiring a server error.
DROP TABLE IF EXISTS test_values_expr_time;
CREATE TABLE test_values_expr_time (x Time) ENGINE = Memory;
SET input_format_values_deduce_templates_of_expressions = 0;
INSERT INTO test_values_expr_time VALUES (9999999 + 0); -- { error VALUE_IS_OUT_OF_RANGE_OF_DATA_TYPE }
SET input_format_values_deduce_templates_of_expressions = 1;
DROP TABLE test_values_expr_time;
