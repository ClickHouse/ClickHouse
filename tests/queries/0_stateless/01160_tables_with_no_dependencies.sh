...
$CLICKHOUSE_CLIENT -q "CREATE TABLE t0 (c0 Int STATISTICS(MinMax((1 AS x) IN (*)))) ENGINE = Memory" --allow_statistics 1
$CLICKHOUSE_CLIENT -q "CREATE TABLE t1 (c0 Int) ENGINE = Distributed('default', 'd0', 't0', nonexistent_column)" 2>&1 | grep -Fa "UNKNOWN_IDENTIFIER" >/dev/null && echo "OK"
