#!/usr/bin/env bash
# Tags: long, no-fasttest, no-object-storage, no-flaky-check
...
RUN_ONLY=""
#RUN_ONLY="Test case 0: insert_method=InsertValues engine=MergeTree use_insert_token=True single_thread=True deduplicate_src_table=True deduplicate_dst_table=True insert_unique_blocks=True"
            ...
            for variant in "match" "invert"; do
                ...
                echo "$THIS_RUN"
                        ...
                        if $CLICKHOUSE_CLIENT --max_insert_block_size 1 -mq "
                            $(python3 "$CURDIR"/03008_deduplication.python insert_several_blocks_into_table "${CASE_ARGS[@]}")
                        " 2> "$CASE_STDERR"; then
                            cat "$CASE_STDERR" >&2
                            echo OK
                        else
                            ...
                            if [ -n "$FAILED_PHASE" ]; then
                                $CLICKHOUSE_CLIENT --max_insert_block_size 1 -mq "
                                    $(python3 "$CURDIR"/03008_deduplication.python insert_several_blocks_into_table "${CASE_ARGS[@]}" --emit-debug-only "$FAILED_PHASE")
                                "
                            fi
                            echo FAIL
                        fi
