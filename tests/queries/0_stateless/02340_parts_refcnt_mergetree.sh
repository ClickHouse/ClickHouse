...
function check_refcnt_for_table()
{
    ...
    # simply filter parts not by "refcount > 1" but with some delta - "4", to
    # avoid flakiness.
    $CLICKHOUSE_CLIENT -q "select table, name, refcount>=6 from system.parts where database = '$CLICKHOUSE_DATABASE' and table = '$table' and refcount >= 4"
