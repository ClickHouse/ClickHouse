...
-- A physical column can still use its old source-part name while a later DROP/CLEAR
-- command uses the current name. The mutation must resolve the physical name too.
DROP TABLE IF EXISTS t_skip_empty_physical_rename_drop;

CREATE TABLE t_skip_empty_physical_rename_drop
(
    key UInt64,
    a UInt64,
    force UInt8
...
INSERT INTO t_skip_empty_physical_rename_drop VALUES (1, 123, 0);
SYSTEM STOP MERGES t_skip_empty_physical_rename_drop;
ALTER TABLE t_skip_empty_physical_rename_drop RENAME COLUMN a TO b, DROP COLUMN b, ADD COLUMN b UInt64 DEFAULT 999;

SELECT 'physical_rename_drop_pending';
SELECT key, b FROM t_skip_empty_physical_rename_drop ORDER BY key;

SYSTEM START MERGES t_skip_empty_physical_rename_drop;
SET mutations_sync = 2;
ALTER TABLE t_skip_empty_physical_rename_drop UPDATE force = force WHERE 1;

SELECT 'physical_rename_drop_materialized';
SELECT key, b FROM t_skip_empty_physical_rename_drop ORDER BY key;
...
CREATE TABLE t_skip_empty_unshared_dotted
(
    key UInt64,
    n UInt8,
    `n.a` UInt64,
...
INSERT INTO t_skip_empty_unshared_dotted VALUES (1, 1, 0, 0);
SYSTEM STOP MERGES t_skip_empty_unshared_dotted;
ALTER TABLE t_skip_empty_unshared_dotted DROP COLUMN n, MODIFY COLUMN `n.a` UInt64 DEFAULT 999;

SELECT 'unshared_dotted_pending';
SELECT key, `n.a` FROM t_skip_empty_unshared_dotted ORDER BY key;

SYSTEM START MERGES t_skip_empty_unshared_dotted;
SET mutations_sync = 2;
ALTER TABLE t_skip_empty_unshared_dotted UPDATE force = force WHERE 1;

SELECT 'unshared_dotted_materialized';
SELECT key, `n.a` FROM t_skip_empty_unshared_dotted ORDER BY key;

DROP TABLE t_skip_empty_unshared_dotted;
