...
# Tag no-parallel: Isolate the 64 replicated materialized-view cases from concurrent test load.
                ...
                for deduplicate_dst_table in "True" "False"; do
                    for insert_unique_blocks in "True" "False"; do
                        ...
                        if $CLICKHOUSE_CLIENT --max_insert_block_size 1 -mq "
                            $(python3 "$CURDIR"/03008_deduplication.python several_mv_into_one_table "${CASE_ARGS[@]}")
                        " 2> "$CASE_STDERR"; then
                            cat "$CASE_STDERR" >&2
                            echo OK
                        else
