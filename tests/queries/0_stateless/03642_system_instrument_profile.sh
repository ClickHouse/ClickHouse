...
$CLICKHOUSE_CLIENT -q "
    ...
    SELECT entry_type, duration_nanoseconds FROM system.trace_log WHERE event_date >= yesterday() AND event_time >= now() - 600 AND query_id = '$query_id' AND trace_type = 'Instrumentation' AND handler = 'profile' AND entry_type = 'Entry' AND function_name LIKE '%QueryMetricLog::startQuery%' AND arrayExists(x -> x LIKE '%dispatchHandler%', symbols);
...
$CLICKHOUSE_CLIENT -q "
    ...
    SELECT countIf(entry_type == 'Entry') > 5, countIf(entry_type == 'Exit') > 5 FROM system.trace_log WHERE event_date >= yesterday() AND event_time >= now() - 600 AND query_id = '$query_id' AND trace_type = 'Instrumentation' AND handler = 'profile' AND function_name LIKE '%recursiveRemoveLowCardinality%' AND arrayExists(x -> x LIKE '%dispatchHandler%', symbols);
"
