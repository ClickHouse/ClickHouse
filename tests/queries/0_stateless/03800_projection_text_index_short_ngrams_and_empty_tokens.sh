...
SETTINGS="--enable_full_text_index=1 --allow_experimental_projection_text_index=1 "
...
(
    ...
    PROJECTION array_idx INDEX arr TYPE text(tokenizer = ngrams(4)),
    PROJECTION map_keys_idx INDEX mapKeys(map) TYPE text(tokenizer = ngrams(4)),
)
ENGINE = MergeTree
ORDER BY (id);
