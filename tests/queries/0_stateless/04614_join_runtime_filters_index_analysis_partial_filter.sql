...
-- Pruning must actually drop granules (otherwise the correctness check above is vacuous). The dim
-- has 4000 distinct keys, below join_runtime_filter_exact_values_limit (10000), so the filter keeps
-- its exact set and pruning goes through getRecordedKeyValues rather than overflowing to a bloom
-- filter / range fallback.
