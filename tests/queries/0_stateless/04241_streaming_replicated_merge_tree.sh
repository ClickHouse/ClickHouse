...
read -r fifo_0 pid_0 < <(spawn $STREAMING_CLIENT -q "SELECT a FROM t_streaming_test_r1 STREAM WHERE p = 0")
read -r fifo_1 pid_1 < <(spawn $STREAMING_CLIENT -q "SELECT a FROM t_streaming_test_r2 STREAM WHERE p = 1")
