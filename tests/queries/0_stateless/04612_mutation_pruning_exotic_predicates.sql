...
CREATE TABLE t_mutation_pruning_exotic (d Date, x UInt32)
ENGINE = MergeTree PARTITION BY toYYYYMM(d) ORDER BY x;

INSERT INTO t_mutation_pruning_exotic VALUES ('2024-01-01', 1);
INSERT INTO t_mutation_pruning_exotic VALUES ('2024-02-01', 2);
...
ALTER TABLE t_mutation_pruning_exotic UPDATE x = 10 WHERE _part LIKE '202402%';
SELECT * FROM t_mutation_pruning_exotic ORDER BY d;

SELECT 'subquery in predicate';
ALTER TABLE t_mutation_pruning_exotic UPDATE x = x + 1 WHERE x IN (SELECT number + 10 FROM numbers(1));
SELECT * FROM t_mutation_pruning_exotic ORDER BY d;
...
CREATE TABLE t_mutation_pruning_exotic_r (d Date, x UInt32)
ENGINE = ReplicatedMergeTree('/clickhouse/tables/{database}/t_mutation_pruning_exotic_r', 'r1')
PARTITION BY toYYYYMM(d) ORDER BY x;

INSERT INTO t_mutation_pruning_exotic_r VALUES ('2024-01-01', 1);
INSERT INTO t_mutation_pruning_exotic_r VALUES ('2024-02-01', 2);
