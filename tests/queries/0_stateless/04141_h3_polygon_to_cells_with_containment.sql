...
-- (it has no bounding-box containment mode), so it is rejected explicitly.
WITH
    [(-122.4089866999972145, 37.813318999983238), (-122.3544736999993603, 37.7198061999978478), (-122.4798767000009008, 37.8151571999998453)] AS ring,
    7 AS resolution
SELECT h3PolygonToCellsWithContainment(ring, resolution, 3); -- { serverError ARGUMENT_OUT_OF_BOUND }
...
SELECT
    ...
    AND arraySort(arrayMap(x -> h3ToString(x), h3PolygonToCellsWithContainment(ring, resolution, 0)))
        != arraySort(arrayMap(x -> h3ToString(x), h3PolygonToCellsWithContainment(ring, resolution, 2)));
