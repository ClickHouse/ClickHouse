...
# Every timing case bounds how long an `isProbablePrime` call runs past the deadline it was given. Each
# keeps its prologue - reading rows, building a block - small and puts the cost inside the function, since
# the prologue is interruptible on its own and would bound the query even unfixed.
DEADLINE_MS=2000
SCALE=1
[ -n "$(${CLICKHOUSE_CLIENT} --query "SELECT value FROM system.build_options WHERE name = 'CXX_FLAGS' AND value LIKE '%sanitize=%'")" ] && SCALE=2
BOUND=$((SCALE * 8000))
# What a bound allows on top of its deadline: a case with a deadline of its own keeps this allowance
# rather than the bound, so shortening a deadline does not loosen the assertion with it.
...
# $1 = label, $2 = query, $3 = overflow mode (default "throw"), $4 = deadline in ms (default `DEADLINE_MS`).
# Two messages report the same enforced deadline and only one carries a number: `CancellationChecker` can
# win the race against the in-function check, and its error has no elapsed part. That is inconclusive, not
# a pass - accepting it on wall time would assert query-level cancellation instead of the checkpoint.
run() {
    ...
    if [ "$mode" = break ]; then
        # Under `break` the client sees no error and no elapsed time, so the cause must be read as
        # `ProfileEvents['OverflowBreak']`: wall time alone scores a fast unrelated failure as a pass. The
        # row count is read too, since `max` over no rows returns 0, which is below the bound.
...
# 1. The shape the CI report showed: one block of UInt256 rows each running the full rounds.
run "uint256 rounds" \
...
# 3. The UInt128 carrier, which reaches the same rounds through its own type instantiation: the checkpoint
#    is not tied to one width.
run "uint128 rounds" \
    "SELECT sum(isProbablePrime(toUInt128(materialize('$P128')), 64)) FROM numbers(100000) SETTINGS max_block_size = 100000"

# 4. Rows that never reach the rounds, which the callback alone would never bound: a wide value fitting in
#    UInt64 returns through the first cheap exit, so those rows are charged through the budget instead.
#    The count must keep one block above the deadline yet stay under the 20M `max_rows_to_read` of the CI
#    `default` profile (tests/config/users.d/limits.yaml), or the block is never reached. `materialize`
#    wraps the finished UInt256, so the per-row prologue is a copy - the fixed side, hence a scaled deadline.
...
#    through a separate branch of the same check. No deadline is set, so only the kill can stop the query,
#    and what is asserted is how long the synchronous kill waits - `KILL ... SYNC` polls until the target
#    leaves the process list. The count is sized against the FASTEST build: the bound scales with the
#    sanitizer while a row is cheaper there, so the margin is tightest where the bound is loosest.
...
# Waiting until the query is RUNNING rather than merely visible: a kill landing before the executor is
# attached, or inside the prologue, is served by the generic path and would return promptly even unfixed.
kill_seen=0
...
# regression must fail with a readable verdict instead of hanging. It stays below the runner's own alarm
# at `int(timeout * 1.1) + 60`, 720 s by default, which would otherwise fire first.
...
#    exits and the narrow path must still answer correctly over a full block, so a "fix" that threw
#    unconditionally or skipped rows fails here. Their cost is a performance property and a CI-safe
#    wall-clock ceiling could not redden on it, so the timings live in the PR body instead.
...
# 7. Results are unchanged by the checkpoint, across both cheap exits, the rounds, and the documented
#    examples. The checkpoint sits between the exits and the rounds, so a misplacement shows up here as a
#    changed answer rather than as a timing difference.
