...
DROP TABLE IF EXISTS t_cuckoo_alter_bypass_type;
...
CREATE TABLE t_cuckoo_alter_bypass_type (`k` UInt64, INDEX idx k TYPE cuckoo_filter(0.01) GRANULARITY 1) ENGINE = MergeTree ORDER BY k SETTINGS index_granularity = 8192;
SET allow_experimental_cuckoo_filter_index = 0;
...
ALTER TABLE t_cuckoo_alter_bypass_type MODIFY COLUMN k UInt32; -- { serverError SUPPORT_IS_DISABLED }
DROP TABLE IF EXISTS t_cuckoo_alter_bypass_fpr;
DROP TABLE IF EXISTS t_cuckoo_alter_bypass_expr;
DROP TABLE IF EXISTS t_cuckoo_alter_bypass_gran;
DROP TABLE IF EXISTS t_cuckoo_alter_bypass_type;
