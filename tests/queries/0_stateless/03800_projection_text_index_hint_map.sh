...
MY_CLICKHOUSE_CLIENT="${CLICKHOUSE_CLIENT} --explain_query_plan_default legacy --enable_full_text_index 1 --allow_experimental_projection_text_index 1 --use_skip_indexes 1 --use_skip_indexes_on_data_read 1 --query_plan_text_index_add_hint 1 --use_query_condition_cache 0 --enable_analyzer 1 --query_plan_direct_read_from_text_index 1 --optimize_functions_to_subcolumns 1 --optimize_move_to_prewhere 1 --query_plan_optimize_prewhere 1"
...
$MY_CLICKHOUSE_CLIENT --query "
    ...
    (
        m Map(String, String),
        PROJECTION idx_mk INDEX mapKeys(m) TYPE text(tokenizer = array),
        PROJECTION idx_mv INDEX mapValues(m) TYPE text(tokenizer = array)
...
function run()
{
    ...
    $MY_CLICKHOUSE_CLIENT --query "
        ...
        (
            EXPLAIN actions = 1, indexes = 1 $query SETTINGS use_skip_indexes_on_data_read = 1, optimize_functions_to_subcolumns = 1
        )
