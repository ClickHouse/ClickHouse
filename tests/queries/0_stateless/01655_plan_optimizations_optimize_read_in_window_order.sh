...
$CLICKHOUSE_CLIENT --query_plan_read_in_order=0 -q "explain plan actions=1, description=1 select n, sum(x) OVER (ORDER BY n, x ROWS BETWEEN 100 PRECEDING AND CURRENT ROW) from ${name}_n SETTINGS optimize_read_in_window_order=0" | grep -i "sort description"
...
echo '  optimize_read_in_window_order=0'
$CLICKHOUSE_CLIENT --query_plan_read_in_order=0 -q "explain plan actions=1, description=1 select n, sum(x) OVER (ORDER BY n, x ROWS BETWEEN 100 PRECEDING AND CURRENT ROW) from ${name}_n_x SETTINGS optimize_read_in_window_order=0" | grep -i "sort description"
...
echo '  optimize_read_in_window_order=0'
$CLICKHOUSE_CLIENT --query_plan_read_in_order=0 -q "SELECT ten, sum(unique1) + sum(unique2) AS res, rank() OVER (ORDER BY sum(unique1) + sum(unique2) ASC) AS rank FROM ${name}_complex GROUP BY ten ORDER BY ten ASC SETTINGS optimize_read_in_window_order=0"
echo '  optimize_read_in_window_order=1'
