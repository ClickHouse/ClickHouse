...
$CLICKHOUSE_CLIENT -q "drop table if exists ${name}_complex"
...
$CLICKHOUSE_CLIENT -q "explain pipeline select n, sum(x) OVER (ORDER BY n, x ROWS BETWEEN 100 PRECEDING AND CURRENT ROW) from ${name}_n SETTINGS optimize_read_in_window_order=0, max_threads=1" | grep -i "MergeTreeThread" | eval $TRIM_LEADING_SPACES
...
$CLICKHOUSE_CLIENT -q "explain pipeline select n, sum(x) OVER (ORDER BY n, x ROWS BETWEEN 100 PRECEDING AND CURRENT ROW) from ${name}_n SETTINGS optimize_read_in_window_order=1, max_threads=1" | grep -i "MergeTreeInOrder" | eval $TRIM_LEADING_SPACES
