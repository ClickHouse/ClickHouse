...
(
    ...
    PROJECTION i0 INDEX c0.c1 TYPE text(tokenizer = splitByString)
)
ENGINE = SummingMergeTree() ORDER BY (id) SETTINGS deduplicate_merge_projection_mode = 'rebuild';
...
(
    id UInt64,
    c1 JSON(s1 String),
    PROJECTION i0 INDEX c1.s1 TYPE text(tokenizer = splitByString),
    PROJECTION i1 INDEX coalesce(c1.s2, '')::String TYPE text(tokenizer = splitByString),
)
ENGINE = SummingMergeTree() ORDER BY (id) SETTINGS deduplicate_merge_projection_mode = 'rebuild';
