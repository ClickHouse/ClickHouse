...
SELECT count() FROM t_json_all_values_array_prefix_suffix WHERE hasAny(data.tags, ['a']) SETTINGS use_skip_indexes = 1;
SELECT count() FROM t_json_all_values_array_prefix_suffix WHERE hasAny(data.tags, ['a']) SETTINGS use_skip_indexes = 0;
SELECT count() FROM t_json_all_values_array_prefix_suffix WHERE hasAll(data.tags, ['a']) SETTINGS use_skip_indexes = 1;
SELECT count() FROM t_json_all_values_array_prefix_suffix WHERE hasAll(data.tags, ['a']) SETTINGS use_skip_indexes = 0;
