...
CREATE WINDOW VIEW ${db}.wv TO ${db}.dst AS SELECT count(a) AS count FROM ${db}.mt GROUP BY hop(now('US/Samoa'), INTERVAL '10' SECOND, INTERVAL '10' SECOND, 'US/Samoa') AS wid;
