...
${CLICKHOUSE_CURL} -sS -H "Accept-Encoding: compress" "${BASE_URL}/play" | grep -o -F -m1 '<title>ClickHouse Query</title>'

echo "--- test 6: body is valid uncompressed HTML (identity) ---"
${CLICKHOUSE_CURL} -sS -H "Accept-Encoding: identity" "${BASE_URL}/play" | grep -o -F -m1 '<title>ClickHouse Query</title>'
