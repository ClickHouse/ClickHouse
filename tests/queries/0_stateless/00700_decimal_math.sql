...
SELECT toDecimal32('1.0', 5) AS x, round(erf(x), 15), round(erfc(x), 15); -- round: erf/erfc differ in the last ULP across libm implementations (glibc vs macOS)
...
SELECT toDecimal64('1.0', 5) AS x, round(erf(x), 15), round(erfc(x), 15); -- round: erf/erfc differ in the last ULP across libm implementations (glibc vs macOS)
...
SELECT toDecimal128('1.0', 5) AS x, round(erf(x), 15), round(erfc(x), 15); -- round: erf/erfc differ in the last ULP across libm implementations (glibc vs macOS)
