...
# Disable force_primary_key_reverse_order: tests async insert with materialized views, creates MergeTree internally
...
# A loaded runner can exceed the default 10 s handshake budget when the views push to the remote target.
${CLICKHOUSE_CLIENT} --handshake_timeout_ms 60000 --async_insert_use_adaptive_busy_timeout 0 --async_insert_busy_timeout_ms 1 --multiquery <<EOF
