...
DROP TABLE IF EXISTS t_04300;
CREATE TABLE t_04300 (id UInt64, v UInt64) ENGINE = MergeTree ORDER BY id;
INSERT INTO t_04300 VALUES (1, 10), (2, 20);

-- Minimal reproducer: EXISTS over an unaliased real-table subquery.
ALTER TABLE t_04300 UPDATE v = 0 WHERE exists((SELECT * FROM system.one)) SETTINGS mutations_sync = 2;
SELECT id, v FROM t_04300 ORDER BY id;
...
DROP TABLE IF EXISTS test_updates_04300;
CREATE TABLE test_updates_04300 (id UInt64, dynamic Dynamic)
    ENGINE = MergeTree ORDER BY id;
INSERT INTO test_updates_04300 VALUES (1, 'a'), (2, 'b');
ALTER TABLE test_updates_04300
    (UPDATE dynamic = NULL WHERE exists((SELECT DISTINCT *, currentDatabase() > database)))
    SETTINGS mutations_sync = 2;
SELECT id, isNull(dynamic) FROM test_updates_04300 ORDER BY id;

-- A NOT EXISTS variant, which the analyzer rewrites to `not exists((subq))`
-- and then expands the same way.
DROP TABLE IF EXISTS t_04300_b;
CREATE TABLE t_04300_b (id UInt64, v UInt64) ENGINE = MergeTree ORDER BY id;
INSERT INTO t_04300_b VALUES (1, 10), (2, 20);
ALTER TABLE t_04300_b UPDATE v = 1
    WHERE NOT exists((SELECT * FROM system.numbers WHERE number > 1000000 LIMIT 1))
    SETTINGS mutations_sync = 2;
SELECT id, v FROM t_04300_b ORDER BY id;

DROP TABLE t_04300;
DROP TABLE test_updates_04300;
DROP TABLE t_04300_b;
