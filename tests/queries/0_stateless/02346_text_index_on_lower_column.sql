...
(
    EXPLAIN actions = 1, indexes = 1 SELECT count() FROM tab WHERE hasToken(text, 'Hello')
...
SELECT trim(explain) FROM
(
    EXPLAIN actions = 1, indexes = 1 SELECT count() FROM tab WHERE hasToken(lower(text), lower('Hello'))
) WHERE explain LIKE '%Filter column%' OR explain LIKE '%Name: idx_text%';
...
SELECT trim(explain) FROM
(
    EXPLAIN actions = 1, indexes = 1 SELECT count() FROM tab WHERE hasAllTokens(text, ['Hello'])
) WHERE explain LIKE '%Filter column%' OR explain LIKE '%Name: idx_text%';
...
SELECT trim(explain) FROM
(
    EXPLAIN actions = 1, indexes = 1 SELECT count() FROM tab WHERE hasAllTokens(lower(text), [lower('Hello')])
) WHERE explain LIKE '%Filter column%' OR explain LIKE '%Name: idx_text%';
