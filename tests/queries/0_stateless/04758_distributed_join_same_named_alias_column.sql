...
-- A distributed JOIN where both tables define an ALIAS column with the SAME name, referenced from an
-- expression clause (JOIN ON / WHERE / GROUP BY / ORDER BY), used to fail with
-- MULTIPLE_EXPRESSIONS_FOR_ALIAS: inlining each ALIAS column re-applied the bare column name as an
-- alias, so two different bodies ended up sharing one alias in the same scope. The inlined body must
-- keep the alias only as a top-level projection output, not inside expression clauses.
...
-- The shape from the original report: a GLOBAL JOIN wrapping a subquery that references both.
