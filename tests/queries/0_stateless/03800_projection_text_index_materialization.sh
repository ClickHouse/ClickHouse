...
{
    $CLICKHOUSE_CLIENT -q "
        SELECT sum(data_compressed_bytes) > 0
    ...
    $CLICKHOUSE_CLIENT --enable_analyzer 1  -q "
        SELECT
            CASE
                WHEN trim(explain) ILIKE '%__text_index_%' THEN 'text_index_used'
                ELSE 'text_index_not_used'
            END
        FROM
        (
...
$CLICKHOUSE_CLIENT -q "
    ...
    ALTER TABLE t_text_index_materialization ADD PROJECTION idx_text INDEX text TYPE text(tokenizer = ngrams(3));
...
$CLICKHOUSE_CLIENT --mutations_sync 2 --enable_full_text_index 1 -q "ALTER TABLE t_text_index_materialization CLEAR PROJECTION idx_text"
echo "After CLEAR INDEX idx_text"
run_test

$CLICKHOUSE_CLIENT --mutations_sync 2 -q "ALTER TABLE t_text_index_materialization MATERIALIZE PROJECTION idx_text"
