...
$CLICKHOUSE_CLIENT --query_plan_merge_expressions=1 --query "EXPLAIN actions=1 SELECT value FROM rocksdb_with_filter LIMIT 1" | grep -A 2 "ReadFromEmbeddedRocksDB"
...
$CLICKHOUSE_CLIENT --query_plan_merge_expressions=1 --query "EXPLAIN actions=1 SELECT value FROM rocksdb_with_filter WHERE key = '5000'" | grep -A 3 "ReadFromEmbeddedRocksDB"
