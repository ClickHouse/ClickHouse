...
# A column qualified with the whole hierarchical name of its table is resolved by the analyzer only.
run "SELECT sales.orders.id, ${db}.sales.orders.x FROM sales.orders ORDER BY id SETTINGS enable_analyzer = 1"
...
run "SELECT ${db}.sub.t.id FROM ${db}.sub.t SETTINGS enable_analyzer = 1"
