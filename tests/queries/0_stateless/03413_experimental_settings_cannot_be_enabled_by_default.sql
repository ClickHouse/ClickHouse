...
-- TEMPORARY EXPERIMENT (revert before merge): 'allow_experimental_adaptive_codec_selection' is turned on
-- server-wide via config.d/merge_tree_settings.xml to exercise adaptive codec selection across CI.
SELECT name, value FROM system.merge_tree_settings WHERE tier = 'Experimental' AND type = 'Bool' AND value != '0' AND name NOT IN ('remove_rolled_back_parts_immediately', 'allow_experimental_adaptive_codec_selection');
