...
SELECT name, value FROM system.settings WHERE tier = 'Experimental' AND type = 'Bool' AND value != '0' AND name NOT IN ('throw_on_unsupported_query_inside_transaction', 'enable_reads_from_columns_cache', 'enable_writes_to_columns_cache', 'ai_function_throw_on_error', 'ai_function_throw_on_quota_exceeded');
