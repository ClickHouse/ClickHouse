...
# `http_allow_table_as_file`, `http_allow_filters_as_path` and
# `http_allow_filters_as_unrecognized_url_parameters`. None of the requests below pass any of
# them, so each one only works if the default is on.
...
echo "-- filters as unrecognized URL parameters"
${CLICKHOUSE_CURL} -sS "${BASE_URL}/${DB}/hits.CSV?b=three"
