...
${CLICKHOUSE_CLIENT} --query "DROP NAMED COLLECTION nc_04907" 2>&1 | grep -o -F "NAMED_COLLECTION_IS_USED"
...
${CLICKHOUSE_CLIENT} --query "CREATE TABLE remote_invalid_04907 (n UInt64) ENGINE = Remote('127.0.0.1:1', dictionary())" 2>&1 | grep -o -F "Table function ('dictionary') requires 1 arguments"
${CLICKHOUSE_CLIENT} --query "CREATE TABLE distributed_invalid_04907 (n UInt64) ENGINE = Distributed(test_cluster_multiple_nodes_all_unavailable, merge(1))" 2>&1 | grep -o -F "Argument 'table_name_regexp' must be a literal with type String, got UInt64"
