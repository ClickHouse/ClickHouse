...
-- used to cause BAD_TYPE_OF_FIELD because Parquet String statistics cannot be compared with Object fields.
-- After the fix, the Parquet filter pushdown is skipped for Dynamic columns, but the query still
-- fails with NO_COMMON_TYPE because String and JSON have no common supertype at execution time.
