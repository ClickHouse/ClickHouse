...
# Parser-affecting settings from a JSON `SET` (`dialect`, `enable_json_ast_dialect`) must NOT
...
${CLICKHOUSE_CLIENT} --enable_json_ast_dialect 1 --dialect clickhouse_json -q "$SET_DIALECT" 2>&1 && echo 'set_dialect_ok'

# 2. JSON `SET enable_json_ast_dialect = 0`: the experimental gate must stay on for the body
#    that carries it (the server enforces the gate before parsing JSON).
SET_GATE=$(${CLICKHOUSE_CLIENT} -q "SELECT parseQueryToJSON('SET enable_json_ast_dialect = 0') FORMAT TSVRaw")
${CLICKHOUSE_CLIENT} --enable_json_ast_dialect 1 --dialect clickhouse_json -q "$SET_GATE" 2>&1 && echo 'set_gate_ok'
...
${CLICKHOUSE_CLIENT} --enable_json_ast_dialect 1 --dialect clickhouse_json -q "$SELECT_JSON"
