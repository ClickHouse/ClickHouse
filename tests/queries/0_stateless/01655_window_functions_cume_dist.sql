...
-- With multiple partitions and ties
TRUNCATE TABLE test_cume_dist;
...
-- With NULL values in ordering column
TRUNCATE TABLE test_cume_dist;
INSERT INTO test_cume_dist VALUES (1, 10), (2, 20), (3, NULL), (4, 30), (5, NULL);
SELECT a, b, cume_dist() OVER (ORDER BY b) FROM test_cume_dist ORDER BY b, a;
SELECT a, b, cume_dist() OVER (ORDER BY b NULLS FIRST) FROM test_cume_dist ORDER BY b, a;
SELECT a, b, cume_dist() OVER (ORDER BY b NULLS LAST) FROM test_cume_dist ORDER BY b, a;
...
-- Frame specification should produce an error
