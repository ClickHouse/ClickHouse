...
-- reported query (these steer the plan through the set-building paths).
...
-- resolves correlated; every reference must be rejected, otherwise a surviving
-- `PLACEHOLDER` (correlated-column marker) reaches `ExpressionActions::execute` and throws
-- the exception `Trying to execute PLACEHOLDER action`, a `LOGICAL_ERROR` that also aborts
-- debug and sanitizer builds. Here `a` exposes only `uid`, so `database` in `b` resolves
-- against the outer scope, making `b` correlated.
