...
-- The addresses are IP literals because attaching a Remote table resolves them, and an unresolvable
-- name is reported as an error in the server log.

DROP TABLE IF EXISTS target_04725;
CREATE TABLE target_04725 (x UInt64) ENGINE = MergeTree ORDER BY x;

-- The length lives in the user name; every '.' of an address becomes '%2E' in the directory name.
-- A name of exactly 255 bytes (user + '@' + escaped address + ':9000' = 233 + 1 + 16 + 1 + 4) is
-- still accepted.
CREATE TABLE at_limit_04725 (x UInt64) ENGINE = Remote('127.0.0.10:9000', currentDatabase(), target_04725, 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa');
...
CREATE TABLE over_limit_04725 (x UInt64) ENGINE = Remote('127.0.0.100:9000', currentDatabase(), target_04725, 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa');
...
CREATE TABLE multi_04725 (x UInt64) ENGINE = Remote('127.0.0.1:9000|127.0.0.100:9000', currentDatabase(), target_04725, 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa');
...
CREATE TABLE with_password_04725 (x UInt64) ENGINE = Remote('127.0.0.10:9000', currentDatabase(), target_04725, 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa', 'secret_04725');
