...
# An ALIAS column shipped as a GLOBAL JOIN temporary table must be computed once per row, and the
# same number of times on every dispatch path. A doubled count means it is computed both when the
# side is materialized and again when the outer column is read back from the temporary table.
