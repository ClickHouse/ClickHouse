...
SELECT toTypeName(multiIf(1, toInt64(1), 0.));
...
SELECT avg(multiIf(1, toInt64(1), 0.));
...
SELECT toTypeName(if(1, toInt32(1), toInt8(2)));
SELECT toTypeName(if(1, toUInt8(1), toInt16(2)));

-- Non-numeric branches still become a Variant even when the setting is on.
SELECT toTypeName(if(1, toInt64(1), 'str'));
