...
PROFILE="p_${CLICKHOUSE_DATABASE}"
PROFILE_OFF="p_off_${CLICKHOUSE_DATABASE}"

${CLICKHOUSE_CLIENT} <<EOF
CREATE SETTINGS PROFILE ${PROFILE} SETTINGS enable_global_with_statement = 1;
CREATE SETTINGS PROFILE ${PROFILE_OFF} SETTINGS enable_global_with_statement = 0;

CREATE TABLE src (id UInt64) ENGINE = MergeTree ORDER BY id;
INSERT INTO src VALUES (1);

CREATE VIEW v_profile_clause AS
    WITH src AS (SELECT 7 AS id) SELECT id FROM (SELECT id FROM src SETTINGS profile = '${PROFILE}');
...
CREATE VIEW v_profile_clause_off AS
    WITH src AS (SELECT 7 AS id) SELECT id FROM (SELECT id FROM src SETTINGS profile = '${PROFILE_OFF}');
...
DROP SETTINGS PROFILE ${PROFILE}, ${PROFILE_OFF};
