...
# wrapper `setBuilderOptionChecked` now validates the bytes and rejects the input
# with `BAD_ARGUMENTS` instead of letting the panic propagate, and the partially
# configured `EngineBuilder` is cleaned up (consumed via `ffi::builder_build`
# plus `ffi::free_engine`) so the rejection path does not leak.
#
