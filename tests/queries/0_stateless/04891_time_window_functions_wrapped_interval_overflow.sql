...
-- overflow guard, after which the window-searching loop of `hop` (and of `windowID`) wrapped around
-- zero and never terminated. With constant arguments the loop runs at analysis time, during
-- constant folding, where the query cannot even be killed.
...
SELECT windowID(toDateTime32('1969-12-31'), toIntervalDay(2147483648), toIntervalDay(2147483648), 'US/Samoa'); -- { serverError BAD_ARGUMENTS }
-- A sane hop is unaffected.
SELECT hop(toDateTime('2026-08-13 10:07:00', 'UTC'), toIntervalMinute(15), toIntervalMinute(60), 'UTC');
SELECT windowID(toDateTime('2026-08-13 10:07:00', 'UTC'), toIntervalMinute(15), toIntervalMinute(60), 'UTC');
