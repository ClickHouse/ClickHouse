...
-- independent arrayJoins keep the in-expression nesting order: the first one written is the outer loop
...
-- and pin the order itself, so both sides can't be wrong together
