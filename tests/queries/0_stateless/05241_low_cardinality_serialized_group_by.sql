...
INSERT INTO lc_serialized_group_by SELECT toString(number % 7), toFixedString(toString(number % 5), 4), if(number % 11 = 0, NULL, toString(number % 3)), number, number FROM numbers(3000);
INSERT INTO lc_serialized_group_by SELECT toString((number * 3 + 2) % 7), toFixedString(toString((number * 4 + 1) % 5), 4), if(number % 5 = 0, NULL, toString((number * 2 + 1) % 3)), number, number FROM numbers(3000);
