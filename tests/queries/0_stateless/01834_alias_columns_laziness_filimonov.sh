...
${CLICKHOUSE_CLIENT} --query_plan_optimize_lazy_materialization=0 --query_plan_remove_unused_columns=0 --profile-events-delay-ms=-1 --print-profile-events --query "SELECT x, y FROM aliases_lazyness WHERE x = 1 FORMAT Null" |& grep -o -e "SleepFunctionMicroseconds.*" -e "SleepFunctionCalls.*"
