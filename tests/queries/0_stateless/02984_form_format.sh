...
FORM_DATA=${USER_FILES_PATH:?}/data.tmp
touch $FORM_DATA    
...
$CLICKHOUSE_CLIENT --query="CREATE TABLE form_table ENGINE = MergeEngine() ORDER BY tuple() AS SELECT * FROM file('${FORM_DATA}','Form')" 