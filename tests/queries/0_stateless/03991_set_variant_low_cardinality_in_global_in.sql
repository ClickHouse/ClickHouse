...
-- No key type can equal the `Variant(LowCardinality(...), ...)` set element, so index analysis declines
-- the atom and the error now comes from the runtime membership cast. The same query over a table with
-- no usable key index already returned this code, so it no longer depends on the key.
