...
${CLICKHOUSE_CURL} -sS -D - "${CLICKHOUSE_URL}" --data-binary "SELECT 1" | grep "X-ClickHouse-Exception-Tag" | head -1 | cut -d':' -f1 | sed 's/$/:/g'
...
RESPONSE=$(${CLICKHOUSE_CURL} -sS -D - "${CLICKHOUSE_URL}" --data-binary "SELECT throwIf(number=3, 'there is a exception') FROM system.numbers SETTINGS max_block_size=1" 2>&1 || true)
...
RESPONSE=$(${CLICKHOUSE_CURL} -sS -D - "${CLICKHOUSE_URL}&wait_end_of_query=0" --data-binary "SELECT number FROM system.numbers LIMIT 100 SETTINGS max_result_rows=3, max_block_size=1" 2>&1 || true)
TAG=$(echo "$RESPONSE" | grep "X-ClickHouse-Exception-Tag" | cut -d':' -f2 | tr -d ' \r\n')
...
RESPONSE=$(${CLICKHOUSE_CURL} -sS -D - "${CLICKHOUSE_URL}&wait_end_of_query=0" --data-binary "SELECT number, throwIf(number=5, '${LONG_MSG}') FROM system.numbers LIMIT 10 SETTINGS max_block_size=1" 2>&1 || true)
