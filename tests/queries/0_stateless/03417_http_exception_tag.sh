...
RESPONSE=$(${CLICKHOUSE_CURL} -sS -D - "${CLICKHOUSE_URL}&wait_end_of_query=0&http_response_buffer_size=1" --data-binary "SELECT number, throwIf(number=10, 'there is a exception') FROM system.numbers SETTINGS max_block_size=1" 2>&1 || true)
...
RESPONSE=$(${CLICKHOUSE_CURL} -sS -D - "${CLICKHOUSE_URL}&wait_end_of_query=0&http_response_buffer_size=1" --data-binary "SELECT number, throwIf(number=10, '${LONG_MSG}') FROM system.numbers SETTINGS max_block_size=1" 2>&1 || true)
