...
# `prepareMutationEntry` adds the mutation to the transaction before `startMutation`
# registers the entry in `current_mutations_by_version`. A rollback in that window
# (e.g. KILL TRANSACTION) called `killMutation` before the entry existed, found nothing
# to remove, and the entry registered afterwards was orphaned: its transaction was gone
# and its CSN could never be assigned. Background jobs then raised a logical error
# ("Cannot find transaction ... that has started mutation ...") and all subsequent
# mutations of the affected parts were blocked.
