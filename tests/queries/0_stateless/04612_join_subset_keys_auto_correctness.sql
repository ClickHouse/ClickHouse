...
-- Left keys 0..599; right keys 0..499, so left rows with user_id >= 500 have no match and
-- exercise outer-join NULL-extension. `request_id` is the low-NDV key that demotion keeps.
INSERT INTO jks2_left SELECT number % 600, number % 10, number FROM numbers(6000);
INSERT INTO jks2_right SELECT number % 500, number % 10, number FROM numbers(5000);
...
SELECT 'prefer_partial_merge_pref_inner' AS t,
    (SELECT count() FROM jks2_left l JOIN jks2_right r ON l.user_id = r.user_id AND l.request_id = r.request_id
        SETTINGS join_algorithm = 'prefer_partial_merge,hash', query_plan_hash_join_subset_keys_auto = 1,
            query_plan_hash_join_subset_keys_min_rows = 0, query_plan_hash_join_subset_keys_min_kept_selectivity = 0.001)
  = (SELECT count() FROM jks2_left l JOIN jks2_right r ON l.user_id = r.user_id AND l.request_id = r.request_id
        SETTINGS join_algorithm = 'prefer_partial_merge,hash', query_plan_hash_join_subset_keys_auto = 0) AS ok;
...
-- them. Verify each outer-join result is unchanged by demotion.
...
SELECT 'left_outer_extra_cond' AS t,
    (SELECT count() FROM jks2_left l LEFT JOIN jks2_right r
        ON l.user_id = r.user_id AND l.request_id = r.request_id AND r.extra < 2500
        SETTINGS join_algorithm = 'hash', query_plan_hash_join_subset_keys_auto = 1,
            query_plan_hash_join_subset_keys_min_rows = 0, query_plan_hash_join_subset_keys_min_kept_selectivity = 0.001)
  = (SELECT count() FROM jks2_left l LEFT JOIN jks2_right r
        ON l.user_id = r.user_id AND l.request_id = r.request_id AND r.extra < 2500
