...
# (e.g. `GRANT SELECT ON db.table`) does not hold `SHOW_DATABASES` on the database.
# Only the `USE` statement requires `SHOW_DATABASES`.
...
${CLICKHOUSE_CLIENT} --user "${TEST_USER}" --database "${CLICKHOUSE_DATABASE}" --query "SELECT n FROM granted_tb"
...
echo "-- USE still requires SHOW_DATABASES"
${CLICKHOUSE_CLIENT} --user "${TEST_USER}" --query "USE ${CLICKHOUSE_DATABASE}" 2>&1 | grep -o "ACCESS_DENIED" | head -1
