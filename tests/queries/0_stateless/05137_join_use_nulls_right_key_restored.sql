...
-- With join_use_nulls, a selected right join key is restored from the left key by the hash join
-- instead of being stored as an extra Nullable copy of the key.
...
SELECT '-- right side of the plan carries only the key';
