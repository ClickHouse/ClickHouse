...
INSERT INTO t SELECT * FROM format(JSONEachRow, '{"ip": "192.168.1.1"}');
...
INSERT INTO t SELECT * FROM format(JSONEachRow, '{"ip": "2001:db8::1"}');
SELECT dynamicType(json.ip) FROM t;
SELECT json.ip FROM t;
SET input_format_try_infer_ipv6 = 0;
DROP TABLE t;
...
CREATE TABLE t (json JSON(max_dynamic_paths=0)) ENGINE = Memory;
SET input_format_try_infer_ipv4 = 1;
INSERT INTO t SELECT * FROM format(JSONEachRow, '{"ip": "1.2.3"}\n{"ip": "256.0.0.1"}\n{"ip": "not-an-ip"}');
SELECT dynamicType(json.ip) FROM t;
SET input_format_try_infer_ipv4 = 0;
DROP TABLE t;
...
SET input_format_try_infer_ipv4 = 0;
INSERT INTO t SELECT * FROM format(JSONEachRow, '{"ip": "10.0.0.1"}');
...
CREATE TABLE t (json JSON(max_dynamic_paths=0)) ENGINE = Memory;
SET input_format_try_infer_ipv4 = 1;
INSERT INTO t SELECT * FROM format(JSONEachRow, '{"ips": ["192.168.1.1", "10.0.0.1"]}');
