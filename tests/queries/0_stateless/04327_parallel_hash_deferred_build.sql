...
-- present in the ANY map, so nothing is inserted and the stored block is dropped; no ref may ever
-- resolve to it. Rows are identical between the inserts, so ANY results stay deterministic.
