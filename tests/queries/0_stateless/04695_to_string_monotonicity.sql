...
-- `'-01:00:00'` is less than `'-99:00:00'` as a string, so the primary key must not exclude the part.
...
-- Local time is turned back from `01:59:59` to `01:00:00` in the middle of the part.
...
-- A time zone with a fixed offset keeps the order, and `toString` is strict there, so `y` stays in the prefix.
