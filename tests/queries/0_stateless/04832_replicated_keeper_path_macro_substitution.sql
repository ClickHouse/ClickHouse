...
CREATE TABLE `a/b` (c0 Int) ENGINE = ReplicatedMergeTree('/clickhouse/04832/{database}/{table}', 'r1') ORDER BY c0; -- { serverError BAD_ARGUMENTS }
-- The replica name becomes a znode name under `<path>/replicas/`, so it is a carrier too.
CREATE TABLE `c/d` (c0 Int) ENGINE = ReplicatedMergeTree('/clickhouse/04832/{database}/x', '{table}') ORDER BY c0; -- { serverError BAD_ARGUMENTS }
...
CREATE TABLE `.` (c0 Int) ENGINE = ReplicatedMergeTree('/clickhouse/04832/{database}/{table}', 'r2') ORDER BY c0; -- { serverError BAD_ARGUMENTS }
CREATE TABLE `..` (c0 Int) ENGINE = ReplicatedMergeTree('/clickhouse/04832/{database}/{table}', 'r3') ORDER BY c0; -- { serverError BAD_ARGUMENTS }
...
CREATE TABLE `e}f` (c0 Int) ENGINE = ReplicatedMergeTree('/clickhouse/04832/{database}/{table}', 'r6b') ORDER BY c0; -- { serverError BAD_ARGUMENTS }

-- Control characters are not valid znode names either. U+0085 is a C1 control, encoded as C2 85.
CREATE TABLE `g\x01h` (c0 Int) ENGINE = ReplicatedMergeTree('/clickhouse/04832/{database}/{table}', 'r6c') ORDER BY c0; -- { serverError BAD_ARGUMENTS }
CREATE TABLE `g\xC2\x85h` (c0 Int) ENGINE = ReplicatedMergeTree('/clickhouse/04832/{database}/{table}', 'r6d') ORDER BY c0; -- { serverError BAD_ARGUMENTS }
-- Both ends of each forbidden range: U+001F and U+007F are single bytes, U+0080 and U+009F need the
-- pair test.
CREATE TABLE `g\x1Fh` (c0 Int) ENGINE = ReplicatedMergeTree('/clickhouse/04832/{database}/{table}', 'r6d0') ORDER BY c0; -- { serverError BAD_ARGUMENTS }
CREATE TABLE `g\x7Fh` (c0 Int) ENGINE = ReplicatedMergeTree('/clickhouse/04832/{database}/{table}', 'r6d1') ORDER BY c0; -- { serverError BAD_ARGUMENTS }
CREATE TABLE `g\xC2\x80h` (c0 Int) ENGINE = ReplicatedMergeTree('/clickhouse/04832/{database}/{table}', 'r6d2') ORDER BY c0; -- { serverError BAD_ARGUMENTS }
CREATE TABLE `g\xC2\x9Fh` (c0 Int) ENGINE = ReplicatedMergeTree('/clickhouse/04832/{database}/{table}', 'r6d3') ORDER BY c0; -- { serverError BAD_ARGUMENTS }
-- A name whose bytes merely look like the above is unaffected: U+00BF is C2 BF, outside the C1 range.
CREATE TABLE `g\xC2\xBFh` (c0 Int) ENGINE = ReplicatedMergeTree('/clickhouse/04832/{database}/{table}', 'r6e') ORDER BY c0;
CREATE TABLE `таблица_🚀_表` (c0 Int) ENGINE = ReplicatedMergeTree('/clickhouse/04832/{database}/{table}', 'r6f') ORDER BY c0;

-- Ordinary and dotted names keep working: the rule is not a character-level ban on '.'.
CREATE TABLE t_plain (c0 Int) ENGINE = ReplicatedMergeTree('/clickhouse/04832/{database}/{table}', 'r7') ORDER BY c0;
CREATE TABLE `my.table` (c0 Int) ENGINE = ReplicatedMergeTree('/clickhouse/04832/{database}/{table}', 'r8') ORDER BY c0;
...
CREATE TABLE `..` (c0 Int) ENGINE = ReplicatedMergeTree('/clickhouse/04832/{database}/x', '{table}') ORDER BY c0; -- { serverError BAD_ARGUMENTS }
...
CREATE TABLE t_explicit_replica (c0 Int) ENGINE = ReplicatedMergeTree('/clickhouse/04832/{database}/{table}', '.') ORDER BY c0;
SELECT replica_name FROM system.replicas WHERE database = currentDatabase() AND table = 't_explicit_replica';

-- The replica name reaches the output only in the second pass here, via a configured macro, while the
-- path is fully literal. So the substitution has to be recorded in that pass for this to be rejected:
-- 'table_' prefixes the value, which keeps a '.' legal, and the control byte is what survives it.
CREATE TABLE `g\x01h` (c0 Int) ENGINE = ReplicatedMergeTree('/clickhouse/04832/literal/x', '{default_name_test}') ORDER BY c0; -- { serverError BAD_ARGUMENTS }

-- A substituted replica name pins the path to the table, so renaming is refused. The path here is
-- literal, so only the replica name can be what makes the restriction apply.
CREATE TABLE t_renamable (c0 Int) ENGINE = ReplicatedMergeTree('/clickhouse/04832/{database}/rn', '{default_name_test}') ORDER BY c0;
RENAME TABLE t_renamable TO t_renamed; -- { serverError NOT_IMPLEMENTED }

-- Paths written out in full are untouched, including the nesting an operator may rely on.
CREATE TABLE t_nested_path (c0 Int) ENGINE = ReplicatedMergeTree('/clickhouse/04832/{database}/a/b/c', 'r9') ORDER BY c0;
