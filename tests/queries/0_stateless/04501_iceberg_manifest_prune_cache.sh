...
$CLICKHOUSE_CLIENT -q "SELECT * FROM iceberg_prune_test WHERE part = 1 AND id = 5 SETTINGS use_iceberg_partition_pruning=1" --query_id=q_prune2 > /dev/null
$CLICKHOUSE_CLIENT -q "SYSTEM FLUSH LOGS"
...
$CLICKHOUSE_CLIENT -q "SELECT id FROM iceberg_prune_test WHERE part = 1 AND id = 6 SETTINGS use_iceberg_partition_pruning=1" --query_id=q_prune3
$CLICKHOUSE_CLIENT -q "SYSTEM FLUSH LOGS"
...
echo "CLEAR OK"
$CLICKHOUSE_CLIENT -q "SELECT * FROM iceberg_prune_test WHERE part = 1 SETTINGS use_iceberg_partition_pruning=1" --query_id=q_prune4 > /dev/null
$CLICKHOUSE_CLIENT -q "SYSTEM FLUSH LOGS"
$CLICKHOUSE_CLIENT -q "SELECT ProfileEvents['IcebergManifestPruneCacheMisses'] > 0 AS miss4 FROM system.query_log WHERE query_id='q_prune4' AND type='QueryFinish'"
