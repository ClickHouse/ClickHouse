...
# must never raise `min_marks_per_task` above the floor (`merge_tree_min_read_task_size = 4`, the
# concurrent-read floors are pinned to one mark). The outer value is raised to 100 GB: under the leak
# the initiator-local pool would jump to `sum_marks / (threads * replicas) / 2` (~32 marks for the
# ~780 marks of this table, 4 threads, 3 replicas).
#
# The table is kept small on purpose: the cost of this test is per task (every task is a coordinator
# round trip), so the mark count and the task floor - not the row count - decide its run time.
...
SETTINGS index_granularity = 128, storage_policy = 's3_cache';

SYSTEM STOP MERGES t_remote_task_size;

INSERT INTO t_remote_task_size SELECT number FROM numbers(100000);
...
--merge_tree_min_read_task_size=4 \
...
# floor (4 marks); a single oversized value means some pool derived the task size from the
# outer 100 GB (~32 marks).
echo "$LOG" | grep -o 'Will use min_marks_per_task=[0-9]*' | awk -F= '
    { seen = 1; if ($2 > 12) oversized++ }
