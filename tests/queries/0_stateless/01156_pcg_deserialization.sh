...
do
  ...
  $CLICKHOUSE_CLIENT -q "create table t (n UInt8, a1 AggregateFunction(groupArraySample(1, /* seed */ 1), UInt8)) engine=$engine"
  $CLICKHOUSE_CLIENT -q "insert into t select number % 5 as n, groupArraySampleState(1, /* seed */ 1)(toUInt8(number)) from numbers(10) group by n"
