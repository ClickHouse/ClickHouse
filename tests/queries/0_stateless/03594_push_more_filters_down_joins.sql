...
FROM (
    ...
    LEFT JOIN t3 ON t1.tid = t3.id
    WHERE true AND (t2.resource_id IS NOT NULL) AND (t2.status IN ('OPEN')) AND (t3.status IN ('BACKLOG'))
    SETTINGS enable_join_runtime_filters=0, query_plan_merge_filter_into_join_condition=0, query_plan_convert_outer_join_to_inner_join=1, query_plan_propagate_predicate_across_join=0
)
