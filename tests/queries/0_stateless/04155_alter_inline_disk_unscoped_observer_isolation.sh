...
# Accept CANNOT_OPEN_FILE alongside BAD_ARGUMENTS: the rejector ALTER's inline cache-wrap
# disk is built during validation, rolled back, then rebuilt on the apply path; if the
# validation build's FileCache StatusFile flock is not yet released by its async destructor,
# the rebuild is rejected with CANNOT_OPEN_FILE instead of the storage-policy guard's
# BAD_ARGUMENTS. Both reject the ALTER and leave the table unchanged; accept either so the
# marker does not depend on cache-teardown timing (cf. 04150).
