...
-- Two needles the dictionary shortcut is not taken for, so both agree with the plain-array oracle
-- printed beside them: a NaN is one dictionary entry but is never equal to itself, and a negative
-- needle is not representable in an unsigned element, so no stored value can equal it.
