...
# The archive format causes root_path to be empty (parent_path of "name.tar"),
# which previously led to stack overflow during cleanup by traversing the entire disk.
# A tar archive is used here because zip archives are rejected for object-storage-backed
# disks (a plain_rewritable disk over S3), see registerBackupEnginesFileAndDisk.cpp.
...
$CLICKHOUSE_CLIENT -q "BACKUP TABLE t_backup_archive_plain TO Disk('disk_s3_plain_rewritable_03517', '${backup_name}.tar') FORMAT Null"

$CLICKHOUSE_CLIENT -q "DROP TABLE IF EXISTS t_restore_archive_plain SYNC"
$CLICKHOUSE_CLIENT -q "RESTORE TABLE t_backup_archive_plain AS t_restore_archive_plain FROM Disk('disk_s3_plain_rewritable_03517', '${backup_name}.tar') FORMAT Null"
