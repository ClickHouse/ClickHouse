...
-- A comma join with `rand` must keep the equality in `WHERE`: as a join key `rand` would be evaluated
-- again per row of each side, so the query would return rows with `r.a` other than 3.
