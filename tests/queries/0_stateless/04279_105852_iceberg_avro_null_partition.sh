...
#
# The manifest is always Avro per the Iceberg spec, so the fix is independent
# of the data file format. We loop over Parquet, ORC, and Avro data file
# formats to confirm the same manifest path is exercised for all of them.
...
for FORMAT in Parquet ORC Avro; do
    TABLE="t_${CLICKHOUSE_DATABASE}_${FORMAT}"
    TABLE_PATH="${USER_FILES_PATH}/${TABLE}/"
    rm -rf "${TABLE_PATH}"

    ${CLICKHOUSE_CLIENT} --query "
        CREATE TABLE ${TABLE} (c0 Nullable(Int32))
        ENGINE = IcebergLocal('${TABLE_PATH}', '${FORMAT}')
        PARTITION BY c0
    "
