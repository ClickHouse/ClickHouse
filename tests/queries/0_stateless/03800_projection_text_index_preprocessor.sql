...
(
    key UInt64,
    str String,
    PROJECTION idx INDEX str TYPE text(tokenizer = 'splitByNonAlpha', preprocessor = lower(str))
)
ENGINE = MergeTree
ORDER BY key;
...
(
    key UInt64,
    str String,
    PROJECTION idx INDEX str TYPE text(tokenizer = 'splitByNonAlpha', preprocessor = concat(str, str))
)
ENGINE = MergeTree
ORDER BY tuple();
...
(
    key UInt64,
    str String,
    PROJECTION idx INDEX str TYPE text(tokenizer = 'splitByNonAlpha', preprocessor = udf_preprocessor(str))
)
ENGINE = MergeTree
ORDER BY tuple();
...
(
    ...
    str String,
    other_str String,
    PROJECTION idx INDEX str TYPE text(tokenizer = 'splitByNonAlpha', preprocessor = lower(other_str))
)
...
(
    ...
    str String,
    other_str String,
    PROJECTION idx INDEX str TYPE text(tokenizer = 'splitByNonAlpha', preprocessor = concat(str, other_str))
)
ENGINE = MergeTree ORDER BY tuple();   -- { serverError BAD_ARGUMENTS }
...
(
    key UInt64,
    str String,
    PROJECTION idx INDEX upper(str) TYPE text(tokenizer = 'splitByNonAlpha', preprocessor = lower(str))
)
ENGINE = MergeTree ORDER BY tuple();   -- { serverError BAD_ARGUMENTS }
...
(
    key UInt64,
    str String,
    PROJECTION idx INDEX lower(str) TYPE text(tokenizer = 'splitByNonAlpha', preprocessor = lower(str))
)
ENGINE = MergeTree ORDER BY tuple();   -- { serverError BAD_ARGUMENTS }
...
(
    key UInt64,
    str String,
    PROJECTION idx INDEX upper(str) TYPE text(tokenizer = 'splitByNonAlpha', preprocessor = lower(upper(str)))
)
ENGINE = MergeTree ORDER BY tuple();   -- { serverError BAD_ARGUMENTS }
...
(
    key UInt64,
    str String,
    PROJECTION idx INDEX str TYPE text(tokenizer = 'splitByNonAlpha', preprocessor = nonExistingFunction)
)
...
(
    key UInt64,
    str String,
    PROJECTION idx INDEX str TYPE text(tokenizer = 'splitByNonAlpha', preprocessor = nonExistingFunction(str))
)
...
(
    key UInt64,
    str String,
    PROJECTION idx INDEX str TYPE text(tokenizer = 'splitByNonAlpha', preprocessor = length(str))
)
...
(
    key UInt64,
    str String,
    PROJECTION idx INDEX str TYPE text(tokenizer = 'splitByNonAlpha', preprocessor = hostname())
)
ENGINE = MergeTree ORDER BY tuple();   -- { serverError INCORRECT_QUERY }
...
(
    key UInt64,
    str String,
    PROJECTION idx INDEX str TYPE text(tokenizer = 'splitByNonAlpha', preprocessor = concat(str, toString(rand())))
)
ENGINE = MergeTree ORDER BY tuple();   -- { serverError INCORRECT_QUERY }
...
(
    ...
    PROJECTION idx INDEX arr_str TYPE text(tokenizer = 'splitByNonAlpha', preprocessor = lower(arr_str))
)
ENGINE = MergeTree ORDER BY tuple();   -- { serverError BAD_ARGUMENTS }
