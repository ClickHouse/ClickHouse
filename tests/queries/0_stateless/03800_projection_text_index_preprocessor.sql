...
(
    key UInt64,
    val String,
    PROJECTION idx INDEX val TYPE text(tokenizer = 'splitByNonAlpha', preprocessor = concat(val, val))
)
ENGINE = MergeTree
ORDER BY tuple();
...
(
    key UInt64,
    val String,
    PROJECTION idx INDEX val TYPE text(tokenizer = 'splitByNonAlpha', preprocessor = udf_preprocessor(val))
)
ENGINE = MergeTree
ORDER BY tuple();
INSERT INTO tab VALUES (1, 'foo'), (2, 'BAR'), (3, 'Baz');
...
(
    ...
    val String,
    other_str String,
    PROJECTION idx INDEX val TYPE text(tokenizer = 'splitByNonAlpha', preprocessor = lower(other_str))
)
ENGINE = MergeTree ORDER BY tuple();  -- { serverError BAD_ARGUMENTS,UNKNOWN_IDENTIFIER,INCORRECT_QUERY }
...
(
    ...
    val String,
    other_str String,
    PROJECTION idx INDEX val TYPE text(tokenizer = 'splitByNonAlpha', preprocessor = concat(val, other_str))
)
ENGINE = MergeTree ORDER BY tuple();   -- { serverError BAD_ARGUMENTS,UNKNOWN_IDENTIFIER,INCORRECT_QUERY }
...
(
    key UInt64,
    val String,
    PROJECTION idx INDEX upper(val) TYPE text(tokenizer = 'splitByNonAlpha', preprocessor = lower(val))
)
ENGINE = MergeTree ORDER BY tuple();   -- { serverError BAD_ARGUMENTS,UNKNOWN_IDENTIFIER,INCORRECT_QUERY }
...
(
    key UInt64,
    val String,
    PROJECTION idx INDEX lower(val) TYPE text(tokenizer = 'splitByNonAlpha', preprocessor = lower(val))
)
ENGINE = MergeTree ORDER BY tuple();   -- { serverError BAD_ARGUMENTS,UNKNOWN_IDENTIFIER,INCORRECT_QUERY }
...
(
    key UInt64,
    val String,
    PROJECTION idx INDEX upper(val) TYPE text(tokenizer = 'splitByNonAlpha', preprocessor = lower(upper(val)))
)
ENGINE = MergeTree ORDER BY tuple();
DROP TABLE tab;
...
(
    key UInt64,
    val String,
    PROJECTION idx INDEX val TYPE text(tokenizer = 'splitByNonAlpha', preprocessor = BAD)
)
ENGINE = MergeTree ORDER BY key;   -- { serverError BAD_ARGUMENTS,UNKNOWN_IDENTIFIER }
...
(
    key UInt64,
    val String,
    PROJECTION idx INDEX val TYPE text(tokenizer = 'splitByNonAlpha', preprocessor = nonExistingFunction(val))
)
...
(
    key UInt64,
    val String,
    PROJECTION idx INDEX val TYPE text(tokenizer = 'splitByNonAlpha', preprocessor = length(val))
)
...
(
    key UInt64,
    val String,
    PROJECTION idx INDEX val TYPE text(tokenizer = 'splitByNonAlpha', preprocessor = hostname())
)
ENGINE = MergeTree ORDER BY tuple();   -- { serverError INCORRECT_QUERY }
...
(
    key UInt64,
    val String,
    PROJECTION idx INDEX val TYPE text(tokenizer = 'splitByNonAlpha', preprocessor = concat(val, toString(rand())))
)
ENGINE = MergeTree ORDER BY tuple();   -- { serverError INCORRECT_QUERY }
DROP TABLE IF EXISTS tab;
