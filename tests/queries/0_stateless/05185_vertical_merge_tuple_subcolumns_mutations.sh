...
${CLICKHOUSE_CLIENT} -q "
    ...
    WHERE database = currentDatabase() AND table = 't_upd' AND event_type = 'MergeParts'
...
echo 'standalone_skip_index_materialized'
print_skip_index_materialized t_ord_stand
...
echo 'packed_skip_index_materialized'
print_skip_index_materialized t_ord_pack
