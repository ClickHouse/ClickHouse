...
SELECT name, engine, parametrized_view_paramters FROM system.tables WHERE name = 'parametrized_view_one_param';

CREATE VIEW parametrized_view_multiple_params AS SELECT * FROM raw_data WHERE id BETWEEN {id_from:UInt32} AND {id_to:UInt32};

SELECT name, engine, parametrized_view_paramters FROM system.tables WHERE name = 'parametrized_view_multiple_params';
