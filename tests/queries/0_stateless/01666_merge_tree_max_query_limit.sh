...
function wait_for_query_to_start() {
    while [[ $($CLICKHOUSE_CURL -sS "$CLICKHOUSE_URL" -d "SELECT sum(read_rows) FROM system.processes WHERE query_id = '$1' settings query_cache_active_usage=false, query_cache_passive_usage=false") == 0 ]]; do sleep 0.1; done
}
...
insert into simple (select number, number + 100 from numbers(5000) settings query_cache_active_usage=false, query_cache_passive_usage=false);
"

query_id="long_running_query-$CLICKHOUSE_DATABASE"

echo "Spin up a long running query"
${CLICKHOUSE_CLIENT} --query "select sleepEachRow(0.1) from simple settings max_block_size = 1, query_cache_passive_usage=false, query_cache_active_usage=false format Null" --query_id "$query_id" >/dev/null 2>&1 &
...
${CLICKHOUSE_CLIENT} --query "select * from simple settings query_cache_active_usage=false, query_cache_passive_usage=false" 2>/dev/null
CODE=$?
...
${CLICKHOUSE_CLIENT} --query "select * from simple where i = 0 settings query_cache_active_usage=false, query_cache_passive_usage=false"

# We can modify the settings to take effect for future queries
echo "Modify min_marks_to_honor_max_concurrent_queries to 1"
${CLICKHOUSE_CLIENT} --query "alter table simple modify setting min_marks_to_honor_max_concurrent_queries = 1, query_cache_active_usage=false, query_cache_passive_usage=false"

# Now smaller queries are also throttled
echo "Check if another query with less marks to read is throttled"
${CLICKHOUSE_CLIENT} --query "select * from simple where i = 0 settings query_cache_active_usage=false, query_cache_passive_usage=false" 2>/dev/null
CODE=$?
[ "$CODE" -ne "202" ] && echo "Expected error code: 202 but got: $CODE" && exit 1
echo "yes"
...
${CLICKHOUSE_CLIENT} --query_id "$query_id" --query "select i from simple where j in (select i from simple where i < 10 settings query_cache_active_usage=false, query_cache_passive_usage=false) settings query_cache_active_usage=false, query_cache_passive_usage=false"
