...
CREATE TABLE source (id UInt64, value String) ENGINE = Memory;
INSERT INTO source SELECT number, 'old_' || toString(number) FROM numbers(10);
...
SELECT dictGet('cache_dict', 'value', toUInt64(0)) FORMAT Null;
SELECT dictGet('cache_dict', 'value', toUInt64(1)) FORMAT Null;
SELECT dictGet('cache_dict', 'value', toUInt64(2)) FORMAT Null;
SELECT dictGet('cache_dict', 'value', toUInt64(3)) FORMAT Null;
...
SELECT dictGet('cache_dict', 'value', toUInt64(4)) FORMAT Null;

-- Make source values new so we can tell cached (old) from re-fetched (new)
TRUNCATE TABLE source;
INSERT INTO source SELECT number, 'new_' || toString(number) FROM numbers(10);

-- Step 4: fetch key 5 -> triggers another eviction (evicts key 1, key 4 survives)
SELECT dictGet('cache_dict', 'value', toUInt64(5)) FORMAT Null;

-- Key 4 was fetched right before key 5, so it must still be cached (old value).
SELECT 'recent_insert_survived', dictGet('cache_dict', 'value', toUInt64(4)) = 'old_4';
-- Key 0 was the least recently used and must have been evicted (re-fetched -> new value).
SELECT 'lru_key_evicted', dictGet('cache_dict', 'value', toUInt64(0)) = 'new_0';
