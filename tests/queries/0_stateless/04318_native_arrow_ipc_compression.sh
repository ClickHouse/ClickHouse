...
# formats. For each codec the data must round-trip through the native writer and reader, and the
# natively-written compressed output must also be readable by a third-party Apache Arrow tool (pyarrow's
# `open_stream`/`open_file`), all matching the original.
...
for format in ArrowStream Arrow; do
    ...
    for codec in lz4_frame zstd; do
        ${CLICKHOUSE_LOCAL} --query "
            ...
            SETTINGS output_format_arrow_compression_method = '${codec}',
                     output_format_arrow_string_as_string = 1,
                     engine_file_truncate_on_insert = 1,
                     max_block_size = 7"
        RESULT=$(${CLICKHOUSE_LOCAL} --query "SELECT * FROM file('${DATA_FILE}', '${format}') ORDER BY i")
        REFERENCE=$(${CLICKHOUSE_LOCAL} --query "${GEN} ORDER BY i")
        if [ "$RESULT" = "$REFERENCE" ]; then
            echo "${format} ${codec}: OK"
        else
            echo "${format} ${codec}: MISMATCH"
        fi
