...
# These queries throw, so they go through the server: a fresh clickhouse-local process symbolizes the
# stack trace of the exception for its query log from scratch, which takes about 30 seconds under MSan.
for value in "'.'" "'+.'" "'-.'" "toFixedString('.', 20)" "toFixedString('+.', 20)" "toFixedString('-.', 20)"
do
    ${CLICKHOUSE_CLIENT} -q "SET date_time_input_format = 'basic', cast_string_to_date_time_mode = 'basic'; SELECT toDateTime64(${value}, 3, 'UTC')" 2>&1 \
