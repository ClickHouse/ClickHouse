...
CURL_OUTPUT="$(
    ...
    --data-urlencode "query=SELECT number, avg(number) FROM numbers(1e7) GROUP BY number FORMAT Null SETTINGS max_threads = 1" \
...
${CLICKHOUSE_CURL} -s -S -v -N -G "${CLICKHOUSE_URL}" \
    --data-urlencode "query=SELECT number, avg(number) FROM numbers(1e5) GROUP BY number FORMAT Null SETTINGS max_threads = 1" \
    --data-urlencode "cancel_http_readonly_queries_on_client_close=1" \
...
CURL_OUTPUT="$(
    ${CLICKHOUSE_CURL} -s -S -v -N -G "${CLICKHOUSE_URL}" \
    --data-urlencode "query=SELECT number, avg(number) FROM numbers(1e12) GROUP BY number FORMAT Null SETTINGS max_threads = 1" \
    --data-urlencode "cancel_http_readonly_queries_on_client_close=1" \
    --data-urlencode "send_progress_in_http_headers=1" \
