...
CREATE TABLE tab(id Int32, vec Array(Float32), INDEX vec_idx vec TYPE usearch()) ENGINE = MergeTree ORDER BY id SETTINGS index_granularity = 8192;
...
CREATE TABLE tab(id Int32, vec Array(Float32), INDEX vec_idx vec TYPE usearch() GRANULARITY 2) ENGINE = MergeTree ORDER BY id SETTINGS index_granularity = 3;
...
-- Test with metric = 'cosineDistance' (Usearch), ScalarKind = 'f64' (Usearch)

CREATE TABLE tab(id Int32, vec Array(Float32), INDEX vec_idx vec TYPE usearch('cosineDistance', 'f64') GRANULARITY 2) ENGINE = MergeTree ORDER BY id SETTINGS index_granularity = 3;
...
SELECT '- Maximum distance is negative';
WITH [0.0, 2.0] AS reference_vec
SELECT id, vec, cosineDistance(vec, reference_vec)
FROM tab
WHERE cosineDistance(vec, reference_vec) < -1.0
...
SELECT '- With setting "max_limit_for_ann_queries"';
EXPLAIN indexes=1
WITH [0.0, 2.0] AS reference_vec
SELECT id, vec, cosineDistance(vec, reference_vec)
FROM tab
