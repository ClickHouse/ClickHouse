...
-- The rejection happens on the DDL path, so such a constraint never reaches the metadata.

DROP TABLE IF EXISTS check_nested_scalar;
CREATE TABLE check_nested_scalar (c0 Int) ENGINE = MergeTree() ORDER BY tuple();
ALTER TABLE check_nested_scalar ADD CONSTRAINT c0 CHECK equals((SELECT 1), 1); -- { serverError BAD_ARGUMENTS }
...
ALTER TABLE check_deep_scalar ADD CONSTRAINT c0 CHECK c0 + (SELECT 1) > 0; -- { serverError BAD_ARGUMENTS }
...
ALTER TABLE check_in_lhs ADD CONSTRAINT c0 CHECK (SELECT 1) IN (1, 2, 3); -- { serverError BAD_ARGUMENTS }
...
ALTER TABLE check_in_rhs_list ADD CONSTRAINT c0 CHECK c0 IN (1, (SELECT 1), 3); -- { serverError BAD_ARGUMENTS }
