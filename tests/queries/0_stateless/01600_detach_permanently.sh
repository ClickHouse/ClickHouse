...
clickhouse_local "SELECT if(count() = 10, 'MV can be detached permanently', 'MV detach failed') FROM db_ordinary.src_mv_with_inner SETTINGS enable_analyzer = 0" 2>&1 | grep -F -c "Table db_ordinary.src_mv_with_inner does not exist. (UNKNOWN_TABLE)"
clickhouse_local "SELECT if(count() = 10, 'MV can be detached permanently', 'MV detach failed') FROM db_ordinary.src_mv_with_inner SETTINGS enable_analyzer = 1" 2>&1 | grep -F -c "Table db_ordinary.src_mv_with_inner does not exist. (UNKNOWN_TABLE)"
