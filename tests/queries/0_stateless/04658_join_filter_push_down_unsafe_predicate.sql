...
-- The conjunct is moved below the join by the main path and copied by the partial path.
-- Run at the shipped default of use_join_disjunctions_push_down so a regression on either
-- path is visible. Pre-fix this printed 0 (the count was about 500).
...
-- Both values of use_join_disjunctions_push_down, so neither push-down path can regress
-- unobserved. Pre-fix both printed 0 (counts about 500).
...
SELECT 'B1 visible non deterministic', abs((
    (SELECT count() FROM (
        ...
        WHERE rand(t_left_big.a) % 2 = 0) SETTINGS query_plan_filter_push_down = 1) /
    (SELECT count() FROM (
        SELECT t_left_big.a FROM t_left_big LEFT JOIN t_right_big ON t_left_big.a = t_right_big.a
