...
select trimLeft(explain) as e from (EXPLAIN indexes=1 WITH [0.0, 2.0] AS reference_vec SELECT id FROM tab ORDER BY L2Distance(vec1, reference_vec) LIMIT 3) where e ilike '%Skip%' or e ilike '%Name: idx%' or e ilike '%vector_similarity%';

SELECT 'Searches on vec2 should be brute force';
select trimLeft(explain) as e from (EXPLAIN indexes=1 WITH [0.0, 2.0] AS reference_vec SELECT id FROM tab ORDER BY L2Distance(vec2, reference_vec) LIMIT 3) where e ilike '%Skip%' or e ilike '%Name: idx%' or e ilike '%vector_similarity%';
