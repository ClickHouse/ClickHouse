...
do
  ...
  then
    ...
    ${CLICKHOUSE_CLIENT} --query "$sql" 2>&1 | grep -oh "Selected .* partitions by partition key, .* parts by partition key, *. parts by primary key, .* marks by primary key, .* marks to read from .* ranges.*$"
