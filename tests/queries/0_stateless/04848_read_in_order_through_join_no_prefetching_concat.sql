...
-- When the read order is preserved through a `JOIN` (`keepLeftPipelineInOrder`), the join keeps
-- exactly the probe-side streams it receives and runs one `JoiningTransform` per stream
-- (`QueryPipelineBuilder::joinPipelinesRightLeft` skips the resize). `PrefetchingConcatProcessor`
-- must NOT collapse a single-part filtered read into one stream there: it would serialize all the
-- per-stream join work. Expect no PrefetchingConcat and a multi-stream join probe side.
...
DROP TABLE IF EXISTS events_04848;
DROP TABLE IF EXISTS payloads_04848;

CREATE TABLE events_04848 (path String, id UInt64)
ENGINE = MergeTree ORDER BY path
SETTINGS index_granularity = 8192
AS SELECT concat('path/', toString(number % 100000), '/file.log'), number % 40 FROM numbers(1000000);

OPTIMIZE TABLE events_04848 FINAL;

CREATE TABLE payloads_04848 (id UInt64, payload String) ENGINE = MergeTree ORDER BY tuple()
...
SELECT count() > 0 FROM (
    EXPLAIN PIPELINE SELECT path, id FROM events_04848
    WHERE path LIKE '%file.log'
    ORDER BY path
) WHERE explain LIKE '%PrefetchingConcat%';

-- Through the JOIN: no PrefetchingConcat.
SELECT 'no_prefetching_concat_through_join';
SELECT count() > 0 FROM (
    EXPLAIN PIPELINE SELECT e.path, p.payload
    FROM events_04848 AS e
    LEFT JOIN payloads_04848 AS p ON e.id = p.id
...
-- trivial).
SELECT 'no_full_sort_through_join';
SELECT count() FROM (
    EXPLAIN PIPELINE SELECT e.path, p.payload
    FROM events_04848 AS e
    LEFT JOIN payloads_04848 AS p ON e.id = p.id
...
DROP TABLE payloads_04848;
DROP TABLE events_04848;
