#!/usr/bin/env bash
# Tags: zookeeper, no-parallel, no-random-settings
...
$CLICKHOUSE_CLIENT -q "INSERT INTO test_01192.mt SELECT number + sleepEachRow(1.5) FROM numbers(10) SETTINGS max_block_size=1" && echo "inserted" &
sleep 1
...
$CLICKHOUSE_CLIENT -q "INSERT INTO test_01192_atomic.mt SELECT number + sleepEachRow(1) + 10 FROM numbers(10) SETTINGS max_block_size=1" && echo "inserted" &
sleep 1
