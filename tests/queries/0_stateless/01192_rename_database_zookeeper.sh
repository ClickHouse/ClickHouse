#!/usr/bin/env bash
# Tags: zookeeper, no-parallel, no-fasttest, no-shared-merge-tree, no-flaky-check, no-azure-blob-storage, no-async-insert
...
# no-async-insert: a queued flush writes after `RENAME DATABASE`, so it fails with `TABLE_UUID_MISMATCH`.
