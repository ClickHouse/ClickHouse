...
-- join_algorithm and query_plan_join_swap_table are pinned too: a merge algorithm or a
-- swapped join drops the runtime filter, which would remove the second shared conjunct and
-- let an unfixed reader pass (runtime filter needs a hash-family join on the left side).
WITH flat AS
(
    SELECT
...
WHERE rn = 1
SETTINGS input_format_parquet_use_native_reader_v3 = 1, optimize_move_to_prewhere = 1, enable_analyzer = 1, query_plan_enable_optimizations = 1, query_plan_optimize_prewhere = 1, enable_join_runtime_filters = 1, join_runtime_filter_min_probe_rows = 0, join_algorithm = 'hash', query_plan_join_swap_table = false;
