...
SELECT count() > 0
FROM (EXPLAIN SELECT min(carrier), max(carrier), count() FROM t_minmax_count_alter SETTINGS optimize_trivial_count_query = 0, optimize_use_implicit_projections = 1)
