...
-- between `max_threads = 3` and `max_threads = 2`, exactly as it does for the default hash aggregation.
--
...
-- returns a different `groupArray` for `max_threads = 3` than for `max_threads = 1`. Comparing arms of
...
-- All partitions span the whole key range, so the in-order read produces streams that overlap in `k`. The data
-- is deliberately small (a fine `index_granularity` gives enough granules for many streams anyway), because the
-- flaky check runs this test several times in a row under a debug build.
...
INSERT INTO t_aio_shuffle_order SELECT number % 100, number FROM numbers(1000);
INSERT INTO t_aio_shuffle_order SELECT number % 100, number + 100000 FROM numbers(1000);
...
INSERT INTO t_aio_shuffle_order_parts SELECT number % 100, number FROM numbers(500);
INSERT INTO t_aio_shuffle_order_parts SELECT number % 100, number + 100000 FROM numbers(500);
INSERT INTO t_aio_shuffle_order_parts SELECT number % 97, number + 200000 FROM numbers(500);
INSERT INTO t_aio_shuffle_order_parts SELECT number % 101, number + 300000 FROM numbers(500);
...
FROM (EXPLAIN PIPELINE SELECT intDiv(k, 10) AS g, groupArray(v) FROM t_aio_shuffle_order GROUP BY g
      SETTINGS max_threads = 3, aggregation_in_order_shuffle = 1, read_in_order_two_level_merge_threshold = 2);

SELECT countIf(explain LIKE '%BufferedShardByHashTransform%') > 0
FROM (EXPLAIN PIPELINE SELECT intDiv(k, 10) AS g, groupArray(v) FROM t_aio_shuffle_order GROUP BY g
      SETTINGS max_threads = 3, aggregation_in_order_shuffle = 1, read_in_order_two_level_merge_threshold = 1000);

SELECT countIf(explain LIKE '%BufferedShardByHashTransform%') > 0
FROM (EXPLAIN PIPELINE SELECT intDiv(k, 10) AS g, groupArray(v) FROM t_aio_shuffle_order_parts GROUP BY g
      SETTINGS max_threads = 5, aggregation_in_order_shuffle = 1, read_in_order_two_level_merge_threshold = 2);

SELECT countIf(explain LIKE '%BufferedShardByHashTransform%') > 0
FROM (EXPLAIN PIPELINE SELECT intDiv(k, 10) AS g, groupArray(v) FROM t_aio_shuffle_order_parts GROUP BY g
      SETTINGS max_threads = 5, aggregation_in_order_shuffle = 1, read_in_order_two_level_merge_threshold = 1000);
...
SELECT
    (SELECT groupBitXor(cityHash64(g, a, x, y)) FROM
        ...
        SETTINGS max_threads = 3, read_in_order_two_level_merge_threshold = 2, aggregation_in_order_shuffle = 1)
  = (SELECT groupBitXor(cityHash64(g, a, x, y)) FROM
        (SELECT intDiv(k, 10) AS g, groupArray(v) AS a, any(v) AS x, anyLast(v) AS y
         FROM t_aio_shuffle_order GROUP BY g)
        SETTINGS max_threads = 3, read_in_order_two_level_merge_threshold = 2, aggregation_in_order_shuffle = 0);

SELECT
    (SELECT groupBitXor(cityHash64(g, a, x, y)) FROM
        (SELECT intDiv(k, 10) AS g, groupArray(v) AS a, any(v) AS x, anyLast(v) AS y
         FROM t_aio_shuffle_order GROUP BY g)
        SETTINGS max_threads = 3, read_in_order_two_level_merge_threshold = 1000, aggregation_in_order_shuffle = 1)
  = (SELECT groupBitXor(cityHash64(g, a, x, y)) FROM
        (SELECT intDiv(k, 10) AS g, groupArray(v) AS a, any(v) AS x, anyLast(v) AS y
         FROM t_aio_shuffle_order GROUP BY g)
        SETTINGS max_threads = 3, read_in_order_two_level_merge_threshold = 1000, aggregation_in_order_shuffle = 0);

SELECT
    (SELECT groupBitXor(cityHash64(g, a, x, y)) FROM
        (SELECT intDiv(k, 10) AS g, groupArray(v) AS a, any(v) AS x, anyLast(v) AS y
         FROM t_aio_shuffle_order_parts GROUP BY g)
        SETTINGS max_threads = 5, read_in_order_two_level_merge_threshold = 2, aggregation_in_order_shuffle = 1)
  = (SELECT groupBitXor(cityHash64(g, a, x, y)) FROM
        (SELECT intDiv(k, 10) AS g, groupArray(v) AS a, any(v) AS x, anyLast(v) AS y
         FROM t_aio_shuffle_order_parts GROUP BY g)
        SETTINGS max_threads = 5, read_in_order_two_level_merge_threshold = 2, aggregation_in_order_shuffle = 0);

SELECT
    (SELECT groupBitXor(cityHash64(g, a, x, y)) FROM
        (SELECT intDiv(k, 10) AS g, groupArray(v) AS a, any(v) AS x, anyLast(v) AS y
         FROM t_aio_shuffle_order_parts GROUP BY g)
        SETTINGS max_threads = 5, read_in_order_two_level_merge_threshold = 1000, aggregation_in_order_shuffle = 1)
  = (SELECT groupBitXor(cityHash64(g, a, x, y)) FROM
        (SELECT intDiv(k, 10) AS g, groupArray(v) AS a, any(v) AS x, anyLast(v) AS y
         FROM t_aio_shuffle_order_parts GROUP BY g)
        SETTINGS max_threads = 5, read_in_order_two_level_merge_threshold = 1000, aggregation_in_order_shuffle = 0);
