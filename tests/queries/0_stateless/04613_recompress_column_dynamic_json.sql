...
INSERT INTO t_recompress_dynamic SELECT number, repeat('a', 200)::Dynamic FROM numbers(30000);
...
SELECT 'dynamic none is large', sum(data_compressed_bytes) > 4000000 FROM system.parts_columns
...
SELECT 'dynamic zstd is small', sum(data_compressed_bytes) < 1000000 FROM system.parts_columns
WHERE database = currentDatabase() AND table = 't_recompress_dynamic' AND column = 'd' AND active;
SELECT 'dynamic point', d::String = repeat('a', 200) FROM t_recompress_dynamic WHERE id = 29999;
...
INSERT INTO t_recompress_json SELECT number, ('{"a":"' || repeat('x', 150) || '","b":' || toString(number) || '}')::JSON FROM numbers(30000);
...
SELECT 'json none is large', sum(data_compressed_bytes) > 3000000 FROM system.parts_columns
...
SELECT 'json zstd is small', sum(data_compressed_bytes) < 1000000 FROM system.parts_columns
WHERE database = currentDatabase() AND table = 't_recompress_json' AND column = 'j' AND active;
SELECT 'json point', j.a = repeat('x', 150), j.b = 29999 FROM t_recompress_json WHERE id = 29999;
