...
DROP TABLE IF EXISTS tab_compact_full SYNC;
DROP TABLE IF EXISTS tab_wide_full SYNC;

CREATE TABLE tab_compact_full(id Int32, str String, INDEX idx str TYPE text(tokenizer = 'default') GRANULARITY 2) ENGINE = ReplicatedMergeTree('/clickhouse/tables/{database}/text_index', 'r1') ORDER BY id SETTINGS min_bytes_for_wide_part = 1e9, min_rows_for_wide_part = 1e9, min_bytes_for_full_part_storage = 0, index_granularity = 3;
CREATE TABLE tab_wide_full(id Int32, str String, INDEX idx str TYPE text(tokenizer = 'default') GRANULARITY 2) ENGINE = ReplicatedMergeTree('/clickhouse/tables/{database}/text_index', 'r2') ORDER BY id SETTINGS min_bytes_for_wide_part = 0, min_rows_for_wide_part = 0, min_bytes_for_full_part_storage = 0, index_granularity = 3;
...
DROP TABLE tab_compact_full SYNC;
DROP TABLE tab_wide_full SYNC;
