...
CREATE TABLE tab_compact_full(id Int32, str String, INDEX idx str TYPE text(tokenizer = 'splitByNonAlpha') GRANULARITY 2) ENGINE = MergeTree ORDER BY id SETTINGS min_bytes_for_wide_part = 1e9, min_rows_for_wide_part = 1e9, min_bytes_for_full_part_storage = 0, index_granularity = 3;
CREATE TABLE tab_wide_full(id Int32, str String, INDEX idx str TYPE text(tokenizer = 'splitByNonAlpha') GRANULARITY 2) ENGINE = MergeTree ORDER BY id SETTINGS min_bytes_for_wide_part = 0, min_rows_for_wide_part = 0, min_bytes_for_full_part_storage = 0, index_granularity = 3;
