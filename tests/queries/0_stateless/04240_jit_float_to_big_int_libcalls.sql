...
-- A float source with an integer destination is never compiled, so it has no JIT arm here and only
-- the interpreted one below: `05055_jit_float_to_integer_cast` covers that direction. The reverse
-- direction is compiled and reaches `__floattisf` / `__floattidf`.
