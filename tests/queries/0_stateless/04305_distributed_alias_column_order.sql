...
-- Constant projection columns mixed with shared-subexpression aliases and an ORDER BY on a
-- non-selected column (the shape of the original repro: a literal CAST constant, plain columns,
-- an ALIAS column and an expression built on top of that alias). This exercises a different facet
-- than the cases above: the projection's available-column pool diverges from the projection output
-- both in ORDER (CSE moves the shared concat(s,'_x') node ahead of the expression that uses it) and
-- in MEMBERSHIP (the literal constants and the ORDER BY key `key` are present in the pool but are not
-- projection outputs, so the pool is not a permutation of the outputs). The header is also
-- structurally different per stage: on shards the alias expands to concat(s,'_x') while the
-- initiator delivers it as a materialized alias column, so the positional rename at the mergeable
-- boundary must line the columns up correctly.
DROP TABLE IF EXISTS local_const_stage;
DROP TABLE IF EXISTS dist_const_stage;

CREATE TABLE local_const_stage
(
    `key` Int32,
    `s` String,
    `c_alias` String ALIAS concat(s, '_x'),
    `c_alias2` String ALIAS concat(s, '_y')
)
ENGINE = MergeTree()
ORDER BY s;

CREATE TABLE dist_const_stage
(
    `key` Int32,
    `s` String,
    `c_alias` String ALIAS concat(s, '_x'),
...
ENGINE = Distributed('test_cluster_two_shards_localhost', currentDatabase(), local_const_stage, rand());

INSERT INTO local_const_stage VALUES (10, 'a'), (20, 'b');

SELECT 'local_const_stage';
SELECT '2026-02-27 14:27:49' :: DateTime AS dt, 42 AS num, s, c_alias, concat(c_alias, '!') AS d, c_alias2
FROM local_const_stage ORDER BY key ASC LIMIT 1;

SELECT 'distributed_const_stage';
SELECT '2026-02-27 14:27:49' :: DateTime AS dt, 42 AS num, s, c_alias, concat(c_alias, '!') AS d, c_alias2
FROM dist_const_stage ORDER BY key ASC LIMIT 1;

DROP TABLE dist_const_stage;
DROP TABLE local_const_stage;
