#!/usr/bin/env bash
# Tags: no-fasttest, no-random-settings, no-random-merge-tree-settings, no-shared-merge-tree, no-encrypted-storage, no-cas-storage
# no-cas-storage: BACKUP via temporary hard links is not supported on a cas disk (Code 344 SUPPORT_IS_DISABLED; BACKUP/RESTORE, B16/B34)
