...
$CLICKHOUSE_CLIENT -m -q "
    ...
    create table data_write (key Int) engine=ReplicatedMergeTree('/tables/{database}/data', 'write') order by key settings write_marks_for_substreams_in_compact_parts=1, storage_policy='nonencrypted';
    create table data_read (key Int) engine=ReplicatedMergeTree('/tables/{database}/data', 'read') order by key settings write_marks_for_substreams_in_compact_parts=1, storage_policy='nonencrypted';
