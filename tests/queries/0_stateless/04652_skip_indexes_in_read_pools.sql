-- Test for use_indexes_refiner_in_read_pools with skip indexes: mark ranges fully filtered out
-- by a skip index evaluated at data-read time (use_skip_indexes_on_data_read) are dropped inside
-- MergeTree read pools before read tasks are created for them.
...
INSERT INTO t_skip_idx_pools
SELECT number, if(number BETWEEN 1600 AND 1610 OR number BETWEEN 1700 AND 1710, 'rare', 'common'), number * 10
FROM numbers(4096);

OPTIMIZE TABLE t_skip_idx_pools FINAL;

-- Multi-threaded read: MergeTreeReadPool.
SELECT /* skip_refiner_query_default_pool */ id, region, value FROM t_skip_idx_pools WHERE region = 'rare' ORDER BY ALL
...
SELECT /* skip_refiner_query_in_order_pool */ id, region, value FROM t_skip_idx_pools WHERE region = 'rare' ORDER BY id LIMIT 5 SETTINGS max_threads = 1, optimize_read_in_order = 1;

-- Prefetched read pool: the refine-and-prefetch job never prefetches dropped ranges.
SELECT /* skip_refiner_query_prefetched_pool */ id, region, value FROM t_skip_idx_pools WHERE region = 'rare' ORDER BY ALL
...
SELECT /* skip_refiner_query_parallel_replicas_default */ id, region, value FROM t_skip_idx_pools WHERE region = 'rare' ORDER BY ALL
...
SELECT count() FROM t_skip_idx_pools WHERE region = 'nonexistent';
...
WHERE current_database = currentDatabase()
    ...
    AND query LIKE '%skip_refiner_query_default_pool%'
...
WHERE current_database = currentDatabase()
    AND type = 'QueryFinish'
    AND query LIKE '%skip_refiner_query_in_order_pool%'
    AND query NOT LIKE '%query_log%';
...
WHERE current_database = currentDatabase()
    AND type = 'QueryFinish'
    AND query LIKE '%skip_refiner_query_prefetched_pool%'
    AND query NOT LIKE '%query_log%';
...
WHERE type = 'QueryFinish'
    AND initial_query_id IN (
        ...
        WHERE current_database = currentDatabase()
            ...
            AND query LIKE '%skip_refiner_query_parallel_replicas_default%'
            AND query NOT LIKE '%query_log%');
...
WHERE type = 'QueryFinish'
    AND initial_query_id IN (
        ...
        WHERE current_database = currentDatabase()
            AND type = 'QueryFinish'
            AND is_initial_query
            AND query LIKE '%skip_refiner_query_parallel_replicas_in_order%'
            AND query NOT LIKE '%query_log%');
...
WHERE type = 'QueryFinish'
    AND initial_query_id IN (
        ...
        WHERE current_database = currentDatabase()
            AND type = 'QueryFinish'
            AND is_initial_query
            AND query LIKE '%skip_refiner_query_parallel_replicas_reverse%'
            AND query NOT LIKE '%query_log%');

DROP TABLE t_skip_idx_pools;
