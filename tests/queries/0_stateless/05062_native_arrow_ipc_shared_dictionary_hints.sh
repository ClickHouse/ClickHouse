...
# them. The values of a shared dictionary are decoded once per distinct requested type and each field reads
# the values decoded for its own type: raw 16-byte binaries as `Int128` for one field and `UInt128` for the
# other, a `date32` dictionary holding an out-of-range day number as `Int32` and as `Int64`, while a `Date32`
# request for the same dictionary is rejected as it is for a flat column. A `LowCardinality` wrapper is a
# requested type of its own that yields the same values as the plain type, and a field is read alone so the
# other field's request plays no part.
#
...
# schema, then one dictionary batch per field in field order (ids 0..3), then the record batch
messages = []
...
assert len(messages) == 6, len(messages)

# Point `b` at `a`'s dictionary (id 1 -> 0) and `d2` at `d1`'s (id 3 -> 2). The ids are the only int64
# values in the schema; pyarrow's read-back below verifies the result.
schema = bytearray(messages[0].metadata.to_pybytes())
for i in range(0, len(schema) - 7, 8):
    value = struct.unpack_from("<q", schema, i)[0]
    if value == 1:
        struct.pack_into("<q", schema, i, 0)
    elif value == 3:
        struct.pack_into("<q", schema, i, 2)
...
for message in [messages[1], messages[3], messages[5]]:
