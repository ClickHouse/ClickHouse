...
# `TTL ... RECOMPRESS` clause must require its `enable_<family>_codec` setting there as well, unlike a
...
$CLICKHOUSE_CLIENT --enable_zxc_codec 0 -q "ATTACH TABLE t_zxc_ttl_attach_full UUID '${UUID}' (d Date, x UInt64) ENGINE = MergeTree ORDER BY x TTL d + INTERVAL 1 DAY RECOMPRESS CODEC(ZXC);" 2>&1 | grep -m 1 -o -F 'BAD_ARGUMENTS'

# A codec that requires a column type is rejected regardless of the setting, exactly as at CREATE.
$CLICKHOUSE_CLIENT --enable_zxc_codec 1 -q "ATTACH TABLE t_zxc_ttl_attach_full UUID '${UUID}' (d Date, x UInt64) ENGINE = MergeTree ORDER BY x TTL d + INTERVAL 1 DAY RECOMPRESS CODEC(T64);" 2>&1 | grep -m 1 -o -F 'BAD_ARGUMENTS'
...
$CLICKHOUSE_CLIENT --allow_repeated_settings --enable_zxc_codec 1 --send_logs_level fatal -q "ATTACH TABLE t_zxc_ttl_attach_full UUID '${UUID}' (d Date, x UInt64) ENGINE = MergeTree ORDER BY x TTL d + INTERVAL 1 DAY RECOMPRESS CODEC(ZXC);"
...
$CLICKHOUSE_CLIENT --enable_zxc_codec 0 -q "DETACH TABLE t_zxc_ttl_attach_full;"
$CLICKHOUSE_CLIENT --enable_zxc_codec 0 -q "ATTACH TABLE t_zxc_ttl_attach_full;"
$CLICKHOUSE_CLIENT -q "SELECT count(), sum(x) FROM t_zxc_ttl_attach_full;"
...
$CLICKHOUSE_CLIENT --enable_zxc_codec 0 --allow_suspicious_ttl_expressions 1 -q "ATTACH TABLE t_zxc_ttl_attach_hatch UUID '${UUID2}' (d Date, x UInt64) ENGINE = MergeTree ORDER BY x TTL d + INTERVAL 1 DAY RECOMPRESS CODEC(ZXC);" 2>&1 | grep -m 1 -o -F 'BAD_ARGUMENTS'
