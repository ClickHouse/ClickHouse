...
for STORAGE_POLICY in 's3_cache' 'local_cache' 'azure_cache'; do
    ...
    $CLICKHOUSE_CLIENT --query "CREATE TABLE test_2242 (key UInt32, value String) Engine=MergeTree() ORDER BY key SETTINGS storage_policy='$STORAGE_POLICY', min_bytes_for_wide_part = 10485760, compress_marks=false, compress_primary_key=false, serialization_info_version = 'basic', compact_parts_max_granules_to_buffer = 1"
