...
-- The other functions that keep samples per bucket deserialize them independently in this release.
SELECT length(finalizeAggregation(CAST(substring(state, 1, 26) || unhex('FFFFFFFFFFFFFFFF') || substring(state, 35) AS AggregateFunction(timeSeriesRateToGrid(10, 120, 10, 70), UInt32, Float64))))
FROM (SELECT CAST(timeSeriesRateToGridState(10, 120, 10, 70)(toUInt32(55), toFloat64(1)) AS String) AS state); -- { serverError CANNOT_READ_ALL_DATA }
SELECT length(finalizeAggregation(CAST(substring(state, 1, 26) || unhex('FFFFFFFFFFFFFFFF') || substring(state, 35) AS AggregateFunction(timeSeriesDerivToGrid(10, 120, 10, 70), UInt32, Float64))))
FROM (SELECT CAST(timeSeriesDerivToGridState(10, 120, 10, 70)(toUInt32(55), toFloat64(1)) AS String) AS state); -- { serverError CANNOT_READ_ALL_DATA }

-- The number of buckets of the outer state is only checked against the bucket count derived from the function
-- parameters, and a huge grid makes that count enormous, so the buckets must not be reserved before they are
-- read. The memory limit keeps a state claiming all of them from silently allocating gigabytes.
SELECT length(finalizeAggregation(CAST(substring(state, 1, 10) || substring(state, 3, 8) AS AggregateFunction(timeSeriesChangesToGrid(0, 2147483648, 1, 1), UInt32, Float64))))
FROM (SELECT CAST(timeSeriesChangesToGridState(0, 2147483648, 1, 1)(toUInt32(0), toFloat64(1)) AS String) AS state)
