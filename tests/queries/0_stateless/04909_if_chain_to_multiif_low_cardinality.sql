...
-- The legacy AST optimizer (`OptimizeIfChainsVisitor`) rewrites the same `if` chains, but purely
-- syntactically: the synthesized `multiIf` is resolved from the query settings and would return
-- `LowCardinality(String)`, while the `if` chain returns plain `String`. The legacy rewrite is
-- therefore disabled while `optimize_if_transform_const_strings_to_lowcardinality` is enabled
-- (see `TreeRewriter.cpp`). That path cannot be exercised from a test any more, because
-- `enable_analyzer = 0` is rejected: the analyzer is mandatory since v26.9.
