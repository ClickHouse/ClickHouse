...
# Truncated Arrow / ArrowStream inputs must surface a clear, specific error code rather than a
...
# its own ClickHouse error codes for truncation/corruption.
# See also 04335_arrow_memory_limit_error_code for surfacing the memory-limit code.
...
rm -f "$DATA_PREFIX".{Arrow,ArrowStream}{,.truncated}
