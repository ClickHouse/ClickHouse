...
--
-- `enable_analyzer = 1` is pinned on both outer queries (like 04490): the scenarios use
...
-- by 04489, which is not analyzer-pinned.
...
SETTINGS log_comment = '04491_parallel_hash', enable_analyzer = 1;
SYSTEM FLUSH LOGS query_log;
SELECT ProfileEvents['JoinInMemoryCompressedColumns'] > 0 FROM system.query_log
WHERE current_database = currentDatabase() AND log_comment = '04491_parallel_hash' AND type = 'QueryFinish'
...
SETTINGS log_comment = '04491_grace_hash', enable_analyzer = 1;
SYSTEM FLUSH LOGS query_log;
SELECT ProfileEvents['JoinInMemoryCompressedColumns'] > 0 FROM system.query_log
WHERE current_database = currentDatabase() AND log_comment = '04491_grace_hash' AND type = 'QueryFinish'
ORDER BY event_time_microseconds DESC LIMIT 1;
