...
-- shape, each varying a single part of that shape: no `arguments` (under a name that is not `lambda`,
-- then under `lambda`), one argument, a first argument that is not a tuple, an argument tuple without
-- its own argument list, an otherwise well-formed lambda whose name is not `lambda`, three arguments,
-- and the `APPLY` transformer child (whose own boundary check accepts a two-child lambda whose first
