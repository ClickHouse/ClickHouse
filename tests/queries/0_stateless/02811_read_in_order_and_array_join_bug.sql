...
from numbers(1000);
create view v4test_array_joins as SELECT * from default.test_array_joins where id != 10;
select * from v4test_array_joins array join columns('^arr') where match(arr_4,'a') and id < 100 order by id format Null settings optimize_read_in_order = 1;
