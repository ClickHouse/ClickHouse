...
${CLICKHOUSE_CURL} -sS -H "Accept-Encoding: zstd" "${BASE_URL}/play" | zstd -d | grep -o -F -m1 '<title>ClickHouse Query</title>'

echo "--- test 9: gzip response is valid and decompressable ---"
${CLICKHOUSE_CURL} -sS -H "Accept-Encoding: gzip" "${BASE_URL}/play" | gzip -d | grep -o -F -m1 '<title>ClickHouse Query</title>'
