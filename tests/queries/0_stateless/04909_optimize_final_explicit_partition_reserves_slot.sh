...
$CLICKHOUSE_CLIENT --receive_timeout 900 --query "OPTIMIZE TABLE t_optimize_explicit_partition_slot PARTITION ID 'all' FINAL" &
optimize_pid=$!
...
while (( SECONDS < deadline )); do
    state=$($CLICKHOUSE_CLIENT --query "
        SELECT
            (SELECT count() FROM system.merges WHERE database = currentDatabase() AND table = 't_optimize_explicit_partition_slot'),
