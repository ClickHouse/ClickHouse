...
-- The `Distinct` above the `INTERSECT DISTINCT` set-operation step is bounded by the smallest
-- input (3000000, the dim side) even when the distinct key `k + v` has no NDV statistics; without
-- the bound the estimate would fall back to 10% of the 600000000-row left input. The join rewrite
-- of `INTERSECT DISTINCT` is turned off: a semi join on a key without statistics keeps the left
-- estimate, which is exactly the fallback.
SELECT 'intersect distinct bound:', countIf(explain LIKE '%Distinct%(rows: ~3000000.0%') > 0 FROM (
    ...
    SETTINGS make_distributed_plan = 1, enable_cascades_optimizer = 1, distributed_plan_execute_locally = 1,
        optimize_rewrite_intersect_except_to_join = 0
) SETTINGS make_distributed_plan = 0, enable_cascades_optimizer = 0;
