...
-- `compatibility` below 26.10) restores the previous behavior, where those rows were aggregated as ordinary values.
...
SELECT 'compatibility 26.9', count(v), uniqExact(v), any(v), length(groupArray(v)) FROM t_variant_compat;
SET compatibility = '';
