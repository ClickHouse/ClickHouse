...
# Save the parse errors synchronously via a direct SELECT instead of a background
# materialized view: a single read consumes every file at once, so there is no
# FileLog watcher thread to wait on and the test is fast and parallel-safe.
${CLICKHOUSE_CLIENT} --query "create table log_errors engine=MergeTree order by tuple() as select _error as error, _raw_record as record, _filename as file from file_log where not isNull(_error) settings stream_like_engine_allow_direct_select = 1;"
