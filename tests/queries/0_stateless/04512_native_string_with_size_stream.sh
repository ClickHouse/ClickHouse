...
native_revision=$(${CLICKHOUSE_CURL} -sS "${CLICKHOUSE_URL}&client_protocol_version=54491&query=SELECT+'abc'+AS+s+FORMAT+Native" | od -An -v -tx1 | tr -d ' \n')
...
buffers_revision=$(${CLICKHOUSE_CURL} -sS "${CLICKHOUSE_URL}&client_protocol_version=54491&query=SELECT+'abc'+AS+s+FORMAT+Buffers" | od -An -v -tx1 | tr -d ' \n')
