...
# The HTTP interface, the pinning of parse-time settings for the server-side reparse and the
# streaming of external data are covered by 05054_polyglot_insert_values_http, kept separate so
# that neither test exceeds the per-run time limit of the flaky check.
