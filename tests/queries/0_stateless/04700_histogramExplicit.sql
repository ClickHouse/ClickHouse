...
SELECT finalizeAggregation(
    (SELECT histogramExplicitStateIf([0, 50, 100], 'zero', 'null', 'nan')(value, ifNull(value > 3, 0))
     FROM (SELECT arrayJoin([NULL, toNullable(0.0), cast('nan', 'Nullable(Float64)'), cast('nan', 'Nullable(Float64)'), toNullable(-100.0), toNullable(25.0), toNullable(75.0), cast('inf', 'Nullable(Float64)')]) AS value))
    +
    (SELECT histogramExplicitStateIf([50, 0, 100], 'zero', 'null', 'nan')(value, ifNull(value > 3, 0))
     FROM (SELECT arrayJoin([NULL, NULL, toNullable(-0.0), toNullable(0.0), cast('nan', 'Nullable(Float64)'), cast('-inf', 'Nullable(Float64)'), toNullable(50.0), toNullable(100.0), toNullable(150.0)]) AS value))
);
...
SELECT histogramExplicitIf([0], 'null')(x, cond) 
FROM values('x Int32, cond UInt8', (-1, 1), (1, 1), (2, 0), (3, 0));
...
SELECT 
    histogramExplicitArray([0, 10], 'nan', 'null', 'zero')(
        arrayFilter(
            (x, mask) -> mask,
...
FROM values(
    'k UInt8, x Array(Nullable(Int32)), cond UInt8',
    (1, [1, NULL], 1),
    (1, [2, NULL], 0),
    (1, [3, NULL], 0)
)
...
FROM
    values('k UInt8, x Nullable(Int32), cond UInt8', 
            (1, null, 1), 
            (1, -1, 1), 
            (1, 5, 0), 
            (2, null, 0),
...
-- non-UInt8 GROUP BY ключ, addBatch()
SELECT k, histogramExplicit([0,10], 'null')(x)
FROM values('k Int64, x Nullable(Int64)', (1,NULL), (1,5), (2,NULL))
GROUP BY k
ORDER BY k;

SELECT histogramExplicitDistinct([0], 'null')(x)
FROM(SELECT arrayJoin([null, 1, 1, 2]) AS x); 

SELECT histogramExplicitOrNullDistinct([0, 10], 'null')(CAST(val AS Nullable(Int64)))
FROM VALUES('val Nullable(Int64)', (NULL), (NULL));
