...
# `execution_time` from the limits transform, `written_bytes` from the counting transform) were
# accounted against the shared per-user intervals, so they behaved as a single global bucket
# regardless of the query pattern. This test checks that two structurally different queries (which
# have different normalized hashes) get independent buckets.
#
