...
$CLICKHOUSE_CLIENT -q "ATTACH TABLE test_attach_reject UUID '${UUID}' (\`n\` Tuple(a UInt32, b UInt32)) ENGINE = SummingMergeTree ORDER BY n SETTINGS allow_tuple_element_aggregation = 1;" 2>&1 | grep -m 1 -o -F 'BAD_ARGUMENTS'
