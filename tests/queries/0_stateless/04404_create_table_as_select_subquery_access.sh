...
${CLICKHOUSE_CLIENT} --user "${user}" --password "${user}" --query "${query}" 2>&1 | grep -Fo "ACCESS_DENIED" | uniq
...
echo "-- once SELECT on t2 is granted, the query succeeds and populates the table:"
${CLICKHOUSE_CLIENT} --query "GRANT SELECT ON ${CLICKHOUSE_DATABASE}.t2 TO ${user}"
${CLICKHOUSE_CLIENT} --user "${user}" --password "${user}" --query "${query}"
