#!/usr/bin/env bash
# Tags: no-parallel
# no-parallel: the test asserts per-query ProfileEvents counts of the access-rights recalculations
# triggered by a GRANT. Other tests running in parallel on the same server change access entities
# (users, roles, quotas) concurrently, and `AccessChangesNotifier` delivers all queued changes in
# the thread of whichever query calls `sendNotifications()`, so a change queued by one query can be
# processed by another query's thread. That moves the recalculations (and their profile events)
# between queries and falsifies the asserted bounds. The events do not record which entity triggered
# a recalculation, so the test cannot filter foreign recalculations out.
