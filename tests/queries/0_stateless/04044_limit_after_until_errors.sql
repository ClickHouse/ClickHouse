-- LIMIT WITH TIES is not supported together with AFTER (the analyzer).
...
-- Non-boolean AFTER condition: String cannot be used in boolean context (the analyzer).
...
-- Non-boolean UNTIL condition: String (the analyzer).
