...
PIVOT (sum(v) FOR k.x IN ('a' AS a)); -- { clientError SYNTAX_ERROR }
...
SELECT * FROM values('k String, v UInt64', ('a', 1))
PIVOT (sum(v), count() FOR k IN ('a' AS a)); -- { clientError SYNTAX_ERROR }

SELECT * FROM values('k UInt64, v UInt64', (1, 1))
PIVOT (sum(v) FOR k IN ((1 + 0) AS one)); -- { clientError SYNTAX_ERROR }

SELECT * FROM values('k String, v UInt64', ('a', 1))
PIVOT (sum(v) FOR k IN ('a' AS same, 'a' AS same)); -- { clientError SYNTAX_ERROR }

SELECT * FROM values('k String, v UInt64', ('a', 1))
PIVOT (sum(arrayMap(x -> x, [v])) FOR k IN ('a' AS a)); -- { clientError SYNTAX_ERROR }

SELECT * FROM values('k String, v UInt64', ('a', 1))
PIVOT (sum(v) OVER () FOR k IN ('a' AS a)); -- { clientError SYNTAX_ERROR }
