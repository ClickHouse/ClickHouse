-- Tags: no-fasttest
-- Big integers (exceeding the 64-bit range) in JSON are stored by simdjson as raw digit strings
-- (parser option `number_as_string`) so ClickHouse can parse them into wider integer types.
...
SELECT '-- Big integer into String (stored as string)';
...
SELECT '-- JSONExtractString/Float with big integers (issue #49895)';
SELECT
    JSONExtractString('{ "a": -57783588950605040000 }', 'a'),
    JSONExtractFloat('{ "a": -57783588950605040000 }', 'a');

SELECT '-- Invalid JSON: big integer followed by a non-structural character must be rejected';
...
SELECT isValidJSON('{"a":123456789123456789123}'); -- valid: a structural "}" follows the digits
