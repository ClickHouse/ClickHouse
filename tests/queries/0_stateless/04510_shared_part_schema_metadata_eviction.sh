...
${CLICKHOUSE_CLIENT} --query "CREATE TABLE t_evict_plain (key Int) ENGINE = MergeTree ORDER BY key SETTINGS old_parts_lifetime = 480"
...
${CLICKHOUSE_CLIENT} --query "CREATE TABLE t_evict_nested (key Int, \`n.a\` Array(Int32), \`n.b\` Array(String)) ENGINE = MergeTree ORDER BY key SETTINGS old_parts_lifetime = 1"
...
${CLICKHOUSE_CLIENT} --query "CREATE TABLE t_evict_nested_noshare (key Int, \`n.a\` Array(Int32), \`n.b\` Array(String)) ENGINE = MergeTree ORDER BY key SETTINGS share_nested_offsets = 0, old_parts_lifetime = 1"
