-- The AND comparison chain optimization prunes always-true comparisons. When one operand is left it
-- replaces the whole AND, so it must still evaluate that operand as a boolean, and it must leave a
-- GROUP BY key alone when the key and its copies elsewhere in the query are typed differently.
...
-- Such a survivor keeps the plan it has today: no AND is left around it, and the same probe returns
-- one when the optimization does not run, so its absence is what the first row asserts.
SELECT count() > 0 FROM (EXPLAIN QUERY TREE run_passes = 1
...
-- Keeping the AND is also what keeps a skip index reachable: the index builders resolve a key column
-- from an atom's own arguments, so an atom that ends up inside another function is dropped and the
-- index is not consulted at all.
...
-- What is left alone is a query level, so a chain at a nested level is still optimized. The outer
-- key of this query is the one the row above asserts is not, so the indexHint here is the nested
-- level's own.
SELECT count() > 0 FROM (EXPLAIN SYNTAX run_query_tree_passes = 1
