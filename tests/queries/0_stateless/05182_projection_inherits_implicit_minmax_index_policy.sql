...
-- The plan lines are extracted rather than trimmed: with parallel replicas the read is nested one level
-- deeper and carries a `│` tree prefix.
...
SELECT extract(explain, 'ReadFromMergeTree \\(.*\\)|Name: auto_minmax_index.*') FROM (EXPLAIN indexes = 1 SELECT count() FROM t_proj_minmax_off WHERE s = '777' AND c = 1554 SETTINGS preferred_optimize_projection_name = 'p_inherit')
WHERE explain LIKE '%ReadFromMergeTree (%' OR explain LIKE '%Name: auto_minmax_index%';

SELECT 'table off, projection explicitly on';
SELECT extract(explain, 'ReadFromMergeTree \\(.*\\)|Name: auto_minmax_index.*') FROM (EXPLAIN indexes = 1 SELECT count() FROM t_proj_minmax_off WHERE s = '777' AND c = 1554 SETTINGS preferred_optimize_projection_name = 'p_explicit_on')
WHERE explain LIKE '%ReadFromMergeTree (%' OR explain LIKE '%Name: auto_minmax_index%';
...
SELECT extract(explain, 'ReadFromMergeTree \\(.*\\)|Name: auto_minmax_index.*') FROM (EXPLAIN indexes = 1 SELECT count() FROM t_proj_minmax_on WHERE s = '777' AND c = 1554 SETTINGS preferred_optimize_projection_name = 'p_inherit')
WHERE explain LIKE '%ReadFromMergeTree (%' OR explain LIKE '%Name: auto_minmax_index%';

SELECT 'table on, projection explicitly off';
SELECT extract(explain, 'ReadFromMergeTree \\(.*\\)|Name: auto_minmax_index.*') FROM (EXPLAIN indexes = 1 SELECT count() FROM t_proj_minmax_on WHERE s = '777' AND c = 1554 SETTINGS preferred_optimize_projection_name = 'p_explicit_off')
WHERE explain LIKE '%ReadFromMergeTree (%' OR explain LIKE '%Name: auto_minmax_index%';
