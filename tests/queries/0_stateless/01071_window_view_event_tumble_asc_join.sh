...
CREATE WINDOW VIEW ${db}.wv TO ${db}.dst WATERMARK=ASCENDING AS SELECT count(a) AS count, sum(${db}.info.value) as sum, tumbleEnd(wid) AS w_end FROM ${db}.mt JOIN ${db}.info ON ${db}.mt.a = ${db}.info.key GROUP BY tumble(timestamp, INTERVAL '5' SECOND, 'US/Samoa') AS wid;
