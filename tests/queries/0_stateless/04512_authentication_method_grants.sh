...
# All administrative statements go over HTTP: spawning the clickhouse-client binary for each of the
# dozens of statements dominates the test's runtime under sanitizers (each start costs seconds).
function admin()
{
...
function cleanup()
{
    admin "DROP USER IF EXISTS ${user}, ${user2}, ${user3}, ${user4}, ${user5}, ${user6}, ${user7}, ${user8}, ${user9}, ${user10}, ${user11}, ${victim}"
    admin "DROP ROLE IF EXISTS ${role}, ${role2}, ${role2}_x, ${role3}"
}
trap cleanup EXIT
...
admin "CREATE USER ${user} IDENTIFIED WITH plaintext_password BY 'full_password', plaintext_password BY 'token_password' VALID UNTIL '2077-01-01' GRANTS (SELECT ON t1, INSERT ON t1)"
admin "CREATE ROLE ${role}"
admin "GRANT SELECT ON ${CLICKHOUSE_DATABASE}.t1 TO ${user}"
admin "GRANT SELECT ON ${CLICKHOUSE_DATABASE}.t2 TO ${role}"
admin "GRANT ${role} TO ${user}"
...
admin "SELECT arrayMap(x -> replaceAll(x, currentDatabase(), 'db'), auth_grants) FROM system.users WHERE name = '${user}'"
...
echo "-- The query result cache must not serve a token results cached under a broader credential of the same user"
...
# rather than being served cached rows without an access check.
login_expect_error "token_password" "ACCESS_DENIED" "SELECT x FROM t2 SETTINGS use_query_cache = 1, query_cache_min_query_runs = 0"

echo "-- A credential-limited session does not use the query result cache, so a later REVOKE is enforced"
...
admin "CREATE USER ${user}_bad IDENTIFIED WITH plaintext_password BY '1' GRANTS ()" 2>&1 | grep -m1 -o "SYNTAX_ERROR" | head -n 1
admin "CREATE USER ${user}_bad IDENTIFIED WITH plaintext_password BY '1' GRANTS SELECT ON t1" 2>&1 | grep -m1 -o "SYNTAX_ERROR" | head -n 1
...
admin "CREATE USER ${user2} IDENTIFIED WITH plaintext_password BY 'full2', plaintext_password BY 'denyall' GRANTS (USAGE ON *.*)"
admin "GRANT SELECT ON ${CLICKHOUSE_DATABASE}.t1 TO ${user2}"
...
admin "SHOW CREATE USER ${user2}" | sed "s/${user2}/user2/g"

echo "-- system.users exposes USAGE ON *.* for the deny-all method"
admin "SELECT auth_grants FROM system.users WHERE name = '${user2}'"
...
admin "CREATE USER ${user}_bad IDENTIFIED WITH plaintext_password BY '1' GRANTS (READ ON S3('s3://bucket/private/.*'))" 2>&1 | grep -m1 -o "NOT_IMPLEMENTED" | head -n 1

echo "-- A filtered source grant in the GRANTS clause is rejected (ALTER USER)"
admin "ALTER USER ${user} ADD IDENTIFIED WITH plaintext_password BY 'filtered_token' GRANTS (READ ON S3('s3://bucket/private/.*'))" 2>&1 | grep -m1 -o "NOT_IMPLEMENTED" | head -n 1
...
admin "CREATE USER ${user}_bad IDENTIFIED WITH ldap SERVER 'srv' GRANTS (SELECT ON t1)" 2>&1 | grep -m1 -o "NOT_IMPLEMENTED" | head -n 1

echo "-- A GRANTS clause on an externally verified authentication method is rejected (CREATE USER, kerberos)"
admin "CREATE USER ${user}_bad IDENTIFIED WITH kerberos GRANTS (SELECT ON t1)" 2>&1 | grep -m1 -o "NOT_IMPLEMENTED" | head -n 1

echo "-- A GRANTS clause on an externally verified authentication method is rejected (ALTER USER, http)"
admin "ALTER USER ${user} ADD IDENTIFIED WITH http SERVER 'srv' SCHEME 'basic' GRANTS (SELECT ON t1)" 2>&1 | grep -m1 -o "NOT_IMPLEMENTED" | head -n 1
...
admin "CREATE USER ${user}_bad IDENTIFIED WITH plaintext_password BY '1' GRANTS (CREATE TEMPORARY TABLE ON t1)" 2>&1 | grep -m1 -o "INVALID_GRANT" | head -n 1

echo "-- A non-grantable element in the GRANTS clause is rejected (ALTER USER, global privilege on a database)"
admin "ALTER USER ${user} ADD IDENTIFIED WITH plaintext_password BY 'bad_token' GRANTS (KILL QUERY ON *)" 2>&1 | grep -m1 -o "INVALID_GRANT" | head -n 1
...
admin "CREATE USER ${user3} IDENTIFIED WITH plaintext_password BY 'full3' GRANTS (READ ON S3)"
admin "SHOW CREATE USER ${user3}" | sed "s/${user3}/user3/g"
admin "SELECT auth_grants FROM system.users WHERE name = '${user3}'"
...
admin "CREATE USER ${user4} IDENTIFIED WITH plaintext_password BY 'full4', plaintext_password BY 'role_token' GRANTS (CREATE ROLE ON *.*, ALTER ROLE ON *.*, DROP ROLE ON *.*)"
admin "GRANT CREATE ROLE, ALTER ROLE, DROP ROLE ON *.* TO ${user4}"
...
admin "CREATE USER ${user5} IDENTIFIED WITH plaintext_password BY 'full5', plaintext_password BY 'token5' GRANTS (SELECT ON t1)"
...
admin "GRANT SELECT ON ${CLICKHOUSE_DATABASE}.t1 TO ${user5}"
admin "GRANT CREATE TEMPORARY TABLE ON *.* TO ${user5}"
admin "GRANT TABLE ENGINE ON Memory TO ${user5}"
...
admin "CREATE USER ${user6} IDENTIFIED WITH sha256_password BY 'shared6', sha256_password BY 'shared6' GRANTS (SELECT ON t1)"
admin "GRANT SELECT ON ${CLICKHOUSE_DATABASE}.t1 TO ${user6}"
admin "GRANT SELECT ON ${CLICKHOUSE_DATABASE}.t2 TO ${user6}"
...
admin "CREATE USER ${user7} IDENTIFIED WITH sha256_password BY 'shared7' GRANTS (SELECT ON t1), sha256_password BY 'shared7' GRANTS (SELECT ON t2)"
admin "GRANT SELECT ON ${CLICKHOUSE_DATABASE}.t1 TO ${user7}"
admin "GRANT SELECT ON ${CLICKHOUSE_DATABASE}.t2 TO ${user7}"
...
admin "CREATE USER ${user8} IDENTIFIED WITH sha256_password BY 'shared8' VALID UNTIL '2000-01-01', sha256_password BY 'shared8' VALID UNTIL '2099-01-01', sha256_password BY 'other8' VALID UNTIL '2099-01-01'"
admin "GRANT SELECT ON ${CLICKHOUSE_DATABASE}.t1 TO ${user8}"
...
admin "CREATE USER ${user9} IDENTIFIED WITH sha256_password BY 'shared9' VALID UNTIL '2000-01-01' GRANTS (SELECT ON t1), sha256_password BY 'shared9'"
admin "GRANT SELECT ON ${CLICKHOUSE_DATABASE}.t1 TO ${user9}"
...
admin "CREATE USER ${user10} IDENTIFIED WITH sha256_password BY 'shared10', ldap SERVER 'unused' VALID UNTIL '2000-01-01'"
admin "GRANT SELECT ON ${CLICKHOUSE_DATABASE}.t1 TO ${user10}"
...
admin "CREATE ROLE ${role3}"
admin "CREATE USER ${victim} IDENTIFIED WITH sha256_password BY 'victim_password'"
admin "GRANT ${role3} TO ${victim}"
admin "CREATE USER ${user11} IDENTIFIED WITH plaintext_password BY 'full11', plaintext_password BY 'user_token' GRANTS (ALTER USER ON *.*)"
admin "GRANT ALTER USER ON *.* TO ${user11}"
