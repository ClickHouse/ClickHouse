-- `Memory`, `Null` and `View` bound their actual source counts regardless of the requested
-- number of streams. A `Merge` read over such children must not reject an excessive stream
-- request with `PARAMETER_OUT_OF_BOUND`: the children report their bounds through
-- `getMaxReadStreams` and the read succeeds.
