...
CREATE MATERIALIZED VIEW test_mv_a Engine=ReplicatedMergeTree () order by tuple()  AS SELECT A, count() c FROM test group by A;

CREATE MATERIALIZED VIEW test_mv_b Engine=ReplicatedMergeTree () partition by A order by tuple()  AS SELECT A, count() c FROM test group by A;

CREATE MATERIALIZED VIEW test_mv_c Engine=ReplicatedMergeTree () order by tuple()  AS SELECT A, count() c FROM test group by A;
