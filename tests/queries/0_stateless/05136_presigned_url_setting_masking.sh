...
# quote along with the credential, e.g. `url_base = 'https://b/f.csv?X-Amz-Signature=[HIDDEN]`, and
...
check_profile "url_base = 'https://bucket.s3.amazonaws.com/f.csv?X-Amz-Credential=AKIAIOSFODNN7EXAMPLE&X-Amz-Signature=$CANARY'"

# 2. The credential is followed by a parameter that is not masked, which ends it before the quote.
check_profile "url_base = 'https://bucket.s3.amazonaws.com/f.csv?X-Amz-Signature=$CANARY&response-content-type=text/csv'"
