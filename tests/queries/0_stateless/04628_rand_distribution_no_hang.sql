-- Extreme distribution parameters used to spin forever inside the rejection loop of the standard
-- library sampler. Such a query could not be stopped by `max_execution_time` or `KILL QUERY`, and
-- constant folding ran into it already during query analysis.
-- `max_rand_distribution_parameter` and `max_rand_distribution_trials` bound the computation time and
-- are disabled by 0, but parameters outside the domain where the samplers terminate at all are always
-- rejected.
...
SELECT count() FROM (SELECT randBinomial(1099511627776, 0.999) FROM numbers(1));
