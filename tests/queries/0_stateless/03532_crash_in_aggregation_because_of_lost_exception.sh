...
# The reproducer is driven by `memory_tracker_fault_probability` and `throwIf`; the two
# `query_profiler_*` settings below are incidental leftovers of the fuzzed settings list. The sampling
# query profiler is not supported under MemorySanitizer, where a non-zero period is rejected with
# `NOT_IMPLEMENTED` - which would make every request fail on the setting instead of reproducing the
# lost exception - so drop only those two settings there.
QUERY_PROFILER_SETTINGS="query_profiler_real_time_period_ns = 25000000, query_profiler_cpu_time_period_ns = 25000000,"
