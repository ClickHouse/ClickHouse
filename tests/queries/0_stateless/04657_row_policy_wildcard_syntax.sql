...
CREATE ROW POLICY policy_04657 ON mydb*.* AS PERMISSIVE FOR SELECT USING 1 TO default; -- { serverError SYNTAX_ERROR }

-- Database prefix wildcard + specific table: also invalid
CREATE ROW POLICY policy_04657 ON mydb*.mytable AS PERMISSIVE FOR SELECT USING 1 TO default; -- { serverError SYNTAX_ERROR }
...
CREATE ROW POLICY policy_04657 ON mytable* AS PERMISSIVE FOR SELECT USING 1 TO default; -- { serverError SYNTAX_ERROR }

-- ALTER ROW POLICY with invalid patterns
ALTER ROW POLICY IF EXISTS policy_04657 ON mydb*.* TO default; -- { serverError SYNTAX_ERROR }
ALTER ROW POLICY IF EXISTS policy_04657 ON mytable* TO default; -- { serverError SYNTAX_ERROR }

-- DROP ROW POLICY with invalid patterns
DROP ROW POLICY IF EXISTS policy_04657 ON mydb*.*; -- { serverError SYNTAX_ERROR }
DROP ROW POLICY IF EXISTS policy_04657 ON mytable*; -- { serverError SYNTAX_ERROR }

-- SHOW POLICIES with invalid patterns
SHOW POLICIES ON mydb*.*; -- { serverError SYNTAX_ERROR }
SHOW POLICIES ON mytable*; -- { serverError SYNTAX_ERROR }

-- SHOW CREATE POLICY with invalid patterns
SHOW CREATE POLICY policy_04657 ON mydb*.*; -- { serverError SYNTAX_ERROR }
SHOW CREATE POLICY policy_04657 ON mytable*; -- { serverError SYNTAX_ERROR }
