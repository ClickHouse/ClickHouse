...
# new analyzer, the session setting alone must not disclose secrets
...
${CLICKHOUSE_CLIENT} --user "$user" --query "EXPLAIN actions = 1 SELECT * FROM $db.encrypted_view $old_settings, explain_query_plan_default = 'legacy'"
${CLICKHOUSE_CLIENT} --user "$user" --query "EXPLAIN header = 1 SELECT * FROM $db.encrypted_view $old_settings"
${CLICKHOUSE_CLIENT} --user "$user" --query "EXPLAIN ANALYZE actions = 1 SELECT * FROM $db.encrypted_view $old_settings"
${CLICKHOUSE_CLIENT} --user "$user" --query "EXPLAIN PIPELINE header = 1 SELECT * FROM $db.sorted_view $old_settings"
${CLICKHOUSE_CLIENT} --user "$user" --query "EXPLAIN PIPELINE SELECT * FROM $db.sorted_view $old_settings"
