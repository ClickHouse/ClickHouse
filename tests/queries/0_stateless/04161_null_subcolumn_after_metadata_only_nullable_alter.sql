...
-- Lazy materialization reads the subcolumn through MergeTreeLazilyReader, whose conversion order
-- differs from the readers chain. The `.null` map must still be derived from the converted parent.
DROP TABLE IF EXISTS t_null_sub_lazy;
CREATE TABLE t_null_sub_lazy (id UInt8, x UInt256)
ENGINE = MergeTree ORDER BY tuple();

INSERT INTO t_null_sub_lazy VALUES (1, 42), (2, 43);
ALTER TABLE t_null_sub_lazy MODIFY COLUMN x Nullable(UInt256) SETTINGS mutations_sync = 2;

SELECT 'lazy materialization';
SELECT id, x, x.null FROM t_null_sub_lazy ORDER BY id LIMIT 5
SETTINGS query_plan_optimize_lazy_materialization = 1, optimize_functions_to_subcolumns = 1;

DROP TABLE t_null_sub_lazy;

-- No-regression guard for the narrowed cast condition: the old block type still resolves `t.a`,
-- so it must be extracted directly and the whole tuple must NOT be cast (casting would throw on
-- the old `b` value). Memory engine keeps the pre-ALTER block.
