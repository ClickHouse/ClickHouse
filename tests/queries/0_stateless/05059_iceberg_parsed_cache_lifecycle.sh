...
# ATTACH with a full definition logs a server-side warning; deliver logs at
# fatal level for those statements so clickhouse-test does not fail on stderr.
# --allow_repeated_settings lets this override the runner-injected --send_logs_level.
QUIET_CLIENT="$CLICKHOUSE_CLIENT --allow_repeated_settings --send_logs_level=fatal"
...
EOF
$QUIET_CLIENT -q "ATTACH TABLE parsed_cache_reader UUID '$READER_UUID' (id UInt64) ENGINE=IcebergLocal('$TABLE_PATH')"
$CLICKHOUSE_CLIENT -q "SELECT sum(id) FROM parsed_cache_reader"
...
$CLICKHOUSE_CLIENT --use_iceberg_metadata_files_cache=0 --allow_repeated_settings --send_logs_level=fatal -q "ATTACH TABLE parsed_cache_disabled UUID '$DISABLED_UUID' (id UInt64) ENGINE=IcebergLocal('$TABLE_PATH')"
