...
def create_default_config(filename):
    ...
    if "kerberos_users.xml" in filename:
        contents = (
            "<clickhouse><users><kerberos_user><kerberos><realm>EXAMPLE.COM"
            "</realm></kerberos></kerberos_user></users></clickhouse>"
        )
    elif "kerberos.xml" in filename:
        contents = (
            "<clickhouse><kerberos><realm>EXAMPLE.COM</realm></kerberos></clickhouse>"
        )
...
def create_server_principal(self, node):
    ...
    try:
        node.cmd("echo pwd | kinit admin/admin")
        node.cmd(
            f'kadmin -w pwd -q "add_principal -randkey HTTP/kerberos_env_{node.name}_1.krbnet"'
        )
        node.cmd(
            f'kadmin -w pwd -q "ktadd -k /etc/krb5.keytab HTTP/kerberos_env_{node.name}_1.krbnet"'
        )
        yield
    finally:
        node.cmd("kdestroy")
