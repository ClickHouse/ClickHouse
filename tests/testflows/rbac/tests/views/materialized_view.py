...
def allow_experimental_alter_materialized_view_structure(node):
    ...
    try:
        with Given("I add allow_experimental_alter_materialized_view_structure to the default query settings"):
            default_query_settings = getsattr(current().context, "default_query_settings", [])
            default_query_settings.append(setting)
        yield
    finally:
        with Finally("I remove allow_experimental_alter_materialized_view_structure from the default query settings"):
...
def create_without_create_view_privilege(self, node=None):
    ...
    with user(node, f"{user_name}"):
        with When("I try to create a view without CREATE VIEW privilege as the user"):
            ...
            node.query(f"CREATE MATERIALIZED VIEW {view_name} ENGINE = Memory AS SELECT 1", settings = [("user", f"{user_name}")],
    ...
    with table(node, f"{table_name}"):
        with user(node, f"{user_name}"):
            ...
            with Then("I try to create a view without select privilege on the table"):
                node.query(f"CREATE MATERIALIZED VIEW {view_name} ENGINE = Memory AS SELECT * FROM {table_name}", settings = [("user", f"{user_name}")],
                    exitcode=exitcode, message=message)
    ...
    with table(node, f"{table0_name},{table1_name},{table2_name},{table3_name}"):
        try:
            with Given("I have some views"):
                node.query(f"CREATE MATERIALIZED VIEW {view0_name} ENGINE = Memory AS SELECT y FROM {table0_name}")
                node.query(f"CREATE MATERIALIZED VIEW {view1_name} ENGINE = Memory AS SELECT y FROM {table1_name} WHERE y IN (SELECT y FROM {view0_name} WHERE y<2)")
                node.query(f"CREATE MATERIALIZED VIEW {view2_name} ENGINE = Memory AS SELECT y FROM {table2_name} JOIN {view1_name} USING y")
...
def create_with_target_table_privilege_directly_or_via_role(self, node=None):
    ...
    if node is None:
        node = self.context.node
    with user(node, f"{user_name}"):
        Scenario(test=create_with_target_table,
            name="create with target table, privilege granted directly")(grant_target_name=user_name, user_name=user_name)

    with user(node, f"{user_name}"), role(node, f"{role_name}"):
        with When("I grant the role to the user"):
            node.query(f"GRANT {role_name} TO {user_name}")
        Scenario(test=create_with_target_table,
            name="create with target table, privilege granted through a role")(grant_target_name=role_name, user_name=user_name)
    ...
    with table(node, f"{table_name}"):
        try:
            with When("I grant CREATE VIEW privilege"):
                node.query(f"GRANT CREATE VIEW ON {view_name} TO {grant_target_name}")
            with Then("I attempt to create a view as the user"):
                node.query(f"CREATE MATERIALIZED VIEW {view_name} TO {table_name} AS SELECT 1",
                    settings = [("user", f"{user_name}")], exitcode=exitcode, message=message)
...
@TestScenario
def create_with_populate_source_table_privilege_granted_directly_or_via_role(self, node=None):
    ...
    if node is None:
        node = self.context.node
    with user(node, f"{user_name}"):
        Scenario(test=create_with_populate_source_table,
            name="create with populate and source table, privilege granted directly")(grant_target_name=user_name, user_name=user_name)

    with user(node, f"{user_name}"), role(node, f"{role_name}"):
        with When("I grant the role to the user"):
            node.query(f"GRANT {role_name} TO {user_name}")
        Scenario(test=create_with_populate_source_table,
            name="create with populate and source table, privilege granted through a role")(grant_target_name=role_name, user_name=user_name)
    ...
    with user(node, f"{user_name}"):
        try:
            with When("I have a view"):
                node.query(f"DROP VIEW IF EXISTS {view_name}")
                node.query(f"CREATE MATERIALIZED VIEW {view_name} ENGINE = Memory AS SELECT 1")
    ...
    with table(node, f"{table0_name},{table1_name},{table2_name}"):
        try:
            with Given("I have a view with a subquery"):
                node.query(f"DROP VIEW IF EXISTS {view_name}")
                node.query(f"CREATE MATERIALIZED VIEW {view_name} ENGINE = Memory AS SELECT * FROM {table0_name} WHERE y IN (SELECT y FROM {table1_name} WHERE y IN (SELECT y FROM {table2_name} WHERE y<2))")
    ...
    with table(node, f"{table0_name},{table1_name}"):
        try:
            with Given("I have a view with a JOIN statement"):
                node.query(f"DROP VIEW IF EXISTS {view_name}")
                node.query(f"CREATE MATERIALIZED VIEW {view_name} ENGINE = Memory AS SELECT * FROM {table0_name} JOIN {table1_name} USING d")
...
@TestScenario
def select_with_privilege_granted_directly_or_via_role_without_target_table_privilege(self, node=None):
    ...
    with user(node, f"{user_name}"):
        Scenario(test=select_without_target_table_privilege,
            name="select without target table privilege, privilege granted directly")(grant_target_name=user_name, user_name=user_name)

    with user(node, f"{user_name}"), role(node, f"{role_name}"):
        with When("I grant the role to the user"):
            node.query(f"GRANT {role_name} TO {user_name}")
        Scenario(test=select_without_target_table_privilege,
            name="select without target table privilege, privilege granted through a role")(grant_target_name=role_name, user_name=user_name)

@TestOutline
def select_without_target_table_privilege(self, grant_target_name, user_name, node=None):
...
@TestScenario
@Requirements(
    RQ_SRS_006_RBAC_MaterializedView_Select_TargetTable("1.0")
)
def select_from_implicit_target_table_privilege_granted_directly_or_via_role(self, node=None):
    ...
    with user(node, f"{user_name}"):
        Scenario(test=select_from_implicit_target_table,
            name="select from implicit target table, privilege granted directly")(grant_target_name=user_name, user_name=user_name)

    with user(node, f"{user_name}"), role(node, f"{role_name}"):
        with When("I grant the role to the user"):
            node.query(f"GRANT {role_name} TO {user_name}")
        Scenario(test=select_from_implicit_target_table,
            name="select from implicit target table, privilege granted through a role")(grant_target_name=role_name, user_name=user_name)
...
def select_from_explicit_target_table_privilege_granted_directly_or_via_role(self, node=None):
    ...
    if node is None:
        node = self.context.node
    with user(node, f"{user_name}"):
        Scenario(test=select_from_explicit_target_table,
            name="select from explicit target table, privilege granted directly")(grant_target_name=user_name, user_name=user_name)

    with user(node, f"{user_name}"), role(node, f"{role_name}"):
        with When("I grant the role to the user"):
            node.query(f"GRANT {role_name} TO {user_name}")
        Scenario(test=select_from_explicit_target_table,
            name="select from explicit target table, privilege granted through a role")(grant_target_name=role_name, user_name=user_name)
...
@TestScenario
@Requirements(
    RQ_SRS_006_RBAC_MaterializedView_Select_SourceTable("1.0")
)
def select_from_source_table_privilege_granted_directly_or_via_role(self, node=None):
    ...
    if node is None:
        node = self.context.node
    with user(node, f"{user_name}"):
        Scenario(test=select_from_source_table,
            name="select from source table, privilege granted directly")(grant_target_name=user_name, user_name=user_name)

    with user(node, f"{user_name}"), role(node, f"{role_name}"):
        with When("I grant the role to the user"):
            node.query(f"GRANT {role_name} TO {user_name}")
        Scenario(test=select_from_source_table,
            name="select from source table, privilege granted through a role")(grant_target_name=role_name, user_name=user_name)
...
def modify_query_with_privilege_granted_directly_or_via_role(self, node=None):
    ...
    if node is None:
        node = self.context.node
    with user(node, f"{user_name}"):
        Scenario(test=modify_query_with_privilege,
            name="modify query privilege granted directly")(grant_target_name=user_name, user_name=user_name)

    with user(node, f"{user_name}"), role(node, f"{role_name}"):
        with When("I grant the role to the user"):
            node.query(f"GRANT {role_name} TO {user_name}")
        Scenario(test=modify_query_with_privilege,
            name="modify query privilege granted through a role")(grant_target_name=role_name, user_name=user_name)
...
def modify_query_with_privilege(self, grant_target_name, user_name, node=None):
    ...
    try:
        ...
        with When("I grant MODIFY QUERY privilege"):
            node.query(f"GRANT ALTER VIEW MODIFY QUERY ON {view_name} TO {grant_target_name}")

        with Then("I modify the view query as the user"):
            node.query(f"ALTER TABLE {view_name} MODIFY QUERY SELECT 2", settings = [("user",f"{user_name}")])
...
def modify_query_with_privilege_revoked_directly_or_from_role(self, node=None):
    ...
    if node is None:
        node = self.context.node
    with user(node, f"{user_name}"):
        Scenario(test=modify_query_with_revoked_privilege,
            name="modify query privilege revoked directly")(grant_target_name=user_name, user_name=user_name)

    with user(node, f"{user_name}"), role(node, f"{role_name}"):
        with When("I grant the role to the user"):
            node.query(f"GRANT {role_name} TO {user_name}")
        Scenario(test=modify_query_with_revoked_privilege,
            name="modify query privilege revoked from a role")(grant_target_name=role_name, user_name=user_name)
...
def modify_query_with_revoked_privilege(self, grant_target_name, user_name, node=None):
    ...
    try:
        ...
        with And("I revoke MODIFY QUERY privilege"):
            node.query(f"REVOKE ALTER VIEW MODIFY QUERY ON {view_name} FROM {grant_target_name}")
         ...
         with user(node, f"{user_name}"):
            try:
                ...
                with And("I grant view MODIFY QUERY privilege to the user"):
                    node.query(f"GRANT ALTER VIEW MODIFY QUERY ON {view_name} TO {user_name}")
                with Then("I attempt to use MODIFY QUERY on the view without privilege on the source table"):
                    node.query(f"ALTER TABLE {view_name} MODIFY QUERY SELECT * FROM {table_name}", settings = [("user",f"{user_name}")],
                        exitcode=exitcode, message=message)
...
@TestScenario
def modify_query_with_source_table_privilege_granted_directly_or_via_role(self, node=None):
    ...
    if node is None:
        node = self.context.node
    with user(node, f"{user_name}"):
        Scenario(test=modify_query_with_source_table_privilege,
            name="modify query with source table, privilege granted directly")(grant_target_name=user_name, user_name=user_name)

    with user(node, f"{user_name}"), role(node, f"{role_name}"):
        with When("I grant the role to the user"):
            node.query(f"GRANT {role_name} TO {user_name}")
        Scenario(test=modify_query_with_source_table_privilege,
            name="modify query with source table, privilege granted through a role")(grant_target_name=role_name, user_name=user_name)

@TestOutline
def modify_query_with_source_table_privilege(self, user_name, grant_target_name, node=None):
...
def modify_query_with_subquery_privilege_granted_directly_or_via_role(self, node=None):
    ...
    with user(node, f"{user_name}"):
        Scenario(test=modify_query_with_subquery,
            name="modify query with subquery, privilege granted directly")(grant_target_name=user_name, user_name=user_name)

    with user(node, f"{user_name}"), role(node, f"{role_name}"):
        with When("I grant the role to the user"):
            node.query(f"GRANT {role_name} TO {user_name}")
        Scenario(test=modify_query_with_subquery,
            name="modify query with subquery, privilege granted through a role")(grant_target_name=role_name, user_name=user_name)
    ...
    with table(node, f"{table0_name},{table1_name},{table2_name}"):
        try:
            ...
            with When("I grant select on all tables"):
                with grant_select_on_table(node, max(permutations(table_count=3))+1, grant_target_name, table0_name, table1_name, table2_name):
                    with Then("I attempt to modify the view query as the user"):
                        output = node.query(modify_query_view_query.format(view_name=view_name, table0_name=table0_name, table1_name=table1_name, table2_name=table2_name), settings = [("user", f"{user_name}")]).output
                        assert output == '0', error()
...
@TestScenario
def modify_query_with_join_query_privilege_granted_directly_or_via_role(self, node=None):
    ...
    with user(node, f"{user_name}"):
        Scenario(test=modify_query_with_join_query,
            name="modify query with join, privilege granted directly")(grant_target_name=user_name, user_name=user_name)

    with user(node, f"{user_name}"), role(node, f"{role_name}"):
        with When("I grant the role to the user"):
            node.query(f"GRANT {role_name} TO {user_name}")
        Scenario(test=modify_query_with_join_query,
            name="modify query with join, privilege granted through a role")(grant_target_name=role_name, user_name=user_name)
    ...
    with table(node, f"{table0_name},{table1_name}"):
        try:
            ...
            with When("I grant select on all tables"):
                with grant_select_on_table(node, max(permutations(table_count=2))+1, grant_target_name, table0_name, table1_name):
                    with Then("I attempt to modify the view query as the user"):
                        node.query(modify_query_view_query.format(view_name=view_name, table0_name=table0_name, table1_name=table1_name), settings = [("user", f"{user_name}")])
...
@TestScenario
def modify_query_with_union_query_privilege_granted_directly_or_via_role(self, node=None):
    ...
    with user(node, f"{user_name}"):
        Scenario(test=modify_query_with_union_query,
            name="modify query with union, privilege granted directly")(grant_target_name=user_name, user_name=user_name)

    with user(node, f"{user_name}"), role(node, f"{role_name}"):
        with When("I grant the role to the user"):
            node.query(f"GRANT {role_name} TO {user_name}")
        Scenario(test=modify_query_with_union_query,
            name="modify query with union, privilege granted through a role")(grant_target_name=role_name, user_name=user_name)
...
@TestScenario
def modify_query_with_join_union_subquery_privilege_granted_directly_or_via_role(self, node=None):
    ...
    with user(node, f"{user_name}"):
        Scenario(test=modify_query_with_join_union_subquery,
            name="modify query with join union subquery, privilege granted directly")(grant_target_name=user_name, user_name=user_name)

    with user(node, f"{user_name}"), role(node, f"{role_name}"):
        with When("I grant the role to the user"):
            node.query(f"GRANT {role_name} TO {user_name}")
        Scenario(test=modify_query_with_join_union_subquery,
            name="modify query with join union subquery, privilege granted through a role")(grant_target_name=role_name, user_name=user_name)

@TestOutline
def modify_query_with_join_union_subquery(self, grant_target_name, user_name, node=None):
...
@TestScenario
def modify_query_with_nested_views_privilege_granted_directly_or_via_role(self, node=None):
    ...
    with user(node, f"{user_name}"):
        Scenario(test=modify_query_with_nested_views,
            name="modify query with nested views, privilege granted directly")(grant_target_name=user_name, user_name=user_name)

    with user(node, f"{user_name}"), role(node, f"{role_name}"):
        with When("I grant the role to the user"):
            node.query(f"GRANT {role_name} TO {user_name}")
        Scenario(test=modify_query_with_nested_views,
            name="modify query with nested views, privilege granted through a role")(grant_target_name=role_name, user_name=user_name)
...
def modify_query_with_nested_views(self, grant_target_name, user_name, node=None):
    ...
    with table(node, f"{table0_name},{table1_name},{table2_name},{table3_name}"):
        try:
            ...
            with When("I grant MODIFY QUERY privilege on view"):
                node.query(f"GRANT ALTER VIEW MODIFY QUERY ON {view3_name} TO {grant_target_name}")
            with Then("I attempt to modify the view query as the user"):
            ...
            for permutation in ([0,1,2,3,7,11,15,31,39,79,95],permutations(table_count=7))[self.context.stress]:
                with grant_select_on_table(node, permutation, grant_target_name, view2_name, table3_name, view1_name, table2_name, view0_name, table1_name, table0_name) as tables_granted:
                    with When(f"permutation={permutation}, tables granted = {tables_granted}"):
                        with Then("I attempt to modify the view query as the user"):
                            node.query(modify_query_view_query.format(view3_name=view3_name, view2_name=view2_name, table3_name=table3_name),
                                settings = [("user", f"{user_name}")], exitcode=exitcode, message=message)

            with When("I grant select on all views"):
                with grant_select_on_table(node, max(permutations(table_count=7))+1, grant_target_name, view0_name, view1_name, view2_name, table0_name, table1_name, table2_name, table3_name):
                    with Then("I attempt to modify the view query as the user"):
                        node.query(modify_query_view_query.format(view3_name=view3_name, view2_name=view2_name, table3_name=table3_name), settings = [("user", f"{user_name}")])
...
def insert_on_source_table_privilege_granted_directly_or_via_role(self, node=None):
    ...
    with user(node, f"{user_name}"):
        Scenario(test=insert_on_source_table,
            name="insert on source table, privilege granted directly")(grant_target_name=user_name, user_name=user_name)

    with user(node, f"{user_name}"), role(node, f"{role_name}"):
        with When("I grant the role to the user"):
            node.query(f"GRANT {role_name} TO {user_name}")
        Scenario(test=insert_on_source_table,
            name="insert on source table, privilege granted through a role")(grant_target_name=role_name, user_name=user_name)
...
def insert_on_source_table(self, grant_target_name, user_name, node=None):
    ...
    with table(node, f"{table0_name},{table1_name}"):
        try:
            with Given("I have a view"):
                node.query(f"DROP VIEW IF EXISTS {view_name}")
                node.query(f"CREATE MATERIALIZED VIEW {view_name} TO {table0_name} AS SELECT * FROM {table1_name}")
...
def insert_with_privilege_granted_directly_or_via_role(self, node=None):
    ...
    if node is None:
        node = self.context.node
    with user(node, f"{user_name}"):
        Scenario(test=insert_with_insert_privilege,
            name="insert on view, privilege granted directly")(grant_target_name=user_name, user_name=user_name)

    with user(node, f"{user_name}"), role(node, f"{role_name}"):
        with When("I grant the role to the user"):
            node.query(f"GRANT {role_name} TO {user_name}")
        Scenario(test=insert_with_insert_privilege,
            name="insert on view, privilege granted through a role")(grant_target_name=role_name, user_name=user_name)
...
def insert_on_target_table_privilege_granted_directly_or_via_role(self, node=None):
    ...
    with user(node, f"{user_name}"):
        Scenario(test=insert_on_target_table,
            name="insert on target table, privilege granted directly")(grant_target_name=user_name, user_name=user_name)

    with user(node, f"{user_name}"), role(node, f"{role_name}"):
        with When("I grant the role to the user"):
            node.query(f"GRANT {role_name} TO {user_name}")
        Scenario(test=insert_on_target_table,
            name="insert on target table, privilege granted through a role")(grant_target_name=role_name, user_name=user_name)
...
def insert_on_implicit_target_table_privilege_granted_directly_or_via_role(self, node=None):
    ...
    with user(node, f"{user_name}"):
        Scenario(test=insert_on_target_table,
            name="insert on implicit target table, privilege granted directly")(grant_target_name=user_name, user_name=user_name)

    with user(node, f"{user_name}"), role(node, f"{role_name}"):
        with When("I grant the role to the user"):
            node.query(f"GRANT {role_name} TO {user_name}")
        Scenario(test=insert_on_target_table,
            name="insert on implicit target table, privilege granted through a role")(grant_target_name=role_name, user_name=user_name)
    ...
    with Pool(3) as pool:
        try:
            for suite in loads(current_module(), Suite):
                Suite(run=suite, parallel=True, executor=pool)
        finally:
            join()
