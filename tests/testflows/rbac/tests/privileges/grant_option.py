...
def grant_option(self, table_type, privilege, node=None):
    ...
    with Suite("user with direct privilege granting to user"):
        with user(node, f"{user0_name},{user1_name}"):
            with When(
                f"I run checks that grant and revoke privilege from {user0_name} to {user1_name}"
            ):
                grant_option_check(
                    grant_option_target=user0_name,
                    grant_target=user1_name,
    ...
    with Suite("user with direct privilege granting to role"):
        with user(node, user0_name), role(node, role1_name):
            with When(
                f"I run checks that grant and revoke privilege from {user0_name} to {role1_name}"
            ):
                grant_option_check(
                    grant_option_target=user0_name,
                    grant_target=role1_name,
                    user_name=user0_name,
                    table_type=table_type,
                    privilege=privilege,
                    node=node,
    ...
    with Suite("user with privilege via role granting to user"):
        with user(node, f"{user0_name},{user1_name}"), role(node, role0_name):
            ...
            with When(
                f"I run checks that grant and revoke privilege from {user0_name} with {role0_name} to {user1_name}"
            ):
                grant_option_check(
                    grant_option_target=role0_name,
                    grant_target=user1_name,
                    user_name=user0_name,
                    table_type=table_type,
                    privilege=privilege,
    ...
    with Suite("user with privilege via role granting to role"):
        with user(node, user0_name), role(node, f"{role0_name},{role1_name}"):
            with When("I grant the role to the user"):
                node.query(f"GRANT {role0_name} TO {user0_name}")
            with When(
                f"I run checks that grant and revoke privilege from {user0_name} with {role0_name} to {role1_name}"
            ):
                grant_option_check(
                    grant_option_target=role0_name,
                    grant_target=role1_name,
...
def grant_option_check(
    grant_option_target, grant_target, user_name, table_type, privilege, node=None
):
    """Run different scenarios to check the user's access with different privileges."""
    exitcode, message = errors.not_enough_privileges(name=f"{user_name}")

    with Scenario(
        "grant by user without privilege", setup=instrument_clickhouse_server_log
    ):
        ...
        with table(node, name=table_name, table_type_name=table_type):
            with Then("I attempt to grant delete privilege without privilege"):
                node.query(
                    f"GRANT {privilege} ON {table_name} TO {grant_target}",
    ...
    with Scenario(
        "grant by user with grant option privilege",
        ...
        with table(node, name=table_name, table_type_name=table_type):
            with When("I grant delete privilege"):
                node.query(
                    f"GRANT {privilege} ON {table_name} TO {grant_option_target} WITH GRANT OPTION"
                )
    ...
    with Scenario(
        "revoke by user with grant option privilege",
        setup=instrument_clickhouse_server_log,
    ):
        table_name = f"merge_tree_{getuid()}"
        with table(node, name=table_name, table_type_name=table_type):
            ...
            with Then("I attempt to revoke delete privilege"):
                node.query(
                    f"REVOKE {privilege} ON {table_name} FROM {grant_target}",
                    settings=[("user", user_name)],
                )

    with Scenario(
        "grant by user with revoked grant option privilege",
        setup=instrument_clickhouse_server_log,
    ):
        table_name = f"merge_tree_{getuid()}"
        with table(node, name=table_name, table_type_name=table_type):
            with When(
                f"I grant delete privilege with grant option to {grant_option_target}"
            ):
                node.query(
                    f"GRANT {privilege} ON {table_name} TO {grant_option_target} WITH GRANT OPTION"
                )
            with And(
                f"I revoke delete privilege with grant option from {grant_option_target}"
            ):
                node.query(
                    f"REVOKE {privilege} ON {table_name} FROM {grant_option_target}"
                )
            with Then("I attempt to grant delete privilege"):
                node.query(
                    f"GRANT {privilege} ON {table_name} TO {grant_target}",
