...
def dict_privileges_granted_directly(self, node=None):
    ...
    with user(node, f"{user_name}"):
        ...
        Suite(
            run=check_privilege,
            examples=Examples(
                "privilege on grant_target_name user_name dict_name",
                [
                    tuple(list(row) + [user_name, user_name, dict_name])
    ...
    with user(node, f"{user_name}"), role(node, f"{role_name}"):
        ...
        Suite(
            ...
            examples=Examples(
                "privilege on grant_target_name user_name dict_name",
                [
                    tuple(list(row) + [role_name, user_name, dict_name])
                    for row in check_privilege.examples
                ],
                args=Args(name="check privilege={privilege}", format_name=True),
            ),
...
def check_privilege(
    self, privilege, on, grant_target_name, user_name, dict_name, node=None
):
