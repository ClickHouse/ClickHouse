...
def privilege_granted_directly_or_via_role(self, table_type, privilege, node=None):
    ...
    with Suite("user with privilege via role", setup=instrument_clickhouse_server_log):
        with user(node, user_name), role(node, role_name):
            ...
            with And(
                f"I run checks that {user_name} with {role_name} is only able to execute ALTER MOVE PARTITION with required privileges"
            ):
                privilege_check(
...
def privilege_check(grant_target_name, user_name, table_type, privilege, node=None):
    ...
    with Scenario("user without privilege", setup=instrument_clickhouse_server_log):
        ...
        with table(node, f"{source_table_name},{target_table_name}", table_type):
            ...
            with Then("I attempt to move partition without privilege"):
                node.query(
                    f"ALTER TABLE {source_table_name} MOVE PARTITION 1 TO TABLE {target_table_name}",
    ...
    with Scenario(
        "user without ALTER MOVE PARTITION privilege",
        ...
        with table(node, f"{source_table_name},{target_table_name}", table_type):
            ...
            with When(
                ...
                node.query(
                    f"GRANT SELECT, ALTER DELETE ON {source_table_name} TO {grant_target_name}"
                )

            with And(f"I grant INSERT on {target_table_name} to {grant_target_name}"):
                node.query(
                    f"GRANT INSERT ON {target_table_name} TO {grant_target_name}"
                )
    ...
    with Scenario(
        "user with ALTER MOVE PARTITION privilege",
        setup=instrument_clickhouse_server_log,
    ):
        source_table_name = f"source_merge_tree_{getuid()}"
        target_table_name = f"target_merge_tree_{getuid()}"
        ...
        with table(node, f"{source_table_name},{target_table_name}", table_type):
            ...
            with When(
                ...
                node.query(
                    f"GRANT SELECT, ALTER DELETE, {privilege} ON {source_table_name} TO {grant_target_name}"
                )
    ...
    with Scenario(
        "user with revoked ALTER MOVE PARTITION privilege",
        setup=instrument_clickhouse_server_log,
    ):
        source_table_name = f"source_merge_tree_{getuid()}"
        target_table_name = f"target_merge_tree_{getuid()}"
        ...
        with table(node, f"{source_table_name},{target_table_name}", table_type):
            ...
            with And("I revoke ALTER MOVE PARTITION privilege"):
                node.query(
                    f"REVOKE {privilege} ON {source_table_name} FROM {grant_target_name}"
                )
    ...
    with Scenario(
        "move partition to source table of a materialized view",
        setup=instrument_clickhouse_server_log,
    ):
        source_table_name = f"source_merge_tree_{getuid()}"
        ...
        with table(
            ...
            try:
                with Given("I have a materialized view"):
                    node.query(
                        f"CREATE MATERIALIZED VIEW {mat_view_name} ENGINE = {table_type} PARTITION BY y ORDER BY d AS SELECT * FROM {mat_view_source_table_name}"
                    )
                ...
                with And(
                    f"I grant INSERT on {mat_view_source_table_name} to {grant_target_name}"
                ):
                    node.query(
                        f"GRANT INSERT ON {mat_view_source_table_name} TO {grant_target_name}"
                    )

                with Then("I attempt to move partitions with ALTER MOVE privilege"):
                    node.query(
                        f"ALTER TABLE {source_table_name} MOVE PARTITION 1 TO TABLE {mat_view_source_table_name}",
                        settings=[("user", user_name)],
                    )
    ...
    with Scenario(
        "move partition to implicit target table of a materialized view",
        setup=instrument_clickhouse_server_log,
    ):
        source_table_name = f"source_merge_tree_{getuid()}"
        mat_view_name = f"mat_view_{getuid()}"
        ...
        with table(
            ...
            try:
                ...
                with And(
                    f"I grant INSERT on {implicit_table_name} to {grant_target_name}"
                ):
                    node.query(
                ...
                with Then("I attempt to move partitions with ALTER MOVE privilege"):
                    node.query(
                        f"ALTER TABLE {source_table_name} MOVE PARTITION 1 TO TABLE {implicit_table_name}",
                        settings=[("user", user_name)],
                    )
