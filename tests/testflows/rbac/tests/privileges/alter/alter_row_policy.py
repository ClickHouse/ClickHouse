        ...
        Suite(
            ...
            examples=Examples(
                ...
                [
                    ...
                    for row in alter_row_policy.examples
                ],
    ...
    with Scenario("ALTER ROW POLICY without privilege"):
        ...
        try:
            ...
            with Then("I check the user can't alter a row policy"):
                node.query(
                    f"ALTER ROW POLICY {alter_row_policy_name} ON {table_name}",
            ...
            with Finally("I drop the row policy"):
                node.query(
                    f"DROP ROW POLICY IF EXISTS {alter_row_policy_name} ON {table_name}"
                )
    ...
    with Scenario("ALTER ROW POLICY on cluster"):
        ...
        try:
            with Given("I have a row policy on a cluster"):
                node.query(
                    f"CREATE ROW POLICY {alter_row_policy_name} ON CLUSTER sharded_cluster ON {table_name}"
                )
            ...
            with Then("I check the user can alter a row policy"):
                node.query(
                    f"ALTER ROW POLICY {alter_row_policy_name} ON CLUSTER sharded_cluster ON {table_name}",
                    settings=[("user", f"{user_name}")],
                )
            ...
            with Finally("I drop the user"):
                node.query(
                    f"DROP ROW POLICY IF EXISTS {alter_row_policy_name} ON CLUSTER sharded_cluster ON {table_name}"
                )
...
def permissive(self, node=None):
    ...
    with table(node, table_name):
        ...
        with When("I alter a row policy to be permissive"):
            node.query(
                f"ALTER ROW POLICY {pol_name} ON {table_name} AS PERMISSIVE FOR SELECT USING y=1 TO default"
            )
...
@TestScenario
@Requirements(RQ_SRS_006_RBAC_RowPolicy_Alter_Access_Restrictive("1.0"))
def restrictive(self, node=None):
    ...
    with table(node, table_name):
        ...
        with And("I alter a row policy to be permissive"):
            node.query(
                f"ALTER ROW POLICY {perm_pol_name} ON {table_name} FOR SELECT USING y=1 OR y=2 TO default"
            )

        with And("I alter a row policy to be restrictive"):
            node.query(
                f"ALTER ROW POLICY {pol_name} ON {table_name} AS RESTRICTIVE FOR SELECT USING y=1 TO default"
            )
...
def for_select(self, node=None):
    ...
    with table(node, table_name):
        ...
        with Given("I alter therow policy to use FOR SELECT"):
            node.query(
                f"Alter ROW POLICY {pol_name} ON {table_name} FOR SELECT USING 1 TO default"
            )
...
@TestScenario
@Requirements(RQ_SRS_006_RBAC_RowPolicy_Alter_Condition("1.0"))
...
@TestScenario
@Requirements(RQ_SRS_006_RBAC_RowPolicy_Alter_Condition_None("1.0"))
...
@TestScenario
@Requirements(RQ_SRS_006_RBAC_RowPolicy_Alter_IfExists("1.0"))
def if_exists(self, node=None):
    ...
    with table(node, table_name):
        ...
        with When("I have alter a row policy to be permissive using IF EXISTS clause"):
            node.query(
                f"ALTER ROW POLICY IF EXISTS {pol_name} ON {table_name} FOR SELECT USING 1 TO default"
            )
...
def dict(self, node=None):
    ...
    try:
        ...
        with When("I alter the row policy to be permissive"):
            node.query(
                f"ALTER ROW POLICY {pol_name} ON {table_name} FOR SELECT USING key=1 TO default"
            )
