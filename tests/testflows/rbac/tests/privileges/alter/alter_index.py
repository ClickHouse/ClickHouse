...
def alter_index_privileges(grants: int):
    ...
    if grants >= permutation_count:
        ...
        if grants & subprivileges["MATERIALIZE INDEX"]:
            privileges.append(
                aliases["MATERIALIZE INDEX"][grants % len(aliases["MATERIALIZE INDEX"])]
            )
        if grants & subprivileges["CLEAR INDEX"]:
            privileges.append(
                aliases["CLEAR INDEX"][grants % len(aliases["CLEAR INDEX"])]
            )
        if grants & subprivileges["DROP INDEX"]:
            privileges.append(
                aliases["DROP INDEX"][grants % len(aliases["DROP INDEX"])]
            )
...
def check_order_by_when_privilege_is_granted(table, user, node):
    ...
    with And("I add new column and modify order using that column"):
        node.query(
            f"ALTER TABLE {table} ADD COLUMN {column} UInt32, MODIFY ORDER BY (b, {column})"
        )
...
def check_sample_by_when_privilege_is_granted(table, user, node):
    ...
    with When(f"I add sample by clause"):
        node.query(f"ALTER TABLE {table} MODIFY SAMPLE BY b", settings=[("user", user)])
...
def check_add_index_when_privilege_is_granted(table, user, node):
    ...
    with Given(f"I add index '{index}'"):  # Column x: String
        node.query(
            f"ALTER TABLE {table} ADD INDEX {index}(x) TYPE set(0) GRANULARITY 1",
            settings=[("user", user)],
        )
...
def check_materialize_index_when_privilege_is_granted(table, user, node):
    ...
    with When(f"I materialize index '{index}'"):
        node.query(
            f"ALTER TABLE {table} MATERIALIZE INDEX {index} IN PARTITION 1 SETTINGS mutations_sync = 2",
            settings=[("user", user)],
        )
...
def check_drop_index_when_privilege_is_granted(table, user, node):
    ...
    with When("I try to drop nonexistent index, throws exception"):
        ...
        node.query(
            f"ALTER TABLE {table} DROP INDEX fake_index",
            settings=[("user", user)],
            exitcode=exitcode,
            message=message,
        )
    ...
    with Then(f"I drop index {index} which exists"):
        node.query(f"ALTER TABLE {table} DROP INDEX {index}", settings=[("user", user)])
...
def check_order_by_when_privilege_is_not_granted(table, user, node):
    ...
    with When("I try to use privilege that has not been granted"):
        ...
        node.query(
            f"ALTER TABLE {table} MODIFY ORDER BY b",
            settings=[("user", user)],
            exitcode=exitcode,
            message=message,
        )
...
def check_sample_by_when_privilege_is_not_granted(table, user, node):
    ...
    with When("I try to use privilege that has not been granted"):
        exitcode, message = errors.not_enough_privileges(user)
        node.query(
            f"ALTER TABLE {table} MODIFY SAMPLE BY b",
            settings=[("user", user)],
            exitcode=exitcode,
            message=message,
        )
...
def check_add_index_when_privilege_is_not_granted(table, user, node):
    ...
    with When("I try to use privilege that has not been granted"):
        exitcode, message = errors.not_enough_privileges(user)
        node.query(
            f"ALTER TABLE {table} ADD INDEX index1 b * length(x) TYPE set(1000) GRANULARITY 4",
            settings=[("user", user)],
            exitcode=exitcode,
            message=message,
        )
...
def check_materialize_index_when_privilege_is_not_granted(table, user, node):
    ...
    with When("I try to use privilege that has not been granted"):
        exitcode, message = errors.not_enough_privileges(user)
        node.query(
            f"ALTER TABLE {table} MATERIALIZE INDEX index1",
            settings=[("user", user)],
            exitcode=exitcode,
            message=message,
        )
...
def check_clear_index_when_privilege_is_not_granted(table, user, node):
    ...
    with When("I try to use privilege that has not been granted"):
        exitcode, message = errors.not_enough_privileges(user)
        node.query(
            f"ALTER TABLE {table} CLEAR INDEX index1 IN PARTITION 1",
            settings=[("user", user)],
            exitcode=exitcode,
            message=message,
        )
...
def check_drop_index_when_privilege_is_not_granted(table, user, node):
    ...
    with When("I try to use privilege that has not been granted"):
        exitcode, message = errors.not_enough_privileges(user)
        node.query(
            f"ALTER TABLE {table} DROP INDEX index1",
            settings=[("user", user)],
            exitcode=exitcode,
            message=message,
        )
        ...
        with When(f"granted={privileges}"):
            with table(node, table_name, table_type):
                try:
                    ...
                    with When(
                        "I grant ALTER INDEX privileges needed for iteration on a cluster"
                    ):
                        node.query(
