        ...
        Suite(
            ...
            examples=Examples(
                ...
                [
                    ...
                    for row in drop_quota.examples
                ],
    ...
    with Scenario("DROP QUOTA without privilege"):
        ...
        try:
            ...
            with Then("I check the user can't drop a quota"):
                node.query(
                    f"DROP QUOTA {drop_row_policy_name}",
    ...
    with Scenario("DROP QUOTA on cluster"):
        ...
        try:
            with Given("I have a quota on a cluster"):
                node.query(
                    f"CREATE QUOTA {drop_row_policy_name} ON CLUSTER sharded_cluster"
                )
            ...
            with Finally("I drop the user"):
                node.query(
                    f"DROP QUOTA IF EXISTS {drop_row_policy_name} ON CLUSTER sharded_cluster"
                )
