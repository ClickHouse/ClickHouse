...
def feature(self, node="clickhouse1"):
    ...
    try:
        ...
        with Scenario(
            "I show row policies",
            requirements=[RQ_SRS_006_RBAC_RowPolicy_ShowRowPolicies("1.0")],
        ):
        ...
        with Scenario(
            "I show row policies using short syntax",
            requirements=[RQ_SRS_006_RBAC_RowPolicy_ShowRowPolicies("1.0")],
        ):
        ...
        with Scenario(
            "I show row policies on a database table",
            requirements=[RQ_SRS_006_RBAC_RowPolicy_ShowRowPolicies_On("1.0")],
        ):
            with cleanup("policy0"):
                with When("I run drop row policy command"):
        ...
        with Scenario(
            "I show row policies on a table",
            requirements=[RQ_SRS_006_RBAC_RowPolicy_ShowRowPolicies_On("1.0")],
        ):
            with cleanup("policy0"):
                with When("I run drop row policy command"):
