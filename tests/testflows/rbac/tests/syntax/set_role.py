...
def feature(self, node="clickhouse1"):
    ...
    with Scenario(
        "I set default role for current user",
        requirements=[RQ_SRS_006_RBAC_SetRole_Default("1.0")],
    ):
        with When("I set default role for current user"):
            node.query("SET ROLE DEFAULT")

    with Scenario(
        "I set no role for current user",
        requirements=[RQ_SRS_006_RBAC_SetRole_None("1.0")],
    ):
        with When("I set no role for current user"):
            node.query("SET ROLE NONE")

    with Scenario(
        "I set nonexistent role, throws exception",
        requirements=[RQ_SRS_006_RBAC_SetRole_None("1.0")],
    ):
    ...
    with Scenario(
        "I set one role for current user", requirements=[RQ_SRS_006_RBAC_SetRole("1.0")]
    ):
        with setup(1):
    ...
    with Scenario(
        "I set multiple roles for current user",
        requirements=[RQ_SRS_006_RBAC_SetRole("1.0")],
    ):
        with setup(2):
            with Given("I have a user"):
                node.query("CREATE USER OR REPLACE user0")
    ...
    with Scenario(
        "I set all roles for current user",
        requirements=[RQ_SRS_006_RBAC_SetRole_All("1.0")],
    ):
        with When("I set all roles for current user"):
            node.query("SET ROLE ALL")

    with Scenario(
        "I set all roles except one for current user",
        requirements=[RQ_SRS_006_RBAC_SetRole_AllExcept("1.0")],
    ):
        with setup(1):
