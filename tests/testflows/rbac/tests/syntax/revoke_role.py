...
def feature(self, node="clickhouse1"):
    ...
    with Scenario(
        "I revoke a role from a user", requirements=[RQ_SRS_006_RBAC_Revoke_Role("1.0")]
    ):
        with setup():
    ...
    with Scenario(
        ...
        with setup(1, 0):
            with When("I revoke nonexistent role from a user"):
                ...
                node.query(
                    "REVOKE role0 FROM user0", exitcode=exitcode, message=message
                )
    ...
    with Scenario(
        ...
        with setup(0, 0):
            with When("I revoke nonexistent role from a nonexistent user"):
                exitcode, message = (
                    errors.role_not_found_in_disk(name="user0")
                    if check_clickhouse_version(">=21.09")(self)
                    else errors.role_not_found_in_disk(name="role0")
                )
                node.query(
                    "REVOKE role0 FROM user0", exitcode=exitcode, message=message
                )
    ...
    with Scenario(
        ...
        with setup():
            with When("I revoke a role from current user"):
                node.query(
                    "REVOKE role0 FROM CURRENT_USER", settings=[("user", "user0")]
                )
    ...
    with Scenario(
        ...
        with setup():
            with When("I revoke a role from all except default"):
                node.query(
                    "REVOKE role0 FROM ALL EXCEPT default", settings=[("user", "user0")]
                )
    ...
    with Scenario(
        "I revoke a role from a role", requirements=[RQ_SRS_006_RBAC_Revoke_Role("1.0")]
    ):
        with setup():
