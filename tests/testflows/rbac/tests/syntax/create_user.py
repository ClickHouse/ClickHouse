...
def feature(self, node="clickhouse1"):
    ...
    with Scenario(
        "I create user that already exists, throws exception",
        requirements=[
            RQ_SRS_006_RBAC_User_Create("1.0"),
            RQ_SRS_006_RBAC_User_Create_Host_Default("1.0"),
        ],
    ...
    with Scenario(
        "I create user or replace, user does not exist",
    ...
    with Scenario(
        "I create user or replace, user does exist",
        requirements=[RQ_SRS_006_RBAC_User_Create_Replace("1.0")],
    ):
        user = "user0"
        with cleanup(user):
    ...
    with Scenario(
        "I create user with no password",
        requirements=[RQ_SRS_006_RBAC_User_Create_Password_NoPassword("1.0")],
    ):
        with cleanup("user1"):
    ...
    with Scenario(
        "I create user with plaintext password",
        requirements=[RQ_SRS_006_RBAC_User_Create_Password_PlainText("1.0")],
    ):
        with cleanup("user1"):
    ...
    with Scenario(
        ...
        requirements=[RQ_SRS_006_RBAC_User_Create_Password_Sha256Password("1.0")],
    ):
        with cleanup("user2"):
    ...
    with Scenario(
        "I create user with sha256_hash password",
        requirements=[RQ_SRS_006_RBAC_User_Create_Password_Sha256Hash("1.0")],
    ):
        with cleanup("user3"):
    ...
    with Scenario(
        ...
        requirements=[RQ_SRS_006_RBAC_User_Create_Password_DoubleSha1Password("1.0")],
    ):
        with cleanup("user3"):
    ...
    with Scenario(
        ...
        requirements=[RQ_SRS_006_RBAC_User_Create_Password_DoubleSha1Hash("1.0")],
    ):
        with cleanup("user3"):
    ...
    with Scenario(
        "I create user with default role set to none",
        requirements=[RQ_SRS_006_RBAC_User_Create_DefaultRole_None("1.0")],
    ):
        with cleanup("user8"):
    ...
    with Scenario(
        ...
        with cleanup("user12"):
            ...
            with When(f"I create user with default role {role}"):
                ...
                node.query(
                    f"CREATE USER user12 DEFAULT ROLE {role}",
                    exitcode=exitcode,
                    message=message,
                )
            del role
    ...
    with Scenario(
        "I create user with settings profile",
    ...
    with Scenario(
        ...
        with cleanup("user18a"):
            ...
            with When(
                ...
                node.query(
                    "CREATE USER user18a SETTINGS PROFILE profile0",
                    exitcode=exitcode,
                    message=message,
                )
            del profile
    ...
    with Scenario(
        "I create user with settings without profile",
        requirements=[RQ_SRS_006_RBAC_User_Create_Settings("1.0")],
    ):
        with cleanup("user12"):
    ...
    with Scenario(
        "I create user on fake cluster, throws exception",
        requirements=[RQ_SRS_006_RBAC_User_Create_OnCluster("1.0")],
    ):
