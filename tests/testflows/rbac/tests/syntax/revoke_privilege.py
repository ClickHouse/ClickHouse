...
@Examples(
    "privilege on allow_column allow_introspection",
    [
        (
            "dictGet",
            ("db0.table0", "db0.*", "*.*", "tb0", "*"),
            False,
            False,
            Requirements(RQ_SRS_006_RBAC_Revoke_Privilege_DictGet("1.0")),
        ),
        (
            "INTROSPECTION",
            ("*.*",),
            False,
            True,
            Requirements(RQ_SRS_006_RBAC_Revoke_Privilege_Introspection("1.0")),
        ),
        (
            "SELECT",
            ("db0.table0", "db0.*", "*.*", "tb0", "*"),
            True,
            False,
            Requirements(RQ_SRS_006_RBAC_Revoke_Privilege_Select("1.0")),
        ),
        (
            "INSERT",
            ("db0.table0", "db0.*", "*.*", "tb0", "*"),
            True,
            False,
            Requirements(RQ_SRS_006_RBAC_Revoke_Privilege_Insert("1.0")),
        ),
        (
            "ALTER",
            ("db0.table0", "db0.*", "*.*", "tb0", "*"),
            False,
            False,
            Requirements(RQ_SRS_006_RBAC_Revoke_Privilege_Alter("1.0")),
        ),
        (
            "CREATE",
            ("db0.table0", "db0.*", "*.*", "tb0", "*"),
            False,
            False,
            Requirements(RQ_SRS_006_RBAC_Revoke_Privilege_Create("1.0")),
        ),
        (
            "DROP",
            ("db0.table0", "db0.*", "*.*", "tb0", "*"),
            False,
            False,
            Requirements(RQ_SRS_006_RBAC_Revoke_Privilege_Drop("1.0")),
        ),
        (
            "TRUNCATE",
            ("db0.table0", "db0.*", "*.*", "tb0", "*"),
            False,
            False,
            Requirements(RQ_SRS_006_RBAC_Revoke_Privilege_Truncate("1.0")),
        ),
        (
            "OPTIMIZE",
            ("db0.table0", "db0.*", "*.*", "tb0", "*"),
            False,
            False,
            Requirements(RQ_SRS_006_RBAC_Revoke_Privilege_Optimize("1.0")),
        ),
        (
            "SHOW",
            ("db0.table0", "db0.*", "*.*", "tb0", "*"),
            True,
            False,
            Requirements(RQ_SRS_006_RBAC_Revoke_Privilege_Show("1.0")),
        ),
        (
            "KILL QUERY",
            ("*.*",),
            False,
            False,
            Requirements(RQ_SRS_006_RBAC_Revoke_Privilege_KillQuery("1.0")),
        ),
        (
            "ACCESS MANAGEMENT",
            ("*.*",),
            False,
            False,
            Requirements(RQ_SRS_006_RBAC_Revoke_Privilege_AccessManagement("1.0")),
        ),
        (
            "SYSTEM",
            ("db0.table0", "db0.*", "*.*", "tb0", "*"),
            False,
            False,
            Requirements(RQ_SRS_006_RBAC_Revoke_Privilege_System("1.0")),
        ),
        (
            "SOURCES",
            ("*.*",),
            False,
            False,
            Requirements(RQ_SRS_006_RBAC_Revoke_Privilege_Sources("1.0")),
        ),
        (
            "ALL",
            ("*.*",),
            True,
            True,
            Requirements(RQ_SRS_006_RBAC_Revoke_Privilege_All("1.0")),
        ),
        (
            "ALL PRIVILEGES",
            ("*.*",),
...
def revoke_privileges(
    self, privilege, on, allow_column, allow_introspection, node="clickhouse1"
):
    revoke_privilege(
        privilege=privilege,
        on=on,
        ...
        with When(f"I revoke {privilege} privilege from user on {on_}"):
            with setup(node):
                ...
                with When("I revoke privilege without columns"):
                    node.query(
                        f"REVOKE {privilege} ON {on_} FROM user0", settings=settings
                    )
                ...
                if allow_column and ("*" not in on_):
                    ...
                    with When("I revoke privilege with columns"):
                        node.query(
                            f"REVOKE {privilege}(x) ON {on_} FROM user0",
                            settings=settings,
                        )
...
def feature(self, node="clickhouse1"):
    ...
    with Scenario(
        "I revoke privilege ON fake CLUSTER, throws exception",
        requirements=[
            RQ_SRS_006_RBAC_Revoke_Privilege_Cluster("1.0"),
            RQ_SRS_006_RBAC_Revoke_Privilege_None("1.0"),
        ],
    ...
    with Scenario(
        "I revoke privilege from multiple users and roles",
        requirements=[
    ...
    with Scenario(
        "I revoke privilege from current user",
        requirements=[
            RQ_SRS_006_RBAC_Revoke_Privilege_From("1.0"),
            RQ_SRS_006_RBAC_Revoke_Privilege_None("1.0"),
        ],
        ...
        with setup(node):
            with When("I revoke privilege from current user"):
                node.query(
                    "REVOKE NONE FROM CURRENT_USER", settings=[("user", "user0")]
                )

    with Scenario(
        "I revoke privilege from all users",
        requirements=[
            RQ_SRS_006_RBAC_Revoke_Privilege_From("1.0"),
            RQ_SRS_006_RBAC_Revoke_Privilege_None("1.0"),
        ],
    ...
    with Scenario(
        "I revoke privilege from default user",
        requirements=[
            RQ_SRS_006_RBAC_Revoke_Privilege_From("1.0"),
            RQ_SRS_006_RBAC_Revoke_Privilege_None("1.0"),
        ],
    ...
    with Scenario(
        "I revoke privilege from nonexistent role, throws exception",
        requirements=[
            RQ_SRS_006_RBAC_Revoke_Privilege_From("1.0"),
            RQ_SRS_006_RBAC_Revoke_Privilege_None("1.0"),
        ],
    ...
    with Scenario(
        "I revoke privilege from ALL EXCEPT nonexistent role, throws exception",
        requirements=[
            RQ_SRS_006_RBAC_Revoke_Privilege_From("1.0"),
            RQ_SRS_006_RBAC_Revoke_Privilege_None("1.0"),
        ],
        ...
        with When(f"I revoke privilege from nonexistent role {role}"):
            exitcode, message = errors.role_not_found_in_disk(role)
            node.query(
                f"REVOKE NONE FROM ALL EXCEPT {role}",
                exitcode=exitcode,
                message=message,
            )

    with Scenario(
        "I revoke privilege from all except some users and roles",
        requirements=[
            RQ_SRS_006_RBAC_Revoke_Privilege_From("1.0"),
            RQ_SRS_006_RBAC_Revoke_Privilege_None("1.0"),
        ],
    ...
    with Scenario(
        "I revoke privilege from all except current user",
        requirements=[
            RQ_SRS_006_RBAC_Revoke_Privilege_From("1.0"),
            RQ_SRS_006_RBAC_Revoke_Privilege_None("1.0"),
        ],
