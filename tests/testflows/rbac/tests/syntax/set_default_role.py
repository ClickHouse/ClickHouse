...
def feature(self, node="clickhouse1"):
    ...
    with Scenario(
        "I set default a nonexistent role to user",
    ...
    with Scenario(
        "I set default ALL EXCEPT a nonexistent role to user",
        requirements=[RQ_SRS_006_RBAC_SetDefaultRole("1.0")],
    ):
        with setup(1, 0):
            with When("I set default nonexistent role to a user"):
                exitcode, message = errors.role_not_found_in_disk(name="role0")
                node.query(
                    "SET DEFAULT ROLE ALL EXCEPT role0 TO user0",
                    exitcode=exitcode,
                    message=message,
                )

    with Scenario(
        "I set default a role to a nonexistent user",
        requirements=[RQ_SRS_006_RBAC_SetDefaultRole("1.0")],
    ):
        with setup(0, 1):
    ...
    try:
        ...
        with Scenario(
            "I set default role for a user to none",
            requirements=[RQ_SRS_006_RBAC_SetDefaultRole_None("1.0")],
        ):
            with When("I set no roles default for user"):
                node.query("SET DEFAULT ROLE NONE TO user0")

        with Scenario(
            "I set one default role for a user",
            requirements=[RQ_SRS_006_RBAC_SetDefaultRole("1.0")],
        ):
        ...
        with Scenario(
            "I set one default role for user default, throws exception",
            requirements=[RQ_SRS_006_RBAC_SetDefaultRole("1.0")],
        ):
        ...
        with Scenario(
            "I set multiple default roles for a user",
            requirements=[RQ_SRS_006_RBAC_SetDefaultRole("1.0")],
        ):
        ...
        with Scenario(
            "I set multiple default roles for multiple users",
            requirements=[RQ_SRS_006_RBAC_SetDefaultRole("1.0")],
        ):
        ...
        with Scenario(
            "I set all roles as default for a user",
            requirements=[RQ_SRS_006_RBAC_SetDefaultRole_All("1.0")],
        ):
            with When("I set all roles default to user"):
                node.query("SET DEFAULT ROLE ALL TO user0")

        with Scenario(
            "I set all roles except one for a user",
            requirements=[RQ_SRS_006_RBAC_SetDefaultRole_AllExcept("1.0")],
        ):
        ...
        with Scenario(
            ...
            requirements=[RQ_SRS_006_RBAC_SetDefaultRole_CurrentUser("1.0")],
        ):
            with When("I set default role to current user"):
                node.query("GRANT ACCESS MANAGEMENT ON *.* TO user0")
                node.query(
                    "SET DEFAULT ROLE role0 TO CURRENT_USER",
                    settings=[("user", "user0")],
                )
