...
def feature(self, node="clickhouse1"):
    ...
    with Scenario(
        "I show create quota",
        requirements=[RQ_SRS_006_RBAC_Quota_ShowCreateQuota_Name("1.0")],
    ):
        with cleanup("quota0"):
    ...
    with Scenario(
        "I show create quota current",
        requirements=[RQ_SRS_006_RBAC_Quota_ShowCreateQuota_Current("1.0")],
    ):
        with cleanup("quota1"):
            with When("I run show create quota command"):
    ...
    with Scenario(
        "I show create quota current short form",
        requirements=[RQ_SRS_006_RBAC_Quota_ShowCreateQuota_Current("1.0")],
    ):
        with cleanup("quota2"):
            with When("I run show create quota command"):
