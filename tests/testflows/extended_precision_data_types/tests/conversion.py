...
@Examples(
    ...
    [
        (
            ...
            Requirements(
                RQ_SRS_020_ClickHouse_Extended_Precision_Conversion_toInt128("1.0")
            ),
            Name("Int128"),
        ),
        (
            ...
            Requirements(
                RQ_SRS_020_ClickHouse_Extended_Precision_Conversion_toInt256("1.0")
            ),
            Name("Int256"),
        ),
        (
            ...
            Requirements(
                RQ_SRS_020_ClickHouse_Extended_Precision_Conversion_toUInt128("1.0")
            ),
            Name("UInt128"),
        ),
        (
            ...
            Requirements(
                RQ_SRS_020_ClickHouse_Extended_Precision_Conversion_toUInt256("1.0")
            ),
            Name("UInt256"),
        ),
    ],
...
def int_conversion(self, int_type, min, max, node=None):
    ...
    with When(f"I convert {min}, {max}, 1 to {int_type}"):
        output = node.query(
            f"SELECT to{int_type}('{min}'), to{int_type}('{max}'), to{int_type}(1) format TabSeparatedRaw"
        ).output
...
def MySQL_func(self, node=None):
    ...
    with table_func(table_name, node, mysql_node) as table_function:
        ...
        with And("I create the table"):
            node.query(
                f"CREATE TABLE {table_name} (id UInt8, int128 Int128, uint128 UInt128, int256 Int256, uint256 UInt256, dec256 Decimal256(0)) Engine = Memory"
            )
