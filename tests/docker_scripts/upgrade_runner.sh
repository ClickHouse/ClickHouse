...
#       not masked.
# `StorageKafka2` + `Local: Broker transport failure` is the same no-broker condition surfaced by the
#       storage background task while attaching tables left by `previous_release_repository/tests/queries`,
#       for example `03923_kafka2_keeper_offsets`. Filtered via regex in the secondary pipe below to require
#       both the storage name and the exact topic-partition failure, so unrelated Kafka errors are not masked.
