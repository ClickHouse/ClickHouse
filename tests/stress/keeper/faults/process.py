...
def _proc_exists(node):
    out = sh(node, "pgrep -x clickhouse >/dev/null 2>&1 || pgrep -x clickhouse-server >/dev/null 2>&1; echo $?")
...
def rcvr(node): sh(node,f"printf 'rcvr\n' | nc -w1 127.0.0.1 {CLIENT_PORT}")

def rqld(node): sh(node,f"printf 'rqld\n' | nc -w1 127.0.0.1 {CLIENT_PORT}")

def ydld(node): sh(node,f"printf 'ydld\n' | nc -w1 127.0.0.1 {CLIENT_PORT}")

def cpu_hog(node, seconds=60): sh(node, f"timeout {seconds} sh -c 'while :; do :; done' &")
...
def mem_hog_block(node, mb=512, seconds=60):
    sh(node, f"python3 - <<'PY'\nimport time\n_ = bytearray({int( max(1, mb) )}*1024*1024)\ntime.sleep({int( max(1, seconds) )})\nPY")

def clock_skew(node, seconds): sh_root(node, f"date -s '@$(( $(date +%s) + {int(seconds)} ))'")

def time_strobe(node, swings=6, step_s=500, interval_s=5):
    for i in range(swings):
        clock_skew(node, step_s if i%2==0 else -step_s); sh(node, f"sleep {interval_s}")
