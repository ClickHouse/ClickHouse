...
def dm_delay(node, ms=3):
    try:
        ...
        try:
            ...
            sh_root(
                node, f"mount --bind /mnt/{dm_name} /var/lib/clickhouse/coordination"
            )
        except Exception as e:
            if STRICT_FAULTS:
        ...
        sh_root(
            node,
            f"umount /var/lib/clickhouse/coordination || true; umount /mnt/{dm_name} || true; dmsetup remove --retry -f {dm_name} || true",
        )
