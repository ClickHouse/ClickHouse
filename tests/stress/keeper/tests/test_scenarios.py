...
def test_scenario(scenario, cluster_factory, request, run_meta):
    ...
    try:
        ...
        ensure_environment(nodes, scenario)
        single_leader(nodes)
        ...
        # Start bench first so it can establish sessions under normal conditions,
        # then activate faults after a short warmup.  Starting faults before the
        # bench is ready can cause initial session-establishment failures under
        # high-latency or follower-delay scenarios (e.g. latency-wan-client-fault,
        # latency-slow-follower-fault) where keeper-bench exhausts retries before
        # the first op is executed.
        bench_runner.start()
        if fault_runner:
            time.sleep(10)
            fault_runner.start()
        ...
        def _write_repro():
            repro_dir = pathlib.Path("tmp")
            repro_dir.mkdir(exist_ok=True)
            repro_path = str(repro_dir / f"keeper_repro_{run_id}.yaml")
