...
def count_leaders(nodes): return sum(1 for n in nodes if is_leader(n))
...
def wchs_total(node):
    ...
    try: return int(mntr(node).get("zk_watch_count","0"))
    except: return 0

def any_ephemerals(node): return "Sessions with Ephemerals" in four(node,"dump")

def lgif(node):
    out=four(node,"lgif"); kv={}
    for line in out.splitlines():
        p=line.split()
        if len(p)>=2:
            try: kv[p[0]]=int(p[1])
...
def ready(node):
    if not CONTROL_PORT:
        return False
    return sh(node, f"curl -sf -o /dev/null -w '%{{http_code}}' http://127.0.0.1:{CONTROL_PORT}/ready")["out"].strip()=="200"
...
def ch_async_metrics(node):
    try:
        txt=node.query("SELECT name, value FROM system.asynchronous_metrics FORMAT JSONEachRow")
        return [json.loads(l) for l in txt.strip().splitlines() if l.strip()]
    except Exception:
        return []
