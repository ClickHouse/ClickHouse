...
if [[ "$EXPORT_S3_STORAGE_POLICIES" == "1" ]]; then
    ...
    if check_clickhouse_version 25.5; then
      ...
      sed "s|<allow_dynamic_cache_resize>1</allow_dynamic_cache_resize>||; $reserve_granularity_sed" $SRC_PATH/config.d/storage_conf.xml >$DEST_SERVER_PATH/config.d/storage_conf.xml
