...
if [[ "$EXPORT_S3_STORAGE_POLICIES" == "1" ]]; then
    ...
    # allow_dynamic_cache_resize was added in 25.5; keep_free_space_eviction_threads in 26.6.
    cp $SRC_PATH/config.d/storage_conf.xml $DEST_SERVER_PATH/config.d/storage_conf.xml
    ...
    check_clickhouse_version 26.6 || sed -i "s|<keep_free_space_eviction_threads>4</keep_free_space_eviction_threads>||" $DEST_SERVER_PATH/config.d/storage_conf.xml
