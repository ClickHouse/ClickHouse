...
if [[ "$EXPORT_S3_STORAGE_POLICIES" == "1" ]]; then
    ...
    # present in any released version yet: 26.7 was released without it, so gate on 26.8 (the first
    ...
    check_clickhouse_version 26.8 || sed -i "s|<use_real_disk_size>1</use_real_disk_size>||" $DEST_SERVER_PATH/config.d/storage_conf.xml
