...
class ElOraculoDeTablas:
    ...
    DETAIL_QUERIES = [
        ...
        "SELECT message FROM system.text_log WHERE event_time >= now() - toIntervalSecond(120) AND message ILIKE '%POTENTIALLY_BROKEN_DATA_PART%' AND message NOT ILIKE '%UNION ALL%' ORDER BY event_time DESC LIMIT 3;",
        "",
        "",
        "SELECT database, table, last_exception FROM system.replicas WHERE readonly_start_time IS NOT NULL LIMIT 3;",
        "SELECT database, table, part_name, exception FROM system.part_log WHERE exception != '' AND event_time > (now() - toIntervalSecond(120)) ORDER BY event_time DESC LIMIT 3;",
        "SELECT message FROM system.text_log WHERE event_time >= now() - toIntervalSecond(120) AND message ILIKE '%REPLICA_ALREADY_EXISTS%' AND message NOT ILIKE '%UNION ALL%' ORDER BY event_time DESC LIMIT 3;",
        "SELECT database, table, last_exception FROM system.replication_queue WHERE last_exception != '' LIMIT 3;",
        "SELECT message FROM system.text_log WHERE event_time >= now() - toIntervalSecond(120) AND message ILIKE '%LOGICAL_ERROR%' AND message NOT ILIKE '%UNION ALL%' ORDER BY event_time DESC LIMIT 3;",
        "SELECT message FROM system.text_log WHERE event_time >= now() - toIntervalSecond(120) AND message ILIKE '%CORRUPTED_DATA%' AND message NOT ILIKE '%UNION ALL%' ORDER BY event_time DESC LIMIT 3;",
    ]
            ...
            try:
                info_str = client.query(
                    ...
                    (SELECT count() x, 3 y FROM system.text_log
                     WHERE event_time >= now() - toIntervalSecond(120) AND message ILIKE '%POTENTIALLY_BROKEN_DATA_PART%' AND message NOT ILIKE '%UNION ALL%')
                     UNION ALL
                    ...
                    (SELECT count() x, 8 y FROM system.text_log
                     WHERE event_time >= now() - toIntervalSecond(120) AND message ILIKE '%REPLICA_ALREADY_EXISTS%' AND message NOT ILIKE '%UNION ALL%')
                     UNION ALL
                    (SELECT count() x, 9 y FROM system.replication_queue WHERE last_exception != '')
                     UNION ALL
                    (SELECT count() x, 10 y FROM system.text_log
                     WHERE event_time >= now() - toIntervalSecond(120) AND message ILIKE '%LOGICAL_ERROR%' AND message NOT ILIKE '%UNION ALL%')
                     UNION ALL
                    (SELECT count() x, 11 y FROM system.text_log
                     WHERE event_time >= now() - toIntervalSecond(120) AND message ILIKE '%CORRUPTED_DATA%' AND message NOT ILIKE '%UNION ALL%')
                    ) tx ORDER BY y;
                    """
                )
            except Exception as ex:
