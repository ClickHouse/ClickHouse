...
## Q35
The query causes unbounded memory growth at larger scale factors (e.g. SF100). `EXPLAIN PLAN`
shows that the planner turns at least one of the three correlated `EXISTS` subqueries into a
cross join (`catalog_sales ⋈ date_dim × R2`, `Type: cross`) followed by a filter on
`c_customer_sk = cs_ship_customer_sk`, instead of a semi-join — the filtered `catalog_sales ⋈
date_dim` intermediate (~19M rows at SF100) is combined via cross join before the correlation
condition is applied, so memory keeps growing instead of the query completing in bounded memory.
The alternative formulation rewrites the three correlated `EXISTS` predicates as non-correlated
`IN` subqueries; the planner then builds each `IN`-set once and applies them as a `Prewhere`
filter directly on `customer`, avoiding the cross join. Verified identical results to the
original on SF1, and SF100 completes in ~16s instead of exceeding available memory.
