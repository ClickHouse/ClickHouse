...
class TestRunner:
    ...
    def run_one_test(self, stream, test_name, test_file):
        ...
        if self.dbms_name == 'ClickHouse' and test_name in ['test/select5.test', 'test/evidence/slt_lang_createtrigger.test', 'test/evidence/slt_lang_replace.test', 'test/evidence/slt_lang_droptrigger.test']:
        ...
        with self.connection.with_one_test_scope():
            ...
            parser = test_parser.TestFileParser(stream, test_name, test_file, self.dbms_name)
