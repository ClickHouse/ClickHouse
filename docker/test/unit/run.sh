...
gdb -q  -ex 'set print inferior-events off' -ex 'set confirm off' -ex 'set print thread-events off' -ex 'handle SIGUSR1 nostop' -ex run -ex bt -ex quit --args ./unit_tests_dbms | tee test_output/test_result.txt
