...
if args.report == "main":
    ...
    def add_backward_incompatible_queries():
        rows = tsvRows("report/backward-incompatible-queries-report.tsv")
        if not rows:
            return

        global unstable_backward_incompatible_queries, slow_average_tests, tables
        text = tableStart("Backwards-incompatible Queries")
    ...
    add_backward_incompatible_queries()
