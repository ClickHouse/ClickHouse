...
LLVM_PROFILE_FILE='client_coverage_%5m.profraw' clickhouse-test -j 8 --testname --shard --zookeeper --print-time --use-skip-list 00676_group_by_in 01533_optimize_skip_merged_partitions 01232_preparing_sets_race_condition 2>&1 | ts '%Y-%m-%d %H:%M:%S' | tee /test_result.txt
