...
bool IQueryTreeNode::isEqual(const IQueryTreeNode & rhs, CompareOptions compare_options) const
{
    ...
    while (!nodes_to_process.empty())
    {
        ...
        /// Quoted aliases are semantic in `standard` mode: `expr AS x` and `expr AS "x"` must not be
        /// deduplicated. Consult the quote bit only when an alias is present (ignore stale quote state).
