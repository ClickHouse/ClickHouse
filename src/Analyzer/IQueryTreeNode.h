...
struct ConvertToASTOptions
{
    ...
    /// Name the active member type of a `Dynamic` constant, so a ClickHouse shard rebuilds the
    /// subtype the initiator held. An external database has no `Dynamic`: it reads the literal as
    /// text, so a named member changes which rows its predicate selects.
    bool name_dynamic_member_types = true;
