...
void inlineMaterializedCTEIfNeeded(QueryTreeNodePtr & node, ContextPtr context)
{
    ...
    {
        /// Kept materialized either because the query reads it from more than one place, or because
        /// analysis proved a single reference site is executed repeatedly (`requireMaterialization`).
