...
public:
    ...
    void visitImpl(QueryNode * query_node)
    {
        if (auto & where_node = query_node->getWhere())
        {
            stack.push_back(&where_node); }
    }
    ...
    bool tryRewrite(JoinNode * join_node)
    {
        ...
        auto & where_condition = *stack.back();
    ...
    void visitImpl(QueryTreeNodePtr & node)
    {
        ...
        if (auto * query_node = node->as<QueryNode>())
            visitImpl(query_node);

        if (auto * join_node = node->as<JoinNode>())
        {
            bool is_rewritten = tryRewrite(join_node);
            if (!is_rewritten && forceRewrite())
            {
        ...
        if (!stack.empty() && stack.back()->get() == node.get())
        {
            stack.pop_back();
        }
    }
...
private:
    ...
    std::deque<QueryTreeNodePtr *> stack;
};
