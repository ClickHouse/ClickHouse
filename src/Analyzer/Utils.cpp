        ...
        case TypeIndex::Object:
        {
            ...
            for (const auto & [path, path_column] : object_column.getDynamicPaths())
            {
                ...
                if (object_column.hasDefaultPathType())
                {
                    ...
                    object[path] = getFieldFromColumnForASTLiteralImpl(variant.getVariantByGlobalDiscriminator(0).getPtr(), variant.getOffsets()[row], object_column.getDefaultPathType(), true, false);
                    continue;
                }
            ...
            for (size_t i = start; i != end; ++i)
            {
                ...
                if (object_column.hasDefaultPathType())
                {
                    object[path] = getFieldFromColumnForASTLiteralImpl(shared_values->getPtr(), i, object_column.getDefaultPathType(), true, false);
                    continue;
                }
