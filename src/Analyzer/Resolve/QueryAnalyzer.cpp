    ...
    const IdentifierResolveScope & scope)
{
    ...
    for (const auto & column_node : column_nodes)
    {
        ...
        auto should_qualify = [&]()
        {
            ...
            if (!need_to_qualify && not_enough_qualified_identifier_parts && IdentifierResolver::tryBindIdentifierToTableExpression(identifier_lookup, table_expression_node, scope))
                need_to_qualify = true;
