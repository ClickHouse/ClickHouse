                ...
                for (const auto & projection_node : projection_list.getNodes())
                {
                    if (projection_node->hasAlias() && identifier_full_name_ == projection_node->getAlias())
                    {
                        ...
                        if (resolved_nodes.size() == 1)
                        {
                            ...
                            /// and left_table_expression is a JOIN or CROSS_JOIN node, we must not assign it as the column source.
                            /// That would create a ColumnNode with a JOIN/CROSS_JOIN source and non-ListNode expression,
