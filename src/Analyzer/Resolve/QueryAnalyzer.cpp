...
extern const SettingsBool max_bytes_in_set;
extern const SettingsBool max_expanded_ast_elements;
...
void QueryAnalyzer::evaluateScalarSubqueryIfNeeded(QueryTreeNodePtr & node, IdentifierResolveScope & scope)
{
    ...
    if (!context->getSettingsRef()[enable_scalar_subquery_optimization] || !useless_literal_types.contains(scalar_type_name)
    ...
    const std::string & aggregate_function_name, NullsAction action, const ContextPtr & context)
{
    ...
    if (aggregate_function_name_lowercase == "countdistinct")
    {
        ...
        || (settings[rewrite_count_distinct_if_with_count_distinct_implementation]
    ...
    IdentifierResolveSettings identifier_resolve_settings)
{
    ...
    if (!resolve_result.resolved_identifier)
    {
        bool prefer_column_name_to_alias_v = scope.context->getSettingsRef()[prefer_column_name_to_alias];
        ...
        if (identifier_lookup.isExpressionLookup())
        {
            ...
            if (alias_it && (*alias_it)->getNodeType() == QueryTreeNodeType::COLUMN)
            {
                ...
                if (column_node.getColumnSource()->getNodeType() == QueryTreeNodeType::ARRAY_JOIN)
                    prefer_column_name_to_alias_v = true;
            }
        }

        if (unlikely(prefer_column_name_to_alias_v))
        {
...
ProjectionNames QueryAnalyzer::resolveFunction(QueryTreeNodePtr & node, IdentifierResolveScope & scope)
{
    ...
    if (!function_node.isWindowFunction())
    {
        ...
        else if (function_name == "grouping")
        {
            ...
            bool force_grouping_standard_compatibility_v = scope.context->getSettingsRef()[force_grouping_standard_compatibility];
            auto grouping_function = std::make_shared<FunctionGrouping>(force_grouping_standard_compatibility_v);
    ...
    if (is_special_function_in)
    {
        ...
        if (first_argument_constant_node && second_argument_constant_node)
        {
            ...
            auto result_block = getSetElementsForConstantValue(
                first_argument_constant_type, second_argument_constant_literal, second_argument_constant_type, settings[transform_null_in]);
    ...
    QueryTreeNodePtr & node, IdentifierResolveScope & scope, bool allow_lambda_expression, bool allow_table_expression, bool ignore_alias)
{
    ...
    /** Update aliases after expression node was resolved.
      ...
      */
    if (!node_alias.empty() && use_alias_table && !scope.group_by_use_nulls_v)
    {
        auto it = scope.aliases.alias_name_to_expression_node->find(node_alias);
        if (it != scope.aliases.alias_name_to_expression_node->end())
            it->second = node;
    ...
    bool nested_table_function)
{
    ...
    uint64_t use_structure_from_insertion_table_in_table_functions_v
        = scope_context->getSettingsRef()[use_structure_from_insertion_table_in_table_functions];
    if (!nested_table_function && use_structure_from_insertion_table_in_table_functions_v && scope_context->hasInsertionTable()
        && table_function_ptr->needStructureHint())
    {
        ...
        if (!insertion_table.empty())
        {
            ...
            for (; expression != expression_list.end() && insert_column_name_it != insert_column_names_end; ++expression)
            {
                if (auto * identifier_node = (*expression)->as<IdentifierNode>())
                {
                    ...
                    if (!virtual_column_names.contains(identifier_node->getIdentifier().getFullName()))
                    {
                        if (asterisk)
                        {
                            if (use_structure_from_insertion_table_in_table_functions_v == 1)
                ...
                {
                    if (use_structure_from_insertion_table_in_table_functions_v == 2 && findIdentifier(*function))
                    {
                        use_columns_from_insert_query = false;
                        break;
                    }
            ...
            if (use_structure_from_insertion_table_in_table_functions_v == 2 && !asterisk)
            {
            ...
            if (use_columns_from_insert_query)
            {
                ...
                else if (use_structure_from_insertion_table_in_table_functions_v == 1)
...
{
    size_t max_subquery_depth_v = scope.context->getSettingsRef()[max_subquery_depth];
    if (max_subquery_depth_v && scope.subquery_depth > max_subquery_depth_v)
        throw Exception(ErrorCodes::TOO_DEEP_SUBQUERIES, "Too deep subqueries. Maximum: {}", max_subquery_depth_v);
    ...
    validateAggregates(query_node, {.group_by_use_nulls = scope.group_by_use_nulls_v});
