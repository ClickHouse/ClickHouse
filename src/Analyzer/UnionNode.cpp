...
NamesAndTypes UnionNode::computeProjectionColumns() const
{
    ...
    for (size_t column_index = 0; column_index < columns_size; ++column_index)
    {
        ...
        auto result_type = getContext()->getSettingsRef()[Setting::use_variant_as_common_type] ? getLeastSupertypeOrVariant(projection_column_types):getLeastSupertype(projection_column_types);
