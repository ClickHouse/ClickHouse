...
{
    LOG_DEBUG(log, "Write entry in snapshot remote_path: {} links: {}", entry.remote_path, entry.link_count);
...
{
    String new_object_path = fmt::format("vfs_snapshots/shapshot_{}/", snapshot_meta.znode_version + 1);
    // FIXME: object_storage->generateObjectKeyForPath is not suitable because it generates
    // keys like data/ial/ddowfjnxhlexodkhjwdwnzdukbolt
    auto new_object_key = object_storage->generateObjectKeyForPath(new_object_path);
    String new_key = new_object_path + new_object_key.serialize();
