...
struct RemoveObjectStorageOperation final : public IDiskObjectStorageOperation
{
    ...
    void finalize() override
    {
        ...
        if (!delete_metadata_only && !metadata_storage.supportsVFS() && !objects_to_remove.objects.empty()
    ...
    void execute(MetadataTransactionPtr tx) override
    {
        for (const auto & [path, if_exists] : remove_paths)
        {
            ...
            try
            {
                auto objects = metadata_storage.getStorageObjects(path);
                auto unlink_outcome = tx->unlinkMetadata(path);
                if (unlink_outcome && !keep_all_batch_data && !metadata_storage.supportsVFS()
                    && !file_names_remove_metadata_only.contains(fs::path(path).filename()))
                {
...
struct RemoveRecursiveObjectStorageOperation final : public IDiskObjectStorageOperation
{
    ...
    std::string getInfoForLog() const override { return fmt::format("RemoveRecursiveObjectStorageOperation (path: {})", path); }
    ...
    void removeMetadataRecursive(MetadataTransactionPtr tx, const std::string & path_to_remove)
    {
        ...
        if (metadata_storage.isFile(path_to_remove))
        {
            try
            {
                ...
                if (unlink_outcome && !metadata_storage.supportsVFS() && !file_names_remove_metadata_only.contains(rel_path))
                {
...
struct ReplaceFileObjectStorageOperation final : public IDiskObjectStorageOperation
{
    ...
    void finalize() override
    {
        /// Read comment inside RemoveObjectStorageOperation class
        /// TL;DR Don't pay any attention to 404 status code
        if (!metadata_storage.supportsVFS() && !objects_to_remove.empty())
    ...
    void execute(MetadataTransactionPtr tx) override
    {
        ...
        for (const auto & object_from : source_blobs)
        {
            ...
            object_storage.copyObjectToAnotherObjectStorage(
                object_from, object_to, read_settings, write_settings, destination_object_storage);
...
struct TruncateFileObjectStorageOperation final : public IDiskObjectStorageOperation
{
    ...
    TruncateFileObjectStorageOperation(
        IObjectStorage & object_storage_, IMetadataStorage & metadata_storage_, const std::string & path_, size_t size_)
        : IDiskObjectStorageOperation(object_storage_, metadata_storage_), path(path_), size(size_)
    {
    }
    ...
    void finalize() override
    {
        ...
        if (!metadata_storage.supportsVFS() && !truncate_outcome->objects_to_remove.empty())
...
{
    operations_to_execute.emplace_back(std::make_unique<PureMetadataObjectStorageOperation>(
        object_storage, metadata_storage, [from_path, to_path](MetadataTransactionPtr tx) { tx->moveFile(from_path, to_path); }));
}

void DiskObjectStorageTransaction::truncateFile(const String & path, size_t size)
{
    operations_to_execute.emplace_back(std::make_unique<TruncateFileObjectStorageOperation>(object_storage, metadata_storage, path, size));
}
...
std::unique_ptr<WriteBufferFromFileBase> DiskObjectStorageTransaction::writeMetadataFile( /// NOLINT
    const std::string & path,
    WriteMode /* mode */,
    ...
    bool autocommit)
{
    ...
    if (autocommit)
    {
        create_metadata_callback = [tx = shared_from_this(), buf = wrapped_buffer.get(), path](size_t /* count */)
        {
            LOG_DEBUG(
                ...
                "create_metadata_callback {} path: {}  buf: {}",
                true,
                path,
                buf->str());

            // TODO(vfs): check empty buf
            DiskObjectStorageMetadata metadata(
                tx->object_storage.getCommonKeyPrefix(), path); // TODO(vfs): handle key_compatibility_prefix setting
    ...
    else
    {
            ...
            = [tx = shared_from_this(), write_op = write_operation.get(), buf = wrapped_buffer.get(), path](size_t /* count */)
        {
            ...
            write_op->setOnExecute([path, metadata](MetadataTransactionPtr metadata_tx)
                                   { metadata_tx->writeMetadataToFile(path, metadata); });
        };
        operations_to_execute.emplace_back(std::move(write_operation));
    }

    LOG_DEBUG(getLogger("DiskObjectStorageTransaction::writeMetadataFile"), "Autocommit {}", autocommit);
...
std::unique_ptr<WriteBufferFromFileBase> DiskObjectStorageTransaction::writeFile( /// NOLINT
    ...
    bool autocommit)
{
    if (settings.write_disk_object_storage_metadata)
        return writeMetadataFile(path, mode, settings, autocommit);
    ...
    if (autocommit)
    {
        create_metadata_callback = [tx = shared_from_this(), mode, path, key_ = std::move(object_key)](size_t count)
        {
            if (mode == WriteMode::Rewrite)
            {
                ...
                if (!tx->object_storage.isPlain() && !tx->metadata_storage.supportsVFS() && tx->metadata_storage.exists(path))
            ...
            write_op->setOnExecute(
                [object_storage_tx, mode, path, key_, count](MetadataTransactionPtr tx)
                {
                    if (mode == WriteMode::Rewrite)
                    {
                        /// Otherwise we will produce lost blobs which nobody points to
                        /// WriteOnce storages are not affected by the issue
                        if (!object_storage_tx->object_storage.isPlain() && !object_storage_tx->metadata_storage.supportsVFS()
                            && object_storage_tx->metadata_storage.exists(path))
                        {
    ...
    const String & path, WriteMode mode, WriteBlobFunction && write_blob_function)
{
    ...
    if (mode == WriteMode::Rewrite)
    {
        if (!object_storage.isWriteOnce() && !metadata_storage.supportsVFS() && metadata_storage.exists(path))
