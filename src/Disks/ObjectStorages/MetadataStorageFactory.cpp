...
void registerMetadataStorageFromDisk(MetadataStorageFactory & factory)
{
    factory.registerMetadataStorageType(
        ...
        {
            auto metadata_path = config.getString(
                config_prefix + ".metadata_path", fs::path(Context::getGlobalContextInstance()->getPath()) / "disks" / name / "");
            auto vfs_log_path = config.getString(
                config_prefix + ".vfs_log_path", fs::path(Context::getGlobalContextInstance()->getPath()) / "vfs" / "logs" / name / "");
            ...
            VFSLogPtr vfs_log = std::make_shared<VFSLog>(vfs_log_path);
            GarbageCollectorSettings gc_settings = {fmt::format("", name), 1000, 0, 0, 100};
            VFSGarbageCollectorPtr gc = std::make_shared<VFSGarbageCollector>(
                name, object_storage, vfs_log, Context::getGlobalContextInstance()->getSchedulePool(), gc_settings);
            return std::make_shared<VFSMetadataStorageFromDisk>(metadata_disk, key_compatibility_prefix, vfs_log, gc);
        });
}
