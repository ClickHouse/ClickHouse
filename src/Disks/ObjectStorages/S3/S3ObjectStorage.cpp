...
ObjectStorageIteratorPtr S3ObjectStorage::iterate(const std::string & path_prefix, size_t max_keys) const
{
    ...
    if (!max_keys)
        max_keys = (*settings_ptr)[S3RequestSetting::list_object_keys_size];
    ...
    ContextPtr context)
{
    ...
    auto new_client = getClient(uri, new_s3_settings->request_settings, new_s3_settings->auth_settings, context, for_disk_s3);
