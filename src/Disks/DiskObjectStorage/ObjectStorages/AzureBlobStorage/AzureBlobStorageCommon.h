...
/// under `NDEBUG`. Never call it directly on a value that comes from a remote endpoint: the endpoint
/// is under no obligation to send the header, and one that does not must not take the server down.
