...
Poco::AutoPtr<Poco::XML::Document> getDiskConfigurationFromASTImpl(const ASTs & disk_args, ContextPtr context, bool is_loading_from_existing_metadata)
{
    ...
    bool object_storage_type_resolution_is_indirect = false;
    ...
    for (const auto & arg : disk_args)
    {
        ...
        if (key == "type" || key == "object_storage_type")
        {
            if (key == "type")
            {
                ...
                if (is_indirect_value(value_str))
            ...
            else
            {
                ...
                if (value_str == "s3" || value_str.starts_with("s3_"))
                    object_storage_type_is_s3 = true;

                if (is_indirect_value(value_str))
                    object_storage_type_resolution_is_indirect = true;
            }
    ...
    const bool object_storage_type_can_be_s3
        = !has_object_storage_type || object_storage_type_resolution_is_indirect || object_storage_type_is_s3;
    const bool include_can_supply_s3_configuration = has_include
        && (!has_type || type_resolution_is_indirect || is_s3_configuration
            || (type_is_object_storage && object_storage_type_can_be_s3));
    const bool apply_s3_credential_checks = is_s3_configuration
        || type_resolution_is_indirect
        || object_storage_type_resolution_is_indirect
        || include_can_supply_s3_configuration;
