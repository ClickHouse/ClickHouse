...
{
    /// Return local path associated with the blobs_to_read.
    /// All blobs in blobs_to_read container are associated with the same local path.
    /// Their number exceeds one in case of Log family engines.
    return blobs_to_read[0].getConnectedPath();
}
