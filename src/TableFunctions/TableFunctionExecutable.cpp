...
- `pool_size`, `max_command_execution_time` — accepted, but have no effect here: the `executable` table function always starts a fresh process per query and never keeps a pool. They matter only for the `ExecutablePool` table engine.
- `command_termination_timeout` — executable script should contain main read-write loop. After table function is destroyed, pipe is closed, and executable file will have `command_termination_timeout` seconds to shutdown, before ClickHouse will send SIGTERM signal to child process. The same budget applies once the command has finished writing its output: with `check_exit_code` enabled, a command that has not exited within `command_termination_timeout` seconds after that fails the query (its exit code could not be checked) and is sent SIGTERM, rather than being waited for indefinitely. A value of `0` sends the signal at once where the command is being discarded, but leaves the wait for its exit status under `check_exit_code` unbounded. Specified in seconds. Default value is 10.
...
- `stderr_reaction` - what is done with the command's stderr output: `none` (read and discarded), `log` (logged at once), `log_first` (the first 4 KiB logged after the command exits), `log_last` (the last 4 KiB), `throw` (any output fails the query; with `log_first`/`log_last` and a non-zero exit code the output is included in the exception). Default value is `none`.
