...
| `compression`     | The existing compression type when used in a `SELECT` query, or the desired compression type when used in an `INSERT` query. Supported compression types are `gz`, `deflate`, `br`, `xz`, `zst`, `lz4`, `bz2`, and `snappy`.                                                                                   |
