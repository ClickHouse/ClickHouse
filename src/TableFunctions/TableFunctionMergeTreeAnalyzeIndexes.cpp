    ...
    bool is_insert_query) const
{
    ...
    if (source_table_id.hasUUID())
    {
        ...
        auto database_and_table = DatabaseCatalog::instance().tryGetWithTable(source_table_id.uuid);
        source_table = DatabaseCatalog::instance().tryGetWithTable(source_table_id.uuid).second;
        if (!source_table)
