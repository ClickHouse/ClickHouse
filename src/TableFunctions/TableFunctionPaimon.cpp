...
ObjectStoragePtr TableFunctionPaimonImpl<Definition, Configuration>::getObjectStorage(const ContextPtr & context, bool create_readonly) const
{
    if (!object_storage)
...
template <typename Definition, typename Configuration>
StorageObjectStorageConfigurationPtr TableFunctionPaimonImpl<Definition, Configuration>::getConfiguration(ContextPtr context) const
{
    if (!configuration)
    {
...
template <typename Definition, typename Configuration>
std::vector<size_t> TableFunctionPaimonImpl<Definition, Configuration>::skipAnalysisForArguments(
...
template <typename Definition, typename Configuration>
std::shared_ptr<typename TableFunctionPaimonImpl<Definition, Configuration>::Settings>
TableFunctionPaimonImpl<Definition, Configuration>::createEmptySettings()
{
    return std::make_shared<DataLakeStorageSettings>();
}

template <typename Definition, typename Configuration>
void TableFunctionPaimonImpl<Definition, Configuration>::parseArguments(const ASTPtr & ast_function, ContextPtr context)
{
...
template <typename Definition, typename Configuration>
ColumnsDescription TableFunctionPaimonImpl<
...
template <typename Definition, typename Configuration>
StoragePtr TableFunctionPaimonImpl<Definition, Configuration>::executeImpl(
    ...
    bool is_insert_query) const
{
    ...
    storage = std::make_shared<StoragePaimon>(
        configuration,
...
template class TableFunctionPaimonImpl<PaimonDefinition, StorageS3Configuration>;
#endif
#if USE_AVRO && USE_AWS_S3
template class TableFunctionPaimonImpl<PaimonS3Definition, StorageS3Configuration>;
#endif
#if USE_AVRO && USE_AZURE_BLOB_STORAGE
template class TableFunctionPaimonImpl<PaimonAzureDefinition, StorageAzureConfiguration>;
#endif
#if USE_AVRO && USE_HDFS
template class TableFunctionPaimonImpl<PaimonHDFSDefinition, StorageHDFSConfiguration>;
#endif
#if USE_AVRO
template class TableFunctionPaimonImpl<PaimonLocalDefinition, StorageLocalConfiguration>;
#endif

/// Cluster definition instantiations (needed as base class for TableFunctionPaimonClusterImpl).
#if USE_AVRO && USE_AWS_S3
template class TableFunctionPaimonImpl<PaimonS3ClusterDefinition, StorageS3Configuration>;
template class TableFunctionPaimonImpl<PaimonClusterDefinition, StorageS3Configuration>;
#endif
#if USE_AVRO && USE_AZURE_BLOB_STORAGE
template class TableFunctionPaimonImpl<PaimonAzureClusterDefinition, StorageAzureConfiguration>;
#endif
#if USE_AVRO && USE_HDFS
template class TableFunctionPaimonImpl<PaimonHDFSClusterDefinition, StorageHDFSConfiguration>;
#endif
