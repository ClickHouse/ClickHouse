...
| `compression_method` | Compression method. Supported values are `none` (no compression), `gz`, `deflate`, `br`, `xz`, `zst`, `lz4`, `bz2`, and `snappy`. For `snappy`, the wire format is selected by the [snappy_mode](/reference/settings/session-settings/other#snappy_mode) setting (`basic` by default). |
