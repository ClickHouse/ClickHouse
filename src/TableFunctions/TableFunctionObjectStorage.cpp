...
The `compression_method` / `compression` argument is not supported by data lake table functions: the underlying data file format carries its own internal codec. Only `auto` (the default) or an empty string is accepted, which is the same as omitting the argument. To configure the codec used when writing, use the format-specific server setting such as `output_format_parquet_compression_method`.
...
- [DeltaLake cluster table function](/reference/functions/table-functions/deltalakeCluster)
    ...
    factory.registerFunction<TableFunctionDeltaLakeAzure>(
         {.description = R"(The table function can be used to read and write the DeltaLake table stored on Azure object store (writes from version 26.9).)",
            .syntax = "deltaLakeAzure(connection_string|storage_account_url, container_name, blobpath, [account_name, account_key, format, structure])",
            .category = FunctionDocumentation::Category::TableFunction},
         {.allow_readonly = false});
#endif
