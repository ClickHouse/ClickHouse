...
| `filename`                    | Appended to the url if specified.                                                                                                                                                                                                 |
| `use_environment_credentials` | Whether a request that carries no credentials of its own may resolve the ones of the server: the environment variables `AWS_CONTAINER_CREDENTIALS_RELATIVE_URI`, `AWS_CONTAINER_CREDENTIALS_FULL_URI`, `AWS_CONTAINER_AUTHORIZATION_TOKEN`, `AWS_EC2_METADATA_DISABLED` on the S3-compatibility path, and Application Default Credentials with `use_native_gcs`. **Named collections default it to `0`**, which is why a collection that only specifies a `url` reads anonymously; the built-in default of `1` applies to the other shapes. Whether `1` is honoured at all for a user query is decided by [`s3_allow_server_credentials_in_user_queries`](/reference/settings/session-settings/s3-allow#s3_allow_server_credentials_in_user_queries). |
| `no_sign_request`             | Disabled by default.                                                                                                                                                                                                              |
| `expiration_window_seconds`   | Default value is 120.                                                                                                                                                                                                             |
