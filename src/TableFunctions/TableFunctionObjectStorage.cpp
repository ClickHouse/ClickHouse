...
s3(mrap_arn [, access_key_id, secret_access_key], key=object_key [, format=... , structure=...])
```
...
To access an Amazon S3 Multi-Region Access Point (MRAP), supply its ARN as the first argument and the object key as a separate named `key` argument:
...
Replace the example ARN and structure with those of your access point and data. Existing credential arguments and credential configuration apply. The ARN identifies the destination; it does not grant access. MRAP requests require credentials and cannot use `NOSIGN` or `no_sign`.

This interface accepts MRAP access-point ARNs in the commercial AWS partition (`arn:aws:s3::...`). A regional access-point ARN or an MRAP HTTPS URL is not an alternative spelling for this interface. The SDK selects the global MRAP endpoint and SigV4A signing. Do not configure a bucket-specific region or force path-style addressing for this target.

The nonempty `key` is passed separately, without URL decoding. Existing read wildcard semantics apply; a `?` is therefore a wildcard during reads. Archive-path syntax is not interpreted by this interface. Ordinary URL arguments continue to include their object path in the URL and do not accept a separate `key`.

For a named collection, use `mrap_arn` and `key` instead of `url` and `filename`:

```sql
CREATE NAMED COLLECTION mrap_data AS
    mrap_arn = 'arn:aws:s3::123456789012:accesspoint/example.mrap',
    key = 'data/smoke.csv',
    format = 'CSV',
    structure = 'id UInt64, value String';

SELECT * FROM s3(mrap_data);
```

Specify credentials using the existing named-collection credential fields or another permitted credential source. Do not combine `mrap_arn` with `url` or `filename`.
