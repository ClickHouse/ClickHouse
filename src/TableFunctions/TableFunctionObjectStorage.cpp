...
- **Application Default Credentials** and **`NOSIGN`** are the authentication modes supported by the SQL surface. Service-account keys and the `google_adc_*` refresh-token flow are supported only by server-configured GCS disks.
