...
void TableFunctionIceberg::parseArgumentsImpl(
    const String & error_message, ASTs & args, ContextPtr context, StorageS3Configuration & base_configuration)
{
    ...
    /// iceberg(source, format, structure, compression_method) and iceberg(source, access_key_id, access_key_id, format)
    ...
    /// iceberg(source, format, structure) and iceberg(source, access_key_id, access_key_id)
    /// We can distinguish them by looking at the 2-nd argument: check if it's a format name or not.
    else if (args.size() == 3)
    {
        auto second_arg = checkAndGetLiteralArgument<String>(args[1], "format/access_key_id");
...
ColumnsDescription TableFunctionIceberg::getActualTableStructure(ContextPtr context) const
{
    if (configuration.structure == "auto")
    {
        ...
        return StorageIceberg::getTableStructureFromData(configuration, std::nullopt, context);
    }
