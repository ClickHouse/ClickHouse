...
template <StorageTimeSeries::TargetKind target_kind>
class TableFunctionTimeSeriesTarget : public ITableFunction
{
public:
    using TargetKind = StorageTimeSeries::TargetKind;

    static constexpr auto name = (target_kind == TargetKind::Data)
        ? "timeSeriesData"
        : ((target_kind == TargetKind::Tags) ? "timeSeriesTags" : "timeSeriesMetrics");
