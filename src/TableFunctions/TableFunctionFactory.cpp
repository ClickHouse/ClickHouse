...
TableFunctionPtr TableFunctionFactory::get(
    const ASTFunction & table_function,
    const Context & context) const
{
    ...
    if (!res)
    {
        ...
        if (!hints.empty())
            throw Exception(
                ErrorCodes::UNKNOWN_FUNCTION, "Unknown table function {}. Maybe you meant: {}", table_function.name, toString(hints));
        else
            throw Exception(ErrorCodes::UNKNOWN_FUNCTION, "Unknown table function {}", table_function.name);
    }

    res->parseArguments(table_function, context);
    return res;
}
