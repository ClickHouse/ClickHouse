...
void TableFunctionPredict::parseArguments(const ASTPtr & ast_function, ContextPtr)
{
    ...
    if (args_func.size() != 1)
        throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Table function '{}' must have an argument list", getName());

    auto * list = args_func[0]->as<ASTExpressionList>();
    if (list->children.size() != 2)
        throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Table function '{}' must have 2 arguments", getName());

    model_name = list->children[0]->as<ASTIdentifier>()->name();
    table_name = list->children[1]->as<ASTIdentifier>()->name();
}
    ...
    bool) const
{
    ...
    col->getData().assign(y_hat.begin(), y_hat.end());
