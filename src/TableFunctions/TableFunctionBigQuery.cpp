...
- When declaring the columns of a persistent `BigQuery`-engine table explicitly, a `RECORD` field may be declared as a plain `Tuple(...)` instead of the inferred `Nullable(Tuple(...))`, at the cost of coercing a whole-record `NULL` to a default tuple; the only accepted difference from the inferred type is dropping a `Nullable` that wraps a `RECORD`'s `Tuple`, and only at that same record — the nullability cannot be moved to a different (inner or outer) record.
