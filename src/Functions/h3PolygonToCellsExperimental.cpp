...
std::pair<const TColumn *, bool>
getArgumentOrConstArgument(const ColumnsWithTypeAndName & arguments, size_t index, std::string_view function_name)
{
    ...
    if (!column)
    {
        if (!column_const)
        {
            throw Exception(
                ...
                arguments[index].column->getName(),
                index + 1,
                function_name,
                demangle(typeid(TColumn).name()));
        }
...
uint32_t parseContainmentMode(const ColumnsWithTypeAndName & arguments, size_t index, std::string_view function_name)
{
    const auto * mode_col = checkAndGetColumnConst<ColumnString>(arguments[index].column.get());
    if (!mode_col)
    {
        throw Exception(
            ErrorCodes::ILLEGAL_COLUMN,
            "Illegal column type {} of argument {} of function {}. Must be a constant String",
            arguments[index].column->getName(),
            index + 1,
            function_name);
    }

    const String mode = mode_col->getValue<String>();
    if (mode == "center")
        return 0;
    if (mode == "full")
        return 1;
    if (mode == "overlapping")
        return 2;
    if (mode == "overlapping_bbox")
        return 3;

    throw Exception(
        ErrorCodes::BAD_ARGUMENTS,
        "Illegal argument '{}' of function {}. 'mode' must be one of: 'center', 'full', 'overlapping', 'overlapping_bbox'",
        mode,
        function_name);
}
...
public:
    ...
    static FunctionPtr create(ContextPtr context) { return std::make_shared<FunctionH3PolygonToCellsExperimental>(context); }
    ...
    ColumnPtr executeImpl(const ColumnsWithTypeAndName & arguments, const DataTypePtr &, size_t input_rows_count) const override
    {
        ...
        const uint32_t flags = parseContainmentMode(arguments, 2, getName());
        ...
        callOnGeometryDataType<SphericalPoint>(arguments[0].type, [&] (const auto & type)
        {
            ...
            for (size_t row = 0; row < input_rows_count; ++row)
            {
                ...
                for (auto & polygon : multi_polygon)
                {
                    ...
                    polygonToCellsExperimental(polygon_wrapper.unwrap(), resolution, flags, static_cast<int64_t>(vec_size), hindex_vec.data());
...
private:
    ...
    public:
        explicit GeoPolygonContainer(std::vector<LatLng> && mainLoop, std::vector<std::vector<LatLng>> && holes = {})
            : mainLoopVerts(std::move(mainLoop))
            , holeVerts(std::move(holes))
        {
        }
...
REGISTER_FUNCTION(H3PolygonToCellsExperimental)
{
    factory.registerFunction<FunctionH3PolygonToCellsExperimental>(FunctionDocumentation{
        ...
        .syntax = "h3PolygonToCellsExperimental(geometry, resolution, mode)",
        .introduced_in = {26, 4},
