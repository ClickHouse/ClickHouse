...
- `icu(locale)` splits strings into locale-aware word tokens using the ICU library's word `BreakIterator` (UAX #29 plus dictionary-based breaking), so languages without whitespace word boundaries (for example Chinese, Japanese, and Thai) are split into meaningful words. The locale is mandatory and is passed as a separate argument, for example `tokens(value, 'icu', 'ja')`. Only available if ClickHouse was built with ICU.
