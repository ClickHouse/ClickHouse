...
private:
        ...
        ColumnString & column_result) const
    {
        ...
        /// `nextInStringLike` of `stringzilla` re-tokenizes the rest of the literal on every call,
        /// `stringLikeToTokens` tokenizes each literal once. The other tokenizers stream tokens without the copy.
        const bool like_tokens_at_once = tokenizer_.getType() == ITokenizer::Type::StringZilla;
        ...
        for (size_t i = 0; i < input_rows_count; ++i)
        {
            ...
            {
                if (like_tokens_at_once)
                {
                    tokens.clear();
                    tokenizer_.stringLikeToTokens(input.data(), input.size(), tokens);
                    for (const auto & like_token : tokens)
                        column_result.insertData(like_token.data(), like_token.size());
                    tokens_count += tokens.size();
                }
                else
                {
                    size_t cur = 0;
                    while (cur < input.size() && tokenizer_.nextInStringLike(input.data(), input.size(), cur, token))
                    {
...
- `stringzilla` splits strings into words using the full [Unicode Text Segmentation (UAX #29)](https://unicode.org/reports/tr29/) word boundary rules, as implemented by the [StringZilla](https://github.com/ashvardanian/StringZilla) library, and drops the segments made of whitespace, punctuation and symbols (including emoji). Unlike `asciiCJK`, words in all scripts are kept whole (for example `café`, `한국어`, `テキスト`), and numbers with separators such as `3.14` and `1,000,000` are one token. Chinese ideographs and Hiragana still become single-character tokens, as UAX #29 does not segment them without a dictionary. `LIKE` patterns are supported: a token next to a wildcard is skipped unless whitespace separates them, since the wildcard may continue it.
...
tokens(value, 'array')
    ...
    FunctionDocumentation::Arguments arguments = {
        ...
        {"tokenizer", "The tokenizer to use. Valid arguments are `splitByNonAlpha`, `splitByString`, `splitByRegexp`, `asciiCJK`, `stringzilla`, `chinese`, `icu`, `japanese`, `ngrams`, `sparseGrams`, and `array`. Optional, if not set explicitly, defaults to `splitByNonAlpha`.", {"const String"}},
    ...
    arguments_like[arg_tokenizer] = {"tokenizer", "The tokenizer to use. Valid arguments are `splitByNonAlpha`, `asciiCJK`, `stringzilla`, `ngrams`, and `sparseGrams`. Optional, if not set explicitly, defaults to `splitByNonAlpha`.", {"const String"}};
