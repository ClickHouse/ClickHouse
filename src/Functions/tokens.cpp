...
- `chinese` segments Chinese text into words using a dictionary and a Hidden Markov Model (the algorithm follows [jieba](https://github.com/fxsjy/jieba); the embedded dictionary/model data is derived from [cppjieba](https://github.com/yanyiwu/cppjieba)). Compared to `asciiCJK`, which makes every CJK character its own token, `chinese` groups consecutive Chinese characters into actual words, giving more meaningful tokens and higher-quality search for Chinese text, at the cost of being Chinese-specific (it does no word splitting of Latin text, though it keeps ASCII alphanumeric runs such as `5G` together and drops ASCII punctuation). There is no separate ICU/SmartCN tokenizer: `asciiCJK` is the rule-based UAX #29 (ICU-style) approach and `chinese` is the dictionary-based jieba method, so the choice is essentially rule-based per-character vs dictionary-based words. An optional `granularity` argument, either `coarse_grained` (default) or `fine_grained`, controls whether overlapping sub-words are also emitted (`fine_grained` improves recall at the cost of a larger index).
