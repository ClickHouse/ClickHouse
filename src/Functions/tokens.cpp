...
- `icu(locale)` splits strings into locale-aware word tokens using the ICU library's Unicode word segmentation algorithm (UAX #29 plus dictionary-based breaking), so languages without whitespace word boundaries (for example Chinese, Japanese, and Thai) are split into meaningful words. The locale is mandatory and is passed as a separate argument, for example `tokens(value, 'icu', 'ja')`.
