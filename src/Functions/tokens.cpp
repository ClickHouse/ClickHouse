...
- `splitByRegexp(re[, extract])` splits strings according to a user-defined regular expression `re`. The regular expression is mandatory, for example, `tokens(value, 'splitByRegexp', '[^\p{L}\p{N}#+]+')`. With the optional `extract` argument left at its default (`false`; `0`/`1` are also accepted), `re` is a separator (also see function [splitByRegexp](/sql-reference/functions/splitting-merging-functions.md/#splitByRegexp)); unlike `splitByString`, a regular expression separator can preserve tokens containing special characters (such as `C++` or `C#`). With `extract = true`, `re` is matched directly instead: each match contributes at most one token - its first capture group, or the whole match if `re` has no capture group - and everything outside the matches is discarded, for example `tokens('tag:hello tag:world', 'splitByRegexp', 'tag:(\w+)', true)` returns `['hello', 'world']`.
...
tokens(value, 'splitByRegexp', regexp[, extract])
...
['abc','bc ','c d',' de','def']
    ...
    {
        "splitByRegexp tokenizer with extract",
