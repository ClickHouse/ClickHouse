...
struct DivideIntegralImpl
{
    ...
    static inline Result apply(A a, B b)
    {
        if constexpr (is_big_int_v<A> && std::is_floating_point_v<B>)
            return A(a. template convert_to<B>() / b);
        else if constexpr (is_big_int_v<B> && std::is_floating_point_v<A>)
            return a / b. template convert_to<A>();
        else if constexpr (is_big_int_v<A> && std::is_same_v<B, UInt8>)
            return checkedDivision(make_signed_t<A>(a), Int16(b)). template convert_to<ResultType>();
        ...
        else if constexpr (is_signed_v<A> || is_signed_v<B>)
        {
            if constexpr ((std::is_integral_v<A> && std::is_integral_v<B>) ||
                (is_big_int_v<A> && std::is_integral_v<B>) ||
                (is_big_int_v<B> && std::is_integral_v<A>) ||
                (is_big_int_v<B> && is_big_int_v<B>))
            {
...
inline Result applyBigIntModulo(A a, B b)
{
    if constexpr (std::is_same_v<A, UInt8>)
        ...
        return (a % UInt16(b)). template convert_to<UInt16>();
    else if constexpr (sizeof(A) > sizeof(B))
        return (a % A(b)). template convert_to<B>();
    else
        return (B(a) % b). template convert_to<B>();
}
