...
inline auto checkedDivision(A a, B b)
{
    ...
    if constexpr (is_big_int_v<A> && is_floating_point_v<B>)
        return static_cast<B>(a) / b;
    else if constexpr (is_big_int_v<B> && is_floating_point_v<A>)
...
struct ModuloImpl
{
    ...
    static inline Result apply(A a, B b)
    {
        if constexpr (is_floating_point_v<ResultType>)
        {
