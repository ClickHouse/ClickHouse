            ...
            && std::is_same_v<ToDataType, DataTypeString>)
        {
            ...
            if (const auto col_from = checkAndGetColumn<ColVecType>(col_with_type_and_name.column.get()))
            {
                ...
                if (null_map)
                {
                    for (size_t i = 0; i < size; ++i)
                    {
                        /// Skip per-row timezone update for null-masked rows to avoid calling
                        /// getDataAt on a nullable timezone column (would throw NOT_IMPLEMENTED).
                        /// We still serialize the stored value with a fallback timezone so the
                        /// nested ColumnString contains a valid non-empty string at null positions.
                        /// Downstream functions that iterate the full inner column
                        /// (e.g. parseDateTimeBestEffort) then never receive an empty string.
