...
struct ToRelativeHourNumImpl
{
    ...
    {
        if constexpr (precision_ == ResultPrecision::Extended)
            return static_cast<UInt32>(time_zone.toStableRelativeHourNum(static_cast<Int64>(t)));
        else
            return static_cast<UInt32>(time_zone.toRelativeHourNum(static_cast<Int64>(t)));
    }
    static inline auto execute(Int32 d, const DateLUTImpl & time_zone)
    {
        if constexpr (precision_ == ResultPrecision::Extended)
...
struct ToRelativeMinuteNumImpl
{
    ...
    static inline UInt32 execute(UInt32 t, const DateLUTImpl & time_zone)
    {
        return static_cast<UInt32>(time_zone.toRelativeMinuteNum(static_cast<Int64>(t)));
    }
    static inline auto execute(Int32 d, const DateLUTImpl & time_zone)
    {
        if constexpr (precision_ == ResultPrecision::Extended)
