...
REGISTER_FUNCTION(IsInfinite)
{
    ...
    FunctionDocumentation::ReturnedValue returned_value = "`1` if x is inifinite, otherwise `0` (including for `NaN`).";
    FunctionDocumentation::Example example1 = {"Test if a number is infinite", R"(
    SELECT
        isInfinite(inf),
        isInfinite(NaN),
        isInfinite(10)
