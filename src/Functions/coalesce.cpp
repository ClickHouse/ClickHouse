...
public:
    ...
    void executeImpl(ColumnsWithTypeAndName & columns, const ColumnNumbers & arguments, size_t result, size_t input_rows_count) const override
    {
        ...
        for (const auto & arg : arguments)
        {
            const auto & type = columns[arg].type;
        ...
        for (size_t i = 0; i < filtered_args.size(); ++i)
        {
            size_t res_pos = temp_columns_columns.size();
            bool is_last = i + 1 == filtered_args.size();
            ...
            {
                temp_columns_columns.emplace_back(ColumnWithTypeAndName{nullptr, std::make_shared<DataTypeUInt8>(), ""});
                is_not_null->build({temp_columns_columns[filtered_args[i]]})->execute(temp_columns_columns, {filtered_args[i]}, res_pos, input_rows_count);
                temp_columns_columns.emplace_back(ColumnWithTypeAndName{nullptr, removeNullable(columns[filtered_args[i]].type), ""});
                assume_not_null->build({temp_columns_columns[filtered_args[i]]})->execute(temp_columns_columns, {filtered_args[i]}, res_pos + 1, input_rows_count);
        ...
        {
            columns[result].column = columns[multi_if_args.front()].column;
            return;
        }
        ...
        for (auto column_num : multi_if_args)
            multi_if_args_elems.emplace_back(temp_columns_columns[column_num]);

        multi_if->build(multi_if_args_elems)->execute(temp_columns_columns, multi_if_args, result, input_rows_count);

        ColumnPtr res = std::move(temp_columns_columns[result].column);

        /// if last argument is not nullable, result should be also not nullable
        if (!columns[multi_if_args.back()].column->isNullable() && res->isNullable())
        {
