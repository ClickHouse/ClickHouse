...
std::string extractTimeZoneNameFromFunctionArguments(const ColumnsWithTypeAndName & arguments, size_t time_zone_arg_num, size_t datetime_arg_num, [[maybe_unused]] bool allow_nonconst_timezone_arguments)
{
    /// Explicit time zone may be passed in last argument.
    if ((arguments.size() == time_zone_arg_num + 1)
       && (arguments[time_zone_arg_num].column))
    {
