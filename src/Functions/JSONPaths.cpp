...
private:
    ...
    {
        /// Variant(None) marker runtime paths (DEFAULT PATH TYPE that cannot be inside Variant)
        /// only mark path presence; the paths themselves are in shared data.
        const bool runtime_paths_have_paths = !type_object.hasDefaultPathType() || isTypeAllowedInsideVariant(type_object.getDefaultPathType());
    ...
    ColumnPtr executeWithTypes(const ColumnObject & column_object, const DataTypeObject & type_object) const
    {
        ...
        if constexpr (Impl::paths_mode == PathsMode::DYNAMIC_PATHS)
        {
            ...
            /// Variant(None) marker runtime paths (DEFAULT PATH TYPE that cannot be inside Variant)
            /// only mark path presence; the paths are in shared data.
            const bool runtime_paths_have_paths = !type_object.hasDefaultPathType() || isTypeAllowedInsideVariant(type_object.getDefaultPathType());
            if (runtime_paths_have_paths)
                for (const auto & [path, _] : dynamic_path_columns)
