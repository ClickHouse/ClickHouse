...
public:
    ...
    {
        if (const ColumnConst * column = checkAndGetColumnConst<ColumnString>(block[arguments[0]].column.get()))
        {
        ...
        block[result].column = block[result].type->createColumnConst(input_rows_count, 0ULL);
    }
};
