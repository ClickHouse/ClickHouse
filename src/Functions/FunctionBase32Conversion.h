...
struct Base32Encode
{
    ...
    static void processString(const ColumnString & src_column, ColumnString::MutablePtr & dst_column, size_t input_rows_count)
    {
        ...
        for (size_t row = 0; row < input_rows_count; ++row)
        {
            ...
            size_t encoded_size = encodeBase32(&src[prev_src_offset], src_length, &dst[current_dst_offset]);
    ...
    static void processFixedString(const ColumnFixedString & src_column, ColumnString::MutablePtr & dst_column, size_t input_rows_count)
    {
        ...
        for (size_t row = 0; row < input_rows_count; ++row)
        {
            size_t encoded_size = encodeBase32(&src[row * N], N, &dst[current_dst_offset]);
            current_dst_offset += encoded_size;
            dst[current_dst_offset] = 0;
            ++current_dst_offset;
...
enum class Base32DecodeErrorHandling : uint8_t
{
    ThrowException,
    ReturnEmptyString
};

template <typename Name, Base32DecodeErrorHandling ErrorHandling>
struct Base32Decode
{
    ...
    static void processString(const ColumnString & src_column, ColumnString::MutablePtr & dst_column, size_t input_rows_count)
    {
        ...
        for (size_t row = 0; row < input_rows_count; ++row)
        {
            size_t current_src_offset = src_offsets[row];
            size_t src_length = current_src_offset - prev_src_offset - 1;
            std::optional<size_t> decoded_size = decodeBase32(&src[prev_src_offset], src_length, &dst[current_dst_offset]);
            if (!decoded_size)
            {
                if constexpr (ErrorHandling == Base32DecodeErrorHandling::ThrowException)
    ...
    static void processFixedString(const ColumnFixedString & src_column, ColumnString::MutablePtr & dst_column, size_t input_rows_count)
    {
        ...
        for (size_t row = 0; row < input_rows_count; ++row)
        {
            std::optional<size_t> decoded_size = decodeBase32(&src[row * N], N, &dst[current_dst_offset]);
            if (!decoded_size)
            {
                if constexpr (ErrorHandling == Base32DecodeErrorHandling::ThrowException)
                    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Invalid Base32 value, cannot be decoded");
