...
public:
    ...
    {
        ColumnPtr first_column = columns[arguments[0]].column;
        const ColumnArray * array_column = checkAndGetColumn<ColumnArray>(columns[arguments[1]].column.get());
        ColumnPtr temp_column;
        if (!array_column)
        {
            const auto * const_array_column = checkAndGetColumnConst<ColumnArray>(columns[arguments[1]].column.get());
