...
public:
    ...
    ~FunctionToStartOfIntervalBase() override = default;
    ...
    DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName & arguments) const override
    {
        ...
        auto check_second_argument = [&]
        {
            ...
            result_type = decideReturnType(interval_type);
        };
    ...
    ColumnPtr executeImpl(const ColumnsWithTypeAndName & arguments, const DataTypePtr & result_type, size_t /* input_rows_count */) const override
    {
        ...
        if (overload == Overload::Origin && arguments.size() >= 3)
...
protected:
    ...
    ColumnPtr dispatchForTimeColumn(
        ...
        const ColumnWithTypeAndName & origin_column, const DataTypePtr & result_type, const DateLUTImpl & time_zone) const
    {
        ...
        const DataTypePtr & result_type, const DateLUTImpl & time_zone, UInt16 scale = 1) const
    {
        ...
        if (!interval_column_const_int64)
            throw Exception(ErrorCodes::ILLEGAL_COLUMN,
                            "Illegal column for 2nd argument of function {}, must be a const time interval",
                            getName());
