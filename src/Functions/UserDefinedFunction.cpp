...
Block UserDefinedFunction::executeCore(const ColumnsWithTypeAndName & arguments) const
{
    ...
    for (size_t j = 0; j < lambda_arg_asts.size(); ++j)
    {
        ...
        if (String(arguments[j].type->getFamilyName()) == "String")
            std::cerr << "********argument: " << column_ptr->getDataAt(0).toString() << std::endl;
        else if (String(arguments[j].type->getFamilyName()) == "UInt32")
            std::cerr << "********argument: " << column_ptr->getUInt(0) << std::endl;
    ...
    if (String(block.getDataTypes().back()->getFamilyName()) == "String")
        std::cerr << "********result: " << block.getColumns().back()->getDataAt(0).toString() << std::endl;
    else if (String(block.getDataTypes().back()->getFamilyName()) == "UInt32")
        std::cerr << "********result: " << block.getColumns().back()->getUInt(0) << std::endl;
    return block;
}
