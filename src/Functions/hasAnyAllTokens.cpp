...
{
    static constexpr size_t supported_number_of_tokens = 64;
    ...
    if (search_tokens->size() > supported_number_of_tokens)
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Function '{}' supports a max of {} search tokens", name, supported_number_of_tokens);
}
...
void executeHasAnyTokens(
    ...
    const Tokens & search_tokens,
    PaddedPODArray<UInt8> & col_result)
{
    std::vector<std::string_view> input_value_tokens;
    for (size_t i = 0; i < input_rows_count; ++i)
    {
        const std::string_view value = col_input.getDataAt(i).toView();
        col_result[i] = false;

        input_value_tokens = token_extractor->getTokensView(value.data(), value.size());
        for (const auto & input_value_token : input_value_tokens)
        {
            if (search_tokens.contains(input_value_token))
            {
                col_result[i] = true;
                break;
            }
...
void executeHasAllTokens(
    ...
    PaddedPODArray<UInt8> & col_result)
{
    const size_t ns = search_tokens.size();
    ...
    for (size_t i = 0; i < input_rows_count; ++i)
    {
        ...
        for (const auto & input_value_token : input_value_tokens)
        {
            if (auto it = search_tokens.find(input_value_token); it != search_tokens.end())
                mask |= (1ULL << it->second);
    ...
    PaddedPODArray<UInt8> & col_result)
{
    ...
    {
        /// No search tokens mean we don't filter and all rows pass
        for (size_t i = 0; i < input_rows_count; ++i)
            col_result[i] = true;
        return;
    }
    ...
    if constexpr (HasTokensTraits::mode == HasAnyAllTokensMode::Any)
        executeHasAnyTokens(token_extractor, col_input, input_rows_count, search_tokens, col_result);
    else if constexpr (HasTokensTraits::mode == HasAnyAllTokensMode::All)
        executeHasAllTokens(token_extractor, col_input, input_rows_count, search_tokens, col_result);
    else
    ...
    const ColumnsWithTypeAndName & arguments, const DataTypePtr &, size_t input_rows_count) const
{
    ...
    if (token_extractor == nullptr)
    {
        ...
        /// Populate tokens from function arguments
