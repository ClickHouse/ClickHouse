...
If the text index has a [postprocessor](/engines/table-engines/mergetree-family/textindexes#postprocessor-argument-optional) expression configured, the postprocessor is applied to both the `input` tokens and the needle tokens after tokenization, so both sides are matched in the same normalized form.
