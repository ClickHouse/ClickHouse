...
hasAnyTokens(input, needles[, tokenizer])
    ...
    FunctionDocumentation::Arguments arguments_hasAnyTokens = {
        ...
        {"tokenizer", "The tokenizer to use. Valid arguments are `splitByNonAlpha`, `splitByString`, `splitByRegexp`, `asciiCJK`, `stringzilla`, `chinese`, `icu('<locale>')`, `japanese`, `ngrams`, `sparseGrams`, and `array`. Optional, if not set explicitly, defaults to `splitByNonAlpha`.", {"const String"}},
    };
