...
struct SimdJSONParser
{
    ...
    public:
        ...
        ALWAYS_INLINE simdjson::dom::object getDom() const { return object; }
    ...
    void reserve(size_t max_size)
    {
        if (parser.allocate(max_size) != simdjson::error_code::SUCCESS)
            throw Exception{
                "Couldn't allocate " + std::to_string(max_size) + " bytes when parsing JSON", ErrorCodes::CANNOT_ALLOCATE_MEMORY};
    }
