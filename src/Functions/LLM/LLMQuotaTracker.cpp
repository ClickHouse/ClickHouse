...
bool LLMQuotaTracker::checkBeforeDispatch(UInt64 estimated_input_tokens, UInt64 batch_rows)
{
    ...
    if (max_rows > 0)
    {
        ...
        if (prev + batch_rows > max_rows)
        {
            ...
            if (on_quota_exceeded != "partial" && on_quota_exceeded != "null")
                throw Exception(ErrorCodes::LIMIT_EXCEEDED,
                    "Limit for LLM rows exceeded: {} rows processed, maximum: {}. "
                    "This is controlled by the 'llm_max_rows_per_query' setting. "
                    "Set 'llm_on_quota_exceeded' to 'partial' to return NULL for remaining rows instead of failing",
                    prev, max_rows);
...
{
    if (on_error == "partial" || on_error == "null")
    {
