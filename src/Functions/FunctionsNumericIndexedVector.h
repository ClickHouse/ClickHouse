...
private:
    ...
    executeImplBridgeFirst(const ColumnsWithTypeAndName & arguments, const DataTypePtr & result_type, size_t input_rows_count) const
    {
        ...
        WhichDataType which(map_type->getKeyType());
#define DISPATCH(TYPE) \
    if (which.idx == TypeIndex::TYPE) \
        return executeImplBridgeSecond<TYPE>(map_type->getValueType(), arguments, result_type, input_rows_count);
        FOR_NUMERIC_INDEXED_VECTOR_INDEX_TYPES(DISPATCH)
#undef DISPATCH
        throw Exception(
            ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "Unexpected index type {} of argument of function {}", from_type->getName(), getName());
    }
...
#undef DISPATCH
    ...
    executeImplBridgeThird(const ColumnsWithTypeAndName & arguments, const DataTypePtr & result_type, size_t input_rows_count) const
    {
        ...
        {
            return executeNumericIndexedVector<BSINumericIndexedVector<IndexType, ValueType>>(arguments, result_type, input_rows_count);
        }
        else
        {
...
#undef DISPATCH
    ...
    ColumnPtr executeImplBridgeThird(const ColumnsWithTypeAndName & arguments, size_t input_rows_count) const
    {
        ...
        if (parameters.empty() || applyVisitor(FieldVisitorToString(), parameters[0]) != "BSI")
        {
            return executeNumericIndexedVectorDataDispatch<BSINumericIndexedVector<IndexType, ValueType>>(arguments, input_rows_count);
        }
        else
        {
            throw Exception(ErrorCodes::LOGICAL_ERROR, "Unsupported parameters");
    ...
    ColumnPtr executeNumericIndexedVectorDataDispatch(const ColumnsWithTypeAndName & arguments, size_t input_rows_count) const
    {
        ...
        if (is_second_column_agg)
        {
            ...
            return executeNumericIndexedVectorData<VectorImpl>(
        ...
        else
        {
            ...
            return executeNumericIndexedVectorDataWithScalar<VectorImpl>(
                first_column_ptr, is_column_const[0], second_column_ptr, is_column_const[1], input_rows_count);
        }
    }
...
#undef DISPATCH
    ...
    ColumnPtr executeImplBridgeThird(const ColumnsWithTypeAndName & arguments, size_t input_rows_count) const
    {
        ...
        if (parameters.empty() || applyVisitor(FieldVisitorToString(), parameters[0]) != "BSI")
        {
            return executeNumericIndexedVector<BSINumericIndexedVector<IndexType, ValueType>>(arguments, input_rows_count);
        }
        else
        {
            throw Exception(ErrorCodes::LOGICAL_ERROR, "Unsupported parameters");
