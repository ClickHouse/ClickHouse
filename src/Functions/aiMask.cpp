...
:::warning
The output is generated by an LLM and is not reliable. Whether PII is actually detected and removed depends
entirely on the chosen model, the prompt, and the input. The model can miss identifiers, redact them only
partially, or alter the surrounding text. `aiMask` does not guarantee that its output is free of PII and
must not be treated as a safe or sufficient anonymization mechanism. Do not rely on it as the only
safeguard before exposing data to untrusted parties or for meeting compliance requirements, and always
