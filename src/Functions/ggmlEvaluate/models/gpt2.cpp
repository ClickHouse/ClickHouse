...
//  bool gpt2_model_load(const std::string & fname, gpt2_model & model, gpt_vocab & vocab, int n_ctx, int n_gpu_layers) {
    ...
    // load vocab
    {
        ...
        if (n_vocab != state.hparams.n_vocab) {
            throw Exception(ErrorCodes::INCORRECT_DATA, "Invalid model file '{}': bad vocab size ({} != {})", fname, n_vocab, state.hparams.n_vocab);
        }
    ...
    // load weights
    {
        ...
        while (true) {
            ...
            if ((nelements*bpe)/ggml_blck_size(tensor->type) != ggml_nbytes(tensor)) {
                throw Exception(ErrorCodes::INCORRECT_DATA, "In model file '{}' : tensor {} has wrong size in model file : got {}, expected {}", fname, name, ggml_nbytes(tensor), nelements * bpe);
            }
...
ggml_cgraph * Gpt2Model::gpt2_graph(const int n_past, const int n_tokens) {
    ...
    struct ggml_tensor * inpL =
        ggml_add(ctx,
                ggml_get_rows(ctx, state.wte, embd),
                ggml_get_rows(ctx, state.wpe, position));
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // norm
        {
            ...
            cur = ggml_add(ctx,
                    ggml_mul(ctx,
                        cur,
                        state.layers[il].ln_1_g),
                    state.layers[il].ln_1_b);
        }
        ...
        // [2304, N]
        {
            cur = ggml_mul_mat(ctx,
                    state.layers[il].c_attn_attn_w,
                    cur);

            cur = ggml_add(ctx,
                    cur,
                    state.layers[il].c_attn_attn_b);
        }
        ...
        // self-attention
        {
            ...
            if (N >= 1) {
                struct ggml_tensor * k = ggml_view_1d(ctx, state.memory_k, N*n_embd, (ggml_element_size(state.memory_k)*n_embd)*(il*n_ctx + n_past));
                struct ggml_tensor * v = ggml_view_1d(ctx, state.memory_v, N*n_embd, (ggml_element_size(state.memory_v)*n_embd)*(il*n_ctx + n_past));
        ...
        {
            cur = ggml_mul_mat(ctx,
                    state.layers[il].c_attn_proj_w,
                    cur);

            cur = ggml_add(ctx,
                    cur,
                    state.layers[il].c_attn_proj_b);
        }
        ...
        // feed-forward network
        {
            // norm
            {
                ...
                cur = ggml_add(ctx,
                        ggml_mul(ctx,
                            cur,
                            state.layers[il].ln_2_g),
                        state.layers[il].ln_2_b);
            }
            ...
            // [3072, N]
            cur = ggml_mul_mat(ctx,
                    state.layers[il].c_mlp_fc_w,
                    cur);

            cur = ggml_add(ctx,
                    cur,
                    state.layers[il].c_mlp_fc_b);
            ...
            // [768, N]
            cur = ggml_mul_mat(ctx,
                    state.layers[il].c_mlp_proj_w,
                    cur);

            cur = ggml_add(ctx,
                    cur,
                    state.layers[il].c_mlp_proj_b);
        }
