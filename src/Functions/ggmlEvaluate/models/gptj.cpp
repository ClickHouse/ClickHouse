...
void GptJModel::loadImpl(ConfigPtr config)
{
    ...
    // load vocab
    {
        ...
        {
            throw Exception(
                ErrorCodes::INCORRECT_DATA, "Invalid model file '{}': bad vocab size ({} != {})", fname, n_vocab, hparams.n_vocab);
        }
    ...
    int n_threads, int n_past, const std::vector<GptVocab::id> & embd_inp, std::vector<float> & embd_w, size_t & mem_per_token)
{
    ...
    for (int il = 0; il < n_layer; ++il)
    {
        ...
        // norm
        {
            ...
            cur = ggml_add(ctx0, ggml_mul(ctx0, ggml_repeat(ctx0, layers[il].ln_1_g, cur), cur), ggml_repeat(ctx0, layers[il].ln_1_b, cur));
        }
        ...
        // self-attention
        {
            struct ggml_tensor * Qcur = ggml_rope_inplace(
                ctx0,
                ggml_reshape_3d(ctx0, ggml_mul_mat(ctx0, layers[il].c_attn_q_proj_w, cur), n_embd / n_head, n_head, N),
                KQ_pos,
                n_rot,
                0,
                0);
            struct ggml_tensor * Kcur = ggml_rope_inplace(
                ctx0,
                ggml_reshape_3d(ctx0, ggml_mul_mat(ctx0, layers[il].c_attn_k_proj_w, cur), n_embd / n_head, n_head, N),
                KQ_pos,
                n_rot,
                0,
                0);
            ...
            // store key and value to memory
            {
                ...
                struct ggml_tensor * k
                    = ggml_view_1d(ctx0, memory_k, N * n_embd, (ggml_element_size(memory_k) * n_embd) * (il * n_ctx + n_past));
            ...
            // projection (no bias)
            cur = ggml_mul_mat(ctx0, layers[il].c_attn_proj_w, cur);
        }
        ...
        // this is independent of the self-attention result, so it could be done in parallel to the self-attention
        {
            ...
            cur = ggml_mul_mat(ctx0, layers[il].c_mlp_fc_w, inpSA);

            cur = ggml_add(ctx0, ggml_repeat(ctx0, layers[il].c_mlp_fc_b, cur), cur);
            ...
            cur = ggml_mul_mat(ctx0, layers[il].c_mlp_proj_w, cur);

            cur = ggml_add(ctx0, ggml_repeat(ctx0, layers[il].c_mlp_proj_b, cur), cur);
        }
    ...
    // norm
    {
        ...
        inpL = ggml_add(ctx0, ggml_mul(ctx0, ggml_repeat(ctx0, ln_f_g, inpL), inpL), ggml_repeat(ctx0, ln_f_b, inpL));
    }
    ...
    // lm_head
    {
        ...
        inpL = ggml_add(ctx0, ggml_repeat(ctx0, lmh_b, inpL), inpL);
    }
