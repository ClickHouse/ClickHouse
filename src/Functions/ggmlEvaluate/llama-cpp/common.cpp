...
std::vector<llama_token> llama_tokenize(
    ...
    n_tokens = llama_tokenize(model, text.data(), static_cast<int>(text.length()), result.data(), static_cast<int>(result.size()), add_special, parse_special);
    if (n_tokens < 0) {
        result.resize(-n_tokens);
        int check = llama_tokenize(model, text.data(), static_cast<int>(text.length()), result.data(), static_cast<int>(result.size()), add_special, parse_special);
