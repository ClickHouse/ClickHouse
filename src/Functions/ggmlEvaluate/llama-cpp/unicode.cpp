...
static std::vector<size_t> unicode_regex_split_custom_gpt2(const std::string & text, const std::vector<size_t> & offsets)
{
    ...
    for (auto offset : offsets)
    {
        ...
        auto _get_cpt = [&](const size_t pos) -> char32_t { return (offset_ini <= pos && pos < offset_end) ? cpts[pos] : 0; };
...
unicode_regex_split_re2(const std::wstring & wtext, const std::wstring & wregex_expr, const std::vector<size_t> & offsets)
{
    ...
    {
        int64_t start_idx = 0;
        re2::StringPiece input(text.data() + start, offset);

        re2::StringPiece match;
        while (RE2::FindAndConsume(&input, expr, &match))
        {
            if (match.data() - (text.data() + start) > start_idx)
            {
                bpe_offsets.emplace_back(match.data() - (text.data() + start) - start_idx);
            }
            bpe_offsets.emplace_back(match.size());
            start_idx = match.data() - (text.data() + start) + match.size();
        }
...
unicode_regex_split_re2(const std::string & text, const std::string & regex_expr, const std::vector<size_t> & offsets)
{
    ...
    for (auto offset : offsets)
    {
        ...
        while (RE2::FindAndConsume(&input, expr, &match))
        {
            int64_t match_start = match.data() - (text.data() + start);
            if (match_start > start_idx)
            {
                bpe_offsets.emplace_back(match_start - start_idx);
            }
            bpe_offsets.emplace_back(match.size());
            start_idx = match_start + match.size();
        }
...
static std::vector<size_t>
unicode_regex_split_custom(const std::string & text, const std::string & regex_expr, const std::vector<size_t> & offsets)
{
