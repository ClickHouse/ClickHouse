...
class FunctionBinaryArithmetic : public IFunction, WithContext
{
        ...
        const NullMap * right_nullmap = nullptr) const
    {
        ...
        if (!right_element_null_map)
        {
            ...
            else if (auto function_builder = getFunctionForTupleArithmetic(new_arguments[0].type, new_arguments[1].type, getContext()))
                res = function_builder->build(new_arguments)->execute(new_arguments, result_array_type, rows_count, /* dry_run = */ false);
            else if (auto tuple_number_function_builder = getFunctionForTupleAndNumberArithmetic(new_arguments[0].type, new_arguments[1].type, getContext()))
...
public:
    ...
    {
        return std::make_shared<FunctionBinaryArithmetic>(context_, left_type, right_type, preserve_nullable_array_arguments_);
    }

    explicit FunctionBinaryArithmetic(
        ContextPtr context_,
        const DataTypePtr & left_type = nullptr,
        const DataTypePtr & right_type = nullptr,
        bool preserve_nullable_array_arguments_ = false)
...
public:
    ...
    FunctionBinaryArithmeticWithConstants(
        ...
        ContextPtr context_,
        bool preserve_nullable_array_arguments_ = false)
        : Base(context_, left_.type, right_.type, preserve_nullable_array_arguments_), left(left_), right(right_), return_type(return_type_)
    {
    }
...
public:
    ...
    FunctionBasePtr buildImpl(const ColumnsWithTypeAndName & arguments, const DataTypePtr & return_type) const override
    {
        ...
        {
            if (division_by_nullable)
                return make_adaptor(FunctionBinaryArithmetic<Op, Name, valid_on_default_arguments, valid_on_float_arguments, true>::create(
                    context, arguments[0].type, arguments[1].type, preserve_nullable_array_arguments));
        }
        return make_adaptor(FunctionBinaryArithmetic<Op, Name, valid_on_default_arguments, valid_on_float_arguments, false>::create(
            context, arguments[0].type, arguments[1].type, preserve_nullable_array_arguments));
    }
