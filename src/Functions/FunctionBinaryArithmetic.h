...
class FunctionBinaryArithmetic : public IFunction
{
    ...
    {
        bool first_arg_is_date_or_datetime_or_string = isDateOrDate32OrTimeOrTime64OrDateTimeOrDateTime64(type0) || isString(type0);
        ...
        const bool valid = castBothTypes(arguments[0].get(), arguments[1].get(), [&](const auto & left, const auto & right)
        {
            ...
            else
            {
                ...
                if constexpr (!std::is_same_v<ResultDataType, InvalidType>)
                {
                    ...
                    else if constexpr (std::is_same_v<ResultDataType, DataTypeDateTime> || std::is_same_v<ResultDataType, DataTypeTime>)
                    {
                        ...
                        if constexpr (std::is_same_v<RightDataType, DataTypeDateTime> || std::is_same_v<RightDataType, DataTypeTime>)
                            tz = &right;
                        if constexpr (std::is_same_v<LeftDataType, DataTypeDateTime> || std::is_same_v<LeftDataType, DataTypeTime>)
                            tz = &left;
...
public:
    ...
    DataTypePtr getReturnTypeImpl(const DataTypes & arguments) const override
    {
        LOG_TRACE(getLogger("TIME DEBUGGING9"), " ");
        if (arguments.size() != 2)
