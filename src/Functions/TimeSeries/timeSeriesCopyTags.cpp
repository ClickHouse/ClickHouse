...
class FunctionTimeSeriesCopyTags : public IFunction, private WithContext
{
public:
    static constexpr auto name = "timeSeriesCopyTags";

    static FunctionPtr create(ContextPtr context_) { return std::make_shared<FunctionTimeSeriesCopyTags>(context_); }
    explicit FunctionTimeSeriesCopyTags(ContextPtr context_) : WithContext(context_) {}
