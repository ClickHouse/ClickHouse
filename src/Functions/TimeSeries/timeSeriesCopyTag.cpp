...
class FunctionTimeSeriesCopyTag : public IFunction, private WithContext
{
public:
    static constexpr auto name = "timeSeriesCopyTag";

    static FunctionPtr create(ContextPtr context_) { return std::make_shared<FunctionTimeSeriesCopyTag>(context_); }
    explicit FunctionTimeSeriesCopyTag(ContextPtr context_) : WithContext(context_) {}
    ...
    ColumnPtr executeImpl(const ColumnsWithTypeAndName & arguments, const DataTypePtr & /* result_type */, size_t input_rows_count) const override
    {
        ...
        auto tag_to_copy = TimeSeriesTagsFunctionHelpers::extractConstStringFromArgument(name, arguments, 2);
...
If the copied tags doesn't present in `src_group` then the function will remove it in `dest_group` as well.
    )";
