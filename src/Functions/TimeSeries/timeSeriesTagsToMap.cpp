...
/// Function timeSeriesTagsToMap([('tag_name_1', 'tag_value_1'), ...], 'tag_name_2', 'tag_value_2', ...)
...
class FunctionTimeSeriesTagsToMap final : public IFunction
{
public:
    static constexpr auto name = "timeSeriesTagsToMap";

    static FunctionPtr create(ContextPtr) { return std::make_shared<FunctionTimeSeriesTagsToMap>(); }
...
REGISTER_FUNCTION(TimeSeriesTagsToMap)
{
...
The result is sorted by tag name, and duplicate tag names and tags with empty values are removed.
    ...
    FunctionDocumentation::Syntax syntax = "timeSeriesTagsToMap(tags_array, tag_name_1, tag_value_1, tag_name_2, tag_value_2, ...)";
...
└──────────────────────────────────────────────────────────────┘
    ...
    factory.registerFunction<FunctionTimeSeriesTagsToMap>(documentation);
}
