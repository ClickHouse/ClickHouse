...
class FunctionTimeSeriesExtractTag : public IFunction, private WithContext
{
public:
    static constexpr auto name = "timeSeriesExtractTag";

    static FunctionPtr create(ContextPtr context_) { return std::make_shared<FunctionTimeSeriesExtractTag>(context_); }
    explicit FunctionTimeSeriesExtractTag(ContextPtr context_) : WithContext(context_) {}
    ...
    ColumnPtr executeImpl(const ColumnsWithTypeAndName & arguments, const DataTypePtr & /* result_type */, size_t input_rows_count) const override
    {
        ...
        auto tag_to_extract = TimeSeriesTagsFunctionHelpers::extractConstStringFromArgument(name, arguments, 1);
