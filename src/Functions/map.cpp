...
/// mapFromString(str_expr, pair_delim, key_value_delim) is a function that allows you to make key-value pairs from a string and delimiters
...
public:
    ...
    DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName & arguments) const override
    {
        FunctionArgumentDescriptors args{
            {"str_expr", &isStringOrFixedString<IDataType>, nullptr, "String or FixedString"}
        };
    ...
    static void executeString(const ColumnString * col_str_in, ColumnPtr & col_res, size_t input_rows_count, char pair_delim, char key_value_delim)
    {
        ...
        offsets->reserve(input_rows_count);
        values_null_map->reserve(input_rows_count);

        ColumnString::Offset prev_res_offset = 0, prev_in_offset = 0;
        ...
        for (size_t i = 0; i < size; ++i)
        {
            ...
            const char * pos_in = reinterpret_cast<const char *>(&in_vec[prev_in_offset]);
            size_t curr_size = in_offsets[i] - prev_in_offset - 1;
            // splitting the string into key-value pairs
            boost::split(key_value_pairs, std::string_view(pos_in, curr_size), boost::is_any_of(pair_delim_str));
            ...
            prev_in_offset = in_offsets[i];
            prev_res_offset += key_value_pairs.size();
            offsets->insert(prev_res_offset);
        }
...
REGISTER_FUNCTION(Map)
{
    ...
    factory.registerAlias("MAP_FROM_ARRAYS", "mapFromArrays", FunctionFactory::CaseInsensitive);
}
