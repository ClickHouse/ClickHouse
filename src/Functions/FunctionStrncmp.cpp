...
public:
    ...
    executeImpl(const ColumnsWithTypeAndName & arguments, const DataTypePtr & /*result_type*/, size_t /*input_rows_count*/) const override
    {
        ...
        auto byts = arguments[4].column->getUInt(0);
        ...
        if (first_string && second_const_string)
        {
            executeStringWithConstString<true>(
                *first_string, first_offset, String(second_const_string->getDataAt(0)), second_offset, byts, result);
        }
        else if (first_string && second_string)
        {
            executeStringWithString(*first_string, first_offset, *second_string, second_offset, byts, result);
        }
        else if (first_fixed_string && second_fixed_string)
        {
            executeFixedStringWithFixedString(*first_fixed_string, first_offset, *second_fixed_string, second_offset, byts, result);
        }
        else if (first_fixed_string && second_string)
        {
            executeFixedStringWithString<true>(*first_fixed_string, first_offset, *second_string, second_offset, byts, result);
        }
        else if (first_string && second_fixed_string)
        {
            executeFixedStringWithString<false>(*second_fixed_string, second_offset, *first_string, first_offset, byts, result);
        }
        else if (first_const_string && second_string)
        {
            executeStringWithConstString<false>(
                *second_string, second_offset, String(first_const_string->getDataAt(0)), first_offset, byts, result);
        }
        else if (first_fixed_string && second_const_string)
        {
            executeFixedStringWithConst<true>(
                *first_fixed_string, first_offset, String(second_const_string->getDataAt(0)), second_offset, byts, result);
        }
        else if (second_fixed_string && first_const_string)
        {
            executeFixedStringWithConst<false>(
                *second_fixed_string, second_offset, String(first_const_string->getDataAt(0)), first_offset, byts, result);
        }
