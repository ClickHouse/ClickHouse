...
inline ALWAYS_INLINE void writeSlice(const GenericArraySlice & slice, GenericArraySink & sink)
{
    const bool can_insert =
        slice.elements->structureEquals(sink.elements) // Check same type
            || (sink.elements.lowCardinality()         // insertRangeFrom casts numeric types if they are different, so it can insert them.
                && slice.elements->lowCardinality()
    ...
    throw Exception(ErrorCodes::LOGICAL_ERROR,
        "Function writeSlice expects same column types or numeric lowCardinality GenericArraySlice and GenericArraySink.");
}
