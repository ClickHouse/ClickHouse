...
public:
    ...
    ColumnPtr executeImpl(const ColumnsWithTypeAndName & arguments, const DataTypePtr &, size_t input_rows_count) const override
    {
        ...
        /// A single box expands to up to max_array_size geohashes inside one call, so a checkpoint placed
        /// around the row loop would not bound the work: it has to be charged inside the expansion.
        ...
        if (WhichDataType(arguments[0].type).isFloat32())
            execute<Float32, UInt8>(lon_min, lat_min, lon_max, lat_max, precision, res, input_rows_count, budget);
        else
            execute<Float64, UInt8>(lon_min, lat_min, lon_max, lat_max, precision, res, input_rows_count, budget);
