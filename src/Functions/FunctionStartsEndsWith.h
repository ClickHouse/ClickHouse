        ...
        while (!haystack_source.isEnd())
        {
            ...
            else
            {
                if constexpr (std::is_same_v<Name, NameStartsWith>) /// startsWith
                    res_data[row_num] = std::string_view(haystack.data, needle.size) == std::string_view(needle.data, needle.size);
                else if constexpr (std::is_same_v<Name, NameEndsWith>) /// endsWith
                    res_data[row_num] = std::string_view(haystack.data + haystack.size - needle.size, needle.size) == std::string_view(needle.data, needle.size);
                else /// startsWithUTF8 or endsWithUTF8
                {
                    ...
                    if constexpr (std::is_same_v<Name, NameStartsWithUTF8>)
                    {
                        ...
                        res_data[row_num] = std::string_view(slice.data, slice.size) == std::string_view(needle.data, needle.size);
                    }
                    else
                    {
