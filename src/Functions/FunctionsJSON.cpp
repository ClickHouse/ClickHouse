...
public:
    ...
    static bool insertResultToColumn(IColumn & dest, const Element & element, std::string_view, const FormatSettings &, String & error)
    {
        ...
        if (!tryGetNumericValueFromJSONElement<JSONParser, NumberType>(value, element, /*convert_bool_to_integer=*/false, /*convert_bool_to_float=*/false, /*allow_type_conversion=*/true, error))
            return false;
