...
public:
    ...
    auto NO_SANITIZE_UNDEFINED execute(const Time64 & t, Args && ... args) const
    {
        ...
        else if constexpr (TransformHasExecuteOverload_v<DecimalUtils::DecimalComponents<Time64>, Args...>)
        {
            ...
            {
                /// `splitFlooringNegative` made `fractional` a non-negative offset, so reassemble with its inverse.
                return DecimalUtils::decimalFromFlooredComponents<Time64>(result, scale_multiplier);
            }
            else
            {
                return result;
