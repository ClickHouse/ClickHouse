...
public:
    ...
    static FunctionPtr create(const ContextPtr & context) { return std::make_shared<FunctionZCurveBase>(context); }
    FunctionZCurveBase(const ContextPtr & /*context*/) {}
    ...
    ColumnPtr executeImpl(const ColumnsWithTypeAndName & arguments, const DataTypePtr &, size_t input_rows_count) const override
    {
        ...
        for (size_t i = 0; i < number_of_elements; ++i)
        {
            ...
            {
                const char * column_data_raw = nullptr;
                if (!isColumnConst(*arg.column))
                {
                    column_data_raw = column_data.data + j * size_per_element;
                }
                else
                {
                    column_data_raw = column_data.data;
                }
                out_data[j] >>= 1;
