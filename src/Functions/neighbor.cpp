...
public:
    ...
    {
        const DataTypePtr & result_type = block[result].type;

        const ColumnWithTypeAndName & source_elem = block[arguments[0]];
        const ColumnWithTypeAndName & offset_elem = block[arguments[1]];
        ...
        if (has_defaults)
        {
            const ColumnWithTypeAndName & default_elem = block[arguments[2]];
        ...
        if (offset_is_constant)
        {
            ...
            if (offset == 0)
            {
                ...
                block[result].column = source_is_constant
