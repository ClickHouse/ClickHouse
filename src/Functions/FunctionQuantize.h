    ...
    DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName & arguments) const override
    {
        if (arguments.size() != 2)
            throw Exception(
                ...
                "Number of arguments for function {} doesn't match: passed {}, expected 2",
                getName(),
                arguments.size());

        const auto * col_size = checkAndGetColumn<ColumnConst>(arguments[1].column.get());
        if (!col_size)
            throw Exception(ErrorCodes::ILLEGAL_COLUMN, "Second argument of function {} must be an uint32", getName());

        size_t array_size = col_size->getValue<UInt32>();
        if (array_size == 0)
        ...
        const DataTypeArray * array_type = typeid_cast<const DataTypeArray *>(data_types[0].get());
        if (!array_type)
        ...
        return std::make_shared<DataTypeFixedString>(fixed_string_length);
    }
