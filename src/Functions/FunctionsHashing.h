...
private:
            ...
            for (size_t i = 0; i < size; ++i)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        hash = apply(reinterpret_cast<const char *>(&value), sizeof(value));
                    }
                }
                ...
                if constexpr (first)
                    ...
                    vec_to[i] = combineHashes(vec_to[i], hash);
            }
        }
    ...
    void executeBigIntType(const IColumn * column, typename ColumnVector<ToType>::Container & vec_to) const
    {
        ...
        if (const ColVecType * col_from = checkAndGetColumn<ColVecType>(column))
        {
            ...
            for (size_t i = 0; i < size; ++i)
            {
                ...
                if constexpr (std::endian::native == std::endian::little)
                    hash = apply(reinterpret_cast<const char *>(&vec_from[i]), sizeof(vec_from[i]));
                else
                {
                    char tmp_buffer[sizeof(vec_from[i])];
                    reverseMemcpy(tmp_buffer, &vec_from[i], sizeof(vec_from[i]));
                    hash = apply(reinterpret_cast<const char *>(tmp_buffer), sizeof(vec_from[i]));
                }
                if constexpr (first)
                    vec_to[i] = hash;
                else
        ...
        else if (auto col_from_const = checkAndGetColumnConst<ColVecType>(column))
        {
            ...
            else
            {
                ...
                hash = apply(reinterpret_cast<const char *>(tmp_buffer), sizeof(value));
            }
            size_t size = vec_to.size();
            if constexpr (first)
                vec_to.assign(size, hash);
    ...
    void executeGeneric(const IColumn * column, typename ColumnVector<ToType>::Container & vec_to) const
    {
        for (size_t i = 0, size = column->size(); i < size; ++i)
        {
            ...
            const ToType hash = apply(bytes.data, bytes.size);
            if constexpr (first)
                vec_to[i] = hash;
            else
                vec_to[i] = combineHashes(vec_to[i], hash);
    ...
    void executeString(const IColumn * column, typename ColumnVector<ToType>::Container & vec_to) const
    {
        ...
        else if (const ColumnFixedString * col_from_fixed = checkAndGetColumn<ColumnFixedString>(column))
        {
            ...
            for (size_t i = 0; i < size; ++i)
            {
                const ToType hash = apply(reinterpret_cast<const char *>(&data[i * n]), n);
                if constexpr (first)
                    vec_to[i] = hash;
                else
                    vec_to[i] = combineHashes(vec_to[i], hash);
        ...
        else if (const ColumnConst * col_from_const = checkAndGetColumnConstStringOrFixedString(column))
        {
            ...
            const ToType hash = apply(value.data(), value.size());
        ...
        if (const ColumnArray * col_from = checkAndGetColumn<ColumnArray>(column))
        {
            ...
            for (size_t i = 0; i < size; ++i)
            {
                ...
                for (size_t j = current_offset; j < next_offset; ++j)
                    vec_to[i] = combineHashes(vec_to[i], vec_temp[j]);
...
public:
    ...
    static ToType apply(const char * begin, size_t size)
    {
        return Impl::apply(begin, size);
    }

    static ToType combineHashes(ToType h1, ToType h2)
    {
