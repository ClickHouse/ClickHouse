...
private:
    ...
    template <bool first>
    void executeGeneric(const KeyColumnsType & key_cols, const IColumn * column, typename ColumnVector<ToType>::Container & vec_to, bool supports_get_data_at) const
    {
        KeyType key{};
        if constexpr (Keyed)
            key = Impl::getKey(key_cols, 0);
        ...
        for (size_t i = 0, size = column->size(); i < size; ++i)
        {
            ...
            {
                /// If column doesn't support getDataAt method we use IColumn::serializeAggregationStateValueIntoArena
                /// to serialize value and calculate hash from it. We use IColumn::serializeAggregationStateValueIntoArena
                /// and not IColumn::serializeValueIntoArena because the first one respects compatibility and the second does not.
                const char * data = nullptr;
                StringRef bytes = column->serializeAggregationStateValueIntoArena(i, arena, data);
                hash = apply(key, bytes.data, bytes.size);
            }
            if constexpr (first)
                vec_to[i] = hash;
    ...
    void executeAny(const KeyColumnsType & key_cols, const IDataType * from_type, const IColumn * icolumn, typename ColumnVector<ToType>::Container & vec_to) const
    {
        ...
        else executeGeneric<first>(key_cols, icolumn, vec_to, from_type->isValueUnambiguouslyRepresentedInContiguousMemoryRegion());
    }
