...
public:
    ...
    ColumnPtr executeImpl(const ColumnsWithTypeAndName & arguments, const DataTypePtr &, size_t input_rows_count) const override
    {
        ...
        for (size_t i = 0; i < input_rows_count; ++i)
            res_data[i] = (Int64(src_data[i]) * 1000 - epoch) << time_shift;
...
public:
    ...
    template <typename T>
    bool executeNonConst(const IColumn & col_src, size_t epoch, ColumnDateTime::Container & res_data, size_t input_rows_count) const
    {
    ...
    bool executeConst(const IColumn & col_src, size_t epoch, ColumnDateTime::Container & res_data, size_t input_rows_count) const
    {
    ...
    ColumnPtr executeImpl(const ColumnsWithTypeAndName & arguments, const DataTypePtr &, size_t input_rows_count) const override
    {
        ...
        if (executeNonConst<ColumnInt64>(col_src, epoch, res_data, input_rows_count)
            || executeNonConst<ColumnUInt64>(col_src, epoch, res_data, input_rows_count))
            return col_res;
        else if (executeConst(col_src, epoch, res_data, input_rows_count))
            return col_res;
        else
...
public:
    ...
    ColumnPtr executeImpl(const ColumnsWithTypeAndName & arguments, const DataTypePtr &, size_t input_rows_count) const override
    {
        ...
        for (size_t i = 0; i < input_rows_count; ++i)
            res_data[i] = static_cast<Int64>(src_data[i] * factor - epoch) << time_shift;
...
public:
    ...
    template <typename T>
    bool executeNonConst(const IColumn & col_src, size_t epoch, ColumnDateTime64::Container & res_data, size_t input_rows_count) const
    {
        if (const auto * col_src_non_const = typeid_cast<const T *>(&col_src))
        {
            const auto & src_data = col_src_non_const->getData();
    ...
    bool executeConst(const IColumn & col_src, size_t epoch, ColumnDateTime64::Container & res_data, size_t input_rows_count) const
    {
        if (const auto * col_src_const = typeid_cast<const ColumnConst *>(&col_src))
        {
            Int64 src_val = col_src_const->getValue<Int64>();
