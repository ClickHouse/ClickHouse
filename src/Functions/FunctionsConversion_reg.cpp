...
For `Float32`/`Float64` inputs, the fractional part is rounded to the nearest value (ties are rounded to even, i.e. banker's rounding), unless the setting `cast_float_to_decimal_uses_rounding = 0`, in which case it is truncated toward zero; for string inputs, excessive digits in the fraction are discarded (not rounded).
