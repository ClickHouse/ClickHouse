...
/// `[]` is structural only when it was added while descending through `Array(JSON)`.
/// Escape literal square brackets and backslashes so path filters can distinguish a
/// stored key such as `items[].price` from the descendant `items[].price` of `items Array(JSON)`.
inline String escapeLiteralPath(std::string_view path)
{
    String result;
...
private:
    ...
    static bool matchesAnyPathOrSubtree(std::string_view path, const VectorWithMemoryTracking<String> & paths)
    {
        while (true)
        {
            ...
            const auto separator = findLastSeparator(path);
    ...
    static size_t findLastSeparator(std::string_view path)
    {
        for (size_t position = path.size(); position != 0; --position)
        {
            ...
            for (size_t i = index; i != 0 && path[i - 1] == '\\'; --i)
