...
/**
 * arrayScan( acc,a1,...,aN->expr, arr1, ..., arrN, acc_initial)
 */
class FunctionArrayScan : public IFunction
{
public:
    static constexpr auto name = "arrayScan";
    static FunctionPtr create(ContextPtr) { return std::make_shared<FunctionArrayScan>(); }
    ...
    DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName & arguments) const override
    {
        ...
        return std::make_shared<DataTypeArray>(accumulator_type);
    }
    ...
    ColumnPtr executeImpl(const ColumnsWithTypeAndName & arguments, const DataTypePtr & result_type, size_t input_rows_count) const override
    {
        ...
        /// We need to prepare a result column that will store arrays of accumulator values
        const auto & nested_type = typeid_cast<const DataTypeArray &>(*result_type).getNestedType();
        auto nested_column = nested_type->createColumn();
        result_col = ColumnArray::create(std::move(nested_column));

        ColumnPtr lambda_col = lambda_function->cloneResized(num_rows);

        /// For arrayScan, we collect intermediate accumulator values for each row
        std::vector<MutableColumnPtr> intermediate_values_for_rows;
        intermediate_values_for_rows.resize(num_rows);
        for (ssize_t i = 0; i < num_rows; ++i)
        {
            intermediate_values_for_rows[i] = accumulator_col->cloneEmpty();
        }

        std::vector<ssize_t> unfinished_row_indices(num_rows);
        for (ssize_t i = 0; i < num_rows; ++i)
            unfinished_row_indices[i] = i;
        ...
        for (size_t slice = 0; slice < max_array_size; ++slice)
        {
            ...
            std::vector<ssize_t> next_unfinished_row_indices;
            next_unfinished_row_indices.reserve(unfinished_rows);
            ...
            for (ssize_t row = 0; row < num_rows; ++row)
            {
                ...
                if (num_elements > slice)
                {
                    ...
                    ++prev_index;
                    next_unfinished_row_indices.push_back(row);
                }
            ...
            if (slice != 0) /// Do not add initial accumulator value
            {
                size_t finished_idx = 0;
                size_t unfinished_idx = 0;

                for (size_t i = 0; i < unfinished_rows; ++i)
                {
                    ssize_t row = unfinished_row_indices[i];
                    if (prev_selector[i] == 0)
                    {
                        intermediate_values_for_rows[row]->insertFrom(*finished_accumulator_values, finished_idx++);
                    }
                    else
                    {
                        intermediate_values_for_rows[row]->insertFrom(*unfinished_accumulator_values, unfinished_idx++);
                    }
                }
            }

            unfinished_row_indices = std::move(next_unfinished_row_indices);
        ...
        if (!unfinished_row_indices.empty())
        {
            for (size_t i = 0; i < unfinished_row_indices.size(); ++i)
            {
                ssize_t row = unfinished_row_indices[i];
                intermediate_values_for_rows[row]->insertFrom(*accumulator_col, i);
            }
        }

        auto & result_array = static_cast<ColumnArray &>(*result_col);
        auto & result_offsets = result_array.getOffsets();
        auto & result_data = result_array.getData();

        for (ssize_t row = 0; row < num_rows; ++row)
        {
            result_data.insertRangeFrom(*intermediate_values_for_rows[row], 0, intermediate_values_for_rows[row]->size());
