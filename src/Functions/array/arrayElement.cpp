...
private:
     ...
     */
    ColumnPtr executeMap(const ColumnMap & col_map, const ColumnsWithTypeAndName & arguments, const DataTypePtr & result_type, size_t input_rows_count) const;
};
...
ColumnPtr FunctionArrayElement::executeMap(
    const ColumnMap & col_map, const ColumnsWithTypeAndName & arguments, const DataTypePtr & /*result_type*/, size_t input_rows_count) const
{
    return col_map.findAll(*arguments[1].column, input_rows_count);
}
...
ColumnPtr FunctionArrayElement::executeImpl(const ColumnsWithTypeAndName & arguments, const DataTypePtr & result_type, size_t input_rows_count) const
{
    ...
    if (col_map)
        return executeMap(*col_map, arguments, result_type, input_rows_count);
