...
public:
    ...
    ColumnPtr executeImpl(const ColumnsWithTypeAndName & arguments, const DataTypePtr &, size_t /*input_rows_count*/) const override
    {
        ...
        for (size_t i = 1; i < arguments.size() - 1; ++i)
        {
            ...
            if (!column_offsets)
            {
                column_offsets = column_array->getOffsetsPtr();
            }
            else
            {
                /// The first condition is optimization: do not compare data if the pointers are equal.
                if (column_array->getOffsetsPtr() != column_offsets
                    && column_array->getOffsets() != typeid_cast<const ColumnArray::ColumnOffsets &>(*column_offsets).getData())
        ...
        for (size_t irow = 0; irow < column_first_array->size(); ++irow) // for each row of result
        {
            ...
            // We initialize it with the starting value given as the last argument.
            ColumnWithTypeAndName accumulator_column = arguments.back();
            ColumnPtr acc(accumulator_column.column->cut(irow, 1));
            auto accumulator = ColumnWithTypeAndName(acc, accumulator_column.type, accumulator_column.name);
            ColumnPtr res(acc);
            ...
            for (size_t iter = 0; arr_cursor < arr_next; ++iter, ++arr_cursor)
            {
                ...
                for (size_t icolumn = 0; icolumn < arrays.size() - 1; ++icolumn)
                {
                    ...
                    iter_arrays.emplace_back(ColumnWithTypeAndName(arr.column->cut(arr_cursor, 1), arr.type, arr.name));
                }
