...
ArraysDepths getArraysDepths(const ColumnsWithTypeAndName & arguments)
{
    ...
    for (size_t i = 0; i < num_arguments; ++i)
    {
        ...
        else
        {
            ...
            if (depth_column && isColumnConst(*depth_column))
            {
                ...
                if (!value)
                    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Incorrect arguments for function arrayEnumerateUniqRanked or arrayEnumerateDenseRanked: depth ({}) cannot be less or equal 0.", std::to_string(value));
    ...
    if (depths.empty())
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Incorrect arguments for function arrayEnumerateUniqRanked or arrayEnumerateDenseRanked: at least one array should be passed.");
    ...
    if (clear_depth > max_array_depth)
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Incorrect arguments for function arrayEnumerateUniqRanked or arrayEnumerateDenseRanked: clear_depth ({}) can't be larger than max_array_depth ({}).", std::to_string(clear_depth), std::to_string(max_array_depth));
