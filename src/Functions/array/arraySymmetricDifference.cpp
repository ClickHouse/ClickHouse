...
class FunctionArraySymmetricDifference : public IFunction
{
public:
    ...
    explicit FunctionArraySymmetricDifference(ContextPtr context_) : context(context_) { }
...
private:
    ...
    template <typename Map, typename Set, typename ColumnType, bool is_numeric_column>
...
DataTypePtr FunctionArraySymmetricDifference::getReturnTypeImpl(const DataTypes & arguments) const
{
...
ColumnPtr FunctionArraySymmetricDifference::castRemoveNullable(const ColumnPtr & column, const DataTypePtr & data_type) const
{
...
FunctionArraySymmetricDifference::CastArgumentsResult FunctionArraySymmetricDifference::castColumns(
...
FunctionArraySymmetricDifference::UnpackedArrays
FunctionArraySymmetricDifference::prepareArrays(const ColumnsWithTypeAndName & columns, ColumnsWithTypeAndName & initial_columns) const
{
...
ColumnPtr FunctionArraySymmetricDifference::executeImpl(
    const ColumnsWithTypeAndName & arguments, const DataTypePtr & result_type, size_t input_rows_count) const
{
    ...
    using DateMap
        = ClearableHashMapWithStackMemory<DataTypeDate::FieldType, size_t, DefaultHash<DataTypeDate::FieldType>, INITIAL_SIZE_DEGREE>;
    using DateSet = ClearableHashSetWithStackMemory<DataTypeDate::FieldType, DefaultHash<DataTypeDate::FieldType>, INITIAL_SIZE_DEGREE>;

    using Date32Map
        = ClearableHashMapWithStackMemory<DataTypeDate32::FieldType, size_t, DefaultHash<DataTypeDate32::FieldType>, INITIAL_SIZE_DEGREE>;
    using Date32Set
        = ClearableHashSetWithStackMemory<DataTypeDate32::FieldType, DefaultHash<DataTypeDate32::FieldType>, INITIAL_SIZE_DEGREE>;
    ...
    using DateTimeMap = ClearableHashMapWithStackMemory<
        ...
        INITIAL_SIZE_DEGREE>;
    using DateTimeSet
        = ClearableHashSetWithStackMemory<DataTypeDateTime::FieldType, DefaultHash<DataTypeDateTime::FieldType>, INITIAL_SIZE_DEGREE>;

    using StringMap = ClearableHashMapWithStackMemory<StringRef, size_t, StringRefHash, INITIAL_SIZE_DEGREE>;
    using StringSet = ClearableHashSetWithStackMemory<StringRef, StringRefHash, INITIAL_SIZE_DEGREE>;
    ...
    if (!result_column)
    {
        ...
        if (which.isDate())
            result_column = execute<DateMap, DateSet, ColumnVector<DataTypeDate::FieldType>, true>(arrays, std::move(column));
        else if (which.isDate32())
            result_column = execute<Date32Map, Date32Set, ColumnVector<DataTypeDate32::FieldType>, true>(arrays, std::move(column));
        else if (which.isDateTime())
            result_column = execute<DateTimeMap, DateTimeSet, ColumnVector<DataTypeDateTime::FieldType>, true>(arrays, std::move(column));
        else if (which.isString())
            result_column = execute<StringMap, StringSet, ColumnString, false>(arrays, std::move(column));
        else if (which.isFixedString())
            result_column = execute<StringMap, StringSet, ColumnFixedString, false>(arrays, std::move(column));
        else
        {
            column = assert_cast<const DataTypeArray &>(*return_type_with_nulls).getNestedType()->createColumn();
            result_column = castRemoveNullable(execute<StringMap, StringSet, IColumn, false>(arrays, std::move(column)), result_type);
        }
    }
...
template <class T>
void FunctionArraySymmetricDifference::NumberExecutor::operator()(TypeList<T>)
{
    using MapContainer = ClearableHashMapWithStackMemory<T, size_t, DefaultHash<T>, INITIAL_SIZE_DEGREE>;
    using SetContainer = ClearableHashSetWithStackMemory<T, DefaultHash<T>, INITIAL_SIZE_DEGREE>;

    if (!result && typeid_cast<const DataTypeNumber<T> *>(data_type.get()))
        result = execute<MapContainer, SetContainer, ColumnVector<T>, true>(arrays, ColumnVector<T>::create());
}

template <class T>
void FunctionArraySymmetricDifference::DecimalExecutor::operator()(TypeList<T>)
{
    using MapContainer = ClearableHashMapWithStackMemory<T, size_t, DefaultHash<T>, INITIAL_SIZE_DEGREE>;
    using SetContainer = ClearableHashSetWithStackMemory<T, DefaultHash<T>, INITIAL_SIZE_DEGREE>;
    ...
    if (!result)
        if (auto * decimal = typeid_cast<const DataTypeDecimal<T> *>(data_type.get()))
            result = execute<MapContainer, SetContainer, ColumnDecimal<T>, true>(arrays, ColumnDecimal<T>::create(0, decimal->getScale()));
}

template <typename Map, typename Set, typename ColumnType, bool is_numeric_column>
ColumnPtr FunctionArraySymmetricDifference::execute(const UnpackedArrays & arrays, MutableColumnPtr result_data_ptr)
{
    ...
    for (const auto & arg : arrays.args)
    {
        ...
        if (!columns.back())
            throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected array type for function arraySymmetricDifference");
