...
public:
    ...
    DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName & arguments) const override
    {
        ...
        ///   strided:     DistanceTransposed(plane..., stride, dims, ref_vec)
        /// If something goes wrong, we fallback to the original DistanceTransposed(qbit, ref_vec, precision[, dims]) handling. The
        ...
        if (arguments.size() == 4)
        {
            const auto & dims_col = arguments[3];
            if (!WhichDataType(dims_col.type).isUInt() || !(dims_col.column && dims_col.column->isConst()))
                throw Exception(
                    ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT,
                    "The fourth argument (dims) of function {} must be a UInt constant, got {}",
                    getName(),
                    dims_col.type->getName());

            const auto dims = dims_col.column->getUInt(0);
            ...
            if (dims == 0 || dims > dimension || dims % stride != 0)
                throw Exception(
                    ErrorCodes::BAD_ARGUMENTS,
                    "The fourth argument (dims) of function {} must be a positive multiple of the QBit stride {} not exceeding the "
                    "dimension {}, got {}",
                    getName(),
                    stride,
                    dimension,
    ...
    struct InternalArguments
    {
        ...
        size_t dims; /// Total number of reconstructed dimensions (num_groups * stride).
    };

    /// Detects and validates the internal forms:
    ///   non-strided: (plane_0, ..., plane_{p-1}, qbit_size, ref_vec)              -- one trailing UInt size
    ///   strided:     (plane_0, ..., plane_{g*p-1}, stride, dims, ref_vec)         -- two trailing UInt sizes, planes group-major
    ...
    std::optional<InternalArguments> parseInternalArguments(const ColumnsWithTypeAndName & arguments, bool require_const_sizes = true) const
    {
        ...
        /// The element right before the reference vector is always a UInt size (qbit_size for non-strided, dims for strided).
        ...
        if (strided)
        {
            ...
            res.dims = arguments[arguments.size() - 2].column->getUInt(0);
            res.num_planes = arguments.size() - 3;

            if (res.stride == 0 || res.dims == 0 || res.dims % res.stride != 0)
                return {};
            const size_t num_groups = res.dims / res.stride;
    ...
    executeImpl(const ColumnsWithTypeAndName & arguments, const DataTypePtr & /* result_type */, size_t input_rows_count) const override
    {
        ...
        /// The user-facing form DistanceTransposed(qbit, ref_vec, precision[, dims]) has a QBit as the first argument.
        ...
        ///   strided:     (plane..., stride, dims, ref_vec)   -- planes group-major
                ...
                [&]<typename RefT, typename CalcT>()
                {
                    return ref_is_const
                        ? executeDistanceCalculation<RefT, CalcT, true>(
                              reference_vector, expanded_planes, precision, stride, dims, input_rows_count)
...
private:
    ...
    /// DistanceTransposed(qbit, ref_vec, precision[, dims]) case. Convert arguments to the internal bit-plane form before executing.
    /// For non-strided QBit this produces (qbit.1, ..., qbit.p, dimension, ref_vec); for strided QBit it produces the group-major
    /// (plane..., stride, dims, ref_vec) form covering only the first `dims / stride` stride groups.
    ColumnPtr executeWithQBitColumnConverted(const ColumnsWithTypeAndName & arguments, size_t input_rows_count) const
    {
        ...
        const size_t dims = (arguments.size() == 4) ? arguments[3].column->getUInt(0) : dimension;
        ...
        if (is_strided)
        {
            ...
            converted_arguments.emplace_back(DataTypeUInt64().createColumnConst(1, dims), std::make_shared<DataTypeUInt64>(), "dims");
        }
        else
        {
            for (size_t bit = 0; bit < precision; ++bit)
    ...
    /// Each stride group is untransposed into its own contiguous slice of the reconstructed `dims`-element vector.
        ...
        size_t input_rows_count) const
    {
        ...
        /// contiguously into a `dims`-element buffer. For the non-strided single group it may be padded above `dims`.
        ...
        if constexpr (std::is_same_v<CalcT, Int8>)
            if (dims > static_cast<size_t>(std::numeric_limits<Int32>::max()) / Kernel::max_int8_simd_term)
                simd_kernel = nullptr;
#endif
        ...
        for (size_t base_row = 0; base_row < input_rows_count; base_row += block_size)
        {
            ...
            for (size_t r = 0; r < rows_in_block; ++r)
            {
                ...
                if (simd_kernel)
                    simd_kernel(dst, ref_data, dims, res);
                else
#endif
                    Kernel::distance(dst, ref_data, dims, res);
            }
        }
