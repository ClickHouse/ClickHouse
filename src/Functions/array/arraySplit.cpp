...
REGISTER_FUNCTION(ArraySplit)
{
    ...
    FunctionDocumentation::Description description_split = "Split a source array into multiple arrays. When a lambda returns true, the array will be split on the right hand side of the element. The array will not be split after the last element.";
    FunctionDocumentation::Syntax syntax_split = "arrayReverseSplit(arr(x[, y1, ..., yN]), source_arr[, cond1_arr, ... , condN_arr])";
    FunctionDocumentation::Arguments arguments_split = {
    	{"func(x [, y1, ..., yN])", "A lambda function `λ(x [, y1, y2, ... yN]) → F(x [, y1, y2, ... yN])` which operates on elements of the source array (`x`) and condition arrays (`y`). [Lambda function](/sql-reference/functions/overview#arrow-operator-and-lambda)."},
