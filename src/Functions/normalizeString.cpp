        ...
        size_t input_rows_count)
    {
        ...
        for (size_t i = 0; i < input_rows_count; ++i)
        {
            ...
            if (from_size > 0)
            {
                ...
                u_strFromUTF8(
                    from_uchars.data(),
                    from_uchars.size(), /// NOLINT (clang-diagnostic-shorten-64-to-32)
                    &from_code_points,
                    reinterpret_cast<const char*>(&data[current_from_offset]),
                    from_size, /// NOLINT (clang-diagnostic-shorten-64-to-32)
                    &err);
                if (U_FAILURE(err))
                ...
                int32_t to_code_points = unorm2_normalize(
                    ...
                    from_code_points,
                    to_uchars.data(),
                    to_uchars.size(), /// NOLINT (clang-diagnostic-shorten-64-to-32)
                    &err);
                if (U_FAILURE(err))
                ...
                u_strToUTF8(
                    ...
                    res_data.size() - current_to_offset, /// NOLINT (clang-diagnostic-shorten-64-to-32)
                    &to_size,
                    to_uchars.data(),
                    to_code_points,
                    &err);
