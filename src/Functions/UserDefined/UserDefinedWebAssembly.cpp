...
private:
        ...
        const ColumnsWithTypeAndName & arguments, size_t start_idx, size_t remaining, size_t budget, size_t & rows_hint) const
    {
        ...
        /// the whole block. A probe serializes the candidate, and a `ColumnConst` argument is
        /// materialized to do it, so a first probe of the whole block would expand exactly the
        /// input the splitting exists to rescue. Measuring one row over-states the marginal cost,
        /// because it carries the whole per-batch state, so the rescaled candidate is an
        /// undershoot that later probes grow into.
