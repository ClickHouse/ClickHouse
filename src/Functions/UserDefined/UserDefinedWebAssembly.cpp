...
private:
        ...
        const ColumnsWithTypeAndName & arguments, size_t start_idx, size_t remaining, size_t budget) const
    {
        ...
        /// rescue. Measuring one row over-states the marginal cost, because it carries the whole
        /// per-batch state, so the rescaled candidate is an undershoot that later probes grow into.
        size_t candidate = 1;
