...
{
    return UserDefinedSQLAggregateFunctionFactory::instance().getCanonicalNameIfAny(name);
}

/*void UserDefinedSQLAggregateFunctionFactory::registerFunction(const String & name, Value creator_with_properties, Case case_sensitiveness)
{
    if (creator_with_properties.creator == nullptr)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "UserDefinedSQLAggregateFunctionFactory: "
    ...
    if (!aggregate_functions.emplace(name, creator_with_properties).second)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "UserDefinedSQLAggregateFunctionFactory: the aggregate function name '{}' is not unique",
            name);
...
void UserDefinedSQLAggregateFunctionFactory::registerNullsActionTransformation(const String & source_ignores_nulls, const String & target_respect_nulls)
{
...
AggregateFunctionPtr UserDefinedSQLAggregateFunctionFactory::get(
    const String & name,
    NullsAction action,
    ...
    AggregateFunctionProperties & out_properties) const
{
    ...
    if (!with_original_arguments)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "UserDefinedSQLAggregateFunctionFactory returned nullptr");
    return with_original_arguments;
}

std::optional<AggregateFunctionWithProperties>
UserDefinedSQLAggregateFunctionFactory::getAssociatedFunctionByNullsAction(const String & name, NullsAction action) const
{
...
AggregateFunctionPtr UserDefinedSQLAggregateFunctionFactory::getImpl(
...
std::optional<AggregateFunctionProperties> UserDefinedSQLAggregateFunctionFactory::tryGetProperties(String name, NullsAction action) const
{
    if (name.size() > MAX_AGGREGATE_FUNCTION_NAME_LENGTH)
        throw Exception(ErrorCodes::TOO_LARGE_STRING_SIZE, "Too long name of aggregate function, maximum: {}", MAX_AGGREGATE_FUNCTION_NAME_LENGTH);
...
bool UserDefinedSQLAggregateFunctionFactory::isAggregateFunctionName(const String & name_) const
{
...
UserDefinedSQLAggregateFunctionFactory & UserDefinedSQLAggregateFunctionFactory::instance()
{
    static UserDefinedSQLAggregateFunctionFactory ret;
    return ret;
}
