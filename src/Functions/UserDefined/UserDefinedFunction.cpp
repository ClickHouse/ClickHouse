...
DataTypePtr UserDefinedFunction::getReturnTypeImpl(const DataTypes &) const
{
    const auto & configuration = executable_function->getConfiguration();
    return configuration.result_type;
}

ColumnPtr UserDefinedFunction::executeImpl(const ColumnsWithTypeAndName & arguments, const DataTypePtr & result_type, size_t input_rows_count) const
{
