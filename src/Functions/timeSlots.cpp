...
public:
    ...
    {
        const auto * starts = checkAndGetColumn<ColumnUInt32>(block[arguments[0]].column.get());
        const auto * const_starts = checkAndGetColumnConst<ColumnUInt32>(block[arguments[0]].column.get());

        const auto * durations = checkAndGetColumn<ColumnUInt32>(block[arguments[1]].column.get());
        const auto * const_durations = checkAndGetColumnConst<ColumnUInt32>(block[arguments[1]].column.get());
        ...
        if (arguments.size() == 3)
        {
            const auto * time_slot_column = checkAndGetColumn<ColumnConst>(block[arguments[2]].column.get());
            if (!time_slot_column)
        ...
        else if (const_starts && durations)
        {
            ...
            throw Exception("Illegal columns " + block[arguments[0]].column->getName()
                    + ", " + block[arguments[1]].column->getName()
                    + ", " + block[arguments[2]].column->getName()
