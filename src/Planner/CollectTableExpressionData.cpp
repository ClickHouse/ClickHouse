...
public:
    ...
    void visitImpl(const QueryTreeNodePtr & node)
    {
        ...
        if (!table_expression)
        {
            ...
            /// Pass the query's own pinned snapshot so identity-partition exclusion is derived from
            /// the metadata fixed at analysis time, not a fresh re-fetch (#110216).
            table_supported_prewhere_columns = storage->supportedPrewhereColumns(table_storage_snapshot);
