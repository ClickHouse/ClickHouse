        ...
        else
        {
            if (!select_query_options.only_analyze)
            {
                ...
                if (select_query_options.build_logical_plan)
                {
                    ...
                    if (table_node)
                    {
                        ...
                        auto reading_from_table = std::make_unique<ReadFromTableStep>(
                            ...
                            mergeChildTableSetKey(table_node));
                    ...
                    else if (table_function_node)
                    {
                        ...
                        auto reading_from_table_function = std::make_unique<ReadFromTableFunctionStep>(
                            ...
                            table_expression_query_info.table_expression_modifiers.value_or(TableExpressionModifiers{}),
                            mergeChildTableSetKey(table_function_node));
                    ...
                    && parallelReplicasEnabledForStorage(storage, query_context, settings))
                {
                    ...
                    if (!is_merge_storage && query_context->canUseParallelReplicasCustomKey() && to_stage_supports_custom_key
                        ...
                        && allowParallelReplicasForJoinTree(parent_join_tree, query_context, settings))
                    {
                        ...
                        if (reading_step_merge_tree && settings[Setting::parallel_replicas_min_number_of_rows_per_replica] > 0)
                        {
                            auto result_ptr
                                ...
                                ? reading_step_merge_tree->estimateRangesToReadWithoutQueryConditionCache()
                                : reading_step_merge_tree->selectRangesToRead();
