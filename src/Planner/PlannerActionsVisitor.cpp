...
public:
    ...
    const ActionsDAG::Node * addConstantIfNecessary(
        const std::string & node_name, const ColumnWithTypeAndName & column, bool is_masked_secret = false)
    {
        auto it = node_name_to_node.find(node_name);
        if (it != node_name_to_node.end())
        {
        ...
        const auto * node = &actions_dag.addColumn(column, is_masked_secret);
        node_name_to_node[node->result_name] = node;
...
PlannerActionsVisitorImpl::NodeNameAndNodeMinLevel PlannerActionsVisitorImpl::visitConstant(const QueryTreeNodePtr & node, const std::string & override_column_name)
{
    ...
    actions_stack[0].addConstantIfNecessary(constant_node_name, column, /* is_masked_secret= */ constant_node.isMasked());
