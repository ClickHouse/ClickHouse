    ...
    const ContextPtr & query_context)
{
    ...
    /// `checkAccessRights` takes a `TableNode`; a parameterized view is a `TableFunctionNode`, so build a temporary one over the same storage.
    TableNode table_node_for_check(storage, storage_id, TableLockHolder{}, storage_snapshot);
    checkAccessRights(table_node_for_check, Names(column_names.begin(), column_names.end()), query_context);
}
