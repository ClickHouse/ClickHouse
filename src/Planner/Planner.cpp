...
void Planner::buildPlanForQueryNode()
{
    ...
    {
        QueryResultCache::Key key(ast, query_context->getCurrentDatabaseInfo(), *settings_copy, query_context->getCurrentQueryId(), query_context->getUserID(), query_context->getCurrentRoles(), /* is_subquery = */ true);
    ...
    if (should_cache && checkCanWriteQueryResultCache(ast, query_context, skip_context_check))
    {
        ...
        QueryResultCache::Key key(
            ast, query_context->getCurrentDatabaseInfo(), *settings_copy, query_plan.getRootNode()->step->getOutputHeader(),
