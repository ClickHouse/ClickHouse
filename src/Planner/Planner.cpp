    ...
    const PlannerContextPtr & planner_context)
{
    ...
    auto expression_step = std::make_unique<ExpressionStep>(query_plan.getCurrentHeader(),
        std::move(actions),
        settings[Setting::enable_adaptive_short_circuit_lazy_execution]);
    ...
    UsefulSets & useful_sets)
{
    ...
    if (expressions_analysis_result.hasLimitBy())
    {
        ...
        addExpressionStep(query_plan, limit_by_analysis_result.before_limit_by_actions, "Before LIMIT BY", useful_sets, planner_context);
...
void Planner::buildPlanForQueryNode()
{
    ...
    if (query_processing_info.isFirstStage())
    {
        if (expression_analysis_result.hasWhere())
            addFilterStep(query_plan, expression_analysis_result.getWhere(), "WHERE", useful_sets, planner_context);

        if (expression_analysis_result.hasAggregation())
        {
            auto & aggregation_analysis_result = expression_analysis_result.getAggregation();
            if (aggregation_analysis_result.before_aggregation_actions)
                addExpressionStep(query_plan, aggregation_analysis_result.before_aggregation_actions, "Before GROUP BY", useful_sets, planner_context);
        ...
        if (!expression_analysis_result.hasAggregation())
        {
            if (expression_analysis_result.hasWindow())
            {
                ...
                if (window_analysis_result.before_window_actions)
                    addExpressionStep(query_plan, window_analysis_result.before_window_actions, "Before WINDOW", useful_sets, planner_context);
            }
            else
            {
                ...
                addExpressionStep(query_plan, projection_analysis_result.projection_actions, "Projection", useful_sets, planner_context);
                ...
                if (expression_analysis_result.hasSort())
                {
                    ...
                    addExpressionStep(query_plan, sort_analysis_result.before_order_by_actions, "Before ORDER BY", useful_sets, planner_context);
                }
            }
        }
    ...
    if (query_processing_info.isSecondStage() || query_processing_info.isFromAggregationState())
    {
        ...
        else if (expression_analysis_result.hasAggregation())
        {
            ...
            if (!having_executed && expression_analysis_result.hasHaving())
                addFilterStep(query_plan, expression_analysis_result.getHaving(), "HAVING", useful_sets, planner_context);
        }
        ...
        else if (expression_analysis_result.hasWindow() || expression_analysis_result.hasAggregation())
        {
            if (expression_analysis_result.hasWindow())
            {
                auto & window_analysis_result = expression_analysis_result.getWindow();
                if (expression_analysis_result.hasAggregation())
                    addExpressionStep(query_plan, window_analysis_result.before_window_actions, "Before window functions", useful_sets, planner_context);

                addWindowSteps(query_plan, planner_context, window_analysis_result);
            }

            if (expression_analysis_result.hasQualify())
                addFilterStep(query_plan, expression_analysis_result.getQualify(), "QUALIFY", useful_sets, planner_context);
        ...
        {
            auto & projection_analysis_result = expression_analysis_result.getProjection();
            addExpressionStep(query_plan, projection_analysis_result.project_names_actions, "Project names", useful_sets, planner_context);
        }
