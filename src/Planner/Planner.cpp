    ...
    const WindowAnalysisResult & window_analysis_result)
{
    ...
    for (size_t i = 0; i < window_descriptions_size; ++i)
    {
        ...
        const bool preserve_num_streams = settings.query_plan_preserve_num_streams_after_window_functions && ((i + 1) == window_descriptions_size);

        auto window_step
            = std::make_unique<WindowStep>(query_plan.getCurrentDataStream(), window_description, window_description.window_functions, preserve_num_streams);
