...
bool lessOp(A a, B b)
{
    ...
    /// int vs float
    if constexpr (is_integer<A> && is_floating_point_v<B>)
    {
    ...
    if constexpr (is_floating_point_v<A> && is_integer<B>)
    {
    ...
    static_assert(is_integer<A> || is_floating_point_v<A>);
    static_assert(is_integer<B> || is_floating_point_v<B>);
    UNREACHABLE();
}
...
inline bool NO_SANITIZE_UNDEFINED convertNumeric(From value, To & result)
{
    ...
    if constexpr (is_floating_point_v<From> && is_floating_point_v<To>)
    {
