...
#define SERVER_SETTINGS(M, ALIAS) \
    ...
    M(Bool, validate_tcp_client_information, false, "Validate client_information in the query packet over the native TCP protocol. ClickHouse-client's with versions 23.7 can't send queries to newer servers with `validate_tcp_client_information` enabled.", 0)
