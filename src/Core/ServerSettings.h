...
#define SERVER_SETTINGS(M, ALIAS) \
    ...
    M(Bool, attach_materialized_views_with_sql_security_none, true, "If true, all materialized views loaded without SQL security statement (e.g. `ignore_empty_sql_security_in_create_view_query = false` or when loading old created views) will use SQL security NONE.", 0)  \
    \
