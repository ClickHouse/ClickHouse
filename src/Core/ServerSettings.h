...
#define SERVER_SETTINGS(M, ALIAS) \
    ...
    M(Float, background_fetches_concurrency_ratio, 2, "Maximum ratio of number of scheduled fetch tasks : number of currently executing fetch tasks", 0) \
    M(Float, reserve_fetch_queue_slot_for_sync_replica_ratio, 0.5, "Reserve some slots in fetching queue to prioritize entries associated with a SYSTEM SYNC REPLICA query", 0) \
