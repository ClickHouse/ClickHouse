...
#define SERVER_SETTINGS(M, ALIAS) \
    ...
    M(UInt64, max_bytes_to_merge_override, 0, "Maximum total size of parts to merge, global override. Zero means unlimited.", 0) \
    M(UInt64, max_bytes_to_mutate_override, 0, "Maximum size of part to mutate, global override. Zero means unlimited.", 0) \
    ...
    M(UInt64, background_early_fetches_pool_size, 8, "The maximum number of threads that will be used for early fetching data parts from another replica for *MergeTree-engine tables in a background.", 0) \
    ...
    M(String, dictionary_user, "default", "Which user to use for dictionary queries.", 0) \
