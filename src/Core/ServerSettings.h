...
#define SERVER_SETTINGS(M, ALIAS) \
    ...
    M(UInt64, max_pending_async_inserts, 500, "Maximum number of pending inserts, if this limit exceeded queue will be forcefully flushed and during that time all new async inserts will wait", 0) \
    M(UInt64, max_pending_async_insert_bytes, 10_GiB, "Limit on size in bytes for all pending inserts in total, if this limit exceeded queue will be forcefully flushed and during that time all new async inserts will wait", 0) \
