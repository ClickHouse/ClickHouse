...
#define SERVER_SETTINGS(M, ALIAS) \
    ...
    M(Int64, heap_profiler_log_sample_rate, 24, "Take memory allocation stack trace once every 2^heap_profiler_log_sample_rate bytes (approximately). 0 to sample every allocation (very slow), -1 to disable", 0) \
    M(Int64, heap_profiler_max_samples, -1, "Number of memory allocation samples to keep in memory and periodically dump to trace_log. Should be on the order of ram_size / 2^heap_profiler_log_sample_rate. Each sample takes around 600 bytes. Set to -1 to pick automatically (constant * available memory / 2^heap_profiler_log_sample_rate). With default settings, heap profiling uses tens of MB of memory.", 0) \
    M(Int64, heap_profiler_dump_period_seconds, 180, "How often to write information about alive memory allocations to system.trace_log, with trace_type = MemoryProfile. Each such dump has a unique profile_id and can be turned into a flame graph. Set to -1 to disable periodic dumps; a dump can still be triggered manually with SYSTEM DUMP HEAP PROFILE, if heap_profiler_log_sample_rate is enabled.", 0) \
