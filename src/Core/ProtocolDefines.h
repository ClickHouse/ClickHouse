...
/// Version 21 writes version 1 of `Window`, which carries hash partitioning and its spill settings.
...
/// First global query-plan version that writes version 1 of `Window`. A hash-partitioned window is not
/// serialized for an older peer, which would compute it with `WindowTransform` over unsorted input.
static constexpr auto DBMS_MIN_QUERY_PLAN_SERIALIZATION_VERSION_WITH_WINDOW_HASH_PARTITIONING = 21;
