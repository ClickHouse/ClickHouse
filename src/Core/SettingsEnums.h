...
#define IMPLEMENT_SETTING_ENUM_IMPL(NEW_NAME, ERROR_CODE_FOR_UNEXPECTED_NAME, PAIRS_TYPE, ...) \
    ...
    { \
        static const UnorderedMapWithGlobalMemoryTracking<EnumType, String> map = [] { \
            UnorderedMapWithGlobalMemoryTracking<EnumType, String> res; \
    ...
    { \
        static const UnorderedMapWithGlobalMemoryTracking<std::string_view, EnumType> map = [] { \
            UnorderedMapWithGlobalMemoryTracking<std::string_view, EnumType> res; \
            for (const auto & [name, val] : PAIRS_TYPE __VA_ARGS__) \
