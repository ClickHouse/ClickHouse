...
This parameter is useful when you are using formats that require a schema definition, such as [Cap'n Proto](https://capnproto.org/) or [Protobuf](https://developers.google.com/protocol-buffers/). The value depends on the format.
    ...
    DECLARE(String, required_message_name, "", R"(
...
- If `required_message_name` is not specified, the message name is inferred from the `message_name` part of the legacy `format_schema` value.
- If `required_message_name` is specified while using the legacy format, an error will be raised.
)", 0) \
