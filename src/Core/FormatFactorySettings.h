...
- Once cached, identical queries from the same user are not executed to fetch the schema again until the cache is explicitly cleared. A `SYSTEM DROP FORMAT SCHEMA CACHE` issued while a running query is using the cached file keeps that file, so the schema query is not re-executed until the cache is cleared again.
