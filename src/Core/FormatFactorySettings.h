...
Takes precedence over `output_format_parquet_auto_assign_field_ids` for any path that appears in the map. It cannot be combined with a datalake table that provides its own column-id mapping (e.g. Iceberg): such a table's metadata is the source of truth for `field_id`s, so putting this setting into an `ENGINE = Iceberg*` table definition is rejected with `BAD_ARGUMENTS`, and a value coming from the session or the profile is ignored for those tables (including writes through the `iceberg*` table functions). A value supplied in the `SETTINGS` clause of a table definition that freezes format settings (`File`, `URL`, object-storage engines writing Parquet) is validated when the `CREATE TABLE` (or full-definition `ATTACH TABLE`) runs, so an invalid map is rejected at DDL time instead of producing a table that fails every `INSERT`. Such a table takes the setting only from its own definition: an ambient value coming from the session or the profile is not frozen onto a table created while it is set, so it neither changes that table's Parquet output nor leaves the table with a map that was never checked against its columns. Everywhere else — `INTO OUTFILE`, the `file`, `url` and `s3` table functions and the other direct Parquet writes — an ambient value applies and is validated when the Parquet output is written, so an invalid value only fails such a write and does not affect queries that write no Parquet.
...
Disabled by default so that existing Parquet output is unchanged. It cannot be used when writing to a datalake table that provides its own column-id mapping: putting it into an `ENGINE = Iceberg*` table definition is rejected with `BAD_ARGUMENTS`, and a value coming from the session or the profile is ignored for those tables (including writes through the `iceberg*` table functions). A value supplied in the `SETTINGS` clause of a table definition that freezes format settings (`File`, `URL`, object-storage engines writing Parquet) is validated when the `CREATE TABLE` (or full-definition `ATTACH TABLE`) runs, together with `output_format_parquet_column_field_ids`, so conflicts such as ambiguous flattened paths are rejected at DDL time instead of producing a table that fails every `INSERT`. Such a table takes the setting only from its own definition: an ambient value coming from the session or the profile is not frozen onto a table created while it is set, so it neither changes that table's Parquet output nor leaves the table with `field_id`s that were never checked against its columns. Everywhere else — `INTO OUTFILE`, the `file`, `url` and `s3` table functions and the other direct Parquet writes — an ambient value applies and is validated when the Parquet output is written, so an invalid combination only fails such a write and does not affect queries that write no Parquet.
)", 0) \
