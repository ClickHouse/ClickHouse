...
#define FORMAT_FACTORY_SETTINGS(M, ALIAS) \
    ...
    M(UInt64, input_format_parquet_min_read_batch_size_rows, 100000, "Minimum number of rows to read in one thread in Parquet format.", 0) \
    M(UInt64, input_format_parquet_min_read_batch_size_bytes, 256 * 100000, "Minimum number of bytes (uncompressed) to read in one thread in Parquet format.", 0) \
