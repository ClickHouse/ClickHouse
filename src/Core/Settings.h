...
#define LIST_OF_SETTINGS(M)                                            \
    ...
    M(SettingMilliseconds, connect_timeout_with_failover_secure_ms, DBMS_DEFAULT_CONNECT_TIMEOUT_WITH_FAILOVER_SECURE_MS, "Connection timeout for selecting first healthy replica (for secure connections).") \
    ...
    M(SettingJoinAlgorithm, join_algorithm, JoinAlgorithm::HASH, "Forward compatibility setting. It does nothing in current version but allows to upgrade without an error.") \
    ...
    \
    M(SettingBool, enable_scalar_subquery_optimization, false, "If it is set to true, prevent scalar subqueries from (de)serializing large scalar values and possibly avoid running the same subquery more than once.") \
    M(SettingBool, optimize_trivial_count_query, true, "Process trivial 'SELECT count() FROM table' query from metadata.") \
    M(SettingBool, enable_early_constant_folding, true, "Enable query optimization where we analyze function and subqueries results and rewrite query if there're constants there") \
    \
