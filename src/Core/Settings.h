...
#define COMMON_SETTINGS(M) \
    ...
    M(UInt64, background_pool_size, 16, "Deprecated. Number of merges that could be performed in background. Has to be not less than `background_merges_pool_size`. Only has meaning at server startup.", 0) \
    M(Float, background_merges_concurrency_ratio, 3, "Ratio between a number of how many merges could be processed and a number threads to process them. Only has meaning at server startup.", 0) \
    ...
    M(Float, background_mutations_concurrency_ratio, 3, "Ratio between a number of how many mutations could be processed and a number threads to process them. Only has meaning at server startup.", 0) \
