...
#define COMMON_SETTINGS(M, ALIAS) \
    ...
    M(Bool, skip_index_allow_mmap, true, "Use mmap() for skip indexes that support it.", 0) \
    M(Bool, skip_index_allow_seekable_read, true, "Allow skip indexes to do random access to the index file instead of reading it all at once. Has any effect only for some index types, in particular usearch.", 0) \
    \
