...
#define COMMON_SETTINGS(M) \
    ...
    M(Bool, distributed_group_by_merge_finalized, false, "If true, Calculate the final aggregation result in different servers respectively instead of sending aggregation states to initiator node for merging, and then merge these finalized result on initiator node. distributed_group_by_no_merge has higher priority than this setting.", 0) \
