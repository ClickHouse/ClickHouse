...
#define COMMON_SETTINGS(M, ALIAS) \
    ...
    M(Bool, enable_materialized_cte, true, "Enable materialized common table expressions, it will be preferred over enable_global_with_statement", 0) \
    M(Bool, send_materialized_cte_tables_to_remote_shard, false, "Send materialized CTE tables to remote shard in distributed query execution", 0) \
