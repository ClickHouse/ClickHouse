...
#define COMMON_SETTINGS(M) \
    ...
    \
    M(Bool, optimize_rewrite_aggregate_function_with_if, false, "Rewrite aggregate functions with if expression as argument when logically equivalent. For example, avg(if(cond, col, null)) can be rewritten to avgIf(cond, col)", 0) \
