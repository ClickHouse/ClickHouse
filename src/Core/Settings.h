...
#define COMMON_SETTINGS(M, ALIAS) \
    ...
    \
    M(Bool, distributed_insert_prefer_non_readonly_replica, true, "If setting is enabled, INSERT query into Distributed will try to prefer non-read-only replicas. Note, that this is only about the order, read-only replicas will not be excluded completely", 0) \
