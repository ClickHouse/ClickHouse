...
#define COMMON_SETTINGS(M, ALIAS) \
    ...
    M(Bool, use_skipping_final, true, "If enable, remove duplicated rows during FINAL by marking rows as deleted and filtering them later instead of merging rows", 0) \
    \
    \
