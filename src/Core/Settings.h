...
#define COMMON_SETTINGS(M) \
    ...
    M(JoinAlgorithm, join_algorithm, JoinAlgorithm::DEFAULT, "Specify join algorithm: 'auto', 'hash', 'partial_merge', 'prefer_partial_merge', , 'parallel_hash', 'parallel_merge'. 'auto' tries to change HashJoin to MergeJoin on the fly to avoid out of memory.", 0) \
