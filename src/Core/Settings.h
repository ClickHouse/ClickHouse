...
#define COMMON_SETTINGS(M, ALIAS) \
    ...
    M(UInt64, cross_join_max_bytes_to_swap_order, 125_MiB, "Maximal size of the table to swap order in CROSS JOIN.", 0) \
    M(Bool, allow_experimental_cross_join_swap_order, false, "Allow optimization with swapping order of table in CROSS JOIN.", 0) \
    \
