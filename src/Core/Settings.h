...
#define COMMON_SETTINGS(M) \
    ...
    M(Bool, distributed_group_by_merge_finalized, false, "If true, Calculate the final aggregating result in different servers respectively, and then merge these finalized result on initiator node. distributed_group_by_no_merge has highter priority than this setting.", 0) \
