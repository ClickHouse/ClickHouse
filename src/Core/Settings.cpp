...
Two inserts into the same table that run at the same time never write the same key: the key an insert has generated is held until that insert is over, and the other insert steps over it exactly like over an object that already exists. A key becomes visible for the readers of the table only after its object has been written, so a `SELECT` never plans an object that is still being written. Inserts running on different servers do not see each other's keys, and there the generated keys can collide as before.
