...
The minimum number of rows a sliding window frame (e.g. `ROWS BETWEEN N PRECEDING AND CURRENT ROW`) must reach before aggregation over it switches to the incremental algorithm (a tree of partial aggregate states rebuilt with `O(log N)` merges per row) instead of re-aggregating the whole frame for every row. Smaller frames use the plain re-aggregation, which is faster for them. Only aggregate functions whose `merge` is equivalent to sequential aggregation use the incremental algorithm. The incremental algorithm groups the rows of a frame differently, so floating-point results (e.g. `sum`, `avg` and statistics moments like `varSamp` or `corr` over `Float64`) can differ in the last bits, and `argMin`/`argMax` can pick a different row among ties. Setting a very large value effectively disables the incremental algorithm.
)", 0) \
