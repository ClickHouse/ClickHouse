...
Do not make `readonly` itself changeable under `readonly = 1`, because a session can then clear it with `SET readonly = 0`: see [constraints on settings](/concepts/features/configuration/settings/constraints-on-settings#readonly-changeable-in-readonly).
)", 0) \
    \
