...
In a lightweight `UPDATE` or `DELETE` on a server configured with `use_analyzer_for_mutations = 0`, a CTE declared as `MATERIALIZED` is rejected regardless of `force_materialized_cte`.
...
A CTE cannot be materialized when the setting `enable_materialized_cte` is disabled.
A CTE in a view definition (`CREATE VIEW`, `CREATE MATERIALIZED VIEW`, `ALTER TABLE ... MODIFY QUERY`) cannot be materialized either, because the definition is stored with the CTE inlined.
A lightweight `UPDATE` or `DELETE` materializes its CTEs. On a server configured with `use_analyzer_for_mutations = 0` such a statement cannot run a CTE at all, so a CTE declared `AS MATERIALIZED` is rejected there regardless of this setting.
)", 0) \
