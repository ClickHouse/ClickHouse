...
The threshold is currently applied only to the ordinary aggregation pipeline. `GROUP BY ... GROUPING SETS ...` and aggregation reading from aggregate projections keep using the strict resize, so they build one partial state per stream (per grouping set, respectively) regardless of this setting. The same holds for the aggregations that ClickHouse plans on its own rather than from a `GROUP BY` clause, such as the deduplication of `FINAL` under `query_plan_optimize_lazy_final` or the merging of already aggregated states.
)", 0) \
