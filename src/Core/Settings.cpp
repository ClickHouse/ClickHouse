...
Execute `INTERSECT DISTINCT` and `EXCEPT DISTINCT` as a `SEMI LEFT JOIN` or `ANTI LEFT JOIN` on all columns followed by `DISTINCT`, so that they use the join algorithms and their optimizations. `NULL` values match each other like in the set operations. The `ALL` modes are not affected.
)", 0) \
...
Let the preliminary (per-stream) `DISTINCT` give up deduplicating mostly-unique input, freeing its hash table and passing the remaining rows through. The preliminary `DISTINCT` is best-effort by design - duplicates from different streams pass through it even when it deduplicates - and the final `DISTINCT` deduplicates its output again, so abandoning gives up the removal of almost nothing and saves the memory and hashing of a second copy of the unique keys. Not applied when the preliminary `DISTINCT` carries a limit hint (a plain `LIMIT` with no subsequent ordering).
    ...
    DECLARE(Bool, allow_parallel_final_distinct, true, R"(
Run the final `DISTINCT` on all threads: its input streams are hash-partitioned by the `DISTINCT` columns, so that equal rows land in the same stream and every stream is deduplicated independently, instead of merging all streams into one that deduplicates the whole input alone. Not applied when the input is sorted by the `DISTINCT` columns (the sorted deduplication keeps its stream order), or when [max_rows_in_distinct](#max_rows_in_distinct) or [max_bytes_in_distinct](#max_bytes_in_distinct) is set: those limits are enforced by the single `DISTINCT` transform that sees the whole merged result.
)", 0) \
