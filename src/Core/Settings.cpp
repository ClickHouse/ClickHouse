...
When [`enable_automatic_use_uncompressed_cache`](/operations/settings/settings#enable_automatic_use_uncompressed_cache) is enabled, ClickHouse may also enable the cache automatically for eligible local reads from tables in the `MergeTree` family if this setting was not overridden and [`uncompressed_cache_size`](/reference/settings/server-settings/settings/uncompressed-cache#uncompressed_cache_size) is greater than `0`.
