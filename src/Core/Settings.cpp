...
The setting is local to the pipeline where it is applied. Full sorts reconstructed from a serialized query plan use a single merge node because the fan-in is not serialized. Serialization version 21 and newer carries the original value only for validation, so a reconstructed full sort still rejects the invalid value 1. When serializing a full sort or deferred logical join to an older version that cannot carry the validation value, serialization fails closed instead of silently ignoring it.
