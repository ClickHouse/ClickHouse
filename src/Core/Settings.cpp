...
The write is also kept single-stream for overwrite-by-key destinations (`KeeperMap`, `EmbeddedRocksDB`, `Redis`, or a table forwarding to one, including as the target of a dependent materialized view), because for them the result of inserting several rows with equal keys depends on the order in which the rows are committed.
