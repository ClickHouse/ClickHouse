...
Write fully qualified URIs into the metadata files of an Iceberg table instead of storage-relative paths, so that external readers can resolve them: `s3://bucket/key` for S3 and `file:///path` for a local disk given an absolute path. Qualifying does not by itself make a location resolvable: a relative local path keeps the leading separator of the storage-relative form and so names a directory the table does not occupy, a path containing `#` loses everything from that character because a URI parser reads it as the start of a fragment, Azure keeps ClickHouse's `azure` storage type as the scheme, and HDFS carries no nameservice authority. Read when the table is created; every later write inherits the form chosen then.
)", EXPERIMENTAL) \
