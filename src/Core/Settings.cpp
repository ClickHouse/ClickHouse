...
The 'pread_threadpool' method hands a read off to a thread pool, unless the data is already in the page cache, which is checked with the `preadv2` system call and the `RWF_NOWAIT` flag. That check needs Linux 5.11 or newer, and the system call must not be blocked by a `seccomp` profile of a container runtime. When it is unavailable, such reads are performed with 'pread' in the calling thread, without the thread pool, because a thread pool hand-off for every read, including the reads that are served from the page cache, is expensive. The server reports this in `system.warnings` on startup.
