...
Number of granules in a compact part of a `MergeTree` table to read it with a buffer per column (the multibuffer reader). Inside a stripe of a compact part the data of a column is contiguous (see the `MergeTree` settings `compact_parts_max_granules_to_buffer` and `compact_parts_max_bytes_to_buffer`), so the buffer of a column reads it sequentially, the columns are prefetched in parallel, and a query that reads a subset of columns does not read the other columns. On remote filesystems the multibuffer reader makes one read request per column instead of one per granule range. A part is read with a single buffer when it contains both fewer granules than this threshold and fewer granules than the number of columns being read, because in that case the single buffer reader issues fewer read requests than the multibuffer reader. A part written without stripes is always read with a single buffer.
)", 0) \
    \
