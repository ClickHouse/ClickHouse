...
Enables or disables sending of [ProfileEvents](/reference/interfaces/specs/NativeProtocol#profileevents) packets to the client.
...
Send sampled query stack traces to clients that support [profile trace packets](/reference/interfaces/specs/NativeProtocol#profiletraces) in the native protocol, or over HTTP when `framing_output_format` is enabled.
...
To start capture before parsing and analysis, enable delivery through native wire or session settings, or HTTP session or URL settings with framing also enabled there. An opt-in supplied only in SQL `SETTINGS` starts capture immediately after those settings are applied, before subsequent analysis and synchronous interpreter work such as `BACKUP` or `RESTORE`. Samples emitted before that point cannot be recovered. A query-level opt-out disables delivery even when the session or URL enables it.
...
Delivery is best effort: queues and batches are bounded, and samples can be dropped when sampling or the client cannot keep up. Stack symbols are resolved on the server, with an empty string for an unresolved frame. Profile trace packets are sent at most once in `interactive_delay` microseconds during execution, independently of `send_profile_events`, with a final drain before the terminal packet. The final drain waits at most ten seconds for the collector; a timeout discards remaining samples and logs a server warning without changing the query result. When a `Cancel` packet cancels a query over the native protocol, queued samples are discarded without waiting for the collector. Disabling delivery also discards queued samples immediately. Plain HTTP responses do not include profile trace packets.
...
Allows to multiplex different parts of the query response in a single stream: chunks of data, totals and extremes, progress packets, profile events (metrics), profile traces (sampled stacks), and server logs - everything that the native protocol supports.

Framing formats are independent of output formats: they encapsulate bytes produced by any output format, by separating and potentially encoding these chunks of bytes. The concatenation of the payloads of all `data`, `totals` and `extremes` packets is exactly what the output format would have produced without framing. Auxiliary packets (`progress`, `log`, `profile_events`, `profile_traces`, `exception`) are represented as JSON.
...
- `None` - transparently routes everything applicable (data, totals, extremes, progress) to the output format, and ignores everything that is not applicable (metrics, profile traces, logs), so everything works as it is by default.
