...
Short-circuit a `JOIN` whose `ON` condition folds to a constant false (or whose input is already an empty source) by replacing each input side that cannot contribute a row (both sides for `INNER`/`CROSS`/`SEMI`, the non-preserved side for `LEFT`/`RIGHT`) with an empty source, so the non-contributing side is not read.
)", 0) \
