...
When enabled, `if`/`multiIf`/`coalesce`/`ifNull`/`array`/`map` over a set of numeric arguments that has no lossless common type (for example a `Decimal` and a `Float64`, or an `Int64` and a `Float64`) resolves to a numeric supertype (`Float64`) instead of a `Variant`. This allows the result to be used directly with value-combining aggregate functions like `sum`, `avg`, `min` and `max`, at the cost of possible precision loss in the conversion. One exception: a nullable numeric `Map` key stays a `Variant`, because `Map` keys cannot be `Nullable`; `map` resolves the numeric supertype for non-nullable keys and for values (nullable or not). When disabled (the default), a `Variant` is produced as before (subject to `use_variant_as_common_type`).
)", 0) \
