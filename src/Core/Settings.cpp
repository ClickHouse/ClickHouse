...
Scan the table once for `INNER`/`LEFT` self-joins and replay from an in-memory buffer instead of performing a second scan. Only fires when both sides resolve to the same `MergeTree` table with identical column lists, the same storage snapshot, and no `FINAL`, `SAMPLE`, `PREWHERE`, row-level filters, parallel replicas, vector search, or pushed-down `WHERE` filters.
)", 0) \
