...
Two inserts into the same table that run at the same time never write the same key: the key an insert has generated is held until that insert is over, and the other insert steps over it exactly like over an object that already exists. A key becomes visible for the readers of the table only after its object has been written, so a `SELECT` never plans an object that is still being written. A truncating insert that runs at the same time as an insert split by size does not delete the objects that insert has already written, and steps over their keys as well. Inserts running on different servers do not see each other's keys, and there the generated keys can collide as before.
...
Two inserts into the same table that run at the same time never write the same name: the name an insert has generated is held until that insert is over, and the other insert steps over it exactly like over a blob that already exists. A name becomes visible for the readers of the table only after its blob has been written, so a `SELECT` never plans a blob that is still being written. Inserts running on different servers do not see each other's names, and there the generated names can collide as before.
...
Two inserts into the same table that run at the same time never write the same name: the name an insert has generated is held until that insert is over, and the other insert steps over it exactly like over a file that already exists. A name becomes visible for the readers of the table only after its file has been written, so a `SELECT` never plans a file that is still being written. Inserts running on different servers do not see each other's names, and there the generated names can collide as before.
