...
Maximum total input (prompt) tokens across all AI function API calls in a single query. Tracked cumulatively from provider responses. Note that this limit may be exceeded by up to one call's worth of input tokens per in-flight request, since a call's input tokens are not known until its response arrives. Set to 0 to disable.
...
Maximum number of HTTP requests that AI functions may dispatch per query. This is a hard cap on the total number of requests across the whole query, counting every AI function it contains. Set to 0 to disable.
)", EXPERIMENTAL) \
