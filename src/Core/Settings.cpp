...
identity for `Float32`/`Float64`: `-0.0` compares equal to `0.0`, and every `NaN` bit pattern compares equal
to every other. A partition expression can tell exactly those values apart, so rows the comparator treats as
