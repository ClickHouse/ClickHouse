...
Prune granules on the probe (left) side of a JOIN with the runtime filter collected from the build (right) side, see `enable_join_runtime_filters`. The pruning happens as a dynamic skip index analysis while the probe side parts are read (see `use_skip_indexes_on_data_read`), so it can use a filter that is only known at runtime.

Only a join key that is a primary key column of the probe side, or is covered by a `minmax`, `set` or `bloom_filter` skip index there, can be pruned. If the runtime filter kept the exact key values, the pruning predicate is an `IN` set of them; if the build side had more keys than `join_runtime_filter_exact_values_limit` and the filter became a bloom filter, only the recorded key range is used, which prunes less. The key range is recorded only for integer and date or date-time keys, so for a join key of any other type (such as `String`, a floating-point number, `Decimal` or `UUID`) the pruning stops once the filter loses the exact values. A `bloom_filter` skip index is the exception: it can test the exact values only, and it tests them one by one for every granule, with false positives that add up over the set, so it is used only while the filter keeps at most `join_runtime_filter_exact_values_limit / 100` exact values (100 with the default limit). A key that is covered by such an index alone is not pruned at all above that cap or once the filter loses the exact values. With `use_primary_key = 0` a primary key column is pruned only by a skip index that covers it.

The build side records the key range of the filter only when some probe side read can prune with it - which, for the same reason, a key covered only by a `bloom_filter` index cannot. The one exception is `use_skip_indexes_on_data_read = 0`: it switches the pruning off, but the key range is still recorded, because that setting is decided when the pipeline is built and not when the plan is optimized.
)", 0) \
