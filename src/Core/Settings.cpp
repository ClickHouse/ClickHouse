...
Allow query plan lazy materialization optimization for `LIMIT WITH TIES` if the limit value does not exceed `query_plan_max_limit_for_lazy_materialization`. Takes effect only if `query_plan_optimize_lazy_materialization` is enabled. Note that `query_plan_max_limit_for_lazy_materialization` bounds the written `LIMIT`, not the number of rows `LIMIT ... WITH TIES` returns: the tied rows past the limit are read lazily too. The optimization needs the `LIMIT` and the reading step in the same query plan fragment, and `LIMIT ... WITH TIES` never gets a preliminary shard-local limit, so this setting has no effect on the remote side of a distributed query or under plan-based parallel replicas.
)", 0) \
