...
Not applied when the input is already globally sorted, because then `DISTINCT` keeps the order of its input and the repartitioning would break it, and not applied when there is a `LIMIT`, because a single stream returns the first values of the input rather than an arbitrary subset. [max_rows_in_distinct](#max_rows_in_distinct) and [max_bytes_in_distinct](#max_bytes_in_distinct) keep their global meaning: the streams hold disjoint parts of one `DISTINCT` set, so their sizes are added up and the limits are checked against the total.
)", 0) \
