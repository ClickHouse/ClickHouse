...
Maximum size in bytes of the per-query reverse dictionary lookup cache used by dictGetKeys. The cache stores serialized key tuples per attribute value to avoid re-scanning the dictionary within the same query. When the limit is reached, entries are evicted using LRU. Set to 0 to disable.
)", 0) \
