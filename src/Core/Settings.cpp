...
This is internal setting that should not be used directly and represents an implementation detail of the 'parallel replicas' mode. This setting will be automatically set up by the initiator server to the table expression it designated for coordinated reading. A replica that plans the same query and designates a different table expression - which can happen when the set of underlying tables of a `Merge` table differs on it - fails the query instead of reading a different table in full. The setting is sent only for queries that read a `Merge` table (or the `merge` table function) with `parallel_replicas_allow_merge_tables` enabled, and is marked important: an older replica that does not know it fails such a query instead of silently re-planning without the initiator's designation.
)", IMPORTANT | BETA) \
    DECLARE(String, parallel_replicas_freshness_checked_tables, "", R"(
This is internal setting that should not be used directly and represents an implementation detail of the 'parallel replicas' mode. When a replica lagging behind by more than `max_replica_delay_for_distributed_queries` must not participate in coordinated reading (falling back to stale replicas is switched off), the initiator sets it to the replicated tables whose replication delay it checked when it selected the replicas. The set of tables matched by a `Merge` table is enumerated again at reading time and may have grown since that check, so reading a replicated table absent from this snapshot fails the query instead of serving data whose freshness nobody verified. The setting is sent only for queries that read a `Merge` table (or the `merge` table function) with `parallel_replicas_allow_merge_tables` enabled, and is marked important: an older replica that does not know it fails such a query instead of silently ignoring the snapshot.
)", IMPORTANT | BETA) \
    DECLARE(String, parallel_replicas_merge_child_tables, "", R"(
This is internal setting that should not be used directly and represents an implementation detail of the 'parallel replicas' mode. Every node of a query over a `Merge` table (or the `merge` table function) must read it through the same set of child tables: a non-designated (sibling) `Merge` leaf is read in full by every participating node, so a diverging set joins different rows on different nodes, and when the initiator builds no local plan (`parallel_replicas_prefer_local_replica` is disabled), the reading coordinator has no pinned snapshot replica and a child table matched by no participating replica would never be announced to it - the rows of that table would silently vanish from the result. The initiator sets this to the child table sets of the `Merge` tables the query reads, keyed by the table expression each of them belongs to, and a node whose `Merge` table resolves to a child set different from the initiator's set for the same table expression fails the query instead. The coordinated read is exempt when the initiator pinned a snapshot replica (announced by a leading `pinned;` marker), because the coordinator then ignores streams the pinned replica did not announce, and the pinned replica reads the ones nobody else announced in full. The setting is sent only for queries that read a `Merge` table with `parallel_replicas_allow_merge_tables` enabled, and is marked important: an older replica that does not know it fails such a query instead of silently ignoring the check.
)", IMPORTANT | BETA) \
