...
Covers `SELECT` (including subqueries, CTEs, and `IN`/`GLOBAL IN table` right-hand-side table references), `INSERT`, and queries addressing a single table by name, such as `SHOW CREATE TABLE`, `EXISTS TABLE`, `CHECK TABLE`, `OPTIMIZE TABLE`, or `ALTER TABLE`. Other query classes — e.g. `SHOW COLUMNS`, `SHOW INDEXES`, `DESCRIBE`, `RENAME`, `EXCHANGE TABLES`, and all `BACKUP` and `RESTORE` forms (`RESTORE TABLE` can fail on the backup contents before ever touching its local destination table, and `BACKUP TABLE` validates and opens its destination before ever reading the source table, so either can fail without touching the named local table; the whole-database and whole-server forms additionally name no explicit table and expand into per-table work only during execution) — are deliberately out of scope and never trigger the `DETACH`/`ATTACH`. The target of a plain `CREATE TABLE`, `CREATE TABLE IF NOT EXISTS`, `ATTACH TABLE`, or `UNDROP TABLE` statement is also out of scope: when a table with that name already exists, such a statement fails or turns into a no-op without touching the existing table, so reattaching it would give the statement a side effect it never has (a `CREATE OR REPLACE`/`REPLACE` form does replace the existing table, but it validates the new definition before the replacement path touches the existing destination — a populating `SELECT`, an `AS src` source, a table function source, or a `CLONE AS` clause is analyzed first, and even a source-less form can be rejected as incomplete — and whether that validation passes cannot be predicted, so every replacing destination stays out of scope as well; the `AS src` source and the external `TO dst` view target of any `CREATE` form are covered in their own right). The tables a `CREATE` statement reads — its `AS src` source, its populating `SELECT`, its `TO dst` target — are out of scope whenever the statement stops on its own destination first: when the destination-side access check (`CREATE TABLE`, `CREATE VIEW`, `CREATE DICTIONARY`, `TABLE ENGINE`, ...) rejects it, when a plain create finds the destination name already taken, when a stub `ATTACH` (no engine and no column list) carries clauses that the stored metadata would override (`AS src`, `AS SELECT`, `TO dst`, `EMPTY`, `CLONE`, ...) and is rejected with `BAD_ARGUMENTS`, or when a temporary-table form is rejected on its syntax alone (`ATTACH` of a temporary, a database-qualified temporary, or a temporary created `ON CLUSTER`), it fails or no-ops without ever reading them. A non-`ATTACH` `CREATE` carrying an external `TimeSeries` `SAMPLES`/`TAGS` target is treated the same way conservatively: the target is resolved and type-checked before any source is read, and whether that check passes cannot be predicted, so such statements never trigger the `DETACH`/`ATTACH`. An unqualified `IN` right-hand side that matches an expression alias the reference can resolve, such as `rhs` in `WITH (1, 2) AS rhs SELECT 1 IN rhs`, names no table and is out of scope too; alias visibility is scoped, so an alias declared in a nested `SELECT` does not take the name away from an enclosing `IN` (`SELECT * FROM lhs WHERE a IN rhs AND 1 = (SELECT 1 AS rhs)` reads the table `rhs`). Any statement carrying an `ON CLUSTER` clause is out of scope entirely: on the initiator the interpreter delegates it to the distributed DDL queue before performing any local table operation (the local host may not even be in the target cluster), and the per-host executions replayed by the DDL worker are not initial queries and are not randomized either. For the same reason, the shapes of `CREATE INDEX` that never reach the `ALTER TABLE ... ADD INDEX` they are rewritten to — `CREATE UNIQUE INDEX` without `create_index_ignore_unique`, and `CREATE INDEX` without a `TYPE`, which either fails or is a no-op — are out of scope, while `DROP INDEX` and the rewritten `CREATE INDEX` are covered; so are `CREATE HYPOTHETICAL INDEX` and `DROP HYPOTHETICAL INDEX`, which only read the table's metadata and update the session-local hypothetical-index store.

The `DETACH`/`ATTACH` happens only when the current user has both the grants the internal `DETACH`/`ATTACH` requires (`DROP TABLE`, `CREATE TABLE`, and `TABLE ENGINE` where applicable) and the access the query itself is going to check on the collected tables (e.g. `SELECT` from all of the tables used in a `SELECT` query), so a query that would be rejected with `ACCESS_DENIED` does not produce `DETACH`/`ATTACH` side effects. Otherwise, the query is executed without the randomization. Tables of databases that do not support detaching tables (e.g. `Replicated`, `Dictionary`, `SQLite`, `Backup`, `MaterializedPostgreSQL`, and the virtual `Filesystem`, `S3`, `HDFS`, `URL`, and data lake databases) are never touched; such databases are rejected before any of their tables is resolved, because in the virtual databases the resolution itself is not free of side effects (e.g. a `URL` database infers the table structure from the data and requires the read source grant already for the resolution).

A table that another query is using at that moment is skipped as well: the internal `DETACH TABLE ... SYNC` waits for the detached table to stop being referenced, with no deadline of its own, so reattaching a table held by a concurrent long-running query would block the current query until that one finishes. Whether the table is idle is decided by briefly probing its exclusive lock.

All of the eligibility checks, and the `DETACH`/`ATTACH` pair itself, are best-effort and point-in-time: the pair is issued by table name and is not serialized against other DDL on that name, so a concurrent `CREATE`, `RENAME`, `EXCHANGE`, or `DROP` of the same name may make it act on a different table than the one collected during preflight, or skip the reattach. The feature does not guarantee that the exact table observed during collection is the one reattached.
)", EXPERIMENTAL) \
