...
Allows creating, copying, adding, or changing [`CODEC`](/reference/statements/create/table/codec#tuple-element-codecs) declarations on stored [`Tuple`](/reference/data-types/tuple) elements.
Existing Tuple element codec metadata can still be replayed by short `ATTACH`, startup, replication, or restore, and can be read, preserved, or have declarations removed when this setting is disabled.
)", EXPERIMENTAL) \
