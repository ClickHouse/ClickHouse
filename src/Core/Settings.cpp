...
Used in workload scheduling. Controls the order in which queries' memory reservations are evicted when their workload is under memory pressure. Among the queries that can free memory, a query with a higher `memory_eviction_score` is evicted before a query with a lower one; queries with an equal `memory_eviction_score` are evicted largest-reservation-first. A query that currently holds no reserved memory is not evicted while one that holds memory exists, regardless of score. The value may be negative to make a query less likely to be evicted.
A value of `0` (the default) leaves the eviction order unchanged.
This setting takes effect only if MEMORY RESERVATION resource is created.
)", EXPERIMENTAL) \
