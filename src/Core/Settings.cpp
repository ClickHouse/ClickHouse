...
A `Variant` is not `Nullable`, so the `-Null` combinator is never applied to it, and before ClickHouse 26.9 the aggregate functions that accept a `Variant` argument natively (`count`, `any`, `groupArray`, `groupConcat`, the `uniq` family, ...) processed those rows as ordinary values: `count` counted them, `any` could return NULL from a group that has non-NULL values, `groupArray` stored the NULLs, and `uniq` counted NULL as a distinct value.

Set this to `false` (or `SET compatibility = '26.8'`) to restore the pre-26.9 behavior. This setting does not affect `-SimpleState` or declared `SimpleAggregateFunction(..., Variant(...))` columns: they always retain their historical behavior so their raw values can be safely re-aggregated during table loading and background merges. Note that this setting only controls how new values are aggregated; it cannot change the meaning of an `AggregateFunction(f, Variant(...))` state that has already been written, because the state representation is the same in both modes.
