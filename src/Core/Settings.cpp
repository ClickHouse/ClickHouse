...
If enabled and the `schema` key of an Iceberg manifest file header carries a schema that differs from the metadata.json schema with the same schema-id, the metadata.json schema is used and the manifest header copy is ignored with a warning. This holds whichever of the two copies is read first: a read registers the metadata.json schemas before walking manifests, but the `remove_orphan_files` and `expire_snapshots` commands and mutation validation walk manifests before any metadata.json schema is known, and a manifest header schema registered there is replaced once metadata.json is read by an operation that has this setting enabled. The choice is made by the operation that meets the conflict, so a table object shared between queries with different values of this setting still behaves per query. If disabled, such a conflict fails the query with an `ICEBERG_SPECIFICATION_VIOLATION` error.

The manifest header schema is only a copy of the table schema at the time the manifest was written, and some writers (e.g. AWS S3 Tables maintenance jobs) have been observed storing degraded copies there. Other query engines resolve schemas from metadata.json and ignore divergent header copies, so the default follows them. A conflict between two metadata.json schema definitions still always fails the query. Two manifest header copies of a schema-id that disagree with each other are tolerated only until metadata.json settles which one is right: if metadata.json does not define that schema-id at all, any use of the schema fails with an `ICEBERG_SPECIFICATION_VIOLATION` error, since neither header copy is authoritative.
)", 0) \
