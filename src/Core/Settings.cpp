...
Require joined subqueries and table functions to have an alias, but only when a missing alias would introduce a real ambiguity: that is, when the unaliased subquery or table function exposes a column whose name also occurs in another table expression of the same join, so that a bare identifier could not be qualified unambiguously. When there is no such name collision the missing alias is harmless and is allowed. Set to `0` to never require an alias.
)", 0) \
