...
When enabled, `if`/`multiIf`/`coalesce`/`ifNull`/`array`/`map` over a set of numeric arguments that has no lossless common type (for example a `Decimal` and a `Float64`, or an `Int64` and a `Float64`) resolves to a numeric supertype (`Float64`) instead of a `Variant`. This allows the result to be used directly with value-combining aggregate functions like `sum`, `avg`, `min` and `max`, at the cost of possible precision loss in the conversion. Two exceptions: a nullable numeric `Map` key stays a `Variant`, because `Map` keys cannot be `Nullable` (`map` resolves the numeric supertype for non-nullable keys and for values, nullable or not); and the expression form `CASE expr WHEN ... END` (function `caseWithExpression`) is outside this setting, so such a `CASE` over numeric branches with no lossless common type still throws `NO_COMMON_TYPE` regardless of this setting (use the searched form `CASE WHEN ... END`, which goes through `multiIf`). When disabled (the default), a `Variant` is produced as before (subject to `use_variant_as_common_type`).
)", 0) \
