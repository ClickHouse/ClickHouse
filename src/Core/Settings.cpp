...
Previews are emitted only while the intermediate state is small: the aggregation state must be single-level, not spilled to disk, with at most `query_result_previews_max_result_rows` keys and at most `query_result_previews_max_result_bytes` of aggregate function states; a sorting must have a `LIMIT` of at most `query_result_previews_max_result_rows` rows. The frequency of previews is limited by `query_result_previews_min_interval_ms`, `query_result_previews_min_rows`, and `query_result_previews_min_bytes`.

For queries with `WITH TOTALS`, `ROLLUP`, `CUBE`, `GROUPING SETS`, window functions, `DISTINCT`, `LIMIT BY`, `WITH FILL`, and for the merging stages of distributed queries, previews are not emitted.
)", EXPERIMENTAL) \
...
The maximum number of rows in the intermediate state (the number of aggregation keys, or the `LIMIT` of a sorting) for which previews of the query result are emitted (see `query_result_previews`). When the state grows above this threshold, previews stop.
)", EXPERIMENTAL) \
    DECLARE(UInt64, query_result_previews_max_result_bytes, 1048576, R"(
The maximum size in bytes of the intermediate state (the memory of aggregate function states) for which previews of the query result are emitted (see `query_result_previews`). When the state grows above this threshold, previews stop.
)", EXPERIMENTAL) \
    \
