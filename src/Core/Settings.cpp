...
For `hash` and `parallel_hash`, compression kicks in adaptively, using the same memory-pressure signal as block shrinking: when the join data accounts for more than half of `max_bytes_in_join`, or the query memory growth exceeds half of the query memory limit (`max_memory_usage`). For `parallel_hash`, the `max_bytes_in_join` trigger is evaluated against the join's total size across all its internal slots - the same total the limit itself is enforced on - so a build side spread evenly over the slots compresses before it can exceed the limit. The `max_memory_usage` (and available-memory) trigger is likewise evaluated per logical join: all slots share one query-memory baseline, so the query-memory growth of the whole build side drives compression rather than each slot's own later baseline. With compression enabled, `parallel_hash` also distributes the build side to its slots by copying each slot's rows instead of sharing one full block among all slots through row selectors: each slot then compresses and accounts exactly the rows it owns, whereas compressing a shared block would keep one compressed copy of the whole block per slot. When neither limit is set, compression still activates automatically under real memory pressure, the same way external aggregation and sorting do: the budget is derived from the memory still available on the server (from `max_server_memory_usage` and the related limits), so a large build side is compressed as it approaches exhausting the available memory rather than staying uncompressed until an out-of-memory condition. If the server has no memory limit configured at all, there is no "before out of memory" signal and only the explicit `max_bytes_in_join` / `max_memory_usage` limits apply. When automatic external spilling is configured (`max_bytes_before_external_join` or `max_bytes_ratio_before_external_join`) - even as the only memory budget - compression also triggers at the point the join would otherwise switch to disk-based `grace_hash` (half of the effective threshold), so a compressible build side shrinks below the threshold and stays in memory instead of spilling; only if the compressed data grows back to the threshold does the join spill.
...
This trades probe-time CPU (right-side blocks must be decompressed on the fly) for lower peak memory, allowing larger joins to run in memory before reaching an out-of-memory condition or spilling to disk. Decompressed data is not kept around: each output batch decompresses the distinct stored blocks it references, holds them only while that batch is materialized, and releases them as soon as it is finished. The working set held at once within a batch is additionally bounded (64 MiB): the output rows are copied grouped by stored block, so each distinct block a batch references is decompressed at most once per batch whatever order the rows reference the blocks in, and blocks already copied into the output are released early once the working set outgrows the bound - a batch referencing more decompressed data than the bound (for example, a build side made of many tiny blocks with a batch matching a different block per row) never re-materializes an unbounded part of the build side. The few paths that must copy in row order (for example `joinGet`) instead stop enforcing the bound for the rest of a batch whose reference pattern would otherwise re-decompress a released block on every block switch, keeping the working set bounded by what that one batch references. The working set is accounted by the query memory tracker as usual.
)", 0) \
