...
- parallel_full_sorting_merge
 ...
 Only plain equality joins whose key types hash consistently with the merge-join comparison are sharded by the hash of the keys. This hash-sharding is skipped for `ASOF` joins and for floating-point / `JSON` / `Object` / `Dynamic` key types, in which case the join runs as a single `full_sorting_merge`. Sides that are already sorted (a MergeTree read in order, or any pre-sorted input) are sharded too: they are scattered by the hash of the join keys while preserving order, so each shard only finishes the sort instead of redoing it — the low-cost in-order read is kept and the merge join still runs on all threads. (An alternative source-side sharding by primary-key ranges, without a shuffle, is available when `query_plan_join_shard_by_pk_ranges` is enabled.) Under `make_distributed_plan` this is an initiator-side restriction only: the hash-sharding rewrite is skipped while the initiator builds the distributed plan, because the scattered sort is not serializable for remote execution, but the local single-fragment plan and the per-worker fragments re-optimize with `make_distributed_plan` disabled and can still take the sharded `parallel_full_sorting_merge` path.
