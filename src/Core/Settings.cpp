...
The setting is local to the pipeline where it is applied. Full sorts reconstructed from a serialized query plan use a single merge node because the fan-in is not serialized. Serialization version 15 and newer carries the original value only for validation, so a reconstructed full sort still rejects the invalid value 1. When serializing a full sort or deferred logical join to an older version that cannot carry the validation value, serialization fails closed instead of silently ignoring it.
...
The value 1 is not allowed: a merge node with a single input would not reduce the number of streams, so no tree could be built. Attempting to build a full-sort pipeline with this value throws `BAD_ARGUMENTS`, on the initiator as well as on a worker that rebuilds the full sort from a serialized plan, so the value is rejected on distributed and `serialize_query_plan` paths too. Note that the error is raised when the query pipeline is built, not when the setting is assigned, so `SET max_streams_per_hierarchical_merge = 1` itself succeeds.
)", 0) \
