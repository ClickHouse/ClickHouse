...
The writing side is parallelized only when it is safe to do so; otherwise it stays single-stream and this setting has no effect on it. In particular, the write is kept single-stream when a destination table (or a table it forwards to) deduplicates inserted blocks and insert deduplication is enabled for the query (see [`deduplicate_insert`](#deduplicate_insert)), when the destination has dependent materialized views (unless [`parallel_view_processing`](#parallel_view_processing) is enabled), and always for `Buffer` destinations. A `Distributed` destination is conservatively treated as deduplicating, because its remote tables are not inspected, so its write fans out only when insert deduplication is disabled for the query.
