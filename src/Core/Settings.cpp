...
- 1 — Enabled.
    ...
    DECLARE(Bool, use_column_refs_for_primary_key_index_analysis, false, R"(
Reference the primary key index columns directly during primary key index analysis in `MergeTree` tables, instead of materializing their values as `Field`.

Applies on top of `use_lightweight_primary_key_index_analysis`, which selects the key columns to analyse; this setting changes how the values of those columns are represented. A `Field` of a `String` or `Decimal` key column allocates and copies the value for every mark boundary, while a reference is a pointer into the index column plus a row number.

Possible values:

- 0 — Disabled. Key values are materialized as `Field`.
- 1 — Enabled.
)", EXPERIMENTAL) \
