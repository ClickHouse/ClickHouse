...
To capture parsing, planning, and analysis, enable delivery before the query is analyzed: use native wire or session settings, or HTTP session or URL settings with framing also enabled there. An opt-in supplied only in the query's SQL `SETTINGS` clause starts capture after analysis. A query-level opt-out disables delivery even when the session or URL enables it.
