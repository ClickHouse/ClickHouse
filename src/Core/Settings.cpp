...
The fallback does NOT apply while resolving `JOIN USING` column lists, because the resolved `ColumnNode` still reports its canonical (dotted) name from `getColumnName`, and `USING` matches left/right keys by that name. In a `USING` context, the short-name fallback is suppressed so the query behaves the same as master — `JOIN ... USING (f1)` against a side that only exposes `f1` via this fallback raises `UNKNOWN_IDENTIFIER`. Use an explicit `AS f1` alias or the canonical dotted name to make such a `USING` join work.
    )", 0) \
    \
