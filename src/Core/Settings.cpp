...
- sorted_merge
 ...
 A merge join that relies on the tables' order instead of sorting: it is available only when both join inputs can be efficiently read in the order of the join keys, so the pre-join sorts become cheap or disappear. Such an input is currently detected only for tables of the `MergeTree` family read directly, with a primary key starting with the join keys; other ordered sources (for example, `Merge` or object-storage tables) are not considered, and the selection falls through to the next algorithm for them. A null-safe equality (`<=>`) on nullable keys is not eligible either: such keys are compared wrapped into `tuple(...)`, and a wrapped key cannot be read in table order. When the tables' order cannot be exploited, the algorithm is simply not selected and the next algorithm in the list is used. This makes it meaningful as a high-priority preference: `sorted_merge,parallel_hash` uses the streaming merge join exactly when it is certainly beneficial and a hash join otherwise. If no other algorithm is listed and the order cannot be exploited, the query fails with an error. The join itself runs in a single stream: the name encodes the parallelism choice, so even enabling `query_plan_join_shard_by_pk_ranges` does not shard a join selected as `sorted_merge` - use `parallel_sorted_merge` for the sharded variant.
