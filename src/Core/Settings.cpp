...
The resolved concrete type is materialized in the table definition, so reading an existing table does not depend on the value of this setting. This covers the expressions a `CREATE` or `ALTER` persists: column declarations, `DEFAULT` / `MATERIALIZED` / `ALIAS` expressions, `ORDER BY`, `PRIMARY KEY`, `PARTITION BY`, `SAMPLE BY` and TTL expressions, skipping indices, constraints, projections, dictionary attributes, the query of `CREATE TABLE ... AS SELECT` and of views, and mutation expressions. It also covers type names that appear inside a string literal rather than as a type: cast expressions (`CAST(x AS UUID)`, `x::UUID`, which the parser canonicalizes into `CAST(x, 'UUID')`), and the functions that declare their result type by name, such as `reinterpret(x, 'UUID')`, `defaultValueOfTypeName('UUID')` and the `JSONExtract` family (`JSONExtract(json, 'UUID')`, `JSONExtractKeysAndValues`, and their case-insensitive variants). It also covers the schema of a table function given as a whole columns declaration list in a string, such as `file('data.csv', 'CSV', 'id UUID')`, `format`, `input` and `generateRandom`; such a string is materialized at creation time, and it is never resolved through this setting when an already persisted definition is executed.

The type names carried in a string are recognized while the query is normalized, before it is persisted, where the expression evaluator is not available. Such a string therefore has to be either a string literal, or a constant expression built from the string functions the normalizer folds on its own: `concat`, `replace`, `replaceOne`, `replaceAll`, `upper`, `ucase`, `lower` and `lcase`. A type name produced by any other expression, such as `CAST(x, trim(' UUID '))`, is opaque at that point and keeps the historical `UUID`, the same as a type name built at run time from a non-constant expression.

The explicit type names `UUID1` (an alias of `UUID`) and `UUID2` are not affected by this setting.
)", 0) \
