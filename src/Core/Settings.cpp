...
Maximum number of trials allowed for random distribution functions such as `randBinomial` and `randNegativeBinomial`. This prevents extremely long computation times with large trial counts. `0` disables the limit; parameters for which the sampler does not terminate at all are still rejected.
)", 0) \
    DECLARE(Float, max_rand_distribution_parameter, 1e6, R"(
Maximum value for distribution shape parameters in random distribution functions such as `randChiSquared`, `randStudentT`, and `randFisherF`. This prevents extremely long computation times with extreme parameter values. `0` disables the limit; parameters for which the sampler does not terminate at all are still rejected.
)", 0) \
