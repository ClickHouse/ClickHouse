...
A value of `0` means use half of the size limit of the columns cache currently in effect, as reported by the `ColumnsCacheSizeLimit` metric. That limit is the server-level `columns_cache_size`, but it is lowered while the rest of the server is short of memory (see `columns_cache_free_memory_ratio`), and the default budget follows it, so a query is not allowed to write more than a shrunk cache can hold. With the default `columns_cache_size_ratio`, half of the limit is the size of the probationary segment of the cache, so the data of a query that passes the gate can be cached completely in one pass.
...
A value of `0` means use half of the size limit of the columns cache currently in effect, as reported by the `ColumnsCacheSizeLimit` metric: the server-level `columns_cache_size`, lowered while the rest of the server is short of memory.
)", BETA) \
