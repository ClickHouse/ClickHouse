...
Maximum number of provider requests an AI function issues at the same time, within one block of rows. The default `1` issues them one at a time.

AI function calls are network-bound, so throughput is `concurrency / request latency` rather than a function of CPU. This setting is the only thing that determines that concurrency: it applies identically to a function evaluated in an `INSERT ... SELECT` and to one evaluated inside a materialized view, and does not depend on how the data happens to be laid out in parts, on block sizes, or on `parallel_view_processing`.

The concurrency applies per AI function call per block, so a query whose pipeline runs several streams may have more requests in flight than this. Use a settings profile constraint (`<constraints><ai_function_max_concurrent_requests><max>...</max></...>`) to put a ceiling on it that a query cannot raise.
...
- A whole batch of requests is dispatched before any of their errors is seen, so a query that fails on its first row may still have issued, and been billed for, up to this many requests.
