...
The setting takes effect only when the planner builds the hash-based pre-aggregation resize stage for a `GROUP BY`, that is, when several input streams are redistributed across the aggregating streams before the partial hash tables are merged. Query shapes without such a stage ignore both thresholds and keep their existing pipeline; enabling the setting does not change them and nothing in `EXPLAIN PIPELINE` indicates otherwise. Those shapes are:

- aggregation in order of the sorting key (when `optimize_aggregation_in_order` applies): each input stream is aggregated as is and no pre-aggregation resize exists;
- reads from storages that already distribute rows evenly across streams, such as `Memory` or `system.numbers` / `numbers_mt`: the planner skips the pre-aggregation resize for them;
...
The setting takes effect only when the planner builds the hash-based pre-aggregation resize stage for a `GROUP BY`; the query shapes that have no such stage (aggregation in order of the sorting key, storages with evenly distributed reads such as `Memory` or `system.numbers`, aggregation over independent partitions, single-stream reads, `GROUPING SETS`, aggregate projections, and the aggregations ClickHouse plans on its own) ignore both thresholds. See `min_rows_per_stream_for_gradual_resize` for the full list.
)", 0) \
