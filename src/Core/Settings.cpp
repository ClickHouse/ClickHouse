...
Functions `hasAnyTokenPrefix`, `hasAnyTokenLike`, `hasAllTokenLike` and `hasTokenMatch` use the text index only through this dictionary scan: with the setting disabled they skip no granules and are evaluated on the column. The setting does not change their result.
)", 0) \
...
Does not apply to `hasAnyTokenPrefix`, `hasAnyTokenLike`, `hasAllTokenLike` and `hasTokenMatch`, whose dictionary scan is bounded by `text_index_like_max_matched_tokens` instead.
...
Applies to `LIKE`, `ILIKE`, `startsWith`, `endsWith`, `hasAnyTokenPrefix`, `hasAnyTokenLike`, `hasAllTokenLike` and `hasTokenMatch`.
...
Maximum number of distinct dictionary tokens that the patterns of `hasAnyTokenPrefix`, `hasAnyTokenLike`, `hasAllTokenLike` and `hasTokenMatch` may match
