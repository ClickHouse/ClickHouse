...
Covers `SELECT` (including subqueries and CTEs), `INSERT`, `BACKUP`/`RESTORE`, and queries addressing a single table by name, such as `SHOW CREATE TABLE`, `EXISTS TABLE`, `CHECK TABLE`, `OPTIMIZE TABLE`, or `ALTER TABLE`. Other query classes — e.g. `SHOW COLUMNS`, `SHOW INDEXES`, `DESCRIBE`, `RENAME`, `EXCHANGE TABLES` — are deliberately out of scope and never trigger the `DETACH`/`ATTACH`.
)", EXPERIMENTAL) \
