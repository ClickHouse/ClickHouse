...
The two-stage search reads the quantized codes in addition to the full-precision vector, so it applies only where the vector can be read lazily for the shortlisted rows: it needs the analyzer, setting `query_plan_optimize_lazy_materialization` and a `LIMIT` (including any `OFFSET`) no greater than `query_plan_max_limit_for_lazy_materialization` (`0` means unbounded), and it does not apply to a table with patch parts, or to a query using `SAMPLE`, `FINAL` on an engine other than `ReplacingMergeTree`, or a filter or row policy that reads the vector column. Otherwise the query runs the exact full-precision scan.
)", 0) \
