...
Enable multithreading after evaluating window functions to allow parallel stream processing
    ...
    M(Bool, query_plan_optimize_lazy_materialization, true, R"(
Use query plan for lazy materialization optimization
)", 0) \
    M(UInt64, query_plan_max_limit_for_lazy_materialization, 10, R"(Control maximum limit value that allows to use query plan for lazy materialization optimization. If zero, there is no limit
)", 0) \
