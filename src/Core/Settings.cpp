...
Some condition shapes (space-filling curves, polygon predicates, non-collapsed `IN`, relaxed predicates such as `match`) are not eligible for the vectorized path and fall back to per-granule evaluation. The same applies to `Nullable` and `LowCardinality(Nullable)` index column types: comparisons over them produce `Nullable` results that the vectorized combinators cannot consume, so such indexes always use per-granule evaluation regardless of the condition shape.
