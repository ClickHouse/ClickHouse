...
Has no effect when the destination table has no dependent materialized views, has no sorting key, or when some element of the sorting key is an expression rather than a plain column. Materialized views targeting an engine whose result depends on the positional order of rows with equal sorting key — `CollapsingMergeTree`, `CoalescingMergeTree`, `GraphiteMergeTree`, or `ReplacingMergeTree` without a version column — observe the rows restored to the original insertion order (from a saved permutation costing 8 bytes per row), so their behavior does not change; the presort is skipped entirely when every dependent view requires the original order. The destination table itself always receives identical parts, because its sink sorts the rows by the same key with a stable sort anyway.

Note that with this setting enabled the queries of the other dependent materialized views observe the rows of each block in sorting-key order instead of the original insertion order, which matters if a view query uses order-dependent constructs such as `LIMIT` without `ORDER BY`, window functions without `ORDER BY`, or aggregate functions like `any`, `groupArray`, and `argMax` with equal ties. Insert deduplication is not affected: the deduplication tokens are derived from the block as it was before the presort, so they do not change when the setting is toggled between retries of the same insert.
