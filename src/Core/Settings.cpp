...
If a table has a space-filling curve in its index, e.g. `ORDER BY mortonEncode(x, y)` or `ORDER BY hilbertEncode(x, y)`, and the query has conditions on its arguments, e.g. `x >= 10 AND x <= 20 AND y >= 20 AND y <= 30`, use the space-filling curve for index analysis.
    ...
    DECLARE(Bool, analyze_index_with_tuple_key_elements, true, R"(
If the sorting key of a table contains a tuple element, e.g. `ORDER BY (a, (b, c))`, analyze the primary index with the tuple expanded into its components, exactly as if the key were `ORDER BY (a, b, c)` (a tuple orders rows lexicographically by its components, so both keys sort rows identically). Conditions on the components, e.g. `b = 1`, can then prune granules by the primary index. Also analyze comparisons of tuples of key columns against tuple constants, e.g. `(b, c) > (1, 'x')`, by the relaxed bound they imply on the first component (`b >= 1`).
)", 0) \
