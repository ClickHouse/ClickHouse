...
#define COMMON_SETTINGS(M, ALIAS) \
    ...
    \
    M(UInt64, distributed_background_insert_max_retries, 0, "Maximum number of file send retries for StorageDistributed.", 0) \
    ...
    M(UInt64, database_replicated_allow_replicated_engine_arguments, 0, "0 - Don't allow to explicitly specify ZooKeeper path and replica name for *MergeTree tables in Replicated databases. 1 - Allow. 2 - Allow, but ignore the specified path and use default one instead. 3 - Allow and don't log a warning.", 0) \
