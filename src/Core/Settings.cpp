...
Formatter '%f' in function 'formatDateTime' prints a single zero instead of six zeros if the formatted value has no fractional seconds.
    ...
    DECLARE(Bool, formatdatetime_f_prints_scale_only, false, R"(
Formatter '%f' in function 'formatDateTime' prints only the scale amount of digits for a DateTime instead of fixed 6 digits.
)", 0) \
