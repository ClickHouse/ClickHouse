...
This is internal setting that should not be used directly and represents an implementation detail of the 'parallel replicas' mode. When the initiator of a query over a `Merge` table (or the `merge` table function) builds no local plan (`parallel_replicas_prefer_local_replica` is disabled), the reading coordinator has no pinned snapshot replica, and a child table matched by no participating replica would never be announced to it - the rows of that table would silently vanish from the result. The initiator sets this to the child table sets of the `Merge` tables the query reads, keyed by the table expression each of them belongs to, and a replica whose `Merge` table resolves to a child set different from the initiator's set for the same table expression fails the query instead. The setting is sent only for queries that read a `Merge` table with `parallel_replicas_allow_merge_tables` enabled, and is marked important: an older replica that does not know it fails such a query instead of silently ignoring the check.
)", IMPORTANT | BETA) \
