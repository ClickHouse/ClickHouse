...
#define COMMON_SETTINGS(M, ALIAS) \
    ...
    M(UInt64, skip_indexes_in_final_correctness_threshold, 0, "If query has FINAL, then skipping data based on indexes may produce incorrect result. This setting can ensure that correct results are returned by scanning newer parts that have overlap with the ranges returned by the skip index. Set to 0 to disable, 1 to enable and >1 to specify the limit on initial ranges beyond which feature will be disabled.(experimental)", 0) \
