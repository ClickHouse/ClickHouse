...
#define LIST_OF_SERVER_SETTINGS_WITHOUT_PATH(DECLARE, ALIAS) \
    DECLARE(DeduplicationUnificationStage, deduplication_unification_stage, DeduplicationUnificationStage::OLD_SEPARATE_HASHES, R"(
        ...
        The value `compatible_double_hashes` means that ClickHouse will use two deduplication hashes: one for sync inserts and another one for async inserts. This value should be used to migrate to the new behavior in a safe way, because both deduplication methods will be used.
        ...
        Finally the value `new_unified_hash` means that ClickHouse will use the same deduplication hashes for sync and async inserts. This value could be enabled on new instances of ClickHouse or on instanses which already used `compatible_double_hashes` value for some time.
    )", 0) \
