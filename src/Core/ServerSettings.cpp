...
If enabled, every ZooKeeper request must have a component name set via `Coordination::setCurrentComponent`. Throws a `LOGICAL_ERROR` exception if the component is missing.
    ...
    DECLARE(Bool, allow_experimental_executable_udf_shared_memory, false, R"(Enable experimental support for the shared-memory transport of executable user-defined functions, declared via `use_shared_memory` in a function's configuration. The transport exchanges bulk data with the command through a shared-memory region (a sealed `memfd` whose descriptor the command inherits) that both sides map, instead of through the `stdin`/`stdout` pipes. It is Linux-only, and a function that asks for it fails to load while this setting is off.)", EXPERIMENTAL) \
