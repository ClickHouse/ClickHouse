...
#define LIST_OF_SERVER_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(String, concurrent_threads_scheduler, "round_robin", "The policy on how to perform a scheduling of CPU slots in Concurrency Control. Algorithm used to govern how limited number `concurrent_threads_soft_limit` of CPU slots are distributed among concurrent queries. Scheduler may be changed at runtime without server restart.", 0), \
    ...
    DECLARE(Bool, memory_worker_correct_memory_tracker, 0, "Whether background memory worker should correct internal memory tracker based on the information from external sources like jemalloc and cgroups", 0) \
    ...
    DECLARE(Bool, send_settings_to_client, false, "Send user settings from server configuration to clients (in the server Hello message).", 0) \
    \
