...
The policy on how to perform a scheduling of CPU slots specified by `concurrent_threads_soft_limit_num` and `concurrent_threads_soft_limit_ratio_to_cores`. Algorithm used to govern how limited number of CPU slots are distributed among concurrent queries. Scheduler may be changed at runtime without server restart.
    ...
    **Example**
    ...
    DECLARE(UInt64, storage_connections_rcvbuf, 204800, R"(The size of the SO_RCVBUF option for storage connections (replication, distributed queries). Defaults to 204800 (200 KB), matching disk_connections_rcvbuf: it caps the per-socket receive buffer that kernel TCP autotuning would otherwise grow to several MiB. This suits intra-region clusters, where the bandwidth-delay product is small; for high-latency links such as cross-region replication, set it to 0 to restore kernel TCP autotuning. Note: changing this setting back to 0 restores autotuning only for newly created connections; existing pooled connections retain fixed buffer sizes until they are recreated.)", 0) \
