...
Configured as `named_collections_storage.type` (`<named_collections_storage><type>` in XML).
    ...
    DECLARE(String, background_queries_registry_zookeeper_path, "/clickhouse/background_queries", R"(The path in [Zoo]Keeper under which queries running in the background (`run_query_in_background`) register themselves. The registry is shown by `system.background_queries`.)", 0, "background_queries_registry.zookeeper_path") \
    DECLARE(UInt64, background_queries_registry_entry_ttl, 1209600, R"(Time in seconds after the last update of an entry of the background queries registry when the entry is removed from [Zoo]Keeper.)", 0, "background_queries_registry.entry_ttl") \
    DECLARE(UInt64, background_queries_registry_max_query_length, 32768, R"(The maximum length in bytes of a query text stored in the background queries registry. Longer queries are truncated.)", 0, "background_queries_registry.max_query_length") \
    DECLARE(UInt64, background_queries_registry_max_error_length, 32768, R"(The maximum length in bytes of an error message stored in the background queries registry. Longer messages are truncated.)", 0, "background_queries_registry.max_error_length") \
