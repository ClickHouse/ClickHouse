...
#define LIST_OF_SERVER_SETTINGS_WITHOUT_PATH(DECLARE, ALIAS) \
    ...
    **See also:**
    ...
    There is one shared cache for the server. Memory is allocated on demand. The cache is used if the query setting `use_uncompressed_cache` is enabled, and may also be used automatically for eligible local reads from tables in the `MergeTree` family when `use_uncompressed_cache` was not explicitly overridden and the query setting `enable_automatic_use_uncompressed_cache` (disabled by default) is enabled.
