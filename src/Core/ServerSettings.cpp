...
The policy on how to perform a scheduling of CPU slots specified by `concurrent_threads_soft_limit_num` and `concurrent_threads_soft_limit_ratio_to_cores`. Algorithm used to govern how limited number of CPU slots are distributed among concurrent queries. Scheduler may be changed at runtime without server restart.
    ...
    **Example**
    ...
    DECLARE(UInt64, background_ann_build_pool_size, 2, R"(The maximum number of threads used for building the table-level ANN (DiskANN) index in the background. Each in-flight build occupies one thread for the duration of the DiskANN FFI call, which is typically minutes. The pool is kept separate from the merge/mutation and common cleanup pools so that a long-running index build never starves those.)", 0) \
