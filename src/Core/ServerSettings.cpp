...
Separator string for the `database_namespace` feature (startup-only, not reloadable).
When non-empty, enables multi-tenant database isolation: users with a `DATABASE NAMESPACE`
will have their database names transparently prefixed with `{ns}{separator}`.

Once enabled, no user (including admins) can `CREATE DATABASE` with a name containing
the separator. This reserves the separator for the `database_namespace` mechanism and
eliminates ambiguity. Pre-existing databases whose names contain the separator (created
before the feature was enabled) remain fully accessible: they load at startup and can be
read, written, and dropped normally.
...
`DATABASE NAMESPACE`. These databases are excluded from `database_namespace` prefixing —
they behave like system databases (`system`, `INFORMATION_SCHEMA`, `default`, etc.).
