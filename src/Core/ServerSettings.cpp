...
The policy on how to perform a scheduling of CPU slots specified by `concurrent_threads_soft_limit_num` and `concurrent_threads_soft_limit_ratio_to_cores`. Algorithm used to govern how limited number of CPU slots are distributed among concurrent queries. Scheduler may be changed at runtime without server restart.
    ...
    **Example**
    ...
    DECLARE(NonZeroUInt64, reader_executor_cache_filler_pool_size, 8, R"(Number of threads in the shared `CacheFiller` pool for `ReaderExecutor` (experimental `use_reader_executor` setting), which runs deferred cache-fill / write-back (put) steps separately from the prefetch pool so cache-fill work does not compete with read-ahead fetches. Applied when the pool is first created at server start; changing it requires a restart (`SYSTEM RELOAD CONFIG` does not resize the live pool).)", EXPERIMENTAL) \
    DECLARE(UInt64, reader_executor_cache_filler_queue_size, 80, R"(Maximum number of put tasks (running + queued) for the shared `ReaderExecutor` `CacheFiller` pool. `submit` returns immediately (with nullptr handle) when this limit is reached; the deferred fill is skipped (the bytes are regenerable by a later read). A value of `0` means "use the default of `reader_executor_cache_filler_pool_size * 10`". Applied when the pool is first created at server start; changing it requires a restart (`SYSTEM RELOAD CONFIG` does not resize the live pool).)", EXPERIMENTAL) \
