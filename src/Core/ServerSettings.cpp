...
Configured as `named_collections_storage.type` (`<named_collections_storage><type>` in XML).
    ...
    DECLARE(UInt64, query_plan_cache_max_size_in_bytes, DEFAULT_QUERY_PLAN_CACHE_MAX_SIZE, R"(The maximum size of the query plan cache in bytes. 0 means the query plan cache is disabled.)", 0, "query_plan_cache.max_size_in_bytes") \
    DECLARE(UInt64, query_plan_cache_max_entries, DEFAULT_QUERY_PLAN_CACHE_MAX_ENTRIES, R"(The maximum number of query plans stored in the cache.)", 0, "query_plan_cache.max_entries") \
