...
If enabled, every ZooKeeper request must have a component name set via `Coordination::setCurrentComponent`. Throws a `LOGICAL_ERROR` exception if the component is missing.
    ...
    DECLARE(Bool, enable_silk_runtime, false, R"(Enable experimental support for the silk fiber runtime: initialize the silk fiber scheduler at server startup, so that subsystems supporting it can run their jobs on fibers. Requires Linux with `io_uring` available to the server process (kernel 5.17 or newer, not blocked by seccomp; the default Docker seccomp profile blocks it). The server refuses to start otherwise.)", EXPERIMENTAL) \
