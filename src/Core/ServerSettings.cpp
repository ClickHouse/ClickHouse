...
This setting can be modified at runtime and will take effect immediately.
:::)", 0) \
    DECLARE(UInt64, skipping_index_cache_max_entries, DEFAULT_SKIPPING_INDEX_CACHE_MAX_ENTRIES, "Maximum number of entries in the skipping index cache. Zero means no entry-count limit.", 0) \
...
ChangeableSettingsMap collectChangeableServerSettings(ContextPtr context)
{
    ...
    ChangeableSettingsMap changeable_settings
        = {
            ...
            {"skipping_index_cache_size", {std::to_string(context->getSkippingIndexCache()->maxSizeInBytes()), ChangeableWithoutRestart::Yes}},
