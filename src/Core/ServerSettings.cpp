...
Each spawned pipeline worker holds one reservation for the duration of its job. For step-driven executors (`PullingPipelineExecutor`, `PushingPipelineExecutor`), the calling thread holds the reservation while it drives an executor step. This avoids charging more than one reservation when one thread keeps several step-driven executors alive.

The reservation itself is charged to the server-wide tracker only. For step-driven executors, the caller flushes its pending real allocation reports before releasing the reservation. Therefore, when this setting is enabled, query-level and user-level memory accounting (including `max_memory_usage`, `memory_usage` in `system.processes`, and memory-based heuristics such as the conversion of aggregation hash tables to two-level or spilling to disk) can become visible sooner between executor steps. Setting it to `0` does not add this flush.
