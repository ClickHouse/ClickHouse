...
#define LIST_OF_SERVER_SETTINGS(DECLARE, ALIAS) \
    ...
    **See also:**
    ...
    DECLARE(String, text_index_posting_list_cache_policy, DEFAULT_TEXT_INDEX_POSTING_LIST_CACHE_POLICY, "Text index posting list cache policy name.", 0) \
    DECLARE(UInt64, text_index_posting_list_cache_size, DEFAULT_TEXT_INDEX_POSTING_LIST_CACHE_MAX_SIZE, R"(Size of cache for text index posting lists. Zero means disabled.

    :::note
    This setting can be modified at runtime and will take effect immediately.
    :::)", 0) \
    DECLARE(UInt64, text_index_posting_list_cache_max_entries, DEFAULT_TEXT_INDEX_POSTING_LIST_CACHE_MAX_ENTRIES, "Size of cache for text index posting list in entries. Zero means disabled.", 0) \
    DECLARE(Double, text_index_posting_list_cache_size_ratio, DEFAULT_TEXT_INDEX_POSTING_LIST_CACHE_SIZE_RATIO, "The size of the protected queue (in case of SLRU policy) in the text index posting list cache relative to the cache's total size.", 0) \
