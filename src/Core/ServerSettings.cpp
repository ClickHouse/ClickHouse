...
Maximum size of batch for MultiRead request to [Zoo]Keeper that support batching. If set to 0, batching is disabled. Available only in ClickHouse Cloud.
    ...
    DECLARE(NonZeroUInt64, ai_service_threadpool_pool_size, 100, R"(Size of the thread pool the AI service uses to issue requests to AI providers. One worker holds one request, so this caps how many AI provider requests the whole server has in flight at once, across all queries. [`ai_function_max_concurrent_requests`](/reference/settings/session-settings/ai-function#ai_function_max_concurrent_requests) bounds a single query's share of it.)", 0) \
    DECLARE(UInt64, ai_service_threadpool_queue_size, 10000, R"(Number of AI provider requests which is possible to push into the AI service pool. A query that submits a request while the queue is full waits for a slot.)", 0) \
