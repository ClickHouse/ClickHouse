...
If enabled, every ZooKeeper request must have a component name set via `Coordination::setCurrentComponent`. Throws a `LOGICAL_ERROR` exception if the component is missing.
    ...
    DECLARE(String, documentation_public_url, "", R"(Public origin used for absolute canonical URLs and sitemap entries served under `/docs`, for example `https://play.clickhouse.com`. Set this explicitly when publishing the embedded source reference, especially behind a reverse proxy. When empty, canonical links are root-relative and `/docs/sitemap.xml` returns HTTP status `503 Service Unavailable`.)", 0) \
