...
:::
    ...
    DECLARE(Bool, disk_connections_use_silk, false, R"(Create HTTP connections of the disk connection group (object storage disks) on Silk fiber sockets and start the Silk fiber scheduler at server startup. Requires a build with Silk (Linux, io_uring). Used by the reader executor fiber path.)", EXPERIMENTAL) \
