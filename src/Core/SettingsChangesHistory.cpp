...
const VersionToSettingsChangesMap & getSettingsChangesHistory()
{
    ...
    std::call_once(initialized_flag, [&]
    {
        ...
        addSettingsChanges(settings_changes_history, "26.10",
        {
            ...
            {"http_allow_database_as_path", false, true, "Enabled by default: the HTTP interface interprets a `/database/` component of the URL path as the current database."},
            {"http_allow_table_as_file", false, true, "Enabled by default: the HTTP interface interprets the last URL path component as `table[.format[.compression]]` and answers it with `SELECT * FROM table`."},
            {"http_allow_filters_as_path", false, true, "Enabled by default: the HTTP interface interprets `/name=value/` path components as filters combined with `AND`."},
            {"http_allow_filters_as_unrecognized_url_parameters", false, true, "Enabled by default: an unrecognized URL parameter is interpreted as a filter instead of being rejected as an unknown setting."},
