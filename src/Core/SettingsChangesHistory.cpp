...
const VersionToSettingsChangesMap & getSettingsChangesHistory()
{
    ...
    std::call_once(initialized_flag, [&]
    {
        ...
        {
            {"optimize_push_subcolumns_into_subqueries", false, true, "New setting to push a subcolumn read through a subquery or a CTE, so that the subquery projects the subcolumn itself instead of the whole column. previous_value=false so `compatibility` with versions below 26.10 restores the pre-existing behavior (no push down)."},
