...
const VersionToSettingsChangesMap & getSettingsChangesHistory()
{
    ...
    std::call_once(initialized_flag, [&]
    {
        ...
        {
            {"presort_inserts_with_materialized_views", false, false, "New setting to sort inserted blocks by the destination table's sorting key before the sink, so that dependent materialized views receive sorted rows and their target sinks can skip the redundant sorting step."},
