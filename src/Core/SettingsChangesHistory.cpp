...
const VersionToSettingsChangesMap & getSettingsChangesHistory()
{
    ...
    std::call_once(initialized_flag, [&]
    {
        ...
        {
            {"allow_experimental_lance", false, false, "New setting to enable the experimental `Lance` table engines and table functions."},
            ...
            {"lance_scan_in_order", true, false, "Changed the Lance scan default to unordered so one producer can feed multiple conversion Sources."},
            ...
            {"lance_batch_readahead", 0, 0, "New setting for Lance scanner batch readahead (0 = library default)."},
            {"lance_io_buffer_size", 0, 0, "New setting for Lance scanner I/O buffer size in bytes (0 = library default)."},
            {"lance_batch_queue_capacity", 0, 0, "New setting for the bounded Lance producer queue batch capacity (0 = automatic)."},
            {"lance_batch_queue_bytes", 0, 0, "New setting for the bounded Lance producer queue byte limit (0 = 64 MiB)."},
            {"lance_max_batch_sources", 0, 0, "New setting limiting ClickHouse conversion Sources per Lance scan (0 = requested stream count)."},
            {"lance_enable_fragment_parallelism", false, true, "New setting to create coarse fragment-group tasks for distributed Lance reads; local reads use one dataset scan."},
            {"lance_fragment_pack_mode", "auto", "auto", "New setting for distributed Lance fragment-group task packing (one/pack/auto)."},
            ...
            {"optimize_move_to_prewhere_if_final", false, true, "Enable the automatic PREWHERE optimization for queries with FINAL by default. Only conditions that depend on the sorting key alone and are deterministic within the query are moved, so results are not affected. `compatibility` below 26.10 restores the previous behavior."},
