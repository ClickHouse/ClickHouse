...
const VersionToSettingsChangesMap & getSettingsChangesHistory()
{
    ...
    std::call_once(initialized_flag, [&]
    {
        ...
        {
            {"aggregate_functions_skip_variant_nulls", false, true, "From 26.8, the aggregate functions that accept a `Variant` argument natively (`count`, `any`, `groupArray`, `groupConcat`, the `uniq` family, ...) skip the rows where the `Variant` holds a NULL value, like they skip the NULL values of a `Nullable` argument. Set this to `false` (or `SET compatibility = '26.7'`) to restore the previous behavior, where those rows were aggregated as ordinary values."},
