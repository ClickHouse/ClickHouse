...
const VersionToSettingsChangesMap & getSettingsChangesHistory()
{
    ...
    std::call_once(initialized_flag, [&]
    {
        ...
        addSettingsChanges(settings_changes_history, "26.7",
        {
            ...
            {"fast_float_math", false, false, "New setting to enable faster, vectorized (SIMD) but less precise implementations of certain float math functions (`exp2`, `exp10`, `log2`, `log10`, `pow`). Worst-case relative error is ~1e-12 for `exp2`/`exp10`, ~5e-9 for `log2`/`log10`, and (for `pow`) ~1e-14 for integer exponents (computed by repeated multiplication, not bit-identical to precise `pow`) / ~1e-12 for a constant positive base, with other cases falling back to precise `pow`. Default false preserves the precise scalar libm results bit-for-bit."},
