...
const VersionToSettingsChangesMap & getSettingsChangesHistory()
{
    ...
    std::call_once(initialized_flag, [&]
    {
        ...
        addSettingsChanges(settings_changes_history, "26.10",
        {
            ...
            {"aggregate_functions_skip_variant_nulls", false, true, "From 26.10, the aggregate functions that accept a `Variant` argument natively (`count`, `any`, `groupArray`, `groupConcat`, the `uniq` family, ...) skip the rows where the `Variant` holds a NULL value, like they skip the NULL values of a `Nullable` argument. The `-SimpleState` combinator and declared `SimpleAggregateFunction(..., Variant(...))` columns are excluded and retain the previous behavior, where those rows were aggregated as ordinary values. Set this to `false` (or `SET compatibility = '26.9'`) to restore that previous behavior for other aggregate functions."},
        });
