...
const VersionToSettingsChangesMap & getSettingsChangesHistory()
{
    ...
    std::call_once(initialized_flag, [&]
    {
        ...
        addSettingsChanges(settings_changes_history, "26.8",
        {
            ...
            {"enable_multi_way_keyed_merge", false, false, "New setting to merge each destination key's parallelizable aggregate states (currently uniqExact) from all source threads in one multi-way wave in the final merge of two-level aggregation, instead of folding the sources in pairwise."},
            {"enable_two_level_promotion_for_parallel_merge", false, false, "New setting to convert all single-level aggregation hash tables to two-level at the start of the final merge when the multi-way keyed merge could engage, so queries at the two-level conversion boundary reliably take the per-bucket merge path."},
