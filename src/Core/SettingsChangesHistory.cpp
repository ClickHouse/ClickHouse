...
const VersionToSettingsChangesMap & getSettingsChangesHistory()
{
    ...
    std::call_once(initialized_flag, [&]
    {
        ...
        addSettingsChanges(settings_changes_history, "26.10",
        {
            ...
            {"enable_join_runtime_filters_index_analysis", false, true, "Enable pruning of granules on the probe (left) side of a JOIN by the runtime filter collected from the build (right) side. Only join keys that are primary key columns of the probe side or are covered by a `minmax`, `set` or `bloom_filter` skip index there are pruned, so the pass either prunes granules that cannot match or does nothing. Once the filter overflows `join_runtime_filter_exact_values_limit` and keeps no exact values, it prunes by the recorded key range, which is recorded only for integer and date or date-time keys, so a key of any other type (such as `String`) is not pruned then. A key covered only by a `bloom_filter` index is pruned by the exact key values of the filter alone, and only while the filter keeps at most `join_runtime_filter_exact_values_limit / 100` of them; the build side does not record the key range for it. With `use_primary_key = 0` a primary key column is pruned only by a skip index that covers it. With `use_skip_indexes_on_data_read = 0` the pruning is off but the build side still records the key range of the filter."},
