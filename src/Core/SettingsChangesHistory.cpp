...
const VersionToSettingsChangesMap & getSettingsChangesHistory()
{
    ...
    std::call_once(initialized_flag, [&]
    {
        ...
        addSettingsChanges(settings_changes_history, "26.8",
        {
            ...
            {"filesystem_cache_verbose_logging", true, false, "New setting gating the per-buffer-refill TEST-level log messages of the filesystem cache read buffer. Before 26.8 they were emitted unconditionally once the log level admitted them, so the previous value is `true` and `SET compatibility = '26.7'` restores that logging."},
        });
