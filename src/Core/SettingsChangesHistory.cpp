...
const VersionToSettingsChangesMap & getSettingsChangesHistory()
{
    ...
    std::call_once(initialized_flag, [&]
    {
        ...
        addSettingsChanges(settings_changes_history, "26.10",
        {
            ...
            {"min_rows_per_stream_for_gradual_resize", 0, 0, "New setting to opt in to starting the `GROUP BY` pre-aggregation stage with one active stream (one per split group when the resize is split, see `min_outstreams_per_resize_after_split`) and switching to all aggregation streams in this stage once the cumulative number of rows crosses the threshold. Disabled by default."},
            {"min_bytes_per_stream_for_gradual_resize", 0, 0, "New setting (in bytes) that, paired with `min_rows_per_stream_for_gradual_resize`, controls when the `GROUP BY` pre-aggregation switches from one active stream (one per split group when the resize is split) to all aggregation streams in this stage."},
