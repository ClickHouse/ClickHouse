...
const VersionToSettingsChangesMap & getSettingsChangesHistory()
{
    ...
    std::call_once(initialized_flag, [&]
    {
        ...
        addSettingsChanges(settings_changes_history, "26.6",
        {
            ...
            {"azure_rbac_warmup_interval_sec", 0, 0, "New setting that defines a period in seconds after Azure client creation during which all retryable operations use an extended retry budget to tolerate Azure RBAC propagation lag."},
            {"azure_rbac_warmup_retry_multiplier", 3, 3, "New setting that controls the retry count multiplier applied during the Azure RBAC warmup interval."},
        });
