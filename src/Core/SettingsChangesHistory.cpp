...
const VersionToSettingsChangesMap & getSettingsChangesHistory()
{
    ...
    std::call_once(initialized_flag, [&]
    {
        ...
        addSettingsChanges(settings_changes_history, "26.8",
        {
            ...
            {"join_algorithm", "direct,parallel_hash,hash", "direct,hash,ie_join", "Appended `ie_join` to the default list, so a join whose `ON` section has only inequality conditions is executed with IEJoin instead of a `CROSS JOIN` with a filter. Being last, it is used only when the other algorithms do not apply. `parallel_hash` is an obsolete alias of `hash` and was dropped from the default list; it is still accepted. Listing `hash` or `parallel_hash` does not control how parallel the join is."},
            ...
            {"parallel_hash_join_threshold", 100'000, 100'000, "The threshold no longer chooses between the `hash` and `parallel_hash` algorithms. When a hash join is used and an estimate of the right table size is available, it decides whether the join may run in parallel. Below the threshold, single-threaded execution; at or above it, multiple threads when `max_threads` > 1. The default is unchanged."},
            {"parallel_non_joined_rows_processing", true, true, "Applies to RIGHT/FULL hash joins, not specifically `parallel_hash`. The default is unchanged."},
