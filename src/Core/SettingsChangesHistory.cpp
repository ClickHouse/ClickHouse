...
const VersionToSettingsChangesMap & getSettingsChangesHistory()
{
    ...
    std::call_once(initialized_flag, [&]
    {
        ...
        addSettingsChanges(settings_changes_history, "26.10",
        {
            ...
            {"s3_skip_hidden_files", false, false, "New setting: skip objects with a path segment starting with `_` or `.` below the non-glob prefix when listing files by glob, like other Hive-ecosystem readers do. Tables with `partition_strategy = 'hive'` always skip such paths."},
            {"azure_skip_hidden_files", false, false, "New setting: skip blobs with a path segment starting with `_` or `.` below the non-glob prefix when listing files by glob, like other Hive-ecosystem readers do. Tables with `partition_strategy = 'hive'` always skip such paths."},
            {"hdfs_skip_hidden_files", false, false, "New setting: skip files with a path segment starting with `_` or `.` below the non-glob prefix when listing files by glob, like other Hive-ecosystem readers do. Tables with `partition_strategy = 'hive'` always skip such paths."},
