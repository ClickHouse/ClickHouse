...
const VersionToSettingsChangesMap & getSettingsChangesHistory()
{
    ...
    std::call_once(initialized_flag, [&]
    {
        ...
        {
            {"enable_parallel_single_level_merge", false, true, "New setting to parallelize the final merge of single-level aggregation hash tables: the key space is partitioned into slices by key hash, and every slice is merged independently by one of the threads, which enumerates all per-thread tables and merges the keys whose hash falls into the slice."},
