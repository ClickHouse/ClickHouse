...
const VersionToSettingsChangesMap & getSettingsChangesHistory()
{
    ...
    std::call_once(initialized_flag, [&]
    {
        ...
        addSettingsChanges(settings_changes_history, "26.10",
        {
            ...
            {"input_format_protobuf_datetime64_scale", -1, -1, "New setting. Integer Protobuf fields read into DateTime64 are Unix seconds by default (-1), so a column precision change does not reinterpret the payload. Set to the column precision N to read scaled ticks of 10^-N seconds; N must equal the DateTime64 precision or the read is rejected. float/double fields always store fractional Unix seconds and are unaffected."},
            {"output_format_protobuf_datetime64_scale", -1, -1, "New setting. Integer Protobuf fields written from DateTime64 are Unix seconds by default (-1), truncating subseconds. Set to the column precision N to write scaled ticks of 10^-N seconds; N must equal the DateTime64 precision or the write is rejected. float/double fields always store fractional Unix seconds and are unaffected."},
