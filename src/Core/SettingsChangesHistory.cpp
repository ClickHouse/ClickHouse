...
const VersionToSettingsChangesMap & getSettingsChangesHistory()
{
    ...
    std::call_once(initialized_flag, [&]
    {
        ...
        addSettingsChanges(settings_changes_history, "26.10",
        {
            ...
            {"query_plan_filter_push_down_below_limit_by", true, true, "Setting newly added in 26.10 to gate the push-down of filters on `LIMIT BY` key columns below the `LIMIT BY` step, which shipped in 26.7. The meaningful semantic change is registered under the 26.7 block so `compatibility` with a version before 26.7 reverts it; the previous value here is deliberately equal to the new one, because 26.7 through 26.9 already had the optimization enabled."},
        });
        ...
        addSettingsChanges(settings_changes_history, "26.7",
        {
            ...
            {"statistics_max_set_size_for_exact_selectivity_estimation", 10000, 10000, "The bound on the cost of estimating the selectivity of `IN` with a large set is kept under `compatibility` with an earlier version: the previous value is deliberately equal to the new one, so that the uncapped estimation, which could add hundreds of milliseconds to the planning of a single query, is not restored."},
            {"query_plan_filter_push_down_below_limit_by", false, true, "Registers under 26.7 the plan change that shipped there with https://github.com/ClickHouse/ClickHouse/pull/110116: filters on `LIMIT BY` key columns started being pushed below the `LIMIT BY` step. The setting itself is new in 26.10; this row exists so that `compatibility` with a version before 26.7 restores the earlier plan, the same way `defer_partition_pruning_after_final` is registered under 26.3."},
        });
