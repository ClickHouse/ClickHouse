...
const VersionToSettingsChangesMap & getSettingsChangesHistory()
{
    ...
    std::call_once(initialized_flag, [&]
    {
        ...
        addSettingsChanges(settings_changes_history, "26.8",
        {
            ...
            {"max_streams_per_hierarchical_merge", 0, 16, "New setting to merge sorted streams in a tree of `MergingSortedTransform` nodes instead of a single fan-in, spreading the contention on one node's mutex over many. previous_value=0 (a single merge) so `compatibility` with versions before 26.8 restores the pre-existing pipeline."},
        });
