...
const VersionToSettingsChangesMap & getSettingsChangesHistory()
{
    ...
    std::call_once(initialized_flag, [&]
    {
        ...
        addSettingsChanges(settings_changes_history, "26.7",
        {
            ...
            {"input_format_parquet_min_bytes_to_split", 0, 256 * 1024 * 1024, "New setting: a single local Parquet file is only parallelized across multiple sources if the query reads at least this many compressed bytes, avoiding per-source overhead on short queries. The previous value `0` (no floor) reproduces the pre-26.7 behavior where the split was driven only by the row-group count, so `compatibility` set to an earlier version keeps parallelizing regardless of read size."},
            {"input_format_parquet_bytes_per_split_bucket", 0, 64 * 1024 * 1024, "New setting: target minimum compressed bytes per bucket when a single local Parquet file is parallelized across multiple sources. The previous value `0` (do not bound the number of buckets by size) reproduces the pre-26.7 behavior where the bucket count was driven only by the row-group count, so `compatibility` set to an earlier version keeps the old split fan-out."},
