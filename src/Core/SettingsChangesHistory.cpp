...
const VersionToSettingsChangesMap & getSettingsChangesHistory()
{
    ...
    std::call_once(initialized_flag, [&]
    {
        ...
        addSettingsChanges(settings_changes_history, "26.8",
        {
            ...
            {"date_time_64_output_format_cut_trailing_zeros_align_to_groups_of_thousands", false, false, "The output scale of this setting widened from [0, 3, 6] to [0, 3, 6, 9]: the trimmed scale is rounded up to the next multiple of three that keeps every significant digit, so a `DateTime64` value with scale 7 to 9 no longer loses the digits past the sixth. `Time64` is trimmed the same way by a direct scalar `toString` or `CAST`, except that an all-zero fraction keeps three digits; a nested `Time64` and the `Time64` row output formats still print the declared scale. The default is unchanged, so `compatibility` restores nothing; this row records the semantics change for the compatibility audit path."},
        });
