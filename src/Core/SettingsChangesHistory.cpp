...
const VersionToSettingsChangesMap & getSettingsChangesHistory()
{
    ...
    std::call_once(initialized_flag, [&]
    {
        ...
        addSettingsChanges(settings_changes_history, "26.10",
        {
            ...
            {"enable_join_key_only_hash_tables", true, true, "Obsolete setting: the hash join no longer uses key-only hash tables."},
            {"join_algorithm", "direct,parallel_hash,hash,ie_join", "direct,hash,ie_join", "`parallel_hash` is an obsolete alias of `hash` and was dropped from the default list; it is still accepted. Listing `hash` or `parallel_hash` does not control how parallel the join is: the hash join builds one shared hash table from the right table in cache-sized partitions, in parallel when `max_threads` > 1 and the right table is not estimated below `parallel_hash_join_threshold`."},
            ...
            {"hash_join_max_fanout_per_pass", 8192, 8192, "New setting: the maximum number of partitions a `partitioned_hash` join writes in one pass over the right table. When the join needs more partitions, it makes several passes."},
            {"hash_join_cap_partitions_by_l1_descriptors", true, true, "New setting: limit the number of partitions of a `partitioned_hash` join so that the per-partition records of the hash table fit in a quarter of the L1 data cache."},
        });
