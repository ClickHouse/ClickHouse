...
const VersionToSettingsChangesMap & getSettingsChangesHistory()
{
    ...
    std::call_once(initialized_flag, [&]
    {
        ...
        {
            {"query_plan_join_subset_keys_auto", false, false, "New setting to enable cardinality-based automatic demotion of JOIN equality keys. Off by default; enable per-query for opt-in experimentation."},
            {"query_plan_join_subset_keys_min_rows", 1000000, 1000000, "New setting: minimum right-side row count for `query_plan_join_subset_keys_auto` to apply."},
            {"query_plan_join_subset_keys_min_kept_selectivity", 0.01, 0.01, "New setting: target selectivity of the kept hash keys for `query_plan_join_subset_keys_auto`."},
