...
const VersionToSettingsChangesMap & getSettingsChangesHistory()
{
    ...
    std::call_once(initialized_flag, [&]
    {
        ...
        {
            {"fast_float_math", false, false, "New setting to enable faster, vectorized (SIMD) but less precise implementations of certain float math functions (`exp2`, `exp10`, `log2`, `log10`, `pow`, `sin`, `cos`, `tan`). Worst-case relative error is ~1e-12 for `exp2`/`exp10`, ~5e-9 for `log2`/`log10`, ~2e-16 (1 ulp) for `sin`/`cos` and ~5e-16 for `tan` with `|x| <= 1e7` (arguments beyond `|x| > 1e8` fall back to precise libm), and (for `pow`) ~1e-14 for a constant integer exponent `n` with `|n| <= 64` (computed by repeated multiplication, not bit-identical to precise `pow`) / ~1e-12 for a constant positive base, with other cases (including a larger integer exponent) falling back to precise `pow`. Default false preserves the precise scalar libm results bit-for-bit."},
