...
const VersionToSettingsChangesMap & getSettingsChangesHistory()
{
    ...
    std::call_once(initialized_flag, [&]
    {
        ...
        addSettingsChanges(settings_changes_history, "26.6",
        {
            ...
            {"shard_by_hash_input_batch_bytes", 0, 2097152, "New setting to control input-batching in `BufferedShardByHashTransform`: accumulates this many bytes before a single `ColumnsScatter::scatter` flush, eliminating per-chunk K×P IColumn allocation overhead. Defaults to 2 MiB; set to 0 to scatter one chunk at a time."},
