...
const VersionToSettingsChangesMap & getSettingsChangesHistory()
{
    ...
    std::call_once(initialized_flag, [&]
    {
        ...
        addSettingsChanges(settings_changes_history, "26.10",
        {
            ...
            {"hash_join_max_fanout_per_pass", 8192, 8192, "New setting: the maximum number of partitions a `hash` join writes in one pass over the right table. When the join needs more partitions, it makes several passes."},
            {"hash_join_cap_partitions_by_l1_descriptors", true, true, "New setting: limit the number of partitions of a `hash` join so that the per-partition records of the hash table fit in a quarter of the L1 data cache."},
        });
