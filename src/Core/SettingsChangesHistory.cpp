...
const VersionToSettingsChangesMap & getSettingsChangesHistory()
{
    ...
    std::call_once(initialized_flag, [&]
    {
        ...
        {
            {"query_plan_remove_redundant_filter_conditions", false, true, "New optimization: remove filter conditions implied by the remaining ones through comparison transitivity when index analysis cannot use them for pruning (e.g. conditions derived by `optimize_and_compare_chain` on non-indexed columns)."},
            {"query_plan_filter_push_down_inferred_only_for_pruning", false, false, "New setting to add join-key-equivalence inferred filter conditions to the other join side only when index analysis there can use them for pruning."},
