...
const VersionToSettingsChangesMap & getSettingsChangesHistory()
{
    ...
    std::call_once(initialized_flag, [&]
    {
        ...
        addSettingsChanges(settings_changes_history, "26.10",
        {
            ...
            {"enable_promql_native_raw_samples", false, false, "New setting to let native PromQL rate fragments consume raw samples arrays; disabled by default."},
            {"enable_promql_native_storage_fusion", false, false, "New setting to fuse supported native PromQL two-rate fragments into the ordered storage read; disabled by default."},
            {"max_promql_native_parallel_lanes", 0, 0, "New setting to cap native PromQL primary-key range layers; zero selects from the read-stream budget, selected work, and native plan shape without changing `max_threads`, while a positive value overrides a plan-specific automatic default."},
            {"max_promql_query_block_size", 0, 0, "New setting to cap block rows across generated SQL and native PromQL plans without changing the caller's outer query plan."},
            ...
            {"max_promql_native_rate_series", 1000000, 1000000, "New setting bounding the number of physical series admitted into one native PromQL rate fragment."},
