...
const VersionToSettingsChangesMap & getSettingsChangesHistory()
{
    ...
    std::call_once(initialized_flag, [&]
    {
        ...
        addSettingsChanges(settings_changes_history, "26.7",
        {
            ...
            {"allow_experimental_bloom_sliced_index", false, false, "New setting to allow the experimental `bloom_sliced` skip index."},
            {"query_plan_direct_read_from_bloom_sliced_index", false, true, "New setting to add staged PREWHERE hints from the `bloom_sliced` skip index. previous_value=false so `compatibility` with versions before 26.7 disables the new plan rewrite."},
        });
