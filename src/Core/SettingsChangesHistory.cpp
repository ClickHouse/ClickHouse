...
const VersionToSettingsChangesMap & getSettingsChangesHistory()
{
    ...
    std::call_once(initialized_flag, [&]
    {
        ...
        addSettingsChanges(settings_changes_history, "26.9",
        {
            ...
            {"object_storage_propagate_credentials_to_other_storages", false, false, "New setting to reuse the base storage's `S3` credentials for a secondary object storage created for a file that Iceberg metadata places outside the table location, even when the target endpoint differs from the base one. Disabled by default: the endpoint comes from the table metadata, so propagating the credentials across endpoints exposes them, including a temporary session token, to whatever host the metadata names. Ignored while `s3_allow_server_credentials_in_user_queries` is disabled."},
