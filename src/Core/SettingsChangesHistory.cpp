...
const VersionToSettingsChangesMap & getSettingsChangesHistory()
{
    ...
    std::call_once(initialized_flag, [&]
    {
        ...
        addSettingsChanges(settings_changes_history, "26.8",
        {
            ...
            {"parallel_replicas_allow_merge_tables", false, false, "New setting to allow parallel replicas over `Merge` tables and the `merge` table function."},
            {"parallel_replicas_designated_table", "", "", "New internal setting: the initiator tells the replicas which table expression it designated for coordinated reading with parallel replicas."},
            {"parallel_replicas_freshness_checked_tables", "", "", "New internal setting: the initiator tells the replicas which replicated tables it checked for replication delay when it selected the replicas, so that a replicated table that started matching a `Merge` table after that check fails the query instead of being served without a freshness check."},
            {"parallel_replicas_merge_child_tables", "", "", "New internal setting: an initiator that builds no local plan tells the replicas which child table sets its `Merge` tables resolved to, so that a replica whose `Merge` table resolves to a different set fails the query instead of silently losing the rows of a child table that no participating replica announced."},
