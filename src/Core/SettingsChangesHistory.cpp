...
const VersionToSettingsChangesMap & getSettingsChangesHistory()
{
    ...
    std::call_once(initialized_flag, [&]
    {
        ...
        addSettingsChanges(settings_changes_history, "26.8",
        {
            ...
            {"query_plan_optimize_join_order_use_proven_uniqueness", false, false, "New experimental setting to use proven uniqueness for join-order cardinality caps; disabled by default."},
            {"query_plan_optimize_join_order_dphyp_proven_edges", false, false, "New experimental setting to let DPhyp install proven singleton transitive hyperedges; disabled by default."},
            {"query_plan_optimize_join_order_data_property_diagnostics", false, false, "New experimental setting to enable join-order data-property collection and logging; disabled by default."},
            {"enable_join_transitive_predicates", true, true, "The default is unchanged, but the behavior expanded: only equalities with a valid transitive comparison domain now feed equivalence classes (cross-domain chains such as UUID = FixedString = Enum are no longer composed), and join reordering now materializes the full equality cut of every class at inner joins, converting optimizer-created cross products in all-inner regions into inner joins."},
            {"distributed_cache_min_inflight_bytes_to_discard_connection_on_seek", 0, 4 * 1024 * 1024, "New setting to drop and reopen a distributed cache connection on a seek when too many in-flight bytes would otherwise be discarded. Defaults to 4 MiB; 0 restores the previous behavior (always reuse the connection via the read range id)."},
