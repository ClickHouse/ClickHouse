...
const VersionToSettingsChangesMap & getSettingsChangesHistory()
{
    ...
    std::call_once(initialized_flag, [&]
    {
        ...
        addSettingsChanges(settings_changes_history, "26.7",
        {
            ...
            {"optimize_prune_impossible_string_comparisons", false, true, "New optimization that replaces comparisons of stringified restricted-alphabet types (e.g. `toString(number) LIKE '%hello%'`) with a constant when the string constant can never appear in the text representation of the type. `compatibility` with versions before 26.7 disables it."},
            {"optimize_destructure_tuple_string_comparisons", false, true, "New optimization that rewrites `toString(tuple) LIKE '%needle%'` into a chain of per-element `LIKE` conditions joined by `OR`, which may allow using a text index. `compatibility` with versions before 26.7 disables it."},
        });
