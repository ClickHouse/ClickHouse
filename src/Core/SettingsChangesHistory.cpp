...
const VersionToSettingsChangesMap & getSettingsChangesHistory()
{
    ...
    std::call_once(initialized_flag, [&]
    {
        ...
        addSettingsChanges(settings_changes_history, "26.9",
        {
            ...
            {"query_plan_filter_push_down_over_any_inner_join", true, true, "New setting that allows the filter push-down optimization to move a predicate over non-key columns below an `ANY INNER JOIN`. The previous value is deliberately equal to the new one: every version before 26.8 pushed such a predicate down, and 26.8 patch releases do so again, so `compatibility` with an earlier version must not keep the predicate above the join."},
