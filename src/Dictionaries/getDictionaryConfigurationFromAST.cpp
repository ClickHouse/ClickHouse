...
String getAttributeExpression(const ASTDictionaryAttributeDeclaration * dict_attr)
{
    ...
    if (const auto * literal = dict_attr->expression->as<ASTLiteral>(); literal && literal->value.getType() == Field::Types::String)
        ...
        expression_str = dict_attr->expression->formatUnsafeWithCredentials();
    ...
    const ASTDictionaryAttributeDeclaration * dict_attr)
{
    ...
    AutoPtr<Text> type(doc->createTextNode(dict_attr->type->formatUnsafeWithCredentials()));
    ...
    const Names & key_columns)
{
    ...
    for (const auto & child : children)
    {
        ...
        AttributeConfiguration attribute_configuration {dict_attr->type->formatUnsafeWithCredentials(), getAttributeExpression(dict_attr)};
