        ...
        for (size_t key_index = 0; key_index < keys_size; ++key_index)
        {
            ...
            if (it)
            {
                ...
                interval_tree.find(date, [&](auto & interval, auto & interval_value_index)
                {
                    if (range)
                    {
                        if (ch_likely(configuration.lookup_strategy == RangeHashedDictionaryLookupStrategy::min) && interval < *range)
                        {
                            range = interval;
        ...
        ValueSetter && set_value) const
{
    ...
    for (size_t key_index = 0; key_index < keys_size; ++key_index)
    {
        ...
        if (ch_unlikely(container_index >= container_size))
        {
...
void RangeHashedDictionary<dictionary_key_type>::blockToAttributes(const Block & block)
{
    ...
    callOnRangeType(dict_struct.range_min->type, [&](const auto & types)
    {
        ...
        for (size_t key_index = 0; key_index < keys_size; ++key_index)
        {
            ...
            if (ch_unlikely(min_range_null_map && (*min_range_null_map)[key_index]))
            {
                lower_bound = std::numeric_limits<RangeStorageType>::min();
                invalid_range = true;
            }

            if (ch_unlikely(max_range_null_map && (*max_range_null_map)[key_index]))
            {
                upper_bound = std::numeric_limits<RangeStorageType>::max();
                invalid_range = true;
            }

            if (ch_unlikely(!configuration.convert_null_range_bound_to_open && invalid_range))
            {
                keys_extractor.rollbackCurrentKey();
                continue;
            }
            ...
            if (ch_unlikely(!emplaced_in_interval_tree))
            {
