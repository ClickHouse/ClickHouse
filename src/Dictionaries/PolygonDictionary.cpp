...
std::vector<IPolygonDictionary::Point> IPolygonDictionary::extractPoints(const Columns &key_columns)
{
    ...
    const auto *const column_x = typeid_cast<const ColumnVector<Float64>*>(key_columns[0].get());
    const auto *const column_y = typeid_cast<const ColumnVector<Float64>*>(key_columns[1].get());
...
{
    const auto *const ptr_multi_polygons = typeid_cast<const ColumnArray*>(column.get());
    ...
    const auto *const ptr_polygons = typeid_cast<const ColumnArray*>(&ptr_multi_polygons->getData());
    if (!ptr_polygons)
    ...
    const auto *const ptr_rings = typeid_cast<const ColumnArray*>(&ptr_polygons->getData());
    if (!ptr_rings)
...
{
    const auto *const ptr_polygons = typeid_cast<const ColumnArray*>(column.get());
    if (!ptr_polygons)
...
{
    const auto *const ptr_points = typeid_cast<const ColumnArray*>(column);
    const auto *const ptr_coord = typeid_cast<const ColumnVector<Float64>*>(&ptr_points->getData());
    if (!ptr_coord)
...
{
    const auto *const ptr_points = typeid_cast<const ColumnTuple*>(column);
    if (!ptr_points)
    ...
    const auto *const column_x = typeid_cast<const ColumnVector<Float64>*>(&ptr_points->getColumn(0));
    const auto *const column_y = typeid_cast<const ColumnVector<Float64>*>(&ptr_points->getColumn(1));
    if (!column_x || !column_y)
        throw Exception{"Expected coordinates to be of type Float64", ErrorCodes::TYPE_MISMATCH};
