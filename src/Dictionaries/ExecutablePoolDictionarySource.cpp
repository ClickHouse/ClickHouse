...
| `command_termination_timeout` | Executable script should contain main read-write loop. After dictionary is destroyed, pipe is closed, and executable file will have `command_termination_timeout` seconds to shutdown before ClickHouse will send SIGTERM signal to child process. The same budget applies to a pooled process that is being discarded once it has finished writing its output (it answered with fewer rows than requested, or closed its `stdout`): with `check_exit_code` enabled, a process that has not exited within `command_termination_timeout` seconds after that fails the load (its exit code could not be checked) and is sent SIGTERM, rather than being waited for indefinitely; a value of `0` sends the signal at once where the process is being discarded, but leaves that wait for the exit status unbounded. Specified in seconds. Default value is `10`. Optional. |
