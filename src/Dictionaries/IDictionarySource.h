...
public:
    ...
    virtual BlockIO loadAll(ContextMutablePtr query_context) = 0;

    /// Returns a pipe with updated data available from this source.
    virtual QueryPipeline loadUpdatedAll(ContextMutablePtr query_context) = 0;
    ...
    /** Returns an input stream with the data for a collection of identifiers.
      ...
      */
    virtual QueryPipeline loadIds(ContextMutablePtr query_context, const std::vector<UInt64> & ids) = 0;
    ...
    /** Returns an input stream with the data for a collection of composite keys.
      ...
      */
    virtual QueryPipeline loadKeys(ContextMutablePtr query_context, const Columns & key_columns, const std::vector<size_t> & requested_rows) = 0;
