...
{
    {
        ...
        /// Try to mark the failed replica's pool so it won't be selected again immediately
        std::string failed_host;
        if (connection_pool_with_failover && connections)
        {
            auto failed_pool = connections->getCurrentConnectionPool();
            if (failed_pool)
            {
                failed_host = failed_pool->getHost();
                connection_pool_with_failover->incrementErrorCount(failed_pool);
                if (log)
                    LOG_DEBUG(log, "incremented connection pool error count for {} due to {} exception", failed_host, last_retry_error_code);
            }
        }
        ...
        if (log)
            LOG_DEBUG(log, "Retrying query on a different replica after error {}{}",
                      last_retry_error_code, failed_host.empty() ? "" : fmt::format(" from host {}", failed_host));
        ...
        case Protocol::Server::Exception:
        {
                ...
                && connection_pool_with_failover)
            {
                ...
                if (log)
                {
                    ...
                    LOG_DEBUG(log, "try {} of {} failed due to error code {}{}",
                              retry_count, static_cast<uint64_t>(max_retries) + 1, packet.exception->code(),
                              failed_host.empty() ? "" : fmt::format(" from host {}", failed_host));
                }
