...
public:
    ...
    virtual void fillFromRowRefs(const DataTypePtr & type, size_t source_column_index_in_block, const UInt64 * row_refs_begin, const UInt64 * row_refs_end, bool row_refs_are_ranges, const std::unordered_map<const Columns *, std::vector<const ColumnReplicated *>> * block_to_replicated_columns = nullptr);

    /// Fills column values from list of blocks and row numbers
    /// `blocks` and `row_nums` must have same size
    virtual void fillFromBlocksAndRowNumbers(const DataTypePtr & type, size_t source_column_index_in_block, const std::vector<const Columns *> & columns, const std::vector<UInt32> & row_nums, const std::unordered_map<const Columns *, std::vector<const ColumnReplicated *>> * block_to_replicated_columns = nullptr);
...
private:
    ...
    /// Fills column values from RowRefList
    /// If row_refs_are_ranges is true, then each RowRefList has one element with >=1 consecutive rows
    void fillFromRowRefs(const DataTypePtr & type, size_t source_column_index_in_block, const UInt64 * row_refs_begin, const UInt64 * row_refs_end, bool row_refs_are_ranges, const std::unordered_map<const Columns *, std::vector<const ColumnReplicated *>> * block_to_replicated_columns = nullptr) override;

    /// Fills column values from list of columns and row numbers
    /// `columns` and `row_nums` must have same size
    void fillFromBlocksAndRowNumbers(const DataTypePtr & type, size_t source_column_index_in_block, const std::vector<const Columns *> & columns, const std::vector<UInt32> & row_nums, const std::unordered_map<const Columns *, std::vector<const ColumnReplicated *>> * block_to_replicated_columns = nullptr) override;
