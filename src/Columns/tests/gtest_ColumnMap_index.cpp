...
template <typename KeyColumnType, typename ValueColumnType, typename KeyType, typename ValueType>
typename ColumnMap::Ptr createMap(const std::initializer_list<std::initializer_list<std::pair<KeyType, ValueType>>> & data)
{
    ...
    ColumnUInt64::Ptr immutable_offsets = std::move(offsets);
    return ColumnMap::create(
            Columns{
                    ColumnArray::create(std::move(keys_column), immutable_offsets),
                    ColumnArray::create(std::move(vals_column), immutable_offsets)
            });
}
...
template <> struct fmt::formatter<DB::Field>: formatter<string_view> {
  ...
  auto format(const DB::Field & f, FormatContext& ctx) {
    return formatter<string_view>::format((std::stringstream() << f).str(), ctx);
  }
};
...
ColumnMapFindAllTestCase makeColumnMapFindAllTestCase(
        std::initializer_list<std::initializer_list<KeyType>> keys,
...
{
    return {
        ColumnMap::create(Columns{createArrayWithLists<KeyColumnType>(keys), createArrayWithLists<ValColumnType>(vals)}),
...
//    ));
    ...
    DoTestFindAll("map with longer keys",
        makeColumnMapFindAllTestCase<ColumnString, ColumnString>(
        {{"alpfa", "bravo", "chralie"}, {"delta", "echo", "foxtrot", "golf",  "hotel"}},
