...
{
    for (const String & name : {"String", "Int64", "Nullable(Int64)"})
    {
        SCOPED_TRACE(name);
...
TEST(ColumnObject, DefaultPathTypeNullableArena)
{
    auto type = DataTypeFactory::instance().get("JSON(max_dynamic_paths=1, DEFAULT PATH TYPE Nullable(Int64))");
    auto column = type->createColumn();
    auto & object = assert_cast<ColumnObject &>(*column);
    object.insert(Object{{"a", Int64(0)}});
    ASSERT_TRUE(object.getDynamicPaths().empty());
    ASSERT_EQ(object.getSharedDataPathsAndValues().second->getName(), "Nullable(Int64)");
