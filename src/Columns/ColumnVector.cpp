...
template <typename T>
void), compareColumnImpl, MULTITARGET_FUNCTION_BODY(( /// NOLINT
    const typename ColumnVector<T>::Container & data,
    T value,
...
ColumnPtr ColumnVector<T>::filter(const IColumn::Filter & filt, ssize_t result_size_hint) const
{
    ...
    /**
     * svcompact, the workhorse for SVE-optimized filtering, only supports
     * 4 and 8-byte data types. Testing shows performance improvements only for
     * 4-byte data types, likely because 128 and 256-bit vectors are not wide enough.
     */
