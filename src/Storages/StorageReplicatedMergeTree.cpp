...
StorageReplicatedMergeTree::StorageReplicatedMergeTree(
    ...
    , replica_path(pathToGenericString(fs::path(zookeeper_path) / "replicas" / replica_name))
    ...
    if (LoadingStrictnessLevel::ATTACH <= mode)
    {
        try
        {
            ...
            else if (current_zookeeper && current_zookeeper->exists(replica_path))
            {
                ...
                if (current_zookeeper->tryGetChildren(pathToGenericString(fs::path(replica_path) / "parts"), parts_on_replica) == Coordination::Error::ZOK)
                {
    ...
    const Strings & replicas, const String & mutation_id) const
{
    ...
    for (const String & replica : *all_required_replicas)
    {
        ...
        while (!partial_shutdown_called)
        {
            ...
            if (!zookeeper->exists(pathToGenericString(fs::path(zookeeper_path) / "replicas" / replica / "is_active")))
            {
        ...
        if (!getZooKeeper()->exists(pathToGenericString(fs::path(zookeeper_path) / "mutations" / mutation_id)))
        {
            throw Exception(ErrorCodes::UNFINISHED, "Mutation {} was killed, manually removed or table was dropped", mutation_id);
        }
...
std::vector<String> getAncestors(const String & path)
{
    ...
    for (auto it = ++full_path.begin(); it != full_path.end(); it++)
    {
        ...
        if (!it->empty())
        {
            ...
            result.push_back(pathToGenericString(ancestor));
        }
    }
...
std::vector<String> StorageReplicatedMergeTree::getZookeeperZeroCopyLockPaths() const
{
    ...
    for (const auto & disk_type: disk_types_with_zero_copy)
    {
        ...
        result.push_back(pathToGenericString(zero_copy_path / actual_table_shared_id));
    }
...
void StorageReplicatedMergeTree::drop()
{
    ...
    if (maybe_has_metadata_in_zookeeper)
    {
        ...
        if (zookeeper->tryGet(pathToGenericString(lost_part_count_path), lost_part_count_str, &lost_part_count_stat))
        {
    ...
    const String & zookeeper_prefix, const StorageMetadataPtr & metadata_snapshot, int32_t * metadata_version, bool strict_check) const
{
    ...
    String metadata_str = zookeeper->get(pathToGenericString(fs::path(zookeeper_prefix) / "metadata"), &metadata_stat);
    ...
    auto columns_from_zk = ColumnsDescription::parse(zookeeper->get(pathToGenericString(fs::path(zookeeper_prefix) / "columns"), &columns_stat));
...
static time_t tryGetPartCreateTime(zkutil::ZooKeeperPtr & zookeeper, const String & replica_path, const String & part_name)
{
    ...
    String unused;
    if (zookeeper->tryGet(pathToGenericString(fs::path(replica_path) / "parts" / part_name), unused, &stat))
        res = stat.ctime / 1000;
...
bool StorageReplicatedMergeTree::checkPartsImpl(bool skip_sanity_checks)
{
    ...
    Strings expected_parts_vec = zookeeper->getChildren(pathToGenericString(fs::path(replica_path) / "parts"));
    ...
    NameSet & absent_replicas_paths)
{
    ...
    for (const String & replica : replicas)
    {
        ...
        if (part_zk_str.empty())
        {
            ...
            if (zookeeper->tryGet(pathToGenericString(fs::path(current_part_path) / "columns"), columns_str) &&
                zookeeper->tryGet(pathToGenericString(fs::path(current_part_path) / "checksums"), checksums_str))
            {
    ...
    {
        const auto storage_settings_ptr = getSettings();
        String part_path = pathToGenericString(fs::path(replica_path) / "parts" / part_name);
...
void StorageReplicatedMergeTree::cloneReplicaIfNeeded(zkutil::ZooKeeperPtr zookeeper)
{
    ...
    for (const String & source_replica_name : replicas)
    {
        ...
        String source_replica_path = pathToGenericString(fs::path(zookeeper_path) / "replicas" / source_replica_name);
        ...
        replicas_paths.emplace_back(pathToGenericString(fs::path(source_replica_path) / "is_lost"));
        replicas_paths.emplace_back(pathToGenericString(fs::path(source_replica_path) / "log_pointer"));
        replicas_paths.emplace_back(pathToGenericString(fs::path(source_replica_path) / "queue"));
    }

    auto replicas_info = zookeeper->tryGet(replicas_paths);

    Strings log_entries = zookeeper->getChildren(pathToGenericString(fs::path(zookeeper_path) / "log"));
    ...
    zookeeper->removeChildren(pathToGenericString(fs::path(replica_path) / "queue"));
    queue.clear();
    ...
    zookeeper->set(pathToGenericString(fs::path(replica_path) / "is_lost"), "0");
}
    ...
    MergeType merge_type)
{
    ...
    for (const auto & part : parts)
        exists_paths.emplace_back(pathToGenericString(fs::path(replica_path) / "parts" / part->name));
    ...
    ops.emplace_back(zkutil::makeCreateRequest(
        pathToGenericString(fs::path(zookeeper_path) / "log/log-"), entry.toString(),
        zkutil::CreateMode::PersistentSequential));

    ops.emplace_back(zkutil::makeSetRequest(
        pathToGenericString(fs::path(zookeeper_path) / "log"), "", log_version)); /// Check and update version.
    ...
    const IMergeTreeDataPart & part, const UUID & new_part_uuid, Int64 mutation_version, int32_t alter_version, int32_t log_version)
{
    ...
    if (!zookeeper->exists(pathToGenericString(fs::path(replica_path) / "parts" / part.name)))
    {
        if (part.modification_time + MAX_AGE_OF_LOCAL_PART_THAT_WASNT_ADDED_TO_ZOOKEEPER < time(nullptr))
        {
...
void StorageReplicatedMergeTree::getRemovePartFromZooKeeperOps(const String & part_name, Coordination::Requests & ops, bool has_children)
{
    ...
    if (has_children)
    {
        ops.emplace_back(zkutil::makeRemoveRequest(pathToGenericString(fs::path(part_path) / "checksums"), -1));
        ops.emplace_back(zkutil::makeRemoveRequest(pathToGenericString(fs::path(part_path) / "columns"), -1));
    }
...
void StorageReplicatedMergeTree::removePartAndEnqueueFetch(const String & part_name, bool storage_init)
{
    ...
    while (true)
    {
        ...
        if (!storage_init)
        {
            ...
            ops.emplace_back(zkutil::makeCheckRequest(pathToGenericString(fs::path(zookeeper_path) / "log"), merge_predicate->getVersion()));
        }
        ...
        ops.emplace_back(zkutil::makeCreateRequest(
            pathToGenericString(fs::path(replica_path) / "queue/queue-"), log_entry->toString(),
            zkutil::CreateMode::PersistentSequential));
...
void StorageReplicatedMergeTree::startBeingLeader(const ZooKeeperRetriesInfo & zookeeper_retries_info)
{
    ...
    {
        auto zookeeper = getZooKeeper();
        zkutil::checkNoOldLeaders(log.load(), *zookeeper, pathToGenericString(fs::path(zookeeper_path) / "leader_election"));
    };
...
{
    auto zookeeper = getZooKeeper();
    return zookeeper->exists(pathToGenericString(fs::path(zookeeper_path) / "replicas" / replica / "parts" / part_name));
}
...
String StorageReplicatedMergeTree::findReplicaHavingPart(const String & part_name, bool active)
{
    ...
    for (const String & replica : replicas)
    {
        ...
        if (checkReplicaHavePart(replica, part_name) &&
            (!active || zookeeper->exists(pathToGenericString(fs::path(zookeeper_path) / "replicas" / replica / "is_active"))))
            return replica;
...
{
    if (!zookeeper_->exists(pathToGenericString(fs::path(zookeeper_path_) / "replicas" / replica_name_ / "is_active")))
    {
        LOG_INFO(log_, "Our replica is not active, nobody will try to fetch anything");
        return {};
    }

    Strings replicas = zookeeper_->getChildren(pathToGenericString(fs::path(zookeeper_path_) / "replicas"));
    Strings our_parts;
    std::vector<ActiveDataPartSet> data_parts_on_replicas;
    for (const String & replica : replicas)
    {
        if (!zookeeper_->exists(pathToGenericString(fs::path(zookeeper_path_) / "replicas" / replica / "is_active")))
        {
            LOG_TRACE(log_, "Replica {} is not active, skipping", replica);
            continue;
        }

        Strings parts = zookeeper_->getChildren(pathToGenericString(fs::path(zookeeper_path_) / "replicas" / replica / "parts"));
...
String StorageReplicatedMergeTree::findReplicaHavingCoveringPartImplLowLevel(LogEntry * entry, const String & part_name, String & found_part_name, bool active)
{
    ...
    for (const String & replica : replicas)
    {
        ...
        if (active && !zookeeper->exists(pathToGenericString(fs::path(zookeeper_path) / "replicas" / replica / "is_active")))
            continue;

        String largest_part_found;
        Strings parts = zookeeper->getChildren(pathToGenericString(fs::path(zookeeper_path) / "replicas" / replica / "parts"));
...
void StorageReplicatedMergeTree::updateQuorum(const String & part_name, bool is_parallel)
{
    ...
    String quorum_status_path = pathToGenericString(fs::path(zookeeper_path) / "quorum" / "status");
    if (is_parallel)
        quorum_status_path = pathToGenericString(fs::path(zookeeper_path) / "quorum" / "parallel" / part_name);
    /// The name of the previous part for which the quorum was reached.
    const String quorum_last_part_path = pathToGenericString(fs::path(zookeeper_path) / "quorum" / "last_part");
...
{
    auto zookeeper = getZooKeeper();
    return zookeeper->exists(pathToGenericString(fs::path(zookeeper_path) / "quorum" / "parallel" / part_info.getPartNameV1()));
}
...
bool StorageReplicatedMergeTree::partIsLastQuorumPart(const MergeTreePartInfo & part_info) const
{
    ...
    const String parts_with_quorum_path = pathToGenericString(fs::path(zookeeper_path) / "quorum" / "last_part");
    ...
    bool try_fetch_shared)
{
    ...
    DataPartPtr part_to_clone;
    {
        ...
        if (source_part && !is_zero_copy_part(source_part))
        {
            ...
            String part_path = pathToGenericString(fs::path(source_replica_path) / "parts" / part_name);
            ...
            else
            {
                ...
                if (zookeeper->tryGet(pathToGenericString(fs::path(part_path) / "columns"), columns_str) &&
                    zookeeper->tryGet(pathToGenericString(fs::path(part_path) / "checksums"), checksums_str))
                {
...
void StorageReplicatedMergeTree::getReplicaDelays(time_t & out_absolute_delay, time_t & out_relative_delay)
{
    ...
    for (const auto & replica : replicas)
    {
        ...
        replica_paths.push_back(pathToGenericString(fs::path(zookeeper_path) / "replicas" / replica / "is_active"));
        replica_paths.push_back(pathToGenericString(fs::path(zookeeper_path) / "replicas" / replica / "min_unprocessed_insert_time"));
    }
...
void StorageReplicatedMergeTree::forgetPartition(const ASTPtr & partition, ContextPtr query_context)
{
    ...
    String partition_id = getPartitionIDFromQuery(partition, query_context);
    String block_numbers_path = pathToGenericString(fs::path(zookeeper_path) / "block_numbers");
    String partition_path = pathToGenericString(fs::path(block_numbers_path) / partition_id);
...
void StorageReplicatedMergeTree::mutate(const MutationCommands & commands, ContextPtr query_context)
{
    ...
    while (true)
    {
        ...
        requests.emplace_back(zkutil::makeCreateRequest(
            pathToGenericString(fs::path(mutations_path) / ""), mutation_entry.toString(), zkutil::CreateMode::PersistentSequential));
...
void StorageReplicatedMergeTree::waitMutation(const String & znode_name, size_t mutations_sync) const
{
    ...
    {
        replicas = zookeeper->getChildren(pathToGenericString(fs::path(zookeeper_path) / "replicas"));
...
try
{
    ...
    {
        exists_paths.emplace_back(pathToGenericString(fs::path(replica_path) / "parts" / part_name));
    }
    ...
    zkutil::ZooKeeper & zookeeper, const String & partition_id, Int64 min_block_num, Int64 max_block_num)
{
    ...
    Strings queries_in_progress = zookeeper.getChildren(pathToGenericString(partition_path));
    ...
    for (const auto & block : queries_in_progress)
    {
        ...
        if (min_block_num <= block_number && block_number <= max_block_num)
            paths_to_get.push_back(pathToGenericString(partition_path / block));
    }
...
{
    Strings blocks;
    if (Coordination::Error::ZOK != zookeeper.tryGetChildren(pathToGenericString(fs::path(zookeeper_path) / blocks_dir_name), blocks))
    ...
    for (const String & block_id : blocks)
        if (startsWith(block_id, partition_prefix))
            paths_to_get.push_back(pathToGenericString(fs::path(zookeeper_path) / blocks_dir_name / block_id));
    ...
    const ContextPtr & query_context)
{
    ...
    for (size_t retry = 0; retry < 1000; ++retry)
    {
        ...
        for (const auto & src_part : src_all_parts)
        {
            ...
            std::vector<std::string> block_id_path = (replace || is_duplicated_part) ? std::vector<std::string>() : std::vector<std::string>{pathToGenericString((fs::path(zookeeper_path) / "blocks" / (partition_id + "_replace_from_" + hash_hex)))};
        ...
        try
        {
            ...
            ops.emplace_back(zkutil::makeSetRequest(pathToGenericString(fs::path(zookeeper_path) / "log"), "", -1));
            ops.emplace_back(zkutil::makeCreateRequest(pathToGenericString(fs::path(zookeeper_path) / "log/log-"), entry->toString(), zkutil::CreateMode::PersistentSequential));
...
void StorageReplicatedMergeTree::movePartitionToTable(const StoragePtr & dest_table, const ASTPtr & partition, ContextPtr query_context)
{
    ...
    for (size_t retry = 0; retry < 1000; ++retry)
    {
        ...
        try
        {
            ...
            /// Just update version, because merges assignment relies on it
            ops.emplace_back(zkutil::makeSetRequest(pathToGenericString(fs::path(dest_table_storage->zookeeper_path) / "log"), "", -1));
            ops.emplace_back(zkutil::makeCreateRequest(pathToGenericString(fs::path(dest_table_storage->zookeeper_path) / "log/log-"),
        ...
        ops_src.emplace_back(zkutil::makeCreateRequest(
            pathToGenericString(fs::path(zookeeper_path) / "log/log-"), entry_delete.toString(), zkutil::CreateMode::PersistentSequential));
        /// Just update version, because merges assignment relies on it
        ops_src.emplace_back(zkutil::makeSetRequest(pathToGenericString(fs::path(zookeeper_path) / "log"), "", -1));
    ...
    const std::vector<String> & block_id_paths) const
{
    ...
    {
        ops.emplace_back(zkutil::makeCreateRequest(
            pathToGenericString(fs::path(replica_path) / "parts" / part->name),
    ...
    else
    {
        ops.emplace_back(zkutil::makeCreateRequest(
            ...
            zkutil::CreateMode::Persistent));
        ops.emplace_back(zkutil::makeCreateRequest(
            pathToGenericString(fs::path(replica_path) / "parts" / part->name / "columns"),
            part->getColumns().toString(),
            zkutil::CreateMode::Persistent));
        ops.emplace_back(zkutil::makeCreateRequest(
            pathToGenericString(fs::path(replica_path) / "parts" / part->name / "checksums"),
    ...
    zkutil::ZooKeeperPtr & zookeeper, String part_name, LogEntry & entry, bool detach, bool throw_if_noop)
{
    ...
    while (true)
    {
        ...
        ops.emplace_back(zkutil::makeCheckRequest(pathToGenericString(fs::path(zookeeper_path) / "log"), merge_predicate->getVersion())); /// Make sure no new events were added to the log.
        ops.emplace_back(zkutil::makeCreateRequest(pathToGenericString(fs::path(zookeeper_path) / "log/log-"), entry.toString(), zkutil::CreateMode::PersistentSequential));
        /// Just update version, because merges assignment relies on it
        ops.emplace_back(zkutil::makeSetRequest(pathToGenericString(fs::path(zookeeper_path) / "log"), "", -1));
        Coordination::Responses responses;
    ...
    std::vector<size_t> & log_entry_ops_idx)
{
    ...
    ops.emplace_back(zkutil::makeCreateRequest(pathToGenericString(fs::path(zookeeper_path) / "log/log-"), entry->toString(),
                                               zkutil::CreateMode::PersistentSequential));
    delimiting_block_lock->getUnlockOp(ops);
...
void StorageReplicatedMergeTree::createTableSharedIDAttempt() const
{
    ...
    String zookeeper_table_id_path = pathToGenericString(fs::path(zookeeper_path) / "table_shared_id");
    String id;
...
zkutil::EphemeralNodeHolderPtr StorageReplicatedMergeTree::lockSharedDataTemporary(const String & part_name, const String & part_id, const DiskPtr & disk) const
{
    ...
    String zookeeper_node = pathToGenericString(fs::path(zc_zookeeper_path) / id / replica_name);
...
std::pair<bool, NameSet> getParentLockedBlobs(const ZooKeeperWithFaultInjectionPtr & zookeeper_ptr, const std::string & zero_copy_part_path_prefix, const MergeTreePartInfo & part_info, MergeTreeDataFormatVersion format_version, LoggerPtr log)
{
    ...
    for (const auto & [parent_candidate_info, part_candidate_info_str] : parts_infos | std::views::reverse)
    {
        ...
        if (part_info.isMutationChildOf(parent_candidate_info))
        {
            ...
            Coordination::Error code{};
            zookeeper_ptr->tryGet(pathToGenericString(fs::path(zero_copy_part_path_prefix) / part_candidate_info_str), files_not_to_remove_str, nullptr, nullptr, &code);
            if (code != Coordination::Error::ZOK)
            {
            ...
            else
            {
                ...
                code = zookeeper_ptr->tryGetChildren(pathToGenericString(fs::path(zero_copy_part_path_prefix) / part_candidate_info_str), children);
                if (code != Coordination::Error::ZOK)
                {
        ...
        LoggerPtr logger, const String & zookeeper_path_old, MergeTreeDataFormatVersion data_format_version, const ContextPtr & local_context)
{
    ...
    for (const auto & zc_zookeeper_path : zc_zookeeper_paths)
    {
        ...
        String zookeeper_part_uniq_node = pathToGenericString(fs::path(zc_zookeeper_path) / part_id);

        /// Delete our replica node for part from zookeeper (we are not interested in it anymore)
        String zookeeper_part_replica_node = pathToGenericString(fs::path(zookeeper_part_uniq_node) / replica_name_);

        auto [has_parent, parent_not_to_remove] = getParentLockedBlobs(
            zookeeper_ptr, pathToGenericString(fs::path(zc_zookeeper_path).parent_path()), part_info, data_format_version, logger);
        ...
        children.clear();
        String zookeeper_part_node = pathToGenericString(fs::path(zookeeper_part_uniq_node).parent_path());
    ...
    const IMergeTreeDataPart & part, const DataSourceDescription & data_source_description) const
{
    ...
    for (const auto & zc_zookeeper_path : zc_zookeeper_paths)
    {
        ...
        {
            String zookeeper_part_uniq_node = pathToGenericString(fs::path(zc_zookeeper_path) / id);
            Strings id_replicas;
    ...
    for (const String & replica : replicas)
        if ((replica != replica_name) && (zookeeper->exists(pathToGenericString(fs::path(zookeeper_path) / "replicas" / replica / "is_active"))))
            active_replicas.push_back(replica);
    ...
    */
    ...
    for (const String & replica : active_replicas)
    {
        String current_replica_path = pathToGenericString(fs::path(zookeeper_path) / "replicas" / replica);
    ...
    const String & part_name, const String & zookeeper_path_old, const ContextPtr & local_context)
{
    ...
    String new_path
        = pathToGenericString(fs::path(local_context->getMacros()->expand(settings[MergeTreeSetting::remote_fs_zero_copy_zookeeper_path].toString()))
    ...
    { /// Compatibility mode for cluster with old and new versions
        String old_path = pathToGenericString(fs::path(zookeeper_path_old) / zero_copy / "shared" / part_name);
...
void StorageReplicatedMergeTree::watchZeroCopyLock(const String & part_name, const DiskPtr & disk)
{
    ...
    if (path)
    {
        ...
        bool exists = zookeeper->tryGetWatch(pathToGenericString(lock_path), replica, nullptr, watch);

        if (exists)
        {
            std::lock_guard lock(existing_zero_copy_locks_mutex);
            existing_zero_copy_locks[pathToGenericString(lock_path)] = ZeroCopyLockDescription{replica, flag};
        }
    }
}
...
bool StorageReplicatedMergeTree::checkZeroCopyLockExists(const String & part_name, const DiskPtr & disk, String & lock_replica)
{
    ...
    {
        auto lock_path = fs::path(*path) / "part_exclusive_lock";
        if (auto it = existing_zero_copy_locks.find(pathToGenericString(lock_path)); it != existing_zero_copy_locks.end())
        {
...
{
    Strings replicas = zookeeper_ptr->getChildren(pathToGenericString(fs::path(zookeeper_path_) / "replicas"));
    ...
    for (const String & replica : replicas)
    {
        if (zookeeper_ptr->exists(pathToGenericString(fs::path(zookeeper_path_) / "replicas" / replica / "parts" / part_name))
            && zookeeper_ptr->exists(pathToGenericString(fs::path(zookeeper_path_) / "replicas" / replica / "is_active")))
            return pathToGenericString(fs::path(zookeeper_path_) / "replicas" / replica);
    }
...
bool StorageReplicatedMergeTree::createEmptyPartInsteadOfLost(zkutil::ZooKeeperPtr zookeeper, const String & lost_part_name)
{
    ...
    try
    {
        ...
        while (true)
        {
            ...
            Strings replicas = zookeeper->getChildren(pathToGenericString(replicas_path), &replicas_stat);
            ...
            ops.emplace_back(zkutil::makeCheckRequest(pathToGenericString(replicas_path), replicas_stat.version));

            for (const String & replica : replicas)
            {
                String current_part_path = pathToGenericString(fs::path(zookeeper_path) / "replicas" / replica / "parts" / lost_part_name);
            ...
            {
                UInt64 lost_part_count = lost_part_count_str.empty() ? 0 : parse<UInt64>(lost_part_count_str);
                ops.emplace_back(zkutil::makeSetRequest(pathToGenericString(lost_part_count_path), toString(lost_part_count + 1), lost_part_count_stat.version));
            }
            else
            {
                ops.emplace_back(zkutil::makeCreateRequest(pathToGenericString(lost_part_count_path), "1", zkutil::CreateMode::Persistent));
            }
...
void StorageReplicatedMergeTree::applyMetadataChangesToCreateQueryForBackup(const ASTPtr & create_query) const
{
    ...
    try
    {
        ...
        auto zookeeper = getZooKeeper();
        auto columns_from_entry = ColumnsDescription::parse(zookeeper->get(pathToGenericString(fs::path(zookeeper_path) / "columns")));
        auto current_metadata = getInMemoryMetadataPtr(getContext(), false);
        auto metadata_from_entry = ReplicatedMergeTreeTableMetadata::parseAndNormalize(
            zookeeper->get(pathToGenericString(fs::path(zookeeper_path) / "metadata")), columns_from_entry,
            current_metadata->add_minmax_index_for_numeric_columns,
            current_metadata->add_minmax_index_for_string_columns,
            getContext());
        const auto table_metadata = ReplicatedMergeTreeTableMetadata(*this, current_metadata);
    ...
    /// Send a list of mutations to the coordination too (we need to find the mutations which are not finished for added part names).
    {
        ...
        Strings mutation_ids;
        {
            ...
            retries_ctl.retryLoop([&]()
            {
                ...
                exists = (zookeeper->tryGetChildren(pathToGenericString(mutations_node_path), mutation_ids) == Coordination::Error::ZOK);
            });
        }
        ...
        if (exists)
        {
            ...
            for (const auto & mutation_id : mutation_ids)
            {
                ...
                retries_ctl.retryLoop([&]()
                {
                    if (!zookeeper || zookeeper->expired())
                        zookeeper = local_context->getZooKeeper();
                    mutation_id_exists = zookeeper->tryGet(pathToGenericString(mutations_node_path / mutation_id), mutation);
                });
