    ...
    const String & source_replica_path, bool to_detached, size_t quorum, zkutil::ZooKeeper::Ptr zookeeper_, bool try_fetch_shared)
{
    ...
    if (part_to_clone)
    {
        ...
        {
            auto [cloned_part, lock] = cloneAndLoadDataPart(part_to_clone, "tmp_clone_", part_info, metadata_snapshot, NO_TRANSACTION_PTR, &hardlinked_files, false);
    ...
    const StoragePtr & source_table, const ASTPtr & partition, bool replace, ContextPtr query_context)
{
    ...
    for (size_t retry = 0; retry < 1000; ++retry)
    {
        ...
        for (const auto & src_part : src_all_parts)
        {
            ...
            auto [dst_part, part_lock] = cloneAndLoadDataPart(src_part, TMP_PREFIX, dst_part_info, metadata_snapshot, NO_TRANSACTION_PTR, &hardlinked_files, copy_instead_of_hardlink);
...
void StorageReplicatedMergeTree::movePartitionToTable(const StoragePtr & dest_table, const ASTPtr & partition, ContextPtr query_context)
{
    ...
    for (size_t retry = 0; retry < 1000; ++retry)
    {
        ...
        for (const auto & src_part : src_all_parts)
        {
            ...
            auto [dst_part, dst_part_lock] = dest_table_storage->cloneAndLoadDataPart(src_part, TMP_PREFIX, dst_part_info, dest_metadata_snapshot, NO_TRANSACTION_PTR, &hardlinked_files, copy_instead_of_hardlink);
