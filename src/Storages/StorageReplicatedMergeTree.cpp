...
bool StorageReplicatedMergeTree::executeLogEntry(LogEntry & entry)
{
    ...
    if (is_get_or_attach || entry.type == LogEntry::MERGE_PARTS || entry.type == LogEntry::MUTATE_PART)
    {
        ...
        if (existing_part)
        {
            if (getZooKeeper()->exists(zkutil::joinZooKeeperPath(replica_path, "parts", existing_part->name)))
            {
    ...
    Int64 wait_for_inactive_timeout, WatchEventByPath & watch_events, bool only_active)
{
    ...
    for (const String & replica : replicas)
    {
        const String is_active_path = zkutil::joinZooKeeperPath(table_zookeeper_path, "replicas", replica, "is_active");
    ...
    Coordination::EventPtr & watch_event)
{
    ...
    else if (!entry.log_entry_id.empty())
    {
        ...
        String log_pointer;
        if (!getZooKeeper()->tryGet(zkutil::joinZooKeeperPath(table_zookeeper_path, "replicas", replica, "log_pointer"), log_pointer))
            return false;
    ...
    if (getZooKeeper()->tryGetChildren(zkutil::joinZooKeeperPath(table_zookeeper_path, "replicas", replica, "queue"), queue_entries)
