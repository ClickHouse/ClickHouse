...
#define RABBITMQ_RELATED_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(ExtStreamingHandleErrorMode, rabbitmq_handle_error_mode, ExtStreamingHandleErrorMode::DEFAULT, "How to handle errors for RabbitMQ engine. Possible values: default (throw an exception after rabbitmq_skip_broken_messages broken messages), stream (save broken messages and errors in virtual columns _raw_message, _error), dead_letter_queue (error related data will be saved in system.dead_letter_queue).", 0) \
