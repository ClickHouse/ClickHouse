        ...
        , milliseconds_to_wait(rabbitmq_settings->rabbitmq_empty_queue_backoff_start_ms)
{
    ...
    try
    {
        ...
        if (connection->connect())
            initRabbitMQ();
        else if (!allow_delayed_initialization)
...
void registerStorageRabbitMQ(StorageFactory & factory)
{
    auto creator_fn = [](const StorageFactory::Arguments & args)
    {
        ...
        return std::make_shared<StorageRabbitMQ>(args.table_id, args.getContext(), args.columns, std::move(rabbitmq_settings), args.attach || args.replicated_create);
    };
