...
#define NATS_RELATED_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(String, nats_ca_file, "", "Path to a file with the trusted CA certificates used to verify the NATS server certificate. Requires nats_secure. Accepted only from the server configuration file or from a named collection defined in it whose destination is not overridden by the query", 0) \
    DECLARE(String, nats_client_cert_file, "", "Path to the client certificate presented to the NATS server. Requires nats_secure and nats_client_key_file. Accepted only from the server configuration file or from a named collection defined in it whose destination is not overridden by the query", 0) \
    DECLARE(String, nats_client_key_file, "", "Path to the private key of nats_client_cert_file. Accepted only from the server configuration file or from a named collection defined in it whose destination is not overridden by the query", 0) \
