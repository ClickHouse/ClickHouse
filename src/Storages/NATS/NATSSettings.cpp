...
#define NATS_RELATED_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(String, nats_credential_file, "", "Path to a NATS credentials file. Accepted only from a named collection defined in the server configuration file, or as `nats.credential_file` in the server configuration itself", 0) \
