...
{
    /// Note that default here means default by implementation and not by nats settings
...
StorageNATS::StorageNATS(
        ...
        , exchange_name(getContext()->getMacros()->expand(nats_settings->nats_exchange_name))
        , format_name(getContext()->getMacros()->expand(nats_settings->nats_format))
        , exchange_type(defineExchangeType(getContext()->getMacros()->expand(nats_settings->nats_exchange_type)))
        , routing_keys(parseSettings(getContext()->getMacros()->expand(nats_settings->nats_routing_key_list)))
        ...
        , num_queues(nats_settings->nats_num_queues.value)
        , queue_base(getContext()->getMacros()->expand(nats_settings->nats_queue_base))
        , queue_settings_list(parseSettings(getContext()->getMacros()->expand(nats_settings->nats_queue_settings_list)))
        ...
        , is_attach(is_attach_)
{
    ...
    try
    {
        connection = std::make_unique<NATSConnection>(configuration, log);
    ...
    connection_task = getContext()->getMessageBrokerSchedulePool().createTask("NATSConnectionTask", [this]{ connectionFunc(); });
    connection_task->deactivate();
}


Names StorageNATS::parseSettings(String settings_list)
{
    Names result;
...
AMQP::ExchangeType StorageNATS::defineExchangeType(String exchange_type_)
{
...
void StorageNATS::initExchange(AMQP::TcpChannel & rabbit_channel)
{
    /// Exchange hierarchy:
...
void StorageNATS::bindExchange(AMQP::TcpChannel & rabbit_channel)
{
    size_t bound_keys = 0;
...
void StorageNATS::bindQueue(size_t queue_id, AMQP::TcpChannel & rabbit_channel)
{
        ...
        unsigned /* num_streams */)
{
    ...
    {
        auto rabbit_source = std::make_shared<NATSSource>(
