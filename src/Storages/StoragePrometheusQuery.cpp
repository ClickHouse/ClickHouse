...
StoragePrometheusQuery::Configuration StoragePrometheusQuery::getConfiguration(ASTs & args, ContextPtr context, bool over_range)
{
    ...
    max_scale = std::max(max_scale, findMaxTimeScaleInPrometheusQuery(promql_tree));
    ...
    if (over_range)
    {
        ...
        String start_time_timezone = getTimeseriesTimezone(start_time_type);
        String end_time_timezone = getTimeseriesTimezone(start_time_type);
        if (!start_time_timezone.empty() && (start_time_timezone == end_time_timezone))
            timezone = start_time_timezone;

        evaluation_range.emplace();
        evaluation_range->start_time = getTimeseriesTime(start_time_field, start_time_type, max_scale);
        evaluation_range->end_time = getTimeseriesTime(end_time_field, end_time_type, max_scale);
        evaluation_range->step = getTimeseriesDuration(step_field, step_type, max_scale);
    }
    else
    {
        auto [evaluation_time_field, evaluation_time_type] = evaluateConstantExpression(args[argument_index++], context);

        max_scale = std::max(max_scale, getTimeseriesScale(evaluation_time_type));
        String evaluation_time_timezone = getTimeseriesTimezone(evaluation_time_type);
        if (!evaluation_time_timezone.empty())
            timezone = evaluation_time_timezone;

        evaluation_time = getTimeseriesTime(evaluation_time_field, evaluation_time_type, max_scale);
    }
    ...
    evaluation_settings.result_timestamp_type = getTimeseriesTimeType(max_scale, timezone);
