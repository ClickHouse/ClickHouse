...
ASTPtr SingleHiveQueryTaskFilesCollector::extractKeyExpressionList(const ASTPtr & node)
{
    if (!node)
...
HiveFiles SingleHiveQueryTaskFilesCollector::collectHiveFilesFromPartition(
...
HiveFilePtr SingleHiveQueryTaskFilesCollector::createHiveFileIfNeeded(
