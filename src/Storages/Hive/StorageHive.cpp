...
public:
    ...
    StorageHiveSource(
        ...
        , max_block_size_v(max_block_size_)
...
StorageHive::totalRowsImpl(const Settings & settings, const ActionsDAG * filter_actions_dag, ContextPtr context_, PruneLevel prune_level) const
{
    ...
    HDFSFSPtr fs = createHDFSFS(builder.get());
    HiveFiles hive_files = collectHiveFiles(settings[max_threads], filter_actions_dag, hive_table_metadata, fs, context_, prune_level);
