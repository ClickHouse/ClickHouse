...
Chunk StorageFileSource::generate()
{
    ...
    /// it is fatal (`error_code` is the code of the caught `Exception`, or empty for foreign exceptions)
    /// or if skipping is disabled, otherwise logs a warning and resets reader state so the outer loop
    /// can advance to the next file. Must be invoked from within a `catch` block.
    auto skip_or_rethrow = [&](std::optional<int> error_code)
    {
        if (error_code && isFatalForSkipFailedFiles(*error_code))
            throw;
