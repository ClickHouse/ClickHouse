...
namespace
{
    struct ReadBufferFromIterator : public IReadBufferIterator, WithContext
    {
    public:
        ReadBufferFromIterator(
        ...
        {
            if (!getContext()->getSettingsRef().schema_inference_use_cache_for_file || !paths_to_archive.empty())
                return;
    ...
    const std::vector<String> & paths_to_archive)
{
    ...
    {
        ReadBufferFromIterator read_buffer_iterator(paths, paths_to_archive, format, compression_method, format_settings, context);
