...
void registerStorageRemote(StorageFactory & factory)
{
    ...
    {
        /// Parsing and target validation are shared with the initiator-side preflight of an
        /// `ON CLUSTER` query, which reaches this engine's requirements without constructing the
        /// storage (see `validateRemoteEngineTarget.h`).
        auto validated = parseAndValidateRemoteEngineTarget(
            args.engine_args,
            args.getLocalContext(),
            args.mode,
            args.query.attach_short_syntax,
            /* columns_given = */ !args.columns.empty(),
            secure,
