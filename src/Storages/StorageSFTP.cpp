#include <Storages/StorageSFTP.h>
...
*/
                                               ...
                                               std::vector<std::string> & result)
    {
        ...
        dirs.emplace(path_for_ls);
        while (!dirs.empty())
        {
            const String full_path = dirs.front();
...
Strings StorageSFTP::getPathsList(const String & table_path, const String & user_files_path, ContextPtr context, size_t & total_bytes_to_read)
{
...
ColumnsDescription StorageSFTP::getTableStructureFromFileDescriptor(ContextPtr context)
{
...
ColumnsDescription StorageSFTP::getTableStructureFromFile(
    const String & format,
...
StorageSFTP::StorageSFTP(const std::string & table_path_, const std::string & user_files_path, CommonArguments args)
    : StorageSFTP(args)
{
...
StorageSFTP::StorageSFTP(const std::string & relative_table_dir_path, CommonArguments args)
    : StorageSFTP(args)
{
...
StorageSFTP::StorageSFTP(CommonArguments args)
...
void StorageSFTP::setStorageMetadata(CommonArguments args)
{
...
class StorageSFTPSource : public ISource
{
public:
    class FilesIterator
    {
    ...
    StorageSFTPSource(
        ...
        std::shared_ptr<StorageSFTP> storage_,
        ...
        if (std::uncaught_exceptions() == 0 && cnt == 1 && !storage->was_renamed)
        {
            ...
            for (auto & file_path_ref : storage->paths)
            {
                ...
                catch (const std::exception & e)
                {
                    ...
                    LOG_ERROR(&Poco::Logger::get("~StorageSFTPSource"), "Failed to rename file {}: {}", file_path_ref, e.what());
                    continue;
                }
            }
        }
...
private:
    std::shared_ptr<StorageSFTP> storage;
    ...
    const size_t max_num_streams)
{
    ...
    auto files_iterator = std::make_shared<StorageSFTPSource::FilesIterator>(paths, paths_to_archive, std::move(files_in_archive));
    ...
    for (size_t i = 0; i < num_streams; ++i)
    {
        ...
        /// If yes, then we should use it in StorageSFTPSource. Atomic bool flag is needed
        ...
        pipes.emplace_back(std::make_shared<StorageSFTPSource>(
...
class StorageSFTPSink final : public SinkToStorage
{
public:
    StorageSFTPSink(
    ...
    String getName() const override { return "StorageSFTPSink"; }
...
class PartitionedStorageSFTPSink : public PartitionedSink
{
public:
    PartitionedStorageSFTPSink(
    ...
    SinkPtr createSinkForPartition(const String & partition_id) override
    {
        ...
        return std::make_shared<StorageSFTPSink>(
            metadata_snapshot,
            table_name_for_log,
            -1,
            /* use_table_fd */false,
...
SinkToStoragePtr StorageSFTP::write(
    const ASTPtr & query,
    ...
    bool /*async_insert*/)
{
    ...
    if (is_partitioned_implementation)
    {
        ...
        return std::make_shared<PartitionedStorageSFTPSink>(
...
bool StorageSFTP::storesDataOnDisk() const
{
    return is_db_table;
}

Strings StorageSFTP::getDataPaths() const
{
    if (paths.empty())
...
void StorageSFTP::rename(const String & new_path_to_table_data, const StorageID & new_table_id)
{
    if (!is_db_table)
        throw Exception(ErrorCodes::DATABASE_ACCESS_DENIED,
...
void registerStorageSFTP(StorageFactory & factory)
{
    ...
    factory.registerStorage(
        ...
        {
            StorageSFTP::CommonArguments storage_args
                {
            ...
            if (engine_args_ast.size() == 1) /// Table in database
                return std::make_shared<StorageSFTP>(factory_args.relative_data_path, storage_args);
            ...
            else if (const auto * literal = engine_args_ast[1]->as<ASTLiteral>())
            {
                ...
                if (type == Field::Types::Int64)
                    ...
                    StorageSFTP::parseFileSource(literal->value.get<String>(), source_path, storage_args.path_to_archive);
                else
            ...
            if (0 <= source_fd) /// File descriptor
                return std::make_shared<StorageSFTP>(source_fd, storage_args);
            else /// User's file
                return std::make_shared<StorageSFTP>(source_path, factory_args.getContext()->getUserFilesPath(), storage_args);
        },
        storage_features);
}
...
SchemaCache & StorageSFTP::getSchemaCache(const ContextPtr & context)
{
...
void StorageSFTP::parseFileSource(String source, String & filename, String & path_to_archive)
{
