...
private:
    ...
    /// shards then sort by one type and the initiator merges by another. `std::nullopt` means the
    /// sorted-by columns are unknown, and then every column is checked. The remote table is only
