...
{
    const NamesAndTypesList & columns_in_data_file = storage_snapshot->metadata->getColumns().getAllPhysical();
    ...
    info.requested_columns
        = storage_snapshot->getColumnsByNames(GetColumnsOptions(GetColumnsOptions::All).withSubcolumns(), columns_to_read);
