...
#define KAFKA_RELATED_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(String, kafka_autodetect_client_rack, "", "Use cloud facilities to set client.rack to communicate with the nearest kafka replica. Supported: 'MSK' (with alias 'AWS/IMDSv2/availability-zone-id'), 'CONFLUENT' (with alias 'AWS/IMDSv2/availability-zone'), 'GCP' (with alias 'GCP/MDS/zone') and 'CLICKHOUSE'", 0) \
