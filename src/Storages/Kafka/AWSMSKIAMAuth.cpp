...
namespace
{
    ...
    void oauthBearerTokenRefreshCallback(
        ...
        void * /*opaque*/)
    {
        auto log = getLogger("AWSMSKIAMAuth");
                ...
                try
                {
                    ...
                    for (const auto & [key, value] : params)
                    {
                        if (key == "region")
                            ...
                            use_environment_credentials = stringToBool(value);
                    }
                }
                catch (const Poco::Exception & e)
                {
                    throw Exception(ErrorCodes::BAD_ARGUMENTS,
                        "Failed to parse OAuth bearer configuration: {}. Config: '{}'",
                        e.displayText(), oauthbearer_config);
                }
            }
