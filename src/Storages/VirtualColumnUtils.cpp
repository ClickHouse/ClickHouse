    ...
    if (node->type == ActionsDAG::ActionType::FUNCTION)
    {
        ...
        else if (node->function_base->getName() == "or")
        {
            ...
            for (auto & child : node_copy.children)
                if (child = splitFilterNodeForAllowedInputs(child, allowed_inputs, additional_nodes, strict); !child)
                    return nullptr;
            ...
            if (const auto * adaptor = typeid_cast<const FunctionToFunctionBaseAdaptor *>(node->function_base.get()))
            {
                if (const auto * index_hint = typeid_cast<const FunctionIndexHint *>(adaptor->getFunction().get()))
                {
                    ...
                    for (const auto & output : index_hint_dag->getOutputs())
                        if (const auto * child_copy = splitFilterNodeForAllowedInputs(output, allowed_inputs, additional_nodes, strict))
...
ActionsDAGPtr splitFilterDagForAllowedInputs(const ActionsDAG::Node * predicate, const Block * allowed_inputs, bool strict)
{
    if (!predicate)
        return nullptr;

    ActionsDAG::Nodes additional_nodes;
    const auto * res = splitFilterNodeForAllowedInputs(predicate, allowed_inputs, additional_nodes, strict);
    if (!res)
        return nullptr;
...
{
    auto dag = splitFilterDagForAllowedInputs(predicate, &block,  /*strict=*/ false);
    if (dag)
        filterBlockWithDAG(dag, block, context);
}
