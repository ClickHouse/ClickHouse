...
void RefreshTask::executeRefresh()
{
    ...
    try
    {
        ...
        new_table_uuid = executeRefreshUnlocked(root_znode_version, deps, log_comment, error_message, cursor_after_refresh, if_changed, out_of_schedule, watermark, unchanged);
    }
    catch (...)
    {
...
std::optional<UUID> RefreshTask::executeRefreshUnlocked(int32_t root_znode_version, std::vector<StorageID> deps, const String & log_comment, String & out_error_message, CursorTreeNodePtr & out_cursor, bool if_changed, bool out_of_schedule, IfChangedWatermark & watermark, bool & out_unchanged)
{
