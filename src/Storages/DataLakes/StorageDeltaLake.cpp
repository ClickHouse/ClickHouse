...
bool StorageDeltaLake::parallelizeOutputAfterReading(ContextPtr context) const
{
    return FormatFactory::instance().checkParallelizeOutputAfterReading(configuration->format, context);
}

bool StorageDeltaLake::supportsSubsetOfColumns(const ContextPtr & context) const
{
    return FormatFactory::instance().checkIfFormatSupportsSubsetOfColumns(configuration->format, context, format_settings);
}

bool StorageDeltaLake::supportsPrewhere() const
{
    return supports_prewhere;
}

bool StorageDeltaLake::canMoveConditionsToPrewhere() const
{
    return supports_prewhere;
}
...
std::optional<UInt64> StorageDeltaLake::totalRows(ContextPtr query_context) const
{
    if (!DeltaLakeMetadata::supportsTotalRows(query_context, object_storage->getType()))
        return std::nullopt;
...
{
    if (!DeltaLakeMetadata::supportsTotalBytes(query_context, object_storage->getType()))
        return std::nullopt;
...
SinkToStoragePtr StorageDeltaLake::write(
    const ASTPtr &,
...
ColumnsDescription StorageDeltaLake::resolveSchemaFromData(
    const ObjectStoragePtr & object_storage,
    const StorageObjectStorageConfigurationPtr & configuration,
    const std::optional<FormatSettings> & format_settings,
    std::string & sample_path,
...
std::string StorageDeltaLake::resolveFormatFromData(
    const ObjectStoragePtr & object_storage,
    const StorageObjectStorageConfigurationPtr & configuration,
    const std::optional<FormatSettings> & format_settings,
    std::string & sample_path,
...
std::pair<ColumnsDescription, std::string> StorageDeltaLake::resolveSchemaAndFormatFromData(
    const ObjectStoragePtr & object_storage,
    const StorageObjectStorageConfigurationPtr & configuration,
    const std::optional<FormatSettings> & format_settings,
    std::string & sample_path,
...
void StorageDeltaLake::addInferredEngineArgsToCreateQuery(ASTs & args, const ContextPtr & context) const
{
    configuration->addStructureAndFormatToArgsIfNeeded(args, "", configuration->format, context, /*with_structure=*/false);
}

SchemaCache & StorageDeltaLake::getSchemaCache(const ContextPtr & context, const std::string & storage_engine_name)
{
...
void StorageDeltaLake::mutate([[maybe_unused]] const MutationCommands & commands, [[maybe_unused]] ContextPtr context_)
{
...
void StorageDeltaLake::alter(const AlterCommands & params, ContextPtr context, AlterLockHolder & /*alter_lock_holder*/)
{
