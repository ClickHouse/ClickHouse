    ...
    , storage_id(table_id_)
{
    ...
    LOG_DEBUG(
        log, "StoragePaimon: lazy_init={}, need_resolve_columns_or_format={}, "
    ...
    try
    {
        ...
        {
            configuration->update(object_storage, context);
            current_metadata = PaimonMetadata::create(object_storage, configuration, context).release();
        }
    }
    catch (...)
    {
    ...
    supports_prewhere = false; /// Prewhere not yet supported for this engine
...
ColumnsDescription StoragePaimon::resolveSchemaFromData(
    const ObjectStoragePtr & object_storage,
    const StorageObjectStorageConfigurationPtr & configuration,
    const std::optional<FormatSettings> & format_settings,
    std::string & sample_path,
...
std::string StoragePaimon::resolveFormatFromData(
    const ObjectStoragePtr & object_storage,
    const StorageObjectStorageConfigurationPtr & configuration,
    const std::optional<FormatSettings> & format_settings,
    std::string & sample_path,
...
std::pair<ColumnsDescription, std::string> StoragePaimon::resolveSchemaAndFormatFromData(
    const ObjectStoragePtr & object_storage,
    const StorageObjectStorageConfigurationPtr & configuration,
    const std::optional<FormatSettings> & format_settings,
    std::string & sample_path,
...
void StoragePaimon::addInferredEngineArgsToCreateQuery(ASTs & args, const ContextPtr & context) const
{
    configuration->addStructureAndFormatToArgsIfNeeded(args, "", configuration->format, context, /*with_structure=*/false);
}

SchemaCache & StoragePaimon::getSchemaCache(const ContextPtr & context, const std::string & storage_engine_name)
{
