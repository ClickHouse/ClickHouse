...
std::string StorageIcebergCluster::getName() const
{
    return configuration->getEngineName();
}

std::optional<UInt64> StorageIcebergCluster::totalRows(ContextPtr query_context) const
{
...
std::optional<UInt64> StorageIcebergCluster::totalBytes(ContextPtr query_context) const
{
    configuration->lazyInitializeIfNeeded(
        object_storage,
        query_context);
...
void StorageIcebergCluster::updateQueryToSendIfNeeded(
    ASTPtr & query,
...
RemoteQueryExecutor::Extension StorageIcebergCluster::getTaskIteratorExtension(
