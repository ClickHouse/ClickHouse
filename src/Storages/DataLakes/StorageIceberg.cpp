...
String StorageIceberg::getPathSample(ContextPtr context)
{
    ...
    , storage_id(table_id_)
{
    ...
    LOG_DEBUG(
        log, "StorageIceberg: lazy_init={}, need_resolve_columns_or_format={}, "
...
bool StorageIceberg::parallelizeOutputAfterReading(ContextPtr context) const
{
    return FormatFactory::instance().checkParallelizeOutputAfterReading(configuration->format, context);
}

bool StorageIceberg::supportsSubsetOfColumns(const ContextPtr & context) const
{
    return FormatFactory::instance().checkIfFormatSupportsSubsetOfColumns(configuration->format, context, format_settings);
}

bool StorageIceberg::supportsPrewhere() const
{
    return supports_prewhere;
}

bool StorageIceberg::canMoveConditionsToPrewhere() const
{
    return supports_prewhere;
}

std::optional<NameSet> StorageIceberg::supportedPrewhereColumns() const
{
    return getInMemoryMetadataPtr()->getColumnsWithoutDefaultExpressions(/*exclude=*/ hive_partition_columns_to_read_from_file_path);
}

IStorage::ColumnSizeByName StorageIceberg::getColumnSizes() const
{
...
void StorageIceberg::updateExternalDynamicMetadataIfExists(ContextPtr query_context)
{
    if (!configuration->isDataLakeConfiguration())
        return;
...
SinkToStoragePtr StorageIceberg::write(
    const ASTPtr &,
...
std::unique_ptr<ReadBufferIterator> StorageIceberg::createReadBufferIterator(
...
ColumnsDescription StorageIceberg::resolveSchemaFromData(
    const ObjectStoragePtr & object_storage,
    const StorageObjectStorageConfigurationPtr & configuration,
    const std::optional<FormatSettings> & format_settings,
    std::string & sample_path,
...
std::string StorageIceberg::resolveFormatFromData(
    const ObjectStoragePtr & object_storage,
    const StorageObjectStorageConfigurationPtr & configuration,
    const std::optional<FormatSettings> & format_settings,
    std::string & sample_path,
...
std::pair<ColumnsDescription, std::string> StorageIceberg::resolveSchemaAndFormatFromData(
    const ObjectStoragePtr & object_storage,
    const StorageObjectStorageConfigurationPtr & configuration,
    const std::optional<FormatSettings> & format_settings,
    std::string & sample_path,
...
void StorageIceberg::addInferredEngineArgsToCreateQuery(ASTs & args, const ContextPtr & context) const
{
    configuration->addStructureAndFormatToArgsIfNeeded(args, "", configuration->format, context, /*with_structure=*/false);
}

SchemaCache & StorageIceberg::getSchemaCache(const ContextPtr & context, const std::string & storage_engine_name)
{
...
void StorageIceberg::mutate([[maybe_unused]] const MutationCommands & commands, [[maybe_unused]] ContextPtr context_)
{
...
void StorageIceberg::checkMutationIsPossible(const MutationCommands & commands, const Settings & /* settings */) const
{
    configuration->checkMutationIsPossible(commands);
}

Pipe StorageIceberg::executeCommand(const String & command_name, const ASTPtr & args, ContextPtr context)
{
...
void StorageIceberg::alter(const AlterCommands & params, ContextPtr context, AlterLockHolder & /*alter_lock_holder*/)
{
...
void StorageIceberg::checkAlterIsPossible(const AlterCommands & commands, ContextPtr /*context*/) const
{
