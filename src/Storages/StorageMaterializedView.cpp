    ...
    : IStorage(table_id_), WithMutableContext(local_context->getGlobalContext())
{
    ...
    auto to_table_id = query.getTargetTableID();
    has_inner_table = to_table_id.empty();
    auto to_inner_uuid = query.getTargetInnerUUID();
    auto inner_table_engine = query.getTargetTableEngine();

    if (has_inner_table && !inner_table_engine)
    ...
    if (inner_table_engine && inner_table_engine->primary_key)
        storage_metadata.primary_key = KeyDescription::getKeyFromAST(inner_table_engine->primary_key->ptr(),
    ...
    {
        const String & engine = inner_table_engine->engine->name;
        ...
        if (inner_table_engine)
            manual_create_query->set(manual_create_query->storage, inner_table_engine);
