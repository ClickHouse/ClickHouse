...
{
class ASTStorage;
struct KinesisSettingsImpl;

#define KINESIS_SETTINGS_SUPPORTED_TYPES(CLASS_NAME, M) \
...
KINESIS_SETTINGS_SUPPORTED_TYPES(KinesisSettings, DECLARE_SETTING_TRAIT)

struct KinesisSettings
{
    KinesisSettings();
    KinesisSettings(const KinesisSettings & settings);
    KinesisSettings(KinesisSettings && settings) noexcept;
    ~KinesisSettings();

    KINESIS_SETTINGS_SUPPORTED_TYPES(KinesisSettings, DECLARE_SETTING_SUBSCRIPT_OPERATOR)
...
private:
    std::unique_ptr<KinesisSettingsImpl> impl;
};
