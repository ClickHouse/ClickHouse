...
#include <IO/ReadBufferFromCOS.h>
#include <IO/ReadHelpers.h>
#include <IO/WriteBufferFromCOS.h>
...
namespace ErrorCodes
{
    ...
    extern const int COS_ERROR;
}


namespace
{
    class StorageCOSSource : public SourceWithProgress
    {
    public:
        ...
        {
            read_buf = wrapReadBufferWithCompressionMethod(std::make_unique<ReadBufferFromCOS>(client, bucket, key), compression_method);
    ...
    class StorageCOSBlockOutputStream : public IBlockOutputStream
    {
    public:
        StorageCOSBlockOutputStream(
            const String & format,
            ...
            : sample_block(sample_block_)
        {
            write_buf = wrapWriteBufferWithCompressionMethod(
                std::make_unique<WriteBufferFromCOS>(client, bucket, key, min_upload_part_size), compression_method, 3);
    ...
    , compression_method(compression_method_)
{
    ...
    client = S3::ClientFactory::instance().create(
        uri_.endpoint, uri_.region, uri_.is_https_scheme, access_key_id_, secret_access_key_);
}
 ...
 */
Strings listFilesWithRegexpMatching(Aws::S3::S3Client & client, const COS::URI & globbed_uri)
{
    ...
    do
    {
        ...
        if (!outcome.IsSuccess())
        {
            ...
            throw Exception(message.str(), ErrorCodes::COS_ERROR);
        }
    ...
    unsigned num_streams)
{
    ...
    for (const String & key : listFilesWithRegexpMatching(*client, uri))
        pipes.emplace_back(std::make_shared<StorageCOSSource>(
            need_path_column,
            need_file_column,
            format_name,
            getName(),
...
BlockOutputStreamPtr StorageCOS::write(const ASTPtr & /*query*/, const StorageMetadataPtr & metadata_snapshot, const Context & /*context*/)
{
    return std::make_shared<StorageCOSBlockOutputStream>(
...
{
    factory.registerStorage("COS", [](const StorageFactory::Arguments & args)
    {
        ASTs & engine_args = args.engine_args;

        if (engine_args.size() < 4 || engine_args.size() > 5)
            throw Exception(
                "Storage COS requires 4 to 5 arguments: url, access_key_id, secret_access_key, name of used format and [compression_method].", ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH);
        ...
        auto access_key_id = engine_args[1]->as<ASTLiteral &>().value.safeGet<String>();
        auto secret_access_key = engine_args[2]->as<ASTLiteral &>().value.safeGet<String>();
        ...
        return StorageCOS::create(cos_uri, access_key_id, secret_access_key, args.table_id, format_name, min_upload_part_size, args.columns, args.constraints, args.context);
    },
    {
...
NamesAndTypesList StorageCOS::getVirtuals() const
{
