...
std::optional<AlterCommand> AlterCommand::parse(const ASTAlterCommand * command_ast)
{
    ...
    else if (command_ast->type == ASTAlterCommand::ADD_INDEX)
    {
        ...
        if (command_ast->index)
            command.after_index_name = command_ast->index->as<ASTIdentifier &>().fullName();
    ...
    else if (command_ast->type == ASTAlterCommand::DROP_CONSTRAINT)
    {
        ...
        command.constraint_name = command_ast->constraint->as<ASTIdentifier &>().fullName();
    ...
    else if (command_ast->type == ASTAlterCommand::DROP_INDEX)
    {
        ...
        command.index_name = command_ast->index->as<ASTIdentifier &>().fullName();
        command.if_exists = command_ast->if_exists;
    ...
    else if (command_ast->type == ASTAlterCommand::RENAME_COLUMN)
    {
        ...
        command.column_name = command_ast->column->as<ASTIdentifier &>().fullName();
        command.rename_to = command_ast->rename_to->as<ASTIdentifier &>().fullName();
        command.if_exists = command_ast->if_exists;
        return command;
    }
    else
