    ...
    const StorageInMemoryMetadata & metadata, const ContextPtr & context) const
{
    ...
    for (const auto & column : metadata.columns.getAllPhysical())
    {
        ...
        if (isTrueMetadataOnlyConversion(column.type.get(), final_data_type.get()))
            return decision;

        decision.lazy_settings = getLazyMetadataConversionSettings(column.type.get(), final_data_type.get(), context);
...
void AlterCommands::validate(const StoragePtr & table, ContextPtr context) const
{
    ...
    for (size_t i = 0; i < size(); ++i)
    {
        ...
        if (command.type == AlterCommand::ADD_COLUMN)
        {
            ...
            for (auto & col : columnsAddedByAlter(all_columns, ColumnDescription(column_name, final_data_type),
