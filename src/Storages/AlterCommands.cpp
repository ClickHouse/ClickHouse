...
bool isMetadataOnlyConversion(const IDataType * from, const IDataType * to, const ContextPtr & context)
{
    ...
    while (true)
    {
        ...
        {
            /// Nullable(Tuple(...)) with subfield additions is NOT metadata-only.
            /// The null map is stored separately from the Tuple substreams, and
            /// `fillMissingColumns` cannot synthesize a matching nested column for
            /// the newly-added subfield — it produces size 0 while the null map has
            /// the correct row count, triggering a LOGICAL_ERROR assertion on read.
            /// Other inner metadata-only conversions (Enum widening, Date<->UInt16)
            /// don't change the substream set, so they remain safe to unwrap.
            const auto * inner_tuple_from = typeid_cast<const DataTypeTuple *>(nullable_from->getNestedType().get());
            const auto * inner_tuple_to = typeid_cast<const DataTypeTuple *>(nullable_to->getNestedType().get());
            if (inner_tuple_from && inner_tuple_to
                && inner_tuple_from->getElements().size() != inner_tuple_to->getElements().size())
                return false;
