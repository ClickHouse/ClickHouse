...
std::optional<MutationCommand> AlterCommand::tryConvertToMutationCommand(
    StorageInMemoryMetadata & metadata, ContextPtr context, bool share_nested_offsets, bool column_ids_active) const
{
...
MutationCommands AlterCommands::getMutationCommands(
    ...
    bool with_alters, bool share_nested_offsets, bool column_ids_active) const
{
    ...
    {
        if (auto mutation_cmd = alter_cmd.tryConvertToMutationCommand(metadata, context, share_nested_offsets, column_ids_active); mutation_cmd)
        {
