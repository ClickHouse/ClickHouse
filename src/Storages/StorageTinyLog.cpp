...
{
public:
    explicit TinyLogBlockOutputStream(StorageTinyLog & storage_, const StorageMetadataPtr & metadata_)
        : storage(storage_), metadata(metadata_), lock(storage_.rwlock)
    {
    }
    ...
    const unsigned /*num_streams*/)
{
    ...
    pipes.emplace_back(std::make_shared<TinyLogSource>(
        max_block_size, Nested::collect(metadata_version->getColumns().getAllPhysical().addTypes(column_names)), *this, context.getSettingsRef().max_read_buffer_size));
...
BlockOutputStreamPtr StorageTinyLog::write(
    const ASTPtr & /*query*/, const StorageMetadataPtr & metadata_version, const Context & /*context*/)
{
    return std::make_shared<TinyLogBlockOutputStream>(*this, metadata_version);
}
