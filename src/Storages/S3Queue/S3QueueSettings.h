...
#define S3QUEUE_RELATED_SETTINGS(M, ALIAS) \
    ...
    M(UInt32, s3queue_polling_backoff_ms, 0, "Polling backoff", 0) \
    M(UInt32, s3queue_tracked_files_limit, 1000, "Max set size for tracking processed files in unordered mode in ZooKeeper", 0) \
    M(UInt32, \
      ...
      0) \
    M(UInt32, s3queue_polling_size, 50, "Maximum files to fetch from S3 with SELECT", 0) \
    M(UInt32, s3queue_cleanup_interval_min_ms, 10000, "Polling backoff min for cleanup for `unordered` mode", 0) \
    M(UInt32, s3queue_cleanup_interval_max_ms, 30000, "Polling backoff max for cleanup for `unordered` mode", 0) \
