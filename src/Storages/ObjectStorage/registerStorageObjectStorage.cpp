...
- The committed snapshot is stored in Keeper together with a marker of the table generation it belongs to (the schema-0 creation timestamp). A stored watermark whose generation is unknown — state written by a server version before generation tracking existed, or state inherited from a Paimon table that was dropped and recreated at the same `paimon_keeper_path` — is discarded, and the next incremental read delivers the current snapshot in full once, then commits the watermark together with the generation marker. This one-time replay is deliberate (fail-close): re-reading a bounded batch is preferred over silently skipping data that may belong to a different table. In particular, the first incremental read after upgrading from a server version without generation tracking re-delivers the current snapshot once.
