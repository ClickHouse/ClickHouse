...
/// True if a `_delta_log/` with any entry exists at `path`. Matches any file (not just `*.json`), so a
/// checkpoint-only log left after log cleanup is still recognized as an existing table.
