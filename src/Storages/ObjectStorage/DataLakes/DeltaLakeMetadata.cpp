...
struct DeltaLakeMetadataImpl
{
        ...
        std::set<String> & result) const
    {
        ...
        auto buf = createReadBuffer(relative_path_with_metadata, object_storage, context, log);
    ...
    size_t readLastCheckpointIfExists() const
    {
        ...
        String json_str;
        auto read_settings = context->getReadSettings();
        RelativePathWithMetadata relative_path_with_metadata(last_checkpoint_file);
        auto buf = createReadBuffer(relative_path_with_metadata, object_storage, context, log);
        readJSONObjectPossiblyInvalid(json_str, *buf);
        ...
        DeltaLakePartitionColumns & file_partition_columns) const
    {
        ...
        auto read_settings = context->getReadSettings();
        RelativePathWithMetadata relative_path_with_metadata(checkpoint_path);
        auto buf = createReadBuffer(relative_path_with_metadata, object_storage, context, log);
