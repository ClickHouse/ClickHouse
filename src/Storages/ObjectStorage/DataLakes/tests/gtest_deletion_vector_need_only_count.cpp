...
{
    /// Large enough that a dense Filter would be expensive; const columns stay O(1) via
    /// cloneResized + rb_range_cardinality. No applied_filter: Iceberg does not chain another
    /// row-number consumer after a DV on the need_only_count path.
