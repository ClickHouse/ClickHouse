...
public:
    ...
    /// `current_table_identity` of 0 means the identity was not latched at create time: there is
    /// nothing to compare, so the watermark is used.  An absent `committed_table_identity` means
    /// the watermark predates identity tracking, so its generation is unknown and it is discarded
    /// once, reading the current table from its first snapshot.
