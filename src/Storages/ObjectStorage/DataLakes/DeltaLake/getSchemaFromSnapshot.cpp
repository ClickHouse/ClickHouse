...
private:
    struct Field;
    /// What kind of node owns the list being visited, which decides a child's writer-path component.
    /// `element`/`key`/`value` are the Parquet writer's own words; the kernel's child names may differ.
    ...
    struct Field
    {
        ...
        /// Delta `timestamp` and `timestamp_ntz` are both DateTime64(6), so `type` cannot say which,
        /// and only `timestamp_ntz` is a wall clock time.
...
private:
    ...
    static ffi::EngineSchemaVisitor createVisitor(SchemaVisitorData & data)
    {
        return ffi::EngineSchemaVisitor{
            ...
            /// The trailing `true` is is_timestamp_ntz.
