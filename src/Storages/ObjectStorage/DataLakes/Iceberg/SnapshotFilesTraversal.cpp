...
/// Walk the historical metadata versions listed in `metadata-log` (recursively, older versions list
    ...
    bool scan_metadata_log_history)
{
    ...
    {
        /// History must only extend `external_files` (the fail-close gate), never `reachable`:
        /// base-subtree files referenced solely by expired history stay eligible as orphans.
