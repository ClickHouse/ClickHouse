...
/// Whether the current user may read what `path` names, decided as it is decided for a read, but without
/// building the storage that would serve it. True for a path the table's own grant covers, and for one
/// no read could open at all, which names no target of its own to authorize.
/// For the metadata-only surfaces, which expose per-file manifest data without ever opening the file.
bool isPathReadAllowed(
    const std::string & table_location,
    const std::string & path,
    const ObjectStoragePtr & base_storage,
    const ContextPtr & context);
