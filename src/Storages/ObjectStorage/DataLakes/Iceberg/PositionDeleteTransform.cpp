...
void IcebergPositionDeleteTransform::initializeDeleteSources()
{
    ...
    for (const auto & position_deletes_object : iceberg_object_info->position_deletes_objects)
    {
        ...
        Block initial_header;
        {
            ...
            auto schema_reader = FormatFactory::instance().getSchemaReader(configuration->format, *read_buf_schema, context);
        ...
        CompressionMethod compression_method = chooseCompressionMethod(object_path, configuration->compression_method);
...
void IcebergBitmapPositionDeleteTransform::transform(Chunk & chunk)
{
    ...
    size_t base_row_num = chunk.getChunkInfos().get<ChunkInfoReadRowsBefore>()->read_rows_before;
