...
void IcebergPositionDeleteTransform::initializeDeleteSources()
{
    ...
    for (const auto & position_deletes_object : iceberg_object_info->info.position_deletes_objects)
    {
        ...
        {
            std::unique_ptr<ReadBuffer> read_buf_schema = createReadBuffer(object_info, object_storage, context, log, format);
        ...
        delete_read_buffers.push_back(createReadBuffer(object_info, object_storage, context, log, format));
