...
void IcebergPositionDeleteTransform::initializeDeleteSources()
{
    ...
    for (const auto & position_deletes_object : iceberg_object_info->parsed_data_file_info.position_deletes_objects)
    {
        ...
        auto relative_path_with_metadata = std::make_shared<RelativePathWithMetadata>(object_path, object_metadata);


        Block initial_header;
        {
            std::unique_ptr<ReadBuffer> read_buf_schema = createReadBuffer(*relative_path_with_metadata, object_storage, context, log);
        ...
        delete_read_buffers.push_back(createReadBuffer(*relative_path_with_metadata, object_storage, context, log));
