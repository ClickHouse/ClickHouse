    ...
    for (const auto & data_file_name : data_file_names)
    {
        ...
        if (data_file_statistics)
        {
            ...
            auto lower_statistics
                = filterWritableStatistics(data_file_statistics->getLowerBounds(), field_id_to_column_index, sample_block);
            if (!lower_statistics.empty())
                set_fields(lower_statistics, Iceberg::f_lower_bounds, dump_fields);
            auto upper_statistics
                = filterWritableStatistics(data_file_statistics->getUpperBounds(), field_id_to_column_index, sample_block);
    ...
    const std::vector<std::vector<std::pair<Field, DataTypePtr>>> & entry_partition_summaries)
{
    ...
    {
        const auto & manifest_entry_name = manifest_entry_names[entry_idx];
