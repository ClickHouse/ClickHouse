...
std::optional<size_t> IcebergMetadata::totalRows(ContextPtr local_context) const
{
    ...
    for (const auto & manifest_list_entry : actual_data_snapshot->manifest_list_entries)
    {
        ...
        /// Position delete record_count from the manifest includes ALL (file_path, pos) pairs
        /// in the delete file, which may span multiple data files. This makes it unreliable
        /// for a simple subtraction. Fall back to full scan if deletes exceed data count.
        if (position_deletes_count.value() > data_count.value())
            return {};
