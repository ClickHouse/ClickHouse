    ...
    const StorageID & table_id_)
{
    ...
    auto metadata_content = createEmptyMetadataFile(configuration_ptr->getPathForRead(), *columns, partition_by, configuration_ptr->getDataLakeSettings()[DataLakeStorageSetting::iceberg_format_version]);
    {
        auto filename = configuration_ptr->getPathForRead() + "metadata/v1.metadata.json";
    ...
    if (catalog)
    {
        auto catalog_filename = configuration_ptr->getTypeName() + "://" + configuration_ptr->getNamespace() + "/" + configuration_ptr->getPathForRead() + "metadata/v1.metadata.json";
