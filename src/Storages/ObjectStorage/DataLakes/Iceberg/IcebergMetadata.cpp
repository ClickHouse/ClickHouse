...
{
    auto manifest_list_file_and_snapshot_id = getRelevantManifestList(object);
    if (manifest_list_file_and_snapshot_id)
    {
        current_snapshot = getSnapshot(manifest_list_file_and_snapshot_id.value().first, manifest_list_file_and_snapshot_id.value().second);
    }
...
bool IcebergMetadata::update(const ContextPtr & local_context)
{
    ...
    if (manifest_list_file && (!current_snapshot.has_value() || (manifest_list_file.value().second != current_snapshot->getSnapshotId())))
    {
        current_snapshot = getSnapshot(manifest_list_file.value().first, manifest_list_file.value().second);
...
std::optional<std::pair<String, Int64>> IcebergMetadata::getRelevantManifestList(const Poco::JSON::Object::Ptr & metadata)
{
    auto configuration_ptr = configuration.lock();
    ...
    {
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Iceberg snapshot id and timestamp settings cannot be changed simultaneously");
    }
    if (timestamp_changed)
    {
        ...
        for (size_t i = 0; i < snapshots->size(); ++i)
        {
            ...
            if (snapshot_timestamp <= query_timestamp && snapshot_timestamp > closest_timestamp)
            {
                ...
                const auto path = snapshot->getValue<String>("manifest-list");
                // TODO: replace with getRelevantFile
...
{
    return IcebergSnapshot{getManifestList(filename), current_snapshot->getSnapshotId()};
}
...
Strings IcebergMetadata::getDataFilesImpl(const ActionsDAG * filter_dag) const
{
    ...
    Strings data_files;
    for (const auto & manifest_list_entry : current_snapshot->getManifestList())
    {
