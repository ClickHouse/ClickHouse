...
bool IcebergMetadata::update(const ContextPtr & local_context)
{
    ...
    if (manifest_list_file && (!current_snapshot.has_value() || (manifest_list_file.value() != current_snapshot->getManifestListName())))
    {
        current_snapshot = getSnapshot(manifest_list_file.value());
...
ManifestList IcebergMetadata::initializeManifestList(const String & filename) const
{
    ...
    for (size_t i = 0; i < manifest_path_col_str->size(); ++i)
    {
        ...
        auto creation_function = [this, current_filename, added_sequence_number](ManifestFileIterator & manifest_file_iterator)
        {
            auto manifest_file_content = this->initializeManifestFile(current_filename, added_sequence_number);
            auto [iterator, _inserted] = this->manifest_files_by_name.emplace(current_filename, std::move(manifest_file_content));
            manifest_file_iterator.setIterator(iterator);
            for (const auto & data_file_path : manifest_file_iterator->getFiles())
            {
                if (std::holds_alternative<DataFileEntry>(data_file_path.file))
                    this->manifest_file_by_data_file.emplace(std::get<DataFileEntry>(data_file_path.file).file_name, manifest_file_iterator);
            }
        };
        auto manifest_file_iterator = ManifestFileIterator{creation_function};
