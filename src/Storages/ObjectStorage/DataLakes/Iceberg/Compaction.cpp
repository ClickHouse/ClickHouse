    ...
    CompressionMethod compression_method)
{
    ...
    const auto [metadata_version, metadata_file_path, _, tied_metadata_paths] = getLatestOrExplicitMetadataFileAndVersion(
        object_storage,
    ...
    const StorageID & table_id)
{
    ...
    for (size_t attempt = 0; attempt < MAX_COMPACTION_RETRIES; ++attempt)
    {
        ...
        const auto [metadata_version, metadata_file_path, _, tied_metadata_paths] = getLatestMetadataFileAndVersionWithCatalog(
            object_storage_,
            catalog,
