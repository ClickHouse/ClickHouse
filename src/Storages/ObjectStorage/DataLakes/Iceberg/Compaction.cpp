    ...
    CompressionMethod compression_method)
{
    ...
    for (size_t i = 0; i < schemas->size(); ++i)
    {
        ...
        persistent_table_components.schema_processor->addIcebergTableSchema(schema_object);
        if (schema_object->getValue<Int32>(Iceberg::f_schema_id) == current_schema_id)
        {
            current_schema = schema_object->getArray(Iceberg::f_fields);
        }
    }

    /// Register snapshot→schema mappings so that ManifestFileIterator can resolve
    /// the write schema for each manifest entry's snapshot_id.
    if (initial_metadata_object->has(Iceberg::f_snapshots))
    {
        auto snapshots_array = initial_metadata_object->getArray(Iceberg::f_snapshots);
        for (size_t i = 0; i < snapshots_array->size(); ++i)
        {
            auto snapshot = snapshots_array->getObject(static_cast<UInt32>(i));
            auto snap_id = snapshot->getValue<Int64>(Iceberg::f_metadata_snapshot_id);
            auto snap_schema_id = snapshot->getValue<Int32>(Iceberg::f_schema_id);
