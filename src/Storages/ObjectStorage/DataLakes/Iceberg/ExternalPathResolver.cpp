...
/// With `read_allowed` given, the target is authorized but not resolved: the decision lands there and
/// the function returns as soon as it is made, without building a storage for a file nobody opens.
    ...
    bool * read_allowed)
{
    ...
    if (target_scheme_normalized == "file")
    {
        ...
        /// Both rejections gate opening the file, not the user's grant on it.
        if (read_allowed == nullptr)
        {
    ...
    /// A table definition is authorized against the URI it names (`ITableFunction::checkSourceAccess`),
    /// but a path outside the table location names a target of its own -- possibly on another source
    /// entirely -- which that grant never covered. Authorize it like the table function which would read
    /// it directly, so the manifest cannot turn a grant on one location into a read of another. Called
    /// at every exit that addresses such a target; a path that stays inside the table location is served
    /// by the grant on the table itself and is not checked again.
    /// The path is checked as the metadata spells it, since that is the spelling a grant for it is
    /// written with, exactly as a table function is checked against the URI it is written with. A
    /// `file://` URI and a bare absolute path name the same local file and a `File` grant is written as
    /// a filesystem path, so both spellings are checked as the path they decompose to.
    auto check_read_access = [&]
    {
        ...
        if (read_allowed)
            *read_allowed = context->getAccess()->isGrantedWithFilter(
...
std::optional<std::pair<DB::ObjectStoragePtr, std::string>> tryResolveObjectStorageForPath(
    ...
    const DB::ContextPtr & context)
{
    return tryResolveObjectStorageForPathImpl(table_location, path, base_storage, external_storages, context, /*read_allowed*/ nullptr);
}
    ...
    const DB::ContextPtr & context)
{
    ...
    tryResolveObjectStorageForPathImpl(table_location, path, base_storage, unused_storages, context, &read_allowed);
    return read_allowed;
}
