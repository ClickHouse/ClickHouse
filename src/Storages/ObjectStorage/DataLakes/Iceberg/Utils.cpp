...
static MetadataFileWithInfo getMetadataFileAndVersion(const std::string & path)
{
    ...
    String version_str;
    /// `vN.metadata.json` or `vN-<random-uuid>.metadata.json` (the latter is
    /// produced by OneLake / Microsoft Fabric).
    if (file_name.starts_with('v'))
    {
        auto sep_pos = file_name.find_first_of(".-");
        if (sep_pos == String::npos || sep_pos <= 1)
            throw Exception(
                ErrorCodes::BAD_ARGUMENTS,
                "Bad metadata file name: '{}'. Expected `vN.metadata.json` or `N-<uuid>.metadata.json` where N is a version number",
                file_name);
        version_str = String(file_name.begin() + 1, file_name.begin() + sep_pos);
    }
