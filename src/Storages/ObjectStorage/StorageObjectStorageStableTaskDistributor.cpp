...
ObjectInfoPtr StorageObjectStorageStableTaskDistributor::getPreQueuedFile(size_t number_of_current_replica)
{
    ...
    while (!files.empty())
    {
        ...
        auto file_identifier = send_over_whole_archive ? next_file->getPathOrPathToArchiveIfArchive() : getMetadataPathFromObjectInfo(next_file).value_or(next_file->getIdentifier());
...
ObjectInfoPtr StorageObjectStorageStableTaskDistributor::getAnyUnprocessedFile(size_t number_of_current_replica)
{
    ...
    if (!unprocessed_files.empty())
    {
        ...
        auto file_path = send_over_whole_archive ? next_file->getPathOrPathToArchiveIfArchive() : getMetadataPathFromObjectInfo(next_file).value_or(next_file->getPath());
        LOG_TRACE(
            log,
