...
std::unique_ptr<ReadBuffer> ReadBufferIterator::recreateLastReadBuffer()
{
    ...
    const auto zstd_window = static_cast<int>(context->getSettingsRef()[zstd_window_log_max]);
    ...
    while (true)
    {
        ...
        if (!query_settings.skip_empty_files || !read_buf->eof())
        {
            ...
            read_buf = wrapReadBufferWithCompressionMethod(
                std::move(read_buf), compression_method, static_cast<int>(getContext()->getSettingsRef()[zstd_window_log_max]));
