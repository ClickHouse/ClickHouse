    ...
    const DB::ContextPtr & context)
{
    ...
    if (target_decomposed.scheme.empty() && target_decomposed.key.starts_with('/'))
    {
        ...
        /// On a local base storage this is a real filesystem path (the spelling
        /// `write_full_path_in_iceberg_metadata = 0` writes), and the base storage is rooted at the table
        /// directory, so a path above it needs its own storage just like `file:///...`.
        /// A path inside the location is never opened as the metadata spells it: `path_resolver` re-roots
        /// it at the table's real directory, which the server already resolved against `user_files`.
    ...
    {
        /// A `file://` URI is always served from the local filesystem of whichever server resolves it,
        /// so a remote authority cannot be honoured: every replica would read its own file under that
        /// path instead of the host the metadata names. Reject it rather than return the wrong data.
        /// RFC 8089 treats an empty authority and `localhost` as the local machine, so both are fine;
        /// normalize `localhost` away to keep the authority empty for the cache key and the
        /// authority comparisons below.
        if (!target_decomposed.authority.empty())
        {
            if (Poco::toLower(target_decomposed.authority) != "localhost")
                throw DB::Exception(
    ...
    if (target_scheme_normalized == "s3" || target_scheme_normalized == "https" || target_scheme_normalized == "http")
    {
        ...
        return getOrCreateStorage(
            ...
            [&](const std::string & storage_name) -> ObjectStoragePtr
            {
                /// Start from the base storage's own S3 configuration whenever its identity applies to this
                /// target, so the secondary storage keeps the auth mechanism and the request settings the base
                /// was configured with: `no_sign_request`, `http_client = gcp_oauth`, the assume-role and
                /// request-token flows, custom headers, timeouts, retries, the proxy resolver, and so on.
                /// Synthesizing a configuration from the endpoint alone would break every table whose base
                /// backend needs any non-default setting as soon as one of its files lives in another bucket.
                /// Only the endpoint, the bucket and the credentials differ from the base.
                /// The clone is a snapshot taken on the cache miss: a later change of the base storage's
                /// settings does not reach an already-created secondary storage.
                ...
                /// Not a server disk: `getClient` applies the session's restriction and fails closed with
                /// `ACCESS_DENIED` instead of resolving the server's own credentials for this endpoint.
                /// Credentials propagated above are explicit, so the restriction does not reject them.
    ...
    /// For `file://` URIs the authority is always empty (enforced above), so using just `"file://"` as the
    ...
    if (target_scheme_normalized == "abfs" && base_storage->getType() == ObjectStorageType::Azure)
    {
        if (auto azure_storage = std::dynamic_pointer_cast<AzureObjectStorage>(base_storage))
        {
            ...
            /// (`processEndpoint` has no key for `sas_auth` / `additional_params`) nor an explicit
            /// workload identity, and `getAuthMethod` would silently fall back to a managed identity.
            return getOrCreateStorage(
                cache_key,
