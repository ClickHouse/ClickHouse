...
ColumnsDescription StorageSystemMutations::getColumnsDescription()
{
    return ColumnsDescription
    {
        ...
        { "author",                        std::make_shared<DataTypeString>(), "The user who initiated the mutation. Note: for `ALTER ... ON CLUSTER` queries it contains the user under which the query was executed on each host, which is the local default user unless the server setting `distributed_ddl_use_initial_user_and_roles` is enabled." },
