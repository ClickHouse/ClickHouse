...
void StorageSystemReplicasCollection::fillData(MutableColumns & res_columns, ContextPtr context, const ActionsDAG::Node *, std::vector<UInt8>) const
{
    ...
    for (const auto & [name, collection] : collections)
    {
        ...
        /// `__type__='replica'` tag injected by legacy `CREATE REPLICA` named-collection flow.
        if (collection->getOrDefault<String>("__type__", "") != "replica")
            continue;
        ...
        res_columns[11]->insert(collection->getCreateStatement(/*show_secrets=*/access_secrets));
    }
}
