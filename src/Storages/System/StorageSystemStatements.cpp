...
{
    for (const auto & [name, statement] : StatementFactory::instance().getAllStatements())
    {
        const auto & documentation = statement.documentation;
        ...
        res_columns[i++]->insert(statement.parent);
