...
{
    applyFiltersImpl(std::move(added_filter_nodes), false);
}

void ReadFromSystemTables::applyFiltersImpl(ActionDAGNodes added_filter_nodes, const bool need_detached_tables_)
{
    ...
    filtered_tables_column = getFilteredTables(predicate, filtered_databases_column, context, need_detached_tables_);
}
...
ReadFromSystemDetachedTables::ReadFromSystemDetachedTables(
    ...
    : ReadFromSystemTables(
        column_names_, query_info_, storage_snapshot_, context_, std::move(sample_block), std::move(columns_mask_), max_block_size_)
{
}

void ReadFromSystemDetachedTables::applyFilters(ActionDAGNodes added_filter_nodes)
{
    applyFiltersImpl(std::move(added_filter_nodes), true);
}
