...
String stripMdxAutogeneratedMarkers(String document)
{
    constexpr std::string_view marker_prefix = "{/*AUTOGENERATED_";
    constexpr std::string_view marker_suffix = "*/}";

    size_t offset = 0;
    while ((offset = document.find(marker_prefix, offset)) != String::npos)
    {
        const size_t marker_end = document.find(marker_suffix, offset + marker_prefix.size());
        if (marker_end == String::npos)
            break;
        document.erase(offset, marker_end + marker_suffix.size() - offset);
    }
    ...
    ContextPtr context)
{
    ...
    appendSystemTableSection(
        ...
        "Description",
        "description",
        documentation ? stripMdxAutogeneratedMarkers(documentation->description) : comment);
    String columns_section = renderSystemTableColumns(table_name, columns);
    if (documentation)
    {
        const String columns_notes = stripMdxAutogeneratedMarkers(documentation->columns_notes);
    ...
    if (documentation)
    {
        appendSystemTableSection(result, "Examples", "examples", stripMdxAutogeneratedMarkers(documentation->examples));
        const String additional_sections = stripMdxAutogeneratedMarkers(documentation->additional_sections);
        if (!additional_sections.empty())
            result += "\n\n" + additional_sections;
        appendSystemTableSection(result, "See also", "see-also", stripMdxAutogeneratedMarkers(documentation->see_also));
    }
...
void StorageSystemDocumentation::fillData(MutableColumns & res_columns, ContextPtr context, const ActionsDAG::Node *, std::vector<UInt8>) const
{
    ...
    /// Start with attached tables so optional/private tables without structured documentation retain their concise
    /// comment-and-columns fallback. Registered documentation is overlaid when available.
    const auto system_database = DatabaseCatalog::instance().tryGetDatabase(DatabaseCatalog::SYSTEM_DATABASE);
    std::unordered_set<String> attached_table_names;
    if (system_database)
    {
        for (auto iterator = system_database->getTablesIterator(context); iterator->isValid(); iterator->next())
        {
            ...
            if (const auto & table = iterator->table())
            {
                ...
                if (metadata_snapshot)
                {
                    ...
                    attached_table_names.emplace(table_name);
                }
            }
        }
    }

    /// Then add registered tables which are not attached in this environment. Their explicit column provider keeps
    /// gated tables and system logs available to `clickhouse-local` documentation generation.
    for (const auto & [table_name, documentation] : getSystemTableDocumentationRegistry())
    {
        if (attached_table_names.contains(table_name))
            continue;
