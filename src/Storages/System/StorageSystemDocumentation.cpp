...
/// database engines, data types, formats, table functions) keep the entire page in `description`, and such
/// a page contains its own Markdown section headers, which per the documentation conventions always carry
/// an explicit `{#anchor}`. The structured metadata fields (`syntax`, `examples`, `related`, ...) then only
/// restate parts of the page in a machine-readable form, so appending sections composed from them would
/// duplicate material the page body already covers. The anchor requirement distinguishes a page section
