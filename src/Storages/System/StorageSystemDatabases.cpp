...
void StorageSystemDatabases::fillData(MutableColumns & res_columns, ContextPtr context, const ActionsDAG::Node * predicate, std::vector<UInt8> columns_mask) const
{
    ...
    /// External databases are always shown in system.databases regardless of show_external_databases_in_system_tables.
    ...
    const auto databases = DatabaseCatalog::instance().getDatabases(GetDatabasesOptions{.with_external_databases = true});
