...
ColumnsDescription StorageSystemViewRefreshes::getColumnsDescription()
{
    return ColumnsDescription{
        ...
        {"last_success_time", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeDateTime>()),
            "Time when the latest successful refresh started. NULL if no successful refresh is known: none has happened since the table was created, or the view did not restore its state across a server restart."},
        ...
        {"last_refresh_time", std::make_shared<DataTypeNullable>(std::make_shared<DataTypeDateTime>()),
            "Time when the latest refresh attempt finished (if known) or started (if unknown or still running). NULL if no refresh attempt is known: none has happened since the table was created, or the view did not restore its state across a server restart."},
