...
void fillDataWithDatabasesTablesColumns(MutableColumns & res_columns, const ContextPtr & context)
{
    ...
    const auto & databases = DatabaseCatalog::instance().getDatabases(GetDatabasesOptions{.with_external_databases = settings[Setting::show_external_databases_in_system_tables]});
