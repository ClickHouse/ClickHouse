    ...
    for (size_t part_number = 0; part_number < all_parts.size(); ++part_number)
    {
        ...
        for (const auto & column : part->getColumns())
        {
            ...
            else
            {
                ...
                {
                    auto substream = ISerialization::getFileNameForStream(column.name, subpath, part->getStreamFileNameSettings());
                    auto filename = IMergeTreeDataPart::getStreamNameForColumn(column.name, subpath, ".bin", part->checksums, part->getStreamFileNameSettings());
            ...
            IDataType::forEachSubcolumn([&](const auto & subpath, const auto & name, const auto & data)
            {
                ...
                auto stream_name = IMergeTreeDataPart::getStreamNameForColumn(subcolumn, subpath, ".bin", part->checksums, part->getStreamFileNameSettings());
                if (stream_name)
                {
