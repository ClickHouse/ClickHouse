...
StorageSystemPartsColumns::StorageSystemPartsColumns(const StorageID & table_id_)
    ...
    ColumnsDescription{
        ...
        {"compression_codec",                          std::make_shared<DataTypeString>(), "Compression codec detected from the column data stream in the data part."},
    ...
    ContextPtr context, MutableColumns & columns, std::vector<UInt8> & columns_mask, const StoragesInfo & info, bool has_state_column)
{
    ...
    for (size_t part_number = 0; part_number < all_parts.size(); ++part_number)
    {
        ...
        for (const auto & column : part->getColumns())
        {
            ...
            if (columns_mask[src_index++])
                columns[res_index++]->insert(part->getColumnCompressionCodec(column)->getCodecDesc()->formatForLogging());
            if (columns_mask[src_index++])
