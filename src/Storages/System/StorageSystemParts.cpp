...
* Definitions:
  ...
  - `data_version` - optional value, incremented when a part is mutated (again, mutated data is always only written to a new part, since parts are immutable)
        ...
        {"part_storage_type",                           std::make_shared<DataTypeString>(),    "The type of `DataPartStorage`. Possible values: `Packed` - all files are stored in a single file, `Full` - each file is stored separately."},
