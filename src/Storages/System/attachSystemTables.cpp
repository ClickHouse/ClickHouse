...
void attachSystemTablesServerExceptOne(ContextPtr context, IDatabase & system_database, bool has_zookeeper, [[maybe_unused]] bool has_keeper_server)
{
    ...
    attachNoDescription<StorageSystemTableSettings>(context, system_database, "table_settings", "Settings of every table as they are actually in effect, which need not be the values its CREATE query states. Describes tables; for the settings an engine supports, see system.engine_settings. An engine that keeps no settings of its own reports only what the table's `SETTINGS` clause states - plain object storage (`S3`, `GCS`, `AzureBlobStorage`, `HDFS`), `File`, `URL` and the `Log` family among them. A setting known only from that clause, because its engine has no settings struct at all, has an empty `default`, `type` and `description` and reports `tier` as `Production`.");
