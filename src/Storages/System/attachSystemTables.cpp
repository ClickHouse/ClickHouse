...
void attachSystemTablesServerExceptOne(ContextPtr context, IDatabase & system_database, bool has_zookeeper, [[maybe_unused]] bool has_keeper_server)
{
    ...
    attach<StorageSystemDimensionalMetrics>(context, system_database, "dimensional_metrics", "Contains dimensional metrics, which have multiple dimensions (labels) to provide more granular information. For example, counting failed merges by their error code. Most metrics are updated in place and are always up to date; metrics sampled by the asynchronous metrics task (such as `filesystem_cache_size_bytes` and `filesystem_cache_elements`) can lag by one `asynchronous_metrics_update_period_s` interval.");
