...
void attachSystemTablesServer(ContextPtr context, IDatabase & system_database, bool has_zookeeper)
{
    ...
    attach<StorageSystemClusters>(
        ...
        system_database,
        "clusters",
        "Contains information about clusters available to the server: from `remote_servers` in the configuration, from SQL `CREATE CLUSTER` (once materialized into the server cluster map), from cluster discovery, and clusters generated by a Replicated database.");
    attach<StorageSystemShards>(
        context,
        system_database,
        "shards",
    ...
    attachNoDescription<StorageSystemReplicasCollection>(context, system_database, "replicas_collection", "Contains named collections created via `CREATE REPLICA` expanded into replica-oriented columns.");
