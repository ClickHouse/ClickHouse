...
void attachSystemTablesServerExceptOne(ContextPtr context, IDatabase & system_database, bool has_zookeeper, [[maybe_unused]] bool has_keeper_server)
{
    ...
    attach<StorageSystemEngineSettings>(context, system_database, "engine_settings", "Settings of every table engine that has settings of its own, with the value the engine uses on this server and its default. Describes engines rather than tables: for the settings in effect for one table, see the table's `SHOW CREATE TABLE`.");
    ...
    attachNoDescription<StorageSystemTableSettings>(context, system_database, "table_settings", "Settings of every table as they are actually in effect, which need not be the values its CREATE query states. Describes tables; for the settings an engine supports, see system.engine_settings.");
