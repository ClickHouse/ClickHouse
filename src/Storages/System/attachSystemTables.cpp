...
Retired event that counted log entries read from the Keeper commit-log cache.

### KeeperLogsPrefetchedEntries {#keeperlogsprefetchedentries}

Retired event that counted Keeper log entries prefetched from the changelog file.

### NaiveBayesClassifierModelsAllocatedBytes {#naivebayesclassifiermodelsallocatedbytes}

Retired event that counted bytes allocated for Naive Bayes Classifier models.

### NaiveBayesClassifierModelsLoaded {#naivebayesclassifiermodelsloaded}

Retired event that counted loaded Naive Bayes Classifier models.

### ReaderExecutorRequestedBytes {#readerexecutorrequestedbytes}

Retired event that counted useful bytes delivered by `ReaderExecutor` read requests.
...
The values are the ones this server uses, which need not be the compiled-in defaults: the `<merge_tree>` and `<replicated_merge_tree>` sections of the server configuration sets them, and the `compatibility` setting rolls them back to an older release's. The `source` column names which of those a value came from, and `changed` is `source != 'default'`.
