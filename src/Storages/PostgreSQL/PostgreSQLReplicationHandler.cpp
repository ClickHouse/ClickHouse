...
PostgreSQLReplicationHandler::PostgreSQLReplicationHandler(
    ...
    , publication_name(getPublicationName(postgres_database_, postgres_schema, postgres_table_, clickhouse_uuid_, replication_settings))
    ...
    , legacy_publication_name(getPublicationName(postgres_database_, /* postgres_schema */ "", postgres_table_, clickhouse_uuid_, replication_settings))
...
void PostgreSQLReplicationHandler::adoptLegacyReplicationIdentityIfNeeded(pqxx::nontransaction & tx)
{
    ...
    /// The generated names differ from the legacy ones only for a non-default schema, and only when a
    /// unique replication consumer identifier is not used (it is schema-independent, so both generated
    /// names coincide with the legacy ones; likewise a user-managed slot keeps its user-provided name).
    /// This also makes the adoption idempotent: once adopted, the names compare equal.
