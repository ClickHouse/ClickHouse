...
std::optional<MergeTreeMutationStatus> StorageMergeTree::getIncompleteMutationsStatus(Int64 mutation_version, std::set<String> * mutation_ids) const
{
    ...
    for (const auto & data_part : data_parts)
    {
        ...
        if (data_version < mutation_version && (!mutation_entry.partition_id || *mutation_entry.partition_id == data_part->info.partition_id))
        {
...
std::vector<MergeTreeMutationStatus> StorageMergeTree::getMutationsStatus() const
{
    ...
    for (const auto & kv : current_mutations_by_version)
    {
        ...
        {
            if (!entry.partition_id || *entry.partition_id == part_versions[i].partition_id)
    ...
    std::unique_lock<std::mutex> & /*currently_processing_in_background_mutex_lock*/)
{
    ...
    for (const auto & part : getDataPartsVectorForInternalUsage())
    {
        ...
        {
            if (!mutations_begin_it->second.partition_id || *mutations_begin_it->second.partition_id == part->info.partition_id)
                break;
