...
{
    {
        ...
        if (detach)
            LOG_INFO_FORMATTED(log, "Detached {} parts inside partition ID {}.", parts_to_remove.size(), partition_id);
        else
            LOG_INFO_FORMATTED(log, "Removed {} parts inside partition ID {}.", parts_to_remove.size(), partition_id);
    }
