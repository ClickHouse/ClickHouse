    ...
    Int64 mutation_version, std::unique_lock<std::mutex> & /*lock*/, std::set<String> * mutation_ids, bool from_another_mutation) const
{
    ...
    result.is_done = true;
    result.finish_time = mutation_entry.finish_time;
    return result;
}
    ...
    for (const auto & kv : current_mutations_by_version)
    {
        ...
        for (const MutationCommand & command : *entry.commands)
        {
            ...
            result.push_back(MergeTreeMutationStatus
            {
                ...
                entry.create_time,
                /* finish_time = */is_done ? (entry.finish_time != 0 ? entry.finish_time : time(nullptr)) : 0,
                block_numbers_map,
...
size_t StorageMergeTree::clearOldMutations(bool truncate)
{
    ...
    std::vector<MergeTreeMutationEntry> mutations_to_delete;
    {
        ...
        for (auto it = begin_it; it != end_it; ++it)
        {
            ...
            {
                entry.is_done = true;
                if (entry.finish_time == 0)
                {
                    entry.finish_time = time(nullptr);
                    entry.writeFinishTime(entry.finish_time);
                }
