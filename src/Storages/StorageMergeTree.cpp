        ...
        if (getPartMutationScope(*data_part, mutation_version, mutation_entry.tid) != PartMutationScope::Outside)
        {
            ...
            else if (txn && !from_another_mutation)
            {
                ...
                LOG_DEBUG(
                    ...
                    data_part->version->getRemovalTIDLockHash(),
...
size_t StorageMergeTree::markFinishedMutations(UInt64 first_just_completed_version)
{
    ...
    for (auto & [mutation_version, entry] : current_mutations_by_version)
    {
        ...
        if (!entry.tid.isNonTransactional() && !entry.csn && !TransactionManager::getCSN(entry.tid))
            erasable_prefix = false;
        if (erasable_prefix)
            ++done_count;
    }
...
size_t StorageMergeTree::clearOldMutations(bool truncate)
{
    ...
    std::vector<MergeTreeMutationEntry> mutations_to_delete;
    {
        ...
        for (size_t i = 0; i < to_delete_count; ++i)
        {
            ...
            /// Same committedness rule as markFinishedMutations: the log may have already dropped a committed TID.
            if (!tid.isNonTransactional() && !it->second.csn && !TransactionManager::getCSN(tid))
