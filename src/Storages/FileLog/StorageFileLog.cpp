...
The `FileLog` engine can silently lose already-consumed rows if the OS page cache is discarded before the inserted data is written to disk. On restart the engine resumes each log file from the offset recorded in its metadata directory, so rows whose recorded offset survived the power loss while the inserted part did not are never re-read: they are lost with no error and `count()` is simply smaller. A plain process kill does not expose this, because the kernel keeps the page cache and eventually writes it back. A loss of the page cache does expose it; examples are a device-level power loss and an unclean host or kernel reset.

Unlike the message-broker engines, `FileLog` cannot be protected against this by making the target durable first. The offset is recorded from inside the reading pipeline, before the insert it belongs to has finished, so setting `fsync_after_insert = 1` on the target `MergeTree` tables does not establish the inserted part as durable before the offset advances. The metadata files holding the offsets are themselves written without an fsync of the file or of its directory, so they carry no durability guarantee of their own either. Treat `FileLog` consumption as best-effort tailing of local files: where no rows may be lost, keep the source log files until the consumed data has been verified in the target, so that consumption can be repeated. Dropping and recreating the table discards the recorded offsets and re-reads the files from the beginning.
)DOCS_MD",
