...
struct TableZnodeInfo
{
    ...
    /// the template cannot be matched at all (it expands {uuid} more than once, say): such a table would keep
    /// its parent znode forever after `DROP TABLE`. Call it before the literal path is written into metadata.
