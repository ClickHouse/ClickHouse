    ...
    size_t subquery_depth)
{
    ...
    if (mutable_context->hasQueryContext())
    {
        if (auto cancel_callback = mutable_context->getQueryContext()->getInteractiveCancelCallback())
            executor.setCancelCallback(std::move(cancel_callback), std::max(UInt64(100), mutable_context->getSettingsRef()[Setting::interactive_delay] / 1000), CancelCallbackMode::PartialResult);
    }
    executor.execute();
