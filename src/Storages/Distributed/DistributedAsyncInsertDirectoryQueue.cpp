...
DistributedAsyncInsertDirectoryQueue::DistributedAsyncInsertDirectoryQueue(
    ...
    , should_batch_inserts(storage.getDistributedSettingsRef().async_insert_batch)
    , split_batch_on_failure(storage.getDistributedSettingsRef().async_insert_split_batch_on_failure)
    ...
    , default_sleep_time(storage.getDistributedSettingsRef().async_insert_sleep_time_ms.totalMilliseconds())
    , sleep_time(default_sleep_time)
    , max_sleep_time(storage.getDistributedSettingsRef().async_insert_max_sleep_time_ms.totalMilliseconds())
