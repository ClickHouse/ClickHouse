...
#define LIST_OF_DISTRIBUTED_SETTINGS(M, ALIAS) \
    ...
    /* ddl rewrite to on cluster settings */ \
    M(Bool, enable_ddl_optimize_rewrite, false, "OPTIMIZE will be rewrited to optimize on corresponding cluster.", 0) \
    M(Bool, enable_ddl_alter_rewrite, false, "ALTER will be rewrited to optimize on corresponding cluster. Deleting and drop partition are supported currently.", 0) \
