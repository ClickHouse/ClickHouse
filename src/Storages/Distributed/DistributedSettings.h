...
#define LIST_OF_DISTRIBUTED_SETTINGS(M, ALIAS) \
    ...
    M(UInt64, async_insert_batch, 0, "Default - distributed_async_insert_batch", 0) ALIAS(monitor_batch_inserts) \
    M(UInt64, async_insert_split_batch_on_failure, 0, "Default - distributed_async_insert_split_batch_on_failure", 0) ALIAS(monitor_split_batch_on_failure) \
    M(Milliseconds, async_insert_sleep_time_ms, 0, "Default - distributed_async_insert_sleep_time_ms", 0) ALIAS(monitor_sleep_time_ms) \
    M(Milliseconds, async_insert_max_sleep_time_ms, 0, "Default - distributed_async_insert_max_sleep_time_ms", 0) ALIAS(monitor_max_sleep_time_ms) \
