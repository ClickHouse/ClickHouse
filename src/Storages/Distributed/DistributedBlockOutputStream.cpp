...
DistributedBlockOutputStream::DistributedBlockOutputStream(
    const Context & context_, StorageDistributed & storage_, const StorageMetadataPtr & metadata_, const ASTPtr & query_ast_, const ClusterPtr & cluster_,
        bool insert_sync_, UInt64 insert_timeout_)
    : context(context_), storage(storage_), metadata(metadata_), query_ast(query_ast_), query_string(queryToString(query_ast_)),
...
void DistributedBlockOutputStream::write(const Block & block)
{
    ...
    for (const auto & col : metadata->getColumns().getMaterialized())
    {
