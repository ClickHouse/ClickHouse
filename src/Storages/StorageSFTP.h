...
class StorageSFTP final : public IStorage
{
public:
    ...
    /// From user's file
    StorageSFTP(const std::string & table_path_, const std::string & user_files_path, CommonArguments args);

    /// From table in database
    StorageSFTP(const std::string & relative_table_dir_path, CommonArguments args);

    explicit StorageSFTP(CommonArguments args);

    std::string getName() const override { return "SFTP"; }
...
protected:
    friend class StorageSFTPSource;
    friend class StorageSFTPSink;
