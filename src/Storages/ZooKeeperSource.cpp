...
NamesAndTypesList getNamesAndTypes()
{
    return {
...
static bool extractPathImpl(const IAST & elem, Paths & res, ContextPtr context, bool allow_unrestricted)
{
    ...
    else if (allow_unrestricted && function->name == "like")
    {
        ...
        {
            char char_1 = pattern[i];
            // Handle escaping of metasymbols
            if (char_1 == '\\' && i + 1 < pattern.size())
            {
                char char_2 = pattern[i + 1];
                if (char_2 == '_' || char_2 == '%')
                {
                    prefix.append(1, char_2);
                    i++; // to skip two bytes
                    continue;
                }
            }
...
Chunk ZooKeeperSource::generate()
{
    if (count == 0) {
        count = 1;
        ...
        MutableColumns res_columns = description.sample_block.cloneEmptyColumns();
        ...
        while (!paths.empty())
        {
            ...
            connection->tryGetChildren(path_corrected, nodes);
            ...
            for (const String & node : nodes)
                futures.push_back(connection->asyncTryGet(path_part + '/' + node));
        ...
        size_t num_row  = res_columns.front()->size();
        return Chunk(std::move(res_columns), num_row);
    } else {
        return {};
    }
};
