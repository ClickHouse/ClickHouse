    ...
    , clean_interval_usec(context_->getSettingsRef().window_view_clean_interval.totalMicroseconds())
{
    ...
    auto target_table_engine = query.getTargetTableEngine();

    if (has_inner_target_table && !target_table_engine)
    ...
    if (auto inner_storage = query.getTargetTableEngine(ViewTarget::Kind::Inner))
    ...
    if (mode < LoadingStrictnessLevel::ATTACH)
    {
        ...
        if (has_inner_target_table)
        {
            ...
            target_create_query->set(target_create_query->storage, target_table_engine);
