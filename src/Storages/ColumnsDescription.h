...
/// admits `column_name`: directly, or as a subcolumn riding its origin column - a read of
/// `json.a` is delegated exactly like a read of `json`, and subcolumn sets (JSON paths) are
/// open-ended, so the contract can only ever enumerate origins.
