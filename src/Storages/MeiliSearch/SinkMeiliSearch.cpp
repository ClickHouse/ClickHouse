...
String getStringRepresentation(const ColumnWithTypeAndName& col, size_t row) {
    ...
    if (elem.getType() == Field::Types::Int64) {
        ...
        return quotify(elem.get<String>());
    }
    return "";
}

String SinkMeiliSearch::getOneElement(const Block& block, int ind) {
    String ans = "{";
    int id = 0;
    for (const auto& col : block) {
        ans += quotify(sample_block.getByPosition(id++).name) + ":" + getStringRepresentation(col, ind) + ",";
    }
    ans.back() = '}';
    return ans;
}

void SinkMeiliSearch::writeBlockData(const Block & block) {
