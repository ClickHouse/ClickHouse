...
#define LIST_OF_MERGE_TREE_SETTINGS(M, ALIAS) \
    ...
    M(Bool, fetch_merged_part_within_region_only, false, "If true, always fetching merged parts from same region only, unless the local merge result in inconsistent part and need to fetch from somewhere to bring all replicas to a consistent state", 0) \
    \
