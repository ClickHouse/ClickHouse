    ...
    MergeTreeIndexWriterStream * positions_stream)
{
    ...
    else
    {
        ...
        /// Do not flush if we can write raw postings.
...
void TextIndexSerialization::serializeHeader(const TextIndexHeader & header, WriteBuffer & ostr)
{
    ...
    if (header.codec_type != IPostingListCodec::Type::None && header.version < MergeTreeTextIndexSerializationVersion::V1_WithCodec)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Text index version 'v0_initial' does not support a posting list codec");

    if (header.has_positions && header.version < MergeTreeTextIndexSerializationVersion::V2_WithPositions)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Text index version {} does not support positions", static_cast<UInt64>(header.version));

    if (header.json_path_values_configuration && header.version < MergeTreeTextIndexSerializationVersion::V3_WithTokenizerConfig)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Text index version {} does not support tokenizer configuration", static_cast<UInt64>(header.version));

    writeVarUInt(static_cast<UInt64>(header.version), ostr);
    ...
    {
        writeVarUInt(static_cast<UInt64>(header.json_path_values_configuration.has_value()), ostr);
        if (header.json_path_values_configuration)
        {
            const auto & configuration = *header.json_path_values_configuration;
...
void MergeTreeIndexGranuleTextWritable::serializeBinaryWithMultipleStreams(MergeTreeIndexOutputStreams & streams) const
{
    ...
    TextIndexHeader header
    {
        ...
        .json_path_values_configuration = params.json_path_values_configuration,
...
{
    addTokenForRow(token, static_cast<UInt32>(current_row), token_position, context);
}

void MergeTreeIndexTextGranuleBuilder::addTokenForRow(
    std::string_view token, UInt32 row, UInt32 token_position, const PostingListBuildContext & context)
{
...
{
public:
    JSONPathValuesTextIndexConsumer(
        MergeTreeIndexTextGranuleBuilder & granule_builder_, const PostingListBuildContext & context_)
    ...
    void addToken(std::string_view token) { granule_builder.addTokenForRow(token, static_cast<UInt32>(current_row), 0, context); }
...
void MergeTreeIndexAggregatorText::update(const Block & block, size_t * pos, size_t limit)
{
    ...
    if (tokenizer->getType() == ITokenizer::Type::JSONPathValues)
    {
        ...
        JSONPathValuesTextIndexConsumer consumer(granule_builder, context);
...
void textIndexValidator(const IndexDescription & index, bool /*attach*/, const MergeTreeSettings & settings)
{
    ...
    else if (tokenizer->getType() == ITokenizer::Type::KeyValuePairs)
    {
