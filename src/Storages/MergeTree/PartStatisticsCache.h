...
/** Cache of deserialized column statistics of data parts of MergeTree tables.
  * Statistics of a part are immutable, so entries are keyed by the table UUID plus the checksum
  * of the part's contents and invalidated when the part is removed.
  */
...
public:
    ...
    /// Calculate key from the table UUID and the total checksum of the part's files.
    static UInt128 hash(const UUID & table_uuid, UInt128 content_checksum);
