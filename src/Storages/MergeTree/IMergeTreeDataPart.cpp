...
void IMergeTreeDataPart::setColumns(const NamesAndTypesList & new_columns, const SerializationInfoByName & new_infos, int32_t new_metadata_version)
{
    ...
    /// `with_collected_nested` is null when `Nested::collect` produced the same list as `original`
    /// (or when `share_nested_offsets` is disabled); fall back to `original` in that case.
