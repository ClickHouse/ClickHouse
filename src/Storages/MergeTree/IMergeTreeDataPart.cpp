...
void IMergeTreeDataPart::loadProjections(bool require_columns_checksums, bool check_consistency)
{
    ...
    {
        String path = getFullRelativePath() + projection.name;
        if (volume->getDisk()->exists(path))
        {
            auto part = storage.createPart(projection.name, {"all", 0, 0, 0}, volume, projection.name, this);
...
void IMergeTreeDataPart::remove(bool keep_s3) const
{
    ...
    #    pragma GCC diagnostic ignored "-Wunused-variable"
            ...
            {
                if (projection_parts.find(file) == projection_parts.end())
