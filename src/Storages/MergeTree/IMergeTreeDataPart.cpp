...
MergeTreeDataPartBuilder IMergeTreeDataPart::getSyntheticProjectionPartBuilder(
    const String & projection_name, ProjectionDescriptionRawPtr projection)
{
    /// Same arena rationale as getProjectionPartBuilder. The storage is only resolved, never created,
    /// and `PartDirIntent::Synthetic` keeps the part from reading or writing its directory.
    MutableDataPartStoragePtr projection_storage;
    {
        ScopedJemallocThreadArena mergetree_arena_scope(JemallocMergeTreeArena::getArenaIndex());
        projection_storage = getDataPartStorage().getProjectionStorage(
            IDataPartStorage::Projection::dirName(projection_name, /* is_temp */ true), /* use_parent_transaction */ false);
    }
    MergeTreeDataPartBuilder builder(storage, projection_name, projection_storage, getReadSettings(), PartDirIntent::Synthetic);
    return builder.withPartInfo(MergeListElement::FAKE_RESULT_PART_FOR_PROJECTION).withParentPart(this).withProjection(projection);
}
