...
void IMergeTreeDataPart::loadChecksums(bool require)
{
    if (auto buf = readFileIfExists("checksums.txt"))
    {
        if (checksums.read(*buf))
        {
            ...
            /// This happens when a replica fetches a part after its projection
            /// was dropped: the sender copies checksums.txt verbatim but skips
            /// the .proj directory.  Strip here so the in-memory checksums are
            /// always consistent with what is actually on disk.
            ...
            if (erased > 0)
                LOG_WARNING(storage.log,
                    "Part {} has {} .proj checksum entr{} with no corresponding "
                    "directory on disk (projection was likely dropped while the part "
                    "was detached or fetched). Stripping from in-memory checksums.",
