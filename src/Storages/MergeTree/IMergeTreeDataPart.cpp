...
bool IMergeTreeDataPart::isSkipIndexInPackedArchive(const IMergeTreeIndex & skip_index) const
{
    ...
    /// Probe the format present in the part (`getDeserializedFormat`), not the writer's current
    /// `getSubstreams()`, so a legacy member inside `skp_idx.packed` on an upgraded part is found.
    for (const auto & substream : skip_index.getDeserializedFormat(checksums, file_name, &getDataPartStorage()).substreams)
