...
struct MergeTreeReaderSettings
{
    ...
    /// served by TopK dynamic filtering): the query condition cache key under which the granules that PREWHERE empties are
    ...
    /// different rows. See `ReadFromMergeTree::getQueryConditionCacheConditionHash`. For any other
    /// read (including a TopK read that only uses a skip index) this is empty and PREWHERE-emptied granules are recorded under the PREWHERE predicate's
    /// own hash instead.
