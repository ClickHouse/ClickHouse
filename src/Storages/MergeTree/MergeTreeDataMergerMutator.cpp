    ...
    const String & prefix)
{
    ...
    for (const auto & projection : metadata_snapshot->getProjections())
    {
        ...
        projection_future_part.path = future_part.path + "/" + projection.name + "/";
    ...
    MergeTreeData::DataPartPtr source_part, const MutationCommands & commands_for_removes, const String & mrk_extension)
{
    ...
    for (const auto & command : commands_for_removes)
    {
        ...
        {
            if (source_part->checksums.has(command.column_name))
                rename_vector.emplace_back(command.column_name, "");
        }
    ...
    const std::set<MergeTreeProjectionPtr> & projections_to_recalc)
{
    ...
    {
        files_to_skip.insert(projection->getName());
    }
    ...
    const MergeTreeData::DataPartPtr & source_part)
{
    ...
    for (const auto & projection : metadata_snapshot->getProjections())
    {
        ...
        if (!source_part->checksums.has(projection.name) && materialized_projections.count(projection.name))
        {
        ...
        for (;;)
        {
            ...
            else if (selected_parts.size() == 1)
            {
                if (next_level_parts.empty())
                {
                    ...
                    selected_parts[0]->renameTo(projection.name, true);
