...
void RPNBuilder<RPNElement>::traverseTree(const RPNBuilderTreeNode & node)
{
    ...
    LOG_DEBUG(&Poco::Logger::get("RPNBuilder"), "Is function node: {}", node.isFunction());
    if (node.isFunction())
    {
        ...
        {
            LOG_DEBUG(&Poco::Logger::get("RPNBuilder"), "Is logical operator node");
        ...
        {
            LOG_DEBUG(&Poco::Logger::get("RPNBuilder"), "Is not logical operator node");
        }
    }
