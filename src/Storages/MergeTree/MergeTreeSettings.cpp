...
void MergeTreeSettingsImpl::checkCompressionCodecSettings() const
{
    ...
    /// (e.g. `PCO`) would only fail later, at the first write — reject it here instead. Building each
    /// codec with a null type (via `get`) additionally rejects a lossy codec such as `SZ3`, which needs a
    /// floating-point column to validate against; this reports the misconfiguration when the table
    /// metadata is created or altered, instead of silently corrupting data on the first part write.
