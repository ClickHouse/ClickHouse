...
- with_key_columns — Store each distinct key of a `Map(String, V)` in independent streams (`m.keys` for the sorted key set, `m.values.<key>` for the values and `m.exists.<key>` for the presence bitmap). Requires `String` keys. Improves reading individual keys from the Map. Must be set explicitly: `map_serialization_version` and `map_serialization_version_for_zero_level_parts` must both be `with_key_columns`. Incompatible with `basic` / `with_buckets` parts.
