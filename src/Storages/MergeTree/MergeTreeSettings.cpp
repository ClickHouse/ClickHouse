...
If enabled, SharedMergeTree uses one of server-level pooled ZooKeeper sessions.
    ...
    DECLARE(Bool, reuse_precomputed_mutations_after_keeper_reconnect, false, R"(
If enabled, an in-progress part mutation is allowed to survive a transient
ZooKeeper (Keeper) session re-establishment instead of being unconditionally
cancelled. The already-computed temporary part is kept and reused once the
session is back, provided the assignment is still valid (the source part still
exists and is still assigned to this replica, the mutation is not done or
killed, the table metadata is unchanged, and no superseding merge or mutation
replaced it). Otherwise the pre-computed result is discarded and the part is
re-mutated as usual. This is a best-effort, fail-closed optimization: it never
commits a result that the current Keeper state would not accept.
)", 0) \
