...
void MergeTreeSinkPatch::finishDelayedChunk()
{
    ...
    for (auto & partition : delayed_chunk->partitions)
    {
        ...
        const std::vector<String> block_ids = partition.block_with_partition.deduplication_info->getBlockIds(part->info.getPartitionId(), storage.getDeduplicationLog() != nullptr);

        auto conflicts = commitPart(part, *partition.block_with_partition.block, block_ids);
