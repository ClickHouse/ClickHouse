...
MergeTreeReaderSettings MergeTreeReaderSettings::createFromContext(const ContextPtr & context)
{
    ...
    /// Refuse to write to the cache from contexts running with relaxed / experimental settings.
    /// See `Settings::isQueryConditionCacheWritable` and issue #104203.
    result.query_condition_cache_writable = result.use_query_condition_cache && settings.isQueryConditionCacheWritable();
