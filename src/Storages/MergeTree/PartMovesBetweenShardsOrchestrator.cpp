    ...
    switch (entry.state.value)
    {
        ...
        case EntryState::DESTINATION_ATTACH:
        {
            ...
            else
            {
                ...
                auto block_number_lock = storage.allocateBlockNumber(part->info.partition_id, std::make_shared<ZooKeeperWithFailtInjection>(zk), attach_log_entry_barrier_path, entry.to_shard);
