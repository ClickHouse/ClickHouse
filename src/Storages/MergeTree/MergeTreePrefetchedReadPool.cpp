    ...
    , prefetch_runner(pool, ThreadName::PREFETCH_READER)
{
    ...
    prefetch_runner.enqueueAndKeepTrack([this, &task, &read_prefetch]
    {
        ...
        readers = MergeTreeReadTask::createReaders(task.read_info, read_prefetch.getExtras(), task.ranges, task.patches_ranges, task.planned_last_mark);
...
void MergeTreePrefetchedReadPool::createPrefetchedReadersForTask(ThreadTask & task)
{
    ...
    auto readers = MergeTreeReadTask::createReaders(task.read_info, extras, task.ranges, task.patches_ranges, task.planned_last_mark);
...
MergeTreeReadTaskPtr MergeTreePrefetchedReadPool::createTask(ThreadTask & task, MergeTreeReadTask * previous_task)
{
    if (task.isValidReadersFuture())
        ...
        return MergeTreeReadPoolBase::createTask(task.read_info, task.ranges, task.patches_ranges, previous_task, updater, task.planned_last_mark);
}
...
void MergeTreePrefetchedReadPool::fillPerThreadTasks(size_t threads, size_t sum_marks)
{
    ...
    /// reused across them, so every task announces the stripe's end, not its own.
    for (auto & [thread_id, tasks] : per_thread_tasks)
    {
        for (size_t begin = 0; begin < tasks.size();)
        {
            ...
            for (size_t task_i = begin; task_i < end; ++task_i)
                tasks[task_i]->planned_last_mark = stripe_last_mark;
            begin = end;
        }
    }
