...
{
    String queue_path = pathToGenericString(fs::path(replica_path) / "queue");
...

    {
        ...
        {  /// Mutation pointer is a part of "state" and must be updated with state mutex
            ...
            if (zookeeper->tryGet(pathToGenericString(fs::path(replica_path) / "mutation_pointer"), mutation_pointer, &mutation_pointer_stat))
...
void ReplicatedMergeTreeQueue::removeProcessedEntry(zkutil::ZooKeeperPtr zookeeper, LogEntryPtr & entry)
{
    ...
    auto code = zookeeper->tryRemove(pathToGenericString(fs::path(replica_path) / "queue" / entry->znode_name));
    if (code != Coordination::Error::ZOK)
...
std::pair<int32_t, int32_t> ReplicatedMergeTreeQueue::pullLogsToQueue(zkutil::ZooKeeperPtr zookeeper, Coordination::WatchCallbackPtr watch_callback, PullLogsReason reason)
{
    ...
    if (index_str.empty())
    {
        ...
        zookeeper->set(pathToGenericString(fs::path(replica_path) / "log_pointer"), toString(index));
    }
    else
    {
        ...
        for (size_t entry_idx = 0, num_entries = log_entries.size(); entry_idx < num_entries;)
        {
            ...
            for (size_t i = 0; i < get_num; ++i)
            {
                ...
                ops.emplace_back(zkutil::makeCreateRequest(
                    pathToGenericString(fs::path(replica_path) / "queue/queue-"), res.data, zkutil::CreateMode::PersistentSequential));
            ...
            ops.emplace_back(zkutil::makeSetRequest(
                pathToGenericString(fs::path(replica_path) / "log_pointer"), toString(last_entry_index + 1), -1));

            if (min_unprocessed_insert_time_changed)
                ops.emplace_back(zkutil::makeSetRequest(
                    pathToGenericString(fs::path(replica_path) / "min_unprocessed_insert_time"), toString(*min_unprocessed_insert_time_changed), -1));
...
int32_t ReplicatedMergeTreeQueue::updateMutations(zkutil::ZooKeeperPtr zookeeper, Coordination::WatchCallbackPtr watch_callback)
{
    ...
    Strings entries_in_zk = zookeeper->getChildrenWatch(
        pathToGenericString(fs::path(zookeeper_path) / "mutations"),
        &mutations_stat,
    ...
    zkutil::ZooKeeperPtr zookeeper, const String & mutation_id)
{
    ...
    auto rc = zookeeper->tryRemove(pathToGenericString(fs::path(zookeeper_path) / "mutations" / mutation_id));
    ...
    const std::optional<ReplicatedMergeTreeLogEntryData> & covering_entry)
{
    ...
    for (Queue::iterator it = queue.begin(); it != queue.end();)
    {
        ...
        if (simple_op_covered || replace_range_covered)
        {
            ...
            auto code = zookeeper->tryRemove(pathToGenericString(fs::path(replica_path) / "queue" / znode_name));
            if (code != Coordination::Error::ZOK)
...
bool ReplicatedMergeTreeQueue::tryFinalizeMutations(zkutil::ZooKeeperPtr zookeeper)
{
    ...
    {
        Coordination::Stat mutation_pointer_stat;
        zookeeper->set(pathToGenericString(fs::path(replica_path) / "mutation_pointer"), finished.back()->znode_name, -1, &mutation_pointer_stat);
