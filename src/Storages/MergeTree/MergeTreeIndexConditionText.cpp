...
static bool tokenizerSplitsAtZeroByte(ITokenizer::Type type)
{
    return type == ITokenizer::Type::SplitByNonAlpha
        ...
        || type == ITokenizer::Type::StringZilla;
}
    ...
    RPNElement & out) const
{
    ...
    if (function_name == "hasPhrase")
    {
        /// Only splitByNonAlpha, splitByString, splitByRegexp, ngrams, asciiCJK, stringzilla, and icu tokenizers are supported with the `hasPhrase` function.
        static const std::unordered_set<std::string_view> supported_tokenizers = {
            ...
            StringZillaTokenizer::getExternalName(),
#if USE_ICU
