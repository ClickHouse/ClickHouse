    ...
    RPNElement & out) const
{
    ...
    if (!indexed_type || *set_key_position >= types.size()
        || !textIndexSetElementIsComparable(
               ...
               has_preprocessor && preprocessor->isASCIILowerOrUpper(),
    ...
    for (size_t row = 0; row < total_row_count; ++row)
    {
        ...
        if (indexed_map_element && indexed_type && isDefaultMapValue(indexed_type, element))
        {
            out.text_search_queries.clear();
            return false;
        }
        ...
        try
        {
            tokens = stringToTokens(Field(String(element)));
        }
