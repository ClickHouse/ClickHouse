...
public:
    ...
    /// Discards `Hint`-mode queries whose estimated cardinality (read postings + `postings_cardinality`
...
private:
    ...
    /// Estimates the cardinality of a query from already-read postings and `postings_cardinality` hints for unread tokens.
