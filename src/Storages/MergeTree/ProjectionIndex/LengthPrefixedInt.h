...
[[noreturn]] inline void throwInvalidMarker(UInt8 first_byte)
{
    throw Exception(
        ErrorCodes::CORRUPTED_DATA,
        "Invalid LengthPrefixedInt marker byte: 0x{:02x} (the only valid >=4-byte markers are 0x{:02x} and 0x{:02x})",
        static_cast<unsigned>(first_byte),
        static_cast<unsigned>(FOUR_BYTE_MARKER),
        static_cast<unsigned>(FIVE_BYTE_MARKER));
}
...
inline void readUInt32Impl(UInt32 & x, ReadBuffer & istr)
{
    ...
    if (first_byte != FIVE_BYTE_MARKER) [[unlikely]]
        throwInvalidMarker(first_byte);
...
/// Read a length-prefixed UInt32 from a raw pointer (caller must ensure sufficient data).
/// Advances `ptr` past the consumed bytes.
inline void readUInt32(UInt32 & x, const uint8_t *& ptr)
{
    const UInt8 first_byte = *ptr++;
