...
{
    auto index_ast = make_intrusive<ASTIndexDeclaration>(proj.index->clone(), proj.type->clone(), proj.name);
                ...
                forEachTokenPadded(
                    ...
                    [&](const char * token_start, size_t token_length)
                    {
                        ...
                        reinterpret_cast<PostingListData *>(it->getMapped())
                            ->writer.add(static_cast<UInt32>(starting_offset + row), posting_arena.get(), packed_buffer);
