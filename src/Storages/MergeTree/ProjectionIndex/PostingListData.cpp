...
private:
    void loadNextBlock()
    {
        ...
        if (count == 128)
            p4d1dec128v32(src_ptr, 128, doc_buffer, last_doc_id);
        else
            p4d1dec32(src_ptr, count, doc_buffer, last_doc_id);
        ...
        if (doc_count > 1)
        {
            ...
            if (in.available() >= bytes)
            {
                uint8_t * packed_buffer_end
                    = p4d1dec32(reinterpret_cast<uint8_t *>(in.position()), doc_count - 1, &embedded_postings[1], last_doc_id);
            ...
            else
            {
                ...
                uint8_t * packed_buffer_end = p4d1dec32(packed_buffer, doc_count - 1, &embedded_postings[1], last_doc_id);
    ...
    const MergeTreeIndexTextParams & index_params) const
{
    ...
    if (doc_count <= MAX_SIZE_OF_EMBEDDED_POSTINGS)
    {
        ...
        uint8_t * packed_buffer_end = p4d1enc32(const_cast<UInt32 *>(&embedded_postings[1]), doc_count - 1, packed_buffer, first_doc_id);
        UInt32 len = static_cast<UInt32>(packed_buffer_end - packed_buffer);
        VarInt::writeVarUInt32(len, wb);
        wb.write(reinterpret_cast<const char *>(packed_buffer), len);
        return;
    ...
    while (it->has_value)
    {
        ...
        uint8_t * end;
        if (n == 128)
            end = p4d1enc128v32(doc_buffer, 128, packed_buffer, last_doc_id);
        else
            end = p4d1enc32(doc_buffer, n, packed_buffer, last_doc_id);
