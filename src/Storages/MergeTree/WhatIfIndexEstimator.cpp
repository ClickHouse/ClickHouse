...
WhatIfResult buildEmptyTableResult(const MergeTreeData & data, const HypotheticalObjectStore & store)
{
    WhatIfResult result;
    ...
    const ASTPtr & select_query, ContextPtr context, const ASTPtr & explain_settings)
{
    ...
    if (read_steps.empty())
    {
        ...
        if (const auto * mt = dynamic_cast<const MergeTreeData *>(storage.get());
            ...
            return buildEmptyTableResult(*mt, local_context->getHypotheticalObjectStore());
    ...
    for (const auto & index_desc : hypo_indexes)
    {
        ...
        if (want_combined && index_result.status == WhatIfCandidateResult::Applicable && index_result.estimate_source == "empirical")
        {
            if (!combined_started)
            {
