...
void IMergeTreeReader::evaluateMissingDefaults(Block additional_columns, Columns & res_columns)
{
    try
    {
        ...
        auto dag = DB::evaluateMissingDefaults(
                additional_columns, columns, metadata_snapshot->getColumns(), Context::getGlobal());
        if (dag)
        {
            auto actions = std::make_shared<
                ExpressionActions>(std::move(dag),
                ExpressionActionsSettings::fromSettings(Context::getGlobal()->getSettingsRef()));
...
void IMergeTreeReader::performRequiredConversions(Columns & res_columns)
{
    try
    {
        ...
        DB::performRequiredConversions(copy_block, columns, Context::getGlobal());
