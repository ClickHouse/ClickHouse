...
IMergeTreeReader::IMergeTreeReader(
    ...
    , part_columns(
          data_part_info_for_read->isWidePart() ? data_part_info_for_read->getColumnsDescriptionWithCollectedNested()
...
{
    columns_to_read.reserve(converted_requested_columns.size());
    auto * requested = data_part_info_for_read->isWidePart() ? &original_requested_columns : &converted_requested_columns;
    serializations.reserve(converted_requested_columns.size());
...
{
    chassert(columns.size() == converted_requested_columns.size());
    ...
    auto it = converted_requested_columns.begin();
...
void IMergeTreeReader::performRequiredConversions(Columns & res_columns) const
{
    try
    {
        size_t num_columns = converted_requested_columns.size();
        ...
        Block copy_block;
        auto name_and_type = converted_requested_columns.begin();
        ...
        DB::performRequiredConversions(copy_block, converted_requested_columns, data_part_info_for_read->getContext());

        /// Move columns from block.
        name_and_type = converted_requested_columns.begin();
        for (size_t pos = 0; pos < num_columns; ++pos, ++name_and_type)
...
{
    if (num_columns_to_read != converted_requested_columns.size())
        throw Exception(
