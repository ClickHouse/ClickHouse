...
MergeTreeIndexPtr usearchIndexCreator(const IndexDescription & index)
{
    ...
    if (!index.arguments.empty())
        metric_kind = distanceFunctionToMetricKind.at(index.arguments[0].get<String>());

    static constexpr auto default_scalar_kind = unum::usearch::scalar_kind_t::f16_k;
    auto scalar_kind = default_scalar_kind;
    if (index.arguments.size() > 1)
        scalar_kind = quantizationToScalarKind.at(index.arguments[1].get<String>());
...
void usearchIndexValidator(const IndexDescription & index, bool /* attach */)
{
    ...
    if (!index.arguments.empty() && !distanceFunctionToMetricKind.contains(index.arguments[0].get<String>()))
    ...
    if (index.arguments.size() > 1 && !quantizationToScalarKind.contains(index.arguments[1].get<String>()))
