...
MergeTreeRangeReader::ReadResult MergeTreeRangeReader::startReadingChain(size_t max_rows, MarkRanges & ranges)
{
    ...
    bool is_projection_index_reader = typeid_cast<MergeTreeReaderProjectionIndex *>(merge_tree_reader) != nullptr;

    if (is_projection_index_reader)
        result.columns.resize(1); /// Filter column generated by projection index
    else
    ...
    /// Extract the filter column produced by the projection index and move it to `final_filter`.
    if (is_projection_index_reader)
    {
        chassert(result.columns.size() == 1);
        chassert(result.columns.front() != nullptr);
...
{
    size_t num_rows = result.total_rows_per_granule;
    ...
    /// If there's a leftover granule, adjust leading_end_part_offset using result.rows_per_granule[i].
    /// This is needed because part_offset columns might be filled in continueReadingChain when projection index is used.
    size_t i = 0;
    if (rows_per_granule.size() == start_ranges.size() + 1)
    {
        leading_end_part_offset = std::min(leading_end_part_offset, leading_begin_part_offset + result.rows_per_granule[i]);
        ++i;
    }
    ...
    for (const auto & start_range : start_ranges)
    {
        ...
        UInt64 end_part_offset
            = std::min(index_granularity->getMarkStartingRow(start_range.range.end), start_part_offset + rows_per_granule[i]);
        ++i;
...
void MergeTreeRangeReader::executePrewhereActionsAndFilterColumns(ReadResult & result, const Block & previous_header, bool is_last_reader) const
{
    ...
    /// TODO(amos): Why not just use header?
