...
MergeTreeRangeReader::ReadResult MergeTreeRangeReader::read(size_t max_rows, MarkRanges & ranges)
{
    ...
    {
        // Slicing deleted rows mask according to the read range into is_deleted:
        ...
        for (const auto & range : read_result.startedRanges())
        {
            ...
            const auto begin_row = index_granularity->getMarkStartingRow(range.range.begin);
            const auto end_row = range.range.end == 0 ? 0 : index_granularity->getRowsCountInRange(range.range.begin, range.range.end);
...
//    }
    ...
    // TODO: maybe combineFilters() with negated read_result.is_deleted ?
