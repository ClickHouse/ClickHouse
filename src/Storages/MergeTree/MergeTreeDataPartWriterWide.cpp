...
void MergeTreeDataPartWriterWide::validateColumnOfFixedSize(const NameAndTypePair & name_type)
{
    ...
    auto mrk_file_in = getDataPartStorage().readFile(mrk_path, getReadSettings(), std::nullopt, std::nullopt);
    ...
    DB::CompressedReadBufferFromFile bin_in(getDataPartStorage().readFile(bin_path, getReadSettings(), std::nullopt, std::nullopt));
