    ...
    const Granule & granule)
{
    if (granule.mark_on_start)
        writeSingleMark(name_and_type, offset_columns, granule.granularity_rows, serialize_settings.path);

    name_and_type.type->serializeBinaryBulkWithMultipleStreams(column, granule.start_row, granule.granularity_rows, serialize_settings, serialization_state);
