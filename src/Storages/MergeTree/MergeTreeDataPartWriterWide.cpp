...
void MergeTreeDataPartWriterWide::validateColumnOfFixedSize(const NameAndTypePair & name_type)
{
    ...
    {
        auto compressed_mrk_in = std::make_unique<CompressedReadBufferFromFile>(std::move(mrk_file_in));
        compressed_mrk_in->allowUnboundedDecompressedSize();
        mrk_in = std::move(compressed_mrk_in);
    }
    else
    ...
    bin_in.allowUnboundedDecompressedSize();
