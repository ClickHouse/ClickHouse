...
private:
    ...
    /// (no nested transformations, and not the UTF-8 variants: those fold non-ASCII code points onto
    /// ASCII letters, which the ASCII-only `ILIKE` on the raw column does not do).
