    ...
    F && callback)
{
    ...
    {
        const bool with_null = isNullableOrLowCardinalityNullable(data_types[col]);
    ...
    if (left_bounded)
    {
        ...
        result = BoolMask::combine(
            ...
            forAnyHyperrectangle(
                ...
                prefix_size + 1, initial_mask, callback));
    ...
    F && callback)
{
    ...
    {
        const bool with_null = isNullableOrLowCardinalityNullable(sparse_data_types[sparse_pos]);
        return key_order.isReversed(key_index)
            ? Range::createRightBounded(sparse_left_keys[sparse_pos], included, with_null)
            : Range::createLeftBounded(sparse_left_keys[sparse_pos], included, with_null);
    };
    ...
    auto values_before_right_boundary = [&](size_t key_index, size_t sparse_pos, bool included) -> Range
    {
        const bool with_null = isNullableOrLowCardinalityNullable(sparse_data_types[sparse_pos]);
        return key_order.isReversed(key_index)
            ? Range::createLeftBounded(sparse_right_keys[sparse_pos], included, with_null)
            : Range::createRightBounded(sparse_right_keys[sparse_pos], included, with_null);
    };
