...
{
    explicit DataPartsLock(std::shared_mutex & data_parts_mutex_, std::function<void(DataPartsLock&)> callback_);
    ~DataPartsLock();
...
public:
    ...
    DataPartsLock lockParts()
    {
        ...
        {
            this->recomputeSharedPartsList(lock);
        });
    }
    ...
    void removePartsFromWorkingSetImmediatelyAndSetTemporaryState(const DataPartsVector & remove, const DataPartsLock & lock);
...
protected:
    ...
    void resetObjectColumnsFromActiveParts(DataPartsLock & lock);
    void updateObjectColumns(const DataPartPtr & part, DataPartsLock & lock);
