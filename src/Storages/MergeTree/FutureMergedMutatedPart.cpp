...
{
    updatePath(storage, reservation->getDisk());
}

void FutureMergedMutatedPart::updatePath(const MergeTreeData & storage, const DiskPtr & disk)
{
    path = fs::path(storage.getFullPathOnDisk(disk)) / name;
    path += "/";
}
