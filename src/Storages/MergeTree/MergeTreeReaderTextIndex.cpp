    ...
    std::string_view token, const TokenPostingsInfo & token_info, size_t block_idx, PostingListCodecPtr posting_list_codec)
{
    ...
    return MergeTreeIndexGranuleText::readPostingsBlock(
        *postings_stream, *deserialization_state, token_info, block_idx, posting_list_codec);
}
...
std::vector<PostingListPtr> MergeTreeReaderTextIndex::readPostingsBlocksForToken(std::string_view token, const TokenPostingsInfo & token_info, const RowsRange & range)
{
    ...
    for (const auto & block_idx : blocks_to_read)
    {
        ...
        if (inserted)
            it->second = readPostingsBlockForToken(token, token_info, block_idx, granule_text.getPostingListCodec());
