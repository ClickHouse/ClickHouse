...
[PRIMARY KEY expr]
[SAMPLE BY expr]
[TTL expr [DELETE|RECOMPRESS codec_name|TO DISK 'xxx'|TO VOLUME 'xxx'|CLEAR INDEX index_name], ...]
[SETTINGS name=value, ...]
[COMMENT 'comment']
...
static StoragePtr create(const StorageFactory::Arguments & args)
{
    ...
    if (is_extended_storage_def)
    {
        ...
        {
            auto new_ttl_entry = TTLDescription::getTTLForColumnFromAST(ast, columns, context, metadata.primary_key, ttl_validation_mode);
...
A table can have an expression for removal of expired rows, and multiple expressions for automatic move of parts between [disks or volumes](#table_engine-mergetree-multiple-volumes), recompression, or clearing secondary index files. When rows in the table expire, ClickHouse deletes all corresponding rows. For parts moving, recompressing, or clearing index files, all rows of a part must satisfy the `TTL` expression criteria.
