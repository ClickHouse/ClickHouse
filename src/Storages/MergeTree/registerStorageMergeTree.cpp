...
:::note
This index type is experimental. Set [`allow_experimental_cuckoo_filter_index`](/reference/settings/session-settings/allow-experimental#allow_experimental_cuckoo_filter_index) to `1` when defining a new experimental index or changing an existing experimental index definition. Unrelated `ALTER` queries on tables that already contain the index, and `DROP INDEX`, do not require the setting. The on-disk format is new; use only when you accept the operational risk of mixed versions and upgrades.
...
:::note
This index type is experimental. Set [`allow_experimental_binary_fuse_filter_index`](/reference/settings/session-settings/allow-experimental#allow_experimental_binary_fuse_filter_index) to `1` when defining a new experimental index or changing an existing experimental index definition. Unrelated `ALTER` queries on tables that already contain the index, and `DROP INDEX`, do not require the setting. The on-disk format is new; use only when you accept the operational risk of mixed versions and upgrades.
