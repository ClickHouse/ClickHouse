...
An `ALIAS` column referenced by an index expression - directly or through matcher expansion - is indexed through its expression, without a conversion to the declared type of the alias. For an alias whose declared type narrows the type of its expression, such as `b UInt8 ALIAS a` over `a UInt16`, an index over `b` therefore aggregates values of `a`. Indices that re-apply the predicate to the indexed values, such as `set`, still prune on predicates over the alias; indices that compare the predicate against stored bounds, such as `minmax`, cannot be used for them. Write the intended expression explicitly, for example `INDEX idx toUInt8(a)`, when the index should aggregate the narrowed values.
