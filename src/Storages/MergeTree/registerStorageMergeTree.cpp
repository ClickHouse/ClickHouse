...
- Lightweight updates (`UPDATE ... SET`, and `ALTER TABLE ... UPDATE` with `alter_update_mode` set to a lightweight mode) run on the leader: they write patch parts and need no hard links, so unlike a heavy mutation they are supported on `plain_rewritable`. They are admission-fenced by the leadership epoch exactly like an `INSERT`, and the leader's cleanup of the patch parts it no longer needs converges on the followers through the refresh scan. A follower rejects them with `TABLE_IS_READ_ONLY`.
