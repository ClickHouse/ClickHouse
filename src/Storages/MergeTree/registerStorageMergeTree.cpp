...
Deserialized granules of `bloom_filter`, `ngrambf_v1`, `tokenbf_v1` and `sparse_grams` indexes are kept in the skipping index cache, so repeated queries do not re-read and re-deserialize them from disk. The cache size is controlled by the server setting `skipping_index_cache_size`, its usage is reported by the metrics `SkippingIndexCacheBytes` and `SkippingIndexCacheCells` and the profile events `SkippingIndexCacheHits` and `SkippingIndexCacheMisses`, and it can be cleared with [`SYSTEM DROP SKIPPING INDEX CACHE`](/reference/statements/system#drop-skipping-index-cache).
