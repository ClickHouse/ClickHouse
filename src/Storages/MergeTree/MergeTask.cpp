...
bool MergeTask::ExecuteAndFinalizeHorizontalPart::prepare() const
{
    ...
    global_ctx->to = std::make_shared<MergedBlockOutputStream>(
        ...
        global_ctx->merging_columns,
        MergeTreeIndexFactory::instance().getMany(global_ctx->metadata_snapshot, global_ctx->merging_skip_indexes),
...
MergeTask::VerticalMergeStage::createPipelineForReadingOneColumn(const String & column_name) const
{
    ...
    {
        indexes_to_recalc = MergeTreeIndexFactory::instance().getMany(global_ctx->metadata_snapshot, indexes_it->second);
...
bool MergeTask::MergeTextIndexStage::prepare() const
{
    ...
    {
        auto index_ptr = MergeTreeIndexFactory::instance().get(global_ctx->metadata_snapshot, index);
