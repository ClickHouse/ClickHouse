...
ReadScope::ReadScope(String part_id_, MarkRanges mark_ranges_, Phase phase_, std::vector<ByteRange> granule_ranges_)
    : part_id(std::move(part_id_))
    , mark_ranges(std::move(mark_ranges_))
    , phase(phase_)
    , granule_ranges(std::move(granule_ranges_))
    ...
    String part_id_, MarkRanges mark_ranges_, Phase phase_, std::vector<ByteRange> granule_ranges_)
{
    ...
    return ReadScopePtr(new ReadScope(
        std::move(part_id_), std::move(mark_ranges_), phase_, std::move(granule_ranges_)));
}
...
String ReadScope::toString() const
{
    ...
    {
        size_t total = 0;
        for (const auto & r : granule_ranges)
            total += r.end - r.begin;
        return total;
    }());
    return result;
...
std::shared_ptr<const ReadScope> ReadScope::adjustForObject(size_t object_start_offset, size_t object_size) const
{
    ...
    if (object_start_offset == 0
        && (granule_ranges.empty() || granule_ranges.back().end <= object_size))
        return shared_from_this();

    size_t object_end = object_start_offset + object_size;

    std::vector<ByteRange> adjusted;
    adjusted.reserve(granule_ranges.size());
    for (const auto & r : granule_ranges)
    {
...
{
    /// Compute the byte range to prefetch from granule_ranges.
    ...
    if (scope.granule_ranges.empty())
    {
        LOG_DEBUG(log, "createObjectReadBuffer: no granule_ranges, falling back. blob={} scope=[{}]",
            object.remote_path, scope.toString());
        return nullptr;
    }

    size_t range_begin = scope.granule_ranges.front().begin;
    size_t range_end = scope.granule_ranges.front().end;
    for (const auto & gr : scope.granule_ranges)
    {
        range_begin = std::min(range_begin, gr.begin);
        range_end = std::max(range_end, gr.end);
    }
