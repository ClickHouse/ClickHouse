...
constexpr auto REPLICATION_PROTOCOL_VERSION_WITH_COLUMN_COMPRESSION_CODECS = 10;
...
void Service::processQuery(const HTMLForm & params, ReadBufferPtr body, WriteBuffer & out, HTTPServerResponse & response)
{
    ...
    response.addCookie({"server_protocol_version", toString(std::min(client_protocol_version, REPLICATION_PROTOCOL_VERSION_WITH_COLUMN_COMPRESSION_CODECS))});
    ...
    bool send_projections)
{
    ...
    for (const auto & name : file_names_without_checksums)
    {
        ...
        if (client_protocol_version < REPLICATION_PROTOCOL_VERSION_WITH_COLUMN_COMPRESSION_CODECS
            && name == IMergeTreeDataPart::COLUMN_COMPRESSION_CODECS_FILE_NAME)
            continue;
    ...
    DiskPtr disk)
{
    ...
    uri.setQueryParameters(
    {
        ...
        {"part",                    part_name},
        {"client_protocol_version", toString(REPLICATION_PROTOCOL_VERSION_WITH_COLUMN_COMPRESSION_CODECS)},
        {"compress",                "false"}
    });
    ...
    bool sync) const
{
    ...
    for (size_t i = 0; i < files; ++i)
    {
        ...
        if (file_name != "checksums.txt" &&
            ...
            file_name != IMergeTreeDataPart::COLUMN_COMPRESSION_CODECS_FILE_NAME &&
