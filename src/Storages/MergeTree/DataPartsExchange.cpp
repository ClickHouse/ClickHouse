    ...
    const std::map<String, std::shared_ptr<IMergeTreeDataPart>> & projections)
{
    ...
    for (auto & [name, projection] : part->getProjectionParts())
    {
        ...
        if (it != projections.end())
        {
            ...
            data_checksums.addFile(name, projection_checksum.getTotalSizeOnDisk(), projection_checksum.getTotalChecksumUInt128());
        }
        else if (part->checksums.has(name))
        {
            // We don't send this projection, just add out checksum to bypass the following check
            const auto & our_checksum = part->checksums.files.find(name)->second;
            data_checksums.addFile(name, our_checksum.file_size, our_checksum.file_hash);
        }
    }
    ...
    const String & path)
{
    ...
    if (path.empty())
    {
        // Download projections if it's not a projection part
        static const String TMP_PREFIX = "tmp_fetch_";
        String tmp_prefix = tmp_prefix_.empty() ? TMP_PREFIX : tmp_prefix_;

        part_relative_path = String(to_detached ? "detached/" : "") + tmp_prefix + part_name;
        part_download_path = data.getRelativeDataPath() + part_relative_path + "/";
        ...
        for (auto i = 0ul; i < projections; ++i)
        {
            ...
            downloadPartToDisk(
                projection_name, replica_path, {}, {}, sync, disk, in, 0, projection_checksum, part_download_path + projection_name + "/");
            checksums.addFile(projection_name, projection_checksum.getTotalSizeOnDisk(), projection_checksum.getTotalChecksumUInt128());
        }
    }
    else
    {
        // Prepare to download the projection part
        if (disk->exists(part_download_path))
            throw Exception("Directory " + fullPath(disk, part_download_path) + " already exists.", ErrorCodes::DIRECTORY_ALREADY_EXISTS);
        disk->createDirectories(part_download_path);
    }
    ...
    // Return nullptr if it's a projection part. The result is unused.
    return nullptr;
}
