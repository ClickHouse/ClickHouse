    ...
    LoggerPtr log)
{
    ...
    if (const auto & prewhere_info = select_query_info.prewhere_info)
    {
        for (const auto * dag : prewhere_info->prewhere_actions.getOutputs())
        {
            if (dag->result_name == prewhere_info->prewhere_column_name)
            {
                ...
                LOG_DEBUG(log, "Query condition cache has dropped {}/{} granules for PREWHERE condition {}.", stats.granules_dropped, stats.total_granules, prewhere_info->prewhere_column_name);
                break;
            }
        }
    }
    ...
    {
        const auto * dag = filter_actions_dag->getOutputs().front(); /// TODO do this for all outputs
        auto stats = drop_mark_ranges(dag);
        LOG_DEBUG(log, "Query condition cache has dropped {}/{} granules for WHERE condition {}.", stats.granules_dropped, stats.total_granules, filter_actions_dag->getOutputs().front()->result_name);
    }
}
