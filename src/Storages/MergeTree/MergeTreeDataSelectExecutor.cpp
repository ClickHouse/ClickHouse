    ...
    LoggerPtr log)
{
    ...
    if (!key_condition_exact_range || !part_offset_condition_exact_range || !total_offset_condition_exact_range)
    {
        ...
        /// Reproduces the range gluing that `genericExclusionSearch` applies, so the short-circuit
        /// below yields ranges identical to the full search.
            ...
            && !total_offset_condition_useful && !part_with_ranges.ranges.empty())
        {
            ...
            {
                /// Reproduce exactly what `genericExclusionSearch` would build.
