...
void ReplicatedMergeTreeBlockOutputStream::write(const Block & block)
{
    ...
    for (auto & current_block : part_blocks)
    {
        ...
        try
        {
            ...
            PartLog::addNewPart(Context::getGlobal(), part, watch.elapsed(), ExecutionStatus(error));
        }
        catch (...)
        {
            PartLog::addNewPart(Context::getGlobal(), part, watch.elapsed(), ExecutionStatus::fromCurrentException(__PRETTY_FUNCTION__));
            throw;
        }
    }
}
