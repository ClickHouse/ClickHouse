...
namespace CurrentMetrics
{
    ...
    extern const Metric ColumnsDescriptionsCacheBytes;
    extern const Metric NamesAndTypesCacheSize;
    extern const Metric NamesAndTypesCacheBytes;
}
...
MergeTreeData::MergeTreeData(
    ...
    , columns_descriptions_bytes_metric_handle(CurrentMetrics::ColumnsDescriptionsCacheBytes)
    , names_and_types_metric_handle(CurrentMetrics::NamesAndTypesCacheSize)
    , names_and_types_bytes_metric_handle(CurrentMetrics::NamesAndTypesCacheBytes)
...
ColumnsDescriptionCache MergeTreeData::getColumnsDescriptionForColumns(const NamesAndTypesList & columns) const
{
    ...
    /// Account for bytes after deduplication, so that a discarded `with_collected_nested`
    /// (null when it duplicates `original`) is not over-counted.
    size_t cache_bytes = 0;
    cache_bytes += cache.original->getBytesAllocated();
    if (cache.with_collected_nested)
    ...
    if (inserted)
    {
        ...
        columns_descriptions_metric_handle.add(1);
        columns_descriptions_bytes_metric_handle.add(cache_bytes);
    }
...
void MergeTreeData::decrefColumnsDescriptionForColumns(const NamesAndTypesList & columns) const
{
    ...
    if (it->second.original.use_count() == 1)
    {
        ...
        if (it->second.with_collected_nested)
        ...
        columns_descriptions_bytes_metric_handle.sub(bytes_size);
    }
}
...
std::shared_ptr<const NamesAndTypesList> MergeTreeData::getCachedNamesAndTypesList(NamesAndTypesList && columns) const
{
    ...
    if (inserted)
    {
        ...
        names_and_types_metric_handle.add(1);
        names_and_types_bytes_metric_handle.add(bytes);
    }
    return *it;
}
...
void MergeTreeData::decrefNamesAndTypesListInSharedCache(const std::shared_ptr<const NamesAndTypesList> & columns) const
{
    ...
    {
        /// `columns` is the shared_ptr still held by the IMergeTreeDataPart that is being destroyed;
        /// unlike `columns_description`, it is not reset before this call, so it still counts.
        /// 1 ref in the container + 1 ref in the part being destroyed => evict once it is the last holder.
        if (it->use_count() == 2)
        {
            const auto bytes = columns->get_allocator().getBytesAllocated();
            columns_list_cache_size_bytes -= bytes;
            names_and_types_metric_handle.sub(1);
            names_and_types_bytes_metric_handle.sub(bytes);
            /// The destruction of the object won't happen here, but in the destructor of the IMergeTreeDataPart.
