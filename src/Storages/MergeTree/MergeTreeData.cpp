    ...
    for (const AlterCommand & command : commands)
    {
            ...
            && new_metadata.getColumns().hasPhysical(command.column_name))
        {
                ...
                && tupleAddsSubfieldsOnly(old_type_it->second, command.data_type.get(), local_context))
            {
                ...
                /// Projections persist their own `primary.idx` and data serialized with
                /// the old tuple shape independently of the parent part. A metadata-only
                /// ALTER emits no `MutationCommand::READ_COLUMN`, so the rebuild logic in
                /// `MutationsInterpreter` never rematerializes the projection part. Reject
                /// when the column is referenced by a projection.
                if (auto it = columns_in_projections.find(command.column_name); it != columns_in_projections.end())
                {
                    throw Exception(
                        ErrorCodes::ALTER_OF_COLUMN_IS_FORBIDDEN,
                        "Adding new Tuple subfields to column '{}' is forbidden because it is used by projection '{}'. "
                        "Drop the projection first, then re-create it after the ALTER",
                        backQuoteIfNeed(command.column_name),
                        it->second);
                }
            }
