    ...
    const MutationCommands & commands, ContextPtr query_context) const
{
    ...
    else if (num_cmds == 1)
    {
        ...
        if (!alter || !alter->partition)
            ...
            return PartitionIds{getPartitionIDFromQuery(ASTPtr(alter->partition), query_context)};
    }
    else
    {
        std::vector<String> area;
        ...
        for (const auto & command : commands)
        {
            ...
            area.push_back(getPartitionIDFromQuery(ASTPtr(alter->partition), query_context));
        }
