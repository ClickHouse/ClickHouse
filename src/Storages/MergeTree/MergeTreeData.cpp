    ...
    ContextPtr query_context) const
{
    ...
    for (size_t row = 0, part_idx = 0; row < rows; ++row, ++part_idx)
    {
        ...
        if (minmax_idx_condition && !minmax_idx_condition->generateForPartition(*part).checkInHyperrectangle(part->getMinMaxIndex()->hyperrectangle, minmax_columns_types).can_be_true)
            continue;
