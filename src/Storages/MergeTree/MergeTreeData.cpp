...
struct PartsPruningResult
{
    ...
    bool shouldSkipPart(const DataPartPtr & part, const PartitionIdToMaxBlock * max_block_numbers_to_read) const
    {
        ...
        if (minmax_idx_condition
            && !minmax_idx_condition->generateForPart(part).checkInHyperrectangle(part->getMinMaxIndex()->hyperrectangle, minmax_columns_types).can_be_true)
            return true;
    ...
    ContextPtr query_context)
{
    ...
    if (filter_dag)
    {
        if (metadata_snapshot->hasPartitionKey())
        {
            ...
            result.minmax_columns_types = minmax_columns.getTypes();
            if (!minmax_columns.empty())
            {
                ...
                result.minmax_idx_condition = std::make_shared<ConditionTemplate<KeyCondition>>(inverted_dag, std::move(key_condition_factory), metadata_snapshot, query_context, /*skip_folding_=*/!query_context->getSettingsRef()[Setting::use_constant_folding_in_index_analysis]);
            }
            ...
            result.partition_pruner.emplace(
                ...
                false /* strict */, !query_settings[Setting::use_partition_pruning]);
        }
    ...
    ContextPtr query_context) const
{
    ...
    for (size_t row = 0, part_idx = 0; row < pruning_result.rows; ++row, ++part_idx)
    {
        ...
        /// Empty parts are skipped without appending to the filter column, so that
        /// it stays row-aligned with `virtual_columns_block` (built with ignore_empty=true).
        if (part->isEmpty())
            continue;
...
/// Info about an aggregate result column produced by column statistics optimization.
    ...
    ContextPtr query_context) const
{
    ...
    for (size_t i = 0; i < aggregate_descriptions.size(); ++i)
    {
        ...
        /// Column statistics min/max equal the result of `min`/`max` aggregation only for
        /// non-Nullable numeric-represented types.  For Nullable, LowCardinality, and
        /// non-numeric types the stored statistics may describe values that differ from what
        /// the aggregation function returns, so fall back to a normal read.
        WhichDataType which(col_desc.type);
        if (which.isNullable() || which.isLowCardinality() || !col_desc.type->isValueRepresentedByNumber())
        {
            LOG_TRACE(log, "Column statistics aggregation: column '{}' has unsupported type for statistics-based aggregation", physical_name);
            return {};
        }

        /// Whether the column has usable statistics is verified at the per-part level:
    ...
    for (const auto & agg_desc : aggregate_descriptions)
    {
        ...
        agg_columns.push_back({agg_desc.column_name, data_type, ColumnAggregateFunction::create(func), func, func_name == "min"});
    }
    ...
    for (size_t row = 0, part_idx = 0; row < surviving_rows; ++row, ++part_idx)
    {
        ...
        for (size_t agg_idx = 0; agg_idx < agg_columns.size(); ++agg_idx)
        {
            ...
            /// time) has statistics that do not describe the values the query sees.
            auto part_column = part->tryGetColumn(arg_col_name);
            if (!part_column || !part_column->type->equals(*agg_col.data_type))
            {
                LOG_TRACE(log, "Column statistics aggregation: part '{}' column '{}' type mismatch", part->name, arg_col_name);
                return {};
            }
