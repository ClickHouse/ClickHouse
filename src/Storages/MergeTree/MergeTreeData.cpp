    ...
    ContextPtr context)
{
    ...
    /// or `ALTER`). Settings-dependent functions (e.g. `toMonday` with
    /// `enable_extended_results_for_datetime_functions`) may return different types when
    /// re-evaluated with a different context. If the expression output types don't match the
    /// key's expected types, add `CAST` operations to ensure consistency. Without this, the
    /// MergeTree part writer would throw a "Bad cast" exception when serializing the primary
    /// index, because the serialization types (from key metadata) don't match the actual column
    /// types (from the re-evaluated expression).
    const auto & key_types = key.data_types;
    const auto & key_names = key.column_names;
    auto & dag_outputs = dag.getOutputs();

    /// The sorting-key's expected type is authoritative — it is fixed at key creation time and the
    /// primary index serializer cannot tolerate a different type. When the same expression name
    /// appears in both the sorting key and a skip index (for example, both use `toMonday(c0)`),
    /// the DAG has only one deduplicated output node for that name. Pin every key-named output
    /// unconditionally, even when the DAG output already matches the key type and no key `CAST`
    /// is needed: a later skip-index `CAST` with a different type would still replace the shared
    /// output node and corrupt the key read. This enforces unconditional key-name priority over
    /// skip-index normalization in both directions — whether the settings drift affects the key,
    /// the index, or both.
    std::unordered_set<std::string> key_output_names(key_names.begin(), key_names.end());
    ...
    for (const auto & index_ptr : indices)
    {
        ...
        {
            if (key_output_names.contains(idx_names[i]))
                continue;
