...
void MergeTreeData::setProperties(const StorageInMemoryMetadata & metadata, bool only_check, bool attach)
{
    ...
    ASTPtr skip_indices_expr_list = std::make_shared<ASTExpressionList>();
    ...
    auto syntax_indices = SyntaxAnalyzer(global_context).analyze(skip_indices_with_primary_key_expr_list, all_columns);
    auto new_indices_expr = ExpressionAnalyzer(skip_indices_expr_list, syntax_indices, global_context).getActions(false);
