...
/// `preprocessor_is_case_folding` marks a preprocessor that maps the bytes it is given without
/// consulting their type, so it yields the same tokens on a set element and on the index column.
