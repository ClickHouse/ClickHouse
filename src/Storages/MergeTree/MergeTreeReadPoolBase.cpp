    ...
    for (const auto & col_name : columns_to_read)
    {
        ...
        if (column)
        {
            if (column->isSubcolumn() && settings[Setting::allow_calculating_subcolumns_sizes_for_merge_tree_reading])
                data_compressed_size += part.getSubcolumnSize(*column).data_compressed;
            else
                data_compressed_size += part.getColumnSize(column->getColumnId()).data_compressed;
        }
    }
...
MergeTreeReadPoolBase::buildReadTaskInfo(const RangesInDataPart & part_with_ranges, const Settings & settings) const
{
    ...
    if (pool_settings.preferred_block_size_bytes > 0)
    {
        ...
        for (const auto & column_name : all_column_names)
        {
            ...
            if (snapshot_column && read_task_info.data_part->getColumnPosition(snapshot_column->getColumnId()))
