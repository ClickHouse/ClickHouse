    ...
    const WhereOptimizerContext & where_optimizer_context) const
{
    ...
    auto move_condition = [&](Conditions::iterator cond_it)
    {
        ...
        {
            if (jt != cond_it && jt->viable && jt->columns_size == group_columns_size && jt->table_columns == group_table_columns)
                group.push_back(jt);
        }
        std::sort(
            group.begin(),
            ...
            { return condition_positions.at(std::to_address(a)) < condition_positions.at(std::to_address(b)); });
        for (auto it : group)
