    ...
    , read_with_direct_io(read_with_direct_io_)
{
    ...
    if (!read_task_info->patch_parts.empty())
    {
        ...
        /// Wrap the single data part into structures expected by buildPatchJoinCachePipeline.
        std::vector<MergeTreeReadTaskInfoPtr> per_part_infos = {read_task_info};
        RangesInDataParts parts_ranges;
        parts_ranges.emplace_back(data_part, nullptr, 0, 0, mark_ranges);

        if (auto processors = buildPatchJoinCachePipeline(
            patch_join_cache,
