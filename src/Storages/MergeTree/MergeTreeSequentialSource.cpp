    ...
    , mark_cache(storage.getContext()->getMarkCache())
{
    ...
    {
        auto options = GetColumnsOptions(GetColumnsOptions::AllPhysical).withExtendedObjects().withVirtuals();  /// TODO: only add _rows_exists column (if it's present on disk)
