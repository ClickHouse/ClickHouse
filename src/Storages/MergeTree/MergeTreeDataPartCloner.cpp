...
bool MergeTreeDataPartCloner::doesStoragePolicyAllowSameDisk(
    MergeTreeData * merge_tree_data,
...
std::pair<MergeTreeData::MutableDataPartPtr, scope_guard> MergeTreeDataPartCloner::cloneSourcePart(
    MergeTreeData * merge_tree_data,
    const MergeTreeData::DataPartPtr & src_part,
    const StorageMetadataPtr & metadata_snapshot,
    const MergeTreePartInfo & dst_part_info,
...
)
{
    ...
    auto src_part_storage = flushPartStorageToDiskIfInMemory(
        ...
        src_flushed_tmp_part
    );

    auto dst_part_storage = hardlinkAllFiles(
        merge_tree_data,
        read_settings,
        write_settings,
        src_part_storage,
...
void MergeTreeDataPartCloner::reserveSpaceOnDisk(const MergeTreeData::DataPartPtr & src_part)
{
...
void MergeTreeDataPartCloner::handleHardLinkedParameterFiles(
    const MergeTreeData::DataPartPtr & src_part,
    const DB::IDataPartStorage::ClonePartParams & params
)
{
...
void MergeTreeDataPartCloner::handleProjections(
    const MergeTreeData::DataPartPtr & src_part,
    const DB::IDataPartStorage::ClonePartParams & params
)
{
...
MergeTreeData::MutableDataPartPtr MergeTreeDataPartCloner::finalizePart(
