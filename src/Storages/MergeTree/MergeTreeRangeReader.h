...
public:
    ...
    ReadStepPerformanceCountersPtr getCounterForProjectionIndexStep()
    {
        if (!projection_index_performance_counter)
            projection_index_performance_counter = std::make_shared<ReadStepPerformanceCounters>();
        return projection_index_performance_counter;
    }

    const std::vector<ReadStepPerformanceCountersPtr> & getCounters() const { return performance_counters; }
    const ReadStepPerformanceCountersPtr & getProjectionIndexCounter() const { return projection_index_performance_counter; }

private:
    std::vector<ReadStepPerformanceCountersPtr> performance_counters;
    ReadStepPerformanceCountersPtr projection_index_performance_counter;
};
...
public:
    ...
    ReadResult startProjectionReadingChain(size_t max_rows, MarkRanges & ranges);
