    ...
    size_t num_streams)
{
    ...
    if (has_row_policy)
    {
        ...
        /// reject virtuals the projection reorders (position-relative) or only exposes under a projection
        /// name absent on the parent (`_parent_part_offset`); the prewhere read pulls the rest itself
