...
MergeTreeReadTaskPtr MergeTreeReadPool::getTask(size_t task_idx, MergeTreeReadTask * previous_task)
{
    while (true)
    {
        ...
        if (!cutRangesToRead(task_idx, part_idx, thread_idx, need_marks, cut_ranges, planned_last_mark))
            return nullptr;
        ...
        return createTask(per_part_infos[part_idx], std::move(task_ranges), previous_task, updater, planned_last_mark);
    }
}

bool MergeTreeReadPool::cutRangesToRead(size_t task_idx, size_t & part_idx, size_t & thread_idx, size_t & need_marks, MarkRanges & ranges_to_get_from_part, size_t & planned_last_mark)
{
    ...
    cutFromThreadTask(thread_tasks, thread_idx, need_marks, ranges_to_get_from_part, planned_last_mark);
    return true;
}
...
bool MergeTreeReadPool::cutMoreRangesToRead(size_t thread_idx, size_t part_idx, size_t need_marks, MarkRanges & ranges_to_get_from_part)
{
    ...
    size_t planned_last_mark_unused = 0;
    cutFromThreadTask(thread_tasks, thread_idx, need_marks, ranges_to_get_from_part, planned_last_mark_unused);
    return true;
}

void MergeTreeReadPool::cutFromThreadTask(ThreadTask & thread_tasks, size_t thread_idx, size_t need_marks, MarkRanges & ranges_to_get_from_part, size_t & planned_last_mark)
{
    auto & thread_task = thread_tasks.parts_and_ranges.back();
    planned_last_mark = thread_task.planned_last_mark;
...
void MergeTreeReadPool::fillPerThreadInfo(size_t threads, size_t sum_marks)
{
    ...
    for (size_t i = 0; i < threads && !parts_queue.empty(); ++i)
    {
        ...
        while (need_marks > 0 && !parts_queue.empty())
        {
            ...
            size_t stripe_last_mark = 0;
            for (const auto & range : ranges_to_get_from_part)
                stripe_last_mark = std::max(stripe_last_mark, range.end);
            threads_tasks[i].parts_and_ranges.push_back({part_idx, ranges_to_get_from_part, stripe_last_mark});
