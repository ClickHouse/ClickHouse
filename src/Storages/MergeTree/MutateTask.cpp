...
namespace FailPoints
{
    ...
    extern const char mutate_task_pause_after_first_block[];
}
...
struct MutationContext
{
    ...
    /// Optional support for surviving a transient ZooKeeper session re-establishment (set via
    /// MutateTask::enableSurvivalAcrossTransientReconnect). All pointers are storage-owned.
    ...
    bool checkOperationIsNotCanceled() const
    {
        ...
        if (!blocker_cancelled)
            return true;

        /// The merges blocker is cancelled. If this mutation is allowed to survive a transient
        /// ZooKeeper session re-establishment and one is currently in progress (or this task has
        /// already detached itself as a survivor), keep computing instead of throwing away the work.
        /// A real shutdown still aborts it: the background executor cancels surviving tasks on a
        /// full shutdown, and we also bail out here if shutdown was requested.
        const bool can_survive = is_surviving_reconnect != nullptr
            && !(shutdown_called != nullptr && shutdown_called->load())
            && (is_surviving_reconnect->load()
                || (transient_reconnect_in_progress != nullptr && transient_reconnect_in_progress->load()));
...
bool PartMergerWriter::mutateOriginalPartAndPrepareProjections()
{
    ...
    do
    {
        ...
        /// Used by tests to pause a mutation while it has work in progress, e.g. to inject a
        /// transient ZooKeeper reconnection at that point.
        FailPointInjection::pauseFailPoint(FailPoints::mutate_task_pause_after_first_block);
...
{
    return std::move(ctx->temporary_directory_lock);
}
