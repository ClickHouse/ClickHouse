...
///
/// template <typename Distance>
/// MergeTreeIndexGranuleHnsw<Distance>::MergeTreeIndexGranuleHnsw(const String & index_name_, const Block & index_sample_block_)
...
///
/// template <typename Distance>
/// MergeTreeIndexGranuleHnsw<Distance>::MergeTreeIndexGranuleHnsw(
...
///
/// template <typename Distance>
/// void MergeTreeIndexGranuleHnsw<Distance>::serializeBinary(WriteBuffer & ostr) const
/// {
...
///
/// template <typename Distance>
/// void MergeTreeIndexGranuleHnsw<Distance>::deserializeBinary(ReadBuffer & istr, MergeTreeIndexVersion /*version*/)
/// {
///     UInt64 dimension;
...
///     auto granule = std::make_shared<MergeTreeIndexGranuleHnsw<Distance>>(index_name, index_sample_block, index);
///     index = nullptr;
///     return granule;
/// }
///
...
///
/// template <typename Distance>
/// std::vector<size_t> MergeTreeIndexConditionHnsw::getUsefulRangesImpl(MergeTreeIndexGranulePtr idx_granule) const
/// {
...
///
///     const auto granule = std::dynamic_pointer_cast<MergeTreeIndexGranuleHnsw<Distance>>(idx_granule);
///     if (granule == nullptr)
...
/// }
///
/// MergeTreeIndexHnsw::MergeTreeIndexHnsw(const IndexDescription & index_, UInt64 trees_, const String & distance_function_)
...
/// {
///     if (distance_function == "L2Distance")
///         return std::make_shared<MergeTreeIndexGranuleHnsw<Annoy::Euclidean>>(index.name, index.sample_block);
///     else if (distance_function == "cosineDistance")
///         return std::make_shared<MergeTreeIndexGranuleHnsw<Annoy::Angular>>(index.name, index.sample_block);
///     std::unreachable();
/// }
///
...
/// {
///     return std::make_shared<MergeTreeIndexConditionHnsw>(index, query, distance_function, context);
/// };
///
...
///
///     return std::make_shared<MergeTreeIndexHnsw>(index, trees, distance_function);
/// }
///
