...
MergeTreeTemporaryPartPtr MergeTreeDataWriter::writeTempPart(BlockWithPartition & block, StorageMetadataPtr metadata_snapshot, ContextPtr context)
{
    ...
    return writeTempPartImpl(block, std::move(metadata_snapshot), std::move(partition_id), /*patch_part_index=*/ std::nullopt, std::move(context), data.insert_increment.get());
}
...
MergeTreeTemporaryPartPtr MergeTreeDataWriter::writeTempPatchPart(
    ...
    ContextPtr context)
{
    return writeTempPartImpl(block, std::move(metadata_snapshot), std::move(partition_id), std::move(patch_part_index), std::move(context), data.insert_increment.get());
}
