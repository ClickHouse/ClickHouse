...
MergeTreeTemporaryPartPtr MergeTreeDataWriter::writeTempPart(BlockWithPartition & block, StorageMetadataPtr metadata_snapshot, ContextPtr context)
{
    ...
    return writeTempPartImpl(block, std::move(metadata_snapshot), std::move(partition_id), /*source_parts_set=*/ std::nullopt, std::move(context), data.insert_increment.get());
}
    ...
    UInt64 block_number)
{
    ...
    if (source_parts_set && !source_parts_set->empty())
