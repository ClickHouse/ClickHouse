...
MergeTreeDataPartWriterCompact::MergeTreeDataPartWriterCompact(
    ...
    , plain_hashing(*plain_file, settings.cryptographic_mode, settings.hash_function)
{
    ...
    marks_file_hashing = std::make_unique<AbstractHashingWriteBuffer>(*marks_file, settings.cryptographic_mode, settings.hash_function);
    ...
    if (data_part_->index_granularity_info.mark_type.compressed)
    {
        ...
        marks_source_hashing = std::make_unique<AbstractHashingWriteBuffer>(*marks_compressor, settings.cryptographic_mode, settings.hash_function);
    }
...
void MergeTreeDataPartWriterCompact::addStreams(const NameAndTypePair & column, const ASTPtr & effective_codec_desc)
{
    ISerialization::StreamCallback callback = [&](const auto & substream_path)
    {
        ...
        if (!stream)
            stream = std::make_shared<CompressedStream>(plain_hashing.getBuf(), compression_codec, settings.cryptographic_mode, settings.hash_function);
