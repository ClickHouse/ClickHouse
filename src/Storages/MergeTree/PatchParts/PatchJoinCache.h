...
/**  We use two-level hash map (_block_number -> (_block_offset -> (block_idx, row_idx))).
  ...
  */
using OffsetsHashMap = absl::flat_hash_map<UInt64, std::pair<UInt32, UInt32>, HashCRC32<UInt64>>;
using PatchHashMap = absl::node_hash_map<UInt64, OffsetsHashMap, HashCRC32<UInt64>>;
