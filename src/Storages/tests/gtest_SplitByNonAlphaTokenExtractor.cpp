...
TEST_P(SplitByNonAlphaTokenExtractorTest, next)
{
    ...
    {
        SCOPED_TRACE(++i);
        ASSERT_TRUE(token_extractor.nextInString(data->data(), data->size(), &pos, &token_start, &token_len));

        EXPECT_EQ(expected_token, std::string_view(data->data() + token_start, token_len))
                << " token_start:" << token_start << " token_len: " << token_len;
    }
    ASSERT_FALSE(token_extractor.nextInString(data->data(), data->size(), &pos, &token_start, &token_len))
