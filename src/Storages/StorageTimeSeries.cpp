...
| `samples` | `Array(Tuple(DateTime64(3), Float64))` by default | Array of (timestamp, value) pairs for a time series. In version 6 a `SELECT` returns one row per stored bucket (`SELECT ... FINAL` merges unmerged rows of the same bucket); use `timeSeriesGroupArray` to merge them into one array. The tuple's timestamp and scalar element types can be derived from the samples `INNER COLUMNS` declaration (see [Specifying outer columns](#specifying-outer-columns)). The column is named `time_series` in tables of [version](#schema-versioning) 2 and earlier |
