...
```sql
CREATE TABLE my_table ENGINE=TimeSeries
SAMPLES INNER COLUMNS (timestamp UInt32 CODEC(Delta, T64, ZSTD(3)), value Float32 CODEC(ZSTD(3)))
```
...
`timestamp CODEC(Delta, T64, ZSTD(3))` and `value CODEC(ZSTD(3))`. Near-monotonic timestamps barely
...
```sql
CREATE TABLE my_table ENGINE=TimeSeries
SAMPLES INNER COLUMNS (timestamp DateTime64(6) CODEC(Delta, T64, ZSTD(3)), value Float32 CODEC(ZSTD(3)))
```
