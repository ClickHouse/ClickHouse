...
namespace
{
    ...
    {
        bool is_inner = !target_info || target_info->table_id.empty();
        StorageID actual_table_id = StorageID::createEmpty();

        if (!is_inner)
        {
            /// A target table is specified.
            actual_table_id = target_info->table_id;

            if (mode < LoadingStrictnessLevel::ATTACH)
            {
                /// If it's not an ATTACH request then
                /// check that the specified target table has all the required columns.
                auto target_table = DatabaseCatalog::instance().getTable(actual_table_id, context);
                auto target_metadata = target_table->getInMemoryMetadataPtr();
                const auto & target_columns = target_metadata->columns;
                TimeSeriesColumnsValidator validator{actual_table_id};
                validator.validateTargetColumns(kind, target_columns, time_series_settings);
            }
        }
        else
        {
            ...
            if (mode >= LoadingStrictnessLevel::ATTACH)
            {
                ...
                actual_table_id = inner_tables_creator.getInnerTableId(kind, target_info);
            }
            else
            {
                /// Create the inner target table.
                actual_table_id = inner_tables_creator.createInnerTable(kind, target_info, context, time_series_columns, time_series_settings);
            }
        }

        return actual_table_id;
    }
}
    ...
    , WithContext(local_context->getGlobalContext())
{
    ...
    if (mode < LoadingStrictnessLevel::ATTACH)
    {
        TimeSeriesColumnsValidator validator{table_id};
        validator.validateColumns(columns, *storage_settings);
    }
    ...
    for (auto kind : {TargetKind::Data, TargetKind::Tags, TargetKind::Metrics})
    {
        ...
        if (const auto * target_info = query.tryGetTarget(kind))
        {
            if (!target_info->table_id.empty())
                target.is_inner_table = false;
            target.table_id = initTarget(kind, target_info, local_context, getStorageID(), columns, *storage_settings, mode);
        }
