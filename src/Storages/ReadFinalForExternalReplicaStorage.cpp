...
{
    const auto metadata = storage->getInMemoryMetadataQueryCached(CurrentThread::tryGetQueryContext());
    ...
    size_t num_streams)
{
    ...
    const auto nested_metadata = nested_storage->getInMemoryMetadataQueryCached(context);
