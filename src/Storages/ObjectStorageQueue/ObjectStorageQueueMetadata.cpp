    ...
    , local_file_statuses(std::make_shared<LocalFileStatuses>())
{
    ...
    {
        LOG_INFO(log, "Initializing regex-based filename parser - partition_by_regex: '{}', ordering_components_regex: '{}'",
                 table_metadata.partition_by_regex, table_metadata.ordering_components_regex);

        filename_parser = std::make_unique<ObjectStorageQueueFilenameParser>(
            table_metadata.partition_by_regex,
            table_metadata.ordering_components_regex);
