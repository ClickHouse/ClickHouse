...
#define OBJECT_STORAGE_QUEUE_RELATED_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(UInt32, persistent_processing_node_ttl_seconds, 6 * 60 * 60, "Time after which a stale processing node (per-file processing node or bucket lock) is cleaned up. Bucket locks refresh their mtime only on commit and per-file processing nodes are never refreshed, so a processor that does not commit for longer than the TTL can have its nodes reclaimed even while alive.", 0) \
