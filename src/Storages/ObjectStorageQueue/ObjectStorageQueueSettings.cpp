...
#define OBJECT_STORAGE_QUEUE_RELATED_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(ObjectStorageQueueBucketAssignmentStrategy, bucket_assignment_strategy, ObjectStorageQueueBucketAssignmentStrategy::PATH, "Bucket assignment strategy: PATH (hash full file path, default) or PARTITION (hash partition key for better locality when partitioning enabled)", 0) \
    ...
    DECLARE(ObjectStorageQueuePartitioningMode, partitioning_mode, ObjectStorageQueuePartitioningMode::NONE, "Partitioning strategy: NONE (no per-partition tracking), HIVE (extract from path structure), or REGEX (extract from filename using regex)", 0) \
    DECLARE(String, partition_by_regex, "", "Regex to extract partition key from filename (for partitioning_mode=REGEX). Use named capture group 'partition' or first capture group. Example: '^([^_]+)_.*' extracts 'hostname' from 'hostname_timestamp_seq'", 0) \
    DECLARE(String, ordering_components_regex, "", "Regex to extract ordering components from filename (for partitioning_mode=REGEX). Must have named groups 'timestamp' and 'sequence'. Example: '.*_(?P<timestamp>\\d{8}T\\d{6}\\.\\d{6}Z)_(?P<sequence>\\d+)'", 0) \
