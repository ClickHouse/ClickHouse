...
#define OBJECT_STORAGE_QUEUE_RELATED_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(Bool, after_processing_move_preserve_tags, true, "S3 only; setting it on any other object storage is an error. If true, a guarded move (after_processing_move_preserve_path = 0) restates the source object's tag set on the destination, which requires permission to read those tags; a move whose tags cannot be read fails and leaves the source in place. Set to false to move without restating tags, as before the destination-collision guard existed", 0) \
