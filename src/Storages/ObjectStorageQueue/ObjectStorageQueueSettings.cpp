...
#define OBJECT_STORAGE_QUEUE_RELATED_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(String, ordered_partition_prefix_suffix, "", "For Ordered mode with regex partitioning and partition bucketing. If non-empty, list each known partition with object-storage prefix '<queue_prefix><partition><suffix>' and resume from that partition's processed watermark. For filenames like 'hostname_nanos', use '_'.", 0) \
    DECLARE(UInt64, ordered_partition_discovery_interval_ms, 60000, "For ordered_partition_prefix_suffix. Minimum interval between full-prefix discovery scans used to find new regex partitions. Zero disables periodic discovery after the first scan.", 0) \
