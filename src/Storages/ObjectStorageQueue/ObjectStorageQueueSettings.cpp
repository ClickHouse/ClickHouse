...
#define OBJECT_STORAGE_QUEUE_RELATED_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(UInt64, failed_file_ttl_sec, 0, "Maximum number of seconds to store terminal failed files in ZooKeeper node (store forever by default). Only applies to files that have exhausted all retries; actively retrying files are preserved to maintain retry state.", 0) \
