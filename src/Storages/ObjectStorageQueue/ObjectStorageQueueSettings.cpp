...
#define OBJECT_STORAGE_QUEUE_RELATED_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(UInt32, persistent_processing_node_ttl_seconds, 6 * 60 * 60, "Time after which an abandoned processing node (per-file processing node or bucket lock) is cleaned up. Active nodes refresh their mtime on commit, so this only applies to nodes left by a crashed processor.", 0) \
