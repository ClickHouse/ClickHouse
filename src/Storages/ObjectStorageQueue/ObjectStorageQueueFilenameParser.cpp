...
std::optional<std::string> ObjectStorageQueueFilenameParser::parse(const std::string & file_name) const
{
    ...
    if (partition_pattern)
    {
        ...
        {
            LOG_ERROR(log, "partition_regex must use only named capture groups (?P<name>...). "
                           "Found {} total groups but only {} are named", num_groups, groups.size());
            return std::nullopt;
        }
        ...
        LOG_TRACE(log, "Parsed file: path={}, basename={}, partition={}", file_name, basename, partition_key);

        return partition_key;
    }
    else
    {
        // This should never be reached due to early isValid() check
        chassert(false && "parse() called without valid partition_pattern");
        return std::nullopt;
    }
}
