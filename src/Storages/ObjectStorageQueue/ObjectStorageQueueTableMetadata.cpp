...
namespace ObjectStorageQueueSetting
{
    ...
    extern const ObjectStorageQueueSettingsString partition_by_regex;
    extern const ObjectStorageQueueSettingsString ordering_components_regex;
...
ObjectStorageQueueTableMetadata::ObjectStorageQueueTableMetadata(
    ...
    , partition_by_regex(engine_settings[ObjectStorageQueueSetting::partition_by_regex])
    , ordering_components_regex(engine_settings[ObjectStorageQueueSetting::ordering_components_regex])
...
String ObjectStorageQueueTableMetadata::toString() const
{
    ...
    json.set("partition_by_regex", partition_by_regex);
    json.set("ordering_components_regex", ordering_components_regex);
...
ObjectStorageQueueTableMetadata::ObjectStorageQueueTableMetadata(const Poco::JSON::Object::Ptr & json)
    ...
    , partition_by_regex(getOrDefault<String>(json, "partition_by_regex", "", ""))
    , ordering_components_regex(getOrDefault<String>(json, "ordering_components_regex", "", ""))
...
void ObjectStorageQueueTableMetadata::checkImmutableFieldsEquals(const ObjectStorageQueueTableMetadata & from_zk) const
{
    ...
    if (partition_by_regex != from_zk.partition_by_regex)
        throw Exception(
            ErrorCodes::METADATA_MISMATCH,
            "Existing table metadata in ZooKeeper differs in partition_by_regex. "
            "Stored in ZooKeeper: {}, local: {}",
            from_zk.partition_by_regex,
            partition_by_regex);

    if (ordering_components_regex != from_zk.ordering_components_regex)
        throw Exception(
            ErrorCodes::METADATA_MISMATCH,
            "Existing table metadata in ZooKeeper differs in ordering_components_regex. "
            "Stored in ZooKeeper: {}, local: {}",
            from_zk.ordering_components_regex,
