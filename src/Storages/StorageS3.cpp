...
public:
        ...
        , file_progress_callback(file_progress_callback_)
    {
        ...
        {
            buffer.emplace_back(globbed_uri.key, S3::getObjectInfo(*client, globbed_uri.bucket, globbed_uri.key, globbed_uri.version_id, request_settings));
...
private:
    ...
    void fillInternalBufferAssumeLocked()
    {
        ...
        if (filter_ast)
        {
            ...
            auto idxs = VirtualColumnUtils::filterPaths(paths, query, virtual_columns, getContext(), filter_ast);

            buffer.reserve(idxs.size());
            for (UInt64 idx : idxs)
            {
...
public:
        ...
        , file_progress_callback(file_progress_callback_)
    {
        ...
        if (filter_ast)
        {
            ...
            auto indexes = VirtualColumnUtils::filterPaths(paths, query, virtual_columns, getContext(), filter_ast);

            Strings filtered_keys;
            filtered_keys.reserve(indexes.size());
            for (auto i : indexes)
                filtered_keys.emplace_back(keys[i]);
    ...
    KeyWithInfo next()
    {
        ...
        S3::ObjectInfo info = S3::getObjectInfo(*client, bucket, key, version_id, request_settings);
        if (file_progress_callback)
            file_progress_callback(FileProgress(0, info.size));
    ...
    public:
        ...
        std::unique_ptr<ReadBuffer> next() override
        {
            while (true)
            {
                ...
                if (getContext()->getSettingsRef().s3_skip_empty_files && current_key_with_info.info.size == 0)
                    continue;
    ...
    const ContextPtr & ctx)
{
    ...
    for (auto it = begin; it < end; ++it)
    {
        ...
        {
            return it->info.last_modification_time;
        };
