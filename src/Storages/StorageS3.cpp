    ...
    unsigned num_streams)
{
    ...
    for (const String & key : listFilesWithRegexpMatching(*client, uri))
        pipes.emplace_back(std::make_shared<StorageS3Source>(
            ...
            context,
            metadata->getColumns().getDefaults(),
            max_block_size,
...
BlockOutputStreamPtr StorageS3::write(const ASTPtr & /*query*/, const StorageMetadataPtr & metadata, const Context & /*context*/)
{
    return std::make_shared<StorageS3BlockOutputStream>(
        format_name, min_upload_part_size, metadata->getSampleBlock(), context_global,
        chooseCompressionMethod(uri.endpoint, compression_method),
