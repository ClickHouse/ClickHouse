...
StorageS3Source::DisclosedGlobIterator::DisclosedGlobIterator(
    ...
    Strings * read_keys_,
    const S3Settings::ReadWriteSettings & rw_settings_)
    : pimpl(std::make_shared<StorageS3Source::DisclosedGlobIterator::Impl>(client_, globbed_uri_, query, virtual_header, context, object_infos_, read_keys_, rw_settings_))
{
}
...
std::unique_ptr<ReadBuffer> StorageS3Source::createS3ReadBuffer(const String & key)
{
    ...
    if (!use_parallel_download || object_too_small)
    {
        ...
        return std::make_unique<ReadBufferFromS3>(client, bucket, key, version_id, rw_settings, getContext()->getReadSettings());
    }
    ...
    auto factory = std::make_unique<ReadBufferS3Factory>(
        client, bucket, key, version_id, download_buffer_size, object_size, rw_settings, getContext()->getReadSettings());
    LOG_TRACE(
    ...
    Strings * read_keys)
{
    ...
    else if (is_key_with_globs)
    {
        ...
        auto glob_iterator = std::make_shared<StorageS3Source::DisclosedGlobIterator>(
            *s3_configuration.client, s3_configuration.uri, query, virtual_block, local_context, object_infos, read_keys, s3_configuration.rw_settings);
...
void StorageS3::updateS3Configuration(ContextPtr ctx, StorageS3::S3Configuration & upd)
{
    ...
    S3::PocoHTTPClientConfiguration client_configuration = S3::ClientFactory::instance().createClientConfiguration(
        ...
        /* for_disk_s3 = */ false,
        upd.rw_settings.get_request_throttler,
        upd.rw_settings.put_request_throttler);
...
void StorageS3::processNamedCollectionResult(StorageS3Configuration & configuration, const std::vector<std::pair<String, ASTPtr>> & key_value_args)
{
    for (const auto & [arg_name, arg_value] : key_value_args)
    {
        if (arg_name == "access_key_id")
            ...
            configuration.rw_settings.min_upload_part_size = checkAndGetLiteralArgument<UInt64>(arg_value, "min_upload_part_size");
        else if (arg_name == "upload_part_size_multiply_factor")
            configuration.rw_settings.upload_part_size_multiply_factor = checkAndGetLiteralArgument<UInt64>(arg_value, "upload_part_size_multiply_factor");
        else if (arg_name == "upload_part_size_multiply_parts_count_threshold")
            configuration.rw_settings.upload_part_size_multiply_parts_count_threshold = checkAndGetLiteralArgument<UInt64>(arg_value, "upload_part_size_multiply_parts_count_threshold");
        else if (arg_name == "max_single_part_upload_size")
            configuration.rw_settings.max_single_part_upload_size = checkAndGetLiteralArgument<UInt64>(arg_value, "max_single_part_upload_size");
        else if (arg_name == "max_connections")
            configuration.rw_settings.max_connections = checkAndGetLiteralArgument<UInt64>(arg_value, "max_connections");
        else
    ...
    ReadBufferIterator read_buffer_iterator = [&, first = true](ColumnsDescription & cached_columns) mutable -> std::unique_ptr<ReadBuffer>
    {
        ...
        return wrapReadBufferWithCompressionMethod(
            std::make_unique<ReadBufferFromS3>(
                s3_configuration.client, s3_configuration.uri.bucket, key, s3_configuration.uri.version_id, s3_configuration.rw_settings, ctx->getReadSettings()),
