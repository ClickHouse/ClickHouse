...
std::optional<Float64> ColumnStatistics::estimateEqual(const Field & val) const
{
    ...
    /// `basic` holds an exact count of the column's default value (0, '', [], ...). When `val` is
    /// that default this is exact and beats every approximate sketch, so consult it first; for any
    /// other value (or a Nullable column) it returns nullopt and we fall through.
