    ...
    if (node.isFunction())
    {
        ...
        if (num_args == 2)
        {
            ...
            if (is_in_operator && !func.getArgumentAt(1).getASTNode())
            {
                ...
                /// Turning the set into ranges costs a `Field` per element, a sort, and one statistics
                /// probe per element. Above the limit that dwarfs the decision it informs, so estimate
                /// from the size of the set and its bounds instead, at a cost independent of the number
                /// of elements. The atom is finalized rather than turned into ranges: a scalar
