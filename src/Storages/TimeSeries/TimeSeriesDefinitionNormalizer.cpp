...
void TimeSeriesDefinitionNormalizer::addMissingInnerEnginesFromAsTable(ASTCreateQuery & create, const ASTCreateQuery & as_create) const
{
    for (auto kind : {TargetKind::Data, TargetKind::Tags, TargetKind::Metrics})
    {
        ...
        if (!inner_storage_def)
        {
            ...
            if (auto inner_storage_def_from_other_table = as_create.getTargetTableEngine(kind))
                create.setTargetTableEngine(kind, typeid_cast<std::shared_ptr<ASTStorage>>(inner_storage_def_from_other_table->clone()));
        }
    }
}
...
void TimeSeriesDefinitionNormalizer::addMissingInnerEngines(ASTCreateQuery & create) const
{
    for (auto kind : {TargetKind::Data, TargetKind::Tags, TargetKind::Metrics})
    {
        ...
        if (inner_storage_def && inner_storage_def->engine)
            continue; /// The inner (data/tags/metrics) ENGINE is set already, skip it.
