    ...
    ConverterContext & context)
{
    ...
    /// If the left argument is empty then the result is also empty. `unless` passes the left
    /// operand's values through, so the result keeps its value type override either way.
