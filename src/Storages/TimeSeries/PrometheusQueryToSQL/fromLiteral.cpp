...
SQLQueryPiece fromLiteral(const PQT::IntervalLiteral * interval_node, ConverterContext & context)
{
    ...
    res.scalar_value = DecimalUtils::convertTo<Float64>(interval.getValue(), interval.getScale());

    auto evaluation_range = context.node_evaluation_range_getter.get(interval_node);
    res.start_time = evaluation_range.start_time;
    res.end_time = evaluation_range.end_time;
    res.step = evaluation_range.step;
