...
enum class StoreMethod
{
    ...
    /// Data are stored in two columns `group` (UInt64), `time_series` (Array(Tuple(timestamp_data_type, scalar_data_type))).
    /// Each row contains samples of one time series sorted by timestamp, and a `group` can appear in multiple rows.
...
struct SQLQueryPiece
{
    ...
    /// If `store_method` is RAW_DATA then the SELECT query outputs two columns `group` (UInt64), `time_series` (Array(Tuple(timestamp_data_type, scalar_data_type))).
