...
enum class StoreMethod
{
    ...
    /// `time_series` (Array(Tuple(timestamp_data_type, scalar_data_type))). For older row-layout tables they are
    /// stored in three columns `group`, `timestamp`, and `value`. A `group` can appear in multiple rows.
