...
{
    /// The column of the inner metric families table with the name of a metric family is named `metric_family` from version 5
    ...
    const String stored_definition_of_version_4 = normalizeNewTable("CREATE TABLE db.ts ENGINE = TimeSeries SETTINGS version = 4");
    EXPECT_EQ(extractInnerColumns(stored_definition_of_version_4, "METRIC FAMILIES"), old_columns);
    EXPECT_EQ(extractInnerEngine(stored_definition_of_version_4, "METRIC FAMILIES"), "ReplacingMergeTree ORDER BY metric_family_name");
    EXPECT_EQ(normalizeExistingTable(stored_definition_of_version_4), stored_definition_of_version_4);
    ...
    EXPECT_EQ(getExceptionCode([]
    {
        normalizeNewTable("CREATE TABLE db.ts ENGINE = TimeSeries SETTINGS version = 4 METRIC FAMILIES INNER COLUMNS (metric_family LowCardinality(String))");
    }), ErrorCodes::INCORRECT_QUERY);
    ...
    EXPECT_TRUE(normalizeNewTable("CREATE TABLE db.ts ENGINE = TimeSeries SETTINGS version = 4 METRIC FAMILIES db.ext_metrics", params).contains("db.ext_metrics"));
    ...
    auto create_with_old_version = [&] { normalizeNewTable("CREATE TABLE db.ts ENGINE = TimeSeries SETTINGS version = 4 METRIC FAMILIES db.ext_metrics", params); };
    ...
    auto definition = normalizeNewTableAs("CREATE TABLE db.copy AS db.src ENGINE = TimeSeries",
        normalizeNewTable("CREATE TABLE db.src ENGINE = TimeSeries SETTINGS version = 4 METRIC FAMILIES INNER COLUMNS (metric_family_name LowCardinality(String)) "
    ...
    definition = normalizeNewTableAs("CREATE TABLE db.copy AS db.src ENGINE = TimeSeries SETTINGS version = 4",
