...
PrometheusQueryProtocol::PrometheusQueryProtocol(ConstStoragePtr time_series_storage_, const ContextPtr & context_)
    : WithContext{context_}
    , time_series_storage(storagePtrToTimeSeries(time_series_storage_))
    , log(getLogger("PrometheusQueryProtocol"))
{
}
...
void PrometheusQueryProtocol::executeRangeQuery(
    WriteBuffer & response,
    const String & promql_query,
    ...
    const String & step_param)
{
    try
    {
        ...
        // Wrap the query with a subquery
        auto wrapped_query_tree = wrapWithSubquery(*query_tree, start_time, end_time, step);

        LOG_DEBUG(log, "Wrapped query with Subquery node. New result type: {}", wrapped_query_tree->getResultType());
...
void PrometheusQueryProtocol::getSeries(
    WriteBuffer & response,
    const String & /* match_param */,
    const String & /* start_param */,
    const String & /* end_param */)
{
    // Simplified implementation - returns empty successful response
    writeString(R"({"status":"success","data":[]})", response);
}

void PrometheusQueryProtocol::getLabels(
    WriteBuffer & response,
    const String & /* match_param */,
    const String & /* start_param */,
    const String & /* end_param */)
{
    // Simplified implementation - returns basic labels
    writeString(R"({"status":"success","data":["__name__","job","instance"]})", response);
}

void PrometheusQueryProtocol::getLabelValues(
    WriteBuffer & response,
...
Field PrometheusQueryProtocol::parseTimestamp(const String & time_param)
{
    if (time_param.empty())
...
Field PrometheusQueryProtocol::parseStep(const String & step_param)
{
    if (step_param.empty())
...
std::unique_ptr<PrometheusQueryTree> PrometheusQueryProtocol::parsePromQLQuery(const String & promql_query)
{
    try
    {
...
void DB::PrometheusQueryProtocol::writeInstantQueryResponse(WriteBuffer & response, const Block & result_block)
{
...
void DB::PrometheusQueryProtocol::writeScalarResult(WriteBuffer & response, const Block & result_block)
{
    ...
    if (!result_block.empty() && result_block.rows() > 0)
    {
        ...
        if (result_block.has(TimeSeriesColumnNames::Scalar))
        {
            ...
            writeString("\"", response);
            writeString(std::to_string(value), response);
            writeString("\"", response);
        }
        else
        {
...
void DB::PrometheusQueryProtocol::writeVectorResult(WriteBuffer & response, const Block & result_block)
{
    writeString("[", response);
...
void DB::PrometheusQueryProtocol::writeMetricLabels(WriteBuffer & response, const Block & result_block, size_t row_index)
{
    writeString("{", response);
...
void DB::PrometheusQueryProtocol::writeRangeQueryHeader(WriteBuffer & response)
{
    writeString(R"({"status":"success","data":{"resultType":"matrix","result":[)", response);
}

void DB::PrometheusQueryProtocol::writeRangeQueryFooter(WriteBuffer & response)
{
            ...
            if (const auto * array_column = typeid_cast<const ColumnArray *>(ts_column.get()))
            {
                ...
                if (const auto * tuple_column = typeid_cast<const ColumnTuple *>(&array_column->getData()))
                {
                    ...
                    for (size_t j = start; j < end; ++j)
                    {
                        ...
                        writeString("[", response);
                        writeString(std::to_string(timestamp_column.getFloat64(j)), response);
                        writeString(",\"", response);
                        writeString(std::to_string(value_column.getFloat64(j)), response);
                        writeString("\"]", response);
                    }
                }
            }
...
void DB::PrometheusQueryProtocol::writeSeriesResponse(WriteBuffer & response, const Block & /* result_block */)
{
    writeString(R"({"status":"success","data":[]})", response);
}

void DB::PrometheusQueryProtocol::writeLabelsResponse(WriteBuffer & response, const Block & /* result_block */)
{
    writeString(R"({"status":"success","data":["__name__","job","instance"]})", response);
}

void DB::PrometheusQueryProtocol::writeLabelValuesResponse(WriteBuffer & response, const Block & /* result_block */)
{
    writeString(R"({"status":"success","data":[]})", response);
}

void DB::PrometheusQueryProtocol::writeErrorResponse(WriteBuffer & response, const String & error_type, const String & error_message)
{
