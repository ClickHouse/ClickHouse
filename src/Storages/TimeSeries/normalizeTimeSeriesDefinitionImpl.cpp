        ...
        switch (inner_table_kind)
        {
            ...
            case ViewTarget::RecentSamples:
            {
                ...
                if (name == TimeSeriesColumnNames::Timestamp)
                {
                    ...
                    return (codec_name == "CODEC(DoubleDelta, ZSTD(1))") || (codec_name == "CODEC(T64, ZSTD(3))");
                }
        ...
        const StorageID & table_id)
    {
        ...
        switch (inner_table_kind)
        {
            ...
            case ViewTarget::RecentSamples:
            {
                ...
                /// Generated `timestamp` columns use `T64` followed by `ZSTD(3)`.
                /// Generated `value` columns use `ZSTD(3)`.
                ...
                if (auto * timestamp_decl = add_column_if_missing(TimeSeriesColumnNames::Timestamp, dataTypeToAST(resolved_types.timestamp_type)))
                    timestamp_decl->setCodec(makeASTFunction(
                        "CODEC", make_intrusive<ASTIdentifier>("T64"), makeASTFunction("ZSTD", make_intrusive<ASTLiteral>(UInt64{3}))));
