...
namespace
{
    ...
    /// `timestamp_type`, `scalar_type`, `id_type`. Bucketed tables declare the timestamp and value types
    /// in `samples`; older row-layout tables declare them in `timestamp` and `value`.
    ...
    bool isGeneratedInnerColumn(const ASTColumnDeclaration & column, ViewTarget::Kind inner_table_kind, const TimeSeriesSettings & settings)
    {
        ...
        switch (inner_table_kind)
        {
            ...
            case ViewTarget::MetricFamilies:
            {
                ...
                if ((name == TimeSeriesColumnNames::getInnerMetricFamily(settings[TimeSeriesSetting::version]))
    ...
    void removeInnerEngineKeysDisabledByNewSettings(
        ASTStorage & inner_engine, ViewTarget::Kind inner_table_kind, const TimeSeriesSettings & new_settings)
    {
        if (inner_table_kind != ViewTarget::MetricFamilies)
            return;
        ...
        const Settings & query_settings)
    {
        ...
        /// The `*_index_granularity` and `*_index_granularity_bytes` settings set the same-named settings
        /// of the inner MergeTree tables, overriding the engine declaration.
        ...
        switch (inner_table_kind)
        {
            ...
            case ViewTarget::RecentSamples:
            {
                ...
                else if (!has_partition_by())
                {
                    ...
                    ASTPtr time_column = has_bucketed_samples
            ...
            case ViewTarget::MetricFamilies:
            {
                ...
                if (needs_sorting_key())
                    set_sorting_key({make_intrusive<ASTIdentifier>(
                        TimeSeriesColumnNames::getInnerMetricFamily(settings[TimeSeriesSetting::version]))});
                break;
            }
        ...
        bool require_aggregating_samples_column = false)
    {
        ...
        switch (target_kind)
        {
            ...
            case ViewTarget::MetricFamilies:
            {
                check_metric_family_column(
                    TimeSeriesColumnNames::getInnerMetricFamily(time_series_settings[TimeSeriesSetting::version]));
...
void normalizeTimeSeriesDefinitionImpl(ASTCreateQuery & create_query, const NormalizeTimeSeriesDefinitionParams & params)
{
    ...
    if (is_new_table)
    {
        ...
        /// Bucket alignment is part of the v6+ physical layout, so pin the effective steps in the definition.
