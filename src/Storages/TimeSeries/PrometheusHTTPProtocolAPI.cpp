    ...
    QueryFinishCallback query_finish_callback)
{
    ...
    /// Do not push a finite limit into SQL. Row validation must run for every matching series, including
    /// rows beyond the response prefix, so a malformed row cannot be hidden by the caller's limit.
    ...
    try
    {
        ...
        /// name in the sorted generated series-label array mean the row stores conflicting values for one
        ...
        bool truncated = false;
        bool first_row = true;
        auto write_row = [&](const auto & metric_name_column,
                             const auto & offsets,
                             const auto & key_column,
                             const auto & value_column,
                             size_t i)
        {
            ...
            writeJSONString(metric_name_column->getDataAt(i), response, format_settings);
        ...
        while (executor.pull(result_block))
        {
            ...
            const auto & metric_name_column = result_block.getByName(TimeSeriesColumnNames::MetricName).column;
            const auto & array_column = typeid_cast<const ColumnArray &>(*result_block.getByName(series_labels_alias).column);
            const auto & offsets = array_column.getOffsets();
            const auto & tuple_column = typeid_cast<const ColumnTuple &>(array_column.getData());
            ...
            for (size_t i = 0; i < result_block.rows(); ++i)
            {
                /// Validate every row, including rows that will not be emitted because of the response limit.
                ...
                write_row(metric_name_column, offsets, key_column, value_column, i);
                ++emitted;
            }
        }
        ...
        /// Store the complete result in the query result cache now (no-op if no cache writers exist in the pipeline):
        /// the executor's destructor cancels the pipeline processors, after which the pending write would be discarded.
        if (!truncated)
            io.pipeline.finalizeWriteInQueryResultCache();
    }
...
/// Includes "__name__" as a virtual label whenever at least one series matches, then queries distinct keys
/// from the tags Map column.
