...
struct TimeSeriesColumnNames
{
    ...
    /// Array of (timestamp, value) pairs of a time series. It's an outer column of a TimeSeries table,
    /// and also a column returned by the table functions prometheusQuery() and prometheusQueryRange()
    /// (they can also output columns `tags`, `value`, and `timestamp`).
    ...
    /// The old name of the "samples" outer column, used by tables of versions 0 and 1.
    /// It's still accepted in a CREATE query, the column is regenerated under the new name.
