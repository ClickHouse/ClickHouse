...
#define LIST_OF_TIME_SERIES_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(UInt64, recent_samples_ttl_seconds, 345600, "Retention of the additional 'recent samples' target table, which every inserted sample is written to as well. TimeSeries drops inner recent-samples partitions whose maximum stored timestamp is older than this interval relative to the newest stored sample. An external recent samples table must retain at least this many seconds of data, which is the user's responsibility. Queries whose time range fits in the retained window prefer the recent samples table to the main samples table (see the query-level setting 'time_series_prefer_recent_samples_table'). The default is 4 days; set to 0 to disable the recent samples table", 0) \
    DECLARE(ASTFunction, recent_samples_partition_by, String{}, "Partition key of the inner 'recent samples' table, for example 'toStartOfHour(timestamp)'. If not set, 'toStartOfInterval(toDateTime(timestamp), toIntervalHour(5))' is used. Requires 'recent_samples_ttl_seconds' to be non-zero", 0) \
    DECLARE(UInt64, recent_samples_index_granularity, 8192, "Sets 'index_granularity' of the inner 'recent samples' table. Requires 'recent_samples_ttl_seconds' to be non-zero", 0) \
