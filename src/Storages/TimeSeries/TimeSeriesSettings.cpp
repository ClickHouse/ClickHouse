...
#define LIST_OF_TIME_SERIES_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(UInt64, tags_deduplication_cache_expiration_seconds, 3600, "Time after which an entry of the deduplication cache of the 'tags' table expires, counted from the moment the time series was written. Every time series is written again at least once per this period, so any difference between the cache and the table disappears within it. The cache is local to the server: it's cleared by 'TRUNCATE TABLE' and by 'SYSTEM DROP TIME SERIES CACHES'. Set to 0 to disable the cache", 0) \
    DECLARE(UInt64, tags_deduplication_cache_size_bytes, 104857600, "Maximum size in bytes of the deduplication cache of the 'tags' table. The cache stores the hashes of the time series recently written to the table, so the same tags aren't written again with every insert; the oldest entries are evicted when the cache is full. The cache is used only when 'store_min_time_and_max_time' is disabled, because otherwise every insert changes 'min_time' and 'max_time'; a non-zero value can't be set for a table storing these columns. Set to 0 to disable the cache, see also 'tags_deduplication_cache_expiration_seconds'", 0) \
    DECLARE(UInt64, metric_families_deduplication_cache_expiration_seconds, 3600, "Time after which an entry of the deduplication cache of the 'metric families' table expires, counted from the moment the metric family was written. Every metric family is written again at least once per this period, so any difference between the cache and the table disappears within it. The cache is local to the server: it's cleared by 'TRUNCATE TABLE' and by 'SYSTEM DROP TIME SERIES CACHES'. Set to 0 to disable the cache", 0) \
    DECLARE(UInt64, metric_families_deduplication_cache_size_bytes, 10485760, "Maximum size in bytes of the deduplication cache of the 'metric families' table. The cache remembers the latest description of each metric family written recently, so the same descriptions aren't written again with every insert; the oldest entries are evicted when the cache is full. Set to 0 to disable the cache, see also 'metric_families_deduplication_cache_expiration_seconds'", 0) \
