...
#define LIST_OF_TIME_SERIES_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(UInt64, tags_cache_max_series, 1000000, "Maximum number of active time series IDs to keep in the in-memory tags cache to avoid redundant inserts into the inner 'tags' table. Set to 0 to disable the cache. Requires 'store_min_time_and_max_time' to be 0: a skipped insert also skips the 'min_time' and 'max_time' the batch would have contributed, which a query uses to find the series.", 0) \
    DECLARE(UInt64, tags_cache_ttl_seconds, 1800, "Interval in seconds after which the tags of an active series are inserted again rather than skipped. Set to 0 to keep skipping them for as long as the series stays in the cache. Ignored while the cache is inactive.", 0) \
