...
#define LIST_OF_TIME_SERIES_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(UInt64, samples_index_granularity_bytes, 262144, "Sets 'index_granularity_bytes' of the inner 'samples' table, which makes the size of granules adapt to the width of a sample row. When set explicitly, it overrides 'index_granularity_bytes' from the engine declaration; otherwise the default is applied only if the granularity of the samples table is not customized in any other way (neither 'samples_index_granularity' nor 'index_granularity' in the engine declaration is set explicitly). Set to 0 to disable the size-based limit. Ignored for an external samples table and a non-MergeTree engine", 0) \
