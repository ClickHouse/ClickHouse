...
#define LIST_OF_TIME_SERIES_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(UInt64, tags_cache_max_series, 0, "Maximum number of active time series IDs to keep in the in-memory tags cache to avoid redundant inserts into the inner 'tags' table. Set to 0 to disable the cache. When storing time bounds, the cache requires version 6 or newer so those bounds remain current in the separate 'tags min max' table.", 0) \
