...
#define LIST_OF_TIME_SERIES_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(UInt64, tags_cache_max_series, 1000000, "Maximum number of active time series IDs to keep in the in-memory tags cache to avoid redundant inserts into the inner 'tags' table. Set to 0 to disable the cache.", 0) \
    DECLARE(UInt64, tags_cache_ttl_seconds, 1800, "Interval in seconds to re-insert tags for active series to refresh min_time and max_time bounds. Set to 0 to disable TTL refresh.", 0) \
