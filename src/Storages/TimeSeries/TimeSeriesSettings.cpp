...
#define LIST_OF_TIME_SERIES_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(UInt64, samples_index_granularity, 8192, "Sets 'index_granularity' of the inner 'samples' table. When set explicitly, it overrides 'index_granularity' from the engine declaration. Ignored for an external samples table and a non-MergeTree engine", 0) \
