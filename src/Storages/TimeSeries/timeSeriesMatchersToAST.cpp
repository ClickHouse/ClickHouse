...
/// Map lookups return the first value for a duplicate key. Validate conflicting duplicate keys before
/// evaluating a matcher that reads the Map, otherwise a selector for a later value silently drops the row
/// and the metadata validation in the SELECT list never sees it.
ASTPtr makeMapDuplicateKeyCheck()
{
    auto make_map_pairs = []
    {
        return makeASTFunction(
            ...
            makeASTFunction("mapValues", make_intrusive<ASTIdentifier>(TimeSeriesColumnNames::Tags)));
    };

    auto make_normalized_map_pairs = [&]
    {
        /// Empty values are absent labels and are removed by the metadata normalization as well.
        return makeASTFunction(
            "arrayDistinct",
            makeASTFunction(
                "arrayFilter",
                makeASTLambda(
                    ...
                    makeASTFunction(
                        ...
                        makeASTFunction(
                            "notEquals",
                            makeASTFunction("tupleElement", make_intrusive<ASTIdentifier>("x"), make_intrusive<ASTLiteral>(UInt64{2})),
                            make_intrusive<ASTLiteral>("")),
                        makeASTFunction(
                            "equals",
                            makeASTFunction("tupleElement", make_intrusive<ASTIdentifier>("x"), make_intrusive<ASTLiteral>(UInt64{1})),
                            make_intrusive<ASTLiteral>("")))),
                make_map_pairs()));
    };

    auto normalized_map_pairs = make_normalized_map_pairs();
    auto distinct_map_label_names = makeASTFunction(
        "arrayDistinct",
        makeASTFunction(
            "arrayMap",
            makeASTLambda(
                {"x"},
                makeASTFunction("tupleElement", make_intrusive<ASTIdentifier>("x"), make_intrusive<ASTLiteral>(UInt64{1}))),
            make_normalized_map_pairs()));
    auto has_conflicting_duplicate_keys = makeASTFunction(
        "notEquals",
        makeASTFunction("length", std::move(normalized_map_pairs)),
        makeASTFunction("length", std::move(distinct_map_label_names)));
    auto throw_if_invalid = makeASTFunction(
        "throwIf",
        std::move(has_conflicting_duplicate_keys),
        make_intrusive<ASTLiteral>("Found two labels with the same name but different values in a row of the 'tags' table"));
    return makeASTFunction("equals", std::move(throw_if_invalid), make_intrusive<ASTLiteral>(UInt64{0}));
}
...
ASTPtr timeSeriesTagNameToAST(const String & tag_name, const std::unordered_map<String, String> & column_name_by_tag_name)
{
    ...
    auto make_map_value = [&]
    {
        return makeASTFunction(
            "if",
            makeMapDuplicateKeyCheck(),
            makeASTFunction("arrayElement", make_intrusive<ASTIdentifier>(TimeSeriesColumnNames::Tags), make_intrusive<ASTLiteral>(tag_name)),
            make_intrusive<ASTLiteral>(""));
    };
    ...
    /// use the dedicated column when it is non-empty and fall back to the Map otherwise. Carrier conflict
    /// validation is deliberately kept out of this expression because it is also used in matcher predicates,
    /// where an exception could be evaluated for a row that another matcher would have excluded.
    return makeASTFunction(
        "if",
