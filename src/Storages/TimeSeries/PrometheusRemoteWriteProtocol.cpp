...
namespace
{
    using TargetKind = StorageTimeSeries::TargetKind;
    ...
    const ColumnDescription & getInsertableColumnDescription(const StorageID & time_series_storage_id, const ColumnsDescription & columns, const String & column_name)
    {
    ...
    IColumn & calculateId(const ColumnDescription & id_column_description, Block & tags_block, const ContextPtr & context)
    {
    ...
    {
        std::vector<std::pair<TargetKind, Block>> blocks;
    };
                            ...
                            const TimeSeriesSettings & time_series_settings)
    {
        ...
        {
            return getInsertableColumnDescription(time_series_storage_id, columns_description, column_name);
        };
        ...
        auto & id_column_in_tags_table = calculateId(columns_description.get(TimeSeriesColumnNames::ID), tags_block, context);
        ...
        res.blocks.emplace_back(TargetKind::Tags, std::move(tags_block));
        res.blocks.emplace_back(TargetKind::Data, std::move(data_block));
                            ...
                            const StorageInMemoryMetadata & time_series_storage_metadata)
    {
        ...
        /// Prepare a result.
        BlocksToInsert res;
        res.blocks.emplace_back(TargetKind::Metrics, std::move(block));
        return res;
    }
