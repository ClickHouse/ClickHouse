...
std::pair<DataTypePtr, DataTypePtr> splitTimeSeriesType(const DataTypePtr & time_series_type)
{
    ...
    if (!array_type)
        throw Exception(ErrorCodes::BAD_TYPE_OF_FIELD,
            "Expected `Array(Tuple(timestamp, value))` as the type of the column with samples, got {}", time_series_type->getName());
    ...
    if (!tuple_type || (tuple_type->getElements().size() != 2))
        throw Exception(ErrorCodes::BAD_TYPE_OF_FIELD,
            "Expected `Tuple(timestamp, value)` as the element type of the column with samples, got {}",
