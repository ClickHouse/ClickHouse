...
struct FilesystemCacheSettings
{
    ...
    /// Emit a TEST-level message per buffer refill. Off by default: the message is formatted once
    /// per read rather than once per file segment, so it costs the reading thread proportionally.
