...
void RequestSettings::finishInit(const DB::Settings & settings, bool validate_settings)
{
    ...
    if (UInt64 max_get_rps = isChanged("max_get_rps") ? get("max_get_rps").safeGet<UInt64>() : settings[s3_max_get_rps])
    {
        size_t default_max_get_burst
            = settings[s3_max_get_burst] ? settings[s3_max_get_burst] : (Throttler::default_burst_seconds * max_get_rps);
    ...
    if (UInt64 max_put_rps = isChanged("max_put_rps") ? get("max_put_rps").safeGet<UInt64>() : settings[s3_max_put_rps])
    {
        size_t default_max_put_burst
            = settings[s3_max_put_burst] ? settings[s3_max_put_burst] : (Throttler::default_burst_seconds * max_put_rps);
