...
ReadSettings ReadSettings::adjustBufferSize(size_t file_size) const
{
    ...
    res.local_fs_settings.buffer_size = std::min(std::max(1uz, file_size), local_fs_settings.buffer_size);
    res.remote_fs_settings.buffer_size = std::min(std::max(1uz, file_size), remote_fs_settings.buffer_size);
