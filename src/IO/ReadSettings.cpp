...
ReadSettings getReadSettingsForMetadata()
{
    ...
    read_settings.local_fs_settings.local_fs_method = LocalFSReadMethod::read;
    ...
    read_settings.local_fs_settings.local_fs_buffer_size = METADATA_FILE_BUFFER_SIZE;
...
ReadSettings ReadSettings::adjustBufferSize(size_t file_size) const
{
    ...
    res.local_fs_settings.local_fs_buffer_size = std::min(std::max(1ul, file_size), local_fs_settings.local_fs_buffer_size);
