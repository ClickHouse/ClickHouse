...
/// Write a string literal accepted by MySQL without changing any of its bytes.
/// MySQL does not recognize all of the backslash escapes used by ClickHouse, for example `\f`.
/// Hexadecimal literals are used for values containing control, non-ASCII, or backslash bytes.
inline void writeMySQLStringLiteral(std::string_view ref, WriteBuffer & buf)
{
    bool use_hex = false;
    for (const char ch : ref)
    {
        const UInt8 c = static_cast<UInt8>(static_cast<unsigned char>(ch));
        if (c < 0x20 || c >= 0x7F || c == '\\')
        {
            use_hex = true;
            break;
    ...
    if (use_hex)
    {
        ...
        {
            const UInt8 c = static_cast<UInt8>(static_cast<unsigned char>(ch));
            writeChar(hex_digits[c >> 4], buf);
            writeChar(hex_digits[c & 0x0F], buf);
        }
        writeChar('\'', buf);
        return;
    }
...
inline void writeStringForValues(std::string_view ref, WriteBuffer & buf, const FormatSettings & settings)
{
    if (settings.values.use_mysql_compatible_escaping)
    {
        writeMySQLStringLiteral(ref, buf);
        return;
    }
