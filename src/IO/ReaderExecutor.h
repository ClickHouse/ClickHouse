...
private:
    ...
    /// `stop` (nullable) is polled between blocks (see `fetchGapsFromSource`).
    Rope readFromSource(
    ...
    /// for fills, `BytesPromoted` for promotes). `abort` (nullable) is polled
    /// between writers - the put step's stop point (only the unconditional
