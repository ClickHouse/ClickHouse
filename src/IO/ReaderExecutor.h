...
private:
    ...
    /// cache-ALIGNED miss ranges (the gap-fetch + write targets). Holds NO
    /// cache buffer - those live in the same plan's `ReadPlan::bufs`.
    /// `aligned_miss` is clamped only to the object end (it may extend past
    /// the plan span so a touched cell is fully populated).
    ...
    /// The IMMUTABLE geometry of one look-ahead plan: the resident layout +
    /// span, queried positionally (RESIDENT run vs GAP). Built once by
    /// `planResidencyWindow`, never mutated after publish, exposed as a
    /// `shared_ptr<const>` snapshot via `ReadPlan::geometry()`. `entries` is
    /// in cache-tier priority order, 1:1 POSITIONAL with `ReadPlan::bufs`.
    /// Empty / `plan_end == plan_start` means no valid plan.
    struct ReadPlanGeometry
    {
        static constexpr size_t npos = static_cast<size_t>(-1);

        /// What the plan holds at an offset: the index of the entry whose
        /// resident range covers it (`npos` = gap), its tier, and the end of
        /// that contiguous resident run.
        struct Resident
        {
            size_t entry = npos;
            CacheTier tier{};
        ...
        /// First non-resident offset at or after `from`; `plan_end` when
        /// everything to the plan end is resident.
        size_t nextGapStart(size_t from) const;

        /// End of the gap starting at `gap_start`: the next resident range's
        /// start, or `plan_end`.
        size_t gapEnd(size_t gap_start) const;

        /// The window to FETCH to serve `req`: `req` rounded OUT to the cache
        /// cell at each edge (it may start LEFT of `req.offset` and end past
        /// `req.end()`; the caller slices back). The widening is BOUNDED by
        /// each tier's alignment grid, NOT the coalesced miss run, and is
        /// clamped into the run so it never reaches resident bytes.
        ByteRange fetchWindowAt(ByteRange req) const;

        /// How far a live connection opened at `from` would stream before it
        /// must reopen: gaps plus resident runs no larger than `min_gap`
        /// (bridged), stopping at the first larger one or `plan_end`. A reach
        /// no larger than a window means a one-shot serves it and no lease is
        /// warranted.
        ...
        /// `MemoryPressureMonitor` level sampled ONCE at plan build; reads
        /// within the plan use it instead of re-querying per call.
