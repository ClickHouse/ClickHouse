...
ConnectionTimeouts ConnectionTimeouts::getTCPTimeoutsWithoutFailover(const Settings & settings)
{
    return ConnectionTimeouts()
        .withConnectionTimeout(settings[DB::connect_timeout])
        .withSendTimeout(settings[DB::send_timeout])
        .withReceiveTimeout(settings[DB::receive_timeout])
        .withTCPKeepAliveTimeout(settings[DB::tcp_keep_alive_timeout])
        .withHandshakeTimeout(settings[DB::handshake_timeout_ms])
        .withHedgedConnectionTimeout(settings[DB::hedged_connection_timeout_ms])
        .withReceiveDataTimeout(settings[DB::receive_data_timeout_ms]);
}
