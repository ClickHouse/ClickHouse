...
void ProgressValues::read(ReadBuffer & in, UInt64 server_revision)
{
    ...
    size_t new_cumulative_processing_time = 0;
    ...
    if (server_revision >= DBMS_MIN_REVISION_WITH_CLIENT_PROCESSING_TIME)
    {
        readVarUInt(new_cumulative_processing_time, in);
    }
    ...
    this->cumulative_processing_time = new_cumulative_processing_time;
}
...
void ProgressValues::write(WriteBuffer & out, UInt64 client_revision) const
{
    ...
    if (client_revision >= DBMS_MIN_REVISION_WITH_CLIENT_PROCESSING_TIME)
    {
        writeVarUInt(this->cumulative_processing_time, out);
    }
}
...
void ProgressValues::writeJSON(WriteBuffer & out) const
{
    ...
    writeCString("\",\"cumulative_processing_time\":\"", out);
    writeText(this->cumulative_processing_time, out);
    writeCString("\"}", out);
}
...
void Progress::read(ReadBuffer & in, UInt64 server_revision)
{
    ...
    cumulative_processing_time.store(values.cumulative_processing_time, std::memory_order_relaxed);
}
