...
/// Like `boundRecordingCreator`, but the buffer also advertises right-bounded reads,
/// so a test can assert a non-live source read is issued as a bounded range.
...
public:
    BoundRecordingReadBuffer(String data_, std::shared_ptr<std::optional<size_t>> recorded_)
        : TestReadBuffer(std::move(data_), std::move(recorded_)) {}

    bool supportsRightBoundedReads() const override { return true; }
};

ReadPipeline::BufferCreator rightBoundedRecordingCreator(const std::string & data, const std::shared_ptr<std::optional<size_t>> & recorded)
{
...
try
{
    ...
    ReadPipeline pipeline;
    pipeline.setSource(
        rightBoundedRecordingCreator(data, recorded),
        StoredObjects{testObject(data.size())},
        rs);
