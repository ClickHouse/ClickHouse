...
std::future<IAsynchronousReader::Result> AsynchronousReadBufferFromFileDescriptor::asyncReadInto(char * data, size_t size, Priority priority)
{
    ...
    {
        return std::async(std::launch::deferred, [offset = request.offset] { return IAsynchronousReader::Result{ .buf = nullptr, .size = 0, .offset = 0, .file_offset_of_buffer_end = offset }; });
    }
