/// The cuDF side of the GPU aggregation: everything here is compiled by nvcc's host pass against
/// the system libstdc++, not by ClickHouse's own toolchain, and it must not include a ClickHouse
/// header. It talks to the rest of the server only through the C boundary declared in the header
/// below - see that file, and `gpu_island_target` in cmake/cuda.cmake.
