...
RestoreCoordinationOnCluster::RestoreCoordinationOnCluster(
    ...
    , stage_sync(/* is_restore = */ true, zkutil::joinZooKeeperPath(zookeeper_path, "stage"), current_host, all_hosts, allow_concurrent_restore_, concurrency_counters_, with_retries, schedule_, process_list_element_, log)
{
...
bool RestoreCoordinationOnCluster::acquireCreatingSharedDatabase(const String & database_name)
{
    ...
    holder.retries_ctl.retryLoop(
        [&, &zk = holder.faulty_zookeeper]()
        {
            ...
            String path = zkutil::joinZooKeeperPath(zookeeper_path, "shared_databases_acquired", escapeForFileName(database_name));
...
bool RestoreCoordinationOnCluster::acquireInsertingDataForKeeperMap(const String & root_zk_path, const String & table_unique_id)
{
    ...
    holder.retries_ctl.retryLoop(
        [&, &zk = holder.faulty_zookeeper]()
        {
            ...
            std::string restore_lock_path = zkutil::joinZooKeeperPath(zookeeper_path, "keeper_map_tables", escapeForFileName(normalized_root_zk_path));
...
void RestoreCoordinationOnCluster::addRocksDBTable(const String & rocksdb_dir, const String & election_id)
{
    ...
    holder.retries_ctl.retryLoop(
        [&, &zk = holder.faulty_zookeeper]()
        {
            ...
            std::string dir_path = zkutil::joinZooKeeperPath(zookeeper_path, "rocksdb_tables", dir_key);
            zk->createIfNotExists(dir_path, "");
            std::string election_path = zkutil::joinZooKeeperPath(dir_path, escapeForFileName(election_id));
