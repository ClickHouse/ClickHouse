...
void BackupImpl::writeBackupMetadata()
{
    ...
    addFileImpl(".backup", read_buffer);
}
...
BackupEntryPtr BackupImpl::readFile(const String & file_name) const
{
    ...
    if (!info.base_size)
    {
        ...
        return std::make_unique<BackupEntryFromCallback>(readFileImpl(file_name), info.size, info.checksum);
    }
    ...
    return std::make_unique<BackupEntryConcat>(
        std::move(base_entry),
        std::make_unique<BackupEntryFromCallback>(readFileImpl(file_name), info.size - info.base_size),
        info.checksum);
}
...
void BackupImpl::addFile(const String & file_name, BackupEntryPtr entry)
{
...

    {
        ...
        addFileImpl(file_name, *maybe_hashing_read_buffer);
