...
BackupCoordinationOnCluster::BackupCoordinationOnCluster(
    ...
    , stage_sync(/* is_restore = */ false, zkutil::joinZooKeeperPath(zookeeper_path, "stage"), current_host, all_hosts, allow_concurrent_backup_, concurrency_counters_, with_retries, schedule_, process_list_element_, log)
{
...
void BackupCoordinationOnCluster::prepareKeeperMapTables() const
{
        ...
        [&, &zk = holder.faulty_zookeeper]()
    {
        ...
        const String tables_path = zkutil::joinZooKeeperPath(zookeeper_path, "keeper_map_tables");

        auto tables = zk->getChildren(tables_path);
        keeper_map_table_infos.reserve(tables.size());

        for (auto & table : tables)
            table = zkutil::joinZooKeeperPath(tables_path, table);
        ...
        for (size_t i = 0; i < tables_info.size(); ++i)
        {
            ...
            keeper_map_table_infos.emplace_back(
                ...
                BackupCoordinationKeeperMapTables::KeeperMapTableInfo{
                    .table_id = zkutil::zooKeeperNodeName(tables[i]), .data_path_in_backup = std::move(data[1])});
        }
    });
...
void BackupCoordinationOnCluster::prepareRocksDBTables() const
{
        ...
        [&, &zk = holder.faulty_zookeeper]()
    {
        ...
        const String tables_path = zkutil::joinZooKeeperPath(zookeeper_path, "rocksdb_tables");
        ...
        for (const auto & escaped_dir_key : zk->getChildren(tables_path))
        {
            ...
            const String dir_path = zkutil::joinZooKeeperPath(tables_path, escaped_dir_key);
            ...
            for (const auto & child : election_children)
                election_paths.push_back(zkutil::joinZooKeeperPath(dir_path, child));
            ...
            for (size_t i = 0; i < election_info.size(); ++i)
            {
                ...
                rocksdb_table_infos.emplace_back(
                    qualified_dir,
                    BackupCoordinationKeeperMapTables::KeeperMapTableInfo{
                        .table_id = unescapeForFileName(zkutil::zooKeeperNodeName(election_paths[i])),
