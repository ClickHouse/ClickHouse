...
void ClientBase::adjustSettings()
{
    ...
    if (!pager.empty())
    {
        ...
        if (!global_context->getSettingsRef().output_format_pretty_row_numbers.changed)
        {
            settings.output_format_pretty_row_numbers = false;
            settings.output_format_pretty_row_numbers.changed = false;
        }
    }
