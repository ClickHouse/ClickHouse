    ...
    query_context->setInputInitializer([this] (ContextPtr context, const StoragePtr & input_storage)
    {
        ...
        else
        {
            ...
            if (dialect == Dialect::prql)
                ...
                parser = std::make_unique<ParserTrinoQuery>(state->max_query_size, state->max_parser_depth, state->max_parser_backtracks, end, settings[Setting::allow_experimental_trino_dialect]);
            else
