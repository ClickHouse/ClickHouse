    ...
    const std::vector<String> & external_roles)
{
    ...
    /// query would rewrite/reject it a second time, and a rule named here may not even exist on
    /// the shard (rule storage is local by default). Stripping it — rather than skipping rule
