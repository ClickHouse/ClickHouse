...
protected:
    ...
    /// Settings to pass to `Connection::sendQuery`: a copy with `compatibility`-derived values and the implicit
    /// server timezone kept but marked unchanged. They still apply locally, but are not serialized to the server,
    /// which uses its own defaults and constraints. Returns nullopt when there are no client-side defaults to filter,
    /// so the caller can send the client settings without copying them.
    std::optional<Settings> settingsWithoutClientSideDefaults() const;
...
protected:
    ...
    String server_default_timezone;
    bool has_implicit_server_timezone = false;
