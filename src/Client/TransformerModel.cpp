...
bool GPTJModel::loadModel(const std::string & file_name)
{
    ...
    // load weights
    {
        while (true)
        {
            ...
            {
                out_err << fmt::format(
                    "{}: tensor '{}' has wrong shape in model file: got [{}, {}], expected [{}, {}]\n",
                    __func__,
                    name,
                    static_cast<int>(tensor->ne[0]),
                    static_cast<int>(tensor->ne[1]),
                    ne[0],
                    ne[1]);
                out_err.next();
                return false;
...
bool GPTJModel::gptjEval(const std::vector<GptVocab::id> & ids_input, std::vector<float> & logits)
{
    ...
    for (int il = 0; il < n_layer; ++il)
    {
        ...
        // self-attention
        {
            struct ggml_tensor * qcur = ggml_rope_inplace(
                ctx0,
                ggml_reshape_3d(ctx0, ggml_mul_mat(ctx0, model.layers[il].c_attn_q_proj_w, cur), n_embd / n_head, n_head, n),
                kq_pos,
                n_rot,
                0);
            struct ggml_tensor * kcur = ggml_rope_inplace(
                ctx0,
                ggml_reshape_3d(ctx0, ggml_mul_mat(ctx0, model.layers[il].c_attn_k_proj_w, cur), n_embd / n_head, n_head, n),
                kq_pos,
                n_rot,
                0);
            ...
            // store key and value to memory
            {
                ...
                struct ggml_tensor * k
                    = ggml_view_1d(ctx0, model.memory_k, n * n_embd, (ggml_element_size(model.memory_k) * n_embd) * (il * n_ctx + n_past));
        ...
        // this is independent of the self-attention result, so it could be done in parallel to the self-attention
        {
            ...
            cur = ggml_mul_mat(ctx0, model.layers[il].c_mlp_fc_w, inp_sa);
    ...
    // norm
    {
        ...
        inp_l = ggml_add(ctx0, ggml_mul(ctx0, ggml_repeat(ctx0, model.ln_f_g, inp_l), inp_l), ggml_repeat(ctx0, model.ln_f_b, inp_l));
    }
    ...
    // lm_head
    {
        ...
        inp_l = ggml_add(ctx0, ggml_repeat(ctx0, model.lmh_b, inp_l), inp_l);
    }
...
std::vector<GptVocab::id> GPTJModel::getTopNIdsFromLogits(const std::vector<float> & logits, size_t top_n)
{
    ...
    std::stable_sort(sorted_idx.begin(), sorted_idx.end(), [&logits](size_t i1, size_t i2) { return logits[i1] > logits[i2]; });
