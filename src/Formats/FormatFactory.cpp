    ...
    const std::optional<UInt64> & min_block_size_bytes) const
{
    ...
    const bool is_random_access_format = creators.random_access_input_creator
    ...
    if (!format)
    {
        // Prefer to use metadata-aware creator if we have object metadata
        if (creators.random_access_input_creator_with_metadata
