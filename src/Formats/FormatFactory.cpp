...
FormatSettings getFormatSettings(const ContextPtr & context, const Settings & settings)
{
    ...
    format_settings.column_binary.allow_experimental = settings[Setting::allow_experimental_column_binary_format];
...
{
    return getOutputFormatWithDefaultSettings(name, buf, sample, FormatSettings{});
}
    ...
    const String & name, WriteBuffer & buf, const Block & sample, const FormatSettings & format_settings) const
{
    ...
    if (!output_getter)
        throw Exception(ErrorCodes::FORMAT_IS_NOT_SUITABLE_FOR_OUTPUT, "Format {} is not suitable for output", name);
    return output_getter(buf, sample, format_settings, {});
}
