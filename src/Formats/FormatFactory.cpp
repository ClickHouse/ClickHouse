...
void FormatFactory::registerInputFormat(const String & name, InputCreator input_creator)
{
    ...
    auto & creators = dict[format_name];
    if (creators.input_creator || creators.random_access_input_creator)
    ...
    registerFileExtension(format_name, format_name);
    KnownFormatNames::instance().add(format_name, /* case_insensitive = */ true);
}
...
{
    auto & target = dict[boost::to_lower_copy(name)].non_trivial_prefix_and_suffix_checker;
    if (target)
...
{
    auto & target = dict[boost::to_lower_copy(name)].append_support_checker;
    if (target)
...
{
    auto format_name = boost::to_lower_copy(name);
    auto & target = dict[format_name].output_creator;
    if (target)
...
{
    file_extension_formats[boost::to_lower_copy(extension)] = boost::to_lower_copy(format_name);
}
...
{
    auto & target = dict[boost::to_lower_copy(name)].file_segmentation_engine_creator;
    if (target)
...
{
    auto & target = dict[boost::to_lower_copy(name)].schema_reader_creator;
    if (target)
...
{
    auto & target = dict[boost::to_lower_copy(name)].external_schema_reader_creator;
    if (target)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "FormatFactory: Schema reader {} is already registered", name);
...
{
    auto & target = dict[boost::to_lower_copy(name)].supports_parallel_formatting;
    if (target)
...
{
    auto & target = dict[boost::to_lower_copy(name)].subset_of_columns_support_checker;
    if (target)
...
{
    auto & target = dict[boost::to_lower_copy(name)].prefers_large_blocks;
    if (target)
...
{
    auto & target = dict[boost::to_lower_copy(name)].additional_info_for_schema_cache_getter;
    if (target)
...
{
    if (boost::to_lower_copy(name) == "parquet" && context->getSettingsRef().input_format_parquet_preserve_order)
        return false;
