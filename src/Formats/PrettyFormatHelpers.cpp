...
/// Non-printable C0 control characters (0x00..0x1F) and DEL (0x7F), except the three that a
/// terminal interprets rather than swallows. Unlike C-style escape sequences (\0, \t, ...), the
/// Unicode "Control Pictures" of these bytes cannot be confused with the literal characters they
/// represent, so the "pretty" formats display them to make the bytes visible.
/// The three exceptions:
/// `TAB` (0x09) advances to the next tab stop, and `UTF8::computeWidth` measures it that way;
/// the line feed (0x0A) breaks the line, which is how a multi-line value is meant to be read and
/// copy-pasted; and `ESC` (0x1B) introduces the ANSI escape sequences, which have to keep working,
/// because that is how the data can carry a visualization.
/// See https://en.wikipedia.org/wiki/Control_Pictures
bool isControlCharacter(unsigned char byte)
{
    return (byte < 0x20 && byte != 0x09 && byte != 0x0A && byte != 0x1B) || byte == 0x7F;
}
