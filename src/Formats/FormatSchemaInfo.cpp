    ...
    fs::path path, const String & default_file_extension, bool is_server, const String & format_schema_path)
{
    ...
    if (path.is_absolute())
    {
        ...
        && !pathStartsWith(default_schema_directory_path / path, default_schema_directory_path))
    {
        if (is_server)
            throw Exception(
                ErrorCodes::BAD_ARGUMENTS,
    ...
    const FormatSettings & settings, const String & format, const Block & header, bool use_autogenerated_schema, bool with_envelope)
{
    ...
    if (!settings.schema.output_format_schema.empty())
    {
        ...
        else if (path.has_parent_path() && !pathStartsWith(default_schema_directory_path / path, default_schema_directory_path))
        {
            if (settings.schema.is_server)
                throw Exception(
                    ErrorCodes::BAD_ARGUMENTS,
    ...
    else
    {
        if (settings.schema.is_server)
        {
            ...
            schema_path = pathToGenericString(fs::path(tmp_file_path).filename());
        }
        else
        {
