...
FormatSchemaInfo::FormatSchemaInfo(
    ...
    : message_name(required_message_name)
    , log(getLogger("FormatSchemaInfo"))
{
    ...
    if (!require_message && !required_message_name.empty())
    {
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "required_message_name is specified {} when not required", required_message_name);
    }
...
FormatSchemaInfo::FormatSchemaInfo(const FormatSettings & settings, const String & format, bool require_message)
    : FormatSchemaInfo(
          ...
          settings.schema.required_message_name,
          format,
          require_message,
...
void FormatSchemaInfo::verifySchemaFileName(const String & format_schema, bool require_message, fs::path & path)
{
    ...
    if (colon_pos != String::npos)
    {
        if (!message_name.empty())
            throw Exception(
                ErrorCodes::BAD_ARGUMENTS,
                "Require message is already defined in `required_message_name` and should not be defined in 'format_schema'");
    ...
    if (require_message)
    {
        if (message_name.empty() && format_schema_message_name.empty())
            throw Exception(ErrorCodes::BAD_ARGUMENTS, "Expected 'required_message_name' setting");
...
void FormatSchemaInfo::handleSchemaContent(const String & content, const String & format, bool is_server, const String & format_schema_path)
{
    ...
    auto file_path = fs::path(format_schema_path) / file_name;
    ...
    processSchemaFile(fs::path(file_name), default_file_extension, is_server, format_schema_path);
}
...
String FormatSchemaInfo::generateSchemaFileName(const String & hashing_content, const String & file_extention)
{
    ...
    if (file_extention.empty())
        return fmt::format("{}{}-{}", CACHE_FILE_NAME_PREFIX, content_sample, content_hash_hex);
    else
        return fmt::format("{}{}-{}.{}", CACHE_FILE_NAME_PREFIX, content_sample, content_hash_hex, file_extention);
}
