...
private:
        ...
        bool skip_typed_path_check = false) const
    {
        ...
        else
        {
            ...
            /// Also when the DEFAULT PATH TYPE cannot be inside Variant (e.g. Nullable), runtime paths
            /// are Variant(None) sparse markers and the value is stored directly in shared data.
            if (column_object.getMaxDynamicPaths() == 0 || (default_path_node && !isTypeAllowedInsideVariant(default_path_type)))
            {
    ...
    bool insertIntoDynamicPath(IColumn & column, const typename JSONParser::Element & element, const JSONExtractInsertSettings & insert_settings, const FormatSettings & format_settings, String & error) const
    {
        ...
        if (default_path_node)
        {
            chassert(isTypeAllowedInsideVariant(default_path_type));
