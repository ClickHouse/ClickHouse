...
public:
    ...
    explicit DataTypeJSONPaths(Paths paths_)
    ...
    DataTypePtr finalize(bool use_string_type_for_ambiguous_paths = false) const
    {
        if (paths.empty())
            throw Exception(
                ErrorCodes::ONLY_NULLS_WHILE_READING_SCHEMA, "Cannot infer named Tuple from JSON object because object is empty");
...
void transformDatesAndDateTimes(DataTypes & data_types, TypeIndexesSet & type_indexes)
{
    ...
    bool all_dates_or_datetimes
        = (type_indexes.size()
           == (static_cast<size_t>(have_dates) + static_cast<size_t>(have_datetimes) + static_cast<size_t>(have_datetimes64)));
    ...
    DataTypes & data_types, const FormatSettings & settings, TypeIndexesSet & type_indexes, JSONInferenceInfo * json_info)
{
    ...
    bool have_numbers
        = type_indexes.contains(TypeIndex::Int64) || type_indexes.contains(TypeIndex::UInt64) || type_indexes.contains(TypeIndex::Float64);
    ...
    {
        if (isNumber(type)
            && (settings.json.read_numbers_as_strings || !json_info || json_info->numbers_parsed_from_json_strings.contains(type.get())))
            type = std::make_shared<DataTypeString>();
    }
...
void mergeJSONPaths(DataTypes & data_types, TypeIndexesSet & type_indexes, const FormatSettings & settings, JSONInferenceInfo * json_info)
{
    ...
    auto transform_func
        = [&](DataTypePtr & type1, DataTypePtr & type2) { transformInferredJSONTypesIfNeeded(type1, type2, settings, json_info); };
    for (auto & type : data_types)
    {
...
template <bool is_json>
DataTypePtr
tryInferDataTypeForSingleFieldImpl(ReadBuffer & buf, const FormatSettings & settings, JSONInferenceInfo * json_info, size_t depth = 1);
    ...
    const DateLUTImpl & utc_time_zone = DateLUT::instance("UTC"))
{
    ...
    switch (settings.date_time_input_format)
    {
        case FormatSettings::DateTimeInputFormat::Basic:
            if (tryReadDateTimeText(
                    date_time,
                    buf,
                    time_zone,
                    /*allowed_date_delimiters=*/"-/:",
        ...
        case FormatSettings::DateTimeInputFormat::BestEffortUS:
            if (tryParseDateTimeBestEffortUSStrict(date_time, buf, time_zone, utc_time_zone, /*allowed_date_delimiters=*/"-/:")
                && buf.eof())
                return true;
            return false;
    }
    ...
    switch (settings.date_time_input_format)
    {
        case FormatSettings::DateTimeInputFormat::Basic:
            return tryReadDateTime64Text(
                ...
                && buf.eof();
        case FormatSettings::DateTimeInputFormat::BestEffort:
            return tryParseDateTime64BestEffortStrict(date_time, 9, buf, time_zone, utc_time_zone, /*allowed_date_delimiters=*/"-/:")
                && buf.eof();
        case FormatSettings::DateTimeInputFormat::BestEffortUS:
            return tryParseDateTime64BestEffortUSStrict(date_time, 9, buf, time_zone, utc_time_zone, /*allowed_date_delimiters=*/"-/:")
                && buf.eof();
    }
}
    ...
    if (settings.try_infer_integers)
    {
        ...
        if (auto * /*string_buf*/ _ = dynamic_cast<ReadBufferFromString *>(&buf))
        {
            ...
            if constexpr (is_json)
            {
                ...
                {
                    buf.position() = number_start;
                    if (tryReadFloat<is_json>(tmp_float, buf, settings, has_fractional))
                        return std::make_shared<DataTypeString>();
                }
            }
        ...
        if constexpr (is_json)
        {
            if (settings.json.read_numbers_as_strings)
            {
                /// Keep the checkpoint so that, after UInt64 overflow, we can read the
                /// number as a string instead.
                UInt64 tmp_uint = 0;
                if (tryReadIntText(tmp_uint, peekable_buf))
                    return std::make_shared<DataTypeUInt64>();

                peekable_buf.rollbackToCheckpoint(/* drop= */ true);
                if (tryReadFloat<is_json>(tmp_float, peekable_buf, settings, has_fractional))
                    return std::make_shared<DataTypeString>();
...
template <bool is_json>
DataTypePtr
tryInferNumberFromStringImpl(std::string_view field, const FormatSettings & settings, JSONInferenceInfo * json_inference_info = nullptr)
{
    ReadBufferFromString buf(field);
    ...
    size_t depth)
{
    ...
    if (settings.max_parser_depth != 0 && depth > settings.max_parser_depth)
        throw Exception(
            ...
            "Maximum parse depth ({}) exceeded. Consider raising max_parser_depth setting.",
...
template <bool is_json>
DataTypePtr
tryInferDataTypeForSingleFieldImpl(ReadBuffer & buf, const FormatSettings & settings, JSONInferenceInfo * json_info, size_t depth)
{
...
static void transformFinalInferredJSONTypeIfNeededImpl(
    DataTypePtr & data_type, const FormatSettings & settings, JSONInferenceInfo * json_info, bool remain_nothing_types = false)
{
    checkStackSize();
...
bool tryInferDateTimeFromString(
    std::string_view field,
    time_t & date_time,
    const FormatSettings & settings,
...
bool tryInferDateTime64FromString(
    std::string_view field,
    DateTime64 & date_time,
    const FormatSettings & settings,
    const DateLUTImpl & time_zone,
