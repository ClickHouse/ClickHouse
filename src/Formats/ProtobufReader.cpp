...
bool ProtobufReader::readFieldNumber(int & field_number_)
{
    ...
    if (ch_unlikely(varint & (static_cast<UInt64>(0xFFFFFFFF) << 32)))
        throwUnknownFormat();
...
UInt64 ProtobufReader::readUInt()
{
    ...
    else
    {
        ...
        else if (ch_unlikely(cursor) > field_end)
            throwUnknownFormat();
    }
    return value;
}
...
{
    if (ch_unlikely(cursor + static_cast<Int64>(sizeof(T)) > field_end))
        throwUnknownFormat();
    T value;
...
{
    if (ch_unlikely(cursor > field_end))
        throwUnknownFormat();
