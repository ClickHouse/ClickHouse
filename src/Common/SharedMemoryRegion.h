...
public:
    ...
    {
        const size_t max_footprint = roundUpToPages(max_size);
        return backing_size > max_size || footprint_size > max_footprint
            || footprint_size + fillCostUpTo(backing_size) > max_footprint;
    }
...
private:
    ...
    /// The pages the file holds (`st_blocks`), as last read; never less than what the server
    /// itself committed.
