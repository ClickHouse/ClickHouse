...
The server successfully detected this situation and will download merged part from the replica to force the byte-identical result.
    ...
    M(PrometheusRemoteWriteDroppedHistograms, "Number of native histogram samples received through the Prometheus remote-write protocol and dropped because the TimeSeries table has no histograms table (its version is older than 6)", ValueType::Number) \
