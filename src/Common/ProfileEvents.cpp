...
The server successfully detected this situation and will download merged part from the replica to force the byte-identical result.
    ...
    M(ExecutableUDFSharedMemoryScrubbedBytes, "Bytes of a pooled shared-memory region of an executable user-defined function zeroed before a borrow, because the previous borrow of that process belonged to a different user: what that user's query left in the region must not be readable by the command while it serves this one.", ValueType::Bytes) \
