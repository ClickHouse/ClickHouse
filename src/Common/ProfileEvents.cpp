...
The server successfully detected this situation and will download merged part from the replica to force the byte-identical result.
    ...
    M(ExecutableUserDefinedFunctionPeakMemoryByteSeconds, "Per-process peak memory used by executable and executable_pool user-defined function child processes integrated over wall clock time, in byte-seconds. For non-pool executable functions the peak is sampled during the input/output phase, with a single best-effort reading when the function closes stdout; memory allocated strictly after stdout close is observed only on a best-effort basis. A short-lived UDF that allocates, writes small output that fits in the pipe buffer, and exits before ClickHouse reads its stdout may report a near-zero peak; CPU and elapsed time are unaffected (taken from wait4 rusage).", ValueType::Number) \
