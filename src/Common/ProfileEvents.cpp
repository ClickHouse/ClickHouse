...
The server successfully detected this situation and will download merged part from the replica to force the byte-identical result.
    ...
    \
    M(AutoParallelReplicasSkippedForcedProjection, "Number of times automatic parallel replicas skipped a query without considering it at all, because `force_use_projection` is set and projection usage cannot be guaranteed with parallel replicas", ValueType::Number) \
