...
The server successfully detected this situation and will download merged part from the replica to force the byte-identical result.
    ...
    M(ParallelReplicasQueryCount, "Number of (sub)queries executed using parallel replicas during a query execution. Note that automatic parallel replicas also increments this while building the probe plan it uses to decide, so a query can carry it without any replica having done work - see `AutoParallelReplicasApplied` and `ParallelReplicasUsedCount` for what was actually executed", ValueType::Number) \
\
    M(AutoParallelReplicasProbePlansBuilt, "Number of times automatic parallel replicas built the probe plan, i.e. re-planned the query with parallel replicas in order to decide whether they pay off", ValueType::Number) \
    M(AutoParallelReplicasProbeMicroseconds, "Time spent building and matching the probe plan of automatic parallel replicas, whether or not the plan was adopted", ValueType::Microseconds) \
    M(AutoParallelReplicasNodeMatchFailed, "Number of times automatic parallel replicas could not find the node of the probe plan in the single-node plan, so no statistics could be attributed and the decision was skipped", ValueType::Number) \
    M(AutoParallelReplicasNoReadFromReplicas, "Number of times the probe plan of automatic parallel replicas contained no read from the other replicas, so there was nothing to cost", ValueType::Number) \
    M(AutoParallelReplicasNoSelectedRows, "Number of times automatic parallel replicas found no selected rows in the index analysis result and skipped the decision", ValueType::Number) \
    ...
    M(AutoParallelReplicasProbeSkippedByThreshold, "Number of times automatic parallel replicas did not even build the probe plan, because index analysis already showed the largest read in the plan gives less than `automatic_parallel_replicas_min_bytes_per_replica` per replica. Unlike `AutoParallelReplicasRejectedByThreshold` this costs nothing - it is the gate working as intended", ValueType::Number) \
