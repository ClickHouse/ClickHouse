...
#define APPLY_FOR_BUILTIN_EVENTS(M) \
    ...
    M(ConcurrentHashJoinBuildMicroseconds, "Time spent building the right side of a `parallel_hash` join: adding right-side blocks and merging the per-slot hash tables. Summed over all build threads.", ValueType::Microseconds) \
    M(ConcurrentHashJoinBuildDispatchMicroseconds, "Part of `ConcurrentHashJoinBuildMicroseconds`: time spent splitting each right-side block across the slots.", ValueType::Microseconds) \
    M(ConcurrentHashJoinBuildInsertMicroseconds, "Part of `ConcurrentHashJoinBuildMicroseconds`: time spent inserting the split blocks into the per-slot hash tables.", ValueType::Microseconds) \
    M(ConcurrentHashJoinBuildMergeMicroseconds, "Part of `ConcurrentHashJoinBuildMicroseconds`: time spent merging the per-slot two-level hash tables into one when the build ends. Zero when the two-level table is not used.", ValueType::Microseconds) \
    M(ConcurrentHashJoinProbeMicroseconds, "Time spent joining left-side blocks in a `parallel_hash` join: splitting the block across the slots, looking rows up, and producing the joined columns. Summed over all probe threads.", ValueType::Microseconds) \
    M(ConcurrentHashJoinProbeDispatchMicroseconds, "Part of `ConcurrentHashJoinProbeMicroseconds`: time spent splitting each left-side block across the slots before any lookup.", ValueType::Microseconds) \
    M(ConcurrentHashJoinProbeLookupMicroseconds, "Part of `ConcurrentHashJoinProbeMicroseconds`: time spent looking rows up in the hash tables and recording the matches. Producing the joined columns is counted in `HashJoinResultBuildOutputMicroseconds`.", ValueType::Microseconds) \
    M(PartitionedHashJoinBuildMicroseconds, "Time spent building the right side of a `partitioned_hash` join: adding right-side blocks, waiting for all build threads, and inserting into the hash table. Summed over all build threads.", ValueType::Microseconds) \
    M(PartitionedHashJoinBuildFillMicroseconds, "Part of `PartitionedHashJoinBuildMicroseconds`: time spent hashing the keys of right-side blocks and storing the blocks as they arrive.", ValueType::Microseconds) \
    M(PartitionedHashJoinBuildHistogramMicroseconds, "Part of `PartitionedHashJoinBuildMicroseconds`: time spent counting the rows of each partition and computing where each partition starts.", ValueType::Microseconds) \
    M(PartitionedHashJoinBuildScatterMicroseconds, "Part of `PartitionedHashJoinBuildMicroseconds`: time spent moving the right-side keys and row references into their partitions, over all passes.", ValueType::Microseconds) \
    M(PartitionedHashJoinBuildInsertMicroseconds, "Part of `PartitionedHashJoinBuildMicroseconds`: time spent inserting the partitioned rows into the hash table, including the serial insert of the rows that did not fit their partition's range.", ValueType::Microseconds) \
    M(PartitionedHashJoinProbeMicroseconds, "Time spent joining left-side blocks in a `partitioned_hash` join. Summed over all probe threads.", ValueType::Microseconds) \
    M(PartitionedHashJoinProbeLookupMicroseconds, "Part of `PartitionedHashJoinProbeMicroseconds`: time spent hashing the left-side keys, looking them up in the hash table, and recording the matches. Producing the joined columns is counted in `HashJoinResultBuildOutputMicroseconds`.", ValueType::Microseconds) \
    M(PartitionedHashJoinPartitions, "Number of partitions a `partitioned_hash` join split its hash table into, added once per join. 1 means the table was built as one partition.", ValueType::Number) \
    M(PartitionedHashJoinInsertedRows, "Number of right-side rows inserted into the hash tables of `partitioned_hash` joins.", ValueType::Number) \
    M(PartitionedHashJoinHashTableBytes, "Bytes allocated for the hash table of a `partitioned_hash` join. The table is sized once, from an estimate of the number of distinct keys.", ValueType::Bytes) \
    M(PartitionedHashJoinOverflowRows, "Number of right-side rows of a `partitioned_hash` join that did not fit in their partition's range of the hash table and were inserted afterwards by one thread.", ValueType::Number) \
    M(PartitionedHashJoinDuplicateRunBytes, "Bytes a `partitioned_hash` join allocated to store the row references of keys that occur more than once.", ValueType::Bytes) \
    M(PartitionedHashJoinTeardownMicroseconds, "Time spent freeing the hash table, the build buffers and the stored right-side blocks of a `partitioned_hash` join.", ValueType::Microseconds) \
    M(PartitionedHashJoinScatterGroups, "Number of groups of right-side blocks a `partitioned_hash` join partitioned separately to limit its peak memory. 1 means all blocks were partitioned at once.", ValueType::Number) \
    M(PartitionedHashJoinTableResizes, "Number of times the hash table of a `partitioned_hash` join grew after inserting started.", ValueType::Number) \
    M(PartitionedHashJoinRowStoreBlocks, "Number of right-side blocks a `partitioned_hash` join stored in row-major form (`RowDataStore`), as `parallel_hash` does when the row store is enabled.", ValueType::Number) \
...
The server successfully detected this situation and will download merged part from the replica to force the byte-identical result.
    ...
    M(DistrCacheInlineWrites, "Distributed Cache write buffer event. Number of cache writes that ran inline on the calling thread (writes of temporary data, which has no object storage write to overlap with)", ValueType::Number) \
    M(DistrCacheSkippedWrites, "Distributed Cache write buffer event. Number of write buffers that skipped the write-through cache write because no background write slot was free (the limit set by the `distributed_cache_write_pool_size` server setting was reached) or a background write could not be started. The data is still written to object storage", ValueType::Number) \
    \
    ...
    M(HashJoinResultFilterLeftMicroseconds, "Time spent selecting the matched left-side rows of a probe block before the joined columns are produced. Counted for `hash`, `parallel_hash`, `partitioned_hash` and `grace_hash`; part of the `*ProbeMicroseconds` events of `parallel_hash` and `partitioned_hash`.", ValueType::Microseconds) \
    M(HashJoinResultBuildOutputMicroseconds, "Time spent producing the joined columns: reading the matched right-side values and repeating each left-side row once per match. Counted for `hash`, `parallel_hash`, `partitioned_hash` and `grace_hash`; part of the `*ProbeMicroseconds` events of `parallel_hash` and `partitioned_hash`.", ValueType::Microseconds) \
    \
