...
#define APPLY_FOR_BUILTIN_EVENTS(M) \
    ...
    M(MutationsSurvivedKeeperReconnect, "Number of times an in-progress part mutation kept running across a transient ZooKeeper (Keeper) session re-establishment (detached to survive it) instead of being cancelled and recomputed from scratch (see the reuse_precomputed_mutations_after_keeper_reconnect setting). Its already-computed result may still be aborted or discarded afterwards, e.g. if the target part is dropped; the number of results actually committed without re-computation is MutationsReusedPrecomputedParts.", ValueType::Number) \
    M(MutationsReusedPrecomputedParts, "Number of times a part mutation committed a result that had been computed before a transient ZooKeeper (Keeper) session re-establishment, avoiding re-computation.", ValueType::Number) \
