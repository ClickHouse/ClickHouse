...
The server successfully detected this situation and will download merged part from the replica to force the byte-identical result.
    ...
    M(ParallelReplicasQueryCount, "Number of (sub)queries planned to be executed using parallel replicas during a query execution. Automatic parallel replicas also increments this for the plan it builds to make its decision, so a counted query may never have run with parallel replicas - see `AutoParallelReplicasApplied` and `ParallelReplicasUsedCount` for what was actually executed", ValueType::Number) \
    \
    M(AutoParallelReplicasSkippedDueToSettings, "Number of times automatic parallel replicas skipped a query without considering it at all, because the settings it was given rule the optimization out regardless of what the query does. Currently that means a forced projection (`force_optimize_projection` or `force_optimize_projection_name`): the plan with parallel replicas is built with projections disabled, so projection usage could not be guaranteed once it replaced the original plan", ValueType::Number) \
    M(AutoParallelReplicasPlanShapeNotSupported, "Number of times automatic parallel replicas skipped a query without building a plan with parallel replicas, because its plan contains steps that cannot collect runtime dataflow statistics. This is the reach of the optimization: the higher it is, the more queries it never gets to consider", ValueType::Number) \
    M(AutoParallelReplicasPlanBuildAttempts, "Number of times automatic parallel replicas tried to build a plan with parallel replicas in order to decide whether it pays off. The attempt is not made for every query - it is skipped when the query is already known not to qualify - and it does not always produce a plan, because the query may turn out not to be executable with parallel replicas at all", ValueType::Number) \
    M(AutoParallelReplicasMicroseconds, "Total time spent in the automatic parallel replicas optimization: the plan traversals, building and matching the plan with parallel replicas, and the cost model, whether or not the plan was adopted", ValueType::Microseconds) \
    M(AutoParallelReplicasPlanNotSuitable, "Number of times automatic parallel replicas could not reach a decision from the plan with parallel replicas: it could not be built, or could not be matched against the single-node plan, or did not give the cost model what it needs, or its reads could not be given the index analysis of the single-node plan. The `AutoParallelReplicas` logger says which case it was", ValueType::Number) \
    M(AutoParallelReplicasNoStatistics, "Number of times automatic parallel replicas had no runtime dataflow statistics for the matched node yet, so it collected them instead of deciding", ValueType::Number) \
    M(AutoParallelReplicasStatisticsDrifted, "Number of times the rows read from storage differed from the cached statistics by more than 2x, so automatic parallel replicas recollected them instead of deciding", ValueType::Number) \
    M(AutoParallelReplicasCostModelEvaluated, "Number of times automatic parallel replicas reached the cost model and compared the local plan against the parallel replicas plan", ValueType::Number) \
    M(AutoParallelReplicasApplied, "Number of times automatic parallel replicas replaced the query plan with the parallel replicas one. This is the number of queries the optimization actually changed", ValueType::Number) \
    M(AutoParallelReplicasSkippedEarly, "Number of times automatic parallel replicas did not even build the plan with parallel replicas, because index analysis already showed the largest read in the plan gives less than `automatic_parallel_replicas_min_bytes_per_replica` per replica", ValueType::Number) \
    M(AutoParallelReplicasRejectedByThreshold, "Number of times automatic parallel replicas declined although the cost model favoured parallel replicas, because the read was below `automatic_parallel_replicas_min_bytes_per_replica`", ValueType::Number) \
    M(AutoParallelReplicasRejectedByCostModel, "Number of times automatic parallel replicas declined because the cost model estimated the local plan to be at least as fast as the parallel replicas one", ValueType::Number) \
    M(AutoParallelReplicasBytesPerReplica, "How many bytes per replica the read of this query gives, as automatic parallel replicas sized it - recorded whatever was decided, so the distribution covers both the queries `automatic_parallel_replicas_min_bytes_per_replica` turns away and the ones it lets through. Beware that the two places it is recorded size the read differently and can disagree by a factor of two: the gate before planning uses the compressed sizes the parts record, the cost model uses the bytes actually read. The gate runs first, so it is the gate's figure - the one reported alongside `AutoParallelReplicasSkippedEarly` - that decides how high the threshold can go before a query stops being considered", ValueType::Bytes) \
    \
