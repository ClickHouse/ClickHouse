...
#define APPLY_FOR_BUILTIN_EVENTS(M) \
    ...
    M(IcebergParallelManifestDecodeThreadsSpawned, "Total number of parallel Iceberg manifest-decode producer threads spawned across all queries (incremented by N when iceberg_parallel_manifest_decode_threads > 1 engages on a query).", ValueType::Number) \
