...
#define APPLY_FOR_BUILTIN_EVENTS(M) \
    ...
    M(ASTFuzzerOracleChecks, "Number of oracle checks attempted by the server-side AST fuzzer (umbrella counter across all oracles).", ValueType::Number) \
    M(ASTFuzzerOracleTLPWhereChecks, "Number of TLP WHERE oracle checks attempted by the server-side AST fuzzer.", ValueType::Number) \
    M(ASTFuzzerOracleNoRECChecks, "Number of NoREC oracle checks attempted by the server-side AST fuzzer.", ValueType::Number) \
    ...
    M(ASTFuzzerOracleTLPDistinctChecks, "Number of TLP DISTINCT oracle checks attempted by the server-side AST fuzzer.", ValueType::Number) \
    M(ASTFuzzerOracleTLPGroupByChecks, "Number of TLP GROUP BY oracle checks attempted by the server-side AST fuzzer.", ValueType::Number) \
    M(ASTFuzzerOracleTLPHavingChecks, "Number of TLP HAVING oracle checks attempted by the server-side AST fuzzer.", ValueType::Number) \
    M(ASTFuzzerOracleDQPChecks, "Number of DQP (differential query plan) oracle checks attempted by the server-side AST fuzzer.", ValueType::Number) \
    ...
    M(ASTFuzzerOraclePrewhereEquivalenceChecks, "Number of PREWHERE-equivalence oracle checks attempted by the server-side AST fuzzer.", ValueType::Number) \
    M(ASTFuzzerOracleSkipIndexEquivalenceChecks, "Number of skip-index-equivalence oracle checks attempted by the server-side AST fuzzer.", ValueType::Number) \
    M(ASTFuzzerOracleSettingFlipSweepChecks, "Number of setting-flip-sweep oracle checks attempted by the server-side AST fuzzer.", ValueType::Number) \
    M(ASTFuzzerOracleCodecRoundtripChecks, "Number of codec round-trip oracle checks attempted by the server-side AST fuzzer.", ValueType::Number) \
    M(ASTFuzzerOracleEngineEquivalenceChecks, "Number of engine-equivalence oracle checks attempted by the server-side AST fuzzer.", ValueType::Number) \
    M(ASTFuzzerOraclePartitionEquivalenceChecks, "Number of partition-equivalence oracle checks attempted by the server-side AST fuzzer.", ValueType::Number) \
    M(ASTFuzzerOracleLowCardinalityEquivalenceChecks, "Number of LowCardinality-equivalence oracle checks attempted by the server-side AST fuzzer.", ValueType::Number) \
    M(ASTFuzzerOracleSampleEquivalenceChecks, "Number of SAMPLE-equivalence oracle checks attempted by the server-side AST fuzzer.", ValueType::Number) \
    M(ASTFuzzerOracleProjectionEquivalenceChecks, "Number of projection-equivalence oracle checks attempted by the server-side AST fuzzer.", ValueType::Number) \
    M(ASTFuzzerOracleAggregateIfIdentityChecks, "Number of aggregate-If identity oracle checks attempted by the server-side AST fuzzer.", ValueType::Number) \
    M(ASTFuzzerOracleNullIdentityChecks, "Number of NULL-identity oracle checks attempted by the server-side AST fuzzer.", ValueType::Number) \
    M(ASTFuzzerOracleCastRoundtripChecks, "Number of CAST round-trip oracle checks attempted by the server-side AST fuzzer.", ValueType::Number) \
    M(ASTFuzzerOracleAggregateStateColumnChecks, "Number of aggregate-state-column oracle checks attempted by the server-side AST fuzzer.", ValueType::Number) \
    M(ASTFuzzerOracleTupleSummingChecks, "Number of tuple-summing oracle checks attempted by the server-side AST fuzzer.", ValueType::Number) \
    M(ASTFuzzerOracleSchemaRoundtripChecks, "Number of schema round-trip oracle checks attempted by the server-side AST fuzzer.", ValueType::Number) \
    M(ASTFuzzerOracleDeleteMutationChecks, "Number of DELETE-mutation oracle checks attempted by the server-side AST fuzzer.", ValueType::Number) \
    M(ASTFuzzerOracleUpdateMutationChecks, "Number of UPDATE-mutation oracle checks attempted by the server-side AST fuzzer.", ValueType::Number) \
    M(ASTFuzzerOracleMaterializeIndexChecks, "Number of MATERIALIZE-INDEX invariance oracle checks attempted by the server-side AST fuzzer.", ValueType::Number) \
    M(ASTFuzzerOraclePredicateDeMorganChecks, "Number of De-Morgan/comparison-symmetry oracle checks attempted by the server-side AST fuzzer.", ValueType::Number) \
    M(ASTFuzzerOracleArrayJoinIdentityChecks, "Number of ARRAY JOIN identity oracle checks attempted by the server-side AST fuzzer.", ValueType::Number) \
    M(ASTFuzzerOracleGroupingSetsChecks, "Number of grouping-modifier equivalence oracle checks attempted by the server-side AST fuzzer.", ValueType::Number) \
    M(ASTFuzzerOracleRowPolicyChecks, "Number of row-policy equivalence oracle checks attempted by the server-side AST fuzzer.", ValueType::Number) \
    M(ASTFuzzerOracleFinalMergeChecks, "Number of FINAL-merge (ReplacingMergeTree dedup) oracle checks attempted by the server-side AST fuzzer.", ValueType::Number) \
    M(ASTFuzzerOracleWithFillChecks, "Number of ORDER BY WITH FILL grid oracle checks attempted by the server-side AST fuzzer.", ValueType::Number) \
    M(ASTFuzzerOraclePipeEquivalenceChecks, "Number of pipe-operator equivalence oracle checks attempted by the server-side AST fuzzer.", ValueType::Number) \
    M(ASTFuzzerOracleDictGetChecks, "Number of dictGet-vs-JOIN oracle checks attempted by the server-side AST fuzzer.", ValueType::Number) \
    M(ASTFuzzerOracleMaterializedColumnChecks, "Number of MATERIALIZED/ALIAS column recomputation oracle checks attempted by the server-side AST fuzzer.", ValueType::Number) \
    M(ASTFuzzerOracleAlterModifyChecks, "Number of ALTER MODIFY COLUMN widen-vs-CAST oracle checks attempted by the server-side AST fuzzer.", ValueType::Number) \
    M(ASTFuzzerOracleLightweightUpdateChecks, "Number of lightweight-update-vs-ALTER-UPDATE oracle checks attempted by the server-side AST fuzzer.", ValueType::Number) \
    M(ASTFuzzerOracleWindowEquivalenceChecks, "Number of window-frame equivalence oracle checks attempted by the server-side AST fuzzer.", ValueType::Number) \
    M(ASTFuzzerOracleJoinOrderSweepChecks, "Number of join-order/algorithm sweep oracle checks attempted by the server-side AST fuzzer.", ValueType::Number) \
    M(ASTFuzzerOracleSequenceFunnelChecks, "Number of sequence/funnel monotonicity oracle checks attempted by the server-side AST fuzzer.", ValueType::Number) \
    M(ASTFuzzerOracleSubcolumnChecks, "Number of composite-subcolumn (Tuple/Array/Map/Nullable) oracle checks attempted by the server-side AST fuzzer.", ValueType::Number) \
    M(ASTFuzzerOracleViewTtlChecks, "Number of materialized-view/recompression-TTL oracle checks attempted by the server-side AST fuzzer.", ValueType::Number) \
    M(ASTFuzzerOracleCorrelatedSubqueryChecks, "Number of correlated-subquery decorrelation oracle checks attempted by the server-side AST fuzzer.", ValueType::Number) \
    M(ASTFuzzerOracleCardinalityChecks, "Number of CERT cardinality-monotonicity oracle checks attempted by the server-side AST fuzzer.", ValueType::Number) \
    M(ASTFuzzerOraclePivotContainmentChecks, "Number of PQS pivot-containment oracle checks attempted by the server-side AST fuzzer.", ValueType::Number) \
