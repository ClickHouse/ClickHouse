...
#define APPLY_FOR_BUILTIN_EVENTS(M) \
    ...
    M(DirectInsertedRows, "Number of rows written by INSERT pipelines into their immediate destination, not counting the rows written by insert-triggered materialized views into their target tables. The immediate destination is not necessarily a table: rows written into the sink of a table function, as in INSERT INTO FUNCTION, are counted here as well. This covers user INSERT queries as well as internal writes going through the INSERT interpreter, such as system log flushes, Buffer table flushes, and the immediate target-table write of a refreshable materialized view refresh. Background streaming pushes (e.g. from Kafka, NATS, RabbitMQ, FileLog, or ObjectStorageQueue tables) skip the destination and only feed the attached materialized views, so they are not counted here.", ValueType::Number) \
    M(DirectInsertedBytes, "Number of uncompressed bytes written by INSERT pipelines into their immediate destination, not counting the bytes written by insert-triggered materialized views into their target tables. The immediate destination is not necessarily a table: bytes written into the sink of a table function, as in INSERT INTO FUNCTION, are counted here as well. This covers user INSERT queries as well as internal writes going through the INSERT interpreter, such as system log flushes, Buffer table flushes, and the immediate target-table write of a refreshable materialized view refresh. Background streaming pushes (e.g. from Kafka, NATS, RabbitMQ, FileLog, or ObjectStorageQueue tables) skip the destination and only feed the attached materialized views, so they are not counted here.", ValueType::Bytes) \
    M(MaterializedViewInsertedRows, "Number of rows written by insert-triggered materialized views into their target tables. For an explicit INSERT query into a materialized view, and likewise for a refreshable materialized view refresh, the immediate write into the target table is counted in DirectInsertedRows, while the writes of the materialized views that depend on that target table are counted here.", ValueType::Number) \
    M(MaterializedViewInsertedBytes, "Number of uncompressed bytes written by insert-triggered materialized views into their target tables. For an explicit INSERT query into a materialized view, and likewise for a refreshable materialized view refresh, the immediate write into the target table is counted in DirectInsertedBytes, while the writes of the materialized views that depend on that target table are counted here.", ValueType::Bytes) \
