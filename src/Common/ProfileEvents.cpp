...
#define APPLY_FOR_BUILTIN_EVENTS(M) \
    ...
    M(DirectInsertedRows, "Number of rows written by INSERT pipelines into their immediate destination table, not counting the rows written by insert-triggered materialized views into their target tables. This covers user INSERT queries as well as internal writes going through the INSERT interpreter, such as system log flushes, Buffer table flushes, and refreshable materialized view refreshes. Background streaming pushes (e.g. from Kafka, NATS, RabbitMQ, FileLog, or ObjectStorageQueue tables) skip the destination table and only feed the attached materialized views, so they are not counted here.", ValueType::Number) \
    M(DirectInsertedBytes, "Number of uncompressed bytes written by INSERT pipelines into their immediate destination table, not counting the bytes written by insert-triggered materialized views into their target tables. This covers user INSERT queries as well as internal writes going through the INSERT interpreter, such as system log flushes, Buffer table flushes, and refreshable materialized view refreshes. Background streaming pushes (e.g. from Kafka, NATS, RabbitMQ, FileLog, or ObjectStorageQueue tables) skip the destination table and only feed the attached materialized views, so they are not counted here.", ValueType::Bytes) \
    M(MaterializedViewInsertedRows, "Number of rows written by insert-triggered materialized views into their target tables. Refreshable materialized view refreshes are counted in DirectInsertedRows instead. For an explicit INSERT query into a materialized view, its immediate target-table write is counted in DirectInsertedRows, while writes to dependent materialized views of that target are counted here.", ValueType::Number) \
    M(MaterializedViewInsertedBytes, "Number of uncompressed bytes written by insert-triggered materialized views into their target tables. Refreshable materialized view refreshes are counted in DirectInsertedBytes instead. For an explicit INSERT query into a materialized view, its immediate target-table write is counted in DirectInsertedBytes, while writes to dependent materialized views of that target are counted here.", ValueType::Bytes) \
