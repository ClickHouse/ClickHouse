...
#define APPLY_FOR_BUILTIN_EVENTS(M) \
    ...
    M(HashJoinPartitionedBuildMicroseconds, "Time spent building the right side of a `partitioned_hash` join: adding right-side blocks, waiting for all build threads, and inserting into the hash table. Summed over all build threads.", ValueType::Microseconds) \
    ...
    M(HashJoinPartitionedBuildInsertMicroseconds, "Part of `HashJoinPartitionedBuildMicroseconds`: time spent inserting the partitioned rows into the hash table, including the serial insert of the rows that did not fit their partition's range.", ValueType::Microseconds) \
    ...
    M(HashJoinPartitionedProbeLookupMicroseconds, "Part of `HashJoinPartitionedProbeMicroseconds`: time spent hashing the left-side keys, looking them up in the hash table, and recording the matches. Producing the joined columns is counted in `HashJoinProbeGatherMicroseconds`, which the hash joins share.", ValueType::Microseconds) \
    ...
    M(HashJoinPartitionOverflowRows, "Number of right-side rows of a `partitioned_hash` join that did not fit in their partition's range of the hash table and were inserted afterwards by one thread.", ValueType::Number) \
    ...
    M(HashJoinRowStoreBlocks, "Number of right-side blocks a `partitioned_hash` join stored in row-major form (`RowDataStore`), as `hash` does when the row store is enabled.", ValueType::Number) \
