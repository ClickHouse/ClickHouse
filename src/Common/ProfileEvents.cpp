...
#define APPLY_FOR_BUILTIN_EVENTS(M) \
    ...
    M(MergesTimeMilliseconds, "Total time spent for background merges.", 0)\
    \
    ...
    M(MutateTaskProjectionsCalculationMicroseconds, "Time spent calculating projections", 0) \
    \
    ...
    \
    M(RegexpCreated, "Compiled regular expressions. Identical regular expressions compiled just once and cached forever.", 0) \
...
The server successfully detected this situation and will download merged part from the replica to force the byte-identical result.
    ...
    M(ReadBufferFromS3ResetSessions, "Number of HTTP sessions that were reset in ReadBufferFromS3.", 0) \
    M(ReadBufferFromS3PreservedSessions, "Number of HTTP sessions that were preserved in ReadBufferFromS3.", 0) \
    \
    ...
    M(AzureUploadPart, "Number of Azure blob storage API UploadPart calls", 0) \
    ...
    M(DiskAzureUploadPart, "Number of Disk Azure blob storage API UploadPart calls", 0) \
    ...
    M(DiskAzureDeleteObjects, "Number of Azure blob storage API DeleteObject(s) calls.", 0) \
