...
#define APPLY_FOR_BUILTIN_EVENTS(M) \
    ...
    M(PartitionedHashJoinBuildLeafMicroseconds, "partitioned_hash (radix_hash): build-side sub-phase, time spent deciding the partition plan of the one shared hash table (`decidePartitionPlan`), in the owner-range inserts (`ownerWaveWorker`, AMAC or sequential), in the serial drain of the overflow rows (`drainOverflow`) and, on the single-partition plan, in `postBuildSinglePartition`, summed across build threads. A subset of `PartitionedHashJoinBuildMicroseconds`.", ValueType::Microseconds) \
    ...
    M(PartitionedHashJoinProbeLookupMicroseconds, "partitioned_hash (radix_hash): probe-side sub-phase, time spent in `PartitionedHashJoin::sharedJoinRightColumns` (the hash-table lookup over the one shared table: hashing each row, the AMAC find ring or plain lookup, plus match bookkeeping; does NOT include gathering any column values, see `HashJoinResultFilterLeftMicroseconds`/`HashJoinResultBuildOutputMicroseconds`), summed across probe threads. A subset of `PartitionedHashJoinProbeMicroseconds`.", ValueType::Microseconds) \
    M(PartitionedHashJoinPartitions, "Number of build partitions (owner ranges of the one shared hash table) chosen by partitioned_hash JOIN build barriers (one count per join; 1 means the single-partition plan).", ValueType::Number) \
