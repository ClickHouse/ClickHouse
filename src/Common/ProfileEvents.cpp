...
The server successfully detected this situation and will download merged part from the replica to force the byte-identical result.
    ...
    M(ExecutableUDFSharedMemoryInputBytes, "Total bytes of serialized input placed into the shared-memory region of executable user-defined functions. The server writes them with pwrite and the command reads them out of its own mapping, so they cross the process boundary once instead of twice as they would through a pipe.", ValueType::Bytes) \
    M(ExecutableUDFSharedMemoryOutputBytes, "Total bytes of serialized output read back from the shared-memory region of executable user-defined functions. The command writes them into its own mapping and the server reads them with pread, so they cross the process boundary once instead of twice as they would through a pipe.", ValueType::Bytes) \
