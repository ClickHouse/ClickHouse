...
The server successfully detected this situation and will download merged part from the replica to force the byte-identical result.
    ...
    M(UniqExactMergeWaves, "Number of existing uniqExact parallel conversion or merge dispatches that processed at least one state conversion or bucket and completed their aggregation thread-pool barrier.", ValueType::Number) \
    M(UniqExactMergeWaveInputStates, "Number of uniqExact states presented to reported parallel conversion or merge waves.", ValueType::Number) \
    M(UniqExactMergeWaveElapsedMicroseconds, "Total wall-clock time of reported uniqExact parallel conversion and merge waves, measured from dispatch through their completion barrier.", ValueType::Microseconds) \
    M(UniqExactMergeWaveCPUTimeMicroseconds, "Total thread CPU time consumed by pooled tasks in reported uniqExact parallel conversion and merge waves, where the per-thread CPU clock is available.", ValueType::Microseconds) \
    M(UniqExactMergeWaveWorkers, "Sum of the number of distinct thread-pool workers that executed each reported uniqExact parallel conversion or merge wave.", ValueType::Number) \
