...
The server successfully detected this situation and will download merged part from the replica to force the byte-identical result.
    ...
    M(AggregationInMemoryMergeInputVariants, "Number of non-empty keyed aggregation variants presented to in-memory final aggregation merges before merge preparation.", ValueType::Number) \
    M(AggregationInMemoryMergeInputTwoLevelVariants, "Number of two-level variants among the non-empty keyed aggregation variants presented to in-memory final aggregation merges before merge preparation.", ValueType::Number) \
    M(AggregationInMemoryMergeInputGroups, "Number of already-materialized hash-table groups across non-empty keyed aggregation variants presented to in-memory final aggregation merges before merge preparation. Excludes adaptive aggregation backlog records not yet drained into those hash tables.", ValueType::Number) \
    M(AggregationInMemoryMergePathSingleLevel, "Number of in-memory final keyed aggregation merges that used a single-level hash-table path.", ValueType::Number) \
    M(AggregationInMemoryMergePathTwoLevel, "Number of in-memory final keyed aggregation merges that used the parallel two-level bucket path.", ValueType::Number) \
    M(AggregationInMemoryMergeBuckets, "Number of two-level buckets successfully merged and converted during in-memory final aggregation merges.", ValueType::Number) \
    M(AggregationInMemoryMergeBucketElapsedMicroseconds, "Total wall-clock time spent merging and converting individual two-level buckets during in-memory final aggregation merges, summed across merge sources.", ValueType::Microseconds) \
    M(AggregationInMemoryMergeBusiestBucketElapsedMicroseconds, "Sum, across successfully completed in-memory final aggregation merge operations, of the wall-clock time of each operation's slowest two-level bucket.", ValueType::Microseconds) \
    M(AggregationInMemoryMergeSources, "Number of merge source processors that completed at least one two-level bucket during successfully completed in-memory final aggregation merges.", ValueType::Number) \
    M(AggregationInMemoryMergeBusiestSourceElapsedMicroseconds, "Sum, across successfully completed in-memory final aggregation merge operations, of the total bucket wall-clock time accumulated by each operation's busiest merge source processor.", ValueType::Microseconds) \
    M(UniqExactMergeWaves, "Number of parallel conversion or merge dispatches reached by the underlying uniqExact state implementation that processed at least one state conversion or bucket and completed their aggregation thread-pool barrier. This is a dispatch-level metric: call forms contribute only for dispatches they reach, and it does not guarantee complete phase coverage for every nullable, variadic, tuple, or combinator wrapper.", ValueType::Number) \
    M(UniqExactMergeWaveInputStates, "Number of uniqExact states presented to reported parallel conversion or merge dispatches.", ValueType::Number) \
    M(UniqExactMergeWaveElapsedMicroseconds, "Total wall-clock time of reported uniqExact parallel conversion and merge dispatches, measured from dispatch through their completion barrier.", ValueType::Microseconds) \
    M(UniqExactMergeWaveCPUTimeMicroseconds, "Total thread CPU time consumed by pooled tasks that processed work in reported uniqExact parallel conversion and merge dispatches, where the per-thread CPU clock is available.", ValueType::Microseconds) \
    M(UniqExactMergeWaveWorkers, "Sum of the number of distinct thread-pool workers that processed at least one state conversion or bucket in each reported uniqExact parallel conversion or merge dispatch.", ValueType::Number) \
