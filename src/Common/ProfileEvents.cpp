...
#define APPLY_FOR_BUILTIN_EVENTS(M) \
    ...
    M(TextIndexDiscardPatternQueryLowSelectivity, "Number of times a LIKE/ILIKE pattern query was bypassed before reading posting lists because one of its matched tokens is present in every row, so those postings cannot prune anything.", ValueType::Number) \
