...
The server successfully detected this situation and will download merged part from the replica to force the byte-identical result.
    ...
    M(IndexGenericExclusionSearchShortCircuit, "Number of times the generic exclusion search over the index marks was short-circuited because the key condition was proven to give the same result for every mark range (e.g. no key column used by the condition is present in the loaded primary key index)", ValueType::Number) \
