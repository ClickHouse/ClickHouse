...
#define APPLY_FOR_BUILTIN_EVENTS(M) \
    ...
    M(ReplicatedQueueScheduleRejections, "Number of times a selected ReplicatedMergeTree replication queue entry was not handed to a background executor because the executor had no free slots. The entry stays in the queue and is retried.", ValueType::Number) \
    \
