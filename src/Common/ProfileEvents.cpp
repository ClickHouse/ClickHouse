...
The server successfully detected this situation and will download merged part from the replica to force the byte-identical result.
    ...
    M(ObjectStorageQueueMoveSourceRewritten, "Number of source objects left in place by after_processing = move because they were rewritten after being copied to the destination", ValueType::Number) \
