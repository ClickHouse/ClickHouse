...
#define APPLY_FOR_BUILTIN_EVENTS(M) \
    ...
    M(HashJoinBuiltWithParallelLayout, "Number of HashJoins built with the parallel map layout, which splits every hash table across 256 buckets so build threads can fill disjoint slots. This is the layout that used to appear in the query plan as a separate ConcurrentHashJoin. The layout follows the right-side size estimate against parallel_hash_join_threshold, not the join_algorithm setting, since `hash` and `parallel_hash` are one algorithm.", ValueType::Number) \
    M(HashJoinBuiltWithSerialLayout, "Number of HashJoins built with the serial map layout, which keeps one bucket per hash table and is filled by a single thread. This is the layout that used to appear in the query plan as a plain HashJoin. See HashJoinBuiltWithParallelLayout for how the layout is chosen.", ValueType::Number) \
    ...
    M(PartitionedHashJoinProbeLookupMicroseconds, "Part of `PartitionedHashJoinProbeMicroseconds`: time spent hashing the left-side keys, looking them up in the hash table, and recording the matches. Producing the joined columns is counted in `HashJoinProbeGatherMicroseconds`, which the hash joins share.", ValueType::Microseconds) \
