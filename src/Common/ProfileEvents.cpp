...
#define APPLY_FOR_BUILTIN_EVENTS(M) \
    ...
    M(ReplicatedQueueScheduleRejections, "Number of times a selected ReplicatedMergeTree replication queue entry was not handed to a background executor because the executor could not accept it (no free slots, or shutdown in progress). The entry stays in the queue and is retried.", ValueType::Number) \
    \
