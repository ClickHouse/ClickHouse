...
#define APPLY_FOR_BUILTIN_EVENTS(M) \
    ...
    M(PatchesMergeOnKeyRowsAddedToHashTable, "Total number of rows added to per-equal-run hash maps when applying v2 patch parts with MergeOnKey mode", ValueType::Number) \
    ...
    M(BuildPatchesMergeOnKeyMicroseconds, "Total time spent building per-equal-run hash maps for v2 patch parts with MergeOnKey mode", ValueType::Number) \
    M(ApplyPatchMergeOnKeyMicroseconds, "Total time spent inside applyPatchMergeOnKey sort-key merge loops", ValueType::Number) \
