...
The server successfully detected this situation and will download merged part from the replica to force the byte-identical result.
    ...
    M(AutoParallelReplicasDecisionSkippedByMode, "Number of times automatic parallel replicas had usable statistics but made no decision, because `automatic_parallel_replicas_mode = 2` only collects statistics and never switches to parallel replicas", ValueType::Number) \
