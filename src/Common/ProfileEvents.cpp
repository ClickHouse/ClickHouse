...
The server successfully detected this situation and will download merged part from the replica to force the byte-identical result.
    ...
    \
    M(AutoParallelReplicasSkippedForcedProjection, "Number of times automatic parallel replicas skipped a query without considering it at all, because `force_optimize_projection` or `force_optimize_projection_name` is set: the plan with parallel replicas is built with projections disabled, so projection usage cannot be guaranteed once it replaces the original plan", ValueType::Number) \
