...
#define APPLY_FOR_BUILTIN_EVENTS(M) \
    ...
    M(TextIndexCoarsenedTokens, "Number of tokens whose posting lists were stored at a coarse (bucket-level) resolution in a text index.", ValueType::Number) \
    M(TextIndexCoarsePostingsExpansions, "Number of coarse posting lists expanded to row-level supersets while reading a text index.", ValueType::Number) \
