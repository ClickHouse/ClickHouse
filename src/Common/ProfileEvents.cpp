...
The server successfully detected this situation and will download merged part from the replica to force the byte-identical result.
    ...
    M(AutoParallelReplicasPlanNotSuitable, "Number of times the plan built with parallel replicas turned out not to be suitable for the optimization, e.g. it could not be matched against the single-node plan, so no decision was made. The `optimizeTree` logger says which case it was", ValueType::Number) \
