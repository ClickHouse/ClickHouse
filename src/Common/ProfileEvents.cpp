...
The server successfully detected this situation and will download merged part from the replica to force the byte-identical result.
    ...
    M(DNSRequests, "Total count of forward DNS requests (host name to addresses) sent to the resolver. Requests served from the DNS cache and host names that are already IP address literals are not counted, so this is the number of requests that actually left the server.", ValueType::Number) \
    M(DNSRequestMicroseconds, "Total time spent in forward DNS requests counted by DNSRequests, including the failed ones counted by DNSRequestError.", ValueType::Microseconds) \
    M(DNSRequestError, "Total count of failed forward DNS requests among the requests counted by DNSRequests. Both requests that ended with an error and requests that returned no addresses are counted. Unlike DNSError, this event has the same boundary as DNSRequests, so a failure rate can be derived from the two.", ValueType::Number) \
