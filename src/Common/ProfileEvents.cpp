...
#define APPLY_FOR_BUILTIN_EVENTS(M) \
    ...
    M(TextIndexDiscardPatternScan, "Number of times pattern-based dictionary scan in a text index was discarded because the number of posting lists to read, or of tokens matched by `hasAnyTokenPrefix`, `hasAnyTokenLike`, `hasAllTokenLike` or `hasTokenMatch`, exceeded its threshold.", ValueType::Number) \
