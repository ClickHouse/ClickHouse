...
#define APPLY_FOR_BUILTIN_EVENTS(M) \
    ...
    M(TextIndexDiscardPatternScan, "Number of times pattern-based dictionary scan in a text index was discarded because the posting lists to read exceeded a threshold, either in number or in the rows they hold.", ValueType::Number) \
    ...
    M(TextIndexDiscardPatternQueryLowSelectivity, "Number of times a LIKE/ILIKE pattern query was bypassed before reading posting lists because its matched tokens provably cover every row of the part, so those postings cannot prune anything.", ValueType::Number) \
