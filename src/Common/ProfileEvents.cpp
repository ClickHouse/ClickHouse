...
The server successfully detected this situation and will download merged part from the replica to force the byte-identical result.
    ...
    M(HTTPUserDirectoryAuthRequests, "Number of logical http user directory authentications that invoke the external HTTP service (one per applicable Basic attempt that reaches the server; a single logical authentication may make several wire requests internally due to retries).", ValueType::Number) \
    M(HTTPUserDirectoryAuthFailures, "Number of Basic external-authentication attempts of an http user directory that fail closed after the network check: external-auth failure, response/schema validation, disallowed/unknown roles, expired valid_until, and materialization failures (unresolvable default_profile, cache-bound rejection). An HTTP 404 (user not found here) is a fallthrough, not a failure, and is not counted; network-policy rejection happens before this stage and is surfaced by IP_ADDRESS_NOT_ALLOWED. The internal already-authenticated path (interserver queries) never contacts the server and is not counted here either, so HTTPUserDirectoryCacheLimitExceeded is not a subset of this counter.", ValueType::Number) \
    M(HTTPUserDirectoryAuthMicroseconds, "Total time spent in external http user directory authentication calls, including retries and failed calls.", ValueType::Microseconds) \
    M(HTTPUserDirectoryUsersCreated, "Number of ephemeral users materialized by http user directories.", ValueType::Number) \
    M(HTTPUserDirectoryCacheLimitExceeded, "Number of new-user materializations rejected by the max_cached_users bound of http user directories. An independent counter, not a subset of HTTPUserDirectoryAuthFailures: it also increments on the internal already-authenticated path (interserver queries), which is outside that counter's scope.", ValueType::Number) \
    \
