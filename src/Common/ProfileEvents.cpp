...
The server successfully detected this situation and will download merged part from the replica to force the byte-identical result.
    ...
    M(ParquetReadRowGroups, "Totally row groups read from parquet data", ValueType::Number) \
    M(ParquetPrunedRowGroups, "Totally row groups pruned from parquet data", ValueType::Number) \
