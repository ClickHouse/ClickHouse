...
The server successfully detected this situation and will download merged part from the replica to force the byte-identical result.
    ...
    M(AutoParallelReplicasRejectedBytesPerReplica, "How many bytes per replica a query turned away by `automatic_parallel_replicas_min_bytes_per_replica` would have read, whichever of the two checks turned it away - `AutoParallelReplicasSkippedEarly` before planning or `AutoParallelReplicasRejectedByThreshold` after the cost model. Take quantiles of it over `system.query_log` to see where the threshold would have to sit for those queries to switch to parallel replicas. The two checks estimate the size differently, the early one from the compressed sizes the parts record and the late one from the bytes actually read, so treat the distribution as approximate", ValueType::Bytes) \
    \
