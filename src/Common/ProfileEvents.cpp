...
#define APPLY_FOR_BUILTIN_EVENTS(M) \
    ...
    M(QueryPlanCacheHits, "Number of times a candidate query plan has been found in the query plan cache. The entry can still be rejected by dependency validation. Only updated for SELECT queries with SETTINGS allow_experimental_query_plan_cache = 1 and enable_query_plan_cache = 1.", ValueType::Number) \
    M(QueryPlanCacheMisses, "Number of times a query plan has not been found in the query plan cache (and required query planning). Only updated for SELECT queries with SETTINGS allow_experimental_query_plan_cache = 1 and enable_query_plan_cache = 1.", ValueType::Number) \
    M(QueryPlanCachePreAnalysisHits, "Number of validated query plan cache hits that skipped analyzer construction. Only updated for SELECT queries with SETTINGS allow_experimental_query_plan_cache = 1 and enable_query_plan_cache = 1.", ValueType::Number) \
    M(QueryPlanCacheValidationMisses, "Number of query plan cache entries rejected because dependency validation failed. Only updated for SELECT queries with SETTINGS allow_experimental_query_plan_cache = 1 and enable_query_plan_cache = 1.", ValueType::Number) \
    M(QueryPlanCacheStaleMisses, "Number of query plan cache entries rejected because the cached plan was stale or corrupt. Only updated for SELECT queries with SETTINGS allow_experimental_query_plan_cache = 1 and enable_query_plan_cache = 1.", ValueType::Number) \
