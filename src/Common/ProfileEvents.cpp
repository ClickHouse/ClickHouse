...
The server successfully detected this situation and will download merged part from the replica to force the byte-identical result.
    ...
    M(AggregationInMemoryMergeInputGroups, "Number of groups across non-empty keyed aggregation variants presented to in-memory final aggregation merges before merge preparation.", ValueType::Number) \
    ...
    M(UniqExactMergeWaves, "Number of parallel conversion or merge dispatches from direct typed single-argument uniqExact implementations that processed at least one state conversion or bucket and completed their aggregation thread-pool barrier. Nullable adaptation, multi-argument and tuple forms, and aggregate combinators are outside this event contract.", ValueType::Number) \
    M(UniqExactMergeWaveInputStates, "Number of uniqExact states presented to reported parallel conversion or merge waves within the UniqExactMergeWaves scope.", ValueType::Number) \
    M(UniqExactMergeWaveElapsedMicroseconds, "Total wall-clock time of reported parallel conversion and merge waves within the UniqExactMergeWaves scope, measured from dispatch through their completion barrier.", ValueType::Microseconds) \
    M(UniqExactMergeWaveCPUTimeMicroseconds, "Total thread CPU time consumed by pooled tasks that processed work in reported parallel conversion and merge waves within the UniqExactMergeWaves scope, where the per-thread CPU clock is available.", ValueType::Microseconds) \
    M(UniqExactMergeWaveWorkers, "Sum of the number of distinct thread-pool workers that processed at least one state conversion or bucket in each reported parallel conversion or merge wave within the UniqExactMergeWaves scope.", ValueType::Number) \
