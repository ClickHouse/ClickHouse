...
#define APPLY_FOR_BUILTIN_EVENTS(M) \
    ...
    M(ReaderExecutorSiblingDeferredBytes, "Bytes ReaderExecutor excluded from a source fetch because a concurrent reader was already downloading them into the cache; a later window serves them from the cache instead of fetching them a second time.", ValueType::Bytes) \
    M(ReaderExecutorSiblingWaits, "Number of times ReaderExecutor waited for a concurrent reader that was downloading the range at the window start, before re-probing the cache. Wait time is counted in FileSegmentWaitMicroseconds.", ValueType::Number) \
