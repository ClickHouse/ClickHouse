...
The server successfully detected this situation and will download merged part from the replica to force the byte-identical result.
    ...
    M(FilesystemCacheFailToReserveSpaceBecauseOfQueryLimit, "Number of times space reservation was skipped because the query reached its filesystem cache write limit (filesystem_cache_query_limit_bytes)", ValueType::Number) \
