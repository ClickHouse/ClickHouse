...
The server successfully detected this situation and will download merged part from the replica to force the byte-identical result.
    ...
    M(ExecutableUDFSharedMemoryInputBytes, "Total bytes of serialized input placed into the shared-memory region of executable user-defined functions. The server serializes them straight into its mapping and the command reads them out of its own, so they are written once and never copied, instead of crossing the process boundary twice as they would through a pipe.", ValueType::Bytes) \
    M(ExecutableUDFSharedMemoryOutputBytes, "Total bytes of serialized output read back from the shared-memory region of executable user-defined functions. The command writes them into its own mapping and the server parses them in place, so they are written once and never copied, instead of crossing the process boundary twice as they would through a pipe.", ValueType::Bytes) \
    ...
    M(ExecutableUDFSharedMemoryScrubbedBytes, "Bytes of a pooled shared-memory region of an executable user-defined function zeroed before a borrow, because the previous borrow of that process belonged to a different user, or to the same user under different roles: what that query left in the region must not be readable by the command while it serves this one.", ValueType::Bytes) \
