...
The server successfully detected this situation and will download merged part from the replica to force the byte-identical result.
    ...
    \
    M(AutoParallelReplicasPlansBuilt, "Number of times automatic parallel replicas built a plan with parallel replicas in order to decide whether it pays off. The plan is not built for every query - it is skipped when the query is already known not to qualify", ValueType::Number) \
    ...
    M(AutoParallelReplicasSkippedByThreshold, "Number of times automatic parallel replicas did not even build the plan with parallel replicas, because index analysis already showed the largest read in the plan gives less than `automatic_parallel_replicas_min_bytes_per_replica` per replica", ValueType::Number) \
