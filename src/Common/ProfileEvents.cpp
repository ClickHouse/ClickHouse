...
The server successfully detected this situation and will download merged part from the replica to force the byte-identical result.
    ...
    M(KeeperRequestRejectedForExceedingMemorySoftLimit, "Number of times a request was rejected due to Keeper server exceeding soft memory limit", ValueType::Number) \
    \
