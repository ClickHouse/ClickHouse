...
#define APPLY_FOR_BUILTIN_EVENTS(M) \
    ...
    M(HashJoinDirectGatheredValues, "Number of right-side output values a hash JOIN emitted by copying them straight from the raw fixed-width data of the stored blocks, instead of through the generic per-row column path.", ValueType::Number) \
