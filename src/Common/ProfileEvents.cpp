...
The server successfully detected this situation and will download merged part from the replica to force the byte-identical result.
    ...
    M(KeeperBatchMaxReadCount, "Number of times the size of batch was limited by the amount", ValueType::Number) \
    M(KeeperBatchMaxReadTotalSize, "Number of times the size of batch was limited by the total bytes size", ValueType::Number) \
