...
#define APPLY_FOR_BUILTIN_EVENTS(M) \
    ...
    M(QueryPrivilegesInfoUpdates, "Number of times a privilege was recorded into the per-query used_privileges / missing_privileges set (eventually flushed to system.query_log). Counts every call to Context::addQueryPrivilegesInfo.", ValueType::Number) \
    M(QueryPrivilegesInfoUpdatesLegacy, "Subset of QueryPrivilegesInfoUpdates: only the recordings that happen on a throwing access check (checkAccess / checkGrantOption). Equal to the historical count before non-throwing isGranted paths also recorded; the delta against QueryPrivilegesInfoUpdates is the audit gap closed.", ValueType::Number) \
