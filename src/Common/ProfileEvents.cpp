...
#define APPLY_FOR_BUILTIN_EVENTS(M) \
    ...
    M(DirectInsertedRows, "Number of rows counted at `INSERT` pipeline hops classified with the `Direct` source. This includes top-level and internal `INSERT` pipelines and fresh forwarded `INSERT` pipelines, such as `Distributed` shard writes. Rows are counted once per pipeline hop, so the same logical row can be counted more than once when an insert is forwarded.", ValueType::Number) \
    M(DirectInsertedBytes, "Number of uncompressed bytes counted at `INSERT` pipeline hops classified with the `Direct` source. This includes top-level and internal `INSERT` pipelines and fresh forwarded `INSERT` pipelines, such as `Distributed` shard writes. Bytes are counted once per pipeline hop, so the same logical bytes can be counted more than once when an insert is forwarded.", ValueType::Bytes) \
    M(MaterializedViewInsertedRows, "Number of rows counted at `INSERT` pipeline hops classified with the `MaterializedView` source. This includes dependent materialized-view output and same-query forwarding hops, such as `Alias`, that preserve this source. Rows are counted once per pipeline hop, so the same logical row can be counted more than once when an insert is forwarded.", ValueType::Number) \
    M(MaterializedViewInsertedBytes, "Number of uncompressed bytes counted at `INSERT` pipeline hops classified with the `MaterializedView` source. This includes dependent materialized-view output and same-query forwarding hops, such as `Alias`, that preserve this source. Bytes are counted once per pipeline hop, so the same logical bytes can be counted more than once when an insert is forwarded.", ValueType::Bytes) \
