...
The server successfully detected this situation and will download merged part from the replica to force the byte-identical result.
    ...
    \
    M(ConcurrencyControlGrantedHard, "Number of CPU slot granted according to guarantee of 1 thread per query and for queries with setting 'use_concurrency_control' = 0", ValueType::Number) \
    M(ConcurrencyControlGrantDelayed, "Number of CPU slot not granted initially and required to wait for a free CPU slot", ValueType::Number) \
    M(ConcurrencyControlAcquiredTotal, "Total number of CPU slot acquired", ValueType::Number) \
    M(ConcurrencyControlAllocationDelayed, "Total number of CPU slot allocations (queries) that were required to wait for slots to upscale", ValueType::Number) \
    \
