...
public:
    ...
    void * alloc(size_t size, size_t alignment = 8) { return CudaHostPinnedMemPool::instance().alloc(size, alignment); }

    /// Free memory range.
    void free(void * buf, size_t /*size*/) { CudaHostPinnedMemPool::instance().free(buf); }
