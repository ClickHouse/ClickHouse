...
StringSearcherBase::~StringSearcherBase() = default;

template <bool CaseSensitive, bool ASCII>
class StringSearcherImpl;
...
template <>
class StringSearcherImpl<false, true> : public StringSearcherBase
{
private:
    /// string to be searched for
    const uint8_t * const needle;
...
template <>
class StringSearcherImpl<false, false> : public StringSearcherBase
{
private:
...
public:
    StringSearcherImpl(const UInt8 * needle_, size_t needle_size_)
        : needle(reinterpret_cast<const uint8_t *>(needle_))
...
template <bool CaseSensitive, bool ASCII>
StringSearcher<CaseSensitive, ASCII>::StringSearcher(const UInt8 * needle_, size_t needle_size)
    : impl(std::make_unique<StringSearcherImpl<CaseSensitive, ASCII>>(needle_, needle_size))
{
}

template <bool CaseSensitive, bool ASCII>
bool StringSearcher<CaseSensitive, ASCII>::compare(const UInt8 * haystack, const UInt8 * haystack_end, const UInt8 * pos) const
{
    return impl->compare(haystack, haystack_end, pos);
}

template <bool CaseSensitive, bool ASCII>
const UInt8 * StringSearcher<CaseSensitive, ASCII>::search(const UInt8 * haystack, const UInt8 * haystack_end) const
{
    return impl->search(haystack, haystack_end);
}

template <bool CaseSensitive, bool ASCII>
const UInt8 * StringSearcher<CaseSensitive, ASCII>::search(const UInt8 * haystack, size_t haystack_size) const
{
    return impl->search(haystack, haystack + haystack_size);
}

template <bool CaseSensitive, bool ASCII>
bool StringSearcher<CaseSensitive, ASCII>::getForceFallback() const
{
    return impl->force_fallback;
}

template <bool CaseSensitive, bool ASCII>
void StringSearcher<CaseSensitive, ASCII>::setForceFallback(bool force_fallback)
{
    impl->force_fallback = force_fallback;
}

template struct StringSearcher<true, true>;
template struct StringSearcher<false, true>;
template struct StringSearcher<true, false>;
template struct StringSearcher<false, false>;
