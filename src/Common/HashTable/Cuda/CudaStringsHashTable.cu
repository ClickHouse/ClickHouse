...
__global__ void kerAddData(
    SizeType str_num,
    char * buf,
    SizeType * offsets,
    SizeType * lens,
    unsigned int seed,
    SizeType buckets_num,
    HashType * hashes,
    SizeType * table_lens,
    Pointer * ptrs,
    SizeType * res_buckets,
    SizeType * new_strings_lens)
{
    SizeType i = blockIdx.x * blockDim.x + threadIdx.x;
    if (!(i < str_num))
        return;
    ...
    while (!hit)
    {
        ...
        {
            HashType old = atomicCAS(
                (unsigned long long int *)&hashes[bucket_i], (unsigned long long int)DBMS_CUDA_EMPTY_HASH_VAL, (unsigned long long int)h);
        ...
        if (hit)
        {
            ...
            {
                Pointer old_ptr;
                old_ptr = reinterpret_cast<Pointer>(atomicCAS(
                    (unsigned long long int *)&ptrs[bucket_i],
    ...
    if (our_string)
        new_strings_lens[bucket_i] = (((len - 1) / 8) + 1) * 8;
}

__global__ void kerCopyAddedStrings(
    SizeType str_num,
    char * buf,
    SizeType * offsets,
    SizeType * lens,
    SizeType * buckets,
    SizeType old_str_buf_sz,
    SizeType * new_strings_offsets,
    Pointer * hash_table_ptrs,
    char * hash_table_str_buf)
{
    SizeType i = blockIdx.x * blockDim.x + threadIdx.x;
    if (!(i < str_num))
        return;
    ...
    Pointer hash_table_old_ptr = hash_table_ptrs[bucket_i], my_ptr = (Pointer) & (buf[offset]);
...
__global__ void
kerCalcTotalAddedLen(SizeType buckets_num, SizeType * new_strings_offsets, SizeType * new_strings_lens, SizeType * res_total_added_len)
{
...
void CudaStringsHashTable::addData(
    SizeType str_num, char * buf, SizeType * offsets, SizeType * lens, SizeType * res_buckets, cudaStream_t stream)
{
    kerFillZero<<<(buckets_num / 256) + 1, 256, 0, stream>>>(buckets_num, buf1->getData());
    kerAddData<<<(str_num / 256) + 1, 256, 0, stream>>>(
        str_num,
        buf,
        offsets,
        lens,
        ...
        buf1->getData());
    CUDA_SAFE_CALL(
        cudaMemcpyFromSymbolAsync(host_table_is_full_flag->getData(), table_is_full, sizeof(bool), 0, cudaMemcpyDeviceToHost, stream));
    ...
    kerCopyAddedStrings<<<(str_num / 256) + 1, 256, 0, stream>>>(
        str_num, buf, offsets, lens, res_buckets, str_buf_sz, buf2->getData(), pointers->getData(), str_buf->getData());

    kerCalcTotalAddedLen<<<1, 1, 0, stream>>>(buckets_num, buf2->getData(), buf1->getData(), cuda_total_added_num->getData());
    CUDA_SAFE_CALL(cudaMemcpyAsync(
        host_total_added_num->getData(), cuda_total_added_num->getData(), sizeof(SizeType), cudaMemcpyDeviceToHost, stream));
    CUDA_SAFE_CALL(cudaStreamSynchronize(stream));
    if ((*host_table_is_full_flag)[0])
    {
        throw std::runtime_error("CudaStringsHashTable::addData: table is full");
    }
    if (str_buf_sz + (*host_total_added_num)[0] > str_buf_max_sz)
        throw std::runtime_error("CudaStringsHashTable::addData: table string buffer is full");
