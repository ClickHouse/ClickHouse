...
public:
    ...
    explicit X509Certificate(X509 *cert_) : certificate(cert_) {}
    explicit operator X509 *() const { return certificate; }

    X509Certificate(const X509Certificate &other)
    {
        certificate = X509_dup(other.certificate);
        if (!certificate)
            throw Exception(ErrorCodes::OPENSSL_ERROR, "X509_dup failed: {}", getOpenSSLErrors());
    }
    ...
    explicit X509Certificate(const std::string & path)
    {
        ...
        if (!certificate)
            throw Exception(ErrorCodes::OPENSSL_ERROR, "PEM_read_bio_X509 failed: {}", getOpenSSLErrors());
...
public:
    ...
    // friend bool operator==(const Subjects & lhs, const Subjects & rhs);
