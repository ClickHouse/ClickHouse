...
static uint32_t DEFAULT_REDIS_CONNECT_TIMEOUT_MS = 500;
...
struct RedisConfiguration
{
    String host;
    uint32_t port;
    uint32_t db_index = DEFAULT_REDIS_DB_INDEX;
    String password = DEFAULT_REDIS_PASSWORD;
    RedisStorageType storage_type;
    uint32_t pool_size = DEFAULT_REDIS_POOL_SIZE;
    uint32_t connect_timeout_ms = DEFAULT_REDIS_CONNECT_TIMEOUT_MS;
    uint32_t operation_timeout_ms = REDIS_LOCK_ACQUIRE_TIMEOUT_MS;
};
