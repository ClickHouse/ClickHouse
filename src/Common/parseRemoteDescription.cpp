...
[[noreturn]] void throwTooManyAddresses(
    const String & func_name, const String & max_addresses_setting, size_t max_addresses, std::optional<size_t> generated)
{
    ...
    const String hint = func_name == "url"
        ? ". Note that 'url' has to read every address generated by the pattern, because HTTP provides no way to"
...
void append(
    ...
    const String & max_addresses_setting)
{
    if (what.empty())
        return;
    ...
    if (what.size() == 1)
    {
        if (to.size() > max_addresses)
            throwTooManyAddresses(func_name, max_addresses_setting, max_addresses, to.size());
        for (auto & elem_to : to)
            elem_to += what.front();
        return;
    }
    ...
    if (to.size() > max_addresses / what.size())
    {
        ...
        throwTooManyAddresses(func_name, max_addresses_setting, max_addresses, generated);
    }
    ...
    const String & max_addresses_setting)
{
        ...
        if (description[i] == '{')
        {
            ...
            if (last_dot != -1)
            {
                ...
                if (right - left + 1 >  max_addresses)
                    throwTooManyAddresses(func_name, max_addresses_setting, max_addresses, right - left + 1);
                ...
                buffer = parseRemoteDescription(description, i + 1, m, separator, max_addresses, func_name, max_addresses_setting);
            ...
            append(cur, buffer, max_addresses, func_name, max_addresses_setting);
            i = m;
        }
    ...
    if (res.size() > max_addresses)
        throwTooManyAddresses(func_name, max_addresses_setting, max_addresses, res.size());
...
{
    /// All the callers (MySQL and PostgreSQL storages, database engines and dictionary sources) take the limit
    /// from `glob_expansion_max_elements`.
    auto addresses = parseRemoteDescription(
        description, 0, description.size(), '|', max_addresses, "remote", GLOB_EXPANSION_MAX_ELEMENTS_SETTING);
