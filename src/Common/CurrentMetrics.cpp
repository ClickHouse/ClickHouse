...
#define APPLY_FOR_BUILTIN_METRICS(M) \
    ...
    M(BuildTextIndexThreads, "Number of threads in the build text index thread pool.") \
    M(BuildTextIndexThreadsActive, "Number of threads in the build text index thread pool running a task.") \
    M(BuildTextIndexThreadsScheduled, "Number of queued or active jobs in the build text index thread pool.") \
