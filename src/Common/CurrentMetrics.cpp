...
#define APPLY_FOR_BUILTIN_METRICS(M) \
    ...
    M(WaitingQuery, "Number of non-internal queries blocked waiting for an asynchronous load or startup job (a table, a database, or the distributed DDL worker), which is what the 'max_waiting_queries' server setting limits") \
