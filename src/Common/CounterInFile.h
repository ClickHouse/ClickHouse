...
{
private:
    /// Fits the widest value and its newline, so the whole record reaches the file in
    /// a single write.
    static constexpr size_t SMALL_READ_WRITE_BUFFER_SIZE = max_int_width<Int64> + 1;

    static_assert(
        SMALL_READ_WRITE_BUFFER_SIZE >= max_int_width<Int64> + 1,
        "a smaller buffer makes writeIntText flush a prefix, so an interrupted writer leaves a "
        "truncated value that reads back as a smaller number");
...
public:
              ...
              const std::function<Int64()> & min_initial_value_provider = {})
    {
        ...
        try
        {
            ...
            {
                /// Used as-is: a hand-written file need not end in a newline, so its
                /// absence must not be read as damage.
            ...
            {
                /// Signed overflow is undefined, and wrapping would hand out a
                /// negative counter, so refuse once the range is used up.
                if (__builtin_add_overflow(res, delta, &res))
                    throw DB::Exception(
                        DB::ErrorCodes::LIMIT_EXCEEDED,
                        "Counter in file {} cannot be advanced by {}: it would exceed the range of Int64",
                        path, delta);
