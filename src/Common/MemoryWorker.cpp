...
MemoryWorker::MemoryWorker(uint64_t period_ms_, bool use_cgroup, std::shared_ptr<PageCache> page_cache_)
...
void MemoryWorker::backgroundThread()
{
    ...
    while (true)
    {
        ...
        if (page_cache)
            page_cache->autoResize(total_memory_tracker.get(), total_memory_tracker.getHardLimit());
