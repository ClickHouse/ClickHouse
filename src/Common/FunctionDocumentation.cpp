...
void formatTypes(std::string & result, const std::vector<std::string> & types)
{
    ...
    for (const auto & type : types)
    {
        ...
        else if (type.starts_with("Float") || type.starts_with("BFloat")) /// matches 'Float32', 'Float64', 'BFloat16' etc
            result += "`](/sql-reference/data-types/float)";
        else if (type.starts_with("Decimal")) /// matches 'Float32', 'Float64', 'BFloat16' etc
            result += "`](/sql-reference/data-types/decimal)"; /// matches 'Decimal(P, S)', 'Decimal32', 'Decimal64' etc
        else if (type == "Date")
