...
template <class Func, bool... Args>
struct BoolArgsToTemplateArgsDispatcher
{
    template <typename... Args1>
    static auto call(Func f, Args1&&... args)
    {
        return f.template operator()<Args...>(std::forward<Args1>(args)...);
    }

    template <class... Args1>
    static auto call(Func f, bool b, Args1&&... ar1)
    {
        if (b)
            return BoolArgsToTemplateArgsDispatcher<Func, Args..., true>::call(f, std::forward<Args1>(ar1)...);
        else
            return BoolArgsToTemplateArgsDispatcher<Func, Args..., false>::call(f, std::forward<Args1>(ar1)...);
    }
};
