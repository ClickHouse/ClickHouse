...
private:
    ...
    struct PathInfo
    {
        ...
        /// Position in the ObjectSharedDataData stream where this path's read ends, so it does not run into the
        /// following paths: the end of the compressed block where the next not requested path begins. Empty
        /// when the read should extend to the range end.
