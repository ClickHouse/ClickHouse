    ...
    SerializeBinaryBulkStatePtr & state) const
{
    ...
    if (settings.native_format && settings.format_settings && settings.format_settings->native.write_json_as_string)
        serialization_version = SerializationVersion(SerializationVersion::STRING);
             ...
             /// With a DEFAULT PATH TYPE that cannot be inside Variant, runtime paths are
             /// Variant(None) presence markers whose missing-ness cannot be represented in the
             /// flattened layout (one value per path per row), so keep the regular serialization
             /// which stores them sparsely in shared data.
             && (!default_path_type || isTypeAllowedInsideVariant(default_path_type)))
...
void SerializationObject::serializeBinary(const IColumn & col, size_t row_num, WriteBuffer & ostr, const FormatSettings & settings) const
{
    ...
    /// presence markers whose values live only in shared data, so they are not serialized as
    /// dynamic paths (they are counted in shared data below).
    const bool dynamic_paths_hold_values = !default_path_type || isTypeAllowedInsideVariant(default_path_type);
    size_t non_null_dynamic_pats = 0;
    if (dynamic_paths_hold_values)
    ...
    for (const auto & [path, column] : dynamic_paths)
    {
        ...
        {
            /// With a DEFAULT PATH TYPE that cannot be inside Variant, the runtime path is a
            /// Variant(None) presence marker and the value lives only in shared data (serialized
            /// below), so skip it here.
            if (default_path_type && !isTypeAllowedInsideVariant(default_path_type))
                continue;
            writeStringBinary(path, ostr);
            if (default_path_type)
            {
                /// DPT runtime paths are Variant(T); serialize the bare nested T value.
                const auto & variant = assert_cast<const ColumnVariant &>(*column);
                default_path_type->getDefaultSerialization()->serializeBinary(variant.getVariantByGlobalDiscriminator(0), variant.getOffsets()[row_num], ostr, settings);
            }
            else
    ...
    try
    {
        ...
        for (size_t i = 0; i != number_of_paths; ++i)
        {
            ...
            if (!shouldSkipPath(path))
            {
                ...
                /// With a DEFAULT PATH TYPE that cannot be inside Variant, runtime paths are
                /// Variant(None) presence markers and their values are serialized as shared data
                /// (see serializeBinary), so treat them as shared data here too.
                else if (auto dynamic_it = dynamic_paths.find(path); dynamic_it != dynamic_paths.end() && (!default_path_type || isTypeAllowedInsideVariant(default_path_type)))
                {
                    /// Check if we already had this path.
                    ...
                    if (default_path_type)
                        deserializeValueIntoVariantPath(default_path_type->getDefaultSerialization(), *dynamic_it->second, istr);
                    else
                        dynamic_serialization->deserializeBinary(*dynamic_it->second, istr, settings);
                }
                /// Try to add a new dynamic paths.
                /// With a DEFAULT PATH TYPE that cannot be inside Variant, values are serialized
                /// as shared data (see serializeBinary), so do not turn them into dynamic paths.
                else if (!default_path_type || isTypeAllowedInsideVariant(default_path_type))
                {
                    if (auto * dynamic_column = column_object.tryToAddNewDynamicPath(path))
                    {
                        if (default_path_type)
                            deserializeValueIntoVariantPath(default_path_type->getDefaultSerialization(), *dynamic_column, istr);
                        else
