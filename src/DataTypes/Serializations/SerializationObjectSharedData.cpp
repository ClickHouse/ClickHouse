    ...
    ISerialization::SerializeBinaryBulkStatePtr & state) const
{
    ...
    else if (serialization_version.value == SerializationVersion::ADVANCED)
    {
        ...
        for (size_t bucket = 0; bucket != buckets; ++bucket)
        {
            auto flattened_paths = buckets_splitter.flattenBucket(bucket, dynamic_type, default_path_type, /* for_shared_data_stream = */ true);
        ...
        if (default_path_type)
        {
            ...
            for (size_t i = nested_offset; i != nested_end; ++i)
                serialization->serializeBinary(values_column, i, *copy_values_stream, FormatSettings{});
        }
