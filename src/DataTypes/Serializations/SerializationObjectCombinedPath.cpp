    ...
    SubstreamsCache * cache) const
{
    ...
    /// subcolumn has type T (dense, missing path reads as default(T)), but the merged result
    /// is Dynamic, so convert T to Dynamic. Missing rows are detected via the value itself:
    /// an explicit NULL (possible only with declared Nullable(T)) means a missing path.
