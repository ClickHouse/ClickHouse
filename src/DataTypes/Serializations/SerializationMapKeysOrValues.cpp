    ...
    SubstreamsCache * cache) const
{
    ...
    else
    {
        ...
        for (size_t bucket = 0; bucket != buckets_info_state_concrete->buckets; ++bucket)
        {
            ...
            keys_or_values_serialization->deserializeBinaryBulkWithMultipleStreams(*mutable_bucket, buckets_rows_offset, buckets_limit, settings, map_keys_or_values_with_buckets_state->bucket_keys_or_values_states[bucket], cache);
        ...
        if (map_keys_or_values_with_buckets_state->has_bucket_index)
        {
            ...
            if (reorder_with_skipped_rows)
            {
                ...
                if (whole_range_column->size() > rows_offset)
                    column.insertRangeFrom(*whole_range_column, rows_offset, whole_range_column->size() - rows_offset);
            }
            else
            {
