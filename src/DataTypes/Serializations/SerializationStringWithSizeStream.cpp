...
void serializeStringSizes(const IColumn & column, WriteBuffer & ostr, UInt64 offset, UInt64 limit)
{
    ...
    if constexpr (std::endian::native == std::endian::big)
    {
        for (size_t i = offset; i < end; ++i)
        {
            ...
            writeBinaryLittleEndian(current_offset - prev_offset - 1, ostr);
    ...
    else
    {
        ...
        for (size_t i = 0; i < count; ++i)
        {
            ...
            sizes[i] = current_offset - prev_offset - 1;
            prev_offset = current_offset;
        }
    ...
    SubstreamsCache * cache) const
{
    ...
    if (cached_column)
    {
        ...
        skipped_bytes = column_string_and_size.getSkippedBytes();
    }
    ...
    else if (ReadBuffer * stream = settings.getter(settings.path))
    {
        ...
        addToOrUpdateSubstreamsCache(cache, cache_path, ColumnStringAndSize::create(column, column_size, nullptr, column_partial_size));
    }
