    ...
    SubstreamsCache * cache) const
{
    ...
    else if (auto * discriminators_stream = settings.getter(settings.path))
    {
        ...
        {
            size_t discriminators_offset = col.getLocalDiscriminatorsPtr()->size();
    ...
    for (size_t i = 0; i != variants.size(); ++i)
    {
        ...
        variants[i]->deserializeBinaryBulkWithMultipleStreams(
            col.getVariantPtrByLocalDiscriminator(i), discriminators_state->variant_rows_offsets[i], variant_limits[i],
    ...
    DeserializeBinaryBulkStateVariantDiscriminators & state) const
{
    ...
    auto & variant_rows_offsets = state.variant_rows_offsets;
    variant_rows_offsets.clear();
    variant_rows_offsets.resize(variants.size(), 0);
