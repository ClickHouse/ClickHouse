...
protected:
    ...
    /// Type and serialization used for the FLATTENED native serialization of runtime paths. For
    /// JSON(DEFAULT PATH TYPE T) with a type that cannot be inside Variant this is bare T (a
    /// Variant(None) marker cannot carry the value, and the flattened format has no separate
    /// shared-data stream); otherwise it matches runtime_path_serialization.
