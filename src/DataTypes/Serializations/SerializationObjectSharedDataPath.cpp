    ...
    ISerialization::SubstreamsCache * cache) const
{
             ...
             || serialization_version.value == SerializationObjectSharedData::SerializationVersion::ADVANCED_CHUNKED)
    {
        ...
        auto chunk_structures = SerializationObjectSharedData::deserializeStructure(rows_offset, limit, settings, *shared_data_structure_state, cache);
        ...
        for (size_t chunk_idx = 0; chunk_idx != chunk_structures->size(); ++chunk_idx)
        {
            ...
            {
                column->assumeMutable()->insertManyDefaults(chunk_structure.limit);
                continue;
            }
            ...
            if (path_data_it != paths_data_chunk.paths_data.end())
            {
                ...
                if (path_subcolumn.empty())
                {
                    column->assumeMutable()->insertRangeFrom(*path_data_it->second, chunk_structure.offset, chunk_structure.limit);
                }
                ...
                else
                {
                    ...
                    column->assumeMutable()->insertRangeFrom(*subcolumn, chunk_structure.offset, chunk_structure.limit);
                }
            }
            ...
            else
            {
                ...
                column->assumeMutable()->insertRangeFrom(*subcolumn_data_it->second, chunk_structure.offset, chunk_structure.limit);
            }
        }
