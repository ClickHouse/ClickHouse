...
String IDataType::getSubcolumnNameForZeroArrayLevel(std::string_view subcolumn_name, const SubstreamData & resolved, size_t initial_array_level)
{
    if (!resolved.is_array_sizes)
        return String(subcolumn_name);

    /// `ArraySizes` is terminal, so the number is always in the last component, and it was assigned
    /// as `initial_array_level` plus the depth of the sizes inside the resolved type, which is the
    /// number they get at level 0.
    ...
    static constexpr std::string_view size_prefix = "size";
    if (!last_component.starts_with(size_prefix))
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Subcolumn '{}' resolved to array sizes, but its name is not 'sizeN'", subcolumn_name);

    size_t level = parse<size_t>(last_component.substr(size_prefix.size()));
    if (level < initial_array_level)
        throw Exception(
            ErrorCodes::LOGICAL_ERROR,
            "Subcolumn '{}' resolved to array sizes at array level {}, which is above the level {} it was resolved at",
            subcolumn_name, level, initial_array_level);

    return String(prefix) + String(size_prefix) + toString(level - initial_array_level);
}
