...
protected:
    ...
    /// other name depends on the level, hence `resolved_data` rather than the name alone: only it
    /// tells whether the name resolved to the array sizes at all.
    static String getSubcolumnNameForZeroArrayLevel(
        std::string_view subcolumn_name,
        const SubstreamData & resolved_data,
        size_t initial_array_level);
