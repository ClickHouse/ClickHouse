...
`SHARED REGEXP` uses partial matching against the full flattened path, so use `^...$` to match the whole path. A path inside an object, like `a.b.c`, is matched by its full path, but paths of objects inside arrays are not matched. Typed paths are not affected by `SHARED REGEXP`, and paths matched by `SKIP` or `SKIP REGEXP` are still skipped. Changing only `SHARED REGEXP` with `ALTER TABLE ... MODIFY COLUMN` does not rewrite data: new inserts use the new rules immediately, and existing data parts use them at their next merge.
