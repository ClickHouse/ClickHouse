...
DataTypePtr DataTypeObject::getTypeOfRuntimePaths(const DataTypePtr & default_path_type)
{
    ...
    /// Variant cannot contain Nullable/Dynamic/Variant/Object. For such default path types
    /// the runtime path uses Variant(None) as a sparse marker (missing path = NULL discriminator)
    /// and the actual values are kept in shared data.
    if (!isTypeAllowedInsideVariant(default_path_type))
        return std::make_shared<DataTypeVariant>(DataTypes{std::make_shared<DataTypeNothing>()}, DataTypeVariant::AllowNothingVariant{});
...
With `DEFAULT PATH TYPE T`, runtime-discovered subcolumns store their values as `T`, and shared data has type `Array(Tuple(String, T))`. Missing paths read as `default(T)`: for example, `0` for `Int64` and an empty string for `String`. The type is never implicitly made `Nullable`; use `Nullable(T)` when missing values should read as `NULL`. JSON null input is treated as a missing path.
