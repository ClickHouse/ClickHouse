...
`SHARED REGEXP` uses partial matching against the full flattened path, so use `^...$` to match the whole path. A path inside an object, like `a.b.c`, is matched by its full path, but paths of objects inside arrays are not matched. Typed paths are not affected by `SHARED REGEXP`, and paths matched by `SKIP` or `SKIP REGEXP` are still skipped. Changing only `SHARED REGEXP` of a `JSON`, `Array(JSON)` or `Nullable(JSON)` column with `ALTER TABLE ... MODIFY COLUMN` does not rewrite data parts. New inserts use the new rules immediately. Parts written before the change keep their old layout on disk until their next merge or mutation, and reading them converts the column to the new type, which moves matched paths to shared data and unmatched paths back to dynamic paths. The rules are applied only when a path is added to a column: if a client sends a block in the `Native` format with the same `JSON` type and matched paths stored as dynamic paths, the inserted part keeps them as dynamic paths until its next merge.
