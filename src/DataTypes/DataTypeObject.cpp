...
std::unique_ptr<ISerialization::SubstreamData> DataTypeObject::getDynamicSubcolumnData(std::string_view subcolumn_name, const SubstreamData & data, size_t initial_array_level, bool throw_if_null) const
{
    ...
    if (!path_subcolumn.empty())
    {
        ...
        /// SerializationObjectDynamicPath resolves this name again against the Dynamic type alone
        /// to extract the subcolumn from a path read from the shared data. Only a dynamic path
        /// keeps the name, but rewriting it for a typed path is harmless: it is unused there.
        path_subcolumn = getSubcolumnNameForZeroArrayLevel(path_subcolumn, *res, initial_array_level);
    }
