...
{
    return recursiveConvertType<DataTypeLowCardinality>(type, [](const auto & type_lc)
    {
        return type_lc.getDictionaryType();
    });
}

ColumnPtr recursiveRemoveLowCardinality(const ColumnPtr & column)
{
    return recursiveConvertColumn<ColumnLowCardinality>(column, [](const auto & column_lc)
    {
        return column_lc.convertToFullColumn();
    });
}
