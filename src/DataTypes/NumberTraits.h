...
template <typename A, typename B> struct ResultOfModulo
{
    ...
    using Type = std::conditional_t<is_floating_point_v<A> || is_floating_point_v<B>, Float64, Type0>;
};
...
template <typename A, typename B> struct ResultOfBit
{
    using Type = typename Construct<
        is_signed_v<A> || is_signed_v<B>,
        false,
        is_floating_point_v<A> || is_floating_point_v<B> ? 8 : max(sizeof(A), sizeof(B))>::Type;
};
...
struct ResultOfIf
{
    static constexpr bool has_float = is_floating_point_v<A> || is_floating_point_v<B>;
    ...
    static constexpr size_t max_size_of_float = max(is_floating_point_v<A> ? sizeof(A) : 0, is_floating_point_v<B> ? sizeof(B) : 0);
...
template <typename A> struct ToInteger
{
    using Type = typename Construct<
        is_signed_v<A>,
        false,
        is_floating_point_v<A> ? 8 : sizeof(A)>::Type;
};
