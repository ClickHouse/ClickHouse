...
static DataTypePtr create32(const ASTPtr & arguments)
{
    ...
    if (arguments->children.size() != 1)
        throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "DateTime32 data type can optionally have only one argument - time zone name");
...
static DataTypePtr create64(const ASTPtr & arguments)
{
    ...
    if (arguments->children.size() > 2)
        throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "DateTime64 data type can optionally have two argument - scale and time zone name");
