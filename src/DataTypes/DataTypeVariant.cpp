...
:::note
Before version `26.8`, the functions that accept a `Variant` natively aggregated those rows as ordinary values: `count` counted them, `any` could return NULL from a group that has non-NULL values, `groupArray` stored the NULLs, and the `uniq` family counted NULL as a distinct value. Set `aggregate_functions_skip_variant_nulls = 0` (or `SET compatibility = '26.7'`) to restore the previous behavior. The setting does not affect `-SimpleState` or declared `SimpleAggregateFunction(..., Variant(...))` columns, which always preserve that historical behavior so values can be safely re-aggregated during table loading and background merges.
...
That rule also holds for the `-Distinct` combinator, whose state stores the distinct argument values themselves and replays them into the nested function when states are merged. The replay is deliberately independent of the setting: the NULL values a `...Distinct` state already contains are always aggregated on replay, exactly as the versions before `26.8` aggregated them, so an already written state — for example a stored `countDistinctState` over a `Variant` — returns the same result under either value of the setting. The setting only controls whether the NULL rows of newly aggregated data enter the state.
