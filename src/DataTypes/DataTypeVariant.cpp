...
:::note
Before version `26.9`, the functions that accept a `Variant` natively aggregated those rows as ordinary values: `count` counted them, `any` could return NULL from a group that has non-NULL values, `groupArray` stored the NULLs, and the `uniq` family counted NULL as a distinct value. Set `aggregate_functions_skip_variant_nulls = 0` (or `SET compatibility = '26.8'`) to restore the previous behavior. The setting does not affect `-SimpleState` or declared `SimpleAggregateFunction(..., Variant(...))` columns, which always preserve that historical behavior so values can be safely re-aggregated during table loading and background merges.
...
That rule also holds for the `-Distinct` combinator, whose state stores the distinct argument values themselves and replays them into the nested function when states are merged. The replay is deliberately independent of the setting: the NULL values a `...Distinct` state already contains are always aggregated on replay, exactly as the versions before `26.9` aggregated them, so an already written state — for example a stored `countDistinctState` over a `Variant` — returns the same result under either value of the setting. The setting only controls whether the NULL rows of newly aggregated data enter the state.

Watch out for the persisted `AggregateFunction(f, Variant(...))` states — the intermediate states of materialized views and `AggregatingMergeTree` tables. The type name and the state layout are the same in both modes, so nothing stops a merge from combining a state written before the upgrade (which aggregated the NULL rows) with a state written after it (which skipped them), and the merged result follows neither contract consistently: for example, `countMerge` over such a mix counts the NULL rows of the old parts but not of the new ones. If a query over mixed states must keep returning the pre-`26.9` result, set `aggregate_functions_skip_variant_nulls = 0` before inserting new data, so that the new states are accumulated under the same rule as the historical ones; otherwise, expect the queries over the existing states to converge to the new NULL-skipping behavior only after the historical data is rewritten (`OPTIMIZE TABLE ... FINAL` does not help — re-merging old states does not remove the NULL rows already aggregated into them; the data has to be re-inserted from the source).
:::
