...
SerializationPtr DataTypeVariant::doGetDefaultSerialization() const
{
    ...
    return std::make_shared<SerializationVariant>(std::move(serializations), std::move(variant_names), variants, SerializationVariant::getVariantsDeserializeTextOrder(variants), getName());
}
