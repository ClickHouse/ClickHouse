...
The name `UUID` resolves to either the `UUID` type (version 1, the historical behavior) or the `UUID2` type (version 2), controlled by the [`uuid_type_version`](/reference/settings/session-settings/other#uuid_type_version) setting:
...
A type name carried in a string is recognized only when the string is a string literal, or a constant expression built from the string functions the normalizer folds on its own: `concat`, `replace`, `replaceOne`, `replaceAll`, `upper`, `ucase`, `lower` and `lcase`. A type name produced by any other expression, such as `CAST(x, trim(' UUID '))`, keeps the historical `UUID`, the same as a type name built at run time from a non-constant expression.
