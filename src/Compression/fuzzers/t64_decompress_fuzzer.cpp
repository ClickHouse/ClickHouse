...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t * data, size_t size)
{
    try
    {
        ...
        /// T64 reads the TypeIndex and Variant from source[0] (the cookie byte)
        /// during decompression, so construction parameters do not affect the paths.
        auto codec = CompressionCodecFactory::instance().get(
            static_cast<uint8_t>(CompressionMethodByte::T64));
