...
{
    size_t compressed_size = ZSTD_compress2(zstd_cctx, dest, ZSTD_compressBound(source_size), source, source_size);
...
{
    UInt8 method_code = static_cast<UInt8>(CompressionMethodByte::ZSTD_QAT);
...
CompressionCodecZSTDQAT::CompressionCodecZSTDQAT(int level_)
    ...
    , level(level_)
    , log(&Poco::Logger::get("CompressionCodecQATZSTD"))
{
    ...
    if (qat_state == QZSTD_OK)
    {
        qat_seq_cctx = QZSTD_createSeqProdState();
        ZSTD_registerSequenceProducer(zstd_cctx, qat_seq_cctx, qatSequenceProducer);
        ZSTD_CCtx_setParameter(zstd_cctx, ZSTD_c_enableSeqProducerFallback, 1);
    }
    else
        qat_seq_cctx = nullptr;
}

CompressionCodecZSTDQAT::~CompressionCodecZSTDQAT()
{
    QZSTD_freeSeqProdState(qat_seq_cctx);
