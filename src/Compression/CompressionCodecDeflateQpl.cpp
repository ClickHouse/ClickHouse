...
char * DeflateQplJobHWPool::hw_jobs_buffer;
    ...
    , distribution(0, MAX_HW_JOB_NUMBER - 1)
{
    ...
    {
        LOG_DEBUG(log,"[ZJJDBG]mlockall failed to lock memory!");
    }
    else
    {
        LOG_DEBUG(log, "[ZJJDBG]mlockall memory!");
    }*/
    ...
    hw_jobs_buffer = reinterpret_cast<char *>(mmap(nullptr, job_size * MAX_HW_JOB_NUMBER, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_PRIVATE | MAP_ANONYMOUS | MAP_POPULATE, -1, 0));
    //mlock(hw_jobs_buffer, job_size * MAX_HW_JOB_NUMBER);
    ...
    {
        qpl_job * qpl_job_ptr = reinterpret_cast<qpl_job *>(hw_jobs_buffer + index * job_size);
...
DeflateQplJobHWPool::~DeflateQplJobHWPool()
{
    ...
    // Unlock all the memory locked by getInstMem
    /*if (munlockall() == -1)
    {
        printf("[ZJJDBG]munlockall failed!");
    }*/

    UInt32 job_size = 0;
    qpl_get_job_size(qpl_path_hardware, &job_size);
    //munlock(hw_jobs_buffer, job_size * MAX_HW_JOB_NUMBER);
    munmap(hw_jobs_buffer, job_size * MAX_HW_JOB_NUMBER);
