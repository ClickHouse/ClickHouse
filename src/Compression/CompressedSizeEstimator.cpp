...
UInt32 CompressedSizeEstimator::getCompressedBlockSize(
    const ICompressionCodec & codec, const char * src, UInt32 src_size, PODArray<char> & scratch)
{
    ...
    if (auto predicted = codec.tryGetCompressedSize(src, src_size))
        return ICompressionCodec::getHeaderSize() + *predicted;
