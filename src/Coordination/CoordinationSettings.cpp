...
#define LIST_OF_COORDINATION_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(Float, hash_map_min_load_factor, 0, "Minimal load factor for the in-memory storage hash map. When set above 0, the index is automatically rehashed to reclaim memory once its load factor drops below this value (and the node count exceeds 'min_node_count_for_auto_optimize'). Automatic rehashing is evaluated when nodes are inserted and during the post-snapshot cleanup; it is intentionally not triggered on individual deletes (a rehash is O(N), so doing it per delete would make a large delete batch O(N^2)). Use the 'clrs' four-letter-word command to force an immediate rehash. 0 disables automatic rehashing", 0) \
    DECLARE(UInt64, min_node_count_for_auto_optimize, 0, "Minimal node count for automatic storage optimization (rehashing of the in-memory index, see 'hash_map_min_load_factor'). The index is not auto-rehashed while it holds fewer nodes than this", 0) \
