...
#define LIST_OF_COORDINATION_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(Milliseconds, slow_member_backpressure_max_duration_ms, 0, "How long the leader holds the commit index back for a replica that fell behind by more than `stale_log_gap` entries and stopped catching up, before giving up on it. While it is active, clients are not acknowledged, which stops the leader running further ahead of that replica and lets it catch up instead of drifting until it needs a snapshot. Nothing happens unless the backpressure is switched on with the `bpon` four letter command, and 0 disables it entirely. Keep it below `operation_timeout_ms`, as no write commits while a replica is being waited for.", 0) \
    DECLARE(Milliseconds, slow_member_backpressure_gap_window_ms, 0, "How long a replica gap has to stop shrinking before the leader starts holding the commit index for it. Being behind is not by itself a reason to wait: a replica draining a backlog after a restart, after installing a snapshot, or right after joining is far behind and recovering on its own, and waiting for it would stall the cluster for nothing. 0 uses four heartbeat intervals. Only meaningful together with `slow_member_backpressure_max_duration_ms`.", 0) \
    DECLARE(UInt64, slow_member_backpressure_max_uncommitted_log_entries, 0, "The value `nuraft_max_uncommitted_log_entries` takes while the leader is holding the commit index for a replica. Holding the commit index does not stop the leader appending, so without this the log runs further ahead and the replica can fall further behind instead of catching up; tightening the limit makes the leader refuse new requests with a retriable error until the replica closes the gap. It also bounds how many entries are left appended but unacknowledged. 0 leaves the limit unchanged.", 0) \
