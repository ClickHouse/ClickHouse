...
#define LIST_OF_COORDINATION_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(UInt64, snapshot_deser_threads, 0, "Number of threads for parallel V8 snapshot decompression and parse during load. 0 = auto (min(4, cores/2), capped at 8). 1 disables parallelism and uses the calling thread.", 0) \
