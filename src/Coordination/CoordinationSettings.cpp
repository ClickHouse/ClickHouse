...
#define LIST_OF_COORDINATION_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(UInt64, min_request_size_for_cache, 50 * 1024, "Obsolete setting, does nothing.", SettingsTierType::OBSOLETE) \
    ...
    DECLARE(Bool, use_batched_log_entries, false, "Serialize each batch of write requests as a single Raft log entry instead of one entry per request. Enable only after all servers in the cluster are upgraded to a version that supports the batched format. Requires use_new_dispatcher.", HOT_RELOAD) \
