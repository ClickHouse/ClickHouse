...
#define LIST_OF_COORDINATION_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(Bool, read_only_waits_same_session_write, false, "When enabled, read requests only wait for uncommitted writes from the same session instead of any session. Improves read latency with many concurrent sessions (e.g. pooled ZooKeeper connections).", 0) \
