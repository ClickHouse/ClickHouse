...
#define LIST_OF_COORDINATION_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(Milliseconds, slow_member_backpressure_max_hold_ms, 0, "How long the leader holds the commit index back for a member that fell behind by more than `stale_log_gap` log entries, before giving up on it and leaving it behind. While the commit index is held, clients are not acknowledged, which stops the leader from outrunning that member and lets it catch up instead of drifting until it needs a snapshot. A member that does not respond, or that is receiving a snapshot, is never waited for, since it cannot catch up any faster. 0 disables the backpressure. Keep it below `operation_timeout_ms`, as no write commits while a member is being waited for.", 0) \
    DECLARE(Milliseconds, slow_member_backpressure_gap_window_ms, 0, "How long a member's gap has to stop shrinking before the leader starts holding the commit index for it. Being behind is not by itself a reason to wait: a member draining a backlog after a restart, after installing a snapshot, or right after joining is far behind and recovering on its own, and waiting for it would stall the cluster for nothing. Every time the gap reaches a new low the window restarts. 0 uses four heartbeat intervals. Only meaningful together with `slow_member_backpressure_max_hold_ms`.", 0) \
...
"jmst,jmfp,jmep,jmdp,"
#endif
"conf,cons,crst,envi,ruok,srst,srvr,stat,wchs,dirs,mntr,isro,rcvr,apiv,csnp,lgif,rqld,rclc,clrs,ftfl,ydld,fcon,fcof,pfev,lgrq";
