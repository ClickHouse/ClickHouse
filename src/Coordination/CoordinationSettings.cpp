...
#define LIST_OF_COORDINATION_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(UInt64, snapshot_deser_threads, 0, "Number of threads for parallel V8 snapshot decompression and parse during load. 0 = auto (clamp(cores/2, 1, 4)); explicit values are clamped to [1, 8]. 1 disables parallelism and uses the calling thread.", 0) \
    DECLARE(UInt64, snapshot_chunk_size, 100000, "Number of nodes per independently-compressed ZSTD frame in V8 snapshots. Only affects write_snapshot_version=8. Distinct from snapshot_transfer_chunk_size which controls network transfer bytes.", 0) \
