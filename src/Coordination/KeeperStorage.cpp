...
void KeeperStorage::UncommittedState::rollbackDelta(const Delta & delta)
{
    ...
    std::visit(
        [&]<typename DeltaType>(const DeltaType & operation)
        {
            ...
            {
                chassert(node);
                storage.acl_map.removeUsage(node->stats.acl_id);
                node = nullptr;
            }
...
bool KeeperStorage::removeNode(const std::string & path, int32_t version, bool update_digest)
{
    auto node_it = container.find(path);
    if (node_it == container.end())
        return false;
    ...
    if (prev_node.stats.getEphemeralOwner() != 0)
    {
        chassert(committed_ephemeral_nodes != 0);
        --committed_ephemeral_nodes;
        std::lock_guard lock(ephemeral_mutex);
        unregisterEphemeralPath(committed_ephemerals, prev_node.stats.getEphemeralOwner(), path, /*throw_if_missing=*/true);
    }
...
bool KeeperStorage::checkCommittedACL(std::string_view path, int32_t permission, int64_t session_id)
{
    ACLId acl_id = 0;
    {
        ...
        auto node_it = container.find(path);
        if (node_it != container.end())
            acl_id = node_it->value.stats.acl_id;
    }
...
public:
    ...
    CollectStatus collect(std::string_view root_path, const KeeperStorage::Node & root_node)
    {
        ...
        addResult(root_path, root_node.stats, root_node.stats.acl_id);
...
private:
    CollectStatus visitMemNode(std::string_view current_path, const ProcessedUncommittedChildren & processed_uncommitted_children)
    {
        ...
        for (const auto & child_name : children)
        {
            ...
            addResult(child_path, child_node.stats, child_node.stats.acl_id);
        }
    ...
    CollectStatus visitUncommitted(const std::string & path, ProcessedUncommittedChildren & processed_uncommitted_children)
    {
             ...
             ++nodes_it)
        {
            ...
            addResult(std::string{node_path}, node_ptr->stats, node_ptr->stats.acl_id);
        }

        return CollectStatus::Ok;
    }

    void addResult(std::string_view path, const KeeperNodeStats & stats, ACLId acl_id)
    {
        std::optional<int64_t> ephemeral_owner;
        if (stats.isEphemeral())
            ephemeral_owner = stats.getEphemeralOwner();
        result.push_back({std::string{path}, acl_id, ephemeral_owner});
    }

    bool checkLimits(const KeeperStorage::Node & node)
    {
        nodes_observed += node.stats.getNumChildren();
...
processLocal(const Coordination::ZooKeeperListRequest & zk_request, KeeperStorage & storage, int64_t session_id, bool check_acl)
{
    ...
    if (!zk_request.path.starts_with(keeper_system_path) && static_cast<size_t>(node_it->value.stats.getNumChildren()) != children->size())
    {
        throw Exception(
            ErrorCodes::LOGICAL_ERROR,
    ...
    for (const auto & child : *children)
    {
        if (Coordination::ListRequestType::ALL == list_request_type || add_child(child))
        {
            ...
            if (with_stat || with_data || check_child_acl)
            {
                ...
                if (check_child_acl && !storage.checkACL(child_it->value.stats.acl_id, Coordination::ACL::Read, session_id, /*committed=*/ true))
                    return auth_error();

                if (with_stat)
                {
                    Coordination::Stat child_stat;
                    child_it->value.stats.setResponseStat(child_stat);
