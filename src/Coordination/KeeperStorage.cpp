...
private:
    CollectStatus visitRocksDBNode(std::string_view current_path, const UncommittedChildren & uncommitted_children) requires Storage::use_rocksdb
    {
    ...
    CollectStatus visitMemNode(std::string_view current_path, const UncommittedChildren & uncommitted_children) requires (!Storage::use_rocksdb)
    {
        std::lock_guard lock(storage.storage_mutex);
