...
TYPED_TEST(CoordinationTest, TestStorageSnapshotTTLRoundTrip)
{
    ...
    DB::KeeperStorageSnapshot<Storage> snapshot(&storage, zxid, nullptr, DB::SnapshotVersion::V9);
    auto buf = manager.serializeSnapshotToBuffer(snapshot);
...
TEST(CoordinationChunkedSnapshotTest, MetadataTrailingBytesRejected)
{
    ...
    DB::KeeperStorageSnapshot<DB::KeeperMemoryStorage> snap(&storage, /*up_to_log_idx=*/7, cluster_cfg, DB::SnapshotVersion::V9);
    DB::KeeperSnapshotManager<DB::KeeperMemoryStorage> mgr(3, keeper_context, /*compress_snapshots_zstd_=*/true);
    auto buf = mgr.serializeSnapshotToBuffer(snap);
    ASSERT_NE(buf, nullptr);
