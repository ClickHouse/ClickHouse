    ...
    bool enable_compression)
{
    ...
    auto state_machine = std::make_shared<KeeperStateMachine<Storage>>(noop_router, snapshots_queue, keeper_context, nullptr);
    ...
    auto restore_machine = std::make_shared<KeeperStateMachine<Storage>>(noop_router, snapshots_queue1, keeper_context, nullptr);
...
TYPED_TEST(CoordinationTest, TestEphemeralNodeRemove)
{
    ...
    auto state_machine = std::make_shared<KeeperStateMachine<Storage>>(noop_router, snapshots_queue, this->keeper_context, nullptr);
    state_machine->init();
