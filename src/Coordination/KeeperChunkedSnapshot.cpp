...
ParseResult parseChunkedSnapshotImpl(SeekableReadBuffer & in, bool check_version)
{
    ...
    for (uint64_t i = 0; i < chunk_count; ++i)
    {
        ...
        // Unknown future chunk types are stored as-is; readers skip them when loading.
        d.type = type_raw <= static_cast<uint8_t>(SnapshotChunkType::NODES) ? static_cast<SnapshotChunkType>(type_raw)
                                                                            : SnapshotChunkType::UNKNOWN;
