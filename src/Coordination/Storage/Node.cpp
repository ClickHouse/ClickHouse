...
NodeRef BlockData::appendNodeNoResize(BlockPtr block, FullNode & node)
{
    ...
    *p = char(uint8_t(node.action));
    ...
    DB::GroupVarint4x32::encode(p, path_prefix_size, path_depth_delta, node.stats.num_children_and_is_ephemeral, 0);
    ...
    std::string * out_path_buf, uint64_t * out_digest, NodeAction * out_action, uint32_t * out_serialized_size)
{
    ...
    if (out_stats || out_path)
    {
        ...
        uint32_t unused = 0;
        DB::GroupVarint4x32::decode(varints, path_prefix_size, path_depth_delta, num_children_and_is_ephemeral, unused);

        if (out_stats)
            out_stats->num_children_and_is_ephemeral = num_children_and_is_ephemeral;
