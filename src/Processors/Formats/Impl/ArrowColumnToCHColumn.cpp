    ...
    const std::optional<std::unordered_map<String, String>> & clickhouse_columns_to_parquet)
{
    switch (arrow_column->type()->id())
    {
        ...
        case arrow::Type::MAP:
        {
            ...
            /// A Map's single Arrow child is that `entries` struct, which is what was just unwrapped.
            ...
            const auto arrow_entries_field = (arrow_field && arrow_field->type()->id() == arrow::Type::MAP)
                ? arrowChildField(arrow_field, 0)
                : assert_cast<const arrow::MapType *>(arrow_column->type().get())->value_field();
        ...
        case arrow::Type::FIXED_SIZE_LIST:
        {
            ...
            const auto arrow_item_field = [&]() -> std::shared_ptr<arrow::Field>
            {
                /// Prefer the schema field's type tree over the data one: the empty column built for
                /// schema inference has its type tree rebuilt from storage types, which loses per-field
                /// metadata and unwraps extension types, while the schema field keeps them.
                if (arrow_field && arrow_field->type()->id() == arrow_column->type()->id())
                    return arrowChildField(arrow_field, 0);

                const auto & arrow_list_like_type = arrow_column->type();
                switch (list_type)
                {
                    case ListType::LargeList:
