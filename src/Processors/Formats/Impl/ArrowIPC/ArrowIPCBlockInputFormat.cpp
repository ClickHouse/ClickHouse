...
| `output_format_arrow_unsupported_types`                                                                                  | What to write for a type that has no Arrow equivalent (e.g. `JSON`, `Dynamic`, `QBit`, `AggregateFunction`): `throw`, `text` (one `serializeText` value per row, as Arrow `Utf8`) or `binary` (one `serializeBinary` value per row, as Arrow `Binary`). | `binary`     |
