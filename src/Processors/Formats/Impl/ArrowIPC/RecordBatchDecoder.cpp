...
DataTypePtr matchColumnNullability(const DataTypePtr & type, const ColumnPtr & column)
{
    ...
    if (col_nullable)
    {
        if (t->canBeInsideNullable())
            return std::make_shared<DataTypeNullable>(t);
        if (typeid_cast<const DataTypeArray *>(t.get()))
            return makeNullableAllowingArray(t);
    }
    return t;
}
    ...
    const ArrowField & field, bool allow_low_cardinality, const DataTypePtr & target_hint, const String & path)
{
    ...
    /// Only wrap in Nullable when the type allows it. A Struct (Tuple) or Array is wrapped only
    /// when the corresponding experimental setting is enabled, or when the requested target type is
    /// already nullable (e.g. reading into an existing `Nullable(Tuple)` or `Nullable(Array)` column).
    /// Otherwise the outer validity is dropped and `decodeColumns` reconciles the reported type
