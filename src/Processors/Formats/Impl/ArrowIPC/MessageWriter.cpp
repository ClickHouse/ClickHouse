...
void writeInt32LE(WriteBuffer & out, Int32 value)
{
...
{
    const Int64 start = bytes_written;
    ...
    writeInt32LE(out, static_cast<Int32>(-1)); /// continuation token 0xFFFFFFFF
    writeInt32LE(out, static_cast<Int32>(padded_metadata));
    ...
    const Int32 metadata_length = static_cast<Int32>(2 * sizeof(Int32) + padded_metadata);
    bytes_written += metadata_length + static_cast<Int64>(body_size);
    return WrittenMessage{start, metadata_length, static_cast<Int64>(body_size)};
}

void MessageWriter::writeEOS()
{
    writeInt32LE(out, static_cast<Int32>(-1));
    writeInt32LE(out, 0);
    bytes_written += 2 * sizeof(Int32);
}

void MessageWriter::writeRaw(const char * data, size_t size)
{
    out.write(data, size);
    bytes_written += static_cast<Int64>(size);
}
