...
void SQLiteOutputFormat::writePrefix()
{
    ...
    if (status != SQLITE_OK) {
        throw Exception::createDeprecated(fmt::format("Cannot create SQLite table. Error status: {}. Message: {}",
                                       status, sqlite3_errstr(status)), ErrorCodes::SQLITE_ENGINE_ERROR);
    }
...
void SQLiteOutputFormat::consume(Chunk chunk)
{
    ...
    if (status != SQLITE_OK) {
        throw Exception::createDeprecated(fmt::format("Cannot insert into SQLite table. Error status: {}. Message: {}",
                                       status, sqlite3_errstr(status)), ErrorCodes::SQLITE_ENGINE_ERROR);
    }
}
