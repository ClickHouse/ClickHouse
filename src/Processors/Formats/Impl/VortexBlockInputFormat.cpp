...
In the current integration, reading a Vortex file is single-threaded: the whole scan (I/O,
decompression, and decoding) runs on one thread, so reading large Vortex files can be
significantly slower than reading `Parquet` files, which ClickHouse decodes with multiple
threads in parallel. Filter pushdown reduces the amount of data decoded by selective queries,
but does not parallelize the scan.
)DOCS_MD"});
}
