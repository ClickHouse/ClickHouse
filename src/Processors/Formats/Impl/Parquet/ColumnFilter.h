...
enum class ColumnFilterKind : uint8_t
{
    Unknown,
    AlwaysTrue,
    AlwaysFalse,
...
public:
    ...
    IsNullFilter() : ColumnFilter(ColumnFilterKind::IsNull, true) { }
...
public:
    static OptionalFilter create(const ActionsDAG::Node & node);
    IsNotNullFilter() : ColumnFilter(ColumnFilterKind::IsNotNull, false) { }
...
{
public:
    AlwaysTrueFilter() : ColumnFilter(ColumnFilterKind::AlwaysTrue, true) { }
...
{
public:
    AlwaysFalseFilter() : ColumnFilter(ColumnFilterKind::AlwaysFalse, false) { }
...
{
public:
    explicit BoolValueFilter(bool value_, bool null_allowed_) : ColumnFilter(ColumnFilterKind::BoolValue, null_allowed_), value(value_) { }
...
public:
    explicit BigIntRangeFilter(const Int64 lower_, const Int64 max_, bool null_allowed_)
        : ColumnFilter(ColumnFilterKind::BigIntRange, null_allowed_)
        , upper(max_)
        , lower(lower_)
...
public:
    BytesValuesFilter(const std::vector<String> & values_, bool null_allowed_)
        : ColumnFilter(ColumnFilterKind::BytesValues, null_allowed_), values_storage(values_), values(values_storage.begin(), values_storage.end())
    {
...
public:
    NegatedBytesValuesFilter(const std::vector<String> & values_, bool null_allowed_)
        : ColumnFilter(ColumnFilterKind::NegatedBytesValues, null_allowed_), non_negated(std::make_unique<BytesValuesFilter>(values_, !null_allowed_))
    {
    }
