...
public:
    ...
    /// When destroyed, the range is marked as no longer needed.
    ...
    public:
        ...
        bool empty() const { return request == nullptr; }
        size_t length() const { chassert(request); return request->length; }
    ...
    private:
        ...
        explicit RequestHandle(RequestState * request_);
    };

    Prefetcher(ReadBuffer * reader_, const ReadOptions & options, SharedParsingThreadPoolPtr thread_pool_);

    /// Waits for in-progress reads to complete, cancels queued reads that haven't started yet.
    ~Prefetcher();

    /// Not thread safe.
    /// All ranges must be preregistered before any reading happens (except direct readSync).
    ...
    RequestHandle preregisterRange(size_t offset, size_t length, bool likely_to_be_used);

    /// Called at most once, after all preregisterRange calls and before all enqueue/getRangeData calls.
    void finalizeRanges();

    /// Returns memory usage estimate (it may be higher than the total length() of the requests if
    /// we took on some read amplification to avoid seeks).
    size_t startPrefetch(const std::vector<const RequestHandle *> & requests_to_start);

    /// The input `request` must come from preregisterRange, not from another splitAndEnqueueRange.
    /// I.e. ranges can't be split more than once.
    /// `subranges` must be sorted.
    /// Returns memory usage estimate.
    size_t splitAndPrefetchRange(
...
private:
    ...
    struct Task
    {
        enum class State : UInt8
        {
            ...
            Deallocated,//TODO: remember to deallocate after running
        };
