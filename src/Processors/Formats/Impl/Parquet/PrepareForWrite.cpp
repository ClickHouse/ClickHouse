...
SchemaElements convertSchema(
    ...
    const IcebergOptionality & iceberg_optionality, std::vector<size_t> * out_uuid2_leaf_columns)
{
    ...
    for (const auto & c : sample)
    {
        ...
        prepareColumnForWrite(
            c.column, c.type, c.name, options, nullptr, &schema, column_field_ids, iceberg_optionality,
...
void prepareColumnForWrite(
    ...
    const IcebergOptionality & iceberg_optionality, std::vector<DataTypePtr> * out_leaf_types)
{
