...
void ReadManager::finishRowGroupStage(size_t row_group_idx, MemoryUsageDiff && diff)
{
    ...
    switch (stage)
    {
        ...
        case ParsingStage::ApplyingColumnIndex:
            ...
            break;
        case ParsingStage::ApplyingOffsetIndexForPrewhereColumns:
            break;
        case ParsingStage::ParsingPrewhereColumns:
            reader.applyPrewhere(row_group);

            for (size_t i = 0; i < reader.primitive_columns.size(); ++i)
            {
                if (reader.primitive_columns[i].only_for_prewhere)
                {
                    auto & column = row_group.columns.at(i);
                    column.column_and_offsets_memory.reset(&diff);
                }
            }

            break;
        case ParsingStage::ApplyingOffsetIndexForRemainingColumns:
            break;
        ...
        switch (stage)
        {
            ...
            case ParsingStage::ParsingPrewhereColumns:
                if (!reader.prewhere_steps.empty())
                {
                    ...
                    if (add_tasks_for_columns.empty())
                        /// PREWHERE expression doesn't use any columns, but we must still run it.
                        reader.applyPrewhere(row_group);
                }
                break;
            case ParsingStage::ApplyingOffsetIndexForPrewhereColumns:
            case ParsingStage::ApplyingOffsetIndexForRemainingColumns:
                if (row_group.filter.rows_pass < row_group.filter.rows_total)
                {
                    for (size_t i = 0; i < reader.primitive_columns.size(); ++i)
                    {
                        if (stage == ParsingStage::ApplyingOffsetIndexForPrewhereColumns &&
                            !reader.primitive_columns[i].use_prewhere)
                            continue;
...
Chunk ReadManager::read()
{
    ...
    for (size_t i = 0; i < reader.output_columns.size(); ++i)
    {
        ...
        if (!idx_in_output_block.has_value() || idx_in_output_block.value() >= columns.size())
            continue;
