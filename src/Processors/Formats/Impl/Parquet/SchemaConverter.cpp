    ...
    bool uuid2_preferred) const
{
    ...
    else if (logical.__isset.UUID)
    {
        ...
        /// Parquet's sort order for `uuid` is unsigned big-endian byte comparison. That is exactly the order of
        /// `UUID2`, so its min/max pair is an interval in the column's own order and stats are usable; ClickHouse
        /// sorts `UUID` by its second half, so for `UUID` the pair bounds nothing and allow_stats stays false.
