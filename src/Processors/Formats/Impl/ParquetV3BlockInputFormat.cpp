...
| `output_format_parquet_column_field_ids`                                       | Explicit Parquet `field_id` overrides per output column. A `Map(String, Int32)` keyed by column name; the integer ids must be written as string literals (e.g. `{'col_a': '1'}`). Dotted nested paths (e.g. `arr.element`, `m.key`, `m.value`, `t.subfield`) are also accepted to assign `field_id`s to nested fields. Useful for producing files compatible with Apache Iceberg, which identifies columns by `field_id`. Ids above `2147483447` are rejected with `BAD_ARGUMENTS`, because Iceberg reserves that range for its own metadata fields and a data column carrying such an id is ignored when the file is read as an Iceberg table. When `output_format_parquet_auto_assign_field_ids` is disabled, a non-empty map must cover every output column and every nested field, otherwise the query fails with `BAD_ARGUMENTS`; enable `output_format_parquet_auto_assign_field_ids` to fill the remaining paths automatically. Cannot be combined with a datalake-provided column-id mapping: `CREATE TABLE ... ENGINE = Iceberg*` rejects this setting (and `output_format_parquet_auto_assign_field_ids`) in the table definition, because the table metadata is the source of truth for `field_id`s; a value coming from the session or the profile is ignored for such tables (including writes through the `iceberg*` table functions). A value supplied in the `SETTINGS` clause of a table definition that freezes format settings (`File`, `URL`, object-storage engines writing Parquet) is validated when the `CREATE TABLE` (or full-definition `ATTACH TABLE`) runs, so an invalid map is rejected at DDL time; such a table takes the setting only from its own definition, so an ambient value coming from the session or the profile is not frozen onto a table created while it is set. Everywhere else an ambient value applies and is only validated when Parquet output is written, so an invalid value only fails such a write and does not affect queries that write no Parquet. | `{}`        |
| `output_format_parquet_auto_assign_field_ids`                                  | Auto-assign a unique sequential Parquet `field_id` (starting at `1`) to every output column and every nested field (Array `element`, Map `key`/`value`, Tuple subfields), matching the Apache Iceberg writer convention. Overrides from `output_format_parquet_column_field_ids` win for the paths they cover. Cannot be combined with a datalake-provided column-id mapping: `CREATE TABLE ... ENGINE = Iceberg*` rejects this setting (and `output_format_parquet_column_field_ids`) in the table definition, because the table metadata is the source of truth for `field_id`s; a value coming from the session or the profile is ignored for such tables (including writes through the `iceberg*` table functions). A value supplied in the `SETTINGS` clause of a table definition that freezes format settings (`File`, `URL`, object-storage engines writing Parquet) is validated together with `output_format_parquet_column_field_ids` when the `CREATE TABLE` (or full-definition `ATTACH TABLE`) runs, so conflicts such as ambiguous flattened paths are rejected at DDL time; such a table takes the setting only from its own definition, so an ambient value coming from the session or the profile is not frozen onto a table created while it is set. Everywhere else an ambient value applies and is only validated when Parquet output is written. | `0`         |
