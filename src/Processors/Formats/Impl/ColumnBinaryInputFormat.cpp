...
:::note Experimental
`ColumnBinary` is experimental. Its frame header carries a magic and a format version, so a reader rejects
bytes it cannot interpret rather than misreading them, but no compatibility between versions is promised yet:
the layout may still change incompatibly, and a future version may simply refuse data written today. Do not
persist `ColumnBinary` data until the layout is frozen.
:::
