...
/// The memory pool to set on `orc::ReaderOptions` of every ORC reader we create: unlike the default
/// one of the library, it is accounted for by the memory tracker and it throws on failure instead of
/// returning a null pointer that the library dereferences.
