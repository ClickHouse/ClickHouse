...
The offsets of the data of the variables are a part of the header, and the header is at the beginning
of the file, so the whole result is kept in memory while it is written, and it is written out at the
end. A large export is therefore limited by the memory available to the query, and not only by the
space for the file: it can exceed `max_memory_usage` even when the resulting file would be valid.
...
`NULL`s again. This requires a value of the type of the column to be left over, which is not the
