        ...
        size_t end)
    {
        ...
        else if (isDecimal(column_type))
        {
            ...
            if (!callOnIndexAndDataType<void>(column_type->getTypeId(), fill_decimal))
                throw Exception{ErrorCodes::UNKNOWN_TYPE, "Decimal type {} is not supported for conversion into a {} data format", column_type_name, format_name};
        }
    ...
    static std::shared_ptr<arrow::DataType> getArrowType(DataTypePtr column_type, const std::string & column_name, const std::string & format_name, bool * is_column_nullable)
    {
        ...
        if (isDecimal(column_type))
        {
            ...
            if (!callOnIndexAndDataType<void>(column_type->getTypeId(), create_arrow_type))
                throw Exception{ErrorCodes::UNKNOWN_TYPE, "Decimal type {} is not supported for conversion into a {} data format", column_type->getFamilyName(), format_name};
            return arrow_type;
        }
        ...
        throw Exception{"The type \"" + column_type->getFamilyName() + "\" of a column \"" + column_name + "\""
