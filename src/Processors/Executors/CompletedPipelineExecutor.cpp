...
void CompletedPipelineExecutor::execute()
{
    if (interactive_timeout_ms)
    {
        ...
        data->executor = std::make_shared<PipelineExecutor>(pipeline.processors, pipeline.process_list_element, pipeline.track_step_stats);
    ...
    {
        PipelineExecutor executor(pipeline.processors, pipeline.process_list_element, pipeline.track_step_stats);
