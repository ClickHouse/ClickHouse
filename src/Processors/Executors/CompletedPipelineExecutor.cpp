...
static void threadFunction(
    CompletedPipelineExecutor::Data & data, ThreadGroupPtr thread_group, ProfileEvents::CountersSeq profile_counters_scopes, size_t num_threads, bool concurrency_control)
{
    try
    {
        ThreadGroupSwitcher switcher(thread_group, ThreadName::COMPLETED_PIPELINE_EXECUTOR, profile_counters_scopes);
...
void CompletedPipelineExecutor::execute()
{
    if (interactive_timeout_ms)
    {
        ...
        auto func = [
            ...
            profile_counters_scopes = CurrentThread::getCountersScopes(),
            concurrency_control = pipeline.getConcurrencyControl()]
        {
            threadFunction(*data_ptr, thread_group, profile_counters_scopes, num_threads, concurrency_control);
        };
