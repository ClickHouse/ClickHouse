...
void ExecutingGraph::initializeExecution(Queue & queue, Queue & async_queue)
{
    ...
    while (!stack.empty())
    {
        ...
        updateNode(*node->processor(), queue, async_queue, scratch);
    }
}

ExecutingGraph::UpdateNodeStatus ExecutingGraph::updateNode(IProcessor & initial, Queue & queue, Queue & async_queue, UpdateNodeScratch & scratch)
{
