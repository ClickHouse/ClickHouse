...
PipelineExecutor::PipelineExecutor(std::shared_ptr<Processors> & processors, QueryStatusPtr elem, bool track_step_stats_)
    ...
    , process_list_element(std::move(elem))
{
    ...
    UInt64 query_start_ns = track_step_stats_ ? clock_gettime_ns() : 0;
    try
    {
        graph = std::make_unique<ExecutingGraph>(processors, profile_processors, track_step_stats_, query_start_ns);
    }
...
void PipelineExecutor::initializeExecution(size_t num_threads, bool concurrency_control)
{
    ...
    tasks.init(num_threads, 1, cpu_slots, profile_processors, trace_processors, track_step_stats, read_progress_callback.get());
