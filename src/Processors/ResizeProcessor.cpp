...
{
    if (const auto * mem_tracker = CurrentThread::getMemoryTracker())
    {
        size_t used_memory = mem_tracker->get();
        size_t hard_limit  = mem_tracker->getHardLimit();
...
ResizeProcessor::Status MemoryDependentResizeProcessor::prepare()
{
    bool is_first_output = true;
    auto output_end = current_output;
