...
void QueryPlan::explainEstimate(MutableColumns & columns)
{
    ...
    processNodeFuncType process_node = [&columns, &process_node](const Node * node)
    {
        if (!node)
            return;

        if (const auto * step = dynamic_cast<ISourceStep *>(node->step.get()))
        {
            step->updateEstimate(columns);
        }
