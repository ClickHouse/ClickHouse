...
public:
    ...
    public:
        ...
        bool operator==(const Value & other) const { return static_cast<const Base &>(*this) == static_cast<const Base &>(other); }
...
public:
    ...
    std::vector<RangesInDataParts> split()
    {
        ...
        while (!parts_ranges_queue.empty())
        {
            ...
            while (rows_in_current_layer < rows_per_layer || layers_intersection_is_too_big() || result_layers.size() == max_layers)
            {
                ...
                while (!parts_ranges_queue.empty() && (!last_value || last_value == parts_ranges_queue.top().value))
                {
    ...
    std::vector<ASTPtr> buildFilters()
    {
        ...
        auto lexicographically_greater = [&](const IndexValue & value)
        {
            ASTPtr result;
