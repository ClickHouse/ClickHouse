...
private:
    /// True if a plain read of `filtered_ranges` would drop rows:
    /// lightweight deletes and pending ALTER DELETE mutations.
    ...
    /// `ranges_in_data_parts` after pruning by a `_part` predicate.
