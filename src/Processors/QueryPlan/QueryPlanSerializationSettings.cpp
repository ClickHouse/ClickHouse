...
#define PLAN_SERIALIZATION_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(Bool, partitioned_hash_join_cap_partitions_by_l1_descriptors, true, "Limit the number of partitions of a `partitioned_hash` join so that the per-partition records of the hash table (where each partition's cells start and end) fit in a quarter of the L1 data cache.", 0) \
    \
