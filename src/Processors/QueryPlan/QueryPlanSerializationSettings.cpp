...
#define PLAN_SERIALIZATION_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(Bool, allow_inequality_join_as_cross_join, false, "Allow a JOIN with no equality in its ON condition to be executed as a CROSS JOIN with the condition applied as a filter.", 0) \
    \
