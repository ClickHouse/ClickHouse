...
#define PLAN_SERIALIZATION_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(Bool, query_plan_join_subset_keys_auto, false, "Use column statistics to automatically demote high-NDV JOIN equality keys to a residual filter.", 0) \
    DECLARE(UInt64, query_plan_join_subset_keys_min_rows, 1000000, "Minimum estimated right-side row count for `query_plan_join_subset_keys_auto` to kick in.", 0) \
    DECLARE(Double, query_plan_join_subset_keys_min_kept_selectivity, 0.01, "Target NDV(kept_keys)/total_rows for `query_plan_join_subset_keys_auto`.", 0) \
