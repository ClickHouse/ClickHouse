...
#define PLAN_SERIALIZATION_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(Bool, enable_parallel_single_level_merge, false, "Parallelize the final merge of single-level aggregation hash tables: every key-hash slice is merged independently by one of the threads, which enumerates all per-thread tables and merges the keys whose hash falls into the slice.", 0) \
