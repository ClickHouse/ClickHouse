...
private:
    ...
    /// After a virtual row went downstream, the merge may leave the output not needed until it reaches
    /// the row's key. Do not read ahead into the buffer while that lasts, otherwise every source would
    /// read speculatively and `virtual_row_prefetch_window` would have no effect.
