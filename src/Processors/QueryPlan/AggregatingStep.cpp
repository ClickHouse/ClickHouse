...
void AggregatingStep::serializeSettings(QueryPlanSerializationSettings & settings, UInt64 version) const
{
    ...
    /// on it (see `spillCodecAuthorizationMustBeSerialized`): `Aggregator::executeOnBlock` reaches
    ...
    /// and must not carry the opt-in. A reader that does not receive it keeps the default (`false`),
    ...
    /// refused for an aggregation that may well never spill. See the matching comment in
