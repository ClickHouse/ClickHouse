...
/// The plan serialization version that first carries the in-memory join compression settings; the
/// comments below call it "the compression version".
constexpr UInt64 compression_version = DBMS_MIN_QUERY_PLAN_SERIALIZATION_VERSION_WITH_JOIN_IN_MEMORY_COMPRESSION;
...
/// The in-memory join compression settings were added to the plan serialization in version
/// DBMS_MIN_QUERY_PLAN_SERIALIZATION_VERSION_WITH_JOIN_IN_MEMORY_COMPRESSION.
/// When serializing for a receiver older than that (a pre-PR server in a mixed-version cluster,
...
TEST(QueryPlanSerializationSettings, JoinCompressionSettingsOmittedForOlderVersions)
{
    ...
    /// A receiver at that version gets all of the settings, including the new ones.
    {
        auto v5 = roundTrip(settings, compression_version);
    ...
    /// An older receiver (version 4, version 3, version 2 and version 1) does not get the new
...
/// off but a non-default `max_memory_usage` - to the compression version and get it rejected by an older worker
/// during a rolling upgrade. Only an actually enabled `enable_join_in_memory_compression` or a
/// step-local `max_memory_usage` override requires the compression version; omitting a query-wide
/// `max_memory_usage` reproduces pre-compression-version behavior.
TEST(QueryPlanSerializationSettings, MinRequiredVersion)
{
    /// Nothing set, or only pre-compression-version settings set: the baseline version is enough.
    {
        QueryPlanSerializationSettings settings;
    ...
    /// drop the requested feature, so it requires the compression version.
    {
        QueryPlanSerializationSettings settings;
        settings[QueryPlanSerializationSetting::enable_join_in_memory_compression] = true;
        EXPECT_EQ(settings.getMinRequiredVersion(), compression_version);
    }

    /// The other compression-version setting (`max_memory_usage`) must not raise the version even when
    ...
    /// from its query context settings (see JoinStepLogical::deserialize), and an older worker
    /// simply behaves like a pre-compression-version server.
    {
        QueryPlanSerializationSettings settings;
        settings[QueryPlanSerializationSetting::max_memory_usage] = 12345;
        EXPECT_EQ(settings.getMinRequiredVersion(), 1u);
    ...
    /// so its fragment must stay on the baseline version and remain readable by an older worker.
    {
        QueryPlanSerializationSettings settings;
        settings[QueryPlanSerializationSetting::enable_join_in_memory_compression] = true;
    ...
    /// A single hash-capable algorithm in the set is enough to require the compression version - including
    ...
    /// would make an older receiver reject it for a setting it would never consume.
    {
        QueryPlanSerializationSettings settings;
        settings[QueryPlanSerializationSetting::enable_join_in_memory_compression] = true;
        ...
        /// A step-local `max_memory_usage` still requires the compression version for such a step: `ConstantJoin`
    ...
    /// JoinSettings::updatePlanSettings) is the other case that requires the compression version even with
    ...
    /// compression-version setting. The hash fallback that makes `prefer_partial_merge` hash-capable is only
    /// reached for a shape `MergeJoin` does not support, so a supported shape must stay on the baseline
    /// version - otherwise an older receiver rejects a fragment it would execute identically.
    {
        QueryPlanSerializationSettings settings;
        settings[QueryPlanSerializationSetting::enable_join_in_memory_compression] = true;
        settings.max_memory_usage_is_step_local = true;
    ...
    /// join_shape_supports_full_sorting_merge_join), and it consumes neither compression-version setting. A hash
    /// fallback listed after it is only reached for an unsupported shape, so a supported shape must
    /// stay on the baseline version - otherwise an older receiver (or an older stateless worker)
    ...
    /// `max_memory_usage` therefore must not raise the version - an older receiver executes such a
    /// fragment identically.
    {
        ...
        /// requires the compression version.
        settings[QueryPlanSerializationSetting::enable_join_in_memory_compression] = false;
        settings[QueryPlanSerializationSetting::join_algorithm]
...
/// and a fragment without compression is serialized below the compression version, so the stream omits it.
...
TEST(QueryPlanSerializationSettings, MaxMemoryUsageChangedFlagAfterRoundTrip)
{
    ...
    /// A compression-version stream carries the value; the receiver sees it as changed.
    EXPECT_TRUE(roundTrip(settings, compression_version).isChanged("max_memory_usage"));

    /// An older stream omits it; the receiver sees it as unchanged and falls back.
