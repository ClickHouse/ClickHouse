...
/// A deterministic function with exactly one row-varying argument cannot produce more distinct
/// values than that argument, regardless of where the constant arguments appear.
TEST(ColumnStatsDerivation, FunctionWithConstantArgumentsPropagatesBound)
{
    tryRegisterFunctions();
    ...
    ActionsDAG dag;
    const auto & date_input = dag.addInput("d", date_type);
    const auto & granularity = dag.addColumn(string_type->createColumnConst(0, Field(String("month"))), string_type, "granularity");

    /// dateTrunc('month', d) has a constant first argument and its row-varying argument second.
    addOutputFunction(dag, "dateTrunc", {&granularity, &date_input}, "month");
    ...
    EXPECT_EQ(stats["month"].avg_bytes, 2);
}

/// Row-constant wrappers can intentionally avoid constant folding. They are still constant with
/// respect to the input rows and must not hide the only row-varying argument from NDV propagation.
TEST(ColumnStatsDerivation, MaterializedConstantArgumentPropagatesBound)
{
    tryRegisterFunctions();
    ...
    ActionsDAG dag;
    const auto & int_input = dag.addInput("n", int_type);
    const auto & one = dag.addColumn(int_type->createColumnConst(0, Field(Int64(1))), int_type, "one");
    const auto & materialized_one = addOutputFunction(dag, "materialize", {&one}, "materialized_one");

    ASSERT_FALSE(materialized_one.column);
    addOutputFunction(dag, "plus", {&materialized_one, &int_input}, "incremented");

    auto stats = statsOf("n", distinct_values);
    stats["n"].avg_bytes = 8;
    remapColumnStats(stats, dag);

    EXPECT_EQ(stats["incremented"].num_distinct_values, distinct_values);
    EXPECT_EQ(stats["incremented"].avg_bytes, 8);
}

/// Always-constant arguments coming through the scalar-subquery wrapper have no constant column on
/// the wrapper node, but must follow the same NDV propagation path as literal constant arguments.
TEST(ColumnStatsDerivation, ScalarSubqueryConstantArgumentPropagatesBound)
{
    tryRegisterFunctions();
    ...
    const auto & date_input = dag.addInput("d", date_type);
    const auto & granularity = dag.addColumn(string_type->createColumnConst(0, Field(String("month"))), string_type, "granularity");
    const auto & scalar_granularity = addOutputFunction(dag, "__scalarSubqueryResult", {&granularity}, "scalar_granularity");

    addOutputFunction(dag, "dateTrunc", {&scalar_granularity, &date_input}, "month");
