...
LimitStep::LimitStep(
    ...
    , is_negative(is_negative_), with_ties(with_ties_), description(std::move(description_))
{
}
...
void LimitStep::transformPipeline(QueryPipelineBuilder & pipeline, const BuildQueryPipelineSettings &)
{
    auto transform = std::make_shared<LimitTransform>(
        pipeline.getHeader(), limit, offset, pipeline.getNumStreams(), always_read_till_end, with_ties, is_negative, description);
