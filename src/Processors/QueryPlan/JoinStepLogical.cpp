...
JoinStepLogical::JoinStepLogical(Header left_header, ContextPtr query_context_)
...
void JoinStepLogical::setHashTableCacheKey(IQueryTreeNode::HashState hash_table_key_hash_)
{
    hash_table_key_hashes.resize(join_operators.size());
    hash_table_key_hashes.back() = std::move(hash_table_key_hash_);
}
    ...
    bool is_explain_logical)
{
    ...
    if (!isCrossOrComma(join_info.kind) && !isPaste(join_info.kind))
    {
        ...
        if (!has_keys)
        {
            if (!TableJoin::isEnabledAlgorithm(join_context.join_settings.join_algorithm, JoinAlgorithm::HASH))
    ...
    if (need_add_nullable && join_context.join_settings.join_use_nulls)
    {
    ...
    addRequiredInputToOutput(left_pre_join_actions, join_context.required_output_columns);
    addRequiredInputToOutput(right_pre_join_actions, join_context.required_output_columns);
    addRequiredInputToOutput(post_join_actions, join_context.required_output_columns);

    ActionsDAG::NodeRawConstPtrs new_outputs;
    for (const auto * output : post_join_actions->getOutputs())
    {
        if (join_context.required_output_columns.contains(output->result_name))
            new_outputs.push_back(output);
    }

    if (new_outputs.empty())
    {
        new_outputs = getAnyColumn(post_join_actions->getOutputs());
    }

    if (result_node.filter)
        new_outputs.push_back(result_node.filter.getNode());
    post_join_actions->getOutputs() = std::move(new_outputs);
    post_join_actions->removeUnusedActions();
...
bool JoinStepLogical::canFlatten(const ContextPtr & context_) const
{
