...
QueryPlanStepPtr JoinStepLogical::deserialize(Deserialization & ctx)
{
    ...
    /// the compression trigger: HashJoin::shrinkStoredBlocksToFit consults it for plain block
    /// shrinking too, and leaving it at 0 here would silently drop that shrink trigger on the remote
    /// fragment (a possible MEMORY_LIMIT_EXCEEDED where the local path would have shrunk). The query
    /// settings travel with the distributed query, so restore the value from the receiver's query
    /// context. A stream at that version always carries the sender's exact value (updatePlanSettings
    /// marks every serialized setting changed), so this never overrides an explicitly sent value.
