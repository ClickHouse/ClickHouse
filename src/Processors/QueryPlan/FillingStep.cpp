...
FillingStep::FillingStep(const DataStream & input_stream_, SortDescription sort_description_, InterpolateDescription interpolate_description_)
    : ITransformingStep(input_stream_, FillingTransform::transformHeader(input_stream_.header, sort_description_/*, interpolate_description_*/), getTraits())
    , sort_description(std::move(sort_description_)), interpolate_description(std::move(interpolate_description_))
{
...
void FillingStep::describeActions(FormatSettings & settings) const
{
    ...
    settings.out << '\n';
    dumpInterpolateDescription(interpolate_description, input_streams.front().header, settings.out);
    settings.out << '\n';
}
...
void FillingStep::describeActions(JSONBuilder::JSONMap & map) const
{
    ...
    map.add("Interpolate Description", explainInterpolateDescription(interpolate_description, input_streams.front().header));
}
