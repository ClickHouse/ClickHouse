...
void CubeStep::serialize(Serialization & ctx) const
{
    ...
    /// Positions were added in Cube step serialization v1. Pre-v1 nodes omit these positions (writing
    /// at v0). On mixed-version clusters, silently dropping them produces wrong results for queries
    /// with duplicate CUBE keys. To prevent silent bugs, fail explicitly if a query repeats a key;
    /// all other queries run as normal.
...
void registerCubeStep(QueryPlanStepRegistry & registry)
{
    /// Cube step serialization v1 appends GROUP BY positions for repeated keys. This version is used
    /// whenever per-step versioning is enabled. Older peers read this as version 0 (deduplicated keys
    /// only), preserving legacy behavior.
