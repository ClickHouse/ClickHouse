...
QueryPlanStepPtr ReadFromIcebergStep::clone() const
{
    return std::make_unique<ReadFromIcebergStep>(*this);
}

void ReadFromIcebergStep::applyFilters(ActionDAGNodes added_filter_nodes)
{
...
void ReadFromIcebergStep::updatePrewhereInfo(const PrewhereInfoPtr & prewhere_info_value)
{
...
void ReadFromIcebergStep::initializePipeline(QueryPipelineBuilder & pipeline, const BuildQueryPipelineSettings &)
{
    createIterator();
    ...
    {
        auto source = std::make_shared<IcebergSource>(
            getName(),
            object_storage,
            configuration,
            storage_snapshot,
...
void ReadFromIcebergStep::createIterator()
{
    if (iterator_wrapper)
        return;
...
bool ReadFromIcebergStep::requestReadingInOrder() const
{
    return metadata->isDataSortedBySortingKey(storage_snapshot->metadata, getContext());
}

InputOrderInfoPtr ReadFromIcebergStep::getDataOrder() const
{
