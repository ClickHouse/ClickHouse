...
private:
    ...
    /// optimization. Carried by clones and over to a projection read by copyJoinRuntimeFilterIndexAnalysisDescriptors,
    /// but not by serialize()/deserialize(), so the pruning is intentionally skipped when the step is rebuilt
    /// for distributed or parallel-replicas reads (results stay correct, only the optimization is lost);
    /// propagating it there is a follow-up. This is part of the setting's documented contract
