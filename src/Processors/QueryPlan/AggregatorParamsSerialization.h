...
/// the stats-collecting settings (only `Aggregating` sends a stats key),
/// `enable_packed_string_keys_in_aggregation` (needs per-step old-peer gating), and the adaptive
/// aggregator settings (only raw-input aggregation uses them, so only `Aggregating` sends them).
