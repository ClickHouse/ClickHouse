...
VectorWithMemoryTracking<Range> intersectWithPrimesDomain(const Ranges & ranges)
{
    const Range primes_domain(FieldRef(UInt64(2)), true, FieldRef(std::numeric_limits<UInt64>::max()), true);

    VectorWithMemoryTracking<Range> intersections;
...
VectorWithMemoryTracking<Interval> rangesToIntervals(const VectorWithMemoryTracking<Range> & ranges)
{
...
Pipe ReadFromSystemPrimesStep::makePipe()
{
    ...
    auto intersect_ranges = [&](const Ranges & ranges) -> std::optional<VectorWithMemoryTracking<Interval>>
    {
        ...
        VectorWithMemoryTracking<Range> intersected = intersectWithPrimesDomain(ranges);
