    ...
    bool * right_branch_selected)
{
    ...
    while (node)
    {
        ...
        if (allow_view_over_mergetree && node != root && typeid_cast<UnionStep *>(node->step.get()))
        {
            ...
            {
                auto child_results = findReadingSteps(child, allow_view_over_mergetree, allow_merge_tables, right_branch_selected);
    ...
    size_t replica_number)
{
    ...
    auto reading_nodes = findReadingSteps(query_plan->getRootNode(), allow_view_over_mergetree, allow_merge_tables, &right_branch_selected);
