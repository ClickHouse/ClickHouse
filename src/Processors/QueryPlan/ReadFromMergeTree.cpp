    ...
    else
    {
                                                         ...
                                                         /// Avoid that SAMPLE-narrowed entries poison the cache (later non-SAMPLE-ing queries would return wrong results).
        {
            ...
            if (outputs.size() == 1 && !skip_top_k && VirtualColumnUtils::isDeterministicAllowingTopKFilter(outputs.front()))
            {
                ...
                size_t hash = queryConditionCacheHash(condition_node->getHash(true /* skip_aliases */),
...
void ReadFromMergeTree::initializePipeline(QueryPipelineBuilder & pipeline, [[maybe_unused]] const BuildQueryPipelineSettings & settings)
{
    ...
    if (reader_settings.use_query_condition_cache)
    {
        ...
        {
            size_t condition_hash = queryConditionCacheHash(condition_node->getHash(true /* skip_aliases */),
                queryConditionCacheSettingsSalt(context->getSettingsRef()));
            if (top_k_filter_info)
