    ...
    const InputOrderInfoPtr & input_order_info)
{
    ...
    if (reader_settings.read_in_order)
    {
        ...
        auto read_type = input_order_info->direction == 1 ? ReadType::InOrder : ReadType::InReverseOrder;
        reading_step_getter
            = [this, &index_build_context, &in_order_column_names_to_read, &info, sorting_expr, &sort_description, read_type](auto parts)
        {
            auto pipe = this->read(
                std::move(parts),
                index_build_context,
