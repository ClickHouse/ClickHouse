    ...
    std::optional<ActionsDAG> & out_projection)
{
    ...
    if (query_info.prewhere_info || query_info.row_level_filter)
    {
        ...
        /// SummingMergeTree's zero-row decision depends on every summing column, so a PREWHERE that
        /// only touches a subset must not strip the rest before SummingSortedTransform (#106125). This
        /// mirrors the sign/version restore above and matches the read set pinned in getReadTaskColumns.
        if (data.merging_params.mode == MergeTreeData::MergingParams::Summing)
            for (const auto & column_name : getSummingColumnsRequiredForFinal(
                     *storage_snapshot->metadata, data.merging_params.columns_to_sum, data.merging_params.allow_tuple_element_aggregation))
                columns_to_restore.insert(column_name);
...
ReadFromMergeTree::RemoveUnusedColumnsResult ReadFromMergeTree::removeUnusedColumns(const std::vector<size_t> & required_output_positions, bool /*remove_inputs*/)
{
    ...
    if (query_info.isFinal())
    {
        const auto required_for_final = getColumnsRequiredForMergingFinal(result_sort_description, data.merging_params, *getStorageMetadata());
