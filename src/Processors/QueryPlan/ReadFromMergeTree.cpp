    ...
    const InputOrderInfoPtr & input_order_info)
{
    ...
    const auto split_ranges
        = [rows_granularity = (*data_settings)[MergeTreeSetting::index_granularity], max_block_size = block_size.max_block_size_rows]
        (const MarkRanges & ranges, int direction)
    {
        return splitRangesToAvoidLargeReads(ranges, direction, rows_granularity, max_block_size);
    };
