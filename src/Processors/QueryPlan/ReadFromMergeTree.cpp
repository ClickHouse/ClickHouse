    ...
    else
    {
                                                         ...
                                                         /// Avoid that SAMPLE-narrowed entries poison the cache (later non-SAMPLE-ing queries would return wrong results).
        {
            condition_hash = getQueryConditionCacheConditionHash(
                ...
                reader_settings.query_condition_cache_settings_salt);
        }
    ...
    UInt64 settings_salt)
{
    ...
    /// The settings salt (`queryConditionCacheSettingsSalt`) is folded in first and the TopK plan
    /// parameters second, in the same order as `updateQueryConditionCache` and
    /// `filterPartsByQueryConditionCache`, so that all writers and the reader agree on the key.
    /// `size_t` (not `UInt64`) so `boost::hash_combine` binds its seed argument on platforms where
    /// they differ (e.g. Apple, where `size_t` is `unsigned long` but `UInt64` is `unsigned long long`).
    size_t hash = queryConditionCacheHash(outputs.front()->getHash(), settings_salt);
