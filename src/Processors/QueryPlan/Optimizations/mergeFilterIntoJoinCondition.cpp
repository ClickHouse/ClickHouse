...
/// reason. Lambda bodies are separate DAGs, so the walk has to descend into them (`findFunctionInSubtrees`).
bool subtreeContainsNonDeterministicFunction(const ActionsDAG::Node * node)
{
    return findFunctionInSubtrees({node}, [](const IFunctionBase & function) { return !function.isDeterministicInScopeOfQuery(); })
        != nullptr;
}
