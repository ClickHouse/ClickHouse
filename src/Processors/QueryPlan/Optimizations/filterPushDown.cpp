...
addNewFilterStepOrThrow(QueryPlan::Node * parent_node, QueryPlan::Nodes & nodes, const ActionsDAGPtr & split_filter,
                    bool remove_filter_column = true, size_t child_idx = 0, bool update_parent_filter = true)
{
    QueryPlan::Node * child_node = parent_node->children.front();
    checkChildrenSize(child_node, child_idx + 1);
    ...
    node.step = std::make_unique<FilterStep>(
        node.children.at(0)->step->getOutputStream(), split_filter, std::move(split_filter_column_name), remove_filter_column);
...
static size_t tryPushDownOverJoinStep(QueryPlan::Node * parent_node, QueryPlan::Nodes & nodes, QueryPlanStepPtr & child)
{
    ...
    std::vector<ActionsDAG::EquivalentColumnsPair> equivalent_columns;
    ...
    if (has_single_clause)
    {
        ...
        for (size_t i = 0; i < key_names_size; ++i)
        {
            ...
            equivalent_columns.emplace_back(left_table_key_name, right_table_key_name);
        }
    }

    auto get_available_columns_for_filter = [&](bool push_to_left_stream, bool filter_push_down_all_input_columns_available)
    {
        const auto & input_header = push_to_left_stream ? left_stream_input_header : right_stream_input_header;
        const auto & equivalent_columns_for_filter = push_to_left_stream ? equivalent_left_stream_column_to_right_stream_column
    ...
    Names left_stream_available_columns_to_push_down = get_available_columns_for_filter(true, left_stream_filter_push_down_all_input_columns_available);
    Names right_stream_available_columns_to_push_down = get_available_columns_for_filter(false, right_stream_filter_push_down_all_input_columns_available);
    ...
    /** We disable push down to right table in cases:
      ...
      */
    if (join_filter_push_down_actions.right_stream_filter_to_push_down && join && join->allowPushDownToRight() && table_join.strictness() != JoinStrictness::Asof)
    {
        updated_steps += addNewFilterStepOrThrow(parent_node,
            nodes,
