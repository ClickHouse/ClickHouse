    ...
    const QueryPlanOptimizationSettings & optimization_settings)
{
    ...
    if (!reading)
        return std::unexpected("the step is not a MergeTree read");
    ...
    if (iter->node->step.get() == reading)
        return std::unexpected("the read is a dangling node of a Merge table");
    ...
    if (normal_projections.empty())
        return std::unexpected("the table has no normal projection");
    ...
    QueryDAG query;
    {
        ...
        if (!query.build(*child))
            return std::unexpected("the steps between the read and its consumer cannot be rewritten onto a projection");
    ...
    if (!force_optimize_projection)
    {
        ...
        if (!has_filter && !can_use_sort_order)
            return std::unexpected("the query has neither a filter nor an ORDER BY a projection could serve");
    }
    ...
    for (const auto * projection : normal_projections)
    {
        ...
        if (projection->where_clause_ast)
        {
            if (!doesQueryFilterImplyProjectionWhere(query.filter_node, projection->where_clause_ast, projection->query_ast, context))
            {
                ...
                reject_reasons[projection] = "the query WHERE does not imply the projection WHERE";
                continue;
            }
        }
        ...
        {
            reject_reasons[projection] = "the projection does not contain all columns required by the query";
        ...
        if (!analyzed)
        {
            reject_reasons[projection] = "no selected part has the projection materialized, or its analysis exceeded the read limits";
            continue;
        }
        ...
        if (candidate.sum_marks > parent_reading_marks)
        {
            ...
            reject_reasons[projection] = stat.description;
    ...
    if (!best_candidate)
    {
        ...
        for (const auto * projection : normal_projections)
        {
            ...
            rejections += fmt::format(
                "projection {} is rejected because {}", backQuoteIfNeed(projection->name), reject_reasons.at(projection));
        }
        return std::unexpected(fmt::format("no projection can be used: {}", rejections));
    }
    ...
    else
    {
        ...
        if (!blocksHaveEqualStructure(*main_stream, **proj_stream))
            return std::unexpected("the projection stream and the table stream have different headers");
