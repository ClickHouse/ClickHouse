...
/// on the join-key values (and equal values hash equally through the `LowCardinality`/`Nullable` wrappers,
/// per the representation-independence contract on `IColumn::computeHashInto`), equal keys land in the same
/// shard on both sides. The join output is unordered.
