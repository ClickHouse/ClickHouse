    ...
    for (const auto & old_key : keys)
    {
        if (hints.contains(old_key) && hints.at(old_key).isRangeLengthLessOrEqualThan(65535))
        {
            ...
            if (type_name == "UInt8" || type_name == "Int8" ||
                ...
                ((type_name == "UInt32" || type_name == "Int32") && !hint.isRangeLengthLessOrEqualThan(65535)))
            {
            ...
            const auto & added_function = dag->addFunction(minus_function, children, old_key);
            ...
            dag->removeUnusedResult(old_key);
            dag->addOrReplaceInOutputs(dag->addCast(added_function, result_type, old_key));
        }
        else
        {
            new_aggregating_keys.push_back(old_key);
...
std::shared_ptr<ActionsDAG> buildSuccessorActionsDag(const Block & header, const Names & changed_keys, const DataTypes& changed_data_types, const DataHints & hints)
{
    ...
    for (size_t i = 0; i < changed_keys.size(); ++i)
    {
        ...
        const auto key_to_remove = changed_key;
        ...
        dag->addOrReplaceInOutputs(dag->addCast(dag->addFunction(plus_function, children, changed_key), changed_data_type, changed_key));
    }
...
size_t tryReduceSortingKeysSize(QueryPlan::Node * node, QueryPlan::Nodes & nodes)
{
    ...
    Names initial_keys(sorting_step->getSortDescription().size());
    for (const auto & column_description : sorting_step->getSortDescription())
        initial_keys.emplace_back(column_description.column_name);
