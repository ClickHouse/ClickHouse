...
public:
    ...
    void visitBottomUpImpl(QueryPlan::Node * current_node, QueryPlan::Node * parent_node)
    {
        ...
        if (aggregating_step)
        {
            ...
            /// the ordinary merging transform reads all the buckets before it returns anything, and it returns
            /// them in an arbitrary order, which the node above cannot merge memory efficiently.
