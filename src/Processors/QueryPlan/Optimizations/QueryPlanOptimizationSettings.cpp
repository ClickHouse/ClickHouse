    ...
    bool is_parallel_replicas_initiator_with_projection_support_)
{
    ...
    {
        LOG_DEBUG(
            getLogger("QueryPlanOptimizationSettings"),
            "Using random seed {} for randomizing join order optimizations",
    ...
    read_in_order = from[Setting::query_plan_enable_optimizations] && from[Setting::optimize_read_in_order]
        && from[Setting::query_plan_read_in_order];
    ...
    limit_by_partitions_independently
        = from[Setting::query_plan_enable_optimizations] && from[Setting::allow_limit_by_partitions_independently];
    ...
    optimize_aggregation_in_order_limit
        = from[Setting::query_plan_enable_optimizations] && from[Setting::optimize_aggregation_in_order_limit];
    ...
    if (make_distributed_plan
        && (from[Setting::allow_experimental_parallel_reading_from_replicas] > 0 || from[Setting::automatic_parallel_replicas_mode] != 0))
        throw Exception(
    ...
    if (!distributed_plan_force_exchange_kind.empty() && distributed_plan_force_exchange_kind != "Persisted"
    ...
    min_columns_for_join_lazy_indexing
        = from[Setting::query_plan_enable_optimizations] ? from[Setting::query_plan_min_columns_for_join_lazy_indexing] : 0;
              ...
              && from->getSettingsRef()[Setting::parallel_replicas_support_projection])
{
    ...
    if (distributed_plan_force_exchange_kind.empty() && !distributed_plan_execute_locally)
    {
        ...
        const bool streaming_listener_configured = config.getUInt("distributed_query.streaming_exchange_port", 0) != 0
