...
/// column statistics metadata without reading index files.
    ...
    size_t max_set_size_for_match)
{
        ...
        && (metadata->hasPartitionKey() || keys.empty()))
    {
        ...
        if (all_min_max)
        {
            /// Respect the feature-gating setting.
            if (!context->getSettingsRef()[Setting::use_statistics_for_min_max_aggregation])
                all_min_max = false;

            /// Per-part column statistics describe the physical rows of a part.  Anything that
            /// changes the values visible at read time (on-the-fly mutations, patch parts,
            /// lightweight deletes) means the stored statistics no longer match what the query
            /// sees, so the optimization must be disabled.
            const auto & mutations_snapshot = reading.getMutationsSnapshot();
            if (all_min_max && (mutations_snapshot->hasDataMutations()
