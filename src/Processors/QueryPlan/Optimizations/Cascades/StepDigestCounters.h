...
{
    /// Completed full-digest passes, whether they produced a fingerprint or bytes.
...
/// RAII scope that makes `counters` the target for `computeStepFullFingerprint` and
/// `stepFullDigestsEqual` to increment on this thread. The digest code has no context parameter,
