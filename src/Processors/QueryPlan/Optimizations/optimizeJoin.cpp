...
static bool canMergeExpressionIntoJoinGraph(const ActionsDAG & dag, bool merge_expression_into_join)
{
    ...
    /// Reordering inside the join graph can compute the expression twice (once for the join key, once for
    /// the output column) and on rows the original order would have discarded, so a non-deterministic
    /// function draws independently in the two places and the result can violate the query's `JOIN ON`.
