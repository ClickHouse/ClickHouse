...
void optimizePrimaryKeyConditionAndLimit(const Stack & stack)
{
    ...
    {
        source_step_with_filter->addFilter(std::make_unique<ActionsDAG>(storage_prewhere_info->prewhere_actions->clone()), storage_prewhere_info->prewhere_column_name);
        if (storage_prewhere_info->row_level_filter)
            source_step_with_filter->addFilter(std::make_unique<ActionsDAG>(storage_prewhere_info->row_level_filter->clone()), storage_prewhere_info->row_level_column_name);
    }
    ...
    for (auto iter = stack.rbegin() + 1; iter != stack.rend(); ++iter)
    {
        ...
        {
            source_step_with_filter->addFilter(std::make_unique<ActionsDAG>(filter_step->getExpression().clone()), filter_step->getFilterColumnName());
        }
