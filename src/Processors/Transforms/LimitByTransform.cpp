...
void LimitBySortedStreamTransform::rememberLastGroupingKey(const Columns & chunk_columns, UInt64 row_idx)
{
    ...
    for (size_t key_idx = 0; key_idx < grouping_key_positions.size(); ++key_idx)
    {
        if (!previous_chunk_last_grouping_key_columns[key_idx]->empty())
            previous_chunk_last_grouping_key_columns[key_idx]->popBack(previous_chunk_last_grouping_key_columns[key_idx]->size());
        previous_chunk_last_grouping_key_columns[key_idx]->insertFrom(*chunk_columns[grouping_key_positions[key_idx]], row_idx);
    }
}
