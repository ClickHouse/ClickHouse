...
void GroupingAggregatedTransform::addChunk(Chunk chunk, size_t input)
{
    ...
    if (auto agg_info = chunk.getChunkInfos().get<AggregatedChunkInfo>())
    {
        ...
        {
            /// An input is allowed to send a bucket out of order only if it announced that bucket as delayed before,
            /// otherwise we could have already pushed that bucket and the same keys would be returned twice.
            chassert(
