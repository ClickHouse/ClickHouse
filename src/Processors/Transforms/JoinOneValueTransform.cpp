...
{
    if (headers.size() != 2)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "JoinOneValueTransform expects 2 inputs, got {}", headers.size());
    if (output_header.columns() != headers[0].columns() + 1)
        throw Exception(
            ErrorCodes::LOGICAL_ERROR, "JoinOneValueTransform expects output header to have one more column than input headers");
    if (headers[1].columns() != 1)
        throw Exception(
            ErrorCodes::LOGICAL_ERROR, "JoinOneValueTransform expects second input to have one column, got {}", headers[1].columns());

    auto insert_ptr = std::find(output_header.begin(), output_header.end(), headers[1].getByPosition(0));

    if (insert_ptr == output_header.end())
        throw Exception(ErrorCodes::LOGICAL_ERROR, "JoinOneValueTransform expects second input to be in output header");
