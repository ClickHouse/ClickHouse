    ...
    , aggregates_mask(getAggregatesMask(params->getHeader(), params->params.aggregates))
{
    ...
    /// The power set is over the list as written, so a repeated key doubles the subsets.
...
{
    /// `__grouping_set` numbers subsets of the deduplicated keys, so several emitted subsets may share a number.
    const size_t num_keys = keys.size();
    UInt64 kept = 0;
