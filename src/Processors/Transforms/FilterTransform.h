...
public:
    FilterTransform(
        ...
        std::optional<size_t> condition_hash_ = std::nullopt, std::optional<String> condition_ = std::nullopt);
...
private:
    ...
    /// ---
    /// If both members set, we need to update the query condition cache at runtime for every processed chunk
    ///
