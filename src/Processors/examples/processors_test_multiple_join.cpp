    ...
    void consume(Chunk chunk) override
    {
        ...
        for (size_t row_num = 0; row_num < rows; ++row_num)
        {
            ...
            for (size_t column_num = 0; column_num < columns; ++column_num)
            {
                if (column_num != 0)
                    writeChar('\t', out);
                getPort().getHeader().getByPosition(column_num).type->getDefaultSerialization()->serializeText(*chunk.getColumns()[column_num], row_num, out, settings);
            }
            writeChar('\n', out);
        }
...
try
{
    auto execute_chain = [](String msg, size_t start1, size_t start2, size_t start3, size_t num_threads)
    {
        ...
        auto transform2 = std::make_shared<SleepyTransform>(10);
        auto transform3 = std::make_shared<SleepyTransform>(10);
        ...
        SortDescription description{SortColumnDescription{std::string("number")}, SortColumnDescription{std::string("number")}, SortColumnDescription{std::string("number")}};
        ...
        WriteBufferFromOStream out(std::cerr);
        printPipelineCompact(processors, out, true);
