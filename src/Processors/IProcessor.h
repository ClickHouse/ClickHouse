...
public:
    ...
    /// Called by the executor once the whole pipeline has finished successfully: every processor is finished,
    /// and the read progress of every source has been reported. The progress of a source may arrive after all
    /// its consumers have finished (e.g. `RemoteSource` drains the remaining packets of a connection after `LIMIT`),
    /// so anything that depends on the final statistics, such as the epilogue of an output format, belongs here.
