...
void MutationsInterpreter::prepare(bool dry_run)
{
    ...
    {
        const auto affected = [&](const String & name)
        {
            return updated_columns.contains(name) || changed_columns.contains(name)
                || patch_updated_columns.contains(name) || all_affected_materialized.contains(name);
        };
        ...
        if (!physical_columns.contains(col))
        {
            if (auto column_in_storage = Nested::tryGetColumnNameInStorage(col, physical_columns))
                return affected(*column_in_storage);
        }
        return false;
    };
