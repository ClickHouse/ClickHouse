...
namespace
{
    ...
    struct ExplainAnalyzedSyntaxMatcher
    {
            ...
            for (const auto & child : node->children)
            {
                ...
                if (const auto * select = node->as<ASTSelectQuery>(); select && child == select->with())
                {
                    if (collect_referenced_with_subqueries)
                    {
                        ...
                        for (const auto & name : referenced_with_subqueries_to_scan)
                        {
                ...
                collectNestedSelectQueries(child, nested_selects, collect_referenced_with_subqueries);
            }
        }
        ...
        static void checkNestedSelectsViewBaseTableAccess(const ASTPtr & node, const Data & data)
        {
            ...
            collectNestedSelectQueries(node_with_substituted_with, nested_selects, !substitute_enclosing_with);
            ...
            for (const auto & nested_select_node : nested_selects)
            {
                ...
                {
                    /// Ok to swallow, as in `resolveThenCheckAccessRights`: a non-ClickHouse exception (e.g. a remote
