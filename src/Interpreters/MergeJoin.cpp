    ...
    , log(&Poco::Logger::get("MergeJoin"))
{
    ...
    if (!size_limits.hasLimits())
    {
        ...
        if (!size_limits.max_bytes)
            throw Exception(ErrorCodes::PARAMETER_OUT_OF_BOUND, "No limit for MergeJoin (max_rows_in_join, max_bytes_in_join or default_max_bytes_in_join have to be set)");
    }
