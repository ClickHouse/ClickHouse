...
/** JIT compilation of a subset of simple regular expressions into a native, SIMD-friendly matcher.
  ...
  * Callers that get a null matcher (unsupported pattern, JIT disabled, no compiled-expression cache or one
  * of zero capacity, or below the compile count threshold) must fall back to the general RE2 engine. The
  * compiled matcher produces results that are bit-for-bit identical to RE2 for the supported subset.
  *
...
/// embedded compiler is disabled, if the compiled-expression cache is missing or has zero capacity and so
/// cannot retain the compiled matcher, or if the pattern has been seen fewer than `min_count_to_compile`
/// times (so that rarely-used patterns are not compiled).
