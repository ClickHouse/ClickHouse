...
DatabaseAndTable Context::getOrCacheStorage(const StorageID & id, const GetDatabasesOptions & options, std::function<DatabaseAndTable()> storage_getter, std::optional<Exception> * exception) const
{
    ...
    {
        DatabaseAndTable storage = DatabaseCatalog::instance().tryGetTableWithDatabase(it->uuid, shared_from_this(), options);
