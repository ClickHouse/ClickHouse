...
struct ContextShared
{
    ...
    std::optional<BackgroundProcessingPool> background_low_priority_pool; /// The thread pool with low cpu priority for the background work performed by the tables.
...
BackgroundProcessingPool & Context::getBackgroundLowPriorityPool()
{
    auto lock = getLock();
    if (!shared->background_low_priority_pool)
    {
        ...
        pool_settings.thread_sleep_seconds = config.getDouble("background_low_priority_processing_pool_thread_sleep_seconds", 10);
        pool_settings.thread_sleep_seconds_random_part = config.getDouble("background_low_priority_processing_pool_thread_sleep_seconds_random_part", 1.0);
        pool_settings.thread_sleep_seconds_if_nothing_to_do = config.getDouble("background_low_priority_processing_pool_thread_sleep_seconds_if_nothing_to_do", 0.1);
        pool_settings.task_sleep_seconds_when_no_work_min = config.getDouble("background_low_priority_processing_pool_task_sleep_seconds_when_no_work_min", 10);
        pool_settings.task_sleep_seconds_when_no_work_max = config.getDouble("background_low_priority_processing_pool_task_sleep_seconds_when_no_work_max", 600);
        pool_settings.task_sleep_seconds_when_no_work_multiplier = config.getDouble("background_low_priority_processing_pool_task_sleep_seconds_when_no_work_multiplier", 1.1);
        pool_settings.task_sleep_seconds_when_no_work_random_part = config.getDouble("background_low_priority_processing_pool_task_sleep_seconds_when_no_work_random_part", 1.0);
        shared->background_low_priority_pool.emplace(settings.background_low_priority_pool_size, pool_settings);
    }
    return *shared->background_low_priority_pool;
}
