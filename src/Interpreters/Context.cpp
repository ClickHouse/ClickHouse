...
void Context::addSkipIndexAccessInfo(const String & table_name, const String & skip_index_name)
{
    if (skip_index_name.empty())
        return;

    std::lock_guard lock(query_access_info->mutex);
    query_access_info->skip_indexes.emplace(table_name + "." + skip_index_name);
}
