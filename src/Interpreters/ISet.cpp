...
void NO_INLINE ISet::insertFromBlockImpl(
    Method & method,
...
void NO_INLINE ISet::insertFromBlockImplCase(
    Method & method,
    const ColumnRawPtrs & key_columns,
    size_t rows,
    SetVariants & variants,
...
void ISet::setHeader(const ColumnsWithTypeAndName & header)
{
    ...
    initialize_data(key_columns);
    //data.init(data.chooseMethod(key_columns, key_sizes));
    
}


bool ISet::insertFromBlock(const ColumnsWithTypeAndName & columns)
{
    Columns cols;
...
bool ISet::insertFromBlock(const Columns & columns)
{
    std::lock_guard lock(rwlock);
...
void ISet::checkIsCreated() const
{
    if (!is_created.load())
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Logical error: Trying to use set before it has been built.");
}

ColumnPtr ISet::execute(const ColumnsWithTypeAndName & columns, bool negative) const
{
...
bool ISet::empty() const
{
    std::shared_lock lock(rwlock);
    return data.empty();
}

size_t ISet::getTotalRowCount() const
{
    std::shared_lock lock(rwlock);
    return data.getTotalRowCount();
}

size_t ISet::getTotalByteCount() const
{
    std::shared_lock lock(rwlock);
...
template <typename Method>
void NO_INLINE ISet::executeImpl(
    Method & method,
    const ColumnRawPtrs & key_columns,
...
void NO_INLINE ISet::executeImplCase(
    Method & method,
    const ColumnRawPtrs & key_columns,
    ColumnUInt8::Container & vec_res,
    bool negative,
...
void ISet::checkColumnsNumber(size_t num_key_columns) const
{
...
bool ISet::areTypesEqual(size_t set_type_idx, const DataTypePtr & other_type) const
{
...
void ISet::checkTypesEqual(size_t set_type_idx, const DataTypePtr & other_type) const
{
