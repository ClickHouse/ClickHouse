...
std::vector<ActionsDAG::SplitResult> ActionsDAG::splitActionsForJoinCondition(
    const std::string & filter_name,
...
)
{
    ...
    auto left_stream_allowed_nodes = get_input_nodes(left_stream_available_columns);
    auto right_stream_allowed_nodes = get_input_nodes(right_stream_available_columns);
    ...
    auto get_expression_side = [](
        const ActionsDAG::Node * expr,
        const std::unordered_set<const Node *> & left_allowed_inputs,
        const std::unordered_set<const Node *> & right_allowed_inputs
    )
    {
        NodeRawConstPtrs nodes_to_process = { expr };
        ...
        while (!nodes_to_process.empty())
        {
            ...
            if (current->type == ActionType::INPUT)
            {
    ...
    auto create_result = [](const Node * lhs, const Node * rhs)
    {
        auto lhs_dag = createActionsForConjunction({ lhs }, {}).value();
        auto rhs_dag = createActionsForConjunction({ rhs }, {}).value();

        return ActionsDAG::SplitResult{ .first = std::move(lhs_dag), .second = std::move(rhs_dag), .split_nodes_mapping = {} };
    };

    std::vector<ActionsDAG::SplitResult> result;
    for (const auto * conjunct : conjuncts_list)
    {
        ...
        if (is_equality)
        {
            ...
            auto lhs_side = get_expression_side(lhs, left_stream_allowed_nodes, right_stream_allowed_nodes);
            auto rhs_side = get_expression_side(rhs, left_stream_allowed_nodes, right_stream_allowed_nodes);
            ...
            {
                result.emplace_back(create_result(lhs, rhs));
            }
            else if (rhs_side == ExpressionSide::LEFT && lhs_side == ExpressionSide::RIGHT)
            {
                result.emplace_back(create_result(rhs, lhs));
            }
        }
    }
