    ...
    /// Phase 1: Traverse the DAG and determine which nodes can be computed
    {
        ...
        while (!stack.empty())
        {
            ...
            while (frame.next_child_to_visit < frame.node->children.size())
            {
                ...
                /// If child is not computable, try to substitute it with a constant
                if (!it->second)
                {
                    if (frame.node->type == ActionsDAG::ActionType::FUNCTION)
                    {
                        ...
                        if (name == "and" || name == "or")
                        {
                            ...
                            /// Replace with constant: 'and' -> 1, 'or' -> 0
                            UInt64 value = name == "and" ? 1 : 0;
                            column.column = child->result_type->createColumnConst(0, value);
