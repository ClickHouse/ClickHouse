...
private:
    ...
    /// `computeRoutes` left it; no-spill readers merge the atomic registers without that lock.
    struct FillLane
    {
    ...
    /// Register-wise union of spill lanes at the last scheduled estimate refresh.
    mutable DenseHyperLogLog live_merged_hll;
