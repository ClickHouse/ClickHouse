...
/** Distinct-key estimate that sizes the one `HashJoinTable`. One sketch per fill thread. `add`
  ...
  * runs inside the fill's row loop, so each register needs one relaxed atomic load and store per row.
  * One fill lane owns each sketch, while a reader may merge it concurrently. Arithmetic waits for
  * `estimate`, which applies the large-range correction that `HyperLogLogCounter::fixRawEstimate`
  * skips above `2^32 / 30`.
  *
...
struct DenseHyperLogLog
{
    ...
    void merge(const DenseHyperLogLog & other)
    {
        for (size_t i = 0; i < register_count; ++i)
        {
            ...
            value.store(
                std::max(value.load(std::memory_order_relaxed), other.registers[i].load(std::memory_order_relaxed)),
    ...
    double estimate() const
    {
        ...
        {
            const UInt8 rank = value.load(std::memory_order_relaxed);
