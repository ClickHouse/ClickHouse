...
{
    explicit MaterializedCTE(const std::string & cte_name_, bool name_is_double_quoted_);
    ...
    /// True iff the CTE was defined with a double-quoted name (`WITH "MyCte" AS ...`), which stays
    /// case-sensitive in `standard`-mode qualifier matching (unquoted `mycte.x` does not bind to it).
