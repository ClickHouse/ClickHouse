...
void InterpreterSystemQuery::startStopAction(StorageActionBlockType action_type, bool start)
{
    ...
    {
        for (auto & elem : DatabaseCatalog::instance().getDatabases(GetDatabasesOptions{.with_external_databases = false}))
        {
...
void InterpreterSystemQuery::restartReplicas(ContextMutablePtr system_context)
{
    ...
    for (auto & elem : catalog.getDatabases(GetDatabasesOptions{.with_external_databases = false}))
    {
...
void InterpreterSystemQuery::loadOrUnloadPrimaryKeysImpl(bool load)
{
    ...
    else
    {
        ...
        for (auto & database : DatabaseCatalog::instance().getDatabases(GetDatabasesOptions{.with_external_databases = false}))
        {
