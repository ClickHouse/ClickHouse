...
std::optional<String> InterpreterSystemQuery::getDetachedDatabase(const ASTSystemQuery & query_)
{
...
void InterpreterSystemQuery::dropDatabaseReplica(ASTSystemQuery & query)
{
    ...
    else if (!query.replica_zk_path.empty())
    {
        ...
        auto detached_database_name = getDetachedDatabase(query);
        if (detached_database_name)
        {
            auto attach_ctx = Context::createCopy(Context::getGlobalContextInstance());
            attach_ctx->setCurrentQueryId(toString(UUIDHelpers::generateV4()));
            String attach_query = fmt::format("ATTACH DATABASE `{}`", *detached_database_name);
            executeQuery(attach_query, attach_ctx);
            SCOPE_EXIT({
                auto detach_ctx = Context::createCopy(Context::getGlobalContextInstance());
                detach_ctx->setCurrentQueryId(toString(UUIDHelpers::generateV4()));
                String detach_query = fmt::format("DETACH DATABASE `{}`", *detached_database_name);
                executeQuery(detach_query, detach_ctx);
            });

            auto database = DatabaseCatalog::instance().getDatabase(*detached_database_name);
            chassert(database);
            dropStorageReplicasFromDatabase(query.replica, database);
        }
        else
