...
private:
    ...
    static void visit(const ASTIdentifier & node, ASTPtr &, Data & data)
    {
        ...
        auto split = Nested::splitName(node.fullName());  /// ParsedParams, Key1

        if (array_join_alias_to_name.count(node.fullName()))
        {
            /// ARRAY JOIN was written with an array column. Example: SELECT K1 FROM ... ARRAY JOIN ParsedParams.Key1 AS K1
            array_join_result_to_source[node.fullName()] = array_join_alias_to_name[node.fullName()];    /// K1 -> ParsedParams.Key1
        }
        else if (array_join_alias_to_name.count(split.first) && !split.second.empty())
        {
            ...
            array_join_result_to_source[node.fullName()]    /// PP.Key1 -> ParsedParams.Key1
                = Nested::concatenateName(array_join_alias_to_name[split.first], split.second);
        }
        else if (array_join_name_to_alias.count(node.fullName()))
        {
            ...
            */
            array_join_result_to_source[    /// PP.Key1 -> ParsedParams.Key1
                array_join_name_to_alias[node.fullName()]] = node.fullName();
        }
        else if (array_join_name_to_alias.count(split.first) && !split.second.empty())
        {
            ...
            */
            array_join_result_to_source[    /// PP.Key1 -> ParsedParams.Key1
                Nested::concatenateName(array_join_name_to_alias[split.first], split.second)] = node.fullName();
        }
    }
};
