...
/// Ends the comment of every union table generated by `prepareUnionTable`. Together with the shape of
/// the table function (see `isGeneratedUnionTableFunction`) it is the ownership marker that lets
/// `prepareUnionTable` tell such a table from a user's table on the same name, which it must not replace. Changing this text would make the tables generated by older versions look
/// like user tables, so they would be kept with their stale definition.
static constexpr std::string_view UNION_TABLE_COMMENT_MARKER
...
/// Whether `ast` is a string literal holding exactly `expected`.
bool isStringLiteral(const ASTPtr & ast, const String & expected)
{
    const auto * literal = ast->as<ASTLiteral>();
    return literal && literal->value.getType() == Field::Types::String && literal->value.safeGet<String>() == expected;
}

/// Whether `function` is the `merge` table function over `log_table_id` and its rotated versions,
/// exactly as `SystemLog::getCreateUnionTableQuery` builds it.
bool isGeneratedMergeFunction(const ASTFunction & function, const StorageID & log_table_id)
{
    ...
    return isStringLiteral(arguments->children[0], log_table_id.database_name)
        && isStringLiteral(arguments->children[1], getRotatedLogTablesRegexp(log_table_id));
}

/// Whether `function` is one of the table functions that `SystemLog::getCreateUnionTableQuery` generates
/// for `log_table_id`: the `merge` over the log table and its rotated versions, that same `merge` wrapped
/// into `clusterAllReplicas`, or `clusterAllReplicas` over the log table alone.
///
/// The log table these read from is pinned, so a user's proxy over anything else is not mistaken for a
/// generated table even if it carries the generated comment. The name of the cluster and the choice
/// between the shapes come from the server configuration, which may well have changed since the table
/// was generated, so they are not pinned: such a table is still ours and has to be brought up to date.
bool isGeneratedUnionTableFunction(const ASTFunction & function, const StorageID & log_table_id)
{
    if (isGeneratedMergeFunction(function, log_table_id))
        return true;
    ...
    if (positional_arguments.size() == 2)
    {
        ...
        return inner_function && isGeneratedMergeFunction(*inner_function, log_table_id);
    }

    if (positional_arguments.size() == 3)
        return isStringLiteral(positional_arguments[1], log_table_id.database_name)
            && isStringLiteral(positional_arguments[2], log_table_id.table_name);

    return false;
}

/// Whether `create_query_ast` is a definition that `prepareUnionTable` generated for `log_table_id`, and
/// therefore a table that it may replace: a data-free proxy over one of the table functions it generates
/// (see `isGeneratedUnionTableFunction`) which carries the comment that every generated definition has had
/// since the feature appeared, stating that the table can be dropped at any time.
///
...
bool isGeneratedUnionTable(const ASTPtr & create_query_ast, const StorageID & log_table_id)
{
    ...
    const auto * table_function = create && create->as_table_function ? create->as_table_function->as<ASTFunction>() : nullptr;
    const auto * comment = create && create->comment ? create->comment->as<ASTLiteral>() : nullptr;

    return table_function && isGeneratedUnionTableFunction(*table_function, log_table_id)
        && comment && comment->value.getType() == Field::Types::String
        && comment->value.safeGet<String>().ends_with(UNION_TABLE_COMMENT_MARKER);
}
...
void SystemLog<LogElement>::prepareUnionTable()
{
    ...
    try
    {
        ...
        /** The decision to replace was taken above, outside any `DDLGuard`, so between reading that table
          ...
          * the loss this check exists to prevent. `CREATE OR REPLACE` runs this callback under the
          * `DDLGuard` of the target name, immediately before the exchange, so it sees the table that is
          * really about to be replaced; throwing here aborts the replacement and leaves that table alone.
          */
        interpreter.setReplacedTableCheck(
            [this](const StorageID & to_replace_id)
            {
                ASTPtr to_replace_create_query_ast = getCreateTableQueryClean(to_replace_id, getContext());
                if (!isGeneratedUnionTable(to_replace_create_query_ast, table_id))
                    throw Exception(
                        ErrorCodes::TABLE_ALREADY_EXISTS,
                        "The name {} is now taken by a table that this server did not generate for {}. Existing: {}",
                        union_table_id.getNameForLogs(),
                        LogElement::name(),
                        to_replace_create_query_ast->formatWithSecretsOneLine());
            });
...
ASTPtr SystemLog<LogElement>::getCreateUnionTableQuery()
{
    ...
    comment += "\n\n";
    comment += UNION_TABLE_COMMENT_MARKER;
