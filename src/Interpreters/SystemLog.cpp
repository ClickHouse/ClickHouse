    ...
    try
    {
        ...
        if (union_table)
        {
            ...
            /** The `CREATE OR REPLACE` below drops whatever occupies this name, which is only safe for
              ...
              * `clusterAllReplicas` table function and hold no data (see `isGeneratedUnionTable`). Users
              * may create tables in the `system` database, and `system.all_query_log` is exactly the name
              * a hand-rolled union of the rotated logs would take - a `MergeTree` history table, a
            ...
            if (!isGeneratedUnionTable(existing_create_query_ast, table_id))
            {
                LOG_ERROR(
                    ...
                    " would drop that table. Rename or drop it to let the union table be created."
                    "\nExisting: {}\nWanted: {}\n.",
                    union_table_id.getNameForLogs(),
                    LogElement::name(),
        ...
        InterpreterCreateQuery interpreter(create_query_ast, query_context);
        interpreter.setInternal(true);
        /** The decision to replace was taken above, outside any `DDLGuard`, so between reading that table
          * and the exchange that drops it another session can `DROP` the generated proxy and `CREATE` a
          * table of its own on the same name - which the exchange would then drop with its data, exactly
          * the loss this check exists to prevent. `CREATE OR REPLACE` of a generated union table definition
          * therefore re-applies the same ownership predicate itself, under the `DDLGuard` of the target
          * name and immediately before the exchange, and refuses with `TABLE_ALREADY_EXISTS` to replace
          * anything that is not a generated union table of this log (see `getSystemLogOfGeneratedUnionTable`).
          * That rule is decided from the query alone, not passed to this interpreter: in a database that
          * replicates its DDL the query is replayed from its text by the DDL worker, where nothing else
          * would be available.
          */
        try
        {
        ...
        {
            /// The name was taken by a user table while this creation was running (the check above), or by
            /// another table published on it in the same window. Either way it is the collision handled up
            /// front, with the same outcome: leave that table alone and report it.
            if (e.code() != ErrorCodes::TABLE_ALREADY_EXISTS)
                throw;

            LOG_ERROR(
                log,
                "Not creating {} for {}: the name was taken by a table that this server did not generate for it while the"
                " union table was being created, and replacing it would drop that table. Rename or drop it to let the"
                " union table be created.\n{}\nWanted: {}\n.",
                union_table_id.getNameForLogs(),
                LogElement::name(),
                e.message(),
                union_create_query);
            /// Not `union_table_broken`, for the same reason as above: the flush that follows the
            /// removal of that table has to create the union table.
            union_table_check_pending = false;
            return;
        }
...
ASTPtr SystemLog<LogElement>::getCreateUnionTableQuery()
{
    ...
    if (union_table_merge_rotated_tables)
        merge_table_function = makeASTFunction(
            ...
            make_intrusive<ASTLiteral>(getRotatedLogTablesRegexp(table_id)));
    ...
    comment += "\n\n";
    comment += SYSTEM_LOG_UNION_TABLE_COMMENT_MARKER;
