...
/// Ends the comment of every union table generated by `prepareUnionTable`. It doubles as the ownership
/// marker that lets `prepareUnionTable` tell such a table from a user's table on the same name, which
/// it must not replace. Changing this text would make the tables generated by older versions look
    ...
    try
    {
        ...
        if (union_table)
        {
            ...
            /** The `CREATE OR REPLACE` below drops whatever occupies this name, which is only safe for
              ...
              * function, so the marker is the comment that every generated definition has carried since
              * the feature appeared (see `getCreateUnionTableQuery`), which states that the table can be
              * dropped at any time. A user table only carries it if the user copied the generated
              * definition, and that promise was then made deliberately.
              */
