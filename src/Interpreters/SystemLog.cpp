    ...
    try
    {
        ...
        if (union_table)
        {
            ...
            /** The `CREATE OR REPLACE` below drops whatever occupies this name, which is only safe for
              ...
              * the rotated logs would take - a `MergeTree` history table, a materialized view's target,
              * or the user's own `AS merge(...)` proxy. Such a table must not be lost to a name collision,
              * so leave it alone.
              *
              * The table function alone does not tell a generated table from a user's proxy over the same
              * function, so a table is recognized as ours only when it is one of the shapes this feature
              * generates for this very log table (see `isGeneratedUnionTableFunction`) and it carries the
              * comment that every generated definition has had since the feature appeared, which states
              * that the table can be dropped at any time. A user table only matches both if the user
              * copied the generated definition, and that promise was then made deliberately.
              */
            ...
            const auto * existing_comment = existing_create && existing_create->comment ? existing_create->comment->as<ASTLiteral>() : nullptr;
            const bool is_generated_union_table = existing_table_function
                && isGeneratedUnionTableFunction(*existing_table_function, table_id)
                && existing_comment && existing_comment->value.getType() == Field::Types::String
                && existing_comment->value.safeGet<String>().ends_with(UNION_TABLE_COMMENT_MARKER);
