    ...
    Pipes & res)
{
    ...
    if (settings.prefer_localhost_replica && shard_info.isLocal())
    {
        ...
        if (!main_table_storage) /// Table is absent on a local server.
        {
            ...
            {
                LOG_WARNING_FORMATTED(&Logger::get("ClusterProxy::SelectStreamFactory"), "There is no table {} on local replica of shard {}, will try remote replicas.", main_table.getNameForLogs(), shard_info.shard_num);
        ...
        LOG_WARNING_FORMATTED(&Logger::get("ClusterProxy::SelectStreamFactory"), "Local replica of shard {} is stale (delay: {}s.)", shard_info.shard_num, local_delay);
            ...
            -> BlockInputStreamPtr
        {
            ...
            catch (const Exception & ex)
            {
                if (ex.code() == ErrorCodes::ALL_CONNECTION_TRIES_FAILED)
                    LOG_WARNING_FORMATTED(&Logger::get("ClusterProxy::SelectStreamFactory"), "Connections to remote replicas of local shard {} failed, will use stale local replica", shard_num);
                else
                    throw;
            }
