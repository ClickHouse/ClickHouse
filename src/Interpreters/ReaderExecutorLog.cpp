...
ColumnsDescription ReaderExecutorLogElement::getColumnsDescription()
{
    ...
    return ColumnsDescription
    {
        ...
        {"cache_get_microseconds", std::make_shared<DataTypeUInt64>(), "Time spent inside `CacheReader::read`."},
        {"cache_populate_microseconds", std::make_shared<DataTypeUInt64>(), "Time spent inside `CacheWriter::write`."},
        ...
        {"prefetch_wasted_cache_bytes", std::make_shared<DataTypeUInt64>(), "Cache-tier bytes a running prefetch materialised into a chain that was then discarded - near-free, unlike wasted source bytes."},
    };
}
