...
private:
    ...
    void startLoading(Info & info, bool forced_to_reload = false, size_t min_id_to_finish_loading_dependencies_ = 1)
    {
        ...
        if (enable_async_loading)
        {
            ...
            try
            {
                thread = ThreadFromGlobalPool{&LoadingDispatcher::doLoading, this, info.name, loading_id, forced_to_reload, min_id_to_finish_loading_dependencies_, true, getCurrentThreadGroupForAsyncCallback()};
            }
            catch (...)
            {
                cancelLoading(info);
