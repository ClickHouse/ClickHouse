...
public:
    ...
    private:
        bool need_flags;
        const UsedFlagsForBlock * flags;

    public:
        UsedFlagsHolder(bool need_flags_, const UsedFlagsForBlock * flags_)
    ...
    UsedFlagsHolder getUsedFlagsHolder(const Block * block_ptr) const
    {
        if (auto it = flags.find(block_ptr); it != flags.end())
