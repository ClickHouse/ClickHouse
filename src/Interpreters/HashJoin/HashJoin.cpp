...
void HashJoin::joinBlock(ScatteredBlock & block, ScatteredBlock & remaining_block)
{
    ...
    const bool joined = joinDispatch(
        ...
        [&](auto kind_, auto strictness_, auto & maps_vector_)
        {
            ...
            //     "block.rows()={}, block.dumpStructure()={}, remaining_block.rows()={}, remaining_block.dumpStructure()={}",
            //     block.rows(),
            //     block.getSourceBlock().dumpStructure(),
            //     remaining_block.rows(),
            //     remaining_block.getSourceBlock().dumpStructure());
            // if (remaining_block.rows())
