...
private:
    ...
    size_t fillColumnsFromData(const HashJoin::StoredBlocksList & columns, MutableColumns & columns_right)
    {
        ...
        {
            /// Resolve a decompressed view of the stored block when compression is active; identity otherwise.
            const StoredBlock & block = *resolve(&*block_it);
            size_t rows_from_block = std::min<size_t>(max_block_size - rows_added, block.columns.at(0)->size() - current_block_start);
            for (size_t j = 0; j < columns_right.size(); ++j)
            {
                if (const auto * replicated_column = block.replicated_columns[j])
                {
                ...
                {
                    const auto & col = block.columns[j];
            ...
            if (rows_added >= max_block_size)
            {
                ...
                if (block.columns.at(0)->size() <= current_block_start)
                {
