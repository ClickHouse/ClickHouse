...
void InterpreterCreateQuery::convertMergeTreeTableIfPossible(ASTCreateQuery & create, DatabasePtr database, bool to_replicated)
{
    ...
    /// Save new metadata. The validation above rejects every conversion the target engine refuses
    /// before this rewrite, so a rejected ATTACH AS [NOT] REPLICATED never reaches this point and
    /// the persisted metadata is only ever rewritten for a conversion that will construct.
