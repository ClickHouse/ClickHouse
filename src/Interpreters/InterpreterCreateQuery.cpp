    ...
    if (create.columns_list)
    {
        ...
        if (create.columns_list->indices)
            for (const auto & index : create.columns_list->indices->children)
            {
                ...
                if (index_desc.type == "vector_similarity" && !settings.allow_experimental_usearch_index)
                    throw Exception(ErrorCodes::INCORRECT_QUERY, "Vector similarity index is disabled. Turn on allow_experimental_usearch_index");
