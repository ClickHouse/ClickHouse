...
MaterializedColumnDependencies::MaterializedColumnDependencies(const ColumnsDescription & columns, const ContextPtr & context)
{
    ...
    for (const auto & column : columns)
    {
        ...
        /// A MATERIALIZED default may read an ALIAS column, and an ALIAS is computed on read and
        /// never stored. Merely adding aliases to the analysis set would let TreeRewriter resolve
        /// the name, but the expression would then demand a column that is not in any part and the
        /// recompute stage below would fail on it, so the reference has to be replaced by what it
        /// stands for. Same visitor `IndexDescription::initExpressionInfo` uses for a skip index
        /// over an ALIAS column. Done before the subcolumn rewrite, so a subcolumn reached through
        /// an alias is normalized too.
