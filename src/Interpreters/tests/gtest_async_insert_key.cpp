...
TEST(AsyncInsertKey, QuotaKeyIsPartOfKey)
{
    /// A quota `KEYED BY client_key` buckets accounting per key value, so inserts with different
    /// quota keys must never coalesce into one flush that bills a single bucket.
    String query_str = "INSERT INTO test (a, b, c) VALUES (1, 2, 3)";
    ParserInsertQuery parser(query_str.data() + query_str.size(), false);
    ASTPtr query = parseQuery(parser, query_str, DBMS_DEFAULT_MAX_QUERY_SIZE, DBMS_DEFAULT_MAX_PARSER_DEPTH, DBMS_DEFAULT_MAX_PARSER_BACKTRACKS);
    ...
    auto make_key = [&](const String & quota_key)
    {
        return AsynchronousInsertQueue::InsertQuery(query, {}, {}, {}, {}, {}, {}, {}, {}, quota_key, settings, kind);
    };

    /// A keyed insert must never coalesce with an unkeyed one (empty quota key).
    auto key_none = make_key("");
    auto key_a = make_key("tenant-a");
    EXPECT_NE(key_none.hash, key_a.hash);
    EXPECT_NE(key_none, key_a);

    /// Two different quota keys must never coalesce with each other.
    auto key_b = make_key("tenant-b");
    EXPECT_NE(key_a.hash, key_b.hash);
    EXPECT_NE(key_a, key_b);

    /// The same quota key still coalesces (batching preserved within one bucket).
    auto key_a_again = make_key("tenant-a");
    EXPECT_EQ(key_a.hash, key_a_again.hash);
    EXPECT_EQ(key_a, key_a_again);
}
