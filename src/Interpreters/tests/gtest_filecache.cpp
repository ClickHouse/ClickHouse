...
TEST_F(FileCacheTest, ExposeEvictionMetrics_SLRUPromotionInducedEviction)
{
    ...
    auto query_context = DB::Context::createCopy(getContext().context);
    query_context->makeQueryContext();
    query_context->setCurrentQueryId("eviction_metrics_promo_test");
    auto query_scope_holder = DB::QueryScope::create(query_context);
    ...
    const size_t file_size = static_cast<size_t>(-1);
    const auto & user = FileCache::getCommonOrigin();
    ...
    /// fills (slru_size_ratio=0.5 → 15 bytes protected), each subsequent
    /// promotion forces downgrade of an existing protected entry.
    /// The downgraded entry hits a full probationary, forcing eviction.
    for (size_t i = 0; i < 6; ++i)
    {
        auto holder = cache.getOrSet(key, i * 10, 5, file_size, {}, 0, user);
        ASSERT_EQ(holder->size(), 1u);
        download(*holder->begin());
    }
    for (size_t i = 0; i < 6; ++i)
...
TEST_F(FileCacheTest, ExposeEvictionMetrics_PruneByClientMetrics)
{
    /// Verify that FileCache::pruneByClientMetrics removes _by_client metric
    /// series for a given user_id without touching series for other users.
    /// This exercises the cleanup path that ~CacheUsage invokes via on_drained.
    ServerUUID::setRandomForUnitTests();
    DB::ThreadStatus thread_status;
    ...
    auto query_context = DB::Context::createCopy(getContext().context);
    query_context->makeQueryContext();
    query_context->setCurrentQueryId("eviction_metrics_prune_test");
    chassert(&DB::CurrentThread::get() == &thread_status);
    auto query_scope_holder = DB::QueryScope::create(query_context);

    auto count_by_client = [](const std::string & cache_name, const std::string & user_id)
    {
        size_t count = 0;
        ::DimensionalMetrics::Factory::instance().forEachFamily([&](::DimensionalMetrics::MetricFamily & f)
        {
            const auto & labels = f.getLabels();
            ...
            if (cn_idx >= labels.size() || cid_idx >= labels.size())
                return;
            f.forEachMetric([&](const ::DimensionalMetrics::LabelValues & vals, const ::DimensionalMetrics::Metric & m)
            {
                if (vals[cn_idx] == cache_name && vals[cid_idx] == user_id && m.get() > 0)
                    ++count;
            });
        });
        return count;
    ...
    auto evict_for_user = [&](DB::FileCache & cache, const std::string & user_id)
    {
        DB::FileCacheOriginInfo user(user_id, 0, DB::FileSegmentKeyType::General);
        auto key = FileCacheKey::fromPath(cache.getName() + "_key");
        for (size_t i = 0; i < 8; ++i)
        {
            auto holder = cache.getOrSet(key, i * 10, 5, static_cast<size_t>(-1), {}, 0, user);
            ASSERT_EQ(holder->size(), 1u);
            download(*holder->begin());
        }
    };

    /// Two caches, each with a distinct user who evicts.
    auto settings_a = make_settings();
    settings_a[FileCacheSetting::path] = cache_base_path2;
    DB::FileCache cache_a("prune_cache_a", settings_a);
    cache_a.initialize();
    evict_for_user(cache_a, "user_alice");

    auto settings_b = make_settings();
    settings_b[FileCacheSetting::path] = cache_base_path3;
    DB::FileCache cache_b("prune_cache_b", settings_b);
    cache_b.initialize();
    evict_for_user(cache_b, "user_bob");

    ASSERT_GT(count_by_client("prune_cache_a", "user_alice"), 0u);
    ASSERT_GT(count_by_client("prune_cache_b", "user_bob"), 0u);

    /// Simulate what ~CacheUsage does via on_drained: prune user_alice's series.
    cache_a.pruneByClientMetrics("user_alice");

    EXPECT_EQ(count_by_client("prune_cache_a", "user_alice"), 0u)
        << "pruneByClientMetrics did not remove user_alice's series";
    EXPECT_GT(count_by_client("prune_cache_b", "user_bob"), 0u)
        << "pruneByClientMetrics incorrectly removed user_bob's series";
}
