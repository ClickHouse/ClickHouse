...
TEST(QueryConditionCacheTimeConditions, DeterministicConditionIsNotDerived)
{
    ...
    EXPECT_FALSE(deriveDeterministicTimeCondition(&condition, TimeConditionRounding::Weaken, 1.0, day_start + 10 * day));
    EXPECT_FALSE(deriveDeterministicTimeCondition(&condition, TimeConditionRounding::Strengthen, 1.0, day_start + 10 * day));
}
...
TEST(QueryConditionCacheTimeConditions, LowerBoundRounding)
{
    ...
    auto weakened = deriveDeterministicTimeCondition(test.condition, TimeConditionRounding::Weaken, 1.0, noon + 10 * day);
    auto strengthened = deriveDeterministicTimeCondition(test.condition, TimeConditionRounding::Strengthen, 1.0, noon + 10 * day);
...
TEST(QueryConditionCacheTimeConditions, WriteKeyMeetsNextCellReadKey)
{
    ...
    auto write_key = deriveDeterministicTimeCondition(writer.condition, TimeConditionRounding::Strengthen, 1.0, constant_of_writer + 10 * day);
    auto read_key = deriveDeterministicTimeCondition(reader.condition, TimeConditionRounding::Weaken, 1.0, constant_of_reader + 10 * day);
...
TEST(QueryConditionCacheTimeConditions, AlignedConstantIsUnchangedInBothDirections)
{
    ...
    auto weakened = deriveDeterministicTimeCondition(test.condition, TimeConditionRounding::Weaken, 1.0, day_start + 10 * day);
    auto strengthened = deriveDeterministicTimeCondition(test.condition, TimeConditionRounding::Strengthen, 1.0, day_start + 10 * day);
    ASSERT_TRUE(weakened);
    ASSERT_TRUE(strengthened);
...
TEST(QueryConditionCacheTimeConditions, DateConstantIsIdentityRounded)
{
    ...
    auto weakened = deriveDeterministicTimeCondition(&condition, TimeConditionRounding::Weaken, 1.0, day_start + day / 2);
    auto strengthened = deriveDeterministicTimeCondition(&condition, TimeConditionRounding::Strengthen, 1.0, day_start + day / 2);
    auto weakened_later = deriveDeterministicTimeCondition(&condition, TimeConditionRounding::Weaken, 1.0, day_start + day - 1);
    ASSERT_TRUE(weakened);
    ASSERT_TRUE(strengthened);
...
TEST(QueryConditionCacheTimeConditions, NotFlipsTheDirection)
{
    ...
    auto weakened = deriveDeterministicTimeCondition(&condition, TimeConditionRounding::Weaken, 1.0, noon + 10 * day);
    ASSERT_TRUE(weakened);
    EXPECT_NE(weakened->condition.find(std::to_string(day_start + day)), String::npos) << weakened->condition;
}
...
TEST(QueryConditionCacheTimeConditions, GridStepFollowsTheInterval)
{
    ...
    auto weakened = deriveDeterministicTimeCondition(test.condition, TimeConditionRounding::Weaken, 1.0, constant + 3600);
    ASSERT_TRUE(weakened);
...
TEST(QueryConditionCacheTimeConditions, UnsupportedShapesAreNotDerived)
{
...

    {
        ...
        EXPECT_FALSE(deriveDeterministicTimeCondition(test.condition, TimeConditionRounding::Weaken, 1.0, now));
    }
    {
    ...
    }
    {
        ...
        EXPECT_FALSE(deriveDeterministicTimeCondition(&condition, TimeConditionRounding::Weaken, 1.0, now));
    }
    {
        /// A non-positive grid factor disables the derivation.
        ComparisonCase test("greaterOrEquals", dateTimeUTC(), Field(static_cast<UInt64>(noon)));
        EXPECT_FALSE(deriveDeterministicTimeCondition(test.condition, TimeConditionRounding::Weaken, 0.0, now));
    }
}
