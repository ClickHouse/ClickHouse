...
/// One deliberate exception: an identity conversion returns the input column unchanged, so where a
/// `Field` is lossy (a `Variant` discriminator) the two differ and the column twin is the correct one.
struct Case
{
