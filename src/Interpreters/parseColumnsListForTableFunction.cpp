...
namespace Setting
{
    ...
    extern const SettingsBool allow_experimental_version_type;
...
DataTypeValidationSettings::DataTypeValidationSettings(const DB::Settings & settings)
    ...
    , allow_experimental_version_type(settings[Setting::allow_experimental_version_type])
...
void validateDataType(const DataTypePtr & type_to_check, const DataTypeValidationSettings & settings)
{
    auto validate_callback = [&](const IDataType & data_type)
    {
        ...
        if (!settings.allow_experimental_version_type)
        {
            if (isVersion(data_type))
            {
                throw Exception(
                    ErrorCodes::ILLEGAL_COLUMN,
                    "Cannot create column with type '{}' because Version type is not allowed. "
                    "Set setting allow_experimental_version_type = 1 in order to allow it",
                    data_type.getName());
            }
        }
