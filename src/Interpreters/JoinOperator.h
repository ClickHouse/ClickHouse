...
struct JoinSettings
{
    ...
    /// Only consulted to decide what a serialized plan must carry, so the question is always about the
    /// peer that will execute the deserialized plan: whether temporary storage is configured there cannot
    /// be observed from here, and the local answer is not a proxy for it.
