...
namespace Setting
{
    ...
    extern const SettingsInsertSelectDeduplicateMode insert_select_deduplicate;
...
QueryPipeline InterpreterInsertQuery::addInsertToSelectPipeline(ASTInsertQuery & query, StoragePtr table, QueryPipelineBuilder & pipeline)
{
    ...
    {
        switch (settings[Setting::insert_select_deduplicate].value)
        {
            case InsertSelectDeduplicateMode::FORCE_ENABLE:
                return true;
            case InsertSelectDeduplicateMode::DISABLE:
                return false;
            case InsertSelectDeduplicateMode::ENABLE_EVEN_FOR_BAD_QUERIES:
                return settings[Setting::insert_deduplicate];
            case InsertSelectDeduplicateMode::ENABLE_WHEN_PROSSIBLE:
            {
                if (!select_query_sorted && settings[Setting::insert_deduplicate])
                    LOG_INFO(logger, "INSERT SELECT deduplication is disabled because SELECT is not stable");

                return select_query_sorted && settings[Setting::insert_deduplicate];
            }
        }
    }();
    ...
    {
        if (settings[Setting::insert_select_deduplicate] == InsertSelectDeduplicateMode::FORCE_ENABLE)
        ...
        chassert(settings[Setting::insert_select_deduplicate] == InsertSelectDeduplicateMode::ENABLE_EVEN_FOR_BAD_QUERIES);
