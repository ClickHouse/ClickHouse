...
QueryPipeline InterpreterInsertQuery::addInsertToSelectPipeline(ASTInsertQuery & query, StoragePtr table, QueryPipelineBuilder & pipeline)
{
    ...
    {
        if (settings[Setting::deduplicate_insert_select] == DeduplicateInsertSelectMode::FORCE_ENABLE)
        ...
        chassert(settings[Setting::deduplicate_insert_select] == DeduplicateInsertSelectMode::ENABLE_EVEN_FOR_BAD_QUERIES);
    ...
    if (deduplicate_insert_select != bool(settings[Setting::insert_deduplicate]))
    {
