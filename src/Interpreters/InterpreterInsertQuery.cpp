    ...
    if (query.select && table->isRemote() && settings.parallel_distributed_insert_select)
    {
        ...
        if (storage_src && storage_dst && storage_src->getClusterName() == storage_dst->getClusterName())
        {
            ...
            for (size_t shard_index : ext::range(0, shards_info.size()))
            {
                ...
                {
                    InterpreterInsertQuery interpreter(new_query, getContext());
                ...
                else
                {
                    ...
                    auto in_stream = std::make_shared<RemoteBlockInputStream>(std::move(connections), new_query_str, Block{}, getContext());
            ...
            res.pipeline = QueryPipeline::unitePipelines(std::move(pipelines), {}, ExpressionActionsSettings::fromContext(getContext()));
        }
    }
