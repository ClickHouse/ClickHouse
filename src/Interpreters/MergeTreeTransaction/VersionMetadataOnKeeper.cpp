...
void VersionMetadataOnKeeper::loadMetadata()
{
    ...
    {
        LOG_TEST(log, "Object {}, metadata_version {}, load metadata content\n{}", getObjectName(), stat.version, content);
...
void VersionMetadataOnKeeper::storeMetadata(bool force)
{
    ...
    LOG_TEST(log, "Object {}, metadata_version {}, store metadata content\n{},", getObjectName(), *metadata_version, content);
}
...
void VersionMetadataOnKeeper::setRemovalTIDLock(TIDHash removal_tid_hash)
{
    auto zookeeper = get_zk_func();
    if (removal_tid_hash)
    {
        ...
        if (removal_tid_hash != locked_by)
            throw Exception(
                ErrorCodes::LOGICAL_ERROR, "Removal TID lock is mismatched, expected {}, actual {}", removal_tid_hash, locked_by);
        return;
    }
...
void VersionMetadataOnKeeper::appendCreationCSNToStoredMetadataImpl()
{
    LOG_TEST(merge_tree_data_part->storage.log, "Object {}, appending creation_csn {}", getObjectName(), getCreationCSN());
...
void VersionMetadataOnKeeper::appendRemovalCSNToStoredMetadataImpl()
{
    LOG_TEST(merge_tree_data_part->storage.log, "Object {}, appending removal_csn {}", getObjectName(), getRemovalCSN());

    if (!metadata_version.has_value())
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Appending removal_csn before loading metadata");
...
void VersionMetadataOnKeeper::appendRemovalTIDToStoredMetadataImpl(const TransactionID & tid)
{
    LOG_TEST(
        merge_tree_data_part->storage.log, "Appending removal TID for {} (creation: {}, removal {})", getObjectName(), creation_tid, tid);
    ...
    WriteBufferFromString buf(content, AppendModeTag{});
    writeRemovalTIDToBuffer(buf, tid);
    buf.finalize();
