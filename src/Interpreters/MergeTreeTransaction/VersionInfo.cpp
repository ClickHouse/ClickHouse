...
bool VersionInfo::isVisible(CSN snapshot_version, TransactionID current_tid) const
{
    ...
    if (creation_tid.isPrehistoric())
        return true;

    [[maybe_unused]] bool had_creation_csn = creation_csn != Tx::UnknownCSN;
    [[maybe_unused]] bool had_removal_tid_lock = removal_tid_lock != 0;
    ...
    if (removal_csn && removal_csn <= snapshot_version)
        return false;
    if (!current_tid.isEmpty() && removal_tid_lock && removal_tid_lock == current_tid.getHash())
        return false;
    ...
    if (creation_csn && creation_csn <= snapshot_version && !removal_tid_lock)
        return true;
    ...
    chassert(current_tid.isEmpty() || (creation_tid != current_tid && removal_tid_lock != current_tid.getHash()));
