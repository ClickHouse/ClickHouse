...
public:
    ...
    /// For `OpenExisting` parts, sets `is_persist_deferrable` to true if no metadata file exists yet
    /// (to defer writes for non-transactional parts). For `CreateFresh` parts, the directory is guaranteed
    /// clean, so `is_persist_deferrable` stays true without probing the disk.
...
private:
    /// Internal implementation of `storeInfo` that requires `persisted_info_mutex` to be held by the caller.
    ...
    std::expected<Int32, StaleVersion> storeInfoUnlocked(VersionInfo new_info) TSA_REQUIRES(persisted_info_mutex);
    ...
    /// Requires `persisted_info_mutex` to be held by the caller.
    /// If `deferred_persist_info` is set, return it.
