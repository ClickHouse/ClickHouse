...
#define LIST_OF_FILE_CACHE_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(NonZeroUInt64, load_metadata_threads, FILECACHE_DEFAULT_LOAD_METADATA_THREADS, "Maximum number of threads used to load cache metadata at server startup", 0) \
    ...
    DECLARE(Bool, write_cache_per_user_id_directory, false, "Deprecated setting, ignored. The behavior is now determined automatically based on cache policy", SettingsTierType::OBSOLETE) \
    ...
    DECLARE(Bool, expose_prometheus_cache_usage_metrics_per_user, false, "Expose Prometheus gauges for filesystem cache usage by cache name and cache client id (the `user_id` label identifies the filesystem cache client, not the SQL user). Off by default. Cardinality grows with distinct cache clients. Restart is required to change this setting: usage is accounted per cache entry from the moment the entry is created, so a cache which was populated while the setting was off cannot be accounted retroactively.", 0) \
...
ColumnsDescription FileCacheSettings::getColumnsDescription()
{
    ...
    for (const auto & setting : impl.all())
    {
        ...
        desc.type = [&]() -> DataTypePtr
        {
            ...
            if (type_name == "UInt64" || type_name == "NonZeroUInt64")
...
void FileCacheSettings::warnAboutObsoleteSettings() const
{
    for (const auto & setting : impl->allChanged())
    {
        if (setting.getTier() == SettingsTierType::OBSOLETE)
            LOG_WARNING(
                ...
                "Filesystem cache setting `{}` is obsolete and has no effect: {}",
...
void FileCacheSettings::validate()
{
    ...
    if (settings[FileCacheSetting::use_split_cache] && isOvercommitPolicy(settings[FileCacheSetting::cache_policy]))
        throw Exception(
