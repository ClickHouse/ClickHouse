...
#define LIST_OF_FILE_CACHE_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(Bool, use_real_disk_size, false, "Account cached file sizes in filesystem block-aligned units (an approximation of the physical on-disk size, not the exact allocated size) instead of written bytes, so FilesystemCacheSize and the eviction-size metrics reflect on-disk block occupancy. Restart is required to change this setting.", 0) \
