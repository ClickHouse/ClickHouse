...
#define LIST_OF_FILE_CACHE_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(Bool, expose_prometheus_cache_usage_metrics_per_user, false, "Expose Prometheus gauges for filesystem cache usage by cache name and user id. Off by default. Cardinality grows with distinct cache users.", 0) \
