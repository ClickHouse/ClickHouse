...
#define LIST_OF_FILE_CACHE_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(Bool, filesystem_cache_expose_prometheus_eviction_metrics, false, "Expose Prometheus metrics for filesystem cache eviction activity (filesystem_cache_evictions_total etc.). Off by default. Can be toggled at runtime via SYSTEM RELOAD CONFIG.", 0) \
    DECLARE(Bool, filesystem_cache_expose_prometheus_eviction_metrics_per_client, false, "Additionally expose per-user-id eviction metrics. Requires filesystem_cache_expose_prometheus_eviction_metrics. Cardinality grows with distinct evicting users.", 0) \
