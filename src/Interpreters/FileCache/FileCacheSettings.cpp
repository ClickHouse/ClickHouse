...
#define LIST_OF_FILE_CACHE_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(Bool, expose_eviction_metrics, false, "Expose Prometheus metrics describing eviction activity for this cache (filesystem_cache_evictions_total, _evicted_bytes_total, _evicted_segment_hits, _evicted_segment_size_bytes). Off by default.", 0) \
    DECLARE(Bool, expose_eviction_metrics_per_client, false, "Additionally expose `_by_client` variants of the eviction metrics, labelled by user id. Requires expose_eviction_metrics. Cardinality grows with the number of distinct evicting users - off by default.", 0) \
