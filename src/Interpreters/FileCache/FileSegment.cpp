    ...
    size_t reserve_hint)
{
...
#if USE_ROCKSDB
    /// Capture before `tryReserve` below mutates `reserved_size`: this is the first reservation
    /// for the segment when nothing has been reserved or downloaded yet. `reserved_size` is always
    /// `>= current_downloaded_size` (checked above), so `reserved_size == 0` means no prior reservation.
    const bool is_first_reservation = (current_downloaded_size == 0 && reserved_size == 0);
#endif
    ...
    if (is_first_reservation)
    {
        ...
        {
            tryLogCurrentException(getLog(), "Failed to add entry to RocksDB index");
        }
    }
#endif
...
void FileSegment::setDownloadedUnlocked(const FileSegmentGuard::Lock & lock)
{
    ...
    {
        tryLogCurrentException(getLog(), "Failed to update entry in RocksDB index");
    }
#endif
...
void FileSegment::detach(const FileSegmentGuard::Lock & lock, const LockedKey &)
{
    ...
    if (added_to_rocksdb)
    {
        ...
        {
            tryLogCurrentException(getLog(), "Failed to remove entry from RocksDB index");
        }
    }
#endif
}
