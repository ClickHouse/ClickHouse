    ...
    std::lock_guard control_lock(control_mutex);
    {
        ...
        if (it != storage_locks.end())
        {
            if (it->second.belongsTo(table))
            {
                ...
                if (action_type == ActionLocks::ViewRefresh || action_type == ActionLocks::ViewRefreshPause)
                {
                    /// `RefreshTask::start` resumes both controls in a single operation.
                    locks.erase(ActionLocks::ViewRefresh);
                    locks.erase(ActionLocks::ViewRefreshPause);
                }
                else
                    locks.erase(action_type);
