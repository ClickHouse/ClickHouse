...
namespace
{
DB::ColumnNumbers calculateKeysPositions(const DB::Block & header, const DB::Streaming::Aggregator::Params & params)
{
    ...
    AggregateDataPtr overflow_row) const
{
    ...
    for (size_t i = 0; i < aggregate_functions.size(); ++i)
    {
        ...
        if (inst->offsets)
            ...
            inst->batch_that->addBatch(row_begin, row_end, places.get(), inst->state_offset, inst->batch_arguments, aggregates_pool, -1);
    }
    ...
    Arena * arena)
{
    ...
    {
        if (inst->offsets)
            inst->batch_that->addBatchSinglePlaceFromInterval(inst->offsets[row_begin], inst->offsets[row_end - 1], res + inst->state_offset, inst->batch_arguments, arena, -1);
        else
            inst->batch_that->addBatchSinglePlaceFromInterval(row_begin, row_end, res + inst->state_offset, inst->batch_arguments, arena, -1);
    }
}
    ...
    bool & no_more_keys) const
{
        ...
        && worth_convert_to_two_level)
    {
        ...
        if (!enoughSpaceInDirectory(tmp_path, size))
            throw Exception(ErrorCodes::NOT_ENOUGH_SPACE, "Not enough space for external aggregation in {}", tmp_path);
...
void Aggregator::writeToTemporaryFile(AggregatedDataVariants & data_variants, const String & tmp_path) const
{
    ...
    NativeWriter block_out(compressed_buf, ClickHouseRevision::getVersionRevision(), getHeader(false));
...
Block Aggregator::convertOneBucketToBlockFinal([[maybe_unused]] AggregatedDataVariants & variants, [[maybe_unused]] ConvertAction action, [[maybe_unused]] size_t bucket) const
{
     throw Exception(ErrorCodes::UNKNOWN_AGGREGATED_DATA_VARIANT, "Unknown aggregated data variant.");
}

Block Aggregator::convertOneBucketToBlockIntermediate([[maybe_unused]] AggregatedDataVariants & variants, [[maybe_unused]] ConvertAction action,[[maybe_unused]]  size_t bucket) const
{
    throw Exception(ErrorCodes::UNKNOWN_AGGREGATED_DATA_VARIANT, "Unknown aggregated data variant.");
}
    ...
    if (!no_more_keys && params.max_rows_to_group_by && result_size > params.max_rows_to_group_by)
    {
        switch (params.group_by_overflow_mode)
        {
            case OverflowMode::THROW:
                throw Exception(ErrorCodes::TOO_MANY_ROWS, "Limit for rows to GROUP BY exceeded: has {} rows, maximum: {}", toString(result_size), toString(params.max_rows_to_group_by));
    ...
    ThreadPool * thread_pool) const
{
    ...
    /// FIXME : separate final vs non-final converting. For non-final converting, we don't need
...
ManyAggregatedDataVariantsPtr Aggregator::prepareVariantsToMerge(ManyAggregatedDataVariants & data_variants) const
{
    ...
    if (non_empty_data->size() > 1)
    {
        ...
        /// at position 0.
        auto result_variants = std::make_shared<AggregatedDataVariants>();
...
void Aggregator::mergeBlocks(BucketToBlocks bucket_to_blocks, AggregatedDataVariants & result, size_t max_threads)
{
    ...
    if (has_two_level)
    {
        ...
        auto merge_bucket = [&bucket_to_blocks, &result, this](size_t bucket, Arena * aggregates_pool, ThreadGroupPtr thread_group)
        {
            SCOPE_EXIT_SAFE(
                if (thread_group)
                    CurrentThread::detachFromGroupIfNotDetached();
