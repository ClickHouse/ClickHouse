...
void RewriteCountDistinctFunctionMatcher::visit(ASTPtr & ast, Data & /*data*/)
{
    ...
    if (!expr_list || expr_list->children.size() != 1)
        return;
    auto * func = expr_list->children.front()->as<ASTFunction>();
    if (!func || (Poco::toLower(func->name) != "countdistinct" && Poco::toLower(func->name) != "uniqexact"))
        return;
    auto arg = func->arguments->children;
    if (arg.size() != 1)
        return;
    if (!arg.front()->as<ASTIdentifier>())
        return;
    if (selectq->tables()->as<ASTTablesInSelectQuery>()->children.front()->as<ASTTablesInSelectQueryElement>()->children.size() != 1)
        return;
    auto * table_expr = selectq->tables()->as<ASTTablesInSelectQuery>()->children.front()->as<ASTTablesInSelectQueryElement>()->children.front()->as<ASTTableExpression>();
    ...
    expr_list->children.front() = makeASTFunction("count");
    ...
    table_expr->subquery = table_expr->children.front();

    auto column_name = arg.front()->as<ASTIdentifier>()->name();
    // Form AST for subquery
    {
        ...
        table_expr->children.front()->as<ASTSubquery>()->children.emplace_back(select_with_union);
    }
}
