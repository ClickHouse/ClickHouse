    ...
    , max_rows_per_postings_list(max_rows_per_postings_list_)
{
    ...
    std::unordered_set supported_tokenizers{
        details::TOKENIZER_DEFAULT,
        details::TOKENIZER_NGRAM,
        details::TOKENIZER_NOOP,
#if USE_CPPJIEBA
        details::TOKENIZER_CHINESE,
#endif
    };
    if (!supported_tokenizers.contains(tokenizer))
        throw Exception(
            ErrorCodes::BAD_ARGUMENTS,
            "The tokenizer of gin index must be one of [{}], but got {}",
            boost::algorithm::join(std::vector<String>(supported_tokenizers.begin(), supported_tokenizers.end()), ", "),
            tokenizer);
}
...
bool GinFilter::contains(const GinFilter & filter, PostingsCacheForStore & cache_store, GinMatch match) const
{
    ...
    return match == GinMatch::ALL ? matchInRange<GinMatch::ALL>(rowid_ranges, *postings_cache)
                                  : matchInRange<GinMatch::ANY>(rowid_ranges, *postings_cache);
}
}
