    ...
    if (options.to_stage > QueryProcessingStage::FetchColumns)
    {
        ...
        if (intermediate_stage)
        {
            ...
            if (expressions.need_aggregate)
                executeMergeAggregated(query_plan, aggregate_overflow_row, aggregate_final, use_grouping_set_key, optimize_distributed_aggregation);
        }
        ...
        if (expressions.first_stage)
        {
            ...
            {
                executeAggregation(
                    query_plan, expressions.before_aggregation, aggregate_overflow_row, aggregate_final, query_info.input_order_info, optimize_distributed_aggregation);
...
void InterpreterSelectQuery::executeAggregation(QueryPlan & query_plan, const ActionsDAGPtr & expression, bool overflow_row, bool final, InputOrderInfoPtr group_by_info, bool optimize_distributed_aggregation)
{
    ...
    auto aggregator_params = getAggregatorParams(query_ptr, *query_analyzer, *context, header_before_aggregation, keys, aggregates, overflow_row, settings,
                 settings.group_by_two_level_threshold, settings.group_by_two_level_threshold_bytes, optimize_distributed_aggregation);
...
void InterpreterSelectQuery::executeMergeAggregated(QueryPlan & query_plan, bool overflow_row, bool final, bool has_grouping_sets, bool optimize_distributed_aggregation)
{
    auto & query = getSelectQuery();
...
void InterpreterSelectQuery::executeRollupOrCube(QueryPlan & query_plan, Modificator modificator)
{
    ...
    auto params = getAggregatorParams(query_ptr, *query_analyzer, *context, header_before_transform, keys, query_analyzer->aggregates(), false, settings, 0, 0, false);
