    ...
    const Context & context)
{
    for (const auto & additional_filter : additional_table_filters)
    {
        ...
        if (table == target.alias ||
            (table == target.table && context.getCurrentDatabase().getFullName() == target.database) ||
    ...
    , prepared_sets(prepared_sets_)
{
    ...
    {
        rewriteMultipleJoins(query_ptr, joined_tables.tablesWithColumns(), context->getCurrentDatabase().getFullName(), context->getSettingsRef());
