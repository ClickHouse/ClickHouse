...
public:
    ...
    std::optional<WasmVal> operator()(WasmCompartment * compartment, std::span<WasmVal> args) const override
    {
        if (args.size() != sizeof...(Args))
            throw Exception(ErrorCodes::WASM_ERROR, "WebAssembly function '{}' expects {} arguments, got {}", function_name, sizeof...(Args), args.size());
    ...
    std::vector<WasmValKind> getArgumentTypes() const override
    {
        return {WasmValTypeToKind<Args>::value...};
    }
