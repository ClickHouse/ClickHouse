...
Block ArrayJoinResultIterator::extractJoinedNested(const ColumnPtr & cut_any_col, const ColumnArray & cut_any_array, size_t window_rows) const
{
    const auto & columns = array_join->columns;
...
Block ArrayJoinResultIterator::next()
{
    ...
    const auto & columns = array_join->columns;
    const bool has_filter = array_join->element_filter != nullptr;

    /// With an element filter a window may end up empty; skip such windows here rather than emit empty
    /// chunks (ArrayJoinTransform is inflating). Without a filter this loop always returns on the first pass.
    while (current_row < total_rows)
    {
        ...
        Block joined_nested = extractJoinedNested(cut_any_col, cut_any_array, window_rows);

        /// Without a filter every element survives; with one, evaluate the mask over the element block.
        IColumn::Filter mask;
        IColumn::Offsets survivor_offsets;
        size_t survivors = cut_any_array.getData().size();
        if (has_filter)
        {
            size_t num_elements = survivors;
            array_join->element_filter->execute(joined_nested, num_elements);
            const auto & filter_column = joined_nested.getByName(array_join->element_filter_column_name).column;
            ...
            survivors = countBytesInFilter(mask);
            ...
            survivor_offsets.resize(window_rows);
        ...
        const IColumn::Offsets & replicate_offsets = has_filter ? survivor_offsets : window_offsets;

        Block res;
        ColumnPtr lazy_indexes;
        for (size_t i = 0; i != block.columns(); ++i)
        {
            ColumnWithTypeAndName current = block.safeGetByPosition(i);
            if (columns.contains(current.name))
            {
                const auto & element = joined_nested.getByName(current.name);
                current.column = has_filter ? element.column->filter(mask, survivors) : element.column;
                current.type = element.type;
            }
            else
            {
                auto cut_column = current.column->cut(current_row, window_rows);
                if (enable_lazy_columns_replication && isLazyReplicationUseful(cut_column))
                {
                    if (!lazy_indexes)
                        lazy_indexes = has_filter ? buildSurvivorIndexes(window_offsets, mask, survivors, window_rows)
                                                  : convertOffsetsToIndexes(window_offsets);
                    current.column = ColumnReplicated::create(cut_column, lazy_indexes);
                }
                else
                    current.column = cut_column->replicate(replicate_offsets);
            }
