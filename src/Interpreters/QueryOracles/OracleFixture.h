...
///
/// All statements run under `OracleExec` (i.e. `makeOracleContext`), whose pins keep the statements
/// single-threaded, free of recursive fuzzing and outside implicit transactions. `valid()` is false when
/// the environment cannot host fixtures (read-only session, `allow_ddl = 0`, system or Replicated
/// database, ...); the oracle must skip in that case.
...
public:
    ...
    /// Allocate a unique table name. Its DROP is armed only when a CREATE naming it succeeds via
    /// `execute`, so this instance never drops a table it did not create. `suffix` is cosmetic.
...
private:
    ...
    std::vector<std::string> allocated;          /// names handed out by allocName
    std::unordered_set<std::string> armed;       /// allocated names whose CREATE succeeded
    bool valid_ = false;
    bool preserved = false;

    /// If `sql` is a CREATE statement, arm the DROP of every allocated name it mentions.
    void armDropsForCreate(const std::string & sql);
};
