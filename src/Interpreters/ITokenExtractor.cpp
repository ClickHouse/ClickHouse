...
void DefaultTokenExtractor::substringToGinFilter(const char * data, size_t length, GinFilter::QueryString & gin_query_info, bool is_prefix, bool is_suffix) const
{
    gin_query_info.setQueryString({data, length});
    ...
    while (cur < length && nextInString(data, length, &cur, &token_start, &token_len))
        ...
        // last token is ignored, unless substring is suffix
        if ((token_start > 0 || is_prefix) && (token_start + token_len < length || is_suffix))
            gin_query_info.addTerm({data + token_start, token_len});
}
