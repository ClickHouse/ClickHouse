...
std::vector<String> TokenizerFactory::extractUnicodeWordParam(std::span<const Field> params)
{
    assertParamsCount(params.size(), 1, UnicodeWordTokenExtractor::getExternalName());
    if (params.empty())
        return std::vector<String>{"，", "。", "！", "？", "；", "：", "、", "“", "”", "‘", "’"};
        ...
        bool only_validate)
{
    ...
    if (tokenizer == UnicodeWordTokenExtractor::getName() || tokenizer == UnicodeWordTokenExtractor::getExternalName())
    {
        auto stop_words = extractUnicodeWordParam(params);
        return only_validate ? nullptr : std::make_unique<UnicodeWordTokenExtractor>(stop_words);
    }
...
bool UnicodeWordTokenExtractor::nextInString(
...
bool UnicodeWordTokenExtractor::nextInStringLike(const char * data, size_t length, size_t & __restrict pos, String & token) const
{
    token.clear();
    size_t token_start;
...
void UnicodeWordTokenExtractor::substringToBloomFilter(
...
void UnicodeWordTokenExtractor::substringToTokens(
