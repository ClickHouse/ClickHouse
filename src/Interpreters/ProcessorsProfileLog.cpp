...
ColumnsDescription ProcessorProfileLogElement::getColumnsDescription()
{
    return ColumnsDescription
    {
        ...
        {"memory_allocated_bytes", std::make_shared<DataTypeUInt64>(), "The number of bytes allocated while executing processor work. It excludes processor prepare, scheduling and destruction, and any memory allocated on other threads, such as by a nested pipeline the processor runs asynchronously."},
        {"memory_freed_bytes", std::make_shared<DataTypeUInt64>(), "The number of bytes freed while executing processor work. It excludes processor prepare, scheduling and destruction, and any memory freed on other threads, such as by a nested pipeline the processor runs asynchronously."},
    };
}
...
void ProcessorProfileLogElement::appendToBlock(MutableColumns & columns) const
{
    ...
    columns[i++]->insert(memory_allocated_bytes);
    columns[i++]->insert(memory_freed_bytes);
}
