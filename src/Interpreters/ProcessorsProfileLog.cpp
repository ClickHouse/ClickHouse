...
ColumnsDescription ProcessorProfileLogElement::getColumnsDescription()
{
    return ColumnsDescription
    {
        ...
        {"memory_usage_delta", std::make_shared<DataTypeInt64>(), "Signed difference between memory allocated and freed while executing processor work. It excludes processor prepare, scheduling and destruction, and can be negative."},
    };
}
...
void ProcessorProfileLogElement::appendToBlock(MutableColumns & columns) const
{
    ...
    columns[i++]->insert(memory_usage_delta);
}
    ...
    if (settings[Setting::log_processors_profiles])
    {
        if (auto processors_profile_log = context->getProcessorsProfileLog())
        {
            ...
            for (const auto & info : profile_infos)
            {
                ...
                processor_elem.memory_usage_delta = info.memory_usage_delta;
