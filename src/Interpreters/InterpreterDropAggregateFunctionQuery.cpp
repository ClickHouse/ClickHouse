...
BlockIO InterpreterDropAggregateFunctionQuery::execute()
{
    ...
    UserDefinedSQLAggregateFunctionFactory::instance().unregisterFunction(current_context, drop_function_query.function_name, throw_if_not_exists);

    return {};
}

void registerInterpreterDropAggregateFunctionQuery(InterpreterFactory & factory)
{
    auto create_fn = [] (const InterpreterFactory::Arguments & args)
    {
        return std::make_unique<InterpreterDropAggregateFunctionQuery>(args.query, args.context);
    };
    factory.registerInterpreter("InterpreterDropAggregateFunctionQuery", create_fn);
}
