...
FilterDAGInfoPtr fragmentGenerateFilterActions(
...
InterpreterSelectQueryFragments::InterpreterSelectQueryFragments(
    const ASTPtr & query_ptr_,
    const ContextPtr & context_,
    const SelectQueryOptions & options_,
    const Names & required_result_column_names_)
    : InterpreterSelectQueryFragments(query_ptr_, context_, std::nullopt, nullptr, options_, required_result_column_names_)
{}
...
InterpreterSelectQueryFragments::InterpreterSelectQueryFragments(
    ...
    Pipe input_pipe_,
    const SelectQueryOptions & options_)
    : InterpreterSelectQueryFragments(query_ptr_, context_, std::move(input_pipe_), nullptr, options_.copy().noSubquery())
{}
...
InterpreterSelectQueryFragments::InterpreterSelectQueryFragments(
    ...
    const SelectQueryOptions & options_)
    : InterpreterSelectQueryFragments(query_ptr_, context_, std::nullopt, storage_, options_.copy().noSubquery(), {}, metadata_snapshot_)
{}
...
InterpreterSelectQueryFragments::~InterpreterSelectQueryFragments() = default;
...
InterpreterSelectQueryFragments::InterpreterSelectQueryFragments(
    ...
    , log(&Poco::Logger::get("InterpreterSelectQueryFragments"))
    ...
    , prepared_sets(prepared_sets_)
{
    ...
    auto analyze = [&] (bool try_move_to_prewhere)
    {
        ...
        if (storage)
        {
            ...
            {
                filter_info = fragmentGenerateFilterActions(
            ...
            {
                additional_filter_info = fragmentGenerateFilterActions(
            ...
            {
                parallel_replicas_custom_filter_info = fragmentGenerateFilterActions(
...
SortDescription InterpreterSelectQueryFragments::getSortDescription(const ASTSelectQuery & query, const ContextPtr & context_)
{
...
UInt64 InterpreterSelectQueryFragments::getLimitForSorting(const ASTSelectQuery & query, const ContextPtr & context_)
{
...
void InterpreterSelectQueryFragments::executeSinglePlan(QueryPlan & query_plan, std::optional<Pipe> prepared_pipe)
{
    ...
    if (options.only_analyze)
    {
        auto read_nothing = std::make_shared<ReadNothingStep>(source_header);
        query_plan.addStep(std::move(read_nothing));

        if (expressions.filter_info)
        {
            auto row_level_security_step = std::make_shared<FilterStep>(
        ...
        if (expressions.prewhere_info)
        {
            ...
            {
                auto row_level_filter_step = std::make_shared<FilterStep>(
                    query_plan.getCurrentDataStream(),
            ...
            auto prewhere_step = std::make_shared<FilterStep>(
                query_plan.getCurrentDataStream(),
    ...
    if (expressions.before_array_join)
    {
        QueryPlanStepPtr before_array_join_step
            = std::make_shared<ExpressionStep>(query_plan.getCurrentDataStream(), expressions.before_array_join);
    ...
    if (expressions.array_join)
    {
        QueryPlanStepPtr array_join_step
            = std::make_shared<ArrayJoinStep>(query_plan.getCurrentDataStream(), expressions.array_join);
    ...
    {
        QueryPlanStepPtr before_join_step = std::make_shared<ExpressionStep>(
            query_plan.getCurrentDataStream(),
    ...
    {
        QueryPlanStepPtr convert_join_step = std::make_shared<ExpressionStep>(
            query_plan.getCurrentDataStream(),
    ...
    if (expressions.hasJoin())
    {
        ...
        {
            QueryPlanStepPtr filled_join_step = std::make_shared<FilledJoinStep>(
                query_plan.getCurrentDataStream(),
                expressions.join,
        ...
        else
        {
            ...
            {
                auto creating_set_step = std::make_shared<CreateSetAndFilterOnTheFlyStep>(
    ...
    SortDescription group_by_sort_description)
{
    ...
    auto merging_aggregated = std::make_shared<MergingAggregatedStep>(
        data_stream,
        params,
        final,
    ...
    return merging_aggregated;
}

void InterpreterSelectQueryFragments::addEmptySourceToQueryPlan(
        ...
        if (query_info.projection->desc->type == ProjectionDescription::Type::Aggregate)
        {
            if (query_info.projection->before_aggregation)
            {
                auto expression_before_aggregation
                    = std::make_shared<ExpressionStep>(query_plan.getCurrentDataStream(), query_info.projection->before_aggregation);
...
RowPolicyFilterPtr InterpreterSelectQueryFragments::getRowPolicyFilter() const
{
    return row_policy_filter;
}

void InterpreterSelectQueryFragments::extendQueryLogElemImpl(QueryLogElement & elem, const ASTPtr & /*ast*/, ContextPtr /*context_*/) const
{
...
bool InterpreterSelectQueryFragments::shouldMoveToPrewhere()
{
    const Settings & settings = context->getSettingsRef();
...
void InterpreterSelectQueryFragments::addPrewhereAliasActions()
{
    auto & expressions = analysis_result;
    if (expressions.filter_info)
    {
...
void InterpreterSelectQueryFragments::executeFetchColumns(QueryPlan & query_plan)
{
    auto & query = getSelectQuery();
    const Settings & settings = context->getSettingsRef();
    ...
    else if (storage)
    {
        ...
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Logical error in InterpreterSelectQueryFragments: nowhere to read");
...
void InterpreterSelectQueryFragments::executeHaving(QueryPlan & query_plan, const ActionsDAGPtr & expression, bool remove_filter)
{
    auto having_step
        = std::make_shared<FilterStep>(query_plan.getCurrentDataStream(), expression, getSelectQuery().having()->getColumnName(), remove_filter);
...
void InterpreterSelectQueryFragments::executeTotalsAndHaving(
...
void InterpreterSelectQueryFragments::executeRollupOrCube(QueryPlan & query_plan, Modificator modificator)
{
    ...
    if (modificator == Modificator::ROLLUP)
        step = std::make_shared<RollupStep>(query_plan.getCurrentDataStream(), std::move(params), final, settings.group_by_use_nulls);
    else if (modificator == Modificator::CUBE)
        step = std::make_shared<CubeStep>(query_plan.getCurrentDataStream(), std::move(params), final, settings.group_by_use_nulls);

    query_plan.addStep(std::move(step));
}

void InterpreterSelectQueryFragments::executeExpression(QueryPlan & query_plan, const ActionsDAGPtr & expression, const std::string & description)
{
    if (!expression)
        return;

    auto expression_step = std::make_shared<ExpressionStep>(query_plan.getCurrentDataStream(), expression);
...
void InterpreterSelectQueryFragments::executeWindow(QueryPlan & query_plan)
{
    ...
    for (size_t i = 0; i < windows_sorted.size(); ++i)
    {
        ...
        auto window_step = std::make_shared<WindowStep>(query_plan.getCurrentDataStream(), window, window.window_functions);
...
void InterpreterSelectQueryFragments::executeOrder(QueryPlan & query_plan, InputOrderInfoPtr input_sorting_info)
{
    auto & query = getSelectQuery();
...
void InterpreterSelectQueryFragments::executeMergeSorted(QueryPlan & query_plan, const std::string & description)
{
    const auto & query = getSelectQuery();
    SortDescription sort_description = getSortDescription(query, context);
    const UInt64 limit = getLimitForSorting(query, context);
...
void InterpreterSelectQueryFragments::executeProjection(QueryPlan & query_plan, const ActionsDAGPtr & expression)
{
    auto projection_step = std::make_shared<ExpressionStep>(query_plan.getCurrentDataStream(), expression);
    projection_step->setStepDescription("Projection");
    query_plan.addStep(std::move(projection_step));
}


void InterpreterSelectQueryFragments::executeDistinct(QueryPlan & query_plan, bool before_order, Names columns, bool pre_distinct)
{
    auto & query = getSelectQuery();
    if (query.distinct)
    {
...
void InterpreterSelectQueryFragments::executeLimitBy(QueryPlan & query_plan)
{
    ...
    auto limit_by = std::make_shared<LimitByStep>(query_plan.getCurrentDataStream(), length, offset, columns);
    query_plan.addStep(std::move(limit_by));
}

void InterpreterSelectQueryFragments::executeWithFill(QueryPlan & query_plan)
{
    auto & query = getSelectQuery();
    if (query.orderBy())
    {
...
void InterpreterSelectQueryFragments::executeLimit(QueryPlan & query_plan)
{
    auto & query = getSelectQuery();
    /// If there is LIMIT
    if (query.limitLength())
    {
        ...
        auto limit = std::make_shared<LimitStep>(
            query_plan.getCurrentDataStream(),
...
void InterpreterSelectQueryFragments::executeOffset(QueryPlan & query_plan)
{
    auto & query = getSelectQuery();
    ...
    if (!query.limitLength() && query.limitOffset())
    {
        ...
        auto offsets_step = std::make_shared<OffsetStep>(query_plan.getCurrentDataStream(), limit_offset);
        query_plan.addStep(std::move(offsets_step));
    }
}

void InterpreterSelectQueryFragments::executeExtremes(QueryPlan & query_plan)
{
    if (!context->getSettingsRef().extremes)
        return;

    auto extremes_step = std::make_shared<ExtremesStep>(query_plan.getCurrentDataStream());
    query_plan.addStep(std::move(extremes_step));
}

void InterpreterSelectQueryFragments::executeSubqueriesInSetsAndJoins(QueryPlan & query_plan)
{
    addCreatingSetsStep(query_plan, prepared_sets, context);
}


void InterpreterSelectQueryFragments::ignoreWithTotals()
{
    getSelectQuery().group_by_with_totals = false;
}

bool InterpreterSelectQueryFragments::autoFinalOnQuery(ASTSelectQuery & query)
{
...
void InterpreterSelectQueryFragments::initSettings()
{
    auto & query = getSelectQuery();
    if (query.settings())
