...
void AsynchronousMetricLog::addValues(const AsynchronousMetricValues & values)
{
    ...
    const auto event_time = time_in_seconds(now);
    const auto event_date = DateLUT::instance().toDayNum(event_time);

    for (const auto & [key, value] : values)
    {
        AsynchronousMetricLogElement element;
        element.event_time = event_time;
        element.event_date = event_date;
