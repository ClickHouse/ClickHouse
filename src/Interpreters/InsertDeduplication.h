...
private:
    ...
    FilterResult deduplicateImpl(FilterResult && filtered, ContextPtr context) const;
    void truncateTokensForRetry();
    Block goRetry(SharedHeader && header, Chunk && filtered_data, Ptr filtered_info, ContextPtr context) const;
