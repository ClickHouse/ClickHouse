...
public:
    ...
    /// The deadline (ms since the steady_clock epoch) for a task appended at `now` with `timeout_ms`,
    /// aligned up to the grid the worker batches deadlines on. Never earlier than `now + timeout_ms`:
    ...
    static UInt64 taskDeadlineMs(std::chrono::steady_clock::time_point now, Int64 timeout_ms);
