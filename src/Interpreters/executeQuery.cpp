    ...
    const ASTPtr & ast_for_formatted_query)
{
    ...
    if (formatted_query_ast && settings[Setting::log_formatted_queries])
        elem.formatted_query = formatted_query_ast->formatForLogging();
    ...
    QueryResultDetails & result_details)
{
    ...
    catch (...)
    {
        ...
        normalized_query_hash = normalizedQueryHash(query_for_logging, false);
        logExceptionBeforeStart(query_for_logging, normalized_query_hash, context, out_ast, query_span, start_watch.elapsedMilliseconds(), internal, log_as_internal, applied_rewrite_rules, ast_for_formatted_query);
        throw;
    }
