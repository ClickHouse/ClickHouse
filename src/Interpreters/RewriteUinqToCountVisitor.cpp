#include <Interpreters/RewriteUinqToCountVisitor.h>
...
static bool matchFnUniq(String func_name)
{
    ...
    return name == "uniq" || name == ""
}

}

/// 'SELECT uniq(x) FROM (SELECT DISTINCT x ...)' to 'SELECT count() FROM (SELECT DISTINCT x ...)'
void RewriteUinqToCountMatcher::visit(ASTPtr & ast, Data & /*data*/)
{
    ...
    if (!func || (Poco::toLower(func->name) != "uniq" && Poco::toLower(func->name) != "uniqexact"))
        return;
