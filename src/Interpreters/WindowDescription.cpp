...
void WindowFrame::checkValid() const
{
    ...
    else if (type == WindowFrame::FrameType::RANGE)
    {
        if (begin_type == BoundaryType::Offset && begin_offset.getType() == Field::Types::Float64 && isNaN(begin_offset.get<Float64>()))
        {
            throw Exception(ErrorCodes::BAD_ARGUMENTS, "Frame start offset for '{}' frame cannot be NaN for Floats", type);
        }

        if (end_type == BoundaryType::Offset && end_offset.getType() == Field::Types::Float64 && isNaN(end_offset.get<Float64>()))
        {
            throw Exception(ErrorCodes::BAD_ARGUMENTS, "Frame end offset for '{}' frame cannot be NaN for Floats", type);
        }
    }
