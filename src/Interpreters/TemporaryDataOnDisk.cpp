...
void TemporaryDataOnDiskScope::deltaAllocAndCheck(ssize_t compressed_delta, ssize_t uncompressed_delta)
{
    ...
    if ((compressed_delta < 0 && std::cmp_less(stat.compressed_size.load(), -compressed_delta)) ||
        (uncompressed_delta < 0 && std::cmp_less(stat.uncompressed_size.load(), -uncompressed_delta)))
    {
