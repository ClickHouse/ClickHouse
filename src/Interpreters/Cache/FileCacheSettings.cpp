...
void FileCacheSettings::loadFromConfig(const Poco::Util::AbstractConfiguration & config, const std::string & config_prefix)
{
    ...
    setting_path = config_prefix + ".background_download_max_memory_usage";
    if (config.has(setting_path))
        background_download_max_memory_usage = parseWithSizeSuffix<uint64_t>(config.getString(setting_path));

    background_download_threadpool_pool_size = config.getUInt64(config_prefix + ".background_download_threadpool_pool_size", 100);
    background_download_threadpool_queue_size = config.getUInt64(config_prefix + ".background_download_threadpool_queue_size", 1000000);
}
