    ...
    const CachePriorityGuard::ReadLock & lock)
{
    ...
    eviction_info.pre_finalize_func = [=, this](const CachePriorityGuard::WriteLock & lk)
    {
        for (const auto & [key, key_candidates] : *downgrade_candidates)
        {
            for (const auto & candidate : key_candidates.candidates)
            {
                ...
                candidate_it->lru_iterator = probationary_queue.add(candidate_it->getEntry(), lk, nullptr);
    ...
    eviction_info.finalize_func = [=](const CacheStateGuard::Lock & lk)
    {
        for (const auto & [key, key_candidates] : *downgrade_candidates)
        {
            for (const auto & candidate : key_candidates.candidates)
