...
size_t estimateBlockWeightBytes(const Block & block)
{
    /// `allocatedBytes` is a cheap deep-size approximation and avoids Native serialization on the miss path.
...
{
    /// Plan-time (`ReadFromMergeTree`) and execution-time (`AggregatingTransform`); execution may skip when plan already probed miss.
...
void PartialAggregateCache::setMaxEntries(size_t max_entries)
{
