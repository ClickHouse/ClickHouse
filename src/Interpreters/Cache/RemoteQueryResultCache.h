...
private:
    ...
    void cancelWrite(const Key & key) override;
    ...
    /// Redis keys of locks held by this node. Prevents a node from polling on its own lock
    /// if hasNonStaleEntry is called again on the same key after lock acquisition.
    std::unordered_set<std::string> held_locks TSA_GUARDED_BY(mutex);
