...
bool FileSegment::assertCorrectnessUnlocked(const FileSegmentGuard::Lock & lock) const
{
    ...
    auto check_iterator = [&](const Priority::IteratorPtr & it)
    {
        ...
        auto entry_size = entry->getSize(IFileCachePriority::Entry::SizeAlignment::NOT_ALIGNED);
        if (entry_size == 0)
        {
        ...
        if (download_state != State::DOWNLOADING && entry_size != reserved_size)
            throw_logical(
                fmt::format("Mismatch of file segment size: entry non-aligned size != reserved_size ({} != {}, entry: {})",
