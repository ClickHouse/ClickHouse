...
bool isSettingIgnoredInQueryPlanCache(std::string_view setting_name)
{
    ...
    return setting_name == "allow_experimental_query_plan_cache"
        ...
        /// Expression JIT: cacheable plans are always built with compilation disabled
        /// (compiled function nodes cannot be serialized); the JIT is applied when the
        /// plan builds its pipelines, so these settings do not affect the cached plan.
        || setting_name == "compile_expressions"
        || setting_name == "min_count_to_compile_expression"
...
bool QueryPlanCacheDependency::operator==(const QueryPlanCacheDependency & other) const
{
...
void QueryPlanCache::updateConfiguration(size_t max_size_in_bytes, size_t max_entries)
{
    ...
    /// max_count=0 means "unlimited" in LRUCachePolicy, but for the query plan cache
...
QueryPlanCache::MappedPtr QueryPlanCache::get(const QueryPlanCacheKey & key)
{
    ...
    if (result)
    {
        ...
        /// `QueryPlanCacheHits` is incremented by the caller only after the entry passed
        /// validation and materialized successfully: a hit means "a cached plan was used",
        /// not "an entry was found".
    }
    else
    {
        ProfileEvents::increment(ProfileEvents::QueryPlanCacheMisses);
...
Int64 computeSchemaHash(const StorageInMemoryMetadata & metadata)
{
    ...
    /// must be part of the hash so that `ALTER TABLE MODIFY COLUMN` invalidates
    /// cached plans: ALIAS columns are evaluated at SELECT time and directly affect
    /// query results.
    ...
    /// body in `select.inner_query`; materialized views in `select.select_query`.
