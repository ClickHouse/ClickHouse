...
/// Returns true if `s` matches the exact pattern generated by the Analyzer:
/// `__table` followed by one or more ASCII digits (e.g. `__table1`, `__table42`).
/// This avoids stripping user-visible identifiers that happen to start with `__table`.
static bool isPlannerGeneratedTableAlias(const String & s)
{
    if (!s.starts_with("__table"))
        return false;
    auto suffix = std::string_view(s).substr(7);
    return !suffix.empty() && std::all_of(suffix.begin(), suffix.end(), [](char c) { return c >= '0' && c <= '9'; });
}
