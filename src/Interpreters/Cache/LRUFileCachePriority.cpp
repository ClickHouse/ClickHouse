    ...
    const CacheStateGuard::Lock * state_lock)
{
    ...
    if (entry->getSize() && !canFit(entry->getSize(), /* elements */1, *state_lock))
    {
        throw Exception(
            ErrorCodes::LOGICAL_ERROR,
    ...
    if (entry->getSize())
        state->add(entry->getSize(), /* elements */1, *state_lock);
...
LRUFileCachePriority::remove(LRUQueue::iterator it, const CachePriorityGuard::WriteLock & lock)
{
    ...
    auto & entry = **it;
    if (entry.getSize())
        state->sub(entry.getSize(), /* elements */1);
    ...
    for (size_t iterated_elements = 0; iterated_elements < max_elements_to_iterate; ++iterated_elements)
    {
        ...
        auto is_evictable_state = [&]() -> bool
        {
            switch (entry.getState())
            {
                ...
                {
                    stat.update(entry.getSize(), FileSegmentKind::Regular, FileCacheReserveStat::State::Invalidated);
                ...
                case Entry::State::Evicting:
                {
                    ...
                    stat.update(entry.getSize(), FileSegmentKind::Regular, FileCacheReserveStat::State::Evicting);
                    return false;
                }
                case Entry::State::Moving:
                {
                    ProfileEvents::increment(ProfileEvents::FilesystemCacheEvictionSkippedMovingFileSegments);
                    stat.update(entry.getSize(), FileSegmentKind::Regular, FileCacheReserveStat::State::Moving);
                    return false;
                }
    ...
    const CacheStateGuard::Lock & state_lock)
{
    ...
    for (const auto & queue_entry : queue)
    {
        /// entry.getSize() == 0 means entry was invalidated.
        if (queue_entry->getSize() != 0 && queue_entry->key == entry.key && queue_entry->offset == entry.offset)
            throw Exception(
                ErrorCodes::LOGICAL_ERROR,
                "Attempt to add duplicate queue entry to queue: {}",
                entry.toString());
    ...
    state->add(entry.getSize(), /* elements */1, state_lock);
    other.state->sub(entry.getSize(), /* elements */1);
...
void LRUFileCachePriority::LRUIterator::invalidate()
{
    ...
    size_t entry_size = entry_ptr->getSize();
    entry_ptr->setSize(0);
    ...
    const CacheStateGuard::Lock & lock)
{
    ...
    size_t elements = entry_ptr->getSize() > 0 ? 0 : 1;

    /// When `use_real_disk_size` is enabled, the aligned size increase might differ
    /// from the raw `size` due to filesystem block alignment.
    size_t prev_aligned_size = entry_ptr->getSize();
    size_t new_aligned_size = entry_ptr->key_metadata->useRealDiskSize()
        ? entry_ptr->key_metadata->alignFileSize(entry_ptr->getSize(IFileCachePriority::Entry::SizeAlignment::NOT_ALIGNED) + size)
        : prev_aligned_size + size;
    size_t aligned_diff = new_aligned_size - prev_aligned_size;

    if (aligned_diff && !cache_priority->canFit(aligned_diff, elements, lock))
    {
        throw Exception(ErrorCodes::LOGICAL_ERROR,
                        "Cannot increment size by {} for entry {}. Current state: {}",
                        aligned_diff, entry_ptr->toString(), cache_priority->getStateInfoForLog(lock));
    }
    ...
    entry_ptr->increaseSize(size);
    if (aligned_diff)
        cache_priority->state->add(aligned_diff, elements, lock);
...
void LRUFileCachePriority::LRUIterator::decrementSize(size_t size)
{
    ...
    auto entry_ptr = entry.lock();
    chassert(entry_ptr);
    chassert(entry_ptr->getSize(IFileCachePriority::Entry::SizeAlignment::NOT_ALIGNED) >= size);

    LOG_TEST(cache_priority->log,
             "Decrement size with {} in LRU queue entry {}",
             size, entry_ptr->toString());

    /// When `use_real_disk_size` is enabled, the aligned size decrease might differ
    /// from the raw `size` due to filesystem block alignment.
    size_t prev_aligned_size = entry_ptr->getSize();
    entry_ptr->decreaseSize(size);
    size_t aligned_diff = prev_aligned_size - entry_ptr->getSize();
    if (aligned_diff)
        cache_priority->state->sub(aligned_diff, 0);
}
