...
{
    extern const DDLReplicatorSettingsBool check_consistency;
    extern const DDLReplicatorSettingsUInt64 max_replication_lag_to_enqueue;
    extern const DDLReplicatorSettingsUInt64 max_retries_before_automatic_recovery;
    extern const DDLReplicatorSettingsUInt64 wait_entry_commited_timeout_sec;
}
...
DDLReplicateWorker::DDLReplicateWorker(
    DDLReplicator * replicator_,
    ContextPtr context_,
    const String & logger_name)
    : DDLWorker(
          /* pool_size */ 1,
          replicator_->getZooKeeperPath() + "/log",
          replicator_->getZooKeeperPath() + "/replicas",
          context_,
          /* config */ nullptr,
          /* prefix */ {},
          replicator_->getZooKeeperName(),
          logger_name)
    , replicator(replicator_)
{
}

String DDLReplicateWorker::enqueueQuery(DDLLogEntry & entry, const ZooKeeperRetriesInfo &)
{
    auto zookeeper = getZooKeeperFromContext();
    return enqueueQueryImpl(zookeeper, entry, replicator);
}

String DDLReplicateWorker::tryEnqueueAndExecuteEntry(DDLLogEntry & entry, ContextPtr query_context, bool internal_query)
{
    ...
    UInt32 max_log_ptr = parse<UInt32>(zookeeper->get(replicator->zookeeper_path + "/max_log_ptr"));

    if (our_log_ptr + replicator->getSettings()[DDLReplicatorSetting::max_replication_lag_to_enqueue] < max_log_ptr)
    ...
    String entry_path = enqueueQueryImpl(zookeeper, entry, replicator, false, query_context->getDDLAdditionalChecksOnEnqueue());
    ...
    auto task = createTask(entry_name, entry_path);
    task->entry = entry;
...
void DDLReplicateWorker::shutdown()
{
    DDLWorker::shutdown();
...
{
    auto component_guard = Coordination::setCurrentComponent("DDLReplicateWorker::waitForReplicaToProcessAllEntries");
    auto zookeeper = getZooKeeperFromContext();
    const auto our_log_ptr_path = replicator->replica_path + "/log_ptr";
    const auto max_log_ptr_path = replicator->zookeeper_path + "/max_log_ptr";
    ...
    /// This equality is enough to say that current replica is up-to-date.
    ...
    /// Now wait for max_log_ptr to be processed
...
String DDLReplicateWorker::enqueueQueryImpl(
...
{
    const String query_path_prefix = replicator->zookeeper_path + "/log/query-";
    ...
    String counter_prefix = replicator->zookeeper_path + "/counter/cnt-";
    String counter_lock_path = replicator->zookeeper_path + "/counter_lock";
    ...
    while (--iters)
    {
        ...
        ops.emplace_back(zkutil::makeCreateRequest(counter_lock_path, replicator->getFullReplicaName(), zkutil::CreateMode::Ephemeral));
    ...
    if (committed)
        ops.emplace_back(zkutil::makeCreateRequest(node_path + "/committed", replicator->getFullReplicaName(), zkutil::CreateMode::Persistent));
    else
        ops.emplace_back(zkutil::makeCreateRequest(node_path + "/try", replicator->getFullReplicaName(), zkutil::CreateMode::Ephemeral));
...
UInt32 DDLReplicateWorker::getLogPointer() const
{
    /// NOTE it may not be equal to the log_ptr in zk:
    ///  - max_id can be equal to log_ptr - 1 due to race condition
    ///    (when it's updated in zk, but not updated in memory yet)
    ///  - max_id can be greater than log_ptr, because log_ptr is not
    ///    updated for failed and dummy entries
    return max_id.load();
}

UInt64 DDLReplicateWorker::getCurrentInitializationDurationMs() const
{
...
bool DDLReplicateWorker::initializeMainThread()
{
    {
        std::lock_guard lock(initialization_duration_timer_mutex);
    ...
    while (!stop_flag)
    {
        try
        {
            ...
            if (replicator->getSettings()[DDLReplicatorSetting::max_retries_before_automatic_recovery]
                && replicator->getSettings()[DDLReplicatorSetting::max_retries_before_automatic_recovery] <= subsequent_errors_count)
            {
                ...
                LOG_WARNING(log, "Got stuck at processing task {}: it failed {} times in a row with the same error. "
                ...
                String digest_str;
                zookeeper->tryGet(replicator->replica_path + "/digest", digest_str);
                LOG_WARNING(log, "Resetting digest from {} to {}", digest_str, FORCE_AUTO_RECOVERY_DIGEST);
                zookeeper->trySet(replicator->replica_path + "/digest", FORCE_AUTO_RECOVERY_DIGEST);
            }
        ...
        {
            tryLogCurrentException(log, "Error on initialization");
...
void DDLReplicateWorker::initializeReplication()
{
    ...
    String active_path = fs::path(replicator->replica_path) / "active";
    ...
    zookeeper->deleteEphemeralNodeIfContentMatches(
        ...
        {
            if (actual_content.ends_with(DDLReplicator::REPLICA_UNSYNCED_MARKER))
                return active_id == actual_content.substr(0, actual_content.size() - strlen(DDLReplicator::REPLICA_UNSYNCED_MARKER));
    ...
    String log_ptr_str = zookeeper->get(replicator->replica_path + "/log_ptr");
    UInt32 our_log_ptr = parse<UInt32>(log_ptr_str);
    UInt32 max_log_ptr = parse<UInt32>(zookeeper->get(replicator->zookeeper_path + "/max_log_ptr"));
    logs_to_keep = parse<UInt32>(zookeeper->get(replicator->zookeeper_path + "/logs_to_keep"));

    UInt64 digest;
    String digest_str;
    UInt64 local_digest;
    if (zookeeper->tryGet(replicator->replica_path + "/digest", digest_str))
    {
        digest = parse<UInt64>(digest_str);
        std::lock_guard lock{replicator->metadata_mutex};
        local_digest = replicator->metadata_digest;
    }
    else
    {
        ...
        std::lock_guard lock{replicator->metadata_mutex};
        digest = local_digest = replicator->metadata_digest;
        digest_str = toString(digest);
        zookeeper->create(replicator->replica_path + "/digest", digest_str, zkutil::CreateMode::Persistent);
    }
    ...
    bool lost_according_to_digest = replicator->getSettings()[DDLReplicatorSetting::check_consistency] && local_digest != digest;
    ...
    if (is_new_replica || lost_according_to_log_ptr || lost_according_to_digest)
    {
        ...
        replicator->recoverLostReplica(zookeeper, our_log_ptr, max_log_ptr);
        ...
        zookeeper->set(replicator->replica_path + "/log_ptr", toString(max_log_ptr));
    ...
    {
        std::lock_guard lock{replicator->metadata_mutex};
        if (!replicator->checkDigestValid(context))
            throw Exception(ErrorCodes::LOGICAL_ERROR, "Inconsistent metadata of {} after reconnection to ZooKeeper", replicator->getName());
    }
    ...
    if (is_new_replica || first_initialization)
    {
        ...
        UInt32 new_max_log_ptr = parse<UInt32>(zookeeper->get(replicator->zookeeper_path + "/max_log_ptr"));
        unsynced_after_recovery = max_log_ptr + replicator->getSettings()[DDLReplicatorSetting::max_replication_lag_to_enqueue] <= new_max_log_ptr;
        ...
        if (unsynced_after_recovery)
            active_id += DDLReplicator::REPLICA_UNSYNCED_MARKER;
    }
    else
    {
    ...
    Coordination::Stat * entry_stats)
{
    ...
    if (unsynced_after_recovery)
    {
        UInt32 max_log_ptr = parse<UInt32>(getAndSetZooKeeper()->get(fs::path(replicator->zookeeper_path) / "max_log_ptr"));
        LOG_TRACE(log, "Replica was not fully synced after recovery: our_log_ptr={}, max_log_ptr={}", our_log_ptr, max_log_ptr);
        chassert(our_log_ptr < max_log_ptr);
        bool became_synced = our_log_ptr + replicator->getSettings()[DDLReplicatorSetting::max_replication_lag_to_enqueue] >= max_log_ptr;
        if (became_synced)
        {
    ...
    if (!dry_run && zookeeper->tryGet(try_node_path, initiator_name, nullptr, wait_committed_or_failed))
    {
        ...
        size_t max_iterations = replicator->getSettings()[DDLReplicatorSetting::wait_entry_commited_timeout_sec];
        size_t iteration = 0;
    ...
    String node_data = zookeeper->get(entry_path, entry_stats);
...
void DDLReplicateWorker::markReplicasActive(bool reinitialized)
{
    if (reinitialized || !active_node_holder_zookeeper || active_node_holder_zookeeper->expired())
    {
        ...
        zookeeper->deleteEphemeralNodeIfContentMatches(
            ...
            [&active_id](const std::string & actual_content)
            {
                if (actual_content.ends_with(DDLReplicator::REPLICA_UNSYNCED_MARKER))
                    return active_id
                        == actual_content.substr(0, actual_content.size() - strlen(DDLReplicator::REPLICA_UNSYNCED_MARKER));
                return active_id == actual_content;
            });
        if (active_node_holder)
            active_node_holder->setAlreadyRemoved();
...
void DDLReplicateWorker::initializeLogPointer(const String & processed_entry_name)
{
    updateMaxDDLEntryID(processed_entry_name);
}

bool DDLReplicateWorker::canRemoveQueueEntry(const String & entry_name, const Coordination::Stat &)
{
    UInt32 entry_number = DDLTaskBase::getLogEntryNumber(entry_name);
    UInt32 max_log_ptr = parse<UInt32>(getZooKeeper()->get(fs::path(replicator->zookeeper_path) / "max_log_ptr"));
    return entry_number + logs_to_keep < max_log_ptr;
}

DDLReplicateTaskPtr DDLReplicateWorker::createTask(const String & name, const String & path) const
{
    return std::make_unique<DDLReplicateTask>(name, path, replicator);
}
