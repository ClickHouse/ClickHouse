...
ColumnsDescription QueryLogElement::getColumnsDescription()
{
    ...
    return ColumnsDescription
    {
        ...
        {"used_join_kinds", array_low_cardinality_string, "Kinds of the joins in the query pipeline, as they were executed. One element per join counted in used_number_of_joins, so a kind used by several joins appears several times."},
        {"used_join_strictness", array_low_cardinality_string, "Strictness of the joins in the query pipeline. Positionally aligned with used_join_kinds: the element at a given index describes the same join in both arrays."},
