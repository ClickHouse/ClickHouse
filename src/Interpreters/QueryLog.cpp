...
ColumnsDescription QueryLogElement::getColumnsDescription()
{
    ...
    return ColumnsDescription
    {
        ...
        {"written_rows", std::make_shared<DataTypeUInt64>(), "The number of rows written by the query, including any rows written by downstream inserts triggered by the pipeline, such as attached materialized views. This applies to inserts, covering both synchronous inserts (`query_kind` = `Insert`) and asynchronous insert flushes (`query_kind` = `AsyncInsertFlush`); for queries that do not write rows, it is 0."},
        {"written_bytes", std::make_shared<DataTypeUInt64>(), "The number of bytes written by the query (uncompressed), including any bytes written by downstream inserts triggered by the pipeline, such as attached materialized views. This applies to inserts, covering both synchronous inserts (`query_kind` = `Insert`) and asynchronous insert flushes (`query_kind` = `AsyncInsertFlush`); for queries that do not write data, it is 0."},
