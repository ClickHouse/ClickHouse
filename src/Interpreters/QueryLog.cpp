...
ColumnsDescription QueryLogElement::getColumnsDescription()
{
    ...
    return ColumnsDescription
    {
        ...
        {"written_rows", std::make_shared<DataTypeUInt64>(), "For INSERT queries, the number of written rows. This includes both the rows from the original insert and any rows written by downstream inserts triggered by the pipeline, such as attached materialized views. For other queries, the column value is 0."},
        {"written_bytes", std::make_shared<DataTypeUInt64>(), "For INSERT queries, the number of written bytes (uncompressed). This includes both the bytes from the original insert and any bytes written by downstream inserts triggered by the pipeline, such as attached materialized views. For other queries, the column value is 0."},
        {"result_rows", std::make_shared<DataTypeUInt64>(), "Number of rows in a result of the SELECT query, or a number of rows in the INSERT query. For INSERT queries, this includes both the rows from the original insert and any rows written by downstream inserts triggered by the pipeline, such as attached materialized views."},
