...
CancellationCode QueryStatus::cancelQueryBestEffort(CancelReason reason) noexcept
{
    try
    {
        /// Set `is_killed` first. Even if a later best-effort executor cancel
        /// cannot run, query code that checks cancellation will still observe
        /// the kill request and unwind normally.
        {
            std::lock_guard<std::mutex> lock(cancel_mutex);
        ...
        /// Do not allocate a full executor snapshot in the OOM path. The query
        /// is already marked killed, so missing a racing executor here is still
        /// safe: execution checks `is_killed` on its normal cancellation points.
        static constexpr size_t batch_size = 16;
        std::array<ExecutorHolderPtr, batch_size> executors_batch;
        ...
        while (true)
        {
            ...
            {
                std::lock_guard lock(executors_mutex);
                /// We cannot erase or copy the whole executor map here. Marking
                /// a holder before releasing the mutex gives the next iteration
                /// a stable way to skip already requested executors without
                /// keeping an iterator across `cancel`.
                for (auto & [_, executor_holder] : executors)
                {
                    if (executor_holder->best_effort_cancel_requested)
                        continue;

                    executor_holder->best_effort_cancel_requested = true;
                    executors_batch[batch_count++] = executor_holder;
            ...
            {
                if (executors_batch[i])
                    executors_batch[i]->cancel();
                executors_batch[i].reset();
            }
        }
...
void ProcessList::killAllQueriesBestEffort() noexcept
{
    try
    {
        static constexpr size_t batch_size = 64;
        std::array<QueryStatusPtr, batch_size> cancelled_processes;

        /// Repeat until a scan under the process-list lock finds no remaining
        /// cancellable query. Each batch holds shared pointers only on the
        /// stack-sized array, so memory use is bounded even with many queries.
        while (true)
        {
            size_t batch_count = 0;
            {
                ...
                for (auto & process : processes)
                {
                    if (process->is_cancelling || process->isKilled())
                        continue;

                    process->is_cancelling = true;
                    cancelled_processes[batch_count++] = process;
            ...
            /// Cancellation may call into query pipelines and distributed
            /// connections, so it must happen without holding `mutex`.
            for (size_t i = 0; i < batch_count; ++i)
            {
                if (cancelled_processes[i])
                    cancelled_processes[i]->cancelQueryBestEffort(CancelReason::CANCELLED_BY_USER);
            }

            {
                Lock lock(mutex);
                /// `ProcessListEntry` destruction waits for `is_cancelling` to
                /// become false. Clear it after cancellation attempts, even if a
                /// particular query was already killed by another path.
                for (size_t i = 0; i < batch_count; ++i)
                {
                    if (cancelled_processes[i])
                    {
                        cancelled_processes[i]->is_cancelling = false;
                        cancelled_processes[i].reset();
                    }
                }
                cancelled_cv.notify_all();
            }
