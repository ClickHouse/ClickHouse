...
DeduplicationInfo::FilterResult DeduplicationInfo::deduplicateSelf(bool deduplication_enabled, const std::string & partition_id, ContextPtr context) const
{
    ...
    return deduplicateImpl(filterImpl(filterSelf(partition_id)), context);
}


DeduplicationInfo::FilterResult DeduplicationInfo::deduplicateImpl(DeduplicationInfo::FilterResult && filtered, ContextPtr context) const
{
    ...
    auto block = goRetry(std::move(header), std::move(chunk), filtered.deduplication_info, context);
...
std::string DeduplicationInfo::debug() const
{
    ...
    return fmt::format(
        "inst: {}, {}, {}, level {}, rows/tokens {}/{}, in block: {}, tokens: {}:[{}], visited views: {}:[{}], chiosen partitions: <{}>, retried view id: {}",
        instance_id,
...
void DeduplicationInfo::truncateTokensForRetry()
{
    for (auto & token : tokens)
    {
        ...
        assert(it != token.extra_tokens.end());
        auto next = ++it;
        chassert(next != token.extra_tokens.end());
        chassert(next->type == TokenDefinition::Extra::Type::SOURCE_NUMBER || next->type == TokenDefinition::Extra::Type::VIEW_NUMBER);
        ++next;
...
Block DeduplicationInfo::goRetry(SharedHeader && header, Chunk && filtered_data, Ptr filtered_info, ContextPtr context) const
{
    ...
    const std::vector<std::string> & existing_block_ids, const std::string & partition_id, ContextPtr context) const
{
    ...
    chassert(!disabled);
    return deduplicateImpl(filterImpl(filterOriginal(existing_block_ids, partition_id)), context);
}
