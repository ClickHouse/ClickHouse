...
void fillFixedBatch(size_t keys_size, const ColumnRawPtrs & key_columns, const Sizes & key_sizes, PaddedPODArray<Key> & out, size_t & offset)
{
    for (size_t i = 0; i < keys_size; ++i)
    {
        if (key_sizes[i] == sizeof(T))
        {
            ...
            const char * source = static_cast<const NewShinyColumnFixedSizeHelper *>(column)->getRawDataBegin<sizeof(T)>();
    ...
    const Sizes * low_cardinality_sizes [[maybe_unused]] = nullptr)
{
    ...
    for (size_t j = 0; j < keys_size; ++j)
    {
        ...
        switch (key_sizes[j])
        {
            case 1:
                {
                memcpy(bytes + offset, static_cast<const NewShinyColumnFixedSizeHelper *>(column)->getRawDataBegin<1>() + index, 1);
                offset += 1;
                }
                break;
            case 2:
                ...
                {
                    memcpy(bytes + offset, static_cast<const NewShinyColumnFixedSizeHelper *>(column)->getRawDataBegin<2>() + index * 2, 2);
                    offset += 2;
                }
                break;
            case 4:
                ...
                {
                    memcpy(bytes + offset, static_cast<const NewShinyColumnFixedSizeHelper *>(column)->getRawDataBegin<4>() + index * 4, 4);
                    offset += 4;
                }
                break;
            case 8:
                ...
                {
                    memcpy(bytes + offset, static_cast<const NewShinyColumnFixedSizeHelper *>(column)->getRawDataBegin<8>() + index * 8, 8);
                    offset += 8;
                }
                break;
            default:
                memcpy(
                    bytes + offset,
                    static_cast<const NewShinyColumnFixedSizeHelper *>(column)->getRawDataBegin<1>() + index * key_sizes[j],
                    key_sizes[j]);
                offset += key_sizes[j];
        }
    }
    ...
    const KeysNullMap<T> & bitmap)
{
    ...
    for (size_t j = 0; j < keys_size; ++j)
    {
        ...
        {
            case 1:
                memcpy(bytes + offset, static_cast<const NewShinyColumnFixedSizeHelper *>(key_columns[j])->getRawDataBegin<1>() + i, 1);
                offset += 1;
                break;
            case 2:
                memcpy(bytes + offset, static_cast<const NewShinyColumnFixedSizeHelper *>(key_columns[j])->getRawDataBegin<2>() + i * 2, 2);
                offset += 2;
                break;
            case 4:
                memcpy(bytes + offset, static_cast<const NewShinyColumnFixedSizeHelper *>(key_columns[j])->getRawDataBegin<4>() + i * 4, 4);
                offset += 4;
                break;
            case 8:
                memcpy(bytes + offset, static_cast<const NewShinyColumnFixedSizeHelper *>(key_columns[j])->getRawDataBegin<8>() + i * 8, 8);
                offset += 8;
                break;
            default:
                memcpy(
                    bytes + offset,
                    static_cast<const NewShinyColumnFixedSizeHelper *>(key_columns[j])->getRawDataBegin<1>() + i * key_sizes[j],
                    key_sizes[j]);
                offset += key_sizes[j];
        }
    }
