...
void NO_INLINE Aggregator::mergeOnIntervalWithoutKey(
    ...
    size_t row_end,
    const AggregateColumnsConstData & aggregate_columns_data,
    std::atomic_bool & is_cancelled) const
{
    /// `data_variants` will destroy the states of aggregate functions in the destructor
    data_variants.aggregator = this;
    data_variants.init(AggregatedDataVariants::Type::without_key);
...
void NO_INLINE Aggregator::mergeWithoutKeyDataImpl(
    ManyAggregatedDataVariants & non_empty_data, std::atomic_bool & is_cancelled) const
{
...
void NO_INLINE Aggregator::mergeBlockWithoutKeyStreamsImpl(
    Block block,
    AggregatedDataVariants & result,
    std::atomic_bool & is_calcelled) const
{
    AggregateColumnsConstData aggregate_columns = params.makeAggregateColumnsData(block);
    mergeWithoutKeyStreamsImpl(result, 0, block.rows(), aggregate_columns, is_calcelled);
}
    ...
    std::atomic_bool & is_cancelled) const
{
    ...
    for (size_t row = row_begin; row < row_end; ++row)
    {
        ...
        for (size_t i = 0; i < params.aggregates_size; ++i)
        {
            ...
            if (aggregate_functions[i]->isAbleToParallelizeMerge())
                aggregate_functions[i]->merge(
                    res + offsets_of_aggregate_states[i], (*aggregate_columns_data[i])[row], thread_pool, result.aggregates_pool, is_cancelled);
            else
                aggregate_functions[i]->merge(
...
bool Aggregator::mergeOnBlock(Block block, AggregatedDataVariants & result, bool & no_more_keys, std::atomic_bool & is_cancelled) const
{
    /// `result` will destroy the states of aggregate functions in the destructor
    result.aggregator = this;
...
void Aggregator::mergeBlocks(BucketToBlocks bucket_to_blocks, AggregatedDataVariants & result, size_t max_threads, std::atomic_bool & is_cancelled)
{
...
Block Aggregator::mergeBlocks(BlocksList & blocks, bool final, std::atomic_bool & is_cancelled)
{
    if (blocks.empty())
        return {};
