...
void Aggregator::normalizeSingleLowCardinalityDictionary(AggregatedDataVariants & result) const
{
    ...
    AdaptiveAggregationProducer * adaptive) const
{
    ...
    /// A zero-row keyed range cannot affect the aggregate state. Defer method selection and dictionary binding
    /// until rows arrive, so an empty `ARRAY JOIN` window cannot lock the producer into a fallback method.
    if (params.keys_size != 0 && row_begin == row_end)
        return true;
