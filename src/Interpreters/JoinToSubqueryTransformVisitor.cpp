    ...
    static void visit(const ASTExpressionList & node, const ASTPtr &, Data & data)
    {
        ...
        for (const auto & child : node.children)
        {
            ...
            else if (child->as<ASTQualifiedAsterisk>())
            {
                ...
                data.addTableColumns(identifier.fullName());
            }
            else
    ...
    struct Data
    {
        ...
        void addIdentirier(const ASTIdentifier & ident)
        {
            for (const auto & aliases : ignored)
                if (aliases.count(ident.fullName()))
                    return;
...
struct CheckAliasDependencyVisitorData
{
    ...
    {
        if (!dependency && aliases.count(ident.fullName()))
            dependency = &ident;
    }
};
    ...
    UniqueShortNames & unique_names)
{
    ...
    {
        bool got_alias = aliases.count(ident->fullName());
        ...
        if (auto table_pos = IdentifierSemantic::chooseTableColumnMatch(*ident, tables, allow_ambiguous))
        {
            ...
            {
                if (got_alias)
                    throw Exception("Alias clashes with qualified column '" + ident->fullName() + "'", ErrorCodes::AMBIGUOUS_COLUMN_NAME);

                String short_name = ident->shortName();
                String original_long_name;
                if (public_identifiers.count(ident))
                    original_long_name = ident->fullName();
                ...
                if (count > 1 || aliases.count(short_name))
                {
                    ...
                    const auto & unique_long_name = ident->fullName();
            ...
            throw Exception("Unknown column name '" + ident->fullName() + "'", ErrorCodes::UNKNOWN_IDENTIFIER);
    }
...
void JoinToSubqueryTransformMatcher::visit(ASTSelectQuery & select, ASTPtr & ast, Data & data)
{
        ...
        if (table->table_join)
        {
            ...
            if (join.on_expression)
            {
                ...
                if (table_pos < tables_count - 1)
                {
                    ...
                    {
                        auto it = data.aliases.find(ident->fullName());
                        if (!on_aliases.count(ident->fullName()) && it != data.aliases.end())
                        {
                            auto alias_expression = it->second;
                            alias_pushdown[table_pos].push_back(alias_expression);
                            on_aliases[ident->fullName()] = alias_expression;
                        }
                    }
                }
            }
    ...
    for (auto & expr : on_aliases)
    {
        ...
        if (check.dependency)
            throw Exception("Cannot rewrite JOINs. Alias '" + expr.first +
                            "' used in ON section depends on another alias '" + check.dependency->fullName() + "'",
    ...
    for (auto * ident : using_identifiers)
        if (on_aliases.count(ident->fullName()))
            throw Exception(
                "Cannot rewrite JOINs. Alias '" + ident->fullName() + "' appears both in ON and USING", ErrorCodes::NOT_IMPLEMENTED);
