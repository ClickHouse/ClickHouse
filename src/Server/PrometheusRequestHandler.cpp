...
{
public:
    explicit ExposeMetricsImpl(PrometheusRequestHandler & parent) : Impl(parent) { createMetricsWriter(); }
...
private:
    void createMetricsWriter()
    {
#ifdef CLICKHOUSE_KEEPER_STANDALONE_BUILD
        metrics_writer = std::make_unique<KeeperPrometheusMetricsWriter>();
#else
        metrics_writer = std::make_unique<PrometheusMetricsWriter>();
#endif
    }

    std::unique_ptr<PrometheusMetricsWriter> metrics_writer;
};
...
PrometheusRequestHandler::PrometheusRequestHandler(IServer & server_, const PrometheusRequestHandlerConfig & config_, const AsynchronousMetrics & async_metrics_)
    : server(server_), config(config_), async_metrics(async_metrics_), log(getLogger("PrometheusRequestHandler"))
{
    createImpl();
}
