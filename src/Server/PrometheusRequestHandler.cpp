...
#include <Storages/TimeSeries/PrometheusQueryProtocol.h>
...
public:
    ...
    {
        auto table = DatabaseCatalog::instance().getTable(StorageID{config().time_series_table_name}, context);
        PrometheusQueryProtocol protocol{table, context};
        ...
        try
        {
            ...
            else if (uri.starts_with("/api/v1/query"))
            {
                ...
                LOG_DEBUG(log(), "Instant query parameters: query='{}', time='{}'", query, time);
