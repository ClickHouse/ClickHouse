...
static arrow::Result<std::tuple<std::shared_ptr<arrow::Schema>, std::vector<std::shared_ptr<arrow::Table>>>> executeSQLtoTables_impl(
    ...
    std::function<bool()> alive_check = nullptr
)
{
    auto query_context = session->makeQueryContext();
    query_context->setCurrentQueryId(""); /// Empty string means the query id will be autogenerated.
    if (alive_check)
        query_context->setConnectionAliveCheck(std::move(alive_check));
...
{
    return executeSQLtoTables_impl(session, sql, false, schema_modifier, block_modifier, std::move(alive_check));
}
...
{
    auto res = executeSQLtoTables_impl(session, sql, true, schema_modifier, block_modifier, std::move(alive_check));
