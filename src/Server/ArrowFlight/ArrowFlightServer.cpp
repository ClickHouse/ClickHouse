...
namespace
{
    ...
    [[nodiscard]] arrow::Status checkNoCustomFormat(const ASTPtr & ast)
    {
    ...
    [[nodiscard]] arrow::Status checkNoInsertReturning(const ASTPtr & ast)
    {
        if (const auto * insert = dynamic_cast<const ASTInsertQuery *>(ast.get()); insert && insert->returning_select)
            return arrow::Status::NotImplemented("`INSERT ... RETURNING` is not supported over Arrow Flight");
        return arrow::Status::OK();
    }

    [[nodiscard]] arrow::Result<ASTPtr> parseAndValidateSQLForArrowFlight(const std::string & sql, const ContextPtr & query_context)
    {
        ...
        ARROW_RETURN_NOT_OK(checkNoInsertReturning(ast));
...
)
{
    ...
    auto parse_status = parseAndValidateSQLForArrowFlight(sql, query_context);
