    ...
    const std::string & username) const
{
    switch (descriptor.type)
    {
        case arrow::flight::FlightDescriptor::PATH:
        {
            ...
            /// The client submitted a path, not SQL — the statement below is synthesized from it.
            return DecodeResult {sql_res.ValueUnsafe(), {}, {}, {}, /* sql_is_synthesized= */ true};
        }
        case arrow::flight::FlightDescriptor::CMD:
        {
            ...
            if (const auto * result_table = res.getTable())
            {
                ...
                return DecodeResult {{}, {}, {}, result_table->ValueUnsafe(), /* sql_is_synthesized= */ false};
            }
            ...
            if (isPreparedStatementHandle(sql_set->sql))
            {
                ...
                return DecodeResult {std::move(resolved_query_res).ValueUnsafe(), {}, {}, {}, sql_set->sql_is_synthesized};
            }

            return DecodeResult {sql_set->sql, sql_set->schema_modifier, sql_set->block_modifier, {}, sql_set->sql_is_synthesized};
        }
        default:
...
/// Query rewrite rules must apply only to the SQL a client actually submitted. Arrow Flight
/// synthesizes SQL for the non-SQL parts of the protocol — a `PATH` descriptor, the metadata
/// commands (`CommandGetTables`, `CommandGetDbSchemas`, ...), `CommandStatementIngest`, and the
/// `system.settings` query behind `GetSessionOptions` — and the client never submitted that SQL, so
/// a rule matching it must neither rewrite it, nor reject the RPC, nor make it side-effecting.
/// Clearing the setting on the per-query context is enough: that context is private to this RPC.
static void clearQueryRulesIfSynthesized(const ContextMutablePtr & query_context, bool sql_is_synthesized)
{
    if (sql_is_synthesized)
...
)
{
    ...
    clearQueryRulesIfSynthesized(query_context, sql_is_synthesized);
...
{
    return executeSQLtoTables_impl(session, sql, false, sql_is_synthesized, schema_modifier, block_modifier);
}
...
{
    auto res = executeSQLtoTables_impl(session, sql, true, sql_is_synthesized, schema_modifier, block_modifier);
    ...
    std::unique_ptr<arrow::flight::FlightInfo> * info)
{
    auto impl = [&]
    {
        ...
        ARROW_ASSIGN_OR_RAISE(std::tie(sql, schema_modifier, block_modifier, table, sql_is_synthesized), decodeDescriptor(request, false, auth.getUsername()))
        ...
        else
        {
            ...
            std::vector<std::shared_ptr<arrow::Table>> tables;
            ARROW_ASSIGN_OR_RAISE(std::tie(schema, tables) , executeSQLtoTables(session, sql, sql_is_synthesized, schema_modifier, block_modifier))
    ...
    std::unique_ptr<arrow::flight::FlightMetadataWriter> writer)
{
    auto impl = [&]
    {
        ...
        ARROW_ASSIGN_OR_RAISE(std::tie(sql, schema_modifier, block_modifier, table, sql_is_synthesized), decodeDescriptor(request, true, auth.getUsername()))
