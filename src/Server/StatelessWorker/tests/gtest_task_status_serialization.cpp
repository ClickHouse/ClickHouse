...
TEST(TaskStatusNegotiation, ClampAndLegacyDefault)
{
    ...
    /// The only case that exercises the clamp: a newer coordinator must be pinned to what this worker can serialize.
    EXPECT_EQ(negotiateTaskStatusVersion(DBMS_TCP_PROTOCOL_VERSION + 100), DBMS_TCP_PROTOCOL_VERSION);
}
