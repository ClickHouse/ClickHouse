...
void HTTPStreamHandler::pushDelayedResults(Output & used_output)
{
...
HTTPStreamHandler::HTTPStreamHandler(IServer & server_, const std::string & name, const std::string & param_name_)
    : server(server_)
...
bool HTTPStreamHandler::authenticateUser(
    ContextPtr context,
...
void HTTPStreamHandler::processQuery(
    ContextPtr context,
    HTTPServerRequest & request,
    HTMLForm & params,
    HTTPServerResponse & response,
    ...
    std::optional<CurrentThread::QueryScope> & query_scope)
{
    ...
    sendSummary(*used_output.out_maybe_delayed_and_compressed, true);

    executeStreamQuery(*in, *used_output.out_maybe_delayed_and_compressed, /* allow_into_outfile = */ false, context,
...
void HTTPStreamHandler::trySendExceptionToClient(
    const std::string & s, int exception_code, HTTPServerRequest & request, HTTPServerResponse & response, Output & used_output)
{
    try
    {
        ...
        else if (used_output.out_maybe_compressed)
        {
            ...
            writeString("event: error", *used_output.out_maybe_compressed);
            writeChar('\n', *used_output.out_maybe_compressed);
            writeString("data: ", *used_output.out_maybe_compressed);
...
bool HTTPStreamHandler::customizeQueryParam(ContextPtr context, const std::string & key, const std::string & value)
{
    if (key == param_name)
...
std::string HTTPStreamHandler::getQuery(HTTPServerRequest & request, HTMLForm & params, ContextPtr context)
{
...
void HTTPStreamHandler::sendSummary(WriteBuffer & ostr, bool is_starting)
{
    writeString("event: summary", ostr);
    writeChar('\n', ostr);
    if (is_starting)
...
// void HTTPStreamHandler::sendHello()
// {
...
// void HTTPStreamHandler::sendTableColumns(const ColumnsDescription & columns)
// {
...
// void HTTPStreamHandler::sendProgress()
// {
...
// void HTTPStreamHandler::sendLogs()
// {
//     if (!state.logs_queue)
//         return;
