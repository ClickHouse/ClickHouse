...
{
public:
    explicit StreamingExchangeLookup(const String & query_id_, ExchangeConnectionsPtr connections_, UInt16 streaming_exchange_port_)
        : query_id(query_id_)
...
{
    return std::make_shared<StreamingExchangeLookup>(query_id, connections, streaming_exchange_port);
}
