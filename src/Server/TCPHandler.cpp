...
void TCPHandler::processQuery(std::shared_ptr<QueryState> & state)
{
    ...
    if (client_tcp_protocol_version >= DBMS_MIN_REVISION_WITH_CLIENT_INFO)
    {
        ...
        /// This marker is generated by the server for authenticated interserver queries. A normal
        /// TCP client can still send the version-gated trailing field, but it must never be able to
        /// opt into the hidden TimeSeries target privilege bypass.
        if (!is_interserver_mode)
            client_info.is_time_series_target_read = false;
    }
