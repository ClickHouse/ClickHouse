...
UUID UsersConfigAccessStorage::generateID(AccessEntityType type, const String & name)
{
    Poco::MD5Engine md5;
    md5.update(name);
...
UUID UsersConfigAccessStorage::generateID(const IAccessEntity & entity) { return generateID(entity.getType(), entity.getName()); }

std::vector<AccessEntityPtr> UsersConfigAccessStorage::parseUsers(const Poco::Util::AbstractConfiguration & config, const std::unordered_set<UUID> & allowed_profile_ids, bool allow_no_password, bool allow_plaintext_password)
{
...
UserPtr UsersConfigAccessStorage::parseUser(const Poco::Util::AbstractConfiguration & config, const String & user_name, const std::unordered_set<UUID> & allowed_profile_ids, bool allow_no_password, bool allow_plaintext_password)
{
...
QuotaPtr UsersConfigAccessStorage::parseQuota(const Poco::Util::AbstractConfiguration & config, const String & quota_name, const std::vector<UUID> & user_ids)
{
...
std::vector<AccessEntityPtr> UsersConfigAccessStorage::parseQuotas(const Poco::Util::AbstractConfiguration & config)
{
    Poco::Util::AbstractConfiguration::Keys user_names;
    config.keys("users", user_names);
...
std::vector<AccessEntityPtr> UsersConfigAccessStorage::parseRowPolicies(const Poco::Util::AbstractConfiguration & config, bool users_without_row_policies_can_read_rows)
{
...
SettingsProfileElements UsersConfigAccessStorage::parseSettingsConstraints(const Poco::Util::AbstractConfiguration & config,
...
std::shared_ptr<SettingsProfile> UsersConfigAccessStorage::parseSettingsProfile(
...
std::vector<AccessEntityPtr> UsersConfigAccessStorage::parseSettingsProfiles(
    const Poco::Util::AbstractConfiguration & config,
    const std::unordered_set<UUID> & allowed_parent_profile_ids,
    const AccessControl & access_control)
{
...
std::unordered_set<UUID> UsersConfigAccessStorage::getAllowedSettingsProfileIDs(const Poco::Util::AbstractConfiguration & config)
{
    Poco::Util::AbstractConfiguration::Keys profile_names;
    config.keys("profiles", profile_names);
