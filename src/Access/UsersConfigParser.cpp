...
namespace
{
    ...
    // Gets all string values under a key, handling both keys with a single value
    // and multiple values under nested sub keys.
    // e.g. <server>server1</server> returns [server1] and
    // <server>
    //     <server1>myserver1</server1>
    //     <server2>myserver2</server2>
    // </server> returns [myserver1, myserver2] when supplied with key "server"
    ...
    UserPtr parseUser(
        ...
        size_t max_authentication_methods_per_user,
        LoggerPtr log)
    {
        ...
        size_t num_authn_types = has_no_password + has_password_plaintext + has_password_sha256_hex + has_password_double_sha1_hex
            + has_ldap + has_kerberos + has_certificates + has_ssh_keys + has_http_auth + has_scram_password_sha256_hex;

        if (num_authn_types < 1)
        ...
        if (has_no_password && num_authn_types > 1)
            throw Exception(ErrorCodes::BAD_ARGUMENTS, "'no_password' cannot be specified with other password fields for user {}.", user_name);
        ...
        {
            const auto scram_password_sha256_hex_passwords = getConfigValues(config, scram_password_config);
            for (const String & pw : scram_password_sha256_hex_passwords)
            {
        ...
        if (has_http_auth)
        {
            ...
            {
                Poco::Util::AbstractConfiguration::Keys method_keys;
                config.keys(http_auth_config, method_keys);

                if (method_keys.empty())
                    throw Exception(ErrorCodes::BAD_ARGUMENTS,
                        "HTTP authentication configuration is empty for user {}.", user_name);

                for (const String & method_key : method_keys)
                {
                    const String method_config = http_auth_config + "." + method_key;

                    if (!config.has(method_config + ".server"))
                        throw Exception(ErrorCodes::BAD_ARGUMENTS,
                            "Missing mandatory 'server' in 'http_authentication.{}' for user {}.",
                            method_key, user_name);

                    if (!config.has(method_config + ".scheme"))
                        throw Exception(ErrorCodes::BAD_ARGUMENTS,
                            "Missing mandatory 'scheme' in 'http_authentication.{}' for user {}.",
                            method_key, user_name);

                    auto server = config.getString(method_config + ".server");
                    auto scheme = config.getString(method_config + ".scheme");

                    if (server.empty())
                        throw Exception(ErrorCodes::BAD_ARGUMENTS,
                            "HTTP authentication server name cannot be empty in 'http_authentication.{}' for user {}.",
                            method_key, user_name);
        ...
        // max_authentication_methods_per_user == 0 means unlimited
        if (max_authentication_methods_per_user != 0 && user->authentication_methods.size() > max_authentication_methods_per_user)
            throw Exception(ErrorCodes::BAD_ARGUMENTS,
                            "User has more than the max allowed authentication methods, check the `max_authentication_methods_per_user` setting");
    ...
    const std::unordered_set<UUID> & role_ids_from_users_config) const
{
    ...
    size_t max_authentication_methods_per_user = access_control.getMaxAuthenticationMethodsAllowed();
    ...
    for (const auto & user_name : user_names)
    {
        try
        {
            users.push_back(parseUser(config, user_name, allowed_profile_ids, role_ids_from_users_config, access_control, no_password_allowed, plaintext_password_allowed, max_authentication_methods_per_user, log));
        }
        catch (Exception & e)
        {
