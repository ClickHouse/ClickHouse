...
#define APPLY_FOR_ACCESS_TYPES(M) \
    ...
    M(CREATE_TEMPORARY_VIEW, "", GLOBAL, CREATE_ARBITRARY_TEMPORARY_VIEW) /* allows to create and manipulate temporary tables;
                                                     implicitly enabled by the grant CREATE_VIEW on any table */ \
    M(CREATE_ARBITRARY_TEMPORARY_VIEW, "", GLOBAL, CREATE)  /* allows to create  and manipulate temporary views
                                                                with arbitrary table engine */\
