...
#           undef MAKE_ACCESS_FLAGS_NODE
        ...
        void collectAllFlags(const Node * start_node = nullptr)
        {
            ...
            all_flags_grantable_on_global_with_parameter_level = all_flags_for_target[NAMED_COLLECTION] | all_flags_for_target[DEFINER];
...
std::unordered_map<AccessFlags::ParameterType, AccessFlags> AccessFlags::splitIntoParameterTypes() const
{
    ...
    auto other_flags = (~named_collection_flags & ~definer_flags) & *this;
    if (other_flags)
