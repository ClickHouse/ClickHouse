...
namespace
{
    std::shared_ptr<Poco::Util::XMLConfiguration> createConfigFromXML(const std::string & xml_content)
    {
        std::istringstream xml_stream(xml_content);
        auto config = std::make_shared<Poco::Util::XMLConfiguration>(xml_stream);
        return config;
    }
}
...
TEST_F(UsersConfigMultipleAuthTest, EmptyLDAPServerThrowsException)
{
    ...
    auto config = createConfigFromXML(xml_config);
    EXPECT_THROW(storage->load(*config, ""), Exception);
}
