...
String makeAccessJSON(const AccessRightsElement & el, const ContextData::QueryAccessInfo & access_info, UserPtr user)
{
    ...
    Poco::JSON::Object input;
    {
        ...
        std::vector<InterpreterOperation> ops;
        {
            std::lock_guard<std::mutex> lock(access_info.mutex);
            ops = access_info.operations;
        }
        if (!ops.empty())
        {
            Poco::JSON::Array operations;
            for (auto op : ops)
                operations.add(toString(op));
            input.set("operations", operations);
        }
...
void OpenPolicyAgentAccess::checkAccess(ContextPtr context, const AccessRightsElement & el) const
{
    ...
    const String data = makeAccessJSON(el, context->getQueryAccessInfo(), context->getUser());
