...
struct RolesOrUsersSet
{
    ...
    RolesOrUsersSet(const ASTRolesOrUsersSet & ast, const AccessControl & manager);
    RolesOrUsersSet(const ASTRolesOrUsersSet & ast, const AccessControl & manager, const std::optional<UUID> & current_user_id);

    std::shared_ptr<ASTRolesOrUsersSet> toAST() const;
    std::shared_ptr<ASTRolesOrUsersSet> toASTWithNames(const AccessControl & manager) const;

    String toString() const;
    String toStringWithNames(const AccessControl & manager) const;
    Strings toStringsWithNames(const AccessControl & manager) const;
    ...
    std::vector<UUID> getMatchingIDs(const AccessControl & manager) const;
...
private:
    void init(const ASTRolesOrUsersSet & ast, const AccessControl * manager = nullptr, const std::optional<UUID> & current_user_id = {});
};
