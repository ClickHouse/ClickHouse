...
public:
    ...
    /// Perform boosting iterations over the entire batch.
    virtual void fit(const FeatureMatrix& batch, const Targets& targets) = 0;

    // Perform boosting iterations with a single sample.
    virtual void fit(const Features& features, const Target& target) = 0;
