...
- If you specify `FINAL` or `FORCE`, optimization is performed even when all the data is already in one part. You can control this behaviour with [optimize_skip_merged_partitions](/reference/settings/session-settings/optimize-skip#optimize_skip_merged_partitions). Also, the merge is forced even if concurrent merges are performed.
...
By default, the resulting merged part is validated in a way similar to [`CHECK TABLE`](/reference/statements/check-table) query. This behavior is controlled by the [optimize_dry_run_check_part](/reference/settings/session-settings/optimize#optimize_dry_run_check_part) setting (enabled by default). Disabling it skips validation, which can be useful for benchmarking the merge itself.
