...
- The `SELECT` body accepts the same forms as a real projection: a reordering projection with `ORDER BY`, or an aggregating one with `GROUP BY`.
...
`CREATE HYPOTHETICAL PROJECTION` requires `SELECT` on the columns the projection reads — column-level `SELECT` is sufficient — because estimating it will read those columns.
