...
`CREATE HYPOTHETICAL PROJECTION` reads no table data — it only validates the definition and stores it in the session — so it does not require `SELECT` on the projection's columns. When `EXPLAIN WHATIF` starts estimating projections it will read those columns, and column-level `SELECT` will be required then, as it already is for [`CREATE HYPOTHETICAL INDEX`](/reference/statements/hypothetical-index#required-privileges).
