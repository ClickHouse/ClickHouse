...
- `single_record` – Return the reformatted query as a single multi-line record instead of one record per line. Default: `1` (controlled by the `explain_syntax_single_record` setting). Set to `0` to restore the historical one-record-per-line output, or set `explain_syntax_single_record = 0` (globally or in per-query `SETTINGS`), or set `compatibility` to any version older than `26.8`.
...
query — and unlike the non-executing `EXPLAIN` forms — in several ways:
- **Quotas and limits.** It is charged against the same [quotas](/concepts/features/configuration/server-config/quotas)
  and subject to the same [limits](/concepts/features/configuration/settings/query-complexity)
  (e.g. `query_selects`, `read_rows`) as running the query directly. Sources exempt from quotas during planning
  (such as `system.one`) are not charged.
- **Failed transactions.** Inside a [transaction](/concepts/features/operations/insert/transactions)
  that has already failed (`ROLLED_BACK`), it is rejected with `INVALID_TRANSACTION`,
  just like a plain `SELECT` — issue `ROLLBACK` first.
- **Streaming reads.** Over a streaming (`FROM ... STREAM`) read it is rejected with
  `NOT_IMPLEMENTED`, because such a read never completes.
- **Distributed queries.** It is not supported for queries executed in
  [distributed](/reference/engines/table-engines/special/distributed) mode.
</Note>
...
- the **left** side of `ALL LEFT` and `ALL FULL`, but only when the query selects nothing from the
  right table and the `ON` section is a plain key equality. Otherwise the probe already records
  which left rows matched — either to materialize the right columns or to evaluate the residual
  condition — and the count is exact without the option.
...
Enabling [`any_join_distinct_right_table_keys`](/reference/settings/session-settings/other#any_join_distinct_right_table_keys) switches `ANY` to the older `RightAny` semantics, which emits one row per left row and therefore keeps both counts. `ANY RIGHT` and `ANY FULL` then report **both** sides, and `ANY INNER` is rewritten to `SEMI LEFT`.

The [`Join`](/reference/engines/table-engines/special/join) table engine follows the same table, using the kind and strictness declared in the engine: `Join(ALL, INNER, …)` reports both sides, `Join(ANY, LEFT, …)` neither.
...
- `max_rows_to_scan` — how many rows the estimate of a hypothetical projection may read before it samples granules instead. Default: `10000000`; `max_rows_to_read`, when set, can only lower it. A sample always covers at least about 30 granules, below which its error cannot be measured, so a lower limit still ends the estimate. `0` reads the parts whole, which is exact but costs as much as reading the table's key columns.
...
- `source` — how the estimate was produced.
  - `empirical`: built the index in memory over the baseline-pruned granules and counted the granules the index would skip. This is an upper bound — see the limitations in [`CREATE HYPOTHETICAL INDEX`](/reference/statements/hypothetical-index#limitations).
...
- `sampled_parts` / `sampled_marks` — `<baseline-pruned> / <total in the table>`. Shows what fraction of the table survived PK, partition, and existing-index pruning, i.e. the input to the hypothetical index. For a hypothetical projection they count what the estimate read out of what reading the parts whole would take, so `sampled_marks` below the total means the estimate was modelled from a sample.
...
Column [statistics](/reference/engines/table-engines/mergetree-family/mergetree#column-statistics) are off by default. To exercise the `statistical` path, define them on the relevant columns first and wait for the materialize mutation to finish:
