...
Hypothetical projections defined with [`CREATE HYPOTHETICAL PROJECTION`](/reference/statements/hypothetical-projection#create-hypothetical-projection) are candidates too. A normal projection is estimated by building its primary index in memory over the parts the query would read and pruning it as a materialized projection would be pruned. The report gives the marks and rows the projection read would touch, a `read_ratio` against the base-table read (below `1x` means less work, above means more) and a `verdict` with the reason behind it, following the optimizer's rule: the projection wins when it reads fewer marks than the base table, or the same number while serving an outer `ORDER BY`. Listed as `status: not_applicable` and not estimated yet: aggregate projections, projections with a `WHERE` clause or their own skip indexes (`WITH SETTINGS add_minmax_index_*`), projections ordered by the commit order (`TYPE commit_order` and its `_block_number, _block_offset` query form), and projections that do not provide every column the query reads. `force_optimize_projection`, `force_optimize_projection_name` and `preferred_optimize_projection_name` are ignored. The mark count is modelled by cutting granules over the whole part the way the writer would, while the writer restarts at every block it stores, so on a part whose granularity is adaptive it can differ from the materialized projection by a granule; a comparison that lands inside that margin is reported as `verdict: too close to call` rather than as a decision. A projection whose definition no longer fits the table is reported with that reason.
