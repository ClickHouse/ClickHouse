...
Hypothetical projections defined with [`CREATE HYPOTHETICAL PROJECTION`](/reference/statements/hypothetical-projection#create-hypothetical-projection) are candidates too. For a normal (sorted) projection the estimate builds the projection's primary index in memory from the parts the query would read, prunes it with the query predicate exactly as a materialized projection would be pruned, and reports the marks, rows and bytes the projection read would touch, a signed `skip_ratio` (negative when the projection would read *more* than the base table) and a `verdict`: whether the optimizer would actually switch to it, which it does only when the projection reads strictly fewer marks. Aggregate projections and projections with a `WHERE` clause are listed with `status: not_applicable` and are not estimated yet. A projection whose definition no longer applies to the table — a dropped column, or a setting change that disables the features it needs — is reported with that reason instead.
...
`marks`, `rows` and `est_bytes` are what the projection read itself would touch, not a share of the base-table read, because a projection granule holds different rows than a base granule. `skip_ratio` is signed: a projection that would read *more* marks than the base table shows a negative value. `verdict` applies the optimizer's own rule — it switches to a projection only when that reads strictly fewer marks — so a candidate can be `applicable` and still not be chosen.
