...
Listed as `status: not_applicable` and not estimated yet:

- aggregate projections;
- projections with a `WHERE` clause or their own skip indexes (`WITH SETTINGS add_minmax_index_*`);
- projections ordered by the commit order (`TYPE commit_order` and its `_block_number, _block_offset` query form);
- projections whose sort key is not among the columns they store (for example `ORDER BY _part_offset`);
