...
public:
    ...
    {
        /// Either `scalar` or `interval` is set.
    ...
    static bool parseScalarOrInterval(std::string_view input, ScalarOrInterval & res, String & error_message, size_t & error_pos);

    /// Finds a time range in a range selector, for example for "[1h30m]" the function returns "1h30m".
    static bool extractTimeRange(std::string_view input, std::string_view & res, String & error_message, size_t & error_pos);

    /// Finds a range and optionally a resolution in a subquery, for example for "[1h:5m]" the function returns "1h" and "5m".
    static bool extractSubqueryRangeAndResolution(std::string_view input, std::string_view & res_range, std::string_view & res_resolution,
