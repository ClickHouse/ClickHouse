...
bool ParserDropAggregateFunctionQuery::parseImpl(IParser::Pos & pos, ASTPtr & node, Expected & expected)
{
    ...
    if (!s_aggregate.ignore(pos, expected))
        return false;
    ...
    auto drop_function_query = std::make_shared<ASTDropAggregateFunctionQuery>();
