...
[[nodiscard]] static bool parseDropReplica(std::shared_ptr<ASTSystemQuery> & res, IParser::Pos & pos, Expected & expected, bool database)
{
    ...
    if (ParserKeyword{Keyword::FROM}.ignore(pos, expected))
    {
        ...
        else if (ParserKeyword{Keyword::ZKPATH}.ignore(pos, expected))
        {
            ...
            {
                ASTPtr restore_and_drop_database_ast;
                if (!ParserStringLiteral{}.parse(pos, restore_and_drop_database_ast, expected))
                    return false;
                res->restore_and_drop_database = restore_and_drop_database_ast->as<ASTLiteral &>().value.safeGet<String>();
            }
        }
        else
            return false;
