...
A trailing `SETTINGS` clause after the `RETURNING` subquery is applied to the source `SELECT`, not to the `RETURNING` subquery. To set options that shape returned rows, place the `SETTINGS` inside the subquery itself, for example `RETURNING (SELECT ... SETTINGS ...)`. Delayed `RETURNING` rejects `database`, query-construction settings (`select`, `filter`, `order`, `sort`, `limit`, `offset`, `page`), query-cache settings, and query-global resource and admission limits.

For `INSERT ... RETURNING`, source-side `SETTINGS` that bind query-global state at query registration are rejected with `NOT_IMPLEMENTED` (the same rejection class as for unsupported `RETURNING` subquery settings). This includes settings such as `max_execution_time`, memory and admission limits, throttling settings, query-cache settings (`use_query_cache`, `enable_reads_from_query_cache`, `enable_writes_to_query_cache`, `query_cache_nondeterministic_function_handling`, `query_cache_system_table_handling`), and `profile`. Query-construction settings (`select`, `filter`, `order`, `sort`, `limit`, `offset`, `page`) are also rejected in source-side `SETTINGS`. `use_concurrency_control` is an exception: it is accepted on source-side `SETTINGS`.

The parenthesized subquery is required. The client receives a single result set: the result of the `RETURNING` subquery.

The `INSERT` runs first using the normal insert pipeline. If the `INSERT` fails, the `RETURNING` subquery is not executed. If the `INSERT` succeeds, the subquery runs in the same session and can reference any table. The subquery may carry its own `SETTINGS`, except for rejected classes listed under Limitations.

Planning and executing the `RETURNING` subquery happens only after the `INSERT` pipeline has finished successfully for supported transports (including native TCP, gRPC, and inlined `VALUES`/`FORMAT`). `Arrow Flight` currently rejects `INSERT ... RETURNING` with `NotImplemented`. If planning the subquery fails (for example, unknown column name), inserted rows remain and the insert is not rolled back. The exception is when the statement runs inside a transaction (an explicit transaction, or `implicit_transaction=1`). There, a failure of the `RETURNING` subquery aborts the transaction and inserted rows are rolled back with it.
...
- `parallel_distributed_insert_select` in query and source `SETTINGS` is not supported with `INSERT ... RETURNING` and is rejected with `NOT_IMPLEMENTED`, because `RETURNING` runs once on the initiator after the insert phase and cannot be executed on each shard's remote `INSERT`. If this setting is enabled only via session or profile defaults, `INSERT ... RETURNING` falls back to the initiator-local source pipeline.
...
- Output format settings (for example, `output_format_json_quote_64bit_integers`) supplied in `RETURNING` subquery `SETTINGS` apply to query planning only. The result is serialized using output format from the outer query context (determined by client transport and connection-level format setting), so output format settings in subquery `SETTINGS` are silently ignored.
...
- If the `INSERT` succeeds but the `RETURNING` subquery fails, inserted data is not rolled back, unless the statement runs inside a transaction (explicit, or `implicit_transaction=1`), in which case inserted rows are rolled back with the aborted transaction.
