...
bool ParserInsertQuery::parseImpl(Pos & pos, ASTPtr & node, Expected & expected)
{
    ...
    if (infile && !compression && s_compression.checkWithoutMoving(pos, expected))
    ...
    if (!infile && s_compression.ignore(pos, expected))
    {
        ...
        if (s_settings.checkWithoutMoving(pos, expected))
            throw Exception(ErrorCodes::SYNTAX_ERROR,
                            "SETTINGS must be written before COMPRESSION, not after: "
                            "INSERT INTO t SETTINGS ... COMPRESSION 'type' FORMAT ...");
    }
...
<Note>
Unlike `FROM INFILE`, where `COMPRESSION` precedes `SETTINGS`, here `SETTINGS` must be written before `COMPRESSION`: `INSERT INTO table SETTINGS ... COMPRESSION type FORMAT format_name`.
</Note>

<Note>
`COMPRESSION` before a bare `FORMAT` also applies to `INSERT ... SELECT ... FROM input(structure) FORMAT format_name`, since [`input()`](/reference/functions/table-functions/input) reads the same client-side-decompressed stdin data as a bare `FORMAT`. Write `COMPRESSION` before the `SELECT`, the same position it takes before a bare `FORMAT`: `INSERT INTO table COMPRESSION type SELECT * FROM input(structure) FORMAT format_name`.
</Note>
