...
void flattenDocument(const std::string & prefix, const rapidjson::Value & document, std::vector<InsertedField> & fields)
{
    for (auto it = document.MemberBegin(); it != document.MemberEnd(); ++it)
    {
        ...
        /// A dialect insert targets a table whose schema is not available to this parser. Preserve
        /// `_id`: a user table may have a real column with this name, and unconditionally dropping
        /// its document identity is irreversible data loss. A document collection is unsupported
        /// by the dialect and fails when its non-`_id` fields do not name table columns.
        if (name.starts_with("$"))
