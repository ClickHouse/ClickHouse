...
/** Regression coverage for the `detectFramingSetting` logic in `programs/server/play.html`.
  ...
  * `src/Parsers/Lexer.cpp` - the very same source exercised here) and looks for a real assignment:
  * a `BareWord` `framing_output_format` immediately followed by `=`. A plain text match is fooled by
  * a `framing_output_format` mention inside a string literal or a comment, e.g.
  * `SELECT 'framing_output_format = None'` (wrongly refused) or `SELECT 'framing_output_format'`
  * (framing wrongly dropped). Walking the tokens ignores such occurrences.
  *
...
FramingSetting detectFramingSetting(const std::string & query)
{
    ...
    for (size_t i = 0; i + 1 < tokens.size(); ++i)
    {
        if (tokens[i].type == DB::TokenType::BareWord
            && toLower(tokens[i].text) == "framing_output_format"
            && tokens[i + 1].type == DB::TokenType::Equals)
        {
