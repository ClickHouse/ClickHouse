...
There is no support for deleting columns in the primary key or the sampling key (columns that are used in the `ENGINE` expression). Changing the type for columns that are included in the primary key is usually possible only if this change does not cause the data to be modified (for example, you are allowed to add values to an Enum or to change a type from `DateTime` to `UInt32`). In addition, non-replicated tables in the `MergeTree` family allow a direct sorting-key integer column to widen to a larger integer type with the same signedness (for example, `UInt16` to `UInt32`). This widening is not supported for partition or sampling keys, tables with a `UNIQUE KEY`, active patch parts, or columns used by projections, TTL expressions or targets, or dependent `MATERIALIZED` columns.
