...
{
    Expected expected;
    Tokens token_subquery(query.data(), query.data() + query.size(), 0, true, /* greedy_errors= */ true);
...
{
    Tokens tokens(text.data(), text.data() + text.size(), 0, true, /* greedy_errors= */ true);
    IParser::Pos pos(tokens, max_depth, max_backtracks);
...
bool ParserKQLQuery::parseImpl(Pos & pos, ASTPtr & node, Expected & expected)
{
    ...
    else
    {
        ...
        if (!operation_pos.empty())
        {
            ...
            Tokens token_subquery(sub_query.data(), sub_query.data() + sub_query.size(), 0, true, /* greedy_errors= */ true);
        ...
        auto set_query_clasue = [&](String op_str, String op_calsue)
        {
            ...
            if (oprator)
            {
                Tokens token_clause(op_calsue.data(), op_calsue.data() + op_calsue.size(), 0, true, /* greedy_errors= */ true);
