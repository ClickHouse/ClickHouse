...
bool ParserKQLMakeSeries ::makeSeries(KQLMakeSeries & kql_make_series, ASTPtr & select_node, uint32_t max_depth, uint32_t max_backtracks)
{
    ...
    {
        Tokens tokens(src.data(), src.data() + src.size(), 0, true, /* greedy_errors= */ true);
    ...
    auto get_group_expression_alias = [&]
    {
        ...
        Tokens tokens(group_expression.data(), group_expression.data() + group_expression.size(), 0, true, /* greedy_errors= */ true);
        IParser::Pos pos(tokens, max_depth, max_backtracks);
        while (isValidKQLPos(pos))
        {
...
bool ParserKQLMakeSeries ::parseImpl(Pos & pos, ASTPtr & node, Expected & expected)
{
    ...
    Tokens token_main_query(kql_make_series.main_query.data(), kql_make_series.main_query.data() + kql_make_series.main_query.size(), 0, true, /* greedy_errors= */ true);
