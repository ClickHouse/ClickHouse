...
`KILL QUERY WHERE query_id = '<id>'`. An id that is not running as that user cancels nothing and returns no
rows, rather than reporting an error. Any other `WHERE` condition, and `ON CLUSTER`, keep requiring the grants
described above.
