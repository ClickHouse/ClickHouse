...
void ASTWindowDefinition::formatImpl(FormattingBuffer out) const
{
    ...
    {
        out.ostr << backQuoteIfNeed(parent_window_name);
    ...
    if (!frame_is_default)
    {
        ...
        {
            out.ostr << "UNBOUNDED FOLLOWING";
        }
        else
        {
            frame_end_offset->formatImpl(out);
            out.ostr << " "
