...
TEST(ParserColumn, AllNonGeneratedColumnOption)
{
    ...
    EXPECT_EQ(declare_options->changes["collate"]->as<ASTIdentifier>()->fullName(), "utf-8");
    EXPECT_EQ(declare_options->changes["column_format"]->as<ASTIdentifier>()->fullName(), "FIXED");
    EXPECT_EQ(declare_options->changes["storage"]->as<ASTIdentifier>()->fullName(), "MEMORY");
