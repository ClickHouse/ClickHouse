...
TEST(ParserTableOptions, AllSubpatitionOptions)
{
    ...
    EXPECT_EQ(declare_options->changes["character_set"]->as<ASTIdentifier>()->fullName(), "utf-8");
    EXPECT_EQ(declare_options->changes["checksum"]->as<ASTLiteral>()->value.safeGet<UInt64>(), 1);
    EXPECT_EQ(declare_options->changes["collate"]->as<ASTIdentifier>()->fullName(), "utf8_bin");
    ...
    EXPECT_EQ(declare_options->changes["engine"]->as<ASTIdentifier>()->fullName(), "INNODB");
    EXPECT_EQ(declare_options->changes["insert_method"]->as<ASTIdentifier>()->fullName(), "NO");
    ...
    EXPECT_EQ(declare_options->changes["pack_keys"]->as<ASTIdentifier>()->fullName(), "DEFAULT");
    EXPECT_EQ(declare_options->changes["password"]->as<ASTLiteral>()->value.safeGet<String>(), "password");
    EXPECT_EQ(declare_options->changes["row_format"]->as<ASTIdentifier>()->fullName(), "DYNAMIC");
    EXPECT_EQ(declare_options->changes["stats_auto_recalc"]->as<ASTIdentifier>()->fullName(), "DEFAULT");
    EXPECT_EQ(declare_options->changes["stats_persistent"]->as<ASTIdentifier>()->fullName(), "DEFAULT");
    EXPECT_EQ(declare_options->changes["stats_sample_pages"]->as<ASTLiteral>()->value.safeGet<UInt64>(), 3);
    EXPECT_EQ(declare_options->changes["tablespace"]->as<ASTIdentifier>()->fullName(), "tablespace_name");

    ASTPtr arguments = declare_options->changes["union"]->as<ASTFunction>()->arguments;
    EXPECT_EQ(arguments->children[0]->as<ASTIdentifier>()->fullName(), "table_01");
    EXPECT_EQ(arguments->children[1]->as<ASTIdentifier>()->fullName(), "table_02");
}
