...
- **Only plain `SELECT` helpers can be materialized inside `WITH RECURSIVE`**: a non-recursive helper CTE whose body is a plain `SELECT` may be declared `MATERIALIZED` in the same `WITH RECURSIVE` clause as a recursive CTE. When such a helper is read from a recursive member, it is materialized once and every recursive iteration reads the same temporary result, even if the helper is referenced only once. This also applies to a `MATERIALIZED` CTE declared in an enclosing `WITH` scope and referenced from a recursive member: what matters is that the reference sits in a repeatedly executed recursive member, not where the CTE is declared. The restriction to a plain `SELECT` body applies only to helpers declared directly inside `WITH RECURSIVE`; enclosing materialized CTEs follow the normal CTE syntax rules.
- **What cannot be materialized in a recursive query**: the recursive CTE itself; a helper declared inside `WITH RECURSIVE` whose body is a top-level set operation (`UNION`, `INTERSECT`, `EXCEPT`), because such a CTE is indistinguishable from the recursive CTE itself until the query is analyzed; and a helper read from a recursive member whose own definition reads that recursive CTE, because a materialized result is computed once while such a definition would have to be recomputed on every step. All three raise an `UNSUPPORTED_METHOD` exception. The first two depend only on the syntax and are raised whether or not `enable_materialized_cte` is enabled. The third is raised only when `enable_materialized_cte = 1`: with the setting off, `MATERIALIZED` is ignored and the CTE behaves as an ordinary one, which is also why the same query without `MATERIALIZED` keeps working. Whether an accepted helper is actually materialized, rather than inlined like an ordinary CTE, likewise depends on that setting.
