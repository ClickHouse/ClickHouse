...
void ASTSelectQuery::readJSON(const Poco::JSON::Object & json)
{
    ...
    if (limit_shuffle && (limitOffset() || limit_with_ties || orderBy() || limitAfter() || limitUntil()))
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "limit_shuffle cannot be used with LIMIT OFFSET, LIMIT WITH TIES, LIMIT AFTER/UNTIL, or ORDER BY during AST JSON deserialization");
