...
void ASTFunction::appendColumnNameImpl(WriteBuffer & ostr) const
{
    ...
    if (is_window_function)
    {
        ...
        {
            writeCString("(", ostr);
            window_definition->format(ostr, FormatSettings{true});
            writeCString(")", ostr);
        }
    }
}
...
static std::optional<String> getOptionalStringLiteral(const ASTPtr & node)
{
...
void ASTFunction::formatImplWithoutAlias(FormattingBuffer out) const
{
    ...
    if (arguments)
    {
        bool is_regex_function = (name == "match" || name == "extract" || name == "extractAll" || name == "replaceRegexpOne"
        ...
        for (size_t i = 0, size = arguments->children.size(); i < size; ++i)
        {
            ...
            if (!out.shouldShowSecrets() && (secret_arguments.start <= i) && (i < secret_arguments.start + secret_arguments.count))
            {
                if (secret_arguments.are_named)
                {
                   assert_cast<const ASTFunction *>(argument.get())->arguments->children[0]->formatImpl(out_dont_need_parens);
                   out.writeOperator(" = ");
                }
                out.writeSecret();
            ...
            if ((i == 1) && is_regex_function)
            {
                auto str = getOptionalStringLiteral(argument);
                if (str.has_value())
                {
            ...
            argument->formatImpl(out_dont_need_parens);
        }
    }
