...
bool ParserStorage::parseImpl(Pos & pos, ASTPtr & node, Expected & expected)
{
    ...
    ParserKeyword s_index_hint("INDEX HINT");
    ...
    while (true)
    {
        ...
        if (!index_hint && s_index_hint.ignore(pos, expected))
        {
            if (expression_p.parse(pos, index_hint, expected))
                continue;
            else
                return false;
        }
    ...
    storage->set(storage->index_hint, index_hint);
