...
**Which tables are deferred:** an engine opts in, which `system.table_engines.supports_deferred_load` reports. Engines that work in the background are always loaded eagerly, because deferring them would cancel that work rather than delay it: the message queue engines (`Kafka`, `RabbitMQ`, `NATS`, `FileLog`, `S3Queue`, `AzureQueue`) would stop consuming, `Distributed` would stop sending the batches its asynchronous inserts left on disk, `Buffer` would stop flushing, and `MaterializedPostgreSQL` would stop consuming its replication slot. Views, materialized views, dictionaries, `TimeSeries` tables, and tables backed by table functions are always loaded eagerly as well.
