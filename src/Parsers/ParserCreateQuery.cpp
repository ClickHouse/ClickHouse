...
Copying the schema requires the `SHOW COLUMNS` privilege on `db.table`. Inheriting a definition that holds credentials, which `SHOW CREATE TABLE` masks, also requires the `SELECT` privilege, because the copy can read through them.
