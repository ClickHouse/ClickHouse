...
bool ParserCreateTableQuery::parseImpl(Pos & pos, ASTPtr & node, Expected & expected)
{
    ...
    auto parse_storage = [&]
    {
        ...
        if (storage && storage->engine && (storage->engine->name == "TimeSeries"))
        {
            ...
            ParserViewTargets({ViewTarget::Samples, ViewTarget::RecentSamples, ViewTarget::Histograms, ViewTarget::Tags, ViewTarget::MetricFamilies}).parse(pos, targets, expected);
        }
