...
Matchers are expanded before expression validation and execution. Unlike in `SELECT` queries, matcher expansion in stored expressions is independent of session settings, so the same table definition always expands to the same expression: `*` never includes `MATERIALIZED` or `ALIAS` columns (the `asterisk_include_materialized_columns` and `asterisk_include_alias_columns` settings have no effect here), while `COLUMNS` matches them. `EPHEMERAL` columns are not included in matcher expansion.
