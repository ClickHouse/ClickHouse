...
Applies to database engines that store table metadata on disk (e.g. `Atomic`, `Ordinary`).

**Which tables are deferred:** an engine opts in, which `system.table_engines.supports_deferred_load` reports. Engines that work in the background are always loaded eagerly, because deferring them would cancel that work rather than delay it: `Distributed` would stop sending the batches its asynchronous inserts left on disk, `Buffer` would stop flushing, and `MaterializedPostgreSQL` would stop consuming its replication slot. The message queue engines (`Kafka`, `RabbitMQ`, `NATS`, `FileLog`, `S3Queue`, `AzureQueue`) consume only to feed their materialized views, so a table that has some is loaded eagerly and one that has none is deferred like any other. Views, materialized views, dictionaries, `Alias` tables, `TimeSeries` tables, and tables backed by table functions are always loaded eagerly as well.
...
- `is_loaded` is `0` while a table is deferred. The `engine` column reports the engine from the `CREATE` query either way.
- The columns that describe the structure and the data of a deferred table are empty, because the real storage does not exist yet: `total_rows`, `total_bytes`, `primary_key`, `sorting_key`, `partition_key`, `sampling_key`, `storage_policy` and `data_paths`. The same holds for the rows a deferred table contributes to `system.parts`, `system.columns` and `system.data_skipping_indices`.
- Reading the system tables never loads a table, so monitoring a database with this setting does not defeat it.
...
- `SHOW COLUMNS` and `SHOW INDEXES` name a single table, so they load it rather than report the empty keys and indices the system tables have for a deferred one.
