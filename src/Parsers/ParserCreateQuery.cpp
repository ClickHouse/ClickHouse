...
It depends on the full-precision vector being read lazily for the shortlisted rows: it needs the analyzer, setting `query_plan_optimize_lazy_materialization` and a `LIMIT` (including any `OFFSET`) no greater than `query_plan_max_limit_for_lazy_materialization` (`0` means unbounded), and it does not apply to a table with patch parts, or to a query using `SAMPLE`, `FINAL` on an engine other than `ReplacingMergeTree`, or a filter or row policy that reads the vector column. Those queries run the exact full-precision scan instead. A query that applies `arrayJoin` before the `ORDER BY`, for example in `WHERE`, is an exception: the two-stage search still runs there although the vector cannot be read lazily, so it reads more than the exact scan.
