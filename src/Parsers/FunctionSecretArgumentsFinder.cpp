...
namespace
{
    ...
    {
        bool changed = false;
        /// The userinfo is taken greedily up to the last at-sign before the path, so a userinfo
        /// whose password itself contains an at-sign is masked whole, not just up to the first one.
        if (maskURIUserinfo(url))
            changed = true;
        if (maskPresignedURLParameters(url))
            changed = true;
        return changed;
    }
}
