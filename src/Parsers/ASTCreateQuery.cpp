...
void ASTCreateQuery::readJSON(const Poco::JSON::Object & json)
{
    ...
    if (child)
    {
        /// `formatImpl` emits `sql_security` only for the shapes accepting it (`supportSQLSecurity()`), but
        /// `InterpreterCreateQuery::createTable` runs `processSQLSecurityOption` for any non-null
        /// `sql_security`. Reject it on other shapes (e.g. a plain `CREATE TABLE`) so the formatted
        /// SQL cannot hide a definer clause that execution still enforces.
        if (!supportSQLSecurity())
            throw Exception(ErrorCodes::BAD_ARGUMENTS,
                "`sql_security` is only valid for VIEW / MATERIALIZED VIEW / TimeSeries tables during AST JSON deserialization");
    ...
    /// `POPULATE` is accepted only by `ParserCreateViewQuery` for a materialized view.
    /// A plain table, an ordinary view and a dictionary never carry it.
    if (is_populate && !is_materialized_view)
        throw Exception(ErrorCodes::BAD_ARGUMENTS,
            "`CreateQuery` has 'is_populate' set but is not a materialized view "
    ...
    /// `EMPTY` is accepted by `ParserCreateTableQuery` (`CREATE TABLE ... EMPTY AS ...`) and by
    /// `ParserCreateViewQuery` for a materialized view.
    ...
    if (is_create_empty && (is_ordinary_view || is_dictionary))
        throw Exception(ErrorCodes::BAD_ARGUMENTS,
            ...
            "deserialization, but the parser accepts `EMPTY` only for tables and materialized views");
