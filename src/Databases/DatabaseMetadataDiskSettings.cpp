...
#define LIST_OF_DATABASE_METADATA_DISK_SETTINGS(DECLARE, DECLARE_WITH_ALIAS) \
    ...
    DECLARE(UInt64, max_rows, 0, R"(Maximum number of active rows across the database's `MergeTree`-family tables (other engines are not counted). 0 means unlimited. Enforced on `INSERT`, table attachment or rename into the database, and partition attach, import, or move operations; a single `INSERT` batch may overshoot it. Enforcement is a best-effort snapshot check, not a reservation: concurrent operations may each observe free headroom and transiently overshoot the limit together; the next checked operation then fails. An `ATTACH` of a part deduplicated by ZooKeeper, and a retry of an `INSERT` that insert deduplication recognizes as already written, stay no-ops and are not rejected by the limit. Only supported by the `Atomic` and `Ordinary` engines.)", 0) \
