...
#define LIST_OF_DATABASE_METADATA_DISK_SETTINGS(DECLARE, DECLARE_WITH_ALIAS) \
    ...
    DECLARE(UInt64, max_tables, 0, R"(Maximum number of tables the database may contain. 0 means unlimited. When the limit is reached, CREATE TABLE and ATTACH TABLE throw an exception. Dictionaries do not count toward the limit.)", 0) \
