    ...
    : IDatabase(name_), WithContext(context_->getGlobalContext()), path(path_), log(getLogger("DatabaseFileSystem(" + name_ + ")"))
{
    ...
    if (!fs::exists(path_as_fs))
    {
        ...
        LOG_WARNING(log, "Path does not exist: {}. The database has no tables until it reappears", pathToGenericString(path_as_fs));
    }
...
bool DatabaseFilesystem::checkTableFilePath(const std::string & table_path, ContextPtr context_, bool throw_on_error) const
{
    ...
    if (!containsGlobs(table_path))
    {
        ...
        if (!existsOrFileNameTooLong([&] { return fs::exists(table_path_as_fs); }))
        {
            if (throw_on_error)
        ...
        if (!existsOrFileNameTooLong([&] { return fs::is_regular_file(table_path_as_fs); }))
        {
            if (throw_on_error)
...
StoragePtr DatabaseFilesystem::tryGetTableFromCache(const std::string & name) const
{
    ...
    if (table && !existsOrFileNameTooLong([&] { return fs::exists(pathFromString(getTablePath(name))); }))
    {
        std::lock_guard lock(mutex);
