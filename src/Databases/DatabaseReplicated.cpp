...
bool DatabaseReplicated::shouldReplicateQuery(const ContextPtr & query_context, const ASTPtr & query_ptr) const
{
    ...
    if (const auto * alter = query_ptr->as<const ASTAlterQuery>())
    {
        if (alter->isAttachAlter() || alter->isFetchAlter() || alter->isDropPartitionAlter() || alter->isFreezeAlter()
            || alter->isUnlockSnapshot() || alter->isReplacePartition())
            return false;
    ...
    const String & node_path, const String & replicas_path, ContextPtr context_, const Strings & hosts_to_wait)
{
    ...
    auto source = std::make_shared<ReplicatedDatabaseQueryStatusSource>(zookeeper_name, node_path, replicas_path, context_, hosts_to_wait);
