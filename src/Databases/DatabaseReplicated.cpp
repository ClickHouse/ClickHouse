...
static UUID getTableUUIDIfReplicated(const String & metadata, ContextPtr context)
{
    ...
    /** No size limit, as everywhere else this metadata is parsed (`parseQueryFromMetadata`, the
      * on-disk loads): the text was generated by a server and is as long as the table's definition
      * needs, while `max_query_size` is about what a *client* may send. A recovering replica has no
      * way to learn the value the creating session raised it to, so bounding the parse here left a
      * table whose metadata exceeds the default beyond reach: recovery threw `Max query size
      * exceeded` before creating it and retried forever, while the replicas that had loaded the very
      * same text from disk kept serving it.
      */
    ...
    ASTPtr query = parseQuery(parser, metadata, /*max_query_size=*/ 0, depth, backtracks);
