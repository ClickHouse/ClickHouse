                                            ...
                                            int64_t expected_max_log_ptr_czxid)
{
    ...
    for (const auto & [table_name, create_table_query] : table_name_to_metadata)
    {
        ...
        tables_dependencies.addDependencies(qualified_name, getDependenciesFromCreateQuery(getContext()->getGlobalContext(), qualified_name, query_ast, getContext()->getCurrentDatabase().getFullName()).dependencies);
    }
