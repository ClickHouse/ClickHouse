...
#define LIST_OF_DATABASE_REPLICATED_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(String, default_replica_path, "/clickhouse/databases/{uuid}", "The path to the database in ZooKeeper. Used during database creation if argumnets are omitted.", 0) \
    DECLARE(String, default_replica_shard_name, "{shard}", "The shard name of the replica in the database. Used during database creation if argumnets are omitted.", 0) \
    DECLARE(String, default_replica_name, "{replica}", "The name of the replica in the database. Used during database creation if argumnets are omitted.", 0) \
