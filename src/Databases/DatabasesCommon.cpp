...
void validateCreateQuery(const ASTCreateQuery & query, const VirtualColumnsDescription & virtuals, ContextPtr context)
{
    ...
    if (storage.ttl_table && primary_key.has_value())
        TTLTableDescription::getTTLForTableFromAST(storage.ttl_table->ptr(), columns_desc, context, *primary_key, /* is_metadata_load */ true, /* allow_suspicious */ true, /* allow_experimental_codecs */ true);
}
}
