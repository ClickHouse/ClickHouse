 ...
 *  - Value range: [0, 15] with 3 elements. other elements do not exists.
...
private:
    ...
    /** We distinguish between the cases where index does not exist and index is 0.
     * zero_indexes is used to store all indexes whose value is 0.
...
public:
    ...
    std::string shortDebugString() const
    {
        ...
        std::string res = fmt::format(
            "vector_type: {}; index_type: {}; value_type: {}; integer_bit_num: {}; fraction_bit_num: {}; "
            "zero_indexes cardinality: {}; "
            "data_array total card: {}, all value sum: {}, size: {}, cardinality: {}.",
            type,
            demangle(typeid(IndexType).name()),
            demangle(typeid(ValueType).name()),
            integer_bit_num,
            fraction_bit_num,
            zero_indexes->size(),
            getAllIndex()->size(),
     ...
     *    - Assert fraction_bit_num == zero in this case
     *
     * 2. Signed integer/float path:
     *    - Uses highest bit (total_bit_num-1) as sign bit.
     *    - Negative numbers are represented using two's complement notation,
     *      so when expanding to 64 bits, all high bits need to be filled with 1.
     *
