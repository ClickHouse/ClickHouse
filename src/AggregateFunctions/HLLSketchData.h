...
public:
    ...
    /// Insert original value without hash, as `datasketches::hll_sketch.update` will do the hash internal.
    void insertOriginal(StringRef value)
    {
        getHLLUpdate()->update(value.data, value.size);
    }

    /// Note that `datasketches::hll_sketch.update` will do the hash again.
