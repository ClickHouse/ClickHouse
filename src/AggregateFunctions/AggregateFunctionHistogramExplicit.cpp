...
{
    /// place is contractually alignof(UInt64)-aligned (see alignOfData() below), but that contract
    /// previously lived only as an unstated assumption inside a bare reinterpret_cast. Asserted here
    /// so a caller that ever violates it (e.g. wrong nestedPlace()/place_offset arithmetic upstream)
    /// fails loudly in debug builds instead of silently reading/writing across a misaligned boundary;
    /// assume_aligned then passes that same guarantee on to the optimizer.
    ALWAYS_INLINE static const UInt64 * getCounts(ConstAggregateDataPtr place)
    {
        chassert(reinterpret_cast<uintptr_t>(place) % alignof(UInt64) == 0);
        return std::assume_aligned<alignof(UInt64)>(reinterpret_cast<const UInt64 *>(place));
    }

    ALWAYS_INLINE static UInt64 * getCounts(AggregateDataPtr place)
    {
        chassert(reinterpret_cast<uintptr_t>(place) % alignof(UInt64) == 0);
        return std::assume_aligned<alignof(UInt64)>(reinterpret_cast<UInt64 *>(place));
    }
...
/// Single abstraction for the two independent per-row skip signals that add()/addBatch*() juggle: an
/// optional -If condition (if_flags) and, separately, an optional NULL map. Previously each caller
/// decided on its own whether/how to check if_flags before reaching per-row classification, which is
/// exactly the kind of duplicated, easy-to-desync logic this replaces. HasNullMap stays a template
/// parameter (not a runtime null check) so the no-null hot path keeps compiling away the null branch
/// entirely, preserving the auto-vectorization the batch paths rely on.
template <bool HasNullMap>
struct RowMask
{
    const UInt8 * __restrict if_flags; /// nullptr => every row is live
    const UInt8 * __restrict null_map; /// only meaningful when HasNullMap is true
...
/// Walks the getNestedFunction() chain (through combinator wrappers such as -Distinct, -OrFill,
/// -State) to find the underlying histogramExplicit instance. Shared by the -Null and -IfNull
/// adapters below, which otherwise each repeated this walk verbatim.
template <typename T>
const AggregateFunctionHistogramExplicit<T> & resolveHistogramTarget(const IAggregateFunction & function, const char * caller_name)
{
    ...
    throw Exception(ErrorCodes::LOGICAL_ERROR, "{}: no histogramExplicit found in the nested function chain", caller_name);
}

/// Bundles the one unsupported assumption AggregateFunctionHistogramExplicitNullAdapter makes about
/// AggregateFunctionOrFill's private layout: that -OrNull/-OrDefault stores its "saw a row" flag as a
/// single byte immediately after its nested function's own place bytes. There is no public accessor
/// for this on AggregateFunctionOrFill today, so every read and write of it is funneled through this
/// one type instead of being recomputed (or re-poked via raw place[offset] = 1) at each call site --
/// which is exactly the kind of drift that has already caused real bugs here across edits (see
/// [[histogram-explicit]]: the flag byte was previously double-applied, then wrongly gated on
/// null_map[i] regardless of whether the nested histogram tracks nulls at all).
/// TODO: replace with a real accessor on AggregateFunctionOrFill (e.g. markSawRow(place)) once one
/// exists; until then, any layout change on the OrFill side must be mirrored here by hand.
struct OrFillSeenFlag
{
    bool present = false;
    size_t offset = 0;

    ALWAYS_INLINE void markSeen(AggregateDataPtr place) const
    {
        chassert(present);
        place[offset] = 1;
    }

    static OrFillSeenFlag detect(const IAggregateFunction & function, const IAggregateFunction & target)
    {
        if (&function == &target)
            return {};
        if (!dynamic_cast<const AggregateFunctionOrFill<true> *>(&function) && !dynamic_cast<const AggregateFunctionOrFill<false> *>(&function))
            return {};

        AggregateFunctionPtr next = function.getNestedFunction();
        if (!next)
            return {};

        /// OrFill may wrap the histogram through one or more layout-transparent hops (e.g. -State) --
        /// walk through them, since they contribute no bytes of their own. Object identity alone
        /// (next.get() == &target) would wrongly reject those, since OrFill's private size_of_data
        /// is captured from its immediate nested function, not necessarily from `target` itself.
        const IAggregateFunction * probe = next.get();
        while (probe != &target)
        {
            AggregateFunctionPtr deeper = probe->getNestedFunction();
            if (!deeper || probe->sizeOfData() != deeper->sizeOfData())
                return {};
            probe = deeper.get();
        }

        return {.present = true, .offset = target.sizeOfData()};
    }
};
...
public:
    ...
    {
        value_target = &resolveHistogramTarget<T>(*nested_function, "AggregateFunctionHistogramExplicitIfNullAdapter");
...
private:
    ...
    /// nestedPlace(place) is only safe to hand to target_histogram's raw fast paths
    /// (addManyNulls/addBatchNullable/addBatchSparse's dynamic dispatch aside) when nothing between
    /// nested_function and target_histogram actually occupies its own bytes in the place buffer --
    /// object identity (nested_function.get() == target_histogram) is stronger than necessary and
    /// wrongly rejects zero-overhead wrappers like AggregateFunctionState (-State), which delegates
    /// sizeOfData()/alignOfData()/create() verbatim and therefore introduces no offset at all. A
    /// wrapper's sizeOfData() equals its nested function's sizeOfData() iff its own footprint is zero,
    /// which is exactly the condition that makes the place address layout-transparent through it.
    static bool isPlaceLayoutTransparentTo(const IAggregateFunction & from, const IAggregateFunction & target)
    {
        const IAggregateFunction * probe = &from;
        while (probe != &target)
        {
            AggregateFunctionPtr next = probe->getNestedFunction();
    ...
    /// See OrFillSeenFlag above for what this represents and why it exists.
    OrFillSeenFlag or_fill_seen_flag_;
...
public:
    AggregateFunctionHistogramExplicitNullAdapter(AggregateFunctionPtr nested_function_, const DataTypes & arguments, const Array & params)
        : Base(std::move(nested_function_), arguments, params)
    {
        target_histogram = &resolveHistogramTarget<T>(*nested_function, "AggregateFunctionHistogramExplicitNullAdapter");

        nested_is_direct_histogram = isPlaceLayoutTransparentTo(*nested_function, *target_histogram);
        or_fill_seen_flag_ = OrFillSeenFlag::detect(*nested_function, *target_histogram);
    }
    ...
    void addManyDefaults(AggregateDataPtr __restrict place, const IColumn ** /*columns*/, size_t length, Arena * /*arena*/) const override
    {
        ...
        if (or_fill_seen_flag_.present)
        {
            nestedTyped().addManyNulls(nestedPlace(place), length);
            /// Mirror addBatchSinglePlaceNotNull's treat_null_rows_as_seen: only mark "saw a row" for
            /// an all-default (NULL) batch when the histogram actually does something with those NULLs.
            if (nestedTyped().preservesNulls())
                or_fill_seen_flag_.markSeen(nestedPlace(place));
            return;
        }
        ...
        ssize_t if_argument_pos) const override
    {
        ...
        if (or_fill_seen_flag_.present)
        {
            ...
            const bool treat_null_rows_as_seen = nestedTyped().preservesNulls();
            const UInt8 * null_map = nullable_column.getNullMapData().data();
            const auto * flags = if_argument_pos >= 0 ? &assert_cast<const ColumnUInt8 &>(*columns[if_argument_pos]).getData() : nullptr;
            ...
            for (size_t i = row_begin; i < row_end; ++i)
            {
                ...
                if (flags && !(*flags)[i])
                    continue;
                if (!treat_null_rows_as_seen && null_map[i])
                    continue;
                or_fill_seen_flag_.markSeen(places[i] + adjusted_place_offset);
            }
            return;
        }
    ...
    /// IAggregateFunctionHelper::addBatchSparse's default dispatches through the sparse column's own
    /// values column (index 0 for implicit default rows), calling add() per row -- it never routes
    /// through nested_function's not-null fast path the way addBatchSinglePlace/addBatch above do, so
    /// it doesn't depend on nested_function being the bare histogram. Written out explicitly (rather
    /// than relying on inherited CRTP dispatch) so a sparse default row -- which for a Nullable column
    /// is NULL -- goes through our own add(), and is therefore correctly counted into the null bucket
...
private:
    ...
    /// mask.skip()/mask.isNull() replace what used to be a raw if_flags pointer plus a separate
    /// HasNullMap-gated null_map check duplicated at each call site.
        ...
        std::span<const BoundaryT> boundaries) const
    {
        ...
        const RowMask<HasNullMap> mask{if_flags, null_map};

        for (size_t i = row_begin; i < row_end; ++i)
            classifyAndIncrement<BoundaryType, HasNullMap>(counts, data, mask, i, offsets, boundaries);
    }
...
public:
    void add(AggregateDataPtr __restrict place, const IColumn ** columns, size_t row_num, Arena *) const override
    {
        ...
        UInt64 * __restrict counts = Data::getCounts(place);
        const SpecialOffsets offsets = getSpecialOffsets();
        const RowMask<false> mask{nullptr, nullptr};

        dispatchBoundaryType(
            [&](auto boundary_type)
            {
                classifyAndIncrement<decltype(boundary_type)::value, false>(counts, data, mask, row_num, offsets, boundaries_native_);
            });
    }
            ...
            [&](auto boundary_type)
            {
                for (size_t i = row_begin; i < row_end; ++i)
                {
                    ...
                    classifyAndIncrement<decltype(boundary_type)::value, HasNullMap>(
                        counts, data, mask, i, offsets, boundaries_native_);
                }
            });
    }
