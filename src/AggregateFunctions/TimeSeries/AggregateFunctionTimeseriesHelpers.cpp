...
└─────────────────────────────────────────────────────────────────────────────────────────────────┘
    ...
    FunctionDocumentation::Description description_timeSeriesAvgOverTimeToGrid = R"(
Aggregate function that calculates Prometheus-style `avg_over_time` values on a regular time grid. For each grid point it averages samples in the half-open window `(timestamp - window, timestamp]`.
    )";
...
timeSeriesAvgOverTimeToGrid(start_timestamp, end_timestamp, grid_step, window)(timestamp, value)
    ...
    FunctionDocumentation::Description description_timeSeriesMaxOverTimeToGrid = R"(
Aggregate function that calculates Prometheus-style `max_over_time` values on a regular time grid. For each grid point it returns the maximum sample value in the half-open window `(timestamp - window, timestamp]`.
    )";
