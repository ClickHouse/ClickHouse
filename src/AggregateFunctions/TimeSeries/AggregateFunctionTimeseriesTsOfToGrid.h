...
struct AggregateFunctionTimeseriesTsOfToGridTraits
{
    ...
    /// (converted to Unix seconds, i.e. divided by the ticks per second of the input timestamps) per grid point.
    struct Aggregator
    {
        ...
        std::optional<ResultType> getResult(GridScaleTimestampType /*grid_timestamp*/) const
        {
            ...
            if (!combined.has_value)
                return std::nullopt;
            return static_cast<ResultType>(static_cast<Int64>(combined.timestamp)) / static_cast<ResultType>(column_ticks_per_second);
        }
    };
