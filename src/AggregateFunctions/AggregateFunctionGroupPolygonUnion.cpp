...
void registerAggregateFunctionGroupPolygonUnion(AggregateFunctionFactory &);
...
- [Geo Types](/sql-reference/data-types/geo)
    ...
    FunctionDocumentation::Arguments arguments
        = {{"geometry", "Column to compute the union of.", {"Ring", "Polygon", "MultiPolygon"}},
           {"correct_geometry",
    ...
    FunctionDocumentation::ReturnedValue returned_value
        = {"A MultiPolygon representing the union of all input geometries.", {"MultiPolygon"}};
...
└────────────────────────────────────────────────────────────────┘
           ...
           {"Alias",
            "SELECT wkt(ST_Union(geom)) AS result FROM test_polygons;",
            R"(
┌─result─────────────────────────────────────────────────────────┐
│ MULTIPOLYGON(((5 10,5 15,15 15,15 5,10 5,10 0,0 0,0 10,5 10))) │
└────────────────────────────────────────────────────────────────┘
