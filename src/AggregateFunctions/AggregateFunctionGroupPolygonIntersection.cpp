...
- [Geo Types](/sql-reference/data-types/geo)
    ...
    FunctionDocumentation::ReturnedValue returned_value
        = {"A MultiPolygon representing the intersection of all input geometries.", {"MultiPolygon"}};
...
└────────────────────────────────────────────┘
           ...
           {"Alias",
            "SELECT wkt(ST_Intersection(geom)) AS result FROM test_polygons;",
            R"(
┌─result─────────────────────────────────────┐
│ MULTIPOLYGON(((5 10,10 10,10 5,5 5,5 10))) │
└────────────────────────────────────────────┘
        ...
        {[](const std::string & name, const DataTypes & argument_types, const Array & parameters, const Settings *)
         {
             ...
             if (argument_types.size() != 1 && argument_types.size() != 2)
                 throw Exception(
                     ErrorCodes::BAD_ARGUMENTS, "Aggregate function {} requires 1 or 2 arguments, got {}", name, argument_types.size());
