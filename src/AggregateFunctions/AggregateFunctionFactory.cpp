...
void EarlyWindowFunctionFactory::registerFunction(const String & name, Creator creator, CaseSensitiveness case_sensitiveness)
{
    if (creator == nullptr)
        throw Exception("EarlyWindowFunctionFactory: the aggregate function " + name + " has been provided "
            " a null constructor", ErrorCodes::LOGICAL_ERROR);

    if (!early_window_functions.emplace(name, creator).second)
        throw Exception("EarlyWindowFunctionFactory: the aggregate function name '" + name + "' is not unique",
            ErrorCodes::LOGICAL_ERROR);

    if (case_sensitiveness == CaseInsensitive
        && !case_insensitive_aggregate_functions.emplace(Poco::toLower(name), creator).second)
        throw Exception("EarlyWindowFunctionFactory: the case insensitive aggregate function name '" + name + "' is not unique",
            ErrorCodes::LOGICAL_ERROR);
}

AggregateFunctionPtr EarlyWindowFunctionFactory::get(
    const String & name,
    const DataTypes & argument_types,
    const Array & parameters,
    ...
    int recursion_level) const
{
        ...
        [](const auto & type) { return type->isNullable(); }))
    {
        AggregateFunctionCombinatorPtr combinator = AggregateFunctionCombinatorFactory::instance().tryFindSuffix("Null");
        if (!combinator)
            throw Exception("Logical error: cannot find early window function combinator to apply a function to Nullable arguments.", ErrorCodes::LOGICAL_ERROR);
        ...
        /// A little hack - if we have NULL arguments, don't even create nested function.
        /// Combinator will check if nested_function was created.
        if (std::none_of(type_without_low_cardinality.begin(), type_without_low_cardinality.end(),
            [](const auto & type) { return type->onlyNull(); }))
            nested_function = getImpl(name, nested_types, nested_parameters, early_window_info, recursion_level);

        return combinator->transformAggregateFunction(nested_function, type_without_low_cardinality, parameters);
    }

    auto res = getImpl(name, type_without_low_cardinality, parameters, early_window_info, recursion_level);
    if (!res)
        throw Exception("Logical error: EarlyWindowFunctionFactory returned nullptr", ErrorCodes::LOGICAL_ERROR);
    return res;
}


AggregateFunctionPtr EarlyWindowFunctionFactory::getImpl(
    const String & name_param,
    const DataTypes & argument_types,
    const Array & parameters,
    EarlyWindowFunctionInfo & early_window_info,
    int recursion_level) const
{
    String name = getAliasToOrName(name_param);
    /// Find by exact match.
    if (auto it = early_window_functions.find(name); it != early_window_functions.end())
        return it->second(name, argument_types, parameters, early_window_info);
    ...
    {
        if (combinator->isForInternalUsageOnly())
            throw Exception("Early window function combinator '" + combinator->getName() + "' is only for internal usage", ErrorCodes::UNKNOWN_AGGREGATE_FUNCTION);

        String nested_name = name.substr(0, name.size() - combinator->getName().size());
        DataTypes nested_types = combinator->transformArguments(argument_types);
        Array nested_parameters = combinator->transformParameters(parameters);

        AggregateFunctionPtr nested_function = get(nested_name, nested_types, nested_parameters, early_window_info, recursion_level + 1);
    ...
    auto hints = this->getHints(name);
    if (!hints.empty())
        throw Exception("Unknown early window function " + name + ". Maybe you meant: " + toString(hints), ErrorCodes::UNKNOWN_AGGREGATE_FUNCTION);
    else
        throw Exception("Unknown early window function " + name, ErrorCodes::UNKNOWN_AGGREGATE_FUNCTION);
}


AggregateFunctionPtr EarlyWindowFunctionFactory::tryGet(
    const String & name,
    const DataTypes & argument_types,
    const Array & parameters,
...
{
    return isAggregateFunctionName(name)
        ? get(name, argument_types, parameters, early_window_info)
        : nullptr;
}


bool EarlyWindowFunctionFactory::isAggregateFunctionName(const String & name, int recursion_level) const
{
    if (early_window_functions.count(name) || isAlias(name))
        return true;
...
EarlyWindowFunctionFactory & EarlyWindowFunctionFactory::instance()
{
    static EarlyWindowFunctionFactory ret;
    return ret;
}
