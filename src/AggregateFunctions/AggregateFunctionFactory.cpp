...
AggregateFunctionPtr AggregateFunctionFactory::get(
    ...
    bool from_declared_simple_aggregate_function,
    ...
    const Settings * settings) const
{
    ...
    if (!from_declared_state_type && !settings && CurrentThread::isInitialized())
    {
        if (auto query_context = CurrentThread::get().tryGetQueryContext())
            settings = &query_context->getSettingsRef();
    }

    if (!from_declared_state_type && settings)
        allow_lossy_numeric_supertype = (*settings)[Setting::allow_lossy_numeric_supertype];
