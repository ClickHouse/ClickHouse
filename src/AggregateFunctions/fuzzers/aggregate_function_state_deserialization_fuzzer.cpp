...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t * data, size_t size)
{
    try
    {
        ...
        try
        {
            /// Parse the aggregate function specification with depth limits
            /// This uses the same parseQuery infrastructure as other parsers with proper depth limiting
            ParserDataType parser;
            String full_type_spec = fmt::format("AggregateFunction({})", args);

            ASTPtr ast = parseQuery(parser,
                                  full_type_spec.data(),
                                  full_type_spec.data() + full_type_spec.size(),
                                  "data type",
                                  max_query_size,
                                  max_parser_depth,
