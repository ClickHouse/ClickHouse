...
createGraphOperation(const std::string & name, const DataTypes & argument_types, const Array & parameters, const Settings *)
{
    ...
    if (GraphOperation::kExpectedParameters != 0)
    {
        if (parameters.size() != GraphOperation::kExpectedParameters)
            throw Exception(
                "Aggregate function " + name + " requires " + std::to_string(GraphOperation::kExpectedParameters) + " parameters",
