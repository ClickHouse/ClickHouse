...
void CudaAggregateFunctionUniq<String>::cudaInitAggregateData(
    CudaSizeType places_num, CudaAggregateDataPtr places, cudaStream_t stream) const
{
    ...
    kerCudaInitAggregateData<<<(places_num/256)+1,256,0,stream>>>(places_num, (Data*)places);
}

void CudaAggregateFunctionUniq<String>::cudaAddBulk(
    ...
    char *tmp_buf, cudaStream_t stream) const
{
    ...
    kerCudaAddBulk<<<(elements_num/256)+1,256,0,stream>>>((Data*)places, elements_num, (UInt64*)tmp_buf, res_buckets);
}

void CudaAggregateFunctionUniq<String>::cudaMergeBulk(
    CudaAggregateDataPtr places, CudaSizeType elements_num,
    CudaAggregateDataPtr places_from, CudaSizeType *res_buckets, 
    cudaStream_t stream) const
{
    kerCudaMergeBulk<<<(elements_num/256)+1,256,0,stream>>>((Data*)places, elements_num, (Data*)places_from, res_buckets);
}
