...
struct MovingSum
{
    using Data = MovingSumData<std::conditional_t<Decimal<T>, Decimal128, NearestFieldType<T>>>;
...
struct MovingAvg
{
    using Data = MovingAvgData<std::conditional_t<Decimal<T>, Decimal128, Float64>>;
    using Function = MovingImpl<T, LimitNumberOfElements, Data>;
};
