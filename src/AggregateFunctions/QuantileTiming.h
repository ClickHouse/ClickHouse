...
namespace detail
{
    ...
    struct QuantileTimingTiny
    {
        ...
        {
            if (ch_unlikely(x > BIG_THRESHOLD))
                x = BIG_THRESHOLD;
...
public:
    ...
    void add(UInt64 x)
    {
        ...
        {
            if (ch_unlikely(tiny.count == TINY_MAX_ELEMS))
                tinyToMedium();

            if (which() == Kind::Medium)
            {
                if (ch_unlikely(mediumIsWorthToConvertToLarge()))
                {
                    mediumToLarge();
                    large->insert(x);
                }
    ...
    void merge(const QuantileTiming & rhs)
    {
        ...
        else
        {
            ...
            if (kind == Kind::Medium && ch_unlikely(mediumIsWorthToConvertToLarge()))
            {
                mediumToLarge();
            }
        }
