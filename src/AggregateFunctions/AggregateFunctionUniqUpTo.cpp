...
void registerAggregateFunctionUniqUpTo(AggregateFunctionFactory & factory)
{
    factory.registerFunction("uniqUpTo", {createAggregateFunctionUniqUpTo, {.description = R"DOC(Calculates the number of distinct values of the argument, but only up to the specified threshold; if the number of distinct values exceeds the threshold, it returns threshold + 1.)DOC", .category = FunctionDocumentation::Category::AggregateFunction}, {.returns_default_when_only_null = true, .support_variant_argument = true}});
}
