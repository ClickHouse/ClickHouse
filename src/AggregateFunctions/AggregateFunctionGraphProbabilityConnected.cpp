...
public:
    ...
    Float64 calculateOperation(ConstAggregateDataPtr place, Arena *) const
    {
        ...
        for (const auto & [from, _] : data(place).graph) 
        {
            if (!visited.has(from))
            {
                UInt64 component_size = data(place).componentSize(from, &visited);
                connected_vertices += component_size * (component_size - 1);
            }
        }
        return static_cast<Float64>(connected_vertices) / data(place).graph.size() / (data(place).graph.size() - 1);
    }
};
