...
struct WindowFunctionWorkspace
{
    ...
    // Cached value of aggregate function supportsWindowFrameSubtraction virtual method
    bool supports_window_frame_subtraction = false;

    // Cached getMonotonicWindowFrameKind(), downgraded to None for frames where the
    // moving min/max doesn't pay off (see the WindowTransform constructor).
    MonotonicWindowFrameKind monotonic_window_frame_kind = MonotonicWindowFrameKind::None;
