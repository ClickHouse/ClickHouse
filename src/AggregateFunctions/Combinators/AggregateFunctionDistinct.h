...
public:
        ...
        const AggregateFunctionProperties & /*properties*/) const override
    {
     ...
     AggregateFunctionPtr getOwnNullAdapterIf(
        ...
        const AggregateFunctionProperties & /*properties*/) const override
    {
        /// preservesNulls() is about the VALUE argument(s), not the trailing -If condition -- the
        /// generic IfNull wrapper already handles a nullable condition on its own (skip rows where
        /// cond is NULL), so only a nullable VALUE argument needs the throw below.
        chassert(!arguments.empty());
        const bool has_nullable_value_argument = std::any_of(
            arguments.begin(), arguments.end() - 1, [](const DataTypePtr & type) { return type->isNullable(); });

        if (has_nullable_value_argument && nested_func->preservesNulls())
        {
