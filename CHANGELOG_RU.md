...
* Добавлена агрегатная функция `maxIntersections(left, right)`, возвращающая максимальное количество интервалов `[left(i); right(i)]`, пересекающихся в одной точке. Функция `intersectionsMaxPos(left, right)` возвращает начало такого "максимального" интервала. (Michael Furmur).
...
* Исправлена ошибка, приводящая к невозможности вставить данные в `Replicated`-таблицу, если в конфигурации кворума `ZooKeeper` задан непустой `chroot`.
* Исправлен алгоритм вертикального мержа при пустом ключе `ORDER BY`.
