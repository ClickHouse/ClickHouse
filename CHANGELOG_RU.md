...
### Новые возможности:
* Поддержка запроса `ALTER TABLE t DELETE WHERE` для нереплицированных MergeTree-таблиц.
...
* Возможность в `users.xml` указывать маску подсети в формате `10.0.0.1/255.255.255.0`.
* Добавлена функция `arrayDistinct()`.

### Улучшения:
* Изменена схема версионирования релизов. Теперь первый компонент содержит год релиза, второй - номер крупных изменений (увеличивается для большинства релизов), третий - патч-версия. Релизы по-прежнему обратно совместимы, если другое не указано в changelog.
...
* Исправлен редкий deadlock в клиентской библиотеке ZooKeeper, который возникал при сетевой ошибке во время вычитывания ответа.
...
* Исправлена работа движка SummingMergeTree со столбцами типа AggregateFunction ([Constantin S. Pan](https://github.com/yandex/ClickHouse/pull/2665)).
...
* Исправлена несовместимость серверов разных версий при распределённых запросах, использующих функцию `CAST` не в верхнем регистре.
