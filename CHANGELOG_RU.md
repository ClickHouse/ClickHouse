...
* Возможность в `users.xml` указывать маску подсети в формате `10.0.0.1/255.255.255.0` ([#2637](https://github.com/yandex/ClickHouse/pull/2637)).
...
### Улучшения:
* Изменена схема версионирования релизов. Теперь первый компонент содержит год релиза, второй - номер крупных изменений (увеличивается для большинства релизов), третий - патч-версия. Релизы по-прежнему обратно совместимы, если другое не указано в changelog ([#2646](https://github.com/yandex/ClickHouse/pull/2646)).
