...
* Изменён генератор случайных чисел для агрегатных функций `quantile`, `quantileDeterministic`. Функция `quantileDeterministic` может выдавать другой результат. Если вам важно, чтобы функция `quantileDeterministic` возвращала тот же результат, что и раньше, обращайтесь к разработчикам ClickHouse.
