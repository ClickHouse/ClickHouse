...
def with_long_retention_tags(artifacts):
    ...
    Every workflow uploading these artifacts has to apply the same tags. The
    upload path is keyed by branch and commit, so an untagged upload replaces an
    object another workflow tagged for long retention with a default-retention
    one. The tags also feed the job digest, so workflows that disagree about
    them cannot share a build cache entry.
    """
    return [
