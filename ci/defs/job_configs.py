...
common_integration_test_job_config = Job.Config(
    ...
    run_in_docker=f"clickhouse/integration-tests-runner+root+--memory={LIMITED_MEM}+--privileged+--dns-search='.'+--security-opt seccomp=unconfined+--cgroupns=private+--cap-add=SYS_PTRACE+{docker_sock_mount}+--volume=clickhouse_integration_tests_volume:/var/lib/docker+--volume=/sys/fs/cgroup:/sys/fs/cgroup:rw",
)
