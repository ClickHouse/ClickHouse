...
class JobConfigs:
    ...
    ci_tests = Job.Config(
        ...
        timeout=1200,
        # NOTE (strtgbb): temp non-blocking — CH server exits 137 during CI Tests setup; dig deeper separately
        allow_failure=True,
    ...
    keeper_stress_job = Job.Config(
        ...
        run_in_docker=(
            ...
            f"--security-opt seccomp=unconfined+--cap-add=SYS_PTRACE+{docker_sock_mount}+--volume=clickhouse_integration_tests_volume:/var/lib/docker+--cgroupns=host+--ulimit nofile=262144:262144"
        ),
        digest_config=Job.CacheDigestConfig(
            include_paths=[
