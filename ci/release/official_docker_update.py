...
def update_library_images(repos: LibraryRepos, dry_run: bool = True) -> None:
    ...
    - Create directories in ClickHouse/docker-library via
        `ci/release/official_docker.py generate-tree`
    - If there aren't changes:
        - Finish
    - Create a new LDF as `ci/release/official_docker.py generate-ldf`
...
def update_ldf_repo(repos: LibraryRepos, dry_run: bool = True) -> None:
    ...
    generate_tree_cmd = (
        f"{executable} ci/release/official_docker.py generate-tree --build --fetch-tags "
    ...
    generate_ldf_cmd = (
        f"{executable} ci/release/official_docker.py generate-ldf -vvv "
        f"--directory {ldf_dir} --dockerfile-glob Dockerfile.ubuntu --image-type server"
    )
    if dry_run:
