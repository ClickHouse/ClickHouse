...
def select(pr_labels, release_branches):
...
class TestSelectVersionSpecific(unittest.TestCase):
    ...
    def test_inactive_label_widens_selection(self):
        ...
        branches = select(["v25.8-must-backport", "v26.2-must-backport"], RB)
        self.assertEqual(branches, RB)
    ...
    def test_floor_newer_than_all_active_yields_no_branches(self):
        ...
        branches = select(["v27.1-must-backport"], RB)
        self.assertEqual(branches, [])

    def test_floor_at_newest_branch_selects_only_it(self):
        branches = select(["v26.3-must-backport"], RB)
...
class TestSelectGeneral(unittest.TestCase):
    def test_general_label_fans_out_to_all_branches(self):
        # A general backport label targets every active release branch.
        branches = select(["pr-must-backport"], RB)
        self.assertEqual(branches, RB)

    def test_critical_bugfix_is_general(self):
        branches = select(["pr-critical-bugfix"], RB)
        self.assertEqual(branches, RB)

    def test_general_label_wins_over_version_label(self):
        # A general label already covers every branch, so adding a
        # version-specific label alongside it changes nothing.
        branches = select(["pr-must-backport", "v26.3-must-backport"], RB)
        self.assertEqual(branches, RB)

    def test_force_fans_out_to_all_branches(self):
        branches = select(["pr-must-backport-force"], RB)
        self.assertEqual(branches, RB)
