...
def _run_standalone(monkeypatch, tmp_path, kv, version, file_lines):
    """Run the check against a cwd holding only the two files it reads.
    ...
    (tmp_path / "cmake" / "autogenerated_versions.txt").write_text(
        f"SET(VERSION_MAJOR {major})\nSET(VERSION_MINOR {minor})\n", encoding="utf-8"
    )
...
def test_renaming_a_record_in_place_is_allowed(monkeypatch, tmp_path):
    ...
    # alias rows). The new name is recorded under the current version block, so the check is
    # satisfied by it alone.
    patch, file_lines = _rename_patch("legacy_name", "renamed_setting", block_version="26.8")
    changed = parse_settings_history_changes(patch, file_lines)
    assert changed == [{"namespace": "Session", "name": "renamed_setting"}]
    assert (
        _run_standalone(
            monkeypatch,
            tmp_path,
            _kv([HISTORY, "src/Core/Settings.cpp"], changed),
            "26.8",
            file_lines,
        )
        == ""
