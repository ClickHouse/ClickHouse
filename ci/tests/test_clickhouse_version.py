"""
Tests for `ci.jobs.scripts.clickhouse_version.ClickHouseVersion.get_current_version_as_dict`.
...
def _patch(monkeypatch, pr_number, commit_count="123"):
    monkeypatch.setattr(
        ClickHouseVersion, "get_release_version_as_dict", classmethod(lambda cls: dict(_RELEASE))
    )
    monkeypatch.setattr(
...
def test_pr_tweak_stable_across_reopen(monkeypatch):
    ...
    v1 = (
        ...
        ClickHouseVersion.get_current_version_as_dict()["string"],
    )[1]
    v2 = (
