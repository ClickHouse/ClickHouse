...
def test_push_uses_app_token_without_verbose_logging(monkeypatch):
    ...
    assert "gh pr edit 123" in checks[1][0]
    assert "--add-label pr-autogenerated-docs" in checks[1][0]
...
def test_pr_create_targets_upstream_repository(monkeypatch):
    ...
    assert "--label pr-autogenerated-docs" in checks[1]


def test_generated_paths_include_localized_navigation():
    for locale in cloud_api_docs_nightly.LOCALE_DIRS:
        assert (
            f"docs/{locale}/products/cloud/api-reference/navigation.json"
            in cloud_api_docs_nightly.DOCS_PATHS
        )
