...
def _versions_content(patch=2):
    """``_VERSIONS_CONTENT`` retargeted at ``26.6.<patch>.1``."""
    return _VERSIONS_CONTENT.replace(
        "SET(VERSION_PATCH 2)", f"SET(VERSION_PATCH {patch})"
    ).replace("26.6.2.1", f"26.6.{patch}.1")


def _build_patch_release_repo(tmp_path, patch=2, prev_tag="v26.6.1.1-stable"):
    ...
    ``prev_tag`` release tag, the version githash pointed at it so two real
    commits make the tip ``26.6.<patch>.2`` (not the refused empty tweak==1
    case), and the real ``ci/`` + ``tests/`` trees symlinked in. Returns
    ``(repo, script)``. Shared by the create-guard tests, which differ only in
    their `gh` stub; ``patch``/``prev_tag`` let a test model the branch's first
    release, whose previous tag is the non-user-facing ``v26.6.1.1-new`` marker.
    """
    repo = tmp_path / "repo"
    repo.mkdir()
    ...
    (repo / "cmake").mkdir()
    (repo / _VERSIONS_FILE).write_text(_versions_content(patch), encoding="utf-8")
    (repo / "src" / "Storages" / "System").mkdir(parents=True)
    (repo / _CONTRIBUTORS_FILE).write_text(
        "const char * auto_contributors[] {\n    nullptr};\n", encoding="utf-8"
    )
    git("add", "-A")
    git("commit", "-q", "-m", "Base release commit")
    git("tag", "-a", prev_tag, "-m", f"Release {prev_tag}")
    prev = subprocess.run(
        ["git", "rev-parse", "HEAD"],
        cwd=repo,
        check=True,
    ...
    # (two here) — a non-empty patch that reaches the create path.
    (repo / _VERSIONS_FILE).write_text(
        _versions_content(patch).replace("0" * 40, prev), encoding="utf-8"
    )
    git("add", "-A")
    git("commit", "-q", "-m", "Point version githash at previous release")
    (repo / "README.md").write_text("clickhouse\n", encoding="utf-8")
...
def _build_new_release_repo(tmp_path):
    """Build a synthetic ``master`` in the state that mints the next new release.

    Master carries the post-cut version file (``26.6.1.1``, type ``testing``) and
    the ``v26.6.1.1-new`` tag pushed when the *previous* release branch was cut —
    the previous tag ``prepare(release_type="new")`` expects. Its tip therefore
    cuts branch ``26.6`` and mints ``v26.7.1.1-new``. Returns ``(repo, script)``.
    """
    repo = tmp_path / "repo"
    repo.mkdir()
    ...
    versions = _versions_content(patch=1).replace("-stable", "-testing")
    (repo / "cmake").mkdir()
    ...
    git("add", "-A")
    git("commit", "-q", "-m", "Update autogenerated version to 26.6.1.1")
    git("tag", "-a", "v26.6.1.1-new", "-m", "Release v26.6.1.1-new")
    cut = _head_sha(repo)
    (repo / _VERSIONS_FILE).write_text(
        versions.replace("0" * 40, cut), encoding="utf-8"
    )
    git("add", "-A")
    git("commit", "-q", "-m", "Point version githash at the previous branch cut")
    (repo / "README.md").write_text("clickhouse\n", encoding="utf-8")
    git("add", "-A")
    git("commit", "-q", "-m", "Development commit")
    git("remote", "add", "origin", str(repo))
    git("fetch", "-q", "origin")
...
def _run_prepare_with_gh_stub(
    tmp_path, repo, script, gh_stub_body, ref="26.6", release_type="patch"
):
    """Run ``--prepare-release-info`` for ``ref`` with a custom `gh` stub."""
    bindir = tmp_path / "bin"
    bindir.mkdir()
    gh_stub = bindir / "gh"
...
#   * matching-refs lists the surviving `auto/v26.6.2.1-stable` head branch (and
#     no release tag at the branch's patch, so the version-bump guard passes);
...
a = " ".join(sys.argv[1:])
if "matching-refs/heads/auto/" in a:
...
# `gh` stub whose `matching-refs/tags/` listing is {tags} — the release tags that
# already exist at the patch the branch is about to release. Everything else
# (head-branch listings, is_latest_release_branch) returns `[]`.
_TAGS_GH_STUB = """\
#!/usr/bin/env python3
import sys

a = " ".join(sys.argv[1:])
if "matching-refs/tags/" in a:
    print('{tags}')
else:
    print("[]")
"""
...
# `gh` stub reporting a surviving `bump_version_26.6.1.1` head branch whose PR is
# in state {state}: `pr list --head <that> --state {state}` returns a PR and
# everything else returns `[]`, so get_pr_state_by_branch reports {state}.
_BUMP_PR_GH_STUB = """\
#!/usr/bin/env python3
import sys

a = " ".join(sys.argv[1:])
if "matching-refs/heads/bump_version_" in a:
    print('["refs/heads/bump_version_26.6.1.1"]')
elif "pr list" in a and "--head bump_version_26.6.1.1" in a and "--state {state}" in a:
    print('[{{"url": "https://github.com/test/clickhouse/pull/888"}}]')
else:
    print("[]")
"""
...
def test_prepare_refuses_when_the_previous_release_version_is_unbumped(tmp_path):
    """Creating the next patch is refused while an earlier release's bump is missing.

    Each release is followed by a version bump that increments the patch on the
    branch, so a published tag at the patch the branch still describes means that
    bump never landed. Here ``v26.6.2.1-stable`` is already published while the
    branch tip still mints ``26.6.2.2``, so ``prepare`` must fail closed instead
    of minting a second release at patch 2 that supersedes the first — exactly
    what happened when ``v26.6.2.160`` stranded ``v26.6.2.158``.
    """
    pytest.importorskip("boto3")  # create_release.py imports s3_helper -> boto3
    ...
    result = _run_prepare_with_gh_stub(
        ...
        repo,
        script,
        _TAGS_GH_STUB.format(tags='["refs/tags/v26.6.2.1-stable"]'),
    )
    assert result.returncode != 0, "unbumped previous release should have failed"
    out = result.stdout + result.stderr
    assert "v26.6.2.1-stable" in out, out
    assert "version bump never landed" in out, out


def test_prepare_ignores_non_release_tags_when_checking_the_version_bump(tmp_path):
    """Tags at the branch's patch that are not releases on it do not block create.

    ``v{major}.{minor}.1.1-new`` marks the master commit a release branch was cut
    from, and the retired ``-testing`` / ``-prestable`` tags were never releases
    on a branch either — yet both share the ``v26.6.1.`` prefix of the branch's
    own first release (``v26.6.1.1-new`` alongside ``v26.6.1.1193-stable`` in the
    real repository). Counting them would refuse every first release of a freshly
    cut branch, so the version-bump guard looks only at ``stable`` / ``lts`` tags
    and ``prepare`` creates ``26.6.1.2`` normally.
    """
    pytest.importorskip("boto3")  # create_release.py imports s3_helper -> boto3

    repo, script = _build_patch_release_repo(
        tmp_path, patch=1, prev_tag="v26.6.1.1-new"
    )
    result = _run_prepare_with_gh_stub(
        tmp_path,
        repo,
        script,
        _TAGS_GH_STUB.format(
            tags='["refs/tags/v26.6.1.1-new", "refs/tags/v26.6.1.9-testing"]'
        ),
    )
    assert result.returncode == 0, (
        f"non-release tags must not block the branch's first release\n"
        f"--- stdout ---\n{result.stdout}\n--- stderr ---\n{result.stderr}"
    )
    with open("/tmp/release_info.json", encoding="utf-8") as f:
        info = json.load(f)
    assert info["release_tag"] == "v26.6.1.2-stable"
    assert info["create_new_release"] is True


def test_prepare_refuses_new_release_while_a_bump_version_pr_is_open(tmp_path):
    """Cutting a new release is refused while an earlier bump PR is still open.

    A new release bumps master's version through a ``bump_version_<version>`` PR
    whose head branch is deleted on merge, so a surviving branch with an OPEN PR
    means master was never bumped for that release. Cutting the next release
    leaves that PR unmergeable — its version file would downgrade master — so
    ``prepare`` fails closed with the PR to finalize instead of the bare
    latest-tag mismatch that this state also produces.
    """
    pytest.importorskip("boto3")  # create_release.py imports s3_helper -> boto3

    repo, script = _build_new_release_repo(tmp_path)
    result = _run_prepare_with_gh_stub(
        tmp_path,
        repo,
        script,
        _BUMP_PR_GH_STUB.format(state="open"),
        ref="master",
        release_type="new",
    )
    assert result.returncode != 0, "open version bump PR should have failed"
    out = result.stdout + result.stderr
    assert "pull/888" in out, out
    assert "version bump PR" in out, out


def test_prepare_creates_new_release_when_the_bump_version_pr_is_merged(tmp_path):
    """A merged bump PR whose head branch survived is not an orphan.

    Head-branch deletion is a repository setting, not a guarantee, so the guard
    confirms the PR's state rather than the branch's existence: with the
    ``bump_version_26.6.1.1`` PR MERGED, master's version was bumped and
    ``prepare`` cuts branch ``26.6`` with ``v26.7.1.1-new`` as it should.
    """
    pytest.importorskip("boto3")  # create_release.py imports s3_helper -> boto3
    ...
    result = _run_prepare_with_gh_stub(
        ...
        repo,
        script,
        _BUMP_PR_GH_STUB.format(state="merged"),
        ref="master",
        release_type="new",
    )
    assert result.returncode == 0, (
        f"a merged bump PR must not block a new release\n"
        f"--- stdout ---\n{result.stdout}\n--- stderr ---\n{result.stderr}"
    )
    with open("/tmp/release_info.json", encoding="utf-8") as f:
        info = json.load(f)
    assert info["release_tag"] == "v26.7.1.1-new"
    assert info["release_branch"] == "26.6"
    assert info["create_new_release"] is True
