...
must keep:
  ...
  * a full patch release drives end to end in ``--dry-run``, and
    ``ReleaseInfo.prepare`` picks the right create/recover/out-of-order path.
...
``s3_helper``); the dry-run / ``prepare`` checks run the script as a subprocess
inside a synthetic git repo, and skip where that dependency is absent. The
shared version arithmetic in ``ci/jobs/scripts/clickhouse_version.py`` is
covered by ``test_clickhouse_version.py``; the empty-release guard in
``ci/jobs/scripts/release_checks.py`` by ``test_release_checks.py``.
"""
...
def _seed_release_anchor(repo, git):
    """Commit a seed that the version-file githash points at, so the strict tweak
    (the first-parent commit count since that githash) resolves to 1 for the
    release commit committed right after. Returns the seed sha to inject as the
    githash. Real release branches always carry a real githash; the version model
    now recomputes the tweak from it rather than trusting a placeholder."""
    (repo / _VERSIONS_FILE).write_text(_VERSIONS_CONTENT, encoding="utf-8")
    git("add", "-A")
    git("commit", "-q", "-m", "Seed commit (previous release anchor)")
    return _head(repo)
...
def test_prepare_recovers_from_tag(tmp_path):
    ...
    seed = _seed_release_anchor(repo, git)
    (repo / _VERSIONS_FILE).write_text(
        _VERSIONS_CONTENT.replace(_ZERO_GITHASH, seed), encoding="utf-8"
    )
    git("add", "-A")
    git("commit", "-q", "-m", "Base release commit")
...
def test_prepare_refuses_out_of_order_commit(tmp_path):
    ...
    later_versions = (
        ...
        .replace(_ZERO_GITHASH, commit_sha)
    )
...
def test_prepare_fails_closed_on_stale_branch_version_file(tmp_path):
    ...
    # version file still says 26.6.2.1 (its githash re-anchored at the base
    # release so the strict tweak recomputes to 1), so prepare will compute the
    # already-used tag v26.6.2.1-stable at a different (tip) commit.
    (repo / _VERSIONS_FILE).write_text(
        _VERSIONS_CONTENT.replace(_ZERO_GITHASH, base), encoding="utf-8"
    )
    (repo / "README.md").write_text("clickhouse\n", encoding="utf-8")
    git("add", "-A")
