"""
Tests for `ci.jobs.scripts.functional_tests_results.FTResultsProcessor`, the
job-side parser that turns `clickhouse-test` output + its exit code into the
report tree.

Two concerns are covered:

* The CIDB-staging-cluster overload heuristic added on @alexey-milovidov's
  request in PR #106154: when the test runner is killed by the harness
  wall-clock timeout but the only evidence in the server's `err.log` is
  repeated `Distributed`-shipping retries to an unresponsive CIDB staging log
  cluster, the run must not be painted red as `Server died` - the ClickHouse
  server was healthy. See ClickHouse/ClickHouse#106154.

* A run stopped early by `--max-failures` / `--max-failures-chain` (exit code
  `MAX_FAILURES_EXIT_CODE`) must be reported as real failures plus a "Too many
  test failures" leaf - NOT as "Server died" with the per-test attribution
  demoted to UNKNOWN (which is what the aborted-run exit codes do).
"""
