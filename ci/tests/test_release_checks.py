...
`pytest ci/tests/test_release_checks.py` from the repo root.

`release_checks` is the dependency-light home of `is_empty_patch_release` (used
by the CreateRelease driver `ci/jobs/create_release.py`'s `ReleaseInfo.prepare`).
It is kept free of the release toolchain's heavy imports (`github`, `boto3`,
`unidiff`, ...) and of the `ci.praktika` stack, so this test imports it as a
plain `ci.jobs.scripts` module without the packages that the `CI Tests` docker
image does not ship.
"""
