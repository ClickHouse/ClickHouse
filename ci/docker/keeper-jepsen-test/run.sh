...
with_auth=$(($RANDOM % 2))
use_bg_thread_for_snapshot_io=$(($RANDOM % 2))

(lein run keeper test-all --nodes-file "$NODES_FILE_PATH" --username "$NODES_USERNAME" --logging-json --password "$NODES_PASSWORD" --time-limit "$TIME_LIMIT" --concurrency 50 -r 50 --snapshot-distance 10000 --stale-log-gap 100 --reserved-log-items 10 --lightweight-run --clickhouse-source "$CLICKHOUSE_PACKAGE" -q --test-count "$TESTS_TO_RUN" --with-auth "$with_auth" --use-bg-thread-for-snapshot-io "$use_bg_thread_for_snapshot_io" || true) | tee "$TEST_OUTPUT/jepsen_run_all_tests.log"
