...
class S3:
    ...
    def upload_asset_streaming(cls, local_path: Path, s3_path: str):
        ...
        if use_gzip:
            cmd = f"gzip -8c {local_path} | aws s3 cp - s3://{s3_path} --content-type {content_type} --content-encoding gzip"
        else:
            cmd = f"aws s3 cp {local_path} s3://{s3_path} --content-type {content_type}"
