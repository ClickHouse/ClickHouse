...
class PullRequestPushYamlGen:
    ...
    def generate(self):
        ...
        if self.workflow_config.event in (
            ...
            concurrency = ""
            if self.workflow_config.config.concurrency_group:
                concurrency = YamlGenerator.Templates.TEMPLATE_CONCURRENCY.format(
                    GROUP=self.workflow_config.config.concurrency_group
                )
            ...
            if self.workflow_config.config.enable_workflow_call:
                ...
                for input_item in self.workflow_config.dispatch_inputs:
                    if input_item.is_boolean:
                        wc_inputs += (
                            YamlGenerator.Templates.TEMPLATE_BOOLEAN_INPUT.format(
                                NAME=input_item.name,
                                DESCRIPTION=input_item.description,
                                DEFAULT_VALUE=input_item.default_value or "false",
                            )
