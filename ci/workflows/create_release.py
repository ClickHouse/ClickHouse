...
# PR check: rehearse both release flows in --dry-run on the small PR pool (no
# release-maker creds, no robot PAT — gh reads use the minted PR token); it
# publishes nothing but keeps every validation, so the recovery-ref guard still
# drives prepare() through its states.
_release_dry_run_digest = Job.CacheDigestConfig(
    include_paths=[
        ...
        "./ci/workflows/create_release_branch.py",
        "./ci/jobs/release_job.py",
        "./ci/jobs/release_branch_job.py",
...
# "new" cuts from master (its vX.Y.1.1-new tag is the state prepare() expects) via release_branch_job.py; "patch" (--ref auto) rehearses a fresh patch and the artifact-download path; "recovery" (--ref recovery-auto) re-publishes a tagged release with --skip-repo --skip-docker, the one mode those flags are valid.
_RELEASE_DRY_RUN_POSITIVE = [
    _dry_run_job(
        "Release Dry Run (new)",
        "./ci/jobs/release_branch_job.py --ref master --dry-run",
    ),
    _dry_run_job(
        "Release Dry Run (patch)",
        "./ci/jobs/release_job.py --ref auto --dry-run",
    ),
    _dry_run_job(
        "Release Dry Run (recovery)",
        "./ci/jobs/release_job.py --ref recovery-auto"
