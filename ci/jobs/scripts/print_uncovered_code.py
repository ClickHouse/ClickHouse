...
if __name__ == "__main__":
    ...
    # Use the stable baseline for LBC: union of all extracted baseline slices.
    # This eliminates false LBC from lines covered in *any* master run.
    # baseline_N.changed.info files are extracted by generate_diff_coverage_report.sh.
    _baseline_slices = [
        f"{temp_dir}/baseline_{i}.changed.info"
        for i in range(2, 7)
        if os.path.exists(f"{temp_dir}/baseline_{i}.changed.info")
        and os.path.getsize(f"{temp_dir}/baseline_{i}.changed.info") > 0
    ]
    if _baseline_slices:
        import subprocess as _sp
        _a_args = " ".join(f"-a '{f}'" for f in [f"{temp_dir}/baseline.changed.info"] + _baseline_slices)
        _stable_path = f"{temp_dir}/stable_baseline.changed.info"
        _r = _sp.run(
            f"lcov {_a_args} --ignore-errors inconsistent,corrupt,unsupported -o '{_stable_path}'",
            shell=True, capture_output=True
        )
        if _r.returncode == 0 and os.path.exists(_stable_path) and os.path.getsize(_stable_path) > 0:
            BASELINE_INFO = _stable_path
            print(f"Using stable LBC baseline (union of {1 + len(_baseline_slices)} master runs)")
        else:
