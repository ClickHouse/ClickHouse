...
def main():
    ...
    # The repo disables `enablePullRequestAutoMerge` (what `gh pr merge --auto`
    # calls) in favor of a merge queue on `master`, so enabling auto-merge means
    # adding the PR to the queue via the `enqueuePullRequest` mutation.
    try:
        Git.enable_automerge(pr_number, "ClickHouse/ClickHouse")
