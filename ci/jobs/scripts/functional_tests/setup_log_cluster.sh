...
function setup_logs_replication()
{
    ...
    clickhouse-client ${NO_AST_FUZZER:-} --query "SELECT name FROM system.tables WHERE database = 'system' AND name LIKE '%\\_log' AND engine LIKE '%MergeTree'" | while read -r table
    do
