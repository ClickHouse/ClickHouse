...
class CIAgent:
    ...
    def extract_autogenerated_content(self, filename: str) -> str:
        ...
        try:
            ...
            if file_content.strip().startswith("API Error: "):
                raise RuntimeError(f"Claude API Error detected in {filename}: {file_content.strip()}")
    ...
    def extract_content_and_format_flag(self, body: str, tag_prefix: str) -> Tuple[str, bool]:
    ...
    def insert_content_between_tags(self, text: str, tag_prefix: str, new_content: str) -> str:
        """
    ...
    def generate_description(self, diff_file: str, human_description: str) -> str:
        try:
            ...
            command = f"cat prompt_description.txt | claude --model '{self.model}' --output-format text --allowedTools 'Bash,Read' --permission-mode acceptEdits -p 'Generate PR description' > pr_description.txt"
            Shell.check(command)
    ...
    def generate_changelog_entry(self, diff_file: str, human_changelog_entry: str) -> str:
        try:
            ...
            command = f"cat prompt_changelog.txt | claude --model '{self.model}' --output-format text --allowedTools 'Bash,Read' --permission-mode acceptEdits -p 'Generate changelog entry' > changelog_entry.txt"
            Shell.check(command)
    ...
    def format_description_or_changelog(self, type: str, content: str):
        if type == "description":
            try:
                ...
                command = f"cat prompt_user_description.txt | claude --model '{self.model}' --output-format text --allowedTools 'Bash,Read' --permission-mode acceptEdits -p 'format' > formatted_user_description.txt"
                Shell.check(command)
                changelog = self.extract_autogenerated_content("formatted_user_description.txt")
                return changelog
            except Exception as e:
                print(f"Error running claude command: {e}")
            ...
            try:
                ...
                command = f"cat prompt_user_changelog.txt | claude --model '{self.model}' --output-format text --allowedTools 'Bash,Read' --permission-mode acceptEdits -p 'format' > formatted_user_changelog.txt"
                Shell.check(command)
                changelog = self.extract_autogenerated_content("formatted_user_changelog.txt")
                return changelog
            except Exception as e:
                print(f"Error running claude command: {e}")
                return f"Error formatting user description: {e}"
    ...
    def update_pr_body_with_generated_content(self, pr_body: str, description: str, changelog_entry: str) -> str:
    ...
    def should_process_section(self, tag_prefix: str) -> Tuple[bool, Tuple[bool, str]]:
        content, should_format = self.extract_content_and_format_flag(self.info.pr_body, tag_prefix)
