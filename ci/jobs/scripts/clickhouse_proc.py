...
class ProcessIdentityUnknown(Exception):
    """Raised when it cannot be established what a pid is, or whether it exists.

    Distinct from "no such process", which is an answer rather than a failure.
    """
...
class ClickHouseProc:
    ...
    # exercised independently of the host in tests.
    HAS_PROC = Path("/proc/self/cmdline").exists()
    # Wall-clock cap on one `ps` call (seconds).
    PS_TIMEOUT = 60
...
"""
    ...
    @classmethod
    def _ps_field(cls, pid, keyword) -> str:
        """One `ps -o <keyword>=` field of `pid`, or "" if there is no such process.

        `-ww` keeps `ps` from truncating its output to the terminal width, which
        is what makes `/proc` the better source on Linux.
        """
        try:
            res = subprocess.run(
                ["ps", "-ww", "-o", f"{keyword}=", "-p", str(pid)],
                capture_output=True,
                text=True,
                timeout=cls.PS_TIMEOUT,
            )
        except (OSError, subprocess.SubprocessError) as e:
            raise ProcessIdentityUnknown(f"`ps` failed for process {pid}: {e}")
        if res.returncode != 0:
            # `ps` exits non-zero with no output when no process matches, which
            # is the answer here and not an error. Anything on stderr is.
            if res.stderr.strip():
                raise ProcessIdentityUnknown(
                    f"`ps` failed for process {pid}: {res.stderr.strip()}"
                )
            return ""
        return res.stdout.strip()

    @classmethod
    def _process_argv0(cls, pid):
        """`argv[0]` of `pid`, or None if there is no such process.

        Raises `ProcessIdentityUnknown` if it cannot be determined either way.
        """
        if cls.HAS_PROC:
            try:
                with open(f"/proc/{pid}/cmdline", "rb") as cmdline:
                    return os.fsdecode(cmdline.read().split(b"\0", 1)[0])
            except OSError:
                # No such pid, or it is gone between the check and the read. A
                # zombie reads back as an empty command line, which matches no
                # server: it holds no locks and its pid cannot be recycled yet.
                return None
        # No `/proc` (the macOS runners). A zombie prints its command in
        # parentheses here, so it matches no server either - same outcome.
        command = cls._ps_field(pid, "command")
        if not command:
            return None
    ...
    def _pid_exists(pid) -> bool:
        try:
            ...
            # EPERM: it exists, we just do not own it.
            return True
        return True
    ...
    def _server_process_alive(cls, pid, unknown_alive=True) -> bool:
        ...
        When the identity cannot be established at all, the pid is reported
        alive if it exists. That direction is deliberate: a server wrongly
        believed gone is never stopped, and an orphan holding this job's stdout
        pipe wedges `praktika run` forever - on the ephemeral macOS runners that
