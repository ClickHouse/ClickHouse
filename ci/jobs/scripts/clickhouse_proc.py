...
"""
    ...
    def start(self):
        ...
        if self.is_db_replicated or self.is_shared_catalog:
            Shell.check(
                f"rm -rf {self.run_path1} {self.run_path2} && mkdir -p {self.run_path1} {self.run_path2}",
                verbose=True,
                strict=True,
            )
        print(f"Starting clickhouse, command: {self.command}")
        ...
        if self.is_db_replicated:
            res = self.start_replica(1) and self.start_replica(2)

        if res:
            print("create minio log webhooks for both audit and server logs")
    ...
    def wait_ready(self):
        ...
        for attempt in range(attempts):
            ...
            if self.proc.poll() is not None:
                print("Server not ready, process is dead")
                Shell.check(
                    f"cat {self.log_dir}/clickhouse-server.err.log", verbose=True
                )
                return False
        else:
            Utils.print_formatted_error(
...
"""
    ...
    def terminate(self):
        ...
        for proc, pid_file in (
            ...
            if proc:
                if not Shell.check(
                    f"clickhouse stop --pid-path {pid_file}", verbose=True
                ):
    ...
    def check_fatal_messeges_in_logs(self):
        ...
        if Shell.check("dmesg > {self.DMESG_LOG}"):
            results.append(
                Result.from_commands_run(
                    name="OOM in dmesg",
...
""".format(
    ...
    def dump_system_tables(self):
        ...
        for table in TABLES:
            ...
            #     if not res:
            #         print(
            #             f"ERROR: Failed to dump system table: {table}: err: {err} out: {out}."
            #         )
            #         self.extra_tests_results.append(
            #             Result(name=f"Scraping {table}.1", status="FAIL", info=err)
            #         )
            #         res = False
            ...
            #     if not res:
            #         print(
            #             f"ERROR: Failed to dump system table from replica: {table}: err: {err} out: {out}."
            #         )
            #         self.extra_tests_results.append(
            #             Result(name=f"Scraping {table}.2", status="FAIL", info=err)
            #         )
            #         res = False
            # if self.is_shared_catalog:
            #     path_arg = f" --path {self.run_path1}"
            #     res, out, err = Shell.get_res_stdout_stderr(
            #         f"clickhouse local {command_args} {path_arg} --query \"select * from system.{table} into outfile '{temp_dir}/system_tables/{table}.1.tsv' format TSVWithNamesAndTypes\" {command_args_post}"
            #     )
            #     if not res:
            #         print(
            #             f"ERROR: Failed to dump system table from shared catalog: {table}: err: {err} out: {out}"
            #         )
            #         self.extra_tests_results.append(
            #             Result(name=f"Scraping {table} (SC)", status="FAIL", info=err)
            #         )
            #         res = False
        Shell.check(
            f"clickhouse local {command_args} {path_arg} -q \"SELECT log FROM minio_audit_logs ORDER BY log.time INTO OUTFILE '{temp_dir}/system_tables/minio_audit_logs.jsonl.zst' FORMAT JSONEachRow\"",
            verbose=True,
        )
        Shell.check(
            f"clickhouse local {command_args} {path_arg} -q \"SELECT log FROM minio_server_logs ORDER BY log.time INTO OUTFILE '{temp_dir}/system_tables/minio_server_logs.jsonl.zst' FORMAT JSONEachRow\"",
            verbose=True,
        )
