...
embedded_doc_files = [
    ...
    # Format settings live here as `DECLARE(...)` macros with embedded description
    # strings; the autogenerated docs are produced from them. The `Settings.cpp`
    # suffix rule below only covers settings declared in `*Settings.cpp` (e.g. the
    # main `Settings.cpp`), so this header has to be listed explicitly.
