...
def belongs_to_autogenerated_category(filename):
    ...
    try:
        ...
        category_pattern = r'FunctionDocumentation::Category::(' + '|'.join(autogenerated_function_categories) + r')'
        matches = re.findall(category_pattern, content)

        if matches:
            print(f"Found autogenerated function categories in {filename}: {set(matches)}")
            return True
        else:
...
def check_docs():
    ...
    if Labels.PR_FEATURE in info.pr_labels:
        ...
        has_source_code_docs_registered = any(
            belongs_to_autogenerated_category(file) for file in changed_files if file.endswith('.cpp')
        )

        if not has_doc_changes and not has_source_code_docs_registered:
            print("No changes in docs for new feature and no autogenerated function categories found")
            return False
    return True
