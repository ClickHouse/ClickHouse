...
def _has_source_and_docs_changes(changed_files):
    normalized = [file.removeprefix(".").removeprefix("/") for file in changed_files]
    return any(file.startswith("src/") for file in normalized) and any(
        file.startswith("docs/") for file in normalized
    )
...
def should_skip_job(job_name):
    ...
    # Source regeneration needs the PR binary, so run it only when a PR changes
    # both source and documentation. A docs-only PR is fully covered by the
    # lightweight Mintlify job and must not pull in Build (arm_binary).
    if (
        job_name == JobNames.DOCS_MINTLIFY_AUTOGENERATED
        and not _has_source_and_docs_changes(changed_files)
    ):
        return True, "Skipped, source regeneration requires both src/ and docs/ changes"
