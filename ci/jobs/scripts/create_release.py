...
class ReleaseInfo:
    ...
    def get_release_version_bump_branch(self):
        # The release-branch version bump lands through a PR into the release
        # branch (the branch is protected by a ruleset that requires pull
        # requests), so it needs its own head branch. `release_tag` is unique
    ...
    def update_version_and_contributors_list(self, dry_run: bool) -> None:
        ...
        with checkout(self.release_branch):
            ...
            if files_changed:
                ...
                Git.push(
                    GITHUB_REPOSITORY,
                    f"HEAD:refs/heads/{bump_branch}",
                    force=True,  # rerun-safe: overwrite a branch left by a prior attempt
                    dry_run=dry_run,
                    strict=True,
                    retries=3,  # transient workflow-scope timeout (see push_release_tag)
                )
                # Rerun-safe: reuse an existing PR instead of failing on a
                # duplicate `gh pr create`.
                bump_pr = (
                    "dry-run"
                    if dry_run
                ...
                if bump_pr:
                    print(
                        f"Release version-bump PR already exists [{bump_pr}]"
                        " - skipping create"
                    )
                else:
                    Shell.check(
                        f"gh pr create --repo {GITHUB_REPOSITORY} "
                        f"--title 'Update autogenerated version to {self.version} on {self.release_branch}' "
                        f"--head {bump_branch} --base {self.release_branch} "
                        "--body 'Automatic version update. Part of the ClickHouse "
                        "release cycle, used by the release CI only.'",
                        strict=True,
                        dry_run=dry_run,
                        verbose=True,
                    )
