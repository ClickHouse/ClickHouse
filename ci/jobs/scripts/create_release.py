...
def push_with_pat(
    ...
    Kept local to `create_release` rather than in `ci.praktika.git`, whose
    `Git.push` intentionally keeps the App token for the non-release
    (`native_jobs`) push path.
    """
    repo_url = (
    ...
    return Shell.check(
        push_cmd, dry_run=dry_run, strict=strict, verbose=True, retries=retries
    )
