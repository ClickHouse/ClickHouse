...
class ReleaseInfo:
    ...
    def update_version_and_contributors_list(self, dry_run: bool) -> None:
        ...
        if self.release_type == "new":
            ...
            # Push the post-release bump straight to master. master moves, so
            # retry: re-fetch its tip and rebuild the commit each round. Skip once
            # master already describes this bump — compare the version, not the
            # file, since VERSION_GITHASH is recomputed from the moving tip.
            with checkout("master"):
                for _ in range(5):
                    if not dry_run:
                        ...
                        if not CHVersion.get_current_version().is_older(version):
                            print(
                                f"master already at version [{version.string}]"
                                " — nothing to push"
                            )
                            break
                    ...
                    if Git.push(
                        ...
                        break
                    print(
                        "Push to master rejected (tip moved?) — re-fetching and retrying"
                    )
                else:
                    raise RuntimeError(
                        "Failed to push version bump to master after retries"
                    )
            ...
            with checkout(self.release_branch):
                ...
                # Rerun-safe: a previous attempt may have already opened the
                # release PR for this branch. Only open it if one does not already
                # exist, so a rerun reuses it instead of failing on a duplicate
                # `gh pr create`.
                existing_release_pr = (
                    "" if dry_run
