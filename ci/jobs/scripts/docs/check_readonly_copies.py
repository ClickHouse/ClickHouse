...
folders. The aggregate docs job bypasses this check only for trusted sync PRs
carrying the ``pr-autogenerated-docs`` label, using the client-sync branch
declared for that source, and originating in this repository rather than a
fork. The job still checks other protected folders and generated regions.
...
def check_readonly_copies(
    changed_files, config_path=CONFIG_PATH, allowed_sync_branch=None
):
    """Return True if no read-only copied docs were edited, else False."""
    sources = load_sources(config_path)
    violations = find_violations(changed_files, sources)
    if allowed_sync_branch:
        violations = [
            (source, files)
            for source, files in violations
            if source.get("sync_branch") != allowed_sync_branch
        ]
    if not violations:
