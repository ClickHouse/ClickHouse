...
def main():
    ...
    for source in (
        "@site/src/components/SettingsInfoBlock",
        "/snippets/components/SettingsInfoBlock/SettingsInfoBlock.jsx",
    ):
        migrated_component, _ = migrate.transform_imports(
            f"import SettingsInfoBlock from '{source}';\n",
            migrate.Lookups(),
            [],
        )
        assert migrated_component == (
    ...
    for component in (
        ...
        import_line = (
            f'import {component} from '
            f'"/snippets/components/{component}/{component}.jsx";'
        )
        assert mod._component_imports_for_page(
            import_line, f"<{component}/>"
        ) == [
            f'import {{ {component} }} from '
            f'"/snippets/components/{component}/{component}.jsx";'
        ]
    ...
    for family_name in ("server-settings", "mergetree-settings", "format-settings"):
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            assert f'export const {family["component_name"]} = () => {{' in explorer
            component_start = explorer.index(
                f'export const {family["component_name"]} = () => {{')
            assert component_start == 0
            assert explorer.index(
                "  const [entries] = useState(() => ("
            ) > component_start
