...
class ClickHouseVersion:
    ...
    release version (see `from_repo`). Its class methods read and
    ...
    @classmethod
    def from_repo(cls, git: Optional[Git] = None) -> "ClickHouseVersion":
        """Read the version from `cmake/autogenerated_versions.txt`.

        When `git` is given, the tweak is derived from the release tags (the
        commit count since the latest tag, or since the `-new` branch-cut tag
        when the current version is still ahead of it).
        """
        v = _read_versions()
        version = cls(v["major"], v["minor"], v["patch"], v["revision"])
    ...
    def get_current_version_as_dict(cls):
        ...
        Unlike `from_repo` (which derives the tweak from git tags
