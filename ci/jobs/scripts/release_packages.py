...
def is_new_ci(release: str) -> bool:
    """Whether `release` uses the new (praktika) CI layout, which decides both
    the S3 prefix and the build-job dir names."""
    major, minor = (int(x) for x in release.split(".")[:2])
    return (major >= 25 and minor >= 3) or major > 25


def s3_release_prefix(release: str) -> str:
    """The S3 key prefix release artifacts live under for this branch."""
    return f"REFs/{release}" if is_new_ci(release) else release
...
def build_job_name(release: str, package_arch: str) -> str:
    """CI job dir that holds the deb/rpm/tgz packages for this arch."""
    if is_new_ci(release):
        return f"build_{package_arch}_release"
    return "package_release" if package_arch == "amd" else "package_aarch64"


def darwin_job_name(release: str, package_arch: str) -> str:
    """CI job dir that holds the macOS binary for this arch."""
    if is_new_ci(release):
        return f"build_{package_arch}_darwin"
    return "binary_darwin" if package_arch == "amd" else "binary_darwin_aarch64"


def iter_package_objects(release: str, version: str):
    ...
    for package_arch in PACKAGE_ARCHS:
        job = build_job_name(release, package_arch)
...
def iter_macos_objects(release: str):
    """Yield `(package_arch, job_name)` for each per-arch macOS build. The S3
    object basename is always `MACOS_S3_OBJECT`."""
    for package_arch in PACKAGE_ARCHS:
        yield package_arch, darwin_job_name(release, package_arch)


def expected_s3_objects(release: str, version: str):
    ...
    for _repo_type, filename, job in iter_package_objects(release, version):
        by_job.setdefault(job, set()).add(filename)
    for _package_arch, job in iter_macos_objects(release):
...
def release_build_artifacts_ready(
    ...
    for job, expected_files in expected_s3_objects(release, version).items():
