...
PATTERN="allow_";
DIFF=$(comm -3 <(grep -v OBSOLETE $ROOT_PATH/src/Core/Settings.cpp | grep -o "\b$PATTERN\w*\b"  | sort -u) <(grep -o -h "\b$PATTERN\w*\b" $ROOT_PATH/src/Databases/enableAllExperimentalSettings.cpp $ROOT_PATH/ci/jobs/scripts/check_style/experimental_settings_ignore.txt | sort -u));
