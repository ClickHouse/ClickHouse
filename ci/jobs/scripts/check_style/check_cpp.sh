...
find "$ROOT_PATH"/{src,programs,utils} -name '*.h' -or -name '*.cpp' |
    grep -vP "$EXCLUDE" |
    xargs grep -P '(hardware_destructive_interference_size|hardware_constructive_interference_size)' | grep -vP ':\s*//' && echo "Use CACHE_LINE_SIZE from Common/CacheLine.h instead"
