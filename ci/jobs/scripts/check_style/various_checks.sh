...
# - 04307, 04338, 04339: the SYSTEM DROP text appears only inside SQL string literals passed to
...
tests_with_system_drop=( $(
    find $ROOT_PATH/tests/queries -iname '*.sql' -or -iname '*.sh' -or -iname '*.py' -or -iname '*.j2' |
        ...
        grep -vP '04307_ast_json_roundtrip_lossless|04338_ast_json_parser_impossible_field_combinations|04339_ast_json_review_followup_hardening' |
        sort -u
) )
