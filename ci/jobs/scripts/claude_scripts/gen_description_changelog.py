...
def generate_description(pr_diff: str, human_description: str, model: str) -> str:
    try: 
        ...
        command = f"cat prompt_description.txt | claude --model '{model}' -p 'Generate PR description and save to pr_description.txt'"
        Shell.check(command)
...
def generate_changelog_entry(pr_diff: str, human_changelog_entry: str, model:str) -> str:
    try:
        ...
        command = f"cat prompt_changelog.txt | claude --model '{model}' -p 'Generate changelog entry and save to changelog_entry.txt'"
        Shell.check(command)
