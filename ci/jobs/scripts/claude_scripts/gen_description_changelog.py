...
def get_human_description_changelog(pr_body: str) -> Tuple[str, str]:
    ...
    for i, line in enumerate(lines):
        ...
        if category_header is None and re.search(
            r"(?i)\*\*\s*changelog\s+category.*\*\*", line
        ):
            category_header = i
    ...
    if changelog_entry_header is not None:
        changelog_entry_end = (
            documentation_entry_header
            if documentation_entry_header is not None
            else len(lines)
        )
...
def generate_description(pr_diff: str, human_description: str, model: str) -> str:
    try:
        ...
        with open("prompt_description.txt", "w") as f:
            f.write(prompt)

        command = f"cat prompt_description.txt | claude --model '{model}' --output-format text -p 'Generate PR description' > pr_description.txt"
        Shell.check(command)
...
def generate_changelog_entry(
    ...
    try:
        ...
        # Write prompt to file and pipe it to claude to avoid shell escaping issues
        with open("prompt_changelog.txt", "w") as f:
            f.write(prompt)

        command = f"cat prompt_changelog.txt | claude --model '{model}' --output-format text -p 'Generate changelog entry' > changelog_entry.txt"
        Shell.check(command)
...
if __name__ == "__main__":
    ...
    human_description, human_changelog_entry = get_human_description_changelog(
        info.pr_body
    )
    ...
    try:
        if not GH.post_updateable_comment(
            comment_tags_and_bodies={
                "llm_generated_description_changelog": f"{comment_body}"
            },
            only_update=True,
        ):
