...
autogeneration workflow. It regenerates settings, functions, SQL statements,
table and database engines, data types, formats, table functions, window
functions, dictionary sources and layouts, system tables, and asynchronous
metrics from the structured documentation embedded in the ClickHouse source and
exposed through the corresponding `system.*` tables.
...
def open_or_refresh_pr():
    ...
    generated_paths = " ".join(
        shlex.quote(path) for path in GENERATED_PATHS
    )
    if not Shell.get_output(
        f"git status --porcelain -- {generated_paths}"
    ).strip():
