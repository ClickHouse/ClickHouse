...
from ci.jobs.docs_autogen_nightly import regenerate as regenerate_source_docs
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Regenerate source-owned reference docs before validation",
        action="store_true",
    )
...
if __name__ == "__main__":
    ...
    results = []
    if args.regenerate_source_docs:
        results.append(
            Result.from_commands_run(
                name="Regenerate source-owned documentation",
                command=regenerate_source_docs,
            )
        )

    if not results or results[-1].is_ok():
        # The mint check definitions are shared with the standalone driver
        # (ci/jobs/scripts/docs/mintlify_docs_check.py). This job already runs
        # inside the docs-builder image with the docs present natively, so it
        # runs them directly; add new checks to DEFAULT_CHECKS, not here.
        # --test selects a subset by sub-check name.
        results.extend(
        ...
        # a localized snippet, or a checker. This is how a GT translation PR
        # that reintroduces broken/unlocalized locale links gets caught before
        # merge.
        ...
        # Quickstarts check: blocking, but only when the PR touches the
        # quickstarts trees or their tooling. Validates frontmatter tags and
        # fails when the generated explorer data was not regenerated and
        # committed.
        ...
        # These guards run from the repo root (not the docs root), so keep them
        # out of the docs_dir loop above.
