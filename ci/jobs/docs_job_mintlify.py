...
from ci.jobs.scripts.docs.check_autogenerated_regions import check_pr
...
def _autogenerated_regions_guard():
    ...
    #
    # The changed lines come from the PR file diffs via the GitHub API, not from
    # local git: the CI checkout is shallow and has no `origin/master` to diff
    # against. The paginated pulls/files API also scales past the ~300-file limit
    # that makes `gh pr diff` return HTTP 406 on large docs PRs.
    ...
    return check_pr(Info().repo_name, Info().pr_number, read_head)
