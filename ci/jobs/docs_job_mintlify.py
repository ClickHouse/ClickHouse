...
WORKFLOW_AUTHENTICATION_PUBLIC_COMMIT_AUTHOR = (
    "212800359+workflow-authentication-public[bot]@users.noreply.github.com"
)
TRUSTED_DOCS_SYNCS = {
    "robot/docs-sync-clickhouse-connect": {
        ...
        "commit_authors": (WORKFLOW_AUTHENTICATION_PUBLIC_COMMIT_AUTHOR,),
    },
...
def _trusted_docs_sync(info):
    ...
    # actor that triggered the current head update, and to a fully trusted set
    # of commit author emails. The commit author check prevents a human commit
    # from being mixed into an otherwise bot-triggered sync PR.
    if not (
    ...
    commit_author_emails = set(info.commit_author_emails)
    trusted_commit_authors = set(trusted_sync["commit_authors"])
    if not commit_author_emails or not commit_author_emails <= trusted_commit_authors:
        return None
    return trusted_sync
