...
SYSTEM_TABLE_DOCS_BINARY = "ci/tmp/clickhouse"
...
def _system_table_pages_are_current():
    if not Shell.check(f"chmod +x {SYSTEM_TABLE_DOCS_BINARY}", verbose=True):
        return False
    if not Shell.check(
        "python3 utils/generate-system-tables-docs"
        f" --binary {SYSTEM_TABLE_DOCS_BINARY}"
        " --docs-dir docs/reference/system-tables",
        verbose=True,
    ):
        return False
    return Shell.check(
        "git diff --exit-code -- docs/reference/system-tables", verbose=True
    )
...
if __name__ == "__main__":
    ...
    if selected("Check generated system-table pages are current"):
        results.append(
            Result.from_commands_run(
                name="Check generated system-table pages are current",
                command=_system_table_pages_are_current,
            )
        )
