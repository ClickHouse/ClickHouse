...
# Client repositories force-push their one-way docs copies to stable branches
# under this prefix. The nightly source-doc generator uses robot/docs-autogen,
# so branch provenance keeps its broader label from bypassing copy ownership.
CLIENT_DOCS_SYNC_BRANCH_PREFIX = "robot/docs-sync-"
...
def _protected_docs_guard():
    ...
    autogenerated = "pr-autogenerated-docs" in info.pr_labels
    client_docs_sync = (
        autogenerated
        and info.git_branch.startswith(CLIENT_DOCS_SYNC_BRANCH_PREFIX)
        and info.fork_name == info.repo_name
    )
    if client_docs_sync:
        print("OK: trusted client docs sync provenance verified.")
        readonly_copies_ok = check_readonly_copies(
            changed_files, allowed_sync_branch=info.git_branch
        )
    else:
        readonly_copies_ok = check_readonly_copies(changed_files)

    if autogenerated and not client_docs_sync:
