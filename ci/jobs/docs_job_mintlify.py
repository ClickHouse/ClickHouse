...
from ci.jobs.scripts.docs.check_autogenerated_regions import check_from_pr_diff
...
# The label the docs-autogen bot adds to its own regeneration pull requests to
# bypass the autogenerated-region guard below. A human can add it too if they
# genuinely need to hand-edit a region, but that is an escape hatch, not a path
# anyone reaches by accident.
AUTOGEN_EXEMPT_LABEL = "pr-autogenerated-docs"


def _autogenerated_regions_guard():
    # Fail edits to the content between the AUTOGENERATED markers -- that region
    # is generated from the ClickHouse source and overwritten on every run, so a
    # hand-edit is lost at the next regeneration. Adding/removing pages, editing
    # frontmatter, and editing prose outside the markers all stay allowed.
    #
    # The changed lines come from the PR diff via the GitHub API (`gh pr diff`),
    # not from local git: the CI checkout is shallow and has no `origin/master`
    # to diff against.
    if AUTOGEN_EXEMPT_LABEL in (Info().pr_labels or []):
        print(f"'{AUTOGEN_EXEMPT_LABEL}' label present; skipping the check.")
        return True
    if not Info().pr_number:
        print("Not a pull request; nothing to guard.")
        return True
    diff = GH.get_pr_diff()
    if not diff:
        # Fail close: without the diff we cannot prove the pull request does not
        # touch a generated region, so do not report success.
        print("Error: could not obtain the PR diff, cannot run the check.")
        return False
    repo_root = Utils.cwd()
    ...
    return check_from_pr_diff(diff, read_head)
...
if __name__ == "__main__":
    ...
    # Also repo-root relative (it diffs with git), so keep it out of the
    # docs_dir loop as well.
    if selected("No direct edits to autogenerated regions"):
        results.append(
            Result.from_commands_run(
                name="No direct edits to autogenerated regions",
                command=_autogenerated_regions_guard,
            )
        )
