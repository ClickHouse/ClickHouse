...
def _autogenerated_regions_guard():
    ...
    changed_files = Info().get_changed_files()
    if changed_files is None:
        # Fail close: without the changed-file list we cannot prove the pull
        # request does not touch a generated region, so do not report success.
        print("Error: the changed-file list is unavailable, cannot run the check.")
        return False
    try:
        readers = git_readers(Info().base_branch)
    except RuntimeError as e:
        # Fail close: without the base ref we cannot compute what changed.
        print(f"Error: {e}")
        return False
    return check_autogenerated_regions(changed_files, *readers)
