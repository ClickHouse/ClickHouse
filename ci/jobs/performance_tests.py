...
# Schema for ClickBench hits table
hits_create_query = """
...
def main():
    ...
    clickbench_requested = bool(test_keyword) and "clickbench" in test_keyword.lower()
    ...
    if res and JobStages.DOWNLOAD_DATASETS in stages:
        ...
        if clickbench_requested:
            # Download ClickBench parquet files (100 files) for hits table
            user_files_path = f"{db_path}/user_files"
            if not Path(f"{user_files_path}/.done").is_file():
                Shell.check(f"mkdir -p {user_files_path}", verbose=True)
                parquet_cmds = [
                    f"wget -q --continue https://datasets.clickhouse.com/hits_compatible/athena_partitioned/hits_{i}.parquet -O {user_files_path}/hits_{i}.parquet"
                    for i in range(100)
                ]
                res = Shell.check_parallel(parquet_cmds, verbose=True)
                results.append(
                    Result(
                        name="Download ClickBench parquet",
                        status=Result.Status.SUCCESS if res else Result.Status.ERROR,
                    )
                )
                if res:
                    Shell.check(f"touch {user_files_path}/.done")
    ...
    if res and JobStages.CONFIGURE in stages:
        ...
        def restart_ch():
            ...
            if not res_:
                return res_
            # Create test.hits table and ingest ClickBench data from parquet files
            if clickbench_requested:
                Shell.check(
                    f"clickhouse-client --port {leftCH.port} --multiquery",
                    verbose=True,
                    strict=True,
                    stdin_str=hits_create_query,
                )
                Shell.check(
                    f"clickhouse-client --port {leftCH.port}",
                    verbose=True,
                    strict=True,
                    stdin_str="INSERT INTO default.hits SELECT * FROM file('hits_*.parquet')",
                )
            leftCH.terminate()
    ...
    if res and JobStages.TEST in stages:
        ...
        if test_keyword:
            ...
            # Do not include clickbench in normal performance tests – it is handled separately
            test_files = [file for file in test_files if file != "clickbench.xml"]
