    ...
    if res and JobStages.REPORT in stages:
        ...
        if Path(f"{perf_wd}/ci-checks.tsv").is_file():
            ...
            with open(f"{perf_wd}/ci-checks.tsv", "r", encoding="utf-8") as f:
                # compare.sh writes this file last, so a failure there (e.g. a full
                # disk) can leave it empty or without the type line. Read both header
                # lines defensively: an unguarded next() would raise StopIteration out
                # of main() and kill the job before its artifacts are uploaded.
                header_lines = list(islice(f, 2))  # column names, then column types
                if len(header_lines) < 2:
                    print(
                        "WARNING: ci-checks.tsv is empty or truncated - skipping test case import"
                    )
                else:
