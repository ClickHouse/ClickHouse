...
def main():
    ...
    test_env = {
        ...
        "PYTHONHASHSEED": "0",  # Ensure deterministic test collection
    }
    test_results = []
    files = []
    ...
    if args.test:
        ...
        if parallel_test_modules:
            test_result_parallel = Result.from_pytest_run(
                command=f"{' '.join(sorted(parallel_test_modules))} --report-log-exclude-logs-on-passed-tests -n {workers} --dist=loadfile --tb=short {repeat_option}",
                cwd="./tests/integration/",
                env=test_env,
        ...
        if sequential_test_modules and fail_num < MAX_FAILS_BEFORE_DROP:
            test_result_sequential = Result.from_pytest_run(
                command=f"{' '.join(sorted(sequential_test_modules))} --report-log-exclude-logs-on-passed-tests --tb=short {repeat_option}",
                cwd="./tests/integration/",
                env=test_env,
