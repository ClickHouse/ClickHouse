...
def get_run_commands_glibc(build_path: Path, result_directory: Path) -> List[str]:
    return [
        f"readelf -s --wide {build_path}/usr/bin/clickhouse | grep '@GLIBC_' > {result_directory}/glibc.log",
        # FIXME: odbc bridge is not present in the deb package
        # f"readelf -s --wide {build_path}/usr/bin/clickhouse-odbc-bridge | grep '@GLIBC_' >> {result_directory}/glibc.log",
        # FIXME: library bridge is not present in the deb package
        # f"readelf -s --wide {build_path}/usr/bin/clickhouse-library-bridge | grep '@GLIBC_' >> {result_directory}/glibc.log",
    ]
