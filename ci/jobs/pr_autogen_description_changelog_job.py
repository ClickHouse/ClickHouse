...
if __name__ == "__main__":
    ...
    results.append(
        Result.from_commands_run(
            name=testname,
            command=Shell.check("gh auth status", verbose=False),
        )
    )
    ...
    should_process_description, description_params = agent.should_process_section(info.pr_body, "BEGIN_DESCRIPTION")
    should_process_changelog, changelog_params = agent.should_process_section(info.pr_body, "BEGIN_CHANGELOG_ENTRY")
