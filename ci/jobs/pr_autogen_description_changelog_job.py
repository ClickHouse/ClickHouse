...
if __name__ == "__main__":
    ...
    os.environ["AWS_REGION"] = "us-east-2" # Inference optimized
    os.environ["CLAUDE_CODE_MAX_OUTPUT_TOKENS"] = "4096"
    os.environ["ANTHROPIC_MODEL"] = "us.anthropic.claude-3-7-sonnet-20250219-v1:0"
    os.environ["ANTHROPIC_DEFAULT_HAIKU_MODEL"] = "us.anthropic.claude-3-5-haiku-20241022-v1:0"
    ...
    if "API Error: " in description_content:
        results.append(Result("PR description check", Result.Status.FAILED, f"Cached API error detected in PR description: {description_content}"))
        Result.create_from(results=results).complete_job()
        sys.exit(1)
        
    if "API Error: " in changelog_content:
        results.append(Result("PR changelog check", Result.Status.FAILED, f"Cached API error detected in PR changelog: {changelog_content}"))  
        Result.create_from(results=results).complete_job()
        sys.exit(1)
    ...
    if should_process_description:
        ...
        results.append(Result(testname, Result.Status.SKIPPED, "Skipping PR description formatting / autogeneration. Either no tags were found, or the tags have body text but format=false"))
    ...
    if should_process_changelog:
        ...
        results.append(Result(testname, Result.Status.SKIPPED, "Skipping PR changelog entry formatting / autogeneration. Either no tags were found, or the tags have body text but format=false"))
    ...
    if should_process_description or should_process_changelog:
        ...
        results.append(Result(testname, Result.Status.SKIPPED, "No conditions were detected for automatically generating or formatting a changelog entry or a PR description"))
