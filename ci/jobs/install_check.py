...
def test_install_tgz(image: DockerImage, debug_symbols: bool) -> List[Result]:
    ...
    # A tgz package is installed by `doinst.sh`, which copies the unpacked tree into the
    # system instead of moving it, so the debug symbols are written twice and take more
    # than a minute. The same tarballs are installed in both images, so unpack them only
    # in the first one - see `main`.
    server_packages = (
