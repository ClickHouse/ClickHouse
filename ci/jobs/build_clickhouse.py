...
def main():
    ...
    # The standalone keeper is a separate clickhouse-keeper binary that needs its
    # own ThinLTO link, roughly doubling release link time. Build it only for
    # official builds (master, pr_number = 0) and private builds (which deploy to
    # staging from PRs); public PR release builds use the multi-call keeper.
    if (is_private or info.pr_number == 0) and "ENABLE_THINLTO=1" in cmake_cmd:
        cmake_cmd += " -DBUILD_STANDALONE_KEEPER=1"
