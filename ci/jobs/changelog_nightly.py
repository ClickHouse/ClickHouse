...
def prepare_repo():
    ...
    # Only the commit graph is needed: `--filter=tree:0` skips the trees and
    # blobs of the whole history (the same way praktika unshallows the
    # repository for its own jobs), and the few old file versions the job
    # does read are fetched lazily from the promisor remote. Fetching the
    # contents as well took ~35 minutes of every run on the CI runners - more
    # than half of the one-hour lifetime of the GitHub App token.
    ...
    if Shell.get_output("git rev-parse --is-shallow-repository").strip() == "true":
        Shell.check(
            f"git fetch --unshallow --no-recurse-submodules --filter=tree:0 "
            f"origin {master_refspec}",
            verbose=True,
            strict=True,
        )
    ...
    Shell.check(
        f"git fetch --tags --no-recurse-submodules --filter=tree:0 "
        f"origin {master_refspec}",
        verbose=True,
        strict=True,
    )
