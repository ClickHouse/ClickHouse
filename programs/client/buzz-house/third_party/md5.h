...
{
private:
    static const constexpr uint32_t S[]
	...
	*/
    static const constexpr uint8_t PADDING[] = {
...
*/
    inline uint32_t rotateLeft(const uint32_t x, const uint32_t n) const { return (x << n) | (x >> (32 - n)); }

    /*
 * Bit-manipulation functions defined by the MD5 algorithm
 */
    inline uint32_t md5f(const uint32_t x, const uint32_t y, const uint32_t z) const { return ((x & y) | (~x & z)); }

    inline uint32_t md5g(const uint32_t x, const uint32_t y, const uint32_t z) const { return ((x & z) | (y & ~z)); }

    inline uint32_t md5h(const uint32_t x, const uint32_t y, const uint32_t z) const { return (x ^ y ^ z); }

    inline uint32_t md5i(const uint32_t x, const uint32_t y, const uint32_t z) const { return (y ^ (x | ~z)); }
        ...
        for (unsigned int i = 0; i < input_len; ++i)
        {
            ...
            if (offset % 64 == 0)
            {
                for (unsigned int j = 0; j < 16; ++j)
                {
                    ...
                    input[j] = static_cast<uint32_t>(ninput[(j * 4) + 3]) << 24 | static_cast<uint32_t>(ninput[(j * 4) + 2]) << 16
                        | static_cast<uint32_t>(ninput[(j * 4) + 1]) << 8 | static_cast<uint32_t>(ninput[(j * 4)]);
                }
