...
/// Also declared in ClientBase.cpp under `USE_FUZZING_MODE`.
...
/// Executed by the runner thread before the first input (fixture schema, data, settings).
String setup_queries_text;

/// `CLICKHOUSE_FUZZER_PRINT_QUERY_ERRORS=1`: print the error of every failed query to stderr
/// (one line per query; useful to see why fuzzed queries do not reach execution).
bool print_query_errors = false;
...
{
    return isMerge(argc, argv);
}

void initialize(const int * argc, char *** argv, const String & setup_queries)
{
    setup_queries_text = setup_queries;
    if (const char * value = getenv("CLICKHOUSE_FUZZER_PRINT_QUERY_ERRORS")) // NOLINT(concurrency-mt-unsafe)
        print_query_errors = std::string_view(value) == "1";
    ...
    for (int i = 1; i < *argc; ++i)
        if (ignore)
        {
            /// Single-dash arguments after `-ignore_remaining_args` belong to the fuzz target itself
            /// (e.g. `-max_ast_depth=N` of `json_ast_sql_execution_fuzzer`); `clickhouse local`
            /// options are always spelled with two dashes.
            std::string_view arg{(*argv)[i]};
...

    {
        ...
        /// The runner also executes the setup queries before it accepts inputs; a fixture with data
        /// takes tens of seconds on a loaded sanitizer build, so allow much longer in that case.
        const auto startup_timeout = std::chrono::seconds(setup_queries_text.empty() ? 30 : 900);
        if (!cv.wait_for(lock, startup_timeout, []{ return state == FuzzerState::WAITING_FOR_INPUT; }))
        {
            signalSafeWrite("[fuzzer] clickhouse local did not become ready in time\n");
            abort();
        }
    }
...
#pragma clang diagnostic pop
    ...
    if (!setup_queries_text.empty())
    {
        /// The fixture must be in place before any input runs; a failure here is fatal so that it is
        /// not mistaken for a fuzzing result.
        try
        {
            processQueryText(setup_queries_text);
        }
        catch (...)
        {
            signalSafeWrite("[fuzzer] The setup queries failed: ");
    ...
    {
        {
            ...
            try
            {
                ...
                {
                    const Exception & e = server_exception ? *server_exception : *client_exception;
                    std::cerr << "[fuzzer] query error: " << getExceptionMessage(e, false) << '\n';
                }
            }
            catch (const Exception & e)
            {
                /// Normal query errors (syntax, runtime, etc.) — ignore, same as
                /// interactive mode in runInteractive.
                if (print_query_errors)
                    std::cerr << "[fuzzer] query error: " << getExceptionMessage(e, false) << '\n';
            }
            catch (...)
            {
