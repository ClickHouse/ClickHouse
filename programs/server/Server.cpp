    ...
    bool start_servers)
{
    ...
    for (const auto & listen_host : listen_hosts)
    {
        ...
        /// Graphite (if defined and not setup yet with http_port)
        port_name = "graphite_carbon.port";
        createServer(config, listen_host, port_name, listen_try, start_servers, servers, [&](UInt16 port) -> ProtocolServerAdapter
        {
            Poco::Net::ServerSocket socket;
            auto address = socketBindListen(config, socket, listen_host, port);
            ...
            return ProtocolServerAdapter(
                listen_host,
                port_name,
                "Graphite: http://" + address.toString(),
                std::make_unique<HTTPServer>(
                    context(), createHandlerFactory(*this, async_metrics, "GraphiteHandler-factory"), server_pool, socket, http_params));
        });
