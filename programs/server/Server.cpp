...
void sanityChecks(Server & server, const ServerSettings & server_settings)
{
    ...
    try
    {
        if (!logs_path.empty() && fs::is_regular_file(logs_path))
        {
            ...
            if (!enoughSpaceInDirectory(pathToGenericString(logs_parent), 1ull << 30))
                server.context()->addOrUpdateWarningMessage(
                    Context::WarningType::AVAILABLE_DISK_SPACE_TOO_LOW_FOR_LOGS,
                    PreformattedMessage::create("Available disk space for logs at server startup is too low (1GiB): {}", pathToGenericString(logs_parent)));
        }
    }
    catch (const std::exception &) // NOLINT(bugprone-empty-catch)
    {
...
#undef COLLECT_FLAG
...

    {
        ...
        std::string dictionaries_lib_path = dictionaries_lib_path_setting.changed
            ? getCanonicalPath(String(dictionaries_lib_path_setting.value), path_str) : pathToGenericString(path / "dictionaries_lib/");
