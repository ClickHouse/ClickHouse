...
static void sanityChecks(Server & server)
{
    ...
    try
    {
        ...
        if (clocksource.find("tsc") == std::string::npos && clocksource.find("kvm-clock") == std::string::npos)
            server.context()->addWarningMessage(
                "Linux is not using a fast clock source. Performance can be degraded. Check " + String(filename));
    }
    catch (...)
    {
    }
    ...
    try
    {
        if (!logs_path.empty())
        {
            ...
            if (!enoughSpaceInDirectory(logs_parent, 1ull << 30))
                server.context()->addWarningMessage(
                    "Available disk space for logs at server startup is too low (1GiB): " + String(logs_parent));
        }
    }
    catch (...)
    {
...
try
{
    ...
    if (config().has("foundationdb"))
        global_context->initMetadataStoreFoundationDB(config(), "foundationdb");
    ...
    if (loaded_config.has_fdb)
    {
        ...
        if (meta_store->isFirstBoot())
        {
            ...
            config_processor.loadConfigWithFDB(loaded_config, meta_store, /* fdb_changed = */ true, /* fallback_to_preprocessed = */ true);
        config_processor.savePreprocessedConfig(loaded_config, config().getString("path", DBMS_DEFAULT_PATH));
        config().removeConfiguration(old_configuration.get());
        config().add(loaded_config.configuration.duplicate(), PRIO_DEFAULT, false);
    }
...
#    endif
    ...
    else if (!server_settings.temporary_data_in_cache.value.empty())
    {
        global_context->setTemporaryStorageInCache(
            server_settings.temporary_data_in_cache, server_settings.max_temporary_data_on_disk_size);
    }
    else
    {
    ...
    if (config().has("merge_tree_metadata_cache"))
    {
        ...
        try
        {
            LOG_DEBUG(
                log,
                "Initializing MergeTree metadata cache, lru_cache_size: {} continue_if_corrupted: {}",
                ReadableSize(size),
    ...
    auto main_config_reloader = std::make_unique<ConfigReloader>(
        ...
        [&](ConfigurationPtr config, XMLDocumentPtr xml_config, bool initial_loading)
        {
            ...
            if (server_settings_.concurrent_threads_soft_limit_num > 0
                && server_settings_.concurrent_threads_soft_limit_num < concurrent_threads_soft_limit)
            ...
            if (global_context->areBackgroundExecutorsInitialized())
            {
                ...
                global_context->getMergeMutateExecutor()->increaseThreadsAndMaxTasksCount(
                    new_pool_size, static_cast<size_t>(new_pool_size * new_ratio));
                global_context->getMergeMutateExecutor()->updateSchedulingPolicy(
                    server_settings_.background_merges_mutations_scheduling_policy.toString());
            }
            ...
            global_context->getMessageBrokerSchedulePool().increaseThreadsCount(
                server_settings_.background_message_broker_schedule_pool_size);
...
#    endif
    ...
    if (settings.async_insert_threads)
        global_context->setAsynchronousInsertQueue(
            std::make_shared<AsynchronousInsertQueue>(global_context, settings.async_insert_threads));
    ...
    SCOPE_EXIT({
        ...
        if (!servers_to_start_before_tables.empty())
        {
            ...
            if (current_connections)
                LOG_INFO(
                    log,
                    "Closed connections to servers for tables. But {} remain. Probably some tables of other users cannot finish their "
                    "connections after context shutdown.",
                    current_connections);
            else
    ...
    LOG_INFO(
        log,
        "Query Profiler disabled because they cannot work under sanitizers"
...

    {
        ...
        if (global_context->hasMetadataStoreFoundationDB())
        {
            global_context->getMetadataStoreFoundationDB()->markFirstBootCompleted();
            LOG_DEBUG(log, "Mark loaded on fdb");
        }
    ...
    bool & is_secure)
{
    auto create_factory = [&](const std::string & type, const std::string & conf_name) -> TCPServerConnectionFactory::Ptr
    {
        ...
        if (type == "prometheus")
            return TCPServerConnectionFactory::Ptr(new HTTPServerConnectionFactory(
                httpContext(), http_params, createHandlerFactory(*this, config, async_metrics, "PrometheusHandler-factory")));
        if (type == "interserver")
            return TCPServerConnectionFactory::Ptr(new HTTPServerConnectionFactory(
                httpContext(), http_params, createHandlerFactory(*this, config, async_metrics, "InterserverIOHTTPHandler-factory")));
    ...
    for (const auto & listen_host : listen_hosts)
    {
        ...
        createServer(
            ...
            [&](UInt16 port) -> ProtocolServerAdapter
            {
                ...
                UNUSED(port);
                throw Exception(
                    ErrorCodes::SUPPORT_IS_DISABLED, "HTTPS protocol is disabled because Poco library was built without NetSSL support.");
#endif
            });
    ...
    std::vector<ProtocolServerAdapter> & servers)
{
                ...
                while (true)
                {
                    ...
                    if (!pset.insert(conf_name).second)
                        throw Exception(
                            ...
                            "Protocol '{}' configuration contains a loop on '{}'",
                            protocol,
                            conf_name);
                }
            }
    ...
    createServers(
        config, listen_hosts, interserver_listen_hosts, listen_try, server_pool, async_metrics, servers, /* start_servers= */ true);
