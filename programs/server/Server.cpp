...
void sanityChecks(Server & server)
{
    ...
    if (server.logger().is(Poco::Message::PRIO_TEST))
        server.context()->addOrUpdateWarningMessage(Context::WarningType::SERVER_LOGGING_LEVEL_TEST, "Server logging level is set to 'test' and performance is degraded. This cannot be used in production.");
    ...
    try
    {
        ...
        if (!fast_clock_sources.contains(readLine(filename)))
            server.context()->addOrUpdateWarningMessage(Context::WarningType::LINUX_FAST_CLOCK_SOURCE_NOT_USED, "Linux is not using a fast clock source. Performance can be degraded. Check " + String(filename));
    }
    ...
    try
    {
        ...
        if (readNumber(filename) == 2)
            server.context()->addOrUpdateWarningMessage(Context::WarningType::LINUX_MEMORY_OVERCOMMIT_DISABLED, "Linux memory overcommit is disabled. Check " + String(filename));
    }
    catch (...) // NOLINT(bugprone-empty-catch)
    {
    }
    ...
    try
    {
        ...
        if (readLine(filename).find("[always]") != std::string::npos)
            server.context()->addOrUpdateWarningMessage(Context::WarningType::LINUX_TRANSPARENT_HUGEPAGES_SET_TO_ALWAYS, "Linux transparent hugepages are set to \"always\". Check " + String(filename));
    }
    catch (...) // NOLINT(bugprone-empty-catch)
    {
    }
    ...
    try
    {
        ...
        if (readNumber(filename) < 30000)
            server.context()->addOrUpdateWarningMessage(Context::WarningType::LINUX_MAX_PID_TOO_LOW, "Linux max PID is too low. Check " + String(filename));
    }
    catch (...) // NOLINT(bugprone-empty-catch)
    {
    }
    ...
    try
    {
        ...
        if (readNumber(filename) < 30000)
            server.context()->addOrUpdateWarningMessage(Context::WarningType::LINUX_MAX_THREADS_COUNT_TOO_LOW, "Linux threads max count is too low. Check " + String(filename));
    }
    catch (...) // NOLINT(bugprone-empty-catch)
    {
    }
    ...
    try
    {
        ...
        if (readNumber(filename) == 0)
            server.context()->addOrUpdateWarningMessage(Context::WarningType::DELAY_ACCOUNTING_DISABLED, "Delay accounting is not enabled, OSIOWaitMicroseconds will not be gathered. You can enable it using `echo 1 > " + String(filename) + "` or by using sysctl.");
    }
    catch (...) // NOLINT(bugprone-empty-catch)
    {
    }
    ...
    if (getBlockDeviceType(dev_id) == BlockDeviceType::ROT && getBlockDeviceReadAheadBytes(dev_id) == 0)
        server.context()->addOrUpdateWarningMessage(Context::WarningType::ROTATIONAL_DISK_WITH_DISABLED_READHEAD, "Rotational disk with disabled readahead is in use. Performance can be degraded. Used for data: " + String(data_path));
#endif

    try
    {
        if (getAvailableMemoryAmount() < (2l << 30))
            server.context()->addOrUpdateWarningMessage(Context::WarningType::AVAILABLE_MEMORY_TOO_LOW, "Available memory at server startup is too low (2GiB).");
    }
    catch (...) // NOLINT(bugprone-empty-catch)
    {
    }
    ...
    try
    {
        if (!enoughSpaceInDirectory(data_path, 1ull << 30))
            server.context()->addOrUpdateWarningMessage(Context::WarningType::AVAILABLE_DISK_SPACE_TOO_LOW_FOR_DATA,"Available disk space for data at server startup is too low (1GiB): " + String(data_path));
    }
    catch (...) // NOLINT(bugprone-empty-catch)
    {
    }
    ...
    try
    {
        if (!logs_path.empty())
        {
            ...
            if (!enoughSpaceInDirectory(logs_parent, 1ull << 30))
                server.context()->addOrUpdateWarningMessage(Context::WarningType::AVAILABLE_DISK_SPACE_TOO_LOW_FOR_LOGS, "Available disk space for logs at server startup is too low (1GiB): " + String(logs_parent));
        }
    }
    catch (...) // NOLINT(bugprone-empty-catch)
    {
    ...
    {
        server.context()->addOrUpdateWarningMessage(Context::WarningType::SETTING_ZERO_COPY_REPLICATION_ENABLED, "The setting 'allow_remote_fs_zero_copy_replication' is enabled for MergeTree tables."
    ...
    try
    {
        ...
        for (const auto & key : keys)
        {
            ...
            if (config.has(full_prefix + ".condition"))
            {
                ...
                {
                    if (result != "0\n" && result != "false\n")
                        context->addOrUpdateWarningMessage(Context::WarningType::SKIPPING_CONDITION_QUERY, fmt::format("The condition query returned `{}`, which can't be interpreted as a boolean (`0`, `false`, `1`, `true`). Will skip this query.", result));
...
try
{
    ...
    global_context->addOrUpdateWarningMessage(Context::WarningType::SERVER_BUILT_IN_DEBUG_MODE, "Server was built in debug mode. It will work slowly.");
#endif

    if (ThreadFuzzer::instance().isEffective())
        global_context->addOrUpdateWarningMessage(Context::WarningType::THREAD_FUZZER_IS_ENABLED, "ThreadFuzzer is enabled. Application will run slowly and unstable.");
    ...
    global_context->addOrUpdateWarningMessage(Context::WarningType::SERVER_BUILT_WITH_SANITIZERS, fmt::format("Server was built with {}. It will work slowly.", log_message));
#endif

#if defined(SANITIZE_COVERAGE) || WITH_COVERAGE
    global_context->addOrUpdateWarningMessage(Context::WarningType::SERVER_BUILT_WITH_COVERAGE, "Server was built with code coverage. It will work slowly.");
#endif
...
#undef COLLECT_FLAG
    ...
    assertProcessUserMatchesDataOwner(path_str, [&](const std::string & message){ global_context->addOrUpdateWarningMessage(Context::WarningType::PROCESS_USER_MATCHES_DATA_OWNER, message); });
        ...
        else
        {
            ...
            else
            {
                ...
                if (ptrace(PTRACE_TRACEME, 0, nullptr, nullptr) == -1)
                {
                    ...
                    global_context->addOrUpdateWarningMessage(Context::WarningType::SERVER_RUN_UNDER_DEBUGGER, fmt::format(
    ...
    /// Try to increase limit on number of threads.
    {
        ...
        {
            global_context->addOrUpdateWarningMessage(Context::WarningType::MAX_NUM_THREADS_LOWER_THAN_LIMIT, "Maximum number of threads is lower than 30000. There could be problems with handling a lot of simultaneous queries.");
        }
    }
