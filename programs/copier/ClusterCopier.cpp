        ...
        const size_t current_piece_number, bool is_unprioritized_task)
{
    ...
    if (!zookeeper->tryGet(partition_piece.getPartitionPieceCleanStartPath(), clean_start_status) || clean_start_status != "ok")
    {
        ...
        if (number_of_splits == 1)
            query_select_ast = get_select_query(split_table_for_current_piece, "count()", /*enable_splitting*/ false);
        else
            query_select_ast = get_select_query(split_table_for_current_piece, "count()", /*enable_splitting*/ true);
        UInt64 count;
        {
