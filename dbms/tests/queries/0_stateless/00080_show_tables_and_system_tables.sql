...
SELECT name, toUInt32(metadata_modification_time) > 0, engine_full, query_create_table FROM system.tables WHERE database = 'test_show_tables' ORDER BY name FORMAT TSVRaw;

DROP DATABASE test_show_tables;


-- Check that query_create_table works for system tables and unusual Databases
...
SELECT sum(ignore(*, metadata_modification_time, engine_full, query_create_table)) FROM system.tables;
