...
SELECT * FROM (SELECT id, event_date, priority, description FROM remote('127.0.0.{2,3}', default, report1) UNION ALL SELECT id, event_date, priority, description FROM remote('127.0.0.{2,3}', default, report2)) ORDER BY id, event_date ASC;
