DROP TABLE IF EXISTS users.perf;
CREATE TABLE users.perf (site String, user_id UInt64, z Float64) ENGINE = Log;

SELECT * FROM (SELECT perf_1.z AS z_1 FROM users.perf AS perf_1);
...
SELECT 
        ...
        FROM ( 
            ...
            FROM users.perf AS perf_1
            WHERE user_id = 000
        ) ALL INNER JOIN (
            SELECT perf_2.site, perf_2.z AS z_2
            FROM users.perf AS perf_2
            WHERE user_id = 999
