...
with client(name='client1>', log=log) as client1, client(name='client2>', log=log) as client2:
    ...
    client1.expect(prompt)
    client1.send("CREATE WINDOW VIEW test.wv TO test.dst WATERMARK INTERVAL '1' SECOND AS SELECT count(a) AS count FROM test.mt GROUP BY TUMBLE(timestamp, INTERVAL '1' SECOND) AS wid;")
    client1.expect(prompt)
    
    client1.send('WATCH test.wv LIMIT 1')
