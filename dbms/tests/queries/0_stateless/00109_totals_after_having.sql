...
SELECT intDiv(number, 2) AS k, count(), argMax(toString(number), number) FROM remote('localhost,127.0.0.{1,2}', test, numbers500k) GROUP BY k WITH TOTALS ORDER BY k LIMIT 10;
