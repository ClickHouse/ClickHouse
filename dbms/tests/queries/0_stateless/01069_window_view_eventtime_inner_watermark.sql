...
CREATE WINDOW VIEW test.wv TO test.dst ENGINE=MergeTree() order by tuple() WATERMARK=INTERVAL '5' SECOND AS SELECT count(a) AS count FROM test.mt GROUP BY TUMBLE(timestamp, INTERVAL '1' SECOND) AS wid;
