...
CREATE TABLE IF NOT EXISTS test_copy_partition_src (
    pk UInt8,
    val UInt32
) ENGINE ReplicatedMergeTree('/clickhouse/tables/test/test_copy_partition_replicated_to_replicated_src', '1') 
  PARTITION BY pk
  ORDER BY (pk, val);
...
CREATE TABLE IF NOT EXISTS test_copy_partition_dest (
    pk UInt8,
    val UInt32
) ENGINE ReplicatedMergeTree('/clickhouse/tables/test/test_copy_partition_replicated_to_replicated_dest', '1') 
  PARTITION BY pk
  ORDER BY (pk, val);
