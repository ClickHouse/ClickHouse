...
CREATE WINDOW VIEW test.wv TO test.dst WATERMARK=INTERVAL '2' SECOND ALLOWED_LATENESS=INTERVAL '2' SECOND AS SELECT count(a) AS count, TUMBLE_END(wid) AS w_end FROM test.mt GROUP BY TUMBLE(timestamp, INTERVAL '5' SECOND) AS wid;
...
INSERT INTO test.mt VALUES (1, '1990/01/01 12:00:12');
