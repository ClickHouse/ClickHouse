...
SELECT groupArray(dummy), count() FROM remote('localhost,127.0.0.{1,2}', system.one);

SELECT length(groupArray(toString(number))), count() FROM (SELECT number FROM system.numbers LIMIT 100000);
SELECT groupArray(toString(dummy)), count() FROM remote('localhost,127.0.0.{1,2}', system.one);
