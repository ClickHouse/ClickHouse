...
$expression_cast = {
    ...
    'function()' => sub { my ($state) = @_; return one_of($state, $functions) . '(' . list_of($state, {min => 0, max => 3}, $expression) . ')' },
    "'\\0'"      => "'\\0'",
    "''"         => "''",
    'NULL'       => 'NULL',
};
...
sub main {
    ...
    $functions = [ grep {not $_ ~~ '__inner_restore_projection__'} @$functions]; # will be removed
