...
our ($query, $expression_cast, $expression, $type, $type_cast, $functions, $table_functions);
...
$expression_cast = {
    ...
    'function()' => sub { my ($state) = @_; return one_of($state, $functions) . '(' . list_of($state, {max => 3}, $expression) . ')' },
    "'\\0'"      => "'\\0'",
    "''"         => "''",
    'NULL'       => 'NULL',
};
