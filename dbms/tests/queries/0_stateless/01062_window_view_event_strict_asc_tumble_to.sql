...
CREATE WINDOW VIEW test.wv TO test.dst WATERMARK=STRICTLY_ASCENDING AS SELECT count(a) AS count, TUMBLE_END(wid) as w_end FROM test.mt GROUP BY TUMBLE(timestamp, INTERVAL '5' SECOND) AS wid;
