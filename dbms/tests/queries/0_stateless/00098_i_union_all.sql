...
SELECT * FROM (SELECT id, event_date, priority, description FROM remote('localhost,127.0.0.{1,2}', test, report1) UNION ALL SELECT id, event_date, priority, description FROM remote('localhost,127.0.0.{1,2}', test, report2)) ORDER BY id, event_date ASC;
