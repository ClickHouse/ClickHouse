...
def test_rename(started_cluster):
    instance = cluster.instances['ch1']
    rules = cluster.pm_random_drops.pop_rules()
    ddl_check_query(instance, "CREATE TABLE rename_shard ON CLUSTER cluster (id Int64, sid String DEFAULT concat('old', toString(id))) ENGINE = ReplicatedMergeTree('/clickhouse/tables/{shard}/staging/test_shard', '{replica}') ORDER BY (id)")
    ddl_check_query(instance, "CREATE TABLE rename_new ON CLUSTER cluster AS rename_shard ENGINE = Distributed(cluster, default, rename_shard, id % 2)")
    ddl_check_query(instance, "RENAME TABLE rename_new TO rename ON CLUSTER cluster;")
    ...
    ddl_check_query(instance, "RENAME TABLE rename TO rename_old, rename_new TO rename ON CLUSTER cluster")
    ...
    for name in ['ch1', 'ch2', 'ch3', 'ch4']:
        cluster.instances[name].query("system sync replica rename_shard")
