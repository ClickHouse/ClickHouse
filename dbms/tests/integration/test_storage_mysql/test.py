...
def test_show_create_table(started_cluster):
    ...
    create_mysql_table(conn, table_name)
    node1.query(
        '''CREATE TABLE {}(id UInt32, name String, age UInt32, money UInt32) ENGINE = MySQL('mysql1:3306', 'clickhouse', '{}', 'root', 'clickhouse');'''.format(
            table_name, table_name))
    assert node1.query("SHOW CREATE TABLE {}".format(
        table_name)).rstrip() == '''CREATE TABLE default.{} (`id` UInt32, `name` String, `age` UInt32, `money` UInt32) ENGINE = MySQL(\\'mysql1:3306\\', \\'clickhouse\\', \\'{}\\', \\'root\\', ******)'''.format(
        table_name, table_name)
