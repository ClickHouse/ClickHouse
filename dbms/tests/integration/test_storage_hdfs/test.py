...
def test_globs_in_read_table(started_cluster):
    ...
    for pattern, value in test_requests:
        ...
        assert node1.query("select * from hdfs(" + inside_table_func + ")") == value * some_data
        assert node1.query("select count(distinct _path) from hdfs(" + inside_table_func + ")").rstrip() == str(value)
