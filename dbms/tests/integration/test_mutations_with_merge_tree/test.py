...
def started_cluster():
    try:
        ...
        instance_test_mutations.query('''INSERT INTO test_mutations_with_ast_elements SELECT '2019-07-29' AS date, 1, toString(number) FROM numbers(100);''')
    finally:
        cluster.shutdown()
...
def test_mutations_with_merge_background_task(started_cluster):
    ...
    all_done = False
    for index in range(100): # wait for replication 80 seconds max
        time.sleep(0.8)
...
def test_mutations_with_truncate_table(started_cluster):
    ...
    assert instance_test_mutations.query("SELECT COUNT() FROM system.mutations WHERE table = 'test_mutations_with_ast_elements'").rstriip() == '0'
