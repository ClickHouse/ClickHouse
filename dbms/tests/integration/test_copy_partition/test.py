...
def test_normal_work(normal_work):
    ...
    node1.query("ALTER TABLE test_table COPY PARTITION 201706 FROM real_table")
...
def test_drop_failover(drop_failover):
    ...
    with PartitionManager() as pm:
        ...
        node3.query("ALTER TABLE test_table COPY PARTITION 201706 FROM real_table")

        # Node3 replace is ok
        assert_eq_with_retry(node3, "SELECT id FROM test_table order by id", '111\n222')
        # Network interrupted -- copy is not ok, but it's ok
        assert_eq_with_retry(node4, "SELECT id FROM test_table order by id", '111')
...
def test_replace_after_replace_failover(replace_after_replace_failover):
    ...
    with PartitionManager() as pm:
        ...
        node5.query("ALTER TABLE test_table COPY PARTITION 201706 FROM real_table")
        ...
        node5.query("ALTER TABLE test_table COPY PARTITION 201706 FROM other_table")
