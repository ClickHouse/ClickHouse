...
CREATE TABLE source_table ENGINE = Log AS SELECT toUInt32(number) as id from numbers(2000000);
HEREDOC
...
DROP TABLE IF EXISTS dummy_queue ON CLUSTER sharded_cluster_secure_long_usernames;
DROP TABLE IF EXISTS dummy_local ON CLUSTER sharded_cluster_secure_long_usernames;
DROP TABLE IF EXISTS dummy ON CLUSTER sharded_cluster_secure_long_usernames;
DROP TABLE IF EXISTS dummy_mv ON CLUSTER sharded_cluster_secure_long_usernames;

CREATE TABLE dummy_queue ON CLUSTER sharded_cluster_secure_long_usernames (
    id UInt32
) ENGINE = Kafka('kafka1:9092,kafka2:9092,kafka3:9092', 'dummytopic', 'dummytopic_consumer_group1', 'JSONEachRow');

CREATE TABLE dummy_local ON CLUSTER sharded_cluster_secure_long_usernames (
    id UInt32
...
CREATE MATERIALIZED VIEW dummy_mv ON CLUSTER sharded_cluster_secure_long_usernames TO dummy AS SELECT id FROM dummy_queue;
HEREDOC
...
docker-compose exec -T kafka1 kafka-consumer-groups --bootstrap-server localhost:9092 --describe --group dummytopic_consumer_group1
