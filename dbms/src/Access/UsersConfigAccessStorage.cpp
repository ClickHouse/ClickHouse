...
namespace
{
    ...
    UUID generateID(const IAccessEntity & entity) { return generateID(entity.getType(), entity.name); }
#endif
}
...
{
    throwReadonlyCannotInsert(entity->getType(), entity->name);
}
...
{
    auto entity = read(id);
    throwReadonlyCannotRemove(entity->getType(), entity->name);
}
...
{
    auto entity = read(id);
    throwReadonlyCannotUpdate(entity->getType(), entity->name);
}
