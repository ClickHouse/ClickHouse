...
{
    const String & name = new_entity->name;
    ...
    if (it != entries.end())
    {
        ...
        throwIDCollisionCannotInsert(id, type, name, existing_entry.entity->getType(), existing_entry.entity->name);
    }
...
void MemoryAccessStorage::removeNoLock(const UUID & id, Notifications & notifications)
{
    ...
    Entry & entry = it->second;
    const String & name = entry.entity->name;
...
void MemoryAccessStorage::updateNoLock(const UUID & id, const UpdateFunc & update_func, Notifications & notifications)
{
    ...
    {
        auto it2 = names.find({new_entity->name, new_entity->getType()});
        if (it2 != names.end())
            throwNameCollisionCannotRename(old_entity->getType(), old_entity->name, new_entity->name);

        names.erase({old_entity->name, old_entity->getType()});
        names[std::pair{new_entity->name, new_entity->getType()}] = &entry;
    }
...
void MemoryAccessStorage::setAllNoLock(const std::vector<std::pair<UUID, AccessEntityPtr>> & all_entities, Notifications & notifications)
{
    ...
    for (const auto & [id, entity] : all_entities)
    {
        ...
        auto it2 = names.find({entity->name, entity->getType()});
        if (it2 != names.end())
        {
