...
void getNoSpaceLeftInfoMessage(std::filesystem::path path, std::string & msg)
{
    ...
    auto fs = DiskSpace::Disk::getStatVFS(path);
    ...
    auto mount_point = DiskSpace::Reservation::getMountPoint(path).string();
    msg += "\nMount point: " + mount_point;
#if defined(__linux__)
    msg += "\nFilesystem: " + DiskSpace::Reservation::getFilesystemName(mount_point);
#endif
}
