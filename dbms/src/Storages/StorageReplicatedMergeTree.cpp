...
bool StorageReplicatedMergeTree::executeLogEntry(const LogEntry & entry)
{
	...
	else if (entry.type == LogEntry::MERGE_PARTS)
	{
		...
		if (!do_fetch)
		{
			...
			MergeList::EntryPtr merge_entry = context.getMergeList().insert(database_name, table_name, entry.new_part_name, context);
...
bool StorageReplicatedMergeTree::optimize(const String & partition, bool final, const Settings & settings)
{
	...
	if (unreplicated_data)
	{
		...
		{
			MergeList::EntryPtr merge_entry = context.getMergeList().insert(database_name, table_name, merged_name, context);
