    ...
    const bool check_columns)
{
    ...
    for (const auto i : ext::range(0, parts.size()))
    {
        ...
        if (check_columns)
        {
            ...
            per_part_pre_columns.push_back(data.columns.getPhysical().addTypes(required_pre_column_names));
            per_part_columns.push_back(data.columns.getPhysical().addTypes(required_column_names));
        }
        else
        {
