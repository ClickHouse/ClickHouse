...
#include <DB/Storages/MergeTree/MergeTreeDataBlockOutputStream.h>
...
MergeTreeData::MergeTreeData(
	...
	: owning_storage(owning_storage_), path(path_), name(name_), full_path(path + escapeForFileName(name) + '/'), columns(columns_),
	context(context_), primary_expr_ast(primary_expr_ast_->clone()),
	date_column_name(date_column_name_), sampling_expression(sampling_expression_),
	index_granularity(index_granularity_),
	mode(mode_), sign_column(sign_column_),
	settings(settings_),
	increment(full_path + "increment.txt"), log(&Logger::get("MergeTreeData: " + name)), shutdown_called(false),
...
BlockOutputStreamPtr MergeTreeData::write(ASTPtr query)
{
	return new MergeTreeDataBlockOutputStream(*this, owning_storage);
}


BlockInputStreams MergeTreeData::read(
			...
			while (need_marks > 0 && !parts.empty())
			{
				...
				if (marks_in_part <= need_marks)
				{
					...
					streams.push_back(new MergeTreeBlockInputStream(
						...
						part.data_part, part.ranges, owning_storage, use_uncompressed_cache,
				...
				streams.push_back(new MergeTreeBlockInputStream(
					full_path + part.data_part->name + '/', max_block_size, column_names, *this,
					part.data_part, ranges_to_get_from_part, owning_storage, use_uncompressed_cache,
					prewhere_actions, prewhere_column));
			}
...
BlockInputStreams MergeTreeData::spreadMarkRangesAmongThreadsFinal(
	RangesInDataParts parts,
	size_t threads,
	const Names & column_names,
	size_t max_block_size,
...
void MergeTreeData::createPositiveSignCondition(ExpressionActionsPtr & out_expression, String & out_column)
{
...
void MergeTreeData::merge(size_t iterations, bool async, bool aggressive)
{
	...
	for (size_t i = 0; i < iterations; ++i)
		merge_threads->schedule(boost::bind(&MergeTreeData::mergeThread, this, while_can, aggressive));
...
void MergeTreeData::mergeThread(bool while_can, bool aggressive)
{
	try
	{
...
void MergeTreeData::joinMergeThreads()
{
...
bool MergeTreeData::selectPartsToMerge(Poco::SharedPtr<CurrentlyMergingPartsTagger> & what, bool merge_anything_for_old_months, bool aggressive)
{
...
void MergeTreeData::mergeParts(Poco::SharedPtr<CurrentlyMergingPartsTagger> & what)
{
	...
	MergeTreeData::DataPartPtr new_data_part = new DataPart(*this);
...
void MergeTreeData::rename(const String & new_path_to_db, const String & new_name)
{
	joinMergeThreads();
