    ...
    data_parts_by_state_and_info(data_parts_indexes.get<TagByStateAndInfo>())
{
    ...
    merging_params.check(getColumns().getPhysical());
...
void MergeTreeData::initPrimaryKey()
{
    ...
    primary_expr = ExpressionAnalyzer(primary_expr_ast, context, nullptr, getColumns().getPhysical()).getActions(false);

    {
        ExpressionActionsPtr projected_expr =
            ExpressionAnalyzer(primary_expr_ast, context, nullptr, getColumns().getPhysical()).getActions(true);
    ...
    if (secondary_sort_expr_ast)
    {
        ...
        secondary_sort_expr = ExpressionAnalyzer(secondary_sort_expr_ast, context, nullptr, getColumns().getPhysical()).getActions(false);

        ExpressionActionsPtr projected_expr =
            ExpressionAnalyzer(secondary_sort_expr_ast, context, nullptr, getColumns().getPhysical()).getActions(true);
...
void MergeTreeData::initPartitionKey()
{
    ...
    partition_expr = ExpressionAnalyzer(partition_expr_ast, context, nullptr, getColumns().getPhysical()).getActions(false);
...
void MergeTreeData::checkAlter(const AlterCommands & commands)
{
    ...
    createConvertExpression(nullptr, getColumns().getPhysical(), new_columns.getPhysical(), unused_expression, unused_map, unused_bool);
}
