...
namespace RemotePartChecker
{
    ...
    Service::Service(StoragePtr & storage_) : owned_storage{storage_}, data{static_cast<StorageReplicatedMergeTree &>(*storage_).getData()}
    {
    }
    ...
    Status Client::check(const std::string & part_name, const std::string & hash, const InterserverIOEndpointLocation & to_location)
    {
        Poco::URI uri;
        uri.setScheme("http");
        uri.setHost(to_location.host);
        uri.setPort(to_location.port);
        uri.setQueryParameters({{"endpoint", getEndpointId(to_location.name)}, {"compress", "false"}, {"part", part_name}, {"hash", hash}});
