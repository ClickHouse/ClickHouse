...
namespace
{
    ...
    public:
        StorageURLBlockInputStream(
            ...
            : name(name_)
        {
            read_buf = getBuffer<ReadWriteBufferFromHTTP>(
                compression_method, uri, method, callback, timeouts, context.getSettingsRef().max_http_get_redirects);
...
BlockInputStreams IStorageURLBase::read(
    const Names & column_names,
    const SelectQueryInfo & query_info,
    const Context & context,
    QueryProcessingStage::Enum processed_stage,
...
void registerStorageURL(StorageFactory & factory)
{
    factory.registerStorage("URL", [](const StorageFactory::Arguments & args) {
        ...
        return StorageURL::create(
            uri, args.database_name, args.table_name, format_name, args.columns, args.constraints, args.context, compression_method);
    });
}
}
