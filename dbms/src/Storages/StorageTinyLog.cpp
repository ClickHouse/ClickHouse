    ...
    log(&Logger::get("StorageTinyLog"))
{
    ...
    for (const auto & col : columns.getPhysical())
    ...
    const unsigned /*num_streams*/)
{
    ...
    return BlockInputStreams(1, std::make_shared<TinyLogBlockInputStream>(
        max_block_size, Nested::collect(columns.getPhysical().addTypes(column_names)), *this, context.getSettingsRef().max_read_buffer_size));
}
