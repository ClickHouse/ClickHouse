                                                    ...
                                                    bool concat_streams)
{
    ...
    if (!storage)
        return BlockInputStreams{
            InterpreterSelectQuery(modified_query_info.query, modified_context, std::make_shared<OneBlockInputStream>(header),
                                   SelectQueryOptions::analyze(processed_stage)).execute().in};
    ...
    else if (processed_stage > storage->getQueryProcessingStage(modified_context))
    {
        ...
        InterpreterSelectQuery interpreter{modified_query_info.query, modified_context, SelectQueryOptions::run(processed_stage)};
    ...
    switch (processed_stage)
    {
        ...
        case QueryProcessingStage::Complete:
            return materializeBlock(InterpreterSelectQuery(
                ...
                SelectQueryOptions::analyze(processed_stage)).getSampleBlock());
    }
