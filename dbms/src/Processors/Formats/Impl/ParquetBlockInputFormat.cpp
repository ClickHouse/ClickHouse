...
std::optional<Chunk> ParquetBlockInputFormat::generate()
{
    Chunk res;
    ...
    if (!in.eof())
    {
        ...
        if (row_group_current < row_group_total)
            throw Exception{"Got new data, but data from previous chunks was not read " + std::to_string(row_group_current) + "/"
