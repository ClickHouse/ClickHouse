...
std::optional<Chunk> ORCBlockInputFormat::generate()
{
    Chunk res;
    ...
    if (!in.eof())
    {
        ...
        bool ok = arrow::adapters::orc::ORCFileReader::Open(in_stream, arrow::default_memory_pool(), &file_reader).ok();
        if (!ok)
            return {};
...
void registerInputFormatProcessorORC(FormatFactory & factory)
{
    factory.registerInputFormatProcessor(
        ...
        [](ReadBuffer & buf,
           ...
           const FormatSettings & /* settings */) { return std::make_shared<ORCBlockInputFormat>(buf, sample, context); });
}
