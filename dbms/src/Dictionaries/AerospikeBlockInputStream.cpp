...
namespace DB {
    ...
    namespace {
                ...
                for (const auto idx : ext::range(1, columns->size())) {
                    ...
                    if (as_val_type(&bin_value) == AS_NIL) {
                        ...
                        if (description.types[idx].second) {
                            ...
                            insertValue(column_nullable.getNestedColumn(), description.types[idx].first, &bin_value,
                                name);
        ...
        private:
            ...
            void insertKey(IColumn& column, const ValueType type, const as_key* key, const std::string& name) const {
                switch (type) {
                    ...
                    default:
                        ...
                        throw Exception{"Type mismatch, expected String (UUID), got type id = " + invalid_type +
                                        " for column "
                                        + name,
            ...
            void
            insertValue(IColumn& column, const ValueType type, const as_bin_value* value, const std::string& name) {
                switch (type) {
                    case ValueType::UInt8:
    ...
    void InitializeBatchKey(as_key* new_key, const char* namespace_name, const char* set_name,
        const std::unique_ptr<as_key>& base_key) {
    ...
    Block AerospikeBlockInputStream::readImpl() {
        ...
        as_error err;
        if (aerospike_batch_get(&client, &err, nullptr, &batch, batchReadCallback,
            static_cast<void*>(&recordsHandler)) != AEROSPIKE_OK) {
