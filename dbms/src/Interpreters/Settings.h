...
#define APPLY_FOR_SETTINGS(M) \
    ...
    M(SettingBool, allow_multiple_joins_emulation, true, "Emulate multiple joins using subselects.") \
    M(SettingBool, multiple_joins_omit_unused_columns, false, "Do not to extract unused columns from joined tables.") \
    M(SettingBool, allow_cross_to_inner_join_conversion, true, "Convert CROSS JOIN to INNER JOIN if possible.") \
