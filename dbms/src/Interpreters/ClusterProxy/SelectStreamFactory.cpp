...
void SelectStreamFactory::createForShard(const Cluster::ShardInfo & shard_info, BlockInputStreams & result, ShardQueries & multiplexed_shards)
{
    ...
    const bool tableFoundLocally = main_table_storage == nullptr;
    if (!tableFoundLocally) /// Table is absent on a local server.
    {
        ...
        return create_local_stream();  /// Let it fail the usual way.
    }

    const auto * replicated_storage = dynamic_cast<const StorageReplicatedMergeTree *>(main_table_storage.get());
    const bool tableIsReplicated = replicated_storage != nullptr;
    if (!tableIsReplicated)
    {
        /// Table is not replicated, use local server.
        return create_local_stream();
    }

    const UInt64 max_allowed_delay = settings.max_replica_delay_for_distributed_queries;
    const UInt32 local_delay = replicated_storage->getAbsoluteDelay();
    const bool tableDelayIsTolerable = !max_allowed_delay || local_delay < max_allowed_delay;
    if (tableDelayIsTolerable)
    {
        return create_local_stream();
    }
    ...
    auto lazily_create_stream = [
            pool = shard_info.pool, shard_num = shard_info.shard_num, modified_query, header = header, modified_query_ast, context = context, throttler = throttler,
