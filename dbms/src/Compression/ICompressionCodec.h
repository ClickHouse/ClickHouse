...
public:
    ...
    virtual std::vector<uint8_t> getHeader() const = 0;
    /// Maximum amount of bytes for compression needed
    virtual size_t getCompressedSize(size_t uncompressed_size) const = 0;
    ...
    virtual bool equals(const ICodec & rhs) const = 0;
