...
struct ConnectionTimeouts
{
    ...
    Poco::Timespan keep_alive_timeout;
    ...
    ConnectionTimeouts(const Poco::Timespan & connection_timeout_,
                       ...
                       const Poco::Timespan & keep_alive_timeout_)
    ...
    ConnectionTimeouts getSaturated(const Poco::Timespan & limit) const
    {
        return ConnectionTimeouts(saturate(connection_timeout, limit),
                                  ...
                                  saturate(keep_alive_timeout, limit));
    }
    ...
    {
        return ConnectionTimeouts(settings.connect_timeout, settings.send_timeout, settings.receive_timeout, settings.keep_alive_timeout);
    }
    ...
    {
        return ConnectionTimeouts(settings.connect_timeout_with_failover_ms, settings.send_timeout, settings.receive_timeout, settings.keep_alive_timeout);
    }

    static ConnectionTimeouts getHTTPTimeouts(const Settings & settings)
    {
        return ConnectionTimeouts(settings.http_connection_timeout, settings.http_send_timeout, settings.http_receive_timeout, settings.keep_alive_timeout);
    }
};
