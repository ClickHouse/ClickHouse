...
public:
    SharedReadBuffer(
            ReadBuffer& in_, const FormatSettings & format_settings_, std::shared_ptr<std::mutex>& mutex_, ChunkCreator getChunk_, size_t min_size_ = 4)
...
private:
    ...
    bool nextImpl() override
    {
        ...
        // TODO create some global variable for min_size
        if (in.eof())
            return false;
        bool res = getChunk(format_settings, in, memory, 4);
        if (!res)
            return false;
