...
namespace
{
    ...
    public:
        SingleEndpointHTTPSessionPool(std::string host_, UInt16 port_, bool https_, size_t max_pool_size_)
            : Base(max_pool_size_, &Poco::Logger::get("HTTPSessionPool")), host(std::move(host_)), port(port_), https(https_)
        {
        }
    };
