...
public:
    ...
    void executeImpl(Block & block, const ColumnNumbers & arguments, size_t result, size_t input_rows_count) override
    {
        ...
        if (has_column_string && has_column_fixed_string)
            Impl::template vector<true, true>(std::move(pattern), data, offsets, fixed_string_N, col_res->getChars(), col_res->getOffsets(), input_rows_count);
        else if (!has_column_string && has_column_fixed_string)
            Impl::template vector<false, true>(std::move(pattern), data, offsets, fixed_string_N, col_res->getChars(), col_res->getOffsets(), input_rows_count);
        else if (has_column_string && !has_column_fixed_string)
            Impl::template vector<true, false>(std::move(pattern), data, offsets, fixed_string_N, col_res->getChars(), col_res->getOffsets(), input_rows_count);
        else
            Impl::template vector<true, true>(std::move(pattern), data, offsets, fixed_string_N, col_res->getChars(), col_res->getOffsets(), input_rows_count);
