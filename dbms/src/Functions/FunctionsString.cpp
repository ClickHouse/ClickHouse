...
template <char not_case_lower_bound, char not_case_upper_bound,
	int to_case(int), void cyrillic_to_case(const UInt8 * &, const UInt8 *, UInt8 * &)>
	void LowerUpperUTF8Impl<not_case_lower_bound, not_case_upper_bound, to_case, cyrillic_to_case>::array(const UInt8 * src, const UInt8 * src_end, UInt8 * dst)
	{
#if __SSE2__
		const auto bytes_sse = sizeof(__m128i);
...
void ReverseImpl::vector(const ColumnString::Chars_t& data, const IColumn::Offsets_t& offsets, ColumnString::Chars_t& res_data, IColumn::Offsets_t& res_offsets)
	{
		res_data.resize(data.size());
		res_offsets.assign(offsets);
		size_t size = offsets.size();
...
void ReverseUTF8Impl::vector(const ColumnString::Chars_t& data, const IColumn::Offsets_t& offsets, ColumnString::Chars_t& res_data, IColumn::Offsets_t& res_offsets)
	{
		res_data.resize(data.size());
		res_offsets.assign(offsets);
		size_t size = offsets.size();
...
void SubstringImpl::vector(const ColumnString::Chars_t& data, const IColumn::Offsets_t& offsets, size_t start, size_t length, ColumnString::Chars_t& res_data, IColumn::Offsets_t& res_offsets)
	{
...
void SubstringImpl::vector_fixed(const ColumnString::Chars_t& data, size_t n, size_t start, size_t length, ColumnString::Chars_t& res_data, IColumn::Offsets_t& res_offsets)
	{
...
void SubstringUTF8Impl::vector(const ColumnString::Chars_t& data, const IColumn::Offsets_t& offsets, size_t start, size_t length, ColumnString::Chars_t& res_data, IColumn::Offsets_t& res_offsets)
	{
		res_data.reserve(data.size());
		size_t size = offsets.size();
		res_offsets.resize(size);
...
void SubstringUTF8Impl::vector_fixed(const ColumnString::Chars_t& data, IColumn::Offset_t n, size_t start, size_t length, ColumnString::Chars_t& res_data, IColumn::Offsets_t& res_offsets)
	{
...
void SubstringUTF8Impl::constant(const std::__cxx11::string& data, size_t start, size_t length, std::__cxx11::string& res_data)
	...
	FunctionPtr FunctionStringOrArrayToT<Impl, Name, ResultType>::create(const Context& context){ return std::make_shared<FunctionStringOrArrayToT>(); }
	template <typename Impl, typename Name, typename ResultType>
