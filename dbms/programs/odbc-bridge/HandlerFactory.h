...
/** Factory for '/ping', '/', '/columns_info', '/identifier_quote' handlers.
  ...
  */
class HandlerFactory : public Poco::Net::HTTPRequestHandlerFactory, WithLogger<>
{
public:
    HandlerFactory(const std::string & name_, size_t keep_alive_timeout_, std::shared_ptr<Context> context_)
        : WithLogger(name_), name(name_), keep_alive_timeout(keep_alive_timeout_), context(context_)
    {
