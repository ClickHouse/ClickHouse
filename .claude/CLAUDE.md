...
When a source-code change requires an update to reference documentation, inspect the target `.mdx` file before editing it, including pages under `docs/reference/` and localized `docs/{locale}/reference/` trees. If the relevant content is between `AUTOGENERATED_START` and `AUTOGENERATED_END`, do not edit the generated region. Update the structured documentation in the defining source registration instead, such as `FunctionDocumentation` or a `DECLARE` doc string, and do not update generated localized output directly. The documentation generator owns the `.mdx` output; direct changes to generated regions are reserved for the dedicated documentation autogeneration workflow and PRs which make changes to these sections without the `pr-autogenerated-docs` label will fail the docs check.
