...
When a change alters the bytes on the wire of the `Native` format or the native TCP protocol - a new packet or handshake field, a protocol revision bump, or a new or changed column encoding - update the matching spec under `docs/reference/interfaces/specs/` in the same change. Refactors and fixes that keep the wire layout do not need a spec update.
