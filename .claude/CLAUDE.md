...
When writing C++ code, always prefer ClickHouse types over their standard-library or fixed-width equivalents: use `UInt64` instead of `uint64_t`, `UInt8` instead of `uint8_t`, `Int32` instead of `int32_t`, and so on. Apply the same rule to header includes: include the ClickHouse header that provides these types (e.g. `<base/types.h>`) instead of the corresponding standard header (e.g. `<cstdint>`). This rule is about the fixed-width integer aliases only. Do not replace a standard type when: it is `std::byte` used for byte-oriented raw storage; a broader-aliasing type (such as `std::byte` or `unsigned char`) is used deliberately for its aliasing behavior; or the type is dictated by an existing or external API. In particular, `UInt8` is `char8_t`, not a byte type, so rewriting `std::byte` or `uint8_t` to `UInt8` breaks raw buffers (e.g. `src/Common/HashTable/HashTable.h`, `src/Compression/CompressionCodecFPC.cpp`, `src/Storages/MergeTree/BitpackingBlockCodec.h`) and external-ABI code (e.g. the OpenSSL `unsigned char *` buffers in `src/Common/OpenSSLHelpers.cpp`). Leave those as-is.
