...
//
// Usage:
//   node server.mjs [--repo <path>] [--base <ref>] [--port 3000] [--out <file>] [--no-open]
//
...
// ── Collect changed files (worktree vs BASE, plus untracked) ────────────────
function collectFiles() {
  const entries = []; // { path, oldPath, status }
  const tokens = gitOrDie(['diff', '--name-status', '-z', BASE])
    .toString('utf8')
    .split('\0')
    .filter((t) => t.length > 0);
  ...
  for (const e of entries) {
    ...
    if (e.status !== 'D') {
      try {
        newBuf = readFileSync(join(ROOT, e.path));
      } catch {
...
const server = createServer((req, res) => {
  ...
  if (req.method === 'GET' && (url.pathname === '/' || url.pathname === '/index.html')) {
    ...
    res.end(readFileSync(join(__dirname, 'ui.html')));
