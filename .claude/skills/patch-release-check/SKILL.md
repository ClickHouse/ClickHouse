...
cadence (~2–2.5 weeks) **and** its branch has unreleased commits (`ahead_by > 0`).
Distinguish that from **due** — a version that just hit its cadence boundary; once the
pipeline is unblocked, automation will release it, so it is not "missing" yet. The
script's `⚠️ MISSING` flag encodes this.
...
**`RUNNER`** — run is `cancelled` with an empty `steps` list and no `runner_name`; it
sat queued ~24h and was cancelled the instant the next day's cron fired. Means no
`[self-hosted, release-maker]` runner picked it up. This is **infra, not a repo
change** — escalate in `#core-ci-info` and check the org/repo Actions runner settings.
