...
- English documentation updates in `/docs` belong in the Mintlify `.mdx` files. For reference documentation under `/docs/reference` or localized `/docs/{locale}/reference` trees, inspect the target file for `{/*AUTOGENERATED_START*/}` / `{/*AUTOGENERATED_END*/}` or the older `<!--AUTOGENERATED_START-->` / `<!--AUTOGENERATED_END-->` form. Treat content between those markers as read-only output: require the change in the defining source registration instead of proposing or accepting a direct `.mdx` edit. Direct generated-region changes are expected only in the dedicated documentation autogeneration PR carrying the `pr-autogenerated-docs` label.
