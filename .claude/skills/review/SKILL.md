...
2) **Impacted surface**
   ...
   - The documented surface is part of the impacted surface. When the diff restricts previously-accepted input or flips a user-visible default, the capability-restriction trigger below fires and the **Documented surface** output section becomes mandatory.
...
- **Capability restriction or user-visible default flip:** fires when the diff makes previously-accepted input rejected (a new exception or refusal path, new validation on existing syntax), removes or gates an existing mechanism behind a setting, or changes a user-visible default — including when the PR carries the `Backward Incompatible Change` category. Enumerate every user-visible identifier whose acceptance, semantics, or default changes: settings, SQL syntax elements (function names, clauses, parameters such as `extra_credentials(role_arn = ...)`), table-function/engine arguments, named-collection keys, credential mechanisms, URL schemes, formats. For each identifier, grep `docs/en/` in this repository and search the Cloud documentation (`gh search code --repo ClickHouse/clickhouse-docs '<identifier>'`; the published site is `https://clickhouse.com/docs`). Read each hit and decide whether the documented flow still works after the change; record the result in the mandatory **Documented surface** output section. A documented flow that the change rejects or breaks is a Blocker unless the same PR updates that documentation and the PR description names the flow and its migration path. This holds even when the restriction is intentional and security-motivated: the finding is the unacknowledged contradiction with the documentation, not the restriction itself. A well-written changelog entry describing the new behavior does not close this gate — the changelog describes what the PR does; this gate checks what the existing docs promise. A pre-existing test that the PR rewrites to assert the new refusal is a signal to look harder, not evidence of coverage: the old assertion was the documented contract. (Motivating failure: PR #106855 restricted `role_arn`-based S3 access, which `docs/en/sql-reference/table-functions/s3.md` documents as the ClickHouse Cloud "Secure S3" flow; no reviewer cross-checked the docs, the backports broke customers, and the only remediation was a downgrade.)
...
- Restricting, rejecting, or removing behavior that `docs/` or the ClickHouse Cloud documentation promises, without the same PR updating those docs and naming the affected flow and its migration path in the description.
...
**Omit any section entirely if there is nothing notable to report in it** — do not include a section just to say "looks good" or "no concerns". The only mandatory sections are Summary and Final Verdict, plus Documented surface whenever the capability-restriction trigger fired.
...
**Documented surface** (mandatory whenever the capability-restriction trigger fired; omit otherwise)
- One line per restricted identifier: the identifier, the `docs/en/` and Cloud-docs hits found for it, and a verdict per hit — still valid, or broken by this change. State explicitly when a grep found no hits. An identifier without a stated grep result means the gate was not run — do not approve in that state.
...
- Approve only if there are no unresolved contract violations, no unresolved high-impact plausible risks, no missing evidence for material claims, and — when the capability-restriction trigger fired — a complete Documented surface section with no broken documented flow left unacknowledged. A `Performance Improvement` without performance evidence, or a `Bug Fix` without regression evidence or a clear exception, should be **⚠️ Request changes**. If not approving, list the **minimum** required actions.
