...
- English documentation updates in `/docs` belong in the Mintlify `.mdx` files. For reference documentation under `/docs/reference`, inspect the target file for `{/*AUTOGENERATED_START*/}` / `{/*AUTOGENERATED_END*/}` or legacy HTML-comment equivalents. Treat content between those markers as read-only output: require the change in the defining source registration instead of proposing or accepting a direct `.mdx` edit. Direct generated-region changes are expected only in the dedicated documentation autogeneration PR carrying the `pr-autogenerated-docs` label.
