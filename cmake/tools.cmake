...
if (NOT LINKER_NAME)
    ...
    elseif (COMPILER_CLANG)
        ...
        find_program (LD64_PATH NAMES "ld64.lld" "ld64") # Mach-O linker
...
if (LINKER_NAME)
    if (COMPILER_CLANG)
        find_program (LINKER_FOUND NAMES ${LINKER_NAME})
        if (NOT LINKER_FOUND)
            message (FATAL_ERROR "Using linker ${LINKER_NAME} but can't find its path.")
        endif ()
        if (LINKER_NAME MATCHES "lld" AND NOT LINKER_NAME MATCHES "ld64") # Default
            if (CMAKE_BUILD_TYPE_UC STREQUAL "RELEASE" OR CMAKE_BUILD_TYPE_UC STREQUAL "RELWITHDEBINFO")
                ...
                set (CMAKE_EXE_LINKER_FLAGS "${CMAKE_EXE_LINKER_FLAGS} --ld-path=${LINKER_FOUND}")
                set (CMAKE_SHARED_LINKER_FLAGS "${CMAKE_SHARED_LINKER_FLAGS} --ld-path=${LINKER_FOUND}")
            endif ()
        elseif (LINKER_NAME MATCHES "ld64") # ld64 for Darwin experimental support
            set (CMAKE_EXE_LINKER_FLAGS "${CMAKE_EXE_LINKER_FLAGS} --ld-path=${LINKER_FOUND}")
            set (CMAKE_SHARED_LINKER_FLAGS "${CMAKE_SHARED_LINKER_FLAGS} --ld-path=${LINKER_FOUND}")
        else ()
            message (FATAL_ERROR "Using linker ${LINKER_NAME} is unsupported for clang.")
        endif ()
    else ()
