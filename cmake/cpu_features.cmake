...
if (ARCH_AARCH64)
    ...
    # When building natively on AArch64 Linux, auto-detect CPU features from /proc/cpuinfo and
    # construct precise compiler flags. For cross-compilation or manual override, fall back to the
    # two fixed profiles selected by NO_ARMV81_OR_HIGHER.
    if (OS_LINUX AND CMAKE_HOST_SYSTEM_PROCESSOR MATCHES "^(aarch64.*|AARCH64.*|arm64.*|ARM64.*)" AND NOT NO_ARMV81_OR_HIGHER)
        # CPU features in /proc/cpuinfo and compiler flags don't align :( ... detect individual
        # features and map them to compiler flags. Full Graviton 3 /proc/cpuinfo is
        ...
        execute_process(
            COMMAND grep -m1 "^Features" /proc/cpuinfo
            OUTPUT_VARIABLE _CPU_FEATURES
            ERROR_QUIET
            OUTPUT_STRIP_TRAILING_WHITESPACE)
        # Pad with spaces for reliable word-boundary matching.
        set(_FEAT " ${_CPU_FEATURES} ")

        # CRC is always enabled — the codebase uses __crc32* intrinsics unconditionally, and even
        # very old ARM CPUs support it.
        set(_MARCH_FLAGS "+crc")
        set(_RUST_TARGET_FEATURES "+crc")

        # NEON (Advanced SIMD) — always enabled on AArch64, required by the ring crate.
        set(_MARCH_FLAGS "${_MARCH_FLAGS}+simd")

        # Crypto extensions.
        if (_FEAT MATCHES " aes ")
            set(_MARCH_FLAGS "${_MARCH_FLAGS}+aes")
            set(_RUST_TARGET_FEATURES "${_RUST_TARGET_FEATURES},+aes")
        endif()
        if (_FEAT MATCHES " sha2 ")
            set(_MARCH_FLAGS "${_MARCH_FLAGS}+sha2")
            set(_RUST_TARGET_FEATURES "${_RUST_TARGET_FEATURES},+sha2")
        endif()
        if (_FEAT MATCHES " sha3 ")
            set(_MARCH_FLAGS "${_MARCH_FLAGS}+sha3")
            set(_RUST_TARGET_FEATURES "${_RUST_TARGET_FEATURES},+sha3")
        endif()

        # ARMv8.1: LSE atomics (no stable Rust target_feature).
        if (_FEAT MATCHES " atomics ")
            set(_MARCH_FLAGS "${_MARCH_FLAGS}+lse")
        endif()

        # ARMv8.2: dot product, RCpc atomics.
        if (_FEAT MATCHES " asimddp ")
            set(_MARCH_FLAGS "${_MARCH_FLAGS}+dotprod")
            set(_RUST_TARGET_FEATURES "${_RUST_TARGET_FEATURES},+dotprod")
        endif()
        if (_FEAT MATCHES " lrcpc ")
            set(_MARCH_FLAGS "${_MARCH_FLAGS}+rcpc")
            set(_RUST_TARGET_FEATURES "${_RUST_TARGET_FEATURES},+rcpc")
        endif()

        # Speculative Store Bypass Safe.
        if (_FEAT MATCHES " ssbs ")
            set(_MARCH_FLAGS "${_MARCH_FLAGS}+ssbs")
            set(_RUST_TARGET_FEATURES "${_RUST_TARGET_FEATURES},+ssbs")
        endif()

        # BFloat16.
        if (_FEAT MATCHES " bf16 ")
            set(_MARCH_FLAGS "${_MARCH_FLAGS}+bf16")
            set(_RUST_TARGET_FEATURES "${_RUST_TARGET_FEATURES},+bf16")
        endif()

        set (COMPILER_FLAGS "${COMPILER_FLAGS} -march=armv8-a${_MARCH_FLAGS}")
        list(APPEND RUSTFLAGS_CPU "-C" "target_feature=${_RUST_TARGET_FEATURES}")
        message(STATUS "Detected ARM CPU features, using -march=armv8-a${_MARCH_FLAGS}")
    else ()
        # Cross-compilation or manual override: use fixed profiles selected by NO_ARMV81_OR_HIGHER.
